...
always_inline uword handoffdemo_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int which, int is_trace)
{
    ...
    if (which == 1) {
        for (i = 0; i < frame->n_vectors; i++) {
            ...
            if (is_trace && (b[0]->flags & VLIB_BUFFER_IS_TRACED)) {
                handoffdemo_trace_t *t = vlib_add_trace(vm, node, b[0], sizeof(*t));
        ...
        n_enq = vlib_buffer_enqueue_to_thread(vm, node, hmp->frame_queue_index, from, thread_indices, frame->n_vectors, 1 /* drop on congestion */);
        if (n_enq < frame->n_vectors) vlib_node_increment_counter(vm, node->node_index, HANDOFFDEMO_ERROR_CONGESTION_DROP, frame->n_vectors - n_enq);
        vlib_node_increment_counter(vm, node->node_index, HANDOFFDEMO_ERROR_HANDED_OFF, n_enq);
...
static uword handoffdemo_node_1_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE))
        return handoffdemo_inline(vm, node, frame, 1 /* which */, 1 /* is_trace */);
    else
        return handoffdemo_inline(vm, node, frame, 1 /* which */, 0 /* is_trace */);
}
...
{
    if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE))
        return handoffdemo_inline(vm, node, frame, 2 /* which */, 1 /* is_trace */);
    else
        return handoffdemo_inline(vm, node, frame, 2 /* which */, 0 /* is_trace */);
}
...
static clib_error_t *handoffdemo_node_init(vlib_main_t *vm)
{
    ...
    hmp->frame_queue_index = vlib_frame_queue_main_init(handoffdemo_node_2.index, 16);
