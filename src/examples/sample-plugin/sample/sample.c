...
int sample_macswap_enable_disable(sample_main_t* sm, u32 sw_if_index, int enable_disable)
{
    ...
    /* Utterly wrong? */
    if (pool_is_free_index(sm->vnet_main->interface_main.sw_interfaces, sw_if_index)) return VNET_API_ERROR_INVALID_SW_IF_INDEX;

    /* Not a physical port? */
    sw = vnet_get_sw_interface(sm->vnet_main, sw_if_index);
    if (sw->type != VNET_SW_INTERFACE_TYPE_HARDWARE) return VNET_API_ERROR_INVALID_SW_IF_INDEX;

    vnet_feature_enable_disable("device-input", "sample", sw_if_index, enable_disable, 0, 0);
...
static clib_error_t* macswap_enable_disable_command_fn(vlib_main_t* vm, unformat_input_t* input, vlib_cli_command_t* cmd)
{
    ...
    switch (rv) {
        ...
        default:
            return clib_error_return(0, "sample_macswap_enable_disable returned %d", rv);
    }
    return 0;
}
...
static void vl_api_sample_macswap_enable_disable_t_handler(vl_api_sample_macswap_enable_disable_t* mp)
{
    ...
    rv = sample_macswap_enable_disable(sm, ntohl(mp->sw_if_index), (int) (mp->enable_disable));
