...
always_inline u32 choose_msg_size(mc_test_main_t *tm)
{
    u32 r = tm->min_n_msg_bytes;
    if (tm->max_n_msg_bytes > tm->min_n_msg_bytes) r += random_u32(&tm->seed) % (1 + tm->max_n_msg_bytes - tm->min_n_msg_bytes);
    return r;
}
...
static void unserialize_test_msg(serialize_main_t *m, va_list *va)
{
    ...
    if (dump_msg) clib_warning("got %d bytes, %U", n_bytes, format_hex_bytes, p, n_bytes);
...
static void test_rx_callback(mc_main_t *mcm, mc_stream_t *stream, mc_peer_id_t peer_id, u32 buffer_index)
{
    if (SERIALIZE) {
        ...
        fformat(stdout, "RX from %U %U\n", stream->transport->format_peer_id, peer_id, format_hex_bytes, dp, tm->n_msg_bytes);

#endif
    }
}

static u8 *test_snapshot_callback(mc_main_t *mcm, u8 *data_vector, u32 last_global_sequence_processed)
{
    if (SERIALIZE) {
        serialize_main_t m;
        ...
        return format(data_vector, "snapshot, last global seq 0x%x", last_global_sequence_processed);
}

static void test_handle_snapshot_callback(mc_main_t *mcm, u8 *data, u32 n_data_bytes)
{
    if (SERIALIZE) {
        serialize_main_t s;
...
static uword mc_test_process(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *f)
{
    mc_test_main_t    *tm  = &mc_test_main;
    ...
    while (1) {
        ...
        switch (event_type) {
            ...
            case EVENT_SEND_DATA: {
                ...
                if (stream_index == ~0) stream_index = mc_stream_join(mcm, &config);
                ...
                clib_warning("done sending %d; %.4e per sec", event_data[0], (f64) event_data[0] / (times[1] - times[0]));
                break;
            }
...
static clib_error_t *mc_test_command(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    f64 npkts = 10;

    if (unformat(input, "join")) {
        vlib_cli_output(vm, "Join stream...\n");
        vlib_process_signal_event(vm, mc_test_process_node.index, EVENT_JOIN_STREAM, 0);
        return 0;
    }
    else if (unformat(input, "send %f", &npkts) || unformat(input, "send")) {
        vlib_process_signal_event(vm, mc_test_process_node.index, EVENT_SEND_DATA, (uword) npkts);
...
static clib_error_t *mc_show_command(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *mc_clear_command(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    mc_main_t *mcm = &mc_socket_main.mc_main;
...
static clib_error_t *mc_config(vlib_main_t *vm, unformat_input_t *input)
{
    mc_test_main_t   *tm    = &mc_test_main;
    mc_socket_main_t *msm   = &mc_socket_main;
    ...
    if (tm->n_packets_to_send > 0) vlib_process_signal_event(vm, mc_test_process_node.index, EVENT_SEND_DATA, (uword) tm->n_packets_to_send);
