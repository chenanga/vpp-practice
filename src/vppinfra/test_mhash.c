...
static clib_error_t *mhash_next_test(mhash_t *h)
{
    hash_next_t   hn = {0};
    hash_pair_t  *p0, *p1;
    ...
    if (!error) error = CLIB_ERROR_ASSERT(!hash_next(h->hash, &hn));

    return error;
}

static clib_error_t *test_word_key(mhash_test_t *ht)
{
    mhash_t _h = {0}, *h = &_h;
    word    i, j;
    ...
    for (i = 0; i < ht->n_iterations; i++) {
        ...
        if ((error = hash_validate(h->hash))) goto done;
    ...
    if ((error = mhash_next_test(h))) goto done;
...
static u8 *test2_format(u8 *s, va_list *args)
{
    ...
    return format(s, "0x%8U <- %U", format_hex_bytes, &p->value[0], hash_value_bytes(h), format_mhash_key, mh, (u32) p->key);
}

static clib_error_t *test_string_key(mhash_test_t *ht, uword is_c_string)
{
    mhash_t _h = {0}, *h = &_h;
    word    i, j;
    ...
    for (i = 0; i < vec_len(keys); i++) {
        ...
        if (is_c_string) vec_terminate_c_string(keys[i]);
...
int test_mhash_main(unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (0 == unformat(input, "iter %d", &ht->n_iterations) && 0 == unformat(input, "print %d", &ht->n_iterations_per_print)
            && 0 == unformat(input, "elts %d", &ht->n_pairs) && 0 == unformat(input, "seed %d", &ht->seed)
            && 0 == unformat(input, "verbose %=", &ht->verbose, 1) && 0 == unformat(input, "valid %d", &ht->n_iterations_per_validate)) {
