...
static clib_mem_page_sz_t legacy_get_log2_default_hugepage_size(void)
{
    ...
    if ((fp = fopen("/proc/meminfo", "r")) == NULL) return CLIB_MEM_PAGE_SZ_UNKNOWN;

    while (fscanf(fp, "%32s", tmp) > 0)
        if (strncmp("Hugepagesize:", tmp, 13) == 0) {
            u32 size;
            if (fscanf(fp, "%u", &size) > 0) log2_page_size = 10 + min_log2(size);
            break;
        }
...
void clib_mem_main_init(void)
{
    ...
    if (mm->log2_page_sz != CLIB_MEM_PAGE_SZ_UNKNOWN) return;
    ...
    /* numa nodes */
    if (syscall(__NR_get_mempolicy, &mode, &nodemask, maxnode, va, flags) == 0) mm->numa_node_bitmap = nodemask;
}
...
__clib_export void clib_mem_vm_randomize_va(uword *requested_va, clib_mem_page_sz_t log2_page_size)
{
    ...
    *requested_va += (clib_cpu_time_now() & bit_mask) * (1ull << log2_page_size);
}
...
__clib_export int clib_mem_vm_create_fd(clib_mem_page_sz_t log2_page_size, char *fmt, ...)
{
    ...
    if (vec_len(s) > 249) vec_set_len(s, 249);
    vec_add1(s, 0);
    ...
    if (fd == -1 && errno == EINVAL && log2_page_size == CLIB_MEM_PAGE_SZ_DEFAULT_HUGE) {
    ...
    if ((memfd_flags & MFD_ALLOW_SEALING) && ((fcntl(fd, F_ADD_SEALS, F_SEAL_SHRINK)) == -1)) {
        vec_reset_length(mm->error);
...
uword clib_mem_vm_reserve(uword start, uword size, clib_mem_page_sz_t log2_page_sz)
{
    ...
    if (start) {
        ...
        if (start & pow2_mask(log2_page_sz)) return ~0;

        base = (void *) start - sys_page_sz;
        base = mmap(base, size + sys_page_sz, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_FIXED_NOREPLACE, -1, 0);
    ...
    base = mmap(0, size + pagesize, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
...
__clib_export clib_mem_vm_map_hdr_t *clib_mem_vm_get_next_map_hdr(clib_mem_vm_map_hdr_t *hdr)
{
    ...
    if (hdr == 0) {
        hdr = mm->first_map;
        if (hdr) mprotect(hdr, sys_page_sz, PROT_READ);
        return hdr;
    }
    next = hdr->next;
    mprotect(hdr, sys_page_sz, PROT_NONE);
    if (next) mprotect(next, sys_page_sz, PROT_READ);
    return next;
}
...
void *clib_mem_vm_map_internal(void *base, clib_mem_page_sz_t log2_page_sz, uword size, int fd, uword offset, char *name)
{
    ...
    if (fd != -1) {
        ...
        if (log2_page_sz > mm->log2_page_sz) is_huge = 1;
    }
    else {
        mmap_flags |= MAP_PRIVATE | MAP_ANONYMOUS;

        if (log2_page_sz == mm->log2_page_sz) log2_page_sz = CLIB_MEM_PAGE_SZ_DEFAULT;
    ...
    if (log2_page_sz == CLIB_MEM_PAGE_SZ_UNKNOWN) return CLIB_MEM_VM_MAP_FAILED;

    size = round_pow2(size, 1ULL << log2_page_sz);

    base = (void *) clib_mem_vm_reserve((uword) base, size, log2_page_sz);

    if (base == (void *) ~0) return CLIB_MEM_VM_MAP_FAILED;

    base = mmap(base, size, PROT_READ | PROT_WRITE, mmap_flags, fd, offset);

    if (base == MAP_FAILED) return CLIB_MEM_VM_MAP_FAILED;
    ...
    hdr = mmap(base - sys_page_sz, sys_page_sz, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE | MAP_FIXED, -1, 0);
...
__clib_export int clib_mem_vm_unmap(void *base)
{
    ...
    map_lock();
    if (mprotect(hdr, sys_page_sz, PROT_READ | PROT_WRITE) != 0) goto out;

    size = hdr->num_pages << hdr->log2_page_sz;
    if (munmap((void *) hdr->base_addr, size) != 0) goto out;
    ...
    if (munmap(hdr, sys_page_sz) != 0) return CLIB_MEM_ERROR;
...
__clib_export void clib_mem_get_page_stats(void *start, clib_mem_page_sz_t log2_page_size, uword n_pages, clib_mem_page_stats_t *stats)
{
    ...
    for (i = 0; i < n_pages; i++) {
        ...
        if (status[i] == -ENOENT && syscall(__NR_mincore, ptr[i], 1, &incore) == 0 && (incore & 1) != 0) {
            if (syscall(__NR_get_mempolicy, &status[i], 0, 0, ptr[i], MPOL_F_NODE | MPOL_F_ADDR) != 0) {
...
__clib_export u64 *clib_mem_vm_get_paddr(void *mem, clib_mem_page_sz_t log2_page_size, int n_pages)
{
    ...
    if ((fd = open((char *) "/proc/self/pagemap", O_RDONLY)) == -1) return 0;

    for (i = 0; i < n_pages; i++) {
        u64   seek, pagemap = 0;
        uword vaddr = pointer_to_uword(mem) + (((u64) i) << log2_page_size);
        seek        = ((u64) vaddr / pagesize) * sizeof(u64);
        if (lseek(fd, seek, SEEK_SET) != seek) goto done;

        if (read(fd, &pagemap, sizeof(pagemap)) != (sizeof(pagemap))) goto done;

        if ((pagemap & (1ULL << 63)) == 0) goto done;
...
__clib_export int clib_mem_set_numa_affinity(u8 numa_node, int force)
{
    ...
    if (mm->numa_node_bitmap == 0) {
        if (numa_node) {
            vec_reset_length(mm->error);
            mm->error = clib_error_return(mm->error, "%s: numa not supported", (char *) __func__);
            return CLIB_MEM_ERROR;
        }
        else
            return 0;
    ...
    rv = syscall(__NR_set_mempolicy, force ? MPOL_BIND : MPOL_PREFERRED, bmp, vec_len(bmp) * sizeof(bmp[0]) * 8 + 1);
