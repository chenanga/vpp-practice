...
__clib_export clib_error_t *clib_sysfs_write(char *file_name, char *fmt, ...)
{
    ...
    if (write(fd, s, vec_len(s)) < 0) error = clib_error_return_unix(0, "write `%s'", file_name);
...
__clib_export clib_error_t *clib_sysfs_read(char *file_name, char *fmt, ...)
{
    ...
    if (result == 0) return clib_error_return(0, "unformat error");
...
clib_error_t *clib_sysfs_set_nr_hugepages(int numa_node, int log2_page_size, int nr)
{
    ...
    if (log2_page_size == 0) log2_page_size = min_log2(clib_mem_get_default_hugepage_size());
...
static clib_error_t *clib_sysfs_get_xxx_hugepages(char *type, int numa_node, int log2_page_size, int *val)
{
    ...
    vec_dec_len(p, 1);
    p     = format(p, "/hugepages/hugepages-%ukB/%s_hugepages%c", page_size, type, 0);
...
{
    return clib_sysfs_get_xxx_hugepages("surplus", numa_node, log2_page_size, v);
}
...
clib_error_t *clib_sysfs_prealloc_hugepages(int numa_node, int log2_page_size, int nr)
{
    ...
    if (error) return error;
    clib_warning("pre-allocating %u additional %uK hugepages on numa node %u", needed, page_size, numa_node);
    return clib_sysfs_set_nr_hugepages(numa_node, log2_page_size, n + needed);
}

__clib_export clib_bitmap_t *clib_sysfs_read_bitmap(char *fmt, ...)
{
    FILE   *fp;
    uword  *r = 0;
    va_list va;
    ...
    if (fp != NULL) {
        ...
        if (fgets((char *) buffer, 256, fp)) {
            ...
            if (unformat(&in, "%U", unformat_bitmap_list, &r) != 1) clib_warning("unformat_bitmap_list failed");
            unformat_free(&in);
        }
        vec_free(buffer);
        fclose(fp);
