...
__clib_export clib_error_t *elf_get_section_by_name(elf_main_t *em, char *section_name, elf_section_t **result)
{
    ...
    if (!p) return clib_error_return(0, "no such section `%s'", section_name);
...
__clib_export clib_error_t *elf_get_section_by_start_address(elf_main_t *em, uword start_address, elf_section_t **result)
{
    ...
    if (!s) return clib_error_return(0, "no section with address 0x%wx", start_address);
    *result = s;
    return 0;
}
...
static u8 *format_elf_section(u8 *s, va_list *args)
{
    ...
    if (!h) return format(s, "%=40s%=10s%=20s%=8s%=16s%=16s%=16s", "Name", "Index", "Type", "Size", "Align", "Address", "File offset");
...
static u8 *format_elf_segment(u8 *s, va_list *args)
{
    ...
    if (!h) return format(s, "%=16s%=16s%=16s%=16s", "Type", "Virt. Address", "Phys. Address", "Size");
...
static u8 *format_elf_symbol_section_name(u8 *s, va_list *args)
{
    ...
    if (si >= ELF_SYMBOL_SECTION_RESERVED_LO && si <= ELF_SYMBOL_SECTION_RESERVED_HI) {
        switch (si) {
#define _(f, n) \
    case n:     \
        t = #f; \
...
u8 *format_elf_symbol(u8 *s, va_list *args)
{
    ...
    if (!sym) return format(s, "%=32s%=16s%=16s%=16s%=16s%=16s", "Symbol", "Size", "Value", "Type", "Visibility", "Section");
...
#define _(f, i) [i] = #f,
        ...
        case ELF_ARCH_X86_64: {
            static char *tab[] = {foreach_elf_x86_64_relocation_type};

#undef _
            if (type < ARRAY_LEN(tab)) t = tab[type];
            break;
        }
...
static u8 *format_elf_relocation(u8 *s, va_list *args)
{
    ...
    if (!r) return format(s, "%=16s%=16s%=16s", "Address", "Type", "Symbol");

    t   = vec_elt_at_index(em->symbol_tables, 0);
    sym = vec_elt_at_index(t->symbols, r->symbol_and_type >> 32);

    s = format(s, "%16Lx%16U", r->address, format_elf_relocation_type, em, r->symbol_and_type & 0xff);
    ...
    if (sym->name != 0) s = format(s, " %s", elf_symbol_name(t, sym));

    {
        i64 a = r->addend;
        if (a != 0) s = format(s, " %c 0x%Lx", a > 0 ? '+' : '-', a > 0 ? a : -a);
    }
...
static u8 *format_elf_dynamic_entry(u8 *s, va_list *args)
{
    ...
    if (!e) return format(s, "%=40s%=16s", "Type", "Data");
...
static u8 *format_elf_file_type(u8 *s, va_list *args)
{
    ...
    if (a >= ELF_ARCH_SPECIFIC_LO && a <= ELF_ARCH_SPECIFIC_HI) return format(s, "arch-specific 0x%x", a - ELF_ARCH_SPECIFIC_LO);

    if (a >= ELF_OS_SPECIFIC_LO && a <= ELF_OS_SPECIFIC_HI) return format(s, "os-specific 0x%x", a - ELF_OS_SPECIFIC_LO);
...
__clib_export u8 *format_elf_main(u8 *s, va_list *args)
{
    ...
    s = format(s,
               ...
               em->first_header.architecture,
               format_elf_file_type,
               em->first_header.file_type,
               format_elf_file_class,
               em->first_header.file_class,
               format_elf_data_encoding,
               em->first_header.data_encoding,
               format_elf_abi,
               em->first_header.abi,
               em->first_header.abi_version);

    s = format(s, "  entry 0x%Lx, arch-flags 0x%x", em->file_header.entry_point, em->file_header.flags);

    if (em->interpreter) s = format(s, "\n  interpreter: %s", em->interpreter);

    {
        ...
        vec_foreach (h, em->sections)
            if (h->header.type != ~0) vec_add1(copy, h[0]);
        ...
        s = format(s,
                   ...
                   fh->section_header_file_offset + (u64) fh->section_header_count * fh->section_header_size);
...

    {
        ...
        vec_foreach (h, em->segments)
            if (h->header.type != ELF_SEGMENT_UNUSED && h->header.type != ~0) vec_add1(copy, h[0]);
    ...
    if ((verbose & FORMAT_ELF_MAIN_RELOCATIONS) && vec_len(em->relocation_tables) > 0) {
    ...
    if ((verbose & FORMAT_ELF_MAIN_DYNAMIC) && vec_len(em->dynamic_entries) > 0) {
...
#undef _
        ...
        if (s->header.type != ELF_SECTION_NO_BITS) vec_add(s->contents, data + s->header.file_offset, s->header.file_size);
    }

    s = vec_elt_at_index(em->sections, fh->section_header_string_table_index);

    em->section_by_name = hash_create_string(/* # elts */ vec_len(em->sections),
                                             /* sizeof of value */ sizeof(uword));

    vec_foreach (s, em->sections) {
        hash_set_mem(em->section_by_name, elf_section_name(em, s), s - em->sections);
        hash_set(em->section_by_start_address, s->header.exec_address, s - em->sections);
    }
}
...
static void add_symbol_table(elf_main_t *em, elf_section_t *s)
{
    ...
    if (s->header.type == ELF_SECTION_DYNAMIC_SYMBOL_TABLE) em->dynamic_symbol_table_index = vec_len(em->symbol_tables);
...
#undef _
    ...
    if (s->header.link == 0) return;

    tab->string_table   = elf_get_section_contents(em, s->header.link, sizeof(tab->string_table[0]));
    tab->symbol_by_name = hash_create_string(/* # elts */ vec_len(tab->symbols),
                                             /* sizeof of value */ sizeof(uword));

    vec_foreach (sym64, tab->symbols) {
        if (sym64->name != 0) hash_set_mem(tab->symbol_by_name, tab->string_table + sym64->name, sym64 - tab->symbols);
    }
}
...
static void add_relocation_table(elf_main_t *em, elf_section_t *s)
{
    ...
    if (em->first_header.file_class == ELF_64BIT) {
        ...
        rs = elf_get_section_contents(em, t->section_index, sizeof(rs[0]) + has_addend * sizeof(rs->addend[0]));
        ...
        if (em->need_byte_swap) {
            ...
            for (i = 0; i < vec_len(r); i++) {
                ...
                if (has_addend) r->addend[0] = elf_swap_u64(em, r->addend[0]);
        ...
        r32s = elf_get_section_contents(em, t->section_index, sizeof(r32s[0]) + has_addend * sizeof(r32s->addend[0]));
...
__clib_export void elf_set_dynamic_entries(elf_main_t *em)
{
        ...
        vec_foreach (e, em->dynamic_entries) {
            switch (e->type) {
                ...
                case ELF_DYNAMIC_ENTRY_VERSYM: {
                    ...
                    if (es) e->data = es->header.exec_address;
                    break;
                }
    ...
    if (em->first_header.file_class == ELF_64BIT) {
        ...
        if (es != em->dynamic_entries) vec_free(es);
    }
    else {
...
clib_error_t *elf_parse(elf_main_t *em, void *data, uword data_bytes)
{
    ...
    em->first_header   = h[0];
    em->need_byte_swap = CLIB_ARCH_IS_BIG_ENDIAN != (h->data_encoding == ELF_TWOS_COMPLEMENT_BIG_ENDIAN);
...
#undef _
    ...
    /* Figure which sections are contained in each segment. */
    {
        ...
        vec_foreach (g, em->segments) {
            ...
            if (g->header.memory_size == 0) continue;
...
static void byte_swap_verneed(elf_main_t *em, elf_dynamic_version_need_union_t *vus)
{
    ...
    for (i = 0; i < vec_len(vus); i++) {
        ...
        if (clib_bitmap_get(entries_swapped, i)) continue;
...
static void set_dynamic_verneed(elf_main_t *em)
{
    ...
    if (vus != em->verneed) vec_free(vus);
}

static void set_symbol_table(elf_main_t *em, u32 table_index) __attribute__((unused));
...
#undef _
        ...
        elf_set_section_contents(em, tab->section_index, syms, vec_bytes(syms));
    }
    else {
        elf32_symbol_t *syms;
        uword           i;
...
static char *elf_find_interpreter(elf_main_t *em, void *data)
{
    ...
    vec_foreach (g, em->segments) {
        if (g->header.type == ELF_SEGMENT_INTERP) break;
    }

    if (g >= vec_end(em->segments)) return 0;
...
static void *elf_get_section_contents_with_starting_address(elf_main_t *em, uword start_address, uword elt_size, u32 *section_index_result)
{
    ...
    if (section_index_result) *section_index_result = s->index;
        ...
        switch (e->type) {
            case ELF_DYNAMIC_ENTRY_STRING_TABLE:
                ...
                em->dynamic_string_table =
                    elf_get_section_contents_with_starting_address(em, e->data, sizeof(u8), &em->dynamic_string_table_section_index);
                break;
            ...
            case ELF_DYNAMIC_ENTRY_VERSYM:
                em->versym = elf_get_section_contents_with_starting_address(em, e->data, sizeof(em->versym[0]), &em->versym_section_index);
                if (em->need_byte_swap) {
                    uword i;
            ...
            case ELF_DYNAMIC_ENTRY_VERSION_NEED:
                em->verneed = elf_get_section_contents_with_starting_address(em, e->data, sizeof(em->verneed[0]), &em->verneed_section_index);
                if (em->need_byte_swap) byte_swap_verneed(em, em->verneed);
                break;
...
__clib_export clib_error_t *elf_read_file(elf_main_t *em, char *file_name)
{
    ...
    elf_main_free(em);
    if (fd >= 0) close(fd);
    if (data) munmap(data, mmap_length);
    return error;
}
...
static u32 string_table_add_name_index(string_table_builder_t *b, u32 index) __attribute__((unused));
...
static void string_table_init(string_table_builder_t *b, u8 *old_table) __attribute__((unused));
...
static u8 *string_table_done(string_table_builder_t *b) __attribute__((unused));
    ...
    /* Figure file offsets and exec addresses for sections. */
    {
        ...
        vec_foreach (s, em->sections) {
            ...
            switch (s->header.type) {
                ...
                case ELF_SECTION_SYMBOL_TABLE:
                    if (!(s->index == em->dynamic_string_table_section_index || s->index == em->file_header.section_header_string_table_index)) {
            ...
            if (s->header.file_offset == ~0) s->header.file_offset = file_offset;
            ...
            if (s->header.type != ELF_SECTION_NO_BITS) file_offset += align_size;
...
__clib_export clib_error_t *elf_write_file(elf_main_t *em, char *file_name)
{
    ...
    if (fd < 0) return clib_error_return_unix(0, "open `%s'", file_name);
...
#undef _
    ...
    /* Write contents for all sections. */
    {
        ...
        vec_foreach (s, em->sections) {
            if (s->header.file_size == 0) continue;
