...
__clib_export i8 *clib_macro_eval(clib_macro_main_t *mm, i8 *s, i32 complain, u16 level, u16 max_level)
{
    ...
    if (level >= max_level) {
        if (complain) clib_warning("circular definition, level %d", level);
            ...
            case '$':
                ...
                if (!(varvalue = builtin_eval(mm, varname, complain))) {
                    ...
                    if (!varvalue) {
                        ...
                        if (tmp) varvalue = (i8 *) format(0, "%s%c", tmp, 0);
                    }
#ifdef CLIB_UNIX
                    /* Look in environment. */
                    if (!varvalue) {
                ...
                if (varvalue) {
                    ...
                    ts = clib_macro_eval(mm, varvalue, complain, level + 1, max_level);
                    vec_free(varvalue);
                    ...
                    if (complain) clib_warning("Undefined Variable Reference: %s\n", varname);
...
{
    char tmp[128];
    if (gethostname(tmp, sizeof(tmp))) return ((i8 *) format(0, "gethostname-error%c", 0));
...
__clib_export u8 *format_clib_macro_main(u8 *s, va_list *args)
{
    ...
    for (i = 0; i < vec_len(nses); i++) {
        ...
        if (evaluate == 0)
            ...
            u8 *rv = (u8 *) clib_macro_eval_dollar(mm, (i8 *) nses[i].name, 0 /* no complain */);
            s      = format(s, "%s\n", rv);
            vec_free(rv);
        }
    }
