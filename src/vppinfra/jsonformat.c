...
int vl_api_u8_string_fromjson(cJSON *o, u8 *s, int len)
{
    ...
    if (!unformat(&input, "0x%U", unformat_hex_string, s)) return -1;
    return 0;
}
...
u8 *u8string_fromjson(cJSON *o, char *fieldname)
{
    ...
    unformat_init_string(&input, p, strlen(p));
    if (!unformat(&input, "0x%U", unformat_hex_string, &s)) return 0;
    return s;
}
...
int vl_api_ip4_address_t_fromjson(void **mp, int *len, cJSON *o, vl_api_ip4_address_t *a)
{
    unformat_input_t input;
    char            *p = cJSON_GetStringValue(o);
    if (!p) return -1;
    unformat_init_string(&input, p, strlen(p));
    if (!unformat(&input, "%U", unformat_ip4_address, a)) return -1;
    return 0;
}

int vl_api_ip4_prefix_t_fromjson(void **mp, int *len, cJSON *o, vl_api_ip4_prefix_t *a)
{
    unformat_input_t input;
    char            *p = cJSON_GetStringValue(o);
    if (!p) return -1;
    unformat_init_string(&input, p, strlen(p));
    if (!unformat(&input, "%U/%d", unformat_ip4_address, &a->address, &a->len)) return -1;
    return 0;
}

int vl_api_ip4_address_with_prefix_t_fromjson(void **mp, int *len, cJSON *o, vl_api_ip4_prefix_t *a)
{
    return vl_api_ip4_prefix_t_fromjson(mp, len, o, a);
}
int vl_api_ip6_address_t_fromjson(void **mp, int *len, cJSON *o, vl_api_ip6_address_t *a)
{
    unformat_input_t input;
    char            *p = cJSON_GetStringValue(o);
    if (!p) return -1;
    unformat_init_string(&input, p, strlen(p));
    if (!unformat(&input, "%U", unformat_ip6_address, a)) return -1;
    return 0;
}

int vl_api_ip6_prefix_t_fromjson(void **mp, int *len, cJSON *o, vl_api_ip6_prefix_t *a)
{
    unformat_input_t input;
    char            *p = cJSON_GetStringValue(o);
    if (!p) return -1;
    unformat_init_string(&input, p, strlen(p));
    if (!unformat(&input, "%U/%d", unformat_ip6_address, &a->address, &a->len)) return -1;
    return 0;
}

int vl_api_ip6_address_with_prefix_t_fromjson(void **mp, int *len, cJSON *o, vl_api_ip6_prefix_t *a)
{
    return vl_api_ip6_prefix_t_fromjson(mp, len, o, a);
}

int vl_api_address_t_fromjson(void **mp, int *len, cJSON *o, vl_api_address_t *a)
{
    unformat_input_t input;
...
int vl_api_prefix_t_fromjson(void **mp, int *len, cJSON *o, vl_api_prefix_t *a)
{
    unformat_input_t input;
...
int vl_api_address_with_prefix_t_fromjson(void **mp, int *len, cJSON *o, vl_api_prefix_t *a)
{
...
int vl_api_mac_address_t_fromjson(void **mp, int *len, cJSON *o, vl_api_mac_address_t *a)
{
    unformat_input_t input;

    char *p = cJSON_GetStringValue(o);
    unformat_init_string(&input, p, strlen(p));
    if (!unformat(&input, "%U", unformat_mac_address, a)) return -1;
    return 0;
}
...
u8 *format_vl_api_mac_address_t(u8 *s, va_list *args)
{
    const mac_address_t *mac = va_arg(*args, mac_address_t *);

    return format(s, "%02x:%02x:%02x:%02x:%02x:%02x", mac->bytes[0], mac->bytes[1], mac->bytes[2], mac->bytes[3], mac->bytes[4], mac->bytes[5]);
}
#define _(T)                                                              \
