...
static inline void clib_bihash_mark_free_48_8(clib_bihash_kv_48_8_t *v)
{
    v->value = 0xFEEDFACE8BADF00DULL;
}

static inline int clib_bihash_is_free_48_8(const clib_bihash_kv_48_8_t *v)
{
    if (v->value == 0xFEEDFACE8BADF00DULL) return 1;
    return 0;
}

static inline u64 clib_bihash_hash_48_8(const clib_bihash_kv_48_8_t *v)
{
...
static inline u8 *format_bihash_kvp_48_8(u8 *s, va_list *args)
{
    clib_bihash_kv_48_8_t *v = va_arg(*args, clib_bihash_kv_48_8_t *);

    s = format(s, "key %llu %llu %llu %llu %llu %llu value %llu", v->key[0], v->key[1], v->key[2], v->key[3], v->key[4], v->key[5], v->value);
    return s;
}

static inline int clib_bihash_key_compare_48_8(u64 *a, u64 *b)
{
#if defined(CLIB_HAVE_VEC512)
    return u64x8_is_equal(u64x8_mask_load_zero(a, 0x3f), u64x8_mask_load_zero(b, 0x3f));
