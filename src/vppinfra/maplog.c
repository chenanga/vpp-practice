...
__clib_export int clib_maplog_init(clib_maplog_init_args_t *a)
{
    ...
    /* Already initialized? */
    if (mm->flags & CLIB_MAPLOG_FLAG_INIT) return (-2);

    clib_memset(mm, 0, sizeof(*mm));

    record_size_in_cache_lines = (a->record_size_in_bytes + CLIB_CACHE_LINE_BYTES - 1) / CLIB_CACHE_LINE_BYTES;

    file_size_in_records = a->file_size_in_bytes / (record_size_in_cache_lines * CLIB_CACHE_LINE_BYTES);
    ...
    a->file_size_in_bytes         = file_size_in_records * record_size_in_cache_lines * CLIB_CACHE_LINE_BYTES;
     ...
     */
    for (i = 0; i < limit; i++) {
        mm->filenames[i] = format(0, "%v_%d", mm->file_basename, mm->current_file_index++);
        ...
        mm->file_baseva[i] = mmap(0, a->file_size_in_bytes, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
...
fail:
    ...
    for (i = 0; i < limit; i++) {
        if (mm->file_baseva[i]) (void) munmap((u8 *) mm->file_baseva[i], a->file_size_in_bytes);
        if (mm->filenames[i]) (void) unlink((char *) mm->filenames[i]);
...
__clib_export u8 *_clib_maplog_get_entry_slowpath(clib_maplog_main_t *mm, u64 my_record_index)
{
    int fd;
    u8 *rv;
    u8  zero               = 0;
    u32 unmap_index        = (mm->current_file_index) & 1;
    u64 file_size_in_bytes = mm->file_size_in_records * mm->record_size_in_cachelines * CLIB_CACHE_LINE_BYTES;
    ...
    mm->filenames[unmap_index] = format(mm->filenames[unmap_index], "%v_%d", mm->file_basename, mm->current_file_index++);

    /* Unmap the previous (full) segment */
    (void) munmap((u8 *) mm->file_baseva[unmap_index], file_size_in_bytes);

    /* Create a new segment */
    fd = open((char *) mm->filenames[unmap_index], O_CREAT | O_RDWR | O_TRUNC, 0600);
    ...
    mm->file_baseva[unmap_index] = mmap(0, file_size_in_bytes, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    ...
    rv = (u8 *) mm->file_baseva[(my_record_index >> mm->log2_file_size_in_records) & 1]
         + (my_record_index & (mm->file_size_in_records - 1)) * mm->record_size_in_cachelines * CLIB_CACHE_LINE_BYTES;
...
void clib_maplog_update_header(clib_maplog_main_t *mm)
{
    ...
    if (!(mm->flags & CLIB_MAPLOG_FLAG_INIT)) return;
    ...
    rv = write(fd, h, sizeof(*h));
    if (rv != sizeof(*h)) clib_unix_warning("rewrite header");
...
__clib_export void clib_maplog_close(clib_maplog_main_t *mm)
{
    ...
    file_size_in_bytes = mm->file_size_in_records * mm->record_size_in_cachelines * CLIB_CACHE_LINE_BYTES;
...
__clib_export int clib_maplog_process(char *file_basename, void *fp_arg)
{
    ...
    file_size_in_bytes = h->file_size_in_records * h->record_size_in_cachelines * CLIB_CACHE_LINE_BYTES;

    records_left = h->number_of_records;

    for (file_index = 0; file_index < h->number_of_files; file_index++) {
        vec_reset_length(this_filename);
        this_filename = format(this_filename, "%s_%llu%c", file_basename, file_index, 0);
        ...
        records_this_file = (records_left > h->file_size_in_records) ? h->file_size_in_records : records_left;

        /*
         * Normal log, or a circular non-wrapped log, or a circular
         * wrapped log which happens to be exactly linear
         */
        if (h->maplog_flag_circular == 0 || h->maplog_flag_wrapped == 0 || ((h->number_of_records % h->file_size_in_records) == 0))
            (*fp)(h, file_baseva, records_this_file);
        else {
            /* "Normal" wrapped circular log */
            u64 first_chunk_record_index      = h->number_of_records & (h->file_size_in_records - 1);
            u64 first_chunk_number_of_records = records_this_file - first_chunk_record_index;
            u8 *chunk_baseva                  = file_baseva + first_chunk_record_index * h->record_size_in_cachelines * h->cacheline_size;
