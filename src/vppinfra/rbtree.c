...
static inline void rb_tree_rotate_left(rb_tree_t *rt, rb_node_t *x)
{
    rb_node_t      *y, *tmp, *xp;
...
static inline void rb_tree_rotate_right(rb_tree_t *rt, rb_node_t *y)
{
...
static inline void rb_tree_fixup_inline(rb_tree_t *rt, rb_node_t *y, rb_node_t *z)
{
    rb_node_t      *zpp, *zp;
    rb_node_index_t zi;
...
__clib_export rb_node_t *rb_tree_successor(rb_tree_t *rt, rb_node_t *x)
{
    ...
    if (x->right != RBTREE_TNIL_INDEX) return rb_tree_min_subtree(rt, rb_node_right(rt, x));
...
__clib_export rb_node_t *rb_tree_predecessor(rb_tree_t *rt, rb_node_t *x)
{
    ...
    if (x->left != RBTREE_TNIL_INDEX) return rb_tree_max_subtree(rt, rb_node_left(rt, x));
...
static inline void rb_tree_transplant(rb_tree_t *rt, rb_node_t *u, rb_node_t *v)
{
    rb_node_t *up;
...
static void rb_tree_del_node_internal(rb_tree_t *rt, rb_node_t *z)
{
    ...
    if (y_original_color == RBTREE_RED) return;
    ...
    while (xi != rt->root && x->color == RBTREE_BLACK) {
        ...
        if (xi == xp->left) {
            ...
            if (wl->color == RBTREE_BLACK && wr->color == RBTREE_BLACK) {
                if (!rb_node_is_tnil(rt, w)) w->color = RBTREE_RED;
                x = xp;
            }
            else {
...
__clib_export void rb_tree_del(rb_tree_t *rt, u32 key)
{
    ...
    if (rb_node_index(rt, n) != RBTREE_TNIL_INDEX) rb_tree_del_node(rt, n);
}
