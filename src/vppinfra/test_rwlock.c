...
void *write_shared_counter(void *arg)
{
...
void *read_shared_counter(void *arg)
{
    f64 *time                   = vec_new(f64, 1);
    *time                       = 0;
    rwlock_test_main_t *rtm     = arg;
    uword               cnt_cpy = 0, exp = rtm->increment_per_thread * rtm->write_cores * rtm->threads_per_core;
...
unsigned test_rwlock(rwlock_test_main_t *rtm, f64 *elapse_time)
{
    int       error = 0, total_threads = (rtm->read_cores + rtm->write_cores) * rtm->threads_per_core;
    ...
    for (unsigned cpu_mask = rtm->cpu_mask_read; cpu_mask; cpu_mask >>= 1) {
        ...
        for (uword t_num = 0; t_num < rtm->threads_per_core; t_num++) {
            ...
            if ((error = pthread_create(&pthread[t_index], NULL, &read_shared_counter, rtm)))
                clib_unix_warning("pthread_create failed with %d", error);

            if ((error = pthread_setaffinity_np(pthread[t_index], sizeof(cpu_set_t), &cpuset)))
                clib_unix_warning("pthread_set_affinity_np failed with %d", error);
        }
        cores_set++;
        cpu_id++;
    }
    ...
    for (unsigned cpu_mask = rtm->cpu_mask_write; cpu_mask; cpu_mask >>= 1) {
        ...
        for (uword t_num = 0; t_num < rtm->threads_per_core; t_num++) {
            uword t_index = cores_set * rtm->threads_per_core + t_num;
            if ((error = pthread_create(&pthread[t_index], NULL, &write_shared_counter, rtm)))
                clib_unix_warning("pthread_create failed with %d", error);
    ...
    for (uword thread_num = 0; thread_num < total_threads; thread_num++) {
        f64 *time;
        if ((error = pthread_join(pthread[thread_num], (void *) &time))) clib_unix_warning("pthread_join failed with %d", error);
        *elapse_time += *time;
        vec_free(time);
    }
...
uword num_cores_in_cpu_mask(uword mask)
{
    uword num_cores = 0;
...
int test_rwlock_main(unformat_input_t *i)
{
    rwlock_test_main_t _rtm, *rtm = &_rtm;
    clib_memset(rtm, 0, sizeof(rwlock_test_main_t));

    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (0 == unformat(i, "threads/core %d", &rtm->threads_per_core) && 0 == unformat(i, "cpu_mask_read %x", &rtm->cpu_mask_read)
            && 0 == unformat(i, "cpu_mask_write %x", &rtm->cpu_mask_write) && 0 == unformat(i, "increment %d", &rtm->increment_per_thread)
    ...
    uword total_increment = rtm->threads_per_core * rtm->write_cores * rtm->increment_per_thread;
