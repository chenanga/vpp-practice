...
u8 *_clib_maplog_get_entry_slowpath(clib_maplog_main_t *mm, u64 my_record_index);
 ...
 */
static inline void *clib_maplog_get_entry(clib_maplog_main_t *mm)
{
    u64 my_record_index;
    u8 *rv;
    ...
    if (PREDICT_FALSE((my_record_index & (mm->file_size_in_records - 1)) == 0)) {
        ...
        if (!(mm->flags & CLIB_MAPLOG_FLAG_CIRCULAR)) {
            ...
            if (my_record_index) return _clib_maplog_get_entry_slowpath(mm, my_record_index);
        }
