...
__clib_export clib_stack_frame_t *clib_stack_frame_get(clib_stack_frame_t *sf)
{
#if HAVE_LIBUNWIND == 1
    Dl_info info = {};
    ...
    if (sf->index == 0) {
        ...
        if (unw_init_local(&cursor, &context) < 0) {
            clib_warning("libunwind: cannot initialize cursor for local unwinding\n");
            return 0;
        }
        if (unw_step(&cursor) < 1) return 0;
    }
    ...
    if (unw_get_proc_name(&cursor, sf->name, sizeof(sf->name), &sf->offset) < 0) sf->name[0] = sf->offset = 0;
