...
__clib_export u8 *format_cpu_uarch(u8 *s, va_list *args)
{
    ...
    if (__get_cpuid(0, &eax, &ebx, &ecx, &edx) == 0) return format(s, "unknown (missing cpuid)");

    if (amd_vendor(ebx, ecx, edx)) amd_vendor = 1;

    if (__get_cpuid(1, &eax, &ebx, &ecx, &edx) == 0) return format(s, "unknown (missing cpuid)");
...
#define _(f, m, a, c) \
    if ((model == m) && (family == f)) return format(s, "[0x%x] %s ([0x%02x] %s) stepping 0x%x", f, a, m, c, stepping);
...
#define _(i, p, a, c, _format)                                                    \
    if ((implementer == i) && (primary_part_number == p)) {                       \
        if (_format)                                                              \
            return format(s, "%s (%s PASS %c%u)", a, c, 'A' + variant, revision); \
        else {                                                                    \
            if (implementer == 0x43) variant++;                                   \
...
#undef _
        return format(s, "unknown (implementer 0x%02x part 0x%03x PASS %u.%u)", implementer, primary_part_number, variant, revision);
...
__clib_export u8 *format_cpu_model_name(u8 *s, va_list *args)
{
    ...
    if (eax < 0x80000004) return format(s, "unknown (missing ext feature)");
...
{
    if (0 == strncmp(flag, pfx, len - 1)) return flag + len - 1;
    return flag;
}
#endif
...
#define _(flag, func, reg, bit) \
    if (clib_cpu_supports_##flag()) s = format(s, "%s ", flag_skip_prefix(#flag, "x86_", sizeof("x86_")));
    foreach_x86_64_flags return s;
#undef _
#elif defined(__aarch64__)
#define _(flag, bit) \
    if (clib_cpu_supports_##flag()) s = format(s, "%s ", flag_skip_prefix(#flag, "aarch64_", sizeof("aarch64_")));
...
__clib_export u8 *format_march_variant(u8 *s, va_list *args)
{
...
__clib_export const clib_cpu_info_t *clib_get_cpu_info()
{
    ...
    if (first_run) {
        ...
        while (!feof(fp)) {
            ...
            buf[127] = '\0';
            if (strstr(buf, "CPU part")) info.aarch64.part_num = strtol(memchr(buf, ':', 128) + 2, NULL, 0);

            if (strstr(buf, "CPU implementer")) info.aarch64.implementer = strtol(memchr(buf, ':', 128) + 2, NULL, 0);
        }
        fclose(fp);
