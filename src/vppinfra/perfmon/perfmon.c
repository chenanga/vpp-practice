...
__clib_export clib_error_t *clib_perfmon_init_by_bundle_name(clib_perfmon_ctx_t *ctx, char *fmt, ...)
{
    ...
    for (clib_perfmon_bundle_reg_t *r = pm->bundle_regs; r; r = r->next) {
        if (strncmp(r->bundle->name, bundle_name, vec_len(bundle_name) - 1)) continue;
        b = r->bundle;
        break;
    }
    ...
    for (int i = 0; i < b->n_events; i++) {
        ...
        if (ctx->debug) fformat(stderr, "perf event %u open, fd %d\n", i, fd);
    ...
    if (err) clib_perfmon_free(ctx);

    vec_free(bundle_name);
    return err;
}

__clib_export void clib_perfmon_free(clib_perfmon_ctx_t *ctx)
{
    ...
    for (int i = 0; i < vec_len(ctx->fds); i++)
        if (ctx->fds[i] > -1) close(ctx->fds[i]);
    vec_free(ctx->fds);
    vec_free(ctx->data);
}

__clib_export void clib_perfmon_clear(clib_perfmon_ctx_t *ctx)
{
...
__clib_export u64 *clib_perfmon_capture(clib_perfmon_ctx_t *ctx, u32 n_ops, char *fmt, ...)
{
    ...
    if ((read(ctx->group_fd, d, read_size) != read_size)) {
        if (ctx->debug) fformat(stderr, "reading of %u bytes failed, %s (%d)\n", read_size, strerror(errno), errno);
        return 0;
    }

    if (ctx->debug) {
        fformat(stderr, "read events: %lu enabled: %lu running: %lu ", d[0], d[1], d[2]);
...
__clib_export void clib_perfmon_capture_group(clib_perfmon_ctx_t *ctx, char *fmt, ...)
{
    ...
    if (cg->name != 0) vec_add2(ctx->capture_groups, cg, 1);
...
__clib_export void clib_perfmon_warmup(clib_perfmon_ctx_t *ctx)
{
    for (u64 i = 0; i < (u64) ctx->ref_clock; i++)
        asm volatile("" : : "r"(i * i) : "memory");
}

__clib_export u8 *format_perfmon_bundle(u8 *s, va_list *args)
{
    ...
    vec_foreach (c, ctx->captures) {
        ...
        for (int i = 0; i < n_row; i++)
            table_format_cell(t, col, i + 1, "%U", ctx->bundle->format_fn, ctx, c, i);
        col++;
    }
