...
static_always_inline void clib_perfmon_ioctl(int fd, u32 req)
{
#ifdef __linux__
#ifdef __x86_64__
    asm volatile("syscall" : : "D"(fd), "S"(req), "a"(__NR_ioctl), "d"(PERF_IOC_FLAG_GROUP) : "rcx", "r11" /* registers modified by kernel */);
#else
...
clib_error_t     *clib_perfmon_init_by_bundle_name(clib_perfmon_ctx_t *ctx, char *fmt, ...);
...
#ifdef __linux__
static_always_inline void clib_perfmon_reset(clib_perfmon_ctx_t *ctx)
{
    clib_perfmon_ioctl(ctx->group_fd, PERF_EVENT_IOC_RESET);
}
static_always_inline void clib_perfmon_enable(clib_perfmon_ctx_t *ctx)
{
    clib_perfmon_ioctl(ctx->group_fd, PERF_EVENT_IOC_ENABLE);
}
static_always_inline void clib_perfmon_disable(clib_perfmon_ctx_t *ctx)
{
