...
static void run_single_wheel(tw_timer_wheel_2t_1w_2048sl_t *tw, u32 n_ticks)
{
    u32 i;
...
static void run_double_wheel(tw_timer_wheel_16t_2w_512sl_t *tw, u32 n_ticks)
{
    u32 i;
    f64 now = tw->last_run_time + 1.01;
...
static void run_two_timer_double_wheel(tw_timer_wheel_2t_2w_512sl_t *tw, u32 n_ticks)
{
    u32 i;
    f64 now = tw->last_run_time + 1.01;
...
static void run_triple_wheel(tw_timer_wheel_4t_3w_256sl_t *tw, u32 n_ticks)
{
    u32 i;
    f64 now = tw->last_run_time + 1.01;
...
static void run_triple_ov_wheel(tw_timer_wheel_1t_3w_1024sl_ov_t *tw, u32 n_ticks)
{
    u32 i;
    f64 now = tw->last_run_time + 1.01;
...
static void expired_timer_double_callback(u32 *expired_timers)
{
    int                   i;
    u32                   pool_index, timer_id;
    tw_timer_test_elt_t  *e;
...
static void expired_timer_two_timer_double_callback(u32 *expired_timers)
{
    int                   i;
    u32                   pool_index, timer_id;
    tw_timer_test_elt_t  *e;
...
static void expired_timer_triple_callback(u32 *expired_timers)
{
    int                   i;
    u32                   pool_index, timer_id;
    tw_timer_test_elt_t  *e;
...
static void expired_timer_triple_ov_callback(u32 *expired_timers)
{
    int                   i;
    u32                   pool_index;
    tw_timer_test_elt_t  *e;
...
static clib_error_t *test2_single(tw_timer_test_main_t *tm)
{
    u32                  i, j;
    tw_timer_test_elt_t *e;
    ...
    tw_timer_wheel_init_2t_1w_2048sl(&tm->single_wheel, expired_timer_single_callback, 1.0 /* timer interval */, ~0);
    ...
    for (i = 0; i < tm->ntimers; i++) {
        ...
        if (expiration_time > max_expiration_time) max_expiration_time = expiration_time;

        e->expected_to_expire = expiration_time + initial_wheel_offset;
        e->stop_timer_handle  = tw_timer_start_2t_1w_2048sl(&tm->single_wheel, e - tm->test_elts, 1 /* timer id */, expiration_time);
    }
    ...
    for (i = 0; i < tm->niter; i++) {
        ...
        pool_foreach (e, tm->test_elts) {
            ...
            if (++j >= tm->ntimers / 4) goto del_and_re_add;
        }
    ...
    del_and_re_add:
        ...
        for (j = 0; j < tm->ntimers / 4; j++) {
            ...
            e->expected_to_expire = expiration_time + tm->single_wheel.current_tick;
            e->stop_timer_handle  = tw_timer_start_2t_1w_2048sl(&tm->single_wheel, e - tm->test_elts, 1 /* timer id */, expiration_time);
        }
        adds += j;
    }
    ...
    fformat(stdout, "%d adds, %d deletes, %d ticks\n", adds, deletes, tm->single_wheel.current_tick);
    fformat(stdout,
            "test ran %.2f seconds, %.2f ops/second\n",
            (after - before),
            ((f64) adds + (f64) deletes + (f64) tm->single_wheel.current_tick) / (after - before));

    if (pool_elts(tm->test_elts)) fformat(stdout, "Note: %d elements remain in pool\n", pool_elts(tm->test_elts));
...
static clib_error_t *test2_double(tw_timer_test_main_t *tm)
{
    u32                  i, j;
    tw_timer_test_elt_t *e;
    u32                  initial_wheel_offset;
    ...
    tw_timer_wheel_init_16t_2w_512sl(&tm->double_wheel, expired_timer_double_callback, 1.0 /* timer interval */, ~0);
    ...
    fformat(stdout,
            "initial wheel time %d, fast index %d slow index %d\n",
    ...
    for (i = 0; i < tm->ntimers; i++) {
        ...
        e->stop_timer_handle = tw_timer_start_16t_2w_512sl(&tm->double_wheel, e - tm->test_elts, 14 /* timer id */, expiration_time);
    }
    ...
    del_and_re_add:
        ...
        for (j = 0; j < tm->ntimers / 4; j++) {
            ...
            e->expected_to_expire = expiration_time + tm->double_wheel.current_tick;
    ...
    fformat(stdout, "%d adds, %d deletes, %d ticks\n", adds, deletes, tm->double_wheel.current_tick);
    fformat(stdout,
            "test ran %.2f seconds, %.2f ops/second\n",
            (after - before),
            ((f64) adds + (f64) deletes + (f64) tm->double_wheel.current_tick) / (after - before));
...
static u32 get_expiration_time(tw_timer_test_main_t *tm)
{
    u32 expiration_time;
    do {
        expiration_time = random_u64(&tm->seed) & ((1 << 17) - 1);
...
static clib_error_t *test2_double_updates(tw_timer_test_main_t *tm)
{
    u32                  i, j;
    tw_timer_test_elt_t *e;
    u32                  initial_wheel_offset;
    ...
    for (i = 0; i < tm->niter; i++) {
        ...
        pool_foreach (e, tm->test_elts) {
            ...
            max_expiration_time   = clib_max(expiration_time, max_expiration_time);
            e->expected_to_expire = expiration_time + tm->double_wheel.current_tick;
            tw_timer_update_16t_2w_512sl(&tm->double_wheel, e->stop_timer_handle, expiration_time);
            if (++j >= tm->ntimers / 4) goto done;
        }
    ...
    fformat(stdout, "%d updates, %d ticks\n", updates, tm->double_wheel.current_tick);
    fformat(stdout,
            "test ran %.2f seconds, %.2f ops/second\n",
            (after - before),
            ((f64) updates + (f64) tm->double_wheel.current_tick) / (after - before));
...
static clib_error_t *test2_triple(tw_timer_test_main_t *tm)
{
    u32                  i, j;
    tw_timer_test_elt_t *e;
    ...
    tw_timer_wheel_init_4t_3w_256sl(&tm->triple_wheel, expired_timer_triple_callback, 1.0 /* timer interval */, ~0);
    ...
    for (i = 0; i < tm->ntimers; i++) {
        ...
        e->stop_timer_handle = tw_timer_start_4t_3w_256sl(&tm->triple_wheel, e - tm->test_elts, 3 /* timer id */, expiration_time);
    }
    ...
    del_and_re_add:
        ...
        for (j = 0; j < tm->ntimers / 4; j++) {
            ...
            e->expected_to_expire = expiration_time + tm->triple_wheel.current_tick;
    ...
    fformat(stdout, "%d adds, %d deletes, %d ticks\n", adds, deletes, tm->triple_wheel.current_tick);
    fformat(stdout,
            "test ran %.2f seconds, %.2f ops/second\n",
            (after - before),
            ((f64) adds + (f64) deletes + (f64) tm->triple_wheel.current_tick) / (after - before));
...
static clib_error_t *test2_triple_ov(tw_timer_test_main_t *tm)
{
    u32                  i, j;
    tw_timer_test_elt_t *e;
    u32                  initial_wheel_offset = 0;
    ...
    tw_timer_wheel_init_1t_3w_1024sl_ov(&tm->triple_ov_wheel, expired_timer_triple_ov_callback, 1.0 /* timer interval */, ~0);
    ...
    for (i = 0; i < tm->ntimers; i++) {
        ...
        e->stop_timer_handle = tw_timer_start_1t_3w_1024sl_ov(&tm->triple_ov_wheel, e - tm->test_elts, 0 /* timer id */, expiration_time);
    }
    ...
    for (i = 0; i < tm->niter; i++) {
        ...
        vec_reset_length(deleted_indices);
        pool_foreach (e, tm->test_elts) {
            tw_timer_stop_1t_3w_1024sl_ov(&tm->triple_ov_wheel, e->stop_timer_handle);
            vec_add1(deleted_indices, e - tm->test_elts);
            if (++j >= tm->ntimers / 4) goto del_and_re_add;
        }
    ...
    del_and_re_add:
        ...
        for (j = 0; j < tm->ntimers / 4; j++) {
            ...
            e->expected_to_expire = expiration_time + tm->triple_ov_wheel.current_tick;
    ...
    fformat(stdout, "%d adds, %d deletes, %d ticks\n", adds, deletes, tm->triple_ov_wheel.current_tick);
    fformat(stdout,
            "test ran %.2f seconds, %.2f ops/second\n",
            (after - before),
            ((f64) adds + (f64) deletes + (f64) tm->triple_ov_wheel.current_tick) / (after - before));
...
static clib_error_t *test1_single(tw_timer_test_main_t *tm)
{
    u32                  i;
    tw_timer_test_elt_t *e;
    u32                  offset;
    ...
    for (i = 0; i < tm->ntimers; i++) {
        ...
        e->stop_timer_handle  = tw_timer_start_2t_1w_2048sl(&tm->single_wheel, e - tm->test_elts, 1 /* timer id */, timer_arg);
    }
    ...
    fformat(stdout, "final wheel time %d, fast index %d\n", tm->single_wheel.current_tick, tm->single_wheel.current_index[TW_TIMER_RING_FAST]);
...
static clib_error_t *test1_double(tw_timer_test_main_t *tm)
{
    u32                  i;
    tw_timer_test_elt_t *e;
    u32                  offset;
    ...
    fformat(stdout, "initial wheel time %d, fast index %d\n", tm->double_wheel.current_tick, tm->double_wheel.current_index[TW_TIMER_RING_FAST]);
    ...
    for (i = 0; i < tm->ntimers; i++) {
        ...
        e->stop_timer_handle  = tw_timer_start_16t_2w_512sl(&tm->double_wheel, e - tm->test_elts, 14 /* timer id */, i + 1);
    }
...
static clib_error_t *test1_two_timer_double(tw_timer_test_main_t *tm)
{
    u32                  i;
    tw_timer_test_elt_t *e;
    u32                  offset;

    tw_timer_wheel_init_2t_2w_512sl(&tm->two_timer_double_wheel, expired_timer_two_timer_double_callback, 1.0 /* timer interval */, ~0);
    ...
    fformat(stdout,
            "initial wheel time %d, fast index %d\n",
    ...
    for (i = 0; i < tm->ntimers; i++) {
        ...
        e->stop_timer_handle  = tw_timer_start_2t_2w_512sl(&tm->two_timer_double_wheel, e - tm->test_elts, 1 /* timer id */, i + 1);
    }
...
static clib_error_t *test3_triple_double(tw_timer_test_main_t *tm)
{
    tw_timer_test_elt_t *e;
    u32                  initial_wheel_offset = 0;
    u32                  expiration_time;
...
static clib_error_t *test4_double_double(tw_timer_test_main_t *tm)
{
    u32                  i;
    tw_timer_test_elt_t *e;
    u32                  initial_wheel_offset;
...
static clib_error_t *test5_double(tw_timer_test_main_t *tm)
{
    u32                  i;
    tw_timer_test_elt_t *e;
    u32                  initial_wheel_offset;
...
static clib_error_t *timer_test_command_fn(tw_timer_test_main_t *tm, unformat_input_t *input)
{
    ...
    if (is_test1 + is_test2 + is_test3 + is_test4 + is_test5 == 0) return clib_error_return(0, "No test specified [test1..n]");

    if (num_wheels < 1 || num_wheels > 3) return clib_error_return(0, "unsupported... 1 or 2 wheels only");
    ...
    if (is_test3) return test3_triple_double(tm);

    if (is_test4) return test4_double_double(tm);

    if (is_test5) return test5_double(tm);
