...
static inline void *BV(alloc_aligned)(BVT(clib_bihash) * h, uword nbytes)
{
    ...
    if (BIHASH_USE_HEAP) {
        ...
        uword chunk_sz = round_pow2(page_sz << BIIHASH_MIN_ALLOC_LOG2_PAGES, CLIB_CACHE_LINE_BYTES);
        ...
        if (nbytes >= chunk_sz) {
            ...
            chunk   = clib_mem_alloc_aligned(nbytes + sizeof(*chunk), CLIB_CACHE_LINE_BYTES);
        ...
        oldheap = clib_mem_set_heap(h->heap);
        chunk   = clib_mem_alloc_aligned(chunk_sz + sizeof(*chunk), CLIB_CACHE_LINE_BYTES);
        clib_mem_set_heap(oldheap);
        chunk->size       = chunk_sz;
    ...
    if (alloc_arena_next(h) > alloc_arena_size(h)) os_out_of_memory();
    ...
    if (alloc_arena_next(h) > alloc_arena_mapped(h)) {
...
#if __linux__
        int mmap_flags_huge = (mmap_flags | MAP_HUGETLB | MAP_LOCKED | BIHASH_LOG2_HUGEPAGE_SIZE << MAP_HUGE_SHIFT);
#endif /* __linux__ */

        /* new allocation is 25% of existing one */
        if (alloc_arena_mapped(h) >> 2 > alloc) alloc = alloc_arena_mapped(h) >> 2;
        ...
        if (rv == MAP_FAILED || mlock(base, alloc) != 0) rv = mmap(base, alloc, PROT_READ | PROT_WRITE, mmap_flags, -1, 0);

        if (rv == MAP_FAILED) os_out_of_memory();
...
static void BV(clib_bihash_instantiate)(BVT(clib_bihash) * h)
{
    ...
    if (BIHASH_USE_HEAP) {
        ...
        alloc_arena(h) = clib_mem_vm_reserve(0, h->memory_size, BIHASH_LOG2_HUGEPAGE_SIZE);
        if (alloc_arena(h) == ~0) os_out_of_memory();
    ...
    if (BIHASH_KVP_AT_BUCKET_LEVEL) bucket_size += h->nbuckets * BIHASH_KVP_PER_PAGE * sizeof(BVT(clib_bihash_kv));
    ...
    if (BIHASH_KVP_AT_BUCKET_LEVEL) {
        ...
        for (i = 0; i < h->nbuckets; i++) {
            ...
            b = (void *) (((uword) b) + sizeof(*b) + (BIHASH_KVP_PER_PAGE * sizeof(BVT(clib_bihash_kv))));
        }
    }
...
void BV(clib_bihash_init2)(BVT(clib_bihash_init2_args) * a)
{
     ...
     */
    if (BIHASH_USE_HEAP) ASSERT(h->memory_size < (1ULL << BIHASH_BUCKET_OFFSET_BITS));

    /* Add this hash table to the list */
    if (a->dont_add_to_all_bihash_list == 0) {
        for (i = 0; i < vec_len(clib_all_bihashes); i++)
            if (clib_all_bihashes[i] == h) goto do_lock;
...
do_lock:
    if (h->alloc_lock) clib_mem_free((void *) h->alloc_lock);

    /*
     * Set up the lock now, so we can use it to make the first add
     * thread-safe
     */
    h->alloc_lock    = clib_mem_alloc_aligned(CLIB_CACHE_LINE_BYTES, CLIB_CACHE_LINE_BYTES);
    h->alloc_lock[0] = 0;
...
void BV(clib_bihash_initiator_init_svm)(BVT(clib_bihash) * h, char *name, u32 nbuckets, u64 memory_size)
{
    uword         bucket_size;
    u8           *mmap_addr;
...
void BV(clib_bihash_responder_init_svm)(BVT(clib_bihash) * h, char *name, int fd)
{
    u8 *mmap_addr;
    u64 memory_size;
    ...
    mmap_addr = mmap(0, memory_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0 /* offset */);
...
void BV(clib_bihash_free)(BVT(clib_bihash) * h)
{
    ...
    if (PREDICT_FALSE(h->instantiated == 0)) goto never_initialized;
...
#else
    if (h->memfd > 0) (void) close(h->memfd);
#endif
    if (BIHASH_USE_HEAP == 0) clib_mem_vm_free((void *) (uword) (alloc_arena(h)), alloc_arena_size(h));
never_initialized:
    ...
    clib_warning("Couldn't find hash table %llx on clib_all_bihashes...", (u64) (uword) h);
}
...
static void BV(value_free)(BVT(clib_bihash) * h, BVT(clib_bihash_value) * v, u32 log2_pages)
{
    ASSERT(h->alloc_lock[0]);
    ...
    if (CLIB_DEBUG > 0) clib_memset_u8(v, 0xFE, sizeof(*v) * (1 << log2_pages));

    v->next_free_as_u64      = (u64) h->freelists[log2_pages];
    h->freelists[log2_pages] = (u64) BV(clib_bihash_get_offset)(h, v);
}

static inline void BV(make_working_copy)(BVT(clib_bihash) * h, BVT(clib_bihash_bucket) * b)
{
    ...
    if (b->log2_pages > log2_working_copy_length) {
         ...
         */
        working_copy                          = BV(alloc_aligned)(h, sizeof(working_copy[0]) * (1 << b->log2_pages));
        h->working_copy_lengths[thread_index] = b->log2_pages;
        h->working_copies[thread_index]       = working_copy;

        BV(clib_bihash_increment_stat)(h, BIHASH_STAT_working_copy_lost, 1ULL << b->log2_pages);
    }
...
static BVT(clib_bihash_value)
    * BV(split_and_rehash)(BVT(clib_bihash) * h, BVT(clib_bihash_value) * old_values, u32 old_log2_pages, u32 new_log2_pages)
{
    ...
    for (i = 0; i < length_in_kvs; i++) {
        ...
        if (BV(clib_bihash_is_free)(&(old_values->kvp[i]))) continue;
        ...
        for (j = 0; j < BIHASH_KVP_PER_PAGE; j++) {
            ...
            if (BV(clib_bihash_is_free)(&(new_v->kvp[j]))) {
                clib_memcpy_fast(&(new_v->kvp[j]), &(old_values->kvp[i]), sizeof(new_v->kvp[j]));
                goto doublebreak;
            }
        }
...
static BVT(clib_bihash_value)
    * BV(split_and_rehash_linear)(BVT(clib_bihash) * h, BVT(clib_bihash_value) * old_values, u32 old_log2_pages, u32 new_log2_pages)
{
    ...
    for (i = 0; i < old_length; i++) {
        ...
        for (; j < new_length; j++) {
            ...
            if (BV(clib_bihash_is_free)(&(old_values->kvp[i]))) goto doublebreak;

            /* New value should never be in use */
            if (BV(clib_bihash_is_free)(&(new_values->kvp[j]))) {
                /* Copy the old value and move along */
                clib_memcpy_fast(&(new_values->kvp[j]), &(old_values->kvp[i]), sizeof(new_values->kvp[j]));
                j++;
                goto doublebreak;
            }
        }
...
static_always_inline int BV(clib_bihash_add_del_inline_with_hash)(BVT(clib_bihash) * h,
                                                                  ...
                                                                  void *overwrite_arg)
{
    ...
    static const BVT(clib_bihash_bucket) mask = {.linear_search = 1, .log2_pages = -1};
    ...
    if (PREDICT_FALSE(h->instantiated == 0)) {
        if (is_add == 0) return (-1);

        BV(clib_bihash_alloc_lock)(h);
        if (h->instantiated == 0) BV(clib_bihash_instantiate)(h);
    ...
    if (is_add) {
         ...
         */
        for (i = 0; i < limit; i++) {
            if (BV(clib_bihash_is_free)(&(v->kvp[i]))) continue;
            if (BV(clib_bihash_key_compare)(v->kvp[i].key, add_v->key)) {
                ...
                if (overwrite_cb) overwrite_cb(&(v->kvp[i]), overwrite_arg);
                clib_memcpy_fast(&(v->kvp[i].value), &add_v->value, sizeof(add_v->value));
                BV(clib_bihash_unlock_bucket)(b);
        ...
        for (i = 0; i < limit; i++) {
            if (BV(clib_bihash_is_free)(&(v->kvp[i]))) {
                ...
                clib_memcpy_fast(&(v->kvp[i]), &add_v->key, sizeof(add_v->key));
                b->refcnt++;
                ASSERT(b->refcnt > 0);
                BV(clib_bihash_unlock_bucket)(b);
        ...
        for (i = 0; i < limit; i++) {
            ...
            if (BV(clib_bihash_key_compare)(v->kvp[i].key, add_v->key)) {
                ...
                if (PREDICT_TRUE(b->refcnt > 1)) {
                    ...
                    if (BIHASH_KVP_AT_BUCKET_LEVEL && b->refcnt == 1 && b->log2_pages > 0) {
    ...
    new_v = BV(split_and_rehash)(h, working_copy, old_log2_pages, new_log2_pages);
    if (new_v == 0) {
    try_resplit:
        resplit_once = 1;
        new_log2_pages++;
        ...
        mark_linear:
            ...
            new_v              = BV(split_and_rehash_linear)(h, working_copy, old_log2_pages, new_log2_pages);
        ...
        BV(clib_bihash_increment_stat)(h, BIHASH_STAT_splits, old_log2_pages + 1);
    }
...
static_always_inline int BV(clib_bihash_add_del_inline)(BVT(clib_bihash) * h,
                                                        BVT(clib_bihash_kv) * add_v,
                                                        int is_add,
                                                        int (*is_stale_cb)(BVT(clib_bihash_kv) *, void *),
                                                        void *arg)
{
    ...
    return BV(clib_bihash_add_del_inline_with_hash)(h, add_v, hash, is_add, is_stale_cb, arg, 0, 0);
}

int BV(clib_bihash_add_del_with_hash)(BVT(clib_bihash) * h, BVT(clib_bihash_kv) * add_v, u64 hash, int is_add)
{
    return BV(clib_bihash_add_del_inline_with_hash)(h, add_v, hash, is_add, 0, 0, 0, 0);
}
...
{
    u64 hash = BV(clib_bihash_hash)(add_v);
    return BV(clib_bihash_add_del_inline_with_hash)(h, add_v, hash, 1, 0, 0, overwrite_cb, arg);
}
...
u8 *BV(format_bihash)(u8 *s, va_list *args)
{
...
#if BIHASH_LAZY_INSTANTIATE
    if (PREDICT_FALSE(h->instantiated == 0)) return format(s, "    empty, uninitialized");
#endif
    ...
    for (i = 0; i < h->nbuckets; i++) {
        ...
        if (b->linear_search) linear_buckets++;

        if (verbose) {
            s = format(s, "[%d]: heap offset %lld, len %d, refcnt %d, linear %d\n", i, b->offset, (1 << b->log2_pages), b->refcnt, b->linear_search);
        }
        ...
        for (j = 0; j < (1 << b->log2_pages); j++) {
            for (k = 0; k < BIHASH_KVP_PER_PAGE; k++) {
                ...
                if (verbose) {
                    if (h->kvp_fmt_fn) {
                        s = format(s, "    %d: %U\n", j * BIHASH_KVP_PER_PAGE + k, h->kvp_fmt_fn, &(v->kvp[k]), verbose);
                    }
                    else {
                        s = format(s, "    %d: %U\n", j * BIHASH_KVP_PER_PAGE + k, BV(format_bihash_kvp), &(v->kvp[k]));
                    }
                }
                active_elements++;
            }
    ...
    for (i = 0; i < vec_len(h->freelists); i++) {
        ...
        if (nfree || verbose) s = format(s, "       [len %d] %u free elts\n", 1 << i, nfree);
    }
    ...
    if (BIHASH_USE_HEAP) {
        ...
        s              = format(s,
                   ...
                   used_bytes >> 20,
                   alloc_arena_size(h),
                   alloc_arena_size(h) >> 20);
    }
    return s;
}

void BV(clib_bihash_foreach_key_value_pair)(BVT(clib_bihash) * h, BV(clib_bihash_foreach_key_value_pair_cb) cb, void *arg)
{
    int i, j, k;
    BVT(clib_bihash_bucket) * b;
    BVT(clib_bihash_value) * v;
...
#if BIHASH_LAZY_INSTANTIATE
    if (PREDICT_FALSE(h->instantiated == 0)) return;
#endif

    for (i = 0; i < h->nbuckets; i++) {
        b = BV(clib_bihash_get_bucket)(h, i);
        if (BV(clib_bihash_bucket_is_empty)(b)) continue;

        v = BV(clib_bihash_get_value)(h, b->offset);
        for (j = 0; j < (1 << b->log2_pages); j++) {
            for (k = 0; k < BIHASH_KVP_PER_PAGE; k++) {
                if (BV(clib_bihash_is_free)(&v->kvp[k])) continue;

                if (BIHASH_WALK_STOP == cb(&v->kvp[k], arg)) return;
                /*
                 * In case the callback deletes the last entry in the bucket...
                 */
                if (BV(clib_bihash_bucket_is_empty)(b)) goto doublebreak;
            }
            v++;
        }
    doublebreak:;
