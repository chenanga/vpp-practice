...
static inline u32 TW(make_internal_timer_handle)(u32 pool_index, u32 timer_id)
{
    u32 handle;
...
static inline void timer_addhead(TWT(tw_timer) * pool, u32 head_index, u32 new_index)
{
...
static inline void timer_add(TWT(tw_timer_wheel) * tw, TWT(tw_timer) * t, u64 interval)
{
    ...
    tw->fast_slot_bitmap = clib_bitmap_set(tw->fast_slot_bitmap, fast_ring_offset, 1);
#endif
#if TW_START_STOP_TRACE_SIZE > 0
    TW(tw_timer_trace)(tw, timer_id, user_id, t - tw->timers);
#endif
        ...
        for (j = 0; j < TW_SLOTS_PER_RING; j++) {
            ...
            while (next_index != ts->head_index) {
                ...
                if (next_index == handle) {
                    clib_warning("handle %d found in ring %d slot %d", handle, i, j);
...
__clib_export void TW(tw_timer_stop)(TWT(tw_timer_wheel) * tw, u32 handle)
{
     ...
     */
    if (pool_is_free_index(tw->timers, handle)) return;
#endif
#if TW_START_STOP_TRACE_SIZE > 0
 ...
 */
static inline u32 *TW(tw_timer_expire_timers_internal)(TWT(tw_timer_wheel) * tw, f64 now, u32 *callback_vector_arg)
{
    u32                    nticks, i;
    tw_timer_wheel_slot_t *ts;
    TWT(tw_timer) * t, *head;
    ...
    if (PREDICT_FALSE(now < tw->next_run_time)) return callback_vector_arg;

    /* Number of ticks which have occurred */
    nticks = tw->ticks_per_second * (now - tw->last_run_time);
    if (nticks == 0) return callback_vector_arg;
    ...
    for (i = 0; i < nticks; i++) {
        ...
        if (TW_TIMER_WHEELS > 1) slow_wheel_index = tw->current_index[TW_TIMER_RING_SLOW];
        if (TW_TIMER_WHEELS > 2) glacier_wheel_index = tw->current_index[TW_TIMER_RING_GLACIER];

#if TW_OVERFLOW_VECTOR > 0
        /* Triple odometer-click? Process the overflow vector... */
        if (PREDICT_FALSE(fast_wheel_index == TW_SLOTS_PER_RING && slow_wheel_index == TW_SLOTS_PER_RING
            ...
            while (next_index != head - tw->timers) {
                ...
                if (PREDICT_FALSE(t->slow_ring_offset == 0 && t->fast_ring_offset == 0 && new_glacier_ring_offset == 0)) {
                ...
                /* Timer timer moves to the fast ring */
                    ...
                    timer_addhead(tw->timers, ts->head_index, t - tw->timers);
#if TW_FAST_WHEEL_BITMAP
                    tw->fast_slot_bitmap = clib_bitmap_set(tw->fast_slot_bitmap, t->fast_ring_offset, 1);
#endif
                }
            }
        }
        ...
        if (PREDICT_FALSE(fast_wheel_index == TW_SLOTS_PER_RING && slow_wheel_index == TW_SLOTS_PER_RING)) {
            ...
            while (next_index != head - tw->timers) {
                ...
                /* Timer expires Right Now */
                if (PREDICT_FALSE(t->slow_ring_offset == 0 && t->fast_ring_offset == 0)) {
                    vec_add1(callback_vector, t->user_handle);
#if TW_START_STOP_TRACE_SIZE > 0
                    TW(tw_timer_trace)(tw, 0xfe, t->user_handle, t - tw->timers);
#endif
...
#if TW_FAST_WHEEL_BITMAP
        tw->fast_slot_bitmap = clib_bitmap_set(tw->fast_slot_bitmap, fast_wheel_index, 0);
#endif
...
#if TW_TIMER_WHEELS > 1
        if (PREDICT_FALSE(fast_wheel_index == TW_SLOTS_PER_RING)) slow_wheel_index++;
        tw->current_index[TW_TIMER_RING_SLOW] = slow_wheel_index;
#endif

#if TW_TIMER_WHEELS > 2
        if (PREDICT_FALSE(slow_wheel_index == TW_SLOTS_PER_RING)) glacier_wheel_index++;
        tw->current_index[TW_TIMER_RING_GLACIER] = glacier_wheel_index;
#endif

        if (vec_len(callback_vector) >= tw->max_expirations) break;
    }

    if (callback_vector_arg == 0) tw->expired_timer_handles = callback_vector;
...
__clib_export u32 TW(tw_timer_first_expires_in_ticks)(TWT(tw_timer_wheel) * tw)
{
    ...
    if (fast_ring_index == TW_SLOTS_PER_RING) return 1;

    first_expiring_index = clib_bitmap_next_set(tw->fast_slot_bitmap, fast_ring_index);
    if (first_expiring_index == ~0) first_expiring_index = TW_SLOTS_PER_RING;

#else

    if (clib_bitmap_is_zero(tw->fast_slot_bitmap)) return TW_SLOTS_PER_RING;

    fast_ring_index = tw->current_index[TW_TIMER_RING_FAST];
    if (fast_ring_index == TW_SLOTS_PER_RING) fast_ring_index = 0;

    first_expiring_index = clib_bitmap_next_set(tw->fast_slot_bitmap, fast_ring_index);
    if (first_expiring_index == ~0 && fast_ring_index != 0) first_expiring_index = clib_bitmap_first_set(tw->fast_slot_bitmap);
#endif
