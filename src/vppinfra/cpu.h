...
#define CLIB_MARCH_FN_VOID_POINTER(fn) clib_march_select_fn_ptr(fn##_march_fn_registrations);

#define _CLIB_MARCH_FN_REGISTRATION(fn)                                                                              \
    static clib_march_fn_registration CLIB_MARCH_SFX(fn##_march_fn_registration) = {.name = CLIB_MARCH_VARIANT_STR}; \
                                                                                                                     \
    static void __clib_constructor fn##_march_register()                                                             \
    {                                                                                                                \
...
static inline int clib_get_cpuid(const u32 lev, u32 *eax, u32 *ebx, u32 *ecx, u32 *edx)
{
    if ((u32) __get_cpuid_max(0x80000000 & lev, 0) < lev) return 0;
    if (lev == 7)
...
static inline int clib_cpu_march_priority_scalar()
{
    return 1;
}

static inline int clib_cpu_march_priority_spr()
{
    if (clib_cpu_supports_enqcmd()) return 300;
    return -1;
}

static inline int clib_cpu_march_priority_icl()
{
    if (clib_cpu_supports_avx512_bitalg()) return 200;
    return -1;
}

static inline int clib_cpu_march_priority_adl()
{
    if (clib_cpu_supports_movdiri() && clib_cpu_supports_avx2()) return 150;
    return -1;
}

static inline int clib_cpu_march_priority_skx()
{
    if (clib_cpu_supports_avx512f()) return 100;
    return -1;
}

static inline int clib_cpu_march_priority_trm()
{
    if (clib_cpu_supports_movdiri()) return 40;
    return -1;
}

static inline int clib_cpu_march_priority_hsw()
{
    if (clib_cpu_supports_avx2()) return 50;
    return -1;
}

static inline int clib_cpu_march_priority_znver4()
{
    if (clib_cpu_supports_avx512_bitalg() && clib_cpu_supports_monitorx()) return 250;
    return -1;
}

static inline int clib_cpu_march_priority_znver3()
{
    if (clib_cpu_supports_avx2() && clib_cpu_supports_monitorx()) return 70;
    return -1;
}

#define X86_CPU_ARCH_PERF_FUNC 0xA

static inline int clib_get_pmu_counter_count(u8 *fixed, u8 *general)
{
#if defined(__x86_64__)
...
static inline int clib_cpu_march_priority_octeontx2()
{
    const clib_cpu_info_t *info = clib_get_cpu_info();

    if (!info || info->aarch64.implementer != AARCH64_CPU_IMPLEMENTER_CAVIUM) return -1;

    if (info->aarch64.part_num == AARCH64_CPU_PART_OCTEONTX2T96 || info->aarch64.part_num == AARCH64_CPU_PART_OCTEONTX2T98) return 20;

    return -1;
}

static inline int clib_cpu_march_priority_thunderx2t99()
{
    const clib_cpu_info_t *info = clib_get_cpu_info();

    if (!info || info->aarch64.implementer != AARCH64_CPU_IMPLEMENTER_CAVIUM) return -1;

    if (info->aarch64.part_num == AARCH64_CPU_PART_THUNDERX2) return 20;

    return -1;
}

static inline int clib_cpu_march_priority_qdf24xx()
{
    const clib_cpu_info_t *info = clib_get_cpu_info();

    if (!info || info->aarch64.implementer != AARCH64_CPU_IMPLEMENTER_QUALCOMM) return -1;

    if (info->aarch64.part_num == AARCH64_CPU_PART_QDF24XX) return 20;

    return -1;
}

static inline int clib_cpu_march_priority_cortexa72()
{
    const clib_cpu_info_t *info = clib_get_cpu_info();

    if (!info || info->aarch64.implementer != AARCH64_CPU_IMPLEMENTER_ARM) return -1;

    if (info->aarch64.part_num == AARCH64_CPU_PART_CORTEXA72) return 10;

    return -1;
}

static inline int clib_cpu_march_priority_neoversen1()
{
    const clib_cpu_info_t *info = clib_get_cpu_info();

    if (!info || info->aarch64.implementer != AARCH64_CPU_IMPLEMENTER_ARM) return -1;

    if (info->aarch64.part_num == AARCH64_CPU_PART_NEOVERSEN1) return 10;

    return -1;
}

static inline int clib_cpu_march_priority_neoversen2()
{
    const clib_cpu_info_t *info = clib_get_cpu_info();

    if (!info || info->aarch64.implementer != AARCH64_CPU_IMPLEMENTER_ARM) return -1;

    if (info->aarch64.part_num == AARCH64_CPU_PART_NEOVERSEN2) return 10;
...
#define CLIB_MARCH_FN_CONSTRUCTOR(fn)                                           \
    static void __clib_constructor CLIB_MARCH_SFX(fn##_march_constructor)(void) \
    {                                                                           \
