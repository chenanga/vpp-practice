...
static inline void clib_bihash_mark_free_12_4(clib_bihash_kv_12_4_t *v)
{
    v->value = 0xFEEDFACE;
}

static inline int clib_bihash_is_free_12_4(const clib_bihash_kv_12_4_t *v)
{
    if (v->value == 0xFEEDFACE) return 1;
    return 0;
}

static inline u64 clib_bihash_hash_12_4(const clib_bihash_kv_12_4_t *v)
{
...
static inline u8 *format_bihash_kvp_12_4(u8 *s, va_list *args)
{
    clib_bihash_kv_12_4_t *v = va_arg(*args, clib_bihash_kv_12_4_t *);

    s = format(s, "key %u %u %u value %u", v->key[0], v->key[1], v->key[2], v->value);
    return s;
}

static inline int clib_bihash_key_compare_12_4(u32 *a, u32 *b)
{
