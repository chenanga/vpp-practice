...
static inline void clib_dlist_init(dlist_elt_t *pool, u32 index)
{
    dlist_elt_t *head = pool_elt_at_index(pool, index);
    clib_memset(head, 0xFF, sizeof(*head));
}

static inline void clib_dlist_addtail(dlist_elt_t *pool, u32 head_index, u32 new_index)
{
...
static inline void clib_dlist_addhead(dlist_elt_t *pool, u32 head_index, u32 new_index)
{
    dlist_elt_t *head = pool_elt_at_index(pool, head_index);
...
static inline void clib_dlist_remove(dlist_elt_t *pool, u32 index)
{
...
static inline u32 clib_dlist_remove_head(dlist_elt_t *pool, u32 head_index)
{
    ...
    if (head->next == ~0 || (head->next == head_index)) return ~0;
