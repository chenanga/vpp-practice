...
void timing_wheel_init(timing_wheel_t *w, u64 current_cpu_time, f64 cpu_clocks_per_second)
{
    ...
    if (w->n_wheel_elt_time_bits <= 0 || w->n_wheel_elt_time_bits >= STRUCT_BITS_OF(timing_wheel_elt_t, cpu_time_relative_to_base))
        w->n_wheel_elt_time_bits = STRUCT_BITS_OF(timing_wheel_elt_t, cpu_time_relative_to_base) - 1;

    w->cpu_time_base                        = current_cpu_time;
    w->time_index_next_cpu_time_base_update = w->current_time_index + ((u64) 1 << (w->n_wheel_elt_time_bits - w->log2_clocks_per_bin));
}
        ...
        vec_foreach (e, level->elts[wi]) {
            ...
            if (e_li == level_index - 1)
                /* If this element was scheduled on the previous level
                   it must be wrapped. */
                _(e_ti + current_time_wheel_index(w, level_index - 1) >= w->bins_per_wheel);
            else {
                _(e_li == level_index);
                if (e_li == 0)
                    _(e_wi == wi);
...
always_inline void free_elt_vector(timing_wheel_t *w, timing_wheel_elt_t *ev)
{
    ...
    if (CLIB_DEBUG > 0) clib_memset(ev, ~0, vec_len(ev) * sizeof(ev[0]));
    vec_set_len(ev, 0);
...
static timing_wheel_elt_t *insert_helper(timing_wheel_t *w, uword level_index, uword rtime)
{
    ...
    if (!level->elts[wheel_index] && vec_len(w->free_elt_vectors)) level->elts[wheel_index] = vec_pop(w->free_elt_vectors);
...
static void timing_wheel_insert_helper(timing_wheel_t *w, u64 insert_cpu_time, u32 user_data)
{
    ...
    if (PREDICT_TRUE(0 == (dt >> BITS(e->cpu_time_relative_to_base)))) {
        ...
        if (insert_cpu_time < w->cached_min_cpu_time_on_wheel) w->cached_min_cpu_time_on_wheel = insert_cpu_time;
    }
    else {
...
{
    return (hash_elts(w->deleted_user_data_hash) > 0 && hash_get(w->deleted_user_data_hash, user_data));
}
...
static timing_wheel_elt_t *delete_user_data(timing_wheel_elt_t *elts, u32 user_data)
{
    ...
    if (!found_match) return elts;

    /* Re-scan to build vector of new elts with matching user_data deleted. */
    new_elts = 0;
    vec_foreach (e, elts) {
        if (e->user_data != user_data) vec_add1(new_elts, e[0]);
    }
    ...
    if (elt_is_deleted(w, user_data)) {
        ...
        vec_foreach (l, w->levels) {
            clib_bitmap_foreach (wi, l->occupancy_bitmap) {
                ...
                if (vec_len(l->elts[wi]) == 0) l->occupancy_bitmap = clib_bitmap_andnoti(l->occupancy_bitmap, wi);
            }
        }

        {
            ...
            pool_foreach (oe, w->overflow_pool) {
                if (oe->user_data == user_data) pool_put(w->overflow_pool, oe);
            }
        }
...
{
    if (!w->deleted_user_data_hash) w->deleted_user_data_hash = hash_create(/* capacity */ 0, /* value bytes */ 0);
...
u64 timing_wheel_next_expiring_elt_time(timing_wheel_t *w)
{
    ...
    min_t  = ~0ULL;
    vec_foreach (l, w->levels) {
        if (!l->occupancy_bitmap) continue;
...

    {
        ...
        if (min_dt != ~0) min_t = w->cpu_time_base + min_dt;
...
static inline void insert_elt(timing_wheel_t *w, timing_wheel_elt_t *e)
{
...
always_inline void validate_expired_elt(timing_wheel_t *w, timing_wheel_elt_t *e, u64 current_cpu_time)
{
    if (CLIB_DEBUG > 0) {
        ...
        ASSERT((e_time >> w->log2_clocks_per_bin) <= (current_cpu_time >> w->log2_clocks_per_bin));
    }
}
...
static u32 *expire_bin(timing_wheel_t *w, uword level_index, uword wheel_index, u64 advance_cpu_time, u32 *expired_user_data)
{
    ...
    if (j < e_len) vec_dec_len(expired_user_data, e_len - j);
        ...
        pool_foreach (oe, w->overflow_pool) {
            ...
            if (0 == ((oe->cpu_time - w->cpu_time_base) >> BITS(e->cpu_time_relative_to_base))) {
                ...
                if (ti <= w->current_time_index) {
                    ...
                    if (!elt_is_deleted(w, oe->user_data)) vec_add1(expired_user_data, oe->user_data);
                }
                else
                         ...
                         u32            *expired_user_data)
{
    ...
    if (level_index + 1 >= vec_len(w->levels)) goto done;

    level = vec_elt_at_index(w->levels, level_index + 1);
    if (!level->occupancy_bitmap) goto done;
    ...
    while (1) {
        ...
        if (clib_bitmap_get_no_check(level->occupancy_bitmap, from_wheel_index)) {
            ...
            vec_foreach (e, es) {
                ...
                if (ti <= advance_time_index) {
                    ...
                    if (!elt_is_deleted(w, e->user_data)) vec_add1(expired_user_data, e->user_data);
                }
                else
        ...
        if (from_wheel_index == to_wheel_index) break;
...
u32 *timing_wheel_advance(timing_wheel_t *w, u64 advance_cpu_time, u32 *expired_user_data, u64 *next_expiring_element_cpu_time)
{
...

    {
        ...
        if (PREDICT_FALSE(current_ti != advance_ti)) {
            if (w->unexpired_elts_pending_insert) vec_set_len(w->unexpired_elts_pending_insert, 0);
            ...
            while (current_ti != advance_ti) {
                ...
                if (c != a) expired_user_data = refill_level(w, level_index, advance_cpu_time, c, a, expired_user_data);
    ...
    for (level_index = 0; level_index < advance_level_index; level_index++) {
        ...
        if (level_index >= vec_len(w->levels)) break;

        level = vec_elt_at_index(w->levels, level_index);
        clib_bitmap_foreach (wi, level->occupancy_bitmap) {
            expired_user_data = expire_bin(w, level_index, wi, advance_cpu_time, expired_user_data);
        }
    }
        ...
        if (level->occupancy_bitmap)
            while (1) {
                if (clib_bitmap_get_no_check(level->occupancy_bitmap, wi))
                    expired_user_data = expire_bin(w, advance_level_index, wi, advance_cpu_time, expired_user_data);

                /* When we jump out, we have already just expired the bin,
                   corresponding to advance_wheel_index */
                if (wi == advance_wheel_index) break;
    ...
    if (next_expiring_element_cpu_time) {
        ...
        if (vec_len(expired_user_data) == n_expired_user_data_before && w->cached_min_cpu_time_on_wheel != 0ULL)
    ...
    if (verbose) {
        ...
        for (l = 0; l < vec_len(w->levels); l++)
            s = format(s,
                       ...
                       indent + 2,
                       l,
                       l < vec_len(w->stats.refills) ? w->stats.refills[l] : (u64) 0);
    }
