                                      ...
                                      clib_time_t                  *clib_time)
{
    ...
    if (clib_time == 0) {
        tb->clib_time = clib_mem_alloc_aligned(sizeof(*clib_time), CLIB_CACHE_LINE_BYTES);
...
__clib_export void clib_timebase_time_to_components(f64 now, clib_timebase_component_t *cp)
{
    ...
    do {
        ...
        if (month == 1 && clib_timebase_is_leap_year(year)) days_in_month++;
...
f64 clib_timebase_offset_from_sunday(u8 *day)
{
    ...
    for (i = 0; i < ARRAY_LEN(day_names_calendar_order); i++) {
        if (!strncmp((char *) day, day_names_calendar_order[i], 3)) return ((f64) i) * 86400.0;
    }
    return 0.0;
}

__clib_export u8 *format_clib_timebase_time(u8 *s, va_list *args)
{
...
uword unformat_clib_timebase_range_hms(unformat_input_t *input, va_list *args)
{
    ...
    start_hour = start_minute = start_second = end_hour = end_minute = end_second = 0;

    if (unformat(input, "%u:%u:%u - %u:%u:%u", &start_hour, &start_minute, &start_second, &end_hour, &end_minute, &end_second))
        ;
    else if (unformat(input, "%u:%u - %u:%u", &start_hour, &start_minute, &end_hour, &end_minute))
        ;
...
__clib_export uword unformat_clib_timebase_range_vector(unformat_input_t *input, va_list *args)
{
    ...
    while (1) {
        if (!day_range_match && unformat(input, "%s - %s", &start_day, &end_day)) {
        ...
        else if (day_range_match && unformat(input, "%U", unformat_clib_timebase_range_hms, tmp)) {
            /* Across the week... */
            for (now = range_start_time_offset; now <= range_end_time_offset; now += 86400.0) {
...
__clib_export f64 clib_timebase_summer_offset(clib_timebase_t *tb, f64 now)
{
    ...
    if (PREDICT_TRUE(now >= tb->cached_year_start && now <= tb->cached_year_end)) {
