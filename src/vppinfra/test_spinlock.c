...
unsigned test_spinlock(spinlock_test_main_t *stm, f64 *elapse_time)
{
    int       error;
    ...
    for (unsigned cpu_mask = stm->cpu_mask; cpu_mask; cpu_mask >>= 1) {
        ...
        for (uword t_num = 0; t_num < stm->threads_per_core; t_num++) {
            ...
            if ((error = pthread_create(&pthread[t_index], NULL, &inc_shared_counter, stm)))
...
int test_spinlock_main(unformat_input_t *i)
{
    spinlock_test_main_t _stm, *stm = &_stm;
    clib_memset(stm, 0, sizeof(spinlock_test_main_t));

    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (0 == unformat(i, "threads/core %d", &stm->threads_per_core) && 0 == unformat(i, "cpu_mask %x", &stm->cpu_mask)
            && 0 == unformat(i, "increment %d", &stm->increment_per_thread) && 0 == unformat(i, "iterations %d", &stm->iterations)) {
    ...
    uword total_increment = stm->threads_per_core * stm->num_cores * stm->increment_per_thread;
