...
__clib_export uword unformat_bitmap_mask(unformat_input_t *input, va_list *va)
{
    ...
    if (unformat(input, "%U", unformat_hex_string, &v)) {
        ...
        for (i = 0; s >= 0; i++, s--)
            bitmap = clib_bitmap_set_multiple(bitmap, s * BITS(v[i]), v[i], BITS(v[i]));
...
__clib_export u8 *format_bitmap_hex(u8 *s, va_list *args)
{
    ...
    while (i > 0) {
        ...
        if (x && is_trailing_zero) is_trailing_zero = 0;

        if (x || !is_trailing_zero) s = format(s, "%x", x);
    }
    return s;
}
...
__clib_export u8 *format_bitmap_list(u8 *s, va_list *args)
{
    ...
    while (1) {
        ...
        if ((fs = clib_bitmap_next_set(bitmap, fc)) == ~0) return s;
        s = format(s, ", ");
    }
}
