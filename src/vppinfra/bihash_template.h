...
typedef BVS(clib_bihash_alloc_chunk)
{
...
static inline void BV(clib_bihash_increment_stat)(BVT(clib_bihash) * h, int stat_id, u64 count)
{
#if BIHASH_ENABLE_STATS
    if (PREDICT_FALSE(h->inc_stats_callback != 0)) h->inc_stats_callback(h, stat_id, count);
#endif
}

#if BIHASH_ENABLE_STATS
static inline void BV(clib_bihash_set_stats_callback)(BVT(clib_bihash) * h, void (*cb)(BVT(clib_bihash) *, int, u64), void *ctx)
{
...
static inline void BV(clib_bihash_unlock_bucket)(BVT(clib_bihash_bucket) * b)
{
    b->lock = 0;
}

static inline void *BV(clib_bihash_get_value)(BVT(clib_bihash) * h, uword offset)
{
...
static inline int BV(clib_bihash_bucket_is_empty)(BVT(clib_bihash_bucket) * b)
{
...
typedef int (*BV(clib_bihash_foreach_key_value_pair_cb))(BVT(clib_bihash_kv) *, void *);
...
static inline BVT(clib_bihash_bucket) * BV(clib_bihash_get_bucket)(BVT(clib_bihash) * h, u64 hash)
{
#if BIHASH_KVP_AT_BUCKET_LEVEL
    uword offset;
    offset = (hash & (h->nbuckets - 1));
    offset = offset * (sizeof(BVT(clib_bihash_bucket)) + (BIHASH_KVP_PER_PAGE * sizeof(BVT(clib_bihash_kv))));
...
static inline int BV(clib_bihash_search_inline_with_hash)(BVT(clib_bihash) * h, u64 hash, BVT(clib_bihash_kv) * key_result)
{
    ...
    if (PREDICT_FALSE(h->instantiated == 0)) return -1;
#endif

    b = BV(clib_bihash_get_bucket)(h, hash);

    if (PREDICT_FALSE(BV(clib_bihash_bucket_is_empty)(b))) return -1;
    ...
    for (i = 0; i < limit; i++) {
        if (BV(clib_bihash_key_compare)(v->kvp[i].key, key_result->key)) {
            rv = v->kvp[i];
            if (BV(clib_bihash_is_free)(&rv)) return -1;
            *key_result = rv;
            return 0;
        }
    }
...
static inline int BV(clib_bihash_search_inline)(BVT(clib_bihash) * h, BVT(clib_bihash_kv) * key_result)
{
    u64 hash;
...
static inline void BV(clib_bihash_prefetch_bucket)(BVT(clib_bihash) * h, u64 hash)
{
    CLIB_PREFETCH(BV(clib_bihash_get_bucket)(h, hash), BIHASH_BUCKET_PREFETCH_CACHE_LINES * CLIB_CACHE_LINE_BYTES, LOAD);
}

static inline void BV(clib_bihash_prefetch_data)(BVT(clib_bihash) * h, u64 hash)
{
    BVT(clib_bihash_value) * v;
    BVT(clib_bihash_bucket) * b;
    ...
    if (PREDICT_FALSE(BV(clib_bihash_bucket_is_empty)(b))) return;

    v = BV(clib_bihash_get_value)(h, b->offset);

    if (PREDICT_FALSE(b->log2_pages && b->linear_search == 0)) v += extract_bits(hash, h->log2_nbuckets, b->log2_pages);

    CLIB_PREFETCH(v, BIHASH_KVP_PER_PAGE * sizeof(BVT(clib_bihash_kv)), LOAD);
}

static inline int BV(clib_bihash_search_inline_2_with_hash)(BVT(clib_bihash) * h,
                                                            u64 hash,
...
static inline int BV(clib_bihash_search_inline_2)(BVT(clib_bihash) * h, BVT(clib_bihash_kv) * search_key, BVT(clib_bihash_kv) * valuep)
{
    u64 hash;
