...
always_inline clib_ring_header_t *clib_ring_header(void *v)
{
    return vec_header(v);
}

always_inline void clib_ring_reset(void *v)
{
...
always_inline void clib_ring_new_inline(void **p, u32 elt_bytes, u32 size, u32 align)
{
    void      *v;
    vec_attr_t va = {.elt_sz = elt_bytes, .hdr_sz = sizeof(clib_ring_header_t), .align = align};
...
always_inline u32 clib_ring_n_enq(void *v)
{
    clib_ring_header_t *h = clib_ring_header(v);
    return h->n_enq;
}

always_inline void *clib_ring_get_last_inline(void *v, u32 elt_bytes, int enqueue)
{
    clib_ring_header_t *h = clib_ring_header(v);
    u32                 slot;

    if (enqueue) {
        if (h->n_enq == _vec_len(v)) return 0;
        slot = h->next;
        h->n_enq++;
        h->next++;
        if (h->next == _vec_len(v)) h->next = 0;
    }
    else {
        if (h->n_enq == 0) return 0;
...
always_inline void *clib_ring_get_first_inline(void *v, u32 elt_bytes, int dequeue)
{
    clib_ring_header_t *h = clib_ring_header(v);
    u32                 slot;
