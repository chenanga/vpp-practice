...
static_always_inline clib_dt_node_t *clib_dt_node_add_child(clib_dt_main_t *dm, clib_dt_node_t *n, char *name)
{
    clib_dt_node_t *cn;
...
void clib_dt_main_free(clib_dt_main_t *dm)
{
...
#ifdef __linux
__clib_export clib_error_t *clib_dt_read_from_sysfs(clib_dt_main_t *dm)
{
    DIR            *dir, **dir_stack = 0;
    struct dirent  *e;
    clib_dt_node_t *n;
    ...
    while (1) {
        ...
        if (e->d_type == DT_REG) {
            ...
            if (fd >= 0) {
                ...
                if (fstat(fd, &st) == 0) {
                    ...
                    if (read(fd, p->data, st.st_size) == st.st_size) {
                        ...
                        if (strncmp("name", p->name, 5) == 0) n->name = p;
                        if ((strncmp("phandle", p->name, 8) == 0) && (p->size == 4)) {
                            u32 phandle = clib_net_to_host_u32(*(u32u *) p->data);
            ...
            DIR *subdir;
            if (strncmp(".", e->d_name, 2) == 0 || strncmp("..", e->d_name, 3) == 0) continue;
            ...
            err = clib_error_return(0, "unknown entry %s [%u]", e->d_name, e->d_type);
    }

done:
    if (err) clib_dt_main_free(dm);
...
clib_dt_node_t *clib_dt_get_child_node(clib_dt_node_t *n, char *name)
{
...
__clib_export clib_dt_node_t *clib_dt_get_node_with_path(clib_dt_main_t *dm, char *fmt, ...)
{
    u8    *s;
    uword *p;
    ...
    if (p) return (clib_dt_node_t *) p[0];

    return 0;
}

__clib_export clib_dt_property_t *clib_dt_get_node_property_by_name(clib_dt_node_t *n, char *name)
{
    vec_foreach_pointer (p, n->properties)
        if (strncmp(name, p->name, sizeof(p->name)) == 0) return p;
    return 0;
}

__clib_export int clib_dt_node_is_compatible(clib_dt_node_t *n, char *comp)
{
    ...
    for (u32 i = 1, len = 1; i <= p->size; i++) {
        if (p->data[i - 1] == 0) {
            if (strncmp(comp, s, len) == 0) return 1;
            s   = (char *) p->data + i;
            len = 1;
        }
        else
...
__clib_export u8 *format_clib_dt_property_data(u8 *s, va_list *args)
{
    ...
    if (is_printable) {
        ...
        if (n_nulls) {
            for (u32 i = 2; i < p->size; i++)
                if (((u8 *) p->data)[i - 1] == 0) s = format(s, ", '%s'", ((u8 *) p->data) + i);
        }
    }
    else {
...
__clib_export clib_dt_node_t *clib_dt_dereference_node(clib_dt_node_t *n, char *name)
{
    clib_dt_property_t *p;
    uword              *h;

    p = clib_dt_get_node_property_by_name(n, name);
    if (!p || (p->size != sizeof(u32))) return 0;

    h = hash_get(n->dt_main->node_by_phandle, clib_net_to_host_u32(*(u32u *) p->data));

    if (h) return (clib_dt_node_t *) h[0];
