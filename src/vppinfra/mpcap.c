...
__clib_export clib_error_t *mpcap_close(mpcap_main_t *pm)
{
    ...
    /* Not open? Done... */
    if ((pm->flags & MPCAP_FLAG_INIT_DONE) == 0) return 0;
    ...
    if ((pm->flags & MPCAP_FLAG_WRITE_ENABLE) == 0) return 0;

    if (truncate(pm->file_name, actual_size) < 0) clib_unix_warning("setting file size to %llu", actual_size);
...
__clib_export clib_error_t *mpcap_init(mpcap_main_t *pm)
{
    ...
    if (pm->flags & MPCAP_FLAG_INIT_DONE) return 0;

    if (!pm->file_name) pm->file_name = "/tmp/vppinfra.mpcap";

    if (pm->flags & MPCAP_FLAG_THREAD_SAFE) clib_spinlock_init(&pm->lock);

    fd = open(pm->file_name, O_CREAT | O_TRUNC | O_RDWR, 0664);
    if (fd < 0) {
        return clib_error_return_unix(0, "failed to create `%s'", pm->file_name);
    }

    if (pm->max_file_size == 0ULL) pm->max_file_size = MPCAP_DEFAULT_FILE_SIZE;
    ...
    pm->file_baseva = mmap(0, pm->max_file_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
...
clib_error_t *mpcap_map(mpcap_main_t *pm)
{
    ...
    if ((statb.st_mode & S_IFREG) == 0) {
        error = clib_error_return(0, "'%s' is not a regular file", pm->file_name);
        goto done;
    }
    ...
    while (ph < (mpcap_packet_header_t *) pm->file_baseva + pm->max_file_size) {
        if (ph->n_packet_bytes_stored_in_file == 0) break;

        packets_read++;
        min_packet_bytes = ph->n_packet_bytes_stored_in_file < min_packet_bytes ? ph->n_packet_bytes_stored_in_file : min_packet_bytes;
        max_packet_bytes = ph->n_packet_bytes_stored_in_file > max_packet_bytes ? ph->n_packet_bytes_stored_in_file : max_packet_bytes;

        ph = (mpcap_packet_header_t *) (((u8 *) (ph)) + sizeof(*ph) + ph->n_packet_bytes_stored_in_file);
    }
