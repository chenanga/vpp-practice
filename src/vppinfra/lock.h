...
static inline void clib_spinlock_init(clib_spinlock_t *p)
{
    *p = clib_mem_alloc_aligned(CLIB_CACHE_LINE_BYTES, CLIB_CACHE_LINE_BYTES);
    clib_memset((void *) *p, 0, CLIB_CACHE_LINE_BYTES);
}

static inline void clib_spinlock_free(clib_spinlock_t *p)
{
    if (*p) {
...
{
    if (PREDICT_FALSE(CLIB_SPINLOCK_IS_LOCKED(p))) return 0;
...
{
    if (PREDICT_FALSE(*p != 0)) clib_spinlock_lock(p);
}

static_always_inline int clib_spinlock_trylock_if_init(clib_spinlock_t *p)
{
    if (PREDICT_FALSE(*p != 0)) return clib_spinlock_trylock(p);
    return 1;
}
...
{
    if (PREDICT_FALSE(*p != 0)) clib_spinlock_unlock(p);
}
...
always_inline void clib_rwlock_init(clib_rwlock_t *p)
{
    *p = clib_mem_alloc_aligned(CLIB_CACHE_LINE_BYTES, CLIB_CACHE_LINE_BYTES);
    clib_memset((void *) *p, 0, CLIB_CACHE_LINE_BYTES);
}

always_inline void clib_rwlock_free(clib_rwlock_t *p)
{
    if (*p) {
        clib_mem_free((void *) *p);
        *p = 0;
...
always_inline void clib_rwlock_reader_lock(clib_rwlock_t *p)
{
    i32 cnt;
    do {
        ...
        while ((cnt = clib_atomic_load_relax_n(&(*p)->rw_cnt)) < 0)
            CLIB_PAUSE();
    } while (!clib_atomic_cmp_and_swap_acq_relax_n(&(*p)->rw_cnt, &cnt, cnt + 1, 1));
    CLIB_LOCK_DBG(p);
}

always_inline void clib_rwlock_reader_unlock(clib_rwlock_t *p)
{
    ASSERT((*p)->rw_cnt > 0);
    CLIB_LOCK_DBG_CLEAR(p);
...
always_inline void clib_rwlock_writer_lock(clib_rwlock_t *p)
{
    i32 cnt = 0;
    do {
...
always_inline void clib_rwlock_writer_unlock(clib_rwlock_t *p)
{
    CLIB_LOCK_DBG_CLEAR(p);
