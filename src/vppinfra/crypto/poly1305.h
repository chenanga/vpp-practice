...
static_always_inline void clib_poly1305_init(clib_poly1305_ctx *ctx, const u8 key[32])
{
    u64u *k = (u64u *) key;
    u64  *h = (u64 *) ctx->h;
    u64  *r = (u64 *) ctx->r;
...
static_always_inline void _clib_poly1305_multiply_and_reduce(u64 h[3], const u64 r[3])
{
    union {
        struct {
            u64 lo, hi;
...
static_always_inline u32 _clib_poly1305_add_blocks(clib_poly1305_ctx *ctx, const u8 *msg, uword n_bytes, const u32 bit17)
{
    u64 r[3], h[3];
...
static_always_inline void clib_poly1305_update(clib_poly1305_ctx *ctx, const u8 *msg, uword len)
{
    uword n_left = len;
    ...
    if (ctx->n_partial_bytes) {
        ...
        if (PREDICT_FALSE(n_left < missing_bytes)) {
            clib_memcpy_fast(ctx->partial.as_u8 + ctx->n_partial_bytes, msg, n_left);
            ctx->n_partial_bytes += n_left;
            return;
        }

        clib_memcpy_fast(ctx->partial.as_u8 + ctx->n_partial_bytes, msg, missing_bytes);
...
static_always_inline void clib_poly1305_final(clib_poly1305_ctx *ctx, u8 *out)
{
...
static_always_inline void clib_poly1305(const u8 *key, const u8 *msg, uword len, u8 *out)
{
