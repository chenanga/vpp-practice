...
aes_ctr_one_block(aes_ctr_ctx_t *ctx, aes_counter_t ctr, const u8 *src, u8 *dst, u32 n_parallel, u32 n_bytes, int rounds, int last)
{
    u32 __clib_aligned(N_AES_BYTES)
    inc[]                       = {N_AES_LANES, 0, 0, 0, N_AES_LANES, 0, 0, 0, N_AES_LANES, 0, 0, 0, N_AES_LANES, 0, 0, 0};
    ...
    if (last) d[n_parallel - 1] = aes_load_partial((u8 *) (sv + n_parallel - 1), n_bytes);
...
static_always_inline void clib_aes_ctr_init(aes_ctr_ctx_t *ctx, const aes_ctr_key_data_t *kd, const u8 *iv, aes_key_size_t ks)
{
    u32x4 ctr = (u32x4) u8x16_reflect(*(u8x16u *) iv);
#if N_AES_LANES == 4
    ctx->ctr = (aes_counter_t) u32x16_splat_u32x4(ctr) + (u32x16){0, 0, 0, 0, 1, 0, 0, 0, 2, 0, 0, 0, 3, 0, 0, 0};
#elif N_AES_LANES == 2
    ctx->ctr = (aes_counter_t) u32x8_splat_u32x4(ctr) + (u32x8){0, 0, 0, 0, 1, 0, 0, 0};
#else
    ctx->ctr = ctr;
#endif
...
static_always_inline void clib_aes_ctr_transform(aes_ctr_ctx_t *ctx, const u8 *src, u8 *dst, u32 n_bytes, aes_key_size_t ks)
{
...
static_always_inline void clib_aes_ctr_key_expand(aes_ctr_key_data_t *kd, const u8 *key, aes_key_size_t ks)
{
...
static_always_inline void clib_aes128_ctr(const aes_ctr_key_data_t *kd, const u8 *src, u32 n_bytes, const u8 *iv, u8 *dst)
{
    aes_ctr_ctx_t ctx;
...
static_always_inline void clib_aes192_ctr(const aes_ctr_key_data_t *kd, const u8 *src, u32 n_bytes, const u8 *iv, u8 *dst)
{
    aes_ctr_ctx_t ctx;
...
static_always_inline void clib_aes256_ctr(const aes_ctr_key_data_t *kd, const u8 *src, u32 n_bytes, const u8 *iv, u8 *dst)
{
    aes_ctr_ctx_t ctx;
