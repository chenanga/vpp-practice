...
typedef enum { AES_GCM_OP_UNKNONW = 0, AES_GCM_OP_ENCRYPT, AES_GCM_OP_DECRYPT, AES_GCM_OP_GMAC } aes_gcm_op_t;
...
static_always_inline u8x16 aes_gcm_final_block(aes_gcm_ctx_t *ctx)
{
    return (u8x16) ((u64x2){ctx->data_bytes, ctx->aad_bytes} << 3);
}

static_always_inline void aes_gcm_ghash_mul_first(aes_gcm_ctx_t *ctx, aes_data_t data, u32 n_lanes)
{
...
static_always_inline void aes_gcm_ghash_mul_next(aes_gcm_ctx_t *ctx, aes_data_t data)
{
#if N_AES_LANES == 4
...
static_always_inline void aes_gcm_ghash_mul_final_block(aes_gcm_ctx_t *ctx)
{
#if N_AES_LANES == 4
...
static_always_inline void aes_gcm_enc_ctr0_round(aes_gcm_ctx_t *ctx, int aes_round)
{
    if (aes_round == 0)
        ctx->EY0 ^= ctx->Ke[0].x1;
...
static_always_inline void aes_gcm_ghash(aes_gcm_ctx_t *ctx, u8 *data, u32 n_left)
{
    uword            i;
    aes_data_t       r = {};
    ...
    if (n_left > 0) {
        ...
        if (ctx->operation == AES_GCM_OP_GMAC) n_lanes++;
        ...
        if (ctx->operation == AES_GCM_OP_GMAC) aes_gcm_ghash_mul_final_block(ctx);
        aes_gcm_ghash_reduce(ctx);
        aes_gcm_ghash_reduce2(ctx);
        aes_gcm_ghash_final(ctx);
    }
    else if (ctx->operation == AES_GCM_OP_GMAC)
        ctx->T = ghash_mul(aes_gcm_final_block(ctx) ^ ctx->T, ctx->Hi[NUM_HI - 1]);
...
static_always_inline void aes_gcm_enc_first_round(aes_gcm_ctx_t *ctx, aes_data_t *r, uword n_blocks)
{
...
#if N_AES_LANES == 4
    const u32x16 ctr_inv_4444 = {0, 0, 0, 4 << 24, 0, 0, 0, 4 << 24, 0, 0, 0, 4 << 24, 0, 0, 0, 4 << 24};
...
static_always_inline void aes_gcm_enc_last_round(aes_gcm_ctx_t *ctx, aes_data_t *r, aes_data_t *d, const aes_expaned_key_t *Ke, uword n_blocks)
{
...
static_always_inline void aes_gcm_calc(aes_gcm_ctx_t *ctx, aes_data_t *d, const u8 *src, u8 *dst, u32 n, u32 n_bytes, int with_ghash)
{
...
#if N_AES_LANES != 1
        if (ctx->last) n_lanes = (n_bytes + 15) / 16;
#endif
    }
    ...
    if (ctx->operation == AES_GCM_OP_DECRYPT) {
        ...
        if (ctx->last) d[n - 1] = aes_load_partial((u8 *) (sv + n - 1), n_bytes);
    }

    /* GHASH multiply block 0 */
    if (with_ghash) aes_gcm_ghash_mul_first(ctx, d[0], n_lanes);
    ...
    if (with_ghash && gc++ < ghash_blocks) aes_gcm_ghash_mul_next(ctx, (d[1]));
    ...
    if (with_ghash && gc++ < ghash_blocks) aes_gcm_ghash_mul_next(ctx, (d[2]));
    ...
    if (with_ghash && gc++ < ghash_blocks) aes_gcm_ghash_mul_next(ctx, (d[3]));
    ...
    if (ctx->operation == AES_GCM_OP_ENCRYPT) {
        ...
        if (ctx->last) d[n - 1] = aes_load_partial(sv + n - 1, n_bytes);
    }
    ...
    if (ctx->last) aes_store_partial(d[n - 1], dv + n - 1, n_bytes);

    /* GHASH reduce 1st step */
    aes_gcm_ghash_reduce(ctx);

    /* GHASH reduce 2nd step */
    if (with_ghash) aes_gcm_ghash_reduce2(ctx);

    /* GHASH final step */
    if (with_ghash) aes_gcm_ghash_final(ctx);
}

static_always_inline void aes_gcm_calc_double(aes_gcm_ctx_t *ctx, aes_data_t *d, const u8 *src, u8 *dst)
{
    const aes_expaned_key_t *k  = ctx->Ke;
    const aes_mem_t         *sv = (aes_mem_t *) src;
    aes_mem_t               *dv = (aes_mem_t *) dst;
...
static_always_inline void aes_gcm_mask_bytes(aes_data_t *d, uword n_bytes)
{
    const union {
        u8         b[64];
        aes_data_t r;
...
static_always_inline void aes_gcm_calc_last(aes_gcm_ctx_t *ctx, aes_data_t *d, int n_blocks, u32 n_bytes)
{
    ...
    if (n_bytes != N_AES_BYTES) aes_gcm_mask_bytes(d + n_blocks - 1, n_bytes);
    ...
    if (n_blocks > 1) aes_gcm_ghash_mul_next(ctx, d[1]);

    aes_gcm_enc_ctr0_round(ctx, 4);
    aes_gcm_enc_ctr0_round(ctx, 5);

    if (n_blocks > 2) aes_gcm_ghash_mul_next(ctx, d[2]);

    aes_gcm_enc_ctr0_round(ctx, 6);
    aes_gcm_enc_ctr0_round(ctx, 7);

    if (n_blocks > 3) aes_gcm_ghash_mul_next(ctx, d[3]);
...
static_always_inline void aes_gcm_enc(aes_gcm_ctx_t *ctx, const u8 *src, u8 *dst, u32 n_left)
{
    aes_data_t d[4];
...
static_always_inline void aes_gcm_dec(aes_gcm_ctx_t *ctx, const u8 *src, u8 *dst, uword n_left)
{
    aes_data_t  d[4] = {};
    ghash_ctx_t gd;
    ...
    ghash_mul_first(&gd, aes_gcm_final_block(ctx) ^ ctx->T, ctx->Hi[NUM_HI - 1]);
...
static_always_inline int aes_gcm(const u8                 *src,
                                 u8                       *dst,
                                 const u8                 *aad,
                                 u8                       *ivp,
                                 u8                       *tag,
                                 ...
                                 aes_gcm_op_t              op)
{
    ...
    aes_gcm_ctx_t
        _ctx = {.counter = 2, .rounds = aes_rounds, .operation = op, .data_bytes = data_bytes, .aad_bytes = aad_bytes, .Ke = kd->Ke, .Hi = kd->Hi},
        *ctx = &_ctx;
...
#elif N_AES_LANES == 2
    ctx->Y = u32x8_splat_u32x4(Y0) + (u32x8){0, 0, 0, 1 << 24, 0, 0, 0, 2 << 24};
#else
    ...
    if (op == AES_GCM_OP_ENCRYPT || op == AES_GCM_OP_GMAC) {
        ...
        if (tag_len) {
            ...
            if ((u8x16_msb_mask(expected == ctx->T) & mask) == mask) return 1;
        }
        else {
            if (u8x16_is_equal(ctx->T, *(u8x16u *) tag)) return 1;
        }
    }
    return 0;
}

static_always_inline void clib_aes_gcm_key_expand(aes_gcm_key_data_t *kd, const u8 *key, aes_key_size_t ks)
{
    u8x16              H;
...
static_always_inline void clib_aes128_gcm_enc(const aes_gcm_key_data_t *kd,
                                              const u8                 *plaintext,
                                              u32                       data_bytes,
                                              const u8                 *aad,
                                              u32                       aad_bytes,
                                              ...
                                              u8                       *tag)
{
    aes_gcm(plaintext, cyphertext, aad, (u8 *) iv, tag, data_bytes, aad_bytes, tag_bytes, kd, AES_KEY_ROUNDS(AES_KEY_128), AES_GCM_OP_ENCRYPT);
}

static_always_inline void clib_aes256_gcm_enc(const aes_gcm_key_data_t *kd,
                                              const u8                 *plaintext,
                                              u32                       data_bytes,
                                              const u8                 *aad,
                                              u32                       aad_bytes,
                                              ...
                                              u8                       *tag)
{
    aes_gcm(plaintext, cyphertext, aad, (u8 *) iv, tag, data_bytes, aad_bytes, tag_bytes, kd, AES_KEY_ROUNDS(AES_KEY_256), AES_GCM_OP_ENCRYPT);
}

static_always_inline int clib_aes128_gcm_dec(const aes_gcm_key_data_t *kd,
                                             const u8                 *cyphertext,
                                             u32                       data_bytes,
                                             const u8                 *aad,
                                             u32                       aad_bytes,
...
static_always_inline int clib_aes256_gcm_dec(const aes_gcm_key_data_t *kd,
                                             const u8                 *cyphertext,
                                             u32                       data_bytes,
                                             const u8                 *aad,
                                             u32                       aad_bytes,
...
static_always_inline void clib_aes128_gmac(const aes_gcm_key_data_t *kd, const u8 *data, u32 data_bytes, const u8 *iv, u32 tag_bytes, u8 *tag)
{
    aes_gcm(0, 0, data, (u8 *) iv, tag, 0, data_bytes, tag_bytes, kd, AES_KEY_ROUNDS(AES_KEY_128), AES_GCM_OP_GMAC);
}

static_always_inline void clib_aes256_gmac(const aes_gcm_key_data_t *kd, const u8 *data, u32 data_bytes, const u8 *iv, u32 tag_bytes, u8 *tag)
{
    aes_gcm(0, 0, data, (u8 *) iv, tag, 0, data_bytes, tag_bytes, kd, AES_KEY_ROUNDS(AES_KEY_256), AES_GCM_OP_GMAC);
}
