...
#define SHA256_CSIGMA0(x)   (SHA256_ROTR(x, 2) ^ SHA256_ROTR(x, 13) ^ SHA256_ROTR(x, 22));
#define SHA256_CSIGMA1(x)   (SHA256_ROTR(x, 6) ^ SHA256_ROTR(x, 11) ^ SHA256_ROTR(x, 25));
#define SHA256_SSIGMA0(x)   (SHA256_ROTR(x, 7) ^ SHA256_ROTR(x, 18) ^ (x >> 3))
#define SHA256_SSIGMA1(x)   (SHA256_ROTR(x, 17) ^ SHA256_ROTR(x, 19) ^ (x >> 10))
...
#define SHA512_CSIGMA0(x)   (SHA512_ROTR(x, 28) ^ SHA512_ROTR(x, 34) ^ SHA512_ROTR(x, 39))
#define SHA512_CSIGMA1(x)   (SHA512_ROTR(x, 14) ^ SHA512_ROTR(x, 18) ^ SHA512_ROTR(x, 41))
#define SHA512_SSIGMA0(x)   (SHA512_ROTR(x, 1) ^ SHA512_ROTR(x, 8) ^ (x >> 7))
#define SHA512_SSIGMA1(x)   (SHA512_ROTR(x, 19) ^ SHA512_ROTR(x, 61) ^ (x >> 6))
...
static const u32 sha256_h[8] = {0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19};
...
static const u64 sha384_h[8] = {0xcbbb9d5dc1059ed8,
                                0x629a292a367cd507,
                                0x9159015a3070dd17,
                                0x152fecd8f70e5939,
                                0x67332667ffc00b31,
...
static const u64 sha512_h[8] = {0x6a09e667f3bcc908,
                                0xbb67ae8584caa73b,
                                0x3c6ef372fe94f82b,
                                0xa54ff53a5f1d36f1,
                                0x510e527fade682d1,
...
static const u64 sha512_224_h[8] = {0x8c3d37c819544da2,
                                    0x73e1996689dcd4d6,
                                    0x1dfab7ae32ff9c82,
                                    0x679dd514582f9fcf,
                                    0x0f6d2b697bd44da8,
...
static const u64 sha512_256_h[8] = {0x22312194fc2bf72c,
                                    0x9f555fa3c84c64c2,
                                    0x2393b86b6f53b151,
                                    0x963877195940eabd,
                                    0x96283ee2a88effe3,
...
static const u64 clib_sha2_512_k[80] = {
    0x428a2f98d728ae22, 0x7137449123ef65cd, 0xb5c0fbcfec4d3b2f, 0xe9b5dba58189dbbc, 0x3956c25bf348b538, 0x59f111f1b605d019, 0x923f82a4af194f9b,
    0xab1c5ed5da6d8118, 0xd807aa98a3030242, 0x12835b0145706fbe, 0x243185be4ee4b28c, 0x550c7dc3d5ffb4e2, 0x72be5d74f27b896f, 0x80deb1fe3b1696b1,
    0x9bdc06a725c71235, 0xc19bf174cf692694, 0xe49b69c19ef14ad2, 0xefbe4786384f25e3, 0x0fc19dc68b8cd5b5, 0x240ca1cc77ac9c65, 0x2de92c6f592b0275,
    0x4a7484aa6ea6e483, 0x5cb0a9dcbd41fbd4, 0x76f988da831153b5, 0x983e5152ee66dfab, 0xa831c66d2db43210, 0xb00327c898fb213f, 0xbf597fc7beef0ee4,
    0xc6e00bf33da88fc2, 0xd5a79147930aa725, 0x06ca6351e003826f, 0x142929670a0e6e70, 0x27b70a8546d22ffc, 0x2e1b21385c26c926, 0x4d2c6dfc5ac42aed,
    0x53380d139d95b3df, 0x650a73548baf63de, 0x766a0abb3c77b2a8, 0x81c2c92e47edaee6, 0x92722c851482353b, 0xa2bfe8a14cf10364, 0xa81a664bbc423001,
    0xc24b8b70d0f89791, 0xc76c51a30654be30, 0xd192e819d6ef5218, 0xd69906245565a910, 0xf40e35855771202a, 0x106aa07032bbd1b8, 0x19a4c116b8d2d0c8,
    0x1e376c085141ab53, 0x2748774cdf8eeb99, 0x34b0bcb5e19b48a8, 0x391c0cb3c5c95a63, 0x4ed8aa4ae3418acb, 0x5b9cca4f7763e373, 0x682e6ff3d6b2b8a3,
    0x748f82ee5defb2fc, 0x78a5636f43172f60, 0x84c87814a1f0ab72, 0x8cc702081a6439ec, 0x90befffa23631e28, 0xa4506cebde82bde9, 0xbef9a3f7b2c67915,
    0xc67178f2e372532b, 0xca273eceea26619c, 0xd186b8c721c0c207, 0xeada7dd6cde0eb1e, 0xf57d4f7fee6ed178, 0x06f067aa72176fba, 0x0a637dc5a2c898a6,
    0x113f9804bef90dae, 0x1b710b35131c471b, 0x28db77f523047d84, 0x32caab7b40c72493, 0x3c9ebe0a15c9bebc, 0x431d67c49c100d4c, 0x4cc5d4becb3e42b6,
...
static_always_inline void clib_sha2_state_init(clib_sha2_state_t *state, clib_sha2_type_t type)
{
...
#ifdef CLIB_SHA256_ISA
static inline void clib_sha256_vec_cycle_w(u32x4 w[], u8 i)
{
    u8 j = (i + 1) % 4;
    u8 k = (i + 2) % 4;
    u8 l = (i + 3) % 4;
...
static inline void clib_sha256_vec_4_rounds(u32x4 w, u8 n, u32x4 s[])
{
#ifdef CLIB_SHA256_ISA_INTEL
    u32x4 r = *(u32x4 *) (clib_sha2_256_k + 4 * n) + w;
    s[0]    = (u32x4) _mm_sha256rnds2_epu32((__m128i) s[0], (__m128i) s[1], (__m128i) r);
    r       = (u32x4) u64x2_interleave_hi((u64x2) r, (u64x2) r);
    s[1]    = (u32x4) _mm_sha256rnds2_epu32((__m128i) s[1], (__m128i) s[0], (__m128i) r);
#elif defined(CLIB_SHA256_ISA_ARM)
    u32x4         r0, s0;
...
static inline u32x4 clib_sha256_vec_load(u32x4 r)
{
...
static inline void clib_sha256_vec_shuffle(u32x4 d[2])
{
#if defined(CLIB_SHA256_ISA_INTEL)
...
static inline void clib_sha256_block(clib_sha2_state_t *st, const u8 *msg, uword n_blocks)
{
#if defined(CLIB_SHA256_ISA)
    u32x4   h[2];
    u32x4u *m = (u32x4u *) msg;
...
static_always_inline void clib_sha512_block(clib_sha2_state_t *st, const u8 *msg, uword n_blocks)
{
    u64           w[80], s[8], i;
    clib_sha2_h_t h;
...
static_always_inline void clib_sha2_update_internal(clib_sha2_state_t *st, u8 block_size, const u8 *msg, uword n_bytes)
{
    uword n_blocks;
    if (st->n_pending) {
...
static_always_inline void clib_sha2_final_internal(clib_sha2_state_t *st, u8 block_size, u8 digest_size, u8 *digest)
{
    int i;
    ...
    st->pending.as_u64[block_size / 8 - 1] = clib_net_to_host_u64(st->total_bytes * 8);
    ...
    if (block_size == CLIB_SHA2_512_BLOCK_SIZE) {
        ...
        if (i * sizeof(u64) < digest_size) ((u32 *) digest)[2 * i] = clib_net_to_host_u32(st->h.h64[i] >> 32);
    }
    else {
        clib_sha256_block(st, st->pending.as_u8, 1);
...
{
    clib_sha2_final_internal(&ctx->state, ctx->block_size, ctx->digest_size, digest);
}
...
static_always_inline void clib_sha2_hmac_key_data(clib_sha2_type_t type, const u8 *key, uword key_len, clib_sha2_hmac_key_data_t *kd)
{
...
static_always_inline void clib_sha2_hmac_init(clib_sha2_hmac_ctx_t *ctx, clib_sha2_type_t type, clib_sha2_hmac_key_data_t *kd)
{
    u8 block_size  = clib_sha2_variants[type].block_size;
...
static_always_inline void clib_sha2_hmac_update(clib_sha2_hmac_ctx_t *ctx, const u8 *msg, uword len)
{
    clib_sha2_update_internal(&ctx->ipad_state, ctx->block_size, msg, len);
}

static_always_inline void clib_sha2_hmac_final(clib_sha2_hmac_ctx_t *ctx, u8 *digest)
{
    u8 i_digest[SHA2_MAX_DIGEST_SIZE];

    clib_sha2_final_internal(&ctx->ipad_state, ctx->block_size, ctx->digest_size, i_digest);
    clib_sha2_update_internal(&ctx->opad_state, ctx->block_size, i_digest, ctx->digest_size);
    clib_sha2_final_internal(&ctx->opad_state, ctx->block_size, ctx->digest_size, digest);
}

static_always_inline void clib_sha2_hmac(clib_sha2_type_t type, const u8 *key, uword key_len, const u8 *msg, uword len, u8 *digest)
{
...
#define clib_hmac_sha512_224(...) clib_sha2_hmac(CLIB_SHA2_512_224, __VA_ARGS__)
