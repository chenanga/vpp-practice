...
static_always_inline u8x16 aes_enc_round_x1(u8x16 a, u8x16 k)
{
#if defined(__AES__)
...
static_always_inline u8x64 aes_enc_round_x4(u8x64 a, u8x64 k)
{
    return (u8x64) _mm512_aesenc_epi128((__m512i) a, (__m512i) k);
}

static_always_inline u8x64 aes_enc_last_round_x4(u8x64 a, u8x64 k)
{
    return (u8x64) _mm512_aesenclast_epi128((__m512i) a, (__m512i) k);
}

static_always_inline u8x64 aes_dec_round_x4(u8x64 a, u8x64 k)
{
    return (u8x64) _mm512_aesdec_epi128((__m512i) a, (__m512i) k);
}

static_always_inline u8x64 aes_dec_last_round_x4(u8x64 a, u8x64 k)
{
...
#ifdef __VAES__
static_always_inline u8x32 aes_enc_round_x2(u8x32 a, u8x32 k)
{
    return (u8x32) _mm256_aesenc_epi128((__m256i) a, (__m256i) k);
}

static_always_inline u8x32 aes_enc_last_round_x2(u8x32 a, u8x32 k)
{
    return (u8x32) _mm256_aesenclast_epi128((__m256i) a, (__m256i) k);
}

static_always_inline u8x32 aes_dec_round_x2(u8x32 a, u8x32 k)
{
    return (u8x32) _mm256_aesdec_epi128((__m256i) a, (__m256i) k);
}

static_always_inline u8x32 aes_dec_last_round_x2(u8x32 a, u8x32 k)
{
    return (u8x32) _mm256_aesdeclast_epi128((__m256i) a, (__m256i) k);
}
#endif

static_always_inline u8x16 aes_enc_last_round_x1(u8x16 a, u8x16 k)
{
#if defined(__AES__)
...
static_always_inline u8x16 aes_dec_round_x1(u8x16 a, u8x16 k)
{
    return (u8x16) _mm_aesdec_si128((__m128i) a, (__m128i) k);
}

static_always_inline u8x16 aes_dec_last_round_x1(u8x16 a, u8x16 k)
{
...
static_always_inline void aes128_key_expand(u8x16 *rk, u8x16u const *k)
{
    rk[0] = k[0];
...
static const u8x16 aese_prep_mask1 = {13, 14, 15, 12, 13, 14, 15, 12, 13, 14, 15, 12, 13, 14, 15, 12};
static const u8x16 aese_prep_mask2 = {12, 13, 14, 15, 12, 13, 14, 15, 12, 13, 14, 15, 12, 13, 14, 15};

static_always_inline void aes128_key_expand_round_neon(u8x16 *rk, u32 rcon)
{
...
static_always_inline void aes192_key_expand_round_neon(u8x8 *rk, u32 rcon)
{
...
static_always_inline void aes192_key_expand(u8x16 *ek, const u8x16u *k)
{
    u8x8 *rk = (u8x8 *) ek;
    ek[0]    = k[0];
    rk[2]    = *(u8x8u *) (k + 1);
...
static_always_inline void aes256_key_expand_round_neon(u8x16 *rk, u32 rcon)
{
    u8x16 r, t, z = {};

    r = vqtbl1q_u8(rk[-1], rcon ? aese_prep_mask1 : aese_prep_mask2);
    r = vaeseq_u8(r, z);
    if (rcon) r ^= (u8x16) vdupq_n_u32(rcon);
    r ^= rk[-2];
...
static_always_inline void aes_enc_round(aes_data_t *r, const aes_expaned_key_t *ek, uword n_blocks)
{
...
static_always_inline void aes_enc_last_round(aes_data_t *r, aes_data_t *d, const aes_expaned_key_t *ek, uword n_blocks)
{
    for (int i = 0; i < n_blocks; i++)
#if N_AES_LANES == 4
