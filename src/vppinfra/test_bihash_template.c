...
static clib_error_t *test_bihash_vec64(test_main_t *tm)
{
...
#if BIHASH_32_64_SVM
    BV(clib_bihash_initiator_init_svm)(h, "test", user_buckets, 0x30000000 /* base_addr */, user_memory_size);
#else
...
static int stale_cb(BVT(clib_bihash_kv) * kv, void *ctx)
{
    test_main_t *tm = ctx;
...
static clib_error_t *test_bihash_stale_overwrite(test_main_t *tm)
{
    BVT(clib_bihash) * h;
    BVT(clib_bihash_kv) kv;
    int i;
...
#if BIHASH_32_64_SVM
    BV(clib_bihash_initiator_init_svm)(h, "test", tm->nbuckets, 0x30000000 /* base_addr */, tm->hash_memory_size);
#else
...
static clib_error_t *test_bihash_threads(test_main_t *tm)
{
    int       i;
    pthread_t handle;
    BVT(clib_bihash) * h;
    ...
    for (i = 0; i < tm->nthreads; i++) {
        rv = pthread_create(&handle, NULL, test_bihash_thread_fn, (void *) (u64) i);
        if (rv) {
...
static clib_error_t *test_bihash_vanilla_overwrite(test_main_t *tm)
{
    int i;
    BVT(clib_bihash) * h;
    BVT(clib_bihash_kv) kv;
...
static clib_error_t *test_bihash_value_assert(test_main_t *tm)
{
    BVT(clib_bihash) * h;
    BVT(clib_bihash_kv) kv;
    ...
    for (acycle = 0; acycle < tm->ncycles; acycle++) {
        ...
        for (j = 0; j < tm->search_iter; j++) {
            for (i = 0; i < tm->nitems; i++) {
                ...
                if (BV(clib_bihash_search)(h, &kv, &kv) < 0)
                    if (BV(clib_bihash_search)(h, &kv, &kv) < 0) clib_warning("[%d] search for key %lld failed unexpectedly\n", i, tm->keys[i]);
                if (kv.value != (u64) (i + 1))
        ...
        for (j = 0; j < tm->search_iter; j++) {
            for (i = 0; i < tm->nitems; i++) {
                ...
                if (p == 0 || p[0] != (uword) (i + 1)) clib_warning("ugh, couldn't find %lld\n", tm->keys[i]);
            }
        }
        ...
        for (i = 0; i < tm->nitems; i++) {
            ...
            if (rv < 0) clib_warning("delete key %lld not ok but should be", tm->keys[i]);
            ...
            if (tm->careful_delete_tests) {
                for (j = 0; j < tm->nitems; j++) {
                    ...
                    if (j <= i && rv >= 0) {
                        clib_warning("i %d j %d search ok but should not be, value %lld", i, j, kv.value);
                    }
                    if (j > i && rv < 0) {
...
clib_error_t *test_bihash_main(test_main_t *tm)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "seed %u", &tm->seed))
            ...
            ;
        else if (unformat(i, "memory-size %U", unformat_memory_size, &tm->hash_memory_size))
            ;
            ...
            return clib_error_return(0, "unknown input '%U'", format_unformat_error, i);
    }
