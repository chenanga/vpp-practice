...
static FORCEINLINE int win32munmap(void* ptr, size_t size)
{
    ...
    while (size) {
        ...
        if (minfo.BaseAddress != cptr || minfo.AllocationBase != cptr || minfo.State != MEM_COMMIT || minfo.RegionSize > size) return -1;
...
__clib_nosanitize_addr static int spin_acquire_lock(int* sl)
{
    int spins = 0;
...
static void init_malloc_global_mutex()
{
    for (;;) {
        ...
        if (stat == 0 && interlockedcompareexchange(&malloc_global_mutex_status, (LONG) -1, (LONG) 0) == 0) {
...
__clib_nosanitize_addr static msegmentptr segment_holding(mstate m, char* addr)
{
...
__clib_nosanitize_addr static int has_segment_link(mstate m, msegmentptr ss)
{
    msegmentptr sp = &m->seg;
    for (;;) {
...
__clib_nosanitize_addr static inline int ok_magic(const mstate m)
{
...
#define set_inuse(M, p, s) \
    ((p)->head = (((p)->head & PINUSE_BIT) | s | CINUSE_BIT), (((mchunkptr) (((char*) (p)) + (s)))->head |= PINUSE_BIT), mark_inuse_foot(M, p, s))

#define set_inuse_and_pinuse(M, p, s) \
    ((p)->head = (s | PINUSE_BIT | CINUSE_BIT), (((mchunkptr) (((char*) (p)) + (s)))->head |= PINUSE_BIT), mark_inuse_foot(M, p, s))
...
static int init_mparams(void)
{
    ...
    if (mparams.magic == 0) {
        ...
        */
        if ((sizeof(size_t) != sizeof(char*)) || (MAX_SIZE_T < MIN_CHUNK_SIZE) || (sizeof(int) < 4) || (MALLOC_ALIGNMENT < (size_t) 8U)
            || ((MALLOC_ALIGNMENT & (MALLOC_ALIGNMENT - SIZE_T_ONE)) != 0) || ((MCHUNK_SIZE & (MCHUNK_SIZE - SIZE_T_ONE)) != 0)
            || ((gsize & (gsize - SIZE_T_ONE)) != 0) || ((psize & (psize - SIZE_T_ONE)) != 0))
            DLM_ABORT;
...
#if !NO_MALLINFO
__clib_nosanitize_addr static struct dlmallinfo internal_mallinfo(mstate m)
{
...
#define unlink_small_chunk(M, P, S)                                                         \
    {                                                                                       \
        ...
        if (RTCHECK(F == smallbin_at(M, I) || (ok_address(M, F) && F->bk == P))) {          \
            ...
            else if (RTCHECK(B == smallbin_at(M, I) || (ok_address(M, B) && B->fd == P))) { \
                F->bk = B;                                                                  \
                B->fd = F;                                                                  \
            }                                                                               \
            else {                                                                          \
...
static void* mmap_alloc(mstate m, size_t nb)
{
    ...
    if (mmsize > nb) { /* Check for wrap around 0 */
        ...
        if (mm != CMFAIL) {
            ...
            if ((m->footprint += mmsize) > m->max_footprint) m->max_footprint = m->footprint;
            assert(is_aligned(chunk2mem(p)));
...
static mchunkptr mmap_resize(mstate m, mchunkptr oldp, size_t nb, int flags)
{
    ...
    if (oldsize >= nb + SIZE_T_SIZE && (oldsize - nb) <= (mparams.granularity << 1))
        ...
        char*  cp        = (char*) CALL_MREMAP((char*) oldp - offset, oldmmsize, newmmsize, flags);
        if (cp != CMFAIL) {
            ...
            if ((m->footprint += newmmsize - oldmmsize) > m->max_footprint) m->max_footprint = m->footprint;
...
__clib_nosanitize_addr static void init_top(mstate m, mchunkptr p, size_t psize)
{
    /* Ensure alignment */
...
__clib_nosanitize_addr static void* prepend_alloc(mstate m, char* newbase, char* oldbase, size_t nb)
{
...
__clib_nosanitize_addr static void add_segment(mstate m, char* tbase, size_t tsize, flag_t mmapped)
{
...
__clib_nosanitize_addr static void* sys_alloc(mstate m, size_t nb)
{
    char*  tbase     = CMFAIL;
    size_t tsize     = 0;
    flag_t mmap_flag = 0;
    ...
    */
    ...
    if (MORECORE_CONTIGUOUS && !use_noncontiguous(m)) {
        ...
        if (ss == 0) { /* First time through or recovery */
            ...
            if (base != CMFAIL) {
                ...
                if (!is_page_aligned(base)) ssize += (page_align((size_t) base) - (size_t) base);
                fp = m->footprint + ssize; /* recheck limits */
                if (ssize > nb && ssize < HALF_MAX_SIZE_T && (m->footprint_limit == 0 || (fp > m->footprint && fp <= m->footprint_limit))
            ...
            if (ssize < HALF_MAX_SIZE_T && (br = (char*) (CALL_MORECORE(ssize))) == ss->base + ss->size) {
                tbase = br;
                tsize = ssize;
            }
        }
    ...
    if (tbase != CMFAIL) {
        if ((m->footprint += tsize) > m->max_footprint) m->max_footprint = m->footprint;
...
__clib_nosanitize_addr static size_t release_unused_segments(mstate m)
{
    size_t      released = 0;
    int         nsegs    = 0;
    ...
    /* Reset check counter */
    m->release_checks = (((size_t) nsegs > (size_t) MAX_RELEASE_CHECK_RATE) ? (size_t) nsegs : (size_t) MAX_RELEASE_CHECK_RATE);
    return released;
}

__clib_nosanitize_addr static int sys_trim(mstate m, size_t pad)
{
    size_t released = 0;
    ensure_initialization();
...
*/
__clib_nosanitize_addr static void dispose_chunk(mstate m, mchunkptr p, size_t psize)
{
...
__clib_nosanitize_addr static void* tmalloc_large(mstate m, size_t nb)
{
    tchunkptr v     = 0;
...
__clib_nosanitize_addr static void* tmalloc_small(mstate m, size_t nb)
{
    tchunkptr t, v;
    size_t    rsize;
    bindex_t  i;
        ...
        if (!PREACTION(fm)) {
            ...
            if (RTCHECK(ok_address(fm, p) && ok_inuse(p))) {
                ...
                if (!pinuse(p)) {
                    ...
                    if (is_mmapped(p)) {
                        psize += prevsize + MMAP_FOOT_PAD;
                        if (CALL_MUNMAP((char*) p - prevsize, psize) == 0) fm->footprint -= psize;
                        goto postaction;
                    }
                    else {
...
static __clib_nosanitize_addr mchunkptr try_realloc_chunk(mstate m, mchunkptr p, size_t nb, int can_move)
{
    mchunkptr newp    = 0;
    size_t    oldsize = chunksize(p);
    mchunkptr next    = chunk_plus_offset(p, oldsize);
    if (RTCHECK(ok_address(m, p) && ok_inuse(p) && ok_next(p, next) && ok_pinuse(next))) {
        if (is_mmapped(p)) {
...
__clib_nosanitize_addr static void* internal_memalign(mstate m, size_t alignment, size_t bytes)
{
    void* mem = 0;
...
static void internal_inspect_all(mstate m, void (*handler)(void* start, void* end, size_t used_bytes, void* callback_arg), void* arg)
{
    if (is_initialized(m)) {
        mchunkptr   top = m->top;
        msegmentptr s;
...
#if MALLOC_INSPECT_ALL
void dlmalloc_inspect_all(void (*handler)(void* start, void* end, size_t used_bytes, void* callback_arg), void* arg)
{
    ensure_initialization();
    if (!PREACTION(gm)) {
...
__clib_nosanitize_addr size_t destroy_mspace(mspace msp)
{
    size_t freed = 0;
    mstate ms    = (mstate) msp;
    if (ok_magic(ms)) {
        ...
        while (sp != 0) {
            ...
            sp = sp->next;
            if ((flag & USE_MMAP_BIT) && !(flag & EXTERN_BIT) && CALL_MUNMAP(base, size) == 0) freed += size;
        }
    }
    else {
...
__clib_nosanitize_addr int mspace_is_heap_object(mspace msp, void* p)
{
    msegment* this_seg;
    char *    pp, *base;
    mstate    ms;
    ...
    while (this_seg) {
        base = this_seg->base;
        if (pp >= base && pp < (base + this_seg->size)) return 1;
        this_seg = this_seg->next;
    }

    if (pp > ms->least_addr && pp <= ms->least_addr + ms->footprint) return 1;

    return 0;
}

__clib_nosanitize_addr void* mspace_least_addr(mspace msp)
{
    mstate ms = (mstate) msp;
...
__clib_nosanitize_addr int mspace_enable_disable_trace(mspace msp, int enable)
{
    mstate ms          = (mstate) msp;
    int    was_enabled = 0;

    if (use_trace(ms)) was_enabled = 1;
...
__clib_nosanitize_addr int mspace_is_traced(mspace msp)
{
    mstate ms = (mstate) msp;

    if (use_trace(ms)) return 1;
    return 0;
}

__clib_nosanitize_addr void* mspace_get_aligned(mspace msp, unsigned long n_user_data_bytes, unsigned long align, unsigned long align_offset)
{
    char*         rv;
...
__clib_nosanitize_addr void mspace_put(mspace msp, void* p_arg)
{
    char*     object_header;
    unsigned* wwp;
    mstate    ms = (mstate) msp;
...
__clib_nosanitize_addr size_t mspace_usable_size_with_delta(const void* p)
{
    size_t    usable_size;
    char*     object_header;
    unsigned* wwp;
...
__clib_nosanitize_addr void* mspace_malloc(mspace msp, size_t bytes)
{
    mstate ms = (mstate) msp;
    if (!ok_magic(ms)) {
        USAGE_ERROR_ACTION(ms, ms);
...
__clib_nosanitize_addr void mspace_free(mspace msp, void* mem)
{
    if (mem != 0) {
        mchunkptr p = mem2chunk(mem);
#if FOOTERS
...
__clib_nosanitize_addr void* mspace_realloc_in_place(mspace msp, void* oldmem, size_t bytes)
{
    void* mem = 0;
    if (oldmem != 0) {
        if (bytes >= MAX_REQUEST) {
...
__clib_nosanitize_addr void* mspace_memalign(mspace msp, size_t alignment, size_t bytes)
{
    mstate ms = (mstate) msp;
    if (!ok_magic(ms)) {
        USAGE_ERROR_ACTION(ms, ms);
...
#if MALLOC_INSPECT_ALL
void mspace_inspect_all(mspace msp, void (*handler)(void* start, void* end, size_t used_bytes, void* callback_arg), void* arg)
{
    mstate ms = (mstate) msp;
    if (ok_magic(ms)) {
        if (!PREACTION(ms)) {
...
#if !NO_MALLINFO
__clib_nosanitize_addr struct dlmallinfo mspace_mallinfo(mspace msp)
{
    mstate ms = (mstate) msp;
    if (!ok_magic(ms)) {
        USAGE_ERROR_ACTION(ms, ms);
...
__clib_nosanitize_addr size_t mspace_usable_size(const void* mem)
{
    if (mem != 0) {
        mchunkptr p = mem2chunk(mem);
        if (is_inuse(p)) return chunksize(p) - overhead_for(p);
