...
clib_error_t *test_valloc(test_main_t *tm)
{
    ...
    for (i = 0; i < tm->nitems; i++) {
        baseva = clib_valloc_alloc(&tm->valloc_main, 1024, 1 /* fail:os_out_of_memory */);
    ...
    for (i = 0; i < tm->niter; i++) {
        ...
        if (tm->item_in_table[index]) {
            if (0) fformat(stdout, "free [%d] %llx\n", index, tm->basevas[index]);
            ...
            if (0) fformat(stdout, "alloc [%d] %llx\n", index, tm->basevas[index]);
        }
        ...
        if (tm->check_every_add_del) {
            for (j = 0; j < tm->nitems; j++) {
                if (tm->item_in_table[j]) {
                    p = hash_get((&tm->valloc_main)->chunk_index_by_baseva, tm->basevas[j]);
                    if (p) {
                    ...
                    if (p) {
                        ...
                        if ((ip->flags & CLIB_VALLOC_BUSY)) {
                            ...
                            fformat(stdout, "%U\n", format_valloc, &tm->valloc_main, 1 /* verbose */);
    ...
    for (i = 0; i < tm->nitems; i++) {
        if (tm->item_in_table[i]) {
            p = hash_get((&tm->valloc_main)->chunk_index_by_baseva, tm->basevas[i]);
            if (p) {
                ip = pool_elt_at_index((&tm->valloc_main)->chunks, p[0]);
    ...
    for (i = 0; i < tm->nitems; i++) {
        if (tm->item_in_table[i]) clib_valloc_free(&tm->valloc_main, tm->basevas[i]);
    }
...
clib_error_t *test_valloc_main(unformat_input_t *i)
{
