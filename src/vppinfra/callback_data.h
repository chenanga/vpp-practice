...
#define clib_callback_data_ensure(set_, value_)     \
    do {                                            \
        ...
        if (!found_) vec_validate(next_, i_);       \
        next_[i_]    = (value_);                    \
        (set_)->next = next_;                       \
        clib_spinlock_unlock_if_init((set_)->lock); \
    } while (0)
...
#define clib_callback_data_get_value(set_, fp_, v_)             \
    ({                                                          \
        ...
        if (PREDICT_TRUE(search_ == 0)) search_ = (set_)->curr; \
...
#define clib_callback_data_check_and_get(set_)                       \
    ({                                                               \
        ...
        if (PREDICT_FALSE((set_)->next != 0)) {                      \
            ...
            curr_         = (set_)->next;                            \
            (set_)->next  = 0;                                       \
            if (PREDICT_FALSE(0 == vec_len(curr_))) vec_free(curr_); \
            (set_)->curr = curr_;                                    \
            clib_spinlock_unlock_if_init((set_)->lock);              \
        }                                                            \
        curr_;                                                       \
...
#define clib_callback_data_prefetch(set_)                                   \
    do {                                                                    \
        if (PREDICT_FALSE((set_)->curr)) clib_prefetch_store((set_)->curr); \
    } while (0)
