...
static int file_exists_and_is_executable(char *dir, char *file)
{
    ...
    yes = (stat(path, &s) >= 0 && S_ISREG(s.st_mode) && 0 != (s.st_mode & (S_IXUSR | S_IXGRP | S_IXOTH)));
...
static char *path_search(char *file)
{
    ...
    if (getenv("PATH") == 0) return file;

    ps.path = split_string(getenv("PATH"), ':');

    for (i = 0; i < vec_len(ps.path); i++)
        if (file_exists_and_is_executable(ps.path[i], file)) break;

    result = 0;
    if (i < vec_len(ps.path)) result = (char *) format(0, "%s/%s%c", ps.path[i], file, 0);
...
static clib_error_t *clib_elf_parse_file(clib_elf_main_t *cem, char *file_name, void *link_address)
{
    ...
    /* Look for CLIB special sections. */
    {
        ...
        vec_foreach (s, em->sections) {
            ...
            if (strcmp((char *) name, section_name_start)) continue;
            ...
            if (p)
                ...
                if (!cem->section_by_name) cem->section_by_name = hash_create_string(0, sizeof(uword));
    ...
    /* No need to keep section contents around. */
    {
        elf_section_t *s;
        vec_foreach (s, em->sections) {
            if (s->header.type != ELF_SECTION_STRING_TABLE) vec_free(s->contents);
        }
    }

done:
    if (error) elf_main_free(em);
    if (fd >= 0) close(fd);
...
static int add_section(struct dl_phdr_info *info, size_t size, void *opaque)
{
    ...
    if (is_main && name != cem->exec_path) vec_free(name);
...
__clib_export uword clib_elf_symbol_by_name(char *by_name, clib_elf_symbol_t *s)
{
    return symbol_by_address_or_name(by_name, /* by_address */ 0, s);
}

__clib_export uword clib_elf_symbol_by_address(uword by_address, clib_elf_symbol_t *s)
{
    return symbol_by_address_or_name(/* by_name */ 0, by_address, s);
}

__clib_export const char *clib_elf_symbol_name(clib_elf_symbol_t *s)
{
    clib_elf_main_t    *cem = &clib_elf_main;
    elf_main_t         *em;
    elf_symbol_table_t *t;
...
__clib_export u8 *format_clib_elf_symbol(u8 *s, va_list *args)
{
    clib_elf_main_t    *cem = &clib_elf_main;
    ...
    if (!sym) /* Just print table headings. */
...
__clib_export u8 *format_clib_elf_symbol_with_address(u8 *s, va_list *args)
{
    ...
    if (clib_elf_symbol_by_address(address, &sym)) {
        em = vec_elt_at_index(cem->elf_mains, sym.elf_main_index);
        t  = vec_elt_at_index(em->symbol_tables, sym.symbol_table_index);
        s  = format(s, "%s + 0x%wx", elf_symbol_name(t, &sym.symbol), address - sym.symbol.value);
    }
    else
