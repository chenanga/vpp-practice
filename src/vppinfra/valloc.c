...
__clib_export void clib_valloc_add_chunk(clib_valloc_main_t *vam, clib_valloc_chunk_t *template)
{
    ...
    if (CLIB_DEBUG > 0 && index != ~0) {
        while (index != ~0) {
            ...
            ASSERT(template->baseva < ch->baseva || template->baseva >= (ch->baseva + ch->size));
    ...
    if (index != ~0) ch = pool_elt_at_index(vam->chunks, index);
    ...
    if (index == ~0 || template->baseva < ch->baseva) {
        ...
        hash_set(vam->chunk_index_by_baseva, new_ch->baseva, new_ch - vam->chunks);
    }
...
__clib_export void clib_valloc_init(clib_valloc_main_t *vam, clib_valloc_chunk_t *template, int need_lock)
{
    ...
    if (need_lock) clib_spinlock_init(&vam->lock);
...
__clib_export uword clib_valloc_alloc(clib_valloc_main_t *vam, uword size, int os_out_of_memory_on_failure)
{
    ...
    while (index != ~0) {
        ...
        if ((ch->flags & CLIB_VALLOC_BUSY) == 0) {
            /* Too small? */
            if (ch->size < size) goto next_chunk;
            /* Exact match? */
            if (ch->size == size) {
...
__clib_export uword clib_valloc_free(clib_valloc_main_t *vam, uword baseva)
{
    ...
    if (ch->prev != ~0) {
         ...
         */
        if ((prev_ch->flags & CLIB_VALLOC_BUSY) == 0 && ((prev_ch->baseva + prev_ch->size) == ch->baseva)) {
    ...
    if (ch->next != ~0) {
        ...
        if ((next_ch->flags & CLIB_VALLOC_BUSY) == 0 && ((ch->baseva + ch->size) == next_ch->baseva)) {
