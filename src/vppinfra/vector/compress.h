...
static_always_inline u64 *clib_compress_u64_x64_masked(u64 *dst, u64 *src, u64 mask)
{
#if defined(CLIB_HAVE_VEC512_COMPRESS) && defined(CLIB_HAVE_VEC512_MASK_LOAD_STORE)
    u64x8u *sv = (u64x8u *) src;
    for (int i = 0; i < 8; i++) {
...
#elif defined(CLIB_HAVE_VEC256_COMPRESS) && defined(CLIB_HAVE_VEC256_MASK_LOAD_STORE)
    u64x4u *sv = (u64x4u *) src;
    for (int i = 0; i < 16; i++) {
...
static_always_inline u32 clib_compress_u64(u64 *dst, u64 *src, u64 *mask, u32 n_elts)
{
    ...
    if (PREDICT_TRUE(n_elts == 0)) return dst - dst0;

    return clib_compress_u64_x64_masked(dst, src, mask[0] & pow2_mask(n_elts)) - dst0;
}
...
static_always_inline u32 *clib_compress_u32_x64_masked(u32 *dst, u32 *src, u64 mask)
{
#if defined(CLIB_HAVE_VEC512_COMPRESS) && defined(CLIB_HAVE_VEC512_MASK_LOAD_STORE)
    u32x16u *sv = (u32x16u *) src;
    for (int i = 0; i < 4; i++) {
...
static_always_inline u32 clib_compress_u32(u32 *dst, u32 *src, u64 *mask, u32 n_elts)
{
    ...
    return clib_compress_u32_x64_masked(dst, src, mask[0] & pow2_mask(n_elts)) - dst0;
}
...
static_always_inline u16 *clib_compress_u16_x64_masked(u16 *dst, u16 *src, u64 mask)
{
#if defined(CLIB_HAVE_VEC512_COMPRESS_U8_U16) && defined(CLIB_HAVE_VEC512_MASK_LOAD_STORE)
    u16x32u *sv = (u16x32u *) src;
    for (int i = 0; i < 2; i++) {
...
static_always_inline u32 clib_compress_u16(u16 *dst, u16 *src, u64 *mask, u32 n_elts)
{
    ...
    return clib_compress_u16_x64_masked(dst, src, mask[0] & pow2_mask(n_elts)) - dst0;
}
...
static_always_inline u8 *clib_compress_u8_x64_masked(u8 *dst, u8 *src, u64 mask)
{
#if defined(CLIB_HAVE_VEC512_COMPRESS_U8_U16) && defined(CLIB_HAVE_VEC512_MASK_LOAD_STORE)
    u8x64u *sv = (u8x64u *) src;
...
static_always_inline u32 clib_compress_u8(u8 *dst, u8 *src, u64 *mask, u32 n_elts)
{
    ...
    return clib_compress_u8_x64_masked(dst, src, mask[0] & pow2_mask(n_elts)) - dst0;
}
