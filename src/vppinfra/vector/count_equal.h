...
static_always_inline uword clib_count_equal_u64(u64 *data, uword max_count)
{
    ...
    if (max_count <= 1) return max_count;
...
static_always_inline uword clib_count_equal_u32(u32 *data, uword max_count)
{
    ...
    while (count + 15 < max_count) {
        ...
        if (bmp != pow2_mask(16)) return count + count_trailing_zeros(~bmp);
    ...
    if (count == max_count)
        ...
        u32 bmp  = u32x16_is_equal_mask(u32x16_mask_load_zero(data, mask), splat) & mask;
    ...
    while (count + 7 < max_count) {
        ...
        if (bmp != pow2_mask(8)) return count + count_trailing_zeros(~bmp);
#else
        bmp = u8x32_msb_mask((u8x32) (u32x8_load_unaligned(data) == splat));
        if (bmp != 0xffffffff) return count + count_trailing_zeros(~bmp) / 4;
#endif
    ...
    if (count == max_count) return count;
        ...
        u32 mask = pow2_mask(max_count - count);
        u32 bmp  = u32x8_is_equal_mask(u32x8_mask_load_zero(data, mask), splat) & mask;
        return count + count_trailing_zeros(~bmp);
    }
#endif
...
static_always_inline uword clib_count_equal_u8(u8 *data, uword max_count)
{
    ...
    while (count + 63 < max_count) {
        ...
        if (bmp != -1) return count + count_trailing_zeros(~bmp);
    ...
    if (count == max_count) return count;
        ...
        u64 bmp  = u8x64_is_equal_mask(u8x64_mask_load_zero(data, mask), splat) & mask;
        return count + count_trailing_zeros(~bmp);
    }
#endif
#elif defined(CLIB_HAVE_VEC256)
    ...
    while (count + 31 < max_count) {
        ...
        if (bmp != 0xffffffff) return count + count_trailing_zeros(~bmp);
    ...
    if (count == max_count) return count;
        ...
        u32 mask = pow2_mask(max_count - count);
        u64 bmp  = u8x32_msb_mask(u8x32_mask_load_zero(data, mask) == splat) & mask;
        return count + count_trailing_zeros(~bmp);
    }
#endif
#elif defined(CLIB_HAVE_VEC128) && defined(CLIB_HAVE_VEC128_MSB_MASK)
    ...
    while (count + 15 < max_count) {
        ...
        if (bmp != 0xffff) return count + count_trailing_zeros(~bmp);
