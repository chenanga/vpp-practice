...
#ifdef __x86_64__
static_always_inline void clib_toeplitz_hash_key_expand_8(u64x2 kv, u64x8u *m)
{
...
void clib_toeplitz_hash_key_expand(u64 *matrixes, u8 *key, int size)
{
...
__clib_export clib_toeplitz_hash_key_t *clib_toeplitz_hash_key_init(u8 *key, u32 keylen)
{
    ...
    size = round_pow2(sizeof(clib_toeplitz_hash_key_t) + round_pow2(keylen, 16), CLIB_CACHE_LINE_BYTES);
#ifdef __x86_64__
...
#ifdef __x86_64__
    clib_toeplitz_hash_key_expand((u64 *) ((u8 *) k + k->gfni_offset), k->data, k->key_length);
#endif

    return k;
}

__clib_export void clib_toeplitz_hash_key_free(clib_toeplitz_hash_key_t *k)
{
    clib_mem_free(k);
}
