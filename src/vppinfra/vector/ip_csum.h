...
static_always_inline u64x2 clib_ip_csum_cvt_and_add_4(u32x4 v)
{
    return ((u64x2) u32x4_interleave_lo((u32x4) v, u32x4_zero()) + (u64x2) u32x4_interleave_hi((u32x4) v, u32x4_zero()));
}
static_always_inline u64 clib_ip_csum_hadd_2(u64x2 v)
{
    return v[0] + v[1];
}
#endif
...
static_always_inline u64x4 clib_ip_csum_cvt_and_add_8(u32x8 v)
{
    return ((u64x4) u32x8_interleave_lo((u32x8) v, u32x8_zero()) + (u64x4) u32x8_interleave_hi((u32x8) v, u32x8_zero()));
}
static_always_inline u64 clib_ip_csum_hadd_4(u64x4 v)
{
...
static_always_inline u64x8 clib_ip_csum_cvt_and_add_16(u32x16 v)
{
    return ((u64x8) u32x16_interleave_lo((u32x16) v, u32x16_zero()) + (u64x8) u32x16_interleave_hi((u32x16) v, u32x16_zero()));
}
static_always_inline u64 clib_ip_csum_hadd_8(u64x8 v)
{
    return clib_ip_csum_hadd_4(u64x8_extract_lo(v) + u64x8_extract_hi(v));
}
#endif

static_always_inline void clib_ip_csum_inline(clib_ip_csum_t *c, u8 *dst, u8 *src, u16 count, int is_copy)
{
    if (c->odd) {
        c->odd = 0;
        c->sum += (u16) src[0] << 8;
    ...
    if (count) {
        ...
        c->odd = count & 1;
        if (is_copy) u32x16_mask_store(v, dst, mask);
    }
    ...
    if (count) {
        ...
        c->odd = count & 1;
        if (is_copy) u32x8_mask_store(v, dst, mask);
    }
...
static_always_inline u16 clib_ip_csum_fold(clib_ip_csum_t *c)
{
    u64 sum = c->sum;
...
static_always_inline void clib_ip_csum_chunk(clib_ip_csum_t *c, u8 *src, u16 count)
{
    return clib_ip_csum_inline(c, 0, src, count, 0);
}

static_always_inline void clib_ip_csum_and_copy_chunk(clib_ip_csum_t *c, u8 *src, u8 *dst, u16 count)
{
    return clib_ip_csum_inline(c, dst, src, count, 1);
}

static_always_inline u16 clib_ip_csum(u8 *src, u16 count)
{
    clib_ip_csum_t c = {};
...
static_always_inline u16 clib_ip_csum_and_copy(u8 *dst, u8 *src, u16 count)
{
    clib_ip_csum_t c = {};
