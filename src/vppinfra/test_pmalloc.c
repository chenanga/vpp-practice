...
clib_error_t *test_palloc(test_main_t *tm)
{
    ...
    if (clib_pmalloc_init(pm, 0, 0) != 0) return clib_error_return(0, "pmalloc init failure");
    ...
    for (i = 0; i < tm->nitems; i++) {
        ...
        if (va == 0)
            clib_error(
                "Failed to alloc %u byte chunk with align %u on numa %u,"
                "\nerror: %U",
                size,
                align,
                numa,
        ...
        if ((pointer_to_uword(va) & (align - 1)) != 0) clib_error(0, "Alignment error: %p not aligned with %u", va, align);
    ...
    if (tm->arena_items) {
        ...
        arena = clib_pmalloc_create_shared_arena(pm, "test arena", tm->arena_pages << 21, tm->arena_log2_pg_sz, tm->arena_numa);
        if (arena == 0) clib_error("Failed to alloc shared arena: %U", format_clib_error, clib_pmalloc_last_error(pm));
...
clib_error_t *test_palloc_main(unformat_input_t *i)
{
