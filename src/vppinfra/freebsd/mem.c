...
void clib_mem_main_init(void)
{
    ...
    /* numa nodes */
    va = mmap(0, page_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    if (va == MAP_FAILED) return;

    if (mlock(va, page_size)) goto done;
...
__clib_export u64 *clib_mem_vm_get_paddr(void *mem, clib_mem_page_sz_t log2_page_size, int n_pages)
{
    ...
    if ((fd = open((char *) "/dev/mem", O_RDONLY)) == -1) return 0;

    for (i = 0; i < n_pages; i++) {
        meme.me_vaddr = pointer_to_uword(mem) + (((u64) i) << log2_page_size);

        if (ioctl(fd, MEM_EXTRACT_PADDR, &meme) == -1) goto done;
