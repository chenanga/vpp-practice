...
static inline clib_pmalloc_chunk_t *get_chunk(clib_pmalloc_page_t *pp, u32 index)
{
    return pool_elt_at_index(pp->chunks, index);
}

static inline uword pmalloc_size2pages(uword size, u32 log2_page_sz)
{
...
__clib_export int clib_pmalloc_init(clib_pmalloc_main_t *pm, uword base_addr, uword size)
{
    ...
    if (pt == 0 || pt[0] == 0) pm->flags |= CLIB_PMALLOC_F_NO_PAGEMAP;
    ...
    if (base == ~0) {
        pm->error = clib_error_return(0, "failed to reserve %u pages", pm->max_pages);
        return -1;
    }
...
static inline void *alloc_chunk_from_page(clib_pmalloc_main_t *pm, clib_pmalloc_page_t *pp, u32 n_blocks, u32 block_align, u32 numa_node)
{
    ...
    if (pp->n_free_blocks < n_blocks) return 0;
...
next_chunk:
    ...
    if (c->size > n_blocks) {
        ...
        if (ct->next != ~0) pool_elt_at_index(pp->chunks, ct->next)->prev = tail_chunk_index;
    }
    else if (c->next != ~0)
        pool_elt_at_index(pp->chunks, c->next)->prev = alloc_chunk_index;

    c  = get_chunk(pp, alloc_chunk_index);
    va = pm->base + ((pp - pm->pages) << pm->def_log2_page_sz) + (c->start << PMALLOC_LOG2_BLOCK_SZ);
...
static void pmalloc_update_lookup_table(clib_pmalloc_main_t *pm, u32 first, u32 count)
{
    ...
    vec_validate_aligned(pm->lookup_table, vec_len(pm->pages) * elts_per_page - 1, CLIB_CACHE_LINE_BYTES);

    p = (uword) first * elts_per_page;
    if (pm->flags & CLIB_PMALLOC_F_NO_PAGEMAP) {
        while (p < (uword) elts_per_page * count) {
            pm->lookup_table[p] = pointer_to_uword(pm->base) + (p << pm->lookup_log2_page_sz);
            p++;
        }
        return;
    }
    ...
    while (p < (uword) elts_per_page * count) {
        ...
        if (fd != -1 && lseek(fd, seek, SEEK_SET) == seek && read(fd, &pa, sizeof(pa)) == (sizeof(pa)) && pa & (1ULL << 63) /* page present bit */) {
    ...
    while (p < (uword) elts_per_page * count) {
        meme.me_vaddr = pointer_to_uword(pm->base) + (p << pm->lookup_log2_page_sz);
        if (ioctl(fd, MEM_EXTRACT_PADDR, &meme) == -1) continue;
...
static inline clib_pmalloc_page_t *pmalloc_map_pages(clib_pmalloc_main_t *pm, clib_pmalloc_arena_t *a, u32 numa_node, u32 n_pages)
{
    ...
    if (a->log2_subpage_sz != clib_mem_get_log2_page_size()) {
        pm->error = clib_sysfs_prealloc_hugepages(numa_node, a->log2_subpage_sz, n_pages);
    ...
    if (rv == CLIB_MEM_ERROR && numa_node != 0) {
        pm->error = clib_error_return_unix(0,
                                           "failed to set mempolicy for "
                                           "numa node %u",
                                           numa_node);
        return 0;
    }
    ...
    if (a->flags & CLIB_PMALLOC_ARENA_F_SHARED_MEM) {
        ...
        if (a->fd == -1) goto error;
        if ((ftruncate(a->fd, size)) == -1) goto error;
    }
    else {
#ifdef __linux__
        if (a->log2_subpage_sz != clib_mem_get_log2_page_size()) mmap_flags |= MAP_HUGETLB;
#endif /* __linux__ */
    ...
    if (mmap(va, size, PROT_READ | PROT_WRITE, mmap_flags, a->fd, 0) == MAP_FAILED) {
        pm->error = clib_error_return_unix(0,
                                           "failed to mmap %u pages at %p "
                                           "fd %d numa %d flags 0x%x",
                                           n_pages,
                                           va,
                                           a->fd,
                                           numa_node,
    ...
    if (a->log2_subpage_sz != clib_mem_get_log2_page_size() && mlock(va, size) != 0) {
    ...
    if (stats.per_numa[numa_node] != 1 && !(numa_node == 0 && stats.unknown == 1)) {
        u16 allocated_at = ~0;
        if (stats.unknown) clib_error_return(0, "unable to get information about numa allocation");

        for (u16 i = 0; i < CLIB_MAX_NUMAS; i++)
            if (stats.per_numa[i] == 1) allocated_at = i;
    ...
    if (a->log2_subpage_sz < pm->lookup_log2_page_sz) {
        ...
        pmalloc_update_lookup_table(pm, vec_len(pm->pages) - n_pages, n_pages);
    }
    else
...
error:
    if (va != MAP_FAILED) {
        /* unmap & reserve */
        munmap(va, size);
        mmap(va, size, PROT_NONE, MAP_FIXED | MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    }
...
__clib_export void *clib_pmalloc_create_shared_arena(clib_pmalloc_main_t *pm, char *name, uword size, u32 log2_page_sz, u32 numa_node)
{
    ...
    else if (log2_page_sz != pm->def_log2_page_sz && log2_page_sz != clib_mem_get_log2_page_size()) {
        pm->error = clib_error_create("unsupported page size (%uKB)", 1 << (log2_page_sz - 10));
        return 0;
    }

    n_pages = pmalloc_size2pages(size, pm->def_log2_page_sz);

    if (n_pages + vec_len(pm->pages) > pm->max_pages) return 0;

    if (numa_node == CLIB_PMALLOC_NUMA_LOCAL) numa_node = clib_get_current_numa_node();
...
static inline void *clib_pmalloc_alloc_inline(clib_pmalloc_main_t *pm, clib_pmalloc_arena_t *a, uword size, uword align, u32 numa_node)
{
    clib_pmalloc_page_t *pp;
    ...
    if (a == 0) {
        if (size > 1ULL << pm->def_log2_page_sz) return 0;
        ...
        if (pm->default_arena_for_numa_node[numa_node] == ~0) {
            ...
            a = pool_elt_at_index(pm->arenas, pm->default_arena_for_numa_node[numa_node]);
    }
    ...
    vec_foreach (page_index, a->page_indices) {
        ...
        void *rv = alloc_chunk_from_page(pm, pp, n_blocks, block_align, numa_node);

        if (rv) return rv;
    }

    if ((a->flags & CLIB_PMALLOC_ARENA_F_SHARED_MEM) == 0 && (pp = pmalloc_map_pages(pm, a, numa_node, 1)))
...
__clib_export void *clib_pmalloc_alloc_aligned(clib_pmalloc_main_t *pm, uword size, uword align)
{
    return clib_pmalloc_alloc_inline(pm, 0, size, align, CLIB_PMALLOC_NUMA_LOCAL);
}
...
static inline int pmalloc_chunks_mergeable(clib_pmalloc_arena_t *a, clib_pmalloc_page_t *pp, u32 ci1, u32 ci2)
{
    ...
    if (c1->used || c2->used) return 0;

    if (c1->start >> (a->log2_subpage_sz - PMALLOC_LOG2_BLOCK_SZ) != c2->start >> (a->log2_subpage_sz - PMALLOC_LOG2_BLOCK_SZ)) return 0;
...
__clib_export void clib_pmalloc_free(clib_pmalloc_main_t *pm, void *va)
{
    ...
    if (pmalloc_chunks_mergeable(a, pp, chunk_index, c->next)) {
        ...
        c->size += next->size;
        c->next = next->next;
        if (next->next != ~0) get_chunk(pp, next->next)->prev = chunk_index;
    ...
    if (pmalloc_chunks_mergeable(a, pp, c->prev, chunk_index)) {
        ...
        prev->size += c->size;
        prev->next = c->next;
        if (c->next != ~0) get_chunk(pp, c->next)->prev = c->prev;
        memset(c, 0, sizeof(*c));
...
static u8 *format_pmalloc_page(u8 *s, va_list *va)
{
    ...
    if (pp->chunks == 0) return s;

    s = format(s, "free %u chunks %u free-chunks %d ", (pp->n_free_blocks) << PMALLOC_LOG2_BLOCK_SZ, pool_elts(pp->chunks), pp->n_free_chunks);

    if (verbose >= 2) {
        clib_pmalloc_chunk_t *c;
        c = pool_elt_at_index(pp->chunks, pp->first_chunk_index);
        s = format(s, "\n%U%12s%12s%8s%8s%8s%8s", format_white_space, indent + 2, "chunk offset", "size", "used", "index", "prev", "next");
        while (1) {
            s = format(s,
...
__clib_export u8 *format_pmalloc(u8 *s, va_list *va)
{
    ...
    if (verbose >= 2) s = format(s, " va-start %p", pm->base);

    if (pm->error) s = format(s, "\n%Ulast-error: %U", format_white_space, indent + 2, format_clib_error, pm->error);
    ...
    pool_foreach (a, pm->arenas) {
        ...
        s = format(s,
                   ...
                   indent + 2,
                   a->name,
                   vec_len(a->page_indices),
                   format_log2_page_size,
                   a->log2_subpage_sz,
                   a->numa_node);
        if (a->fd != -1) s = format(s, " shared fd %d", a->fd);
        if (verbose >= 1)
            vec_foreach (page_index, a->page_indices) {
                pp = vec_elt_at_index(pm->pages, *page_index);
                s  = format(s, "\n%U%U", format_white_space, indent + 4, format_pmalloc_page, pp, verbose);
            }
    }
...
__clib_export u8 *format_pmalloc_map(u8 *s, va_list *va)
{
    ...
    vec_foreach_index (index, pm->lookup_table) {
        ...
        va         = pointer_to_uword(pm->base) + ((uword) index << pm->lookup_log2_page_sz);
        pa         = va - *lookup_val;
        s = format(s, "\n %16p %13p %8U", uword_to_pointer(va, u64), uword_to_pointer(pa, u64), format_log2_page_size, pm->lookup_log2_page_sz);
    }
    return s;
}
