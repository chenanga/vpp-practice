...
static void mheap_get_trace_internal(const clib_mem_heap_t *heap, uword offset, uword size)
{
    ...
    if (heap != tm->current_traced_mheap || mheap_trace_thread_disable) return;

    clib_spinlock_lock(&tm->lock);

    /* heap could have changed while we were waiting on the lock */
    if (heap != tm->current_traced_mheap) goto out;
    ...
    foreach_clib_stack_frame (sf) {
        if (index >= 0) {
            if (index == ARRAY_LEN(trace.callers)) break;
    ...
    if (!tm->trace_by_callers) tm->trace_by_callers = hash_create_shmem(0, sizeof(trace.callers), sizeof(uword));
...
static void mheap_put_trace_internal(const clib_mem_heap_t *heap, uword offset, uword size)
{
    mheap_trace_t      *t;
    uword               trace_index, *p;
    mheap_trace_main_t *tm = &mheap_trace_main;
...
static clib_mem_heap_t *clib_mem_create_heap_internal(void *base, uword size, clib_mem_page_sz_t log2_page_sz, int is_locked, char *name)
{
    ...
    if (base == 0) {
        ...
        base         = clib_mem_vm_map_internal(0, log2_page_sz, size, -1, 0, "main heap");

        if (base == CLIB_MEM_VM_MAP_FAILED) return 0;
    ...
    if (is_locked) flags |= CLIB_MEM_HEAP_F_LOCKED;
...
static void *clib_mem_init_internal(void *base, uword size, clib_mem_page_sz_t log2_page_sz)
{
    ...
    h = clib_mem_create_heap_internal(base, size, log2_page_sz, 1 /*is_locked */, "main heap");
...
{
    return clib_mem_init_internal(memory, memory_size, CLIB_MEM_PAGE_SZ_DEFAULT);
}
...
__clib_export void clib_mem_destroy(void)
{
    ...
    if (heap->mspace == tm->current_traced_mheap) mheap_trace(heap, 0);
...
__clib_export u8 *format_clib_mem_usage(u8 *s, va_list *va)
{
    ...
    return format(s, "$$$$ heap at %llx verbose %d", clib_mem_get_heap(), verbose);
}
...
static int mheap_trace_sort(const void *_t1, const void *_t2)
{
    ...
    if (!cmp) cmp = (word) t2->n_allocations - (word) t1->n_allocations;
    return cmp;
}
...
u8 *format_mheap_trace(u8 *s, va_list *va)
{
    ...
    clib_spinlock_lock(&tm->lock);
    if (vec_len(tm->traces) > 0 && clib_mem_get_heap() == tm->current_traced_mheap) {
        have_traces = 1;
        ...
        qsort(traces_copy, vec_len(traces_copy), sizeof(traces_copy[0]), mheap_trace_sort);
        ...
        vec_foreach (t, traces_copy) {
            ...
            if (!verbose && n >= 50) continue;
            n++;

            if (t == traces_copy) s = format(s, "%=9s%=9s %=10s Traceback\n", "Bytes", "Count", "Sample");
            s      = format(s, "%9d%9d %p", t->n_bytes, t->n_allocations, t->offset);
            indent = format_get_indent(s);
            for (i = 0; i < ARRAY_LEN(t->callers) && t->callers[i]; i++) {
                if (i > 0) s = format(s, "%U", format_white_space, indent);
    ...
    clib_spinlock_unlock(&tm->lock);
    if (have_traces == 0) s = format(s, "no traced allocations\n");
...
__clib_export u8 *format_clib_mem_heap(u8 *s, va_list *va)
{
    ...
    if (heap == 0) heap = clib_mem_get_heap();

    mi = mspace_mallinfo(heap->mspace);

    s = format(s, "base %p, size %U", heap->base, format_memory_size, heap->size);
...
#undef _
    ...
    if (heap->log2_page_sz != CLIB_MEM_PAGE_SZ_UNKNOWN) {
        ...
        clib_mem_get_page_stats(heap->base, heap->log2_page_sz, heap->size >> heap->log2_page_sz, &stats);
        s = format(s, "\n%U%U", format_white_space, indent, format_clib_mem_page_stats, &stats);
    }
    ...
    s = format(s,
               ...
               mi.keepcost);
    if (verbose > 0) {
        s = format(s, "\n%Ufree chunks %llu free fastbin blks %llu", format_white_space, indent + 2, mi.ordblks, mi.smblks);
        s = format(s, "\n%Umax total allocated %U", format_white_space, indent + 2, format_msize, mi.usmblks);
    }

    if (heap->flags & CLIB_MEM_HEAP_F_TRACED) s = format(s, "\n%U", format_mheap_trace, tm, verbose);
    return s;
}

__clib_export __clib_flatten void clib_mem_get_heap_usage(clib_mem_heap_t *heap, clib_mem_usage_t *usage)
{
...
__clib_export mheap_trace_t *clib_mem_trace_dup(clib_mem_heap_t *heap)
{
    mheap_trace_main_t *tm          = &mheap_trace_main;
...
__clib_export clib_mem_heap_t *clib_mem_create_heap(void *base, uword size, int is_locked, char *fmt, ...)
{
    ...
    h = clib_mem_create_heap_internal(base, size, log2_page_sz, is_locked, name);
    vec_free(s);
    return h;
}
...
__clib_export void clib_mem_destroy_heap(clib_mem_heap_t *h)
{
    ...
    if (h->mspace == tm->current_traced_mheap) mheap_trace(h, 0);

    destroy_mspace(h->mspace);
    if (h->flags & CLIB_MEM_HEAP_F_UNMAP_ON_DESTROY) clib_mem_vm_unmap(h->base);
}

__clib_export __clib_flatten uword clib_mem_get_heap_free_space(clib_mem_heap_t *h)
{
    struct dlmallinfo dlminfo = mspace_mallinfo(h->mspace);
    return dlminfo.fordblks;
}

__clib_export __clib_flatten void *clib_mem_get_heap_base(clib_mem_heap_t *h)
{
    return h->base;
}

__clib_export __clib_flatten uword clib_mem_get_heap_size(clib_mem_heap_t *heap)
{
    return heap->size;
}

/* Memory allocator which may call os_out_of_memory() if it fails */
static inline void *clib_mem_heap_alloc_inline(void *heap, uword size, uword align, int os_out_of_memory_on_failure)
{
    ...
    if (PREDICT_FALSE(0 == p)) {
        if (os_out_of_memory_on_failure) os_out_of_memory();
        return 0;
    }

    if (PREDICT_FALSE(h->flags & CLIB_MEM_HEAP_F_TRACED)) mheap_get_trace_internal(h, pointer_to_uword(p), clib_mem_size(p));
...
__clib_export __clib_flatten void *clib_mem_alloc(uword size)
{
    return clib_mem_heap_alloc_inline(0,
                                      size,
                                      CLIB_MEM_MIN_ALIGN,
                                      /* os_out_of_memory */ 1);
}

__clib_export __clib_flatten void *clib_mem_alloc_aligned(uword size, uword align)
{
    return clib_mem_heap_alloc_inline(0,
                                      size,
                                      align,
...
/* Memory allocator which calls os_out_of_memory() when it fails */
__clib_export __clib_flatten void *clib_mem_alloc_or_null(uword size)
{
    return clib_mem_heap_alloc_inline(0,
                                      size,
                                      CLIB_MEM_MIN_ALIGN,
...
__clib_export __clib_flatten void *clib_mem_alloc_aligned_or_null(uword size, uword align)
{
    return clib_mem_heap_alloc_inline(0,
                                      size,
                                      align,
...
__clib_export __clib_flatten void *clib_mem_heap_alloc(void *heap, uword size)
{
    return clib_mem_heap_alloc_inline(heap,
                                      size,
                                      CLIB_MEM_MIN_ALIGN,
                                      /* os_out_of_memory */ 1);
}

__clib_export __clib_flatten void *clib_mem_heap_alloc_aligned(void *heap, uword size, uword align)
{
    return clib_mem_heap_alloc_inline(heap,
                                      size,
                                      align,
...
__clib_export __clib_flatten void *clib_mem_heap_alloc_or_null(void *heap, uword size)
{
    return clib_mem_heap_alloc_inline(heap,
                                      size,
                                      CLIB_MEM_MIN_ALIGN,
...
__clib_export __clib_flatten void *clib_mem_heap_alloc_aligned_or_null(void *heap, uword size, uword align)
{
    return clib_mem_heap_alloc_inline(heap,
                                      size,
                                      align,
...
__clib_export __clib_flatten void *clib_mem_heap_realloc_aligned(void *heap, void *p, uword new_size, uword align)
{
    ...
    if (new_size == old_alloc_size) return p;

    if (p && pointer_is_aligned(p, align) && mspace_realloc_in_place(h->mspace, p, new_size)) {
...
__clib_export __clib_flatten void *clib_mem_heap_realloc(void *heap, void *p, uword new_size)
{
    return clib_mem_heap_realloc_aligned(heap, p, new_size, CLIB_MEM_MIN_ALIGN);
}

__clib_export __clib_flatten void *clib_mem_realloc_aligned(void *p, uword new_size, uword align)
{
    return clib_mem_heap_realloc_aligned(0, p, new_size, align);
}

__clib_export __clib_flatten void *clib_mem_realloc(void *p, uword new_size)
{
    return clib_mem_heap_realloc_aligned(0, p, new_size, CLIB_MEM_MIN_ALIGN);
}

__clib_export __clib_flatten uword clib_mem_heap_is_heap_object(void *heap, void *p)
{
    clib_mem_heap_t *h = heap ? heap : clib_mem_get_per_cpu_heap();
    return mspace_is_heap_object(h->mspace, p);
}

__clib_export __clib_flatten uword clib_mem_is_heap_object(void *p)
{
    return clib_mem_heap_is_heap_object(0, p);
}

__clib_export __clib_flatten void clib_mem_heap_free(void *heap, void *p)
{
    clib_mem_heap_t *h    = heap ? heap : clib_mem_get_per_cpu_heap();
    ...
    if (PREDICT_FALSE(h->flags & CLIB_MEM_HEAP_F_TRACED)) mheap_put_trace_internal(h, pointer_to_uword(p), size);
    clib_mem_poison(p, clib_mem_size(p));

    mspace_free(h->mspace, p);
}

__clib_export __clib_flatten void clib_mem_free(void *p)
{
    clib_mem_heap_free(0, p);
}

__clib_export __clib_flatten uword clib_mem_size(void *p)
{
    return mspace_usable_size(p);
}

__clib_export void clib_mem_free_s(void *p)
{
    uword size = clib_mem_size(p);
    clib_mem_unpoison(p, size);
