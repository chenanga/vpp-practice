...
__test_funct_fn void wrapper(u8 *dst, u8 *src, uword n)
{
...
static clib_error_t *validate_one(clib_error_t *err, u8 *d, u8 *s, u16 n, u8 off, int is_const)
{
    for (int i = 0; i < n; i++)
        if (d[i] != s[i])
...
static clib_error_t *test_clib_memcpy_x86_64(clib_error_t *err)
{
    u8 src[MAX_LEN + 192];
    u8 dst[MAX_LEN + 192];
    ...
    for (int j = 0; j < ARRAY_LEN(const_tests); j++) {
        ...
        if ((err = validate_one(err, d, s, n, 0, /* is_const */ 1))) return err;
    }
    ...
    for (u16 n = 1; n <= MAX_LEN; n++) {
        for (int off = 0; off < 64; off += 7) {
            ...
            if ((err = validate_one(err, d, s, n, off, /* is_const */ 0))) return err;
        }
    }
    return err;
}
