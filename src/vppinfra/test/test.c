...
int test_march_supported(clib_march_variant_type_t type)
{
#define _(s, n) \
    if (CLIB_MARCH_VARIANT_TYPE_##s == type) return clib_cpu_march_priority_##s();
    foreach_march_variant
#undef _
        return 0;
}

clib_error_t *test_funct(test_main_t *tm)
{
    for (int i = 0; i < CLIB_MARCH_TYPE_N_VARIANTS; i++) {
        test_registration_t *r = tm->registrations[i];

        if (r == 0 || test_march_supported(i) < 0) continue;

        fformat(stdout, "\nMultiarch Variant: %U\n", format_march_variant, i);
        fformat(stdout, "-------------------------------------------------------\n");
        while (r) {
            clib_error_t *err;
            if (tm->filter && strstr(r->name, (char *) tm->filter) == 0) goto next;
            err = (r->fn)(0);
...
#ifdef __linux__
clib_error_t *test_perf(test_main_t *tm)
{
    clib_error_t      *err = 0;
    clib_perfmon_ctx_t _ctx, *ctx = &_ctx;

    if ((err = clib_perfmon_init_by_bundle_name(ctx, "%s", tm->bundle ? (char *) tm->bundle : "default"))) return err;
...
void *test_mem_alloc_and_splat(uword elt_size, uword n_elts, void *elt)
{
    ...
    if (data_size < alloc_size) clib_memset_u8(e, 0, alloc_size - data_size);
    vec_add1(test_main.allocated_mem, rv);
    return rv;
}
