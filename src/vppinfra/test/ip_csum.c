...
static u8 test1[] = {0x45, 0x00, 0x00, 0x73, 0x00, 0x00, 0x40, 0x00, 0x40, 0x11, 0x00, 0x00, 0xc0, 0xa8, 0x00, 0x01, 0xc0, 0xa8, 0x00, 0xc7, 0x00};
...
static clib_error_t *test_clib_ip_csum(clib_error_t *err)
{
    u8 *buf;
...
void __test_perf_fn perftest_ip4_hdr(test_perf_t *tp)
{
    u32  n    = tp->n_ops;
...
void __test_perf_fn perftest_tcp_payload(test_perf_t *tp)
{
    u32             n    = tp->n_ops;
...
void __test_perf_fn perftest_byte(test_perf_t *tp)
{
...
REGISTER_TEST(clib_ip_csum) = {
    ...
    .perf_tests = PERF_TESTS({.name = "fixed size (per IPv4 Header)", .n_ops = 1024, .fn = perftest_ip4_hdr},
                             {.name = "fixed size (per 1460 byte block)", .n_ops = 16, .arg0 = 1460, .fn = perftest_tcp_payload},
