...
#define __test_funct_fn static __clib_noinline __clib_noclone __clib_section(".test_func")
#define __test_perf_fn  static __clib_noinline __clib_noclone __clib_section(".test_perf")

#define REGISTER_TEST(x)                                                                                                                     \
    test_registration_t            CLIB_MARCH_SFX(__test_##x);                                                                               \
    static void __clib_constructor CLIB_MARCH_SFX(__test_registration_##x)(void)                                                             \
    {                                                                                                                                        \
        test_registration_t *r                                           = &CLIB_MARCH_SFX(__test_##x);                                      \
        r->next                                                          = test_main.registrations[CLIB_MARCH_SFX(CLIB_MARCH_VARIANT_TYPE)]; \
...
#ifdef __linux__
static_always_inline void test_perf_event_reset(test_perf_t *t)
{
    clib_perfmon_ioctl(t->fd, PERF_EVENT_IOC_RESET);
}
static_always_inline void test_perf_event_enable(test_perf_t *t)
{
    clib_perfmon_ioctl(t->fd, PERF_EVENT_IOC_ENABLE);
}
static_always_inline void test_perf_event_disable(test_perf_t *t)
{
