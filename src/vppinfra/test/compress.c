...
__test_funct_fn u32 clib_compress_u64_wrapper(u64 *dst, u64 *src, u64 *mask, u32 n_elts)
{
    return clib_compress_u64(dst, src, mask, n_elts);
}

__test_funct_fn u32 clib_compress_u32_wrapper(u32 *dst, u32 *src, u64 *mask, u32 n_elts)
{
    return clib_compress_u32(dst, src, mask, n_elts);
}

__test_funct_fn u32 clib_compress_u16_wrapper(u16 *dst, u16 *src, u64 *mask, u32 n_elts)
{
    return clib_compress_u16(dst, src, mask, n_elts);
}

__test_funct_fn u32 clib_compress_u8_wrapper(u8 *dst, u8 *src, u64 *mask, u32 n_elts)
{
...
static clib_error_t *test_clib_compress_u64(clib_error_t *err)
{
    u32 i, j;
    ...
    for (i = 0; i < ARRAY_LEN(tests); i++) {
        ...
        for (j = 0; j < t->n_elts; j++) {
            if ((t->mask[j >> 6] & (1ULL << (j & 0x3f))) == 0) continue;
            if (dp[0] != src[j])
        ...
        if (dst[dp - dst + 1] != 0xa5a5a5a5a5a5a5a5) return clib_error_return(err, "buffer overrun in testcase %u", i);
#endif /* CLIB_SANITIZE_ADDR */

        if (dp - dst != r) return clib_error_return(err, "wrong number of elts in testcase %u", i);
    }

    return err;
}

static clib_error_t *test_clib_compress_u32(clib_error_t *err)
{
    u32 i, j;
...
#ifndef CLIB_SANITIZE_ADDR
        if (dst[dp - dst + 1] != 0xa5a5a5a5) return clib_error_return(err, "buffer overrun in testcase %u", i);
#endif /* CLIB_SANITIZE_ADDR */
...
static clib_error_t *test_clib_compress_u16(clib_error_t *err)
{
    u32 i, j;
...
#ifndef CLIB_SANITIZE_ADDR
        if (dst[dp - dst + 1] != 0xa5a5) return clib_error_return(err, "buffer overrun in testcase %u", i);
#endif /* CLIB_SANITIZE_ADDR */
...
static clib_error_t *test_clib_compress_u8(clib_error_t *err)
{
    u32 i, j;
...
#ifndef CLIB_SANITIZE_ADDR
        if (dst[dp - dst + 1] != 0xa5) return clib_error_return(err, "buffer overrun in testcase %u", i);
#endif /* CLIB_SANITIZE_ADDR */
