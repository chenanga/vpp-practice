...
#define foreach_clib_count_equal(type)                                                                        \
    typedef uword(wrapper_fn_##type)(type * a, uword maxcount);                                               \
                                                                                                              \
    __test_funct_fn uword clib_count_equal_##type##_wrapper(type *a, uword maxcount)                          \
    {                                                                                                         \
    ...
    {                                                                                                         \
        ...
        void *map;                                                                                            \
                                                                                                              \
        u16   lengths[] = {1, 2, 3, 5, 7, 9, 15, 16, 17, 31, 32, 33, 255, 256, 257};                          \
        type *data;                                                                                           \
                                                                                                              \
        map = clib_mem_vm_map(0, 2ULL << ps, ps, "test");                                                     \
        if (map == CLIB_MEM_VM_MAP_FAILED) return clib_error_return(err, "clib_mem_vm_map failed");           \
                                                                                                              \
        ...
        for (u8 d = 0; d < 255; d++) {                                                                        \
            ...
            for (int i = 0; i < ARRAY_LEN(lengths); i++) {                                                    \
                ...
                if ((rv = wfn_##type(data - len, len)) != len) {                                              \
                    err = clib_error_return(err, "testcase 1 failed for len %u data %u(rv %u)", len, d, rv);  \
                    goto done;                                                                                \
                }                                                                                             \
                                                                                                              \
                data[-1] = d + 1;                                                                             \
                if (len > 1 && ((rv = wfn_##type(data - len, len)) != len - 1)) {                             \
                    err = clib_error_return(err, "testcase 2 failed for len %u data %u (rv %u)", len, d, rv); \
                    goto done;                                                                                \
                }                                                                                             \
                data[-1] = d;                                                                                 \
                                                                                                              \
                ...
                if (len > 2 && ((rv = wfn_##type(data - len, len)) != len - 2)) {                             \
                    err = clib_error_return(err, "testcase 3 failed for len %u data %u (rv %u)", len, d, rv); \
                    goto done;                                                                                \
                }                                                                                             \
                data[-2] = d;                                                                                 \
            }                                                                                                 \
