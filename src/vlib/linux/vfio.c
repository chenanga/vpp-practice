...
clib_error_t *vfio_map_physmem_page(vlib_main_t *vm, void *addr)
{
    ...
    if (lvm->container_fd == -1) return clib_error_return(0, "No cointainer fd");
    ...
    lvm->physmem_pages_mapped = clib_bitmap_set(lvm->physmem_pages_mapped, page_index, 1);
    return 0;
}
...
static clib_error_t *open_vfio_iommu_group(int group, int is_noiommu)
{
    ...
    if (lvm->container_fd == -1) {
        if ((fd = open("/dev/vfio/vfio", O_RDWR)) == -1) return clib_error_return_unix(0, "failed to open VFIO container");
    ...
    if (fd < 0) return clib_error_return_unix(0, "open '%s'", s);
    ...
    if (ioctl(fd, VFIO_GROUP_SET_CONTAINER, &lvm->container_fd) < 0) {
        err = clib_error_return_unix(0, "ioctl(VFIO_GROUP_SET_CONTAINER) '%s'", s);
        goto error;
    }
...
clib_error_t *linux_vfio_group_get_device_fd(vlib_pci_addr_t *addr, int *fdp, int *is_noiommu)
{
    ...
    *is_noiommu = 0;
    tmpstr      = clib_file_get_resolved_basename("/sys/bus/pci/devices/%U/iommu_group", format_vlib_pci_addr, addr);
    if (tmpstr) {
        iommu_group = atoi((char *) tmpstr);
        vec_free(tmpstr);
    }
    else {
        err = clib_error_return(0, "Cannot find IOMMU group for PCI device ", "'%U'", format_vlib_pci_addr, addr);
        goto error;
    }
    vec_reset_length(s);

    s   = format(s, "/sys/bus/pci/devices/%U/iommu_group/name%c", format_vlib_pci_addr, addr, 0);
    err = clib_sysfs_read((char *) s, "%s", &tmpstr);
    if (err == 0) {
        if (strncmp((char *) tmpstr, "vfio-noiommu", 12) == 0) *is_noiommu = 1;
        ...
        clib_error_free(err);
    vec_reset_length(s);
    if ((err = open_vfio_iommu_group(iommu_group, *is_noiommu))) return err;
    ...
    if ((fd = ioctl(g->fd, VFIO_GROUP_GET_DEVICE_FD, (char *) s)) < 0) {
        err = clib_error_return_unix(0, "ioctl(VFIO_GROUP_GET_DEVICE_FD) '%U'", format_vlib_pci_addr, addr);
        goto error;
    }
    vec_reset_length(s);
...
u8 *format_vfio_region_info(u8 *s, va_list *args)
{
    ...
    s = format(s, "region_info index:%u size:0x%lx offset:0x%lx flags:", r->index, r->size, r->offset);

    if (r->flags & VFIO_REGION_INFO_FLAG_READ) s = format(s, " rd");

    if (r->flags & VFIO_REGION_INFO_FLAG_WRITE) s = format(s, " wr");

    if (r->flags & VFIO_REGION_INFO_FLAG_MMAP) s = format(s, " mmap");

#ifdef VFIO_REGION_INFO_FLAG_CAPS
    if (r->flags & VFIO_REGION_INFO_FLAG_CAPS) s = format(s, " caps");
#endif
    ...
    if ((r->flags & VFIO_REGION_INFO_FLAG_CAPS) == 0) return s;
    ...
    do {
        ...
        if (cap->id == VFIO_REGION_INFO_CAP_SPARSE_MMAP) s = format(s, " sparse-mmap");
#endif
#ifdef VFIO_REGION_INFO_CAP_TYPE
        if (cap->id == VFIO_REGION_INFO_CAP_TYPE) s = format(s, " type");
#endif
#ifdef VFIO_REGION_INFO_CAP_MSIX_MAPPABLE
        if (cap->id == VFIO_REGION_INFO_CAP_MSIX_MAPPABLE) s = format(s, " msix-mappable");
#endif
        cap_offset = cap->next;
    } while (cap_offset);
#endif
...
u8 *format_vfio_irq_set(u8 *s, va_list *args)
{
    struct vfio_irq_set *is     = va_arg(*args, struct vfio_irq_set *);
    u32                  indent = format_get_indent(s);

    s = format(s, "index:%u start:%u count:%u flags: 0x%x", is->index, is->start, is->count, is->flags);

    s = format(s, " (data:");
    if (is->flags & VFIO_IRQ_SET_DATA_NONE) s = format(s, " none");
    if (is->flags & VFIO_IRQ_SET_DATA_BOOL) s = format(s, " bool");
    if (is->flags & VFIO_IRQ_SET_DATA_EVENTFD) s = format(s, " eventfd");

    s = format(s, ", action:");
    if (is->flags & VFIO_IRQ_SET_ACTION_MASK) s = format(s, " mask");
    if (is->flags & VFIO_IRQ_SET_ACTION_UNMASK) s = format(s, " unmask");
    if (is->flags & VFIO_IRQ_SET_ACTION_TRIGGER) s = format(s, " trigger");
    vec_add1(s, ')');
