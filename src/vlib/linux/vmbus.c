...
uword unformat_vlib_vmbus_addr(unformat_input_t *input, va_list *args)
{
    ...
    if (!unformat(input, "%U", unformat_token, "a-zA-Z0-9-", &s)) return 0;

    if (vec_len(s) != 36) goto fail;

    if (s[8] != '-' || s[13] != '-' || s[18] != '-' || s[23] != '-') goto fail;
...
static clib_error_t *vlib_vmbus_raise_lower(int fd, const char *upper_name)
{
    ...
    while ((e = readdir(dir))) {
        ...
        if (strncmp(e->d_name, "lower_", 6)) continue;
    ...
    if (!e) goto done; /* no lower device */

    if (ioctl(fd, SIOCGIFFLAGS, &ifr) < 0)
        error = clib_error_return_unix(0, "ioctl fetch intf %s flags", ifr.ifr_name);
    else if (!(ifr.ifr_flags & IFF_UP)) {
        ifr.ifr_flags |= IFF_UP;

        if (ioctl(fd, SIOCSIFFLAGS, &ifr) < 0) error = clib_error_return_unix(0, "ioctl set intf %s flags", ifr.ifr_name);
    }
done:
...
clib_error_t *vlib_vmbus_bind_to_uio(vlib_vmbus_addr_t *addr)
{
    ...
    dev_dir_name = format(0, "%s/%U", sysfs_vmbus_dev_path, format_vlib_vmbus_addr, addr);

    driver_name = clib_file_get_resolved_basename("%v/driver", dev_dir_name);

    /* skip if not using the Linux kernel netvsc driver */
    if (!driver_name || strcmp("hv_netvsc", (char *) driver_name) != 0) goto done;

    /* if uio_hv_generic is not loaded, then can't use native DPDK driver. */
    if (!directory_exists("/sys/module/uio_hv_generic")) goto done;

    s   = format(s, "%v/net%c", dev_dir_name, 0);
    dir = opendir((char *) s);
    vec_reset_length(s);

    if (!dir) return clib_error_return(0, "VMBUS failed to open %s", s);
    ...
    if (!ifname) {
        error = clib_error_return(0, "VMBUS device %U eth not found", format_vlib_vmbus_addr, addr);
        goto done;
    }
    ...
    if (ifr.ifr_flags & IFF_UP) {
        error = clib_error_return(0, "Skipping VMBUS device %U as host interface %s is up", format_vlib_vmbus_addr, addr, ifname);
        close(fd);
        goto done;
    }
    ...
    if (access((char *) s, F_OK) == 0) {
        ...
        error = clib_sysfs_write((char *) s, "%U", format_vlib_vmbus_addr, addr);
...
static clib_error_t *scan_vmbus_addr(void *arg, u8 *dev_dir_name, u8 *ignored)
{
    ...
    if (!unformat(&input, "/sys/bus/vmbus/devices/%U", unformat_vlib_vmbus_addr, &addr))
...
vlib_vmbus_addr_t *vlib_vmbus_get_all_dev_addrs()
{
    ...
    err = foreach_directory_file((char *) sysfs_vmbus_dev_path, scan_vmbus_addr, &addrs, /* scan_dirs */ 0);
    if (err) {
        vec_free(addrs);
        return 0;
    }
