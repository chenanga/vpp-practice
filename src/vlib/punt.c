...
u8 *format_vlib_punt_reason(u8 *s, va_list *args)
{
    ...
    if (flags_format)
        return (format(s, "[%d] %v flags: %U", pr, punt_reason_data[pr].pd_name, flags_format, flags));
    else
...
{
    hash_set(punt_reg_db, punt_reg_mk_key(pr->pr_reason, pr->pr_node_index), pr - punt_reg_pool);
}
...
static void punt_reg_mk_dp(vlib_punt_reason_t reason)
{
    ...
    vec_foreach (prip, pris) {
        ...
        if (pr->pr_reason == reason) vec_add1(edges, pr->pr_edge);
    }
...
int vlib_punt_register(vlib_punt_hdl_t client, vlib_punt_reason_t reason, const char *node_name)
{
    ...
    if (reason >= punt_reason_last) return -1;
    if (!punt_validate_client(client)) return -2;
    ...
    if (~0 != pri) {
        ...
        pr->pr_edge       = vlib_node_add_next(vm, punt_from->index, pr->pr_node_index);

        pri = pr - punt_reg_pool;

        if (0 == punt_reason_data[reason].pd_users++ && NULL != punt_reason_data[reason].pd_fn)
            punt_reason_data[reason].pd_fn(VLIB_ENABLE, punt_reason_data[reason].pd_data);
...
int vlib_punt_unregister(vlib_punt_hdl_t client, vlib_punt_reason_t reason, const char *node_name)
{
    ...
    if (~0 != pri) {
        ...
        if (0 == pr->pr_locks) {
            if (0 == --punt_reason_data[reason].pd_users && NULL != punt_reason_data[reason].pd_fn)
                punt_reason_data[reason].pd_fn(VLIB_DISABLE, punt_reason_data[reason].pd_data);
            punt_reg_remove(pr);
...
{
    if (reason < punt_reason_last) return (0);
...
uword unformat_punt_client(unformat_input_t *input, va_list *args)
{
    ...
    return unformat_user(input, unformat_hash_vec_string, punt_client_db, result);
}
...
u8 *format_punt_reg(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U -> %U", format_vlib_punt_reason, pr->pr_reason, format_vlib_node_name, vlib_get_main(), pr->pr_node_index);
...
u8 *format_punt_reason_data(u8 *s, va_list *args)
{
    ...
    u32                *pci;
    if (pd->flags_format)
        s = format(s, "[%d] %v flags: %U from:[", pd->pd_reason, pd->pd_name, pd->flags_format, pd->flags);
    else
...
static clib_error_t *punt_client_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (~0 != pci) {
        vlib_cli_output(vm, "%U", format_punt_client, pci, PUNT_FORMAT_FLAG_DETAIL);
    }
    else {
        u8 *name;

        hash_foreach(name, pci, punt_client_db, ({ vlib_cli_output(vm, "%U", format_punt_client, pci, PUNT_FORMAT_FLAG_NONE); }));
    }
...
static clib_error_t *punt_db_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    hash_foreach(key, pri, punt_reg_db, ({ vlib_cli_output(vm, " %U", format_punt_reg, pri); }));

    vlib_cli_output(vm, "\nDerived data-plane data-base:");
    vlib_cli_output(vm, "  (for each punt-reason the edge[s] from punt-dispatch)");
...
static clib_error_t *punt_stats_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (ii = 0; ii < vlib_combined_counter_n_counters(cm); ii++) {
        ...
        vlib_cli_output(vm, "%U packets:%lld bytes:%lld", format_vlib_punt_reason, ii, c.packets, c.bytes);
    }
