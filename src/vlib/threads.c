...
static inline void barrier_trace_sync(f64 t_entry, f64 t_open, f64 t_closed)
{
    if (!vlib_worker_threads->barrier_elog_enabled) return;
    ...
    ed->caller   = elog_string(&vlib_global_main.elog_main, (char *) vlib_worker_threads[0].barrier_caller);
    ed->t_entry  = (int) (1000000.0 * t_entry);
    ed->t_open   = (int) (1000000.0 * t_open);
    ed->t_closed = (int) (1000000.0 * t_closed);
}

static inline void barrier_trace_sync_rec(f64 t_entry)
{
    if (!vlib_worker_threads->barrier_elog_enabled) return;
...
static inline void barrier_trace_release_rec(f64 t_entry)
{
    if (!vlib_worker_threads->barrier_elog_enabled) return;
...
static inline void barrier_trace_release(f64 t_entry, f64 t_closed_total, f64 t_update_main)
{
    if (!vlib_worker_threads->barrier_elog_enabled) return;
...
void vlib_set_thread_name(char *name)
{
    ...
    if (thread) {
        ...
        if (rv) clib_warning("pthread_setname_np returned %d", rv);
    }
}
...
static int sort_registrations_by_no_clone(void *a0, void *a1)
{
    ...
    return ((i32) ((*tr0)->no_data_structure_clone) - ((i32) ((*tr1)->no_data_structure_clone)));
}
...
clib_error_t *vlib_thread_init(vlib_main_t *vm)
{
    ...
    stats_num_worker_threads_dir_index = vlib_stats_add_gauge("/sys/num_worker_threads");
    ...
    for (i = 0; i < tm->skip_cores; i++) {
        ...
        if (c == ~0) return clib_error_return(0, "no available cpus to skip");

        avail_cpu = clib_bitmap_set(avail_cpu, c, 0);
    }

    /* if main thread affinity is unspecified, set to current running cpu */
    if (tm->main_lcore == ~0) tm->main_lcore = sched_getcpu();
    ...
    if (!tm->cpu_socket_bitmap) tm->cpu_socket_bitmap = clib_bitmap_set(0, 0, 1);
    ...
    if (tm->main_lcore != ~0) {
        ...
        if (pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), &cpuset)) {
            return clib_error_return(0, "could not pin main thread to cpu %u", tm->main_lcore);
        }
    }
    ...
    if (tm->sched_policy != ~0) {
        ...
        if (!sched_getparam(w->lwp, &sched_param)) {
            if (tm->sched_priority != ~0) sched_param.sched_priority = tm->sched_priority;
    ...
    for (i = 0; i < vec_len(tm->registrations); i++) {
        ...
        /* construct coremask */
        if (tr->use_pthreads || !tr->count) continue;
...
void vlib_worker_thread_init(vlib_worker_thread_t *w)
{
    ...
    if (rv) clib_warning("Failed to set the worker signal mask");

    clib_mem_set_heap(w->thread_mheap);

    if (vec_len(tm->thread_prefix) && w->registration->short_name) {
        w->name = format(0, "%v_%s_%d%c", tm->thread_prefix, w->registration->short_name, w->instance_id, '\0');
...
void *vlib_worker_thread_bootstrap_fn(void *arg)
{
    ...
    if (CLIB_DEBUG > 0) {
        ...
        if (frame_addr < (void *) w->thread_stack || frame_addr > (void *) w->thread_stack + VLIB_THREAD_STACK_SIZE) {
            /* heap is not set yet */
...
void vlib_get_thread_core_numa(vlib_worker_thread_t *w, unsigned cpu_id)
{
    ...
    clib_bitmap_foreach (node, nbmp) {
        ...
        if (clib_bitmap_get(cbmp, cpu_id)) numa_id = node;
...
static clib_error_t *vlib_launch_thread_int(void *fp, vlib_worker_thread_t *w, unsigned cpu_id)
{
    ...
    if (mm->per_numa_mheaps[w->numa_id] == 0) {
        ...
        if (tm->numa_heap_size) {
            ...
            numa_heap = clib_mem_create_heap(0 /* DIY */, tm->numa_heap_size, 1 /* is_locked */, "numa %u heap", w->numa_id);
    ...
    if (pthread_attr_init(&attr)) return clib_error_return_unix(0, "pthread_attr_init");

    if (pthread_attr_setstack(&attr, w->thread_stack, VLIB_THREAD_STACK_SIZE)) return clib_error_return_unix(0, "pthread_attr_setstack");

    if (pthread_create(&worker, &attr, fp_arg, (void *) w)) return clib_error_return_unix(0, "pthread_create");

    if (pthread_setaffinity_np(worker, sizeof(cpu_set_t), &cpuset)) return clib_error_return_unix(0, "pthread_setaffinity_np");

    if (pthread_attr_destroy(&attr)) return clib_error_return_unix(0, "pthread_attr_destroy");
...
static clib_error_t *start_workers(vlib_main_t *vm)
{
    ...
    vgm->elog_main.lock    = clib_mem_alloc_aligned(CLIB_CACHE_LINE_BYTES, CLIB_CACHE_LINE_BYTES);
    vgm->elog_main.lock[0] = 0;

    clib_callback_data_init(&vm->vlib_node_runtime_perf_callbacks, &vm->worker_thread_main_loop_callback_lock);

    vec_validate_aligned(vgm->vlib_mains, n_vlib_mains - 1, CLIB_CACHE_LINE_BYTES);
    vec_set_len(vgm->vlib_mains, 0);
    vec_add1_aligned(vgm->vlib_mains, vm, CLIB_CACHE_LINE_BYTES);

    if (n_vlib_mains > 1) {
        vlib_worker_threads->wait_at_barrier    = clib_mem_alloc_aligned(sizeof(u32), CLIB_CACHE_LINE_BYTES);
        vlib_worker_threads->workers_at_barrier = clib_mem_alloc_aligned(sizeof(u32), CLIB_CACHE_LINE_BYTES);

        vlib_worker_threads->node_reforks_required = clib_mem_alloc_aligned(sizeof(u32), CLIB_CACHE_LINE_BYTES);
        ...
        *vlib_worker_threads->node_reforks_required      = 0;
        ...
        for (i = 0; i < vec_len(tm->registrations); i++) {
            ...
            for (k = 0; k < tr->count; k++) {
                ...
                if (tr->mheap_size)
                    w->thread_mheap = clib_mem_create_heap(0,
                                                           tr->mheap_size,
                                                           /* unlocked */ 0,
                                                           "%s%d heap",
                                                           tr->name,
                ...
                w->thread_stack        = vlib_thread_stack_init(w - vlib_worker_threads);
                w->thread_function     = tr->function;
                w->thread_function_arg = w;
                w->instance_id         = k;
                w->registration        = tr;

                w->elog_track.name = (char *) format(0, "%s %d", tr->name, k + 1);
                vec_add1(w->elog_track.name, 0);
                elog_track_register(vlib_get_elog_main(), &w->elog_track);

                if (tr->no_data_structure_clone) continue;

                /* Fork vlib_global_main et al. Look for bugs here */
                oldheap = clib_mem_set_heap(w->thread_mheap);

                vm_clone = clib_mem_alloc_aligned(sizeof(*vm_clone), CLIB_CACHE_LINE_BYTES);
                clib_memcpy(vm_clone, vlib_get_first_main(), sizeof(*vm_clone));

                vm_clone->thread_index         = worker_thread_index;
                vm_clone->pending_rpc_requests = 0;
                vec_validate(vm_clone->pending_rpc_requests, 0);
                vec_set_len(vm_clone->pending_rpc_requests, 0);
                clib_memset(&vm_clone->random_buffer, 0, sizeof(vm_clone->random_buffer));
                clib_spinlock_init(&vm_clone->worker_thread_main_loop_callback_lock);
                clib_callback_data_init(&vm_clone->vlib_node_runtime_perf_callbacks, &vm_clone->worker_thread_main_loop_callback_lock);

                nm       = &vlib_get_first_main()->node_main;
                nm_clone = &vm_clone->node_main;
                /* fork next frames array, preserving node runtime indices */
                nm_clone->next_frames = vec_dup_aligned(nm->next_frames, CLIB_CACHE_LINE_BYTES);
                ...
                for (j = 0; j < vec_len(nm->nodes); j++) {
                    ...
                    clib_memset(&n->stats_last_clear, 0, sizeof(n->stats_last_clear));
                    vec_add1(nm_clone->nodes, n);
                    n++;
                }
                nm_clone->nodes_by_type[VLIB_NODE_TYPE_INTERNAL] = vec_dup_aligned(nm->nodes_by_type[VLIB_NODE_TYPE_INTERNAL], CLIB_CACHE_LINE_BYTES);
                vec_foreach (rt, nm_clone->nodes_by_type[VLIB_NODE_TYPE_INTERNAL]) {
                    vlib_node_t *n = vlib_get_node(vm, rt->node_index);
                    /* copy initial runtime_data from node */
                    if (n->runtime_data && n->runtime_data_bytes > 0)
                        clib_memcpy(rt->runtime_data, n->runtime_data, clib_min(VLIB_NODE_RUNTIME_DATA_SIZE, n->runtime_data_bytes));
                }

                nm_clone->nodes_by_type[VLIB_NODE_TYPE_INPUT] = vec_dup_aligned(nm->nodes_by_type[VLIB_NODE_TYPE_INPUT], CLIB_CACHE_LINE_BYTES);
                clib_interrupt_init(&nm_clone->input_node_interrupts, vec_len(nm_clone->nodes_by_type[VLIB_NODE_TYPE_INPUT]));
                clib_interrupt_init(&nm_clone->pre_input_node_interrupts, vec_len(nm_clone->nodes_by_type[VLIB_NODE_TYPE_PRE_INPUT]));
                ...
                nm_clone->nodes_by_type[VLIB_NODE_TYPE_PRE_INPUT] =
                    vec_dup_aligned(nm->nodes_by_type[VLIB_NODE_TYPE_PRE_INPUT], CLIB_CACHE_LINE_BYTES);
                ...
                vec_add1_aligned(vgm->vlib_mains, vm_clone, CLIB_CACHE_LINE_BYTES);

                /* Switch to the stats segment ... */
                vlib_stats_validate(stats_err_entry_index, worker_thread_index, vec_len(fvm->error_main.counters) - 1);
                ...
                vm_clone->error_main.counters_last_clear =
                    vec_dup_aligned(vlib_get_first_main()->error_main.counters_last_clear, CLIB_CACHE_LINE_BYTES);
        ...
        for (i = 0; i < vec_len(tm->registrations); i++) {
            ...
            for (j = 0; j < tr->count; j++) {
                ...
                w->thread_function_arg = w;
                w->instance_id         = j;
                w->elog_track.name     = (char *) format(0, "%s %d", tr->name, j + 1);
                w->registration        = tr;
                vec_add1(w->elog_track.name, 0);
                elog_track_register(vlib_get_elog_main(), &w->elog_track);
            }
    ...
    for (i = 0; i < vec_len(tm->registrations); i++) {
        ...
        if (tr->use_pthreads || tm->use_pthreads) {
            for (j = 0; j < tr->count; j++) {
                ...
                err = vlib_launch_thread_int(vlib_worker_thread_bootstrap_fn, w, 0);
                if (err) clib_unix_error("%U, thread %s init on cpu %d failed", format_clib_error, err, tr->name, 0);
            }
        }
        else {
            uword c;
            clib_bitmap_foreach (c, tr->coremask) {
                ...
                if (err) clib_unix_error("%U, thread %s init on cpu %d failed", format_clib_error, err, tr->name, c);
            }
        }
    }
    ...
    {
        clib_error_t *err;
        err = vlib_call_init_exit_functions(vm, &vgm->num_workers_change_function_registrations, 1 /* call_once */, 1 /* is_global */);
        if (err) clib_error_report(err);
    }
...
static inline void worker_thread_node_runtime_update_internal(void)
{
    int                  i, j;
    vlib_main_t         *vm;
    vlib_node_main_t    *nm, *nm_clone;
...
void vlib_worker_thread_node_refork(void)
{
    ...
    clib_memcpy_fast(&vm_clone->error_main, &vm->error_main, sizeof(vm->error_main));
    ...
    /* Allocate all nodes in single block for speed */
    new_n_clone = clib_mem_alloc_no_fail(vec_len(nm->nodes) * sizeof(*new_n_clone));
    for (j = 0; j < vec_len(nm->nodes); j++) {
        ...
        if (j >= vec_len(old_nodes_clone)) {
            ...
            clib_memset(&new_n_clone->stats_total, 0, sizeof(new_n_clone->stats_total));
            clib_memset(&new_n_clone->stats_last_clear, 0, sizeof(new_n_clone->stats_last_clear));
        }
        else {
            vlib_node_t *old_n_clone = old_nodes_clone[j];
            /* Copy stats if the old data is valid */
            clib_memcpy_fast(&new_n_clone->stats_total, &old_n_clone->stats_total, sizeof(new_n_clone->stats_total));
            clib_memcpy_fast(&new_n_clone->stats_last_clear, &old_n_clone->stats_last_clear, sizeof(new_n_clone->stats_last_clear));
    ...
    vec_foreach (rt, nm_clone->nodes_by_type[VLIB_NODE_TYPE_INTERNAL]) {
        ...
        if (n->runtime_data && n->runtime_data_bytes > 0)
            clib_memcpy_fast(rt->runtime_data, n->runtime_data, clib_min(VLIB_NODE_RUNTIME_DATA_SIZE, n->runtime_data_bytes));
    }
    ...
    nm_clone->nodes_by_type[VLIB_NODE_TYPE_INPUT] = vec_dup_aligned(nm->nodes_by_type[VLIB_NODE_TYPE_INPUT], CLIB_CACHE_LINE_BYTES);
    clib_interrupt_resize(&nm_clone->input_node_interrupts, vec_len(nm_clone->nodes_by_type[VLIB_NODE_TYPE_INPUT]));
    clib_interrupt_resize(&nm_clone->pre_input_node_interrupts, vec_len(nm_clone->nodes_by_type[VLIB_NODE_TYPE_PRE_INPUT]));
    ...
    nm_clone->nodes_by_type[VLIB_NODE_TYPE_PRE_INPUT] = vec_dup_aligned(nm->nodes_by_type[VLIB_NODE_TYPE_PRE_INPUT], CLIB_CACHE_LINE_BYTES);
...
static clib_error_t *cpu_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "use-pthreads"))
            ...
            ;
        else if (unformat(input, "numa-heap-size %U", unformat_memory_size, &tm->numa_heap_size))
            ;
        else if (unformat(input, "coremask-%s %U", &name, unformat_bitmap_mask, &bitmap)
                 || unformat(input, "corelist-%s %U", &name, unformat_bitmap_list, &bitmap)) {
            p = hash_get_mem(tm->thread_registrations_by_name, name);
            if (p == 0) return clib_error_return(0, "no such thread type '%s'", name);

            tr = (vlib_thread_registration_t *) p[0];

            if (tr->use_pthreads) return clib_error_return(0, "corelist cannot be set for '%s' threads", name);
            if (tr->count) return clib_error_return(0, "core placement of '%s' threads is already configured", name);

            tr->coremask = bitmap;
            tr->count    = clib_bitmap_count_set_bits(tr->coremask);
            use_corelist = 1;
        }
        else if (unformat(input, "scheduler-policy %U", unformat_sched_policy, &tm->sched_policy))
            ;
        else if (unformat(input, "scheduler-priority %u", &tm->sched_priority))
            ;
        else if (unformat(input, "%s %u", &name, &count)) {
            p = hash_get_mem(tm->thread_registrations_by_name, name);
            if (p == 0) return clib_error_return(0, "no such thread type 3 '%s'", name);

            tr = (vlib_thread_registration_t *) p[0];

            if (tr->fixed_count) return clib_error_return(0, "number of '%s' threads not configurable", name);
            if (tr->count) return clib_error_return(0, "number of '%s' threads is already configured", name);
    ...
    if (use_corelist && tm->main_lcore == ~0)
        return clib_error_return(0,
                                 "main-core must be specified when using "
    ...
    if (tm->sched_priority != ~0) {
        if (tm->sched_policy == SCHED_FIFO || tm->sched_policy == SCHED_RR) {
            ...
            if (tm->sched_priority > prio_max) tm->sched_priority = prio_max;
            if (tm->sched_priority < prio_min) tm->sched_priority = prio_min;
        }
        else {
            return clib_error_return(0, "scheduling priority (%d) is not allowed for `normal` scheduling policy", tm->sched_priority);
        }
    }
    tr = tm->next;

    if (!tm->thread_prefix) tm->thread_prefix = format(0, "vpp");
...
{
    if (vlib_get_n_threads() < 2) return (1);
...
void vlib_worker_thread_barrier_sync_int(vlib_main_t *vm, const char *func_name)
{
    ...
    if (vlib_get_n_threads() < 2) return;
    ...
    if (PREDICT_FALSE(vec_len(vm->barrier_perf_callbacks) != 0))
        clib_call_callbacks(vm->barrier_perf_callbacks, vm, vm->clib_time.last_cpu_time, 0 /* enter */);
    ...
    for (i = 1; i < vlib_get_n_threads(); i++) {
        ...
        max_vector_rate  = clib_max(max_vector_rate, (f64) vlib_last_vectors_per_main_loop(ovm));
    }
    ...
    if (max_vector_rate > 10.0) {
        while (1) {
            ...
            if (now >= vm->barrier_no_close_before) break;
            if ((vm->barrier_no_close_before - now) > (2.0 * BARRIER_MINIMUM_OPEN_LIMIT)) {
                clib_warning("clock change: would have waited for %.4f seconds", (vm->barrier_no_close_before - now));
                break;
            }
        }
    }
...
void vlib_worker_thread_barrier_release(vlib_main_t *vm)
{
    ...
    if (vgm->need_vlib_worker_thread_node_runtime_update) {
        ...
        refork_needed = 1;
        clib_atomic_fetch_add(vlib_worker_threads->node_reforks_required, (vlib_get_n_threads() - 1));
        now           = vlib_time_now(vm);
    ...
    *vlib_worker_threads->wait_at_barrier = 0;
    ...
    if (PREDICT_FALSE(vec_len(vm->barrier_perf_callbacks) != 0))
        clib_call_callbacks(vm->barrier_perf_callbacks, vm, vm->clib_time.last_cpu_time, 1 /* leave */);
}

static void vlib_worker_sync_rpc(void *args)
{
...
{
    if (PREDICT_FALSE(!vlib_num_workers())) return;

    if (!(*vlib_worker_threads->wait_at_barrier) && !clib_atomic_swap_rel_n(&vlib_worker_threads->wait_before_barrier, 1)) {
        u32 thread_index = vlib_get_thread_index();
        vlib_rpc_call_main_thread(vlib_worker_sync_rpc, (u8 *) &thread_index, sizeof(thread_index));
    ...
    while (vlib_num_workers() > (*vlib_worker_threads->workers_at_barrier + vlib_worker_threads->workers_before_barrier))
        ;
}
...
void vlib_workers_continue(void)
{
    ...
    while (vlib_worker_threads->done_work_before_barrier < vlib_worker_threads->workers_before_barrier)
        ;
...
void vlib_worker_wait_one_loop(void)
{
    ...
    if (vlib_worker_thread_barrier_held()) return;
...
void vlib_worker_thread_fn(void *arg)
{
    ...
    e = vlib_call_init_exit_functions_no_sort(vm, &vgm->worker_init_function_registrations, 1 /* call_once */, 0 /* is_global */);
...
extern clib_march_fn_registration *vlib_frame_queue_dequeue_with_aux_fn_march_fn_registrations;
extern clib_march_fn_registration *vlib_frame_queue_dequeue_fn_march_fn_registrations;
u32                                vlib_frame_queue_main_init(u32 node_index, u32 frame_queue_nelts)
{
    ...
    if (frame_queue_nelts == 0) frame_queue_nelts = FRAME_QUEUE_MAX_NELTS;
    ...
    ASSERT(node);
    if (node->aux_offset) {
        fqm->frame_queue_dequeue_fn = CLIB_MARCH_FN_VOID_POINTER(vlib_frame_queue_dequeue_with_aux_fn);
    }
    else {
        fqm->frame_queue_dequeue_fn = CLIB_MARCH_FN_VOID_POINTER(vlib_frame_queue_dequeue_fn);
    }
...
{
    ASSERT(vlib_get_thread_index() == 0);
    vlib_process_signal_event(vlib_get_main(), args->node_index, args->type_opaque, args->data);
}
...
clib_error_t *threads_init(vlib_main_t *vm)
{
    ...
    if (tm->main_lcore == ~0 && tm->n_vlib_mains > 1)
        return clib_error_return(0,
                                 "Configuration error, a main core must "
...
static clib_error_t *show_clock_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    clib_timebase_init(tb, 0 /* GMT */, CLIB_TIMEBASE_DAYLIGHT_NONE, &vm->clib_time);

    vlib_cli_output(vm, "%U, %U GMT", format_clib_time, &vm->clib_time, verbose, format_clib_timebase_time, clib_timebase_now(tb));

    vlib_cli_output(vm, "Time last barrier release %.9f", vm->time_last_barrier_release);

    foreach_vlib_main () {
        vlib_cli_output(vm, "%d: %U", this_vlib_main->thread_index, format_clib_time, &this_vlib_main->clib_time, verbose);

        vlib_cli_output(vm,
                        "Thread %d offset %.9f error %.9f",
                        this_vlib_main->thread_index,
                        this_vlib_main->time_offset,
                        vm->time_last_barrier_release - this_vlib_main->time_last_barrier_release);
    }
    return 0;
}
