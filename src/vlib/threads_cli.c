...
static clib_error_t *show_threads_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (i = 0; i < vec_len(vlib_worker_threads); i++) {
        ...
        line = format(line, "%-7d%-20s%-12s%-8d", i, w->name ? w->name : (u8 *) "", w->registration ? w->registration->name : "", w->lwp);
...
static clib_error_t *trace_frame_queue(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (index > vec_len(tm->frame_queue_mains) - 1) {
        error = clib_error_return(0, "expecting valid worker handoff queue index");
        goto done;
    }
    ...
    vec_validate_aligned(fqm->frame_queue_traces, num_fq - 1, CLIB_CACHE_LINE_BYTES);
    vec_validate_aligned(fqm->frame_queue_histogram, num_fq - 1, CLIB_CACHE_LINE_BYTES);
