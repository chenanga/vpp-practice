...
clib_error_t *vlib_physmem_shared_map_create(vlib_main_t *vm, char *name, uword size, u32 log2_page_sz, u32 numa_node, u32 *map_index)
{
    ...
    va = clib_pmalloc_create_shared_arena(pm, name, size, log2_page_sz, numa_node);

    if (va == 0) return clib_error_return(0, "%U", format_clib_error, clib_pmalloc_last_error(pm));
    ...
    *map_index = map->index = map - vpm->maps;
    ...
    for (i = 0; i < a->n_pages; i++) {
        uword pa = clib_pmalloc_get_pa(pm, (u8 *) va + (i << a->log2_subpage_sz));

        /* maybe iova */
        if (pa == 0) pa = pointer_to_uword(va);
...
clib_error_t *vlib_physmem_init(vlib_main_t *vm)
{
    ...
    if (pt && pt[0]) vpm->flags |= VLIB_PHYSMEM_MAIN_F_HAVE_PAGEMAP;
    vec_free(pt);

#ifdef __linux__
    if ((error = linux_vfio_init(vm))) return error;
#endif /* __linux__ */

    p = clib_mem_alloc_aligned(sizeof(clib_pmalloc_main_t), CLIB_CACHE_LINE_BYTES);
    memset(p, 0, sizeof(clib_pmalloc_main_t));
    vpm->pmalloc_main = (clib_pmalloc_main_t *) p;

    if (vpm->base_addr == 0) vpm->base_addr = VLIB_PHYSMEM_DEFAULT_BASE_ADDDR;
    ...
    vpm->max_size  = (uword) vpm->pmalloc_main->max_pages << vpm->pmalloc_main->def_log2_page_sz;
...
static clib_error_t *vlib_physmem_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "base-addr 0x%lx", &vpm->base_addr))
            ;
        else if (unformat(input, "max-size %U", unformat_memory_size, &vpm->max_size))
            ;
        else
