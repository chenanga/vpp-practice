...
void vlib_stats_segment_lock(void)
{
    ...
    if (sm->shared_header->in_progress && vm->thread_index == sm->locking_thread_index) goto done;
...
static void hash_set_str_key_alloc(uword **h, const char *key, uword v)
{
...
static void hash_unset_str_key_free(uword **h, const char *key)
{
...
u32 vlib_stats_create_counter(vlib_stats_entry_t *e)
{
    vlib_stats_segment_t *sm = vlib_stats_get_segment();
    u32                   index;
...
void vlib_stats_remove_entry(u32 entry_index)
{
    ...
    if (entry_index >= vec_len(sm->directory_vector)) return;
...
static void vlib_stats_set_entry_name(vlib_stats_entry_t *e, char *s)
{
...
static u32 vlib_stats_new_entry_internal(stat_directory_type_t t, u8 *name)
{
    vlib_stats_segment_t       *sm            = vlib_stats_get_segment();
...
vlib_stats_string_vector_t vlib_stats_add_string_vector(char *fmt, ...)
{
    ...
    if (index == CLIB_U32_MAX) return 0;

    sv                                        = vec_new_generic(vlib_stats_string_vector_t, 0, sizeof(vlib_stats_header_t), 0, sm->heap);
    sh                                        = vec_header(sv);
...
void vlib_stats_set_string_vector(vlib_stats_string_vector_t *svp, u32 vector_index, char *fmt, ...)
{
    vlib_stats_segment_t *sm = vlib_stats_get_segment();
    ...
    if (fmt[0] == 0) {
        if (vec_len(e->string_vector) <= vector_index) return;

        if (e->string_vector[vector_index] == 0) return;
    ...
    if (s == 0) s = vec_new_heap(u8 *, 0, sm->heap);
...
u32 vlib_stats_add_counter_vector(char *fmt, ...)
{
    ...
    name = va_format(0, fmt, &va);
    va_end(va);
    return vlib_stats_new_entry_internal(STAT_DIR_TYPE_COUNTER_VECTOR_SIMPLE, name);
}
...
u32 vlib_stats_add_counter_pair_vector(char *fmt, ...)
{
    ...
    name = va_format(0, fmt, &va);
    va_end(va);
    return vlib_stats_new_entry_internal(STAT_DIR_TYPE_COUNTER_VECTOR_COMBINED, name);
}

static int vlib_stats_validate_will_expand_internal(u32 entry_index, va_list *va)
{
    vlib_stats_segment_t *sm = vlib_stats_get_segment();
    vlib_stats_entry_t   *e  = vlib_stats_get_entry(sm, entry_index);
    void                 *oldheap;
    ...
    if (e->type == STAT_DIR_TYPE_COUNTER_VECTOR_SIMPLE) {
        ...
        if (idx0 >= vec_len(data)) goto done;

        for (u32 i = 0; i <= idx0; i++)
            if (idx1 >= vec_max_len(data[i])) goto done;
    }
...
void vlib_stats_validate(u32 entry_index, ...)
{
    ...
    if (will_expand) vlib_stats_segment_lock();
    ...
    if (will_expand) vlib_stats_segment_unlock();
}
...
void vlib_stats_register_collector_fn(vlib_stats_collector_reg_t *reg)
{
    vlib_stats_segment_t   *sm = vlib_stats_get_segment();
