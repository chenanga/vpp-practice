...
typedef enum { STAT_COUNTER_HEARTBEAT = 0, STAT_COUNTER_LAST_STATS_CLEAR, STAT_COUNTER_BOOTTIME, STAT_COUNTERS } stat_segment_counter_t;
...
typedef struct {
    ...
    void                       *heap;
    vlib_stats_shared_header_t *shared_header; /* pointer to shared memory segment */
    int                         memfd;
...
static_always_inline vlib_stats_segment_t *vlib_stats_get_segment()
{
    return &vlib_stats_main.segment;
}

static_always_inline vlib_stats_entry_t *vlib_stats_get_entry(vlib_stats_segment_t *sm, u32 entry_index)
{
...
static_always_inline void *vlib_stats_get_entry_data_pointer(u32 entry_index)
{
...
void                       vlib_stats_set_string_vector(vlib_stats_string_vector_t *sv, u32 index, char *fmt, ...);
