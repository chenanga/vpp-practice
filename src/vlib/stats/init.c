...
static void vector_rate_collector_fn(vlib_stats_collector_data_t *d)
{
    ...
    loops_counters = vlib_stats_get_entry_data_pointer(vlib_loops_stats_counter_index);
...
clib_error_t *vlib_stats_init(vlib_main_t *vm)
{
    ...
    if (memory_size == 0) memory_size = STAT_SEGMENT_DEFAULT_SIZE;

    if (sm->log2_page_sz == CLIB_MEM_PAGE_SZ_UNKNOWN) sm->log2_page_sz = CLIB_MEM_PAGE_SZ_DEFAULT;

    mfd = clib_mem_vm_create_fd(sm->log2_page_sz, mem_name);

    if (mfd == -1) return clib_error_return(0, "stat segment memory fd failure: %U", format_clib_error, clib_mem_get_last_error());
    /* Set size */
    ...
    if (memaddr == CLIB_MEM_VM_MAP_FAILED) return clib_error_return(0, "stat segment mmap failure");

    sys_page_sz = clib_mem_get_page_size();

    heap      = clib_mem_create_heap(((u8 *) memaddr) + sys_page_sz, memory_size - sys_page_sz, 1 /* locked */, mem_name);
    sm->heap  = heap;
    sm->memfd = mfd;
    ...
    sm->directory_vector          = vec_new_heap(typeof(sm->directory_vector[0]), STAT_COUNTERS, heap);
...
#undef _
    ...
    reg.entry_index                = vlib_stats_add_counter_vector("/sys/vector_rate_per_worker");
    vlib_loops_stats_counter_index = vlib_stats_add_counter_vector("/sys/loops_per_worker");
...
static clib_error_t *statseg_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        /* DEPRECATE: default (does nothing) */
            ...
            ;
        else if (unformat(input, "size %U", unformat_memory_size, &sm->memory_size))
            ;
        else if (unformat(input, "page-size %U", unformat_log2_page_size, &sm->log2_page_sz))
            ;
     ...
     */
    if (vec_len(sm->socket_name)) vec_terminate_c_string(sm->socket_name);
...
static clib_error_t *stats_segment_socket_init(void)
{
    ...
    s->flags  = CLIB_SOCKET_F_IS_SERVER | CLIB_SOCKET_F_SEQPACKET | CLIB_SOCKET_F_ALLOW_GROUP_WRITE | CLIB_SOCKET_F_PASSCRED;

    if ((error = clib_socket_init(s))) return error;
...
static clib_error_t *statseg_init(vlib_main_t *vm)
{
    ...
    if (!vec_len(sm->socket_name)) sm->socket_name = format(0, "%s/%s%c", vlib_unix_get_runtime_dir(), STAT_SEGMENT_SOCKET_FILENAME, 0);
