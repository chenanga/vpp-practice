...
enum { NODE_CLOCKS, NODE_VECTORS, NODE_CALLS, NODE_SUSPENDS, N_NODE_COUNTERS };
...
static inline void update_node_counters(vlib_stats_segment_t *sm)
{
    clib_bitmap_t *bmp       = 0;
    ...
    vlib_node_get_nodes(0 /* vm, for barrier sync */, (u32) ~0 /* all threads */, 1 /* include stats */, 0 /* barrier sync */, &node_dups, &stat_vms);
    ...
    for (i = 0; i < n_nodes; i++)
        if (vec_is_equal(node_data[i].name, node_dups[0][i]->name) == 0) bmp = clib_bitmap_set(bmp, i, 1);
    ...
    if (bmp) {
        ...
        clib_bitmap_foreach (i, bmp) {
            ...
            for (int j = 0; j < ARRAY_LEN(node_counters); j++) {
                vlib_stats_validate(node_counters[j].entry_index, last_thread, n_nodes - 1);
                node_data[i].symlinks[j] = vlib_stats_add_symlink(node_counters[j].entry_index,
                                                                  n->index,
                                                                  "/nodes/%U/%s",
                                                                  format_vlib_stats_symlink,
                                                                  n->name,
                                                                  node_counters[j].name);
    ...
    for (j = 0; j < vec_len(node_dups); j++) {
        ...
        for (i = 0; i < vec_len(nodes); i++) {
            ...
            counters    = vlib_stats_get_entry_data_pointer(node_counters[NODE_CLOCKS].entry_index);
            c           = counters[j];
            c[n->index] = n->stats_total.clocks - n->stats_last_clear.clocks;

            counters    = vlib_stats_get_entry_data_pointer(node_counters[NODE_VECTORS].entry_index);
            c           = counters[j];
            c[n->index] = n->stats_total.vectors - n->stats_last_clear.vectors;

            counters    = vlib_stats_get_entry_data_pointer(node_counters[NODE_CALLS].entry_index);
            c           = counters[j];
            c[n->index] = n->stats_total.calls - n->stats_last_clear.calls;

            counters    = vlib_stats_get_entry_data_pointer(node_counters[NODE_SUSPENDS].entry_index);
            c           = counters[j];
...
static void do_stat_segment_updates(vlib_main_t *vm, vlib_stats_segment_t *sm)
{
    if (sm->node_counters_enabled) update_node_counters(sm);
...
static uword stat_segment_collector_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    if (sm->node_counters_enabled) {
        ...
        for (int x = 0; x < ARRAY_LEN(node_counters); x++) {
            node_counters[x].entry_index = vlib_stats_add_counter_vector("/sys/node/%s", node_counters[x].name);
