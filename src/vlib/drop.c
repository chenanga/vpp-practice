...
static u8 *validate_error(vlib_main_t *vm, vlib_error_t *e, u32 index)
{
    ...
    if (node_index >= vec_len(vm->node_main.nodes)) return format(0, "[%d], node index out of range 0x%x, error 0x%x", index, node_index, e[0]);

    n = vlib_get_node(vm, node_index);
    if (code >= n->n_errors) return format(0, "[%d], code %d out of range for node %v", index, code, n->name);
...
always_inline u32 counter_index(vlib_main_t *vm, vlib_error_t e)
{
    ...
    if (ci >= n->n_errors) return CLIB_U32_MAX;
...
static_always_inline uword process_drop_punt(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, error_disposition_t disposition)
{
    ...
    if (node->flags & VLIB_NODE_FLAG_TRACE) trace_errors(vm, node, frame);
    ...
    while (n_left) {
        ...
        if (c_index != CLIB_U32_MAX) em->counters[c_index] += count;
    ...
    if (disposition == ERROR_DISPOSITION_DROP || !vm->os_punt_frame) {
        ...
        if (disposition == ERROR_DISPOSITION_PUNT && !vm->os_punt_frame) vlib_frame_free(vm, frame);
    }
    else
...
VLIB_REGISTER_NODE(error_punt_node) = {
    .name           = "punt",
    .flags          = (VLIB_NODE_FLAG_FRAME_NO_FREE_AFTER_DISPATCH | VLIB_NODE_FLAG_IS_PUNT),
    .vector_size    = sizeof(u32),
    .format_trace   = format_error_trace,
    .validate_frame = validate_error_frame,
};
