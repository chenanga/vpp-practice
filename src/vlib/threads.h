...
typedef u32(vlib_frame_queue_dequeue_fn_t)(vlib_main_t *vm, struct vlib_frame_queue_main_t_ *fqm);
...
#define vlib_worker_thread_barrier_sync(X)                    \
    {                                                         \
        vlib_worker_thread_barrier_sync_int(X, __FUNCTION__); \
    }
...
{
    if (CLIB_DEBUG > 0) {
        if (vlib_get_thread_index()) fformat(stderr, "%s: SMP unsafe warning...\n", __FUNCTION__);
    }
}

always_inline int __foreach_vlib_main_helper(vlib_main_t *ii, vlib_main_t **p, int checks)
{
    vlib_main_t *vm;
    u32          index = ii - (vlib_main_t *) 0;

    if (index >= vec_len(vlib_global_main.vlib_mains)) return 0;
...
#define foreach_vlib_main__(checks)                                                                           \
    for (vlib_main_t *ii = 0, *this_vlib_main; __foreach_vlib_main_helper(ii, &this_vlib_main, checks); ii++) \
        if (this_vlib_main)
...
#ifdef __linux__
#define foreach_sched_policy foreach_sched_policy_posix foreach_sched_policy_linux
#else
...
#define VLIB_REGISTER_THREAD(x, ...)                                                                                  \
    ...
    static void                            __vlib_add_thread_registration_##x(void) __attribute__((__constructor__)); \
    ...
    static void __vlib_rm_thread_registration_##x(void) __attribute__((__destructor__));                              \
...
always_inline u32 vlib_num_workers()
{
    return vlib_thread_main.n_vlib_mains - 1;
}

always_inline u32 vlib_get_worker_thread_index(u32 worker_index)
{
    return worker_index + 1;
}

always_inline u32 vlib_get_worker_index(u32 thread_index)
{
    return thread_index - 1;
}

always_inline u32 vlib_get_current_worker_index()
{
...
static inline void vlib_worker_thread_barrier_check(void)
{
    if (PREDICT_FALSE(*vlib_worker_threads->wait_at_barrier)) {
        ...
        if (CLIB_DEBUG > 0) vm->parked_at_barrier = 0;
        clib_atomic_fetch_add(vlib_worker_threads->workers_at_barrier, -1);
...
always_inline vlib_main_t *vlib_get_worker_vlib_main(u32 worker_index)
{
    vlib_main_t        *vm;
    vlib_thread_main_t *tm = &vlib_thread_main;
...
static inline u8 vlib_thread_is_main_w_barrier(void)
{
