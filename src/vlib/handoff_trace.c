...
static u8 *format_handoff_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "HANDED-OFF: from thread %d trace index %d", t->prev_thread, t->prev_trace_index);
    return s;
}
...
static uword handoff_trace_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    vlib_node_increment_counter(vm, node->node_index, HANDOFF_TRACE_ERROR_BUGS, frame->n_vectors);
...
int vlib_add_handoff_trace(vlib_main_t *vm, vlib_buffer_t *b)
{
    ...
    handoff_trace_t     *t;
    vlib_node_runtime_t *node = vlib_node_get_runtime(vm, handoff_trace_node.index);

    if (PREDICT_FALSE(!vlib_trace_buffer(vm, node, 0 /* fake next frame index */, b, 1 /* follow chain */))) return 0;
