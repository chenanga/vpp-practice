...
#define VLIB_PLUGIN_REGISTER() vlib_plugin_registration_t vlib_plugin_registration __clib_export __clib_section(".vlib_plugin_registration")
...
#define vlib_call_plugin_init_function(vm, p, x)                                                                                   \
    ({                                                                                                                             \
        ...
        _fptr                = vlib_get_plugin_symbol(p, CLIB_STRING_MACRO(_vlib_init_function_##x));                              \
        if (_fptr == 0) {                                                                                                          \
            _error = clib_error_return(0, "Plugin %s and/or symbol %s not found.", p, CLIB_STRING_MACRO(_vlib_init_function_##x)); \
        }                                                                                                                          \
        else {                                                                                                                     \
            _f = (void *) (_fptr[0]);                                                                                              \
        }                                                                                                                          \
