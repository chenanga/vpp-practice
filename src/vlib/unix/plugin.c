...
void *vlib_get_plugin_symbol(const char *plugin_name, const char *symbol_name)
{
    ...
    if ((p = hash_get_mem(pm->plugin_by_name_hash, plugin_name)) == 0) return 0;
...
static clib_error_t *r2_to_reg(elf_main_t *em, vlib_plugin_r2_t *r2, vlib_plugin_registration_t *reg, elf_section_t *data_section)
{
...
#undef _
    ...
    if (r2->version.length >= ARRAY_LEN(reg->version) - 1) return clib_error_return(0, "Version string too long");

    if (r2->version_required.length >= ARRAY_LEN(reg->version_required) - 1) return clib_error_return(0, "Version-required string too long");

    if (r2->overrides.length >= ARRAY_LEN(reg->overrides) - 1) return clib_error_return(0, "Override string too long");

    /* Compatibility with C-initializer */
    memcpy((void *) reg->version, data + r2->version.data_segment_offset, r2->version.length);
    memcpy((void *) reg->version_required, data + r2->version_required.data_segment_offset, r2->version_required.length);
    memcpy((void *) reg->overrides, data + r2->overrides.data_segment_offset, r2->overrides.length);

    if (r2->early_init.length > 0) {
        u8 *ei = 0;
        vec_validate(ei, r2->early_init.length + 1);
        memcpy(ei, data + r2->early_init.data_segment_offset, r2->early_init.length);
    ...
    if (r2->description.length > 0) {
        ...
        memcpy(desc, data + r2->description.data_segment_offset, r2->description.length);
...
static int load_one_plugin(plugin_main_t *pm, plugin_info_t *pi, int from_early_init)
{
    ...
    if (elf_read_file(&em, (char *) pi->filename)) return -1;
    ...
    if (error == 0) {
            ...
            vec_foreach (rt, em.relocation_tables) {
                vec_foreach (r, rt->relocations) {
                    if (r->address >= symok->value && r->address < symok->value + symok->size) {
                        *(uword *) ((void *) data + r->address - symok->value) += r->addend - data_section->header.exec_address;
                    }
                }
            }
        }
        ...
        if (pm->plugins_default_disable) reg->default_disabled = 1;
        reread_reg = 0;
        goto process_reg;
    }
    else
...
process_reg:
    ...
    version_required = str_array_to_vec((char *) &reg->version_required, sizeof(reg->version_required));

    if ((strlen(version_required) > 0) && (strncmp(vlib_plugin_app_version, version_required, strlen(version_required)))) {
        PLUGIN_LOG_ERR("Plugin %s version mismatch: %s != %s", pi->name, vlib_plugin_app_version, reg->version_required);
    ...
    if (reg->overrides[0]) {
        ...
        while (1) {
            if (*sp == 0 || (sp >= (u8 *) overrides + ARRAY_LEN(reg->overrides))) break;
            if (*sp == ' ' || *sp == ',') {
                sp++;
                continue;
            }
            ep = sp;
            while (*ep && *ep != ' ' && *ep != ',' && ep < (u8 *) overrides + ARRAY_LEN(reg->overrides))
                ep++;
            if (*ep == ' ' || *ep == ',') ep--;
            ...
            if (p)
                ...
                hash_set_mem(pm->plugin_overrides_by_name_hash, override_name_copy, overridden_by_name_copy);
            }
            sp = *ep ? ep + 1 : ep;
        }
    }
    ...
    handle = dlopen((char *) pi->filename, RTLD_LAZY | (reg->deep_bind ? RTLD_DEEPBIND : 0));
#else
    ...
    if (reread_reg) reg = dlsym(pi->handle, "vlib_plugin_registration");
    ...
    if (reg->early_init) {
        ...
        if (h) {
            ...
            if (error) {
                u8 *err = format(0, "%s: %U%c", pi->name, format_clib_error, error, 0);
            ...
            PLUGIN_LOG_ERR("Plugin %s: early init function %s set but not found", (char *) pi->name, reg->early_init);
    }
        ...
        while ((entry = readdir(dp))) {
            ...
            if (pm->plugin_name_filter) {
                ...
                for (j = 0; j < vec_len(pm->plugin_name_filter); j++)
                    if (entry->d_name[j] != pm->plugin_name_filter[j]) goto next;
            }
            ...
            /* unreadable */
            if (!ext || (strcmp(ext, ".so") != 0) || stat((char *) filename, &statb) < 0) {
            ignore:
                vec_free(filename);
                continue;
            }
            ...
            if (!S_ISREG(statb.st_mode)) goto ignore;
...
int vlib_plugin_early_init(vlib_main_t *vm)
{
    ...
    if (pm->plugin_path == 0) pm->plugin_path = format(0, "%s", vlib_plugin_path);

    if (pm->plugin_path_add) pm->plugin_path = format(pm->plugin_path, ":%s", pm->plugin_path_add);
...
static clib_error_t *vlib_plugins_show_cmd_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    hash_foreach_mem(key, value, pm->plugin_by_name_hash, {
        if (key != 0) {
            ...
            s  = format(s, "%3d. %-40s %-32s %s\n", index, key, pi->version, (pi->reg && pi->reg->description) ? pi->reg->description : "");
            index++;
        }
    });
...
static clib_error_t *config_one_plugin(vlib_main_t *vm, char *name, unformat_input_t *input)
{
    ...
    if (pm->config_index_by_name == 0) pm->config_index_by_name = hash_create_string(0, sizeof(uword));
...
clib_error_t *vlib_plugin_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        if (unformat(input, "%s %v", &s, &v)) {
            if (strncmp((const char *) s, "plugins", 8) == 0) {
                if (vec_len(in.buffer) > 0) vec_add1(in.buffer, ' ');
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(input, "plugin default %U", unformat_vlib_cli_sub_input, &sub_input)) {
            pm->plugins_default_disable = unformat(&sub_input, "disable") ? 1 : 0;
            unformat_free(&sub_input);
        }
        else if (unformat(input, "plugin %s %U", &s, unformat_vlib_cli_sub_input, &sub_input)) {
