...
always_inline void msg_handler(mc_main_t *mcm, u32 buffer_index, u32 handler_frees_buffer, void *_h)
{
    ...
    if (!handler_frees_buffer) vlib_buffer_free_one(vm, buffer_index);
}
...
static clib_error_t *sendmsg_helper(mc_socket_main_t *msm, int socket, struct sockaddr_in *tx_addr, u32 buffer_index)
{
    ...
    if (msm->iovecs) vec_set_len(msm->iovecs, 0);

    n_bytes = append_buffer_index_to_iovec(vm, buffer_index, &msm->iovecs);
    ASSERT(n_bytes <= msm->mc_main.transport.max_packet_size);
    if (n_bytes > msm->mc_main.transport.max_packet_size) clib_error("sending packet larger than interface MTU %d bytes", n_bytes);

    h.msg_iov    = msm->iovecs;
    h.msg_iovlen = vec_len(msm->iovecs);

    n_retries = 0;
    while ((n_bytes_tx = sendmsg(socket, &h, /* flags */ 0)) != n_bytes && errno == EAGAIN)
        n_retries++;
...
static clib_error_t *tx_buffer(void *transport, mc_transport_type_t type, u32 buffer_index)
{
    ...
    if (type != MC_TRANSPORT_USER_REQUEST_TO_RELAY) vlib_buffer_free_one(vm, buffer_index);
    return error;
}
...
static clib_error_t *recvmsg_helper(mc_socket_main_t *msm, int socket, struct sockaddr_in *rx_addr, u32 *buffer_index, u32 drop_message)
{
...

    {
        ...
        if (n_bytes_left < 0) return clib_error_return_unix(0, "recvmsg");
    }
...
static clib_error_t *from_relay_socket_read_ready(clib_file_t *uf)
{
    ...
    error = recvmsg_helper(msm,
                           ...
                           0);
    if (!error) {
        msg_handler(mcm, bi, /* handler_frees_buffer */ 1, mc_msg_user_request_handler);
    }
    return error;
}
    ...
    if (!error) {
        ...
        switch (clib_host_to_net_u32(mp->type)) {
            ...
            case MC_MSG_TYPE_join_reply:
                msg_handler(mcm, bi, /* handler_frees_buffer */ 0, mc_msg_join_reply_handler);
                break;
...
static clib_error_t *ack_socket_read_ready(clib_file_t *uf)
{
    ...
    error = recvmsg_helper(msm,
                           ...
                           &bi,
                           /* drop_message */ 0);
    if (!error) msg_handler(mcm, bi, /* handler_frees_buffer */ 0, mc_msg_user_ack_handler);
    return error;
}
...
static clib_error_t *catchup_socket_write_ready(clib_file_t *uf, int is_server)
{
    ...
    while (1) {
        ...
        n_this_write = clib_min(vec_len(c->output_vector) - c->output_vector_n_written, msm->rx_mtu_n_bytes - 64 /* ip + tcp + option allowance */);

        if (n_this_write <= 0) break;

        do {
            n = write(uf->file_descriptor, c->output_vector + c->output_vector_n_written, n_this_write);
...
static clib_error_t *catchup_listen_read_ready(clib_file_t *uf)
{
    ...
    c->socket = accept(uf->file_descriptor, (struct sockaddr *) &client_addr, (socklen_t *) &client_len);
    ...
    {
        int one = 1;
        if ((setsockopt(c->socket, IPPROTO_TCP, TCP_NODELAY, (void *) &one, sizeof(one))) < 0) {
...
static word find_and_bind_to_free_port(word sock, word port)
{
    for (; port < 1 << 16; port++) {
        ...
        if (bind(sock, (struct sockaddr *) &a, sizeof(a)) >= 0) break;
    }
...
static clib_error_t *setup_mutlicast_socket(mc_socket_main_t *msm, mc_multicast_socket_t *ms, char *type, uword udp_port)
{
    ...
    if (!msm->multicast_ttl) msm->multicast_ttl = 1;

    /* mastership (multicast) TX socket */
    if ((ms->socket = socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0) return clib_error_return_unix(0, "%s socket", type);

    {
        u8 ttl = msm->multicast_ttl;

        if ((setsockopt(ms->socket, IPPROTO_IP, IP_MULTICAST_TTL, (void *) &ttl, sizeof(ttl))) < 0)
            return clib_error_return_unix(0, "%s set multicast ttl", type);
    }

    if (setsockopt(ms->socket, SOL_SOCKET, SO_REUSEADDR, &one, sizeof(one)) < 0) return clib_error_return_unix(0, "%s setsockopt SO_REUSEADDR", type);
    ...
    if (bind(ms->socket, (struct sockaddr *) &ms->tx_addr, sizeof(ms->tx_addr)) < 0) return clib_error_return_unix(0, "%s bind", type);
    ...
    if ((setsockopt(ms->socket, IPPROTO_IP, IP_ADD_MEMBERSHIP, (void *) &mcast_req, sizeof(mcast_req))) < 0)
        return clib_error_return_unix(0, "%s IP_ADD_MEMBERSHIP setsockopt", type);

    if (ioctl(ms->socket, FIONBIO, &one) < 0) return clib_error_return_unix(0, "%s set FIONBIO", type);

    /* FIXME remove this when we support tx_ready. */
    {
        u32       len = 1 << 20;
        socklen_t sl  = sizeof(len);
        if (setsockopt(ms->socket, SOL_SOCKET, SO_SNDBUF, &len, sl) < 0) clib_unix_error("setsockopt");
    }
...
static clib_error_t *socket_setup(mc_socket_main_t *msm)
{
    ...
    if (!msm->base_multicast_udp_port_host_byte_order)
        msm->base_multicast_udp_port_host_byte_order = 0xffff - ((MC_N_TRANSPORT_TYPE + 2 /* ack socket, catchup socket */) - 1);

    port = msm->base_multicast_udp_port_host_byte_order;

    error = setup_mutlicast_socket(msm, &msm->multicast_sockets[MC_TRANSPORT_MASTERSHIP], "mastership", port++);
    if (error) return error;

    error = setup_mutlicast_socket(msm, &msm->multicast_sockets[MC_TRANSPORT_JOIN], "join", port++);
    if (error) return error;

    error = setup_mutlicast_socket(msm, &msm->multicast_sockets[MC_TRANSPORT_USER_REQUEST_TO_RELAY], "to relay", port++);
    if (error) return error;

    error = setup_mutlicast_socket(msm, &msm->multicast_sockets[MC_TRANSPORT_USER_REQUEST_FROM_RELAY], "from relay", port++);
    if (error) return error;

    /* ACK rx socket */
    msm->ack_socket = socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (msm->ack_socket < 0) return clib_error_return_unix(0, "ack socket");

    msm->ack_udp_port = find_and_bind_to_free_port(msm->ack_socket, port++);

    if (ioctl(msm->ack_socket, FIONBIO, &one) < 0) return clib_error_return_unix(0, "ack socket FIONBIO");

    msm->catchup_server_socket = socket(AF_INET, SOCK_STREAM, 0);
    if (msm->catchup_server_socket < 0) return clib_error_return_unix(0, "catchup server socket");

    msm->catchup_tcp_port = find_and_bind_to_free_port(msm->catchup_server_socket, port++);

    if (ioctl(msm->catchup_server_socket, FIONBIO, &one) < 0) return clib_error_return_unix(0, "catchup server socket FIONBIO");

    if (listen(msm->catchup_server_socket, 5) < 0) return clib_error_return_unix(0, "catchup server socket listen");
...
{
    clib_file_t *uf     = pool_elt_at_index(file_main.file_pool, c->clib_file_index);
    u8          *result = 0;
    ...
    if (vec_len(c->output_vector) > 0) {
        ...
        uf->flags |= UNIX_FILE_DATA_AVAILABLE_TO_WRITE;
        if (!skip_update) file_main.file_update(uf, UNIX_FILE_UPDATE_MODIFY);
    }
    return result;
}
...
static uword catchup_request_fun(void *transport_main, u32 stream_index, mc_peer_id_t catchup_peer_id)
{
    ...
    if (connect(c->socket, (const void *) &addr, sizeof(addr)) < 0 && errno != EINPROGRESS) {
        clib_unix_warning("connect to %U fails", format_socket_peer_id, catchup_peer_id);
        return 0;
    }
...

    {
        ...
        mp               = catchup_add_pending_output(c,
                                        sizeof(mp[0]), /* set_output_vector */
                                        0);
...
static int find_interface_ip4_address(char *if_name, u32 *ip4_address, u32 *mtu)
{
    ...
    if (mtu) *mtu = ifr.ifr_mtu - (/* IP4 header */ 20 + /* UDP header */ 8);
...
clib_error_t *mc_socket_main_init(mc_socket_main_t *msm, char **intfc_probe_list, int n_intfcs_to_probe)
{
    ...
    /* 239.255.0.7 */
    if (!msm->multicast_tx_ip4_address_host_byte_order) msm->multicast_tx_ip4_address_host_byte_order = 0xefff0007;

    {
        ...
        if (!win) return clib_error_return(0, "can't find interface ip4 address");

        msm->if_ip4_address_net_byte_order = a;
    }

    msm->rx_mtu_n_bytes   = mtu;
    msm->rx_mtu_n_buffers = msm->rx_mtu_n_bytes / vlib_buffer_get_default_data_size(vm);
    msm->rx_mtu_n_buffers += (msm->rx_mtu_n_bytes % vlib_buffer_get_default_data_size(vm)) != 0;

    error = socket_setup(msm);
    if (error) return error;

    mcm->transport.our_ack_peer_id = mc_socket_set_peer_id(msm->if_ip4_address_net_byte_order, msm->ack_udp_port);

    mcm->transport.our_catchup_peer_id = mc_socket_set_peer_id(msm->if_ip4_address_net_byte_order, msm->catchup_tcp_port);
