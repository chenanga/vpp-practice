        ...
        switch (e->action) {
            ...
            case VLIB_LEX_ADD_TO_NUMBER:
                ...
                if (tv >= lm->current_number_base) {
                    tv = 10 + c - 'A';
                    if (tv >= lm->current_number_base) tv = 10 + c - 'a';
                }
            ...
            case VLIB_LEX_KEYWORD_CHECK: {
                ...
                if (p) {
                    rv->token = p[0];
                    if (LEX_DEBUG > 0) clib_warning("keyword '%s' token %s", lm->token_buffer, lm->lex_token_names[rv->token]);
                }
                else {
                    /* it's a WORD */
                    rv->token            = VLIB_LEX_word;
                    rv->value.as_pointer = vec_dup(lm->token_buffer);
                    if (LEX_DEBUG > 0) clib_warning("%s, value '%s'", lm->lex_token_names[VLIB_LEX_word], rv->value.as_pointer);
                }
            ...
            case VLIB_LEX_RETURN:
                ...
                if (LEX_DEBUG > 0) {
                    clib_warning("table %s char '%c'(0x%02x) next table %s return %s",
                                 ...
                                 c,
                                 c,
                                 lm->lex_tables[e->next_table_index].name,
                                 lm->lex_token_names[e->token]);
                    if (rv->token == VLIB_LEX_number) clib_warning("  numeric value 0x%x (%d)", rv->value, rv->value);
                }
                return;
        }
    }
...
u16 vlib_lex_add_table(char *name)
{
    ...
    vlib_lex_set_action_range(t - lm->lex_tables, 1, 0x7F, VLIB_LEX_IGNORE, ~0, t - lm->lex_tables);

    vlib_lex_set_action_range(t - lm->lex_tables, 0, 0, VLIB_LEX_RETURN, VLIB_LEX_eof, t - lm->lex_tables);
...
{
    if (lm->pushback_vector) vec_set_len(lm->pushback_vector, 0);
    lm->pushback_sp = -1;
...
#define _(f)                                  \
    {                                         \
        u16 tmp = vlib_lex_add_token(lm, #f); \
        ASSERT(tmp == VLIB_LEX_##f);          \
    }
