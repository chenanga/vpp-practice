...
always_inline u32 punt_replicate(vlib_main_t         *vm,
                                 ...
                                 u32                 *n_dispatched)
{
    ...
    n_cloned0 = vlib_buffer_clone(vm, bi0, punt_clones[thread_index], n_clones0, 2 * CLIB_CACHE_LINE_BYTES);
...
always_inline u32 punt_dispatch_one(vlib_main_t                  *vm,
                                    vlib_node_runtime_t          *node,
                                    ...
                                    u32                          *n_dispatched)
{
    ...
    if (PREDICT_FALSE(pr0 >= vec_len(punt_dp_db))) {
        ...
        vlib_increment_combined_counter(cm, thread_index, pr0, 1, vlib_buffer_length_in_chain(vm, b0));
        ...
        if (PREDICT_TRUE(1 == vec_len(punt_dp_db[pr0]))) {
            ...
            *n_dispatched = *n_dispatched + 1;
             ...
             */
            next0 = punt_replicate(vm, node, thread_index, b0, bi0, pr0, next_index, n_left_to_next, to_next, n_dispatched);
        }
    }
...
VLIB_NODE_FN(punt_dispatch_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 4 && n_left_to_next > 2) {
            ...
            next0 = punt_dispatch_one(vm, node, cm, thread_index, bi0, &next_index, &n_left_to_next, &to_next, &n_dispatched);
    ...
    vlib_node_increment_counter(vm, node->node_index, PUNT_ERROR_DISPATCHED, n_dispatched);
