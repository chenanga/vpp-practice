...
typedef enum { FIFO_SEGMENT_FTYPE_NONE = -1, FIFO_SEGMENT_RX_FIFO = 0, FIFO_SEGMENT_TX_FIFO, FIFO_SEGMENT_N_FTYPES } fifo_segment_ftype_t;
...
always_inline void *fifo_segment_ptr(fifo_segment_t *fs, uword offset)
{
    return (void *) ((u8 *) fs->h + offset);
}

always_inline uword fifo_segment_offset(fifo_segment_t *fs, void *p)
{
