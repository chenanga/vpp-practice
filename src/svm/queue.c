...
svm_queue_t *svm_queue_init(void *base, int nels, int elsize)
{
    ...
    if (pthread_mutex_init(&q->mutex, &attr)) clib_unix_warning("mutex_init");
    ...
    if (pthread_cond_init(&q->condvar, &cattr)) clib_unix_warning("cond_init1");
...
svm_queue_t *svm_queue_alloc_and_init(int nels, int elsize, int consumer_pid)
{
    ...
    q = clib_mem_alloc_aligned(sizeof(svm_queue_t) + nels * elsize, CLIB_CACHE_LINE_BYTES);
    clib_memset(q, 0, sizeof(*q));
...
void svm_queue_lock(svm_queue_t *q)
{
    ...
    if (PREDICT_FALSE(rv == EOWNERDEAD)) pthread_mutex_consistent(&q->mutex);
}

static int svm_queue_trylock(svm_queue_t *q)
{
    int rv = pthread_mutex_trylock(&q->mutex);
    if (PREDICT_FALSE(rv == EOWNERDEAD)) rv = pthread_mutex_consistent(&q->mutex);
    return rv;
}
...
static inline void svm_queue_send_signal_inline(svm_queue_t *q, u8 is_prod)
{
    if (q->producer_evtfd == -1) {
        ...
        if (PREDICT_FALSE(rv < 0)) clib_unix_warning("signal write on %d returned %d", fd, rv);
    }
}
...
static inline void svm_queue_wait_inline(svm_queue_t *q)
{
    if (q->producer_evtfd == -1) {
...
static inline int svm_queue_timedwait_inline(svm_queue_t *q, double timeout)
{
    struct timespec ts;
...
int svm_queue_add_nolock(svm_queue_t *q, u8 *elem)
{
    ...
    if (need_broadcast) svm_queue_send_signal_inline(q, 1);
    return 0;
}
...
void svm_queue_add_raw(svm_queue_t *q, u8 *elem)
{
    ...
    if (q->cursize == 1) svm_queue_send_signal_inline(q, 1);
}
...
int svm_queue_sub(svm_queue_t *q, u8 *elem, svm_q_conditional_wait_t cond, u32 time)
{
    ...
    if (q->cursize == q->maxsize) need_broadcast = 1;

    q->cursize--;

    if (q->head == q->maxsize) q->head = 0;

    if (need_broadcast) svm_queue_send_signal_inline(q, 0);
...
int svm_queue_sub2(svm_queue_t *q, u8 *elem)
{
    ...
    if (PREDICT_FALSE(q->head == q->maxsize)) q->head = 0;
    svm_queue_unlock(q);
...
int svm_queue_sub_raw(svm_queue_t *q, u8 *elem)
{
    ...
    if (PREDICT_FALSE(need_broadcast)) svm_queue_send_signal_inline(q, 0);
