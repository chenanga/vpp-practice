...
always_inline void ssvm_lock_non_recursive(ssvm_shared_header_t *h, u32 tag)
{
    while (clib_atomic_test_and_set(&h->lock))
        CLIB_PAUSE();
...
always_inline void ssvm_unlock_non_recursive(ssvm_shared_header_t *h)
{
    h->tag = 0;
    clib_atomic_release(&h->lock);
}
...
static inline void *ssvm_mem_alloc(ssvm_private_t *ssvm, uword size)
{
    clib_mem_heap_t *oldheap;
    void            *rv;
