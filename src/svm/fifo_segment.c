...
static inline void *fsh_alloc_aligned(fifo_segment_header_t *fsh, uword size, uword align)
{
    ...
    if (new_pos >= fsh->max_byte_index) return 0;

    while (!clib_atomic_cmp_and_swap_acq_relax(&fsh->byte_index, &cur_pos, &new_pos, 1 /* weak */)) {
        cur_pos_align = round_pow2_u64(cur_pos, align);
        new_pos       = cur_pos_align + size;
        if (new_pos >= fsh->max_byte_index) return 0;
    }
...
static inline void *fsh_alloc(fifo_segment_header_t *fsh, uword size)
{
    return fsh_alloc_aligned(fsh, size, 8);
}

static inline fifo_segment_slice_t *fsh_slice_get(fifo_segment_header_t *fsh, u32 slice_index)
{
    return &fsh->slices[slice_index];
}

static inline fifo_slice_private_t *fs_slice_private_get(fifo_segment_t *fs, u32 slice_index)
{
...
static inline uword fsh_n_free_bytes(fifo_segment_header_t *fsh)
{
...
static inline void fsh_cached_bytes_add(fifo_segment_header_t *fsh, uword size)
{
    clib_atomic_fetch_add_rel(&fsh->n_cached_bytes, size);
}

static inline void fsh_cached_bytes_sub(fifo_segment_header_t *fsh, uword size)
{
    clib_atomic_fetch_sub_rel(&fsh->n_cached_bytes, size);
}

static inline uword fsh_n_cached_bytes(fifo_segment_header_t *fsh)
{
    uword n_cached = clib_atomic_load_relax_n(&fsh->n_cached_bytes);
    return n_cached;
}

static inline void fsh_active_fifos_update(fifo_segment_header_t *fsh, int inc)
{
    clib_atomic_fetch_add_rel(&fsh->n_active_fifos, inc);
}

static inline u32 fsh_n_active_fifos(fifo_segment_header_t *fsh)
{
    return clib_atomic_load_relax_n(&fsh->n_active_fifos);
}

static inline uword fs_virtual_mem(fifo_segment_t *fs)
{
...
static inline int fss_chunk_fl_index_is_valid(fifo_segment_slice_t *fss, u32 fl_index)
{
...
static void fss_chunk_free_list_push(fifo_segment_header_t *fsh, fifo_segment_slice_t *fss, u32 fl_index, svm_fifo_chunk_t *c)
{
    ...
    } while (!__atomic_compare_exchange(&fss->free_chunks[fl_index], &old_head, &new_head, 0 /* weak */, __ATOMIC_RELEASE, __ATOMIC_ACQUIRE));
}
...
static inline void pfss_fifo_add_active_list(fifo_slice_private_t *pfss, svm_fifo_t *f)
{
...
static inline void pfss_fifo_del_active_list(fifo_slice_private_t *pfss, svm_fifo_t *f)
{
...
static inline uword fss_fl_chunk_bytes(fifo_segment_slice_t *fss)
{
    return clib_atomic_load_relax_n(&fss->n_fl_chunk_bytes);
}

static inline void fss_fl_chunk_bytes_add(fifo_segment_slice_t *fss, uword size)
{
    clib_atomic_fetch_add_relax(&fss->n_fl_chunk_bytes, size);
}

static inline void fss_fl_chunk_bytes_sub(fifo_segment_slice_t *fss, uword size)
{
...
int fifo_segment_init(fifo_segment_t *fs)
{
    ...
    for (i = 0; i < fs->n_slices; i++)
        fs->slices[i].fifos = clib_mem_bulk_init(sizeof(svm_fifo_t), CLIB_CACHE_LINE_BYTES, 32);
...
int fifo_segment_attach(fifo_segment_main_t *sm, fifo_segment_create_args_t *a)
{
    ...
    if (!fs->ssvm.sh->opaque[0]) goto done;

    fsh = fs->h        = (void *) fs->ssvm.sh + (uword) fs->ssvm.sh->opaque[0];
    fs->max_byte_index = fsh->max_byte_index;
    vec_validate(fs->slices, 0);
    fs->slices[0].fifos = clib_mem_bulk_init(sizeof(svm_fifo_t), CLIB_CACHE_LINE_BYTES, 32);
...
static inline u32 fs_freelist_for_size(u32 size)
{
    ...
    return clib_min(max_log2(size) - FIFO_SEGMENT_MIN_LOG2_FIFO_SIZE, FS_CHUNK_VEC_LEN - 1);
}

static inline u32 fs_freelist_index_to_size(u32 fl_index)
{
    return 1 << (fl_index + FIFO_SEGMENT_MIN_LOG2_FIFO_SIZE);
}

static inline int fs_chunk_size_is_valid(fifo_segment_header_t *fsh, u32 size)
{
    /*
     * 4K minimum. It's not likely that anything good will happen
     * with a smaller FIFO.
     */
    return size >= FIFO_SEGMENT_MIN_FIFO_SIZE && size <= (1ULL << fsh->max_log2_fifo_size);
}
...
{
    if (fsh_try_alloc_fifo_hdr_batch(fsh, fss, batch_size)) return 0;
...
static svm_fifo_shared_t *fsh_try_alloc_fifo_hdr(fifo_segment_header_t *fsh, fifo_segment_slice_t *fss)
{
    ...
    if (!fss->free_fifos) {
        if (fsh_try_alloc_fifo_hdr_batch(fsh, fss, FIFO_SEGMENT_ALLOC_BATCH_SIZE)) return 0;
    }
...
free_list:
    ...
    if (c) {
        ...
        if (chunk_size <= n_free) {
            ...
            if (!fsh_try_alloc_chunk_batch(fsh, fss, fl_index, batch)) goto free_list;
        }
...
void fs_fifo_free(fifo_segment_t *fs, svm_fifo_t *f, u32 slice_index)
{
    ...
    if (CLIB_DEBUG) clib_memset(f, 0xfc, sizeof(*f));
...
void fifo_segment_detach_fifo(fifo_segment_t *fs, svm_fifo_t **f)
{
    ...
    if (of->flags & SVM_FIFO_F_LL_TRACKED) pfss_fifo_del_active_list(pfss, of);
...
void fifo_segment_attach_fifo(fifo_segment_t *fs, svm_fifo_t **f, u32 slice_index)
{
    ...
    nf->next = nf->prev = 0;
    if (nf->flags & SVM_FIFO_F_LL_TRACKED) pfss_fifo_add_active_list(pfss, nf);
...
void fifo_segment_msg_qs_discover(fifo_segment_t *fs, int *fds, u32 n_fds)
{
    ...
    for (i = 0; i < n_mqs; i++) {
        ...
        smq = (svm_msg_q_shared_t *) ((u8 *) fs->h + offset);
        svm_msg_q_attach(mq, smq);
        if (n_fds) svm_msg_q_set_eventfd(mq, fds[i]);
        offset += size;
    }
}
...
void fifo_segment_preallocate_fifo_pairs(fifo_segment_t *fs, u32 rx_fifo_size, u32 tx_fifo_size, u32 *n_fifo_pairs)
{
    ...
    /* Parameter check */
    if (rx_fifo_size == 0 || tx_fifo_size == 0 || *n_fifo_pairs == 0) return;
    ...
    if (!pairs_per_slice) return;

    for (i = 0; i < fs->n_slices; i++) {
        alloc_now = clib_min(pairs_per_slice, *n_fifo_pairs);
        if (0 == alloc_now) break;

        fss = fsh_slice_get(fsh, i);
        if (fs_try_alloc_fifo_batch(fsh, fss, rx_fl_index, alloc_now)) clib_warning("rx prealloc failed: pairs %u", alloc_now);
        if (fs_try_alloc_fifo_batch(fsh, fss, tx_fl_index, alloc_now)) clib_warning("tx prealloc failed: pairs %u", alloc_now);
...
{
    if (!fs->high_watermark || !fs->low_watermark) return MEMORY_PRESSURE_NO_PRESSURE;
...
u8 *format_fifo_segment(u8 *s, va_list *args)
{
    ...
    s = format(s,
               "%U%v type: %U size: %U active fifos: %u",
               format_white_space,
               2,
               ssvm_name(&fs->ssvm),
    ...
    for (slice_index = 0; slice_index < fs->n_slices; slice_index++) {
        ...
        for (i = 0; i < FS_CHUNK_VEC_LEN; i++) {
            c = fss_chunk_free_list_head(fsh, fss, i);
            if (c == 0 && fss->num_chunks[i] == 0) continue;
            count = 0;
            while (c) {
                c = fs_chunk_ptr(fsh, c->next);
                count++;
            ...
            s          = format(s, "%U%-5u kB: %u/%u\n", format_white_space, indent + 2, chunk_size >> 10, count, fss->num_chunks[i]);
