 ...
 * Internal function.
 */
static inline void f_load_head_tail_cons(svm_fifo_t *f, u32 *head, u32 *tail)
{
 ...
 * Internal function
 */
static inline void f_load_head_tail_prod(svm_fifo_t *f, u32 *head, u32 *tail)
{
 ...
 * Internal function.
 */
static inline void f_load_head_tail_all_acq(svm_fifo_t *f, u32 *head, u32 *tail)
{
 ...
 * Internal function.
 */
static inline u32 f_cursize(svm_fifo_t *f, u32 head, u32 tail)
{
    return tail - head;
}
 ...
 * Internal function
 */
static inline u32 f_free_count(svm_fifo_t *f, u32 head, u32 tail)
{
    return (f->shr->size - f_cursize(f, head, tail));
}

always_inline u32 f_chunk_end(svm_fifo_chunk_t *c)
{
    return c->start_byte + c->length;
}

always_inline int f_pos_lt(u32 a, u32 b)
{
    return ((i32) (a - b) < 0);
}

always_inline int f_pos_leq(u32 a, u32 b)
{
    return ((i32) (a - b) <= 0);
}

always_inline int f_pos_gt(u32 a, u32 b)
{
    return ((i32) (a - b) > 0);
}

always_inline int f_pos_geq(u32 a, u32 b)
{
    return ((i32) (a - b) >= 0);
}

always_inline u8 f_chunk_includes_pos(svm_fifo_chunk_t *c, u32 pos)
{
    return (f_pos_geq(pos, c->start_byte) && f_pos_lt(pos, c->start_byte + c->length));
}

always_inline svm_fifo_chunk_t *f_start_cptr(svm_fifo_t *f)
{
    return fs_chunk_ptr(f->fs_hdr, f->shr->start_chunk);
}

always_inline svm_fifo_chunk_t *f_end_cptr(svm_fifo_t *f)
{
    return fs_chunk_ptr(f->fs_hdr, f->shr->end_chunk);
}

always_inline svm_fifo_chunk_t *f_head_cptr(svm_fifo_t *f)
{
    return fs_chunk_ptr(f->fs_hdr, f->shr->head_chunk);
}

always_inline svm_fifo_chunk_t *f_tail_cptr(svm_fifo_t *f)
{
    return fs_chunk_ptr(f->fs_hdr, f->shr->tail_chunk);
}

always_inline svm_fifo_chunk_t *f_cptr(svm_fifo_t *f, fs_sptr_t cp)
{
    return fs_chunk_ptr(f->fs_hdr, cp);
}

always_inline fs_sptr_t f_csptr(svm_fifo_t *f, svm_fifo_chunk_t *c)
{
    return fs_chunk_sptr(f->fs_hdr, c);
}

always_inline void f_csptr_link(svm_fifo_t *f, fs_sptr_t cp, svm_fifo_chunk_t *c)
{
 ...
 */
static inline u32 svm_fifo_max_dequeue_cons(svm_fifo_t *f)
{
    u32 tail, head;
 ...
 * @return	max number of bytes that can be dequeued
 */
static inline u32 svm_fifo_max_dequeue_prod(svm_fifo_t *f)
{
    u32 tail, head;
 ...
 */
static inline int svm_fifo_is_full_prod(svm_fifo_t *f)
{
 ...
 */
static inline int svm_fifo_is_full(svm_fifo_t *f)
{
 ...
 */
static inline int svm_fifo_is_empty_cons(svm_fifo_t *f)
{
 ...
 * @return	1 if fifo is empty 0 otherwise
 */
static inline int svm_fifo_is_empty_prod(svm_fifo_t *f)
{
 ...
 */
static inline int svm_fifo_is_empty(svm_fifo_t *f)
{
 ...
 */
static inline u8 svm_fifo_is_wrapped(svm_fifo_t *f)
{
    u32 head, tail;
    f_load_head_tail_all_acq(f, &head, &tail);
    return head > tail;
 ...
 */
static inline u32 svm_fifo_max_enqueue_prod(svm_fifo_t *f)
{
    u32 head, tail;
    f_load_head_tail_prod(f, &head, &tail);
 ...
 */
static inline u8 svm_fifo_n_subscribers(svm_fifo_t *f)
{
...
static inline ooo_segment_t *svm_fifo_newest_ooo_segment(svm_fifo_t *f)
{
    if (f->ooos_newest == OOO_SEGMENT_INVALID_INDEX) return 0;
    return pool_elt_at_index(f->ooo_segments, f->ooos_newest);
}

static inline void svm_fifo_newest_ooo_segment_reset(svm_fifo_t *f)
{
    f->ooos_newest = OOO_SEGMENT_INVALID_INDEX;
}

static inline u32 ooo_segment_offset_prod(svm_fifo_t *f, ooo_segment_t *s)
{
    u32 tail;
    /* load-relaxed: producer owned index */
    tail = f->shr->tail;
...
static inline u32 ooo_segment_length(svm_fifo_t *f, ooo_segment_t *s)
{
    return s->length;
}

static inline u32 svm_fifo_size(svm_fifo_t *f)
{
    return f->shr->size;
}

static inline void svm_fifo_set_size(svm_fifo_t *f, u32 size)
{
    if (size > (1 << f->fs_hdr->max_log2_fifo_size)) return;
    fsh_virtual_mem_update(f->fs_hdr, f->shr->slice_index, (int) f->shr->size - size);
    f->shr->size = size;
}
 ...
 */
static inline int svm_fifo_has_event(svm_fifo_t *f)
{
 ...
 */
always_inline u8 svm_fifo_set_event(svm_fifo_t *f)
{
 ...
 * @param f	fifo
 */
always_inline void svm_fifo_unset_event(svm_fifo_t *f)
{
 ...
 */
static inline void svm_fifo_add_want_deq_ntf(svm_fifo_t *f, u8 ntf_type)
{
 ...
 */
static inline void svm_fifo_del_want_deq_ntf(svm_fifo_t *f, u8 ntf_type)
{
 ...
 */
static inline u32 svm_fifo_get_want_deq_ntf(svm_fifo_t *f)
{
 ...
 * @param f	fifo
 */
static inline void svm_fifo_clear_deq_ntf(svm_fifo_t *f)
{
    u32 want_deq_ntf = svm_fifo_get_want_deq_ntf(f);
    /* Set the flag if want ntf if full or empty was requested */
    if (want_deq_ntf & (SVM_FIFO_WANT_DEQ_NOTIF_IF_FULL | SVM_FIFO_WANT_DEQ_NOTIF_IF_EMPTY)) clib_atomic_store_rel_n(&f->shr->has_deq_ntf, 1);
    if (want_deq_ntf & SVM_FIFO_WANT_DEQ_NOTIF) svm_fifo_del_want_deq_ntf(f, SVM_FIFO_WANT_DEQ_NOTIF);
}
 ...
 */
static inline u32 svm_fifo_has_deq_ntf(svm_fifo_t *f)
{
 ...
 * @param f	fifo
 */
static inline void svm_fifo_reset_has_deq_ntf(svm_fifo_t *f)
{
    f->shr->has_deq_ntf = 0;
}
 ...
 */
static inline u8 svm_fifo_needs_deq_ntf(svm_fifo_t *f, u32 n_last_deq)
{
    ...
    if (want_ntf & SVM_FIFO_WANT_DEQ_NOTIF_IF_FULL) {
        ...
        u32 size    = f->shr->size;
        if (max_deq < size && max_deq + n_last_deq >= size && !svm_fifo_has_deq_ntf(f)) return 1;
    }
    if (want_ntf & SVM_FIFO_WANT_DEQ_NOTIF_IF_EMPTY) {
        if (!svm_fifo_has_deq_ntf(f) && svm_fifo_is_empty(f)) return 1;
    }
    return 0;
}
 ...
 */
static inline void svm_fifo_set_deq_thresh(svm_fifo_t *f, u32 thresh)
{
