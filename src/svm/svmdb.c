...
svmdb_client_t *svmdb_map(svmdb_map_args_t *dba)
{
    ...
    if (CLIB_DEBUG > 2) {
        ...
        clib_warning("[%d] adding fake client (pid=0) so '%s' won't be unlinked", getpid(), db_rp->region_name);
    ...
    hp->namespaces[SVMDB_NAMESPACE_STRING] = hash_create_string(0, sizeof(uword));
...
static void notify_value(svmdb_value_t *v, svmdb_action_t a)
{
    ...
    for (i = 0; i < vec_len(v->notifications); i++) {
        ...
        if (np->action == a) {
            ...
            do {
                rv = 0;
                if (sigqueue(np->pid, np->signum, sv) == 0) break;
                rv = errno;
            } while (rv == EAGAIN);
            if (rv == 0) continue;
    ...
    for (i = 0; i < vec_len(dead_registrations); i++) {
        ...
        clib_warning("dead reg pid %d sig %d action %d opaque %x", np->pid, np->signum, np->action, np->opaque);
...
int svmdb_local_add_del_notification(svmdb_client_t *client, svmdb_notification_args_t *a)
{
    ...
    if (hp == 0) {
        local_set_variable_nolock(client, a->nspace, (u8 *) a->var, placeholder_value, a->elsize);
        /* might have moved */
        h  = shm->namespaces[a->nspace];
        hp = hash_get_pair_mem(h, a->var);
        ASSERT(hp);
    ...
    for (i = 0; i < vec_len(value->notifications); i++) {
        ...
        if ((np->pid == client->pid) && (np->signum == a->signum) && (np->action == a->action) && (np->opaque == a->opaque)) {
            if (a->add_del == 0 /* delete */) {
                vec_delete(value->notifications, 1, i);
                goto out;
            }
            else { /* add */
                clib_warning("%s: ignore dup reg pid %d signum %d action %d opaque %x", a->var, client->pid, a->signum, a->action, a->opaque);
                rv = -2;
                goto out;
            }
        }
...
static void local_unset_variable_nolock(svmdb_client_t *client, svmdb_namespace_t namespace, char *var)
{
    ...
    if (hp) {
        ...
        if (vec_len(oldvalue->notifications)) notify_value(oldvalue, SVMDB_ACTION_UNSET);
...
void svmdb_local_set_string_variable(svmdb_client_t *client, char *var, char *val)
{
    ...
    local_set_variable_nolock(client, SVMDB_NAMESPACE_STRING, (u8 *) var, (u8 *) val, 1 /* elsize */);
    svm_pop_heap(oldheap);
    region_unlock(client->db_rp);
}
...
void svmdb_local_dump_strings(svmdb_client_t *client)
{
    ...
    hash_foreach_mem(key, value, h, ({
                         ...
                         fformat(stdout, "%s: %s\n", key, vec_len(v->value) ? v->value : (u8 *) "(nil)");
                     }));
    region_unlock(client->db_rp);
}
...
int svmdb_local_serialize_strings(svmdb_client_t *client, char *filename)
{
    ...
    if (strstr(filename, "..") || strchr(filename, '/')) {
        error = clib_error_return(0, "Illegal characters in filename '%s'", filename);
        goto out;
    }
...
out:
    if (fd > 0 && close(fd) < 0) error = clib_error_return_unix(0, "close fd %d", fd);
...
int svmdb_local_unserialize_strings(svmdb_client_t *client, char *filename)
{
    ...
    for (i = 0; i < nelts; i++) {
        ...
        local_set_variable_nolock(client, SVMDB_NAMESPACE_STRING, key, value, 1 /* elsize */);
        vec_free(key);
        vec_free(value);
    }
    svm_pop_heap(oldheap);
...
void svmdb_local_set_vec_variable(svmdb_client_t *client, char *var, void *val_arg, u32 elsize)
{
    ...
    local_unset_variable_nolock(client, SVMDB_NAMESPACE_VEC, var);
    local_set_variable_nolock(client, SVMDB_NAMESPACE_VEC, (u8 *) var, val, elsize);
...
void svmdb_local_dump_vecs(svmdb_client_t *client)
{
    ...
    hash_foreach_mem(
        ...
        ({
            svmdb_value_t *v = pool_elt_at_index(shm->values, value);
            (void) fformat(stdout, "%s:\n %U (%.2f)\n", key, format_hex_bytes, v->value, vec_len(v->value) * v->elsize, ((f64 *) (v->value))[0]);
        }));
