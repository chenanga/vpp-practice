...
static void *svm_map_region_nolock(svm_map_region_args_t *a)
{
    ...
    if (pthread_mutex_trylock(&rp->mutex)) {
        clib_warning("rp->mutex LOCKED by pid %d, tag %d, cleared...", rp->mutex_owner_pid, rp->mutex_owner_tag);
...
static void *svm_existing_region_map_nolock(void *root_arg, svm_map_region_args_t *a)
{
    ...
    a->size += MMAP_PAGESIZE + (a->pvt_heap_size ? a->pvt_heap_size : SVM_PVT_MHEAP_SIZE);
...
void repair(char *chroot_path, int crash_root_region)
{
    ...
    root_rp = mmap(0, MMAP_PAGESIZE, PROT_READ | PROT_WRITE, MAP_SHARED, svm_fd, 0);
    ...
    root_rp = (void *) mmap(uword_to_pointer(a->baseva, void *), a->size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED, svm_fd, 0);
    ...
    if (pthread_mutex_trylock(&root_rp->mutex)) {
        clib_warning("root_rp->mutex LOCKED by pid %d, tag %d, cleared...", root_rp->mutex_owner_pid, root_rp->mutex_owner_tag);
