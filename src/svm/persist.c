...
clib_error_t *persist_malloc(persist_main_t *pm)
{
    ...
    demo1 = svmdb_local_get_variable_reference(pm->c, SVMDB_NAMESPACE_VEC, "demo1_location");
    ...
    if (demo1 == 0) {
         ...
         */
        svmdb_local_set_vec_variable(pm->c, "demo1_location", demo1, sizeof(demo1));
    }
    else {
...
clib_error_t *persist_serialize(persist_main_t *pm)
{
    ...
    /* Get back the root pointer */
    checkpoint = svmdb_local_get_vec_variable(pm->c, "demo1_checkpoint", sizeof(u8));
    ...
    if (checkpoint == 0) {
        ...
        svmdb_local_set_vec_variable(pm->c, "demo1_checkpoint", checkpoint, sizeof(u8));
...
int main(int argc, char **argv)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "malloc"))
            ...
            error = clib_error_return(0, "Unknown flavor '%U'", format_unformat_error, input);
            break;
        }
    }
