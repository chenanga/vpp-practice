...
CLIB_MARCH_FN(svm_fifo_copy_to_chunk, void, svm_fifo_t *f, svm_fifo_chunk_t *c, u32 tail_idx, const u8 *src, u32 len, fs_sptr_t *last)
{
    ...
    ASSERT(f_pos_geq(tail_idx, c->start_byte) && f_pos_lt(tail_idx, c->start_byte + c->length));
...
CLIB_MARCH_FN(svm_fifo_copy_from_chunk, void, svm_fifo_t *f, svm_fifo_chunk_t *c, u32 head_idx, u8 *dst, u32 len, fs_sptr_t *last)
{
    ...
    ASSERT(f_pos_geq(head_idx, c->start_byte) && f_pos_lt(head_idx, c->start_byte + c->length));
...
static inline void svm_fifo_copy_to_chunk(svm_fifo_t *f, svm_fifo_chunk_t *c, u32 tail_idx, const u8 *src, u32 len, fs_sptr_t *last)
{
    CLIB_MARCH_FN_SELECT(svm_fifo_copy_to_chunk)(f, c, tail_idx, src, len, last);
}

static inline void svm_fifo_copy_from_chunk(svm_fifo_t *f, svm_fifo_chunk_t *c, u32 head_idx, u8 *dst, u32 len, fs_sptr_t *last)
{
    CLIB_MARCH_FN_SELECT(svm_fifo_copy_from_chunk)(f, c, head_idx, dst, len, last);
}

static inline u32 ooo_segment_end_pos(ooo_segment_t *s)
{
...
static inline ooo_segment_t *ooo_segment_prev(svm_fifo_t *f, ooo_segment_t *s)
{
    if (s->prev == OOO_SEGMENT_INVALID_INDEX) return 0;
    return pool_elt_at_index(f->ooo_segments, s->prev);
}

static inline ooo_segment_t *ooo_segment_next(svm_fifo_t *f, ooo_segment_t *s)
{
    if (s->next == OOO_SEGMENT_INVALID_INDEX) return 0;
    return pool_elt_at_index(f->ooo_segments, s->next);
}

static inline ooo_segment_t *ooo_segment_alloc(svm_fifo_t *f, u32 start, u32 length)
{
    ooo_segment_t *s;
...
static inline void ooo_segment_free(svm_fifo_t *f, u32 index)
{
...
static void ooo_segment_add(svm_fifo_t *f, u32 offset, u32 head, u32 tail, u32 length)
{
    ...
    while (s->next != OOO_SEGMENT_INVALID_INDEX && f_pos_lt(s->start, offset_pos))
...
__clib_unused static ooo_segment_t *ooo_segment_last(svm_fifo_t *f)
{
    ooo_segment_t *s;

    if (f->ooos_list_head == OOO_SEGMENT_INVALID_INDEX) return 0;
...
svm_fifo_t *svm_fifo_alloc(u32 data_size_in_bytes)
{
    ...
    c                 = clib_mem_alloc_aligned_or_null(sizeof(*c) + rounded_data_size, CLIB_CACHE_LINE_BYTES);
    if (!c) {
        clib_mem_free(f);
        return 0;
    }
...
svm_fifo_chunk_t *svm_fifo_chunk_alloc(u32 size)
{
    ...
    c            = clib_mem_alloc_aligned_or_null(sizeof(*c) + rounded_size, CLIB_CACHE_LINE_BYTES);
    if (c == 0) return 0;
...
u32 svm_fifo_max_read_chunk(svm_fifo_t *f)
{
    ...
    if (!f->shr->head_chunk) {
        ...
        if (PREDICT_FALSE(!f->shr->head_chunk)) return 0;
    }
...
static rb_node_t *f_find_node_rbtree(rb_tree_t *rt, u32 pos)
{
    ...
    if (PREDICT_FALSE(rb_node_is_tnil(rt, cur))) return 0;
    ...
    if (rb_node_is_tnil(rt, cur)) return 0;
...
static svm_fifo_chunk_t *f_find_chunk_rbtree(rb_tree_t *rt, u32 pos)
{
    ...
    if (f_chunk_includes_pos(c, pos)) return c;
...
static void f_update_ooo_enq(svm_fifo_t *f, u32 start_pos, u32 end_pos)
{
    ...
    if (rt->root == RBTREE_TNIL_INDEX) {
        ...
        c->enq_rb_index = rb_tree_add_custom(rt, c->start_byte, pointer_to_uword(c), f_pos_lt);
    }
    else {
    ...
    if (f_chunk_includes_pos(c, start_pos)) f->ooo_enq = c;

    if (f_chunk_includes_pos(c, end_pos)) return;

    do {
        c = f_cptr(f, c->next);
        if (!c || c->enq_rb_index != RBTREE_TNIL_INDEX) break;
...
static void f_update_ooo_deq(svm_fifo_t *f, u32 start_pos, u32 end_pos)
{
    ...
    if (rt->root == RBTREE_TNIL_INDEX) {
        ...
        c->deq_rb_index = rb_tree_add_custom(rt, c->start_byte, pointer_to_uword(c), f_pos_lt);
    }
    else {
        cur = f_find_node_rbtree(rt, start_pos);
        c   = uword_to_pointer(cur->opaque, svm_fifo_chunk_t *);
    ...
    if (f_chunk_includes_pos(c, start_pos)) f->ooo_deq = c;

    if (f_chunk_includes_pos(c, end_pos)) return;

    do {
        c = f_cptr(f, c->next);
        if (!c || c->deq_rb_index != RBTREE_TNIL_INDEX) break;
...
static svm_fifo_chunk_t *f_lookup_clear_enq_chunks(svm_fifo_t *f, svm_fifo_chunk_t *start, u32 end_pos)
{
    ...
    if (f->ooos_list_head == OOO_SEGMENT_INVALID_INDEX && c && c->enq_rb_index != RBTREE_TNIL_INDEX) {
        n = rb_node(rt, c->enq_rb_index);
        rb_tree_del_node(rt, n);
        c->enq_rb_index = RBTREE_TNIL_INDEX;
    }
...
void svm_fifo_overwrite_head(svm_fifo_t *f, u8 *src, u32 len)
{
    ...
    if (!f->shr->head_chunk) f->shr->head_chunk = f_csptr(f, svm_fifo_find_chunk(f, head));
    ...
    if (len <= n_chunk)
        ...
        clib_memcpy_fast(&f_cptr(f, c->next)->data[0], src + n_chunk, len - n_chunk);
    }
}
...
static int f_try_chunk_alloc(svm_fifo_t *f, u32 head, u32 tail, u32 len)
{
    ...
    if (!f->shr->tail_chunk) f->shr->tail_chunk = f_csptr(f, c);
...
int svm_fifo_enqueue(svm_fifo_t *f, u32 len, const u8 *src)
{
    ...
    if (PREDICT_FALSE(free_count == 0)) return SVM_FIFO_EFULL;
...
int svm_fifo_enqueue_with_offset(svm_fifo_t *f, u32 offset, u32 len, u8 *src)
{
    ...
    if ((len + offset) > free_count) return SVM_FIFO_EFULL;

    enq_pos = tail + offset;

    if (f_pos_gt(enq_pos + len, f_chunk_end(f_end_cptr(f)))) {
        if (PREDICT_FALSE(f_try_chunk_alloc(f, head, tail, offset + len))) return SVM_FIFO_EGROW;
    }

    svm_fifo_trace_add(f, offset, len, 1);
    ooo_segment_add(f, offset, head, tail, len);

    if (!f->ooo_enq || !f_chunk_includes_pos(f->ooo_enq, enq_pos)) f_update_ooo_enq(f, enq_pos, enq_pos + len);

    svm_fifo_copy_to_chunk(f, f->ooo_enq, enq_pos, src, len, &last);
    if (last != F_INVALID_CPTR) f->ooo_enq = f_cptr(f, last);
...
void svm_fifo_enqueue_nocopy(svm_fifo_t *f, u32 len)
{
    ...
    if (rb_tree_is_init(&f->ooo_enq_lookup)) {
        f->shr->tail_chunk = f_csptr(f, f_lookup_clear_enq_chunks(f, f_tail_cptr(f), tail));
        f->ooo_enq         = 0;
    }
    else {
        f->shr->tail_chunk = f_csptr(f, svm_fifo_find_next_chunk(f, f_tail_cptr(f), tail));
    }
...
int svm_fifo_enqueue_segments(svm_fifo_t *f, const svm_fifo_seg_t segs[], u32 n_segs, u8 allow_partial)
{
    ...
    if (!allow_partial) {
        if (PREDICT_FALSE(free_count < len)) return SVM_FIFO_EFULL;

        if (f_pos_gt(tail + len, f_chunk_end(f_end_cptr(f)))) {
            if (PREDICT_FALSE(f_try_chunk_alloc(f, head, tail, len))) return SVM_FIFO_EGROW;
        }

        for (i = 0; i < n_segs; i++) {
            svm_fifo_copy_to_chunk(f, f_tail_cptr(f), tail, segs[i].data, segs[i].len, &f->shr->tail_chunk);
            tail += segs[i].len;
        }
    }
    else {
        ...
        if (f_pos_gt(tail + n_left, f_chunk_end(f_end_cptr(f)))) {
            if (PREDICT_FALSE(f_try_chunk_alloc(f, head, tail, n_left))) {
                ...
                if (!n_left) return SVM_FIFO_EGROW;
            }
        }
        ...
        while (n_left) {
            ...
            svm_fifo_copy_to_chunk(f, f_tail_cptr(f), tail, segs[i].data, to_copy, &f->shr->tail_chunk);
            n_left -= to_copy;
            tail += to_copy;
            i++;
        }
...
always_inline svm_fifo_chunk_t *f_unlink_chunks(svm_fifo_t *f, u32 end_pos, u8 maybe_ooo)
{
    ...
    if (maybe_ooo) {
        if (f->ooo_deq && f_pos_lt(f->ooo_deq->start_byte, f_chunk_end(c))) f->ooo_deq = 0;
    }
    else {
        if (PREDICT_FALSE(f->ooo_deq != 0)) f->ooo_deq = 0;
    }
...
int svm_fifo_dequeue(svm_fifo_t *f, u32 len, u8 *dst)
{
    ...
    if (PREDICT_FALSE(cursize == 0)) return SVM_FIFO_EEMPTY;

    len = clib_min(cursize, len);

    if (!f->shr->head_chunk) f->shr->head_chunk = f_csptr(f, svm_fifo_find_chunk(f, head));

    svm_fifo_copy_from_chunk(f, f_head_cptr(f), head, dst, len, &f->shr->head_chunk);
    head = head + len;
    ...
    if (f_pos_geq(head, f_chunk_end(f_start_cptr(f)))) fsh_collect_chunks(f->fs_hdr, f->shr->slice_index, f_unlink_chunks(f, head, 0));
...
int svm_fifo_peek(svm_fifo_t *f, u32 offset, u32 len, u8 *dst)
{
    ...
    if (PREDICT_FALSE(cursize < offset)) return SVM_FIFO_EEMPTY;

    len      = clib_min(cursize - offset, len);
    head_idx = head + offset;

    clib_mem_unpoison(f->ooo_deq, sizeof(*f->ooo_deq));
    if (!f->ooo_deq || !f_chunk_includes_pos(f->ooo_deq, head_idx)) f_update_ooo_deq(f, head_idx, head_idx + len);

    svm_fifo_copy_from_chunk(f, f->ooo_deq, head_idx, dst, len, &last);
    if (last != F_INVALID_CPTR) f->ooo_deq = f_cptr(f, last);
    return len;
}
...
int svm_fifo_dequeue_drop(svm_fifo_t *f, u32 len)
{
    ...
    if (f_pos_geq(head, f_chunk_end(f_start_cptr(f)))) {
        fsh_collect_chunks(f->fs_hdr, f->shr->slice_index, f_unlink_chunks(f, head, 1));
        f->shr->head_chunk = f_chunk_includes_pos(f_start_cptr(f), head) ? f->shr->start_chunk : 0;
    }
...
void svm_fifo_dequeue_drop_all(svm_fifo_t *f)
{
    ...
    if (!f->shr->head_chunk || !f_chunk_includes_pos(f_head_cptr(f), head)) f->shr->head_chunk = f_csptr(f, svm_fifo_find_chunk(f, head));

    f->shr->head_chunk = f_csptr(f, f_lookup_clear_deq_chunks(f, f_head_cptr(f), tail));

    if (f_pos_geq(tail, f_chunk_end(f_start_cptr(f)))) fsh_collect_chunks(f->fs_hdr, f->shr->slice_index, f_unlink_chunks(f, tail, 0));
...
int svm_fifo_fill_chunk_list(svm_fifo_t *f)
{
    ...
    if (f_chunk_end(f_end_cptr(f)) - head >= f->shr->size) return 0;

    if (f_try_chunk_alloc(f, head, tail, f->shr->size - (tail - head))) return SVM_FIFO_EGROW;
...
int svm_fifo_provision_chunks(svm_fifo_t *f, svm_fifo_seg_t *fs, u32 n_segs, u32 len)
{
    ...
    if (f_free_count(f, head, tail) < len) return SVM_FIFO_EFULL;

    n_avail = f_chunk_end(f_end_cptr(f)) - tail;

    if (n_avail < len && f_try_chunk_alloc(f, head, tail, len)) return SVM_FIFO_EGROW;

    if (!fs || !n_segs) return 0;
...
int svm_fifo_segments(svm_fifo_t *f, u32 offset, svm_fifo_seg_t *fs, u32 *n_segs, u32 max_bytes)
{
    ...
    if (offset >= cursize) return SVM_FIFO_EEMPTY;
...
void svm_fifo_clone(svm_fifo_t *df, svm_fifo_t *sf)
{
    ...
    clib_memcpy_fast(f_head_cptr(df)->data, f_head_cptr(sf)->data, f_head_cptr(sf)->length);
...
{
    if (f->shr->n_subscribers >= SVM_FIFO_MAX_EVT_SUBSCRIBERS) return;
...
void svm_fifo_del_subscriber(svm_fifo_t *f, u8 subscriber)
{
    ...
    for (i = 0; i < f->shr->n_subscribers; i++) {
        if (f->shr->subscribers[i] != subscriber) continue;
...
u8 svm_fifo_is_sane(svm_fifo_t *f)
{
    ...
    if (f->shr->tail_chunk && !f_chunk_includes_pos(f_tail_cptr(f), f->shr->tail)) return 0;
    if (f->ooo_deq) {
        if (rb_tree_is_init(&f->ooo_deq_lookup)) {
            if (f_pos_lt(f->ooo_deq->start_byte, f_start_cptr(f)->start_byte) || f_pos_gt(f->ooo_deq->start_byte, f_chunk_end(f_end_cptr(f))))
                return 0;

            tmp = f_find_chunk_rbtree(&f->ooo_deq_lookup, f->ooo_deq->start_byte);
        }
        else
    ...
    if (f->ooo_enq) {
        if (rb_tree_is_init(&f->ooo_enq_lookup)) {
            if (f_pos_lt(f->ooo_enq->start_byte, f_start_cptr(f)->start_byte) || f_pos_gt(f->ooo_enq->start_byte, f_chunk_end(f_end_cptr(f))))
                return 0;

            tmp = f_find_chunk_rbtree(&f->ooo_enq_lookup, f->ooo_enq->start_byte);
        }
        else {
            tmp = svm_fifo_find_next_chunk(f, f_tail_cptr(f), f->ooo_enq->start_byte);
        }
        if (tmp != f->ooo_enq) return 0;
    }
    ...
    if (f_start_cptr(f)->next) {
        ...
        if (chunks_bytes < f->shr->tail - f->shr->head) return 0;
    }
...
u8 *format_ooo_segment(u8 *s, va_list *args)
{
    ...
    s = format(s, "[%u, %u], len %u, next %d, prev %d", seg->start, seg->start + seg->length, seg->length, seg->next, seg->prev);
    return s;
}
...
u8 *svm_fifo_replay(u8 *s, svm_fifo_t *f, u8 no_read, u8 verbose)
{
    ...
    for (i = 0; i < trace_len; i++) {
        ...
        if (trace[i].action == 1) {
            if (verbose) s = format(s, "adding [%u, %u]:", trace[i].offset, (trace[i].offset + trace[i].len));
            svm_fifo_enqueue_with_offset(placeholder_fifo, trace[i].offset, trace[i].len, &data[offset]);
        }
        else if (trace[i].action == 2) {
            if (verbose) s = format(s, "adding [%u, %u]:", 0, trace[i].len);
            svm_fifo_enqueue(placeholder_fifo, trace[i].len, &data[offset]);
        }
        else if (!no_read) {
            if (verbose) s = format(s, "read: %u", trace[i].len);
            svm_fifo_dequeue_drop(placeholder_fifo, trace[i].len);
        }
        if (verbose) s = format(s, "%U", format_svm_fifo, placeholder_fifo, 1);
    }
...
u8 *format_ooo_list(u8 *s, va_list *args)
{
    ...
    while (ooo_segment_index != OOO_SEGMENT_INVALID_INDEX) {
        ...
        s                 = format(s, "%U%U\n", format_white_space, indent, format_ooo_segment, f, seg);
...
u8 *format_svm_fifo(u8 *s, va_list *args)
{
    ...
    s = format(s, "cursize %u nitems %u has_event %d min_alloc %u\n", svm_fifo_max_dequeue(f), f->shr->size, f->shr->has_event, f->shr->min_alloc);
    s = format(s, "%Uhead %u tail %u segment manager %u\n", format_white_space, indent, f->shr->head, f->shr->tail, f->segment_manager);
    ...
    if (verbose) {
        s = format(s, "%Uooo pool %d active elts newest %u\n", format_white_space, indent, pool_elts(f->ooo_segments), f->ooos_newest);
        if (svm_fifo_has_ooo_data(f)) s = format(s, " %U", format_ooo_list, f, indent, verbose);
    }
    return s;
}
