...
u64 svm_get_global_region_base_va()
{
    ...
    if ((fd = open("/proc/self/maps", 0)) < 0) clib_unix_error("open '/proc/self/maps'");

    unformat_init_clib_file(&input, fd);
    while (unformat_check_input(&input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(&input, "%llx-%llx", &start, &end)) end--;
...
static u8 *format_svm_flags(u8 *s, va_list *args)
{
    ...
    if (f & SVM_FLAGS_MHEAP) s = format(s, "MHEAP ");
    if (f & SVM_FLAGS_FILE) s = format(s, "FILE ");
    if (f & SVM_FLAGS_NODATA) s = format(s, "NODATA ");
    if (f & SVM_FLAGS_NEED_DATA_INIT) s = format(s, "INIT ");
...
static int svm_data_region_create(svm_map_region_args_t *a, svm_region_t *rp)
{
    ...
    map_size = rp->virtual_size - (MMAP_PAGESIZE + (a->pvt_heap_size ? a->pvt_heap_size : SVM_PVT_MHEAP_SIZE));
    ...
    if (a->flags & SVM_FLAGS_FILE) {
        ...
        ASSERT(map_size <= rp->virtual_size - (MMAP_PAGESIZE + SVM_PVT_MHEAP_SIZE));

        if (mmap(rp->data_base, map_size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED, fd, 0) == MAP_FAILED) {
    ...
    if (a->flags & SVM_FLAGS_MHEAP) {
        rp->data_heap = clib_mem_create_heap(rp->data_base, map_size, 1 /* locked */, "svm data");
...
static int svm_data_region_map(svm_map_region_args_t *a, svm_region_t *rp)
{
    ...
    if (a->flags & SVM_FLAGS_FILE) {
        ...
        ASSERT(map_size <= rp->virtual_size - (MMAP_PAGESIZE + (a->pvt_heap_size ? a->pvt_heap_size : SVM_PVT_MHEAP_SIZE)));
...
u8 *shm_name_from_svm_map_region_args(svm_map_region_args_t *a)
{
    ...
    if (a->root_path) {
        ...
        if (a->root_path[0] == '/') root_path_offset++;

        if (a->name[0] == '/') name_offset = 1;

        shm_name = format(0, "/%s-%s%c", &a->root_path[root_path_offset], &a->name[name_offset], 0);
    }
    else
...
void svm_region_init_mapped_region(svm_map_region_args_t *a, svm_region_t *rp)
{
    ...
    if (pthread_mutexattr_init(&attr)) clib_unix_warning("mutexattr_init");

    if (pthread_mutexattr_setpshared(&attr, PTHREAD_PROCESS_SHARED)) clib_unix_warning("mutexattr_setpshared");

    if (pthread_mutex_init(&rp->mutex, &attr)) clib_unix_warning("mutex_init");

    if (pthread_mutexattr_destroy(&attr)) clib_unix_warning("mutexattr_destroy");

    if (pthread_condattr_init(&cattr)) clib_unix_warning("condattr_init");

    if (pthread_condattr_setpshared(&cattr, PTHREAD_PROCESS_SHARED)) clib_unix_warning("condattr_setpshared");

    if (pthread_cond_init(&rp->condvar, &cattr)) clib_unix_warning("cond_init");

    if (pthread_condattr_destroy(&cattr)) clib_unix_warning("condattr_destroy");
    ...
    rp->region_heap = clib_mem_create_heap(uword_to_pointer(a->baseva + MMAP_PAGESIZE, void *),
    ...
    overhead_space = MMAP_PAGESIZE /* header */ + ((a->pvt_heap_size != 0) ? a->pvt_heap_size : SVM_PVT_MHEAP_SIZE);

    bit       = 0;
    data_base = (uword) rp->virtual_base;

    if (a->flags & SVM_FLAGS_NODATA) rp->flags |= SVM_FLAGS_NEED_DATA_INIT;
...
void *svm_map_region(svm_map_region_args_t *a)
{
    ...
    if (CLIB_DEBUG > 1) clib_warning("[%d] map region %s: shm_open (%s)", getpid(), a->name, shm_name);

    svm_fd = shm_open((char *) shm_name, O_RDWR | O_CREAT | O_EXCL, 0777);

    if (svm_fd >= 0) {
        if (fchmod(svm_fd, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP) < 0) clib_unix_warning("segment chmod");
        /* This turns out to fail harmlessly if the client starts first */
        if (fchown(svm_fd, a->uid, a->gid) < 0) clib_unix_warning("segment chown [ok if client starts first]");
        ...
        rp = mmap(uword_to_pointer(a->baseva, void *), a->size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED, svm_fd, 0);
        ...
        rp = (void *) mmap(uword_to_pointer(a->baseva, void *), a->size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED, svm_fd, 0);
        ...
        if (pid_holding_region_lock && kill(pid_holding_region_lock, 0) < 0) {
            pthread_mutexattr_t attr;
            clib_warning("region %s mutex held by dead pid %d, tag %d, force unlock", rp->region_name, pid_holding_region_lock, rp->mutex_owner_tag);
        ...
        if (dead_region_recovery) clib_warning("recovery: attempt to re-lock region");

        region_lock(rp, 2);
        oldheap = svm_push_pvt_heap(rp);
        vec_add1(rp->client_pids, getpid());

        if (dead_region_recovery) clib_warning("recovery: attempt svm_data_region_map");
        ...
        if (dead_region_recovery) clib_warning("unlock and continue");
...
void *svm_region_find_or_create(svm_map_region_args_t *a)
{
    ...
    a->size += MMAP_PAGESIZE + ((a->pvt_heap_size != 0) ? a->pvt_heap_size : SVM_PVT_MHEAP_SIZE);
    ...
    if (a->root_path == NULL) a->root_path = (char *) mp->root_path;
    ...
    if (index >= root_rp->bitmap_size) {
        clib_warning("region %s: not enough VM to allocate 0x%llx (%lld)", root_rp->region_name, a->size, a->size);
        svm_pop_heap(oldheap);
        region_unlock(root_rp);
        return 0;
    }
...
void svm_region_unlink(svm_region_t *rp)
{
    ...
    shm_name     = shm_name_from_svm_map_region_args(a);
    if (CLIB_DEBUG > 1) clib_warning("[%d] shm_unlink (%s)", getpid(), shm_name);
...
void svm_region_unmap_internal(void *rp_arg, u8 is_client)
{
    ...
    if (CLIB_DEBUG > 1) clib_warning("[%d] unmap region %s", getpid(), rp->region_name);
...
found:
    ...
    if (nclients_left == 0) {
        ...
        if (!is_client) svm_region_unlink(rp);
...
found:
    ...
    if (!is_client && vec_len(root_rp->client_pids) == 0) svm_region_unlink(root_rp);
...
void svm_client_scan_this_region_nolock(svm_region_t *rp)
{
    ...
    for (j = 0; j < vec_len(rp->client_pids); j++) {
        if (mypid == rp->client_pids[j]) continue;
        if (rp->client_pids[j] && (kill(rp->client_pids[j], 0) < 0)) {
            clib_warning("%s: cleanup ghost pid %d", rp->region_name, rp->client_pids[j]);
...
void svm_client_scan(const char *root_path)
{
    ...
    for (j = 0; j < vec_len(root_rp->client_pids); j++) {
        if (mypid == root_rp->client_pids[j]) continue;
        if (root_rp->client_pids[j] && (kill(root_rp->client_pids[j], 0) < 0)) {
            clib_warning("%s: cleanup ghost pid %d", root_rp->region_name, root_rp->client_pids[j]);
