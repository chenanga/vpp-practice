...
static init_fn   server_init_fns[SSVM_N_SEGMENT_TYPES] = {ssvm_server_init_shm, ssvm_server_init_memfd, ssvm_server_init_private};
static init_fn   client_init_fns[SSVM_N_SEGMENT_TYPES] = {ssvm_client_init_shm, ssvm_client_init_memfd, ssvm_client_init_private};
static delete_fn delete_fns[SSVM_N_SEGMENT_TYPES]      = {ssvm_delete_shm, ssvm_delete_memfd, ssvm_delete_private};
...
int ssvm_server_init_shm(ssvm_private_t *ssvm)
{
    ...
    if (ssvm->ssvm_size == 0) return SSVM_API_ERROR_NO_SIZE;

    if (CLIB_DEBUG > 1) clib_warning("[%d] creating segment '%s'", getpid(), ssvm->name);
    ...
    if (fchmod(ssvm_fd, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP) < 0) clib_unix_warning("ssvm segment chmod");
    if (svm_get_root_rp()) {
        /* TODO: is this really needed? */
        svm_main_region_t *smr = svm_get_root_rp()->data_base;
        if (fchown(ssvm_fd, smr->uid, smr->gid) < 0) clib_unix_warning("ssvm segment chown");
    }
    ...
    sh = clib_mem_vm_map_shared(uword_to_pointer(requested_va, void *), ssvm->ssvm_size, ssvm_fd, 0, (char *) ssvm->name);
    ...
    sh->heap       = clib_mem_create_heap(((u8 *) sh) + page_size, ssvm->ssvm_size - page_size, 1 /* locked */, "ssvm server shm");
...
int ssvm_client_init_shm(ssvm_private_t *ssvm)
{
    ...
    while (ssvm->attach_timeout-- > 0) {
        if (ssvm_fd < 0) ssvm_fd = shm_open((char *) ssvm->name, O_RDWR, 0777);
        if (ssvm_fd < 0) {
            sleep(1);
            continue;
        }
        ...
        if (stat.st_size > 0) goto map_it;
    }
    clib_warning("client timeout");
    return SSVM_API_ERROR_CLIENT_TIMEOUT;

map_it:
    sh = (void *) mmap(0, MMAP_PAGESIZE, PROT_READ | PROT_WRITE, MAP_SHARED, ssvm_fd, 0);
    if (sh == MAP_FAILED) {
...
re_map_it:
    ...
    sh = ssvm->sh = (void *) mmap((void *) ssvm->requested_va, ssvm->ssvm_size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED, ssvm_fd, 0);
...
void ssvm_delete_shm(ssvm_private_t *ssvm)
{
    ...
    if (CLIB_DEBUG > 1) clib_warning("[%d] unlinking ssvm (%s) backing file '%s'", getpid(), ssvm->name, fn);

    /* Throw away the backing file */
    if (unlink((char *) fn) < 0) clib_unix_warning("unlink segment '%s'", ssvm->name);
...
int ssvm_server_init_memfd(ssvm_private_t *memfd)
{
    ...
    if (memfd->ssvm_size == 0) return SSVM_API_ERROR_NO_SIZE;

    ASSERT(vec_c_string_is_terminated(memfd->name));

    if (memfd->huge_page)
        memfd->fd = clib_mem_vm_create_fd(CLIB_MEM_PAGE_SZ_DEFAULT_HUGE, (char *) memfd->name);
    else
        memfd->fd = clib_mem_vm_create_fd(CLIB_MEM_PAGE_SZ_DEFAULT, (char *) memfd->name);
    ...
    sh = clib_mem_vm_map_shared(uword_to_pointer(memfd->requested_va, void *), memfd->ssvm_size, memfd->fd, 0, (char *) memfd->name);
    if (sh == CLIB_MEM_VM_MAP_FAILED) {
    ...
    sh->heap  = clib_mem_create_heap(((u8 *) sh) + page_size, memfd->ssvm_size - page_size, 1 /* locked */, "ssvm server memfd");
    oldheap   = ssvm_push_heap(sh);
...
int ssvm_client_init_memfd(ssvm_private_t *memfd)
{
     ...
     */
    sh = (void *) mmap(0, page_size, PROT_READ | PROT_WRITE, MAP_SHARED, memfd->fd, 0);
    ...
    if (memfd->requested_va) mmap_flags |= MAP_FIXED;

    /*
     * Remap the segment at the 'right' address
     */
    sh = (void *) mmap(uword_to_pointer(memfd->requested_va, void *), memfd->ssvm_size, PROT_READ | PROT_WRITE, mmap_flags, memfd->fd, 0);
...
int ssvm_server_init_private(ssvm_private_t *ssvm)
{
    ...
    sh = clib_mem_vm_map(0, rnd_size + page_size, log2_page_size, (char *) ssvm->name);
    if (sh == CLIB_MEM_VM_MAP_FAILED) {
        clib_unix_warning("private map failed");
        return SSVM_API_ERROR_CREATE_FAILURE;
    }

    heap = clib_mem_create_heap((u8 *) sh + page_size, rnd_size, 1 /* locked */, "ssvm server private");
    if (heap == 0) {
