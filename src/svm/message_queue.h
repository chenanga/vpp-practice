...
#define SVM_MQ_INVALID_MSG \
    {                      \
        .as_u64 = ~0       \
    }

typedef enum svm_msg_q_wait_type_ { SVM_MQ_WAIT_EMPTY, SVM_MQ_WAIT_FULL } svm_msg_q_wait_type_t;
 ...
 */
static inline u32 svm_msg_q_size(svm_msg_q_t *mq)
{
 ...
 */
static inline u8 svm_msg_q_is_full(svm_msg_q_t *mq)
{
    return (svm_msg_q_size(mq) == mq->q.shr->maxsize);
}

static inline u8 svm_msg_q_ring_is_full(svm_msg_q_t *mq, u32 ring_index)
{
    svm_msg_q_ring_t *ring = vec_elt_at_index(mq->rings, ring_index);
    return (clib_atomic_load_relax_n(&ring->shr->cursize) >= ring->nitems);
}

static inline u8 svm_msg_q_or_ring_is_full(svm_msg_q_t *mq, u32 ring_index)
{
 ...
 */
static inline u8 svm_msg_q_is_empty(svm_msg_q_t *mq)
{
 ...
 */
static inline u8 svm_msg_q_msg_is_invalid(svm_msg_q_msg_t *msg)
{
 ...
 */
static inline int svm_msg_q_try_lock(svm_msg_q_t *mq)
{
    if (mq->q.evtfd == -1) {
        int rv = pthread_mutex_trylock(&mq->q.shr->mutex);
        if (PREDICT_FALSE(rv == EOWNERDEAD)) rv = pthread_mutex_consistent(&mq->q.shr->mutex);
        return rv;
    }
    else {
 ...
 */
static inline int svm_msg_q_lock(svm_msg_q_t *mq)
{
    if (mq->q.evtfd == -1) {
 ...
 */
static inline void svm_msg_q_unlock(svm_msg_q_t *mq)
{
    if (mq->q.evtfd == -1) {
...
static inline int svm_msg_q_get_eventfd(svm_msg_q_t *mq)
{
    return mq->q.evtfd;
}
