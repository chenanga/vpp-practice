...
static inline svm_msg_q_ring_t *svm_msg_q_ring_inline(svm_msg_q_t *mq, u32 ring_index)
{
...
static inline void *svm_msg_q_ring_data(svm_msg_q_ring_t *ring, u32 elt_index)
{
...
static void svm_msg_q_init_mutex(svm_msg_q_shared_queue_t *sq)
{
    ...
    if (pthread_mutexattr_setpshared(&attr, PTHREAD_PROCESS_SHARED)) clib_unix_warning("pthread_mutexattr_setpshared");
    if (pthread_mutexattr_setrobust(&attr, PTHREAD_MUTEX_ROBUST)) clib_unix_warning("setrobust");
    if (pthread_mutex_init(&sq->mutex, &attr)) clib_unix_warning("mutex_init");
    ...
    if (pthread_cond_init(&sq->condvar, &cattr)) clib_unix_warning("cond_init1");
    if (pthread_condattr_destroy(&cattr)) clib_unix_warning("cond_init2");
}
...
uword svm_msg_q_size_to_alloc(svm_msg_q_cfg_t *cfg)
{
    ...
    for (i = 0; i < cfg->n_rings; i++) {
        if (cfg->ring_cfgs[i].data) continue;
...
void svm_msg_q_cleanup(svm_msg_q_t *mq)
{
    ...
    if (mq->q.evtfd != -1) close(mq->q.evtfd);
}
...
static void svm_msg_q_send_signal(svm_msg_q_t *mq, u8 is_consumer)
{
    if (mq->q.evtfd == -1) {
        ...
        if (is_consumer) pthread_mutex_unlock(&mq->q.shr->mutex);
    }
    else {
        int __clib_unused rv;
        u64               data = 1;

        if (mq->q.evtfd < 0) return;

        rv = write(mq->q.evtfd, &data, sizeof(data));
        if (PREDICT_FALSE(rv < 0)) clib_unix_warning("signal write on %d returned %d", mq->q.evtfd, rv);
    }
}
...
svm_msg_q_msg_t svm_msg_q_alloc_msg(svm_msg_q_t *mq, u32 nbytes)
{
    ...
    vec_foreach (ring, mq->rings) {
        sr = ring->shr;
        if (ring->elsize < nbytes || sr->cursize == ring->nitems) continue;
        msg.ring_index = ring - mq->rings;
        msg.elt_index  = sr->tail;
        sr->tail       = (sr->tail + 1) % ring->nitems;
...
void svm_msg_q_free_msg(svm_msg_q_t *mq, svm_msg_q_msg_t *msg)
{
    ...
    if (msg->elt_index == sr->head) {
        ...
        clib_warning("message out of order: elt %u head %u ring %u", msg->elt_index, sr->head, msg->ring_index);
    ...
    if (PREDICT_FALSE(need_signal)) svm_msg_q_send_signal(mq, 1 /* is consumer */);
}
...
static int svm_msq_q_msg_is_valid(svm_msg_q_t *mq, svm_msg_q_msg_t *msg)
{
    ...
    if (vec_len(mq->rings) <= msg->ring_index) return 0;
...
void svm_msg_q_add_raw(svm_msg_q_t *mq, svm_msg_q_msg_t *msg)
{
    ...
    if (!sz) svm_msg_q_send_signal(mq, 0 /* is consumer */);
}
...
int svm_msg_q_sub_raw(svm_msg_q_t *mq, svm_msg_q_msg_t *elem)
{
    ...
    if (PREDICT_FALSE(sz == sq->maxsize)) svm_msg_q_send_signal(mq, 1 /* is consumer */);
...
int svm_msg_q_sub(svm_msg_q_t *mq, svm_msg_q_msg_t *msg, svm_q_conditional_wait_t cond, u32 time)
{
    ...
    if (svm_msg_q_is_empty(mq)) {
        if (cond == SVM_Q_NOWAIT) {
            ...
            if ((rc = svm_msg_q_timedwait(mq, time))) return rc;
        }
        else {
...
int svm_msg_q_timedwait(svm_msg_q_t *mq, double timeout)
{
    if (mq->q.evtfd == -1) {
        ...
        rv         = setsockopt(mq->q.evtfd, SOL_SOCKET, SO_RCVTIMEO, (const char *) &tv, sizeof tv);
        if (rv < 0) {
        ...
        if (rv < 0) clib_warning("read %u", errno);
