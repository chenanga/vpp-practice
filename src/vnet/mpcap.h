 ...
 *
 */
static inline void *mpcap_add_packet(mpcap_main_t *pm, f64 time_now, u32 n_bytes_in_trace, u32 n_bytes_in_packet)
{
    mpcap_packet_header_t *h;
    u8                    *d;

    /* File already closed? */
    if (PREDICT_FALSE(pm->flags & MPCAP_FLAG_INIT_DONE) == 0) return 0;

    d = pm->current_va;
    pm->current_va += sizeof(h[0]) + n_bytes_in_trace;

    /* Out of space? */
    if (PREDICT_FALSE(pm->current_va >= pm->file_baseva + pm->max_file_size)) return 0;
    h                                = (void *) (d);
 ...
 *
 */
static inline void mpcap_add_buffer(mpcap_main_t *pm, vlib_main_t *vm, f64 time_now, u32 buffer_index, u32 n_bytes_in_trace)
{
    ...
    if (pm->n_packets_captured >= pm->n_packets_to_capture) mpcap_close(pm);
