...
ipfix_exporter_t *vnet_ipfix_exporter_lookup(const ip_address_t *ipfix_collector)
{
    flow_report_main_t *frm = &flow_report_main;
    ipfix_exporter_t   *exp;

    pool_foreach (exp, frm->exporters) {
        if (ip_address_cmp(&exp->ipfix_collector, ipfix_collector) == 0) return exp;
    }
...
static int vl_api_set_ipfix_exporter_t_internal(u32               client_index,
                                                vl_api_address_t *mp_collector_address,
                                                u16               mp_collector_port,
                                                vl_api_address_t *mp_src_address,
                                                u32               mp_vrf_id,
                                                u32               mp_path_mtu,
                                                u32               mp_template_interval,
                                                bool              mp_udp_checksum,
                                                bool              use_index_0,
                                                bool              is_create)
{
    ...
    if (use_index_0) {
        ...
        if (is_create) {
            ...
            if (!exp) {
                ...
                if (pool_elts(frm->exporters) >= IPFIX_EXPORTERS_MAX) return VNET_API_ERROR_INVALID_VALUE;
            ...
            /* Delete the exporter */
            exp = vnet_ipfix_exporter_lookup(&collector);
            if (!exp) return VNET_API_ERROR_NO_SUCH_ENTRY;
    ...
    if (collector_port == (u16) ~0) collector_port = UDP_DST_PORT_ipfix;
    ...
    if (fib_id == ~0) {
        ...
        if (!p) return VNET_API_ERROR_NO_SUCH_FIB;
        fib_index = p[0];
    }

    path_mtu = ntohl(mp_path_mtu);
    if (path_mtu == ~0) path_mtu = 512;  // RFC 7011 section 10.3.3.
    template_interval = ntohl(mp_template_interval);
    if (template_interval == ~0) template_interval = 20;
     ...
     */
    if (!ip_address_is_zero(&collector) && ip_address_is_zero(&src)) return VNET_API_ERROR_INVALID_VALUE;
    if (collector.version != src.version) return VNET_API_ERROR_INVALID_VALUE;

    if (path_mtu > 1450 /* vpp does not support fragmentation */) return VNET_API_ERROR_INVALID_VALUE;

    if (path_mtu < 68) return VNET_API_ERROR_INVALID_VALUE;
...
static void vl_api_set_ipfix_exporter_t_handler(vl_api_set_ipfix_exporter_t *mp)
{
    ...
    flow_report_main_t                *frm = &flow_report_main;
    int                                rv  = vl_api_set_ipfix_exporter_t_internal(mp->client_index,
                                                  &mp->collector_address,
                                                  mp->collector_port,
                                                  &mp->src_address,
                                                  mp->vrf_id,
                                                  mp->path_mtu,
                                                  mp->template_interval,
                                                  mp->udp_checksum,
                                                  USE_INDEX_0,
                                                  0);
...
static void vl_api_ipfix_exporter_create_delete_t_handler(vl_api_ipfix_exporter_create_delete_t *mp)
{
...
static void vl_api_ipfix_exporter_dump_t_handler(vl_api_ipfix_exporter_dump_t *mp)
{
    ...
    rmp->_vl_msg_id = ntohs((REPLY_MSG_ID_BASE) + VL_API_IPFIX_EXPORTER_DETAILS);
...
static void ipfix_all_fill_details(vl_api_ipfix_all_exporter_details_t *rmp, ipfix_exporter_t *exp)
{
    ip4_main_t *im = &ip4_main;
    u32         vrf_id;
...
static void ipfix_all_exporter_details(flow_report_main_t *frm, u32 index, vl_api_registration_t *rp, u32 context)
{
    ipfix_exporter_t *exp = pool_elt_at_index(frm->exporters, index);

    vl_api_ipfix_all_exporter_details_t *rmp;

    REPLY_MACRO_DETAILS4(VL_API_IPFIX_ALL_EXPORTER_DETAILS, rp, context, ({ ipfix_all_fill_details(rmp, exp); }));
}

static void vl_api_ipfix_all_exporter_get_t_handler(vl_api_ipfix_all_exporter_get_t *mp)
{
    flow_report_main_t                    *frm = &flow_report_main;
    vl_api_ipfix_all_exporter_get_reply_t *rmp;
    int                                    rv = 0;

    REPLY_AND_DETAILS_MACRO(VL_API_IPFIX_ALL_EXPORTER_GET_REPLY, frm->exporters, ({ ipfix_all_exporter_details(frm, cursor, rp, mp->context); }));
}
...
static void vl_api_set_ipfix_classify_stream_t_handler(vl_api_set_ipfix_classify_stream_t *mp)
{
    ...
    if (fcm->src_port != 0 && (fcm->domain_id != domain_id || fcm->src_port != (u16) src_port)) {
        int rv = vnet_stream_change(exp, fcm->domain_id, fcm->src_port, domain_id, (u16) src_port);
        ASSERT(rv == 0);
    }
...
static void vl_api_ipfix_classify_table_add_del_t_handler(vl_api_ipfix_classify_table_add_del_t *mp)
{
    ...
    if (is_add == 0 || (rv && is_add)) ipfix_classify_delete_table(table - fcm->tables);
...
static void vl_api_ipfix_classify_table_dump_t_handler(vl_api_ipfix_classify_table_dump_t *mp)
{
    ...
    for (i = 0; i < vec_len(fcm->tables); i++)
        if (ipfix_classify_table_index_valid(i)) send_ipfix_classify_table_details(i, reg, mp->context);
}
...
static void vl_api_ipfix_flush_t_handler(vl_api_ipfix_flush_t *mp)
{
    ...
    vlib_process_signal_event(vm, flow_report_process_node.index, 1 /* type_opaque */, 0 /* data */);
