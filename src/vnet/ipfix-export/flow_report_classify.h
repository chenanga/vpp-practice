...
#define foreach_ipfix_ip6_field                                                                                      \
    _(ip6->src_address.as_u8, ((u32[]){0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF}), sourceIPv6Address, 16)      \
    _(ip6->dst_address.as_u8, ((u32[]){0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF}), destinationIPv6Address, 16) \
...
static_always_inline u8 ipfix_classify_table_index_valid(u32 index)
{
    ...
    return index < vec_len(fcm->tables) && fcm->tables[index].classify_table_index != ~0;
}
...
static_always_inline ipfix_classify_table_t *ipfix_classify_add_table(void)
{
    ...
    for (i = 0; i < vec_len(fcm->tables); i++)
        if (!ipfix_classify_table_index_valid(i)) return &fcm->tables[i];
