...
static_always_inline u8 stream_index_valid(ipfix_exporter_t *exp, u32 index)
{
    return index < vec_len(exp->streams) && exp->streams[index].domain_id != ~0;
}

static_always_inline flow_report_stream_t *add_stream(ipfix_exporter_t *exp)
{
    u32 i;
    for (i = 0; i < vec_len(exp->streams); i++)
        if (!stream_index_valid(exp, i)) return &exp->streams[i];
    u32 index = vec_len(exp->streams);
    vec_validate(exp->streams, index);
    return &exp->streams[index];
}

static_always_inline void delete_stream(ipfix_exporter_t *exp, u32 index)
{
...
static i32 find_stream(ipfix_exporter_t *exp, u32 domain_id, u16 src_port)
{
    ...
    for (i = 0; i < vec_len(exp->streams); i++)
        if (stream_index_valid(exp, i)) {
            ...
            if (domain_id == stream->domain_id) {
                if (src_port != stream->src_port) return -2;
                return i;
            }
...
int send_template_packet(flow_report_main_t *frm, ipfix_exporter_t *exp, flow_report_t *fr, u32 *buffer_indexp)
{
    u32                          bi0;
    vlib_buffer_t               *b0;
    ...
    if (fr->update_rewrite || fr->rewrite == 0) {
        if (ip_address_is_zero(&exp->ipfix_collector) || ip_address_is_zero(&exp->src_address)) {
            vlib_node_set_state(frm->vlib_main, flow_report_process_node.index, VLIB_NODE_STATE_DISABLED);
            return -1;
        }
        vec_free(fr->rewrite);
        fr->update_rewrite = 1;
    ...
    if (fr->update_rewrite) {
        fr->rewrite        = fr->rewrite_callback(exp, fr, exp->collector_port, fr->report_elements, fr->n_report_elements, fr->stream_indexp);
        fr->update_rewrite = 0;
    }

    if (vlib_buffer_alloc(vm, &bi0, 1) != 1) return -1;
    ...
    h->export_time = (u32) (((f64) frm->unix_time_0) + (vlib_time_now(frm->vlib_main) - frm->vlib_time_0));
    ...
    if (exp->udp_checksum || ip_addr_version(&exp->ipfix_collector) == AF_IP6) {
        ...
        if (ip_addr_version(&exp->ipfix_collector) == AF_IP4)
            ...
            int bogus     = 0;
            udp->checksum = ip6_tcp_udp_icmp_compute_checksum(vm, b0, ip, &bogus);
        }
...
u32 always_inline ipfix_write_headers(ipfix_exporter_t *exp, void *data, void **ip, udp_header_t **udp, u32 len)
{
    if (ip_addr_version(&exp->ipfix_collector) == AF_IP4) {
        ip4_ipfix_template_packet_t *tp4;
        ip4_header_t                *ip4;
...
u8 *vnet_flow_rewrite_generic_callback(ipfix_exporter_t       *exp,
                                       flow_report_t          *fr,
                                       u16                     collector_port,
                                       ...
                                       u32                    *stream_indexp)
{
    ...
    *stream_indexp = fr->stream_index;
    ...
    vec_validate_aligned(rewrite, size + n_elts * sizeof(ipfix_field_specifier_t) - 1, CLIB_CACHE_LINE_BYTES);
...
vlib_buffer_t *vnet_ipfix_exp_get_buffer(vlib_main_t *vm, ipfix_exporter_t *exp, flow_report_t *fr, u32 thread_index)
{
    u32            bi0;
    vlib_buffer_t *b0;

    if (fr->per_thread_data[thread_index].buffer) return fr->per_thread_data[thread_index].buffer;

    if (vlib_buffer_alloc(vm, &bi0, 1) != 1) return NULL;
 ...
 */
void vnet_ipfix_exp_send_buffer(vlib_main_t          *vm,
                                ...
                                vlib_buffer_t        *b0)
{
    ...
    /* nothing to send */
    if (fr->per_thread_data[thread_index].next_data_offset <= exp->all_headers_size) return;

    ip_len = ipfix_write_headers(exp, (void *) vlib_buffer_get_current(b0), &ip, &udp, b0->current_length);
     ...
     */
    h->sequence_number = clib_atomic_fetch_add(&stream->sequence_number, fr->per_thread_data[thread_index].n_data_records);
     ...
     * RFC 7011: 3.4.3
     */
    s->set_id_length  = ipfix_set_id_length(fr->template_id, b0->current_length - (ip_len + sizeof(*udp) + sizeof(*h)));
    h->version_length = version_length(b0->current_length - (ip_len + sizeof(*udp)));
    ...
    if (ip_addr_version(&exp->ipfix_collector) == AF_IP4) {
        ...
        ip6 = (ip6_header_t *) ip;
        /* Ipv6 payload length does not include the IPv6 header */
        ip6->payload_length = clib_host_to_net_u16(b0->current_length - sizeof(*ip6));
        udp->length         = clib_host_to_net_u16(b0->current_length - sizeof(*ip6));
    }
...
static void flow_report_process_send(vlib_main_t        *vm,
    ...
    while (1) {
        ...
        pool_foreach (exp, frm->exporters) {
            ...
            vec_foreach (fr, exp->reports) {
                ...
                send_template = now > (fr->last_template_sent + exp->template_interval);
                send_template += fr->last_template_sent == 0;
                template_bi = ~0;
                rv          = 0;

                if (send_template) rv = send_template_packet(frm, exp, fr, &template_bi);
                 ...
                 * time
                 */
                next_template = (fr->last_template_sent + exp->template_interval) - now;
                wait_time     = clib_min(wait_time, next_template);

                if (ip_addr_version(&exp->ipfix_collector) == AF_IP4) {
                    flow_report_process_send(vm, frm, exp, fr, ip4_lookup_node_index, template_bi);
                }
                else {
                    flow_report_process_send(vm, frm, exp, fr, ip6_lookup_node_index, template_bi);
                }
            }
        }
    }
...
int vnet_flow_report_add_del(ipfix_exporter_t *exp, vnet_flow_report_add_del_args_t *a, u16 *template_id)
{
    int                   i;
    int                   found_index = ~0;
    flow_report_t        *fr;
    ...
    if (si == -2) return VNET_API_ERROR_INVALID_VALUE;
    if (si == -1 && a->is_add == 0) return VNET_API_ERROR_NO_SUCH_ENTRY;

    for (i = 0; i < vec_len(exp->reports); i++) {
        fr = vec_elt_at_index(exp->reports, i);
        if (fr->opaque.as_uword == a->opaque.as_uword && fr->rewrite_callback == a->rewrite_callback
            && fr->flow_data_callback == a->flow_data_callback) {
            found_index = i;
            if (template_id) *template_id = fr->template_id;
            break;
        }
    }
    ...
    if (a->is_add == 0) {
        if (found_index != ~0) {
            for (int i = 0; i < vec_len(exp->reports[found_index].per_thread_data); i++) {
                u32 bi;
                if (exp->reports[found_index].per_thread_data[i].buffer) {
                    bi = vlib_get_buffer_index(vm, exp->reports[found_index].per_thread_data[i].buffer);
            ...
            stream->n_reports--;
            if (stream->n_reports == 0) delete_stream(exp, si);
            return 0;
        }
        return VNET_API_ERROR_NO_SUCH_ENTRY;
    }

    if (found_index != ~0) return VNET_API_ERROR_VALUE_EXIST;
    ...
    switch (error) {
        ...
        case VNET_API_ERROR_INVALID_VALUE:
            return clib_error_return(0,
                                     "Expecting either still unused values "
                                     "for both domain_id and src_port "
                                     "or already used values for both fields");
        default:
            return clib_error_return(0, "vnet_flow_report_add_del returned %d", error);
    }
}
...
void vnet_flow_reports_reset(ipfix_exporter_t *exp)
{
    ...
    for (i = 0; i < vec_len(exp->streams); i++)
        if (stream_index_valid(exp, i)) exp->streams[i].sequence_number = 0;
...
int vnet_stream_change(ipfix_exporter_t *exp, u32 old_domain_id, u16 old_src_port, u32 new_domain_id, u16 new_src_port)
{
    ...
    if (old_domain_id != new_domain_id || old_src_port != new_src_port) vnet_stream_reset(exp, stream_index);
    return 0;
}
...
static clib_error_t *set_ipfix_exporter_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "collector %U", unformat_ip4_address, &collector.ip.ip4))
            ;
            ...
            if (!p) return clib_error_return(0, "fib ID %d doesn't exist\n", fib_id);
            fib_index = p[0];
        }
     ...
     */
    if (!ip_address_is_zero(&collector) && ip_address_is_zero(&src)) return clib_error_return(0, "src address required");
    if (collector.version != src.version) return clib_error_return(0, "src address and dest address must use same IP version");

    if (path_mtu > 1450 /* vpp does not support fragmentation */) return clib_error_return(0, "too big path-mtu value, maximum is 1450");

    if (path_mtu < 68) return clib_error_return(0, "too small path-mtu value, minimum is 68");
    ...
    exp->all_headers_size = ip_header_size + sizeof(udp_header_t) + sizeof(ipfix_message_header_t) + sizeof(ipfix_set_header_t);

    /* Reset report streams if we are reconfiguring IP addresses */
    if (ip_address_cmp(&exp->ipfix_collector, &collector) || ip_address_cmp(&exp->src_address, &src) || exp->collector_port != collector_port)
    ...
    if (collector.ip.ip4.as_u32)
        vlib_cli_output(vm,
                        ...
                        "udp checksum %s",
                        format_ip4_address,
                        &exp->ipfix_collector.ip.ip4,
                        format_ip4_address,
                        &exp->src_address.ip.ip4,
                        fib_index,
                        path_mtu,
                        template_interval,
