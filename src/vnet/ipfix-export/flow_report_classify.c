...
u8 *ipfix_classify_template_rewrite(ipfix_exporter_t       *exp,
                                    flow_report_t          *fr,
                                    u16                     collector_port,
        ...
        for (j = 0; j < (1 << b->log2_pages); j++) {
            for (k = 0; k < t->entries_per_page; k++) {
                ...
                if (PREDICT_FALSE(b0 == 0)) {
                    if (vlib_buffer_alloc(vm, &bi0, 1) != 1) goto flush;
                    b0 = vlib_get_buffer(vm, bi0);

                    u32 copy_len = sizeof(ip4_header_t) + sizeof(udp_header_t) + sizeof(ipfix_message_header_t);
...
#undef _
                ...
                /* Add packetTotalCount manually */
                {
                    ...
                    clib_memcpy_fast(b0->data + next_offset, &packets, sizeof(packets));
                ...
                if (next_offset + next_record_size > exp->path_mtu) {
                    s->set_id_length   = ipfix_set_id_length(fr->template_id, next_offset - (sizeof(*ip) + sizeof(*udp) + sizeof(*h)));
                    h->version_length  = version_length(next_offset - (sizeof(*ip) + sizeof(*udp)));
...
static clib_error_t *ipfix_classify_table_add_del_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (fcm->src_port == 0) clib_error_return(0, "call 'set ipfix classify stream' first");
    ...
    if (is_add == -1) return clib_error_return(0, "expecting: add|del");
    if (classify_table_index == ~0) return clib_error_return(0, "classifier table not specified");
    if (ip_version == 0) return clib_error_return(0, "IP version not specified");
    ...
    if (is_add) {
        if (table) return clib_error_return(0, "Specified classifier table already used");
        table                       = ipfix_classify_add_table();
        table->classify_table_index = classify_table_index;
    }
    else {
        if (!table) return clib_error_return(0, "Specified classifier table not registered");
    }
...
static clib_error_t *flow_report_classify_init(vlib_main_t *vm)
{
    ...
    if ((error = vlib_call_init_function(vm, flow_report_init))) return error;
