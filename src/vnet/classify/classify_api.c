...
static void vl_api_classify_pcap_lookup_table_t_handler(vl_api_classify_pcap_lookup_table_t *mp)
{
    ...
    if (n_skip > 5 || n_match == 0 || n_match > 5 || mask_len != n_match * sizeof(u32x4) || sw_if_index == ~0
    ...
    if (table_chain != ~0) table_index = classify_lookup_chain(table_chain, mask_vec, n_skip, n_match);

    vec_free(mask_vec);

out:
    rmp              = vl_msg_api_alloc(sizeof(*rmp));
    rmp->_vl_msg_id  = ntohs(REPLY_MSG_ID_BASE + VL_API_CLASSIFY_PCAP_LOOKUP_TABLE_REPLY);
...
static void vl_api_classify_pcap_set_table_t_handler(vl_api_classify_pcap_set_table_t *mp)
{
    ...
    if (sw_if_index == ~0 || (table_index != ~0 && pool_is_free_index(cm->tables, table_index))) {
        rv = VNET_API_ERROR_INVALID_VALUE;
        goto out;
    }
     ...
     */
    if (table_index != ~0 && mp->sort_masks) table_index = classify_sort_table_chain(cm, table_index);

    classify_set_pcap_chain(cm, sw_if_index, table_index);

out:
    rmp              = vl_msg_api_alloc(sizeof(*rmp));
    rmp->_vl_msg_id  = ntohs(REPLY_MSG_ID_BASE + VL_API_CLASSIFY_PCAP_SET_TABLE_REPLY);
    rmp->context     = mp->context;
    rmp->retval      = ntohl(rv);
    rmp->table_index = htonl(table_index);
...
out:
    ...
    rmp->_vl_msg_id = ntohs(REPLY_MSG_ID_BASE + VL_API_CLASSIFY_PCAP_GET_TABLES_REPLY);
    rmp->context    = mp->context;
    rmp->retval     = ntohl(rv);
...
static void vl_api_classify_trace_lookup_table_t_handler(vl_api_classify_trace_lookup_table_t *mp)
{
    ...
    u32 n_match  = ntohl(mp->match_n_vectors);
    u32 mask_len = ntohl(mp->mask_len);
    if (n_skip > 5 || n_match == 0 || n_match > 5 || mask_len != n_match * sizeof(u32x4)) {
        rv = VNET_API_ERROR_INVALID_VALUE;
        goto out;
    }
...
out:
    rmp              = vl_msg_api_alloc(sizeof(*rmp));
    rmp->_vl_msg_id  = ntohs((REPLY_MSG_ID_BASE + VL_API_CLASSIFY_TRACE_LOOKUP_TABLE_REPLY));
    rmp->context     = mp->context;
    rmp->retval      = ntohl(rv);
    rmp->table_index = htonl(table_index);
...
out:
    rmp              = vl_msg_api_alloc(sizeof(*rmp));
    rmp->_vl_msg_id  = ntohs((REPLY_MSG_ID_BASE + VL_API_CLASSIFY_TRACE_SET_TABLE_REPLY));
    rmp->context     = mp->context;
    rmp->retval      = ntohl(rv);
    rmp->table_index = htonl(table_index);
...
out:
    count           = vec_len(tables);
    rmp             = vl_msg_api_alloc_as_if_client(sizeof(*rmp) + count * sizeof(u32));
    rmp->_vl_msg_id = ntohs(REPLY_MSG_ID_BASE + VL_API_CLASSIFY_TRACE_GET_TABLES_REPLY);
    rmp->context    = mp->context;
    rmp->retval     = ntohl(rv);
    rmp->count      = htonl(count);
...
#undef _
    ...
    {
        if (table_index != ~0 && pool_is_free_index(cm->tables, table_index)) table_index = ~0;
    }
...
static void vl_api_classify_add_del_session_t_handler(vl_api_classify_add_del_session_t *mp)
{
    ...
    rv = vnet_classify_add_del_session(cm, table_index, mp->match, hit_next_index, opaque_index, advance, action, metadata, mp->is_add);
...
static void vl_api_policer_classify_set_interface_t_handler(vl_api_policer_classify_set_interface_t *mp)
{
    ...
    rv = vnet_set_policer_classify_intfc(vm, sw_if_index, ip4_table_index, ip6_table_index, l2_table_index, mp->is_add);
...
static void vl_api_policer_classify_dump_t_handler(vl_api_policer_classify_dump_t *mp)
{
    ...
    if (filter_sw_if_index >= vec_len(pcm->classify_table_index_by_sw_if_index[mp->type])) return;

    if (filter_sw_if_index != ~0)
        vec_tbl = &pcm->classify_table_index_by_sw_if_index[mp->type][filter_sw_if_index];
    else
        vec_tbl = pcm->classify_table_index_by_sw_if_index[mp->type];

    if (vec_len(vec_tbl)) {
        for (i = 0; i < vec_len(vec_tbl); i++) {
            if (vec_elt(vec_tbl, i) == ~0) continue;

            send_policer_classify_details(i, vec_elt(vec_tbl, i), reg, mp->context);
        }
    }
}
...
static void vl_api_classify_table_ids_t_handler(vl_api_classify_table_ids_t *mp)
{
    ...
    rmp             = vl_msg_api_alloc_as_if_client(sizeof(*rmp) + count * sizeof(u32));
    rmp->_vl_msg_id = ntohs(REPLY_MSG_ID_BASE + VL_API_CLASSIFY_TABLE_IDS_REPLY);
    rmp->context    = mp->context;
...
static void vl_api_classify_table_by_interface_t_handler(vl_api_classify_table_by_interface_t *mp)
{
    ...
    for (type = 0; type < IN_OUT_ACL_N_TABLES; type++) {
        u32 *vec_tbl = am->classify_table_index_by_sw_if_index[IN_OUT_ACL_INPUT_TABLE_GROUP][type];
        if (vec_len(vec_tbl)) {
...
static void vl_api_classify_table_info_t_handler(vl_api_classify_table_info_t *mp)
{
    ...
    pool_foreach (t, cm->tables) {
        if (table_id == t - cm->tables) {
            ...
            rmp->_vl_msg_id       = ntohs(REPLY_MSG_ID_BASE + VL_API_CLASSIFY_TABLE_INFO_REPLY);
            rmp->context          = mp->context;
...
static void send_classify_session_details(vl_api_registration_t *reg, u32 table_id, u32 match_length, vnet_classify_entry_t *e, u32 context)
{
    ...
    rmp->_vl_msg_id     = ntohs(REPLY_MSG_ID_BASE + VL_API_CLASSIFY_SESSION_DETAILS);
    rmp->context        = context;
    rmp->table_id       = ntohl(table_id);
            ...
            for (i = 0; i < t->nbuckets; i++) {
                ...
                for (j = 0; j < (1 << b->log2_pages); j++) {
                    for (k = 0; k < t->entries_per_page; k++) {
                        ...
                        if (vnet_classify_entry_is_free(v)) continue;

                        send_classify_session_details(reg, table_id, t->match_n_vectors * sizeof(u32x4), v, mp->context);
                    }
                }
            }
            break;
...
static void vl_api_flow_classify_set_interface_t_handler(vl_api_flow_classify_set_interface_t *mp)
{
    ...
    rv = vnet_set_flow_classify_intfc(vm, sw_if_index, ip4_table_index, ip6_table_index, mp->is_add);
...
static void vl_api_flow_classify_dump_t_handler(vl_api_flow_classify_dump_t *mp)
{
    ...
    if (vec_len(vec_tbl)) {
        for (i = 0; i < vec_len(vec_tbl); i++) {
            ...
            send_flow_classify_details(i, vec_elt(vec_tbl, i), reg, mp->context);
        }
    }
}
...
static void vl_api_input_acl_set_interface_t_handler(vl_api_input_acl_set_interface_t *mp)
{
    ...
    rv = vnet_set_input_acl_intfc(vm, sw_if_index, ip4_table_index, ip6_table_index, l2_table_index, mp->is_add);
...
static void vl_api_punt_acl_add_del_t_handler(vl_api_punt_acl_add_del_t *mp)
{
    vlib_main_t                     *vm = vlib_get_main();
    vl_api_punt_acl_add_del_reply_t *rmp;
    int                              rv;

    rv = vnet_set_in_out_acl_intfc(vm,
                                   0 /* sw_if_index */,
                                   ~0 /* ip4_table_index */,
                                   ~0 /* ip6_table_index */,
                                   ~0 /* l2_table_index */,
                                   ntohl(mp->ip4_table_index),
                                   ntohl(mp->ip6_table_index),
                                   mp->is_add,
                                   0 /* is_output */);
...
static void vl_api_punt_acl_get_t_handler(vl_api_punt_acl_get_t *mp)
{
    ...
    u32 *const *tables          = am->classify_table_index_by_sw_if_index[IN_OUT_ACL_INPUT_TABLE_GROUP];
...
static void vl_api_output_acl_set_interface_t_handler(vl_api_output_acl_set_interface_t *mp)
{
    ...
    rv = vnet_set_output_acl_intfc(vm, sw_if_index, ip4_table_index, ip6_table_index, l2_table_index, mp->is_add);
...
static clib_error_t *classify_api_hookup(vlib_main_t *vm)
{
     ...
     */
    vl_api_increase_msg_trace_size(am, VL_API_CLASSIFY_ADD_DEL_TABLE, 5 * sizeof(u32x4));
    vl_api_increase_msg_trace_size(am, VL_API_CLASSIFY_ADD_DEL_SESSION, 5 * sizeof(u32x4));
