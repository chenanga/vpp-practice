...
static u8 *format_flow_classify_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "FLOW_CLASSIFY: sw_if_index %d next %d table %d offset %d", t->sw_if_index, t->next_index, t->table_index, t->offset);
    return s;
}
...
static inline uword flow_classify_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, flow_classify_table_id_t tid)
{
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_TRUE(table_index0 != ~0)) {
                ...
                if (e0) {
                    ...
                    misses++;
                    vnet_classify_add_del_session(vcm, table_index0, h0, ~0, 0, 0, 0, 0, 1);
                    /* increment counter */
                    vnet_classify_find_entry(t0, h0, hash0, now);
                }
            }
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                flow_classify_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
    ...
    vlib_node_increment_counter(vm, node->node_index, FLOW_CLASSIFY_ERROR_MISS, misses);
    vlib_node_increment_counter(vm, node->node_index, FLOW_CLASSIFY_ERROR_HIT, hits);
    vlib_node_increment_counter(vm, node->node_index, FLOW_CLASSIFY_ERROR_CHAIN_HIT, chain_hits);
    vlib_node_increment_counter(vm, node->node_index, FLOW_CLASSIFY_ERROR_DROP, drop);
