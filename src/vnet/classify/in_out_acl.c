...
static int vnet_in_out_acl_feature_enable(in_out_acl_main_t *am, u32 sw_if_index, in_out_acl_table_id_t tid, int feature_enable, int is_output)
{
    ...
    switch (tid) {
        ...
        case IN_OUT_ACL_TABLE_L2:
            if (is_output)
                l2output_intf_bitmap_enable(sw_if_index, L2OUTPUT_FEAT_ACL, feature_enable);
            else
                l2input_intf_bitmap_enable(sw_if_index, L2INPUT_FEAT_ACL, feature_enable);
            return 0;
        ...
        case IN_OUT_ACL_TABLE_IP4_PUNT:
            if (sw_if_index != 0) return VNET_API_ERROR_INVALID_INTERFACE;
            arc_name     = "ip4-punt";
    ...
    rv = vnet_feature_enable_disable(arc_name, feature_name, sw_if_index, feature_enable, 0, 0);
    if (rv) return rv;
...
int vnet_set_in_out_acl_intfc(vlib_main_t *vm,
                              ...
                              u32          ip6_table_index,
                              u32          l2_table_index,
                              u32          ip4_punt_table_index,
                              u32          ip6_punt_table_index,
                              u32          is_add,
                              u32          is_output)
{
    ...
    u32                   acl[IN_OUT_ACL_N_TABLES] = {
                          ...
                          ip6_table_index,
                          l2_table_index,
                          ip4_punt_table_index,
                          ip6_punt_table_index,
    };
    u32 ti;
    int rv;
    ...
    for (ti = 0; ti < IN_OUT_ACL_N_TABLES; ti++) {
        if (acl[ti] == ~0) continue;

        if (pool_is_free_index(vcm->tables, acl[ti])) return VNET_API_ERROR_NO_SUCH_TABLE;

        vec_validate_init_empty(am->classify_table_index_by_sw_if_index[is_output][ti], sw_if_index, ~0);

        /* Reject any DEL operation with wrong sw_if_index */
        if (!is_add && (acl[ti] != am->classify_table_index_by_sw_if_index[is_output][ti][sw_if_index])) {
            clib_warning("Non-existent intf_idx=%d with table_index=%d for delete", sw_if_index, acl[ti]);
            return VNET_API_ERROR_NO_SUCH_TABLE;
        }

        /* Return ok on ADD operaton if feature is already enabled */
        if (is_add && am->classify_table_index_by_sw_if_index[is_output][ti][sw_if_index] != ~0) return 0;

        rv = vnet_in_out_acl_feature_enable(am, sw_if_index, ti, is_add, is_output);
        if (rv) return rv;

        if (is_add)
            am->classify_table_index_by_sw_if_index[is_output][ti][sw_if_index] = acl[ti];
        else
            am->classify_table_index_by_sw_if_index[is_output][ti][sw_if_index] = ~0;
    }
...
{
    return vnet_set_in_out_acl_intfc(vm,
                                     sw_if_index,
                                     ip4_table_index,
                                     ip6_table_index,
                                     l2_table_index,
                                     ~0 /* ip4_punt_table_index */,
                                     ~0 /* ip6_punt_table_index */,
                                     is_add,
...
static clib_error_t *set_in_out_acl_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd, u32 is_output)
{
    ...
    if (!idx_cnt) return clib_error_return(0, "Table index should be specified.");

    if (idx_cnt > 1) return clib_error_return(0, "Only one table index per API is allowed.");
...
{
    return set_in_out_acl_command_fn(vm, input, cmd, IN_OUT_ACL_INPUT_TABLE_GROUP);
}

static clib_error_t *set_output_acl_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    return set_in_out_acl_command_fn(vm, input, cmd, IN_OUT_ACL_OUTPUT_TABLE_GROUP);
}
...
u8 *format_vnet_in_out_acl_info(u8 *s, va_list *va)
{
    ...
    if (tid == ~0) {
        s = format(s, "%10s%20s\t\t%s", "Intfc idx", "Classify table", "Interface name");
        return s;
    }

    s = format(s, "%10d%20d\t\t%U", sw_if_idx, tid, format_vnet_sw_if_index_name, am->vnet_main, sw_if_idx);
...
static clib_error_t *show_in_out_acl_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd, u32 is_output)
{
    ...
    if (type == IN_OUT_ACL_N_TABLES) return clib_error_return(0, is_output ? "Invalid output ACL table type." : "Invalid input ACL table type.");

    vec_tbl = am->classify_table_index_by_sw_if_index[is_output][type];

    if (vec_len(vec_tbl))
        vlib_cli_output(vm, "%U", format_vnet_in_out_acl_info, am, ~0 /* hdr */, ~0);
    else
        vlib_cli_output(vm, is_output ? "No output ACL tables configured" : "No input ACL tables configured");

    for (i = 0; i < vec_len(vec_tbl); i++) {
        if (vec_elt(vec_tbl, i) == ~0) continue;

        vlib_cli_output(vm, "%U", format_vnet_in_out_acl_info, am, i, vec_elt(vec_tbl, i));
    }
...
{
    return show_in_out_acl_command_fn(vm, input, cmd, IN_OUT_ACL_INPUT_TABLE_GROUP);
}

static clib_error_t *show_outacl_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    return show_in_out_acl_command_fn(vm, input, cmd, IN_OUT_ACL_OUTPUT_TABLE_GROUP);
}
