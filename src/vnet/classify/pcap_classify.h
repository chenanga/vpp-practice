...
static_always_inline int vnet_is_packet_pcaped(vnet_pcap_t *pp, vlib_buffer_t *b, u32 sw_if_index)
{
    ...
    if (pcap_sw_if_index != 0) {
        if (~0 == sw_if_index) sw_if_index = vnet_buffer(b)->sw_if_index[VLIB_RX];
        if (pcap_sw_if_index != sw_if_index) return 0; /* wrong interface, skip */
    }

    if (pcap_error_index != (vlib_error_t) ~0 && pcap_error_index != b->error) return 0; /* wrong error */

    if (filter_classify_table_index != ~0 && pp->current_filter_function(b, filter_classify_table_index, 0 /* full classify */) != 1)
