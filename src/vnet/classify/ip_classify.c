...
static u8 *format_ip_classify_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "IP_CLASSIFY: next_index %d, table %d, entry %d", t->next_index, t->table_index, t->entry_index);
    return s;
}
...
static inline uword ip_classify_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ip4)
{
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_TRUE(table_index0 != ~0)) {
                ...
                if (e0) {
                    ...
                    next0 = (e0->next_index < node->n_next_nodes) ? e0->next_index : next0;
                    hits++;
                }
                else {
                    while (1) {
                        if (t0->next_table_index != ~0)
                            ...
                            next0 = (t0->miss_next_index < n_next) ? t0->miss_next_index : next0;
                            misses++;
                            break;
                        }
            ...
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                ip_classify_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                t->next_index          = next0;
    ...
    vlib_node_increment_counter(vm, node->node_index, IP_CLASSIFY_ERROR_MISS, misses);
    vlib_node_increment_counter(vm, node->node_index, IP_CLASSIFY_ERROR_HIT, hits);
    vlib_node_increment_counter(vm, node->node_index, IP_CLASSIFY_ERROR_CHAIN_HIT, chain_hits);
