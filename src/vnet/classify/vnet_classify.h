...
static inline u32 vnet_classify_hash_packet_inline(const vnet_classify_table_t *t, const u8 *h)
{
    u64 xor_sum;
    ASSERT(t);
    ...
    if (PREDICT_FALSE(load_mask >> 8)) xor_sum_x8 ^= u64x8_mask_load_zero(data + 1, load_mask >> 8) & mask[1];
    ...
    if (PREDICT_FALSE(load_mask >> 8)) xor_sum_x4 ^= u64x4_mask_load_zero(data + 2, load_mask >> 8) & mask[2];
...
static inline void vnet_classify_prefetch_bucket(vnet_classify_table_t *t, u64 hash)
{
    u32 bucket_index;
...
static inline vnet_classify_entry_t *vnet_classify_get_entry(const vnet_classify_table_t *t, uword offset)
{
...
static inline uword vnet_classify_get_offset(vnet_classify_table_t *t, vnet_classify_entry_t *v)
{
    u8 *hp, *vp;
...
static inline vnet_classify_entry_t *vnet_classify_entry_at_index(const vnet_classify_table_t *t, vnet_classify_entry_t *e, u32 index)
{
    u8 *eu8;
...
static inline void vnet_classify_prefetch_entry(vnet_classify_table_t *t, u64 hash)
{
    u32                     bucket_index;
    u32                     value_index;
...
static_always_inline int vnet_classify_entry_is_equal(vnet_classify_entry_t *v, const u8 *d, u8 *m, u32 match_n_vectors, u16 load_mask)
{
#if defined(CLIB_HAVE_VEC512) && defined(CLIB_HAVE_VEC512_MASK_LOAD_STORE)
    ...
    r = (u64x8_mask_load_zero(data, load_mask) & mask[0]) ^ u64x8_mask_load_zero(key, load_mask);
    load_mask >>= 8;

    if (PREDICT_FALSE(load_mask)) r |= (u64x8_mask_load_zero(data + 1, load_mask) & mask[1]) ^ u64x8_mask_load_zero(key + 1, load_mask);

    if (u64x8_is_all_zero(r)) return 1;
    ...
    r = (u64x4_mask_load_zero(data, load_mask) & mask[0]) ^ u64x4_mask_load_zero(key, load_mask);
    load_mask >>= 4;

    r |= (u64x4_mask_load_zero(data + 1, load_mask) & mask[1]) ^ u64x4_mask_load_zero(key + 1, load_mask);
    load_mask >>= 4;

    if (PREDICT_FALSE(load_mask)) r |= (u64x4_mask_load_zero(data + 2, load_mask) & mask[2]) ^ u64x4_mask_load_zero(key + 2, load_mask);

    if (u64x4_is_all_zero(r)) return 1;
    ...
    if (u64x2_is_all_zero(r)) return 1;
...
static inline vnet_classify_entry_t *vnet_classify_find_entry_inline(const vnet_classify_table_t *t, const u8 *h, u32 hash, f64 now)
{
    ...
    for (i = 0; i < limit; i++) {
        if (vnet_classify_entry_is_equal(v, h, mask, match_n_vectors, load_mask)) {
            if (PREDICT_TRUE(now)) {
                v->hits++;
                v->last_heard = now;
            }
