    ...
    for (i = 0; i < t->nbuckets; i++) {
        ...
        for (j = 0; j < (1 << b->log2_pages); j++) {
            for (k = 0; k < t->entries_per_page; k++) {
                ...
                if (vnet_classify_entry_is_busy(v)) active_elements++;
            }
        }
    }

    if (active_elements != t->active_elements) clib_warning("found %u expected %u elts", active_elements, t->active_elements);
}
#else
void mv(vnet_classify_table_t *t)
{
                                               ...
                                               u32                   match_n_vectors)
{
    ...
    t->mheap = clib_mem_create_heap(0, memory_size, 1 /* locked */, "classify");
...
void vnet_classify_delete_table_index(vnet_classify_main_t *cm, u32 table_index, int del_chain)
{
    ...
    if (pool_is_free_index(cm->tables, table_index)) return;

    t = pool_elt_at_index(cm->tables, table_index);
    if (del_chain && t->next_table_index != ~0) /* Recursively delete the entire chain */
...
static vnet_classify_entry_t *vnet_classify_entry_alloc(vnet_classify_table_t *t, u32 log2_pages)
{
    ...
    required_length = (sizeof(vnet_classify_entry_t) + (t->match_n_vectors * sizeof(u32x4))) * t->entries_per_page * (1 << log2_pages);
...
static inline void make_working_copy(vnet_classify_table_t *t, vnet_classify_bucket_t *b)
{
    ...
    required_length     = (sizeof(vnet_classify_entry_t) + (t->match_n_vectors * sizeof(u32x4))) * t->entries_per_page * (1 << b->log2_pages);

    t->saved_bucket.as_u64 = b->as_u64;
    oldheap                = clib_mem_set_heap(t->mheap);

    if (required_length > working_copy_length) {
        if (working_copy) clib_mem_free(working_copy);
        working_copy                    = clib_mem_alloc_aligned(required_length, CLIB_CACHE_LINE_BYTES);
    ...
    for (i = 0; i < length_in_entries; i++) {
        ...
        if (vnet_classify_entry_is_busy(v)) {
            ...
            for (j = 0; j < t->entries_per_page; j++) {
                new_v = vnet_classify_entry_at_index(t, new_values, new_hash + j);

                if (vnet_classify_entry_is_free(new_v)) {
                    clib_memcpy_fast(new_v, v, sizeof(vnet_classify_entry_t) + (t->match_n_vectors * sizeof(u32x4)));
...
static int vnet_classify_add_del(vnet_classify_table_t *t, vnet_classify_entry_t *add_v, int is_add)
{
    ...
    if (b->offset == 0) {
        ...
        clib_memcpy_fast(v, add_v, sizeof(vnet_classify_entry_t) + t->match_n_vectors * sizeof(u32x4));
    ...
    if (is_add) {
        ...
        for (i = 0; i < limit; i++) {
            ...
            if (!memcmp(v->key, add_v->key, t->match_n_vectors * sizeof(u32x4))) {
                ...
                clib_memset(v, 0xff, sizeof(vnet_classify_entry_t) + t->match_n_vectors * sizeof(u32x4));
                v->flags |= VNET_CLASSIFY_ENTRY_FREE;
    ...
    if (t->saved_bucket.linear_search) goto linear_resplit;
    ...
    try_resplit:
        ...
        linear_resplit:
            ...
            new_v = split_and_rehash_linear(t, working_copy, old_log2_pages, new_log2_pages);
            /* A new linear-search bucket? */
            if (!t->saved_bucket.linear_search) t->linear_buckets++;
    ...
    for (i = 0; i < limit; i++) {
        ...
        if (vnet_classify_entry_is_free(new_v)) {
            clib_memcpy_fast(new_v, add_v, sizeof(vnet_classify_entry_t) + t->match_n_vectors * sizeof(u32x4));
            new_v->flags &= ~(VNET_CLASSIFY_ENTRY_FREE);
...
vnet_classify_entry_t *vnet_classify_find_entry(const vnet_classify_table_t *t, u8 *h, u32 hash, f64 now)
{
...
u8 *format_classify_entry(u8 *s, va_list *args)
{
    ...
    s = format(s, "        k: %U\n", format_hex_bytes, e->key, t->match_n_vectors * sizeof(u32x4));
...
u8 *format_classify_table(u8 *s, va_list *args)
{
    ...
    for (i = 0; i < t->nbuckets; i++) {
        b = &t->buckets[i];
        if (b->offset == 0) {
            if (verbose > 1) s = format(s, "[%d]: empty\n", i);
            continue;
        }
        ...
        if (verbose) {
            s = format(s,
                       ...
                       i,
                       b->offset,
                       (1 << b->log2_pages) * t->entries_per_page,
        ...
        for (j = 0; j < (1 << b->log2_pages); j++) {
            for (k = 0; k < t->entries_per_page; k++) {
                ...
                if (verbose) {
                    s = format(s, "    %d: %U\n", j * t->entries_per_page + k, format_classify_entry, t, v);
                }
                active_elements++;
            }
        }
                                ...
                                int                   del_chain)
{
    ...
    if (is_add) {
        ...
        {
            if (memory_size == 0) return VNET_API_ERROR_INVALID_MEMORY_SIZE;

            if (nbuckets == 0) return VNET_API_ERROR_INVALID_VALUE;

            if (match < 1 || match > 5) return VNET_API_ERROR_INVALID_VALUE;

            t                      = vnet_classify_new_table(cm, mask, nbuckets, memory_size, skip, match);
        ...
        {
            vnet_classify_main_t *cm = &vnet_classify_main;
            if (pool_is_free_index(cm->tables, *table_index)) return VNET_API_ERROR_CLASSIFY_TABLE_NOT_FOUND;
...
#define _(a) \
    if (a) clib_memset(&tcp->a, 0xff, sizeof(tcp->a));
    foreach_tcp_proto_field;
#undef _
...
#define _(a) \
    if (a) clib_memset(&udp->a, 0xff, sizeof(udp->a));
    foreach_udp_proto_field;
#undef _
...
#define _(a) \
    if (a) clib_memset(&ip->a, 0xff, sizeof(ip->a));
    foreach_ip4_proto_field;
#undef _

    if (src_address) ip->src_address.as_u32 = src_prefix_mask;

    if (dst_address) ip->dst_address.as_u32 = dst_prefix_mask;

    ip->ip_version_and_header_length = 0;

    if (version) ip->ip_version_and_header_length |= 0xF0;

    if (hdr_length) ip->ip_version_and_header_length |= 0x0F;
...
#undef _
    ...
    found_something = version + traffic_class + flow_label + src_address + dst_address + protocol;
...
#undef _
    ...
    if (version) ip_version_traffic_class_and_flow_label |= 0xF0000000;

    if (traffic_class) ip_version_traffic_class_and_flow_label |= 0x0FF00000;

    if (flow_label) ip_version_traffic_class_and_flow_label |= 0x000FFFFF;

    ip->ip_version_traffic_class_and_flow_label = clib_host_to_net_u32(ip_version_traffic_class_and_flow_label);
...
uword unformat_l2_mask(unformat_input_t *input, va_list *args)
{
    ...
    if ((src + dst + proto + tag1 + tag2 + dot1q + dot1ad + ignore_tag1 + ignore_tag2 + cos1 + cos2) == 0) return 0;

    if (tag1 || ignore_tag1 || cos1 || dot1q) len = 18;
    if (tag2 || ignore_tag2 || cos2 || dot1ad) len = 22;

    vec_validate(mask, len - 1);

    if (dst) clib_memset(mask, 0xff, 6);

    if (src) clib_memset(mask + 6, 0xff, 6);
    ...
    if (tag2 || dot1ad) {
        ...
        if (cos2) mask[18] |= 0xe0;
        if (proto) mask[21] = mask[20] = 0xff;
        if (tag1) {
            mask[15] = 0xff;
            mask[14] = 0x0f;
        }
        if (cos1) mask[14] |= 0xe0;
        *maskp = mask;
        return 1;
    }
    if (tag1 | dot1q) {
        ...
        if (cos1) mask[14] |= 0xe0;
        if (proto) mask[16] = mask[17] = 0xff;
        *maskp = mask;
        return 1;
    }
    if (cos2) mask[18] |= 0xe0;
    if (cos1) mask[14] |= 0xe0;
    if (proto) mask[12] = mask[13] = 0xff;
...
uword unformat_classify_mask(unformat_input_t *input, va_list *args)
{
    ...
    if (mask || l2 || l3 || l4) {
        ...
        *skipp = i / sizeof(u32x4);
        ...
        if (match == 0) clib_warning("BUG: match 0");
...
#define _(n, N)                                  \
    if (unformat(input, #n)) {                   \
        next_index = L2_INPUT_CLASSIFY_NEXT_##N; \
        goto out;                                \
    }
...
#define _(n, N)                                   \
    if (unformat(input, #n)) {                    \
        next_index = L2_OUTPUT_CLASSIFY_NEXT_##N; \
        goto out;                                 \
    }
...
static clib_error_t *classify_table_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "del"))
            ...
            ;
        else if (unformat(input, "mask %U", unformat_classify_mask, &mask, &skip, &match))
            ;
            ...
            ;
        else if (unformat(input, "miss-next %U", unformat_ip_next_index, &miss_next_index))
            ;
        else if (unformat(input, "l2-input-miss-next %U", unformat_l2_input_next_index, &miss_next_index))
            ;
        else if (unformat(input, "l2-output-miss-next %U", unformat_l2_output_next_index, &miss_next_index))
            ;
        else if (unformat(input, "acl-miss-next %U", unformat_acl_next_index, &miss_next_index))
            ;
    ...
    if (is_add && mask == 0 && table_index == ~0) return clib_error_return(0, "Mask required");
    ...
    if (!is_add && table_index == ~0) return clib_error_return(0, "table index required for delete");

    rv = vnet_classify_add_del_table(cm,
                                     mask,
                                     nbuckets,
                                     (u32) memory_size,
                                     skip,
...
u32 classify_get_pcap_chain(vnet_classify_main_t *cm, u32 sw_if_index)
{
    ...
    if (sw_if_index != ~0 && (sw_if_index < vec_len(cm->classify_table_index_by_sw_if_index)))
...
void classify_set_pcap_chain(vnet_classify_main_t *cm, u32 sw_if_index, u32 table_index)
{
    ...
    if (sw_if_index != ~0 && table_index != ~0) vec_validate_init_empty(cm->classify_table_index_by_sw_if_index, sw_if_index, ~0);

    if (table_index == ~0) {
        u32 old_table_index = ~0;

        if (sw_if_index < vec_len(cm->classify_table_index_by_sw_if_index)) old_table_index = cm->classify_table_index_by_sw_if_index[sw_if_index];
     ...
     */
    if (vec_len(cm->classify_table_index_by_sw_if_index) > sw_if_index) cm->classify_table_index_by_sw_if_index[sw_if_index] = table_index;
    if (sw_if_index > 0) {
...
u32 classify_lookup_chain(u32 table_index, u8 *mask, u32 n_skip, u32 n_match)
{
    ...
    if (table_index == ~0) return ~0;

    for (cti = table_index; cti != ~0; cti = t->next_table_index) {
        t = pool_elt_at_index(cm->tables, cti);

        /* Classifier geometry mismatch, can't use this table. */
        if (t->match_n_vectors != n_match || t->skip_n_vectors != n_skip) continue;

        /* Masks aren't congruent, can't use this table. */
        if (t->match_n_vectors * sizeof(u32x4) != vec_len(mask)) continue;

        /* Masks aren't bit-for-bit identical, can't use this table. */
        if (memcmp(t->mask, mask, t->match_n_vectors * sizeof(u32x4))) continue;
...
static clib_error_t *classify_filter_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "del"))
            ...
            if (sw_if_index == 0) return clib_error_return(0, "Local interface not supported...");
        }
    ...
    if (is_add && mask == 0)
        ...
        err = clib_error_return(0, "Packet trace and pcap are mutually exclusive...");
     ...
     */
    if (unformat(line_input, "match %U", unformat_classify_match, cm, &match_vector, table_index) == 0) return 0;

    /*
     * We use hit or miss to determine whether to trace or pcap pkts
     * so the session setup is very limited
     */
    rv = vnet_classify_add_del_session(cm,
                                       table_index,
                                       match_vector,
                                       0 /* hit_next_index */,
                                       0 /* opaque_index */,
...
u8 *format_vnet_classify_table(u8 *s, va_list *args)
{
    ...
    if (index == ~0) {
        s = format(s, "\n%10s%10s%10s%10s", "TableIdx", "Sessions", "NextTbl", "NextNode", verbose ? "Details" : "");
        return s;
    }
    ...
    s = format(s, "\n  Heap: %U", format_clib_mem_heap, t->mheap, 0 /*verbose */);
    ...
    s = format(s,
               ...
               t->match_n_vectors,
               t->current_data_flag,
               t->current_data_offset);
    s = format(s, "\n  mask %U", format_hex_bytes, t->mask, t->match_n_vectors * sizeof(u32x4));
    s = format(s, "\n  linear-search buckets %d\n", t->linear_buckets);

    if (verbose == 0) return s;
...
static clib_error_t *show_classify_tables_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    pool_foreach (t, cm->tables) {
        if (match_index == ~0 || (match_index == t - cm->tables)) vec_add1(indices, t - cm->tables);
    }

    if (vec_len(indices)) {
        for (i = 0; i < vec_len(indices); i++) {
            vlib_cli_output(vm, "%U", format_vnet_classify_table, cm, verbose, ~0 /* hdr */);
            vlib_cli_output(vm, "%U", format_vnet_classify_table, cm, verbose, indices[i]);
        }
    }
    else
...
uword unformat_ip4_match(unformat_input_t *input, va_list *args)
{
    ...
    if (version + hdr_length + src + dst + proto + tos + length + fragment_id + ttl + checksum == 0) return 0;
    ...
    if (src) ip->src_address.as_u32 = src_val.as_u32;

    if (dst) ip->dst_address.as_u32 = dst_val.as_u32;

    if (proto) ip->protocol = proto_val;

    /* These are not, but they're included for completeness */
    if (version) ip->ip_version_and_header_length |= (version_val & 0xF) << 4;

    if (hdr_length) ip->ip_version_and_header_length |= (hdr_length_val & 0xF);

    if (tos) ip->tos = tos_val;

    if (length) ip->length = clib_host_to_net_u16(length_val);

    if (ttl) ip->ttl = ttl_val;

    if (checksum) ip->checksum = clib_host_to_net_u16(checksum_val);
...
uword unformat_ip6_match(unformat_input_t *input, va_list *args)
{
    ...
    if (version + traffic_class + flow_label + src + dst + proto + payload_length + hop_limit == 0) return 0;
    ...
    if (src) clib_memcpy_fast(&ip->src_address, &src_val, sizeof(ip->src_address));

    if (dst) clib_memcpy_fast(&ip->dst_address, &dst_val, sizeof(ip->dst_address));

    if (proto) ip->protocol = proto_val;

    ip_version_traffic_class_and_flow_label = 0;

    if (version) ip_version_traffic_class_and_flow_label |= (version_val & 0xF) << 28;

    if (traffic_class) ip_version_traffic_class_and_flow_label |= (traffic_class_val & 0xFF) << 20;

    if (flow_label) ip_version_traffic_class_and_flow_label |= (flow_label_val & 0xFFFFF);

    ip->ip_version_traffic_class_and_flow_label = clib_host_to_net_u32(ip_version_traffic_class_and_flow_label);

    if (payload_length) ip->payload_length = clib_host_to_net_u16(payload_length_val);

    if (hop_limit) ip->hop_limit = hop_limit_val;
...
uword unformat_l2_match(unformat_input_t *input, va_list *args)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "src %U", unformat_ethernet_address, &src_val))
            ...
            dst = 1;
        else if (unformat(input, "proto %U", unformat_ethernet_type_host_byte_order, &proto_val))
            proto = 1;
    ...
    if ((src + dst + proto + tag1 + tag2 + ignore_tag1 + ignore_tag2 + cos1 + cos2) == 0) return 0;

    if (tag1 || ignore_tag1 || cos1) len = 18;
    if (tag2 || ignore_tag2 || cos2) len = 22;

    vec_validate_aligned(match, len - 1, sizeof(u32x4));

    if (dst) clib_memcpy_fast(match, dst_val, 6);

    if (src) clib_memcpy_fast(match + 6, src_val, 6);

    if (tag2) {
        /* inner vlan tag */
        match[19] = tag2_val[1];
        match[18] = tag2_val[0];
        if (cos2) match[18] |= (cos2_val & 0x7) << 5;
        if (proto) {
        ...
        if (cos1) match[14] |= (cos1_val & 0x7) << 5;
        *matchp = match;
        return 1;
    }
    if (tag1) {
...
uword unformat_classify_match(unformat_input_t *input, va_list *args)
{
    ...
    if (pool_is_free_index(cm->tables, table_index)) return 0;
    ...
    if (match || l2 || l3 || l4) {
        if (l2 || l3 || l4) {
            if (add_l2) {
                ...
                if (l2 == 0) vec_validate_aligned(l2, 13, sizeof(u32x4));
                match = l2;
                if (l3) {
        ...
        vec_validate_aligned(match, ((t->match_n_vectors + t->skip_n_vectors) * sizeof(u32x4)) - 1, sizeof(u32x4));

        /* Set size, include skipped vectors */
        vec_set_len(match, (t->match_n_vectors + t->skip_n_vectors) * sizeof(u32x4));
                                  ...
                                  int                   is_add)
{
    ...
    if (pool_is_free_index(cm->tables, table_index)) return VNET_API_ERROR_NO_SUCH_TABLE;
    ...
    if (e->action == CLASSIFY_ACTION_SET_IP4_FIB_INDEX)
        e->metadata = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, metadata, FIB_SOURCE_CLASSIFY);
    else if (e->action == CLASSIFY_ACTION_SET_IP6_FIB_INDEX)
        e->metadata = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP6, metadata, FIB_SOURCE_CLASSIFY);
    ...
    clib_memcpy_fast(&e->key, match + t->skip_n_vectors * sizeof(u32x4), t->match_n_vectors * sizeof(u32x4));
    ...
    if (rv) return VNET_API_ERROR_NO_SUCH_ENTRY;
    return 0;
}
...
static clib_error_t *classify_session_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "del"))
            is_add = 0;
        else if (unformat(input, "hit-next %U", unformat_ip_next_index, &hit_next_index))
            ;
        else if (unformat(input, "l2-input-hit-next %U", unformat_l2_input_next_index, &hit_next_index))
            ;
        else if (unformat(input, "l2-output-hit-next %U", unformat_l2_output_next_index, &hit_next_index))
            ;
        else if (unformat(input, "acl-hit-next %U", unformat_acl_next_index, &hit_next_index))
            ;
        else if (unformat(input, "policer-hit-next %U", unformat_policer_next_index, &hit_next_index))
            ;
        else if (unformat(input, "opaque-index %lld", &opaque_index))
            ;
        else if (unformat(input, "match %U", unformat_classify_match, cm, &match, table_index))
            ;
            ...
            for (i = 0; i < vec_len(cm->unformat_opaque_index_fns); i++) {
                if (unformat(input, "%U", cm->unformat_opaque_index_fns[i], &opaque_index)) goto found_opaque;
            }
            break;
        }
    found_opaque:;
    ...
    if (table_index == ~0) return clib_error_return(0, "Table index required");

    if (is_add && match == 0) return clib_error_return(0, "Match value required");

    rv = vnet_classify_add_del_session(cm, table_index, match, hit_next_index, opaque_index, advance, action, metadata, is_add);
    ...
    switch (rv) {
        ...
        default:
            return clib_error_return(0, "vnet_classify_add_del_session returned %d", rv);
    }
...
static uword unformat_opaque_sw_if_index(unformat_input_t *input, va_list *args)
{
    ...
    if (unformat(input, "opaque-sw_if_index %U", unformat_vnet_sw_interface, vnet_get_main(), &sw_if_index)) {
        *opaquep = sw_if_index;
        return 1;
    }
    return 0;
...
static uword unformat_ip_next_node(unformat_input_t *input, va_list *args)
{
    ...
    if (unformat(input, "ip6-node %U", unformat_vlib_node, cm->vlib_main, &node_index)) {
        next_index = vlib_node_add_next(cm->vlib_main, ip6_classify_node.index, node_index);
    }
    else if (unformat(input, "ip4-node %U", unformat_vlib_node, cm->vlib_main, &node_index)) {
        next_index = vlib_node_add_next(cm->vlib_main, ip4_classify_node.index, node_index);
    }
    else
        return 0;
...
static uword unformat_acl_next_node(unformat_input_t *input, va_list *args)
{
    ...
    if (unformat(input, "ip6-node %U", unformat_vlib_node, cm->vlib_main, &node_index)) {
        next_index = vlib_node_add_next(cm->vlib_main, ip6_inacl_node.index, node_index);
    }
    else if (unformat(input, "ip4-node %U", unformat_vlib_node, cm->vlib_main, &node_index)) {
        next_index = vlib_node_add_next(cm->vlib_main, ip4_inacl_node.index, node_index);
    }
    else
        return 0;
...
static uword unformat_l2_input_next_node(unformat_input_t *input, va_list *args)
{
    ...
    if (unformat(input, "input-node %U", unformat_vlib_node, cm->vlib_main, &node_index)) {
        next_index = vlib_node_add_next(cm->vlib_main, l2_input_classify_node.index, node_index);
...
static uword unformat_l2_output_next_node(unformat_input_t *input, va_list *args)
{
    ...
    if (unformat(input, "output-node %U", unformat_vlib_node, cm->vlib_main, &node_index)) {
        next_index = vlib_node_add_next(cm->vlib_main, l2_output_classify_node.index, node_index);
...
static clib_error_t *vnet_classify_init(vlib_main_t *vm)
{
    ...
    vnet_classify_register_unformat_opaque_index_fn(unformat_opaque_sw_if_index);

    vnet_classify_register_unformat_ip_next_index_fn(unformat_ip_next_node);

    vnet_classify_register_unformat_l2_next_index_fn(unformat_l2_input_next_node);

    vnet_classify_register_unformat_l2_next_index_fn(unformat_l2_output_next_node);
...
VLIB_REGISTER_TRACE_FILTER_FUNCTION(vnet_is_packet_traced_fn, static) = {.name        = "vnet_is_packet_traced",
...
static clib_error_t *test_classify_churn(test_classify_main_t *tm)
{
    ...
    tm->table = vnet_classify_new_table(tm->classify_main, (u8 *) mask, tm->buckets, tm->memory_size, 0 /* skip */, 3 /* vectors to match */);
    tm->table->miss_next_index = IP_LOOKUP_NEXT_DROP;
    tm->table_index            = tm->table - tm->classify_main->tables;
    vlib_cli_output(vm, "Created table %d, buckets %d", tm->table_index, tm->buckets);

    vlib_cli_output(vm, "Initialize: add %d (approx. half of %d sessions)...", tm->sessions / 2, tm->sessions);
    ...
    for (i = 0; i < tm->sessions / 2; i++) {
        ...
        if (rv != 0) clib_warning("add: returned %d", rv);

        if (tm->verbose) vlib_cli_output(vm, "add: %U", format_ip4_address, &ep->addr.as_u32);
    }

    vlib_cli_output(vm, "Execute %d random add/delete operations", tm->iterations);
    ...
    for (i = 0; i < tm->iterations; i++) {
        ...
        if (tm->verbose) vlib_cli_output(vm, "%s: %U", is_add ? "add" : "del", format_ip4_address, &ep->addr.as_u32);
        ...
        rv = vnet_classify_add_del_session(tm->classify_main,
                                           ...
                                           is_add);
        if (rv != 0)
            vlib_cli_output(vm, "%s[%d]: %U returned %d", is_add ? "add" : "del", index, format_ip4_address, &ep->addr.as_u32, rv);
        else
            ep->in_table = is_add;
    }

    vlib_cli_output(vm, "Remove remaining %d entries from the table", tm->table->active_elements);
    ...
    for (i = 0; i < tm->sessions; i++) {
        ...
        ep = tm->entries + i;
        if (ep->in_table == 0) continue;

        data->ip.src_address.as_u32 = ep->addr.as_u32;

        hash = vnet_classify_hash_packet(tm->table, (u8 *) data);

        e = vnet_classify_find_entry(tm->table, (u8 *) data, hash, 0 /* time_now */);
        if (e == 0) {
            clib_warning("Couldn't find %U index %d which should be present", format_ip4_address, ep->addr, i);
            continue;
        }
        ...
        if (tm->verbose) vlib_cli_output(vm, "del: %U", format_ip4_address, &ep->addr.as_u32);
    }

    vlib_cli_output(vm, "%d entries remain, MUST be zero", tm->table->active_elements);
    ...
    vnet_classify_delete_table_index(tm->classify_main, tm->table_index, 1 /* del_chain */);
    tm->table       = 0;
    tm->table_index = ~0;
    vec_free(tm->entries);
