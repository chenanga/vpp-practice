                                              ...
                                              int                      feature_enable)
{
    ...
    if (tid == FLOW_CLASSIFY_TABLE_IP4) {
        vnet_feature_enable_disable("ip4-unicast", "ip4-flow-classify", sw_if_index, feature_enable, 0, 0);
        arc = vnet_get_feature_arc_index("ip4-unicast");
    }
    else {
        vnet_feature_enable_disable("ip6-unicast", "ip6-flow-classify", sw_if_index, feature_enable, 0, 0);
...
int vnet_set_flow_classify_intfc(vlib_main_t *vm, u32 sw_if_index, u32 ip4_table_index, u32 ip6_table_index, u32 is_add)
{
    ...
    for (ti = 0; ti < FLOW_CLASSIFY_N_TABLES; ti++) {
        ...
        vec_validate_init_empty(fcm->classify_table_index_by_sw_if_index[ti], sw_if_index, ~0);

        /* Reject any DEL operation with wrong sw_if_index */
        if (!is_add && (pct[ti] != fcm->classify_table_index_by_sw_if_index[ti][sw_if_index])) {
        ...
        if (is_add && fcm->classify_table_index_by_sw_if_index[ti][sw_if_index] != ~0) return 0;
...
static clib_error_t *set_flow_classify_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    rv = vnet_set_flow_classify_intfc(vm, sw_if_index, ip4_table_index, ip6_table_index, is_add);
...
static clib_error_t *show_flow_classify_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (type == FLOW_CLASSIFY_N_TABLES) return clib_error_return(0, "Invalid table type.");
    ...
    for (i = 0; i < vec_len(vec_tbl); i++) {
        ...
        vlib_cli_output(vm, "%10d%20d\t\t%U", i, vec_elt(vec_tbl, i), format_vnet_sw_if_index_name, fcm->vnet_main, i);
    }
