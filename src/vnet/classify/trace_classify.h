...
static inline int vnet_is_packet_traced_inline(vlib_buffer_t *b, u32 classify_table_index, int func)
{
    ...
    if (classify_table_index == ~0) return -1;

    /* This will happen... */
    if (pool_is_free_index(vcm->tables, classify_table_index)) return -1;
    ...
    e = vnet_classify_find_entry(t, vlib_buffer_get_current(b), hash, 0 /* time = 0, disables hit-counter */);
    ...
    while (1) {
        ...
        if (PREDICT_TRUE(t->next_table_index == ~0)) return 0;
