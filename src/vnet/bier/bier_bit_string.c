...
#define BIER_GET_STRING_POS(_bp, _byte, _bit, _str)                                                       \
    {                                                                                                     \
        _bp--;                                                                                            \
        _byte = ((BIER_BBS_LEN_TO_BUCKETS((_str)->bbs_len) - 1) - (_bp / BIER_BIT_MASK_BITS_PER_BUCKET)); \
        _bit  = _bp % BIER_BIT_MASK_BITS_PER_BUCKET;                                                      \
    }

static inline int bier_bit_pos_is_valid(bier_bp_t bp, const bier_bit_string_t *bbs)
{
    if (!((bp <= BIER_BBS_LEN_TO_BITS((bbs)->bbs_len)) && (bp >= 1))) {
        return (0);
    }
    return (1);
}
...
u8 *format_bier_bit_string(u8 *string, va_list *args)
{
    ...
    for (index = 0; index < (bs->bbs_len / 4); index++) {
        ...
        string = format(string, "%s%X", index ? ":" : "", clib_net_to_host_u32(ptr[index]));
    }
