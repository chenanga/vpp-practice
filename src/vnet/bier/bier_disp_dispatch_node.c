...
always_inline uword bier_disp_dispatch_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
             ...
             * DSCP mumble mumble...
             */
            vlib_buffer_advance(b0, (vnet_buffer(b0)->mpls.bier.n_bytes + sizeof(*hdr0)));
            ...
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                bier_disp_dispatch_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
                tr->pproto                     = pproto0;
...
VLIB_NODE_FN(bier_disp_dispatch_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (bier_disp_dispatch_inline(vm, node, frame));
}

VLIB_REGISTER_NODE(bier_disp_dispatch_node) = {.name        = "bier-disp-dispatch",
