...
static uword bier_lookup(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            bier_bit_string_init(&bbs, bt0->bt_id.bti_hdr_len, buckets_copy);
            ...
            for (index = 0; index < num_buckets; index++) {
                ...
                while (bucket) {
                    ...
                    fbs += (((num_buckets - 1) - index) * BIER_BIT_MASK_BITS_PER_INT);
                    ...
                    if (PREDICT_TRUE(INDEX_INVALID != bfmi0)) {
                         ...
                         */
                        bier_bit_string_clear_string(&bfm0->bfm_bits.bfmb_input_reset_string, &bbs);
                        bucket = ((int *) bbs.bbs_buckets)[index];
                         ...
                         */
                        vlib_node_increment_counter(vm, node->node_index, BIER_LOOKUP_ERROR_FMASK_UNRES, 1);
                        bucket = ((int *) bbs.bbs_buckets)[index];
                        continue;
                    }
                }
            ...
            if (PREDICT_TRUE(0 != n_clones)) {
                ...
                num_cloned = vlib_buffer_clone(vm, bi0, blm->blm_clones[thread_index], n_clones, VLIB_BUFFER_CLONE_HEAD_SIZE);

                if (num_cloned != n_clones) {
                    vec_set_len(blm->blm_clones[thread_index], num_cloned);
                    vlib_node_increment_counter(vm, node->node_index, BIER_LOOKUP_ERROR_BUFFER_ALLOCATION_FAILURE, 1);
                }
                ...
                for (clone = 0; clone < num_cloned; clone++) {
                    ...
                    vnet_buffer(c0)->ip.adj_index[VLIB_TX] = blm->blm_fmasks[thread_index][clone];
    ...
    vlib_node_increment_counter(vm, bier_lookup_node.index, BIER_LOOKUP_ERROR_NONE, from_frame->n_vectors);
...
static u8 *format_bier_lookup_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "BIER: next [%d], tbl:%d BFM:%d", t->next_index, t->bt_index, t->bfm_index);
    return s;
}
...
clib_error_t *bier_lookup_module_init(vlib_main_t *vm)
{
    ...
    for (thread_index = 0; thread_index <= vlib_num_workers(); thread_index++) {
        /*
