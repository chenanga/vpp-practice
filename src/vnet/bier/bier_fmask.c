...
static inline index_t bier_fmask_get_index(const bier_fmask_t *bfm)
{
...
{
    bits->bfmb_refs = clib_mem_alloc(sizeof(bits->bfmb_refs[0]) * bier_hdr_len_id_to_num_bits(hlid));
    clib_memset(bits->bfmb_refs, 0, (sizeof(bits->bfmb_refs[0]) * bier_hdr_len_id_to_num_bits(hlid)));

    bits->bfmb_input_reset_string.bbs_len = bier_hdr_len_id_to_num_buckets(hlid);

    /*
     * The buckets are accessed in the switch path
     */
    bits->bfmb_input_reset_string.bbs_buckets =
        clib_mem_alloc_aligned(sizeof(bits->bfmb_input_reset_string.bbs_buckets[0]) * bier_hdr_len_id_to_num_buckets(hlid), CLIB_CACHE_LINE_BYTES);
    clib_memset(bits->bfmb_input_reset_string.bbs_buckets,
                0,
                sizeof(bits->bfmb_input_reset_string.bbs_buckets[0]) * bier_hdr_len_id_to_num_buckets(hlid));
}
...
static void bier_fmask_stack(bier_fmask_t *bfm)
{
    ...
    fib_path_list_contribute_forwarding(bfm->bfm_pl, fct, FIB_PATH_LIST_FWD_FLAG_COLLAPSE, &via_dpo);
    ...
    dpo_stack(DPO_BIER_FMASK, DPO_PROTO_BIER, &bfm->bfm_dpo, &via_dpo);
    dpo_reset(&via_dpo);
}
...
void bier_fmask_contribute_forwarding(index_t bfmi, dpo_id_t *dpo)
{
    ...
    if (bfm->bfm_flags & BIER_FMASK_FLAG_FORWARDING) {
        dpo_set(dpo, DPO_BIER_FMASK, DPO_PROTO_BIER, bfmi);
    }
    else {
...
{
    return (fib_node_child_add(FIB_NODE_TYPE_BIER_FMASK, bfmi, child_type, child_index));
};
...
void bier_fmask_child_remove(fib_node_index_t bfmi, u32 sibling_index)
{
    ...
    fib_node_child_remove(FIB_NODE_TYPE_BIER_FMASK, bfmi, sibling_index);
}
    ...
    *bfm->bfm_id = *fmid;
    ...
    if (!(bfm->bfm_flags & BIER_FMASK_FLAG_DISP)) {
        if (NULL != rpath->frp_label_stack) {
             ...
             */
            id = bier_bift_id_encode(btid->bti_set, btid->bti_sub_domain, btid->bti_hdr_len);
    ...
    bfm->bfm_pl      = fib_path_list_create((FIB_PATH_LIST_FLAG_SHARED | FIB_PATH_LIST_FLAG_NO_URPF), rpaths);
    bfm->bfm_sibling = fib_path_list_child_add(bfm->bfm_pl, FIB_NODE_TYPE_BIER_FMASK, bier_fmask_get_index(bfm));
    vec_free(rpaths);
...
static void bier_fmask_destroy(bier_fmask_t *bfm)
{
    ...
    fib_path_list_child_remove(bfm->bfm_pl, bfm->bfm_sibling);
    dpo_reset(&bfm->bfm_dpo);
...
u8 *format_bier_fmask(u8 *s, va_list *ap)
{
    ...
    s = format(s,
               ...
               &bfm->bfm_bits.bfmb_input_reset_string,
    ...
    if (bfm->bfm_flags & BIER_FMASK_FLAG_MPLS) {
        s = format(s, "  output-label:%U", format_mpls_unicast_label, vnet_mpls_uc_get_label(clib_net_to_host_u32(bfm->bfm_label)));
    }
    else {
        s = format(s, "  output-bfit:[%U]", format_bier_bift_id, vnet_mpls_uc_get_label(clib_net_to_host_u32(bfm->bfm_label)));
    }
    s = format(s, "\n %U%U", format_white_space, indent, format_dpo_id, &bfm->bfm_dpo, indent + 2);
    ...
    *btid = *bier_table_get_id(bfm->bfm_id->bfmi_bti);
    ...
    switch (bfm->bfm_id->bfmi_nh_type) {
        case BIER_NH_UDP:
            ...
            break;
        case BIER_NH_IP:
            memcpy(&rpath->frp_addr, &bfm->bfm_id->bfmi_nh, sizeof(rpath->frp_addr));
            break;
    }
}
...
{
    return ((bier_fmask_t *) (((char *) node) - STRUCT_OFFSET_OF(bier_fmask_t, bfm_node)));
}
...
{
    fib_show_memory_usage("BIER-fmask", pool_elts(bier_fmask_pool), pool_len(bier_fmask_pool), sizeof(bier_fmask_t));
}
...
const static char *const        bier_fmask_mpls_nodes[]         = {"bier-output", NULL};
...
static clib_error_t *bier_fmask_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (INDEX_INVALID == bfmi) {
        pool_foreach (bfm, bier_fmask_pool) {
            vlib_cli_output(vm, "[@%d] %U", bier_fmask_get_index(bfm), format_bier_fmask, bier_fmask_get_index(bfm), 0);
        }
    }
    else {
