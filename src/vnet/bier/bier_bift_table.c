...
void bier_bift_table_entry_add(bier_bift_id_t id, const dpo_id_t *dpo)
{
    if (NULL == bier_bift_table) {
         ...
         */
        bier_bift_table = clib_mem_alloc_aligned(sizeof(*bier_bift_table), CLIB_CACHE_LINE_BYTES);
        clib_memset(bier_bift_table, 0, sizeof(*bier_bift_table));

        for (ii = 0; ii < BIER_BIFT_N_ENTRIES; ii++) {
            dpo_stack_from_node(bier_bift_input_node.index, &bier_bift_table->bblt_dpos[ii], drop_dpo_get(DPO_PROTO_BIER));
        }
         ...
         * UDP port
         */
        udp_register_dst_port(vlib_get_main(), UDP_DST_PORT_BIER, bier_bift_input_node.index, 0);
        udp_register_dst_port(vlib_get_main(), UDP_DST_PORT_BIER, bier_bift_input_node.index, 1);
    }

    dpo_stack_from_node(bier_bift_input_node.index, &bier_bift_table->bblt_dpos[id], dpo);
...
void bier_bift_table_entry_remove(bier_bift_id_t id)
{
    ...
    if (0 == bier_bift_table->bblt_n_entries) {
        udp_unregister_dst_port(vlib_get_main(), UDP_DST_PORT_BIER, 0);
        udp_unregister_dst_port(vlib_get_main(), UDP_DST_PORT_BIER, 1);
...
static uword bier_bift_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    vlib_node_increment_counter(vm, bier_bift_input_node.index, BIER_INPUT_ERROR_PKTS_VALID, from_frame->n_vectors);
...
static u8 *format_bier_bift_input_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "BIFT-ID:[%U]", format_bier_bift_id, vnet_mpls_uc_get_label(t->bift_id));
    return s;
}
    ...
    if (~0 == set) {
        ...
        for (ii = 0; ii < BIER_BIFT_N_ENTRIES; ii++) {
            if (!dpo_is_drop(&bier_bift_table->bblt_dpos[ii])) {
                ...
                vlib_cli_output(vm, "set: %d, sub-domain:%d, BSL:%U", set, sub_domain, format_bier_hdr_len_id, bsl);
                vlib_cli_output(vm, "  %U", format_dpo_id, &bier_bift_table->bblt_dpos[ii], 0);
            }
        }
    }
    else {
        ...
        id = bier_bift_id_encode(set, sub_domain, bier_hdr_bit_len_to_id(hdr_len));

        if (!dpo_is_drop(&bier_bift_table->bblt_dpos[id])) {
            vlib_cli_output(vm, "set: %d, sub-domain:%d, BSL:%U", set, sub_domain, format_bier_hdr_len_id, hdr_len);
            vlib_cli_output(vm, "  %U", format_dpo_id, &bier_bift_table->bblt_dpos[id], 0);
        }
    }
done:
    return (error);
