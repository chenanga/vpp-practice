...
static void bier_disp_entry_restack(bier_disp_entry_t *bde, bier_hdr_proto_id_t pproto)
{
    ...
    if (FIB_NODE_INDEX_INVALID == pli) {
        dpo_copy(&via_dpo, drop_dpo_get(bier_hdr_proto_to_dpo(pproto)));
    }
    else {
        fib_path_list_contribute_forwarding(pli,
                                            fib_forw_chain_type_from_dpo_proto(bier_hdr_proto_to_dpo(pproto)),
    ...
    dpo_stack(DPO_BIER_DISP_ENTRY, DPO_PROTO_BIER, &bde->bde_fwd[pproto].bde_dpo, &via_dpo);
}
...
void bier_disp_entry_path_add(index_t bdei, bier_hdr_proto_id_t pproto, const fib_route_path_t *rpaths)
{
     ...
     */
    if (FIB_NODE_INDEX_INVALID == *pli) {
        *pli = fib_path_list_create((FIB_PATH_LIST_FLAG_SHARED | FIB_PATH_LIST_FLAG_NO_URPF), rpaths);
    }
    else {
        *pli = fib_path_list_copy_and_path_add(old_pli, (FIB_PATH_LIST_FLAG_SHARED | FIB_PATH_LIST_FLAG_NO_URPF), rpaths);
    }
...
int bier_disp_entry_path_remove(index_t bdei, bier_hdr_proto_id_t pproto, const fib_route_path_t *rpaths)
{
     ...
     */
    if (FIB_NODE_INDEX_INVALID != *pli) {
        *pli = fib_path_list_copy_and_path_remove(old_pli, (FIB_PATH_LIST_FLAG_SHARED | FIB_PATH_LIST_FLAG_NO_URPF), rpaths);
...
u8 *format_bier_disp_entry(u8 *s, va_list *args)
{
    ...
    FOR_EACH_BIER_HDR_PROTO(pproto)
    {
        if (INDEX_INVALID != bde->bde_pl[pproto]) {
            s = format(s, "\n%U%U\n", format_white_space, indent + 2, format_bier_hdr_proto, pproto);
            s = format(s, "%U", format_fib_path_list, bde->bde_pl[pproto], indent + 4);

            if (flags & BIER_SHOW_DETAIL) {
                s = format(s, "\n%UForwarding:", format_white_space, indent + 4);
                s = format(s, "\n%Urpf-id:%d", format_white_space, indent + 6, bde->bde_fwd[pproto].bde_rpf_id);
                s = format(s, "\n%U%U", format_white_space, indent + 6, format_dpo_id, &bde->bde_fwd[pproto].bde_dpo, indent + 2);
            }
        }
    }
    return (s);
...
{
    fib_show_memory_usage("BIER disposition", pool_elts(bier_disp_entry_pool), pool_len(bier_disp_entry_pool), sizeof(bier_disp_entry_t));
}
...
{
    dpo_register(DPO_BIER_DISP_ENTRY, &bier_disp_entry_vft, bier_disp_entry_nodes);
...
static clib_error_t *show_bier_disp_entry(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (INDEX_INVALID == bdei) {
        ...
        if (pool_is_free_index(bier_disp_entry_pool, bdei)) {
            ...
            vlib_cli_output(vm, "%U", format_bier_disp_entry, bdei, 1, BIER_SHOW_DETAIL);
        }
    }
    return (NULL);
}
