...
static void bier_entry_table_ecmp_walk_add_fmask(index_t btei, void *arg)
{
    ...
    if (FIB_NODE_INDEX_INVALID != be->be_path_list) {
        ...
        fib_path_list_contribute_forwarding(be->be_path_list, FIB_FORW_CHAIN_TYPE_BIER, FIB_PATH_LIST_FWD_FLAG_COLLAPSE, &dpo);
        ...
        if (dpo.dpoi_type == DPO_LOAD_BALANCE) {
            ...
            choice = load_balance_get_bucket_i(lb, btid->bti_ecmp & (lb->lb_n_buckets_minus_1));
        }
        else {
            choice = &dpo;
        }
        ...
        if (choice->dpoi_type == DPO_BIER_FMASK) {
            bier_table_ecmp_set_fmask(btei, be->be_bp, choice->dpoi_index);
        }
        else {
            /*
...
void bier_entry_delete(index_t bei)
{
     ...
     */
    if (FIB_NODE_INDEX_INVALID != be->be_path_list) {
        fib_path_list_walk(be->be_path_list, bier_entry_unlink_walk, be);
        fib_path_list_child_remove(be->be_path_list, be->be_sibling_index);

        be->be_path_list = FIB_NODE_INDEX_INVALID;
        bier_table_ecmp_walk(be->be_bti, bier_entry_table_ecmp_walk_add_fmask, be);
    }
...
void bier_entry_path_add(index_t bei, const fib_route_path_t *rpaths)
{
    ...
    if (FIB_NODE_INDEX_INVALID == be->be_path_list) {
        ...
        be->be_path_list     = fib_path_list_create((FIB_PATH_LIST_FLAG_SHARED | FIB_PATH_LIST_FLAG_NO_URPF), rpaths);
        be->be_sibling_index = fib_path_list_child_add(be->be_path_list, FIB_NODE_TYPE_BIER_ENTRY, bier_entry_get_index(be));
    }
    else {
        old_pl_index = be->be_path_list;

        be->be_path_list = fib_path_list_copy_and_path_add(old_pl_index, (FIB_PATH_LIST_FLAG_SHARED | FIB_PATH_LIST_FLAG_NO_URPF), rpaths);

        fib_path_list_child_remove(old_pl_index, be->be_sibling_index);
        be->be_sibling_index = fib_path_list_child_add(be->be_path_list, FIB_NODE_TYPE_BIER_ENTRY, bier_entry_get_index(be));
    }
    /*
     ...
     */
    fib_path_list_walk(be->be_path_list, bier_entry_link_walk, be);
    if (FIB_NODE_INDEX_INVALID != old_pl_index) {
        fib_path_list_walk(old_pl_index, bier_entry_unlink_walk, be);
    }
...
int bier_entry_path_remove(index_t bei, const fib_route_path_t *rpaths)
{
    ...
    be->be_path_list = fib_path_list_copy_and_path_remove(old_pl_index, (FIB_PATH_LIST_FLAG_SHARED | FIB_PATH_LIST_FLAG_NO_URPF), rpaths);
...
u8 *format_bier_entry(u8 *s, va_list *ap)
{
    ...
    if (flags & BIER_SHOW_DETAIL) {
        ...
        s = format(s, "  %U", format_dpo_id, &dpo, 2);
        s = format(s, "\n");
    }
...
{
    return ((bier_entry_t *) (((char *) node) - STRUCT_OFFSET_OF(bier_entry_t, be_node)));
}
