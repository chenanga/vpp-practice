...
static inline index_t bier_table_get_index(const bier_table_t *bt)
{
...
static u32 bier_table_mk_key(const bier_table_id_t *id)
{
     ...
     * for later
     */
    u32 key = ((id->bti_sub_domain << 24) | (id->bti_set << 16) | (id->bti_ecmp << 8) | (id->bti_hdr_len << 4) | (id->bti_type));
...
static void bier_table_init(bier_table_t *bt, const bier_table_id_t *id, mpls_label_t ll)
{
    ...
    if (bier_table_is_main(bt)) {
        vec_validate_init_empty_aligned(bt->bt_entries, num_entries, INDEX_INVALID, CLIB_CACHE_LINE_BYTES);
    }
    else {
        vec_validate_init_empty_aligned(bt->bt_fmasks, num_entries, INDEX_INVALID, CLIB_CACHE_LINE_BYTES);
    }
}
...
static void bier_table_rm_bift(bier_table_t *bt)
{
    ...
    bier_bift_table_entry_remove(bier_bift_id_encode(bt->bt_id.bti_set, bt->bt_id.bti_sub_domain, bt->bt_id.bti_hdr_len));
}
...
static void bier_table_mk_bift(bier_table_t *bt)
{
    ...
    bier_bift_table_entry_add(bier_bift_id_encode(bt->bt_id.bti_set, bt->bt_id.bti_sub_domain, bt->bt_id.bti_hdr_len), &dpo);
...
static void bier_table_rm_lfib(bier_table_t *bt)
{
    if (FIB_NODE_INDEX_INVALID != bt->bt_lfei) {
        fib_table_entry_delete_index(bt->bt_lfei, FIB_SOURCE_BIER);
        fib_table_unlock(MPLS_FIB_DEFAULT_TABLE_ID, FIB_PROTOCOL_MPLS, FIB_SOURCE_BIER);
    }
    bt->bt_lfei = FIB_NODE_INDEX_INVALID;
}
...
static void bier_table_destroy(bier_table_t *bt)
{
    ...
    hash_unset(bier_tables_by_key, bier_table_mk_key(&bt->bt_id));
...
static void bier_table_mk_lfib(bier_table_t *bt)
{
    ...
    if (MPLS_LABEL_INVALID != bt->bt_ll) {
        ...
        fib_table_find_or_create_and_lock(FIB_PROTOCOL_MPLS, MPLS_FIB_DEFAULT_TABLE_ID, FIB_SOURCE_BIER);

        /*
         * stack the entry on the forwarding chain produced by the
         * path-list via the ECMP tables.
         */
        fib_path_list_contribute_forwarding(bt->bt_pl, FIB_FORW_CHAIN_TYPE_BIER, FIB_PATH_LIST_FWD_FLAG_COLLAPSE, &dpo);

        mpls_fib_index = fib_table_find(FIB_PROTOCOL_MPLS, MPLS_FIB_DEFAULT_TABLE_ID);
        bt->bt_lfei    = fib_table_entry_special_dpo_add(mpls_fib_index, &pfx, FIB_SOURCE_BIER, FIB_ENTRY_FLAG_EXCLUSIVE, &dpo);
        dpo_reset(&dpo);
    }
}
...
{
    fib_show_memory_usage("BIER-table", pool_elts(bier_table_pool), pool_len(bier_table_pool), sizeof(bier_table_t));
}
...
const static char *const        bier_table_mpls_nodes[]         = {"bier-input", NULL};
...
void bier_table_ecmp_walk(index_t bti, bier_table_ecmp_walk_fn_t fn, void *ctx)
{
    ...
    if (FIB_NODE_INDEX_INVALID != bt->bt_pl) {
        fib_path_list_walk(bt->bt_pl, bier_table_ecmp_walk_path_list, &ewc);
    }
}
...
u8 *format_bier_table_entry(u8 *s, va_list *ap)
{
    ...
    if (bier_table_is_main(bt)) {
        ...
        if (INDEX_INVALID != bei) {
            s = format(s, "%U", format_bier_entry, bei, BIER_SHOW_DETAIL);
        }
    }
    else {
        index_t bfmi;
        ...
        if (INDEX_INVALID != bfmi) {
            s = format(s, "%U", format_bier_fmask, bfmi, BIER_SHOW_DETAIL);
        }
    }
    return (s);
}
...
u8 *format_bier_table(u8 *s, va_list *ap)
{
    ...
    s = format(s, "[@%d] bier-table:[%U local-label:%U", bti, format_bier_table_id, &bt->bt_id, format_mpls_unicast_label, bt->bt_ll);
        ...
        if (bier_table_is_main(bt)) {
            ...
            vec_foreach_index (ii, bt->bt_fmasks) {
                if (INDEX_INVALID != bt->bt_fmasks[ii]) {
                    s = format(s, "\n bp:%d\n %U", ii, format_bier_fmask, bt->bt_fmasks[ii], 2);
                }
            }
        }
    }
