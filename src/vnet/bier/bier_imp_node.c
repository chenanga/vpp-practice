...
always_inline uword
bier_imp_dpo_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, fib_protocol_t fproto, bier_hdr_proto_id_t bproto)
{
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (FIB_PROTOCOL_IP4 == fproto) {
                ...
                if (0 == vnet_buffer(b0)->ip.flow_hash) {
                    vnet_buffer(b0)->ip.flow_hash = ip4_compute_flow_hash(ip0, IP_FLOW_HASH_DEFAULT);
                }
            }
            if (FIB_PROTOCOL_IP6 == fproto) {
                 ...
                 */
                if (0 == vnet_buffer(b0)->ip.flow_hash) {
                    vnet_buffer(b0)->ip.flow_hash = ip6_compute_flow_hash(ip0, IP_FLOW_HASH_DEFAULT);
                }
            }

            /* Paint the BIER header */
            vlib_buffer_advance(b0, -(sizeof(bier_hdr_t) + bier_hdr_len_id_to_num_bytes(bimp0->bi_tbl.bti_hdr_len)));
            ...
            if (PREDICT_FALSE(BIER_RX_ITF == vnet_buffer(b0)->ip.adj_index[VLIB_RX])) {
                ...
                clib_memcpy_fast(hdr0, &bimp0->bi_hdr, (sizeof(bier_hdr_t) + bier_hdr_len_id_to_num_bytes(bimp0->bi_tbl.bti_hdr_len)));
                /*
                 * Fixup the entropy and protocol, both of which have a
                 * zero value post the paint job
                 */
                hdr0->bh_oam_dscp_proto |= clib_host_to_net_u16(bproto << BIER_HDR_PROTO_FIELD_SHIFT);
                hdr0->bh_first_word |=
                    clib_host_to_net_u32((vnet_buffer(b0)->ip.flow_hash & BIER_HDR_ENTROPY_FIELD_MASK) << BIER_HDR_ENTROPY_FIELD_SHIFT);
...
VLIB_NODE_FN(bier_imp_ip4_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (bier_imp_dpo_inline(vm, node, frame, FIB_PROTOCOL_IP4, BIER_HDR_PROTO_IPV4));
}

VLIB_REGISTER_NODE(bier_imp_ip4_node) = {.name        = "bier-imp-ip4",
...
VLIB_NODE_FN(bier_imp_ip6_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (bier_imp_dpo_inline(vm, node, frame, FIB_PROTOCOL_IP6, BIER_HDR_PROTO_IPV6));
}

VLIB_REGISTER_NODE(bier_imp_ip6_node) = {.name        = "bier-imp-ip6",
                                         .vector_size = sizeof(u32),
