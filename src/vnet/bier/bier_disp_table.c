...
index_t bier_disp_table_add_or_lock(u32 table_id)
{
    ...
    if (INDEX_INVALID == bdti) {
        pool_get_aligned(bier_disp_table_pool, bdt, CLIB_CACHE_LINE_BYTES);

        bdt->bdt_table_id = table_id;
        bdt->bdt_locks    = 0;

        hash_set(bier_disp_table_id_to_index, table_id, bier_disp_table_get_index(bdt));
...
{
    dpo_set(dpo, DPO_BIER_DISP_TABLE, DPO_PROTO_BIER, bdti);
}
...
u8 *format_bier_disp_table(u8 *s, va_list *ap)
{
    ...
    s = format(s, "bier-disp-table:[%d]; table-id:%d locks:%d", bdti, bdt->bdt_table_id, bdt->bdt_locks);
    ...
    if (flags & BIER_SHOW_DETAIL) {
        ...
        for (ii = 0; ii < BIER_BP_MAX; ii++) {
            if (INDEX_INVALID != bdt->bdt_db[ii]) {
                u16 src = ii;
                s       = format(s, "\n%Usrc:%d", format_white_space, indent + 1, clib_host_to_net_u16(src));
                s       = format(s, "\n%U", format_bier_disp_entry, bdt->bdt_db[ii], indent + 4, BIER_SHOW_BRIEF);
            }
        }
    }
    return (s);
...
static u8 *format_bier_disp_table_dpo(u8 *s, va_list *ap)
{
    ...
    return (format(s, "%U", format_bier_disp_table, bdti, indent, BIER_SHOW_BRIEF));
}
...
{
    fib_show_memory_usage("BIER disposition table", pool_elts(bier_disp_table_pool), pool_len(bier_disp_table_pool), sizeof(bier_disp_table_t));
}
...
const static char *const bier_disp_table_bier_nodes[] = {"bier-disp-lookup", NULL};
...
{
    dpo_register(DPO_BIER_DISP_TABLE, &bier_disp_table_dpo_vft, bier_disp_table_nodes);
...
static clib_error_t *show_bier_disp_table(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (INDEX_INVALID == bdti) {
        pool_foreach (bdt, bier_disp_table_pool) {
            vlib_cli_output(vm, "%U", format_bier_disp_table, bier_disp_table_get_index(bdt), 0, BIER_SHOW_BRIEF);
        }
    }
    else {
        if (pool_is_free_index(bier_disp_table_pool, bdti)) {
            ...
            vlib_cli_output(vm, "%U", format_bier_disp_table, bdti, 0, BIER_SHOW_DETAIL);
        }
    }
    return (NULL);
}
