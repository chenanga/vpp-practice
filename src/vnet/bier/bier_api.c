...
static void send_bier_route_details(const bier_table_t *bt, const bier_entry_t *be, void *args)
{
    ...
    fib_path_list_walk_w_ext(be->be_path_list, NULL, fib_path_encode, &path_ctx);
...
static void vl_api_bier_imp_add_t_handler(vl_api_bier_imp_add_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_BIER_IMP_ADD_REPLY, ({ rmp->bi_index = ntohl(bii); }));
}
...
static void send_bier_imp_details(vl_api_registration_t *reg, u32 context, const bier_imp_t *bi)
{
    ...
    n_bytes = bier_hdr_len_id_to_num_bytes(bier_hdr_get_len_id(&copy));
...
static void send_bier_disp_table_details(vl_api_registration_t *reg, u32 context, const bier_disp_table_t *bdt)
{
    ...
    if (!mp) return;
    clib_memset(mp, 0, sizeof(*mp));
    mp->_vl_msg_id = ntohs(REPLY_MSG_ID_BASE + VL_API_BIER_DISP_TABLE_DETAILS);
    mp->context    = context;
...
static void vl_api_bier_disp_entry_add_del_t_handler(vl_api_bier_disp_entry_add_del_t *mp)
{
    ...
    vec_foreach_index (ii, brps) {
        ...
        if (FIB_API_PATH_NH_PROTO_IP4 == mp->bde_paths[ii].proto) {
            clib_memcpy(&brp->frp_addr.ip4, &mp->bde_paths[ii].nh.address.ip4, sizeof(brp->frp_addr.ip4));
        }
        else if (FIB_API_PATH_NH_PROTO_IP6 == mp->bde_paths[ii].proto) {
            clib_memcpy(&brp->frp_addr.ip6, &mp->bde_paths[ii].nh.address.ip6, sizeof(brp->frp_addr.ip6));
        }
        if (ip46_address_is_zero(&brp->frp_addr)) {
            ...
            switch (mp->bde_payload_proto) {
                ...
                case BIER_HDR_PROTO_IPV6: {
                    ...
                    fproto = (mp->bde_payload_proto == BIER_HDR_PROTO_IPV4 ? FIB_PROTOCOL_IP4 : FIB_PROTOCOL_IP6);

                    if (brp->frp_flags & FIB_ROUTE_PATH_RPF_ID) {
                        fti = mfib_table_find(fproto, ntohl(mp->bde_paths[ii].table_id));
                    }
                    else {
                        fti = fib_table_find(fproto, ntohl(mp->bde_paths[ii].table_id));
                    }
    ...
    if (mp->bde_is_add) {
        bier_disp_table_entry_path_add(table_id, bp, mp->bde_payload_proto, brps);
    }
    else {
        bier_disp_table_entry_path_remove(table_id, bp, mp->bde_payload_proto, brps);
    }
...
static void send_bier_disp_entry_details(const bier_disp_table_t *bdt, const bier_disp_entry_t *bde, u16 bp, void *args)
{
    ...
    FOR_EACH_BIER_HDR_PROTO(pproto)
    {
        ...
        if (INDEX_INVALID != pl) {
            ...
            clib_memset(mp, 0, m_size);
            mp->_vl_msg_id = ntohs(REPLY_MSG_ID_BASE + VL_API_BIER_DISP_ENTRY_DETAILS);
            mp->context    = ctx->context;
...
static void vl_api_bier_disp_entry_dump_t_handler(vl_api_bier_disp_entry_dump_t *mp)
{
    ...
    bier_disp_table_walk(ntohl(mp->bde_tbl_id), send_bier_disp_entry_details, &ctx);
}
