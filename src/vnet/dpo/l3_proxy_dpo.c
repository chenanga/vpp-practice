...
static u8 *format_l3_proxy_dpo(u8 *s, va_list *ap)
{
    ...
    if (~0 != l3p->l3p_sw_if_index) {
        return (format(s, "dpo-l3_proxy: %U", format_vnet_sw_if_index_name, vnm, l3p->l3p_sw_if_index));
    }
    else {
...
{
    fib_show_memory_usage("L3 Proxy", pool_elts(l3_proxy_dpo_pool), pool_len(l3_proxy_dpo_pool), sizeof(l3_proxy_dpo_t));
}
