...
always_inline uword load_balance_map_db_hash_key_from_index(uword index)
{
    return 1 + 2 * index;
}

always_inline uword load_balance_map_db_hash_key_is_index(uword key)
{
    return key & 1;
}

always_inline uword load_balance_map_db_hash_key_2_index(uword key)
{
...
static void load_balance_map_db_insert(load_balance_map_t *lbm)
{
     ...
     */
    hash_set(load_balance_map_db, load_balance_map_db_hash_key_from_index(load_balance_map_get_index(lbm)), load_balance_map_get_index(lbm));
    ...
    vec_foreach (lbmp, lbm->lbm_paths) {
        ...
        lbmp->lbmp_sibling = fib_node_list_push_front(list, 0, FIB_NODE_TYPE_FIRST, load_balance_map_get_index(lbm));
    }
...
static void load_balance_map_db_remove(load_balance_map_t *lbm)
{
    ...
    hash_unset(load_balance_map_db, load_balance_map_db_hash_key_from_index(load_balance_map_get_index(lbm)));
...
static void load_balance_map_fill(load_balance_map_t *lbm)
{
     ...
     */
    if (jj == n_buckets) {
        memcpy(lbm->lbm_buckets, tmp_buckets, sizeof(lbm->lbm_buckets[0]) * n_buckets);
    }
    else {
        /*
...
static load_balance_map_t *load_balance_map_init(load_balance_map_t *lbm, u32 n_buckets, u32 sum_of_weights)
{
    ...
    load_balance_map_logger = vlib_log_register_class("dpo", "load-balance-map");
...
static walk_rc_t load_balance_map_path_state_change_walk(fib_node_ptr_t *fptr, void *ctx)
{
    load_balance_map_t *lbm;
...
{
    fib_show_memory_usage("Load-Balance Map", pool_elts(load_balance_map_pool), pool_len(load_balance_map_pool), sizeof(load_balance_map_t));
}
...
static clib_error_t *load_balance_map_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (INDEX_INVALID != lbmi) {
        ...
        pool_foreach (lbm, load_balance_map_pool) {
            vlib_cli_output(vm, "%U", format_load_balance_map, load_balance_map_get_index(lbm), 0);
        }
    }
