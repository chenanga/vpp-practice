...
void ip_null_dpo_add_and_lock(dpo_proto_t proto, ip_null_dpo_action_t action, dpo_id_t *dpo)
{
    ...
    ASSERT((proto == DPO_PROTO_IP4) || (proto == DPO_PROTO_IP6));
...
always_inline const ip_null_dpo_t *ip_null_dpo_get(index_t indi)
{
...
static u8 *format_ip_null_dpo(u8 *s, va_list *ap)
{
    ...
    return (format(s, "%U-null action:%s", format_dpo_proto, proto, ip_null_action_strings[ind->ind_action]));
}
...
always_inline uword ip_null_dpo_switch(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_ip4)
{
    ...
    if (time_now - time_last_seed_change > 1e-1) {
        uword i;
        u32  *r = clib_random_buffer_get_data(&vm->random_buffer, sizeof(hash_seeds));
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(!drop0)) {
                if (is_ip4) {
                    ...
                    if (IP_NULL_ACTION_SEND_ICMP_UNREACH == ind0->ind_action) {
                        ...
                        icmp4_error_set_vnet_buffer(p0, ICMP4_destination_unreachable, ICMP4_destination_unreachable_destination_unreachable_host, 0);
                    }
                    ...
                    if (IP_NULL_ACTION_SEND_ICMP_UNREACH == ind0->ind_action) {
                        next0 = IP_NULL_NEXT_ICMP;
                        icmp6_error_set_vnet_buffer(p0, ICMP6_destination_unreachable, ICMP6_destination_unreachable_no_route_to_destination, 0);
                    }
                    else if (IP_NULL_ACTION_SEND_ICMP_PROHIBIT == ind0->ind_action) {
                        next0 = IP_NULL_NEXT_ICMP;
                        icmp6_error_set_vnet_buffer(p0,
            ...
            if (PREDICT_FALSE(p0->flags & VLIB_BUFFER_IS_TRACED)) {
                ip_null_dpo_trace_t *tr = vlib_add_trace(vm, node, p0, sizeof(*tr));
                tr->ind_index           = indi0;
            }
