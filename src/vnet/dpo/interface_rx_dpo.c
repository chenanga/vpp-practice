...
static inline interface_rx_dpo_t *interface_rx_dpo_get_from_dpo(const dpo_id_t *dpo)
{
...
static inline index_t interface_rx_dpo_get_index(interface_rx_dpo_t *ido)
{
...
static void interface_rx_dpo_unlock(dpo_id_t *dpo)
{
    ...
    if (0 == ido->ido_locks) {
        interface_rx_dpo_db[ido->ido_proto][ido->ido_sw_if_index] = INDEX_INVALID;
...
void interface_rx_dpo_add_or_lock(dpo_proto_t proto, u32 sw_if_index, dpo_id_t *dpo)
{
    ...
    vec_validate_init_empty(interface_rx_dpo_db[proto], sw_if_index, INDEX_INVALID);
    ...
    if (INDEX_INVALID == interface_rx_dpo_db[proto][sw_if_index]) {
        ...
        interface_rx_dpo_db[proto][sw_if_index] = interface_rx_dpo_get_index(ido);
    }
    else {
...
static u8 *format_interface_rx_dpo(u8 *s, va_list *ap)
{
    ...
    return format(s, "%U-rx-dpo: %U", format_vnet_sw_if_index_name, vnm, ido->ido_sw_if_index, format_dpo_proto, ido->ido_proto);
}

static void interface_rx_dpo_mem_show(void)
{
    fib_show_memory_usage("Interface", pool_elts(interface_rx_dpo_pool), pool_len(interface_rx_dpo_pool), sizeof(interface_rx_dpo_t));
}
...
{
    dpo_register(DPO_INTERFACE_RX, &interface_rx_dpo_vft, interface_rx_dpo_nodes);
}
#endif /* CLIB_MARCH_VARIANT */
...
always_inline uword interface_rx_dpo_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, u8 is_l2)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (is_l2) vnet_update_l2_len(b0);
...
VLIB_NODE_FN(interface_rx_dpo_ip4_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return (interface_rx_dpo_inline(vm, node, from_frame, 0));
}

VLIB_NODE_FN(interface_rx_dpo_ip6_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return (interface_rx_dpo_inline(vm, node, from_frame, 0));
}

VLIB_NODE_FN(interface_rx_dpo_l2_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
