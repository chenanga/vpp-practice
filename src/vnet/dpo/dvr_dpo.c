...
static inline dvr_dpo_t *dvr_dpo_get_from_dpo(const dpo_id_t *dpo)
{
...
static inline index_t dvr_dpo_get_index(dvr_dpo_t *dd)
{
...
static void dvr_dpo_unlock(dpo_id_t *dpo)
{
    ...
    if (0 == dd->dd_locks) {
        if (DPO_PROTO_IP4 == dd->dd_proto) {
            vnet_feature_enable_disable("ip4-output", "ip4-dvr-reinject", dd->dd_sw_if_index, 0, 0, 0);
        }
        else {
            vnet_feature_enable_disable("ip6-output", "ip6-dvr-reinject", dd->dd_sw_if_index, 0, 0, 0);
        }
...
void dvr_dpo_add_or_lock(u32 sw_if_index, dpo_proto_t dproto, dpo_id_t *dpo)
{
    ...
    vec_validate_init_empty(dvr_dpo_db[dproto], sw_if_index, INDEX_INVALID);
    ...
    if (INDEX_INVALID == dvr_dpo_db[dproto][sw_if_index]) {
        ...
        if (l2_input_is_bridge(config) || l2_input_is_xconnect(config)) {
        ...
        if (DPO_PROTO_IP4 == dproto)
            vnet_feature_enable_disable("ip4-output", "ip4-dvr-reinject", dd->dd_sw_if_index, 1, 0, 0);
        else if (DPO_PROTO_IP6 == dproto)
            vnet_feature_enable_disable("ip6-output", "ip6-dvr-reinject", dd->dd_sw_if_index, 1, 0, 0);
        else
            ASSERT(0);
    }
    else {
...
{
    fib_show_memory_usage("DVR", pool_elts(dvr_dpo_pool), pool_len(dvr_dpo_pool), sizeof(dvr_dpo_t));
}
...
{
    dpo_register(DPO_DVR, &dvr_dpo_vft, dvr_dpo_nodes);
}
#endif /* CLIB_MARCH_VARIANT */
...
always_inline uword dvr_dpo_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, u8 is_ip6)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next > 2) {
            ...
            vnet_buffer(b0)->l2.l2_len = vnet_buffer(b0)->ip.save_rewrite_length = len0;
            vnet_buffer(b1)->l2.l2_len = vnet_buffer(b1)->ip.save_rewrite_length = len1;
            ...
            vnet_feature_arc_start(lm->output_feature_arc_index, dd0->dd_sw_if_index, &next0, b0);
            vnet_feature_arc_start(lm->output_feature_arc_index, dd1->dd_sw_if_index, &next1, b1);
...
static u8 *format_dvr_dpo_trace(u8 *s, va_list *args)
{
    ...
    s                               = format(s, "%U sw_if_index:%d", format_white_space, indent, t->sw_if_index);
    return s;
}

VLIB_NODE_FN(ip4_dvr_dpo_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return (dvr_dpo_inline(vm, node, from_frame, 0));
}

VLIB_NODE_FN(ip6_dvr_dpo_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
always_inline uword dvr_reinject_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    u32 n_left_from, next_index, *from, *to_next;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next > 2) {
            ...
            if (b0->flags & VNET_BUFFER_F_IS_DVR) {
                ddi0  = vnet_buffer(b0)->ip.adj_index[VLIB_TX];
                dd0   = dvr_dpo_get(ddi0);
                next0 = (dd0->dd_reinject == DVR_REINJECT_L2 ? DVR_REINJECT_NEXT_L2 : DVR_REINJECT_NEXT_L3);
            }
            else
            ...
            if (b1->flags & VNET_BUFFER_F_IS_DVR) {
                ddi1  = vnet_buffer(b1)->ip.adj_index[VLIB_TX];
                dd1   = dvr_dpo_get(ddi1);
                next1 = (dd1->dd_reinject == DVR_REINJECT_L2 ? DVR_REINJECT_NEXT_L2 : DVR_REINJECT_NEXT_L3);
            }
            else
...
VLIB_NODE_FN(ip4_dvr_reinject_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return (dvr_reinject_inline(vm, node, from_frame));
}

VLIB_NODE_FN(ip6_dvr_reinject_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return (dvr_reinject_inline(vm, node, from_frame));
}
