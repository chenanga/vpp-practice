...
#define REP_DBG(_rep, _fmt, _args...)                                                                                                    \
    {                                                                                                                                    \
        vlib_log_debug(replicate_logger, "rep:[%U]:" _fmt, format_replicate, replicate_get_index(_rep), REPLICATE_FORMAT_NONE, ##_args); \
    }
...
static inline index_t replicate_get_index(const replicate_t *rep)
{
    return (rep - replicate_pool);
}

static inline dpo_id_t *replicate_get_buckets(replicate_t *rep)
{
...
static replicate_t *replicate_alloc_i(void)
{
    ...
    vlib_validate_combined_counter(&(replicate_main.repm_counters), replicate_get_index(rep));
    vlib_zero_combined_counter(&(replicate_main.repm_counters), replicate_get_index(rep));
...
static replicate_t *replicate_create_i(u32 num_buckets, dpo_proto_t rep_proto)
{
    ...
    if (!REP_HAS_INLINE_BUCKETS(rep)) {
        vec_validate_aligned(rep->rep_buckets, rep->rep_n_buckets - 1, CLIB_CACHE_LINE_BYTES);
    }
...
static inline void replicate_set_bucket_i(replicate_t *rep, u32 bucket, dpo_id_t *buckets, const dpo_id_t *next)
{
...
int replicate_is_drop(const dpo_id_t *dpo)
{
    ...
    if (DPO_REPLICATE != dpo->dpoi_type) return (0);
...
static inline void replicate_set_n_buckets(replicate_t *rep, u32 n_buckets)
{
...
void replicate_multipath_update(const dpo_id_t *dpo, load_balance_path_t *next_hops)
{
    ...
    repi      = dpo->dpoi_index & ~MPLS_IS_REPLICATE;
    rep       = replicate_get(repi);
    nhs       = replicate_multipath_next_hop_fixup(next_hops, rep->rep_proto);
    n_buckets = vec_len(nhs);

    if (n_buckets > REP_MAX_BUCKETS) {
        vlib_log_err(replicate_logger, "Too many paths for replicate, truncating %d -> %d", n_buckets, REP_MAX_BUCKETS);
    ...
    if (0 == rep->rep_n_buckets) {
        ...
        if (!REP_HAS_INLINE_BUCKETS(rep)) vec_validate_aligned(rep->rep_buckets, rep->rep_n_buckets - 1, CLIB_CACHE_LINE_BYTES);

        replicate_fill_buckets(rep, nhs, replicate_get_buckets(rep), n_buckets);
    }
    else {
        /*
        ...
        if (n_buckets == rep->rep_n_buckets) {
             ...
             * update it last.
             */
            if (n_buckets > REP_NUM_INLINE_BUCKETS && rep->rep_n_buckets <= REP_NUM_INLINE_BUCKETS) {
                /*
                ...
                vec_validate_aligned(rep->rep_buckets, n_buckets - 1, CLIB_CACHE_LINE_BYTES);

                replicate_fill_buckets(rep, nhs, rep->rep_buckets, n_buckets);
                CLIB_MEMORY_BARRIER();
                replicate_set_n_buckets(rep, n_buckets);
                ...
                if (n_buckets <= REP_NUM_INLINE_BUCKETS) {
                    ...
                    vec_validate_aligned(new_buckets, n_buckets - 1, CLIB_CACHE_LINE_BYTES);
            ...
            if (n_buckets <= REP_NUM_INLINE_BUCKETS && rep->rep_n_buckets > REP_NUM_INLINE_BUCKETS) {
                 ...
                 */
                replicate_fill_buckets(rep, nhs, rep->rep_buckets_inline, n_buckets);
                CLIB_MEMORY_BARRIER();
                replicate_set_n_buckets(rep, n_buckets);
                CLIB_MEMORY_BARRIER();
                ...
                replicate_fill_buckets(rep, nhs, buckets, n_buckets);
...
index_t replicate_dup(replicate_flags_t flags, index_t repi)
{
    ...
    if (rep->rep_flags == flags || flags & REPLICATE_FLAGS_HAS_LOCAL) {
        /*
        ...
        if (rep->rep_n_buckets == 1) {
            ...
            replicate_set_bucket_i(copy, 0, replicate_get_buckets(copy), drop_dpo_get(rep->rep_proto));
        }
        else {
            dpo_id_t *old_buckets, *copy_buckets;
            u16       bucket, pos;

            copy = replicate_create_i(rep->rep_n_buckets - 1, rep->rep_proto);
            ...
            for (bucket = 0; bucket < rep->rep_n_buckets; bucket++) {
                if (!dpo_is_receive(&old_buckets[bucket])) {
                    replicate_set_bucket_i(copy, pos, copy_buckets, (&old_buckets[bucket]));
                    pos++;
                }
            }
        }
...
{
    fib_show_memory_usage("replicate", pool_elts(replicate_pool), pool_len(replicate_pool), sizeof(replicate_t));
}
...
static clib_error_t *replicate_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (INDEX_INVALID != repi) {
        if (pool_is_free_index(replicate_pool, repi))
            ...
            vlib_cli_output(vm, "%U", format_replicate, repi, REPLICATE_FORMAT_DETAIL);
    }
    else {
        replicate_t *rep;

        pool_foreach (rep, replicate_pool) {
            vlib_cli_output(vm, "%U", format_replicate, replicate_get_index(rep), REPLICATE_FORMAT_NONE);
        }
    }
...
static uword replicate_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            vlib_increment_combined_counter(cm, thread_index, repi0, 1, vlib_buffer_length_in_chain(vm, b0));

            vec_validate(rm->clones[thread_index], rep0->rep_n_buckets - 1);

            num_cloned = vlib_buffer_clone(vm, bi0, rm->clones[thread_index], rep0->rep_n_buckets, VLIB_BUFFER_CLONE_HEAD_SIZE);

            if (num_cloned != rep0->rep_n_buckets) {
                vlib_node_increment_counter(vm, node->node_index, REPLICATE_DPO_ERROR_BUFFER_ALLOCATION_FAILURE, 1);
            }
...
static u8 *format_replicate_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "replicate: %d via %U", t->rep_index, format_dpo_id, &t->dpo, 0);
    return s;
}
