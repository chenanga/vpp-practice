...
void pw_cw_dpo_create(const dpo_id_t *parent, dpo_id_t *dpo)
{
     ...
     */
    dpo_stack(DPO_PW_CW, parent->dpoi_proto, &pwcw->pwcw_parent, parent);

    /*
     * set up the return DPO to refer to this object
     */
    dpo_set(dpo, DPO_PW_CW, parent->dpoi_proto, pw_cw_dpo_get_index(pwcw));
}
...
always_inline uword pw_cw_pop_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
{
    fib_show_memory_usage("PW-CW", pool_elts(pw_cw_dpo_pool), pool_len(pw_cw_dpo_pool), sizeof(pw_cw_dpo_t));
}
