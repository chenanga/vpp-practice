...
static void vl_api_sw_interface_set_flags_t_handler(vl_api_sw_interface_set_flags_t *mp)
{
    ...
    flags = ((ntohl(mp->flags)) & IF_STATUS_API_FLAG_ADMIN_UP) ? VNET_SW_INTERFACE_FLAG_ADMIN_UP : 0;
...
static void vl_api_sw_interface_set_ip_directed_broadcast_t_handler(vl_api_sw_interface_set_ip_directed_broadcast_t *mp)
{
    ...
    vnet_sw_interface_ip_directed_broadcast(vnet_get_main(), sw_if_index, mp->enable);
...
{
    vnet_hw_interface_t *hi        = vnet_get_sup_hw_interface(am->vnet_main, swif->sw_if_index);
    vnet_device_class_t *dev_class = vnet_get_device_class(am->vnet_main, hi->dev_class_index);
    ...
    mp->flags |= (swif->flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP) ? IF_STATUS_API_FLAG_ADMIN_UP : 0;
    mp->flags |= (hi->flags & VNET_HW_INTERFACE_FLAG_LINK_UP) ? IF_STATUS_API_FLAG_LINK_UP : 0;
    ...
    mp->link_duplex        = ntohl(((hi->flags & VNET_HW_INTERFACE_FLAG_DUPLEX_MASK) >> VNET_HW_INTERFACE_FLAG_DUPLEX_SHIFT));
    ...
    strncpy((char *) mp->interface_name, (char *) interface_name, ARRAY_LEN(mp->interface_name) - 1);

    if (dev_class && dev_class->name) strncpy((char *) mp->interface_dev_type, (char *) dev_class->name, ARRAY_LEN(mp->interface_dev_type) - 1);

    /* Send the L2 address for ethernet physical intfcs */
    if (swif->sup_sw_if_index == swif->sw_if_index && hi->hw_class_index == ethernet_hw_interface_class.index) {
        ...
        mp->sub_if_flags          = ntohl(sub->eth.raw_flags & SUB_IF_API_FLAG_MASK_VNET);
    }
    ...
    if (l2vtr_get(am->vlib_main, am->vnet_main, swif->sw_if_index, &vtr_op, &vtr_push_dot1q, &vtr_tag1, &vtr_tag2) != 0) {
        // error - default to disabled
        mp->vtr_op = ntohl(L2_VTR_DISABLED);
        clib_warning("cannot get vlan tag rewrite for sw_if_index %d", swif->sw_if_index);
    }
    else {
    ...
    if (!l2pbb_get(am->vlib_main, am->vnet_main, swif->sw_if_index, &pbb_vtr_op, &outer_tag, &eth_hdr, &b_vlanid, &i_sid)) {
    ...
    if (tag) strncpy((char *) mp->tag, (char *) tag, ARRAY_LEN(mp->tag) - 1);
...
static void vl_api_sw_interface_dump_t_handler(vl_api_sw_interface_dump_t *mp)
{
    ...
    if (!mp->name_filter_valid && sw_if_index != ~0 && sw_if_index != 0) {
        ...
        if (!vnet_sw_if_index_is_api_valid(sw_if_index)) return;

        swif = vec_elt_at_index(im->sw_interfaces, sw_if_index);

        vec_reset_length(name);
        name = format(name, "%U%c", format_vnet_sw_interface_name, am->vnet_main, swif, 0);
    ...
    pool_foreach (swif, im->sw_interfaces) {
        if (!vnet_swif_is_api_visible(swif)) continue;
        vec_reset_length(name);
        name = format(name, "%U%c", format_vnet_sw_interface_name, am->vnet_main, swif, 0);

        if (filter && !strcasestr((char *) name, (char *) filter)) continue;
...
static void vl_api_sw_interface_add_del_address_t_handler(vl_api_sw_interface_add_del_address_t *mp)
{
    ...
    if (mp->del_all)
        ...
        error = ip6_add_del_interface_address(vm, ntohl(mp->sw_if_index), (void *) &address.ip6, mp->prefix.len, is_del);
    else
        error = ip4_add_del_interface_address(vm, ntohl(mp->sw_if_index), (void *) &address.ip4, mp->prefix.len, is_del);
...
void fib_table_bind(fib_protocol_t fproto, u32 sw_if_index, u32 fib_index)
{
    ...
    if (FIB_PROTOCOL_IP6 == fproto) {
        ...
        vec_foreach (cb, ip6_main.table_bind_callbacks)
            cb->function(&ip6_main, cb->function_opaque, sw_if_index, fib_index, ip6_main.fib_index_by_sw_if_index[sw_if_index]);

        /* unlock currently assigned tables */
        if (0 != ip6_main.fib_index_by_sw_if_index[sw_if_index])
            fib_table_unlock(ip6_main.fib_index_by_sw_if_index[sw_if_index], FIB_PROTOCOL_IP6, FIB_SOURCE_INTERFACE);
        ...
        vec_foreach (cb, ip4_main.table_bind_callbacks)
            cb->function(&ip4_main, cb->function_opaque, sw_if_index, fib_index, ip4_main.fib_index_by_sw_if_index[sw_if_index]);

        /* unlock currently assigned tables */
        if (0 != ip4_main.fib_index_by_sw_if_index[sw_if_index])
            fib_table_unlock(ip4_main.fib_index_by_sw_if_index[sw_if_index], FIB_PROTOCOL_IP4, FIB_SOURCE_INTERFACE);

        if (0 != table_id) {
            /* we need to lock the table now it's inuse */
            fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, table_id, FIB_SOURCE_INTERFACE);
        }
...
void mfib_table_bind(fib_protocol_t fproto, u32 sw_if_index, u32 mfib_index)
{
    ...
    if (FIB_PROTOCOL_IP6 == fproto) {
        if (0 != ip6_main.mfib_index_by_sw_if_index[sw_if_index])
            mfib_table_unlock(ip6_main.mfib_index_by_sw_if_index[sw_if_index], FIB_PROTOCOL_IP6, MFIB_SOURCE_INTERFACE);

        if (0 != table_id) {
            /* we need to lock the table now it's inuse */
            mfib_table_lock(mfib_index, FIB_PROTOCOL_IP6, MFIB_SOURCE_INTERFACE);
        }
        ...
        if (0 != ip4_main.mfib_index_by_sw_if_index[sw_if_index])
            mfib_table_unlock(ip4_main.mfib_index_by_sw_if_index[sw_if_index], FIB_PROTOCOL_IP4, MFIB_SOURCE_INTERFACE);

        if (0 != table_id) {
            /* we need to lock the table now it's inuse */
            mfib_index = mfib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, table_id, MFIB_SOURCE_INTERFACE);
        }
...
int ip_table_bind(fib_protocol_t fproto, u32 sw_if_index, u32 table_id)
{
  ...
  ({
    ...
    if (mfib_index != ~0) mfib_table_bind(fproto, sw_if_index, mfib_index);
...
static void send_sw_interface_get_table_reply(vl_api_registration_t *reg, u32 context, int retval, u32 vrf_id)
{
    ...
    clib_memset(mp, 0, sizeof(*mp));
    mp->_vl_msg_id = ntohs(REPLY_MSG_ID_BASE + VL_API_SW_INTERFACE_GET_TABLE_REPLY);
    mp->context    = context;
...
static void vl_api_sw_interface_get_table_t_handler(vl_api_sw_interface_get_table_t *mp)
{
    ...
    if (mp->is_ipv6) fib_proto = FIB_PROTOCOL_IP6;
...
static void vl_api_sw_interface_set_unnumbered_t_handler(vl_api_sw_interface_set_unnumbered_t *mp)
{
    ...
    rv = vnet_sw_interface_update_unnumbered(unnumbered_sw_if_index, sw_if_index, mp->is_add);
done:
...
static void vl_api_sw_interface_clear_stats_t_handler(vl_api_sw_interface_clear_stats_t *mp)
{
    ...
    if (mp->sw_if_index != ~0) VALIDATE_SW_IF_INDEX(mp);
                                    ...
                                    enum api_events            events)
{
    ...
    mp->flags        = 0;
    mp->flags |= (events & API_ADMIN_UP_EVENT) ? IF_STATUS_API_FLAG_ADMIN_UP : 0;
    mp->flags |= (events & API_LINK_STATE_UP_EVENT) ? IF_STATUS_API_FLAG_LINK_UP : 0;
    mp->flags   = ntohl(mp->flags);
...
static uword link_state_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    while (1) {
        ...
        for (i = 0; i < vec_len(event_by_sw_if_index); i++) {
            if (event_by_sw_if_index[i] == 0) continue;

            pool_foreach (reg, vam->interface_events_registrations) {
                vl_reg = vl_api_client_index_to_registration(reg->client_index);
                if (vl_reg) send_sw_interface_event(vam, reg, vl_reg, i, event_by_sw_if_index[i]);
            }
        }
...
static clib_error_t *link_up_down_function(vnet_main_t *vm, u32 hw_if_index, u32 flags)
{
    ...
    if (vam->link_state_process_up) {
        enum api_events event = ((flags & VNET_HW_INTERFACE_FLAG_LINK_UP) ? API_LINK_STATE_UP_EVENT : API_LINK_STATE_DOWN_EVENT);
        vlib_process_signal_event(vam->vlib_main, link_state_process_node.index, event, hi->sw_if_index);
    }
    return 0;
}

static clib_error_t *admin_up_down_function(vnet_main_t *vm, u32 sw_if_index, u32 flags)
{
    vpe_api_main_t *vam = &vpe_api_main;
     ...
     */
    if (vam->link_state_process_up) {
        enum api_events event = ((flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP) ? API_ADMIN_UP_EVENT : API_ADMIN_DOWN_EVENT);
        vlib_process_signal_event(vam->vlib_main, link_state_process_node.index, event, sw_if_index);
    }
    return 0;
}
...
static clib_error_t *sw_interface_add_del_function(vnet_main_t *vm, u32 sw_if_index, u32 flags)
{
    ...
    if (vam->link_state_process_up) {
        enum api_events event = flags ? API_SW_INTERFACE_ADD_EVENT : API_SW_INTERFACE_DEL_EVENT;
        vlib_process_signal_event(vam->vlib_main, link_state_process_node.index, event, sw_if_index);
    }
    return 0;
}
...
static void vl_api_sw_interface_add_del_mac_address_t_handler(vl_api_sw_interface_add_del_mac_address_t *mp)
{
    ...
    error = vnet_hw_interface_add_del_mac_address(vnm, hi->hw_if_index, mp->addr, mp->is_add);
    if (error) {
...
static void vl_api_sw_interface_get_mac_address_t_handler(vl_api_sw_interface_get_mac_address_t *mp)
{
    ...
    if (si->type == VNET_SW_INTERFACE_TYPE_HARDWARE) eth_if = ethernet_get_interface(&ethernet_main, si->hw_if_index);
    ...
    rmp->_vl_msg_id = htons(REPLY_MSG_ID_BASE + VL_API_SW_INTERFACE_GET_MAC_ADDRESS_REPLY);
    rmp->context    = mp->context;
    rmp->retval     = htonl(rv);
    if (!rv && eth_if) mac_address_encode(&eth_if->address.mac, rmp->mac_address);
...
static void vl_api_sw_interface_set_rx_mode_t_handler(vl_api_sw_interface_set_rx_mode_t *mp)
{
    ...
    error   = set_hw_interface_change_rx_mode(vnm, si->hw_if_index, mp->queue_id_valid, ntohl(mp->queue_id), (vnet_hw_if_rx_mode) rx_mode);
...
static void
send_interface_rx_placement_details(vpe_api_main_t *am, vl_api_registration_t *rp, u32 sw_if_index, u32 worker_id, u32 queue_id, u8 mode, u32 context)
{
    vl_api_sw_interface_rx_placement_details_t *mp;
    mp = vl_msg_api_alloc(sizeof(*mp));
    clib_memset(mp, 0, sizeof(*mp));

    mp->_vl_msg_id  = htons(REPLY_MSG_ID_BASE + VL_API_SW_INTERFACE_RX_PLACEMENT_DETAILS);
...
static void vl_api_sw_interface_rx_placement_dump_t_handler(vl_api_sw_interface_rx_placement_dump_t *mp)
{
    ...
    if (sw_if_index == ~0) {
        ...
        vec_foreach (qptr, all_queues) {
            ...
            send_interface_rx_placement_details(am, reg, hw_if->sw_if_index, current_thread, qptr[0]->queue_id, qptr[0]->mode, mp->context);
        }
        vec_free(all_queues);
    }
    else {
        ...
        for (i = 0; i < vec_len(hw->rx_queue_indices); i++) {
            vnet_hw_if_rx_queue_t *rxq = vnet_hw_if_get_rx_queue(vnm, hw->rx_queue_indices[i]);
            send_interface_rx_placement_details(am, reg, hw->sw_if_index, rxq->thread_index, rxq->queue_id, rxq->mode, mp->context);
        }
    }
...
static void vl_api_sw_interface_set_rx_placement_t_handler(vl_api_sw_interface_set_rx_placement_t *mp)
{
    ...
    error = set_hw_interface_rx_placement(si->hw_if_index, ntohl(mp->queue_id), ntohl(mp->worker_id), mp->is_main);
    if (error) {
        rv = VNET_API_ERROR_UNIMPLEMENTED;
        clib_error_report(error);
        goto out;
...
static void send_interface_tx_placement_details(vnet_hw_if_tx_queue_t **all_queues, u32 index, vl_api_registration_t *rp, u32 context)
{
    vnet_main_t                                *vnm = vnet_get_main();
    ...
    REPLY_MACRO_DETAILS5_END(VL_API_SW_INTERFACE_TX_PLACEMENT_DETAILS, n, rp, context, ({
...
static void vl_api_sw_interface_tx_placement_get_t_handler(vl_api_sw_interface_tx_placement_get_t *mp)
{
    vnet_main_t                                  *vnm        = vnet_get_main();
    ...
    REPLY_AND_DETAILS_VEC_MACRO_END(VL_API_SW_INTERFACE_TX_PLACEMENT_GET_REPLY, all_queues, mp, rmp, rv, ({
                                        send_interface_tx_placement_details(all_queues, cursor, rp, mp->context);
                                    }));
...
static void vl_api_sw_interface_set_tx_placement_t_handler(vl_api_sw_interface_set_tx_placement_t *mp)
{
    ...
    switch (rv) {
        ...
        case VNET_API_ERROR_INVALID_QUEUE:
            error = clib_error_return(0, "unknown queue %u on interface %s", queue_id, vnet_get_hw_interface(vnet_get_main(), si->hw_if_index)->name);
            break;
        default:
            break;
    }
...
static void vl_api_create_subif_t_handler(vl_api_create_subif_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_CREATE_SUBIF_REPLY, ({ rmp->sw_if_index = ntohl(sub_sw_if_index); }));
}
...
static void vl_api_interface_name_renumber_t_handler(vl_api_interface_name_renumber_t *mp)
{
    ...
    rv = vnet_interface_name_renumber(ntohl(mp->sw_if_index), ntohl(mp->new_show_dev_instance));
...
static void vl_api_create_loopback_t_handler(vl_api_create_loopback_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_CREATE_LOOPBACK_REPLY, ({ rmp->sw_if_index = ntohl(sw_if_index); }));
}
...
static void vl_api_create_loopback_instance_t_handler(vl_api_create_loopback_instance_t *mp)
{
    ...
    mac_address_decode(mp->mac_address, &mac);
    rv = vnet_create_loopback_interface(&sw_if_index, (u8 *) &mac, is_specified, user_instance);

    REPLY_MACRO2(VL_API_CREATE_LOOPBACK_INSTANCE_REPLY, ({ rmp->sw_if_index = ntohl(sw_if_index); }));
}
...
static void vl_api_collect_detailed_interface_stats_t_handler(vl_api_collect_detailed_interface_stats_t *mp)
{
    ...
    rv = vnet_sw_interface_stats_collect_enable_disable(ntohl(mp->sw_if_index), mp->enable_disable);
...
static void vl_api_pcap_set_filter_function_t_handler(vl_api_pcap_set_filter_function_t *mp)
{
    vnet_main_t                             *vnm = vnet_get_main();
...
static void vl_api_pcap_trace_on_t_handler(vl_api_pcap_trace_on_t *mp)
{
    ...
    if (!unformat_user(&filename, unformat_vlib_tmpfile, &capture_args.filename)) {
    ...
    unformat_user(&drop_err_name, unformat_vlib_error, vlib_get_main(), &capture_args.drop_err);
...
static void vl_api_pcap_trace_off_t_handler(vl_api_pcap_trace_off_t *mp)
{
...
static clib_error_t *interface_api_hookup(vlib_main_t *vm)
{
    ...
    vl_api_set_msg_thread_safe(am, REPLY_MSG_ID_BASE + VL_API_SW_INTERFACE_DUMP, 1);
    vl_api_set_msg_thread_safe(am, REPLY_MSG_ID_BASE + VL_API_SW_INTERFACE_DETAILS, 1);
    vl_api_set_msg_thread_safe(am, REPLY_MSG_ID_BASE + VL_API_SW_INTERFACE_TAG_ADD_DEL, 1);
    vl_api_set_msg_thread_safe(am, REPLY_MSG_ID_BASE + VL_API_SW_INTERFACE_SET_INTERFACE_NAME, 1);

    /* Do not replay VL_API_SW_INTERFACE_DUMP messages */
    vl_api_allow_msg_replay(am, REPLY_MSG_ID_BASE + VL_API_SW_INTERFACE_DUMP, 0);

    /* Mark these APIs as autoendian */
    vl_api_set_msg_autoendian(am, REPLY_MSG_ID_BASE + VL_API_SW_INTERFACE_SET_TX_PLACEMENT, 1);
    vl_api_set_msg_autoendian(am, REPLY_MSG_ID_BASE + VL_API_SW_INTERFACE_TX_PLACEMENT_GET, 1);
