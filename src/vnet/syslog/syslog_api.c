...
static void vl_api_syslog_set_sender_t_handler(vl_api_syslog_set_sender_t *mp)
{
    ...
    int rv = set_syslog_sender(&collector, ntohs(mp->collector_port), &src, ntohl(mp->vrf_id), ntohl(mp->max_msg_size));
...
static void vl_api_syslog_get_sender_t_handler(vl_api_syslog_get_sender_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_SYSLOG_GET_SENDER_REPLY, ({
                     clib_memcpy(&rmp->collector_address, &(sm->collector), sizeof(ip4_address_t));
                     clib_memcpy(&rmp->src_address, &(sm->src_address), sizeof(ip4_address_t));
...
static void vl_api_syslog_get_filter_t_handler(vl_api_syslog_get_filter_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_SYSLOG_GET_FILTER_REPLY, ({ rv = syslog_severity_encode(sm->severity_filter, &rmp->severity); }))
}
