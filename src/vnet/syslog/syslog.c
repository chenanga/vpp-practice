...
static u8 *format_syslog_header(u8 *s, va_list *args)
{
    ...
    return format(s,
                  ...
                  h->timestamp + sm->time_offset,
                  format_ip4_address,
                  &sm->src_address,
                  h->app_name ? h->app_name : NILVALUE,
                  sm->procid,
...
static u8 *format_syslog_msg(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U %U", format_syslog_header, &m->header, format_syslog_structured_data, m->structured_data);
    /* free-form message is optional */
    if (m->msg) s = format(s, " %s", m->msg);
...
void syslog_msg_add_sd_param(syslog_msg_t *syslog_msg, char *name, char *fmt, ...)
{
    ...
    syslog_msg->structured_data[syslog_msg->curr_sd_index] =
        format(syslog_msg->structured_data[syslog_msg->curr_sd_index], " %s=\"%s\"", name, value);
    vec_free(value);
}
...
int syslog_msg_send(syslog_msg_t *syslog_msg)
{
    ...
    if (vlib_buffer_alloc(vm, &bi, 1) != 1) return -1;
...
vnet_api_error_t set_syslog_sender(ip4_address_t *collector, u16 collector_port, ip4_address_t *src, u32 vrf_id, u32 max_msg_size)
{
    ...
    if (max_msg_size < DEFAULT_MAX_MSG_SIZE) return VNET_API_ERROR_INVALID_VALUE;

    if (collector->as_u32 == 0 || collector_port == 0 || src->as_u32 == 0) return VNET_API_ERROR_INVALID_VALUE;
...
static clib_error_t *set_syslog_sender_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "collector %U", unformat_ip4_address, &collector))
            ;
    ...
    if (max_msg_size < DEFAULT_MAX_MSG_SIZE) {
        ret = clib_error_return(0, "too small max-msg-size value, minimum is %u", DEFAULT_MAX_MSG_SIZE);
        goto done;
    }

    vnet_api_error_t rv = set_syslog_sender(&collector, collector_port, &src, vrf_id, max_msg_size);

    if (rv) ret = clib_error_return(0, "set syslog sender failed rv=%d:%U", (int) rv, format_vnet_api_errno, rv);
...
static clib_error_t *show_syslog_sender_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (sm->fib_index != ~0) vrf_id = fib_table_get_table_id(sm->fib_index, FIB_PROTOCOL_IP4);
...
static clib_error_t *test_syslog_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (unformat(line_input, "%U", unformat_syslog_facility, &facility)) {
        if (unformat(line_input, "%U", unformat_syslog_severity, &severity)) {
            if (syslog_severity_filter_block(severity)) goto done;

            if (unformat(line_input, "%s", &app_name)) {
                if (unformat(line_input, "%s", &msgid)) {
                    syslog_msg_init(&syslog_msg, facility, severity, (char *) app_name, (char *) msgid);
                    while (unformat(line_input, "sd-id %s", &sd_id)) {
                        syslog_msg_sd_init(&syslog_msg, (char *) sd_id);
                        while (unformat(line_input, "sd-param %s %s", &param_name, &param_value)) {
                            syslog_msg_add_sd_param(&syslog_msg, (char *) param_name, (char *) param_value);
                            vec_free(param_name);
                    ...
                    if (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT)
                        syslog_msg_add_msg(&syslog_msg, "%U", format_unformat_input, line_input);
...
static clib_error_t *set_syslog_filter_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "severity %U", unformat_syslog_severity, &sm->severity_filter))
            ;
        else {
            ret = clib_error_return(0, "Unknown input `%U'", format_unformat_error, line_input);
            goto done;
...
static clib_error_t *show_syslog_filter_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vlib_cli_output(vm, "severity-filter: %U", format_syslog_severity, sm->severity_filter);
...
static clib_error_t *syslog_init(vlib_main_t *vm)
{
    ...
    sm->time_offset = (f64) timeval_0.tv_sec + (((f64) timeval_0.tv_usec) * 1e-6) - vlib_time_0;
