...
static u8 *format_gso_trace(u8 *s, va_list *args)
{
    ...
    if (t->flags & VNET_BUFFER_F_GSO) {
        s = format(s, "gso_sz %d gso_l4_hdr_sz %d\n%U", t->gso_size, t->gso_l4_hdr_sz, format_generic_header_offset, &t->gho);
    }
    else {
        s = format(s, "non-gso buffer\n%U", format_generic_header_offset, &t->gho);
    }

    return s;
}

static_always_inline void tso_segment_ipip_tunnel_fixup(vlib_main_t *vm, vnet_interface_per_thread_data_t *ptd, vlib_buffer_t *sb0)
{
    ...
    while (i < n_tx_bufs) {
        ...
        if (vnet_buffer(b0)->oflags & VNET_BUFFER_OFFLOAD_F_OUTER_IP_CKSUM) {
            ip4->length   = clib_host_to_net_u16(b0->current_length - (outer_l3_hdr_offset - b0->current_data));
            ip4->checksum = ip4_header_checksum(ip4);
            vnet_buffer_offload_flags_clear(b0, VNET_BUFFER_OFFLOAD_F_OUTER_IP_CKSUM | VNET_BUFFER_OFFLOAD_F_TNL_IPIP);
        }
        else {
            ip6->payload_length = clib_host_to_net_u16(b0->current_length - (l3_hdr_offset - b0->current_data));
...
static_always_inline void tso_segment_vxlan_tunnel_headers_fixup(vlib_main_t *vm, vlib_buffer_t *b)
{
    ip4_header_t *ip4                 = 0;
    ip6_header_t *ip6                 = 0;
    udp_header_t *udp                 = 0;
    ...
    if (vnet_buffer(b)->oflags & VNET_BUFFER_OFFLOAD_F_OUTER_IP_CKSUM) {
        ip4->length   = clib_host_to_net_u16(b->current_length - (outer_l3_hdr_offset - b->current_data));
        ip4->checksum = ip4_header_checksum(ip4);
        if (vnet_buffer(b)->oflags & VNET_BUFFER_OFFLOAD_F_OUTER_UDP_CKSUM) {
            udp->length = clib_host_to_net_u16(b->current_length - (outer_l4_hdr_offset - b->current_data));
            // udp checksum is 0, in udp tunnel
            udp->checksum = 0;
        }
        vnet_buffer_offload_flags_clear(
            b,
            VNET_BUFFER_OFFLOAD_F_OUTER_IP_CKSUM | VNET_BUFFER_OFFLOAD_F_OUTER_UDP_CKSUM | VNET_BUFFER_OFFLOAD_F_TNL_VXLAN);
    }
    else {
        ip6->payload_length = clib_host_to_net_u16(b->current_length - (outer_l4_hdr_offset - b->current_data));
        ...
        if (vnet_buffer(b)->oflags & VNET_BUFFER_OFFLOAD_F_OUTER_UDP_CKSUM) {
            ...
            vnet_buffer_offload_flags_clear(b, VNET_BUFFER_OFFLOAD_F_OUTER_UDP_CKSUM | VNET_BUFFER_OFFLOAD_F_TNL_VXLAN);
        }
    }
}

static_always_inline void tso_segment_vxlan_tunnel_fixup(vlib_main_t *vm, vnet_interface_per_thread_data_t *ptd, vlib_buffer_t *sb0)
{
    u16 n_tx_bufs = vec_len(ptd->split_buffers);
    u16 i         = 0;
                                           ...
                                           generic_header_offset_t          *gho)
{
     ...
     */
    size = clib_min(gso_size, vlib_buffer_get_default_data_size(vm) - l234_sz - gho->l2_hdr_offset);
                                                     ...
                                                     generic_header_offset_t *gho)
{
    ...
    *p_dst_left = clib_min(gso_size, vlib_buffer_get_default_data_size(vm) - (template_data_sz + nb0->current_data));
    *p_dst_ptr  = vlib_buffer_get_current(nb0) + template_data_sz;

    tcp_header_t *tcp = (tcp_header_t *) (vlib_buffer_get_current(nb0) + gho->l4_hdr_offset);
...
static_always_inline void tso_fixup_segmented_buf(vlib_main_t             *vm,
                                                  ...
                                                  generic_header_offset_t *gho)
{
    ip4_header_t *ip4 = (ip4_header_t *) (vlib_buffer_get_current(b0) + gho->l3_hdr_offset);
    ip6_header_t *ip6 = (ip6_header_t *) (vlib_buffer_get_current(b0) + gho->l3_hdr_offset);
    tcp_header_t *tcp = (tcp_header_t *) (vlib_buffer_get_current(b0) + gho->l4_hdr_offset);

    tcp->flags = tcp_flags;

    if (is_ip6) {
        ip6->payload_length = clib_host_to_net_u16(b0->current_length - gho->l4_hdr_offset);
        if (gho->gho_flags & GHO_F_TCP) {
            int bogus     = 0;
            tcp->checksum = 0;
            tcp->checksum = ip6_tcp_udp_icmp_compute_checksum(vm, b0, ip6, &bogus);
            vnet_buffer_offload_flags_clear(b0, VNET_BUFFER_OFFLOAD_F_TCP_CKSUM);
        }
    }
    else {
        ip4->length = clib_host_to_net_u16(b0->current_length - gho->l3_hdr_offset);
        if (gho->gho_flags & GHO_F_IP4) ip4->checksum = ip4_header_checksum(ip4);
        if (gho->gho_flags & GHO_F_TCP) {
            tcp->checksum = 0;
            tcp->checksum = ip4_tcp_udp_compute_checksum(vm, b0, ip4);
        }
        vnet_buffer_offload_flags_clear(b0, (VNET_BUFFER_OFFLOAD_F_IP_CKSUM | VNET_BUFFER_OFFLOAD_F_TCP_CKSUM));
    }
    ...
    if (!is_l2 && ((gho->gho_flags & GHO_F_TUNNEL) == 0)) {
        ...
        if (adj0->lookup_next_index == IP_LOOKUP_NEXT_MIDCHAIN && adj0->sub_type.midchain.fixup_func) /* calls e.g. ipip44_fixup */
            adj0->sub_type.midchain.fixup_func(vm, adj0, b0, adj0->sub_type.midchain.fixup_data);
    }
}
                                            ...
                                            int                               is_ip6)
{
    ...
    tcp_header_t *tcp = (tcp_header_t *) (vlib_buffer_get_current(sb0) + gho->l4_hdr_offset);
    ...
    u32 default_bflags  = sb0->flags & ~(VNET_BUFFER_F_GSO | VLIB_BUFFER_NEXT_PRESENT);
    u16 l234_sz         = gho->hdr_sz;
    int first_data_size = clib_min(gso_size, sb0->current_length - l234_sz);
    next_tcp_seq += first_data_size;

    if (PREDICT_FALSE(!tso_alloc_tx_bufs(vm, ptd, sb0, n_bytes_b0, l234_sz, gso_size, first_data_size, gho))) return 0;

    vlib_buffer_t *b0 = vlib_get_buffer(vm, ptd->split_buffers[0]);
    tso_init_buf_from_template_base(b0, sb0, default_bflags, l234_sz + first_data_size);
    ...
    if (total_src_left) {
        ...
        tso_fixup_segmented_buf(vm, b0, tcp_flags_no_fin_psh, is_l2, is_ip6, gho);
        ...
        tso_init_buf_from_template(vm, cdb0, b0, l234_sz, gso_size, &dst_ptr, &dst_left, next_tcp_seq, default_bflags, gho);

        /* an arbitrary large number to catch the runaway loops */
        int nloops = 2000;
        while (total_src_left) {
            if (nloops-- <= 0) clib_panic("infinite loop detected");
            ...
            if (0 == dst_left && total_src_left) {
                ...
                tso_fixup_segmented_buf(vm, cdb0, tcp_flags_no_fin_psh, is_l2, is_ip6, gho);
                ASSERT(dbi < vec_len(ptd->split_buffers));
                cdb0 = vlib_get_buffer(vm, ptd->split_buffers[dbi++]);
                tso_init_buf_from_template(vm, cdb0, b0, l234_sz, gso_size, &dst_ptr, &dst_left, next_tcp_seq, default_bflags, gho);
            }
...
__clib_unused u32 gso_segment_buffer(vlib_main_t                      *vm,
                                     vnet_interface_per_thread_data_t *ptd,
                                     u32                               bi,
                                     vlib_buffer_t                    *b,
                                     generic_header_offset_t          *gho,
                                                ...
                                                int                  do_segmentation)
{
    ...
    vnet_interface_per_thread_data_t *ptd          = vec_elt_at_index(im->per_thread_data, thread_index);
    ...
    while (n_left_from > 0) {
        ...
        if (!do_segmentation)
            while (from + 8 <= from_end && n_left_to_next >= 4) {
                ...
                if (PREDICT_FALSE(hi->sw_if_index != swif0)) {
                    ...
                    if ((hi0->caps & VNET_HW_IF_CAP_TCP_GSO) == 0 && (b[0]->flags & VNET_BUFFER_F_GSO)) break;
                }
                if (PREDICT_FALSE(hi->sw_if_index != swif1)) {
                    hi1 = vnet_get_sup_hw_interface(vnm, swif1);
                    if (!(hi1->caps & VNET_HW_IF_CAP_TCP_GSO) && (b[1]->flags & VNET_BUFFER_F_GSO)) break;
                }
                if (PREDICT_FALSE(hi->sw_if_index != swif2)) {
                    hi2 = vnet_get_sup_hw_interface(vnm, swif2);
                    if ((hi2->caps & VNET_HW_IF_CAP_TCP_GSO) == 0 && (b[2]->flags & VNET_BUFFER_F_GSO)) break;
                }
                if (PREDICT_FALSE(hi->sw_if_index != swif3)) {
                    hi3 = vnet_get_sup_hw_interface(vnm, swif3);
                    if (!(hi3->caps & VNET_HW_IF_CAP_TCP_GSO) && (b[3]->flags & VNET_BUFFER_F_GSO)) break;
                }
                ...
                if (b[0]->flags & VLIB_BUFFER_IS_TRACED) {
                    ...
                    vnet_generic_header_offset_parser(b[0], &t0->gho, is_l2, is_ip4, is_ip6);
                }
                if (b[1]->flags & VLIB_BUFFER_IS_TRACED) {
                    ...
                    vnet_generic_header_offset_parser(b[1], &t1->gho, is_l2, is_ip4, is_ip6);
                }
                if (b[2]->flags & VLIB_BUFFER_IS_TRACED) {
                    ...
                    vnet_generic_header_offset_parser(b[2], &t2->gho, is_l2, is_ip4, is_ip6);
                }
                if (b[3]->flags & VLIB_BUFFER_IS_TRACED) {
                    ...
                    vnet_generic_header_offset_parser(b[3], &t3->gho, is_l2, is_ip4, is_ip6);
                }
        ...
        while (from + 1 <= from_end && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(hi->sw_if_index != swif0)) {
                hi0 = vnet_get_sup_hw_interface(vnm, swif0);
                if ((hi0->caps & VNET_HW_IF_CAP_TCP_GSO) == 0 && (b[0]->flags & VNET_BUFFER_F_GSO)) do_segmentation0 = 1;
            }
            else
            ...
            if (do_segmentation0) {
                if (PREDICT_FALSE(b[0]->flags & VNET_BUFFER_F_GSO)) {
                    ...
                    n_tx_bytes = gso_segment_buffer_inline(vm, ptd, b[0], is_l2);

                    if (PREDICT_FALSE(n_tx_bytes == 0)) {
                        drop_one_buffer_and_count(vm, vnm, node, from - 1, hi->sw_if_index, GSO_ERROR_NO_BUFFERS);
                        b += 1;
                        continue;
                    }

                    if (PREDICT_FALSE(vnet_buffer(b[0])->oflags & VNET_BUFFER_OFFLOAD_F_TNL_VXLAN)) {
                        tso_segment_vxlan_tunnel_fixup(vm, ptd, b[0]);
                    }
                    else if (PREDICT_FALSE(vnet_buffer(b[0])->oflags & VNET_BUFFER_OFFLOAD_F_TNL_IPIP)) {
                    ...
                    while (n_tx_bufs > 0) {
                        ...
                        while (n_tx_bufs > 0 && n_left_to_next > 0) {
                            ...
                            vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, sbi0, next0);
                        }
                        vlib_put_next_frame(vm, node, next_index, n_left_to_next);
                        if (n_tx_bufs > 0) vlib_get_next_frame(vm, node, next_index, to_next, n_left_to_next);
                    }
...
static_always_inline uword vnet_gso_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_l2, int is_ip4, int is_ip6)
{
    ...
    if (frame->n_vectors > 0) {
        ...
        hi                  = vnet_get_sup_hw_interface(vnm, vnet_buffer(b)->sw_if_index[VLIB_TX]);

        if (hi->caps & (VNET_HW_IF_CAP_TCP_GSO | VNET_HW_IF_CAP_VXLAN_TNL_GSO))
            return vnet_gso_node_inline(vm,
                                        node,
                                        frame,
                                        vnm,
                                        hi,
...
VLIB_NODE_FN(gso_l2_ip4_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return vnet_gso_inline(vm, node, frame, 1 /* l2 */, 1 /* ip4 */, 0 /* ip6 */);
}

VLIB_NODE_FN(gso_l2_ip6_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return vnet_gso_inline(vm, node, frame, 1 /* l2 */, 0 /* ip4 */, 1 /* ip6 */);
}

VLIB_NODE_FN(gso_ip4_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return vnet_gso_inline(vm, node, frame, 0 /* l2 */, 1 /* ip4 */, 0 /* ip6 */);
}

VLIB_NODE_FN(gso_ip6_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return vnet_gso_inline(vm, node, frame, 0 /* l2 */, 0 /* ip4 */, 1 /* ip6 */);
}
