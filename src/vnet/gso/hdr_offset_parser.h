...
#define GHO_F_TUNNEL (GHO_F_VXLAN_TUNNEL | GHO_F_GENEVE_TUNNEL | GHO_F_IPIP_TUNNEL | GHO_F_IPIP6_TUNNEL | GHO_F_GRE_TUNNEL)

#define GHO_F_OUTER_HDR (GHO_F_OUTER_IP4 | GHO_F_OUTER_IP6 | GHO_F_OUTER_TCP | GHO_F_OUTER_UDP)

#define GHO_F_INNER_HDR (GHO_F_IP4 | GHO_F_IP6 | GHO_F_UDP | GHO_F_TCP)
...
{
    if ((gho->gho_flags & GHO_F_TUNNEL) && (gho->gho_flags & GHO_F_OUTER_HDR) && (b0->current_data == gho->outer_l2_hdr_offset))
...
{
    if ((gho->gho_flags & GHO_F_TUNNEL) && (gho->gho_flags & GHO_F_OUTER_HDR) && (b0->current_data == gho->l2_hdr_offset))
        vlib_buffer_advance(b0, -gho->outer_hdr_sz);
}
...
static_always_inline void vnet_ipip_inner_header_parser_inline(vlib_buffer_t *b0, generic_header_offset_t *gho)
{
    ...
    if (l4_proto == IP_PROTOCOL_TCP) {
        ...
        udp_header_t *udp = (udp_header_t *) (vlib_buffer_get_current(b0) + gho->l4_hdr_offset);
...
static_always_inline void vnet_vxlan_inner_header_parser_inline(vlib_buffer_t *b0, generic_header_offset_t *gho)
{
    ...
    if ((gho->gho_flags & GHO_F_VXLAN_TUNNEL) == 0) return;
