...
u32 gso_segment_buffer(vlib_main_t                      *vm,
...
static_always_inline void gso_init_bufs_from_template_base(vlib_buffer_t **bufs, vlib_buffer_t *b0, u32 flags, u16 n_bufs, u16 hdr_sz)
{
    u32 i = n_bufs;
    while (i >= 6) {
        /* prefetches */
        ...
        bufs[0]->current_config_index = bufs[1]->current_config_index = b0->current_config_index;
...
static_always_inline void gso_fixup_segmented_buf(vlib_main_t    *vm,
                                                  vlib_buffer_t  *b0,
                                                  u32             next_tcp_seq,
                                                  int             is_l2,
                                                  u8              oflags,
                                                  ...
                                                  vlib_buffer_t  *b1)
{
    ...
    if (oflags & VNET_BUFFER_OFFLOAD_F_IP_CKSUM) {
        ip4->length   = clib_host_to_net_u16(b0->current_length - hdr_sz + (l4_hdr_offset - l3_hdr_offset) + l4_hdr_sz);
        ip4->checksum = 0;
        ...
        c->sum += clib_host_to_net_u32((clib_net_to_host_u16(ip4->length) - ip4_header_bytes(ip4)) + (ip4->protocol << 16));
    }
    else {
    ...
    if (is_prefetch) CLIB_PREFETCH(vlib_buffer_get_current(b1) + hdr_sz, CLIB_CACHE_LINE_BYTES, LOAD);
...
static_always_inline u32 gso_segment_buffer_inline(vlib_main_t *vm, vnet_interface_per_thread_data_t *ptd, vlib_buffer_t *b, int is_l2)
{
    ...
    u32            default_bflags = b->flags & ~(VNET_BUFFER_F_GSO | VLIB_BUFFER_NEXT_PRESENT);
    ...
    u16            size      = clib_min(gso_size, vlib_buffer_get_default_data_size(vm) - hdr_sz);
    ...
    while (data_size) {
        ...
        if (0 == dst_left && data_size) {
            ...
            gso_fixup_segmented_buf(vm, bufs[i], tcp_seq, is_l2, oflags, hdr_sz, l4_hdr_sz, &c, tcp_flags_no_fin_psh, 1, bufs[i + 1]);
            i++;
            dst_left = size;
            dst_ptr  = vlib_buffer_get_current(bufs[i]) + hdr_sz;
    ...
    ASSERT((i + 1) == n_alloc);
    n_tx_bytes += bufs[i]->current_length;
    gso_fixup_segmented_buf(vm, bufs[i], tcp_seq, is_l2, oflags, hdr_sz, l4_hdr_sz, &c, tcp_flags, 0, NULL);
