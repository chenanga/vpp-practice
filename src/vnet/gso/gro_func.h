...
{
    if (((b->current_length - l234_sz) <= 0) || ((flags &= ~(TCP_FLAG_ACK | TCP_FLAG_PSH)) != 0)) return 1;
    return 0;
}
...
{
    if (b0->flags & VNET_BUFFER_F_IS_IP4) return VNET_BUFFER_F_IS_IP4;
    if (b0->flags & VNET_BUFFER_F_IS_IP6) return VNET_BUFFER_F_IS_IP6;
    if (is_l2) {
        ...
        if (ethertype == ETHERNET_TYPE_IP4) return VNET_BUFFER_F_IS_IP4;
        if (ethertype == ETHERNET_TYPE_IP6) return VNET_BUFFER_F_IS_IP6;
    }
    else {
        if ((((u8 *) vlib_buffer_get_current(b0))[0] & 0xf0) == 0x40) return VNET_BUFFER_F_IS_IP4;
        if ((((u8 *) vlib_buffer_get_current(b0))[0] & 0xf0) == 0x60) return VNET_BUFFER_F_IS_IP6;
    }
...
static_always_inline gro_packet_action_t gro_tcp_sequence_check(tcp_header_t *tcp0, tcp_header_t *tcp1, u32 payload_len0)
{
    ...
    /* next packet, enqueue */
    if (PREDICT_TRUE(next_tcp_seq0 + payload_len0 == next_tcp_seq1)) return GRO_PACKET_ACTION_ENQUEUE;
    /* flush all packets */
    else
...
static_always_inline void gro_merge_buffers(vlib_main_t *vm, vlib_buffer_t *b0, vlib_buffer_t *b1, u32 bi1, u32 payload_len1, u16 l234_sz1)
{
    ...
    if (PREDICT_FALSE((b0->flags & VLIB_BUFFER_NEXT_PRESENT) == 0)) b0->total_length_not_including_first_buffer = 0;
...
static_always_inline u32 gro_validate_checksum(vlib_main_t *vm, vlib_buffer_t *b0, generic_header_offset_t *gho0, int is_ip4)
{
    ...
    if (b0->flags & VNET_BUFFER_F_OFFLOAD) return VNET_BUFFER_F_L4_CHECKSUM_CORRECT;
...
gro_fix_padded_packet_len(vlib_buffer_t *b0, generic_header_offset_t *gho0, ip4_header_t *ip4_0, ip6_header_t *ip6_0, u32 pkt_len0, u16 l234_sz0)
{
    u32 tcp_payload_len0 = 0;
    if (gho0->gho_flags & GHO_F_IP4) {
        tcp_payload_len0 = clib_net_to_host_u16(ip4_0->length) - ip4_header_bytes(ip4_0) - gho0->l4_hdr_sz;
    }
    else {
        tcp_payload_len0 = clib_net_to_host_u16(ip6_0->payload_length) - gho0->l4_hdr_sz;
    }
...
static_always_inline u32 gro_get_packet_data(vlib_main_t *vm, vlib_buffer_t *b0, generic_header_offset_t *gho0, gro_flow_key_t *flow_key0, u8 is_l2)
{
    ...
    if (is_ip0 & VNET_BUFFER_F_IS_IP4)
        vnet_generic_header_offset_parser(b0, gho0, is_l2, 1 /* is_ip4 */, 0 /* is_ip6 */);
    else if (is_ip0 & VNET_BUFFER_F_IS_IP6)
        vnet_generic_header_offset_parser(b0, gho0, is_l2, 0 /* is_ip4 */, 1 /* is_ip6 */);
    else
        return 0;

    if (PREDICT_FALSE((gho0->gho_flags & GHO_F_TCP) == 0)) return 0;
    ...
    if (PREDICT_FALSE(gro_is_bad_packet(b0, tcp0->flags, l234_sz0))) return 0;
    ...
    if (gho0->gho_flags & GHO_F_IP4) {
        ...
        gro_get_ip4_flow_from_packet(sw_if_index0, ip4_0, tcp0, flow_key0, is_l2);
    }
    else if (gho0->gho_flags & GHO_F_IP6) {
        flags = gro_validate_checksum(vm, b0, gho0, 0);
        gro_get_ip6_flow_from_packet(sw_if_index0, ip6_0, tcp0, flow_key0, is_l2);
    }
    else
        return 0;

    if (PREDICT_FALSE((flags & VNET_BUFFER_F_L4_CHECKSUM_CORRECT) == 0)) return 0;

    pkt_len0 = vlib_buffer_length_in_chain(vm, b0);
    if (PREDICT_FALSE(pkt_len0 >= TCP_MAX_GSO_SZ)) return 0;

    if (PREDICT_FALSE(pkt_len0 <= GRO_PADDED_PACKET_SIZE)) {
        pkt_len0 = gro_fix_padded_packet_len(b0, gho0, ip4_0, ip6_0, pkt_len0, l234_sz0);
    }
    return pkt_len0;
}
...
static_always_inline u32 gro_coalesce_buffers(vlib_main_t *vm, vlib_buffer_t *b0, vlib_buffer_t *b1, u32 bi1, u8 is_l2)
{
    ...
    if (is_ip1 & VNET_BUFFER_F_IS_IP4)
        vnet_generic_header_offset_parser(b1, &gho1, is_l2, 1 /* is_ip4 */, 0 /* is_ip6 */);
    else if (is_ip1 & VNET_BUFFER_F_IS_IP6)
        vnet_generic_header_offset_parser(b1, &gho1, is_l2, 0 /* is_ip4 */, 1 /* is_ip6 */);
    else
        return 0;
    ...
    if (gro_is_bad_packet(b0, tcp0->flags, l234_sz0) || gro_is_bad_packet(b1, tcp1->flags, l234_sz1)) return 0;
    ...
    if ((gho0.gho_flags & GHO_F_IP4) && (gho1.gho_flags & GHO_F_IP4)) {
        gro_get_ip4_flow_from_packet(sw_if_index0, ip4_0, tcp0, &flow_key0, is_l2);
        gro_get_ip4_flow_from_packet(sw_if_index1, ip4_1, tcp1, &flow_key1, is_l2);
    }
    else if ((gho0.gho_flags & GHO_F_IP6) && (gho1.gho_flags & GHO_F_IP6)) {
        gro_get_ip6_flow_from_packet(sw_if_index0, ip6_0, tcp0, &flow_key0, is_l2);
        gro_get_ip6_flow_from_packet(sw_if_index1, ip6_1, tcp1, &flow_key1, is_l2);
    }
    else
        return 0;

    if (gro_flow_is_equal(&flow_key0, &flow_key1) == 0) return 0;

    payload_len0 = pkt_len0 - l234_sz0;
    payload_len1 = pkt_len1 - l234_sz1;

    if (pkt_len0 >= TCP_MAX_GSO_SZ || pkt_len1 >= TCP_MAX_GSO_SZ || (pkt_len0 + payload_len1) >= TCP_MAX_GSO_SZ) return 0;

    if (gro_tcp_sequence_check(tcp0, tcp1, payload_len0) == GRO_PACKET_ACTION_ENQUEUE) {
...
static_always_inline void gro_fixup_header(vlib_main_t *vm, vlib_buffer_t *b0, u32 ack_number, u8 is_l2)
{
    ...
    if (gho0.gho_flags & GHO_F_IP4) {
        ip4_header_t *ip4              = (ip4_header_t *) (vlib_buffer_get_current(b0) + gho0.l3_hdr_offset);
        ...
        vnet_buffer_offload_flags_set(b0, (VNET_BUFFER_OFFLOAD_F_TCP_CKSUM | VNET_BUFFER_OFFLOAD_F_IP_CKSUM));
    }
    else if (gho0.gho_flags & GHO_F_IP6) {
        ip6_header_t *ip6              = (ip6_header_t *) (vlib_buffer_get_current(b0) + gho0.l3_hdr_offset);
        ip6->payload_length            = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, b0) - gho0.l4_hdr_offset);
    ...
    tcp_header_t *tcp0              = (tcp_header_t *) (vlib_buffer_get_current(b0) + gho0.l4_hdr_offset);
    ...
    if (flow_table->flow_table_size > 0) {
        ...
        while (i < GRO_FLOW_TABLE_MAX_SIZE) {
            ...
            if (gro_flow->n_buffers && gro_flow_is_timeout(vm, gro_flow)) {
                ...
                gro_fixup_header(vm, b0, gro_flow->last_ack_number, flow_table->is_l2);
...
static_always_inline void vnet_gro_flow_table_schedule_node_on_dispatcher(vlib_main_t *vm, vnet_hw_if_tx_queue_t *txq, gro_flow_table_t *flow_table)
{
...
static_always_inline u32 vnet_gro_flush_all_packets(vlib_main_t      *vm,
...
static_always_inline u32 vnet_gro_flow_table_inline(vlib_main_t *vm, gro_flow_table_t *flow_table, u32 bi0, u32 *to)
{
    ...
    tcp0 = (tcp_header_t *) (vlib_buffer_get_current(b0) + gho0.l4_hdr_offset);
    if (PREDICT_TRUE(((tcp0->flags & TCP_FLAG_PSH) == 0) && (pkt_len0 > GRO_MIN_PACKET_SIZE)))
    ...
    if (PREDICT_FALSE(gro_flow->n_buffers == 0)) {
        ...
        if (is_ip_s & VNET_BUFFER_F_IS_IP4)
            vnet_generic_header_offset_parser(b_s, &gho_s, is_l2, 1 /* is_ip4 */, 0 /* is_ip6 */);
        else if (is_ip_s & VNET_BUFFER_F_IS_IP6)
            vnet_generic_header_offset_parser(b_s, &gho_s, is_l2, 0 /* is_ip4 */, 1 /* is_ip6 */);

        tcp_s                      = (tcp_header_t *) (vlib_buffer_get_current(b_s) + gho_s.l4_hdr_offset);
        pkt_len_s                  = vlib_buffer_length_in_chain(vm, b_s);
        l234_sz0                   = gho0.hdr_sz;
        l234_sz_s                  = gho_s.hdr_sz;
        payload_len0               = pkt_len0 - l234_sz0;
        payload_len_s              = pkt_len_s - l234_sz_s;
        gro_packet_action_t action = gro_tcp_sequence_check(tcp_s, tcp0, payload_len_s);

        if (PREDICT_TRUE(action == GRO_PACKET_ACTION_ENQUEUE)) {
            if (PREDICT_TRUE(((pkt_len_s + payload_len0) < TCP_MAX_GSO_SZ) && (gro_flow->n_buffers < GRO_FLOW_N_BUFFERS))) {
                flow_table->total_vectors++;
                ...
                return vnet_gro_flush_all_packets(vm, flow_table, gro_flow, b_s, to, bi_s, bi0, is_l2);
            else {
    ...
    if (PREDICT_TRUE(((b[0]->flags & VNET_BUFFER_F_GSO) == 0))) {
        while (n_left_from > 1) {
            if (PREDICT_TRUE(((b[bi]->flags & VNET_BUFFER_F_GSO) == 0))) {
                u32 ret;
                if ((ret = gro_coalesce_buffers(vm, b[0], b[bi], from[bi], is_l2)) != 0) {
                    n_left_from -= 1;
                    bi += 1;
                    ack_number = ret;
                    continue;
