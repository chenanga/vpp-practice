...
{
    if (first->flow_data[0] == second->flow_data[0] && first->flow_data[1] == second->flow_data[1] && first->flow_data[2] == second->flow_data[2]
        && first->flow_data[3] == second->flow_data[3] && first->flow_data[4] == second->flow_data[4]
...
{
    if (gro_flow->next_timeout_ts < vlib_time_now(vm)) return 1;
    return 0;
}
...
static_always_inline u32 gro_flow_table_init(gro_flow_table_t **flow_table, u8 is_l2, u32 node_index)
{
    ...
    flow_table_temp                   = (gro_flow_table_t *) clib_mem_alloc(sizeof(gro_flow_table_t));
...
{
    if (flow_table) flow_table->timeout_ts = vlib_time_now(vm) + timeout_expire;
}

static_always_inline u8 gro_flow_table_is_timeout(vlib_main_t *vm, gro_flow_table_t *flow_table)
{
    if (flow_table && (flow_table->timeout_ts < vlib_time_now(vm))) return 1;
    return 0;
}

static_always_inline u8 gro_flow_table_is_enable(gro_flow_table_t *flow_table)
{
    if (flow_table) return flow_table->is_enable;
...
{
    if (flow_table) flow_table->is_enable = is_enable;
}

static_always_inline void gro_flow_table_free(gro_flow_table_t *flow_table)
{
    if (flow_table) clib_mem_free(flow_table);
}
...
static_always_inline gro_flow_t *gro_flow_table_get_flow(gro_flow_table_t *flow_table, gro_flow_key_t *flow_key)
{
    ...
    while (i < GRO_FLOW_TABLE_MAX_SIZE) {
        gro_flow = &flow_table->gro_flow[i];
        if (gro_flow_is_equal(flow_key, &gro_flow->flow_key)) return gro_flow;
        i++;
    }
    return (0);
}
...
static_always_inline u8 *gro_flow_table_format(u8 *s, va_list *args)
{
    ...
    if (flow_table->n_vectors) {
        double average_rate = (double) flow_table->total_vectors / (double) flow_table->n_vectors;
