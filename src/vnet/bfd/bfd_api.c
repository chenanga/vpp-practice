...
static void vl_api_bfd_udp_upd_t_handler(vl_api_bfd_udp_add_t *mp)
{
    ...
    rv = bfd_udp_upd_session(BFD_UDP_API_PARAM_FROM_MP(mp),
                             clib_net_to_host_u32(mp->desired_min_tx),
                             clib_net_to_host_u32(mp->required_min_rx),
                             mp->detect_mult,
                             mp->is_authenticated,
    ...
    BAD_SW_IF_INDEX_LABEL;
    REPLY_MACRO2(VL_API_BFD_UDP_UPD_REPLY, ({ rmp->stats_index = clib_host_to_net_u32(0); }));
}
...
void send_bfd_udp_session_details(vl_api_registration_t *reg, u32 context, bfd_session_t *bs)
{
    ...
    if ((!bs->auth.is_delayed && bs->auth.curr_key) || (bs->auth.is_delayed && bs->auth.next_key)) {
...
static void vl_api_bfd_udp_session_dump_t_handler(vl_api_bfd_udp_session_dump_t *mp)
{
    ...
    pool_foreach (bs, bfd_main.sessions) {
        if (bs->transport == BFD_TRANSPORT_UDP4 || bs->transport == BFD_TRANSPORT_UDP6) send_bfd_udp_session_details(reg, mp->context, bs);
    }
}
...
static void vl_api_bfd_udp_session_set_flags_t_handler(vl_api_bfd_udp_session_set_flags_t *mp)
{
    ...
    rv = bfd_udp_session_set_flags(vlib_get_main(), BFD_UDP_API_PARAM_FROM_MP(mp), clib_net_to_host_u32(mp->flags) & IF_STATUS_API_FLAG_ADMIN_UP);
...
static void vl_api_bfd_auth_set_key_t_handler(vl_api_bfd_auth_set_key_t *mp)
{
    ...
    int                              rv = bfd_auth_set_key(clib_net_to_host_u32(mp->conf_key_id), mp->auth_type, mp->key_len, mp->key);
...
static void vl_api_bfd_udp_auth_activate_t_handler(vl_api_bfd_udp_auth_activate_t *mp)
{
    ...
    rv = bfd_udp_auth_activate(BFD_UDP_API_PARAM_FROM_MP(mp), clib_net_to_host_u32(mp->conf_key_id), mp->bfd_key_id, mp->is_delayed);
...
static void vl_api_bfd_udp_auth_deactivate_t_handler(vl_api_bfd_udp_auth_deactivate_t *mp)
{
    ...
    rv = bfd_udp_auth_deactivate(BFD_UDP_API_PARAM_FROM_MP(mp), mp->is_delayed);
