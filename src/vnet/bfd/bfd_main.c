...
static void bfd_validate_counters(bfd_main_t *bm)
{
    vlib_validate_combined_counter(&bm->rx_counter, pool_elts(bm->sessions));
    vlib_validate_combined_counter(&bm->rx_echo_counter, pool_elts(bm->sessions));
    vlib_validate_combined_counter(&bm->tx_counter, pool_elts(bm->sessions));
    vlib_validate_combined_counter(&bm->tx_echo_counter, pool_elts(bm->sessions));
}
...
always_inline u64 bfd_time_now_nsec(vlib_main_t *vm, f64 *vm_time)
{
    f64 _vm_time = vlib_time_now(vm);
    if (vm_time) *vm_time = _vm_time;
...
u8 *format_bfd_auth_key(u8 *s, va_list *args)
{
    ...
    if (key) {
        s = format(s,
                   ...
                   key->auth_type,
                   bfd_auth_type_str(key->auth_type),
                   key->conf_key_id,
                   key->use_count);
    }
    else {
...
static void bfd_set_diag(bfd_session_t *bs, bfd_diag_code_e code)
{
    if (bs->local_diag != code) {
        BFD_DBG("set local_diag, bs_idx=%d: '%d:%s'", bs->bs_idx, code, bfd_diag_code_string(code));
        bs->local_diag = code;
    }
}
...
static void bfd_set_state(vlib_main_t *vm, bfd_main_t *bm, bfd_session_t *bs, bfd_state_e new_state, int handling_wakeup)
{
    if (bs->local_state != new_state) {
        BFD_DBG("Change state, bs_idx=%d: %s->%s", bs->bs_idx, bfd_state_string(bs->local_state), bfd_state_string(new_state));
        bs->local_state = new_state;
        bfd_on_state_change(bm, bs, bfd_time_now_nsec(vm, NULL), handling_wakeup);
    }
}
...
static void bfd_set_poll_state(bfd_session_t *bs, bfd_poll_state_e state)
{
    if (bs->poll_state != state) {
        BFD_DBG("Setting poll state=%s, bs_idx=%u", bfd_poll_state_string(state), bs->bs_idx);
        bs->poll_state = state;
    }
}

static void bfd_recalc_tx_interval(bfd_session_t *bs)
{
    bs->transmit_interval_nsec = clib_max(bs->effective_desired_min_tx_nsec, bs->remote_min_rx_nsec);
    BFD_DBG("Recalculated transmit interval " BFD_CLK_FMT, BFD_CLK_PRN(bs->transmit_interval_nsec));
}

static void bfd_recalc_echo_tx_interval(bfd_session_t *bs)
{
    bs->echo_transmit_interval_nsec = clib_max(bs->effective_desired_min_tx_nsec, bs->remote_min_echo_rx_nsec);
    BFD_DBG("Recalculated echo transmit interval " BFD_CLK_FMT, BFD_CLK_PRN(bs->echo_transmit_interval_nsec));
}
...
static void bfd_calc_next_tx(bfd_main_t *bm, bfd_session_t *bs, u64 now)
{
    if (bs->local_detect_mult > 1) {
        ...
        bs->tx_timeout_nsec = bs->last_tx_nsec + (1 - .25 * (random_f64(&bm->random_seed))) * bs->transmit_interval_nsec;
        if (bs->tx_timeout_nsec < now) {
            ...
            BFD_DBG(
                ...
                "tx_timeout is %lu)",
                (now - bs->tx_timeout_nsec) / bs->transmit_interval_nsec,
                now,
                bs->tx_timeout_nsec);
        ...
        bs->tx_timeout_nsec = bs->last_tx_nsec + (.9 - .15 * (random_f64(&bm->random_seed))) * bs->transmit_interval_nsec;
        if (bs->tx_timeout_nsec < now) {
            /*
             * the timeout is in the past, which means that either remote
             * demand mode was set or performance/clock issues ...
    ...
    if (bs->tx_timeout_nsec) {
        BFD_DBG("Next transmit in %lu nsec/%.02fs@%lu", bs->tx_timeout_nsec - now, (bs->tx_timeout_nsec - now) * SEC_PER_NSEC, bs->tx_timeout_nsec);
    }
}

static void bfd_calc_next_echo_tx(bfd_session_t *bs, u64 now)
{
    bs->echo_tx_timeout_nsec = bs->echo_last_tx_nsec + bs->echo_transmit_interval_nsec;
    if (bs->echo_tx_timeout_nsec < now) {
        ...
        BFD_DBG(
            ...
            "is %lu)",
            (now - bs->echo_tx_timeout_nsec) / bs->echo_transmit_interval_nsec,
            now,
...
static void bfd_recalc_detection_time(bfd_session_t *bs)
{
    if (bs->local_state == BFD_STATE_init || bs->local_state == BFD_STATE_up) {
        bs->detection_time_nsec = bs->remote_detect_mult * clib_max(bs->effective_required_min_rx_nsec, bs->remote_desired_min_tx_nsec);
        BFD_DBG("Recalculated detection time %lu nsec/%.3fs", bs->detection_time_nsec, bs->detection_time_nsec * SEC_PER_NSEC);
    }
}
...
static void bfd_set_timer(bfd_main_t *bm, bfd_session_t *bs, u64 now, int handling_wakeup)
{
    ...
    if (BFD_STATE_up != bs->local_state || (!bs->remote_demand && bs->remote_min_rx_usec) || BFD_POLL_NOT_NEEDED != bs->poll_state) {
    ...
    BFD_DBG(
        ...
        bs->echo_tx_timeout_nsec,
        rx_timeout,
        next == tx_timeout ? "tx" : (next == bs->echo_tx_timeout_nsec ? "echo tx" : "rx"));
    if (next) {
        int send_signal     = 0;
        bs->event_time_nsec = next;
        /* add extra tick if it's not even */
        u32 wheel_time_ticks = (bs->event_time_nsec - now) / bm->nsec_per_tw_tick + ((bs->event_time_nsec - now) % bm->nsec_per_tw_tick != 0);
        BFD_DBG(
            "event_time_nsec %lu (%lu nsec/%.3fs in future) -> "
            "wheel_time_ticks %u",
            bs->event_time_nsec,
            bs->event_time_nsec - now,
            (bs->event_time_nsec - now) * SEC_PER_NSEC,
            wheel_time_ticks);
        wheel_time_ticks = wheel_time_ticks ? wheel_time_ticks : 1;
        bfd_lock(bm);
        if (bs->tw_id) {
            TW(tw_timer_update)(&bm->wheel, bs->tw_id, wheel_time_ticks);
            BFD_DBG("tw_timer_update(%p, %u, %lu);", &bm->wheel, bs->tw_id, wheel_time_ticks);
        }
        else {
            bs->tw_id = TW(tw_timer_start)(&bm->wheel, bs->bs_idx, 0, wheel_time_ticks);
            BFD_DBG("tw_timer_start(%p, %u, 0, %lu) == %u;", &bm->wheel, bs->bs_idx, wheel_time_ticks);
        }

        if (!handling_wakeup) {
            /* Send only if it is earlier than current awaited wakeup time */
            send_signal = (bs->event_time_nsec < bm->bfd_process_next_wakeup_nsec) &&
                          /*
                           ...
                           */
                          (bm->bfd_process_next_wakeup_nsec - bs->event_time_nsec > 2 * bm->bfd_process_wakeup_event_delay_nsec) &&
        ...
        if (send_signal) {
            vlib_process_signal_event_mt(bm->vlib_main, bm->bfd_process_node_index, BFD_EVENT_RESCHEDULE, ~0);
        }
    }
}
...
static void bfd_set_effective_desired_min_tx(bfd_main_t *bm, bfd_session_t *bs, u64 now, u64 desired_min_tx_nsec)
{
    ...
    BFD_DBG("Set effective desired min tx to " BFD_CLK_FMT, BFD_CLK_PRN(bs->effective_desired_min_tx_nsec));
...
static void bfd_set_effective_required_min_rx(bfd_session_t *bs, u64 required_min_rx_nsec)
{
    bs->effective_required_min_rx_nsec = required_min_rx_nsec;
    BFD_DBG("Set effective required min rx to " BFD_CLK_FMT, BFD_CLK_PRN(bs->effective_required_min_rx_nsec));
    bfd_recalc_detection_time(bs);
}

static void bfd_set_remote_required_min_rx(bfd_session_t *bs, u32 remote_required_min_rx_usec)
{
    if (bs->remote_min_rx_usec != remote_required_min_rx_usec) {
        ...
        BFD_DBG("Set remote min rx to " BFD_CLK_FMT, BFD_CLK_PRN(bs->remote_min_rx_nsec));
        bfd_recalc_detection_time(bs);
        bfd_recalc_tx_interval(bs);
    }
}

static void bfd_set_remote_required_min_echo_rx(bfd_session_t *bs, u32 remote_required_min_echo_rx_usec)
{
    if (bs->remote_min_echo_rx_usec != remote_required_min_echo_rx_usec) {
        ...
        bs->remote_min_echo_rx_nsec = bfd_usec_to_nsec(bs->remote_min_echo_rx_usec);
        BFD_DBG("Set remote min echo rx to " BFD_CLK_FMT, BFD_CLK_PRN(bs->remote_min_echo_rx_nsec));
        bfd_recalc_echo_tx_interval(bs);
    }
}
...
void bfd_session_start(bfd_main_t *bm, bfd_session_t *bs)
{
    ...
    vlib_log_info(bm->log_class, "start BFD session: %U", format_bfd_session_brief, bs);
    bfd_set_effective_required_min_rx(bs, bs->config_required_min_rx_nsec);
    bfd_recalc_tx_interval(bs);
    vlib_process_signal_event(bm->vlib_main, bm->bfd_process_node_index, BFD_EVENT_NEW_SESSION, bs->bs_idx);
...
void bfd_session_set_flags(vlib_main_t *vm, bfd_session_t *bs, u8 admin_up_down)
{
    ...
    if (admin_up_down) {
        ...
        vlib_log_info(bm->log_class, "set session admin-up: %U", format_bfd_session_brief, bs);
        ...
        vlib_log_info(bm->log_class, "set session admin-down: %U", format_bfd_session_brief, bs);
...
u8 *format_bfd_pkt(u8 *s, va_list *args)
{
    ...
    if (len > STRUCT_SIZE_OF(bfd_pkt_t, head)) {
        ...
        if (len >= sizeof(bfd_pkt_t) && pkt->head.length >= sizeof(bfd_pkt_t)) {
            ...
            s = format(s, "    required min echo rx interval: %u", clib_net_to_host_u32(pkt->req_min_echo_rx));
        }
        if (len >= sizeof(bfd_pkt_with_common_auth_t) && pkt->head.length >= sizeof(bfd_pkt_with_common_auth_t) && bfd_pkt_get_auth_present(pkt)) {
            const bfd_pkt_with_common_auth_t *with_auth = (void *) pkt;
            const bfd_auth_common_t          *common    = &with_auth->common_auth;
            s                                           = format(s, "\n    auth len: %u\n", common->len);
            s                                           = format(s, "    auth type: %u:%s", common->type, bfd_auth_type_str(common->type));
            if (len >= sizeof(bfd_pkt_with_sha1_auth_t) && pkt->head.length >= sizeof(bfd_pkt_with_sha1_auth_t)
                && (BFD_AUTH_TYPE_keyed_sha1 == common->type || BFD_AUTH_TYPE_meticulous_keyed_sha1 == common->type)) {
                ...
                s                                         = format(s, "    key id: %u\n", sha1->key_id);
                s                                         = format(s, "    hash: %U", format_hex_bytes, sha1->hash, sizeof(sha1->hash));
            }
        }
    }
...
static void bfd_rpc_event_cb(const bfd_rpc_event_t *a)
{
    ...
    if (valid_bs) bfd_event(bm, &session_data);
}
...
static void bfd_rpc_notify_listeners_cb(const bfd_rpc_notify_listeners_t *a)
{
    ...
    if (!pool_is_free_index(bm->sessions, bs_idx)) {
        ...
        BFD_DBG("Ignoring notify RPC for non-existent session index %u", bs_idx);
    }
    bfd_unlock(bm);
}
...
static void bfd_on_state_change(bfd_main_t *bm, bfd_session_t *bs, u64 now, int handling_wakeup)
{
    ...
    switch (bs->local_state) {
        case BFD_STATE_admin_down:
            bs->echo = 0;
            bfd_set_effective_desired_min_tx(bm, bs, now, clib_max(bs->config_desired_min_tx_nsec, bm->default_desired_min_tx_nsec));
            bfd_set_effective_required_min_rx(bs, bs->config_required_min_rx_nsec);
        ...
        case BFD_STATE_init:
            bs->echo = 0;
            bfd_set_effective_desired_min_tx(bm, bs, now, bs->config_desired_min_tx_nsec);
            bfd_set_timer(bm, bs, now, handling_wakeup);
            break;
        case BFD_STATE_up:
            bfd_set_effective_desired_min_tx(bm, bs, now, bs->config_desired_min_tx_nsec);
...
static void bfd_on_config_change(bfd_main_t *bm, bfd_session_t *bs, u64 now)
{
    /*
     * if remote demand mode is set and we need to do a poll, set the next
     * timeout so that the session wakes up immediately
     */
    if (bs->remote_demand && BFD_POLL_NEEDED == bs->poll_state && bs->poll_state_start_or_timeout_nsec < now) {
        bs->tx_timeout_nsec = now;
    }
    bfd_recalc_detection_time(bs);
    bfd_set_timer(bm, bs, now, 0);
...
static int bfd_transport_control_frame(vlib_main_t *vm, vlib_node_runtime_t *rt, u32 bi, bfd_session_t *bs)
{
    switch (bs->transport) {
        case BFD_TRANSPORT_UDP4:
            BFD_DBG("Transport bfd via udp4, bs_idx=%u", bs->bs_idx);
...
static int bfd_transport_echo(vlib_main_t *vm, vlib_node_runtime_t *rt, u32 bi, bfd_session_t *bs)
{
    switch (bs->transport) {
        case BFD_TRANSPORT_UDP4:
            BFD_DBG("Transport bfd echo via udp4, bs_idx=%u", bs->bs_idx);
...
static void bfd_add_sha1_auth_section(vlib_main_t *vm, vlib_buffer_t *b, bfd_session_t *bs)
{
     ...
     */
    clib_memcpy(auth->hash, bs->auth.curr_key->key, sizeof(bs->auth.curr_key->key));
    ...
    if (bs->auth.curr_key) {
        ...
        switch (auth_type) {
            ...
            case BFD_AUTH_TYPE_meticulous_keyed_md5:
                vlib_log_crit(bm->log_class, "internal error, unexpected BFD auth type '%d'", auth_type);
                break;
...
{
    if (BFD_STATE_up == bs->local_state && BFD_STATE_up == bs->remote_state && bs->remote_min_echo_rx_usec > 0) {
        switch (bs->transport) {
            case BFD_TRANSPORT_UDP4:
...
static void bfd_init_control_frame(bfd_session_t *bs, vlib_buffer_t *b)
{
    ...
    if (bs->echo) {
        pkt->req_min_rx = clib_host_to_net_u32(bfd_nsec_to_usec(bs->effective_required_min_rx_nsec));
    }
    else {
...
static void bfd_process_trace_buf(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_buffer_t *b, bfd_session_t *bs)
{
    ...
    if (n_trace > 0) {
        ...
        if (vlib_trace_buffer(vm, rt, 0, b, 0)) {
            ...
            tr->bs_idx = bs->bs_idx;
            u64 len    = (b->current_length < sizeof(tr->data)) ? b->current_length : sizeof(tr->data);
            tr->len    = len;
...
static void bfd_send_echo(vlib_main_t *vm, vlib_node_runtime_t *rt, bfd_main_t *bm, bfd_session_t *bs, u64 now)
{
    ...
    if (now >= bs->echo_tx_timeout_nsec) {
        ...
        pkt->expire_time_nsec = now + bs->echo_transmit_interval_nsec * bs->local_detect_mult;
        pkt->checksum         = bfd_calc_echo_checksum(bs->local_discr, pkt->expire_time_nsec, bs->echo_secret);
        ...
        BFD_DBG("No need to send echo packet now, now is %lu, tx_timeout is %lu", now, bs->echo_tx_timeout_nsec);
    }
}
...
static void bfd_send_periodic(vlib_main_t *vm, vlib_node_runtime_t *rt, bfd_main_t *bm, bfd_session_t *bs, u64 now)
{
    ...
    if (BFD_POLL_NOT_NEEDED == bs->poll_state && bs->remote_demand && BFD_STATE_up == bs->local_state && BFD_STATE_up == bs->remote_state) {
        /*
    ...
    if (now >= bs->tx_timeout_nsec) {
        BFD_DBG("\nSending periodic control frame: %U", format_bfd_session, bs);
        u32 bi;
        if (vlib_buffer_alloc(vm, &bi, 1) != 1) {
            vlib_log_crit(bm->log_class, "buffer allocation failure");
            return;
        ...
        switch (bs->poll_state) {
            case BFD_POLL_NEEDED:
                if (now < bs->poll_state_start_or_timeout_nsec) {
                    BFD_DBG("Cannot start a poll sequence yet, need to wait for " BFD_CLK_FMT,
                            BFD_CLK_PRN(bs->poll_state_start_or_timeout_nsec - now));
                    break;
                }
        ...
        BFD_DBG("No need to send control frame now, now is %lu, tx_timeout is %lu", now, bs->tx_timeout_nsec);
    }
}
...
static void bfd_check_rx_timeout(vlib_main_t *vm, bfd_main_t *bm, bfd_session_t *bs, u64 now, int handling_wakeup)
{
    ...
    else if (bs->echo && bs->echo_last_rx_nsec + bs->echo_transmit_interval_nsec * bs->local_detect_mult <= now) {
...
void bfd_on_timeout(vlib_main_t *vm, vlib_node_runtime_t *rt, bfd_main_t *bm, bfd_session_t *bs, u64 now)
{
    ...
    switch (bs->local_state) {
        ...
        case BFD_STATE_up:
            bfd_check_rx_timeout(vm, bm, bs, now, 1);
            if (BFD_POLL_NOT_NEEDED == bs->poll_state && !bs->echo && bfd_is_echo_possible(bs)) {
                ...
                bs->echo_tx_timeout_nsec = now;
                bfd_set_effective_required_min_rx(bs, clib_max(bm->min_required_min_rx_while_echo_nsec, bs->config_required_min_rx_nsec));
...
u8 *format_bfd_process_trace(u8 *s, va_list *args)
{
    CLIB_UNUSED(vlib_main_t * vm)   = va_arg(*args, vlib_main_t *);
    CLIB_UNUSED(vlib_node_t * node) = va_arg(*args, vlib_node_t *);
    bfd_process_trace_t *t          = va_arg(*args, bfd_process_trace_t *);

    s = format(s, "bs_idx=%u => %U", t->bs_idx, format_bfd_pkt, t->len, t->data);
 ...
 */
static uword bfd_process(vlib_main_t *vm, vlib_node_runtime_t *rt, CLIB_UNUSED(vlib_frame_t *f))
{
    bfd_main_t *bm      = &bfd_main;
    u32        *expired = 0;
    ...
    while (1) {
        ...
        BFD_DBG("wakeup, now is %llunsec, vlib_time_now() is %.9f", now, vm_time);
        bfd_lock(bm);
        f64 timeout;
        if (pool_elts(bm->sessions)) {
            u32 first_expires_in_ticks = TW(tw_timer_first_expires_in_ticks)(&bm->wheel);
            if (!first_expires_in_ticks) {
                BFD_DBG("tw_timer_first_expires_in_ticks(%p) returns 0ticks", &bm->wheel);
                timeout = bm->wheel.next_run_time - vm_time;
                BFD_DBG("wheel.next_run_time is %.9f", bm->wheel.next_run_time);
                u64 next_expire_nsec             = now + timeout * SEC_PER_NSEC;
                bm->bfd_process_next_wakeup_nsec = next_expire_nsec;
                bfd_unlock(bm);
            }
            else {
                BFD_DBG("tw_timer_first_expires_in_ticks(%p) returns %luticks", &bm->wheel, first_expires_in_ticks);
                u64 next_expire_nsec             = now + first_expires_in_ticks * bm->nsec_per_tw_tick;
                bm->bfd_process_next_wakeup_nsec = next_expire_nsec;
                bfd_unlock(bm);
            ...
            BFD_DBG("vlib_process_wait_for_event_or_clock(vm, %.09f)", timeout);
        ...
        switch (event_type) {
            ...
            case BFD_EVENT_RESCHEDULE:
                ...
                bfd_lock(bm);
                bm->bfd_process_wakeup_event_delay_nsec = now - bm->bfd_process_wakeup_event_start_nsec;
                bm->bfd_process_wakeup_events_in_flight--;
                bfd_unlock(bm);
            ...
            case BFD_EVENT_NEW_SESSION:
                vec_foreach (session_index, event_data) {
                    ...
                    if (!pool_is_free_index(bm->sessions, *session_index)) {
                        bfd_session_t *bs = pool_elt_at_index(bm->sessions, *session_index);
                        bfd_send_periodic(vm, rt, bm, bs, now);
                        bfd_set_timer(bm, bs, now, 1);
                    }
                    else {
                        BFD_DBG("Ignoring event for non-existent session index %u", (u32) *session_index);
                    }
                    bfd_unlock(bm);
                }
                break;
        ...
        bfd_lock(bm);
        expired = TW(tw_timer_expire_timers_vec)(&bm->wheel, vm_time, expired);
...
static clib_error_t *bfd_sw_interface_up_down(CLIB_UNUSED(vnet_main_t *vnm), CLIB_UNUSED(u32 sw_if_index), u32 flags)
{
    // bfd_main_t *bm = &bfd_main;
...
static clib_error_t *bfd_hw_interface_up_down(CLIB_UNUSED(vnet_main_t *vnm), CLIB_UNUSED(u32 hw_if_index), u32 flags)
{
    // bfd_main_t *bm = &bfd_main;
...
static clib_error_t *bfd_main_init(vlib_main_t *vm)
{
    ...
    bm->default_desired_min_tx_nsec         = bfd_usec_to_nsec(BFD_DEFAULT_DESIRED_MIN_TX_USEC);
    bm->min_required_min_rx_while_echo_nsec = bfd_usec_to_nsec(BFD_REQUIRED_MIN_RX_USEC_WHILE_ECHO);
    BFD_DBG("tw_timer_wheel_init(%p, %p, %.04f, %u)", &bm->wheel, NULL, 1.00 / BFD_TW_TPS, ~0);
    ...
    if (n_vlib_mains > 1) clib_spinlock_init(&bm->lock);
...
void bfd_put_session(bfd_main_t *bm, bfd_session_t *bs)
{
    ...
    vlib_log_info(bm->log_class, "delete session: %U", format_bfd_session_brief, bs);
    if (bs->auth.curr_key) {
...
bfd_error_t bfd_verify_pkt_common(const bfd_pkt_t *pkt)
{
    if (1 != bfd_pkt_get_version(pkt)) {
        BFD_ERR("BFD verification failed - unexpected version: '%d'", bfd_pkt_get_version(pkt));
        return BFD_ERROR_VERSION;
    }
    if (pkt->head.length < sizeof(bfd_pkt_t) || (bfd_pkt_get_auth_present(pkt) && pkt->head.length < sizeof(bfd_pkt_with_common_auth_t))) {
        BFD_ERR(
    ...
    if (!pkt->head.detect_mult) {
        BFD_ERR("BFD verification failed - unexpected detect-mult: '%d'", pkt->head.detect_mult);
        return BFD_ERROR_DETECT_MULTI;
    }
    if (bfd_pkt_get_multipoint(pkt)) {
        BFD_ERR("BFD verification failed - unexpected multipoint: '%d'", bfd_pkt_get_multipoint(pkt));
        return BFD_ERROR_MULTI_POINT;
    }
    if (!pkt->my_disc) {
        BFD_ERR("BFD verification failed - unexpected my-disc: '%d'", pkt->my_disc);
...
{
    if (BFD_AUTH_TYPE_meticulous_keyed_md5 == auth_type || BFD_AUTH_TYPE_meticulous_keyed_sha1 == auth_type) {
        return 1;
    }
    return 0;
}
...
static int bfd_verify_pkt_auth_seq_num(vlib_main_t *vm, bfd_session_t *bs, u32 received_seq_num, int is_meticulous)
{
    ...
    if (bs->auth.remote_seq_number_known) {
        ...
        if (bs->auth.remote_seq_number > max_u32 - 3 * bs->local_detect_mult) {
            ...
            if (received_seq_num > x && received_seq_num < bs->auth.remote_seq_number + is_meticulous) {
                BFD_ERR("Recvd sequence number=%u out of ranges <0, %u>, <%u, %u>",
                        received_seq_num,
                        x,
                        bs->auth.remote_seq_number + is_meticulous,
                        max_u32);
                return 0;
            }
        }
            ...
            const u32 max = bs->auth.remote_seq_number + 3 * bs->local_detect_mult;
            if (received_seq_num < min || received_seq_num > max) {
                BFD_ERR("Recvd sequence number=%u out of range <%u, %u>", received_seq_num, min, max);
                return 0;
            }
        }
    }
...
static int bfd_verify_pkt_auth_key_sha1(vlib_main_t     *vm,
                                        const bfd_pkt_t *pkt,
                                        u32              pkt_size,
                                        CLIB_UNUSED(bfd_session_t *bs),
                                        u8              bfd_key_id,
                                        bfd_auth_key_t *auth_key)
{
    ASSERT(auth_key->auth_type == BFD_AUTH_TYPE_keyed_sha1 || auth_key->auth_type == BFD_AUTH_TYPE_meticulous_keyed_sha1);
    ...
    if (pkt_size < sizeof(*with_sha1) || with_sha1->sha1_auth.type_len.len < sizeof(with_sha1->sha1_auth)) {
        BFD_ERR(
            "BFD size mismatch, payload size=%u, expected=%u, auth_len=%u, "
            "expected=%u",
            pkt_size,
            sizeof(*with_sha1),
            with_sha1->sha1_auth.type_len.len,
            sizeof(with_sha1->sha1_auth));
        return 0;
    }
    if (with_sha1->sha1_auth.key_id != bfd_key_id) {
        BFD_ERR("BFD key ID mismatch, packet key ID=%u doesn't match key ID=%u%s",
                with_sha1->sha1_auth.key_id,
                bfd_key_id,
                bs->auth.is_delayed ? " (but a delayed auth change is scheduled)" : "");
        return 0;
    }

    u8 hash_from_packet[STRUCT_SIZE_OF(bfd_auth_sha1_t, hash)];
    u8 calculated_hash[STRUCT_SIZE_OF(bfd_auth_sha1_t, hash)];
    clib_memcpy(hash_from_packet, with_sha1->sha1_auth.hash, sizeof(with_sha1->sha1_auth.hash));
    clib_memcpy(with_sha1->sha1_auth.hash, auth_key->key, sizeof(auth_key->key));
    vnet_crypto_op_t op;
    vnet_crypto_op_init(&op, VNET_CRYPTO_OP_SHA1_HASH);
    op.src    = (u8 *) with_sha1;
    ...
    clib_memcpy(with_sha1->sha1_auth.hash, hash_from_packet, sizeof(with_sha1->sha1_auth.hash));

    if (0 == memcmp(calculated_hash, hash_from_packet, sizeof(calculated_hash))) {
        clib_memcpy(with_sha1->sha1_auth.hash, hash_from_packet, sizeof(hash_from_packet));
        return 1;
    }
...
static int bfd_verify_pkt_auth_key(vlib_main_t *vm, const bfd_pkt_t *pkt, u32 pkt_size, bfd_session_t *bs, u8 bfd_key_id, bfd_auth_key_t *auth_key)
{
    ...
    switch (auth_key->auth_type) {
        case BFD_AUTH_TYPE_reserved:
            vlib_log_err(bm->log_class, "internal error, unexpected auth_type=%d:%s", auth_key->auth_type, bfd_auth_type_str(auth_key->auth_type));
            return 0;
        case BFD_AUTH_TYPE_simple_password:
            /* fallthrough */
        case BFD_AUTH_TYPE_keyed_md5:
        ...
        case BFD_AUTH_TYPE_meticulous_keyed_sha1:
            do {
                const u32 seq_num = clib_net_to_host_u32(((bfd_pkt_with_sha1_auth_t *) pkt)->sha1_auth.seq_num);
                return bfd_verify_pkt_auth_seq_num(vm, bs, seq_num, bfd_auth_type_is_meticulous(auth_key->auth_type))
                       && bfd_verify_pkt_auth_key_sha1(vm, pkt, pkt_size, bs, bfd_key_id, auth_key);
            } while (0);
    }
    return 0;
}
    ...
    if (bfd_pkt_get_auth_present(pkt)) {
        ...
        if (!bs->auth.curr_key) {
            ...
            if (bs->auth.is_delayed && bs->auth.next_key) {
                ...
                if (bfd_verify_pkt_auth_key(vm, pkt, pkt_size, bs, bs->auth.next_bfd_key_id, bs->auth.next_key)) {
            ...
            if (bfd_verify_pkt_auth_key(vm, pkt, pkt_size, bs, bs->auth.curr_bfd_key_id, bs->auth.curr_key)) {
...
bfd_error_t bfd_consume_pkt(vlib_main_t *vm, bfd_main_t *bm, const bfd_pkt_t *pkt, u32 bs_idx)
{
    bfd_lock_check(bm);
    ...
    bs->last_rx_nsec  = now;
    if (bfd_pkt_get_auth_present(pkt)) {
        bfd_auth_type_e auth_type = ((bfd_pkt_with_common_auth_t *) (pkt))->common_auth.type;
        switch (auth_type) {
            case BFD_AUTH_TYPE_reserved:
                /* fallthrough */
            case BFD_AUTH_TYPE_simple_password:
            ...
            case BFD_AUTH_TYPE_keyed_md5:
                /* fallthrough */
            case BFD_AUTH_TYPE_meticulous_keyed_md5:
                vlib_log_crit(bm->log_class, "internal error, unexpected auth_type=%d:%s", auth_type, bfd_auth_type_str(auth_type));
                break;
            case BFD_AUTH_TYPE_keyed_sha1:
                /* fallthrough */
            case BFD_AUTH_TYPE_meticulous_keyed_sha1:
                do {
                    bfd_pkt_with_sha1_auth_t *with_sha1 = (bfd_pkt_with_sha1_auth_t *) pkt;
                    bs->auth.remote_seq_number          = clib_net_to_host_u32(with_sha1->sha1_auth.seq_num);
                    bs->auth.remote_seq_number_known    = 1;
                    BFD_DBG("Received sequence number %u", bs->auth.remote_seq_number);
                } while (0);
        }
    }
    bs->remote_desired_min_tx_nsec = bfd_usec_to_nsec(clib_net_to_host_u32(pkt->des_min_tx));
    bs->remote_detect_mult         = pkt->head.detect_mult;
    bfd_set_remote_required_min_rx(bs, clib_net_to_host_u32(pkt->req_min_rx));
    bfd_set_remote_required_min_echo_rx(bs, clib_net_to_host_u32(pkt->req_min_echo_rx));
    if (bfd_pkt_get_final(pkt)) {
        if (BFD_POLL_IN_PROGRESS == bs->poll_state) {
            ...
            if (BFD_STATE_up == bs->local_state) {
                bfd_set_effective_desired_min_tx(bm, bs, now, bs->config_desired_min_tx_nsec);
                bfd_set_effective_required_min_rx(bs, clib_max(bs->echo * bm->min_required_min_rx_while_echo_nsec, bs->config_required_min_rx_nsec));
            }
        }
             ...
             */
            BFD_DBG("Next poll sequence can commence in " BFD_CLK_FMT, BFD_CLK_PRN(now - bs->poll_state_start_or_timeout_nsec));
            bs->poll_state_start_or_timeout_nsec = now + (now - bs->poll_state_start_or_timeout_nsec);
            BFD_DBG("Poll sequence terminated, but another is needed, bs_idx=%u", bs->bs_idx);
            bfd_set_poll_state(bs, BFD_POLL_NEEDED);
        }
    }
    bfd_calc_next_tx(bm, bs, now);
    ...
    if (BFD_STATE_admin_down == bs->local_state) {
        BFD_DBG("Session is admin-down, ignoring packet, bs_idx=%u", bs->bs_idx);
    ...
    if (BFD_STATE_admin_down == bs->remote_state) {
        ...
        if (BFD_STATE_up == bs->remote_state || BFD_STATE_init == bs->remote_state) {
            bfd_set_diag(bs, BFD_DIAG_CODE_no_diag);
            bfd_set_state(vm, bm, bs, BFD_STATE_up, 0);
        }
    }
...
bfd_session_t *bfd_consume_echo_pkt(vlib_main_t *vm, bfd_main_t *bm, vlib_buffer_t *b)
{
    ...
    u64 checksum = bfd_calc_echo_checksum(bs->local_discr, pkt->expire_time_nsec, bs->echo_secret);
    ...
    if (pkt->expire_time_nsec < now) {
        BFD_DBG("Stale packet received, expire time %lu < now %lu", pkt->expire_time_nsec, now);
    }
    else {
        bs->echo_last_rx_nsec = now;
    }
...
u8 *format_bfd_session(u8 *s, va_list *args)
{
    ...
    s                       = format(s,
               ...
               bs->config_required_min_rx_usec,
               1,
               bs->local_detect_mult,
               bs->remote_min_rx_usec,
               bs->remote_min_echo_rx_usec,
...
{
    const bfd_session_t *bs = va_arg(*args, bfd_session_t *);
    s = format(s, "bs_idx=%u local-state=%s remote-state=%s", bs->bs_idx, bfd_state_string(bs->local_state), bfd_state_string(bs->remote_state));
    return s;
}

unsigned bfd_auth_type_supported(bfd_auth_type_e auth_type)
{
    if (auth_type == BFD_AUTH_TYPE_keyed_sha1 || auth_type == BFD_AUTH_TYPE_meticulous_keyed_sha1) {
        return 1;
    }
    return 0;
}
...
vnet_api_error_t bfd_auth_activate(bfd_session_t *bs, u32 conf_key_id, u8 bfd_key_id, u8 is_delayed)
{
    ...
    if (!key_idx_p) {
        vlib_log_err(bm->log_class, "authentication key with config ID %u doesn't exist)", conf_key_id);
    ...
    vlib_log_info(bm->log_class, "session auth modified: %U", format_bfd_session_brief, bs);
    return 0;
}
...
{
    if (bs->local_detect_mult != detect_mult || bs->config_desired_min_tx_usec != desired_min_tx_usec
        || bs->config_required_min_rx_usec != required_min_rx_usec) {
        BFD_DBG("\nChanging session params: %U", format_bfd_session, bs);
        switch (bs->poll_state) {
            case BFD_POLL_NOT_NEEDED:
                if (BFD_STATE_up == bs->local_state || BFD_STATE_init == bs->local_state) {
                    /* poll sequence is not needed for detect multiplier change */
                    if (bs->config_desired_min_tx_usec != desired_min_tx_usec || bs->config_required_min_rx_usec != required_min_rx_usec) {
                        bfd_set_poll_state(bs, BFD_POLL_NEEDED);
                    }
                }
                break;
            ...
            case BFD_POLL_IN_PROGRESS:
                /* poll sequence is not needed for detect multiplier change */
                if (bs->config_desired_min_tx_usec != desired_min_tx_usec || bs->config_required_min_rx_usec != required_min_rx_usec) {
                    BFD_DBG("Poll in progress, queueing extra poll, bs_idx=%u", bs->bs_idx);
        ...
        bs->config_required_min_rx_usec = required_min_rx_usec;
        bs->config_required_min_rx_nsec = bfd_usec_to_nsec(required_min_rx_usec);
        BFD_DBG("\nChanged session params: %U", format_bfd_session, bs);

        vlib_log_info(bm->log_class, "changed session params: %U", format_bfd_session_brief, bs);
        vlib_process_signal_event(bm->vlib_main, bm->bfd_process_node_index, BFD_EVENT_CONFIG_CHANGED, bs->bs_idx);
    }
    else {
...
vnet_api_error_t bfd_auth_set_key(u32 conf_key_id, u8 auth_type, u8 key_len, const u8 *key_data)
{
    ...
    if (!bfd_auth_type_supported(auth_type)) {
        vlib_log_err(bm->log_class, "unsupported auth type=%d:%s", auth_type, bfd_auth_type_str(auth_type));
    ...
    if (key_idx_p) {
        ...
        hash_set(bm->auth_key_by_conf_key_id, conf_key_id, auth_key - bm->auth_keys);
    }
...
vnet_api_error_t bfd_auth_del_key(u32 conf_key_id)
{
    ...
    if (key_idx_p) {
        ...
        /* no such key */
        vlib_log_err(bm->log_class, "authentication key with conf ID %u does not exist", conf_key_id);
        return VNET_API_ERROR_BFD_ENOENT;
    }
    return 0;
}
