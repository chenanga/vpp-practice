...
static inline void bfd_lock(bfd_main_t *bm)
{
...
static inline void bfd_unlock(bfd_main_t *bm)
{
    uword my_thread_index = __os_thread_index;
...
static inline void bfd_lock_check(bfd_main_t *bm)
{
    if (PREDICT_FALSE(bm->lock_recursion_count < 1)) clib_warning("lock check failure");
}
