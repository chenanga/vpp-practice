...
static u8 *format_bfd_session_cli(u8 *s, va_list *args)
{
    ...
    switch (bs->transport) {
        case BFD_TRANSPORT_UDP4:
            s = format(s,
                       ...
                       "IPv4 address",
                       format_ip4_address,
                       bs->udp.key.local_addr.ip4.as_u8,
                       format_ip4_address,
                       bs->udp.key.peer_addr.ip4.as_u8);
            break;
        case BFD_TRANSPORT_UDP6:
            s = format(s,
                       ...
                       "IPv6 address",
                       format_ip6_address,
                       &bs->udp.key.local_addr.ip6,
                       format_ip6_address,
                       &bs->udp.key.peer_addr.ip6);
            break;
    }
    s       = format(s, "%10s %-32s %20s %20s\n", "", "Session state", bfd_state_string(bs->local_state), bfd_state_string(bs->remote_state));
    s       = format(s, "%10s %-32s %20s %20s\n", "", "Diagnostic code", bfd_diag_code_string(bs->local_diag), bfd_diag_code_string(bs->remote_diag));
    s       = format(s, "%10s %-32s %20u %20u\n", "", "Detect multiplier", bs->local_detect_mult, bs->remote_detect_mult);
    s       = format(s, "%10s %-32s %20u %20llu\n", "", "Required Min Rx Interval (usec)", bs->config_required_min_rx_usec, bs->remote_min_rx_usec);
    s       = format(s,
               "%10s %-32s %20u %20u\n",
               "",
    ...
    s       = format(s, "%10s %-32s %20u\n", "", "Transmit interval", bfd_nsec_to_usec(bs->transmit_interval_nsec));
    u64 now = clib_cpu_time_now();
    u8 *tmp = NULL;
    if (bs->last_tx_nsec > 0) {
        tmp = format(tmp, "%.2fs ago", (now - bs->last_tx_nsec) * vm->clib_time.seconds_per_clock);
        s   = format(s, "%10s %-32s %20v\n", "", "Last control frame tx", tmp);
        vec_reset_length(tmp);
    }
    if (bs->last_rx_nsec) {
        tmp = format(tmp, "%.2fs ago", (now - bs->last_rx_nsec) * vm->clib_time.seconds_per_clock);
        s   = format(s, "%10s %-32s %20v\n", "", "Last control frame rx", tmp);
        vec_reset_length(tmp);
    }
    s = format(s, "%10s %-32s %20u %20llu\n", "", "Min Echo Rx Interval (usec)", 1, bs->remote_min_echo_rx_usec);
    if (bs->echo) {
        s   = format(s, "%10s %-32s %20u\n", "", "Echo transmit interval", bfd_nsec_to_usec(bs->echo_transmit_interval_nsec));
        tmp = format(tmp, "%.2fs ago", (now - bs->echo_last_tx_nsec) * vm->clib_time.seconds_per_clock);
        s   = format(s, "%10s %-32s %20v\n", "", "Last echo frame tx", tmp);
        vec_reset_length(tmp);
        tmp = format(tmp, "%.6fs", (bs->echo_last_rx_nsec - bs->echo_last_tx_nsec) * vm->clib_time.seconds_per_clock);
        s   = format(s, "%10s %-32s %20v\n", "", "Last echo frame roundtrip time", tmp);
    }
    vec_free(tmp);
    tmp = NULL;
    s   = format(s, "%10s %-32s %20s %20s\n", "", "Demand mode", "no", bs->remote_demand ? "yes" : "no");
    s   = format(s, "%10s %-32s %20s\n", "", "Poll state", bfd_poll_state_string(bs->poll_state));
    if (bs->auth.curr_key) {
        s = format(s, "%10s %-32s %20u\n", "", "Authentication config key ID", bs->auth.curr_key->conf_key_id);
        s = format(s, "%10s %-32s %20u\n", "", "Authentication BFD key ID", bs->auth.curr_bfd_key_id);
        s = format(s, "%10s %-32s %20u %20u\n", "", "Sequence number", bs->auth.local_seq_number, bs->auth.remote_seq_number);
    }
    return s;
}
...
static clib_error_t *show_bfd(vlib_main_t *vm, unformat_input_t *input, CLIB_UNUSED(vlib_cli_command_t *lmd))
{
    ...
    if (unformat(line_input, "keys")) {
        ...
        u8             *s   = format(NULL, "%=10s %=25s %=10s\n", "Configuration Key ID", "Type", "Use Count");
        pool_foreach (key, bm->auth_keys) {
            s = format(s, "%10u %-25s %10u\n", key->conf_key_id, bfd_auth_type_str(key->auth_type), key->use_count);
        }
        ...
        int           have_usable_ip6;
        ip6_address_t ip6;
        bfd_udp_get_echo_source(&is_set, &sw_if_index, &have_usable_ip4, &ip4, &have_usable_ip6, &ip6);
        if (is_set) {
            vnet_sw_interface_t *sw_if = vnet_get_sw_interface_or_null(&vnet_main, sw_if_index);
            vnet_hw_interface_t *hw_if = vnet_get_hw_interface(&vnet_main, sw_if->hw_if_index);
...
static clib_error_t *bfd_cli_key_add(vlib_main_t *vm, unformat_input_t *input, CLIB_UNUSED(vlib_cli_command_t *lmd))
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(line_input, "type %U", unformat_token, "a-zA-Z0-9-", &vec_auth_type)) {
            if (vec_len(vec_auth_type) == sizeof(keyed_sha1) - 1 && 0 == memcmp(vec_auth_type, keyed_sha1, sizeof(keyed_sha1) - 1)) {
                auth_type = BFD_AUTH_TYPE_keyed_sha1;
            }
            else if (vec_len(vec_auth_type) == sizeof(meticulous_keyed_sha1) - 1
                     && 0 == memcmp(vec_auth_type, meticulous_keyed_sha1, sizeof(meticulous_keyed_sha1) - 1)) {
        ...
        else if (unformat(line_input, "secret %U", unformat_hex_string, &secret)) {
            /* nothing to do here */
        }
        else {
            ret = clib_error_return(0, "Unknown input `%U'", format_unformat_error, line_input);
            goto out;
        }
    }
    ...
    vnet_api_error_t rv = bfd_auth_set_key(key_id, auth_type, vec_len(secret), secret);
    if (rv) {
        ret = clib_error_return(0, "`bfd_auth_set_key' API call failed, rv=%d:%U", (int) rv, format_vnet_api_errno, rv);
    }
...
static clib_error_t *bfd_cli_key_del(vlib_main_t *vm, unformat_input_t *input, CLIB_UNUSED(vlib_cli_command_t *lmd))
{
    ...
    if (rv) {
        ret = clib_error_return(0, "`bfd_auth_del_key' API call failed, rv=%d:%U", (int) rv, format_vnet_api_errno, rv);
    }
...
static uword bfd_cli_unformat_ip46_address(unformat_input_t *input, va_list *args)
{
...
#define foreach_bfd_cli_udp_session_add_cli_param(F)                                              \
    F(u32, sw_if_index, INTERFACE_STR, mandatory, "%U", unformat_vnet_sw_interface, &vnet_main)   \
    F(ip46_address_t, local_addr, LOCAL_ADDR_STR, mandatory, "%U", bfd_cli_unformat_ip46_address) \
    F(ip46_address_t, peer_addr, PEER_ADDR_STR, mandatory, "%U", bfd_cli_unformat_ip46_address)   \
    ...
    if (detect_mult > 255) {
        ret = clib_error_return(0, "%s value `%u' out of range <1,255>", DETECT_MULT_STR, detect_mult);
        goto out;
    }

    if (have_bfd_key_id && bfd_key_id > 255) {
        ret = clib_error_return(0, "%s value `%u' out of range <1,255>", BFD_KEY_ID_STR, bfd_key_id);
        goto out;
    }

    vnet_api_error_t rv = bfd_udp_add_session(sw_if_index,
                                              &local_addr,
                                              &peer_addr,
                                              desired_min_tx,
                                              required_min_rx,
                                              ...
                                              bfd_key_id);
    if (rv) {
        ret = clib_error_return(0, "`bfd_add_add_session' API call failed, rv=%d:%U", (int) rv, format_vnet_api_errno, rv);
        goto out;
    }
...
#define foreach_bfd_cli_udp_session_mod_cli_param(F)                                              \
    ...
    vnet_api_error_t rv = bfd_udp_mod_session(sw_if_index, &local_addr, &peer_addr, desired_min_tx, required_min_rx, detect_mult);
    if (rv) {
        ret = clib_error_return(0, "`bfd_udp_mod_session' API call failed, rv=%d:%U", (int) rv, format_vnet_api_errno, rv);
        goto out;
    }
...
#define foreach_bfd_cli_udp_session_del_cli_param(F)                                              \
    ...
    vnet_api_error_t rv = bfd_udp_del_session(sw_if_index, &local_addr, &peer_addr);
    if (rv) {
        ret = clib_error_return(0, "`bfd_udp_del_session' API call failed, rv=%d:%U", (int) rv, format_vnet_api_errno, rv);
        goto out;
    }
...
#define foreach_bfd_cli_udp_session_set_flags_cli_param(F)                                        \
    ...
    F(ip46_address_t, local_addr, LOCAL_ADDR_STR, mandatory, "%U", bfd_cli_unformat_ip46_address) \
    F(ip46_address_t, peer_addr, PEER_ADDR_STR, mandatory, "%U", bfd_cli_unformat_ip46_address)   \
    F(u8 *, admin_up_down_token, ADMIN_STR, mandatory, "%v", &admin_up_down_token)
    ...
    if (!memcmp(admin_up_down_token, up, sizeof(up) - 1)) {
        ...
        ret = clib_error_return(0, "Unrecognized value for `%s' parameter: `%v'", ADMIN_STR, admin_up_down_token);
        goto out;
    }
    vnet_api_error_t rv = bfd_udp_session_set_flags(vm, sw_if_index, &local_addr, &peer_addr, admin_up_down);
    if (rv) {
        ret = clib_error_return(0, "`bfd_udp_session_set_flags' API call failed, rv=%d:%U", (int) rv, format_vnet_api_errno, rv);
        goto out;
    }
...
#define foreach_bfd_cli_udp_session_auth_activate_cli_param(F)                                    \
    ...
    if (have_delayed_token) {
        ...
        if (!memcmp(delayed_token, yes, sizeof(yes) - 1)) {
            ...
            ret = clib_error_return(0, "Unrecognized value for `%s' parameter: `%v'", DELAYED_STR, delayed_token);
            goto out;
        }
    }
    ...
    vnet_api_error_t rv = bfd_udp_auth_activate(sw_if_index, &local_addr, &peer_addr, conf_key_id, bfd_key_id, is_delayed);
    if (rv) {
        ret = clib_error_return(0, "`bfd_udp_auth_activate' API call failed, rv=%d:%U", (int) rv, format_vnet_api_errno, rv);
        goto out;
    }
...
#define foreach_bfd_cli_udp_session_auth_deactivate_cli_param(F)                                  \
    ...
    vnet_api_error_t rv = bfd_udp_auth_deactivate(sw_if_index, &local_addr, &peer_addr, is_delayed);
    if (rv) {
        ret = clib_error_return(0, "`bfd_udp_auth_deactivate' API call failed, rv=%d:%U", (int) rv, format_vnet_api_errno, rv);
        goto out;
    }
...
static clib_error_t *bfd_cli_udp_set_echo_source(vlib_main_t *vm, unformat_input_t *input, CLIB_UNUSED(vlib_cli_command_t *lmd))
{
    clib_error_t    *ret = NULL;
    unformat_input_t _line_input, *line_input = &_line_input;
#define foreach_bfd_cli_udp_set_echo_source_cli_param(F) F(u32, sw_if_index, INTERFACE_STR, mandatory, "%U", unformat_vnet_sw_interface, &vnet_main)
    ...
    if (rv) {
        ret = clib_error_return(0, "`bfd_udp_set_echo_source' API call failed, rv=%d:%U", (int) rv, format_vnet_api_errno, rv);
        goto out;
    }
...
static clib_error_t *bfd_cli_udp_del_echo_source(vlib_main_t *vm, unformat_input_t *input, CLIB_UNUSED(vlib_cli_command_t *lmd))
{
    ...
    if (rv) {
        return clib_error_return(0, "`bfd_udp_del_echo_source' API call failed, rv=%d:%U", (int) rv, format_vnet_api_errno, rv);
    }
