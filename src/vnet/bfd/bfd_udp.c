...
#define F(sym, str) STATIC_ASSERT((int) BFD_ERROR_##sym == (int) BFD_UDP_ERROR_##sym, "BFD error enums mismatch");
foreach_bfd_error(F)
#undef F
    STATIC_ASSERT((int) BFD_N_ERROR <= (int) BFD_UDP_N_ERROR, "BFD error enum sizes mismatch");
...
void bfd_udp_update_stat_segment_entry(u32 entry, u64 value)
{
...
{
    vnet_sw_interface_t *sw_if = vnet_get_sw_interface_or_null(bfd_udp_main.vnet_main, sw_if_index);
    if (sw_if) {
...
int bfd_udp_is_echo_available(bfd_transport_e transport)
{
     ...
     */
    vnet_sw_interface_t *sw_if = vnet_get_sw_interface_or_null(bfd_udp_main.vnet_main, bfd_udp_main.echo_source_sw_if_index);
    if (sw_if && sw_if->flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP) {
        if (BFD_TRANSPORT_UDP4 == transport) {
            ip4_main_t             *im = &ip4_main;
            ip_interface_address_t *ia = NULL;
            foreach_ip_interface_address(&im->lookup_main, ia, bfd_udp_main.echo_source_sw_if_index, 0 /* honor unnumbered */, ({
...
int bfd_add_udp4_transport(vlib_main_t *vm, u32 bi, const bfd_session_t *bs, int is_echo)
{
    ...
    headers->udp.src_port                     = clib_host_to_net_u16(bfd_udp_bs_idx_to_sport(bs->bs_idx));
    if (is_echo) {
        int rv;
...
int bfd_add_udp6_transport(vlib_main_t *vm, u32 bi, const bfd_session_t *bs, int is_echo)
{
    ...
    headers = vlib_buffer_get_current(b);
    clib_memset(headers, 0, sizeof(*headers));
    headers->ip6.ip_version_traffic_class_and_flow_label = clib_host_to_net_u32(0x6 << 28);
    ...
    if (is_echo) {
        ...
        clib_memcpy_fast(&headers->ip6.dst_address, &key->local_addr.ip6, sizeof(headers->ip6.dst_address));

        headers->udp.dst_port = clib_host_to_net_u16(UDP_DST_PORT_bfd_echo6);
    }
    else {
        clib_memcpy_fast(&headers->ip6.src_address, &key->local_addr.ip6, sizeof(headers->ip6.src_address));
        clib_memcpy_fast(&headers->ip6.dst_address, &key->peer_addr.ip6, sizeof(headers->ip6.dst_address));
        headers->udp.dst_port = clib_host_to_net_u16(UDP_DST_PORT_bfd6);
    }

    /* fix ip payload length and udp length */
    const u16 udp_length        = vlib_buffer_length_in_chain(vm, b) - (sizeof(headers->ip6));
    headers->udp.length         = clib_host_to_net_u16(udp_length);
    headers->ip6.payload_length = headers->udp.length;

    /* IPv6 UDP checksum is mandatory */
    int bogus             = 0;
    headers->udp.checksum = ip6_tcp_udp_icmp_compute_checksum(vm, b, &headers->ip6, &bogus);
    ASSERT(bogus == 0);
                                          ...
                                          vlib_combined_counter_main_t *tx_counter)
{
    ...
    vlib_increment_combined_counter(tx_counter, vm->thread_index, bs->bs_idx, 1, vlib_buffer_length_in_chain(vm, b));
}
...
int bfd_udp_calc_next_node(const struct bfd_session_s *bs, u32 *next_node)
{
    ...
    if (!vnet_sw_interface_is_up(vnm, bus->key.sw_if_index)) return 0;
...
int bfd_transport_udp4(vlib_main_t *vm, vlib_node_runtime_t *rt, u32 bi, const struct bfd_session_s *bs, int is_echo)
{
    u32         next_node;
    int         rv = bfd_udp_calc_next_node(bs, &next_node);
    bfd_main_t *bm = bfd_udp_main.bfd_main;
    if (rv) {
        bfd_create_frame_to_next_node(vm, rt, bi, bs, next_node, is_echo ? &bm->tx_echo_counter : &bm->tx_counter);
    }
    return rv;
}

int bfd_transport_udp6(vlib_main_t *vm, vlib_node_runtime_t *rt, u32 bi, const struct bfd_session_s *bs, int is_echo)
{
    u32         next_node;
    int         rv = bfd_udp_calc_next_node(bs, &next_node);
    bfd_main_t *bm = bfd_udp_main.bfd_main;
                                                     ...
                                                     bfd_session_t       **bs_out)
{
    ...
    if (tmp) {
        vlib_log_err(bum->log_class, "duplicate bfd-udp session, existing bs_idx=%d", tmp->bs_idx);
    ...
    BFD_DBG("session created, bs_idx=%u, sw_if_index=%d, local=%U, peer=%U",
            ...
            &key->peer_addr,
            IP46_TYPE_ANY);
    vlib_log_info(bum->log_class, "create BFD session: %U", format_bfd_session, bs);
    const ip46_address_t *peer = (vnet_sw_interface_is_p2p(vnet_get_main(), key->sw_if_index) ? &zero_addr : &key->peer_addr);
    if (BFD_TRANSPORT_UDP4 == t) {
        bus->adj_index = adj_nbr_add_or_lock(FIB_PROTOCOL_IP4, VNET_LINK_IP4, peer, key->sw_if_index);
        BFD_DBG(
        ...
        bfd_udp_update_stat_segment_entry(bum->udp4_sessions_count_stat_seg_entry, bum->udp4_sessions_count);
        if (1 == bum->udp4_sessions_count) {
            udp_register_dst_port(vm, UDP_DST_PORT_bfd4, bfd_udp4_input_node.index, 1);
            udp_register_dst_port(vm, UDP_DST_PORT_bfd_echo4, bfd_udp_echo4_input_node.index, 1);
        }
    }
    else {
        bus->adj_index = adj_nbr_add_or_lock(FIB_PROTOCOL_IP6, VNET_LINK_IP6, peer, key->sw_if_index);
        BFD_DBG(
        ...
        if (1 == bum->udp6_sessions_count) {
            ...
            udp_register_dst_port(vm, UDP_DST_PORT_bfd_echo6, bfd_udp_echo6_input_node.index, 0);
        }
    }
    *bs_out = bs;
    return bfd_session_set_params(bum->bfd_main, bs, desired_min_tx_usec, required_min_rx_usec, detect_mult);
}
...
static vnet_api_error_t bfd_udp_validate_api_input(u32 sw_if_index, const ip46_address_t *local_addr, const ip46_address_t *peer_addr)
{
    ...
    vnet_sw_interface_t *sw_if = vnet_get_sw_interface_or_null(bfd_udp_main.vnet_main, sw_if_index);
    if (!sw_if) {
        vlib_log_err(bum->log_class, "got NULL sw_if when getting interface by index %u", sw_if_index);
        return VNET_API_ERROR_INVALID_SW_IF_INDEX;
    }
    if (ip46_address_is_ip4(local_addr)) {
        if (!ip46_address_is_ip4(peer_addr)) {
            vlib_log_err(bum->log_class, "IP family mismatch (local is ipv4, peer is ipv6)");
            return VNET_API_ERROR_INVALID_ARGUMENT;
        }
    }
    else {
        if (ip46_address_is_ip4(peer_addr)) {
            vlib_log_err(bum->log_class, "IP family mismatch (local is ipv6, peer is ipv4)");
            return VNET_API_ERROR_INVALID_ARGUMENT;
        }
    }
...
static vnet_api_error_t bfd_udp_find_session_by_api_input(u32                   sw_if_index,
                                                          ...
                                                          bfd_session_t       **bs_out)
{
    vnet_api_error_t rv = bfd_udp_validate_api_input(sw_if_index, local_addr, peer_addr);
    if (!rv) {
        bfd_udp_main_t *bum = &bfd_udp_main;
        bfd_udp_key_t   key;
        bfd_udp_key_init(&key, sw_if_index, local_addr, peer_addr);
...
static vnet_api_error_t bfd_udp_add_and_start_session(u32                   sw_if_index,
                                                      const ip46_address_t *local_addr,
                                                      const ip46_address_t *peer_addr,
                                                      u32                   desired_min_tx_usec,
                                                      u32                   required_min_rx_usec,
                                                      ...
                                                      u8                    bfd_key_id)
{
    ...
    rv = bfd_udp_add_session_internal(vlib_get_main(),
                                      &bfd_udp_main,
                                      sw_if_index,
                                      desired_min_tx_usec,
    ...
    if (!rv && is_authenticated) {
        rv = bfd_auth_activate(bs, conf_key_id, bfd_key_id, 0 /* is not delayed */);
        if (rv) {
                                     ...
                                     u8                    bfd_key_id)
{
    ...
    vnet_api_error_t rv = bfd_api_verify_common(sw_if_index, desired_min_tx_usec, detect_mult, local_addr, peer_addr);

    if (!rv)
        rv = bfd_udp_add_and_start_session(sw_if_index,
                                           local_addr,
                                           peer_addr,
                                           desired_min_tx_usec,
                                           required_min_rx_usec,
...
vnet_api_error_t bfd_udp_upd_session(u32                   sw_if_index,
                                     const ip46_address_t *local_addr,
                                     const ip46_address_t *peer_addr,
                                     u32                   desired_min_tx_usec,
                                     u32                   required_min_rx_usec,
                                     ...
                                     u8                    bfd_key_id)
{
    ...
    if (!rv) {
        ...
        rv = bfd_udp_find_session_by_api_input(sw_if_index, local_addr, peer_addr, &bs);
        if (VNET_API_ERROR_BFD_ENOENT == rv)
            ...
            rv = bfd_session_set_params(bfd_udp_main.bfd_main, bs, desired_min_tx_usec, required_min_rx_usec, detect_mult);
    }
                                     ...
                                     u8                    detect_mult)
{
    ...
    bfd_lock(bm);
    vnet_api_error_t rv = bfd_udp_find_session_by_api_input(sw_if_index, local_addr, peer_addr, &bs);
    if (rv) {
        bfd_unlock(bm);
        return rv;
    }

    error = bfd_session_set_params(bfd_udp_main.bfd_main, bs, desired_min_tx_usec, required_min_rx_usec, detect_mult);
    bfd_unlock(bm);
    return error;
}
...
vnet_api_error_t bfd_udp_session_set_flags(vlib_main_t          *vm,
                                           u32                   sw_if_index,
                                           const ip46_address_t *local_addr,
                                           const ip46_address_t *peer_addr,
                                           u8                    admin_up_down)
...
static_always_inline vl_counter_bfd_udp_enum_t bfd_error_to_udp(bfd_error_t e)
{
...
static vl_counter_bfd_udp_enum_t bfd_udp4_verify_transport(const ip4_header_t *ip4, const udp_header_t *udp, const bfd_session_t *bs)
{
    const bfd_udp_session_t *bus = &bs->udp;
    const bfd_udp_key_t     *key = &bus->key;
    ...
    if (ip4->ttl != expected_ttl) {
        BFD_ERR("IPv4 unexpected TTL value %u, expected %u", ip4->ttl, expected_ttl);
        return BFD_UDP_ERROR_TTL;
    }
    if (clib_net_to_host_u16(udp->src_port) < 49152) {
        BFD_ERR("Invalid UDP src port %u, out of range <49152,65535>", udp->src_port);
    }
...
static bfd_error_t bfd_rpc_update_session(vlib_main_t *vm, u32 bs_idx, const bfd_pkt_t *pkt)
{
    bfd_main_t *bm = &bfd_main;
    bfd_error_t err;
    bfd_lock(bm);
...
static vl_counter_bfd_udp_enum_t bfd_udp4_scan(vlib_main_t *vm, vlib_buffer_t *b, bfd_session_t **bs_out)
{
    const bfd_pkt_t *pkt = vlib_buffer_get_current(b);
    if (sizeof(*pkt) > b->current_length) {
        BFD_ERR("Payload size %d too small to hold bfd packet of minimum size %d", b->current_length, sizeof(*pkt));
    ...
    if (pkt->head.length > udp_payload_length) {
        BFD_ERR("BFD packet length is larger than udp payload length (%u > %u)", pkt->head.length, udp_payload_length);
        return BFD_UDP_ERROR_LENGTH;
    }
    vl_counter_bfd_udp_enum_t err;
    if (BFD_UDP_ERROR_NONE != (err = bfd_error_to_udp(bfd_verify_pkt_common(pkt)))) {
        return err;
    }
    bfd_session_t *bs = NULL;
    if (pkt->your_disc) {
        BFD_DBG("Looking up BFD session using discriminator %u", pkt->your_disc);
...
static vl_counter_bfd_udp_enum_t bfd_udp6_verify_transport(const ip6_header_t *ip6, const udp_header_t *udp, const bfd_session_t *bs)
{
    const bfd_udp_session_t *bus = &bs->udp;
    const bfd_udp_key_t     *key = &bus->key;
    if (ip6->src_address.as_u64[0] != key->peer_addr.ip6.as_u64[0] && ip6->src_address.as_u64[1] != key->peer_addr.ip6.as_u64[1]) {
        BFD_ERR("IP src addr mismatch, got %U, expected %U", format_ip6_address, ip6, format_ip6_address, &key->peer_addr.ip6);
        return BFD_UDP_ERROR_SRC_MISMATCH;
    }
    if (ip6->dst_address.as_u64[0] != key->local_addr.ip6.as_u64[0] && ip6->dst_address.as_u64[1] != key->local_addr.ip6.as_u64[1]) {
        BFD_ERR("IP dst addr mismatch, got %U, expected %U", format_ip6_address, ip6, format_ip6_address, &key->local_addr.ip6);
        return BFD_UDP_ERROR_DST_MISMATCH;
    }
    const u8 expected_hop_limit = 255;
    if (ip6->hop_limit != expected_hop_limit) {
        BFD_ERR("IPv6 unexpected hop-limit value %u, expected %u", ip6->hop_limit, expected_hop_limit);
        return BFD_UDP_ERROR_TTL;
    }
    if (clib_net_to_host_u16(udp->src_port) < 49152) {
        BFD_ERR("Invalid UDP src port %u, out of range <49152,65535>", udp->src_port);
...
static vl_counter_bfd_udp_enum_t bfd_udp6_scan(vlib_main_t *vm, vlib_buffer_t *b, bfd_session_t **bs_out)
{
    const bfd_pkt_t *pkt = vlib_buffer_get_current(b);
    if (sizeof(*pkt) > b->current_length) {
        BFD_ERR("Payload size %d too small to hold bfd packet of minimum size %d", b->current_length, sizeof(*pkt));
...
static uword bfd_udp_input(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f, int is_ipv6)
{
    ...
    while (n_left_from > 0) {
        ...
        if (b0->flags & VLIB_BUFFER_IS_TRACED) {
            ...
            len     = (b0->current_length < sizeof(t0->data)) ? b0->current_length : sizeof(t0->data);
            t0->len = len;
        ...
        if (BFD_UDP_ERROR_NONE == error0) {
            vlib_increment_combined_counter(&bm->rx_counter, vm->thread_index, bs->bs_idx, 1, vlib_buffer_length_in_chain(vm, b0));
            /*
            ...
            if (bfd_pkt_get_poll(pkt)) {
                ...
                if (is_ipv6) {
                    vlib_node_increment_counter(vm, bfd_udp6_input_node.index, error0, 1);
                }
                else {
                    vlib_node_increment_counter(vm, bfd_udp4_input_node.index, error0, 1);
                }
                const bfd_udp_session_t *bus = &bs->udp;
                ip_adjacency_t          *adj = adj_get(bus->adj_index);
                switch (adj->lookup_next_index) {
...
static uword bfd_udp_echo_input(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f, int is_ipv6)
{
    ...
    while (n_left_from > 0) {
        ...
        if ((bs = bfd_consume_echo_pkt(vm, bfd_udp_main.bfd_main, b0))) {
            ...
            b0->error = rt->errors[BFD_UDP_ERROR_NONE];
            if (is_ipv6) {
                vlib_node_increment_counter(vm, bfd_udp_echo6_input_node.index, BFD_UDP_ERROR_NONE, 1);
            }
            else {
                vlib_node_increment_counter(vm, bfd_udp_echo4_input_node.index, BFD_UDP_ERROR_NONE, 1);
            }
        ...
        if (bs) {
            vlib_increment_combined_counter(&bm->rx_echo_counter, vm->thread_index, bs->bs_idx, 1, vlib_buffer_length_in_chain(vm, b0));
        }
...
u8 *bfd_echo_input_format_trace(u8 *s, va_list *args)
{
    ...
    const bfd_udp_echo_input_trace_t *t = va_arg(*args, bfd_udp_echo_input_trace_t *);
...
static clib_error_t *bfd_udp_sw_if_add_del(CLIB_UNUSED(vnet_main_t *vnm), u32 sw_if_index, u32 is_create)
{
    u32            *to_be_freed = NULL;
    bfd_udp_main_t *bum         = &bfd_udp_main;
    BFD_DBG("sw_if_add_del called, sw_if_index=%u, is_create=%u", sw_if_index, is_create);
    if (!is_create) {
        bfd_session_t *bs;
        pool_foreach (bs, bum->bfd_main->sessions) {
            if (bs->transport != BFD_TRANSPORT_UDP4 && bs->transport != BFD_TRANSPORT_UDP6) {
                continue;
            }
...
clib_error_t *bfd_udp_stats_init(bfd_udp_main_t *bum)
{
    ...
    if (~0 == bum->udp4_sessions_count_stat_seg_entry) {
        return clib_error_return(0, "Could not create stat segment entry for %s", name4);
    }
    ...
    if (~0 == bum->udp6_sessions_count_stat_seg_entry) {
        return clib_error_return(0, "Could not create stat segment entry for %s", name6);
    }
...
static clib_error_t *bfd_udp_init(vlib_main_t *vm)
{
    ...
    mhash_init(&bfd_udp_main.bfd_session_idx_by_bfd_key, sizeof(uword), sizeof(bfd_udp_key_t));
