...
u8 *format_ip_port_and_mask(u8 *s, va_list *args)
{
    ...
    if (pm->port == 0 && pm->mask == 0) return format(s, "any");

    if (pm->mask == 0xffff) return format(s, "%u", pm->port);
...
u8 *format_ip_protocol_and_mask(u8 *s, va_list *args)
{
    ...
    if (pm->prot == 0 && pm->mask == 0) return format(s, "any");
...
u8 *format_flow_error(u8 *s, va_list *args)
{
    ...
    if (error == 0) return format(s, "no error");

#define _(v, n, str) \
    if (error == v) return format(s, #str);
    foreach_flow_error;
#undef _
...
#define _(a, b, c) \
    if (actions & (1 << a)) t = format(t, "%s%s", t ? " " : "", c);
    foreach_flow_action
#undef _
        s = format(s, "%v", t);
    vec_free(t);
...
u8 *format_flow_enabled_hw(u8 *s, va_list *args)
{
    ...
    if (f == 0) return format(s, "not found");
...
static clib_error_t *show_flow_entry(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
    ...
    if (!unformat_user(input, unformat_line_input, line_input)) goto no_args;
    ...
    if (index != ~0) {
        if ((f = vnet_get_flow(index)) == 0) return clib_error_return(0, "no such flow");
        ...
        hash_foreach(hw_if_index, private_data, f->private_data, ({
                         ...
                         if (dev_class->format_flow) vlib_cli_output(vm, "  %U\n", dev_class->format_flow, hi->dev_instance, f->index, private_data);
                     }));
        return 0;
    }
...
static clib_error_t *show_flow_interface(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
    ...
    if (hw_if_index == ~0) return clib_error_return(0, "please specify interface");

    hi        = vnet_get_hw_interface(vnm, hw_if_index);
    dev_class = vnet_get_device_class(vnm, hi->dev_class_index);
    if (dev_class->format_flow == 0) return clib_error_return(0, "not supported");
...
static clib_error_t *test_flow(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
    ...
    vnet_main_t     *vnm = vnet_get_main();
    unformat_input_t _line_input, *line_input = &_line_input;
    enum { FLOW_UNKNOWN_ACTION, FLOW_ADD, FLOW_DEL, FLOW_ENABLE, FLOW_DISABLE } action = FLOW_UNKNOWN_ACTION;
    enum {
        FLOW_UNKNOWN_CLASS,
        FLOW_ETHERNET_CLASS,
        FLOW_IPV4_CLASS,
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "add"))
            ...
            ;
        else if (unformat(line_input, "eth-type %U", unformat_ethernet_type_host_byte_order, &eth_type))
            ...
            if (protocol.prot == IP_PROTOCOL_L2TP) l2tpv3oip_set = true;
        }
            ...
            ;
        else if (unformat(line_input, "next-node %U", unformat_vlib_node, vm, &flow.redirect_node_index))
...
#undef _
#define _(a, b, c) else if (!clib_strcmp(c, (const char *) type_str)) flow.rss_types |= (1ULL << a);
...
#undef _
    ...
    if (hw_if_index == ~0 && (action == FLOW_ENABLE || action == FLOW_DISABLE)) return clib_error_return(0, "Please specify interface name");
    ...
    switch (action) {
        case FLOW_ADD:
            if (flow.actions == 0) return clib_error_return(0, "Please specify at least one action");
            ...
            if (flow_class == FLOW_ETHERNET_CLASS) {
                 ...
                 */
                if (type == VNET_FLOW_TYPE_IP4) ip4_ptr->protocol.mask = protocol.mask;
                ...
                switch (protocol.prot) {
                    ...
                    case IP_PROTOCOL_IPV6:
                        clib_memcpy(&flow.ip4_ip6.in_src_addr, &in_ip6s, sizeof(ip6_address_and_mask_t));
                        clib_memcpy(&flow.ip4_ip6.in_dst_addr, &in_ip6d, sizeof(ip6_address_and_mask_t));
                ...
                clib_memcpy(&flow.ip6_n_tuple.src_addr, &ip6s, sizeof(ip6_address_and_mask_t));
                clib_memcpy(&flow.ip6_n_tuple.dst_addr, &ip6d, sizeof(ip6_address_and_mask_t));
                 ...
                 * used to identify the next layer type: e.g. UDP/TCP or IPSEC_ESP
                 */
                if (type == VNET_FLOW_TYPE_IP6) ip6_ptr->protocol.mask = protocol.mask;
                ...
                switch (protocol.prot) {
                    ...
                    case IP_PROTOCOL_UDP:
                        ...
                        if (type == VNET_FLOW_TYPE_IP6_VXLAN) flow.ip6_vxlan.vni = vni;
                        break;
                    case IP_PROTOCOL_IP_IN_IP:
                        ...
                        break;
                    case IP_PROTOCOL_IPV6:
                        clib_memcpy(&flow.ip6_ip6.in_src_addr, &in_ip6s, sizeof(ip6_address_and_mask_t));
                        clib_memcpy(&flow.ip6_ip6.in_dst_addr, &in_ip6d, sizeof(ip6_address_and_mask_t));
            ...
            if (type == VNET_FLOW_TYPE_GENERIC) {
                clib_memcpy(flow.generic.pattern.spec, spec, sizeof(flow.generic.pattern.spec));
                clib_memcpy(flow.generic.pattern.mask, mask, sizeof(flow.generic.pattern.mask));
            }

            flow.type = type;
            rv        = vnet_flow_add(vnm, &flow, &flow_index);
            if (!rv) vlib_cli_output(vm, "flow %u added", flow_index);
        ...
        default:
            return clib_error_return(0,
                                     "please specify action (add, del, enable,"
                                     " disable)");
    }

    if (rv < 0) return clib_error_return(0, "flow error: %U", format_flow_error, rv);
...
static u8 *format_flow_match_element(u8 *s, va_list *args)
{
    ...
    if (strncmp(type, "u8", 2) == 0) return format(s, "%d", *(u8 *) ptr);

    if (strncmp(type, "u16", 3) == 0) return format(s, "%d", *(u16 *) ptr);

    if (strncmp(type, "u32", 3) == 0) return format(s, "%d", *(u32 *) ptr);
    ...
    if (strncmp(type, "ip4_address_t", 13) == 0) return format(s, "%U", format_ip4_address, ptr);

    if (strncmp(type, "ip4_address_and_mask_t", 13) == 0) return format(s, "%U", format_ip4_address_and_mask, ptr);

    if (strncmp(type, "ip6_address_t", 13) == 0) return format(s, "%U", format_ip6_address, ptr);

    if (strncmp(type, "ip6_address_and_mask_t", 13) == 0) return format(s, "%U", format_ip6_address_and_mask, ptr);

    if (strncmp(type, "ip_prot_and_mask_t", 13) == 0) return format(s, "%U", format_ip_protocol_and_mask, ptr);

    if (strncmp(type, "ip_port_and_mask_t", 18) == 0) return format(s, "%U", format_ip_port_and_mask, ptr);

    s = format(s, "unknown type '%s'", type);
    return s;
}

#define _fe(a, b) s2 = format(s2, "%s%s %U", s2 ? ", " : "", #b, format_flow_match_element, #a, &f->b);
#define _(a, b, c)                                                                \
...
#define _(a, b, c) \
    if (f->type == VNET_FLOW_TYPE_##a) return format(s, "%U", format_flow_match_##b, &f->b);
    foreach_flow_type;
#undef _
...
static u8 *format_flow(u8 *s, va_list *args)
{
    ...
    s = format(s, "flow-index %u type %s active %u", f->index, flow_type_strings[f->type], hash_elts(f->private_data)),
    ...
    if (f->actions & VNET_FLOW_ACTION_DROP) t = format(t, "%sdrop", t ? ", " : "");

    if (f->actions & VNET_FLOW_ACTION_MARK) t = format(t, "%smark %u", t ? ", " : "", f->mark_flow_id);

    if (f->actions & VNET_FLOW_ACTION_REDIRECT_TO_QUEUE) t = format(t, "%sredirect-to-queue %u", t ? ", " : "", f->redirect_queue);

    if (f->actions & VNET_FLOW_ACTION_REDIRECT_TO_NODE)
        t = format(t, "%snext-node %U", t ? ", " : "", format_vlib_node_name, vm, f->redirect_node_index);

    if (f->actions & VNET_FLOW_ACTION_BUFFER_ADVANCE) t = format(t, "%sbuffer-advance %d", t ? ", " : "", f->buffer_advance);
