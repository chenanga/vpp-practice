...
int vnet_flow_get_range(vnet_main_t *vnm, char *owner, u32 count, u32 *start)
{
    ...
    /* skip 0 */
    if (fm->flows_used == 0) fm->flows_used = 1;
...
{
    vnet_flow_main_t *fm = &flow_main;
    if (pool_is_free_index(fm->global_flow_pool, flow_index)) return 0;
...
int vnet_flow_del(vnet_main_t *vnm, u32 flow_index)
{
    ...
    if (f == 0) return VNET_FLOW_ERROR_NO_SUCH_ENTRY;

    hash_foreach(hw_if_index, private_data, f->private_data, ({ vnet_flow_disable(vnm, flow_index, hw_if_index); }));
...
int vnet_flow_enable(vnet_main_t *vnm, u32 flow_index, u32 hw_if_index)
{
    ...
    if (!vnet_hw_interface_is_valid(vnm, hw_if_index)) return VNET_FLOW_ERROR_NO_SUCH_INTERFACE;

    /* don't enable flow twice */
    if (hash_get(f->private_data, hw_if_index) != 0) return VNET_FLOW_ERROR_ALREADY_DONE;

    hi        = vnet_get_hw_interface(vnm, hw_if_index);
    dev_class = vnet_get_device_class(vnm, hi->dev_class_index);

    if (dev_class->flow_ops_function == 0) return VNET_FLOW_ERROR_NOT_SUPPORTED;

    if (f->actions & VNET_FLOW_ACTION_REDIRECT_TO_NODE) {
        vnet_hw_interface_t *hw             = vnet_get_hw_interface(vnm, hw_if_index);
        f->redirect_device_input_next_index = vlib_node_add_next(vnm->vlib_main, hw->input_node_index, f->redirect_node_index);
    }

    rv = dev_class->flow_ops_function(vnm, VNET_FLOW_DEV_OP_ADD_FLOW, hi->dev_instance, flow_index, &private_data);
...
int vnet_flow_disable(vnet_main_t *vnm, u32 flow_index, u32 hw_if_index)
{
    ...
    if ((p = hash_get(f->private_data, hw_if_index)) == 0) return VNET_FLOW_ERROR_ALREADY_DONE;

    hi        = vnet_get_hw_interface(vnm, hw_if_index);
    dev_class = vnet_get_device_class(vnm, hi->dev_class_index);

    rv = dev_class->flow_ops_function(vnm, VNET_FLOW_DEV_OP_DEL_FLOW, hi->dev_instance, flow_index, p);
