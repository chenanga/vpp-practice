...
always_inline u32 ho_hash(u64 key)
{
...
static inline u64 ipv6_get_key(ip6_header_t *ip)
{
    ...
    hash_key = ip->src_address.as_u64[0] ^ rotate_left(ip->src_address.as_u64[1], 13) ^ rotate_left(ip->dst_address.as_u64[0], 26)
...
static inline u64 mpls_get_key(mpls_unicast_header_t *m)
{
    ...
    if (PREDICT_TRUE(m->label_exp_s_ttl & clib_net_to_host_u32(MPLS_BOTTOM_OF_STACK_BIT_MASK))) {
...
static inline u64 eth_get_sym_key(ethernet_header_t *h0)
{
    u64 hash_key;

    if (PREDICT_TRUE(h0->type) == clib_host_to_net_u16(ETHERNET_TYPE_IP4)) {
        ip4_header_t *ip = (ip4_header_t *) (h0 + 1);
        hash_key         = (u64) (ip->src_address.as_u32 ^ ip->dst_address.as_u32 ^ ip->protocol);
    }
    else if (h0->type == clib_host_to_net_u16(ETHERNET_TYPE_IP6)) {
        ip6_header_t *ip = (ip6_header_t *) (h0 + 1);
        hash_key =
            (u64) (ip->src_address.as_u64[0] ^ ip->src_address.as_u64[1] ^ ip->dst_address.as_u64[0] ^ ip->dst_address.as_u64[1] ^ ip->protocol);
    }
    else if (h0->type == clib_host_to_net_u16(ETHERNET_TYPE_MPLS)) {
        hash_key = mpls_get_key((mpls_unicast_header_t *) (h0 + 1));
    }
    else if (PREDICT_FALSE((h0->type == clib_host_to_net_u16(ETHERNET_TYPE_VLAN)) || (h0->type == clib_host_to_net_u16(ETHERNET_TYPE_DOT1AD)))) {
...
static inline u64 eth_get_key(ethernet_header_t *h0)
{
    ...
    else if ((h0->type == clib_host_to_net_u16(ETHERNET_TYPE_VLAN)) || (h0->type == clib_host_to_net_u16(ETHERNET_TYPE_DOT1AD))) {
        ethernet_vlan_header_t *outer = (ethernet_vlan_header_t *) (h0 + 1);
...
void handoff_eth_func(void **p, u32 *hash, u32 n_packets)
{
...
void handoff_eth_sym_func(void **p, u32 *hash, u32 n_packets)
{
    u32 n_left_from = n_packets;
