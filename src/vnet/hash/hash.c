...
{
    vnet_hash_function_registration_t *hash = va_arg(*args, vnet_hash_function_registration_t *);
...
vnet_hash_fn_t vnet_hash_default_function(vnet_hash_fn_type_t ftype)
{
    ...
    while (hash) {
        if (hash->priority > tmp_hash->priority) tmp_hash = hash;
        hash = hash->next;
    }
...
vnet_hash_fn_t vnet_hash_function_from_name(const char *name, vnet_hash_fn_type_t ftype)
{
    vnet_hash_function_registration_t *hash = vnet_hash_main.hash_registrations;
    while (hash) {
        if (strcmp(hash->name, name) == 0) break;
        hash = hash->next;
    }
    if (!hash) return (0);
...
vnet_hash_function_registration_t *vnet_hash_function_from_func(vnet_hash_fn_t fn, vnet_hash_fn_type_t ftype)
{
    vnet_hash_function_registration_t *hash = vnet_hash_main.hash_registrations;
    while (hash) {
        if (hash->function[ftype] == fn) break;
        hash = hash->next;
    }
    return hash;
}
