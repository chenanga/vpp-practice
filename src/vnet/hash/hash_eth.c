...
static_always_inline u16 *locate_ethertype(ethernet_header_t *eth)
{
    u16                    *ethertype_p;
...
static void hash_eth_l2(void **p, u32 *hash, u32 n_packets)
{
...
static_always_inline u32 hash_eth_l23_inline(void **p)
{
    ethernet_header_t *eth = *p;
    u8                 ip_version;
    ip4_header_t      *ip4;
    ...
    if (ip_version == 0x4) {
        ...
        a    = clib_mem_unaligned(mac1, u32) ^ clib_mem_unaligned(mac2, u32) ^ clib_mem_unaligned(mac3, u32);
    ...
    if (ip_version == 0x6) {
        ...
        hash = lb_hash_hash(clib_mem_unaligned(&ip6->src_address.as_uword[0], uword),
                            ...
                            clib_mem_unaligned(&ip6->dst_address.as_uword[1], uword),
                            a);
        return hash;
    }
...
static void hash_eth_l23(void **p, u32 *hash, u32 n_packets)
{
    u32 n_left_from = n_packets;
...
static_always_inline u32 hash_eth_l34_inline(void **p)
{
    ethernet_header_t *eth = *p;
    u8                 ip_version;
    uword              is_tcp_udp;
...
static void hash_eth_l34(void **p, u32 *hash, u32 n_packets)
{
    u32 n_left_from = n_packets;
