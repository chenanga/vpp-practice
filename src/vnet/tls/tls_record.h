...
static inline u32 tls_handshake_message_len(tls_handshake_msg_t *msg)
{
    u8 *p = (u8 *) msg;
...
static inline u8 tls_record_type_is_valid(tls_record_type_t type)
{
    switch (type) {
...
static inline u8 tls_record_hdr_is_valid(tls_record_header_t rec_hdr)
{
    u16 rec_len;

    if (!tls_record_type_is_valid(rec_hdr.type)) return 0;

    /* Support for SSLv3 and TLS1.0 to TLS1.3 */
    if (rec_hdr.version.major != TLS_MAJOR_VERSION) return 0;

    rec_len = clib_net_to_host_u16(rec_hdr.length);
    if (rec_len == 0 || rec_len > TLS_FRAGMENT_MAX_ENC_LEN) return 0;
