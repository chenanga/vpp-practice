...
crypto_engine_type_t tls_get_available_engine(void)
{
    ...
    for (i = 0; i < vec_len(tls_vfts); i++) {
        if (tls_vfts[i].ctx_alloc) return i;
    }
...
static crypto_engine_type_t tls_get_engine_type(crypto_engine_type_t requested, crypto_engine_type_t preferred)
{
    if (requested != CRYPTO_ENGINE_NONE) {
        if (tls_vfts[requested].ctx_alloc) return requested;
        return CRYPTO_ENGINE_NONE;
    }
    if (!tls_vfts[preferred].ctx_alloc) return tls_get_available_engine();
    return preferred;
}

int tls_add_vpp_q_rx_evt(session_t *s)
{
    if (svm_fifo_set_event(s->rx_fifo)) session_send_io_evt_to_thread(s->rx_fifo, SESSION_IO_EVT_RX);
    return 0;
}
...
{
    if (svm_fifo_set_event(s->tx_fifo)) session_send_io_evt_to_thread(s->tx_fifo, SESSION_IO_EVT_TX);
    return 0;
}

static inline int tls_add_app_q_evt(app_worker_t *app_wrk, session_t *app_session)
{
...
{
    if (CLIB_DEBUG) memset(ctx, 0xfb, sizeof(*ctx));
...
u32 tls_ctx_half_open_alloc(void)
{
    ...
    if (vec_len(tm->postponed_ho_free)) tls_flush_postponed_ho_cleanups();
...
static void tls_ctx_ho_try_free(u32 ho_index)
{
    tls_ctx_t *ctx;
    ...
    if (!(ctx->flags & TLS_CONN_F_NO_APP_SESSION)) session_half_open_delete_notify(&ctx->connection);
...
void tls_notify_app_enqueue(tls_ctx_t *ctx, session_t *app_session)
{
    ...
    if (PREDICT_TRUE(app_wrk != 0)) tls_add_app_q_evt(app_wrk, app_session);
}
...
int tls_notify_app_connected(tls_ctx_t *ctx, session_error_t err)
{
    ...
    if (ctx->tls_type == TRANSPORT_PROTO_DTLS) {
        ...
        app_session->session_type = session_type_from_proto_and_ip(TRANSPORT_PROTO_DTLS, ctx->tcp_is_ip4);
    }
    else {
        app_session->session_type = session_type_from_proto_and_ip(TRANSPORT_PROTO_TLS, ctx->tcp_is_ip4);
    }
    ...
    if (app_worker_connect_notify(app_wrk, app_session, SESSION_E_NONE, parent_app_api_ctx)) {
...
send_reply:
    return app_worker_connect_notify(app_wrk, 0, err, ctx->parent_app_api_context);
}
...
void tls_session_reset_callback(session_t *ts)
{
    tls_ctx_t *ctx;
...
void tls_session_disconnect_callback(session_t *tls_session)
{
    ...
    TLS_DBG(1, "TCP disconnecting handle %x session %u", tls_session->opaque, tls_session->session_index);

    ASSERT(tls_session->thread_index == vlib_get_thread_index() || vlib_thread_is_main_w_barrier());
...
int tls_session_accept_callback(session_t *ts)
{
    ...
    TLS_DBG(1, "Accept on listener %u new connection [%u]%x", tls_listener->opaque, vlib_get_thread_index(), ctx_handle);
    ...
    if (ts->session_state < SESSION_STATE_READY) ts->session_state = SESSION_STATE_READY;
...
int tls_app_rx_callback(session_t *ts)
{
    ...
    if (PREDICT_FALSE(ts->flags & SESSION_F_IS_MIGRATING)) return 0;

    /* Read rescheduled but underlying transport deleted now */
    if (PREDICT_FALSE((ts->session_state == SESSION_STATE_TRANSPORT_DELETED))) return 0;

    ctx = tls_ctx_get(ts->opaque);
    if (PREDICT_FALSE((ctx->flags & TLS_CONN_F_NO_APP_SESSION) || (ctx->flags & TLS_CONN_F_APP_CLOSED))) {
...
int tls_session_connected_cb(u32 tls_app_index, u32 ho_ctx_index, session_t *tls_session, session_error_t err)
{
    ...
    TLS_DBG(1, "TCP connect for %u returned %u. New connection [%u]%x", ho_ctx_index, err, vlib_get_thread_index(), (ctx) ? ctx_handle : ~0);
    ...
    if (tls_session->session_state < SESSION_STATE_READY) tls_session->session_state = SESSION_STATE_READY;
...
int tls_session_connected_callback(u32 tls_app_index, u32 ho_ctx_index, session_t *tls_session, session_error_t err)
{
    ...
    if (session_get_transport_proto(tls_session) == TRANSPORT_PROTO_TCP)
        return tls_session_connected_cb(tls_app_index, ho_ctx_index, tls_session, err);
    else
        return dtls_session_connected_cb(tls_app_index, ho_ctx_index, tls_session, err);
}
...
static void tls_app_session_cleanup(session_t *s, session_cleanup_ntf_t ntf)
{
    ...
    if (ntf == SESSION_CLEANUP_TRANSPORT) {
        ...
        if (s->session_state == SESSION_STATE_TRANSPORT_DELETED) session_close(s);
        return;
    }

    ctx = tls_ctx_get(s->opaque);
    if (!(ctx->flags & TLS_CONN_F_NO_APP_SESSION)) session_transport_delete_notify(&ctx->connection);
    tls_ctx_free(ctx);
}
...
static void dtls_migrate_ctx(void *arg)
{
    ...
    if (svm_fifo_max_dequeue(us->tx_fifo)) session_send_io_evt_to_thread(us->tx_fifo, SESSION_IO_EVT_TX);
}
...
static void dtls_session_migrate_callback(session_t *us, session_handle_t new_sh)
{
    ...
    session_send_rpc_evt_to_thread(new_thread, dtls_migrate_ctx, (void *) cloned_ctx);

    tls_ctx_free(ctx);
}

static void tls_session_transport_closed_callback(session_t *ts)
{
    tls_ctx_t *ctx;

    ctx = tls_ctx_get_w_thread(ts->opaque, ts->thread_index);
    if (!(ctx->flags & TLS_CONN_F_NO_APP_SESSION)) session_transport_closed_notify(&ctx->connection);
}
...
u32 tls_start_listen(u32 app_listener_index, transport_endpoint_cfg_t *tep)
{
    ...
    if (tls_vfts[engine_type].ctx_start_listen(lctx)) {
        vnet_unlisten_args_t a = {.handle = lctx->tls_session_handle, .app_index = tls_main.app_index, .wrk_map_index = 0};
        if ((vnet_unlisten(&a))) clib_warning("unlisten returned");
        tls_listener_ctx_free(lctx);
        lctx_index = SESSION_INVALID_INDEX;
    }

    TLS_DBG(1, "Started listening %d, engine type %d", lctx_index, engine_type);
    return lctx_index;
}
...
int tls_custom_tx_callback(void *session, transport_send_params_t *sp)
{
    ...
    if (PREDICT_FALSE(as->session_state >= SESSION_STATE_TRANSPORT_CLOSED || as->session_state <= SESSION_STATE_ACCEPTING)) {
...
u8 *format_tls_ctx(u8 *s, va_list *args)
{
    ...
    s = format(s,
               "[%d:%d][%s] app_wrk %u index %u engine %u ts %d:%d",
               ctx->c_thread_index,
               ctx->c_s_index,
               proto,
...
static u8 *format_tls_listener_ctx(u8 *s, va_list *args)
{
    ...
    tls_listener = app_listener_get_session(al);
    s            = format(s,
               "[%d:%d][%s] app_wrk %u engine %u ts %d:%d",
               ctx->c_thread_index,
               ctx->c_s_index,
               proto,
               ctx->parent_app_wrk_index,
               ctx->tls_ctx_engine,
               tls_listener->thread_index,
               tls_listener->session_index);
...
u8 *format_tls_listener(u8 *s, va_list *args)
{
    ...
    if (verbose) s = format(s, "%-" SESSION_CLI_STATE_LEN "U", format_tls_ctx_state, ctx);
    return s;
}
...
u8 *format_tls_half_open(u8 *s, va_list *args)
{
    ...
    s      = format(s,
               "[%d:%d][%s] half-open app_wrk %u engine %u ts %d:%d",
               ho_ctx->c_thread_index,
               ho_ctx->c_s_index,
               "TLS",
               ho_ctx->parent_app_wrk_index,
               ho_ctx->tls_ctx_engine,
               tcp_ho->thread_index,
               tcp_ho->session_index);
...
static void tls_transport_endpoint_get(u32 ctx_handle, u32 thread_index, transport_endpoint_t *tep, u8 is_lcl)
{
    ...
    if (ts && ts->session_state < SESSION_STATE_TRANSPORT_DELETED) session_get_endpoint(ts, tep, is_lcl);
}
...
int dtls_connect(transport_endpoint_cfg_t *tep)
{
    ...
    if ((rv = vnet_connect(cargs))) return rv;

    TLS_DBG(1, "New DTLS connect request %x engine %d", ctx_handle, engine_type);
    return ctx_handle;
}
...
u8 *format_dtls_half_open(u8 *s, va_list *args)
{
    ...
    s  = format(s,
               "[%d:%d][%s] half-open app_wrk %u engine %u us %d:%d",
               ho_ctx->c_thread_index,
               ho_ctx->c_s_index,
               "DTLS",
               ho_ctx->parent_app_wrk_index,
...
static clib_error_t *tls_init(vlib_main_t *vm)
{
    ...
    if (!tm->ca_cert_path) tm->ca_cert_path = TLS_CA_CERT_PATH;
    ...
    transport_register_protocol(TRANSPORT_PROTO_TLS, &tls_proto, FIB_PROTOCOL_IP4, ~0);
    transport_register_protocol(TRANSPORT_PROTO_TLS, &tls_proto, FIB_PROTOCOL_IP6, ~0);

    transport_register_protocol(TRANSPORT_PROTO_DTLS, &dtls_proto, FIB_PROTOCOL_IP4, ~0);
    transport_register_protocol(TRANSPORT_PROTO_DTLS, &dtls_proto, FIB_PROTOCOL_IP6, ~0);
    return 0;
}
...
static clib_error_t *tls_config_fn(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "use-test-cert-in-ca"))
            ...
            ;
        else if (unformat(input, "first-segment-size %U", unformat_memory_size, &tm->first_seg_size))
            ;
        else if (unformat(input, "add-segment-size %U", unformat_memory_size, &tm->add_seg_size))
            ;
