...
static inline uword qos_store_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, qos_source_t qos_src)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            qos0 = *(qos_bits_t *) vnet_feature_next_with_data(&next0, b0, sizeof(qos_bits_t));
            ...
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                qos_store_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                t->bits              = qos0;
            }
