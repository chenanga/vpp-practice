...
static inline uword qos_record_inline(vlib_main_t         *vm,
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (DPO_PROTO_IP6 == dproto) {
                ...
                vlan0 = (vlib_buffer_get_current(b0) - sizeof(ethernet_vlan_header_t));
            ...
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                qos_record_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                t->bits               = qos0;
            }
        ...
        non_ip:
            if (is_l2) {
                vlib_buffer_advance(b0, -l2_len);
                next0 = vnet_l2_feature_next(b0, l2_qos_input_next[qos_src], L2INPUT_FEAT_L2_IP_QOS_RECORD);
            }
            else
...
{
    return (qos_record_inline(vm, node, frame, QOS_SOURCE_IP, DPO_PROTO_IP4, 0));
}

VLIB_NODE_FN(ip6_qos_record_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (qos_record_inline(vm, node, frame, QOS_SOURCE_IP, DPO_PROTO_IP6, 0));
}

VLIB_NODE_FN(mpls_qos_record_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (qos_record_inline(vm, node, frame, QOS_SOURCE_MPLS, DPO_PROTO_MPLS, 0));
}

VLIB_NODE_FN(vlan_ip4_qos_record_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (qos_record_inline(vm, node, frame, QOS_SOURCE_VLAN, DPO_PROTO_ETHERNET, 0));
}
