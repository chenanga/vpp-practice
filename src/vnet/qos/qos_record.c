...
static void qos_record_feature_config(u32 sw_if_index, qos_source_t input_source, u8 enable)
{
    switch (input_source) {
        case QOS_SOURCE_IP:
            ip_feature_enable_disable(AF_IP6, N_SAFI, IP_FEATURE_INPUT, "ip6-qos-record", sw_if_index, enable, NULL, 0);
            ip_feature_enable_disable(AF_IP4, N_SAFI, IP_FEATURE_INPUT, "ip4-qos-record", sw_if_index, enable, NULL, 0);
            l2input_intf_bitmap_enable(sw_if_index, L2INPUT_FEAT_L2_IP_QOS_RECORD, enable);
            break;
        case QOS_SOURCE_MPLS:
            vnet_feature_enable_disable("mpls-input", "mpls-qos-record", sw_if_index, enable, NULL, 0);
            break;
        case QOS_SOURCE_VLAN:
            ip_feature_enable_disable(AF_IP6, N_SAFI, IP_FEATURE_INPUT, "vlan-ip6-qos-record", sw_if_index, enable, NULL, 0);
            ip_feature_enable_disable(AF_IP4, N_SAFI, IP_FEATURE_INPUT, "vlan-ip4-qos-record", sw_if_index, enable, NULL, 0);
            vnet_feature_enable_disable("mpls-input", "vlan-mpls-qos-record", sw_if_index, enable, NULL, 0);
            break;
        case QOS_SOURCE_EXT:
...
{
    if (vec_len(qos_record_configs[input_source]) <= sw_if_index) return VNET_API_ERROR_NO_MATCHING_INTERFACE;

    if (0 == qos_record_configs[input_source][sw_if_index]) return VNET_API_ERROR_VALUE_EXIST;
...
void qos_record_walk(qos_record_walk_cb_t fn, void *c)
{
    ...
    FOR_EACH_QOS_SOURCE(qs)
    {
        ...
        vec_foreach_index (sw_if_index, qos_record_configs[qs]) {
            if (0 != qos_record_configs[qs][sw_if_index]) fn(sw_if_index, qs, c);
        }
    }
}
...
clib_error_t *qos_record_init(vlib_main_t *vm)
{
    ...
    FOR_EACH_QOS_SOURCE(qs)
    feat_bitmap_init_next_nodes(vm, node->index, L2INPUT_N_FEAT, l2input_get_feat_names(), l2_qos_input_next[qs]);
    return 0;
}
...
static clib_error_t *qos_record_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (0xff == qs) return clib_error_return(0, "input location must be specified");
...
static void qos_record_show_one_interface(vlib_main_t *vm, u32 sw_if_index)
{
    ...
    FOR_EACH_QOS_SOURCE(qs)
    {
        if (vec_len(qos_record_configs[qs]) <= sw_if_index) continue;
        if (0 != (n_cfgs[qs] = qos_record_configs[qs][sw_if_index])) set = true;
    }
    ...
    if (set) {
        ...
        FOR_EACH_QOS_SOURCE(qs)
        {
            if (n_cfgs[qs] != 0) vlib_cli_output(vm, "  %U", format_qos_source, qs);
        }
    }
}
