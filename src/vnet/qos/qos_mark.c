...
{
    vnet_feature_enable_disable("ip6-output", "ip6-qos-mark", sw_if_index, enable, NULL, 0);
    vnet_feature_enable_disable("ip4-output", "ip4-qos-mark", sw_if_index, enable, NULL, 0);
}
...
void qos_mark_vlan_enable_disable(u32 sw_if_index, u8 enable)
{
     ...
     */
    vnet_feature_enable_disable("ip6-output", "vlan-ip6-qos-mark", sw_if_index, enable, NULL, 0);
    vnet_feature_enable_disable("ip4-output", "vlan-ip4-qos-mark", sw_if_index, enable, NULL, 0);
    vnet_feature_enable_disable("mpls-output", "vlan-mpls-qos-mark", sw_if_index, enable, NULL, 0);
}

void qos_mark_mpls_enable_disable(u32 sw_if_index, u8 enable)
{
    vnet_feature_enable_disable("mpls-output", "mpls-qos-mark", sw_if_index, enable, NULL, 0);
}
...
int qos_mark_enable(u32 sw_if_index, qos_source_t output_source, qos_egress_map_id_t mid)
{
    ...
    vec_validate_init_empty(qos_mark_configs[output_source], sw_if_index, INDEX_INVALID);

    qemi = qos_egress_map_find(mid);

    if (INDEX_INVALID == qemi) return VNET_API_ERROR_NO_SUCH_TABLE;
...
{
    if (vec_len(qos_mark_configs[output_source]) <= sw_if_index) return VNET_API_ERROR_NO_MATCHING_INTERFACE;
    if (INDEX_INVALID == qos_mark_configs[output_source][sw_if_index]) return VNET_API_ERROR_VALUE_EXIST;
...
void qos_mark_walk(qos_mark_walk_cb_t fn, void *c)
{
    ...
    FOR_EACH_QOS_SOURCE(qs)
    {
        ...
        vec_foreach_index (sw_if_index, qos_mark_configs[qs]) {
            if (INDEX_INVALID != qos_mark_configs[qs][sw_if_index]) fn(sw_if_index, qos_egress_map_get_id(qos_mark_configs[qs][sw_if_index]), qs, c);
        }
    }
}
...
static clib_error_t *qos_mark_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (0xff == qs) return clib_error_return(0, "output location must be specified");
...
static void qos_mark_show_one_interface(vlib_main_t *vm, u32 sw_if_index)
{
    ...
    FOR_EACH_QOS_SOURCE(qs)
    {
        if (vec_len(qos_mark_configs[qs]) <= sw_if_index) continue;
        if (INDEX_INVALID != (qemis[qs] = qos_mark_configs[qs][sw_if_index])) set = true;
    }
    ...
    if (set) {
        ...
        FOR_EACH_QOS_SOURCE(qs)
        {
            if (qemis[qs] != INDEX_INVALID) vlib_cli_output(vm, "  %U: map:%d", format_qos_source, qs, qemis[qs]);
        }
    }
}
