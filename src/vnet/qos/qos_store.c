...
static void qos_store_feature_config(u32 sw_if_index, qos_source_t input_source, u8 enable, qos_bits_t value)
{
    switch (input_source) {
        case QOS_SOURCE_IP:
            vnet_feature_enable_disable("ip6-unicast", "ip6-qos-store", sw_if_index, enable, &value, sizeof(value));
            vnet_feature_enable_disable("ip6-multicast", "ip6-qos-store", sw_if_index, enable, &value, sizeof(value));
            vnet_feature_enable_disable("ip4-unicast", "ip4-qos-store", sw_if_index, enable, &value, sizeof(value));
            vnet_feature_enable_disable("ip4-multicast", "ip4-qos-store", sw_if_index, enable, &value, sizeof(value));
            break;
...
int qos_store_enable(u32 sw_if_index, qos_source_t input_source, qos_bits_t value)
{
    ...
    if (QOS_SOURCE_IP != input_source) return VNET_API_ERROR_UNIMPLEMENTED;
...
int qos_store_disable(u32 sw_if_index, qos_source_t input_source)
{
    ...
    if (vec_len(qos_store_configs[input_source]) <= sw_if_index) return VNET_API_ERROR_NO_MATCHING_INTERFACE;

    qst = &qos_store_configs[input_source][sw_if_index];

    if (0 == qst->qst_n_cfgs) return VNET_API_ERROR_VALUE_EXIST;
...
void qos_store_walk(qos_store_walk_cb_t fn, void *c)
{
    ...
    FOR_EACH_QOS_SOURCE(qs)
    {
        ...
        vec_foreach_index (sw_if_index, qos_store_configs[qs]) {
            ...
            if (0 != qst->qst_n_cfgs) fn(sw_if_index, qs, qst->qst_value, c);
        }
    }
}
...
static clib_error_t *qos_store_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (~0 == value) return clib_error_return(0, "value to be stored must be specified");
...
static void qos_store_show_one_interface(vlib_main_t *vm, u32 sw_if_index)
{
    ...
    FOR_EACH_QOS_SOURCE(qs)
    {
        if (vec_len(qos_store_configs[qs]) <= sw_if_index) continue;
        if (0 != (n_cfgs[qs] = qos_store_configs[qs][sw_if_index].qst_n_cfgs)) set = true;
    }
    ...
    if (set) {
        ...
        FOR_EACH_QOS_SOURCE(qs)
        {
            if (n_cfgs[qs] != 0) vlib_cli_output(vm, "  %U -> %d", format_qos_source, qs, qos_store_configs[qs][sw_if_index].qst_value);
        }
    }
}
