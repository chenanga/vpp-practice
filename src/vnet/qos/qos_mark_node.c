...
always_inline qos_egress_map_t *qos_egress_map_interface(u32 sw_if_index, qos_source_t output_source)
{
    ASSERT(vec_len(qos_mark_configs[output_source]) > sw_if_index);

    return pool_elt_at_index(qem_pool, qos_mark_configs[output_source][sw_if_index]);
}
...
static inline uword qos_mark_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, qos_source_t output_source, int is_ip6)
{
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_TRUE(b0->flags & VNET_BUFFER_F_QOS_DATA_VALID)) {
                ...
                if (QOS_SOURCE_IP == output_source) {
                    if (is_ip6) {
                        ip6_0 = (vlib_buffer_get_current(b0) + vnet_buffer(b0)->ip.save_rewrite_length);

                        ip6_set_traffic_class_network_order(ip6_0, qos0);
                    }
                    else {
                        ip4_0 = (vlib_buffer_get_current(b0) + vnet_buffer(b0)->ip.save_rewrite_length);
                    ...
                    mpls_bytes_0 = (vlib_buffer_get_current(b0) + vnet_buffer(b0)->mpls.save_rewrite_length);
                    ...
                    vlan0 = (vlib_buffer_get_current(b0) + sizeof(ethernet_header_t));
...
static u8 *format_qos_mark_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "source:%U qos:%d used:%s", format_qos_source, t->input, t->bits, (t->used ? "yes" : "no"));
