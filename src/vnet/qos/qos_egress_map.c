...
void qos_egress_map_update(qos_egress_map_id_t mid, qos_source_t input_source, qos_bits_t *values)
{
    ...
    clib_memcpy(qem->qem_output[input_source], values, sizeof(qem->qem_output[input_source]));
}
...
void qos_egress_map_walk(qos_egress_map_walk_cb_t fn, void *c)
{
    ...
    hash_foreach(qid, qmi, qem_db, ({ fn(qid, pool_elt_at_index(qem_pool, qmi), c); }));
}
...
static clib_error_t *qos_egress_map_update_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "delete") || unformat(input, "del"))
            ...
            if (NULL == qem) return clib_error_return(0, "map-id must be specified");

            while (unformat(input, "[%U][%d]=%d", unformat_qos_source, &qs, &qi, &qo))
                qem->qem_output[qs][qi] = qo;
            break;
        }
    }

    if (!add) qos_egress_map_delete(map_id);
...
u8 *format_qos_egress_map(u8 *s, va_list *args)
{
    ...
    {
        s = format(s, "%U%U:[", format_white_space, indent, format_qos_source, qs);
...
static clib_error_t *qos_egress_map_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (~0 == map_id) {
        ...
        hash_foreach(map_id, qemi, qem_db, ({
                         vlib_cli_output(vm, " Map-ID:%d\n%U", map_id, format_qos_egress_map, pool_elt_at_index(qem_pool, qemi), 2);
                     }));
    }
    else {
        ...
        if (NULL == qem) {
            ...
            vlib_cli_output(vm, " Map-ID:%d\n%U", map_id, format_qos_egress_map, qem, 2);
        }
    }
