...
void vl_api_qos_store_enable_disable_t_handler(vl_api_qos_store_enable_disable_t *mp)
{
    ...
    if (0 == rv) {
        if (mp->enable)
            rv = qos_store_enable(ntohl(mp->store.sw_if_index), qs, mp->store.value);
        else
...
void vl_api_qos_egress_map_update_t_handler(vl_api_qos_egress_map_update_t *mp)
{
    ...
    {
        qos_egress_map_update(ntohl(mp->map.id), qs, &mp->map.rows[qs].outputs[0]);
    }
...
void vl_api_qos_mark_enable_disable_t_handler(vl_api_qos_mark_enable_disable_t *mp)
{
    ...
    if (0 == rv) {
        if (mp->enable)
            rv = qos_mark_enable(ntohl(mp->mark.sw_if_index), qs, ntohl(mp->mark.map_id));
        else
