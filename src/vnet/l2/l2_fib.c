...
#define _(a, v, t)                                                        \
    {                                                                     \
        if (flags & L2FIB_ENTRY_RESULT_FLAG_##a) s = format(s, "%s ", t); \
    }
...
u8 *format_vnet_sw_if_index_name_with_NA(u8 *s, va_list *args)
{
    ...
    if (sw_if_index == ~0) return format(s, "N/A");

    vnet_sw_interface_t *swif = vnet_get_sw_interface_or_null(vnm, sw_if_index);
    if (!swif) return format(s, "Stale");
...
static int l2fib_show_walk_cb(BVT(clib_bihash_kv) * kvp, void *arg)
{
    ...
    if (ctx->verbose && ((ctx->bd_index >> 31) || (ctx->bd_index == key.fields.bd_index))) {
        u8 *s = NULL;

        if (ctx->learn && l2fib_entry_result_is_set_AGE_NOT(&result)) return (BIHASH_WALK_CONTINUE); /* skip provisioned macs */

        if (ctx->add && !l2fib_entry_result_is_set_AGE_NOT(&result)) return (BIHASH_WALK_CONTINUE); /* skip learned macs */
        ...
        vlib_cli_output(ctx->vm,
                        ...
                        key.fields.mac,
                        key.fields.bd_index,
                        result.fields.sw_if_index == ~0 ? -1 : result.fields.sw_if_index,
...
static clib_error_t *show_l2fib(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "raw")) {
            ...
            if (p) {
                ...
                return clib_error_return(0, "bridge domain id %d doesn't exist\n", bd_id);
        }
        else
            break;
    }
    ...
    if (ctx.total_entries == 0)
        ...
        vlib_cli_output(vm,
                        "L2FIB total/learned entries: %d/%d  "
                        "Last scan time: %.4esec  Learn limit: %d ",
                        ctx.total_entries,
                        lm->global_learn_count,
                        msm->age_scan_duration,
                        lm->global_learn_limit);
        if (lm->client_pid)
            vlib_cli_output(vm,
                            "L2MAC events client PID: %d  "
                            "Last e-scan time: %.4esec  Delay: %.2esec  "
                            "Max macs in event: %d",
                            lm->client_pid,
                            msm->evt_scan_duration,
                            msm->event_scan_delay,
                            msm->max_macs_in_event);
    }
...
void l2fib_table_init(void)
{
    ...
    if (mp->mac_table_initialized == 1) return;

    BV(clib_bihash_init)(&mp->mac_table, "l2fib mac table", mp->mac_table_n_buckets, mp->mac_table_memory_size);
...
void l2fib_clear_table(void)
{
    ...
    if (mp->mac_table_initialized == 0) return;
...
static l2fib_seq_num_t l2fib_cur_seq_num(u32 bd_index, u32 sw_if_index)
{
    ...
    return l2_fib_mk_seq_num(bd_config->seq_num, l2_input_seq_num(sw_if_index));
}
...
void l2fib_add_entry(const u8 *mac, u32 bd_index, u32 sw_if_index, l2fib_entry_result_flags_t flags)
{
    ...
    if (fm->mac_table_initialized == 0) l2fib_table_init();
    ...
    if (BV(clib_bihash_search)(&fm->mac_table, &kv, &kv)) {
        ...
        if (!l2fib_entry_result_is_set_AGE_NOT(&result)) {
            ...
            if (lm->global_learn_count) lm->global_learn_count--;
            if (bd_config->learn_count) bd_config->learn_count--;
        }
    }
...
static clib_error_t *l2fib_test_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (mac_set == 0) return clib_error_return(0, "mac not set");

    if (is_add == 0 && is_del == 0 && is_check == 0) return clib_error_return(0, "noop: pick at least one of (add,del,check)");

    clib_memcpy_fast(save_mac, mac, 6);

    if (is_add) {
        for (i = 0; i < count; i++) {
            l2fib_add_entry(mac, bd_index, sw_if_index, L2FIB_ENTRY_RESULT_FLAG_NONE);
    ...
    if (is_check) {
        ...
        if (mp->mac_table_initialized == 0) return clib_error_return(0, "mac table is not initialized");
...
u32 l2fib_del_entry(const u8 *mac, u32 bd_index, u32 sw_if_index)
{
    ...
    if (mp->mac_table_initialized == 0) return 1;

    /* set up key */
    kv.key = l2fib_make_key(mac, bd_index);

    if (BV(clib_bihash_search)(&mp->mac_table, &kv, &kv)) return 1;

    result.raw = kv.value;

    /*  check if sw_if_index of entry match */
    if ((sw_if_index != 0) && (sw_if_index != result.fields.sw_if_index)) return 1;

    /* decrement counter if dynamically learned mac */
    if (!l2fib_entry_result_is_set_AGE_NOT(&result)) {
        l2_bridge_domain_t *bd_config = vec_elt_at_index(l2input_main.bd_configs, bd_index);
        if (l2learn_main.global_learn_count) l2learn_main.global_learn_count--;
        if (bd_config->learn_count) bd_config->learn_count--;
    }
...
static clib_error_t *l2fib_set_scan_delay(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!unformat(input, "%d", &scan_delay)) {
        error = clib_error_return(0, "expecting delay but got `%U'", format_unformat_error, input);
        goto done;
    }
...
void l2fib_start_ager_scan(vlib_main_t *vm)
{
    ...
    vlib_process_signal_event(vm, l2fib_mac_age_scanner_process_node.index, evt, 0);
}
...
void l2fib_flush_all_mac(vlib_main_t *vm)
{
    l2_bridge_domain_t *bd_config;
    vec_foreach (bd_config, l2input_main.bd_configs)
        if (bd_is_valid(bd_config)) bd_config->seq_num += 1;
...
static clib_error_t *l2fib_flush_mac_bd(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!unformat(input, "%d", &bd_id)) {
        error = clib_error_return(0, "expecting bridge-domain id but got `%U'", format_unformat_error, input);
        goto done;
    }
...
clib_error_t *l2fib_sw_interface_up_down(vnet_main_t *vnm, u32 sw_if_index, u32 flags)
{
    ...
    if ((flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP) == 0 && l2_input_is_bridge(config)) l2fib_flush_int_mac(vnm->vlib_main, sw_if_index);
    return 0;
}
...
{
    l2fib_main_t           *fm = &l2fib_main;
    vl_api_l2_macs_event_t *mp = vl_msg_api_alloc(sizeof(*mp) + (fm->max_macs_in_event * sizeof(vl_api_mac_entry_t)));
...
static_always_inline f64 l2fib_scan(vlib_main_t *vm, f64 start_time, u8 event_only)
{
    ...
    for (i = 0; i < h->nbuckets; i++) {
        ...
        if (i < (h->nbuckets - 3)) {
            BVT(clib_bihash_bucket) *b = BV(clib_bihash_get_bucket)(h, i + 3);
        ...
        for (j = 0; j < (1 << b->log2_pages); j++) {
            for (k = 0; k < BIHASH_KVP_PER_PAGE; k++) {
                ...
                if (client) {
                    if (PREDICT_FALSE(evt_idx >= fm->max_macs_in_event)) {
                        ...
                        if (reg && vl_api_can_send_msg(reg)) {
                            ...
                            if (reg)
                                clib_warning(
                                    "MAC event to pid %d queue stuffed!"
                                    " %d MAC entries lost",
                                    client,
                                    evt_idx);
                        }
                    ...
                    if (l2fib_entry_result_is_set_LRN_EVT(&result)) {
                        ...
                        mp->mac[evt_idx].action      = l2fib_entry_result_is_set_LRN_MOV(&result) ? (vl_api_mac_event_action_t) MAC_EVENT_ACTION_MOVE
                ...
                if (event_only || l2fib_entry_result_is_set_AGE_NOT(&result)) continue; /* skip aging - static_mac always age_not */
                ...
                if (result.fields.sn != sn) goto age_out; /* stale mac */

                l2_bridge_domain_t *bd_config = vec_elt_at_index(l2input_main.bd_configs, bd_index);

                if (bd_config->mac_age == 0) continue; /* skip aging */

                i16 delta = (u8) (start_time / 60) - result.fields.timestamp;
                delta += delta < 0 ? 256 : 0;

                if (delta < bd_config->mac_age) continue; /* still valid */
    ...
    vec_foreach_index (bd_index, l2input_main.bd_configs) {
        vec_elt(l2input_main.bd_configs, bd_index).learn_count = vec_elt(bd_learn_counts, bd_index);
    }
    ...
    while (1) {
        ...
        switch (event_type) {
            case ~0: /* timer expired */
                if (lm->client_pid != 0 && start_time < next_age_scan_time) scan = SCAN_MAC_EVENT;
                break;
        ...
        if (scan == SCAN_MAC_EVENT)
            ...
            if (scan == SCAN_MAC_AGE) l2fib_main.age_scan_duration = l2fib_scan(vm, start_time, 0);
...
clib_error_t *l2fib_init(vlib_main_t *vm)
{
    ...
    if (mp->mac_table_n_buckets == 0) mp->mac_table_n_buckets = L2FIB_NUM_BUCKETS;
    if (mp->mac_table_memory_size == 0) mp->mac_table_memory_size = L2FIB_MEMORY_SIZE;
    mp->mac_table_initialized = 0;
...
static clib_error_t *lfib_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "table-size %U", unformat_memory_size, &table_size))
            ;
    ...
    if (n_buckets != ~0) {
        if (!is_pow2(n_buckets)) return clib_error_return(0, "num-buckets must be power of 2");
        lm->mac_table_n_buckets = n_buckets;
    }

    if (table_size != ~0) lm->mac_table_memory_size = table_size;
    return 0;
}
