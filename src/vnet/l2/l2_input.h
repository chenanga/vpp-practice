...
{
    return ((bd_config->feature_bitmap & L2INPUT_FEAT_FLOOD) == L2INPUT_FEAT_FLOOD);
}

static_always_inline u8 bd_feature_uu_flood(l2_bridge_domain_t *bd_config)
{
    return ((bd_config->feature_bitmap & L2INPUT_FEAT_UU_FLOOD) == L2INPUT_FEAT_UU_FLOOD);
}
...
{
    return ((bd_config->feature_bitmap & L2INPUT_FEAT_LEARN) == L2INPUT_FEAT_LEARN);
}
...
{
    return ((bd_config->feature_bitmap & L2INPUT_FEAT_ARP_UFWD) == L2INPUT_FEAT_ARP_UFWD);
}

static inline bool l2_input_is_bridge(const l2_input_config_t *input)
{
    return (input->flags & L2_INPUT_FLAG_BRIDGE);
}

static inline bool l2_input_is_xconnect(const l2_input_config_t *input)
{
    return (input->flags & L2_INPUT_FLAG_XCONNECT);
}

static inline bool l2_input_is_bvi(const l2_input_config_t *input)
{
...
static inline u16 vnet_update_l2_len(vlib_buffer_t *b)
{
...
static inline u32 vnet_l2_compute_flow_hash(vlib_buffer_t *b)
{
    ...
    if (ethertype == ETHERNET_TYPE_IP4)
        ...
        return mpls_compute_flow_hash((mpls_unicast_header_t *) l3h, IP_FLOW_HASH_DEFAULT);
    else {
        u32  a, b, c;
