...
static void vl_api_l2_xconnect_dump_t_handler(vl_api_l2_xconnect_dump_t *mp)
{
    ...
    vec_foreach_index (sw_if_index, l2im->configs) {
        ...
        if (l2_input_is_xconnect(config)) send_l2_xconnect_details(reg, mp->context, sw_if_index, config->output_sw_if_index);
    }
}
...
static void vl_api_l2_fib_table_dump_t_handler(vl_api_l2_fib_table_dump_t *mp)
{
    ...
    vec_foreach_index (ni, l2fe_key) {
        send_l2fib_table_entry(am, reg, vec_elt_at_index(l2fe_key, ni), vec_elt_at_index(l2fe_res, ni), mp->context);
    }
    vec_free(l2fe_key);
    vec_free(l2fe_res);
}
...
static void vl_api_l2fib_add_del_t_handler(vl_api_l2fib_add_del_t *mp)
{
    ...
    if (mp->is_add) {
        if (mp->filter_mac)
            ...
            if (mp->static_mac) flags |= L2FIB_ENTRY_RESULT_FLAG_STATIC;
            if (mp->bvi_mac) flags |= L2FIB_ENTRY_RESULT_FLAG_BVI;
            l2fib_add_entry(mac.bytes, bd_index, sw_if_index, flags);
        }
    }
    else {
        u32 sw_if_index = ntohl(mp->sw_if_index);
        if (l2fib_del_entry(mac.bytes, bd_index, sw_if_index)) rv = VNET_API_ERROR_NO_SUCH_ENTRY;
    }
...
static void vl_api_want_l2_macs_events2_t_handler(vl_api_want_l2_macs_events2_t *mp)
{
    ...
    if (mp->enable_disable) {
        if ((lm->client_pid == 0) || (lm->client_pid == pid)) {
            ...
            if (fm->event_scan_delay == 0.0) fm->event_scan_delay = (f64) (10) * 10e-3;
...
static void vl_api_want_l2_macs_events_t_handler(vl_api_want_l2_macs_events_t *mp)
{
    ...
    if (mp->enable_disable) {
        if ((lm->client_pid == 0) || (lm->client_pid == pid)) {
            if ((mp->max_macs_in_event == 0) || (mp->scan_delay == 0) || (learn_limit == 0) || (learn_limit > L2LEARN_DEFAULT_LIMIT)) {
                rv = VNET_API_ERROR_INVALID_VALUE;
                goto exit;
            }
            lm->client_pid   = pid;
...
static void vl_api_l2_flags_t_handler(vl_api_l2_flags_t *mp)
{
    ...
    if (flags & L2_LEARN) bitmap |= L2INPUT_FEAT_LEARN;

    if (flags & L2_FWD) bitmap |= L2INPUT_FEAT_FWD;

    if (flags & L2_FLOOD) bitmap |= L2INPUT_FEAT_FLOOD;

    if (flags & L2_UU_FLOOD) bitmap |= L2INPUT_FEAT_UU_FLOOD;

    if (flags & L2_ARP_TERM) bitmap |= L2INPUT_FEAT_ARP_TERM;
...
static void vl_api_bridge_domain_add_del_v2_t_handler(vl_api_bridge_domain_add_del_v2_t *mp)
{
    vl_api_bridge_domain_add_del_v2_reply_t *rmp;
    u32                                      bd_id = ntohl(mp->bd_id);
    int                                      rv    = 0;

    if ((~0 == bd_id) && (mp->is_add)) bd_id = bd_get_unused_id();
    ...
    REPLY_MACRO2(VL_API_BRIDGE_DOMAIN_ADD_DEL_V2_REPLY, ({ rmp->bd_id = htonl(bd_id); }));
}
...
static void send_bridge_domain_details(l2input_main_t *l2im, vl_api_registration_t *reg, l2_bridge_domain_t *bd_config, u32 n_sw_ifs, u32 context)
{
    ...
    mp = vl_msg_api_alloc(sizeof(*mp) + (n_sw_ifs * sizeof(vl_api_bridge_domain_sw_if_t)));
    clib_memset(mp, 0, sizeof(*mp));
    ...
    if (bd_config->bd_tag) {
        strncpy((char *) mp->bd_tag, (char *) bd_config->bd_tag, ARRAY_LEN(mp->bd_tag) - 1);
...
static void vl_api_bridge_domain_dump_t_handler(vl_api_bridge_domain_dump_t *mp)
{
    ...
    if (filter_sw_if_index != ~0) return; /* UNIMPLEMENTED */
    ...
    if (bd_id == ~0)
        ...
        if (bd_index == ~0) return;

        end = bd_index + 1;
    }

    for (; bd_index < end; bd_index++) {
        l2_bridge_domain_t *bd_config = l2input_bd_config_from_index(l2im, bd_index);
        /* skip placeholder bd_id 0 */
        if (bd_config && (bd_config->bd_id > 0)) send_bridge_domain_details(l2im, reg, bd_config, vec_len(bd_config->members), mp->context);
    }
}
...
static bd_flags_t bd_flags_decode(vl_api_bd_flags_t v)
{
    ...
    if (v & BRIDGE_API_FLAG_LEARN) f |= L2_LEARN;
    if (v & BRIDGE_API_FLAG_FWD) f |= L2_FWD;
    if (v & BRIDGE_API_FLAG_FLOOD) f |= L2_FLOOD;
    if (v & BRIDGE_API_FLAG_UU_FLOOD) f |= L2_UU_FLOOD;
    if (v & BRIDGE_API_FLAG_ARP_TERM) f |= L2_ARP_TERM;
    if (v & BRIDGE_API_FLAG_ARP_UFWD) f |= L2_ARP_UFWD;
...
out:
    REPLY_MACRO2(VL_API_BRIDGE_FLAGS_REPLY, ({ rmp->resulting_feature_bitmap = ntohl(bitmap); }));
}
...
static void vl_api_l2_interface_vlan_tag_rewrite_t_handler(vl_api_l2_interface_vlan_tag_rewrite_t *mp)
{
    ...
    rv = l2vtr_configure(vm, vnm, ntohl(mp->sw_if_index), vtr_op, ntohl(mp->push_dot1q), ntohl(mp->tag1), ntohl(mp->tag2));
...
static void vl_api_sw_interface_set_l2_xconnect_t_handler(vl_api_sw_interface_set_l2_xconnect_t *mp)
{
    ...
    if (mp->enable) {
        ...
        rv = set_int_l2_mode(vm, vnm, MODE_L2_XC, rx_sw_if_index, 0, L2_BD_PORT_TYPE_NORMAL, 0, tx_sw_if_index);
    }
    else {
        rv = set_int_l2_mode(vm, vnm, MODE_L3, rx_sw_if_index, 0, L2_BD_PORT_TYPE_NORMAL, 0, 0);
    }
...
static void vl_api_sw_interface_set_l2_bridge_t_handler(vl_api_sw_interface_set_l2_bridge_t *mp)
{
    ...
    if (mp->enable) {
        ...
        rv = set_int_l2_mode(vm, vnm, MODE_L2_BRIDGE, rx_sw_if_index, bd_index, pt, mp->shg, 0);
    }
    else {
    ...
    for (bd_index = start; bd_index < end; bd_index++) {
        ...
        if (bd_is_valid(bd_config)) {
            ...
            hash_foreach(ip4_addr.as_u32, mac64, bd_config->mac_by_ip4, ({
                             ...
                             send_bd_ip_mac_entry(am, reg, bd_id, &ip, IP46_TYPE_IP4, &mac, mp->context);
                         }));
            ...
            hash_foreach_mem(ip6_addr, mac64, bd_config->mac_by_ip6, ({
                                 ...
                                 send_bd_ip_mac_entry(am, reg, bd_id, &ip, IP46_TYPE_IP6, &mac, mp->context);
                             }));
        }
    }
}
...
static void vl_api_bd_ip_mac_add_del_t_handler(vl_api_bd_ip_mac_add_del_t *mp)
{
    ...
    if (bd_add_del_ip_mac(bd_index, type, &ip_addr, &mac, mp->is_add)) rv = VNET_API_ERROR_UNSPECIFIED;
...
extern void l2_efp_filter_configure(vnet_main_t *vnet_main, u32 sw_if_index, u8 enable);
...
{
    extern int                       vnet_l2_patch_add_del(u32 rx_sw_if_index, u32 tx_sw_if_index, int is_add);
    ...
    rv = vnet_l2_patch_add_del(ntohl(mp->rx_sw_if_index), ntohl(mp->tx_sw_if_index), (int) (mp->is_add != 0));
...
static void vl_api_sw_interface_set_vpath_t_handler(vl_api_sw_interface_set_vpath_t *mp)
{
    ...
    vnet_feature_enable_disable("ip4-unicast", "vpath-input-ip4", sw_if_index, mp->enable, 0, 0);
    vnet_feature_enable_disable("ip4-multicast", "vpath-input-ip4", sw_if_index, mp->enable, 0, 0);
    vnet_feature_enable_disable("ip6-unicast", "vpath-input-ip6", sw_if_index, mp->enable, 0, 0);
    vnet_feature_enable_disable("ip6-multicast", "vpath-input-ip6", sw_if_index, mp->enable, 0, 0);
...
static void vl_api_bvi_create_t_handler(vl_api_bvi_create_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_BVI_CREATE_REPLY, ({ rmp->sw_if_index = ntohl(sw_if_index); }));
}
...
static bool l2_arp_term_publish_event_is_equal(const l2_arp_term_publish_event_t *e1, const l2_arp_term_publish_event_t *e2)
{
    ...
    return (ip46_address_is_equal(&e1->ip, &e2->ip) && (e1->sw_if_index == e2->sw_if_index) && (mac_address_equal(&e1->mac, &e2->mac)));
}
    ...
    while (1) {
        ...
        if (event_type == L2_ARP_TERM_EVENT_PUBLISH) {
            ...
            vec_foreach (event, l2am->publish_events) {
                ...
                if (l2_arp_term_publish_event_is_equal(event, (l2_arp_term_publish_event_t *) &last_event) && (now - last) < 10.0) {
                    continue;
                }
                last_event = *event;
                last       = now;
                ...
                pool_foreach (reg, vpe_api_main.l2_arp_term_events_registrations) {
                    ...
                    if (vl_reg && vl_api_can_send_msg(vl_reg)) {
                        ...
                        vevent->_vl_msg_id   = htons(REPLY_MSG_ID_BASE + VL_API_L2_ARP_TERM_EVENT);
...
static void vl_api_want_l2_arp_term_events_t_handler(vl_api_want_l2_arp_term_events_t *mp)
{
    ...
    if (p) {
        if (mp->enable) {
            ...
            hash_unset(am->l2_arp_term_events_registration_hash, mp->client_index);
            if (pool_elts(am->l2_arp_term_events_registrations) == 0) l2_arp_term_set_publisher_node(false);
            goto reply;
        }
    }
    if (mp->enable == 0) {
    ...
    hash_set(am->l2_arp_term_events_registration_hash, rp->client_index, rp - am->l2_arp_term_events_registrations);
...
static clib_error_t *l2_api_hookup(vlib_main_t *vm)
{
    ...
    vl_api_set_msg_thread_safe(am, REPLY_MSG_ID_BASE + VL_API_BRIDGE_DOMAIN_DUMP, 1);
