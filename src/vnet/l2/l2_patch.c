...
static u8 *format_l2_patch_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "L2_PATCH: rx %d tx %d", t->rx_sw_if_index, t->tx_sw_if_index);
    return s;
}
...
static_always_inline void l2_patch_trace(vlib_main_t *vm, vlib_node_runtime_t *node, l2_patch_main_t *l2pm, vlib_buffer_t *b, u32 sw_if_index)
{
    ...
    if ((b->flags & VLIB_BUFFER_IS_TRACED) == 0) return;
...
l2_patch_inline(vlib_main_t *vm, vlib_node_runtime_t *node, l2_patch_main_t *l2pm, vlib_buffer_t **b, u16 *next, u32 n_left, int do_trace)
{
    u32 sw_if_index[4];
    ...
    while (n_left >= 4) {
        ...
        vnet_buffer(b[0])->sw_if_index[VLIB_TX] = l2pm->tx_sw_if_index_by_rx_sw_if_index[sw_if_index[0]];
        vnet_buffer(b[1])->sw_if_index[VLIB_TX] = l2pm->tx_sw_if_index_by_rx_sw_if_index[sw_if_index[1]];
        vnet_buffer(b[2])->sw_if_index[VLIB_TX] = l2pm->tx_sw_if_index_by_rx_sw_if_index[sw_if_index[2]];
        vnet_buffer(b[3])->sw_if_index[VLIB_TX] = l2pm->tx_sw_if_index_by_rx_sw_if_index[sw_if_index[3]];
    ...
    while (n_left) {
        ...
        if (do_trace) l2_patch_trace(vm, node, l2pm, b[0], sw_if_index[0]);
...
int vnet_l2_patch_add_del(u32 rx_sw_if_index, u32 tx_sw_if_index, int is_add)
{
    ...
    if (rxhi->sw_if_index != rx_sw_if_index) return VNET_API_ERROR_INVALID_SW_IF_INDEX;

    txhi = vnet_get_sup_hw_interface(l2pm->vnet_main, tx_sw_if_index);
    if (txhi->sw_if_index != tx_sw_if_index) return VNET_API_ERROR_INVALID_SW_IF_INDEX_2;

    if (is_add) {
        tx_next_index = vlib_node_add_next(l2pm->vlib_main, l2_patch_node.index, txhi->output_node_index);

        vec_validate_init_empty(l2pm->tx_next_by_rx_sw_if_index, rx_sw_if_index, ~0);

        l2pm->tx_next_by_rx_sw_if_index[rx_sw_if_index] = tx_next_index;
        vec_validate_init_empty(l2pm->tx_sw_if_index_by_rx_sw_if_index, rx_sw_if_index, ~0);
        l2pm->tx_sw_if_index_by_rx_sw_if_index[rx_sw_if_index] = txhi->sw_if_index;

        ethernet_set_flags(l2pm->vnet_main, rxhi->hw_if_index, ETHERNET_INTERFACE_FLAG_ACCEPT_ALL);

        vnet_feature_enable_disable("device-input", "l2-patch", rxhi->sw_if_index, 1, 0, 0);
        vnet_feature_enable_disable("port-rx-eth", "l2-patch", rxhi->sw_if_index, 1, 0, 0);
    }
    else {
        ethernet_set_flags(l2pm->vnet_main,
                           rxhi->hw_if_index,
                           /*ETHERNET_INTERFACE_FLAG_DEFAULT_L3 */ 0);

        vnet_feature_enable_disable("device-input", "l2-patch", rxhi->sw_if_index, 0, 0, 0);
        vnet_feature_enable_disable("port-rx-eth", "l2-patch", rxhi->sw_if_index, 0, 0, 0);
...
static clib_error_t *test_patch_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "rx %U", unformat_vnet_sw_interface, l2pm->vnet_main, &rx_sw_if_index))
            rx_set = 1;
        else if (unformat(line_input, "tx %U", unformat_vnet_sw_interface, l2pm->vnet_main, &tx_sw_if_index))
            tx_set = 1;
    ...
    switch (rv) {
        ...
        default:
            error = clib_error_return(0, "WARNING: vnet_l2_patch_add_del returned %d", rv);
            goto done;
    }
...
static clib_error_t *show_l2patch(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    ASSERT(vec_len(l2pm->tx_next_by_rx_sw_if_index) == vec_len(l2pm->tx_sw_if_index_by_rx_sw_if_index));

    for (rx_sw_if_index = 0; rx_sw_if_index < vec_len(l2pm->tx_sw_if_index_by_rx_sw_if_index); rx_sw_if_index++) {
        u32 tx_sw_if_index = l2pm->tx_sw_if_index_by_rx_sw_if_index[rx_sw_if_index];
        if (tx_sw_if_index != ~0) {
            no_entries = 0;
            vlib_cli_output(vm,
                            "%26U -> %U",
    ...
    if (no_entries) vlib_cli_output(vm, "no l2patch entries");
