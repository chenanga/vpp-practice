...
static u8 *format_l2_xcrw_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "L2_XCRW: next index %d tx_fib_index %d", t->next_index, t->tx_fib_index);
    return s;
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            vnet_buffer(b1)->sw_if_index[VLIB_TX] = adj1->rewrite_header.sw_if_index;

            em->counters[node_counter_base_index + next1]++;

            if (PREDICT_TRUE(next0 > 0)) {
                u8 *h0 = vlib_buffer_get_current(b0);
                vnet_rewrite_one_header(adj0[0], h0, adj0->rewrite_header.data_bytes);
            ...
            if (PREDICT_TRUE(next1 > 0)) {
                ...
                vnet_rewrite_one_header(adj1[0], h1, adj1->rewrite_header.data_bytes);
            ...
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE))) {
                ...
                if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b1->flags & VLIB_BUFFER_IS_TRACED))) {
                    l2_xcrw_trace_t *t = vlib_add_trace(vm, node, b1, sizeof(*t));
                    t->next_index      = next1;
...
int vnet_configure_l2_xcrw(vlib_main_t *vm, vnet_main_t *vnm, u32 l2_sw_if_index, u32 tx_fib_index, u8 *rewrite, u32 next_node_index, int is_add)
{
    ...
    if (is_add) {
        ...
        if (t->tunnel_sw_if_index == 0)
            ...
            vnet_sw_interface_set_flags(vnm, t->tunnel_sw_if_index, VNET_SW_INTERFACE_FLAG_ADMIN_UP);
         ...
         */
        a->rewrite_header.next_index = vlib_node_add_next(vm, l2_xcrw_node.index, next_node_index);

        if (vec_len(rewrite)) vnet_rewrite_set_data(a[0], rewrite, vec_len(rewrite));

        set_int_l2_mode(vm, vnm, MODE_L2_XC, t->l2_sw_if_index, 0, L2_BD_PORT_TYPE_NORMAL, 0, t->tunnel_sw_if_index);
        hash_set(xcm->tunnel_index_by_l2_sw_if_index, t->l2_sw_if_index, t - xcm->tunnels);
        return 0;
    }
    else {
        p = hash_get(xcm->tunnel_index_by_l2_sw_if_index, l2_sw_if_index);
        if (p == 0) return VNET_API_ERROR_INVALID_SW_IF_INDEX;
        ...
        set_int_l2_mode(vm, vnm, MODE_L3, t->l2_sw_if_index, 0, L2_BD_PORT_TYPE_NORMAL, 0, 0);
...
static clib_error_t *set_l2_xcrw_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!unformat(line_input, "%U", unformat_vnet_sw_interface, vnm, &l2_sw_if_index)) {
        error = clib_error_return(0, "unknown input '%U'", format_unformat_error, line_input);
        goto done;
    }

    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "next %U", unformat_vlib_node, vm, &next_node_index))
            ;
    ...
    rv = vnet_configure_l2_xcrw(vm, vnm, l2_sw_if_index, tx_fib_index, rw, next_node_index, is_add);
    ...
    switch (rv) {
        ...
        case VNET_API_ERROR_INVALID_SW_IF_INDEX:
            error = clib_error_return(0, "%U not cross-connected", format_vnet_sw_if_index_name, vnm, l2_sw_if_index);
            goto done;
...
static u8 *format_l2xcrw(u8 *s, va_list *args)
{
    ...
    s = format(s, "%-25U %U ", format_vnet_sw_if_index_name, vnm, t->l2_sw_if_index, format_vnet_sw_if_index_name, vnm, t->tunnel_sw_if_index);

    a = vec_elt_at_index(xcm->adj_by_sw_if_index, t->l2_sw_if_index);

    s = format(s, "next %U ", format_vlib_next_node_name, vm, l2_xcrw_node.index, a->rewrite_header.next_index);

    if (a->rewrite_header.sw_if_index != ~0) s = format(s, "tx fib index %d ", a->rewrite_header.sw_if_index);

    if (a->rewrite_header.data_bytes) {
        rewrite_string = (u8 *) (a + 1);
        rewrite_string -= a->rewrite_header.data_bytes;
        s = format(s, "rewrite data: %U ", format_hex_bytes, rewrite_string, a->rewrite_header.data_bytes);
    }
