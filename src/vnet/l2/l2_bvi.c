...
int l2_bvi_create(u32 user_instance, const mac_address_t *mac_in, u32 *sw_if_indexp)
{
    ...
    *sw_if_indexp = (u32) ~0;
    ...
    slot = vlib_node_add_named_next_with_slot(vm, hw_if->tx_node_index, "l2-input", 0);
    ASSERT(slot == 0);
...
int l2_bvi_delete(u32 sw_if_index)
{
    ...
    if (hw == 0 || hw->dev_class_index != bvi_device_class.index) return VNET_API_ERROR_INVALID_SW_IF_INDEX;

    if (bvi_instance_free(hw->dev_instance) < 0) return VNET_API_ERROR_INVALID_SW_IF_INDEX;
...
static clib_error_t *l2_bvi_create_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (rv) return clib_error_return(0, "BVI create failed");
...
static clib_error_t *l2_bvi_delete_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (~0 != sw_if_index) {
        ...
        if (rv) return clib_error_return(0, "BVI delete failed");
    }
    else
