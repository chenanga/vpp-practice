...
static inline uword l2_in_out_acl_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_output)
{
    ...
    while (n_left_from > 2) {
        ...
        table_index0 = am->classify_table_index_by_sw_if_index[is_output][tid][sw_if_index0];

        sw_if_index1 = vnet_buffer(b1)->sw_if_index[is_output ? VLIB_TX : VLIB_RX];
        table_index1 = am->classify_table_index_by_sw_if_index[is_output][tid][sw_if_index1];
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            next0 = vnet_l2_feature_next(b0, msm->feat_next_node_index[is_output], is_output ? L2OUTPUT_FEAT_ACL : L2INPUT_FEAT_ACL);
            ...
            if (PREDICT_TRUE(table_index0 != ~0)) {
                ...
                if (e0) {
                    ...
                    next0 = (e0->next_index < ACL_NEXT_INDEX_N_NEXT) ? e0->next_index : next0;

                    hits++;

                    if (is_output)
                        error0 = (next0 == ACL_NEXT_INDEX_DENY) ? L2_OUTACL_ERROR_SESSION_DENY : L2_INACL_ERROR_NONE;
                    else
                        error0 = (next0 == ACL_NEXT_INDEX_DENY) ? L2_OUTACL_ERROR_SESSION_DENY : L2_OUTACL_ERROR_NONE;
                    ...
                    while (1) {
                        if (PREDICT_TRUE(t0->next_table_index != ~0))
                            ...
                            next0 = (t0->miss_next_index < ACL_NEXT_INDEX_N_NEXT) ? t0->miss_next_index : next0;

                            misses++;

                            if (is_output)
                                error0 = (next0 == ACL_NEXT_INDEX_DENY) ? L2_OUTACL_ERROR_TABLE_MISS : L2_OUTACL_ERROR_NONE;
                            else
                                error0 = (next0 == ACL_NEXT_INDEX_DENY) ? L2_INACL_ERROR_TABLE_MISS : L2_INACL_ERROR_NONE;
                            b0->error = node->errors[error0];
                            break;
                        }
                        ...
                        if (e0) {
                            ...
                            if (is_output)
                                ...
                                error0 = (next0 == ACL_NEXT_INDEX_DENY) ? L2_INACL_ERROR_SESSION_DENY : L2_INACL_ERROR_NONE;
                            b0->error = node->errors[error0];
                            break;
                        }
                    }
            ...
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                l2_in_out_acl_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                t->sw_if_index           = vnet_buffer(b0)->sw_if_index[is_output ? VLIB_TX : VLIB_RX];
                t->next_index            = next0;
    ...
    vlib_node_increment_counter(vm, node->node_index, is_output ? L2_OUTACL_ERROR_MISS : L2_INACL_ERROR_MISS, misses);
    vlib_node_increment_counter(vm, node->node_index, is_output ? L2_OUTACL_ERROR_HIT : L2_INACL_ERROR_HIT, hits);
    vlib_node_increment_counter(vm, node->node_index, is_output ? L2_OUTACL_ERROR_CHAIN_HIT : L2_INACL_ERROR_CHAIN_HIT, chain_hits);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return l2_in_out_acl_node_fn(vm, node, frame, IN_OUT_ACL_OUTPUT_TABLE_GROUP);
}
