...
#define _(a, v, s)                                                                 \
    always_inline int l2fib_entry_result_is_set_##a(const l2fib_entry_result_t *r) \
    {                                                                              \
...
#undef _
    static inline void
    l2fib_entry_result_set_bits(l2fib_entry_result_t *r, l2fib_entry_result_flags_t bits)
{
    r->fields.flags |= bits;
}

static inline void l2fib_entry_result_clear_bits(l2fib_entry_result_t *r, l2fib_entry_result_flags_t bits)
{
    r->fields.flags &= ~bits;
}
...
always_inline u64 l2fib_make_key(const u8 *mac_address, u16 bd_index)
{
                                         ...
                                         l2fib_entry_result_t *result3)
{
    ...
    if ((key0->raw == cached_key->raw) && (key1->raw == cached_key->raw) && (key2->raw == cached_key->raw) && (key3->raw == cached_key->raw)) {
        /* Both hit in the one-entry cache */
        result0->raw = cached_result->raw;
        result1->raw = cached_result->raw;
...
static inline void l2fib_add_filter_entry(const u8 *mac, u32 bd_index)
{
    l2fib_add_entry(mac, bd_index, ~0, (L2FIB_ENTRY_RESULT_FLAG_FILTER | L2FIB_ENTRY_RESULT_FLAG_STATIC));
}
