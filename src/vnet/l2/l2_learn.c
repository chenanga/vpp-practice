                                          ...
                                          u8                    timestamp)
{
    ...
    *next0 = vnet_l2_feature_next(b0, msm->feat_next_node_index, L2INPUT_FEAT_LEARN);
    ...
    if (PREDICT_TRUE(result0->fields.sw_if_index == sw_if_index0)) {
        ...
        if (PREDICT_TRUE(check == 0)) return;                          /* MAC entry up to date */
        if (l2fib_entry_result_is_set_AGE_NOT(result0)) return;        /* Static MAC always age_not */
        if (msm->global_learn_count > msm->global_learn_limit) return; /* Above learn limit - do not update */
        if (bd_config->learn_count > bd_config->learn_limit) return;   /* Above bridge domain learn limit - do not update */
         ...
         * or SN obsolete */
        if ((*count > 2) && (dtime == 1) && (dsn == 0)) return;
        ...
        *count += 1;
        ...
        if ((msm->global_learn_count >= msm->global_learn_limit) || (bd_config->learn_count >= bd_config->learn_limit)) {
            /*
        ...
        if (msm->client_pid != 0)
            l2fib_entry_result_set_bits(result0, (L2FIB_ENTRY_RESULT_FLAG_LRN_EVT | L2FIB_ENTRY_RESULT_FLAG_LRN_MOV));
        else
            l2fib_entry_result_clear_bits(result0, (L2FIB_ENTRY_RESULT_FLAG_LRN_EVT | L2FIB_ENTRY_RESULT_FLAG_LRN_MOV));
...
static_always_inline uword l2learn_node_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int do_trace)
{
    ...
    while (n_left > 8) {
        ...
        if (do_trace) {
            ...
            if (b[1]->flags & VLIB_BUFFER_IS_TRACED) {
                l2learn_trace_t *t = vlib_add_trace(vm, node, b[1], sizeof(*t));
            ...
            if (b[2]->flags & VLIB_BUFFER_IS_TRACED) {
                l2learn_trace_t *t = vlib_add_trace(vm, node, b[2], sizeof(*t));
            ...
            if (b[3]->flags & VLIB_BUFFER_IS_TRACED) {
                l2learn_trace_t *t = vlib_add_trace(vm, node, b[3], sizeof(*t));
        ...
        /* process 4 pkts */
        vlib_node_increment_counter(vm, l2learn_node.index, L2LEARN_ERROR_L2LEARN, 4);

        l2fib_lookup_4(msm->mac_table,
                       &cached_key,
                       &cached_result,
                       h0->src_address,
                       h1->src_address,
        ...
        l2learn_process(node, msm, &em->counters[node_counter_base_index], b[0], sw_if_index0, &key0, &cached_key, &count, &result0, next, timestamp);

        l2learn_process(node,
                        msm,
    ...
    while (n_left > 0) {
        ...
        /* process 1 pkt */
        vlib_node_increment_counter(vm, l2learn_node.index, L2LEARN_ERROR_L2LEARN, 1);

        l2fib_lookup_1(msm->mac_table, &cached_key, &cached_result, h0->src_address, vnet_buffer(b[0])->l2.bd_index, &key0, &result0);
...
{
    if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE))) return l2learn_node_inline(vm, node, frame, 1 /* do_trace */);
...
clib_error_t *l2learn_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, l2learn_node.index, L2INPUT_N_FEAT, l2input_get_feat_names(), mp->feat_next_node_index);
