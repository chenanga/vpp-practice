...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 6 && n_left_to_next >= 2) {
            ...
            /* Determine next node */
            next0 = vnet_l2_feature_next(b0, msm->l2_out_feat_next, L2OUTPUT_FEAT_EFP_FILTER);
            next1 = vnet_l2_feature_next(b1, msm->l2_out_feat_next, L2OUTPUT_FEAT_EFP_FILTER);

            /* perform the efp filter check on two packets */

            extract_keys(msm->vnet_main, sw_if_index0, b0, &port_sw_if_index0, &first_ethertype0, &outer_id0, &inner_id0, &match_flags0);

            extract_keys(msm->vnet_main, sw_if_index1, b1, &port_sw_if_index1, &first_ethertype1, &outer_id1, &inner_id1, &match_flags1);
            ...
            matched0 = eth_identify_subint(hi0, match_flags0, main_intf0, vlan_intf0, qinq_intf0, &subint_sw_if_index0, &error0, &is_l20);

            matched1 = eth_identify_subint(hi1, match_flags1, main_intf1, vlan_intf1, qinq_intf1, &subint_sw_if_index1, &error1, &is_l21);
            ...
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE))) {
                if (b0->flags & VLIB_BUFFER_IS_TRACED) {
                    ...
                    l2_efp_filter_trace_t *t  = vlib_add_trace(vm, node, b0, sizeof(*t));
                ...
                if (b1->flags & VLIB_BUFFER_IS_TRACED) {
                    ...
                    l2_efp_filter_trace_t *t  = vlib_add_trace(vm, node, b1, sizeof(*t));
...
clib_error_t *l2_efp_filter_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, l2_efp_filter_node.index, L2OUTPUT_N_FEAT, l2output_get_feat_names(), mp->l2_out_feat_next);
