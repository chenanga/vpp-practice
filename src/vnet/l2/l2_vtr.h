...
always_inline u32 l2_vtr_process(vlib_buffer_t *b0, vtr_config_t *config)
{
    ...
    clib_memcpy_fast(eth + 12 - 8, &config->raw_tags, sizeof(config->raw_tags));
    ...
    vnet_buffer(b0)->l2_hdr_offset -= (word) config->push_bytes - (word) config->pop_bytes;

    /* Update vlan tag count */
    ethernet_buffer_adjust_vlan_count_by_bytes(b0, (word) config->push_bytes - (word) config->pop_bytes);
...
always_inline u32 l2_pbb_process(vlib_buffer_t *b0, ptr_config_t *config)
{
    ...
    if (config->pop_bytes > 0) {
        ...
        if (clib_net_to_host_u16(ph->priority_dei_id) != clib_net_to_host_u16(config->macs_tags.priority_dei_id)
            || clib_net_to_host_u32(ph->priority_dei_uca_res_sid) != clib_net_to_host_u32(config->macs_tags.priority_dei_uca_res_sid))
            return 1;
