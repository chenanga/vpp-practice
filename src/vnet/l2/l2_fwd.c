                                        ...
                                        u16                  *next0)
{
    ...
    if (PREDICT_FALSE(try_flood)) {
        ...
        *next0                                = L2FWD_NEXT_L2_OUTPUT;
        ...
        if (!l2fib_entry_result_is_set_AGE_NOT(result0)) {
            ...
            l2fib_seq_num_t expected_sn = l2_fib_update_seq_num(in_sn, l2_input_seq_num(result0->fields.sw_if_index));
        ...
        /* perform BVI check */
            ...
            u32 rc;
            rc = l2_to_bvi(vm, msm->vnet_main, b0, vnet_buffer(b0)->sw_if_index[VLIB_TX], &msm->l3_next, next0);
    ...
    if (PREDICT_FALSE(try_flood)) {
         ...
         */
        if (vnet_buffer(b0)->l2.feature_bitmap & (L2INPUT_FEAT_UU_FLOOD | L2INPUT_FEAT_UU_FWD)) {
            *next0 = vnet_l2_feature_next(b0, msm->feat_next_node_index, L2INPUT_FEAT_FWD);
        }
        else {
...
static_always_inline uword l2fwd_node_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int do_trace)
{
    ...
    while (n_left >= 8) {
        ...
        l2fwd_process(vm, node, msm, em, b[1], sw_if_index1, &result1, next + 1);
        l2fwd_process(vm, node, msm, em, b[2], sw_if_index2, &result2, next + 2);
        l2fwd_process(vm, node, msm, em, b[3], sw_if_index3, &result3, next + 3);
...
{
    if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE))) return l2fwd_node_inline(vm, node, frame, 1 /* do_trace */);
...
clib_error_t *l2fwd_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, l2fwd_node.index, L2INPUT_N_FEAT, l2input_get_feat_names(), mp->feat_next_node_index);
