...
static void vl_api_l2_fib_table_details_t_handler(vl_api_l2_fib_table_details_t *mp)
{
...
static void vl_api_l2_xconnect_details_t_handler(vl_api_l2_xconnect_details_t *mp)
{
    vat_main_t *vam = l2_test_main.vat_main;
    fformat(vam->ofp, "%15d%15d", ntohl(mp->rx_sw_if_index), ntohl(mp->tx_sw_if_index));
}

static int api_l2_xconnect_dump(vat_main_t *vam)
{
...
static int api_want_l2_arp_term_events(vat_main_t *vam)
{
    return -1;
}

static int api_want_l2_macs_events(vat_main_t *vam)
{
...
static int api_l2fib_flush_all(vat_main_t *vam)
{
    return -1;
}

static void increment_mac_address(u8 *mac)
{
    u64 tmp = *((u64 *) mac);
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "mac %U", unformat_ethernet_address, mac))
            ...
            if (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
                if (unformat(i, "%U", unformat_vnet_sw_interface, vnm, &sw_if_index)) sw_if_index_set = 1;
            }
            else
                break;
        }
...
static int api_l2fib_flush_int(vat_main_t *vam)
{
    vnet_main_t              *vnm = vnet_get_main();
    unformat_input_t         *i   = vam->input;
...
static int api_bridge_domain_set_mac_age(vat_main_t *vam)
{
    unformat_input_t                   *i = vam->input;
...
static int api_l2fib_set_scan_delay(vat_main_t *vam)
{
    return -1;
}

static int api_want_l2_macs_events2(vat_main_t *vam)
{
    return -1;
}
...
static void vl_api_l2_flags_reply_t_handler(vl_api_l2_flags_reply_t *mp)
{
    vat_main_t *vam    = l2_test_main.vat_main;
...
static int api_l2fib_flush_bd(vat_main_t *vam)
{
    unformat_input_t        *i = vam->input;
...
static int api_bridge_domain_add_del_v2(vat_main_t *vam)
{
    return -1;
}

static void vl_api_bridge_domain_add_del_v2_reply_t_handler(vl_api_bridge_domain_add_del_v2_reply_t *mp)
{
}
...
static int api_l2_interface_pbb_tag_rewrite(vat_main_t *vam)
{
    vnet_main_t                           *vnm = vnet_get_main();
    unformat_input_t                      *i   = vam->input;
...
#undef _
        ...
        else if (unformat(i, "dmac %U", unformat_ethernet_address, dmac)) dmac_set++;
        else if (unformat(i, "smac %U", unformat_ethernet_address, smac)) smac_set++;
    ...
    if (((vtr_op == L2_VTR_PUSH_2) || (vtr_op == L2_VTR_TRANSLATE_2_2)) && ((dmac_set == 0) || (smac_set == 0) || (sid == ~0))) {
        errmsg(
            "push and translate_qinq operations require dmac, smac, sid and "
            "optionally vlanid");
        return -99;
    }
...
static int api_sw_interface_set_l2_xconnect(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "rx_sw_if_index %d", &rx_sw_if_index))
            ...
            if (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
                if (unformat(i, "%U", unformat_vnet_sw_interface, vnm, &rx_sw_if_index)) rx_sw_if_index_set = 1;
            }
            else
                break;
        }
            ...
            if (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
                if (unformat(i, "%U", unformat_vnet_sw_interface, vnm, &tx_sw_if_index)) tx_sw_if_index_set = 1;
            }
            else
                break;
        }
...
static void vl_api_bd_ip_mac_details_t_handler(vl_api_bd_ip_mac_details_t *mp)
{
    vat_main_t *vam = &vat_main;

    print(vam->ofp, "\n%-5d %U %U", ntohl(mp->entry.bd_id), format_vl_api_mac_address, mp->entry.mac, format_vl_api_address, &mp->entry.ip);
}

static void vl_api_bvi_create_reply_t_handler(vl_api_bvi_create_reply_t *mp)
{
}
...
static int api_sw_interface_set_l2_bridge(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "sw_if_index %d", &rx_sw_if_index))
            ...
            bd_id_set = 1;
        else if (unformat(i, "%U", unformat_vnet_sw_interface, vnm, &rx_sw_if_index))
            rx_sw_if_index_set = 1;
...
static void vl_api_bridge_flags_reply_t_handler(vl_api_bridge_flags_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    i32         retval = ntohl(mp->retval);
    if (vam->async_mode) {
...
static int api_bridge_domain_set_learn_limit(vat_main_t *vam)
{
    return -1;
}
...
static void vl_api_bridge_domain_details_t_handler(vl_api_bridge_domain_details_t *mp)
{
    ...
    print(vam->ofp, "\n%-3s %-3s %-3s %-3s %-3s %-6s %-3s", " ID", "LRN", "FWD", "FLD", "BVI", "UU-FWD", "#IF");

    print(vam->ofp,
          "%3d %3d %3d %3d %3d %6d %3d",
          ntohl(mp->bd_id),
          mp->learn,
          mp->forward,
          mp->flood,
          ntohl(mp->bvi_sw_if_index),
          ntohl(mp->uu_fwd_sw_if_index),
          n_sw_ifs);
    ...
    if (n_sw_ifs) {
        ...
        for (i = 0; i < n_sw_ifs; i++) {
            ...
            print(vam->ofp, "%7d     %3d  %s", sw_if_index, sw_ifs->shg, sw_if_name ? (char *) sw_if_name : "sw_if_index not found!");
...
static int api_bridge_domain_set_default_learn_limit(vat_main_t *vam)
{
    return -1;
}

static int api_bd_ip_mac_flush(vat_main_t *vam)
{
    unformat_input_t         *i = vam->input;
...
static int api_bd_ip_mac_dump(vat_main_t *vam)
{
    unformat_input_t        *i = vam->input;
    ...
    fformat(vam->ofp, "\n%-5s %-7s %-20s %-30s", "bd_id", "is_ipv6", "mac_address", "ip_address");
...
static int api_bvi_create(vat_main_t *vam)
{
    return -1;
}

static int api_bvi_delete(vat_main_t *vam)
{
    return -1;
}
