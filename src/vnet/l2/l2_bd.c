...
{
    if (bd_is_valid(bd_config)) return;
    bd_config->feature_bitmap     = ~(L2INPUT_FEAT_ARP_TERM | L2INPUT_FEAT_UU_FWD | L2INPUT_FEAT_ARP_UFWD);
    ...
    bd_config->mac_by_ip6         = hash_create_mem(0, sizeof(ip6_address_t), sizeof(uword));
}
...
u32 bd_add_bd_index(bd_main_t *bdm, u32 bd_id)
{
    ...
    l2input_main.bd_configs[rv].learn_limit = l2learn_main.bd_default_learn_limit;
    l2input_main.bd_configs[rv].learn_count = 0;

    return rv;
}

static inline void bd_free_ip_mac_tables(l2_bridge_domain_t *bd)
{
    u64            mac_addr;
...
{
    bd_config->flood_count = (vec_len(bd_config->members) - (bd_config->tun_master_count ? bd_config->tun_normal_count : 0));
...
{
    u32                  ix    = 0;
    vnet_sw_interface_t *sw_if = vnet_get_sw_interface(vnet_get_main(), member->sw_if_index);
    ...
    switch (sw_if->flood_class) {
        ...
        case VNET_FLOOD_CLASS_NORMAL:
            ix = (vec_len(bd_config->members) - bd_config->tun_normal_count - bd_config->no_flood_count);
            break;
...
u32 bd_remove_member(l2_bridge_domain_t *bd_config, u32 sw_if_index)
{
    ...
    vec_foreach_index (ix, bd_config->members) {
        ...
        if (m->sw_if_index == sw_if_index) {
            vnet_sw_interface_t *sw_if = vnet_get_sw_interface(vnet_get_main(), sw_if_index);
...
{
    if (bd_index < vec_len(l2input_main.bd_configs)) return (vec_elt_at_index(l2input_main.bd_configs, bd_index));
    return (NULL);
}
...
void bd_set_mac_age(vlib_main_t *vm, u32 bd_index, u8 age)
{
    ...
    vlib_process_signal_event(vm,
                              ...
                              enable ? L2_MAC_AGE_PROCESS_EVENT_START : L2_MAC_AGE_PROCESS_EVENT_STOP,
                              0);
}
...
static clib_error_t *bd_learn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (bd_id == 0) return clib_error_return(0, "No operations on the default bridge domain are supported");

    p = hash_get(bdm->bd_index_by_bd_id, bd_id);

    if (p == 0) return clib_error_return(0, "No such bridge domain %d", bd_id);
...
static clib_error_t *bd_default_learn_limit(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!unformat(input, "%d", &learn_limit)) {
        error = clib_error_return(0, "expecting per bridge-domain max entry number got`%U'", format_unformat_error, input);
        goto done;
    }
...
static clib_error_t *bd_mac_age(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!unformat(input, "%u", &age)) {
        error = clib_error_return(0, "expecting ageing time in minutes but got `%U'", format_unformat_error, input);
        goto done;
    }

    /* set the bridge domain flag */
    if (age > 255) {
        error = clib_error_return(0, "mac aging time cannot be bigger than 255");
        goto done;
    }
...
static clib_error_t *bd_learn_limit(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!unformat(input, "%u", &learn_limit)) {
        error = clib_error_return(0, "expecting maxium number of learned entries but got `%U'", format_unformat_error, input);
        goto done;
    }
...
VLIB_CLI_COMMAND(bd_learn_limit_cli, static) = {
    ...
    .short_help = "set bridge-domain learn-limit <bridge-domain-id> <learn-limit>",
...
u32 bd_add_del_ip_mac(u32 bd_index, ip46_type_t type, const ip46_address_t *ip, const mac_address_t *mac, u8 is_add)
{
    ...
    if (IP46_TYPE_IP6 == type) {
        ...
        if (is_add) {
            if (old_mac && (*old_mac == new_mac)) /* mac entry already exist */
                return 0;
...
static clib_error_t *bd_arp_entry(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (unformat(input, "%U", unformat_ip4_address, &ip_addr.ip4)) {
        ...
        error = clib_error_return(0, "expecting IP address but got `%U'", format_unformat_error, input);
        goto done;
    }

    if (!unformat(input, "%U", unformat_mac_address_t, &mac)) {
        error = clib_error_return(0, "expecting MAC address but got `%U'", format_unformat_error, input);
        goto done;
    }
    ...
    if (bd_add_del_ip_mac(bd_index, type, &ip_addr, &mac, is_add)) {
        error = clib_error_return(0,
                                  "MAC %s for IP %U and MAC %U failed",
                                  is_add ? "add" : "del",
                                  format_ip46_address,
                                  &ip_addr,
                                  IP46_TYPE_ANY,
...
static u8 *format_uu_cfg(u8 *s, va_list *args)
{
    ...
    if (bd_config->feature_bitmap & L2INPUT_FEAT_UU_FWD)
        return (format(s, "%U", format_vnet_sw_if_index_name_with_NA, vnet_get_main(), bd_config->uu_fwd_sw_if_index));
...
static clib_error_t *bd_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (unformat(input, "%d", &bd_id)) {
        ...
        if (unformat(input, "bd-tag")) bd_tag = 1;
    ...
    for (bd_index = start; bd_index < end; bd_index++) {
        ...
        if (bd_is_valid(bd_config)) {
            ...
            if (detail) vlib_cli_output(vm, "%U", format_l2_input_feature_bitmap, bd_config->feature_bitmap);
            vec_reset_length(as);
            ...
            if (detail || intf) {
                ...
                vec_foreach_index (i, bd_config->members) {
                    l2_flood_member_t *member       = vec_elt_at_index(bd_config->members, i);
            ...
            if ((detail || arp) && (bd_config->feature_bitmap & L2INPUT_FEAT_ARP_TERM)) {
                u32            ip4_addr;
                ...
                hash_foreach(ip4_addr, mac_addr, bd_config->mac_by_ip4, ({
                                 vlib_cli_output(vm, "%=40U => %=20U", format_ip4_address, &ip4_addr, format_ethernet_address, &mac_addr);
                             }));

                hash_foreach_mem(ip6_addr, mac_addr, bd_config->mac_by_ip6, ({
                                     vlib_cli_output(vm, "%=40U => %=20U", format_ip6_address, ip6_addr, format_ethernet_address, &mac_addr);
                                 }));
            }
...
int bd_add_del(l2_bridge_domain_add_del_args_t *a)
{
    ...
    if (a->is_add) {
        if (bd_index != ~0) return VNET_API_ERROR_BD_ALREADY_EXISTS;
        if (a->bd_id > L2_BD_ID_MAX) return VNET_API_ERROR_BD_ID_EXCEED_MAX;
        ...
        if (enable_flags) bd_set_flags(vm, bd_index, enable_flags, 1 /* enable */);

        if (disable_flags) bd_set_flags(vm, bd_index, disable_flags, 0 /* disable */);

        bd_set_mac_age(vm, bd_index, a->mac_age);

        if (a->bd_tag) bd_set_bd_tag(vm, bd_index, a->bd_tag);
        ...
        if (bd_index == ~0) return VNET_API_ERROR_NO_SUCH_ENTRY;
        if (bd_index == 0) return VNET_API_ERROR_BD_NOT_MODIFIABLE;
        if (vec_len(l2input_main.bd_configs[bd_index].members)) return VNET_API_ERROR_BD_IN_USE;
...
static clib_error_t *bd_add_del_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    u8                              is_add = 1;
    u32                             bd_id  = ~0;
    u32                             flood = 1, forward = 1, learn = 1, uu_flood = 1, arp_term = 0, arp_ufwd = 0;
    u32                             mac_age = 0;
    u8                             *bd_tag  = NULL;
    ...
    switch (rv) {
        case 0:
            if (is_add) vlib_cli_output(vm, "bridge-domain %d", bd_id);
            break;
...
u32 bd_get_unused_id(void)
{
    ...
    for (j = 0; j < 1 << 10; j++) {
        ...
        for (i = 0; i < 1 << 10; i++) {
            ...
            seed &= L2_BD_ID_MAX;
            if (seed == 0) continue;
            if (bd_find_index(bdm, seed) == ~0) return seed;
        }
    }
