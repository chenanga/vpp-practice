...
static u8 *format_arp_term_input_trace(u8 *s, va_list *va)
{
    ...
    s = format(s, "%U", t->packet_data[0] == 0 ? format_ethernet_arp_header : format_ip6_header, t->packet_data, sizeof(t->packet_data));
...
static int l2_arp_term_publish(l2_arp_term_publish_event_t *ctx)
{
    ...
    vlib_process_signal_event(vlib_get_main(), l2_arp_term_process_node.index, L2_ARP_TERM_EVENT_PUBLISH, 0);

    return 0;
}

static inline void l2_arp_term_publish_v4_dp(u32 sw_if_index, const ethernet_arp_ip4_over_ethernet_address_t *a)
{
    l2_arp_term_main_t *l2am = &l2_arp_term_main;

    if (!l2am->publish) return;
    ...
    vl_api_rpc_call_main_thread(l2_arp_term_publish, (u8 *) &args, sizeof(args));
}

static inline void l2_arp_term_publish_v6_dp(u32 sw_if_index, const ip6_address_t *addr, const mac_address_t *mac)
{
    l2_arp_term_main_t *l2am = &l2_arp_term_main;
...
static inline int vnet_ip6_nd_term(vlib_main_t         *vm,
                                   ...
                                   u16                  bd_index)
{
    ...
    if (ndh->icmp.type != ICMP6_neighbor_solicitation && ndh->icmp.type != ICMP6_neighbor_advertisement) return 0;

    if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (p0->flags & VLIB_BUFFER_IS_TRACED))) {
        u8 *t0 = vlib_add_trace(vm, node, p0, sizeof(icmp6_input_trace_t));
    ...
    if (ndh->icmp.type == ICMP6_neighbor_solicitation) {
        ...
        if (macp) { /* found ip-mac entry, generate eighbor advertisement response */
            int                  bogus_length;
            ...
            ndh->advertisement_flags = clib_host_to_net_u32(ICMP6_NEIGHBOR_ADVERTISEMENT_FLAG_SOLICITED | ICMP6_NEIGHBOR_ADVERTISEMENT_FLAG_OVERRIDE);
            ndh->icmp.checksum       = 0;
            ndh->icmp.checksum       = ip6_tcp_udp_icmp_compute_checksum(vm, p0, ip, &bogus_length);
            clib_memcpy(eth->dst_address, eth->src_address, 6);
            clib_memcpy(eth->src_address, macp, 6);
            vlib_error_count(vm, error_node->node_index, ICMP6_ERROR_NEIGHBOR_ADVERTISEMENTS_TX, 1);
            return 1;
        }
    }
...
static uword arp_term_l2bd(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (vnet_buffer(p0)->l2.shg != 0) goto next_l2_feature;
            ...
            if (p0->flags & VNET_BUFFER_F_LOCALLY_ORIGINATED) goto next_l2_feature;

            if (ethertype0 != ETHERNET_TYPE_ARP) goto check_ip6_nd;
            ...
            error0 = 0;
            error0 = (arp0->l2_type != clib_net_to_host_u16(ETHERNET_ARP_HARDWARE_TYPE_ethernet) ? ETHERNET_ARP_ERROR_l2_type_not_ethernet : error0);
            error0 = (arp0->l3_type != clib_net_to_host_u16(ETHERNET_TYPE_IP4) ? ETHERNET_ARP_ERROR_l3_type_not_ip4 : error0);
            ...
            if (PREDICT_FALSE(!ethernet_mac_address_equal(eth0->src_address, arp0->ip4_over_ethernet[0].mac.bytes))
                || ethernet_address_cast(arp0->ip4_over_ethernet[0].mac.bytes)) {
                /* VRRP virtual MAC may be different to SMAC in ARP reply */
                if (clib_memcmp(arp0->ip4_over_ethernet[0].mac.bytes, vrrp_prefix, sizeof(vrrp_prefix)) != 0) {
                    error0 = ETHERNET_ARP_ERROR_l2_address_mismatch;
                    goto drop;
                }
            }
            if (PREDICT_FALSE(ip4_address_is_multicast(&arp0->ip4_over_ethernet[0].ip4))) {
                error0 = ETHERNET_ARP_ERROR_l3_src_address_not_local;
                goto drop;
            }

            /* Check if anyone want ARP request events for L2 BDs */
            l2_arp_term_publish_v4_dp(sw_if_index0, &arp0->ip4_over_ethernet[0]);

            /* lookup BD mac_by_ip4 hash table for MAC entry */
            ip0       = arp0->ip4_over_ethernet[1].ip4.as_u32;
            bd_index0 = vnet_buffer(p0)->l2.bd_index;
            if (PREDICT_FALSE((bd_index0 != last_bd_index) || (last_bd_index == (u16) ~0))) {
            ...
            if (PREDICT_FALSE(!macp0)) goto next_l2_feature; /* MAC not found */
            if (PREDICT_FALSE(arp0->ip4_over_ethernet[0].ip4.as_u32 == arp0->ip4_over_ethernet[1].ip4.as_u32)) goto next_l2_feature; /* GARP */
        ...
        check_ip6_nd:
            ...
            if (PREDICT_FALSE(ethertype0 == ETHERNET_TYPE_IP6 && iph0->protocol == IP_PROTOCOL_ICMP6
                              && !ip6_address_is_unspecified(&iph0->src_address))) {
                sw_if_index0 = vnet_buffer(p0)->sw_if_index[VLIB_RX];
                if (vnet_ip6_nd_term(vm, node, p0, eth0, iph0, sw_if_index0, vnet_buffer(p0)->l2.bd_index)) goto output_response;
            }

        next_l2_feature : {
            next0 = vnet_l2_feature_next(p0, arp_term_next_node_index, L2INPUT_FEAT_ARP_TERM);
            vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, pi0, next0);
            continue;
        }

        drop:
            if (0 == arp0->ip4_over_ethernet[0].ip4.as_u32 || (arp0->ip4_over_ethernet[0].ip4.as_u32 == arp0->ip4_over_ethernet[1].ip4.as_u32)) {
    ...
    vlib_error_count(vm, node->node_index, ETHERNET_ARP_ERROR_replies_sent, n_replies_sent);
...
clib_error_t *arp_term_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, arp_term_l2bd_node.index, L2INPUT_N_FEAT, l2input_get_feat_names(), arp_term_next_node_index);
    return 0;
}
