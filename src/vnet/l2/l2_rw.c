...
static u8 *format_l2_rw_entry(u8 *s, va_list *args)
{
    ...
    s                 = format(s,
               ...
               format_hex_bytes,
               e->mask,
               e->rewrite_n_vectors * sizeof(u32x4),
               format_hex_bytes,
               e->value,
               e->rewrite_n_vectors * sizeof(u32x4));
    s                 = format(s, "      hits:%d skip_bytes:%d", e->hit_count, e->skip_n_vectors * sizeof(u32x4));
    return s;
}
...
static u8 *format_l2_rw_config(u8 *s, va_list *args)
{
    ...
    return format(s, "table-index:%d miss-index:%d", c->table_index, c->miss_index);
}
...
static u8 *format_l2_rw_trace(u8 *s, va_list *args)
{
    ...
    return format(s, "l2-rw: sw_if_index %d, table %d, entry %d", t->sw_if_index, t->classify_table_index, t->rewrite_entry_index);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 6 && n_left_to_next >= 2) {
            ...
            next0 = vnet_l2_feature_next(b0, rw->feat_next_node_index, L2INPUT_FEAT_RW);
            next1 = vnet_l2_feature_next(b1, rw->feat_next_node_index, L2INPUT_FEAT_RW);
...
static clib_error_t *l2_rw_entry_cli_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!mask || !value) return clib_error_return(0, "Unspecified mask or value");

    if (vec_len(mask) != vec_len(value)) return clib_error_return(0, "Mask and value lengths must be identical");

    int ret;
    if ((ret = l2_rw_mod_entry(&index, mask, value, vec_len(mask), skip, del))) return clib_error_return(0, "Could not add entry");
...
VLIB_CLI_COMMAND(l2_rw_entry_cli, static) = {
    ...
    .short_help = "l2 rewrite entry [index <index>] [mask <hex-mask>] [value <hex-value>] [skip <n_bytes>] [del]",
...
int l2_rw_interface_set_table(u32 sw_if_index, u32 table_index, u32 miss_index)
{
    ...
    if (c->table_index == ~0) clib_bitmap_set(rw->configs_bitmap, sw_if_index, 0);
...
static clib_error_t *l2_rw_interface_cli_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (sw_if_index == ~0) return clib_error_return(0, "You must specify an interface 'iface <interface>'", format_unformat_error, input);
    int ret;
...
VLIB_CLI_COMMAND(l2_rw_interface_cli, static) = {
    ...
    .short_help = "set interface l2 rewrite <interface> [table <table index>] [miss-index <entry-index>]",
...
{
    l2_rw_main_t *rw = &l2_rw_main;
    if (clib_bitmap_count_set_bits(rw->configs_bitmap) == 0) vlib_cli_output(vm, "No interface is currently using l2 rewrite\n");
...
static clib_error_t *l2_rw_show_entries_cli_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    l2_rw_main_t  *rw = &l2_rw_main;
    l2_rw_entry_t *e;
    if (pool_elts(rw->entries) == 0) vlib_cli_output(vm, "No entries\n");
...
static clib_error_t *l2_rw_set_cli_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (unformat_check_input(input) == UNFORMAT_END_OF_INPUT || !unformat(input, "%d", &bridge_domain)) {
        return clib_error_return(0, "You must specify a bridge domain");
    }

    if (unformat_check_input(input) != UNFORMAT_END_OF_INPUT && unformat(input, "disable")) {
        disable = 1;
    }

    if (l2_rw_enable_disable(bridge_domain, disable)) return clib_error_return(0, "Could not enable or disable rewrite");
...
static clib_error_t *l2_rw_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, l2_rw_node.index, L2INPUT_N_FEAT, l2input_get_feat_names(), rw->feat_next_node_index);
    return 0;
}
