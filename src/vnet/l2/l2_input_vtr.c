...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 6 && n_left_to_next >= 2) {
            ...
            next0 = vnet_l2_feature_next(b0, msm->feat_next_node_index, L2INPUT_FEAT_VTR);
            next1 = vnet_l2_feature_next(b1, msm->feat_next_node_index, L2INPUT_FEAT_VTR);
            ...
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE))) {
                ...
                if (b1->flags & VLIB_BUFFER_IS_TRACED) {
                    l2_invtr_trace_t  *t  = vlib_add_trace(vm, node, b1, sizeof(*t));
...
clib_error_t *l2_invtr_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, l2_invtr_node.index, L2INPUT_N_FEAT, l2input_get_feat_names(), mp->feat_next_node_index);
