...
static_always_inline u32 l2_to_bvi_dmac_check(vnet_hw_interface_t *hi, u8 *dmac, ethernet_interface_t *ei, u8 have_sec_dmac)
{
    ...
    if (ethernet_mac_address_equal(dmac, hi->hw_address)) return TO_BVI_ERR_OK;
...
l2_to_bvi(vlib_main_t *vlib_main, vnet_main_t *vnet_main, vlib_buffer_t *b0, u32 bvi_sw_if_index, next_by_ethertype_t *l3_next, u16 *next0)
{
    ...
    if (!ethernet_address_cast(e0->dst_address)) {
        vnet_hw_interface_t  *hi = vnet_get_sup_hw_interface(vnet_main, bvi_sw_if_index);
        ethernet_interface_t *ei = ethernet_get_interface(em, hi->hw_if_index);
        u32                   rv;

        if (PREDICT_FALSE(ei && (vec_len(ei->secondary_addrs) > 0)))
            rv = l2_to_bvi_dmac_check(hi, e0->dst_address, ei, 1 /* have_sec_dmac */);
        else
            rv = l2_to_bvi_dmac_check(hi, e0->dst_address, ei, 0 /* have_sec_dmac */);

        if (rv != TO_BVI_ERR_OK) return rv;
    }
...
extern int l2_bvi_create(u32 instance, const mac_address_t *mac, u32 *sw_if_index);
