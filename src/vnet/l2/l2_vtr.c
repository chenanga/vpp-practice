                    ...
                    u16          vlan_outer_tag)
{
    ...
    if (vtr_op == L2_VTR_POP_2) {
        ...
        clib_memcpy_fast(in_config->macs_tags.b_dst_address, b_dmac, sizeof(in_config->macs_tags.b_dst_address));
        clib_memcpy_fast(in_config->macs_tags.b_src_address, b_smac, sizeof(in_config->macs_tags.b_src_address));
    ...
    if (config) config->out_vtr_flag = (u8) enable;
              ...
              u32               *i_sid)
{
    ...
    *i_sid     = 0;
    ...
    if (in_config->push_and_pop_bytes == 0) {
        ...
        clib_memcpy_fast(&eth_hdr->dst_address, in_config->macs_tags.b_dst_address, sizeof(eth_hdr->dst_address));
        clib_memcpy_fast(&eth_hdr->src_address, in_config->macs_tags.b_src_address, sizeof(eth_hdr->src_address));
 ...
 * The CLI format is:
 *    set interface l2 pbb-tag-rewrite <interface> [disable | pop | push | translate_pbb_stag <outer_tag> dmac <address> smac <address> s_id <nn>
 * [b_vlanid <nn>]]
 */
