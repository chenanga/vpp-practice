...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            bd_config    = vec_elt_at_index(l2input_main.bd_configs, vnet_buffer(b0)->l2.bd_index);
            ...
            if (0 == n_clones) {
                ...
                vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, bi0, L2FLOOD_NEXT_DROP);
                continue;
            }
            else if (n_clones > 1) {
...
clib_error_t *l2flood_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, l2flood_node.index, L2INPUT_N_FEAT, l2input_get_feat_names(), mp->feat_next_node_index);
