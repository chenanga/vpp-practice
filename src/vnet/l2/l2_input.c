...
u8 *format_l2_input_features(u8 *s, va_list *args)
{
    ...
    if (l2_input_is_bridge(l2_input)) fb &= l2_input->bd_feature_bitmap;
...
u8 *format_l2_input(u8 *s, va_list *args)
{
    ...
    if (l2_input_is_bridge(l2_input)) {
        ...
        s = format(s,
                   ...
                   bd_id,
                   bd_find_index(bdm, bd_id),
                   l2_input->shg,
                   l2_input_is_bvi(l2_input) ? "bvi" : " ");
    }
    else if (l2_input_is_xconnect(l2_input))
        s = format(s, "  L2 xconnect %U", format_vnet_sw_if_index_name, vnet_get_main(), l2_input->output_sw_if_index);
...
clib_error_t *l2input_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, l2input_node.index, L2INPUT_N_FEAT, l2input_get_feat_names(), mp->feat_next_node_index);
...
void l2input_interface_mac_change(u32 sw_if_index, const u8 *old_address, const u8 *new_address)
{
    ...
    if (l2_input_is_bridge(intf_config) && l2_input_is_bvi(intf_config)) {
        ...
        l2fib_add_entry(new_address, intf_config->bd_index, sw_if_index, L2FIB_ENTRY_RESULT_FLAG_BVI | L2FIB_ENTRY_RESULT_FLAG_STATIC);
    }
}
                    ...
                    u32               xc_sw_if_index)            /* peer interface for xconnect       */
{
    ...
    if (l2fib_main.mac_table_initialized == 0) l2fib_table_init();
    ...
    if (l2_input_is_bridge(config)) {
        ...
        if ((config->feature_bitmap & L2INPUT_FEAT_LEARN) || (bd_config->feature_bitmap & L2INPUT_FEAT_LEARN)) l2fib_flush_int_mac(vm, sw_if_index);
    ...
    vec_validate_init_empty(l2om->output_node_index_vec, sw_if_index, L2OUTPUT_NEXT_DROP);
    ...
    if (mode == MODE_L3) {
        ...
        if (mode == MODE_L2_BRIDGE) {
             ...
             *
             */
            if (!hi) return MODE_ERROR_ETH; /* non-ethernet */
             ...
             */
            config->feature_bitmap |= (L2INPUT_FEAT_FWD | L2INPUT_FEAT_UU_FLOOD | L2INPUT_FEAT_UU_FWD | L2INPUT_FEAT_FLOOD | L2INPUT_FEAT_LEARN
                                       | L2INPUT_FEAT_ARP_UFWD | L2INPUT_FEAT_ARP_TERM);
            ...
            if (L2_BD_PORT_TYPE_BVI == port_type) {
                ...
                l2fib_add_entry(hi->hw_address, bd_index, sw_if_index, L2FIB_ENTRY_RESULT_FLAG_BVI | L2FIB_ENTRY_RESULT_FLAG_STATIC);
            ...
            if (L2_BD_PORT_TYPE_NORMAL == port_type || L2_BD_PORT_TYPE_BVI == port_type) {
            ...
            /* Make sure last-chance drop is configured */
            config->feature_bitmap |= L2INPUT_FEAT_DROP | L2INPUT_FEAT_INPUT_CLASSIFY;
    ...
    if (hi->hw_class_index == ethernet_hw_interface_class.index) {
        if ((hi->l2_if_count == 1) && (l2_if_adjust == 1)) {
            ...
            ethernet_set_flags(vnet_main, hi->hw_if_index, ETHERNET_INTERFACE_FLAG_ACCEPT_ALL);
        }
...
static clib_error_t *l2_input_interface_add_del(vnet_main_t *vnm, u32 sw_if_index, u32 is_add)
{
    if (!is_add) {
        ...
        if (sw_if_index < vec_len(l2input_main.configs)) {
            ...
            if (l2_input_is_xconnect(config)) set_int_l2_mode(vm, vnm, MODE_L3, config->output_sw_if_index, 0, L2_BD_PORT_TYPE_NORMAL, 0, 0);
            if (l2_input_is_xconnect(config) || l2_input_is_bridge(config))
                set_int_l2_mode(vm, vnm, MODE_L3, sw_if_index, 0, L2_BD_PORT_TYPE_NORMAL, 0, 0);
        }
    }
...
static clib_error_t *int_l2_bridge(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (bd_id > L2_BD_ID_MAX) {
        error = clib_error_return(0, "bridge domain ID exceed 16M limit", format_unformat_error, input);
        goto done;
    }
    ...
    if (unformat(input, "bvi")) port_type = L2_BD_PORT_TYPE_BVI;
    ...
    if ((rc = set_int_l2_mode(vm, vnm, MODE_L2_BRIDGE, sw_if_index, bd_index, port_type, shg, 0))) {
        if (rc == MODE_ERROR_ETH) {
            error = clib_error_return(0, "bridged interface must be ethernet", format_unformat_error, input);
        }
        else if (rc == MODE_ERROR_BVI_DEF) {
            error = clib_error_return(0, "bridge-domain already has a bvi interface", format_unformat_error, input);
        }
        else {
            error = clib_error_return(0, "invalid configuration for interface", format_unformat_error, input);
        }
        goto done;
    }
...
static clib_error_t *int_l2_xc(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!unformat_user(input, unformat_vnet_sw_interface, vnm, &xc_sw_if_index)) {
        error = clib_error_return(0, "unknown peer interface `%U'", format_unformat_error, input);
        goto done;
    }

    /* set the interface mode */
    if (set_int_l2_mode(vm, vnm, MODE_L2_XC, sw_if_index, 0, L2_BD_PORT_TYPE_NORMAL, 0, xc_sw_if_index)) {
        error = clib_error_return(0, "invalid configuration for interface", format_unformat_error, input);
        goto done;
    }
...
static clib_error_t *int_l3(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    /* set the interface mode */
    if (set_int_l2_mode(vm, vnm, MODE_L3, sw_if_index, 0, L2_BD_PORT_TYPE_NORMAL, 0, 0)) {
        error = clib_error_return(0, "invalid configuration for interface", format_unformat_error, input);
        goto done;
    }
...
static clib_error_t *show_int_mode(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vec_foreach (si, sis) {
        ...
        if (l2_input_is_bridge(config)) {
            ...
            args = format(0, "bd_id %d%s shg %d", bd_id, l2_input_is_bvi(config) ? " bvi" : "", config->shg);
        }
        else if (l2_input_is_xconnect(config)) {
            mode = "l2 xconnect";
            args = format(0, "%U", format_vnet_sw_if_index_name, vnm, config->output_sw_if_index);
        }
        else {
            mode = "l3";
            args = format(0, " ");
        }
        vlib_cli_output(vm, "%s %U %v\n", mode, format_vnet_sw_if_index_name, vnm, si->sw_if_index, args);
        vec_free(args);
    }
