...
#define get_u16(addr) (*((u16 *) (addr)))
        ...
        feat_mask &= ~(L2INPUT_FEAT_FWD | L2INPUT_FEAT_UU_FLOOD | L2INPUT_FEAT_UU_FWD);

        if (ethertype != ETHERNET_TYPE_ARP) feat_mask &= ~(L2INPUT_FEAT_ARP_UFWD);
         ...
         */
            ...
            if (ethertype == ETHERNET_TYPE_ARP) {
                ...
                if (arp0->opcode == clib_host_to_net_u16(ETHERNET_ARP_OPCODE_request)) vnet_buffer(b0)->l2.shg = 0;
            }
            else /* must be ICMPv6 */
            {
                ...
                if (ndh0->icmp.type == ICMP6_neighbor_solicitation) vnet_buffer(b0)->l2.shg = 0;
            }
        }
    }
    else {
         ...
         */
        if (PREDICT_FALSE(vnet_buffer(b0)->sw_if_index[VLIB_TX] == L2INPUT_BVI)) vnet_buffer(b0)->l2.shg = 0;
    }
    ...
    if (l2_input_is_bridge(config)) {
        ...
        vnet_buffer(b0)->l2.l2fib_sn = l2_fib_mk_seq_num(config->bd_seq_num, config->seq_num);
    ...
    *next0 = feat_bitmap_get_next_node_index(msm->feat_next_node_index, feature_bitmap);
}
        ...
        while (n_left >= 8) {
            ...
            if (do_trace) {
                ...
                if (b[0]->flags & VLIB_BUFFER_IS_TRACED) {
                    ...
                    l2input_trace_t   *t  = vlib_add_trace(vm, node, b[0], sizeof(*t));
                    t->sw_if_index        = sw_if_index0;
                    t->feat_mask          = vnet_buffer(b[0])->l2.feature_bitmap;
                    clib_memcpy_fast(t->dst_and_src, h0->dst_address, sizeof(h0->dst_address) + sizeof(h0->src_address));
                }
                if (b[1]->flags & VLIB_BUFFER_IS_TRACED) {
                    ethernet_header_t *h1 = vlib_buffer_get_current(b[1]);
                    l2input_trace_t   *t  = vlib_add_trace(vm, node, b[1], sizeof(*t));
                    t->sw_if_index        = sw_if_index1;
                    t->feat_mask          = vnet_buffer(b[1])->l2.feature_bitmap;
                    clib_memcpy_fast(t->dst_and_src, h1->dst_address, sizeof(h1->dst_address) + sizeof(h1->src_address));
                }
                if (b[2]->flags & VLIB_BUFFER_IS_TRACED) {
                    ethernet_header_t *h2 = vlib_buffer_get_current(b[2]);
                    l2input_trace_t   *t  = vlib_add_trace(vm, node, b[2], sizeof(*t));
                    t->sw_if_index        = sw_if_index2;
                    t->feat_mask          = vnet_buffer(b[2])->l2.feature_bitmap;
                    clib_memcpy_fast(t->dst_and_src, h2->dst_address, sizeof(h2->dst_address) + sizeof(h2->src_address));
                }
                if (b[3]->flags & VLIB_BUFFER_IS_TRACED) {
                    ethernet_header_t *h3 = vlib_buffer_get_current(b[3]);
                    l2input_trace_t   *t  = vlib_add_trace(vm, node, b[3], sizeof(*t));
                    t->sw_if_index        = sw_if_index3;
                    t->feat_mask          = vnet_buffer(b[3])->l2.feature_bitmap;
                    clib_memcpy_fast(t->dst_and_src, h3->dst_address, sizeof(h3->dst_address) + sizeof(h3->src_address));
                }
            }
...
{
    if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE))) return l2input_node_inline(vm, node, frame, 1 /* do_trace */);
