...
{
    if (shg != vnet_buffer(b)->l2.shg) return;
                                                        ...
                                                        int                  update_feature_bitmap)
{
    while (n_left >= 8) {
        ...
        if (l2_vtr) {
            int i;
            for (i = 0; i < 4; i++) {
                u32 failed1 = l2_efp && l2_efp_filter_process(b[i], &(config->input_vtr));
                u32 failed2 = l2_vtr_process(b[i], &(config->output_vtr));
                if (PREDICT_FALSE(failed1 | failed2)) {
                    next[i] = L2OUTPUT_NEXT_DROP;
                    if (failed2) b[i]->error = node->errors[L2OUTPUT_ERROR_VTR_DROP];
                    if (failed1) b[i]->error = node->errors[L2OUTPUT_ERROR_EFP_DROP];
                }
            }
        }
    ...
    while (n_left) {
        if (update_feature_bitmap) vnet_buffer(b[0])->l2.feature_bitmap = config->feature_bitmap;

        if (l2_vtr) {
            u32 failed1 = l2_efp && l2_efp_filter_process(b[0], &(config->input_vtr));
            ...
            if (PREDICT_FALSE(failed1 | failed2)) {
                *next = L2OUTPUT_NEXT_DROP;
                ...
                if (failed1) b[0]->error = node->errors[L2OUTPUT_ERROR_EFP_DROP];
            }
        }
        ...
        if (shg_set) split_horizon_violation(node, config->shg, b[0], next);
                                                 ...
                                                 int                  l2_pbb)
{
    ...
    if (config->shg == 0 && feature_bitmap == 0) {
        if ((l2_efp | l2_vtr | l2_pbb) == 0) return;
        l2output_process_batch_inline(vm, node, config, b, cdo, next, n_left, l2_efp, l2_vtr, l2_pbb, 0, 0);
    }
    else if (config->shg == 0)
        l2output_process_batch_inline(vm, node, config, b, cdo, next, n_left, l2_efp, l2_vtr, l2_pbb, 0, 1);
    else if (feature_bitmap == 0)
        l2output_process_batch_inline(vm, node, config, b, cdo, next, n_left, l2_efp, l2_vtr, l2_pbb, 1, 0);
    else
        l2output_process_batch_inline(vm, node, config, b, cdo, next, n_left, l2_efp, l2_vtr, l2_pbb, 1, 1);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left) {
        ...
        if (PREDICT_FALSE((feature_bitmap & ~L2OUTPUT_FEAT_OUTPUT) != 0))
            new_next = feat_bitmap_get_next_node_index(l2output_main.l2_out_feat_next, feature_bitmap);
        else
            new_next = vec_elt(l2output_main.output_node_index_vec, sw_if_index[0]);
        clib_memset_u16(nexts + off, new_next, count);

        if (new_next == L2OUTPUT_NEXT_DROP) {
            l2output_set_buffer_error(b, count, node->errors[L2OUTPUT_ERROR_MAPPING_DROP]);
            continue;
        }
        ...
        else if (config->out_vtr_flag && config->output_pbb_vtr.push_and_pop_bytes)
            l2output_process_batch(vm,
                                   node,
                                   config,
                                   b,
    ...
    if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE))) {
        ...
        while (n_left) {
            if (PREDICT_FALSE(b[0]->flags & VLIB_BUFFER_IS_TRACED)) {
                ethernet_header_t *h;
                l2output_trace_t  *t = vlib_add_trace(vm, node, b[0], sizeof(*t));
    ...
    vlib_node_increment_counter(vm, l2output_node.index, L2OUTPUT_ERROR_L2OUTPUT, frame->n_vectors);
...
static clib_error_t *l2output_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, l2output_node.index, L2OUTPUT_N_FEAT, l2output_get_feat_names(), mp->l2_out_feat_next);
...
{
    vnet_hw_interface_t *hw0 = vnet_get_sup_hw_interface(vnet_main, sw_if_index);

    /* dynamically create graph node arc  */
    u32 next                                         = vlib_node_add_next(vlib_main, l2output_node.index, hw0->output_node_index);
