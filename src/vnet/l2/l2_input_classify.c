...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    l2_input_classify_runtime_t *rt         = (l2_input_classify_runtime_t *) node->runtime_data;
    u32                          hits       = 0;
    u32                          misses     = 0;
    u32                          chain_hits = 0;
    f64                          now;
    ...
    while (n_left_from >= 4) {
        ...
        type_index0 = (type0 == ETHERNET_TYPE_IP4) ? L2_INPUT_CLASSIFY_TABLE_IP4 : L2_INPUT_CLASSIFY_TABLE_OTHER;
        type_index0 = (type0 == ETHERNET_TYPE_IP6) ? L2_INPUT_CLASSIFY_TABLE_IP6 : type_index0;

        type_index1 = (type1 == ETHERNET_TYPE_IP4) ? L2_INPUT_CLASSIFY_TABLE_IP4 : L2_INPUT_CLASSIFY_TABLE_OTHER;
        type_index1 = (type1 == ETHERNET_TYPE_IP6) ? L2_INPUT_CLASSIFY_TABLE_IP6 : type_index1;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(next0 == 0)) b0->error = node->errors[L2_INPUT_CLASSIFY_ERROR_DROP];

            /* Determine the next node and remove ourself from bitmap */
            if (PREDICT_TRUE(next0 == ~0))
                next0 = vnet_l2_feature_next(b0, cm->l2_inp_feat_next, L2INPUT_FEAT_INPUT_CLASSIFY);
            else
                vnet_buffer(b0)->l2.feature_bitmap &= ~L2INPUT_FEAT_INPUT_CLASSIFY;

            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                l2_input_classify_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
    ...
    vlib_node_increment_counter(vm, node->node_index, L2_INPUT_CLASSIFY_ERROR_MISS, misses);
    vlib_node_increment_counter(vm, node->node_index, L2_INPUT_CLASSIFY_ERROR_HIT, hits);
    vlib_node_increment_counter(vm, node->node_index, L2_INPUT_CLASSIFY_ERROR_CHAIN_HIT, chain_hits);
...
clib_error_t *l2_input_classify_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, l2_input_classify_node.index, L2INPUT_N_FEAT, l2input_get_feat_names(), cm->l2_inp_feat_next);
    rt->l2cm = cm;
...
{
    l2input_intf_bitmap_enable(sw_if_index, L2INPUT_FEAT_INPUT_CLASSIFY, (u32) enable_disable);
}
...
int vnet_l2_input_classify_set_tables(u32 sw_if_index, u32 ip4_table_index, u32 ip6_table_index, u32 other_table_index)
{
    ...
    vec_validate(cm->classify_table_index_by_sw_if_index[L2_INPUT_CLASSIFY_TABLE_IP4], sw_if_index);

    vec_validate(cm->classify_table_index_by_sw_if_index[L2_INPUT_CLASSIFY_TABLE_IP6], sw_if_index);

    vec_validate(cm->classify_table_index_by_sw_if_index[L2_INPUT_CLASSIFY_TABLE_OTHER], sw_if_index);

    cm->classify_table_index_by_sw_if_index[L2_INPUT_CLASSIFY_TABLE_IP4][sw_if_index] = ip4_table_index;

    cm->classify_table_index_by_sw_if_index[L2_INPUT_CLASSIFY_TABLE_IP6][sw_if_index] = ip6_table_index;

    cm->classify_table_index_by_sw_if_index[L2_INPUT_CLASSIFY_TABLE_OTHER][sw_if_index] = other_table_index;
...
static clib_error_t *int_l2_input_classify_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    switch (rv) {
        ...
        default:
            return clib_error_return(0, "vnet_l2_input_classify_set_tables: %d", rv);
            break;
    }
