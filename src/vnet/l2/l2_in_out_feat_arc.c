...
static u8 *format_l2_in_out_feat_arc_trace(u8 *s, u32 is_output, va_list *args)
{
    ...
    s = format(s,
               ...
               is_output ? "OUT-FEAT-ARC" : "IN-FEAT-ARC",
               t->arc_head,
               t->feature_bitmap,
               t->ethertype,
               t->sw_if_index,
...
{
    return format_l2_in_out_feat_arc_trace(s, IN_OUT_FEAT_ARC_INPUT_TABLE_GROUP, args);
}

static u8 *format_l2_out_feat_arc_trace(u8 *s, va_list *args)
{
    return format_l2_in_out_feat_arc_trace(s, IN_OUT_FEAT_ARC_OUTPUT_TABLE_GROUP, args);
}
                                                  ...
                                                  u16            *out_next)
{
    ...
    for (ii = 0; ii < vector_sz; ii++) {
        ...
        if (PREDICT_TRUE(vnet_have_features(feature_arc, sw_if_index[ii])))
            vnet_feature_arc_start(feature_arc, sw_if_index[ii], &next_index, b[ii]);
        else
            next_index = vnet_l2_feature_next(b[ii], next_nodes, is_output ? L2OUTPUT_FEAT_OUTPUT_FEAT_ARC : L2INPUT_FEAT_INPUT_FEAT_ARC);
...
static_always_inline void set_next_in_arc_tail_xN(int vector_sz, int is_output, u32 *next_nodes, vlib_buffer_t **b, u16 *out_next)
{
    int ii;
    for (ii = 0; ii < vector_sz; ii++) {
        out_next[ii] = vnet_l2_feature_next(b[ii], next_nodes, is_output ? L2OUTPUT_FEAT_OUTPUT_FEAT_ARC : L2INPUT_FEAT_INPUT_FEAT_ARC);
    }
}
                                         ...
                                         u16                 *next)
{
    ...
    for (ii = 0; ii < vector_sz; ii++)
        if (PREDICT_FALSE(b[ii]->flags & VLIB_BUFFER_IS_TRACED)) {
            l2_in_out_feat_arc_trace_t *t = vlib_add_trace(vm, node, b[ii], sizeof(*t));
            t->arc_head                   = arc_head;
...
always_inline uword l2_in_out_feat_arc_node_fn(vlib_main_t              *vm,
                                               vlib_node_runtime_t      *node,
                                               vlib_frame_t             *frame,
                                               int                       is_output,
                                               ...
                                               int                       do_trace)
{
    ...
    CLIB_PREFETCH(next_node_indices, sizeof(fam->feat_next_node_index[is_output]), LOAD);
    ...
    while (n_left > 3 * L2_FEAT_ARC_VEC_SIZE) {
        ...
        if (arc_head) {
            ...
            set_next_in_arc_head_xN(vec_sz,
                                    is_output,
                                    next_node_indices,
                                    b,
                                    sw_if_index,
            ...
            set_next_in_arc_tail_xN(vec_sz, is_output, next_node_indices, b, next);
        }
        if (do_trace) maybe_trace_xN(vec_sz, arc_head, vm, node, b, sw_if_index, ethertype, next);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE))
        return l2_in_out_feat_arc_node_fn(vm, node, frame, IN_OUT_FEAT_ARC_INPUT_TABLE_GROUP, &l2_in_feat_arc_node, 1, 1);
    else
        return l2_in_out_feat_arc_node_fn(vm, node, frame, IN_OUT_FEAT_ARC_INPUT_TABLE_GROUP, &l2_in_feat_arc_node, 1, 0);
}
...
{
    if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE))
        return l2_in_out_feat_arc_node_fn(vm, node, frame, IN_OUT_FEAT_ARC_OUTPUT_TABLE_GROUP, &l2_out_feat_arc_node, 1, 1);
    else
        return l2_in_out_feat_arc_node_fn(vm, node, frame, IN_OUT_FEAT_ARC_OUTPUT_TABLE_GROUP, &l2_out_feat_arc_node, 1, 0);
}
...
{
    if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE))
        return l2_in_out_feat_arc_node_fn(vm, node, frame, IN_OUT_FEAT_ARC_INPUT_TABLE_GROUP, &l2_in_feat_arc_end_node, 0, 1);
    else
        return l2_in_out_feat_arc_node_fn(vm, node, frame, IN_OUT_FEAT_ARC_INPUT_TABLE_GROUP, &l2_in_feat_arc_end_node, 0, 0);
}
...
{
    if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE))
        return l2_in_out_feat_arc_node_fn(vm, node, frame, IN_OUT_FEAT_ARC_OUTPUT_TABLE_GROUP, &l2_out_feat_arc_end_node, 0, 1);
    else
        return l2_in_out_feat_arc_node_fn(vm, node, frame, IN_OUT_FEAT_ARC_OUTPUT_TABLE_GROUP, &l2_out_feat_arc_end_node, 0, 0);
}
...
{
    if (is_output)
        l2output_intf_bitmap_enable(sw_if_index, L2OUTPUT_FEAT_OUTPUT_FEAT_ARC, (u32) enable_disable);
    else
        l2input_intf_bitmap_enable(sw_if_index, L2INPUT_FEAT_INPUT_FEAT_ARC, (u32) enable_disable);
}
#endif /* CLIB_MARCH_VARIANT */
...
clib_error_t *l2_in_out_feat_arc_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm,
                                l2_out_feat_arc_end_node.index,
                                L2OUTPUT_N_FEAT,
...
static int l2_has_features(u32 sw_if_index, int is_output)
{
    int                        has_features = 0;
    l2_in_out_feat_arc_main_t *mp           = &l2_in_out_feat_arc_main;
    has_features += vnet_have_features(mp->ip4_feat_arc_index[is_output], sw_if_index);
    has_features += vnet_have_features(mp->ip6_feat_arc_index[is_output], sw_if_index);
    has_features += vnet_have_features(mp->nonip_feat_arc_index[is_output], sw_if_index);
...
static int l2_is_output_arc(u8 arc_index)
{
    ...
    return (mp->ip4_feat_arc_index[idx] == arc_index || mp->ip6_feat_arc_index[idx] == arc_index || mp->nonip_feat_arc_index[idx] == arc_index);
}
                                   ...
                                   u32         n_feature_config_bytes)
{
    ...
    if (arc_index == (u8) ~0) return VNET_API_ERROR_INVALID_VALUE;

    /* check the state before we tried to enable/disable */
    int had_features = vnet_have_features(arc_index, sw_if_index);

    int ret = vnet_feature_enable_disable(arc_name, node_name, sw_if_index, enable_disable, feature_config, n_feature_config_bytes);
    if (ret) return ret;
    ...
    if (had_features != has_features) {
        if (l2_is_output_arc(arc_index)) {
            vnet_l2_in_out_feat_arc_enable_disable(sw_if_index, 1, l2_has_features(sw_if_index, 1));
        }
        if (l2_is_input_arc(arc_index)) {
            vnet_l2_in_out_feat_arc_enable_disable(sw_if_index, 0, l2_has_features(sw_if_index, 0));
        }
    }
    return 0;
}
