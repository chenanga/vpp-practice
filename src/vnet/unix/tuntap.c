...
static uword tuntap_tx(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    for (i = 0; i < n_packets; i++) {
        ...
        if (tm->threads[thread_index].iovecs) vec_set_len(tm->threads[thread_index].iovecs, 0);
        ...
        if (writev(tm->dev_net_tun_fd, tm->threads[thread_index].iovecs, vec_len(tm->threads[thread_index].iovecs)) < l) clib_unix_warning("writev");

        n_bytes += l;
    }

    /* Update tuntap interface output stats. */
    vlib_increment_combined_counter(im->combined_sw_if_counters + VNET_INTERFACE_COUNTER_TX, vm->thread_index, tm->sw_if_index, n_packets, n_bytes);
...
static uword tuntap_rx(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
       ...
       Turn them into iovecs to pass to readv. */
    {
        ...
        n_bytes_left      = readv(tm->dev_net_tun_fd, tm->threads[thread_index].iovecs, tm->mtu_buffers);
        n_bytes_in_packet = n_bytes_left;
        if (n_bytes_left <= 0) {
            if (errno != EAGAIN) clib_unix_warning("readv %d", n_bytes_left);
            return 0;
        }
...

    {
        ...
        if (tm->have_normal_interface) {
            ...
            if (!(si->flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP)) next_index = VNET_DEVICE_INPUT_NEXT_DROP;
        }
...
static clib_error_t *tuntap_exit(vlib_main_t *vm)
{
    ...
    /* Not present. */
    if (!tm->dev_net_tun_fd || tm->dev_net_tun_fd < 0) return 0;

    sfd = socket(AF_INET, SOCK_STREAM, 0);
    if (sfd < 0) clib_unix_warning("provisioning socket");

    clib_memset(&ifr, 0, sizeof(ifr));
    strncpy(ifr.ifr_name, tm->tun_name, sizeof(ifr.ifr_name) - 1);

    /* get flags, modify to bring down interface... */
    if (ioctl(sfd, SIOCGIFFLAGS, &ifr) < 0) clib_unix_warning("SIOCGIFFLAGS");

    ifr.ifr_flags &= ~(IFF_UP | IFF_RUNNING);

    if (ioctl(sfd, SIOCSIFFLAGS, &ifr) < 0) clib_unix_warning("SIOCSIFFLAGS");

    /* Turn off persistence */
    if (ioctl(tm->dev_net_tun_fd, TUNSETPERSIST, 0) < 0) clib_unix_warning("TUNSETPERSIST");
    close(tm->dev_tap_fd);
    if (tm->dev_net_tun_fd >= 0) close(tm->dev_net_tun_fd);
    if (sfd >= 0) close(sfd);
...
static clib_error_t *tuntap_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "mtu %d", &tm->mtu_bytes))
            ...
            is_ether = 1;
        else if (unformat(input, "have-normal-interface") || unformat(input, "have-normal"))
    ...
    if (is_ether) flags = IFF_TAP | IFF_NO_PI;
    ...
    if (have_normal_interface) {
        ...
        tm->hw_if_index = vnet_register_interface(vnm, tuntap_dev_class.index, 0 /* device instance */, tuntap_interface_class.index, 0);
    ...
    if (error) {
        if (tm->dev_net_tun_fd >= 0) close(tm->dev_net_tun_fd);
        if (tm->dev_tap_fd >= 0) close(tm->dev_tap_fd);
    }
                                          ...
                                          u32            is_delete)
{
    ...
    if (tm->have_normal_interface || tm->dev_tap_fd < 0) return;
    ...
    if (fib_table_get_index_for_sw_if_index(FIB_PROTOCOL_IP4, sw_if_index) != fib_table_get_index_for_sw_if_index(FIB_PROTOCOL_IP4, tm->sw_if_index))
        return;
    ...
    clib_memset(&ifr, 0, sizeof(ifr));
    snprintf(ifr.ifr_name, sizeof(ifr.ifr_name), "%s:%d", tm->tun_name, (int) (ap - tm->subifs));
    ...
    if (!is_delete) {
        ...
        if (ioctl(tm->dev_tap_fd, SIOCSIFADDR, &ifr) < 0) clib_unix_warning("ioctl SIOCSIFADDR");

        sin->sin_addr.s_addr = im->fib_masks[address_length];
        if (ioctl(tm->dev_tap_fd, SIOCSIFNETMASK, &ifr) < 0) clib_unix_warning("ioctl SIOCSIFNETMASK");
    }
    else {
    ...
    /* get flags, modify to bring up interface... */
    if (ioctl(tm->dev_tap_fd, SIOCGIFFLAGS, &ifr) < 0) clib_unix_warning("ioctl SIOCGIFFLAGS");
    ...
    if (ioctl(tm->dev_tap_fd, SIOCSIFFLAGS, &ifr) < 0) clib_unix_warning("ioctl SIOCSIFFLAGS");
}
                                          ...
                                          u32            is_delete)
{
     ...
     * If we don't do this overlapping address spaces in the different tables
     * breaks the linux host's routing tables */
    if (fib_table_get_index_for_sw_if_index(FIB_PROTOCOL_IP6, sw_if_index) != fib_table_get_index_for_sw_if_index(FIB_PROTOCOL_IP6, tm->sw_if_index))
        return;
    ...
    if (!is_delete) {
        ...
        if (sockfd < 0) clib_unix_warning("get ifindex socket");

        if (ioctl(sockfd, SIOGIFINDEX, &ifr) < 0) clib_unix_warning("get ifindex");

        ifr6.ifr6_ifindex   = ifr.ifr_ifindex;
        ifr6.ifr6_prefixlen = address_length;
        clib_memcpy_fast(&ifr6.ifr6_addr, address, 16);

        if (ioctl(sockfd, SIOCSIFADDR, &ifr6) < 0) clib_unix_warning("set address");
        ...
        if (ioctl(sockfd, SIOCDIFADDR, &ifr6) < 0) clib_unix_warning("del address");
...
static uword tuntap_intfc_tx(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    if (tm->have_normal_interface) return tuntap_tx(vm, node, frame);
...
static clib_error_t *tuntap_init(vlib_main_t *vm)
{
    ...
    vec_validate_aligned(tm->threads, m->n_vlib_mains - 1, CLIB_CACHE_LINE_BYTES);
