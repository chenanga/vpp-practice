...
u8 RT(rule_is_exact_match)(RTT(mma_rule) * key, RTT(mma_rule) * r)
{
    ...
    for (i = 0; i < ARRAY_LEN(key->match.as_u64); i++) {
        if (key->match.as_u64[i] != r->match.as_u64[i]) return 0;
    }
    for (i = 0; i < ARRAY_LEN(key->mask.as_u64); i++) {
        if (key->mask.as_u64[i] != r->mask.as_u64[i]) return 0;
    }
    return 1;
}
...
u8 RT(rule_is_match_for_key)(RTT(mma_mask_or_match) * key, RTT(mma_rule) * r)
{
    ...
    *tkp = *key;
    ...
    for (i = 0; i < ARRAY_LEN(tkp->as_u64); i++) {
        if (tkp->as_u64[i] != r->match.as_u64[i]) return 0;
    }
    return 1;
}
...
RTT(mma_rule) * RT(mma_rule_free)(RTT(mma_rules_table) * srt, RTT(mma_rule) * rule)
{
...
RTT(mma_rule) * RT(mma_rules_table_get_rule)(RTT(mma_rules_table) * srt, u32 srt_index)
{
    if (!pool_is_free_index(srt->rules, srt_index)) return (srt->rules + srt_index);
    return 0;
}
...
u32 RT(mma_rules_table_lookup)(RTT(mma_rules_table) * srt, RTT(mma_mask_or_match) * key, u32 rule_index)
{
    ...
    if (!RT(rule_is_match_for_key)(key, rp)) return MMA_TABLE_INVALID_INDEX;
    for (i = 0; i < vec_len(rp->next_indices); i++) {
        rv = RT(mma_rules_table_lookup)(srt, key, rp->next_indices[i]);
        if (rv != MMA_TABLE_INVALID_INDEX) return (rv);
    }
...
int RT(mma_rules_table_add_rule)(RTT(mma_rules_table) * srt, RTT(mma_rule) * rule)
{
    ...
    parent_index = RT(mma_rules_table_lookup_rule)(srt, &rule->match, srt->root_index);
    ...
    if (!added) vec_add1(next_indices, rule_index);
...
int RT(mma_rules_table_del_rule)(RTT(mma_rules_table) * srt, RTT(mma_rule) * rule, u32 rule_index)
{
    ...
    if (!RT(rule_is_match_for_key)(&rule->match, rp)) return MMA_TABLE_INVALID_INDEX;
    if (RT(rule_is_exact_match)(rule, rp)) {
        if (rule_index == srt->root_index) rp->action_index = MMA_TABLE_INVALID_INDEX;
        return 1;
    }
    for (i = 0; i < vec_len(rp->next_indices); i++) {
        ...
        if (rv == 1) {
            ...
            if (vec_len(child->next_indices)) vec_append(next_indices, child->next_indices);
            left_to_add = vec_len(rp->next_indices) - i - 1;
            if (left_to_add) {
                vec_add2(next_indices, new_elts, left_to_add);
                clib_memcpy_fast(new_elts, &rp->next_indices[i + 1], left_to_add * sizeof(u32));
            }
