...
static inline int mq_try_lock(svm_msg_q_t *mq)
{
    int rv, n_try = 0;
...
always_inline u8 mq_event_ring_index(session_evt_type_t et)
{
    return (et >= SESSION_CTRL_EVT_RPC ? SESSION_MQ_CTRL_EVT_RING : SESSION_MQ_IO_EVT_RING);
}
...
void app_worker_del_all_events(app_worker_t *app_wrk)
{
    ...
    for (thread_index = 0; thread_index < vec_len(app_wrk->wrk_evts); thread_index++) {
        while (clib_fifo_elts(app_wrk->wrk_evts[thread_index])) {
            ...
            switch (evt->event_type) {
                ...
                case SESSION_CTRL_EVT_CLEANUP:
                    ...
                    if (evt->as_u64[0] >> 32 != SESSION_CLEANUP_SESSION) break;
...
always_inline int app_worker_flush_events_inline(app_worker_t *app_wrk, u32 thread_index, u8 is_builtin)
{
    ...
    for (i = 0; i < n_evts; i++) {
        ...
        switch (evt->event_type) {
            case SESSION_IO_EVT_RX:
                ...
                if (PREDICT_FALSE(s->session_state == SESSION_STATE_ACCEPTING || s->session_state == SESSION_STATE_CONNECTING)) break;
            ...
            case SESSION_CTRL_EVT_BOUND:
                ...
                if (is_builtin) break;
                app->cb_fns.session_listened_callback(app_wrk->wrk_index, evt->as_u64[1] >> 32, evt->session_handle, evt->as_u64[1] & 0xffffffff);
                break;
            case SESSION_CTRL_EVT_ACCEPTED:
                ...
                if (is_builtin) {
                    if (old_state >= SESSION_STATE_TRANSPORT_CLOSING) {
                        ...
                        if (!(s->flags & SESSION_F_APP_CLOSED)) app->cb_fns.session_disconnect_callback(s);
                    }
                }
                break;
            case SESSION_CTRL_EVT_CONNECTED:
                if (!(evt->as_u64[1] & 0xffffffff)) {
                    ...
                    s = 0;
                rv = app->cb_fns.session_connected_callback(app_wrk->wrk_index, evt->as_u64[1] >> 32, s, evt->as_u64[1] & 0xffffffff);
                if (!s) break;
                if (rv) {
                    session_detach_app(s);
                    break;
            ...
            case SESSION_CTRL_EVT_RESET:
                s = session_get(evt->session_index, thread_index);
                if (!(s->flags & SESSION_F_APP_CLOSED)) app->cb_fns.session_reset_callback(s);
                break;
            case SESSION_CTRL_EVT_UNLISTEN_REPLY:
                if (is_builtin) break;
                app->cb_fns.session_unlistened_callback(app_wrk->wrk_index, evt->session_handle, evt->as_u64[1] >> 32, evt->as_u64[1] & 0xffffffff);
                break;
            case SESSION_CTRL_EVT_MIGRATED:
                s = session_get(evt->session_index, thread_index);
            ...
            case SESSION_CTRL_EVT_TRANSPORT_CLOSED:
                ...
                if (PREDICT_FALSE(s->app_wrk_index == APP_INVALID_INDEX)) break;
                if (app->cb_fns.session_transport_closed_callback) app->cb_fns.session_transport_closed_callback(s);
                break;
            case SESSION_CTRL_EVT_CLEANUP:
                s = session_get(evt->as_u64[0] & 0xffffffff, thread_index);
                /* Notification enqueued before session was refused by app */
                if (PREDICT_TRUE(s->app_wrk_index != APP_INVALID_INDEX)) {
                    if (app->cb_fns.session_cleanup_callback) app->cb_fns.session_cleanup_callback(s, evt->as_u64[0] >> 32);
                }
                if (evt->as_u64[0] >> 32 != SESSION_CLEANUP_SESSION) break;
                uword_to_pointer(evt->as_u64[1], void (*)(session_t * s))(s);
                break;
            case SESSION_CTRL_EVT_HALF_CLEANUP:
                ...
                if (app->cb_fns.half_open_cleanup_callback) app->cb_fns.half_open_cleanup_callback(s);
                pool_put_index(app_wrk->half_open_table, s->ho_index);
                session_free(s);
                break;
            case SESSION_CTRL_EVT_APP_ADD_SEGMENT:
                app->cb_fns.add_segment_callback(app_wrk->wrk_index, evt->as_u64[1]);
                break;
            case SESSION_CTRL_EVT_APP_DEL_SEGMENT:
                app->cb_fns.del_segment_callback(app_wrk->wrk_index, evt->as_u64[1]);
                break;
            default:
    ...
    if (!is_builtin) {
        svm_msg_q_unlock(mq);
        if (mq_is_cong && i == n_evts) app_worker_unset_wrk_mq_congested(app_wrk, thread_index);
    }
...
int app_wrk_flush_wrk_events(app_worker_t *app_wrk, u32 thread_index)
{
    if (app_worker_application_is_builtin(app_wrk))
        return app_worker_flush_events_inline(app_wrk, thread_index, 1 /* is_builtin */);
    else
        return app_worker_flush_events_inline(app_wrk, thread_index, 0 /* is_builtin */);
}

static inline int session_wrk_flush_events(session_worker_t *wrk)
{
    ...
    while (app_wrk_index != ~0) {
        ...
        if (!clib_fifo_elts(app_wrk->wrk_evts[thread_index])) clib_bitmap_set(wrk->app_wrks_pending_ntf, app_wrk->wrk_index, 0);

        app_wrk_index = clib_bitmap_next_set(wrk->app_wrks_pending_ntf, app_wrk_index + 1);
    }

    if (!clib_bitmap_is_zero(wrk->app_wrks_pending_ntf)) vlib_node_set_interrupt_pending(wrk->vm, session_input_node.index);
