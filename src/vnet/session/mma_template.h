...
typedef int (*RTT(rule_cmp_fn))(RTT(mma_rule) * rule1, RTT(mma_rule) * rule2);
typedef struct {
    /** Root for rules tree */
    u32 root_index;
...
RTT(mma_rule) * RT(session_rule_free)(RTT(mma_rules_table) * srt, RTT(mma_rule) * rule);
RTT(mma_rule) * RT(mma_table_get_rule)(RTT(mma_rules_table) * srt, u32 srt_index);
