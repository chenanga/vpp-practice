...
VLIB_REGISTER_LOG_CLASS(session_api_log, static) = {.class_name = "session", .subclass_name = "api"};

#define log_debug(fmt, ...) vlib_log_debug(session_api_log.class, "%s: " fmt, __func__, __VA_ARGS__)
...
static int verify_message_len(void *mp, u64 expected_len, char *where)
{
    ...
    if (supplied_len < expected_len) {
        log_err("%s: Supplied message length %d is less than expected %d", where, supplied_len, expected_len);
        return 0;
    }
    else {
        return 1;
...
static void vl_api_session_sdl_add_del_t_handler(vl_api_session_sdl_add_del_t *mp)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (rv) {
            ...
            for (int j = i - 1; j >= 0; j--) {
                ...
                if (rv2)
                    log_err(
                        "rollback session_sdl add del returned on %U "
                        "@index %d: %U",
                        format_ip46_address,
                        &table_args->lcl.fp_addr,
                        IP46_TYPE_ANY,
...
{
    clib_error_t *error;
    if (vl_api_registration_file_index(reg) == VL_API_INVALID_FI) return SESSION_E_BAPI_NO_FD;
...
static int mq_send_session_accepted_cb(session_t *s)
{
    ...
    if (session_has_transport(s)) {
        ...
        if (application_is_proxy(app)) {
            listener = app_worker_first_listener(app_wrk, session_get_fib_proto(s), session_get_transport_proto(s));
            if (listener) m.listener_handle = listen_session_get_handle(listener);
        }
    ...
    if (application_original_dst_is_enabled(app)) {
        session_get_original_dst(&m.lcl, &m.rmt, session_get_transport_proto(s), &m.original_dst_ip4, &m.original_dst_port);
    }
...
static inline void mq_send_session_close_evt(app_worker_t *app_wrk, session_handle_t sh, session_evt_type_t evt_type)
{
...
static inline void mq_notify_close_subscribers(u32 app_index, session_handle_t sh, svm_fifo_t *f, session_evt_type_t evt_type)
{
    ...
    for (i = 0; i < f->shr->n_subscribers; i++) {
        if (!(app_wrk = application_get_worker(app, f->shr->subscribers[i]))) continue;
...
static void mq_send_session_disconnected_cb(session_t *s)
{
    ...
    if (svm_fifo_n_subscribers(s->rx_fifo)) mq_notify_close_subscribers(app_wrk->app_index, sh, s->rx_fifo, SESSION_CTRL_EVT_DISCONNECTED);
}
...
static void mq_send_session_reset_cb(session_t *s)
{
    ...
    if (svm_fifo_n_subscribers(s->rx_fifo)) mq_notify_close_subscribers(app_wrk->app_index, sh, s->rx_fifo, SESSION_CTRL_EVT_RESET);
}
...
static void mq_send_unlisten_cb(u32 app_wrk_index, session_handle_t sh, u32 context, int rv)
{
...
static int mq_send_add_segment_cb(u32 app_wrk_index, u64 segment_handle)
{
    ...
    app_wrk_send_ctrl_evt_fd(app_wrk, SESSION_CTRL_EVT_APP_ADD_SEGMENT, &m, sizeof(m), sp->fd);
...
static int mq_send_del_segment_cb(u32 app_wrk_index, u64 segment_handle)
{
    ...
    app_wrk_send_ctrl_evt(app_wrk, SESSION_CTRL_EVT_APP_DEL_SEGMENT, &m, sizeof(m));
...
static void mq_send_session_cleanup_cb(session_t *s, session_cleanup_ntf_t ntf)
{
    ...
    if (ntf == SESSION_CLEANUP_TRANSPORT && s->session_state != SESSION_STATE_TRANSPORT_DELETED) return;
...
static int mq_send_io_rx_event(session_t *s)
{
...
static int mq_send_io_tx_event(session_t *s)
{
    app_worker_t    *app_wrk = app_worker_get(s->app_wrk_index);
...
static void vl_api_session_enable_disable_t_handler(vl_api_session_enable_disable_t *mp)
{
    ...
    if (vnet_session_enable_disable(vm, &args)) rv = VNET_API_ERROR_INVALID_ARGUMENT;
    REPLY_MACRO(VL_API_SESSION_ENABLE_DISABLE_REPLY);
}

static void vl_api_session_enable_disable_v2_t_handler(vl_api_session_enable_disable_v2_t *mp)
{
    ...
    STATIC_ASSERT((session_rt_engine_type_t) RT_BACKEND_ENGINE_API_DISABLE == RT_BACKEND_ENGINE_DISABLE, "API value mismatch");
    STATIC_ASSERT((session_rt_engine_type_t) RT_BACKEND_ENGINE_API_NONE == RT_BACKEND_ENGINE_NONE, "API value mismatch");
    STATIC_ASSERT((session_rt_engine_type_t) RT_BACKEND_ENGINE_API_RULE_TABLE == RT_BACKEND_ENGINE_RULE_TABLE, "API value mismatch");
    STATIC_ASSERT((session_rt_engine_type_t) RT_BACKEND_ENGINE_API_SDL == RT_BACKEND_ENGINE_SDL, "API value mismatch");
    ...
    if (vnet_session_enable_disable(vm, &args)) rv = VNET_API_ERROR_INVALID_VALUE;

    REPLY_MACRO(VL_API_SESSION_ENABLE_DISABLE_V2_REPLY);
}

static void vl_api_session_sapi_enable_disable_t_handler(vl_api_session_sapi_enable_disable_t *mp)
{
...
static void vl_api_app_attach_t_handler(vl_api_app_attach_t *mp)
{
    ...
    STATIC_ASSERT(sizeof(u64) * APP_OPTIONS_N_OPTIONS <= sizeof(mp->options), "Out of options, fix api message definition");
    ...
    REPLY_MACRO3(VL_API_APP_ATTACH_REPLY, ((!rv) ? vec_len(((fifo_segment_t *) a->segment)->ssvm.name) : 0), ({
                     if (!rv) {
    ...
    if (n_fds) session_send_fds(reg, fds, n_fds);
    vec_free(fds);
}
...
static void vl_api_app_worker_add_del_t_handler(vl_api_app_worker_add_del_t *mp)
{
    ...
    REPLY_MACRO3(VL_API_APP_WORKER_ADD_DEL_REPLY, ((!rv && mp->is_add) ? vec_len(args.segment->name) : 0), ({
...
static void vl_api_app_namespace_add_del_t_handler(vl_api_app_namespace_add_del_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_APP_NAMESPACE_ADD_DEL_REPLY, ({
                     if (!rv) rmp->appns_index = clib_host_to_net_u32(appns_index);
                 }));
}
...
static void vl_api_app_namespace_add_del_v4_t_handler(vl_api_app_namespace_add_del_v4_t *mp)
{
...
static void vl_api_session_rule_add_del_t_handler(vl_api_session_rule_add_del_t *mp)
{
    ...
    args.appns_index             = clib_net_to_host_u32(mp->appns_index);
    args.scope                   = mp->scope;
    args.transport_proto         = api_session_transport_proto_decode(&mp->transport_proto) == TRANSPORT_PROTO_UDP ? 1 : 0;
...
static void
send_session_rule_details4(mma_rule_16_t *rule, u8 is_local, u8 transport_proto, u32 appns_index, u8 *tag, vl_api_registration_t *reg, u32 context)
{
...
static void
send_session_rule_details6(mma_rule_40_t *rule, u8 is_local, u8 transport_proto, u32 appns_index, u8 *tag, vl_api_registration_t *reg, u32 context)
{
    vl_api_session_rules_details_t *rmp   = 0;
        ...
        ({
            if (st->srtg_handle != SESSION_SRTG_HANDLE_INVALID)
                for (tp = 0; tp < TRANSPORT_N_PROTOS; tp++) {
                    ...
                    send_session_rules_table_details(srt, st->active_fib_proto, tp, st->is_local, st->appns_index, reg, mp->context);
                }
        }));
}
...
static void send_session_sdl_details(u32 fei, ip46_address_t *lcl_ip, u16 fp_len, u32 action_index, u32 fp_proto, u8 *tag, void *args)
{
...
static void vl_api_session_sdl_dump_t_handler(vl_api_session_rules_dump_t *mp)
{
    vl_api_registration_t     *reg;
    session_table_t           *st;
...
static void vl_api_app_add_cert_key_pair_t_handler(vl_api_app_add_cert_key_pair_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_APP_ADD_CERT_KEY_PAIR_REPLY, ({
                     if (!rv) rmp->index = clib_host_to_net_u32(a->index);
                 }));
}
...
static void sapi_add_del_worker_handler(app_namespace_t *app_ns, clib_socket_t *cs, app_sapi_worker_add_del_msg_t *mp)
{
    ...
    if (!mp->is_add && appns_sapi_enabled()) return;
 ...
 */
static clib_error_t *sapi_socket_receive_wait(clib_socket_t *cs, u8 *msg, u32 msg_len)
{
    clib_error_t *err;
    int           n_tries = 5;
...
send_reply:
    ...
    if (!rv && mp->is_add) rmp->index = a->index;
...
int appns_sapi_add_ns_socket(app_namespace_t *app_ns)
{
    ...
    if (!app_ns->sock_name) app_ns->sock_name = format(0, "%s%v%c", dir, app_ns->ns_id, 0);
    ...
    cs->flags  = CLIB_SOCKET_F_IS_SERVER | CLIB_SOCKET_F_ALLOW_GROUP_WRITE | CLIB_SOCKET_F_SEQPACKET | CLIB_SOCKET_F_PASSCRED;
    ...
    if (clib_socket_prefix_get_type(cs->config) == CLIB_SOCKET_TYPE_UNIX && stat((char *) app_ns->sock_name, &file_stat) == -1) return -1;
