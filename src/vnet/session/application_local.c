...
static inline ct_worker_t *ct_worker_get(u32 thread_index)
{
...
static ct_connection_t *ct_connection_get(u32 ct_index, u32 thread_index)
{
    ...
    if (pool_is_free_index(wrk->connections, ct_index)) return 0;
...
static ct_connection_t *ct_half_open_get(u32 ho_index)
{
    ct_main_t *cm = &ct_main;
...
static void ct_set_invalid_app_wrk(ct_connection_t *ct, u8 is_client)
{
    ...
    if (is_client) {
        ...
        if (peer_ct) ct->client_wrk = APP_INVALID_INDEX;
    }
    else {
...
static inline u64 ct_client_seg_handle(u64 server_sh, u32 client_wrk_index)
{
...
static void ct_session_dealloc_fifos(ct_connection_t *ct, svm_fifo_t *rx_fifo, svm_fifo_t *tx_fifo)
{
    ...
    if (ct->flags & CT_CONN_F_CLIENT) {
        cnt = __atomic_sub_fetch(&ct_seg->client_n_sessions, 1, __ATOMIC_RELAXED);
    }
    else {
        cnt = __atomic_sub_fetch(&ct_seg->server_n_sessions, 1, __ATOMIC_RELAXED);
    }
    ...
    if (ct->flags & CT_CONN_F_CLIENT) {
        ...
        if (s->app_wrk_index == APP_INVALID_INDEX) ct_set_invalid_app_wrk(ct, 1 /* is_client */);
    }
    else {
        ...
        s = session_get(ct->c_s_index, ct->c_thread_index);
        if (s->app_wrk_index == APP_INVALID_INDEX) ct_set_invalid_app_wrk(ct, 0 /* is_client */);
    }

    if (!(ct_seg->flags & CT_SEGMENT_F_CLIENT_DETACHED) || !(ct_seg->flags & CT_SEGMENT_F_SERVER_DETACHED)) goto done;
    ...
    if (app_wrk) {
        ...
        if (!segment_manager_app_detached(csm)) app_worker_del_segment_notify(app_wrk, ct_client_seg_handle(ct->segment_handle, ct->client_wrk));
    }
    ...
    if (segment_manager_app_detached(sm) && !segment_manager_has_fifos(sm)) segment_manager_free_safe(sm);
...
static void ct_session_force_disconnect_server(ct_connection_t *sct)
{
    sct->peer_index = ~0;
...
int ct_session_connect_notify(session_t *ss, session_error_t err)
{
    ...
    if (PREDICT_FALSE(err)) goto connect_error;
...
cleanup_client:
    ...
    if (cct->client_rx_fifo) ct_session_dealloc_fifos(cct, cct->client_rx_fifo, cct->client_tx_fifo);
    ct_connection_free(cct);
    return -1;
}

static inline ct_segment_t *ct_lookup_free_segment(ct_main_t *cm, segment_manager_t *sm, u32 seg_ctx_index)
{
    ...
    pool_foreach (ct_seg, seg_ctx->segments) {
        ...
        if (free_bytes > max_free_bytes && fifo_segment_num_fifos(fs) / 2 < max_fifos) {
...
static ct_segment_t *ct_alloc_segment(ct_main_t *cm, app_worker_t *server_wrk, u64 table_handle, segment_manager_t *sm, u32 client_wrk_index)
{
    ...
    fs_index = segment_manager_add_segment2(sm, seg_size, FIFO_SEGMENT_F_CUSTOM_USE);
    if (fs_index < 0) return 0;
    ...
    if (app_worker_add_segment_notify(server_wrk, seg_handle)) goto error;
...
static int ct_init_accepted_session(app_worker_t *server_wrk, ct_connection_t *ct, session_t *ls, session_t *ll)
{
    ...
    if (spp) {
        ...
        if (ct_seg) {
            ...
            ct_seg->flags &= ~(CT_SEGMENT_F_SERVER_DETACHED | CT_SEGMENT_F_CLIENT_DETACHED);
    ...
    if (fs_index == ~0) {
        ...
        ct_seg = ct_alloc_segment(cm, server_wrk, table_handle, sm, ct->client_wrk);
        if (!ct_seg) {
            clib_rwlock_writer_unlock(&cm->app_segs_lock);
            return -1;
        }
    ...
    rv = segment_manager_try_alloc_fifos(fs, ls->thread_index, props->rx_fifo_size, props->tx_fifo_size, &ls->rx_fifo, &ls->tx_fifo);
    if (rv) {
        segment_manager_segment_reader_unlock(sm);
...
static void ct_accept_one(u32 thread_index, u32 ho_index)
{
    ...
    ll                   = listen_session_get(ll_index);
    ss->session_type     = session_type_from_proto_and_ip(TRANSPORT_PROTO_NONE, sct->c_is_ip4);
    ...
    cct->segment_handle = ct_client_seg_handle(sct->segment_handle, cct->client_wrk);
...
static void ct_accept_rpc_wrk_handler(void *rpc_args)
{
    ...
    if (need_rpc) session_send_rpc_evt_to_thread_force(thread_index, ct_accept_rpc_wrk_handler, uword_to_pointer(thread_index, void *));
}

static void ct_fwrk_flush_connects(void *rpc_args)
{
    ...
    for (thread_index = fwrk_index; thread_index < n_workers; thread_index++) {
        if (!vec_len(cm->fwrk_pending_connects[thread_index])) continue;

        wrk = ct_worker_get(thread_index);

        /* Connects can be done without worker barrier, grab dst worker lock */
        if (thread_index != fwrk_index) clib_spinlock_lock(&wrk->pending_connects_lock);

        clib_fifo_add(wrk->pending_connects, cm->fwrk_pending_connects[thread_index], vec_len(cm->fwrk_pending_connects[thread_index]));
        if (!wrk->have_connects) {
            wrk->have_connects = 1;
            need_rpc           = 1;
        }

        if (thread_index != fwrk_index) clib_spinlock_unlock(&wrk->pending_connects_lock);
...
static void ct_program_connect_to_wrk(u32 ho_index)
{
    ct_main_t *cm = &ct_main;
    u32        thread_index;
    ...
    if (!cm->fwrk_have_flush) {
        session_send_rpc_evt_to_thread_force(cm->fwrk_thread, ct_fwrk_flush_connects, uword_to_pointer(thread_index, void *));
...
static u32 ct_start_listen(u32 app_listener_index, transport_endpoint_cfg_t *tep)
{
...
static transport_connection_t *ct_session_half_open_get(u32 ct_index)
{
...
static void ct_session_cleanup(u32 conn_index, u32 thread_index)
{
    ...
    if (peer_ct) peer_ct->peer_index = ~0;
...
static int ct_session_connect(transport_endpoint_cfg_t *tep)
{
    ...
    if (lh == SESSION_DROP_HANDLE) return SESSION_E_FILTERED;

    if (lh == SESSION_INVALID_HANDLE) goto global_scope;
     ...
     */
    if (al->app_index == app->app_index) goto global_scope;
...
global_scope:
    if (session_endpoint_is_local(sep)) return SESSION_E_NOROUTE;

    if (!application_has_global_scope(app)) return SESSION_E_SCOPE;
    ...
    if (ll) {
        al = app_listener_get(ll->al_index);
        if (al->app_index != app->app_index) return ct_connect(app_wrk, ll, sep_ext);
    }
...
static inline int ct_close_is_reset(ct_connection_t *ct, session_t *s)
{
    if (ct->flags & CT_CONN_F_RESET) return 1;
    if (ct->flags & CT_CONN_F_CLIENT)
...
static void ct_session_cleanup_server_session(session_t *s)
{
    ct_connection_t *ct;
...
static void ct_session_postponed_cleanup(ct_connection_t *ct)
{
    ct_connection_t *peer_ct;
    app_worker_t    *app_wrk;
    session_t       *s;
    ...
    if (app_wrk) app_worker_cleanup_notify(app_wrk, s, SESSION_CLEANUP_TRANSPORT);
    ...
    if (ct->flags & CT_CONN_F_CLIENT) {
        ...
        if (app_wrk) {
            ...
            app_worker_cleanup_notify_custom(app_wrk, s, SESSION_CLEANUP_SESSION, ct_session_cleanup_server_session);
        }
        else {
            ct_connection_free(ct);
        }
...
static void ct_handle_cleanups(void *args)
{
    ...
    if (clib_fifo_elts(wrk->pending_cleanups)) {
        wrk->have_cleanups = 1;
        session_send_rpc_evt_to_thread_force(thread_index, ct_handle_cleanups, uword_to_pointer(thread_index, void *));
    }
}

static void ct_program_cleanup(ct_connection_t *ct)
{
    ct_cleanup_req_t *req;
    uword             thread_index;
    ct_worker_t      *wrk;
...
static void ct_session_reset(u32 ct_index, u32 thread_index)
{
    ct_connection_t *ct;
    ct = ct_connection_get(ct_index, thread_index);
...
static u8 *format_ct_connection_id(u8 *s, va_list *args)
{
    ...
    if (ct->c_is_ip4) {
        s = format(s,
                   ...
                   format_ip4_address,
                   &ct->c_lcl_ip4,
                   clib_net_to_host_u16(ct->c_lcl_port),
                   format_ip4_address,
                   &ct->c_rmt_ip4,
                   clib_net_to_host_u16(ct->c_rmt_port));
    }
    else {
        s = format(s,
                   "[%d:%d][CT:%U] %U:%d->%U:%d",
...
static int ct_custom_tx(void *session, transport_send_params_t *sp)
{
    ...
    if (session_has_transport(s)) return 0;
    ...
    if (!ct_session_tx(s)) sp->flags = TRANSPORT_SND_F_DESCHED;
...
static int ct_app_rx_evt(transport_connection_t *tc)
{
    ...
    s = session_get(ct->c_s_index, ct->c_thread_index);
    if (session_has_transport(s) || s->session_state < SESSION_STATE_READY) return -1;
    peer_ct = ct_connection_get(ct->peer_index, tc->thread_index);
    if (!peer_ct || (peer_ct->flags & CT_CONN_F_HALF_OPEN)) return -1;
    ps = session_get(peer_ct->c_s_index, peer_ct->c_thread_index);
    if (ps->session_state >= SESSION_STATE_TRANSPORT_CLOSING) return -1;
...
static u8 *format_ct_listener(u8 *s, va_list *args)
{
    ...
    if (verbose) s = format(s, "%-" SESSION_CLI_STATE_LEN "s", "LISTEN");
    return s;
}
...
static u8 *format_ct_half_open(u8 *s, va_list *args)
{
    ...
    s                         = format(s, "%-" SESSION_CLI_ID_LEN "U", format_ct_connection_id, ct);
    if (verbose) s = format(s, "%-" SESSION_CLI_STATE_LEN "s", "HALF-OPEN");
    return s;
}
...
static inline int ct_session_can_tx(session_t *s)
{
    return (s->session_state == SESSION_STATE_READY || s->session_state == SESSION_STATE_CLOSING || s->session_state == SESSION_STATE_APP_CLOSED);
}
...
int ct_session_tx(session_t *s)
{
    ...
    if (!ct_session_can_tx(s)) return 0;
    ct      = (ct_connection_t *) session_get_transport(s);
    peer_ct = ct_connection_get(ct->peer_index, ct->c_thread_index);
    if (!peer_ct) return 0;
    peer_s = session_get(peer_ct->c_s_index, peer_ct->c_thread_index);
    if (peer_s->session_state >= SESSION_STATE_TRANSPORT_CLOSING) return 0;
...
{
    transport_register_protocol(TRANSPORT_PROTO_NONE, &cut_thru_proto, FIB_PROTOCOL_IP4, ~0);
    transport_register_protocol(TRANSPORT_PROTO_NONE, &cut_thru_proto, FIB_PROTOCOL_IP6, ~0);
    return 0;
}
