...
u8 *format_session_fifos(u8 *s, va_list *args)
{
    ...
    if (!ss->rx_fifo || !ss->tx_fifo) return s;
...
u8 *format_session_flags(u8 *s, va_list *args)
{
    ...
    for (i = 0; i < SESSION_N_FLAGS; i++)
        if (ss->flags & (1 << i)) last = i;

    for (i = 0; i < last; i++) {
        if (ss->flags & (1 << i)) s = format(s, "%s, ", session_flags_str[i]);
    }
    if (last >= 0) s = format(s, "%s", session_flags_str[last]);
...
u8 *format_session(u8 *s, va_list *args)
{
    ...
    if (ss->session_state >= SESSION_STATE_ACCEPTING || ss->session_state == SESSION_STATE_CREATED) {
        s = format(s, "%U", format_transport_connection, tp, ss->connection_index, ss->thread_index, verbose);
        if (verbose == 1) s = format(s, "%v", str);
        if (verbose > 1) {
            s = format(s, "%U", format_session_fifos, ss, verbose);
            s = format(s, " session: state: %U opaque: 0x%x flags: %U\n", format_session_state, ss, ss->opaque, format_session_flags, ss);
        }
    }
    else if (ss->session_state == SESSION_STATE_LISTENING) {
        s = format(s, "%U%v", format_transport_listen_connection, tp, ss->connection_index, ss->thread_index, verbose, str);
        ...
        if (ss->flags & SESSION_F_HALF_OPEN) {
            s = format(s, "%U", format_transport_half_open_connection, tp, ss->connection_index, ss->thread_index, verbose);
            s = format(s, "%v", str);
        }
        else
            s = format(s, "%U", format_transport_connection, tp, ss->connection_index, ss->thread_index, verbose);
...
uword unformat_stream_session_id(unformat_input_t *input, va_list *args)
{
    ...
    if (unformat(input, "%U:%d->%U:%d", unformat_ip4_address, &lcl->ip4, lcl_port, unformat_ip4_address, &rmt->ip4, rmt_port)) {
        *is_ip4      = 1;
        tuple_is_set = 1;
    }
    else if (unformat(input, "%U:%d->%U:%d", unformat_ip6_address, &lcl->ip6, lcl_port, unformat_ip6_address, &rmt->ip6, rmt_port)) {
        *is_ip4      = 0;
        tuple_is_set = 1;
    }
...
#define _(sym, str)                   \
    ...
    foreach_session_state
#undef _
        done : vec_free(state_vec);
    return rv;
}

static uword unformat_ip_port(unformat_input_t *input, va_list *args)
{
    ip46_address_t *ip   = va_arg(*args, ip46_address_t *);
    u16            *port = va_arg(*args, u16 *);

    if (unformat(input, "%U:%d", unformat_ip46_address, ip, IP46_TYPE_ANY, port))
        ;
...
uword unformat_session(unformat_input_t *input, va_list *args)
{
    ...
    if (!unformat(input, "%U", unformat_stream_session_id, &proto, &fib_index, &lcl, &rmt, &lcl_port, &rmt_port, &is_ip4)) return 0;

    if (is_ip4)
        s = session_lookup_safe4(fib_index, &lcl.ip4, &rmt.ip4, clib_host_to_net_u16(lcl_port), clib_host_to_net_u16(rmt_port), proto);
    else
        s = session_lookup_safe6(fib_index, &lcl.ip6, &rmt.ip6, clib_host_to_net_u16(lcl_port), clib_host_to_net_u16(rmt_port), proto);
    if (s) {
        *result = s;
        return 1;
    }
...
uword unformat_transport_connection(unformat_input_t *input, va_list *args)
{
    ...
    if (!unformat(input, "%U", unformat_stream_session_id, &fib_index, &proto, &lcl, &rmt, &lcl_port, &rmt_port, &is_ip4)) return 0;

    proto = (proto == (u8) ~0) ? suggested_proto : proto;
    if (proto == (u8) ~0) return 0;
    if (is_ip4)
        tc = session_lookup_connection4(fib_index, &lcl.ip4, &rmt.ip4, clib_host_to_net_u16(lcl_port), clib_host_to_net_u16(rmt_port), proto);
    else
        tc = session_lookup_connection6(fib_index, &lcl.ip6, &rmt.ip6, clib_host_to_net_u16(lcl_port), clib_host_to_net_u16(rmt_port), proto);
...
static void session_cli_show_all_sessions(vlib_main_t *vm, int verbose)
{
    ...
    for (thread_index = 0; thread_index < vec_len(smm->wrk); thread_index++) {
        ...
        if (!verbose) {
            vlib_cli_output(vm, "Thread %d: %d sessions", thread_index, pool_elts(pool));
            continue;
        }

        if (pool_elts(pool) > 50) {
            vlib_cli_output(vm,
                            "Thread %u: %d sessions. Verbose output "
        ...
        if (verbose == 1)
            vlib_cli_output(vm,
                            "%s%-" SESSION_CLI_ID_LEN "s%-" SESSION_CLI_STATE_LEN "s%-10s%-10s",
                            thread_index ? "\n" : "",
                            "Connection",
                            "State",
                            "Rx-f",
        ...
        if (!n_closed)
            vlib_cli_output(vm, "Thread %d: active sessions %u", thread_index, pool_elts(pool) - n_closed);
        else
            vlib_cli_output(vm, "Thread %d: active sessions %u closed %u", thread_index, pool_elts(pool) - n_closed, n_closed);
    }
}
...
static int session_cli_filter_check(session_t *s, session_cli_filter_t *sf)
{
    transport_connection_t *tc;
    ...
    if (sf->states) {
        ...
        vec_foreach (state, sf->states)
            if (s->session_state == *state) goto check_transport;
        return 0;
    }
...
check_transport:
    ...
    if (!sf->endpt_flags) return 1;

    tc = session_get_transport(s);
    if (sf->endpt_flags & SESSION_CLI_FILTER_ENDPT_LOCAL) {
        if (!ip46_address_cmp(&sf->endpt.ip, &tc->lcl_ip) && (sf->endpt.port == 0 || sf->endpt.port == clib_net_to_host_u16(tc->lcl_port))) return 1;
    }
    if (sf->endpt_flags & SESSION_CLI_FILTER_ENDPT_REMOTE) {
        if (!ip46_address_cmp(&sf->endpt.ip, &tc->rmt_ip) && (sf->endpt.port == 0 || sf->endpt.port == clib_net_to_host_u16(tc->rmt_port))) return 1;
    }
    return 0;
}

static void session_cli_show_session_filter(vlib_main_t *vm, session_cli_filter_t *sf)
{
    ...
    if (sf->range.end < sf->range.start) {
        vlib_cli_output(vm, "invalid range start: %u end: %u", sf->range.start, sf->range.end);
        return;
    }
    ...
    if (sf->transport_proto == TRANSPORT_PROTO_INVALID && sf->states == 0 && !sf->verbose && (sf->range.start == 0 && sf->range.end == ~0)) {
        vlib_cli_output(vm, "Thread %d: %u sessions", sf->thread_index, pool_elts(pool));
        return;
    }

    max_index = pool_len(pool) ? pool_len(pool) - 1 : 0;
    for (i = sf->range.start; i <= clib_min(sf->range.end, max_index); i++) {
        if (pool_is_free_index(pool, i)) continue;

        s = pool_elt_at_index(pool, i);

        if (!session_cli_filter_check(s, sf)) continue;

        count += 1;
        if (sf->verbose) {
            if (!(sf->flags & SESSION_CLI_FILTER_FORCE_PRINT) && (count > 50 || (sf->verbose > 1 && count > 10))) {
                output_suppressed = 1;
                continue;
            }
    ...
    if (!output_suppressed)
        vlib_cli_output(vm, "Thread %d: %u sessions matched filter", sf->thread_index, count);
    else
        vlib_cli_output(vm,
...
void session_cli_show_events_thread(vlib_main_t *vm, u32 thread_index)
{
    ...
    vlib_cli_output(vm, " evt elements alloc: %u", clib_llist_elts(wrk->event_elts));
    vlib_cli_output(vm, " ctrl evt elt data alloc: %d", clib_llist_elts(wrk->ctrl_evts_data));
}
...
static u8 *format_rt_backend(u8 *s, va_list *args)
{
...
static clib_error_t *show_session_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
         ...
         * listeners
         */
        else if (unformat(input, "listeners %U", unformat_transport_proto, &transport_proto))
            do_listeners = 1;
        /*
         * session events
         */
         ...
         */
        ...
        else if (unformat(input, "thread %u index %u", &thread_index, &session_index)) {
        ...
        else if (unformat(input, "thread %u proto %U index %u", &thread_index, unformat_transport_proto, &transport_proto, &transport_index)) {
            transport_connection_t *tc;
            tc = transport_get_connection(transport_proto, transport_index, thread_index);
            if (!tc) {
                vlib_cli_output(vm,
                                "transport connection %u thread %u is not"
                                " allocated",
                                transport_index,
                                thread_index);
                goto done;
            ...
            if (!s) {
                vlib_cli_output(vm,
                                "session for transport connection %u "
        ...
        else if (unformat(input, "proto %U", unformat_transport_proto, &sf.transport_proto))
            do_filter = 1;
        ...
        else if (unformat(input, "lcl %U", unformat_ip_port, &sf.endpt.ip, &sf.endpt.port)) {
            sf.endpt_flags |= SESSION_CLI_FILTER_ENDPT_LOCAL;
            do_filter = 1;
        }
        else if (unformat(input, "rmt %U", unformat_ip_port, &sf.endpt.ip, &sf.endpt.port)) {
            sf.endpt_flags |= SESSION_CLI_FILTER_ENDPT_REMOTE;
            do_filter = 1;
        }
        else if (unformat(input, "ep %U", unformat_ip_port, &sf.endpt.ip, &sf.endpt.port)) {
            sf.endpt_flags |= SESSION_CLI_FILTER_ENDPT_REMOTE | SESSION_CLI_FILTER_ENDPT_LOCAL;
            do_filter = 1;
        }
    ...
    if (one_session) {
        ...
        if (do_elog && s->session_state != SESSION_STATE_LISTENING) {
            ...
            if (track_index != ~0) str = format(str, " session elog:\n%U", format_elog_track, em, dt, track_index);
        }
    ...
    if (do_listeners) {
        ...
        vlib_cli_output(vm, "%-" SESSION_CLI_ID_LEN "s%-24s", "Listener", "App");

        pool_foreach (s, smm->wrk[0].sessions) {
            if (s->session_state != SESSION_STATE_LISTENING || s->session_type != sst) continue;
...
static clib_error_t *clear_session_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!clear_all && session_index == ~0) return clib_error_return(0, "session <nn> required, but not set.");

    if (session_index != ~0) {
        session = session_get_if_valid(session_index, thread_index);
        if (!session) return clib_error_return(0, "no session %d on thread %d", session_index, thread_index);
...
static clib_error_t *show_session_fifo_trace_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    str = is_rx ? svm_fifo_dump_trace(str, s->rx_fifo) : svm_fifo_dump_trace(str, s->tx_fifo);
...
static clib_error_t *session_replay_fifo_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    str = is_rx ? svm_fifo_replay(str, s->rx_fifo, 0, 1) : svm_fifo_replay(str, s->tx_fifo, 0, 1);
...
static clib_error_t *session_enable_disable_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (smm->is_enabled && args.is_en)
        if (args.rt_engine_type != smm->rt_engine_type) return clib_error_return(0, "session is already enable. Must disable first");
...
VLIB_CLI_COMMAND(session_enable_disable_command, static) = {
    .path       = "session",
    .short_help = "session { enable [ rt-backend sdl | rule-table ] } | { disable }",
    .function   = session_enable_disable_fn,
};

static clib_error_t *show_session_stats_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    session_main_t   *smm = &session_main;
    session_worker_t *wrk;
    unsigned int     *e;

    if (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) return clib_error_return(0, "unknown input `%U'", format_unformat_error, input);

    vec_foreach (wrk, smm->wrk) {
        vlib_cli_output(vm, "Thread %u:\n", wrk - smm->wrk);
        e = wrk->stats.errors;
#define _(name, str) \
    if (e[SESSION_EP_##name]) vlib_cli_output(vm, " %lu %s", e[SESSION_EP_##name], str);
...
static clib_error_t *clear_session_stats_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    session_main_t   *smm = &session_main;
    session_worker_t *wrk;
