...
u8 *format_transport_flags(u8 *s, va_list *args)
{
    ...
    for (i = 0; i < last; i++) {
        if (flags & (1 << i)) s = format(s, "%s, ", transport_flags_str[i]);
    }
    if (last >= 0) s = format(s, "%s", transport_flags_str[last]);
...
u8 *format_transport_connection(u8 *s, va_list *args)
{
    ...
    s  = format(s, "%U", tp_vft->format_connection, conn_index, thread_index, verbose);
    tc = tp_vft->get_connection(conn_index, thread_index);
    if (tc && verbose > 1) {
        indent = format_get_indent(s) + 1;
        if (transport_connection_is_tx_paced(tc))
            s = format(s, "%Upacer: %U\n", format_white_space, indent, format_transport_pacer, &tc->pacer, tc->thread_index);
        s = format(s, "%Utransport: flags: %U\n", format_white_space, indent, format_transport_flags, tc->flags);
    }
    return s;
}
...
static u8 unformat_transport_str_match(unformat_input_t *input, const char *str)
{
    ...
    if (strlen(str) > vec_len(input->buffer) - input->index) return 0;

    for (i = 0; i < strlen(str); i++) {
        if (input->buffer[i + input->index] != str[i]) return 0;
    }
    return 1;
}
...
{
    if (transport_proto >= vec_len(tp_vfts)) return 0;
...
{
    if (tp_vfts[tp].cleanup_ho) tp_vfts[tp].cleanup_ho(conn_index);
}

int transport_connect(transport_proto_t tp, transport_endpoint_cfg_t *tep)
{
    if (PREDICT_FALSE(!tp_vfts[tp].connect)) return SESSION_E_TRANSPORT_NO_REG;
    return tp_vfts[tp].connect(tep);
}

void transport_half_close(transport_proto_t tp, u32 conn_index, u8 thread_index)
{
    if (tp_vfts[tp].half_close) tp_vfts[tp].half_close(conn_index, thread_index);
}
...
{
    if (PREDICT_FALSE(!tp_vfts[tp].start_listen)) return SESSION_E_TRANSPORT_NO_REG;
...
always_inline void default_get_transport_endpoint(transport_connection_t *tc, transport_endpoint_t *tep, u8 is_lcl)
{
    if (is_lcl) {
...
{
    if (!tp_vfts[tp].attribute) return -1;
...
void transport_endpoint_free(u32 tepi)
{
    transport_main_t *tm = &tp_main;
    pool_put_index(tm->local_endpoints, tepi);
}

always_inline local_endpoint_t *transport_endpoint_alloc(void)
{
    transport_main_t *tm = &tp_main;
...
static void transport_cleanup_freelist(void)
{
    transport_main_t *tm = &tp_main;
    local_endpoint_t *lep;
    u32              *lep_indexp;
...
void transport_program_endpoint_cleanup(u32 lepi)
{
    transport_main_t *tm       = &tp_main;
    u8                flush_fl = 0;
    ...
    if (!tm->lcl_endpts_cleanup_pending && vec_len(tm->lcl_endpts_freelist) > 32) {
    ...
    if (flush_fl) session_send_rpc_evt_to_thread_force(transport_cl_thread(), transport_cleanup_freelist, 0);
}

int transport_release_local_endpoint(u8 proto, ip46_address_t *lcl_ip, u16 port)
{
    transport_main_t *tm = &tp_main;
    local_endpoint_t *lep;
    u32               lepi;

    lepi = transport_endpoint_lookup(&tm->local_endpoints_table, proto, lcl_ip, port);
    if (lepi == ENDPOINT_INVALID_INDEX) return -1;
...
static int transport_endpoint_mark_used(u8 proto, ip46_address_t *ip, u16 port)
{
    transport_main_t *tm = &tp_main;
    local_endpoint_t *lep;
    u32               tei;
    ...
    tei = transport_endpoint_lookup(&tm->local_endpoints_table, proto, ip, port);
    if (tei != ENDPOINT_INVALID_INDEX) return SESSION_E_PORTINUSE;
 ...
 */
int transport_alloc_local_port(u8 proto, ip46_address_t *lcl_addr, transport_endpoint_cfg_t *rmt)
{
    transport_main_t *tm  = &tp_main;
    ...
    for (tries = 0; tries < limit; tries++) {
        ...
        if (!transport_endpoint_mark_used(proto, lcl_addr, port)) return port;

        /* IP:port pair already in use, check if 6-tuple available */
        if (session_lookup_connection(rmt->fib_index, lcl_addr, &rmt->ip, port, rmt->port, proto, rmt->is_ip4)) continue;
...
static session_error_t transport_find_local_ip_for_remote(u32 *sw_if_index, transport_endpoint_t *rmt, ip46_address_t *lcl_addr)
{
    ...
    if (*sw_if_index == ENDPOINT_INVALID_INDEX) {
        ...
        if (fei == FIB_NODE_INDEX_INVALID) return SESSION_E_NOROUTE;

        *sw_if_index = fib_entry_get_resolving_interface(fei);
        if (*sw_if_index == ENDPOINT_INVALID_INDEX) return SESSION_E_NOINTF;
    }
...
int transport_alloc_local_endpoint(u8 proto, transport_endpoint_cfg_t *rmt_cfg, ip46_address_t *lcl_addr, u16 *lcl_port)
{
    ...
    if (ip_is_zero(&rmt_cfg->peer.ip, rmt_cfg->peer.is_ip4)) {
        error = transport_find_local_ip_for_remote(&rmt_cfg->peer.sw_if_index, rmt, lcl_addr);
        if (error) return error;
    }
    else {
        /* Assume session layer vetted this address */
        clib_memcpy_fast(lcl_addr, &rmt_cfg->peer.ip, sizeof(rmt_cfg->peer.ip));
    }

    /* Cleanup freelist if need be */
    if (vec_len(tm->lcl_endpts_freelist)) transport_cleanup_freelist();
    ...
    if (rmt_cfg->peer.port == 0) {
        ...
        *lcl_port = rmt_cfg->peer.port;
        ...
        if (!transport_endpoint_mark_used(proto, lcl_addr, rmt_cfg->peer.port)) return 0;

        /* IP:port pair already in use, check if 6-tuple available */
        if (session_lookup_connection(rmt->fib_index, lcl_addr, &rmt->ip, rmt_cfg->peer.port, rmt->port, proto, rmt->is_ip4))
...
static inline u32 spacer_max_burst(spacer_t *pacer, clib_us_time_t time_now)
{
...
static inline void spacer_update_bucket(spacer_t *pacer, u32 bytes)
{
    pacer->bucket -= bytes;
}

static inline void spacer_set_pace_rate(spacer_t *pacer, u64 rate_bytes_per_sec, clib_us_time_t rtt, clib_time_type_t sec_per_loop)
{
     ...
     */
    max_time         = clib_max(rtt / TRANSPORT_PACER_BURSTS_PER_RTT, (clib_us_time_t) (sec_per_loop * CLIB_US_TIME_FREQ));
    max_time         = clib_clamp(max_time, 1 /* 1us */, 1000 /* 1ms */);
    pacer->max_burst = (rate_bytes_per_sec * max_time) * CLIB_US_TIME_PERIOD;
    pacer->max_burst = clib_clamp(pacer->max_burst, TRANSPORT_PACER_MIN_BURST, TRANSPORT_PACER_MAX_BURST);
}

static inline u64 spacer_pace_rate(spacer_t *pacer)
{
    return pacer->bytes_per_sec;
}

static inline void spacer_reset(spacer_t *pacer, clib_us_time_t time_now, u64 bucket)
{
    pacer->last_update = time_now;
    pacer->bucket      = bucket;
}
...
{
    spacer_set_pace_rate(&tc->pacer, rate_bytes_per_sec, rtt, transport_seconds_per_loop(tc->thread_index));
    spacer_reset(&tc->pacer, transport_us_time_now(tc->thread_index), start_bucket);
}
...
void transport_connection_tx_pacer_init(transport_connection_t *tc, u64 rate_bytes_per_sec, u32 initial_bucket)
{
    ...
    transport_connection_tx_pacer_reset(tc, rate_bytes_per_sec, initial_bucket, 1e6);
}

void transport_connection_tx_pacer_update(transport_connection_t *tc, u64 bytes_per_sec, clib_us_time_t rtt)
{
    spacer_set_pace_rate(&tc->pacer, bytes_per_sec, rtt, transport_seconds_per_loop(tc->thread_index));
}

u32 transport_connection_tx_pacer_burst(transport_connection_t *tc)
{
    return spacer_max_burst(&tc->pacer, transport_us_time_now(tc->thread_index));
}
...
{
    if (transport_connection_is_tx_paced(tc)) spacer_update_bucket(&tc->pacer, bytes);
}
...
void transport_connection_reschedule(transport_connection_t *tc)
{
    ...
    if (transport_max_tx_dequeue(tc))
        ...
        if (svm_fifo_max_dequeue_cons(s->tx_fifo))
            if (svm_fifo_set_event(s->tx_fifo)) sesssion_reschedule_tx(tc);
    }
}
...
void transport_update_time(clib_time_type_t time_now, u8 thread_index)
{
    ...
    vec_foreach (vft, tp_vfts) {
        if (vft->update_time) (vft->update_time)(time_now, thread_index);
    }
}
...
void transport_enable_disable(vlib_main_t *vm, u8 is_en)
{
    transport_proto_vft_t *vft;
    vec_foreach (vft, tp_vfts) {
        if (vft->enable) (vft->enable)(vm, is_en);

        if (vft->update_time) session_register_update_time_fn(vft->update_time, is_en);
    }
}
...
void transport_init(void)
{
    ...
    if (smm->local_endpoints_table_buckets == 0) smm->local_endpoints_table_buckets = 250000;
    if (smm->local_endpoints_table_memory == 0) smm->local_endpoints_table_memory = 512 << 20;
    ...
    clib_bihash_init_24_8(&tm->local_endpoints_table, "local endpoints table", smm->local_endpoints_table_buckets, smm->local_endpoints_table_memory);
