...
VLIB_REGISTER_LOG_CLASS(session_sdl_log, static) = {.class_name = "session", .subclass_name = "sdl"};

#define log_debug(fmt, ...) vlib_log_debug(session_sdl_log.class, "%s: " fmt, __func__, __VA_ARGS__)
#define log_warn(fmt, ...)  vlib_log_warn(session_sdl_log.class, fmt, __VA_ARGS__)
#define log_err(fmt, ...)   vlib_log_err(session_sdl_log.class, fmt, __VA_ARGS__)
...
static fib_route_path_t *session_sdl_fib_create_route_paths(u32 fib_index, dpo_proto_t dpo_proto)
{
...
static void session_sdl_dpo_lock(dpo_id_t *dpo)
{
}

static void session_sdl_dpo_unlock(dpo_id_t *dpo)
{
}

static u8 *format_session_sdl_dpo(u8 *s, va_list *va)
{
...
static u32 session_sdl_lookup6(u32 srtg_handle, u32 proto, ip6_address_t *lcl_ip, ip6_address_t *rmt_ip, u16 lcl_port, u16 rmt_port)
{
    ...
    if (sdlb->ip6_fib_index == ~0) return SESSION_TABLE_INVALID_INDEX;
    lbi = ip6_fib_table_fwding_lookup(sdlb->ip6_fib_index, lcl_ip);
    dpo = load_balance_get_fwd_bucket(load_balance_get(lbi), 0);
    if (dpo->dpoi_type != sdl_dpo_type) return SESSION_TABLE_INVALID_INDEX;
    return (dpo->dpoi_index);
}

static u32 session_sdl_lookup4(u32 srtg_handle, u32 proto, ip4_address_t *lcl_ip, ip4_address_t *rmt_ip, u16 lcl_port, u16 rmt_port)
{
    session_rules_table_t *srt  = srtg_handle_to_srt(srtg_handle, 0);
    session_sdl_block_t   *sdlb = &srt->sdl_block;
    index_t                lbi;
    ...
    if (sdlb->ip_fib_index == ~0) return SESSION_TABLE_INVALID_INDEX;
...
static fib_table_walk_rc_t session_sdl4_fib_show_walk_cb(fib_node_index_t fei, void *arg)
{
...
static fib_table_walk_rc_t session_sdl6_fib_table_show_walk(fib_node_index_t fei, void *arg)
{
...
static void session_sdl_fib_table_show(u32 fei, ip46_address_t *lcl_ip, u16 fp_len, u32 action_index, u32 fp_proto, u8 *tag, void *args)
{
    vlib_main_t *vm   = args;
    u32          type = (fp_proto == FIB_PROTOCOL_IP4) ? IP46_TYPE_IP4 : IP46_TYPE_IP6;

    vlib_cli_output(vm,
                    "[%d] rule: %U/%d action: %d tag %U",
                    fei,
                    format_ip46_address,
                    lcl_ip,
                    type,
...
static void session_sdl_cli_dump(vlib_main_t *vm, u32 srtg_handle, u32 proto, u8 fib_proto)
{
    session_rules_table_t *srt  = srtg_handle_to_srt(srtg_handle, 0);
    session_sdl_block_t   *sdlb = &srt->sdl_block;
...
static void session_sdl4_fib_table_show_one(session_rules_table_t *srt, u32 fib_index, vlib_main_t *vm, ip4_address_t *address, u32 mask_len)
{
    ip4_fib_t       *fib;
    ...
    if (fei != FIB_NODE_INDEX_INVALID && fib_entry_is_sourced(fei, sdl_fib_src)) {
        ...
        const dpo_id_t *dpo       = load_balance_get_fwd_bucket(load_balance_get(lbi), 0);

        session_sdl_fib_table_show(fei, &pfx.fp_addr, pfx.fp_len, dpo->dpoi_index, FIB_PROTOCOL_IP4, tag, vm);
    }
}

static void session_sdl6_fib_table_show_one(session_rules_table_t *srt, u32 fib_index, vlib_main_t *vm, ip6_address_t *address, u32 mask_len)
{
    fib_node_index_t fei;
    ...
    if (fei != FIB_NODE_INDEX_INVALID && fib_entry_is_sourced(fei, sdl_fib_src)) {
        ...
        session_sdl_fib_table_show(fei, &pfx.fp_addr, pfx.fp_len, dpo->dpoi_index, FIB_PROTOCOL_IP6, tag, vm);
    }
}

static void session_sdl_show_rule(vlib_main_t    *vm,
                                  u32             srtg_handle,
                                  u32             proto,
                                  ...
                                  u8              is_ip4)
{
    ...
    sdlb = &srt->sdl_block;
    if (is_ip4)
        session_sdl4_fib_table_show_one(srt, sdlb->ip_fib_index, vm, &lcl_ip->ip4, 32);
    else
        session_sdl6_fib_table_show_one(srt, sdlb->ip6_fib_index, vm, &lcl_ip->ip6, 128);
}

static void session_sdl_table_init(session_table_t *st, u8 fib_proto)
{
    ...
    if (fib_proto == FIB_PROTOCOL_IP4 || all) {
        ...
        sdlb->ip_fib_index = fib_table_find_or_create_and_lock_w_name(FIB_PROTOCOL_IP4, sdlb->ip_table_id, sdl_fib_src, (const u8 *) name);
    }
    ...
    if (fib_proto == FIB_PROTOCOL_IP6 || all) {
        ...
        sdlb->ip6_fib_index = fib_table_find_or_create_and_lock_w_name(FIB_PROTOCOL_IP6, sdlb->ip6_table_id, sdl_fib_src, (const u8 *) name);
    }
...
static void session_sdl_table_free(session_table_t *st, u8 fib_proto)
{
...
static session_error_t session_sdl_add_del(u32 srtg_handle, u32 proto, session_rule_table_add_del_args_t *args)
{
    session_rules_table_t *srt  = srtg_handle_to_srt(srtg_handle, 0);
    session_sdl_block_t   *sdlb = &srt->sdl_block;
    u32                    fib_index;
    ...
    if (!(args->lcl_port == 0 && args->rmt_port == 0 && args->rmt.fp_addr.ip4.as_u32 == 0)) return SESSION_E_NOSUPPORT;

    fei = session_rules_table_rule_for_tag(srt, args->tag);
    if (args->is_add && fei != SESSION_RULES_TABLE_INVALID_INDEX) return SESSION_E_INVALID;
    ...
    if (args->is_add) {
        ...
        fei = fib_table_entry_path_add2(fib_index, &pfx, sdl_fib_src, FIB_ENTRY_FLAG_EXCLUSIVE, paths);
...
static void session_sdl_fib_init(void)
{
    static u32 session_fib_inited = 0;

    if (session_fib_inited) return;
    session_fib_inited = 1;
    sdl_fib_src        = fib_source_allocate("session sdl", FIB_SOURCE_PRIORITY_LOW, FIB_SOURCE_BH_SIMPLE);
    sdl_dpo_type       = dpo_register_new_type(&session_sdl_dpo_vft, session_sdl_dpo_nodes);
}

static void session_sdl_app_namespace_walk_cb(app_namespace_t *app_ns, void *ctx)
{
...
clib_error_t *session_sdl_enable_disable(int enable)
{
    ...
    if (enable) {
        ...
        if (error) log_err("error in disabling sdl: %U", format_clib_error, error);
    }
 ...
 * Source Deny List
 */
static clib_error_t *session_sdl_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    u32                         appns_index;
    ...
    if (session_sdl_is_enabled() == 0) return clib_error_return(0, "session sdl engine is not enabled");
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "del"))
            ...
            ;
        else if (unformat(input, "%U/%d", unformat_ip4_address, &lcl_ip.ip4, &lcl_plen)) {
            fib_proto = FIB_PROTOCOL_IP4;
            conn_set  = 1;
        }
        else if (unformat(input, "%U/%d", unformat_ip6_address, &lcl_ip.ip6, &lcl_plen)) {
...
VLIB_CLI_COMMAND(session_sdl_command, static) = {
    .path = "session sdl",
    .short_help =
        "session sdl [add|del] [appns <ns_id>] <lcl-ip/plen> action "
        "<action> [tag <tag>]",
    .function   = session_sdl_command_fn,
    .is_mp_safe = 1,
};

static clib_error_t *show_session_sdl_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    u32              fib_index;
    ip46_address_t   lcl_ip;
    u8               show_one = 0;
    ...
    if (show_one) {
        ...
        if (st && (st->srtg_handle != SESSION_SRTG_HANDLE_INVALID))
            session_rules_table_show_rule(vm, st->srtg_handle, 0, &lcl_ip, 0, 0, 0, (fib_proto == FIB_PROTOCOL_IP4));
        goto done;
    }
    ...
    if (st && (st->srtg_handle != SESSION_SRTG_HANDLE_INVALID)) session_rules_table_cli_dump(vm, st->srtg_handle, 0, FIB_PROTOCOL_IP4);

    fib_index = app_namespace_get_fib_index(app_ns, FIB_PROTOCOL_IP6);
    st        = session_table_get_for_fib_index(FIB_PROTOCOL_IP6, fib_index);
    if (st && (st->srtg_handle != SESSION_SRTG_HANDLE_INVALID)) session_rules_table_cli_dump(vm, st->srtg_handle, 0, FIB_PROTOCOL_IP6);
done:
    vec_free(ns_id);
    return 0;
}
...
void session_sdl_table_walk4(u32 srtg_handle, session_sdl_table_walk_fn_t fn, void *args)
{
    ...
    vec_foreach (fei, ctx.ifsw_indicies) {
        if (*fei != FIB_NODE_INDEX_INVALID && fib_entry_is_sourced(*fei, sdl_fib_src)) {
            ...
            fn(*fei, &pfx.fp_addr, pfx.fp_len, dpo->dpoi_index, FIB_PROTOCOL_IP4, tag, args);
        }
    }
...
void session_sdl_table_walk6(u32 srtg_handle, session_sdl_table_walk_fn_t fn, void *args)
{
    ...
    vec_foreach (fei, ctx.entries) {
        if (*fei != FIB_NODE_INDEX_INVALID && fib_entry_is_sourced(*fei, sdl_fib_src)) {
            ...
            fn(*fei, &pfx.fp_addr, pfx.fp_len, dpo->dpoi_index, FIB_PROTOCOL_IP6, tag, args);
        }
    }
