...
#define SESSION_EVT_DEQ_NODE_HANDLER(_wrk, _node_evt, _ntx)                         \
    {                                                                               \
        ...
        ed->data[0] = _node_evt;                                                    \
        ed->data[1] = _ntx;                                                         \
        ed->data[2] = (_wrk->last_vlib_time - _wrk->last_event_poll) * 1000000.0;   \
        ed->data[3] = (vlib_time_now(_wrk->vm) - _wrk->last_vlib_time) * 1000000.0; \
    }
#else
#define SESSION_EVT_DEQ_NODE_HANDLER(_wrk, _node_evt, _ntx)
...
#define SESSION_EVT_POLL_GAP(_wrk)                                                        \
    {                                                                                     \
        ...
        DEC_SESSION_ED(_e, 1);                                                            \
        ed->data[0] = (u32) ((_wrk->last_vlib_time - _wrk->last_event_poll) * 1000000.0); \
    }
#define SESSION_EVT_POLL_GAP_TRACK_HANDLER(_wrk)                                                   \
    {                                                                                              \
        if (PREDICT_TRUE(_wrk->last_event_poll != 0.0))                                            \
            if (_wrk->last_vlib_time > _wrk->last_event_poll + 500e-6) SESSION_EVT_POLL_GAP(_wrk); \
...
#define SESSION_EVT_DSP_CNTRS_NEW_IO_EVTS_HANDLER(_wrk, _diff, _args...)                            \
    ...
    sde->counters[SESS_Q_CNT_NEW_EVTS].u64 += sde->counters[SESS_Q_CNT_IO_EVTS].u64 - sde->prev_io; \
    sde->prev_io = sde->counters[SESS_Q_CNT_IO_EVTS].u64;

#define SESSION_EVT_DSP_CNTRS_OLD_IO_EVTS_HANDLER(_wrk, _diff, _args...)     \
    session_dbg_evts_t *sde = &session_dbg_main.wrk[_wrk->vm->thread_index]; \
    sde->counters[SESS_Q_CLK_OLD_IO_EVTS].f64 += _diff;                      \
    sde->counters[SESS_Q_CNT_OLD_EVTS].u64 += sde->counters[SESS_Q_CNT_IO_EVTS].u64 - sde->prev_io;
...
#define SESSION_EVT_COUNTS_HANDLER(_node_evt, _cnt, _wrk)                                      \
    {                                                                                          \
        session_dbg_main.wrk[_wrk->vm->thread_index].counters[SESS_Q_##_node_evt].u64 += _cnt; \
    }
...
#define SESSION_EVT_DISPATCH_START_HANDLER(_wrk)                            \
    {                                                                       \
        session_dbg_evts_t *sde;                                            \
        sde = &session_dbg_main.wrk[_wrk->vm->thread_index];                \
        if (SESSION_DEQ_EVTS > 1) SESSION_EVT_DEQ_NODE_HANDLER(_wrk, 0, 0); \
...
#define SESSION_EVT_DISPATCH_END_HANDLER(_wrk, _ntx)                       \
    {                                                                      \
        ...
        session_dbg_evts_t *sde;                                           \
        sde = &session_dbg_main.wrk[_wrk->vm->thread_index];               \
        if (_ntx) SESSION_EVT_DEQ_NODE_HANDLER(_wrk, 1, _ntx);             \
...
#define session_evt_grp_dbg_lvl(_evt) session_dbg_main.grp_dbg_lvl[session_evt_grp(_evt)]
#define SESSION_EVT(_evt, _args...)                                                                           \
    do {                                                                                                      \
        if (PREDICT_FALSE(session_evt_grp_dbg_lvl(_evt) >= session_evt_lvl(_evt))) CC(_evt, _HANDLER)(_args); \
    } while (0)
#else
#define SESSION_EVT(_evt, _args...)
