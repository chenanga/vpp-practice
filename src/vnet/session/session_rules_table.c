...
VLIB_REGISTER_LOG_CLASS(session_rt_log, static) = {.class_name = "session", .subclass_name = "rt"};

#define log_debug(fmt, ...) vlib_log_debug(session_rt_log.class, "%s: " fmt, __func__, __VA_ARGS__)
#define log_warn(fmt, ...)  vlib_log_warn(session_rt_log.class, fmt, __VA_ARGS__)
...
void session_rules_table_add_tag(session_rules_table_t *srt, u8 *tag, u32 rule_index, u8 is_ip4)
{
    ...
    if (tag == 0) return;
    rip = hash_get_mem(srt->rules_by_tag, tag);
    if (rip) session_rules_table_del_tag(srt, tag, is_ip4);
...
{
    uword *rp;
    if (tag == 0) return SESSION_RULES_TABLE_INVALID_INDEX;
...
u8 *format_session_rule_tag(u8 *s, va_list *args)
{
    static u8 *null_tag = 0;
    u8        *tag      = va_arg(*args, u8 *);

    if (!null_tag) null_tag = format(0, "none");
...
u8 *format_session_rule4(u8 *s, va_list *args)
{
    ...
    s = format(s,
               "[%d] rule: %U/%d %d %U/%d %d action: %d tag: %U",
               ri,
               format_ip4_address,
               &match->lcl_ip,
...
u8 *format_session_rule6(u8 *s, va_list *args)
{
    ...
    s = format(s,
               ...
               format_ip6_address,
               &match->rmt_ip,
               ip6_mask_to_preflen(&mask->rmt_ip),
               match->rmt_port,
               sr->action_index,
               format_session_rule_tag,
               tag);
...
int rule_cmp_16(mma_rule_16_t *rule1, mma_rule_16_t *rule2)
{
    ...
    if (m1->rmt_ip.as_u32 != m2->rmt_ip.as_u32) return (m1->rmt_ip.as_u32 < m2->rmt_ip.as_u32 ? -1 : 1);
    if (m1->lcl_ip.as_u32 != m2->lcl_ip.as_u32) return (m1->lcl_ip.as_u32 < m2->lcl_ip.as_u32 ? -1 : 1);
    if (m1->rmt_port != m2->rmt_port) return (m1->rmt_port < m2->rmt_port ? -1 : 1);
    if (m1->lcl_port != m2->lcl_port) return (m1->lcl_port < m2->lcl_port ? -1 : 1);
    return 0;
}
...
int rule_cmp_40(mma_rule_40_t *rule1, mma_rule_40_t *rule2)
{
    ...
    if (r1->rmt_ip.as_u64[0] != r2->rmt_ip.as_u64[0]) return (r1->rmt_ip.as_u64[0] < r2->rmt_ip.as_u64[0] ? -1 : 1);
    if (r1->rmt_ip.as_u64[1] != r2->rmt_ip.as_u64[1]) return (r1->rmt_ip.as_u64[1] < r2->rmt_ip.as_u64[1] ? -1 : 1);
    if (r1->lcl_ip.as_u64[0] != r2->lcl_ip.as_u64[0]) return (r1->lcl_ip.as_u64[0] < r2->lcl_ip.as_u64[0] ? -1 : 1);
    if (r1->lcl_ip.as_u64[1] != r2->lcl_ip.as_u64[1]) return (r1->lcl_ip.as_u64[1] < r2->lcl_ip.as_u64[1]) ? -1 : 1;
    if (r1->rmt_port != r2->rmt_port) return (r1->rmt_port < r2->rmt_port ? -1 : 1);
    if (r1->lcl_port != r2->lcl_port) return (r1->lcl_port < r2->lcl_port ? -1 : 1);
    return 0;
}
...
void session_rules_table_init_rule_16(mma_rule_16_t *rule, fib_prefix_t *lcl, u16 lcl_port, fib_prefix_t *rmt, u16 rmt_port)
{
    ...
    ip4_prefix_max_address_host_order(&rmt->fp_addr.ip4, rmt->fp_len, &max_match->rmt_ip);
    ip4_prefix_max_address_host_order(&lcl->fp_addr.ip4, lcl->fp_len, &max_match->lcl_ip);
...
void session_rules_table_init_rule_40(mma_rule_40_t *rule, fib_prefix_t *lcl, u16 lcl_port, fib_prefix_t *rmt, u16 rmt_port)
{
    ...
    ip6_prefix_max_address_host_order(&rmt->fp_addr.ip6, rmt->fp_len, &max_match->rmt_ip);
    ip6_prefix_max_address_host_order(&lcl->fp_addr.ip6, lcl->fp_len, &max_match->lcl_ip);
    max_match->lcl_port = lcl_port == 0 ? (u16) ~0 : lcl_port;
    max_match->rmt_port = rmt_port == 0 ? (u16) ~0 : rmt_port;
}
...
static u32 session_rules_table_lookup_rule4(session_rules_table_t *srt, ip4_address_t *lcl_ip, ip4_address_t *rmt_ip, u16 lcl_port, u16 rmt_port)
{
    ...
    if (srt4->rules == 0) return SESSION_TABLE_INVALID_INDEX;
    return mma_rules_table_lookup_rule_16(srt4, (mma_mask_or_match_16_t *) &key, srt4->root_index);
}

u32 session_rules_table_lookup4_(u32 srtg_handle, u32 proto, ip4_address_t *lcl_ip, ip4_address_t *rmt_ip, u16 lcl_port, u16 rmt_port)
{
    ...
    if (srt4->rules == 0) return SESSION_TABLE_INVALID_INDEX;
    return mma_rules_table_lookup_16(srt4, (mma_mask_or_match_16_t *) &key, srt4->root_index);
}

session_rules_table_t *srtg_handle_to_srt(u32 srtg_handle, u32 proto)
{
    session_rules_table_group_t *srtg = pool_elt_at_index(srt_instances, srtg_handle);
    session_rules_table_t       *srt  = &srtg->session_rules[proto];

    return srt;
}

static u32 session_rules_table_lookup_rule6(session_rules_table_t *srt, ip6_address_t *lcl_ip, ip6_address_t *rmt_ip, u16 lcl_port, u16 rmt_port)
{
    ...
    if (srt6->rules == 0) return SESSION_TABLE_INVALID_INDEX;
    clib_memcpy_fast(&key.lcl_ip, lcl_ip, sizeof(*lcl_ip));
    clib_memcpy_fast(&key.rmt_ip, rmt_ip, sizeof(*rmt_ip));
    return mma_rules_table_lookup_rule_40(srt6, (mma_mask_or_match_40_t *) &key, srt6->root_index);
}

u32 session_rules_table_lookup6_(u32 srtg_handle, u32 proto, ip6_address_t *lcl_ip, ip6_address_t *rmt_ip, u16 lcl_port, u16 rmt_port)
{
    session_rules_table_t    *srt  = srtg_handle_to_srt(srtg_handle, proto);
    mma_rules_table_40_t     *srt6 = &srt->session_rules_tables_40;
    session_mask_or_match_6_t key  = {
    ...
    clib_memcpy_fast(&key.lcl_ip, lcl_ip, sizeof(*lcl_ip));
    clib_memcpy_fast(&key.rmt_ip, rmt_ip, sizeof(*rmt_ip));
    return mma_rules_table_lookup_40(srt6, (mma_mask_or_match_40_t *) &key, srt6->root_index);
}
 ...
 */
session_error_t session_rules_table_add_del_(u32 srtg_handle, u32 proto, session_rule_table_add_del_args_t *args)
{
    session_rules_table_t *srt       = srtg_handle_to_srt(srtg_handle, proto);
    ...
    if (args->is_add && ri_from_tag != SESSION_RULES_TABLE_INVALID_INDEX) return SESSION_E_INVALID;
    ...
    if (fib_proto == FIB_PROTOCOL_IP4) {
        ...
        if (args->is_add) {
            mma_rule_16_t *rule4;
            rule4               = session_rules_table_alloc_rule_16(srt4, &args->lcl, args->lcl_port, &args->rmt, args->rmt_port);
            ...
            if (!rv) {
                ...
                ri = session_rules_table_lookup_rule4(srt, &args->lcl.fp_addr.ip4, &args->rmt.fp_addr.ip4, args->lcl_port, args->rmt_port);
            ...
            if (ri_from_tag != SESSION_RULES_TABLE_INVALID_INDEX) {
                ...
                session_rules_table_init_rule_16(rule, &args->lcl, args->lcl_port, &args->rmt, args->rmt_port);
                mma_rules_table_del_rule_16(srt4, rule, srt4->root_index);
            }
        }
    }
        ...
        srt6 = &srt->session_rules_tables_40;
        if (args->is_add) {
            rule6               = session_rules_table_alloc_rule_40(srt6, &args->lcl, args->lcl_port, &args->rmt, args->rmt_port);
            ...
            if (!rv) {
                ...
                ri = session_rules_table_lookup_rule6(srt, &args->lcl.fp_addr.ip6, &args->rmt.fp_addr.ip6, args->lcl_port, args->rmt_port);
                if (ri != SESSION_RULES_TABLE_INVALID_INDEX) {
            ...
            if (ri_from_tag != SESSION_RULES_TABLE_INVALID_INDEX) {
                ...
                rule = &_rule;
                clib_memset(rule, 0, sizeof(*rule));
                session_rules_table_init_rule_40(rule, &args->lcl, args->lcl_port, &args->rmt, args->rmt_port);
                mma_rules_table_del_rule_40(srt6, rule, srt6->root_index);
            }
        }
    }
...
void session_rules_table_free_(session_table_t *st, u8 fib_proto)
{
    session_rules_table_group_t *srtg = pool_elt_at_index(srt_instances, st->srtg_handle);
...
void srtg_instance_free(session_table_t *st)
{
    session_rules_table_group_t *srtg = pool_elt_at_index(srt_instances, st->srtg_handle);
...
session_rules_table_group_t *srtg_instance_alloc(session_table_t *st, u32 n_proto)
{
...
void session_rules_table_init_(session_table_t *st, u8 fib_proto)
{
    mma_rules_table_16_t        *srt4;
    mma_rules_table_40_t        *srt6;
    mma_rule_16_t               *rule4;
    ...
    vec_foreach (srt, srtg->session_rules) {
        ...
        ASSERT(srt4->rules == 0);
        rule4               = session_rules_table_alloc_rule_16(srt4, &null_prefix, 0, &null_prefix, 0);
        ...
        srt6 = &srt->session_rules_tables_40;
        ASSERT(srt6->rules == 0);
        rule6               = session_rules_table_alloc_rule_40(srt6, &null_prefix, 0, &null_prefix, 0);
...
void session_rules_table_show_rule_(vlib_main_t    *vm,
                                    u32             srtg_handle,
                                    u32             proto,
                                    ...
                                    u8              is_ip4)
{
    ...
    if (is_ip4) {
        ...
        ri  = mma_rules_table_lookup_rule_16(srt4, (mma_mask_or_match_16_t *) &key, srt4->root_index);
        ...
        ri  = mma_rules_table_lookup_rule_40(srt6, (mma_mask_or_match_40_t *) &key, srt6->root_index);
        sr6 = mma_rules_table_get_rule_40(srt6, ri);
        vlib_cli_output(vm, "%U", format_session_rule6, srt, sr6);
    }
}

void session_rules_table_cli_dump_(vlib_main_t *vm, u32 srtg_handle, u32 proto, u8 fib_proto)
{
    session_rules_table_t *srt = srtg_handle_to_srt(srtg_handle, proto);
    if (fib_proto == FIB_PROTOCOL_IP4) {
        mma_rules_table_16_t *srt4;
...
static void session_rules_table_app_namespace_walk_cb(app_namespace_t *app_ns, void *ctx)
{
    ...
    if (st) session_rules_table_free(st, FIB_PROTOCOL_MAX);
    ...
    if (st) session_rules_table_free(st, FIB_PROTOCOL_IP4);
    ...
    st          = session_table_get(table_index);
    if (st) session_rules_table_free(st, FIB_PROTOCOL_IP6);
}

clib_error_t *session_rules_table_enable_disable(int enable)
{
    clib_error_t *error;
...
clib_error_t *session_rt_backend_enable_disable(session_rt_engine_type_t rt_engine_type)
{
    session_main_t *smm   = &session_main;
    clib_error_t   *error = 0;

    if (rt_engine_type < RT_BACKEND_ENGINE_DISABLE || rt_engine_type > RT_BACKEND_ENGINE_SDL)
    ...
    if (!error) smm->rt_engine_type = rt_engine_type;
    return error;
}

clib_error_t *session_rule_table_register_engine(const session_rt_engine_vft_t *vft)
{
    if (session_rt_engine_vft == vft) return 0;
    if (session_rt_engine_vft) return clib_error_return(0, "session rule engine is already registered");

    session_rt_engine_vft = vft;
    return 0;
}

clib_error_t *session_rule_table_deregister_engine(const session_rt_engine_vft_t *vft)
{
    if (session_rt_engine_vft == vft)
        session_rt_engine_vft = 0;
    else
        return clib_error_return(0, "session rule engine is not registered to this engine");
