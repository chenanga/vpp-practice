...
static clib_error_t *show_session_dbg_clock_cycles_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    u32 thread;
...
#define _(sym, disp, type, str)                                                             \
    if (disp) {                                                                             \
        if (!type)                                                                          \
            vlib_cli_output(vm, "\t %25s : %12lu ", str, sdm->counters[SESS_Q_##sym].u64);  \
        else                                                                                \
            vlib_cli_output(vm, "\t %25s : %12.3f ", str, sdm->counters[SESS_Q_##sym].f64); \
    }
...
static_always_inline f64 session_dbg_time_now(u32 thread)
{
...
static clib_error_t *clear_session_dbg_clock_cycles_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static void session_debug_show_groups(vlib_main_t *vm)
{
    session_dbg_main_t *sdm = &session_dbg_main;
    int                 i   = 0;
    ...
    for (i = 0; i < SESSION_EVT_N_GRP; i++)
        vlib_cli_output(vm, "%-10d%-30s%-10d", i, session_evt_grp_str[i], sdm->grp_dbg_lvl[i]);
}

static clib_error_t *session_debug_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    session_dbg_main_t *sdm = &session_dbg_main;
    u32                 group, level = ~0;
...
static u8 session_node_cmp_event(session_event_t *e, svm_fifo_t *f)
{
    switch (e->event_type) {
        ...
        case SESSION_IO_EVT_TX_FLUSH:
            if (e->session_index == f->shr->master_session_index) return 1;
            break;
        case SESSION_CTRL_EVT_CLOSE:
        case SESSION_CTRL_EVT_RPC:
            break;
...
u8 session_node_lookup_fifo_event(svm_fifo_t *f, session_event_t *e)
{
    ...
    clib_llist_foreach(wrk->event_elts, evt_list, pool_elt_at_index(wrk->event_elts, wrk->new_head), elt, ({
    ...
    clib_llist_foreach(wrk->event_elts, evt_list, pool_elt_at_index(wrk->event_elts, wrk->old_head), elt, ({
                           found = session_node_cmp_event(&elt->evt, f);
                           if (found) {
                               clib_memcpy_fast(e, &elt->evt, sizeof(*e));
                               goto done;
