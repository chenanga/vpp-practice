...
#define foreach_session_endpoint_fields foreach_transport_endpoint_cfg_fields _(u8, transport_proto)
...
#define _(sym, str) SESSION_ENDPT_CFG_F_##sym = 1 << SESSION_ENDPT_CFG_F_BIT_##sym,
    foreach_session_endpoint_cfg_flags
#undef _
...
#define TRANSPORT_ENDPOINT_NULL                                                                                                      \
    {                                                                                                                                \
        .sw_if_index = ENDPOINT_INVALID_INDEX, .ip = SESSION_IP46_ZERO, .fib_index = ENDPOINT_INVALID_INDEX, .is_ip4 = 0, .port = 0, \
    }
...
#define SESSION_ENDPOINT_CFG_NULL                                                                                                         \
    {                                                                                                                                     \
        .sw_if_index = ENDPOINT_INVALID_INDEX, .ip = SESSION_IP46_ZERO, .fib_index = ENDPOINT_INVALID_INDEX, .is_ip4 = 0, .port = 0,      \
        .peer = TRANSPORT_ENDPOINT_NULL, .transport_proto = 0, .app_wrk_index = ENDPOINT_INVALID_INDEX, .opaque = ENDPOINT_INVALID_INDEX, \
...
always_inline u8 session_endpoint_fib_proto(session_endpoint_t *sep)
{
    return sep->is_ip4 ? FIB_PROTOCOL_IP4 : FIB_PROTOCOL_IP6;
}

static inline u8 session_endpoint_is_local(session_endpoint_t *sep)
{
    return (ip_is_zero(&sep->ip, sep->is_ip4) || ip_is_local_host(&sep->ip, sep->is_ip4));
}

static inline u8 session_endpoint_is_zero(session_endpoint_t *sep)
{
...
typedef enum session_ft_action_ { SESSION_FT_ACTION_ENQUEUED, SESSION_FT_ACTION_DEQUEUED, SESSION_FT_ACTION_N_ACTIONS } session_ft_action_t;
...
always_inline session_type_t session_type_from_proto_and_ip(transport_proto_t proto, u8 is_ip4)
{
    return (proto << 1 | is_ip4);
}

always_inline transport_proto_t session_type_transport_proto(session_type_t st)
{
    return (st >> 1);
}

always_inline u8 session_type_is_ip4(session_type_t st)
{
    return (st & 1);
}

always_inline transport_proto_t session_get_transport_proto(session_t *s)
{
    return (s->session_type >> 1);
}

always_inline fib_protocol_t session_get_fib_proto(session_t *s)
{
    u8 is_ip4 = s->session_type & 1;
    return (is_ip4 ? FIB_PROTOCOL_IP4 : FIB_PROTOCOL_IP6);
}

always_inline u8 session_has_transport(session_t *s)
{
    return (session_get_transport_proto(s) != TRANSPORT_PROTO_NONE);
}

static inline transport_service_type_t session_transport_service_type(session_t *s)
{
...
static inline transport_tx_fn_type_t session_transport_tx_fn_type(session_t *s)
{
    transport_proto_t tp;
    tp = session_get_transport_proto(s);
...
static inline u8 session_tx_is_dgram(session_t *s)
{
    return (session_transport_tx_fn_type(s) == TRANSPORT_TX_DGRAM);
}

always_inline session_handle_t session_handle(session_t *s)
{
    return s->handle;
}

always_inline u32 session_index_from_handle(session_handle_tu_t handle)
{
    return handle.session_index;
}

always_inline u32 session_thread_from_handle(session_handle_tu_t handle)
{
    return handle.thread_index;
}

always_inline void session_parse_handle(session_handle_tu_t handle, u32 *index, u32 *thread_index)
{
    *index        = handle.session_index;
    *thread_index = handle.thread_index;
}

static inline session_handle_t session_make_handle(u32 session_index, u32 data)
{
    return ((session_handle_tu_t){.session_index = session_index, .thread_index = data}).handle;
}
...
typedef enum { SESSION_MQ_IO_EVT_RING, SESSION_MQ_CTRL_EVT_RING, SESSION_MQ_N_RINGS } session_mq_rings_e;
...
STATIC_ASSERT(sizeof(session_dgram_hdr_t) == (SESSION_CONN_ID_LEN + 10), "session conn id wrong length");
