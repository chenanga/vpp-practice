...
always_inline u32 segment_manager_segment_index(segment_manager_t *sm, fifo_segment_t *seg)
{
 ...
 */
static inline int segment_manager_add_segment_inline(segment_manager_t *sm, uword segment_size, u8 notify_app, u8 flags, u8 need_lock)
{
     ...
     */
    if (need_lock) clib_rwlock_writer_lock(&sm->segments_rwlock);
     ...
     * of that requested */
    segment_size += sizeof(fifo_segment_header_t) + vlib_thread_main.n_vlib_mains * sizeof(fifo_segment_slice_t) + FIFO_SEGMENT_ALLOC_OVERHEAD;
    ...
    seg_name = format(0, "seg-%u-%u-%u%c", app_wrk->app_index, app_wrk->wrk_index, smm->seg_name_counter++, 0);
    ...
    if ((rv = ssvm_server_init(&fs->ssvm, props->segment_type))) {
        clib_warning("svm_master_init ('%v', %u) failed", seg_name, segment_size);
    ...
    if (need_lock) clib_rwlock_writer_unlock(&sm->segments_rwlock);
...
{
    return segment_manager_add_segment_inline(sm, segment_size, notify_app, 0 /* flags */, 0 /* need_lock */);
}

int segment_manager_add_segment2(segment_manager_t *sm, uword segment_size, u8 flags)
{
    return segment_manager_add_segment_inline(sm, segment_size, 0, flags, vlib_num_workers());
}
...
void segment_manager_del_segment(segment_manager_t *sm, fifo_segment_t *fs)
{
    ...
    if (CLIB_DEBUG) clib_memset(fs, 0xfb, sizeof(*fs));
    pool_put(sm->segments, fs);
}

static fifo_segment_t *segment_manager_get_segment_if_valid(segment_manager_t *sm, u32 segment_index)
{
    if (pool_is_free_index(sm->segments, segment_index)) return 0;
 ...
 */
static inline void sm_lock_and_del_segment_inline(segment_manager_t *sm, u32 fs_index, u8 check_if_empty)
{
    fifo_segment_t *fs;
    u8              is_prealloc;
    ...
    if (check_if_empty && fifo_segment_has_fifos(fs)) goto done;

    is_prealloc = fifo_segment_flags(fs) & FIFO_SEGMENT_F_IS_PREALLOCATED;
    if (is_prealloc && !segment_manager_app_detached(sm)) goto done;
...
fifo_segment_t *segment_manager_get_segment_w_handle(u64 segment_handle)
{
    ...
    segment_manager_parse_segment_handle(segment_handle, &sm_index, &segment_index);
    sm = segment_manager_get(sm_index);
    if (!sm || pool_is_free_index(sm->segments, segment_index)) return 0;
    return pool_elt_at_index(sm->segments, segment_index);
}
...
int segment_manager_init(segment_manager_t *sm)
{
    ...
    sm->max_fifo_size = props->max_fifo_size ? props->max_fifo_size : sm_main.default_max_fifo_size;
    sm->max_fifo_size = clib_max(sm->max_fifo_size, 4096);

    segment_manager_set_watermarks(sm, props->high_watermark, props->low_watermark);
    return 0;
}
...
int segment_manager_init_first(segment_manager_t *sm)
{
    ...
    props          = segment_manager_properties_get(sm);
    first_seg_size = clib_max(props->segment_size, sm_main.default_segment_size);
    ...
    if (props->prealloc_fifos) {
        ...
        if (first_seg_size > approx_total_size) max_seg_size = first_seg_size;
        approx_segment_count = (approx_total_size + (max_seg_size - 1)) / max_seg_size;
        ...
        for (i = 0; i < approx_segment_count + 1; i++) {
            ...
            if (i == 0) sm->event_queue = segment_manager_alloc_queue(fs, props);

            fifo_segment_preallocate_fifo_pairs(fs, props->rx_fifo_size, props->tx_fifo_size, &prealloc_fifo_pairs);
            fifo_segment_flags(fs) = FIFO_SEGMENT_F_IS_PREALLOCATED;
            if (prealloc_fifo_pairs == 0) break;
        }
        return 0;
    }
    ...
    if (props->prealloc_fifo_hdrs) {
        ...
        for (; i < fs->n_slices; i++) {
            if (fifo_segment_prealloc_fifo_hdrs(fs, i, hdrs_per_slice)) return SESSION_E_SEG_CREATE;
        }
    }
...
void segment_manager_free(segment_manager_t *sm)
{
    ...
    ASSERT(vlib_get_thread_index() == 0 && !segment_manager_has_fifos(sm) && segment_manager_app_detached(sm));

    if (sm->flags & SEG_MANAGER_F_DETACHED_LISTENER) segment_manager_cleanup_detached_listener(sm);
    ...
    if (CLIB_DEBUG) clib_memset(sm, 0xfe, sizeof(*sm));
...
static void sm_free_w_index_helper(void *arg)
{
    ...
    if ((sm = segment_manager_get_if_valid(sm_index))) segment_manager_free(sm);
}
...
void segment_manager_free_safe(segment_manager_t *sm)
{
    if (!vlib_thread_is_main_w_barrier()) {
        ...
        vlib_rpc_call_main_thread(sm_free_w_index_helper, (u8 *) &sm_index, sizeof(sm_index));
    }
    else {
...
{
    if (pool_is_free_index(sm_main.segment_managers, index)) return 0;
    return pool_elt_at_index(sm_main.segment_managers, index);
}
...
u8 segment_manager_has_fifos(segment_manager_t *sm)
{
    ...
    segment_manager_foreach_segment_w_lock(seg, sm, ({
                                               if (CLIB_DEBUG && !first && !fifo_segment_has_fifos(seg)
                                                   ...
                                                   clib_warning("segment %d has no fifos!", segment_manager_segment_index(sm, seg));
                                                   first = 0;
                                               }
...
void segment_manager_del_sessions(segment_manager_t *sm)
{
    ...
    segment_manager_foreach_segment_w_lock(fs, sm, ({
                                               for (slice_index = 0; slice_index < fs->n_slices; slice_index++) {
                                                    ...
                                                    */
                                                   while (f) {
                                                       session = session_get_if_valid(f->shr->master_session_index, f->master_thread_index);
                                                       if (session) vec_add1(handles, session_handle(session));
                                                       f = f->next;
                                                   }
                                               }
                                    ...
                                    svm_fifo_t    **tx_fifo)
{
    ...
    *rx_fifo     = fifo_segment_alloc_fifo_w_slice(fs, thread_index, rx_fifo_size, FIFO_SEGMENT_RX_FIFO);

    tx_fifo_size = clib_max(tx_fifo_size, sm_main.default_fifo_size);
    *tx_fifo     = fifo_segment_alloc_fifo_w_slice(fs, thread_index, tx_fifo_size, FIFO_SEGMENT_TX_FIFO);
...
static inline int sm_lookup_segment_and_alloc_fifos(segment_manager_t       *sm,
                                                    segment_manager_props_t *props,
                                                    u32                      thread_index,
                                                    svm_fifo_t             **rx_fifo,
                                                    svm_fifo_t             **tx_fifo)
{
    ...
    pool_foreach (cur, sm->segments) {
        if (fifo_segment_flags(cur) & FIFO_SEGMENT_F_CUSTOM_USE) continue;
    ...
    if (PREDICT_FALSE(!fs)) return SESSION_E_SEG_NO_SPACE;

    return segment_manager_try_alloc_fifos(fs, thread_index, props->rx_fifo_size, props->tx_fifo_size, rx_fifo, tx_fifo);
}

static int sm_lock_and_alloc_segment_and_fifos(segment_manager_t       *sm,
                                               segment_manager_props_t *props,
                                               u32                      thread_index,
                                               svm_fifo_t             **rx_fifo,
                                               svm_fifo_t             **tx_fifo)
{
    ...
    if (!props->add_segment) return SESSION_E_SEG_NO_SPACE;

    clib_rwlock_writer_lock(&sm->segments_rwlock);

    /* Make sure there really is no free space. Another worker might've freed
     * some fifos or allocated a segment */
    rv = sm_lookup_segment_and_alloc_fifos(sm, props, thread_index, rx_fifo, tx_fifo);
    if (!rv) goto done;

    new_fs_index = segment_manager_add_segment(sm, 0 /* segment_size*/, 1 /* notify_app */);
    if (new_fs_index < 0) {
        rv = SESSION_E_SEG_CREATE;
        goto done;
    }
    fs = segment_manager_get_segment(sm, new_fs_index);
    rv = segment_manager_try_alloc_fifos(fs, thread_index, props->rx_fifo_size, props->tx_fifo_size, rx_fifo, tx_fifo);
    if (rv) {
...
int segment_manager_alloc_session_fifos(segment_manager_t *sm, u32 thread_index, svm_fifo_t **rx_fifo, svm_fifo_t **tx_fifo)
{
     ...
     */
    if (PREDICT_FALSE(rv < 0)) return sm_lock_and_alloc_segment_and_fifos(sm, props, thread_index, rx_fifo, tx_fifo);
...
void segment_manager_dealloc_fifos(svm_fifo_t *rx_fifo, svm_fifo_t *tx_fifo)
{
    ...
    if (!rx_fifo || !tx_fifo) return;

    /* Thread that allocated the fifos must be the one to clean them up */
    ASSERT(rx_fifo->master_thread_index == vlib_get_thread_index() || rx_fifo->refcnt > 1 || vlib_thread_is_main_w_barrier());

    /* It's possible to have no segment manager if the session was removed
     * as result of a detach. */
    if (!(sm = segment_manager_get_if_valid(rx_fifo->segment_manager))) return;
    ...
    if (PREDICT_FALSE(try_delete)) {
        ...
        sm_lock_and_del_segment_inline(sm, segment_index, 1 /* check_if_empty */);
...
svm_msg_q_t *segment_manager_alloc_queue(fifo_segment_t *segment, segment_manager_props_t *props)
{
    ...
    svm_msg_q_ring_cfg_t rc[SESSION_MQ_N_RINGS] = {{props->evt_q_size, fifo_evt_size, 0}, {notif_q_size, session_evt_size, 0}};
    cfg->consumer_pid                           = 0;
    cfg->n_rings                                = 2;
    ...
    if (props->use_mq_eventfd) {
        if (svm_msg_q_alloc_eventfd(q)) clib_warning("failed to alloc eventfd");
    }
    return q;
}
...
static u8 *format_segment_manager(u8 *s, va_list *args)
{
    ...
    if (!verbose || !pool_elts(sm->segments)) return s;

    s = format(s, "\n\n");

    segment_manager_foreach_segment_w_lock(seg, sm, ({ s = format(s, " *%U", format_fifo_segment, seg, verbose); }));
...
static clib_error_t *segment_manager_show_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!unformat_user(input, unformat_line_input, line_input)) {
        vlib_cli_output(vm, "%d segment managers allocated", pool_elts(smm->segment_managers));
        return 0;
    }
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "segments"))
            ...
            vlib_cli_output(vm, "unknown input [%U]", format_unformat_error, line_input);
            goto done;
        }
    }

    if (!pool_elts(smm->segment_managers)) goto done;
...
void segment_manager_format_sessions(segment_manager_t *sm, int verbose)
{
    ...
    if (!sm) {
        if (verbose)
            vlib_cli_output(vm, "%-" SESSION_CLI_ID_LEN "s%-20s%-15s%-10s", "Connection", "App", "API Client", "SegManager");
        else
            vlib_cli_output(vm, "%-" SESSION_CLI_ID_LEN "s%-20s", "Connection", "App");
        return;
    }
    ...
    pool_foreach (fs, sm->segments) {
        for (slice_index = 0; slice_index < fs->n_slices; slice_index++) {
            ...
            while (f) {
                ...
                if (verbose)
                    s = format(s,
                               "%-" SESSION_CLI_ID_LEN "v%-20v%-15u%-10u",
                               str,
                               app_name,
...
{
    ASSERT(high_watermark <= 100 && low_watermark <= 100 && low_watermark <= high_watermark);
