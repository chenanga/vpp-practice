...
static inline int session_send_evt_to_thread(void *data, void *args, u32 thread_index, session_evt_type_t evt_type)
{
    ...
    if (PREDICT_FALSE(svm_msg_q_lock(mq))) return -1;
    ...
    if (PREDICT_FALSE(wrk->state == SESSION_WRK_INTERRUPT)) vlib_node_set_interrupt_pending(wrk->vm, session_queue_node.index);
...
{
    return session_send_evt_to_thread(&f->shr->master_session_index, 0, f->master_thread_index, evt_type);
}
...
int session_program_tx_io_evt(session_handle_tu_t sh, session_evt_type_t evt_type)
{
...
int session_program_rx_io_evt(session_handle_tu_t sh)
{
    if (sh.thread_index == vlib_get_thread_index()) {
        ...
        return session_send_evt_to_thread((void *) &sh.session_index, 0, (u32) sh.thread_index, SESSION_IO_EVT_BUILTIN_RX);
    }
}
...
int session_send_ctrl_evt_to_thread(session_t *s, session_evt_type_t evt_type)
{
    ...
    ASSERT(evt_type == SESSION_CTRL_EVT_CLOSE || evt_type == SESSION_CTRL_EVT_HALF_CLOSE || evt_type == SESSION_CTRL_EVT_RESET);
...
void session_free(session_t *s)
{
    ...
    if (CLIB_DEBUG) clib_memset(s, 0xFA, sizeof(*s));
...
u8 session_is_valid(u32 si, u8 thread_index)
{
    ...
    if (s->thread_index != thread_index || s->session_index != si) return 0;

    if (s->session_state == SESSION_STATE_TRANSPORT_DELETED || s->session_state <= SESSION_STATE_LISTENING) return 1;

    if ((s->session_state == SESSION_STATE_CONNECTING || s->session_state == SESSION_STATE_TRANSPORT_CLOSED) && (s->flags & SESSION_F_HALF_OPEN))
        return 1;

    tc = session_get_transport(s);
    if (s->connection_index != tc->c_index || s->thread_index != tc->thread_index || tc->s_index != si) return 0;
...
static void session_cleanup_notify(session_t *s, session_cleanup_ntf_t ntf)
{
    ...
    if (PREDICT_FALSE(!app_wrk)) {
        if (ntf == SESSION_CLEANUP_TRANSPORT) return;
...
static void session_delete(session_t *s)
{
    ...
    if ((rv = session_lookup_del_session(s))) clib_warning("session %u hash delete rv %d", s->session_index, rv);
...
void session_cleanup_half_open(session_handle_t ho_handle)
{
    ...
    if (ho->flags & SESSION_F_IS_MIGRATING) {
        ...
        transport_cleanup(session_get_transport_proto(ho), ho->connection_index, ho->al_index /* overloaded */);
    }
        ...
        if (ho->session_state != SESSION_STATE_TRANSPORT_CLOSED) {
            ...
            if (tc && !(tc->flags & TRANSPORT_CONNECTION_F_NO_LOOKUP)) session_lookup_del_half_open(tc);
        }
...
void session_half_open_delete_notify(transport_connection_t *tc)
{
    ...
    if (ho->session_state != SESSION_STATE_TRANSPORT_CLOSED) {
        if (!(tc->flags & TRANSPORT_CONNECTION_F_NO_LOOKUP)) session_lookup_del_half_open(tc);
    }
    ...
    if (tc->thread_index == transport_cl_thread()) {
        ...
        session_send_rpc_evt_to_thread_force(transport_cl_thread(), session_half_open_free_rpc, args);
    }
}
 ...
 */
always_inline void session_enqueue_discard_chain_bytes(vlib_main_t *vm, vlib_buffer_t *b, vlib_buffer_t **chain_b, u32 n_bytes_to_drop)
{
    ...
    *chain_b = next;
    ...
    if (to_drop == 0) b->total_length_not_including_first_buffer -= n_bytes_to_drop;
}

/**
 * Enqueue buffer chain tail
 */
always_inline int session_enqueue_chain_tail(session_t *s, vlib_buffer_t *b, u32 offset, u8 is_in_order)
{
    ...
    if (is_in_order && offset) {
        ...
        if (diff > b->total_length_not_including_first_buffer) return 0;
        chain_b = b;
    ...
    do {
        ...
        if (is_in_order) {
            ...
            if (rv == len) {
                ...
                if (written > b->total_length_not_including_first_buffer) return written;
...
void session_wrk_program_app_wrk_evts(session_worker_t *wrk, u32 app_wrk_index)
{
    u8 need_interrupt;
    ...
    if (need_interrupt) vlib_node_set_interrupt_pending(wrk->vm, session_input_node.index);
}

always_inline void session_program_io_event(app_worker_t *app_wrk, session_t *s, session_evt_type_t et, u8 is_cl)
{
    if (is_cl) {
...
static inline int session_notify_subscribers(u32 app_index, session_t *s, svm_fifo_t *f, session_evt_type_t evt_type)
{
    app_worker_t  *app_wrk;
    application_t *app;
    u8             is_cl;
...
always_inline int session_enqueue_notify_inline(session_t *s, u8 is_cl)
{
    app_worker_t *app_wrk;

    app_wrk = app_worker_get_if_valid(s->app_wrk_index);
    if (PREDICT_FALSE(!app_wrk)) return -1;

    session_program_io_event(app_wrk, s, SESSION_IO_EVT_RX, is_cl);

    if (PREDICT_FALSE(svm_fifo_n_subscribers(s->rx_fifo))) return session_notify_subscribers(app_wrk->app_index, s, s->rx_fifo, SESSION_IO_EVT_RX);
...
int session_dequeue_notify(session_t *s)
{
    ...
    if (PREDICT_FALSE(svm_fifo_n_subscribers(s->tx_fifo))) return session_notify_subscribers(app_wrk->app_index, s, s->tx_fifo, SESSION_IO_EVT_TX);
 ...
 */
void session_main_flush_enqueue_events(transport_proto_t transport_proto, u32 thread_index)
{
    session_worker_t *wrk = session_main_get_worker(thread_index);
...
int session_enqueue_stream_connection(transport_connection_t *tc, vlib_buffer_t *b, u32 offset, u8 queue_event, u8 is_in_order)
{
    ...
    if (is_in_order) {
        ...
        if (PREDICT_FALSE((b->flags & VLIB_BUFFER_NEXT_PRESENT) && !rv)) session_enqueue_chain_tail(s, b, offset + b->current_length, 0);
...
always_inline int session_enqueue_dgram_connection_inline(session_t           *s,
                                                          ...
                                                          u32                  is_cl)
{
    ...
    if (PREDICT_TRUE(!(b->flags & VLIB_BUFFER_NEXT_PRESENT))) {
        svm_fifo_seg_t segs[2] = {{(u8 *) hdr, sizeof(*hdr)}, {vlib_buffer_get_current(b), b->current_length}};
        ...
        while (it) {
            ...
            n_segs++;
            if (!(it->flags & VLIB_BUFFER_NEXT_PRESENT)) break;
    ...
    if (queue_event && rv > 0) {
         ...
         * flushed by calling @ref session_main_flush_enqueue_events () */
        if (!(s->flags & SESSION_F_RX_EVT)) {
            u32               thread_index = is_cl ? vlib_get_thread_index() : s->thread_index;
            session_worker_t *wrk          = session_main_get_worker(thread_index);
...
int session_enqueue_dgram_connection2(session_t *s, session_dgram_hdr_t *hdr, vlib_buffer_t *b, u8 proto, u8 queue_event)
{
    return session_enqueue_dgram_connection_inline(s, hdr, b, proto, queue_event, 1 /* is_cl */);
}

int session_enqueue_dgram_connection_cl(session_t *s, session_dgram_hdr_t *hdr, vlib_buffer_t *b, u8 proto, u8 queue_event)
{
    session_t *awls;
...
u32 session_tx_fifo_dequeue_drop(transport_connection_t *tc, u32 max_bytes)
{
    ...
    if (svm_fifo_needs_deq_ntf(s->tx_fifo, max_bytes)) session_dequeue_notify(s);
...
int session_stream_connect_notify(transport_connection_t *tc, session_error_t err)
{
    ...
    if (err) return app_worker_connect_notify(app_wrk, s, err, opaque);
...
static void session_switch_pool_closed_rpc(void *arg)
{
    session_handle_t sh;
    session_t       *s;
...
static void session_switch_pool(void *cb_args)
{
    ...
    if (s->session_state >= SESSION_STATE_TRANSPORT_CLOSING) goto app_closed;

    new_sh = session_make_handle(args->new_session_index, args->new_thread_index);
...
app_closed:
    ...
    sh = session_handle(s);
    session_send_rpc_evt_to_thread(args->new_thread_index, session_switch_pool_closed_rpc, uword_to_pointer(sh, void *));
    clib_mem_free(cb_args);
}
...
int session_dgram_connect_notify(transport_connection_t *tc, u32 old_thread_index, session_t **new_session)
{
    ...
    if (!(tc->flags & TRANSPORT_CONNECTION_F_NO_LOOKUP)) session_lookup_add_connection(tc, session_handle(new_s));
    ...
    session_send_rpc_evt_to_thread(rpc_args->thread_index, session_switch_pool, rpc_args);
...
void session_transport_closing_notify(transport_connection_t *tc)
{
    ...
    s = session_get(tc->s_index, tc->thread_index);
    if (s->session_state >= SESSION_STATE_TRANSPORT_CLOSING) return;
...
void session_transport_delete_notify(transport_connection_t *tc)
{
    ...
    if (!(s = session_get_if_valid(tc->s_index, tc->thread_index))) return;
...
void session_transport_closed_notify(transport_connection_t *tc)
{
    ...
    if (s->session_state >= SESSION_STATE_TRANSPORT_CLOSED) return;
    ...
    app_wrk = app_worker_get_if_valid(s->app_wrk_index);
    if (app_wrk) app_worker_transport_closed_notify(app_wrk, s);
}
...
int session_stream_accept_notify(transport_connection_t *tc)
{
    ...
    app_wrk = app_worker_get_if_valid(s->app_wrk_index);
    if (!app_wrk) return -1;
    if (s->session_state != SESSION_STATE_CREATED) return 0;
...
int session_open_cl(session_endpoint_cfg_t *rmt, session_handle_t *rsh)
{
    ...
    if (transport_connection_is_cless(tc)) s->flags |= SESSION_F_IS_CLESS;
...
int session_open_vc(session_endpoint_cfg_t *rmt, session_handle_t *rsh)
{
    ...
    *rsh              = session_handle(ho);
    ...
    if (!(tc->flags & TRANSPORT_CONNECTION_F_NO_LOOKUP)) session_lookup_add_half_open(tc, tc->c_index);
...
int session_listen(session_t *ls, session_endpoint_cfg_t *sep)
{
    ...
    if (transport_connection_is_cless(session_get_transport(ls))) ls->flags |= SESSION_F_IS_CLESS;
...
int session_stop_listen(session_t *s)
{
    ...
    if (s->session_state != SESSION_STATE_LISTENING) return SESSION_E_NOLISTEN;

    tc = transport_get_listener(tp, s->connection_index);

    /* If no transport, assume everything was cleaned up already */
    if (!tc) return SESSION_E_NONE;

    if (!(tc->flags & TRANSPORT_CONNECTION_F_NO_LOOKUP)) session_lookup_del_connection(tc);
...
{
    if (!s || (s->flags & SESSION_F_APP_CLOSED)) return;
    ...
    if (s->session_state >= SESSION_STATE_CLOSING) {
        ...
        if (s->session_state == SESSION_STATE_TRANSPORT_CLOSED || s->session_state == SESSION_STATE_TRANSPORT_DELETED)
            session_program_transport_ctrl_evt(s, SESSION_CTRL_EVT_CLOSE);
        return;
    }
    ...
    if (s->tx_fifo) svm_fifo_clear_deq_ntf(s->tx_fifo);
...
{
    if (s->session_state >= SESSION_STATE_CLOSING) return;
    /* Drop all outstanding tx data
     * App might disconnect session before connected, in this case,
     * tx_fifo may not be setup yet, so clear only it's inited. */
    if (s->tx_fifo) svm_fifo_dequeue_drop_all(s->tx_fifo);
    session_set_state(s, SESSION_STATE_CLOSING);
...
void session_transport_close(session_t *s)
{
    if (s->session_state >= SESSION_STATE_APP_CLOSED) {
        if (s->session_state == SESSION_STATE_TRANSPORT_CLOSED) session_set_state(s, SESSION_STATE_CLOSED);
        /* If transport is already deleted, just free the session. Half-opens
         * expected to be already cleaning up at this point */
        else if (s->session_state >= SESSION_STATE_TRANSPORT_DELETED && !(s->flags & SESSION_F_HALF_OPEN))
            session_program_cleanup(s);
        return;
    }
...
void session_transport_cleanup(session_t *s)
{
    ...
    session_lookup_del_session(s);
    if (s->session_state != SESSION_STATE_TRANSPORT_DELETED) transport_cleanup(session_get_transport_proto(s), s->connection_index, s->thread_index);
 ...
 */
void session_vpp_wrk_mqs_alloc(session_main_t *smm)
{
    ...
    svm_msg_q_ring_cfg_t rc[SESSION_MQ_N_RINGS] = {{mq_q_length, evt_size, 0}, {mq_q_length >> 1, 256, 0}};
    cfg->consumer_pid                           = 0;
    cfg->n_rings                                = 2;
...
u64 session_segment_handle(session_t *s)
{
    ...
    if (!s->rx_fifo) return SESSION_INVALID_HANDLE;
                              ...
                              u16                  *original_dst_port)
{
    ...
    if (!smm->original_dst_lookup || !i2o_dst->is_ip4) return;
    smm->original_dst_lookup(&i2o_src->ip.ip4, i2o_src->port, &i2o_dst->ip.ip4, i2o_dst->port, proto, original_dst, original_dst_port);
}
...
void session_register_transport(transport_proto_t transport_proto, const transport_proto_vft_t *vft, u8 is_ip4, u32 output_node)
{
    ...
    if (output_node != ~0) next_index = vlib_node_add_next(vlib_get_main(), session_queue_node.index, output_node);
...
void session_register_update_time_fn(session_update_time_fn fn, u8 is_add)
{
    ...
    if (is_add) {
        ...
        if (found) vec_del1(smm->update_time_fns, fi_pos);
    }
}
...
transport_connection_t *session_get_transport(session_t *s)
{
    if (s->session_state != SESSION_STATE_LISTENING)
        return transport_get_connection(session_get_transport_proto(s), s->connection_index, s->thread_index);
    else
...
{
    if (s->session_state != SESSION_STATE_LISTENING)
        return transport_get_endpoint(session_get_transport_proto(s), s->connection_index, s->thread_index, tep, is_lcl);
    else
        return transport_get_listener_endpoint(session_get_transport_proto(s), s->connection_index, tep, is_lcl);
}

int session_transport_attribute(session_t *s, u8 is_get, transport_endpt_attr_t *attr)
{
    if (s->session_state < SESSION_STATE_READY) return -1;

    return transport_connection_attribute(session_get_transport_proto(s), s->connection_index, s->thread_index, is_get, attr);
}
...
static void session_stats_collector_fn(vlib_stats_collector_data_t *d)
{
...
static void session_stats_collector_init(void)
{
...
static clib_error_t *session_manager_main_enable(vlib_main_t *vm, session_rt_engine_type_t rt_engine_type)
{
    session_main_t     *smm = &session_main;
    ...
    if (session_rt_backend_enable_disable(rt_engine_type)) return clib_error_return(0, "error on enable backend engine");

    /* We only initialize once and do not de-initialized on disable */
    if (smm->is_initialized) goto done;

    num_threads = 1 /* main thread */ + vtm->n_threads;

    if (num_threads < 1) return clib_error_return(0, "n_thread_stacks not set");
    ...
    for (i = 0; i < num_threads; i++) {
        ...
        if (!smm->no_adaptive && smm->use_private_rx_mqs) session_wrk_enable_adaptive_mode(wrk);
    }
    ...
    if (smm->preallocated_sessions) {
        if (num_threads == 1) {
            ...
            int j;
            preallocated_sessions_per_worker = (1.1 * (f64) smm->preallocated_sessions / (f64) (num_threads - 1));
...
static void session_manager_main_disable(vlib_main_t *vm, session_rt_engine_type_t rt_engine_type)
{
...
void session_dma_completion_cb(vlib_main_t *vm, struct vlib_dma_batch *batch)
{
    session_worker_t *wrk;
    ...
    wrk->trans_head++;
    if (wrk->trans_head == wrk->trans_size) wrk->trans_head = 0;
    return;
}

static void session_prepare_dma_args(vlib_dma_config_t *args)
{
    args->max_batches         = 16;
...
static void session_node_enable_dma(u8 is_en, int n_vlibs)
{
    ...
    if (is_en) {
        ...
        if (wrk->config_index >= 0) vlib_dma_config_del(vm, wrk->config_index);
    }
    int i;
    for (i = 0; i < n_vlibs; i++) {
        ...
        if (is_en) {
            if (config_index >= 0) wrk->dma_enabled = true;
            wrk->dma_trans = (session_dma_transfer *) clib_mem_alloc(sizeof(session_dma_transfer) * DMA_TRANS_SIZE);
            bzero(wrk->dma_trans, sizeof(session_dma_transfer) * DMA_TRANS_SIZE);
        }
        else {
            if (wrk->dma_trans) clib_mem_free(wrk->dma_trans);
        }
        wrk->trans_head = 0;
        wrk->trans_tail = 0;
...
static void session_main_start_q_process(vlib_main_t *vm, vlib_node_state_t state)
{
    vlib_node_t *n;
    ...
    for (i = 0; i < n_vlibs; i++) {
        ...
        if (i == 0 && n_vlibs > 1) {
            ...
            if (is_en) {
                ...
                vlib_process_signal_event_mt(vm, session_queue_process_node.index, SESSION_Q_PROCESS_STOP, 0);
            }
            if (!sm->poll_main) continue;
        }
        vlib_node_set_state(vm, session_input_node.index, mstate);
        vlib_node_set_state(vm, session_queue_node.index, state);
    }

    if (sm->use_private_rx_mqs) application_enable_rx_mqs_nodes(is_en);

    if (sm->dma_enabled) session_node_enable_dma(is_en, n_vlibs);
}

clib_error_t *vnet_session_enable_disable(vlib_main_t *vm, session_enable_disable_args_t *args)
{
    clib_error_t *error = 0;

    if (args->is_en) {
        if (session_main.is_enabled) return 0;
...
static clib_error_t *session_config_fn(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "wrk-mq-length %d", &nitems)) {
        ...
        else if (unformat(input, "wrk-mqs-segment-size %U", unformat_memory_size, &smm->wrk_mqs_segment_size))
            ;
        else if (unformat(input, "preallocated-sessions %d", &smm->preallocated_sessions))
            ;
        else if (unformat(input, "v4-session-table-buckets %d", &smm->configured_v4_session_table_buckets))
            ;
        else if (unformat(input, "v4-halfopen-table-buckets %d", &smm->configured_v4_halfopen_table_buckets))
            ;
        else if (unformat(input, "v6-session-table-buckets %d", &smm->configured_v6_session_table_buckets))
            ;
        else if (unformat(input, "v6-halfopen-table-buckets %d", &smm->configured_v6_halfopen_table_buckets))
            ;
        else if (unformat(input, "v4-session-table-memory %U", unformat_memory_size, &tmp)) {
            if (tmp >= 0x100000000) return clib_error_return(0, "memory size %llx (%lld) too large", tmp, tmp);
            smm->configured_v4_session_table_memory = tmp;
        }
        else if (unformat(input, "v4-halfopen-table-memory %U", unformat_memory_size, &tmp)) {
            if (tmp >= 0x100000000) return clib_error_return(0, "memory size %llx (%lld) too large", tmp, tmp);
            smm->configured_v4_halfopen_table_memory = tmp;
        }
        else if (unformat(input, "v6-session-table-memory %U", unformat_memory_size, &tmp)) {
            if (tmp >= 0x100000000) return clib_error_return(0, "memory size %llx (%lld) too large", tmp, tmp);
            smm->configured_v6_session_table_memory = tmp;
        }
        else if (unformat(input, "v6-halfopen-table-memory %U", unformat_memory_size, &tmp)) {
            if (tmp >= 0x100000000) return clib_error_return(0, "memory size %llx (%lld) too large", tmp, tmp);
        ...
        else if (unformat(input, "local-endpoints-table-buckets %d", &smm->local_endpoints_table_buckets))
            ;
            ...
            smm->original_dst_lookup = vlib_get_plugin_symbol("nat_plugin.so", "nat44_original_dst_lookup");
        }
        /*
         ...
         */
            ...
            ;
        else if (unformat(input, "evt_qs_seg_size %U", unformat_memory_size, &smm->wrk_mqs_segment_size))
            ;
