...
static inline void session_wrk_send_evt_to_main(session_worker_t *wrk, session_evt_elt_t *elt)
{
    ...
    if (is_empty) session_send_rpc_evt_to_thread(0, session_wrk_handle_evts_main_rpc, uword_to_pointer(thread_index, void *));
}
...
static void session_wrk_timerfd_update(session_worker_t *wrk, u64 time_ns)
{
    ...
    if (timerfd_settime(wrk->timerfd, 0, &its, NULL) == -1) clib_warning("timerfd_settime");
}

always_inline u64 session_wrk_tfd_timeout(session_wrk_state_t state, u32 thread_index)
{
...
static inline void session_wrk_set_state(session_worker_t *wrk, session_wrk_state_t state)
{
    u64 time_ns;

    wrk->state = state;
    if (wrk->timerfd == -1) return;
...
static void session_mq_listen_handler(session_worker_t *wrk, session_evt_elt_t *elt)
{
    ...
    if (mp->ext_config) a->sep_ext.ext_cfg = session_mq_get_ext_config(app, mp->ext_config);

    if ((rv = vnet_listen(a))) session_worker_stat_error_inc(wrk, rv, 1);

    app_wrk = application_get_worker(app, mp->wrk_index);
    app_worker_listened_notify(app_wrk, a->handle, mp->context, rv);

    if (mp->ext_config) session_mq_free_ext_config(app, mp->ext_config);
...
static void session_mq_listen_uri_handler(session_worker_t *wrk, session_evt_elt_t *elt)
{
    vnet_listen_args_t        _a, *a = &_a;
...
static void session_mq_handle_connects_rpc(void *arg)
{
    ...
    if (!fwrk->n_pending_connects) return;
    ...
    if (fwrk->n_pending_connects > 0) {
        session_send_rpc_evt_to_thread_force(fwrk->vm->thread_index, session_mq_handle_connects_rpc, 0);
    }
}
...
static void session_mq_connect_uri_handler(session_worker_t *wrk, session_evt_elt_t *elt)
{
    vnet_connect_args_t        _a, *a = &_a;
...
static void app_mq_detach_handler(session_worker_t *wrk, session_evt_elt_t *elt)
{
...
static void session_mq_unlisten_handler(session_worker_t *wrk, session_evt_elt_t *elt)
{
    ...
    if ((rv = vnet_unlisten(a))) session_worker_stat_error_inc(wrk, rv, 1);
...
static void session_mq_accepted_reply_handler(session_worker_t *wrk, session_evt_elt_t *elt)
{
    ...
    if (session_thread_from_handle(mp->handle) == 0 && vlib_num_workers() && vlib_get_thread_index() != 0) {
        session_wrk_send_evt_to_main(wrk, elt);
        return;
    }
    ...
    if (!svm_fifo_is_empty_prod(s->rx_fifo)) app_worker_rx_notify(app_wrk, s);
...
static void session_mq_reset_reply_handler(void *data)
{
    ...
    if (!s || s->session_state < SESSION_STATE_TRANSPORT_CLOSING) return;

    app_wrk = app_worker_get(s->app_wrk_index);
    if (!app_wrk || app_wrk->app_index != app->app_index) {
        clib_warning("App %u does not own handle 0x%lx!", app->app_index, mp->handle);
        return;
    }
...
static void session_mq_disconnected_handler(void *data)
{
    ...
    if (!(app_wrk && app && app->app_index == app_wrk->app_index)) {
        clib_warning("could not disconnect session: %llu app: %u", mp->handle, mp->client_index);
        return;
    }
    ...
    svm_msg_q_lock_and_alloc_msg_w_ring(app_wrk->event_queue, SESSION_MQ_CTRL_EVT_RING, SVM_Q_WAIT, msg);
...
static void session_mq_worker_update_handler(void *data)
{
    ...
    if (app_wrk->app_index != app->app_index) {
        clib_warning("app %u does not own session %llu", app->app_index, mp->handle);
        return;
    }
    ...
    if (rv) session_stat_error_inc(rv, 1);
    ...
    rmp->handle     = mp->handle;
    if (s->rx_fifo) rmp->rx_fifo = fifo_segment_fifo_offset(s->rx_fifo);
    if (s->tx_fifo) rmp->tx_fifo = fifo_segment_fifo_offset(s->tx_fifo);
     ...
     */
    if (s->tx_fifo && !svm_fifo_is_empty(s->tx_fifo)) session_send_io_evt_to_thread(s->tx_fifo, SESSION_IO_EVT_TX);

    if (s->rx_fifo && !svm_fifo_is_empty(s->rx_fifo)) app_worker_rx_notify(app_wrk, s);

    if (s->session_state >= SESSION_STATE_TRANSPORT_CLOSING) app_worker_close_notify(app_wrk, s);
}
...
static void session_mq_transport_attr_handler(void *data)
{
    ...
    if (!rv && mp->is_get) rmp->attr = mp->attr;
    svm_msg_q_add_and_unlock(app_wrk->event_queue, msg);
}

void session_wrk_handle_evts_main_rpc(void *args)
{
...
static u8 *format_session_queue_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "session index %d thread index %d", t->session_index, t->server_thread_index);
    return s;
}
...
static void
session_tx_trace_frame(vlib_main_t *vm, vlib_node_runtime_t *node, u32 next_index, vlib_buffer_t **bufs, u16 n_segs, session_t *s, u32 n_trace)
{
    vlib_buffer_t **b = bufs;

    while (n_trace && n_segs) {
        if (PREDICT_TRUE(vlib_trace_buffer(vm, node, next_index, b[0], 1 /* follow_chain */))) {
            session_queue_trace_t *t = vlib_add_trace(vm, node, b[0], sizeof(*t));
...
always_inline int session_tx_fill_dma_transfers(session_worker_t *wrk, session_tx_context_t *ctx, vlib_buffer_t *b)
{
    ...
    n_bytes_read = svm_fifo_segments(ctx->s->tx_fifo, ctx->sp.tx_offset, data_fs, &n_segs, len_to_deq);
...
always_inline int session_tx_fill_dma_transfers_tail(session_worker_t *wrk, session_tx_context_t *ctx, vlib_buffer_t *b, u32 len_to_deq, u8 *data)
{
    vlib_main_t   *vm = wrk->vm;
    int            n_bytes_read, len_write;
    svm_fifo_seg_t data_fs[2];
    ...
    while (n_bytes_read) {
        wrk->batch_num++;
        vlib_dma_batch_add(vm, wrk->batch, data, data_fs[n_transfers].data, data_fs[n_transfers].len);
...
always_inline int session_tx_copy_data(session_worker_t *wrk, session_tx_context_t *ctx, vlib_buffer_t *b, u32 len_to_deq, u8 *data0)
{
    int n_bytes_read;
    if (PREDICT_TRUE(!wrk->dma_enabled))
        n_bytes_read = svm_fifo_peek(ctx->s->tx_fifo, ctx->sp.tx_offset, len_to_deq, data0);
    else
        n_bytes_read = session_tx_fill_dma_transfers(wrk, ctx, b);
    return n_bytes_read;
}

always_inline int session_tx_copy_data_tail(session_worker_t *wrk, session_tx_context_t *ctx, vlib_buffer_t *b, u32 len_to_deq, u8 *data)
{
    int n_bytes_read;
    if (PREDICT_TRUE(!wrk->dma_enabled))
        n_bytes_read = svm_fifo_peek(ctx->s->tx_fifo, ctx->sp.tx_offset, len_to_deq, data);
    else
        n_bytes_read = session_tx_fill_dma_transfers_tail(wrk, ctx, b, len_to_deq, data);
    return n_bytes_read;
}

always_inline void session_tx_fifo_chain_tail(session_worker_t *wrk, session_tx_context_t *ctx, vlib_buffer_t *b, u16 *n_bufs, u8 peek_data)
{
    vlib_main_t   *vm = wrk->vm;
    ...
    chain_b       = b;
    left_from_seg = clib_min(ctx->sp.snd_mss - b->current_length, ctx->left_to_snd);
    ...
    for (j = 1; j < ctx->n_bufs_per_seg; j++) {
        ...
        *n_bufs -= 1;
        ...
        if (peek_data) {
            n_bytes_read = session_tx_copy_data_tail(wrk, ctx, b, len_to_deq, data);
            ...
            if (ctx->transport_vft->transport_options.tx_type == TRANSPORT_TX_DGRAM) {
                ...
                if (hdr->data_offset == hdr->data_length) {
                    ...
                    if (ctx->left_to_snd > n_bytes_read) svm_fifo_peek(ctx->s->tx_fifo, 0, sizeof(ctx->hdr), (u8 *) &ctx->hdr);
                }
                else if (ctx->left_to_snd == n_bytes_read)
                    svm_fifo_overwrite_head(ctx->s->tx_fifo, (u8 *) &ctx->hdr, sizeof(session_dgram_pre_hdr_t));
            }
            else
                n_bytes_read = svm_fifo_dequeue(ctx->s->tx_fifo, len_to_deq, data);
        }
        ASSERT(n_bytes_read == len_to_deq);
    ...
    ASSERT(to_deq == 0 && b->total_length_not_including_first_buffer == left_from_seg);
    ctx->left_to_snd -= left_from_seg;
}

always_inline void session_tx_fill_buffer(session_worker_t *wrk, session_tx_context_t *ctx, vlib_buffer_t *b, u16 *n_bufs, u8 peek_data)
{
    u32 len_to_deq;
    u8 *data0;
    int n_bytes_read;
    ...
    if (peek_data) {
        ...
        if (ctx->transport_vft->transport_options.tx_type == TRANSPORT_TX_DGRAM) {
            ...
            if (transport_connection_is_cless(ctx->tc)) {
                clib_memcpy_fast(data0 - sizeof(session_dgram_hdr_t), hdr, sizeof(*hdr));
            }
            hdr->data_offset += n_bytes_read;
            if (hdr->data_offset == hdr->data_length) {
                offset = hdr->data_length + SESSION_CONN_HDR_LEN;
            ...
            n_bytes_read = svm_fifo_dequeue(ctx->s->tx_fifo, len_to_deq, data0);
            ASSERT(n_bytes_read > 0);
        }
    }
     ...
     */
    if (PREDICT_FALSE(ctx->n_bufs_per_seg > 1 && ctx->left_to_snd)) session_tx_fifo_chain_tail(wrk, ctx, b, n_bufs, peek_data);
}

always_inline u8 session_tx_not_ready(session_t *s, u8 peek_data)
{
    if (peek_data) {
        if (PREDICT_TRUE(s->session_state == SESSION_STATE_READY)) return 0;
         ...
         * session is not ready or closed */
            ...
            if (s->session_state == SESSION_STATE_ACCEPTING && (s->flags & SESSION_F_CUSTOM_TX)) return 0;
            return 1;
        }
            ...
            if (s->session_state != SESSION_STATE_TRANSPORT_DELETED && (s->flags & SESSION_F_CUSTOM_TX)) return 0;
            return 2;
        }
    }
    else {
        if (s->session_state == SESSION_STATE_TRANSPORT_DELETED) return 2;
    }
    return 0;
}

always_inline transport_connection_t *session_tx_get_transport(session_tx_context_t *ctx, u8 peek_data)
{
    if (peek_data) {
        return ctx->transport_vft->get_connection(ctx->s->connection_index, ctx->s->thread_index);
    }
    else {
...
always_inline void session_tx_set_dequeue_params(vlib_main_t *vm, session_tx_context_t *ctx, u32 max_segs, u8 peek_data)
{
    ...
    if (peek_data) {
        ...
        if (ctx->transport_vft->transport_options.tx_type == TRANSPORT_TX_DGRAM) {
            ...
            svm_fifo_peek(ctx->s->tx_fifo, 0, sizeof(ctx->hdr), (u8 *) &ctx->hdr);
            ...
            if (PREDICT_FALSE(ctx->max_dequeue < ctx->hdr.data_length + sizeof(ctx->hdr))) {
                ctx->max_len_to_snd = 0;
                return;
            }
             ...
             * chains */
            chain_limit = clib_min(n_bytes_per_buf - TRANSPORT_MAX_HDRS_LEN, ctx->sp.snd_mss);
            if (ctx->hdr.data_length <= chain_limit) {
                ...
                while (offset < max_offset) {
                    svm_fifo_peek(ctx->s->tx_fifo, offset, sizeof(ctx->hdr), (u8 *) &hdr);
                    dgram_len = hdr.data_length - hdr.data_offset;
                    if (offset + sizeof(hdr) + hdr.data_length > ctx->max_dequeue || first_dgram_len != dgram_len) break;
    ...
    if (ctx->max_dequeue < ctx->sp.snd_space) {
        ...
        ctx->max_len_to_snd = (ctx->max_dequeue > ctx->sp.snd_mss) ? (ctx->max_dequeue - (ctx->max_dequeue % ctx->sp.snd_mss)) : ctx->max_dequeue;
        /* TODO Nagle ? */
    }
    else {
    ...
    if (ctx->n_segs_per_evt > 1) {
        ...
        n_bytes_last_seg    = TRANSPORT_MAX_HDRS_LEN + ctx->max_len_to_snd - ((ctx->n_segs_per_evt - 1) * ctx->sp.snd_mss);
        ctx->n_bufs_per_seg = ceil((f64) n_bytes_per_seg / n_bytes_per_buf);
        n_bufs_last_seg     = ceil((f64) n_bytes_last_seg / n_bytes_per_buf);
        ctx->n_bufs_needed  = ((ctx->n_segs_per_evt - 1) * ctx->n_bufs_per_seg) + n_bufs_last_seg;
    }
    else {
    ...
    ctx->deq_per_first_buf = clib_min(ctx->sp.snd_mss, n_bytes_per_buf - TRANSPORT_MAX_HDRS_LEN);
}

always_inline void session_tx_maybe_reschedule(session_worker_t *wrk, session_tx_context_t *ctx, session_evt_elt_t *elt)
{
    session_t *s = ctx->s;

    svm_fifo_unset_event(s->tx_fifo);
    if (svm_fifo_max_dequeue_cons(s->tx_fifo) > ctx->sp.tx_offset) {
        if (svm_fifo_set_event(s->tx_fifo)) session_evt_add_head_old(wrk, elt);
    }
    else {
...
always_inline void session_tx_add_pending_buffer(session_worker_t *wrk, u32 bi, u32 next_index)
{
    if (PREDICT_TRUE(!wrk->dma_enabled)) {
...
always_inline int session_tx_fifo_read_and_snd_i(session_worker_t    *wrk,
                                                 ...
                                                 u8                   peek_data)
{
    ...
    if (PREDICT_FALSE((rv = session_tx_not_ready(ctx->s, peek_data)))) {
        if (rv < 2) session_evt_add_old(wrk, elt);
    ...
    if (PREDICT_FALSE(e->event_type == SESSION_IO_EVT_TX_FLUSH)) {
        if (ctx->transport_vft->flush_data) ctx->transport_vft->flush_data(ctx->tc);
    ...
    if (ctx->s->flags & SESSION_F_CUSTOM_TX) {
        ...
        *n_tx_packets += n_custom_tx;
        if (PREDICT_FALSE(ctx->s->session_state >= SESSION_STATE_TRANSPORT_CLOSED)) {
    ...
    if (transport_connection_is_descheduled(ctx->tc)) transport_connection_clear_descheduled(ctx->tc);
    ...
    if (!ctx->sp.snd_space) {
        ...
        if (ctx->sp.flags & TRANSPORT_SND_F_DESCHED) transport_connection_deschedule(ctx->tc);
    ...
    if (transport_connection_is_tx_paced(ctx->tc)) {
        ...
        ctx->sp.snd_space = snd_space >= ctx->sp.snd_mss ? snd_space - snd_space % ctx->sp.snd_mss : snd_space;
    }
    ...
    vec_validate_aligned(ctx->tx_buffers, ctx->n_bufs_needed - 1, CLIB_CACHE_LINE_BYTES);
    n_bufs = vlib_buffer_alloc(vm, ctx->tx_buffers, ctx->n_bufs_needed);
    if (PREDICT_FALSE(n_bufs < ctx->n_bufs_needed)) {
        if (n_bufs) vlib_buffer_free(vm, ctx->tx_buffers, n_bufs);
        session_evt_add_head_old(wrk, elt);
        vlib_node_increment_counter(wrk->vm, node->node_index, SESSION_QUEUE_ERROR_NO_BUFFER, 1);
        return SESSION_TX_NO_BUFFERS;
    }

    if (transport_connection_is_tx_paced(ctx->tc)) transport_connection_tx_pacer_update_bytes(ctx->tc, ctx->max_len_to_snd);
    ...
    ctx->transport_vft->push_header(ctx->tc, ctx->transport_pending_bufs, ctx->n_segs_per_evt);

    if (PREDICT_FALSE((n_trace = vlib_get_trace_count(vm, node)) > 0))
        session_tx_trace_frame(vm, node, next_index, ctx->transport_pending_bufs, ctx->n_segs_per_evt, ctx->s, n_trace);

    if (PREDICT_FALSE(n_bufs)) vlib_buffer_free(vm, ctx->tx_buffers, n_bufs);

    *n_tx_packets += ctx->n_segs_per_evt;

    SESSION_EVT(SESSION_EVT_DEQ, ctx->s, ctx->max_len_to_snd, ctx->max_dequeue, ctx->s->tx_fifo->shr->has_event, wrk->last_vlib_time);
    ...
    if (!peek_data) {
        ...
        if (ctx->transport_vft->transport_options.tx_type == TRANSPORT_TX_DGRAM) n_dequeued += ctx->n_segs_per_evt * SESSION_CONN_HDR_LEN;
        if (svm_fifo_needs_deq_ntf(ctx->s->tx_fifo, n_dequeued)) session_dequeue_notify(ctx->s);
    }
    return SESSION_TX_OK;
}
...
int session_tx_fifo_dequeue_internal(session_worker_t *wrk, vlib_node_runtime_t *node, session_evt_elt_t *elt, int *n_tx_packets)
{
    ...
    if (PREDICT_FALSE((s->session_state >= SESSION_STATE_TRANSPORT_CLOSED)
                      || (s->session_state == SESSION_STATE_CONNECTING && (s->flags & SESSION_F_HALF_OPEN))))
        return 0;
    ...
    sp->flags          = 0;
    sp->bytes_dequeued = 0;
    sp->max_burst_size = clib_min(SESSION_NODE_FRAME_SIZE - *n_tx_packets, TRANSPORT_PACER_MAX_BURST_PKTS);
    ...
    *n_tx_packets += n_packets;
    ...
    if (sp->bytes_dequeued && svm_fifo_needs_deq_ntf(s->tx_fifo, sp->bytes_dequeued)) session_dequeue_notify(s);

    return n_packets;
}

always_inline session_t *session_event_get_session(session_worker_t *wrk, session_event_t *e)
{
    if (PREDICT_FALSE(pool_is_free_index(wrk->sessions, e->session_index))) return 0;

    ASSERT(session_is_valid(e->session_index, wrk->vm->thread_index));
    return pool_elt_at_index(wrk->sessions, e->session_index);
}

always_inline void session_event_dispatch_ctrl(session_worker_t *wrk, session_evt_elt_t *elt)
{
    clib_llist_index_t ei;
    void (*fp)(void *);
    session_event_t *e;
    ...
    if (!clib_llist_elt_is_linked(elt, evt_list)) {
        e = &elt->evt;
        if (e->event_type >= SESSION_CTRL_EVT_BOUND) session_evt_ctrl_data_free(wrk, elt);
        clib_llist_put(wrk->event_elts, elt);
    }
    SESSION_EVT(SESSION_EVT_COUNTS, CNT_CTRL_EVTS, 1, wrk);
}

always_inline void session_event_dispatch_io(session_worker_t *wrk, vlib_node_runtime_t *node, session_evt_elt_t *elt, int *n_tx_packets)
{
    session_main_t    *smm = &session_main;
    app_worker_t      *app_wrk;
    clib_llist_index_t ei;
    ...
    switch (e->event_type) {
        ...
        case SESSION_IO_EVT_RX:
            s = session_event_get_session(wrk, e);
            if (!s || s->session_state >= SESSION_STATE_TRANSPORT_CLOSED) break;
            transport_app_rx_evt(session_get_transport_proto(s), s->connection_index, s->thread_index);
            break;
        case SESSION_IO_EVT_BUILTIN_RX:
            s = session_event_get_session(wrk, e);
            if (PREDICT_FALSE(!s || s->session_state >= SESSION_STATE_CLOSING)) break;
        ...
        case SESSION_IO_EVT_TX_MAIN:
            ...
            if (PREDICT_FALSE(!s)) break;
            wrk->ctx.s = s;
            if (PREDICT_TRUE(s != 0)) (smm->session_tx_fns[s->session_type])(wrk, node, elt, n_tx_packets);
            break;
        default:
            clib_warning("unhandled event type %d", e->event_type);
    }
    ...
    /* Regrab elements in case pool moved */
    elt = clib_llist_elt(wrk->event_elts, ei);
    if (!clib_llist_elt_is_linked(elt, evt_list)) clib_llist_put(wrk->event_elts, elt);
}

static const u32 session_evt_msg_sizes[] = {
#define _(symc, sym) [SESSION_CTRL_EVT_##symc] = sizeof(session_##sym##_msg_t),
    foreach_session_ctrl_evt
#undef _
};

always_inline void session_update_time_subscribers(session_main_t *smm, clib_time_type_t now, u32 thread_index)
{
...
always_inline void session_evt_add_to_list(session_worker_t *wrk, session_event_t *evt)
{
    session_evt_elt_t *elt;
    ...
    if (evt->event_type >= SESSION_CTRL_EVT_RPC) {
        ...
        if (evt->event_type >= SESSION_CTRL_EVT_BOUND) {
            ...
            clib_memcpy_fast(session_evt_ctrl_data(wrk, elt), evt->data, session_evt_msg_sizes[evt->event_type]);
        }
        else {
...
{
    vlib_buffer_enqueue_to_next_vec(wrk->vm, node, &wrk->pending_tx_buffers, &wrk->pending_tx_nexts, vec_len(wrk->pending_tx_nexts));
...
static void session_wrk_update_state(session_worker_t *wrk)
{
    ...
    if (wrk->state == SESSION_WRK_POLLING) {
        if (clib_llist_elts(wrk->event_elts) == 5 && vlib_last_vectors_per_main_loop(vm) < 1) {
            session_wrk_set_state(wrk, SESSION_WRK_INTERRUPT);
            vlib_node_set_state(vm, session_queue_node.index, VLIB_NODE_STATE_INTERRUPT);
        }
    }
...
static uword session_queue_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    if (PREDICT_FALSE(wrk->dma_enabled)) {
        if (wrk->trans_head == ((wrk->trans_tail + 1) & (wrk->trans_size - 1))) return 0;
        wrk->batch = vlib_dma_batch_new(vm, wrk->config_index);
        if (!wrk->batch) return 0;
    }
    ...
    if (PREDICT_FALSE(wrk->dma_enabled)) {
        if (wrk->batch_num) {
            ...
            wrk->batch_num = 0;
            wrk->trans_tail++;
            if (wrk->trans_tail == wrk->trans_size) wrk->trans_tail = 0;
        }
    ...
    if (vec_len(wrk->pending_tx_buffers)) session_flush_pending_tx_buffers(wrk, node);
    ...
    if (wrk->flags & SESSION_WRK_F_ADAPTIVE) session_wrk_update_state(wrk);
...
static clib_error_t *session_wrk_tfd_read_ready(clib_file_t *cf)
{
    ...
    if (rv < 0 && errno != EAGAIN) clib_unix_warning("failed");
    return 0;
}
...
void session_wrk_enable_adaptive_mode(session_worker_t *wrk)
{
    ...
    if ((wrk->timerfd = timerfd_create(CLOCK_MONOTONIC, TFD_NONBLOCK)) < 0) clib_warning("timerfd_create");
...
{
    if (vlib_get_n_threads() < 2) return 0;
    ...
    while (1) {
        ...
        switch (event_type) {
            ...
            case SESSION_Q_PROCESS_STOP:
                ...
                vec_free(event_data);
                vlib_node_set_state(vm, session_queue_process_node.index, VLIB_NODE_STATE_DISABLED);
                timeout = 100000.0;
                break;
            case ~0:
...
static_always_inline uword session_queue_pre_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    if (!sm->wrk[0].vpp_event_queue) return 0;
    node = vlib_node_get_runtime(vm, session_queue_node.index);
