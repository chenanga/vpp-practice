...
{
    if (pool_is_free_index(app_workers, wrk_index)) return 0;
    return pool_elt_at_index(app_workers, wrk_index);
}
...
void app_worker_free(app_worker_t *app_wrk)
{
    ...
    if (CLIB_DEBUG) clib_memset(app_wrk, 0xfe, sizeof(*app_wrk));
...
static int app_worker_alloc_session_fifos(segment_manager_t *sm, session_t *s)
{
    ...
    if ((rv = segment_manager_alloc_session_fifos(sm, s->thread_index, &rx_fifo, &tx_fifo))) return rv;
...
int app_worker_alloc_wrk_cl_session(app_worker_t *app_wrk, session_t *ls)
{
    svm_fifo_t        *rx_fifo = 0, *tx_fifo = 0;
    segment_manager_t *sm;
...
void app_worker_free_wrk_cl_session(app_worker_t *app_wrk, session_t *ls)
{
    app_listener_t *al;
    session_t      *s;
...
int app_worker_init_listener(app_worker_t *app_wrk, session_t *ls)
{
    ...
    if (!(sm = app_worker_alloc_segment_manager(app_wrk))) return SESSION_E_ALLOC;
    ...
    if (ls->flags & SESSION_F_IS_CLESS) return app_worker_alloc_wrk_cl_session(app_wrk, ls);
...
session_error_t app_worker_start_listen(app_worker_t *app_wrk, app_listener_t *app_listener)
{
    ...
    if (clib_bitmap_get(app_listener->workers, app_wrk->wrk_map_index)) return SESSION_E_ALREADY_LISTENING;

    app_listener->workers = clib_bitmap_set(app_listener->workers, app_wrk->wrk_map_index, 1);

    if (app_listener->session_index != SESSION_INVALID_INDEX) {
        ls = session_get(app_listener->session_index, 0);
        if ((rv = app_worker_init_listener(app_wrk, ls))) return rv;
    }
...
static void app_worker_stop_listen_session(app_worker_t *app_wrk, session_t *ls)
{
    ...
    if (PREDICT_FALSE(!sm_indexp)) return;

    if (ls->flags & SESSION_F_IS_CLESS) app_worker_free_wrk_cl_session(app_wrk, ls);
...
int app_worker_stop_listen(app_worker_t *app_wrk, app_listener_t *al)
{
    ...
    if (!clib_bitmap_get(al->workers, app_wrk->wrk_map_index)) return 0;
    ...
    if (clib_bitmap_is_zero(al->workers)) app_listener_cleanup(al);
...
int app_worker_init_accepted(session_t *s)
{
    ...
    if (PREDICT_FALSE(app_worker_mq_is_congested(app_wrk))) return -1;

    s->app_wrk_index = app_wrk->wrk_index;
    app              = application_get(app_wrk->app_index);
    if (app->cb_fns.fifo_tuning_callback) s->flags |= SESSION_F_CUSTOM_FIFO_TUNING;

    sm = app_worker_get_listen_segment_manager(app_wrk, listener);
    if (app_worker_alloc_session_fifos(sm, s)) return -1;

    return 0;
}

int app_worker_listened_notify(app_worker_t *app_wrk, session_handle_t alsh, u32 opaque, session_error_t err)
{
    session_event_t evt = {.event_type = SESSION_CTRL_EVT_BOUND, .as_u64[0] = alsh, .as_u64[1] = (u64) opaque << 32 | (u32) err};
...
int app_worker_unlisten_reply(app_worker_t *app_wrk, session_handle_t sh, u32 opaque, session_error_t err)
{
    session_event_t evt = {.event_type = SESSION_CTRL_EVT_UNLISTEN_REPLY, .as_u64[0] = sh, .as_u64[1] = (u64) opaque << 32 | (u32) err};
...
int app_worker_init_connected(app_worker_t *app_wrk, session_t *s)
{
    ...
    if (application_is_builtin_proxy(app)) return app->cb_fns.proxy_alloc_session_fifos(s);
...
int app_worker_cleanup_notify_custom(app_worker_t *app_wrk, session_t *s, session_cleanup_ntf_t ntf, void (*cleanup_cb)(session_t *s))
{
    session_event_t evt = {.event_type = SESSION_CTRL_EVT_CLEANUP,
                           .as_u64[0]  = (u64) ntf << 32 | s->session_index,
...
{
    session_event_t evt = {.event_type = SESSION_CTRL_EVT_MIGRATED, .as_u64[0] = s->session_index, .as_u64[1] = new_sh};
...
int app_worker_own_session(app_worker_t *app_wrk, session_t *s)
{
    ...
    if (s->session_state == SESSION_STATE_LISTENING) return application_change_listener_owner(s, app_wrk);
    ...
    sm = app_worker_get_connect_segment_manager(app_wrk);
    if ((rv = app_worker_alloc_session_fifos(sm, s))) return rv;

    if (!svm_fifo_is_empty_cons(rxf)) svm_fifo_clone(s->rx_fifo, rxf);

    if (!svm_fifo_is_empty_cons(txf)) svm_fifo_clone(s->tx_fifo, txf);
...
int app_worker_connect_session(app_worker_t *app_wrk, session_endpoint_cfg_t *sep, session_handle_t *rsh)
{
    if (PREDICT_FALSE(app_worker_mq_is_congested(app_wrk))) return SESSION_E_REFUSED;
...
session_t *app_worker_first_listener(app_worker_t *app_wrk, u8 fib_proto, u8 transport_proto)
{
    ...
    hash_foreach(handle, sm_index, app_wrk->listeners_table, ({
                     ...
                     if (listener->session_type == sst && !(listener->flags & SESSION_F_PROXY)) return listener;
                 }));
...
{
    session_event_t evt = {.event_type = SESSION_CTRL_EVT_APP_ADD_SEGMENT, .as_u64[1] = segment_handle};
...
{
    session_event_t evt = {.event_type = SESSION_CTRL_EVT_APP_DEL_SEGMENT, .as_u64[1] = segment_handle};
...
static int app_wrk_send_fd(app_worker_t *app_wrk, int fd)
{
    if (!appns_sapi_enabled()) {
        ...
        if (vl_api_registration_file_index(reg) == VL_API_INVALID_FI) return -1;
...
void app_worker_add_event(app_worker_t *app_wrk, session_t *s, session_evt_type_t evt_type)
{
    session_event_t *evt;
...
always_inline void app_wrk_send_ctrl_evt_inline(app_worker_t *app_wrk, u8 evt_type, void *msg, u32 msg_len, int fd)
{
...
void app_wrk_send_ctrl_evt_fd(app_worker_t *app_wrk, u8 evt_type, void *msg, u32 msg_len, int fd)
{
    app_wrk_send_ctrl_evt_inline(app_wrk, evt_type, msg, msg_len, fd);
}

void app_wrk_send_ctrl_evt(app_worker_t *app_wrk, u8 evt_type, void *msg, u32 msg_len)
{
...
void app_worker_unset_wrk_mq_congested(app_worker_t *app_wrk, u32 thread_index)
{
...
u8 *format_app_worker_listener(u8 *s, va_list *args)
{
    ...
    if (!app_wrk) {
        if (verbose)
            s = format(s, "%-" SESSION_CLI_ID_LEN "s%-25s%-10s%-15s%-15s%-10s", "Connection", "App", "Wrk", "API Client", "ListenerID", "SegManager");
        else
    ...
    if (verbose) {
        ...
        s   = format(s, "%-" SESSION_CLI_ID_LEN "v%-25v%-10v%-15u%-15u%-10u", str, app_name, buf, app_wrk->api_client_index, handle, sm_index);
        vec_free(buf);
    }
    else
...
void app_worker_format_connects(app_worker_t *app_wrk, int verbose)
{
    ...
    if (app_wrk->connects_seg_manager == (u32) ~0) return;
