...
typedef struct session_cb_vft_ {
    ...
    int (*session_listened_callback)(u32 app_wrk_index, u32 api_context, session_handle_t handle, int rv);
...
static inline void app_alloc_ctrl_evt_to_vpp(svm_msg_q_t *mq, app_session_evt_t *app_evt, u8 evt_type)
{
...
static inline void app_send_ctrl_evt_to_vpp(svm_msg_q_t *mq, app_session_evt_t *app_evt)
{
 ...
 */
static inline int app_send_io_evt_to_vpp(svm_msg_q_t *mq, u32 session_index, u8 evt_type, u8 noblock)
{
    session_event_t *evt;
    svm_msg_q_msg_t  msg;

    if (noblock) {
        if (svm_msg_q_try_lock(mq)) return -1;
...
always_inline int app_send_dgram_raw_gso(svm_fifo_t              *f,
                                         ...
                                         u8                       noblock)
{
    ...
    int                 rv;
    if (svm_fifo_max_enqueue_prod(f) < (sizeof(session_dgram_hdr_t) + len)) return 0;
    ...
    if (do_evt) {
        if (svm_fifo_set_event(f)) app_send_io_evt_to_vpp(vpp_evt_q, f->shr->master_session_index, evt_type, noblock);
    }
    return len;
}

always_inline int app_send_dgram(app_session_t *s, u8 *data, u32 len, u8 noblock)
{
    return app_send_dgram_raw(s->tx_fifo, &s->transport, s->vpp_evt_q, data, len, SESSION_IO_EVT_TX, 1 /* do_evt */, noblock);
}

always_inline int app_send_stream_raw(svm_fifo_t *f, svm_msg_q_t *vpp_evt_q, u8 *data, u32 len, u8 evt_type, u8 do_evt, u8 noblock)
{
    int rv;

    rv = svm_fifo_enqueue(f, len, data);
    if (do_evt) {
        if (rv > 0 && svm_fifo_set_event(f)) app_send_io_evt_to_vpp(vpp_evt_q, f->shr->master_session_index, evt_type, noblock);
    }
    return rv;
}

always_inline int app_send_stream(app_session_t *s, u8 *data, u32 len, u8 noblock)
{
    return app_send_stream_raw(s->tx_fifo, s->vpp_evt_q, data, len, SESSION_IO_EVT_TX, 1 /* do_evt */, noblock);
}

always_inline int app_send(app_session_t *s, u8 *data, u32 len, u8 noblock)
{
    if (s->is_dgram) return app_send_dgram(s, data, len, noblock);
    return app_send_stream(s, data, len, noblock);
}

always_inline int app_recv_dgram_raw(svm_fifo_t *f, u8 *buf, u32 len, app_session_transport_t *at, u8 clear_evt, u8 peek)
{
    ...
    if (max_deq <= sizeof(session_dgram_hdr_t)) {
        if (clear_evt) svm_fifo_unset_event(f);
        return 0;
    }
    ...
    if (max_deq < (ph.data_length + SESSION_CONN_HDR_LEN) && len >= ph.data_length) return 0;
...
always_inline int app_recv_dgram(app_session_t *s, u8 *buf, u32 len)
{
    return app_recv_dgram_raw(s->rx_fifo, buf, len, &s->transport, 1, 0);
}

always_inline int app_recv_stream_raw(svm_fifo_t *f, u8 *buf, u32 len, u8 clear_evt, u8 peek)
{
    if (clear_evt) svm_fifo_unset_event(f);

    if (peek) return svm_fifo_peek(f, 0, len, buf);

    return svm_fifo_dequeue(f, len, buf);
}

always_inline int app_recv_stream(app_session_t *s, u8 *buf, u32 len)
{
    return app_recv_stream_raw(s->rx_fifo, buf, len, 1, 0);
}

always_inline int app_recv(app_session_t *s, u8 *data, u32 len)
{
    if (s->is_dgram) return app_recv_dgram(s, data, len);
...
static inline u8 *format_session_error(u8 *s, va_list *args)
{
...
STATIC_ASSERT(sizeof(u64) * APP_OPTIONS_N_OPTIONS <= sizeof(((app_sapi_attach_msg_t *) 0)->options), "Out of options, fix message definition");
...
static inline void session_endpoint_alloc_ext_cfg(session_endpoint_cfg_t *sep_ext, transport_endpt_ext_cfg_type_t type)
{
