...
always_inline void make_v4_ss_kv(session_kv4_t *kv, ip4_address_t *lcl, ip4_address_t *rmt, u16 lcl_port, u16 rmt_port, u8 proto)
{
...
always_inline void make_v4_listener_kv(session_kv4_t *kv, ip4_address_t *lcl, u16 lcl_port, u8 proto)
{
...
always_inline void make_v4_proxy_kv(session_kv4_t *kv, ip4_address_t *lcl, u8 proto)
{
    kv->key[0] = (u64) lcl->as_u32;
...
always_inline void make_v4_ss_kv_from_tc(session_kv4_t *kv, transport_connection_t *tc)
{
    make_v4_ss_kv(kv, &tc->lcl_ip.ip4, &tc->rmt_ip.ip4, tc->lcl_port, tc->rmt_port, tc->proto);
}

always_inline void make_v6_ss_kv(session_kv6_t *kv, ip6_address_t *lcl, ip6_address_t *rmt, u16 lcl_port, u16 rmt_port, u8 proto)
{
...
always_inline void make_v6_listener_kv(session_kv6_t *kv, ip6_address_t *lcl, u16 lcl_port, u8 proto)
{
    kv->key[0] = lcl->as_u64[0];
    kv->key[1] = lcl->as_u64[1];
    kv->key[2] = 0;
...
always_inline void make_v6_proxy_kv(session_kv6_t *kv, ip6_address_t *lcl, u8 proto)
{
    kv->key[0] = lcl->as_u64[0];
    kv->key[1] = lcl->as_u64[1];
    kv->key[2] = 0;
...
always_inline void make_v6_ss_kv_from_tc(session_kv6_t *kv, transport_connection_t *tc)
{
    make_v6_ss_kv(kv, &tc->lcl_ip.ip6, &tc->rmt_ip.ip6, tc->lcl_port, tc->rmt_port, tc->proto);
}

static inline u8 session_table_alloc_needs_sync(void)
{
    return !vlib_thread_is_main_w_barrier() && (vlib_num_workers() > 1);
}

static_always_inline u8 session_table_is_alloced(u8 fib_proto, u32 fib_index)
{
    return (vec_len(fib_index_to_table_index[fib_proto]) > fib_index && fib_index_to_table_index[fib_proto][fib_index] != ~0);
}
...
static session_table_t *session_table_get_or_alloc(u8 fib_proto, u32 fib_index)
{
    ...
    if (session_table_is_alloced(fib_proto, fib_index)) {
        ...
        vec_validate_init_empty(fib_index_to_table_index[fib_proto], fib_index, ~0);
...
static session_table_t *session_table_get_for_connection(transport_connection_t *tc)
{
    ...
    if (vec_len(fib_index_to_table_index[fib_proto]) <= tc->fib_index) return 0;
    return session_table_get(fib_index_to_table_index[fib_proto][tc->fib_index]);
}

session_table_t *session_table_get_for_fib_index(u32 fib_proto, u32 fib_index)
{
    if (vec_len(fib_index_to_table_index[fib_proto]) <= fib_index) return 0;
    return session_table_get(fib_index_to_table_index[fib_proto][fib_index]);
}

u32 session_lookup_get_index_for_fib(u32 fib_proto, u32 fib_index)
{
    if (vec_len(fib_index_to_table_index[fib_proto]) <= fib_index) return SESSION_TABLE_INVALID_INDEX;
...
int session_lookup_add_connection(transport_connection_t *tc, u64 value)
{
    ...
    if (tc->is_ip4) {
        ...
        kv4.value = value;
        return clib_bihash_add_del_16_8(&st->v4_session_hash, &kv4, 1 /* is_add */);
    }
    else {
        make_v6_ss_kv_from_tc(&kv6, tc);
        kv6.value = value;
        return clib_bihash_add_del_48_8(&st->v6_session_hash, &kv6, 1 /* is_add */);
    }
}
...
int session_lookup_del_connection(transport_connection_t *tc)
{
    ...
    if (tc->is_ip4) {
        make_v4_ss_kv_from_tc(&kv4, tc);
        return clib_bihash_add_del_16_8(&st->v4_session_hash, &kv4, 0 /* is_add */);
    }
    else {
        make_v6_ss_kv_from_tc(&kv6, tc);
        return clib_bihash_add_del_48_8(&st->v6_session_hash, &kv6, 0 /* is_add */);
    }
}
...
int session_lookup_del_session(session_t *s)
{
    ...
    ts = transport_get_connection(session_get_transport_proto(s), s->connection_index, s->thread_index);
    if (!ts || (ts->flags & TRANSPORT_CONNECTION_F_NO_LOOKUP)) return 0;
    return session_lookup_del_connection(ts);
}

static u8 session_lookup_action_index_is_valid(u32 action_index)
{
    if (action_index == SESSION_RULES_TABLE_ACTION_ALLOW || action_index == SESSION_RULES_TABLE_INVALID_INDEX) return 0;
    return 1;
}
...
static session_t *session_lookup_action_to_session(u32 action_index, u8 fib_proto, u8 transport_proto)
{
    ...
    return session_lookup_app_listen_session(app_index, fib_proto, transport_proto);
}
...
session_t *session_lookup_rules_table_session4(session_table_t *st, u8 proto, ip4_address_t *lcl, u16 lcl_port, ip4_address_t *rmt, u16 rmt_port)
{
    ...
    action_index = session_rules_table_lookup4(st->srtg_handle, proto, lcl, rmt, lcl_port, rmt_port);
    app_index    = session_lookup_action_to_handle(action_index);
    /* Nothing sophisticated for now, action index is app index */
    return session_lookup_app_listen_session(app_index, FIB_PROTOCOL_IP4, proto);
}
...
session_t *session_lookup_rules_table_session6(session_table_t *st, u8 proto, ip6_address_t *lcl, u16 lcl_port, ip6_address_t *rmt, u16 rmt_port)
{
    ...
    action_index = session_rules_table_lookup6(st->srtg_handle, proto, lcl, rmt, lcl_port, rmt_port);
    app_index    = session_lookup_action_to_handle(action_index);
    return session_lookup_app_listen_session(app_index, FIB_PROTOCOL_IP6, proto);
}
...
u64 session_lookup_endpoint_listener(u32 table_index, session_endpoint_t *sep, u8 use_rules)
{
    ...
    st = session_table_get(table_index);
    if (!st) return SESSION_INVALID_HANDLE;
    if (sep->is_ip4) {
        session_kv4_t kv4;
        ip4_address_t lcl4;
        ...
        if (rv == 0) return kv4.value;
        if (use_rules) {
            if (st->srtg_handle == SESSION_SRTG_HANDLE_INVALID) return SESSION_INVALID_HANDLE;
            clib_memset(&lcl4, 0, sizeof(lcl4));
            ai = session_rules_table_lookup4(st->srtg_handle, sep->transport_proto, &lcl4, &sep->ip.ip4, 0, sep->port);
            if (session_lookup_action_index_is_valid(ai)) return session_lookup_action_to_handle(ai);
        }
    }
    else {
        session_kv6_t kv6;
        ...
        if (rv == 0) return kv6.value;

        if (use_rules) {
            if (st->srtg_handle == SESSION_SRTG_HANDLE_INVALID) return SESSION_INVALID_HANDLE;
            clib_memset(&lcl6, 0, sizeof(lcl6));
            ai = session_rules_table_lookup6(st->srtg_handle, sep->transport_proto, &lcl6, &sep->ip.ip6, 0, sep->port);
            if (session_lookup_action_index_is_valid(ai)) return session_lookup_action_to_handle(ai);
        }
    }
    return SESSION_INVALID_HANDLE;
...
static inline session_t *session_lookup_listener4_i(session_table_t *st, ip4_address_t *lcl, u16 lcl_port, u8 proto, u8 use_wildcard)
{
    session_kv4_t kv4;
    int           rv;
    ...
    rv = clib_bihash_search_inline_16_8(&st->v4_session_hash, &kv4);
    if (rv == 0) return listen_session_get((u32) kv4.value);
...
static session_t *session_lookup_listener6_i(session_table_t *st, ip6_address_t *lcl, u16 lcl_port, u8 proto, u8 ip_wildcard)
{
    ...
    rv = clib_bihash_search_inline_48_8(&st->v6_session_hash, &kv6);
    if (rv == 0) return listen_session_get((u32) kv6.value);
...
session_t *session_lookup_listener(u32 table_index, session_endpoint_t *sep)
{
    ...
    if (!st) return 0;
    if (sep->is_ip4)
        return session_lookup_listener4_i(st, &sep->ip.ip4, sep->port, sep->transport_proto, 0);
    else
        return session_lookup_listener6_i(st, &sep->ip.ip6, sep->port, sep->transport_proto, 0);
    return 0;
}
...
session_t *session_lookup_listener_wildcard(u32 table_index, session_endpoint_t *sep)
{
    ...
    if (!st) return 0;
    if (sep->is_ip4)
        return session_lookup_listener4_i(st, &sep->ip.ip4, sep->port, sep->transport_proto, 1 /* use_wildcard */);
    else
        return session_lookup_listener6_i(st, &sep->ip.ip6, sep->port, sep->transport_proto, 1 /* use_wildcard */);
    return 0;
}
...
int session_lookup_add_half_open(transport_connection_t *tc, u64 value)
{
    ...
    if (tc->is_ip4) {
        make_v4_ss_kv_from_tc(&kv4, tc);
        kv4.value = value;
        return clib_bihash_add_del_16_8(&st->v4_half_open_hash, &kv4, 1 /* is_add */);
    }
    else {
        make_v6_ss_kv_from_tc(&kv6, tc);
        kv6.value = value;
        return clib_bihash_add_del_48_8(&st->v6_half_open_hash, &kv6, 1 /* is_add */);
    }
}
...
int session_lookup_del_half_open(transport_connection_t *tc)
{
    ...
    if (tc->is_ip4) {
        make_v4_ss_kv_from_tc(&kv4, tc);
        return clib_bihash_add_del_16_8(&st->v4_half_open_hash, &kv4, 0 /* is_add */);
    }
    else {
        make_v6_ss_kv_from_tc(&kv6, tc);
        return clib_bihash_add_del_48_8(&st->v6_half_open_hash, &kv6, 0 /* is_add */);
    }
}
...
u64 session_lookup_half_open_handle(transport_connection_t *tc)
{
    ...
    st = session_table_get_for_fib_index(transport_connection_fib_proto(tc), tc->fib_index);
    if (!st) return HALF_OPEN_LOOKUP_INVALID_VALUE;
    if (tc->is_ip4) {
        make_v4_ss_kv(&kv4, &tc->lcl_ip.ip4, &tc->rmt_ip.ip4, tc->lcl_port, tc->rmt_port, tc->proto);
        rv = clib_bihash_search_inline_16_8(&st->v4_half_open_hash, &kv4);
        if (rv == 0) return kv4.value;
    }
    else {
        make_v6_ss_kv(&kv6, &tc->lcl_ip.ip6, &tc->rmt_ip.ip6, tc->lcl_port, tc->rmt_port, tc->proto);
                                                      ...
                                                      u8            *result)
{
    ...
    if (rv == 0) {
        ...
        return transport_get_connection(proto, s->connection_index, thread_index);
    }
     ...
     */
    rv = clib_bihash_search_inline_16_8(&st->v4_half_open_hash, &kv4);
    if (rv == 0) return transport_get_half_open(proto, kv4.value & 0xFFFFFFFF);
    ...
    if (st->srtg_handle != SESSION_SRTG_HANDLE_INVALID) {
        ...
        if (session_lookup_action_index_is_valid(action_index)) {
            ...
            if ((s = session_lookup_action_to_session(action_index, FIB_PROTOCOL_IP4, proto)))
    ...
    if (s) return transport_get_listener(proto, s->connection_index);
...
transport_connection_t *session_lookup_connection4(u32 fib_index, ip4_address_t *lcl, ip4_address_t *rmt, u16 lcl_port, u16 rmt_port, u8 proto)
{
    ...
    if (rv == 0) {
        ...
        return transport_get_connection(proto, s->connection_index, s->thread_index);
    }
    ...
    if (st->srtg_handle != SESSION_SRTG_HANDLE_INVALID) {
        ...
        action_index = session_rules_table_lookup4(st->srtg_handle, proto, lcl, rmt, lcl_port, rmt_port);
        if (session_lookup_action_index_is_valid(action_index)) {
            if (action_index == SESSION_RULES_TABLE_ACTION_DROP) return 0;
            if ((s = session_lookup_action_to_session(action_index, FIB_PROTOCOL_IP4, proto)))
                return transport_get_listener(proto, s->connection_index);
            return 0;
        }
...
session_t *session_lookup_safe4(u32 fib_index, ip4_address_t *lcl, ip4_address_t *rmt, u16 lcl_port, u16 rmt_port, u8 proto)
{
    ...
    make_v4_ss_kv(&kv4, lcl, rmt, lcl_port, rmt_port, proto);
    rv = clib_bihash_search_inline_16_8(&st->v4_session_hash, &kv4);
    if (rv == 0) return session_get_from_handle_safe(kv4.value);
    ...
    if (st->srtg_handle != SESSION_SRTG_HANDLE_INVALID) {
        ...
        if (session_lookup_action_index_is_valid(action_index)) {
            if (action_index == SESSION_RULES_TABLE_ACTION_DROP) return 0;
            return session_lookup_action_to_session(action_index, FIB_PROTOCOL_IP4, proto);
        }
    }
     ...
     *  If nothing is found, check if any listener is available
     */
    if ((s = session_lookup_listener4_i(st, lcl, lcl_port, proto, 1))) return s;
                                                      ...
                                                      u8            *result)
{
    ...
    /* Try half-open connections */
    rv = clib_bihash_search_inline_48_8(&st->v6_half_open_hash, &kv6);
    if (rv == 0) return transport_get_half_open(proto, kv6.value & 0xFFFFFFFF);
    ...
    if (st->srtg_handle != SESSION_SRTG_HANDLE_INVALID) {
        ...
        if (session_lookup_action_index_is_valid(action_index)) {
            ...
            if ((s = session_lookup_action_to_session(action_index, FIB_PROTOCOL_IP6, proto)))
                return transport_get_listener(proto, s->connection_index);
            return 0;
        }
    }
...
session_t *session_lookup_safe6(u32 fib_index, ip6_address_t *lcl, ip6_address_t *rmt, u16 lcl_port, u16 rmt_port, u8 proto)
{
    ...
    make_v6_ss_kv(&kv6, lcl, rmt, lcl_port, rmt_port, proto);
    rv = clib_bihash_search_inline_48_8(&st->v6_session_hash, &kv6);
    if (rv == 0) return session_get_from_handle_safe(kv6.value);
    ...
    if (st->srtg_handle != SESSION_SRTG_HANDLE_INVALID) {
        ...
        if (session_lookup_action_index_is_valid(action_index)) {
            if (action_index == SESSION_RULES_TABLE_ACTION_DROP) return 0;
            return session_lookup_action_to_session(action_index, FIB_PROTOCOL_IP6, proto);
        }
    }

    /* If nothing is found, check if any listener is available */
    if ((s = session_lookup_listener6_i(st, lcl, lcl_port, proto, 1))) return s;
    return 0;
}

transport_connection_t *
session_lookup_connection(u32 fib_index, ip46_address_t *lcl, ip46_address_t *rmt, u16 lcl_port, u16 rmt_port, u8 proto, u8 is_ip4)
{
    if (is_ip4)
        return session_lookup_connection4(fib_index, &lcl->ip4, &rmt->ip4, lcl_port, rmt_port, proto);
    else
        return session_lookup_connection6(fib_index, &lcl->ip6, &rmt->ip6, lcl_port, rmt_port, proto);
}
...
session_error_t vnet_session_rule_add_del(session_rule_add_del_args_t *args)
{
    ...
    if (args->scope > 3) return SESSION_E_INVALID;

    if (args->transport_proto != TRANSPORT_PROTO_TCP && args->transport_proto != TRANSPORT_PROTO_UDP) return SESSION_E_INVALID;
    ...
    if ((args->scope & SESSION_RULE_SCOPE_GLOBAL) || args->scope == 0) {
        ...
        if ((rv = session_rules_table_add_del(st->srtg_handle, args->transport_proto, &args->table_args))) return rv;
    }
    if (args->scope & SESSION_RULE_SCOPE_LOCAL) {
        ...
        rv = session_rules_table_add_del(st->srtg_handle, args->transport_proto, &args->table_args);
    }
    return rv;
}

static void session_lookup_fib_table_lock(u32 fib_index, u32 protocol)
{
...
static void session_lookup_fib_table_unlock(u32 fib_index, u32 protocol)
{
...
u8 *format_ip4_session_lookup_kvp(u8 *s, va_list *args)
{
    ...
    if (!is_local) {
        ...
        app_wrk  = app_worker_get(session->app_wrk_index);
        app_name = application_name_from_index(app_wrk->app_index);
        str = format(0, "[%U] %U:%d", format_transport_proto_short, key->proto, format_ip4_address, &key->src, clib_net_to_host_u16(key->src_port));
...
static int ip4_session_table_show(clib_bihash_kv_16_8_t *kvp, void *arg)
{
    ...
    vlib_cli_output(ctx->vm, "%U", format_ip4_session_lookup_kvp, kvp, ctx->is_local);
    return 1;
}
...
void session_lookup_show_table_entries(vlib_main_t *vm, session_table_t *table, u8 type, u8 is_local)
{
    ...
    switch (type) {
            /* main table v4 */
        case 0:
            ip4_session_table_walk(&table->v4_session_hash, ip4_session_table_show, &ctx);
            break;
        default:
...
static clib_error_t *session_rule_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (session_rule_table_is_enabled() == 0) return clib_error_return(0, "session rule table engine is not enabled");
    ...
    if ((rv = vnet_session_rule_add_del(&args))) error = clib_error_return(0, "rule add del returned %u", rv);
...
void session_lookup_dump_rules_table(u32 fib_index, u8 fib_proto, u8 transport_proto)
{
    ...
    if (st == 0) return;
    session_rules_table_cli_dump(vm, st->srtg_handle, transport_proto, fib_proto);
}
...
static clib_error_t *show_session_rules_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (show_one) {
        if (st) session_rules_table_show_rule(vm, st->srtg_handle, transport_proto, &lcl_ip, lcl_port, &rmt_ip, rmt_port, is_ip4);
        goto done;
    }

    vlib_cli_output(vm, "%U rules table", format_transport_proto, transport_proto);
    if (scope == SESSION_RULE_SCOPE_LOCAL) {
        if (st) {
            session_rules_table_cli_dump(vm, st->srtg_handle, transport_proto, FIB_PROTOCOL_IP4);
            session_rules_table_cli_dump(vm, st->srtg_handle, transport_proto, FIB_PROTOCOL_IP6);
        }
    }
    else {
        /*
         ...
         */
        st = session_table_get_for_fib_index(FIB_PROTOCOL_IP4, app_ns->ip4_fib_index);
        if (st) session_rules_table_cli_dump(vm, st->srtg_handle, transport_proto, FIB_PROTOCOL_IP4);

        st = session_table_get_for_fib_index(FIB_PROTOCOL_IP6, app_ns->ip6_fib_index);
        if (st) session_rules_table_cli_dump(vm, st->srtg_handle, transport_proto, FIB_PROTOCOL_IP6);
    }
done:
    vec_free(ns_id);
    return 0;
...
u8 *format_session_lookup_tables(u8 *s, va_list *args)
{
    ...
    s      = format(s, "%Umax fib-index:\t%u\n", format_white_space, indent, vec_len(fibs) - 1);
    s      = format(s, "%Utable memory:\t%U\n", format_white_space, indent, format_memory_size, total_mem);
    s      = format(s, "%Uvec memory:\t%U\n", format_white_space, indent, format_memory_size, vec_mem_size(fibs));

    return s;
}

static clib_error_t *show_session_lookup_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    session_table_t *st;
    u32              fib_index = ~0;
    ...
    vlib_cli_output(vm, "ip4 fib lookup tables:\n %U", format_session_lookup_tables, FIB_PROTOCOL_IP4);
    vlib_cli_output(vm, "ip6 fib lookup tables:\n %U", format_session_lookup_tables, FIB_PROTOCOL_IP6);
...
void session_lookup_init(void)
{
     ...
     * "ip table del"
     */
    slm->fib_src = fib_source_allocate("session lookup", FIB_SOURCE_PRIORITY_LOW, FIB_SOURCE_BH_SIMPLE);
...
void session_lookup_table_cleanup(u32 fib_proto, u32 fib_index)
{
    ...
    if (fib_index_to_lock_count[fib_proto][fib_index] == 0) {
        ...
        if (st) {
            ...
            if (vec_len(fib_index_to_table_index[fib_proto]) > fib_index) fib_index_to_table_index[fib_proto][fib_index] = ~0;
        }
    }
}
