...
app_listener_t *app_listener_get(u32 app_listener_index)
{
    app_main_t *am = &app_main;
...
static void app_listener_free(application_t *app, app_listener_t *app_listener)
{
    ...
    if (CLIB_DEBUG) clib_memset(app_listener, 0xfa, sizeof(*app_listener));
...
session_handle_t app_listen_session_handle(session_t *ls)
{
    ...
    if (ls->al_index == SESSION_INVALID_INDEX) return listen_session_get_handle(ls);
...
app_listener_t *app_listener_lookup(application_t *app, session_endpoint_cfg_t *sep_ext)
{
     ...
     */
    if (!application_has_local_scope(app) && ip_is_zero(&sep_ext->ip, sep_ext->is_ip4) && sep_ext->sw_if_index != ENDPOINT_INVALID_INDEX) {
        if ((iface_ip = ip_interface_get_first_ip(sep_ext->sw_if_index, sep_ext->is_ip4))) {
...
int app_listener_alloc_and_init(application_t *app, session_endpoint_cfg_t *sep, app_listener_t **listener)
{
    ...
    if (application_has_local_scope(app) && session_endpoint_is_local((session_endpoint_t *) sep)) {
        ...
        local_st          = session_type_from_proto_and_ip(TRANSPORT_PROTO_NONE, sep->is_ip4);
...
static app_worker_t *app_listener_select_worker(app_listener_t *al)
{
    application_t *app;
    u32            wrk_index;
    ...
    if (wrk_index == ~0) wrk_index = clib_bitmap_first_set(al->workers);
...
{
    if (al->session_index == SESSION_INVALID_INDEX) return 0;
...
{
    if (al->local_index == SESSION_INVALID_INDEX) return 0;
    return listen_session_get(al->local_index);
}

session_t *app_listener_get_wrk_cl_session(app_listener_t *al, u32 wrk_map_index)
{
...
{
    if (pool_is_free_index(app->worker_maps, map_index)) return 0;
...
u32 application_session_table(application_t *app, u8 fib_proto)
{
    ...
    if (!application_has_global_scope(app)) return APP_INVALID_INDEX;
    if (fib_proto == FIB_PROTOCOL_IP4)
        return session_lookup_get_index_for_fib(fib_proto, app_ns->ip4_fib_index);
    else
        return session_lookup_get_index_for_fib(fib_proto, app_ns->ip6_fib_index);
}

u32 application_local_session_table(application_t *app)
{
    app_namespace_t *app_ns;
    if (!application_has_local_scope(app)) return APP_INVALID_INDEX;
    app_ns = app_namespace_get(app->ns_index);
...
{
    if (api_client_index != APP_INVALID_INDEX) hash_set(app_main.app_by_api_client_index, api_client_index, app_index);
}
...
application_t *application_lookup(u32 api_client_index)
{
    ...
    if (p) return application_get_if_valid(p[0]);
...
application_t *application_lookup_name(const u8 *name)
{
    ...
    if (p) return application_get(p[0]);
...
void appsl_pending_rx_mqs_del(appsl_wrk_t *aw, app_rx_mq_elt_t *elt)
{
    ...
    if (elt == aw->pending_rx_mqs) aw->pending_rx_mqs = elt->next;
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    do {
        if (!(elt->flags & APP_RX_MQ_F_POSTPONED)) rv = read(svm_msg_q_get_eventfd(elt->mq), &buf, sizeof(buf));
    ...
    if (aw->pending_rx_mqs) vlib_node_set_interrupt_pending(vm, appsl_rx_mqs_input_node.index);

    if (n_msgs && wrk->state == SESSION_WRK_INTERRUPT) vlib_node_set_interrupt_pending(vm, session_queue_node.index);
...
static clib_error_t *app_rx_mq_fd_read_ready(clib_file_t *cf)
{
    ...
    if ((mqe->flags & APP_RX_MQ_F_PENDING) || svm_msg_q_is_empty(mqe->mq)) return 0;
...
static void app_rx_mqs_epoll_add(application_t *app, app_rx_mq_elt_t *mqe)
{
    ...
    template.description          = format(0, "app-%u-rx-mq-%u", app->app_index, thread_index);
...
static void app_rx_mqs_epoll_del(application_t *app, app_rx_mq_elt_t *mqe)
{
    ...
    if (mqe->flags & APP_RX_MQ_F_PENDING) appsl_pending_rx_mqs_del(aw, mqe);
...
static int app_rx_mqs_alloc(application_t *app)
{
    ...
    svm_msg_q_ring_cfg_t rc[SESSION_MQ_N_RINGS] = {{evt_q_length, evt_size, 0}, {evt_q_length >> 1, 256, 0}};
    cfg->consumer_pid                           = 0;
    cfg->n_rings                                = 2;
...
{
    if (application_use_private_rx_mqs()) return &app->rx_mqs_segment;
...
{
    if (pool_is_free_index(app_main.app_pool, app_index)) return 0;

    return pool_elt_at_index(app_main.app_pool, app_index);
}

static int _null_app_tx_callback(session_t *s)
{
    return 0;
}
...
{
    if (cb_fns->session_accept_callback == 0) clib_warning("No accept callback function provided");
    if (cb_fns->session_connected_callback == 0) clib_warning("No session connected callback function provided");
    if (cb_fns->session_disconnect_callback == 0) clib_warning("No session disconnect callback function provided");
    if (cb_fns->session_reset_callback == 0) clib_warning("No session reset callback function provided");
    if (!cb_fns->builtin_app_tx_callback) cb_fns->builtin_app_tx_callback = _null_app_tx_callback;
}
...
static u8 application_verify_cfg(ssvm_segment_type_t st)
{
    ...
    if (st == SSVM_SEGMENT_MEMFD) {
        ...
        if (!is_valid) clib_warning("memfd seg: vpp's event qs IN binary api svm region");
        return is_valid;
    }
    else if (st == SSVM_SEGMENT_SHM) {
        is_valid = (session_main_get_wrk_mqs_segment() == 0);
        if (!is_valid) clib_warning("shm seg: vpp's event qs NOT IN binary api svm region");
        return is_valid;
    }
    else
        return 1;
...
static session_error_t application_alloc_and_init(app_init_args_t *a)
{
     ...
     */
    if ((opts[APP_OPTIONS_FLAGS] & APP_OPTIONS_FLAGS_IS_BUILTIN) && !(opts[APP_OPTIONS_FLAGS] & APP_OPTIONS_FLAGS_MEMFD_FOR_BUILTIN))
        seg_type = SSVM_SEGMENT_PRIVATE;

    if ((opts[APP_OPTIONS_FLAGS] & APP_OPTIONS_FLAGS_EVT_MQ_USE_EVENTFD) && seg_type != SSVM_SEGMENT_MEMFD) {
        clib_warning(
    ...
    if (!application_verify_cfg(seg_type)) return SESSION_E_NOSUPPORT;

    if (opts[APP_OPTIONS_PREALLOC_FIFO_PAIRS] && opts[APP_OPTIONS_PREALLOC_FIFO_HDRS]) return SESSION_E_NOSUPPORT;
    ...
    if (!application_has_global_scope(app) && !application_has_local_scope(app)) app->flags |= APP_OPTIONS_FLAGS_USE_GLOBAL_SCOPE;
    ...
    if (opts[APP_OPTIONS_FLAGS] & APP_OPTIONS_FLAGS_USE_HUGE_PAGE) props->huge_page = 1;
    if (opts[APP_OPTIONS_RX_FIFO_SIZE]) props->rx_fifo_size = opts[APP_OPTIONS_RX_FIFO_SIZE];
    if (opts[APP_OPTIONS_TX_FIFO_SIZE]) props->tx_fifo_size = opts[APP_OPTIONS_TX_FIFO_SIZE];
    if (opts[APP_OPTIONS_EVT_QUEUE_SIZE]) props->evt_q_size = opts[APP_OPTIONS_EVT_QUEUE_SIZE];
    if (opts[APP_OPTIONS_FLAGS] & APP_OPTIONS_FLAGS_EVT_MQ_USE_EVENTFD) props->use_mq_eventfd = 1;
    if (opts[APP_OPTIONS_TLS_ENGINE]) app->tls_engine = opts[APP_OPTIONS_TLS_ENGINE];
    if (opts[APP_OPTIONS_MAX_FIFO_SIZE]) props->max_fifo_size = opts[APP_OPTIONS_MAX_FIFO_SIZE];
    if (opts[APP_OPTIONS_HIGH_WATERMARK]) props->high_watermark = opts[APP_OPTIONS_HIGH_WATERMARK];
    if (opts[APP_OPTIONS_LOW_WATERMARK]) props->low_watermark = opts[APP_OPTIONS_LOW_WATERMARK];
    if (opts[APP_OPTIONS_PCT_FIRST_ALLOC]) props->pct_first_alloc = opts[APP_OPTIONS_PCT_FIRST_ALLOC];
    ...
    APP_DBG("New app name: %v api index: %u index %u", app->name, a->api_client_index, app->app_index);
...
static void application_free(application_t *app)
{
    ...
    if (application_is_proxy(app)) application_remove_proxy(app);
     ...
     */
    if (application_is_builtin(app)) application_name_table_del(app);
    vec_free(app->name);
...
static void application_detach_process(application_t *app, u32 api_client_index)
{
    ...
    APP_DBG("Detaching for app %v index %u api client index %u", app->name, app->app_index, api_client_index);

    pool_foreach (wrk_map, app->worker_maps) {
        app_wrk = app_worker_get(wrk_map->wrk_index);
        if (app_wrk->api_client_index == api_client_index) vec_add1(wrks, app_wrk->wrk_index);
    }

    if (!vec_len(wrks)) {
        clib_warning("no workers for app %u api_index %u", app->app_index, api_client_index);
        return;
    }
...
void application_namespace_cleanup(app_namespace_t *app_ns)
{
    ...
    pool_foreach (app, app_main.app_pool)
        if (app->ns_index == ns_index) vec_add1(app_indices, app->app_index);
...
always_inline u32 app_listener_cl_flow_hash(session_dgram_hdr_t *hdr)
{
    u32 hash = 0;
...
session_t *app_listener_select_wrk_cl_session(session_t *ls, session_dgram_hdr_t *hdr)
{
    u32             wrk_map_index = 0;
    app_listener_t *al;
    ...
    if (al->workers[0] != 1) {
        ...
        if (wrk_map_index == ~0) wrk_map_index = clib_bitmap_first_set(al->workers);
    }
...
session_error_t vnet_app_worker_add_del(vnet_app_worker_add_del_args_t *a)
{
    ...
    if (a->is_add) {
        if ((rv = application_alloc_worker_and_init(app, &app_wrk))) return rv;
        ...
        if (!wrk_map) return SESSION_E_INVALID;

        app_wrk = app_worker_get(wrk_map->wrk_index);
        if (!app_wrk) return SESSION_E_INVALID;

        application_api_table_del(app_wrk->api_client_index);
        if (appns_sapi_enabled()) sapi_socket_close_w_handle(app_wrk->api_client_index);
        app_worker_free(app_wrk);
        app_worker_map_free(app, wrk_map);
        if (application_n_workers(app) == 0) application_free(app);
    }
    return 0;
}

static session_error_t app_validate_namespace(u8 *namespace_id, u64 secret, u32 *app_ns_index)
{
    app_namespace_t *app_ns;
    ...
    *app_ns_index = app_namespace_index_from_id(namespace_id);
    if (*app_ns_index == APP_NAMESPACE_INVALID_INDEX) return SESSION_E_INVALID_NS;
    app_ns = app_namespace_get(*app_ns_index);
    if (!app_ns) return SESSION_E_INVALID_NS;
    if (app_ns->ns_secret != secret) return SESSION_E_WRONG_NS_SECRET;
    return 0;
}
...
static u8 *app_name_from_api_index(u32 api_client_index)
{
    ...
    if (regp) return format(0, "%s", regp->name);

    clib_warning("api client index %u does not have an api registration!", api_client_index);
...
session_error_t vnet_application_attach(vnet_app_attach_args_t *a)
{
    ...
    if (app) return SESSION_E_APP_ATTACHED;
    ...
    if (!a->use_sock_api) {
        ...
        if ((rv = app_validate_namespace(a->namespace_id, secret, &app_ns_index))) return rv;
        a->options[APP_OPTIONS_NAMESPACE] = app_ns_index;
    }

    if ((rv = application_alloc_and_init((app_init_args_t *) a))) return rv;
    ...
    if (!application_is_builtin(app) && application_use_private_rx_mqs()) rv = app_rx_mqs_alloc(app);
...
static u8 session_endpoint_in_ns(session_endpoint_cfg_t *sep)
{
    u8 is_lep;

    if (sep->flags & SESSION_ENDPT_CFG_F_PROXY_LISTEN) return 1;

    is_lep = session_endpoint_is_local((session_endpoint_t *) sep);
    if (!is_lep && sep->sw_if_index != ENDPOINT_INVALID_INDEX && !ip_interface_has_address(sep->sw_if_index, &sep->ip, sep->is_ip4)) {
        clib_warning("sw_if_index %u not configured with ip %U", sep->sw_if_index, format_ip46_address, &sep->ip, sep->is_ip4);
        return 0;
    }
...
static void session_endpoint_update_for_app(session_endpoint_cfg_t *sep, application_t *app, u8 is_connect)
{
    ...
    if (app->flags & APP_OPTIONS_FLAGS_IS_TRANSPORT_APP) ns_index = sep->ns_index;
    ...
    if (!is_connect) {
        ...
        if (app_ns->sw_if_index != APP_NAMESPACE_INVALID_INDEX && sep->peer.sw_if_index != ENDPOINT_INVALID_INDEX
...
session_error_t vnet_listen(vnet_listen_args_t *a)
{
    ...
    app = application_get_if_valid(a->app_index);
    if (!app) return SESSION_E_NOAPP;

    app_wrk = application_get_worker(app, a->wrk_map_index);
    if (!app_wrk) return SESSION_E_INVALID_APPWRK;

    a->sep_ext.app_wrk_index = app_wrk->wrk_index;

    session_endpoint_update_for_app(&a->sep_ext, app, 0 /* is_connect */);
    if (!session_endpoint_in_ns(&a->sep_ext)) return SESSION_E_INVALID_NS;
    ...
    if (app_listener) {
        if (app_listener->app_index != app->app_index) return SESSION_E_ALREADY_LISTENING;
        if ((rv = app_worker_start_listen(app_wrk, app_listener))) return rv;
     ...
     */
    if ((rv = app_listener_alloc_and_init(app, &a->sep_ext, &app_listener))) return rv;
...
session_error_t vnet_connect(vnet_connect_args_t *a)
{
    ...
    if (session_endpoint_is_zero(&a->sep)) return SESSION_E_INVALID_RMT_IP;
    ...
    if (application_has_local_scope(client)) {
        ...
        if (!rv || rv != SESSION_E_LOCAL_CONNECT) return rv;
    }
    /*
...
session_error_t vnet_unlisten(vnet_unlisten_args_t *a)
{
    ...
    if (!(app = application_get_if_valid(a->app_index))) return SESSION_E_NOAPP;

    if (!(al = app_listener_get_w_handle(a->handle))) return SESSION_E_NOLISTEN;
...
session_error_t vnet_shutdown_session(vnet_shutdown_args_t *a)
{
    ...
    if (!s) return SESSION_E_NOSESSION;

    app_wrk = app_worker_get(s->app_wrk_index);
    if (app_wrk->app_index != a->app_index) return SESSION_E_OWNER;
...
int application_change_listener_owner(session_t *s, app_worker_t *app_wrk)
{
    ...
    if (!old_wrk) return SESSION_E_INVALID_APPWRK;

    hash_unset(old_wrk->listeners_table, listen_session_get_handle(s));
    if (session_transport_service_type(s) == TRANSPORT_SERVICE_CL && s->rx_fifo) segment_manager_dealloc_fifos(s->rx_fifo, s->tx_fifo);
    ...
    app_listener->workers = clib_bitmap_set(app_listener->workers, old_wrk->wrk_map_index, 0);
...
static clib_error_t *application_start_stop_proxy_fib_proto(application_t *app, u8 fib_proto, u8 transport_proto, u8 is_start)
{
    ...
    if (!ip_is_zero(&tc->lcl_ip, 1)) {
        ...
        if (is_start)
            session_lookup_add_session_endpoint(sti, (session_endpoint_t *) &sep, s->session_index);
        else
            session_lookup_del_session_endpoint(sti, (session_endpoint_t *) &sep);
    }
...
static void application_start_stop_proxy_local_scope(application_t *app, u8 transport_proto, u8 is_start)
{
    ...
    if (is_start) {
        session_lookup_add_session_endpoint(app_ns->local_table_index, &sep, app->app_index);
        sep.is_ip4 = 0;
        session_lookup_add_session_endpoint(app_ns->local_table_index, &sep, app->app_index);
    }
    else {
...
{
    if (application_has_local_scope(app)) application_start_stop_proxy_local_scope(app, transport_proto, is_start);

    if (application_has_global_scope(app)) {
        application_start_stop_proxy_fib_proto(app, FIB_PROTOCOL_IP4, transport_proto, is_start);
        application_start_stop_proxy_fib_proto(app, FIB_PROTOCOL_IP6, transport_proto, is_start);
    }
}
...
void application_setup_proxy(application_t *app)
{
    ...
    transport_proto_foreach(tp, transports) application_start_stop_proxy(app, tp, 1);
}
...
void application_remove_proxy(application_t *app)
{
    ...
    transport_proto_foreach(tp, transports) application_start_stop_proxy(app, tp, 0);
}
...
static void application_format_listeners(application_t *app, int verbose)
{
    ...
    if (!app) {
        vlib_cli_output(vm, "%U", format_app_worker_listener, NULL /* header */, 0, 0, verbose);
        return;
    }

    pool_foreach (wrk_map, app->worker_maps) {
        app_wrk = app_worker_get(wrk_map->wrk_index);
        if (hash_elts(app_wrk->listeners_table) == 0) continue;
        hash_foreach(handle, sm_index, app_wrk->listeners_table, ({
                         vlib_cli_output(vm, "%U", format_app_worker_listener, app_wrk, handle, sm_index, verbose);
                     }));
    }
}
...
u8 *format_cert_key_pair(u8 *s, va_list *args)
{
    ...
    if (ckpair->cert_key_index == 0)
        ...
        s = format(s, "%d (cert:%d, key:%d)", ckpair->cert_key_index, cert_len, key_len);
    return s;
}
...
u8 *format_application(u8 *s, va_list *args)
{
    ...
    if (app == 0) {
        if (!verbose) s = format(s, "%-10s%-20s%-40s", "Index", "Name", "Namespace");
        return s;
    }
    ...
    props       = application_segment_manager_properties(app);
    if (!verbose) {
        s = format(s, "%-10u%-20v%-40v", app->app_index, app_name, app_ns_name);
        return s;
    }
    ...
    s = format(s, "rx-fifo-size %U tx-fifo-size %U workers:\n", format_memory_size, props->rx_fifo_size, format_memory_size, props->tx_fifo_size);
...
static inline void appliction_format_app_mq(vlib_main_t *vm, application_t *app)
{
    app_worker_map_t *map;
    app_worker_t     *wrk;
    int               i;
    ...
    pool_foreach (map, app->worker_maps) {
        ...
        vlib_cli_output(vm, "[A%d][%d]%U", app->app_index, map->wrk_index, format_svm_msg_q, wrk->event_queue);
    }

    for (i = 0; i < vec_len(app->rx_mqs); i++)
        vlib_cli_output(vm, "[A%d][R%d]%U", app->app_index, i, format_svm_msg_q, app->rx_mqs[i].mq);
}
...
static clib_error_t *appliction_format_all_app_mq(vlib_main_t *vm)
{
    ...
    for (i = 0; i < n_threads; i++) {
        vlib_cli_output(vm, "[Ctrl%d]%U", i, format_svm_msg_q, session_main_get_vpp_event_queue(i));
    }
...
static clib_error_t *show_app_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (do_mq && app_index != ~0) {
        ...
        if (!app) return clib_error_return(0, "No app with index %u", app_index);
    ...
    if (!do_server && !do_client) {
        ...
        pool_foreach (app, app_main.app_pool) {
            ...
            if ((!do_transports && !is_ta) || (do_transports && is_ta)) vlib_cli_output(vm, "%U", format_application, app, 0);
        }
    }
...
{
    if (pool_is_free_index(app_main.cert_key_pair_store, index)) return 0;
...
{
    app_cert_key_pair_t *ckpair;
    if (!(ckpair = app_cert_key_pair_get_if_valid(index))) return -1;
    if (vec_search(ckpair->app_interests, app_index) != ~0) vec_add1(ckpair->app_interests, app_index);
    return 0;
}
...
int vnet_app_del_cert_key_pair(u32 index)
{
    ...
    vec_foreach (app_index, ckpair->app_interests) {
        if ((app = application_get_if_valid(*app_index)) && app->cb_fns.app_cert_key_pair_delete_callback)
...
VLIB_CLI_COMMAND(show_app_command, static) = {
    .path = "show app",
    .short_help =
        "show app [index] [server|client] [mq] [verbose] "
        "[transports]",
