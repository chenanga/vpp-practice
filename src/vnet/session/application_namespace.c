...
void app_namespace_walk(app_namespace_walk_fn_t fn, void *ctx)
{
...
app_namespace_t *app_namespace_get_from_id(const u8 *ns_id)
{
    ...
    if (index == APP_NAMESPACE_INVALID_INDEX) return 0;
...
app_namespace_t *app_namespace_alloc(const u8 *ns_id)
{
    ...
    hash_set_mem(app_namespace_lookup_table, app_ns->ns_id, app_ns - app_namespace_pool);

    return app_ns;
}

session_error_t vnet_app_namespace_add_del(vnet_app_namespace_add_del_args_t *a)
{
    app_namespace_t *app_ns;
    session_table_t *st;
    u32              ns_index;
    ...
    if (a->is_add) {
        if (a->sw_if_index != APP_NAMESPACE_INVALID_INDEX && !vnet_get_sw_interface_or_null(vnet_get_main(), a->sw_if_index))
            return SESSION_E_INVALID;

        if (a->sw_if_index != APP_NAMESPACE_INVALID_INDEX) {
            a->ip4_fib_id = fib_table_get_table_id_for_sw_if_index(FIB_PROTOCOL_IP4, a->sw_if_index);
            a->ip6_fib_id = fib_table_get_table_id_for_sw_if_index(FIB_PROTOCOL_IP6, a->sw_if_index);
        }
        if (a->sw_if_index == APP_NAMESPACE_INVALID_INDEX && a->ip4_fib_id == APP_NAMESPACE_INVALID_INDEX) return SESSION_E_INVALID;
        ...
        if (!app_ns) return SESSION_E_INVALID;

        application_namespace_cleanup(app_ns);

        if (app_sapi_enabled) appns_sapi_del_ns_socket(app_ns);

        st = session_table_get(app_ns->local_table_index);

        session_table_free(st, FIB_PROTOCOL_MAX);
        if (app_ns->sock_name) vec_free(app_ns->sock_name);
...
u32 app_namespace_index_from_id(const u8 *ns_id)
{
    ...
    vec_free(key);
    if (!indexp) return APP_NAMESPACE_INVALID_INDEX;
    return *indexp;
}
...
{
    return fib_proto == FIB_PROTOCOL_IP4 ? app_ns->ip4_fib_index : app_ns->ip6_fib_index;
}
...
int appns_sapi_enable_disable(int is_enable)
{
    ...
    if (pool_elts(app_namespace_pool)) return -1;
...
void app_namespaces_init(void)
{
    ...
    if (!app_namespace_lookup_table) app_namespace_lookup_table = hash_create_vec(0, sizeof(u8), sizeof(uword));
...
static clib_error_t *app_ns_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "add"))
            ...
            sw_if_index_set = 1;
        else if (unformat(line_input, "if %U", unformat_vnet_sw_interface, vnm, &sw_if_index))
            sw_if_index_set = 1;
  ...
  vnet_app_namespace_add_del_args_t args = {
    ...
    if ((rv = vnet_app_namespace_add_del(&args))) error = clib_error_return(0, "app namespace add del returned %d", rv);
...
u8 *format_app_namespace(u8 *s, va_list *args)
{
    ...
    s = format(s, "Application namespace [%u]\nid:        %s\nsecret:    %lu", app_namespace_index(app_ns), app_ns->ns_id, app_ns->ns_secret);
    if (app_ns->sw_if_index != (u32) ~0) s = format(s, "\nInterface: %U", format_vnet_sw_if_index_name, vnm, app_ns->sw_if_index);
    if (app_ns->sock_name) s = format(s, "\nSocket:    %s", app_ns->sock_name);
...
static void app_namespace_show_api(vlib_main_t *vm, app_namespace_t *app_ns)
{
    ...
    if (!pool_elts(app_ns->app_sockets)) return;
    ...
    pool_foreach (cs, app_ns->app_sockets) {
        ...
        vlib_cli_output(vm, "%12d%12d%5u", app_wrk->app_index, app_wrk->wrk_map_index, cf->file_descriptor);
    }
}
...
do_ns_list:
    ...
    pool_foreach (app_ns, app_namespace_pool) {
        ...
        table_format_cell(t, i, j++, "%U", format_vnet_sw_if_index_name, vnm, app_ns->sw_if_index);
