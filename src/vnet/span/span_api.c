...
static void vl_api_sw_interface_span_enable_disable_t_handler(vl_api_sw_interface_span_enable_disable_t *mp)
{
    ...
    rv = span_add_delete_entry(vm,
                               ntohl(mp->sw_if_index_from),
                               ntohl(mp->sw_if_index_to),
                               ntohl(mp->state),
    ...
    vec_foreach (si, sm->interfaces) {
        ...
        if (rxm->num_mirror_ports || txm->num_mirror_ports) {
            ...
            clib_bitmap_foreach (i, b) {
                ...
                rmp->_vl_msg_id = ntohs(REPLY_MSG_ID_BASE + VL_API_SW_INTERFACE_SPAN_DETAILS);
                rmp->context    = mp->context;

                rmp->sw_if_index_from = htonl(si - sm->interfaces);
                rmp->sw_if_index_to   = htonl(i);
                rmp->state            = htonl((clib_bitmap_get(rxm->mirror_ports, i) + clib_bitmap_get(txm->mirror_ports, i) * 2));
                rmp->is_l2            = mp->is_l2;
