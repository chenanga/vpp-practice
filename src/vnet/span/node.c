                                      ...
                                      span_feat_t          sf)
{
    ...
    if (sw_if_index0 >= vec_len(sm->interfaces)) return;

    si0 = vec_elt_at_index(sm->interfaces, sw_if_index0);
    sm0 = &si0->mirror_rxtx[sf][rxtx];

    if (sm0->num_mirror_ports == 0) return;

    /* Don't do it again */
    if (PREDICT_FALSE(b0->flags & VNET_BUFFER_F_SPAN_CLONE)) return;
    ...
    clib_bitmap_foreach (i, sm0->mirror_ports) {
        ...
        if (PREDICT_TRUE(c0 != 0)) {
            ...
            if (sf == SPAN_FEAT_L2) vnet_buffer(c0)->l2.feature_bitmap = L2OUTPUT_FEAT_OUTPUT;
...
static_always_inline uword span_node_inline_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, vlib_rx_or_tx_t rxtx, span_feat_t sf)
{
    ...
    vec_validate_aligned(mirror_frames, sm->max_sw_if_index, CLIB_CACHE_LINE_BYTES);
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            switch (sf) {
                case SPAN_FEAT_L2:
                    if (rxtx == VLIB_RX) {
                        next0 = vnet_l2_feature_next(b0, sm->l2_input_next, L2INPUT_FEAT_SPAN);
                        next1 = vnet_l2_feature_next(b1, sm->l2_input_next, L2INPUT_FEAT_SPAN);
                    }
                    else {
                        next0 = vnet_l2_feature_next(b0, sm->l2_output_next, L2OUTPUT_FEAT_SPAN);
                        next1 = vnet_l2_feature_next(b1, sm->l2_output_next, L2OUTPUT_FEAT_SPAN);
                    }
                    break;
...
#define span_node_defs                                                                                                                         \
    .vector_size = sizeof(u32), .format_trace = format_span_trace, .type = VLIB_NODE_TYPE_INTERNAL, .n_errors = ARRAY_LEN(span_error_strings), \
    .error_strings = span_error_strings, .n_next_nodes = 0, .next_nodes = {[0] = "error-drop"}
...
clib_error_t *span_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, span_l2_input_node.index, L2INPUT_N_FEAT, l2input_get_feat_names(), sm->l2_input_next);

    feat_bitmap_init_next_nodes(vm, span_l2_output_node.index, L2OUTPUT_N_FEAT, l2output_get_feat_names(), sm->l2_output_next);
    return 0;
}
