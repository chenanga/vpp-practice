...
static clib_error_t *bond_interface_admin_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    ...
    bif->admin_up = is_up;
    if (is_up) vnet_hw_interface_set_flags(vnm, bif->hw_if_index, VNET_HW_INTERFACE_FLAG_LINK_UP);
    return 0;
}
...
static clib_error_t *bond_add_del_mac_address(vnet_hw_interface_t *hi, const u8 *address, u8 is_add)
{
    ...
    if (!bif) {
        return clib_error_return(0, "No bond interface found for sw_if_index %u", hi->sw_if_index);
    }
    ...
    vec_foreach_index (i, bif->members) {
        ...
        error = vnet_hw_interface_add_del_mac_address(vnm, s_hi->hw_if_index, address, is_add);
        ...
        if (error) {
            ...
            for (j = i - 1; j > -1; j--) {
                ...
                vnet_hw_interface_add_del_mac_address(vnm, s_hi->hw_if_index, address, !(is_add));
            }
...
static_always_inline u32 bond_lb_round_robin(bond_if_t *bif, vlib_buffer_t *b0, uword n_members)
{
    ...
    if (bif->lb_rr_last_index >= n_members) bif->lb_rr_last_index = 0;

    return bif->lb_rr_last_index;
}

static_always_inline void bond_tx_hash(vlib_main_t *vm, bond_per_thread_data_t *ptd, bond_if_t *bif, vlib_buffer_t **b, u32 *h, u32 n_left)
{
...
static_always_inline void bond_tx_no_hash(vlib_main_t *vm, bond_if_t *bif, vlib_buffer_t **b, u32 *h, u32 n_left, uword n_members, u32 lb_alg)
{
    while (n_left >= 8) {
        // Prefetch next iteration
        vlib_prefetch_buffer_header(b[4], LOAD);
...
bond_update_sw_if_index(bond_per_thread_data_t *ptd, bond_if_t *bif, u32 *bi, vlib_buffer_t **b, u32 *data, u32 n_left, int single_sw_if_index)
{
    ...
    while (n_left >= 8) {
        ...
        if (PREDICT_FALSE(single_sw_if_index)) {
            ...
            vnet_buffer(b[0])->sw_if_index[VLIB_TX] = *vec_elt_at_index(bif->active_members, h[0]);
            vnet_buffer(b[1])->sw_if_index[VLIB_TX] = *vec_elt_at_index(bif->active_members, h[1]);
            vnet_buffer(b[2])->sw_if_index[VLIB_TX] = *vec_elt_at_index(bif->active_members, h[2]);
            vnet_buffer(b[3])->sw_if_index[VLIB_TX] = *vec_elt_at_index(bif->active_members, h[3]);
...
static_always_inline void bond_tx_trace(vlib_main_t *vm, vlib_node_runtime_t *node, bond_if_t *bif, vlib_buffer_t **b, u32 n_left, u32 *h)
{
    ...
    while (n_trace > 0 && n_left > 0) {
        if (PREDICT_TRUE(vlib_trace_buffer(vm, node, 0, b[0], 0 /* follow_chain */))) {
            ...
            if (!h) {
                ...
                t0->bond_sw_if_index = *vec_elt_at_index(bif->active_members, h[0]);
                h++;
            }
        }
        b++;
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    if (PREDICT_FALSE(n_members == 0)) {
        ...
        vlib_increment_simple_counter(vnet_main.interface_main.sw_if_counters + VNET_INTERFACE_COUNTER_DROP,
                                      ...
                                      bif->sw_if_index,
                                      frame->n_vectors);
        vlib_error_count(vm, node->node_index, BOND_TX_ERROR_NO_MEMBER, frame->n_vectors);
        return frame->n_vectors;
    }
    ...
    if (bif->n_numa_members >= 1) n_members = bif->n_numa_members;
    ...
    for (p = 0; p < n_members; p++) {
        ...
        if (PREDICT_TRUE(ptd->per_port_queue[p].n_buffers)) {
            ...
            clib_memcpy_fast(to_next, ptd->per_port_queue[p].buffers, f->n_vectors * sizeof(u32));
...
static walk_rc_t bond_active_interface_switch_cb(vnet_main_t *vnm, u32 sw_if_index, void *arg)
{
    ...
    ip4_neighbor_advertise(bm->vlib_main, bm->vnet_main, sw_if_index, vlib_get_thread_index(), NULL);
    ip6_neighbor_advertise(bm->vlib_main, bm->vnet_main, sw_if_index, vlib_get_thread_index(), NULL);
...
static uword bond_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    while (1) {
        ...
        for (i = 0; i < vec_len(event_data); i++) {
            ...
            if (vnet_get_hw_interface_or_null(vnm, hw_if_index)) /* walk hw interface to process all subinterfaces */
                vnet_hw_interface_walk_sw(vnm, hw_if_index, bond_active_interface_switch_cb, 0);
        }
