...
static u8 *format_bond_input_trace(u8 *s, va_list *args)
{
    ...
    s = format(s,
               ...
               t->ethernet.src_address,
               format_ethernet_address,
               t->ethernet.dst_address,
...
static_always_inline u8 packet_is_cdp(ethernet_header_t *eth)
{
    ...
    return ((eth->type == htons(ETHERNET_TYPE_CDP))
            || ((llc->src_sap == 0xAA) && (llc->control == 0x03) && (snap->protocol == htons(0x2000)) && (snap->oui[0] == 0) && (snap->oui[1] == 0)
                && (snap->oui[2] == 0x0C)));
}

static inline void bond_sw_if_idx_rewrite(vlib_main_t         *vm,
                                          ...
                                          u32                 *n_rx_bytes)
{
    ...
    *n_rx_bytes += b->current_length;
    ...
    if (!ethernet_frame_is_tagged(ntohs(ethertype))) {
        ...
        if (PREDICT_TRUE((ethertype != htons(ETHERNET_TYPE_SLOW_PROTOCOLS)) && !packet_is_cdp(eth)
        ...
        ethertype = clib_mem_unaligned(ethertype_p, u16);
        if (PREDICT_TRUE((ethertype != htons(ETHERNET_TYPE_SLOW_PROTOCOLS)) && (ethertype != htons(ETHERNET_TYPE_CDP))
                         && (ethertype != htons(ETHERNET_TYPE_802_1_LLDP)))) {
            /* Change the physical interface to bond interface */
            vnet_buffer(b)->sw_if_index[VLIB_RX] = bond_sw_if_index;
            return;
...
static inline void bond_update_next(vlib_main_t         *vm,
                                    vlib_node_runtime_t *node,
                                    u32                 *last_member_sw_if_index,
                                    ...
                                    vlib_error_t        *error)
{
    ...
    *error      = 0;
    ...
    if (PREDICT_TRUE(*last_member_sw_if_index == member_sw_if_index)) goto next;
    ...
    *last_member_sw_if_index = member_sw_if_index;
    ...
    if (PREDICT_FALSE((bif->mode == BOND_MODE_ACTIVE_BACKUP) && vec_len(bif->active_members) && (member_sw_if_index != bif->active_members[0]))) {
        *bond_sw_if_index = member_sw_if_index;
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left >= 4) {
        ...
        if (PREDICT_TRUE(x == 0)) {
             ...
             */
            ASSERT((vnet_buffer(b[0])->feature_arc_index == vnet_buffer(b[1])->feature_arc_index)
                   && (vnet_buffer(b[0])->feature_arc_index == vnet_buffer(b[2])->feature_arc_index)
                   && (vnet_buffer(b[0])->feature_arc_index == vnet_buffer(b[3])->feature_arc_index));
            if (PREDICT_FALSE(vnet_get_feature_count(vnet_buffer(b[0])->feature_arc_index, last_member_sw_if_index) > 1))
            ...
            if (next_index == BOND_INPUT_NEXT_DROP) {
                ...
                bond_sw_if_idx_rewrite(vm, node, b[0], bond_sw_if_index, &n_rx_packets, &n_rx_bytes);
                bond_sw_if_idx_rewrite(vm, node, b[1], bond_sw_if_index, &n_rx_packets, &n_rx_bytes);
                bond_sw_if_idx_rewrite(vm, node, b[2], bond_sw_if_index, &n_rx_packets, &n_rx_bytes);
                bond_sw_if_idx_rewrite(vm, node, b[3], bond_sw_if_index, &n_rx_packets, &n_rx_bytes);
            }
        }
        else {
            bond_update_next(vm, node, &last_member_sw_if_index, sw_if_index[0], &bond_sw_if_index, b[0], &next_index, &error);
            next[0] = next_index;
            if (next_index == BOND_INPUT_NEXT_DROP)
                b[0]->error = error;
            else
            ...
            bond_update_next(vm, node, &last_member_sw_if_index, sw_if_index[1], &bond_sw_if_index, b[1], &next_index, &error);
            next[1] = next_index;
            if (next_index == BOND_INPUT_NEXT_DROP)
                b[1]->error = error;
            else
            ...
            bond_update_next(vm, node, &last_member_sw_if_index, sw_if_index[2], &bond_sw_if_index, b[2], &next_index, &error);
            next[2] = next_index;
            if (next_index == BOND_INPUT_NEXT_DROP)
                b[2]->error = error;
            else
            ...
            bond_update_next(vm, node, &last_member_sw_if_index, sw_if_index[3], &bond_sw_if_index, b[3], &next_index, &error);
            next[3] = next_index;
            if (next_index == BOND_INPUT_NEXT_DROP)
                b[3]->error = error;
            else
    ...
    if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE))) {
        ...
        while (n_left) {
            if (PREDICT_FALSE(b[0]->flags & VLIB_BUFFER_IS_TRACED)) {
                ...
                clib_memcpy_fast(&t0->ethernet, vlib_buffer_get_current(b[0]), sizeof(ethernet_header_t));
...
VLIB_REGISTER_NODE(bond_input_node) = {.name          = "bond-input",
...
static clib_error_t *bond_sw_interface_up_down(vnet_main_t *vnm, u32 sw_if_index, u32 flags)
{
    ...
    if (mif) {
        if (mif->lacp_enabled) return 0;

        /* port_enabled is both admin up and hw link up */
        mif->port_enabled = ((flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP) && vnet_sw_interface_is_link_up(vnm, sw_if_index));
...
static clib_error_t *bond_hw_interface_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    ...
    if (mif) {
        ...
        /* port_enabled is both admin up and hw link up */
        mif->port_enabled = ((flags & VNET_HW_INTERFACE_FLAG_LINK_UP) && vnet_sw_interface_is_admin_up(vnm, sw->sw_if_index));
        if (mif->port_enabled == 0)
            bond_disable_collecting_distributing(vm, mif);
        else
            bond_enable_collecting_distributing(vm, mif);
