...
static inline uword unformat_bond_mode(unformat_input_t *input, va_list *args)
{
    u8 *r = va_arg(*args, u8 *);
...
static inline u8 *format_bond_mode(u8 *s, va_list *args)
{
...
static inline uword unformat_bond_load_balance(unformat_input_t *input, va_list *args)
{
    u8 *r = va_arg(*args, u8 *);
...
static inline u8 *format_bond_load_balance(u8 *s, va_list *args)
{
    u32 i = va_arg(*args, u32);
    u8 *t = 0;
...
static inline void bond_register_callback(lacp_enable_disable_func func)
{
...
static inline bond_if_t *bond_get_bond_if_by_sw_if_index(u32 sw_if_index)
{
    bond_main_t *bm = &bond_main;
    uword       *p;
...
static inline bond_if_t *bond_get_bond_if_by_dev_instance(u32 dev_instance)
{
    bond_main_t *bm = &bond_main;
...
static inline member_if_t *bond_get_member_by_sw_if_index(u32 sw_if_index)
{
    bond_main_t *bm  = &bond_main;
    member_if_t *mif = 0;
    uword        p;
    ...
    if (sw_if_index < vec_len(bm->member_by_sw_if_index)) {
        ...
        if (p) mif = pool_elt_at_index(bm->neighbors, p >> 1);
    }
