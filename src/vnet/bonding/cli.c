...
void bond_disable_collecting_distributing(vlib_main_t *vm, member_if_t *mif)
{
    ...
    vec_foreach_index (i, bif->active_members) {
        ...
        if (p == mif->sw_if_index) {
            if ((bif->mode == BOND_MODE_ACTIVE_BACKUP) && (i == 0) && (vec_len(bif->active_members) > 1))
    ...
    if (switching_active) vlib_process_signal_event(bm->vlib_main, bond_process_node.index, BOND_SEND_GARP_NA, bif->hw_if_index);
...
static int bond_member_sort(void *a1, void *a2)
{
     ...
     *
     */
    if (mif2->weight > mif1->weight) return 1;
    if (mif2->weight < mif1->weight)
        return -1;
    else {
        if (mif2->is_local_numa > mif1->is_local_numa) return 1;
        if (mif2->is_local_numa < mif1->is_local_numa)
            ...
            if (bif->active_members[0] == mif2->sw_if_index) return 1;
            if (bif->active_members[0] == mif1->sw_if_index) return -1;
            /* go for the tiebreaker as the last resort */
            if (mif1->sw_if_index > mif2->sw_if_index) return 1;
...
static void bond_sort_members(bond_if_t *bif)
{
    ...
    if (old_active != bif->active_members[0]) vlib_process_signal_event(bm->vlib_main, bond_process_node.index, BOND_SEND_GARP_NA, bif->hw_if_index);
}
...
void bond_enable_collecting_distributing(vlib_main_t *vm, member_if_t *mif)
{
    ...
    vec_foreach_index (i, bif->active_members) {
        p = *vec_elt_at_index(bif->active_members, i);
        if (p == mif->sw_if_index) goto done;
    }

    if (mif->lacp_enabled && bif->numa_only && (vm->numa_node == hw->numa_node)) {
        vec_insert_elts(bif->active_members, &mif->sw_if_index, 1, bif->n_numa_members);
        bif->n_numa_members++;
    }
    else
    ...
    if (bif->mode == BOND_MODE_ACTIVE_BACKUP) {
        if (vec_len(bif->active_members) == 1) /* First member becomes active? */
            vlib_process_signal_event(bm->vlib_main, bond_process_node.index, BOND_SEND_GARP_NA, bif->hw_if_index);
        else
...
int bond_dump_ifs(bond_interface_details_t **out_bondifs)
{
    ...
    pool_foreach (bif, bm->interfaces) {
        ...
        clib_memcpy(bondif->interface_name, hi->name, MIN(ARRAY_LEN(bondif->interface_name) - 1, vec_len((const char *) hi->name)));
...
int bond_dump_member_ifs(member_interface_details_t **out_memberifs, u32 bond_sw_if_index)
{
    ...
    vec_foreach (sw_if_index, bif->members) {
        ...
        if (mif) {
            ...
            clib_memcpy(memberif->interface_name, hi->name, MIN(ARRAY_LEN(memberif->interface_name) - 1, vec_len((const char *) hi->name)));
            /* enforce by memset() above */
            ASSERT(0 == memberif->interface_name[ARRAY_LEN(memberif->interface_name) - 1]);
...
static void bond_member_add_del_mac_addrs(bond_if_t *bif, u32 mif_sw_if_index, u8 is_add)
{
    ...
    vec_foreach (sec_mac, b_ei->secondary_addrs)
        vnet_hw_interface_add_del_mac_address(vnm, s_hwif->hw_if_index, sec_mac->mac.bytes, is_add);
}
...
static void bond_delete_neighbor(vlib_main_t *vm, bond_if_t *bif, member_if_t *mif)
{
    ...
    bif->port_number_bitmap                     = clib_bitmap_set(bif->port_number_bitmap, ntohs(mif->actor_admin.port_number) - 1, 0);
    ...
    vnet_feature_enable_disable("device-input", "bond-input", mif->sw_if_index, 0, 0, 0);

    /* Put back the old mac */
    vnet_hw_interface_change_mac_address(vnm, mif_hw->hw_if_index, mif->persistent_hw_address);

    /* delete the bond's secondary/virtual mac addrs from the member */
    bond_member_add_del_mac_addrs(bif, mif->sw_if_index, 0 /* is_add */);

    if ((bif->mode == BOND_MODE_LACP) && bm->lacp_enable_disable) (*bm->lacp_enable_disable)(vm, bif, mif, 0);

    if (bif->mode == BOND_MODE_LACP) {
        vlib_stats_remove_entry(bm->stats[bif->sw_if_index][mif->sw_if_index].actor_state);
        vlib_stats_remove_entry(bm->stats[bif->sw_if_index][mif->sw_if_index].partner_state);
    }
...
int bond_delete_if(vlib_main_t *vm, u32 sw_if_index)
{
    ...
    if (hw == NULL || bond_dev_class.index != hw->dev_class_index) return VNET_API_ERROR_INVALID_SW_IF_INDEX;
    ...
    vec_foreach (mif_sw_if_index, s_list) {
        ...
        if (mif) bond_delete_neighbor(vm, bif, mif);
    }
    vec_free(s_list);
...
void bond_create_if(vlib_main_t *vm, bond_create_if_args_t *args)
{
    ...
    if (bif->id == ~0) bif->id = bif->dev_instance;
     ...
     */
    if (args->gso) {
        vnet_hw_if_set_caps(vnm, bif->hw_if_index, VNET_HW_IF_CAP_TCP_GSO | VNET_HW_IF_CAP_TX_TCP_CKSUM | VNET_HW_IF_CAP_TX_UDP_CKSUM);
    }
    if (vlib_get_thread_main()->n_vlib_mains > 1) clib_spinlock_init(&bif->lockp);

    vnet_hw_interface_set_flags(vnm, bif->hw_if_index, VNET_HW_INTERFACE_FLAG_LINK_UP);
...
static clib_error_t *bond_create_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    /* Get a line of input. */
    if (!unformat_user(input, unformat_line_input, line_input)) return clib_error_return(0, "Missing required arguments.");
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "mode %U", unformat_bond_mode, &args.mode))
            mode_is_set = 1;
                 ...
                 && unformat(line_input, "load-balance %U", unformat_bond_load_balance, &args.lb))
            ;
        else if (unformat(line_input, "hw-addr %U", unformat_ethernet_address, args.hw_addr))
            args.hw_addr_set = 1;
            ...
            if (args.mode == BOND_MODE_LACP)
                ...
                return clib_error_return(0, "Only lacp mode supports numa-only so far!");
            }
        }
        else {
            unformat_free(line_input);
    ...
    if (mode_is_set == 0) return clib_error_return(0, "Missing bond mode");

    bond_create_if(vm, &args);

    if (!args.rv) vlib_cli_output(vm, "%U\n", format_vnet_sw_if_index_name, vnet_get_main(), args.sw_if_index);
...
static clib_error_t *bond_delete_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    /* Get a line of input. */
    if (!unformat_user(input, unformat_line_input, line_input)) return clib_error_return(0, "Missing <interface>");
    ...
    if (sw_if_index == ~0) return clib_error_return(0, "please specify interface name or sw_if_index");
...
void bond_add_member(vlib_main_t *vm, bond_add_member_args_t *args)
{
    ...
    if (bond_get_member_by_sw_if_index(args->member)) {
        ...
        args->error = clib_error_return(0, "interface was already added as member");
        return;
    }
    mif_hw = vnet_get_sup_hw_interface(vnm, args->member);
    if (mif_hw->dev_class_index == bond_dev_class.index) {
        args->rv    = VNET_API_ERROR_INVALID_INTERFACE;
        args->error = clib_error_return(0, "bond interface cannot be added as member");
        return;
    }
    if (bif->gso && !(mif_hw->caps & VNET_HW_IF_CAP_TCP_GSO)) {
        args->rv    = VNET_API_ERROR_INVALID_INTERFACE;
        args->error = clib_error_return(0, "member interface is not gso capable");
        return;
    }
    if (bif->mode == BOND_MODE_LACP) {
        u32 actor_idx, partner_idx;

        actor_idx = vlib_stats_add_gauge("/if/lacp/%u/%u/state", bif->sw_if_index, args->member);
        if (actor_idx == ~0) {
            args->rv = VNET_API_ERROR_INVALID_INTERFACE;
            return;
        }

        partner_idx = vlib_stats_add_gauge("/if/lacp/%u/%u/partner-state", bif->sw_if_index, args->member);
        if (partner_idx == ~0) {
    ...
    vec_validate_aligned(bm->member_by_sw_if_index, mif->sw_if_index, CLIB_CACHE_LINE_BYTES);
    /*
     ...
     */
    bm->member_by_sw_if_index[mif->sw_if_index] = (uword) (((mif - bm->neighbors) << 1) | 1);
    ...
    if (bif->use_custom_mac) {
        vnet_hw_interface_change_mac_address(vnm, mif_hw->hw_if_index, bif->hw_address);
    }
    else {
    ...
    if (bif_hw->l2_if_count)
        ethernet_set_flags(vnm, mif_hw->hw_if_index, ETHERNET_INTERFACE_FLAG_ACCEPT_ALL);
    else
    ...
    if (bif->mode == BOND_MODE_LACP) {
        if (bm->lacp_enable_disable) (*bm->lacp_enable_disable)(vm, bif, mif, 1);
    }
    ...
    vec_foreach_index (thread_index, bm->per_thread_data) {
        bond_per_thread_data_t *ptd = vec_elt_at_index(bm->per_thread_data, thread_index);

        vec_validate_aligned(ptd->per_port_queue, vec_len(bif->members) - 1, CLIB_CACHE_LINE_BYTES);
    ...
    args->rv = vnet_feature_enable_disable("device-input", "bond-input", mif->sw_if_index, 1, 0, 0);

    if (args->rv) {
        args->error = clib_error_return(0, "Error encountered on input feature arc enable");
    }
}
...
static clib_error_t *add_member_interface_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    args.group  = ~0;
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%U %U", unformat_vnet_sw_interface, vnm, &args.group, unformat_vnet_sw_interface, vnm, &args.member))
            ;
            ...
            args.error = clib_error_return(0, "unknown input `%U'", format_unformat_error, input);
            break;
        }
    }
    unformat_free(line_input);
    ...
    if (args.group == ~0) return clib_error_return(0, "Missing bond interface");
    if (args.member == ~0) return clib_error_return(0, "please specify valid member interface name");
...
static clib_error_t *detach_interface_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    args.member = ~0;
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%U", unformat_vnet_sw_interface, vnm, &args.member))
            ;
        else {
            args.error = clib_error_return(0, "unknown input `%U'", format_unformat_error, input);
            break;
...
static void show_bond(vlib_main_t *vm)
{
    ...
    vlib_cli_output(vm, "%-16s %-12s %-13s %-13s %-14s %s", "interface name", "sw_if_index", "mode", "load balance", "active members", "members");
...
static void show_bond_details(vlib_main_t *vm)
{
    ...
    pool_foreach (bif, bm->interfaces) {
        ...
        vlib_cli_output(vm, "  load balance: %U", format_bond_load_balance, bif->lb);
        if (bif->gso) vlib_cli_output(vm, "  gso enable");
        if (bif->mode == BOND_MODE_ROUND_ROBIN) vlib_cli_output(vm, "  last xmit member index: %u", bif->lb_rr_last_index);
        vlib_cli_output(vm, "  number of active members: %d", vec_len(bif->active_members));
        vec_foreach (sw_if_index, bif->active_members) {
            vlib_cli_output(vm, "    %U", format_vnet_sw_if_index_name, vnet_get_main(), *sw_if_index);
            if (bif->mode == BOND_MODE_ACTIVE_BACKUP) {
...
void bond_set_intf_weight(vlib_main_t *vm, bond_set_intf_weight_args_t *args)
{
    ...
    if (bif->mode != BOND_MODE_ACTIVE_BACKUP) {
        args->rv    = VNET_API_ERROR_INVALID_ARGUMENT;
        args->error = clib_error_return(0, "Weight valid for active-backup only");
        return;
    }
     ...
     */
    if (!vnet_sw_interface_is_up(vnm, mif->sw_if_index) || (vec_len(bif->active_members) == 1)
        || ((bif->active_members[0] == mif->sw_if_index) && (mif->weight >= old_weight)))
        return;
...
static clib_error_t *bond_set_intf_cmd(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "sw_if_index %d", &sw_if_index))
            ...
            clib_error_return(0, "unknown input `%U'", format_unformat_error, input);
            break;
        }
    }
...
clib_error_t *bond_cli_init(vlib_main_t *vm)
{
    ...
    vec_validate_aligned(bm->per_thread_data, vlib_get_thread_main()->n_vlib_mains - 1, CLIB_CACHE_LINE_BYTES);
