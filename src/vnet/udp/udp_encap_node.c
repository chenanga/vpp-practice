...
always_inline uword udp_encap_inline(vlib_main_t         *vm,
                                     ...
                                     ip_address_family_t  payload_family)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            vlib_increment_combined_counter(cm, thread_index, uei0, 1, vlib_buffer_length_in_chain(vm, b0));
            vlib_increment_combined_counter(cm, thread_index, uei1, 1, vlib_buffer_length_in_chain(vm, b1));
            ...
            if (encap_family == AF_IP6) {
                ...
                if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                    udp6_encap_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
                ...
                if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
                    udp6_encap_trace_t *tr = vlib_add_trace(vm, node, b1, sizeof(*tr));
                ...
                if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                    udp4_encap_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
                ...
                if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
                    udp4_encap_trace_t *tr = vlib_add_trace(vm, node, b1, sizeof(*tr));
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (encap_family == AF_IP6) {
                const u8 n_bytes = sizeof(udp_header_t) + sizeof(ip6_header_t);
                ip_udp_encap_one(vm, b0, (u8 *) &ue0->ue_hdrs.ip6, n_bytes, encap_family, payload_family, ue0->ue_flags);
                ...
                ip_udp_encap_one(vm, b0, (u8 *) &ue0->ue_hdrs.ip4, n_bytes, encap_family, payload_family, ue0->ue_flags);
