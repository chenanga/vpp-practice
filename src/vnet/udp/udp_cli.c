...
static u8 *format_udp_cfg_flags(u8 *s, va_list *args)
{
    ...
    if (last >= 0) s = format(s, " cfg: ");
    for (i = 0; i < last; i++) {
        if (tc->cfg_flags & (1 << i)) s = format(s, "%s, ", udp_cfg_flags_str[i]);
    }
    if (last >= 0) s = format(s, "%s", udp_cfg_flags_str[last]);
    return s;
}
...
static u8 *format_udp_connection_flags(u8 *s, va_list *args)
{
    ...
    for (i = 0; i < UDP_CONN_N_FLAGS; i++)
        if (uc->flags & (1 << i)) last = i;
    for (i = 0; i < last; i++) {
        if (uc->flags & (1 << i)) s = format(s, "%s, ", udp_connection_flags_str[i]);
    }
    if (last >= 0) s = format(s, "%s", udp_connection_flags_str[last]);
    return s;
}
...
static u8 *format_udp_vars(u8 *s, va_list *args)
{
    ...
    s = format(s, " index %u%U flags: %U\n", uc->c_c_index, format_udp_cfg_flags, uc, format_udp_connection_flags, uc);
...
u8 *format_udp_connection(u8 *s, va_list *args)
{
    ...
    if (verbose) {
        s = format(s, "%-" SESSION_CLI_STATE_LEN "s", (uc->flags & UDP_CONN_F_LISTEN) ? "LISTEN" : "OPENED", uc);
...
static void table_format_udp_port_(vlib_main_t *vm, udp_main_t *um, table_t *t, int *c, int port, int bind, int is_ip4)
{
    const udp_dst_port_info_t *pi;

    if (bind && !udp_is_valid_dst_port(port, is_ip4)) return;
    ...
    table_format_cell(t, *c, 2, ~0 == pi->node_index ? "none" : "%U", format_vlib_node_name, vm, pi->node_index);
    table_format_cell(t, *c, 3, "%s", pi->name);

    (*c)++;
}

static void table_format_udp_port(vlib_main_t *vm, udp_main_t *um, table_t *t, int *c, int port, int bind, int ip4, int ip6)
{
    if (ip4) table_format_udp_port_(vm, um, t, c, port, bind, 1 /* is_ip4 */);
    if (ip6) table_format_udp_port_(vm, um, t, c, port, bind, 0 /* is_ip4 */);
}

static clib_error_t *show_udp_ports(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static void table_format_udp_transport_port_(vlib_main_t *vm, table_t *t, int *c, int port, int is_ip4)
{
    udp_main_t *um = &udp_main;
    u32         refcnt;
    u16         port_ne;
...
static void table_format_udp_transport_port(vlib_main_t *vm, table_t *t, int *c, int port, int ipv)
{
    if (ipv == -1 || ipv == 0) table_format_udp_transport_port_(vm, t, c, port, 1 /* is_ip4 */);
    if (ipv == -1 || ipv == 1) table_format_udp_transport_port_(vm, t, c, port, 0 /* is_ip4 */);
}

static clib_error_t *show_udp_transport_ports(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    table_t       table = {}, *t = &table;
