...
static void send_udp_encap_details(const udp_encap_t *ue, vl_api_registration_t *reg, u32 context)
{
    ...
    if (FIB_PROTOCOL_IP4 == ue->ue_ip_proto) {
        clib_memcpy(&mp->udp_encap.src_ip.un.ip4, &ue->ue_hdrs.ip4.ue_ip4.src_address, 4);
        clib_memcpy(&mp->udp_encap.dst_ip.un.ip4, &ue->ue_hdrs.ip4.ue_ip4.dst_address, 4);
        ...
        clib_memcpy(&mp->udp_encap.src_ip.un.ip6, &ue->ue_hdrs.ip6.ue_ip6.src_address, 16);
        clib_memcpy(&mp->udp_encap.dst_ip.un.ip6, &ue->ue_hdrs.ip6.ue_ip6.dst_address, 16);
    ...
    mp->udp_encap.table_id = htonl(fib_table_get_table_id(ue->ue_fib_index, ue->ue_ip_proto));
...
static void vl_api_udp_encap_add_t_handler(vl_api_udp_encap_add_t *mp)
{
    ...
    if (mp->udp_encap.src_port == 0) flags |= UDP_ENCAP_FIXUP_UDP_SRC_PORT_ENTROPY;

    uei = udp_encap_add_and_lock(fproto, fib_index, &src_ip, &dst_ip, ntohs(mp->udp_encap.src_port), ntohs(mp->udp_encap.dst_port), flags);
...
static void vl_api_udp_decap_add_del_t_handler(vl_api_udp_decap_add_del_t *mp)
{
    ...
    if (mp->is_add) {
        u32 node_index = udp_api_decap_proto_to_index(vm, ntohl(mp->udp_decap.next_proto));
        if (node_index == ~0)
            rv = VNET_API_ERROR_INVALID_PROTOCOL;
        else
            udp_register_dst_port(vm, ntohs(mp->udp_decap.port), node_index, mp->udp_decap.is_ip4);
    }
    else
        udp_unregister_dst_port(vm, ntohs(mp->udp_decap.port), mp->udp_decap.is_ip4);
...
static clib_error_t *udp_api_hookup(vlib_main_t *vm)
{
    ...
    vl_api_set_msg_thread_safe(am, REPLY_MSG_ID_BASE + VL_API_UDP_ENCAP_DUMP, 1);
