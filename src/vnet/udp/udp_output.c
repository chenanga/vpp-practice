...
#define udp_node_index(node_id, is_ip4) ((is_ip4) ? udp4_##node_id##_node.index : udp6_##node_id##_node.index)

typedef enum udp_output_next_ { UDP_OUTPUT_NEXT_DROP, UDP_OUTPUT_NEXT_IP_LOOKUP, UDP_OUTPUT_N_NEXT } udp_output_next_t;
...
static u8 *format_udp_tx_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U\n%U%U", format_udp_connection, uc, 1, format_white_space, indent, format_udp_header, &t->udp_header, 128);

    return s;
}

always_inline udp_connection_t *udp_output_get_connection(vlib_buffer_t *b, u32 thread_index)
{
    if (PREDICT_FALSE(vnet_buffer(b)->tcp.flags & UDP_CONN_F_LISTEN)) return udp_listener_get(vnet_buffer(b)->tcp.connection_index);

    return udp_connection_get(vnet_buffer(b)->tcp.connection_index, thread_index);
}

static void udp46_output_trace_frame(vlib_main_t *vm, vlib_node_runtime_t *node, u32 *to_next, u32 n_bufs)
{
    udp_connection_t *uc;
    udp_tx_trace_t   *t;
    vlib_buffer_t    *b;
...
always_inline void udp_output_handle_packet(udp_connection_t *uc0, vlib_buffer_t *b0, vlib_node_runtime_t *error_node, u16 *next0, u8 is_ip4)
{
...
always_inline uword udp46_output_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ip4)
{
