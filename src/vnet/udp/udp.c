...
static void udp_connection_register_port(u16 lcl_port, u8 is_ip4)
{
    ...
    __atomic_add_fetch(&um->transport_ports_refcnt[is_ip4][lcl_port], 1, __ATOMIC_RELAXED);
}
...
static void udp_connection_unregister_port(u16 lcl_port, u8 is_ip4)
{
    ...
    if (__atomic_sub_fetch(&um->transport_ports_refcnt[is_ip4][lcl_port], 1, __ATOMIC_RELAXED)) return;
...
void udp_connection_free(udp_connection_t *uc)
{
    ...
    if (CLIB_DEBUG) clib_memset(uc, 0xFA, sizeof(*uc));
    pool_put(wrk->connections, uc);
}

static void udp_connection_cleanup(udp_connection_t *uc)
{
    transport_release_local_endpoint(TRANSPORT_PROTO_UDP, &uc->c_lcl_ip, uc->c_lcl_port);
...
static void udp_handle_cleanups(void *args)
{
...
static void udp_connection_program_cleanup(udp_connection_t *uc)
{
    ...
    if (vec_len(wrk->pending_cleanups) == 1)
        session_send_rpc_evt_to_thread_force(thread_index, udp_handle_cleanups, uword_to_pointer(thread_index, void *));
}
...
static u32 udp_session_bind(u32 session_index, transport_endpoint_cfg_t *lcl)
{
    udp_main_t               *um = vnet_get_udp_main();
    ...
    if (udp_connection_port_used_extern(clib_net_to_host_u16(lcl->port), lcl->is_ip4)) {
    ...
    listener->mss         = lcl->mss ? lcl->mss : udp_default_mtu(um, listener->c_is_ip4);
    ...
    if (!um->csum_offload) listener->cfg_flags |= UDP_CFG_F_NO_CSUM_OFFLOAD;
...
always_inline u16 udp_compute_checksum(vlib_main_t *vm, vlib_buffer_t *b, u8 csum_offload, u8 is_ip4)
{
    u16 csum = 0;
    ...
    if (csum_offload)
        ...
        if (is_ip4)
            csum = ip4_tcp_udp_compute_checksum(vm, b, vlib_buffer_get_current(b));
        else {
            int bogus = 0;
            csum      = ip6_tcp_udp_icmp_compute_checksum(vm, b, vlib_buffer_get_current(b), &bogus);
        }
    }
...
always_inline u32 udp_push_one_header(vlib_main_t *vm, udp_connection_t *uc, vlib_buffer_t *b, u8 is_cless)
{
    udp_header_t *uh;
    ...
    if (!is_cless) {
        ...
        if (uc->c_is_ip4)
            vlib_buffer_push_ip4_custom(vm, b, &uc->c_lcl_ip4, &uc->c_rmt_ip4, IP_PROTOCOL_UDP, udp_csum_offload(uc), 0 /* is_df */, uc->c_dscp);
        else
            vlib_buffer_push_ip6(vm, b, &uc->c_lcl_ip6, &uc->c_rmt_ip6, IP_PROTOCOL_UDP);
        ...
        if (uc->c_is_ip4)
            vlib_buffer_push_ip4_custom(vm, b, &hdr.lcl_ip.ip4, &hdr.rmt_ip.ip4, IP_PROTOCOL_UDP, udp_csum_offload(uc), 0 /* is_df */, uc->c_dscp);
        else
            vlib_buffer_push_ip6(vm, b, &hdr.lcl_ip.ip6, &hdr.rmt_ip.ip6, IP_PROTOCOL_UDP);
    ...
    uh->checksum = udp_compute_checksum(vm, b, udp_csum_offload(uc), uc->c_is_ip4);

    return 0;
}

always_inline void udp_push_header_batch(udp_connection_t *uc, vlib_buffer_t **bs, u32 n_bufs, u8 is_cless)
{
...
static u32 udp_push_header(transport_connection_t *tc, vlib_buffer_t **bs, u32 n_bufs)
{
    udp_connection_t *uc;
    ...
    if (PREDICT_FALSE(uc->flags & UDP_CONN_F_CLOSING)) {
        if (!transport_tx_fifo_has_dgram(&uc->connection)) udp_connection_program_cleanup(uc);
    }
...
static void udp_session_close(u32 connection_index, u32 thread_index)
{
    ...
    uc = udp_connection_get(connection_index, thread_index);
    if (!uc || (uc->flags & UDP_CONN_F_MIGRATED)) return;
...
static int udp_open_connection(transport_endpoint_cfg_t *rmt)
{
    ...
    rv = transport_alloc_local_endpoint(TRANSPORT_PROTO_UDP, rmt, &lcl_addr, &lcl_port);
    if (rv) return rv;

    if (udp_connection_port_used_extern(clib_net_to_host_u16(lcl_port), rmt->is_ip4)) {
        /* If specific source port was requested abort */
        if (rmt->peer.port) {
            transport_release_local_endpoint(TRANSPORT_PROTO_UDP, &lcl_addr, lcl_port);
            return SESSION_E_PORTINUSE;
        }

        /* Try to find a port that's not used */
        while (udp_connection_port_used_extern(clib_net_to_host_u16(lcl_port), rmt->is_ip4)) {
            transport_release_local_endpoint(TRANSPORT_PROTO_UDP, &lcl_addr, lcl_port);
            lcl_port = transport_alloc_local_port(TRANSPORT_PROTO_UDP, &lcl_addr, rmt);
            if ((int) lcl_port < 1) return SESSION_E_PORTINUSE;
        }
    }
    ...
    if (rmt->peer.sw_if_index != ENDPOINT_INVALID_INDEX) uc->sw_if_index = rmt->peer.sw_if_index;
    ...
    if (!um->csum_offload) uc->cfg_flags |= UDP_CFG_F_NO_CSUM_OFFLOAD;
...
static void udp_realloc_ports_sv(u16 **ports_nh_svp)
{
    ...
    ports_nh_sv = sparse_vec_new(/* elt bytes */ sizeof(ports_nh_sv[0]),
...
static clib_error_t *udp_enable_disable(vlib_main_t *vm, u8 is_en)
{
    udp_main_t *um = &udp_main;

    if (!is_en || um->is_init) return 0;
...
static clib_error_t *udp_init(vlib_main_t *vm)
{
    ...
    if (pi == 0) return clib_error_return(0, "UDP protocol info AWOL");
    pi->format_header    = format_udp_header;
    pi->unformat_pg_edit = unformat_pg_udp_header;

    /* Register as transport with session layer */
    transport_register_protocol(TRANSPORT_PROTO_UDP, &udp_proto, FIB_PROTOCOL_IP4, udp4_output_node.index);
    transport_register_protocol(TRANSPORT_PROTO_UDP, &udp_proto, FIB_PROTOCOL_IP6, udp6_output_node.index);
    ...
    um->local_to_input_edge[UDP_IP4] = vlib_node_add_next(vm, udp4_local_node.index, udp4_input_node.index);
    um->local_to_input_edge[UDP_IP6] = vlib_node_add_next(vm, udp6_local_node.index, udp6_input_node.index);
...
VLIB_INIT_FUNCTION(udp_init) = {
    .runs_after = VLIB_INITS("ip_main_init", "ip4_lookup_init", "ip6_lookup_init"),
};
