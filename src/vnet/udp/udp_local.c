...
typedef enum { UDP_LOCAL_NEXT_PUNT, UDP_LOCAL_NEXT_DROP, UDP_LOCAL_NEXT_ICMP, UDP_LOCAL_N_NEXT } udp_local_next_t;
...
always_inline void udp_dispatch_error(vlib_node_runtime_t *node, vlib_buffer_t *b, u32 advance, u8 is_ip4, u32 *next)
{
    ...
    if (PREDICT_FALSE(punt_unknown)) {
        ...
        *next    = UDP_LOCAL_NEXT_DROP;
        ...
        if (is_ip4) {
            icmp4_error_set_vnet_buffer(b, ICMP4_destination_unreachable, ICMP4_destination_unreachable_port_unreachable, 0);
            b->error = node->errors[UDP_ERROR_NO_LISTENER];
            *next    = UDP_LOCAL_NEXT_ICMP;
        }
        else {
            icmp6_error_set_vnet_buffer(b, ICMP6_destination_unreachable, ICMP6_destination_unreachable_port_unreachable, 0);
            b->error = node->errors[UDP_ERROR_NO_LISTENER];
            *next    = UDP_LOCAL_NEXT_ICMP;
        }
    }
...
always_inline uword udp46_local_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, int is_ip4)
{
    udp_main_t                 *um = &udp_main;
    __attribute__((unused)) u32 n_left_from, next_index, *from, *to_next;
    u16                        *next_by_dst_port = (is_ip4 ? um->next_by_dst_port4 : um->next_by_dst_port6);
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (PREDICT_FALSE(b0->current_length < advance0 + sizeof(*h0))) {
                ...
                if (PREDICT_FALSE(clib_net_to_host_u16(h0->length) > vlib_buffer_length_in_chain(vm, b0))) {
                    b0->error = node->errors[UDP_ERROR_LENGTH_ERROR];
                    next0     = UDP_LOCAL_NEXT_DROP;
                }
            }
            ...
            if (PREDICT_FALSE(b1->current_length < advance1 + sizeof(*h1))) {
                ...
                if (PREDICT_FALSE(clib_net_to_host_u16(h1->length) > vlib_buffer_length_in_chain(vm, b1))) {
                    b1->error = node->errors[UDP_ERROR_LENGTH_ERROR];
                    next1     = UDP_LOCAL_NEXT_DROP;
                }
            }
            ...
            if (PREDICT_TRUE(next0 == UDP_LOCAL_NEXT_PUNT && next1 == UDP_LOCAL_NEXT_PUNT)) {
                sparse_vec_index2(next_by_dst_port, h0->dst_port, h1->dst_port, &i0, &i1);
                next0 = vec_elt(next_by_dst_port, i0);
                next1 = vec_elt(next_by_dst_port, i1);

                if (PREDICT_FALSE(i0 == SPARSE_VEC_INVALID_INDEX || next0 == UDP_NO_NODE_SET)) {
                ...
                if (PREDICT_FALSE(i1 == SPARSE_VEC_INVALID_INDEX || next1 == UDP_NO_NODE_SET)) {
            ...
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                udp_local_rx_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
            ...
            if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
                udp_local_rx_trace_t *tr = vlib_add_trace(vm, node, b1, sizeof(*tr));
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_TRUE(clib_net_to_host_u16(h0->length) <= vlib_buffer_length_in_chain(vm, b0))) {
                i0    = sparse_vec_index(next_by_dst_port, h0->dst_port);
                next0 = vec_elt(next_by_dst_port, i0);
...
void udp_add_dst_port(udp_main_t *um, udp_dst_port_t dst_port, char *dst_port_name, u8 is_ip4)
{
    ...
    if (pi->name) hash_set_mem(um->dst_port_info_by_name[is_ip4], pi->name, i);
}
...
void udp_register_dst_port(vlib_main_t *vm, udp_dst_port_t dst_port, u32 node_index, u8 is_ip4)
{
    ...
    pi->next_index = vlib_node_add_next(vm, is_ip4 ? udp4_local_node.index : udp6_local_node.index, node_index);

    /* Setup udp protocol -> next index sparse vector mapping. */
    if (is_ip4)
        n = sparse_vec_validate(um->next_by_dst_port4, clib_host_to_net_u16(dst_port));
    else
        n = sparse_vec_validate(um->next_by_dst_port6, clib_host_to_net_u16(dst_port));
...
u8 udp_is_valid_dst_port(udp_dst_port_t dst_port, u8 is_ip4)
{
    udp_main_t *um               = &udp_main;
    u16        *next_by_dst_port = is_ip4 ? um->next_by_dst_port4 : um->next_by_dst_port6;
    uword       index            = sparse_vec_index(next_by_dst_port, clib_host_to_net_u16(dst_port));
    return (index != SPARSE_VEC_INVALID_INDEX && vec_elt(next_by_dst_port, index) != UDP_NO_NODE_SET);
}
...
clib_error_t *udp_local_init(vlib_main_t *vm)
{
    ...
    um->next_by_dst_port4 = sparse_vec_new(/* elt bytes */ sizeof(um->next_by_dst_port4[0]),
                                           /* bits in index */ BITS(((udp_header_t *) 0)->dst_port));

    um->next_by_dst_port6 = sparse_vec_new(/* elt bytes */ sizeof(um->next_by_dst_port6[0]),
                                           /* bits in index */ BITS(((udp_header_t *) 0)->dst_port));
