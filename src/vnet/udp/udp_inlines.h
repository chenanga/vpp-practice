...
always_inline void *vlib_buffer_push_udp(vlib_buffer_t *b, u16 sp, u16 dp)
{
    udp_header_t *uh;
    u16           udp_len = sizeof(udp_header_t) + b->current_length;
    if (PREDICT_FALSE(b->flags & VLIB_BUFFER_TOTAL_LENGTH_VALID)) udp_len += b->total_length_not_including_first_buffer;
 ...
 */
always_inline u16 ip_udp_sport_entropy(vlib_buffer_t *b0)
{
...
always_inline u32 ip_udp_compute_flow_hash(vlib_buffer_t *b0, u8 is_ip4)
{
    ip4_header_t *ip4;
    ip6_header_t *ip6;
...
always_inline void ip_udp_fixup_one(vlib_main_t *vm, vlib_buffer_t *b0, u8 is_ip4, u8 sport_entropy)
{
    u16           new_l0;
    udp_header_t *udp0;
    ...
    if (is_ip4) {
        ...
        new_l0       = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, b0) - sizeof(*ip0));
        udp0->length = new_l0;

        if (sport_entropy) udp0->src_port = ip_udp_sport_entropy(b0);
    }
    else {
        ip6_header_t *ip0;
        int           bogus0;
...
always_inline void ip_udp_encap_one(vlib_main_t            *vm,
                                    vlib_buffer_t          *b0,
                                    u8                     *ec0,
                                    word                    ec_len,
                                    ...
                                    udp_encap_fixup_flags_t flags)
{
    ...
    if (payload_family < N_AF) {
        vnet_calc_checksums_inline(vm, b0, payload_family == AF_IP4, payload_family == AF_IP6);

        /* Сalculate flow hash to be used for entropy */
        if (sport_entropy && 0 == vnet_buffer(b0)->ip.flow_hash)
            vnet_buffer(b0)->ip.flow_hash = ip_udp_compute_flow_hash(b0, payload_family == AF_IP4);
    }
...
always_inline void ip_udp_encap_two(vlib_main_t            *vm,
                                    vlib_buffer_t          *b0,
                                    vlib_buffer_t          *b1,
                                    u8                     *ec0,
                                    u8                     *ec1,
                                    ...
                                    udp_encap_fixup_flags_t flags1)
{
    ...
    if (payload_family < N_AF) {
        ...
        /* Сalculate flow hash to be used for entropy */
        if (sport_entropy0 && 0 == vnet_buffer(b0)->ip.flow_hash) vnet_buffer(b0)->ip.flow_hash = ip_udp_compute_flow_hash(b0, payload_ip4);
        if (sport_entropy1 && 0 == vnet_buffer(b1)->ip.flow_hash) vnet_buffer(b1)->ip.flow_hash = ip_udp_compute_flow_hash(b1, payload_ip4);
    }
    ...
    if (encap_family == AF_IP4) {
        ...
        new_l0       = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, b0) - sizeof(*ip0));
        new_l1       = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, b1) - sizeof(*ip1));
        udp0->length = new_l0;
        udp1->length = new_l1;

        if (sport_entropy0) udp0->src_port = ip_udp_sport_entropy(b0);
        if (sport_entropy1) udp1->src_port = ip_udp_sport_entropy(b1);
    }
    else {
