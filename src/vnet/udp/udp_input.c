...
static u8 *format_udp_input_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "UDP_INPUT: connection %d, disposition %d, thread %d", t->connection, t->disposition, t->thread_index);
    return s;
}
...
always_inline void udp_input_inc_counter(vlib_main_t *vm, u8 is_ip4, u8 evt, u8 val)
{
    if (is_ip4)
...
#define udp_store_err_counters(vm, is_ip4, cnts)                        \
    {                                                                   \
        ...
        for (i = 0; i < UDP_N_ERROR; i++)                               \
            if (cnts[i]) udp_input_inc_counter(vm, is_ip4, i, cnts[i]); \
    }
...
static void udp_trace_buffer(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t *b, session_t *s, u16 error0)
{
    ...
    if (PREDICT_TRUE(!(b->flags & VLIB_BUFFER_IS_TRACED))) return;
...
static udp_connection_t *udp_connection_accept(udp_connection_t *listener, session_dgram_hdr_t *hdr, u32 thread_index)
{
    ...
    if (session_dgram_accept(&uc->connection, listener->c_s_index, listener->c_thread_index)) {
...
static void udp_connection_enqueue(udp_connection_t    *uc0,
                                   session_t           *s0,
                                   ...
                                   u32                 *error0)
{
    ...
    if (!(uc0->flags & UDP_CONN_F_CONNECTED)) {
        ...
        wrote0 = session_enqueue_dgram_connection_cl(s0, hdr0, b, TRANSPORT_PROTO_UDP, queue_event);

        clib_spinlock_unlock(&uc0->rx_lock);

        /* Expect cl udp enqueue to fail because fifo enqueue */
        if (PREDICT_FALSE(wrote0 == 0)) *error0 = UDP_ERROR_FIFO_FULL;

        return;
    }

    if (svm_fifo_max_enqueue_prod(s0->rx_fifo) < hdr0->data_length + sizeof(session_dgram_hdr_t)) {
    ...
    if (s0->thread_index != thread_index) {
        wrote0 = session_enqueue_dgram_connection2(s0, hdr0, b, TRANSPORT_PROTO_UDP, queue_event && !svm_fifo_has_event(s0->rx_fifo));
    }
    else {
        wrote0 = session_enqueue_dgram_connection(s0, hdr0, b, TRANSPORT_PROTO_UDP, queue_event);
    }

    /* In some rare cases, session_enqueue_dgram_connection can fail because a
     * chunk cannot be allocated in the RX FIFO */
    if (PREDICT_FALSE(wrote0 == 0)) *error0 = UDP_ERROR_FIFO_NOMEM;
}

always_inline session_t *udp_parse_and_lookup_buffer(vlib_buffer_t *b, session_dgram_hdr_t *hdr, u8 is_ip4)
{
    udp_header_t *udp;
    u32           fib_index;
    session_t    *s;
    ...
    if (is_ip4) {
        ...
        s = session_lookup_safe4(fib_index, &ip4->dst_address, &ip4->src_address, udp->dst_port, udp->src_port, TRANSPORT_PROTO_UDP);
    }
    else {
        ip6_header_t *ip60;
        ...
        s = session_lookup_safe6(fib_index, &ip60->dst_address, &ip60->src_address, udp->dst_port, udp->src_port, TRANSPORT_PROTO_UDP);
    }
...
always_inline uword udp46_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_ip4)
{
    ...
    while (n_left_from > 0) {
        ...
        if (s0->session_state == SESSION_STATE_OPENED) {
            ...
            if (uc0->flags & UDP_CONN_F_CONNECTED) {
                ...
                if (s0->thread_index != thread_index) {
                     ...
                     */
                    uc0 = udp_connection_clone_safe(s0->connection_index, s0->thread_index);
                    ASSERT(s0->session_index == uc0->c_s_index);

                    /*
                     * Ask session layer for a new session.
                     */
                    session_dgram_connect_notify(&uc0->connection, s0->thread_index, &s0);
                    queue_event = 0;
                }
                else
                    s0->session_state = SESSION_STATE_READY;
            }
            udp_connection_enqueue(uc0, s0, &hdr0, thread_index, b[0], queue_event, &error0);
        }
        else if (s0->session_state == SESSION_STATE_READY || s0->session_state == SESSION_STATE_ACCEPTING) {
            uc0 = udp_connection_from_transport(session_get_transport(s0));
            udp_connection_enqueue(uc0, s0, &hdr0, thread_index, b[0], 1, &error0);
        }
        ...
        if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE)) udp_trace_buffer(vm, node, b[0], s0, error0);
