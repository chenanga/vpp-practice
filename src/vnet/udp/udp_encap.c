                               ...
                               udp_encap_fixup_flags_t flags)
{
    ...
    switch (proto) {
        case FIB_PROTOCOL_IP4:
            ...
            ue->ue_hdrs.ip4.ue_ip4.checksum                     = ip4_header_checksum(&ue->ue_hdrs.ip4.ue_ip4);
        ...
        case FIB_PROTOCOL_IP6:
            pfx_len                                                        = 128;
            ue->ue_hdrs.ip6.ue_ip6.ip_version_traffic_class_and_flow_label = clib_host_to_net_u32(6 << 28);
    ...
    ue->ue_fib_entry_index = fib_entry_track(fib_index, &dst_pfx, FIB_NODE_TYPE_UDP_ENCAP, uei, &ue->ue_fib_sibling);
...
u8 *format_udp_encap_fixup_flags(u8 *s, va_list *args)
{
    udp_encap_fixup_flags_t flags = va_arg(*args, udp_encap_fixup_flags_t);

    if (flags == UDP_ENCAP_FIXUP_NONE) return format(s, "none");

    if (flags & UDP_ENCAP_FIXUP_UDP_SRC_PORT_ENTROPY) s = format(s, "%s", "src-port-is-entropy");
...
static u8 *format_udp_encap_i(u8 *s, va_list *args)
{
    ...
    if (FIB_PROTOCOL_IP4 == ue->ue_ip_proto) {
        s = format(s,
                   "ip:[src:%U, dst:%U] udp:[src:%d, dst:%d] flags:%U",
                   format_ip4_address,
    ...
    if (details) {
        ...
        s = format(s, "\n%U%U", format_white_space, indent + 2, format_dpo_id, &ue->ue_dpo, indent + 3);
    }
    return (s);
}
...
static udp_encap_t *udp_encap_from_fib_node(fib_node_t *node)
{
    ...
    return ((udp_encap_t *) (((char *) node) - STRUCT_OFFSET_OF(udp_encap_t, ue_fib_node)));
}
...
clib_error_t *udp_encap_init(vlib_main_t *vm)
{
    ...
    udp_encap_dpo_types[FIB_PROTOCOL_IP4] = dpo_register_new_type(&udp_encap_dpo_vft, udp4_encap_nodes);
    udp_encap_dpo_types[FIB_PROTOCOL_IP6] = dpo_register_new_type(&udp_encap_dpo_vft, udp6_encap_nodes);
...
clib_error_t *udp_encap_cli(vlib_main_t *vm, unformat_input_t *main_input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "index %d", &uei))
            ...
            is_del = 1;
        else if (unformat(line_input, "%U %U", unformat_ip4_address, &src_ip.ip4, unformat_ip4_address, &dst_ip.ip4))
            fproto = FIB_PROTOCOL_IP4;
        else if (unformat(line_input, "%U %U", unformat_ip6_address, &src_ip.ip6, unformat_ip6_address, &dst_ip.ip6))
    ...
    if (!is_del && fproto != FIB_PROTOCOL_MAX) {
        ...
        uei = udp_encap_add_and_lock(fproto, fib_index, &src_ip, &dst_ip, src_port, dst_port, flags);
...
VLIB_CLI_COMMAND(udp_encap_add_command, static) = {
    .path = "udp encap",
    .short_help =
        "udp encap [add|del] <id ID> <src-ip> <dst-ip> [<src-port>] "
