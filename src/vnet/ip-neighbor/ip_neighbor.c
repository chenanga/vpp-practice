...
static ip_neighbor_db_t ip_neighbor_db[N_AF] = {[AF_IP4] =
                                                    {
                                                        .ipndb_limit = 50000,
...
{
    if (pool_is_free_index(ip_neighbor_pool, ipni)) return (NULL);
...
static void ip_neighbor_refresh(ip_neighbor_t *ipn)
{
    ...
    if (ip_neighbor_is_dynamic(ipn)) {
        if (~0 == ipn->ipn_elt) /* first time insertion */
        ...
        head = pool_elt_at_index(ip_neighbor_elt_pool, ip_neighbor_list_head[ip_neighbor_get_af(ipn)]);
...
static void ip_neighbor_db_add(const ip_neighbor_t *ipn)
{
    ...
    if (!ip_neighbor_db[af].ipndb_hash[sw_if_index])
        ip_neighbor_db[af].ipndb_hash[sw_if_index] = hash_create_mem(0, sizeof(ip_neighbor_key_t), sizeof(index_t));

    hash_set_mem(ip_neighbor_db[af].ipndb_hash[sw_if_index], ipn->ipn_key, ip_neighbor_get_index(ipn));
...
static ip_neighbor_t *ip_neighbor_db_find(const ip_neighbor_key_t *key)
{
    ...
    if (key->ipnk_sw_if_index >= vec_len(ip_neighbor_db[af].ipndb_hash)) return NULL;

    p = hash_get_mem(ip_neighbor_db[af].ipndb_hash[key->ipnk_sw_if_index], key);

    if (p) return ip_neighbor_get(p[0]);
...
static void ip_neighbor_adj_fib_add(ip_neighbor_t *ipn, u32 fib_index)
{
    ...
    if (af == AF_IP6 && ip6_address_is_link_local_unicast(&ip_addr_v6(&ipn->ipn_key->ipnk_ip))) {
        ip6_ll_prefix_t pfx = {
            .ilp_addr        = ip_addr_v6(&ipn->ipn_key->ipnk_ip),
            .ilp_sw_if_index = ipn->ipn_key->ipnk_sw_if_index,
        };
        ipn->ipn_fib_entry_index = ip6_ll_table_entry_update(&pfx, FIB_ROUTE_PATH_FLAG_NONE);
    }
    else {
        ...
        ipn->ipn_fib_entry_index = fib_table_entry_path_add(fib_index,
                                                            &pfx,
                                                            FIB_SOURCE_ADJ,
        ...
        if (1 == ip_neighbor_db[af].ipndb_n_elts_per_fib[fib_index]) fib_table_lock(fib_index, fproto, FIB_SOURCE_ADJ);
    }
}
...
static void ip_neighbor_adj_fib_remove(ip_neighbor_t *ipn, u32 fib_index)
{
    ...
    if (FIB_NODE_INDEX_INVALID != ipn->ipn_fib_entry_index) {
        if (AF_IP6 == af && ip6_address_is_link_local_unicast(&ip_addr_v6(&ipn->ipn_key->ipnk_ip))) {
            ip6_ll_prefix_t pfx = {
                .ilp_addr        = ip_addr_v6(&ipn->ipn_key->ipnk_ip),
                .ilp_sw_if_index = ipn->ipn_key->ipnk_sw_if_index,
            };
            ...
            if (0 == ip_neighbor_db[af].ipndb_n_elts_per_fib[fib_index]) fib_table_unlock(fib_index, fproto, FIB_SOURCE_ADJ);
        }
    }
}
...
static void ip_neighbor_mk_complete(adj_index_t ai, ip_neighbor_t *ipn)
{
    adj_nbr_update_rewrite(ai,
                           ...
                           ethernet_build_rewrite(vnet_get_main(), ipn->ipn_key->ipnk_sw_if_index, adj_get_link_type(ai), ipn->ipn_mac.bytes));
}
...
static void ip_neighbor_mk_incomplete(adj_index_t ai)
{
    ...
    adj_nbr_update_rewrite(
        ...
        ethernet_build_rewrite(vnet_get_main(), adj->rewrite_header.sw_if_index, VNET_LINK_ARP, VNET_REWRITE_FOR_SW_INTERFACE_ADDRESS_BROADCAST));
}
...
static void ip_neighbor_destroy(ip_neighbor_t *ipn)
{
    ...
    IP_NEIGHBOR_DBG("free: %U", format_ip_neighbor, vlib_time_now(vlib_get_main()), ip_neighbor_get_index(ipn));
    ...
    adj_nbr_walk_nh(ipn->ipn_key->ipnk_sw_if_index,
                    ...
                    ipn);
    ip_neighbor_adj_fib_remove(ipn, fib_table_get_index_for_sw_if_index(ip_address_family_to_fib_proto(af), ipn->ipn_key->ipnk_sw_if_index));
...
{
    if (!ip_neighbor_db[af].ipndb_recycle) return false;
    ...
    if (clib_llist_is_empty(ip_neighbor_elt_pool, ipne_anchor, head)) return (false);
...
static ip_neighbor_t *ip_neighbor_alloc(const ip_neighbor_key_t *key, const mac_address_t *mac, ip_neighbor_flags_t flags)
{
    ...
    if (ip_neighbor_db[af].ipndb_limit && (ip_neighbor_db[af].ipndb_n_elts >= ip_neighbor_db[af].ipndb_limit)) {
        if (!ip_neighbor_force_reuse(af)) return (NULL);
    }
    ...
    if (!(ipn->ipn_flags & IP_NEIGHBOR_FLAG_NO_FIB_ENTRY))
        ip_neighbor_adj_fib_add(ipn, fib_table_get_index_for_sw_if_index(ip_address_family_to_fib_proto(af), ipn->ipn_key->ipnk_sw_if_index));
...
int ip_neighbor_add(const ip_address_t *ip, const mac_address_t *mac, u32 sw_if_index, ip_neighbor_flags_t flags, u32 *stats_index)
{
    ...
    if (ipn) {
        ...
        if (!(flags & IP_NEIGHBOR_FLAG_STATIC) && (ipn->ipn_flags & IP_NEIGHBOR_FLAG_STATIC)) {
            /* if MAC address match, still check to send event */
            if (0 == mac_address_cmp(&ipn->ipn_mac, mac)) goto check_customers;
            return -2;
        }
        ...
        if (NULL == ipn) return VNET_API_ERROR_LIMIT_EXCEEDED;
    }

    /* Update time stamp and flags. */
    ip_neighbor_refresh(ipn);

    adj_nbr_walk_nh(ipn->ipn_key->ipnk_sw_if_index, fproto, &ip_addr_46(&ipn->ipn_key->ipnk_ip), ip_neighbor_mk_complete_walk, ipn);
...
check_customers:
    ...
    if (stats_index)
        *stats_index = adj_nbr_find(fproto, fib_proto_to_link(fproto), &ip_addr_46(&ipn->ipn_key->ipnk_ip), ipn->ipn_key->ipnk_sw_if_index);
    return 0;
}
...
int ip_neighbor_del(const ip_address_t *ip, u32 sw_if_index)
{
    ...
    IP_NEIGHBOR_INFO("delete: %U, %U", format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index, format_ip_address, ip);
    ...
    if (NULL == ipn) return (VNET_API_ERROR_NO_SUCH_ENTRY);
...
{
    IP_NEIGHBOR_INFO("delete-all: %U, %U", format_ip_address_family, af, format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index);
...
void ip_neighbor_update(vnet_main_t *vnm, adj_index_t ai)
{
    ...
    ip_address_from_46(&adj->sub_type.nbr.next_hop, adj->ia_nh_proto, &key.ipnk_ip);
    ...
    switch (adj->lookup_next_index) {
        case IP_LOOKUP_NEXT_ARP:
            if (NULL != ipn) {
                adj_nbr_walk_nh(adj->rewrite_header.sw_if_index, adj->ia_nh_proto, &adj->sub_type.nbr.next_hop, ip_neighbor_mk_complete_walk, ipn);
            }
            else {
                /*
                ...
                adj_nbr_update_rewrite(
                    ai,
                    ADJ_NBR_REWRITE_FLAG_INCOMPLETE,
                    ethernet_build_rewrite(vnm, adj->rewrite_header.sw_if_index, VNET_LINK_ARP, VNET_REWRITE_FOR_SW_INTERFACE_ADDRESS_BROADCAST));
        ...
        case IP_LOOKUP_NEXT_REWRITE:
            ...
            if (NULL != ipn) ip_neighbor_mk_complete(ai, ipn);
            break;
...
{
    ip_neighbor_add(&l->ip, &l->mac, l->sw_if_index, IP_NEIGHBOR_FLAG_DYNAMIC, NULL);
}
...
static clib_error_t *ip_neighbor_cmd(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        if (unformat(input, "%U %U %U", unformat_vnet_sw_interface, vnm, &sw_if_index, unformat_ip_address, &ip, unformat_mac_address_t, &mac))
            ;
    ...
    if (sw_if_index == ~0 || ip_address_is_zero(&ip) || mac_address_is_zero(&mac))
        return clib_error_return(0, "specify interface, IP address and MAC: `%U'", format_unformat_error, input);
...
VLIB_CLI_COMMAND(ip_neighbor_command, static) = {
    ...
    .short_help =
        "set ip neighbor [del] <intfc> <ip-address> <mac-address> "
...
VLIB_CLI_COMMAND(ip_neighbor_command2, static) = {
    .path = "ip neighbor",
    .short_help =
        "ip neighbor [del] [flush] <intfc> <ip-address> <mac-address> "
        "[static] [no-fib-entry] [count <count>]",
    .function = ip_neighbor_cmd,
};
...
static int ip_neighbor_sort(void *a1, void *a2)
{
    ...
    cmp = vnet_sw_interface_compare(vnet_get_main(), ipn1->ipn_key->ipnk_sw_if_index, ipn2->ipn_key->ipnk_sw_if_index);
    if (!cmp) cmp = ip_address_cmp(&ipn1->ipn_key->ipnk_ip, &ipn2->ipn_key->ipnk_ip);
    return cmp;
}
...
static index_t *ip_neighbor_entries(u32 sw_if_index, ip_address_family_t af)
{
    ...
    pool_foreach (ipn, ip_neighbor_pool) {
        if ((sw_if_index == ~0 || ipn->ipn_key->ipnk_sw_if_index == sw_if_index) && (N_AF == af || ip_neighbor_get_af(ipn) == af))
            vec_add1(ipnis, ip_neighbor_get_index(ipn));
    }

    if (ipnis) vec_sort_with_function(ipnis, ip_neighbor_sort);
    return ipnis;
}
...
static clib_error_t *ip_neighbor_show_sorted_i(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd, ip_address_family_t af)
{
    ...
    vlib_cli_output(vm, "%=12s%=40s%=6s%=20s%=24s", "Age", "IP", "Flags", "Ethernet", "Interface");
    ...
    clib_llist_foreach_reverse(ip_neighbor_elt_pool, ipne_anchor, head, elt, ({
                                   vlib_cli_output(vm, "%U", format_ip_neighbor, now, elt->ipne_index);
                               }));
...
static clib_error_t *ip_neighbor_show_i(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd, ip_address_family_t af)
{
    ...
    sw_if_index = ~0;
    (void) unformat_user(input, unformat_vnet_sw_interface, vnet_get_main(), &sw_if_index);

    ipnis = ip_neighbor_entries(sw_if_index, af);
    now   = vlib_time_now(vm);

    if (ipnis) vlib_cli_output(vm, "%=12s%=40s%=6s%=20s%=24s", "Age", "IP", "Flags", "Ethernet", "Interface");
...
void ip_neighbor_probe_dst(u32 sw_if_index, u32 thread_index, ip_address_family_t af, const ip46_address_t *dst)
{
    if (!vnet_sw_interface_is_admin_up(vnet_get_main(), sw_if_index)) return;
    ...
    if (~0 == sw_if_index) {
        ...
        vec_foreach (hash, ip_neighbor_db[af].ipndb_hash) {
            hash_foreach(key, ipni, *hash, ({
                             if (WALK_STOP == cb(ipni, ctx)) break;
                         }));
        }
    }
    else {
        ...
        if (vec_len(ip_neighbor_db[af].ipndb_hash) <= sw_if_index) return;
...
void ip_neighbor_populate(ip_address_family_t af, u32 sw_if_index)
{
    ...
    IP_NEIGHBOR_DBG("populate: %U %U", format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index, format_ip_address_family, af);

    pool_foreach (ipn, ip_neighbor_pool) {
        if (ip_neighbor_get_af(ipn) == af && ipn->ipn_key->ipnk_sw_if_index == sw_if_index) vec_add1(ipnis, ipn - ip_neighbor_pool);
    }
...
void ip_neighbor_flush(ip_address_family_t af, u32 sw_if_index)
{
    ...
    IP_NEIGHBOR_DBG("flush: %U %U", format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index, format_ip_address_family, af);

    pool_foreach (ipn, ip_neighbor_pool) {
        if (ip_neighbor_get_af(ipn) == af && ipn->ipn_key->ipnk_sw_if_index == sw_if_index && ip_neighbor_is_dynamic(ipn))
...
walk_rc_t ip_neighbor_mark_one(index_t ipni, void *ctx)
{
    ip_neighbor_t *ipn;
 ...
 * Remove any arp entries associated with the specified interface
 */
static clib_error_t *ip_neighbor_add_del_sw_interface(vnet_main_t *vnm, u32 sw_if_index, u32 is_add)
{
    IP_NEIGHBOR_DBG("interface-change: %U  %s", format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index, (is_add ? "add" : "del"));
...
static walk_rc_t ip_neighbor_walk_covered(index_t ipni, void *arg)
{
    ...
    if (AF_IP4 == ip_addr_version(&ctx->addr)) {
        if (ip4_destination_matches_route(&ip4_main, &ip_addr_v4(&ipn->ipn_key->ipnk_ip), &ip_addr_v4(&ctx->addr), ctx->length)
            && ip_neighbor_is_dynamic(ipn)) {
            vec_add1(ctx->ipnis, ip_neighbor_get_index(ipn));
        }
    }
    else if (AF_IP6 == ip_addr_version(&ctx->addr)) {
        if (ip6_destination_matches_route(&ip6_main, &ip_addr_v6(&ipn->ipn_key->ipnk_ip), &ip_addr_v6(&ctx->addr), ctx->length)
            && ip_neighbor_is_dynamic(ipn)) {
            vec_add1(ctx->ipnis, ip_neighbor_get_index(ipn));
        }
    }
...
static ip_neighbor_age_state_t ip_neighbour_age_out(index_t ipni, f64 now, f64 *wait)
{
    ...
    *wait     = ipndb_age;
    ...
    if (ttl > ipndb_age) {
        IP_NEIGHBOR_DBG("aged: %U @%f - %f > %d", format_ip_neighbor, now, ipni, now, ipn->ipn_time_last_updated, ipndb_age);
        if (ipn->ipn_n_probes > 2) {
            ...
            *wait = 1;
            ...
            ip_neighbor_probe_dst(ip_neighbor_get_sw_if_index(ipn), vlib_get_thread_index(), af, &ip_addr_46(&ipn->ipn_key->ipnk_ip));
...
int ip_neighbor_config(ip_address_family_t af, u32 limit, u32 age, bool recycle)
{
    ...
    vlib_process_signal_event(vlib_get_main(),
                              (AF_IP4 == af ? ip4_neighbor_age_process_node.index : ip6_neighbor_age_process_node.index),
                              IP_NEIGHBOR_AGE_PROCESS_WAKEUP,
                              0);
...
int ip_neighbor_get_config(ip_address_family_t af, u32 *limit, u32 *age, bool *recycle)
{
...
static void ip_neighbor_stats_show_one(vlib_main_t *vm, vnet_main_t *vnm, u32 sw_if_index)
{
    vlib_cli_output(vm, "  %U", format_vnet_sw_if_index_name, vnm, sw_if_index);
    vlib_cli_output(vm, "    arp:%U", format_ip_neighbor_counters, &ip_neighbor_counters[AF_IP4], sw_if_index);
    vlib_cli_output(vm, "    nd: %U", format_ip_neighbor_counters, &ip_neighbor_counters[AF_IP6], sw_if_index);
}

static walk_rc_t ip_neighbor_stats_show_cb(vnet_main_t *vnm, vnet_sw_interface_t *si, void *ctx)
{
...
static clib_error_t *ip_neighbor_stats_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vnet_main_t *vnm;
    u32          sw_if_index;
...
static clib_error_t *ip_neighbor_init(vlib_main_t *vm)
{
    ...
    FOR_EACH_IP_ADDRESS_FAMILY(af)
    ip_neighbor_list_head[af] = clib_llist_make_head(ip_neighbor_elt_pool, ipne_anchor);
