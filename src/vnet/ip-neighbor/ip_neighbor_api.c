...
static vl_api_ip_neighbor_flags_t ip_neighbor_flags_encode(ip_neighbor_flags_t f)
{
    ...
    if (f & IP_NEIGHBOR_FLAG_STATIC) v |= IP_API_NEIGHBOR_FLAG_STATIC;
    if (f & IP_NEIGHBOR_FLAG_NO_FIB_ENTRY) v |= IP_API_NEIGHBOR_FLAG_NO_FIB_ENTRY;
...
void ip_neighbor_handle_event(ip_neighbor_event_t *ipne)
{
    ...
    if (NULL == ipn) /* Client can cancel, die, etc. */
        return;
    ...
    if (vl_api_can_send_msg(reg)) {
        ...
        if (vlib_time_now(vlib_get_main()) > last_time + 10.0) {
            clib_warning("neighbor event for %U to pid %d: queue stuffed!", format_ip_address, &ipn->ipn_key->ipnk_ip, ipne->ipne_watch.ipw_pid);
...
static walk_rc_t send_ip_neighbor_details(index_t ipni, void *arg)
{
    ...
    mp->age        = clib_host_to_net_f64((vlib_time_now(vlib_get_main()) - ipn->ipn_time_last_updated));
    ip_neighbor_encode(&mp->neighbor, ipn);
...
static ip_neighbor_flags_t ip_neighbor_flags_decode(vl_api_ip_neighbor_flags_t v)
{
    ...
    if (v & IP_API_NEIGHBOR_FLAG_STATIC) f |= IP_NEIGHBOR_FLAG_STATIC;
    if (v & IP_API_NEIGHBOR_FLAG_NO_FIB_ENTRY) f |= IP_NEIGHBOR_FLAG_NO_FIB_ENTRY;
...
static void vl_api_ip_neighbor_add_del_t_handler(vl_api_ip_neighbor_add_del_t *mp, vlib_main_t *vm)
{
    ...
    if (!(flags & IP_NEIGHBOR_FLAG_STATIC) && !(flags & IP_NEIGHBOR_FLAG_DYNAMIC)) flags |= IP_NEIGHBOR_FLAG_DYNAMIC;
     ...
     */
    if (mp->is_add)
        rv = ip_neighbor_add(&ip, &mac, ntohl(mp->neighbor.sw_if_index), flags, &stats_index);
    else
        rv = ip_neighbor_del(&ip, ntohl(mp->neighbor.sw_if_index));

    BAD_SW_IF_INDEX_LABEL;

    REPLY_MACRO2(VL_API_IP_NEIGHBOR_ADD_DEL_REPLY, ({ rmp->stats_index = htonl(stats_index); }));
}
...
static void vl_api_ip_neighbor_config_t_handler(vl_api_ip_neighbor_config_t *mp)
{
    ...
    if (!rv) rv = ip_neighbor_config(af, ntohl(mp->max_number), ntohl(mp->max_age), mp->recycle);

    REPLY_MACRO(VL_API_IP_NEIGHBOR_CONFIG_REPLY);
}

static void vl_api_ip_neighbor_config_get_t_handler(vl_api_ip_neighbor_config_get_t *mp)
{
    ...
    if (!rv) rv = ip_neighbor_get_config(af, &max_number, &max_age, &recycle);
...
static void vl_api_ip_neighbor_flush_t_handler(vl_api_ip_neighbor_flush_t *mp)
{
    ...
    if (!rv) ip_neighbor_del_all(af, ntohl(mp->sw_if_index));
