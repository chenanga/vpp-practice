...
#define _(a, b, c, d) \
    if (flags & IP_NEIGHBOR_FLAG_##a) s = format(s, "%s", d);
...
u8 *format_ip_neighbor_key(u8 *s, va_list *va)
{
    ...
    return (format(s, "[%U, %U]", format_vnet_sw_if_index_name, vnet_get_main(), key->ipnk_sw_if_index, format_ip_address, &key->ipnk_ip));
}
...
u8 *format_ip_neighbor_watcher(u8 *s, va_list *va)
{
    ...
    return (format(s, "[pid:%d, client:%d]", clib_host_to_net_u32(watcher->ipw_pid), clib_host_to_net_u32(watcher->ipw_client)));
}
...
static void ip_neighbor_alloc_one_ctr(ip_neighbor_counters_t *ctr, vlib_dir_t dir, ip_neighbor_counter_type_t type, u32 sw_if_index)
{
    vlib_validate_simple_counter(&(ctr->ipnc[dir][type]), sw_if_index);
    vlib_zero_simple_counter(&(ctr->ipnc[dir][type]), sw_if_index);
}

void ip_neighbor_alloc_ctr(ip_neighbor_counters_t *ctr, u32 sw_if_index)
{
...
u8 *format_ip_neighbor_counters(u8 *s, va_list *args)
{
...
#define _(a, b) s = format(s, "%s:%lld ", b, vlib_get_simple_counter(&ctr->ipnc[dir][IP_NEIGHBOR_CTR_##a], sw_if_index));
