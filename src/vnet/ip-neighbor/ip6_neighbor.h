...
extern void ip6_neighbor_advertise(vlib_main_t *vm, vnet_main_t *vnm, u32 sw_if_index, u32 thread_index, const ip6_address_t *addr);

extern void ip6_neighbor_probe_dst(u32 sw_if_index, u32 thread_index, const ip6_address_t *dst);

always_inline vlib_buffer_t *ip6_neighbor_probe(vlib_main_t         *vm,
                                                vnet_main_t         *vnm,
                                                u32                  sw_if_index,
                                                u32                  thread_index,
                                                ...
                                                const ip6_address_t *dst)
{
    ...
    h0 = vlib_packet_template_get_packet(vm, &ip6_neighbor_packet_template, &bi0);
    if (!h0) return NULL;

    /* if the interface has been disabled for ip6, later steps to retrieve
     * an adjacency will result in a segv.
     */
    if (!ip6_link_is_enabled(sw_if_index)) return NULL;
    ...
    clib_memcpy(h0->link_layer_option.ethernet_address, hw_if0->hw_address, vec_len(hw_if0->hw_address));

    /* $$$$ appears we need this; why is the checksum non-zero? */
    h0->neighbor.icmp.checksum = 0;
    h0->neighbor.icmp.checksum = ip6_tcp_udp_icmp_compute_checksum(vm, 0, &h0->ip, &bogus_length);
    ...
    vlib_increment_simple_counter(&ip_neighbor_counters[AF_IP6].ipnc[VLIB_TX][IP_NEIGHBOR_CTR_REQUEST], thread_index, sw_if_index, 1);
