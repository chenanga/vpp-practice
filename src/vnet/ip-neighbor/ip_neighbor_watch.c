...
static clib_error_t *want_ip_neighbor_events_reaper(u32 client_index)
{
    ...
    mhash_foreach(key, v, &ipnw_db.ipnwdb_hash, ({
                      ...
                      vec_foreach_index_backwards (pos, watchers) {
                          if (watchers[pos].ipw_client == client_index) vec_del1(watchers, pos);
                      }

                      if (vec_len(watchers) == 0) vec_add1(empty_keys, *key);
                  }));
...
void ip_neighbor_watch(const ip_address_t *ip, u32 sw_if_index, const ip_neighbor_watcher_t *watch)
{
    ...
    if (p) {
        ...
        if (~0 != vec_search_with_function(ipws, watch, ip_neighbor_watch_cmp)) /* duplicate */
            return;
    }
...
void ip_neighbor_unwatch(const ip_address_t *ip, u32 sw_if_index, const ip_neighbor_watcher_t *watch)
{
    ...
    if (vec_len(ipws) == 0) mhash_unset(&ipnw_db.ipnwdb_hash, &key, NULL);
}
...
static void ip_neighbor_signal(ip_neighbor_watcher_t *watchers, index_t ipni, ip_neighbor_event_flags_t flags)
{
    ...
    vec_foreach (watcher, watchers) {
        ...
        ipne             = vlib_process_signal_event_data(vlib_get_main(), ip_neighbor_event_process_node.index, 0, 1, sizeof(*ipne));
...
{
    mhash_init(&ipnw_db.ipnwdb_hash, sizeof(ip_neighbor_watcher_t *), sizeof(ip_neighbor_key_t));
    return (NULL);
}
