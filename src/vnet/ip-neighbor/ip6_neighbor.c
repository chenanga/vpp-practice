...
void ip6_neighbor_probe_dst(u32 sw_if_index, u32 thread_index, const ip6_address_t *dst)
{
    ip6_address_t src;

    if (fib_sas6_get(sw_if_index, dst, &src) || ip6_sas_by_sw_if_index(sw_if_index, dst, &src))
        ip6_neighbor_probe(vlib_get_main(), vnet_get_main(), sw_if_index, thread_index, &src, dst);
}

void ip6_neighbor_advertise(vlib_main_t *vm, vnet_main_t *vnm, u32 sw_if_index, u32 thread_index, const ip6_address_t *addr)
{
    ...
    if (NULL == addr) addr = ip6_interface_first_address(i6m, sw_if_index);

    if (addr) {
        log_debug("Sending unsolicitated NA IP6 address %U on sw_if_idex %d", format_ip6_address, addr, sw_if_index);

        /* Form unsolicited neighbor advertisement packet from NS pkt template */
        int                                   bogus_length;
        u32                                   bi = 0;
        icmp6_neighbor_solicitation_header_t *h  = vlib_packet_template_get_packet(vm, &ip6_neighbor_packet_template, &bi);
        if (!h) return;

        ip6_set_reserved_multicast_address(&h->ip.dst_address, IP6_MULTICAST_SCOPE_link_local, IP6_MULTICAST_GROUP_ID_all_hosts);
        h->ip.src_address                = addr[0];
        h->neighbor.icmp.type            = ICMP6_neighbor_advertisement;
        h->neighbor.target_address       = addr[0];
        h->neighbor.advertisement_flags  = clib_host_to_net_u32(ICMP6_NEIGHBOR_ADVERTISEMENT_FLAG_OVERRIDE);
        ...
        h->neighbor.icmp.checksum = ip6_tcp_udp_icmp_compute_checksum(vm, 0, &h->ip, &bogus_length);
        ASSERT(bogus_length == 0);

        /* Setup MAC header with IP6 Etype and mcast DMAC */
        vlib_buffer_t *b = vlib_get_buffer(vm, bi);
        ip6_multicast_ethernet_address(dst_address, IP6_MULTICAST_GROUP_ID_all_hosts);
        rewrite     = ethernet_build_rewrite(vnm, sw_if_index, VNET_LINK_IP6, dst_address);
        ...
        vlib_increment_simple_counter(&ip_neighbor_counters[AF_IP6].ipnc[VLIB_TX][IP_NEIGHBOR_CTR_GRAT], thread_index, sw_if_index, 1);
    }
}
...
static uword ip6_discover_neighbor_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_glean)
{
    ...
    while (n_left_from > 0) {
        vlib_get_next_frame(vm, node, IP6_NBR_NEXT_DROP, to_next_drop, n_left_to_next_drop);
        ...
        while (n_left_from > 0 && n_left_to_next_drop > 0) {
            ...
            if (!(hw_if0->flags & VNET_HW_INTERFACE_FLAG_LINK_UP)) drop0 = 1;

            if (!ip6_link_is_enabled(sw_if_index0)) drop0 = 1;
             ...
             */
            if (IP_LOOKUP_NEXT_REWRITE == adj0->lookup_next_index) drop0 = 1;
            ...
            b0 = ip6_neighbor_probe(vm, vnm, sw_if_index0, thread_index, &src, &ip0->dst_address);
...
static clib_error_t *ip6_neighbor_init(vlib_main_t *vm)
{
    ...
    p.ip.payload_length                          = clib_host_to_net_u16(sizeof(p) - STRUCT_OFFSET_OF(icmp6_neighbor_solicitation_header_t, neighbor));
