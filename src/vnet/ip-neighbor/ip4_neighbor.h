...
extern void ip4_neighbor_probe_dst(u32 sw_if_index, u32 thread_index, const ip4_address_t *dst);
extern void ip4_neighbor_advertise(vlib_main_t *vm, vnet_main_t *vnm, u32 sw_if_index, u32 thread_index, const ip4_address_t *addr);

always_inline vlib_buffer_t *ip4_neighbor_probe(vlib_main_t          *vm,
                                                vnet_main_t          *vnm,
                                                ...
                                                const ip4_address_t  *dst)
{
    ...
    /* Send ARP request. */
    h0 = vlib_packet_template_get_packet(vm, &ip4_main.ip4_arp_request_packet_template, &bi0);
    /* Seems we're out of buffers */
    if (PREDICT_FALSE(!h0)) return (NULL);
    ...
    vlib_increment_simple_counter(&ip_neighbor_counters[AF_IP4].ipnc[VLIB_TX][IP_NEIGHBOR_CTR_REQUEST],
                                  vm->thread_index,
