...
static clib_error_t *monitor_interface_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    const vnet_main_t                  *vnm              = vnet_get_main();
    const vlib_combined_counter_main_t *counters         = vnm->interface_main.combined_sw_if_counters;
    ...
    vlib_get_combined_counter(counters + VNET_INTERFACE_COUNTER_RX, hw_if_index, &vrx[spin]);
    vlib_get_combined_counter(counters + VNET_INTERFACE_COUNTER_TX, hw_if_index, &vtx[spin]);
    ts[spin] = vlib_time_now(vm);

    while (refresh_count--) {
        f64 sleep_interval, tsd;

        while (((sleep_interval = ts[spin] + refresh_interval - vlib_time_now(vm)) > 0.0)) {
...
VLIB_CLI_COMMAND(monitor_interface_command, static) = {
    ...
    .short_help = "monitor interface <interface> [interval <intv>] [count <count>]",
