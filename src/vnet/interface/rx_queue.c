...
static u32 next_thread_index(vnet_main_t *vnm, u32 thread_index)
{
    ...
    if (thread_index != 0 && (thread_index < vdm->first_worker_thread_index || thread_index > vdm->last_worker_thread_index)) {
        thread_index = vdm->next_worker_thread_index++;
        if (vdm->next_worker_thread_index > vdm->last_worker_thread_index) vdm->next_worker_thread_index = vdm->first_worker_thread_index;
    }
...
u32 vnet_hw_if_register_rx_queue(vnet_main_t *vnm, u32 hw_if_index, u32 queue_id, u32 thread_index)
{
    ...
    hash_set_mem_alloc(&im->rxq_index_by_hw_if_index_and_queue_id, &key, queue_index);
    ...
    log_debug("register: interface %v queue-id %u thread %u", hi->name, queue_id, thread_index);
...
void vnet_hw_if_unregister_all_rx_queues(vnet_main_t *vnm, u32 hw_if_index)
{
    ...
    for (int i = 0; i < vec_len(hi->rx_queue_indices); i++) {
        ...
        key = rx_queue_key(rxq->hw_if_index, rxq->queue_id);
        if (PREDICT_FALSE(rxq->mode == VNET_HW_IF_RX_MODE_INTERRUPT || rxq->mode == VNET_HW_IF_RX_MODE_ADAPTIVE)) {
            vm          = vlib_get_main_by_index(rxq->thread_index);
            queue_index = vnet_hw_if_get_rx_queue_index_by_id(vnm, hw_if_index, rxq->queue_id);
...
void vnet_hw_if_set_rx_queue_file_index(vnet_main_t *vnm, u32 queue_index, u32 file_index)
{
    ...
    log_debug("set_file_index: interface %v queue-id %u file-index %u", hi->name, rxq->queue_id, file_index);
}
...
void vnet_hw_if_set_input_node(vnet_main_t *vnm, u32 hw_if_index, u32 node_index)
{
    ...
    log_debug("set_input_node: node %U for interface %v", format_vlib_node_name, vm, node_index, hi->name);
}
...
int vnet_hw_if_set_rx_queue_mode(vnet_main_t *vnm, u32 queue_index, vnet_hw_if_rx_mode mode)
{
    ...
    if (mode == VNET_HW_IF_RX_MODE_DEFAULT) mode = hi->default_rx_mode;
    ...
    if (dc->rx_mode_change_function) {
        clib_error_t *err = dc->rx_mode_change_function(vnm, rxq->hw_if_index, rxq->queue_id, mode);
        if (err) {
            log_err(
    ...
    rxq->mode = mode;
    log_debug("set_rx_queue_mode: interface %v queue-id %u mode set to %U", hi->name, rxq->queue_id, format_vnet_hw_if_rx_mode, mode);
    return 0;
}
...
void vnet_hw_if_set_rx_queue_thread_index(vnet_main_t *vnm, u32 queue_index, u32 thread_index)
{
    ...
    if (rxq->file_index != ~0) clib_file_set_polling_thread(&file_main, rxq->file_index, thread_index);
...
vnet_hw_if_rxq_poll_vector_t *vnet_hw_if_generate_rxq_int_poll_vector(vlib_main_t *vm, vlib_node_runtime_t *node)
{
    ...
    while ((int_num = clib_interrupt_get_next_and_clear(rt->rxq_interrupts, int_num)) != -1) {
