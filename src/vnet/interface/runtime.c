...
void vnet_hw_if_update_runtime_data(vnet_main_t *vnm, u32 hw_if_index)
{
    ...
    log_debug("update node '%U' triggered by interface %v", format_vlib_node_name, vm, node_index, hi->name);

    vec_validate(d, n_threads - 1);
    vec_validate(a, n_threads - 1);
    vec_validate_init_empty(per_thread_node_state, n_threads - 1, VLIB_NODE_STATE_DISABLED);
    ...
    pool_foreach (rxq, im->hw_if_rx_queues) {
        ...
        if (per_thread_node_state[ti] == VLIB_NODE_STATE_POLLING) continue;

        if (rxq->mode == VNET_HW_IF_RX_MODE_INTERRUPT || rxq->mode == VNET_HW_IF_RX_MODE_ADAPTIVE)
            per_thread_node_state[ti] = VLIB_NODE_STATE_INTERRUPT;

        if (rxq->mode == VNET_HW_IF_RX_MODE_ADAPTIVE) per_thread_node_adaptive[ti] = 1;
    }
    ...
    for (int i = 0; i < n_threads; i++) {
        ...
        if (something_changed_on_rx == 0) {
            ...
            if (vec_len(rt->rxq_vector_int) != vec_len(d[i]))
                something_changed_on_rx = 1;
            else if (memcmp(d[i], rt->rxq_vector_int, vec_len(d[i]) * sizeof(**d)))
                something_changed_on_rx = 1;
            if (clib_interrupt_get_n_int(rt->rxq_interrupts) != last_int + 1) something_changed_on_rx = 1;

            if (something_changed_on_rx == 0 && per_thread_node_adaptive[i]) {
                if (vec_len(rt->rxq_vector_poll) != vec_len(a[i]))
                    something_changed_on_rx = 1;
                else if (memcmp(a[i], rt->rxq_vector_poll, vec_len(a[i]) * sizeof(**a)))
                    something_changed_on_rx = 1;
            }
        }
    }
    ...
    if (vec_len(hi->tx_queue_indices) > 0) {
        new_out_runtimes = vec_dup_aligned(hi->output_node_thread_runtimes, CLIB_CACHE_LINE_BYTES);
        vec_validate_aligned(new_out_runtimes, n_threads - 1, CLIB_CACHE_LINE_BYTES);
        ...
        for (u32 i = 0; i < vec_len(new_out_runtimes); i++) {
            ...
            for (u32 j = 0; j < total_queues; j++) {
                ...
                vnet_hw_if_tx_frame_t  frame       = {.shared_queue = 0, .hints = 7, .queue_id = ~0};
                vnet_hw_if_tx_queue_t *txq         = vnet_hw_if_get_tx_queue(vnm, queue_index);
                if (!clib_bitmap_get(txq->threads, i)) continue;
             ...
             */
            if (rt->n_queues > 0) {
                if (!is_pow2(n_queues)) n_queues = max_pow2(n_queues);

                vec_validate_aligned(rt->lookup_table, n_queues - 1, CLIB_CACHE_LINE_BYTES);

                for (u32 k = 0; k < vec_len(rt->lookup_table); k++) {
                    rt->lookup_table[k] = rt->frame[k % rt->n_queues].queue_id;
                    log_debug(
                        "tx queue lookup table changed for interface %v, "
                        "(lookup table [%u]=%u)",
                        hi->name,
                        k,
                        rt->lookup_table[k]);
    ...
    if (something_changed_on_rx || something_changed_on_tx) {
        ...
        if (something_changed_on_rx) {
            for (int i = 0; i < n_threads; i++) {
                ...
                if (rt->rxq_interrupts) {
                    ...
                    int   int_num = -1;
                    while ((int_num = clib_interrupt_get_next_and_clear(in, int_num)) != -1) {
                ...
                vlib_node_set_flag(vm, node_index, VLIB_NODE_FLAG_ADAPTIVE_MODE, per_thread_node_adaptive[i]);
        ...
        log_debug("skipping update of node '%U', no changes detected", format_vlib_node_name, vm, node_index);
