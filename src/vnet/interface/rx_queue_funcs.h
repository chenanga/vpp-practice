...
static_always_inline vnet_hw_if_rx_queue_t *vnet_hw_if_get_rx_queue(vnet_main_t *vnm, u32 queue_index)
{
    ...
    if (pool_is_free_index(im->hw_if_rx_queues, queue_index)) return 0;
...
static_always_inline void vnet_hw_if_rx_queue_set_int_pending(vnet_main_t *vnm, u32 queue_index)
{
    ...
    if (PREDICT_FALSE(rxq->mode != VNET_HW_IF_RX_MODE_INTERRUPT && rxq->mode != VNET_HW_IF_RX_MODE_ADAPTIVE)) return;
...
static_always_inline int vnet_hw_if_rxq_cmp_cli_api(vnet_hw_if_rx_queue_t **a, vnet_hw_if_rx_queue_t **b)
{
    ...
    if (a[0]->thread_index != b[0]->thread_index) return 2 * (a[0]->thread_index > b[0]->thread_index) - 1;

    vnm   = vnet_get_main();
    hif_a = vnet_get_hw_interface(vnm, a[0]->hw_if_index);
    hif_b = vnet_get_hw_interface(vnm, b[0]->hw_if_index);

    if (hif_a->input_node_index != hif_b->input_node_index) return 2 * (hif_a->input_node_index > hif_b->input_node_index) - 1;

    if (a[0]->hw_if_index != b[0]->hw_if_index) return 2 * (a[0]->hw_if_index > b[0]->hw_if_index) - 1;

    if (a[0]->queue_id != b[0]->queue_id) return 2 * (a[0]->queue_id > b[0]->queue_id) - 1;
