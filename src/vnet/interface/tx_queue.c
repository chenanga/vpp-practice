...
u32 vnet_hw_if_register_tx_queue(vnet_main_t *vnm, u32 hw_if_index, u32 queue_id)
{
    ...
    hash_set_mem_alloc(&im->txq_index_by_hw_if_index_and_queue_id, &key, queue_index);
...
void vnet_hw_if_tx_queue_assign_thread(vnet_main_t *vnm, u32 queue_index, u32 thread_index)
{
    ...
    if (clib_bitmap_count_set_bits(txq->threads) > 1) txq->shared_queue = 1;
    log_debug("assign_thread: interface %v queue-id %u thread %u queue-shared %s",
              hi->name,
              txq->queue_id,
              thread_index,
...
void vnet_hw_if_tx_queue_unassign_thread(vnet_main_t *vnm, u32 queue_index, u32 thread_index)
{
    ...
    if (clib_bitmap_count_set_bits(txq->threads) < 2) txq->shared_queue = 0;
    log_debug("unassign_thread: interface %v queue-id %u thread %u queue-shared %s",
              hi->name,
              txq->queue_id,
              thread_index,
              (txq->shared_queue == 1 ? "yes" : "no"));
