...
#define log_debug(fmt, ...) vlib_log_debug(if_caps_log.class, fmt, __VA_ARGS__)
...
void vnet_hw_if_change_caps(vnet_main_t *vnm, u32 hw_if_index, vnet_hw_if_caps_change_t *caps)
{
    ...
    log_debug("change: interface %U, set: %U, cleared: %U",
              ...
              vnm,
              hw_if_index,
              format_vnet_hw_if_caps,
              (old ^ hi->caps) & caps->val,
              format_vnet_hw_if_caps,
              (old ^ hi->caps) & ~caps->val);
}

u8 *format_vnet_hw_if_caps(u8 *s, va_list *va)
{
...
#undef _
    ...
    if (caps == 0) return format(s, "none");
