 ...
 */
always_inline u32 mpls_compute_flow_hash(const mpls_unicast_header_t *hdr, flow_hash_config_t flow_hash_config)
{
    /*
    ...
    switch (((u8 *) hdr)[0] >> 4) {
        case 4:
            ...
            hash ^= ip4_compute_flow_hash((const ip4_header_t *) hdr, flow_hash_config);
            break;
        case 6:
            /* incorporate the v6 flow-hash */
            hash ^= ip6_compute_flow_hash((const ip6_header_t *) hdr, flow_hash_config);
            break;
        case 5:
