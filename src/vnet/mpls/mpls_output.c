...
typedef enum { MPLS_OUTPUT_MODE, MPLS_OUTPUT_MIDCHAIN_MODE } mpls_output_mode_t;
...
static u8 *format_mpls_output_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "adj-idx %d : %U flow hash: 0x%08x", t->adj_index, format_ip_adjacency, t->adj_index, FORMAT_IP_ADJACENCY_NONE, t->flow_hash);
    return s;
}

static inline uword mpls_output_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, mpls_output_mode_t mode)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            vnet_rewrite_two_headers(adj0[0], adj1[0], hdr0, hdr1, sizeof(ethernet_header_t));
            ...
            if (PREDICT_TRUE(vlib_buffer_length_in_chain(vm, p0) <= adj0[0].rewrite_header.max_l3_packet_bytes)) {
                ...
                if (PREDICT_FALSE(adj0[0].rewrite_header.flags & VNET_REWRITE_HAS_FEATURES))
                    vnet_feature_arc_start(mm->output_feature_arc_index, adj0[0].rewrite_header.sw_if_index, &next0, p0);
            }
            else {
                error0 = IP4_ERROR_MTU_EXCEEDED;
                next0  = MPLS_OUTPUT_NEXT_FRAG;
                vlib_node_increment_counter(vm, mpls_output_node.index, MPLS_ERROR_PKTS_NEED_FRAG, 1);
            }
            if (PREDICT_TRUE(vlib_buffer_length_in_chain(vm, p1) <= adj1[0].rewrite_header.max_l3_packet_bytes)) {
                ...
                if (PREDICT_FALSE(adj1[0].rewrite_header.flags & VNET_REWRITE_HAS_FEATURES))
                    vnet_feature_arc_start(mm->output_feature_arc_index, adj1[0].rewrite_header.sw_if_index, &next1, p1);
            }
            else {
            ...
            if (PREDICT_FALSE(p0->flags & VLIB_BUFFER_IS_TRACED)) {
                mpls_output_trace_t *tr = vlib_add_trace(vm, node, p0, sizeof(*tr));
                tr->adj_index           = vnet_buffer(p0)->ip.adj_index[VLIB_TX];
                tr->flow_hash           = vnet_buffer(p0)->ip.flow_hash;
            }
            if (PREDICT_FALSE(p1->flags & VLIB_BUFFER_IS_TRACED)) {
                mpls_output_trace_t *tr = vlib_add_trace(vm, node, p1, sizeof(*tr));
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            /* Guess we are only writing on simple Ethernet header. */
            vnet_rewrite_one_header(adj0[0], hdr0, sizeof(ethernet_header_t));
    ...
    vlib_node_increment_counter(vm, mpls_output_node.index, MPLS_ERROR_PKTS_ENCAP, from_frame->n_vectors);

    return from_frame->n_vectors;
}

VLIB_NODE_FN(mpls_output_node)
...
VLIB_NODE_FN(mpls_midchain_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
static uword mpls_frag(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(p0->flags & VLIB_BUFFER_IS_TRACED)) {
                mpls_frag_trace_t *tr = vlib_add_trace(vm, node, p0, sizeof(*tr));
                tr->mtu               = mtu;
            ...
            if (PREDICT_TRUE(error0 == IP_FRAG_ERROR_NONE)) {
                ...
                next0 = (IP_LOOKUP_NEXT_MIDCHAIN == adj0->lookup_next_index ? MPLS_FRAG_NEXT_REWRITE_MIDCHAIN : MPLS_FRAG_NEXT_REWRITE);
            }
            else {
 ...
 * exception case.
 */
VLIB_NODE_FN(mpls_adj_incomplete_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(p0->flags & VLIB_BUFFER_IS_TRACED)) {
                mpls_adj_incomplete_trace_t *tr = vlib_add_trace(vm, node, p0, sizeof(*tr));
                tr->next                        = next0;
            }
...
static u8 *format_mpls_adj_incomplete_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "%Unext:%d", format_white_space, indent, t->next);
    return (s);
}
