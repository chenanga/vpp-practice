...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left > 2) {
        ...
        mt0 = pool_elt_at_index(mpls_tunnel_pool, mpls_tunnel_db[sw_if_index0]);
        mt1 = pool_elt_at_index(mpls_tunnel_pool, mpls_tunnel_db[sw_if_index1]);
        ...
        if (PREDICT_FALSE(b[0]->flags & VLIB_BUFFER_IS_TRACED)) {
            mpls_tunnel_trace_t *tr = vlib_add_trace(vm, node, b[0], sizeof(*tr));
            tr->tunnel_id           = mpls_tunnel_db[sw_if_index0];
        }
        if (PREDICT_FALSE(b[1]->flags & VLIB_BUFFER_IS_TRACED)) {
            mpls_tunnel_trace_t *tr = vlib_add_trace(vm, node, b[1], sizeof(*tr));
...
{
    if ((vec_len(mpls_tunnel_db) <= sw_if_index) || (~0 == mpls_tunnel_db[sw_if_index])) return (NULL);

    return (pool_elt_at_index(mpls_tunnel_pool, mpls_tunnel_db[sw_if_index]));
}
...
static fib_path_list_walk_rc_t mpls_tunnel_collect_forwarding(fib_node_index_t pl_index, fib_node_index_t path_index, void *arg)
{
     ...
     */
    path_ext = fib_path_ext_list_find_by_path_index(&ctx->mt->mt_path_exts, path_index);
     ...
     * info for this path.
     */
    ctx->next_hops = fib_path_ext_stack(path_ext, DPO_PROTO_MPLS, ctx->fct, ctx->next_hops);
...
static void mpls_tunnel_mk_lb(mpls_tunnel_t *mt, vnet_link_t linkt, fib_forward_chain_type_t fct, dpo_id_t *dpo_lb)
{
    ...
    if (FIB_NODE_INDEX_INVALID != mt->mt_path_list) {
        fib_path_list_walk(mt->mt_path_list, mpls_tunnel_collect_forwarding, &ctx);
    }
    ...
    if (!dpo_id_is_valid(dpo_lb)) {
        ...
        if (mt->mt_flags & MPLS_TUNNEL_FLAG_MCAST) {
            dpo_set(dpo_lb, DPO_REPLICATE, lb_proto, replicate_create(0, lb_proto));
        }
        else {
            ...
            dpo_set(dpo_lb, DPO_LOAD_BALANCE, lb_proto, load_balance_create(0, lb_proto, fhc));
        }
    }
    ...
    if (mt->mt_flags & MPLS_TUNNEL_FLAG_MCAST) {
        ...
        load_balance_multipath_update(dpo_lb, ctx.next_hops, LOAD_BALANCE_FLAG_NONE);
...
static void mpls_tunnel_stack(adj_index_t ai)
{
    ...
    if (NULL == mt || FIB_NODE_INDEX_INVALID == mt->mt_path_list) return;
     ...
     */
    if (vnet_hw_interface_get_flags(vnet_get_main(), mt->mt_hw_if_index) & VNET_HW_INTERFACE_FLAG_LINK_UP) {
    ...
    mt->mt_sibling_index = fib_path_list_child_add(mt->mt_path_list, FIB_NODE_TYPE_MPLS_TUNNEL, mt - mpls_tunnel_pool);
...
static void mpls_tunnel_restack(mpls_tunnel_t *mt)
{
    ...
    if (mt->mt_flags & MPLS_TUNNEL_FLAG_L2) {
        ...
        dpo_stack_from_node(mpls_tunnel_tx.index, &mt->mt_l2_lb, &dpo);
        dpo_reset(&dpo);
    }
    else {
        FOR_EACH_FIB_IP_PROTOCOL(proto)
        {
            adj_nbr_walk(mt->mt_sw_if_index, proto, mpls_adj_walk_cb, NULL);
        }
    }
}
...
static void mpls_tunnel_update_adj(vnet_main_t *vnm, u32 sw_if_index, adj_index_t ai)
{
    ...
    switch (adj->lookup_next_index) {
        ...
        case IP_LOOKUP_NEXT_BCAST:
            adj_nbr_midchain_update_rewrite(ai, mpls_tunnel_fixup, NULL, ADJ_FLAG_NONE, mpls_tunnel_build_rewrite_i());
            break;
        case IP_LOOKUP_NEXT_MCAST:
             ...
             */
            adj_mcast_midchain_update_rewrite(ai, mpls_tunnel_fixup, NULL, ADJ_FLAG_NONE, mpls_tunnel_build_rewrite_i(), 0, 0);
            break;
...
void vnet_mpls_tunnel_del(u32 sw_if_index)
{
    ...
    if (FIB_NODE_INDEX_INVALID != mt->mt_path_list) fib_path_list_child_remove(mt->mt_path_list, mt->mt_sibling_index);
...
u32 vnet_mpls_tunnel_create(u8 l2_only, u8 is_multicast, u8 *tag)
{
    ...
    if (is_multicast) mt->mt_flags |= MPLS_TUNNEL_FLAG_MCAST;
    if (l2_only) mt->mt_flags |= MPLS_TUNNEL_FLAG_L2;
    if (tag)
     ...
     */
    mt->mt_hw_if_index = vnet_register_interface(vnm, mpls_tunnel_class.index, mti, mpls_tunnel_hw_interface_class.index, mti);
    hi                 = vnet_get_hw_interface(vnm, mt->mt_hw_if_index);

    if (mt->mt_flags & MPLS_TUNNEL_FLAG_L2)
        vnet_set_interface_output_node(vnm, mt->mt_hw_if_index, mpls_tunnel_tx.index);
    else
        vnet_set_interface_l3_output_node(vnm->vlib_main, hi->sw_if_index, (u8 *) "tunnel-output");
...
void vnet_mpls_tunnel_path_add(u32 sw_if_index, fib_route_path_t *rpaths)
{
    ...
    if (FIB_NODE_INDEX_INVALID == mt->mt_path_list) {
        ...
        mt->mt_sibling_index = fib_path_list_child_add(mt->mt_path_list, FIB_NODE_TYPE_MPLS_TUNNEL, mti);
    }
    else {
        ...
        mt->mt_path_list = fib_path_list_copy_and_path_add(old_pl_index, FIB_PATH_LIST_FLAG_SHARED, rpaths);
    ...
    vec_foreach (rpath, rpaths) {
        fib_path_ext_list_insert(&mt->mt_path_exts, mt->mt_path_list, FIB_PATH_EXT_MPLS, rpath);
    }
    mpls_tunnel_restack(mt);
}
...
int vnet_mpls_tunnel_path_remove(u32 sw_if_index, fib_route_path_t *rpaths)
{
    ...
    if (FIB_NODE_INDEX_INVALID == mt->mt_path_list) {
        ...
        mt->mt_path_list = fib_path_list_copy_and_path_remove(old_pl_index, FIB_PATH_LIST_FLAG_SHARED, rpaths);
         ...
         */
        fib_path_ext_list_remove(&mt->mt_path_exts, FIB_PATH_EXT_MPLS, rpaths);
...
static clib_error_t *vnet_create_mpls_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "del %U", unformat_vnet_sw_interface, vnm, &sw_if_index))
            is_del = 1;
        else if (unformat(line_input, "add %U", unformat_vnet_sw_interface, vnm, &sw_if_index))
            is_del = 0;
    ...
    if (is_del) {
        ...
        if (0 == vec_len(rpath.frp_label_stack)) {
            error = clib_error_return(0, "No Output Labels '%U'", format_unformat_error, line_input);
            goto done;
        }
...
static u8 *format_mpls_tunnel(u8 *s, va_list *args)
{
    ...
    s = format(s, "mpls-tunnel%d: sw_if_index:%d hw_if_index:%d", mt - mpls_tunnel_pool, mt->mt_sw_if_index, mt->mt_hw_if_index);
    ...
    if (mt->mt_flags & MPLS_TUNNEL_FLAG_L2) {
        s = format(s, " forwarding: %U\n", format_fib_forw_chain_type, FIB_FORW_CHAIN_TYPE_ETHERNET);
...
static clib_error_t *show_mpls_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (pool_elts(mpls_tunnel_pool) == 0) vlib_cli_output(vm, "No MPLS tunnels configured...");
    ...
    if (~0 == mti) {
        pool_foreach (mt, mpls_tunnel_pool) {
            vlib_cli_output(vm, "[@%d] %U", mt - mpls_tunnel_pool, format_mpls_tunnel, mt);
        }
    }
    else {
        if (pool_is_free_index(mpls_tunnel_pool, mti)) return clib_error_return(0, "Not a tunnel index %d", mti);
...
static mpls_tunnel_t *mpls_tunnel_from_fib_node(fib_node_t *node)
{
    ...
    return ((mpls_tunnel_t *) (((char *) node) - STRUCT_OFFSET_OF(mpls_tunnel_t, mt_node)));
}
