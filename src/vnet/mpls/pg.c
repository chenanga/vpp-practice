...
static inline void pg_mpls_header_init(pg_mpls_header_t *e)
{
...
uword unformat_pg_mpls_header(unformat_input_t *input, va_list *args)
{
    ...
    h = pg_create_edit_group(s, sizeof(h[0]), sizeof(mpls_unicast_header_t), &group_index);
    pg_mpls_header_init(h);

    error = 1;
    if (!unformat(input, "%U", unformat_pg_edit, unformat_mpls_label_net_byte_order, &h->label)) goto done;
