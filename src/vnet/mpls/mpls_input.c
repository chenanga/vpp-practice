...
#define _(a, b) \
    if (t->next_index == MPLS_INPUT_NEXT_##a) next_name = b;
    foreach_mpls_input_next;
#undef _
...
static inline uword mpls_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    cm = vec_elt_at_index(vnm->interface_main.sw_if_counters, VNET_INTERFACE_COUNTER_MPLS);
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                mpls_input_trace_t *tr   = vlib_add_trace(vm, node, b0, sizeof(*tr));
                tr->next_index           = next0;
                tr->label_net_byte_order = *((u32 *) h0);
            }
            if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
                mpls_input_trace_t *tr   = vlib_add_trace(vm, node, b1, sizeof(*tr));
                tr->next_index           = next1;
    ...
    vlib_node_increment_counter(vm, mpls_input_node.index, MPLS_ERROR_PKTS_DECAP, from_frame->n_vectors);
...
static void mpls_setup_nodes(vlib_main_t *vm)
{
    ...
    ethernet_register_input_type(vm, ETHERNET_TYPE_MPLS, mpls_input_node.index);
}
