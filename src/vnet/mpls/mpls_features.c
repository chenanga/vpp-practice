...
always_inline uword mpls_terminate(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int error_code)
{
    u32  *buffers   = vlib_frame_vector_args(frame);
    uword n_packets = frame->n_vectors;

    if (node->flags & VLIB_NODE_FLAG_TRACE) mpls_drop_trace(vm, node, frame);
...
static clib_error_t *mpls_sw_interface_add_del(vnet_main_t *vnm, u32 sw_if_index, u32 is_add)
{
    ...
    vnet_feature_enable_disable("mpls-input", "mpls-not-enabled", sw_if_index, is_add, 0, 0);
