...
u8 mpls_sw_interface_is_enabled(u32 sw_if_index)
{
    ...
    if (vec_len(mm->mpls_enabled_by_sw_if_index) <= sw_if_index) return (0);

    return (mm->mpls_enabled_by_sw_if_index[sw_if_index]);
}

void mpls_interface_state_change_add_callback(mpls_interface_state_change_function_t *function, uword opaque)
{
...
int mpls_sw_interface_enable_disable(mpls_main_t *mm, u32 sw_if_index, u8 is_enable)
{
    ...
    lfib_index = fib_table_find(FIB_PROTOCOL_MPLS, MPLS_FIB_DEFAULT_TABLE_ID);

    if (~0 == lfib_index) return VNET_API_ERROR_NO_SUCH_FIB;
    ...
    if (is_enable) {
        ...
        if (0 != --mm->mpls_enabled_by_sw_if_index[sw_if_index]) return (0);

        fib_table_unlock(mm->fib_index_by_sw_if_index[sw_if_index], FIB_PROTOCOL_MPLS, FIB_SOURCE_INTERFACE);
    }

    vnet_feature_enable_disable("mpls-input", "mpls-not-enabled", sw_if_index, !is_enable, 0, 0);
...
static clib_error_t *mpls_interface_enable_disable(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (unformat(input, "enable"))
        ...
        error = clib_error_return(0, "expected 'enable' or 'disable'", format_unformat_error, input);
        goto done;
    }

    rv = mpls_sw_interface_enable_disable(&mpls_main, sw_if_index, enable);

    if (VNET_API_ERROR_NO_SUCH_FIB == rv) error = clib_error_return(0, "default MPLS table must be created first");
