...
uword unformat_mpls_label_net_byte_order(unformat_input_t *input, va_list *args)
{
    ...
    if (!unformat(input, "MPLS: label %d", &label)) return 0;
...
u8 *format_mpls_unicast_header_net_byte_order(u8 *s, va_list *args)
{
    ...
    return format(s, "%U", format_mpls_unicast_header_host_byte_order, &h_host);
}
...
static clib_error_t *vnet_mpls_local_label(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (MPLS_LABEL_INVALID == local_label) {
        error = clib_error_return(0, "local-label required: %U", format_unformat_error, input);
        goto done;
    }
    ...
    if (is_ip) {
        ...
        if (FIB_NODE_INDEX_INVALID == fib_index) {
            error = clib_error_return(0, "%U table-id %d does not exist", format_fib_protocol, pfx.fp_proto, table_id);
            goto done;
        }
        ...
        if (FIB_NODE_INDEX_INVALID == fib_index) {
            error = clib_error_return(0, "MPLS table-id %d does not exist", table_id);
            goto done;
        }

        if (is_del) {
            fib_table_entry_path_remove2(fib_index, &pfx, FIB_SOURCE_CLI, rpaths);
        }
        else {
            fib_node_index_t lfe;

            lfe = fib_table_entry_path_add2(fib_index, &pfx, FIB_SOURCE_CLI, FIB_ENTRY_FLAG_NONE, rpaths);

            if (FIB_NODE_INDEX_INVALID == lfe) {
                error = clib_error_return(0,
                                          "Failed to create %U-%U in MPLS table-id %d",
                                          format_mpls_unicast_label,
                                          local_label,
                                          format_mpls_eos_bit,
                                          eos,
