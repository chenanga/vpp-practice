...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 8 && n_left_to_next >= 4) {
            ...
            lfib_index0 = vec_elt(mm->fib_index_by_sw_if_index, vnet_buffer(b0)->sw_if_index[VLIB_RX]);
            lfib_index1 = vec_elt(mm->fib_index_by_sw_if_index, vnet_buffer(b1)->sw_if_index[VLIB_RX]);
            lfib_index2 = vec_elt(mm->fib_index_by_sw_if_index, vnet_buffer(b2)->sw_if_index[VLIB_RX]);
            lfib_index3 = vec_elt(mm->fib_index_by_sw_if_index, vnet_buffer(b3)->sw_if_index[VLIB_RX]);
            ...
            if (MPLS_IS_REPLICATE & lbi0) {
                ...
                if (PREDICT_FALSE(lb0->lb_n_buckets > 1)) {
                    hash_c0 = vnet_buffer(b0)->ip.flow_hash = mpls_compute_flow_hash(h0, lb0->lb_hash_config);
            ...
            if (MPLS_IS_REPLICATE & lbi1) {
                ...
                vnet_buffer(b1)->ip.adj_index[VLIB_TX] = (lbi1 & ~MPLS_IS_REPLICATE);
            }
            else {
                ...
                if (PREDICT_FALSE(lb1->lb_n_buckets > 1)) {
                    hash_c1 = vnet_buffer(b1)->ip.flow_hash = mpls_compute_flow_hash(h1, lb1->lb_hash_config);
                    dpo1                                    = load_balance_get_fwd_bucket(lb1, (hash_c1 & (lb1->lb_n_buckets_minus_1)));
                }
                else {
            ...
            if (MPLS_IS_REPLICATE & lbi2) {
                ...
                vnet_buffer(b2)->ip.adj_index[VLIB_TX] = (lbi2 & ~MPLS_IS_REPLICATE);
            }
            else {
                ...
                if (PREDICT_FALSE(lb2->lb_n_buckets > 1)) {
                    hash_c2 = vnet_buffer(b2)->ip.flow_hash = mpls_compute_flow_hash(h2, lb2->lb_hash_config);
                    dpo2                                    = load_balance_get_fwd_bucket(lb2, (hash_c2 & (lb2->lb_n_buckets_minus_1)));
                }
                else {
                ...
                vlib_increment_combined_counter(cm, thread_index, lbi2, 1, vlib_buffer_length_in_chain(vm, b2));
            }
            if (MPLS_IS_REPLICATE & lbi3) {
                next3                                  = mpls_lookup_to_replicate_edge;
                vnet_buffer(b3)->ip.adj_index[VLIB_TX] = (lbi3 & ~MPLS_IS_REPLICATE);
            }
            else {
                ...
                if (PREDICT_FALSE(lb3->lb_n_buckets > 1)) {
                    hash_c3 = vnet_buffer(b3)->ip.flow_hash = mpls_compute_flow_hash(h3, lb3->lb_hash_config);
                    dpo3                                    = load_balance_get_fwd_bucket(lb3, (hash_c3 & (lb3->lb_n_buckets_minus_1)));
                }
                else {
                ...
                vlib_increment_combined_counter(cm, thread_index, lbi3, 1, vlib_buffer_length_in_chain(vm, b3));
            }
            ...
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                mpls_lookup_trace_t *tr  = vlib_add_trace(vm, node, b0, sizeof(*tr));
                tr->next_index           = next0;
                tr->lb_index             = lbi0;
            ...
            if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
                mpls_lookup_trace_t *tr  = vlib_add_trace(vm, node, b1, sizeof(*tr));
                tr->next_index           = next1;
                tr->lb_index             = lbi1;
            ...
            if (PREDICT_FALSE(b2->flags & VLIB_BUFFER_IS_TRACED)) {
                mpls_lookup_trace_t *tr  = vlib_add_trace(vm, node, b2, sizeof(*tr));
                tr->next_index           = next2;
                tr->lb_index             = lbi2;
            ...
            if (PREDICT_FALSE(b3->flags & VLIB_BUFFER_IS_TRACED)) {
                mpls_lookup_trace_t *tr  = vlib_add_trace(vm, node, b3, sizeof(*tr));
                tr->next_index           = next3;
                tr->lb_index             = lbi3;
    ...
    vlib_node_increment_counter(vm, mm->mpls_lookup_node_index, MPLS_ERROR_PKTS_DECAP, from_frame->n_vectors);
...
static u8 *format_mpls_load_balance_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "MPLS: next [%d], LB index %d hash 0x%08x", t->next_index, t->lb_index, t->hash);
    return s;
}

VLIB_NODE_FN(mpls_load_balance_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (PREDICT_FALSE(lb0->lb_n_buckets > 1)) {
                if (PREDICT_TRUE(vnet_buffer(p0)->ip.flow_hash)) {
                    ...
                    hc0 = vnet_buffer(p0)->ip.flow_hash = mpls_compute_flow_hash(mpls0, lb0->lb_hash_config);
                }
            ...
            if (PREDICT_FALSE(lb1->lb_n_buckets > 1)) {
                if (PREDICT_TRUE(vnet_buffer(p1)->ip.flow_hash)) {
                    ...
                    hc1 = vnet_buffer(p1)->ip.flow_hash = mpls_compute_flow_hash(mpls1, lb1->lb_hash_config);
                }
            ...
            vlib_increment_combined_counter(cm, thread_index, lbi0, 1, vlib_buffer_length_in_chain(vm, p0));
            vlib_increment_combined_counter(cm, thread_index, lbi1, 1, vlib_buffer_length_in_chain(vm, p1));

            if (PREDICT_FALSE(p0->flags & VLIB_BUFFER_IS_TRACED)) {
                mpls_load_balance_trace_t *tr = vlib_add_trace(vm, node, p0, sizeof(*tr));
                tr->next_index                = next0;
                tr->lb_index                  = lbi0;
                tr->hash                      = hc0;
            }
            if (PREDICT_FALSE(p1->flags & VLIB_BUFFER_IS_TRACED)) {
                mpls_load_balance_trace_t *tr = vlib_add_trace(vm, node, p1, sizeof(*tr));
                tr->next_index                = next1;
                tr->lb_index                  = lbi1;
                tr->hash                      = hc1;
            }

            vlib_validate_buffer_enqueue_x2(vm, node, next, to_next, n_left_to_next, pi0, pi1, next0, next1);
        }
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            vlib_validate_buffer_enqueue_x1(vm, node, next, to_next, n_left_to_next, pi0, next0);
        }
...
static clib_error_t *mpls_lookup_init(vlib_main_t *vm)
{
    ...
    if ((error = vlib_call_init_function(vm, mpls_init))) return error;

    mpls_lookup_to_replicate_edge = vlib_node_add_named_next(vm, mm->mpls_lookup_node_index, "mpls-replicate");
