...
void mpls_table_delete(u32 table_id, u8 is_api)
{
    ...
    if (~0 != fib_index) {
        fib_table_unlock(fib_index, FIB_PROTOCOL_MPLS, (is_api ? FIB_SOURCE_API : FIB_SOURCE_CLI));
    }
}
...
void vl_api_mpls_table_add_del_t_handler(vl_api_mpls_table_add_del_t *mp)
{
    ...
    if (mp->mt_is_add)
        mpls_table_create(ntohl(mp->mt_table.mt_table_id), 1, mp->mt_table.mt_name);
    else
...
static int mpls_ip_bind_unbind_handler(vnet_main_t *vnm, vl_api_mpls_ip_bind_unbind_t *mp)
{
    ...
    mpls_fib_index = fib_table_find(FIB_PROTOCOL_MPLS, ntohl(mp->mb_mpls_table_id));
    ...
    if (~0 == ip_fib_index) return VNET_API_ERROR_NO_SUCH_FIB;

    if (mp->mb_is_bind)
        fib_table_entry_local_label_add(ip_fib_index, &pfx, ntohl(mp->mb_label));
    else
        fib_table_entry_local_label_remove(ip_fib_index, &pfx, ntohl(mp->mb_label));
...
static int mpls_route_add_del_t_handler(vnet_main_t *vnm, vl_api_mpls_route_add_del_t *mp, u32 *stats_index)
{
    ...
    rv = fib_api_table_id_decode(FIB_PROTOCOL_MPLS, ntohl(mp->mr_route.mr_table_id), &fib_index);
    if (0 != rv) goto out;
    ...
    rv = fib_api_route_add_del(mp->mr_is_add,
                               ...
                               &pfx,
                               FIB_SOURCE_API,
                               (mp->mr_route.mr_is_multicast ? FIB_ENTRY_FLAG_MULTICAST : FIB_ENTRY_FLAG_NONE),
                               rpaths);

    if (mp->mr_is_add && 0 == rv) *stats_index = fib_table_entry_get_stats_index(fib_index, &pfx);
...
void vl_api_mpls_route_add_del_t_handler(vl_api_mpls_route_add_del_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_MPLS_ROUTE_ADD_DEL_REPLY, ({ rmp->stats_index = htonl(stats_index); }));
}
...
void mpls_table_create(u32 table_id, u8 is_api, const u8 *name)
{
     ...
     * deleted only once.
     */
    fib_table_find_or_create_and_lock_w_name(FIB_PROTOCOL_MPLS, table_id, (is_api ? FIB_SOURCE_API : FIB_SOURCE_CLI), name);
}
...
static void vl_api_mpls_tunnel_add_del_t_handler(vl_api_mpls_tunnel_add_del_t *mp)
{
    ...
    if (mp->mt_is_add) {
        if (~0 == tunnel_sw_if_index)
            tunnel_sw_if_index = vnet_mpls_tunnel_create(mp->mt_tunnel.mt_l2_only, mp->mt_tunnel.mt_is_multicast, mp->mt_tunnel.mt_tag);
        ...
        tunnel_index       = vnet_mpls_tunnel_get_index(tunnel_sw_if_index);
        tunnel_sw_if_index = ntohl(mp->mt_tunnel.mt_sw_if_index);
        if (!vnet_mpls_tunnel_path_remove(tunnel_sw_if_index, rpaths)) vnet_mpls_tunnel_del(tunnel_sw_if_index);
    }
...
static void vl_api_sw_interface_set_mpls_enable_t_handler(vl_api_sw_interface_set_mpls_enable_t *mp)
{
    ...
    rv = mpls_sw_interface_enable_disable(&mpls_main, ntohl(mp->sw_if_index), mp->enable);
...
static void send_mpls_tunnel_entry(u32 mti, void *arg)
{
    ...
    if (~0 != ctx->sw_if_index && mt->mt_sw_if_index != ctx->sw_if_index) return;
    ...
    fib_path_list_walk_w_ext(mt->mt_path_list, &mt->mt_path_exts, fib_path_encode, &path_ctx);
...
static void send_mpls_table_details(vpe_api_main_t *am, vl_api_registration_t *reg, u32 context, const fib_table_t *table)
{
    ...
    memcpy(mp->mt_table.mt_name, table->ft_desc, clib_min(vec_len(table->ft_desc), sizeof(mp->mt_table.mt_name)));
...
static void send_mpls_interface_details(vpe_api_main_t *am, vl_api_registration_t *reg, u32 context, const u32 sw_if_index)
{
...
static void vl_api_mpls_interface_dump_t_handler(vl_api_mpls_interface_dump_t *mp)
{
    vpe_api_main_t        *am = &vpe_api_main;
    vl_api_registration_t *reg;
    ...
    if (sw_if_index == ~0) {
        pool_foreach (si, im->sw_interfaces) {
            if (mpls_sw_interface_is_enabled(si->sw_if_index)) {
                send_mpls_interface_details(am, reg, mp->context, si->sw_if_index);
            }
        }
    }
    else {
...
static void send_mpls_route_details(vpe_api_main_t *am, vl_api_registration_t *reg, u32 context, fib_node_index_t fib_entry_index)
{
    ...
    mp->mr_route.mr_table_id  = htonl(fib_table_get_table_id(fib_entry_get_fib_index(fib_entry_index), pfx->fp_proto));
...
static void vl_api_mpls_route_dump_t_handler(vl_api_mpls_route_dump_t *mp)
{
    ...
    fib_index = fib_table_find(FIB_PROTOCOL_MPLS, ntohl(mp->table.mt_table_id));

    if (INDEX_INVALID != fib_index) {
        fib_table_walk(fib_index, FIB_PROTOCOL_MPLS, vl_api_mpls_route_dump_table_walk, &ctx);
...
static clib_error_t *mpls_api_hookup(vlib_main_t *vm)
{
     ...
     */
    vl_api_increase_msg_trace_size(am, VL_API_MPLS_TUNNEL_ADD_DEL, 8 * sizeof(u32));
