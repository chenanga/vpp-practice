...
static u8 *format_policer_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "VNET_POLICER: sw_if_index %d policer_index %d next %d", t->sw_if_index, t->policer_index, t->next_index);
    return s;
}
...
static inline uword vnet_policer_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, vlib_dir_t dir)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            act0 = vnet_policer_police(vm, b0, pi0, time_in_policer_periods, POLICE_CONFORM /* no chaining */, true);

            act1 = vnet_policer_police(vm, b1, pi1, time_in_policer_periods, POLICE_CONFORM /* no chaining */, true);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return policer_handoff(vm, node, frame, vnet_policer_main.fq_index[VLIB_RX], ~0);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return policer_handoff(vm, node, frame, vnet_policer_main.fq_index[VLIB_TX], ~0);
}
...
static inline uword policer_classify_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, policer_classify_table_id_t tid)
{
    u32                           n_left_from, *from, *to_next;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (tid == POLICER_CLASSIFY_TABLE_L2) {
                ...
                next0 = vnet_l2_feature_next(b0, pcm->feat_next_node_index, L2INPUT_FEAT_POLICER_CLAS);
            }
            else
            ...
            if (PREDICT_TRUE(table_index0 != ~0)) {
                ...
                if (e0) {
                    act0 = vnet_policer_police(vm, b0, e0->next_index, time_in_policer_periods, e0->opaque_index, false);
...
static clib_error_t *policer_classify_init(vlib_main_t *vm)
{
    ...
    feat_bitmap_init_next_nodes(vm, l2_policer_classify_node.index, L2INPUT_N_FEAT, l2input_get_feat_names(), pcm->feat_next_node_index);
