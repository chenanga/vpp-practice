...
static void vl_api_policer_add_del_t_handler(vl_api_policer_add_del_t *mp)
{
    ...
    if (mp->is_add) {
        ...
        if (p != NULL) rv = policer_del(vm, p[0]);
    }
...
static_always_inline void policer_set_configuration(qos_pol_cfg_params_st *cfg, vl_api_policer_config_t *infos)
{
    clib_memset(cfg, 0, sizeof(*cfg));
...
static void vl_api_policer_add_t_handler(vl_api_policer_add_t *mp)
{
    vlib_main_t                *vm = vlib_get_main();
...
static void vl_api_policer_del_t_handler(vl_api_policer_del_t *mp)
{
    vlib_main_t                *vm = vlib_get_main();
...
static void vl_api_policer_update_t_handler(vl_api_policer_update_t *mp)
{
    vlib_main_t                   *vm = vlib_get_main();
...
static void vl_api_policer_reset_t_handler(vl_api_policer_reset_t *mp)
{
    vlib_main_t                  *vm = vlib_get_main();
...
static void vl_api_policer_bind_t_handler(vl_api_policer_bind_t *mp)
{
    ...
    rv = VNET_API_ERROR_NO_SUCH_ENTRY;
    if (p != NULL) rv = policer_bind_worker(p[0], worker_index, bind_enable);

    REPLY_MACRO(VL_API_POLICER_BIND_REPLY);
}

static void vl_api_policer_bind_v2_t_handler(vl_api_policer_bind_v2_t *mp)
{
...
static void vl_api_policer_input_t_handler(vl_api_policer_input_t *mp)
{
    ...
    rv = VNET_API_ERROR_NO_SUCH_ENTRY;
    if (p != NULL) rv = policer_input(p[0], sw_if_index, VLIB_RX, apply);

    BAD_SW_IF_INDEX_LABEL;
    REPLY_MACRO(VL_API_POLICER_INPUT_REPLY);
}

static void vl_api_policer_input_v2_t_handler(vl_api_policer_input_v2_t *mp)
{
...
static void vl_api_policer_output_t_handler(vl_api_policer_output_t *mp)
{
    ...
    rv = VNET_API_ERROR_NO_SUCH_ENTRY;
    if (p != NULL) rv = policer_input(p[0], sw_if_index, VLIB_TX, apply);

    BAD_SW_IF_INDEX_LABEL;
    REPLY_MACRO(VL_API_POLICER_OUTPUT_REPLY);
}

static void vl_api_policer_output_v2_t_handler(vl_api_policer_output_v2_t *mp)
{
    vl_api_policer_output_reply_t *rmp;
    u32                            policer_index;
    u32                            sw_if_index;
...
static void send_policer_details(qos_pol_cfg_params_st *config, policer_t *policer, vl_api_registration_t *reg, u32 context)
{
...
static void vl_api_policer_dump_v2_t_handler(vl_api_policer_dump_v2_t *mp)
{
    vl_api_registration_t *reg;
    vnet_policer_main_t   *pm = &vnet_policer_main;
    qos_pol_cfg_params_st *config;
    ...
    if (~0 == policer_index) {
        ...
        if (pool_is_free_index(pm->policers, policer_index)) return;
