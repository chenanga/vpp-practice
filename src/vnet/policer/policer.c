...
u8 *format_policer_handoff_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "policer %d, handoff thread %d to %d", t->policer_index, t->current_worker_index, t->next_worker_index);
...
int policer_add(vlib_main_t *vm, const u8 *name, const qos_pol_cfg_params_st *cfg, u32 *policer_index)
{
...
int policer_del(vlib_main_t *vm, u32 policer_index)
{
    ...
    if (pool_is_free_index(pm->policers, policer_index)) return VNET_API_ERROR_NO_SUCH_ENTRY;
...
static uword unformat_policer_type(unformat_input_t *input, va_list *va)
{
    ...
    if (!unformat(input, "type")) return 0;
...
static uword unformat_policer_round_type(unformat_input_t *input, va_list *va)
{
    ...
    if (!unformat(input, "round")) return 0;
...
static uword unformat_policer_rate_type(unformat_input_t *input, va_list *va)
{
    ...
    if (!unformat(input, "rate")) return 0;
...
static uword unformat_policer_cir(unformat_input_t *input, va_list *va)
{
    ...
    if (unformat(input, "cir %u", &c->rb.kbps.cir_kbps)) return 1;
    return 0;
}
...
static uword unformat_policer_eir(unformat_input_t *input, va_list *va)
{
    ...
    if (unformat(input, "eir %u", &c->rb.kbps.eir_kbps)) return 1;
    return 0;
}
...
static uword unformat_policer_cb(unformat_input_t *input, va_list *va)
{
    ...
    if (unformat(input, "cb %u", &c->rb.kbps.cb_bytes)) return 1;
    return 0;
}
...
static uword unformat_policer_eb(unformat_input_t *input, va_list *va)
{
    ...
    if (unformat(input, "eb %u", &c->rb.kbps.eb_bytes)) return 1;
    return 0;
}
...
static clib_error_t *policer_del_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (~0 != policer_index) rv = policer_del(vm, policer_index);
...
static clib_error_t *policer_bind_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (bind && ~0 == worker) {
        error = clib_error_return(0, "specify worker to bind to: `%U'", format_unformat_error, line_input);
    }
    else {
        if (~0 == policer_index && 0 != name) {
            p = hash_get_mem(pm->policer_index_by_name, name);
        ...
        rv = VNET_API_ERROR_NO_SUCH_ENTRY;
        if (~0 != policer_index) rv = policer_bind_worker(policer_index, worker, bind);
...
static clib_error_t *policer_input_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (~0 == sw_if_index) {
        error = clib_error_return(0, "specify interface to apply to: `%U'", format_unformat_error, line_input);
    }
    else {
        if (~0 == policer_index && 0 != name) {
            p = hash_get_mem(pm->policer_index_by_name, name);
        ...
        rv = VNET_API_ERROR_NO_SUCH_ENTRY;
        if (~0 != policer_index) rv = policer_input(policer_index, sw_if_index, dir, apply);
...
static clib_error_t *policer_reset_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t     _line_input, *line_input = &_line_input;
    clib_error_t        *error = NULL;
    vnet_policer_main_t *pm    = &vnet_policer_main;
    ...
    rv = VNET_API_ERROR_NO_SUCH_ENTRY;
    if (~0 != policer_index) rv = policer_reset(vm, policer_index);
...
VLIB_CLI_COMMAND(configure_policer_command, static) = {.path = "configure policer",
                                                       .short_help =
                                                           "configure policer [name <name> | index <index>] [type 1r2c | "
                                                           "1r3c | 2r3c-2698 "
...
VLIB_CLI_COMMAND(policer_add_command, static) = {.path = "policer add",
                                                 .short_help =
                                                     "policer add name <name> [type 1r2c | 1r3c | 2r3c-2698 | "
                                                     "2r3c-4115] [color-aware] [cir <cir>] [cb <cb>] [eir <eir>] "
                                                     "[eb <eb>] [rate kbps | pps] [round closest | up | down] "
                                                     "[conform-action drop | transmit | mark-and-transmit <dscp>] "
                                                     "[exceed-action drop | transmit | mark-and-transmit <dscp>] "
...
VLIB_CLI_COMMAND(policer_reset_command, static) = {.path       = "policer reset",
...
static clib_error_t *show_policer_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (~0 == policer_index && 0 != name) {
        ...
        if (pi != NULL) policer_index = pi[0];
    }

    if (~0 == policer_index || pool_is_free_index(pm->policers, policer_index)) goto done;
...
static clib_error_t *show_policer_pools_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vlib_cli_output(vm, "pool sizes: configs=%d policers=%d", pool_elts(pm->configs), pool_elts(pm->policers));
    return 0;
}
