...
#define QOS_POL_COMM_BKT_LIMIT_MAX       ((u64) QOS_POL_COMM_BKT_LIMIT_MANT_MAX << (u64) QOS_POL_COMM_BKT_LIMIT_EXP_MAX)
...
#define QOS_POL_EXT_BKT_LIMIT_MAX        ((u64) QOS_POL_EXTD_BKT_LIMIT_MANT_MAX << (u64) QOS_POL_EXTD_BKT_LIMIT_EXP_MAX)
...
static int pol_validate_cfg_params(qos_pol_cfg_params_st *cfg)
{
    ...
    if ((cfg->rfc == QOS_POLICER_TYPE_2R3C_RFC_2698) && (cfg->rb.kbps.eir_kbps < cfg->rb.kbps.cir_kbps)) {
        QOS_DEBUG_ERROR("CIR (%u kbps) is greater than PIR (%u kbps)", cfg->rb.kbps.cir_kbps, cfg->rb.kbps.eir_kbps);
        return (EINVAL);
    }
    ...
    if (cir_hw > QOS_POL_AVG_RATE_MAX) {
        QOS_DEBUG_ERROR(
            "hw cir (%u bytes/tick) is greater than the "
    ...
    if (eir_hw > QOS_POL_PEAK_RATE_MAX) {
        QOS_DEBUG_ERROR(
            "hw eir (%u bytes/tick) is greater than the "
    ...
    if (cfg->rb.kbps.eir_kbps && (cfg->rfc < QOS_POLICER_TYPE_2R3C_RFC_2698)) {
        QOS_DEBUG_ERROR("EIR: %u kbps for a 1-rate policer (rfc: %u)", cfg->rb.kbps.eir_kbps, cfg->rfc);
        return (EINVAL);
    }
...
static int pol_convert_cfg_rates_to_hw(qos_pol_cfg_params_st *cfg, qos_pol_hw_params_st *hw)
{
    ...
    rc    = qos_pol_round(numer, denom, &rnd_value, cfg->rnd_type);
    if (rc != 0) {
        QOS_DEBUG_ERROR("Rounding error, rate: %d kbps, rounding_type: %d", cfg->rb.kbps.cir_kbps, cfg->rnd_type);
        return (rc);
    }
    cir_hw = (u32) rnd_value;
    ...
    if ((cir_hw == 0) && (eir_hw == 0)) {
        ...
        qos_convert_value_to_exp_mant_fmt(hi_rate, (u16) QOS_POL_RATE_EXP_MAX, (u16) QOS_POL_AVG_RATE_MANT_MAX, cfg->rnd_type, &exp, &hi_mant);
    }
    ...
    if ((hw->avg_rate_man == 0) && (cfg->rb.kbps.cir_kbps)) {
        ...
        hw->avg_rate_man = 1;
        QOS_DEBUG_INFO(
            "CIR = 0 during rounding. Bump it up to %u "
            "bytes/tick",
    ...
    if ((hw->peak_rate_man == 0) && eir_kbps) {
        ...
        hw->peak_rate_man = 1;
        QOS_DEBUG_INFO(
            "EIR = 0 during rounding. Bump it up to %u "
            "bytes/tick",
    ...
    QOS_DEBUG_INFO(
        "Configured(rounded) values, cir: %u "
        "kbps (mant: %u, exp: %u, rate: %u bytes/tick)",
        cfg->rb.kbps.cir_kbps,
        hw->avg_rate_man,
        hw->rate_exp,
        cir_rnded);

    QOS_DEBUG_INFO(
        "Configured(rounded) values, eir: %u "
        "kbps (mant: %u, exp: %u, rate: %u bytes/tick)",
...
static void
pol_rnd_burst_byte_fmt(u64 cfg_burst, u16 max_exp_value, u16 max_mant_value, u32 max_bkt_value, u32 rate_hw, u8 *exp, u32 *mant, u32 *bkt_value)
{
    ...
    if (cfg_burst > bkt_max) {
        QOS_DEBUG_ERROR(
            "burst 0x%llx bytes is greater than the max. "
    ...
    if (cfg_burst < QOS_POL_MIN_BURST_BYTE) {
         ...
         */
        QOS_DEBUG_INFO(
            "burst 0x%llx bytes is less than the min "
    ...
    qos_convert_value_to_exp_mant_fmt(cfg_burst, max_exp_value, max_mant_value, QOS_ROUND_TO_DOWN, exp, mant);
...
static int pol_convert_cfg_burst_to_hw(qos_pol_cfg_params_st *cfg, qos_pol_hw_params_st *hw)
{
    ...
    pol_rnd_burst_byte_fmt(cfg->rb.kbps.cb_bytes,
                           (u16) QOS_POL_COMM_BKT_LIMIT_EXP_MAX,
                           (u16) QOS_POL_COMM_BKT_LIMIT_MANT_MAX,
                           ...
                           &bkt_value);
    QOS_DEBUG_INFO(
        "Committed burst, burst_limit: 0x%llx mant : %u, "
        "exp: %u, rnded: 0x%llx cb:%u bytes",
        cfg->rb.kbps.cb_bytes,
        temp_mant,
        temp_exp,
        ((u64) temp_mant << (u64) temp_exp),
        bkt_value);
    ...
    pol_rnd_burst_byte_fmt(eb_bytes,
                           (u16) QOS_POL_EXTD_BKT_LIMIT_EXP_MAX,
                           (u16) QOS_POL_EXTD_BKT_LIMIT_MANT_MAX,
                           (u32) QOS_POL_EXTD_BKT_MAX,
                           rate_hw,
                           &temp_exp,
                           &temp_mant,
                           &bkt_value);
    ...
    QOS_DEBUG_INFO(
        "Excess burst, burst_limit: 0x%llx mant: %u, "
...
int pol_compute_hw_params(qos_pol_cfg_params_st *cfg, qos_pol_hw_params_st *hw)
{
    ...
    if (rc != 0) {
        QOS_DEBUG_ERROR(
            "Unable to convert config params to hw params. "
            "Error: %d",
            rc);
        return (-1);
    }
 ...
 */
static int compute_policer_params(u64  hz,             /* CPU speed in clocks per second */
...
int x86_pol_compute_hw_params(qos_pol_cfg_params_st *cfg, policer_t *hw)
{
    ...
    if ((cfg->rfc == QOS_POLICER_TYPE_1R2C) || (cfg->rfc == QOS_POLICER_TYPE_1R3C_RFC_2697)) {
        ...
        if ((cfg->rb.kbps.cir_kbps == 0) || (cfg->rb.kbps.eir_kbps != 0) || ((cfg->rb.kbps.cb_bytes == 0) && (cfg->rb.kbps.eb_bytes == 0))) {
    ...
    else if ((cfg->rfc == QOS_POLICER_TYPE_2R3C_RFC_2698) || (cfg->rfc == QOS_POLICER_TYPE_2R3C_RFC_4115)) {
        /* Two-rate policer */
...
int pol_logical_2_physical(const qos_pol_cfg_params_st *cfg, policer_t *phys)
{
    ...
    switch (cfg->rate_type) {
        ...
        case QOS_RATE_PPS:
            ...
            kbps_cfg.rb.kbps.cb_bytes = qos_convert_burst_ms_to_bytes((u32) cfg->rb.pps.cb_ms, kbps_cfg.rb.kbps.cir_kbps);
            kbps_cfg.rb.kbps.eb_bytes = qos_convert_burst_ms_to_bytes((u32) cfg->rb.pps.eb_ms, kbps_cfg.rb.kbps.eir_kbps);
            break;
        default:
...
static int pol_convert_hw_to_cfg_params(qos_pol_hw_params_st *hw, qos_pol_cfg_params_st *cfg)
{
    ...
    if ((hw->rfc == IPE_RFC_RFC4115) && (hw->peak_rate_man << hw->rate_exp) == 0 && !(hw->extd_bkt_limit_man)) {
        /*
    ...
    QOS_DEBUG_INFO(
        "configured params, cir: %u kbps, eir: %u kbps, cb "
...
int pol_physical_2_logical(policer_t *phys, qos_pol_cfg_params_st *cfg)
{
    ...
    if (rc != 0) {
        QOS_DEBUG_ERROR(
            "Unable to convert hw params to config params. "
            "Error: %d",
            rc);
        return (-1);
    }
    ...
    switch (cfg->rate_type) {
        ...
        case QOS_RATE_PPS:
            ...
            cfg->rb.pps.cb_ms   = qos_convert_burst_bytes_to_ms(kbps_cfg.rb.kbps.cb_bytes, kbps_cfg.rb.kbps.cir_kbps);
            cfg->rb.pps.eb_ms   = qos_convert_burst_bytes_to_ms(kbps_cfg.rb.kbps.eb_bytes, kbps_cfg.rb.kbps.eir_kbps);
            break;
        default:
            QOS_DEBUG_ERROR("Illegal rate type");
            return (-1);
