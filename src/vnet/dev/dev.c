...
vnet_dev_bus_t *vnet_dev_find_device_bus(vlib_main_t *vm, vnet_dev_device_id_t id)
{
    ...
    pool_foreach (bus, dm->buses) {
        ...
        if (strncmp(bus->registration->name, id, n)) continue;

        if (strncmp(VNET_DEV_DEVICE_ID_PREFIX_DELIMITER, id + n, dl)) continue;
...
void *vnet_dev_get_device_info(vlib_main_t *vm, vnet_dev_device_id_t id)
{
    vnet_dev_bus_t *bus;
...
vnet_dev_t *vnet_dev_alloc(vlib_main_t *vm, vnet_dev_device_id_t id, vnet_dev_driver_t *driver)
{
    vnet_dev_main_t *dm  = &vnet_dev_main;
    vnet_dev_t      *dev = 0, **devp = 0;

    dev = vnet_dev_alloc_with_data(sizeof(vnet_dev_t), driver->registration->device_data_sz);
    ...
    if ((vnet_dev_process_create(vm, dev)) == VNET_DEV_OK) return dev;

    vnet_dev_free(vm, dev);
    return 0;
}

vnet_dev_rv_t vnet_dev_init(vlib_main_t *vm, vnet_dev_t *dev)
{
    vnet_dev_main_t *dm  = &vnet_dev_main;
    ...
    if ((rv = bus->ops.device_open(vm, dev)) != VNET_DEV_OK) return rv;

    if (dev->ops.alloc) {
        rv = dev->ops.alloc(vm, dev);
        if (rv != VNET_DEV_OK) {
            log_err(dev, "device init failed [rv %d]", rv);
            if (dev->ops.deinit) dev->ops.deinit(vm, dev);
            if (dev->ops.free) dev->ops.free(vm, dev);
            return rv;
        }
    }
...
void vnet_dev_deinit(vlib_main_t *vm, vnet_dev_t *dev)
{
    ...
    if (bus->ops.device_close) bus->ops.device_close(vm, dev);
...
void vnet_dev_free(vlib_main_t *vm, vnet_dev_t *dev)
{
    vnet_dev_main_t *dm = &vnet_dev_main;
...
vnet_dev_rv_t vnet_dev_reset(vlib_main_t *vm, vnet_dev_t *dev)
{
    vnet_dev_rv_t rv;
    ...
    if (dev->ops.reset == 0) return VNET_DEV_ERR_NOT_SUPPORTED;
...
void vnet_dev_detach(vlib_main_t *vm, vnet_dev_t *dev)
{
    foreach_vnet_dev_port (p, dev)
        if (p->interface_created) vnet_dev_port_if_remove(vm, p);
    vnet_dev_deinit(vm, dev);
    vnet_dev_free(vm, dev);
}

vnet_dev_rv_t vnet_dev_dma_mem_alloc(vlib_main_t *vm, vnet_dev_t *dev, u32 size, u32 align, void **pp)
{
    vnet_dev_main_t *dm  = &vnet_dev_main;
    vnet_dev_bus_t  *bus = pool_elt_at_index(dm->buses, dev->bus_index);
    vnet_dev_rv_t    rv;
    ...
    if (!bus->ops.dma_mem_alloc_fn) return VNET_DEV_ERR_NOT_SUPPORTED;

    rv = bus->ops.dma_mem_alloc_fn(vm, dev, size, align, pp);
    if (rv == VNET_DEV_OK)
        log_debug(dev, "%u bytes va %p dma-addr 0x%lx numa %u align %u", size, *pp, vnet_dev_get_dma_addr(vm, dev, *pp), dev->numa_node, align);
    return rv;
}

void vnet_dev_dma_mem_free(vlib_main_t *vm, vnet_dev_t *dev, void *p)
{
    vnet_dev_main_t *dm  = &vnet_dev_main;
    vnet_dev_bus_t  *bus = pool_elt_at_index(dm->buses, dev->bus_index);
    ...
    if (p == 0 || !bus->ops.dma_mem_free_fn) return;

    return bus->ops.dma_mem_free_fn(vm, dev, p);
}

clib_error_t *vnet_dev_admin_up_down_fn(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    ...
    if (rv != VNET_DEV_OK) return clib_error_return(0, "failed to change port admin state: %U", format_vnet_dev_rv, rv);

    return 0;
}

static void vnet_dev_feature_update_cb(u32 sw_if_index, u8 arc_index, u8 is_enable, void *cb)
{
    vlib_main_t                *vm  = vlib_get_main();
    ...
    if (arc_index != vdm->eth_port_rx_feature_arc_index) return;

    hw   = vnet_get_sup_hw_interface(vnm, sw_if_index);
    port = vnet_dev_get_port_from_dev_instance(hw->dev_instance);

    if (port == 0 || port->intf.sw_if_index != sw_if_index) return;

    if (vnet_have_features(arc_index, sw_if_index)) {
        cm                   = &fm->feature_config_mains[arc_index];
        current_config_index = vec_elt(cm->config_index_by_sw_if_index, sw_if_index);
        vnet_get_config_data(&cm->config_main, &current_config_index, &next_index, 0);
        if (port->intf.feature_arc == 0 || port->intf.rx_next_index != next_index || port->intf.current_config_index != current_config_index) {
        ...
        if (port->intf.feature_arc) {
            ...
            port->intf.rx_next_index =
                port->intf.redirect_to_node ? port->intf.redirect_to_node_next_index : vnet_dev_default_next_index_by_port_type[port->attr.type];
    ...
    if (update_runtime) {
        foreach_vnet_dev_port_rx_queue (rxq, port)
            vnet_dev_rx_queue_rt_request(vm, rxq, (vnet_dev_rx_queue_rt_req_t){.update_next_index = 1, .update_feature_arc = 1});
        log_debug(port->dev,
                  "runtime update requested due to chgange in "
                  "feature arc configuration");
    }
}

static int sort_driver_registrations(void *a0, void *a1)
{
...
static clib_error_t *vnet_dev_main_init(vlib_main_t *vm)
{
    vnet_dev_main_t                 *dm            = &vnet_dev_main;
    ...
    for (vnet_dev_bus_registration_t *r = dm->bus_registrations; r; r = r->next_registration) {
        vnet_dev_bus_t *bus;
        pool_get_zero(dm->buses, bus);
        bus->registration = r;
        bus->index        = bus - dm->buses;
        bus->ops          = r->ops;
        if (!r->device_data_size || r->device_data_size > STRUCT_SIZE_OF(vnet_dev_t, bus_data))
            return clib_error_return(0, "bus device data for bus '%s' is too big not specified", r->name);

        log_debug(0, "bus '%s' registered", r->name);
    }

    for (vnet_dev_driver_registration_t *r = dm->driver_registrations; r; r = r->next_registration)
        vec_add1(drv, r);
    ...
    vec_foreach_pointer (r, drv) {
        ...
        if (bus_index < 0) return clib_error_return(0, "unknown bus '%s'", r->bus);
        ...
        *dev_class           = (vnet_device_class_t){
        ...
        log_debug(0, "driver '%s' registered on bus '%s'", r->name, bus->registration->name);

        if (temp_space_sz < r->runtime_temp_space_sz) temp_space_sz = r->runtime_temp_space_sz;
    }

    if (dm->startup_config) log_debug(0, "startup config: %v", dm->startup_config);
    ...
    if (temp_space_sz > 0) {
        ...
        dm->log2_runtime_temp_space_sz = get_lowest_set_bit_index(max_pow2(sz));
        ...
        log_debug(0,
                  ...
                  "per thread (total %u)",
                  temp_space_sz,
                  1 << dm->log2_runtime_temp_space_sz,
                  sz);
    }
...
clib_error_t *vnet_dev_num_workers_change(vlib_main_t *vm)
{
    vnet_dev_main_t *dm = &vnet_dev_main;

    if (dm->log2_runtime_temp_space_sz > 0) {
        const u32 align = CLIB_CACHE_LINE_BYTES;
        uword     sz    = (1ULL << dm->log2_runtime_temp_space_sz) * vlib_get_n_threads();
        if (dm->runtime_temp_spaces) clib_mem_free(dm->runtime_temp_spaces);
        dm->runtime_temp_spaces = clib_mem_alloc_aligned(sz, align);
        clib_memset(dm->runtime_temp_spaces, 0, sz);
