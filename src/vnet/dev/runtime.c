...
static void _vnet_dev_rt_exec_op(vlib_main_t *vm, vnet_dev_rt_op_t *op)
{
    ...
    foreach_vnet_dev_port_rx_queue (q, port) {
        if (q->rx_thread_index != vm->thread_index) continue;
...
static uword vnet_dev_rt_mgmt_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
vnet_dev_rv_t vnet_dev_rt_exec_ops(vlib_main_t *vm, vnet_dev_t *dev, vnet_dev_rt_op_t *ops, u32 n_ops)
{
    ...
    if (vlib_worker_thread_barrier_held()) {
        for (op = ops; op < (ops + n_ops); op++) {
            ...
            log_debug(dev, "port %u rx node runtime update on thread %u executed locally", op->port->port_id, op->thread_index);
        }
        return VNET_DEV_OK;
    }

    while (n_ops) {
        if (op->thread_index != vm->thread_index) break;
    ...
    if (n_ops == 0) return VNET_DEV_OK;

    for (op = ops; op < (ops + n_ops); op++) {
        if (op->thread_index == vm->thread_index && (op->in_order == 0 || vec_len(remote_ops) == 0)) {
            _vnet_dev_rt_exec_op(vm, op);
            log_debug(dev,
    ...
    if (remote_ops == 0) return VNET_DEV_OK;

    __atomic_store_n(&rt_ops, remote_ops, __ATOMIC_RELEASE);

    clib_bitmap_foreach (i, remote_bmp) {
        vlib_node_set_interrupt_pending(vlib_get_main_by_index(i), vnet_dev_rt_mgmt_node.index);
        log_debug(dev, "interrupt sent to %s node on thread %u", vnet_dev_rt_mgmt_node.name, i);
    }
