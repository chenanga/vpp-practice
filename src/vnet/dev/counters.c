...
vnet_dev_counter_main_t *vnet_dev_counters_alloc(vlib_main_t *vm, vnet_dev_counter_t *counters, u16 n_counters, char *fmt, ...)
{
    ...
    vec_validate_aligned(cm->counter_data, n_counters - 1, CLIB_CACHE_LINE_BYTES);
    vec_validate_aligned(cm->counter_start, n_counters - 1, CLIB_CACHE_LINE_BYTES);

    return cm;
}

void vnet_dev_counters_clear(vlib_main_t *vm, vnet_dev_counter_main_t *cm)
{
...
void vnet_dev_counters_free(vlib_main_t *vm, vnet_dev_counter_main_t *cm)
{
    vec_free(cm->desc);
...
u8 *format_vnet_dev_counter_name(u8 *s, va_list *va)
{
    ...
    if (c->type == VNET_DEV_CTR_TYPE_VENDOR) {
        ...
        if (c->unit < ARRAY_LEN(units) && units[c->unit]) s = format(s, " %s", units[c->unit]);

        if (c->dir < ARRAY_LEN(directions) && directions[c->dir]) s = format(s, " %s", directions[c->dir]);
    }
...
u8 *format_vnet_dev_counters(u8 *s, va_list *va)
{
    ...
    foreach_vnet_dev_counter (c, cm) {
        if (a->show_zero_counters == 0 && cm->counter_data[c->index] == 0) continue;

        if (line++) s = format(s, "\n%U", format_white_space, indent);

        s = format(s, "%-45U%lu", format_vnet_dev_counter_name, c, cm->counter_data[c->index]);
    }
