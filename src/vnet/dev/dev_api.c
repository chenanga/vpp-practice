...
#define _(b, n, d) STATIC_ASSERT((int) VL_API_DEV_FLAG_##n == (int) VNET_DEV_F_##n, "");
foreach_vnet_dev_flag;
#undef _

#define _(b, n, d) STATIC_ASSERT((int) VL_API_DEV_PORT_FLAG_##n == (int) VNET_DEV_PORT_F_##n, "");
foreach_vnet_dev_port_flag;
#undef _

static void vl_api_dev_attach_t_handler(vl_api_dev_attach_t *mp)
{
    ...
    if (rv != VNET_DEV_OK) error_string = format(0, "%U", format_vnet_dev_rv, rv);
    ...
    REPLY_MACRO3_END(VL_API_DEV_ATTACH_REPLY, vec_len(error_string), ({
                         ...
                         if (error_string) {
                             rmp->dev_index = ~0;
                             vl_api_vec_to_api_string(error_string, &rmp->error_string);
                         }
                         else
...
static void vl_api_dev_detach_t_handler(vl_api_dev_detach_t *mp)
{
    vlib_main_t               *vm = vlib_get_main();
    ...
    REPLY_MACRO3_END(VL_API_DEV_DETACH_REPLY, vec_len(error_string), ({
                         rmp->retval = rv;
                         if (error_string) vl_api_vec_to_api_string(error_string, &rmp->error_string);
                     }));

    vec_free(error_string);
}

static void vl_api_dev_create_port_if_t_handler(vl_api_dev_create_port_if_t *mp)
{
    vlib_main_t                       *vm = vlib_get_main();
...
static void vl_api_dev_remove_port_if_t_handler(vl_api_dev_remove_port_if_t *mp)
{
    vlib_main_t                       *vm = vlib_get_main();
...
static clib_error_t *vnet_dev_api_hookup(vlib_main_t *vm)
{
    ...
    foreach_int (i, VL_API_DEV_ATTACH, VL_API_DEV_DETACH, VL_API_DEV_CREATE_PORT_IF, VL_API_DEV_REMOVE_PORT_IF)
