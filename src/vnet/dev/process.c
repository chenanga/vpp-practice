...
static vnet_dev_rv_t vnet_dev_process_one_event(vlib_main_t *vm, vnet_dev_t *dev, vnet_dev_event_data_t *ed)
{
    vnet_dev_port_t *p;
...
static uword vnet_dev_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    vnet_dev_t *dev = *((vnet_dev_t **) vlib_node_get_runtime_data(vm, rt->node_index));

    log_debug(dev, "process '%U' started", format_vlib_node_name, vm, rt->node_index);
    ...
    while (dev->process_node_quit == 0) {
        ...
        if (new_event_data) {
            ...
            vec_foreach (ed, event_data) {
                ...
                if (ed->reply_needed) vlib_process_signal_event(vm, ed->calling_process_index, ed->event, rv);
            }
        ...
        pool_foreach (pop, dev->periodic_ops) {
            ...
            if (pop->last_run + pop->interval < next) next = pop->last_run + pop->interval;
        }
    ...
    log_debug(dev, "process '%U' quit", format_vlib_node_name, vm, rt->node_index);
...
vnet_dev_rv_t vnet_dev_process_create(vlib_main_t *vm, vnet_dev_t *dev)
{
    vnet_dev_main_t *dm = &vnet_dev_main;
    vlib_node_t     *n;
    uword            l;
    ...
    if (l > 0) {
        ...
        log_debug(dev, "process node '%U' (%u) reused", format_vlib_node_name, vm, n->index, n->index);
    }
    else {
        ...
        log_debug(dev, "process node '%U' (%u) created", format_vlib_node_name, vm, r.index, r.index);
    }
...
static void vnet_dev_process_event_send(vlib_main_t *vm, vnet_dev_t *dev, vnet_dev_event_data_t ed)
{
    vnet_dev_event_data_t *edp = vlib_process_signal_event_data(vm, dev->process_node_index, 0, 1, sizeof(ed));
    *edp                       = ed;
}

static vnet_dev_rv_t vnet_dev_process_event_send_and_wait(vlib_main_t *vm, vnet_dev_t *dev, vnet_dev_event_data_t ed)
{
    ...
    if (ed.calling_process_index == dev->process_node_index) return vnet_dev_process_one_event(vm, dev, &ed);
    ...
    if (event != ed.event) {
        log_err(dev, "%s", event == VNET_DEV_EVENT_CLOCK ? "timeout waiting for process node to respond" : "unexpected event received");
...
void vnet_dev_process_quit(vlib_main_t *vm, vnet_dev_t *dev)
{
    vnet_dev_event_data_t ed = {.event = VNET_DEV_EVENT_PROCESS_QUIT};
    vnet_dev_process_event_send_and_wait(vm, dev, ed);
}

static int _vnet_dev_poll_add(vlib_main_t *vm, vnet_dev_t *dev, vnet_dev_periodic_op_t pop)
{
    ...
    pool_foreach (p, dev->periodic_ops)
        if (p->op == pop.op && p->arg == pop.arg) return 0;

    pool_get_zero(dev->periodic_ops, p);
    *p = pop;
    if (pool_elts(dev->periodic_ops) == 1) vnet_dev_process_event_send(vm, dev, ed);
    return 1;
}

static int _vnet_dev_poll_remove(vlib_main_t *vm, vnet_dev_t *dev, void *op, void *arg)
{
    ...
    pool_foreach (pop, dev->periodic_ops)
        if (pop->op == op && pop->arg == arg) {
            ...
            if (pool_elts(dev->periodic_ops) == 0) vnet_dev_process_event_send(vm, dev, ed);
            return 1;
        }
    return 0;
}

void vnet_dev_poll_dev_add(vlib_main_t *vm, vnet_dev_t *dev, f64 interval, vnet_dev_op_no_rv_t *dev_op)
{
    vnet_dev_periodic_op_t pop = {
        .interval = interval,
    ...
    if (_vnet_dev_poll_add(vm, dev, pop) == 0) log_warn(dev, "poll_dev_add: op already exists, not added");
}

void vnet_dev_poll_dev_remove(vlib_main_t *vm, vnet_dev_t *dev, vnet_dev_op_no_rv_t *dev_op)
{
    if (_vnet_dev_poll_remove(vm, dev, (void *) dev_op, (void *) dev) == 0) log_warn(dev, "poll_dev_remove: op not found, not removed");
}

void vnet_dev_poll_port_add(vlib_main_t *vm, vnet_dev_port_t *port, f64 interval, vnet_dev_port_op_no_rv_t *port_op)
{
    ...
    if (_vnet_dev_poll_add(vm, dev, pop) == 0) log_warn(dev, "poll_port_add: op already exists, not added");
}

void vnet_dev_poll_port_remove(vlib_main_t *vm, vnet_dev_port_t *port, vnet_dev_port_op_no_rv_t *port_op)
{
    vnet_dev_t *dev = port->dev;
    if (_vnet_dev_poll_remove(vm, dev, (void *) port_op, (void *) port) == 0) log_warn(dev, "poll_port_remove: op not found, not removed");
}

vnet_dev_rv_t vnet_dev_process_port_cfg_change_req(vlib_main_t *vm, vnet_dev_port_t *port, vnet_dev_port_cfg_change_req_t *pccr)
{
...
vnet_dev_rv_t vnet_dev_process_call_op(vlib_main_t *vm, vnet_dev_t *dev, vnet_dev_op_t *op)
{
    const vnet_dev_event_data_t ed = {
...
vnet_dev_rv_t vnet_dev_process_call_op_no_rv(vlib_main_t *vm, vnet_dev_t *dev, vnet_dev_op_no_rv_t *op)
{
    const vnet_dev_event_data_t ed = {
...
void vnet_dev_process_call_op_no_wait(vlib_main_t *vm, vnet_dev_t *dev, vnet_dev_op_no_rv_t *op)
{
    const vnet_dev_event_data_t ed = {
...
vnet_dev_rv_t vnet_dev_process_call_port_op(vlib_main_t *vm, vnet_dev_port_t *port, vnet_dev_port_op_t *op)
{
    const vnet_dev_event_data_t ed = {
...
vnet_dev_rv_t vnet_dev_process_call_port_op_no_rv(vlib_main_t *vm, vnet_dev_port_t *port, vnet_dev_port_op_no_rv_t *op)
{
    const vnet_dev_event_data_t ed = {
...
void vnet_dev_process_call_port_op_no_wait(vlib_main_t *vm, vnet_dev_port_t *port, vnet_dev_port_op_no_rv_t *op)
{
    const vnet_dev_event_data_t ed = {
