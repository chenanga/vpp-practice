...
#define VNET_DEV_ARG_BOOL(ud, n, d, ...)                                            \
    {                                                                               \
        .type = VNET_DEV_ARG_TYPE_BOOL, .id = ud, .name = n, .desc = d, __VA_ARGS__ \
    }
#define VNET_DEV_ARG_UINT32(ud, n, d, ...)                                            \
    {                                                                                 \
        .type = VNET_DEV_ARG_TYPE_UINT32, .id = ud, .name = n, .desc = d, __VA_ARGS__ \
    }
#define VNET_DEV_ARG_STRING(ud, n, d, ...)                                            \
    {                                                                                 \
        .type = VNET_DEV_ARG_TYPE_STRING, .id = ud, .name = n, .desc = d, __VA_ARGS__ \
    }
...
#define VNET_DEV_ARGS(...)              \
    (vnet_dev_arg_t[])                  \
    {                                   \
        __VA_ARGS__, VNET_DEV_ARG_END() \
    }

#define foreach_vnet_dev_args(a, d)      for (typeof((d)->args[0]) *(a) = (d)->args; (a) < vec_end((d)->args); (a)++)
#define foreach_vnet_dev_port_args(a, p) for (typeof((p)->args[0]) *(a) = (p)->args; (a) < vec_end((p)->args); (a)++)
