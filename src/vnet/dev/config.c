...
static clib_error_t *vnet_dev_config_one_interface(vlib_main_t *vm, unformat_input_t *input, vnet_dev_api_create_port_if_args_t *args)
{
    clib_error_t *err = 0;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        if (unformat(input, "name %U", unformat_c_string_array, args->intf_name, sizeof(args->intf_name)))
            ;
        ...
        else if (unformat(input, "flags %U", unformat_vnet_dev_port_flags, &args->flags))
            ;
        else if (unformat(input, "args %U", unformat_single_quoted_string, &args->args))
            ;
        else {
...
static clib_error_t *vnet_dev_config_one_device(vlib_main_t *vm, unformat_input_t *input, char *device_id)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        if (unformat(input, "driver %U", unformat_c_string_array, args.driver_name, sizeof(args.driver_name)))
            ;
        else if (unformat(input, "flags %U", unformat_vnet_dev_flags, &args.flags))
            ;
        else if (unformat(input, "args %U", unformat_single_quoted_string, &args.args))
            ;
        else if (unformat(input, "port %u %U", &n, unformat_vlib_cli_sub_input, &sub_input)) {
    ...
    if (err == 0) {
        ...
        if (rv == VNET_DEV_OK) {
            vec_foreach (if_args, if_args_vec) {
                ...
                if (rv != VNET_DEV_OK) break;
            }
        }

        if (rv != VNET_DEV_OK) err = clib_error_return(0, "error: %U for device '%s'", format_vnet_dev_rv, rv, device_id);
    }
...
uword dev_config_process_node_fn(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    vnet_dev_main_t *dm = &vnet_dev_main;
    unformat_input_t input;
    clib_error_t    *err = 0;

    if (dm->startup_config == 0) return 0;
    ...
    while (!err && unformat_check_input(&input) != UNFORMAT_END_OF_INPUT) {
        ...
        if (unformat(&input, "dev %U %U", unformat_c_string_array, device_id, sizeof(device_id), unformat_vlib_cli_sub_input, &sub_input)) {
...
static clib_error_t *devices_config(vlib_main_t *vm, unformat_input_t *input)
{
    vnet_dev_main_t *dm = &vnet_dev_main;
    uword            c;
