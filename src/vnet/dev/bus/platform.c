...
#define log_debug(dev, f, ...)                                               \
    vlib_log(VLIB_LOG_LEVEL_DEBUG,                                           \
             ...
             ##__VA_ARGS__)
#define log_err(dev, f, ...) vlib_log(VLIB_LOG_LEVEL_ERR, dev_log.class, "%U" f, format_vnet_dev_log, dev, 0, ##__VA_ARGS__)

#define PLATFORM_DEV_PATH "/sys/bus/platform/devices"

clib_dt_main_t vnet_dev_bus_platform_dt_main;

vnet_dev_rv_t vnet_dev_bus_platform_dt_node_from_device_id(clib_dt_node_t **nodep, char *device_id)
{
    ...
    if (strncmp(CLIB_DT_LINUX_PREFIX, path, sizeof(CLIB_DT_LINUX_PREFIX) - 1) != 0) return VNET_DEV_ERR_BUG;

    n = clib_dt_get_node_with_path(dm, "%s", path + sizeof(CLIB_DT_LINUX_PREFIX) - 1);
...
static void *vnet_dev_bus_platform_get_device_info(vlib_main_t *vm, char *device_id)
{
    clib_dt_node_t                      *n = 0;
    ...
    if (n) {
        ...
        log_debug(0, "node found, is compatible %U", format_clib_dt_property_data, compatible);
...
static void vnet_dev_bus_platform_free_device_info(vlib_main_t *vm, void *p)
{
    clib_mem_free(p);
}

static void vnet_dev_bus_platform_close(vlib_main_t *vm, vnet_dev_t *dev)
{
    log_debug(dev, "");
}

static vnet_dev_rv_t vnet_dev_bus_platform_open(vlib_main_t *vm, vnet_dev_t *dev)
{
    clib_dt_node_t                      *n  = 0;
...
static u8 *format_dev_bus_platform_device_info(u8 *s, va_list *args)
{
...
static u8 *format_dev_bus_platform_device_addr(u8 *s, va_list *args)
{
    vnet_dev_t *dev = va_arg(*args, vnet_dev_t *);
