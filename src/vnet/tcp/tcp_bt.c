...
{
    if (pool_is_free_index(bt->samples, bts_index)) return 0;
...
{
    if (!bts) return TCP_BTS_INVALID_INDEX;
    return bts - bt->samples;
}

static inline int bt_seq_lt(u32 a, u32 b)
{
    return seq_lt(a, b);
}
...
static tcp_bt_sample_t *bt_alloc_sample(tcp_byte_tracker_t *bt, u32 min_seq, u32 max_seq)
{
    ...
    bts->min_seq          = min_seq;
    bts->max_seq          = max_seq;
    rb_tree_add_custom(&bt->sample_lookup, bts->min_seq, bts - bt->samples, bt_seq_lt);
    return bts;
}
...
static void bt_free_sample(tcp_byte_tracker_t *bt, tcp_bt_sample_t *bts)
{
    ...
    if (CLIB_DEBUG) memset(bts, 0xfc, sizeof(*bts));
...
static tcp_bt_sample_t *bt_merge_sample(tcp_byte_tracker_t *bt, tcp_bt_sample_t *prev, tcp_bt_sample_t *cur)
{
    ...
    if (bt_sample_index(bt, cur) == bt->tail) bt->tail = bt_sample_index(bt, prev);
...
static tcp_bt_sample_t *bt_lookup_seq(tcp_byte_tracker_t *bt, u32 seq)
{
    ...
    while (seq != cur->key) {
        ...
        if (rb_node_is_tnil(rt, cur)) {
            ...
            if (seq_geq(seq, bts->min_seq)) return bts;

            return 0;
        }
    }

    if (!rb_node_is_tnil(rt, cur)) return bt_get_sample(bt, cur->opaque);
...
static void bt_update_sample(tcp_byte_tracker_t *bt, tcp_bt_sample_t *bts, u32 seq)
{
    rb_tree_del_custom(&bt->sample_lookup, bts->min_seq, bt_seq_lt);
    bts->min_seq = seq;
    rb_tree_add_custom(&bt->sample_lookup, bts->min_seq, bt_sample_index(bt, bts), bt_seq_lt);
}
...
static tcp_bt_sample_t *bt_fix_overlapped(tcp_byte_tracker_t *bt, tcp_bt_sample_t *start, u32 seq, u8 is_end)
{
    ...
    if (cur && seq_lt(cur->min_seq, seq)) bt_update_sample(bt, cur, seq);
...
int tcp_bt_is_sane(tcp_byte_tracker_t *bt)
{
    ...
    if (pool_elts(bt->samples) != pool_elts(bt->sample_lookup.nodes) - 1) return 0;

    if (bt->head == TCP_BTS_INVALID_INDEX) {
        if (bt->tail != TCP_BTS_INVALID_INDEX) return 0;
    ...
    if (!bts || bts->prev != TCP_BTS_INVALID_INDEX) return 0;
    ...
    while (bts) {
        ...
        if (tmp) {
            if (tmp->prev != bt_sample_index(bt, bts)) {
                clib_warning("next %u thinks prev is %u should be %u", bts->next, tmp->prev, bt_sample_index(bt, bts));
                return 0;
            }
            if (!seq_lt(bts->min_seq, tmp->min_seq)) return 0;
        }
        else {
            if (bt->tail != bt_sample_index(bt, bts)) return 0;
...
void tcp_bt_track_tx(tcp_connection_t *tc, u32 len)
{
    ...
    if (tail && tail->max_seq == tc->snd_nxt && !(tail->flags & TCP_BTS_IS_SACKED) && tail->tx_time == tcp_time_now_us(tc->c_thread_index)) {
        tail->max_seq += len;
        return;
    }
...
void tcp_bt_track_rxt(tcp_connection_t *tc, u32 start, u32 end)
{
    ...
    if (bts && bts->max_seq == start && bts->tx_time == tcp_time_now_us(tc->c_thread_index)) {
        bts->max_seq = end;
        next         = bt_next_sample(bt, bts);
        if (next) bt_fix_overlapped(bt, next, end, is_end);
    ...
    /* Head in the past */
    if (seq_lt(bts->min_seq, tc->snd_una)) bt_update_sample(bt, bts, tc->snd_una);
    ...
    if (bts->min_seq == start) {
        ...
        if (bts_flags & TCP_BTS_IS_RXT) cur->flags |= TCP_BTS_IS_RXT_LOST;
        cur->next = next_index;
        cur->prev = prev_index;
...
{
    if (bts->flags & TCP_BTS_IS_SACKED) return;

    if (rs->prior_delivered && rs->prior_delivered >= bts->delivered) return;
...
static void tcp_bt_walk_samples_ooo(tcp_connection_t *tc, tcp_rate_sample_t *rs)
{
    ...
    for (i = 0; i < vec_len(blks); i++) {
        ...
        if (seq_lt(blk->end, tc->snd_una)) continue;

        cur = bt_lookup_seq(bt, blk->start);
        if (!cur) continue;

        ASSERT(seq_geq(blk->start, cur->min_seq) && seq_lt(blk->start, cur->max_seq));
...
void tcp_bt_sample_delivery_rate(tcp_connection_t *tc, tcp_rate_sample_t *rs)
{
    ...
    if (PREDICT_FALSE(tc->flags & TCP_CONN_FINSNT)) return;
    ...
    if (!delivered || tc->bt->head == TCP_BTS_INVALID_INDEX) return;

    tc->delivered += delivered;
    tc->delivered_time = tcp_time_now_us(tc->c_thread_index);

    if (tc->app_limited && tc->delivered > tc->app_limited) tc->app_limited = 0;

    if (tc->bytes_acked) tcp_bt_walk_samples(tc, rs);

    if (tc->sack_sb.last_sacked_bytes) tcp_bt_walk_samples_ooo(tc, rs);

    rs->interval_time    = clib_max((tc->delivered_time - rs->prior_time), rs->interval_time);
...
u8 *format_tcp_bt_sample(u8 *s, va_list *args)
{
    ...
    s                     = format(s,
               ...
               bts->max_seq - tc->iss,
               bts->delivered,
               now - bts->delivered_time,
               now - bts->tx_time,
               now - bts->first_tx_time,
               bts->flags);
    return s;
}
