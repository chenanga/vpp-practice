...
static void vl_api_tcp_configure_src_addresses_t_handler(vl_api_tcp_configure_src_addresses_t *mp)
{
    ...
    if (fa_af == IP46_TYPE_IP6)
        rv = tcp_configure_v6_source_address_range(vm, &first_address.ip6, &last_address.ip6, vrf_id);
    else
        rv = tcp_configure_v4_source_address_range(vm, &first_address.ip4, &last_address.ip4, vrf_id);
