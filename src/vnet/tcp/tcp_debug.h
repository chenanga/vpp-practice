...
#define TCP_EVT(_evt, _args...) \
    if (PREDICT_FALSE(tcp_evt_grp_dbg_lvl(_evt) >= tcp_evt_lvl(_evt))) tcp_evt_handler(_evt, _args)
...
#define TCP_DECLARE_ETD(_tc, _e, _size) \
    ...
    ed = ELOG_TRACK_DATA(&vlib_global_main.elog_main, _e, _tc->c_elog_track)
...
#define TCP_EVT_DEALLOC_HANDLER(_tc, ...)                                                      \
    {                                                                                          \
        ...
        vec_add1(tcp_dbg_main.free_track_indices, _tc->c_elog_track.track_index_plus_one - 1); \
    }
...
#define TCP_EVT_SYN_RCVD_HANDLER(_tc, _init, ...)            \
    {                                                        \
        if (_init) TCP_EVT_INIT_HANDLER(_tc, 0);             \
        ELOG_TYPE_DECLARE(_e) = {                            \
...
#define TCP_EVT_CC_EVT_HANDLER(_tc, _sub_evt, ...)                                                                          \
    {                                                                                                                       \
        if (_tc->snd_una != _tc->iss) TCP_EVT_CC_STAT_PRINT(_tc);                                                           \
        if ((_sub_evt <= 1 && TCP_DEBUG_CC > 1) || (_sub_evt > 1 && TCP_DEBUG_CC > 0)) TCP_EVT_CC_EVT_PRINT(_tc, _sub_evt); \
    }
#else
#define TCP_EVT_CC_EVT_HANDLER(_tc, _sub_evt, ...)
...
#define tcp_cc_time_to_print_stats(_tc) _tc->c_cc_stat_tstamp + STATS_INTERVAL < tcp_time_now_us(_tc->c_thread_index) || tcp_in_fastrecovery(_tc)
    ...
    {                                                                                                    \
        ...
        if (PREDICT_FALSE(tcp_cfg.buffer_fail_fraction != 0.0)) {                                        \
            if (PREDICT_TRUE(buffer_fail_counters[thread_index] > 0)) {                                  \
                if ((1.0 / (f32) (buffer_fail_counters[thread_index])) < tcp_cfg.buffer_fail_fraction) { \
