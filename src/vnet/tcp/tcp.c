...
static void tcp_add_del_adj_cb(tcp_add_del_adj_args_t *args)
{
    u32 ai;
    if (args->is_add) {
        adj_nbr_add_or_lock(args->nh_proto, args->link_type, &args->ip, args->sw_if_index);
    }
    else {
        ai = adj_nbr_find(FIB_PROTOCOL_IP6, VNET_LINK_IP6, &args->ip, args->sw_if_index);
        if (ai != ADJ_INDEX_INVALID) adj_unlock(ai);
    }
}
...
static void tcp_cc_init(tcp_connection_t *tc)
{
    ...
    if (tcp_opts_sack_permitted(&tc->rcv_opts)) tc->snd_congestion = tc->iss;
...
{
    if (tc->cc_algo->cleanup) tc->cc_algo->cleanup(tc);
}
...
static u32 tcp_connection_bind(u32 session_index, transport_endpoint_cfg_t *lcl)
{
    ...
    if (ip_is_zero(&lcl->ip, 1) && lcl->sw_if_index != ENDPOINT_INVALID_INDEX) {
        if ((iface_ip = ip_interface_get_first_ip(lcl->sw_if_index, lcl->is_ip4))) ip_set(&lcl->ip, iface_ip, lcl->is_ip4);
    }
...
static u32 tcp_session_bind(u32 session_index, transport_endpoint_cfg_t *tep)
{
...
static void tcp_connection_unbind(u32 listener_index)
{
    ...
    /* Poison the entry */
    if (CLIB_DEBUG > 0) clib_memset(tc, 0xFA, sizeof(*tc));
...
{
    ASSERT(vlib_get_thread_index() == tc->c_thread_index || vlib_thread_is_main_w_barrier());
...
int tcp_half_open_connection_cleanup(tcp_connection_t *tc)
{
    ...
    if (tc->c_thread_index != vlib_get_thread_index()) return 1;
...
void tcp_connection_cleanup(tcp_connection_t *tc)
{
    ...
    if (!(tc->cfg_flags & TCP_CFG_F_NO_ENDPOINT)) transport_release_local_endpoint(TRANSPORT_PROTO_TCP, &tc->c_lcl_ip, tc->c_lcl_port);
    ...
    if (tc->state == TCP_STATE_SYN_SENT) {
        ...
        if (!tc->c_is_ip4 && ip6_address_is_link_local_unicast(&tc->c_rmt_ip6)) tcp_add_del_adjacency(tc, 0);
        ...
        if (tc->cfg_flags & TCP_CFG_F_RATE_SAMPLE) tcp_bt_cleanup(tc);
...
void tcp_connection_close(tcp_connection_t *tc)
{
    ...
    switch (tc->state) {
        ...
        case TCP_STATE_SYN_RCVD:
            ...
            tcp_timer_update(&wrk->timer_wheel, tc, TCP_TIMER_WAITCLOSE, tcp_cfg.finwait1_time);
            break;
...
static void tcp_session_half_close(u32 conn_index, u32 thread_index)
{
    ...
    if (tc->state != TCP_STATE_ESTABLISHED) return;
    if (!transport_max_tx_dequeue(&tc->connection))
        tcp_send_fin(tc);
    else
        tc->flags |= TCP_CONN_FINPNDG;
...
static u32 tcp_generate_random_iss(tcp_connection_t *tc)
{
    ...
    if (tc->c_is_ip4)
        ...
        tmp = tc->c_lcl_ip.ip6.as_u64[0] ^ tc->c_lcl_ip.ip6.as_u64[1] ^ tc->c_rmt_ip.ip6.as_u64[0] ^ tc->c_rmt_ip.ip6.as_u64[1];
...
static void tcp_init_mss(tcp_connection_t *tc)
{
    ...
    if (tcp_opts_tstamp(&tc->rcv_opts)) tc->snd_mss -= TCP_OPTION_LEN_TIMESTAMP + 2 /* alignment */;
}
...
void tcp_init_snd_vars(tcp_connection_t *tc)
{
     ...
     */
    if (!tc->iss) tc->iss = tcp_generate_random_iss(tc);
    tc->snd_una = tc->iss;
    tc->snd_nxt = tc->iss + 1;
    tc->srtt    = 0.1 * THZ; /* 100 ms */

    if (!tcp_cfg.csum_offload) tc->cfg_flags |= TCP_CFG_F_NO_CSUM_OFFLOAD;
}
...
void tcp_connection_init_vars(tcp_connection_t *tc)
{
    ...
    if (tc->state == TCP_STATE_SYN_RCVD) tcp_init_snd_vars(tc);

    tcp_cc_init(tc);

    if (!tc->c_is_ip4 && ip6_address_is_link_local_unicast(&tc->c_rmt_ip6)) tcp_add_del_adjacency(tc, 1);

    /*  tcp_connection_fib_attach (tc); */

    if (transport_connection_is_tx_paced(&tc->connection) || tcp_cfg.enable_tx_pacing) tcp_enable_pacing(tc);

    if (tc->cfg_flags & TCP_CFG_F_RATE_SAMPLE) tcp_bt_init(tc);

    if (!tcp_cfg.allow_tso) tc->cfg_flags |= TCP_CFG_F_NO_TSO;

    tc->start_ts = tcp_time_now_us(tc->c_thread_index);
}

static int tcp_alloc_custom_local_endpoint(ip46_address_t *lcl_addr, u16 *lcl_port, transport_endpoint_cfg_t *rmt)
{
    tcp_main_t *tm = vnet_get_tcp_main();
    int         index, port;
    ...
    if (rmt->is_ip4) {
        ...
        if (tm->last_v4_addr_rotor >= vec_len(tcp_cfg.ip4_src_addrs)) tm->last_v4_addr_rotor = 0;
        clib_memset(lcl_addr, 0, sizeof(*lcl_addr));
        lcl_addr->ip4.as_u32 = tcp_cfg.ip4_src_addrs[index].as_u32;
    }
    else {
        index = tm->last_v6_addr_rotor++;
        if (tm->last_v6_addr_rotor >= vec_len(tcp_cfg.ip6_src_addrs)) tm->last_v6_addr_rotor = 0;
        clib_memcpy_fast(&lcl_addr->ip6, &tcp_cfg.ip6_src_addrs[index], sizeof(ip6_address_t));
    }
    port = transport_alloc_local_port(TRANSPORT_PROTO_TCP, lcl_addr, rmt);
    if (port < 1) return SESSION_E_NOPORT;
    *lcl_port = port;
    return 0;
}
...
static int tcp_session_open(transport_endpoint_cfg_t *rmt)
{
     ...
     */
    if ((rmt->is_ip4 && vec_len(tcp_cfg.ip4_src_addrs)) || (!rmt->is_ip4 && vec_len(tcp_cfg.ip6_src_addrs)))
        rv = tcp_alloc_custom_local_endpoint(&lcl_addr, &lcl_port, rmt);
    else
        rv = transport_alloc_local_endpoint(TRANSPORT_PROTO_TCP, rmt, &lcl_addr, &lcl_port);
    ...
    tcp_connection_timers_init(tc);
    tc->mss = rmt->mss;
    if (rmt->peer.sw_if_index != ENDPOINT_INVALID_INDEX) tc->sw_if_index = rmt->peer.sw_if_index;
...
static u8 *format_tcp_listener_session(u8 *s, va_list *args)
{
    ...
    if (verbose) s = format(s, "%-" SESSION_CLI_STATE_LEN "U", format_tcp_state, tc->state);
    if (verbose == 2) s = format(s, "\n%U", format_tcp_listener_connection, tc);
    return s;
}
...
static u8 *format_tcp_half_open_session(u8 *s, va_list *args)
{
    ...
    s = format(s, "%-" SESSION_CLI_ID_LEN "U", format_tcp_connection_id, tc);
    if (verbose) s = format(s, "%-" SESSION_CLI_STATE_LEN "v", state);
    vec_free(state);
    return s;
}
    ...
    switch (attr->type) {
        ...
        case TRANSPORT_ENDPT_ATTR_FLAGS:
            ...
            if (attr->flags & TRANSPORT_ENDPT_ATTR_F_GSO) {
                if (!(tc->cfg_flags & TCP_CFG_F_TSO)) tcp_check_gso(tc);
        ...
        case TRANSPORT_ENDPT_ATTR_CC_ALGO:
            if (tc->cc_algo == tcp_cc_algo_get(attr->cc_algo)) break;
            tcp_cc_cleanup(tc);
...
static int tcp_get_attribute(tcp_connection_t *tc, transport_endpt_attr_t *attr)
{
    ...
    switch (attr->type) {
        ...
        case TRANSPORT_ENDPT_ATTR_FLAGS:
            attr->flags = 0;
            if (!(tc->cfg_flags & TCP_CFG_F_NO_CSUM_OFFLOAD)) attr->flags |= TRANSPORT_ENDPT_ATTR_F_CSUM_OFFLOAD;
            if (tc->cfg_flags & TCP_CFG_F_TSO) attr->flags |= TRANSPORT_ENDPT_ATTR_F_GSO;
            if (tc->cfg_flags & TCP_CFG_F_RATE_SAMPLE) attr->flags |= TRANSPORT_ENDPT_ATTR_F_RATE_SAMPLING;
            break;
        case TRANSPORT_ENDPT_ATTR_CC_ALGO:
...
always_inline u32 tcp_round_snd_space(tcp_connection_t *tc, u32 snd_space)
{
     ...
     * don't try at all */
    if (PREDICT_FALSE(snd_space < tc->snd_mss)) return snd_space < tc->cwnd ? 0 : snd_space;
 ...
 */
static inline u32 tcp_snd_space_inline(tcp_connection_t *tc)
{
    int snd_space;
     ...
     */
    if (PREDICT_FALSE(tcp_in_cong_recovery(tc) || tc->state == TCP_STATE_CLOSED)) return 0;
    ...
    if (PREDICT_FALSE(tc->rcv_dupacks || tc->sack_sb.sacked_bytes)) {
        ...
        n_pkts = tcp_opts_sack_permitted(&tc->rcv_opts) ? tc->sack_sb.reorder - 1 : 2;

        if ((seq_lt(tc->limited_transmit, tc->snd_nxt - n_pkts * tc->snd_mss) || seq_gt(tc->limited_transmit, tc->snd_nxt)))
...
static int tcp_session_send_params(transport_connection_t *trans_conn, transport_send_params_t *sp)
{
    ...
    sp->snd_space = clib_min(tcp_snd_space_inline(tc), tc->snd_wnd - (tc->snd_nxt - tc->snd_una));
...
static void tcp_dispatch_pending_timers(tcp_worker_ctx_t *wrk)
{
    ...
    if (!(n_timers = clib_fifo_elts(wrk->pending_timers))) return;
    ...
    for (i = 0; i < clib_min(n_timers, wrk->max_timers_per_loop); i++) {
        ...
        if (PREDICT_FALSE(!(tc->pending_timers & (1 << timer_id)))) continue;

        tc->pending_timers &= ~(1 << timer_id);

        /* Skip timer if it was rearmed while pending dispatch */
        if (PREDICT_FALSE(tc->timers[timer_id] != TCP_TIMER_HANDLE_INVALID)) continue;

        (*timer_expiration_handlers[timer_id])(tc);
    }

    if (thread_index == 0 && clib_fifo_elts(wrk->pending_timers)) session_queue_run_on_main_thread(wrk->vm);
}
...
static void tcp_session_flush_data(transport_connection_t *tconn)
{
    ...
    if (tc->flags & TCP_CONN_PSH_PENDING) return;
...
static int tcp_session_app_rx_evt(transport_connection_t *conn)
{
    ...
    if (!(tc->flags & TCP_CONN_ZERO_RWND_SENT)) return 0;
...
{
    if (!transport_connection_is_tx_paced(&tc->connection)) return;

    f64 srtt = clib_min((f64) tc->srtt * TCP_TICK, tc->mrtt_us);

    transport_connection_tx_pacer_update(&tc->connection, tcp_cc_get_pacing_rate(tc), srtt * CLIB_US_TIME_FREQ);
}

void tcp_connection_tx_pacer_reset(tcp_connection_t *tc, u32 window, u32 start_bucket)
{
    f64 srtt = clib_min((f64) tc->srtt * TCP_TICK, tc->mrtt_us);
    transport_connection_tx_pacer_reset(&tc->connection, tcp_cc_get_pacing_rate(tc), start_bucket, srtt * CLIB_US_TIME_FREQ);
}

void tcp_reschedule(tcp_connection_t *tc)
{
    if (tcp_in_cong_recovery(tc) || tcp_snd_space_inline(tc)) transport_connection_reschedule(&tc->connection);
}
...
static void tcp_expired_timers_dispatch(u32 *expired_timers)
{
    ...
    max_loops                = clib_max((u32) 0.5 * TCP_TIMER_TICK * wrk->vm->loops_per_second, 1);
    max_per_loop             = clib_max((n_left + n_expired) / max_loops, 10);
    max_per_loop             = clib_min(max_per_loop, VLIB_FRAME_SIZE);
    wrk->max_timers_per_loop = clib_max(n_left ? wrk->max_timers_per_loop : 0, max_per_loop);
...
static void tcp_stats_collector_fn(vlib_stats_collector_data_t *d)
{
    tcp_main_t       *tm       = vnet_get_tcp_main();
...
static void tcp_counters_init(tcp_main_t *tm)
{
    vlib_stats_collector_reg_t r = {};
    u32                        idx;

    if (tm->counters_init) return;
...
#define _(name, type, str) vlib_stats_add_symlink(idx, TCP_STAT_##name, "/sys/tcp/%s", CLIB_STRING_MACRO(name));
    foreach_tcp_wrk_stat
#undef _
...
static clib_error_t *tcp_main_enable(vlib_main_t *vm)
{
    ...
    /* Already initialized */
    if (tm->wrk_ctx) return 0;

    if ((error = vlib_call_init_function(vm, ip_main_init))) return error;
    if ((error = vlib_call_init_function(vm, ip4_lookup_init))) return error;
    if ((error = vlib_call_init_function(vm, ip6_lookup_init))) return error;
    ...
    wrk                   = &tm->wrk_ctx[0];
    wrk->tco_next_node[0] = vlib_node_get_next(vm, session_queue_node.index, tcp4_output_node.index);
    wrk->tco_next_node[1] = vlib_node_get_next(vm, session_queue_node.index, tcp6_output_node.index);
    ...
    for (thread = 0; thread < num_threads; thread++) {
         ...
         */
        if ((thread > 0 || num_threads == 1) && prealloc_conn_per_wrk) pool_init_fixed(wrk->connections, prealloc_conn_per_wrk);

        tcp_timer_initialize_wheel(&wrk->timer_wheel, tcp_expired_timers_dispatch, vlib_time_now(vm));
    }
...
{
    if (is_en) {
        if (tcp_main.is_enabled) return 0;
...
static clib_error_t *tcp_init(vlib_main_t *vm)
{
    ...
    if (pi == 0) return clib_error_return(0, "TCP protocol info AWOL");
    pi->format_header    = format_tcp_header;
    pi->unformat_pg_edit = unformat_pg_tcp_header;

    /* Register as transport with session layer */
    transport_register_protocol(TRANSPORT_PROTO_TCP, &tcp_proto, FIB_PROTOCOL_IP4, tcp4_output_node.index);
    transport_register_protocol(TRANSPORT_PROTO_TCP, &tcp_proto, FIB_PROTOCOL_IP6, tcp6_output_node.index);
