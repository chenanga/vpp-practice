...
static void tcp_debug_show_groups(void)
{
    ...
    for (i = 0; i < TCP_EVT_N_GRP; i++)
        vlib_cli_output(vm, "%-10d%-30s%-10d", i, tcp_evt_grp_str[i], tdm->grp_dbg_lvl[i]);
}
...
static void tcp_debug_check_lc(void)
{
    ...
    if (tdm->grp_dbg_lvl[TCP_EVT_GRP_LC]) return;
    ...
    if (have_enabled) tdm->grp_dbg_lvl[TCP_EVT_GRP_LC] = 1;
}
...
static clib_error_t *tcp_debug_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!TCP_DEBUG_ALWAYS) return clib_error_return(0, "must compile with TCP_DEBUG_ALWAYS set");
