...
static u8 *format_tcp_cfg_flags(u8 *s, va_list *args)
{
    ...
    for (i = 0; i < TCP_CFG_N_FLAG_BITS; i++)
        if (tc->cfg_flags & (1 << i)) last = i;
    for (i = 0; i < last; i++) {
        if (tc->cfg_flags & (1 << i)) s = format(s, "%s, ", tcp_cfg_flags_str[i]);
    }
    if (last >= 0) s = format(s, "%s", tcp_cfg_flags_str[last]);
    return s;
}
...
static u8 *format_tcp_connection_flags(u8 *s, va_list *args)
{
    ...
    for (i = 0; i < TCP_CONN_N_FLAG_BITS; i++)
        if (tc->flags & (1 << i)) last = i;
    for (i = 0; i < last; i++) {
        if (tc->flags & (1 << i)) s = format(s, "%s, ", tcp_connection_flags_str[i]);
    }
    if (last >= 0) s = format(s, "%s", tcp_connection_flags_str[last]);
    return s;
}
...
static u8 *format_tcp_timers(u8 *s, va_list *args)
{
    ...
    for (i = 0; i < TCP_N_TIMERS; i++)
        if (tc->timers[i] != TCP_TIMER_HANDLE_INVALID) last = i;

    for (i = 0; i < last; i++) {
        if (tc->timers[i] != TCP_TIMER_HANDLE_INVALID) s = format(s, "%s,", tcp_conn_timers[i]);
    }

    if (last >= 0) s = format(s, "%s", tcp_conn_timers[i]);
...
static u8 *format_tcp_congestion(u8 *s, va_list *args)
{
    ...
    if (tcp_in_fastrecovery(tc)) prr_space = tcp_fastrecovery_prr_snd_space(tc);
    s = format(s,
...
static u8 *format_tcp_stats(u8 *s, va_list *args)
{
    ...
    s                        = format(s,
               ...
               tc->errors.above_data_wnd,
               tc->errors.below_ack_wnd,
...
static u8 *format_tcp_vars(u8 *s, va_list *args)
{
    ...
    s                    = format(s,
               ...
               format_tcp_timers,
               tc);
    s                    = format(s, " snd_una %u snd_nxt %u", tc->snd_una - tc->iss, tc->snd_nxt - tc->iss);
    s                    = format(s, " rcv_nxt %u rcv_las %u\n", tc->rcv_nxt - tc->irs, tc->rcv_las - tc->irs);
    s                    = format(s, " snd_wnd %u rcv_wnd %u rcv_wscale %u ", tc->snd_wnd, tc->rcv_wnd, tc->rcv_wscale);
    s                    = format(s, "snd_wl1 %u snd_wl2 %u\n", tc->snd_wl1 - tc->irs, tc->snd_wl2 - tc->iss);
    s = format(s, " flight size %u out space %u rcv_wnd_av %u", tcp_flight_size(tc), tcp_available_output_snd_space(tc), tcp_rcv_wnd_available(tc));
    ...
    if (vec_len(tc->snd_sacks)) s = format(s, " sacks tx: %U\n", format_tcp_sacks, tc);
...
u8 *format_tcp_listener_connection(u8 *s, va_list *args)
{
    tcp_connection_t *tc = va_arg(*args, tcp_connection_t *);

    s = format(s, " index: %u cfg_flags: %U cong_algo: %s snd_mss: %u\n", tc->c_c_index, format_tcp_cfg_flags, tc, tc->cc_algo->name, tc->snd_mss);
    s = format(s,
               " next_node %u opaque 0x%x fib_index %u sw_if_index %d",
               tc->next_node_index,
               tc->next_node_opaque,
...
u8 *format_tcp_sacks(u8 *s, va_list *args)
{
    ...
    for (i = 0; i < len - 1; i++) {
        block = &sacks[i];
        s     = format(s, " start %u end %u\n", block->start - tc->irs, block->end - tc->irs);
    }
    if (len) {
        block = &sacks[len - 1];
        s     = format(s, " start %u end %u", block->start - tc->irs, block->end - tc->irs);
    }
    return s;
}
...
u8 *format_tcp_rcv_sacks(u8 *s, va_list *args)
{
    ...
    for (i = 0; i < len - 1; i++) {
        block = &sacks[i];
        s     = format(s, " start %u end %u\n", block->start - tc->iss, block->end - tc->iss);
    }
    if (len) {
        block = &sacks[len - 1];
        s     = format(s, " start %u end %u", block->start - tc->iss, block->end - tc->iss);
    }
    return s;
}
...
int tcp_configure_v4_source_address_range(vlib_main_t *vm, ip4_address_t *start, ip4_address_t *end, u32 table_id)
{
    ...
    if ((end_host_byte_order - start_host_byte_order) > (10 << 10)) return VNET_API_ERROR_INVALID_ARGUMENT;
    ...
    if (fei == FIB_NODE_INDEX_INVALID) return VNET_API_ERROR_NEXT_HOP_NOT_IN_FIB;

    sw_if_index = fib_entry_get_resolving_interface(fei);
    if (sw_if_index == (u32) ~0) return VNET_API_ERROR_NO_MATCHING_INTERFACE;
    ...
    do {
        ...
        receive_dpo_add_or_lock(DPO_PROTO_IP4, sw_if_index /* sw_if_index */, NULL, &dpo);
        prefix.fp_len             = 32;
        prefix.fp_proto           = FIB_PROTOCOL_IP4;
        prefix.fp_addr.ip4.as_u32 = start->as_u32;

        fib_table_entry_special_dpo_update(fib_index, &prefix, FIB_SOURCE_API, FIB_ENTRY_FLAG_EXCLUSIVE, &dpo);
        dpo_reset(&dpo);
...
int tcp_configure_v6_source_address_range(vlib_main_t *vm, ip6_address_t *start, ip6_address_t *end, u32 table_id)
{
    ...
    while (1) {
        ...
        receive_dpo_add_or_lock(DPO_PROTO_IP6, sw_if_index /* sw_if_index */, NULL, &dpo);

        fib_table_entry_special_dpo_update(fib_index, &prefix, FIB_SOURCE_API, FIB_ENTRY_FLAG_EXCLUSIVE, &dpo);
        dpo_reset(&dpo);

        /* Done with the entire range? */
        if (!memcmp(start, end, sizeof(start[0]))) break;

        /* Increment the address. DGMS. */
        tmp = start[0];
        for (i = 15; i >= 0; i--) {
            tmp.as_u8[i] += 1;
            if (tmp.as_u8[i] != 0) break;
        }
        start[0] = tmp;
    }
    return 0;
...
static clib_error_t *tcp_src_address_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "%U - %U", unformat_ip4_address, &v4start, unformat_ip4_address, &v4end))
            v4set = 1;
        else if (unformat(input, "%U", unformat_ip4_address, &v4start)) {
            memcpy(&v4end, &v4start, sizeof(v4start));
            v4set = 1;
        }
        else if (unformat(input, "%U - %U", unformat_ip6_address, &v6start, unformat_ip6_address, &v6end))
            v6set = 1;
    ...
    if (!v4set && !v6set) return clib_error_return(0, "at least one v4 or v6 address required");

    if (v4set) {
        rv = tcp_configure_v4_source_address_range(vm, &v4start, &v4end, table_id);
        switch (rv) {
            case 0:
                break;
            ...
            case VNET_API_ERROR_INVALID_ARGUMENT:
                return clib_error_return(0, "Invalid address range %U - %U", format_ip4_address, &v4start, format_ip4_address, &v4end);
            default:
    ...
    if (v6set) {
        rv = tcp_configure_v6_source_address_range(vm, &v6start, &v6end, table_id);
        switch (rv) {
            case 0:
                break;
...
static u8 *tcp_scoreboard_dump_trace(u8 *s, sack_scoreboard_t *sb)
{
    ...
    if (!sb->trace) return s;

    s = format(s, "scoreboard trace:");
    vec_foreach (block, sb->trace) {
        s = format(s, "{%u, %u, %u, %u, %u}, ", block->start, block->end, block->ack, block->snd_una_max, block->group);
        if ((++i % 3) == 0) s = format(s, "\n");
    }
    return s;
#else
...
static clib_error_t *tcp_show_scoreboard_trace_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
    ...
    u8                     *s = 0;
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "%U", unformat_transport_connection, &tconn, TRANSPORT_PROTO_TCP))
            ;
        else
...
u8 *tcp_scoreboard_replay(u8 *s, tcp_connection_t *tc, u8 verbose)
{
    ...
    while (left < trace_len) {
        ...
        while (trace[left].group == group) {
            if (trace[left].ack != 0) {
                if (verbose) s = format(s, "Adding ack %u, snd_una_max %u, segs: ", trace[left].ack, trace[left].snd_nxt);
                placeholder_tc->snd_nxt = trace[left].snd_nxt;
                next_ack                = trace[left].ack;
                has_new_ack             = 1;
            }
            else {
                if (verbose) s = format(s, "[%u, %u], ", trace[left].start, trace[left].end);
        ...
        if (has_new_ack) placeholder_tc->snd_una = next_ack;

        if (verbose) s = format(s, "result: %U", format_tcp_scoreboard, &placeholder_tc->sack_sb);
    }
    s = format(s, "result: %U", format_tcp_scoreboard, &placeholder_tc->sack_sb);
...
static clib_error_t *show_tcp_punt_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
    ...
    vlib_cli_output(vm, "IPv4 TCP punt: %s", tm->punt_unknown4 ? "enabled" : "disabled");
    vlib_cli_output(vm, "IPv6 TCP punt: %s", tm->punt_unknown6 ? "enabled" : "disabled");
    return 0;
}
...
static u8 *format_tcp_cfg(u8 *s, va_list *args)
{
    ...
    s = format(s, "min rx fifo size: %U\n", format_memory_size, tm_cfg.min_rx_fifo);
    ...
    s = format(s, "max gso packet size: %U\n", format_memory_size, tm_cfg.max_gso_size);
    ...
    s = format(s, "tcp preallocated connections: %u", tm_cfg.preallocated_connections);

    return s;
}

static clib_error_t *show_tcp_cfg_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    tcp_main_t *tm = vnet_get_tcp_main();
...
static clib_error_t *show_tcp_stats_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (thread = 0; thread < vec_len(tm->wrk_ctx); thread++) {
        ...
        if (clib_fifo_elts(wrk->pending_timers)) vlib_cli_output(vm, " %lu pending timers", clib_fifo_elts(wrk->pending_timers));

#define _(name, type, str) \
    if (wrk->stats.name) vlib_cli_output(vm, " %lu %s", wrk->stats.name, str);
...
uword unformat_tcp_cc_algo(unformat_input_t *input, va_list *va)
{
    ...
    if (unformat(input, "%s", &cc_algo_name) && ((p = hash_get_mem(tm->cc_algo_by_name, cc_algo_name)))) {
        *result = *p;
        found   = 1;
    }
...
uword unformat_tcp_cc_algo_cfg(unformat_input_t *input, va_list *va)
{
    ...
    vec_foreach (cc_alg, tm->cc_algos) {
        if (!unformat(input, cc_alg->name)) continue;

        if (cc_alg->unformat_cfg && unformat(input, "%U", unformat_vlib_cli_sub_input, &sub_input)) {
            if (cc_alg->unformat_cfg(&sub_input)) found = 1;
        }
    }
    return found;
}
...
static clib_error_t *tcp_config_fn(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "preallocated-connections %d", &tcp_cfg.preallocated_connections))
            ;
        /* Config deprecated. Will be removed in a later release */
        else if (unformat(input, "preallocated-half-open-connections %d", &tmp))
            ;
        else if (unformat(input, "buffer-fail-fraction %f", &tcp_cfg.buffer_fail_fraction))
            ;
        else if (unformat(input, "max-rx-fifo %U", unformat_memory_size, &memory_size)) {
            if (memory_size >= 0x100000000) {
                return clib_error_return(0, "max-rx-fifo %llu (0x%llx) too large", memory_size, memory_size);
            }
            tcp_cfg.max_rx_fifo = memory_size;
        }
        else if (unformat(input, "min-rx-fifo %U", unformat_memory_size, &memory_size)) {
            if (memory_size >= 0x100000000) {
                return clib_error_return(0, "min-rx-fifo %llu (0x%llx) too large", memory_size, memory_size);
            }
            tcp_cfg.min_rx_fifo = memory_size;
        }
        else if (unformat(input, "mtu %u", &mtu))
            tcp_cfg.default_mtu = mtu;
        else if (unformat(input, "rwnd-min-update-ack %d", &tcp_cfg.rwnd_min_update_ack))
            ;
