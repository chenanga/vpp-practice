...
always_inline void tcp_node_inc_counter_i(vlib_main_t *vm, u32 tcp4_node, u32 tcp6_node, u8 is_ip4, u32 evt, u32 val)
{
    if (is_ip4)
...
#define tcp_inc_counter(node_id, err, count) tcp_node_inc_counter_i(vm, tcp4_##node_id##_node.index, tcp6_##node_id##_node.index, is_ip4, err, count)
...
#define tcp_store_err_counters(node_id, cnts)                  \
    {                                                          \
        ...
        for (i = 0; i < TCP_N_ERROR; i++)                      \
            if (cnts[i]) tcp_inc_counter(node_id, i, cnts[i]); \
    }

always_inline tcp_header_t *tcp_buffer_hdr(vlib_buffer_t *b)
{
    ASSERT((signed) b->current_data >= (signed) -VLIB_BUFFER_PRE_DATA_SIZE);
    return (tcp_header_t *) (b->data + b->current_data + vnet_buffer(b)->tcp.hdr_offset);
}

always_inline tcp_connection_t *tcp_connection_get(u32 conn_index, u32 thread_index)
{
    tcp_worker_ctx_t *wrk = tcp_get_worker(thread_index);
    if (PREDICT_FALSE(pool_is_free_index(wrk->connections, conn_index))) return 0;
    return pool_elt_at_index(wrk->connections, conn_index);
}

always_inline tcp_connection_t *tcp_connection_get_if_valid(u32 conn_index, u32 thread_index)
{
    tcp_worker_ctx_t *wrk;
    if (thread_index >= vec_len(tcp_main.wrk_ctx)) return 0;
    wrk = tcp_get_worker(thread_index);
    if (pool_is_free_index(wrk->connections, conn_index)) return 0;
    return pool_elt_at_index(wrk->connections, conn_index);
}

always_inline void tcp_connection_set_state(tcp_connection_t *tc, tcp_state_t state)
{
    tc->state = state;
    TCP_EVT(TCP_EVT_STATE_CHANGE, tc);
}

always_inline tcp_connection_t *tcp_listener_get(u32 tli)
{
    tcp_connection_t *tc = 0;
    if (!pool_is_free_index(tcp_main.listener_pool, tli)) tc = pool_elt_at_index(tcp_main.listener_pool, tli);
    return tc;
}

always_inline tcp_connection_t *tcp_half_open_connection_get(u32 conn_index)
{
 ...
 */
always_inline u32 tcp_bytes_out(const tcp_connection_t *tc)
{
    if (tcp_opts_sack_permitted(&tc->rcv_opts))
        return tc->sack_sb.sacked_bytes + tc->sack_sb.lost_bytes;
    else
        return clib_min(tc->rcv_dupacks * tc->snd_mss, tc->snd_nxt - tc->snd_una);
}

/**
 * Our estimate of the number of bytes in flight (pipe size)
 */
always_inline u32 tcp_flight_size(const tcp_connection_t *tc)
{
    int flight_size;

    flight_size = (int) (tc->snd_nxt - tc->snd_una) - tcp_bytes_out(tc) + tc->snd_rxt_bytes - tc->rxt_delivered;
 ...
 */
always_inline u32 tcp_initial_cwnd(const tcp_connection_t *tc)
{
    if (tcp_cfg.initial_cwnd_multiplier > 0) return tcp_cfg.initial_cwnd_multiplier * tc->snd_mss;
 ...
 * to the cwnd.
 */
always_inline void tcp_cwnd_accumulate(tcp_connection_t *tc, u32 thresh, u32 bytes)
{
...
always_inline u32 tcp_loss_wnd(const tcp_connection_t *tc)
{
    /* Whatever we have in flight + the packet we're about to send */
    return tcp_flight_size(tc) + tc->snd_mss;
}

always_inline u32 tcp_available_snd_wnd(const tcp_connection_t *tc)
{
    return clib_min(tc->cwnd, tc->snd_wnd);
}

always_inline u32 tcp_available_output_snd_space(const tcp_connection_t *tc)
{
    u32 available_wnd = tcp_available_snd_wnd(tc);
    int flight_size   = (int) (tc->snd_nxt - tc->snd_una);

    if (available_wnd <= flight_size) return 0;
 ...
 */
always_inline u32 tcp_available_cc_snd_space(const tcp_connection_t *tc)
{
    u32 available_wnd = tcp_available_snd_wnd(tc);
...
always_inline u8 tcp_is_lost_fin(tcp_connection_t *tc)
{
    if ((tc->flags & TCP_CONN_FINSNT) && (tc->snd_nxt - tc->snd_una == 1)) return 1;
    return 0;
}
 ...
 */
always_inline u32 tcp_time_tstamp(u32 thread_index)
{
 ...
 */
always_inline u32 tcp_tstamp(tcp_connection_t *tc)
{
    return (tcp_main.wrk_ctx[tc->c_thread_index].time_tstamp - tc->timestamp_delta);
}

always_inline f64 tcp_time_now_us(u32 thread_index)
{
    return tcp_main.wrk_ctx[thread_index].time_us;
}

always_inline void tcp_set_time_now(tcp_worker_ctx_t *wrk, f64 now)
{
...
always_inline void tcp_update_time_now(tcp_worker_ctx_t *wrk)
{
...
always_inline tcp_connection_t *tcp_input_lookup_buffer(vlib_buffer_t *b, u8 thread_index, u32 *error, u8 is_ip4, u8 is_nolookup)
{
    ...
    if (is_ip4) {
        ...
        if (!is_nolookup)
            tc = session_lookup_connection_wt4(fib_index,
                                               &ip4->dst_address,
                                               &ip4->src_address,
                                               tcp->dst_port,
                                               tcp->src_port,
        ...
        n_data_bytes                   = clib_net_to_host_u16(ip6->payload_length) - n_advance_bytes;
        ...
        if (!is_nolookup) {
            if (PREDICT_FALSE(ip6_address_is_link_local_unicast(&ip6->dst_address))) {
                ip6_main_t *im = &ip6_main;
                fib_index      = vec_elt(im->fib_index_by_sw_if_index, vnet_buffer(b)->ip.rx_sw_if_index);
            }

            tc = session_lookup_connection_wt6(fib_index,
                                               &ip6->dst_address,
                                               &ip6->src_address,
                                               tcp->dst_port,
                                               tcp->src_port,
    ...
    if (is_nolookup) tc = (transport_connection_t *) tcp_connection_get(vnet_buffer(b)->tcp.connection_index, thread_index);
    ...
    vnet_buffer(b)->tcp.seq_end     = vnet_buffer(b)->tcp.seq_number + n_data_bytes;
 ...
 */
always_inline void tcp_init_w_buffer(tcp_connection_t *tc, vlib_buffer_t *b, u8 is_ip4)
{
    ...
    if (is_ip4) {
        ...
        ip6_header_t *ip6 = vlib_buffer_get_current(b);
        clib_memcpy_fast(&tc->c_lcl_ip6, &ip6->dst_address, sizeof(ip6_address_t));
        clib_memcpy_fast(&tc->c_rmt_ip6, &ip6->src_address, sizeof(ip6_address_t));
    }
    ...
    if (tcp_opts_wscale(&tc->rcv_opts)) tc->snd_wscale = tc->rcv_opts.wscale;

    tc->snd_wnd = clib_net_to_host_u16(th->window) << tc->snd_wscale;
}

always_inline void tcp_update_rto(tcp_connection_t *tc)
{
    tc->rto = clib_min(tc->srtt + (tc->rttvar << 2), TCP_RTO_MAX);
    tc->rto = clib_max(tc->rto, TCP_RTO_MIN);
}

always_inline u8 tcp_is_descheduled(tcp_connection_t *tc)
{
 ...
 */
always_inline void *vlib_buffer_push_tcp_net_order(vlib_buffer_t *b, u16 sp, u16 dp, u32 seq, u32 ack, u8 tcp_hdr_opts_len, u8 flags, u16 wnd)
{
    tcp_header_t *th;
 ...
 * @return - pointer to start of TCP header
 */
always_inline void *vlib_buffer_push_tcp(vlib_buffer_t *b, u16 sp_net, u16 dp_net, u32 seq, u32 ack, u8 tcp_hdr_opts_len, u8 flags, u16 wnd)
{
    return vlib_buffer_push_tcp_net_order(b,
                                          sp_net,
                                          dp_net,
