...
void newreno_rcv_cong_ack(tcp_connection_t *tc, tcp_cc_ack_t ack_type, tcp_rate_sample_t *rs)
{
    ...
    if (PREDICT_TRUE(tcp_opts_sack_permitted(tc))) return;
    ...
    if (ack_type == TCP_CC_DUPACK) {
        ...
        if (tc->bytes_acked > tc->snd_mss) tc->cwnd += tc->snd_mss;
    }
}
...
const static tcp_cc_algorithm_t tcp_newreno = {.name         = "newreno",
