...
#define TCP_TO_TIMER_TICK                       \
    TCP_TICK * 10000 /**< Factor for converting \
...
typedef enum _tcp_cc_algorithm_type { TCP_CC_NEWRENO, TCP_CC_CUBIC, TCP_CC_LAST = TCP_CC_CUBIC } tcp_cc_algorithm_type_e;

typedef struct _tcp_cc_algorithm tcp_cc_algorithm_t;

typedef enum _tcp_cc_ack_t { TCP_CC_ACK, TCP_CC_DUPACK, TCP_CC_PARTIALACK } tcp_cc_ack_t;
...
struct _tcp_cc_algorithm {
    ...
    void (*rcv_cong_ack)(tcp_connection_t *tc, tcp_cc_ack_t ack, tcp_rate_sample_t *rs);
...
#define tcp_in_cong_recovery(tc) ((tc)->flags & (TCP_CONN_FAST_RECOVERY | TCP_CONN_RECOVERY))

always_inline void tcp_cong_recovery_off(tcp_connection_t *tc)
{
...
always_inline tcp_connection_t *tcp_get_connection_from_transport(transport_connection_t *tconn)
{
