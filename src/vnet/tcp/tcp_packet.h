...
always_inline void ip4_tcp_reply_x1(ip4_header_t *ip0, tcp_header_t *tcp0)
{
    u32 src0, dst0;
...
always_inline void ip4_tcp_reply_x2(ip4_header_t *ip0, ip4_header_t *ip1, tcp_header_t *tcp0, tcp_header_t *tcp1)
{
 ...
 */
always_inline int tcp_options_parse(tcp_header_t *th, tcp_options_t *to, u8 is_syn)
{
    const u8    *data;
    ...
    to->flags &= (TCP_OPTS_FLAG_SACK_PERMITTED | TCP_OPTS_FLAG_WSCALE | TCP_OPTS_FLAG_TSTAMP | TCP_OPTS_FLAG_MSS);
    ...
    for (; opts_len > 0; opts_len -= opt_len, data += opt_len) {
        ...
        if (kind == TCP_OPTION_EOL)
            ...
            /* broken options */
            if (opts_len < 2) return -1;
            opt_len = data[1];

            /* weird option length */
            if (opt_len < 2 || opt_len > opts_len) return -1;
        }
        ...
        switch (kind) {
            ...
            case TCP_OPTION_WINDOW_SCALE:
                ...
                if ((opt_len == TCP_OPTION_LEN_WINDOW_SCALE) && tcp_syn(th)) {
                    ...
                    to->wscale = data[2];
                    if (to->wscale > TCP_MAX_WND_SCALE) to->wscale = TCP_MAX_WND_SCALE;
                }
                break;
            case TCP_OPTION_TIMESTAMP:
                if (is_syn) to->flags |= TCP_OPTS_FLAG_TSTAMP;
                if ((to->flags & TCP_OPTS_FLAG_TSTAMP) && opt_len == TCP_OPTION_LEN_TIMESTAMP) {
            ...
            case TCP_OPTION_SACK_PERMITTED:
                if (!is_syn) break;
                if (opt_len == TCP_OPTION_LEN_SACK_PERMITTED && tcp_syn(th)) to->flags |= TCP_OPTS_FLAG_SACK_PERMITTED;
                break;
            case TCP_OPTION_SACK_BLOCK:
                /* If SACK permitted was not advertised or a SYN, break */
                if ((to->flags & TCP_OPTS_FLAG_SACK_PERMITTED) == 0 || tcp_syn(th)) break;

                /* If too short or not correctly formatted, break */
                if (opt_len < 10 || ((opt_len - 2) % TCP_OPTION_LEN_SACK_BLOCK)) break;
 ...
 */
always_inline u32 tcp_options_write(u8 *data, tcp_options_t *opts)
{
    u32 opts_len = 0;
    u32 buf, seq_len = 4;
