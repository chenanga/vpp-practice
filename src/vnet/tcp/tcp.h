...
always_inline tcp_main_t *vnet_get_tcp_main()
{
    return &tcp_main;
}

always_inline tcp_worker_ctx_t *tcp_get_worker(u32 thread_index)
{
...
#define tcp_validate_txf_size(_tc, _a) ASSERT(_tc->state != TCP_STATE_ESTABLISHED || transport_max_tx_dequeue(&_tc->connection) >= _a)
