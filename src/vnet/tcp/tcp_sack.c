...
static void scoreboard_remove_hole(sack_scoreboard_t *sb, sack_scoreboard_hole_t *hole)
{
    ...
    if (scoreboard_hole_index(sb, hole) == sb->cur_rxt_hole) sb->cur_rxt_hole = TCP_INVALID_SACK_HOLE_INDEX;

    /* Poison the entry */
    if (CLIB_DEBUG > 0) clib_memset(hole, 0xfe, sizeof(*hole));
...
always_inline void scoreboard_update_sacked(sack_scoreboard_t *sb, u32 start, u32 end, u8 has_rxt, u16 snd_mss)
{
    if (!has_rxt) {
    ...
    if (seq_geq(start, sb->high_rxt)) return;

    sb->rxt_sacked += seq_lt(end, sb->high_rxt) ? (end - start) : (sb->high_rxt - start);
}

always_inline void scoreboard_update_bytes(sack_scoreboard_t *sb, u32 ack, u32 snd_mss)
{
    ...
    while (sacked <= (sb->reorder - 1) * snd_mss && blks < sb->reorder) {
        if (right->is_lost) sb->lost_bytes += scoreboard_hole_bytes(right);
                                                 ...
                                                 u8                     *snd_limited)
{
    ...
    if (hole && seq_lt(sb->high_rxt, hole->start)) sb->high_rxt = hole->start;
...
void scoreboard_rxt_mark_lost(sack_scoreboard_t *sb, u32 snd_una, u32 snd_nxt)
{
    sack_scoreboard_hole_t *hole;
...
void scoreboard_clear_reneging(sack_scoreboard_t *sb, u32 start, u32 end)
{
    ...
    scoreboard_clear(sb);
    last_hole          = scoreboard_insert_hole(sb, TCP_INVALID_SACK_HOLE_INDEX, start, end);
    last_hole->is_lost = 1;
...
u8 tcp_scoreboard_is_sane_post_recovery(tcp_connection_t *tc)
{
    ...
    return (!hole || (seq_geq(hole->start, tc->snd_una) && seq_lt(hole->end, tc->snd_nxt)));
}
...
void tcp_rcv_sacks(tcp_connection_t *tc, u32 ack)
{
    ...
    if (!tcp_opts_sack(&tc->rcv_opts) && !sb->sacked_bytes && sb->head == TCP_INVALID_SACK_HOLE_INDEX) return;
    ...
    while (blk < vec_end(tc->rcv_opts.sacks)) {
        if (seq_lt(blk->start, blk->end) && seq_gt(blk->start, tc->snd_una) && seq_gt(blk->start, ack) && seq_lt(blk->start, tc->snd_nxt)
    ...
    if (vec_len(tc->rcv_opts.sacks) == 0) return;
    ...
    if (sb->head == TCP_INVALID_SACK_HOLE_INDEX) {
        ...
        if (PREDICT_FALSE(sb->is_reneging)) {
            ...
            if (seq_leq(tc->snd_nxt, sb->high_sacked)) {
                ...
                if (seq_leq(ack, tc->snd_una)) return;
            ...
            hole     = scoreboard_insert_hole(sb, TCP_INVALID_SACK_HOLE_INDEX, sb->high_sacked, tc->snd_nxt);
            sb->tail = scoreboard_hole_index(sb, hole);
        }
        /* Not reneging and no holes. Insert the first that covers all
         * outstanding bytes */
        else {
            hole            = scoreboard_insert_hole(sb, TCP_INVALID_SACK_HOLE_INDEX, tc->snd_una, tc->snd_nxt);
            sb->tail        = scoreboard_hole_index(sb, hole);
        ...
        if (seq_gt(tc->snd_nxt, hole->end)) {
            ...
            /* New hole after high sacked block */
                ...
                scoreboard_insert_hole(sb, sb->tail, sb->high_sacked, tc->snd_nxt);
            }
        }
        /* Keep track of max byte sacked for when the last hole
         * is acked */
        high_sacked = seq_max(rcv_sacks[vec_len(rcv_sacks) - 1].end, sb->high_sacked);
    }
    ...
    if (PREDICT_FALSE(sb->is_reneging)) {
        sb->last_bytes_delivered += clib_min(hole->start - tc->snd_una, ack - tc->snd_una);
    ...
    while (hole && blk_index < vec_len(rcv_sacks)) {
        ...
        if (seq_leq(blk->start, hole->start)) {
            ...
            if (seq_geq(blk->end, hole->end)) {
                ...
                scoreboard_update_sacked(sb, hole->start, hole->end, has_rxt, tc->snd_mss);
                scoreboard_remove_hole(sb, hole);
                hole = next_hole;
            }
            /* Partial 'head' overlap */
                ...
                if (seq_gt(blk->end, hole->start)) {
                    scoreboard_update_sacked(sb, hole->start, blk->end, has_rxt, tc->snd_mss);
                    hole->start = blk->end;
                }
                blk_index++;
            }
            ...
            if (seq_lt(blk->end, hole->end)) {
                ...
                scoreboard_update_sacked(sb, blk->start, blk->end, has_rxt, tc->snd_mss);

                blk_index++;
                ASSERT(hole->next == scoreboard_hole_index(sb, next_hole));
            }
            else if (seq_lt(blk->start, hole->end)) {
                scoreboard_update_sacked(sb, blk->start, hole->end, has_rxt, tc->snd_mss);
                hole->end = blk->start;
            }
            hole = scoreboard_next_hole(sb, hole);
        }
    ...
    ASSERT(sb->sacked_bytes == 0 || tcp_in_recovery(tc) || sb->sacked_bytes <= tc->snd_nxt - seq_max(tc->snd_una, ack));
    ASSERT(sb->last_sacked_bytes + sb->lost_bytes <= tc->snd_nxt - seq_max(tc->snd_una, ack) || tcp_in_recovery(tc));
    ASSERT(sb->head == TCP_INVALID_SACK_HOLE_INDEX || tcp_in_recovery(tc) || sb->is_reneging || sb->holes[sb->head].start == ack);
    ASSERT(sb->last_lost_bytes <= sb->lost_bytes);
    ASSERT((ack - tc->snd_una) + sb->last_sacked_bytes - sb->last_bytes_delivered >= sb->rxt_sacked);
    ASSERT((ack - tc->snd_una) >= tc->sack_sb.last_bytes_delivered || (tc->flags & TCP_CONN_FINSNT));
...
static u8 tcp_sack_vector_is_sane(sack_block_t *sacks)
{
    ...
    for (i = 1; i < vec_len(sacks); i++) {
        if (sacks[i - 1].end == sacks[i].start) return 0;
    }
    return 1;
}
...
void tcp_update_sack_list(tcp_connection_t *tc, u32 start, u32 end)
{
    ...
    for (i = 0; i < vec_len(tc->snd_sacks); i++) {
        ...
        if (seq_leq(tc->snd_sacks[i].start, tc->rcv_nxt)) continue;

        /* Merge or drop if segment overlapped by the new segment */
        if (block && (seq_geq(tc->snd_sacks[i].end, new_list[0].start) && seq_leq(tc->snd_sacks[i].start, new_list[0].end))) {
            if (seq_lt(tc->snd_sacks[i].start, new_list[0].start)) new_list[0].start = tc->snd_sacks[i].start;
            if (seq_lt(new_list[0].end, tc->snd_sacks[i].end)) new_list[0].end = tc->snd_sacks[i].end;
            continue;
        }

        /* Save to new SACK list if we have space. */
        if (vec_len(new_list) < TCP_MAX_SACK_BLOCKS) vec_add1(new_list, tc->snd_sacks[i]);
    }
