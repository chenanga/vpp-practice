...
static u8 *format_syn_filter4_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "SYN_FILTER4: next index %d, %s", t->next_index, t->not_a_syn ? "not a syn" : "syn");
    if (t->not_a_syn == 0)
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 8 && n_left_to_next >= 4) {
            ...
            vnet_get_config_data(&cm->config_main, &b1->current_config_index, &next1, 0 /* sizeof (c0[0]) */);
            vnet_get_config_data(&cm->config_main, &b2->current_config_index, &next2, 0 /* sizeof (c0[0]) */);
            vnet_get_config_data(&cm->config_main, &b3->current_config_index, &next3, 0 /* sizeof (c0[0]) */);

            /* Not TCP? */
            ip0 = vlib_buffer_get_current(b0);
            if (ip0->protocol != IP_PROTOCOL_TCP) goto trace00;
             ...
             * correct code.
             */
            if (PREDICT_TRUE(!(tcp0->flags & 0x2))) goto trace00;
        ...
        trace00:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                syn_filter4_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
            ...
            if (ip1->protocol != IP_PROTOCOL_TCP) goto trace01;
             ...
             * correct code.
             */
            if (PREDICT_TRUE(!(tcp1->flags & 0x2))) goto trace01;
        ...
        trace01:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b1->flags & VLIB_BUFFER_IS_TRACED))) {
                syn_filter4_trace_t *t = vlib_add_trace(vm, node, b1, sizeof(*t));
            ...
            if (ip2->protocol != IP_PROTOCOL_TCP) goto trace02;
             ...
             * correct code.
             */
            if (PREDICT_TRUE(!(tcp2->flags & 0x2))) goto trace02;
        ...
        trace02:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b2->flags & VLIB_BUFFER_IS_TRACED))) {
                syn_filter4_trace_t *t = vlib_add_trace(vm, node, b2, sizeof(*t));
            ...
            if (ip3->protocol != IP_PROTOCOL_TCP) goto trace03;
             ...
             * correct code.
             */
            if (PREDICT_TRUE(!(tcp3->flags & 0x2))) goto trace03;
        ...
        trace03:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b3->flags & VLIB_BUFFER_IS_TRACED))) {
                syn_filter4_trace_t *t = vlib_add_trace(vm, node, b3, sizeof(*t));
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            /* Not TCP? */
            ip0 = vlib_buffer_get_current(b0);
            if (ip0->protocol != IP_PROTOCOL_TCP) goto trace0;
             ...
             * correct code.
             */
            if (PREDICT_TRUE(!(tcp0->flags & 0x2))) goto trace0;
    ...
    vlib_node_increment_counter(vm, syn_filter4_node.index, SYN_FILTER_ERROR_OK, ok_syn_packets);
...
int syn_filter_enable_disable(u32 sw_if_index, int enable_disable)
{
    ...
    rv = vnet_feature_enable_disable("ip4-local", "syn-filter-4", sw_if_index, enable_disable, 0, 0);
