 ...
 */
always_inline u32 tcp_initial_wnd_unscaled(tcp_connection_t *tc)
{
...
u32 tcp_initial_window_to_advertise(tcp_connection_t *tc)
{
    ...
    if (tc->state != TCP_STATE_SYN_RCVD || tcp_opts_wscale(&tc->rcv_opts)) tc->rcv_wscale = tcp_window_compute_scale(tcp_cfg.max_rx_fifo);
...
static inline void tcp_update_rcv_wnd(tcp_connection_t *tc)
{
    ...
    if (PREDICT_FALSE(wnd < tc->rcv_opts.mss)) wnd = 0;
 ...
 */
static inline u32 tcp_window_to_advertise(tcp_connection_t *tc, tcp_state_t state)
{
    if (state < TCP_STATE_ESTABLISHED) return tcp_initial_window_to_advertise(tc);
...
static int tcp_make_established_options(tcp_connection_t *tc, tcp_options_t *opts)
{
    ...
    if (tcp_opts_sack_permitted(&tc->rcv_opts)) {
        if (vec_len(tc->snd_sacks)) {
            ...
            if (tc->snd_sack_pos >= vec_len(tc->snd_sacks)) tc->snd_sack_pos = 0;
            opts->sacks         = &tc->snd_sacks[tc->snd_sack_pos];
            opts->n_sack_blocks = vec_len(tc->snd_sacks) - tc->snd_sack_pos;
            opts->n_sack_blocks = clib_min(opts->n_sack_blocks, TCP_OPTS_MAX_SACK_BLOCKS);
...
always_inline int tcp_make_options(tcp_connection_t *tc, tcp_options_t *opts, tcp_state_t state)
{
    switch (state) {
...
void tcp_update_burst_snd_vars(tcp_connection_t *tc)
{
    ...
    tc->snd_opts_len = tcp_make_options(tc, &tc->snd_opts, TCP_STATE_ESTABLISHED);

    /* XXX check if MTU has been updated */
    tc->snd_mss = clib_min(tc->mss, tc->rcv_opts.mss) - tc->snd_opts_len;
    ASSERT(tc->snd_mss > 0);

    tcp_options_write(tm->wrk_ctx[tc->c_thread_index].cached_opts, &tc->snd_opts);

    tcp_update_rcv_wnd(tc);

    if (tc->cfg_flags & TCP_CFG_F_RATE_SAMPLE) tcp_bt_check_app_limited(tc);
...
u16 ip6_tcp_compute_checksum_custom(vlib_main_t *vm, vlib_buffer_t *p0, ip46_address_t *src, ip46_address_t *dst)
{
    ...
    sum0                           = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, p0)) + clib_host_to_net_u16(IP_PROTOCOL_TCP);
    payload_length_host_byte_order = vlib_buffer_length_in_chain(vm, p0);

    for (i = 0; i < ARRAY_LEN(src->ip6.as_uword); i++) {
        sum0 = ip_csum_with_carry(sum0, clib_mem_unaligned(&src->ip6.as_uword[i], uword));
        sum0 = ip_csum_with_carry(sum0, clib_mem_unaligned(&dst->ip6.as_uword[i], uword));
    }

    return ip_calculate_l4_checksum(vm, p0, sum0, payload_length_host_byte_order, NULL, 0, NULL);
}
...
u16 ip4_tcp_compute_checksum_custom(vlib_main_t *vm, vlib_buffer_t *p0, ip46_address_t *src, ip46_address_t *dst)
{
    ...
    payload_length_host_byte_order = vlib_buffer_length_in_chain(vm, p0);
    sum0                           = clib_host_to_net_u32(payload_length_host_byte_order + (IP_PROTOCOL_TCP << 16));
...
static inline u16 tcp_compute_checksum(tcp_connection_t *tc, vlib_buffer_t *b)
{
    u16 checksum = 0;
    if (PREDICT_FALSE(tc->cfg_flags & TCP_CFG_F_NO_CSUM_OFFLOAD)) {
        ...
        if (tc->c_is_ip4)
            checksum = ip4_tcp_compute_checksum_custom(vm, b, &tc->c_lcl_ip, &tc->c_rmt_ip);
        else
            checksum = ip6_tcp_compute_checksum_custom(vm, b, &tc->c_lcl_ip, &tc->c_rmt_ip);
    }
    else {
 ...
 * Prepare ACK
 */
static inline void tcp_make_ack_i(tcp_connection_t *tc, vlib_buffer_t *b, tcp_state_t state, u8 flags)
{
    ...
    th = vlib_buffer_push_tcp(b, tc->c_lcl_port, tc->c_rmt_port, tc->snd_nxt, tc->rcv_nxt, tcp_hdr_opts_len, flags, wnd);
 ...
 */
static inline void tcp_make_ack(tcp_connection_t *tc, vlib_buffer_t *b)
{
...
void tcp_make_syn(tcp_connection_t *tc, vlib_buffer_t *b)
{
    ...
    th = vlib_buffer_push_tcp(b, tc->c_lcl_port, tc->c_rmt_port, tc->iss, tc->rcv_nxt, tcp_hdr_opts_len, TCP_FLAG_SYN, initial_wnd);
    vnet_buffer(b)->tcp.connection_index = tc->c_c_index;
    tcp_options_write((u8 *) (th + 1), &snd_opts);
    th->checksum = tcp_compute_checksum(tc, b);
}
...
static void tcp_make_synack(tcp_connection_t *tc, vlib_buffer_t *b)
{
    ...
    th = vlib_buffer_push_tcp(b, tc->c_lcl_port, tc->c_rmt_port, tc->iss, tc->rcv_nxt, tcp_hdr_opts_len, TCP_FLAG_SYN | TCP_FLAG_ACK, initial_wnd);
    tcp_options_write((u8 *) (th + 1), snd_opts);
...
static void tcp_enqueue_half_open(tcp_worker_ctx_t *wrk, tcp_connection_t *tc, vlib_buffer_t *b, u32 bi)
{
    ...
    session_add_pending_tx_buffer(vm->thread_index, bi, wrk->tco_next_node[!tc->c_is_ip4]);

    if (vm->thread_index == 0 && vlib_num_workers()) session_queue_run_on_main_thread(vm);
}
...
static void tcp_enqueue_to_output(tcp_worker_ctx_t *wrk, vlib_buffer_t *b, u32 bi, u8 is_ip4)
{
    ...
    session_add_pending_tx_buffer(wrk->vm->thread_index, bi, wrk->tco_next_node[!is_ip4]);
}
...
int tcp_buffer_make_reset(vlib_main_t *vm, vlib_buffer_t *b, u8 is_ip4)
{
     ...
     */
    if (b->flags & VLIB_BUFFER_NEXT_PRESENT) vlib_buffer_free_one(vm, b->next_buffer);
     ...
     */
    th = vlib_buffer_push_tcp_net_order(b, dst_port, src_port, seq, ack, sizeof(tcp_header_t), flags, 0);

    if (is_ip4) {
        ih4          = vlib_buffer_push_ip4(vm, b, &dst_ip4, &src_ip4, IP_PROTOCOL_TCP, 1);
...
void tcp_send_reset_w_pkt(tcp_connection_t *tc, vlib_buffer_t *pkt, u32 thread_index, u8 is_ip4)
{
    ...
    th           = vlib_buffer_push_tcp_net_order(b, pkt_th->dst_port, pkt_th->src_port, seq, ack, tcp_hdr_len, flags, 0);
    th->checksum = tcp_compute_checksum(tc, b);

    tcp_enqueue_half_open(wrk, tc, b, bi);
    TCP_EVT(TCP_EVT_RST_SENT, tc);
    vlib_node_increment_counter(vm, tcp_node_index(output, tc->c_is_ip4), TCP_ERROR_RST_SENT, 1);
}
...
void tcp_send_reset(tcp_connection_t *tc)
{
    ...
    flags            = TCP_FLAG_RST | TCP_FLAG_ACK;
    th               = vlib_buffer_push_tcp(b, tc->c_lcl_port, tc->c_rmt_port, tc->snd_nxt, tc->rcv_nxt, tcp_hdr_opts_len, flags, advertise_wnd);
...
void tcp_send_syn(tcp_connection_t *tc)
{
     ...
     */
    tcp_timer_update(&wrk->timer_wheel, tc, TCP_TIMER_RETRANSMIT_SYN, (u32) tc->rto * TCP_TO_TIMER_TICK);

    if (PREDICT_FALSE(!vlib_buffer_alloc(vm, &bi, 1))) {
        tcp_timer_update(&wrk->timer_wheel, tc, TCP_TIMER_RETRANSMIT_SYN, tcp_cfg.alloc_err_timeout);
        tcp_worker_stats_inc(wrk, no_buffer, 1);
        return;
    }
...
void tcp_send_synack(tcp_connection_t *tc)
{
    ...
    if (PREDICT_FALSE(!vlib_buffer_alloc(vm, &bi, 1))) {
        tcp_timer_update(&wrk->timer_wheel, tc, TCP_TIMER_RETRANSMIT, tcp_cfg.alloc_err_timeout);
        tcp_worker_stats_inc(wrk, no_buffer, 1);
        return;
    }
...
void tcp_send_fin(tcp_connection_t *tc)
{
    ...
    if ((tc->flags & TCP_CONN_SNDACK) && !tc->pending_dupacks) tc->flags &= ~TCP_CONN_SNDACK;
 ...
 */
always_inline void tcp_push_hdr_i(tcp_connection_t *tc, vlib_buffer_t *b, u32 snd_nxt, u8 compute_opts, u8 maybe_burst, u8 update_snd_nxt)
{
    ...
    if (PREDICT_FALSE(b->flags & VLIB_BUFFER_NEXT_PRESENT)) data_len += b->total_length_not_including_first_buffer;

    vnet_buffer(b)->tcp.flags            = 0;
    vnet_buffer(b)->tcp.connection_index = tc->c_c_index;

    if (compute_opts) tc->snd_opts_len = tcp_make_options(tc, &tc->snd_opts, tc->state);
    ...
    if (PREDICT_FALSE(tc->flags & TCP_CONN_PSH_PENDING)) {
        if (seq_geq(tc->psh_seq, snd_nxt) && seq_lt(tc->psh_seq, snd_nxt + data_len)) flags |= TCP_FLAG_PSH;
    }
    th = vlib_buffer_push_tcp(b, tc->c_lcl_port, tc->c_rmt_port, snd_nxt, tc->rcv_nxt, tcp_hdr_opts_len, flags, advertise_wnd);

    if (maybe_burst) {
        clib_memcpy_fast((u8 *) (th + 1), tm->wrk_ctx[tc->c_thread_index].cached_opts, tc->snd_opts_len);
    }
    else {
    ...
    if (update_snd_nxt) tc->snd_nxt += data_len;
    tc->rcv_las = tc->rcv_nxt;
...
always_inline u32 tcp_buffer_len(vlib_buffer_t *b)
{
...
always_inline u32 tcp_push_one_header(tcp_connection_t *tc, vlib_buffer_t *b)
{
    if (tc->cfg_flags & TCP_CFG_F_RATE_SAMPLE) tcp_bt_track_tx(tc, tcp_buffer_len(b));
...
u32 tcp_session_push_header(transport_connection_t *tconn, vlib_buffer_t **bs, u32 n_bufs)
{
    ...
    while (n_bufs) {
        if (n_bufs > 1) vlib_prefetch_buffer_header(bs[1], STORE);
...
void tcp_program_dupack(tcp_connection_t *tc)
{
    ...
    if (tc->pending_dupacks < 255) tc->pending_dupacks += 1;
}
...
static int tcp_prepare_segment(tcp_worker_ctx_t *wrk, tcp_connection_t *tc, u32 offset, u32 max_deq_bytes, vlib_buffer_t **b)
{
    ...
    /* Split mss into multiple buffers */
        ...
        vec_validate_aligned(wrk->tx_buffers, n_bufs_per_seg - 1, CLIB_CACHE_LINE_BYTES);
        n_bufs = vlib_buffer_alloc(vm, wrk->tx_buffers, n_bufs_per_seg);
        if (PREDICT_FALSE(n_bufs != n_bufs_per_seg)) {
            if (n_bufs) vlib_buffer_free(vm, wrk->tx_buffers, n_bufs);
            tcp_worker_stats_inc(wrk, no_buffer, 1);
            return 0;
        }
        ...
        *b                   = vlib_get_buffer(vm, wrk->tx_buffers[--n_bufs]);
        data                 = tcp_init_buffer(vm, *b);
        n_bytes              = session_tx_fifo_peek_bytes(&tc->connection, data, offset, bytes_per_buffer - TRANSPORT_MAX_HDRS_LEN);
        b[0]->current_length = n_bytes;
        ...
        for (i = 1; i < n_bufs_per_seg; i++) {
            ...
            n_peeked              = session_tx_fifo_peek_bytes(&tc->connection, data, offset + n_bytes, len_to_deq);
...
static u32 tcp_prepare_retransmit_segment(tcp_worker_ctx_t *wrk, tcp_connection_t *tc, u32 offset, u32 max_deq_bytes, vlib_buffer_t **b)
{
    ...
    if (tc->cfg_flags & TCP_CFG_F_RATE_SAMPLE) tcp_bt_track_rxt(tc, start, start + n_bytes);
...
static void tcp_check_sack_reneging(tcp_connection_t *tc)
{
    ...
    if (!sb->is_reneging && (!hole || hole->start == tc->snd_una)) return;
...
static void tcp_cc_init_rxt_timeout(tcp_connection_t *tc)
{
    ...
    if (!tcp_in_fastrecovery(tc)) tcp_cc_congestion(tc);
...
void tcp_timer_retransmit_handler(tcp_connection_t *tc)
{
    ...
    if (PREDICT_FALSE(tc->state == TCP_STATE_SYN_SENT)) return;

    /* Wait-close and retransmit could pop at the same time */
    if (tc->state == TCP_STATE_CLOSED) return;
    ...
    if (tc->state >= TCP_STATE_ESTABLISHED) {
        ...
        if (tcp_opts_sack_permitted(&tc->rcv_opts)) scoreboard_init_rxt(&tc->sack_sb, tc->snd_una + n_bytes);
    ...
    /* Retransmit SYN-ACK */
        ...
        tc->rto_boff += 1;
        if (tc->rto_boff > TCP_RTO_SYN_RETRIES) tc->rto = clib_min(tc->rto << 1, TCP_RTO_MAX);
...
void tcp_timer_retransmit_syn_handler(tcp_connection_t *tc)
{
    ...
    if (tc->flags & TCP_CONN_HALF_OPEN_DONE) {
        if (tcp_half_open_connection_cleanup(tc)) TCP_DBG("could not remove half-open connection");
        return;
    }
...
void tcp_timer_persist_handler(tcp_connection_t *tc)
{
    ...
    if (tc->state == TCP_STATE_CLOSED || tc->snd_wnd > tc->snd_mss || (tc->flags & TCP_CONN_FINSNT)) goto update_scheduler;
    ...
    if (available_bytes <= offset) goto update_scheduler;
    ...
    tc->snd_opts_len = tcp_make_options(tc, &tc->snd_opts, tc->state);
    max_snd_bytes    = clib_min(clib_min(tc->snd_mss, available_bytes), tm->bytes_per_buffer - TRANSPORT_MAX_HDRS_LEN);
    if (tc->snd_wnd > 0) max_snd_bytes = clib_min(tc->snd_wnd, max_snd_bytes);
    n_bytes           = session_tx_fifo_peek_bytes(&tc->connection, data, offset, max_snd_bytes);
    b->current_length = n_bytes;
    ASSERT(n_bytes != 0 && (tcp_timer_is_active(tc, TCP_TIMER_RETRANSMIT) || tc->snd_una == tc->snd_nxt || tc->rto_boff > 1));
...
update_scheduler:
    ...
    if (tcp_is_descheduled(tc)) transport_connection_reschedule(&tc->connection);
}
...
static int tcp_transmit_unsent(tcp_worker_ctx_t *wrk, tcp_connection_t *tc, u32 burst_size)
{
    ...
    while (n_segs < burst_size) {
        ...
        if (tc->cfg_flags & TCP_CFG_F_RATE_SAMPLE) tcp_bt_track_tx(tc, n_written);
...
int tcp_fastrecovery_prr_snd_space(tcp_connection_t *tc)
{
    ...
    if (pipe > tc->ssthresh) {
        space = ((int) tc->prr_delivered * ((f64) tc->ssthresh / tc->prev_cwnd)) - prr_out;
    }
    else {
        int limit;
...
static inline u8 tcp_retransmit_should_retry_head(tcp_connection_t *tc, sack_scoreboard_t *sb)
{
    u32 tx_adv_sack = sb->high_sacked - tc->snd_congestion;
    f64 rr          = (f64) tc->ssthresh / tc->prev_cwnd;

    if (tcp_fastrecovery_first(tc)) return 1;

    return (tx_adv_sack > (tc->snd_una - tc->prr_start) * rr);
}

static inline u8 tcp_max_tx_deq(tcp_connection_t *tc)
{
    return (transport_max_tx_dequeue(&tc->connection) - (tc->snd_nxt - tc->snd_una));
}

#define scoreboard_rescue_rxt_valid(_sb, _tc) (seq_geq(_sb->rescue_rxt, _tc->snd_una) && seq_leq(_sb->rescue_rxt, _tc->snd_congestion))
...
static int tcp_retransmit_sack(tcp_worker_ctx_t *wrk, tcp_connection_t *tc, u32 burst_size)
{
    ...
    if (snd_space < tc->snd_mss) goto done;

    sb = &tc->sack_sb;

    /* Check if snd_una is a lost retransmit */
    if (pool_elts(sb->holes) && seq_gt(sb->high_sacked, tc->snd_congestion) && tc->rxt_head != tc->snd_una
    ...
    while (snd_space > 0 && n_segs < burst_size) {
        hole = scoreboard_next_rxt_hole(sb, hole, max_deq != 0, &can_rescue, &snd_limited);
        if (!hole) {
            ...
            if (max_deq) {
                ...
                snd_space  = clib_min(clib_max(max_deq, tc->snd_mss), snd_space);
                ...
                if (max_deq > n_segs_new * tc->snd_mss) tcp_program_retransmit(tc);

                n_segs += n_segs_new;
                goto done;
            }

            if (!can_rescue || scoreboard_rescue_rxt_valid(sb, tc)) break;
        ...
        if (snd_space < tc->snd_mss && max_bytes > snd_space) break;
        max_bytes = clib_min(max_bytes, snd_space);
    ...
    if (hole) tcp_program_retransmit(tc);
...
static int tcp_retransmit_no_sack(tcp_worker_ctx_t *wrk, tcp_connection_t *tc, u32 burst_size)
{
    ...
    if (!tcp_fastrecovery_first(tc)) goto send_unsent;

    /* RFC 6582: [If a partial ack], retransmit the first unacknowledged
     * segment. */
    while (snd_space > 0 && n_segs < burst_size) {
        max_bytes = clib_min(tc->snd_mss, tc->snd_congestion - tc->snd_una - offset);
        if (!max_bytes) break;
        n_written = tcp_prepare_retransmit_segment(wrk, tc, offset, max_bytes, &b);
    ...
    if (n_segs == burst_size) goto done;

send_unsent:

    /* RFC 6582: Send a new segment if permitted by the new value of cwnd. */
    if (snd_space < tc->snd_mss || tc->snd_mss == 0) goto done;
    ...
    if (max_deq) {
        ...
        if (n_segs_now && max_deq > n_segs_now * tc->snd_mss) tcp_program_retransmit(tc);
        n_segs += n_segs_now;
    }
...
static int tcp_send_acks(tcp_connection_t *tc, u32 max_burst_size)
{
    ...
    if (!tc->pending_dupacks) {
        if (tcp_in_cong_recovery(tc) || !tcp_max_tx_deq(tc) || tc->state != TCP_STATE_ESTABLISHED) {
            tcp_send_ack(tc);
            return 1;
        }
        return 0;
...
static int tcp_do_retransmit(tcp_connection_t *tc, u32 max_burst_size)
{
    ...
    if (PREDICT_FALSE(tc->state == TCP_STATE_CLOSED)) return 0;
...
int tcp_session_custom_tx(void *conn, transport_send_params_t *sp)
{
    ...
    if (!(tc->flags & TCP_CONN_SNDACK)) return n_segs;

    tc->flags &= ~TCP_CONN_SNDACK;

    /* We have retransmitted packets and no dupack */
    if (n_segs && !tc->pending_dupacks) return n_segs;
...
static void tcp_output_handle_link_local(tcp_connection_t *tc0, vlib_buffer_t *b0, u16 *next0, u32 *error0)
{
     ...
     * not be removed */
    ai = adj_nbr_find(FIB_PROTOCOL_IP6, VNET_LINK_IP6, &tc0->c_rmt_ip, tc0->sw_if_index);
...
static void tcp46_output_trace_frame(vlib_main_t *vm, vlib_node_runtime_t *node, u32 *to_next, u32 n_bufs)
{
    ...
    for (i = 0; i < n_bufs; i++) {
        ...
        tc = tcp_connection_get(vnet_buffer(b)->tcp.connection_index, vm->thread_index);
...
always_inline void tcp_output_push_ip(vlib_main_t *vm, vlib_buffer_t *b0, tcp_connection_t *tc0, u8 is_ip4)
{
    TCP_EVT(TCP_EVT_OUTPUT, tc0, ((tcp_header_t *) vlib_buffer_get_current(b0))->flags, b0->current_length);

    if (is_ip4)
        vlib_buffer_push_ip4(vm, b0, &tc0->c_lcl_ip4, &tc0->c_rmt_ip4, IP_PROTOCOL_TCP, tcp_csum_offload(tc0));
    else
        vlib_buffer_push_ip6_custom(vm, b0, &tc0->c_lcl_ip6, &tc0->c_rmt_ip6, IP_PROTOCOL_TCP, tc0->ipv6_flow_label);
}

always_inline void tcp_check_if_gso(tcp_connection_t *tc, vlib_buffer_t *b)
{
    if (PREDICT_TRUE(!(tc->cfg_flags & TCP_CFG_F_TSO))) return;

    u16 data_len = b->current_length - sizeof(tcp_header_t) - tc->snd_opts_len;

    if (PREDICT_FALSE(b->flags & VLIB_BUFFER_TOTAL_LENGTH_VALID)) data_len += b->total_length_not_including_first_buffer;
...
always_inline void tcp_output_handle_packet(tcp_connection_t *tc0, vlib_buffer_t *b0, vlib_node_runtime_t *error_node, u16 *next0, u8 is_ip4)
{
    ...
    if (!is_ip4) {
        ...
        if (PREDICT_FALSE(ip6_address_is_link_local_unicast(&tc0->c_rmt_ip6))) tcp_output_handle_link_local(tc0, b0, next0, &error0);
...
always_inline uword tcp46_output_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ip4)
{
    ...
    if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE)) tcp46_output_trace_frame(vm, node, from, n_left_from);
    ...
    while (n_left_from >= 4) {
        ...
        tc0 = tcp_connection_get(vnet_buffer(b[0])->tcp.connection_index, thread_index);
        tc1 = tcp_connection_get(vnet_buffer(b[1])->tcp.connection_index, thread_index);
...
static void tcp_reset_trace_frame(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t **bs, u32 n_bufs, u8 is_ip4)
{
    tcp_header_t   *tcp;
    tcp_tx_trace_t *t;
    int             i;
...
static uword tcp46_reset_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_ip4)
{
    vlib_buffer_t *bufs[VLIB_FRAME_SIZE], **b;
    u16            nexts[VLIB_FRAME_SIZE], *next;
    ...
    while (n_left_from > 0) {
        ...
        vnet_buffer(b[0])->sw_if_index[VLIB_TX] = vec_elt(ip4_main.fib_index_by_sw_if_index, vnet_buffer(b[0])->sw_if_index[VLIB_RX]);
    ...
    if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE)) tcp_reset_trace_frame(vm, node, bufs, frame->n_vectors, is_ip4);
...
VLIB_NODE_FN(tcp4_reset_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return tcp46_reset_inline(vm, node, from_frame, 1);
}

VLIB_NODE_FN(tcp6_reset_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
