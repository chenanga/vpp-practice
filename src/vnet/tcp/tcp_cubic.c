...
static inline f64 cubic_time(u32 thread_index)
{
 ...
 */
static inline u64 W_cubic(cubic_data_t *cd, f64 t)
{
    f64 diff = t - cd->K;
 ...
 * RFC 8312 Eq. 2
 */
static inline f64 K_cubic(cubic_data_t *cd, u32 wnd)
{
 ...
 */
static inline u32 W_est(cubic_data_t *cd, f64 t, f64 rtt)
{
...
static void cubic_congestion(tcp_connection_t *tc)
{
    ...
    if (cubic_cfg.fast_convergence && w_max < cd->w_max) w_max = w_max * ((1.0 + beta_cubic) / 2.0);
...
static void cubic_rcv_ack(tcp_connection_t *tc, tcp_rate_sample_t *rs)
{
    ...
    if (tc->cwnd >= tc->tx_fifo_size) return;
...
void cubic_event(tcp_connection_t *tc, tcp_cc_event_t evt)
{
    cubic_data_t *cd;
    f64           now;

    if (evt != TCP_CC_EVT_START_TX) return;
    ...
    if (now > tc->mrtt_us + 1) cd->t_start = now;
}
