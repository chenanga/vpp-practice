 ...
 *
 */
always_inline u8 tcp_segment_in_rcv_wnd(tcp_connection_t *tc, u32 seq, u32 end_seq)
{
    return (seq_geq(end_seq, tc->rcv_las) && seq_leq(seq, tc->rcv_nxt + tc->rcv_wnd));
}
 ...
 */
always_inline int tcp_segment_check_paws(tcp_connection_t *tc)
{
    return tcp_opts_tstamp(&tc->rcv_opts) && timestamp_lt(tc->rcv_opts.tsval, tc->tsval_recent);
}

/**
 * Update tsval recent
 */
always_inline void tcp_update_timestamp(tcp_connection_t *tc, u32 seq, u32 seq_end)
{
    /*
     ...
     */
    if (tcp_opts_tstamp(&tc->rcv_opts) && seq_leq(seq, tc->rcv_las) && seq_leq(tc->rcv_las, seq_end)) {
...
static int tcp_segment_validate(tcp_worker_ctx_t *wrk, tcp_connection_t *tc0, vlib_buffer_t *b0, tcp_header_t *th0, u32 *error0)
{
    ...
    if (PREDICT_FALSE(tcp_segment_check_paws(tc0))) {
        *error0 = TCP_ERROR_PAWS;
        TCP_EVT(TCP_EVT_PAWS_FAIL, tc0, vnet_buffer(b0)->tcp.seq_number, vnet_buffer(b0)->tcp.seq_end);

        /* If it just so happens that a segment updates tsval_recent for a
         * segment over 24 days old, invalidate tsval_recent. */
        if (timestamp_lt(tc0->tsval_recent_age + TCP_PAWS_IDLE, tcp_time_tstamp(tc0->c_thread_index))) {
    ...
    if (!tcp_segment_in_rcv_wnd(tc0, vnet_buffer(b0)->tcp.seq_number, vnet_buffer(b0)->tcp.seq_end)) {
        /* SYN/SYN-ACK retransmit */
        if (tcp_syn(th0) && vnet_buffer(b0)->tcp.seq_number == tc0->rcv_nxt - 1) {
        ...
        if (tc0->rcv_wnd < tc0->snd_mss && tc0->rcv_nxt == vnet_buffer(b0)->tcp.seq_number) goto check_reset;
        ...
        if (tcp_in_recovery(tc0) && seq_geq(vnet_buffer(b0)->tcp.seq_number, tc0->rcv_las - tc0->rcv_wnd)
            && seq_leq(vnet_buffer(b0)->tcp.seq_end, tc0->rcv_nxt + tc0->rcv_wnd))
            goto check_reset;
        ...
        *error0 = TCP_ERROR_RCV_WND;
        ...
        if ((tc0->flags & TCP_CONN_ZERO_RWND_SENT) && seq_lt(vnet_buffer(b0)->tcp.seq_end, tc0->rcv_las + tc0->rcv_opts.mss))
            *error0 = TCP_ERROR_ZERO_RWND;

        tc0->errors.below_data_wnd += seq_lt(vnet_buffer(b0)->tcp.seq_end, tc0->rcv_las);
    ...
    tcp_update_timestamp(tc0, vnet_buffer(b0)->tcp.seq_number, vnet_buffer(b0)->tcp.seq_end);
    return 0;
...
always_inline int tcp_rcv_ack_no_cc(tcp_connection_t *tc, vlib_buffer_t *b, u32 *error)
{
    /* SND.UNA =< SEG.ACK =< SND.NXT */
    if (!(seq_leq(tc->snd_una, vnet_buffer(b)->tcp.ack_number) && seq_leq(vnet_buffer(b)->tcp.ack_number, tc->snd_nxt))) {
...
static inline void tcp_estimate_rtt_us(tcp_connection_t *tc, f64 mrtt)
{
...
static int tcp_update_rtt(tcp_connection_t *tc, tcp_rate_sample_t *rs, u32 ack)
{
    ...
    if (tcp_in_cong_recovery(tc)) {
        ...
        if (!(tc->cfg_flags & TCP_CFG_F_RATE_SAMPLE) || (rs->flags & TCP_BTS_IS_RXT)) goto done;
        if (rs->rtt_time) tcp_estimate_rtt_us(tc, rs->rtt_time);
...
estimate_rtt:
    ...
    if (mrtt == 0 || mrtt > TCP_RTT_MAX) goto done;
...
static void tcp_estimate_initial_rtt(tcp_connection_t *tc)
{
    ...
    if (tc->rtt_ts) {
        ...
        if (tc->rto_boff && mrtt > 1 * THZ) mrtt = 1 * THZ;
...
static void tcp_handle_postponed_dequeues(tcp_worker_ctx_t *wrk)
{
    ...
    if (!vec_len(wrk->pending_deq_acked)) return;
    ...
    if (seq_lt(tc->snd_wl1, seq) || (tc->snd_wl1 == seq && seq_leq(tc->snd_wl2, ack))) {
        ...
        if (PREDICT_FALSE(tc->snd_wnd < tc->snd_mss)) {
            if (!tcp_timer_is_active(tc, TCP_TIMER_RETRANSMIT)) {
                 ...
                 * acked. */
                if (tcp_timer_is_active(tc, TCP_TIMER_PERSIST)) tcp_persist_timer_reset(&wrk->timer_wheel, tc);
            ...
            if (PREDICT_FALSE(tcp_is_descheduled(tc))) tcp_reschedule(tc);
...
static void tcp_cc_init_congestion(tcp_connection_t *tc)
{
    ...
    if (!tcp_opts_sack_permitted(&tc->rcv_opts)) tc->cwnd += TCP_DUPACK_THRESHOLD * tc->snd_mss;
...
static inline u8 tcp_cc_is_spurious_timeout_rxt(tcp_connection_t *tc)
{
    return (tcp_in_recovery(tc) && tc->rto_boff == 1 && tc->snd_rxt_ts && tcp_opts_tstamp(&tc->rcv_opts)
            && timestamp_lt(tc->rcv_opts.tsecr, tc->snd_rxt_ts));
}

static inline u8 tcp_cc_is_spurious_retransmit(tcp_connection_t *tc)
{
    return (tcp_cc_is_spurious_timeout_rxt(tc));
}

static inline u8 tcp_should_fastrecover(tcp_connection_t *tc, u8 has_sack)
{
    if (!has_sack) {
        ...
        if (seq_leq(tc->snd_una, tc->snd_congestion)
            && ((!(tc->cwnd > tc->snd_mss && tc->bytes_acked <= 4 * tc->snd_mss)) || (tc->rcv_opts.tsecr != tc->tsecr_last_ack))) {
            tc->rcv_dupacks = 0;
            return 0;
        }
    }
...
static int tcp_cc_try_recover(tcp_connection_t *tc)
{
    ...
    if (tcp_in_fastrecovery(tc) && !is_spurious) tcp_cc_recovered(tc);
...
static void tcp_cc_handle_event(tcp_connection_t *tc, tcp_rate_sample_t *rs, u32 is_dack)
{
    ...
    if (PREDICT_FALSE(tcp_is_lost_fin(tc) || tc->sack_sb.is_reneging)) return;
    ...
    if (!tcp_in_cong_recovery(tc)) {
        ...
        if (tcp_should_fastrecover(tc, has_sack)) {
            ...
            if (has_sack) scoreboard_init_rxt(&tc->sack_sb, tc->snd_una);
     ...
     */
    if (has_sack) {
        if (!tc->bytes_acked && tc->sack_sb.rxt_sacked) tcp_fastrecovery_first_on(tc);
        ...
        if (is_dack)
            ...
            tc->prr_delivered += tc->bytes_acked - clib_min(tc->bytes_acked, tc->snd_mss * tc->rcv_dupacks);

        /* If partial ack, assume that the first un-acked segment was lost */
        if (tc->bytes_acked || tc->rcv_dupacks == TCP_DUPACK_THRESHOLD) tcp_fastrecovery_first_on(tc);
    }
...
{
    if (!tcp_in_cong_recovery(tc)) return;

    if (tcp_opts_sack_permitted(&tc->rcv_opts)) tcp_rcv_sacks(tc, tc->snd_una);

    tc->bytes_acked = 0;

    if (tc->cfg_flags & TCP_CFG_F_RATE_SAMPLE) tcp_bt_sample_delivery_rate(tc, rs);
 ...
 */
always_inline u8 tcp_ack_is_dupack(tcp_connection_t *tc, vlib_buffer_t *b, u32 prev_snd_wnd, u32 prev_snd_una)
{
    return ((vnet_buffer(b)->tcp.ack_number == prev_snd_una) && seq_gt(tc->snd_nxt, tc->snd_una)
            && (vnet_buffer(b)->tcp.seq_end == vnet_buffer(b)->tcp.seq_number) && (prev_snd_wnd == tc->snd_wnd));
}
...
static u8 tcp_ack_is_cc_event(tcp_connection_t *tc, vlib_buffer_t *b, u32 prev_snd_wnd, u32 prev_snd_una, u8 *is_dack)
{
    ...
    *is_dack = tc->sack_sb.last_sacked_bytes || tcp_ack_is_dupack(tc, b, prev_snd_wnd, prev_snd_una);
...
static int tcp_rcv_ack(tcp_worker_ctx_t *wrk, tcp_connection_t *tc, vlib_buffer_t *b, tcp_header_t *th, u32 *error)
{
    ...
    if (PREDICT_FALSE(seq_lt(vnet_buffer(b)->tcp.ack_number, tc->snd_una))) {
        ...
        *error = TCP_ERROR_ACK_OLD;
        ...
        if (seq_lt(vnet_buffer(b)->tcp.ack_number, tc->snd_una - tc->rcv_wnd)) return -1;
    ...
    if (tcp_opts_sack_permitted(&tc->rcv_opts)) tcp_rcv_sacks(tc, vnet_buffer(b)->tcp.ack_number);

    prev_snd_wnd = tc->snd_wnd;
    prev_snd_una = tc->snd_una;
    tcp_update_snd_wnd(tc, vnet_buffer(b)->tcp.seq_number, vnet_buffer(b)->tcp.ack_number, clib_net_to_host_u16(th->window) << tc->snd_wscale);
    tc->bytes_acked = vnet_buffer(b)->tcp.ack_number - tc->snd_una;
    tc->snd_una     = vnet_buffer(b)->tcp.ack_number;
    ...
    if (tc->cfg_flags & TCP_CFG_F_RATE_SAMPLE)
        ...
        rs.delivered = tc->bytes_acked + tc->sack_sb.last_sacked_bytes - tc->sack_sb.last_bytes_delivered;

    if (tc->bytes_acked + tc->sack_sb.last_sacked_bytes) {
        tcp_update_rtt(tc, &rs, vnet_buffer(b)->tcp.ack_number);
        if (tc->bytes_acked) tcp_program_dequeue(wrk, tc);
    }
    ...
    if (tcp_ack_is_cc_event(tc, b, prev_snd_wnd, prev_snd_una, &is_dack)) {
        ...
        *error = TCP_ERROR_ACK_DUP;
        if (vnet_buffer(b)->tcp.data_len || tcp_is_fin(th)) return 0;
        return -1;
    }
...
static void tcp_rcv_fin(tcp_worker_ctx_t *wrk, tcp_connection_t *tc, vlib_buffer_t *b, u32 *error)
{
    ...
    if (vnet_buffer(b)->tcp.seq_end != tc->rcv_nxt) return;
    ...
    tcp_timer_update(&wrk->timer_wheel, tc, TCP_TIMER_WAITCLOSE, tcp_cfg.closewait_time);
...
static int tcp_session_enqueue_data(tcp_connection_t *tc, vlib_buffer_t *b, u16 data_len)
{
    ...
    /* If more data written than expected, account for out-of-order bytes. */
        ...
        if (tc->rcv_wnd < tc->snd_mss) return TCP_ERROR_ZERO_RWND;
...
static int tcp_session_enqueue_ooo(tcp_connection_t *tc, vlib_buffer_t *b, u16 data_len)
{
    ...
    rv = session_enqueue_stream_connection(&tc->connection, b, vnet_buffer(b)->tcp.seq_number - tc->rcv_nxt, 0 /* queue event */, 0);
...
static int tcp_buffer_discard_bytes(vlib_buffer_t *b, u32 n_bytes_to_drop)
{
    ...
    if (n_bytes_to_drop > b->current_length) {
        if (!(b->flags & VLIB_BUFFER_NEXT_PRESENT)) return -1;
        do {
        ...
        if (n_bytes_to_drop > first) b->total_length_not_including_first_buffer -= n_bytes_to_drop - first;
    }
    else
...
static int tcp_segment_rcv(tcp_worker_ctx_t *wrk, tcp_connection_t *tc, vlib_buffer_t *b)
{
    ...
    if (PREDICT_FALSE(b->current_length > n_data_bytes)) b->current_length = n_data_bytes;
    ...
    if (PREDICT_FALSE(vnet_buffer(b)->tcp.seq_number != tc->rcv_nxt)) {
        ...
        tc->errors.above_data_wnd += seq_gt(vnet_buffer(b)->tcp.seq_end, tc->rcv_las + tc->rcv_wnd);
        goto done;
    }
...
static u8 *format_tcp_rx_trace(u8 *s, va_list *args)
{
    ...
    if (!tc->c_lcl_port)
        s = format(s, "no tcp connection\n%U%U", format_white_space, indent, format_tcp_header, &t->tcp_header, 128);
    else
        s = format(s,
                   "%U state %U\n%U%U",
...
static void tcp_established_trace_frame(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_ip4)
{
    ...
    while (n_left >= 1) {
        ...
        if (b0->flags & VLIB_BUFFER_IS_TRACED) {
            ...
            tc0 = tcp_connection_get(vnet_buffer(b0)->tcp.connection_index, vm->thread_index);
            th0 = tcp_buffer_hdr(b0);
...
always_inline uword tcp46_established_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ip4)
{
    ...
    if (node->flags & VLIB_NODE_FLAG_TRACE) tcp_established_trace_frame(vm, node, frame, is_ip4);
    ...
    while (n_left_from > 0) {
        ...
        tc = tcp_connection_get(vnet_buffer(b[0])->tcp.connection_index, thread_index);
        ...
        if (PREDICT_FALSE(tcp_rcv_ack(wrk, tc, b[0], th, &error))) goto done;

        /* 6: check the URG bit TODO */

        /* 7: process the segment text */
        if (vnet_buffer(b[0])->tcp.data_len) error = tcp_segment_rcv(wrk, tc, b[0]);

        /* 8: check the FIN bit */
        if (PREDICT_FALSE(tcp_is_fin(th))) tcp_rcv_fin(wrk, tc, b[0], &error);
...
static u8 tcp_lookup_is_valid(tcp_connection_t *tc, vlib_buffer_t *b, tcp_header_t *hdr)
{
    ...
    /* Proxy case */
    if (tc->c_lcl_port == 0 && tc->state == TCP_STATE_LISTEN) return 1;
    ...
    if (tc->connection.is_ip4) {
        ...
        val_l       = !ip4_address_compare(&ip4_hdr->dst_address, &tc->connection.lcl_ip.ip4);
        val_l       = val_l || ip_is_zero(&tc->connection.lcl_ip, 1);
        val_r       = !ip4_address_compare(&ip4_hdr->src_address, &tc->connection.rmt_ip.ip4);
        ...
        val_l       = !ip6_address_compare(&ip6_hdr->dst_address, &tc->connection.lcl_ip.ip6);
        val_l       = val_l || ip_is_zero(&tc->connection.lcl_ip, 0);
        val_r       = !ip6_address_compare(&ip6_hdr->src_address, &tc->connection.rmt_ip.ip6);
        val_r       = val_r || tc->state == TCP_STATE_LISTEN;
        is_ip_valid = val_l && val_r;
    }

    u8 is_valid = (tc->c_lcl_port == hdr->dst_port && (tc->state == TCP_STATE_LISTEN || tc->c_rmt_port == hdr->src_port) && is_ip_valid);

    if (!is_valid) {
        handle = session_lookup_half_open_handle(&tc->connection);
        tmp    = session_lookup_half_open_connection(handle & 0xFFFFFFFF, tc->c_proto, tc->c_is_ip4);
...
static tcp_connection_t *tcp_lookup_listener(vlib_buffer_t *b, u32 fib_index, int is_ip4)
{
    ...
    if (is_ip4) {
        ...
        s                 = session_lookup_listener4(fib_index, &ip4->dst_address, tcp->dst_port, TRANSPORT_PROTO_TCP, 1);
    }
    else {
        ip6_header_t *ip6 = vlib_buffer_get_current(b);
        tcp_header_t *tcp = tcp_buffer_hdr(b);
        s                 = session_lookup_listener6(fib_index, &ip6->dst_address, tcp->dst_port, TRANSPORT_PROTO_TCP, 1);
    }
    if (PREDICT_TRUE(s != 0))
        return tcp_get_connection_from_transport(transport_get_listener(TRANSPORT_PROTO_TCP, s->connection_index));
    else
        return 0;
}
...
static void tcp46_syn_sent_trace_frame(vlib_main_t *vm, vlib_node_runtime_t *node, u32 *from, u32 n_bufs)
{
    ...
    for (i = 0; i < n_bufs; i++) {
        ...
        if (!(b->flags & VLIB_BUFFER_IS_TRACED)) continue;
        tc = tcp_half_open_connection_get(vnet_buffer(b)->tcp.connection_index);
        t  = vlib_add_trace(vm, node, b, sizeof(*t));
        tcp_set_rx_trace_data(t, tc, tcp_buffer_hdr(b), b, 1);
    }
}

always_inline void tcp_check_tx_offload(tcp_connection_t *tc, int is_ipv4)
{
    ...
    if (PREDICT_FALSE(lb->lb_n_buckets > 1)) return;
    dpo = load_balance_get_bucket_i(lb, 0);

    sw_if_idx = dpo_get_urpf(dpo);
    if (PREDICT_FALSE(sw_if_idx == ~0)) return;

    hw_if = vnet_get_sup_hw_interface(vnm, sw_if_idx);
    if (hw_if->caps & VNET_HW_IF_CAP_TCP_GSO) tc->cfg_flags |= TCP_CFG_F_TSO;
}

static void tcp_input_trace_frame(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t **bs, u16 *nexts, u32 n_bufs, u8 is_ip4)
{
    tcp_connection_t *tc;
    tcp_header_t     *tcp;
    tcp_rx_trace_t   *t;
    ...
    for (i = 0; i < n_bufs; i++) {
        if (!(bs[i]->flags & VLIB_BUFFER_IS_TRACED)) continue;

        t = vlib_add_trace(vm, node, bs[i], sizeof(*t));
        if (nexts[i] == TCP_INPUT_NEXT_DROP || nexts[i] == TCP_INPUT_NEXT_PUNT || nexts[i] == TCP_INPUT_NEXT_RESET) {
            tc = 0;
        }
        else {
            ...
            if (flags == TCP_STATE_LISTEN)
                ...
                tc = tcp_half_open_connection_get(vnet_buffer(bs[i])->tcp.connection_index);
            else
                tc = tcp_connection_get(vnet_buffer(bs[i])->tcp.connection_index, vm->thread_index);
        }
...
always_inline uword tcp46_syn_sent_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ip4)
{
    ...
    if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE)) tcp46_syn_sent_trace_frame(vm, node, from, n_left_from);
    ...
    while (n_left_from > 0) {
        ...
        tc = tcp_half_open_connection_get(vnet_buffer(b[0])->tcp.connection_index);
        if (PREDICT_FALSE(tc == 0)) {
            error = TCP_ERROR_INVALID_CONNECTION;
            goto drop;
        }
        ...
        if (PREDICT_FALSE(tcp->dst_port != tc->c_lcl_port || tcp->src_port != tc->c_rmt_port)) {
            error = TCP_ERROR_INVALID_CONNECTION;
            goto drop;
        }
        ...
        if (tcp_ack(tcp)) {
            if (seq_leq(ack, tc->iss) || seq_gt(ack, tc->snd_nxt)) {
                if (!tcp_rst(tcp)) tcp_send_reset_w_pkt(tc, b[0], thread_index, is_ip4);
        ...
        if (tcp_rst(tcp)) {
            ...
            if (tcp_ack(tcp)) tcp_rcv_rst(wrk, tc);
        ...
        new_tc->snd_wnd = clib_net_to_host_u16(tcp->window) << new_tc->snd_wscale;
        new_tc->snd_wl1 = seq;
        new_tc->snd_wl2 = ack;
        ...
        if (PREDICT_TRUE(tcp_ack(tcp))) {
            ...
            if (session_stream_connect_notify(&new_tc->connection, SESSION_E_NONE)) {
        ...
        if (!(new_tc->cfg_flags & TCP_CFG_F_NO_TSO)) tcp_check_tx_offload(new_tc, is_ip4);

        /* Read data, if any */
        if (PREDICT_FALSE(vnet_buffer(b[0])->tcp.data_len)) {
            clib_warning("rcvd data in syn-sent");
            error = tcp_segment_rcv(wrk, new_tc, b[0]);
            if (error == TCP_ERROR_ACK_OK) error = TCP_ERROR_SYN_ACKS_RCVD;
        }
        else {
    ...
    cleanup_ho:
        ...
        if (tcp_half_open_connection_cleanup(tc)) tc->flags |= TCP_CONN_HALF_OPEN_DONE;
...
VLIB_NODE_FN(tcp4_syn_sent_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return tcp46_syn_sent_inline(vm, node, from_frame, 1 /* is_ip4 */);
}

VLIB_NODE_FN(tcp6_syn_sent_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
static void tcp46_rcv_process_trace_frame(vlib_main_t *vm, vlib_node_runtime_t *node, u32 *from, u32 n_bufs)
{
    ...
    for (i = 0; i < n_bufs; i++) {
        b = vlib_get_buffer(vm, from[i]);
        if (!(b->flags & VLIB_BUFFER_IS_TRACED)) continue;
        tc = tcp_connection_get(vnet_buffer(b)->tcp.connection_index, thread_index);
        t  = vlib_add_trace(vm, node, b, sizeof(*t));
        tcp_set_rx_trace_data(t, tc, tcp_buffer_hdr(b), b, 1);
    }
}
 ...
 * as per RFC793 p. 64
 */
always_inline uword tcp46_rcv_process_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ip4)
{
    ...
    if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE)) tcp46_rcv_process_trace_frame(vm, node, from, n_left_from);
    ...
    while (n_left_from > 0) {
        ...
        if (CLIB_DEBUG) {
            if (!(tc->connection.flags & TRANSPORT_CONNECTION_F_NO_LOOKUP)) {
                ...
                tmp = tcp_lookup_connection(tc->c_fib_index, b[0], thread_index, is_ip4);
                if (tmp->state != tc->state) {
                    if (tc->state != TCP_STATE_CLOSED) clib_warning("state changed");
                    goto drop;
                }
            }
        }
        ...
        /* 1-4: check SEQ, RST, SYN */
        if (PREDICT_FALSE(tcp_segment_validate(wrk, tc, b[0], tcp, &error))) goto drop;
        ...
        switch (tc->state) {
            case TCP_STATE_SYN_RCVD:
                ...
                if (!(tc->cfg_flags & TCP_CFG_F_NO_TSO)) tcp_check_tx_offload(tc, is_ip4);

                /* Initialize session variables */
                tc->snd_una = vnet_buffer(b[0])->tcp.ack_number;
                tc->snd_wnd = clib_net_to_host_u16(tcp->window) << tc->rcv_opts.wscale;
            ...
            case TCP_STATE_ESTABLISHED:
                ...
                if (tcp_rcv_ack(wrk, tc, b[0], tcp, &error)) goto drop;
            ...
            case TCP_STATE_FIN_WAIT_1:
                ...
                if (tc->flags & TCP_CONN_FINPNDG) {
                    ...
                    if (max_deq <= tc->burst_acked) tcp_send_fin(tc);
                ...
                /* If FIN is ACKed */
                    ...
                    tcp_timer_set(&wrk->timer_wheel, tc, TCP_TIMER_WAITCLOSE, tcp_cfg.finwait2_time);

                    /* Don't try to deq the FIN acked */
                    if (tc->burst_acked > 1) session_tx_fifo_dequeue_drop(&tc->connection, tc->burst_acked - 1);
                    tc->burst_acked = 0;
                }
                break;
            case TCP_STATE_FIN_WAIT_2:
                ...
                if (tcp_rcv_ack_no_cc(tc, b[0], &error)) goto drop;
                tc->burst_acked = 0;
                break;
            case TCP_STATE_CLOSE_WAIT:
                ...
                if (!(tc->flags & TCP_CONN_FINPNDG)) break;

                /* Still have outstanding tx data */
                max_deq = transport_max_tx_dequeue(&tc->connection);
                if (max_deq > tc->burst_acked) break;

                tcp_connection_timers_reset(tc);
                tcp_send_fin(tc);
                tcp_connection_set_state(tc, TCP_STATE_LAST_ACK);
                tcp_timer_set(&wrk->timer_wheel, tc, TCP_TIMER_WAITCLOSE, tcp_cfg.lastack_time);
                break;
            case TCP_STATE_CLOSING:
                /* In addition to the processing for the ESTABLISHED state, if
                ...
                if (tc->snd_una != tc->snd_nxt) goto drop;

                tcp_connection_timers_reset(tc);
                tcp_connection_set_state(tc, TCP_STATE_TIME_WAIT);
                tcp_timer_set(&wrk->timer_wheel, tc, TCP_TIMER_WAITCLOSE, tcp_cfg.timewait_time);
                session_transport_closed_notify(&tc->connection);
                goto drop;
        ...
        switch (tc->state) {
            ...
            case TCP_STATE_FIN_WAIT_2:
                ...
                if (vnet_buffer(b[0])->tcp.seq_end != tc->rcv_nxt) goto drop;
                break;
            case TCP_STATE_CLOSE_WAIT:
            case TCP_STATE_CLOSING:
            case TCP_STATE_LAST_ACK:
        ...
        switch (tc->state) {
            ...
            case TCP_STATE_FIN_WAIT_1:
                ...
                if (tc->flags & TCP_CONN_FINPNDG) {
                    ...
                    tcp_timer_update(&wrk->timer_wheel, tc, TCP_TIMER_WAITCLOSE, tcp_cfg.closing_time);
                }
                break;
            case TCP_STATE_FIN_WAIT_2:
...
VLIB_NODE_FN(tcp4_rcv_process_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return tcp46_rcv_process_inline(vm, node, from_frame, 1 /* is_ip4 */);
}

VLIB_NODE_FN(tcp6_rcv_process_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
static void tcp46_listen_trace_frame(vlib_main_t *vm, vlib_node_runtime_t *node, u32 *to_next, u32 n_bufs)
{
    ...
    for (i = 0; i < n_bufs; i++) {
        ...
        if (!(b->flags & VLIB_BUFFER_IS_TRACED)) continue;
        if (vnet_buffer(b)->tcp.flags == TCP_STATE_LISTEN) tc = tcp_listener_get(vnet_buffer(b)->tcp.connection_index);
        t = vlib_add_trace(vm, node, b, sizeof(*t));
        tcp_set_rx_trace_data(t, tc, tcp_buffer_hdr(b), b, 1);
    }
}
 ...
 */
always_inline int syn_during_timewait(tcp_connection_t *tc, vlib_buffer_t *b, u32 *iss)
{
    ...
    *iss = 0;
    ...
    if (!paws_reject
        && (seq_geq(vnet_buffer(b)->tcp.seq_number, tc->rcv_nxt)
            || (tcp_opts_tstamp(&tc->rcv_opts) && timestamp_lt(tc->tsval_recent, tc->rcv_opts.tsval)))) {
         ...
         */
        tw_iss = tc->snd_nxt + tcp_available_snd_wnd(tc) + (uword) (tcp_time_now_us(tc->c_thread_index) * 1e6) % 65535;
        if (tw_iss == 0) tw_iss++;
        *iss = tw_iss;
 ...
 */
always_inline uword tcp46_listen_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ip4)
{
    ...
    if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE)) tcp46_listen_trace_frame(vm, node, from, n_left_from);
    ...
    while (n_left_from > 0) {
        ...
        child = tcp_lookup_connection(lc->c_fib_index, b[0], thread_index, is_ip4);
        ...
        if (session_stream_accept(&child->connection, lc->c_s_index, lc->c_thread_index, 0 /* notify */)) {
...
VLIB_NODE_FN(tcp4_listen_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return tcp46_listen_inline(vm, node, from_frame, 1 /* is_ip4 */);
}

VLIB_NODE_FN(tcp6_listen_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
always_inline uword tcp46_drop_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ip4)
{
...
static inline void tcp_input_dispatch_buffer(tcp_main_t *tm, tcp_connection_t *tc, vlib_buffer_t *b, u16 *next, u16 *err_counters)
{
    tcp_header_t *tcp;
    u32           error;
    u8            flags;
    ...
    *next = tm->dispatch_table[tc->state][flags].next;
    ...
    if (PREDICT_FALSE(error != TCP_ERROR_NONE)) {
        ...
        if (error == TCP_ERROR_DISPATCH)
            clib_warning("tcp conn %u disp error state %U flags %U", tc->c_c_index, format_tcp_state, tc->state, format_tcp_flags, (int) flags);
    }
}

always_inline uword tcp46_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ip4, u8 is_nolookup)
{
    u32            n_left_from, *from, thread_index = vm->thread_index;
    ...
    while (n_left_from >= 4) {
        ...
        tc0 = tcp_input_lookup_buffer(b[0], thread_index, &error0, is_ip4, is_nolookup);
        tc1 = tcp_input_lookup_buffer(b[1], thread_index, &error1, is_ip4, is_nolookup);
    ...
    if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE)) tcp_input_trace_frame(vm, node, bufs, nexts, frame->n_vectors, is_ip4);
...
VLIB_NODE_FN(tcp4_input_nolookup_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return tcp46_input_inline(vm, node, from_frame, 1 /* is_ip4 */, 1 /* is_nolookup */);
}

VLIB_NODE_FN(tcp6_input_nolookup_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return tcp46_input_inline(vm, node, from_frame, 0 /* is_ip4 */, 1 /* is_nolookup */);
}
...
VLIB_NODE_FN(tcp4_input_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return tcp46_input_inline(vm, node, from_frame, 1 /* is_ip4 */, 0 /* is_nolookup */);
}

VLIB_NODE_FN(tcp6_input_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return tcp46_input_inline(vm, node, from_frame, 0 /* is_ip4 */, 0 /* is_nolookup */);
}
...
#define _(t, f, n, e)                                     \
    ...
    _(LISTEN, TCP_FLAG_SYN | TCP_FLAG_ACK, TCP_INPUT_NEXT_RESET, TCP_ERROR_ACK_INVALID);
    _(LISTEN, TCP_FLAG_SYN | TCP_FLAG_RST, TCP_INPUT_NEXT_DROP, TCP_ERROR_SEGMENT_INVALID);
    ...
    _(LISTEN, TCP_FLAG_FIN | TCP_FLAG_ACK, TCP_INPUT_NEXT_RESET, TCP_ERROR_SEGMENT_INVALID);
    _(LISTEN, TCP_FLAG_FIN | TCP_FLAG_RST, TCP_INPUT_NEXT_DROP, TCP_ERROR_SEGMENT_INVALID);
    _(LISTEN, TCP_FLAG_FIN | TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_DROP, TCP_ERROR_SEGMENT_INVALID);
    _(LISTEN, TCP_FLAG_FIN | TCP_FLAG_SYN, TCP_INPUT_NEXT_DROP, TCP_ERROR_SEGMENT_INVALID);
    _(LISTEN, TCP_FLAG_FIN | TCP_FLAG_SYN | TCP_FLAG_ACK, TCP_INPUT_NEXT_DROP, TCP_ERROR_SEGMENT_INVALID);
    ...
    _(SYN_RCVD, TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(SYN_RCVD, TCP_FLAG_SYN, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(SYN_RCVD, TCP_FLAG_SYN | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(SYN_RCVD, TCP_FLAG_SYN | TCP_FLAG_RST, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(SYN_RCVD, TCP_FLAG_SYN | TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(SYN_RCVD, TCP_FLAG_FIN, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(SYN_RCVD, TCP_FLAG_FIN | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(SYN_RCVD, TCP_FLAG_FIN | TCP_FLAG_RST, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(SYN_RCVD, TCP_FLAG_FIN | TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(SYN_RCVD, TCP_FLAG_FIN | TCP_FLAG_SYN, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(SYN_RCVD, TCP_FLAG_FIN | TCP_FLAG_SYN | TCP_FLAG_RST, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(SYN_RCVD, TCP_FLAG_FIN | TCP_FLAG_SYN | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(SYN_RCVD, TCP_FLAG_FIN | TCP_FLAG_SYN | TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(SYN_RCVD, 0, TCP_INPUT_NEXT_DROP, TCP_ERROR_SEGMENT_INVALID);
    /* SYN-ACK for a SYN */
    _(SYN_SENT, TCP_FLAG_SYN | TCP_FLAG_ACK, TCP_INPUT_NEXT_SYN_SENT, TCP_ERROR_NONE);
    ...
    _(SYN_SENT, TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_SYN_SENT, TCP_ERROR_NONE);
    _(SYN_SENT, TCP_FLAG_FIN, TCP_INPUT_NEXT_SYN_SENT, TCP_ERROR_NONE);
    _(SYN_SENT, TCP_FLAG_FIN | TCP_FLAG_ACK, TCP_INPUT_NEXT_SYN_SENT, TCP_ERROR_NONE);
    ...
    _(ESTABLISHED, TCP_FLAG_FIN | TCP_FLAG_ACK, TCP_INPUT_NEXT_ESTABLISHED, TCP_ERROR_NONE);
    _(ESTABLISHED, TCP_FLAG_FIN | TCP_FLAG_RST, TCP_INPUT_NEXT_ESTABLISHED, TCP_ERROR_NONE);
    _(ESTABLISHED, TCP_FLAG_FIN | TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_ESTABLISHED, TCP_ERROR_NONE);
    _(ESTABLISHED, TCP_FLAG_FIN | TCP_FLAG_SYN, TCP_INPUT_NEXT_ESTABLISHED, TCP_ERROR_NONE);
    _(ESTABLISHED, TCP_FLAG_FIN | TCP_FLAG_SYN | TCP_FLAG_ACK, TCP_INPUT_NEXT_ESTABLISHED, TCP_ERROR_NONE);
    _(ESTABLISHED, TCP_FLAG_FIN | TCP_FLAG_SYN | TCP_FLAG_RST, TCP_INPUT_NEXT_ESTABLISHED, TCP_ERROR_NONE);
    _(ESTABLISHED, TCP_FLAG_FIN | TCP_FLAG_SYN | TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_ESTABLISHED, TCP_ERROR_NONE);
    _(ESTABLISHED, TCP_FLAG_RST, TCP_INPUT_NEXT_ESTABLISHED, TCP_ERROR_NONE);
    _(ESTABLISHED, TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_ESTABLISHED, TCP_ERROR_NONE);
    _(ESTABLISHED, TCP_FLAG_SYN, TCP_INPUT_NEXT_ESTABLISHED, TCP_ERROR_NONE);
    _(ESTABLISHED, TCP_FLAG_SYN | TCP_FLAG_ACK, TCP_INPUT_NEXT_ESTABLISHED, TCP_ERROR_NONE);
    _(ESTABLISHED, TCP_FLAG_SYN | TCP_FLAG_RST, TCP_INPUT_NEXT_ESTABLISHED, TCP_ERROR_NONE);
    _(ESTABLISHED, TCP_FLAG_SYN | TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_ESTABLISHED, TCP_ERROR_NONE);
    ...
    _(FIN_WAIT_1, TCP_FLAG_ACK | TCP_FLAG_FIN, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    ...
    _(FIN_WAIT_1, TCP_FLAG_FIN | TCP_FLAG_SYN, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(FIN_WAIT_1, TCP_FLAG_FIN | TCP_FLAG_SYN | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(FIN_WAIT_1, TCP_FLAG_FIN | TCP_FLAG_SYN | TCP_FLAG_RST, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(FIN_WAIT_1, TCP_FLAG_FIN | TCP_FLAG_SYN | TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(FIN_WAIT_1, TCP_FLAG_FIN | TCP_FLAG_RST, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(FIN_WAIT_1, TCP_FLAG_FIN | TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(FIN_WAIT_1, TCP_FLAG_SYN, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(FIN_WAIT_1, TCP_FLAG_SYN | TCP_FLAG_RST, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(FIN_WAIT_1, TCP_FLAG_SYN | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(FIN_WAIT_1, TCP_FLAG_SYN | TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(FIN_WAIT_1, TCP_FLAG_RST, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(FIN_WAIT_1, TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    ...
    _(CLOSING, TCP_FLAG_SYN | TCP_FLAG_RST, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(CLOSING, TCP_FLAG_SYN | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(CLOSING, TCP_FLAG_SYN | TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(CLOSING, TCP_FLAG_RST, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(CLOSING, TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(CLOSING, TCP_FLAG_FIN, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(CLOSING, TCP_FLAG_FIN | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(CLOSING, TCP_FLAG_FIN | TCP_FLAG_RST, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(CLOSING, TCP_FLAG_FIN | TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(CLOSING, TCP_FLAG_FIN | TCP_FLAG_SYN, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(CLOSING, TCP_FLAG_FIN | TCP_FLAG_SYN | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(CLOSING, TCP_FLAG_FIN | TCP_FLAG_SYN | TCP_FLAG_RST, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(CLOSING, TCP_FLAG_FIN | TCP_FLAG_SYN | TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    /* FIN confirming that the peer (app) has closed */
    _(FIN_WAIT_2, TCP_FLAG_FIN, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(FIN_WAIT_2, TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(FIN_WAIT_2, TCP_FLAG_FIN | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(FIN_WAIT_2, TCP_FLAG_RST, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(FIN_WAIT_2, TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(FIN_WAIT_2, TCP_FLAG_SYN, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(FIN_WAIT_2, TCP_FLAG_SYN | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(CLOSE_WAIT, TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(CLOSE_WAIT, TCP_FLAG_FIN | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(CLOSE_WAIT, TCP_FLAG_RST, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(CLOSE_WAIT, TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    ...
    _(LAST_ACK, TCP_FLAG_FIN | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(LAST_ACK, TCP_FLAG_FIN | TCP_FLAG_SYN, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(LAST_ACK, TCP_FLAG_FIN | TCP_FLAG_SYN | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(LAST_ACK, TCP_FLAG_FIN | TCP_FLAG_RST, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(LAST_ACK, TCP_FLAG_FIN | TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(LAST_ACK, TCP_FLAG_FIN | TCP_FLAG_SYN | TCP_FLAG_RST, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(LAST_ACK, TCP_FLAG_FIN | TCP_FLAG_SYN | TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(LAST_ACK, TCP_FLAG_RST, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(LAST_ACK, TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(LAST_ACK, TCP_FLAG_SYN, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(LAST_ACK, TCP_FLAG_SYN | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(LAST_ACK, TCP_FLAG_SYN | TCP_FLAG_RST, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(LAST_ACK, TCP_FLAG_SYN | TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    ...
    _(TIME_WAIT, TCP_FLAG_FIN | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(TIME_WAIT, TCP_FLAG_RST, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    _(TIME_WAIT, TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_RCV_PROCESS, TCP_ERROR_NONE);
    ...
    _(CLOSED, TCP_FLAG_RST | TCP_FLAG_ACK, TCP_INPUT_NEXT_DROP, TCP_ERROR_CONNECTION_CLOSED);
    ...
    _(CLOSED, TCP_FLAG_FIN | TCP_FLAG_ACK, TCP_INPUT_NEXT_RESET, TCP_ERROR_CONNECTION_CLOSED);
#undef _
}
...
static clib_error_t *tcp_input_init(vlib_main_t *vm)
{
    ...
    if ((error = vlib_call_init_function(vm, tcp_init))) return error;
