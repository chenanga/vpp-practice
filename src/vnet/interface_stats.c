...
int vnet_sw_interface_stats_collect_enable_disable(u32 sw_if_index, u8 enable)
{
    ...
    vnet_feature_enable_disable("device-input", "stats-collect-rx", sw_if_index, enable, 0, 0);
    vnet_feature_enable_disable("interface-output", "stats-collect-tx", sw_if_index, enable, 0, 0);
...
static_always_inline uword stats_collect_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, vlib_rx_or_tx_t rxtx)
{
    ...
    while (n_left_from > 0) {
        ...
        if (VLIB_RX == rxtx) {
            ...
            {
                vlib_increment_combined_counter(vnet_main.interface_main.combined_sw_if_counters + ct,
