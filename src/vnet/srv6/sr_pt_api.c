...
static void send_sr_pt_iface_details(sr_pt_iface_t *t, vl_api_registration_t *reg, u32 context)
{
...
static void vl_api_sr_pt_iface_dump_t_handler(vl_api_sr_pt_iface_dump_t *mp)
{
...
static void vl_api_sr_pt_iface_add_t_handler(vl_api_sr_pt_iface_add_t *mp)
{
    ...
    rv = sr_pt_add_iface(ntohl(mp->sw_if_index), ntohs(mp->id), mp->ingress_load, mp->egress_load, mp->tts_template);

    BAD_SW_IF_INDEX_LABEL;
    REPLY_MACRO(VL_API_SR_PT_IFACE_ADD_REPLY);
}

static void vl_api_sr_pt_iface_del_t_handler(vl_api_sr_pt_iface_del_t *mp)
{
...
static clib_error_t *sr_pt_api_hookup(vlib_main_t *vm)
{
    /*
