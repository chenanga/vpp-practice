...
static void vl_api_sr_localsid_add_del_t_handler(vl_api_sr_localsid_add_del_t *mp)
{
     ...
     */
    if (mp->behavior == SR_BEHAVIOR_X || mp->behavior == SR_BEHAVIOR_DX6 || mp->behavior == SR_BEHAVIOR_DX4 || mp->behavior == SR_BEHAVIOR_DX2)
...
static void vl_api_sr_policy_add_t_handler(vl_api_sr_policy_add_t *mp)
{
     ...
     */
    int rv = 0;
    rv     = sr_policy_add(&bsid_addr, segments, NULL, ntohl(mp->sids.weight), mp->is_spray, ntohl(mp->fib_table), mp->is_encap, 0, NULL);
    vec_free(segments);
...
static void vl_api_sr_policy_add_v2_t_handler(vl_api_sr_policy_add_v2_t *mp)
{
     ...
     */
    int rv = 0;
    rv     = sr_policy_add(&bsid_addr, segments, &encap_src, ntohl(mp->sids.weight), mp->type, ntohl(mp->fib_table), mp->is_encap, 0, NULL);
    vec_free(segments);

    REPLY_MACRO(VL_API_SR_POLICY_ADD_V2_REPLY);
}

static void vl_api_sr_policy_mod_v2_t_handler(vl_api_sr_policy_mod_v2_t *mp)
{
...
static void vl_api_sr_steering_add_del_t_handler(vl_api_sr_steering_add_del_t *mp)
{
    ...
    if (mp->traffic_type == SR_STEER_L2) VALIDATE_SW_IF_INDEX(mp);
...
static void send_sr_localsid_details(ip6_sr_localsid_t *t, vl_api_registration_t *reg, u32 context)
{
    ...
    if (t->behavior == SR_BEHAVIOR_T || t->behavior == SR_BEHAVIOR_DT6)
        rmp->xconnect_iface_or_vrf_table = htonl(fib_table_get_table_id(t->sw_if_index, FIB_PROTOCOL_IP6));
    else if (t->behavior == SR_BEHAVIOR_DT4)
        rmp->xconnect_iface_or_vrf_table = htonl(fib_table_get_table_id(t->sw_if_index, FIB_PROTOCOL_IP4));
    else
...
static void send_sr_localsid_with_packet_stats_details(int local_sid_index, ip6_sr_localsid_t *t, vl_api_registration_t *reg, u32 context)
{
    ...
    rmp = vl_msg_api_alloc(sizeof(*rmp));
    clib_memset(rmp, 0, sizeof(*rmp));
    rmp->_vl_msg_id = ntohs(REPLY_MSG_ID_BASE + VL_API_SR_LOCALSIDS_WITH_PACKET_STATS_DETAILS);
    ip6_address_encode(&t->localsid, rmp->addr);
    rmp->end_psp    = t->end_psp;
    rmp->behavior   = t->behavior;
    rmp->fib_table  = htonl(t->fib_table);
    ...
    rmp->context = context;
    vlib_get_combined_counter(&(sm->sr_ls_valid_counters), local_sid_index, &good_traffic);
    vlib_get_combined_counter(&(sm->sr_ls_invalid_counters), local_sid_index, &bad_traffic);
...
static void vl_api_sr_localsids_with_packet_stats_dump_t_handler(vl_api_sr_localsids_with_packet_stats_dump_t *mp)
{
    vl_api_registration_t *reg;
    ip6_sr_main_t         *sm            = &sr_main;
...
static void send_sr_policies_details(ip6_sr_policy_t *t, vl_api_registration_t *reg, u32 context)
{
    ...
    rmp = vl_msg_api_alloc(sizeof(*rmp) + vec_len(t->segments_lists) * sizeof(vl_api_srv6_sid_list_t));
    clib_memset(rmp, 0, (sizeof(*rmp) + vec_len(t->segments_lists) * sizeof(vl_api_srv6_sid_list_t)));
...
static void send_sr_policies_v2_details(ip6_sr_policy_t *t, vl_api_registration_t *reg, u32 context)
{
...
static void vl_api_sr_policies_v2_dump_t_handler(vl_api_sr_policies_v2_dump_t *mp)
{
    vl_api_registration_t *reg;
    ip6_sr_main_t         *sm = &sr_main;
    ip6_sr_policy_t       *t;
...
static void send_sr_policies_details_with_sl_index(ip6_sr_policy_t *t, vl_api_registration_t *reg, u32 context)
{
    ...
    rmp = vl_msg_api_alloc(sizeof(*rmp) + vec_len(t->segments_lists) * sizeof(vl_api_srv6_sid_list_with_sl_index_t));
    clib_memset(rmp, 0, (sizeof(*rmp) + vec_len(t->segments_lists) * sizeof(vl_api_srv6_sid_list_with_sl_index_t)));

    rmp->_vl_msg_id = ntohs(REPLY_MSG_ID_BASE + VL_API_SR_POLICIES_WITH_SL_INDEX_DETAILS);
    ip6_address_encode(&t->bsid, rmp->bsid);
    rmp->is_encap      = t->is_encap;
    rmp->is_spray      = t->type;
    rmp->fib_table     = htonl(t->fib_table);
...
static void send_sr_steering_pol_details(ip6_sr_steering_policy_t *t, vl_api_registration_t *reg, u32 context)
{
    ...
    ip_address_encode(&t->classify.l3.prefix, IP46_TYPE_ANY, &rmp->prefix.address);
