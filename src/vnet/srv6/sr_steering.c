                       ...
                       u8              traffic_type)
{
    ...
    if (traffic_type == SR_STEER_IPV4 || traffic_type == SR_STEER_IPV6) {
        ...
        if (pool_is_free_index(sm->vnet_main->interface_main.sw_interfaces, sw_if_index)) return -3;

        vnet_sw_interface_t *sw = vnet_get_sw_interface(sm->vnet_main, sw_if_index);
        if (sw->type != VNET_SW_INTERFACE_TYPE_HARDWARE) return -3;
    }
    else
        return -1;
    ...
    if (p) {
        ...
        if (is_del) {
            if (steer_pl->classify.traffic_type == SR_STEER_IPV6) {
                ...
                fib_table_entry_delete(fib_table_find(FIB_PROTOCOL_IP6, steer_pl->classify.l3.fib_table), &pfx, FIB_SOURCE_SR);
            }
                ...
                fib_table_entry_delete(fib_table_find(FIB_PROTOCOL_IP4, steer_pl->classify.l3.fib_table), &pfx, FIB_SOURCE_SR);
            }
            else if (steer_pl->classify.traffic_type == SR_STEER_L2) {
                /* Remove HW redirection */
                int ret = vnet_feature_enable_disable("device-input", "sr-pl-rewrite-encaps-l2", sw_if_index, 0, 0, 0);
                ...
                if (hi->sw_if_index == sw_if_index) ethernet_set_flags(vnm, hi->hw_if_index, 0);
            }
    ...
    if (traffic_type == SR_STEER_IPV4 || traffic_type == SR_STEER_IPV6) {
        clib_memcpy_fast(&steer_pl->classify.l3.prefix, prefix, sizeof(ip46_address_t));
    ...
    if (traffic_type == SR_STEER_L2) {
        if (!sr_policy->is_encap) goto cleanup_error_encap;

        if (vnet_feature_enable_disable("device-input", "sr-pl-rewrite-encaps-l2", sw_if_index, 1, 0, 0)) goto cleanup_error_redirection;

        /* Set promiscous mode on interface */
        vnet_main_t         *vnm = vnet_get_main();
        vnet_hw_interface_t *hi  = vnet_get_sup_hw_interface(vnm, sw_if_index);
        /* Make sure it is main interface */
        if (hi->sw_if_index == sw_if_index) ethernet_set_flags(vnm, hi->hw_if_index, ETHERNET_INTERFACE_FLAG_ACCEPT_ALL);
    }
...
update_fib:
    ...
    if (traffic_type == SR_STEER_IPV6) {
        ...
        fib_table_entry_path_add(fib_table_find(FIB_PROTOCOL_IP6, (table_id != (u32) ~0 ? table_id : 0)),
                                 &pfx,
                                 FIB_SOURCE_SR,
                                 FIB_ENTRY_FLAG_LOOSE_URPF_EXEMPT,
                                 DPO_PROTO_IP6,
                                 (ip46_address_t *) &sr_policy->bsid,
                                 ~0,
                                 sm->fib_table_ip6,
                                 1,
                                 NULL,
        ...
        fib_table_entry_path_add(fib_table_find(FIB_PROTOCOL_IP4, (table_id != (u32) ~0 ? table_id : 0)),
                                 &pfx,
                                 FIB_SOURCE_SR,
                                 FIB_ENTRY_FLAG_LOOSE_URPF_EXEMPT,
                                 DPO_PROTO_IP6,
        ...
        if (sw_if_index < vec_len(sm->sw_iface_sr_policies))
            ...
            vec_resize(sm->sw_iface_sr_policies, (pool_len(sm->vnet_main->interface_main.sw_interfaces) - vec_len(sm->sw_iface_sr_policies)));
            sm->sw_iface_sr_policies[sw_if_index] = steer_pl->sr_policy;
        }
    }
...
static clib_error_t *sr_steer_policy_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (!traffic_type && unformat(input, "l2 %U", unformat_vnet_sw_interface, vnm, &sw_if_index))
            traffic_type = SR_STEER_L2;
        else if (!sr_policy_set && unformat(input, "via index %d", &sr_policy_index))
            sr_policy_set = 1;
        else if (!sr_policy_set && unformat(input, "via bsid %U", unformat_ip6_address, &bsid))
            sr_policy_set = 1;
    ...
    if (!traffic_type) return clib_error_return(0, "No L2/L3 traffic specified");
    if (!is_del && !sr_policy_set) return clib_error_return(0, "No SR policy specified");
    ...
    rv = sr_steering_policy(is_del,
                            (sr_policy_index == ~(u32) 0 ? &bsid : NULL),
                            sr_policy_index,
                            fib_table,
                            &prefix,
                            dst_mask_width,
    ...
    switch (rv) {
        ...
        case -2:
            return clib_error_return(0, "The requested SR policy could not be located. Review the BSID/index.");
        case -3:
            return clib_error_return(0, "Unable to do SW redirect. Incorrect interface.");
        case -4:
            return clib_error_return(0, "The requested SR steering policy could not be deleted.");
        case -5:
            return clib_error_return(0, "The SR policy is not an encapsulation one.");
        default:
...
static clib_error_t *show_sr_steering_policies_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (i = 0; i < vec_len(steer_policies); i++) {
        ...
        if (steer_pl->classify.traffic_type == SR_STEER_L2) {
            vlib_cli_output(vm, "L2 %U\t%U", format_vnet_sw_if_index_name, vnm, steer_pl->classify.l2.sw_if_index, format_ip6_address, &pl->bsid);
        }
        else if (steer_pl->classify.traffic_type == SR_STEER_IPV4) {
            vlib_cli_output(vm,
                            "L3 %U/%d\t%U",
...
clib_error_t *sr_steering_init(vlib_main_t *vm)
{
    ...
    mhash_init(&sm->sr_steer_policies_hash, sizeof(uword), sizeof(sr_steering_key_t));
