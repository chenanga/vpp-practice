...
static clib_error_t *set_sr_hop_limit_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (unformat_check_input(input) == UNFORMAT_END_OF_INPUT) return clib_error_return(0, "No value specified");
    if (!unformat(input, "%d", &hop_limit)) return clib_error_return(0, "Invalid value");
    if (hop_limit <= 0 || hop_limit > 255) return clib_error_return(0, "Value out of range [1-255]");
 ...
 */
static inline u8 *compute_rewrite_encaps(ip6_address_t *sl, ip6_address_t *src_v6addr, u8 type)
{
    ip6_header_t    *iph;
    ip6_sr_header_t *srh;
    ...
    iph                                          = (ip6_header_t *) rs;
    iph->ip_version_traffic_class_and_flow_label = clib_host_to_net_u32(0 | ((6 & 0xF) << 28));
    ...
    if (type == SR_POLICY_TYPE_TEF) {
        ...
        srh->length        = ((sizeof(ip6_sr_header_t) + (vec_len(sl) * sizeof(ip6_address_t)) + sizeof(ip6_sr_pt_tlv_t)) / 8) - 1;
 ...
 */
static inline u8 *compute_rewrite_insert(ip6_address_t *sl, u8 type)
{
    ip6_sr_header_t *srh;
    ip6_address_t   *addrp, *this_address;
    u32              header_length = 0;
    ...
    srh->length        = ((sizeof(ip6_sr_header_t) + ((vec_len(sl) + 1) * sizeof(ip6_address_t))) / 8) - 1;
    srh->flags         = 0x00;
    srh->tag           = 0x0000;
 ...
 */
static inline u8 *compute_rewrite_bsid(ip6_address_t *sl)
{
    ip6_sr_header_t *srh;
    ip6_address_t   *addrp, *this_address;
    u32              header_length = 0;
 ...
 */
static inline ip6_sr_sl_t *create_sl(ip6_sr_policy_t *sr_policy, ip6_address_t *sl, ip6_address_t *encap_src, u32 weight, u8 is_encap)
{
    ...
    if (sr_policy->plugin) {
        plugin = pool_elt_at_index(sm->policy_plugin_functions, sr_policy->plugin - SR_BEHAVIOR_LAST);
    ...
    if (is_encap) {
        if (!sr_policy->plugin) {
            dpo_set(&segment_list->ip6_dpo, sr_pr_encaps_dpo_type, DPO_PROTO_IP6, segment_list - sm->sid_lists);
            dpo_set(&segment_list->ip4_dpo, sr_pr_encaps_dpo_type, DPO_PROTO_IP4, segment_list - sm->sid_lists);
            dpo_set(&segment_list->bsid_dpo, sr_pr_bsid_encaps_dpo_type, DPO_PROTO_IP6, segment_list - sm->sid_lists);
        }
        else {
            dpo_set(&segment_list->ip6_dpo, plugin->dpo, DPO_PROTO_IP6, segment_list - sm->sid_lists);
            dpo_set(&segment_list->ip4_dpo, plugin->dpo, DPO_PROTO_IP4, segment_list - sm->sid_lists);
            dpo_set(&segment_list->bsid_dpo, plugin->dpo, DPO_PROTO_IP6, segment_list - sm->sid_lists);
        }
    }
    else {
        if (!sr_policy->plugin) {
            dpo_set(&segment_list->ip6_dpo, sr_pr_insert_dpo_type, DPO_PROTO_IP6, segment_list - sm->sid_lists);
            dpo_set(&segment_list->bsid_dpo, sr_pr_bsid_insert_dpo_type, DPO_PROTO_IP6, segment_list - sm->sid_lists);
        }
        else {
            dpo_set(&segment_list->ip6_dpo, plugin->dpo, DPO_PROTO_IP6, segment_list - sm->sid_lists);
            dpo_set(&segment_list->bsid_dpo, plugin->dpo, DPO_PROTO_IP6, segment_list - sm->sid_lists);
 ...
 */
static inline void update_lb(ip6_sr_policy_t *sr_policy)
{
    ...
    if (!dpo_id_is_valid(&sr_policy->bsid_dpo)) {
        ...
        fhc = fib_table_get_flow_hash_config(sr_policy->fib_table, FIB_PROTOCOL_IP6);

        dpo_set(&sr_policy->bsid_dpo, DPO_LOAD_BALANCE, DPO_PROTO_IP6, load_balance_create(0, DPO_PROTO_IP6, fhc));

        dpo_set(&sr_policy->ip6_dpo, DPO_LOAD_BALANCE, DPO_PROTO_IP6, load_balance_create(0, DPO_PROTO_IP6, fhc));

        /* Update FIB entry's to point to the LB DPO in the main FIB and hidden one */
        fib_table_entry_special_dpo_update(fib_table_find(FIB_PROTOCOL_IP6, sr_policy->fib_table),
                                           &pfx,
                                           FIB_SOURCE_SR,
                                           FIB_ENTRY_FLAG_EXCLUSIVE,
                                           &sr_policy->bsid_dpo);

        fib_table_entry_special_dpo_update(sm->fib_table_ip6, &pfx, FIB_SOURCE_SR, FIB_ENTRY_FLAG_EXCLUSIVE, &sr_policy->ip6_dpo);

        if (sr_policy->is_encap) {
            dpo_set(&sr_policy->ip4_dpo, DPO_LOAD_BALANCE, DPO_PROTO_IP4, load_balance_create(0, DPO_PROTO_IP4, fhc));

            fib_table_entry_special_dpo_update(sm->fib_table_ip4, &pfx, FIB_SOURCE_SR, FIB_ENTRY_FLAG_EXCLUSIVE, &sr_policy->ip4_dpo);
        }
    }
    ...
    /* Update LB multipath */
    load_balance_multipath_update(&sr_policy->bsid_dpo, b_path_vector, LOAD_BALANCE_FLAG_NONE);
    load_balance_multipath_update(&sr_policy->ip6_dpo, ip6_path_vector, LOAD_BALANCE_FLAG_NONE);
    if (sr_policy->is_encap) load_balance_multipath_update(&sr_policy->ip4_dpo, ip4_path_vector, LOAD_BALANCE_FLAG_NONE);
 ...
 */
static inline void update_replicate(ip6_sr_policy_t *sr_policy)
{
    u32                *sl_index;
    ip6_sr_sl_t        *segment_list;
    ip6_sr_main_t      *sm = &sr_main;
    ...
    /* In case LB does not exist, create it */
    if (!dpo_id_is_valid(&sr_policy->bsid_dpo)) {
        dpo_set(&sr_policy->bsid_dpo, DPO_REPLICATE, DPO_PROTO_IP6, replicate_create(0, DPO_PROTO_IP6));

        dpo_set(&sr_policy->ip6_dpo, DPO_REPLICATE, DPO_PROTO_IP6, replicate_create(0, DPO_PROTO_IP6));
        ...
        if (sr_policy->is_encap) {
            dpo_set(&sr_policy->ip4_dpo, DPO_REPLICATE, DPO_PROTO_IP4, replicate_create(0, DPO_PROTO_IP4));
    ...
    if (sr_policy->is_encap) replicate_multipath_update(&sr_policy->ip4_dpo, ip4_path_vector);
}
                  ...
                  void          *ls_plugin_mem)
{
    ...
    u32 fib_index = fib_table_find(FIB_PROTOCOL_IP6, (fib_table != (u32) ~0 ? fib_table : 0));
    if (fib_index == ~0) return -13;

    /* Lookup whether there exists an entry for the BSID */
    fib_node_index_t fei = fib_table_lookup_exact_match(fib_index, &pfx);
    if (FIB_NODE_INDEX_INVALID != fei) return -12;  // There is an entry for such lookup
    ...
    if (sm->fib_table_ip6 == (u32) ~0) {
        sm->fib_table_ip6 = fib_table_create_and_lock(FIB_PROTOCOL_IP6, FIB_SOURCE_SR, "SRv6 steering of IP6 prefixes through BSIDs");
        sm->fib_table_ip4 = fib_table_create_and_lock(FIB_PROTOCOL_IP6, FIB_SOURCE_SR, "SRv6 steering of IP4 prefixes through BSIDs");
    }

    /* Create IPv6 FIB for the BindingSID attached to the DPO of the only SL */
    if (sr_policy->type == SR_POLICY_TYPE_DEFAULT || sr_policy->type == SR_POLICY_TYPE_TEF)
        update_lb(sr_policy);
...
int sr_policy_del(ip6_address_t *bsid, u32 index)
{
    ...
    fib_table_entry_special_remove(fib_table_find(FIB_PROTOCOL_IP6, sr_policy->fib_table), &pfx, FIB_SOURCE_SR);

    fib_table_entry_special_remove(sm->fib_table_ip6, &pfx, FIB_SOURCE_SR);

    if (sr_policy->is_encap) fib_table_entry_special_remove(sm->fib_table_ip4, &pfx, FIB_SOURCE_SR);
    ...
    vec_foreach (sl_index, sr_policy->segments_lists) {
        ...
        if (!sr_policy->is_encap) vec_free(segment_list->rewrite_bsid);
                  ...
                  u32            weight)
{
    ...
    {
        /* Create the new SL */
        segment_list = create_sl(sr_policy, segments, encap_src, weight, sr_policy->is_encap);
    ...
    else if (operation == 2) /* Delete SR List from an existing SR policy */
    {
        ...
        if (vec_len(sr_policy->segments_lists) == 1) return -21;

        /* Check that the SR list does exist and is assigned to the sr policy */
        vec_foreach (sl_index_iterate, sr_policy->segments_lists)
            if (*sl_index_iterate == sl_index) break;

        if (*sl_index_iterate != sl_index) return -22;
    ...
    else if (operation == 3) /* Modify the weight of an existing SR List */
    {
        ...
        if (*sl_index_iterate != sl_index) return -32;
        ...
        /* Update LB */
        if (sr_policy->type == SR_POLICY_TYPE_DEFAULT) update_lb(sr_policy);
    }
    else /* Incorrect op. */
        return -1;
...
static clib_error_t *sr_policy_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (!is_add && !is_mod && !is_del && unformat(input, "add"))
            ...
            clib_memcpy_fast(this_seg->as_u8, next_address.as_u8, sizeof(*this_seg));
        }
            ...
            vec_foreach (plugin_it, vec_plugins) {
                if (unformat(input, "%U", (*plugin_it)->ls_unformat, &ls_plugin_mem)) {
    ...
    if (!is_add && !is_mod && !is_del) return clib_error_return(0, "Incorrect CLI");

    if (!policy_set) return clib_error_return(0, "No SR policy BSID or index specified");
    ...
    if (is_add) {
        ...
        if (vec_len(segments) == 0) return clib_error_return(0, "No Segment List specified");

        rv = sr_policy_add(&bsid, segments, encap_src, weight, type, fib_table, is_encap, behavior, ls_plugin_mem);

        vec_free(segments);
    }
    else if (is_del)
        rv = sr_policy_del((sr_policy_index != (u32) ~0 ? NULL : &bsid), sr_policy_index);
    else if (is_mod) {
        if (!operation) return clib_error_return(0, "No SL modification specified");
        if (operation != 1 && sl_index == (u32) ~0) return clib_error_return(0, "No Segment List index specified");
        if (operation == 1 && vec_len(segments) == 0) return clib_error_return(0, "No Segment List specified");
        if (operation == 3 && weight == (u32) ~0) return clib_error_return(0, "No new weight for the SL specified");

        rv =
            sr_policy_mod((sr_policy_index != (u32) ~0 ? NULL : &bsid), sr_policy_index, fib_table, operation, segments, encap_src, sl_index, weight);

        if (segments) vec_free(segments);
    }
...
static clib_error_t *show_sr_policies_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vec_foreach_index (i, vec_policies) {
        ...
        vlib_cli_output(vm, "[%u].-\tBSID: %U", (u32) (sr_policy - sm->sr_policies), format_ip6_address, &sr_policy->bsid);
        vlib_cli_output(vm, "\tBehavior: %s", (sr_policy->is_encap ? "Encapsulation" : "SRH insertion"));
        if (sr_policy->is_encap) {
            vlib_cli_output(vm, "\tEncapSrcIP: %U", format_ip6_address, &sr_policy->encap_src);
        }
        switch (sr_policy->type) {
            ...
            case SR_POLICY_TYPE_TEF:
                vlib_cli_output(vm, "\tType: %s", "TEF (Timestamp, Encapsulate, and Forward)");
                break;
            default:
                vlib_cli_output(vm, "\tType: %s", "Default");
                break;
        }
        vlib_cli_output(vm, "\tFIB table: %u", (sr_policy->fib_table != (u32) ~0 ? sr_policy->fib_table : 0));
...
{
    vlib_cli_output(vm, "SR encaps source addr = %U", format_ip6_address, sr_get_encaps_source());
...
static u8 *format_sr_policy_rewrite_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "SR-policy-rewrite: src %U dst %U", format_ip6_address, &t->src, format_ip6_address, &t->dst);
 ...
 */
static_always_inline void srv6_tef_behavior(vlib_node_runtime_t *node, vlib_buffer_t *b0, ip6_header_t *ip0)
{
    ip6_sr_header_t *srh;
    ip6_sr_pt_tlv_t *srh_pt_tlv;
    timestamp_64_t   ts;
    ...
    srh_pt_tlv = (ip6_sr_pt_tlv_t *) ((u8 *) ip0 + sizeof(ip6_header_t) + sizeof(ip6_sr_header_t) + sizeof(ip6_address_t) * (srh->last_entry + 1));
                                               ...
                                               u8                   policy_type)
{
    ...
    ip0->ip_version_traffic_class_and_flow_label =
        clib_host_to_net_u32(0 | (clib_net_to_host_u32(ip0_encap->ip_version_traffic_class_and_flow_label) & 0xfff00000) | (flow_label & 0x0000ffff));
    if (policy_type == SR_POLICY_TYPE_TEF) srv6_tef_behavior(node, b0, ip0);
}
...
static uword sr_policy_rewrite_encaps(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 8 && n_left_to_next >= 4) {
            ...
            ASSERT(b1->current_data + VLIB_BUFFER_PRE_DATA_SIZE >= vec_len(sl1->rewrite));
            ASSERT(b2->current_data + VLIB_BUFFER_PRE_DATA_SIZE >= vec_len(sl2->rewrite));
            ASSERT(b3->current_data + VLIB_BUFFER_PRE_DATA_SIZE >= vec_len(sl3->rewrite));
            ...
            clib_memcpy_fast(((u8 *) ip0_encap) - vec_len(sl0->rewrite), sl0->rewrite, vec_len(sl0->rewrite));
            clib_memcpy_fast(((u8 *) ip1_encap) - vec_len(sl1->rewrite), sl1->rewrite, vec_len(sl1->rewrite));
            clib_memcpy_fast(((u8 *) ip2_encap) - vec_len(sl2->rewrite), sl2->rewrite, vec_len(sl2->rewrite));
            clib_memcpy_fast(((u8 *) ip3_encap) - vec_len(sl3->rewrite), sl3->rewrite, vec_len(sl3->rewrite));
            ...
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE))) {
                if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                    ...
                    clib_memcpy_fast(tr->src.as_u8, ip0->src_address.as_u8, sizeof(tr->src.as_u8));
                    clib_memcpy_fast(tr->dst.as_u8, ip0->dst_address.as_u8, sizeof(tr->dst.as_u8));
                }

                if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
                    sr_policy_rewrite_trace_t *tr = vlib_add_trace(vm, node, b1, sizeof(*tr));
                    clib_memcpy_fast(tr->src.as_u8, ip1->src_address.as_u8, sizeof(tr->src.as_u8));
                    clib_memcpy_fast(tr->dst.as_u8, ip1->dst_address.as_u8, sizeof(tr->dst.as_u8));
                }

                if (PREDICT_FALSE(b2->flags & VLIB_BUFFER_IS_TRACED)) {
                    sr_policy_rewrite_trace_t *tr = vlib_add_trace(vm, node, b2, sizeof(*tr));
                    clib_memcpy_fast(tr->src.as_u8, ip2->src_address.as_u8, sizeof(tr->src.as_u8));
                    clib_memcpy_fast(tr->dst.as_u8, ip2->dst_address.as_u8, sizeof(tr->dst.as_u8));
                }

                if (PREDICT_FALSE(b3->flags & VLIB_BUFFER_IS_TRACED)) {
                    sr_policy_rewrite_trace_t *tr = vlib_add_trace(vm, node, b3, sizeof(*tr));
                    clib_memcpy_fast(tr->src.as_u8, ip3->src_address.as_u8, sizeof(tr->src.as_u8));
                    clib_memcpy_fast(tr->dst.as_u8, ip3->dst_address.as_u8, sizeof(tr->dst.as_u8));
                }
            }
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE) && PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                sr_policy_rewrite_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
                clib_memcpy_fast(tr->src.as_u8, ip0->src_address.as_u8, sizeof(tr->src.as_u8));
                clib_memcpy_fast(tr->dst.as_u8, ip0->dst_address.as_u8, sizeof(tr->dst.as_u8));
            }
    ...
    /* Update counters */
    vlib_node_increment_counter(vm, sr_policy_rewrite_encaps_node.index, SR_POLICY_REWRITE_ERROR_COUNTER_TOTAL, encap_pkts);
    vlib_node_increment_counter(vm, sr_policy_rewrite_encaps_node.index, SR_POLICY_REWRITE_ERROR_COUNTER_BSID, bsid_pkts);
...
static_always_inline void encaps_processing_v4(vlib_node_runtime_t *node, vlib_buffer_t *b0, ip6_header_t *ip0, ip4_header_t *ip0_encap)
{
    ...
    ip0->ip_version_traffic_class_and_flow_label =
        clib_host_to_net_u32(0 | ((6 & 0xF) << 28) | ((ip0_encap->tos & 0xFF) << 20) | (flow_label & 0x0000ffff));
...
always_inline u32 ip_flow_hash(void *data)
{
...
always_inline u64 mac_to_u64(u8 *m)
{
    return (*((u64 *) m) & 0xffffffffffff);
}

always_inline u32 l2_flow_hash(vlib_buffer_t *b0)
{
...
static uword sr_policy_rewrite_encaps_l2(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 8 && n_left_to_next >= 4) {
            ...
            sp0 = pool_elt_at_index(sm->sr_policies, sm->sw_iface_sr_policies[vnet_buffer(b0)->sw_if_index[VLIB_RX]]);

            sp1 = pool_elt_at_index(sm->sr_policies, sm->sw_iface_sr_policies[vnet_buffer(b1)->sw_if_index[VLIB_RX]]);

            sp2 = pool_elt_at_index(sm->sr_policies, sm->sw_iface_sr_policies[vnet_buffer(b2)->sw_if_index[VLIB_RX]]);

            sp3         = pool_elt_at_index(sm->sr_policies, sm->sw_iface_sr_policies[vnet_buffer(b3)->sw_if_index[VLIB_RX]]);
            ...
            if (vec_len(sp2->segments_lists) == 1)
                ...
                vnet_buffer(b2)->ip.adj_index[VLIB_TX] = sp2->segments_lists[(vnet_buffer(b2)->ip.flow_hash & (vec_len(sp2->segments_lists) - 1))];
            }
            ...
            if (vec_len(sp3->segments_lists) == 1)
                ...
                vnet_buffer(b3)->ip.adj_index[VLIB_TX] = sp3->segments_lists[(vnet_buffer(b3)->ip.flow_hash & (vec_len(sp3->segments_lists) - 1))];
            }
            ...
            clib_memcpy_fast(((u8 *) en0) - vec_len(sl0->rewrite), sl0->rewrite, vec_len(sl0->rewrite));
            clib_memcpy_fast(((u8 *) en1) - vec_len(sl1->rewrite), sl1->rewrite, vec_len(sl1->rewrite));
            clib_memcpy_fast(((u8 *) en2) - vec_len(sl2->rewrite), sl2->rewrite, vec_len(sl2->rewrite));
            clib_memcpy_fast(((u8 *) en3) - vec_len(sl3->rewrite), sl3->rewrite, vec_len(sl3->rewrite));
            ...
            ip0->payload_length = clib_host_to_net_u16(b0->current_length - sizeof(ip6_header_t));
            ip1->payload_length = clib_host_to_net_u16(b1->current_length - sizeof(ip6_header_t));
            ip2->payload_length = clib_host_to_net_u16(b2->current_length - sizeof(ip6_header_t));
            ip3->payload_length = clib_host_to_net_u16(b3->current_length - sizeof(ip6_header_t));
            ...
            ip0->ip_version_traffic_class_and_flow_label = clib_host_to_net_u32(0 | ((6 & 0xF) << 28) | ((0x00) << 20) | (flow_label0 & 0xffff));
            ip1->ip_version_traffic_class_and_flow_label = clib_host_to_net_u32(0 | ((6 & 0xF) << 28) | ((0x00) << 20) | (flow_label1 & 0xffff));
            ip2->ip_version_traffic_class_and_flow_label = clib_host_to_net_u32(0 | ((6 & 0xF) << 28) | ((0x00) << 20) | (flow_label2 & 0xffff));
            ip3->ip_version_traffic_class_and_flow_label = clib_host_to_net_u32(0 | ((6 & 0xF) << 28) | ((0x00) << 20) | (flow_label3 & 0xffff));
...
static uword sr_policy_rewrite_insert(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 8 && n_left_to_next >= 4) {
            ...
            if (ip0->protocol == IP_PROTOCOL_IP6_HOP_BY_HOP_OPTIONS)
                sr0 = (ip6_sr_header_t *) (((void *) (ip0 + 1)) + ip6_ext_header_len(ip0 + 1));
            else
                sr0 = (ip6_sr_header_t *) (ip0 + 1);

            if (ip1->protocol == IP_PROTOCOL_IP6_HOP_BY_HOP_OPTIONS)
                sr1 = (ip6_sr_header_t *) (((void *) (ip1 + 1)) + ip6_ext_header_len(ip1 + 1));
            else
                sr1 = (ip6_sr_header_t *) (ip1 + 1);

            if (ip2->protocol == IP_PROTOCOL_IP6_HOP_BY_HOP_OPTIONS)
                sr2 = (ip6_sr_header_t *) (((void *) (ip2 + 1)) + ip6_ext_header_len(ip2 + 1));
            else
                sr2 = (ip6_sr_header_t *) (ip2 + 1);

            if (ip3->protocol == IP_PROTOCOL_IP6_HOP_BY_HOP_OPTIONS)
                sr3 = (ip6_sr_header_t *) (((void *) (ip3 + 1)) + ip6_ext_header_len(ip3 + 1));
            else
                sr3 = (ip6_sr_header_t *) (ip3 + 1);

            clib_memcpy_fast((u8 *) ip0 - vec_len(sl0->rewrite), (u8 *) ip0, (void *) sr0 - (void *) ip0);
            clib_memcpy_fast((u8 *) ip1 - vec_len(sl1->rewrite), (u8 *) ip1, (void *) sr1 - (void *) ip1);
            clib_memcpy_fast((u8 *) ip2 - vec_len(sl2->rewrite), (u8 *) ip2, (void *) sr2 - (void *) ip2);
            clib_memcpy_fast((u8 *) ip3 - vec_len(sl3->rewrite), (u8 *) ip3, (void *) sr3 - (void *) ip3);

            clib_memcpy_fast(((u8 *) sr0 - vec_len(sl0->rewrite)), sl0->rewrite, vec_len(sl0->rewrite));
            clib_memcpy_fast(((u8 *) sr1 - vec_len(sl1->rewrite)), sl1->rewrite, vec_len(sl1->rewrite));
            clib_memcpy_fast(((u8 *) sr2 - vec_len(sl2->rewrite)), sl2->rewrite, vec_len(sl2->rewrite));
            clib_memcpy_fast(((u8 *) sr3 - vec_len(sl3->rewrite)), sl3->rewrite, vec_len(sl3->rewrite));
    ...
    /* Update counters */
    vlib_node_increment_counter(vm, sr_policy_rewrite_insert_node.index, SR_POLICY_REWRITE_ERROR_COUNTER_TOTAL, insert_pkts);
    vlib_node_increment_counter(vm, sr_policy_rewrite_insert_node.index, SR_POLICY_REWRITE_ERROR_COUNTER_BSID, bsid_pkts);
    return from_frame->n_vectors;
}
...
static uword sr_policy_rewrite_b_insert(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 8 && n_left_to_next >= 4) {
            ...
            sl2 = pool_elt_at_index(sm->sid_lists, vnet_buffer(b2)->ip.adj_index[VLIB_TX]);
            sl3 = pool_elt_at_index(sm->sid_lists, vnet_buffer(b3)->ip.adj_index[VLIB_TX]);
            ASSERT(b0->current_data + VLIB_BUFFER_PRE_DATA_SIZE >= vec_len(sl0->rewrite_bsid));
            ASSERT(b1->current_data + VLIB_BUFFER_PRE_DATA_SIZE >= vec_len(sl1->rewrite_bsid));
            ASSERT(b2->current_data + VLIB_BUFFER_PRE_DATA_SIZE >= vec_len(sl2->rewrite_bsid));
            ASSERT(b3->current_data + VLIB_BUFFER_PRE_DATA_SIZE >= vec_len(sl3->rewrite_bsid));
            ...
            clib_memcpy_fast((u8 *) ip0 - vec_len(sl0->rewrite_bsid), (u8 *) ip0, (void *) sr0 - (void *) ip0);
            clib_memcpy_fast((u8 *) ip1 - vec_len(sl1->rewrite_bsid), (u8 *) ip1, (void *) sr1 - (void *) ip1);
            clib_memcpy_fast((u8 *) ip2 - vec_len(sl2->rewrite_bsid), (u8 *) ip2, (void *) sr2 - (void *) ip2);
            clib_memcpy_fast((u8 *) ip3 - vec_len(sl3->rewrite_bsid), (u8 *) ip3, (void *) sr3 - (void *) ip3);

            clib_memcpy_fast(((u8 *) sr0 - vec_len(sl0->rewrite_bsid)), sl0->rewrite_bsid, vec_len(sl0->rewrite_bsid));
            clib_memcpy_fast(((u8 *) sr1 - vec_len(sl1->rewrite_bsid)), sl1->rewrite_bsid, vec_len(sl1->rewrite_bsid));
            clib_memcpy_fast(((u8 *) sr2 - vec_len(sl2->rewrite_bsid)), sl2->rewrite_bsid, vec_len(sl2->rewrite_bsid));
            clib_memcpy_fast(((u8 *) sr3 - vec_len(sl3->rewrite_bsid)), sl3->rewrite_bsid, vec_len(sl3->rewrite_bsid));
            ...
            new_l0 = clib_net_to_host_u16(ip0->payload_length) + vec_len(sl0->rewrite_bsid);
            new_l1 = clib_net_to_host_u16(ip1->payload_length) + vec_len(sl1->rewrite_bsid);
            new_l2 = clib_net_to_host_u16(ip2->payload_length) + vec_len(sl2->rewrite_bsid);
            new_l3 = clib_net_to_host_u16(ip3->payload_length) + vec_len(sl3->rewrite_bsid);
                                                         ...
                                                         u8                   policy_type)
{
    ...
    if (PREDICT_FALSE(!sr0)) goto error_bsid_encaps;
...
static uword sr_policy_rewrite_b_encaps(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 8 && n_left_to_next >= 4) {
            ...
            sr0 = ip6_ext_header_find(vm, b0, ip0_encap, IP_PROTOCOL_IPV6_ROUTE, NULL);
            sr1 = ip6_ext_header_find(vm, b1, ip1_encap, IP_PROTOCOL_IPV6_ROUTE, NULL);
            sr2 = ip6_ext_header_find(vm, b2, ip2_encap, IP_PROTOCOL_IPV6_ROUTE, NULL);
            sr3 = ip6_ext_header_find(vm, b3, ip3_encap, IP_PROTOCOL_IPV6_ROUTE, NULL);

            end_bsid_encaps_srh_processing(node, b0, ip0_encap, sr0, &next0, sl0->policy_type);
            end_bsid_encaps_srh_processing(node, b1, ip1_encap, sr1, &next1, sl1->policy_type);
            end_bsid_encaps_srh_processing(node, b2, ip2_encap, sr2, &next2, sl2->policy_type);
            end_bsid_encaps_srh_processing(node, b3, ip3_encap, sr3, &next3, sl3->policy_type);
                                ...
                                sr_p_plugin_callback_t *removal_fn)
{
    ...
    /* Else create a new one and set hash key */
        ...
        hash_set_mem(sm->policy_plugin_functions_by_key, fn_name, plugin - sm->policy_plugin_functions);
    }
...
clib_error_t *sr_policy_rewrite_init(vlib_main_t *vm)
{
    ...
    mhash_init(&sm->sr_policies_index_hash, sizeof(uword), sizeof(ip6_address_t));

    /* Init SR VPO DPOs type */
    sr_pr_encaps_dpo_type = dpo_register_new_type(&sr_policy_rewrite_vft, sr_pr_encaps_nodes);

    sr_pr_insert_dpo_type = dpo_register_new_type(&sr_policy_rewrite_vft, sr_pr_insert_nodes);

    sr_pr_bsid_encaps_dpo_type = dpo_register_new_type(&sr_policy_rewrite_vft, sr_pr_bsid_encaps_nodes);

    sr_pr_bsid_insert_dpo_type = dpo_register_new_type(&sr_policy_rewrite_vft, sr_pr_bsid_insert_nodes);
