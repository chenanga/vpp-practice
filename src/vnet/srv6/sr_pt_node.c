...
static u8 *format_pt_trace(u8 *s, va_list *args)
{
    ...
    switch (t->behavior) {
        case PT_BEHAVIOR_MID:
            s = format(s,
                       ...
                       t->id,
                       t->load,
                       clib_host_to_net_u32(t->t64.sec),
                       clib_host_to_net_u32(t->t64.nsec),
                       t->tts_template,
                       t->tts);
            break;
        default:
...
static_always_inline void pt_midpoint_processing(vlib_main_t         *vm,
                                                 ...
                                                 timestamp_64_t       t64)
{
    ...
    if (ip0->protocol == IP_PROTOCOL_IP6_HOP_BY_HOP_OPTIONS) {
        ...
        if (hbh_opt->type == IP6_HBH_PT_TYPE) {
            ...
            clib_memcpy_fast(&hbh_opt_pt->cmd_stack[1], &hbh_opt_pt->cmd_stack[0], 33);
            hbh_opt_pt->cmd_stack[0].oif_oil = clib_host_to_net_u16(ls->id << 4);
            hbh_opt_pt->cmd_stack[0].oif_oil |= ls->egress_load;
            switch (ls->tts_template) {
                case SR_PT_TTS_TEMPLATE_0:
                    hbh_opt_pt->cmd_stack[0].tts = t64.nsec >> SR_PT_TTS_SHIFT_TEMPLATE_0;
                    break;
                case SR_PT_TTS_TEMPLATE_1:
                    hbh_opt_pt->cmd_stack[0].tts = t64.nsec >> SR_PT_TTS_SHIFT_TEMPLATE_1;
                    break;
                case SR_PT_TTS_TEMPLATE_2:
                    hbh_opt_pt->cmd_stack[0].tts = t64.nsec >> SR_PT_TTS_SHIFT_TEMPLATE_2;
                    break;
