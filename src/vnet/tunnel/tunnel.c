...
#define _(a, b, c) \
    if (f & TUNNEL_ENCAP_DECAP_FLAG_##a) s = format(s, "%s ", b);
        foreach_tunnel_encap_decap_flag
#undef _
    }
    return (s);
...
#define _(a, b, c) \
    if (f & TUNNEL_FLAG_##a) s = format(s, "%s ", c);
        foreach_tunnel_flag
#undef _
    }
    return (s);
...
u8 *format_tunnel(u8 *s, va_list *args)
{
    ...
    if (t->t_flags & TUNNEL_FLAG_RESOLVED) s = format(s, " [resolved via fib-entry: %d]", t->t_fib_entry_index);
...
uword unformat_tunnel(unformat_input_t *input, va_list *args)
{
    ...
    if (!unformat(input, "tunnel")) return (0);
    ...
    unformat(input, "%U", unformat_tunnel_encap_decap_flags, &t->t_encap_decap_flags);
...
int tunnel_resolve(tunnel_t *t, fib_node_type_t child_type, index_t child_index)
{
    ...
    if (t->t_fib_index == ~((u32) 0)) return VNET_API_ERROR_NO_SUCH_FIB;

    t->t_fib_entry_index = fib_entry_track(t->t_fib_index, &pfx, child_type, child_index, &t->t_sibling);
...
{
    if (t->t_flags & TUNNEL_FLAG_RESOLVED) fib_entry_untrack(t->t_fib_entry_index, t->t_sibling);
...
void tunnel_contribute_forwarding(const tunnel_t *t, dpo_id_t *dpo)
{
    ...
    fct = fib_forw_chain_type_from_fib_proto(ip_address_family_to_fib_proto(ip_addr_version(&t->t_src)));
...
void tunnel_build_v6_hdr(const tunnel_t *t, ip_protocol_t next_proto, ip6_header_t *ip)
{
    ...
    ip6_set_flow_label_network_order(ip, ip6_compute_flow_hash(ip, IP_FLOW_HASH_DEFAULT));
}
