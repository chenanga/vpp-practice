...
{
    if (f & ~TUNNEL_ENCAP_DECAP_FLAG_MASK) /* unknown flags set */
...
{
    if (f & ~TUNNEL_FLAG_MASK) /* unknown flags set */
        return (VNET_API_ERROR_INVALID_VALUE_2);
...
vl_api_tunnel_flags_t tunnel_flags_encode(tunnel_flags_t in)
{
    ...
    if (in & TUNNEL_FLAG_TRACK_MTU) out |= TUNNEL_API_FLAG_TRACK_MTU;
...
int tunnel_decode(const vl_api_tunnel_t *in, tunnel_t *out)
{
    ...
    if (ip_addr_version(&out->t_src) != ip_addr_version(&out->t_dst)) return (VNET_API_ERROR_INVALID_PROTOCOL);

    if (0 == ip_address_cmp(&out->t_src, &out->t_dst)) return (VNET_API_ERROR_SAME_SRC_DST);

    rv = tunnel_encap_decap_flags_decode(in->encap_decap_flags, &out->t_encap_decap_flags);
    ...
    out->t_fib_index = fib_table_find(ip_address_family_to_fib_proto(ip_addr_version(&out->t_dst)), out->t_table_id);

    if (~0 == out->t_fib_index) return (VNET_API_ERROR_NO_SUCH_FIB);
...
void tunnel_encode(const tunnel_t *in, vl_api_tunnel_t *out)
{
    ...
    out->encap_decap_flags = tunnel_encap_decap_flags_encode(in->t_encap_decap_flags);
