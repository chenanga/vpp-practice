...
always_inline uword ipip_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, bool is_ipv6)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (is_ipv6) {
                ...
                /* Check for outer fragmentation */
                if (ip40->flags_and_fragment_offset & clib_host_to_net_u16(IP4_HEADER_FLAG_MORE_FRAGMENTS)) {
                    next0     = IPIP_INPUT_NEXT_DROP;
                    b0->error = node->errors[IPIP_ERROR_FRAGMENTED_PACKET];
                    goto drop;
                }
            ...
            if (inner_protocol0 == IP_PROTOCOL_IPV6) {
                ...
                if (is_ipv6)
                    ...
                    tunnel_decap_fixup_6o4(t0->flags, (ip6_header_t *) (ip40 + 1), ip40);
            }
            else if (inner_protocol0 == IP_PROTOCOL_IP_IN_IP) {
                next0 = IPIP_INPUT_NEXT_IP4_INPUT;

                if (is_ipv6)
                    tunnel_decap_fixup_4o6(t0->flags, (ip4_header_t *) (ip60 + 1), ip60);
                else
                ...
                if (is_ipv6)
                    tunnel_decap_fixup_mplso6(t0->flags, (mpls_unicast_header_t *) (ip60 + 1), ip60);
                else
                    tunnel_decap_fixup_mplso4(t0->flags, (mpls_unicast_header_t *) ip40 + 1, ip40);
            }

            if (!is_ipv6 && t0->mode == IPIP_MODE_6RD && t0->sixrd.security_check) {
                ip6_header_t *inner_ip60 = vlib_buffer_get_current(b0);
                if (sixrd_get_addr_net(t0, inner_ip60->src_address.as_u64[0]) != ip40->src_address.as_u32) {
                    next0     = IPIP_INPUT_NEXT_DROP;
                    b0->error = node->errors[IPIP_ERROR_NO_TUNNEL];
                    goto drop;
                }
        ...
        drop:
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                ipip_rx_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
    ...
    vlib_node_increment_counter(vm, !is_ipv6 ? ipip4_input_node.index : ipip6_input_node.index, IPIP_ERROR_DECAP_PKTS, from_frame->n_vectors);
