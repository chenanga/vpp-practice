...
static u8 *format_ipip_tunnel(u8 *s, va_list *args)
{
    ...
    switch (t->mode) {
        ...
        case IPIP_MODE_P2P:
            ...
            break;
        case IPIP_MODE_P2MP:
            s = format(s, "[%d] instance %d p2mp src %U ", t->dev_instance, t->user_instance, format_ip46_address, &t->tunnel_src, type);
            break;
    }
...
static clib_error_t *show_ipip_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (pool_elts(gm->tunnels) == 0) vlib_cli_output(vm, "No IPIP tunnels configured...");
    ...
    if (ti == ~0) {
        ...
        if (pool_is_free_index(gm->tunnels, ti)) return clib_error_return(0, "unknown index:%d", ti);
        t = pool_elt_at_index(gm->tunnels, ti);
        if (t) vlib_cli_output(vm, "%U", format_ipip_tunnel, t);
    }
    return 0;
}
...
static clib_error_t *ipip_tunnel_hash_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    hash_foreach(key, index, im->tunnel_by_key, ({ vlib_cli_output(vm, " %U -> %d", format_ipip_tunnel_key, key, index); }));
...
static clib_error_t *create_sixrd_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (~0 == ip4_fib_index) {
        ...
        rv = sixrd_add_tunnel(&ip6_prefix,
                              ip6_prefix_len,
                              &ip4_prefix,
                              ip4_prefix_len,
                              &ip4_src,
        ...
        if (rv) error = clib_error_return(0, "adding tunnel failed %d", rv);
    }
