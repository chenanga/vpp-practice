...
u8 *format_ipip_tx_trace(u8 *s, va_list *args)
{
    ...
    s = format(s,
               "IPIP: tunnel %d len %d src %U dst %U",
               t->tunnel_id,
               t->length,
               format_ip46_address,
               &t->src,
...
static u8 *ipip_build_rewrite(vnet_main_t *vnm, u32 sw_if_index, vnet_link_t link_type, const void *dst_address)
{
    ...
    switch (t->transport) {
        case IPIP_TRANSPORT_IP4:
            ...
            if (!(t->flags & TUNNEL_ENCAP_DECAP_FLAG_ENCAP_COPY_DSCP)) ip4_header_set_dscp(ip4, t->dscp);
            if (t->flags & TUNNEL_ENCAP_DECAP_FLAG_ENCAP_SET_DF) ip4_header_set_df(ip4);
        ...
        case IPIP_TRANSPORT_IP6:
            ...
            if (!(t->flags & TUNNEL_ENCAP_DECAP_FLAG_ENCAP_COPY_DSCP)) ip6_set_dscp_network_order(ip6, t->dscp);
...
static void ipip64_fixup(vlib_main_t *vm, const ip_adjacency_t *adj, vlib_buffer_t *b, const void *data)
{
    ...
    if (PREDICT_FALSE(b->flags & VNET_BUFFER_F_GSO)) {
        ...
        vnet_buffer_offload_flags_set(b, VNET_BUFFER_OFFLOAD_F_OUTER_IP_CKSUM | VNET_BUFFER_OFFLOAD_F_TNL_IPIP);
    }
    else
        ip4->checksum = ip4_header_checksum(ip4);
}
...
static void ipip46_fixup(vlib_main_t *vm, const ip_adjacency_t *adj, vlib_buffer_t *b, const void *data)
{
    ...
    ip6->payload_length = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, b) - sizeof(*ip6));
...
static void ipipm4_fixup(vlib_main_t *vm, const ip_adjacency_t *adj, vlib_buffer_t *b, const void *data)
{
    ...
    ip4         = vlib_buffer_get_current(b);
    ip4->length = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, b) - sizeof(*ip4));
...
static void ipip_tunnel_stack(adj_index_t ai)
{
    ...
    if ((vnet_hw_interface_get_flags(vnet_get_main(), t->hw_if_index) & VNET_HW_INTERFACE_FLAG_LINK_UP) == 0) {
...
{
    if (t->transport == IPIP_TRANSPORT_IP6 && lt == VNET_LINK_IP6) return (ipip66_fixup);
    if (t->transport == IPIP_TRANSPORT_IP6 && lt == VNET_LINK_IP4) return (ipip46_fixup);
    if (t->transport == IPIP_TRANSPORT_IP6 && lt == VNET_LINK_MPLS) return (ipipm6_fixup);
    if (t->transport == IPIP_TRANSPORT_IP4 && lt == VNET_LINK_IP6) return (ipip64_fixup);
    if (t->transport == IPIP_TRANSPORT_IP4 && lt == VNET_LINK_MPLS) return (ipipm4_fixup);
...
void ipip_update_adj(vnet_main_t *vnm, u32 sw_if_index, adj_index_t ai)
{
     ...
     * a path.
     */
    if (!(t->flags & TUNNEL_ENCAP_DECAP_FLAG_ENCAP_INNER_HASH)) af |= ADJ_FLAG_MIDCHAIN_IP_STACK;

    fixup = ipip_get_fixup(t, adj_get_link_type(ai), &af);
    adj_nbr_midchain_update_rewrite(ai,
                                    fixup,
                                    uword_to_pointer(t->flags, void *),
                                    af,
                                    ipip_build_rewrite(vnm, sw_if_index, adj_get_link_type(ai), &t->tunnel_dst));
    ipip_tunnel_stack(ai);
}
...
static adj_walk_rc_t mipip_mk_complete_walk(adj_index_t ai, void *data)
{
     ...
     * a path.
     */
    if (!(ctx->t->flags & TUNNEL_ENCAP_DECAP_FLAG_ENCAP_INNER_HASH)) af |= ADJ_FLAG_MIDCHAIN_IP_STACK;
    ...
    adj_nbr_midchain_update_rewrite(
        ...
        af,
        ipip_build_rewrite(vnet_get_main(), ctx->t->sw_if_index, adj_get_link_type(ai), &teib_entry_get_nh(ctx->ne)->fp_addr));
...
void mipip_update_adj(vnet_main_t *vnm, u32 sw_if_index, adj_index_t ai)
{
    ...
    ne = teib_entry_find_46(sw_if_index, adj->ia_nh_proto, &adj->sub_type.nbr.next_hop);

    if (NULL == ne) {
        // no TEIB entry to provide the next-hop
        fixup = ipip_get_fixup(t, adj_get_link_type(ai), &af);
        adj_nbr_midchain_update_rewrite(ai, fixup, uword_to_pointer(t->flags, void *), ADJ_FLAG_NONE, NULL);
        return;
    }

    mipip_walk_ctx_t ctx = {.t = t, .ne = ne};
    adj_nbr_walk_nh(sw_if_index, adj->ia_nh_proto, &adj->sub_type.nbr.next_hop, mipip_mk_complete_walk, &ctx);
}
...
static u8 *format_ipip_tunnel_name(u8 *s, va_list *args)
{
    ...
    if (dev_instance >= vec_len(gm->tunnels)) return format(s, "<improperly-referenced>");
...
VNET_DEVICE_CLASS(ipip_device_class) = {.name                   = "IPIP tunnel device",
...
{
    ipip_main_t *gm = &ipip_main;
    if (vec_len(gm->tunnel_index_by_sw_if_index) <= sw_if_index) return NULL;
...
{
    ipip_mk_key_i(t->transport, t->mode, &t->tunnel_src, &t->tunnel_dst, t->fib_index, key);
}
...
static void ipip_teib_mk_key(const ipip_tunnel_t *t, const teib_entry_t *ne, ipip_tunnel_key_t *key)
{
    ...
    ipip_mk_key_i(t->transport, IPIP_MODE_P2P, &t->tunnel_src, &nh->fp_addr, teib_entry_get_fib_index(ne), key);
}
...
static void ipip_teib_entry_added(const teib_entry_t *ne)
{
    ...
    if (vec_len(gm->tunnel_index_by_sw_if_index) < sw_if_index) return;

    t_idx = gm->tunnel_index_by_sw_if_index[sw_if_index];

    if (INDEX_INVALID == t_idx) return;
                    ...
                    u32                       *sw_if_indexp)
{
    ...
    if (tmode == TUNNEL_MODE_MP && !ip46_address_is_zero(dst)) return (VNET_API_ERROR_INVALID_DST_ADDRESS);
    ...
    if (t) {
        if (sw_if_indexp) sw_if_indexp[0] = t->sw_if_index;
    ...
    hw_if_index = vnet_register_interface(vnm,
                                          ipip_device_class.index,
                                          t_idx,
                                          (mode == IPIP_MODE_P2P ? ipip_hw_interface_class.index : mipip_hw_interface_class.index),
                                          t_idx);
    ...
    vnet_set_interface_l3_output_node(gm->vlib_main, sw_if_index, (u8 *) "tunnel-output");
    ...
    if (t->mode == IPIP_MODE_P2MP) teib_walk_itf(t->sw_if_index, ipip_tunnel_add_teib_walk, t);
...
int ipip_del_tunnel(u32 sw_if_index)
{
    ...
    t = ipip_tunnel_db_find_by_sw_if_index(sw_if_index);
    if (t == NULL) return VNET_API_ERROR_NO_SUCH_ENTRY;

    if (t->mode == IPIP_MODE_P2MP) teib_walk_itf(t->sw_if_index, ipip_tunnel_delete_teib_walk, t);
