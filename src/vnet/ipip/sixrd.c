...
static inline sixrd_adj_delegate_t *sixrd_adj_from_base(adj_delegate_t *ad)
{
    if (ad == NULL) return (NULL);
    return (pool_elt_at_index(sixrd_adj_delegate_pool, ad->ad_index));
}

static inline const sixrd_adj_delegate_t *sixrd_adj_from_const_base(const adj_delegate_t *ad)
{
    if (ad == NULL) {
        return (NULL);
    }
...
static void ip6ip_fixup(vlib_main_t *vm, const ip_adjacency_t *adj, vlib_buffer_t *b0, const void *data)
{
    ...
    ip4_header_t        *ip4 = vlib_buffer_get_current(b0);
    ip4->length              = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, b0));
    ip4->dst_address.as_u32  = sixrd_get_addr_net(t, adj->sub_type.nbr.next_hop.as_u64[0]);
    ip4->checksum            = ip4_header_checksum(ip4);
}
...
static void ip6ip_tunnel_stack(adj_index_t ai, u32 fib_entry_index)
{
     ...
     * in the midchain
     */
    if (vnet_hw_interface_get_flags(vnet_get_main(), t->hw_if_index) & VNET_HW_INTERFACE_FLAG_LINK_UP) {
        adj_nbr_midchain_stack_on_fib_entry(ai, fib_entry_index, FIB_FORW_CHAIN_TYPE_UNICAST_IP4);
    }
    else {
...
static void sixrd_tunnel_stack(adj_index_t ai, u32 fib_index)
{
    ...
    lookup_dpo_add_or_lock_w_fib_index(fib_index, DPO_PROTO_IP4, LOOKUP_UNICAST, LOOKUP_INPUT_DST_ADDR, LOOKUP_TABLE_FROM_CONFIG, &dpo);
...
static void sixrd_update_adj(vnet_main_t *vnm, u32 sw_if_index, adj_index_t ai)
{
    ...
    if (IP_LOOKUP_NEXT_BCAST == adj->lookup_next_index) {
        adj_nbr_midchain_update_rewrite(ai, sixrd_fixup, t, ADJ_FLAG_NONE, sixrd_build_rewrite(vnm, sw_if_index, adj_get_link_type(ai), NULL));
        ...
        da4.as_u32 = sixrd_get_addr_net(t, adj->sub_type.nbr.next_hop.as_u64[0]);
        ...
        adj_nbr_midchain_update_rewrite(ai, ip6ip_fixup, t, ADJ_FLAG_NONE, sixrd_build_rewrite(vnm, sw_if_index, adj_get_link_type(ai), NULL));

        sixrd_ad = sixrd_adj_from_base(adj_delegate_get(adj, sixrd_adj_delegate_type));
        if (sixrd_ad == NULL) {
            ...
            sixrd_ad->sixrd_fib_entry_index =
                fib_entry_track(t->fib_index, &pfx, sixrd_fib_node_type, sixrd_ad - sixrd_adj_delegate_pool, &sixrd_ad->sixrd_sibling);
...
VNET_DEVICE_CLASS(sixrd_device_class) = {.name                   = "ip6ip-6rd",
                     ...
                     u32           *sw_if_index)
{
    ...
    if ((ip6_prefix_len + 32 - ip4_prefix_len) > 64) return VNET_API_ERROR_INVALID_VALUE;
    ...
    ipip_mk_key_i(IPIP_TRANSPORT_IP4, IPIP_MODE_6RD, &src, &dst, ip4_fib_index, &key);

    t = ipip_tunnel_db_find(&key);
    if (t) return VNET_API_ERROR_IF_ALREADY_EXISTS;
    ...
    t->sixrd.shift             = (ip4_prefix_len < 32) ? 64 - ip6_prefix_len - (32 - ip4_prefix_len) : 0;

    /* Create interface */
    u32 hw_if_index = vnet_register_interface(vnet_get_main(), sixrd_device_class.index, t_idx, sixrd_hw_interface_class.index, t_idx);
    ...
    vnet_set_interface_l3_output_node(gm->vlib_main, hi->sw_if_index, (u8 *) "tunnel-output");

    ipip_tunnel_db_add(t, &key);

    vec_validate_init_empty(gm->tunnel_index_by_sw_if_index, hi->sw_if_index, ~0);
    gm->tunnel_index_by_sw_if_index[hi->sw_if_index] = t_idx;

    vnet_hw_interface_set_flags(vnet_get_main(), hw_if_index, VNET_HW_INTERFACE_FLAG_LINK_UP);
    vnet_sw_interface_set_flags(vnet_get_main(), hi->sw_if_index, VNET_SW_INTERFACE_FLAG_ADMIN_UP);
    ...
    fib_table_entry_update_one_path(ip6_fib_index,
                                    &pfx6,
                                    FIB_SOURCE_6RD,
    ...
    *sw_if_index = t->sw_if_index;
    ...
    if (!gm->ip4_protocol_registered) {
        vlib_node_t *ipip4_input = vlib_get_node_by_name(gm->vlib_main, (u8 *) "ipip4-input");
        ASSERT(ipip4_input);
...
int sixrd_del_tunnel(u32 sw_if_index)
{
    ...
    if (!t) {
        clib_warning("SIXRD tunnel delete: tunnel does not exist: %d", sw_if_index);
        return -1;
    }
    ...
    fib_table_entry_path_remove(t->sixrd.ip6_fib_index,
                                &pfx6,
                                FIB_SOURCE_6RD,
                                DPO_PROTO_IP6,
                                &ADJ_BCAST_ADDR,
...
{
    return ((sixrd_adj_delegate_t *) (((char *) node) - STRUCT_OFFSET_OF(sixrd_adj_delegate_t, sixrd_node)));
}
...
static clib_error_t *sixrd_init(vlib_main_t *vm)
{
    ...
    sixrd_adj_delegate_type = adj_delegate_register_new_type(&sixrd_adj_delegate_vft);
    sixrd_fib_node_type     = fib_node_register_new_type("sixrd", &sixrd_fib_node_vft);
