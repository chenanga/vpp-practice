...
static void vl_api_ipip_add_tunnel_t_handler(vl_api_ipip_add_tunnel_t *mp)
{
    ...
    fib_index = fib_table_find(fib_proto_from_ip46(itype[0]), ntohl(mp->tunnel.table_id));

    if (~0 == fib_index) {
        rv = VNET_API_ERROR_NO_SUCH_FIB;
    }
    else {
        rv = ipip_add_tunnel((itype[0] == IP46_TYPE_IP6 ? IPIP_TRANSPORT_IP6 : IPIP_TRANSPORT_IP4),
                             ntohl(mp->tunnel.instance),
                             &src,
                             &dst,
                             fib_index,
                             flags,
                             ip_dscp_decode(mp->tunnel.dscp),
                             mode,
                             &sw_if_index);
    }

out:
    REPLY_MACRO2(VL_API_IPIP_ADD_TUNNEL_REPLY, ({ rmp->sw_if_index = ntohl(sw_if_index); }));
}
...
static vl_api_tunnel_mode_t ipip_tunnel_mode_encode(ipip_mode_t mode)
{
    switch (mode) {
        case IPIP_MODE_P2P:
...
static void send_ipip_tunnel_details(ipip_tunnel_t *t, vl_api_ipip_tunnel_dump_t *mp)
{
    ...
    ft = fib_table_get(t->fib_index, (is_ipv6 ? FIB_PROTOCOL_IP6 : FIB_PROTOCOL_IP4));
...
static void vl_api_ipip_tunnel_dump_t_handler(vl_api_ipip_tunnel_dump_t *mp)
{
    ...
    if (sw_if_index == ~0) {
        ...
        if (t) send_ipip_tunnel_details(t, mp);
    }
}
...
static void vl_api_ipip_6rd_add_tunnel_t_handler(vl_api_ipip_6rd_add_tunnel_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_IPIP_6RD_ADD_TUNNEL_REPLY, ({ rmp->sw_if_index = htonl(sixrd_tunnel_index); }));
}
