...
{
    if (enable) {
        vnet_feature_enable_disable("ip6-unicast", "ip6-unicast-nd-proxy", sw_if_index, 1, NULL, 0);
        vnet_feature_enable_disable("ip6-multicast", "ip6-multicast-nd-proxy", sw_if_index, 1, NULL, 0);
    }
    else {
        vnet_feature_enable_disable("ip6-unicast", "ip6-unicast-nd-proxy", sw_if_index, 0, NULL, 0);
        vnet_feature_enable_disable("ip6-multicast", "ip6-multicast-nd-proxy", sw_if_index, 0, NULL, 0);
    }
    return 0;
}
...
static clib_error_t *set_int_ip6_nd_proxy_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (~0 == sw_if_index) return clib_error_return(0, "unknown input '%U'", format_unformat_error, input);
...
static u8 *format_ip6_nd_proxy_trace(u8 *s, va_list *args)
{
    ...
    if (t->is_multicast)
        s = format(s, "%U %U multicast ", format_white_space, indent, format_vnet_sw_if_index_name, vnm, t->sw_if_index);
    else
        s = format(s, "%U %U unicast ", format_white_space, indent, format_vnet_sw_if_index_name, vnm, t->sw_if_index);
            ...
            if (~0 == fib_index) {
                *next0 = ICMP6_NEIGHBOR_SOLICITATION_NEXT_DROP;
                ...
                if (ip6_address_is_link_local_unicast(&ip6->dst_address)) {
                    fei = ip6_fib_table_lookup_exact_match(ip6_ll_fib_get(sw_if_index0), &ip6->dst_address, 128);
                }
                else {
                ...
                if (FIB_NODE_INDEX_INVALID != fei) {
                    *next0 = ICMP6_NEIGHBOR_SOLICITATION_NEXT_REPLY;
                    icmp6_send_neighbor_advertisement(vm, b0, ip6, icmp6_nsa, icmp6_nd_ell_addr, sw_if_index0);
                }
            }
    ...
    if (PREDICT_FALSE(ip6->protocol == IP_PROTOCOL_ICMP6)) {
        ...
        if (type0 == ICMP6_neighbor_solicitation || type0 == ICMP6_neighbor_advertisement) {
            ...
            if (type0 == ICMP6_neighbor_solicitation) {
                if ((icmp6_nd_ell_addr->header.type == ICMP6_NEIGHBOR_DISCOVERY_OPTION_source_link_layer_address)
                    && (!ip6_address_is_unspecified(&ip6->src_address)) && (!ip6_address_is_link_local_unicast(&ip6->src_address))) {
                    ip_neighbor_learn_t learn = {.sw_if_index = sw_if_index0,
                                                 .ip          = {
                                                              .version = AF_IP6,
                                                              .ip.ip6  = ip6->src_address,
                                                 }};
                    clib_memcpy(&learn.mac, icmp6_nd_ell_addr->ethernet_address, sizeof(learn.mac));
            ...
            {
                icmp6_neighbor_solicitation_or_advertisement_header_t         *icmp6_nsa         = (void *) icmp0;
                icmp6_neighbor_discovery_ethernet_link_layer_address_option_t *icmp6_nd_ell_addr = (void *) (icmp6_nsa + 1);
                if ((icmp6_nd_ell_addr->header.type == ICMP6_NEIGHBOR_DISCOVERY_OPTION_target_link_layer_address)
                    && (!ip6_address_is_unspecified(&ip6->src_address)) && (!ip6_address_is_link_local_unicast(&ip6->src_address))) {
                    ip_neighbor_learn_t learn = {.sw_if_index = sw_if_index0,
                                                 .ip          = {
                                                              .version = AF_IP6,
