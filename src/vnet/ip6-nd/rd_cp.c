...
static int add_slaac_address(vlib_main_t *vm, u32 sw_if_index, u8 address_length, const ip6_address_t *address, f64 due_time)
{
    ...
    rv = ip6_add_del_interface_address(vm, sw_if_index, &slaac_address->address, address_length, 0);
...
static void add_default_route(vlib_main_t *vm, u32 sw_if_index, const ip6_address_t *next_hop_address, f64 due_time)
{
    ...
    {
        u32          fib_index = fib_table_get_index_for_sw_if_index(FIB_PROTOCOL_IP6, default_route->sw_if_index);
        fib_prefix_t pfx       = {
...
static int remove_slaac_address(vlib_main_t *vm, slaac_address_t *slaac_address)
{
    ...
    rv = ip6_add_del_interface_address(vm, slaac_address->sw_if_index, &slaac_address->address, slaac_address->address_length, 1);
...
static void remove_default_route(vlib_main_t *vm, default_route_t *default_route)
{
...

    {
        ...
        fib_table_entry_path_remove(fib_index, &pfx, FIB_SOURCE_API, DPO_PROTO_IP6, &nh, default_route->sw_if_index, 0, 1, FIB_ROUTE_PATH_FLAG_NONE);
    }
...
static u8 ip6_prefixes_equal(ip6_address_t *prefix1, ip6_address_t *prefix2, u8 len)
{
    if (len >= 64) {
        ...
        if (len == 64) return 1;
        return prefix1->as_u64[1] >> (128 - len) == prefix2->as_u64[1] >> (128 - len);
    }
...
static void ip6_ra_report_handler(const ip6_ra_report_t *r)
{
    ...
    if (sw_if_index >= vec_len(rm->config_by_sw_if_index)) return;
    ...
    if (if_config->install_default_routes) {
        ...
        pool_foreach (default_route, rm->default_route_pool) {
            if (default_route->sw_if_index != sw_if_index)
                ;
            else if (0 != memcmp(&default_route->router_address, &r->router_address, 16))
                ;
            else {
    ...
    default_route_pool_foreach_out:
        ...
        if (!route_already_present) {
            if (router_lifetime_in_sec != 0) add_default_route(vm, sw_if_index, &r->router_address, current_time + router_lifetime_in_sec);
        }
        else {
    ...
    for (i = 0; i < n_prefixes; i++) {
        ...
        if (!(prefix->flags & PREFIX_FLAG_A)) continue;

        dst_address   = &prefix->prefix.fp_addr.ip6;
        prefix_length = prefix->prefix.fp_len;

        if (ip6_address_is_link_local_unicast(dst_address)) continue;
        ...
        pool_foreach (slaac_address, rm->slaac_address_pool) {
            if (slaac_address->sw_if_index != sw_if_index)
                ...
                ;
            else if (!ip6_prefixes_equal(&slaac_address->address, dst_address, prefix_length))
                ;
            else {
        ...
        do {
            ...
            pool_foreach_index (index, rm->slaac_address_pool) {
                ...
                if (slaac_address->due_time > current_time) {
                    if (slaac_address->due_time < due_time) due_time = slaac_address->due_time;
                }
                else {
            ...
            pool_foreach_index (index, rm->default_route_pool) {
                ...
                if (default_route->due_time > current_time) {
                    if (default_route->due_time < due_time) due_time = default_route->due_time;
                }
                else
                    remove_default_route(vm, default_route);
            }
...
static void interrupt_process(void)
{
    ...
    vlib_process_signal_event(vm, rd_cp_process_node.index, RD_CP_EVENT_INTERRUPT, 0);
}
...
int rd_cp_set_address_autoconfig(u32 sw_if_index, u8 enable, u8 install_default_routes)
{
    ...
    if (!enable) install_default_routes = 0;
    ...
    vec_validate_init_empty(rm->config_by_sw_if_index, sw_if_index, empty_config);
    if_config = &rm->config_by_sw_if_index[sw_if_index];

    if (!if_config->enabled && enable) ip6_link_enable(sw_if_index, NULL);

    if ((!if_config->enabled && enable) || (!if_config->install_default_routes && install_default_routes))
...
static clib_error_t *ip6_nd_address_autoconfig(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "%U", unformat_vnet_sw_interface, vnm, &sw_if_index))
            ;
        if (unformat(input, "default-route")) default_route = 1;
    ...
    if (sw_if_index != ~0) {
        if (rd_cp_set_address_autoconfig(sw_if_index, enable, default_route) != 0) error = clib_error_return(0, "Invalid sw_if_index");
    }
    else
