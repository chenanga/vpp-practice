                                                                        ...
                                                                        uword                is_solicitation)
{
    ...
    option_type =
        (is_solicitation ? ICMP6_NEIGHBOR_DISCOVERY_OPTION_source_link_layer_address : ICMP6_NEIGHBOR_DISCOVERY_OPTION_target_link_layer_address);
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (!ip6_sadd_unspecified && !ip6_sadd_link_local) {
                ...
                if (ADJ_INDEX_INVALID != src_adj_index0) {
                    ...
                    is_rewrite0 = (ni0 >= IP_LOOKUP_NEXT_ARP) && (ni0 < IP6_LOOKUP_N_NEXT);

                    error0 =
                        ((adj0->rewrite_header.sw_if_index != sw_if_index0 || !is_rewrite0) ? ICMP6_ERROR_NEIGHBOR_SOLICITATION_SOURCE_NOT_ON_LINK
                                                                                            : error0);
                }
                else {
            ...
            if (PREDICT_TRUE(error0 == ICMP6_ERROR_NONE && o0 != 0 && !ip6_sadd_unspecified)) {
                ip_neighbor_learn_t learn = {.sw_if_index = sw_if_index0,
                                             .ip          = {
                                                          .version = AF_IP6,
                                                          .ip.ip6  = (is_solicitation ? ip0->src_address : h0->target_address),
                                             }};
            ...
            if (is_solicitation && error0 == ICMP6_ERROR_NONE) {
                ...
                if (~0 == fib_index) {
                    ...
                    if (ip6_address_is_link_local_unicast(&h0->target_address)) {
                        fei = ip6_fib_table_lookup_exact_match(ip6_ll_fib_get(sw_if_index0), &h0->target_address, 128);
                    }
                    else {
                        fei = ip6_fib_table_lookup_exact_match(fib_index, &h0->target_address, 128);
                    }
                    ...
                    if (FIB_NODE_INDEX_INVALID == fei) {
                        ...
                        if (FIB_ENTRY_FLAG_LOCAL & fib_entry_get_flags_for_source(fei, FIB_SOURCE_INTERFACE)) {
                            /* It's an address that belongs to one of our interfaces
                             * that's good. */
                        }
                        else if (FIB_ENTRY_FLAG_LOCAL & fib_entry_get_flags_for_source(fei, FIB_SOURCE_IP6_ND)) {
                            /* It's one of our link local addresses
                             * that's good. */
                        }
                        else if (fib_entry_is_sourced(fei, FIB_SOURCE_IP6_ND_PROXY)) {
            ...
            vlib_increment_simple_counter(&ip_neighbor_counters[AF_IP6].ipnc[VLIB_RX][c_type], vm->thread_index, sw_if_index0, 1);

            if (is_solicitation && error0 == ICMP6_ERROR_NONE) {
                icmp6_send_neighbor_advertisement(vm, p0, ip0, h0, o0, sw_if_index0);
...
static void ip6_nd_link_enable(u32 sw_if_index)
{
    ...
    ASSERT(INDEX_INVALID == ip6_link_delegate_get(sw_if_index, ip6_nd_delegate_id));

    pool_get_zero(ip6_nd_pool, ind);

    ind->sw_if_index = sw_if_index;

    ip6_link_delegate_update(sw_if_index, ip6_nd_delegate_id, ind - ip6_nd_pool);
}
...
{
    return icmp6_neighbor_solicitation_or_advertisement(vm,
                                                        node,
                                                        frame,
                                                        /* is_solicitation */
                                                        1);
