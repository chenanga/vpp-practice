...
static void vl_api_ip6nd_proxy_dump_t_handler(vl_api_ip6nd_proxy_dump_t *mp)
{
    ...
    pool_foreach_index (fib_index, im6->fibs) {
        fib_table_walk(fib_index, FIB_PROTOCOL_IP6, api_ip6nd_proxy_fib_table_walk, &ctx);
    }
    ...
    vec_foreach (feip, ctx.indices) {
        ...
        send_ip6nd_proxy_details(reg, mp->context, &pfx->fp_addr, fib_entry_get_resolving_interface(*feip));
    }
...
static void ip6_radv_prefix_encode(f64 now, const ip6_radv_prefix_t *in, vl_api_ip6nd_ra_prefix_t *out)
{
    ...
    if (in->adv_valid_lifetime_in_secs != ~0) {
        out->valid_lifetime_expires = clib_host_to_net_f64(in->valid_lifetime_expires - now);
    }

    if (in->adv_pref_lifetime_in_secs != ~0) {
        out->pref_lifetime_expires = clib_host_to_net_f64(in->pref_lifetime_expires - now);
    }
...
static void send_sw_interface_ip6nd_ra_details(vl_api_registration_t *reg, u32 context, ip6_ra_t *radv_info)
{
    ...
    rmp->_vl_msg_id = ntohs(VL_API_SW_INTERFACE_IP6ND_RA_DETAILS + REPLY_MSG_ID_BASE);
    rmp->context    = context;
    ...
    rmp->adv_neighbor_reachable_time = htonl(radv_info->adv_neighbor_reachable_time_in_msec);
    rmp->adv_retransmit_interval     = htonl(radv_info->adv_time_in_msec_between_retransmitted_neighbor_solicitations);
    ...
    if (radv_info->last_radv_time > 0.0) {
        rmp->last_radv_time = clib_host_to_net_f64(now - radv_info->last_radv_time);
    }

    if ((radv_info->next_multicast_time - radv_info->last_multicast_time) > 0.0) {
        rmp->last_multicast_time = clib_host_to_net_f64(now - radv_info->last_multicast_time);
        rmp->next_multicast_time = clib_host_to_net_f64(radv_info->next_multicast_time - now);
    }

    rmp->initial_adverts_count    = htonl(radv_info->initial_adverts_count);
    rmp->initial_adverts_interval = clib_host_to_net_f64(radv_info->initial_adverts_interval);
...
static walk_rc_t api_dump_ip6_ra_itf_walk_fn(u32 sw_if_index, void *arg)
{
...
static void vl_api_sw_interface_ip6nd_ra_dump_t_handler(vl_api_sw_interface_ip6nd_ra_dump_t *mp)
{
    vl_api_registration_t *reg;
    u32                    sw_if_index;
    ip6_ra_t              *radv_info;
...
static void vl_api_ip6nd_send_router_solicitation_t_handler(vl_api_ip6nd_send_router_solicitation_t *mp)
{
    ...
    icmp6_send_router_solicitation(vm, ntohl(mp->sw_if_index), mp->stop, &params);
}
...
static void ip6_ra_handle_report(const ip6_ra_report_t *rap)
{
    ...
    pool_foreach (rp, vpe_api_main.ip6_ra_events_registrations) {
        ...
        if (vl_reg && vl_api_can_send_msg(vl_reg)) {
            ...
            u32 event_size = (sizeof(vl_api_ip6_ra_event_t) + vec_len(rap->prefixes) * sizeof(vl_api_ip6_ra_prefix_info_t));
            ...
            event->router_lifetime_in_sec          = clib_host_to_net_u16(rap->router_lifetime_in_sec);
            event->neighbor_reachable_time_in_msec = clib_host_to_net_u32(rap->neighbor_reachable_time_in_msec);
            ...
            for (j = 0; j < vec_len(rap->prefixes); j++) {
                ...
                prefix->preferred_time = clib_host_to_net_u32(info->preferred_time);
                prefix++;
            }
...
static void vl_api_want_ip6_ra_events_t_handler(vl_api_want_ip6_ra_events_t *mp)
{
    ...
    hash_set(am->ip6_ra_events_registration_hash, rp->client_index, rp - am->ip6_ra_events_registrations);
