...
static inline ip6_mld_t *ip6_mld_get_itf(u32 sw_if_index)
{
    index_t imi;

    imi = ip6_link_delegate_get(sw_if_index, ip6_mld_delegate_id);

    if (INDEX_INVALID != imi) return (pool_elt_at_index(ip6_mld_pool, imi));
...
static void ip6_neighbor_add_mld_prefix(ip6_mld_t *imd, ip6_address_t *addr)
{
    ...
    if (!mcast_group_info) {
        ...
        mhash_set(&imd->address_to_mldp_index,
                  addr,
                  mi, /* old_value */
                  0);
...
static void ip6_neighbor_del_mld_prefix(ip6_mld_t *imd, ip6_address_t *addr)
{
    ...
    if (mcast_group_info) {
        mhash_unset(&imd->address_to_mldp_index,
                    addr,
                    /* old_value */ 0);
...
static void ip6_mld_link_enable(u32 sw_if_index)
{
    ...
    ASSERT(INDEX_INVALID == ip6_link_delegate_get(sw_if_index, ip6_mld_delegate_id));

    pool_get_zero(ip6_mld_pool, imd);

    imd->sw_if_index = sw_if_index;

    mhash_init(&imd->address_to_mldp_index, sizeof(uword), sizeof(ip6_address_t));

    /* add multicast groups we will always be reporting  */
    ip6_neighbor_add_mld_grp(imd, IP6_MULTICAST_SCOPE_link_local, IP6_MULTICAST_GROUP_ID_all_hosts);
    ip6_neighbor_add_mld_grp(imd, IP6_MULTICAST_SCOPE_link_local, IP6_MULTICAST_GROUP_ID_all_routers);
    ip6_neighbor_add_mld_grp(imd, IP6_MULTICAST_SCOPE_link_local, IP6_MULTICAST_GROUP_ID_mldv2_routers);

    ip6_link_delegate_update(sw_if_index, ip6_mld_delegate_id, imd - ip6_mld_pool);
}
...
static void ip6_mld_delegate_disable(index_t imdi)
{
    ...
    /* clean MLD pools */
    pool_flush(m, imd->mldp_group_pool, ({ mhash_unset(&imd->address_to_mldp_index, &m->mcast_address, 0); }));
...
static void ip6_neighbor_send_mldpv2_report(u32 sw_if_index)
{
    ...
    if (!vnet_sw_interface_is_admin_up(vnm, sw_if_index)) return;
    ...
    ip6_address_copy(&ip0->src_address, ip6_get_link_local_address(sw_if_index));

    /* destination is all mldpv2 routers */
    ip6_set_reserved_multicast_address(&ip0->dst_address, IP6_MULTICAST_SCOPE_link_local, IP6_MULTICAST_GROUP_ID_mldv2_routers);
    ...
    pool_foreach (m, imd->mldp_group_pool) {
        ...
        if (vlib_buffer_add_data(vm, &bo0, (void *) &rr, sizeof(icmp6_multicast_address_record_t))) {
    ...
    rh0->icmp.checksum = ip6_tcp_udp_icmp_compute_checksum(vm, b0, ip0, &bogus_length);
     ...
     */
    vnet_buffer(b0)->sw_if_index[VLIB_RX] = vnet_main.local_interface_sw_if_index;
...
static u8 *format_ip6_mld(u8 *s, va_list *args)
{
    ...
    pool_foreach (m, imd->mldp_group_pool) {
        s = format(s, "%U%U\n", format_white_space, indent + 2, format_ip6_address, &m->mcast_address);
    }
