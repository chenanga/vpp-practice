...
static int api_want_ip6_ra_events(vat_main_t *vam)
{
    return -1;
}

static int api_ip6nd_send_router_solicitation(vat_main_t *vam)
{
    return -1;
}

static int api_ip6nd_proxy_add_del(vat_main_t *vam)
{
...
static int api_ip6nd_proxy_dump(vat_main_t *vam)
{
...
static void vl_api_ip6nd_proxy_details_t_handler(vl_api_ip6nd_proxy_details_t *mp)
{
    vat_main_t *vam = &vat_main;

    print(vam->ofp, "host %U sw_if_index %d", format_vl_api_ip6_address, mp->ip, ntohl(mp->sw_if_index));
}
...
static int api_ip6nd_proxy_enable_disable(vat_main_t *vam)
{
    // not yet implemented
    return -1;
}

static int api_sw_interface_ip6nd_ra_dump(vat_main_t *vam)
{
    unformat_input_t                    *i = vam->input;
...
static void vl_api_sw_interface_ip6nd_ra_details_t_handler(vl_api_sw_interface_ip6nd_ra_details_t *mp)
{
    ...
    /* Print it */
    print(vam->ofp, "sw_if_index: %u, send_radv: %s", sw_if_index, (send_radv ? "on" : "off"));
}
