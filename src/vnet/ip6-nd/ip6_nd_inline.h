                                                            ...
                                                            u32                                                            sw_if_index0)
{
    ...
    if (!ip6_address_is_unspecified(&ip6_h->src_address))
        ...
        ip6_set_reserved_multicast_address(&ip6_h->dst_address, IP6_MULTICAST_SCOPE_link_local, IP6_MULTICAST_GROUP_ID_all_hosts);
    ...
    if (eth_if && icmp6_nd_ell_addr) {
        ...
        icmp6_nd_ell_addr->header.type = ICMP6_NEIGHBOR_DISCOVERY_OPTION_target_link_layer_address;
    }

    icmp6_nsa->advertisement_flags = clib_host_to_net_u32(ICMP6_NEIGHBOR_ADVERTISEMENT_FLAG_SOLICITED | ICMP6_NEIGHBOR_ADVERTISEMENT_FLAG_OVERRIDE);
    ...
    if (ip6_ra_adv_enabled(sw_if_index0)) icmp6_nsa->advertisement_flags |= clib_host_to_net_u32(ICMP6_NEIGHBOR_ADVERTISEMENT_FLAG_ROUTER);

    icmp6_nsa->icmp.checksum = 0;
    icmp6_nsa->icmp.checksum = ip6_tcp_udp_icmp_compute_checksum(vm, b, ip6_h, &bogus_length);
    ...
    if (eth_if) clib_memcpy(eth->src_address, &eth_if->address, 6);
    ...
    vnet_buffer(b)->sw_if_index[VLIB_RX] = vnet_main.local_interface_sw_if_index;

    vlib_increment_simple_counter(&ip_neighbor_counters[AF_IP6].ipnc[VLIB_TX][IP_NEIGHBOR_CTR_REPLY], vm->thread_index, sw_if_index0, 1);
}
