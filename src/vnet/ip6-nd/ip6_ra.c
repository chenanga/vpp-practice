...
ip6_ra_t *ip6_ra_get_itf(u32 sw_if_index)
{
    index_t rai;

    rai = ip6_link_delegate_get(sw_if_index, ip6_ra_delegate_id);

    if (INDEX_INVALID != rai) return (pool_elt_at_index(ip6_ra_pool, rai));

    return (NULL);
}

u8 ip6_ra_adv_enabled(u32 sw_if_index)
{
    ip6_ra_t *ra;
...
void ip6_ra_itf_walk(ip6_ra_itf_walk_fn_t fn, void *ctx)
{
    ip6_ra_t *radv_info;

    pool_foreach (radv_info, ip6_ra_pool) {
        if (WALK_STOP == fn(radv_info->sw_if_index, ctx)) break;
    }
}
...
static void ip6_neighbor_syslog(vlib_main_t *vm, int priority, char *fmt, ...)
{
    ...
    if ((priority > LOG_EMERG) || !(logmask & (1 << priority))) return;
    ...
    if (fmt) {
        ...
        ed->s[0] = elog_string(vlib_get_elog_main(), log_level_strings[priority]);
...
static_always_inline uword icmp6_router_solicitation(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    if (node->flags & VLIB_NODE_FLAG_TRACE)
        vlib_trace_frame_buffers_only(vm,
                                      node,
                                      from,
                                      frame->n_vectors,
                                      /* stride */ 1,
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (ip6_address_is_unspecified(&ip0->dst_address)) is_solicitation = 0;
            ...
            if (!is_unspecified && !is_link_local) {
                ...
                if (ADJ_INDEX_INVALID != src_adj_index0) {
                    ...
                    error0 = (adj0->rewrite_header.sw_if_index != sw_if_index0 ? ICMP6_ERROR_ROUTER_SOLICITATION_SOURCE_NOT_ON_LINK : error0);
                }
                else {
            ...
            if (PREDICT_TRUE(error0 == ICMP6_ERROR_NONE && o0 != 0 && !is_unspecified && !is_link_local)) {
            ...
            if (error0 == ICMP6_ERROR_NONE) {
                ...
                if (error0 == ICMP6_ERROR_NONE) {
                    ...
                    error0 = ((!radv_info || 0 == radv_info->send_radv) ? ICMP6_ERROR_ROUTER_SOLICITATION_RADV_NOT_CONFIG : error0);
                    if (error0 == ICMP6_ERROR_NONE) {
                        f64 now = vlib_time_now(vm);

                        /* for solicited adverts - need to rate limit */
                        if (is_solicitation) {
                            if (0 != radv_info->last_radv_time && (now - radv_info->last_radv_time) < MIN_DELAY_BETWEEN_RAS)
                                is_dropped = 1;
                            else
                        ...
                        rh.neighbor_reachable_time_in_msec = clib_host_to_net_u32(radv_info->adv_neighbor_reachable_time_in_msec);
                        ...
                        if (vlib_buffer_add_data(vm, &bi0, (void *) &rh, sizeof(icmp6_router_advertisement_header_t))) {
                        ...
                        if (radv_info->adv_link_layer_address) {
                            ...
                            if (vlib_buffer_add_data(vm, &bi0, (void *) &h, sizeof(icmp6_neighbor_discovery_ethernet_link_layer_address_option_t))) {
                                error0 = ICMP6_ERROR_ALLOC_FAILURE;
                                goto drop0;
                            }
                        ...
                        if (radv_info->adv_link_mtu) {
                            ...
                            if (vlib_buffer_add_data(vm, &bi0, (void *) &h, sizeof(icmp6_neighbor_discovery_mtu_option_t))) {
                                error0 = ICMP6_ERROR_ALLOC_FAILURE;
                                goto drop0;
                            }
                        }
                        ...
                        pool_foreach (pr_info, radv_info->adv_prefixes_pool) {
                            if (pr_info->enabled && (!pr_info->decrement_lifetime_flag || (pr_info->pref_lifetime_expires > 0))) {
                                ...
                                if (radv_info->cease_radv && pr_info->deprecated_prefix_flag) {
                                    ...
                                    if (pr_info->decrement_lifetime_flag) {
                                        pr_info->adv_valid_lifetime_in_secs =
                                            ((pr_info->valid_lifetime_expires > now)) ? (pr_info->valid_lifetime_expires - now) : 0;

                                        pr_info->adv_pref_lifetime_in_secs =
                                            ((pr_info->pref_lifetime_expires > now)) ? (pr_info->pref_lifetime_expires - now) : 0;
                                    }
                                ...
                                if (h.valid_time == 0) clib_warning("BUG: send RA with valid_time 0");
                                ...
                                if (vlib_buffer_add_data(vm, &bi0, (void *) &h, sizeof(icmp6_neighbor_discovery_prefix_information_option_t))) {
                                    error0 = ICMP6_ERROR_ALLOC_FAILURE;
                                    goto drop0;
                                }
                            }
                        ...
                        if (!is_unspecified && radv_info->send_unicast) {
                            ...
                            ip6_set_reserved_multicast_address(&ip0->dst_address, IP6_MULTICAST_SCOPE_link_local, IP6_MULTICAST_GROUP_ID_all_hosts);
                        }

                        /* source address MUST be the link-local address */
                        ip6_address_copy(&ip0->src_address, ip6_get_link_local_address(radv_info->sw_if_index));
                        ...
                        rh0->icmp.checksum                       = ip6_tcp_udp_icmp_compute_checksum(vm, p0, ip0, &bogus_length);
                        ...
                        if (is_solicitation) {
                            ...
                            vlib_buffer_advance(p0, vnet_buffer(p0)->l2_hdr_offset);
                            eth0 = vlib_buffer_get_current(p0);
                            clib_memcpy(eth0->dst_address, eth0->src_address, 6);
                            clib_memcpy(eth0->src_address, &eth_if0->address, 6);
                            next0                                 = is_dropped ? next0 : ICMP6_ROUTER_SOLICITATION_NEXT_REPLY_TX;
                            ...
                            if (adj_index0 == INDEX_INVALID)
                                ...
                                next0                                  = is_dropped ? next0 : ICMP6_ROUTER_SOLICITATION_NEXT_REPLY_RW;
        ...
        drop0:
            ...
            if (error0 != ICMP6_ERROR_NONE) vlib_error_count(vm, error_node->node_index, error0, 1);
...
static_always_inline uword icmp6_router_advertisement(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            error0 = (!ip6_address_is_link_local_unicast(&ip0->src_address)) ? ICMP6_ERROR_ROUTER_ADVERTISEMENT_SOURCE_NOT_LINK_LOCAL : error0;
            ...
            if (error0 == ICMP6_ERROR_NONE) {
                ...
                if (error0 == ICMP6_ERROR_NONE) {
                    ...
                    if (error0 == ICMP6_ERROR_NONE) {
                        ...
                        r.flags                           = h0->flags;
                        r.router_lifetime_in_sec          = clib_net_to_host_u16(h0->router_lifetime_in_sec);
                        r.neighbor_reachable_time_in_msec = clib_net_to_host_u32(h0->neighbor_reachable_time_in_msec);
                        r.time_in_msec_between_retransmitted_neighbor_solicitations =
                            clib_net_to_host_u32(h0->time_in_msec_between_retransmitted_neighbor_solicitations);
                        r.prefixes = 0;

                        /* validate advertised information */
                        if ((h0->current_hop_limit && radv_info->curr_hop_limit) && (h0->current_hop_limit != radv_info->curr_hop_limit)) {
                            ip6_neighbor_syslog(vm,
                                                LOG_WARNING,
                        ...
                        if ((h0->flags & ICMP6_ROUTER_DISCOVERY_FLAG_ADDRESS_CONFIG_VIA_DHCP) != radv_info->adv_managed_flag) {
                            ip6_neighbor_syslog(vm,
                                                LOG_WARNING,
                        ...
                        if ((h0->flags & ICMP6_ROUTER_DISCOVERY_FLAG_OTHER_CONFIG_VIA_DHCP) != radv_info->adv_other_flag) {
                            ip6_neighbor_syslog(vm,
                                                LOG_WARNING,
                        ...
                        if ((h0->neighbor_reachable_time_in_msec && radv_info->adv_neighbor_reachable_time_in_msec)
                            && (h0->neighbor_reachable_time_in_msec != clib_host_to_net_u32(radv_info->adv_neighbor_reachable_time_in_msec))) {
                            ip6_neighbor_syslog(vm,
                                                LOG_WARNING,
                        ...
                        while (options_len0 > 0) {
                            ...
                            switch (option_type) {
                                case ICMP6_NEIGHBOR_DISCOVERY_OPTION_source_link_layer_address: {
                                    ...
                                    if (opt_len < sizeof(*h)) break;

                                    memcpy(r.slla, h->ethernet_address, 6);
                                } break;

                                case ICMP6_NEIGHBOR_DISCOVERY_OPTION_mtu: {
                                    icmp6_neighbor_discovery_mtu_option_t *h = (icmp6_neighbor_discovery_mtu_option_t *) (o0);

                                    if (opt_len < sizeof(*h)) break;

                                    r.mtu = clib_net_to_host_u32(h->mtu);

                                    if ((h->mtu && radv_info->adv_link_mtu) && (h->mtu != clib_host_to_net_u32(radv_info->adv_link_mtu))) {
                                        ip6_neighbor_syslog(vm,
                                                            LOG_WARNING,
                                ...
                                case ICMP6_NEIGHBOR_DISCOVERY_OPTION_prefix_information: {
                                    ...
                                    vec_validate(r.prefixes, vec_len(r.prefixes));
                                    ra_report_prefix_info_t *prefix = vec_elt_at_index(r.prefixes, vec_len(r.prefixes) - 1);
                                    ...
                                    pool_foreach (pr_info, radv_info->adv_prefixes_pool) {
                                        ...
                                        if (pr_info->enabled && (pr_info->prefix_len == h->dst_address_length)
                                            && ip6_address_is_equal_masked(&pr_info->prefix, &h->dst_address, &mask)) {
                                            /* found it */
                                            if (!pr_info->decrement_lifetime_flag && valid != pr_info->adv_valid_lifetime_in_secs) {
                                                ip6_neighbor_syslog(vm,
                                                                    LOG_WARNING,
                                            ...
                                            if (!pr_info->decrement_lifetime_flag && preferred != pr_info->adv_pref_lifetime_in_secs) {
                                                ip6_neighbor_syslog(vm,
                                                                    LOG_WARNING,
...
static inline f64 random_f64_from_to(f64 from, f64 to)
{
    static u32 seed     = 0;
...
static inline u8 get_mac_address(u32 sw_if_index, u8 *address)
{
    vnet_main_t         *vnm   = vnet_get_main();
    vnet_hw_interface_t *hw_if = vnet_get_sup_hw_interface(vnm, sw_if_index);
    if (!hw_if->hw_address) return 1;
    clib_memcpy(address, hw_if->hw_address, 6);
    return 0;
}

static inline vlib_buffer_t *create_buffer_for_rs(vlib_main_t *vm, ip6_ra_t *radv_info)
{
    u32                                 bi0;
    vlib_buffer_t                      *p0;
    ...
    vnet_buffer(p0)->ip.adj_index[VLIB_TX] = ip6_link_get_mcast_adj(sw_if_index);
    ...
    rh->link_layer_option.header.type = ICMP6_NEIGHBOR_DISCOVERY_OPTION_source_link_layer_address;
    if (0 != get_mac_address(sw_if_index, rh->link_layer_option.ethernet_address)) {
        clib_warning("interface with sw_if_index %u has no mac address", sw_if_index);
    ...
    rh->ip.ip_version_traffic_class_and_flow_label = clib_host_to_net_u32(0x6 << 28);
    rh->ip.payload_length                          = clib_host_to_net_u16(payload_length);
    rh->ip.protocol                                = IP_PROTOCOL_ICMP6;
    rh->ip.hop_limit                               = 255;
    ip6_address_copy(&rh->ip.src_address, ip6_get_link_local_address(radv_info->sw_if_index));
    /* set address ff02::2 */
    rh->ip.dst_address.as_u64[0] = clib_host_to_net_u64(0xff02ULL << 48);
    rh->ip.dst_address.as_u64[1] = clib_host_to_net_u64(2);

    rh->neighbor.icmp.checksum = ip6_tcp_udp_icmp_compute_checksum(vm, p0, &rh->ip, &bogus_length);

    return p0;
}

static inline void stop_sending_rs(vlib_main_t *vm, ip6_ra_t *ra)
{
    u32 bi0;
...
static inline bool check_send_rs(vlib_main_t *vm, ip6_ra_t *radv_info, f64 current_time, f64 *due_time)
{
    vlib_buffer_t *p0;
    vlib_frame_t  *f;
    u32           *to_next;
    ...
    if (!radv_info->keep_sending_rs) return false;
    ...
    if (c0 == NULL) return radv_info->keep_sending_rs;
    ...
    if (params->mrc != 0 && --radv_info->n_left == 0)
        ...
        radv_info->sleep_interval = (2 + random_f64_from_to(-0.1, 0.1)) * radv_info->sleep_interval;
        if (radv_info->sleep_interval > params->mrt) radv_info->sleep_interval = (1 + random_f64_from_to(-0.1, 0.1)) * params->mrt;

        radv_info->due_time = current_time + radv_info->sleep_interval;

        if (params->mrd != 0 && current_time > radv_info->start_time + params->mrd)
            stop_sending_rs(vm, radv_info);
        else
            *due_time = radv_info->due_time;
    }
...
static uword send_rs_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f0)
{
    ...
    while (true) {
        ...
        do {
            ...
            pool_foreach (radv_info, ip6_ra_pool) {
                if (check_send_rs(vm, radv_info, current_time, &dt) && (dt < due_time)) due_time = dt;
            }
            current_time = vlib_time_now(vm);
...
static const ethernet_interface_t *ip6_ra_get_eth_itf(u32 sw_if_index)
{
    ...
    if (sw->type == VNET_SW_INTERFACE_TYPE_HARDWARE) return (ethernet_get_interface(&ethernet_main, sw->hw_if_index));
...
static void ip6_ra_link_enable(u32 sw_if_index)
{
    ...
    ASSERT(INDEX_INVALID == ip6_link_delegate_get(sw_if_index, ip6_ra_delegate_id));
    ...
    radv_info->adv_link_mtu = vnet_sw_interface_get_mtu(vnet_get_main(), sw_if_index, VNET_MTU_IP6);

    mhash_init(&radv_info->address_to_prefix_index, sizeof(uword), sizeof(ip6_address_t));

    ip6_link_delegate_update(sw_if_index, ip6_ra_delegate_id, radv_info - ip6_ra_pool);
}
...
static void ip6_ra_delegate_disable(index_t rai)
{
    ...
    pool_flush(p, radv_info->adv_prefixes_pool, ({ mhash_unset(&radv_info->address_to_prefix_index, &p->prefix, 0); }));
...
void ip6_ra_update_secondary_radv_info(ip6_address_t *address, u8 prefix_len, u32 primary_sw_if_index, u32 valid_time, u32 preferred_time)
{
    ...
    for (i = 0; i < vec_len(radv_indices); i++) {
        ...
        if (radv_info->sw_if_index == primary_sw_if_index) continue;

        pool_foreach (this_prefix, radv_info->adv_prefixes_pool) {
            if (this_prefix->prefix_len == prefix_len && ip6_address_is_equal_masked(&this_prefix->prefix, address, &mask)) {
                int rv = ip6_ra_prefix(vm,
                                       ...
                                       0 /* no_onlink */,
                                       0 /* is_no */);
                if (rv != 0) clib_warning("ip6_neighbor_ra_prefix returned %d", rv);
            }
        }
    }
}
...
static uword ip6_ra_process_timer_event(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    pool_foreach (radv_info, ip6_ra_pool) {
        ...
        if (radv_info->send_radv && (now >= radv_info->next_multicast_time)) {
            ...
            f64 rfn = (radv_info->max_radv_interval - radv_info->min_radv_interval) * random_f64(&radv_info->seed) + radv_info->min_radv_interval;

            /* multicast send - compute next multicast send time */
            if (radv_info->initial_adverts_sent > 0) {
                radv_info->initial_adverts_sent--;
                if (rfn > radv_info->initial_adverts_interval) rfn = radv_info->initial_adverts_interval;

                /* check to see if we are ceasing to send */
                if (radv_info->initial_adverts_sent == 0)
                    if (radv_info->cease_radv) radv_info->send_radv = 0;
            }
            ...
            h0->ip.payload_length =
                clib_host_to_net_u16(sizeof(icmp6_router_solicitation_header_t) - STRUCT_OFFSET_OF(icmp6_router_solicitation_header_t, neighbor));
...
static void ip6_ra_signal_report(ip6_ra_report_t *r)
{
    ...
    if (!vec_len(ip6_ra_listeners)) return;
                  ...
                  u8           is_no)
{
    ...
    if (!radv_info) return (VNET_API_ERROR_IP6_NOT_ENABLED);

    /* Start off believing that we're going to send radv's */
    radv_info->send_radv = 1;

    if ((max_interval != 0) && (min_interval == 0)) min_interval = .75 * max_interval;
    ...
    if (lifetime) {
        if (lifetime > MAX_DEF_RTR_LIFETIME) lifetime = MAX_DEF_RTR_LIFETIME;

        if (lifetime <= max_interval) return VNET_API_ERROR_INVALID_VALUE;
    }

    if (min_interval != 0) {
        if ((min_interval > .75 * max_interval) || (min_interval < 3)) return VNET_API_ERROR_INVALID_VALUE;
    }

    if ((initial_count > MAX_INITIAL_RTR_ADVERTISEMENTS) || (initial_interval > MAX_INITIAL_RTR_ADVERT_INTERVAL)) return VNET_API_ERROR_INVALID_VALUE;
    ...
    /* restart */
    if ((cease != 0) && (is_no)) radv_info->send_radv = 1;
                  ...
                  u8             is_no)
{
    ...
    if (is_no) {
        /* delete */
        if (!prefix) return VNET_API_ERROR_INVALID_VALUE; /* invalid prefix */

        if (prefix->prefix_len != prefix_len) return VNET_API_ERROR_INVALID_VALUE_2;

        /* FIXME - Should the DP do this or the CP ? */
        /* do specific delete processing here before returning */
        /* try to remove from routing table */

        mhash_unset(&radv_info->address_to_prefix_index,
                    prefix_addr,
                    /* old_value */ 0);
    ...
    if (!prefix) {
        ...
        mhash_set(&radv_info->address_to_prefix_index,
                  prefix_addr,
                  pi,
                  /* old_value */ 0);
...
clib_error_t *ip6_ra_cmd(vlib_main_t *vm, unformat_input_t *main_input, vlib_cli_command_t *cmd)
{
    ...
    /* Get a line of input. */
    if (!unformat_user(main_input, unformat_line_input, line_input)) return 0;
    ...
    if (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat_user(line_input, unformat_vnet_sw_interface, vnm, &sw_if_index)) {
            ...
            if (!ip6_link_is_enabled(sw_if_index)) {
                error = clib_error_return(0, "IP6 nt enabler interface %U'", format_unformat_error, line_input);
                goto done;
            }
        }
        else {
            error = clib_error_return(0, "invalid interface name %U'", format_unformat_error, line_input);
            goto done;
        }
    }
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "no"))
            is_no = 1;
        else if (unformat(line_input, "prefix %U/%d", unformat_ip6_address, &ip6_addr, &addr_len)) {
            add_radv_info = 0;
        }
        ...
        else if (unformat(line_input, "ra-suppress") || unformat(line_input, "ra-surpress")) {
            suppress = 1;
        }
        else if (unformat(line_input, "ra-suppress-link-layer") || unformat(line_input, "ra-surpress-link-layer")) {
            ...
            if (!unformat(line_input, "%d", &ra_min_interval)) ra_min_interval = 0;
        }
    ...
    if (add_radv_info) {
        ...
        while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
            ...
            else if (unformat(line_input, "%d %d", &valid_lifetime_in_secs, &pref_lifetime_in_secs))
                break;
            else
                break;
        }
        ...
        /* get the rest of the command */
        while (!use_prefix_default_values && unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
...
static u8 *format_ip6_ra(u8 *s, va_list *args)
{
    ...
    pool_foreach (p, radv_info->adv_prefixes_pool) {
        s = format(s, "%Uprefix %U, length %d\n", format_white_space, indent + 2, format_ip6_address, &p->prefix, p->prefix_len);
    }
    ...
    s = format(s, "%UND advertised reachable time is %d\n", format_white_space, indent, radv_info->adv_neighbor_reachable_time_in_msec);
    s = format(s,
