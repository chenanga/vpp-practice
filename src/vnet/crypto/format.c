...
uword unformat_vnet_crypto_alg(unformat_input_t *input, va_list *args)
{
    ...
    if (!unformat(input, "%s", &name)) return 0;
...
u8 *format_vnet_crypto_op(u8 *s, va_list *args)
{
    ...
    return format(s, "%U-%U", format_vnet_crypto_op_type, otd->type, format_vnet_crypto_alg, otd->alg);
}
...
#undef _
    ...
    if (opt >= VNET_CRYPTO_OP_N_TYPES) return format(s, "unknown");
...
#undef _
    ...
    if (st >= VNET_CRYPTO_OP_N_STATUS) return format(s, "unknown");
...
u8 *format_vnet_crypto_engine(u8 *s, va_list *args)
{
    ...
    if (crypto_engine_index == ~0) return s;
...
{
    vnet_crypto_async_op_type_t opt       = va_arg(*args, vnet_crypto_async_op_type_t);
    char                       *strings[] = {
...
#undef _
    ...
    if (opt >= VNET_CRYPTO_ASYNC_OP_N_TYPES) return format(s, "unknown");
...
u8 *format_vnet_crypto_async_op(u8 *s, va_list *args)
{
    ...
    return format(s, "%U-%U", format_vnet_crypto_async_op_type, otd->type, format_vnet_crypto_async_alg, otd->alg);
}
