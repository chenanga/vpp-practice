...
#define _(n, s, k, t, a) VNET_CRYPTO_OP_##n##_TAG##t##_AAD##a##_ENC, VNET_CRYPTO_OP_##n##_TAG##t##_AAD##a##_DEC,
    foreach_crypto_aead_async_alg
#undef _
#define _(c, h, s, k, d) VNET_CRYPTO_OP_##c##_##h##_TAG##d##_ENC, VNET_CRYPTO_OP_##c##_##h##_TAG##d##_DEC,
        foreach_crypto_link_async_alg
#undef _
...
void vnet_crypto_register_enqueue_handler(vlib_main_t *vm, u32 engine_index, vnet_crypto_async_op_id_t opt, vnet_crypto_frame_enqueue_t *enq_fn);
...
{
    if (CLIB_DEBUG > 0) clib_memset(op, 0xfe, sizeof(*op));
    op->op        = type;
    op->flags     = 0;
    op->key_index = ~0;
    op->n_chunks  = 0;
...
static_always_inline void vnet_crypto_async_reset_frame(vnet_crypto_async_frame_t *f)
{
    ...
    ASSERT(f != 0);
    ASSERT((f->state == VNET_CRYPTO_FRAME_STATE_NOT_PROCESSED || f->state == VNET_CRYPTO_FRAME_STATE_ELT_ERROR));
    opt = f->op;
    if (CLIB_DEBUG > 0) clib_memset(f, 0xfe, sizeof(*f));
    f->state  = VNET_CRYPTO_FRAME_STATE_NOT_PROCESSED;
    f->op     = opt;
    f->n_elts = 0;
}
