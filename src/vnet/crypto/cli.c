...
static clib_error_t *show_crypto_engines_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (unformat_user(input, unformat_line_input, line_input)) unformat_free(line_input);
...
static u8 *format_vnet_crypto_engine_candidates(u8 *s, va_list *args)
{
    ...
    if (is_async) {
        ...
        vec_foreach (e, cm->engines) {
            void *h = is_chained ? (void *) e->chained_ops_handlers[id] : (void *) e->ops_handlers[id];
...
static u8 *format_vnet_crypto_handlers(u8 *s, va_list *args)
{
    ...
    for (i = 0; i < VNET_CRYPTO_OP_N_TYPES; i++) {
        ...
        od = cm->opt_data + id;
        if (first == 0) s = format(s, "\n%U", format_white_space, indent);
        s = format(s, "%-16U", format_vnet_crypto_op_type, od->type);

        s     = format(s, "%-28U", format_vnet_crypto_engine_candidates, id, od->active_engine_index_simple, 0, 0);
        s     = format(s, "%U", format_vnet_crypto_engine_candidates, id, od->active_engine_index_chained, 1, 0);
        first = 0;
    }
    return s;
}
...
static clib_error_t *show_crypto_handlers_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vlib_cli_output(vm, "%-16s%-16s%-28s%s", "Algo", "Type", "Simple", "Chained");

    for (i = 0; i < VNET_CRYPTO_N_ALGS; i++)
        vlib_cli_output(vm, "%-20U%U", format_vnet_crypto_alg, i, format_vnet_crypto_handlers, i);
...
static clib_error_t *set_crypto_handler_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (all) {
        ...
        if (rc) vlib_cli_output(vm, "failed to set crypto engine!");
    }
    else {
        vec_foreach (arg, args) {
            rc = vnet_crypto_set_handler2(arg[0], engine, oct);
            if (rc) {
                vlib_cli_output(vm, "failed to set engine %s for %s!", engine, arg[0]);
            }
        }
    }
...
static u8 *format_vnet_crypto_async_handlers(u8 *s, va_list *args)
{
    ...
    for (i = 0; i < VNET_CRYPTO_ASYNC_OP_N_TYPES; i++) {
        ...
        s     = format(s, "%U", format_vnet_crypto_engine_candidates, id, od->active_engine_index_async, 0, 1);
        first = 0;
    }
    return s;
}
...
static clib_error_t *show_crypto_async_handlers_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (i = 0; i < VNET_CRYPTO_N_ASYNC_ALGS; i++)
        vlib_cli_output(vm, "%-28U%U", format_vnet_crypto_async_alg, i, format_vnet_crypto_async_handlers, i);
...
static clib_error_t *show_crypto_async_status_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (i = 0; i < tm->n_vlib_mains; i++) {
        vlib_node_state_t state = vlib_node_get_state(vlib_get_main_by_index(i), cm->crypto_node_index);
        if (state == VLIB_NODE_STATE_POLLING) vlib_cli_output(vm, "threadId: %-6d POLLING", i);
        if (state == VLIB_NODE_STATE_INTERRUPT) vlib_cli_output(vm, "threadId: %-6d INTERRUPT", i);
        if (state == VLIB_NODE_STATE_DISABLED) vlib_cli_output(vm, "threadId: %-6d DISABLED", i);
    }
    return 0;
}
...
static clib_error_t *set_crypto_async_dispatch_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    clib_error_t    *error    = 0;
    u8               adaptive = 0;
