...
int vnet_crypto_is_set_handler(vnet_crypto_alg_t alg)
{
    ...
    if (alg >= vec_len(cm->algs)) return 0;

    for (i = 0; i < VNET_CRYPTO_OP_N_TYPES; i++)
        if ((opt = cm->algs[alg].op_by_type[i]) != 0) break;

    if (opt >= vec_len(cm->ops_handlers)) return 0;
                                             ...
                                             vnet_crypto_chained_ops_handler_t *cfn)
{
    ...
    vec_validate_aligned(cm->ops_handlers, VNET_CRYPTO_N_OP_IDS - 1, CLIB_CACHE_LINE_BYTES);
    vec_validate_aligned(cm->chained_ops_handlers, VNET_CRYPTO_N_OP_IDS - 1, CLIB_CACHE_LINE_BYTES);
    ...
    if (fn) {
        ...
        if (ae->priority < e->priority) crypto_set_active_engine(otd, opt, engine_index, CRYPTO_OP_SIMPLE);
    }
    ...
    if (cfn) {
        ...
        if (ae->priority < e->priority) crypto_set_active_engine(otd, opt, engine_index, CRYPTO_OP_CHAINED);
    }
...
void vnet_crypto_register_enqueue_handler(vlib_main_t *vm, u32 engine_index, vnet_crypto_async_op_id_t opt, vnet_crypto_frame_enqueue_t *enqueue_hdl)
{
    vnet_crypto_main_t          *cm = &crypto_main;
    vnet_crypto_engine_t        *ae, *e = vec_elt_at_index(cm->engines, engine_index);
    vnet_crypto_async_op_data_t *otd = cm->async_opt_data + opt;
    vec_validate_aligned(cm->enqueue_handlers, VNET_CRYPTO_ASYNC_OP_N_IDS, CLIB_CACHE_LINE_BYTES);

    if (!enqueue_hdl) return;
...
static int engine_index_cmp(void *v1, void *v2)
{
    u32 *a1 = v1;
    u32 *a2 = v2;
...
static void vnet_crypto_update_cm_dequeue_handlers(void)
{
    vnet_crypto_main_t          *cm = &crypto_main;
    ...
    for (i = 0; i < VNET_CRYPTO_ASYNC_OP_N_IDS; i++) {
        ...
        if (otd->active_engine_index_async == ~0) continue;
        e = cm->engines + otd->active_engine_index_async;
        if (!e->dequeue_handler) continue;
...
void vnet_crypto_register_dequeue_handler(vlib_main_t *vm, u32 engine_index, vnet_crypto_frame_dequeue_t *deq_fn)
{
    vnet_crypto_main_t   *cm = &crypto_main;
...
u32 vnet_crypto_key_add(vlib_main_t *vm, vnet_crypto_alg_t alg, u8 *data, u16 length)
{
    ...
    if (!vnet_crypto_key_len_check(alg, length)) return ~0;
    ...
    vec_foreach (engine, cm->engines)
        if (engine->key_op_handler) engine->key_op_handler(vm, VNET_CRYPTO_KEY_OP_ADD, index);
    return index;
}
...
void vnet_crypto_key_del(vlib_main_t *vm, vnet_crypto_key_index_t index)
{
    ...
    vec_foreach (engine, cm->engines)
        if (engine->key_op_handler) engine->key_op_handler(vm, VNET_CRYPTO_KEY_OP_DEL, index);
...
void vnet_crypto_key_update(vlib_main_t *vm, vnet_crypto_key_index_t index)
{
    ...
    vec_foreach (engine, cm->engines)
        if (engine->key_op_handler) engine->key_op_handler(vm, VNET_CRYPTO_KEY_OP_MODIFY, index);
}

vnet_crypto_async_alg_t vnet_crypto_link_algs(vnet_crypto_alg_t crypto_alg, vnet_crypto_alg_t integ_alg)
{
#define _(c, h, s, k, d) \
    if (crypto_alg == VNET_CRYPTO_ALG_##c && integ_alg == VNET_CRYPTO_ALG_HMAC_##h) return VNET_CRYPTO_ALG_##c##_##h##_TAG##d;
...
u32 vnet_crypto_register_post_node(vlib_main_t *vm, char *post_node_name)
{
    ...
    vec_foreach (nn, cm->next_nodes) {
        if (nn->node_idx == pn->index) return nn->next_idx;
    }
...
void vnet_crypto_set_async_dispatch(u8 mode, u8 adaptive)
{
    ...
    vlib_node_state_t   state = mode ? VLIB_NODE_STATE_INTERRUPT : VLIB_NODE_STATE_POLLING;

    for (i = vlib_num_workers() > 0; i < tm->n_vlib_mains; i++) {
        vlib_main_t *ovm = vlib_get_main_by_index(i);
        vlib_node_set_state(ovm, node_index, state);
        vlib_node_set_flag(ovm, node_index, VLIB_NODE_FLAG_ADAPTIVE_MODE, adaptive);
    }
}
...
int vnet_crypto_is_set_async_handler(vnet_crypto_async_op_id_t op)
{
    ...
    return (op < vec_len(cm->enqueue_handlers) && NULL != cm->enqueue_handlers[op]);
}
...
#define _(n, s, l) vnet_crypto_init_cipher_data(VNET_CRYPTO_ALG_##n, VNET_CRYPTO_OP_##n##_ENC, VNET_CRYPTO_OP_##n##_DEC, s, 0);
    foreach_crypto_cipher_alg;
#undef _
#define _(n, s, l) vnet_crypto_init_cipher_data(VNET_CRYPTO_ALG_##n, VNET_CRYPTO_OP_##n##_ENC, VNET_CRYPTO_OP_##n##_DEC, s, 1);
    foreach_crypto_aead_alg;
#undef _
#define _(n, s) vnet_crypto_init_hmac_data(VNET_CRYPTO_ALG_HMAC_##n, VNET_CRYPTO_OP_##n##_HMAC, "hmac-" s);
    foreach_crypto_hmac_alg;
#undef _
#define _(n, s) vnet_crypto_init_hash_data(VNET_CRYPTO_ALG_HASH_##n, VNET_CRYPTO_OP_##n##_HASH, s);
    foreach_crypto_hash_alg;
#undef _
#define _(n, s, k, t, a)                                                    \
