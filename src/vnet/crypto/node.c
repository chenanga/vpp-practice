...
static u8 *format_crypto_dispatch_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U: %U", format_vnet_crypto_async_op, t->op, format_vnet_crypto_op_status, t->op_status);
    return s;
}
                                              ...
                                              u32                         *n_total)
{
    ...
    *n_total += n_elts;
    ...
    while (cf || n_elts) {
        if (cf) {
            ...
            if (cf->state == VNET_CRYPTO_FRAME_STATE_SUCCESS) {
                ...
                for (i = 0; i < cf->n_elts; i++) {
                    if (cf->elts[i].status != VNET_CRYPTO_OP_STATUS_COMPLETED) {
                        ...
                        vlib_node_increment_counter(vm, node->node_index, cf->elts[i].status, 1);
                    }
                    else
            ...
            if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE)) {
                ...
                for (i = 0; i < cf->n_elts; i++) {
                    ...
                    if (b->flags & VLIB_BUFFER_IS_TRACED) vnet_crypto_async_add_trace(vm, node, b, cf->op, cf->elts[i].status);
                }
            }
        ...
        if (n_elts > 0
            && ((node->state == VLIB_NODE_STATE_POLLING && (node->flags & VLIB_NODE_FLAG_SWITCH_FROM_POLLING_TO_INTERRUPT_MODE))
                || node->state == VLIB_NODE_STATE_INTERRUPT)) {
            vlib_node_set_interrupt_pending(vlib_get_main_by_index(enqueue_thread_idx), cm->crypto_node_index);
        }
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    vec_foreach_index (index, cm->dequeue_handlers) {
        n_cache = crypto_dequeue_frame(vm, node, ct, cm->dequeue_handlers[index], n_cache, &n_dispatched);
    }
    if (n_cache) vlib_buffer_enqueue_to_next_vec(vm, node, &ct->buffer_indices, &ct->nexts, n_cache);
