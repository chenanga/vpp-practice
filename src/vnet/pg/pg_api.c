...
static void vl_api_pg_create_interface_t_handler(vl_api_pg_create_interface_t *mp)
{
    ...
    u32             pg_if_id = pg_interface_add_or_get(pg, ntohl(mp->interface_id), mp->gso_enabled, ntohl(mp->gso_size), 0, PG_MODE_ETHERNET);
    pg_interface_t *pi       = pool_elt_at_index(pg->interfaces, pg_if_id);

    REPLY_MACRO2(VL_API_PG_CREATE_INTERFACE_REPLY, ({ rmp->sw_if_index = ntohl(pi->sw_if_index); }));
}
...
static void vl_api_pg_create_interface_v2_t_handler(vl_api_pg_create_interface_v2_t *mp)
{
    ...
    pg_main_t      *pg       = &pg_main;
    u32             pg_if_id = pg_interface_add_or_get(pg, ntohl(mp->interface_id), mp->gso_enabled, ntohl(mp->gso_size), 0, (u8) mp->mode);
    pg_interface_t *pi       = pool_elt_at_index(pg->interfaces, pg_if_id);

    REPLY_MACRO2(VL_API_PG_CREATE_INTERFACE_V2_REPLY, ({ rmp->sw_if_index = ntohl(pi->sw_if_index); }));
}
...
static void vl_api_pg_interface_enable_disable_coalesce_t_handler(vl_api_pg_interface_enable_disable_coalesce_t *mp)
{
    ...
    vnet_hw_interface_t *hw  = vnet_get_sup_hw_interface_api_visible_or_null(vnm, sw_if_index);

    if (hw) {
        pg_interface_t *pi = pool_elt_at_index(pg->interfaces, hw->dev_instance);
        if (pi->gso_enabled)
            pg_interface_enable_disable_coalesce(pi, mp->coalesce_enabled, hw->tx_node_index);
        else
...
static void vl_api_pg_capture_t_handler(vl_api_pg_capture_t *mp)
{
    ...
    if (hw_if_index != ~0) {
        ...
        char             *pcap_file_name = vl_api_from_api_to_new_c_string(&mp->pcap_file_name);
