 ...
 *
 */
always_inline void vxlan_gpe_encap_one_inline(vxlan_gpe_main_t *ngm, vlib_buffer_t *b0, vxlan_gpe_tunnel_t *t0, u32 *next0, ip_address_family_t af)
{
    ASSERT(sizeof(ip4_vxlan_gpe_header_t) == 36);
    ASSERT(sizeof(ip6_vxlan_gpe_header_t) == 56);

    ip_udp_encap_one(ngm->vlib_main, b0, t0->rewrite, t0->rewrite_size, af, N_AF, UDP_ENCAP_FIXUP_NONE);
 ...
 *
 */
always_inline void vxlan_gpe_encap_two_inline(vxlan_gpe_main_t   *ngm,
                                              vlib_buffer_t      *b0,
                                              vlib_buffer_t      *b1,
                                              ...
                                              ip_address_family_t af)
{
    ...
    ip_udp_encap_one(ngm->vlib_main, b0, t0->rewrite, t0->rewrite_size, af, N_AF, UDP_ENCAP_FIXUP_NONE);
    ip_udp_encap_one(ngm->vlib_main, b1, t1->rewrite, t1->rewrite_size, af, N_AF, UDP_ENCAP_FIXUP_NONE);
...
static uword vxlan_gpe_encap(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (sw_if_index0 != vnet_buffer(b[0])->sw_if_index[VLIB_TX]) {
                ...
                hi0          = vnet_get_sup_hw_interface(vnm, vnet_buffer(b[0])->sw_if_index[VLIB_TX]);
            ...
            if (sw_if_index1 != vnet_buffer(b[1])->sw_if_index[VLIB_TX]) {
                if (sw_if_index0 == vnet_buffer(b[1])->sw_if_index[VLIB_TX]) {
                    ...
                    hi1          = vnet_get_sup_hw_interface(vnm, vnet_buffer(b[1])->sw_if_index[VLIB_TX]);
                    t1           = pool_elt_at_index(ngm->tunnels, hi1->dev_instance);
                    af_1         = (t1->flags & VXLAN_GPE_TUNNEL_IS_IPV4 ? AF_IP4 : AF_IP6);
                }
            }

            if (PREDICT_TRUE(af_0 == af_1)) {
                vxlan_gpe_encap_two_inline(ngm, b[0], b[1], t0, t1, &next0, &next1, af_0);
            }
            else {
            ...
            if (PREDICT_FALSE(b[0]->flags & VLIB_BUFFER_IS_TRACED)) {
                vxlan_gpe_encap_trace_t *tr = vlib_add_trace(vm, node, b[0], sizeof(*tr));
                tr->tunnel_index            = t0 - ngm->tunnels;
            }

            if (PREDICT_FALSE(b[1]->flags & VLIB_BUFFER_IS_TRACED)) {
                vxlan_gpe_encap_trace_t *tr = vlib_add_trace(vm, node, b[1], sizeof(*tr));
    ...
    vlib_node_increment_counter(vm, node->node_index, VXLAN_GPE_ENCAP_ERROR_ENCAPSULATED, pkts_encapsulated);
