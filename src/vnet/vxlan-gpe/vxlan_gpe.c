...
u8 *format_vxlan_gpe_tunnel(u8 *s, va_list *args)
{
  ...
  */
    ...
    if (PREDICT_FALSE(ip46_address_is_multicast(&t->remote))) s = format(s, "mcast-sw-if-idx %d ", t->mcast_sw_if_index);
...
static void vxlan_gpe_tunnel_restack_dpo(vxlan_gpe_tunnel_t *t)
{
    ...
    fib_forward_chain_type_t forw_type   = ip46_address_is_ip4(&t->remote) ? FIB_FORW_CHAIN_TYPE_UNICAST_IP4 : FIB_FORW_CHAIN_TYPE_UNICAST_IP6;
...
static vxlan_gpe_tunnel_t *vxlan_gpe_tunnel_from_fib_node(fib_node_t *node)
{
    ...
    return ((vxlan_gpe_tunnel_t *) (((char *) node) - STRUCT_OFFSET_OF(vxlan_gpe_tunnel_t, node)));
}
...
static inline void mcast_shared_add(ip46_address_t *remote, fib_node_index_t mfei, adj_index_t ai)
{
...
static inline void mcast_shared_remove(ip46_address_t *remote)
{
...
int vnet_vxlan_gpe_add_del_tunnel(vnet_vxlan_gpe_add_del_tunnel_args_t *a, u32 *sw_if_indexp)
{
    ...
    /* Set udp-ports */
    if (a->local_port == 0) a->local_port = is_ip6 ? UDP_DST_PORT_VXLAN6_GPE : UDP_DST_PORT_VXLAN_GPE;

    if (a->remote_port == 0) a->remote_port = is_ip6 ? UDP_DST_PORT_VXLAN6_GPE : UDP_DST_PORT_VXLAN_GPE;
...
#define _(x) t->x = a->x;
        ...
        if (!a->is_ip6)
            foreach_copy_ipv4 else foreach_copy_ipv6
#undef _
        ...
        if (!is_ip6) {
            ...
            hash_set_mem(ngm->vxlan4_gpe_tunnel_by_key, key4_copy, t - ngm->tunnels);
        }
        else {
            key6_copy = clib_mem_alloc(sizeof(*key6_copy));
            clib_memcpy_fast(key6_copy, &key6, sizeof(*key6_copy));
            hash_set_mem(ngm->vxlan6_gpe_tunnel_by_key, key6_copy, t - ngm->tunnels);
        }

        if (vec_len(ngm->free_vxlan_gpe_tunnel_hw_if_indices) > 0) {
            vnet_interface_main_t *im = &vnm->interface_main;
            hw_if_index               = ngm->free_vxlan_gpe_tunnel_hw_if_indices[vec_len(ngm->free_vxlan_gpe_tunnel_hw_if_indices) - 1];
            ...
            hw_if_index = vnet_register_interface(vnm, vxlan_gpe_device_class.index, t - ngm->tunnels, vxlan_gpe_hw_class.index, t - ngm->tunnels);
            hi          = vnet_get_hw_interface(vnm, hw_if_index);
        }
        ...
        if (!ip46_address_is_multicast(&t->remote)) {
            ...
            t->fib_entry_index =
                fib_entry_track(t->encap_fib_index, &tun_remote_pfx, FIB_NODE_TYPE_VXLAN_GPE_TUNNEL, t - ngm->tunnels, &t->sibling_index);
             ...
             */
            if (vtep_addr_ref(&ngm->vtep_table, t->encap_fib_index, &t->remote) == 1) {
                 ...
                 */
                mfib_table_entry_path_update(t->encap_fib_index, &mpfx, MFIB_SOURCE_VXLAN_GPE, MFIB_ENTRY_FLAG_NONE, &path);

                path.frp_sw_if_index = a->mcast_sw_if_index;
                path.frp_flags       = FIB_ROUTE_PATH_FLAG_NONE;
                path.frp_mitf_flags  = MFIB_ITF_FLAG_ACCEPT;
                mfei                 = mfib_table_entry_path_update(t->encap_fib_index, &mpfx, MFIB_SOURCE_VXLAN_GPE, MFIB_ENTRY_FLAG_NONE, &path);
        ...
        si->flags |= VNET_SW_INTERFACE_FLAG_HIDDEN;
        set_int_l2_mode(ngm->vlib_main, vnm, MODE_L3, t->sw_if_index, 0, L2_BD_PORT_TYPE_NORMAL, 0, 0);
        ...
        else if (vtep_addr_unref(&ngm->vtep_table, t->encap_fib_index, &t->remote) == 0) {
    ...
    if (a->is_add) {
        ...
        if (!is_ip6 && !udp_is_valid_dst_port(a->local_port, 1))
            udp_register_dst_port(ngm->vlib_main, a->local_port, vxlan4_gpe_input_node.index, 1 /* is_ip4 */);
        if (is_ip6 && !udp_is_valid_dst_port(a->remote_port, 0))
            udp_register_dst_port(ngm->vlib_main, a->remote_port, vxlan6_gpe_input_node.index, 0 /* is_ip4 */);
    }
...
static clib_error_t *vxlan_gpe_add_del_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "del"))
            is_add = 0;
        else if (unformat(line_input, "local %U", unformat_ip4_address, &local.ip4)) {
            local_set = 1;
            ipv4_set  = 1;
        }
        else if (unformat(line_input, "remote %U", unformat_ip4_address, &remote.ip4)) {
            remote_set = 1;
            ipv4_set   = 1;
        }
        else if (unformat(line_input, "local %U", unformat_ip6_address, &local.ip6)) {
            local_set = 1;
            ipv6_set  = 1;
        }
        else if (unformat(line_input, "remote %U", unformat_ip6_address, &remote.ip6)) {
            remote_set = 1;
            ipv6_set   = 1;
        }
...
#undef _
    ...
    switch (rv) {
        ...
        default:
            error = clib_error_return(0, "vnet_vxlan_gpe_add_del_tunnel returned %d", rv);
            goto done;
    }
...
static clib_error_t *show_vxlan_gpe_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (pool_elts(ngm->tunnels) == 0) vlib_cli_output(vm, "No vxlan-gpe tunnels configured.");
...
{
    if (is_ip6)
        vnet_feature_enable_disable("ip6-unicast", "ip6-vxlan-gpe-bypass", sw_if_index, is_enable, 0, 0);
    else
        vnet_feature_enable_disable("ip4-unicast", "ip4-vxlan-gpe-bypass", sw_if_index, is_enable, 0, 0);
}
...
clib_error_t *vxlan_gpe_init(vlib_main_t *vm)
{
    ...
    ngm->vxlan4_gpe_tunnel_by_key = hash_create_mem(0, sizeof(vxlan4_gpe_tunnel_key_t), sizeof(uword));

    ngm->vxlan6_gpe_tunnel_by_key = hash_create_mem(0, sizeof(vxlan6_gpe_tunnel_key_t), sizeof(uword));

    ngm->mcast_shared = hash_create_mem(0, sizeof(ip46_address_t), sizeof(mcast_shared_t));
    ngm->vtep_table   = vtep_table_create();

    /* Register the list of standard decap protocols supported */
    vxlan_gpe_register_decap_protocol(VXLAN_GPE_PROTOCOL_IP4, VXLAN_GPE_INPUT_NEXT_IP4_INPUT);
    vxlan_gpe_register_decap_protocol(VXLAN_GPE_PROTOCOL_IP6, VXLAN_GPE_INPUT_NEXT_IP6_INPUT);
    vxlan_gpe_register_decap_protocol(VXLAN_GPE_PROTOCOL_ETHERNET, VXLAN_GPE_INPUT_NEXT_L2_INPUT);
