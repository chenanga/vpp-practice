...
static void vl_api_vxlan_gpe_add_del_tunnel_v2_t_handler(vl_api_vxlan_gpe_add_del_tunnel_v2_t *mp)
{
...
out:
    REPLY_MACRO2(VL_API_VXLAN_GPE_ADD_DEL_TUNNEL_V2_REPLY, ({ rmp->sw_if_index = ntohl(sw_if_index); }));
}
...
static void send_vxlan_gpe_tunnel_details(vxlan_gpe_tunnel_t *t, vl_api_registration_t *reg, u32 context)
{
    ...
    rmp->_vl_msg_id = ntohs(REPLY_MSG_ID_BASE + VL_API_VXLAN_GPE_TUNNEL_DETAILS);

    ip_address_encode(&t->local, is_ipv6 ? IP46_TYPE_IP6 : IP46_TYPE_IP4, &rmp->local);
    ip_address_encode(&t->remote, is_ipv6 ? IP46_TYPE_IP6 : IP46_TYPE_IP4, &rmp->remote);
...
static void vl_api_vxlan_gpe_tunnel_dump_t_handler(vl_api_vxlan_gpe_tunnel_dump_t *mp)
{
    ...
    if (~0 == sw_if_index) {
        ...
        if ((sw_if_index >= vec_len(vgm->tunnel_index_by_sw_if_index)) || (~0 == vgm->tunnel_index_by_sw_if_index[sw_if_index])) {
            return;
        }
...
static void send_vxlan_gpe_tunnel_v2_details(vxlan_gpe_tunnel_t *t, vl_api_registration_t *reg, u32 context)
{
    ...
    rmp = vl_msg_api_alloc(sizeof(*rmp));
    clib_memset(rmp, 0, sizeof(*rmp));
    rmp->_vl_msg_id = ntohs(REPLY_MSG_ID_BASE + VL_API_VXLAN_GPE_TUNNEL_V2_DETAILS);
...
static void vl_api_vxlan_gpe_tunnel_v2_dump_t_handler(vl_api_vxlan_gpe_tunnel_v2_dump_t *mp)
{
    vl_api_registration_t *reg;
    vxlan_gpe_main_t      *vgm = &vxlan_gpe_main;
    vxlan_gpe_tunnel_t    *t;
...
static clib_error_t *vxlan_gpe_api_hookup(vlib_main_t *vm)
{
    ...
    vl_api_increase_msg_trace_size(am, VL_API_VXLAN_GPE_ADD_DEL_TUNNEL, 17 * sizeof(u32));
