...
static u8 *format_vxlan_gpe_rx_trace(u8 *s, va_list *args)
{
    ...
    if (t->tunnel_index != ~0) {
        s = format(s, "VXLAN-GPE: tunnel %d next %d error %d", t->tunnel_index, t->next_index, t->error);
    }
    else {
...
static const vxlan_gpe_decap_info_t decap_not_found = {.tunnel_index = ~0,
                                                       .next_index   = VXLAN_GPE_INPUT_NEXT_DROP,
                                                       .error        = VXLAN_GPE_ERROR_NO_SUCH_TUNNEL};

always_inline vxlan_gpe_decap_info_t vxlan4_gpe_find_tunnel(vxlan_gpe_main_t *nngm, vxlan4_gpe_tunnel_cache_t *cache, ip4_vxlan_gpe_header_t *iuvn4_0)
{
    ...
    if (PREDICT_TRUE(key4.as_u64[0] == cache->key.as_u64[0] && key4.as_u64[1] == cache->key.as_u64[1])) {
        /* cache hit */
        return cache->val;
    }
    ...
    if (PREDICT_TRUE(p != 0)) {
        u32 next =
            (iuvn4_0->vxlan.protocol < VXLAN_GPE_PROTOCOL_MAX) ? nngm->decap_next_node_list[iuvn4_0->vxlan.protocol] : VXLAN_GPE_INPUT_NEXT_DROP;
...
always_inline vxlan_gpe_decap_info_t vxlan6_gpe_find_tunnel(vxlan_gpe_main_t *nngm, vxlan6_gpe_tunnel_cache_t *cache, ip6_vxlan_gpe_header_t *iuvn6_0)
{
    /* Make sure VXLAN GPE tunnel exist according to packet S/D IP, UDP port and
     * VNI */
    ...
    if (PREDICT_TRUE(p != 0)) {
        u32 next =
            (iuvn6_0->vxlan.protocol < VXLAN_GPE_PROTOCOL_MAX) ? nngm->decap_next_node_list[iuvn6_0->vxlan.protocol] : VXLAN_GPE_INPUT_NEXT_DROP;
 ...
 *
 */
always_inline uword vxlan_gpe_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, u8 is_ip4)
{
    ...
    vlib_node_increment_counter(vm,
                                is_ip4 ? vxlan4_gpe_input_node.index : vxlan6_gpe_input_node.index,
 ...
 *
 */
VLIB_NODE_FN(vxlan4_gpe_input_node)
 ...
 *
 */
VLIB_NODE_FN(vxlan6_gpe_input_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
always_inline uword ip_vxlan_gpe_bypass_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u32 is_ip4)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (PREDICT_FALSE(di0.tunnel_index == ~0)) goto exit0; /* unknown interface */

            /* Validate DIP against VTEPs */
            if (is_ip4) {
#ifdef CLIB_HAVE_VEC512
                if (!vtep4_check_vector(&ngm->vtep_table, b0, ip40, &last_vtep4, &ngm->vtep4_u512))
#else
                ...
                if (!vtep6_check(&ngm->vtep_table, b0, ip60, &last_vtep6)) goto exit0; /* no local VTEP for VXLAN packet */
            }
        ...
        exit0:
            ...
            if (PREDICT_FALSE(di1.tunnel_index == ~0)) goto exit1; /* unknown interface */

            /* Validate DIP against VTEPs */
            if (is_ip4) {
#ifdef CLIB_HAVE_VEC512
                if (!vtep4_check_vector(&ngm->vtep_table, b1, ip41, &last_vtep4, &ngm->vtep4_u512))
#else
                ...
                if (!vtep6_check(&ngm->vtep_table, b1, ip61, &last_vtep6)) goto exit1; /* no local VTEP for VXLAN packet */
            }
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(di0.tunnel_index == ~0)) goto exit; /* unknown interface */
            ...
            if (is_ip4) {
                ...
                if (!vtep6_check(&ngm->vtep_table, b0, ip60, &last_vtep6)) goto exit; /* no local VTEP for VXLAN packet */
            }
...
VLIB_NODE_FN(ip4_vxlan_gpe_bypass_node)
...
VLIB_NODE_FN(ip6_vxlan_gpe_bypass_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
