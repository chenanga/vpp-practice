...
teib_entry_t *teib_entry_find(u32 sw_if_index, const ip_address_t *peer)
{
    ...
    if (NULL != p) return teib_entry_get(p[0]);
...
static void teib_adj_fib_add(const ip_address_t *ip, u32 sw_if_index, u32 peer_fib_index)
{
    if (AF_IP6 == ip_addr_version(ip) && ip6_address_is_link_local_unicast(&ip_addr_v6(ip))) {
        ip6_ll_prefix_t pfx = {
        ...
        fib_table_entry_path_add(peer_fib_index,
                                 &pfx,
                                 FIB_SOURCE_ADJ,
        ...
        if (0 == teib_db.td_n_entries[ip_addr_version(ip)]++) fib_table_lock(peer_fib_index, pfx.fp_proto, FIB_SOURCE_ADJ);
    }
}

static void teib_adj_fib_remove(ip_address_t *ip, u32 sw_if_index, u32 peer_fib_index)
{
    if (AF_IP6 == ip_addr_version(ip) && ip6_address_is_link_local_unicast(&ip_addr_v6(ip))) {
        ip6_ll_prefix_t pfx = {
            .ilp_addr        = ip_addr_v6(ip),
        ...
        ip_address_to_fib_prefix(ip, &pfx);
        fib_table_entry_path_remove(peer_fib_index,
                                    &pfx,
                                    FIB_SOURCE_ADJ,
                                    fib_proto_to_dpo(pfx.fp_proto),
                                    &pfx.fp_addr,
        ...
        if (0 == --teib_db.td_n_entries[ip_addr_version(ip)]) fib_table_unlock(peer_fib_index, pfx.fp_proto, FIB_SOURCE_ADJ);
    }
}
...
int teib_entry_add(u32 sw_if_index, const ip_address_t *peer, u32 nh_table_id, const ip_address_t *nh)
{
    ...
    nh_proto = (AF_IP4 == ip_addr_version(nh) ? FIB_PROTOCOL_IP4 : FIB_PROTOCOL_IP6);

    peer_fib_index = fib_table_get_index_for_sw_if_index(ip_address_family_to_fib_proto(peer->version), sw_if_index);
...
int teib_entry_del(u32 sw_if_index, const ip_address_t *peer)
{
    ...
    if (te != NULL) {
        ...
        TEIB_INFO("no such entry: %U, %U", format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index, format_ip_address, peer);
...
u8 *format_teib_entry(u8 *s, va_list *args)
{
    ...
    s = format(s, "[%d] ", tei);
    s = format(s, "%U:", format_vnet_sw_if_index_name, vnm, te->te_key->tk_sw_if_index);
    s = format(s, "%U", format_ip_address, &te->te_key->tk_peer, IP46_TYPE_ANY);
    s = format(s, " via [%d]:%U", fib_table_get_table_id(te->te_nh_fib_index, te->te_nh.fp_proto), format_fib_prefix, &te->te_nh);
...
void teib_walk_itf(u32 sw_if_index, teib_walk_cb_t fn, void *ctx)
{
    ...
    pool_foreach_index (tei, teib_pool) {
        if (sw_if_index == teib_entry_get_sw_if_index(teib_entry_get(tei))) fn(tei, ctx);
    }
}
...
static void teib_walk_itf_proto(u32 sw_if_index, ip_address_family_t af, teib_walk_cb_t fn, void *ctx)
{
    ...
    pool_foreach_index (tei, teib_pool) {
        if (sw_if_index == teib_entry_get_sw_if_index(teib_entry_get(tei)) && af == teib_entry_get_af(teib_entry_get(tei))) fn(tei, ctx);
    }
}
...
static walk_rc_t teib_walk_table_bind(index_t tei, void *arg)
{
    ...
    TEIB_TE_INFO(te, "bind: %d -> %d", ctx->old_peer_fib_index, ctx->new_peer_fib_index);

    teib_adj_fib_remove(&te->te_key->tk_peer, te->te_key->tk_sw_if_index, ctx->old_peer_fib_index);
    teib_adj_fib_add(&te->te_key->tk_peer, te->te_key->tk_sw_if_index, ctx->new_peer_fib_index);
