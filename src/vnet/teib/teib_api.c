...
static void vl_api_teib_entry_add_del_t_handler(vl_api_teib_entry_add_del_t *mp)
{
    ...
    if (mp->is_add)
        rv = teib_entry_add(ntohl(mp->entry.sw_if_index), &peer, ntohl(mp->entry.nh_table_id), &nh);
    else
...
static walk_rc_t vl_api_teib_send_one(index_t nei, void *arg)
{
    ...
    mp->entry.nh_table_id = htonl(fib_table_get_table_id(teib_entry_get_fib_index(ne), pfx->fp_proto));
