...
static int api_hw_interface_set_mtu(vat_main_t *vam)
{
    unformat_input_t              *i = vam->input;
...
static int api_sw_interface_tag_add_del(vat_main_t *vam)
{
    unformat_input_t                  *i = vam->input;
    ...
    mp->sw_if_index = ntohl(sw_if_index);
    mp->is_add      = enable;
    if (enable) strncpy((char *) mp->tag, (char *) tag, ARRAY_LEN(mp->tag) - 1);
    vec_free(tag);
...
static int api_sw_interface_add_del_mac_address(vat_main_t *vam)
{
    unformat_input_t                          *i   = vam->input;
...
static void vl_api_sw_interface_details_t_handler(vl_api_sw_interface_details_t *mp)
{
    ...
    if (mp->sw_if_index != mp->sup_sw_if_index) {
        ...
        strncpy((char *) sub->interface_name, (char *) s, vec_len(sub->interface_name));
...
static int api_sw_interface_get_mac_address(vat_main_t *vat)
{
    return -1;
}
...
static int api_sw_interface_add_del_address(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "del-all"))
            ...
            sw_if_index_set = 1;
        else if (unformat(i, "%U/%d", unformat_ip4_address, &v4address, &address_length))
            v4_address_set = 1;
        else if (unformat(i, "%U/%d", unformat_ip6_address, &v6address, &address_length))
            v6_address_set = 1;
        else
            break;
    }
...
static int api_sw_interface_get_table(vat_main_t *vam)
{
    unformat_input_t                *i = vam->input;
...
static int api_sw_interface_set_rx_mode(vat_main_t *vam)
{
    unformat_input_t                  *i = vam->input;
...
static void vl_api_sw_interface_get_table_reply_t_handler(vl_api_sw_interface_get_table_reply_t *mp)
{
...
static int api_sw_interface_address_replace_begin(vat_main_t *vam)
{
    return -1;
}

static int api_sw_interface_set_mac_address(vat_main_t *vam)
{
    return -1;
}

static int api_sw_interface_set_rx_placement(vat_main_t *vam)
{
    unformat_input_t                       *i = vam->input;
...
static int api_sw_interface_set_tx_placement(vat_main_t *vam)
{
    unformat_input_t                       *i = vam->input;
...
static int api_interface_name_renumber(vat_main_t *vam)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%U", api_unformat_sw_if_index, vam, &sw_if_index))
            ;
        else if (unformat(line_input, "sw_if_index %d", &sw_if_index))
            ;
        else if (unformat(line_input, "new_show_dev_instance %d", &new_show_dev_instance))
            ;
        else
            break;
    }
...
static int api_create_loopback_instance(vat_main_t *vat)
{
    return -1;
}
...
static int api_create_loopback(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "mac %U", unformat_ethernet_address, mac_address)) mac_set = 1;
    ...
    if (is_specified) {
        ...
        if (is_specified) mp_lbi->user_instance = htonl(user_instance);
        if (mac_set) clib_memcpy(mp_lbi->mac_address, mac_address, sizeof(mac_address));
        S(mp_lbi);
    }
    else {
        /* Construct the API message */
        M(CREATE_LOOPBACK, mp);
        if (mac_set) clib_memcpy(mp->mac_address, mac_address, sizeof(mac_address));
        S(mp);
    }
...
static void vl_api_create_subif_reply_t_handler(vl_api_create_subif_reply_t *mp)
{
    vat_main_t *vam   = interface_test_main.vat_main;
    vam->result_ready = 1;
}
...
static void vl_api_sw_interface_rx_placement_details_t_handler(vl_api_sw_interface_rx_placement_details_t *mp)
{
    vat_main_t *vam       = interface_test_main.vat_main;
    ...
    print(vam->ofp,
          ...
          (worker_id == 0) ? "main" : "worker",
          worker_id,
          ntohl(mp->queue_id),
          (mp->mode == 1) ? "polling" : ((mp->mode == 2) ? "interrupt" : "adaptive"));
}

static __clib_unused void vl_api_sw_interface_tx_placement_details_t_handler(vl_api_sw_interface_tx_placement_details_t *mp)
{
    vat_main_t *vam    = interface_test_main.vat_main;
...
static void vl_api_create_vlan_subif_reply_t_handler(vl_api_create_vlan_subif_reply_t *mp)
{
    vat_main_t *vam   = interface_test_main.vat_main;
    vam->result_ready = 1;
}

static void vl_api_create_loopback_reply_t_handler(vl_api_create_loopback_reply_t *mp)
{
    vat_main_t *vam   = interface_test_main.vat_main;
    vam->result_ready = 1;
}

static void vl_api_create_loopback_instance_reply_t_handler(vl_api_create_loopback_instance_reply_t *mp)
{
    vat_main_t *vam   = interface_test_main.vat_main;
    vam->result_ready = 1;
}
...
static int api_collect_detailed_interface_stats(vat_main_t *vam)
{
    return -1;
}

static int api_sw_interface_rx_placement_dump(vat_main_t *vam)
{
    unformat_input_t                        *i = vam->input;
    ...
    fformat(vam->ofp, "\n%-11s %-11s %-6s %-5s %-4s", "sw_if_index", "main/worker", "thread", "queue", "mode");
...
static int api_sw_interface_tx_placement_get(vat_main_t *vam)
{
    unformat_input_t                       *i = vam->input;
    ...
    fformat(vam->ofp, "\n%-11s %-6s %-7s %-11s", "sw_if_index", "queue", "shared", "threads");
...
static void vl_api_sw_interface_tx_placement_get_reply_t_handler()
{
}
...
static int api_sw_interface_address_replace_end(vat_main_t *vam)
{
    return -1;
}

static int api_sw_interface_set_ip_directed_broadcast(vat_main_t *vam)
{
    return -1;
}

static int api_sw_interface_set_mtu(vat_main_t *vam)
{
    return -1;
}

static int api_sw_interface_set_promisc(vat_main_t *vam)
{
    return -1;
}
...
static int api_sw_interface_set_interface_name(vat_main_t *vam)
{
    return -1;
}

static int api_pcap_set_filter_function(vat_main_t *vam)
{
...
static int api_pcap_trace_on(vat_main_t *vam)
{
    return -1;
}

static int api_pcap_trace_off(vat_main_t *vam)
{
    return -1;
}
