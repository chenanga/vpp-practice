...
int p2p_ethernet_add_del(vlib_main_t *vm, u32 parent_if_index, u8 *client_mac, u32 p2pe_subif_id, int is_add, u32 *p2pe_if_index)
{
    ...
    if (p2pe_if_index) *p2pe_if_index = ~0;
    ...
    if (is_add) {
        if (p2pe_sw_if_index == ~0) {
            ...
            if (hi->bond_info == VNET_HW_INTERFACE_BOND_INFO_SLAVE) return VNET_API_ERROR_BOND_SLAVE_NOT_ALLOWED;

            u64    sup_and_sub_key = ((u64) (hi->sw_if_index) << 32) | (u64) p2pe_subif_id;
            uword *p;
            p = hash_get_mem(im->sw_if_index_by_sup_and_sub, &sup_and_sub_key);
            if (p) {
                if (CLIB_DEBUG > 0)
                    clib_warning("p2p ethernet sub-interface on sw_if_index %d with sub id %d already exists\n", hi->sw_if_index, p2pe_subif_id);
            ...
            clib_memcpy(template.p2p.client_mac, client_mac, sizeof(template.p2p.client_mac));

            if (vnet_create_sw_interface(vnm, &template, &p2pe_sw_if_index)) return VNET_API_ERROR_SUBIF_CREATE_FAILED;
            ...
            /* Allocate counters for this interface. */
            {
                ...
                for (i = 0; i < vec_len(im->sw_if_counters); i++) {
                    vlib_validate_simple_counter(&im->sw_if_counters[i], p2pe_sw_if_index);
                    vlib_zero_simple_counter(&im->sw_if_counters[i], p2pe_sw_if_index);
                }

                for (i = 0; i < vec_len(im->combined_sw_if_counters); i++) {
                    vlib_validate_combined_counter(&im->combined_sw_if_counters[i], p2pe_sw_if_index);
                    vlib_zero_combined_counter(&im->combined_sw_if_counters[i], p2pe_sw_if_index);
                }
            ...
            vnet_interface_main_t *im = &vnm->interface_main;
            sup_and_sub_key           = ((u64) (hi->sw_if_index) << 32) | (u64) p2pe_subif_id;
            u64 *kp                   = clib_mem_alloc(sizeof(*kp));

            *kp = sup_and_sub_key;
            hash_set(hi->sub_interface_sw_if_index_by_id, p2pe_subif_id, p2pe_sw_if_index);
            ...
            hash_set_mem(p2pm->p2p_ethernet_by_key, p_p2pe_key, p2pe_sw_if_index);

            if (p2pe_if_index) *p2pe_if_index = p2pe_sw_if_index;

            vec_validate(p2pm->p2p_ethernet_by_sw_if_index, parent_if_index);
            if (p2pm->p2p_ethernet_by_sw_if_index[parent_if_index] == 0) {
                vnet_feature_enable_disable("device-input", "p2p-ethernet-input", parent_if_index, 1, 0, 0);
                vnet_feature_enable_disable("port-rx-eth", "p2p-ethernet-input", parent_if_index, 1, 0, 0);
                /* Set promiscuous mode on the l2 interface */
                ethernet_set_flags(vnm, parent_if_index, ETHERNET_INTERFACE_FLAG_ACCEPT_ALL);
            }
            p2pm->p2p_ethernet_by_sw_if_index[parent_if_index]++;
            /* set the interface mode */
            set_int_l2_mode(vm, vnm, MODE_L3, p2pe_sw_if_index, 0, L2_BD_PORT_TYPE_NORMAL, 0, 0);
            return 0;
        }
        return VNET_API_ERROR_SUBIF_ALREADY_EXISTS;
    }
        ...
        if (p2pe_sw_if_index == ~0)
            ...
            if (!rv) {
                ...
                if (p2pm->p2p_ethernet_by_sw_if_index[parent_if_index] == 1) {
                    vnet_feature_enable_disable("device-input", "p2p-ethernet-input", parent_if_index, 0, 0, 0);
                    vnet_feature_enable_disable("port-rx-eth", "p2p-ethernet-input", parent_if_index, 0, 0, 0);
...
static clib_error_t *vnet_p2p_ethernet_add_del(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (hw_if_index == ~0) return clib_error_return(0, "Please specify parent interface ...");
    if (!remote_mac) return clib_error_return(0, "Please specify client MAC address ...");
    if (sub_id == ~0 && is_add) return clib_error_return(0, "Please specify sub-interface id ...");

    u32 rv;
    rv = p2p_ethernet_add_del(vm, hw_if_index, client_mac, sub_id, is_add, 0);
    switch (rv) {
        case VNET_API_ERROR_BOND_SLAVE_NOT_ALLOWED:
            return clib_error_return(0, "not allowed as parent interface belongs to a BondEthernet interface");
        case -1:
            return clib_error_return(0, "p2p ethernet for given parent interface and client mac already exists");
        case -2:
            return clib_error_return(0, "couldn't create p2p ethernet subinterface");
        case -3:
            return clib_error_return(0, "p2p ethernet for given parent interface and client mac doesn't exist");
        default:
            break;
    }
    return 0;
...
static clib_error_t *p2p_ethernet_init(vlib_main_t *vm)
{
    ...
    p2pm->p2p_ethernet_by_key = hash_create_mem(0, sizeof(p2p_key_t), sizeof(uword));
