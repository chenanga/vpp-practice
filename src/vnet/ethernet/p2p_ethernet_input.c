...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    vlib_combined_counter_main_t *cm                     = vnet_get_main()->interface_main.combined_sw_if_counters;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (rx0 != ~0) {
                ...
                if (PREDICT_FALSE(n_trace > 0 && vlib_trace_buffer(vm, node, next_index, b0, 1 /* follow_chain */))) {
                ...
                vlib_increment_combined_counter(cm, thread_index, rx0, 1, vlib_buffer_length_in_chain(vm, b0));
            }
            if (rx1 != ~0) {
                /* Send pkt to p2p_ethernet RX interface */
                ...
                if (PREDICT_FALSE(n_trace > 0 && vlib_trace_buffer(vm, node, next_index, b1, 1 /* follow_chain */))) {
                ...
                vlib_increment_combined_counter(cm, thread_index, rx1, 1, vlib_buffer_length_in_chain(vm, b1));
            }

            /* verify speculative enqueue, maybe switch current next frame */
            vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, bi0, next0);
            vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, bi1, next1);
        }
    ...
    vlib_node_increment_counter(vm, p2p_ethernet_input_node.index, P2PE_ERROR_HITS, n_p2p_ethernet_packets);
