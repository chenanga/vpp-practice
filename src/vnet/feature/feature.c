...
static clib_error_t *vnet_feature_init(vlib_main_t *vm)
{
    ...
    while (areg) {
        ...
        if (areg->arc_index_ptr) *areg->arc_index_ptr = arc_index;
    ...
    while (areg) {
        ...
        vcm       = &cm->config_main;
        if ((error = vnet_feature_arc_init(vm,
                                           vcm,
                                           areg->start_nodes,
                                           areg->n_start_nodes,
                                           areg->last_in_arc,
        ...
        if (areg->last_in_arc && vec_len(features_in_order) > 0) {
            ...
            if (strncmp(areg->last_in_arc, last_feature, strlen(areg->last_in_arc)))
                clib_warning("WARNING: %s arc: last node is %s, but expected %s!", areg->arc_name, last_feature, areg->last_in_arc);
        }
                                           ...
                                           u32   n_feature_config_bytes)
{
    ...
    if (feature_index == ~0) return VNET_API_ERROR_INVALID_VALUE_2;
    ...
    if (!enable_disable && feature_count < 1) return 0;

    ci = (enable_disable ? vnet_config_add_feature
                         : vnet_config_del_feature)(vlib_get_main(), &cm->config_main, ci, feature_index, feature_config, n_feature_config_bytes);
    if (ci == ~0) {
        return 0;
    }
    ...
    fm->sw_if_index_has_features[arc_index] = clib_bitmap_set(fm->sw_if_index_has_features[arc_index], sw_if_index, (feature_count > 0));
                                ...
                                u32         n_feature_config_bytes)
{
    ...
    return vnet_feature_enable_disable_with_index(arc_index, feature_index, sw_if_index, enable_disable, feature_config, n_feature_config_bytes);
}
...
int vnet_feature_is_enabled(const char *arc_name, const char *feature_node_name, u32 sw_if_index)
{
    ...
    /* No such feature? */
    if (feature_index == (u32) ~0) return VNET_API_ERROR_INVALID_VALUE_2;
    ...
    vec_foreach (f, current_config->features) {
        if (f->feature_index == feature_index) /* Feature was enabled */
            return 1;
    }
...
u32 vnet_feature_reset_end_node(u8 arc_index, u32 sw_if_index)
{
    ...
    if (ci != ~0) cm->config_index_by_sw_if_index[sw_if_index] = ci;

    i16 feature_count;

    if (NULL == fm->feature_count_by_sw_if_index || vec_len(fm->feature_count_by_sw_if_index) <= arc_index
...
u32 vnet_feature_modify_end_node(u8 arc_index, u32 sw_if_index, u32 end_node_index)
{
    ...
    if (end_node_index == ~0) return VNET_API_ERROR_INVALID_VALUE_2;
...
void vnet_interface_features_show(vlib_main_t *vm, u32 sw_if_index, int verbose)
{
    ...
    vlib_cli_output(vm, "Feature paths configured on %U...", format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index);
    ...
    while (areg) {
        ...
        current_config_index = vec_elt(cm[feature_arc].config_index_by_sw_if_index, sw_if_index);
        cfg_index            = vec_elt(vcm->config_pool_index_by_user_index, current_config_index);
...
static clib_error_t *set_interface_features_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%U %s arc %s", unformat_vnet_sw_interface, vnm, &sw_if_index, &feature_name, &arc_name))
            ;
    ...
    if (arc_index == (u8) ~0) {
        error = clib_error_return(0, "Unknown arc name (%s)... ", (const char *) arc_name);
        goto done;
    }
    ...
    if (reg->enable_disable_cb) error = reg->enable_disable_cb(sw_if_index, enable);
    if (!error) vnet_feature_enable_disable((const char *) arc_name, (const char *) feature_name, sw_if_index, enable, 0, 0);
...
static clib_error_t *vnet_feature_add_del_sw_interface(vnet_main_t *vnm, u32 sw_if_index, u32 is_add)
{
    ...
    for (far = fm->next_arc; far != 0; far = far->next) {
        ...
        vnet_feature_config_main_t *cm        = vec_elt_at_index(fm->feature_config_mains, arc_index);
        const u32 ci = vec_len(cm->config_index_by_sw_if_index) <= sw_if_index ? ~0 : vec_elt(cm->config_index_by_sw_if_index, sw_if_index);

        if (~0 == ci) continue;

        fm->sw_if_index_has_features[arc_index] = clib_bitmap_set(fm->sw_if_index_has_features[arc_index], sw_if_index, 0);

        vnet_feature_reg_invoke(sw_if_index, arc_index, 0);

        if (vec_len(fm->feature_count_by_sw_if_index[arc_index]) > sw_if_index) vec_elt(fm->feature_count_by_sw_if_index[arc_index], sw_if_index) = 0;
