                                    ...
                                    char                                 ***in_feature_nodes)
{
    ...
    if (last_in_arc) {
        while (this_reg) {
            ...
            if (clib_strcmp(this_reg->node_name, last_in_arc)) {
                 ...
                 */
                constraint_tuple = format(0, "%s,%s%c", this_reg->node_name, last_in_arc, 0);
    ...
    while (this_reg) {
        ...
        while (these_constraints && these_constraints[0]) {
            ...
            constraint_tuple = format(0, "%s,%s%c", node_name, this_constraint_c, 0);
            vec_add1(constraints, constraint_tuple);
            these_constraints++;
        }
        ...
        while (these_constraints && these_constraints[0]) {
            ...
            constraint_tuple = format(0, "%s,%s%c", this_constraint_c, node_name, 0);
            vec_add1(constraints, constraint_tuple);
            these_constraints++;
        }
    ...
    for (i = 0; i < vec_len(constraints); i++) {
         ...
         */
        if (p == 0) {
            clib_warning("feature node '%s' not found (before '%s', arc '%s')", a_name, b_name, first_reg->arc_name);
            continue;
        }
        a_index = p[0];
        ...
        if (p == 0) {
            clib_warning("feature node '%s' not found (after '%s', arc '%s')", b_name, a_name, first_reg->arc_name);
            continue;
        }
        b_index = p[0];
...
again:
    ...
    if (vec_len(result) != n_features) return clib_error_return(0, "Arc '%s': failed to find a suitable feature order!", first_reg->arc_name);
    ...
    for (i = n_features - 1; i >= 0; i--) {
        ...
        if (this_reg->feature_index_ptr) *this_reg->feature_index_ptr = n_features - (i + 1);
        this_reg->feature_index = n_features - (i + 1);
        vec_add1(feature_nodes, this_reg->node_name);
    }

    /* Set up the config infrastructure */
    vnet_config_init(vm, vcm, feature_start_nodes, num_feature_start_nodes, feature_nodes, vec_len(feature_nodes));
