...
static_always_inline void esp_process_ops(vlib_main_t *vm, vlib_node_runtime_t *node, vnet_crypto_op_t *ops, vlib_buffer_t *b[], u16 *nexts, int e)
{
    ...
    while (n_fail) {
        ...
        if (op->status != VNET_CRYPTO_OP_STATUS_COMPLETED) {
            ...
            esp_decrypt_set_next_index(b[bi], node, vm->thread_index, err, bi, nexts, ESP_DECRYPT_NEXT_DROP, vnet_buffer(b[bi])->ipsec.sad_index);
            n_fail--;
        }
        op++;
    }
                                                  ...
                                                  int                     e)
{
    ...
    if (PREDICT_TRUE(n_ops == 0)) return;
...
always_inline void esp_remove_tail(vlib_main_t *vm, vlib_buffer_t *b, vlib_buffer_t *last, u16 tail)
{
    vlib_buffer_t *before_last = b;

    if (b != last) b->total_length_not_including_first_buffer -= tail;
...
always_inline void esp_remove_tail_and_tfc_padding(vlib_main_t                     *vm,
                                                   vlib_node_runtime_t             *node,
                                                   ...
                                                   int                              is_ip6)
{
    ...
    if (is_ip6) {
        ...
        ip_packet_length        = clib_net_to_host_u16(ip6->payload_length) + sizeof(ip6_header_t);
    }
    else {
    ...
    if (PREDICT_FALSE(total_buffer_length < ip_packet_length + tail)) {
        esp_decrypt_set_next_index(b, node, vm->thread_index, ESP_DECRYPT_ERROR_NO_TAIL_SPACE, 0, next, ESP_DECRYPT_NEXT_DROP, pd->sa_index);
        return;
    }
                                      ...
                                      u16                        *dif)
{
    ...
    clib_memcpy_fast(lb_curr, vlib_buffer_get_tail(before_last) - first_sz, first_sz);
...
static_always_inline u16 esp_insert_esn(vlib_main_t                *vm,
                                        ...
                                        u8                         *payload)
{
    if (!ipsec_sa_is_set_USE_ESN(sa)) return 0;
                                                 ...
                                                 u32                             *integ_total_len)
{
    ...
    while (1) {
            ...
            if (ipsec_sa_is_set_USE_ESN(sa0)) {
                ...
                if (space_left < N_HI_ESN_BYTES) {
                    if (pd2->icv_removed) {
                        ...
                        u32 tmp_bi = 0;
                        if (vlib_buffer_alloc(vm, &tmp_bi, 1) != 1) return -1;
        ...
        if (!(cb->flags & VLIB_BUFFER_NEXT_PRESENT)) break;

        cb = vlib_get_buffer(vm, cb->next_buffer);
    }

    if (n_ch) *n_ch = n_chunks;
    if (integ_total_len) *integ_total_len = total_len;
                ...
                if (pd2->lb->current_length < icv_sz) {
                    ...
                    *tag    = esp_move_icv(vm, b, pd, pd2, icv_sz, &dif);
                    ...
                    if (pd2->lb == b) {
                        ...
                        total_len  = total_len + pd2->lb->current_length - ch[-1].len;
...
static_always_inline esp_decrypt_error_t esp_decrypt_prepare_sync_op(vlib_main_t                *vm,
                                                                     ipsec_per_thread_data_t    *ptd,
                                                                     ipsec_sa_t                 *sa0,
                                                                     u8                         *payload,
                                                                     u16                         len,
                                                                     ...
                                                                     u32                         index)
{
    ...
    if (PREDICT_TRUE(sa0->integ_op_id != VNET_CRYPTO_OP_NONE)) {
        ...
        if (pd->is_chain) {
            ...
            if (pd2->lb->current_length < icv_sz) {
                u8 extra_esn = 0;
                op->digest   = esp_move_icv_esn(vm, b, pd, pd2, icv_sz, sa0, &extra_esn, &op->len);
            ...
            if (esp_decrypt_chain_integ(vm, ptd, pd, pd2, sa0, b, icv_sz, payload, pd->current_length, &op->digest, &op->n_chunks, 0) < 0)
                return ESP_DECRYPT_ERROR_NO_BUFFERS;
        }
        else {
            integ_ops = &ptd->integ_ops;
            esp_insert_esn(vm, sa0, pd, pd2, &op->len, &op->digest, &len, b, payload);
        }
    out:
    ...
    if (sa0->crypto_dec_op_id != VNET_CRYPTO_OP_NONE) {
        ...
        if (ipsec_sa_is_set_IS_CTR(sa0)) {
            ...
            esp_ctr_nonce_t *nonce = (esp_ctr_nonce_t *) (payload - esp_sz - pd->hdr_sz - sizeof(*nonce));
            if (ipsec_sa_is_set_IS_AEAD(sa0)) {
        ...
        if (pd->is_chain && (pd2->lb != b)) {
            ...
            op->flags |= VNET_CRYPTO_OP_FLAG_CHAINED_BUFFERS;
            op->chunk_index = vec_len(ptd->chunks);
            esp_decrypt_chain_crypto(vm, ptd, pd, pd2, sa0, b, icv_sz, payload, len - pd->iv_sz + pd->icv_sz, &op->tag, &op->n_chunks);
                                                                         ...
                                                                         u16                         async_next)
{
    ...
    if (!ipsec_sa_is_set_IS_AEAD(sa0)) {
        ...
        integ_len          = len;
        if (PREDICT_TRUE(sa0->integ_op_id != VNET_CRYPTO_OP_NONE)) flags |= VNET_CRYPTO_OP_FLAG_HMAC_CHECK;
        ...
        if (pd->is_chain) {
            ...
            if (pd2->lb->current_length < icv_sz) {
                u8 extra_esn = 0;
                tag          = esp_move_icv_esn(vm, b, pd, pd2, icv_sz, sa0, &extra_esn, &integ_len);
            ...
            if (esp_decrypt_chain_integ(vm, ptd, pd, pd2, sa0, b, icv_sz, payload, pd->current_length, &tag, 0, &integ_len) < 0) {
...
out:
    ...
    if (pd->is_chain && (pd2->lb != b)) {
        ...
        crypto_len = esp_decrypt_chain_crypto(vm, ptd, pd, pd2, sa0, b, icv_sz, payload, len - pd->iv_sz + pd->icv_sz, &tag, 0);
    }
    ...
    *async_pd2 = *pd2;
    ...
    vnet_crypto_async_add_to_frame(vm,
                                   f,
                                   key_index,
                                   crypto_len,
                                                  ...
                                                  int                               is_async)
{
    ...
    if (PREDICT_FALSE(ipsec_sa_is_set_ANTI_REPLAY_HUGE(sa0))) {
        if (ipsec_sa_anti_replay_and_sn_advance(sa0, pd->seq, pd->seq_hi, true, NULL, true)) {
            esp_decrypt_set_next_index(b, node, vm->thread_index, ESP_DECRYPT_ERROR_REPLAY, 0, next, ESP_DECRYPT_NEXT_DROP, pd->sa_index);
            return;
        }
        n_lost = ipsec_sa_anti_replay_advance(sa0, vm->thread_index, pd->seq, pd->seq_hi, true);
    }
    else {
        if (ipsec_sa_anti_replay_and_sn_advance(sa0, pd->seq, pd->seq_hi, true, NULL, false)) {
            esp_decrypt_set_next_index(b, node, vm->thread_index, ESP_DECRYPT_ERROR_REPLAY, 0, next, ESP_DECRYPT_NEXT_DROP, pd->sa_index);
            return;
        }
        n_lost = ipsec_sa_anti_replay_advance(sa0, vm->thread_index, pd->seq, pd->seq_hi, false);
    }

    vlib_prefetch_simple_counter(&ipsec_sa_err_counters[IPSEC_SA_ERROR_LOST], vm->thread_index, pd->sa_index);
    ...
    if (pd->is_chain) {
        ...
        if (lb->current_length < sizeof(esp_footer_t) + icv_sz) {
            ...
            esp_footer_t *f = (esp_footer_t *) (lb->data + lb->current_data + lb->current_length - sizeof(esp_footer_t) - icv_sz);
            pad_length      = f->pad_length;
            next_header     = f->next_header;
        }
    }
    ...
    {
        u8  udp_sz    = (is_ip6 == 0 && pd->flags & IPSEC_SA_FLAG_UDP_ENCAP) ? sizeof(udp_header_t) : 0;
        ...
        if (is_ip6) {
            ...
            sum               = ip_csum_update(sum, ip4->protocol, next_header, ip4_header_t, protocol);
        ...
        if (PREDICT_TRUE(next_header == IP_PROTOCOL_IP_IN_IP)) {
            ...
            esp_remove_tail_and_tfc_padding(vm, node, pd, b, lb, next, tail, false);
        }
        else if (next_header == IP_PROTOCOL_IPV6) {
            next[0]           = ESP_DECRYPT_NEXT_IP6_INPUT;
            b->current_data   = pd->current_data + adv;
            b->current_length = pd->current_length - adv;
            esp_remove_tail_and_tfc_padding(vm, node, pd, b, lb, next, tail, true);
        }
        ...
        else if ((next[0] = vec_elt(next_by_next_header, next_header)) != (u16) ~0) {
            b->current_data   = pd->current_data + adv;
            b->current_length = pd->current_length - adv;
            esp_remove_tail(vm, b, lb, tail);
        }
        else {
            esp_decrypt_set_next_index(b, node, vm->thread_index, ESP_DECRYPT_ERROR_UNSUP_PAYLOAD, 0, next, ESP_DECRYPT_NEXT_DROP, pd->sa_index);
            return;
        }
        ...
        if (is_tun) {
            if (ipsec_sa_is_set_IS_PROTECT(sa0)) {
                ...
                itp = ipsec_tun_protect_get(vnet_buffer(b)->ipsec.protect_index);
                ...
                if (PREDICT_TRUE(next_header == IP_PROTOCOL_IP_IN_IP)) {
                    ...
                    if (!ip46_address_is_equal_v4(&itp->itp_tun.src, &ip4->dst_address)
                        || !ip46_address_is_equal_v4(&itp->itp_tun.dst, &ip4->src_address)) {
                        esp_decrypt_set_next_index(b,
                                                   node,
                                                   vm->thread_index,
                    ...
                    if (!ip46_address_is_equal_v6(&itp->itp_tun.src, &ip6->dst_address)
                        || !ip46_address_is_equal_v6(&itp->itp_tun.dst, &ip6->src_address)) {
                        esp_decrypt_set_next_index(b,
                                                   node,
                                                   vm->thread_index,
                                                   ESP_DECRYPT_ERROR_TUN_NO_PROTO,
    ...
    if (PREDICT_FALSE(n_lost)) vlib_increment_simple_counter(&ipsec_sa_err_counters[IPSEC_SA_ERROR_LOST], vm->thread_index, pd->sa_index, n_lost);
}

always_inline uword
esp_decrypt_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, int is_ip6, int is_tun, u16 async_next_node)
{
    ...
    while (n_left > 0) {
        ...
        if (n_bufs == 0) {
            ...
            esp_decrypt_set_next_index(b[0], node, thread_index, err, n_noop, noop_nexts, ESP_DECRYPT_NEXT_DROP, vnet_buffer(b[0])->ipsec.sad_index);
            goto next;
        }

        if (vnet_buffer(b[0])->ipsec.sad_index != current_sa_index) {
            if (current_sa_pkts)
                vlib_increment_combined_counter(&ipsec_sa_counters, thread_index, current_sa_index, current_sa_pkts, current_sa_bytes);
            current_sa_bytes = current_sa_pkts = 0;

            current_sa_index = vnet_buffer(b[0])->ipsec.sad_index;
            vlib_prefetch_combined_counter(&ipsec_sa_counters, thread_index, current_sa_index);
        ...
        if (PREDICT_FALSE((u16) ~0 == sa0->thread_index)) {
             ...
             * another thread */
            clib_atomic_cmp_and_swap(&sa0->thread_index, ~0, ipsec_sa_assign_thread(thread_index));
        }

        if (PREDICT_FALSE(thread_index != sa0->thread_index)) {
            vnet_buffer(b[0])->ipsec.thread_index = sa0->thread_index;
            err                                   = ESP_DECRYPT_ERROR_HANDOFF;
            esp_decrypt_set_next_index(b[0], node, thread_index, err, n_noop, noop_nexts, ESP_DECRYPT_NEXT_HANDOFF, current_sa_index);
            goto next;
        }
        ...
        /* anti-reply check */
        if (PREDICT_FALSE(ipsec_sa_is_set_ANTI_REPLAY_HUGE(sa0))) {
            anti_replay_result = ipsec_sa_anti_replay_and_sn_advance(sa0, pd->seq, ~0, false, &pd->seq_hi, true);
        }
        else {
            anti_replay_result = ipsec_sa_anti_replay_and_sn_advance(sa0, pd->seq, ~0, false, &pd->seq_hi, false);
        }

        if (anti_replay_result) {
            err = ESP_DECRYPT_ERROR_REPLAY;
            esp_decrypt_set_next_index(b[0], node, thread_index, err, n_noop, noop_nexts, ESP_DECRYPT_NEXT_DROP, current_sa_index);
            goto next;
        }
        ...
        if (is_async) {
             ...
             */
            if (NULL == async_frames[async_op] || vnet_crypto_async_frame_is_full(async_frames[async_op])) {
            ...
            err = esp_decrypt_prepare_async_frame(vm,
                                                  ptd,
            ...
            err = esp_decrypt_prepare_sync_op(vm, ptd, sa0, payload, len, cpd.icv_sz, cpd.iv_sz, pd, pd2, b[0], n_sync);
            if (err != ESP_DECRYPT_ERROR_RX_PKTS) {
                esp_decrypt_set_next_index(b[0], node, thread_index, err, 0, sync_next, ESP_DECRYPT_NEXT_DROP, current_sa_index);
            }
        }
        /* next */
    next:
    ...
    vec_foreach (async_frame, ptd->async_frames) {
        ...
        if (vnet_crypto_async_submit_open_frame(vm, *async_frame) < 0) {
            n_noop += esp_async_recycle_failed_submit(vm,
                                                      *async_frame,
                                                      node,
                                                      ESP_DECRYPT_ERROR_CRYPTO_ENGINE_ERROR,
                                                      IPSEC_SA_ERROR_CRYPTO_ENGINE_ERROR,
                                                      n_noop,
                                                      noop_bi,
                                                      noop_nexts,
                                                      ESP_DECRYPT_NEXT_DROP,
    ...
    if (n_sync) {
        esp_process_ops(vm, node, ptd->integ_ops, sync_bufs, sync_nexts, ESP_DECRYPT_ERROR_INTEG_ERROR);
        esp_process_chained_ops(vm, node, ptd->chained_integ_ops, sync_bufs, sync_nexts, ptd->chunks, ESP_DECRYPT_ERROR_INTEG_ERROR);

        esp_process_ops(vm, node, ptd->crypto_ops, sync_bufs, sync_nexts, ESP_DECRYPT_ERROR_DECRYPTION_FAILED);
        esp_process_chained_ops(vm, node, ptd->chained_crypto_ops, sync_bufs, sync_nexts, ptd->chunks, ESP_DECRYPT_ERROR_DECRYPTION_FAILED);
    }
    ...
    while (n_left) {
        if (n_left >= 2) {
            ...
            /* esp_footer_t */
            CLIB_PREFETCH(data + pd[1].current_length - pd[1].icv_sz - 2, CLIB_CACHE_LINE_BYTES, LOAD);

            /* packet headers */
            CLIB_PREFETCH(data - CLIB_CACHE_LINE_BYTES, CLIB_CACHE_LINE_BYTES * 2, LOAD);
        }

        /* save the sa_index as GRE_teb post_crypto changes L2 opaque */
        if (PREDICT_FALSE(b[0]->flags & VLIB_BUFFER_IS_TRACED)) current_sa_index = vnet_buffer(b[0])->ipsec.sad_index;

        if (sync_next[0] >= ESP_DECRYPT_N_NEXT) esp_decrypt_post_crypto(vm, node, next_by_next_header, pd, pd2, b[0], sync_next, is_ip6, is_tun, 0);
    ...
    vlib_node_increment_counter(vm, node->node_index, ESP_DECRYPT_ERROR_RX_PKTS, from_frame->n_vectors);

    if (n_sync) vlib_buffer_enqueue_to_next(vm, node, sync_bi, sync_nexts, n_sync);

    if (n_noop) vlib_buffer_enqueue_to_next(vm, node, noop_bi, noop_nexts, n_noop);

    return (from_frame->n_vectors);
}

always_inline uword esp_decrypt_post_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, int is_ip6, int is_tun)
{
    ...
    while (n_left > 0) {
        ...
        if (!pd->is_chain)
            esp_decrypt_post_crypto(vm, node, next_by_next_header, pd, 0, b[0], next, is_ip6, is_tun, 1);
        else {
            esp_decrypt_packet_data2_t *pd2 = esp_post_data2(b[0]);
            esp_decrypt_post_crypto(vm, node, next_by_next_header, pd, pd2, b[0], next, is_ip6, is_tun, 1);
        }
        ...
        if (PREDICT_FALSE(b[0]->flags & VLIB_BUFFER_IS_TRACED)) {
            ipsec_sa_t                *sa0 = ipsec_sa_get(pd->sa_index);
            esp_decrypt_trace_t       *tr;
            esp_decrypt_packet_data_t *async_pd = &(esp_post_data(b[0]))->decrypt_data;
            tr                                  = vlib_add_trace(vm, node, b[0], sizeof(*tr));
    ...
    vlib_node_increment_counter(vm, node->node_index, ESP_DECRYPT_ERROR_RX_POST_PKTS, n_left);
...
VLIB_NODE_FN(esp4_decrypt_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return esp_decrypt_inline(vm, node, from_frame, 0, 0, esp_decrypt_async_next.esp4_post_next);
}

VLIB_NODE_FN(esp4_decrypt_post_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return esp_decrypt_post_inline(vm, node, from_frame, 0, 0);
}

VLIB_NODE_FN(esp4_decrypt_tun_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return esp_decrypt_inline(vm, node, from_frame, 0, 1, esp_decrypt_async_next.esp4_tun_post_next);
}

VLIB_NODE_FN(esp4_decrypt_tun_post_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return esp_decrypt_post_inline(vm, node, from_frame, 0, 1);
}

VLIB_NODE_FN(esp6_decrypt_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return esp_decrypt_inline(vm, node, from_frame, 1, 0, esp_decrypt_async_next.esp6_post_next);
}

VLIB_NODE_FN(esp6_decrypt_post_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return esp_decrypt_post_inline(vm, node, from_frame, 1, 0);
}

VLIB_NODE_FN(esp6_decrypt_tun_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return esp_decrypt_inline(vm, node, from_frame, 1, 1, esp_decrypt_async_next.esp6_tun_post_next);
}

VLIB_NODE_FN(esp6_decrypt_tun_post_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
static clib_error_t *esp_decrypt_init(vlib_main_t *vm)
{
    ...
    im->esp4_dec_fq_index     = vlib_frame_queue_main_init(esp4_decrypt_node.index, im->handoff_queue_size);
    im->esp6_dec_fq_index     = vlib_frame_queue_main_init(esp6_decrypt_node.index, im->handoff_queue_size);
    im->esp4_dec_tun_fq_index = vlib_frame_queue_main_init(esp4_decrypt_tun_node.index, im->handoff_queue_size);
    im->esp6_dec_tun_fq_index = vlib_frame_queue_main_init(esp6_decrypt_tun_node.index, im->handoff_queue_size);
