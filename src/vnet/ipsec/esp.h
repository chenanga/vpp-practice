...
always_inline int esp_seq_advance(ipsec_sa_t *sa)
{
    if (PREDICT_TRUE(ipsec_sa_is_set_USE_ESN(sa))) {
        if (PREDICT_FALSE(sa->seq == ESP_SEQ_MAX)) {
            if (PREDICT_FALSE(ipsec_sa_is_set_USE_ANTI_REPLAY(sa) && sa->seq_hi == ESP_SEQ_MAX)) return 1;
            sa->seq_hi++;
        }
        sa->seq++;
    }
    else {
        if (PREDICT_FALSE(ipsec_sa_is_set_USE_ANTI_REPLAY(sa) && sa->seq == ESP_SEQ_MAX)) return 1;
        sa->seq++;
    }
...
always_inline u16 esp_aad_fill(u8 *data, const esp_header_t *esp, const ipsec_sa_t *sa, u32 seq_hi)
{
    esp_aead_t *aad;
...
always_inline u32 esp_encrypt_err_to_sa_err(u32 err)
{
    switch (err) {
...
always_inline u32 esp_decrypt_err_to_sa_err(u32 err)
{
    switch (err) {
...
always_inline void
esp_encrypt_set_next_index(vlib_buffer_t *b, vlib_node_runtime_t *node, u32 thread_index, u32 err, u16 index, u16 *nexts, u16 drop_next, u32 sa_index)
{
    ipsec_set_next_index(b, node, thread_index, err, esp_encrypt_err_to_sa_err(err), index, nexts, drop_next, sa_index);
}

always_inline void
esp_decrypt_set_next_index(vlib_buffer_t *b, vlib_node_runtime_t *node, u32 thread_index, u32 err, u16 index, u16 *nexts, u16 drop_next, u32 sa_index)
{
    ipsec_set_next_index(b, node, thread_index, err, esp_decrypt_err_to_sa_err(err), index, nexts, drop_next, sa_index);
}
...
#define esp_post_data(b) ((esp_post_data_t *) ((u8 *) ((b)->opaque) + STRUCT_OFFSET_OF(vnet_buffer_opaque_t, unused)))
...
#define esp_post_data2(b) ((esp_decrypt_packet_data2_t *) ((u8 *) ((b)->opaque2) + STRUCT_OFFSET_OF(vnet_buffer_opaque2_t, unused)))
...
always_inline u32 esp_async_recycle_failed_submit(vlib_main_t               *vm,
                                                  ...
                                                  bool                       is_encrypt)
{
    ...
    while (n_drop--) {
        ...
        ipsec_set_next_index(b, node, vm->thread_index, err, ipsec_sa_err, index, nexts, drop_next_index, sa_index);
        bi++;
        index++;
    }
