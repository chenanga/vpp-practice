...
static_always_inline void ah_process_ops(vlib_main_t *vm, vlib_node_runtime_t *node, vnet_crypto_op_t *ops, vlib_buffer_t *b[], u16 *nexts)
{
    ...
    while (n_fail) {
        ...
        if (op->status != VNET_CRYPTO_OP_STATUS_COMPLETED) {
            u32 bi = op->user_data;
            ah_decrypt_set_next_index(b[bi],
                                      node,
                                      vm->thread_index,
...
always_inline uword ah_decrypt_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, int is_ip6)
{
    u32                      n_left, *from;
    ...
    while (n_left > 0) {
        ...
        if (anti_replay_result) {
            ah_decrypt_set_next_index(b[0], node, vm->thread_index, AH_DECRYPT_ERROR_REPLAY, 0, next, AH_DECRYPT_NEXT_DROP, current_sa_index);
            goto next;
        }
        ...
        if (PREDICT_TRUE(sa0->integ_alg != IPSEC_INTEG_ALG_NONE)) {
            if (PREDICT_FALSE(ipsec_sa_is_set_USE_ESN(sa0) && pd->current_data + b[0]->current_length + sizeof(u32) > buffer_data_size)) {
                ah_decrypt_set_next_index(b[0],
                                          node,
                                          vm->thread_index,
            ...
            if (ipsec_sa_is_set_USE_ESN(sa0)) {
                ...
                clib_memcpy(op->src + b[0]->current_length, &seq_hi, sizeof(seq_hi));
            }
            clib_memcpy(op->digest, ah0->auth_data, pd->icv_size);
            clib_memset(ah0->auth_data, 0, pd->icv_size);

            if (is_ip6) {
                pd->ip_version_traffic_class_and_flow_label  = ih6->ip_version_traffic_class_and_flow_label;
                pd->hop_limit                                = ih6->hop_limit;
                ih6->ip_version_traffic_class_and_flow_label = 0x60;
                ih6->hop_limit                               = 0;
                pd->nexthdr                                  = ah0->nexthdr;
                pd->icv_padding_len                          = ah_calc_icv_padding_len(pd->icv_size, 1 /* is_ipv6 */);
            }
            else {
                pd->tos             = ih4->tos;
                pd->ttl             = ih4->ttl;
                ...
                ih4->ttl            = 0;
                ih4->checksum       = 0;
                pd->icv_padding_len = ah_calc_icv_padding_len(pd->icv_size, 0 /* is_ipv6 */);
            }
        }
    ...
    while (n_left > 0) {
        ...
        if (next[0] < AH_DECRYPT_N_NEXT) goto trace;
        ...
        if (PREDICT_TRUE(sa0->integ_alg != IPSEC_INTEG_ALG_NONE)) {
            ...
            if (PREDICT_FALSE(ipsec_sa_is_set_ANTI_REPLAY_HUGE(sa0))) {
                if (ipsec_sa_anti_replay_and_sn_advance(sa0, pd->seq, pd->seq_hi, true, NULL, true)) {
                    ah_decrypt_set_next_index(b[0], node, vm->thread_index, AH_DECRYPT_ERROR_REPLAY, 0, next, AH_DECRYPT_NEXT_DROP, pd->sa_index);
                    goto trace;
                }
                n_lost = ipsec_sa_anti_replay_advance(sa0, thread_index, pd->seq, pd->seq_hi, true);
            }
            else {
                ...
                n_lost = ipsec_sa_anti_replay_advance(sa0, thread_index, pd->seq, pd->seq_hi, false);
            }
            vlib_prefetch_simple_counter(&ipsec_sa_err_counters[IPSEC_SA_ERROR_LOST], thread_index, pd->sa_index);
        }

        u16 ah_hdr_len = sizeof(ah_header_t) + pd->icv_size + pd->icv_padding_len;
        vlib_buffer_advance(b[0], pd->ip_hdr_size + ah_hdr_len);
        b[0]->flags |= VLIB_BUFFER_TOTAL_LENGTH_VALID;
        b[0]->flags &= ~(VNET_BUFFER_F_L4_CHECKSUM_COMPUTED | VNET_BUFFER_F_L4_CHECKSUM_CORRECT);

        if (PREDICT_TRUE(ipsec_sa_is_set_IS_TUNNEL(sa0))) { /* tunnel mode */
            ...
            if (is_ip6) {
                ...
                if (ah_hdr_len >= sizeof(ip6_header_t))
                    clib_memcpy(oh6, b[0]->data + pd->current_data, sizeof(ip6_header_t));
                else
                    memmove(oh6, b[0]->data + pd->current_data, sizeof(ip6_header_t));

                next[0]                                      = AH_DECRYPT_NEXT_IP6_INPUT;
                oh6->protocol                                = pd->nexthdr;
                oh6->hop_limit                               = pd->hop_limit;
                oh6->ip_version_traffic_class_and_flow_label = pd->ip_version_traffic_class_and_flow_label;
                oh6->payload_length                          = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, b[0]) - sizeof(ip6_header_t));
            }
            else {
                vlib_buffer_advance(b[0], -sizeof(ip4_header_t));
                oh4 = vlib_buffer_get_current(b[0]);
                if (ah_hdr_len >= sizeof(ip4_header_t))
                    clib_memcpy(oh4, b[0]->data + pd->current_data, sizeof(ip4_header_t));
                else
                    memmove(oh4, b[0]->data + pd->current_data, sizeof(ip4_header_t));
                ...
                oh4->length                       = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, b[0]));
                oh4->ttl                          = pd->ttl;
                oh4->tos                          = pd->tos;
        ...
        if (PREDICT_FALSE(n_lost)) vlib_increment_simple_counter(&ipsec_sa_err_counters[IPSEC_SA_ERROR_LOST], thread_index, pd->sa_index, n_lost);

        vnet_buffer(b[0])->sw_if_index[VLIB_TX] = (u32) ~0;
    trace:
        if (PREDICT_FALSE(b[0]->flags & VLIB_BUFFER_IS_TRACED)) {
            sa0                    = ipsec_sa_get(vnet_buffer(b[0])->ipsec.sad_index);
            ah_decrypt_trace_t *tr = vlib_add_trace(vm, node, b[0], sizeof(*tr));
...
static clib_error_t *ah_decrypt_init(vlib_main_t *vm)
{
    ...
    im->ah4_dec_fq_index = vlib_frame_queue_main_init(ah4_decrypt_node.index, im->handoff_queue_size);
    im->ah6_dec_fq_index = vlib_frame_queue_main_init(ah6_decrypt_node.index, im->handoff_queue_size);
