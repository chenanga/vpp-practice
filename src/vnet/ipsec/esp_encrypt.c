...
static_always_inline u8 *esp_add_footer_and_icv(vlib_main_t *vm, vlib_buffer_t **last, u8 esp_align, u8 icv_sz, u16 buffer_data_size, uword total_len)
{
    ...
    esp_footer_t *f          = (esp_footer_t *) (vlib_buffer_get_current(last[0]) + last[0]->current_length + pad_bytes);
    u16           tail_sz    = sizeof(esp_footer_t) + pad_bytes + icv_sz;

    if (last[0]->current_data + last[0]->current_length + tail_sz > buffer_data_size) {
        u32 tmp_bi = 0;
        if (vlib_buffer_alloc(vm, &tmp_bi, 1) != 1) return 0;
...
#else
    return (!(nexthdr ^ IP_PROTOCOL_IP6_HOP_BY_HOP_OPTIONS) || !(nexthdr ^ IP_PROTOCOL_IPV6_ROUTE) || !(nexthdr ^ IP_PROTOCOL_IPV6_FRAGMENTATION));
#endif
}
 ...
 * details. */
static_always_inline void *esp_generate_iv(ipsec_sa_t *sa, void *payload, int iv_sz)
{
                                                  ...
                                                  u16                     drop_next)
{
    ...
    while (n_fail) {
        ...
        if (op->status != VNET_CRYPTO_OP_STATUS_COMPLETED) {
            u32 bi = op->user_data;
            esp_encrypt_set_next_index(b[bi],
                                       node,
                                       vm->thread_index,
...
always_inline void esp_prepare_sync_op(vlib_main_t             *vm,
                                       ipsec_per_thread_data_t *ptd,
                                       ...
                                       esp_header_t            *esp)
{
    if (sa0->crypto_enc_op_id) {
        ...
        if (PREDICT_FALSE(lb != b[0])) {
            ...
            esp_encrypt_chain_crypto(vm, ptd, sa0, b[0], lb, icv_sz, crypto_start, crypto_len + icv_sz, &op->n_chunks);
        }
        else {
            /* not chained */
                                                  ...
                                                  vlib_buffer_t             *lb)
{
    ...
    if (lb != b) {
        ...
        crypto_total_len = esp_encrypt_chain_crypto(vm, ptd, sa, b, lb, icv_sz, b->data + crypto_start_offset, crypto_total_len + icv_sz, 0);
    }
    ...
    if (sa->integ_op_id) {
        ...
        if (b != lb) {
            integ_total_len = esp_encrypt_chain_integ(vm,
                                                      ptd,
                                                      sa,
                                                      b,
                                                      lb,
 ...
 */
static_always_inline void set_ip6_udp_cksum_offload(vlib_buffer_t *b, i16 l3_hdr_offset, i16 l4_hdr_offset)
{
    ...
    b->flags |= (VNET_BUFFER_F_IS_IP6 | VNET_BUFFER_F_L3_HDR_OFFSET_VALID | VNET_BUFFER_F_L4_HDR_OFFSET_VALID);
}

always_inline uword
esp_encrypt_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, vnet_link_t lt, int is_tun, u16 async_next_node)
{
    ...
    u16 drop_next = (lt == VNET_LINK_IP6 ? ESP_ENCRYPT_NEXT_DROP6 : (lt == VNET_LINK_IP4 ? ESP_ENCRYPT_NEXT_DROP4 : ESP_ENCRYPT_NEXT_DROP_MPLS));
    u16 handoff_next =
        (lt == VNET_LINK_IP6 ? ESP_ENCRYPT_NEXT_HANDOFF6 : (lt == VNET_LINK_IP4 ? ESP_ENCRYPT_NEXT_HANDOFF4 : ESP_ENCRYPT_NEXT_HANDOFF_MPLS));
    ...
    while (n_left > 0) {
        ...
        if (n_left > 2) {
            ...
            CLIB_PREFETCH(vlib_buffer_get_tail(b[1]), CLIB_CACHE_LINE_BYTES, LOAD);
        }

        vnet_calc_checksums_inline(vm, b[0], b[0]->flags & VNET_BUFFER_F_IS_IP4, b[0]->flags & VNET_BUFFER_F_IS_IP6);
        vnet_calc_outer_checksums_inline(vm, b[0]);

        if (is_tun) {
            /* we are on a ipsec tunnel's feature arc */
            vnet_buffer(b[0])->ipsec.sad_index = sa_index0 = ipsec_tun_protect_get_sa_out(vnet_buffer(b[0])->ip.adj_index[VLIB_TX]);
        ...
        if (sa_index0 != current_sa_index) {
            if (current_sa_packets)
                vlib_increment_combined_counter(&ipsec_sa_counters, thread_index, current_sa_index, current_sa_packets, current_sa_bytes);
            ...
            sa_drop_no_crypto =
                ((sa0->crypto_alg == IPSEC_CRYPTO_ALG_NONE && sa0->integ_alg == IPSEC_INTEG_ALG_NONE) && !ipsec_sa_is_set_NO_ALGO_NO_DROP(sa0));
        ...
        if (PREDICT_FALSE(sa_drop_no_crypto != 0)) {
            ...
            esp_encrypt_set_next_index(b[0], node, thread_index, err, n_noop, noop_nexts, drop_next, sa_index0);
            goto trace;
        }
        ...
        if (PREDICT_FALSE(thread_index != sa0->thread_index)) {
            ...
            esp_encrypt_set_next_index(b[0], node, thread_index, err, n_noop, noop_nexts, handoff_next, current_sa_index);
            goto trace;
        }
        ...
        if (n_bufs == 0) {
            ...
            esp_encrypt_set_next_index(b[0], node, thread_index, err, n_noop, noop_nexts, drop_next, current_sa_index);
            goto trace;
        }
        ...
        if (ipsec_sa_is_set_IS_TUNNEL(sa0)) {
            ...
            next_hdr_ptr = esp_add_footer_and_icv(vm, &lb, esp_align, icv_sz, buffer_data_size, vlib_buffer_length_in_chain(vm, b[0]));
            if (!next_hdr_ptr) {
                err = ESP_ENCRYPT_ERROR_NO_BUFFERS;
                esp_encrypt_set_next_index(b[0], node, thread_index, err, n_noop, noop_nexts, drop_next, current_sa_index);
                goto trace;
            ...
            if (ipsec_sa_is_set_UDP_ENCAP(sa0)) {
                ...
                esp_fill_udp_hdr(sa0, (udp_header_t *) (payload - hdr_len), payload_len_total + hdr_len);
            }
            ...
            if (ipsec_sa_is_set_IS_TUNNEL_V6(sa0)) {
                ...
                if (VNET_LINK_IP6 == lt) {
                    *next_hdr_ptr = IP_PROTOCOL_IPV6;
                    tunnel_encap_fixup_6o6(sa0->tunnel_flags, (const ip6_header_t *) payload, ip6);
                }
                else if (VNET_LINK_IP4 == lt) {
                    *next_hdr_ptr = IP_PROTOCOL_IP_IN_IP;
                    tunnel_encap_fixup_4o6(sa0->tunnel_flags, b[0], (const ip4_header_t *) payload, ip6);
                }
                else if (VNET_LINK_MPLS == lt) {
                    *next_hdr_ptr = IP_PROTOCOL_MPLS_IN_IP;
                    tunnel_encap_fixup_mplso6(sa0->tunnel_flags, b[0], (const mpls_unicast_header_t *) payload, ip6);
                }
                else
                    ASSERT(0);
                ...
                if (VNET_LINK_IP6 == lt) {
                    *next_hdr_ptr = IP_PROTOCOL_IPV6;
                    tunnel_encap_fixup_6o4_w_chksum(sa0->tunnel_flags, (const ip6_header_t *) payload, ip4);
                }
                else if (VNET_LINK_IP4 == lt) {
                    *next_hdr_ptr = IP_PROTOCOL_IP_IN_IP;
                    tunnel_encap_fixup_4o4_w_chksum(sa0->tunnel_flags, (const ip4_header_t *) payload, ip4);
                }
                else if (VNET_LINK_MPLS == lt) {
                    *next_hdr_ptr = IP_PROTOCOL_MPLS_IN_IP;
                    tunnel_encap_fixup_mplso4_w_chksum(sa0->tunnel_flags, (const mpls_unicast_header_t *) payload, ip4);
                }
                else
                    ASSERT(0);
            ...
            if (ipsec_sa_is_set_UDP_ENCAP(sa0) && ipsec_sa_is_set_IS_TUNNEL_V6(sa0)) {
        ...
        else /* transport mode */
        {
             ...
             */
            ip_len =
                (VNET_LINK_IP6 == lt ? esp_get_ip6_hdr_len((ip6_header_t *) old_ip_hdr, &ext_hdr) : ip4_header_bytes((ip4_header_t *) old_ip_hdr));
            ...
            if (VNET_LINK_IP6 == lt) {
                ...
                if (PREDICT_TRUE(NULL == ext_hdr)) {
                    ...
                    *next_hdr_ptr     = ext_hdr->next_hdr;
                    ext_hdr->next_hdr = (udp) ? IP_PROTOCOL_UDP : IP_PROTOCOL_IPSEC_ESP;
                }
                ip6->payload_length = clib_host_to_net_u16(len - sizeof(ip6_header_t));
            }
            else if (VNET_LINK_IP4 == lt) {
        ...
        if (is_async) {
            ...
            esp_prepare_sync_op(vm, ptd, crypto_ops, integ_ops, sa0, sa0->seq_hi, payload, payload_len, iv_sz, icv_sz, n_sync, b, lb, hdr_len, esp);
    ...
    if (INDEX_INVALID != current_sa_index)
        vlib_increment_combined_counter(&ipsec_sa_counters, thread_index, current_sa_index, current_sa_packets, current_sa_bytes);
    if (n_sync) {
        esp_process_ops(vm, node, ptd->crypto_ops, sync_bufs, sync_nexts, drop_next);
        esp_process_chained_ops(vm, node, ptd->chained_crypto_ops, sync_bufs, sync_nexts, ptd->chunks, drop_next);

        esp_process_ops(vm, node, ptd->integ_ops, sync_bufs, sync_nexts, drop_next);
        esp_process_chained_ops(vm, node, ptd->chained_integ_ops, sync_bufs, sync_nexts, ptd->chunks, drop_next);
...
always_inline uword esp_encrypt_post_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    vlib_buffer_t *bufs[VLIB_FRAME_SIZE], **b    = bufs;
    ...
    while (n_left > 8) {
        ...
        if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE)) {
            if (b[0]->flags & VLIB_BUFFER_IS_TRACED) {
                esp_encrypt_post_trace_t *tr = vlib_add_trace(vm, node, b[0], sizeof(*tr));
                tr->next_index               = next[0];
            }
            if (b[1]->flags & VLIB_BUFFER_IS_TRACED) {
                esp_encrypt_post_trace_t *tr = vlib_add_trace(vm, node, b[1], sizeof(*tr));
                tr->next_index               = next[1];
            }
            if (b[2]->flags & VLIB_BUFFER_IS_TRACED) {
                esp_encrypt_post_trace_t *tr = vlib_add_trace(vm, node, b[2], sizeof(*tr));
                tr->next_index               = next[2];
            }
            if (b[3]->flags & VLIB_BUFFER_IS_TRACED) {
                esp_encrypt_post_trace_t *tr = vlib_add_trace(vm, node, b[3], sizeof(*tr));
    ...
    vlib_node_increment_counter(vm, node->node_index, ESP_ENCRYPT_ERROR_POST_RX_PKTS, frame->n_vectors);
    vlib_buffer_enqueue_to_next(vm, node, from, nexts, frame->n_vectors);
    return frame->n_vectors;
}

VLIB_NODE_FN(esp4_encrypt_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return esp_encrypt_inline(vm, node, from_frame, VNET_LINK_IP4, 0, esp_encrypt_async_next.esp4_post_next);
}
...
VLIB_NODE_FN(esp4_encrypt_post_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
VLIB_NODE_FN(esp6_encrypt_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return esp_encrypt_inline(vm, node, from_frame, VNET_LINK_IP6, 0, esp_encrypt_async_next.esp6_post_next);
}
...
VLIB_NODE_FN(esp6_encrypt_post_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return esp_encrypt_post_inline(vm, node, from_frame);
}
...
VLIB_NODE_FN(esp4_encrypt_tun_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return esp_encrypt_inline(vm, node, from_frame, VNET_LINK_IP4, 1, esp_encrypt_async_next.esp4_tun_post_next);
}
...
VLIB_NODE_FN(esp4_encrypt_tun_post_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return esp_encrypt_post_inline(vm, node, from_frame);
}
...
VLIB_NODE_FN(esp6_encrypt_tun_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return esp_encrypt_inline(vm, node, from_frame, VNET_LINK_IP6, 1, esp_encrypt_async_next.esp6_tun_post_next);
}
...
VLIB_NODE_FN(esp6_encrypt_tun_post_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return esp_encrypt_post_inline(vm, node, from_frame);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return esp_encrypt_inline(vm, node, from_frame, VNET_LINK_MPLS, 1, esp_encrypt_async_next.esp_mpls_tun_post_next);
}
...
static clib_error_t *esp_encrypt_init(vlib_main_t *vm)
{
    ...
    im->esp4_enc_fq_index         = vlib_frame_queue_main_init(esp4_encrypt_node.index, im->handoff_queue_size);
    im->esp6_enc_fq_index         = vlib_frame_queue_main_init(esp6_encrypt_node.index, im->handoff_queue_size);
    im->esp4_enc_tun_fq_index     = vlib_frame_queue_main_init(esp4_encrypt_tun_node.index, im->handoff_queue_size);
    im->esp6_enc_tun_fq_index     = vlib_frame_queue_main_init(esp6_encrypt_tun_node.index, im->handoff_queue_size);
    im->esp_mpls_enc_tun_fq_index = vlib_frame_queue_main_init(esp_mpls_encrypt_tun_node.index, im->handoff_queue_size);
