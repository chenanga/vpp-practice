...
#define ITP_DBG(_itp, _fmt, _args...)                                                                     \
    {                                                                                                     \
        vlib_log_debug(ipsec_tun_protect_logger, "[%U]: " _fmt, format_ipsec_tun_protect, _itp, ##_args); \
    }

#define ITP_DBG2(_fmt, _args...)                                 \
    {                                                            \
        vlib_log_debug(ipsec_tun_protect_logger, _fmt, ##_args); \
    }
...
void ipsec_tun_register_nodes(ip_address_family_t af)
{
    if (0 == ipsec_tun_node_regs[af]++) {
        if (AF_IP4 == af)
            ip4_register_protocol(IP_PROTOCOL_IPSEC_ESP, ipsec4_tun_input_node.index);
        else
            ip6_register_protocol(IP_PROTOCOL_IPSEC_ESP, ipsec6_tun_input_node.index);
...
static inline const ipsec_tun_protect_t *ipsec_tun_protect_from_const_base(const adj_delegate_t *ad)
{
...
static void ipsec_tun_setup_tx_nodes(u32 sw_if_index, const ipsec_tun_protect_t *itp)
{
    vnet_feature_modify_end_node(ip4_main.lookup_main.output_feature_arc_index, sw_if_index, ipsec_tun_protect_get_adj_next(VNET_LINK_IP4, itp));
    vnet_feature_modify_end_node(ip6_main.lookup_main.output_feature_arc_index, sw_if_index, ipsec_tun_protect_get_adj_next(VNET_LINK_IP6, itp));
    vnet_feature_modify_end_node(mpls_main.output_feature_arc_index, sw_if_index, ipsec_tun_protect_get_adj_next(VNET_LINK_MPLS, itp));
}

static void ipsec_tun_protect_add_adj(adj_index_t ai, const ipsec_tun_protect_t *itp)
{
    vec_validate_init_empty(ipsec_tun_protect_sa_by_adj_index, ai, INDEX_INVALID);
    ...
    if (NULL == itp) {
        ...
        adj_nbr_midchain_update_next_node(ai, ipsec_tun_protect_get_adj_next(adj_get_link_type(ai), itp));
    }
}
...
static index_t ipsec_tun_protect_find(u32 sw_if_index, const ip_address_t *nh)
{
    ...
    if (vec_len(itp_db.id_itf) <= sw_if_index) return INDEX_INVALID;

    if (vnet_sw_interface_is_p2p(vnet_get_main(), sw_if_index)) return (itp_db.id_itf[sw_if_index].id_itp);
...
static void ipsec_tun_protect_rx_db_add(ipsec_main_t *im, const ipsec_tun_protect_t *itp)
{
    ...
    if (ip46_address_is_zero(&itp->itp_crypto.dst)) return;
    ...
    FOR_EACH_IPSEC_PROTECT_INPUT_SAI(itp, sai, ({
                                         ...
                                         if (ip46_address_is_ip4(&itp->itp_crypto.dst)) {
                                             ...
                                             ipsec4_tunnel_mk_key(&key, &itp->itp_crypto.dst.ip4, clib_host_to_net_u32(sa->spi));
...
static adj_walk_rc_t ipsec_tun_protect_adj_add(adj_index_t ai, void *arg)
{
    ...
    adj_delegate_add(adj_get(ai), ipsec_tun_adj_delegate_type, itp - ipsec_tun_protect_pool);
    ipsec_tun_protect_add_adj(ai, itp);

    if (itp->itp_flags & IPSEC_PROTECT_ITF) ipsec_itf_adj_stack(ai, itp->itp_out_sa);
...
static void ipsec_tun_protect_tx_db_add(ipsec_tun_protect_t *itp)
{
    ...
    vec_validate_init_empty(itp_db.id_itf, itp->itp_sw_if_index, IPSEC_TUN_PROTECT_DEFAULT_DB_ENTRY);
    ...
    if (vnet_sw_interface_is_p2p(vnet_get_main(), itp->itp_sw_if_index)) {
        ...
        adj_nbr_walk(itp->itp_sw_if_index, nh_proto, ipsec_tun_protect_adj_add, itp);
    }
    else {
        if (NULL == idi->id_hash) {
            idi->id_hash = hash_create_mem(0, sizeof(ip_address_t), sizeof(uword));
            /*
        ...
        adj_nbr_walk_nh(itp->itp_sw_if_index, nh_proto, &nh, ipsec_tun_protect_adj_add, itp);

        ipsec_tun_register_nodes(FIB_PROTOCOL_IP6 == nh_proto ? AF_IP6 : AF_IP4);
    }
}
...
static adj_walk_rc_t ipsec_tun_protect_adj_remove(adj_index_t ai, void *arg)
{
    ...
    if (itp->itp_flags & IPSEC_PROTECT_ITF) ipsec_itf_adj_unstack(ai);
...
static void ipsec_tun_protect_tx_db_remove(ipsec_tun_protect_t *itp)
{
    ...
    if (vnet_sw_interface_is_p2p(vnet_get_main(), itp->itp_sw_if_index)) {
        ...
        FOR_EACH_FIB_IP_PROTOCOL(nh_proto)
        adj_nbr_walk(itp->itp_sw_if_index, nh_proto, ipsec_tun_protect_adj_remove, itp);
    }
    else {
        adj_nbr_walk_nh(itp->itp_sw_if_index, nh_proto, &nh, ipsec_tun_protect_adj_remove, itp);
        ...
        ipsec_tun_unregister_nodes(FIB_PROTOCOL_IP6 == nh_proto ? AF_IP6 : AF_IP4);
    }
}
...
static void ipsec_tun_protect_config(ipsec_main_t *im, ipsec_tun_protect_t *itp, u32 sa_out, u32 *sas_in)
{
    ...
    if (itp->itp_flags & IPSEC_PROTECT_ITF) ipsec_sa_set_NO_ALGO_NO_DROP(ipsec_sa_get(itp->itp_out_sa));

    FOR_EACH_IPSEC_PROTECT_INPUT_SAI(itp, sai, ({ ipsec_sa_lock(sai); }));
...
static void ipsec_tun_protect_unconfig(ipsec_main_t *im, ipsec_tun_protect_t *itp)
{
    ...
    FOR_EACH_IPSEC_PROTECT_INPUT_SA(itp, sa, ({ ipsec_sa_unset_IS_PROTECT(sa); }));
    ...
    FOR_EACH_IPSEC_PROTECT_INPUT_SAI(itp, sai, ({ ipsec_sa_unlock(sai); }));
...
int ipsec_tun_protect_update(u32 sw_if_index, const ip_address_t *nh, u32 sa_out, u32 *sas_in)
{
    ...
    if (NULL == nh) nh = &IP_ADDR_ALL_0;

    ITP_DBG2("update: %U/%U", format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index, format_ip_address, nh);
    ...
    if (INDEX_INVALID == itpi) {
        ...
        rv = dev_class->ip_tun_desc(sw_if_index, &itp->itp_tun.src, &itp->itp_tun.dst, &is_l2);
        ...
        if (ip46_address_is_zero(&itp->itp_tun.src)) {
            ...
            ipsec_tun_protect_update_from_teib(itp, teib_entry_find(sw_if_index, nh));
        }

        if (is_l2) itp->itp_flags |= IPSEC_PROTECT_L2;
    ...
    vec_foreach (saip, sas_in)
        ipsec_sa_unlock(*saip);
    vec_free(sas_in);
...
static int ipsec_tun_protect_del_by_idx(index_t itpi)
{
    ipsec_tun_protect_t *itp;
    ipsec_main_t        *im;
    ...
    if (INDEX_INVALID == itpi) return (VNET_API_ERROR_NO_SUCH_ENTRY);

    itp = ipsec_tun_protect_get(itpi);
    ipsec_tun_protect_unconfig(im, itp);

    if (ADJ_INDEX_INVALID != itp->itp_ai) adj_unlock(itp->itp_ai);
...
int ipsec_tun_protect_del(u32 sw_if_index, const ip_address_t *nh)
{
    ...
    ITP_DBG2("delete: %U/%U", format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index, format_ip_address, nh);
...
walk_rc_t ipsec_tun_interface_cleanup(index_t itpi, void *ctx)
{
    ipsec_tun_protect_del_by_idx(itpi);
    return WALK_CONTINUE;
}

static clib_error_t *ipsec_tun_interface_add_del(vnet_main_t *vnm, u32 sw_if_index, u32 is_add)
{
    if (is_add) return 0;
...
void ipsec_tun_protect_walk_itf(u32 sw_if_index, ipsec_tun_protect_walk_cb_t fn, void *ctx)
{
    ...
    if (vec_len(itp_db.id_itf) <= sw_if_index) return;

    idi = &itp_db.id_itf[sw_if_index];

    hash_foreach(key, itpi, idi->id_hash, ({ fn(itpi, ctx); }));
    if (INDEX_INVALID != idi->id_itp) fn(idi->id_itp, ctx);
}
...
static void ipsec_tun_feature_update(u32 sw_if_index, u8 arc_index, u8 is_enable, void *data)
{
    ...
    if (arc_index != feature_main.device_input_feature_arc_index) return;

    /* Only p2p tunnels supported */
    itpi = ipsec_tun_protect_find(sw_if_index, &IP_ADDR_ALL_0);
    if (itpi == INDEX_INVALID) return;

    itp = ipsec_tun_protect_get(itpi);

    if (is_enable) {
        u32 decrypt_tun = ip46_address_is_ip4(&itp->itp_crypto.dst) ? ipsec_main.esp4_decrypt_tun_node_index : ipsec_main.esp6_decrypt_tun_node_index;

        if (!(itp->itp_flags & IPSEC_PROTECT_FEAT)) {
            itp->itp_flags |= IPSEC_PROTECT_FEAT;
            vnet_feature_modify_end_node(feature_main.device_input_feature_arc_index, sw_if_index, decrypt_tun);
        }
    }
    else {
        if (itp->itp_flags & IPSEC_PROTECT_FEAT) {
            ...
            u32 eth_in = vlib_get_node_by_name(vlib_get_main(), (u8 *) "ethernet-input")->index;

            vnet_feature_modify_end_node(feature_main.device_input_feature_arc_index, sw_if_index, eth_in);
        }
    }
...
{
    ipsec_tun_protect_add_adj(ad->ad_adj_index, ipsec_tun_protect_get(ad->ad_index));
}
...
static void ipsec_tun_protect_adj_delegate_adj_created(adj_index_t ai)
{
    ...
    if (!adj_is_midchain(ai)) return;

    vec_validate_init_empty(ipsec_tun_protect_sa_by_adj_index, ai, INDEX_INVALID);

    adj = adj_get(ai);

    ip_address_from_46(&adj->sub_type.midchain.next_hop, adj->ia_nh_proto, &ip);

    itpi = ipsec_tun_protect_find(adj->rewrite_header.sw_if_index, &ip);

    if (INDEX_INVALID != itpi) ipsec_tun_protect_adj_add(ai, ipsec_tun_protect_get(itpi));
}
...
static void ipsec_tun_teib_entry_added(const teib_entry_t *ne)
{
    ...
    itpi = ipsec_tun_protect_find(teib_entry_get_sw_if_index(ne), teib_entry_get_peer(ne));

    if (INDEX_INVALID == itpi) return;
...
void ipsec_tun_table_init(ip_address_family_t af, uword table_size, u32 n_buckets)
{
    ...
    if (AF_IP4 == af)
        clib_bihash_init_8_16(&im->tun4_protect_by_key, "IPSec IPv4 tunnels", n_buckets, table_size);
    else
        clib_bihash_init_24_16(&im->tun6_protect_by_key, "IPSec IPv6 tunnels", n_buckets, table_size);
}
...
static clib_error_t *ipsec_tunnel_protect_init(vlib_main_t *vm)
{
    ...
    im = &ipsec_main;
    clib_bihash_init_24_16(&im->tun6_protect_by_key, "IPSec IPv6 tunnels", IPSEC_TUN_DEFAULT_HASH_NUM_BUCKETS, IPSEC_TUN_DEFAULT_HASH_MEMORY_SIZE);
