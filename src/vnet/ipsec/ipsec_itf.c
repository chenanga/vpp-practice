...
{
    if (vec_len(ipsec_itf_index_by_sw_if_index) <= sw_if_index) return NULL;
...
static clib_error_t *ipsec_itf_admin_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    ...
    if (itf) ipsec_tun_protect_walk_itf(itf->ii_sw_if_index, ipsec_tun_protect_walk_state_change, itf);
...
{
    adj_nbr_midchain_update_rewrite(ai, NULL, NULL, ADJ_FLAG_MIDCHAIN_IP_STACK, ipsec_itf_build_rewrite());
}
...
void ipsec_itf_reset_tx_nodes(u32 sw_if_index)
{
    vnet_feature_modify_end_node(ip4_main.lookup_main.output_feature_arc_index,
                                 sw_if_index,
                                 vlib_get_node_by_name(vlib_get_main(), (u8 *) "ip4-drop")->index);
    vnet_feature_modify_end_node(ip6_main.lookup_main.output_feature_arc_index,
                                 sw_if_index,
                                 vlib_get_node_by_name(vlib_get_main(), (u8 *) "ip6-drop")->index);
    vnet_feature_modify_end_node(mpls_main.output_feature_arc_index, sw_if_index, vlib_get_node_by_name(vlib_get_main(), (u8 *) "mpls-drop")->index);
}
...
int ipsec_itf_create(u32 user_instance, tunnel_mode_t mode, u32 *sw_if_indexp)
{
    ...
    *sw_if_indexp = (u32) ~0;
    ...
    hw_if_index = vnet_register_interface(vnm,
                                          ...
                                          (mode == TUNNEL_MODE_P2P ? ipsec_hw_interface_class.index : ipsec_p2mp_hw_interface_class.index),
                                          t_idx);

    hi = vnet_get_hw_interface(vnm, hw_if_index);
    vnet_sw_interface_set_mtu(vnm, hi->sw_if_index, 9000);

    vec_validate_init_empty(ipsec_itf_index_by_sw_if_index, hi->sw_if_index, INDEX_INVALID);
...
int ipsec_itf_delete(u32 sw_if_index)
{
    ...
    if (hw == 0 || hw->dev_class_index != ipsec_itf_device_class.index) return VNET_API_ERROR_INVALID_SW_IF_INDEX;

    ipsec_itf_t *ipsec_itf;
    ipsec_itf = ipsec_itf_find_by_sw_if_index(sw_if_index);
    if (NULL == ipsec_itf) return VNET_API_ERROR_INVALID_SW_IF_INDEX;

    if (ipsec_itf_instance_free(hw->dev_instance) < 0) return VNET_API_ERROR_INVALID_SW_IF_INDEX;
...
void ipsec_itf_walk(ipsec_itf_walk_cb_t cb, void *ctx)
{
    ...
    pool_foreach (itf, ipsec_itf_pool) {
        if (WALK_CONTINUE != cb(itf, ctx)) break;
    }
}
...
static clib_error_t *ipsec_itf_create_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (rv) return clib_error_return(0, "iPSec interface create failed");
...
static clib_error_t *ipsec_itf_delete_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (~0 != sw_if_index) {
        ...
        if (rv) return clib_error_return(0, "ipsec interface delete failed");
    }
    else
        return clib_error_return(0, "no such interface: %U", format_unformat_error, input);
