...
static void vl_api_ipsec_tunnel_protect_update_t_handler(vl_api_ipsec_tunnel_protect_update_t *mp)
{
    ...
    rv = ipsec_tun_protect_update(sw_if_index, &nh, ntohl(mp->tunnel.sa_out), sa_ins);
...
static walk_rc_t send_ipsec_tunnel_protect_details(index_t itpi, void *arg)
{
    ...
    mp->_vl_msg_id = ntohs(REPLY_MSG_ID_BASE + VL_API_IPSEC_TUNNEL_PROTECT_DETAILS);
    ...
    FOR_EACH_IPSEC_PROTECT_INPUT_SA(itp, sa, ({ mp->tun.sa_in[ii++] = htonl(sa->id); }));
...
static void vl_api_ipsec_tunnel_protect_dump_t_handler(vl_api_ipsec_tunnel_protect_dump_t *mp)
{
    ...
    if (~0 == sw_if_index) {
        ...
        ipsec_tun_protect_walk_itf(sw_if_index, send_ipsec_tunnel_protect_details, &ctx);
    }
}
...
static void vl_api_ipsec_spd_entry_add_del_t_handler(vl_api_ipsec_spd_entry_add_del_t *mp)
{
    ...
    p.protocol    = mp->entry.protocol ? mp->entry.protocol : IPSEC_POLICY_PROTOCOL_ANY;
    ...
    rv      = ipsec_policy_mk_type(mp->entry.is_outbound, p.is_ipv6, p.policy, &p.type);
    if (rv) goto out;
...
out:
    REPLY_MACRO2(VL_API_IPSEC_SPD_ENTRY_ADD_DEL_REPLY, ({ rmp->stat_index = ntohl(stat_index); }));
}

static void vl_api_ipsec_spd_entry_add_del_v2_t_handler(vl_api_ipsec_spd_entry_add_del_v2_t *mp)
{
    vlib_main_t                            *vm __attribute__((unused)) = vlib_get_main();
    vl_api_ipsec_spd_entry_add_del_reply_t *rmp;
    ip46_type_t                             itype;
...
out:
    REPLY_MACRO2(VL_API_IPSEC_SPD_ENTRY_ADD_DEL_V2_REPLY, ({ rmp->stat_index = ntohl(stat_index); }));
}
...
out:
    REPLY_MACRO2(VL_API_IPSEC_SAD_ENTRY_ADD_DEL_REPLY, { rmp->stat_index = htonl(sa_index); });
}
...
static void vl_api_ipsec_sad_entry_add_del_v2_t_handler(vl_api_ipsec_sad_entry_add_del_v2_t *mp)
{
    ...
    rv = tunnel_encap_decap_flags_decode(mp->entry.tunnel_flags, &tun.t_encap_decap_flags);
...
out:
    REPLY_MACRO2(VL_API_IPSEC_SAD_ENTRY_ADD_DEL_V2_REPLY, { rmp->stat_index = htonl(sa_index); });
}
...
static int ipsec_sad_entry_add_v3(const vl_api_ipsec_sad_entry_v3_t *entry, u32 *sa_index)
{
    ...
    return ipsec_sa_add_and_lock(id,
                                 spi,
                                 proto,
                                 crypto_alg,
                                 &crypto_key,
...
static void vl_api_ipsec_sad_entry_add_del_v3_t_handler(vl_api_ipsec_sad_entry_add_del_v3_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_IPSEC_SAD_ENTRY_ADD_DEL_V3_REPLY, { rmp->stat_index = htonl(sa_index); });
}

static int ipsec_sad_entry_add_v4(const vl_api_ipsec_sad_entry_v4_t *entry, u32 *sa_index)
{
    ipsec_key_t        crypto_key, integ_key;
    ipsec_crypto_alg_t crypto_alg;
    ipsec_integ_alg_t  integ_alg;
    ...
    return ipsec_sa_add_and_lock(id,
                                 ...
                                 htons(entry->udp_src_port),
                                 htons(entry->udp_dst_port),
                                 ntohl(entry->anti_replay_window_size),
                                 &tun,
                                 sa_index);
}
...
static void vl_api_ipsec_sad_entry_add_t_handler(vl_api_ipsec_sad_entry_add_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_IPSEC_SAD_ENTRY_ADD_REPLY, { rmp->stat_index = htonl(sa_index); });
}

static void vl_api_ipsec_sad_entry_add_v2_t_handler(vl_api_ipsec_sad_entry_add_v2_t *mp)
{
    vl_api_ipsec_sad_entry_add_reply_t *rmp;
    u32                                 sa_index = ~0;
    int                                 rv;
    ...
    REPLY_MACRO2(VL_API_IPSEC_SAD_ENTRY_ADD_V2_REPLY, { rmp->stat_index = htonl(sa_index); });
}

static void vl_api_ipsec_sad_entry_update_t_handler(vl_api_ipsec_sad_entry_update_t *mp)
{
    ...
    rv = ipsec_sa_update(id, htons(mp->udp_src_port), htons(mp->udp_dst_port), &tun, mp->is_tun);

out:
    REPLY_MACRO(VL_API_IPSEC_SAD_ENTRY_UPDATE_REPLY);
}

static void vl_api_ipsec_sad_bind_t_handler(vl_api_ipsec_sad_bind_t *mp)
{
...
static void vl_api_ipsec_sad_unbind_t_handler(vl_api_ipsec_sad_unbind_t *mp)
{
...
static void send_ipsec_spd_details(ipsec_policy_t *p, vl_api_registration_t *reg, u32 context)
{
    ...
    mp->entry.is_outbound = ((p->type == IPSEC_SPD_POLICY_IP6_OUTBOUND) || (p->type == IPSEC_SPD_POLICY_IP4_OUTBOUND));

    ip_address_encode(&p->laddr.start, IP46_TYPE_ANY, &mp->entry.local_address_start);
    ip_address_encode(&p->laddr.stop, IP46_TYPE_ANY, &mp->entry.local_address_stop);
    ip_address_encode(&p->raddr.start, IP46_TYPE_ANY, &mp->entry.remote_address_start);
    ip_address_encode(&p->raddr.stop, IP46_TYPE_ANY, &mp->entry.remote_address_stop);
...
static void vl_api_ipsec_spd_dump_t_handler(vl_api_ipsec_spd_dump_t *mp)
{
    ...
    FOR_EACH_IPSEC_SPD_POLICY_TYPE(ptype)
    {
        vec_foreach (ii, spd->policies[ptype]) {
            ...
            if (mp->sa_id == ~(0) || ntohl(mp->sa_id) == policy->sa_id) send_ipsec_spd_details(policy, reg, mp->context);
        }
    }
}
...
static void send_ipsec_spd_interface_details(vl_api_registration_t *reg, u32 spd_index, u32 sw_if_index, u32 context)
{
    ...
    mp = vl_msg_api_alloc(sizeof(*mp));
    clib_memset(mp, 0, sizeof(*mp));
    mp->_vl_msg_id = ntohs(REPLY_MSG_ID_BASE + VL_API_IPSEC_SPD_INTERFACE_DETAILS);
    mp->context    = context;
...
static void vl_api_ipsec_spd_interface_dump_t_handler(vl_api_ipsec_spd_interface_dump_t *mp)
{
    ...
    if (mp->spd_index_valid) {
        ...
        hash_foreach(k, v, im->spd_index_by_sw_if_index, ({
                         if (v == spd_index) send_ipsec_spd_interface_details(reg, v, k, mp->context);
                     }));
    }
    else {
        hash_foreach(k, v, im->spd_index_by_sw_if_index, ({ send_ipsec_spd_interface_details(reg, v, k, mp->context); }));
    }
}
...
static void vl_api_ipsec_itf_create_t_handler(vl_api_ipsec_itf_create_t *mp)
{
    ...
    if (!rv) rv = ipsec_itf_create(ntohl(mp->itf.user_instance), mode, &sw_if_index);

    REPLY_MACRO2(VL_API_IPSEC_ITF_CREATE_REPLY, ({ rmp->sw_if_index = htonl(sw_if_index); }));
}
...
static walk_rc_t send_ipsec_itf_details(ipsec_itf_t *itf, void *arg)
{
    ...
    if (~0 != ctx->sw_if_index && ctx->sw_if_index != itf->ii_sw_if_index) return (WALK_CONTINUE);
...
static walk_rc_t send_ipsec_sa_details(ipsec_sa_t *sa, void *arg)
{
    ...
    if (ipsec_sa_is_set_USE_ANTI_REPLAY(sa)) {
        mp->replay_window = clib_host_to_net_u64(ipsec_sa_anti_replay_get_64b_window(sa));
    }
...
static walk_rc_t send_ipsec_sa_v2_details(ipsec_sa_t *sa, void *arg)
{
    ...
    mp->entry.tunnel_flags = tunnel_encap_decap_flags_encode(sa->tunnel.t_encap_decap_flags);
...
static walk_rc_t send_ipsec_sa_v3_details(ipsec_sa_t *sa, void *arg)
{
    ...
    if (ipsec_sa_is_set_IS_TUNNEL(sa)) tunnel_encode(&sa->tunnel, &mp->entry.tunnel);
...
static walk_rc_t send_ipsec_sa_v4_details(ipsec_sa_t *sa, void *arg)
{
    ipsec_dump_walk_ctx_t        *ctx = arg;
...
static void vl_api_ipsec_sa_v4_dump_t_handler(vl_api_ipsec_sa_v4_dump_t *mp)
{
    vl_api_registration_t *reg;
...
static walk_rc_t send_ipsec_sa_v5_details(ipsec_sa_t *sa, void *arg)
{
    ipsec_dump_walk_ctx_t        *ctx = arg;
    ...
    if (ipsec_sa_is_set_USE_ANTI_REPLAY(sa)) {
        ...
        mp->entry.anti_replay_window_size = clib_host_to_net_u32(IPSEC_SA_ANTI_REPLAY_WINDOW_SIZE(sa));
    }
...
static void vl_api_ipsec_sa_v5_dump_t_handler(vl_api_ipsec_sa_v5_dump_t *mp)
{
    vl_api_registration_t *reg;
...
static void vl_api_ipsec_backend_dump_t_handler(vl_api_ipsec_backend_dump_t *mp)
{
    ...
    pool_foreach (ab, im->ah_backends) {
        ...
        mp->context    = context;
        snprintf((char *) mp->name, sizeof(mp->name), "%.*s", vec_len(ab->name), ab->name);
    ...
    pool_foreach (eb, im->esp_backends) {
        ...
        mp->_vl_msg_id = ntohs(REPLY_MSG_ID_BASE + VL_API_IPSEC_BACKEND_DETAILS);
        mp->context    = context;
        snprintf((char *) mp->name, sizeof(mp->name), "%.*s", vec_len(eb->name), eb->name);
