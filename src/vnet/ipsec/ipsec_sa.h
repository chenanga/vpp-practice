...
#define IPSEC_CRYPTO_ALG_IS_NULL_GMAC(_alg) \
    ((_alg == IPSEC_CRYPTO_ALG_AES_NULL_GMAC_128) || (_alg == IPSEC_CRYPTO_ALG_AES_NULL_GMAC_192) || (_alg == IPSEC_CRYPTO_ALG_AES_NULL_GMAC_256))

#define IPSEC_CRYPTO_ALG_IS_GCM(_alg) \
    (((_alg == IPSEC_CRYPTO_ALG_AES_GCM_128) || (_alg == IPSEC_CRYPTO_ALG_AES_GCM_192) || (_alg == IPSEC_CRYPTO_ALG_AES_GCM_256)))

#define IPSEC_CRYPTO_ALG_IS_CTR(_alg) \
    (((_alg == IPSEC_CRYPTO_ALG_AES_CTR_128) || (_alg == IPSEC_CRYPTO_ALG_AES_CTR_192) || (_alg == IPSEC_CRYPTO_ALG_AES_CTR_256)))

#define IPSEC_CRYPTO_ALG_CTR_AEAD_OTHERS(_alg) (_alg == IPSEC_CRYPTO_ALG_CHACHA20_POLY1305)
...
typedef enum { IPSEC_PROTOCOL_AH = 0, IPSEC_PROTOCOL_ESP = 1 } __clib_packed ipsec_protocol_t;
...
#define foreach_ipsec_sa_err                                                           \
    ...
    _(4, CRYPTO_ENGINE_ERROR, crypto_engine_error, "crypto engine error (dropped)")    \
    _(5, REPLAY, replay, "SA replayed packet")                                         \
    _(6, RUNT, runt, "undersized packet")                                              \
    _(7, NO_BUFFERS, no_buffers, "no buffers (dropped)")                               \
    _(8, OVERSIZED_HEADER, oversized_header, "buffer with oversized header (dropped)") \
    _(9, NO_TAIL_SPACE, no_tail_space, "no enough buffer tail space (dropped)")        \
...
extern int     ipsec_sa_update(u32 id, u16 src_port, u16 dst_port, const tunnel_t *tun, bool is_tun);
...
extern void    ipsec_sa_set_crypto_alg(ipsec_sa_t *sa, ipsec_crypto_alg_t crypto_alg);
extern void    ipsec_sa_set_integ_alg(ipsec_sa_t *sa, ipsec_integ_alg_t integ_alg);
...
extern uword unformat_ipsec_crypto_alg(unformat_input_t *input, va_list *args);
...
#define IPSEC_SA_ANTI_REPLAY_WINDOW_SIZE(_sa) \
    (u32)(PREDICT_FALSE(ipsec_sa_is_set_ANTI_REPLAY_HUGE(_sa)) ? clib_bitmap_bytes(_sa->replay_window_huge) * 8 : BITS(_sa->replay_window))

#define IPSEC_SA_ANTI_REPLAY_WINDOW_SIZE_KNOWN_WIN(_sa, _is_huge) \
    (u32)(_is_huge ? clib_bitmap_bytes(_sa->replay_window_huge) * 8 : BITS(_sa->replay_window))

#define IPSEC_SA_ANTI_REPLAY_WINDOW_N_SEEN(_sa)                                                                      \
    (u64)(PREDICT_FALSE(ipsec_sa_is_set_ANTI_REPLAY_HUGE(_sa)) ? clib_bitmap_count_set_bits(_sa->replay_window_huge) \
                                                               : count_set_bits(_sa->replay_window))

#define IPSEC_SA_ANTI_REPLAY_WINDOW_N_SEEN_KNOWN_WIN(_sa, _is_huge) \
    (u64)(_is_huge ? clib_bitmap_count_set_bits(_sa->replay_window_huge) : count_set_bits(_sa->replay_window))

#define IPSEC_SA_ANTI_REPLAY_WINDOW_MAX_INDEX(_sa) (u32)(IPSEC_SA_ANTI_REPLAY_WINDOW_SIZE(_sa) - 1)

#define IPSEC_SA_ANTI_REPLAY_WINDOW_MAX_INDEX_KNOWN_WIN(_sa, _is_huge) (u32)(IPSEC_SA_ANTI_REPLAY_WINDOW_SIZE(_sa, _is_huge) - 1)
 ...
 *  Bl = Tl - W + 1
 */
#define IPSEC_SA_ANTI_REPLAY_WINDOW_LOWER_BOUND(_sa) (u32)(_sa->seq - IPSEC_SA_ANTI_REPLAY_WINDOW_SIZE(_sa) + 1)

#define IPSEC_SA_ANTI_REPLAY_WINDOW_LOWER_BOUND_KNOWN_WIN(_sa, _is_huge) \
    (u32)(_sa->seq - IPSEC_SA_ANTI_REPLAY_WINDOW_SIZE_KNOWN_WIN(_sa, _is_huge) + 1)

always_inline u64 ipsec_sa_anti_replay_get_64b_window(const ipsec_sa_t *sa)
{
    if (!ipsec_sa_is_set_ANTI_REPLAY_HUGE(sa)) return sa->replay_window;

    u64 w;
    u32 window_size  = IPSEC_SA_ANTI_REPLAY_WINDOW_SIZE(sa);
    u32 tl_win_index = sa->seq & (window_size - 1);

    if (PREDICT_TRUE(tl_win_index >= 63)) return clib_bitmap_get_multiple(sa->replay_window_huge, tl_win_index - 63, 64);

    w = clib_bitmap_get_multiple_no_check(sa->replay_window_huge, 0, tl_win_index + 1) << (63 - tl_win_index);
    w |= clib_bitmap_get_multiple_no_check(sa->replay_window_huge, window_size - 63 + tl_win_index, 63 - tl_win_index);

    return w;
}

always_inline int ipsec_sa_anti_replay_check(const ipsec_sa_t *sa, u32 seq, bool ar_huge)
{
 ...
 */
always_inline int ipsec_sa_anti_replay_and_sn_advance(const ipsec_sa_t *sa,
                                                      u32               seq,
                                                      u32               hi_seq_used,
                                                      bool              post_decrypt,
                                                      u32              *hi_seq_req,
                                                      bool              ar_huge)
{
    ...
    u32 window_size        = IPSEC_SA_ANTI_REPLAY_WINDOW_SIZE_KNOWN_WIN(sa, ar_huge);
    u32 window_lower_bound = IPSEC_SA_ANTI_REPLAY_WINDOW_LOWER_BOUND_KNOWN_WIN(sa, ar_huge);

    if (!ipsec_sa_is_set_USE_ESN(sa)) {
        if (hi_seq_req) /* no ESN, therefore the hi-seq is always 0 */
            *hi_seq_req = 0;

        if (!ipsec_sa_is_set_USE_ANTI_REPLAY(sa)) return 0;

        if (PREDICT_TRUE(seq > sa->seq)) return 0;

        /* does the packet fall out on the left of the window */
        if (sa->seq >= seq + window_size) return 1;
        ...
        if (hi_seq_req) {
            if (seq >= sa->seq)
                ...
                *hi_seq_req = sa->seq_hi;
                ...
                if (sa->seq - seq > (1 << 30)) /* It's really really really old => it wrapped */
    ...
    if (PREDICT_TRUE(window_size > 0 && sa->seq >= window_size - 1)) {
        ...
        if (seq < window_lower_bound) {
            ...
            if (post_decrypt) {
                ...
                if (hi_seq_req) *hi_seq_req = sa->seq_hi + 1;
                return 0;
            }
        }
        else {
             ...
             * end of the window.
             */
            if (hi_seq_req) *hi_seq_req = sa->seq_hi;
            if (seq <= sa->seq)
                /*
...
always_inline u32 ipsec_sa_anti_replay_window_shift(ipsec_sa_t *sa, u32 inc, bool ar_huge)
{
    u32 n_lost      = 0;
    u32 seen        = 0;
    u32 window_size = IPSEC_SA_ANTI_REPLAY_WINDOW_SIZE_KNOWN_WIN(sa, ar_huge);
    ...
    if (inc < window_size) {
        if (ar_huge) {
            ...
            u32            window_next_lower_bound = (window_lower_bound + inc) & (window_size - 1);
            ...
            if (i_word_start + inc <= uword_bits) {
                ...
                full_words = (inc + i_word_start - uword_bits - i_word_end) >> log2_uword_bits;
                ...
                while (full_words >= 8) {
                    if (full_words >= 16) {
                        ...
                        clib_prefetch_store(&window[(i_block + 8) & (n_blocks - 1)]);
                    }
            ...
            clib_bitmap_set_no_check(window, (sa->seq + inc) & (window_size - 1), 1);
        }
        else {
            /*
            ...
            sa->replay_window = ((sa->replay_window) >> inc) | (1ULL << (window_size - 1));
        }
        ...
        n_lost = window_size - IPSEC_SA_ANTI_REPLAY_WINDOW_N_SEEN_KNOWN_WIN(sa, ar_huge);
        ...
        if (PREDICT_FALSE(ar_huge)) {
            ...
            clib_bitmap_set_no_check(sa->replay_window_huge, (sa->seq + inc) & (window_size - 1), 1);
        }
        else {
 ...
 * the branch cost.
 */
always_inline u64 ipsec_sa_anti_replay_advance(ipsec_sa_t *sa, u32 thread_index, u32 seq, u32 hi_seq, bool ar_huge)
{
    u64 n_lost      = 0;
    u32 window_size = IPSEC_SA_ANTI_REPLAY_WINDOW_SIZE_KNOWN_WIN(sa, ar_huge);
    u32 pos;
    ...
    if (ipsec_sa_is_set_USE_ESN(sa)) {
        ...
        if (wrap == 0 && seq > sa->seq) {
            ...
            pos = ~seq + sa->seq + 1;
            if (ar_huge)
                clib_bitmap_set_no_check(sa->replay_window_huge, seq & (window_size - 1), 1);
            else
 ...
 */
always_inline u16 ipsec_sa_assign_thread(u16 thread_id)
{
    return ((thread_id) ? thread_id : (unix_time_now_nsec() % vlib_num_workers()) + 1);
}

always_inline ipsec_sa_t *ipsec_sa_get(u32 sa_index)
{
