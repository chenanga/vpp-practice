...
static u8 *format_ipsec_policy_with_suffix(u8 *s, va_list *args, u8 *suffix)
{
...
u8 *format_ipsec_fp_policy(u8 *s, va_list *args)
{
...
static int ipsec_fp_table_walk_ip4_cb(clib_bihash_kv_16_8_t *kvp, void *arg)
{
    ...
    vec_foreach (policy_id, val->fp_policies_ids) {
        ...
        if (p->type == ctx->t) vec_add1(ctx->policies, *policy_id);
    }

    return BIHASH_WALK_CONTINUE;
}

static int ipsec_fp_table_walk_ip6_cb(clib_bihash_kv_40_8_t *kvp, void *arg)
{
    ipsec_fp_walk_ctx_t *ctx = (ipsec_fp_walk_ctx_t *) arg;
    ipsec_main_t        *im  = &ipsec_main;
    ipsec_policy_t      *p;
...
u8 *format_ipsec_fp_policies(u8 *s, va_list *args)
{
    ipsec_main_t           *im  = &ipsec_main;
    ...
    switch (t) {
        ...
        case IPSEC_SPD_POLICY_IP4_INBOUND_DISCARD:
            if (INDEX_INVALID != ip4_in_lookup_hash_idx) {
                clib_bihash_16_8_t *bihash_table = pool_elt_at_index(im->fp_ip4_lookup_hashes_pool, ip4_in_lookup_hash_idx);

                clib_bihash_foreach_key_value_pair_16_8(bihash_table, ipsec_fp_table_walk_ip4_cb, &ctx);
            }
        ...
        case IPSEC_SPD_POLICY_IP6_INBOUND_DISCARD:
            if (INDEX_INVALID != ip6_in_lookup_hash_idx) {
                clib_bihash_40_8_t *bihash_table = pool_elt_at_index(im->fp_ip6_lookup_hashes_pool, ip6_in_lookup_hash_idx);

                clib_bihash_foreach_key_value_pair_40_8(bihash_table, ipsec_fp_table_walk_ip6_cb, &ctx);
            }

            break;
        case IPSEC_SPD_POLICY_IP4_OUTBOUND:
            if (INDEX_INVALID != ip4_out_lookup_hash_idx) {
                clib_bihash_16_8_t *bihash_table = pool_elt_at_index(im->fp_ip4_lookup_hashes_pool, ip4_out_lookup_hash_idx);
        ...
        case IPSEC_SPD_POLICY_IP6_OUTBOUND:
            if (INDEX_INVALID != ip6_out_lookup_hash_idx) {
                clib_bihash_40_8_t *bihash_table = pool_elt_at_index(im->fp_ip6_lookup_hashes_pool, ip6_out_lookup_hash_idx);
...
u8 *format_ipsec_out_spd_flow_cache(u8 *s, va_list *args)
{
    ipsec_main_t *im = &ipsec_main;

    s = format(s, "\nipv4-outbound-spd-flow-cache-entries: %u", im->ipsec4_out_spd_flow_cache_entries);

    return (s);
}

u8 *format_ipsec_in_spd_flow_cache(u8 *s, va_list *args)
{
    ipsec_main_t *im = &ipsec_main;

    s = format(s, "\nipv4-inbound-spd-flow-cache-entries: %u", im->ipsec4_in_spd_flow_cache_entries);
...
#define _(v, f, str) \
    if (flags & IPSEC_SA_FLAG_##f) s = format(s, "%s ", str);
...
u8 *format_ipsec_sa(u8 *s, va_list *args)
{
    ...
    if (!(flags & IPSEC_FORMAT_DETAIL)) goto done;

    s = format(s, "\n   locks %d", sa->node.fn_locks);
    s = format(s, "\n   salt 0x%x", clib_net_to_host_u32(sa->salt));
    s = format(s, "\n   thread-index:%d", sa->thread_index);
    s = format(s, "\n   seq %u seq-hi %u", sa->seq, sa->seq_hi);
    s = format(s, "\n   window-size: %llu", IPSEC_SA_ANTI_REPLAY_WINDOW_SIZE(sa));
    s = format(s, "\n   window: Bl <- %U Tl", format_ipsec_replay_window, ipsec_sa_anti_replay_get_64b_window(sa));
    ...
    if (sa->integ_alg && (flags & IPSEC_FORMAT_INSECURE))
        ...
        s = format(s, " key [redacted]");
    s = format(s, "\n   UDP:[src:%d dst:%d]", clib_host_to_net_u16(sa->udp_hdr.src_port), clib_host_to_net_u16(sa->udp_hdr.dst_port));

    vlib_get_combined_counter(&ipsec_sa_counters, sai, &counts);
    s = format(s, "\n   tx/rx:[packets:%Ld bytes:%Ld]", counts.packets, counts.bytes);
    s = format(s, "\n   SA errors:");
...
#undef _
        ...
        if (ipsec_sa_is_set_IS_TUNNEL(sa)) s = format(s, "\n%U", format_tunnel, &sa->tunnel, 3);
...
u8 *format_ipsec_tun_protect_index(u8 *s, va_list *args)
{
    ...
    if (pool_is_free_index(ipsec_tun_protect_pool, itpi)) return (format(s, "No such tunnel index: %d", itpi));
...
u8 *format_ipsec_tun_protect_flags(u8 *s, va_list *args)
{
    ...
    if (IPSEC_PROTECT_NONE == flags) s = format(s, "none");
#define _(a, b, c)                                              \
...
u8 *format_ipsec_tun_protect(u8 *s, va_list *args)
{
    ...
    s = format(s,
               ...
               itp->itp_flags);
    if (!ip_address_is_zero(itp->itp_key)) s = format(s, ": %U", format_ip_address, itp->itp_key);
    s = format(s, "\n output-sa:");
    s = format(s, "\n  %U", format_ipsec_sa, itp->itp_out_sa, IPSEC_FORMAT_BRIEF);

    s = format(s, "\n input-sa:");
    FOR_EACH_IPSEC_PROTECT_INPUT_SAI(itp, sai, ({ s = format(s, "\n  %U", format_ipsec_sa, sai, IPSEC_FORMAT_BRIEF); }));
...
u8 *format_ipsec_itf(u8 *s, va_list *a)
{
    ...
    s   = format(s, "[%d] %U %U", ii, format_vnet_sw_if_index_name, vnet_get_main(), itf->ii_sw_if_index, format_tunnel_mode, itf->ii_mode);
