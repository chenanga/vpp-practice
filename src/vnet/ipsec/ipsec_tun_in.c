...
always_inline u16 ipsec_ip4_if_no_tunnel(vlib_node_runtime_t *node, vlib_buffer_t *b, const esp_header_t *esp, const ip4_header_t *ip4)
{
    if (PREDICT_FALSE(0 == esp->spi)) {
        b->error       = node->errors[IPSEC_TUN_ERROR_SPI_0];
        b->punt_reason = ipsec_punt_reason[(ip4->protocol == IP_PROTOCOL_UDP ? IPSEC_PUNT_IP4_SPI_UDP_0 : IPSEC_PUNT_IP4_NO_SUCH_TUNNEL)];
    }
    else {
...
always_inline u16 ipsec_ip6_if_no_tunnel(vlib_node_runtime_t *node, vlib_buffer_t *b, const esp_header_t *esp, const ip6_header_t *ip6)
{
    if (PREDICT_FALSE(0 == esp->spi)) {
        b->error       = node->errors[IPSEC_TUN_ERROR_SPI_0];
        b->punt_reason = ipsec_punt_reason[(ip6->protocol == IP_PROTOCOL_UDP ? IPSEC_PUNT_IP6_SPI_UDP_0 : IPSEC_PUNT_IP6_NO_SUCH_TUNNEL)];
    }
    else {
        b->error       = node->errors[IPSEC_TUN_ERROR_NO_TUNNEL];
...
always_inline uword ipsec_tun_protect_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, int is_ip6)
{
    ...
    clib_memset_u16(nexts, is_ip6 ? im->esp6_decrypt_next_index : im->esp4_decrypt_next_index, n_left_from);
    ...
    while (n_left_from > 0) {
        ...
        if (is_ip6) {
            ...
            if (ip60->protocol == IP_PROTOCOL_UDP) {
                ...
                esp0        = (esp_header_t *) ((u8 *) ip60 + sizeof(ip6_header_t) + sizeof(udp_header_t));
                hdr_sz0     = 0;
                buf_rewind0 = sizeof(ip6_header_t) + sizeof(udp_header_t);

                const udp_header_t *udp0 = (udp_header_t *) ((u8 *) ip60 + sizeof(ip6_header_t));

                /* length 9 = sizeof(udp_header) + 1 byte of special SPI */
                if (clib_net_to_host_u16(udp0->length) == 9 && esp0->spi_bytes[0] == 0xff) {
            ...
            if (ip40->protocol == IP_PROTOCOL_UDP) {
                /* NAT UDP port 4500 case, don't advance any more */
                esp0        = (esp_header_t *) ((u8 *) ip40 + ip4_header_bytes(ip40) + sizeof(udp_header_t));
                hdr_sz0     = 0;
                buf_rewind0 = ip4_header_bytes(ip40) + sizeof(udp_header_t);

                const udp_header_t *udp0 = (udp_header_t *) ((u8 *) ip40 + ip4_header_bytes(ip40));
        ...
        if (len0 < sizeof(esp_header_t)) {
            ...
            next[0] = is_ip6 ? VNET_DEVICE_INPUT_NEXT_IP6_DROP : VNET_DEVICE_INPUT_NEXT_IP4_DROP;
            vlib_buffer_advance(b[0], -buf_rewind0);
            goto trace00;
        }
        ...
        if (is_ip6) {
            ...
            if (memcmp(key60, &last_key6, sizeof(last_key6)) == 0) {
                ...
                int rv = clib_bihash_search_inline_24_16(&im->tun6_protect_by_key, &bkey60);
                if (!rv) {
                    clib_memcpy_fast(&itr0, &bkey60.value, sizeof(itr0));
                    clib_memcpy_fast(&last_result, &bkey60.value, sizeof(last_result));
            ...
            if (key40->key == last_key4.key) {
                ...
                int rv = clib_bihash_search_inline_8_16(&im->tun4_protect_by_key, &bkey40);
                if (!rv) {
                    clib_memcpy_fast(&itr0, &bkey40.value, sizeof(itr0));
                    clib_memcpy_fast(&last_result, &bkey40.value, sizeof(last_result));
        ...
        if (PREDICT_FALSE(!vnet_sw_interface_is_admin_up(vnm, sw_if_index0))) {
            vlib_increment_combined_counter(drop_counter, thread_index, sw_if_index0, 1, len0);
            n_disabled++;
            ...
            if (PREDICT_TRUE(sw_if_index0 == last_sw_if_index)) {
                ...
                if (n_packets && !(itr0.flags & IPSEC_PROTECT_ENCAPED)) {
                    vlib_increment_combined_counter(rx_counter, thread_index, last_sw_if_index, n_packets, n_bytes);
                }
            ...
            next[0] = is_ip6 ? im->esp6_decrypt_tun_next_index : im->esp4_decrypt_tun_next_index;
    ...
    trace00:
        if (PREDICT_FALSE(is_trace)) {
            if (b[0]->flags & VLIB_BUFFER_IS_TRACED) {
                ipsec_tun_protect_input_trace_t *tr = vlib_add_trace(vm, node, b[0], sizeof(*tr));
                if (is_ip6)
                    clib_memcpy(&tr->kv6, &bkey60, sizeof(tr->kv6));
                else
                    clib_memcpy(&tr->kv4, &bkey40, sizeof(tr->kv4));
                tr->is_ip6 = is_ip6;
                tr->seq    = (len0 >= sizeof(*esp0) ? clib_host_to_net_u32(esp0->seq) : ~0);
            }
        }
    ...
    vlib_node_increment_counter(vm, node->node_index, IPSEC_TUN_ERROR_RX, from_frame->n_vectors - (n_disabled + n_no_tunnel));
    vlib_node_increment_counter(vm, node->node_index, IPSEC_TUN_ERROR_NO_TUNNEL, n_no_tunnel);
