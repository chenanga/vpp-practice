...
static_always_inline u64 ipsec4_hash_16_8(ipsec4_hash_kv_16_8_t *v)
{
...
static_always_inline int ipsec4_hash_key_compare_16_8(u64 *a, u64 *b)
{
...
static_always_inline void ipsec_spinlock_lock(i32 *lock)
{
    i32 free = 0;
...
static_always_inline void ipsec_spinlock_unlock(i32 *lock)
{
 ...
 * to next nodes.
 */
always_inline void ipsec_set_next_index(vlib_buffer_t       *b,
                                        ...
                                        u32                  sa_index)
{
    ...
    if (PREDICT_TRUE(ipsec_sa_err != ~0)) vlib_increment_simple_counter(&ipsec_sa_err_counters[ipsec_sa_err], thread_index, sa_index, 1);
}
...
extern clib_error_t *ipsec_register_next_header(vlib_main_t *vm, u8 next_header, const char *next_node);
