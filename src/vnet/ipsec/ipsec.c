...
clib_error_t *ipsec_register_next_header(vlib_main_t *vm, u8 next_header, const char *next_node)
{
    ...
    for (i = 1; i < ARRAY_LEN(esp_decrypt_nodes); i++) {
        ...
        if (slot != vlib_node_get_next(vm, esp_decrypt_nodes[i]->index, node->index))
            return clib_error_return(0, "next node already exists with different next index");
    ...
    if (~0 == slot) {
        ...
        for (i = 0; i < ARRAY_LEN(esp_decrypt_nodes); i++)
            vlib_node_add_next_with_slot(vm, esp_decrypt_nodes[i]->index, node->index, slot);
    }
...
static clib_error_t *ipsec_check_ah_support(ipsec_sa_t *sa)
{
    ...
    if (sa->integ_alg == IPSEC_INTEG_ALG_NONE) return clib_error_return(0, "unsupported none integ-alg");

    if (!vnet_crypto_is_set_handler(im->integ_algs[sa->integ_alg].alg))
        return clib_error_return(0, "No crypto engine support for %U", format_ipsec_integ_alg, sa->integ_alg);
...
static clib_error_t *ipsec_check_esp_support(ipsec_sa_t *sa)
{
    ...
    if (IPSEC_CRYPTO_ALG_NONE != sa->crypto_alg) {
        if (!vnet_crypto_is_set_handler(im->crypto_algs[sa->crypto_alg].alg))
            return clib_error_return(0, "No crypto engine support for %U", format_ipsec_crypto_alg, sa->crypto_alg);
    }
...
{
    ipsec_ah_backend_t *ah = pool_elt_at_index(im->ah_backends, im->ah_current_backend);
    if (ah->add_del_sa_sess_cb) {
        clib_error_t *err = ah->add_del_sa_sess_cb(sa_index, is_add);
        if (err) return err;
    }
    ipsec_esp_backend_t *esp = pool_elt_at_index(im->esp_backends, im->esp_current_backend);
...
static inline uword ipsec_udp_registration_key(u16 port, u8 is_ip4)
{
...
void ipsec_unregister_udp_port(u16 port, u8 is_ip4)
{
    ipsec_main_t *im = &ipsec_main;
    u32           n_regs;
    uword        *p, key;
...
void ipsec_register_udp_port(u16 port, u8 is_ip4)
{
    ipsec_main_t *im = &ipsec_main;
    ...
    key        = ipsec_udp_registration_key(port, is_ip4);
    node_index = (is_ip4) ? ipsec4_tun_input_node.index : ipsec6_tun_input_node.index;
    p          = hash_get(im->udp_port_registrations, key);

    n_regs = (p ? p[0] : 0);

    if (0 == n_regs++) udp_register_dst_port(vlib_get_main(), port, node_index, is_ip4);
                              ...
                              add_del_sa_sess_cb_t ah_add_del_sa_sess_cb)
{
    ...
    ipsec_add_node(vm, ah4_encrypt_node_name, "ipsec4-output-feature", &b->ah4_encrypt_node_index, &b->ah4_encrypt_next_index);
    ipsec_add_node(vm, ah4_decrypt_node_name, "ipsec4-input-feature", &b->ah4_decrypt_node_index, &b->ah4_decrypt_next_index);
    ipsec_add_node(vm, ah6_encrypt_node_name, "ipsec6-output-feature", &b->ah6_encrypt_node_index, &b->ah6_encrypt_next_index);
    ipsec_add_node(vm, ah6_decrypt_node_name, "ipsec6-input-feature", &b->ah6_decrypt_node_index, &b->ah6_decrypt_next_index);
                               ...
                               add_del_sa_sess_cb_t esp_add_del_sa_sess_cb)
{
    ...
    ipsec_add_node(vm, esp4_encrypt_node_name, "ipsec4-output-feature", &b->esp4_encrypt_node_index, &b->esp4_encrypt_next_index);
    ipsec_add_node(vm, esp4_decrypt_node_name, "ipsec4-input-feature", &b->esp4_decrypt_node_index, &b->esp4_decrypt_next_index);
    ipsec_add_node(vm, esp6_encrypt_node_name, "ipsec6-output-feature", &b->esp6_encrypt_node_index, &b->esp6_encrypt_next_index);
    ipsec_add_node(vm, esp6_decrypt_node_name, "ipsec6-input-feature", &b->esp6_decrypt_node_index, &b->esp6_decrypt_next_index);
    ipsec_add_node(vm, esp4_decrypt_tun_node_name, "ipsec4-tun-input", &b->esp4_decrypt_tun_node_index, &b->esp4_decrypt_tun_next_index);
    ipsec_add_node(vm, esp6_decrypt_tun_node_name, "ipsec6-tun-input", &b->esp6_decrypt_tun_node_index, &b->esp6_decrypt_tun_next_index);

    b->esp6_encrypt_tun_node_index     = vlib_get_node_by_name(vm, (u8 *) esp6_encrypt_node_tun_name)->index;
    b->esp_mpls_encrypt_tun_node_index = vlib_get_node_by_name(vm, (u8 *) esp_mpls_encrypt_node_tun_name)->index;
    b->esp4_encrypt_tun_node_index     = vlib_get_node_by_name(vm, (u8 *) esp4_encrypt_node_tun_name)->index;
...
clib_error_t *ipsec_rsc_in_use(ipsec_main_t *im)
{
    ...
    if (pool_elts(ipsec_sa_pool) > 0) return clib_error_return(0, "%d SA entries configured", pool_elts(ipsec_sa_pool));
    if (ipsec_itf_count() > 0) return clib_error_return(0, "%d IPSec interface configured", ipsec_itf_count());
...
{
    if (ipsec_rsc_in_use(im)) return VNET_API_ERROR_RSRC_IN_USE;

    if (pool_is_free_index(im->ah_backends, backend_idx)) return VNET_API_ERROR_INVALID_VALUE;
...
int ipsec_select_esp_backend(ipsec_main_t *im, u32 backend_idx)
{
    ...
    if (pool_is_free_index(im->esp_backends, backend_idx)) return VNET_API_ERROR_INVALID_VALUE;
...
static void crypto_engine_backend_register_post_node(vlib_main_t *vm)
{
    ...
    eit->esp4_post_next         = vnet_crypto_register_post_node(vm, "esp4-encrypt-post");
    eit->esp6_post_next         = vnet_crypto_register_post_node(vm, "esp6-encrypt-post");
    eit->esp4_tun_post_next     = vnet_crypto_register_post_node(vm, "esp4-encrypt-tun-post");
    eit->esp6_tun_post_next     = vnet_crypto_register_post_node(vm, "esp6-encrypt-tun-post");
    eit->esp_mpls_tun_post_next = vnet_crypto_register_post_node(vm, "esp-mpls-encrypt-tun-post");

    dit                     = &esp_decrypt_async_next;
    dit->esp4_post_next     = vnet_crypto_register_post_node(vm, "esp4-decrypt-post");
    dit->esp6_post_next     = vnet_crypto_register_post_node(vm, "esp6-decrypt-post");
    dit->esp4_tun_post_next = vnet_crypto_register_post_node(vm, "esp4-decrypt-tun-post");
    dit->esp6_tun_post_next = vnet_crypto_register_post_node(vm, "esp6-decrypt-tun-post");
}
...
static clib_error_t *ipsec_init(vlib_main_t *vm)
{
    ...
    u32 idx = ipsec_register_ah_backend(vm,
                                        im,
    ...
    idx                     = ipsec_register_esp_backend(vm,
                                     im,
                                     "crypto engine backend",
                                     "esp4-encrypt",
                                     "esp4-encrypt-tun",
    ...
    if ((error = vlib_call_init_function(vm, ipsec_cli_init))) return error;
    ...
    im->epoch_count                       = 0;
    im->ipsec4_out_spd_hash_num_buckets   = IPSEC4_OUT_SPD_DEFAULT_HASH_NUM_BUCKETS;
    ...
    vec_validate_init_empty_aligned(im->next_header_registrations, 255, ~0, CLIB_CACHE_LINE_BYTES);
...
static clib_error_t *ipsec_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "ipv6-outbound-spd-fast-path on")) {
            ...
            im->fp_spd_ipv4_in_is_enabled = 0;
        else if (unformat(input, "spd-fast-path-num-buckets %d", &ipsec_spd_fp_num_buckets)) {
            /* Number of bihash buckets is power of 2 >= input */
            im->fp_lookup_hash_buckets = 1ULL << max_log2(ipsec_spd_fp_num_buckets);
        }
            ...
            im->output_flow_cache_flag = 0;
        else if (unformat(input, "ipv4-outbound-spd-hash-buckets %d", &ipsec4_out_spd_hash_num_buckets)) {
            /* Size of hash is power of 2 >= number of buckets */
            im->ipsec4_out_spd_hash_num_buckets = 1ULL << max_log2(ipsec4_out_spd_hash_num_buckets);
        }
            ...
            im->input_flow_cache_flag = 0;
        else if (unformat(input, "ipv4-inbound-spd-hash-buckets %d", &ipsec4_in_spd_hash_num_buckets)) {
            im->ipsec4_in_spd_hash_num_buckets = 1ULL << max_log2(ipsec4_in_spd_hash_num_buckets);
        }
        else if (unformat(input, "ip4 %U", unformat_vlib_cli_sub_input, &sub_input)) {
            uword table_size = ~0;
            u32   n_buckets  = ~0;
            ...
            while (unformat_check_input(&sub_input) != UNFORMAT_END_OF_INPUT) {
                if (unformat(&sub_input, "num-buckets %u", &n_buckets))
                    ...
                    return clib_error_return(0, "unknown input `%U'", format_unformat_error, &sub_input);
            }

            ipsec_tun_table_init(AF_IP4, table_size, n_buckets);
        }
        else if (unformat(input, "ip6 %U", unformat_vlib_cli_sub_input, &sub_input)) {
            uword table_size = ~0;
            u32   n_buckets  = ~0;
        ...
        else if (unformat(input, "async-handoff-queue-size %d", &handoff_queue_size)) {
    ...
    if (im->output_flow_cache_flag) {
        vec_add2(im->ipsec4_out_spd_hash_tbl, im->ipsec4_out_spd_hash_tbl, im->ipsec4_out_spd_hash_num_buckets);
    }
    if (im->input_flow_cache_flag) {
        vec_add2(im->ipsec4_in_spd_hash_tbl, im->ipsec4_in_spd_hash_tbl, im->ipsec4_in_spd_hash_num_buckets);
    }

    if (fp_spd_ip4_enabled) pool_alloc_aligned(im->fp_ip4_lookup_hashes_pool, IPSEC_FP_IP4_HASHES_POOL_SIZE, CLIB_CACHE_LINE_BYTES);

    if (fp_spd_ip6_enabled) pool_alloc_aligned(im->fp_ip6_lookup_hashes_pool, IPSEC_FP_IP6_HASHES_POOL_SIZE, CLIB_CACHE_LINE_BYTES);
