...
static inline uword ipsec_output_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, int is_ipv6)
{
    ...
    while (n_left_from > 0) {
        ...
        ip0          = (ip4_header_t *) ((u8 *) vlib_buffer_get_current(b0) + iph_offset);
        ...
        if (is_ipv6) {
            ip6_0 = (ip6_header_t *) ((u8 *) vlib_buffer_get_current(b0) + iph_offset);
	  ...
	  clib_warning ("sw_if_index0 %u spd_index0 %u spd_id %u",
        ...
        if (PREDICT_TRUE(p0 != NULL)) {
                ...
                if (PREDICT_FALSE(b0->flags & VNET_BUFFER_F_OFFLOAD)) {
                    ...
                    if (is_ipv6) {
                        if (PREDICT_FALSE(oflags & VNET_BUFFER_OFFLOAD_F_TCP_CKSUM)) {
                            tcp0->checksum = ip6_tcp_udp_icmp_compute_checksum(vm, b0, ip6_0, &bogus);
                        }
            ...
            vlib_increment_combined_counter(&ipsec_spd_policy_counters, thread_index, pi0, 1, bytes0);
        }
        else {
            pi0 = ~0;
            nc_nomatch++;
        ...
        if (PREDICT_FALSE((last_next_node_index != next_node_index) || f == 0)) {
            ...
            if (f) vlib_put_frame_to_node(vm, last_next_node_index, f);
        ...
        if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE) && PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
            ...
            if (spd0) tr->spd_id = spd0->id;
            tr->policy_id = pi0;
        }
    }

    vlib_put_frame_to_node(vm, next_node_index, f);
    vlib_node_increment_counter(vm, node->node_index, IPSEC_OUTPUT_ERROR_POLICY_PROTECT, nc_protect);
    vlib_node_increment_counter(vm, node->node_index, IPSEC_OUTPUT_ERROR_POLICY_BYPASS, nc_bypass);
    vlib_node_increment_counter(vm, node->node_index, IPSEC_OUTPUT_ERROR_POLICY_DISCARD, nc_discard);
    vlib_node_increment_counter(vm, node->node_index, IPSEC_OUTPUT_ERROR_POLICY_NO_MATCH, nc_nomatch);
