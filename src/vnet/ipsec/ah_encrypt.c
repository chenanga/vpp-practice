...
static_always_inline void ah_process_ops(vlib_main_t *vm, vlib_node_runtime_t *node, vnet_crypto_op_t *ops, vlib_buffer_t *b[], u16 *nexts)
{
    ...
    while (n_fail) {
        ...
        if (op->status != VNET_CRYPTO_OP_STATUS_COMPLETED) {
            u32 bi = op->user_data;
            ah_encrypt_set_next_index(b[bi],
                                      node,
                                      vm->thread_index,
...
always_inline uword ah_encrypt_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ip6)
{
    ...
    while (n_left > 0) {
        ...
        if (PREDICT_FALSE(esp_seq_advance(sa0))) {
            ah_encrypt_set_next_index(b[0], node, vm->thread_index, AH_ENCRYPT_ERROR_SEQ_CYCLED, 0, next, AH_ENCRYPT_NEXT_DROP, current_sa_index);
            pd->skip = 1;
            goto next;
        }
        ...
        if (is_ip6) {
            ...
            if (PREDICT_FALSE(ipsec_sa_is_set_IS_TUNNEL(sa0))) {
                ...
                tunnel_encap_fixup_6o6(sa0->tunnel_flags, &ih6_0->ip6, &oh6_0->ip6);
            }
            pd->ip_version_traffic_class_and_flow_label = oh6_0->ip6.ip_version_traffic_class_and_flow_label;
            ...
            oh6_0->ip6.payload_length = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, b[0]) - sizeof(ip6_header_t));
            oh6_0->ah.hdrlen          = (sizeof(ah_header_t) + icv_size + padding_len) / 4 - 2;
        }
        else {
            ...
            if (PREDICT_FALSE(ipsec_sa_is_set_IS_TUNNEL(sa0))) {
                if (sa0->tunnel.t_dscp)
                    ...
                    if (!(sa0->tunnel_flags & TUNNEL_ENCAP_DECAP_FLAG_ENCAP_COPY_DSCP)) pd->tos &= 0x3;
                    if (!(sa0->tunnel_flags & TUNNEL_ENCAP_DECAP_FLAG_ENCAP_COPY_ECN)) pd->tos &= 0xfc;
                }
            }
            else {
                pd->tos = ih0->ip4.tos;
            ...
            clib_memcpy_fast(&oh0->ip4, &ip4_hdr_template, sizeof(ip4_header_t) - sizeof(ip4_address_pair_t));

            oh0->ip4.length = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, b[0]));
            oh0->ah.spi     = clib_net_to_host_u32(sa0->spi);
            oh0->ah.seq_no  = clib_net_to_host_u32(sa0->seq);
            oh0->ah.nexthdr = next_hdr_type;
            oh0->ah.hdrlen  = (sizeof(ah_header_t) + icv_size + padding_len) / 4 - 2;
        }

        if (PREDICT_TRUE(!is_ip6 && ipsec_sa_is_set_IS_TUNNEL(sa0) && !ipsec_sa_is_set_IS_TUNNEL_V6(sa0))) {
            clib_memcpy_fast(&oh0->ip4.address_pair, &sa0->ip4_hdr.address_pair, sizeof(ip4_address_pair_t));

            next[0]                                  = sa0->dpo.dpoi_next_node;
            vnet_buffer(b[0])->ip.adj_index[VLIB_TX] = sa0->dpo.dpoi_index;
        }
        else if (is_ip6 && ipsec_sa_is_set_IS_TUNNEL(sa0) && ipsec_sa_is_set_IS_TUNNEL_V6(sa0)) {
            clib_memcpy_fast(&oh6_0->ip6.src_address, &sa0->ip6_hdr.src_address, sizeof(ip6_address_t) * 2);
            next[0]                                  = sa0->dpo.dpoi_next_node;
            vnet_buffer(b[0])->ip.adj_index[VLIB_TX] = sa0->dpo.dpoi_index;
        }
        ...
        if (PREDICT_TRUE(sa0->integ_op_id)) {
            ...
            op->digest = vlib_buffer_get_current(b[0]) + ip_hdr_size + sizeof(ah_header_t);
    ...
    next:
        if (PREDICT_FALSE(b[0]->flags & VLIB_BUFFER_IS_TRACED)) {
            ...
            ah_encrypt_trace_t *tr = vlib_add_trace(vm, node, b[0], sizeof(*tr));
            tr->spi                = sa0->spi;
            tr->seq_lo             = sa0->seq;
    ...
    while (n_left) {
        if (pd->skip) goto next_pkt;

        if (is_ip6) {
            oh6_0                                              = (ip6_and_ah_header_t *) (b[0]->data + pd->current_data);
            oh6_0->ip6.hop_limit                               = pd->hop_limit;
            oh6_0->ip6.ip_version_traffic_class_and_flow_label = pd->ip_version_traffic_class_and_flow_label;
        }
        else {
...
static clib_error_t *ah_encrypt_init(vlib_main_t *vm)
{
    ...
    im->ah4_enc_fq_index = vlib_frame_queue_main_init(ah4_encrypt_node.index, im->handoff_queue_size);
    im->ah6_enc_fq_index = vlib_frame_queue_main_init(ah6_encrypt_node.index, im->handoff_queue_size);
