...
static clib_error_t *ipsec_call_add_del_callbacks(ipsec_main_t *im, ipsec_sa_t *sa, u32 sa_index, int is_add)
{
    ...
    switch (sa->protocol) {
        case IPSEC_PROTOCOL_AH:
            ...
            if (ab->add_del_sa_sess_cb) return ab->add_del_sa_sess_cb(sa_index, is_add);
            break;
        case IPSEC_PROTOCOL_ESP:
            eb = pool_elt_at_index(im->esp_backends, im->esp_current_backend);
            if (eb->add_del_sa_sess_cb) return eb->add_del_sa_sess_cb(sa_index, is_add);
            break;
    }
    return 0;
}
...
static void ipsec_sa_stack(ipsec_sa_t *sa)
{
    ...
    if (IPSEC_PROTOCOL_AH == sa->protocol)
        dpo_stack_from_node((ipsec_sa_is_set_IS_TUNNEL_V6(sa) ? im->ah6_encrypt_node_index : im->ah4_encrypt_node_index), &sa->dpo, &tmp);
    else
        dpo_stack_from_node((ipsec_sa_is_set_IS_TUNNEL_V6(sa) ? im->esp6_encrypt_node_index : im->esp4_encrypt_node_index), &sa->dpo, &tmp);
    dpo_reset(&tmp);
}

void ipsec_sa_set_async_mode(ipsec_sa_t *sa, int is_enabled)
{
    if (is_enabled) {
...
void ipsec_sa_set_crypto_alg(ipsec_sa_t *sa, ipsec_crypto_alg_t crypto_alg)
{
    ...
    if (IPSEC_CRYPTO_ALG_IS_GCM(crypto_alg) || IPSEC_CRYPTO_ALG_CTR_AEAD_OTHERS(crypto_alg)) {
...
#define _(n, s, k)                                                                                                                \
    if (sa->crypto_sync_enc_op_id == VNET_CRYPTO_OP_##n##_ENC) sa->crypto_async_enc_op_id = VNET_CRYPTO_OP_##n##_TAG16_AAD12_ENC; \
    if (sa->crypto_sync_dec_op_id == VNET_CRYPTO_OP_##n##_DEC) sa->crypto_async_dec_op_id = VNET_CRYPTO_OP_##n##_TAG16_AAD12_DEC;
        foreach_crypto_aead_alg
#undef _
    }
    else {
#define _(n, s, k)                                                                                                               \
    if (sa->crypto_sync_enc_op_id == VNET_CRYPTO_OP_##n##_ENC) sa->crypto_async_enc_op_id = VNET_CRYPTO_OP_##n##_TAG16_AAD8_ENC; \
    if (sa->crypto_sync_dec_op_id == VNET_CRYPTO_OP_##n##_DEC) sa->crypto_async_dec_op_id = VNET_CRYPTO_OP_##n##_TAG16_AAD8_DEC;
        foreach_crypto_aead_alg
#undef _
    }

#define _(c, h, s, k, d)                                                                                            \
    if (sa->crypto_sync_enc_op_id == VNET_CRYPTO_OP_##c##_ENC && sa->integ_sync_op_id == VNET_CRYPTO_OP_##h##_HMAC) \
        sa->crypto_async_enc_op_id = VNET_CRYPTO_OP_##c##_##h##_TAG##d##_ENC;                                       \
    if (sa->crypto_sync_dec_op_id == VNET_CRYPTO_OP_##c##_DEC && sa->integ_sync_op_id == VNET_CRYPTO_OP_##h##_HMAC) \
        sa->crypto_async_dec_op_id = VNET_CRYPTO_OP_##c##_##h##_TAG##d##_DEC;
    foreach_crypto_link_async_alg
#undef _
}

int ipsec_sa_update(u32 id, u16 src_port, u16 dst_port, const tunnel_t *tun, bool is_tun)
{
    ipsec_main_t *im = &ipsec_main;
    ipsec_sa_t   *sa;
    u32           sa_index;
    ...
    if (is_tun && ipsec_sa_is_set_IS_TUNNEL(sa)
        && (ip_address_cmp(&tun->t_src, &sa->tunnel.t_src) != 0 || ip_address_cmp(&tun->t_dst, &sa->tunnel.t_dst) != 0)) {
        ...
        if (ipsec_sa_is_set_IS_INBOUND(sa) && ip_address_cmp(&tun->t_src, &sa->tunnel.t_src) != 0 && !ip46_address_is_zero(&tun->t_src.ip)) {
            if (ip46_address_is_ip4(&sa->tunnel.t_src.ip)) {
                ipsec4_tunnel_kv_t    old_key, new_key;
                clib_bihash_kv_8_16_t res, *bkey = (clib_bihash_kv_8_16_t *) &old_key;

                ipsec4_tunnel_mk_key(&old_key, &sa->tunnel.t_src.ip.ip4, clib_host_to_net_u32(sa->spi));
                ipsec4_tunnel_mk_key(&new_key, &tun->t_src.ip.ip4, clib_host_to_net_u32(sa->spi));
                ...
                clib_bihash_kv_24_16_t res, *bkey = (clib_bihash_kv_24_16_t *) &old_key;

                if (!clib_bihash_search_24_16(&im->tun6_protect_by_key, bkey, &res)) {
                    clib_bihash_add_del_24_16(&im->tun6_protect_by_key, &res, 0);
                    clib_memcpy(&res.key, &new_key.key, 3);
                    clib_bihash_add_del_24_16(&im->tun6_protect_by_key, &res, 1);
                }
            }
        }
        ...
        if (!ipsec_sa_is_set_IS_INBOUND(sa)) {
            ...
            if (ipsec_sa_is_set_IS_TUNNEL_V6(sa)) {
                tunnel_build_v6_hdr(&sa->tunnel, (ipsec_sa_is_set_UDP_ENCAP(sa) ? IP_PROTOCOL_UDP : IP_PROTOCOL_IPSEC_ESP), &sa->ip6_hdr);
            }
            else {
                tunnel_build_v4_hdr(&sa->tunnel, (ipsec_sa_is_set_UDP_ENCAP(sa) ? IP_PROTOCOL_UDP : IP_PROTOCOL_IPSEC_ESP), &sa->ip4_hdr);
            }
        }
    }

    if (ipsec_sa_is_set_UDP_ENCAP(sa)) {
        if (dst_port != IPSEC_UDP_PORT_NONE && dst_port != clib_net_to_host_u16(sa->udp_hdr.dst_port)) {
            if (ipsec_sa_is_set_IS_INBOUND(sa)) {
                ipsec_unregister_udp_port(clib_net_to_host_u16(sa->udp_hdr.dst_port), !ipsec_sa_is_set_IS_TUNNEL_V6(sa));
                ipsec_register_udp_port(dst_port, !ipsec_sa_is_set_IS_TUNNEL_V6(sa));
            }
            sa->udp_hdr.dst_port = clib_host_to_net_u16(dst_port);
        }
        if (src_port != IPSEC_UDP_PORT_NONE && src_port != clib_net_to_host_u16(sa->udp_hdr.src_port))
                          ...
                          u32               *sa_out_index)
{
    ...
    p = hash_get(im->sa_index_by_sa_id, id);
    if (p) return VNET_API_ERROR_ENTRY_ALREADY_EXISTS;

    if (getrandom(rand, sizeof(rand), 0) != sizeof(rand)) return VNET_API_ERROR_INIT_FAILED;
    ...
    if (ipsec_sa_is_set_USE_ANTI_REPLAY(sa) && anti_replay_window_size > 64) ipsec_sa_set_ANTI_REPLAY_HUGE(sa);

    clib_memcpy(&sa->crypto_key, ck, sizeof(sa->crypto_key));

    sa->crypto_sync_key_index = vnet_crypto_key_add(vm, im->crypto_algs[crypto_alg].alg, (u8 *) ck->data, ck->len);
    ...
    if (integ_alg != IPSEC_INTEG_ALG_NONE) {
        sa->integ_sync_key_index = vnet_crypto_key_add(vm, im->integ_algs[integ_alg].alg, (u8 *) ik->data, ik->len);
    ...
    if (sa->crypto_async_enc_op_id && !ipsec_sa_is_set_IS_AEAD(sa))
        sa->crypto_async_key_index = vnet_crypto_key_add_linked(vm, sa->crypto_sync_key_index,
    ...
    if (ipsec_sa_is_set_IS_TUNNEL(sa) && AF_IP6 == ip_addr_version(&tun->t_src)) ipsec_sa_set_IS_TUNNEL_V6(sa);
    ...
    if (ipsec_sa_is_set_UDP_ENCAP(sa)) {
        ...
        if (ipsec_sa_is_set_IS_INBOUND(sa)) ipsec_register_udp_port(clib_host_to_net_u16(sa->udp_hdr.dst_port), !ipsec_sa_is_set_IS_TUNNEL_V6(sa));
    }
    ...
    if (ipsec_sa_is_set_ANTI_REPLAY_HUGE(sa)) {
        ...
        sa->replay_window_huge  = clib_bitmap_set_region(0, 0, 1, anti_replay_window_size);
    }
    else {
        sa->replay_window = ~0;
    }
    ...
    if (sa_out_index) *sa_out_index = sa_index;
...
static void ipsec_sa_del(ipsec_sa_t *sa)
{
    ...
    if (ipsec_sa_is_set_IS_ASYNC(sa)) {
        if (!ipsec_sa_is_set_IS_AEAD(sa)) vnet_crypto_key_del(vm, sa->crypto_async_key_index);
    }

    if (ipsec_sa_is_set_UDP_ENCAP(sa) && ipsec_sa_is_set_IS_INBOUND(sa))
        ipsec_unregister_udp_port(clib_net_to_host_u16(sa->udp_hdr.dst_port), !ipsec_sa_is_set_IS_TUNNEL_V6(sa));

    if (ipsec_sa_is_set_IS_TUNNEL(sa) && !ipsec_sa_is_set_IS_INBOUND(sa)) dpo_reset(&sa->dpo);
    vnet_crypto_key_del(vm, sa->crypto_sync_key_index);
    if (sa->integ_alg != IPSEC_INTEG_ALG_NONE) vnet_crypto_key_del(vm, sa->integ_sync_key_index);
    if (ipsec_sa_is_set_ANTI_REPLAY_HUGE(sa)) clib_bitmap_free(sa->replay_window_huge);
    pool_put(ipsec_sa_pool, sa);
}

int ipsec_sa_bind(u32 id, u32 worker, bool bind)
{
    ipsec_main_t *im = &ipsec_main;
    uword        *p;
    ipsec_sa_t   *sa;
    ...
    if (worker >= vlib_num_workers()) return VNET_API_ERROR_INVALID_WORKER;
...
void ipsec_sa_walk(ipsec_sa_walk_cb_t cb, void *ctx)
{
    ...
    pool_foreach (sa, ipsec_sa_pool) {
        if (WALK_CONTINUE != cb(sa, ctx)) break;
    }
}
...
static ipsec_sa_t *ipsec_sa_from_fib_node(fib_node_t *node)
{
    ...
    return ((ipsec_sa_t *) (((char *) node) - STRUCT_OFFSET_OF(ipsec_sa_t, node)));
}
...
clib_error_t *ipsec_sa_init(vlib_main_t *vm)
{
    fib_node_register_type(FIB_NODE_TYPE_IPSEC_SA, &ipsec_sa_vft);

#define _(index, val, err, desc)                                                                            \
    ipsec_sa_err_counters[index].name              = (char *) format(0, "SA-" #err "%c", 0);                \
    ipsec_sa_err_counters[index].stat_segment_name = (char *) format(0, "/net/ipsec/sa/err/" #err "%c", 0); \
