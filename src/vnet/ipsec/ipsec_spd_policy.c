...
{
    if (is_outbound) {
        *type = (is_ipv6 ? IPSEC_SPD_POLICY_IP6_OUTBOUND : IPSEC_SPD_POLICY_IP4_OUTBOUND);
        return (0);
    }
    else {
        switch (action) {
            case IPSEC_POLICY_ACTION_PROTECT:
                *type = (is_ipv6 ? IPSEC_SPD_POLICY_IP6_INBOUND_PROTECT : IPSEC_SPD_POLICY_IP4_INBOUND_PROTECT);
                return (0);
            case IPSEC_POLICY_ACTION_BYPASS:
                *type = (is_ipv6 ? IPSEC_SPD_POLICY_IP6_INBOUND_BYPASS : IPSEC_SPD_POLICY_IP4_INBOUND_BYPASS);
                return (0);
            case IPSEC_POLICY_ACTION_DISCARD:
                *type = (is_ipv6 ? IPSEC_SPD_POLICY_IP6_INBOUND_DISCARD : IPSEC_SPD_POLICY_IP4_INBOUND_DISCARD);
                return (0);
...
static_always_inline int ipsec_is_policy_inbound(ipsec_policy_t *policy)
{
    if (policy->type == IPSEC_SPD_POLICY_IP4_INBOUND_PROTECT || policy->type == IPSEC_SPD_POLICY_IP4_INBOUND_BYPASS
...
static_always_inline int ipsec_is_fp_enabled(ipsec_main_t *im, ipsec_spd_t *spd, ipsec_policy_t *policy)
{
    if ((im->fp_spd_ipv4_out_is_enabled && PREDICT_TRUE(INDEX_INVALID != spd->fp_spd.ip4_out_lookup_hash_idx)
         && policy->type == IPSEC_SPD_POLICY_IP4_OUTBOUND)
        || (im->fp_spd_ipv4_in_is_enabled && PREDICT_TRUE(INDEX_INVALID != spd->fp_spd.ip4_in_lookup_hash_idx)
            && (policy->type == IPSEC_SPD_POLICY_IP4_INBOUND_PROTECT || policy->type == IPSEC_SPD_POLICY_IP4_INBOUND_BYPASS
                || policy->type == IPSEC_SPD_POLICY_IP4_INBOUND_DISCARD))
        || (im->fp_spd_ipv6_in_is_enabled && PREDICT_TRUE(INDEX_INVALID != spd->fp_spd.ip6_in_lookup_hash_idx)
            && (policy->type == IPSEC_SPD_POLICY_IP6_INBOUND_PROTECT || policy->type == IPSEC_SPD_POLICY_IP6_INBOUND_BYPASS
                || policy->type == IPSEC_SPD_POLICY_IP6_INBOUND_DISCARD))
        || (im->fp_spd_ipv6_out_is_enabled && PREDICT_TRUE(INDEX_INVALID != spd->fp_spd.ip6_out_lookup_hash_idx)
...
int ipsec_add_del_policy(vlib_main_t *vm, ipsec_policy_t *policy, int is_add, u32 *stat_index)
{
    ...
    if (!p) return VNET_API_ERROR_SYSCALL_ERROR_1;

    spd_index = p[0];
    spd       = pool_elt_at_index(im->spds, spd_index);
    if (!spd) return VNET_API_ERROR_SYSCALL_ERROR_1;

    if (im->output_flow_cache_flag && !policy->is_ipv6 && policy->type == IPSEC_SPD_POLICY_IP4_OUTBOUND) {
        /*
        ...
        if (im->epoch_count == 0xFFFFFFFF) {
            ...
            clib_memset_u8(im->ipsec4_out_spd_hash_tbl, 0, im->ipsec4_out_spd_hash_num_buckets * (sizeof(*(im->ipsec4_out_spd_hash_tbl))));
        }
    ...
    if ((policy->type == IPSEC_SPD_POLICY_IP4_INBOUND_PROTECT || policy->type == IPSEC_SPD_POLICY_IP4_INBOUND_BYPASS
        ...
        if (im->input_epoch_count == 0xFFFFFFFF) {
            /* Reset all the entries in flow cache */
            clib_memset_u8(im->ipsec4_in_spd_hash_tbl, 0, im->ipsec4_in_spd_hash_num_buckets * (sizeof(*(im->ipsec4_in_spd_hash_tbl))));
        }
        /* Increment epoch counter by 1 */
    ...
    if (is_add) {
        ...
        if (policy->policy == IPSEC_POLICY_ACTION_PROTECT) {
            ...
            if (INDEX_INVALID == sa_index) return VNET_API_ERROR_SYSCALL_ERROR_1;
         ...
         **/
        if (ipsec_is_fp_enabled(im, spd, policy)) return ipsec_fp_add_del_policy((void *) &spd->fp_spd, policy, 1, stat_index);
        ...
        vec_foreach_index (i, spd->policies[policy->type]) {
            ipsec_policy_t *p = pool_elt_at_index(im->policies, spd->policies[policy->type][i]);
        ...
        *stat_index = policy_index;
...

        {
            ...
            return ipsec_fp_add_del_policy((void *) &spd->fp_spd, policy, 0, stat_index);
        }

        vec_foreach_index (ii, (spd->policies[policy->type])) {
            vp = pool_elt_at_index(im->policies, spd->policies[policy->type][ii]);
...
static_always_inline void ipsec_fp_release_mask_type(ipsec_main_t *im, u32 mask_type_index)
{
    ipsec_fp_mask_type_entry_t *mte = pool_elt_at_index(im->fp_mask_types, mask_type_index);
    mte->refcount--;
    if (mte->refcount == 0) {
...
static_always_inline u32 find_mask_type_index(ipsec_main_t *im, ipsec_fp_5tuple_t *mask)
{
    ipsec_fp_mask_type_entry_t *mte;

    pool_foreach (mte, im->fp_mask_types) {
        if (memcmp(&mte->mask, mask, sizeof(*mask)) == 0) return (mte - im->fp_mask_types);
    }

    return ~0;
}

static_always_inline void fill_ip6_hash_policy_kv(ipsec_fp_5tuple_t *match, ipsec_fp_5tuple_t *mask, clib_bihash_kv_40_8_t *kv)
{
...
static_always_inline void fill_ip4_hash_policy_kv(ipsec_fp_5tuple_t *match, ipsec_fp_5tuple_t *mask, clib_bihash_kv_16_8_t *kv)
{
    ipsec_fp_lookup_value_t *kv_val = (ipsec_fp_lookup_value_t *) &kv->value;
...
static_always_inline u16 mask_out_highest_set_bit_u16(u16 x)
{
    x |= x >> 8;
    x |= x >> 4;
    x |= x >> 2;
...
static_always_inline u32 mask_out_highest_set_bit_u32(u32 x)
{
    x |= x >> 16;
    x |= x >> 8;
    x |= x >> 4;
...
static_always_inline u64 mask_out_highest_set_bit_u64(u64 x)
{
    x |= x >> 32;
    x |= x >> 16;
    x |= x >> 8;
...
static_always_inline void ipsec_fp_get_policy_ports_mask(ipsec_policy_t *policy, ipsec_fp_5tuple_t *mask)
{
    if (PREDICT_TRUE((policy->protocol == IP_PROTOCOL_TCP) || (policy->protocol == IP_PROTOCOL_UDP) || (policy->protocol == IP_PROTOCOL_SCTP))) {
...
static_always_inline void ipsec_fp_ip4_get_policy_mask(ipsec_policy_t *policy, ipsec_fp_5tuple_t *mask, bool inbound)
{
    ...
    if (inbound && (policy->type == IPSEC_SPD_POLICY_IP4_INBOUND_PROTECT && policy->sa_index != INDEX_INVALID)) {
        ipsec_sa_t *s = ipsec_sa_get(policy->sa_index);

        if (ipsec_sa_is_set_IS_TUNNEL(s)) goto set_spi_mask;
    }
    ...
    *prmask = *praddr_start ^ *praddr_stop;
     ...
     * order.
     */
    *plmask = clib_host_to_net_u32(mask_out_highest_set_bit_u32(clib_net_to_host_u32(*plmask)));

    *prmask = clib_host_to_net_u32(mask_out_highest_set_bit_u32(clib_net_to_host_u32(*prmask)));

set_spi_mask:
    if (inbound) {
        if (policy->type != IPSEC_SPD_POLICY_IP4_INBOUND_PROTECT) mask->spi = 0;
...
static_always_inline void ipsec_fp_ip6_get_policy_mask(ipsec_policy_t *policy, ipsec_fp_5tuple_t *mask, bool inbound)
{
    ...
    if (inbound && (policy->type == IPSEC_SPD_POLICY_IP6_INBOUND_PROTECT && policy->sa_index != INDEX_INVALID)) {
        ipsec_sa_t *s = ipsec_sa_get(policy->sa_index);
    ...
    *prmask = (*praddr_start++ ^ *praddr_stop++);
     ...
     * order.
     */
    *plmask = clib_host_to_net_u64(mask_out_highest_set_bit_u64(clib_net_to_host_u64(*plmask)));
    ...
    *prmask = clib_host_to_net_u64(mask_out_highest_set_bit_u64(clib_net_to_host_u64(*prmask)));
...
set_spi_mask:
    if (inbound) {
        if (policy->type != IPSEC_SPD_POLICY_IP6_INBOUND_PROTECT) mask->spi = 0;
...
static_always_inline void ipsec_fp_get_policy_5tuple(ipsec_policy_t *policy, ipsec_fp_5tuple_t *tuple, bool inbound)
{
    ...
    if (inbound) {
        if ((policy->type == IPSEC_SPD_POLICY_IP4_INBOUND_PROTECT || policy->type == IPSEC_SPD_POLICY_IP6_INBOUND_PROTECT)
            && policy->sa_index != INDEX_INVALID) {
            ipsec_sa_t *s = ipsec_sa_get(policy->sa_index);
...
static_always_inline int ipsec_fp_mask_type_idx_cmp(ipsec_fp_mask_id_t *mask_id, u32 *idx)
{
    return mask_id->mask_type_idx == *idx;
}

int ipsec_fp_ip4_add_policy(ipsec_main_t *im, ipsec_spd_fp_t *fp_spd, ipsec_policy_t *policy, u32 *stat_index)
{
    ...
    ipsec_fp_lookup_value_t    *result_val = (ipsec_fp_lookup_value_t *) &result.value;
    ...
    clib_bihash_16_8_t *bihash_table = inbound ? pool_elt_at_index(im->fp_ip4_lookup_hashes_pool, fp_spd->ip4_in_lookup_hash_idx)
                                               : pool_elt_at_index(im->fp_ip4_lookup_hashes_pool, fp_spd->ip4_out_lookup_hash_idx);
    ...
    *stat_index = policy_index;
    ...
    if (res != 0) {
        ...
        vec_foreach_index (i, result_val->fp_policies_ids) {
            ipsec_policy_t *p = pool_elt_at_index(im->policies, result_val->fp_policies_ids[i]);
    ...
    searched_idx = vec_search_with_function(fp_spd->fp_mask_ids[policy->type], &mask_index, ipsec_fp_mask_type_idx_cmp);
    if (~0 == searched_idx) {
...
int ipsec_fp_ip6_add_policy(ipsec_main_t *im, ipsec_spd_fp_t *fp_spd, ipsec_policy_t *policy, u32 *stat_index)
{
    u32                         mask_index, searched_idx;
    ipsec_policy_t             *vp;
    ipsec_fp_mask_type_entry_t *mte;
    ...
    *stat_index                      = policy_index;
    mask_index                       = find_mask_type_index(im, &mask);
    clib_bihash_40_8_t *bihash_table = inbound ? pool_elt_at_index(im->fp_ip6_lookup_hashes_pool, fp_spd->ip6_in_lookup_hash_idx)
                                               : pool_elt_at_index(im->fp_ip6_lookup_hashes_pool, fp_spd->ip6_out_lookup_hash_idx);
...
int ipsec_fp_ip6_del_policy(ipsec_main_t *im, ipsec_spd_fp_t *fp_spd, ipsec_policy_t *policy)
{
    int                      res;
    ...
    vec_foreach_index (ii, result_val->fp_policies_ids) {
        vp = pool_elt_at_index(im->policies, *(result_val->fp_policies_ids + ii));
        if (ipsec_policy_is_equal(vp, policy)) {
            ...
            vec_foreach_index (imt, fp_spd->fp_mask_ids[policy->type]) {
                if ((fp_spd->fp_mask_ids[policy->type] + imt)->mask_type_idx == vp->fp_mask_type_id) {
                    if ((fp_spd->fp_mask_ids[policy->type] + imt)->refcount-- == 1) vec_del1(fp_spd->fp_mask_ids[policy->type], imt);
...
int ipsec_fp_ip4_del_policy(ipsec_main_t *im, ipsec_spd_fp_t *fp_spd, ipsec_policy_t *policy)
{
    int                      res;
    ipsec_fp_5tuple_t        mask = {0}, policy_5tuple;
    clib_bihash_kv_16_8_t    kv;
...
int ipsec_fp_add_del_policy(void *fp_spd, ipsec_policy_t *policy, int is_add, u32 *stat_index)
{
    ipsec_main_t *im = &ipsec_main;

    if (is_add)
        if (policy->is_ipv6)
            return ipsec_fp_ip6_add_policy(im, (ipsec_spd_fp_t *) fp_spd, policy, stat_index);
        else
            return ipsec_fp_ip4_add_policy(im, (ipsec_spd_fp_t *) fp_spd, policy, stat_index);
