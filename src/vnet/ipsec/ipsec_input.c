...
static u8 *format_ipsec_input_trace(u8 *s, va_list *args)
{
    ...
    s = format(s,
               "%U: sa_id %u type: %u spd %u policy %d spi %u (0x%08x) seq %u",
               format_ip_protocol,
               t->proto,
               t->sa_id,
               t->policy_type,
...
always_inline void ipsec4_input_spd_add_flow_cache_entry(ipsec_main_t *im, u32 sa, u32 da, ipsec_spd_policy_type_t policy_type, u32 pol_id)
{
    u64 hash;
    u8  is_overwrite = 0, is_stale_overwrite = 0;
    /* Store in network byte order to avoid conversion on lookup */
    ipsec4_inbound_spd_tuple_t ip4_tuple = {.ip4_src_addr  = (ip4_address_t) clib_host_to_net_u32(sa),
                                            .ip4_dest_addr = (ip4_address_t) clib_host_to_net_u32(da),
                                            .policy_type   = policy_type};

    ip4_tuple.kv_16_8.value = (((u64) pol_id) << 32) | ((u64) im->input_epoch_count);
    ...
    if (PREDICT_FALSE(is_overwrite)) is_stale_overwrite = (im->input_epoch_count != ((u32) (im->ipsec4_in_spd_hash_tbl[hash].value & 0xFFFFFFFF)));
    clib_memcpy_fast(&im->ipsec4_in_spd_hash_tbl[hash], &ip4_tuple.kv_16_8, sizeof(ip4_tuple.kv_16_8));
    ...
    if (!is_overwrite || is_stale_overwrite) clib_atomic_fetch_add_relax(&im->ipsec4_in_spd_flow_cache_entries, 1);

    return;
}

always_inline ipsec_policy_t *ipsec4_input_spd_find_flow_cache_entry(ipsec_main_t *im, u32 sa, u32 da, ipsec_spd_policy_type_t policy_type)
{
    ipsec_policy_t            *p = NULL;
    ipsec4_hash_kv_16_8_t      kv_result;
    u64                        hash;
    ipsec4_inbound_spd_tuple_t ip4_tuple = {.ip4_src_addr = (ip4_address_t) sa, .ip4_dest_addr = (ip4_address_t) da, .policy_type = policy_type};
    ...
    if (ipsec4_hash_key_compare_16_8((u64 *) &ip4_tuple.kv_16_8, (u64 *) &kv_result)) {
        if (im->input_epoch_count == ((u32) (kv_result.value & 0xFFFFFFFF))) {
            ...
            p = pool_elt_at_index(im->policies, ((u32) (kv_result.value >> 32)));
        }
    }
...
always_inline void ipsec_fp_in_5tuple_from_ip4_range(ipsec_fp_5tuple_t *tuple, u32 sa, u32 da, u32 spi, u8 action)
{
...
always_inline void ipsec_fp_in_5tuple_from_ip6_range(ipsec_fp_5tuple_t *tuple, ip6_address_t *sa, ip6_address_t *da, u32 spi, u8 action)
...
always_inline ipsec_policy_t *ipsec_input_policy_match(ipsec_spd_t *spd, u32 sa, u32 da, ipsec_spd_policy_type_t policy_type)
{
    ...
    vec_foreach (i, spd->policies[policy_type]) {
        ...
        if (da < clib_net_to_host_u32(p->laddr.start.ip4.as_u32)) continue;

        if (da > clib_net_to_host_u32(p->laddr.stop.ip4.as_u32)) continue;

        if (sa < clib_net_to_host_u32(p->raddr.start.ip4.as_u32)) continue;

        if (sa > clib_net_to_host_u32(p->raddr.stop.ip4.as_u32)) continue;
...
always_inline ipsec_policy_t *ipsec_input_protect_policy_match(ipsec_spd_t *spd, u32 sa, u32 da, u32 spi)
{
    ...
    vec_foreach (i, spd->policies[IPSEC_SPD_POLICY_IP4_INBOUND_PROTECT]) {
        ...
        if (spi != s->spi) continue;

        if (ipsec_sa_is_set_IS_TUNNEL(s)) {
            if (da != clib_net_to_host_u32(s->tunnel.t_dst.ip.ip4.as_u32)) continue;

            if (sa != clib_net_to_host_u32(s->tunnel.t_src.ip.ip4.as_u32)) continue;
    ...
    return_policy:
        if (im->input_flow_cache_flag) {
            /* Add an Entry in Flow cache */
            ipsec4_input_spd_add_flow_cache_entry(im, sa, da, IPSEC_SPD_POLICY_IP4_INBOUND_PROTECT, *i);
        }
...
{
    if ((memcmp(a->as_u64, la->as_u64, 2 * sizeof(u64)) >= 0) && (memcmp(a->as_u64, ua->as_u64, 2 * sizeof(u64)) <= 0)) return 1;
    return 0;
}

always_inline void ipsec_collect_ah_trace(vlib_buffer_t      **b,
                                          ...
                                          u32                  pi0)
{
    if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE) && PREDICT_FALSE(b[0]->flags & VLIB_BUFFER_IS_TRACED)) {
...
always_inline void ipsec_ah_packet_process(vlib_main_t         *vm,
                                           ipsec_main_t        *im,
                                           ip4_header_t        *ip0,
                                           ah_header_t         *ah0,
                                           u32                  thread_index,
...

{
    ...
    while (1) {
        if (search_flow_cache) {
            p0 = ipsec4_input_spd_find_flow_cache_entry(im, ip0->src_address.as_u32, ip0->dst_address.as_u32, IPSEC_SPD_POLICY_IP4_INBOUND_PROTECT);
        }
        else {
        ...
        if (PREDICT_TRUE((p0 != NULL) & (has_space0))) {
            *ipsec_matched += 1;
            pi0 = p0 - im->policies;
            vlib_increment_combined_counter(&ipsec_spd_policy_counters, thread_index, pi0, 1, clib_net_to_host_u16(ip0->length));
        ...
        if (search_flow_cache) {
            p0 = ipsec4_input_spd_find_flow_cache_entry(im, ip0->src_address.as_u32, ip0->dst_address.as_u32, IPSEC_SPD_POLICY_IP4_INBOUND_BYPASS);
        }
        ...
        if (search_flow_cache) {
            p0 = ipsec4_input_spd_find_flow_cache_entry(im, ip0->src_address.as_u32, ip0->dst_address.as_u32, IPSEC_SPD_POLICY_IP4_INBOUND_DISCARD);
        }
...
always_inline void ipsec_esp_packet_process(vlib_main_t         *vm,
                                            ipsec_main_t        *im,
                                            ip4_header_t        *ip0,
                                            esp_header_t        *esp0,
                                            u32                  thread_index,
...
udp_or_esp:
    ...
    if (im->fp_spd_ipv4_in_is_enabled && PREDICT_TRUE(INDEX_INVALID != spd0->fp_spd.ip4_in_lookup_hash_idx)) {
        ipsec_fp_in_5tuple_from_ip4_range(&tuples[0],
...
always_inline ipsec_policy_t *ipsec6_input_protect_policy_match(ipsec_spd_t *spd, ip6_address_t *sa, ip6_address_t *da, u32 spi)
{
    ipsec_main_t   *im = &ipsec_main;
    ipsec_policy_t *p;
    ipsec_sa_t     *s;
    ...
    vec_foreach (i, spd->policies[IPSEC_SPD_POLICY_IP6_INBOUND_PROTECT]) {
        ...
        if (ipsec_sa_is_set_IS_TUNNEL(s)) {
            if (!ip6_address_is_equal(sa, &s->tunnel.t_src.ip.ip6)) continue;

            if (!ip6_address_is_equal(da, &s->tunnel.t_dst.ip.ip6)) continue;

            return p;
        }

        if (!ip6_addr_match_range(sa, &p->raddr.start.ip6, &p->raddr.stop.ip6)) continue;

        if (!ip6_addr_match_range(da, &p->laddr.start.ip6, &p->laddr.stop.ip6)) continue;
...
VLIB_NODE_FN(ipsec4_input_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
        ...
        if (ip0->protocol == IP_PROTOCOL_UDP) {
            ...
            if (clib_host_to_net_u16(4500) == udp0->dst_port) {
                ...
                ipsec_esp_packet_process(vm,
                                         ...
                                         &ipsec_unprocessed,
                                         next);
                if (ipsec_bypassed > 0) goto ipsec_bypassed;
            }
        }
    ...
    vlib_node_increment_counter(vm, ipsec4_input_node.index, IPSEC_INPUT_ERROR_RX_POLICY_MATCH, ipsec_matched);

    vlib_node_increment_counter(vm, ipsec4_input_node.index, IPSEC_INPUT_ERROR_RX_POLICY_NO_MATCH, ipsec_unprocessed);

    vlib_node_increment_counter(vm, ipsec4_input_node.index, IPSEC_INPUT_ERROR_RX_POLICY_DISCARD, ipsec_dropped);

    vlib_node_increment_counter(vm, ipsec4_input_node.index, IPSEC_INPUT_ERROR_RX_POLICY_BYPASS, ipsec_bypassed);
...
VLIB_NODE_FN(ipsec6_input_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
		...
		("packet received from %U to %U spi %u size %u spd_id %u",
		 ...
		 spd0->id);
#endif
                if (im->fp_spd_ipv6_in_is_enabled && PREDICT_TRUE(INDEX_INVALID != spd0->fp_spd.ip6_in_lookup_hash_idx)) {
                    ipsec_fp_in_5tuple_from_ip6_range(&tuples[0],
                                                      &ip0->src_address,
                                                      &ip0->dst_address,
                                                      clib_net_to_host_u32(esp0->spi),
                    ...
                    p0 = ipsec6_input_protect_policy_match(spd0, &ip0->src_address, &ip0->dst_address, clib_net_to_host_u32(esp0->spi));
                ...
                p0 = ipsec6_input_protect_policy_match(spd0, &ip0->src_address, &ip0->dst_address, clib_net_to_host_u32(ah0->spi));
    ...
    vlib_node_increment_counter(vm, ipsec6_input_node.index, IPSEC_INPUT_ERROR_RX_PKTS, from_frame->n_vectors - ipsec_unprocessed);

    vlib_node_increment_counter(vm, ipsec6_input_node.index, IPSEC_INPUT_ERROR_RX_POLICY_MATCH, ipsec_matched);
