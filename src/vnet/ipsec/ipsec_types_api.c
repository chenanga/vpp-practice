...
ipsec_sa_flags_t ipsec_sa_flags_decode(vl_api_ipsec_sad_flags_t in)
{
    ...
    if (in & IPSEC_API_SAD_FLAG_USE_ESN) flags |= IPSEC_SA_FLAG_USE_ESN;
    if (in & IPSEC_API_SAD_FLAG_USE_ANTI_REPLAY) flags |= IPSEC_SA_FLAG_USE_ANTI_REPLAY;
    if (in & IPSEC_API_SAD_FLAG_IS_TUNNEL) flags |= IPSEC_SA_FLAG_IS_TUNNEL;
    if (in & IPSEC_API_SAD_FLAG_IS_TUNNEL_V6) flags |= IPSEC_SA_FLAG_IS_TUNNEL_V6;
    if (in & IPSEC_API_SAD_FLAG_UDP_ENCAP) flags |= IPSEC_SA_FLAG_UDP_ENCAP;
    if (in & IPSEC_API_SAD_FLAG_IS_INBOUND) flags |= IPSEC_SA_FLAG_IS_INBOUND;
    if (in & IPSEC_API_SAD_FLAG_ASYNC) flags |= IPSEC_SA_FLAG_IS_ASYNC;
...
vl_api_ipsec_sad_flags_t ipsec_sad_flags_encode(const ipsec_sa_t* sa)
{
    ...
    if (ipsec_sa_is_set_USE_ESN(sa)) flags |= IPSEC_API_SAD_FLAG_USE_ESN;
    ...
    if (ipsec_sa_is_set_IS_TUNNEL_V6(sa)) flags |= IPSEC_API_SAD_FLAG_IS_TUNNEL_V6;
    if (ipsec_sa_is_set_UDP_ENCAP(sa)) flags |= IPSEC_API_SAD_FLAG_UDP_ENCAP;
    if (ipsec_sa_is_set_IS_INBOUND(sa)) flags |= IPSEC_API_SAD_FLAG_IS_INBOUND;
    if (ipsec_sa_is_set_IS_ASYNC(sa)) flags |= IPSEC_API_SAD_FLAG_ASYNC;
