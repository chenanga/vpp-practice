...
u8 ip_is_local_host(ip46_address_t *ip46_address, u8 is_ip4)
{
    if (is_ip4)
        ...
        return (ip46_address->as_u64[0] == 0 && clib_net_to_host_u64(ip46_address->as_u64[1]) == 1);
}
...
{
    return (ip6_address->as_u64[0] == 0 && clib_net_to_host_u64(ip6_address->as_u64[1]) == 1);
}
                               ...
                               u32                     n_feature_config_bytes)
{
    if (IP_FEATURE_INPUT == loc) {
        if (N_SAFI == safi) FOR_EACH_IP_ADDRESS_SUB_FAMILY(safi)
        vnet_feature_enable_disable(ip_arc_names[loc][af][safi], feature_name, sw_if_index, enable, feature_config, n_feature_config_bytes);
        else vnet_feature_enable_disable(ip_arc_names[loc][af][safi], feature_name, sw_if_index, enable, feature_config, n_feature_config_bytes);
    }
    else
        vnet_feature_enable_disable(ip_arc_names[loc][af][SAFI_UNICAST], feature_name, sw_if_index, enable, feature_config, n_feature_config_bytes);
}
...
uword unformat_ip_address_family(unformat_input_t *input, va_list *args)
{
    ...
    if (unformat(input, "ip4") || unformat(input, "ipv4") || unformat(input, "IP4") || unformat(input, "IPv4")) {
        *af = AF_IP4;
        return (1);
    }
    else if (unformat(input, "ip6") || unformat(input, "ipv6") || unformat(input, "IP6") || unformat(input, "IPv6")) {
        *af = AF_IP6;
        return (1);
    }
    return (0);
