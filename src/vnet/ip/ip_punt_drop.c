...
static void ip_punt_redirect_stack(ip_punt_redirect_rx_t *ipr)
{
    ...
    fib_path_list_contribute_forwarding(ipr->pl, ipr->payload_type, FIB_PATH_LIST_FWD_FLAG_COLLAPSE, &dpo);

    if (FIB_PROTOCOL_IP4 == ipr->fproto)
        pnode = vlib_get_node_by_name(vlib_get_main(), (u8 *) "ip4-punt-redirect");
    else
        pnode = vlib_get_node_by_name(vlib_get_main(), (u8 *) "ip6-punt-redirect");
...
index_t ip_punt_redirect_find(fib_protocol_t fproto, u32 rx_sw_if_index)
{
    ...
    if (vec_len(rxs) <= rx_sw_if_index) return (INDEX_INVALID);
...
void ip_punt_redirect_add(fib_protocol_t fproto, u32 rx_sw_if_index, fib_forward_chain_type_t ct, const fib_route_path_t *rpaths)
{
    ...
    if (~0 == rx_sw_if_index) rx_sw_if_index = 0;

    vec_validate_init_empty(ip_punt_redirect_cfg.redirect_by_rx_sw_if_index[fproto], rx_sw_if_index, INDEX_INVALID);

    pool_get(ip_punt_redirect_cfg.pool, ipr);
    ipri = ipr - ip_punt_redirect_cfg.pool;

    ip_punt_redirect_cfg.redirect_by_rx_sw_if_index[fproto][rx_sw_if_index] = ipri;
    ...
    ipr->sibling = fib_path_list_child_add(ipr->pl, FIB_NODE_TYPE_IP_PUNT_REDIRECT, ipri);
...
void ip_punt_redirect_del(fib_protocol_t fproto, u32 rx_sw_if_index)
{
    ...
    if ((vec_len(rxs) <= rx_sw_if_index) || (INDEX_INVALID == rxs[rx_sw_if_index])) return;
...
u8 *format_ip_punt_redirect(u8 *s, va_list *args)
{
    ...
    vec_foreach_index (rx_sw_if_index, rxs) {
        if (INDEX_INVALID == rxs[rx_sw_if_index]) continue;

        rx = ip_punt_redirect_get(rxs[rx_sw_if_index]);

        s = format(s, " rx %U via:\n", format_vnet_sw_if_index_name, vnm, rx_sw_if_index);
...
void ip_punt_redirect_walk(fib_protocol_t fproto, ip_punt_redirect_walk_cb_t cb, void *ctx)
{
    ...
    vec_foreach_index (ii, rxs) {
        if (INDEX_INVALID == rxs[ii]) continue;
...
{
    return ((ip_punt_redirect_rx_t *) (((char *) node) - STRUCT_OFFSET_OF(ip_punt_redirect_rx_t, node)));
}
...
{
    fib_node_register_type(FIB_NODE_TYPE_IP_PUNT_REDIRECT, &ip_punt_redirect_vft);
