...
#define _(pos, len, value, name, str) VNET_PUNT_REASON_F_##name = ((value) << (pos)),
    foreach_vnet_punt_reason_flag
#undef _
} __clib_packed vnet_punt_reason_flag_t;

enum vnet_punt_reason_flag_mask_t_ {
#define _(pos, len, value, name, str) VNET_PUNT_REASON_F_MASK_##name = (((1 << (len)) - 1) << (pos)),
    foreach_vnet_punt_reason_flag
#undef _
};
...
#define _(pos, len, value, name, str)                                                   \
    static_always_inline int vnet_punt_reason_flag_is_##name(vnet_punt_reason_flag_t f) \
    {                                                                                   \
...
extern void punt_client_walk(punt_type_t pt, punt_client_walk_cb_t cb, void *ctx);
...
static_always_inline punt_client_t *punt_client_l4_get(ip_address_family_t af, u16 port)
{
    ...
    if (p) return (pool_elt_at_index(pm->punt_client_pool, p[0]));
...
static_always_inline punt_client_t *punt_client_ip_proto_get(ip_address_family_t af, ip_protocol_t proto)
{
    ...
    p = hash_get(pm->db.clients_by_ip_proto, punt_client_ip_proto_mk_key(af, proto));
...
static_always_inline punt_client_t *punt_client_exception_get(vlib_punt_reason_t reason)
{
    ...
    if (reason >= vec_len(pm->db.clients_by_exception)) return (NULL);

    pci = pm->db.clients_by_exception[reason];

    if (~0 != pci) return (pool_elt_at_index(pm->punt_client_pool, pci));
