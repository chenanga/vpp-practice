...
always_inline vtep_table_t vtep_table_create()
{
    vtep_table_t t = {};
...
always_inline void vtep4_key_init(vtep4_key_t *k4)
{
    k4->as_u64 = ~((u64) 0);
}

always_inline void vtep6_key_init(vtep6_key_t *k6)
{
...
always_inline u8 vtep4_check(vtep_table_t *t, vlib_buffer_t *b0, ip4_header_t *ip40, vtep4_key_t *last_k4)
{
    vtep4_key_t k4;
    k4.addr.as_u32 = ip40->dst_address.as_u32;
    k4.fib_index   = vlib_buffer_get_ip4_fib_index(b0);
    if (PREDICT_TRUE(k4.as_u64 == last_k4->as_u64)) return VTEP_CHECK_PASS_UNCHANGED;
    if (PREDICT_FALSE(!hash_get(t->vtep4, k4.as_u64))) return VTEP_CHECK_FAIL;
...
always_inline u8 vtep4_check_vector(vtep_table_t *t, vlib_buffer_t *b0, ip4_header_t *ip40, vtep4_key_t *last_k4, vtep4_cache_t *vtep4_u512)
{
    vtep4_key_t k4;
    k4.addr.as_u32 = ip40->dst_address.as_u32;
    k4.fib_index   = vlib_buffer_get_ip4_fib_index(b0);
    ...
    if (PREDICT_TRUE(result != 0)) {
        last_k4->as_u64 = vtep4_u512->vtep4_cache[count_trailing_zeros(result)].as_u64;
...
always_inline u8 vtep6_check(vtep_table_t *t, vlib_buffer_t *b0, ip6_header_t *ip60, vtep6_key_t *last_k6)
{
    vtep6_key_t k6;
    k6.fib_index = vlib_buffer_get_ip6_fib_index(b0);
    if (PREDICT_TRUE(k6.fib_index == last_k6->fib_index && ip60->dst_address.as_u64[0] == last_k6->addr.as_u64[0]
                     && ip60->dst_address.as_u64[1] == last_k6->addr.as_u64[1])) {
        return VTEP_CHECK_PASS_UNCHANGED;
    }
    k6.addr = ip60->dst_address;
    if (PREDICT_FALSE(!hash_get_mem(t->vtep6, &k6))) return VTEP_CHECK_FAIL;
    *last_k6 = k6;
    return VTEP_CHECK_PASS;
}
