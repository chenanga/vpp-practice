...
{
    return policer_handoff(vm, node, frame, ip6_punt_policer_cfg.fq_index, ip6_punt_policer_cfg.policer_index);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (ip_punt_policer(vm, node, frame, vnet_feat_arc_ip6_punt.feature_arc_index, ip6_punt_policer_cfg.policer_index));
}
...
VNET_FEATURE_INIT(ip6_punt_policer_node,
                  static) = {.arc_name = "ip6-punt", .node_name = "ip6-punt-policer", .runs_before = VNET_FEATURES("ip6-punt-redirect")};

VLIB_NODE_FN(ip6_drop_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    if (node->flags & VLIB_NODE_FLAG_TRACE) ip6_forward_next_trace(vm, node, frame, VLIB_TX);

    return ip_drop_or_punt(vm, node, frame, vnet_feat_arc_ip6_drop.feature_arc_index);
}

VLIB_NODE_FN(ip6_not_enabled_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    if (node->flags & VLIB_NODE_FLAG_TRACE) ip6_forward_next_trace(vm, node, frame, VLIB_TX);
...
VLIB_NODE_FN(ip6_punt_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    if (node->flags & VLIB_NODE_FLAG_TRACE) ip6_forward_next_trace(vm, node, frame, VLIB_TX);

    return ip_drop_or_punt(vm, node, frame, vnet_feat_arc_ip6_punt.feature_arc_index);
}
...
static clib_error_t *ip6_punt_police_cmd(vlib_main_t *vm, unformat_input_t *main_input, vlib_cli_command_t *cmd)
{
    ...
    if (is_add && ~0 == policer_index) {
        error = clib_error_return(0, "expected policer index `%U'", format_unformat_error, line_input);
        goto done;
    }
    if (!is_add) policer_index = ~0;
...
VLIB_NODE_FN(ip6_punt_redirect_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (ip_punt_redirect(vm, node, frame, vnet_feat_arc_ip6_punt.feature_arc_index, FIB_PROTOCOL_IP6));
}
...
VNET_FEATURE_INIT(ip6_punt_redirect_node,
                  static) = {.arc_name = "ip6-punt", .node_name = "ip6-punt-redirect", .runs_before = VNET_FEATURES("error-punt")};
...
{
    ip_punt_redirect_add(FIB_PROTOCOL_IP6, rx_sw_if_index, FIB_FORW_CHAIN_TYPE_UNICAST_IP6, rpaths);

    if (1 == ++ip6_punt_redirect_enable_counts) vnet_feature_enable_disable("ip6-punt", "ip6-punt-redirect", 0, 1, 0, 0);
}

void ip6_punt_redirect_del(u32 rx_sw_if_index)
{
    ASSERT(ip6_punt_redirect_enable_counts);
    if (0 == --ip6_punt_redirect_enable_counts) vnet_feature_enable_disable("ip6-punt", "ip6-punt-redirect", 0, 0, 0, 0);
...
static clib_error_t *ip6_punt_redirect_cmd(vlib_main_t *vm, unformat_input_t *main_input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "del"))
            ...
            rx_sw_if_index = 0;
        else if (unformat(line_input, "rx %U", unformat_vnet_sw_interface, vnm, &rx_sw_if_index))
            ;
    ...
    if (is_add) {
        if (vec_len(rpaths)) ip6_punt_redirect_add_paths(rx_sw_if_index, rpaths);
    }
    else {
