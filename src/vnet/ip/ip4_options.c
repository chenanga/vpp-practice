    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (b->flags & VLIB_BUFFER_IS_TRACED) {
                ip4_options_trace_t *t = vlib_add_trace(vm, node, b, sizeof(*t));
...
u8 *format_ip4_options_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "%Uoption:[0x%x,0x%x,0x%x,0x%x]", format_white_space, indent, t->option[0], t->option[1], t->option[2], t->option[3]);
    return s;
}
