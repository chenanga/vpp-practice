...
static u8 icmp6_to_icmp_updater_pointer_table[] = {0,  1,  ~0, ~0, 2,  2,  9,  8,  12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12,
                                                   12, 12, 12, 12, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24};
...
ip6_parse(vlib_main_t *vm, vlib_buffer_t *b, ip6_header_t *ip6, u32 buff_len, u8 *l4_protocol, u16 *l4_offset, u16 *frag_hdr_offset)
{
    ip6_ext_hdr_chain_t hdr_chain;
    int                 res = ip6_ext_header_walk(b, ip6, IP_PROTOCOL_IPV6_FRAGMENTATION, &hdr_chain);
    if (res < 0) {
        return -1;
    }
 ...
 */
always_inline u16 ip6_get_port(vlib_main_t   *vm,
                               vlib_buffer_t *b,
                               ip6_header_t  *ip6,
                               u16            buffer_len,
                               u8            *ip_protocol,
                               ...
                               u32           *tcp_seq_number)
{
    ...
    if (frag_offset && ip6_frag_hdr_offset(((ip6_frag_hdr_t *) u8_ptr_add(ip6, frag_offset)))) return 0;  // Can't deal with non-first fragment for
                                                                                                          // now
    ...
    if (l4_protocol == IP_PROTOCOL_TCP || l4_protocol == IP_PROTOCOL_UDP) {
        if (src_port) *src_port = ((udp_header_t *) (l4))->src_port;
        if (dst_port) *dst_port = ((udp_header_t *) (l4))->dst_port;
        if (icmp_type_or_tcp_flags && l4_protocol == IP_PROTOCOL_TCP) *icmp_type_or_tcp_flags = ((tcp_header_t *) (l4))->flags;
        if (tcp_ack_number && l4_protocol == IP_PROTOCOL_TCP) *tcp_ack_number = ((tcp_header_t *) (l4))->ack_number;
        if (tcp_seq_number && l4_protocol == IP_PROTOCOL_TCP) *tcp_seq_number = ((tcp_header_t *) (l4))->seq_number;
    }
    else if (l4_protocol == IP_PROTOCOL_ICMP6) {
        icmp46_header_t *icmp = (icmp46_header_t *) (l4);
        if (icmp_type_or_tcp_flags) *icmp_type_or_tcp_flags = ((icmp46_header_t *) (l4))->type;
        if (icmp->type == ICMP6_echo_request) {
            if (src_port) *src_port = ((u16 *) (icmp))[2];
            if (dst_port) *dst_port = ((u16 *) (icmp))[2];
        }
            ...
            if (ip6_parse(vm, b, inner_ip6, ip6_pay_len - 8, &inner_l4_protocol, &inner_l4_offset, &inner_frag_offset)) return 0;

            if (inner_frag_offset && ip6_frag_hdr_offset(((ip6_frag_hdr_t *) u8_ptr_add(inner_ip6, inner_frag_offset)))) return 0;

            inner_l4 = u8_ptr_add(inner_ip6, inner_l4_offset);
            if (inner_l4_protocol == IP_PROTOCOL_TCP || inner_l4_protocol == IP_PROTOCOL_UDP) {
                if (src_port) *src_port = ((udp_header_t *) (inner_l4))->dst_port;
                if (dst_port) *dst_port = ((udp_header_t *) (inner_l4))->src_port;
            }
                ...
                if (inner_icmp->type == ICMP6_echo_request) {
                    if (src_port) *src_port = ((u16 *) (inner_icmp))[2];
                    if (dst_port) *dst_port = ((u16 *) (inner_icmp))[2];
                }
...
{
    return (clib_net_to_host_u32(ip_version_traffic_class_and_flow_label) & 0x0ff00000) >> 20;
}
 ...
 * @returns 0 on success, non-zero value otherwise.
 */
always_inline int icmp6_to_icmp(vlib_main_t             *vm,
                                vlib_buffer_t           *p,
                                ...
                                void                    *inner_ctx)
{
    ...
    if (ip6->protocol != IP_PROTOCOL_ICMP6) return -1;

    // There are no fragmented ICMP messages, so no extension header for now
    if (icmp6_to_icmp_header(icmp, &inner_ip6)) return -1;
    ...
    if (inner_ip6) {
        ...
        if (ip6_parse(vm, p, inner_ip6, ip6_pay_len - 8, &inner_protocol, &inner_l4_offset, &inner_frag_offset)) return -1;
        ...
        if (inner_protocol == IP_PROTOCOL_TCP) {
            ...
            inner_icmp->type  = (inner_icmp->type == ICMP6_echo_request) ? ICMP4_echo_request : ICMP4_echo_reply;
        ...
        if ((rv = inner_fn(inner_ip6, inner_ip4, inner_ctx)) != 0) return rv;

        inner_ip4->ip_version_and_header_length = IP4_VERSION_AND_HEADER_LENGTH_NO_OPTIONS;
        inner_ip4->tos                          = ip6_translate_tos(inner_ip6->ip_version_traffic_class_and_flow_label);
        inner_ip4->length                       = u16_net_add(inner_ip6->payload_length, sizeof(*ip4) + sizeof(*ip6) - inner_l4_offset);
        ...
        if (inner_ip4->protocol == IP_PROTOCOL_ICMP) {
            ...
            csum                        = ip_incremental_checksum(0, inner_icmp, clib_net_to_host_u16(inner_ip4->length) - sizeof(*inner_ip4));
    ...
    if ((rv = fn(ip6, ip4, ctx)) != 0) return rv;
    ...
    ip4->length   = u16_net_add(ip6->payload_length, (inner_ip6 == NULL) ? sizeof(*ip4) : (2 * sizeof(*ip4) - sizeof(*ip6)));
