    ...
    switch (t->action) {
        ...
        case REASS_FINISH:
            s = format(s,
                       ...
                       t->ip_proto,
                       clib_net_to_host_u16(t->l4_src_port),
...
always_inline void ip6_sv_reass_free(vlib_main_t *vm, ip6_sv_reass_main_t *rm, ip6_sv_reass_per_thread_t *rt, ip6_sv_reass_t *reass)
{
    clib_bihash_kv_48_8_t kv;
    ...
    vlib_buffer_free(vm, reass->cached_buffers, vec_len(reass->cached_buffers));
...
always_inline void ip6_sv_reass_init(ip6_sv_reass_t *reass)
{
    reass->cached_buffers = NULL;
    reass->is_complete    = false;
}

always_inline ip6_sv_reass_t *ip6_sv_reass_find_or_create(vlib_main_t               *vm,
...
ip6_sv_reass_update(vlib_main_t *vm, vlib_node_runtime_t *node, ip6_sv_reass_main_t *rm, ip6_sv_reass_t *reass, u32 bi0, ip6_frag_hdr_t *frag_hdr)
{
    ...
    if (0 == fragment_first) {
        ...
        if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
            ip6_sv_reass_add_trace(vm, node, reass, bi0, REASS_FINISH, reass->ip_proto, reass->l4_src_port, reass->l4_dst_port);
        }
    }
    vec_add1(reass->cached_buffers, bi0);
    if (!reass->is_complete) {
        if (PREDICT_FALSE(fb->flags & VLIB_BUFFER_IS_TRACED)) {
            ip6_sv_reass_add_trace(vm, node, reass, bi0, REASS_FRAGMENT_CACHE, reass->ip_proto, reass->l4_src_port, reass->l4_dst_port);
        }
...
always_inline bool ip6_sv_reass_verify_upper_layer_present(vlib_node_runtime_t *node, vlib_buffer_t *b, ip6_ext_hdr_chain_t *hc)
{
...
always_inline bool ip6_sv_reass_verify_fragment_multiple_8(vlib_main_t *vm, vlib_buffer_t *b, ip6_frag_hdr_t *frag_hdr)
{
...
always_inline bool ip6_sv_reass_verify_packet_size_lt_64k(vlib_main_t *vm, vlib_buffer_t *b, ip6_frag_hdr_t *frag_hdr)
{
    vnet_buffer_opaque_t *vnb             = vnet_buffer(b);
...
always_inline uword
ip6_sv_reassembly_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, bool is_feature, bool custom_next, bool custom_context)
{
    ...
    u32                       *context;
    if (custom_context) context = vlib_frame_aux_args(frame);
    ...
    while (n_left_from > 0) {
        if (custom_context)
            vlib_get_next_frame_with_aux_safe(vm, node, next_index, to_next, to_next_aux, n_left_to_next);
        else
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (res >= 0 && hdr_chain.eh[res].protocol == IP_PROTOCOL_IPV6_FRAGMENTATION) {
                ...
                is_atomic_fragment = (0 == ip6_frag_hdr_offset(frag_hdr) && !ip6_frag_hdr_more(frag_hdr));
            }

            if (res < 0 || hdr_chain.eh[res].protocol != IP_PROTOCOL_IPV6_FRAGMENTATION || is_atomic_fragment) {
                ...
                next0                                           = custom_next ? vnet_buffer(b0)->ip.reass.next_index : IP6_SV_REASSEMBLY_NEXT_INPUT;
                if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                    ip6_sv_reass_add_trace(vm,
                                           node,
                                           NULL,
                                           bi0,
                                           REASS_PASSTHROUGH,
            ...
            if (0 == ip6_frag_hdr_offset(frag_hdr)) {
                ...
                if (!ip6_sv_reass_verify_upper_layer_present(node, b0, &hdr_chain)) {
                    next0 = IP6_SV_REASSEMBLY_NEXT_ICMP_ERROR;
                    goto packet_enqueue;
                }
            }
            if (!ip6_sv_reass_verify_fragment_multiple_8(vm, b0, frag_hdr) || !ip6_sv_reass_verify_packet_size_lt_64k(vm, b0, frag_hdr)) {
                next0 = IP6_SV_REASSEMBLY_NEXT_ICMP_ERROR;
                goto packet_enqueue;
            }
            ...
            if (custom_context)
                kv.k.as_u64[4] = (u64) *context << 32 | (u64) frag_hdr->identification;
            else
                kv.k.as_u64[4] =
                    ((u64) vec_elt(ip6_main.fib_index_by_sw_if_index, vnet_buffer(b0)->sw_if_index[VLIB_RX])) << 32 | (u64) frag_hdr->identification;
            kv.k.as_u64[5] = ip0->protocol;

            ip6_sv_reass_t *reass = ip6_sv_reass_find_or_create(vm, rm, rt, &kv, &do_handoff);

            if (PREDICT_FALSE(do_handoff)) {
                next0                                        = IP6_SV_REASSEMBLY_NEXT_HANDOFF;
                vnet_buffer(b0)->ip.reass.owner_thread_index = kv.v.thread_index;
                if (custom_context) forward_context = 1;
                goto packet_enqueue;
            }
            ...
            if (reass->is_complete) {
                vnet_buffer(b0)->ip.reass.is_non_first_fragment  = !!ip6_frag_hdr_offset(frag_hdr);
                vnet_buffer(b0)->ip.reass.ip_proto               = reass->ip_proto;
                vnet_buffer(b0)->ip.reass.icmp_type_or_tcp_flags = reass->icmp_type_or_tcp_flags;
                vnet_buffer(b0)->ip.reass.tcp_ack_number         = reass->tcp_ack_number;
                vnet_buffer(b0)->ip.reass.tcp_seq_number         = reass->tcp_seq_number;
                vnet_buffer(b0)->ip.reass.l4_src_port            = reass->l4_src_port;
                vnet_buffer(b0)->ip.reass.l4_dst_port            = reass->l4_dst_port;
                next0                                            = custom_next ? vnet_buffer(b0)->ip.reass.next_index : IP6_SV_REASSEMBLY_NEXT_INPUT;
                if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                    ip6_sv_reass_add_trace(vm, node, reass, bi0, REASS_FRAGMENT_FORWARD, reass->ip_proto, reass->l4_src_port, reass->l4_dst_port);
                }
                goto packet_enqueue;
            }
            ...
            if (reass->is_complete) {
                ...
                vec_foreach_index (idx, reass->cached_buffers) {
                    ...
                    frag_hdr                                         = vlib_buffer_get_current(b0) + vnet_buffer(b0)->ip.reass.ip6_frag_hdr_offset;
                    vnet_buffer(b0)->ip.reass.is_non_first_fragment  = !!ip6_frag_hdr_offset(frag_hdr);
                    vnet_buffer(b0)->ip.reass.ip_proto               = reass->ip_proto;
                    vnet_buffer(b0)->ip.reass.icmp_type_or_tcp_flags = reass->icmp_type_or_tcp_flags;
                    vnet_buffer(b0)->ip.reass.tcp_ack_number         = reass->tcp_ack_number;
        ...
        packet_enqueue:
            ...
            if (custom_context && forward_context) {
                ...
                vlib_validate_buffer_enqueue_with_aux_x1(vm, node, next_index, to_next, to_next_aux, n_left_to_next, bi0, *context, next0);
            }
            else
                vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, bi0, next0);
...
{
    return ip6_sv_reassembly_inline(vm, node, frame, false /* is_feature */, false /* custom next */, false /* custom context */);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip6_sv_reassembly_inline(vm, node, frame, true /* is_feature */, false /* custom next */, false /* custom context */);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip6_sv_reassembly_inline(vm, node, frame, false /* is_feature */, true /* custom next */, true /* custom context */);
}
...
vnet_api_error_t ip6_sv_reass_set(u32 timeout_ms, u32 max_reassemblies, u32 max_reassembly_length, u32 expire_walk_interval_ms)
{
    ...
    ip6_sv_reass_set_params(timeout_ms, max_reassemblies, max_reassembly_length, expire_walk_interval_ms);
    vlib_process_signal_event(ip6_sv_reass_main.vlib_main, ip6_sv_reass_main.ip6_sv_reass_expire_node_idx, IP6_EVENT_CONFIG_CHANGED, 0);
    ...
    if (ip6_sv_reass_main.max_reass_n > 0 && new_nbuckets > old_nbuckets) {
        ...
        clib_bihash_init_48_8(&new_hash, "ip6-sv-reass", new_nbuckets, new_nbuckets * 1024);
        clib_bihash_foreach_key_value_pair_48_8(&ip6_sv_reass_main.hash, ip6_rehash_cb, &ctx);
        if (ctx.failure) {
            ...
            clib_memcpy_fast(&ip6_sv_reass_main.hash, &new_hash, sizeof(ip6_sv_reass_main.hash));
...
static clib_error_t *ip6_sv_reass_init_function(vlib_main_t *vm)
{
    ...
    clib_bihash_init_48_8(&rm->hash, "ip6-sv-reass", nbuckets, nbuckets * 1024);
    ...
    rm->fq_feature_index        = vlib_frame_queue_main_init(ip6_sv_reass_node_feature.index, 0);
    rm->fq_custom_context_index = vlib_frame_queue_main_init(ip6_sv_reass_custom_context_node.index, 0);
...
static u8 *format_ip6_sv_reass(u8 *s, va_list *args)
{
    ...
    s = format(s, "ID: %lu, key: %U, trace_op_counter: %u\n", reass->id, format_ip6_sv_reass_key, &reass->key, reass->trace_op_counter);
    vlib_buffer_t *b;
    u32           *bip;
    u32            counter = 0;
...
static clib_error_t *show_ip6_sv_reass(vlib_main_t *vm, unformat_input_t *input, CLIB_UNUSED(vlib_cli_command_t *lmd))
{
    ...
    vlib_cli_output(vm, "Maximum configured concurrent shallow virtual IP6 reassemblies per worker-thread: %lu\n", (long unsigned) rm->max_reass_n);
    vlib_cli_output(vm,
                    "Maximum configured amount of fragments per shallow "
                    "virtual IP6 reassembly: %lu\n",
                    (long unsigned) rm->max_reass_len);
    vlib_cli_output(vm, "Maximum configured shallow virtual IP6 reassembly timeout: %lums\n", (long unsigned) rm->timeout_ms);
    vlib_cli_output(vm,
...
static u8 *format_ip6_sv_reassembly_handoff_trace(u8 *s, va_list *args)
{
    CLIB_UNUSED(vlib_main_t * vm)        = va_arg(*args, vlib_main_t *);
    CLIB_UNUSED(vlib_node_t * node)      = va_arg(*args, vlib_node_t *);
    ip6_sv_reassembly_handoff_trace_t *t = va_arg(*args, ip6_sv_reassembly_handoff_trace_t *);

    s = format(s, "ip6-sv-reassembly-handoff: next-worker %d", t->next_worker_index);

    return s;
}

always_inline uword
ip6_sv_reassembly_handoff_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, bool is_feature, bool custom_context)
{
    ip6_sv_reass_main_t *rm = &ip6_sv_reass_main;
    ...
    fq_index = (is_feature) ? rm->fq_feature_index : (custom_context ? rm->fq_custom_context_index : rm->fq_index);
    ...
    while (n_left_from > 0) {
        ...
        if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b[0]->flags & VLIB_BUFFER_IS_TRACED))) {
            ip6_sv_reassembly_handoff_trace_t *t = vlib_add_trace(vm, node, b[0], sizeof(*t));
    ...
    if (custom_context)
        n_enq = vlib_buffer_enqueue_to_thread_with_aux(vm, node, fq_index, from, context, thread_indices, frame->n_vectors, 1);
    else
        n_enq = vlib_buffer_enqueue_to_thread(vm, node, fq_index, from, thread_indices, frame->n_vectors, 1);

    if (n_enq < frame->n_vectors)
        vlib_node_increment_counter(vm, node->node_index, IP6_SV_REASSEMBLY_HANDOFF_ERROR_CONGESTION_DROP, frame->n_vectors - n_enq);
    return frame->n_vectors;
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip6_sv_reassembly_handoff_inline(vm, node, frame, false /* is_feature */, false /* custom_context */);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip6_sv_reassembly_handoff_inline(vm, node, frame, true /* is_feature */, false /* custom_context */);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip6_sv_reassembly_handoff_inline(vm, node, frame, false /* is_feature */, true /* custom_context */);
}
...
int ip6_sv_reass_enable_disable_with_refcnt(u32 sw_if_index, int is_enable)
{
    ...
    if (is_enable) {
        if (!rm->feature_use_refcount_per_intf[sw_if_index]) {
            ...
            return vnet_feature_enable_disable("ip6-unicast", "ip6-sv-reassembly-feature", sw_if_index, 1, 0, 0);
        }
        ++rm->feature_use_refcount_per_intf[sw_if_index];
    }
    else {
        --rm->feature_use_refcount_per_intf[sw_if_index];
        if (!rm->feature_use_refcount_per_intf[sw_if_index])
            return vnet_feature_enable_disable("ip6-unicast", "ip6-sv-reassembly-feature", sw_if_index, 0, 0, 0);
    }
    return 0;
}
...
{
    return vlib_node_add_next(vlib_get_main(), ip6_sv_reassembly_custom_context_handoff_node.index, node_index);
}
#endif
