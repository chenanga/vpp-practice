...
always_inline u32 ip6_full_reass_buffer_get_data_offset(vlib_buffer_t *b)
{
    vnet_buffer_opaque_t *vnb = vnet_buffer(b);
    return vnb->ip.reass.range_first - vnb->ip.reass.fragment_first;
}

always_inline u16 ip6_full_reass_buffer_get_data_len(vlib_buffer_t *b)
{
    vnet_buffer_opaque_t *vnb = vnet_buffer(b);
    return clib_min(vnb->ip.reass.range_last, vnb->ip.reass.fragment_last) - (vnb->ip.reass.fragment_first + ip6_full_reass_buffer_get_data_offset(b))
           + 1;
}
...
typedef enum { NORMAL, FEATURE, CUSTOM } ip6_full_reass_node_type_t;
...
{
    ip6_full_reass_range_trace_t *trace = va_arg(*args, ip6_full_reass_range_trace_t *);
    s                                   = format(s,
...
static u8 *format_ip6_full_reass_trace(u8 *s, va_list *args)
{
    ...
    if (~0 != t->reass_id) {
        if (t->is_after_handoff) {
            s      = format(s, "%U\n", format_ip6_header, &t->ip6_header, sizeof(t->ip6_header));
            s      = format(s, "  %U\n", format_ip6_frag_hdr, &t->ip6_frag_header, sizeof(t->ip6_frag_header));
            indent = 2;
        }
        s      = format(s, "%Ureass id: %u, op id: %u, ", format_white_space, indent, t->reass_id, t->op_id);
        indent = format_get_indent(s);
        s      = format(s,
                   "first bi: %u, data len: %u, ip/fragment[%u, %u]",
                   t->trace_range.first_bi,
                   t->total_data_len,
                   t->fragment_first,
                   t->fragment_last);
    }
    switch (t->action) {
        case RANGE_NEW:
            s = format(s, "\n%Unew %U", format_white_space, indent, format_ip6_full_reass_range_trace, &t->trace_range);
            break;
        case RANGE_DISCARD:
            s = format(s, "\n%Udiscard %U", format_white_space, indent, format_ip6_full_reass_range_trace, &t->trace_range);
            break;
        case RANGE_OVERLAP:
            s = format(s, "\n%Uoverlap %U", format_white_space, indent, format_ip6_full_reass_range_trace, &t->trace_range);
            break;
        case ICMP_ERROR_FL_TOO_BIG:
            s = format(s, "\n%Uicmp-error - frag_len > 65535 %U", format_white_space, indent, format_ip6_full_reass_range_trace, &t->trace_range);
            break;
        case ICMP_ERROR_FL_NOT_MULT_8:
            s = format(s, "\n%Uicmp-error - frag_len mod 8 != 0 %U", format_white_space, indent, format_ip6_full_reass_range_trace, &t->trace_range);
            break;
        case ICMP_ERROR_RT_EXCEEDED:
            s = format(s, "\n%Uicmp-error - reassembly time exceeded", format_white_space, indent);
            break;
        case FINALIZE:
            s = format(s, "\n%Ufinalize reassembly", format_white_space, indent);
            break;
        case HANDOFF:
            s = format(s, "handoff from thread #%u to thread #%u", t->thread_id, t->thread_id_to);
            break;
        case PASSTHROUGH:
...
static void ip6_full_reass_add_trace(vlib_main_t                     *vm,
                                     ...
                                     u32                              thread_id_to)
{
    ...
    if (pool_is_free_index(vm->trace_main.trace_buffer_pool, vlib_buffer_get_trace_index(b))) {
    ...
    if (t->is_after_handoff) {
        clib_memcpy(&t->ip6_header, vlib_buffer_get_current(b), clib_min(sizeof(t->ip6_header), b->current_length));
        if (ip6_frag_header) {
            clib_memcpy(&t->ip6_frag_header, ip6_frag_header, sizeof(t->ip6_frag_header));
        }
        else {
...
always_inline void ip6_full_reass_free_ctx(ip6_full_reass_per_thread_t *rt, ip6_full_reass_t *reass)
{
    pool_put(rt->pool, reass);
    --rt->reass_n;
}

always_inline void ip6_full_reass_free(ip6_full_reass_main_t *rm, ip6_full_reass_per_thread_t *rt, ip6_full_reass_t *reass)
{
    clib_bihash_kv_48_8_t kv;
...
always_inline void ip6_full_reass_drop_all(vlib_main_t *vm, vlib_node_runtime_t *node, ip6_full_reass_t *reass, u32 *n_left_to_next, u32 **to_next)
{
    ...
    if (~0 != reass->error_next_index && reass->error_next_index < node->n_next_nodes) {
        u32 next_index;
        ...
        vlib_node_increment_counter(vm, node->node_index, IP6_ERROR_REASS_TO_CUSTOM_APP, vec_len(to_free));
            ...
            while (vec_len(to_free) > 0 && (*n_left_to_next) > 0) {
                ...
                if (~0 != bi) {
                    ...
                    if (PREDICT_FALSE(b->flags & VLIB_BUFFER_IS_TRACED)) {
                        ip6_full_reass_add_trace(vm, node, reass, bi, NULL, RANGE_DISCARD, ~0);
                    }
                    *to_next[0] = bi;
                    (*to_next) += 1;
                    (*n_left_to_next) -= 1;
...
always_inline void sanitize_reass_buffers_add_missing(vlib_main_t *vm, ip6_full_reass_t *reass, u32 *bi0)
{
    u32                   range_bi = reass->first_bi;
    vlib_buffer_t        *range_b;
    vnet_buffer_opaque_t *range_vnb;
...
always_inline void ip6_full_reass_on_timeout(vlib_main_t         *vm,
                                             vlib_node_runtime_t *node,
                                             ip6_full_reass_t    *reass,
                                             u32                 *icmp_bi,
                                             u32                 *n_left_to_next,
    ...
    if (~0 == reass->next_index)  // custom apps don't want icmp
    {
        ...
        if (0 == vnet_buffer(b)->ip.reass.fragment_first) {
            *icmp_bi = reass->first_bi;
            if (PREDICT_FALSE(b->flags & VLIB_BUFFER_IS_TRACED)) {
                ip6_full_reass_add_trace(vm, node, reass, reass->first_bi, NULL, ICMP_ERROR_RT_EXCEEDED, ~0);
            }
            ...
            icmp6_error_set_vnet_buffer(b, ICMP6_time_exceeded, ICMP6_time_exceeded_fragment_reassembly_time_exceeded, 0);
        }
    }
    ip6_full_reass_drop_all(vm, node, reass, n_left_to_next, to_next);
}

always_inline ip6_full_reass_t *ip6_full_reass_find_or_create(vlib_main_t                 *vm,
                                                              vlib_node_runtime_t         *node,
...
again:
    ...
    if (!skip_bihash && !clib_bihash_search_48_8(&rm->hash, &kv->kv, &kv->kv)) {
        ...
        reass = pool_elt_at_index(rm->per_thread_data[kv->v.memory_owner_thread_index].pool, kv->v.reass_index);

        if (now > reass->last_heard + rm->timeout) {
            vlib_node_increment_counter(vm, node->node_index, IP6_ERROR_REASS_TIMEOUT, 1);
            ip6_full_reass_on_timeout(vm, node, reass, icmp_bi, n_left_to_next, to_next);
...
always_inline ip6_full_reass_rc_t ip6_full_reass_finalize(vlib_main_t                 *vm,
                                                          vlib_node_runtime_t         *node,
                                                          ip6_full_reass_main_t       *rm,
                                                          ip6_full_reass_per_thread_t *rt,
                                                          ip6_full_reass_t            *reass,
                                                          ...
                                                          bool                         is_custom_app)
{
    ...
    *error0 = IP6_ERROR_NONE;
    ...
    do {
        ...
        if (!(vnb->ip.reass.range_first >= vnb->ip.reass.fragment_first) && !(vnb->ip.reass.range_last > vnb->ip.reass.fragment_first)) {
            rv = IP6_FULL_REASS_RC_INTERNAL_ERROR;
            goto free_buffers_and_return;
        }

        u32 data_len   = ip6_full_reass_buffer_get_data_len(tmp);
        u32 trim_front = vnet_buffer(tmp)->ip.reass.ip6_frag_hdr_offset + sizeof(*frag_hdr) + ip6_full_reass_buffer_get_data_offset(tmp);
        u32 trim_end   = vlib_buffer_length_in_chain(vm, tmp) - trim_front - data_len;
        if (tmp_bi == reass->first_bi) {
            ...
            trim_front = 0;
            trim_end   = vlib_buffer_length_in_chain(vm, tmp) - data_len - (vnet_buffer(tmp)->ip.reass.ip6_frag_hdr_offset + sizeof(*frag_hdr));
            if (!(vlib_buffer_length_in_chain(vm, tmp) - trim_end > 0)) {
                rv = IP6_FULL_REASS_RC_INTERNAL_ERROR;
                goto free_buffers_and_return;
            }
        }
        u32 keep_data = vlib_buffer_length_in_chain(vm, tmp) - trim_front - trim_end;
        while (1) {
            if (trim_front) {
        ...
        sub_chain_bi = vnet_buffer(vlib_get_buffer(vm, sub_chain_bi))->ip.reass.next_range_bi;
    ...
    int                   res      = ip6_ext_header_walk(first_b, ip, IP_PROTOCOL_IPV6_FRAGMENTATION, &hdr_chain);
    if (res < 0 || (hdr_chain.eh[res].protocol != IP_PROTOCOL_IPV6_FRAGMENTATION)) {
        rv = IP6_FULL_REASS_RC_INTERNAL_ERROR;
        goto free_buffers_and_return;
    }
    ...
    memmove(frag_hdr, (u8 *) frag_hdr + sizeof(*frag_hdr), first_b->current_length - ip6_frag_hdr_offset - sizeof(ip6_frag_hdr_t));
    first_b->current_length -= sizeof(*frag_hdr);
    ip->payload_length = clib_host_to_net_u16(total_length + first_b->current_length - sizeof(*ip));
    ...
    if (PREDICT_FALSE(first_b->flags & VLIB_BUFFER_IS_TRACED)) {
        ip6_full_reass_add_trace(vm, node, reass, reass->first_bi, NULL, FINALIZE, ~0);
#if 0
    ...
    vlib_node_increment_counter(vm, node->node_index, IP6_ERROR_REASS_SUCCESS, 1);

    vlib_node_increment_counter(vm, node->node_index, IP6_ERROR_REASS_FRAGMENTS_REASSEMBLED, reass->fragments_n);
...
always_inline void ip6_full_reass_insert_range_in_chain(vlib_main_t *vm, ip6_full_reass_t *reass, u32 prev_range_bi, u32 new_next_bi)
{
...
always_inline ip6_full_reass_rc_t ip6_full_reass_update(vlib_main_t                 *vm,
                                                        vlib_node_runtime_t         *node,
                                                        ip6_full_reass_main_t       *rm,
                                                        ip6_full_reass_per_thread_t *rt,
                                                        ip6_full_reass_t            *reass,
                                                        ...
                                                        int                          skip_bihash)
{
    ...
    fvnb->ip.reass.ip6_frag_hdr_offset = (u8 *) frag_hdr - (u8 *) vlib_buffer_get_current(fb);
    ip6_header_t *fip                  = vlib_buffer_get_current(fb);
    if (fb->current_length < sizeof(*fip) || fvnb->ip.reass.ip6_frag_hdr_offset == 0 || fvnb->ip.reass.ip6_frag_hdr_offset >= fb->current_length) {
        return IP6_FULL_REASS_RC_INTERNAL_ERROR;
    }

    u32 fragment_first = fvnb->ip.reass.fragment_first = ip6_frag_hdr_offset_bytes(frag_hdr);
    u32 fragment_length = vlib_buffer_length_in_chain(vm, fb) - (fvnb->ip.reass.ip6_frag_hdr_offset + sizeof(*frag_hdr));
    if (0 == fragment_length) {
        return IP6_FULL_REASS_RC_INVALID_FRAG_LEN;
    }
    u32 fragment_last = fvnb->ip.reass.fragment_last = fragment_first + fragment_length - 1;
    ...
    reass->min_fragment_length = clib_min(clib_net_to_host_u16(fip->payload_length), fvnb->ip.reass.estimated_mtu);
    while (~0 != candidate_range_bi) {
        ...
        if (fragment_first > candidate_vnb->ip.reass.range_last) {
            ...
            if (candidate_vnb->ip.reass.range_last < fragment_last && ~0 == candidate_range_bi) {
        ...
        else if (fragment_first == candidate_vnb->ip.reass.range_first && fragment_last == candidate_vnb->ip.reass.range_last) {
            // duplicate fragment - ignore
        }
        else {
            // overlapping fragment - not allowed by RFC 8200
            if (PREDICT_FALSE(fb->flags & VLIB_BUFFER_IS_TRACED)) {
                ip6_full_reass_add_trace(vm, node, reass, *bi0, frag_hdr, RANGE_OVERLAP, ~0);
            }
...
check_if_done_maybe:
    if (consumed) {
        if (PREDICT_FALSE(fb->flags & VLIB_BUFFER_IS_TRACED)) {
            ip6_full_reass_add_trace(vm, node, reass, *bi0, frag_hdr, RANGE_NEW, ~0);
        }
    }
    else if (skip_bihash) {
    ...
    if (~0 != reass->last_packet_octet && reass->data_len == reass->last_packet_octet + 1) {
        *handoff_thread_idx         = reass->sendout_thread_index;
        int                 handoff = reass->memory_owner_thread_index != reass->sendout_thread_index;
        ip6_full_reass_rc_t rc      = ip6_full_reass_finalize(vm, node, rm, rt, reass, bi0, next0, error0, is_custom_app);
...
always_inline bool ip6_full_reass_verify_upper_layer_present(vlib_node_runtime_t *node, vlib_buffer_t *b, ip6_ext_hdr_chain_t *hc)
{
    int nh = hc->eh[hc->length - 1].protocol;
    /* Checking to see if it's a terminating header */
    if (ip6_ext_hdr(nh)) {
        icmp6_error_set_vnet_buffer(b, ICMP6_parameter_problem, ICMP6_parameter_problem_first_fragment_has_incomplete_header_chain, 0);
...
always_inline bool ip6_full_reass_verify_fragment_multiple_8(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t *b, ip6_frag_hdr_t *frag_hdr)
{
    vnet_buffer_opaque_t *vnb             = vnet_buffer(b);
    ip6_header_t         *ip              = vlib_buffer_get_current(b);
    int                   more_fragments  = ip6_frag_hdr_more(frag_hdr);
    u32                   fragment_length = vlib_buffer_length_in_chain(vm, b) - (vnb->ip.reass.ip6_frag_hdr_offset + sizeof(*frag_hdr));
    if (more_fragments && 0 != fragment_length % 8) {
        icmp6_error_set_vnet_buffer(b,
...
always_inline bool ip6_full_reass_verify_packet_size_lt_64k(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t *b, ip6_frag_hdr_t *frag_hdr)
{
    vnet_buffer_opaque_t *vnb             = vnet_buffer(b);
...
always_inline uword
ip6_full_reassembly_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, bool is_feature, bool is_custom_app, bool is_local)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            int res = ip6_ext_header_walk(b0, ip0, IP_PROTOCOL_IPV6_FRAGMENTATION, &hdr_chain);
            if (res < 0 || hdr_chain.eh[res].protocol != IP_PROTOCOL_IPV6_FRAGMENTATION) {
                vlib_node_increment_counter(vm, node->node_index, IP6_ERROR_REASS_NO_FRAG_HDR, 1);
                // this is a mangled packet - no fragmentation
                next0 = is_custom_app ? fvnb->ip.reass.error_next_index : IP6_FULL_REASSEMBLY_NEXT_DROP;
                ip6_full_reass_add_trace(vm, node, NULL, bi0, NULL, PASSTHROUGH, ~0);
                goto skip_reass;
            }
            ...
            vlib_node_increment_counter(vm, node->node_index, IP6_ERROR_REASS_FRAGMENTS_RCVD, 1);
            frag_hdr                                      = ip6_ext_next_header_offset(ip0, hdr_chain.eh[res].offset);
            vnet_buffer(b0)->ip.reass.ip6_frag_hdr_offset = hdr_chain.eh[res].offset;

            if (0 == ip6_frag_hdr_offset(frag_hdr)) {
                // first fragment - verify upper-layer is present
                if (!ip6_full_reass_verify_upper_layer_present(node, b0, &hdr_chain)) {
                    next0 = is_custom_app ? fvnb->ip.reass.error_next_index : IP6_FULL_REASSEMBLY_NEXT_ICMP_ERROR;
                    goto skip_reass;
                }
            }

            if (!ip6_full_reass_verify_fragment_multiple_8(vm, node, b0, frag_hdr)
                || !ip6_full_reass_verify_packet_size_lt_64k(vm, node, b0, frag_hdr)) {
                next0 = is_custom_app ? fvnb->ip.reass.error_next_index : IP6_FULL_REASSEMBLY_NEXT_ICMP_ERROR;
                goto skip_reass;
            }
            ...
            if (0 == ip6_frag_hdr_offset(frag_hdr) && !ip6_frag_hdr_more(frag_hdr)) {
                // this is atomic fragment and needs to be processed separately
                skip_bihash = 1;
            }
            else {
                u32 fib_index  = (vnet_buffer(b0)->sw_if_index[VLIB_TX] == (u32) ~0)
                                     ? vec_elt(ip6_main.fib_index_by_sw_if_index, vnet_buffer(b0)->sw_if_index[VLIB_RX])
                ...
                kv.k.as_u64[4] = ((u64) fib_index) << 32 | (u64) frag_hdr->identification;
            ...
            ip6_full_reass_t *reass =
                ip6_full_reass_find_or_create(vm, node, rm, rt, &kv, &icmp_bi, &do_handoff, skip_bihash, &n_left_to_next, &to_next);
            ...
            if (PREDICT_FALSE(do_handoff)) {
                ...
                u32 handoff_thread_idx;
                u32 counter = ~0;
                switch (ip6_full_reass_update(vm,
                                              node,
                                              rm,
                                              rt,
                                              reass,
            ...
            skip_reass:
                ...
                if (next0 == IP6_FULL_REASSEMBLY_NEXT_HANDOFF) {
                    if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                        ip6_full_reass_add_trace(vm, node, NULL, bi0, frag_hdr, HANDOFF, vnet_buffer(b0)->ip.reass.owner_thread_index);
                    }
                }
                ...
                if (is_custom_app) {
                    vlib_node_increment_counter(vm, node->node_index, IP6_ERROR_REASS_TO_CUSTOM_APP, 1);
                }
            ...
            if (~0 != icmp_bi) {
                ...
                to_next += 1;
                n_left_to_next -= 1;
                vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, icmp_bi, next0);
            }
        next_packet:
            from += 1;
            n_left_from -= 1;
...
VLIB_NODE_FN(ip6_full_reass_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip6_full_reassembly_inline(vm, node, frame, false /* is_feature */, false /* is_custom_app */, false /* is_local */);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip6_full_reassembly_inline(vm, node, frame, false /* is_feature */, false /* is_custom_app */, true /* is_local */);
}
...
VLIB_NODE_FN(ip6_full_reass_node_feature)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip6_full_reassembly_inline(vm, node, frame, true /* is_feature */, false /* is_custom_app */, false /* is_local */);
}
...
VNET_FEATURE_INIT(ip6_full_reassembly_feature, static) = {
    ...
    .runs_before = VNET_FEATURES("ip6-lookup", "ipsec6-input-feature"),
    .runs_after  = 0,
};
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip6_full_reassembly_inline(vm, node, frame, false /* is_feature */, true /* is_custom_app */, false /* is_local */);
}
...
static u32 ip6_full_reass_get_nbuckets()
{
    ...
    nbuckets = (u32) (rm->max_reass_n * (vlib_num_workers() + 1) / IP6_FULL_REASS_HT_LOAD_FACTOR);

    for (i = 0; i < 31; i++)
        if ((1 << i) >= nbuckets) break;
    nbuckets = 1 << i;
...
vnet_api_error_t ip6_full_reass_set(u32 timeout_ms, u32 max_reassemblies, u32 max_reassembly_length, u32 expire_walk_interval_ms)
{
    ...
    ip6_full_reass_set_params(timeout_ms, max_reassemblies, max_reassembly_length, expire_walk_interval_ms);
    vlib_process_signal_event(ip6_full_reass_main.vlib_main, ip6_full_reass_main.ip6_full_reass_expire_node_idx, IP6_EVENT_CONFIG_CHANGED, 0);
    ...
    if (ip6_full_reass_main.max_reass_n > 0 && new_nbuckets > old_nbuckets) {
        ...
        clib_bihash_init_48_8(&new_hash, "ip6-full-reass", new_nbuckets, new_nbuckets * 1024);
        clib_bihash_foreach_key_value_pair_48_8(&ip6_full_reass_main.hash, ip6_rehash_cb, &ctx);
        if (ctx.failure) {
            ...
            clib_memcpy_fast(&ip6_full_reass_main.hash, &new_hash, sizeof(ip6_full_reass_main.hash));
...
static clib_error_t *ip6_full_reass_init_function(vlib_main_t *vm)
{
    ...
    clib_bihash_init_48_8(&rm->hash, "ip6-full-reass", nbuckets, nbuckets * 1024);

    node = vlib_get_node_by_name(vm, (u8 *) "ip6-icmp-error");
    ASSERT(node);
    rm->ip6_icmp_error_idx = node->index;

    if ((error = vlib_call_init_function(vm, ip_main_init))) return error;
    ip6_register_protocol(IP_PROTOCOL_IPV6_FRAGMENTATION, ip6_local_full_reass_node.index);
    rm->is_local_reass_enabled = 1;

    rm->fq_index         = vlib_frame_queue_main_init(ip6_full_reass_node.index, 0);
    rm->fq_local_index   = vlib_frame_queue_main_init(ip6_local_full_reass_node.index, 0);
    rm->fq_feature_index = vlib_frame_queue_main_init(ip6_full_reass_node_feature.index, 0);
    rm->fq_custom_index  = vlib_frame_queue_main_init(ip6_full_reass_node_custom.index, 0);
...
static uword ip6_full_reass_walk_expired(vlib_main_t *vm, vlib_node_runtime_t *node, CLIB_UNUSED(vlib_frame_t *f))
{
    ...
    while (true) {
        vlib_process_wait_for_event_or_clock(vm, (f64) rm->expire_walk_interval_ms / (f64) MSEC_PER_SEC);
        ...
        for (thread_index = 0; thread_index < nthreads; ++thread_index) {
            ...
            u32 end = beg + (IP6_FULL_REASS_MAX_REASSEMBLIES_DEFAULT * IP6_FULL_REASS_EXPIRE_WALK_INTERVAL_DEFAULT_MS / MSEC_PER_SEC + 1);
            ...
            vec_foreach (i, pool_indexes_to_free) {
                ...
                ip6_full_reass_on_timeout(vm, node, reass, &icmp_bi, &n_left_to_next, &to_next);
                if (~0 != icmp_bi) vec_add1(vec_icmp_bi, icmp_bi);

                ip6_full_reass_free(rm, rt, reass);
            }

            clib_spinlock_unlock(&rt->lock);
            if (reass_timeout_cnt) vlib_node_increment_counter(vm, node->node_index, IP6_ERROR_REASS_TIMEOUT, reass_timeout_cnt);
        }

        while (vec_len(vec_icmp_bi) > 0) {
            vlib_frame_t *f              = vlib_get_frame_to_node(vm, rm->ip6_icmp_error_idx);
            ...
            while (vec_len(vec_icmp_bi) > 0 && n_left_to_next > 0) {
                ...
                vlib_buffer_t *b  = vlib_get_buffer(vm, bi);
                if (PREDICT_FALSE(b->flags & VLIB_BUFFER_IS_TRACED)) trace_frame = 1;
                to_next[0] = bi;
                ++f->n_vectors;
                to_next += 1;
                n_left_to_next -= 1;
...
static u8 *format_ip6_full_reass_key(u8 *s, va_list *args)
{
    ...
    s                         = format(s,
               ...
               format_ip6_address,
               &key->dst,
               clib_net_to_host_u16(key->frag_id),
               key->proto);
    return s;
}
...
static clib_error_t *show_ip6_full_reass(vlib_main_t *vm, unformat_input_t *input, CLIB_UNUSED(vlib_cli_command_t *lmd))
{
    ...
    vlib_cli_output(vm, "Maximum configured concurrent full IP6 reassemblies per worker-thread: %lu\n", (long unsigned) rm->max_reass_n);
    vlib_cli_output(vm,
                    "Maximum configured amount of fragments "
                    "per full IP6 reassembly: %lu\n",
                    (long unsigned) rm->max_reass_len);
    vlib_cli_output(vm, "Maximum configured full IP6 reassembly timeout: %lums\n", (long unsigned) rm->timeout_ms);
    vlib_cli_output(vm, "Maximum configured full IP6 reassembly expire walk interval: %lums\n", (long unsigned) rm->expire_walk_interval_ms);
...
{
    return vnet_feature_enable_disable("ip6-unicast", "ip6-full-reassembly-feature", sw_if_index, enable_disable, 0, 0);
}
#endif /* CLIB_MARCH_VARIANT */

#define foreach_ip6_full_reassembly_handoff_error _(CONGESTION_DROP, "congestion drop")
...
static u8 *format_ip6_full_reassembly_handoff_trace(u8 *s, va_list *args)
{
    CLIB_UNUSED(vlib_main_t * vm)          = va_arg(*args, vlib_main_t *);
    CLIB_UNUSED(vlib_node_t * node)        = va_arg(*args, vlib_node_t *);
    ip6_full_reassembly_handoff_trace_t *t = va_arg(*args, ip6_full_reassembly_handoff_trace_t *);

    s = format(s, "ip6-full-reassembly-handoff: next-worker %d", t->next_worker_index);

    return s;
}

always_inline uword
ip6_full_reassembly_handoff_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, ip6_full_reass_node_type_t type, bool is_local)
{
    ip6_full_reass_main_t *rm = &ip6_full_reass_main;
    ...
    while (n_left_from > 0) {
        ...
        if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b[0]->flags & VLIB_BUFFER_IS_TRACED))) {
            ip6_full_reassembly_handoff_trace_t *t = vlib_add_trace(vm, node, b[0], sizeof(*t));
    ...
    if (n_enq < frame->n_vectors)
        vlib_node_increment_counter(vm, node->node_index, IP6_FULL_REASSEMBLY_HANDOFF_ERROR_CONGESTION_DROP, frame->n_vectors - n_enq);
    return frame->n_vectors;
}

VLIB_NODE_FN(ip6_full_reassembly_handoff_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip6_full_reassembly_handoff_inline(vm, node, frame, NORMAL, false /* is_local */);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip6_full_reassembly_handoff_inline(vm, node, frame, NORMAL, true /* is_feature */);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip6_full_reassembly_handoff_inline(vm, node, frame, FEATURE, false /* is_local */);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip6_full_reassembly_handoff_inline(vm, node, frame, CUSTOM, false /* is_local */);
}
...
int ip6_full_reass_enable_disable_with_refcnt(u32 sw_if_index, int is_enable)
{
    ...
    if (is_enable) {
        if (!rm->feature_use_refcount_per_intf[sw_if_index]) {
            ...
            return vnet_feature_enable_disable("ip6-unicast", "ip6-full-reassembly-feature", sw_if_index, 1, 0, 0);
        }
        ++rm->feature_use_refcount_per_intf[sw_if_index];
    }
    else {
        --rm->feature_use_refcount_per_intf[sw_if_index];
        if (!rm->feature_use_refcount_per_intf[sw_if_index])
            return vnet_feature_enable_disable("ip6-unicast", "ip6-full-reassembly-feature", sw_if_index, 0, 0, 0);
    }
    return 0;
}
