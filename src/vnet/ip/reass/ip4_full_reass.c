...
always_inline u32 ip4_full_reass_buffer_get_data_offset(vlib_buffer_t *b)
{
    vnet_buffer_opaque_t *vnb = vnet_buffer(b);
    return vnb->ip.reass.range_first - vnb->ip.reass.fragment_first;
}

always_inline u16 ip4_full_reass_buffer_get_data_len(vlib_buffer_t *b)
{
    vnet_buffer_opaque_t *vnb = vnet_buffer(b);
    return clib_min(vnb->ip.reass.range_last, vnb->ip.reass.fragment_last) - (vnb->ip.reass.fragment_first + ip4_full_reass_buffer_get_data_offset(b))
           + 1;
}
...
typedef enum { NORMAL, FEATURE, CUSTOM } ip4_full_reass_node_type_t;
...
{
    ip4_full_reass_range_trace_t *trace = va_arg(*args, ip4_full_reass_range_trace_t *);
    s                                   = format(s,
...
static u8 *format_ip4_full_reass_trace(u8 *s, va_list *args)
{
    ...
    if (~0 != t->reass_id) {
        if (t->is_after_handoff) {
            s      = format(s, "%U\n", format_ip4_header, &t->ip4_header, sizeof(t->ip4_header));
            indent = 2;
        }
    ...
    switch (t->action) {
        case RANGE_SHRINK:
            s = format(s, "\n%Ushrink %U by %u", format_white_space, indent, format_ip4_full_reass_range_trace, &t->trace_range, t->size_diff);
            break;
        case RANGE_DISCARD:
            s = format(s, "\n%Udiscard %U", format_white_space, indent, format_ip4_full_reass_range_trace, &t->trace_range);
            break;
        case RANGE_NEW:
            s = format(s, "\n%Unew %U", format_white_space, indent, format_ip4_full_reass_range_trace, &t->trace_range);
            break;
        case RANGE_OVERLAP:
            s = format(s, "\n%Uoverlapping/ignored %U", format_white_space, indent, format_ip4_full_reass_range_trace, &t->trace_range);
            break;
        case FINALIZE:
                                     ...
                                     u32                              thread_id_to)
{
    ...
    if (t->is_after_handoff) {
        clib_memcpy(&t->ip4_header, vlib_buffer_get_current(b), clib_min(sizeof(t->ip4_header), b->current_length));
    }
    if (reass) {
        t->reass_id             = reass->id;
...
always_inline void ip4_full_reass_free_ctx(ip4_full_reass_per_thread_t *rt, ip4_full_reass_t *reass)
{
    pool_put(rt->pool, reass);
    --rt->reass_n;
}

always_inline void ip4_full_reass_free(ip4_full_reass_main_t *rm, ip4_full_reass_per_thread_t *rt, ip4_full_reass_t *reass)
{
    clib_bihash_kv_16_8_t kv = {};
...
always_inline void ip4_full_reass_drop_all(vlib_main_t *vm, vlib_node_runtime_t *node, ip4_full_reass_t *reass)
{
    ...
    if (~0 != reass->error_next_index && reass->error_next_index < node->n_next_nodes) {
        ...
        vlib_node_increment_counter(vm, node->node_index, IP4_ERROR_REASS_TO_CUSTOM_APP, n_free);

        if (node->flags & VLIB_NODE_FLAG_TRACE)
            for (u32 i = 0; i < n_free; i++) {
                vlib_buffer_t *b = vlib_get_buffer(vm, to_free[i]);
                if (PREDICT_FALSE(b->flags & VLIB_BUFFER_IS_TRACED)) ip4_full_reass_add_trace(vm, node, reass, to_free[i], RANGE_DISCARD, 0, ~0);
            }

        vlib_buffer_enqueue_to_single_next(vm, node, to_free, reass->error_next_index, n_free);
    }
    else {
...
always_inline void sanitize_reass_buffers_add_missing(vlib_main_t *vm, ip4_full_reass_t *reass, u32 *bi0)
{
    u32                   range_bi = reass->first_bi;
    vlib_buffer_t        *range_b;
    vnet_buffer_opaque_t *range_vnb;
...
always_inline void ip4_full_reass_init(ip4_full_reass_t *reass)
{
    reass->first_bi          = ~0;
...
always_inline ip4_full_reass_t *ip4_full_reass_find_or_create(vlib_main_t                 *vm,
                                                              vlib_node_runtime_t         *node,
...
again:
    ...
    if (!clib_bihash_search_16_8(&rm->hash, &kv->kv, &kv->kv)) {
        ...
        if (now > reass->last_heard + rm->timeout) {
            vlib_node_increment_counter(vm, node->node_index, IP4_ERROR_REASS_TIMEOUT, 1);
...
always_inline ip4_full_reass_rc_t ip4_full_reass_finalize(vlib_main_t                 *vm,
                                                          vlib_node_runtime_t         *node,
                                                          ip4_full_reass_main_t       *rm,
                                                          ip4_full_reass_per_thread_t *rt,
                                                          ip4_full_reass_t            *reass,
                                                          ...
                                                          bool                         is_custom)
{
    ...
    do {
        ...
        u32 trim_front = ip4_header_bytes(ip) + ip4_full_reass_buffer_get_data_offset(tmp);
        ...
        if (tmp_bi == reass->first_bi) {
            ...
            trim_front = 0;
            trim_end   = vlib_buffer_length_in_chain(vm, tmp) - data_len - ip4_header_bytes(ip);
    ...
    if (PREDICT_FALSE(first_b->flags & VLIB_BUFFER_IS_TRACED)) {
        ip4_full_reass_add_trace(vm, node, reass, reass->first_bi, FINALIZE, 0, ~0);
#if 0
    ...
    *bi0 = reass->first_bi;
    ...
    vlib_node_increment_counter(vm, node->node_index, IP4_ERROR_REASS_SUCCESS, 1);

    vlib_node_increment_counter(vm, node->node_index, IP4_ERROR_REASS_FRAGMENTS_REASSEMBLED, reass->fragments_n);
...
always_inline ip4_full_reass_rc_t ip4_full_reass_insert_range_in_chain(vlib_main_t *vm, ip4_full_reass_t *reass, u32 prev_range_bi, u32 new_next_bi)
{
...
ip4_full_reass_remove_range_from_chain(vlib_main_t *vm, vlib_node_runtime_t *node, ip4_full_reass_t *reass, u32 prev_range_bi, u32 discard_bi)
{
    ...
    while (1) {
        ...
        if (PREDICT_FALSE(discard_b->flags & VLIB_BUFFER_IS_TRACED)) {
            ip4_full_reass_add_trace(vm, node, reass, discard_bi, RANGE_DISCARD, 0, ~0);
        }
...
always_inline ip4_full_reass_rc_t ip4_full_reass_update(vlib_main_t                 *vm,
                                                        vlib_node_runtime_t         *node,
                                                        ip4_full_reass_main_t       *rm,
                                                        ip4_full_reass_per_thread_t *rt,
                                                        ip4_full_reass_t            *reass,
                                                        ...
                                                        u32                         *handoff_thread_idx)
{
    ...
    const u32           fragment_length = clib_net_to_host_u16(fip->length) - ip4_header_bytes(fip);
    ...
    if (~0 == reass->first_bi) {
        ...
        rc = ip4_full_reass_insert_range_in_chain(vm, reass, prev_range_bi, *bi0);
    ...
    reass->min_fragment_length = clib_min(clib_net_to_host_u16(fip->length), fvnb->ip.reass.estimated_mtu);
    while (~0 != candidate_range_bi) {
        ...
        if (fragment_last < candidate_vnb->ip.reass.range_first) {
            ...
            if (fragment_first >= candidate_vnb->ip.reass.range_first && fragment_last <= candidate_vnb->ip.reass.range_last) {
                // this fragment is a (sub)part of existing range, ignore it
                if (PREDICT_FALSE(fb->flags & VLIB_BUFFER_IS_TRACED)) {
                    ip4_full_reass_add_trace(vm, node, reass, *bi0, RANGE_OVERLAP, 0, ~0);
                }
                break;
            }
            int discard_candidate = 0;
            if (fragment_first < candidate_vnb->ip.reass.range_first) {
                u32 overlap = fragment_last - candidate_vnb->ip.reass.range_first + 1;
                if (overlap < ip4_full_reass_buffer_get_data_len(candidate_b)) {
                    ...
                    if (PREDICT_FALSE(fb->flags & VLIB_BUFFER_IS_TRACED)) {
                        ip4_full_reass_add_trace(vm, node, reass, candidate_range_bi, RANGE_SHRINK, 0, ~0);
                    }
                    rc = ip4_full_reass_insert_range_in_chain(vm, reass, prev_range_bi, *bi0);
                    if (IP4_REASS_RC_OK != rc) {
                        return rc;
                ...
                u32 overlap = candidate_vnb->ip.reass.range_last - fragment_first + 1;
                if (overlap < ip4_full_reass_buffer_get_data_len(candidate_b)) {
                    ...
                    if (~0 != candidate_vnb->ip.reass.next_range_bi) {
                        ...
                        rc = ip4_full_reass_insert_range_in_chain(vm, reass, candidate_range_bi, *bi0);
                        if (IP4_REASS_RC_OK != rc) {
                            return rc;
                        }
                        consumed = 1;
            ...
            if (discard_candidate) {
                ...
                rc = ip4_full_reass_remove_range_from_chain(vm, node, reass, prev_range_bi, candidate_range_bi);
                if (IP4_REASS_RC_OK != rc) {
                    return rc;
                }
                if (~0 != next_range_bi) {
    ...
    if (~0 != reass->last_packet_octet && reass->data_len == reass->last_packet_octet + 1) {
        *handoff_thread_idx = reass->sendout_thread_index;
        ...
        rc                  = ip4_full_reass_finalize(vm, node, rm, rt, reass, bi0, next0, error0, is_custom);
...
always_inline uword
ip4_full_reass_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, ip4_full_reass_node_type_t type, bool is_local)
{
    ...
    while (n_left > 0) {
        ...
        const u32 fragment_length = clib_net_to_host_u16(ip0->length) - ip4_header_bytes(ip0);
        const u32 fragment_last   = fragment_first + fragment_length - 1;

        /* Keep track of received fragments */
        vlib_node_increment_counter(vm, node->node_index, IP4_ERROR_REASS_FRAGMENTS_RCVD, 1);

        if (fragment_first > fragment_last || fragment_first + fragment_length > UINT16_MAX - 20
        ...
        u32 fib_index = (vnet_buffer(b0)->sw_if_index[VLIB_TX] == (u32) ~0)
                            ? vec_elt(ip4_main.fib_index_by_sw_if_index, vnet_buffer(b0)->sw_if_index[VLIB_RX])
        ...
        ip4_full_reass_t *reass = ip4_full_reass_find_or_create(vm, node, rm, rt, &kv, &do_handoff);
        ...
        if (PREDICT_FALSE(do_handoff)) {
            ...
            u32 handoff_thread_idx;
            u32 counter = ~0;
            switch (ip4_full_reass_update(vm, node, rm, rt, reass, &bi0, &next0, &error0, CUSTOM == type, &handoff_thread_idx)) {
        ...
        if (bi0 != ~0) {
            ...
            if (next0 == IP4_FULL_REASS_NEXT_HANDOFF) {
                if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                    ip4_full_reass_add_trace(vm, node, NULL, bi0, HANDOFF, 0, vnet_buffer(b0)->ip.reass.owner_thread_index);
                }
            }
            ...
            if (CUSTOM == type) {
                vlib_node_increment_counter(vm, node->node_index, IP4_ERROR_REASS_TO_CUSTOM_APP, 1);
            }
...
VLIB_NODE_FN(ip4_full_reass_node)
...
VLIB_NODE_FN(ip4_full_reass_node_feature)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip4_full_reass_inline(vm, node, frame, FEATURE, false /* is_local */);
}
...
VNET_FEATURE_INIT(ip4_full_reass_feature, static) = {
    ...
    .runs_before = VNET_FEATURES("ip4-lookup", "ipsec4-input-feature", "ip4-sv-reassembly-feature"),
    .runs_after  = 0,
};

VLIB_NODE_FN(ip4_full_reass_node_custom)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
{
    return vlib_node_add_next(vlib_get_main(), ip4_full_reass_node_custom.index, node_index);
}

always_inline u32 ip4_full_reass_get_nbuckets()
{
    ip4_full_reass_main_t *rm = &ip4_full_reass_main;
    u32                    nbuckets;
    u8                     i;
    ...
    nbuckets = (u32) (rm->max_reass_n * (vlib_num_workers() + 1) / IP4_REASS_HT_LOAD_FACTOR);
...
vnet_api_error_t ip4_full_reass_set(u32 timeout_ms, u32 max_reassemblies, u32 max_reassembly_length, u32 expire_walk_interval_ms)
{
    ...
    ip4_full_reass_set_params(timeout_ms, max_reassemblies, max_reassembly_length, expire_walk_interval_ms);
    vlib_process_signal_event(ip4_full_reass_main.vlib_main, ip4_full_reass_main.ip4_full_reass_expire_node_idx, IP4_EVENT_CONFIG_CHANGED, 0);
    ...
    if (ip4_full_reass_main.max_reass_n > 0 && new_nbuckets > old_nbuckets) {
        ...
        clib_bihash_init_16_8(&new_hash, "ip4-dr", new_nbuckets, new_nbuckets * 1024);
        clib_bihash_foreach_key_value_pair_16_8(&ip4_full_reass_main.hash, ip4_rehash_cb, &ctx);
        if (ctx.failure) {
            ...
            clib_memcpy_fast(&ip4_full_reass_main.hash, &new_hash, sizeof(ip4_full_reass_main.hash));
...
static clib_error_t *ip4_full_reass_init_function(vlib_main_t *vm)
{
    ...
    rm->fq_local_index   = vlib_frame_queue_main_init(ip4_local_full_reass_node.index, 0);
    rm->fq_feature_index = vlib_frame_queue_main_init(ip4_full_reass_node_feature.index, 0);
    rm->fq_custom_index  = vlib_frame_queue_main_init(ip4_full_reass_node_custom.index, 0);
...
static uword ip4_full_reass_walk_expired(vlib_main_t *vm, vlib_node_runtime_t *node, CLIB_UNUSED(vlib_frame_t *f))
{
    ...
    while (true) {
        ...
        for (thread_index = 0; thread_index < nthreads; ++thread_index) {
            ...
            u32 end = beg + (IP4_REASS_MAX_REASSEMBLIES_DEFAULT * IP4_REASS_EXPIRE_WALK_INTERVAL_DEFAULT_MS / MSEC_PER_SEC + 1);
...
static u8 *format_ip4_full_reass_key(u8 *s, va_list *args)
{
    ...
    s                         = format(s,
               "fib_index: %u, src: %U, dst: %U, frag_id: %u, proto: %u",
               key->fib_index,
               format_ip4_address,
               &key->src,
               format_ip4_address,
...
static clib_error_t *show_ip4_reass(vlib_main_t *vm, unformat_input_t *input, CLIB_UNUSED(vlib_cli_command_t *lmd))
{
    ...
    vlib_cli_output(vm, "Maximum configured concurrent full IP4 reassemblies per worker-thread: %lu\n", (long unsigned) rm->max_reass_n);
    vlib_cli_output(vm,
                    "Maximum configured amount of fragments "
                    "per full IP4 reassembly: %lu\n",
                    (long unsigned) rm->max_reass_len);
    vlib_cli_output(vm, "Maximum configured full IP4 reassembly timeout: %lums\n", (long unsigned) rm->timeout_ms);
    vlib_cli_output(vm, "Maximum configured full IP4 reassembly expire walk interval: %lums\n", (long unsigned) rm->expire_walk_interval_ms);
    return 0;
}
...
{
    return vnet_feature_enable_disable("ip4-unicast", "ip4-full-reassembly-feature", sw_if_index, enable_disable, 0, 0);
}
#endif /* CLIB_MARCH_VARIANT */
...
static u8 *format_ip4_full_reass_handoff_trace(u8 *s, va_list *args)
{
    CLIB_UNUSED(vlib_main_t * vm)     = va_arg(*args, vlib_main_t *);
    CLIB_UNUSED(vlib_node_t * node)   = va_arg(*args, vlib_node_t *);
    ip4_full_reass_handoff_trace_t *t = va_arg(*args, ip4_full_reass_handoff_trace_t *);

    s = format(s, "ip4-full-reassembly-handoff: next-worker %d", t->next_worker_index);

    return s;
}

always_inline uword
ip4_full_reass_handoff_node_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, ip4_full_reass_node_type_t type, bool is_local)
{
    ip4_full_reass_main_t *rm = &ip4_full_reass_main;
    ...
    while (n_left_from > 0) {
        ...
        if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b[0]->flags & VLIB_BUFFER_IS_TRACED))) {
            ip4_full_reass_handoff_trace_t *t = vlib_add_trace(vm, node, b[0], sizeof(*t));
    ...
    if (n_enq < frame->n_vectors)
        vlib_node_increment_counter(vm, node->node_index, IP4_FULL_REASS_HANDOFF_ERROR_CONGESTION_DROP, frame->n_vectors - n_enq);
    return frame->n_vectors;
}

VLIB_NODE_FN(ip4_full_reass_handoff_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip4_full_reass_handoff_node_inline(vm, node, frame, NORMAL, false /* is_local */);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip4_full_reass_handoff_node_inline(vm, node, frame, NORMAL, true /* is_local */);
}
...
VLIB_NODE_FN(ip4_full_reass_feature_handoff_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip4_full_reass_handoff_node_inline(vm, node, frame, FEATURE, false /* is_local */);
}
...
VLIB_NODE_FN(ip4_full_reass_custom_handoff_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip4_full_reass_handoff_node_inline(vm, node, frame, CUSTOM, false /* is_local */);
}
...
int ip4_full_reass_enable_disable_with_refcnt(u32 sw_if_index, int is_enable)
{
    ...
    if (is_enable) {
        if (!rm->feature_use_refcount_per_intf[sw_if_index]) {
            ...
            return vnet_feature_enable_disable("ip4-unicast", "ip4-full-reassembly-feature", sw_if_index, 1, 0, 0);
        }
        ++rm->feature_use_refcount_per_intf[sw_if_index];
    }
    else {
        --rm->feature_use_refcount_per_intf[sw_if_index];
        if (!rm->feature_use_refcount_per_intf[sw_if_index])
            return vnet_feature_enable_disable("ip4-unicast", "ip4-full-reassembly-feature", sw_if_index, 0, 0, 0);
    }
    return 0;
}
