...
always_inline void ip4_sv_reass_free(vlib_main_t *vm, ip4_sv_reass_main_t *rm, ip4_sv_reass_per_thread_t *rt, ip4_sv_reass_t *reass)
{
    clib_bihash_kv_16_8_t kv;
...
always_inline void ip4_sv_reass_init(ip4_sv_reass_t *reass)
{
    reass->cached_buffers = NULL;
    reass->is_complete    = false;
}

always_inline ip4_sv_reass_t *ip4_sv_reass_find_or_create(vlib_main_t               *vm,
...
ip4_sv_reass_update(vlib_main_t *vm, vlib_node_runtime_t *node, ip4_sv_reass_main_t *rm, ip4_header_t *ip0, ip4_sv_reass_t *reass, u32 bi0)
{
    ...
    if (0 == fragment_first) {
        ...
        if (!reass->l4_src_port || !reass->l4_dst_port) return IP4_SV_REASS_RC_UNSUPP_IP_PROTO;
        if (IP_PROTOCOL_TCP == reass->ip_proto) {
            ...
            reass->icmp_type_or_tcp_flags = ((icmp46_header_t *) (ip0 + 1))->type;
        }
        reass->is_complete = true;
        vlib_buffer_t *b0  = vlib_get_buffer(vm, bi0);
        if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
            ip4_sv_reass_add_trace(vm,
                                   node,
                                   reass,
                                   bi0,
                                   REASS_FINISH,
    ...
    if (!reass->is_complete) {
        if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
            ip4_sv_reass_add_trace(vm, node, reass, bi0, REASS_FRAGMENT_CACHE, ~0, ~0, ~0, vnet_buffer(b0)->ip.reass.l4_layer_truncated);
        }
...
always_inline int l4_layer_truncated(ip4_header_t *ip)
{
    ...
    return ((u8 *) ip + ip4_header_bytes(ip) + l4_layer_length[ip->protocol] > (u8 *) ip + clib_net_to_host_u16(ip->length));
}

always_inline uword ip4_sv_reass_inline(vlib_main_t         *vm,
                                        vlib_node_runtime_t *node,
                                        vlib_frame_t        *frame,
                                        bool                 is_feature,
                                        ...
                                        bool                 with_custom_context)
{
    ...
    u32                       *context;
    if (with_custom_context) context = vlib_frame_aux_args(frame);
    ...
    while (n_left_from >= 2) {
        ...
        ip4_header_t *ip0 =
            (ip4_header_t *) u8_ptr_add(vlib_buffer_get_current(b0), (is_output_feature ? 1 : 0) * vnet_buffer(b0)->ip.save_rewrite_length);
        ip4_header_t *ip1 =
            (ip4_header_t *) u8_ptr_add(vlib_buffer_get_current(b1), (is_output_feature ? 1 : 0) * vnet_buffer(b1)->ip.save_rewrite_length);

        if (PREDICT_FALSE(ip4_get_fragment_more(ip0) || ip4_get_fragment_offset(ip0))
            || (ip4_get_fragment_more(ip1) || ip4_get_fragment_offset(ip1))) {
            // fragment found, go slow path
            b -= 2;
            if (b - bufs > 0) {
                vlib_buffer_enqueue_to_next(vm, node, from, (u16 *) nexts, b - bufs);
            }
            goto slow_path;
        }
        if (is_feature) {
            ...
            next0 = is_custom ? vnet_buffer(b0)->ip.reass.next_index : IP4_SV_REASSEMBLY_NEXT_INPUT;
        }
        ...
        if (l4_layer_truncated(ip0)) {
            ...
            if (IP_PROTOCOL_TCP == ip0->protocol) {
                vnet_buffer(b0)->ip.reass.icmp_type_or_tcp_flags = ((tcp_header_t *) (ip0 + 1))->flags;
                vnet_buffer(b0)->ip.reass.tcp_ack_number         = ((tcp_header_t *) (ip0 + 1))->ack_number;
                vnet_buffer(b0)->ip.reass.tcp_seq_number         = ((tcp_header_t *) (ip0 + 1))->seq_number;
            }
            else if (IP_PROTOCOL_ICMP == ip0->protocol) {
                vnet_buffer(b0)->ip.reass.icmp_type_or_tcp_flags = ((icmp46_header_t *) (ip0 + 1))->type;
            }
        ...
        if (is_feature) {
            ...
            next1 = is_custom ? vnet_buffer(b1)->ip.reass.next_index : IP4_SV_REASSEMBLY_NEXT_INPUT;
        }
        ...
        if (l4_layer_truncated(ip1)) {
            ...
            if (IP_PROTOCOL_TCP == ip1->protocol) {
                vnet_buffer(b1)->ip.reass.icmp_type_or_tcp_flags = ((tcp_header_t *) (ip1 + 1))->flags;
                vnet_buffer(b1)->ip.reass.tcp_ack_number         = ((tcp_header_t *) (ip1 + 1))->ack_number;
                vnet_buffer(b1)->ip.reass.tcp_seq_number         = ((tcp_header_t *) (ip1 + 1))->seq_number;
            }
            else if (IP_PROTOCOL_ICMP == ip1->protocol) {
                vnet_buffer(b1)->ip.reass.icmp_type_or_tcp_flags = ((icmp46_header_t *) (ip1 + 1))->type;
            }
        ...
        next[1] = next1;
        next += 2;
        if (with_custom_context) context += 2;
    }
    ...
    while (n_left_from > 0) {
        ...
        next[0] = next0;
        next += 1;
        if (with_custom_context) context += 1;
    }
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (fragment_first > fragment_last || fragment_first + fragment_length > UINT16_MAX - 20
                || (fragment_length < 8 && ip4_get_fragment_more(ip0)))  // 8 is minimum frag length per RFC 791
            {
            ...
            if (with_custom_context)
                kv.k.as_u64[0] = (u64) *context | (u64) ip0->src_address.as_u32 << 32;
            else
                kv.k.as_u64[0] =
                    (u64) vec_elt(ip4_main.fib_index_by_sw_if_index, vnet_buffer(b0)->sw_if_index[VLIB_RX]) | (u64) ip0->src_address.as_u32 << 32;
            kv.k.as_u64[1] = (u64) ip0->dst_address.as_u32 | (u64) ip0->fragment_id << 32 | (u64) ip0->protocol << 48;

            ip4_sv_reass_t *reass = ip4_sv_reass_find_or_create(vm, rm, rt, &kv, &do_handoff);
            ...
            if (PREDICT_FALSE(do_handoff)) {
                ...
                if (with_custom_context) forward_context = 1;
                goto packet_enqueue;
            }
            ...
            if (reass->is_complete) {
                ...
                vnet_buffer(b0)->ip.reass.is_non_first_fragment  = !!fragment_first;
            ...
            ip4_sv_reass_rc_t rc      = ip4_sv_reass_update(vm, node, rm, ip0, reass, bi0);
            u32               counter = ~0;
            switch (rc) {
            ...
            if (reass->is_complete) {
                ...
                vec_foreach_index (idx, reass->cached_buffers) {
                    ...
                    ip0                = (ip4_header_t *) u8_ptr_add(vlib_buffer_get_current(b0),
                                                      (is_output_feature ? 1 : 0) * vnet_buffer(b0)->ip.save_rewrite_length);
                    ...
                    to_next += 1;
                    n_left_to_next -= 1;
                    vnet_buffer(b0)->ip.reass.is_non_first_fragment  = !!ip4_get_fragment_offset(ip0);
                    vnet_buffer(b0)->ip.reass.ip_proto               = reass->ip_proto;
                    vnet_buffer(b0)->ip.reass.icmp_type_or_tcp_flags = reass->icmp_type_or_tcp_flags;
                    vnet_buffer(b0)->ip.reass.tcp_ack_number         = reass->tcp_ack_number;
                    vnet_buffer(b0)->ip.reass.tcp_seq_number         = reass->tcp_seq_number;
...
#ifndef CLIB_MARCH_VARIANT
always_inline u32 ip4_sv_reass_get_nbuckets()
{
    ip4_sv_reass_main_t *rm = &ip4_sv_reass_main;
    u32                  nbuckets;
    u8                   i;
...
vnet_api_error_t ip4_sv_reass_set(u32 timeout_ms, u32 max_reassemblies, u32 max_reassembly_length, u32 expire_walk_interval_ms)
{
    ...
    ip4_sv_reass_set_params(timeout_ms, max_reassemblies, max_reassembly_length, expire_walk_interval_ms);
    vlib_process_signal_event(ip4_sv_reass_main.vlib_main, ip4_sv_reass_main.ip4_sv_reass_expire_node_idx, IP4_EVENT_CONFIG_CHANGED, 0);
    ...
    if (ip4_sv_reass_main.max_reass_n > 0 && new_nbuckets > old_nbuckets) {
        ...
        clib_bihash_foreach_key_value_pair_16_8(&ip4_sv_reass_main.hash, ip4_rehash_cb, &ctx);
        if (ctx.failure) {
            ...
            clib_memcpy_fast(&ip4_sv_reass_main.hash, &new_hash, sizeof(ip4_sv_reass_main.hash));
...
static clib_error_t *ip4_sv_reass_init_function(vlib_main_t *vm)
{
    ...
    rm->fq_feature_index        = vlib_frame_queue_main_init(ip4_sv_reass_node_feature.index, 0);
    rm->fq_custom_context_index = vlib_frame_queue_main_init(ip4_sv_reass_custom_context_node.index, 0);
...
static u8 *format_ip4_sv_reass(u8 *s, va_list *args)
{
    ...
    s = format(s, "ID: %lu, key: %U trace_op_counter: %u\n", reass->id, format_ip4_sv_reass_key, &reass->key, reass->trace_op_counter);
...
static clib_error_t *show_ip4_reass(vlib_main_t *vm, unformat_input_t *input, CLIB_UNUSED(vlib_cli_command_t *lmd))
{
    ...
    vlib_cli_output(vm, "Maximum configured concurrent shallow virtual IP4 reassemblies per worker-thread: %lu\n", (long unsigned) rm->max_reass_n);
    vlib_cli_output(vm,
                    "Maximum configured amount of fragments per shallow "
                    "virtual IP4 reassembly: %lu\n",
                    (long unsigned) rm->max_reass_len);
    vlib_cli_output(vm, "Maximum configured shallow virtual IP4 reassembly timeout: %lums\n", (long unsigned) rm->timeout_ms);
    vlib_cli_output(vm,
...
static u8 *format_ip4_sv_reass_handoff_trace(u8 *s, va_list *args)
{
    CLIB_UNUSED(vlib_main_t * vm)   = va_arg(*args, vlib_main_t *);
    CLIB_UNUSED(vlib_node_t * node) = va_arg(*args, vlib_node_t *);
    ip4_sv_reass_handoff_trace_t *t = va_arg(*args, ip4_sv_reass_handoff_trace_t *);

    s = format(s, "ip4-sv-reassembly-handoff: next-worker %d", t->next_worker_index);

    return s;
}

always_inline uword
ip4_sv_reass_handoff_node_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, bool is_feature, bool is_custom_context)
{
    ip4_sv_reass_main_t *rm = &ip4_sv_reass_main;
    ...
    if (is_custom_context) context = vlib_frame_aux_args(frame);
    ...
    fq_index = (is_feature) ? rm->fq_feature_index : (is_custom_context ? rm->fq_custom_context_index : rm->fq_index);
    ...
    while (n_left_from > 0) {
        ...
        if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b[0]->flags & VLIB_BUFFER_IS_TRACED))) {
            ip4_sv_reass_handoff_trace_t *t = vlib_add_trace(vm, node, b[0], sizeof(*t));
    ...
    if (n_enq < frame->n_vectors)
        vlib_node_increment_counter(vm, node->node_index, IP4_SV_REASSEMBLY_HANDOFF_ERROR_CONGESTION_DROP, frame->n_vectors - n_enq);
    return frame->n_vectors;
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip4_sv_reass_handoff_node_inline(vm, node, frame, false /* is_feature */, false /* is_custom_context */);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip4_sv_reass_handoff_node_inline(vm, node, frame, false /* is_feature */, true /* is_custom_context */);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return ip4_sv_reass_handoff_node_inline(vm, node, frame, true /* is_feature */, false /* is_custom_context */);
}
...
int ip4_sv_reass_enable_disable_with_refcnt(u32 sw_if_index, int is_enable)
{
    ...
    if (is_enable) {
        if (!rm->feature_use_refcount_per_intf[sw_if_index]) {
            ...
            return vnet_feature_enable_disable("ip4-unicast", "ip4-sv-reassembly-feature", sw_if_index, 1, 0, 0);
        }
        ++rm->feature_use_refcount_per_intf[sw_if_index];
    }
    else {
        if (rm->feature_use_refcount_per_intf[sw_if_index]) --rm->feature_use_refcount_per_intf[sw_if_index];
        if (!rm->feature_use_refcount_per_intf[sw_if_index])
            return vnet_feature_enable_disable("ip4-unicast", "ip4-sv-reassembly-feature", sw_if_index, 0, 0, 0);
    }
    return 0;
}
...
{
    return vlib_node_add_next(vlib_get_main(), ip4_sv_reass_custom_node.index, node_index);
}

uword ip4_sv_reass_custom_context_register_next_node(uword node_index)
{
    return vlib_node_add_next(vlib_get_main(), ip4_sv_reass_custom_context_node.index, node_index);
}
...
int ip4_sv_reass_output_enable_disable_with_refcnt(u32 sw_if_index, int is_enable)
{
    ...
    if (is_enable) {
        if (!rm->output_feature_use_refcount_per_intf[sw_if_index]) {
            ...
            return vnet_feature_enable_disable("ip4-output", "ip4-sv-reassembly-output-feature", sw_if_index, 1, 0, 0);
        }
        ++rm->output_feature_use_refcount_per_intf[sw_if_index];
    }
    else {
        if (rm->output_feature_use_refcount_per_intf[sw_if_index]) --rm->output_feature_use_refcount_per_intf[sw_if_index];
        if (!rm->output_feature_use_refcount_per_intf[sw_if_index])
            return vnet_feature_enable_disable("ip4-output", "ip4-sv-reassembly-output-feature", sw_if_index, 0, 0, 0);
    }
    return 0;
}
#endif
