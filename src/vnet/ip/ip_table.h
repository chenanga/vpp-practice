...
#define _VNET_IP_TABLE_FUNCTION_DECL_PRIO(f, tag, p)                                                 \
                                                                                                     \
    static void __vnet_ip_table_function_init_##tag##_##f(void) __attribute__((__constructor__));    \
                                                                                                     \
    ...
    static void __vnet_ip_table_function_deinit_##tag##_##f(void) __attribute__((__destructor__));   \
                                                                                                     \
    ...
    {                                                                                                \
        ...
        if (vnm->tag##_functions[p]->fp == f) {                                                      \
            vnm->tag##_functions[p] = vnm->tag##_functions[p]->next_ip_table_function;               \
            return;                                                                                  \
        }                                                                                            \
        next = vnm->tag##_functions[p];                                                              \
        while (next->next_ip_table_function) {                                                       \
            if (next->next_ip_table_function->fp == f) {                                             \
                next->next_ip_table_function = next->next_ip_table_function->next_ip_table_function; \
                return;                                                                              \
            }                                                                                        \
...
#define _VNET_IP_TABLE_FUNCTION_DECL_PRIO(f, tag, p) static __clib_unused void *__clib_unused_##f = f;
#endif

#define _VNET_IP_TABLE_FUNCTION_DECL(f, tag) _VNET_IP_TABLE_FUNCTION_DECL_PRIO(f, tag, VNET_ITF_FUNC_PRIORITY_LOW)
