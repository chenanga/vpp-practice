...
uword unformat_ip_prefix(unformat_input_t *input, va_list *args)
{
    ...
    int plen;
    if (unformat(input, "%U/%d", unformat_ip_address, &ip_prefix_addr(a), &plen)) {
        ip_prefix_len(a) = plen;
        if ((ip_prefix_version(a) == AF_IP4 && 32 < ip_prefix_len(a)) || (ip_prefix_version(a) == AF_IP6 && 128 < ip_prefix_len(a))) {
...
bool ip_address_is_zero(const ip_address_t *ip)
{
    switch (ip_addr_version(ip)) {
        ...
        case AF_IP6:
            return (ip_addr_v6(ip).as_u64[0] == 0 && ip_addr_v6(ip).as_u64[1] == 0);
            break;
    }
    return false;
}
...
{
    int res = 0;
    if (ip_addr_version(ip1) != ip_addr_version(ip2)) return -1;
...
void ip_address_to_fib_prefix(const ip_address_t *addr, fib_prefix_t *prefix)
{
    if (addr->version == AF_IP4) {
        ...
        memcpy(&prefix->fp_addr.ip4, &addr->ip.ip4, sizeof(prefix->fp_addr.ip4));
    }
    else {
        prefix->fp_len   = 128;
        prefix->fp_proto = FIB_PROTOCOL_IP6;
        memcpy(&prefix->fp_addr.ip6, &addr->ip.ip6, sizeof(prefix->fp_addr.ip6));
    }
    prefix->___fp___pad = 0;
}

void ip_address_to_prefix(const ip_address_t *addr, ip_prefix_t *prefix)
{
...
{
    ip46_address_increment((ip_addr_version(ip) == AF_IP4 ? IP46_TYPE_IP4 : IP46_TYPE_IP6), &ip_addr_46(ip));
}
...
int ip_prefix_cmp(const ip_prefix_t *ipp1, const ip_prefix_t *ipp2)
{
    ...
    if (cmp == 0) {
        if (ip_prefix_len(&p1) < ip_prefix_len(&p2)) {
            ...
            if (ip_prefix_len(&p1) > ip_prefix_len(&p2)) cmp = 2;
        }
    }
    return cmp;
}
...
static bool ip4_prefix_validate(const ip_prefix_t *ip)
{
    ...
    if (ip_prefix_len(ip) > 32) return (false);
...
static bool ip6_prefix_validate(const ip_prefix_t *ip)
{
    ...
    if (ip_prefix_len(ip) > 128) return (false);

    ip6_addr = ip_prefix_v6(ip);
    ip6_preflen_to_mask(ip_prefix_len(ip), &ip6_mask);

    return (((ip6_addr.as_u64[0] & ip6_mask.as_u64[0]) == ip6_addr.as_u64[0]) && ((ip6_addr.as_u64[1] & ip6_mask.as_u64[1]) == ip6_addr.as_u64[1]));
}
...
void ip6_address_normalize(ip6_address_t *ip6, u8 preflen)
{
    ...
    if (preflen == 0) {
        ...
        ip6->as_u64[0] &= clib_host_to_net_u64(0xffffffffffffffffL << (64 - preflen));
        ip6->as_u64[1] = 0;
    }
    else
        ip6->as_u64[1] &= clib_host_to_net_u64(0xffffffffffffffffL << (128 - preflen));
}
...
void ip6_preflen_to_mask(u8 pref_len, ip6_address_t *mask)
{
    if (pref_len == 0) {
        ...
        mask->as_u64[0] = clib_host_to_net_u64(0xffffffffffffffffL << (64 - pref_len));
        mask->as_u64[1] = 0;
    }
    else {
        mask->as_u64[0] = 0xffffffffffffffffL;
        mask->as_u64[1] = clib_host_to_net_u64(0xffffffffffffffffL << (128 - pref_len));
    }
}
...
{
    if (mask->as_u64[1] != 0) return 128 - log2_first_set(clib_net_to_host_u64(mask->as_u64[1]));
    if (mask->as_u64[0] != 0) return 64 - log2_first_set(clib_net_to_host_u64(mask->as_u64[0]));
    return 0;
}
