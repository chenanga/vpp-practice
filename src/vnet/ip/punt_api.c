...
static int vl_api_punt_decode(const vl_api_punt_t *in, punt_reg_t *out)
{
    ...
    switch (out->type) {
        ...
        case PUNT_TYPE_EXCEPTION:
            return (vl_api_punt_exception_decode(&in->punt.exception, &out->punt.exception));
        case PUNT_TYPE_IP_PROTO:
            return (vl_api_punt_ip_proto_decode(&in->punt.ip_proto, &out->punt.ip_proto));
    }
...
static void vl_api_punt_socket_register_t_handler(vl_api_punt_socket_register_t *mp)
{
    ...
    error = vnet_punt_socket_add(vm, ntohl(mp->header_version), &pr, (char *) mp->pathname);
    if (error) {
        rv = -1;
        clib_error_report(error);
    }
    ...
    REPLY_MACRO2(VL_API_PUNT_SOCKET_REGISTER_REPLY, ({ memcpy((char *) rmp->pathname, p, sizeof(rmp->pathname)); }));
}
...
static void vl_api_punt_socket_dump_t_handler(vl_api_punt_socket_dump_t *mp)
{
    ...
    if (0 != vl_api_punt_type_decode(mp->type, &pt)) return;
...
static int punt_reason_dump_walk_cb(vlib_punt_reason_t id, const u8 *name, void *args)
{
    ...
    if (ctx->name) {
        ...
        if (vec_cmp(name, ctx->name)) /* not the reasonn we're lookgin for */
            return 1;
    }
