...
*/
always_inline uword udp46_punt_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, int is_ip4)
{
    ...
    while (n_left_from > 0) {
        ...
        vlib_get_next_frame(vm, node, punt_next_punt(is_ip4), to_next, n_left_to_next);
...
static u8 *format_udp_punt_trace(u8 *s, va_list *args)
{
    ...
    if (t->is_midchain) {
        s = format(s, "\n%U(buffer is part of chain)", format_white_space, indent);
    }
    s = format(s, "\n%U%U", format_white_space, indent, format_hex_bytes, t->packet_data, sizeof(t->packet_data));

    return s;
}

always_inline uword
punt_socket_inline2(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, punt_type_t pt, ip_address_family_t af, ip_protocol_t protocol)
{
    ...
    u32                 node_index = (AF_IP4 == af ? udp4_punt_socket_node.index : udp6_punt_socket_node.index);
    ...
    for (i = 0; i < n_packets; i++) {
        ...
        if (PREDICT_FALSE(NULL == c)) {
            vlib_node_increment_counter(vm, node_index, PUNT_ERROR_SOCKET_TX_ERROR, 1);
            goto error;
        }
        ...
        if (PREDICT_FALSE(b->flags & VLIB_BUFFER_IS_TRACED)) {
            ...
            clib_memcpy_fast(t->packet_data, vlib_buffer_get_current(b), sizeof(t->packet_data));
        }
...
always_inline uword punt_socket_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, punt_type_t pt, ip_address_family_t af)
{
...
static uword icmp6_punt_socket(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return punt_socket_inline2(vm, node, from_frame, PUNT_TYPE_L4, AF_IP6, IP_PROTOCOL_ICMP6);
}

static uword exception_punt_socket(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return punt_socket_inline(vm, node, from_frame, PUNT_TYPE_EXCEPTION, AF_IP4);
}
...
static u8 *format_punt_trace(u8 *s, va_list *va)
{
    ...
    s                               = format(s, "%U Action: %d", format_vnet_sw_if_index_name, vnm, t->sw_if_index, t->action);
    return s;
}
...
static uword punt_socket_rx_fd(vlib_main_t *vm, vlib_node_runtime_t *node, u32 fd)
{
    ...
    if (PREDICT_FALSE(n_trace > 0 && vlib_trace_buffer(vm, node, next_index, b, 1 /* follow_chain */))) {
        punt_trace_t *t;
    ...
    vlib_validate_buffer_enqueue_x1(vm, node, next, to_next, n_left_to_next, bi, next_index);
