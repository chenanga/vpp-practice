...
extern uword unformat_ip_address_family(unformat_input_t *input, va_list *args);
...
extern ip_address_family_t ip_address_family_from_fib_proto(fib_protocol_t fp);
...
extern uword unformat_ip_sub_address_family(unformat_input_t *input, va_list *args);
extern u8   *format_ip_sub_address_family(u8 *s, va_list *args);

#define FOR_EACH_IP_ADDRESS_SUB_FAMILY(_safi) for (_safi = SAFI_UNICAST; _safi <= SAFI_MULTICAST; _safi++)
...
#define IP_ADDRESS_V4_ALL_0S                  \
    {                                         \
        .ip.ip4.as_u32 = 0, .version = AF_IP4 \
    }
#define IP_ADDRESS_V6_ALL_0S                       \
    {                                              \
        .ip.ip6.as_u64 = {0, 0}, .version = AF_IP6 \
    }
...
extern int            ip_address_cmp(const ip_address_t *ip1, const ip_address_t *ip2);
...
extern void           ip_address_set(ip_address_t *dst, const void *src, ip_address_family_t version);
...
extern fib_protocol_t ip_address_to_46(const ip_address_t *addr, ip46_address_t *a);
extern void           ip_address_from_46(const ip46_address_t *a, fib_protocol_t fproto, ip_address_t *addr);
...
extern void  ip_address_to_fib_prefix(const ip_address_t *addr, fib_prefix_t *prefix);
extern void  ip_address_to_prefix(const ip_address_t *addr, ip_prefix_t *prefix);
extern void  ip_prefix_to_fib_prefix(const ip_prefix_t *ipp, fib_prefix_t *fibp);
...
extern void ip4_prefix_max_address_host_order(ip4_address_t *ip, u8 plen, ip4_address_t *res);
extern void ip6_prefix_max_address_host_order(ip6_address_t *ip, u8 plen, ip6_address_t *res);
