...
static_always_inline void ip_in_out_acl_inline_trace(vlib_main_t           *vm,
                                                     ...
                                                     const int              do_trace)
{
    ...
    if (n_left >= 2) {
        ...
        if (t[2]->current_data_flag == CLASSIFY_FLAG_USE_CURR_DATA)
            h[2] = (void *) vlib_buffer_get_current(b[0]) + t[2]->current_data_offset;
        else
            h[2] = b[0]->data;

        if (t[3]->current_data_flag == CLASSIFY_FLAG_USE_CURR_DATA)
            h[3] = (void *) vlib_buffer_get_current(b[1]) + t[3]->current_data_offset;
        else
            h[3] = b[1]->data;

        if (is_output) {
            /* Save the rewrite length, since we are using the l2_classify struct */
            vnet_buffer(b[0])->l2.l2_len = vnet_buffer(b[0])->ip.save_rewrite_length;
            /* advance the match pointer so the matching happens on IP header */
            h[2] += vnet_buffer(b[0])->l2.l2_len;

            /* Save the rewrite length, since we are using the l2_classify struct */
            vnet_buffer(b[1])->l2.l2_len = vnet_buffer(b[1])->ip.save_rewrite_length;
            /* advance the match pointer so the matching happens on IP header */
        ...
        vnet_classify_prefetch_bucket(t[2], vnet_buffer(b[0])->l2_classify.hash);
        vnet_classify_prefetch_bucket(t[3], vnet_buffer(b[1])->l2_classify.hash);
    }
    ...
    while (n_left >= 2) {
        ...
        if (n_left >= 4) {
            sw_if_index[2] = ~0 == way ? 0 : vnet_buffer(b[2])->sw_if_index[way];
            sw_if_index[3] = ~0 == way ? 0 : vnet_buffer(b[3])->sw_if_index[way];
            ...
            if (t[2]->current_data_flag == CLASSIFY_FLAG_USE_CURR_DATA)
                h[2] = (void *) vlib_buffer_get_current(b[2]) + t[2]->current_data_offset;
            else
                h[2] = b[2]->data;

            if (t[3]->current_data_flag == CLASSIFY_FLAG_USE_CURR_DATA)
                h[3] = (void *) vlib_buffer_get_current(b[3]) + t[3]->current_data_offset;
            else
                h[3] = b[3]->data;

            if (is_output) {
                /* Save the rewrite length, since we are using the l2_classify struct */
                vnet_buffer(b[2])->l2.l2_len = vnet_buffer(b[2])->ip.save_rewrite_length;
                /* advance the match pointer so the matching happens on IP header */
                h[2] += vnet_buffer(b[2])->l2.l2_len;

                /* Save the rewrite length, since we are using the l2_classify struct */
                vnet_buffer(b[3])->l2.l2_len = vnet_buffer(b[3])->ip.save_rewrite_length;
                /* advance the match pointer so the matching happens on IP header */
            ...
            vnet_classify_prefetch_bucket(t[2], vnet_buffer(b[2])->l2_classify.hash);
            vnet_classify_prefetch_bucket(t[3], vnet_buffer(b[3])->l2_classify.hash);
        }

        /* find entry for b[0] & b[1] */
        vnet_get_config_data(cm,
                             &b[0]->current_config_index,
                             &_next[0],
                             /* # bytes of config data */ 0);
        vnet_get_config_data(cm,
                             &b[1]->current_config_index,
                             &_next[1],
                             /* # bytes of config data */ 0);
        ...
        if (PREDICT_TRUE(table_index[0] != ~0)) {
            ...
            if (e[0]) {
                vnet_buffer(b[0])->l2_classify.opaque_index = e[0]->opaque_index;
                vlib_buffer_advance(b[0], e[0]->advance);

                _next[0] = (e[0]->next_index < n_next_nodes) ? e[0]->next_index : _next[0];

                hits++;

                b[0]->error = (_next[0] == ACL_NEXT_INDEX_DENY) ? error_deny : error_none;

                if (!is_output) {
                    if (e[0]->action == CLASSIFY_ACTION_SET_IP4_FIB_INDEX || e[0]->action == CLASSIFY_ACTION_SET_IP6_FIB_INDEX)
                        vnet_buffer(b[0])->sw_if_index[VLIB_TX] = e[0]->metadata;
                    else if (e[0]->action == CLASSIFY_ACTION_SET_METADATA) {
                        vnet_buffer(b[0])->ip.adj_index[VLIB_TX] = e[0]->metadata;
                ...
                while (1) {
                    ...
                    if (PREDICT_TRUE(table_index[0] != ~0))
                        ...
                        _next[0] = (t[0]->miss_next_index < n_next_nodes) ? t[0]->miss_next_index : _next[0];

                        misses++;

                        b[0]->error = (_next[0] == ACL_NEXT_INDEX_DENY) ? error_miss : error_none;
                        break;
                    }
                    ...
                    /* advance the match pointer so the matching happens on IP header */
                    if (is_output) h[0] += vnet_buffer(b[0])->l2.l2_len;
        ...
        if (PREDICT_TRUE(table_index[1] != ~0)) {
            e[1] = vnet_classify_find_entry_inline(t[1], (u8 *) h[1], hash[1], now);
            if (e[1]) {
                vnet_buffer(b[1])->l2_classify.opaque_index = e[1]->opaque_index;
                vlib_buffer_advance(b[1], e[1]->advance);

                _next[1] = (e[1]->next_index < n_next_nodes) ? e[1]->next_index : _next[1];

                hits++;

                b[1]->error = (_next[1] == ACL_NEXT_INDEX_DENY) ? error_deny : error_none;

                if (!is_output) {
                    if (e[1]->action == CLASSIFY_ACTION_SET_IP4_FIB_INDEX || e[1]->action == CLASSIFY_ACTION_SET_IP6_FIB_INDEX)
                ...
                while (1) {
                    ...
                    if (PREDICT_TRUE(table_index[1] != ~0))
                        ...
                        _next[1] = (t[1]->miss_next_index < n_next_nodes) ? t[1]->miss_next_index : _next[1];

                        misses++;

                        b[1]->error = (_next[1] == ACL_NEXT_INDEX_DENY) ? error_miss : error_none;
                        break;
                    }

                    if (t[1]->current_data_flag == CLASSIFY_FLAG_USE_CURR_DATA)
                        h[1] = (void *) vlib_buffer_get_current(b[1]) + t[1]->current_data_offset;
                    else
                        h[1] = b[1]->data;

                    /* advance the match pointer so the matching happens on IP header */
                    if (is_output) h[1] += vnet_buffer(b[1])->l2.l2_len;

                    hash[1] = vnet_classify_hash_packet_inline(t[1], (u8 *) h[1]);
                    e[1]    = vnet_classify_find_entry_inline(t[1], (u8 *) h[1], hash[1], now);
                    if (e[1]) {
                        vnet_buffer(b[1])->l2_classify.opaque_index = e[1]->opaque_index;
                        vlib_buffer_advance(b[1], e[1]->advance);
        ...
        if (do_trace && b[0]->flags & VLIB_BUFFER_IS_TRACED) {
            ip_in_out_acl_trace_t *_t = vlib_add_trace(vm, node, b[0], sizeof(*_t));
            _t->sw_if_index           = ~0 == way ? 0 : vnet_buffer(b[0])->sw_if_index[way];
            _t->next_index            = _next[0];
            _t->table_index           = table_index[0];
            _t->offset                = (e[0] && t[0]) ? vnet_classify_get_offset(t[0], e[0]) : ~0;
        }

        if (do_trace && b[1]->flags & VLIB_BUFFER_IS_TRACED) {
            ip_in_out_acl_trace_t *_t = vlib_add_trace(vm, node, b[1], sizeof(*_t));
            _t->sw_if_index           = ~0 == way ? 0 : vnet_buffer(b[1])->sw_if_index[way];
            _t->next_index            = _next[1];
            _t->table_index           = table_index[1];
            _t->offset                = (e[1] && t[1]) ? vnet_classify_get_offset(t[1], e[1]) : ~0;
        }
    ...
    while (n_left > 0) {
        ...
        if (t0->current_data_flag == CLASSIFY_FLAG_USE_CURR_DATA)
            h0 = (void *) vlib_buffer_get_current(b[0]) + t0->current_data_offset;
        else
            h0 = b[0]->data;
        ...
        vnet_buffer(b[0])->l2_classify.hash = vnet_classify_hash_packet(t0, (u8 *) h0);
        ...
        if (PREDICT_TRUE(table_index0 != ~0)) {
            ...
            /* advance the match pointer so the matching happens on IP header */
            if (is_output) h0 += vnet_buffer(b[0])->l2.l2_len;
            ...
            if (e0) {
                ...
                next0 = (e0->next_index < n_next_nodes) ? e0->next_index : next0;

                hits++;

                b[0]->error = (next0 == ACL_NEXT_INDEX_DENY) ? error_deny : error_none;

                if (!is_output) {
                    if (e0->action == CLASSIFY_ACTION_SET_IP4_FIB_INDEX || e0->action == CLASSIFY_ACTION_SET_IP6_FIB_INDEX)
                ...
                while (1) {
                    ...
                    if (PREDICT_TRUE(table_index0 != ~0))
                        ...
                        next0 = (t0->miss_next_index < n_next_nodes) ? t0->miss_next_index : next0;

                        misses++;

                        b[0]->error = (next0 == ACL_NEXT_INDEX_DENY) ? error_miss : error_none;
                        break;
                    }
        ...
        if (do_trace && b[0]->flags & VLIB_BUFFER_IS_TRACED) {
            ip_in_out_acl_trace_t *t = vlib_add_trace(vm, node, b[0], sizeof(*t));
            t->sw_if_index           = ~0 == way ? 0 : vnet_buffer(b[0])->sw_if_index[way];
            t->next_index            = next0;
...
static_always_inline uword ip_in_out_acl_inline(vlib_main_t                    *vm,
                                                vlib_node_runtime_t            *node,
                                                vlib_frame_t                   *frame,
                                                const in_out_acl_table_id_t     tid,
                                                u32                            *fib_index_by_sw_if_index,
                                                const vlib_node_registration_t *parent_error_node,
                                                const u32                       error_none_index,
                                                const u32                       error_deny_index,
                                                const u32                       error_miss_index,
                                                const vlib_rx_or_tx_t           way,
                                                const int                       is_output)
{
    ...
    const u32                 *table_index_by_sw_if_index = am->classify_table_index_by_sw_if_index[is_output][tid];
    vnet_config_main_t        *cm                         = am->vnet_config_main[is_output][tid];
    const vlib_node_runtime_t *error_node                 = vlib_node_get_runtime(vm, parent_error_node->index);
...
#define ip_in_out_acl_inline_trace__(do_trace)             \
    ...
    vlib_node_increment_counter(vm, node->node_index, is_output ? IP_OUTACL_ERROR_MISS : IP_INACL_ERROR_MISS, misses);
    vlib_node_increment_counter(vm, node->node_index, is_output ? IP_OUTACL_ERROR_HIT : IP_INACL_ERROR_HIT, hits);
    vlib_node_increment_counter(vm, node->node_index, is_output ? IP_OUTACL_ERROR_CHAIN_HIT : IP_INACL_ERROR_CHAIN_HIT, chain_hits);
...
{
    return ip_in_out_acl_inline(vm,
                                node,
                                frame,
