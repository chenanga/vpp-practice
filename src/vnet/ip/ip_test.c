...
static int api_ip_route_add_del_v2(vat_main_t *vam)
{
    return -1;
}

static void set_ip4_address(vl_api_address_t *a, u32 v)
{
    if (a->af == ADDRESS_IP4) {
...
static void increment_v4_address(vl_api_ip4_address_t *i)
{
...
static void increment_v6_address(vl_api_ip6_address_t *i)
{
...
static void increment_address(vl_api_address_t *a)
{
    if (a->af == ADDRESS_IP4)
...
static uword unformat_fib_path(unformat_input_t *input, va_list *args)
{
...
static int api_ip_route_add_del(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "%U", unformat_vl_api_prefix, &pfx))
            ...
            ;
        else if (unformat(i, "via %U", unformat_fib_path, vam, &paths[path_count])) {
            path_count++;
            if (8 == path_count) {
                errmsg("max 8 paths");
                return -99;
    ...
    if (count > 1) {
        ...
        while (vat_time_now(vam) < timeout)
            if (vam->result_ready == 1) goto out;
        vam->retval = -99;

    out:
        if (vam->retval == -99) errmsg("timeout");
        ...
        print(vam->ofp, "%d routes in %.6f secs, %.2f routes/sec", count, after - before, count / (after - before));
    }
    else {
        int ret;
...
static int api_ip_table_add_del(vat_main_t *vam)
{
    unformat_input_t          *i = vam->input;
...
static int api_ip_table_add_del_v2(vat_main_t *vam)
{
    unformat_input_t             *i = vam->input;
...
static int api_ip_table_replace_begin(vat_main_t *vam)
{
    unformat_input_t                *i = vam->input;
...
static int api_ip_table_flush(vat_main_t *vam)
{
    unformat_input_t        *i = vam->input;
...
static int api_ip_table_allocate(vat_main_t *vam)
{
    return -1;
}

static void vl_api_ip_table_allocate_reply_t_handler(vl_api_ip_table_allocate_reply_t *mp)
{
}

static void vl_api_ip_route_add_del_v2_reply_t_handler(vl_api_ip_route_add_del_v2_reply_t *mp)
{
}

static void vl_api_ip_route_details_t_handler(vl_api_ip_route_details_t *mp)
{
}

static void vl_api_ip_route_v2_details_t_handler(vl_api_ip_route_v2_details_t *mp)
{
}

static void vl_api_ip_route_add_del_reply_t_handler(vl_api_ip_route_add_del_reply_t *mp)
{
    vat_main_t *vam   = ip_test_main.vat_main;
    vam->result_ready = 1;
}

static void vl_api_ip_route_lookup_reply_t_handler(vl_api_ip_route_lookup_reply_t *mp)
{
}

static void vl_api_ip_route_lookup_v2_reply_t_handler(vl_api_ip_route_lookup_v2_reply_t *mp)
{
}

static int api_set_ip_flow_hash_router_id(vat_main_t *vat)
{
    return -1;
}

static int api_ip_route_lookup(vat_main_t *vat)
{
    return -1;
}

static int api_ip_route_lookup_v2(vat_main_t *vat)
{
    return -1;
}
...
static int api_mfib_signal_dump(vat_main_t *vat)
{
    return -1;
}

static int api_ip_punt_police(vat_main_t *vat)
{
    return -1;
}

static int api_ip_punt_redirect(vat_main_t *vat)
{
    return -1;
}

static int api_add_del_ip_punt_redirect_v2(vat_main_t *vat)
{
    return -1;
}

static int api_ip_punt_redirect_dump(vat_main_t *vat)
{
    return -1;
}

static void vl_api_ip_punt_redirect_details_t_handler(vl_api_ip_punt_redirect_details_t *mp)
{
    /**/
}

static int api_ip_punt_redirect_v2_dump(vat_main_t *vat)
{
    return -1;
}

static void vl_api_ip_punt_redirect_v2_details_t_handler(vl_api_ip_punt_redirect_v2_details_t *mp)
{
    /**/
}

static int api_ip_address_dump(vat_main_t *vam)
{
    unformat_input_t         *i = vam->input;
...
static void vl_api_sw_interface_ip6_get_link_local_address_reply_t_handler(vl_api_sw_interface_ip6_get_link_local_address_reply_t *mp)
{
}
...
static int api_sw_interface_ip6_get_link_local_address(vat_main_t *vam)
{
    return -1;
}

static int api_ip_path_mtu_replace_end(vat_main_t *vam)
{
    return -1;
}

static int api_ioam_enable(vat_main_t *vam)
{
...
static int api_ip_reassembly_get(vat_main_t *vam)
{
    return -1;
}

static int api_ip_path_mtu_replace_begin(vat_main_t *vam)
{
    return -1;
}

static int api_ip_path_mtu_update(vat_main_t *vam)
{
    return -1;
}

static int api_ioam_disable(vat_main_t *vam)
{
...
static int api_ip_source_and_port_range_check_add_del(vat_main_t *vam)
{
    unformat_input_t                                *input = vam->input;
    ...
    if (vrf_id == 0) {
        errmsg(
            "VRF ID should not be default. Should be distinct VRF for this "
            "purpose.");
        return -99;
    }
...
static int api_ip_reassembly_set(vat_main_t *vat)
{
    return -1;
}

static int api_ip_container_proxy_add_del(vat_main_t *vam)
{
...
static int api_ip_reassembly_enable_disable(vat_main_t *vat)
{
    return -1;
}

static int api_ip_local_reass_enable_disable(vat_main_t *vat)
{
    return -1;
}

static int api_ip_local_reass_get(vat_main_t *vat)
{
    return -1;
}

static void vl_api_ip_local_reass_get_reply_t_handler(vl_api_ip_local_reass_get_reply_t *mp)
{
}

static void vl_api_ip_reassembly_get_reply_t_handler(vl_api_ip_reassembly_get_reply_t *mp)
{
}
...
int api_ip_source_and_port_range_check_interface_add_del(vat_main_t *vam)
{
    ...
    if (tcp_out_vrf_id == 0 || udp_out_vrf_id == 0 || tcp_in_vrf_id == 0 || udp_in_vrf_id == 0) {
        errmsg(
            "VRF ID should not be default. Should be distinct VRF for this "
            "purpose.");
        return -99;
...
static void vl_api_ip_container_proxy_details_t_handler(vl_api_ip_container_proxy_details_t *mp)
{
}

static int api_ip_container_proxy_dump(vat_main_t *vam)
{
    return -1;
}

static int api_ip_dump(vat_main_t *vam)
{
    vl_api_ip_dump_t      *mp;
    vl_api_control_ping_t *mp_ping;
...
static void vl_api_mfib_signal_details_t_handler(vl_api_mfib_signal_details_t *mp)
{
}

static void vl_api_ip_mroute_details_t_handler(vl_api_ip_mroute_details_t *mp)
{
    vat_main_t *vam   = ip_test_main.vat_main;
    vam->result_ready = 1;
}

static int api_ip_mroute_dump(vat_main_t *vam)
{
    unformat_input_t        *input = vam->input;
    vl_api_control_ping_t   *mp_ping;
...
static int api_set_ip_flow_hash_v2(vat_main_t *vat)
{
    return -1;
}

static int api_set_ip_flow_hash_v3(vat_main_t *vat)
{
    return -1;
}
...
static void vl_api_ip_mroute_add_del_reply_t_handler(vl_api_ip_mroute_add_del_reply_t *mp)
{
    vat_main_t *vam   = ip_test_main.vat_main;
    vam->result_ready = 1;
}

static int api_ip_mtable_dump(vat_main_t *vam)
{
...
static void vl_api_ip_mtable_details_t_handler(vl_api_ip_mtable_details_t *mp)
{
    vat_main_t *vam   = ip_test_main.vat_main;
    vam->result_ready = 1;
}

static int api_ip_table_replace_end(vat_main_t *vam)
{
    unformat_input_t              *i = vam->input;
...
static int api_ip_table_dump(vat_main_t *vam)
{
...
static void vl_api_ip_table_details_t_handler(vl_api_ip_table_details_t *mp)
{
    vat_main_t *vam = ip_test_main.vat_main;

    fformat(vam->ofp, "%s; table-id %d, prefix %U/%d", mp->table.name, ntohl(mp->table.table_id));
    vam->result_ready = 1;
}

static int api_ip_path_mtu_get(vat_main_t *vat)
{
    return -1;
}

static int api_ip_route_v2_dump(vat_main_t *vat)
{
    return -1;
}

static void vl_api_ip_path_mtu_get_reply_t_handler(vl_api_ip_path_mtu_get_reply_t *mp)
{
}

static int api_ip_route_dump(vat_main_t *vam)
{
    unformat_input_t       *input = vam->input;
...
static void vl_api_ip_address_details_t_handler(vl_api_ip_address_details_t *mp)
{
    ...
    if (!details || vam->current_sw_if_index >= vec_len(details) || !details[vam->current_sw_if_index].present) {
...
#define addresses (current_ip_details->addr)
    ...
    vec_validate_init_empty(addresses, vec_len(addresses), empty_ip_address_details);
...
static int api_ip_unnumbered_dump(vat_main_t *vam)
{
    return -1;
}

static void vl_api_ip_unnumbered_details_t_handler(vl_api_ip_unnumbered_details_t *mp)
{
}
...
static void vl_api_ip_details_t_handler(vl_api_ip_details_t *mp)
{
    ...
    vec_validate_init_empty(vam->ip_details_by_sw_if_index[vam->is_ipv6], sw_if_index, empty_ip_details);

    ip = vec_elt_at_index(vam->ip_details_by_sw_if_index[vam->is_ipv6], sw_if_index);
