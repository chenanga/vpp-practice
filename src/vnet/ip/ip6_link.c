    ...
    {                                                      \
        if (NULL != _il) {                                 \
            vec_foreach (_ild, _il->il_delegates) {        \
                if (ip6_link_delegate_is_init(_ild)) body; \
            }                                              \
        }                                                  \
    }
...
static ip6_link_t *ip6_link_get(u32 sw_if_index)
{
    ...
    if (sw_if_index >= vec_len(ip6_links)) return (NULL);

    il = &ip6_links[sw_if_index];

    if (!ip6_link_is_enabled_i(il)) return (NULL);
...
int ip6_link_enable(u32 sw_if_index, const ip6_address_t *link_local_addr)
{
    ...
    if (NULL == il) {
        ...
        IP6_LINK_INFO("enable: %U", format_vnet_sw_if_index_name, vnm, sw_if_index);
        ...
        if (sw_sup->type == VNET_SW_INTERFACE_TYPE_HARDWARE) eth = ethernet_get_interface(&ethernet_main, sw_sup->hw_if_index);

        /* use a user provided LL address if given */
        if (NULL != link_local_addr) ip6_address_copy(&il->il_ll_addr, link_local_addr);

        /* generate from ethernet MAC */
        if (ip6_address_is_zero(&il->il_ll_addr) && NULL != eth) ip6_link_local_address_from_mac(&il->il_ll_addr, eth->address.mac.bytes);

        /* choose a random address */
        if (ip6_address_is_zero(&il->il_ll_addr)) {
            il->il_ll_addr.as_u64[0] = clib_host_to_net_u64(0xFE80000000000000ULL);
         ...
         */
        if (NULL != eth) il->il_mcast_adj = adj_mcast_add_or_lock(FIB_PROTOCOL_IP6, VNET_LINK_IP6, sw_if_index);

        /* inform all register clients */
        ip6_link_delegate_id_t id;
        FOREACH_IP6_LINK_DELEGATE_ID(id)
        {
            if (NULL != il_delegate_vfts[id].ildv_enable) il_delegate_vfts[id].ildv_enable(il->il_sw_if_index);
        }
...
static void ip6_link_delegate_flush(ip6_link_t *il)
{
    ...
    FOREACH_IP6_LINK_DELEGATE(ild, il, ({ il_delegate_vfts[ild->ild_type].ildv_disable(ild->ild_index); }));
...
static void ip6_link_last_lock_gone(ip6_link_t *il)
{
    ...
    IP6_LINK_INFO("last-lock: %U", format_vnet_sw_if_index_name, vnet_get_main(), il->il_sw_if_index);
...
static void ip6_link_unlock(ip6_link_t *il)
{
    ...
    if (0 == il->il_locks) ip6_link_last_lock_gone(il);
}
...
int ip6_link_disable(u32 sw_if_index)
{
    ...
    if (NULL == il) return (VNET_API_ERROR_IP6_NOT_ENABLED);

    IP6_LINK_INFO("disable: %U", format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index);
...
adj_index_t ip6_link_get_mcast_adj(u32 sw_if_index)
{
    ...
    if (NULL == il) return (INDEX_INVALID);
...
int ip6_link_set_local_address(u32 sw_if_index, const ip6_address_t *address)
{
    ...
    if (NULL == il) return ip6_link_enable(sw_if_index, address);
    ...
    IP6_LINK_INFO("set-ll: %U -> %U", format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index, format_ip6_address, address);
    ...
    FOREACH_IP6_LINK_DELEGATE(ild, il, ({
                                  if (NULL != il_delegate_vfts[ild->ild_type].ildv_ll_change)
                                      il_delegate_vfts[ild->ild_type].ildv_ll_change(ild->ild_index, &il->il_ll_addr);
                              }));
...
void ip6_link_delegate_remove(u32 sw_if_index, ip6_link_delegate_id_t id, index_t ii)
{
    ...
    if (NULL != il) {
        if (vec_len(il->il_delegates) > id) {
            clib_memcpy(&il->il_delegates[id], &ip6_link_delegate_uninit, sizeof(il->il_delegates[0]));
        }
    }
}
    ...
    FOREACH_IP6_LINK_DELEGATE(ild, il, ({
                                  if (is_delete) {
                                      if (NULL != il_delegate_vfts[ild->ild_type].ildv_addr_del)
                                          il_delegate_vfts[ild->ild_type].ildv_addr_del(ild->ild_index, address, address_length);
                                  }
                                  else {
                                      if (NULL != il_delegate_vfts[ild->ild_type].ildv_addr_add)
                                          il_delegate_vfts[ild->ild_type].ildv_addr_add(ild->ild_index, address, address_length);
                                  }
                              }));
}
...
static clib_error_t *ip6_link_interface_add_del(vnet_main_t *vnm, u32 sw_if_index, u32 is_add)
{
    if (!is_add) {
        ...
        IP6_LINK_DBG("link-del: %U", format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index);

        if (NULL != il) /* force cleanup */
...
static clib_error_t *test_ip6_link_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (unformat(input, "%U", unformat_ethernet_address, mac)) {
        ...
        vlib_cli_output(vm, "Original MAC address: %U", format_ethernet_address, mac);
    }
...
static u8 *ip6_print_addrs(u8 *s, u32 *addrs)
{
    ...
    for (i = 0; i < vec_len(addrs); i++) {
        ...
        s = format(s, "%U%U/%d\n", format_white_space, 4, format_ip6_address, address, a->address_length);
    }
...
static u8 *format_ip6_link(u8 *s, va_list *arg)
{
    ...
    if (!ip6_link_is_enabled_i(il)) return (s);
    ...
    vec_validate_init_empty(lm->if_address_pool_index_by_sw_if_index, il->il_sw_if_index, ~0);
    ...
    FOREACH_IP6_LINK_DELEGATE(ild, il, ({ s = format(s, "%U", il_delegate_vfts[ild->ild_type].ildv_format, ild->ild_index, 2); }));
...
static clib_error_t *enable_ip6_interface_cmd(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (unformat_user(input, unformat_vnet_sw_interface, vnm, &sw_if_index)) {
        if (ip6_link_enable(sw_if_index, NULL)) error = clib_error_return(0, "Failed\n");
    }
    else {
...
static clib_error_t *disable_ip6_interface_cmd(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (unformat_user(input, unformat_vnet_sw_interface, vnm, &sw_if_index)) {
        if (ip6_link_disable(sw_if_index)) error = clib_error_return(0, "Failed\n");
    }
    else {
        error = clib_error_return(0, "unknown interface\n'", format_unformat_error, input);
    }
