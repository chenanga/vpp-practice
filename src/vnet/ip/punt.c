...
static void punt_client_l4_db_add(ip_address_family_t af, u16 port, u32 index)
{
    ...
    pm->db.clients_by_l4_port = hash_set(pm->db.clients_by_l4_port, punt_client_l4_mk_key(af, port), index);
}
...
static void punt_client_ip_proto_db_add(ip_address_family_t af, ip_protocol_t proto, u32 index)
{
    ...
    pm->db.clients_by_ip_proto = hash_set(pm->db.clients_by_ip_proto, punt_client_ip_proto_mk_key(af, proto), index);
}
...
static clib_error_t *punt_socket_register_l4(vlib_main_t *vm, ip_address_family_t af, u8 protocol, u16 port, char *client_pathname)
{
    ...
    if (port == (u16) ~0) return clib_error_return(0, "Port number required");

    u32 node_index;
    switch (protocol) {
        case IP_PROTOCOL_UDP:
            node_index = (af == AF_IP4 ? udp4_punt_socket_node.index : udp6_punt_socket_node.index);
            udp_register_dst_port(vm, port, node_index, af == AF_IP4);
            break;
        case IP_PROTOCOL_ICMP6:
            if (af != AF_IP6)
                return clib_error_return(0, "only UDP or ICMP6 protocol (%d, %d) is supported, got %d", IP_PROTOCOL_UDP, IP_PROTOCOL_ICMP6, protocol);

            node_index = icmp6_punt_socket_node.index;
            icmp6_register_type(vm, port, node_index);
            break;
        default:
            return clib_error_return(0, "only UDP or ICMP6 protocol (%d) is supported, got %d", IP_PROTOCOL_UDP, protocol);
    }
    ...
    snprintf(c->caddr.sun_path, sizeof(c->caddr.sun_path), "%s", client_pathname);
...
static clib_error_t *punt_socket_register_exception(vlib_main_t *vm, vlib_punt_reason_t reason, char *client_pathname)
{
    ...
    snprintf(pc->caddr.sun_path, sizeof(pc->caddr.sun_path), "%s", client_pathname);
    pc->caddr.sun_family          = AF_UNIX;
    pc->reg.type                  = PUNT_TYPE_EXCEPTION;
    pc->reg.punt.exception.reason = reason;

    vlib_punt_register(pm->hdl, pc->reg.punt.exception.reason, "exception-punt-socket");
...
static clib_error_t *punt_socket_unregister_l4(ip_address_family_t af, ip_protocol_t protocol, u16 port)
{
    ...
    if (~0 != pci) pool_put_index(punt_main.punt_client_pool, pci);
...
clib_error_t *vnet_punt_socket_add(vlib_main_t *vm, u32 header_version, const punt_reg_t *pr, char *client_pathname)
{
    ...
    if (!pm->is_configured) return clib_error_return(0, "socket is not configured");

    if (header_version != PUNT_PACKETDESC_VERSION) return clib_error_return(0, "Invalid packet descriptor version");

    if (strncmp(client_pathname, vnet_punt_get_server_pathname(), UNIX_PATH_MAX) == 0)
        return clib_error_return(0, "Punt socket: Invalid client path: %s", client_pathname);

    /* Register client */
    switch (pr->type) {
        case PUNT_TYPE_L4:
            return (punt_socket_register_l4(vm, pr->punt.l4.af, pr->punt.l4.protocol, pr->punt.l4.port, client_pathname));
        case PUNT_TYPE_IP_PROTO:
            return (punt_socket_register_ip_proto(vm, pr->punt.ip_proto.af, pr->punt.ip_proto.protocol, client_pathname));
        case PUNT_TYPE_EXCEPTION:
            return (punt_socket_register_exception(vm, pr->punt.exception.reason, client_pathname));
    }
...
clib_error_t *vnet_punt_socket_del(vlib_main_t *vm, const punt_reg_t *pr)
{
    ...
    switch (pr->type) {
        case PUNT_TYPE_L4:
            return (punt_socket_unregister_l4(pr->punt.l4.af, pr->punt.l4.protocol, pr->punt.l4.port));
        case PUNT_TYPE_IP_PROTO:
            return (punt_socket_unregister_ip_proto(pr->punt.ip_proto.af, pr->punt.ip_proto.protocol));
        case PUNT_TYPE_EXCEPTION:
...
static clib_error_t *punt_l4_add_del(vlib_main_t *vm, ip_address_family_t af, ip_protocol_t protocol, u16 port, bool is_add)
{
    ...
    if (protocol != IP_PROTOCOL_UDP && protocol != IP_PROTOCOL_TCP)
        return clib_error_return(0, "only UDP (%d) and TCP (%d) protocols are supported, got %d", IP_PROTOCOL_UDP, IP_PROTOCOL_TCP, protocol);
    ...
    if (port == (u16) ~0) {
        ...
        const vlib_node_registration_t *punt_node = is_ip4 ? &udp4_punt_node : &udp6_punt_node;

        if (protocol == IP_PROTOCOL_TCP) return clib_error_return(0, "punt TCP ports is not supported yet");
...
static clib_error_t *punt_exception_add_del(vlib_punt_reason_t reason, bool is_add)
{
    ...
    const char             *node_name = vnet_punt_reason_flag_is_IP6_PACKET(flag) ? "ip6-punt" : "ip4-punt";
    if (is_add)
    ...
    if (!rv)
        ...
        return clib_error_return(0, is_add ? "Existing punting registration..." : "Punting registration not found...");
}
...
clib_error_t *vnet_punt_add_del(vlib_main_t *vm, const punt_reg_t *pr, bool is_add)
{
    switch (pr->type) {
        case PUNT_TYPE_L4:
            return (punt_l4_add_del(vm, pr->punt.l4.af, pr->punt.l4.protocol, pr->punt.l4.port, is_add));
        case PUNT_TYPE_EXCEPTION:
...
static clib_error_t *punt_cli(vlib_main_t *vm, unformat_input_t *input__, vlib_cli_command_t *cmd)
{
    ...
    if (!unformat_user(input__, unformat_line_input, input)) return 0;

    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "del"))
            is_add = false;
        else if (unformat(input, "reason %U", unformat_punt_reason, &pr.punt.exception.reason))
...
void punt_client_walk(punt_type_t pt, punt_client_walk_cb_t cb, void *ctx)
{
    ...
    switch (pt) {
        case PUNT_TYPE_L4: {
            ...
            hash_foreach(key, pci, pm->db.clients_by_l4_port, ({ cb(pool_elt_at_index(pm->punt_client_pool, pci), ctx); }));
            break;
        }
        case PUNT_TYPE_IP_PROTO: {
            u32 pci, key;

            hash_foreach(key, pci, pm->db.clients_by_ip_proto, ({ cb(pool_elt_at_index(pm->punt_client_pool, pci), ctx); }));
            break;
        }
        case PUNT_TYPE_EXCEPTION: {
            u32 *pci;
            ...
            vec_foreach (pci, pm->db.clients_by_exception) {
                if (~0 != *pci) cb(pool_elt_at_index(pm->punt_client_pool, *pci), ctx);
            }
    ...
    switch (pc->reg.type) {
        case PUNT_TYPE_L4:
            s = format(s,
                       ...
                       pc->reg.punt.l4.af,
                       format_ip_protocol,
                       pc->reg.punt.l4.protocol,
                       pc->reg.punt.l4.port);
            break;
        case PUNT_TYPE_IP_PROTO:
            s = format(s, "%U %U", format_ip_address_family, pc->reg.punt.ip_proto.af, format_ip_protocol, pc->reg.punt.ip_proto.protocol);
            break;
        case PUNT_TYPE_EXCEPTION:
            s = format(s, " %U", format_vlib_punt_reason, pc->reg.punt.exception.reason);
            break;
    }
...
clib_error_t *ip_punt_init(vlib_main_t *vm)
{
    ...
    pm->interface_output_node = vlib_get_node_by_name(vm, (u8 *) "interface-output");

    if ((error = vlib_call_init_function(vm, punt_init))) return error;

    pm->hdl = vlib_punt_client_register("ip-punt");

    vec_validate_aligned(pm->thread_data, tm->n_vlib_mains, CLIB_CACHE_LINE_BYTES);
...
#define _(pos, len, value, name, str) \
    if (vnet_punt_reason_flag_is_##name(flag)) s = format(s, "%s ", str);
...
static clib_error_t *punt_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    if (setsockopt(pm->socket_fd, SOL_SOCKET, SO_SNDBUF, &n_bytes, sizeof(n_bytes)) == -1) {
