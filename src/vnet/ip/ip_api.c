...
static void vl_api_sw_interface_ip6_enable_disable_t_handler(vl_api_sw_interface_ip6_enable_disable_t *mp)
{
    ...
    rv = ((mp->enable == 1) ? ip6_link_enable(ntohl(mp->sw_if_index), NULL) : ip6_link_disable(ntohl(mp->sw_if_index)));
...
static void send_ip_table_details(vpe_api_main_t *am, vl_api_registration_t *reg, u32 context, const fib_table_t *table)
{
    ...
    memcpy(mp->table.name, table->ft_desc, clib_min(vec_len(table->ft_desc), sizeof(mp->table.name)));
...
static void vl_api_ip_table_dump_t_handler(vl_api_ip_table_dump_t *mp)
{
    ...
    pool_foreach (fib_table, ip6_main.fibs) {
        ...
        if (fib_table->ft_flags & FIB_TABLE_FLAG_IP6_LL) continue;
        send_ip_table_details(am, reg, mp->context, fib_table);
    }
}
...
static void send_ip_route_details(vpe_api_main_t *am, vl_api_registration_t *reg, u32 context, fib_node_index_t fib_entry_index)
{
    ...
    mp->route.table_id    = htonl(fib_table_get_table_id(fib_entry_get_fib_index(fib_entry_index), pfx->fp_proto));
    mp->route.n_paths     = path_count;
    mp->route.stats_index = htonl(fib_table_entry_get_stats_index(fib_entry_get_fib_index(fib_entry_index), pfx));
...
static void vl_api_ip_route_dump_t_handler(vl_api_ip_route_dump_t *mp)
{
    ...
    if (INDEX_INVALID == fib_index) return;
...
static void vl_api_ip_route_v2_dump_t_handler(vl_api_ip_route_v2_dump_t *mp)
{
    ...
    if (src)
        fib_table_walk_w_src(fib_index, fproto, src, vl_api_ip_fib_dump_walk, &ctx);
    else
        fib_table_walk(fib_index, fproto, vl_api_ip_fib_dump_walk, &ctx);
...
static void send_ip_mroute_details(vpe_api_main_t *am, vl_api_registration_t *reg, u32 context, fib_node_index_t mfib_entry_index)
{
    ...
    mp->route.table_id = htonl(mfib_table_get_table_id(mfib_entry_get_fib_index(mfib_entry_index), pfx->fp_proto));
    mp->route.n_paths  = path_count;
    fp                 = mp->route.paths;
    vec_foreach (rpath, rpaths) {
...
static void vl_api_ip_punt_redirect_t_handler(vl_api_ip_punt_redirect_t *mp, vlib_main_t *vm)
{
    ...
    if (!vnet_sw_if_index_is_api_valid(ntohl(mp->punt.tx_sw_if_index))) goto bad_sw_if_index;
...
static clib_error_t *call_elf_section_ip_table_callbacks(vnet_main_t *vnm, u32 table_id, u32 flags, _vnet_ip_table_function_list_elt_t **elts)
{
    ...
    for (prio = VNET_IP_TABLE_FUNC_PRIORITY_LOW; prio <= VNET_IP_TABLE_FUNC_PRIORITY_HIGH; prio++) {
        elt = elts[prio];
...
void ip_table_delete(fib_protocol_t fproto, u32 table_id, u8 is_api)
{
    ...
    if (0 != table_id) {
        ...
        if ((~0 != fib_index) || (~0 != mfib_index))
            call_elf_section_ip_table_callbacks(vnm, table_id, 0 /* is_add */, vnm->ip_table_add_del_functions);

        if (~0 != fib_index) {
            fib_table_unlock(fib_index, fproto, (is_api ? FIB_SOURCE_API : FIB_SOURCE_CLI));
        }
        if (~0 != mfib_index) {
            mfib_table_unlock(mfib_index, fproto, (is_api ? MFIB_SOURCE_API : MFIB_SOURCE_CLI));
        }
    }
}
...
u32 ip_table_get_unused_id(fib_protocol_t fproto)
{
    ...
    for (j = 0; j < 1 << 10; j++) {
        ...
        for (i = 0; i < 1 << 10; i++) {
            ...
            seed += (2 * (i % 2) - 1) * i;
            if (seed == ~0) continue;
            if (fib_table_find(fproto, seed) == ~0) return seed;
        }
    }
...
void vl_api_ip_table_add_del_t_handler(vl_api_ip_table_add_del_t *mp)
{
    ...
    fib_protocol_t                   fproto   = (mp->table.is_ip6 ? FIB_PROTOCOL_IP6 : FIB_PROTOCOL_IP4);
    u32                              table_id = ntohl(mp->table.table_id);
    int                              rv       = 0;

    if (mp->is_add) {
        ip_table_create(fproto, table_id, 1 /* is_api */, 1 /* create_mfib */, mp->table.name);
    }
    else {
...
void vl_api_ip_table_add_del_v2_t_handler(vl_api_ip_table_add_del_v2_t *mp)
{
    ...
    if (mp->is_add) {
        ip_table_create(fproto, table_id, 1 /* is_api */, mp->create_mfib, mp->table.name);
    }
    else {
        ip_table_delete(fproto, table_id, 1);
    }
...
void vl_api_ip_table_allocate_t_handler(vl_api_ip_table_allocate_t *mp)
{
    ...
    if (~0 == table_id) table_id = ip_table_get_unused_id(fproto);
...
static int ip_route_add_del_t_handler(vl_api_ip_route_add_del_t *mp, u32 *stats_index)
{
    ...
    rv = fib_api_table_id_decode(pfx.fp_proto, ntohl(mp->route.table_id), &fib_index);
    if (0 != rv) goto out;

    if (0 != mp->route.n_paths) vec_validate(rpaths, mp->route.n_paths - 1);
    ...
    for (ii = 0; ii < mp->route.n_paths; ii++) {
        ...
        if ((rpath->frp_flags & FIB_ROUTE_PATH_LOCAL) && (~0 == rpath->frp_sw_if_index))
            entry_flags |= (FIB_ENTRY_FLAG_CONNECTED | FIB_ENTRY_FLAG_LOCAL);

        if (0 != rv) goto out;
    }

    rv = fib_api_route_add_del(mp->is_add, mp->is_multipath, fib_index, &pfx, FIB_SOURCE_API, entry_flags, rpaths);

    if (mp->is_add && 0 == rv) *stats_index = fib_table_entry_get_stats_index(fib_index, &pfx);
...
static int ip_route_add_del_v2_t_handler(vl_api_ip_route_add_del_v2_t *mp, u32 *stats_index)
{
    ...
    rv = fib_api_route_add_del(mp->is_add, mp->is_multipath, fib_index, &pfx, src, entry_flags, rpaths);
...
void vl_api_ip_route_add_del_t_handler(vl_api_ip_route_add_del_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_IP_ROUTE_ADD_DEL_REPLY, ({ rmp->stats_index = htonl(stats_index); }))
}
...
void vl_api_ip_route_lookup_t_handler(vl_api_ip_route_lookup_t *mp)
{
    ...
    rv = fib_api_table_id_decode(lookup.fp_proto, ntohl(mp->table_id), &fib_index);
    if (PREDICT_TRUE(!rv)) {
        if (mp->exact)
    ...
    REPLY_MACRO3_ZERO(VL_API_IP_ROUTE_LOOKUP_REPLY, npaths * sizeof(*fp), ({
                          if (!rv) {
...
void ip_table_create(fib_protocol_t fproto, u32 table_id, u8 is_api, u8 create_mfib, const u8 *name)
{
    u32          fib_index, mfib_index;
    vnet_main_t *vnm = vnet_get_main();
    ...
    if (0 != table_id) {
         ...
         */
        fib_table_find_or_create_and_lock_w_name(fproto, table_id, (is_api ? FIB_SOURCE_API : FIB_SOURCE_CLI), name);

        if (create_mfib) {
            /* same for mfib, if needs be */
            mfib_index = mfib_table_find(fproto, table_id);
            mfib_table_find_or_create_and_lock_w_name(fproto, table_id, (is_api ? MFIB_SOURCE_API : MFIB_SOURCE_CLI), name);
        }
        else
            mfib_index = 0;

        if ((~0 == fib_index) || (~0 == mfib_index))
            call_elf_section_ip_table_callbacks(vnm, table_id, 1 /* is_add */, vnm->ip_table_add_del_functions);
    }
}

static u32
mroute_add_del_handler(u8 is_add, u8 is_multipath, u32 fib_index, const mfib_prefix_t *prefix, u32 entry_flags, u32 rpf_id, fib_route_path_t *rpaths)
{
    u32 mfib_entry_index = ~0;

    if (0 == vec_len(rpaths)) {
        mfib_entry_index = mfib_table_entry_update(fib_index, prefix, MFIB_SOURCE_API, rpf_id, entry_flags);
    }
    else {
        if (is_add) {
            mfib_entry_index = mfib_table_entry_paths_update(fib_index, prefix, MFIB_SOURCE_API, entry_flags, rpaths);
        }
        else {
            mfib_table_entry_paths_remove(fib_index, prefix, MFIB_SOURCE_API, rpaths);
        }
    }
...
static int api_mroute_add_del_t_handler(vl_api_ip_mroute_add_del_t *mp, u32 *stats_index)
{
    ...
    rv = mfib_api_table_id_decode(pfx.fp_proto, ntohl(mp->route.table_id), &fib_index);
    if (0 != rv) goto out;
    ...
    mfib_entry_index = mroute_add_del_handler(mp->is_add, mp->is_add, fib_index, &pfx, eflags, ntohl(mp->route.rpf_id), rpaths);

    if (~0 != mfib_entry_index) *stats_index = mfib_entry_get_stats_index(mfib_entry_index);
...
void vl_api_ip_mroute_add_del_t_handler(vl_api_ip_mroute_add_del_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_IP_MROUTE_ADD_DEL_REPLY, ({ rmp->stats_index = htonl(stats_index); }));
}
...
static void vl_api_ip_unnumbered_dump_t_handler(vl_api_ip_unnumbered_dump_t *mp)
{
    ...
    if (~0 != sw_if_index) {
        ...
        if (si->flags & VNET_SW_INTERFACE_FLAG_UNNUMBERED) {
            send_ip_unnumbered_details(am, reg, sw_if_index, si->unnumbered_sw_if_index, mp->context);
        }
    }
    else {
        pool_foreach (si, im->sw_interfaces) {
            if ((si->flags & VNET_SW_INTERFACE_FLAG_UNNUMBERED)) {
                send_ip_unnumbered_details(am, reg, si->sw_if_index, si->unnumbered_sw_if_index, mp->context);
            }
        }
    }
...
#define _(a, b) \
    if (mp->a) flow_hash_config |= b;
    foreach_flow_hash_bit_v1;
#undef _

    rv = ip_flow_hash_set((mp->is_ipv6 ? AF_IP6 : AF_IP4), table_id, flow_hash_config);
...
static void vl_api_set_ip_flow_hash_v2_t_handler(vl_api_set_ip_flow_hash_v2_t *mp)
{
    ...
    if (!rv) rv = ip_flow_hash_set(af, htonl(mp->table_id), htonl(mp->flow_hash_config));

    REPLY_MACRO(VL_API_SET_IP_FLOW_HASH_V2_REPLY);
}

static void vl_api_set_ip_flow_hash_v3_t_handler(vl_api_set_ip_flow_hash_v3_t *mp)
{
...
void vl_mfib_signal_send_one(vl_api_registration_t *reg, u32 context, const mfib_signal_t *mfs)
{
    ...
    mfib            = mfib_table_get(mfib_entry_get_fib_index(mfs->mfs_entry), prefix->fp_proto);
...
static int ip_container_proxy_send_details(const fib_prefix_t *pfx, u32 sw_if_index, void *args)
{
    ...
    clib_memset(mp, 0, sizeof(*mp));
    mp->_vl_msg_id = ntohs(REPLY_MSG_ID_BASE + VL_API_IP_CONTAINER_PROXY_DETAILS);
...
static void vl_api_ioam_enable_t_handler(vl_api_ioam_enable_t *mp)
{
     ...
     * is supported */
    error = ip6_ioam_enable(mp->trace_enable, mp->pot_enable, mp->seqno, mp->analyse);
    if (error) {
        clib_error_report(error);
        rv = clib_error_get_code(error);
    }
...
static void vl_api_ip_source_and_port_range_check_add_del_t_handler(vl_api_ip_source_and_port_range_check_add_del_t *mp)
{
    ...
    if (FIB_PROTOCOL_IP6 == pfx.fp_proto) {
        rv = ip6_source_and_port_range_check_add_del(&pfx.fp_addr.ip6, pfx.fp_len, vrf_id, low_ports, high_ports, is_add);
    }
    else {
        rv = ip4_source_and_port_range_check_add_del(&pfx.fp_addr.ip4, pfx.fp_len, vrf_id, low_ports, high_ports, is_add);
    }
...
static void vl_api_sw_interface_ip6_get_link_local_address_t_handler(vl_api_sw_interface_ip6_get_link_local_address_t *mp)
{
    ...
    if (NULL == ip) rv = VNET_API_ERROR_IP6_NOT_ENABLED;
...
static void vl_api_ip_table_replace_begin_t_handler(vl_api_ip_table_replace_begin_t *mp)
{
    ...
    if (INDEX_INVALID == fib_index)
        ...
        if (mfib_index != INDEX_INVALID) mfib_table_mark(mfib_index, fproto, MFIB_SOURCE_API);
    }
...
static void vl_api_ip_table_replace_end_t_handler(vl_api_ip_table_replace_end_t *mp)
{
    ...
    if (INDEX_INVALID == fib_index)
        ...
        if (mfib_index != INDEX_INVALID) mfib_table_sweep(mfib_index, fproto, MFIB_SOURCE_API);
    }
...
static void vl_api_ip_table_flush_t_handler(vl_api_ip_table_flush_t *mp)
{
    ...
    if (INDEX_INVALID == fib_index)
        ...
        pool_foreach (si, im->sw_interfaces) {
            if (fib_index == fib_table_get_index_for_sw_if_index(fproto, si->sw_if_index)) {
                u32 flags = si->flags;
        ...
        if (mfib_index != INDEX_INVALID) mfib_table_flush(mfib_index, fproto, MFIB_SOURCE_API);
    }
...
void vl_api_ip_reassembly_set_t_handler(vl_api_ip_reassembly_set_t *mp)
{
    ...
    switch ((vl_api_ip_reass_type_t) clib_net_to_host_u32(mp->type)) {
        ...
        case IP_REASS_TYPE_SHALLOW_VIRTUAL:
            if (mp->is_ip6) {
                rv = ip6_sv_reass_set(clib_net_to_host_u32(mp->timeout_ms),
                                      clib_net_to_host_u32(mp->max_reassemblies),
                                      clib_net_to_host_u32(mp->max_reassembly_length),
                                      clib_net_to_host_u32(mp->expire_walk_interval_ms));
            }
...
void vl_api_ip_reassembly_get_t_handler(vl_api_ip_reassembly_get_t *mp)
{
    ...
    switch ((vl_api_ip_reass_type_t) clib_net_to_host_u32(mp->type)) {
        case IP_REASS_TYPE_FULL:
            if (mp->is_ip6) {
                rmp->is_ip6 = 1;
                ip6_full_reass_get(&timeout_ms, &max_reassemblies, &max_reassembly_length, &expire_walk_interval_ms);
            }
            else {
                rmp->is_ip6 = 0;
                ip4_full_reass_get(&timeout_ms, &max_reassemblies, &max_reassembly_length, &expire_walk_interval_ms);
            }
            break;
        case IP_REASS_TYPE_SHALLOW_VIRTUAL:
            if (mp->is_ip6) {
                rmp->is_ip6 = 1;
                ip6_sv_reass_get(&timeout_ms, &max_reassemblies, &max_reassembly_length, &expire_walk_interval_ms);
            }
            else {
                rmp->is_ip6 = 0;
                ip4_sv_reass_get(&timeout_ms, &max_reassemblies, &max_reassembly_length, &expire_walk_interval_ms);
            }
            break;
    }
    ...
    rmp->expire_walk_interval_ms = clib_host_to_net_u32(expire_walk_interval_ms);
...
void vl_api_ip_reassembly_enable_disable_t_handler(vl_api_ip_reassembly_enable_disable_t *mp)
{
    ...
    switch ((vl_api_ip_reass_type_t) clib_net_to_host_u32(mp->type)) {
        case IP_REASS_TYPE_FULL:
            rv = ip4_full_reass_enable_disable(clib_net_to_host_u32(mp->sw_if_index), mp->enable_ip4);
            if (0 == rv) rv = ip6_full_reass_enable_disable(clib_net_to_host_u32(mp->sw_if_index), mp->enable_ip6);
            break;
        case IP_REASS_TYPE_SHALLOW_VIRTUAL:
            rv = ip4_sv_reass_enable_disable(clib_net_to_host_u32(mp->sw_if_index), mp->enable_ip4);
            if (0 == rv) {
                rv = ip6_sv_reass_enable_disable(clib_net_to_host_u32(mp->sw_if_index), mp->enable_ip6);
            }
            break;
    }
...
void vl_api_ip_local_reass_enable_disable_t_handler(vl_api_ip_local_reass_enable_disable_t *mp)
{
...
void vl_api_ip_local_reass_get_t_handler(vl_api_ip_local_reass_get_t *mp)
{
...
static walk_rc_t send_ip_punt_redirect_details(u32 rx_sw_if_index, const ip_punt_redirect_rx_t *ipr, void *arg)
{
    ...
    ip_address_encode(&path_ctx.rpaths[0].frp_addr, fib_proto_to_ip46(ipr->fproto), &mp->punt.nh);
...
static walk_rc_t send_ip_punt_redirect_v2_details(u32 rx_sw_if_index, const ip_punt_redirect_rx_t *ipr, void *arg)
{
    ...
    clib_memset(mp, 0, sizeof(*mp));
    mp->_vl_msg_id          = ntohs(REPLY_MSG_ID_BASE + VL_API_IP_PUNT_REDIRECT_V2_DETAILS);
    mp->context             = ctx->context;
    mp->punt.rx_sw_if_index = htonl(rx_sw_if_index);
...
static void vl_api_ip_punt_redirect_dump_t_handler(vl_api_ip_punt_redirect_dump_t *mp)
{
    ...
    vl_api_ip_punt_redirect_dump_common(&ctx, fproto, ntohl(mp->sw_if_index), send_ip_punt_redirect_details);
}
...
static void vl_api_ip_punt_redirect_v2_dump_t_handler(vl_api_ip_punt_redirect_v2_dump_t *mp)
{
    ...
    vl_api_ip_punt_redirect_dump_common(&ctx, fproto, ntohl(mp->sw_if_index), send_ip_punt_redirect_v2_details);
}
...
void vl_api_ip_path_mtu_update_t_handler(vl_api_ip_path_mtu_update_t *mp)
{
    ...
    rv = ip_path_mtu_update(&nh, ntohl(mp->pmtu.table_id), ntohs(mp->pmtu.path_mtu));
...
static void send_ip_path_mtu_details(index_t ipti, vl_api_registration_t *rp, u32 context)
{
    ...
    REPLY_MACRO_DETAILS4(VL_API_IP_PATH_MTU_DETAILS, rp, context, ({
                             ...
                             rmp->pmtu.table_id = htonl(ip_pmtu_get_table_id(ipt));
...
static void vl_api_ip_path_mtu_get_t_handler(vl_api_ip_path_mtu_get_t *mp)
{
    ...
    REPLY_AND_DETAILS_MACRO(VL_API_IP_PATH_MTU_GET_REPLY, ip_pmtu_pool, ({ send_ip_path_mtu_details(cursor, rp, mp->context); }));
}
...
static clib_error_t *ip_api_hookup(vlib_main_t *vm)
{
     ...
     */
    vl_api_set_msg_thread_safe(am, REPLY_MSG_ID_BASE + VL_API_IP_ROUTE_ADD_DEL, 1);
    vl_api_set_msg_thread_safe(am, REPLY_MSG_ID_BASE + VL_API_IP_ROUTE_ADD_DEL_REPLY, 1);
    vl_api_set_msg_thread_safe(am, REPLY_MSG_ID_BASE + VL_API_IP_ROUTE_ADD_DEL_V2, 1);
    vl_api_set_msg_thread_safe(am, REPLY_MSG_ID_BASE + VL_API_IP_ROUTE_ADD_DEL_V2_REPLY, 1);
    vl_api_set_msg_thread_safe(am, REPLY_MSG_ID_BASE + VL_API_IP_ADDRESS_DUMP, 1);
