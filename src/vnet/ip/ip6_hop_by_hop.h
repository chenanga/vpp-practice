...
extern clib_error_t *ip6_ioam_enable(int has_trace_option, int has_pot_option, int has_seqno_option, int has_analyse_option);

extern int ip6_ioam_set_destination(ip6_address_t *addr, u32 mask_width, u32 vrf_id, int is_add, int is_pop, int is_none);
...
static inline u32 ioam_flow_add(u8 encap, u8 *flow_name)
{
    ip6_hop_by_hop_ioam_main_t *hm    = &ip6_hop_by_hop_ioam_main;
    flow_data_t                *flow  = 0;
    u32                         index = 0;
    ...
    if (!encap) IOAM_SET_DECAP(index);

    for (i = 0; i < 255; i++) {
        if (hm->flow_handler[i]) flow->ctx[i] = hm->flow_handler[i](index, 1);
    }
    return (index);
}

always_inline ip6_hop_by_hop_option_t *ip6_hbh_get_option(ip6_hop_by_hop_header_t *hbh0, u8 option_to_search)
{
    ...
    while (opt0 < limit0) {
        ...
        switch (type0) {
            ...
            case 1: /* PadN */
                break;
            default:
                if (type0 == option_to_search) return opt0;
                break;
        }
