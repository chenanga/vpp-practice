 ...
 */
always_inline uword ip_punt_policer(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 arc_index, u32 policer_index)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            act0 = vnet_policer_police(vm, b0, policer_index, time_in_policer_periods, POLICE_CONFORM, true);
            act1 = vnet_policer_police(vm, b1, policer_index, time_in_policer_periods, POLICE_CONFORM, true);
 ...
 */
extern void ip_punt_redirect_add(fib_protocol_t fproto, u32 rx_sw_if_index, fib_forward_chain_type_t ct, const fib_route_path_t *rpaths);
...
extern void ip_punt_redirect_walk(fib_protocol_t fproto, ip_punt_redirect_walk_cb_t cb, void *ctx);
...
always_inline uword ip_punt_redirect(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 arc_index, fib_protocol_t fproto)
{
    u32                        *from, *to_next, n_left_from, n_left_to_next, next_index;
    vnet_feature_main_t        *fm = &feature_main;
    vnet_feature_config_main_t *cm = &fm->feature_config_mains[arc_index];
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (vec_len(redirects) > rx_sw_if_index0) {
                ...
                if (INDEX_INVALID == rrxi0) rrxi0 = redirects[0];
            }
            ...
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                ip_punt_redirect_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                t->next                     = next0;
                t->rrxi                     = rrxi0;
            }
...
always_inline uword ip_drop_or_punt(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 arc_index)
{
    u32 *from, *to_next, n_left_from, n_left_to_next, next_index;
