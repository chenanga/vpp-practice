...
int ip_prefix_decode2(const vl_api_prefix_t* in, ip_prefix_t* out)
{
    ...
    if (!ip_prefix_validate(out)) return (VNET_API_ERROR_IP_PREFIX_INVALID);
    return (0);
}
...
{
    out->fp_proto    = (ADDRESS_IP6 == in->af ? FIB_PROTOCOL_IP6 : FIB_PROTOCOL_IP4);
    ...
    if (!ip46_address_is_zero(&out->fp_src_addr)) out->fp_len = (out->fp_proto == FIB_PROTOCOL_IP6 ? 256 : 64);
}
