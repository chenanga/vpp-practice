...
static u8 *format_ip_frag_trace(u8 *s, va_list *args)
{
    ...
    s                               = format(s, "mtu: %u pkt-size: %u fragments: %u next: %d", t->mtu, t->pkt_size, t->n_fragments, t->next);
    return s;
}
...
{
    vnet_buffer(to)->sw_if_index[VLIB_RX] = vnet_buffer(from)->sw_if_index[VLIB_RX];
    vnet_buffer(to)->sw_if_index[VLIB_TX] = vnet_buffer(from)->sw_if_index[VLIB_TX];
     ...
     */
    vnet_buffer(to)->ip.adj_index[VLIB_RX] = vnet_buffer(from)->ip.adj_index[VLIB_RX];
    vnet_buffer(to)->ip.adj_index[VLIB_TX] = vnet_buffer(from)->ip.adj_index[VLIB_TX];
...
ip_frag_error_t ip4_frag_do_fragment(vlib_main_t *vm, u32 from_bi, u16 mtu, u16 l2unfragmentablesize, u32 **buffer)
{
    ...
    max        = (clib_min(mtu, vlib_buffer_get_default_data_size(vm)) - head_bytes) & ~0x7;
    ...
    if (ip4->flags_and_fragment_offset & clib_host_to_net_u16(IP4_HEADER_FLAG_DONT_FRAGMENT)) {
    ...
    while (rem) {
        ...
        if (from_b->flags & VNET_BUFFER_F_L4_HDR_OFFSET_VALID) {
            vnet_buffer(to_b)->l4_hdr_offset =
                (vnet_buffer(to_b)->l3_hdr_offset + (vnet_buffer(from_b)->l4_hdr_offset - vnet_buffer(from_b)->l3_hdr_offset));
        ...
        while (1) {
            ...
            bytes_to_copy = left_in_to_buffer <= left_in_from_buffer ? left_in_to_buffer : left_in_from_buffer;
        ...
        to_ip4->flags_and_fragment_offset = clib_host_to_net_u16((fo >> 3) + ip_frag_offset);
        to_ip4->flags_and_fragment_offset |= clib_host_to_net_u16(((len != rem) || more) << 13);
...
static inline uword frag_node_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u32 node_index, bool is_ip6)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (!is_ip6 && error0 == IP_FRAG_ERROR_DONT_FRAGMENT_SET) {
                icmp4_error_set_vnet_buffer(p0,
                ...
                next0 = (error0 == IP_FRAG_ERROR_NONE ? vnet_buffer(p0)->ip_frag.next_index : IP_FRAG_NEXT_DROP);
            }
            ...
            while (frag_left > 0) {
                while (frag_left > 0 && n_left_to_next > 0) {
                    ...
                    vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, i, next0);
                }
    ...
    vlib_node_increment_counter(vm, node_index, IP_FRAG_ERROR_FRAGMENT_SENT, frag_sent);
    vlib_node_increment_counter(vm, node_index, IP_FRAG_ERROR_SMALL_PACKET, small_packets);
...
{
    return frag_node_inline(vm, node, frame, ip4_frag_node.index, 0 /* is_ip6 */);
}

static uword ip6_frag(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return frag_node_inline(vm, node, frame, ip6_frag_node.index, 1 /* is_ip6 */);
}
...
ip_frag_error_t ip6_frag_do_fragment(vlib_main_t *vm, u32 from_bi, u16 mtu, u16 l2unfragmentablesize, u32 **buffer)
{
    ...
    head_bytes = (sizeof(ip6_header_t) + sizeof(ip6_frag_hdr_t) + l2unfragmentablesize);
    ...
    vlib_buffer_t *org_from_b          = from_b;
    u16            fo                  = 0;
    u16            left_in_from_buffer = from_b->current_length - (l2unfragmentablesize + sizeof(ip6_header_t));
    u16            ptr                 = 0;
    ...
    while (rem) {
        ...
        /* Copy ip6 header */
        clib_memcpy_fast(to_b->data, org_from_packet, l2unfragmentablesize + sizeof(ip6_header_t));
        ...
        to_b->current_length                  = len + head_bytes;
        to_ip6->payload_length                = clib_host_to_net_u16(len + sizeof(ip6_frag_hdr_t));
        to_ip6->protocol                      = IP_PROTOCOL_IPV6_FRAGMENTATION;
        to_frag_hdr->fragment_offset_and_more = ip6_frag_hdr_offset_and_more((fo >> 3), len != rem);
