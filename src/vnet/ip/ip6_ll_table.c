...
{
    return (ip6_fib_table_lookup(ip6_ll_fib_get(prefix->ilp_sw_if_index), &prefix->ilp_addr, 128));
}

fib_node_index_t ip6_ll_table_lookup_exact_match(const ip6_ll_prefix_t *prefix)
{
    return (ip6_fib_table_lookup_exact_match(ip6_ll_fib_get(prefix->ilp_sw_if_index), &prefix->ilp_addr, 128));
}
...
static void ip6_ll_fib_create(u32 sw_if_index)
{
    ...
    desc = format(NULL, "IP6-link-local:%U", format_vnet_sw_if_index_name, vnm, sw_if_index);

    ip6_ll_table.ilt_fibs[sw_if_index] = ip6_fib_table_create_and_lock(FIB_SOURCE_IP6_ND, FIB_TABLE_FLAG_IP6_LL, desc);
    ...
    fib_table_entry_update_one_path(ip6_ll_table.ilt_fibs[sw_if_index],
                                    &pfx,
                                    FIB_SOURCE_SPECIAL,
                                    (FIB_ENTRY_FLAG_ATTACHED | FIB_ENTRY_FLAG_NO_ATTACHED_EXPORT),
                                    DPO_PROTO_IP6,
                                    NULL,
                                    sw_if_index,
                                    ~0,
...
fib_node_index_t ip6_ll_table_entry_update(const ip6_ll_prefix_t *ilp, fib_route_path_flags_t flags)
{
    ...
    if (flags & FIB_ROUTE_PATH_LOCAL) rpath.frp_addr.ip6 = ilp->ilp_addr;
    ...
    ip6_ll_entry_index = fib_table_entry_update(ip6_ll_fib_get(ilp->ilp_sw_if_index),
                                                &fp,
                                                FIB_SOURCE_IP6_ND,
                                                (flags & FIB_ROUTE_PATH_LOCAL ? FIB_ENTRY_FLAG_LOCAL : FIB_ENTRY_FLAG_NONE),
                                                rpaths);
    vec_free(rpaths);
...
void ip6_ll_table_entry_delete(const ip6_ll_prefix_t *ilp)
{
    ...
    if (~0 == fib_index) return;

    ip6_ll_entry_index = ip6_ll_table_lookup_exact_match(ilp);
    if (FIB_NODE_INDEX_INVALID == ip6_ll_entry_index) return;
     ...
     */
    if (0 == fib_table_get_num_entries(fib_index, FIB_PROTOCOL_IP6, FIB_SOURCE_IP6_ND)) {
...
{
    vlib_cli_output(vm, "%U", format_fib_entry, ip6_ll_table_lookup(ilp), (detail ? FIB_ENTRY_FORMAT_DETAIL2 : FIB_ENTRY_FORMAT_DETAIL));
}
...
static void ip6_ll_table_show_all(vlib_main_t *vm, u32 fib_index)
{
    ...
    vec_foreach (fib_entry_index, ctx.entries) {
        vlib_cli_output(vm, "%U", format_fib_entry, *fib_entry_index, FIB_ENTRY_FORMAT_BRIEF);
    }
...
static clib_error_t *ip6_ll_show_fib(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(input, "%U/%d", unformat_ip6_address, &matching_address, &mask_len))
            matching = 1;
...
static clib_error_t *ip6_ll_sw_interface_add_del(vnet_main_t *vnm, u32 sw_if_index, u32 is_add)
{
