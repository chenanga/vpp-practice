...
#define IP_PMTU_TRKR_DBG(_ipt, _fmt, _args...)                                                            \
    {                                                                                                     \
        vlib_log_debug(ip_pmtu_logger, "[%U]: " _fmt ": ", format_ip_pmtu, _ipt - ip_pmtu_pool, ##_args); \
    }
...
static void ip_pmtu_adj_delegate_adj_created(adj_index_t ai)
{
    ...
    table_id = fib_table_get_table_id_for_sw_if_index(adj->ia_nh_proto, adj->rewrite_header.sw_if_index);
    ...
    adj_delegate_add(adj, ip_pmtu_adj_delegate_type, ipp_ad - ip_path_mtu_adj_delegate_pool);
...
static adj_walk_rc_t ip_ptmu_adj_walk_update(adj_index_t ai, void *ctx)
{
    ...
    if (ad)
        ...
        adj_delegate_add(adj_get(ai), ip_pmtu_adj_delegate_type, ipp_ad - ip_path_mtu_adj_delegate_pool);
    }
...
void ip_pmtu_dpo_add_or_lock(u16 pmtu, const dpo_id_t *parent, dpo_id_t *dpo)
{
    ip_pmtu_dpo_t *ipm;
...
u8 *format_ip_pmtu_dpo(u8 *s, va_list *ap)
{
    ...
    s = format(s, "ip-pmtu-dpo: %U, mtu:%d", format_dpo_proto, ipm->ipm_proto, ipm->ipm_pmtu);
...
static void ip_pmtu_dpo_interpose(const dpo_id_t *original, const dpo_id_t *parent, dpo_id_t *clone)
{
    ...
    dpo_stack(ip_pmtu_dpo_type, ipm_clone->ipm_proto, &ipm_clone->ipm_dpo, parent);
    dpo_set(clone, ip_pmtu_dpo_type, ipm_clone->ipm_proto, ip_pmtu_dpo_get_index(ipm_clone));
}
...
static bool ip_mtu_fib_entry_is_attached(fib_node_index_t fib_entry)
{
    ...
    return (FIB_ENTRY_FLAG_ATTACHED & fib_entry_get_flags(cover) || FIB_ENTRY_FLAG_ATTACHED & fib_entry_get_flags(fib_entry));
}
...
static index_t ip_pmtu_alloc(u32 fib_index, const fib_prefix_t *pfx, const ip_pmtu_key_t *key, u16 pmtu)
{
    ...
    ipt->ipt_fib_entry = fib_entry_track(fib_index, pfx, ip_pmtu_fib_type, ipti, &ipt->ipt_sibling);
    ...
    if (ip_mtu_fib_entry_is_attached(ipt->ipt_fib_entry)) {
        ...
        adj_nbr_walk_nh(sw_if_index, pfx->fp_proto, &pfx->fp_addr, ip_ptmu_adj_walk_update, &ipt->ipt_oper_pmtu);
    }
    else {
         ...
         */
        ip_pmtu_dpo_add_or_lock(ipt->ipt_oper_pmtu, drop_dpo_get(fib_proto_to_dpo(pfx->fp_proto)), &ip_dpo);

        fib_table_entry_special_dpo_add(fib_index, pfx, ip_pmtu_source, FIB_ENTRY_FLAG_INTERPOSE, &ip_dpo);
        dpo_reset(&ip_dpo);
    }
...
static void ip_pmtu_stack(ip_pmtu_t *ipt)
{
    ...
    if (is_attached) {
        ...
        if (ipt->ipt_oper_pmtu < dpo_mtu) {
            ...
            fib_table_entry_special_dpo_update(fib_index, pfx, ip_pmtu_source, FIB_ENTRY_FLAG_INTERPOSE, &ip_dpo);
            dpo_reset(&ip_dpo);
        }
    }
...
static index_t ip_pmtu_destroy(index_t ipti, const ip_pmtu_key_t *key)
{
    ...
    if (ipt->ipt_flags & IP_PMTU_FLAG_REMOTE)
        fib_table_entry_special_remove(fib_entry_get_fib_index(ipt->ipt_fib_entry), pfx, ip_pmtu_source);
    else
        /* remove the delegate from all the adjacencies */
        adj_nbr_walk_nh(fib_entry_get_resolving_interface(ipt->ipt_fib_entry), pfx->fp_proto, &pfx->fp_addr, ip_ptmu_adj_walk_remove, NULL);
...
int ip_path_mtu_update(const ip_address_t *nh, u32 table_id, u16 pmtu)
{
    ...
    if (~0 == fib_index) return (VNET_API_ERROR_NO_SUCH_TABLE);
    ...
    if (!ip_path_mtu_value_invalid(pmtu)) {
        ...
        if (NULL != p) ip_pmtu_destroy(p[0], &key);
    }
...
void ip_path_mtu_walk(ip_path_mtu_walk_t fn, void *ctx)
{
    ...
    pool_foreach_index (ipmi, ip_pmtu_pool) {
        if (WALK_STOP == fn(ipmi, ctx)) break;
    }
}
...
{
    return ((ip_pmtu_t *) (((char *) node) - STRUCT_OFFSET_OF(ip_pmtu_t, ipt_node)));
}
...
{
    ip_pmtu_adj_delegate_type = adj_delegate_register_new_type(&ip_path_adj_delegate_vft);
    ...
    ip_pmtu_dpo_type = dpo_register_new_type(&ip_pmtu_dpo_vft, ip_pmtu_dpo_nodes);
