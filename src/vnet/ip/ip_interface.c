...
clib_error_t *ip_interface_address_add(ip_lookup_main_t *lm, u32 sw_if_index, void *addr_fib, u32 address_length, u32 *result_if_address_index)
{
    ...
    /* Verify given length. */
    if ((address_length == 0) || (lm->is_ip6 && address_length > 128) || (!lm->is_ip6 && address_length > 32)) {
    ...
    a->address_key            = mhash_set(&lm->address_to_if_address_index, addr_fib, ai, /* old_value */ 0);
    a->address_length         = address_length;
    a->sw_if_index            = sw_if_index;
    a->flags                  = 0;
    a->prev_this_sw_interface = pi;
    a->next_this_sw_interface = ~0;
    if (prev) prev->next_this_sw_interface = ai;
...
clib_error_t *ip_interface_address_del(ip_lookup_main_t *lm, vnet_main_t *vnm, u32 address_index, void *addr_fib, u32 address_length, u32 sw_if_index)
{
    ...
    if (a->next_this_sw_interface != ~0) {
        ...
        if (a->prev_this_sw_interface == ~0) lm->if_address_pool_index_by_sw_if_index[a->sw_if_index] = a->next_this_sw_interface;
    }

    if ((a->next_this_sw_interface == ~0) && (a->prev_this_sw_interface == ~0)) lm->if_address_pool_index_by_sw_if_index[a->sw_if_index] = ~0;

    mhash_unset(&lm->address_to_if_address_index,
                addr_fib,
                /* old_value */ 0);
...
u8 ip_interface_has_address(u32 sw_if_index, ip46_address_t *ip, u8 is_ip4)
{
    ...
    if (is_ip4) {
        ...
        foreach_ip_interface_address(lm4, ia, sw_if_index, 1 /* unnumbered */, ({
                                         ...
                                         if (ip4_address_compare(ip4, &ip->ip4) == 0) return 1;
                                     }));
    }
    else {
        ...
        foreach_ip_interface_address(lm6, ia, sw_if_index, 1 /* unnumbered */, ({
                                         ...
                                         if (ip6_address_compare(ip6, &ip->ip6) == 0) return 1;
                                     }));
    }
    return 0;
}
...
void *ip_interface_get_first_ip(u32 sw_if_index, u8 is_ip4)
{
    ...
    if (is_ip4) {
        foreach_ip_interface_address(lm4, ia, sw_if_index, 1 /* unnumbered */, ({ return ip_interface_address_get_address(lm4, ia); }));
    }
    else {
        foreach_ip_interface_address(lm6, ia, sw_if_index, 1 /* unnumbered */, ({
                                         ip6_address_t *rv;
                                         ...
                                         if (!ip6_address_is_link_local_unicast(rv)) return rv;
                                     }));
    }
...
walk_rc_t ip_interface_address_mark_one_interface(vnet_main_t *vnm, vnet_sw_interface_t *si, void *ctx)
{
    ip_lookup_main_t       *lm4 = &ip4_main.lookup_main;
    ip_lookup_main_t       *lm6 = &ip6_main.lookup_main;
    ip_interface_address_t *ia  = 0;

    foreach_ip_interface_address(lm4, ia, si->sw_if_index, 1 /* unnumbered */, ({ ia->flags |= IP_INTERFACE_ADDRESS_FLAG_STALE; }));
    foreach_ip_interface_address(lm6, ia, si->sw_if_index, 1 /* unnumbered */, ({ ia->flags |= IP_INTERFACE_ADDRESS_FLAG_STALE; }));
...
{
    vnet_sw_interface_walk(vnet_get_main(), ip_interface_address_mark_one_interface, NULL);
}
...
static walk_rc_t ip_interface_address_sweep_one_interface(vnet_main_t *vnm, vnet_sw_interface_t *si, void *ctx)
{
    ...
    foreach_ip_interface_address(&im4->lookup_main, ia, si->sw_if_index, 1, ({
        if (ia->flags & IP_INTERFACE_ADDRESS_FLAG_STALE) {
            ip4_address_t *x = (ip4_address_t *) ip_interface_address_get_address(&im4->lookup_main, ia);
    ...
    foreach_ip_interface_address(&im6->lookup_main, ia, si->sw_if_index, 1, ({
        if (ia->flags & IP_INTERFACE_ADDRESS_FLAG_STALE) {
            ip6_address_t *x = (ip6_address_t *) ip_interface_address_get_address(&im6->lookup_main, ia);
    ...
    for (i = 0; i < vec_len(ip4_addrs); i++)
        ip4_add_del_interface_address(vm, si->sw_if_index, &ip4_addrs[i], ip4_masks[i], 1 /* is_del */);
    for (i = 0; i < vec_len(ip6_addrs); i++)
        ip6_add_del_interface_address(vm, si->sw_if_index, &ip6_addrs[i], ip6_masks[i], 1 /* is_del */);
...
{
    vnet_sw_interface_walk(vnet_get_main(), ip_interface_address_sweep_one_interface, NULL);
}
