 ...
 */
typedef int (*ip4_to_ip6_set_fn_t)(vlib_buffer_t *b, ip4_header_t *ip4, ip6_header_t *ip6, void *ctx);

static u8 icmp_to_icmp6_updater_pointer_table[] = {0, 1, 4, 4, ~0, ~0, ~0, ~0, 7, 6, ~0, ~0, 8, 8, 8, 8, 24, 24, 24, 24};
 ...
 */
always_inline u16 ip4_get_port(ip4_header_t *ip, u8 sender)
{
 ...
 */
always_inline int icmp_to_icmp6_header(icmp46_header_t *icmp, ip4_header_t **inner_ip4)
{
    *inner_ip4 = NULL;
    switch (icmp->type) {
 ...
 * @returns 0 on success, non-zero value otherwise.
 */
always_inline int icmp_to_icmp6(vlib_buffer_t *p, ip4_to_ip6_set_fn_t fn, void *ctx, ip4_to_ip6_set_fn_t inner_fn, void *inner_ctx)
{
    ...
    if (icmp_to_icmp6_header(icmp, &inner_ip4)) return -1;
    ...
    if (inner_ip4) {
        ...
        if (PREDICT_FALSE(ip4_is_fragment(inner_ip4))) {
            ...
            memmove(u8_ptr_add(ip6, sizeof(*ip6) - sizeof(*ip4)), ip4, 20 + 8);
        ...
        if (PREDICT_TRUE(inner_ip4->protocol == IP_PROTOCOL_TCP)) {
            ...
            *inner_L4_checksum = ip_csum_fold(ip_csum_sub_even(*inner_L4_checksum, *((u64 *) (&inner_ip4->src_address))));
            ...
            if (*inner_L4_checksum) *inner_L4_checksum = ip_csum_fold(ip_csum_sub_even(*inner_L4_checksum, *((u64 *) (&inner_ip4->src_address))));
        }
            ...
            inner_icmp->type    = (inner_icmp->type == ICMP4_echo_request) ? ICMP6_echo_request : ICMP6_echo_reply;
        ...
        if ((rv = inner_fn(p, inner_ip4, inner_ip6, inner_ctx)) != 0) return rv;
        ...
        if (PREDICT_FALSE(inner_frag != NULL)) {
            ...
            inner_ip6->payload_length            = clib_host_to_net_u16(clib_net_to_host_u16(inner_ip6->payload_length) + sizeof(*inner_frag));
        }
        ...
        if (inner_ip6->protocol == IP_PROTOCOL_ICMP6) {
            ...
            csum                 = ip_csum_with_carry(csum, clib_host_to_net_u16(inner_ip6->protocol));
            ...
            csum                 = ip_incremental_checksum(csum, inner_icmp, clib_net_to_host_u16(inner_ip6->payload_length));
        ...
        if (PREDICT_FALSE(ip4->flags_and_fragment_offset & clib_host_to_net_u16(IP4_HEADER_FLAG_MORE_FRAGMENTS))) {
            vlib_buffer_advance(p, sizeof(*ip4) - sizeof(*ip6) - sizeof(*outer_frag));
            ip6           = vlib_buffer_get_current(p);
    ...
    if ((rv = fn(p, ip4, ip6, ctx)) != 0) return rv;
    ...
    if (PREDICT_FALSE(outer_frag != NULL)) {
        ...
        ip6->payload_length                  = u16_net_add(ip6->payload_length, sizeof(*outer_frag));
    }
