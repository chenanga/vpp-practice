...
typedef enum { IP46_TYPE_ANY, IP46_TYPE_BOTH = IP46_TYPE_ANY, IP46_TYPE_IP4, IP46_TYPE_IP6 } ip46_type_t;
...
always_inline u8 ip46_address_is_ip4(const ip46_address_t *ip46)
{
    return (((ip46)->pad[0] | (ip46)->pad[1] | (ip46)->pad[2]) == 0);
}

always_inline void ip46_address_mask_ip4(ip46_address_t *ip46)
{
    ((ip46)->pad[0] = (ip46)->pad[1] = (ip46)->pad[2] = 0);
}

always_inline void ip46_address_set_ip4(ip46_address_t *ip46, const ip4_address_t *ip)
{
    ip46_address_mask_ip4(ip46);
    ip46->ip4 = *ip;
}

always_inline void ip46_address_reset(ip46_address_t *ip46)
{
    ip46->as_u64[0] = ip46->as_u64[1] = 0;
}

always_inline int ip46_address_cmp(const ip46_address_t *ip46_1, const ip46_address_t *ip46_2)
{
    return (memcmp(ip46_1, ip46_2, sizeof(*ip46_1)));
}

always_inline u8 ip46_address_is_zero(const ip46_address_t *ip46)
{
    return (ip46->as_u64[0] == 0 && ip46->as_u64[1] == 0);
}

always_inline u8 ip46_address_is_equal(const ip46_address_t *ip46_1, const ip46_address_t *ip46_2)
{
    return ((ip46_1->as_u64[0] == ip46_2->as_u64[0]) && (ip46_1->as_u64[1] == ip46_2->as_u64[1]));
}
...
{
    return ((ip46->ip6.as_u64[0] == ip6->as_u64[0]) && (ip46->ip6.as_u64[1] == ip6->as_u64[1]));
}
...
always_inline ip46_address_t to_ip46(u32 is_ipv6, u8 *buf)
{
    ip46_address_t ip;
    if (is_ipv6)
...
always_inline ip46_type_t ip46_address_get_type(const ip46_address_t *ip)
{
    return (ip46_address_is_ip4(ip) ? IP46_TYPE_IP4 : IP46_TYPE_IP6);
}

always_inline uword ip46_address_is_multicast(const ip46_address_t *a)
{
    return ip46_address_is_ip4(a) ? ip4_address_is_multicast(&a->ip4) : ip6_address_is_multicast(&a->ip6);
}
