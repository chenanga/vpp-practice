...
{
    return policer_handoff(vm, node, frame, ip4_punt_policer_cfg.fq_index, ip4_punt_policer_cfg.policer_index);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (ip_punt_policer(vm, node, frame, vnet_feat_arc_ip4_punt.feature_arc_index, ip4_punt_policer_cfg.policer_index));
}
...
VLIB_NODE_FN(ip4_punt_redirect_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return (ip_punt_redirect(vm, node, frame, vnet_feat_arc_ip4_punt.feature_arc_index, FIB_PROTOCOL_IP4));
}
...
VLIB_NODE_FN(ip4_drop_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    if (node->flags & VLIB_NODE_FLAG_TRACE) ip4_forward_next_trace(vm, node, frame, VLIB_TX);

    return ip_drop_or_punt(vm, node, frame, vnet_feat_arc_ip4_drop.feature_arc_index);
}

VLIB_NODE_FN(ip4_not_enabled_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    if (node->flags & VLIB_NODE_FLAG_TRACE) ip4_forward_next_trace(vm, node, frame, VLIB_TX);
...
static uword ip4_punt(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    return ip_drop_or_punt(vm, node, frame, vnet_feat_arc_ip4_punt.feature_arc_index);
}
...
void ip4_punt_redirect_add_paths(u32 rx_sw_if_index, const fib_route_path_t *rpaths)
{
    ip_punt_redirect_add(FIB_PROTOCOL_IP4, rx_sw_if_index, FIB_FORW_CHAIN_TYPE_UNICAST_IP4, rpaths);

    if (1 == ++ip4_punt_redirect_enable_counts) vnet_feature_enable_disable("ip4-punt", "ip4-punt-redirect", 0, 1, 0, 0);
}

void ip4_punt_redirect_del(u32 rx_sw_if_index)
{
    ASSERT(ip4_punt_redirect_enable_counts);
    if (0 == --ip4_punt_redirect_enable_counts) vnet_feature_enable_disable("ip4-punt", "ip4-punt-redirect", 0, 0, 0, 0);
...
static clib_error_t *ip4_punt_redirect_cmd(vlib_main_t *vm, unformat_input_t *main_input, vlib_cli_command_t *cmd)
{
    ...
    if (is_add) {
        if (vec_len(rpaths)) ip4_punt_redirect_add_paths(rx_sw_if_index, rpaths);
    }
    else {
