...
static int api_proxy_arp_dump(vat_main_t *vam)
{
    return -1;
}

static int api_proxy_arp_intfc_dump(vat_main_t *vam)
{
    return -1;
}

static void vl_api_proxy_arp_details_t_handler(vl_api_proxy_arp_details_t *mp)
{
}

static void vl_api_proxy_arp_intfc_details_t_handler(vl_api_proxy_arp_intfc_details_t *mp)
{
}
...
static int api_proxy_arp_add_del(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "vrf %d", &vrf_id))
            ;
        else if (unformat(i, "%U - %U", unformat_vl_api_ip4_address, &lo, unformat_vl_api_ip4_address, &hi))
            range_set = 1;
