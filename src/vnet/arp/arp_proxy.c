...
void proxy_arp_walk(proxy_arp_walk_t cb, void *data)
{
    ...
    vec_foreach (pa, am->proxy_arps) {
        if (!cb(&pa->lo_addr, &pa->hi_addr, pa->fib_index, data)) break;
    }
}
...
int arp_proxy_disable(u32 sw_if_index)
{
    ...
    if (am->enabled_by_sw_if_index[sw_if_index]) {
        vnet_feature_enable_disable("arp", "arp-proxy", sw_if_index, 0, NULL, 0);
    }
...
int arp_proxy_enable(u32 sw_if_index)
{
    ...
    if (!am->enabled_by_sw_if_index[sw_if_index]) {
        vnet_feature_enable_disable("arp", "arp-proxy", sw_if_index, 1, NULL, 0);
    }
...
static int vnet_proxy_arp_add_del(const ip4_address_t *lo_addr, const ip4_address_t *hi_addr, u32 fib_index, int is_del)
{
    ...
    vec_foreach (pa, am->proxy_arps) {
        if (pa->lo_addr.as_u32 == lo_addr->as_u32 && pa->hi_addr.as_u32 == hi_addr->as_u32 && pa->fib_index == fib_index) {
    ...
    if (found_at_index != ~0) {
        ...
        if (is_del) vec_delete(am->proxy_arps, 1, found_at_index);
        return 0;
    }
    /* delete, no such entry */
    if (is_del) return VNET_API_ERROR_NO_SUCH_ENTRY;
...
void proxy_arp_intfc_walk(proxy_arp_intf_walk_t cb, void *data)
{
    ...
    vec_foreach (enabled, am->enabled_by_sw_if_index) {
        if (*enabled) cb(enabled - am->enabled_by_sw_if_index, data);
    }
}
...
static clib_error_t *set_arp_proxy(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (~0 == fib_index) return (clib_error_return(0, "no such table: %d", table_id));
...
static uword arp_proxy(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (0 == error0 && is_request0) {
                u32 this_addr = clib_net_to_host_u32(arp0->ip4_over_ethernet[1].ip4.as_u32);
                ...
                vec_foreach (pa, am->proxy_arps) {
                    ...
                    if ((this_addr >= lo_addr && this_addr <= hi_addr) && (fib_index0 == pa->fib_index)) {
    ...
    vlib_error_count(vm, node->node_index, ARP_ERROR_REPLIES_SENT, n_arp_replies_sent);
...
static clib_error_t *show_ip4_arp(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (vec_len(am->proxy_arps)) {
        ...
        vec_foreach (pa, am->proxy_arps) {
            vlib_cli_output(vm, "Fib_index %d   %U - %U ", pa->fib_index, format_ip4_address, &pa->lo_addr, format_ip4_address, &pa->hi_addr);
        }
    }
