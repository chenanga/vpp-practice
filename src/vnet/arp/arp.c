...
static uword unformat_ethernet_arp_opcode_host_byte_order(unformat_input_t *input, va_list *args)
{
    ...
    /* Named type. */
    if (unformat_user(input, unformat_vlib_number_by_name, am->opcode_by_name, &i)) {
        *result = i;
        return 1;
    }
...
{
    int *result = va_arg(*args, int *);
    if (!unformat_user(input, unformat_ethernet_arp_opcode_host_byte_order, result)) return 0;
...
static u8 *format_ethernet_arp_input_trace(u8 *s, va_list *va)
{
    ...
    s = format(s, "%U", format_ethernet_arp_header, t->packet_data, sizeof(t->packet_data));
...
{
    if (vec_len(am->ethernet_arp_by_sw_if_index) <= sw_if_index) return 0;
...
{
    if (arp_is_enabled(am, sw_if_index)) return;
...
static int arp_unnumbered(vlib_buffer_t *p0, u32 input_sw_if_index, u32 conn_sw_if_index)
{
    ...
    if (!(si->flags & VNET_SW_INTERFACE_FLAG_UNNUMBERED && (si->unnumbered_sw_if_index == conn_sw_if_index))) {
...
always_inline u32 arp_learn(u32 sw_if_index, const ethernet_arp_ip4_over_ethernet_address_t *addr)
{
...
static uword arp_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            error0 = (arp0->l2_type != clib_net_to_host_u16(ETHERNET_ARP_HARDWARE_TYPE_ethernet) ? ARP_ERROR_L2_TYPE_NOT_ETHERNET : error0);
            error0 = (arp0->l3_type != clib_net_to_host_u16(ETHERNET_TYPE_IP4) ? ARP_ERROR_L3_TYPE_NOT_IP4 : error0);
            error0 = (0 == arp0->ip4_over_ethernet[0].ip4.as_u32 ? ARP_ERROR_L3_DST_ADDRESS_UNSET : error0);

            if (ARP_ERROR_REPLIES_SENT == error0) {
                next0 = ARP_INPUT_NEXT_DISABLED;
                vnet_feature_arc_start(am->feature_arc_index, vnet_buffer(p0)->sw_if_index[VLIB_RX], &next0, p0);
            }
            else
...
static uword arp_reply(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            dst_fei           = ip4_fib_table_lookup(ip4_fib_get(fib_index0), &arp0->ip4_over_ethernet[1].ip4, 32);
...

            {
                ...
                do {
                    src_fei       = ip4_fib_table_lookup(ip4_fib_get(fib_index0), &arp0->ip4_over_ethernet[0].ip4, mask);
                    ...
                    FOR_EACH_SRC_ADDED(src_fib_entry, src, source, ({
                                            ...
                                            * interface address. */
                                           if ((FIB_ENTRY_FLAG_ATTACHED & src_flags) || (FIB_ENTRY_FLAG_CONNECTED & src_flags)) {
                                               attached = 1;
                                               break;
                                           }
                                           /*
                ...
                if (!attached && !arp_unnumbered(p0, sw_if_index0, conn_sw_if_index0)) {
                    /*
            ...
            switch (arp_dst_fib_check(dst_fei, &dst_flags)) {
                case ARP_DST_FIB_ADJ:
                    ...
                    else if (arp0->ip4_over_ethernet[0].ip4.as_u32 == arp0->ip4_over_ethernet[1].ip4.as_u32) {
                        vlib_increment_simple_counter(&ip_neighbor_counters[AF_IP4].ipnc[VLIB_RX][IP_NEIGHBOR_CTR_GRAT],
                                                      vm->thread_index,
                                                      sw_if_index0,
                                                      1);
            ...
            is_vrrp_reply0 = ((arp0->opcode == clib_host_to_net_u16(ETHERNET_ARP_OPCODE_reply))
                              && (!memcmp(arp0->ip4_over_ethernet[0].mac.bytes, vrrp_prefix, sizeof(vrrp_prefix))));

            /* Trash ARP packets whose ARP-level source addresses do not
               match their L2-frame-level source addresses, unless it's
               a reply from a VRRP virtual router */
            if (!ethernet_mac_address_equal(eth_rx->src_address, arp0->ip4_over_ethernet[0].mac.bytes) && !is_vrrp_reply0) {
                error0 = ARP_ERROR_L2_ADDRESS_MISMATCH;
                goto drop;
            }

            vlib_increment_simple_counter(&ip_neighbor_counters[AF_IP4].ipnc[VLIB_RX][arp0->opcode == clib_host_to_net_u16(ETHERNET_ARP_OPCODE_reply)
            ...
            else if (arp0->opcode == clib_host_to_net_u16(ETHERNET_ARP_OPCODE_request) && (dst_is_local0 == 0)) {
                goto next_feature;
            }

            /* Honor unnumbered interface, if any */
            if (sw_if_index0 != conn_sw_if_index0 || sw_if_index0 != fib_entry_get_resolving_interface(src_fei)) {
                /*
            ...
            if (arp0->ip4_over_ethernet[0].ip4.as_u32 == arp0->ip4_over_ethernet[1].ip4.as_u32) {
            ...
            if (!error0) error0 = arp_learn(sw_if_index0, &arp0->ip4_over_ethernet[1]);

            vlib_increment_simple_counter(&ip_neighbor_counters[AF_IP4].ipnc[VLIB_TX][IP_NEIGHBOR_CTR_REPLY], vm->thread_index, sw_if_index0, 1);
            n_replies_sent += 1;
            goto enqueue;
    ...
    vlib_error_count(vm, node->node_index, ARP_ERROR_REPLIES_SENT, n_replies_sent);
...
static inline void pg_ethernet_arp_header_init(pg_ethernet_arp_header_t *p)
{
...
uword unformat_pg_arp_header(unformat_input_t *input, va_list *args)
{
    ...
    p = pg_create_edit_group(s, sizeof(p[0]), sizeof(ethernet_arp_header_t), &group_index);
    ...
    if (!unformat(input,
                  ...
                  &p->ip4_over_ethernet[0].mac,
                  unformat_pg_edit,
                  unformat_ip4_address,
                  &p->ip4_over_ethernet[0].ip4,
                  unformat_pg_edit,
                  unformat_mac_address_t,
                  &p->ip4_over_ethernet[1].mac,
                  unformat_pg_edit,
                  unformat_ip4_address,
                  &p->ip4_over_ethernet[1].ip4)) {
...
{
    ethernet_arp_main_t *am = &ethernet_arp_main;
    if (is_add) arp_disable(am, sw_if_index);
    return (NULL);
}
