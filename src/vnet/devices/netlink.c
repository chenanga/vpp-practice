...
static clib_error_t *vnet_netlink_msg_send(vnet_netlink_msg_t *m, vnet_netlink_msg_t **replies)
{
    ...
    if ((sock = socket(AF_NETLINK, SOCK_RAW, NETLINK_ROUTE)) == -1) return clib_error_return_unix(0, "socket(AF_NETLINK)");
    ...
    if ((send(sock, m->data, vec_len(m->data), 0)) == -1) err = clib_error_return_unix(0, "send");

    if ((len = recv(sock, buf, sizeof(buf), 0)) == -1) err = clib_error_return_unix(0, "recv");
    ...
    for (nh = (struct nlmsghdr *) buf; NLMSG_OK(nh, len); nh = NLMSG_NEXT(nh, len)) {
        ...
        if (nh->nlmsg_type == NLMSG_ERROR) {
            ...
            if (e->error) err = clib_error_return(0, "netlink error %d", e->error);
            goto done;
        }
...
clib_error_t *vnet_netlink_set_link_name(int ifindex, char *new_ifname)
{
    ...
    vnet_netlink_msg_init(&m, RTM_SETLINK, NLM_F_REQUEST, &ifmsg, sizeof(struct ifinfomsg));

    vnet_netlink_msg_add_rtattr(&m, IFLA_IFNAME, new_ifname, strlen(new_ifname) + 1);

    err = vnet_netlink_msg_send(&m, NULL);
    if (err) err = clib_error_return(0, "set link name %U", format_clib_error, err);
    return err;
}
...
clib_error_t *vnet_netlink_set_link_netns(int ifindex, int netns_fd, char *new_ifname)
{
    ...
    if (new_ifname) vnet_netlink_msg_add_rtattr(&m, IFLA_IFNAME, new_ifname, strlen(new_ifname) + 1);

    err = vnet_netlink_msg_send(&m, NULL);
    if (err) err = clib_error_return(0, "set link netns %U", format_clib_error, err);
    return err;
}
...
clib_error_t *vnet_netlink_set_link_master(int ifindex, char *master_ifname)
{
    ...
    if ((i = if_nametoindex(master_ifname)) == 0) clib_error_return_unix(0, "unknown master interface '%s'", master_ifname);

    vnet_netlink_msg_init(&m, RTM_SETLINK, NLM_F_REQUEST, &ifmsg, sizeof(struct ifinfomsg));
    vnet_netlink_msg_add_rtattr(&m, IFLA_MASTER, &i, sizeof(int));
    err = vnet_netlink_msg_send(&m, NULL);
    if (err) err = clib_error_return(0, "set link master %U", format_clib_error, err);
    return err;
}
...
clib_error_t *vnet_netlink_set_link_addr(int ifindex, u8 *mac)
{
    ...
    err = vnet_netlink_msg_send(&m, NULL);
    if (err) err = clib_error_return(0, "set link addr %U", format_clib_error, err);
    return err;
}
...
clib_error_t *vnet_netlink_set_link_state(int ifindex, int up)
{
    ...
    vnet_netlink_msg_init(&m, RTM_SETLINK, NLM_F_REQUEST, &ifmsg, sizeof(struct ifinfomsg));
    err = vnet_netlink_msg_send(&m, NULL);
    if (err) err = clib_error_return(0, "set link state %U", format_clib_error, err);
    return err;
}
...
clib_error_t *vnet_netlink_get_link_mtu(int ifindex, u32 *mtu)
{
    ...
    vnet_netlink_msg_init(&m, RTM_GETLINK, NLM_F_REQUEST, &ifmsg, sizeof(struct ifinfomsg));
    ...
    if (vec_len(replies) != 1) {
        err = clib_error_return(0, "got %d != 1 netlink reply msg", vec_len(replies));
        goto done;
    }

    struct nlmsghdr *nh = (struct nlmsghdr *) replies[0].data;
    if (nh->nlmsg_type != RTM_NEWLINK) {
        err = clib_error_return(0, "netlink reply has wrong type: %d != RTM_NEWLINK", nh->nlmsg_type);
        goto done;
    }
...
clib_error_t *vnet_netlink_set_link_mtu(int ifindex, int mtu)
{
    ...
    vnet_netlink_msg_add_rtattr(&m, IFLA_MTU, &mtu, sizeof(int));
    err = vnet_netlink_msg_send(&m, NULL);
    if (err) err = clib_error_return(0, "set link mtu %U", format_clib_error, err);
    return err;
}
...
clib_error_t *vnet_netlink_add_ip4_addr(int ifindex, void *addr, int pfx_len)
{
    ...
    vnet_netlink_msg_init(&m, RTM_NEWADDR, NLM_F_REQUEST | NLM_F_CREATE | NLM_F_REPLACE, &ifa, sizeof(struct ifaddrmsg));

    vnet_netlink_msg_add_rtattr(&m, IFA_LOCAL, addr, 4);
    vnet_netlink_msg_add_rtattr(&m, IFA_ADDRESS, addr, 4);
    err = vnet_netlink_msg_send(&m, NULL);
    if (err) err = clib_error_return(0, "add ip4 addr %U", format_clib_error, err);
    return err;
}
...
clib_error_t *vnet_netlink_add_ip6_addr(int ifindex, void *addr, int pfx_len)
{
    ...
    err = vnet_netlink_msg_send(&m, NULL);
    if (err) err = clib_error_return(0, "add ip6 addr %U", format_clib_error, err);
    return err;
}
...
clib_error_t *vnet_netlink_add_ip4_route(void *dst, u8 dst_len, void *gw)
{
    ...
    vnet_netlink_msg_init(&m, RTM_NEWROUTE, NLM_F_REQUEST | NLM_F_CREATE | NLM_F_REPLACE, &rtm, sizeof(struct rtmsg));

    vnet_netlink_msg_add_rtattr(&m, RTA_GATEWAY, gw, 4);
    vnet_netlink_msg_add_rtattr(&m, RTA_DST, dst ? dst : dflt, 4);
    err = vnet_netlink_msg_send(&m, NULL);
    if (err) err = clib_error_return(0, "add ip4 route %U", format_clib_error, err);
    return err;
}
...
clib_error_t *vnet_netlink_add_ip6_route(void *dst, u8 dst_len, void *gw)
{
    ...
    err = vnet_netlink_msg_send(&m, NULL);
    if (err) err = clib_error_return(0, "add ip6 route %U", format_clib_error, err);
    return err;
}
...
clib_error_t *vnet_netlink_del_ip4_addr(int ifindex, void *addr, int pfx_len)
{
    ...
    vnet_netlink_msg_init(&m, RTM_DELADDR, NLM_F_REQUEST, &ifa, sizeof(struct ifaddrmsg));

    vnet_netlink_msg_add_rtattr(&m, IFA_LOCAL, addr, 4);
    vnet_netlink_msg_add_rtattr(&m, IFA_ADDRESS, addr, 4);
    err = vnet_netlink_msg_send(&m, NULL);
    if (err) err = clib_error_return(0, "del ip4 addr %U", format_clib_error, err);
    return err;
}
...
clib_error_t *vnet_netlink_del_ip6_addr(int ifindex, void *addr, int pfx_len)
{
    ...
    vnet_netlink_msg_add_rtattr(&m, IFA_ADDRESS, addr, 16);
    err = vnet_netlink_msg_send(&m, NULL);
    if (err) err = clib_error_return(0, "del ip6 addr %U", format_clib_error, err);
    return err;
}
