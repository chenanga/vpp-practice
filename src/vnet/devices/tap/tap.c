...
static clib_error_t *virtio_eth_set_max_frame_size(vnet_main_t *vnm, vnet_hw_interface_t *hi, u32 frame_size)
{
    /* nothing for now */
    return 0;
}
...
static void tap_free(vlib_main_t *vm, virtio_if_t *vif)
{
    ...
    vec_foreach_index (i, vif->vhost_fds)
        if (vif->vhost_fds[i] != -1) close(vif->vhost_fds[i]);
...
void tap_create_if(vlib_main_t *vm, tap_create_if_args_t *args)
{
    ...
    if (args->id != ~0) {
        ...
        args->id = clib_bitmap_next_clear(tm->tap_ids, args->auto_id_offset % TAP_MAX_INSTANCE);
    }
    ...
    if (args->host_namespace) {
        ...
        if ((nfd = clib_netns_open(args->host_namespace)) == -1) {
            ...
            args->error = clib_error_return_unix(0, "clib_netns_open '%s'", args->host_namespace);
            goto error;
        }
        if (clib_setns(nfd) == -1) {
    ...
    if (args->host_if_name != NULL) {
        ...
        clib_memcpy(ifr.ifr_name, host_if_name, clib_min(IFNAMSIZ, vec_len(host_if_name)));
    }
    ...
    tap_log_dbg(vif, "TUNSETIFF fd %d name %s flags 0x%x", tfd, ifr.ifr_ifrn.ifrn_name, ifr.ifr_flags);
    ...
    for (i = 1; i < vif->num_rxqs; i++) {
        ...
        tap_log_dbg(vif, "TUNSETIFF fd %d name %s flags 0x%x", qfd, ifr.ifr_ifrn.ifrn_name, ifr.ifr_flags);
    }

    for (i = 0; i < vif->num_rxqs; i++) {
        tap_log_dbg(vif, "TUNSETVNETHDRSZ: fd %d vnet_hdr_sz %u", vif->tap_fds[i], hdrsz);
        _IOCTL(vif->tap_fds[i], TUNSETVNETHDRSZ, &hdrsz);

        tap_log_dbg(vif, "TUNSETSNDBUF: fd %d sndbuf %d", vif->tap_fds[i], sndbuf);
        _IOCTL(vif->tap_fds[i], TUNSETSNDBUF, &sndbuf);

        tap_log_dbg(vif, "TUNSETOFFLOAD: fd %d offload 0x%lx", vif->tap_fds[i], offload);
    ...
    virtio_log_debug(vif, "VHOST_GET_FEATURES: features 0x%lx", vif->remote_features);
    ...
    if ((vif->remote_features & VIRTIO_FEATURE(VIRTIO_NET_F_MRG_RXBUF)) == 0) {
        ...
        args->error = clib_error_return(0,
                                        "vhost-net backend doesn't support "
    ...
    if (vif->type == VIRTIO_IF_TYPE_TAP) {
        if (ethernet_mac_address_is_zero(args->host_mac_addr.bytes)) ethernet_mac_address_generate(args->host_mac_addr.bytes);
    ...
    if (args->host_ip4_prefix_len) {
        args->error = vnet_netlink_add_ip4_addr(vif->ifindex, &args->host_ip4_addr, args->host_ip4_prefix_len);
        if (args->error) {
            args->rv = VNET_API_ERROR_NETLINK_ERROR;
            goto error;
        }
    ...
    if (args->host_ip6_prefix_len) {
        args->error = vnet_netlink_add_ip6_addr(vif->ifindex, &args->host_ip6_addr, args->host_ip6_prefix_len);
        if (args->error) {
            args->rv = VNET_API_ERROR_NETLINK_ERROR;
            goto error;
        }
    ...
    for (i = 0; i < num_vhost_queues; i++) {
        if (i < vif->num_rxqs && (args->error = virtio_vring_init(vm, vif, RX_QUEUE(i), args->rx_ring_sz))) {
            args->rv = VNET_API_ERROR_INIT_FAILED;
            goto error;
        }

        if (i < vif->num_txqs && (args->error = virtio_vring_init(vm, vif, TX_QUEUE(i), args->tx_ring_sz))) {
            args->rv = VNET_API_ERROR_INIT_FAILED;
            goto error;
        }
    }
    ...
    for (i = 0; i < num_vhost_queues; i++) {
        ...
        virtio_log_debug(vif, "VHOST_SET_FEATURES: fd %u features 0x%lx", fd, vif->features);
    ...
    for (i = 0; i < num_vhost_queues * 2; i++) {
        ...
        if (i & 1) {
            if (qp >= vif->num_txqs) continue;
            vring = vec_elt_at_index(vif->txq_vrings, qp);
        }
        else {
            if (qp >= vif->num_rxqs) continue;
    ...
    if (vif->type == VIRTIO_IF_TYPE_TAP) {
        if (!args->mac_addr_set) ethernet_mac_address_generate(args->mac_addr.bytes);

        clib_memcpy(vif->mac_addr, args->mac_addr.bytes, 6);
        if (args->host_bridge) vif->host_bridge = format(0, "%s%c", args->host_bridge, 0);
    }
    vif->host_if_name = format(0, "%s%c", host_if_name, 0);
    if (args->host_namespace) vif->net_ns = format(0, "%s%c", args->host_namespace, 0);
    ...
    if (args->host_ip4_prefix_len) clib_memcpy(&vif->host_ip4_addr, &args->host_ip4_addr, 4);
    if (args->host_ip6_prefix_len) clib_memcpy(&vif->host_ip6_addr, &args->host_ip6_addr, 16);
    ...
    if (vif->type != VIRTIO_IF_TYPE_TUN) {
        ...
        vif->hw_if_index = vnet_register_interface(vnm,
                                                   virtio_device_class.index,
                                                   vif->dev_instance /* device instance */,
                                                   tun_device_hw_interface_class.index,
                                                   vif->dev_instance);
    }
    ...
    if (args->tap_flags & TAP_FLAG_GSO)
        cc.val |= VNET_HW_IF_CAP_TCP_GSO | VNET_HW_IF_CAP_TX_TCP_CKSUM | VNET_HW_IF_CAP_TX_UDP_CKSUM;
    else if (args->tap_flags & TAP_FLAG_CSUM_OFFLOAD)
...
error:
    ...
    if (vhost_mem) clib_mem_free(vhost_mem);
    if (old_netns_fd != -1) {
...
int tap_delete_if(vlib_main_t *vm, u32 sw_if_index)
{
    ...
    if (hw == NULL || virtio_device_class.index != hw->dev_class_index) return VNET_API_ERROR_INVALID_SW_IF_INDEX;

    vif = pool_elt_at_index(mm->interfaces, hw->dev_instance);

    if ((vif->type != VIRTIO_IF_TYPE_TAP) && (vif->type != VIRTIO_IF_TYPE_TUN)) return VNET_API_ERROR_INVALID_INTERFACE;
...
error:
    if (err) {
        clib_warning("Error %s checksum offload on sw_if_index %d", enable_disable ? "enabling" : "disabling", sw_if_index);
...
int tap_gso_enable_disable(vlib_main_t *vm, u32 sw_if_index, int enable_disable, int is_packet_coalesce)
{
    ...
    if (enable_disable) {
        ...
        vif->gso_enabled          = 1;
        vif->csum_offload_enabled = 1;
        if (is_packet_coalesce) virtio_set_packet_coalesce(vif);
    }
    else {
        cc.val                    = 0;
        vif->gso_enabled          = 0;
...
error:
    if (err) {
        clib_warning("Error %s gso on sw_if_index %d", enable_disable ? "enabling" : "disabling", sw_if_index);
        return VNET_API_ERROR_SYSCALL_ERROR_3;
    }
    return 0;
}
...
int tap_dump_ifs(tap_interface_details_t **out_tapids)
{
    ...
    pool_foreach (vif, mm->interfaces) {
        if ((vif->type != VIRTIO_IF_TYPE_TAP) && (vif->type != VIRTIO_IF_TYPE_TUN)) continue;
        ...
        clib_memcpy(tapid->dev_name, hi->name, MIN(ARRAY_LEN(tapid->dev_name) - 1, vec_len(hi->name)));
        ...
        if (vif->host_if_name) {
            clib_memcpy(tapid->host_if_name, vif->host_if_name, MIN(ARRAY_LEN(tapid->host_if_name) - 1, vec_len(vif->host_if_name)));
        }
        if (vif->net_ns) {
            clib_memcpy(tapid->host_namespace, vif->net_ns, MIN(ARRAY_LEN(tapid->host_namespace) - 1, vec_len(vif->net_ns)));
        }
        if (vif->host_bridge) {
            clib_memcpy(tapid->host_bridge, vif->host_bridge, MIN(ARRAY_LEN(tapid->host_bridge) - 1, vec_len(vif->host_bridge)));
        }
        if (vif->host_ip4_prefix_len) clib_memcpy(tapid->host_ip4_addr.as_u8, &vif->host_ip4_addr, 4);
        tapid->host_ip4_prefix_len = vif->host_ip4_prefix_len;
        if (vif->host_ip6_prefix_len) clib_memcpy(tapid->host_ip6_addr.as_u8, &vif->host_ip6_addr, 16);
...
int tap_set_carrier(u32 hw_if_index, u32 carrier_up)
{
    ...
    if (!ret) vif->host_carrier_up = (carrier_up != 0);
#endif
...
int tap_set_speed(u32 hw_if_index, u32 speed)
{
    ...
    clib_memcpy(ifr.ifr_name, vif->host_if_name, strlen((const char *) vif->host_if_name));
