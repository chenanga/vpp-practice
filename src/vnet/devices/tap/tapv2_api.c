...
static void vl_api_tap_create_v3_t_handler(vl_api_tap_create_v3_t *mp)
{
    ...
    if (mp->host_if_name_set) ap->host_if_name = format(0, "%s%c", mp->host_if_name, 0);

    if (mp->host_mac_addr_set) {
        mac_address_decode(mp->host_mac_addr, &ap->host_mac_addr);
    }

    if (mp->host_namespace_set) ap->host_namespace = format(0, "%s%c", mp->host_namespace, 0);

    if (mp->host_bridge_set) ap->host_bridge = format(0, "%s%c", mp->host_bridge, 0);
    ...
    rv = ap->rv;
    REPLY_MACRO2_END(VL_API_TAP_CREATE_V3_REPLY, ({ rmp->sw_if_index = ap->sw_if_index; }));
}
...
static void vl_api_tap_create_v2_t_handler(vl_api_tap_create_v2_t *mp)
{
    ...
    if (mp->num_rx_queues > 1) ap->num_rx_queues = mp->num_rx_queues;
    ...
    STATIC_ASSERT(((int) TAP_API_FLAG_GSO == (int) TAP_FLAG_GSO), "tap gso api flag mismatch");
    STATIC_ASSERT(((int) TAP_API_FLAG_CSUM_OFFLOAD == (int) TAP_FLAG_CSUM_OFFLOAD), "tap checksum offload api flag mismatch");
    STATIC_ASSERT(((int) TAP_API_FLAG_PERSIST == (int) TAP_FLAG_PERSIST), "tap persist api flag mismatch");
    STATIC_ASSERT(((int) TAP_API_FLAG_ATTACH == (int) TAP_FLAG_ATTACH), "tap attach api flag mismatch");
    STATIC_ASSERT(((int) TAP_API_FLAG_TUN == (int) TAP_FLAG_TUN), "tap tun api flag mismatch");
    STATIC_ASSERT(((int) TAP_API_FLAG_GRO_COALESCE == (int) TAP_FLAG_GRO_COALESCE), "tap gro coalesce api flag mismatch");
    STATIC_ASSERT(((int) TAP_API_FLAG_PACKED == (int) TAP_FLAG_PACKED), "tap packed api flag mismatch");
    STATIC_ASSERT(((int) TAP_API_FLAG_IN_ORDER == (int) TAP_FLAG_IN_ORDER), "tap in-order api flag mismatch");
...
static void vl_api_tap_delete_v2_t_handler(vl_api_tap_delete_v2_t *mp)
{
    ...
    if (!rv) vnet_clear_sw_interface_tag(vnm, sw_if_index);
}
...
static void tap_send_sw_interface_details(vpe_api_main_t *am, vl_api_registration_t *reg, tap_interface_details_t *tap_if, u32 context)
{
    ...
    mp->_vl_msg_id  = htons(REPLY_MSG_ID_BASE + VL_API_SW_INTERFACE_TAP_V2_DETAILS);
    mp->id          = htonl(tap_if->id);
    mp->sw_if_index = htonl(tap_if->sw_if_index);
    mp->tap_flags   = htonl(tap_if->tap_flags);
    clib_memcpy(mp->dev_name, tap_if->dev_name, MIN(ARRAY_LEN(mp->dev_name) - 1, strlen((const char *) tap_if->dev_name)));
    ...
    clib_memcpy(mp->host_if_name, tap_if->host_if_name, MIN(ARRAY_LEN(mp->host_if_name) - 1, strlen((const char *) tap_if->host_if_name)));
    clib_memcpy(mp->host_namespace, tap_if->host_namespace, MIN(ARRAY_LEN(mp->host_namespace) - 1, strlen((const char *) tap_if->host_namespace)));
    clib_memcpy(mp->host_bridge, tap_if->host_bridge, MIN(ARRAY_LEN(mp->host_bridge) - 1, strlen((const char *) tap_if->host_bridge)));
    mp->host_mtu_size = htonl(tap_if->host_mtu_size);
    mac_address_encode(&tap_if->host_mac_addr, mp->host_mac_addr);

    if (tap_if->host_ip4_prefix_len) ip4_address_encode(&tap_if->host_ip4_addr, mp->host_ip4_prefix.address);
    mp->host_ip4_prefix.len = tap_if->host_ip4_prefix_len;
    if (tap_if->host_ip6_prefix_len) ip6_address_encode(&tap_if->host_ip6_addr, mp->host_ip6_prefix.address);
...
static void vl_api_sw_interface_tap_v2_dump_t_handler(vl_api_sw_interface_tap_v2_dump_t *mp)
{
    ...
    vec_foreach (tap_if, tapifs) {
        if ((filter_sw_if_index == ~0) || (tap_if->sw_if_index == filter_sw_if_index)) tap_send_sw_interface_details(am, reg, tap_if, mp->context);
    }
