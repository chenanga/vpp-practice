...
static clib_error_t *pipe_admin_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    ...
    hash_foreach(id, sw_if_index, hi->sub_interface_sw_if_index_by_id, ({ vnet_sw_interface_set_flags(vnm, sw_if_index, flags); }));
...
static_always_inline uword pipe_rx(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    if (node->flags & VLIB_NODE_FLAG_TRACE) vlib_trace_frame_buffers_only(vm, node, from, n_left_from, sizeof(from[0]), sizeof(pipe_rx_trace_t));
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            b0->flags |= VNET_BUFFER_F_L2_HDR_OFFSET_VALID | VNET_BUFFER_F_L3_HDR_OFFSET_VALID;
            b1->flags |= VNET_BUFFER_F_L2_HDR_OFFSET_VALID | VNET_BUFFER_F_L3_HDR_OFFSET_VALID;
...
int vnet_create_pipe_interface(u8 is_specified, u32 user_instance, u32 *parent_sw_if_index, u32 pipe_sw_if_index[2])
{
    ...
    *parent_sw_if_index = hi->sw_if_index;
    slot                = vlib_node_add_named_next_with_slot(vm, hi->tx_node_index, "pipe-rx", VNET_PIPE_TX_NEXT_ETHERNET_INPUT);
...
static clib_error_t *create_pipe_interfaces(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    rv = vnet_create_pipe_interface(is_specified, user_instance, &sw_if_index, pipe_sw_if_index);

    if (rv) return clib_error_return(0, "vnet_create_pipe_interface failed");
...
static clib_error_t *delete_pipe_interfaces(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (rv) return clib_error_return(0, "vnet_delete_pipe_interface failed");
