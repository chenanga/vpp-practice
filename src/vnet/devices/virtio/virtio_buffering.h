...
{
    if (*buffering) return clib_error_return(0, "buffering: already initialized");

    if (!is_pow2(size)) return clib_error_return(0, "buffering: size must be power of 2");

    if (size > 32768) return clib_error_return(0, "buffering: size must be 32768 or lower");

    if (size == 0) size = VIRTIO_BUFFERING_DEFAULT_SIZE;

    virtio_vring_buffering_t *b_temp = 0;
    b_temp                           = (virtio_vring_buffering_t *) clib_mem_alloc(sizeof(virtio_vring_buffering_t));
    if (!b_temp) return clib_error_return(0, "buffering: memory allocation failed");
...
static_always_inline void virtio_vring_buffering_buffers_free(vlib_main_t *vm, virtio_vring_buffering_t *buffering)
{
    ...
    if (n_buffers) {
        vlib_buffer_free_from_ring(vm, buffering->buffers, buffering->front, buffering->size, n_buffers);
...
{
    if (buffering) return buffering->is_enable;
...
{
    if (buffering) buffering->is_enable = is_enable;
}
...
static_always_inline u16 virtio_vring_buffering_store_packets(virtio_vring_buffering_t *buffering, u32 *bi, u16 n_store)
{
    ...
    if (!virtio_vring_buffering_is_enable(buffering) || virtio_vring_buffering_is_full(buffering)) return 0;
...
static_always_inline u32 virtio_vring_buffering_read_from_front(virtio_vring_buffering_t *buffering)
{
    ...
    if (virtio_vring_buffering_is_empty(buffering)) return bi;
...
{
    if (buffering && virtio_vring_buffering_is_timeout(vm, buffering) && virtio_vring_n_buffers(buffering)) {
        ...
        virtio_vring_buffering_set_timeout(vm, buffering, VIRTIO_BUFFERING_TIMEOUT);
    }
}

static_always_inline u8 *virtio_vring_buffering_format(u8 *s, va_list *args)
{
    virtio_vring_buffering_t *buffering = va_arg(*args, virtio_vring_buffering_t *);
    u32                       indent    = format_get_indent(s);

    if (!buffering) return s;
