...
static u8 *format_virtio_device(u8 *s, va_list *args)
{
    ...
    if (verbose) {
        ...
        vec_foreach (vring, vif->rxq_vrings) {
            s = format(s, "\n%U %8u : %llu", format_white_space, indent + 4, RX_QUEUE_ACCESS(vring->queue_id), vring->total_packets);
        }
        s = format(s, "\n%U TX QUEUE : Total Packets", format_white_space, indent + 4);
        vec_foreach (vring, vif->txq_vrings) {
            s = format(s, "\n%U %8u : %llu", format_white_space, indent + 4, TX_QUEUE_ACCESS(vring->queue_id), vring->total_packets);
        }
    }
...
static u8 *format_virtio_tx_trace(u8 *s, va_list *va)
{
    ...
    s = format(s, "%Ubuffer 0x%x: %U\n", format_white_space, indent, t->buffer_index, format_vnet_buffer_no_chain, &t->buffer);
    s = format(s, "%U%U\n", format_white_space, indent, format_generic_header_offset, &t->gho);
    s = format(s, "%U%U", format_white_space, indent, format_ethernet_header_with_length, t->buffer.pre_data, sizeof(t->buffer.pre_data));
    return s;
}
...
static void virtio_tx_trace(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t *b0, u32 bi, int is_tun)
{
    ...
    if (is_tun) {
        ...
        vnet_generic_header_offset_parser(b0, &t->gho, 1, b0->flags & VNET_BUFFER_F_IS_IP4, b0->flags & VNET_BUFFER_F_IS_IP6);

    clib_memcpy_fast(&t->buffer, b0, sizeof(*b0) - sizeof(b0->pre_data));
    clib_memcpy_fast(t->buffer.pre_data, vlib_buffer_get_current(b0), sizeof(t->buffer.pre_data));
}
...
static void virtio_interface_drop_inline(vlib_main_t *vm, virtio_if_t *vif, uword node_index, u32 *buffers, u16 n, virtio_tx_func_error_t error)
{
    ...
    vlib_increment_simple_counter(vnet_main.interface_main.sw_if_counters + VNET_INTERFACE_COUNTER_DROP, vm->thread_index, vif->sw_if_index, n);
...
static void virtio_free_used_device_desc_split(vlib_main_t *vm, vnet_virtio_vring_t *vring, uword node_index)
{
    ...
    while (n_left) {
        ...
        vlib_buffer_free_from_ring(vm, vring->buffers, slot, sz, (n_buffers - slot));
     ...
     */
    if (out_of_order_count) vlib_error_count(vm, node_index, VIRTIO_TX_ERROR_OUT_OF_ORDER, out_of_order_count);

    vring->desc_in_use   = used;
    vring->last_used_idx = last;
}

static void virtio_free_used_device_desc_packed(vlib_main_t *vm, vnet_virtio_vring_t *vring, uword node_index)
{
...
static void virtio_free_used_device_desc(vlib_main_t *vm, vnet_virtio_vring_t *vring, uword node_index, int packed)
{
    if (packed)
...
static void set_checksum_offsets(vlib_buffer_t *b, vnet_virtio_net_hdr_v1_t *hdr, const int is_l2)
{
    ...
    if (b->flags & VNET_BUFFER_F_IS_IP4) {
        ...
        if (oflags & VNET_BUFFER_OFFLOAD_F_IP_CKSUM) ip4->checksum = ip4_header_checksum(ip4);
        ...
        if (oflags & VNET_BUFFER_OFFLOAD_F_TCP_CKSUM) {
            ...
            udp_header_t *udp = (udp_header_t *) (b->data + vnet_buffer(b)->l4_hdr_offset);
...
static void set_gso_offsets(vlib_buffer_t *b, vnet_virtio_net_hdr_v1_t *hdr, const int is_l2)
{
    vnet_buffer_oflags_t oflags        = vnet_buffer(b)->oflags;
    i16                  l4_hdr_offset = vnet_buffer(b)->l4_hdr_offset - b->current_data;
...
static u16 add_buffer_to_slot(vlib_main_t         *vm,
                              ...
                              int                  is_any_layout)
{
    ...
    if (PREDICT_TRUE((b->flags & VLIB_BUFFER_NEXT_PRESENT) == 0)) {
        d->addr  = ((is_pci) ? vlib_buffer_get_current_pa(vm, b) : pointer_to_uword(vlib_buffer_get_current(b))) - hdr_sz;
        ...
        vnet_virtio_vring_desc_t *id    = (vnet_virtio_vring_desc_t *) vlib_buffer_get_current(indirect_desc);
        u32                       count = 1;
        if (is_pci) {
            ...
            while (b->flags & VLIB_BUFFER_NEXT_PRESENT) {
                ...
                if (PREDICT_FALSE(count == VIRTIO_TX_MAX_CHAIN_LEN)) {
                    if (b->flags & VLIB_BUFFER_NEXT_PRESENT) vlib_error_count(vm, node->node_index, VIRTIO_TX_ERROR_TRUNC_PACKET, 1);
                    break;
                }
            }
        }
        ...
        if (n_buffers_in_chain > free_desc_count) return n_buffers_in_chain;
    ...
    if (drop_inline != ~0) virtio_interface_drop_inline(vm, vif, node->node_index, &bi, 1, drop_inline);

    return n_added;
}

static u16 add_buffer_to_slot_packed(vlib_main_t         *vm,
                                     vlib_node_runtime_t *node,
                                     virtio_if_t         *vif,
                                     vnet_virtio_vring_t *vring,
                                     u32                  bi,
                                     ...
                                     int                  is_any_layout)
{
    ...
    if (PREDICT_TRUE((b->flags & VLIB_BUFFER_NEXT_PRESENT) == 0)) {
        ...
        vnet_virtio_vring_packed_desc_t *id    = (vnet_virtio_vring_packed_desc_t *) vlib_buffer_get_current(indirect_desc);
        u32                              count = 1;
        if (is_pci) {
            d->addr  = vlib_physmem_get_pa(vm, id);
            id->addr = vlib_buffer_get_current_pa(vm, b) - hdr_sz;
...
static uword virtio_interface_tx_packed_gso_inline(vlib_main_t         *vm,
                                                   vlib_node_runtime_t *node,
                                                   virtio_if_t         *vif,
                                                   ...
                                                   const int            csum_offload)
{
    ...
    int       is_indirect   = ((vif->features & VIRTIO_FEATURE(VIRTIO_RING_F_INDIRECT_DESC)) != 0);
    int       is_any_layout = ((vif->features & VIRTIO_FEATURE(VIRTIO_F_ANY_LAYOUT)) != 0);
    ...
    if (vif->packet_buffering) {
        ...
        while (n_buffers_left && used < sz) {
            ...
            if (bi == ~0) break;
            n_added =
                add_buffer_to_slot_packed(vm, node, vif, vring, bi, next, hdr_sz, do_gso, csum_offload, is_pci, is_tun, is_indirect, is_any_layout);
            n_buffers_left--;
            if (PREDICT_FALSE(n_added == 0)) continue;
    ...
    while (n_left && used < sz) {
        ...
        n_added = add_buffer_to_slot_packed(vm,
                                            node,
                                            vif,
                                            vring,
                                            buffers[0],
    ...
    if (n_left != n_vectors || n_buffers != n_buffers_left) {
        ...
        vring->desc_in_use = used;
        CLIB_MEMORY_BARRIER();
        if (vring->device_event->flags != VRING_EVENT_F_DISABLE) virtio_kick(vm, vring, vif);
    }

    return n_left;
}

static void virtio_find_free_desc(vnet_virtio_vring_t *vring, u16 size, u16 mask, u16 req, u16 next, u32 *first_free_desc_index, u16 *free_desc_count)
{
    u16 start = 0;
...
static u16 virtio_interface_tx_split_gso_inline(vlib_main_t         *vm,
                                                vlib_node_runtime_t *node,
                                                virtio_if_t         *vif,
                                                virtio_if_type_t     type,
                                                vnet_virtio_vring_t *vring,
                                                ...
                                                int                  csum_offload)
{
    ...
    if (PREDICT_FALSE(vring->flags & VRING_TX_OUT_OF_ORDER)) {
        ...
        virtio_find_free_desc(vring, sz, mask, n_left, next, &first_free_desc_index, &free_desc_count);

        if (free_desc_count) next = first_free_desc_index;
    }
    else
    ...
    if (n_left != n_vectors || n_buffers != n_buffers_left) {
        ...
        vring->desc_next   = next;
        vring->desc_in_use = used;
        if ((clib_atomic_load_seq_cst(&vring->used->flags) & VRING_USED_F_NO_NOTIFY) == 0) virtio_kick(vm, vring, vif);
    }

    return n_left;
}

static u16 virtio_interface_tx_gso_inline(vlib_main_t         *vm,
                                          vlib_node_runtime_t *node,
                                          virtio_if_t         *vif,
                                          virtio_if_type_t     type,
                                          vnet_virtio_vring_t *vring,
...
{
    if (packed)
        return virtio_interface_tx_packed_gso_inline(vm, node, vif, type, vring, buffers, n_left, do_gso, csum_offload);
    else
        return virtio_interface_tx_split_gso_inline(vm, node, vif, type, vring, buffers, n_left, do_gso, csum_offload);
}

static u16 virtio_interface_tx_inline(vlib_main_t         *vm,
                                      vlib_node_runtime_t *node,
                                      virtio_if_t         *vif,
                                      vnet_virtio_vring_t *vring,
                                      virtio_if_type_t     type,
                                      ...
                                      int                  packed)
{
    ...
    if (hw->caps & VNET_HW_IF_CAP_TCP_GSO)
        return virtio_interface_tx_gso_inline(vm, node, vif, type, vring, buffers, n_left, packed, 1 /* do_gso */, 1 /* checksum offload */);
    else if (hw->caps & VNET_HW_IF_CAP_L4_TX_CKSUM)
        return virtio_interface_tx_gso_inline(vm, node, vif, type, vring, buffers, n_left, packed, 0 /* no do_gso */, 1 /* checksum offload */);
    else
        return virtio_interface_tx_gso_inline(vm, node, vif, type, vring, buffers, n_left, packed, 0 /* no do_gso */, 0 /* no checksum offload */);
}

VNET_DEVICE_CLASS_TX_FN(virtio_device_class)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    if (tf->shared_queue) clib_spinlock_lock(&vring->lockp);

    if (vif->packet_coalesce) {
        n_vectors = n_left = vnet_gro_inline(vm, vring->flow_table, buffers, n_left, to);
        buffers            = to;
        virtio_txq_clear_scheduled(vring);
    }
...
retry:
    ...
    if (vif->type == VIRTIO_IF_TYPE_TAP)
        n_left = virtio_interface_tx_inline(vm, node, vif, vring, VIRTIO_IF_TYPE_TAP, &buffers[n_vectors - n_left], n_left, packed);
    else if (vif->type == VIRTIO_IF_TYPE_PCI)
        n_left = virtio_interface_tx_inline(vm, node, vif, vring, VIRTIO_IF_TYPE_PCI, &buffers[n_vectors - n_left], n_left, packed);
    else if (vif->type == VIRTIO_IF_TYPE_TUN)
        n_left = virtio_interface_tx_inline(vm, node, vif, vring, VIRTIO_IF_TYPE_TUN, &buffers[n_vectors - n_left], n_left, packed);
    else
        ASSERT(0);

    if (n_left && retry_count--) goto retry;

    if (vif->packet_buffering && n_left) {
        u16 n_buffered = virtio_vring_buffering_store_packets(vring->buffering, &buffers[n_vectors - n_left], n_left);
        n_left -= n_buffered;
    }
    if (n_left) virtio_interface_drop_inline(vm, vif, node->node_index, &buffers[n_vectors - n_left], n_left, VIRTIO_TX_ERROR_NO_FREE_SLOTS);

    if (tf->shared_queue) clib_spinlock_unlock(&vring->lockp);
...
static void virtio_set_interface_next_node(vnet_main_t *vnm, u32 hw_if_index, u32 node_index)
{
    ...
    vif->per_interface_next_index = vlib_node_add_next(vlib_get_main(), virtio_input_node.index, node_index);
}
...
static void virtio_set_rx_interrupt(virtio_if_t *vif, vnet_virtio_vring_t *vring)
{
    if (vif->is_packed)
...
static void virtio_set_rx_polling(virtio_if_t *vif, vnet_virtio_vring_t *vring)
{
    if (vif->is_packed)
        vring->driver_event->flags |= VRING_EVENT_F_DISABLE;
    else
