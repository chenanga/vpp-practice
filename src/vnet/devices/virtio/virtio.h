...
extern void   virtio_show(vlib_main_t *vm, u32 *hw_if_indices, u8 show_descr, virtio_if_type_t type);
extern void   virtio_set_packet_coalesce(virtio_if_t *vif);
clib_error_t *virtio_set_packet_buffering(virtio_if_t *vif, u16 size);
extern void   virtio_pci_legacy_notify_queue(vlib_main_t *vm, virtio_if_t *vif, u16 queue_id, u16 queue_notify_offset);
extern void   virtio_pci_modern_notify_queue(vlib_main_t *vm, virtio_if_t *vif, u16 queue_id, u16 queue_notify_offset);
...
static_always_inline void virtio_kick(vlib_main_t *vm, vnet_virtio_vring_t *vring, virtio_if_t *vif)
{
    if (vif->type == VIRTIO_IF_TYPE_PCI) {
        if (vif->is_modern)
            virtio_pci_modern_notify_queue(vm, vif, vring->queue_id, vring->queue_notify_offset);
        else
            virtio_pci_legacy_notify_queue(vm, vif, vring->queue_id, vring->queue_notify_offset);
    }
    else {
        u64               x = 1;
        int __clib_unused r;
...
static_always_inline u8 virtio_txq_is_scheduled(vnet_virtio_vring_t *vring)
{
    if (vring) return (vring->flags & VRING_TX_SCHEDULED);
    return 1;
}

static_always_inline void virtio_txq_set_scheduled(vnet_virtio_vring_t *vring)
{
    if (vring) vring->flags |= VRING_TX_SCHEDULED;
}

static_always_inline void virtio_txq_clear_scheduled(vnet_virtio_vring_t *vring)
{
    if (vring) vring->flags &= ~VRING_TX_SCHEDULED;
}

static_always_inline void vnet_virtio_vring_init(vnet_virtio_vring_t *vring, u16 queue_size, void *p, u32 align)
{
    vring->queue_size   = queue_size;
    vring->desc         = p;
    vring->avail        = (vnet_virtio_vring_avail_t *) ((char *) p + queue_size * sizeof(vnet_virtio_vring_desc_t));
    vring->used         = (vnet_virtio_vring_used_t *) ((char *) p
                                                + ((sizeof(vnet_virtio_vring_desc_t) * queue_size + sizeof(u16) * (3 + queue_size) + align - 1)
                                                   & ~(align - 1)));
    vring->avail->flags = VIRTIO_RING_FLAG_MASK_INT;
}

static_always_inline u16 vnet_virtio_vring_size(u16 queue_size, u32 align)
{
    return ((sizeof(vnet_virtio_vring_desc_t) * queue_size + sizeof(u16) * (3 + queue_size) + align - 1) & ~(align - 1)) + sizeof(u16) * 3
           + sizeof(vnet_virtio_vring_used_elem_t) * queue_size;
}

#define virtio_log_debug(vif, f, ...)                                                                                  \
    {                                                                                                                  \
        vlib_log(VLIB_LOG_LEVEL_DEBUG, virtio_main.log_default, "%U: " f, format_virtio_log_name, vif, ##__VA_ARGS__); \
    };

#define virtio_log_warning(vif, f, ...)                                                                                  \
    {                                                                                                                    \
        vlib_log(VLIB_LOG_LEVEL_WARNING, virtio_main.log_default, "%U: " f, format_virtio_log_name, vif, ##__VA_ARGS__); \
    };

#define virtio_log_error(vif, f, ...)                                                                                \
    {                                                                                                                \
        vlib_log(VLIB_LOG_LEVEL_ERR, virtio_main.log_default, "%U: " f, format_virtio_log_name, vif, ##__VA_ARGS__); \
    };
