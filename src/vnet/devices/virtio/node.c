...
static u8 *format_virtio_input_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "virtio: hw_if_index %d next-index %d vring %u len %u", t->hw_if_index, t->next_index, t->ring, t->len);
    s = format(s,
...
static_always_inline void virtio_needs_csum(vlib_buffer_t *b0, vnet_virtio_net_hdr_v1_t *hdr, u8 *l4_proto, u8 *l4_hdr_sz, virtio_if_type_t type)
{
...
static_always_inline void fill_gso_buffer_flags(vlib_buffer_t *b0, vnet_virtio_net_hdr_v1_t *hdr, u8 l4_proto, u8 l4_hdr_sz)
{
...
static_always_inline u16 virtio_n_left_to_process(vnet_virtio_vring_t *vring, const int packed)
{
    if (packed)
...
static_always_inline u16 virtio_get_slot_id(vnet_virtio_vring_t *vring, const int packed, u16 last, u16 mask)
{
    if (packed)
...
static_always_inline u16 virtio_get_len(vnet_virtio_vring_t *vring, const int packed, const int hdr_sz, u16 last, u16 mask)
{
    if (packed)
...
#define virtio_packed_check_n_left(vring, last)                                                                                                   \
    do {                                                                                                                                          \
        ...
        u16                              flags = d->flags;                                                                                        \
        if ((flags & VRING_DESC_F_AVAIL) != (vring->used_wrap_counter << 7) || (flags & VRING_DESC_F_USED) != (vring->used_wrap_counter << 15)) { \
            n_left = 0;                                                                                                                           \
        }                                                                                                                                         \
    } while (0)
...
static_always_inline void virtio_device_input_ethernet(vlib_main_t         *vm,
                                                          ...
                                                          int                  packed)
{
    ...
    if (PREDICT_FALSE(n_left == vring->queue_size)) {
         ...
         * enough.
         */
        vlib_error_count(vm, node->node_index, VIRTIO_INPUT_ERROR_FULL_RX_QUEUE, 1);
    }
    ...
    if (type == VIRTIO_IF_TYPE_TUN) {
        ...
        if (PREDICT_FALSE(vif->per_interface_next_index != ~0)) next_index = vif->per_interface_next_index;
    ...
    while (n_left) {
        ...
        while (n_left && n_left_to_next) {
            ...
            if (hdr_sz == sizeof(vnet_virtio_net_hdr_v1_t)) num_buffers = hdr->num_buffers;

            b0->flags          = VLIB_BUFFER_TOTAL_LENGTH_VALID;
            b0->current_data   = 0;
            b0->current_length = len;

            if (checksum_offload_enabled) virtio_needs_csum(b0, hdr, &l4_proto, &l4_hdr_sz, type);

            if (gso_enabled) fill_gso_buffer_flags(b0, hdr, l4_proto, l4_hdr_sz);
            ...
            if (type == VIRTIO_IF_TYPE_TUN) {
                ...
                if (PREDICT_FALSE(vif->per_interface_next_index != ~0)) next0 = vif->per_interface_next_index;
            }
            else {
                /* copy feature arc data from template */
                b0->current_config_index           = bt.current_config_index;
                vnet_buffer(b0)->feature_arc_index = vnet_buffer(&bt)->feature_arc_index;
            }
            ...
            if (type == VIRTIO_IF_TYPE_TUN) vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, bi0, next0);

            /* next packet */
            n_rx_packets++;
            n_rx_bytes += len;
        }
        virtio_device_input_ethernet(vm, node, next_index, vif->sw_if_index, vif->hw_if_index);
...
virtio_device_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, virtio_if_t *vif, u16 qid, virtio_if_type_t type)
{
    ...
    if (vif->is_packed) {
        if (vif->gso_enabled)
            rv = virtio_device_input_gso_inline(vm, node, frame, vif, vring, type, 1, 1, 1);
        else if (vif->csum_offload_enabled)
            rv = virtio_device_input_gso_inline(vm, node, frame, vif, vring, type, 0, 1, 1);
        else
            rv = virtio_device_input_gso_inline(vm, node, frame, vif, vring, type, 0, 0, 1);

        virtio_refill_vring_packed(vm, vif, type, vring, hdr_sz, node->node_index);
    }
    else {
        if (vif->gso_enabled)
            rv = virtio_device_input_gso_inline(vm, node, frame, vif, vring, type, 1, 1, 0);
        else if (vif->csum_offload_enabled)
            rv = virtio_device_input_gso_inline(vm, node, frame, vif, vring, type, 0, 1, 0);
        else
            rv = virtio_device_input_gso_inline(vm, node, frame, vif, vring, type, 0, 0, 0);

        virtio_refill_vring_split(vm, vif, type, vring, hdr_sz, node->node_index);
    }
    return rv;
}
    ...
    vec_foreach (p, pv) {
        ...
        if (vif->flags & VIRTIO_IF_FLAG_ADMIN_UP) {
            if (vif->type == VIRTIO_IF_TYPE_TAP)
                n_rx += virtio_device_input_inline(vm, node, frame, vif, p->queue_id, VIRTIO_IF_TYPE_TAP);
            else if (vif->type == VIRTIO_IF_TYPE_PCI)
                n_rx += virtio_device_input_inline(vm, node, frame, vif, p->queue_id, VIRTIO_IF_TYPE_PCI);
            else if (vif->type == VIRTIO_IF_TYPE_TUN)
                n_rx += virtio_device_input_inline(vm, node, frame, vif, p->queue_id, VIRTIO_IF_TYPE_TUN);
        }
    }
