...
clib_error_t *virtio_vring_init(vlib_main_t *vm, virtio_if_t *vif, u16 idx, u16 sz)
{
    ...
    if (!is_pow2(sz)) return clib_error_return(0, "ring size must be power of 2");

    if (sz > 32768) return clib_error_return(0, "ring size must be 32768 or lower");
    ...
    virtio_log_debug(vif, "vring %u size %u call_fd %d kick_fd %d", idx, vring->queue_size, vring->call_fd, vring->kick_fd);

    return 0;
}

inline void virtio_free_buffers(vlib_main_t *vm, vnet_virtio_vring_t *vring)
{
...
{
    vnet_virtio_vring_t *vring = vec_elt_at_index(vif->rxq_vrings, RX_QUEUE_ACCESS(idx));
    ...
    if (vring->desc) clib_mem_free(vring->desc);
    if (vring->avail) clib_mem_free(vring->avail);
...
{
    vnet_virtio_vring_t *vring = vec_elt_at_index(vif->txq_vrings, TX_QUEUE_ACCESS(idx));
...
void virtio_set_packet_coalesce(virtio_if_t *vif)
{
    ...
    vec_foreach (vring, vif->txq_vrings) {
        gro_flow_table_init(&vring->flow_table, vif->type & (VIRTIO_IF_TYPE_TAP | VIRTIO_IF_TYPE_PCI), hw->tx_node_index);
    }
}
...
clib_error_t *virtio_set_packet_buffering(virtio_if_t *vif, u16 buffering_size)
{
    ...
    vec_foreach (vring, vif->txq_vrings) {
        if ((error = virtio_vring_buffering_init(&vring->buffering, hw->tx_node_index, buffering_size))) {
            break;
        }
    }
...
static void virtio_vring_fill(vlib_main_t *vm, virtio_if_t *vif, vnet_virtio_vring_t *vring)
{
    if (vif->is_packed)
        virtio_refill_vring_packed(vm, vif, vif->type, vring, vif->virtio_net_hdr_sz, virtio_input_node.index);
    else
        virtio_refill_vring_split(vm, vif, vif->type, vring, vif->virtio_net_hdr_sz, virtio_input_node.index);
}
...
void virtio_vring_set_rx_queues(vlib_main_t *vm, virtio_if_t *vif)
{
    ...
    vec_foreach (vring, vif->rxq_vrings) {
        vring->queue_index       = vnet_hw_if_register_rx_queue(vnm, vif->hw_if_index, RX_QUEUE_ACCESS(vring->queue_id), VNET_HW_IF_RXQ_THREAD_ANY);
        vring->buffer_pool_index = vlib_buffer_pool_get_default_for_numa(vm, vnet_hw_if_get_rx_queue_numa_node(vnm, vring->queue_index));
        if (vif->type == VIRTIO_IF_TYPE_TAP || vif->type == VIRTIO_IF_TYPE_TUN) {
            clib_file_t f = {
                ...
                .description     = format(0, "%U vring %u", format_virtio_device_name, vif->dev_instance, vring->queue_id),
            };

            vring->call_file_index = clib_file_add(&file_main, &f);
            vnet_hw_if_set_rx_queue_file_index(vnm, vring->queue_index, vring->call_file_index);
        }
        else if ((vif->type == VIRTIO_IF_TYPE_PCI) && (vif->support_int_mode) && (vif->msix_enabled == VIRTIO_MSIX_ENABLED)) {
            u32 file_index;
            file_index = vlib_pci_get_msix_file_index(vm, vif->pci_dev_handle, i + 1);
            vnet_hw_if_set_rx_queue_file_index(vnm, vring->queue_index, file_index);
            i++;
        }
        vnet_hw_if_set_rx_queue_mode(vnm, vring->queue_index, VNET_HW_IF_RX_MODE_POLLING);
...
void virtio_vring_set_tx_queues(vlib_main_t *vm, virtio_if_t *vif)
{
    ...
    vec_foreach (vring, vif->txq_vrings) {
        vring->queue_index = vnet_hw_if_register_tx_queue(vnm, vif->hw_if_index, TX_QUEUE_ACCESS(vring->queue_id));
    }

    if (vif->num_txqs == 0) {
        virtio_log_error(vif, "Interface %U has 0 txq", format_vnet_hw_if_index_name, vnm, vif->hw_if_index);
        return;
    }
...
inline void virtio_set_net_hdr_size(virtio_if_t *vif)
{
    if (vif->features & VIRTIO_FEATURE(VIRTIO_NET_F_MRG_RXBUF) || vif->features & VIRTIO_FEATURE(VIRTIO_F_VERSION_1))
        vif->virtio_net_hdr_sz = sizeof(vnet_virtio_net_hdr_v1_t);
    else
        vif->virtio_net_hdr_sz = sizeof(vnet_virtio_net_hdr_t);
}

inline void virtio_show(vlib_main_t *vm, u32 *hw_if_indices, u8 show_descr, virtio_if_type_t type)
{
    u32                  i, j, hw_if_index;
    virtio_if_t         *vif;
...
#undef _
    ...
    if (!hw_if_indices) return;

    for (hw_if_index = 0; hw_if_index < vec_len(hw_if_indices); hw_if_index++) {
        vnet_hw_interface_t *hi = vnet_get_hw_interface(vnm, hw_if_indices[hw_if_index]);
        vif                     = pool_elt_at_index(mm->interfaces, hi->dev_instance);
        if (vif->type != type) continue;
        vlib_cli_output(vm, "Interface: %U (ifindex %d)", format_vnet_hw_if_index_name, vnm, hw_if_indices[hw_if_index], vif->hw_if_index);
        if (type == VIRTIO_IF_TYPE_PCI) {
            vlib_cli_output(vm, "  PCI Address: %U", format_vlib_pci_addr, &vif->pci_addr);
        }
        if (type & (VIRTIO_IF_TYPE_TAP | VIRTIO_IF_TYPE_TUN)) {
            u8 *str = 0;
            if (vif->host_if_name) vlib_cli_output(vm, "  name \"%s\"", vif->host_if_name);
            if (vif->net_ns) vlib_cli_output(vm, "  host-ns \"%s\"", vif->net_ns);
            if (vif->host_mtu_size) vlib_cli_output(vm, "  host-mtu-size \"%d\"", vif->host_mtu_size);
            if (type == VIRTIO_IF_TYPE_TAP) vlib_cli_output(vm, "  host-mac-addr: %U", format_ethernet_address, vif->host_mac_addr);
        ...
        if (type & (VIRTIO_IF_TYPE_TAP | VIRTIO_IF_TYPE_PCI)) vlib_cli_output(vm, "  Mac Address: %U", format_ethernet_address, vif->mac_addr);
        ...
        while (flag_entry->str) {
            if (vif->flags & (1ULL << flag_entry->bit)) vlib_cli_output(vm, "    %s (%d)", flag_entry->str, flag_entry->bit);
            flag_entry++;
        }
        if (type == VIRTIO_IF_TYPE_PCI) {
        ...
        while (feat_entry->str) {
            if (vif->features & (1ULL << feat_entry->bit)) vlib_cli_output(vm, "    %s (%d)", feat_entry->str, feat_entry->bit);
            feat_entry++;
        }
        vlib_cli_output(vm, "  remote-features 0x%lx", vif->remote_features);
        feat_entry = (struct feat_struct *) &feat_array;
        while (feat_entry->str) {
            if (vif->remote_features & (1ULL << feat_entry->bit)) vlib_cli_output(vm, "    %s (%d)", feat_entry->str, feat_entry->bit);
            feat_entry++;
        }
        vlib_cli_output(vm, "  Number of RX Virtqueue  %u", vif->num_rxqs);
        vlib_cli_output(vm, "  Number of TX Virtqueue  %u", vif->num_txqs);
        if (type == VIRTIO_IF_TYPE_PCI && vif->cxq_vring != NULL && vif->features & VIRTIO_FEATURE(VIRTIO_NET_F_CTRL_VQ))
        ...
        vec_foreach_index (i, vif->rxq_vrings) {
            ...
            if (vif->is_packed) {
                ...
                vlib_cli_output(vm, "    avail wrap counter %d, used wrap counter %d", vring->avail_wrap_counter, vring->used_wrap_counter);
            }
            else
                vlib_cli_output(vm,
                                ...
                                vring->used->idx);
            if (type & (VIRTIO_IF_TYPE_TAP | VIRTIO_IF_TYPE_TUN)) {
                vlib_cli_output(vm, "    kickfd %d, callfd %d", vring->kick_fd, vring->call_fd);
            }
            if (show_descr) {
                vlib_cli_output(vm, "\n  descriptor table:\n");
                ...
                for (j = 0; j < vring->queue_size; j++) {
                    if (vif->is_packed) {
                        vnet_virtio_vring_packed_desc_t *desc = &vring->packed_desc[j];
                        vlib_cli_output(vm,
        ...
        vec_foreach_index (i, vif->txq_vrings) {
            ...
            if (vring->flow_table) {
                vlib_cli_output(vm, "    %U", gro_flow_table_format, vring->flow_table);
            }
            if (vif->packet_buffering) {
                vlib_cli_output(vm, "    %U", virtio_vring_buffering_format, vring->buffering);
            }
            if (show_descr) {
                vlib_cli_output(vm, "\n  descriptor table:\n");
                vlib_cli_output(vm, "   id          addr         len  flags  next/id      user_addr\n");
