...
static const u8 virtio_rss_key[VIRTIO_NET_RSS_MAX_KEY_SIZE] = {
    0x6d, 0x5a, 0x56, 0xda, 0x25, 0x5b, 0x0e, 0xc2, 0x41, 0x67, 0x25, 0x3d, 0x43, 0xa3, 0x8f, 0xb0, 0xd0, 0xca, 0x2b, 0xcb,
    0xae, 0x7b, 0x30, 0xb4, 0x77, 0xcb, 0x2d, 0xa3, 0x80, 0x30, 0xf2, 0x0c, 0x6a, 0x42, 0xb7, 0x3b, 0xbe, 0xac, 0x01, 0xfa,
};

static pci_device_id_t virtio_pci_device_ids[] = {
    {.vendor_id = PCI_VENDOR_ID_VIRTIO, .device_id = PCI_DEVICE_ID_VIRTIO_NIC},
    {.vendor_id = PCI_VENDOR_ID_VIRTIO, .device_id = PCI_DEVICE_ID_VIRTIO_NIC_MODERN},
    {0},
};
...
{
    if (vif->features & VIRTIO_FEATURE(VIRTIO_NET_F_MAC)) vif->virtio_pci_func->set_mac(vm, vif);
}
...
static u16 virtio_pci_is_link_up(vlib_main_t *vm, virtio_if_t *vif)
{
     ...
     */
    u16 status = 1;
    if (vif->features & VIRTIO_FEATURE(VIRTIO_NET_F_STATUS)) status = vif->virtio_pci_func->get_device_status(vm, vif);
    return status;
}
...
static void virtio_pci_irq_handler(vlib_main_t *vm, vlib_pci_dev_handle_t h)
{
    ...
    if (isr & VIRTIO_PCI_ISR_CONFIG) virtio_pci_irq_config_handler(vm, h, line);
}

inline void device_status(vlib_main_t *vm, virtio_if_t *vif)
{
...
#undef _
    ...
    while (status_entry->str) {
        if (vif->status & status_entry->bit) vlib_cli_output(vm, "    %s (%x)", status_entry->str, status_entry->bit);
        status_entry++;
    }
}
...
static int virtio_pci_send_ctrl_msg_packed(vlib_main_t *vm, virtio_if_t *vif, virtio_ctrl_msg_t *data, u32 len)
{
    ...
    d->addr  = vlib_buffer_get_current_pa(vm, b) + STRUCT_OFFSET_OF(virtio_ctrl_msg_t, data);
    d->len   = len;
    d->id    = next;
    d->flags = flags;
    ...
    d->addr  = vlib_buffer_get_current_pa(vm, b) + STRUCT_OFFSET_OF(virtio_ctrl_msg_t, status);
    ...
    do {
        flags = d->flags;
    } while ((flags & VRING_DESC_F_AVAIL) != (vring->used_wrap_counter << 7) || (flags & VRING_DESC_F_USED) != (vring->used_wrap_counter << 15));
    ...
    CLIB_MEMORY_BARRIER();
    clib_memcpy(&result, vlib_buffer_get_current(b), sizeof(virtio_ctrl_msg_t));
...
static int virtio_pci_disable_offload(vlib_main_t *vm, virtio_if_t *vif)
{
    ...
    status = virtio_pci_send_ctrl_msg(vm, vif, &offload_hdr, sizeof(offloads));
...
static int virtio_pci_enable_checksum_offload(vlib_main_t *vm, virtio_if_t *vif)
{
    ...
    status = virtio_pci_send_ctrl_msg(vm, vif, &csum_offload_hdr, sizeof(offloads));
...
static int virtio_pci_enable_gso(vlib_main_t *vm, virtio_if_t *vif)
{
    ...
    u64 offloads       = VIRTIO_FEATURE(VIRTIO_NET_F_GUEST_CSUM) | VIRTIO_FEATURE(VIRTIO_NET_F_GUEST_TSO4) | VIRTIO_FEATURE(VIRTIO_NET_F_GUEST_TSO6);
...
static int virtio_pci_offloads(vlib_main_t *vm, virtio_if_t *vif, int gso_enabled, int csum_offload_enabled)
{
    ...
    if ((vif->features & VIRTIO_FEATURE(VIRTIO_NET_F_CTRL_VQ)) && (vif->features & VIRTIO_FEATURE(VIRTIO_NET_F_CTRL_GUEST_OFFLOADS))) {
        if (gso_enabled && (vif->features & (VIRTIO_FEATURE(VIRTIO_NET_F_HOST_TSO4) | VIRTIO_FEATURE(VIRTIO_NET_F_HOST_TSO6)))) {
        ...
        else if (csum_offload_enabled && (vif->features & VIRTIO_FEATURE(VIRTIO_NET_F_CSUM))) {
    ...
    if (cc.mask) vnet_hw_if_change_caps(vnm, vif->hw_if_index, &cc);
...
static int virtio_pci_enable_multiqueue_rss(vlib_main_t *vm, virtio_if_t *vif, u16 num_queues)
{
    virtio_ctrl_msg_t      mq_hdr;
    virtio_net_rss_config *rss    = (virtio_net_rss_config *) mq_hdr.data;
    virtio_net_ctrl_ack_t  status = VIRTIO_NET_ERR;

    STATIC_ASSERT(sizeof(*rss) <= sizeof(mq_hdr.data), "virtio_net_rss_config size too big");
    mq_hdr.ctrl.class = VIRTIO_NET_CTRL_MQ;
    ...
    clib_memcpy(rss->hash_key_data, virtio_rss_key, VIRTIO_NET_RSS_MAX_KEY_SIZE);
...
clib_error_t *virtio_pci_control_vring_packed_init(vlib_main_t *vm, virtio_if_t *vif, u16 queue_num)
{
    ...
    if (queue_size > 32768) return clib_error_return(0, "ring size must be 32768 or lower");
    ...
    i = (((queue_size * sizeof(vnet_virtio_vring_packed_desc_t)) + sizeof(vnet_virtio_vring_desc_event_t) + VNET_VIRTIO_PCI_VRING_ALIGN - 1)
         & ~(VNET_VIRTIO_PCI_VRING_ALIGN - 1))
        + sizeof(vnet_virtio_vring_desc_event_t);

    ptr = vlib_physmem_alloc_aligned_on_numa(vm, i, VNET_VIRTIO_PCI_VRING_ALIGN, vif->numa_node);
    if (!ptr) return vlib_physmem_last_error(vm);
    clib_memset(ptr, 0, i);

    vring->packed_desc = ptr;

    vring->driver_event           = ptr + (queue_size * sizeof(vnet_virtio_vring_packed_desc_t));
    vring->driver_event->off_wrap = 0;
    vring->driver_event->flags    = VRING_EVENT_F_DISABLE;

    vring->device_event =
        ptr
        + (((queue_size * sizeof(vnet_virtio_vring_packed_desc_t)) + sizeof(vnet_virtio_vring_desc_event_t) + VNET_VIRTIO_PCI_VRING_ALIGN - 1)
           & ~(VNET_VIRTIO_PCI_VRING_ALIGN - 1));
    ...
    virtio_log_debug(vif, "control-queue: number %u, size %u", queue_num, queue_size);
    vif->virtio_pci_func->setup_queue(vm, vif, queue_num, vring);
    vring->queue_notify_offset = vif->notify_off_multiplier * vif->virtio_pci_func->get_queue_notify_off(vm, vif, queue_num);
    virtio_log_debug(vif, "queue-notify-offset: number %u, offset %u", queue_num, vring->queue_notify_offset);
    return error;
}
...
clib_error_t *virtio_pci_control_vring_split_init(vlib_main_t *vm, virtio_if_t *vif, u16 queue_num)
{
    ...
    queue_size = vif->virtio_pci_func->get_queue_size(vm, vif, queue_num);
    if (!virtio_pci_queue_size_valid(queue_size)) clib_warning("queue size is not valid");

    if (!is_pow2(queue_size)) return clib_error_return(0, "ring size must be power of 2");
...
clib_error_t *virtio_pci_vring_split_init(vlib_main_t *vm, virtio_if_t *vif, u16 queue_num, u16 txq_size)
{
    clib_error_t        *error      = 0;
    u16                  queue_size = 0;
    vnet_virtio_vring_t *vring;
    ...
    if (queue_num % 2) {
        if (txq_size) {
            virtio_log_debug(vif, "tx-queue: number %u, default-size %u", queue_num, queue_size);
            vif->virtio_pci_func->set_queue_size(vm, vif, queue_num, txq_size);
            queue_size = vif->virtio_pci_func->get_queue_size(vm, vif, queue_num);
            virtio_log_debug(vif, "tx-queue: number %u, new size %u", queue_num, queue_size);
        }
        vec_validate_aligned(vif->txq_vrings, TX_QUEUE_ACCESS(queue_num), CLIB_CACHE_LINE_BYTES);
        vring = vec_elt_at_index(vif->txq_vrings, TX_QUEUE_ACCESS(queue_num));
        clib_spinlock_init(&vring->lockp);
    }
    else {
        vec_validate_aligned(vif->rxq_vrings, RX_QUEUE_ACCESS(queue_num), CLIB_CACHE_LINE_BYTES);
    ...
    if (queue_num % 2) {
        virtio_log_debug(vif, "tx-queue: number %u, size %u", queue_num, queue_size);
        clib_memset_u32(vring->buffers, ~0, queue_size);
    }
    else {
        virtio_log_debug(vif, "rx-queue: number %u, size %u", queue_num, queue_size);
    }
    vring->queue_size = queue_size;
    if (vif->virtio_pci_func->setup_queue(vm, vif, queue_num, vring)) return clib_error_return(0, "error in queue address setup");
...
clib_error_t *virtio_pci_vring_init(vlib_main_t *vm, virtio_if_t *vif, u16 queue_num, u16 txq_size)
{
    if (vif->is_packed)
...
static void virtio_negotiate_features(vlib_main_t *vm, virtio_if_t *vif, u64 req_features)
{
    ...
    u64 supported_features = VIRTIO_FEATURE(VIRTIO_NET_F_CSUM) | VIRTIO_FEATURE(VIRTIO_NET_F_GUEST_CSUM)
                             | VIRTIO_FEATURE(VIRTIO_NET_F_CTRL_GUEST_OFFLOADS) | VIRTIO_FEATURE(VIRTIO_NET_F_MTU) | VIRTIO_FEATURE(VIRTIO_NET_F_MAC)
                             | VIRTIO_FEATURE(VIRTIO_NET_F_GSO) | VIRTIO_FEATURE(VIRTIO_NET_F_GUEST_TSO4) | VIRTIO_FEATURE(VIRTIO_NET_F_GUEST_TSO6)
                             | VIRTIO_FEATURE(VIRTIO_NET_F_GUEST_UFO) | VIRTIO_FEATURE(VIRTIO_NET_F_HOST_TSO4)
                             | VIRTIO_FEATURE(VIRTIO_NET_F_HOST_TSO6) | VIRTIO_FEATURE(VIRTIO_NET_F_HOST_UFO) | VIRTIO_FEATURE(VIRTIO_NET_F_MRG_RXBUF)
                             | VIRTIO_FEATURE(VIRTIO_NET_F_STATUS) | VIRTIO_FEATURE(VIRTIO_NET_F_CTRL_VQ) | VIRTIO_FEATURE(VIRTIO_NET_F_MQ)
                             | VIRTIO_FEATURE(VIRTIO_F_NOTIFY_ON_EMPTY) | VIRTIO_FEATURE(VIRTIO_F_ANY_LAYOUT)
                             | VIRTIO_FEATURE(VIRTIO_RING_F_INDIRECT_DESC);

    if (vif->rss_enabled) supported_features |= VIRTIO_FEATURE(VIRTIO_NET_F_RSS);

    if (vif->is_modern) supported_features |= VIRTIO_FEATURE(VIRTIO_F_VERSION_1);

    if (vif->is_packed) {
        supported_features |= (VIRTIO_FEATURE(VIRTIO_F_RING_PACKED) | VIRTIO_FEATURE(VIRTIO_F_IN_ORDER));
    }
    ...
    if (vif->features & VIRTIO_FEATURE(VIRTIO_NET_F_MTU)) {
        ...
        if (mtu < 64) vif->features &= ~VIRTIO_FEATURE(VIRTIO_NET_F_MTU);
    }

    if ((vif->features & (VIRTIO_FEATURE(VIRTIO_F_RING_PACKED))) == 0) vif->is_packed = 0;
...
int virtio_pci_reset_device(vlib_main_t *vm, virtio_if_t *vif)
{
    ...
    if ((status & VIRTIO_CONFIG_STATUS_ACK) && (status & VIRTIO_CONFIG_STATUS_DRIVER))
        vif->status = status;
    else
        return -1;
...
clib_error_t *virtio_pci_read_caps(vlib_main_t *vm, virtio_if_t *vif, void **bar)
{
    ...
    if ((error = vlib_pci_read_config_u8(vm, h, PCI_CAPABILITY_LIST, &pos))) {
        ...
        return clib_error_return(error, "error in reading capabilty list position");
    }
    while (pos) {
        if ((error = vlib_pci_read_write_config(vm, h, VLIB_READ, pos, &cap, sizeof(cap)))) {
            virtio_log_error(vif, "%s [%2x]", "error in reading the capability at", pos);
            return clib_error_return(error, "error in reading the capability at [%2x]", pos);
        }

        if (cap.cap_vndr == PCI_CAP_ID_MSIX) {
            u16 flags, table_size, table_size_mask = 0x07FF;

            if ((error = vlib_pci_read_write_config(vm, h, VLIB_READ, pos + 2, &flags, sizeof(flags))))
                return clib_error_return(error, "error in reading the capability at [%2x]", pos + 2);

            table_size = flags & table_size_mask;
            virtio_log_debug(vif, "flags:0x%x %s 0x%x", flags, "msix interrupt vector table-size", table_size);
        ...
        if (cap.cap_vndr != PCI_CAP_ID_VNDR) {
            virtio_log_debug(vif, "[%2x] %s %2x ", pos, "skipping non VNDR cap id:", cap.cap_vndr);
            goto next;
        }

        virtio_log_debug(vif, "[%4x] cfg type: %u, bar: %u, offset: %04x, len: %u", pos, cap.cfg_type, cap.bar, cap.offset, cap.length);
        ...
        switch (cap.cfg_type) {
            ...
            case VIRTIO_PCI_CAP_NOTIFY_CFG:
                if ((error = vlib_pci_read_write_config(vm,
                                                        h,
                                                        VLIB_READ,
                                                        pos + sizeof(cap),
                                                        &vif->notify_off_multiplier,
                                                        sizeof(vif->notify_off_multiplier)))) {
                    virtio_log_error(vif, "notify off multiplier is not given");
                }
                else {
                    virtio_log_debug(vif, "notify off multiplier is %u", vif->notify_off_multiplier);
...
static clib_error_t *virtio_pci_device_init(vlib_main_t *vm, virtio_if_t *vif, virtio_pci_create_if_args_t *args, void **bar)
{
    ...
    if ((vif->remote_features & VIRTIO_FEATURE(VIRTIO_RING_F_INDIRECT_DESC)) == 0) {
        virtio_log_warning(vif,
                           "error encountered: vhost-net backend doesn't "
     ...
     */
    vif->virtio_pci_func->set_status(vm, vif, VIRTIO_CONFIG_STATUS_FEATURES_OK);
    status = vif->virtio_pci_func->get_status(vm, vif);
    if (!(status & VIRTIO_CONFIG_STATUS_FEATURES_OK)) {
        args->rv = VNET_API_ERROR_UNSUPPORTED;
        virtio_log_error(vif, "error encountered: Device doesn't support requested features");
        return clib_error_return(error, "Device doesn't support requested features");
    }
    vif->status = status;
    ...
    if (vif->msix_enabled == VIRTIO_MSIX_ENABLED) {
        if (vif->msix_table_size <= vif->max_queue_pairs) {
            virtio_log_error(vif, "error MSIX lines (%u) <= Number of RXQs (%u)", vif->msix_table_size, vif->max_queue_pairs);
            return clib_error_return(error, "error MSIX lines (%u) <= Number of RXQs (%u)", vif->msix_table_size, vif->max_queue_pairs);
        }
    }
    ...
    for (int i = 0; i < vif->max_queue_pairs; i++) {
        ...
        if ((error = virtio_pci_vring_init(vm, vif, TX_QUEUE(i), args->tx_queue_size))) {
            args->rv = VNET_API_ERROR_INIT_FAILED;
    ...
    if (vif->features & VIRTIO_FEATURE(VIRTIO_NET_F_CTRL_VQ)) {
        if ((error = virtio_pci_control_vring_init(vm, vif, vif->max_queue_pairs * 2))) {
            virtio_log_warning(vif, "%s (%u) %s", "error in control-queue", vif->max_queue_pairs * 2, "initialization");
            if (vif->features & VIRTIO_FEATURE(VIRTIO_NET_F_MQ)) vif->features &= ~VIRTIO_FEATURE(VIRTIO_NET_F_MQ);
        }
    }
    else {
    ...
    if (vif->msix_enabled == VIRTIO_MSIX_ENABLED) {
        int i, j;
        if (vif->virtio_pci_func->set_config_irq(vm, vif, 0) == VIRTIO_MSI_NO_VECTOR) {
        ...
        for (i = 0, j = 1; i < vif->max_queue_pairs; i++, j++) {
            if (vif->virtio_pci_func->set_queue_irq(vm, vif, j, RX_QUEUE(i)) == VIRTIO_MSI_NO_VECTOR) {
                virtio_log_warning(vif, "queue (%u) vector is not set at %u", RX_QUEUE(i), j);
            }
            else {
                virtio_log_debug(vif, "%s (%u) %s %u", "queue", RX_QUEUE(i), "msix vector is set at", j);
            }
        }
    }
...
void virtio_pci_create_if(vlib_main_t *vm, virtio_pci_create_if_args_t *args)
{
    ...
    pool_foreach (vif, vim->interfaces) {
        if (vif->pci_addr.as_u32 == args->addr) {
            ...
            args->error = clib_error_return(error, "PCI address in use");
            vlib_log(VLIB_LOG_LEVEL_ERR, vim->log_default, "%U: %s", format_vlib_pci_addr, &args->addr, " PCI address in use");
            return;
        }
    }
    ...
    if (args->bind) {
        ...
        error               = vlib_pci_bind_to_uio(vm, &pci, (char *) "auto", VIRTIO_BIND_FORCE == args->bind);
        if (error) {
            args->rv    = VNET_API_ERROR_INVALID_INTERFACE;
            args->error = clib_error_return(error, "%U: %s", format_vlib_pci_addr, &pci, "error encountered on binding pci device");
            vlib_log(VLIB_LOG_LEVEL_ERR, vim->log_default, "%U: %s", format_vlib_pci_addr, &pci, "error encountered on binding pci devicee");
            return;
        }
    }
    ...
    if (args->virtio_flags & VIRTIO_FLAG_PACKED) vif->is_packed = 1;

    if ((error = vlib_pci_device_open(vm, (vlib_pci_addr_t *) &vif->pci_addr, virtio_pci_device_ids, &h))) {
        args->rv    = VNET_API_ERROR_INVALID_INTERFACE;
        args->error = clib_error_return(error, "pci-addr %U", format_vlib_pci_addr, &vif->pci_addr);
        vlib_log(VLIB_LOG_LEVEL_ERR, vim->log_default, "%U: %s", format_vlib_pci_addr, &vif->pci_addr, "error encountered on pci device open");
    ...
    for (u32 i = 0; i <= 5; i++) {
        if ((error = vlib_pci_map_region(vm, h, i, &bar[i]))) {
            ...
            virtio_log_debug(vif, "pci map region for bar %u at %p", i, bar[i]);
        }
    }
    ...
    if (interrupt_count > 1) {
        if ((error = vlib_pci_register_msix_handler(vm, h, 0, 1, &virtio_pci_irq_config_handler))) {
            args->rv = VNET_API_ERROR_INVALID_REGISTRATION;
            virtio_log_error(vif, "error encountered on pci register msix handler 0");
            goto error;
        }

        if ((error = vlib_pci_register_msix_handler(vm, h, 1, (interrupt_count - 1), &virtio_pci_irq_queue_handler))) {
            args->rv = VNET_API_ERROR_INVALID_REGISTRATION;
            virtio_log_error(vif, "error encountered on pci register msix handler 1");
            goto error;
        }
         ...
         */
        if ((error = vlib_pci_register_intx_handler(vm, h, &virtio_pci_irq_handler))) {
            virtio_log_error(vif, "error encountered on pci register interrupt handler");
            goto error;
        }
        vif->support_int_mode = 1;
    ...
    if (args->virtio_flags & VIRTIO_FLAG_BUFFERING) {
        ...
        if (error) {
            args->rv = VNET_API_ERROR_INIT_FAILED;
            virtio_log_error(vif, "error encountered during packet buffering init");
            goto error;
        }
        /*
    ...
    virtio_pci_offloads(vm, vif, args->gso_enabled, args->checksum_offload_enabled);

    if ((vif->features & VIRTIO_FEATURE(VIRTIO_NET_F_CTRL_VQ)) && (vif->features & VIRTIO_FEATURE(VIRTIO_NET_F_MQ))) {
        if (vif->features & VIRTIO_FEATURE(VIRTIO_NET_F_RSS)) {
            if (virtio_pci_enable_multiqueue_rss(vm, vif, vif->max_queue_pairs)) virtio_log_warning(vif, "multiqueue with rss is not set");
        }
        else {
            if (virtio_pci_enable_multiqueue(vm, vif, vif->max_queue_pairs)) virtio_log_warning(vif, "multiqueue is not set");
        }
    }
    return;
...
int virtio_pci_delete_if(vlib_main_t *vm, virtio_if_t *vif)
{
    ...
    if (vif->type != VIRTIO_IF_TYPE_PCI) return VNET_API_ERROR_INVALID_INTERFACE;
    ...
    if (vif->virtio_pci_func) {
        ...
        if (vif->features & VIRTIO_FEATURE(VIRTIO_NET_F_CTRL_VQ)) vif->virtio_pci_func->del_queue(vm, vif, vif->max_queue_pairs * 2);
    ...
    if (vif->packet_buffering) virtio_pre_input_node_disable(vm, vif);
