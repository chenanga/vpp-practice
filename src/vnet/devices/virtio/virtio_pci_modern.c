...
static u64 virtio_pci_modern_get_device_features(vlib_main_t *vm, virtio_if_t *vif)
{
    ...
    virtio_pci_reg_write_u32(vif, VIRTIO_DEVICE_FEATURE_SELECT_OFFSET(vif), VIRTIO_FEATURE_SELECT_LO);
    features_lo = virtio_pci_reg_read_u32(vif, VIRTIO_DEVICE_FEATURE_OFFSET(vif));
    virtio_pci_reg_write_u32(vif, VIRTIO_DEVICE_FEATURE_SELECT_OFFSET(vif), VIRTIO_FEATURE_SELECT_HI);
    features_hi  = virtio_pci_reg_read_u32(vif, VIRTIO_DEVICE_FEATURE_OFFSET(vif));
...
{
    u64 features_lo, features_hi;
    virtio_pci_reg_write_u32(vif, VIRTIO_DRIVER_FEATURE_SELECT_OFFSET(vif), VIRTIO_FEATURE_SELECT_LO);
    features_lo = virtio_pci_reg_read_u32(vif, VIRTIO_DRIVER_FEATURE_OFFSET(vif));
...
static void virtio_pci_modern_set_driver_features(vlib_main_t *vm, virtio_if_t *vif, u64 features)
{
    ...
    virtio_pci_reg_write_u32(vif, VIRTIO_DRIVER_FEATURE_SELECT_OFFSET(vif), VIRTIO_FEATURE_SELECT_LO);
    virtio_pci_reg_write_u32(vif, VIRTIO_DRIVER_FEATURE_OFFSET(vif), features_lo);
    virtio_pci_reg_write_u32(vif, VIRTIO_DRIVER_FEATURE_SELECT_OFFSET(vif), VIRTIO_FEATURE_SELECT_HI);
    virtio_pci_reg_write_u32(vif, VIRTIO_DRIVER_FEATURE_OFFSET(vif), features_hi);
...
{
    u16 msix_config;
    msix_config = virtio_pci_reg_read_u16(vif, VIRTIO_MSIX_CONFIG_VECTOR_OFFSET(vif));
    return msix_config;
}

static u16 virtio_pci_modern_set_msix_config(vlib_main_t *vm, virtio_if_t *vif, u16 msix_config)
{
    virtio_pci_reg_write_u16(vif, VIRTIO_MSIX_CONFIG_VECTOR_OFFSET(vif), msix_config);
...
{
    if (status != VIRTIO_CONFIG_STATUS_RESET) status |= virtio_pci_modern_get_status(vm, vif);
...
static u8 virtio_pci_modern_get_config_generation(virtio_if_t *vif)
{
    ...
    config_generation    = virtio_pci_reg_read_u8(vif, VIRTIO_CONFIG_GENERATION_OFFSET(vif));
    return config_generation;
}

static void virtio_pci_modern_set_queue_select(virtio_if_t *vif, u16 queue_select)
{
    virtio_pci_reg_write_u16(vif, VIRTIO_QUEUE_SELECT_OFFSET(vif), queue_select);
}
...
static u16 virtio_pci_modern_get_queue_msix_vector(virtio_if_t *vif)
{
    ...
    queue_msix_vector     = virtio_pci_reg_read_u16(vif, VIRTIO_QUEUE_MSIX_VECTOR_OFFSET(vif));
...
{
    virtio_pci_modern_set_queue_select(vif, queue_id);
    virtio_pci_reg_write_u16(vif, VIRTIO_QUEUE_MSIX_VECTOR_OFFSET(vif), queue_msix_vector);
...
static u16 virtio_pci_modern_get_queue_enable(virtio_if_t *vif, u16 queue_id)
{
    u16 queue_enable = 0;
    virtio_pci_modern_set_queue_select(vif, queue_id);
    queue_enable = virtio_pci_reg_read_u16(vif, VIRTIO_QUEUE_ENABLE_OFFSET(vif));
    return queue_enable;
}
...
{
    virtio_pci_modern_set_queue_select(vif, queue_id);
    virtio_pci_reg_write_u16(vif, VIRTIO_QUEUE_ENABLE_OFFSET(vif), queue_enable);
}
...
static u16 virtio_pci_modern_get_queue_notify_off(vlib_main_t *vm, virtio_if_t *vif, u16 queue_id)
{
    ...
    virtio_pci_modern_set_queue_select(vif, queue_id);
    queue_notify_off = virtio_pci_reg_read_u16(vif, VIRTIO_QUEUE_NOTIFY_OFF_OFFSET(vif));
...
{
    u64 queue_driver = 0;
    queue_driver     = virtio_pci_reg_read_u64(vif, VIRTIO_QUEUE_DRIVER_OFFSET(vif));
    return queue_driver;
}

static void virtio_pci_modern_set_queue_driver(virtio_if_t *vif, u64 queue_driver)
{
    virtio_pci_reg_write_u64(vif, VIRTIO_QUEUE_DRIVER_OFFSET(vif), queue_driver);
}

static u64 virtio_pci_modern_get_queue_device(virtio_if_t *vif)
{
    u64 queue_device = 0;
    queue_device     = virtio_pci_reg_read_u64(vif, VIRTIO_QUEUE_DEVICE_OFFSET(vif));
    return queue_device;
}

static void virtio_pci_modern_set_queue_device(virtio_if_t *vif, u64 queue_device)
{
    virtio_pci_reg_write_u64(vif, VIRTIO_QUEUE_DEVICE_OFFSET(vif), queue_device);
}

static u8 virtio_pci_modern_setup_queue(vlib_main_t *vm, virtio_if_t *vif, u16 queue_id, vnet_virtio_vring_t *vring)
{
    u64 desc, avail, used;
    ...
    if (desc != virtio_pci_modern_get_queue_desc(vif)) return 1;

    virtio_pci_modern_set_queue_driver(vif, avail);
    if (avail != virtio_pci_modern_get_queue_driver(vif)) return 1;

    virtio_pci_modern_set_queue_device(vif, used);
    if (used != virtio_pci_modern_get_queue_device(vif)) return 1;

    virtio_pci_modern_set_queue_enable(vif, queue_id, 1);

    if (virtio_pci_modern_get_queue_enable(vif, queue_id)) return 0;
...
static u16 virtio_pci_modern_get_max_virtqueue_pairs(vlib_main_t *vm, virtio_if_t *vif)
{
    ...
    max_virtqueue_pairs     = virtio_pci_reg_read_u16(vif, VIRTIO_MAX_VIRTQUEUE_PAIRS_OFFSET(vif));
    u16 supported_queues    = virtio_pci_modern_get_num_queues(vif);
    virtio_log_debug(vif, "max-virtqueue-pairs %u, supported-queues %u", max_virtqueue_pairs, supported_queues);
...
static void virtio_pci_modern_read_config(vlib_main_t *vm, virtio_if_t *vif, void *dst, int len, u32 addr)
{
    ...
    do {
        ...
        u16 max_queue_pairs = virtio_pci_modern_get_max_virtqueue_pairs(vm, vif);
        u16 mtu             = virtio_pci_modern_get_device_mtu(vm, vif);
        virtio_log_debug(vif, "status %u, max_queue_pairs %u, mtu %u", status, max_queue_pairs, mtu);
...
inline void virtio_pci_modern_notify_queue(vlib_main_t *vm, virtio_if_t *vif, u16 queue_id, u16 queue_notify_off)
{
    virtio_pci_reg_write_u16(vif, VIRTIO_NOTIFICATION_OFFSET(vif) + queue_notify_off, queue_id);
}
