...
static clib_error_t *virtio_pci_create_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%U", unformat_vlib_pci_addr, &args.addr))
            ...
            if (unformat(line_input, "size %u", &buffering_size)) args.buffering_size = buffering_size;
        }
...
VLIB_CLI_COMMAND(virtio_pci_create_command, static) = {
    ...
    .short_help =
        "create interface virtio <pci-address> "
...
static clib_error_t *virtio_pci_delete_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (hw == NULL || virtio_device_class.index != hw->dev_class_index) return clib_error_return(0, "not a virtio interface");

    vif = pool_elt_at_index(vim->interfaces, hw->dev_instance);

    if (virtio_pci_delete_if(vm, vif) < 0) return clib_error_return(0, "not a virtio pci interface");
...
static clib_error_t *virtio_pci_enable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (virtio_pci_enable_disable_offloads(vm, vif, gso_enabled, checksum_offload_enabled, offloads_disabled) < 0)
...
VLIB_CLI_COMMAND(virtio_pci_enable_command, static) = {
    .path = "set virtio pci",
    .short_help =
        "set virtio pci {<interface> | sw_if_index <sw_idx>}"
...
static clib_error_t *show_virtio_pci_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (vec_len(hw_if_indices) == 0) {
        ...
        if (vif->type == VIRTIO_IF_TYPE_PCI) vif->virtio_pci_func->device_debug_config_space(vm, vif);
    }
