...
more:
    ...
    n_slots = vlib_buffer_alloc_to_ring_from_pool(vm, vring->buffers, next, vring->queue_size, n_refill, vring->buffer_pool_index);

    if (PREDICT_FALSE(n_slots != n_refill)) {
        vlib_error_count(vm, node_index, VIRTIO_INPUT_ERROR_BUFFER_ALLOC, n_refill - n_slots);
        if (n_slots == 0) return;
    }
    ...
    while (n_slots) {
        ...
        d->addr  = ((type == VIRTIO_IF_TYPE_PCI) ? vlib_buffer_get_current_pa(vm, b) : pointer_to_uword(vlib_buffer_get_current(b)));
    ...
    if ((clib_atomic_load_seq_cst(&vring->used->flags) & VRING_USED_F_NO_NOTIFY) == 0) {
...
more:
    ...
    n_slots          = vlib_buffer_alloc_to_ring_from_pool(vm, vring->buffers, next, sz, n_refill, vring->buffer_pool_index);
