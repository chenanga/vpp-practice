...
#define PCI_CONFIG_SIZE(vif) ((vif->msix_enabled == VIRTIO_MSIX_ENABLED) ? 24 : 20)
...
{
    u32 host_features;
    vlib_pci_read_io_u32(vm, vif->pci_dev_handle, VIRTIO_PCI_HOST_FEATURES, &host_features);
...
static u64 virtio_pci_legacy_get_guest_features(vlib_main_t *vm, virtio_if_t *vif)
{
    ...
    vlib_pci_read_io_u32(vm, vif->pci_dev_handle, VIRTIO_PCI_GUEST_FEATURES, &guest_features);
...
static void virtio_pci_legacy_set_guest_features(vlib_main_t *vm, virtio_if_t *vif, u64 guest_features)
{
    ...
    vlib_pci_write_io_u32(vm, vif->pci_dev_handle, VIRTIO_PCI_GUEST_FEATURES, &gf);
    vlib_pci_read_io_u32(vm, vif->pci_dev_handle, VIRTIO_PCI_GUEST_FEATURES, &features);
...
{
    if (status != VIRTIO_CONFIG_STATUS_RESET) status |= virtio_pci_legacy_get_status(vm, vif);
...
{
    u16 queue_num = 0;
    vlib_pci_write_io_u16(vm, vif->pci_dev_handle, VIRTIO_PCI_QUEUE_SEL, &queue_id);
    vlib_pci_read_io_u16(vm, vif->pci_dev_handle, VIRTIO_PCI_QUEUE_NUM, &queue_num);
    return queue_num;
}
...
static u8 virtio_pci_legacy_setup_queue(vlib_main_t *vm, virtio_if_t *vif, u16 queue_id, vnet_virtio_vring_t *vring)
{
    u64 addr  = vlib_physmem_get_pa(vm, vring->desc) >> VIRTIO_PCI_QUEUE_ADDR_SHIFT;
...
inline void virtio_pci_legacy_notify_queue(vlib_main_t *vm, virtio_if_t *vif, u16 queue_id, u16 queue_notify_off)
{
    vlib_pci_write_io_u16(vm, vif->pci_dev_handle, VIRTIO_PCI_QUEUE_NOTIFY, &queue_id);
}

/* Enable one vector (0) for Link State Intrerrupt */
static u16 virtio_pci_legacy_set_config_irq(vlib_main_t *vm, virtio_if_t *vif, u16 vec)
{
    vlib_pci_write_io_u16(vm, vif->pci_dev_handle, VIRTIO_MSI_CONFIG_VECTOR, &vec);
    vlib_pci_read_io_u16(vm, vif->pci_dev_handle, VIRTIO_MSI_CONFIG_VECTOR, &vec);
    return vec;
}
...
{
    vlib_pci_write_io_u16(vm, vif->pci_dev_handle, VIRTIO_PCI_QUEUE_SEL, &queue_id);
    vlib_pci_write_io_u16(vm, vif->pci_dev_handle, VIRTIO_MSI_QUEUE_VECTOR, &vec);
    vlib_pci_read_io_u16(vm, vif->pci_dev_handle, VIRTIO_MSI_QUEUE_VECTOR, &vec);
    return vec;
}

static void virtio_pci_legacy_get_mac(vlib_main_t *vm, virtio_if_t *vif)
{
    virtio_pci_legacy_read_config(vm, vif, vif->mac_addr, sizeof(vif->mac_addr), 0);
}

static void virtio_pci_legacy_set_mac(vlib_main_t *vm, virtio_if_t *vif)
{
    virtio_pci_legacy_write_config(vm, vif, vif->mac_addr, sizeof(vif->mac_addr), 0);
}

static u16 virtio_pci_legacy_get_device_status(vlib_main_t *vm, virtio_if_t *vif)
{
    u16 status = 0;
    virtio_pci_legacy_read_config(vm, vif, &status, sizeof(status), STRUCT_OFFSET_OF(virtio_net_config_t, status));
    return status;
}
...
static u16 virtio_pci_legacy_get_max_queue_pairs(vlib_main_t *vm, virtio_if_t *vif)
{
    ...
    virtio_pci_legacy_read_config(vm,
                                  vif,
...
{
    virtio_net_config_t config;
    virtio_pci_legacy_read_config(vm, vif, &config.mtu, sizeof(config.mtu), STRUCT_OFFSET_OF(virtio_net_config_t, mtu));
    return config.mtu;
}
...
static void virtio_pci_legacy_device_debug_config_space(vlib_main_t *vm, virtio_if_t *vif)
{
    ...
    u16 data_u16;
    u8  data_u8;
    vlib_pci_read_io_u32(vm, vif->pci_dev_handle, VIRTIO_PCI_HOST_FEATURES, &data_u32);
    vlib_cli_output(vm, "remote features 0x%lx", data_u32);
    vlib_pci_read_io_u32(vm, vif->pci_dev_handle, VIRTIO_PCI_GUEST_FEATURES, &data_u32);
    vlib_cli_output(vm, "guest features 0x%lx", data_u32);
    vlib_pci_read_io_u32(vm, vif->pci_dev_handle, VIRTIO_PCI_QUEUE_PFN, &data_u32);
    vlib_cli_output(vm, "queue address 0x%lx", data_u32);
    vlib_pci_read_io_u16(vm, vif->pci_dev_handle, VIRTIO_PCI_QUEUE_NUM, &data_u16);
    vlib_cli_output(vm, "queue size 0x%x", data_u16);
    vlib_pci_read_io_u16(vm, vif->pci_dev_handle, VIRTIO_PCI_QUEUE_SEL, &data_u16);
    vlib_cli_output(vm, "queue select 0x%x", data_u16);
    vlib_pci_read_io_u16(vm, vif->pci_dev_handle, VIRTIO_PCI_QUEUE_NOTIFY, &data_u16);
    ...
    if (vif->msix_enabled == VIRTIO_MSIX_ENABLED) {
        vlib_pci_read_io_u16(vm, vif->pci_dev_handle, VIRTIO_MSI_CONFIG_VECTOR, &data_u16);
        vlib_cli_output(vm, "config vector 0x%x", data_u16);
        u16 queue_id = 0;
        vlib_pci_write_io_u16(vm, vif->pci_dev_handle, VIRTIO_PCI_QUEUE_SEL, &queue_id);
        vlib_pci_read_io_u16(vm, vif->pci_dev_handle, VIRTIO_MSI_QUEUE_VECTOR, &data_u16);
