...
static void worker_handoff_trace_frame(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t **bufs, u16 *threads, u32 n_vectors)
{
...
VLIB_NODE_FN(worker_handoff_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    n_enq = vlib_buffer_enqueue_to_thread(vm, node, hm->frame_queue_index, from, thread_indices, frame->n_vectors, 1);

    if (n_enq < frame->n_vectors) vlib_node_increment_counter(vm, node->node_index, WORKER_HANDOFF_ERROR_CONGESTION_DROP, frame->n_vectors - n_enq);
...
int interface_handoff_enable_disable(vlib_main_t *vm, u32 sw_if_index, uword *bitmap, u8 is_sym, int is_l4, int enable_disable)
{
    handoff_main_t              *hm = &handoff_main;
    ...
    if (clib_bitmap_last_set(bitmap) >= hm->num_workers) return VNET_API_ERROR_INVALID_WORKER;
    ...
    if (enable_disable) {
        ...
        if (is_sym) {
            if (is_l4) return VNET_API_ERROR_UNIMPLEMENTED;

            d->hash_fn = vnet_hash_function_from_name("handoff-eth-sym", VNET_HASH_FN_TYPE_ETHERNET);
        }
        else {
            if (is_l4)
                d->hash_fn = vnet_hash_default_function(VNET_HASH_FN_TYPE_ETHERNET);
            else
                d->hash_fn = vnet_hash_function_from_name("handoff-eth", VNET_HASH_FN_TYPE_ETHERNET);
        }
    }

    vnet_feature_enable_disable("device-input", "worker-handoff", sw_if_index, enable_disable, 0, 0);
    vnet_feature_enable_disable("port-rx-eth", "worker-handoff", sw_if_index, enable_disable, 0, 0);
    return rv;
}
...
static clib_error_t *set_interface_handoff_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (bitmap == 0) return clib_error_return(0, "Please specify list of workers...");

    rv = interface_handoff_enable_disable(vm, sw_if_index, bitmap, is_sym, is_l4, enable_disable);
...
clib_error_t *handoff_init(vlib_main_t *vm)
{
    ...
    if ((error = vlib_call_init_function(vm, threads_init))) return error;
