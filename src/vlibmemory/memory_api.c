...
static void memclnt_queue_callback(vlib_main_t *vm)
{
    ...
    if (PREDICT_FALSE(vec_len(vl_api_queue_cursizes) != 1 + vec_len(am->vlib_private_rps))) {
        vl_shmem_hdr_t *shmem_hdr = am->shmem_hdr;
        svm_queue_t    *q;

        if (shmem_hdr == 0) return;
...
u32 vl_api_memclnt_create_internal(char *name, svm_queue_t *q)
{
    ...
    *regpp  = clib_mem_alloc(sizeof(vl_api_registration_t));
    ...
    return vl_msg_api_handle_from_index_and_epoch(regp->vl_api_registration_pool_index, am->shmem_hdr->application_restarts);
}
...
void vl_api_memclnt_create_t_handler(vl_api_memclnt_create_t *mp)
{
    ...
    *regpp  = clib_mem_alloc(sizeof(vl_api_registration_t));
    ...
    if (am->serialized_message_table_in_shmem == 0) am->serialized_message_table_in_shmem = vl_api_serialize_message_table(am, 0);
    ...
    rp->index         = vl_msg_api_handle_from_index_and_epoch(regp->vl_api_registration_pool_index, am->shmem_hdr->application_restarts);
...
void vl_api_memclnt_create_v2_t_handler(vl_api_memclnt_create_v2_t *mp)
{
    vl_api_registration_t           **regpp;
    vl_api_registration_t            *regp;
...
void vl_api_memclnt_delete_t_handler(vl_api_memclnt_delete_t *mp)
{
    ...
    if (epoch != (am->shmem_hdr->application_restarts & VL_API_EPOCH_MASK)) {
        clib_warning("Stale clnt delete index %d old epoch %d cur epoch %d",
                     client_index,
                     epoch,
    ...
    if (!pool_is_free(am->vl_clients, regpp)) {
        ...
        if (!mp->do_cleanup) {
            ...
            if (client_index != regp->vl_api_registration_pool_index) {
                clib_warning("mismatch client_index %d pool_index %d", client_index, regp->vl_api_registration_pool_index);
                vl_msg_api_free(rp);
                return;
            }
        }
        ...
        *regpp = 0;
        ...
        for (i = 0; i < vec_len(am->vlib_private_rps); i++) {
            ...
            if (am->vlib_private_rps[i] == am->vlib_rp) {
                ...
                if (munmap((void *) virtual_base, virtual_size) < 0) clib_unix_warning("munmap");
        ...
        if (private_registration == 0) {
            pool_put_index(am->vl_clients, regp->vl_api_registration_pool_index);
            oldheap = vl_msg_push_heap();
            if (mp->do_cleanup) svm_queue_free(regp->vl_input_queue);
            vec_free(regp->name);
...
int vl_mem_api_init(const char *region_name)
{
    ...
    if ((rv = vl_map_shmem(region_name, 1 /* is_vlib */)) < 0) return rv;
...
clib_error_t *map_api_segment_init(vlib_main_t *vm)
{
    ...
    if ((rv = vl_mem_api_init(am->region_name)) < 0) {
        return clib_error_return(0, "vl_mem_api_init (%s) failed", am->region_name);
    }
    return 0;
}
...
static void send_memclnt_keepalive(vl_api_registration_t *regp, f64 now)
{
    ...
    mp->context    = mp->client_index =
        vl_msg_api_handle_from_index_and_epoch(regp->vl_api_registration_pool_index, am->shmem_hdr->application_restarts);

    regp->unanswered_pings++;

    /* Failure-to-send due to a stuffed queue is absolutely expected */
    if (svm_queue_add(q, (u8 *) &mp, 1 /* nowait */)) vl_msg_api_free_w_region(regp->vlib_rp, mp);
}
        ...
        if (regp->last_heard < (now - 10.0)) {
            if (regp->unanswered_pings == 2) {
                ...
                if (kill(q->consumer_pid, 0) >= 0) {
                    clib_warning("REAPER: lazy binary API client '%s'", regp->name);
                    regp->unanswered_pings = 0;
                    regp->last_heard       = now;
                }
                else {
                    clib_warning("REAPER: binary API client '%s' died", regp->name);
...
void vl_mem_api_dead_client_scan(api_main_t *am, vl_shmem_hdr_t *shm, f64 now)
{
    ...
    pool_foreach (regpp, am->vl_clients) {
        if (!(*regpp)->keepalive) continue;
        vl_mem_send_client_keepalive_w_reg(am, now, regpp, &dead_indices, &confused_indices);
    }
    ...
    if (PREDICT_FALSE(vec_len(dead_indices) > 0)) {
        ...
        for (i = 0; i < vec_len(dead_indices); i++) {
            ...
            if (regpp) {
                ...
                handle = vl_msg_api_handle_from_index_and_epoch(dead_indices[i], shm->application_restarts);
                vl_api_call_reaper_functions(handle);
            }
        }
        ...
        for (i = 0; i < vec_len(dead_indices); i++) {
            ...
            if (regpp) {
                ...
                *regpp = 0;
                ...
                svm_pop_heap(oldheap);
                clib_warning("Duplicate free, client index %d", regpp - am->vl_clients);
                oldheap = svm_push_data_heap(am->vlib_rp);
            }
        }
...
static void vl_mem_api_handler_with_vm_node(api_main_t          *am,
                                            ...
                                            u8                   is_private)
{
    ...
    if (m && m->handler) {
        ...
        if (PREDICT_FALSE(am->rx_trace && am->rx_trace->enabled)) vl_msg_api_trace(am, am->rx_trace, the_msg);
        ...
        if (PREDICT_FALSE(vl_mem_api_fuzz_hook != 0)) (*vl_mem_api_fuzz_hook)(id, the_msg);
        ...
        if (PREDICT_FALSE(vec_len(am->perf_counter_cbs) != 0)) clib_call_callbacks(am->perf_counter_cbs, am, id, 0 /* before */);

        (*handler)(the_msg, vm, node);

        if (PREDICT_FALSE(vec_len(am->perf_counter_cbs) != 0)) clib_call_callbacks(am->perf_counter_cbs, am, id, 1 /* after */);
        if (is_private) {
            am->vlib_rp   = old_vlib_rp;
            am->shmem_hdr = save_shmem_hdr;
        }
        if (!is_mp_safe) vl_msg_api_barrier_release();
    }
    else {
...
static inline int void_mem_api_handle_msg_i(api_main_t *am, svm_region_t *vlib_rp, vlib_main_t *vm, vlib_node_runtime_t *node, u8 is_private)
{
    svm_queue_t *q;
    uword        mp;
    ...
    if (!svm_queue_sub2(q, (u8 *) &mp)) {
        ...
        vl_mem_api_handler_with_vm_node(am, vlib_rp, (void *) mp, vm, node, is_private);
        return 0;
    }
    return -1;
}
...
{
    api_main_t *am = vlibapi_get_main();
    return void_mem_api_handle_msg_i(am, am->vlib_rp, vm, node, 0 /* is_private */);
}
...
int vl_mem_api_handle_rpc(vlib_main_t *vm, vlib_node_runtime_t *node)
{
    ...
    if (PREDICT_TRUE(vec_len(vm->processing_rpc_requests))) {
        ...
        for (i = 0; i < vec_len(vm->processing_rpc_requests); i++) {
            ...
            vl_mem_api_handler_with_vm_node(am, am->vlib_rp, (void *) mp, vm, node, 0 /* is_private */);
        }
...
{
    api_main_t *am = vlibapi_get_main();
    return void_mem_api_handle_msg_i(am, am->vlib_private_rps[reg_index], vm, node, 1 /* is_private */);
}
...
svm_queue_t *vl_api_client_index_to_input_queue(u32 index)
{
    ...
    if (index == (u32) ~0) return (am->shmem_hdr->vl_input_queue);
...
u8 *format_api_message_rings(u8 *s, va_list *args)
{
    ...
    if (shmem_hdr == 0) return format(s, "%8s %8s %8s %8s %8s\n", "Owner", "Size", "Nitems", "Hits", "Misses");
...
static clib_error_t *vl_api_ring_command(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cli_cmd)
{
    ...
    vlib_cli_output(vm, "%U", format_api_message_rings, am, 0 /* print header */, 0 /* notused */);

    vlib_cli_output(vm, "%U", format_api_message_rings, am, shmem_hdr, 1 /* main segment */);
    ...
    found:
        vlib_cli_output(vm, "%U", format_api_message_rings, am, 0 /* print header */, 0 /* notused */);
        vlib_cli_output(vm, "%U", format_api_message_rings, am, shmem_hdr, 0 /* main segment */);
    }
...
clib_error_t *vlibmemory_init(vlib_main_t *vm)
{
    ...
    a->baseva        = (am->global_baseva != 0) ? am->global_baseva : +svm_get_global_region_base_va();
    a->size          = (am->global_size != 0) ? am->global_size : SVM_GLOBAL_REGION_SIZE;
    a->flags         = SVM_FLAGS_NODATA;
    a->uid           = am->api_uid;
    a->gid           = am->api_gid;
    a->pvt_heap_size = (am->global_pvt_heap_size != 0) ? am->global_pvt_heap_size : SVM_PVT_MHEAP_SIZE;
