...
#define _(n)                                                                                                  \
    do {                                                                                                      \
        f64 percent;                                                                                          \
        percent = ((f64) vector_rate_histogram[SLEEP_##n##_US]) / (f64) total_counts;                         \
        percent *= 100.0;                                                                                     \
        vlib_cli_output(vm, "Sleep %3d us: %llu, %.2f%%", n, vector_rate_histogram[SLEEP_##n##_US], percent); \
    } while (0);
...
static clib_error_t *vl_api_client_command(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cli_cmd)
{
    ...
    if (!pool_elts(am->vl_clients)) goto socket_clients;
    vlib_cli_output(vm, "Shared memory clients");
    vlib_cli_output(vm, "%20s %8s %14s %18s %s", "Name", "PID", "Queue Length", "Queue VA", "Health");
    ...
    pool_foreach (regpp, am->vl_clients) {
        ...
        if (regp) {
            ...
            vlib_cli_output(vm, "%20s %8d %14d 0x%016llx %s\n", regp->name, q->consumer_pid, q->cursize, q, health);
        }
        else {
            clib_warning("NULL client registration index %d", regpp - am->vl_clients);
    ...
    if (am->missing_clients) vlib_cli_output(vm, "%u messages with missing clients", am->missing_clients);
socket_clients:
...
static clib_error_t *vl_api_message_table_command(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cli_cmd)
{
    ...
    if (verbose == 0)
        ...
        vlib_cli_output(vm, "%-4s %-40s %6s %7s", "ID", "Name", "Bounce", "MP-safe");

    for (i = 1; i < vec_len(am->msg_data); i++) {
        vl_api_msg_data_t *m = vl_api_get_msg_data(am, i);
        if (verbose == 0) {
            vlib_cli_output(vm, "%-4d %s", i, m->name ? m->name : "  [no handler]");
        }
        else {
            vlib_cli_output(vm, "%-4d %-40s %6d %7d", i, m->name ? m->name : "  [no handler]", m->bounce, m->is_mp_safe);
        }
    }
...
static void vl_msg_api_process_file(vlib_main_t *vm, u8 *filename, u32 first_index, u32 last_index, vl_api_replay_t which)
{
    ...
    if (hp->wrapped) vlib_cli_output(vm, "Note: wrapped/incomplete trace, results may vary\n");
...
#define assert_size(size_left, s)                  \
    ...
    for (i = 0; i < nitems_msgtbl; i++) {
        ...
        if (~0 == msg_index2) vlib_cli_output(vm, "warning: can't find msg index for id %d\n", msg_index);
    ...
    for (i = 0; i < first_index; i++) {
        ...
        if (msg_id >= vec_len(msgid_vec) || msgid_vec[msg_id] >= vec_len(am->msg_data))
            vlib_cli_output(vm, "warning: unknown msg id %d for msg number %d\n", msg_id, i);

        msg += size;
    }

    if (which == REPLAY) am->replay_in_progress = 1;
    ...
    for (; i <= last_index; i++) {
        ...
        if (which == DUMP) vlib_cli_output(vm, "---------- trace %d -----------\n", i);
        ...
        if (msg_id >= vec_len(msgid_vec) || msgid_vec[msg_id] >= vec_len(am->msg_data)) {
            vlib_cli_output(vm, "warning: unknown msg id %d for msg number %d, skipping\n", msg_id, i);
            msg += size;
            continue;
        }
         ...
         * network byte order.
         */
        if (((which == DUMP || which == DUMP_JSON) && clib_arch_is_little_endian)) {
        ...
        switch (which) {
            case DUMP_JSON:
                vlib_cli_output(vm, "%U", format_vl_api_msg_json, am, msg_id, tmpbuf + sizeof(uword));
                break;

            case DUMP:
                vlib_cli_output(vm, "%U", format_vl_api_msg_text, am, msg_id, tmpbuf + sizeof(uword));
                break;
            ...
            case INITIALIZERS:
                if (m) {
                    ...
                    s = format(0, "static u8 * vl_api_%s_%d[%d] = {", m->name, i, m->trace_size);
            ...
            case REPLAY:
                if (m && m->handler && m->replay_allowed) {
                    if (!m->is_mp_safe) vl_msg_api_barrier_sync();
                    m->handler(tmpbuf + sizeof(uword));
                    if (!m->is_mp_safe) vl_msg_api_barrier_release();
                }
                else {
                    if (m && m->replay_allowed) vlib_cli_output(vm, "Skipping msg id %d: no handler\n", msg_id);
                    break;
                }
                break;
        }
...
static int file_exists(u8 *fname)
{
    FILE *fp = 0;
...
static int vl_msg_print_trace(u8 *msg, void *ctx)
{
    ...
    vlib_cli_output(a->vm, "%U\n", is_json ? format_vl_api_msg_json : format_vl_api_msg_text, am, msg_id, msg);

    vec_free(tmpbuf);
    return 0;
}

static int vl_msg_api_dump_trace(vlib_main_t *vm, vl_api_trace_which_t which, u8 is_json)
{
    api_main_t     *am = vlibapi_get_main();
    vl_api_trace_t *tp;
    ...
    if (tp == 0 || tp->nitems == 0 || vec_len(tp->traces) == 0) return -1;
...
static char *vl_msg_read_file(FILE *f)
{
...
static u16 vl_msg_find_id_by_name_and_crc(vlib_main_t *vm, api_main_t *am, char *name)
{
    uword *p;
...
static u16 vl_msg_find_id_by_name(vlib_main_t *vm, api_main_t *am, char *name)
{
    uword *p;
...
static int vl_msg_exec_json_command(vlib_main_t *vm, cJSON *o)
{
    api_main_t        *am = vlibapi_get_main();
    u16                msg_id;
    int                len = 0, rv = -1;
    ...
    if (m->replay_allowed) {
        if (proc_warning) vlib_cli_output(vm, "warning: msg %d has different signature\n");
        ...
        if (clib_arch_is_little_endian) m->endian_handler(msg, 1 /* to network */);
    ...
    if (msg) cJSON_free(msg);
    return rv;
}

static void vl_msg_replay_json(vlib_main_t *vm, u8 *filename)
{
    api_main_t *am = vlibapi_get_main();
    cJSON      *o  = 0;
    int         rv = 0;
    ...
    if (rv < 0) vlib_cli_output(vm, "error during replaying API trace");

    cJSON_Delete(o);
}

static void vl_msg_dump_file_json(vlib_main_t *vm, u8 *filename)
{
    FILE *f = fopen((char *) filename, "r");
    char *buf;
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "on") || unformat(line_input, "enable")) {
            ...
            if (strstr((char *) filename, "..") || index((char *) filename, '/')) {
                vlib_cli_output(vm, "illegal characters in filename '%s'", filename);
                goto out;
            }
            ...
            vlib_cli_output(vm, "%U", format_vl_msg_api_trace_status, am, which);
        }
...
static clib_error_t *api_trace_config_fn(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(input, "save-api-table %s", &am->save_msg_table_filename))
            ;
        else
            return clib_error_return(0, "unknown input `%U'", format_unformat_error, input);
    }
...
static clib_error_t *api_queue_config_fn(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "length %d", &nitems) || (unformat(input, "len %d", &nitems))) {
            if (nitems >= 1024)
                am->vlib_input_queue_length = nitems;
            else
                clib_warning("vlib input queue length %d too small, ignored", nitems);
        }
        else
            return clib_error_return(0, "unknown input `%U'", format_unformat_error, input);
    }
...
static clib_error_t *dump_api_table_file_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "file %s", &filename))
            ;
        else if (unformat(input, "compare-current") || unformat(input, "compare"))
            compare_current = 1;
    ...
    if (numeric_sort && compare_current) return clib_error_return(0, "Comparison and numeric sorting are incompatible");

    if (filename == 0) return clib_error_return(0, "File not specified");
    ...
    if (compare_current) {
        ...
        for (i = 0; i < vec_len(table);) {
            ...
            /* Identical pair? */
            if (!strncmp((char *) table[i].name_and_crc, (char *) table[i + 1].name_and_crc, vec_len(table[i].name_and_crc))) {
                i += 2;
                continue;
            }
            ...
            if (i + 1 == vec_len(table) || strcmp((char *) table[i].name, (char *) table[i + 1].name)) {
            last_unique:
                vlib_cli_output(vm, "%-60s | only in %s", table[i].name, table[i].which ? "image" : "file");
                i++;
                continue;
            }
        ...
        if (ndifferences == 0)
            ...
            vlib_cli_output(vm, "\nFound %u api message signature differences", ndifferences);
        goto cleanup;
    }
    ...
    for (i = 0; i < vec_len(table); i++) {
        item = table + i;
        vlib_cli_output(vm, "%-60s %8u %10s", item->name, item->msg_index, item->crc);
    }
