...
static int vl_socket_client_read_internal(socket_client_main_t *scm, int wait)
{
    ...
    if (scm->socket_fd == 0) return -1;

    if (wait) timeout = clib_time_now(&scm->clib_time) + wait;

    while (1) {
        current_rx_index = vec_len(scm->socket_rx_buffer);
        while (current_rx_index < sizeof(*mbp)) {
            vec_validate(scm->socket_rx_buffer, current_rx_index + scm->socket_buffer_size - 1);
            n = read(scm->socket_fd, scm->socket_rx_buffer + current_rx_index, scm->socket_buffer_size);
            if (n < 0) {
...
#if CLIB_DEBUG > 1
        if (n > 0) clib_warning("read %d bytes", n);
#endif
        ...
        while (current_rx_index < msg_size) {
            n = read(scm->socket_fd, scm->socket_rx_buffer + current_rx_index, msg_size - current_rx_index);
            if (n < 0) {
                if (errno == EAGAIN) continue;
        ...
        if (vec_len(scm->socket_rx_buffer) >= data_len + sizeof(*mbp)) {
            ...
            if (vec_len(scm->socket_rx_buffer) == 0 && scm->control_pings_outstanding == 0) break;
        }
        if (wait && clib_time_now(&scm->clib_time) >= timeout) return -1;
    }
    return 0;
}
...
void vl_socket_client_disconnect2(socket_client_main_t *scm)
{
    ...
    if (scm->socket_fd && (close(scm->socket_fd) < 0)) clib_unix_warning("close");
    scm->socket_fd = 0;
}
...
{
    char          msgbuf[16];
    char          ctl[CMSG_SPACE(sizeof(int) * n_fds) + CMSG_SPACE(sizeof(struct ucred))];
    struct msghdr mh = {0};
    struct iovec  iov[1];
    ssize_t       size = 0;
#ifdef __linux__
    ...
    if (size != 5) {
        return (size == 0) ? clib_error_return(0, "disconnected") : clib_error_return_unix(0, "recvmsg: malformed message (fd %d)", socket_fd);
    }
...
{
    return vl_sock_api_recv_fd_msg_internal(socket_client_ctx, fds, n_fds, wait);
}
...
static void vl_api_sock_init_shm_reply_t_handler(vl_api_sock_init_shm_reply_t *mp)
{
    ...
    if (retval) clib_warning("WARNING: segment map returned %d", retval);
    ...
    if (scm->want_shm_pthread) {
        vl_client_connect_to_vlib_no_map("pvt", (char *) new_name, 32 /* input_queue_length */);
    }
    else {
        vl_client_connect_to_vlib_no_rx_pthread_no_map("pvt", (char *) new_name, 32
                                                       /* input_queue_length */
        );
    }
...
int vl_socket_client_connect_internal(socket_client_main_t *scm, char *socket_path, char *client_name, u32 socket_buffer_size)
{
    ...
    /* Already connected? */
    if (scm->socket_fd) return (-2);

    /* bogus call? */
    if (socket_path == 0 || client_name == 0) return (-3);
    ...
    scm->socket_buffer_size = socket_buffer_size ? socket_buffer_size : SOCKET_CLIENT_DEFAULT_BUFFER_SIZE;
    ...
    if (vl_socket_client_write_internal(scm) <= 0) return (-1);

    if (vl_socket_client_read_internal(scm, 5)) return (-1);
...
{
    return vl_socket_client_connect_internal(socket_client_ctx, socket_path, client_name, socket_buffer_size);
}
...
int vl_socket_client_connect2(socket_client_main_t *scm, char *socket_path, char *client_name, u32 socket_buffer_size)
{
    ...
    old_ctx = vl_socket_client_ctx_push(scm);
    rv      = vl_socket_client_connect_internal(socket_client_ctx, socket_path, client_name, socket_buffer_size);
    vl_socket_client_ctx_pop(old_ctx);
    return rv;
}
...
int vl_socket_client_init_shm_internal(socket_client_main_t *scm, vl_api_shm_elem_config_t *config, int want_pthread)
{
    ...
    mp = vl_socket_client_msg_alloc2(scm, sizeof(*mp) + vec_len(config) * sizeof(u64));
    ...
    if (vl_socket_client_read_internal(scm, 1)) return -1;
...
{
    return vl_socket_client_init_shm_internal(socket_client_ctx, config, want_pthread);
}
...
int vl_socket_client_init_shm2(socket_client_main_t *scm, vl_api_shm_elem_config_t *config, int want_pthread)
{
    ...
    old_ctx = vl_socket_client_ctx_push(scm);
    rv      = vl_socket_client_init_shm_internal(socket_client_ctx, config, want_pthread);
    vl_socket_client_ctx_pop(old_ctx);
    return rv;
}
...
{
    if (!scm->socket_fd) return clib_error_return(0, "no socket");
