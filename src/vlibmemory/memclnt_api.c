...
static void vl_api_get_first_msg_id_t_handler(vl_api_get_first_msg_id_t *mp)
{
    ...
    if (am->msg_range_by_name == 0) goto out;
...
void vl_api_api_versions_t_handler(vl_api_api_versions_t *mp)
{
    ...
    for (i = 0; i < nmsg; ++i) {
        ...
        strncpy((char *) rmp->api_versions[i].name, vl->name, ARRAY_LEN(rmp->api_versions[i].name));
        rmp->api_versions[i].name[ARRAY_LEN(rmp->api_versions[i].name) - 1] = '\0';
    }
...
static void vl_api_control_ping_t_handler(vl_api_control_ping_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_CONTROL_PING_REPLY, ({ rmp->vpe_pid = ntohl(getpid()); }));
}

static void vl_api_get_api_json_t_handler(vl_api_get_api_json_t *mp)
{
...
static void send_one_plugin_msg_ids_msg(u8 *name, u16 first_msg_id, u16 last_msg_id)
{
    ...
    strncpy((char *) mp->plugin_name, (char *) name, sizeof(mp->plugin_name) - 1);
...
void vl_api_save_msg_table(void)
{
     ...
     */
    if (strstr((char *) am->save_msg_table_filename, "..") || index((char *) am->save_msg_table_filename, '/')) {
        clib_warning("illegal save-message-table filename '%s'", am->save_msg_table_filename);
        return;
    }
    ...
    if (rv != vec_len(serialized_message_table)) clib_unix_warning("write");

    rv = close(fd);
    if (rv < 0) clib_unix_warning("close");
...
static uword vl_api_clnt_process(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *f)
{
    ...
    e = vlib_call_init_exit_functions(vm, &vgm->api_init_function_registrations, 1 /* call_once */, 1 /* is_global */);
    ...
    for (i = 0; i < vec_len(am->msg_ranges); i++) {
        ...
        send_one_plugin_msg_ids_msg(rp->name, rp->first_msg_id, rp->last_msg_id);
    }

    /*
     * Save the api message table snapshot, if configured
     */
    if (am->save_msg_table_filename) vl_api_save_msg_table();
    ...
    while (1) {
        ...
        while (1) {
            if (vl_mem_api_handle_rpc(vm, node) || vl_mem_api_handle_msg_main(vm, node)) {
        ...
        if (PREDICT_FALSE(vec_len(am->vlib_private_rps))) {
            if (private_segment_rotor >= vec_len(am->vlib_private_rps)) private_segment_rotor = 0;
...
{
    vlib_node_set_state(vm, vl_api_clnt_node.index, (enable ? VLIB_NODE_STATE_POLLING : VLIB_NODE_STATE_DISABLED));
}
...
static void vl_api_rpc_call_t_handler(vl_api_rpc_call_t *mp)
{
    ...
    if (mp->function == 0) {
        ...
        if (mp->need_barrier_sync) vlib_worker_thread_barrier_sync(vm);

        fp = uword_to_pointer(mp->function, int (*)(void *));
        rv = fp(mp->data);

        if (mp->need_barrier_sync) vlib_worker_thread_barrier_release(vm);
    }
...
always_inline void vl_api_rpc_call_main_thread_inline(void *fp, u8 *data, u32 data_length, u8 force_rpc)
{
    ...
    if (vm == vm_global) clib_spinlock_lock_if_init(&vm_global->pending_rpc_lock);
    vec_add1(vm->pending_rpc_requests, (uword) mp);
    if (vm == vm_global) clib_spinlock_unlock_if_init(&vm_global->pending_rpc_lock);
}
...
{
    vl_api_rpc_call_main_thread_inline(fp,
                                       data,
                                       data_length, /*force_rpc */
                                       0);
}
...
static void vl_api_trace_plugin_msg_ids_t_handler(vl_api_trace_plugin_msg_ids_t *mp)
{
    ...
    if (am->replay_in_progress == 0) return;

    p = hash_get_mem(am->msg_range_by_name, mp->plugin_name);
    if (p == 0) {
        clib_warning("WARNING: traced plugin '%s' not in current image", mp->plugin_name);
        return;
    }
