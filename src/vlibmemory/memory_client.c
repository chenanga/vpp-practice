...
static void vl_api_rx_thread_exit_t_handler(vl_api_rx_thread_exit_t *mp)
{
    ...
    if (mm->rx_thread_jmpbuf_valid) longjmp(mm->rx_thread_jmpbuf, 1);
}
...
static void vl_api_name_and_crc_free(void)
{
    ...
    if (!am->msg_index_by_name_and_crc) return;

    hash_foreach_pair(hp, am->msg_index_by_name_and_crc, ({ vec_add1(keys, (u8 *) hp->key); }));
...
int vl_client_api_map(const char *region_name)
{
    ...
    if ((rv = vl_map_shmem(region_name, 0 /* is_vlib */)) < 0) return rv;
                                    ...
                                    int   do_map)
{
    ...
    if (vl_client_connect(client_name, 0 /* punt quota */, rx_queue_size /* input queue */) < 0) {
        vl_client_api_unmap();
        return -1;
    }
    ...
    if (thread_fn) {
        ...
        rv = pthread_create(&mm->rx_thread_handle, NULL /*attr */, thread_fn, thread_fn_arg);
        if (rv) {
...
{
    return connect_to_vlib_internal(svm_name, client_name, rx_queue_size, rx_thread_fn, 0 /* thread fn arg */, 1 /* do map */);
}

int vl_client_connect_to_vlib_no_rx_pthread(const char *svm_name, const char *client_name, int rx_queue_size)
{
    return connect_to_vlib_internal(svm_name, client_name, rx_queue_size, 0 /* no rx_thread_fn */, 0 /* no thread fn arg */, 1 /* do map */);
}

int vl_client_connect_to_vlib_no_map(const char *svm_name, const char *client_name, int rx_queue_size)
{
    return connect_to_vlib_internal(svm_name, client_name, rx_queue_size, rx_thread_fn, 0 /* no thread fn arg */, 0 /* dont map */);
}

int vl_client_connect_to_vlib_no_rx_pthread_no_map(const char *svm_name, const char *client_name, int rx_queue_size)
{
    return connect_to_vlib_internal(svm_name, client_name, rx_queue_size, 0 /* no thread_fn */, 0 /* no thread fn arg */, 0 /* dont map */);
}
...
{
    return connect_to_vlib_internal(svm_name, client_name, rx_queue_size, thread_fn, arg, 1 /* do map */);
}
...
static void disconnect_from_vlib_internal(u8 do_unmap)
{
    ...
    if (mm->connected_to_vlib) {
        ...
        if (do_unmap) vl_client_api_unmap();
    }
...
u16 vl_client_get_first_plugin_msg_id(const char *plugin_name)
{
    ...
    api_main_t                *am = vlibapi_get_main();
    memory_client_main_t      *mm = vlibapi_get_memory_client_main();
    vl_api_msg_data_t         *m  = vl_api_get_msg_data(am, VL_API_GET_FIRST_MSG_ID_REPLY);
    f64                        timeout;
    void                      *old_handler;
    clib_time_t                clib_time;
    u16                        rv = ~0;

    if (strlen(plugin_name) + 1 > sizeof(mp->name)) return (rv);
    ...
    if (!m->calc_size_func) {
        m->calc_size_func = (uword(*)(void *)) vl_api_get_first_msg_id_reply_t_calc_size;
    }
    ...
    if (!am->my_registration) {
        ...
        if (vl_socket_client_write() <= 0) goto sock_err;
        if (vl_socket_client_read(1)) goto sock_err;
...
result:
    ...
    if (rv == (u16) ~0) clib_warning("plugin '%s' not registered", plugin_name);
