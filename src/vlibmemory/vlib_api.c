...
static void vl_api_cli_inband_t_handler(vl_api_cli_inband_t *mp)
{
    ...
    if (vl_msg_api_get_msg_length(mp) < vl_api_string_len(&mp->cmd) + sizeof(*mp)) {
        rv = -1;
        goto error;
    }
    ...
    unformat_init_string(&input, (char *) cmd_vec, vl_api_string_len(&mp->cmd));
    rv = vlib_cli_input(vm, &input, inband_cli_output, (uword) &out_vec);
    unformat_free(&input);

error:
    REPLY_MACRO3(VL_API_CLI_INBAND_REPLY, vec_len(out_vec), ({ vl_api_vec_to_api_string(out_vec, &rmp->reply); }));
    vec_free(out_vec);
    vec_free(cmd_vec);
}
...
static void vl_api_get_node_index_t_handler(vl_api_get_node_index_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_GET_NODE_INDEX_REPLY, ({ rmp->node_index = htonl(node_index); }));
}
...
out:
    REPLY_MACRO2(VL_API_ADD_NODE_NEXT_REPLY, ({ rmp->next_index = htonl(next_index); }));
}
...
static void get_thread_data(vl_api_thread_data_t *td, int index)
{
    ...
    td->id                  = htonl(index);
    if (w->name) strncpy((char *) td->name, (char *) w->name, ARRAY_LEN(td->name) - 1);
    if (w->registration) strncpy((char *) td->type, (char *) w->registration->name, ARRAY_LEN(td->type) - 1);
    td->pid        = htonl(w->lwp);
...
static void vl_api_get_node_graph_t_handler(vl_api_get_node_graph_t *mp)
{
    ...
    vlib_node_get_nodes(vm, 0 /* main threads */, 0 /* include stats */, 1 /* barrier sync */, &node_dups, &stat_vms);
    vector = vlib_node_serialize(vm, node_dups, vector, 1 /* include nexts */, 1 /* include stats */);

    vl_msg_pop_heap(oldheap);

    REPLY_MACRO2(VL_API_GET_NODE_GRAPH_REPLY, ({ rmp->reply_in_shmem = (uword) vector; }));
}
...
out:
    REPLY_MACRO2(VL_API_GET_NEXT_INDEX_REPLY, ({ rmp->next_index = htonl(next_index); }));
}
...
static void vl_api_get_f64_endian_value_t_handler(vl_api_get_f64_endian_value_t *mp)
{
    ...
    if (1.0 != clib_net_to_host_f64(mp->f64_one)) rv = VNET_API_ERROR_API_ENDIAN_FAILED;

    REPLY_MACRO2(VL_API_GET_F64_ENDIAN_VALUE_REPLY, ({ rmp->f64_one_result = clib_host_to_net_f64(one); }));
}
...
static void vl_api_get_f64_increment_by_one_t_handler(vl_api_get_f64_increment_by_one_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_GET_F64_INCREMENT_BY_ONE_REPLY, ({ rmp->f64_value = clib_host_to_net_f64(clib_net_to_host_f64(mp->f64_value) + 1.0); }));
}

#include <vlibmemory/vlib.api.c>
static clib_error_t *vlib_apis_hookup(vlib_main_t *vm)
{
