...
void vl_sock_api_dump_clients(vlib_main_t *vm, api_main_t *am)
{
     ...
     */
    if (pool_elts(sm->registration_pool) < 2) return;
...
void vl_socket_api_send(vl_api_registration_t *rp, u8 *elem)
{
    ...
    sock_rp = pool_elt_at_index(sm->registration_pool, rp->vl_api_registration_pool_index);
    ASSERT(sock_rp);
    ...
    if (vec_len(sock_rp->output_vector) > 0 && !(cf->flags & UNIX_FILE_DATA_AVAILABLE_TO_WRITE)) {
...
void vl_socket_free_registration_index(u32 pool_index)
{
    ...
    vl_api_call_reaper_functions(clib_host_to_net_u32(sock_api_registration_handle(rp)));

    ASSERT(rp->registration_type != REGISTRATION_TYPE_FREE);
    for (i = 0; i < vec_len(rp->additional_fds_to_close); i++)
        if (close(rp->additional_fds_to_close[i]) < 0) clib_unix_warning("close");
...
static void vl_socket_request_remove_reg_index(u32 reg_index)
{
    ...
    vl_api_force_rpc_call_main_thread(socket_cleanup_pending_remove_registration_cb, (void *) &reg_index, sizeof(u32));
}
...
clib_error_t *vl_socket_read_ready(clib_file_t *uf)
{
    ...
    n = read(uf->file_descriptor, socket_main.input_buffer, vec_len(socket_main.input_buffer));
    ...
    do {
        ...
        if (vec_len(msg_buffer) <= sizeof(msgbuf_t)) /* No, so fragment is not a full message. */
            goto save_and_split;

        /* Now we know how big the full message will be. */
        msgbuf_len = ntohl(((msgbuf_t *) msg_buffer)->data_len) + sizeof(msgbuf_t);
        ...
        save_and_split:
            ...
            /* But if we were using the input buffer, save the fragment. */
            {
                ...
                clib_memcpy_fast(rp->unprocessed_input, msg_buffer, vec_len(msg_buffer));
        ...
        vlib_process_signal_event(vm, vl_api_clnt_node.index, SOCKET_READ_EVENT, a - socket_main.process_args);
        if (vec_len(msg_buffer) > msgbuf_len) /* There are some fragments left. Shrink the msg_buffer to simplify logic. */
...
clib_error_t *vl_socket_write_ready(clib_file_t *uf)
{
    ...
    if (vec_len(rp->output_vector) <= 0 && (uf->flags & UNIX_FILE_DATA_AVAILABLE_TO_WRITE)) {
...
void vl_api_sockclnt_create_t_handler(vl_api_sockclnt_create_t *mp)
{
    ...
    if (!regp || regp->registration_type != REGISTRATION_TYPE_SOCKET_SERVER) {
        clib_warning("unsupported API call: already connected though shared memory?");
        return;
    }
...
void vl_api_sock_init_shm_t_handler(vl_api_sock_init_shm_t *mp)
{
    ...
    if ((rv = ssvm_server_init_memfd(memfd))) goto reply;
    ...
    vl_init_shmem(vlib_rp, config, 1 /* is_vlib (dont-care) */, 1 /* is_private */);
...
static clib_error_t *socksvr_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    if (!vec_len(sm->socket_name)) sm->socket_name = format(0, "%s/%s", vlib_unix_get_runtime_dir(), API_SOCKET_FILENAME);
