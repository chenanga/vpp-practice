...
static void vl_api_cli_reply_t_handler(vl_api_cli_reply_t *mp)
{
...
static void vl_api_cli_inband_reply_t_handler(vl_api_cli_inband_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    i32         retval = ntohl(mp->retval);
    ...
    vam->retval = retval;
    if (retval == 0) vam->cmd_reply = vl_api_from_api_to_new_vec(mp, &mp->reply);
    vam->result_ready = 1;
}

static void vl_api_get_node_index_reply_t_handler(vl_api_get_node_index_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    i32         retval = ntohl(mp->retval);
    if (vam->async_mode) {
        ...
        vam->retval = retval;
        if (retval == 0) errmsg("node index %d", ntohl(mp->node_index));
        vam->result_ready = 1;
    }
}

static void vl_api_get_next_index_reply_t_handler(vl_api_get_next_index_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    i32         retval = ntohl(mp->retval);
    if (vam->async_mode) {
        ...
        vam->retval = retval;
        if (retval == 0) errmsg("next node index %d", ntohl(mp->next_index));
        vam->result_ready = 1;
    }
}

static void vl_api_add_node_next_reply_t_handler(vl_api_add_node_next_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    i32         retval = ntohl(mp->retval);
    if (vam->async_mode) {
        ...
        vam->retval = retval;
        if (retval == 0) errmsg("next index %d", ntohl(mp->next_index));
        vam->result_ready = 1;
    }
}

static void vl_api_get_f64_endian_value_reply_t_handler(vl_api_get_f64_endian_value_reply_t *mp)
{
    // not yet implemented
}

static void vl_api_get_f64_increment_by_one_reply_t_handler(vl_api_get_f64_increment_by_one_reply_t *mp)
{
    // not yet implemented
}

static int api_get_f64_endian_value(vat_main_t *vam)
{
    // not yet implemented
    return -1;
}

static int api_get_f64_increment_by_one(vat_main_t *vam)
{
    // not yet implemented
    return -1;
}
 ...
 */
static int exec_inband(vat_main_t *vam)
{
    vl_api_cli_inband_t *mp;
    unformat_input_t    *i = vam->input;
    int                  ret;

    if (vec_len(i->buffer) == 0) return -1;
    ...
    if (vec_len(vam->cmd_reply)) print(vam->ofp, "%v", (char *) (vam->cmd_reply));
    return ret;
}
static int api_cli_inband(vat_main_t *vam)
{
...
static int api_cli(vat_main_t *vam)
{
    return exec_inband(vam);
}
...
static void vl_api_show_threads_reply_t_handler(vl_api_show_threads_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    i32         retval = ntohl(mp->retval);
    int         i, count = 0;

    if (retval >= 0) count = ntohl(mp->count);

    for (i = 0; i < count; i++)
        print(vam->ofp,
              "\n%-2d %-11s %-11s %-5d %-6d %-4d %-6d",
              ntohl(mp->thread_data[i].id),
              mp->thread_data[i].name,
              mp->thread_data[i].type,
              ntohl(mp->thread_data[i].pid),
              ntohl(mp->thread_data[i].cpu_id),
              ntohl(mp->thread_data[i].core),
...
static int api_show_threads(vat_main_t *vam)
{
    vl_api_show_threads_t *mp;
    int                    ret;

    print(vam->ofp, "\n%-2s %-11s %-11s %-5s %-6s %-4s %-6s", "ID", "Name", "Type", "LWP", "cpu_id", "Core", "Socket");
...
static void vl_api_get_node_graph_reply_t_handler(vl_api_get_node_graph_reply_t *mp)
{
    vat_main_t  *vam    = &vat_main;
    i32          retval = ntohl(mp->retval);
    u8          *pvt_copy, *reply;
...
static void local_setup_message_id_table(vat_main_t *vam)
{
    ...
    hash_set_mem(vam->help_by_name, "exec", "usage: exec <vpe-debug-CLI-command>");
}
