...
static bool filter_message(char *msgname)
{
    char **p = filter_messages_strings;

    while (*p) {
        if (strcmp(*p, msgname) == 0) return true;
        p++;
    }
    return false;
}
...
static u8 *vat2_find_plugin_path(void)
{
    ...
    if ((rv = readlink("/proc/self/exe", path, PATH_MAX - 1)) == -1) return 0;

    /* readlink doesn't provide null termination */
    path[rv] = 0;

    /* strip filename */
    if ((p = strrchr(path, '/')) == 0) return 0;
    *p = 0;
...
void vac_callback(unsigned char *data, int len)
{
...
static int register_function(char *pluginpath)
{
    int loaded;
...
void vat2_register_function(char *name, cJSON (*f)(cJSON *), cJSON (*tojson)(void *), u32 crc)
{
...
static int vat2_exec_command_by_name(char *msgname, cJSON *o)
{
    u32 crc = 0;
    if (filter_message(msgname)) return 0;
...
static int vat2_exec_command(cJSON *o)
{
...
static void print_template(char *msgname)
{
    uword *p = hash_get_mem(function_by_name, msgname);
    if (!p) goto error;
...
int main(int argc, char **argv)
{
    ...
    while ((c = getopt_long(argc, argv, "hdp:f:t:", long_options, &option_index)) != -1) {
        switch (c) {
            case 0:
                if (option_index == 3) dump_api = true;
                break;
            case 'd':
                vat2_debug = true;
                break;
