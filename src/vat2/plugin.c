...
static int load_one_plugin(plugin_info_t *pi)
{
...
static u8 **split_plugin_path(u8 *plugin_path)
{
    int  i;
    u8 **rv   = 0;
        ...
        while ((entry = readdir(dp))) {
            ...
            if (filter) {
                ...
                for (j = 0; j < vec_len(filter); j++)
                    if (entry->d_name[j] != filter[j]) goto next;
            }

            plugin_name = format(0, "%s/%s%c", plugin_path[i], entry->d_name, 0);
            ...
            if (p == 0) {
                ...
                hash_set_mem(plugin_by_name_hash, plugin_name, pi - plugin_info);
                *loaded = *loaded + 1;
            }
        next:;
        }
