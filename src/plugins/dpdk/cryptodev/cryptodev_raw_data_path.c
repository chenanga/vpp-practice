...
static_always_inline u64 compute_ofs_linked_alg(vnet_crypto_async_frame_elt_t *fe, i16 *min_ofs, u32 *max_end)
{
    ...
    u32                      integ_end  = fe->integ_start_offset + fe->crypto_total_length + fe->integ_length_adj;
                                                   ...
                                                   u32                    size)
{
    ...
    if (vlib_buffer_chain_linearize(vm, b) > CRYPTODEV_MAX_N_SGL) return -1;
...
static_always_inline void cryptodev_reset_ctx(cryptodev_engine_thread_t *cet)
{
    ...
    rte_cryptodev_configure_raw_dp_ctx(cet->cryptodev_id, cet->cryptodev_q, cet->ctx, RTE_CRYPTO_OP_WITH_SESSION, sess_ctx, 0);
}
...
static_always_inline void cryptodev_frame_linked_algs_enqueue_internal(vlib_main_t *vm, vnet_crypto_async_frame_t *frame, cryptodev_op_type_t op_type)
{
    ...
    i16                            min_ofs;
    u32                            max_end;
    u32                            max_to_enq = clib_min(CRYPTODE_ENQ_MAX, frame->n_elts - ring->frames[*enq].enq_elts_head);
    u8                             is_update  = 0;
    int                            status;
    ...
    while (n_elts) {
        ...
        if (PREDICT_FALSE(last_key_index != fe->key_index)) {
            ...
            if (PREDICT_FALSE(key->keys[vm->numa_node][op_type] == 0)) {
                ...
                if (PREDICT_FALSE(status < 0)) goto error_exit;
            }
             ...
             * packet is processed */
            if (PREDICT_FALSE(cet->reset_sess == 0)) cet->reset_sess = key->keys[vm->numa_node][op_type];

            sess_ctx.crypto_sess = key->keys[vm->numa_node][op_type];

            status =
                rte_cryptodev_configure_raw_dp_ctx(cet->cryptodev_id, cet->cryptodev_q, cet->ctx, RTE_CRYPTO_OP_WITH_SESSION, sess_ctx, is_update);
            if (PREDICT_FALSE(status < 0)) goto error_exit;
        ...
        if (PREDICT_FALSE(fe->flags & VNET_CRYPTO_OP_FLAG_CHAINED_BUFFERS)) {
            ...
            if (cryptodev_frame_build_sgl(vm, cmt->iova_mode, vec, &n_seg, b[0], max_end - min_ofs - vec->len) < 0) goto error_exit;
        }

        status = rte_cryptodev_raw_enqueue(cet->ctx, vec, n_seg, cofs, &iv_vec, &digest_vec, 0, (void *) frame);
        if (PREDICT_FALSE(status < 0)) goto error_exit;
...
static_always_inline void cryptodev_raw_aead_enqueue_internal(vlib_main_t               *vm,
                                                              ...
                                                              u8                         aad_len)
{
    ...
    while (n_elts) {
        ...
        if (cmt->iova_mode == RTE_IOVA_VA) {
            ...
            vec[0].base     = (void *) (b[0]->data + fe->crypto_start_offset);
            vec[0].iova     = vlib_buffer_get_pa(vm, b[0]) + fe->crypto_start_offset;
            vec[0].len      = fe->crypto_total_length;
            iv_vec.va       = (void *) fe->iv;
            iv_vec.iova     = vlib_physmem_get_pa(vm, fe->iv);
            aad_vec.va      = (void *) (cet->aad_buf + aad_offset);
        ...
        if (PREDICT_FALSE(fe->flags & VNET_CRYPTO_OP_FLAG_CHAINED_BUFFERS)) {
            vec[0].len = b[0]->current_data + b[0]->current_length - fe->crypto_start_offset;
            status     = cryptodev_frame_build_sgl(vm, cmt->iova_mode, vec, &n_seg, b[0], fe->crypto_total_length - vec[0].len);
            if (status < 0) goto error_exit;
        }

        status = rte_cryptodev_raw_enqueue(cet->ctx, vec, n_seg, cofs, &iv_vec, &digest_vec, &aad_vec, (void *) frame);
        if (PREDICT_FALSE(status < 0)) goto error_exit;
...
static_always_inline void cryptodev_post_dequeue(void *frame, u32 index, u8 is_op_success)
{
    ...
    f->elts[index].status = is_op_success ? VNET_CRYPTO_OP_STATUS_COMPLETED : VNET_CRYPTO_OP_STATUS_FAIL_BAD_HMAC;
}

static_always_inline u8 cryptodev_raw_dequeue_internal(vlib_main_t *vm, u32 *enqueue_thread_idx)
{
    cryptodev_main_t          *cmt = &cryptodev_main;
    cryptodev_engine_thread_t *cet = cmt->per_thread_data + vm->thread_index;
    vnet_crypto_async_frame_t *frame;
    ...
    n_deq = rte_cryptodev_raw_dequeue_burst(cet->ctx, NULL, max_to_deq, cryptodev_post_dequeue, (void **) &frame, 0, &n_success, &dequeue_status);
    ...
    if (PREDICT_FALSE(n_success < n_deq)) {
        ...
        for (i = 0; i < n_deq; i++) {
            if (frame->elts[idx + i].status != VNET_CRYPTO_OP_STATUS_COMPLETED) ring->frames[*deq].frame_elts_errs_mask |= 1 << (idx + i);
        }
    }
    ...
    int res = rte_cryptodev_raw_dequeue_done(cet->ctx, cet->inflight - inflight);
    ERROR_ASSERT(res == 0);
    cet->inflight = inflight;
    return dequeue_more;
}

static_always_inline void cryptodev_enqueue_frame_to_qat(vlib_main_t *vm, cryptodev_cache_ring_elt_t *ring_elt)
{
    ...
    if (linked_or_aad_len == 1)
        ...
        cryptodev_raw_aead_enqueue_internal(vm, ring_elt->f, op_type, linked_or_aad_len);
}
...
static_always_inline vnet_crypto_async_frame_t *cryptodev_raw_dequeue(vlib_main_t *vm, u32 *nb_elts_processed, u32 *enqueue_thread_idx)
{
    ...
    if (PREDICT_TRUE(ring->frames[ring->enq_head].f != 0)) cryptodev_enqueue_frame_to_qat(vm, &ring->frames[ring->enq_head]);

    if (PREDICT_TRUE(ring_elt->f != 0) && (ring_elt->n_elts == ring_elt->deq_elts_tail)) {
...
static_always_inline int cryptodev_raw_enq_aead_aad_0_enc(vlib_main_t *vm, vnet_crypto_async_frame_t *frame)
{
...
static_always_inline int cryptodev_raw_enq_aead_aad_0_dec(vlib_main_t *vm, vnet_crypto_async_frame_t *frame)
{
...
clib_error_t *cryptodev_register_raw_hdl(vlib_main_t *vm, u32 eidx)
{
    ...
    if (!support_raw_api) return cryptodev_register_cop_hdl(vm, eidx);
    ...
    vec_foreach (cet, cmt->per_thread_data) {
        ...
        cet->aad_buf = rte_zmalloc_socket(0, CRYPTODEV_NB_CRYPTO_OPS * CRYPTODEV_MAX_AAD_SIZE, CLIB_CACHE_LINE_BYTES, numa);
        if (cet->aad_buf == 0) {
        ...
        cet->ctx = rte_zmalloc_socket(0, max_ctx_size, CLIB_CACHE_LINE_BYTES, numa);
        if (!cet->ctx) {
...
#define _(a, b, c, d, e, f, g)                                                                                                            \
    ...
    if (cryptodev_check_cap_support(&cap_aead_idx, g, e, f)) {                                                                            \
        vnet_crypto_register_enqueue_handler(vm, eidx, VNET_CRYPTO_OP_##a##_TAG##e##_AAD##f##_ENC, cryptodev_raw_enq_aead_aad_##f##_enc); \
        vnet_crypto_register_enqueue_handler(vm, eidx, VNET_CRYPTO_OP_##a##_TAG##e##_AAD##f##_DEC, cryptodev_raw_enq_aead_aad_##f##_dec); \
        ref_cnt++;                                                                                                                        \
    }
...
#define _(a, b, c, d, e)                                                                                                           \
    ...
    if (cryptodev_check_cap_support(&cap_cipher_idx, c, -1, -1) && cryptodev_check_cap_support(&cap_auth_idx, -1, e, -1)) {        \
        vnet_crypto_register_enqueue_handler(vm, eidx, VNET_CRYPTO_OP_##a##_##d##_TAG##e##_ENC, cryptodev_raw_enq_linked_alg_enc); \
        vnet_crypto_register_enqueue_handler(vm, eidx, VNET_CRYPTO_OP_##a##_##d##_TAG##e##_DEC, cryptodev_raw_enq_linked_alg_dec); \
        ref_cnt++;                                                                                                                 \
    }
