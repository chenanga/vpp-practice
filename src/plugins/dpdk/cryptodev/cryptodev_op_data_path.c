...
{
    u64 index;
    if (mode == RTE_IOVA_VA) return (rte_iova_t) pointer_to_uword(data);

    index = clib_pmalloc_get_page_index(pm, data);
    return pointer_to_uword(data) - pm->lookup_table[index];
}

static_always_inline void cryptodev_validate_mbuf(struct rte_mbuf *mb, vlib_buffer_t *b)
{
    /* on vnet side vlib_buffer current_length is updated by cipher padding and
     * icv_sh. mbuf needs to be sync with these changes */
    u16 data_len = b->current_length + (b->data + b->current_data - rte_pktmbuf_mtod(mb, u8 *));
...
static_always_inline void cryptodev_validate_mbuf_chain(vlib_main_t *vm, struct rte_mbuf *mb, vlib_buffer_t *b)
{
    ...
    while (b->flags & VLIB_BUFFER_NEXT_PRESENT) {
        ...
        if (PREDICT_FALSE((b->flags & VLIB_BUFFER_EXT_HDR_VALID) == 0)) rte_pktmbuf_reset(mb);
        last_mb->next = mb;
        last_mb       = mb;
        ...
        first_mb->nb_segs++;
        if (PREDICT_FALSE(b->ref_count > 1)) mb->pool = dpdk_no_cache_mempool_by_buffer_pool_index[b->buffer_pool_index];
    }
}
...
static_always_inline int cryptodev_frame_linked_algs_enqueue(vlib_main_t *vm, vnet_crypto_async_frame_t *frame, cryptodev_op_type_t op_type)
{
    ...
    cryptodev_cache_ring_elt_t *ring_elt = cryptodev_cache_ring_push(ring, frame);

    if (PREDICT_FALSE(ring_elt == NULL)) return -1;
...
static_always_inline void cryptodev_frame_linked_algs_enqueue_internal(vlib_main_t *vm, vnet_crypto_async_frame_t *frame, cryptodev_op_type_t op_type)
{
    cryptodev_main_t              *cmt  = &cryptodev_main;
    ...
    if (PREDICT_FALSE(frame == 0 || frame->n_elts == 0)) return;

    max_to_enq = clib_min(CRYPTODE_ENQ_MAX, frame->n_elts - ring->frames[*enq].enq_elts_head);

    if (cet->inflight + max_to_enq > CRYPTODEV_MAX_INFLIGHT) return;

    n_elts = max_to_enq;

    if (PREDICT_FALSE(rte_mempool_get_bulk(cet->cop_pool, (void **) cops, n_elts) < 0)) {
    ...
    while (n_elts) {
        ...
        if (last_key_index != fe->key_index) {
            ...
            if (key->keys[vm->numa_node][op_type] == 0) {
                if (PREDICT_FALSE(cryptodev_session_create(vm, last_key_index, 0) < 0)) {
                    cryptodev_mark_frame_fill_err(frame,
                                                  ring->frames[*enq].frame_elts_errs_mask,
                                                  ring->frames[*enq].enq_elts_head,
                                                  max_to_enq,
        ...
        sop->auth.digest.phys_addr = cryptodev_get_iova(pm, cmt->iova_mode, fe->digest);
    ...
    n_enqueue = rte_cryptodev_enqueue_burst(cet->cryptodev_id, cet->cryptodev_q, (struct rte_crypto_op **) cops, max_to_enq);
...
static_always_inline int cryptodev_aead_enqueue_internal(vlib_main_t *vm, vnet_crypto_async_frame_t *frame, cryptodev_op_type_t op_type, u8 aad_len)
{
    cryptodev_main_t              *cmt  = &cryptodev_main;
    cryptodev_engine_thread_t     *cet  = cmt->per_thread_data + vm->thread_index;
    cryptodev_cache_ring_t        *ring = &cet->cache_ring;
    ...
    if (PREDICT_FALSE(frame == 0 || frame->n_elts == 0)) return -1;

    if (cet->inflight + max_to_enq > CRYPTODEV_MAX_INFLIGHT) return -1;
    ...
    while (n_elts) {
        ...
        if (last_key_index != fe->key_index) {
            ...
            last_key_index = fe->key_index;
            if (key->keys[vm->numa_node][op_type] == 0) {
                if (PREDICT_FALSE(cryptodev_session_create(vm, last_key_index, aad_len) < 0)) {
                    cryptodev_mark_frame_fill_err(frame,
                                                  ring->frames[*enq].frame_elts_errs_mask,
                                                  ring->frames[*enq].enq_elts_head,
                                                  max_to_enq,
                    ...
                    goto error_exit;
                         ...
                         rte_cryptodev_sym_session_opaque_data_get(key->keys[vm->numa_node][op_type]) != (u64) aad_len
#else
                         key->keys[vm->numa_node][op_type]->opaque_data != aad_len
#endif
                         )) {
                cryptodev_sess_handler(vm, VNET_CRYPTO_KEY_OP_DEL, fe->key_index, aad_len);
                if (PREDICT_FALSE(cryptodev_session_create(vm, last_key_index, aad_len) < 0)) {
                    cryptodev_mark_frame_fill_err(frame,
                                                  ring->frames[*enq].frame_elts_errs_mask,
                                                  ring->frames[*enq].enq_elts_head,
        ...
        sop->aead.digest.phys_addr = cryptodev_get_iova(pm, cmt->iova_mode, fe->tag);
        if (PREDICT_FALSE(fe->flags & VNET_CRYPTO_OP_FLAG_CHAINED_BUFFERS))
            cryptodev_validate_mbuf_chain(vm, sop->m_src, b);
        else
            cryptodev_validate_mbuf(sop->m_src, b);
...
static_always_inline u8 cryptodev_frame_dequeue_internal(vlib_main_t *vm, u32 *enqueue_thread_idx)
{
    cryptodev_main_t              *cmt   = &cryptodev_main;
    cryptodev_engine_thread_t     *cet   = cmt->per_thread_data + vm->thread_index;
    ...
    left_to_deq = ring->frames[*deq].f->n_elts - ring->frames[*deq].deq_elts_tail;
    ...
    n_deq = rte_cryptodev_dequeue_burst(cet->cryptodev_id, cet->cryptodev_q, (struct rte_crypto_op **) cops, max_to_deq);

    if (n_deq == 0) return dequeue_more;
    ...
    while (n_elts > 4) {
        ...
        err1 |= ((u64) (fe[1].status == VNET_CRYPTO_OP_STATUS_COMPLETED)) << (n + 1);
        err2 |= ((u64) (fe[2].status == VNET_CRYPTO_OP_STATUS_COMPLETED)) << (n + 2);
        err3 |= ((u64) (fe[3].status == VNET_CRYPTO_OP_STATUS_COMPLETED)) << (n + 3);
    ...
    if (cryptodev_cache_ring_update_deq_tail(ring, deq)) {
        u32 fr_processed = (CRYPTODEV_CACHE_QUEUE_SIZE - ring->tail + ring->deq_tail) & CRYPTODEV_CACHE_QUEUE_MASK;
...
static_always_inline void cryptodev_enqueue_frame(vlib_main_t *vm, cryptodev_cache_ring_elt_t *ring_elt)
{
    ...
    if (linked_or_aad_len == 1)
        ...
        cryptodev_aead_enqueue_internal(vm, ring_elt->f, op_type, linked_or_aad_len);
}
...
static_always_inline vnet_crypto_async_frame_t *cryptodev_frame_dequeue(vlib_main_t *vm, u32 *nb_elts_processed, u32 *enqueue_thread_idx)
{
    ...
    if (PREDICT_TRUE(ring->frames[ring->enq_head].f != 0)) cryptodev_enqueue_frame(vm, &ring->frames[ring->enq_head]);

    if (PREDICT_TRUE(ring_elt->f != 0)) {
        if (ring_elt->n_elts == ring_elt->deq_elts_tail) {
            *nb_elts_processed = ring_elt->n_elts;
            vlib_node_set_interrupt_pending(vlib_get_main_by_index(vm->thread_index), cm->crypto_node_index);
...
static_always_inline int cryptodev_enqueue_aead_aad_0_enc(vlib_main_t *vm, vnet_crypto_async_frame_t *frame)
{
    return cryptodev_frame_aead_enqueue(vm, frame, CRYPTODEV_OP_TYPE_ENCRYPT, 0);
}
static_always_inline int cryptodev_enqueue_aead_aad_8_enc(vlib_main_t *vm, vnet_crypto_async_frame_t *frame)
{
    return cryptodev_frame_aead_enqueue(vm, frame, CRYPTODEV_OP_TYPE_ENCRYPT, 8);
}
static_always_inline int cryptodev_enqueue_aead_aad_12_enc(vlib_main_t *vm, vnet_crypto_async_frame_t *frame)
{
    return cryptodev_frame_aead_enqueue(vm, frame, CRYPTODEV_OP_TYPE_ENCRYPT, 12);
}

static_always_inline int cryptodev_enqueue_aead_aad_0_dec(vlib_main_t *vm, vnet_crypto_async_frame_t *frame)
{
    return cryptodev_frame_aead_enqueue(vm, frame, CRYPTODEV_OP_TYPE_DECRYPT, 0);
}
static_always_inline int cryptodev_enqueue_aead_aad_8_dec(vlib_main_t *vm, vnet_crypto_async_frame_t *frame)
{
    return cryptodev_frame_aead_enqueue(vm, frame, CRYPTODEV_OP_TYPE_DECRYPT, 8);
}
static_always_inline int cryptodev_enqueue_aead_aad_12_dec(vlib_main_t *vm, vnet_crypto_async_frame_t *frame)
{
    return cryptodev_frame_aead_enqueue(vm, frame, CRYPTODEV_OP_TYPE_DECRYPT, 12);
}

static_always_inline int cryptodev_enqueue_linked_alg_enc(vlib_main_t *vm, vnet_crypto_async_frame_t *frame)
{
    return cryptodev_frame_linked_algs_enqueue(vm, frame, CRYPTODEV_OP_TYPE_ENCRYPT);
}

static_always_inline int cryptodev_enqueue_linked_alg_dec(vlib_main_t *vm, vnet_crypto_async_frame_t *frame)
{
    return cryptodev_frame_linked_algs_enqueue(vm, frame, CRYPTODEV_OP_TYPE_DECRYPT);
}
...
clib_error_t *cryptodev_register_cop_hdl(vlib_main_t *vm, u32 eidx)
{
    ...
    vec_foreach (cet, cmt->per_thread_data) {
        ...
        cet->cop_pool    = rte_mempool_create((char *) name,
                                           CRYPTODEV_NB_CRYPTO_OPS,
                                           sizeof(cryptodev_op_t),
                                           0,
        ...
        vec_free(name);
        if (!cet->cop_pool) {
            error = clib_error_return(0, "Failed to create cryptodev op pool %s", name);
...
#define _(a, b, c, d, e, f, g)                                                                                                            \
    ...
    if (cryptodev_check_cap_support(&cap_aead_idx, g, e, f)) {                                                                            \
        vnet_crypto_register_enqueue_handler(vm, eidx, VNET_CRYPTO_OP_##a##_TAG##e##_AAD##f##_ENC, cryptodev_enqueue_aead_aad_##f##_enc); \
        vnet_crypto_register_enqueue_handler(vm, eidx, VNET_CRYPTO_OP_##a##_TAG##e##_AAD##f##_DEC, cryptodev_enqueue_aead_aad_##f##_dec); \
        ref_cnt++;                                                                                                                        \
    }
...
#define _(a, b, c, d, e)                                                                                                           \
    ...
    if (cryptodev_check_cap_support(&cap_cipher_idx, c, -1, -1) && cryptodev_check_cap_support(&cap_auth_idx, -1, e, -1)) {        \
        vnet_crypto_register_enqueue_handler(vm, eidx, VNET_CRYPTO_OP_##a##_##d##_TAG##e##_ENC, cryptodev_enqueue_linked_alg_enc); \
        vnet_crypto_register_enqueue_handler(vm, eidx, VNET_CRYPTO_OP_##a##_##d##_TAG##e##_DEC, cryptodev_enqueue_linked_alg_dec); \
        ref_cnt++;                                                                                                                 \
    }
        foreach_cryptodev_link_async_alg
#undef _

        if (ref_cnt) vnet_crypto_register_dequeue_handler(vm, eidx, cryptodev_frame_dequeue);

    return 0;

error_exit:
    vec_foreach (cet, cmt->per_thread_data) {
        if (cet->cop_pool) rte_mempool_free(cet->cop_pool);
    }
