...
#define CRYPTODEV_CACHE_RING_GET_FRAME(r, i) ((r)->frames[(i) &CRYPTODEV_CACHE_QUEUE_MASK].f)

#define CRYPTODEV_CACHE_RING_GET_ERR_MASK(r, i) ((r)->frames[(i) &CRYPTODEV_CACHE_QUEUE_MASK].frame_elts_errs_mask)

#define CRYPTODEV_CACHE_RING_GET_FRAME_ELTS_INFLIGHT(r, i) \
    (((r)->frames[(i) &CRYPTODEV_CACHE_QUEUE_MASK].enq_elts_head) - ((r)->frames[(i) &CRYPTODEV_CACHE_QUEUE_MASK].deq_elts_tail))

static_always_inline void cryptodev_cache_ring_update_enq_head(cryptodev_cache_ring_t *r, vnet_crypto_async_frame_t *f)
{
...
static_always_inline bool cryptodev_cache_ring_update_deq_tail(cryptodev_cache_ring_t *r, u16 *const deq)
{
...
static_always_inline u64 cryptodev_mark_frame_fill_err(vnet_crypto_async_frame_t *f, u64 current_err, u16 index, u16 n, vnet_crypto_op_status_t op_s)
{
    u64 err = current_err;
    u16 i;
...
static_always_inline cryptodev_cache_ring_elt_t *cryptodev_cache_ring_push(cryptodev_cache_ring_t *r, vnet_crypto_async_frame_t *f)
{
    u16 head = r->head;
    u16 tail = r->tail;
     ...
     **/
#if CLIB_DEBUG > 0
    u16 n_cached = (head >= tail) ? (head - tail) : (CRYPTODEV_CACHE_QUEUE_MASK - tail + head);
    ERROR_ASSERT(n_cached < CRYPTODEV_CACHE_QUEUE_SIZE);
    ERROR_ASSERT(r->raw == 0 && r->frames[head].raw == 0 && r->frames[head].f == 0);
#endif
    /*the ring capacity is CRYPTODEV_CACHE_QUEUE_SIZE - 1*/
    if (PREDICT_FALSE(head + 1) == tail) return 0;
...
static_always_inline vnet_crypto_async_frame_t *cryptodev_cache_ring_pop(cryptodev_cache_ring_t *r)
{
    vnet_crypto_async_frame_t  *f;
    u16                         tail     = r->tail;
    ...
    ERROR_ASSERT(ring_elt->deq_elts_tail == ring_elt->enq_elts_head && ring_elt->deq_elts_tail == ring_elt->n_elts);

    f        = CRYPTODEV_CACHE_RING_GET_FRAME(r, tail);
    f->state = CRYPTODEV_CACHE_RING_GET_ERR_MASK(r, r->tail) == 0 ? VNET_CRYPTO_FRAME_STATE_SUCCESS : VNET_CRYPTO_FRAME_STATE_ELT_ERROR;
