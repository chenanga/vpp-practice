                                            ...
                                            u32                          aad_len)
{
    ...
    if (key->alg == VNET_CRYPTO_ALG_AES_128_GCM || key->alg == VNET_CRYPTO_ALG_AES_192_GCM || key->alg == VNET_CRYPTO_ALG_AES_256_GCM) {
    ...
    aead_xform->op            = (op_type == CRYPTODEV_OP_TYPE_ENCRYPT) ? RTE_CRYPTO_AEAD_OP_ENCRYPT : RTE_CRYPTO_AEAD_OP_DECRYPT;
...
static_always_inline int prepare_linked_xform(struct rte_crypto_sym_xform *xforms, cryptodev_op_type_t op_type, const vnet_crypto_key_t *key)
{
    ...
    if (!key_cipher || !key_auth) return -1;
...
static_always_inline void cryptodev_session_del(cryptodev_session_t *sess)
{
    u32 n_devs, i;
    ...
    for (i = 0; i < n_devs; i++)
        ...
        if (rte_cryptodev_sym_session_free(i, sess) == 0) break;
#else
...
static int check_cipher_support(enum rte_crypto_cipher_algorithm algo, u32 key_size)
{
    ...
    vec_foreach (vcap, cmt->supported_caps) {
        if (vcap->xform_type != RTE_CRYPTO_SYM_XFORM_CIPHER) continue;
        if (vcap->cipher.algo != algo) continue;
        vec_foreach (s, vcap->cipher.key_sizes)
            if (*s == key_size) return 1;
    }
...
static int check_auth_support(enum rte_crypto_auth_algorithm algo, u32 digest_size)
{
    ...
    vec_foreach (vcap, cmt->supported_caps) {
        if (vcap->xform_type != RTE_CRYPTO_SYM_XFORM_AUTH) continue;
        if (vcap->auth.algo != algo) continue;
        vec_foreach (s, vcap->auth.digest_sizes)
            if (*s == digest_size) return 1;
    }
...
static_always_inline int check_aead_support(enum rte_crypto_aead_algorithm algo, u32 key_size, u32 digest_size, u32 aad_size)
{
    ...
    vec_foreach (vcap, cmt->supported_caps) {
        if (vcap->xform_type != RTE_CRYPTO_SYM_XFORM_AEAD) continue;
        if (vcap->aead.algo != algo) continue;
    ...
    if (key_match == 1 && digest_match == 1 && aad_match == 1) return 1;
...
#define _(a, b, c, d, e)                     \
    case VNET_CRYPTO_ALG_##a##_##d##_TAG##e: \
        if (check_cipher_support(RTE_CRYPTO_CIPHER_##b, c) && check_auth_support(RTE_CRYPTO_AUTH_##d##_HMAC, e)) return 1;
            foreach_cryptodev_link_async_alg
#undef _
                default : return 0;
        }
...
#define _(a, b, c, d, e, f, g)                                           \
    if (key->alg == VNET_CRYPTO_ALG_##a) {                               \
        if (check_aead_support(RTE_CRYPTO_AEAD_##c, g, e, f)) matched++; \
    }
...
void cryptodev_sess_handler(vlib_main_t *vm, vnet_crypto_key_op_t kop, vnet_crypto_key_index_t idx, u32 aad_len)
{
    ...
    if (kop == VNET_CRYPTO_KEY_OP_DEL || kop == VNET_CRYPTO_KEY_OP_MODIFY) {
        if (idx >= vec_len(cmt->keys)) return;

        vec_foreach_index (i, cmt->per_numa_data) {
            if (!ckey->keys) continue;
            if (!ckey->keys[i]) continue;
    ...
    if (cryptodev_check_supported_vnet_alg(key) == 0) return;
...
/*static*/ void cryptodev_key_handler(vlib_main_t *vm, vnet_crypto_key_op_t kop, vnet_crypto_key_index_t idx)
{
    cryptodev_sess_handler(vm, kop, idx, 8);
}

clib_error_t *allocate_session_pools(u32 numa_node, cryptodev_session_pool_t *sess_pools_elt, u32 len)
{
    cryptodev_main_t *cmt = &cryptodev_main;
    u8               *name;
...
#if RTE_VERSION >= RTE_VERSION_NUM(22, 11, 0, 0)
    sess_pools_elt->sess_pool = rte_cryptodev_sym_session_pool_create((char *) name, CRYPTODEV_NB_SESSION, cmt->sess_sz, 0, 0, numa_node);
#else
    sess_pools_elt->sess_pool = rte_cryptodev_sym_session_pool_create((char *) name, CRYPTODEV_NB_SESSION, 0, 0, 0, numa_node);
#endif
    ...
    sess_pools_elt->sess_priv_pool =
        rte_mempool_create((char *) name, CRYPTODEV_NB_SESSION * (cmt->drivers_cnt), cmt->sess_sz, 0, 0, NULL, NULL, NULL, NULL, numa_node, 0);
...
clear_mempools:
    if (error) {
        vec_free(name);
        if (sess_pools_elt->sess_pool) rte_mempool_free(sess_pools_elt->sess_pool);
#if RTE_VERSION < RTE_VERSION_NUM(22, 11, 0, 0)
        if (sess_pools_elt->sess_priv_pool) rte_mempool_free(sess_pools_elt->sess_priv_pool);
#endif
        return error;
    }
    return 0;
...
int cryptodev_session_create(vlib_main_t *vm, vnet_crypto_key_index_t idx, u32 aad_len)
{
    ...
    vec_foreach (sess_pools_elt, numa_data->sess_pools) {
        if (sess_pools_elt->sess_pool == NULL) {
            error = allocate_session_pools(numa_node, sess_pools_elt, vec_len(numa_data->sess_pools) - 1);
            if (error) {
                ret = -1;
                goto clear_key;
            }
    ...
    sessions[CRYPTODEV_OP_TYPE_ENCRYPT] = rte_cryptodev_sym_session_create(sess_pool);

    sessions[CRYPTODEV_OP_TYPE_DECRYPT] = rte_cryptodev_sym_session_create(sess_pool);
#endif
    ...
    sessions[CRYPTODEV_OP_TYPE_DECRYPT] = rte_cryptodev_sym_session_create(dev_id, xforms_dec, sess_pool);
    if (!sessions[CRYPTODEV_OP_TYPE_ENCRYPT] || !sessions[CRYPTODEV_OP_TYPE_DECRYPT]) {
        ret = -1;
        goto clear_key;
    }

    rte_cryptodev_sym_session_opaque_data_set(sessions[CRYPTODEV_OP_TYPE_ENCRYPT], aad_len);
    ...
    vec_foreach (dev_inst, cmt->cryptodev_inst) {
        ...
        if (sessions[CRYPTODEV_OP_TYPE_ENCRYPT]->sess_data[driver_id].data && sessions[CRYPTODEV_OP_TYPE_DECRYPT]->sess_data[driver_id].data)
            continue;

        ret = rte_cryptodev_sym_session_init(dev_id, sessions[CRYPTODEV_OP_TYPE_ENCRYPT], xforms_enc, sess_priv_pool);
        ret = rte_cryptodev_sym_session_init(dev_id, sessions[CRYPTODEV_OP_TYPE_DECRYPT], xforms_dec, sess_priv_pool);
        if (ret < 0) goto clear_key;
    }
    ...
    CLIB_MEMORY_STORE_BARRIER();
    ckey->keys[numa_node][CRYPTODEV_OP_TYPE_ENCRYPT] = sessions[CRYPTODEV_OP_TYPE_ENCRYPT];
    ckey->keys[numa_node][CRYPTODEV_OP_TYPE_DECRYPT] = sessions[CRYPTODEV_OP_TYPE_DECRYPT];
...
static_always_inline int cryptodev_assign_resource(cryptodev_engine_thread_t *cet, u32 cryptodev_inst_index, cryptodev_resource_assign_op_t op)
{
    ...
    if (cet->inflight) return -EBUSY;

    switch (op) {
        case CRYPTODEV_RESOURCE_ASSIGN_AUTO:
            if (clib_bitmap_count_set_bits(cmt->active_cdev_inst_mask) >= vec_len(cmt->cryptodev_inst)) return -1;
        ...
        case CRYPTODEV_RESOURCE_ASSIGN_UPDATE:
            ...
            if (clib_bitmap_get(cmt->active_cdev_inst_mask, cryptodev_inst_index) == 1) return -EBUSY;

            vec_foreach_index (idx, cmt->cryptodev_inst) {
                cinst = cmt->cryptodev_inst + idx;
                if (cinst->dev_id == cet->cryptodev_id && cinst->q_id == cet->cryptodev_q) break;
            }
            /* invalid existing worker resource assignment */
            if (idx >= vec_len(cmt->cryptodev_inst)) return -EINVAL;
            clib_spinlock_lock(&cmt->tlock);
...
static u8 *format_cryptodev_inst(u8 *s, va_list *args)
{
    ...
    vec_foreach_index (thread_index, cmt->per_thread_data) {
        ...
        if (vlib_num_workers() > 0 && thread_index == 0) continue;
    ...
    if (thread_index == vec_len(cmt->per_thread_data)) s = format(s, "%s\n", "free");
...
static clib_error_t *cryptodev_show_cache_rings_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    cryptodev_main_t *cmt          = &cryptodev_main;
    u32               thread_index = 0;
    u16               i;
    vec_foreach_index (thread_index, cmt->per_thread_data) {
        ...
        u16 n_frames_inflight = (enq_head == deq_tail) ? 0 : ((CRYPTODEV_CACHE_QUEUE_SIZE + enq_head - deq_tail) & CRYPTODEV_CACHE_QUEUE_MASK);
        /* even if some elements of dequeued frame are still pending for deq
         * we consider the frame as processed */
        u16 n_frames_processed = ((tail == deq_tail) && (ring->frames[deq_tail].f == 0))
                                     ? 0
        ...
        u16 n_frames_pending = (head == enq_head) ? 0 : ((CRYPTODEV_CACHE_QUEUE_SIZE - enq_head + head) & CRYPTODEV_CACHE_QUEUE_MASK) - 1;
        ...
        vlib_cli_output(vm,
                        ...
                        "to be enqueued: %u",
                        ring->frames[enq_head].n_elts,
                        elts_to_enq);
        vlib_cli_output(vm,
                        "Elements in current frame to be dequeued: %u, waiting "
                        "to be dequeued: %u",
                        ring->frames[deq_tail].n_elts,
                        elts_to_deq);
        vlib_cli_output(vm, "\n\n");
    }
    return 0;
...
static clib_error_t *cryptodev_set_assignment_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (thread_index == 0 && vlib_num_workers() > 0) {
        error = clib_error_return(0, "assign crypto resource for master thread");
        return error;
    }
...
static int cryptodev_configure(vlib_main_t *vm, u32 cryptodev_id)
{
     ...
     */
#if RTE_VERSION >= RTE_VERSION_NUM(22, 11, 0, 0)
    if (cmt->drivers_cnt == 1 && cmt->driver_id != info.driver_id) return -1;
#endif

    if (!(info.feature_flags & RTE_CRYPTODEV_FF_SYMMETRIC_CRYPTO)) return -1;
    ...
    for (i = 0; i < info.max_nb_queue_pairs; i++) {
        ...
        ret = rte_cryptodev_queue_pair_setup(cryptodev_id, i, &qp_cfg, info.device->numa_node);
        if (ret) {
            clib_warning("Cryptodev: Configure device %u queue %u failed %d", cryptodev_id, i, ret);
            break;
        }
    }

    if (i != info.max_nb_queue_pairs) return -1;
    ...
    for (i = 0; i < info.max_nb_queue_pairs; i++) {
        ...
        snprintf(cdev_inst->desc, strlen(dev_name) + 9, "%s_q%u", info.device->name, i);
    }
...
int cryptodev_check_cap_support(struct rte_cryptodev_sym_capability_idx *idx, u32 key_size, u32 digest_size, u32 aad_size)
{
    ...
    vec_foreach (cap, cmt->supported_caps) {
        if (cap->xform_type != idx->type) continue;

        if (idx->type == RTE_CRYPTO_SYM_XFORM_AUTH && cap->auth.algo == idx->algo.auth
            && cryptodev_supports_param_value(cap->auth.digest_sizes, digest_size))
            return 1;

        if (idx->type == RTE_CRYPTO_SYM_XFORM_CIPHER && cap->cipher.algo == idx->algo.cipher
            && cryptodev_supports_param_value(cap->cipher.key_sizes, key_size))
            return 1;

        if (idx->type == RTE_CRYPTO_SYM_XFORM_AEAD && cap->aead.algo == idx->algo.aead
            && cryptodev_supports_param_value(cap->aead.key_sizes, key_size) && cryptodev_supports_param_value(cap->aead.digest_sizes, digest_size)
...
static void remove_unsupported_param_size(u32 **param_sizes, u32 param_size_min, u32 param_size_max, u32 increment)
{
    ...
    while (i < vec_len(*param_sizes)) {
        u32 found_param = 0;
        for (cap_param_size = param_size_min; cap_param_size <= param_size_max; cap_param_size += increment) {
        ...
        if (!found_param) /* no such param_size in cap so delete  this size in temp_cap params */
...
static u32 cryptodev_remove_unsupported_param_sizes(cryptodev_capability_t *temp_cap, const struct rte_cryptodev_capabilities *dev_caps)
{
    ...
    while (cap->op != RTE_CRYPTO_OP_TYPE_UNDEFINED) {
        if (cap->sym.xform_type == temp_cap->xform_type) switch (cap->sym.xform_type) {
                case RTE_CRYPTO_SYM_XFORM_CIPHER:
                    if (cap->sym.cipher.algo == temp_cap->cipher.algo) {
                        remove_unsupported_param_size(&temp_cap->cipher.key_sizes,
                                                      cap->sym.cipher.key_size.min,
                                                      cap->sym.cipher.key_size.max,
                                                      cap->sym.cipher.key_size.increment);
                        if (vec_len(temp_cap->cipher.key_sizes) > 0) cap_found = 1;
                    }
                    break;
                case RTE_CRYPTO_SYM_XFORM_AUTH:
                    if (cap->sym.auth.algo == temp_cap->auth.algo) {
                        remove_unsupported_param_size(&temp_cap->auth.digest_sizes,
                                                      cap->sym.auth.digest_size.min,
                                                      cap->sym.auth.digest_size.max,
                                                      cap->sym.auth.digest_size.increment);
                        if (vec_len(temp_cap->auth.digest_sizes) > 0) cap_found = 1;
                    }
                    break;
                case RTE_CRYPTO_SYM_XFORM_AEAD:
                    if (cap->sym.aead.algo == temp_cap->aead.algo) {
                        remove_unsupported_param_size(&temp_cap->aead.key_sizes,
                                                      cap->sym.aead.key_size.min,
                                                      cap->sym.aead.key_size.max,
                                                      cap->sym.aead.key_size.increment);
                        remove_unsupported_param_size(&temp_cap->aead.aad_sizes,
                                                      cap->sym.aead.aad_size.min,
                                                      cap->sym.aead.aad_size.max,
                                                      cap->sym.aead.aad_size.increment);
                        remove_unsupported_param_size(&temp_cap->aead.digest_sizes,
                                                      cap->sym.aead.digest_size.min,
                                                      cap->sym.aead.digest_size.max,
                                                      cap->sym.aead.digest_size.increment);
                        if (vec_len(temp_cap->aead.key_sizes) > 0 && vec_len(temp_cap->aead.aad_sizes) > 0
...
static void cryptodev_get_common_capabilities()
{
    ...
    if (vec_len(cmt->cryptodev_inst) == 0) return;
    dev_inst = vec_elt_at_index(cmt->cryptodev_inst, 0);
    ...
    while (cap->op != RTE_CRYPTO_OP_TYPE_UNDEFINED) {
        ...
        switch (cap->sym.xform_type) {
            case RTE_CRYPTO_SYM_XFORM_CIPHER:
                ...
                for (param = cap->sym.cipher.key_size.min; param <= cap->sym.cipher.key_size.max; param += cap->sym.cipher.key_size.increment) {
                    vec_add1(tmp_cap.cipher.key_sizes, param);
                    if (cap->sym.cipher.key_size.increment == 0) break;
                }
                break;
            case RTE_CRYPTO_SYM_XFORM_AUTH:
                tmp_cap.auth.algo         = cap->sym.auth.algo;
                tmp_cap.auth.digest_sizes = 0;
                for (param = cap->sym.auth.digest_size.min; param <= cap->sym.auth.digest_size.max; param += cap->sym.auth.digest_size.increment) {
                    vec_add1(tmp_cap.auth.digest_sizes, param);
                    if (cap->sym.auth.digest_size.increment == 0) break;
                }
                break;
            case RTE_CRYPTO_SYM_XFORM_AEAD:
                ...
                for (param = cap->sym.aead.key_size.min; param <= cap->sym.aead.key_size.max; param += cap->sym.aead.key_size.increment) {
                    vec_add1(tmp_cap.aead.key_sizes, param);
                    if (cap->sym.aead.key_size.increment == 0) break;
                }
                for (param = cap->sym.aead.aad_size.min; param <= cap->sym.aead.aad_size.max; param += cap->sym.aead.aad_size.increment) {
                    vec_add1(tmp_cap.aead.aad_sizes, param);
                    if (cap->sym.aead.aad_size.increment == 0) break;
                }
                for (param = cap->sym.aead.digest_size.min; param <= cap->sym.aead.digest_size.max; param += cap->sym.aead.digest_size.increment) {
                    vec_add1(tmp_cap.aead.digest_sizes, param);
                    if (cap->sym.aead.digest_size.increment == 0) break;
                }
                break;
            default:
                break;
    ...
    while (cap_id < vec_len(cmt->supported_caps)) {
        ...
        vec_foreach (dev_inst, cmt->cryptodev_inst) {
            ...
            if (previous_dev_id != dev_id) {
                ...
                cap_is_supported = cryptodev_remove_unsupported_param_sizes(&cmt->supported_caps[cap_id], dev_caps);
                if (!cap_is_supported) {
...
static int cryptodev_probe(vlib_main_t *vm, u32 n_workers)
{
    ...
    if (n_queues < n_workers) return -1;

    for (i = 0; i < rte_cryptodev_count(); i++)
        cryptodev_configure(vm, i);

    if (vec_len(cmt->cryptodev_inst) == 0) return -1;
    ...
    if (vec_len(cmt->cryptodev_inst) < n_workers) return -1;

    return 0;
}

#if RTE_VERSION < RTE_VERSION_NUM(22, 11, 0, 0)
static void is_drv_unique(u32 driver_id, u32 **unique_drivers)
{
    u32 *unique_elt;
    u8   found = 0;
    ...
    if (!found) vec_add1(*unique_drivers, driver_id);
}
#endif
...
clib_error_t *dpdk_cryptodev_init(vlib_main_t *vm)
{
    ...
    if (cryptodev_probe(vm, n_workers) < 0) return 0;
    ...
    vec_foreach (dev_inst, cmt->cryptodev_inst) {
        ...
        u32 sess_sz  = rte_cryptodev_sym_get_private_session_size(dev_inst->dev_id);
    ...
    vec_validate_aligned(cmt->per_thread_data, tm->n_vlib_mains - 1, CLIB_CACHE_LINE_BYTES);
