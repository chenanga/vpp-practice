...
#define foreach_dpdk_tx_func_error _(PKT_DROP, "Tx packet drops (dpdk tx failure)")
...
static clib_error_t *dpdk_add_del_mac_address(vnet_hw_interface_t *hi, const u8 *address, u8 is_add)
{
    ...
    if (is_add)
        error = rte_eth_dev_mac_addr_add(xd->port_id, (struct rte_ether_addr *) address, 0);
    else
        error = rte_eth_dev_mac_addr_remove(xd->port_id, (struct rte_ether_addr *) address);
...
static void dpdk_tx_trace_buffer(dpdk_main_t *dm, vlib_node_runtime_t *node, dpdk_device_t *xd, u16 queue_id, vlib_buffer_t *buffer)
{
    ...
    clib_memcpy_fast(&t0->buffer, buffer, sizeof(buffer[0]) - sizeof(buffer->pre_data));
    clib_memcpy_fast(t0->buffer.pre_data, buffer->data + buffer->current_data, sizeof(t0->buffer.pre_data));
    clib_memcpy_fast(&t0->data, mb->buf_addr + mb->data_off, sizeof(t0->data));
}
...
static_always_inline void dpdk_validate_rte_mbuf(vlib_main_t *vm, vlib_buffer_t *b, int maybe_multiseg)
{
    ...
    mb->pkt_len       = maybe_multiseg ? vlib_buffer_length_in_chain(vm, b) : b->current_length;
 ...
 */
static_always_inline u32 tx_burst_vector_internal(vlib_main_t *vm, dpdk_device_t *xd, struct rte_mbuf **mb, u32 n_left, int queue_id, u8 is_shared)
{
    dpdk_tx_queue_t *txq;
    u32              n_retry;
    int              n_sent = 0;
...
static_always_inline void dpdk_buffer_tx_offload(dpdk_device_t *xd, vlib_buffer_t *b, struct rte_mbuf *mb)
{
    ...
    if (PREDICT_TRUE(((b->flags & VNET_BUFFER_F_OFFLOAD) | tso) == 0)) return;
    ...
    if (vxlan_tunnel) {
        ol_flags |= outer_ip_cksum ? RTE_MBUF_F_TX_OUTER_IPV4 | RTE_MBUF_F_TX_OUTER_IP_CKSUM : RTE_MBUF_F_TX_OUTER_IPV6;
        ol_flags |= RTE_MBUF_F_TX_TUNNEL_VXLAN;
        mb->l2_len       = vnet_buffer(b)->l3_hdr_offset - vnet_buffer2(b)->outer_l4_hdr_offset;
        mb->l3_len       = vnet_buffer(b)->l4_hdr_offset - vnet_buffer(b)->l3_hdr_offset;
        mb->outer_l2_len = vnet_buffer2(b)->outer_l3_hdr_offset - b->current_data;
        mb->outer_l3_len = vnet_buffer2(b)->outer_l4_hdr_offset - vnet_buffer2(b)->outer_l3_hdr_offset;
    ...
    if (tso) {
        ...
        max_pkt_len = outer_hdr_len + mb->l2_len + mb->l3_len + mb->l4_len + mb->tso_segsz;
        if (mb->tso_segsz != 0 && mb->pkt_len > max_pkt_len) ol_flags |= (tcp_cksum ? RTE_MBUF_F_TX_TCP_SEG : RTE_MBUF_F_TX_UDP_SEG);
    }
    ...
    /* we are trying to help compiler here by using local ol_flags with known
       state of all flags */
    if (xd->flags & DPDK_DEVICE_FLAG_INTEL_PHDR_CKSUM) rte_net_intel_cksum_flags_prepare(mb, ol_flags);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *f)
{
    ...
    vlib_get_buffers_with_offset(vm, vlib_frame_vector_args(f), (void **) ptd->mbufs, n_packets, -(i32) sizeof(struct rte_mbuf));
    ...
    while (n_left >= 8) {
        ...
        if (PREDICT_FALSE((xd->flags & DPDK_DEVICE_FLAG_TX_OFFLOAD) && (or_flags & VNET_BUFFER_F_OFFLOAD))) {
        ...
        if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE)) {
            if (b[0]->flags & VLIB_BUFFER_IS_TRACED) dpdk_tx_trace_buffer(dm, node, xd, queue_id, b[0]);
            if (b[1]->flags & VLIB_BUFFER_IS_TRACED) dpdk_tx_trace_buffer(dm, node, xd, queue_id, b[1]);
            if (b[2]->flags & VLIB_BUFFER_IS_TRACED) dpdk_tx_trace_buffer(dm, node, xd, queue_id, b[2]);
            if (b[3]->flags & VLIB_BUFFER_IS_TRACED) dpdk_tx_trace_buffer(dm, node, xd, queue_id, b[3]);
        }
    ...
    n_left              = tx_burst_vector_internal(vm, xd, ptd->mbufs, n_packets, queue_id, is_shared);
...
static clib_error_t *dpdk_interface_admin_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    ...
    if (xd->flags & DPDK_DEVICE_FLAG_PMD_INIT_FAIL) return clib_error_return(0, "Interface not initialized");

    if (is_up) {
        if ((xd->flags & DPDK_DEVICE_FLAG_ADMIN_UP) == 0) {
            dpdk_device_start(xd);
            if (vec_len(xd->errors)) return clib_error_create("Interface start failed");
        ...
        if ((xd->flags & DPDK_DEVICE_FLAG_ADMIN_UP) != 0) dpdk_device_stop(xd);
        xd->flags &= ~DPDK_DEVICE_FLAG_ADMIN_UP;
    }
...
static void dpdk_set_interface_next_node(vnet_main_t *vnm, u32 hw_if_index, u32 node_index)
{
    ...
    xd->per_interface_next_index = vlib_node_add_next(vlib_get_main(), dpdk_input_node.index, node_index);
}
...
static clib_error_t *dpdk_subif_add_del_function(vnet_main_t *vnm, u32 hw_if_index, struct vnet_sw_interface_t *st, int is_add)
{
    ...
    if (xd->driver->program_vlans == 0) goto done;

    if (t->sub.eth.flags.no_tags == 1) goto done;
    ...
    if ((r = rte_eth_dev_set_vlan_offload(xd->port_id, vlan_offload))) {
        xd->num_subifs = prev_subifs;
        err            = clib_error_return(0, "rte_eth_dev_set_vlan_offload[%d]: err %d", xd->port_id, r);
        goto done;
    }

    if ((r = rte_eth_dev_vlan_filter(xd->port_id, t->sub.eth.outer_vlan_id, is_add))) {
        xd->num_subifs = prev_subifs;
        err            = clib_error_return(0, "rte_eth_dev_vlan_filter[%d]: err %d", xd->port_id, r);
        goto done;
    }
...
static clib_error_t *dpdk_interface_set_rss_queues(struct vnet_main_t *vnm, struct vnet_hw_interface_t *hi, clib_bitmap_t *bitmap)
{
    ...
    /* update reta table */
    reta_conf = (struct rte_eth_rss_reta_entry64 *) clib_mem_alloc(dev_info.reta_size / RTE_ETH_RETA_GROUP_SIZE * sizeof(*reta_conf));
    if (reta_conf == NULL) {
        err = clib_error_return(0, "clib_mem_alloc failed");
        goto done;
    }

    clib_memset(reta_conf, 0, dev_info.reta_size / RTE_ETH_RETA_GROUP_SIZE * sizeof(*reta_conf));
    ...
    ret = rte_eth_dev_rss_reta_update(xd->port_id, reta_conf, dev_info.reta_size);
    if (ret) {
    ...
    if (reta) clib_mem_free(reta);
    if (reta_conf) clib_mem_free(reta_conf);
...
static clib_error_t *dpdk_interface_rx_mode_change(vnet_main_t *vnm, u32 hw_if_index, u32 qid, vnet_hw_if_rx_mode mode)
{
    ...
    clib_file_t         *f;
    int                  rv = 0;
    if (!(xd->flags & DPDK_DEVICE_FLAG_INT_SUPPORTED)) return clib_error_return(0, "unsupported op (is the interface up?)", rv);
    if (mode == VNET_HW_IF_RX_MODE_POLLING && !(xd->flags & DPDK_DEVICE_FLAG_INT_UNMASKABLE))
    ...
    if (rv) return clib_error_return(0, "dpdk_interface_rx_mode_change err %d", rv);
    return 0;
}
