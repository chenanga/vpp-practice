...
static_always_inline void dpdk_device_flag_set(dpdk_device_t *xd, __typeof__(xd->flags) flag, int val)
{
...
static inline void dpdk_get_xstats(dpdk_device_t *xd, u32 thread_index)
{
    int ret;
    int i;
    int len;
    if (!(xd->flags & DPDK_DEVICE_FLAG_ADMIN_UP)) return;
    if (xd->driver == 0) return;
    ...
    if (len == vec_len(xd->driver->xstats_counters)) {
        vec_foreach_index (i, xd->xstats) {
            vlib_set_simple_counter(&xd->driver->xstats_counters[i], thread_index, xd->sw_if_index, xd->xstats[i].value);
        }
    }
    else {
        dpdk_log_warn("rte_eth_xstats_get vector size mismatch (%d/%d", len, vec_len(xd->driver->xstats_counters));
    }
}
...
#define DPDK_UPDATE_COUNTER(vnm, tidx, xd, stat, cnt)                                                                                            \
    do {                                                                                                                                         \
        ...
        if (PREDICT_FALSE(_v != _lv)) {                                                                                                          \
            if (PREDICT_FALSE(_v < _lv))                                                                                                         \
                dpdk_log_warn("%v: %s counter decreased (before %lu after %lu)", xd->name, #stat, _lv, _v);                                      \
            else                                                                                                                                 \
                vlib_increment_simple_counter(vec_elt_at_index((vnm)->interface_main.sw_if_counters, cnt), (tidx), (xd)->sw_if_index, _v - _lv); \
        }                                                                                                                                        \
    } while (0)

static inline void dpdk_update_counters(dpdk_device_t *xd, f64 now)
{
    ...
    DPDK_UPDATE_COUNTER(vnm, thread_index, xd, rx_nombuf, VNET_INTERFACE_COUNTER_RX_NO_BUF);
    DPDK_UPDATE_COUNTER(vnm, thread_index, xd, imissed, VNET_INTERFACE_COUNTER_RX_MISS);
    DPDK_UPDATE_COUNTER(vnm, thread_index, xd, ierrors, VNET_INTERFACE_COUNTER_RX_ERROR);
