...
#define FLOW_IS_IPV4_CLASS(f)                                                                                                              \
    ((f->type == VNET_FLOW_TYPE_IP4) || (f->type == VNET_FLOW_TYPE_IP4_N_TUPLE) || (f->type == VNET_FLOW_TYPE_IP4_N_TUPLE_TAGGED)          \
     ...
     || (f->type == VNET_FLOW_TYPE_IP4_IP4) || (f->type == VNET_FLOW_TYPE_IP4_IP6) || (f->type == VNET_FLOW_TYPE_IP4_IP4_N_TUPLE)          \
...
#define FLOW_IS_IPV6_CLASS(f)                                                                                                     \
    ((f->type == VNET_FLOW_TYPE_IP6) || (f->type == VNET_FLOW_TYPE_IP6_N_TUPLE) || (f->type == VNET_FLOW_TYPE_IP6_N_TUPLE_TAGGED) \
     || (f->type == VNET_FLOW_TYPE_IP6_VXLAN) || (f->type == VNET_FLOW_TYPE_IP6_IP4) || (f->type == VNET_FLOW_TYPE_IP6_IP6)       \
     || (f->type == VNET_FLOW_TYPE_IP6_IP4_N_TUPLE) || (f->type == VNET_FLOW_TYPE_IP6_IP6_N_TUPLE))

/* check if flow is VLAN sensitive */
#define FLOW_HAS_VLAN_TAG(f) ((f->type == VNET_FLOW_TYPE_IP4_N_TUPLE_TAGGED) || (f->type == VNET_FLOW_TYPE_IP6_N_TUPLE_TAGGED))
...
#define FLOW_HAS_INNER_N_TUPLE(f)                                                                                                              \
    ((f->type == VNET_FLOW_TYPE_IP4_IP4_N_TUPLE) || (f->type == VNET_FLOW_TYPE_IP4_IP6_N_TUPLE) || (f->type == VNET_FLOW_TYPE_IP6_IP4_N_TUPLE) \
     || (f->type == VNET_FLOW_TYPE_IP6_IP6_N_TUPLE))

/* constant structs */
static const struct rte_flow_attr ingress = {.ingress = 1};

static inline bool mac_address_is_all_zero(const u8 addr[6])
{
    int i = 0;
...
static inline void dpdk_flow_convert_rss_types(u64 type, u64 *dpdk_rss_type)
{
...
#undef _
#define _(n, f, s) \
    if (n != -1 && BIT_IS_SET(type, n)) *dpdk_rss_type |= f;
...
static inline void dpdk_flow_convert_rss_queues(u32 queue_index, u32 queue_num, struct rte_flow_action_rss *rss)
{
...
static inline enum rte_eth_hash_function dpdk_flow_convert_rss_func(vnet_rss_function_t func)
{
...
static int dpdk_flow_add(dpdk_device_t *xd, vnet_flow_t *f, dpdk_flow_entry_t *fe)
{
    ...
    if (f->actions & (~xd->supported_flow_actions)) return VNET_FLOW_ERROR_NOT_SUPPORTED;
    ...
    if (FLOW_IS_ETHERNET_CLASS(f)) goto pattern_end;
    ...
    if (flow_class == FLOW_IPV4_CLASS) {
        ...
        if ((!ip4_ptr->src_addr.mask.as_u32) && (!ip4_ptr->dst_addr.mask.as_u32) && (!ip4_ptr->protocol.mask)) {
            item->spec = NULL;
            item->mask = NULL;
        }
        else {
        ...
        if ((ip6_ptr->src_addr.mask.as_u64[0] == 0) && (ip6_ptr->src_addr.mask.as_u64[1] == 0) && (ip6_ptr->dst_addr.mask.as_u64[0] == 0)
            && (ip6_ptr->dst_addr.mask.as_u64[1] == 0) && (!ip6_ptr->protocol.mask)) {
            item->spec = NULL;
            item->mask = NULL;
        }
        else {
            clib_memcpy(ip6[0].hdr.src_addr, &ip6_ptr->src_addr.addr, ARRAY_LEN(ip6_ptr->src_addr.addr.as_u8));
            clib_memcpy(ip6[1].hdr.src_addr, &ip6_ptr->src_addr.mask, ARRAY_LEN(ip6_ptr->src_addr.mask.as_u8));
            clib_memcpy(ip6[0].hdr.dst_addr, &ip6_ptr->dst_addr.addr, ARRAY_LEN(ip6_ptr->dst_addr.addr.as_u8));
            clib_memcpy(ip6[1].hdr.dst_addr, &ip6_ptr->dst_addr.mask, ARRAY_LEN(ip6_ptr->dst_addr.mask.as_u8));
    ...
    if (FLOW_IS_L3_TYPE(f)) goto pattern_end;
...
#define fill_inner_ip6_with_outer_ipv(OUTER_IP_VER)                                                                                    \
    if (f->type == VNET_FLOW_TYPE_IP##OUTER_IP_VER##_IP6 || f->type == VNET_FLOW_TYPE_IP##OUTER_IP_VER##_IP6_N_TUPLE) {                \
        vnet_flow_ip##OUTER_IP_VER##_ip6_t *ptr = &f->ip##OUTER_IP_VER##_ip6;                                                          \
        if ((ptr->in_src_addr.mask.as_u64[0] == 0) && (ptr->in_src_addr.mask.as_u64[1] == 0) && (ptr->in_dst_addr.mask.as_u64[0] == 0) \
            && (ptr->in_dst_addr.mask.as_u64[1] == 0) && (!ptr->in_protocol.mask)) {                                                   \
            item->spec = NULL;                                                                                                         \
            item->mask = NULL;                                                                                                         \
        }                                                                                                                              \
        else {                                                                                                                         \
            clib_memcpy(in_ip6[0].hdr.src_addr, &ptr->in_src_addr.addr, ARRAY_LEN(ptr->in_src_addr.addr.as_u8));                       \
            clib_memcpy(in_ip6[1].hdr.src_addr, &ptr->in_src_addr.mask, ARRAY_LEN(ptr->in_src_addr.mask.as_u8));                       \
            clib_memcpy(in_ip6[0].hdr.dst_addr, &ptr->in_dst_addr.addr, ARRAY_LEN(ptr->in_dst_addr.addr.as_u8));                       \
            clib_memcpy(in_ip6[1].hdr.dst_addr, &ptr->in_dst_addr.mask, ARRAY_LEN(ptr->in_dst_addr.mask.as_u8));                       \
            item->spec = in_ip6;                                                                                                       \
            item->mask = in_ip6 + 1;                                                                                                   \
        }                                                                                                                              \
    }
...
#define fill_inner_ip4_with_outer_ipv(OUTER_IP_VER)                                                                     \
    if (f->type == VNET_FLOW_TYPE_IP##OUTER_IP_VER##_IP4 || f->type == VNET_FLOW_TYPE_IP##OUTER_IP_VER##_IP4_N_TUPLE) { \
...
#define fill_inner_n_tuple(OUTER_IP_VER, INNER_IP_VER)                                                                      \
    if (f->type == VNET_FLOW_TYPE_IP##OUTER_IP_VER##_IP##INNER_IP_VER##_N_TUPLE) {                                          \
        vnet_flow_ip##OUTER_IP_VER##_ip##INNER_IP_VER##_n_tuple_t *ptr = &f->ip##OUTER_IP_VER##_ip##INNER_IP_VER##_n_tuple; \
        ...
        fill_inner_n_tuple(6, 4) fill_inner_n_tuple(4, 4) fill_inner_n_tuple(6, 6) fill_inner_n_tuple(4, 6)
...
pattern_end:
    ...
    if (f->actions & VNET_FLOW_ACTION_RSS) {
        ...
        if (f->queue_num) /* convert rss queues to array */
    ...
    rv = rte_flow_validate(xd->device_index, &ingress, items, actions, &xd->last_flow_error);
    ...
    fe->handle = rte_flow_create(xd->device_index, &ingress, items, actions, &xd->last_flow_error);

    if (!fe->handle) rv = VNET_FLOW_ERROR_NOT_SUPPORTED;
...
int dpdk_flow_ops_fn(vnet_main_t *vnm, vnet_flow_dev_op_t op, u32 dev_instance, u32 flow_index, uword *private_data)
{
    ...
    if (op == VNET_FLOW_DEV_OP_DEL_FLOW) {
        ...
        if ((rv = rte_flow_destroy(xd->device_index, fe->handle, &xd->last_flow_error))) return VNET_FLOW_ERROR_INTERNAL;
    ...
    if (op != VNET_FLOW_DEV_OP_ADD_FLOW) return VNET_FLOW_ERROR_NOT_SUPPORTED;
    ...
    if (flow->actions & (VNET_FLOW_ACTION_MARK | VNET_FLOW_ACTION_REDIRECT_TO_NODE | VNET_FLOW_ACTION_BUFFER_ADVANCE)) {
        /* reserve slot 0 */
        if (xd->flow_lookup_entries == 0) pool_get_aligned(xd->flow_lookup_entries, fle, CLIB_CACHE_LINE_BYTES);
        ...
        clib_memset(fle, -1, sizeof(*fle));
        if (flow->actions & VNET_FLOW_ACTION_MARK) fle->flow_id = flow->mark_flow_id;
        if (flow->actions & VNET_FLOW_ACTION_REDIRECT_TO_NODE) fle->next_index = flow->redirect_device_input_next_index;
        if (flow->actions & VNET_FLOW_ACTION_BUFFER_ADVANCE) fle->buffer_advance = flow->buffer_advance;
    }
    else
        fe->mark = 0;
    ...
    switch (flow->type) {
        ...
        case VNET_FLOW_TYPE_GENERIC:
            if ((rv = dpdk_flow_add(xd, flow, fe))) goto done;
            break;
        default:
            rv = VNET_FLOW_ERROR_NOT_SUPPORTED;
            goto done;
...
u8 *format_dpdk_flow(u8 *s, va_list *args)
{
    ...
    if (flow_index == ~0) {
        s = format(s, "%-25s: %U\n", "supported flow actions", format_flow_actions, xd->supported_flow_actions);
        s = format(s, "%-25s: %d\n", "last DPDK error type", xd->last_flow_error.type);
        s = format(s, "%-25s: %s\n", "last DPDK error message", xd->last_flow_error.message ? xd->last_flow_error.message : "n/a");
        return s;
    }

    if (private_data >= vec_len(xd->flow_entries)) return format(s, "unknown flow");
