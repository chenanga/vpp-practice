...
STATIC_ASSERT((u64) (RTE_MBUF_F_RX_IP_CKSUM_BAD | RTE_MBUF_F_RX_L4_CKSUM_BAD | RTE_MBUF_F_RX_FDIR | RTE_MBUF_F_RX_LRO) < (1ULL << 32),
              "dpdk flags not in lower word, fix needed");

STATIC_ASSERT(RTE_MBUF_F_RX_L4_CKSUM_BAD == (1ULL << 3), "bit number of RTE_MBUF_F_RX_L4_CKSUM_BAD is no longer 3!");
...
static_always_inline uword dpdk_process_rx_burst(vlib_main_t *vm, dpdk_per_thread_data_t *ptd, uword n_rx_packets, int maybe_multiseg, u32 *or_flagsp)
{
    ...
    while (n_left) {
        ...
        if (maybe_multiseg) n_bytes += dpdk_process_subseq_segs(vm, b[0], mb[0], &bt);
...
static_always_inline void dpdk_process_flow_offload(dpdk_device_t *xd, dpdk_per_thread_data_t *ptd, uword n_rx_packets)
{
    ...
    for (n = 0; n < n_rx_packets; n++) {
        if ((ptd->flags[n] & RTE_MBUF_F_RX_FDIR_ID) == 0) continue;

        fle = pool_elt_at_index(xd->flow_lookup_entries, ptd->mbufs[n]->hash.fdir.hi);

        if (fle->next_index != (u16) ~0) ptd->next[n] = fle->next_index;
...
dpdk_device_input(vlib_main_t *vm, dpdk_main_t *dm, dpdk_device_t *xd, vlib_node_runtime_t *node, u32 thread_index, u16 queue_id)
{
    ...
    if ((xd->flags & DPDK_DEVICE_FLAG_ADMIN_UP) == 0) return 0;
    ...
    while (n_rx_packets < DPDK_RX_BURST_SZ) {
        ...
        n = rte_eth_rx_burst(xd->port_id, queue_id, ptd->mbufs + n_rx_packets, n_to_rx);
        n_rx_packets += n;

        if (n < n_to_rx) break;
    }

    if (n_rx_packets == 0) return 0;
    ...
    if (PREDICT_FALSE(xd->per_interface_next_index != ~0)) next_index = xd->per_interface_next_index;

    /* as all packets belong to the same interface feature arc lookup
       can be don once and result stored in the buffer template */
    if (PREDICT_FALSE(vnet_device_input_have_features(xd->sw_if_index))) vnet_feature_start_device_input(xd->sw_if_index, &next_index, bt);
    ...
    if (PREDICT_FALSE((or_flags & RTE_MBUF_F_RX_LRO))) dpdk_process_lro_offload(xd, ptd, n_rx_packets);

    if (PREDICT_FALSE((or_flags & RTE_MBUF_F_RX_L4_CKSUM_BAD) && (xd->buffer_flags & VNET_BUFFER_F_L4_CHECKSUM_CORRECT))) {
        for (n = 0; n < n_rx_packets; n++) {
             ...
             */
            b0 = vlib_buffer_from_rte_mbuf(ptd->mbufs[n]);
            b0->flags ^= (ptd->flags[n] & RTE_MBUF_F_RX_L4_CKSUM_BAD) << (VNET_BUFFER_F_LOG2_L4_CHECKSUM_CORRECT - 3);
        }
    }
    ...
    if (PREDICT_FALSE(or_flags & RTE_MBUF_F_RX_FDIR)) {
        ...
        /* flow offload - process if rx flow offload enabled and at least one
           packet is marked */
        if (PREDICT_FALSE((xd->flags & DPDK_DEVICE_FLAG_RX_FLOW_OFFLOAD) && (or_flags & RTE_MBUF_F_RX_FDIR)))
            dpdk_process_flow_offload(xd, ptd, n_rx_packets);

        /* enqueue buffers to the next node */
        vlib_get_buffer_indices_with_offset(vm, (void **) ptd->mbufs, ptd->buffers, n_rx_packets, sizeof(struct rte_mbuf));

        vlib_buffer_enqueue_to_next(vm, node, ptd->buffers, ptd->next, n_rx_packets);
    }
    else {
        u32 *to_next, n_left_to_next;

        vlib_get_new_next_frame(vm, node, next_index, to_next, n_left_to_next);
        vlib_get_buffer_indices_with_offset(vm, (void **) ptd->mbufs, to_next, n_rx_packets, sizeof(struct rte_mbuf));
        ...
        if (PREDICT_TRUE(next_index == VNET_DEVICE_INPUT_NEXT_ETHERNET_INPUT)) {
            ...
            if (xd->flags & DPDK_DEVICE_FLAG_RX_IP4_CKSUM && (or_flags & RTE_MBUF_F_RX_IP_CKSUM_BAD) == 0) f->flags |= ETH_INPUT_FRAME_F_IP4_CKSUM_OK;
    ...
    if (PREDICT_FALSE((n_trace = vlib_get_trace_count(vm, node)))) {
        if (single_next) vlib_get_buffer_indices_with_offset(vm, (void **) ptd->mbufs, ptd->buffers, n_rx_packets, sizeof(struct rte_mbuf));
        ...
        while (n_trace && n_left) {
            ...
            if (single_next == 0) next_index = next[0];

            if (PREDICT_TRUE(vlib_trace_buffer(vm, node, next_index, b0, /* follow_chain */ 0))) {
                ...
                clib_memcpy_fast(&t0->data, mb[0]->buf_addr + mb[0]->data_off, sizeof t0->data);
                n_trace--;
            }
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *f)
{
    ...
    for (int i = 0; i < vec_len(pv); i++) {
        ...
        n_rx_packets += dpdk_device_input(vm, dm, xd, node, thread_index, pv[i].queue_id);
    }
    return n_rx_packets;
}
