...
#define foreach_dpdk_pkt_offload_flag foreach_dpdk_pkt_rx_offload_flag foreach_dpdk_pkt_tx_offload_flag
...
#define _(a, b, c) \
    if (xd->flags & (1 << a)) t = format(t, "%s%s", t ? " " : "", c);
...
static u8 *format_dpdk_link_status(u8 *s, va_list *args)
{
    ...
    if (l->link_status) {
        ...
        s = format(s, "%s duplex ", (l->link_duplex == RTE_ETH_LINK_FULL_DUPLEX) ? "full" : "half");
        s = format(s, "max-frame-size %d %s\n", hi->max_frame_size, promisc ? " promisc" : "");
    }
    else
        s = format(s, "\n");
...
#define _(n, v, str)                                                                      \
    if (bitmap & v) {                                                                     \
        if (format_get_indent(s) > 72) s = format(s, "\n%U", format_white_space, indent); \
        s = format(s, "%s ", str);                                                        \
    }
...
u8 *format_dpdk_rss_hf_name(u8 *s, va_list *args)
{
    ...
    if (!bitmap) return format(s, "none");
...
static u8 *format_dpdk_device_module_info(u8 *s, va_list *args)
{
    ...
    if (rte_eth_dev_get_module_info(xd->port_id, &mi) != 0) return format(s, "unknown");

    ei.length = mi.eeprom_len;
    ei.data   = clib_mem_alloc(mi.eeprom_len);

    if (rte_eth_dev_get_module_eeprom(xd->port_id, &ei) == 0) {
        s = format(s, "%U", format_sfp_eeprom, ei.data + (mi.type == RTE_ETH_MODULE_SFF_8436 ? 0x80 : 0));
    }
    else
...
u8 *format_dpdk_burst_fn(u8 *s, va_list *args)
{
    dpdk_device_t    *xd  = va_arg(*args, dpdk_device_t *);
...
#define rte_eth_fp_ops rte_eth_devices
    ...
    p = (dir == VLIB_TX) ? rte_eth_fp_ops[xd->port_id].tx_pkt_burst : rte_eth_fp_ops[xd->port_id].rx_pkt_burst;
...
{
    struct rte_eth_switch_info *si = va_arg(*args, struct rte_eth_switch_info *);

    if (si->name) s = format(s, "name %s ", si->name);
...
u8 *format_dpdk_rte_device(u8 *s, va_list *args)
{
    struct rte_device *d = va_arg(*args, struct rte_device *);

    if (!d) return format(s, "not available");

#if RTE_VERSION >= RTE_VERSION_NUM(22, 11, 0, 0)
    s = format(s, "name: %s, numa: %d", rte_dev_name(d), rte_dev_numa_node(d));

    if (rte_dev_driver(d)) s = format(s, ", driver: %s", rte_driver_name(rte_dev_driver(d)));

    if (rte_dev_bus(d)) s = format(s, ", bus: %s", rte_bus_name(rte_dev_bus(d)));
#else
    s = format(s, "name: %s, numa: %d", d->name, d->numa_node);

    if (d->driver) s = format(s, ", driver: %s", d->driver->name);

    if (d->bus) s = format(s, ", bus: %s", d->bus->name);
#endif
...
u8 *format_dpdk_device(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U\n%Ucarrier %U", format_dpdk_device_type, dev_instance, format_white_space, indent + 2, format_dpdk_link_status, xd);
    s = format(s, "%Uflags: %U\n", format_white_space, indent + 2, format_dpdk_device_flags, xd);
#if RTE_VERSION >= RTE_VERSION_NUM(22, 11, 0, 0)
    if (rte_dev_devargs(di.device) && rte_dev_devargs(di.device)->args)
        s = format(s, "%UDevargs: %s\n", format_white_space, indent + 2, rte_dev_devargs(di.device)->args);
#else
    if (di.device->devargs && di.device->devargs->args) s = format(s, "%UDevargs: %s\n", format_white_space, indent + 2, di.device->devargs->args);
#endif
    s = format(s,
               ...
               di.max_rx_queues,
               xd->conf.n_rx_desc,
               di.rx_desc_lim.nb_min,
               di.rx_desc_lim.nb_max,
    ...
    s = format(s,
               ...
               di.max_tx_queues,
               xd->conf.n_tx_desc,
               di.tx_desc_lim.nb_min,
               di.tx_desc_lim.nb_max,
               di.tx_desc_lim.nb_align);

    rss_conf.rss_key = 0;
    rss_conf.rss_hf  = 0;
    retval           = rte_eth_dev_rss_hash_conf_get(xd->port_id, &rss_conf);
    if (retval < 0) clib_warning("rte_eth_dev_rss_hash_conf_get returned %d", retval);
    ...
    if (di.switch_info.domain_id != RTE_ETH_DEV_SWITCH_DOMAIN_ID_INVALID) {
        s = format(s, "%Uswitch info: %U\n", format_white_space, indent + 2, format_switch_info, &di.switch_info);
    }

    if (1 < verbose) {
        s = format(s, "%Umodule: %U\n", format_white_space, indent + 2, format_dpdk_device_module_info, xd);
    }
    ...
    s        = format(s, "%Urx offload avail:  %U\n", format_white_space, indent + 2, format_dpdk_rx_offload_caps, di.rx_offload_capa);
    s        = format(s, "%Urx offload active: %U\n", format_white_space, indent + 2, format_dpdk_rx_offload_caps, xd->enabled_rx_off);
    s        = format(s, "%Utx offload avail:  %U\n", format_white_space, indent + 2, format_dpdk_tx_offload_caps, di.tx_offload_capa);
    s        = format(s, "%Utx offload active: %U\n", format_white_space, indent + 2, format_dpdk_tx_offload_caps, xd->enabled_tx_off);
    s        = format(s,
               "%Urss avail:         %U\n"
                      "%Urss active:        %U\n",
               format_white_space,
    ...
    s = format(s, "%Utx burst function: %U\n", format_white_space, indent + 2, format_dpdk_burst_fn, xd, VLIB_TX);
    ...
    s = format(s, "%Urx burst function: %U\n", format_white_space, indent + 2, format_dpdk_burst_fn, xd, VLIB_RX);

    /* $$$ MIB counters  */
    {
#define _(N, V)                                                                                                \
    if (xd->stats.V != 0) {                                                                                    \
        s = format(s, "\n%U%-40U%16Lu", format_white_space, indent + 2, format_c_identifier, #N, xd->stats.V); \
    }
...
#undef _
    ...
    if (ret >= 0 && ret <= len) {
        vec_foreach_index (i, xd->xstats) {
            ...
            if (verbose == 2 || (verbose && xstat->value)) {
                xs = format(xs, "\n%U%-38s%16Lu", format_white_space, indent + 4, xstat_names[i].name, xstat->value);
            }
        }
    ...
    if (xs) {
        s = format(s, "\n%Uextended stats:%v", format_white_space, indent + 2, xs);
        vec_free(xs);
    }

    if (vec_len(xd->errors)) {
        s = format(s, "%UErrors:\n  %U", format_white_space, indent, format_dpdk_device_errors, xd);
    }
...
u8 *format_dpdk_tx_trace(u8 *s, va_list *va)
{
    ...
    s = format(s, "%U tx queue %d", format_vnet_sw_if_index_name, vnm, xd->sw_if_index, t->queue_index);

    s = format(s, "\n%Ubuffer 0x%x: %U", format_white_space, indent, t->buffer_index, format_vnet_buffer_no_chain, &t->buffer);

    s = format(s, "\n%U%U", format_white_space, indent, format_dpdk_rte_mbuf, &t->mb, &t->data);

    s = format(s, "\n%U%U", format_white_space, indent, format_ethernet_header_with_length, t->buffer.pre_data, sizeof(t->buffer.pre_data));
...
u8 *format_dpdk_rx_trace(u8 *s, va_list *va)
{
    ...
    s = format(s, "%U rx queue %d", format_vnet_sw_if_index_name, vnm, xd->sw_if_index, t->queue_index);
    ...
    if (vm->trace_main.verbose) {
        s = format(s, "\n%UPacket Dump%s", format_white_space, indent + 2, t->mb.data_len > sizeof(t->data) ? " (truncated)" : "");
        s = format(s,
                   "\n%U%U",
                   format_white_space,
                   indent + 4,
    ...
    if (!f) f = format_hex_bytes;
    s = format(s, "\n%U%U", format_white_space, indent, f, t->buffer.pre_data, sizeof(t->buffer.pre_data));
...
static inline u8 *format_dpdk_pkt_types(u8 *s, va_list *va)
{
    ...
    if (!*pkt_types) return s;

    s = format(s, "Packet Types");

#define _(L, F, S)                                                                                                       \
    if ((*pkt_types & RTE_PTYPE_##L##_MASK) == RTE_PTYPE_##L##_##F) {                                                    \
        s = format(s, "\n%U%s (0x%04x) %s", format_white_space, indent, "RTE_PTYPE_" #L "_" #F, RTE_PTYPE_##L##_##F, S); \
    }
...
#define _(F, P, S)                                                                   \
    {                                                                                \
        rx_dynflag_offset = rte_mbuf_dynflag_lookup(RTE_MBUF_DYNFLAG_##F##_NAME, P); \
        if (rx_dynflag_offset >= 0) {                                                \
            rx_dynflag = (u64) 1 << rx_dynflag_offset;                               \
            if (*ol_flags & rx_dynflag) {                                            \
                s = format(s, "\n%U%s %s", format_white_space, indent, #F, S);       \
            }                                                                        \
        }                                                                            \
    }
...
u8 *format_dpdk_rte_mbuf_vlan(u8 *s, va_list *va)
{
    ...
    if (clib_net_to_host_u16(vlan_hdr->type) == ETHERNET_TYPE_DOT1AD) {
        s = format(s, "%U 802.1q vlan ", format_ethernet_vlan_tci, clib_net_to_host_u16(vlan_hdr->priority_cfi_and_id));
        vlan_hdr++;
    }

    s = format(s, "%U", format_ethernet_vlan_tci, clib_net_to_host_u16(vlan_hdr->priority_cfi_and_id));
...
u8 *format_dpdk_rte_mbuf(u8 *s, va_list *va)
{
    ...
    if (mb->ol_flags) s = format(s, "\n%U%U", format_white_space, indent, format_dpdk_pkt_offload_flags, &mb->ol_flags);

    if ((mb->ol_flags & RTE_MBUF_F_RX_VLAN) && ((mb->ol_flags & (RTE_MBUF_F_RX_VLAN_STRIPPED | RTE_MBUF_F_RX_QINQ_STRIPPED)) == 0)) {
        ethernet_vlan_header_tv_t *vlan_hdr = ((ethernet_vlan_header_tv_t *) &(eth_hdr->type));
        s                                   = format(s, " %U", format_dpdk_rte_mbuf_vlan, vlan_hdr);
    }

    if (mb->packet_type) s = format(s, "\n%U%U", format_white_space, indent, format_dpdk_pkt_types, &mb->packet_type);
...
clib_error_t *unformat_rss_fn(unformat_input_t *input, uword *rss_fn)
{
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (0)
            ;
#undef _
#define _(n, f, s) else if (unformat(input, s)) *rss_fn |= f;
