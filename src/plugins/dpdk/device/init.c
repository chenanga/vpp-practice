...
static clib_error_t *dpdk_set_max_frame_size(vnet_main_t *vnm, vnet_hw_interface_t *hi, u32 frame_size)
{
    ...
    if (rv < 0) {
        dpdk_log_err("[%u] rte_eth_dev_set_mtu failed (mtu %u, rv %d)", xd->port_id, mtu, rv);
        switch (rv) {
            case -ENOTSUP:
                return vnet_error(VNET_ERR_UNSUPPORTED, "dpdk driver doesn't support MTU change");
            case -EBUSY:
        ...
        dpdk_log_debug("[%u] max_frame_size set to %u by setting MTU to %u", xd->port_id, frame_size, mtu);
...
static int check_l3cache()
{
    ...
    while ((dp = readdir(dir_cache)) != NULL) {
        if (dp->d_type == DT_DIR) {
            ...
            if ((err = clib_sysfs_read((char *) p, "%d", &level_cache))) clib_error_free(err);
    ...
    if (dir_cache != NULL) closedir(dir_cache);

    return 0;
}

static dpdk_device_config_t *dpdk_find_startup_config(struct rte_eth_dev_info *di)
{
    dpdk_main_t           *dm = &dpdk_main;
    ...
    if ((vmbus_dev = dpdk_get_vmbus_device(di))) {
        ...
        if (unformat(&input_vmbus, "%U", unformat_vlib_vmbus_addr, &vmbus_addr))
            p = mhash_get(&dm->conf->device_config_index_by_vmbus_addr, &vmbus_addr);
        unformat_free(&input_vmbus);
    }
#endif /* __linux__ */

    if (p) return pool_elt_at_index(dm->conf->dev_confs, p[0]);
...
void dpdk_counters_xstats_init(dpdk_device_t *xd)
{
    ...
    if (len < 0) {
        dpdk_log_err("[%u] rte_eth_xstats_get_names failed: %d", xd->port_id, len);
        return;
    }
    /* Counters for this driver is already initialised */
    if (vec_len(dr->xstats_counters) == len) {
        vec_foreach_index (i, dr->xstats_counters) {
            vlib_validate_simple_counter(&dr->xstats_counters[i], xd->sw_if_index);
    ...
    if (ret >= 0 && ret <= len) {
        ...
        vec_foreach_index (i, xstats_names) {
            name = (char *) format(0, "/if/%s/%s%c", dr->drivers->name, xstats_names[i].name, 0);
            ...
            if (vlib_stats_find_entry_index(name) != STAT_SEGMENT_INDEX_INVALID) {
                vec_free(name);
                name = (char *) format(0, "/if/%s/%s_%d%c", dr->drivers->name, xstats_names[i].name, i, 0);
            }
...
static clib_error_t *dpdk_lib_init(dpdk_main_t *dm)
{
    ...
    vec_validate_aligned(dm->per_thread_data, tm->n_vlib_mains - 1, CLIB_CACHE_LINE_BYTES);
    ...
    if_num_fmt = dm->conf->interface_name_format_decimal ? "%d/%d/%d" : "%x/%x/%x";
    ...
    dm->default_port_conf.rss_hf           = RTE_ETH_RSS_IP | RTE_ETH_RSS_UDP | RTE_ETH_RSS_TCP;
    dm->default_port_conf.max_lro_pkt_size = DPDK_MAX_LRO_SIZE_DEFAULT;

    if ((clib_mem_get_default_hugepage_size() == 2 << 20) && check_l3cache() == 0)
    ...
    RTE_ETH_FOREACH_DEV (port_id) {
        ...
        if ((rv = rte_eth_dev_info_get(port_id, &di)) != 0) {
            dpdk_log_warn("[%u] failed to get device info. skipping device.", port_id);
            continue;
        }

        if (di.device == 0) {
            dpdk_log_warn("[%u] missing device info. Skipping '%s' device", port_id, di.driver_name);
            continue;
        }
        ...
        if (devconf->is_blacklisted) {
            dpdk_log_notice("[%d] Device %s blacklisted. Skipping...", port_id, di.driver_name);
            continue;
        }
        ...
        if ((dr = dpdk_driver_find(di.driver_name, &xd->if_desc))) {
            ...
            if (dr->use_intel_phdr_cksum) dpdk_device_flag_set(xd, DPDK_DEVICE_FLAG_INTEL_PHDR_CKSUM, 1);
            if (dr->int_unmaskable) dpdk_device_flag_set(xd, DPDK_DEVICE_FLAG_INT_UNMASKABLE, 1);
        }
        else
        ...
        if (devconf->name) {
            ...
            if (dr && dr->interface_name_prefix) {
                ...
                if (di.speed_capa & mask) dpdk_log_warn("[%u] unknown speed capability 0x%x reported", xd->port_id, di.speed_capa & mask);

                for (int i = 0; i < ARRAY_LEN(if_name_prefixes); i++)
                    if (if_name_prefixes[i].link_speed & di.speed_capa) {
                        xd->name = format(xd->name, "%s", if_name_prefixes[i].pfx);
                        break;
                    }
                if (xd->name == 0) xd->name = format(xd->name, "Ethernet");
            }
            ...
            if ((di.switch_info.domain_id != RTE_ETH_DEV_SWITCH_DOMAIN_ID_INVALID) && (di.switch_info.port_id != (uint16_t) -1))
        ...
        if (devconf->num_tx_queues > 0) {
            if (di.max_tx_queues < devconf->num_tx_queues)
                dpdk_log_warn(
                    "[%u] Configured number of TX queues (%u) is "
                    "bigger than maximum supported (%u)",
                    port_id,
                    devconf->num_tx_queues,
                    di.max_tx_queues);
        ...
        if (devconf->num_rx_queues > 1 && di.max_rx_queues >= devconf->num_rx_queues) {
            ...
            if (devconf->rss_fn) {
                ...
                if (unsupported_bits) dpdk_log_warn("Unsupported RSS hash functions: %U", format_dpdk_rss_hf_name, unsupported_bits);
            }
            xd->conf.rss_hf &= di.flow_type_rss_offloads;
            dpdk_log_debug("[%u] rss_hf: %U", port_id, format_dpdk_rss_hf_name, xd->conf.rss_hf);
        }
...
#define RTE_VLAN_HLEN 4
        ...
        if (q < (2 * xd->driver_frame_overhead) && q > 0 && di.max_mtu != UINT16_MAX) xd->driver_frame_overhead = q;
        dpdk_log_debug(
            "[%u] min_mtu: %u, max_mtu: %u, min_rx_bufsize: %u, "
        ...
        dpdk_log_debug("[%u] n_rx_queues: %u n_tx_queues: %u n_rx_desc: %u n_tx_desc: %u",
                       port_id,
                       xd->conf.n_rx_queues,
                       xd->conf.n_tx_queues,
                       xd->conf.n_rx_desc,
        ...
        vec_validate_aligned(xd->rx_queues, xd->conf.n_rx_queues - 1, CLIB_CACHE_LINE_BYTES);
        vec_validate_aligned(xd->tx_queues, xd->conf.n_tx_queues - 1, CLIB_CACHE_LINE_BYTES);
        ...
        if (numa_node == SOCKET_ID_ANY) /* numa_node is not set, default to 0 */
        ...
        if (devconf->tag) vnet_set_sw_interface_tag(vnm, devconf->tag, sw->sw_if_index);

        ethernet_set_flags(vnm, xd->hw_if_index, ETHERNET_INTERFACE_FLAG_DEFAULT_L3);
        ...
        if (devconf->workers) {
            ...
            clib_bitmap_foreach (j, devconf->workers) {
                ...
                rxq->queue_index     = vnet_hw_if_register_rx_queue(vnm, xd->hw_if_index, q++, vdm->first_worker_thread_index + j);
            }
        }
        else
            for (q = 0; q < xd->conf.n_rx_queues; q++) {
                dpdk_rx_queue_t *rxq = vec_elt_at_index(xd->rx_queues, q);
                rxq->queue_index     = vnet_hw_if_register_rx_queue(vnm, xd->hw_if_index, q, VNET_HW_IF_RXQ_THREAD_ANY);
            }

        for (q = 0; q < xd->conf.n_tx_queues; q++) {
            dpdk_tx_queue_t *txq = vec_elt_at_index(xd->tx_queues, q);
            txq->queue_index     = vnet_hw_if_register_tx_queue(vnm, xd->hw_if_index, q);
        }
        ...
        if (devconf->tso == DPDK_DEVICE_TSO_ON) {
            ...
            if (xd->conf.enable_tcp_udp_checksum == 0)
                dpdk_log_warn("[%u] TCP/UDP checksum offload must be enabled", xd->port_id);
        ...
        if (devconf->max_lro_pkt_size) xd->conf.max_lro_pkt_size = devconf->max_lro_pkt_size;

        if (devconf->disable_rxq_int) xd->conf.enable_rxq_int = 0;

        dpdk_device_setup(xd);

        /* rss queues should be configured after dpdk_device_setup() */
        if (devconf->rss_queues) {
            if (vnet_hw_interface_set_rss_queues(vnet_get_main(), hi, devconf->rss_queues)) dpdk_log_warn("[%u] Failed to set rss queues", port_id);
        }

        if (vec_len(xd->errors)) dpdk_log_err("[%u] setup failed Errors:\n  %U", port_id, format_dpdk_device_errors, xd);
...
static void dpdk_bind_devices_to_uio(dpdk_config_main_t *conf)
{
    ...
    vec_foreach (addr, addrs) {
         ...
         * on FreeBSD.
         */
        if (d->device_class != PCI_CLASS_NETWORK && d->device_class != PCI_CLASS_PROCESSOR_CO) continue;
#else
        if (d->device_class != PCI_CLASS_NETWORK_ETHERNET && d->device_class != PCI_CLASS_PROCESSOR_CO) continue;
#endif /* __FreeBSD__ */
        ...
        for (i = 0; i < vec_len(conf->blacklist_by_pci_vendor_and_device); i++) {
            ...
            if (d->vendor_id == vendor && d->device_id == device) {
                ...
                if (devconf == 0) {
                    ...
                    hash_set(conf->device_config_index_by_pci_addr, addr->as_u32, devconf - conf->dev_confs);
        ...
        /* virtio */
        if (d->vendor_id == 0x1af4 && (d->device_id == VIRTIO_PCI_LEGACY_DEVICEID_NET || d->device_id == VIRTIO_PCI_MODERN_DEVICEID_NET))
            ;
        /* vmxnet3 */
        ...
        /* all Intel QAT devices VFs */
                 ...
                 && (d->device_id == 0x0443 || d->device_id == 0x18a1 || d->device_id == 0x19e3 || d->device_id == 0x37c9 || d->device_id == 0x6f55
                     || d->device_id == 0x18ef || d->device_id == 0x4941 || d->device_id == 0x4943 || d->device_id == 0x4945))
            ;
        /* Cisco VIC */
        else if (d->vendor_id == 0x1137 && (d->device_id == 0x0043 || d->device_id == 0x0071))
            ;
        /* Chelsio T4/T5 */
        ...
        /* Mellanox BF2, BF3 */
        else if (d->vendor_id == 0x15b3 && (d->device_id == 0xa2d6 || d->device_id == 0xa2dc)) {
            continue;
        }
        /* Broadcom NetXtreme S, and E series only */
        else if (d->vendor_id == 0x14e4
                 && ((d->device_id >= 0x16c0 && d->device_id != 0x16c6 && d->device_id != 0x16c7 && d->device_id != 0x16dd && d->device_id != 0x16f7
                      && d->device_id != 0x16fd && d->device_id != 0x16fe && d->device_id != 0x170d && d->device_id != 0x170c
                      && d->device_id != 0x170e && d->device_id != 0x1712 && d->device_id != 0x1713)
                     || (d->device_id == 0x1604 || d->device_id == 0x1605 || d->device_id == 0x1614 || d->device_id == 0x1606
        ...
        error = vlib_pci_bind_to_uio(vm, addr, (char *) conf->uio_driver_name, conf->uio_bind_force);
...
static void dpdk_bind_vmbus_devices_to_uio(dpdk_config_main_t *conf)
{
    ...
    vec_foreach (addr, addrs) {
        dpdk_device_config_t *devconf = 0;
        if (num_whitelisted) {
            uword *p = mhash_get(&conf->device_config_index_by_vmbus_addr, addr);
            if (!p) {
                /* No devices blacklisted, but have whitelisted. blacklist all
                 * non-whitelisted */
                pool_get(conf->dev_confs, devconf);
                mhash_set(&conf->device_config_index_by_vmbus_addr, addr, devconf - conf->dev_confs, 0);
...
uword unformat_max_simd_bitwidth(unformat_input_t *input, va_list *va)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (!unformat(input, "%u", max_simd_bitwidth)) goto error;

        if (*max_simd_bitwidth != DPDK_MAX_SIMD_BITWIDTH_256 && *max_simd_bitwidth != DPDK_MAX_SIMD_BITWIDTH_512) goto error;
    }
    return 1;
error:
    return 0;
                                        ...
                                        u8                      is_default)
{
    ...
    if (is_default) {
        ...
        p = hash_get(conf->device_config_index_by_pci_addr, ((vlib_pci_addr_t *) (addr))->as_u32);

        if (!p) {
            pool_get(conf->dev_confs, devconf);
            hash_set(conf->device_config_index_by_pci_addr, ((vlib_pci_addr_t *) (addr))->as_u32, devconf - conf->dev_confs);
        }
        else
            return clib_error_return(0, "duplicate configuration for PCI address %U", format_vlib_pci_addr, addr);
    }
    else if (addr_type == VNET_DEV_ADDR_VMBUS) {
        p = mhash_get(&conf->device_config_index_by_vmbus_addr, (vlib_vmbus_addr_t *) (addr));
        ...
        if (!p) {
            ...
            return clib_error_return(0, "duplicate configuration for VMBUS address %U", format_vlib_vmbus_addr, addr);
    }
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "num-rx-queues %u", &devconf->num_rx_queues))
            ...
            ;
        else if (unformat(input, "workers %U", unformat_bitmap_list, &devconf->workers))
            ;
            ...
            ;
        else if (unformat(input, "rss-queues %U", unformat_bitmap_list, &devconf->rss_queues))
            ;
        else if (unformat(input, "max-lro-pkt-size %u", &devconf->max_lro_pkt_size))
            ;
        else {
    ...
    else if (devconf->workers && clib_bitmap_count_set_bits(devconf->workers) != devconf->num_rx_queues)
...
static clib_error_t *dpdk_log_read_ready(clib_file_t *uf)
{
    ...
    while (n == n_try) {
        ...
        if (n < 0 && errno != EAGAIN) return clib_error_return_unix(0, "read");
    ...
    while (unformat_user(&input, unformat_line, &line)) {
        ...
        if (strstr((char *) line, "WARNING! Base virtual address hint"))
            skip = 1;
        else if (strstr((char *) line,
                        "This may cause issues with mapping "
                        "memory into secondary processes"))
            skip = 1;
        vec_pop(line);
        if (!skip) dpdk_log_notice("%v", line);
        vec_free(line);
    }
...
static clib_error_t *dpdk_set_stat_poll_interval(f64 interval)
{
    if (interval < DPDK_MIN_STATS_POLL_INTERVAL) return clib_error_return(0, "wrong stats-poll-interval value");

    dpdk_main.stat_poll_interval = interval;
    return 0;
}

static clib_error_t *dpdk_set_link_state_poll_interval(f64 interval)
{
    if (interval < DPDK_MIN_LINK_POLL_INTERVAL) return clib_error_return(0, "wrong link-state-poll-interval value");
...
static clib_error_t *dpdk_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    mhash_init(&conf->device_config_index_by_vmbus_addr, sizeof(uword), sizeof(vlib_vmbus_addr_t));
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        if (unformat(input, "no-hugetlb")) {
            ...
            if (unformat(input, "enable-outer-checksum-offload")) dm->default_port_conf.enable_outer_checksum_offload = 1;
        }
        ...
        else if (unformat(input, "max-simd-bitwidth %U", unformat_max_simd_bitwidth, &conf->max_simd_bitwidth))
            ;
        ...
        else if (unformat(input, "dev default %U", unformat_vlib_cli_sub_input, &sub_input)) {
            error = dpdk_device_config(conf, 0, VNET_DEV_ADDR_ANY, &sub_input, 1);

            if (error) return error;
        }
        else if (unformat(input, "dev %U %U", unformat_vlib_pci_addr, &pci_addr, unformat_vlib_cli_sub_input, &sub_input)) {
            error = dpdk_device_config(conf, &pci_addr, VNET_DEV_ADDR_PCI, &sub_input, 0);
        ...
        else if (unformat(input, "dev %U %U", unformat_vlib_vmbus_addr, &vmbus_addr, unformat_vlib_cli_sub_input, &sub_input)) {
            error = dpdk_device_config(conf, &vmbus_addr, VNET_DEV_ADDR_VMBUS, &sub_input, 0);

            if (error) return error;

            num_whitelisted++;
        }
        else if (unformat(input, "dev %U", unformat_vlib_vmbus_addr, &vmbus_addr)) {
            error = dpdk_device_config(conf, &vmbus_addr, VNET_DEV_ADDR_VMBUS, 0, 0);
        ...
        else if (unformat(input, "blacklist %U", unformat_vlib_vmbus_addr, &vmbus_addr)) {
            vec_add1(conf->blacklist_by_vmbus_addr, vmbus_addr);
        }
        else if (unformat(input, "blacklist %x:%x:%x.%x", &domain, &bus, &device, &func)) {
            tmp = format(0, "-b%c", 0);
            vec_add1(conf->eal_init_args, tmp);
            tmp = format(0, "%04x:%02x:%02x.%x%c", domain, bus, device, func, 0);
            vec_add1(conf->eal_init_args, tmp);
        }
        else if (unformat(input, "blacklist %x:%x", &vendor, &device)) {
            u32 blacklist_entry;
            if (vendor > 0xFFFF) return clib_error_return(0, "blacklist PCI vendor out of range");
            if (device > 0xFFFF) return clib_error_return(0, "blacklist PCI device out of range");
            blacklist_entry = (vendor << 16) | (device & 0xffff);
            vec_add1(conf->blacklist_by_pci_vendor_and_device, blacklist_entry);
        }
...
#define _(a)                                                                                                            \
    ...
    {                                                                                                                   \
        if (!strncmp(#a, "file-prefix", 11)) file_prefix = 1;                                                           \
        tmp = format(0, "--%s%c", #a, 0);                                                                               \
        vec_add1(conf->eal_init_args, tmp);                                                                             \
        vec_add1(s, 0);                                                                                                 \
        if (!strncmp(#a, "vdev", 4))                                                                                    \
            if (strstr((char *) s, "af_packet")) clib_warning("af_packet obsoleted. Use CLI 'create host-interface'."); \
...
#undef _
    ...
    if (!conf->uio_driver_name) conf->uio_driver_name = format(0, "auto%c", 0);
    ...
    if (eal_no_hugetlb == 0) {
        ...
        clib_bitmap_foreach (x, tm->cpu_socket_bitmap) {
            ...
            if ((e = clib_sysfs_prealloc_hugepages(x, 0, n_pages))) clib_error_report(e);
        }
#endif /* __linux__ */
    }
    ...
    if (no_pci == 0 && geteuid() == 0) dpdk_bind_devices_to_uio(conf);

    if (no_vmbus == 0 && geteuid() == 0) dpdk_bind_vmbus_devices_to_uio(conf);

#define _(x) \
    if (devconf->x == 0 && conf->default_devconf.x > 0) devconf->x = conf->default_devconf.x;
    ...
    pool_foreach (devconf, conf->dev_confs) {
        ...
        if (num_whitelisted > 0 && devconf->is_blacklisted == 0) {
            ...
            vec_add1(conf->eal_init_args, tmp);
            if (devconf->devargs) {
                tmp = format(0, "%U,%s%c", fmt_func, fmt_addr, devconf->devargs, 0);
            }
            else {
...
#undef _
    ...
    if (socket_mem) clib_warning("socket-mem argument is deprecated");
    ...
    int log_fds[2] = {0};
    if (pipe(log_fds) == 0) {
        if (fcntl(log_fds[0], F_SETFL, O_NONBLOCK) == 0 && fcntl(log_fds[1], F_SETFL, O_NONBLOCK) == 0) {
    ...
    for (i = 1; i < vec_len(conf->eal_init_args); i++)
        conf->eal_init_args_str = format(conf->eal_init_args_str, "%s ", conf->eal_init_args[i]);
    ...
    if (ret < 0) return clib_error_return(0, "rte_eal_init returned %d", ret);

    /* enable the AVX-512 vPMDs in DPDK */
    if (clib_cpu_supports_avx512_bitalg() && conf->max_simd_bitwidth == DPDK_MAX_SIMD_BITWIDTH_DEFAULT)
        rte_vect_set_max_simd_bitwidth(RTE_VECT_SIMD_512);
    else if (conf->max_simd_bitwidth != DPDK_MAX_SIMD_BITWIDTH_DEFAULT)
        rte_vect_set_max_simd_bitwidth(conf->max_simd_bitwidth == DPDK_MAX_SIMD_BITWIDTH_256 ? RTE_VECT_SIMD_256 : RTE_VECT_SIMD_512);

    /* main thread 1st */
    if ((error = dpdk_buffer_pools_create(vm))) return error;
...
void dpdk_update_link_state(dpdk_device_t *xd, f64 now)
{
    ...
    if (LINK_STATE_ELOGS) {
        ...
        ed->old_link_state = (u8) vnet_hw_interface_is_link_up(vnm, xd->hw_if_index);
        ed->new_link_state = (u8) xd->link.link_status;
    }

    hw_flags_chg = ((xd->link.link_duplex != prev_link.link_duplex) || (xd->link.link_status != prev_link.link_status));

    if (xd->link.link_speed != prev_link.link_speed)
        vnet_hw_interface_set_link_speed(vnm, xd->hw_if_index, (xd->link.link_speed == UINT32_MAX) ? UINT32_MAX : xd->link.link_speed * 1000);

    if (hw_flags_chg) {
        if (xd->link.link_status) hw_flags |= VNET_HW_INTERFACE_FLAG_LINK_UP;
...
static uword dpdk_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    f64 timeout = clib_min(dm->link_state_poll_interval, dm->stat_poll_interval);

    while (1) {
        f64 min_wait = clib_max(timeout, DPDK_MIN_POLL_INTERVAL);
        vlib_process_wait_for_event_or_clock(vm, min_wait);

        timeout = clib_min(dm->link_state_poll_interval, dm->stat_poll_interval);

        if (dm->admin_up_down_in_progress) /* skip the poll if an admin up down is in progress (on any interface) */
            continue;

        vec_foreach (xd, dm->devices) {
            f64 now = vlib_time_now(vm);
            if ((now - xd->time_last_stats_update) >= dm->stat_poll_interval) dpdk_update_counters(xd, now);
            if ((now - xd->time_last_link_update) >= dm->link_state_poll_interval) dpdk_update_link_state(xd, now);
        }

        now = vlib_time_now(vm);
        vec_foreach (xd, dm->devices) {
            timeout = clib_min(timeout, xd->time_last_stats_update + dm->stat_poll_interval - now);
            timeout = clib_min(timeout, xd->time_last_link_update + dm->link_state_poll_interval - now);
        }
    }
    return 0;
}
...
static clib_error_t *dpdk_init(vlib_main_t *vm)
{
    ...
    STATIC_ASSERT(offsetof(dpdk_device_t, cacheline0) == 0, "Cache line marker must be 1st element in dpdk_device_t");
    STATIC_ASSERT(offsetof(dpdk_device_t, cacheline1) == CLIB_CACHE_LINE_BYTES, "Data in cache line 0 is bigger than cache line size");
    STATIC_ASSERT(offsetof(frame_queue_trace_t, cacheline0) == 0, "Cache line marker must be 1st element in frame_queue_trace_t");
...
static clib_error_t *dpdk_worker_thread_init(vlib_main_t *vm)
{
    if (rte_thread_register() < 0) clib_panic("dpdk: cannot register thread %u - %s", vm->thread_index, rte_strerror(rte_errno));
    return 0;
}
