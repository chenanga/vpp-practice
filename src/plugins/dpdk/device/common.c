...
} tx_off_caps_map[] = {{RTE_ETH_TX_OFFLOAD_IPV4_CKSUM, VNET_HW_IF_CAP_TX_IP4_CKSUM},
...
{
    dpdk_log_err("Interface %U error %d: %s", format_dpdk_device_name, xd->device_index, rv, rte_strerror(rv));
    xd->errors = clib_error_return(xd->errors, "%s[port:%d, errno:%d]: %s", str, xd->port_id, rv, rte_strerror(rv));
}
...
void dpdk_device_setup(dpdk_device_t *xd)
{
    ...
    dpdk_log_debug("[%u] configuring device %U", xd->port_id, format_dpdk_rte_device, dev_info.device);

    /* create rx and tx offload wishlist */
    rxo = RTE_ETH_RX_OFFLOAD_IPV4_CKSUM;
    txo = 0;

    if (xd->conf.enable_tcp_udp_checksum) rxo |= RTE_ETH_RX_OFFLOAD_UDP_CKSUM | RTE_ETH_RX_OFFLOAD_TCP_CKSUM;

    if (xd->conf.disable_tx_checksum_offload == 0 && xd->conf.enable_outer_checksum_offload)
        txo |= RTE_ETH_TX_OFFLOAD_OUTER_IPV4_CKSUM | RTE_ETH_TX_OFFLOAD_OUTER_UDP_CKSUM;

    if (xd->conf.disable_tx_checksum_offload == 0) txo |= RTE_ETH_TX_OFFLOAD_IPV4_CKSUM | RTE_ETH_TX_OFFLOAD_TCP_CKSUM | RTE_ETH_TX_OFFLOAD_UDP_CKSUM;
    ...
    if (xd->conf.enable_lro) rxo |= RTE_ETH_RX_OFFLOAD_TCP_LRO;

    /* per-device offload config */
    if (xd->conf.enable_tso) txo |= RTE_ETH_TX_OFFLOAD_TCP_CKSUM | RTE_ETH_TX_OFFLOAD_TCP_TSO | RTE_ETH_TX_OFFLOAD_VXLAN_TNL_TSO;

    if (xd->conf.disable_rx_scatter) rxo &= ~RTE_ETH_RX_OFFLOAD_SCATTER;

    /* mask unsupported offloads */
    rxo &= dev_info.rx_offload_capa;
    txo &= dev_info.tx_offload_capa;

    dpdk_log_debug("[%u] Supported RX offloads: %U", xd->port_id, format_dpdk_rx_offload_caps, dev_info.rx_offload_capa);
    dpdk_log_debug("[%u] Configured RX offloads: %U", xd->port_id, format_dpdk_rx_offload_caps, rxo);
    dpdk_log_debug("[%u] Supported TX offloads: %U", xd->port_id, format_dpdk_tx_offload_caps, dev_info.tx_offload_capa);
    dpdk_log_debug("[%u] Configured TX offloads: %U", xd->port_id, format_dpdk_tx_offload_caps, txo);

    /* finalize configuration */
    conf.rxmode.offloads = rxo;
    conf.txmode.offloads = txo;
    if (rxo & RTE_ETH_RX_OFFLOAD_TCP_LRO) conf.rxmode.max_lro_pkt_size = xd->conf.max_lro_pkt_size;

    if (xd->conf.enable_lsc_int) conf.intr_conf.lsc = 1;
    if (xd->conf.enable_rxq_int) conf.intr_conf.rxq = 1;
    ...
    if (rxo & DEV_RX_OFFLOAD_JUMBO_FRAME) {
        ...
        xd->max_supported_frame_size = clib_min(1500 + xd->driver_frame_overhead, buf_sz);
    }
#else
    ...
    max_frame_size = clib_min(xd->max_supported_frame_size, ethernet_main.default_mtu + hi->frame_overhead);
...
retry:
    rv = rte_eth_dev_configure(xd->port_id, xd->conf.n_rx_queues, xd->conf.n_tx_queues, &conf);
...
#if RTE_VERSION < RTE_VERSION_NUM(21, 11, 0, 0)
    rte_eth_dev_set_mtu(xd->port_id, max_frame_size - xd->driver_frame_overhead);
#endif

    hi->max_frame_size = 0;
    vnet_hw_interface_set_max_frame_size(vnm, xd->hw_if_index, max_frame_size);
    dpdk_log_debug(
        "[%u] max_frame_size %u max max_frame_size %u "
    ...
    for (j = 0; j < xd->conf.n_tx_queues; j++) {
        rv = rte_eth_tx_queue_setup(xd->port_id, j, xd->conf.n_tx_desc, xd->cpu_socket, 0);

        /* retry with any other CPU socket */
        if (rv < 0) rv = rte_eth_tx_queue_setup(xd->port_id, j, xd->conf.n_tx_desc, SOCKET_ID_ANY, 0);
        if (rv < 0) dpdk_device_error(xd, "rte_eth_tx_queue_setup", rv);
    ...
    for (j = 0; j < xd->conf.n_rx_queues; j++) {
        ...
        u8                  bpidx = vlib_buffer_pool_get_default_for_numa(vm, vnet_hw_if_get_rx_queue_numa_node(vnm, rxq->queue_index));
        vlib_buffer_pool_t *bp    = vlib_get_buffer_pool(vm, bpidx);
        struct rte_mempool *mp    = dpdk_mempool_by_buffer_pool_index[bpidx];

        rv = rte_eth_rx_queue_setup(xd->port_id, j, xd->conf.n_rx_desc, xd->cpu_socket, 0, mp);

        /* retry with any other CPU socket */
        if (rv < 0) rv = rte_eth_rx_queue_setup(xd->port_id, j, xd->conf.n_rx_desc, SOCKET_ID_ANY, 0, mp);

        rxq->buffer_pool_index = bp->index;

        if (rv < 0) dpdk_device_error(xd, "rte_eth_rx_queue_setup", rv);
    }

    if (vec_len(xd->errors)) goto error;

    xd->buffer_flags = (VLIB_BUFFER_TOTAL_LENGTH_VALID | VLIB_BUFFER_EXT_HDR_VALID);

    if ((rxo & (RTE_ETH_RX_OFFLOAD_TCP_CKSUM | RTE_ETH_RX_OFFLOAD_UDP_CKSUM)) == (RTE_ETH_RX_OFFLOAD_TCP_CKSUM | RTE_ETH_RX_OFFLOAD_UDP_CKSUM))
        xd->buffer_flags |= (VNET_BUFFER_F_L4_CHECKSUM_COMPUTED | VNET_BUFFER_F_L4_CHECKSUM_CORRECT);

    dpdk_device_flag_set(xd, DPDK_DEVICE_FLAG_RX_IP4_CKSUM, rxo & RTE_ETH_RX_OFFLOAD_IPV4_CKSUM);
    dpdk_device_flag_set(xd, DPDK_DEVICE_FLAG_MAYBE_MULTISEG, rxo & RTE_ETH_RX_OFFLOAD_SCATTER);
    dpdk_device_flag_set(
        xd,
        DPDK_DEVICE_FLAG_TX_OFFLOAD,
        (txo & (RTE_ETH_TX_OFFLOAD_TCP_CKSUM | RTE_ETH_TX_OFFLOAD_UDP_CKSUM)) == (RTE_ETH_TX_OFFLOAD_TCP_CKSUM | RTE_ETH_TX_OFFLOAD_UDP_CKSUM));
    ...
    for (int i = 0; i < ARRAY_LEN(tx_off_caps_map); i++) {
        ...
        caps.mask |= v->caps;
        if ((v->offload & txo) == v->offload) caps.val |= v->caps;
    }
    ...
    if (xd->flags & DPDK_DEVICE_FLAG_ADMIN_UP) dpdk_device_start(xd);
...
static void dpdk_setup_interrupts(dpdk_device_t *xd)
{
    ...
    if (!xd->conf.enable_rxq_int) return;
    ...
    if (xd->flags & DPDK_DEVICE_FLAG_INT_SUPPORTED) {
        ...
        for (int q = 0; q < xd->conf.n_rx_queues; q++) {
            ...
            f.description        = format(0, "%U queue %u", format_dpdk_device_name, xd->device_index, q);
            rxq->clib_file_index = clib_file_add(&file_main, &f);
            vnet_hw_if_set_rx_queue_file_index(vnm, rxq->queue_index, rxq->clib_file_index);
            if (xd->flags & DPDK_DEVICE_FLAG_INT_UNMASKABLE) {
                clib_file_main_t *fm = &file_main;
                clib_file_t      *f  = pool_elt_at_index(fm->file_pool, rxq->clib_file_index);
...
void dpdk_device_start(dpdk_device_t *xd)
{
    ...
    if (xd->flags & DPDK_DEVICE_FLAG_PMD_INIT_FAIL) return;
    ...
    dpdk_log_debug("[%u] RX burst function: %U", xd->port_id, format_dpdk_burst_fn, xd, VLIB_RX);
    dpdk_log_debug("[%u] TX burst function: %U", xd->port_id, format_dpdk_burst_fn, xd, VLIB_TX);

    dpdk_setup_interrupts(xd);

    if (xd->default_mac_address) rv = rte_eth_dev_default_mac_addr_set(xd->port_id, (void *) xd->default_mac_address);

    if (rv) dpdk_device_error(xd, "rte_eth_dev_default_mac_addr_set", rv);
    ...
    dpdk_log_info("Interface %U started", format_dpdk_device_name, xd->device_index);
}
...
void dpdk_device_stop(dpdk_device_t *xd)
{
    ...
    dpdk_log_info("Interface %U stopped", format_dpdk_device_name, xd->device_index);
}

void vl_api_force_rpc_call_main_thread(void *fp, u8 *data, u32 data_length);

always_inline int dpdk_port_state_callback_inline(dpdk_portid_t port_id, enum rte_eth_event_type type, void *param)
{
...
int dpdk_port_state_callback(dpdk_portid_t port_id, enum rte_eth_event_type type, void *param, void *ret_param __attribute__((unused)))
{
