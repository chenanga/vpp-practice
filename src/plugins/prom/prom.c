...
static u8 *make_stat_name(char *name)
{
    ...
    pm->name_scratch_pad = format(pm->name_scratch_pad, "%v%s", pm->stat_name_prefix, name);
    return pm->name_scratch_pad;
}

static u8 *dump_counter_vector_simple(stat_segment_data_t *res, u8 *s, u8 used_only)
{
    u8  need_header = 1;
    int j, k;
    u8 *name;
    ...
    for (k = 0; k < vec_len(res->simple_counter_vec); k++)
        for (j = 0; j < vec_len(res->simple_counter_vec[k]); j++) {
            if (used_only && !res->simple_counter_vec[k][j]) continue;
            if (need_header) {
                s           = format(s, "# TYPE %v counter\n", name);
                need_header = 0;
            }
            s = format(s, "%v{thread=\"%d\",interface=\"%d\"} %lld\n", name, k, j, res->simple_counter_vec[k][j]);
        }

    return s;
}

static u8 *dump_counter_vector_combined(stat_segment_data_t *res, u8 *s, u8 used_only)
{
    u8  need_header = 1;
    int j, k;
    u8 *name;
    ...
    for (k = 0; k < vec_len(res->simple_counter_vec); k++)
        for (j = 0; j < vec_len(res->combined_counter_vec[k]); j++) {
            if (used_only && !res->combined_counter_vec[k][j].packets) continue;
            if (need_header) {
            ...
            s = format(s, "%v_bytes{thread=\"%d\",interface=\"%d\"} %lld\n", name, k, j, res->combined_counter_vec[k][j].bytes);
        }

    return s;
}

static u8 *dump_scalar_index(stat_segment_data_t *res, u8 *s, u8 used_only)
{
    u8 *name;

    if (used_only && !res->scalar_value) return s;
...
static u8 *dump_name_vector(stat_segment_data_t *res, u8 *s, u8 used_only)
{
    u8 *name;
    int k;
    ...
    for (k = 0; k < vec_len(res->name_vector); k++)
        s = format(s, "%v_info{index=\"%d\",name=\"%s\"} 1\n", name, k, res->name_vector[k]);

    return s;
}

static u8 *scrape_stats_segment(u8 *s, u8 **patterns, u8 used_only)
{
...
retry:
    ...
    if (res == 0) { /* Memory layout has changed */
        if (stats) vec_free(stats);
        stats = stat_segment_ls(patterns);
        goto retry;
    }
...
static void send_data_to_hss(hss_session_handle_t sh)
{
...
static void send_data_to_hss_rpc(void *rpc_args)
{
    send_data_to_hss(*(hss_session_handle_t *) rpc_args);
}

static uword prom_scraper_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    while (1) {
        ...
        switch (event_type) {
            ...
            case PROM_SCRAPER_EVT_RUN:
                ...
                pm->stats = scrape_stats_segment(pm->stats, pm->stats_patterns, pm->used_only);
                vec_foreach (sh_as_uword, event_data) {
                    sh.as_u64 = (u64) *sh_as_uword;
                    session_send_rpc_evt_to_thread_force(sh.thread_index, send_data_to_hss_rpc, sh_as_uword);
                }
...
static void prom_scraper_process_enable(vlib_main_t *vm)
{
    prom_main_t *pm = &prom_main;
    vlib_node_t *n;

    vlib_node_set_state(vm, prom_scraper_process_node.index, VLIB_NODE_STATE_POLLING);
...
static void signal_run_to_scraper(uword *args)
{
    prom_main_t *pm = &prom_main;
    ASSERT(vlib_get_thread_index() == 0);
    vlib_process_signal_event(pm->vm, pm->scraper_node_index, PROM_SCRAPER_EVT_RUN, *args);
}

hss_url_handler_rc_t prom_stats_dump(hss_url_handler_args_t *args)
{
    ...
    if (vm->thread_index != 0)
        vl_api_rpc_call_main_thread(signal_run_to_scraper, (u8 *) &args->sh, sizeof(args->sh));
    else
...
void prom_stat_patterns_add(u8 **patterns)
{
    ...
    vec_foreach (pattern, patterns) {
        ...
        vec_foreach (existing, pm->stats_patterns) {
            if (vec_len(*existing) != len) continue;
            if (!memcmp(*existing, *pattern, len - 1)) {
                found = 1;
                break;
            }
        }
        if (!found) vec_add1(pm->stats_patterns, *pattern);
    }
}
...
static void prom_stat_segment_client_init(void)
{
    ...
    scm->directory_vector = stat_segment_adjust(scm, (void *) scm->shared_header->directory_vector);
}
...
void prom_enable(vlib_main_t *vm)
{
    ...
    pm->register_url = vlib_get_plugin_symbol("http_static_plugin.so", "hss_register_url_handler");
    pm->send_data    = vlib_get_plugin_symbol("http_static_plugin.so", "hss_session_send_data");
    pm->register_url(prom_stats_dump, "stats.prom", HTTP_REQ_GET);

    pm->is_enabled = 1;
    pm->vm         = vm;
    if (!pm->stat_name_prefix) pm->stat_name_prefix = format(0, "vpp");

    prom_scraper_process_enable(vm);
    prom_stat_segment_client_init();
}

static clib_error_t *prom_init(vlib_main_t *vm)
{
    prom_main_t *pm = &prom_main;
