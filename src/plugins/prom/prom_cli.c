...
static uword unformat_stats_patterns(unformat_input_t *input, va_list *args)
{
...
static clib_error_t *prom_patterns_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "show"))
            ...
            is_clear = 1;
        else if (unformat(line_input, "add %U", unformat_stats_patterns, &pattern)) {
    ...
    if (is_clear) prom_stat_patterns_free();
...
static clib_error_t *prom_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "enable"))
            is_enable = 1;
        else if (unformat(line_input, "min-scrape-interval %f", &pm->min_scrape_interval))
            ;
        ...
        else if (unformat(line_input, "stat-name-prefix %_%v%_", &stat_name_prefix))
            prom_stat_name_prefix_set(stat_name_prefix);
        else if (unformat(line_input, "stat-patterns %U", unformat_stats_patterns, &patterns))
...
no_input:
    ...
    if (is_enable && !pm->is_enabled) prom_enable(vm);
...
VLIB_CLI_COMMAND(prom_enable_command, static) = {
    .path = "prom",
    .short_help =
        "prom [enable] [min-scrape-interval <n>] [used-only] "
