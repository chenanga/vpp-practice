...
uword unformat_perfmon_bundle_name(unformat_input_t *input, va_list *args)
{
    ...
    if (p) b[0] = (perfmon_bundle_t *) p[0];

    vec_free(str);
    return p ? 1 : 0;
}

uword unformat_perfmon_active_type(unformat_input_t *input, va_list *args)
{
    perfmon_bundle_t      *b           = va_arg(*args, perfmon_bundle_t *);
...
#undef _
    ...
    for (int i = PERFMON_BUNDLE_TYPE_NODE; i < PERFMON_BUNDLE_TYPE_MAX; i++) {
        ...
        if (strncmp(str, _str_types[i], strlen(_str_types[i])) == 0 && (b->type_flags & 1 << i)) {
            *bundle_type = i;
            break;
        }
    }
...
{
    perfmon_bundle_t            *b   = va_arg(*args, perfmon_bundle_t *);
    format_perfmon_bundle_args_t cfg = va_arg(*args, format_perfmon_bundle_args_t);
...
#undef _
        ...
        if (b == 0) return format(s, "%-20s%-20s%-20s%s", "Name", "Type(s)", "Source", "Description");
    ...
    if (cfg != FORMAT_PERFMON_BUNDLE_NONE) {
        ...
        for (int i = 0; i < b->n_events; i++) {
            ...
            if (format_config && cfg == FORMAT_PERFMON_BUNDLE_SHOW_CONFIG) s = format(s, " (%U)", format_config, e->config);
        ...
        for (int i = PERFMON_BUNDLE_TYPE_NODE; i < PERFMON_BUNDLE_TYPE_MAX; i++) {
            ...
            if (b->type_flags & 1 << i) _bundle_type = format(_bundle_type, "%s,", bundle_type[i]);
        }
        /* remove any stray commas */
        if ((vl = vec_len(_bundle_type))) _bundle_type[vl - 1] = 0;

        s = format(s, "%-20s%-20s%s", _bundle_type, b->src->name, b->description);
    }
...
static clib_error_t *show_perfmon_bundle_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (unformat_user(input, unformat_line_input, line_input)) {
        while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
            if (unformat(line_input, "verbose"))
                verbose = 1;
            else if (unformat(line_input, "%U", unformat_perfmon_bundle_name, &b))
                vec_add(vb, &b, 1);
            else
    ...
    if (cfg == FORMAT_PERFMON_BUNDLE_NONE) vlib_cli_output(vm, "%U\n", format_perfmon_bundle, 0, cfg);
    ...
    for (int i = 0; i < vec_len(vb); i++)
        ...
        if (vb[i]->type_flags) vlib_cli_output(vm, "%U\n", format_perfmon_bundle, vb[i], cfg);
...
u8 *format_perfmon_source(u8 *s, va_list *args)
{
    ...
    if (src == 0) return format(s, "%-20s%-9s %s", "Name", "NumEvents", "Description");
    ...
    if (verbose) {
        ...
        if (src->instances_by_type) {
            ...
            for (int i = 0; i < vec_len(src->instances_by_type); i++) {
                ...
                if (vec_len(it->instances) == 0) continue;
                s = format(s, "  %s:\n   ", it->name);
...
static clib_error_t *show_perfmon_source_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (unformat_user(input, unformat_line_input, line_input)) {
        while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
            if (unformat(line_input, "verbose"))
                verbose = 1;
            else if (unformat(line_input, "%U", unformat_perfmon_source_name, &s))
                vec_add(vs, &s, 1);
            else
                return clib_error_return(0, "unknown input `%U'", format_unformat_error, line_input);
        }
    ...
    if (verbose == 0) vlib_cli_output(vm, "%U\n", format_perfmon_source, 0, 0);
...
static clib_error_t *show_perfmon_stats_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (b == 0) return clib_error_return(0, "no bundle selected");

    n_instances = vec_len(it->instances);
    vec_validate(readings, n_instances - 1);

    /*Only perform read() for THREAD or SYSTEM bundles*/
    for (int i = 0; i < n_instances && b->active_type != PERFMON_BUNDLE_TYPE_NODE; i++) {
    ...
    for (int i = 0; i < n_instances; i++) {
        ...
        if (b->active_type == PERFMON_BUNDLE_TYPE_NODE) {
            ...
            for (int j = 0; j < tr->n_nodes; j++)
                if (tr->node_stats[j].n_calls) {
                    ...
                    table_format_cell(t, ++col, -1, "%U", format_vlib_node_name, vm, j, b->active_type);
                    table_set_cell_align(t, col, -1, TTAA_RIGHT);
                    table_set_cell_fg_color(t, col, -1, TTAC_CYAN);

                    if (PREDICT_TRUE(clib_bitmap_is_zero(b->event_disabled))) clib_memcpy_fast(&ns, tr->node_stats + j, sizeof(ns));
                    /* if some events are not implemented, we need to realign these
                       to display under the correct column headers */
                        ...
                        /* loop through all events in bundle + manually copy into
                           ...
                           implemented */
                        int num_enabled_events = b->n_events - clib_bitmap_count_set_bits(b->event_disabled);
                    ...
                    for (int j = 0; j < n_row; j++) {
                        if (clib_bitmap_get(b->column_disabled, j))
                            ...
                            table_format_cell(t, col, j, "%U", b->format_fn, &ns, j, b->active_type);
                    }
                }
        }
        else /* b->type != PERFMON_BUNDLE_TYPE_NODE */
        {
            if (PREDICT_TRUE(clib_bitmap_is_zero(b->event_disabled))) {
                for (int j = 0; j < n_row; j++)
                    table_format_cell(t, i, j, "%U", b->format_fn, r, j, b->active_type);
            }
               ...
               implemented, we need to realign readings under column headings */
                ...
                for (int j = 0; j < n_row; j++) {
                    if (clib_bitmap_get(b->column_disabled, j))
                        ...
                        table_format_cell(t, i, j, "%U", b->format_fn, aligned_r, j, b->active_type);
                }
            }
        }
        col++;
    ...
    if (b->footer) vlib_cli_output(vm, "\n%s\n", b->footer);
...
static clib_error_t *perfmon_start_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (pm->is_running) return clib_error_return(0, "please stop first");

    if (unformat_user(input, unformat_line_input, line_input) == 0) return clib_error_return(0, "please specify bundle name");

    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "bundle %U", unformat_perfmon_bundle_name, &b))
            ;
        else if (unformat(line_input, "type %U", unformat_perfmon_active_type, b, &bundle_type))
            ;
        else
            return clib_error_return(0, "unknown input '%U'", format_unformat_error, line_input);
    }
    ...
    if (b == 0) return clib_error_return(0, "please specify bundle name");

    /* if there is more than one valid mode */
    if (count_set_bits(b->type_flags) > 1) {
        /* what did the user indicate */
        if (!bundle_type) return clib_error_return(0, "please specify a valid type");
    }
    /* otherwise just use the default  */
    else if (!bundle_type) {
        bundle_type = (perfmon_bundle_type_t) count_trailing_zeros(b->type_flags);
    }
