...
u8 *format_intel_core_config(u8 *s, va_list *args)
{
    ...
    s = format(s, "event=0x%02x, umask=0x%02x", config & 0xff, (config >> 8) & 0xff);

    if ((v = (config >> 18) & 1)) s = format(s, ", edge=%u", v);

    if ((v = (config >> 19) & 1)) s = format(s, ", pc=%u", v);

    if ((v = (config >> 21) & 1)) s = format(s, ", any=%u", v);

    if ((v = (config >> 23) & 1)) s = format(s, ", inv=%u", v);

    if ((v = (config >> 24) & 0xff)) s = format(s, ", cmask=0x%02x", v);

    /* show the raw config, for convenience sake */
    if (!((config >> 16) & 0xffff)) s = format(s, ", raw=r%x", config & 0xffff);

    return s;
}

static clib_error_t *intel_core_init(vlib_main_t *vm, perfmon_source_t *src)
{
    u32 eax, ebx, ecx, edx;
    if (__get_cpuid(0, &eax, &ebx, &ecx, &edx) == 0) return clib_error_return(0, "unknown CPU (missing cpuid)");

    // GenuineIntel
    if (ebx != 0x756e6547 || ecx != 0x6c65746e || edx != 0x49656e69) return clib_error_return(0, "not a IA-32 CPU");
    return 0;
}

perfmon_event_type_t intel_core_get_event_type(u32 event)
{
...
static u8 is_enough_counters(perfmon_bundle_t *b)
{
    ...
    if (!clib_get_pmu_counter_count(&cpu[PERFMON_EVENT_TYPE_FIXED], &cpu[PERFMON_EVENT_TYPE_GENERAL])) return 0;
    ...
    return cpu[PERFMON_EVENT_TYPE_GENERAL] >= bl[PERFMON_EVENT_TYPE_GENERAL] && cpu[PERFMON_EVENT_TYPE_FIXED] >= bl[PERFMON_EVENT_TYPE_FIXED];
}
...
u8 intel_bundle_supported(perfmon_bundle_t *b)
{
    ...
    if (!is_enough_counters(b)) return 0;

    if (!b->cpu_supports) return 1;

    for (int i = 0; i < b->n_cpu_supports; ++i)
        if (supports[i].cpu_supports()) return 1;
