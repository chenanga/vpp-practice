...
static intel_uncore_unit_type_names_t uncore_unit_names[] = {
    {INTEL_UNCORE_UNIT_IIO, PERFMON_STRINGS("PCIe0", "PCIe1", "MCP", "PCIe2", "PCIe3", "CBDMA/DMI")}};
...
#define _(unit, event, umask, ch_mask, fc_mask, n, suffix, desc)                                      \
    [INTEL_UNCORE_E_##unit##_##n##_##suffix] = {                                                      \
        .config             = (event) | (umask) << 8 | (u64) (ch_mask) << 36 | (u64) (fc_mask) << 48, \
        .name               = #n "." #suffix,                                                         \
        .description        = desc,                                                                   \
...
static u8 *format_instance_name(intel_uncore_unit_type_t u, char *unit_fmt, u8 socket_id, u8 ubox)
{
    u8 *s = 0;
...
static void intel_uncore_add_unit(perfmon_source_t *src, intel_uncore_unit_type_t u, char *name, char *type_str, char *fmt, int *socket_by_cpu_id)
{
    ...
    while (1) {
        ...
        if ((err = clib_sysfs_read((char *) s, "%u", &perf_type))) break;
        vec_reset_length(s);

        s = format(s, "%s_%s_%u/cpumask%c", base_path, name, i, 0);
        if ((err = clib_sysfs_read((char *) s, "%U", unformat_bitmap_list, &cpumask))) break;
        vec_reset_length(s);
        ...
        clib_bitmap_foreach (j, cpumask) {
            ...
            in->cpu  = j;
            in->pid  = -1;
            in->name = (char *) format_instance_name(u, fmt, socket_by_cpu_id[j], i);
...
static clib_error_t *intel_uncore_init(vlib_main_t *vm, perfmon_source_t *src)
{
    ...
    if (!node_bitmap) return clib_error_return(0, "failed to discover numa topology");

    clib_bitmap_foreach (i, node_bitmap) {
        s = format(s, "/sys/devices/system/node/node%u/cpulist%c", i, 0);
        if ((err = clib_sysfs_read((char *) s, "%U", unformat_bitmap_list, &cpumask))) {
            clib_error_free(err);
        ...
        if (!cpumask) {
            clib_error_free(err);
            err = clib_error_return(0, "while discovering numa topology: cpumask unexpectedly NULL");
            goto done;
        }
...
#define _(t, n, name, fmt) intel_uncore_add_unit(src, INTEL_UNCORE_UNIT_##t, n, name, fmt, numa_by_cpu_id);
