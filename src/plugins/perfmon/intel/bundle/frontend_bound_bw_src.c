...
static u8 *format_intel_frontend_bound_bw_src(u8 *s, va_list *args)
{
    perfmon_node_stats_t *ss   = va_arg(*args, perfmon_node_stats_t *);
    int                   row  = va_arg(*args, int);
    f64                   sv   = 0;
    f64                   uops = ss->value[DSB_UOPS] + ss->value[MS_UOPS] + ss->value[MITE_UOPS] + ss->value[LSD_UOPS];

    if (!ss->n_packets) return s;
...
PERFMON_REGISTER_BUNDLE(intel_core_frontend_bound_bw_src) = {
    ...
    .description    = "Topdown FrontEnd-bound BandWidth - % uops from each uop fetch source",
    .source         = "intel-core",
    ...
    .column_headers = PERFMON_STRINGS("UOPs/PKT", "% DSB UOPS", "% MS UOPS", "% MITE UOPS", "% LSD UOPS"),
    .footer =
