...
static u8 *format_intel_core_cache_hit_miss(u8 *s, va_list *args)
{
    ...
    switch (row) {
        ...
        case 1:
            ...
            break;
        case 2:
            s = format(s, "%0.2f", (f64) (ns->value[1] - clib_min(ns->value[1], ns->value[2])) / ns->n_packets);
            break;
        case 3:
            s = format(s, "%0.2f", (f64) ns->value[2] / ns->n_packets);
            break;
        case 4:
            s = format(s, "%0.2f", (f64) (ns->value[2] - clib_min(ns->value[2], ns->value[3])) / ns->n_packets);
            break;
        case 5:
...
PERFMON_REGISTER_BUNDLE(intel_core_cache_miss_hit) = {
    ...
    .column_headers = PERFMON_STRINGS("L1 hit/pkt", "L1 miss/pkt", "L2 hit/pkt", "L2 miss/pkt", "L3 hit/pkt", "L3 miss/pkt"),
};
