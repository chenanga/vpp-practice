...
static u32 get_sad_ctrl_cfg(vlib_pci_addr_t *addr)
{
    int fd = 0;
    u32 value;
    u8 *dev_node_name = format(0, "%s/%02x/%02x.%x", procfs_pci_path, addr->bus, addr->slot, addr->function);

    fd = open((char *) dev_node_name, O_RDWR);
    if (fd < 0) return -1;

    if (pread(fd, &value, sizeof(u32), SNR_ICX_SAD_CONTROL_CFG_OFFSET) < sizeof(u32)) value = -1;
...
static u64 get_bus_to_sad_mappings(vlib_main_t *vm, index_t **ph, iio_uncore_sad_t **pp)
{
    index_t          *h = 0;
    ...
    vec_foreach (addr, addrs) {
        ...
        if (d->vendor_id == PCM_INTEL_PCI_VENDOR_ID && d->device_id == SNR_ICX_MESH2IIO_MMAP_DID) {
...
u8 *format_stack_socket(u8 *s, va_list *va)
{
...
static clib_error_t *init_intel_uncore_iio_bw(vlib_main_t *vm, struct perfmon_bundle *b)
{
    index_t          *h    = 0;
    ...
    s = format(0, "%-10s%-5s%-13s%-12s%-14s%-16s%s\n", "Stack", "Port", "Address", "VID:PID", "Link Speed", "Driver", "Product Name");
    ...
    vec_foreach (addr, addrs) {
        ...
        if (d->device_class != PCI_CLASS_NETWORK_ETHERNET) continue;

        s = format(s,
                   "%-10U%-5U%-13U%04x:%04x   %-14U%-16s%v\n",
                   format_stack_socket,
                   p,
                   h,
                   addr,
...
static u8 *format_intel_uncore_iio_bw(u8 *s, va_list *args)
{
...
static perfmon_cpu_supports_t iio_bw_cpu_supports[] = {{is_icelake, PERFMON_BUNDLE_TYPE_SYSTEM}};
...
PERFMON_REGISTER_BUNDLE(intel_uncore_iio_bw_pci) = {
    ...
    .column_headers = PERFMON_STRINGS("RunTime", "PCIe Rd/P0", "PCIe Wr/P0", "PCIe Rd/P2", "PCIe Wr/P2")};

PERFMON_REGISTER_BUNDLE(intel_uncore_iio_bw_cpu) = {.name           = "iio-bandwidth-cpu",
                                                    ...
                                                    .format_fn      = format_intel_uncore_iio_bw,
                                                    .init_fn        = init_intel_uncore_iio_bw,
                                                    .column_headers = PERFMON_STRINGS("RunTime", "CPU Rd/P0", "CPU Wr/P0", "CPU Rd/P2", "CPU Wr/P2")};
