...
static u8 *format_intel_backend_bound_core(u8 *s, va_list *args)
{
    ...
    switch (row) {
        ...
        case 5:
            ...
            break;
        case 6:
            sv = (ss->value[PORT4_9] + ss->value[PORT7_8]) / (f64) (4 * ss->value[DISTRIBUTED]);
            break;
    }
...
PERFMON_REGISTER_BUNDLE(intel_core_backend_bound_core) = {
    ...
    .column_headers = PERFMON_STRINGS("Clocks/Packet", "%Port0", "%Port1", "%Port5", "%Port6", "%Load", "%Store"),
};
