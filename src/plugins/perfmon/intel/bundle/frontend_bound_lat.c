...
static u8 *format_intel_frontend_bound_lat(u8 *s, va_list *args)
{
    ...
    switch (row) {
        ...
        case 2:
            ...
            break;
        case 3:
            sv = (ss->value[RESTEER] + (ss->value[BACLEARS] * BA_Clear_Cost)) / cycles;
            break;
        case 4:
...
PERFMON_REGISTER_BUNDLE(intel_core_frontend_bound_lat) = {
    ...
    .description =
        "Topdown FrontEnd-bound Latency - % cycles not retiring uops "
    ...
    .column_headers = PERFMON_STRINGS("Clocks/Packet", "% iCache Miss", "% DSB Switch", "% Branch Resteer", "% MS Switch"),
    .footer =
