...
static u8 *format_intel_uncore_imc_bw(u8 *s, va_list *args)
{
    ...
    switch (col) {
        case 0:
            ...
            break;
        case 1:
            if (r->time_running) s = format(s, "%9.2f", (f64) r->value[0] * 64 * 1e-6 / tr);
            break;
        case 2:
            if (r->time_running) s = format(s, "%9.2f", (f64) r->value[1] * 64 * 1e-6 / tr);
            break;
        case 3:
            if (r->time_running) s = format(s, "%9.2f", (f64) (r->value[0] + r->value[1]) * 64 * 1e-6 / tr);
            break;
        default:
            break;
    }
...
PERFMON_REGISTER_BUNDLE(intel_uncore_imc_bw) = {
    ...
    .column_headers = PERFMON_STRINGS("RunTime", "Reads (MB/s)", "Writes (MB/s)", "Total (MB/s)"),
};
