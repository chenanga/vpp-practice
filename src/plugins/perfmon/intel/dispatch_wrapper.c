...
static_always_inline void perfmon_read_pmcs(u64 *counters, u32 *indexes, u8 n_counters)
{
    for (int i = 0; i < n_counters; i++)
        counters[i] = _rdpmc(indexes[i] - 1);
}

static_always_inline uword perfmon_dispatch_wrapper_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 n_events)
{
    perfmon_main_t           *pm = &perfmon_main;
    perfmon_thread_runtime_t *rt = vec_elt_at_index(pm->thread_runtimes, vm->thread_index);
    perfmon_node_stats_t     *s  = vec_elt_at_index(rt->node_stats, node->node_index);
...
static_always_inline u32 perfmon_mmap_read_index(const struct perf_event_mmap_page *mmap_page)
{
    u32 idx;
    u32 seq;
...
static_always_inline clib_error_t *read_mmap_indexes(perfmon_bundle_t *b)
{
    perfmon_main_t *pm = &perfmon_main;
...
clib_error_t *intel_config_dispatch_wrapper(perfmon_bundle_t *b, vlib_node_function_t **dispatch_wrapper)
{
    clib_error_t *err = 0;
    if ((err = read_mmap_indexes(b)) != 0) return err;

    (*dispatch_wrapper) = perfmon_dispatch_wrappers[b->n_events];
    return 0;
}

#define foreach_n_events _(1) _(2) _(3) _(4) _(5) _(6) _(7) _(8) _(9) _(10) _(11) _(12)

#define _(x)                                                                                                  \
    static uword perfmon_dispatch_wrapper##x(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame) \
    {                                                                                                         \
