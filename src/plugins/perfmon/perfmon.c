...
void perfmon_reset(vlib_main_t *vm)
{
    ...
    for (int i = 0; i < vec_len(pm->thread_runtimes); i++) {
        ...
        for (int j = 0; j < PERF_MAX_EVENTS; j++)
            if (tr->mmap_pages[j]) munmap(tr->mmap_pages[j], page_size);
    }
...
static clib_error_t *perfmon_set(vlib_main_t *vm, perfmon_bundle_t *b)
{
    ...
    if (b->active_type == PERFMON_BUNDLE_TYPE_NODE) is_node = 1;
    ...
    if (s->instances_by_type == 0) {
        ...
        if (is_node) vec_validate(pm->thread_runtimes, vlib_get_n_threads() - 1);
    }
    else {
        ...
        if (e->type_from_instance) {
            ...
            for (int i = 1; i < b->n_events; i++) {
                ...
                ASSERT(e->type_from_instance == 1 && e->instance_type == instance_type);
            }
        }
    ...
    for (int i = 0; i < vec_len(it->instances); i++) {
        ...
        perf_event_open:
            log_debug(
                ...
                in->cpu,
                pm->group_fds[i]);
            fd = syscall(__NR_perf_event_open, &pe, in->pid, in->cpu, pm->group_fds[i], 0);

            if (fd == -1) {
                if (errno == EOPNOTSUPP) /* 64b counters not supported on aarch64 */
                {
            ...
            if (pm->group_fds[i] == -1) pm->group_fds[i] = fd;

            if (is_node) {
                perfmon_thread_runtime_t *tr;
                tr                              = vec_elt_at_index(pm->thread_runtimes, i);
                tr->mmap_pages[n_events_opened] = mmap(0, page_size, PROT_READ, MAP_SHARED, fd, 0);
        ...
        if (is_node && n_events_opened) {
            ...
            vec_validate_aligned(rt->node_stats, n_nodes - 1, CLIB_CACHE_LINE_BYTES);
        }
    }
...
clib_error_t *perfmon_start(vlib_main_t *vm, perfmon_bundle_t *b)
{
    ...
    if ((err = perfmon_set(vm, b)) != 0) return err;

    n_groups = vec_len(pm->group_fds);

    for (int i = 0; i < n_groups; i++) {
        if (ioctl(pm->group_fds[i], PERF_EVENT_IOC_ENABLE, PERF_IOC_FLAG_GROUP) == -1) {
            perfmon_reset(vm);
    ...
    if (b->active_type == PERFMON_BUNDLE_TYPE_NODE) {
        ...
        for (int i = 0; i < vlib_get_n_threads(); i++)
            vlib_node_set_dispatch_wrapper(vlib_get_main_by_index(i), dispatch_wrapper);
    }
...
clib_error_t *perfmon_stop(vlib_main_t *vm)
{
    ...
    if (pm->is_running != 1) return clib_error_return(0, "not running");
    ...
    for (int i = 0; i < n_groups; i++) {
        if (ioctl(pm->group_fds[i], PERF_EVENT_IOC_DISABLE, PERF_IOC_FLAG_GROUP) == -1) {
            perfmon_reset(vm);
...
static clib_error_t *perfmon_init(vlib_main_t *vm)
{
    ...
    while (s) {
        clib_error_t *err;
        if (hash_get_mem(pm->source_by_name, s->name) != 0) clib_panic("duplicate source name '%s'", s->name);
        if (s->init_fn && ((err = (s->init_fn)(vm, s)))) {
            log_warn("skipping source '%s' - %U", s->name, format_clib_error, err);
            clib_error_free(err);
            s = s->next;
            continue;
        }
    ...
    while (b) {
        ...
        if ((p = hash_get_mem(pm->source_by_name, b->source)) == 0) {
            log_debug("missing source '%s', skipping bundle '%s'", b->source, b->name);
            b = b->next;
            continue;
        }
        ...
        if (b->init_fn && ((err = (b->init_fn)(vm, b)))) {
            log_warn("skipping bundle '%s' - %U", b->name, format_clib_error, err);
            clib_error_free(err);
            b = b->next;
            continue;
        }
