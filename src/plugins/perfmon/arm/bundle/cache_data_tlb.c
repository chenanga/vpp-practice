...
enum { L1D_TLB, L1D_TLB_REFILL, L2D_TLB, L2D_TLB_REFILL };

static u8 *format_arm_cache_data_tlb(u8 *s, va_list *args)
{
    ...
    switch (row) {
        ...
        case 2:
            s = format(s, "%.2f%%", (ns->value[L1D_TLB] ? (f64) ns->value[L1D_TLB_REFILL] / ns->value[L1D_TLB] * 100 : 0));
            break;
        ...
        case 5:
            s = format(s, "%.2f%%", (ns->value[L2D_TLB] ? (f64) ns->value[L2D_TLB_REFILL] / ns->value[L2D_TLB] * 100 : 0));
            break;
...
PERFMON_REGISTER_BUNDLE(arm_cache_data_tlb) = {
    ...
    .column_headers = PERFMON_STRINGS("L1D-TLB: access", "refill", "\%*", "L2D-TLB: access", "refill", "\%*", "pkts"),
    /*
     * set a bit for every event used in each column
     * this allows us to disable columns at bundle registration if an
     * event is not supported
     */
    .column_events = PERFMON_COLUMN_EVENTS(SET_BIT(L1D_TLB),
                                           SET_BIT(L1D_TLB_REFILL),
                                           SET_BIT(L1D_TLB) | SET_BIT(L1D_TLB_REFILL),
                                           SET_BIT(L2D_TLB),
                                           SET_BIT(L2D_TLB_REFILL),
                                           SET_BIT(L2D_TLB) | SET_BIT(L2D_TLB_REFILL),
                                           0),
    .footer =
