...
enum { BR_RETIRED, BR_MIS_PRED_RETIRED, BR_PRED, BR_MIS_PRED };

static u8 *format_arm_branch_pred(u8 *s, va_list *args)
{
    ...
    switch (row) {
        ...
        case 2:
            s = format(s, "%.2f", (f64) ns->value[BR_MIS_PRED_RETIRED] / ns->n_calls);
            break;

        case 3:
            s = format(s, "%.2f", (f64) ns->value[BR_MIS_PRED_RETIRED] / ns->n_packets);
            break;

        case 4:
            s = format(s, "%.2f%%", (ns->value[BR_RETIRED] ? (f64) ns->value[BR_MIS_PRED_RETIRED] / ns->value[BR_RETIRED] * 100 : 0));
            break;
        ...
        case 9:
            s = format(s, "%.2f%%", (ns->value[BR_PRED] ? (f64) ns->value[BR_MIS_PRED] / ns->value[BR_PRED] * 100 : 0));
            break;
...
PERFMON_REGISTER_BUNDLE(arm_branch_pred) = {
    ...
    .column_headers = PERFMON_STRINGS("[1.1]", "[1.2]", "[1.3]", "[1.4]", "\%", "[2.1]", "[2.2]", "[2.3]", "[2.4]", "\%", "pkts"),
    /*
     * set a bit for every event used in each column
     * this allows us to disable columns at bundle registration if an
     * event is not supported
     */
    .column_events = PERFMON_COLUMN_EVENTS(SET_BIT(BR_RETIRED),
                                           SET_BIT(BR_RETIRED),
                                           SET_BIT(BR_MIS_PRED_RETIRED),
                                           SET_BIT(BR_MIS_PRED_RETIRED),
                                           SET_BIT(BR_RETIRED) | SET_BIT(BR_MIS_PRED_RETIRED),
                                           SET_BIT(BR_PRED),
                                           SET_BIT(BR_PRED),
                                           SET_BIT(BR_MIS_PRED),
                                           SET_BIT(BR_MIS_PRED),
                                           SET_BIT(BR_PRED) | SET_BIT(BR_MIS_PRED),
                                           0),
    .footer =
