...
static u8 *format_arm_cache_inst_tlb(u8 *s, va_list *args)
{
    ...
    switch (row) {
        ...
        case 2:
            s = format(s, "%.2f%%", (ns->value[L1I_TLB] ? (f64) ns->value[L1I_TLB_REFILL] / ns->value[L1I_TLB] * 100 : 0));
            break;
        ...
        case 5:
            s = format(s, "%.2f%%", (ns->value[L2I_TLB] ? (f64) ns->value[L2I_TLB_REFILL] / ns->value[L2I_TLB] * 100 : 0));
            break;
...
PERFMON_REGISTER_BUNDLE(arm_cache_inst_tlb) = {
    .name           = "cache-inst-tlb",
    .description    = "L1/L2 instruction TLB cache accesses, refills, walks per packet",
    .source         = "arm",
    ...
    .column_headers = PERFMON_STRINGS("L1I-TLB: access", "refill", "\%*", "L2I-TLB: access", "refill", "\%*", "pkts"),
    /*
     * set a bit for every event used in each column
     * this allows us to disable columns at bundle registration if an
     * event is not supported
     */
    .column_events = PERFMON_COLUMN_EVENTS(SET_BIT(L1I_TLB),
                                           SET_BIT(L1I_TLB_REFILL),
                                           SET_BIT(L1I_TLB) | SET_BIT(L1I_TLB_REFILL),
                                           SET_BIT(L2I_TLB),
                                           SET_BIT(L2I_TLB_REFILL),
                                           SET_BIT(L2I_TLB) | SET_BIT(L2I_TLB_REFILL),
                                           0),
    .footer =
