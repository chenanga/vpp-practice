...
enum { L1I_CACHE, L1I_CACHE_REFILL, L2I_CACHE, L2I_CACHE_REFILL };

static u8 *format_arm_cache_inst(u8 *s, va_list *args)
{
    ...
    switch (row) {
        ...
        case 1:
            s = format(s, "%.2f", (f64) ns->value[L1I_CACHE_REFILL] / ns->n_packets);
            break;

        case 2:
            s = format(s, "%.2f%%", (ns->value[L1I_CACHE] ? (f64) ns->value[L1I_CACHE_REFILL] / ns->value[L1I_CACHE] * 100 : 0));
            break;
        ...
        case 4:
            s = format(s, "%.2f", (f64) ns->value[L2I_CACHE_REFILL] / ns->n_packets);
            break;

        case 5:
            s = format(s, "%.2f%%", (ns->value[L2I_CACHE] ? (f64) ns->value[L2I_CACHE_REFILL] / ns->value[L2I_CACHE] * 100 : 0));
            break;
...
PERFMON_REGISTER_BUNDLE(arm_cache_inst) = {.name           = "cache-inst",
                                           ...
                                           .column_headers = PERFMON_STRINGS("L1I: access", "refill", "\%*", "L2I: access", "refill", "\%*", "pkts"),
                                           /*
                                            * set a bit for every event used in each column
                                            * this allows us to disable columns at bundle registration if an
                                            * event is not supported
                                            */
                                           .column_events = PERFMON_COLUMN_EVENTS(SET_BIT(L1I_CACHE),
                                                                                  SET_BIT(L1I_CACHE_REFILL),
