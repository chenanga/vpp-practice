...
enum { L1D_CACHE, L1D_CACHE_REFILL, L2D_CACHE, L2D_CACHE_REFILL, L3D_CACHE, L3D_CACHE_REFILL };

static u8 *format_arm_cache_data(u8 *s, va_list *args)
{
    ...
    switch (row) {
        ...
        case 1:
            s = format(s, "%.2f", (f64) ns->value[L1D_CACHE_REFILL] / ns->n_packets);
            break;

        case 2:
            s = format(s, "%.2f%%", (ns->value[L1D_CACHE] ? (f64) ns->value[L1D_CACHE_REFILL] / ns->value[L1D_CACHE] * 100 : 0));
            break;
        ...
        case 4:
            s = format(s, "%.2f", (f64) ns->value[L2D_CACHE_REFILL] / ns->n_packets);
            break;

        case 5:
            s = format(s, "%.2f%%", (ns->value[L2D_CACHE] ? (f64) ns->value[L2D_CACHE_REFILL] / ns->value[L2D_CACHE] * 100 : 0));
            break;
        ...
        case 7:
            s = format(s, "%.2f", (f64) ns->value[L3D_CACHE_REFILL] / ns->n_packets);
            break;

        case 8:
            s = format(s, "%.2f%%", (ns->value[L3D_CACHE] ? (f64) ns->value[L3D_CACHE_REFILL] / ns->value[L3D_CACHE] * 100 : 0));
            break;
...
PERFMON_REGISTER_BUNDLE(arm_cache_data) = {
    ...
    .column_headers = PERFMON_STRINGS("L1D: access", "refill", "\%*", "L2D: access", "refill", "\%*", "L3D: access", "refill", "\%*", "pkts"),
    /*
     * set a bit for every event used in each column
     * this allows us to disable columns at bundle registration if an
     * event is not supported
     */
    .column_events = PERFMON_COLUMN_EVENTS(SET_BIT(L1D_CACHE),
                                           SET_BIT(L1D_CACHE_REFILL),
                                           SET_BIT(L1D_CACHE) | SET_BIT(L1D_CACHE_REFILL),
                                           SET_BIT(L2D_CACHE),
                                           SET_BIT(L2D_CACHE_REFILL),
                                           SET_BIT(L2D_CACHE) | SET_BIT(L2D_CACHE_REFILL),
                                           SET_BIT(L3D_CACHE),
                                           SET_BIT(L3D_CACHE_REFILL),
                                           SET_BIT(L3D_CACHE) | SET_BIT(L3D_CACHE_REFILL),
                                           0),
    .footer =
        "all stats are per packet except refill rate (\%)\n"
