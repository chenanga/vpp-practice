...
enum { MEM_ACCESS, BUS_ACCESS, MEMORY_ERROR };

static u8 *format_arm_memory_access(u8 *s, va_list *args)
{
...
PERFMON_REGISTER_BUNDLE(arm_memory_access) = {
    ...
    .column_headers = PERFMON_STRINGS("Mem-access/pkt", "Bus-access/pkt", "Total-mem-errors", "pkts"),
    /*
     * set a bit for every event used in each column
     * this allows us to disable columns at bundle registration if an
     * event is not supported
     */
    .column_events = PERFMON_COLUMN_EVENTS(SET_BIT(MEM_ACCESS), SET_BIT(BUS_ACCESS), SET_BIT(MEMORY_ERROR), 0),
    .footer =
