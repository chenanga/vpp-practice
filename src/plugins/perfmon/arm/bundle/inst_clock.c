...
enum { CPU_CYCLES, INST_RETIRED };

static u8 *format_arm_inst_clock(u8 *s, va_list *args)
{
    ...
    switch (row) {
        ...
        case 4:
            s = format(s, "%.2f", (f64) ns->n_packets / ns->n_calls); /* Packets/Call */
            break;

        case 5:
            s = format(s, "%.2f", (f64) ns->value[0] / ns->n_packets); /* Clocks/Packet */
            break;

        case 6:
            s = format(s, "%.2f", (f64) ns->value[1] / ns->n_packets); /* Instructions/Packet */
            break;
...
PERFMON_REGISTER_BUNDLE(arm_inst_clock) = {
    .name           = "inst-and-clock",
    .description    = "CPU cycles, instructions, instructions/packet, cycles/packet and IPC",
    .source         = "arm",
    ...
    .column_headers = PERFMON_STRINGS("Packets", "Calls", "CPU Cycles", "Inst*", "Pkts/Call", "Cycles/Pkt", "Inst/Pkt", "IPC"),
    /*
     * set a bit for every event used in each column
     * this allows us to disable columns at bundle registration if an
     * event is not supported
     */
    .column_events = PERFMON_COLUMN_EVENTS(0,
                                           0,
                                           SET_BIT(CPU_CYCLES),
                                           SET_BIT(INST_RETIRED),
                                           0,
                                           SET_BIT(CPU_CYCLES),
                                           SET_BIT(INST_RETIRED),
                                           SET_BIT(CPU_CYCLES) | SET_BIT(INST_RETIRED)),
    .footer =
        "* Instructions retired: the counter increments for every "
