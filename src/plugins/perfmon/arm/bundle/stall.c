...
enum { STALL_BACKEND, STALL_FRONTEND };

static u8 *format_arm_stall(u8 *s, va_list *args)
{
...
PERFMON_REGISTER_BUNDLE(arm_stall) = {
    ...
    .column_headers = PERFMON_STRINGS("Backend/pkt", "Frontend/pkt", "Backend/call", "Frontend/call", "packets", "calls"),
    /*
     * set a bit for every event used in each column
     * this allows us to disable columns at bundle registration if an
     * event is not supported
     */
    .column_events = PERFMON_COLUMN_EVENTS(SET_BIT(STALL_BACKEND), SET_BIT(STALL_FRONTEND), SET_BIT(STALL_BACKEND), SET_BIT(STALL_FRONTEND), 0, 0),
    .footer =
