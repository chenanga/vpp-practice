...
static_always_inline u64 get_pmc_register(u32 pmc_idx)
{
    u64 value = 0;
    if (pmc_idx == 31) /* i.e. CPU Cycle event code 0x11 - need to read via pmccntr_el0 */
...
static_always_inline u64 read_pmc_from_mmap(struct perf_event_mmap_page *pc)
{
    u32 seq, idx, width;
    u64 offset = 0;
    s64 pmc    = 0;
...
static_always_inline void perfmon_read_pmcs(u64 *counters, perfmon_thread_runtime_t *rt, u8 n_counters)
{
    switch (n_counters) {
        default:
        case 7:
...
uword perfmon_dispatch_wrapper(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
clib_error_t *arm_config_dispatch_wrapper(perfmon_bundle_t *b, vlib_node_function_t **dispatch_wrapper)
{
