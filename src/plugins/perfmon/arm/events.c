...
u8 *format_arm_config(u8 *s, va_list *args)
{
...
static clib_error_t *arm_init(vlib_main_t *vm, perfmon_source_t *src)
{
    clib_error_t *err;
    ...
    */
    ...
    const char    *event_path = "/sys/bus/event_source/devices/armv8_pmuv3_0/events";
    DIR           *event_dir  = opendir(event_path);

    if (event_dir == NULL) {
        err = clib_error_return_unix(0, "error listing directory: %s", event_path);
    ...
    while ((dir_entry = readdir(event_dir)) != NULL) {
        if (dir_entry->d_name[0] != '.') {
            ...
            if (unformat(&input, "event=0x%x", &config)) {
                ...
                log_debug("found supported event in sysfs: %s \'%s\' 0x%x", dir_entry->d_name, tmpstr, config);
            }
            else {
                err = clib_error_create("error parsing event: %s %s", dir_entry->d_name, tmpstr);
                log_err("%U", format_clib_error, err);
                continue;
            }
        }
    ...
    for (int i = 0; i < ARRAY_LEN(events); i++) {
        if (clib_bitmap_get(bitmap, events[i].config)) events[i].implemented = 1;
    }
...
u8 arm_bundle_supported(perfmon_bundle_t *b)
{
    clib_bitmap_alloc(b->event_disabled, b->n_events);
    for (u32 i = 0; i < b->n_events; i++) {
        perfmon_event_t *e = b->src->events + b->events[i];
        if (!e->implemented) {
            log_debug("bundle \'%s\': perf event %s is not implemented on this CPU", b->name, e->name);
    ...
    if (b->column_events) {
        ...
        clib_bitmap_foreach (disabled_event, b->event_disabled) {
            for (u32 j = 0; j < b->n_columns; j++) {
                if (clib_bitmap_get(b->column_disabled, j)) continue;
                if (GET_BIT(b->column_events[j], disabled_event)) {
                    clib_bitmap_set(b->column_disabled, j, 1);
                    log_debug("bundle \'%s\': disabling column %d as event unsupported", b->name, j);
                }
            }
        }
    }
