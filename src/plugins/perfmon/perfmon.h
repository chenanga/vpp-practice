...
typedef clib_error_t *(perfmon_source_config_dispatch_wrapper_t) (perfmon_bundle_t *b, vlib_node_function_t **dispatch_wrapper);
...
#define PERFMON_BUNDLE_TYPE_TO_FLAGS(type)                                           \
    ({                                                                               \
        ...
        if (type == PERFMON_BUNDLE_TYPE_NODE_OR_THREAD)                              \
            rtype = 1 << PERFMON_BUNDLE_TYPE_THREAD | 1 << PERFMON_BUNDLE_TYPE_NODE; \
        else                                                                         \
            rtype = 1 << type;                                                       \
        rtype;                                                                       \
    })

always_inline uword perfmon_cpu_update_bundle_type(perfmon_bundle_t *b)
{
    ...
    if (!supports)
        ...
        for (int i = 0; i < b->n_cpu_supports; ++i)
            if (supports[i].cpu_supports()) type |= PERFMON_BUNDLE_TYPE_TO_FLAGS(supports[i].bundle_type);
    }
...
#define PERFMON_REGISTER_BUNDLE(x)                                                               \
    ...
    {                                                                                            \
        ...
        __perfmon_bundle_##x.type_flags = perfmon_cpu_update_bundle_type(&__perfmon_bundle_##x); \
