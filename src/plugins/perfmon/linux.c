...
static u8 *format_context_switches(u8 *s, va_list *args)
{
    ...
    switch (row) {
        case 0:
            ...
            break;
        case 1:
            if (r->time_running) s = format(s, "%9.2f", (f64) r->value[0] / t);
            break;
    }
    return s;
}
...
static u8 *format_page_faults(u8 *s, va_list *args)
{
    ...
    switch (row) {
        ...
        case 1:
            ...
            break;
        case 2:
            if (r->time_running) s = format(s, "%9.2f", (f64) r->value[1] / t);
            break;
    }
    return s;
}
...
PERFMON_REGISTER_BUNDLE(page_faults) = {
    ...
    .column_headers = PERFMON_STRINGS("RunTime", "MinorPageFaults/Sec", "MajorPageFaults/Sec"),
};
