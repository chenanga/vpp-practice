...
static inline u32 srtp_ctx_alloc_w_thread(u32 thread_index)
{
    srtp_tc_t *ctx;
    pool_get_aligned_safe(srtp_main.ctx_pool[thread_index], ctx, CLIB_CACHE_LINE_BYTES);
...
static inline srtp_tc_t *srtp_ctx_get_w_thread(u32 ctx_index, u32 thread_index)
{
...
{
    if (!ctx->is_migrated) srtp_ctx_free_policy(ctx);
    pool_put(srtp_main.ctx_pool[ctx->c_thread_index], ctx);
}

static inline u32 srtp_ctx_attach(u32 thread_index, void *ctx_ptr)
{
    srtp_tc_t *ctx;
...
static inline void *srtp_ctx_detach(srtp_tc_t *ctx)
{
    srtp_tc_t *sc;
...
static int srtp_ctx_init_client(srtp_tc_t *ctx)
{
    ...
    app_session->session_type     = session_type_from_proto_and_ip(TRANSPORT_PROTO_SRTP, ctx->udp_is_ip4);

    if ((err = app_worker_init_connected(app_wrk, app_session))) goto failed;

    app_session->session_state = SESSION_STATE_READY;
    if (app_worker_connect_notify(app_wrk, app_session, SESSION_E_NONE, ctx->parent_app_api_context)) {
...
static int srtp_ctx_init_server(srtp_tc_t *ctx)
{
    ...
    if (srtp_create(&ctx->srtp_ctx, ctx->srtp_policy) != srtp_err_status_ok) return -1;
...
{
    if (srtp_dealloc(ctx->srtp_ctx)) SRTP_DBG(0, "%u failed to cleanup srtp state", ctx->c_c_index);
    return 0;
}

static inline int srtp_ctx_write(srtp_tc_t *ctx, session_t *app_session, transport_send_params_t *sp)
{
    ...
    while (to_deq > 0) {
        ...
        rv = app_send_dgram_raw(us->tx_fifo, &at, mq, (u8 *) buf, len, SESSION_IO_EVT_TX, 1 /* do_evt */, 0 /* noblock */);
        ASSERT(rv == len);
    ...
    if (svm_fifo_needs_deq_ntf(app_session->tx_fifo, n_wrote)) session_dequeue_notify(app_session);

    if (n_wrote) {
        if (svm_fifo_set_event(us->tx_fifo)) session_send_io_evt_to_thread(us->tx_fifo, SESSION_IO_EVT_TX);
    }

    if (PREDICT_FALSE(ctx->app_closed && !svm_fifo_max_enqueue_prod(us->rx_fifo))) {
...
void srtp_notify_app_enqueue(srtp_tc_t *ctx, session_t *app_session)
{
    ...
    if (PREDICT_TRUE(app_wrk != 0)) app_worker_rx_notify(app_wrk, app_session);
}

static inline int srtp_ctx_read(srtp_tc_t *ctx, session_t *us)
{
    app_session_transport_t at;
    ...
    while (svm_fifo_max_dequeue_cons(us->rx_fifo) > 0) {
        ...
        rv = app_recv_dgram_raw(us->rx_fifo, (u8 *) buf, 2000, &at, 0 /* clear evt */, 0 /* peek */);
        ASSERT(rv > 0);
        len = rv;
...
int srtp_session_accept_callback(session_t *us)
{
    ...
    clib_memcpy(ctx->srtp_policy[0].key, lctx->srtp_policy[0].key, SRTP_MAX_KEYLEN);
    ctx->srtp_policy[1].key = clib_mem_alloc(SRTP_MAX_KEYLEN);
    clib_memcpy(ctx->srtp_policy[1].key, lctx->srtp_policy[1].key, SRTP_MAX_KEYLEN);

    app_session                = session_alloc(ctx->c_thread_index);
    app_session->session_state = SESSION_STATE_CREATED;
    ctx->c_s_index             = app_session->session_index;

    SRTP_DBG(1, "Accept on listener %u new connection [%u]%x", srtp_listener->opaque, vlib_get_thread_index(), ctx_handle);
...
static void srtp_app_session_cleanup(session_t *s, session_cleanup_ntf_t ntf)
{
    ...
    if (!ctx->no_app_session) session_transport_delete_notify(&ctx->connection);
    srtp_ctx_deinit(ctx);
    srtp_ctx_free(ctx);
}
...
static void srtp_migrate_ctx(void *arg)
{
    ...
    session_dgram_connect_notify(&ctx->connection, old_thread_index, &new_app_session);
...
static void srtp_session_migrate_callback(session_t *us, session_handle_t new_sh)
{
    ...
    SRTP_DBG(1, "ctx %u attached to udp %x session migrating", cloned_ctx->c_c_index, new_sh);

    session_send_rpc_evt_to_thread(new_thread, srtp_migrate_ctx, (void *) cloned_ctx);
...
static clib_error_t *srtp_enable(vlib_main_t *vm, u8 is_en)
{
    ...
    if (vnet_application_attach(a)) return clib_error_return(0, "failed to attach srtp app");
...
static void srtp_disconnect_transport(srtp_tc_t *ctx)
{
    ...
    if (vnet_disconnect_session(&a)) SRTP_DBG(0, "disconnect returned");
}
...
static u32 srtp_start_listen(u32 app_listener_index, transport_endpoint_cfg_t *tep)
{
...
u32 srtp_stop_listen(u32 lctx_index)
{
    ...
    if ((rv = vnet_unlisten(&a))) SRTP_DBG(0, "unlisten returned %d", rv);
...
int srtp_custom_tx_callback(void *session, transport_send_params_t *sp)
{
    ...
    if (PREDICT_FALSE(app_session->session_state >= SESSION_STATE_TRANSPORT_CLOSED)) return 0;

    ctx = srtp_ctx_get_w_thread(app_session->connection_index, app_session->thread_index);
...
static u8 *format_srtp_listener_ctx(u8 *s, va_list *args)
{
    ...
    s            = format(s,
               ...
               ctx->c_s_index,
               ctx->parent_app_wrk_index,
               udp_listener->thread_index,
               udp_listener->session_index);
...
u8 *format_srtp_listener(u8 *s, va_list *args)
{
    ...
    if (verbose) s = format(s, "%-" SESSION_CLI_STATE_LEN "U", format_srtp_ctx_state, ctx);
    return s;
}
...
{
    transport_register_protocol(TRANSPORT_PROTO_SRTP, &srtp_proto, FIB_PROTOCOL_IP4, ~0);
    transport_register_protocol(TRANSPORT_PROTO_SRTP, &srtp_proto, FIB_PROTOCOL_IP6, ~0);
    return 0;
}
