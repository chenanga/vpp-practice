...
static clib_error_t *test_lisp_msg_push_ecm()
{
    ...
    b->current_data   = sizeof(udp_header_t) + sizeof(ip4_header_t) + sizeof(ecm_hdr_t) + 1;
...
static clib_error_t *test_lisp_msg_parse_mapping_record()
{
    ...
    clib_mem_free(b);
    if (locs) vec_free(locs);
    return error;
}

static map_request_hdr_t *build_map_request(lisp_cp_main_t *lcm, vlib_buffer_t *b, gid_address_t *rlocs)
{
    ...
    h = lisp_msg_put_mreq(lcm, b, seid, deid, rlocs, is_smr_invoked, rloc_probe_set, &nonce);
    vec_free(rlocs);
    return h;
}

static void generate_rlocs(gid_address_t **rlocs, u32 *count)
{
...
static clib_error_t *test_lisp_msg_put_mreq_with_lcaf()
{
    ...
    gid_address_t g = {.type = GID_ADDR_IP_PREFIX, .ippref = ippref, .vni = 0x90919293, .vni_mask = 0x17};
    vec_add1(rlocs, g);
...
static mapping_t *build_test_map_records()
{
    mapping_t *records = 0;

    mapping_t r = {.ttl = MAP_REGISTER_DEFAULT_TTL, .eid = {.type = GID_ADDR_MAC, .mac = {1, 2, 3, 4, 5, 6}, .vni = 0x0}};

    locator_t loc = {.weight   = 1,
                     .priority = 2,
                     .local    = 1,
                     .address  = {.type = GID_ADDR_IP_PREFIX, .ippref = {.addr = {.ip.ip4.as_u32 = 0x99887766, .version = AF_IP4}}}};
...
static void free_test_map_records(mapping_t *maps)
{
    mapping_t *map;
...
static clib_error_t *test_lisp_map_register()
{
    vlib_buffer_t *b;
    clib_error_t  *error = 0;
    u64            nonce;
    ...
    lisp_msg_put_map_register(b, records, 1 /* want map notify */, 20 /* length of HMAC_SHA_1_96 */, &nonce, &msg_len);
    ...
    u8 expected_data[] = {
        ...
        0x00,
        0x01,
        0x01, /* type; rsvd; want notify; REC count */
        0x00,
        0x00,
        0x00,
        0x00,
        ...
        0x01,
        0x51,
        0x80, /* default ttl (24h = 86400s) */
        0x01,
        0x00,
        0x00,
        0x00, /* loc count, eid len, ACT, A */
        0x00,
        0x00,
        0x40,
        0x05, /* rsvd, map ver num, AFI = MAC */
        0x01,
        0x02,
        0x03,
        0x04,
        ...
        0x01,
        0x00,
        0x00, /* prio, weight, mprio, mweight */
        0x00,
        0x04,
        0x00,
        0x01, /* flags, AFI = ipv4 */
...
static vlib_buffer_t *create_buffer(u8 *data, u32 data_len)
{
    vlib_buffer_t *b;
...
static clib_error_t *test_lisp_parse_map_reply()
{
    clib_error_t *error = 0;
...
static clib_error_t *test_lisp_parse_lcaf()
{
    int            i;
    clib_error_t  *error = 0;
    gid_address_t  eid;
    ...
    u8 map_reply_records[] = {
        ...
        0x18, /* IID mask-len */
        0x00,
        0x16, /* iid length + next AFI lenght */
        /* LCAF Instance ID */
        0x22,
        0x44,
        0x66,
...
static int lisp_cp_serdes_tests(vlib_main_t *vm, unformat_input_t *input)
{
    clib_error_t *error;
...
static clib_error_t *test_locator_type(void)
{
    ...
    /* local locator */
    locator_t loc1, loc2 = {.local = 1, .state = 2, .sw_if_index = 8, .priority = 3, .weight = 100, .mpriority = 4, .mweight = 101};
    ...
    gid_address_t nested_gid          = {.type = GID_ADDR_IP_PREFIX, .ippref = nested_ippref};

    lcaf_t lcaf           = {.type = LCAF_INSTANCE_ID, .uni = {.vni_mask_len = 5, .vni = 0xa1b2c3d4, .gid_addr = &nested_gid}};
...
static clib_error_t *test_gid_write_nsh(void)
{
    clib_error_t *error = 0;
...
static clib_error_t *test_gid_parse_nsh()
{
    clib_error_t *error = 0;
    gid_address_t _gid_addr, *gid_addr  = &_gid_addr;
    gid_address_t _gid_addr_copy, *copy = &_gid_addr_copy;
...
static clib_error_t *test_gid_parse_lcaf()
{
    ...
    u8 data[] = {
        ...
        0x18, /* IID mask-len */
        0x00,
        0x0a, /* iid length + next AFI lenght */
        /* LCAF Instance ID */
        0x00,
        0x00,
        0x00,
...
static clib_error_t *test_src_dst_with_vni_serdes(void)
{
    clib_error_t *error = 0;
    u8           *b     = clib_mem_alloc(500);
    clib_memset(b, 0, 500);

    fid_address_t src = {.type = FID_ADDR_IP_PREF, .ippref = {.len = 24, .addr = {.version = AF_IP4, .ip.ip4.data = {0x1, 0x2, 0x3, 0x0}}}};

    fid_address_t dst = {.type = FID_ADDR_IP_PREF, .ippref = {.len = 16, .addr = {.version = AF_IP4, .ip.ip4.data = {0x9, 0x8, 0x0, 0x0}}}};

    source_dest_t sd = {.src = src, .dst = dst};

    gid_address_t g = {.sd = sd, .type = GID_ADDR_SRC_DST, .vni = 0x12345678, .vni_mask = 0x9};
...
static clib_error_t *test_src_dst_deser_bad_afi(void)
{
    clib_error_t *error = 0;
...
static clib_error_t *test_src_dst_serdes(void)
{
    clib_error_t *error = 0;
    ...
    fid_address_t src = {.type = FID_ADDR_MAC, .mac = {0x11, 0x22, 0x33, 0x44, 0x55, 0x66}};

    fid_address_t dst = {.type = FID_ADDR_MAC, .mac = {0x10, 0x21, 0x32, 0x43, 0x54, 0x65}};

    source_dest_t sd = {.src = src, .dst = dst};

    gid_address_t g = {.sd = sd, .type = GID_ADDR_SRC_DST, .vni = 0x0, .vni_mask = 0x0};
...
static clib_error_t *test_gid_address_write(void)
{
    ...
    gid_address_t g = {.ippref = ippref[0], .type = GID_ADDR_IP_PREFIX, .vni = 0x01020304, .vni_mask = 0x18};
...
static int lisp_cp_types_tests(vlib_main_t *vm, unformat_input_t *input)
{
    clib_error_t *error;
...
static clib_error_t *lisp_cp_test(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
    int res = 0;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        if (unformat(input, "types")) {
            ...
            if ((res = lisp_cp_serdes_tests(vm, input))) goto done;
            if ((res = lisp_cp_types_tests(vm, input))) goto done;
        }
        else
            break;
    }
    ...
    if (res) return clib_error_return(0, "rbtree unit test failed");
    return 0;
}
