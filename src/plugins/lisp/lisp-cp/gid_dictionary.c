                                           ...
                                           void                        *arg)
{
    ...
    BV(clib_bihash_foreach_key_value_pair)(&sfib4->ip4_lookup_table, foreach_sfib4_subprefix, &a);
}
                                           ...
                                           void                        *arg)
{
    ...
    BV(clib_bihash_foreach_key_value_pair)(&sfib6->ip6_lookup_table, foreach_sfib6_subprefix, &a);
}
...
void gid_dict_foreach_subprefix(gid_dictionary_t *db, gid_address_t *eid, foreach_subprefix_match_cb_t cb, void *arg)
{
    ...
    if (AF_IP4 == ip_prefix_version(ippref))
        gid_dict_foreach_ip4_subprefix(db, gid_address_vni(eid), &gid_address_sd_src_ippref(eid), &gid_address_sd_dst_ippref(eid), cb, arg);
    else
        gid_dict_foreach_ip6_subprefix(db, gid_address_vni(eid), &gid_address_sd_src_ippref(eid), &gid_address_sd_dst_ippref(eid), cb, arg);
}

void gid_dict_foreach_l2_arp_ndp_entry(gid_dictionary_t *db, BV(clib_bihash_foreach_key_value_pair_cb) cb, void *ht)
{
...
static u64 arp_ndp_lookup(gid_l2_arp_ndp_table_t *db, u32 bd, ip_address_t *key)
{
    ...
    rv = BV(clib_bihash_search_inline_2)(&db->arp_ndp_lookup_table, &kv, &value);
...
u64 gid_dictionary_lookup(gid_dictionary_t *db, gid_address_t *key)
{
    switch (gid_address_type(key)) {
        case GID_ADDR_IP_PREFIX:
            return ip_sd_lookup(db, gid_address_vni(key), &gid_address_ippref(key), 0);
        case GID_ADDR_MAC:
            return mac_sd_lookup(&db->sd_mac_table, gid_address_vni(key), gid_address_mac(key), 0);
        case GID_ADDR_SRC_DST:
            switch (gid_address_sd_dst_type(key)) {
                case FID_ADDR_IP_PREF:
                    return ip_sd_lookup(db, gid_address_vni(key), &gid_address_sd_dst_ippref(key), &gid_address_sd_src_ippref(key));
                    break;
                case FID_ADDR_MAC:
                    return mac_sd_lookup(&db->sd_mac_table, gid_address_vni(key), gid_address_sd_dst_mac(key), gid_address_sd_src_mac(key));
                    break;
                default:
                    clib_warning("Source/Dest address type %d not supported!", gid_address_sd_dst_type(key));
                    break;
            }
            break;
        case GID_ADDR_ARP:
        case GID_ADDR_NDP:
            return arp_ndp_lookup(&db->arp_ndp_table, gid_address_arp_ndp_bd(key), &gid_address_arp_ndp_ip(key));
        case GID_ADDR_NSH:
            return nsh_lookup(&db->nsh_table, gid_address_vni(key), gid_address_nsh_spi(key), gid_address_nsh_si(key));
        default:
...
u32 gid_dictionary_sd_lookup(gid_dictionary_t *db, gid_address_t *dst, gid_address_t *src)
{
    switch (gid_address_type(dst)) {
        case GID_ADDR_IP_PREFIX:
            return ip_sd_lookup(db, gid_address_vni(dst), &gid_address_ippref(dst), &gid_address_ippref(src));
        case GID_ADDR_MAC:
            return mac_sd_lookup(&db->sd_mac_table, gid_address_vni(dst), gid_address_mac(dst), gid_address_mac(src));
        case GID_ADDR_SRC_DST:
            switch (gid_address_sd_dst_type(dst)) {
                case FID_ADDR_IP_PREF:
                    return ip_sd_lookup(db, gid_address_vni(dst), &gid_address_sd_dst_ippref(dst), &gid_address_sd_src_ippref(dst));
                    break;
                case FID_ADDR_MAC:
                    return mac_sd_lookup(&db->sd_mac_table, gid_address_vni(dst), gid_address_sd_dst_mac(dst), gid_address_sd_src_mac(dst));
                    break;
                default:
                    clib_warning("Source/Dest address type %d not supported!", gid_address_sd_dst_type(dst));
                    break;
            }
            break;
        case GID_ADDR_NSH:
...
static u32 add_del_ip4_key(gid_ip4_table_t *db, u32 vni, ip_prefix_t *pref, u32 val, u8 is_add)
{
    ...
    if (is_add) {
        db->ip4_non_empty_dst_address_length_bitmap = clib_bitmap_set(db->ip4_non_empty_dst_address_length_bitmap, 32 - plen, 1);
        ...
        if (db->ip4_prefix_len_refcount[plen] == 0) {
            db->ip4_non_empty_dst_address_length_bitmap = clib_bitmap_set(db->ip4_non_empty_dst_address_length_bitmap, 32 - plen, 0);
            ip4_compute_prefix_lengths_in_search_order(db);
        }
    }
    ...
    if (BV(clib_bihash_search)(&db->ip4_lookup_table, &kv, &value) == 0) old_val = value.value;
...
static void ip4_lookup_init(gid_ip4_table_t *db)
{
    ...
    clib_memset(db->ip4_prefix_len_refcount, 0, sizeof(db->ip4_prefix_len_refcount));
    ...
    if (db->ip4_lookup_table_nbuckets == 0) db->ip4_lookup_table_nbuckets = IP4_LOOKUP_DEFAULT_HASH_NUM_BUCKETS;

    db->ip4_lookup_table_nbuckets = 1 << max_log2(db->ip4_lookup_table_nbuckets);

    if (db->ip4_lookup_table_size == 0) db->ip4_lookup_table_size = IP4_LOOKUP_DEFAULT_HASH_MEMORY_SIZE;
...
static u32 add_del_sd_ip4_key(gid_dictionary_t *db, u32 vni, ip_prefix_t *dst_pref, ip_prefix_t *src_pref, u32 val, u8 is_add)
{
    ...
    if (is_add) {
        if (GID_LOOKUP_MISS == sfi) {
            ...
            add_del_ip4_key(&db->dst_ip4_table, vni, dst_pref, sfib - db->src_ip4_table_pool, is_add);
            if (src_pref)
        ...
        if (GID_LOOKUP_MISS != sfi) {
            ...
            clib_warning("cannot delete dst mapping %U!", format_ip_prefix, dst_pref);
    }
    return old_val;
}
...
static u32 add_del_ip6_key(gid_ip6_table_t *db, u32 vni, ip_prefix_t *pref, u32 val, u8 is_add)
{
    ...
    if (is_add) {
        db->ip6_non_empty_dst_address_length_bitmap = clib_bitmap_set(db->ip6_non_empty_dst_address_length_bitmap, 128 - plen, 1);
        ...
        if (db->ip6_prefix_len_refcount[plen] == 0) {
            db->ip6_non_empty_dst_address_length_bitmap = clib_bitmap_set(db->ip6_non_empty_dst_address_length_bitmap, 128 - plen, 0);
            ip6_compute_prefix_lengths_in_search_order(db);
        }
    }
    ...
    if (BV(clib_bihash_search)(&db->ip6_lookup_table, &kv, &value) == 0) old_val = value.value;
...
static u32 add_del_mac(gid_mac_table_t *db, u32 vni, u8 *dst_mac, u8 *src_mac, u32 val, u8 is_add)
{
    ...
    if (BV(clib_bihash_search)(&db->mac_lookup_table, &kv, &value) == 0) old_val = value.value;
...
static void ip6_lookup_init(gid_ip6_table_t *db)
{
    ...
    clib_memset(db->ip6_prefix_len_refcount, 0, sizeof(db->ip6_prefix_len_refcount));
    ...
    for (i = 0; i < ARRAY_LEN(db->ip6_fib_masks); i++) {
        ...
        if (i1) db->ip6_fib_masks[i].as_u32[i0] = clib_host_to_net_u32(pow2_mask(i1) << (32 - i1));
    }

    if (db->ip6_lookup_table_nbuckets == 0) db->ip6_lookup_table_nbuckets = IP6_LOOKUP_DEFAULT_HASH_NUM_BUCKETS;

    db->ip6_lookup_table_nbuckets = 1 << max_log2(db->ip6_lookup_table_nbuckets);

    if (db->ip6_lookup_table_size == 0) db->ip6_lookup_table_size = IP6_LOOKUP_DEFAULT_HASH_MEMORY_SIZE;
...
static u32 add_del_sd_ip6_key(gid_dictionary_t *db, u32 vni, ip_prefix_t *dst_pref, ip_prefix_t *src_pref, u32 val, u8 is_add)
{
    ...
    if (is_add) {
        if (GID_LOOKUP_MISS == sfi) {
            ...
            add_del_ip6_key(&db->dst_ip6_table, vni, dst_pref, sfib - db->src_ip6_table_pool, is_add);
            if (src_pref)
...
static u32 add_del_ip(gid_dictionary_t *db, u32 vni, ip_prefix_t *dst_key, ip_prefix_t *src_key, u32 value, u8 is_add)
{
    switch (ip_prefix_version(dst_key)) {
        ...
        case AF_IP6:
            ...
            break;
        default:
            clib_warning("address type %d not supported!", ip_prefix_version(dst_key));
            break;
    }
    return ~0;
}
...
static u32 add_del_sd(gid_dictionary_t *db, u32 vni, source_dest_t *key, u32 value, u8 is_add)
{
    switch (sd_dst_type(key)) {
        case FID_ADDR_IP_PREF:
            add_del_ip(db, vni, &sd_dst_ippref(key), &sd_src_ippref(key), value, is_add);

        case FID_ADDR_MAC:
            return add_del_mac(&db->sd_mac_table, vni, sd_dst_mac(key), sd_src_mac(key), value, is_add);
...
static u64 add_del_arp_ndp(gid_l2_arp_ndp_table_t *db, u32 bd, ip_address_t *key, u64 value, u8 is_add)
{
    ...
    make_arp_ndp_key(&kv, bd, key);
    if (BV(clib_bihash_search)(&db->arp_ndp_lookup_table, &kv, &result) == 0) old_val = result.value;

    if (is_add) {
        kv.value = value;
        BV(clib_bihash_add_del)(&db->arp_ndp_lookup_table, &kv, 1 /* is_add */);
        db->count++;
    }
    else {
        BV(clib_bihash_add_del)(&db->arp_ndp_lookup_table, &kv, 0 /* is_add */);
        db->count--;
    }
    return old_val;
}
...
static u32 add_del_nsh(gid_nsh_table_t *db, u32 vni, u32 spi, u8 si, u32 value, u8 is_add)
{
    ...
    make_nsh_key(&kv, vni, spi, si);
    if (BV(clib_bihash_search)(&db->nsh_lookup_table, &kv, &result) == 0) old_val = result.value;
...
u32 gid_dictionary_add_del(gid_dictionary_t *db, gid_address_t *key, u64 value, u8 is_add)
{
    switch (gid_address_type(key)) {
        case GID_ADDR_IP_PREFIX:
            return add_del_ip(db, gid_address_vni(key), &gid_address_ippref(key), 0, (u32) value, is_add);
        case GID_ADDR_MAC:
            return add_del_mac(&db->sd_mac_table, gid_address_vni(key), gid_address_mac(key), 0, (u32) value, is_add);
        case GID_ADDR_SRC_DST:
            return add_del_sd(db, gid_address_vni(key), &gid_address_sd(key), (u32) value, is_add);
        case GID_ADDR_ARP:
        case GID_ADDR_NDP:
            return add_del_arp_ndp(&db->arp_ndp_table, gid_address_arp_ndp_bd(key), &gid_address_arp_ndp_ip(key), value, is_add);
        case GID_ADDR_NSH:
            return add_del_nsh(&db->nsh_table, gid_address_vni(key), gid_address_nsh_spi(key), gid_address_nsh_si(key), value, is_add);
...
{
    if (db->mac_lookup_table_nbuckets == 0) db->mac_lookup_table_nbuckets = MAC_LOOKUP_DEFAULT_HASH_NUM_BUCKETS;

    db->mac_lookup_table_nbuckets = 1 << max_log2(db->mac_lookup_table_nbuckets);

    if (db->mac_lookup_table_size == 0) db->mac_lookup_table_size = MAC_LOOKUP_DEFAULT_HASH_MEMORY_SIZE;

    BV(clib_bihash_init)(&db->mac_lookup_table, "mac lookup table", db->mac_lookup_table_nbuckets, db->mac_lookup_table_size);
}
...
static void arp_ndp_lookup_init(gid_l2_arp_ndp_table_t *db)
{
    ...
    db->arp_ndp_lookup_table_nbuckets = 1 << max_log2(db->arp_ndp_lookup_table_nbuckets);

    if (db->arp_ndp_lookup_table_size == 0) db->arp_ndp_lookup_table_size = ARP_NDP_LOOKUP_DEFAULT_HASH_MEMORY_SIZE;

    BV(clib_bihash_init)(&db->arp_ndp_lookup_table, "arp ndp lookup table", db->arp_ndp_lookup_table_nbuckets, db->arp_ndp_lookup_table_size);
}

static void nsh_lookup_init(gid_nsh_table_t *db)
{
    if (db->nsh_lookup_table_nbuckets == 0) db->nsh_lookup_table_nbuckets = MAC_LOOKUP_DEFAULT_HASH_NUM_BUCKETS;

    db->nsh_lookup_table_nbuckets = 1 << max_log2(db->nsh_lookup_table_nbuckets);

    if (db->nsh_lookup_table_size == 0) db->nsh_lookup_table_size = MAC_LOOKUP_DEFAULT_HASH_MEMORY_SIZE;

    BV(clib_bihash_init)(&db->nsh_lookup_table, "nsh lookup table", db->nsh_lookup_table_nbuckets, db->nsh_lookup_table_size);
}
