...
static clib_error_t *lisp_show_adjacencies_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "vni %d", &vni))
            ...
            vlib_cli_output(vm, "parse error: '%U'", format_unformat_error, line_input);
    ...
    vec_foreach (adj, adjs) {
        vlib_cli_output(vm, "%U %40U\n", format_gid_address, &adj->leid, format_gid_address, &adj->reid);
    }
    vec_free(adjs);
...
static clib_error_t *lisp_add_del_map_server_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!rv) vlib_cli_output(vm, "failed to %s map-server!", is_add ? "add" : "delete");
...
static clib_error_t *lisp_add_del_local_eid_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "add"))
            ...
            ;
        else if (unformat(line_input, "key-id %U", unformat_hmac_key_id, &key_id))
            ;
            ...
            if (!p) {
                error = clib_error_return(0, "locator-set %s doesn't exist", locator_set_name);
                goto done;
            }
    ...
    if (0 != rv) {
        error = clib_error_return(0, "failed to %s local mapping!", is_add ? "add" : "delete");
    }
done:
    vec_free(eids);
    if (locator_set_name) vec_free(locator_set_name);
...
VLIB_CLI_COMMAND(one_add_del_local_eid_command) = {
    .path = "one eid-table",
    .short_help =
        "one eid-table add/del [vni <vni>] eid <eid> "
...
static clib_error_t *lisp_add_del_ndp_entry_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (rc) clib_warning("Failed to %s ndp entry!", is_add ? "add" : "delete");
...
static clib_error_t *lisp_add_del_l2_arp_entry_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (rc) clib_warning("Failed to %s l2 arp entry!", is_add ? "add" : "delete");
...
static clib_error_t *lisp_show_l2_arp_entries_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    hash_foreach_pair(p, ht, ({
                          ...
                          vec_foreach (e, entries) {
                              vlib_cli_output(vm, "\t%U -> %U", format_ip4_address, &e->ip4, format_mac_address, e->mac);
                          }
                          vec_free(entries);
                      }));
...
static clib_error_t *lisp_show_ndp_entries_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    hash_foreach_pair(p, ht, ({
                          ...
                          vec_foreach (e, entries) {
                              vlib_cli_output(vm, "\t%U -> %U", format_ip6_address, &e->ip6, format_mac_address, e->mac);
                          }
                          vec_free(entries);
                      }));
...
static clib_error_t *lisp_add_del_remote_mapping_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(line_input, "rloc %U", unformat_ip_address, &gid_address_ip(&rloc.address))) {
        ...
        else if (unformat(line_input, "action %U", unformat_negative_mapping_action, &action))
            ;
        else {
    ...
    if (rv) clib_warning("failed to %s remote mapping!", is_add ? "add" : "delete");
...
static clib_error_t *lisp_add_del_adjacency_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "del"))
            ...
            ;
        else if (unformat(line_input, "reid %U", unformat_ip_prefix, reid_ippref)) {
        ...
        else if (unformat(line_input, "leid %U", unformat_ip_prefix, leid_ippref)) {
    ...
    if ((gid_address_type(&leid) != gid_address_type(&reid))
        || (gid_address_type(&reid) == GID_ADDR_IP_PREFIX && ip_prefix_version(reid_ippref) != ip_prefix_version(leid_ippref))) {
    ...
    if (vnet_lisp_add_del_adjacency(a)) clib_warning("failed to %s adjacency!", is_add ? "add" : "delete");
...
VLIB_CLI_COMMAND(one_add_del_adjacency_command) = {
    .path = "one adjacency",
    .short_help =
        "one adjacency add|del vni <vni> reid <remote-eid> "
        "leid <local-eid>",
...
static clib_error_t *lisp_map_request_mode_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    /* Get a line of input. */
    if (!unformat_user(input, unformat_line_input, i)) return 0;
...
{
    vlib_cli_output(vm, "map-request mode: %U", format_lisp_map_request_mode, vnet_lisp_get_map_request_mode());
    return 0;
}
...
static clib_error_t *lisp_nsh_set_locator_set_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (0 != rv) {
        error = clib_error_return(0, "failed to %s NSH mapping!", is_add ? "add" : "delete");
    }
...
static clib_error_t *lisp_pitr_set_locator_set_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (0 != rv) {
        error = clib_error_return(0, "failed to %s pitr!", is_add ? "add" : "delete");
    }
...
static u8 *format_eid_entry(u8 *s, va_list *args)
{
    ...
    u8 *type = ls->local ? format(0, "local(%s)", ls->name) : format(0, "remote");

    if (vec_len(ls->locator_indices) == 0) {
        s = format(s, "%-35U%-20saction:%-30U%-20u%-u", format_gid_address, gid, type, format_negative_mapping_action, mapit->action, ttl, aut);
    }
    else {
        vec_foreach (loc_index, ls->locator_indices) {
            locator_t *l = pool_elt_at_index(lcm->locator_pool, loc_index[0]);
            if (l->local)
                loc = format(0, "%U", format_vnet_sw_if_index_name, vnm, l->sw_if_index);
            else
                loc = format(0, "%U", format_ip_address, &gid_address_ip(&l->address));

            if (first_line) {
                s          = format(s, "%-35U%-20s%-30v%-20u%-u\n", format_gid_address, gid, type, loc, ttl, aut);
                first_line = 0;
            }
            else
                s = format(s, "%55s%v\n", "", loc);
...
static clib_error_t *lisp_show_eid_table_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vlib_cli_output(vm, "%-35s%-20s%-30s%-20s%-s", "EID", "type", "locators", "ttl", "authoritative");

    if (print_all) {
        pool_foreach (mapit, lcm->mapping_pool) {
            if (mapit->pitr_set || mapit->nsh_set) continue;

            locator_set_t *ls = pool_elt_at_index(lcm->locator_set_pool, mapit->locator_set_index);
            if (filter && !((1 == filter && ls->local) || (2 == filter && !ls->local))) {
                continue;
            }
            vlib_cli_output(vm, "%U", format_eid_entry, lcm->vnet_main, lcm, mapit, ls);
        }
    }
    else {
        mi = gid_dictionary_lookup(&lcm->mapping_index_by_gid, &eid);
        if ((u32) ~0 == mi) goto done;
...
{
    if (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) return clib_error_return(0, "parse error: '%U'", format_unformat_error, input);
...
static clib_error_t *lisp_show_eid_table_map_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    hash_foreach_pair(p, vni_table, ({ vlib_cli_output(vm, "%=10d%=10d", p->key, p->value[0]); }));
...
static clib_error_t *lisp_add_del_locator_set_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (0 != rv) {
        error = clib_error_return(0, "failed to %s locator-set!", is_add ? "add" : "delete");
    }
...
VLIB_CLI_COMMAND(one_cp_add_del_locator_set_command) = {
    ...
    .short_help =
        "one locator-set add/del <name> [iface <iface-name> "
        "p <priority> w <weight>]",
...
VLIB_CLI_COMMAND(one_cp_add_del_locator_in_set_command) = {
    .path = "one locator",
    .short_help =
        "one locator add/del locator-set <name> iface <iface-name> "
        "p <priority> w <weight>",
...
static clib_error_t *lisp_cp_show_locator_sets_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vlib_cli_output(vm, "%s%=16s%=16s%=16s", "Locator-set", "Locator", "Priority", "Weight");
    ...
    pool_foreach (lsit, lcm->locator_set_pool) {
        ...
        vec_foreach (locit, lsit->locator_indices) {
            ...
            if (loc->local)
                ...
                msg = format(msg, "%16U%16d%16d\n", format_ip_address, &gid_address_ip(&loc->address), loc->priority, loc->weight);
            next_line = 1;
        }
        vlib_cli_output(vm, "%v", msg);
        vec_free(msg);
...
static clib_error_t *lisp_add_del_map_resolver_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (0 != rv) {
        error = clib_error_return(0, "failed to %s map-resolver!", is_add ? "add" : "delete");
    }
...
static clib_error_t *lisp_add_del_mreq_itr_rlocs_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (0 != rv) {
        error = clib_error_return(0, "failed to %s map-request itr-rlocs!", is_add ? "add" : "delete");
    }
...
static clib_error_t *lisp_use_petr_set_locator_set_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (vnet_lisp_use_petr(&ip, is_add)) {
        error = clib_error_return(0, "failed to %s petr!", is_add ? "add" : "delete");
    }
...
static clib_error_t *lisp_show_stats_details_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (vec_len(stats) > 0)
        vlib_cli_output(vm, "[src-EID, dst-EID] [loc-rloc, rmt-rloc] count bytes\n");
    else
