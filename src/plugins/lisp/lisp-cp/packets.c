...
u16 udp_ip6_checksum(ip6_header_t *ip6, udp_header_t *up, u32 len)
{
    ...
    if (len & 1) sum += clib_host_to_net_u16((*(const u8 *) sp) << 8);
...
u16 udp_checksum(udp_header_t *uh, u32 udp_len, void *ih, u8 version)
{
    switch (version) {
        case AF_IP4:
            return (udp_ip4_checksum(uh, udp_len, ((ip4_header_t *) ih)->src_address.as_u8, ((ip4_header_t *) ih)->dst_address.as_u8));
        case AF_IP6:
...
void *pkt_push_ip(vlib_main_t *vm, vlib_buffer_t *b, ip_address_t *src, ip_address_t *dst, u32 proto, u8 csum_offload)
{
    if (ip_addr_version(src) != ip_addr_version(dst)) {
        clib_warning("src %U and dst %U IP have different AFI! Discarding!", format_ip_address, src, format_ip_address, dst);
        return 0;
    }

    switch (ip_addr_version(src)) {
        case AF_IP4:
            return vlib_buffer_push_ip4(vm, b, &ip_addr_v4(src), &ip_addr_v4(dst), proto, csum_offload);
            break;
        case AF_IP6:
            return vlib_buffer_push_ip6(vm, b, &ip_addr_v6(src), &ip_addr_v6(dst), proto);
            break;
    }
...
void *pkt_push_udp_and_ip(vlib_main_t *vm, vlib_buffer_t *b, u16 sp, u16 dp, ip_address_t *sip, ip_address_t *dip, u8 csum_offload)
{
    ...
    if (csum_offload) {
        ...
        udpsum = udp_checksum(uh, clib_net_to_host_u16(uh->length), ih, ip_addr_version(sip));
        if (udpsum == (u16) ~0) {
