...
ip_interface_address_t *ip_interface_get_first_interface_address(ip_lookup_main_t *lm, u32 sw_if_index, u8 loop)
{
    ...
    if (loop && swif->flags & VNET_SW_INTERFACE_FLAG_UNNUMBERED) sw_if_index = swif->unnumbered_sw_if_index;
    u32 ia = (vec_len((lm)->if_address_pool_index_by_sw_if_index) > (sw_if_index))
...
static int dp_add_del_iface(lisp_cp_main_t *lcm, u32 vni, u8 is_l2, u8 is_add, u8 with_default_route)
{
    ...
    if (is_add) {
        if (is_l2)
            ...
            lisp_gpe_tenant_l3_iface_add_or_lock(vni, dp_table[0], with_default_route);
    }
    else {
        if (is_l2)
...
static void dp_del_fwd_entry(lisp_cp_main_t *lcm, u32 dst_map_index)
{
    ...
    if (fe->is_src_dst) gid_address_copy(&a->lcl_eid, &fe->leid);
...
static u32 get_locator_pairs(lisp_cp_main_t *lcm, mapping_t *lcl_map, mapping_t *rmt_map, locator_pair_t **locator_pairs)
{
    ...
    if (!rmt_ls || vec_len(rmt_ls->locator_indices) == 0) return 0;
    ...
    while (1) {
        ...
        for (i = 0; i < vec_len(rmt_ls->locator_indices); i++) {
            if (0 != hash_get(checked, i)) continue;

            li = vec_elt(rmt_ls->locator_indices, i);
            lp = pool_elt_at_index(lcm->locator_pool, li);

            /* we don't support non-IP locators for now */
            if (gid_address_type(&lp->address) != GID_ADDR_IP_PREFIX) continue;

            if ((found && lp->priority == limitp) || (!found && lp->priority >= limitp)) {
                rmt = lp;
        ...
        if (rmt != 0) {
            ...
            esi = ip_fib_get_egress_iface_for_dst(lcm, &gid_address_ip(&rmt->address));
            if ((u32) ~0 == esi) continue;
            ...
            for (i = 0; i < vec_len(lcl_ls->locator_indices); i++) {
                ...
                if (sl->sw_if_index == esi) {
                    ...
                    if (0 == ip_interface_get_first_ip_address(lcm, sl->sw_if_index, gid_address_ip_version(&rmt->address), lcl_addr)) continue;
...
static void dp_add_fwd_entry(lisp_cp_main_t *lcm, u32 src_map_index, u32 dst_map_index)
{
    ...
    feip = hash_get(lcm->fwd_entry_by_mapping_index, dst_map_index);
    if (feip) dp_del_fwd_entry(lcm, dst_map_index);
    ...
    if (MR_MODE_SRC_DST == lcm->map_request_mode) {
        if (GID_ADDR_SRC_DST == gid_address_type(rmt_eid)) {
            gid_address_sd_to_flat(&a->rmt_eid, rmt_eid, &gid_address_sd_dst(rmt_eid));
            gid_address_sd_to_flat(&a->lcl_eid, rmt_eid, &gid_address_sd_src(rmt_eid));
        }
        else {
    ...
    if (rv) {
        if (a->locator_pairs) vec_free(a->locator_pairs);
        return;
    }
    ...
    rmts_stored_idxp = hash_get(lcm->lcl_to_rmt_adjs_by_lcl_idx, src_map_index);
    if (!rmts_stored_idxp) {
...
static int dp_add_fwd_entry_from_mt(u32 si, u32 di)
{
    ...
    vl_api_rpc_call_main_thread(dp_add_fwd_entry_thread_fn, (u8 *) &a, sizeof(a));
    return 0;
}
...
lisp_adjacency_t *vnet_lisp_adjacencies_get_by_vni(u32 vni)
{
    ...
    pool_foreach (fwd, lcm->fwd_entry_pool) {
        if (gid_address_vni(&fwd->reid) != vni) continue;
...
int vnet_lisp_add_del_map_server(ip_address_t *addr, u8 is_add)
{
    ...
    if (is_add) {
        if (get_map_server(addr)) {
            clib_warning("map-server %U already exists!", format_ip_address, addr);
            return -1;
        }
        ...
        if (vec_len(lcm->map_servers) == 1) lcm->do_map_server_election = 1;
    }
    else {
        for (i = 0; i < vec_len(lcm->map_servers); i++) {
            ms = vec_elt_at_index(lcm->map_servers, i);
            if (!ip_address_cmp(&ms->address, addr)) {
                if (!ip_address_cmp(&ms->address, &lcm->active_map_server)) lcm->do_map_server_election = 1;
...
int vnet_lisp_map_cache_add_del(vnet_lisp_add_del_mapping_args_t *a, u32 *map_index_result)
{
    ...
    if (a->is_add) {
        ...
        if (mi != GID_LOOKUP_MISS && !gid_address_cmp(&old_map->eid, &a->eid)) {
            clib_warning("eid %U found in the eid-table", format_gid_address, &a->eid);
        ...
        gid_dictionary_add_del(&lcm->mapping_index_by_gid, &a->eid, map_index, 1);

        if (pool_is_free_index(lcm->locator_set_pool, a->locator_set_index)) {
            clib_warning("Locator set with index %d doesn't exist", a->locator_set_index);
            return VNET_API_ERROR_INVALID_VALUE;
        }

        /* add eid to list of eids supported by locator-set */
        vec_validate(lcm->locator_set_to_eids, a->locator_set_index);
        eid_indexes = vec_elt_at_index(lcm->locator_set_to_eids, a->locator_set_index);
        ...
        if (mi == GID_LOOKUP_MISS) {
            clib_warning("eid %U not found in the eid-table", format_gid_address, &a->eid);
            return VNET_API_ERROR_INVALID_VALUE;
        }
        ...
        if (m->local) {
            ...
            if (rmts_idxp) {
                rmts = pool_elt_at_index(lcm->lcl_to_rmt_adjacencies, rmts_idxp[0]);
...
int vnet_lisp_add_del_local_mapping(vnet_lisp_add_del_mapping_args_t *a, u32 *map_index_result)
{
    ...
    if (!dp_table && GID_ADDR_NSH != type) {
        clib_warning("vni %d not associated to a %s!", vni, GID_ADDR_IP_PREFIX == type ? "vrf" : "bd");
        return VNET_API_ERROR_INVALID_VALUE;
    }
...
static int add_l2_arp_bd(BVT(clib_bihash_kv) * kvp, void *arg)
{
    ...
    if (AF_IP6 == version) return (BIHASH_WALK_CONTINUE);
...
u32 *vnet_lisp_l2_arp_bds_get(void)
{
    ...
    gid_dict_foreach_l2_arp_ndp_entry(&lcm->mapping_index_by_gid, add_l2_arp_bd, &bds);
    return bds;
}
...
static int add_ndp_bd(BVT(clib_bihash_kv) * kvp, void *arg)
{
    u32 **ht      = arg;
    u32   version = (u32) kvp->key[0];
    if (AF_IP4 == version) return (BIHASH_WALK_CONTINUE);
...
u32 *vnet_lisp_ndp_bds_get(void)
{
    ...
    gid_dict_foreach_l2_arp_ndp_entry(&lcm->mapping_index_by_gid, add_ndp_bd, &bds);
    return bds;
}
...
lisp_api_ndp_entry_t *vnet_lisp_ndp_entries_get_by_bd(u32 bd)
{
    ...
    gid_dict_foreach_l2_arp_ndp_entry(&lcm->mapping_index_by_gid, add_ndp_entry, &a);
    return entries;
}
...
int vnet_lisp_add_del_l2_arp_ndp_entry(gid_address_t *key, u8 *mac, u8 is_add)
{
    ...
    if (is_add) {
        ...
        gid_dictionary_add_del(&lcm->mapping_index_by_gid, key, val, 1 /* is_add */);
    }
    else {
        if (res == GID_LOOKUP_MISS_L2) {
            clib_warning("ONE entry %U not found - cannot delete!", format_gid_address, key);
            return -1;
        }
        gid_dictionary_add_del(&lcm->mapping_index_by_gid, key, 0, 0 /* is_add */);
    }
...
int vnet_lisp_eid_table_map(u32 vni, u32 dp_id, u8 is_l2, u8 is_add)
{
    ...
    if (is_add) {
        ...
        /* create dp iface */
        dp_add_del_iface(lcm, vni, is_l2, 1 /* is_add */, 1 /* with_default_route */);
    }
    else {
        if (!dp_idp || !vnip) {
            clib_warning(
...
static u8 compare_locators(lisp_cp_main_t *lcm, u32 *old_ls_indexes, locator_t *new_locators)
{
    ...
    if (vec_len(old_ls_indexes) != vec_len(new_locators)) return 1;
    ...
    for (i = 0; i < vec_len(new_locators); i++) {
        ...
        if (locator_cmp(old_loc, new_loc)) return 1;
    }
    return 0;
}
...
static void remove_mapping_if_needed(u32 mi, void *arg)
{
    ...
    if (a->is_negative) {
        if (0 != vec_len(ls->locator_indices)) delete = 1;
    }
    else {
        if (0 == vec_len(ls->locator_indices)) delete = 1;
    }

    if (delete) vec_add1(a->eids_to_be_deleted, m->eid);
}
...
static void remove_overlapping_sub_prefixes(lisp_cp_main_t *lcm, gid_address_t *eid, u8 is_negative)
{
    ...
    if (MR_MODE_SRC_DST != lcm->map_request_mode) return;

    /* ... and  only for IP prefix */
    if (GID_ADDR_SRC_DST != gid_address_type(eid) || (FID_ADDR_IP_PREF != gid_address_sd_dst_type(eid))) return;

    a.is_negative = is_negative;
    a.lcm         = lcm;

    gid_dict_foreach_subprefix(&lcm->mapping_index_by_gid, eid, remove_mapping_if_needed, &a);
    ...
    vec_foreach (e, a.eids_to_be_deleted) {
        ...
        adj_args->is_add = 0;
        if (vnet_lisp_add_del_adjacency(adj_args)) clib_warning("failed to del adjacency!");
...
int vnet_lisp_add_mapping(vnet_lisp_add_del_mapping_args_t *a, locator_t *rlocs, u32 *res_map_index, u8 *is_updated)
{
    ...
    if (res_map_index) res_map_index[0] = ~0;
    if (is_updated) is_updated[0] = 0;
    ...
    vec_foreach (loc, rlocs) {
        ...
        if (is_local_ip(lcm, &ip_prefix_addr(p))) {
            clib_warning("RLOC %U matches a local address!", format_gid_address, &loc->address);
    ...
    if (old_map && gid_address_cmp(&old_map->eid, &a->eid) == 0) {
        ...
        if (old_map->action != a->action && NULL != is_updated) is_updated[0] = 1;

        old_map->authoritative = a->authoritative;
        old_map->ttl           = a->ttl;

        old_ls = pool_elt_at_index(lcm->locator_set_pool, old_map->locator_set_index);
        if (compare_locators(lcm, old_ls->locator_indices, ls_args->locators)) {
            ...
            if (is_updated) is_updated[0] = 1;
        }
        if (res_map_index) res_map_index[0] = mi;
    }
    /* new mapping */
    else {
        if (is_updated) is_updated[0] = 1;
        ...
        if (res_map_index) res_map_index[0] = dst_map_index;
    }
...
int vnet_lisp_del_mapping(gid_address_t *eid, u32 *res_map_index)
{
    ...
    if (old_map == 0 || gid_address_cmp(&old_map->eid, eid) != 0) {
        clib_warning("cannot delete mapping for eid %U", format_gid_address, eid);
        return -1;
    }
    ...
    if (old_map->timer_set) TW(tw_timer_stop)(&lcm->wheel, old_map->timer_handle);
...
cleanup:
    if (map_indices) vec_free(map_indices);
    return rv;
}
...
int vnet_lisp_add_del_adjacency(vnet_lisp_add_del_adjacency_args_t *a)
{
    ...
    remote_mi = gid_dictionary_sd_lookup(&lcm->mapping_index_by_gid, &a->reid, &a->leid);
    if (GID_LOOKUP_MISS == remote_mi) {
        clib_warning("Remote eid %U not found. Cannot add adjacency!", format_gid_address, &a->reid);
    ...
    if (a->is_add) {
        ...
        if (lcm->flags & LISP_FLAG_PITR_MODE) {
            ...
            if (gid_address_type(&a->reid) == GID_ADDR_NSH) {
                ...
                local_mi = gid_dictionary_lookup(&lcm->mapping_index_by_gid, &a->leid);
            }
        }

        if (GID_LOOKUP_MISS == local_mi) {
            clib_warning("Local eid %U not found. Cannot add adjacency!", format_gid_address, &a->leid);
...
int vnet_lisp_nsh_set_locator_set(u8 *locator_set_name, u8 is_add)
{
    ...
    if (is_add) {
        if (lcm->nsh_map_index == (u32) ~0) {
            ...
            if (~0 == vnet_lisp_gpe_add_nsh_iface(lgm)) return -1;
        }
    }
    else {
        if (lcm->nsh_map_index != (u32) ~0) {
...
static void clean_locator_to_locator_set(lisp_cp_main_t *lcm, u32 lsi)
{
    ...
    for (i = 0; i < vec_len(ls->locator_indices); i++) {
        ...
        ls_indexes = vec_elt_at_index(lcm->locator_to_locator_sets, loc_indexp[0]);
        for (j = 0; j < vec_len(ls_indexes[0]); j++) {
            ls_indexp = vec_elt_at_index(ls_indexes[0], j);
            if (ls_indexp[0] == lsi) break;
        }
...
static inline uword *get_locator_set_index(vnet_lisp_add_del_locator_set_args_t *a, uword *p)
{
    lisp_cp_main_t *lcm = vnet_lisp_cp_get_main();
...
static inline int is_locator_in_locator_set(lisp_cp_main_t *lcm, locator_set_t *ls, locator_t *loc)
{
    locator_t *itloc;
    u32       *locit;
    ...
    vec_foreach (locit, ls->locator_indices) {
        ...
        if ((ls->local && itloc->sw_if_index == loc->sw_if_index) || (!ls->local && !gid_address_cmp(&itloc->address, &loc->address))) {
...
static void update_fwd_entries_by_locator_set(lisp_cp_main_t *lcm, u32 ls_index, u8 remove_only)
{
    ...
    if (vec_len(lcm->locator_set_to_eids) <= ls_index) return;
...
static inline void remove_locator_from_locator_set(locator_set_t *ls, u32 *locit, u32 ls_index, u32 loc_id)
{
    lisp_cp_main_t *lcm        = vnet_lisp_cp_get_main();
    u32           **ls_indexes = NULL;
...
int vnet_lisp_add_del_locator(vnet_lisp_add_del_locator_set_args_t *a, locator_set_t *ls, u32 *ls_result)
{
    ...
    if (ls == 0) {
        ...
        if (!ls) {
            clib_warning("locator-set %d to be overwritten doesn't exist!", p[0]);
            return VNET_API_ERROR_INVALID_ARGUMENT;
        }
    }

    if (a->is_add) {
        if (ls_result) ls_result[0] = p[0];
        ...
        vec_foreach (itloc, a->locators) {
            ...
            ls_indexes = vec_elt_at_index(lcm->locator_to_locator_sets, loc_index);
        ...
        vec_foreach (itloc, a->locators) {
            ...
            vec_foreach (locit, ls->locator_indices) {
                ...
                else if (0 == loc->local && !gid_address_cmp(&loc->address, &itloc->address)) {
                    removed = 1;
                    remove_locator_from_locator_set(ls, locit, ls_index, loc_id);
                }
                ...
                if (removed) {
                    ...
                    update_fwd_entries_by_locator_set(lcm, ls_index, vec_len(ls->locator_indices) == 0);
                }
        ...
        /* new locator-set */
            ...
            if (a->local) {
                ...
                if (!lcm->locator_set_index_by_name)
                    lcm->locator_set_index_by_name = hash_create_vec(/* size */ 0, sizeof(ls->name[0]), sizeof(uword));
            ...
            ls->local = a->local;
            if (ls_result) ls_result[0] = ls_index;
        }
...
{
    udp_register_dst_port(vm, UDP_DST_PORT_lisp_cp, lisp_cp_input_node.index, 1 /* is_ip4 */);
    udp_register_dst_port(vm, UDP_DST_PORT_lisp_cp6, lisp_cp_input_node.index, 0 /* is_ip4 */);
}
...
static void lisp_cp_enable_l2_l3_ifaces(lisp_cp_main_t *lcm, u8 with_default_route)
{
    ...
    hash_foreach(vni, dp_table, lcm->table_id_by_vni, ({ dp_add_del_iface(lcm, vni, /* is_l2 */ 0, /* is_add */ 1, with_default_route); }));
    hash_foreach(vni, dp_table, lcm->bd_id_by_vni, ({ dp_add_del_iface(lcm, vni, /* is_l2 */ 1, 1, with_default_route); }));
}
...
clib_error_t *vnet_lisp_enable_disable(u8 is_enable)
{
    ...
    if (error) {
        return clib_error_return(0, "failed to %s data-plane!", a->is_en ? "enable" : "disable");
    }
    ...
    if (is_enable) vnet_lisp_create_retry_process(lcm);
...
int vnet_lisp_add_del_map_resolver(vnet_lisp_add_del_map_resolver_args_t *a)
{
    ...
    if (a->is_add) {
        if (get_map_resolver(&a->address)) {
            clib_warning("map-resolver %U already exists!", format_ip_address, &a->address);
            return -1;
        }
        ...
        if (vec_len(lcm->map_resolvers) == 1) lcm->do_map_resolver_election = 1;
    }
    else {
        for (i = 0; i < vec_len(lcm->map_resolvers); i++) {
            mr = vec_elt_at_index(lcm->map_resolvers, i);
            if (!ip_address_cmp(&mr->address, &a->address)) {
                if (!ip_address_cmp(&mr->address, &lcm->active_map_resolver)) lcm->do_map_resolver_election = 1;
...
u8 *format_lisp_cp_lookup_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "LISP-CP-LOOKUP: map-resolver: %U destination eid %U", format_ip_address, &t->map_resolver_ip, format_gid_address, &t->dst_eid);
    return s;
}
...
static gid_address_t *build_itr_rloc_list(lisp_cp_main_t *lcm, locator_set_t *loc_set)
{
    ...
    for (i = 0; i < vec_len(loc_set->locator_indices); i++) {
        ...
        foreach_ip_interface_address(&lcm->im4->lookup_main, ia, loc->sw_if_index, 1 /* unnumbered */, ({
        ...
        /* Add ipv6 locators */
        foreach_ip_interface_address(&lcm->im6->lookup_main, ia, loc->sw_if_index, 1 /* unnumbered */, ({
                                        ...
                                        u32            *bi_res)
{
    ...
    /* put lisp msg */
    lisp_msg_put_mreq(lcm, b, NULL, deid, itr_rlocs, 0 /* smr invoked */, 1 /* rloc probe */, nonce_res);

    /* push outer ip header */
    pkt_push_udp_and_ip(vm, b, LISP_CONTROL_PORT, LISP_CONTROL_PORT, sloc, rloc, 1);
                                                     ...
                                                     u32            *bi_res)
{
    ...
    if (MR_MODE_SRC_DST == lcm->map_request_mode && GID_ADDR_SRC_DST != gid_address_type(deid)) {
        gid_address_t sd;
        clib_memset(&sd, 0, sizeof(sd));
        build_src_dst(&sd, seid, deid);
        lisp_msg_put_mreq(lcm, b, seid, &sd, rlocs, is_smr_invoked, 0 /* rloc probe */, nonce_res);
    }
    else {
        /* put lisp msg */
        lisp_msg_put_mreq(lcm, b, seid, deid, rlocs, is_smr_invoked, 0 /* rloc probe */, nonce_res);
    }
    ...
    /* push outer ip header */
    pkt_push_udp_and_ip(vm, b, LISP_CONTROL_PORT, LISP_CONTROL_PORT, sloc, mr_ip, 1);
...
#define _(name)                                                         \
    static int elect_map_##name(lisp_cp_main_t *lcm)                    \
    {                                                                   \
        lisp_msmr_t *mr;                                                \
...
static void add_locators(lisp_cp_main_t *lcm, mapping_t *m, u32 locator_set_index, ip_address_t *probed_loc)
{
    ...
    m->locators       = 0;
    locator_set_t *ls = pool_elt_at_index(lcm->locator_set_pool, locator_set_index);
    vec_foreach (li, ls->locator_indices) {
        ...
        if (loc->local) {
            ...
            if (probed_loc && ip_address_cmp(probed_loc, new_ip) == 0) new.probed = 1;
        }
...
static int update_map_register_auth_data(map_register_hdr_t *map_reg_hdr, lisp_key_type_t key_id, u8 *key, u16 auth_data_len, u32 msg_len)
{
    ...
    ki = vnet_crypto_key_add(lcm->vlib_main, lisp_key_type_to_crypto_alg(key_id), key, vec_len(key));
                                         ...
                                         u32            *bi_res)
{
    ...
    map_reg_hdr   = lisp_msg_put_map_register(b, records, want_map_notif, auth_data_len, nonce_res, &msg_len);

    update_map_register_auth_data(map_reg_hdr, key_id, key, auth_data_len, msg_len);

    /* push outer ip header */
    pkt_push_udp_and_ip(vm, b, LISP_CONTROL_PORT, LISP_CONTROL_PORT, sloc, ms_ip, 1);
...
#define _(name)                                                                                                               \
    static int get_egress_map_##name##_ip(lisp_cp_main_t *lcm, ip_address_t *ip)                                              \
    {                                                                                                                         \
        lisp_msmr_t *mr;                                                                                                      \
        while (lcm->do_map_##name##_election | (0 == ip_fib_get_first_egress_ip_for_dst(lcm, &lcm->active_map_##name, ip))) { \
            if (0 == elect_map_##name(lcm)) /* all map resolvers/servers are down */                                          \
            {                                                                                                                 \
...
static int send_rloc_probe(lisp_cp_main_t *lcm, gid_address_t *deid, u32 local_locator_set_index, ip_address_t *sloc, ip_address_t *rloc)
{
    ...
    next_index = (ip_addr_version(rloc) == AF_IP4) ? ip4_lookup_node.index : ip6_lookup_node.index;
...
static int send_rloc_probes(lisp_cp_main_t *lcm)
{
    ...
    pool_foreach (e, lcm->fwd_entry_pool) {
        if (vec_len(e->locator_pairs) == 0) continue;
        ...
        vec_foreach (lp, e->locator_pairs) {
            if (lp->priority != lprio) break;

            /* get first remote locator */
            send_rloc_probe(lcm, &e->reid, lm->locator_set_index, &lp->lcl_loc, &lp->rmt_loc);
            rloc_probes_sent++;
        }
    }

    vlib_node_increment_counter(vlib_get_main(), lisp_cp_output_node.index, LISP_CP_OUTPUT_ERROR_RLOC_PROBES_SENT, rloc_probes_sent);
    return 0;
}
...
static int send_map_register(lisp_cp_main_t *lcm, u8 want_map_notif)
{
    ...
    if (get_egress_map_server_ip(lcm, &sloc) < 0) return -1;
    ...
    vec_foreach (r, records) {
        ...
        if (!key) continue; /* no secret key -> map-register cannot be sent */
        ...
        b = build_map_register(lcm, &sloc, &lcm->active_map_server, &nonce, want_map_notif, group, key_id, key, &bi);
        vec_free(group);
        if (!b) continue;

        vnet_buffer(b)->sw_if_index[VLIB_TX] = 0;

        next_index = (ip_addr_version(&lcm->active_map_server) == AF_IP4) ? ip4_lookup_node.index : ip6_lookup_node.index;
        ...
        hash_set(lcm->map_register_messages_by_nonce, nonce, pmr - lcm->pending_map_registers_pool);
    }
    free_map_register_records(records);

    vlib_node_increment_counter(vlib_get_main(), lisp_cp_output_node.index, LISP_CP_OUTPUT_ERROR_MAP_REGISTERS_SENT, map_registers_sent);
...
static int _send_encapsulated_map_request(lisp_cp_main_t *lcm, gid_address_t *seid, gid_address_t *deid, u8 is_smr_invoked, u8 is_resend)
{
    ...
    pool_foreach (pmr, lcm->pending_map_requests_pool) {
        if (!gid_address_cmp(&pmr->src, seid) && !gid_address_cmp(&pmr->dst, deid)) {
            duplicate_pmr = pmr;
            break;
        }
    }
    ...
    if (!pitr_mode && gid_address_type(deid) != GID_ADDR_NSH) {
        ...
        if (map_index == ~0) {
            clib_warning("No local mapping found in eid-table for %U!", format_gid_address, seid);
            return -1;
        }
        ...
        if (!map->local) {
            clib_warning("Mapping found for src eid %U is not marked as local!", format_gid_address, seid);
            return -1;
        }
    ...
    if (get_egress_map_resolver_ip(lcm, &sloc) < 0) {
        if (duplicate_pmr) duplicate_pmr->to_be_removed = 1;
        return -1;
    }

    /* build the encapsulated map request */
    b = build_encapsulated_map_request(lcm, seid, deid, loc_set, &lcm->active_map_resolver, &sloc, is_smr_invoked, &nonce, &bi);

    if (!b) return -1;

    /* set fib index to default and lookup node */
    vnet_buffer(b)->sw_if_index[VLIB_TX] = 0;
    next_index                           = (ip_addr_version(&lcm->active_map_resolver) == AF_IP4) ? ip4_lookup_node.index : ip6_lookup_node.index;
    ...
    vlib_node_increment_counter(vlib_get_main(), lisp_cp_output_node.index, LISP_CP_OUTPUT_ERROR_MAP_REQUESTS_SENT, 1);
    ...
    /* if there is a pending request already update it */
    {
        ...
        hash_set(lcm->pending_map_requests_by_nonce, nonce, duplicate_pmr - lcm->pending_map_requests_pool);
    }
    else {
        ...
        hash_set(lcm->pending_map_requests_by_nonce, nonce, pmr - lcm->pending_map_requests_pool);
    }
...
static u32 lisp_get_vni_from_buffer_ip(lisp_cp_main_t *lcm, vlib_buffer_t *b, u8 version)
{
    ...
    table_id =
        fib_table_get_table_id_for_sw_if_index((version == AF_IP4 ? FIB_PROTOCOL_IP4 : FIB_PROTOCOL_IP6), vnet_buffer(b)->sw_if_index[VLIB_RX]);
...
always_inline u32 lisp_get_bd_from_buffer_eth(vlib_buffer_t *b)
{
    u32 sw_if_index0;
...
always_inline u32 lisp_get_vni_from_buffer_eth(lisp_cp_main_t *lcm, vlib_buffer_t *b)
{
    uword *vnip;
    u32    vni = ~0;
...
void get_src_and_dst_eids_from_buffer(lisp_cp_main_t *lcm, vlib_buffer_t *b, gid_address_t *src, gid_address_t *dst, u16 type)
{
    ...
    if (LISP_AFI_IP == type || LISP_AFI_IP6 == type) {
        ...
        if (clib_net_to_host_u16(eh->type) == ETHERNET_TYPE_ARP) {
            ...
            if (clib_net_to_host_u16(ah->opcode) != ETHERNET_ARP_OPCODE_request) {
                clib_memset(&gid_address_arp_ndp_ip(dst), 0, sizeof(ip_address_t));
            ...
            clib_memcpy(&gid_address_arp_ip4(dst), &ah->ip4_over_ethernet[1].ip4, 4);
        }
        else {
            if (clib_net_to_host_u16(eh->type) == ETHERNET_TYPE_IP6) {
                ...
                if (IP_PROTOCOL_ICMP6 == ip->protocol) {
                    ...
                    if (ndh->icmp.type == ICMP6_neighbor_solicitation) {
                        ...
                        opt = (void *) (ndh + 1);
                        if ((opt->header.type != ICMP6_NEIGHBOR_DISCOVERY_OPTION_source_link_layer_address) || (opt->header.n_data_u64s != 1)) {
                            clib_memset(&gid_address_arp_ndp_ip(dst), 0, sizeof(ip_address_t));
                            ip_addr_version(&gid_address_arp_ndp_ip(dst)) = AF_IP6;
                            gid_address_arp_ndp_bd(dst)                   = ~0;
                            gid_address_type(src)                         = GID_ADDR_NO_ADDRESS;
                            return;
                        }

                        gid_address_ndp_bd(dst) = lisp_get_bd_from_buffer_eth(b);
                        ip_address_set(&gid_address_arp_ndp_ip(dst), &ndh->target_address, AF_IP6);
                        return;
                    }
                }
            }
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (gid_address_type(&dst) == GID_ADDR_ARP) {
                ...
                if (GID_LOOKUP_MISS_L2 == mac0) goto drop;
                ...
                arp0                       = (ethernet_arp_header_t *) (((u8 *) eth0) + sizeof(*eth0));
                arp0->opcode               = clib_host_to_net_u16(ETHERNET_ARP_OPCODE_reply);
                arp0->ip4_over_ethernet[1] = arp0->ip4_over_ethernet[0];
                mac_address_from_u64(&arp0->ip4_over_ethernet[0].mac, mac0);
                clib_memcpy(&arp0->ip4_over_ethernet[0].ip4, &gid_address_arp_ip4(&dst), 4);
                ...
                ip0->hop_limit   = 255;
                opt              = (void *) (ndh + 1);
                opt->header.type = ICMP6_NEIGHBOR_DISCOVERY_OPTION_target_link_layer_address;
                clib_memcpy(opt->ethernet_address, (u8 *) &mac0, 6);
                ndh->icmp.type = ICMP6_neighbor_advertisement;
                ndh->advertisement_flags =
                    clib_host_to_net_u32(ICMP6_NEIGHBOR_ADVERTISEMENT_FLAG_SOLICITED | ICMP6_NEIGHBOR_ADVERTISEMENT_FLAG_OVERRIDE);
                ndh->icmp.checksum = 0;
                ndh->icmp.checksum = ip6_tcp_udp_icmp_compute_checksum(vm, b0, ip0, &bogus_length);
                clib_memcpy(eh0->dst_address, eh0->src_address, 6);
                clib_memcpy(eh0->src_address, (u8 *) &mac0, 6);
                b0->error = node->errors[LISP_CP_LOOKUP_ERROR_NDP_NEIGHBOR_ADVERTISEMENT_TX];
                next0     = LISP_CP_LOOKUP_NEXT_ARP_NDP_REPLY_TX;
                goto enqueue;
            }
            ...
            if (~0 != di) {
                ...
                if (m->action == LISP_SEND_MAP_REQUEST) {
                    /* send map-request */
                    queue_map_request(&src, &dst, 0 /* smr_invoked */, 0 /* is_resend */);
                }
                else {
        ...
        enqueue:
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                lisp_cp_lookup_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));

                clib_memset(tr, 0, sizeof(*tr));
                if ((gid_address_type(&dst) == GID_ADDR_NDP) || (gid_address_type(&dst) == GID_ADDR_ARP))
                    clib_memcpy(&tr->dst_eid, &dst, sizeof(gid_address_t));
                else
                    gid_address_copy(&tr->dst_eid, &dst);
                ip_address_copy(&tr->map_resolver_ip, &lcm->active_map_resolver);
            }
...
static void process_expired_mapping(lisp_cp_main_t *lcm, u32 mi)
{
    ...
    clib_memset(a, 0, sizeof(*a));
    a->rmt_eid = fe->reid;
    if (fe->is_src_dst) a->lcl_eid = fe->leid;
    a->vni = gid_address_vni(&fe->reid);

    rv = vnet_lisp_gpe_get_fwd_stats(a, &c);
    if (0 == rv) have_stats = 1;
...
void *process_map_reply(map_records_arg_t *a)
{
    ...
    vec_foreach (m, a->mappings) {
        ...
        if (dst_map_index == (u32) ~0) continue;
        ...
        if (is_changed) {
            ...
            if (vnet_lisp_add_del_adjacency(adj_args)) clib_warning("failed to add adjacency!");
        }

        if ((u32) ~0 != m->ttl) mapping_start_expiration_timer(lcm, dst_map_index, (m->ttl == 0) ? 0 : MAPPING_TIMEOUT);
    }

    /* remove pending map request entry */

    clib_fifo_foreach(noncep, pmr->nonces, ({ hash_unset(lcm->pending_map_requests_by_nonce, noncep[0]); }));
...
static void process_map_notify(map_records_arg_t *a)
{
    ...
    if (!pmr_index) {
        clib_warning("No pending map-register entry with nonce %lu!", a->nonce);
        return;
    }
...
static mapping_t *get_mapping(lisp_cp_main_t *lcm, gid_address_t *e)
{
    ...
    if (~0 == mi) {
        clib_warning("eid %U not found in map-cache!", unformat_gid_address, e);
        return 0;
    }
...
static map_records_arg_t *parse_map_notify(vlib_buffer_t *b)
{
    ...
    if (!is_auth_data_valid(mnotif_hdr, vlib_buffer_get_tail(b) - (u8 *) mnotif_hdr, key_id, key)) {
        clib_warning(
...
build_map_reply(lisp_cp_main_t *lcm, ip_address_t *sloc, ip_address_t *dst, u64 nonce, u8 probe_bit, mapping_t *records, u16 dst_port, u32 *bi_res)
{
    vlib_buffer_t *b;
    u32            bi;
    vlib_main_t   *vm = lcm->vlib_main;
...
static int send_map_reply(lisp_cp_main_t *lcm, u32 mi, ip_address_t *dst, u8 probe_bit, u64 nonce, u16 dst_port, ip_address_t *probed_loc)
{
    ...
    if (!ip_fib_get_first_egress_ip_for_dst(lcm, dst, &src)) {
        clib_warning("can't find interface address for %U", format_ip_address, dst);
        return -1;
    }

    b = build_map_reply(lcm, &src, dst, nonce, probe_bit, records, dst_port, &bi);
    if (!b) return -1;
    free_map_register_records(records);
...
static void find_ip_header(vlib_buffer_t *b, u8 **ip_hdr)
{
    ...
    *ip_hdr = b->data + start;
    if ((u8 *) *ip_hdr > (u8 *) vlib_buffer_get_current(b)) *ip_hdr = 0;
}
...
void process_map_request(vlib_main_t *vm, vlib_node_runtime_t *node, lisp_cp_main_t *lcm, vlib_buffer_t *b)
{
    ...
    if (!MREQ_SMR(mreq_hdr) && !MREQ_RLOC_PROBE(mreq_hdr)) {
        clib_warning("Only SMR Map-Requests and RLOC probe supported for now!");
        return;
    }
    ...
    for (i = 0; i < MREQ_REC_COUNT(mreq_hdr); i++) {
        ...
        if (MREQ_SMR(mreq_hdr)) {
            ...
            send_map_reply(lcm, mi, dst_loc, 1 /* probe-bit */, nonce, src_port, &probed_loc);
        }
    }

done:
    vlib_node_increment_counter(vm, node->node_index, LISP_CP_INPUT_ERROR_RLOC_PROBE_REQ_RECEIVED, rloc_probe_recv);
    vec_free(itr_rlocs);
}
...
{
    u32                n_left_from, *from, *to_next_drop, rloc_probe_rep_recv = 0, map_notifies_recv = 0;
    ...
    while (n_left_from > 0) {
        ...
        vlib_get_next_frame(vm, node, LISP_CP_INPUT_NEXT_DROP, to_next_drop, n_left_to_next_drop);
        while (n_left_from > 0 && n_left_to_next_drop > 0) {
            ...
            switch (type) {
                case LISP_MAP_REPLY:
                    ...
                    if (a) {
                        if (a->is_rloc_probe) rloc_probe_rep_recv++;
    ...
    vlib_node_increment_counter(vm, node->node_index, LISP_CP_INPUT_ERROR_RLOC_PROBE_REP_RECEIVED, rloc_probe_rep_recv);
    vlib_node_increment_counter(vm, node->node_index, LISP_CP_INPUT_ERROR_MAP_NOTIFIES_RECEIVED, map_notifies_recv);
    return from_frame->n_vectors;
}
...
clib_error_t *lisp_cp_init(vlib_main_t *vm)
{
    ...
    if ((error = vlib_call_init_function(vm, lisp_gpe_init))) return error;
    ...
    lcm->pitr_map_index = ~0;
    lcm->petr_map_index = ~0;
    clib_memset(&lcm->active_map_resolver, 0, sizeof(lcm->active_map_resolver));
    ...
    TW(tw_timer_wheel_init)(&lcm->wheel, 0 /* no callback */, 1e-3 /* timer period 1ms */, ~0 /* max expirations per call */);
    lcm->nsh_map_index             = ~0;
...
lisp_api_stats_t *vnet_lisp_get_stats(void)
{
    ...
    hash_foreach_mem(key, index, lgm->lisp_stats_index_by_key, {
        if (lisp_stats_api_fill(lcm, lgm, &stat, key, index)) vec_add1(stats, stat);
    });
...
static int queue_map_request(gid_address_t *seid, gid_address_t *deid, u8 smr_invoked, u8 is_resend)
{
    ...
    vl_api_rpc_call_main_thread(send_map_request_thread_fn, (u8 *) &a, sizeof(a));
    return 0;
}
        ...
        /* too many retries -> assume current map resolver is not available */
        {
            ...
            if (!mr) {
                clib_warning(
                    ...
                    "by the user recently.",
                    format_ip_address,
                    &lcm->active_map_resolver);
            }
            else {
                clib_warning("map resolver %U is unreachable, ignoring", format_ip_address, &lcm->active_map_resolver);
            ...
            queue_map_request(&r->src, &r->dst, r->is_smr_invoked, 1 /* resend */);
        }
        else {
            /* try again */
            queue_map_request(&r->src, &r->dst, r->is_smr_invoked, 1 /* resend */);
...
static void remove_dead_pending_map_requests(lisp_cp_main_t *lcm)
{
    ...
    pool_foreach (pmr, lcm->pending_map_requests_pool) {
        if (pmr->to_be_removed) {
            clib_fifo_foreach(nonce, pmr->nonces, ({ hash_unset(lcm->pending_map_requests_by_nonce, nonce[0]); }));
...
static void update_rloc_probing(lisp_cp_main_t *lcm, f64 dt)
{
    ...
    if (!lcm->is_enabled || !lcm->rloc_probing) return;
    ...
    if (r->time_to_expire < 0) {
        ...
        if (lcm->expired_map_registers >= lcm->max_expired_map_registers) {
            ...
            if (!ms) {
                ...
                clib_warning("map server %U is unreachable, ignoring", format_ip_address, &lcm->active_map_server);
...
static void update_map_register(lisp_cp_main_t *lcm, f64 dt)
{
    ...
    if (!lcm->is_enabled || !lcm->map_registering) return;
...
static uword send_map_resolver_service(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    while (1) {
        ...
        pool_foreach (pmr, lcm->pending_map_requests_pool) {
            if (!pmr->to_be_removed) update_pending_request(pmr, period);
        }
        ...
        expired = TW(tw_timer_expire_timers_vec)(&lcm->wheel, vlib_time_now(vm), expired);
...
vnet_api_error_t vnet_lisp_stats_enable_disable(u8 enable)
{
    ...
    if (vnet_lisp_enable_disable_status() == 0) return VNET_API_ERROR_LISP_DISABLED;
...
{
    if (lcm->retry_service_index) return;

    lcm->retry_service_index = vlib_process_create(vlib_get_main(), "lisp-retry-service", send_map_resolver_service, 16 /* stack_bytes */);
}
...
u32 vnet_lisp_set_transport_protocol(u8 protocol)
{
    ...
    if (protocol < LISP_TRANSPORT_PROTOCOL_UDP || protocol > LISP_TRANSPORT_PROTOCOL_API) return VNET_API_ERROR_INVALID_ARGUMENT;
...
int vnet_lisp_enable_disable_xtr_mode(u8 is_enabled)
{
    ...
    if (pitr_mode && is_enabled) return VNET_API_ERROR_INVALID_ARGUMENT;
...
int vnet_lisp_enable_disable_pitr_mode(u8 is_enabled)
{
    ...
    if (xtr_mode && is_enabled) return VNET_API_ERROR_INVALID_VALUE;
