...
#define foreach_vpe_api_msg                                                               \
    ...
    _(SHOW_ONE_MAP_REGISTER_FALLBACK_THRESHOLD, show_one_map_register_fallback_threshold) \
    _(SHOW_ONE_STATUS, show_one_status)                                                   \
    _(ONE_ADD_DEL_MAP_REQUEST_ITR_RLOCS, one_add_del_map_request_itr_rlocs)               \
...
static locator_t *unformat_one_locs(vl_api_remote_locator_t *rmt_locs, u32 rloc_num)
{
    ...
    for (i = 0; i < rloc_num; i++) {
        ...
        loc.address.ippref.len = ip_address_max_len(loc.address.ippref.addr.version);
...
static void vl_api_show_one_map_register_ttl_t_handler(vl_api_show_one_map_register_ttl_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_SHOW_ONE_MAP_REGISTER_TTL_REPLY, ({ rmp->ttl = clib_host_to_net_u32(ttl); }));
}
...
static void vl_api_one_add_del_locator_set_t_handler(vl_api_one_add_del_locator_set_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_ONE_ADD_DEL_LOCATOR_SET_REPLY, ({ rmp->ls_index = clib_host_to_net_u32(ls_index); }));
}
...
static int unformat_one_eid_api(gid_address_t *dst, u32 vni, vl_api_eid_t *eid)
{
    ...
    switch (eid->type) {
        case EID_TYPE_API_PREFIX:
            ...
            if (prefix.fp_proto == FIB_PROTOCOL_IP4) gid_address_ip_set(dst, &prefix.fp_addr.ip4, AF_IP4);
            if (prefix.fp_proto == FIB_PROTOCOL_IP6) gid_address_ip_set(dst, &prefix.fp_addr.ip6, AF_IP6);
...
static void vl_api_one_add_del_local_eid_t_handler(vl_api_one_add_del_local_eid_t *mp)
{
    ...
    if (mp->key.id != KEY_ID_API_HMAC_NO_KEY) key = format(0, "%s", mp->key.key);
...
static void vl_api_one_eid_table_add_del_map_t_handler(vl_api_one_eid_table_add_del_map_t *mp)
{
    ...
    int                                       rv = 0;
    rv = vnet_lisp_eid_table_map(clib_net_to_host_u32(mp->vni), clib_net_to_host_u32(mp->dp_table), mp->is_l2, mp->is_add);
...
static void vl_api_show_one_map_request_mode_t_handler(vl_api_show_one_map_request_mode_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_SHOW_ONE_MAP_REQUEST_MODE_REPLY, ({ rmp->mode = vnet_lisp_get_map_request_mode(); }));
}
...
static void vl_api_one_add_del_adjacency_t_handler(vl_api_one_add_del_adjacency_t *mp)
{
    ...
    rv = unformat_one_eid_api(&a->leid, clib_net_to_host_u32(mp->vni), &mp->leid);
    rv |= unformat_one_eid_api(&a->reid, clib_net_to_host_u32(mp->vni), &mp->reid);
...
static void vl_api_one_locator_dump_t_handler(vl_api_one_locator_dump_t *mp)
{
    ...
    if (pool_is_free_index(lcm->locator_set_pool, ls_index)) return;
...
static void send_one_locator_set_details(lisp_cp_main_t *lcm, locator_set_t *lsit, vl_api_registration_t *reg, u32 context, u32 ls_index)
{
    ...
    if (lsit->local) {
        ...
        strncpy((char *) rmp->ls_name, (char *) lsit->name, vec_len(lsit->name));
    }
    else {
...
static void vl_api_one_locator_set_dump_t_handler(vl_api_one_locator_set_dump_t *mp)
{
    ...
    pool_foreach (lsit, lcm->locator_set_pool) {
        if (filter && !((1 == filter && lsit->local) || (2 == filter && !lsit->local))) {
            continue;
        }
        send_one_locator_set_details(lcm, lsit, reg, mp->context, lsit - lcm->locator_set_pool);
    }
}
...
static void send_one_eid_table_details(mapping_t *mapit, vl_api_registration_t *reg, u32 context, u8 filter)
{
    ...
    if (mapit->pitr_set || mapit->nsh_set) return;
    ...
    switch (gid_address_type(gid)) {
        ...
        case GID_ADDR_MAC:
            eid_type       = EID_TYPE_API_MAC;
            rmp->seid.type = eid_type;
            mac_address_encode((mac_address_t *) gid_address_mac(gid), rmp->seid.address.mac);
            break;
        case GID_ADDR_NSH:
            eid_type                  = EID_TYPE_API_NSH;
            rmp->seid.type            = eid_type;
            rmp->seid.address.nsh.spi = clib_host_to_net_u32(gid_address_nsh_spi(gid));
...
static void vl_api_one_eid_table_dump_t_handler(vl_api_one_eid_table_dump_t *mp)
{
    ...
    if (mp->eid_set) {
        ...
        send_one_eid_table_details(mapit, reg, mp->context, 0 /* ignore filter */);
    }
    else {
        pool_foreach (mapit, lcm->mapping_pool) {
            send_one_eid_table_details(mapit, reg, mp->context, mp->filter);
        }
    }
}
...
static void vl_api_one_eid_table_map_dump_t_handler(vl_api_one_eid_table_map_dump_t *mp)
{
    ...
    hash_foreach_pair(p, vni_table, ({ send_eid_table_map_pair(p, reg, mp->context); }));
}
...
static void one_adjacency_copy(vl_api_one_adjacency_t *dst, lisp_adjacency_t *adjs)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        switch (gid_address_type(&adj->reid)) {
            case GID_ADDR_IP_PREFIX:
                eid_type = EID_TYPE_API_PREFIX;
                ip_prefix_to_fib_prefix(&gid_address_ippref(&adj->reid), &rfib_prefix);
                ip_prefix_to_fib_prefix(&gid_address_ippref(&adj->leid), &lfib_prefix);
                ip_prefix_encode(&rfib_prefix, &a.reid.address.prefix);
                ip_prefix_encode(&lfib_prefix, &a.leid.address.prefix);
                break;
            case GID_ADDR_MAC:
                eid_type = EID_TYPE_API_PREFIX;
                mac_address_encode((mac_address_t *) gid_address_mac(&adj->reid), a.reid.address.mac);
                mac_address_encode((mac_address_t *) gid_address_mac(&adj->leid), a.leid.address.mac);
                break;
            case GID_ADDR_NSH:
                eid_type               = EID_TYPE_API_PREFIX;
                a.reid.address.nsh.spi = clib_host_to_net_u32(gid_address_nsh_spi(&adj->reid));
                a.reid.address.nsh.si  = gid_address_nsh_si(&adj->reid);
                a.leid.address.nsh.spi = clib_host_to_net_u32(gid_address_nsh_spi(&adj->leid));
...
static void vl_api_show_one_rloc_probe_state_t_handler(vl_api_show_one_rloc_probe_state_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_SHOW_ONE_RLOC_PROBE_STATE_REPLY, { rmp->is_enable = vnet_lisp_rloc_probe_state_get(); });
}
...
static void vl_api_show_one_map_register_state_t_handler(vl_api_show_one_map_register_state_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_SHOW_ONE_MAP_REGISTER_STATE_REPLY, { rmp->is_enable = vnet_lisp_map_register_state_get(); });
}
...
static void vl_api_one_eid_table_vni_dump_t_handler(vl_api_one_eid_table_vni_dump_t *mp)
{
    ...
    hash_foreach_pair(p, lcm->table_id_by_vni, ({ hash_set(vnis, p->key, 0); }));

    hash_foreach_pair(p, lcm->bd_id_by_vni, ({ hash_set(vnis, p->key, 0); }));

    hash_foreach_pair(p, vnis, ({ send_eid_table_vni(p->key, reg, mp->context); }));
...
static void vl_api_one_get_map_request_itr_rlocs_t_handler(vl_api_one_get_map_request_itr_rlocs_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_ONE_GET_MAP_REQUEST_ITR_RLOCS_REPLY,
                 ({ strncpy((char *) rmp->locator_set_name, (char *) tmp_str, ARRAY_LEN(rmp->locator_set_name) - 1); }));
...
static void vl_api_show_one_pitr_t_handler(vl_api_show_one_pitr_t *mp)
{
    ...
    u8 is_enable = (lcm->flags & LISP_FLAG_PITR_MODE) && lcm->pitr_map_index != ~0;
...
static void vl_api_show_one_stats_enable_disable_t_handler(vl_api_show_one_stats_enable_disable_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_SHOW_ONE_STATS_ENABLE_DISABLE_REPLY, ({ rmp->is_enable = vnet_lisp_stats_enable_disable_state(); }));
}
...
static void vl_api_one_ndp_bd_get_t_handler(vl_api_one_ndp_bd_get_t *mp)
{
    ...
    REPLY_MACRO4(VL_API_ONE_NDP_BD_GET_REPLY, size, {
        ...
        hash_foreach_pair(p, bds, ({ rmp->bridge_domains[i++] = clib_host_to_net_u32(p->key); }));
    });
...
static void vl_api_show_one_map_register_fallback_threshold_t_handler(vl_api_show_one_map_register_fallback_threshold_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_SHOW_ONE_MAP_REGISTER_FALLBACK_THRESHOLD_REPLY, ({ rmp->value = clib_host_to_net_u32(value); }));
}
...
static void vl_api_one_get_transport_protocol_t_handler(vl_api_one_get_transport_protocol_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_ONE_GET_TRANSPORT_PROTOCOL_REPLY, ({ rmp->protocol = proto; }));
}
...
static void vl_api_one_show_xtr_mode_t_handler(vl_api_one_show_xtr_mode_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_ONE_SHOW_XTR_MODE_REPLY, { rmp->is_enable = vnet_lisp_get_xtr_mode(); });
}
...
static void vl_api_one_show_pitr_mode_t_handler(vl_api_one_show_pitr_mode_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_ONE_SHOW_PITR_MODE_REPLY, { rmp->is_enable = vnet_lisp_get_pitr_mode(); });
}
...
static void vl_api_one_show_petr_mode_t_handler(vl_api_one_show_petr_mode_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_ONE_SHOW_PETR_MODE_REPLY, { rmp->is_enable = vnet_lisp_get_petr_mode(); });
}
