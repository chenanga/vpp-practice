...
{
    mapping_record_hdr_t *p   = vlib_buffer_put_uninit(b, sizeof(mapping_record_hdr_t));
...
u32 lisp_msg_parse_mapping_record(vlib_buffer_t *b, gid_address_t *eid, locator_t **locs, locator_t *probed_)
{
    ...
    if (!vlib_buffer_has_space(b, sizeof(mapping_record_hdr_t))) return ~0;
    ...
    vlib_buffer_pull(b, len);
    if (GID_ADDR_IP_PREFIX == gid_address_type(eid)) gid_address_ippref_len(eid) = MAP_REC_EID_PLEN(h);
    ...
    /* XXX */
    if (probed_ != 0 && probed) *probed_ = *probed;
