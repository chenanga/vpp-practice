...
size_to_write_fct size_to_write_fcts[GID_ADDR_TYPES] =
    {ip_prefix_size_to_write, lcaf_size_to_write, mac_size_to_write, sd_size_to_write, nsh_size_to_write, 0 /* arp */, no_addr_size_to_write};

serdes_fct write_fcts[GID_ADDR_TYPES] = {ip_prefix_write, lcaf_write, mac_write, sd_write, nsh_write, 0 /* arp */, no_addr_write};

cast_fct cast_fcts[GID_ADDR_TYPES] = {ip_prefix_cast, lcaf_cast, mac_cast, sd_cast, nsh_cast, no_addr_cast, no_addr_cast};

addr_len_fct addr_len_fcts[GID_ADDR_TYPES] = {ip_prefix_length, lcaf_length, mac_length, sd_length, nsh_length, 0 /* arp */, no_addr_length};

copy_fct copy_fcts[GID_ADDR_TYPES] = {ip_prefix_copy, lcaf_copy, mac_copy, sd_copy, nsh_copy, no_addr_copy, no_addr_copy};
...
#define _(cond, name) EXPAND_FCN(cond, name##_cmp),
        foreach_lcaf_type
#undef _
};

addr_len_fct lcaf_body_length_fcts[LCAF_TYPES] = {
#define _(cond, name) EXPAND_FCN(cond, name##_length),
    foreach_lcaf_type
#undef _
};

copy_fct lcaf_copy_fcts[LCAF_TYPES] = {
#define _(cond, name) EXPAND_FCN(cond, name##_copy),
    foreach_lcaf_type
#undef _
};

free_fct lcaf_free_fcts[LCAF_TYPES] = {
#define _(cond, name) EXPAND_FCN(cond, name##_free),
    foreach_lcaf_type
#undef _
};

size_to_write_fct lcaf_size_to_write_fcts[LCAF_TYPES] = {
#define _(cond, name) EXPAND_FCN(cond, name##_size_to_write),
    foreach_lcaf_type
#undef _
};

serdes_fct lcaf_write_fcts[LCAF_TYPES] = {
#define _(cond, name) EXPAND_FCN(cond, name##_write),
    foreach_lcaf_type
#undef _
};

serdes_fct lcaf_parse_fcts[LCAF_TYPES] = {
#define _(cond, name) EXPAND_FCN(cond, name##_parse),
    foreach_lcaf_type
#undef _
};
...
u8 *format_gid_address(u8 *s, va_list *args)
{
    ...
    switch (type) {
        case GID_ADDR_IP_PREFIX:
            return format(s, "[%d] %U", gid_address_vni(a), format_ip_prefix, &gid_address_ippref(a));
        case GID_ADDR_SRC_DST:
            return format(s,
                          "[%d] %U|%U",
                          gid_address_vni(a),
                          format_fid_address,
                          &gid_address_sd_src(a),
                          format_fid_address,
                          &gid_address_sd_dst(a));
        case GID_ADDR_MAC:
            return format(s, "[%d] %U", gid_address_vni(a), format_mac_address, &gid_address_mac(a));
        case GID_ADDR_ARP:
        case GID_ADDR_NDP:
            return format(s, "[%d, %U]", gid_address_arp_ndp_bd(a), format_ip_address, &gid_address_arp_ndp_ip(a));
        case GID_ADDR_NSH:
...
uword unformat_gid_address(unformat_input_t *input, va_list *args)
{
    ...
    if (unformat(input, "%U|%U", unformat_fid_address, &sim1, unformat_fid_address, &sim2)) {
        ...
        case GID_ADDR_SRC_DST:
            switch (gid_address_sd_dst_type(g)) {
                ...
                case FID_ADDR_MAC:
                    ...
                    break;
                default:
                    clib_warning("Source/Dest address type %d not supported!", gid_address_sd_dst_type(g));
                    break;
            }
            break;
        case GID_ADDR_IP_PREFIX:
...
int nsh_cmp(void *a1, void *a2)
{
    ...
    if (n1->spi != n2->spi) return 1;
    if (n1->si != n2->si) return 1;
    return 0;
}
...
u16 try_parse_src_dst_lcaf(u8 *p, gid_address_t *a)
{
    ...
    if (LCAF_SOURCE_DEST != lcaf_type(&lcaf)) return ~0;
...
{
    if (gid_address_type(a) != GID_ADDR_LCAF) return;
...
u16 sd_write(u8 *p, void *a)
{
    ...
    *(u16 *) p = clib_host_to_net_u16(LISP_AFI_LCAF);
    ...
    u16 lcaf_len   = sizeof(lcaf_src_dst_hdr_t) + fid_addr_size_to_write(&sd_src(sd)) + fid_addr_size_to_write(&sd_dst(sd));
    LCAF_LENGTH(h) = clib_host_to_net_u16(lcaf_len);
...
u16 nsh_write(u8 *p, void *a)
{
    ...
    u32 s             = clib_host_to_net_u32(gid_address_nsh_spi(g) << 8 | gid_address_nsh_si(g));
    LCAF_SPI_SI(&spi) = s;
...
u16 vni_size_to_write(void *a)
{
    ...
    if (gid_address_type(g) == GID_ADDR_SRC_DST) /* special case where nested LCAF is supported */
...
{
    source_dest_t *sd = a;
    return sizeof(u16) + sizeof(lcaf_hdr_t) + sizeof(lcaf_src_dst_hdr_t) + fid_addr_size_to_write(&sd_src(sd)) + fid_addr_size_to_write(&sd_dst(sd));
}
...
{
    if (0 != gid_address_vni(gid)) return vni_write(b, gid);
...
{
    if (0 != gid_address_vni(gid)) return vni_size_to_write(gid);
...
int vni_cmp(void *a1, void *a2)
{
    ...
    if (vni_mask_len(v1) != vni_mask_len(v2)) return -1;
    if (vni_vni(v1) != vni_vni(v2)) return -1;
...
{
    if (fid_addr_type(a1) != fid_addr_type(a2)) return -1;
...
int sd_cmp(void *a1, void *a2)
{
    ...
    if (fid_addr_cmp(&sd_dst(sd1), &sd_dst(sd2))) return -1;
    if (fid_addr_cmp(&sd_src(sd1), &sd_src(sd2))) return -1;
    return 0;
}
...
int gid_address_cmp(gid_address_t *a1, gid_address_t *a2)
{
    ...
    int     cmp = -1;
    if (!a1 || !a2) return -1;
    if (gid_address_type(a1) != gid_address_type(a2)) return -1;
    if (gid_address_vni(a1) != gid_address_vni(a2)) return -1;
    ...
    switch (gid_address_type(a1)) {
        ...
        case GID_ADDR_LCAF:
            ...
            if (lcaf_type(lcaf1) == lcaf_type(lcaf2)) cmp = (*lcaf_cmp_fcts[lcaf_type(lcaf1)])(lcaf1, lcaf2);
            break;
        case GID_ADDR_MAC:
...
u32 locator_parse(void *b, locator_t *loc)
{
    ...
    h = b;
    if (!LOC_REACHABLE(h) && LOC_LOCAL(h)) status = 0;
...
void locator_copy(locator_t *dst, locator_t *src)
{
    ...
    if (!src->local) gid_address_copy(&dst->address, &src->address);
}

u32 locator_cmp(locator_t *l1, locator_t *l2)
{
    u32 ret = 0;
    if ((ret = gid_address_cmp(&l1->address, &l2->address)) != 0) return 1;
...
{
    if (!l->local) gid_address_free(&l->address);
}
...
void build_src_dst(gid_address_t *sd, gid_address_t *src, gid_address_t *dst)
{
    ...
    switch (gid_address_type(dst)) {
        case GID_ADDR_IP_PREFIX:
            ...
            ip_prefix_copy(&gid_address_sd_src_ippref(sd), &gid_address_ippref(src));
            ip_prefix_copy(&gid_address_sd_dst_ippref(sd), &gid_address_ippref(dst));
            break;
        case GID_ADDR_MAC:
            ...
            break;
        default:
            clib_warning("Unsupported gid type %d while conversion!", gid_address_type(dst));
            break;
    }
}
