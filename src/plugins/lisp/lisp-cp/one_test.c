...
#define LISP_PING(_lm, mp_ping)                                                                       \
    if (!(_lm)->ping_id) (_lm)->ping_id = vl_msg_api_get_msg_index((u8 *) (VL_API_CONTROL_PING_CRC)); \
...
static uword unformat_nsh_address(unformat_input_t *input, va_list *args)
{
...
static u8 *format_lisp_transport_protocol(u8 *s, va_list *args)
{
...
static void vl_api_one_locator_details_t_handler(vl_api_one_locator_details_t *mp)
{
    vat_main_t *vam = &vat_main;
    u8         *s   = 0;

    if (mp->local) {
        s = format(s, "%=16d%=16d%=16d", ntohl(mp->sw_if_index), mp->priority, mp->weight);
    }
    else {
        s = format(s, "%=16U%=16d%=16d", format_ip46_address, mp->ip_address, mp->priority, mp->weight);
    }
...
static void vl_api_one_locator_set_details_t_handler(vl_api_one_locator_set_details_t *mp)
{
    vat_main_t *vam     = &vat_main;
    u8         *ls_name = 0;
...
static void vl_api_one_get_transport_protocol_reply_t_handler(vl_api_one_get_transport_protocol_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    ...
    if (vam->async_mode) {
        ...
        print(vam->ofp, "Transport protocol: %U", format_lisp_transport_protocol, proto);
        vam->retval       = retval;
        vam->result_ready = 1;
    }
}

static void vl_api_one_add_del_locator_set_reply_t_handler(vl_api_one_add_del_locator_set_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    i32         retval = ntohl(mp->retval);
    if (vam->async_mode) {
    ...
    switch (eid->type) {
        case EID_TYPE_API_PREFIX:
            if (eid->address.prefix.address.af)
                return format(s, "%U/%d", format_ip6_address, eid->address.prefix.address.un.ip6, eid->address.prefix.len);
            return format(s, "%U/%d", format_ip4_address, eid->address.prefix.address.un.ip4, eid->address.prefix.len);
...
static u8 *format_lisp_eid_vat(u8 *s, va_list *args)
{
    ...
    if (is_src_dst) s = format(s, "%U|", format_lisp_flat_eid, seid);
...
static void vl_api_one_eid_table_details_t_handler(vl_api_one_eid_table_details_t *mp)
{
    vat_main_t *vam = &vat_main;
    u8         *s = 0, *eid = 0;
    ...
    eid = format(0, "%U", format_lisp_eid_vat, &mp->deid, &mp->seid, mp->is_src_dst);
    vec_add1(eid, 0);
...
static void vl_api_one_stats_details_t_handler(vl_api_one_stats_details_t *mp)
{
    vat_main_t    *vam  = &vat_main;
    u8            *seid = 0, *deid = 0;
...
static void vl_api_one_eid_table_map_details_t_handler(vl_api_one_eid_table_map_details_t *mp)
{
    vat_main_t *vam = &vat_main;

    u8 *line = format(0, "%=10d%=10d", clib_net_to_host_u32(mp->vni), clib_net_to_host_u32(mp->dp_table));
    print(vam->ofp, "%v", line);
    vec_free(line);
}

static void vl_api_one_eid_table_vni_details_t_handler(vl_api_one_eid_table_vni_details_t *mp)
{
    vat_main_t *vam = &vat_main;
...
static void vl_api_show_one_map_register_fallback_threshold_reply_t_handler(vl_api_show_one_map_register_fallback_threshold_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
...
static void vl_api_show_one_map_register_state_reply_t_handler(vl_api_show_one_map_register_state_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    int         retval = clib_net_to_host_u32(mp->retval);
...
static void vl_api_show_one_rloc_probe_state_reply_t_handler(vl_api_show_one_rloc_probe_state_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    int         retval = clib_net_to_host_u32(mp->retval);
...
static void vl_api_show_one_stats_enable_disable_reply_t_handler(vl_api_show_one_stats_enable_disable_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    int         retval = clib_net_to_host_u32(mp->retval);
...
static void vl_api_one_ndp_entries_get_reply_t_handler(vl_api_one_ndp_entries_get_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    u32         i, n;
    int         retval = clib_net_to_host_u32(mp->retval);
    ...
    for (i = 0; i < n; i++)
        print(vam->ofp, "%U -> %U", format_ip6_address, &mp->entries[i].ip6, format_ethernet_address, mp->entries[i].mac);
...
static void vl_api_one_l2_arp_entries_get_reply_t_handler(vl_api_one_l2_arp_entries_get_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    u32         i, n;
    int         retval = clib_net_to_host_u32(mp->retval);
    ...
    for (i = 0; i < n; i++)
        print(vam->ofp, "%U -> %U", format_ip4_address, &mp->entries[i].ip4, format_ethernet_address, mp->entries[i].mac);
...
static void vl_api_one_ndp_bd_get_reply_t_handler(vl_api_one_ndp_bd_get_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    u32         i, n;
    int         retval = clib_net_to_host_u32(mp->retval);
...
static void vl_api_one_l2_arp_bd_get_reply_t_handler(vl_api_one_l2_arp_bd_get_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    u32         i, n;
    int         retval = clib_net_to_host_u32(mp->retval);
...
static void vl_api_one_adjacencies_get_reply_t_handler(vl_api_one_adjacencies_get_reply_t *mp)
{
    vat_main_t             *vam = &vat_main;
    u32                     i, n;
    int                     retval = clib_net_to_host_u32(mp->retval);
    ...
    for (i = 0; i < n; i++) {
        a = &mp->adjacencies[i];
        print(vam->ofp, "%U %40U", format_lisp_flat_eid, a->leid, format_lisp_flat_eid, a->reid);
    }
...
static void vl_api_one_map_server_details_t_handler(vl_api_one_map_server_details_t *mp)
{
    vat_main_t *vam = &vat_main;

    print(vam->ofp, "%=20U", mp->ip_address.af ? format_ip6_address : format_ip4_address, mp->ip_address.un);
}

static void vl_api_one_map_resolver_details_t_handler(vl_api_one_map_resolver_details_t *mp)
{
    vat_main_t *vam = &vat_main;
...
static void vl_api_show_one_status_reply_t_handler(vl_api_show_one_status_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    i32         retval = ntohl(mp->retval);

    if (0 <= retval) {
        print(vam->ofp, "feature: %s\ngpe: %s", mp->feature_status ? "enabled" : "disabled", mp->gpe_status ? "enabled" : "disabled");
    }
...
static void vl_api_one_get_map_request_itr_rlocs_reply_t_handler(vl_api_one_get_map_request_itr_rlocs_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    i32         retval = ntohl(mp->retval);
...
static void vl_api_show_one_map_request_mode_reply_t_handler(vl_api_show_one_map_request_mode_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    i32         retval = ntohl(mp->retval);
    ...
    if (0 <= retval) {
        u32 mode = mp->mode;
        print(vam->ofp, "map_request_mode: %U", format_lisp_map_request_mode, mode);
    }
...
static void vl_api_one_show_xtr_mode_reply_t_handler(vl_api_one_show_xtr_mode_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    i32         retval = ntohl(mp->retval);
...
static void vl_api_one_show_pitr_mode_reply_t_handler(vl_api_one_show_pitr_mode_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    i32         retval = ntohl(mp->retval);
...
static void vl_api_one_show_petr_mode_reply_t_handler(vl_api_one_show_petr_mode_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    i32         retval = ntohl(mp->retval);
...
static void vl_api_show_one_use_petr_reply_t_handler(vl_api_show_one_use_petr_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    i32         retval = ntohl(mp->retval);
    ...
    if (0 <= retval) {
        ...
        if (mp->status) {
            print(vam->ofp, "Proxy-ETR address; %U", mp->ip_address.af ? format_ip6_address : format_ip4_address, mp->ip_address.un);
        }
    }
...
static void vl_api_show_one_nsh_mapping_reply_t_handler(vl_api_show_one_nsh_mapping_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    i32         retval = ntohl(mp->retval);

    if (0 <= retval) {
        print(vam->ofp, "%-20s%-16s", mp->is_set ? "set" : "not-set", mp->is_set ? (char *) mp->locator_set_name : "");
    }
...
static void vl_api_show_one_map_register_ttl_reply_t_handler(vl_api_show_one_map_register_ttl_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    i32         retval = ntohl(mp->retval);
...
static void vl_api_show_one_pitr_reply_t_handler(vl_api_show_one_pitr_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    i32         retval = ntohl(mp->retval);

    if (0 <= retval) {
        print(vam->ofp, "%-20s%-16s", mp->status ? "enabled" : "disabled", mp->status ? (char *) mp->locator_set_name : "");
    }
...
static uword unformat_lisp_eid_vat(unformat_input_t *input, va_list *args)
{
...
static void lisp_eid_put_vat(vl_api_eid_t *eid, const lisp_eid_vat_t *vat_eid)
{
    eid->type = vat_eid->type;
    switch (eid->type) {
        case EID_TYPE_API_PREFIX:
            if (ip46_address_is_ip4(&vat_eid->addr.ip)) {
                clib_memcpy(&eid->address.prefix.address.un.ip4, &vat_eid->addr.ip.ip4, 4);
                eid->address.prefix.address.af = ADDRESS_IP4;
                eid->address.prefix.len        = vat_eid->len;
            }
            else {
                clib_memcpy(&eid->address.prefix.address.un.ip6, &vat_eid->addr.ip.ip6, 16);
                eid->address.prefix.address.af = ADDRESS_IP6;
                eid->address.prefix.len        = vat_eid->len;
            }
            return;
        case EID_TYPE_API_MAC:
            clib_memcpy(&eid->address.mac, &vat_eid->addr.mac, sizeof(eid->address.mac));
            return;
        case EID_TYPE_API_NSH:
            clib_memcpy(&eid->address.nsh, &vat_eid->addr.nsh, sizeof(eid->address.nsh));
            return;
        default:
            ASSERT(0);
            return;
...
static int api_one_add_del_locator_set(vat_main_t *vam)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(input, "sw_if_index %u p %u w %u", &sw_if_index, &priority, &weight)) {
            locator.sw_if_index = htonl(sw_if_index);
            locator.priority    = priority;
            locator.weight      = weight;
            vec_add1(locators, locator);
        }
        else if (unformat(input, "iface %U p %u w %u", unformat_sw_if_index, vam, &sw_if_index, &priority, &weight)) {
            locator.sw_if_index = htonl(sw_if_index);
            locator.priority    = priority;
            locator.weight      = weight;
            vec_add1(locators, locator);
    ...
    mp->is_add = is_add;
    clib_memcpy(mp->locator_set_name, locator_set_name, vec_len(locator_set_name));
    vec_free(locator_set_name);

    mp->locator_num = clib_host_to_net_u32(vec_len(locators));
    if (locators) clib_memcpy(mp->locators, locators, data_len);
    vec_free(locators);
...
static int api_one_add_del_locator(vat_main_t *vam)
{
    unformat_input_t             *input = vam->input;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(input, "iface %U", unformat_sw_if_index, vam, &tmp_if_index)) {
...
static int api_one_add_del_local_eid(vat_main_t *vam)
{
    unformat_input_t               *input = vam->input;
...
static int api_one_add_del_map_server(vat_main_t *vam)
{
    unformat_input_t                *input = vam->input;
...
static int api_one_add_del_map_resolver(vat_main_t *vam)
{
    unformat_input_t                  *input = vam->input;
...
static int api_one_rloc_probe_enable_disable(vat_main_t *vam)
{
    unformat_input_t                       *input = vam->input;
...
static int api_one_map_register_enable_disable(vat_main_t *vam)
{
    unformat_input_t                         *input = vam->input;
...
static int api_one_enable_disable(vat_main_t *vam)
{
    unformat_input_t            *input = vam->input;
...
static int api_one_enable_disable_xtr_mode(vat_main_t *vam)
{
    unformat_input_t                     *input = vam->input;
...
static int api_one_show_xtr_mode(vat_main_t *vam)
{
...
static int api_one_enable_disable_pitr_mode(vat_main_t *vam)
{
    unformat_input_t                      *input = vam->input;
...
static int api_one_show_pitr_mode(vat_main_t *vam)
{
...
static int api_one_enable_disable_petr_mode(vat_main_t *vam)
{
    unformat_input_t                      *input = vam->input;
...
static int api_one_show_petr_mode(vat_main_t *vam)
{
...
static int api_show_one_map_register_state(vat_main_t *vam)
{
...
static int api_show_one_rloc_probe_state(vat_main_t *vam)
{
...
static int api_one_add_del_ndp_entry(vat_main_t *vam)
{
...
static int api_one_add_del_l2_arp_entry(vat_main_t *vam)
{
...
static int api_one_ndp_bd_get(vat_main_t *vam)
{
...
static int api_one_ndp_entries_get(vat_main_t *vam)
{
...
static int api_one_l2_arp_bd_get(vat_main_t *vam)
{
...
static int api_one_l2_arp_entries_get(vat_main_t *vam)
{
...
static int api_one_stats_enable_disable(vat_main_t *vam)
{
...
static int api_show_one_stats_enable_disable(vat_main_t *vam)
{
...
static int api_show_one_map_request_mode(vat_main_t *vam)
{
...
static int api_one_map_request_mode(vat_main_t *vam)
{
    unformat_input_t              *input = vam->input;
 ...
 * @return return code
 */
static int api_one_pitr_set_locator_set(vat_main_t *vam)
{
    u8                                 ls_name_set = 0;
    unformat_input_t                  *input       = vam->input;
...
static int api_one_nsh_set_locator_set(vat_main_t *vam)
{
    u8                                ls_name_set = 0;
    unformat_input_t                 *input       = vam->input;
...
static int api_show_one_pitr(vat_main_t *vam)
{
...
static int api_one_use_petr(vat_main_t *vam)
{
    unformat_input_t      *input = vam->input;
...
static int api_show_one_nsh_mapping(vat_main_t *vam)
{
...
static int api_show_one_use_petr(vat_main_t *vam)
{
    vl_api_show_one_use_petr_t *mp;
    int                         ret;
 ...
 */
static int api_one_eid_table_add_del_map(vat_main_t *vam)
{
    unformat_input_t                   *input = vam->input;
 ...
 * @return return code
 */
static int api_one_add_del_remote_mapping(vat_main_t *vam)
{
    unformat_input_t                    *input = vam->input;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(input, "action %U", unformat_negative_mapping_action, &action)) {
            ;
        }
        else {
 ...
 * @return return code
 */
static int api_one_add_del_adjacency(vat_main_t *vam)
{
    unformat_input_t               *input = vam->input;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(input, "reid %U/%d", unformat_ip46_address, &reid.addr.ip, &reid.len)) {
            reid.type = 0; /* ipv4 */
        }
        else if (unformat(input, "reid %U", unformat_ethernet_address, &reid.addr.mac)) {
            reid.type = 1; /* mac */
        }
        else if (unformat(input, "leid %U/%d", unformat_ip46_address, &leid.addr.ip, &leid.len)) {
            leid.type = 0; /* ipv4 */
        }
        else if (unformat(input, "leid %U", unformat_ethernet_address, &leid.addr.mac)) {
            leid.type = 1; /* mac */
        }
        else if (unformat(input, "vni %d", &vni)) {
            ;
...
static int api_one_map_register_fallback_threshold(vat_main_t *vam)
{
    unformat_input_t                             *input = vam->input;
...
static int api_show_one_map_register_fallback_threshold(vat_main_t *vam)
{
...
uword unformat_lisp_transport_protocol(unformat_input_t *input, va_list *args)
{
    u32 *proto = va_arg(*args, u32 *);
...
static int api_one_set_transport_protocol(vat_main_t *vam)
{
    unformat_input_t                    *input = vam->input;
...
static int api_one_get_transport_protocol(vat_main_t *vam)
{
...
static int api_one_map_register_set_ttl(vat_main_t *vam)
{
    unformat_input_t                  *input = vam->input;
...
static int api_show_one_map_register_ttl(vat_main_t *vam)
{
 ...
 * @return return code
 */
static int api_one_add_del_map_request_itr_rlocs(vat_main_t *vam)
{
    unformat_input_t                           *input = vam->input;
...
static int api_one_locator_dump(vat_main_t *vam)
{
    unformat_input_t          *input = vam->input;
    ...
    if (is_index_set)
        ...
        vec_add1(ls_name, 0);
        strncpy((char *) mp->ls_name, (char *) ls_name, sizeof(mp->ls_name) - 1);
    }
...
static int api_one_locator_set_dump(vat_main_t *vam)
{
...
static int api_one_eid_table_map_dump(vat_main_t *vam)
{
    u8                               is_l2    = 0;
    u8                               mode_set = 0;
    unformat_input_t                *input    = vam->input;
...
static int api_one_eid_table_vni_dump(vat_main_t *vam)
{
...
static int api_one_eid_table_dump(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "eid %U/%d", unformat_ip46_address, &eid.addr.ip, &eid.len)) {
            eid_set  = 1;
            eid.type = 0;
        }
        else if (unformat(i, "eid %U", unformat_ethernet_address, &eid.addr.mac)) {
            eid_set  = 1;
            eid.type = 1;
        }
    ...
    if (!vam->json_output) {
        print(vam->ofp, "%-35s%-20s%-30s%-20s%-20s%-10s%-20s", "EID", "type", "ls_index", "ttl", "authoritative", "key_id", "key");
    }
...
static int api_one_adjacencies_get(vat_main_t *vam)
{
    unformat_input_t             *i = vam->input;
...
static int api_one_map_server_dump(vat_main_t *vam)
{
...
static int api_one_map_resolver_dump(vat_main_t *vam)
{
...
static int api_one_stats_flush(vat_main_t *vam)
{
...
static int api_one_stats_dump(vat_main_t *vam)
{
...
static int api_show_one_status(vat_main_t *vam)
{
...
static int api_one_get_map_request_itr_rlocs(vat_main_t *vam)
{
