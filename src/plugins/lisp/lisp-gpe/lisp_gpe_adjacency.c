...
static void lisp_gpe_adj_stack_one(lisp_gpe_adjacency_t *ladj, adj_index_t ai)
{
    ...
    adj_nbr_midchain_stack_on_fib_entry(ai, lgt->fib_entry_index, lisp_gpe_adj_get_fib_chain_type(ladj));
}
...
static void lisp_gpe_adj_stack(lisp_gpe_adjacency_t *ladj)
{
     ...
     */
    adj_nbr_walk_nh(ladj->sw_if_index, nh_proto, &nh, lisp_gpe_adj_walk_cb, ladj);
}
...
static void lisp_gpe_increment_stats_counters(lisp_cp_main_t *lcm, const ip_adjacency_t *adj, vlib_buffer_t *b)
{
    ...
    if (PREDICT_FALSE(~0 == di)) {
        clib_warning("dst mapping not found (%U, %U)", format_gid_address, &src, format_gid_address, &dst);
        return;
    }

    feip = hash_get(lcm->fwd_entry_by_mapping_index, di);
    if (PREDICT_FALSE(!feip)) return;
    ...
    vlib_increment_combined_counter(&lgm->counters, vlib_get_thread_index(), p[0], 1, bytes);
}
...
static void lisp_gpe_fixup(vlib_main_t *vm, const ip_adjacency_t *adj, vlib_buffer_t *b, const void *data)
{
    ...
    if (lcm->flags & LISP_FLAG_STATS_ENABLED) lisp_gpe_increment_stats_counters(lcm, adj, b);

    /* Fixup the checksum and len fields in the LISP tunnel encap
     * that was applied at the midchain node */
    ip_udp_fixup_one(vm, b, is_v4_packet(vlib_buffer_get_current(b)), UDP_ENCAP_FIXUP_NONE);
}
...
void lisp_gpe_update_adjacency(vnet_main_t *vnm, u32 sw_if_index, adj_index_t ai)
{
    ...
    adj_nbr_midchain_update_rewrite(ai,
                                    ...
                                    NULL,
                                    af,
                                    lisp_gpe_tunnel_build_rewrite(lgt, ladj, lisp_gpe_adj_proto_from_vnet_link_type(linkt)));
...
index_t lisp_gpe_adjacency_find_or_create_and_lock(const locator_pair_t *pair, u32 overlay_table_id, u32 vni)
{
     ...
     */
    l3si = lisp_gpe_sub_interface_find_or_create_and_lock(&pair->lcl_loc, overlay_table_id, vni);
    ...
    if (INDEX_INVALID == lai) {
        ...
        if (ladj->vni) ladj->flags = LISP_GPE_FLAGS_I;
         ...
         */
        ladj->fib_entry_child_index = fib_entry_child_add(lgt->fib_entry_index, FIB_NODE_TYPE_LISP_ADJ, lai);
...
{
    return ((lisp_gpe_adjacency_t *) ((char *) node - STRUCT_OFFSET_OF(lisp_gpe_adjacency_t, fib_node)));
}
...
u8 *format_lisp_gpe_adjacency(u8 *s, va_list *args)
{
    ...
    lisp_gpe_adjacency_format_flags_t flags = va_arg(*args, lisp_gpe_adjacency_format_flags_t);
    ...
    if (flags & LISP_GPE_ADJ_FORMAT_FLAG_DETAIL) {
        s = format(s, " %U\n", format_lisp_gpe_sub_interface, lisp_gpe_sub_interface_get(ladj->lisp_l3_sub_index));
        s = format(s, " %U\n", format_lisp_gpe_tunnel, lisp_gpe_tunnel_get(ladj->tunnel_index));
    }
    else {
        s = format(s, " LISP L3 sub-interface index: %d,", ladj->lisp_l3_sub_index);
...
static clib_error_t *lisp_gpe_adjacency_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (pool_elts(lisp_adj_pool) == 0) vlib_cli_output(vm, "No lisp-gpe Adjacencies");

    if (unformat(input, "%d", &index)) {
        ladj = lisp_gpe_adjacency_get_i(index);
        vlib_cli_output(vm, "%U", format_lisp_gpe_adjacency, ladj, LISP_GPE_ADJ_FORMAT_FLAG_DETAIL);
    }
    else {
        pool_foreach (ladj, lisp_adj_pool) {
            vlib_cli_output(vm, "[%d] %U\n", ladj - lisp_adj_pool, format_lisp_gpe_adjacency, ladj, LISP_GPE_ADJ_FORMAT_FLAG_NONE);
        }
    }
...
{
    BV(clib_bihash_init)(&lisp_adj_db, "Adjacency Neighbour table", LISP_ADJ_NBR_DEFAULT_HASH_NUM_BUCKETS, LISP_ADJ_NBR_DEFAULT_HASH_MEMORY_SIZE);
