    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                lisp_gpe_tx_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
...
u8 *format_lisp_gpe_header_with_length(u8 *s, va_list *args)
{
    ...
    if (max_header_bytes != 0 && header_bytes > max_header_bytes) return format(s, "lisp-gpe header truncated");

    s = format(s, "flags: ");
#define _(n, v) \
    if (h->flags & v) s = format(s, "%s ", #n);
...
static uword l2_lisp_gpe_interface_tx(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            /* lookup dst + src mac */
            lbi0                                   = lisp_l2_fib_lookup(lgm, vnet_buffer(b0)->l2.bd_index, e0->src_address, e0->dst_address);
            ...
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                l2_lisp_gpe_tx_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
                tr->dpo_index              = lbi0;
            }
            vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, bi0, l2_arc_to_lb);
        }
...
static uword nsh_lisp_gpe_interface_tx(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                nsh_lisp_gpe_tx_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
...
static vnet_hw_interface_t *lisp_gpe_create_iface(lisp_gpe_main_t *lgm, u32 vni, u32 dp_table, vnet_device_class_t *dev_class, tunnel_lookup_t *tuns)
{
    ...
    if (flen > 0) {
        ...
        vlib_zero_combined_counter(&im->combined_sw_if_counters[VNET_INTERFACE_COUNTER_TX], hi->sw_if_index);
        vlib_zero_combined_counter(&im->combined_sw_if_counters[VNET_INTERFACE_COUNTER_RX], hi->sw_if_index);
        vlib_zero_simple_counter(&im->sw_if_counters[VNET_INTERFACE_COUNTER_DROP], hi->sw_if_index);
        vnet_interface_counter_unlock(im);
    }
    else {
        hw_if_index = vnet_register_interface(vnm, dev_class->index, vni, lisp_gpe_hw_class.index, 0);
        hi          = vnet_get_hw_interface(vnm, hw_if_index);
    }
...
static void lisp_gpe_iface_set_table(u32 sw_if_index, u32 table_id)
{
    ...
    fib_index                                      = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, table_id, FIB_SOURCE_LISP);
    ip4_main.fib_index_by_sw_if_index[sw_if_index] = fib_index;
    ip4_sw_interface_enable_disable(sw_if_index, 1);

    fib_index                                      = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP6, table_id, FIB_SOURCE_LISP);
...
static void lisp_gpe_tenant_add_default_routes(u32 table_id)
{
    ...
    FOR_EACH_FIB_IP_PROTOCOL(proto)
    {
         ...
         */
        fib_index = fib_table_find_or_create_and_lock(prefix.fp_proto, table_id, FIB_SOURCE_LISP);
        fib_table_entry_special_dpo_add(fib_index, &prefix, FIB_SOURCE_LISP, FIB_ENTRY_FLAG_EXCLUSIVE, lisp_cp_dpo_get(fib_proto_to_dpo(proto)));
    }
}
...
u32 lisp_gpe_add_l3_iface(lisp_gpe_main_t *lgm, u32 vni, u32 table_id, u8 with_default_routes)
{
    ...
    hi = lisp_gpe_create_iface(lgm, vni, table_id, &lisp_gpe_device_class, l3_ifaces);

    /* insert default routes that point to lisp-cp lookup */
    lisp_gpe_iface_set_table(hi->sw_if_index, table_id);
    if (with_default_routes) lisp_gpe_tenant_add_default_routes(table_id);
...
u32 lisp_gpe_add_l2_iface(lisp_gpe_main_t *lgm, u32 vni, u32 bd_id)
{
    ...
    /* create lisp iface and populate tunnel tables */
    hi = lisp_gpe_create_iface(lgm, vni, bd_index, &l2_lisp_gpe_device_class, &lgm->l2_ifaces);

    /* enable interface */
    vnet_sw_interface_set_flags(vnm, hi->sw_if_index, VNET_SW_INTERFACE_FLAG_ADMIN_UP);
    vnet_hw_interface_set_flags(vnm, hi->hw_if_index, VNET_HW_INTERFACE_FLAG_LINK_UP);

    l2_arc_to_lb = vlib_node_add_named_next(vlib_get_main(), hi->tx_node_index, "l2-load-balance");

    /* we're ready. add iface to l2 bridge domain */
    set_int_l2_mode(lgm->vlib_main, vnm, MODE_L2_BRIDGE, hi->sw_if_index, bd_index, L2_BD_PORT_TYPE_NORMAL, 0, 0);
...
void lisp_gpe_del_l2_iface(lisp_gpe_main_t *lgm, u32 vni, u32 bd_id)
{
    ...
    set_int_l2_mode(lgm->vlib_main, lgm->vnet_main, MODE_L3, hi->sw_if_index, 0, L2_BD_PORT_TYPE_NORMAL, 0, 0);
...
u32 vnet_lisp_gpe_add_nsh_iface(lisp_gpe_main_t *lgm)
{
    ...
    /* create lisp iface and populate tunnel tables */
    hi = lisp_gpe_create_iface(lgm, 0, 0, &nsh_lisp_gpe_device_class, &lgm->nsh_ifaces);
...
static clib_error_t *lisp_gpe_add_del_iface_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (vrf_is_set && bd_index_is_set) {
        error = clib_error_return(0, "Cannot set both vrf and brdige domain index!");
        goto done;
    }
