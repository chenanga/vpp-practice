...
static clib_error_t *lisp_gpe_add_del_fwd_entry_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    u8               reid_set = 0, leid_set = 0, is_negative = 0, dp_table_set = 0, vni_set = 0;
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(line_input, "negative action %U", unformat_negative_mapping_action, &action)) {
            is_negative = 1;
        }
        else if (unformat(line_input, "loc-pair %U %U w %d", unformat_ip_address, &lloc, unformat_ip_address, &rloc, &w)) {
    ...
    if (!leid_set) {
        ...
        if (GID_ADDR_IP_PREFIX == gid_address_type(reid)) gid_address_ip_version(leid) = gid_address_ip_version(reid);
    }
    ...
    if (0 != rv) {
        vlib_cli_output(vm, "failed to %s gpe tunnel!", is_add ? "add" : "delete");
    }
...
clib_error_t *vnet_lisp_gpe_enable_disable(vnet_lisp_gpe_enable_disable_args_t *a)
{
    ...
    if (a->is_en) {
        lgm->is_en = 1;
        udp_register_dst_port(vm, UDP_DST_PORT_lisp_gpe, lisp_gpe_ip4_input_node.index, 1 /* is_ip4 */);
        udp_register_dst_port(vm, UDP_DST_PORT_lisp_gpe6, lisp_gpe_ip6_input_node.index, 0 /* is_ip4 */);
    }
    else {
        /* remove all entries */
...
int vnet_gpe_set_encap_mode(gpe_encap_mode_t mode)
{
    ...
    if (mode >= GPE_ENCAP_COUNT) return VNET_API_ERROR_INVALID_GPE_MODE;

    if (pool_elts(lgm->lisp_fwd_entry_pool) != 0) return VNET_API_ERROR_LISP_GPE_ENTRIES_PRESENT;
...
static clib_error_t *gpe_set_encap_mode_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (rv) {
        return clib_error_return(0, "Error: invalid mode or GPE entries are present!");
    }
...
static clib_error_t *lisp_show_iface_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    hash_foreach_pair(p, lgm->l3_ifaces.hw_if_index_by_dp_table, ({ vlib_cli_output(vm, "%=10d%=10d", p->key, p->value[0]); }));

    if (0 != lgm->l2_ifaces.hw_if_index_by_dp_table) {
        vlib_cli_output(vm, "%=10s%=12s", "bd_id", "hw_if_index");
        hash_foreach_pair(p, lgm->l2_ifaces.hw_if_index_by_dp_table, ({ vlib_cli_output(vm, "%=10d%=10d", p->key, p->value[0]); }));
    }
    return 0;
}
...
static clib_error_t *gpe_show_native_fwd_rpath_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (vec_len(lgm->native_fwd_rpath[AF_IP4])) {
        vec_foreach (rpath, lgm->native_fwd_rpath[AF_IP4]) {
            vlib_cli_output(vm,
                            "nh: %U fib_index %u sw_if_index %u",
                            format_ip46_address,
                            &rpath->frp_addr,
                            IP46_TYPE_IP4,
                            rpath->frp_fib_index,
...
void gpe_update_native_fwd_path(u8 ip_version)
{
    ...
    vec_foreach (lfei, lgm->native_fwd_lfes[ip_version]) {
        ...
        fib_table_entry_update(lfe->eid_fib_index, &fib_prefix, FIB_SOURCE_LISP, FIB_ENTRY_FLAG_NONE, lgm->native_fwd_rpath[ip_version]);
    }
}
    ...
    if (a->is_add) {
        ...
        vec_foreach (rpath, lgm->native_fwd_rpath[ip_version]) {
            if (!fib_route_path_cmp(rpath, &a->rpath)) {
                vec_del1(lgm->native_fwd_rpath[ip_version], rpath - lgm->native_fwd_rpath[ip_version]);
                break;
            }
        }
    }
...
static clib_error_t *gpe_native_forward_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(line_input, "via %U", unformat_ip4_address, &rpath.frp_addr.ip4)) {
            rpath.frp_weight      = 1;
            rpath.frp_sw_if_index = ~0;
            rpath.frp_proto       = DPO_PROTO_IP4;
        }
        else if (unformat(line_input, "via %U", unformat_ip6_address, &rpath.frp_addr.ip6)) {
            rpath.frp_weight      = 1;
            rpath.frp_sw_if_index = ~0;
            rpath.frp_proto       = DPO_PROTO_IP6;
        }
    ...
    if ((u32) ~0 == table_id) {
        ...
        rpath.frp_fib_index = fib_table_find(dpo_proto_to_fib(rpath.frp_proto), table_id);
...
clib_error_t *lisp_gpe_init(vlib_main_t *vm)
{
    ...
    lgm->lisp_gpe_fwd_entries = hash_create_mem(0, sizeof(lisp_gpe_fwd_entry_key_t), sizeof(uword));

    lgm->lisp_stats_index_by_key = hash_create_mem(0, sizeof(lisp_stats_key_t), sizeof(uword));
...
static clib_error_t *lisp_gpe_test_send_nsh_packet(u8 *file_name)
{
    ...
    if (!file_name) return clib_error_create("no pcap file specified!");
    ...
    u32 bi;
    if (vlib_buffer_alloc(lgm->vlib_main, &bi, 1) != 1) return clib_error_create("cannot allocate memory!");
    ...
    if (hip == 0) return clib_error_create("The NSH 0 interface doesn't exist");
    ...
    vlib_node_t *n = vlib_get_node_by_name(lgm->vlib_main, (u8 *) "interface-output-arc-end");
