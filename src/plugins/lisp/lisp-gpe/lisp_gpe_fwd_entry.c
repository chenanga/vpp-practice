...
static u32 ip_dst_fib_add_route(u32 dst_fib_index, const ip_prefix_t *dst_prefix)
{
     ...
     */
    if (dst_fei == FIB_NODE_INDEX_INVALID || NULL == fib_entry_get_source_data(dst_fei, FIB_SOURCE_LISP)) {
        dpo_id_t src_lkup_dpo = DPO_INVALID;

        /* create a new src FIB.  */
        src_fib_index = fib_table_create_and_lock(dst_fib_prefix.fp_proto,
                                                  FIB_SOURCE_LISP,
                                                  "LISP-src for [%d,%U]",
                                                  dst_fib_index,
                                                  format_fib_prefix,
        ...
        lookup_dpo_add_or_lock_w_fib_index(src_fib_index,
                                           (ip_prefix_version(dst_prefix) == AF_IP6 ? DPO_PROTO_IP6 : DPO_PROTO_IP4),
                                           LOOKUP_UNICAST,
         ...
         */
        dst_fei = fib_table_entry_special_dpo_add(dst_fib_index, &dst_fib_prefix, FIB_SOURCE_LISP, FIB_ENTRY_FLAG_EXCLUSIVE, &src_lkup_dpo);
         ...
         */
        src_fib_index = *(u32 *) fib_entry_get_source_data(dst_fei, FIB_SOURCE_LISP);
    }
...
static void ip_src_dst_fib_del_route(u32 src_fib_index, const ip_prefix_t *src_prefix, u32 dst_fib_index, const ip_prefix_t *dst_prefix)
{
    ...
    if (fib_table_lookup_exact_match(src_fib_index, &default_pref) != FIB_NODE_INDEX_INVALID) have_default = 1;

    n_entries = fib_table_get_num_entries(src_fib_index, src_fib_prefix.fp_proto, FIB_SOURCE_LISP);
    if (n_entries == 0 || (have_default && n_entries == 1)) {
        /*
         * remove src FIB default route
         */
        if (have_default) fib_table_entry_special_remove(src_fib_index, &default_pref, FIB_SOURCE_LISP);

        /*
         * there's nothing left now, unlock the source FIB and the
         * destination route
         */
        fib_table_entry_special_remove(dst_fib_index, &dst_fib_prefix, FIB_SOURCE_LISP);
        fib_table_unlock(src_fib_index, src_fib_prefix.fp_proto, FIB_SOURCE_LISP);
    }
}
...
static fib_node_index_t ip_src_fib_add_route_w_dpo(u32 src_fib_index, const ip_prefix_t *src_prefix, const dpo_id_t *src_dpo)
{
    ...
    if (FIB_NODE_INDEX_INVALID == src_fei || !fib_entry_is_sourced(src_fei, FIB_SOURCE_LISP)) {
        fei = fib_table_entry_special_dpo_add(src_fib_index, &src_fib_prefix, FIB_SOURCE_LISP, FIB_ENTRY_FLAG_EXCLUSIVE, src_dpo);
    }
    return fei;
}
...
static fib_route_path_t *lisp_gpe_mk_fib_paths(const lisp_fwd_path_t *paths)
{
    ...
    vec_foreach_index (ii, paths) {
        if (paths[0].priority != best_priority) break;
...
static fib_node_index_t ip_src_fib_add_route(u32 src_fib_index, const ip_prefix_t *src_prefix, const lisp_fwd_path_t *paths)
{
    ...
    fib_node_index_t fib_entry_index = fib_table_entry_update(src_fib_index, &src_fib_prefix, FIB_SOURCE_LISP, FIB_ENTRY_FLAG_NONE, rpaths);
    vec_free(rpaths);
...
static void gpe_native_fwd_add_del_lfe(lisp_gpe_fwd_entry_t *lfe, u8 is_add)
{
    ...
    if (is_add) {
        if (!found) vec_add1(lgm->native_fwd_lfes[ip_version], new_lfei);
    }
    else {
        if (found) vec_del1(lgm->native_fwd_lfes[ip_version], lfei[0]);
    }
}
...
static index_t create_fib_entries(lisp_gpe_fwd_entry_t *lfe)
{
    ...
    if (lfe->is_src_dst) {
        lfe->src_fib_index = ip_dst_fib_add_route(lfe->eid_fib_index, &lfe->key->rmt.ippref);
        ...
        switch (lfe->action) {
            case LISP_FORWARD_NATIVE:
                ...
                if (vec_len(lgm->native_fwd_rpath[ip_version])) {
                    ...
                    fi = fib_table_entry_update(lfe->eid_fib_index,
                                                &fib_prefix,
                                                FIB_SOURCE_LISP,
                                                FIB_ENTRY_FLAG_NONE,
...
static void delete_fib_entries(lisp_gpe_fwd_entry_t *lfe)
{
    ...
    if (lfe->is_src_dst)
        ip_src_dst_fib_del_route(lfe->src_fib_index, &lfe->key->lcl.ippref, lfe->eid_fib_index, &lfe->key->rmt.ippref);
    else {
        ip_prefix_to_fib_prefix(&lfe->key->rmt.ippref, &dst_fib_prefix);
        fib_table_entry_delete(lfe->src_fib_index, &dst_fib_prefix, FIB_SOURCE_LISP);
...
static lisp_gpe_fwd_entry_t *find_fwd_entry(lisp_gpe_main_t *lgm, vnet_lisp_gpe_add_del_fwd_entry_args_t *a, lisp_gpe_fwd_entry_key_t *key)
{
    ...
    if (GID_ADDR_IP_PREFIX == gid_address_type(&a->rmt_eid)) {
         ...
         */
        ip_prefix_version(&gid_address_ippref(&a->lcl_eid)) = ip_prefix_version(&gid_address_ippref(&a->rmt_eid));
    }
...
static void lisp_gpe_fwd_entry_mk_paths(lisp_gpe_fwd_entry_t *lfe, vnet_lisp_gpe_add_del_fwd_entry_args_t *a)
{
    ...
    vec_foreach_index (index, a->locator_pairs) {
        ...
        path->lisp_adj = lisp_gpe_adjacency_find_or_create_and_lock(&a->locator_pairs[index], a->dp_table, lfe->key->vni);
    }
...
void vnet_lisp_gpe_add_fwd_counters(vnet_lisp_gpe_add_del_fwd_entry_args_t *a, u32 fwd_entry_index)
{
    ...
    if (LISP_GPE_FWD_ENTRY_TYPE_NORMAL != lfe->type) return;
    ...
    vec_foreach (path, lfe->paths) {
        ...
        hash_set_mem(lgm->lisp_stats_index_by_key, key_copy, placeholder_elt - lgm->placeholder_stats_pool);

        vlib_validate_combined_counter(&lgm->counters, placeholder_elt - lgm->placeholder_stats_pool);
        vlib_zero_combined_counter(&lgm->counters, placeholder_elt - lgm->placeholder_stats_pool);
    }
}
...
static int add_ip_fwd_entry(lisp_gpe_main_t *lgm, vnet_lisp_gpe_add_del_fwd_entry_args_t *a)
{
    ...
    if (NULL != lfe) /* don't support updates */
    ...
    hash_set_mem(lgm->lisp_gpe_fwd_entries, lfe->key, lfe - lgm->lisp_fwd_entry_pool);
    a->fwd_entry_index = lfe - lgm->lisp_fwd_entry_pool;

    fproto = (AF_IP4 == ip_prefix_version(&fid_addr_ippref(&lfe->key->rmt)) ? FIB_PROTOCOL_IP4 : FIB_PROTOCOL_IP6);

    lfe->type          = (a->is_negative ? LISP_GPE_FWD_ENTRY_TYPE_NEGATIVE : LISP_GPE_FWD_ENTRY_TYPE_NORMAL);
    lfe->tenant        = lisp_gpe_tenant_find_or_create(lfe->key->vni);
    lfe->eid_table_id  = a->table_id;
    lfe->eid_fib_index = fib_table_find_or_create_and_lock(fproto, lfe->eid_table_id, FIB_SOURCE_LISP);
...
static u32 lisp_l2_fib_add_del_entry(u16 bd_index, u8 src_mac[6], u8 dst_mac[6], const dpo_id_t *dpo, u8 is_add)
{
    ...
    if (BV(clib_bihash_search)(&lgm->l2_fib, &kv, &value) == 0) old_val = value.value;
...
static void l2_fib_init(lisp_gpe_main_t *lgm)
{
    ...
    BV(clib_bihash_init)(&lgm->l2_fib, "l2 fib", 1 << max_log2(L2_FIB_DEFAULT_HASH_NUM_BUCKETS), L2_FIB_DEFAULT_HASH_MEMORY_SIZE);
...
static void del_l2_fwd_entry_i(lisp_gpe_main_t *lgm, lisp_gpe_fwd_entry_t *lfe)
{
    ...
    lisp_l2_fib_add_del_entry(lfe->l2.eid_bd_index, fid_addr_mac(&lfe->key->lcl), fid_addr_mac(&lfe->key->rmt), NULL, 0);
...
static int del_l2_fwd_entry(lisp_gpe_main_t *lgm, vnet_lisp_gpe_add_del_fwd_entry_args_t *a)
{
    ...
    if (NULL == lfe) return VNET_API_ERROR_INVALID_VALUE;
...
static void lisp_gpe_l2_update_fwding(lisp_gpe_fwd_entry_t *lfe)
{
    ...
    if (LISP_GPE_FWD_ENTRY_TYPE_NEGATIVE != lfe->type) {
        fib_path_list_contribute_forwarding(lfe->l2.path_list_index, FIB_FORW_CHAIN_TYPE_ETHERNET, FIB_PATH_LIST_FWD_FLAG_NONE, &lfe->l2.dpo);
    ...
    lisp_l2_fib_add_del_entry(lfe->l2.eid_bd_index, fid_addr_mac(&lfe->key->lcl), fid_addr_mac(&lfe->key->rmt), &dpo, 1);
...
static int add_l2_fwd_entry(lisp_gpe_main_t *lgm, vnet_lisp_gpe_add_del_fwd_entry_args_t *a)
{
    ...
    if (LISP_GPE_FWD_ENTRY_TYPE_NEGATIVE != lfe->type) {
        ...
        lfe->l2.path_list_index = fib_path_list_create(FIB_PATH_LIST_FLAG_NONE, rpaths);

        /*
         * become a child of the path-list so we receive updates when
         * its forwarding state changes. this includes an implicit lock.
         */
        lfe->l2.child_index = fib_path_list_child_add(lfe->l2.path_list_index, FIB_NODE_TYPE_LISP_GPE_FWD_ENTRY, lfe - lgm->lisp_fwd_entry_pool);
    }
    else {
        lfe->action = a->action;
    }
...
static u32 lisp_nsh_fib_add_del_entry(u32 spi_si_host_order, u32 lfei, u8 is_add)
{
    ...
    if (BV(clib_bihash_search)(&lgm->nsh_fib, &kv, &value) == 0) old_val = value.value;
...
{
    BV(clib_bihash_init)(&lgm->nsh_fib, "nsh fib", 1 << max_log2(NSH_FIB_DEFAULT_HASH_NUM_BUCKETS), NSH_FIB_DEFAULT_HASH_MEMORY_SIZE);
...
static void del_nsh_fwd_entry_i(lisp_gpe_main_t *lgm, lisp_gpe_fwd_entry_t *lfe)
{
    ...
    if (LISP_GPE_FWD_ENTRY_TYPE_NEGATIVE != lfe->type) {
        ...
        fib_path_list_child_remove(lfe->nsh.path_list_index, lfe->nsh.child_index);
...
static void lisp_gpe_nsh_update_fwding(lisp_gpe_fwd_entry_t *lfe)
{
    ...
    if (LISP_GPE_FWD_ENTRY_TYPE_NEGATIVE != lfe->type) {
        fib_path_list_contribute_forwarding(lfe->nsh.path_list_index, FIB_FORW_CHAIN_TYPE_NSH, FIB_PATH_LIST_FWD_FLAG_NONE, &lfe->nsh.dpo);
        ...
        if (DPO_LOAD_BALANCE == lfe->nsh.dpo.dpoi_type) {
            ...
            tmp  = load_balance_get_bucket_i(lb, hash & lb->lb_n_buckets_minus_1);
    ...
    lisp_nsh_fib_add_del_entry(fid_addr_nsh(&lfe->key->rmt), lfe - lgm->lisp_fwd_entry_pool, 1);
    dpo_reset(&dpo);
}
...
static int add_nsh_fwd_entry(lisp_gpe_main_t *lgm, vnet_lisp_gpe_add_del_fwd_entry_args_t *a)
{
    ...
    if (LISP_GPE_FWD_ENTRY_TYPE_NEGATIVE != lfe->type) {
        ...
        lfe->nsh.path_list_index = fib_path_list_create(FIB_PATH_LIST_FLAG_NONE, rpaths);

        /*
         * become a child of the path-list so we receive updates when
         * its forwarding state changes. this includes an implicit lock.
         */
        lfe->nsh.child_index = fib_path_list_child_add(lfe->nsh.path_list_index, FIB_NODE_TYPE_LISP_GPE_FWD_ENTRY, lfe - lgm->lisp_fwd_entry_pool);
    }
    else {
        lfe->action = a->action;
    }
...
{
    return ((lisp_gpe_fwd_entry_t *) (((char *) node) - STRUCT_OFFSET_OF(lisp_gpe_fwd_entry_t, node)));
}
...
static u8 *format_lisp_fwd_path(u8 *s, va_list *ap)
{
    ...
    s = format(s, "adj:[%U]\n", format_lisp_gpe_adjacency, lisp_gpe_adjacency_get(lfp->lisp_adj), LISP_GPE_ADJ_FORMAT_FLAG_NONE);
...
static u8 *format_lisp_gpe_fwd_entry(u8 *s, va_list *ap)
{
    ...
    lisp_gpe_fwd_entry_format_flag_t flags = va_arg(*ap, lisp_gpe_fwd_entry_format_flag_t);
    ...
    if (LISP_GPE_FWD_ENTRY_TYPE_NEGATIVE == lfe->type) {
        s = format(s, "\n Negative - action:%U", format_negative_mapping_action, lfe->action);
    }
    else {
        lisp_fwd_path_t *path;
...
static clib_error_t *lisp_gpe_fwd_entry_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (unformat(input, "vni %d", &vni))
        ...
        if (!pool_is_free_index(lgm->lisp_fwd_entry_pool, index)) {
            ...
            vlib_cli_output(vm, "[%d@] %U", index, format_lisp_gpe_fwd_entry, lfe, LISP_GPE_FWD_ENTRY_FORMAT_DETAIL);
        }
        else {
    ...
    pool_foreach (lfe, lgm->lisp_fwd_entry_pool) {
        if ((vni == ~0) || (lfe->key->vni == vni)) vlib_cli_output(vm, "%U", format_lisp_gpe_fwd_entry, lfe, LISP_GPE_FWD_ENTRY_FORMAT_NONE);
    }
...
clib_error_t *lisp_gpe_fwd_entry_init(vlib_main_t *vm)
{
    ...
    if ((error = vlib_call_init_function(vm, lisp_cp_dpo_module_init))) return (error);
...
lisp_api_gpe_fwd_entry_t *vnet_lisp_gpe_fwd_entries_get_by_vni(u32 vni)
{
    ...
    pool_foreach (lfe, lgm->lisp_fwd_entry_pool) {
        if (lfe->key->vni == vni) {
            ...
            e.vni      = lfe->key->vni;
            if (lfe->type == LISP_GPE_FWD_ENTRY_TYPE_NEGATIVE) e.action = lfe->action;
...
int vnet_lisp_gpe_get_fwd_stats(vnet_lisp_gpe_add_del_fwd_entry_args_t *a, vlib_counter_t *c)
{
    ...
    if (NULL == lfe) return -1;

    if (LISP_GPE_FWD_ENTRY_TYPE_NEGATIVE == lfe->type) return -1;

    if (~0 == lfe->dpoi_index) return -1;

    vlib_get_combined_counter(&load_balance_main.lbm_to_counters, lfe->dpoi_index, c);
    return 0;
}
