...
static void vl_api_gpe_fwd_entry_path_dump_t_handler(vl_api_gpe_fwd_entry_path_dump_t *mp)
{
    ...
    if (pool_is_free_index(lgm->lisp_fwd_entry_pool, mp->fwd_entry_index)) return;

    lfe = pool_elt_at_index(lgm->lisp_fwd_entry_pool, mp->fwd_entry_index);

    if (LISP_GPE_FWD_ENTRY_TYPE_NEGATIVE == lfe->type) return;
    ...
    vec_foreach (path, lfe->paths) {
        ...
        rmp->_vl_msg_id = clib_host_to_net_u16(VL_API_GPE_FWD_ENTRY_PATH_DETAILS);

        const lisp_gpe_adjacency_t *ladj = lisp_gpe_adjacency_get(path->lisp_adj);
    ...
    vec_foreach (e, src) {
        ...
        switch (fid_addr_type(&e->leid)) {
            case FID_ADDR_IP_PREF:
                ...
                ip_prefix_encode2(&fid_addr_ippref(&e->leid), &dst[i].leid.address.prefix);
                ip_prefix_encode2(&fid_addr_ippref(&e->reid), &dst[i].reid.address.prefix);
                break;
            case FID_ADDR_MAC:
                mac_address_encode((mac_address_t *) fid_addr_mac(&e->leid), dst[i].leid.address.mac);
                mac_address_encode((mac_address_t *) fid_addr_mac(&e->reid), dst[i].reid.address.mac);
...
static void vl_api_gpe_fwd_entry_vnis_get_t_handler(vl_api_gpe_fwd_entry_vnis_get_t *mp)
{
    ...
    REPLY_MACRO4(VL_API_GPE_FWD_ENTRY_VNIS_GET_REPLY, size, {
        ...
        hash_foreach_pair(p, vnis, ({ rmp->vnis[i++] = clib_host_to_net_u32(p->key); }));
    });
...
static void vl_api_gpe_add_del_fwd_entry_t_handler(vl_api_gpe_add_del_fwd_entry_t *mp)
{
    ...
    a->vni           = mp->vni;
    a->action        = mp->action;
    if (mp->loc_num == 0) a->is_negative = 1;

    rv = vnet_lisp_gpe_add_del_fwd_entry(a, 0);
    vec_free(pairs);
send_reply:
    REPLY_MACRO2(VL_API_GPE_ADD_DEL_FWD_ENTRY_REPLY, { rmp->fwd_entry_index = clib_host_to_net_u32(a->fwd_entry_index); });
}
...
static void vl_api_gpe_add_del_iface_t_handler(vl_api_gpe_add_del_iface_t *mp)
{
    ...
    if (mp->is_l2) {
        if (mp->is_add) {
            if (~0 == lisp_gpe_tenant_l2_iface_add_or_lock(vni, dp_table)) rv = 1;
        }
        else
        ...
        if (mp->is_add) {
            if (~0 == lisp_gpe_tenant_l3_iface_add_or_lock(vni, dp_table, 1)) rv = 1;
        }
        else
...
static void vl_api_gpe_get_encap_mode_t_handler(vl_api_gpe_get_encap_mode_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_GPE_GET_ENCAP_MODE_REPLY, ({ rmp->encap_mode = vnet_gpe_get_encap_mode(); }));
}
...
static void vl_api_gpe_add_del_native_fwd_rpath_t_handler(vl_api_gpe_add_del_native_fwd_rpath_t *mp)
{
    ...
    if (mp->nh_addr.af)
        clib_memcpy(&a->rpath.frp_addr.ip6, mp->nh_addr.un.ip6, sizeof(ip6_address_t));
    else
        clib_memcpy(&a->rpath.frp_addr.ip4, mp->nh_addr.un.ip4, sizeof(ip4_address_t));

    a->is_add              = mp->is_add;
    a->rpath.frp_proto     = mp->nh_addr.af ? DPO_PROTO_IP6 : DPO_PROTO_IP4;
    a->rpath.frp_fib_index = fib_table_find(dpo_proto_to_fib(a->rpath.frp_proto), clib_net_to_host_u32(mp->table_id));
...
static void vl_api_gpe_native_fwd_rpaths_get_t_handler(vl_api_gpe_native_fwd_rpaths_get_t *mp)
{
    ...
    REPLY_MACRO4(VL_API_GPE_NATIVE_FWD_RPATHS_GET_REPLY, size, {
        ...
        gpe_native_fwd_rpaths_copy(rmp->entries, lgm->native_fwd_rpath[rpath_index]);
