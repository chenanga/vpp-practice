...
u8 *lisp_gpe_tunnel_build_rewrite(const lisp_gpe_tunnel_t *lgt, const lisp_gpe_adjacency_t *ladj, lisp_gpe_next_protocol_e payload_proto)
{
    ...
    if (GPE_ENCAP_VXLAN == encap_mode) /* unset P flag */
...
index_t lisp_gpe_tunnel_find_or_create_and_lock(const locator_pair_t *pair, u32 rloc_fib_index)
{
    ...
    if (NULL == lgt) {
        ...
        lgt->fib_entry_index = fib_table_entry_special_add(rloc_fib_index, &pfx, FIB_SOURCE_RR, FIB_ENTRY_FLAG_NONE);

        hash_set_mem(lisp_gpe_tunnel_db, lgt->key, (lgt - lisp_gpe_tunnel_pool));
    }
...
u8 *format_lisp_gpe_tunnel(u8 *s, va_list *args)
{
    ...
    s = format(s, "  local: %U remote: %U\n", format_ip_address, &lgt->key->lcl, format_ip_address, &lgt->key->rmt);
...
static clib_error_t *show_lisp_gpe_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (pool_elts(lisp_gpe_tunnel_pool) == 0) vlib_cli_output(vm, "No lisp-gpe tunnels configured...");
...
{
    lisp_gpe_tunnel_db = hash_create_mem(0, sizeof(lisp_gpe_tunnel_key_t), sizeof(uword));
