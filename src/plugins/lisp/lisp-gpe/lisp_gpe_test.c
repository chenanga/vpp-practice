...
static void vl_api_gpe_add_del_fwd_entry_reply_t_handler(vl_api_gpe_add_del_fwd_entry_reply_t *mp)
{
...
static void api_gpe_fwd_entry_net_to_host(vl_api_gpe_fwd_entry_t *e)
{
...
static void gpe_fwd_entries_get_reply_t_net_to_host(vl_api_gpe_fwd_entries_get_reply_t *mp)
{
    u32 i;
...
static u8 *format_gpe_encap_mode(u8 *s, va_list *args)
{
...
static void vl_api_gpe_get_encap_mode_reply_t_handler(vl_api_gpe_get_encap_mode_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
...
static void vl_api_gpe_fwd_entry_path_details_t_handler(vl_api_gpe_fwd_entry_path_details_t *mp)
{
    vat_main_t *vam                               = &vat_main;
    ...
    print(vam->ofp, "w:%d %30U %30U", mp->rmt_loc.weight, format_ip_address_fcn, &mp->lcl_loc.addr.un, format_ip_address_fcn, &mp->rmt_loc.addr.un);
}

static void vl_api_gpe_fwd_entries_get_reply_t_handler(vl_api_gpe_fwd_entries_get_reply_t *mp)
{
    vat_main_t             *vam = &vat_main;
    u32                     i;
    ...
    for (i = 0; i < mp->count; i++) {
        e = &mp->entries[i];
        print(vam->ofp, "%10d %10d %U %40U", e->fwd_entry_index, e->dp_table, format_lisp_flat_eid, e->leid, format_lisp_flat_eid, e->reid);
    }
...
static void vl_api_gpe_native_fwd_rpaths_get_reply_t_handler(vl_api_gpe_native_fwd_rpaths_get_reply_t *mp)
{
    vat_main_t                    *vam = &vat_main;
    u32                            i, n;
    int                            retval = clib_net_to_host_u32(mp->retval);
    ...
    for (i = 0; i < n; i++) {
        r = &mp->entries[i];
        print(vam->ofp,
              "fib_index: %d sw_if_index %d nh %U",
...
static void vl_api_gpe_fwd_entry_vnis_get_reply_t_handler(vl_api_gpe_fwd_entry_vnis_get_reply_t *mp)
{
    vat_main_t *vam = &vat_main;
    u32         i, n;
    int         retval = clib_net_to_host_u32(mp->retval);
...
static int api_gpe_add_del_fwd_entry(vat_main_t *vam)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(input, "loc-pair %U %U", unformat_ip4_address, &lcl_rloc4, unformat_ip4_address, &rmt_rloc4)) {
            rloc.addr.af = 0;
        ...
        else if (unformat(input, "loc-pair %U %U", unformat_ip6_address, &lcl_rloc6, unformat_ip6_address, &rmt_rloc6)) {
            rloc.addr.af = 1;
    ...
    M2(GPE_ADD_DEL_FWD_ENTRY, mp, sizeof(vl_api_gpe_locator_t) * vec_len(rmt_locs) * 2);
    ...
    if (0 != rmt_locs && 0 != lcl_locs) {
        ...
        clib_memcpy(mp->locs, lcl_locs, (sizeof(vl_api_gpe_locator_t) * vec_len(lcl_locs)));

        u32 offset = sizeof(vl_api_gpe_locator_t) * vec_len(lcl_locs);
        clib_memcpy(((u8 *) mp->locs) + offset, rmt_locs, (sizeof(vl_api_gpe_locator_t) * vec_len(rmt_locs)));
    }
    vec_free(lcl_locs);
    vec_free(rmt_locs);
...
static int api_gpe_enable_disable(vat_main_t *vam)
{
    unformat_input_t            *input = vam->input;
...
uword unformat_gpe_encap_mode(unformat_input_t *input, va_list *args)
{
    u32 *mode = va_arg(*args, u32 *);
...
static int api_gpe_get_encap_mode(vat_main_t *vam)
{
...
static int api_gpe_set_encap_mode(vat_main_t *vam)
{
    unformat_input_t            *input = vam->input;
...
static int api_gpe_add_del_iface(vat_main_t *vam)
{
    unformat_input_t           *input = vam->input;
...
static int api_gpe_fwd_entries_get(vat_main_t *vam)
{
    ...
    if (!vam->json_output) {
        print(vam->ofp, "%10s %10s %s %40s", "fwd_index", "dp_table", "leid", "reid");
    }
...
static int api_gpe_native_fwd_rpaths_get(vat_main_t *vam)
{
    unformat_input_t                   *i = vam->input;
...
static int api_gpe_fwd_entry_vnis_get(vat_main_t *vam)
{
...
static int api_gpe_add_del_native_fwd_rpath(vat_main_t *vam)
{
    unformat_input_t                      *i = vam->input;
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "del"))
            is_add = 0;
        else if (unformat(i, "via %U %U", unformat_ip4_address, &ip4, unformat_sw_if_index, vam, &nh_sw_if_index)) {
            ip_set = 1;
            is_ip4 = 1;
        }
        else if (unformat(i, "via %U %U", unformat_ip6_address, &ip6, unformat_sw_if_index, vam, &nh_sw_if_index)) {
            ip_set = 1;
            is_ip4 = 0;
        }
...
static int api_gpe_fwd_entry_path_dump(vat_main_t *vam)
{
