...
{
    hash_set_mem(lisp_gpe_sub_interfaces, l3s->key, l3s - lisp_gpe_sub_interface_pool);
    hash_set_mem(lisp_gpe_sub_interfaces_sw_if_index, l3s->key, l3s->sw_if_index);
}
...
{
    fib_table_unlock(ip4_main.fib_index_by_sw_if_index[sw_if_index], FIB_PROTOCOL_IP4, FIB_SOURCE_LISP);
    ip4_main.fib_index_by_sw_if_index[sw_if_index] = 0;
    ip4_sw_interface_enable_disable(sw_if_index, 0);

    fib_table_unlock(ip6_main.fib_index_by_sw_if_index[sw_if_index], FIB_PROTOCOL_IP6, FIB_SOURCE_LISP);
...
index_t lisp_gpe_sub_interface_find_or_create_and_lock(const ip_address_t *lrloc, u32 overlay_table_id, u32 vni)
{
    ...
    if (INDEX_INVALID == l3si) {
         ...
         */
        main_sw_if_index = lisp_gpe_tenant_l3_iface_add_or_lock(vni, overlay_table_id, 1 /* with_default_route */);
        ...
        if (NULL != vnet_create_sw_interface(vnet_get_main(), &sub_itf_template, &sub_sw_if_index)) return (INDEX_INVALID);
        ...
        vnet_sw_interface_set_flags(vnet_get_main(), l3s->sw_if_index, VNET_SW_INTERFACE_FLAG_ADMIN_UP);
        vnet_set_interface_l3_output_node(vlib_get_main(), l3s->sw_if_index, (u8 *) "lisp-tunnel-output");
...
static clib_error_t *lisp_gpe_sub_interface_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vlib_cli_output(vm, "%-16s%=8s%=15s%s", "Name", "VNI", "sw_if_index", "local RLOC");
...
{
    lisp_gpe_sub_interfaces             = hash_create_mem(0, sizeof(lisp_gpe_sub_interface_key_t), sizeof(uword));
    lisp_gpe_sub_interfaces_sw_if_index = hash_create_mem(0, sizeof(lisp_gpe_sub_interface_key_t), sizeof(uword));
