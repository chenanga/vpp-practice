...
static u8 *format_ip6_adl_allowlist_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "IP6_ADL_ALLOWLIST: sw_if_index %d, next index %d", t->sw_if_index, t->next_index);
    return s;
}
...
VLIB_NODE_FN(ip6_adl_allowlist_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            lb_index0 = ip6_fib_table_fwding_lookup(c0->fib_index, &ip0->src_address);
            ...
            lb_index1 = ip6_fib_table_fwding_lookup(c1->fib_index, &ip1->src_address);
            ...
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                ip6_adl_allowlist_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                t->sw_if_index               = sw_if_index0;
                t->next_index                = next0;
            }

            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b1->flags & VLIB_BUFFER_IS_TRACED))) {
                ip6_adl_allowlist_trace_t *t = vlib_add_trace(vm, node, b1, sizeof(*t));
    ...
    vlib_node_increment_counter(vm, node->node_index, IP6_ADL_ALLOWLIST_ERROR_ALLOWED, allowed_packets);
