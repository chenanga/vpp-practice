...
static u8 *format_ip4_adl_allowlist_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "IP4_ADL_ALLOWLIST: sw_if_index %d, next index %d", t->sw_if_index, t->next_index);
    return s;
}
...
VLIB_NODE_FN(ip4_adl_allowlist_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            c0 = vnet_get_config_data(&ccm0->config_main, &adl_buffer(b0)->adl.current_config_index, &next0, sizeof(c0[0]));

            lb_index0 = ip4_fib_forwarding_lookup(c0->fib_index, &ip0->src_address);

            ASSERT(lb_index0 == ip4_fib_table_lookup_lb(ip4_fib_get(c0->fib_index), &ip0->src_address));
            ...
            c1 = vnet_get_config_data(&ccm1->config_main, &adl_buffer(b1)->adl.current_config_index, &next1, sizeof(c1[0]));

            lb_index1 = ip4_fib_forwarding_lookup(c1->fib_index, &ip1->src_address);

            ASSERT(lb_index1 == ip4_fib_table_lookup_lb(ip4_fib_get(c1->fib_index), &ip1->src_address));
            lb1  = load_balance_get(lb_index1);
            dpo1 = load_balance_get_bucket_i(lb1, 0);

            vlib_increment_combined_counter(vcm, thread_index, lb_index0, 1, vlib_buffer_length_in_chain(vm, b0) + sizeof(ethernet_header_t));

            vlib_increment_combined_counter(vcm, thread_index, lb_index1, 1, vlib_buffer_length_in_chain(vm, b1) + sizeof(ethernet_header_t));
            ...
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                ip4_adl_allowlist_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                t->sw_if_index               = sw_if_index0;
                t->next_index                = next0;
            }

            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b1->flags & VLIB_BUFFER_IS_TRACED))) {
                ip4_adl_allowlist_trace_t *t = vlib_add_trace(vm, node, b1, sizeof(*t));
    ...
    vlib_node_increment_counter(vm, node->node_index, IP4_ADL_ALLOWLIST_ERROR_ALLOWED, allowed_packets);
