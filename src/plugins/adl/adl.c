...
static clib_error_t *adl_sw_interface_add_del(vnet_main_t *vnm, u32 sw_if_index, u32 is_add)
{
     ...
     */
    if (hi->dev_class_index == vnet_local_interface_device_class.index) return 0;

    for (address_family = VNET_ADL_IP4; address_family < VNET_N_ADLS; address_family++) {
        ...
        if (!(acm->config_main.node_index_by_feature_index)) {
            switch (address_family) {
                case VNET_ADL_IP4: {
                    ...
                    vnet_config_init(vm, &acm->config_main, start_nodes, ARRAY_LEN(start_nodes), feature_nodes, ARRAY_LEN(feature_nodes));
                } break;
                case VNET_ADL_IP6: {
                    static char *start_nodes[]   = {"adl-input"};
                    static char *feature_nodes[] = {
        ...
        vec_validate_init_empty(acm->config_index_by_sw_if_index, sw_if_index, ~0);
        ...
        if (is_add)
            ci = vnet_config_add_feature(vm, &acm->config_main, ci, default_next, data, sizeof(*data));
        else {
            /* If the feature was actually configured */
            if (ci != ~0) {
                ci = vnet_config_del_feature(vm, &acm->config_main, ci, default_next, data, sizeof(*data));
            }
        }
...
int adl_interface_enable_disable(u32 sw_if_index, int enable_disable)
{
     ...
     */
    vnet_feature_enable_disable("device-input", "adl-input", sw_if_index, enable_disable, 0, 0);
    return 0;
}
...
static clib_error_t *adl_enable_disable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "disable"))
            enable_disable = 0;
        else if (unformat(input, "%U", unformat_vnet_sw_interface, cm->vnet_main, &sw_if_index))
            ;
        else
            break;
    }
    ...
    switch (rv) {
        ...
        default:
            return clib_error_return(0, "adl_interface_enable_disable returned %d", rv);
    }
    return 0;
}
...
int adl_allowlist_enable_disable(adl_allowlist_enable_disable_args_t *a)
{
    ...
    for (address_family = VNET_ADL_IP4; address_family < VNET_N_ADLS; address_family++) {
        ...
        if (is_add)
            ci = vnet_config_add_feature(vm, &acm->config_main, ci, next_to_add_del, data, sizeof(*data));
        else {
            /* If the feature was actually configured... */
            if (ci != ~0) {
                /* delete it */
                ci = vnet_config_del_feature(vm, &acm->config_main, ci, next_to_add_del, data, sizeof(*data));
            }
        }
...
static clib_error_t *adl_allowlist_enable_disable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    switch (rv) {
        ...
        default:
            return clib_error_return(0, "adl_allowlist_enable_disable returned %d", rv);
    }
...
VLIB_CLI_COMMAND(adl_allowlist_command, static) = {
    .path       = "adl allowlist",
    .short_help = "adl allowlist <interface-name> [ip4][ip6][default][fib-id <NN>][disable]",
