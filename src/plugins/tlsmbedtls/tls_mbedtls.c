...
void *mbedtls_calloc_fn(size_t n, size_t size)
{
    void *ptr;
...
void mbedtls_free_fn(void *ptr)
{
    if (ptr) clib_mem_free(ptr);
}
#endif
...
static u32 mbedtls_ctx_alloc(void)
{
    ...
    pool_get_aligned_safe(tm->ctx_pool[thread_index], ctx, CLIB_CACHE_LINE_BYTES);
    if (!(*ctx)) *ctx = clib_mem_alloc(sizeof(mbedtls_ctx_t));
...
static void mbedtls_ctx_free(tls_ctx_t *ctx)
{
    ...
    if (mc->ssl.state == MBEDTLS_SSL_HANDSHAKE_OVER && !(ctx->flags & TLS_CONN_F_PASSIVE_CLOSE)) mbedtls_ssl_close_notify(&mc->ssl);
    ...
    pool_put_index(mbedtls_main.ctx_pool[ctx->c_thread_index], mc->mbedtls_ctx_index);
}

static tls_ctx_t *mbedtls_ctx_get(u32 ctx_index)
{
    mbedtls_ctx_t **ctx;
    ctx = pool_elt_at_index(mbedtls_main.ctx_pool[vlib_get_thread_index()], ctx_index);
    return &(*ctx)->ctx;
}
...
{
    u8 thread_index = vlib_get_thread_index();
    if (PREDICT_FALSE(!mbedtls_main.ctr_drbgs[thread_index].f_entropy)) tls_init_ctr_seed_drbgs();
...
static int tls_net_send(void *ctx_indexp, const unsigned char *buf, size_t len)
{
    ...
    if (rv < 0) return MBEDTLS_ERR_SSL_WANT_WRITE;
...
static int mbedtls_ctx_init_client(tls_ctx_t *ctx)
{
    ...
    if ((rv = mbedtls_ssl_config_defaults(&mc->conf, MBEDTLS_SSL_IS_CLIENT, MBEDTLS_SSL_TRANSPORT_STREAM, MBEDTLS_SSL_PRESET_DEFAULT)) != 0) {
    ...
    mbedtls_ssl_conf_rng(&mc->conf, mbedtls_ctr_drbg_random, tls_get_ctr_drbg());
    ...
    if ((rv = mbedtls_ssl_set_hostname(&mc->ssl, (const char *) ctx->srv_hostname)) != 0) {
     ...
     */
    TLS_DBG(1, "Initiating handshake for [%u]%u", ctx->c_thread_index, mc->mbedtls_ctx_index);
    while (mc->ssl.state != MBEDTLS_SSL_HANDSHAKE_OVER) {
        rv = mbedtls_ssl_handshake_step(&mc->ssl);
        if (rv != 0) break;
    }
    TLS_DBG(2, "tls state for [%u]%u is %u", ctx->c_thread_index, mc->mbedtls_ctx_index, mc->ssl.state);
    return 0;
}
...
static int mbedtls_ctx_init_server(tls_ctx_t *ctx)
{
    ...
    if (!ckpair->cert || !ckpair->key) {
        TLS_DBG(1, " failed\n  ! tls cert and/or key not configured %d", ctx->parent_app_wrk_index);
        return -1;
    }

    rv = mbedtls_x509_crt_parse(&mc->srvcert, (const unsigned char *) ckpair->cert, vec_len(ckpair->cert));
    if (rv != 0) {
        TLS_DBG(1, " failed\n  !  mbedtls_x509_crt_parse returned %d", rv);
        goto exit;
    }

    rv = mbedtls_pk_parse_key(&mc->pkey, (const unsigned char *) ckpair->key, vec_len(ckpair->key), NULL, 0);
    if (rv != 0) {
     ...
     */
    if ((rv = mbedtls_ssl_config_defaults(&mc->conf, MBEDTLS_SSL_IS_SERVER, MBEDTLS_SSL_TRANSPORT_STREAM, MBEDTLS_SSL_PRESET_DEFAULT)) != 0) {
    ...
    mbedtls_ssl_conf_ca_chain(&mc->conf, &mm->cacert, NULL);
    if ((rv = mbedtls_ssl_conf_own_cert(&mc->conf, &mc->srvcert, &mc->pkey)) != 0) {
...
static int mbedtls_ctx_handshake_rx(tls_ctx_t *ctx)
{
    ...
    if (mc->ssl.state != MBEDTLS_SSL_HANDSHAKE_OVER) return 0;
    ...
    TLS_DBG(1, "Handshake for %u complete. TLS cipher is %x", mc->mbedtls_ctx_index, mc->ssl.session->ciphersuite);
    return 0;
}
...
static int mbedtls_ctx_write(tls_ctx_t *ctx, session_t *app_session, transport_send_params_t *sp)
{
    ...
    if (deq_now < deq_max) app_session->flags |= SESSION_F_CUSTOM_TX;
...
static int mbedtls_ctx_read(tls_ctx_t *ctx, session_t *tls_session)
{
    ...
    enq = svm_fifo_enqueue(app_session->rx_fifo, read, mm->rx_bufs[thread_index]);
    ASSERT(enq == read);
    vec_reset_length(mm->rx_bufs[thread_index]);

    if (svm_fifo_max_dequeue_cons(tls_session->rx_fifo)) tls_add_vpp_q_builtin_rx_evt(tls_session);

    if (enq > 0) tls_notify_app_enqueue(ctx, app_session);
...
static int mbedtls_transport_reset(tls_ctx_t *ctx)
{
    if (!mbedtls_handshake_is_over(ctx)) {
        session_close(session_get_from_handle(ctx->tls_session_handle));
        return 0;
...
static int mbedtls_reinit_ca_chain(void)
{
    /* Not supported Yet */
    return 0;
}
...
int tls_init_ca_chain(void)
{
    ...
    if (tm->use_test_cert_in_ca) {
        rv = mbedtls_x509_crt_parse(&mm->cacert, (const unsigned char *) test_srv_crt_rsa, test_srv_crt_rsa_len);
        if (rv < 0) {
