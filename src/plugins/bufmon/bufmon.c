...
static u32 bufmon_alloc_free_callback(vlib_main_t *vm, u32 n_buffers, const int is_free)
{
    ...
    if (PREDICT_FALSE(vm->thread_index >= vec_len(bm->ptd))) {
        clib_warning("bufmon: thread index %d unknown for buffer %s (%d)", vm->thread_index, is_free ? "free" : "alloc", n_buffers);
        return n_buffers;
    }
...
static uword bufmon_dispatch_wrapper(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    if (frame) pnd->in += bufmon_count_buffers(vm, frame);
    ...
    for (; pending_frames < vec_len(nm->pending_frames); pending_frames++) {
        vlib_pending_frame_t *p = vec_elt_at_index(nm->pending_frames, pending_frames);
...
{
    if (vlib_buffer_set_alloc_free_callback(vm, bufmon_alloc_callback, bufmon_free_callback)) goto err0;

    foreach_vlib_main ()
        if (vlib_node_set_dispatch_wrapper(this_vlib_main, bufmon_dispatch_wrapper)) goto err1;

    vec_validate_aligned(bufmon_main.ptd, vlib_thread_main.n_vlib_mains - 1, CLIB_CACHE_LINE_BYTES);
    return 0;
...
static clib_error_t *show_buffer_traces(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (status) {
        vlib_cli_output(vm, "buffers tracing is %s", bm->enabled ? "on" : "off");
        return 0;
    }

    vlib_cli_output(vm, "%U\n\n", format_vlib_buffer_pool_all, vm);
    vlib_cli_output(vm, "%30s%20s%20s%20s%20s%20s", "Node", "Allocated", "Freed", "In", "Out", "Buffered");
    vec_foreach (ptd, bm->ptd) {
        vec_foreach (pnd, ptd->pnd) {
            ...
            if (0 == in && 0 == out) continue;       /* skip nodes w/o activity */
            if (0 == buffered && !verbose) continue; /* if not verbose, skip nodes w/o buffered buffers */
            vlib_cli_output(vm,
                            "%30U%20lu%20lu%20lu%20lu%20ld",
