...
static inline void http_conn_timer_start(http_conn_t *hc)
{
    ...
    hc->timer_handle = tw_timer_start_2t_1w_2048sl(&twc->tw, hs_handle, 0, timeout);
    clib_spinlock_unlock(&twc->tw_lock);
}

static inline void http_conn_timer_stop(http_conn_t *hc)
{
    http_tw_ctx_t *twc = &http_tw_ctx;

    if (hc->timer_handle == ~0) return;
...
static inline void http_conn_timer_update(http_conn_t *hc)
{
    http_tw_ctx_t *twc = &http_tw_ctx;
    u64            timeout;
