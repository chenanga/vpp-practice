...
#define HTTP_DBG(_lvl, _fmt, _args...) \
    if (_lvl <= HTTP_DEBUG) clib_warning(_fmt, ##_args)
#else
#define HTTP_DBG(_lvl, _fmt, _args...)
#endif
...
STATIC_ASSERT(sizeof(http_conn_id_t) <= TRANSPORT_CONN_ID_LEN, "ctx id must be less than TRANSPORT_CONN_ID_LEN");
...
typedef enum http_msg_type_ { HTTP_MSG_REQUEST, HTTP_MSG_REPLY } http_msg_type_t;
...
#define foreach_http_content_type                                                             \
    ...
    _(APP_XSLX, ".xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet") \
    _(APP_XUL, ".xul", "application/vnd.mozilla.xul+xml")                                     \
    _(APP_X_WWW_FORM_URLENCODED, ".invalid", "application/x-www-form-urlencoded")             \
...
typedef enum http_msg_data_type_ { HTTP_MSG_DATA_INLINE, HTTP_MSG_DATA_PTR } http_msg_data_type_t;
...
always_inline int _validate_target_syntax(u8 *target, int is_query, int *is_encoded)
{
    int i, encoded = 0;
    ...
    for (i = 0; i < vec_len(target); i++) {
        if (clib_bitmap_get_no_check(valid_chars, target[i])) continue;
        /* target was already split after first question mark,
         * for query it is valid character */
        if (is_query && target[i] == '?') continue;
        /* pct-encoded = "%" HEXDIG HEXDIG */
        if (target[i] == '%') {
            if ((i + 2) > vec_len(target)) return -1;
            if (!isxdigit(target[i + 1]) || !isxdigit(target[i + 2])) return -1;
            i += 2;
            encoded = 1;
            continue;
        }
    ...
    if (is_encoded) *is_encoded = encoded;
    return 0;
}
 ...
 */
always_inline int http_validate_abs_path_syntax(u8 *path, int *is_encoded)
{
 ...
 * @return @c 0 on success.
 */
always_inline int http_validate_query_syntax(u8 *query, int *is_encoded)
{
 ...
 */
always_inline u8 *http_percent_decode(u8 *src)
{
    int i;
    u8 *decoded_uri = 0;
 ...
 * The caller is always responsible to free the returned vector.
 */
always_inline u8 *http_path_remove_dot_segments(u8 *path)
{
    u32 *segments = 0, *segments_len = 0, segment_len;
    u8  *new_path = 0;
    int  i, ii;

    if (!path) return vec_new(u8, 0);
    ...
    for (i = 1; i < (vec_len(path) - 1); i++) {
        if (path[i] == '/') vec_add1(segments, i + 1);
    }
    /* dummy tail */
    ...
    for (i = 0; i < vec_len(segments_len); i++) {
        ...
        if (segment_len == 2 && path[segments[i]] == '.')
            segment_len = 0;
        else if (segment_len == 3 && path[segments[i]] == '.' && path[segments[i] + 1] == '.') {
            segment_len = 0;
    ...
    for (i = 0; i < vec_len(segments_len); i++) {
        if (segments_len[i]) vec_add(new_path, path + segments[i], segments_len[i]);
    }
    vec_free(segments);
...
always_inline int _parse_field_name(u8 **pos, u8 *end, u8 **field_name_start, u32 *field_name_len)
{
    u32 name_len = 0;
    u8 *p;
...
always_inline int _parse_field_value(u8 **pos, u8 *end, u8 **field_value_start, u32 *field_value_len)
{
    u32 value_len = 0;
    u8 *p;
 ...
 */
always_inline void http_free_header_table(http_header_table_t *ht)
{
 ...
 */
always_inline int http_parse_headers(u8 *headers, http_header_table_t **header_table)
{
    ...
    do {
        ...
        if (p) {
            ...
            header->value = format(header->value, ", %U%c", format_ascii_bytes, value_start, value_len, 0);
            vec_free(name);
            continue;
        }
 ...
 */
always_inline const char *http_get_header(http_header_table_t *header_table, const char *name)
{
    uword            *p;
    http_header_ht_t *header;
 ...
 */
always_inline void http_add_header(http_header_t **headers, const char *name, uword name_len, const char *value, uword value_len)
{
 ...
 * The caller is always responsible to free the returned vector.
 */
always_inline u8 *http_serialize_headers(http_header_t *headers)
{
...
always_inline int http_parse_authority_form_target(u8 *target, http_uri_t *authority)
{
    ...
    if (unformat(&input, "[%U]:%d", unformat_ip6_address, &authority->ip.ip6, &port)) {
        authority->port   = clib_host_to_net_u16(port);
        authority->is_ip4 = 0;
    }
    else if (unformat(&input, "%U:%d", unformat_ip4_address, &authority->ip.ip4, &port)) {
        authority->port   = clib_host_to_net_u16(port);
        authority->is_ip4 = 1;
    }
...
always_inline u8 *http_serialize_authority_form_target(http_uri_t *authority)
{
    u8 *s;

    if (authority->is_ip4)
        s = format(0, "%U:%d", format_ip4_address, &authority->ip.ip4, clib_net_to_host_u16(authority->port));
    else
        s = format(0, "[%U]:%d", format_ip6_address, &authority->ip.ip6, clib_net_to_host_u16(authority->port));
