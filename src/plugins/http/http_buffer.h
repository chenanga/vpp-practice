...
static inline void http_buffer_free(http_buffer_t *hb)
{
    if (hb->vft) hb->vft->free(hb);
}

static inline svm_fifo_seg_t *http_buffer_get_segs(http_buffer_t *hb, u32 max_len, u32 *n_segs)
{
    return hb->vft->get_segs(hb, max_len, n_segs);
}

static inline u32 http_buffer_drain(http_buffer_t *hb, u32 len)
{
    return hb->vft->drain(hb, len);
}

static inline u8 http_buffer_is_drained(http_buffer_t *hb)
{
