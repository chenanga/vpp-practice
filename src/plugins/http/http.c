...
static u8 *format_http_state(u8 *s, va_list *va)
{
...
#define http_state_change(_hc, _state)                                                                                \
    do {                                                                                                              \
        HTTP_DBG(1, "changing http state %U -> %U", format_http_state, (_hc)->http_state, format_http_state, _state); \
        (_hc)->http_state = _state;                                                                                   \
    } while (0)

static inline int http_state_is_tx_valid(http_conn_t *hc)
{
    http_state_t state = hc->http_state;
    return (state == HTTP_STATE_APP_IO_MORE_DATA || state == HTTP_STATE_WAIT_APP_REPLY || state == HTTP_STATE_WAIT_APP_METHOD);
}

static inline int http_state_is_rx_valid(http_conn_t *hc)
{
    http_state_t state = hc->http_state;
    return (state == HTTP_STATE_WAIT_SERVER_REPLY || state == HTTP_STATE_CLIENT_IO_MORE_DATA || state == HTTP_STATE_WAIT_CLIENT_METHOD);
}

static inline http_worker_t *http_worker_get(u32 thread_index)
{
    return &http_main.wrk[thread_index];
}

static inline u32 http_conn_alloc_w_thread(u32 thread_index)
{
...
static inline http_conn_t *http_conn_get_w_thread(u32 hc_index, u32 thread_index)
{
    http_worker_t *wrk = http_worker_get(thread_index);
...
static u32 http_listener_alloc(void)
{
...
http_conn_t *http_listener_get(u32 lhc_index)
{
    return pool_elt_at_index(http_main.listener_pool, lhc_index);
}

void http_listener_free(http_conn_t *lhc)
{
    http_main_t *hm = &http_main;

    vec_free(lhc->app_name);
    if (CLIB_DEBUG) memset(lhc, 0xfc, sizeof(*lhc));
...
void http_disconnect_transport(http_conn_t *hc)
{
    ...
    if (vnet_disconnect_session(&a)) clib_warning("disconnect returned");
}

static void http_conn_timeout_cb(void *hc_handlep)
{
    http_conn_t *hc;
    uword        hs_handle;
    ...
    HTTP_DBG(1, "terminate thread %d index %d hs %llx", hs_handle >> 24, hs_handle & 0x00FFFFFF, hc);
    if (!hc) return;
...
int http_ts_accept_callback(session_t *ts)
{
    ...
    HTTP_DBG(1, "Accepted on listener %u new connection [%u]%x", ts_listener->opaque, vlib_get_thread_index(), hc_index);
...
static int http_ts_connected_callback(u32 http_app_index, u32 ho_hc_index, session_t *ts, session_error_t err)
{
    u32           new_hc_index;
    session_t    *as;
    ...
    if (err) {
        clib_warning("half-open hc index %d, error: %U", ho_hc_index, format_session_error, err);
        app_wrk = app_worker_get_if_valid(ho_hc->h_pa_wrk_index);
        if (app_wrk) app_worker_connect_notify(app_wrk, 0, err, ho_hc->h_pa_app_api_ctx);
        return 0;
    }
    ...
    as->session_type     = session_type_from_proto_and_ip(TRANSPORT_PROTO_HTTP, session_type_is_ip4(ts->session_type));

    HTTP_DBG(1, "half-open hc index %d,  hc index %d", ho_hc_index, new_hc_index);
...
static void http_ts_disconnect_callback(session_t *ts)
{
    http_conn_t *hc;

    hc = http_conn_get_w_thread(ts->opaque, ts->thread_index);

    if (hc->state < HTTP_CONN_STATE_TRANSPORT_CLOSED) hc->state = HTTP_CONN_STATE_TRANSPORT_CLOSED;

    /* Nothing more to rx, propagate to app */
    if (!svm_fifo_max_dequeue_cons(ts->rx_fifo)) session_transport_closing_notify(&hc->connection);
}

static void http_ts_reset_callback(session_t *ts)
{
    http_conn_t *hc;
...
static const char *http_post_request_template =
    "POST %s HTTP/1.1\r\n"
    "Host: %v\r\n"
    "User-Agent: %v\r\n"
    "Content-Length: %llu\r\n"
...
static u32 http_send_data(http_conn_t *hc, u8 *data, u32 length)
{
    ...
    if (svm_fifo_set_event(ts->tx_fifo)) session_program_tx_io_evt(ts->handle, SESSION_IO_EVT_TX);

    return rv;
}

static void http_send_error(http_conn_t *hc, http_status_code_t ec)
{
    http_main_t *hm = &http_main;
    u8          *data;
    f64          now;

    if (ec >= HTTP_N_STATUS) ec = HTTP_STATUS_INTERNAL_ERROR;

    now  = clib_timebase_now(&hm->timebase);
    data = format(0, http_error_template, http_status_code_str[ec], format_clib_timebase_time, now);
    HTTP_DBG(1, "%v", data);
    http_send_data(hc, data, vec_len(data));
    vec_free(data);
}

static int http_read_message(http_conn_t *hc)
{
    u32        max_deq;
    session_t *ts;
    int        n_read;
    ...
    if (PREDICT_FALSE(max_deq == 0)) return -1;
...
static void http_read_message_drop(http_conn_t *hc, u32 len)
{
    session_t *ts;
    ...
    if (svm_fifo_is_empty(ts->rx_fifo)) svm_fifo_unset_event(ts->rx_fifo);
}

static void http_read_message_drop_all(http_conn_t *hc)
{
    session_t *ts;
 ...
 */
static inline int v_find_index(u8 *vec, u32 offset, u32 num, char *str)
{
    ...
    if (num) {
        if (num < slen) return -1;
        end_index = clib_min(end_index, offset + num - slen);
    }

    for (; start_index <= end_index; start_index++) {
        if (!memcmp(vec + start_index, str, slen)) return start_index;
    }

    return -1;
}

static void http_identify_optional_query(http_conn_t *hc)
{
    int i;
    for (i = hc->target_path_offset; i < (hc->target_path_offset + hc->target_path_len); i++) {
        if (hc->rx_buf[i] == '?') {
            hc->target_query_offset = i + 1;
            hc->target_query_len    = hc->target_path_offset + hc->target_path_len - hc->target_query_offset;
...
static int http_get_target_form(http_conn_t *hc)
{
    int i;

    /* "*" */
    if ((hc->rx_buf[hc->target_path_offset] == '*') && (hc->target_path_len == 1)) {
...
static int http_parse_request_line(http_conn_t *hc, http_status_code_t *ec)
{
    int i, target_len;
    ...
    if (isdigit(hc->rx_buf[i + 6])) {
        if (hc->rx_buf[i + 6] != '1') {
            clib_warning("HTTP major version '%c' not supported", hc->rx_buf[i + 6]);
...
static int http_parse_status_line(http_conn_t *hc)
{
    int i;
...
static int http_identify_headers(http_conn_t *hc, http_status_code_t *ec)
{
    int i;

    /* check if we have any header */
    if ((hc->rx_buf[hc->rx_buf_offset] == '\r') && (hc->rx_buf[hc->rx_buf_offset + 1] == '\n')) {
...
static int http_identify_message_body(http_conn_t *hc, http_status_code_t *ec)
{
    ...
    i = v_find_index(hc->rx_buf, hc->headers_offset, hc->headers_len, "Content-Length:");
    if (i < 0) {
...
static http_sm_result_t http_state_wait_server_reply(http_conn_t *hc, transport_send_params_t *sp)
{
    int                rv;
    http_msg_t         msg = {};
    app_worker_t      *app_wrk;
    ...
    svm_fifo_seg_t segs[2] = {{(u8 *) &msg, sizeof(msg)}, {hc->rx_buf, len}};
    ...
    if (app_wrk) app_worker_rx_notify(app_wrk, as);
    return HTTP_SM_STOP;
...
static http_sm_result_t http_state_wait_client_method(http_conn_t *hc, transport_send_params_t *sp)
{
    http_status_code_t ec;
    app_worker_t      *app_wrk;
    http_msg_t         msg;
    ...
    max_deq = clib_min(hc->control_data_len + hc->body_len, vec_len(hc->rx_buf));
...
static http_sm_result_t http_state_wait_app_reply(http_conn_t *hc, transport_send_params_t *sp)
{
    http_main_t       *hm = &http_main;
    u8                *response;
    u32                sent;
    ...
    if (msg.data.headers_len) {
        ...
        if (msg.data.type == HTTP_MSG_DATA_PTR) {
            ...
            rv = svm_fifo_dequeue(as->tx_fifo, sizeof(app_headers_ptr), (u8 *) &app_headers_ptr);
...
static http_sm_result_t http_state_wait_app_method(http_conn_t *hc, transport_send_params_t *sp)
{
    http_msg_t       msg;
    session_t       *as;
    ...
    if (msg.data.type == HTTP_MSG_DATA_PTR) {
        uword target_ptr;
        rv = svm_fifo_dequeue(as->tx_fifo, sizeof(target_ptr), (u8 *) &target_ptr);
        ASSERT(rv == sizeof(target_ptr));
        target = uword_to_pointer(target_ptr, u8 *);
    }
    else {
        vec_validate(target_buff, msg.data.target_path_len - 1);
        rv = svm_fifo_dequeue(as->tx_fifo, msg.data.target_path_len, target_buff);
...
static http_sm_result_t http_state_client_io_more_data(http_conn_t *hc, transport_send_params_t *sp)
{
    session_t     *as, *ts;
    app_worker_t  *app_wrk;
    ...
    if (hc->to_recv == 0) http_state_change(hc, hc->is_server ? HTTP_STATE_WAIT_APP_REPLY : HTTP_STATE_WAIT_APP_METHOD);

    app_wrk = app_worker_get_if_valid(as->app_wrk_index);
    if (app_wrk) app_worker_rx_notify(app_wrk, as);

    if (svm_fifo_max_dequeue_cons(ts->rx_fifo)) session_enqueue_notify(ts);

    return HTTP_SM_STOP;
}

static http_sm_result_t http_state_app_io_more_data(http_conn_t *hc, transport_send_params_t *sp)
{
    ...
    ts       = session_get_from_handle(hc->h_tc_session_handle);
    if ((seg = http_buffer_get_segs(hb, max_send, &n_segs))) sent = svm_fifo_enqueue_segments(ts->tx_fifo, seg, n_segs, 1 /* allow partial */);
    ...
    if (!http_buffer_is_drained(hb)) {
        if (sent && svm_fifo_set_event(ts->tx_fifo)) session_program_tx_io_evt(ts->handle, SESSION_IO_EVT_TX);
        ...
        if (sent && svm_fifo_set_event(ts->tx_fifo)) session_program_tx_io_evt(ts->handle, SESSION_IO_EVT_TX_FLUSH);

        /* Finished transaction:
         * server back to HTTP_STATE_WAIT_METHOD
         * client to HTTP_STATE_WAIT_SERVER_REPLY */
        http_state_change(hc, hc->is_server ? HTTP_STATE_WAIT_CLIENT_METHOD : HTTP_STATE_WAIT_SERVER_REPLY);
        http_buffer_free(&hc->tx_buf);
    }
...
typedef http_sm_result_t (*http_sm_handler)(http_conn_t *, transport_send_params_t *sp);
...
static void http_req_run_state_machine(http_conn_t *hc, transport_send_params_t *sp)
{
...
static int http_ts_rx_callback(session_t *ts)
{
    http_conn_t *hc;
    ...
    if (!http_state_is_rx_valid(hc)) {
        if (hc->state != HTTP_CONN_STATE_CLOSED)
            clib_warning("app data req state '%U' session state %u", format_http_state, hc->http_state, hc->state);
...
int http_ts_builtin_tx_callback(session_t *ts)
{
    http_conn_t *hc;
...
static void http_ts_cleanup_callback(session_t *ts, session_cleanup_ntf_t ntf)
{
    http_conn_t *hc;
...
int http_add_segment_callback(u32 client_index, u64 segment_handle)
{
    /* No-op for builtin */
    return 0;
}

int http_del_segment_callback(u32 client_index, u64 segment_handle)
{
    return 0;
}
...
static clib_error_t *http_transport_enable(vlib_main_t *vm, u8 is_en)
{
    ...
    if (vnet_application_attach(a)) return clib_error_return(0, "failed to attach http app");

    hm->app_index = a->app_index;
    vec_free(a->name);

    if (hm->is_init) return 0;

    vec_validate(hm->wrk, vlib_num_workers());

    clib_timebase_init(&hm->timebase, 0 /* GMT */, CLIB_TIMEBASE_DAYLIGHT_NONE, &vm->clib_time /* share the system clock */);
...
static int http_transport_connect(transport_endpoint_cfg_t *tep)
{
    ...
    if (sep->is_ip4)
        hc->host = format(0, "%U:%d", format_ip4_address, &sep->ip.ip4, clib_net_to_host_u16(sep->port));
    else
        hc->host = format(0, "%U:%d", format_ip6_address, &sep->ip.ip6, clib_net_to_host_u16(sep->port));
...
static u32 http_start_listen(u32 app_listener_index, transport_endpoint_cfg_t *tep)
{
    ...
    if (vnet_listen(args)) return SESSION_INVALID_INDEX;
...
static u32 http_stop_listen(u32 listener_index)
{
    http_conn_t *lhc;
    int          rv;
    ...
    if ((rv = vnet_unlisten(&a))) clib_warning("unlisten returned %d", rv);
...
static void http_transport_close(u32 hc_index, u32 thread_index)
{
    session_t   *as;
    http_conn_t *hc;
...
static transport_connection_t *http_transport_get_connection(u32 hc_index, u32 thread_index)
{
    http_conn_t *hc = http_conn_get_w_thread(hc_index, thread_index);
    return &hc->connection;
}

static transport_connection_t *http_transport_get_listener(u32 listener_index)
{
    http_conn_t *lhc = http_listener_get(listener_index);
    return &lhc->connection;
}

static int http_app_tx_callback(void *session, transport_send_params_t *sp)
{
    ...
    if (hc->state == HTTP_CONN_STATE_APP_CLOSED) {
        if (!svm_fifo_max_dequeue_cons(as->tx_fifo)) http_disconnect_transport(hc);
    }
...
static void http_transport_get_endpoint(u32 hc_index, u32 thread_index, transport_endpoint_t *tep, u8 is_lcl)
{
    http_conn_t *hc = http_conn_get_w_thread(hc_index, thread_index);
    session_t   *ts;
...
static u8 *format_http_connection(u8 *s, va_list *args)
{
    http_conn_t *hc = va_arg(*args, http_conn_t *);
    session_t   *ts;

    ts = session_get_from_handle(hc->h_tc_session_handle);
    s  = format(s, "[%d:%d][H] app_wrk %u ts %d:%d", hc->c_thread_index, hc->c_s_index, hc->h_pa_wrk_index, ts->thread_index, ts->session_index);

    return s;
}

static u8 *format_http_listener(u8 *s, va_list *args)
{
    ...
    s = format(s, "[%d:%d][H] app_wrk %u ts %d:%d", lhc->c_thread_index, lhc->c_s_index, lhc->h_pa_wrk_index, lts->thread_index, lts->session_index);

    return s;
}

static u8 *format_http_conn_state(u8 *s, va_list *args)
{
    http_conn_t *hc = va_arg(*args, http_conn_t *);
...
static u8 *format_http_transport_connection(u8 *s, va_list *args)
{
    ...
    if (verbose) {
        ...
        if (verbose > 1) s = format(s, "\n");
    }

    return s;
}

static u8 *format_http_transport_listener(u8 *s, va_list *args)
{
    u32               tc_index     = va_arg(*args, u32);
    ...
    if (verbose) s = format(s, "%-" SESSION_CLI_STATE_LEN "U", format_http_conn_state, lhc);
    return s;
}
...
static clib_error_t *http_transport_init(vlib_main_t *vm)
{
    http_main_t *hm = &http_main;
    int          i;

    transport_register_protocol(TRANSPORT_PROTO_HTTP, &http_proto, FIB_PROTOCOL_IP4, ~0);
    transport_register_protocol(TRANSPORT_PROTO_HTTP, &http_proto, FIB_PROTOCOL_IP6, ~0);
...
static clib_error_t *http_config_fn(vlib_main_t *vm, unformat_input_t *input)
{
    http_main_t *hm = &http_main;
    uword        mem_sz;

    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "first-segment-size %U", unformat_memory_size, &mem_sz)) {
            hm->first_seg_size = clib_max(mem_sz, 1 << 20);
            if (hm->first_seg_size != mem_sz) clib_warning("first seg size too small %u", mem_sz);
        }
        else if (unformat(input, "add-segment-size %U", unformat_memory_size, &mem_sz)) {
            hm->add_seg_size = clib_max(mem_sz, 1 << 20);
            if (hm->add_seg_size != mem_sz) clib_warning("add seg size too small %u", mem_sz);
        }
        else if (unformat(input, "fifo-size %U", unformat_memory_size, &mem_sz)) {
            hm->fifo_size = clib_clamp(mem_sz, 4 << 10, 2 << 30);
            if (hm->fifo_size != mem_sz) clib_warning("invalid fifo size %lu", mem_sz);
        }
        else
