...
static void http_timer_process_expired_cb(u32 *expired_timers)
{
    ...
    for (i = 0; i < vec_len(expired_timers); i++) {
        ...
        session_send_rpc_evt_to_thread(hs_handle >> 24, twc->cb_fn, uword_to_pointer(hs_handle, void *));
    }
}

static uword http_timer_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    http_tw_ctx_t      *twc = &http_tw_ctx;
    f64                 now, timeout = 1.0;
    uword              *event_data = 0;
...
void http_timers_init(vlib_main_t *vm, http_conn_timeout_fn *cb_fn)
{
    ...
    tw_timer_wheel_init_2t_1w_2048sl(&twc->tw, http_timer_process_expired_cb, 1.0 /* timer interval */, ~0);
    clib_spinlock_init(&twc->tw_lock);
    twc->cb_fn = cb_fn;

    vlib_node_set_state(vm, http_timer_process_node.index, VLIB_NODE_STATE_POLLING);
