...
{
    return ((clib_net_to_host_u32(h->vni_rsvd) & GENEVE_VNI_MASK) >> GENEVE_VNI_SHIFT);
}

static inline u32 vnet_get_geneve_vni_network_order(geneve_header_t* h)
{
...
static inline void vnet_set_geneve_critical_bit(geneve_header_t* h, u8 critical_opts)
{
    ...
    h->first_word |= ((critical_opts << GENEVE_C_BIT_SHIFT) & GENEVE_C_BIT_MASK);
}
...
static inline void vnet_geneve_hdr_1word_ntoh(geneve_header_t* h)
{
    h->first_word = clib_net_to_host_u32(h->first_word);
}

static inline void vnet_geneve_hdr_1word_hton(geneve_header_t* h)
{
