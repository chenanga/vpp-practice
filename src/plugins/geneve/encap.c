...
#define foreach_geneve_encap_error _(ENCAPSULATED, "good packets encapsulated")
...
always_inline uword geneve_encap_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, u32 is_ip4)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (is_ip4) {
                u8 ip4_geneve_base_header_len   = sizeof(ip4_header_t) + sizeof(udp_header_t) + GENEVE_BASE_HEADER_LENGTH;
...
#undef _
                ...
                new_l0         = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, b[0]) - sizeof(*ip4_0));
                udp0->length   = new_l0;
                udp0->src_port = flow_hash0;
                udp1           = (udp_header_t *) (ip4_1 + 1);
                new_l1         = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, b[1]) - sizeof(*ip4_1));
                udp1->length   = new_l1;
            ...
            else /* ipv6 */
            {
                ...
                u8 ip6_geneve_base_header_len   = sizeof(ip6_header_t) + sizeof(udp_header_t) + GENEVE_BASE_HEADER_LENGTH;
...
#undef _
                ...
                new_l0                = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, b[0]) - sizeof(*ip6_0));
                ip6_0->payload_length = new_l0;
                new_l1                = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, b[1]) - sizeof(*ip6_1));
                ...
                udp0->checksum = ip6_tcp_udp_icmp_compute_checksum(vm, b[0], ip6_0, &bogus);
                ASSERT(bogus == 0);
                if (udp0->checksum == 0) udp0->checksum = 0xffff;
                udp1->checksum = ip6_tcp_udp_icmp_compute_checksum(vm, b[1], ip6_1, &bogus);
                ASSERT(bogus == 0);
            ...
            if (PREDICT_FALSE((sw_if_index0 != stats_sw_if_index) || (sw_if_index1 != stats_sw_if_index))) {
                stats_n_packets -= 2;
                stats_n_bytes -= len0 + len1;
            ...
            if (PREDICT_FALSE(b[0]->flags & VLIB_BUFFER_IS_TRACED)) {
                geneve_encap_trace_t *tr = vlib_add_trace(vm, node, b[0], sizeof(*tr));
                tr->tunnel_index         = t0 - vxm->tunnels;
                tr->vni                  = t0->vni;
            }

            if (PREDICT_FALSE(b[1]->flags & VLIB_BUFFER_IS_TRACED)) {
                geneve_encap_trace_t *tr = vlib_add_trace(vm, node, b[1], sizeof(*tr));
...
#undef _
    ...
    vlib_node_increment_counter(vm, node->node_index, GENEVE_ENCAP_ERROR_ENCAPSULATED, pkts_encapsulated);
