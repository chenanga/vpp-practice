...
static u8 *format_geneve_rx_trace(u8 *s, va_list *args)
{
    ...
    if (t->tunnel_index != ~0) {
        s = format(s, "GENEVE decap from geneve_tunnel%d vni %d next %d error %d", t->tunnel_index, t->vni_rsvd, t->next_index, t->error);
    }
    else {
        s = format(s, "GENEVE decap error - tunnel for vni %d does not exist", t->vni_rsvd);
    }
    return s;
}

always_inline u32 validate_geneve_fib(vlib_buffer_t *b, geneve_tunnel_t *t, u32 is_ip4)
{
    ...
    if (is_ip4)
        fib_index = (vnet_buffer(b)->sw_if_index[VLIB_TX] == (u32) ~0) ? vec_elt(ip4_main.fib_index_by_sw_if_index, sw_if_index)
                                                                       : vnet_buffer(b)->sw_if_index[VLIB_TX];
    else
        fib_index = (vnet_buffer(b)->sw_if_index[VLIB_TX] == (u32) ~0) ? vec_elt(ip6_main.fib_index_by_sw_if_index, sw_if_index)
                                                                       : vnet_buffer(b)->sw_if_index[VLIB_TX];

    return (fib_index == t->encap_fib_index);
}

always_inline uword geneve_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, u32 is_ip4)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            /* pop (ip, udp, geneve) */
            if (is_ip4) {
                vlib_buffer_advance(b0, sizeof(*ip4_0) + sizeof(udp_header_t) + GENEVE_BASE_HEADER_LENGTH + vnet_get_geneve_options_len(geneve0));
                vlib_buffer_advance(b1, sizeof(*ip4_1) + sizeof(udp_header_t) + GENEVE_BASE_HEADER_LENGTH + vnet_get_geneve_options_len(geneve1));
            }
            else {
                vlib_buffer_advance(b0, sizeof(*ip6_0) + sizeof(udp_header_t) + GENEVE_BASE_HEADER_LENGTH + vnet_get_geneve_options_len(geneve0));
                vlib_buffer_advance(b1, sizeof(*ip6_1) + sizeof(udp_header_t) + GENEVE_BASE_HEADER_LENGTH + vnet_get_geneve_options_len(geneve1));
            }
            ...
            if (PREDICT_FALSE(vnet_get_geneve_version(geneve0) != GENEVE_VERSION)) {
            ...
            if (is_ip4) {
                ...
                if (PREDICT_TRUE(ip4_0->dst_address.as_u32 == t0->local.ip4.as_u32)) goto next0; /* valid packet */
            ...
            else /* !is_ip4 */
            {
                ...
                /* Validate GENEVE tunnel SIP against packet DIP */
                if (PREDICT_TRUE(ip6_address_is_equal(&ip6_0->dst_address, &t0->local.ip6))) goto next0; /* valid packet */
        ...
        next0:
            ...
            if (PREDICT_TRUE(next0 == GENEVE_INPUT_NEXT_L2_INPUT)) vnet_update_l2_len(b0);
        ...
        trace0:
            ...
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                geneve_rx_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
                tr->next_index        = next0;
                tr->error             = error0;
            ...
            if (PREDICT_FALSE(vnet_get_geneve_version(geneve1) != GENEVE_VERSION)) {
            ...
            if (is_ip4) {
                ...
                /* Validate GENEVE tunnel SIP against packet DIP */
                if (PREDICT_TRUE(ip4_1->dst_address.as_u32 == t1->local.ip4.as_u32)) goto next1; /* valid packet */
            ...
            else /* !is_ip4 */
            {
                ...
                /* Validate GENEVE tunnel SIP against packet DIP */
                if (PREDICT_TRUE(ip6_address_is_equal(&ip6_1->dst_address, &t1->local.ip6))) goto next1; /* valid packet */
        ...
        next1:
            ...
            /* Required to make the l2 tag push / pop code work on l2 subifs */
            if (PREDICT_TRUE(next1 == GENEVE_INPUT_NEXT_L2_INPUT)) vnet_update_l2_len(b1);
        ...
        trace1:
            ...
            if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
                geneve_rx_trace_t *tr = vlib_add_trace(vm, node, b1, sizeof(*tr));
                tr->next_index        = next1;
                tr->error             = error1;
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (is_ip4) {
                ...
                /* Validate GENEVE tunnel SIP against packet DIP */
                if (PREDICT_TRUE(ip4_0->dst_address.as_u32 == t0->local.ip4.as_u32)) goto next00; /* valid packet */
                if (PREDICT_FALSE(ip4_address_is_multicast(&ip4_0->dst_address))) {
                    key4_0.remote = ip4_0->dst_address.as_u32;
                    key4_0.vni    = vnet_get_geneve_vni_network_order(geneve0);
                    /* Make sure mcast GENEVE tunnel exist by packet DIP and VNI */
            ...
            else /* !is_ip4 */
            {
                ...
                /* Validate GENEVE tunnel SIP against packet DIP */
                if (PREDICT_TRUE(ip6_address_is_equal(&ip6_0->dst_address, &t0->local.ip6))) goto next00; /* valid packet */
                if (PREDICT_FALSE(ip6_address_is_multicast(&ip6_0->dst_address))) {
                    key6_0.remote.as_u64[0] = ip6_0->dst_address.as_u64[0];
                    key6_0.remote.as_u64[1] = ip6_0->dst_address.as_u64[1];
                    key6_0.vni              = vnet_get_geneve_vni_network_order(geneve0);
    ...
    vlib_node_increment_counter(vm, is_ip4 ? geneve4_input_node.index : geneve6_input_node.index, GENEVE_ERROR_DECAPSULATED, pkts_decapsulated);
...
VLIB_NODE_FN(geneve4_input_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    return geneve_input(vm, node, from_frame, /* is_ip4 */ 1);
}

VLIB_NODE_FN(geneve6_input_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
always_inline uword ip_geneve_bypass_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u32 is_ip4)
{
    geneve_main_t       *vxm = &geneve_main;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (udp0->dst_port != clib_host_to_net_u16(UDP_DST_PORT_geneve)) goto exit0; /* not GENEVE packet */
            ...
            if (is_ip4) {
                ...
                if (!vtep6_check(&vxm->vtep_table, b0, ip60, &last_vtep6)) goto exit0; /* no local VTEP for GENEVE packet */
            }
        ...
        exit0:
            ...
            if (udp1->dst_port != clib_host_to_net_u16(UDP_DST_PORT_geneve)) goto exit1; /* not GENEVE packet */
            ...
            if (is_ip4) {
                ...
                if (!vtep6_check(&vxm->vtep_table, b1, ip61, &last_vtep6)) goto exit1; /* no local VTEP for GENEVE packet */
            }
            ...
            next1     = error1 ? IP_GENEVE_BYPASS_NEXT_DROP : IP_GENEVE_BYPASS_NEXT_GENEVE;
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (udp0->dst_port != clib_host_to_net_u16(UDP_DST_PORT_geneve)) goto exit; /* not GENEVE packet */
            ...
            if (is_ip4) {
                ...
                if (!vtep6_check(&vxm->vtep_table, b0, ip60, &last_vtep6)) goto exit; /* no local VTEP for GENEVE packet */
            }
...
VLIB_NODE_FN(ip4_geneve_bypass_node)
...
VLIB_NODE_FN(ip6_geneve_bypass_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
