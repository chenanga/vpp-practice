...
u8 *format_geneve_encap_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "GENEVE encap to geneve_tunnel%d vni %d", t->tunnel_index, t->vni);
    return s;
}
...
static void geneve_tunnel_restack_dpo(geneve_tunnel_t *t)
{
    ...
    u32                      encap_index = ip46_address_is_ip4(&t->remote) ? geneve4_encap_node.index : geneve6_encap_node.index;
...
static geneve_tunnel_t *geneve_tunnel_from_fib_node(fib_node_t *node)
{
    ...
    return ((geneve_tunnel_t *) (((char *) node) - STRUCT_OFFSET_OF(geneve_tunnel_t, node)));
}
...
static bool geneve_decap_next_is_valid(geneve_main_t *vxm, u32 is_ip6, u32 decap_next_index)
{
    ...
    u32                  input_idx = (!is_ip6) ? geneve4_input_node.index : geneve6_input_node.index;
...
int vnet_geneve_add_del_tunnel(vnet_geneve_add_del_tunnel_args_t *a, u32 *sw_if_indexp)
{
    ...
    if (a->is_add) {
        ...
        if (a->decap_next_index == ~0) a->decap_next_index = GENEVE_INPUT_NEXT_L2_INPUT;
        if (!geneve_decap_next_is_valid(vxm, is_ip6, a->decap_next_index)) return VNET_API_ERROR_INVALID_DECAP_NEXT;
...
#undef _
        ...
        /* copy the key */
        if (is_ip6)
            hash_set_mem_alloc(&vxm->geneve6_tunnel_by_key, &key6, t - vxm->tunnels);
        else
        ...
        if (a->l3_mode) {
            ...
            hw_if_index = vnet_register_interface(vnm, geneve_device_class.index, t - vxm->tunnels, geneve_hw_class.index, t - vxm->tunnels);
        }

        hi = vnet_get_hw_interface(vnm, hw_if_index);

        /* Set geneve tunnel output node */
        u32 encap_index = !is_ip6 ? geneve4_encap_node.index : geneve6_encap_node.index;
        ...
        if (!ip46_address_is_multicast(&t->remote)) {
            ...
            t->fib_entry_index =
                fib_entry_track(t->encap_fib_index, &tun_remote_pfx, FIB_NODE_TYPE_GENEVE_TUNNEL, t - vxm->tunnels, &t->sibling_index);
             ...
             */
            if (vtep_addr_ref(&vxm->vtep_table, t->encap_fib_index, &t->remote) == 1) {
                 ...
                 */
                mfib_table_entry_path_update(t->encap_fib_index, &mpfx, MFIB_SOURCE_GENEVE, MFIB_ENTRY_FLAG_NONE, &path);

                path.frp_sw_if_index = a->mcast_sw_if_index;
                path.frp_flags       = FIB_ROUTE_PATH_FLAG_NONE;
                path.frp_mitf_flags  = MFIB_ITF_FLAG_ACCEPT;
                mfei                 = mfib_table_entry_path_update(t->encap_fib_index, &mpfx, MFIB_SOURCE_GENEVE, MFIB_ENTRY_FLAG_NONE, &path);
        ...
        set_int_l2_mode(vxm->vlib_main, vnm, MODE_L3, t->sw_if_index, 0, L2_BD_PORT_TYPE_NORMAL, 0, 0);
        ...
        else if (vtep_addr_unref(&vxm->vtep_table, t->encap_fib_index, &t->remote) == 0) {
    ...
    if (a->is_add) {
        ...
        if (!is_ip6 && !udp_is_valid_dst_port(UDP_DST_PORT_geneve, 1))
            udp_register_dst_port(vxm->vlib_main, UDP_DST_PORT_geneve, geneve4_input_node.index, 1);
        if (is_ip6 && !udp_is_valid_dst_port(UDP_DST_PORT_geneve6, 0))
            udp_register_dst_port(vxm->vlib_main, UDP_DST_PORT_geneve6, geneve6_input_node.index, 0);
    }
...
static uword get_decap_next_for_node(u32 node_index, u32 ipv4_set)
{
    geneve_main_t *vxm        = &geneve_main;
    vlib_main_t   *vm         = vxm->vlib_main;
    uword          input_node = (ipv4_set) ? geneve4_input_node.index : geneve6_input_node.index;
...
static clib_error_t *geneve_add_del_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (ip46_address_cmp(&local, &remote) == 0) {
        error = clib_error_return(0, "local and remote addresses are identical");
        goto done;
    }
...
#undef _
    ...
    switch (rv) {
        ...
        default:
            error = clib_error_return(0, "vnet_geneve_add_del_tunnel returned %d", rv);
            goto done;
    }
...
static clib_error_t *show_geneve_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (pool_elts(vxm->tunnels) == 0) vlib_cli_output(vm, "No geneve tunnels configured...");
...
{
    if (is_ip6)
        vnet_feature_enable_disable("ip6-unicast", "ip6-geneve-bypass", sw_if_index, is_enable, 0, 0);
    else
        vnet_feature_enable_disable("ip4-unicast", "ip4-geneve-bypass", sw_if_index, is_enable, 0, 0);
}
...
clib_error_t *geneve_init(vlib_main_t *vm)
{
    ...
    vxm->geneve6_tunnel_by_key = hash_create_mem(0, sizeof(geneve6_tunnel_key_t), sizeof(uword));
