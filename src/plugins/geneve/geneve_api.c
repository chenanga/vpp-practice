...
static void vl_api_geneve_add_del_tunnel_t_handler(vl_api_geneve_add_del_tunnel_t *mp)
{
    ...
    if (ip46_address_is_multicast(&a.remote) && !vnet_sw_if_index_is_api_valid(a.mcast_sw_if_index)) {
...
out:
    REPLY_MACRO2(VL_API_GENEVE_ADD_DEL_TUNNEL_REPLY, ({ rmp->sw_if_index = ntohl(sw_if_index); }));
}
...
out:
    REPLY_MACRO2(VL_API_GENEVE_ADD_DEL_TUNNEL2_REPLY, ({ rmp->sw_if_index = ntohl(sw_if_index); }));
}
...
static void send_geneve_tunnel_details(geneve_tunnel_t *t, vl_api_registration_t *reg, u32 context)
{
    ...
    ip_address_encode(&t->local, is_ipv6 ? IP46_TYPE_IP6 : IP46_TYPE_IP4, &rmp->src_address);
    ip_address_encode(&t->remote, is_ipv6 ? IP46_TYPE_IP6 : IP46_TYPE_IP4, &rmp->dst_address);
    rmp->encap_vrf_id = htonl(is_ipv6 ? im6->fibs[t->encap_fib_index].ft_table_id : im4->fibs[t->encap_fib_index].ft_table_id);
...
static void vl_api_geneve_tunnel_dump_t_handler(vl_api_geneve_tunnel_dump_t *mp)
{
    ...
    if (~0 == sw_if_index) {
        ...
        if ((sw_if_index >= vec_len(vxm->tunnel_index_by_sw_if_index)) || (~0 == vxm->tunnel_index_by_sw_if_index[sw_if_index])) {
            return;
        }
...
static clib_error_t *geneve_api_hookup(vlib_main_t *vm)
{
    ...
    vl_api_increase_msg_trace_size(am, VL_API_GENEVE_ADD_DEL_TUNNEL + REPLY_MSG_ID_BASE, 16 * sizeof(u32));
