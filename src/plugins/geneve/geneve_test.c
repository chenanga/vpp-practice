...
static void vl_api_geneve_add_del_tunnel_reply_t_handler(vl_api_geneve_add_del_tunnel_reply_t *mp)
{
...
static void vl_api_geneve_add_del_tunnel2_reply_t_handler(vl_api_geneve_add_del_tunnel2_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    i32         retval = ntohl(mp->retval);
    if (vam->async_mode) {
...
static int api_sw_interface_set_geneve_bypass(vat_main_t *vam)
{
...
static uword unformat_geneve_decap_next(unformat_input_t *input, va_list *args)
{
...
static int api_geneve_add_del_tunnel(vat_main_t *vam)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(line_input, "group %U %U", unformat_ip4_address, &dst.ip4, unformat_sw_if_index, vam, &mcast_sw_if_index)) {
            grp_set = dst_set = 1;
            ipv4_set          = 1;
        }
        ...
        else if (unformat(line_input, "group %U %U", unformat_ip6_address, &dst.ip6, unformat_sw_if_index, vam, &mcast_sw_if_index)) {
            grp_set = dst_set = 1;
            ipv6_set          = 1;
        }
...
static int api_geneve_add_del_tunnel2(vat_main_t *vam)
{
    return api_geneve_add_del_tunnel(vam);
}

static void vl_api_geneve_tunnel_details_t_handler(vl_api_geneve_tunnel_details_t *mp)
{
    vat_main_t    *vam = &vat_main;
...
static int api_geneve_tunnel_dump(vat_main_t *vam)
{
    unformat_input_t            *i = vam->input;
    ...
    if (!geneve_test_main.ping_id) geneve_test_main.ping_id = vl_msg_api_get_msg_index((u8 *) (VL_API_CONTROL_PING_CRC));
