...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    if (mtq->n_enq) {
        ...
        if (PREDICT_FALSE(pp2_ppio_get_num_outq_done(ppio, hif, qid, &n_done)))
            vlib_error_count(vm, node->node_index, MVPP2_TX_NODE_CTR_PPIO_GET_NUM_OUTQ_DONE, 1);

        if (n_done) {
            vlib_buffer_free_from_ring(vm, mtq->buffers, (mtq->next - mtq->n_enq) & mask, sz, n_done);
            mtq->n_enq -= n_done;
        }
    }
    ...
    if (pp2_ppio_send(ppio, hif, qid, descs, &n_sent)) {
        ...
        vlib_buffer_copy_indices_to_ring(mtq->buffers, buffers, mtq->next & mask, sz, n_sent);
        mtq->next += n_sent;
        mtq->n_enq += n_sent;
    }
    ...
    if (PREDICT_FALSE(n_sent != n_vectors)) {
        ...
        vlib_error_count(vm, node->node_index, MVPP2_TX_NODE_CTR_NO_FREE_SLOTS, n_vectors - n_sent);
    }
