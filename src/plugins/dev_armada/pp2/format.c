...
static inline u32 mrvl_get_u32_bits(void *start, int offset, int first, int last)
{
...
u8 *format_pp2_ppio_link_info(u8 *s, va_list *args)
{
    ...
    u32 port_speeds[] = {
        [MV_NET_LINK_SPEED_10]    = 10,
        [MV_NET_LINK_SPEED_100]   = 100,
        [MV_NET_LINK_SPEED_1000]  = 1000,
        [MV_NET_LINK_SPEED_2500]  = 2500,
    ...
    s = format(s, "duplex %s speed %d up %d phy_mode %s", port_duplex[li->duplex], port_speeds[li->speed], li->up, port_phy_modes[li->phy_mode]);

    return s;
}

u8 *format_mvpp2_port_status(u8 *s, va_list *args)
{
    ...
    if (mp->ppio == 0 || pp2_ppio_get_link_info(mp->ppio, &li)) return format(s, "link info not available");

    return format(s, "%U", format_pp2_ppio_link_info, &li);
}

u8 *format_mvpp2_dev_info(u8 *s, va_list *args)
{
    vnet_dev_format_args_t __clib_unused *a   = va_arg(*args, vnet_dev_format_args_t *);
...
u8 *format_mvpp2_rx_desc(u8 *s, va_list *args)
...
u8 *format_mvpp2_rx_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "pp2: %v (%d) next-node %U", hi->name, hw_if_index, format_vlib_next_node_name, vm, node->index, t->rxq->next_index);
    s = format(s, "\n%U%U", format_white_space, indent + 2, format_mvpp2_rx_desc, d);
