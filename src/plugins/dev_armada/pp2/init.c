...
static u8 *mvpp2_probe(vlib_main_t *vm, vnet_dev_bus_index_t bus_index, void *dev_info)
{
    vnet_dev_bus_platform_device_info_t *di = dev_info;

    if (clib_dt_node_is_compatible(di->node, "marvell,armada-7k-pp22")) return format(0, "Marvell Armada Packet Processor v2.2");
    return 0;
}
static void mvpp2_global_deinit(vlib_main_t *vm, vnet_dev_t *dev)
{
...
static void mvpp2_deinit(vlib_main_t *vm, vnet_dev_t *dev)
{
    log_debug(dev, "");
    mvpp2_global_deinit(vm, dev);
}

static vnet_dev_rv_t mvpp2_global_init(vlib_main_t *vm, vnet_dev_t *dev)
{
    mvpp2_device_t *md = vnet_dev_get_data(dev);
    ...
    free_bpools = pow2_mask(MVPP2_NUM_BPOOLS) ^ init_params.bm_pool_reserved_map;

    if (n_threads > count_set_bits(free_hifs)) {
        log_err(dev, "no enough HIFs (needed %u available %u)", n_threads, count_set_bits(free_hifs));
        rv = VNET_DEV_ERR_INIT_FAILED;
        goto done;
    }
    ...
    for (u32 i = 0; i < n_threads; i++) {
        ...
        if (mrv < 0) {
            log_err(dev, "pp2_hif_init failed for hif %u thread %u, err %d", index, i, mrv);
            rv = VNET_DEV_ERR_INIT_FAILED;
            goto done;
        }
        ...
        if (mrv < 0) {
            log_err(dev, "pp2_bpool_init failed for bpool %u thread %u, err %d", index, i, mrv);
            rv = VNET_DEV_ERR_INIT_FAILED;
            goto done;
        }
        log_debug(dev, "pp2_bpool_init(bpool %u, thread %u) pool-%u:%u ok", index, i, md->thread[i].bpool->pp2_id, md->thread[i].bpool->id);
...
static vnet_dev_rv_t mvpp2_init(vlib_main_t *vm, vnet_dev_t *dev)
{
    mvpp2_device_t                      *md = vnet_dev_get_data(dev);
    vnet_dev_rv_t                        rv = VNET_DEV_OK;
    ...
    if (!clib_dt_node_is_compatible(dd->node, "marvell,armada-7k-pp22")) return VNET_DEV_ERR_NOT_SUPPORTED;
    ...
    if (pp_id < 0) return VNET_DEV_ERR_UNKNOWN_DEVICE;

    if ((mvpp2_global_init(vm, dev)) != VNET_DEV_OK) return rv;
    ...
    vec_foreach_pointer (cn, dd->node->child_nodes) {
        ...
        if (!clib_dt_proprerty_is_u32(p)) continue;

        ppio_id = clib_dt_proprerty_get_u32(p);
        log_debug(dev, "found port with ppio id %u", ppio_id);

        if (pp2_ppio_available(md->pp_id, ppio_id) == 0) continue;
        ...
        if (srv < 0) {
            log_warn(dev, "unable to get hw address, skipping port %u", ppio_id);
            continue;
        }

        log_debug(dev, "adding ppio %u (netdev name %s, hwaddr %U)", ppio_id, netdev_name, format_ethernet_address, s.ifr_addr.sa_data);
        ...
        vnet_dev_set_hw_addr_eth_mac(&port_add_args.port.attr.hw_addr, (u8 *) s.ifr_addr.sa_data);

        vnet_dev_port_add(vm, dev, ppio_id, &port_add_args);
    }

    if (rv != VNET_DEV_OK) mvpp2_deinit(vm, dev);
    return rv;
}
