...
vnet_dev_rv_t mvpp2_port_init(vlib_main_t *vm, vnet_dev_port_t *port)
{
    ...
    if (mrv) {
        ...
        log_err(dev, "port %u ppio '%s' init failed, rv %d", port->port_id, match, mrv);
        goto done;
    }
    ...
    if (mrv) {
        rv = VNET_DEV_ERR_INIT_FAILED;
        log_err(dev, "failed to get link info for port %u, rv %d", port->port_id, mrv);
        goto done;
    }
    ...
    if (rv != VNET_DEV_OK) mvpp2_port_stop(vm, port);
    return rv;
}

void mvpp2_port_deinit(vlib_main_t *vm, vnet_dev_port_t *port)
{
    mvpp2_port_t *mp = vnet_dev_get_port_data(port);
...
void mvpp2_port_poll(vlib_main_t *vm, vnet_dev_port_t *port)
{
    mvpp2_port_t                 *mp      = vnet_dev_get_port_data(port);
    vnet_dev_t                   *dev     = port->dev;
...
vnet_dev_rv_t mvpp2_port_start(vlib_main_t *vm, vnet_dev_port_t *port)
{
    mvpp2_port_t *mp = vnet_dev_get_port_data(port);
    int           mrv;
...
void mvpp2_port_stop(vlib_main_t *vm, vnet_dev_port_t *port)
{
    int           rv;
    mvpp2_port_t *mp = vnet_dev_get_port_data(port);
    ...
    if (mp->is_enabled) {
        ...
        if (rv) log_err(port->dev, "pp2_ppio_disable() failed, rv %d", rv);
...
vnet_dev_rv_t mvpp2_port_cfg_change_validate(vlib_main_t *vm, vnet_dev_port_t *port, vnet_dev_port_cfg_change_req_t *req)
{
    vnet_dev_rv_t rv = VNET_DEV_OK;
...
vnet_dev_rv_t mvpp2_port_cfg_change(vlib_main_t *vm, vnet_dev_port_t *port, vnet_dev_port_cfg_change_req_t *req)
{
    mvpp2_port_t *mp = vnet_dev_get_port_data(port);
    vnet_dev_rv_t rv = VNET_DEV_OK;
    eth_addr_t    addr;
    ...
    switch (req->type) {
        case VNET_DEV_PORT_CFG_PROMISC_MODE:
            ...
            if (mrv) {
                ...
                log_debug(port->dev, "pp2_ppio_set_promisc: promisc %u", req->promisc);
            break;
        ...
        case VNET_DEV_PORT_CFG_ADD_SECONDARY_HW_ADDR:
            ...
            if (mrv) {
                ...
                log_debug(port->dev, "pp2_ppio_add_mac_addr: %U added", format_ethernet_address, &addr);
            break;
        ...
        case VNET_DEV_PORT_CFG_REMOVE_SECONDARY_HW_ADDR:
            ...
            if (mrv) {
                ...
                log_debug(port->dev, "pp2_ppio_remove_mac_addr: %U added", format_ethernet_address, &addr);
            break;
