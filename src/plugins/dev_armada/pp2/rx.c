...
static_always_inline void mvpp2_rx_trace(vlib_main_t          *vm,
...
static_always_inline uword mrvl_pp2_rx_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, vnet_dev_rx_queue_t *rxq)
{
    ...
    if (PREDICT_FALSE(pp2_ppio_recv(mp->ppio, 0, rxq->queue_id, descs, &n_desc))) {
    ...
    for (d = descs, bi = buffers; n_desc >= 4; d += 2, bi += 2, n_desc -= 2) {
        ...
        if (PREDICT_FALSE(n_trace > 0)) {
            ...
            if (n_trace > 0) mvpp2_rx_trace(vm, node, rxq, b1, &n_trace, d + 1);
        }
    }
    ...
    for (; n_desc; d++, bi++, n_desc--) {
        ...
        if (PREDICT_FALSE(n_trace > 0)) mvpp2_rx_trace(vm, node, rxq, b0, &n_trace, d);
    }

    vlib_buffer_enqueue_to_single_next(vm, node, buffers, next_index, n_rx_packets);
    ...
    if (PREDICT_FALSE(pp2_bpool_get_num_buffs(bpool, &n_bufs))) {
        vlib_error_count(vm, node->node_index, MVPP2_RX_NODE_CTR_BPOOL_GET_NUM_BUFFS, 1);
        goto done;
    }
    ...
    while (n_bufs >= MRVL_PP2_BUFF_BATCH_SZ) {
        ...
        if (PREDICT_FALSE(n_alloc == 0)) {
            vlib_error_count(vm, node->node_index, MVPP2_RX_NODE_CTR_BUFFER_ALLOC, 1);
            goto done;
        }
        ...
        if (PREDICT_FALSE(pp2_bpool_put_buffs(hif, bre, &i))) {
            vlib_error_count(vm, node->node_index, MVPP2_RX_NODE_CTR_BPOOL_PUT_BUFFS, 1);
            vlib_buffer_free(vm, buffers, n_alloc);
            goto done;
        }

        if (PREDICT_FALSE(i != n_alloc)) vlib_buffer_free(vm, buffers + i, n_alloc - i);
