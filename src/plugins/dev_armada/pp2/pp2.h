...
#define log_debug(dev, f, ...) \
    vlib_log(VLIB_LOG_LEVEL_DEBUG, mvpp2_log.class, "%U" f, format_vnet_dev_log, (dev), clib_string_skip_prefix(__func__, "mvpp2_"), ##__VA_ARGS__)
#define log_info(dev, f, ...)   vlib_log(VLIB_LOG_LEVEL_INFO, mvpp2_log.class, "%U" f, format_vnet_dev_log, (dev), 0, ##__VA_ARGS__)
#define log_notice(dev, f, ...) vlib_log(VLIB_LOG_LEVEL_NOTICE, mvpp2_log.class, "%U" f, format_vnet_dev_log, (dev), 0, ##__VA_ARGS__)
#define log_warn(dev, f, ...)   vlib_log(VLIB_LOG_LEVEL_WARNING, mvpp2_log.class, "%U" f, format_vnet_dev_log, (dev), 0, ##__VA_ARGS__)
#define log_err(dev, f, ...)    vlib_log(VLIB_LOG_LEVEL_ERR, mvpp2_log.class, "%U" f, format_vnet_dev_log, (dev), 0, ##__VA_ARGS__)
...
#define foreach_mvpp2_tx_node_counter                          \
    ...
    _(PPIO_GET_NUM_OUTQ_DONE, ppio_get_num_outq_done, ERROR, "pp2_ppio_get_num_outq_done errors")
...
#define foreach_mvpp2_rx_node_counter                                                  \
    ...
    _(BPOOL_GET_NUM_BUFFS, bpool_get_num_bufs, ERROR, "pp2_bpool_get_num_buffs error") \
