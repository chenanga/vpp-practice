...
static int api_nsh_add_del_entry(vat_main_t *vam)
{
    ...
    if (nsp_set == 0) return -1;  // TODO Error type for this cond: clib_error_return (0, "nsp not specified");

    if (nsi_set == 0) return -2;  // TODO Error type for this cond:clib_error_return (0, "nsi not specified");
...
static void vl_api_nsh_entry_details_t_handler(vl_api_nsh_entry_details_t *mp)
{
...
static int api_nsh_add_del_map(vat_main_t *vam)
{
    ...
    if (nsp_set == 0 || nsi_set == 0) return -1;  // TODO create return value: clib_error_return (0, "nsp nsi pair required. Key: for NSH entry");
...
static void vl_api_nsh_map_details_t_handler(vl_api_nsh_map_details_t *mp)
{
    vat_main_t *vam = &vat_main;

    fformat(vam->ofp, "%14d%14d%14d%14d\n", ntohl(mp->nsp_nsi), ntohl(mp->mapped_nsp_nsi), ntohl(mp->sw_if_index), ntohl(mp->next_node));
}
