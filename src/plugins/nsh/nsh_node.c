...
always_inline void nsh_md2_encap(vlib_buffer_t *b, nsh_base_header_t *hdr, nsh_entry_t *nsh_entry)
{
    ...
    nsh_base->length = (nsh_base->length & NSH_TTL_L2_MASK) | ((nsh_entry->rewrite_size >> 2) & NSH_LEN_MASK);
    return;
}

always_inline void nsh_md2_swap(vlib_buffer_t *b, nsh_base_header_t *hdr, u32 header_len, nsh_entry_t *nsh_entry, u32 *next, u32 drop_node_val)
{
    nsh_main_t        *nm = &nsh_main;
    nsh_base_header_t *nsh_base;
    nsh_tlv_header_t  *opt0;
...
always_inline void nsh_md2_decap(vlib_buffer_t *b, nsh_base_header_t *hdr, u32 *header_len, u32 *next, u32 drop_node_val)
{
    nsh_main_t       *nm = &nsh_main;
    nsh_md2_data_t   *opt0;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (node_type == NSH_INPUT_TYPE) {
                ...
                ttl0        = (hdr0->ver_o_c & NSH_TTL_H4_MASK) << 2 | (hdr0->length & NSH_TTL_L2_MASK) >> 6;
                ttl0        = ttl0 - 1;
                ...
                char placeholder_dst_address[6] = {0x11, 0x22, 0x33, 0x44, 0x55, 0x66};
                char placeholder_src_address[6] = {0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc};
                clib_memcpy_fast(placeholder_eth0.dst_address, placeholder_dst_address, 6);
                clib_memcpy_fast(placeholder_eth0.src_address, placeholder_src_address, 6);
                placeholder_eth0.type = 0x0800;
                vlib_buffer_advance(b0, -(word) sizeof(ethernet_header_t));
                hdr0 = vlib_buffer_get_current(b0);
                clib_memcpy_fast(hdr0, &placeholder_eth0, (word) sizeof(ethernet_header_t));
            ...
            if (PREDICT_TRUE(map0->nsh_action == NSH_ACTION_SWAP)) {
                /* Manipulate MD2 */
                if (PREDICT_FALSE(hdr0->md_type == 2)) {
                    nsh_md2_swap(b0, hdr0, header_len0, nsh_entry0, &next0, NSH_NODE_NEXT_DROP);
                    if (PREDICT_FALSE(next0 == NSH_NODE_NEXT_DROP)) {
                        error0 = NSH_NODE_ERROR_INVALID_OPTIONS;
                        goto trace0;
                    }
        ...
        trace0:
            ...
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                ...
                clib_memcpy_fast(&(tr->trace_data), hdr0, ((hdr0->length & NSH_LEN_MASK) * 4));
            }
            ...
            if (node_type == NSH_INPUT_TYPE) {
                ...
                ttl1        = (hdr1->ver_o_c & NSH_TTL_H4_MASK) << 2 | (hdr1->length & NSH_TTL_L2_MASK) >> 6;
                ttl1        = ttl1 - 1;
                ...
                char placeholder_dst_address[6] = {0x11, 0x22, 0x33, 0x44, 0x55, 0x66};
                char placeholder_src_address[6] = {0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc};
                clib_memcpy_fast(placeholder_eth1.dst_address, placeholder_dst_address, 6);
                clib_memcpy_fast(placeholder_eth1.src_address, placeholder_src_address, 6);
                placeholder_eth1.type = 0x0800;
                vlib_buffer_advance(b1, -(word) sizeof(ethernet_header_t));
                hdr1 = vlib_buffer_get_current(b1);
                clib_memcpy_fast(hdr1, &placeholder_eth1, (word) sizeof(ethernet_header_t));
            ...
            if (PREDICT_FALSE(map1->nsh_action == NSH_ACTION_POP)) {
                ...
                if (PREDICT_FALSE(hdr1->md_type == 2)) {
                    ...
                    vnet_buffer(b1)->sw_if_index[VLIB_RX] = map1->rx_sw_if_index;
                }
            ...
            if (PREDICT_TRUE(map1->nsh_action == NSH_ACTION_SWAP)) {
                /* Manipulate MD2 */
                if (PREDICT_FALSE(hdr1->md_type == 2)) {
                    nsh_md2_swap(b1, hdr1, header_len1, nsh_entry1, &next1, NSH_NODE_NEXT_DROP);
                    if (PREDICT_FALSE(next1 == NSH_NODE_NEXT_DROP)) {
                        error1 = NSH_NODE_ERROR_INVALID_OPTIONS;
                        goto trace1;
                    }
        ...
        trace1:
            ...
            if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
                ...
                clib_memcpy_fast(&(tr->trace_data), hdr1, ((hdr1->length & NSH_LEN_MASK) * 4));
            }
        ...
        trace00:
            ...
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                nsh_input_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
                clib_memcpy_fast(&(tr->trace_data[0]), hdr0, ((hdr0->length & NSH_LEN_MASK) * 4));
            }
