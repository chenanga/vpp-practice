...
void nsh_md2_ioam_refresh_output_feature_on_all_dest(void)
{
    ...
    pool_foreach (t, hm->dst_tunnels) {
        nsh_md2_ioam_enable_disable_for_dest(gm->vlib_main, t->dst_addr, t->outer_fib_index, (t->fp_proto == FIB_PROTOCOL_IP4), 1
                                             /* is_add */
        );
    }
    return;
...
void nsh_md2_ioam_clear_output_feature_on_select_intfs(void)
{
    ...
    for (sw_if_index0 = 0; sw_if_index0 < vec_len(hm->bool_ref_by_sw_if_index); sw_if_index0++) {
        if (hm->bool_ref_by_sw_if_index[sw_if_index0] == 0xFF) {
            nsh_md2_ioam_set_clear_output_feature_on_intf(gm->vlib_main, sw_if_index0, 0);
        }
    }
...
int nsh_md2_ioam_disable_for_dest(vlib_main_t *vm, ip46_address_t dst_addr, u32 outer_fib_index, u8 ipv4_set)
{
    ...
    nsh_md2_ioam_enable_disable_for_dest(gm->vlib_main, dst_addr, outer_fib_index, ipv4_set, 0);
    ...
    pool_foreach (t, hm->dst_tunnels) {
        nsh_md2_ioam_enable_disable_for_dest(gm->vlib_main, t->dst_addr, t->outer_fib_index, (t->fp_proto == FIB_PROTOCOL_IP4), 1 /* is_add */);
    }
    nsh_md2_ioam_clear_output_feature_on_select_intfs();
    return (0);
}
...
static clib_error_t *nsh_md2_ioam_set_transit_rewrite_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (dst_addr_set == 0) return clib_error_return(0, "LISP-GPE Tunnel destination address not specified");
    if (ipv4_set && ipv6_set) return clib_error_return(0, "both IPv4 and IPv6 addresses specified");
    if (!disable) {
        nsh_md2_ioam_enable_disable_for_dest(gm->vlib_main, dst_addr, outer_fib_index, ipv4_set, 1);
    }
    else {
