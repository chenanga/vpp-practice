...
#define foreach_nsh_md2_ioam_encap_transit_error _(ENCAPSULATED, "good packets encapsulated")
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE((ip0->ip_version_and_header_length & 0xF0) == 0x40)) {
                ...
                if (PREDICT_FALSE((ip0->protocol == IP_PROTOCOL_UDP) && (clib_net_to_host_u16(udp_hdr0->dst_port) == UDP_DST_PORT_lisp_gpe))) {
                    ...
                    if (PREDICT_FALSE(lisp_gpe_hdr0->next_protocol == LISP_GPE_NEXT_PROTO_NSH) && (nsh_hdr->md_type == 2)) {
                        uword               *t  = NULL;
                        ...
                        if (t) {
                            ...
                            nsh_md2_ioam_encap_decap_ioam_v4_one_inline(vm, node, b0, &next0, NSH_MD2_IOAM_ENCAP_TRANSIT_IOAM_NEXT_DROP, 1
                                                                        /* use_adj */
                            );
                            vlib_buffer_advance(b0, -(word) (sizeof(ethernet_header_t)));
                        }
