...
always_inline trace_profile *nsh_trace_profile_find(void)
{
...
always_inline void nsh_md2_ioam_trace_stats_increment_counter(u32 counter_index, u64 increment)
{
...
int nsh_md2_ioam_trace_rewrite_handler(u8 *rewrite_string, u8 *rewrite_size)
{
    ...
    trace_option->ioam_trace_type     = clib_host_to_net_u16(profile->trace_type & TRACE_TYPE_MASK);

    *rewrite_size = sizeof(nsh_md2_ioam_trace_option_t) + (trace_option_elts * trace_data_size);
...
{
    u8                           elt_index = 0;
    nsh_md2_ioam_trace_option_t *trace     = (nsh_md2_ioam_trace_option_t *) ((u8 *) opt);
    time_u64_t                   time_u64;
    u32                         *elt;
    int                          rv              = 0;
    trace_profile               *profile         = NULL;
    ...
    if (PREDICT_TRUE(trace->data_list_elts_left)) {
         ...
         */
        elt_index = trace->data_list_elts_left * fetch_trace_data_size(ioam_trace_type) / 4;
        elt       = &trace->elts[elt_index];
        if (ioam_trace_type & BIT_TTL_NODEID) {
            ip4_header_t *ip0 = vlib_buffer_get_current(b);
            *elt              = clib_host_to_net_u32(((ip0->ttl - 1) << 24) | profile->node_id);
            elt++;
        }
        ...
        if (ioam_trace_type & BIT_TIMESTAMP) {
            ...
            f64 time_f64 = (f64) (((f64) hm->unix_time_0) + (vlib_time_now(gm->vlib_main) - hm->vlib_time_0));
        ...
        nsh_md2_ioam_trace_stats_increment_counter(NSH_MD2_IOAM_TRACE_SUCCESS, 1);
    }
    else {
...
{
    clib_memcpy_fast(new_opt, old_opt, new_opt->length + sizeof(nsh_tlv_header_t));
...
static clib_error_t *nsh_md2_ioam_show_ioam_trace_cmd_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (i = 0; i < NSH_MD2_IOAM_TRACE_N_STATS; i++) {
        s = format(s, " %s - %lu\n", nsh_md2_ioam_trace_stats_strings[i], hm->counters[i]);
    }
...
static clib_error_t *nsh_md2_ioam_trace_init(vlib_main_t *vm)
{
    ...
    if (nsh_md2_register_option(clib_host_to_net_u16(0x9),
                                ...
                                nsh_md2_ioam_trace_data_list_trace_handler)
        < 0)
        return (clib_error_create("registration of NSH_MD2_IOAM_OPTION_TYPE_TRACE failed"));
...
static int nsh_md2_ioam_trace_get_sizeof_handler(u32 *result)
{
    ...
    *result = 0;
    ...
    size += sizeof(nsh_md2_ioam_trace_option_t) + profile->num_elts * trace_data_size;
    *result = size;
...
int nsh_md2_ioam_trace_profile_setup(void)
{
    ...
    if (nsh_md2_ioam_trace_get_sizeof_handler(&trace_size) < 0) return (-1);
