...
int nsh_md2_register_option(u16 class,
                            u8  type,
                            u8  option_size,
                            int add_options(u8 *opt, u8 *opt_size),
                            int options(vlib_buffer_t *b, nsh_tlv_header_t *opt),
                            int swap_options(vlib_buffer_t *b, nsh_tlv_header_t *old_opt, nsh_tlv_header_t *new_opt),
                            ...
                            u8 *trace(u8 *s, nsh_tlv_header_t *opt))
{
    ...
    hash_set_mem(nm->nsh_option_map_by_key, key_copy, nsh_option - nm->nsh_option_mappings);
...
clib_error_t *nsh_init(vlib_main_t *vm)
{
    ...
    nm->nsh_mapping_by_mapped_key = hash_create_mem(0, sizeof(u32), sizeof(uword));

    nm->nsh_entry_by_key = hash_create_mem(0, sizeof(u32), sizeof(uword));

    nm->nsh_proxy_session_by_key = hash_create_mem(0, sizeof(nsh_proxy_session_by_key_t), sizeof(uword));

    nm->nsh_option_map_by_key = hash_create_mem(0, sizeof(nsh_option_map_by_key_t), sizeof(uword));
    ...
    next_node = vlib_node_add_next(vm, vxlan4_gpe_input_node.index, nm->nsh_input_node_index);
    vlib_node_add_next(vm, vxlan4_gpe_input_node.index, nm->nsh_proxy_node_index);
    ...
    vlib_node_add_next(vm, vxlan6_gpe_input_node.index, nm->nsh_input_node_index);
    vlib_node_add_next(vm, vxlan6_gpe_input_node.index, nm->nsh_proxy_node_index);
    ...
    vxlan4_input = vlib_get_plugin_symbol("vxlan_plugin.so", "vxlan4_input_node");
    vxlan6_input = vlib_get_plugin_symbol("vxlan_plugin.so", "vxlan6_input_node");
    ...
    vlib_node_add_next(vm, ip4_classify_node.index, nm->nsh_classifier_node_index);
    vlib_node_add_next(vm, ip6_classify_node.index, nm->nsh_classifier_node_index);
    vlib_node_add_next(vm, l2_input_classify_node.index, nm->nsh_classifier_node_index);

    /* Add Ethernet+NSH support */
    ethernet_register_input_type(vm, ETHERNET_TYPE_NSH, nm->nsh_input_node_index);
