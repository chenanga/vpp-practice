...
static void send_nsh_entry_details(nsh_entry_t *t, vl_api_registration_t *rp, u32 context)
{
    ...
    rmp->ttl           = (t->nsh_base.ver_o_c & NSH_TTL_H4_MASK) << 2 | (t->nsh_base.length & NSH_TTL_L2_MASK) >> 6;
...
static void vl_api_nsh_add_del_map_t_handler(vl_api_nsh_add_del_map_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_NSH_ADD_DEL_MAP_REPLY, ({ rmp->map_index = htonl(map_index); }));
}
    ...
    if (nsh_base->md_type == 1) {
        ...
        while (opt0 < limit0) {
            ...
            if (nm->add_options[nsh_option->option_id] != NULL) {
                if (0 != nm->add_options[nsh_option->option_id]((u8 *) nsh_md2, &new_option_size)) {
                    goto next_tlv_md2;
                }
...
int nsh_add_del_map(nsh_add_del_map_args_t *a, u32 *map_indexp)
{
    ...
    if (a->is_add) {
        ...
        if (entry) return -1;  // TODO API_ERROR_INVALID_VALUE;
        ...
        if (vec_len(nm->free_nsh_tunnel_hw_if_indices) > 0) {
            nsh_hw_if = nm->free_nsh_tunnel_hw_if_indices[vec_len(nm->free_nsh_tunnel_hw_if_indices) - 1];
            ...
            nsh_hw_if             = vnet_register_interface(vnm, nsh_device_class.index, map_index, nsh_hw_class.index, map_index);
            hi                    = vnet_get_hw_interface(vnm, nsh_hw_if);
        ...
        if (!entry) return -2;  // TODO API_ERROR_NO_SUCH_ENTRY;

        map = pool_elt_at_index(nm->nsh_mappings, entry[0]);

        vnet_sw_interface_set_flags(vnm, map->nsh_sw_if, VNET_SW_INTERFACE_FLAG_ADMIN_DOWN);
    ...
    if (map_indexp) *map_indexp = map_index;
...
int nsh_add_del_proxy_session(nsh_add_del_map_args_t *a)
{
    ...
    if (a->is_add) {
        ...
        hash_set_mem(nm->nsh_proxy_session_by_key, key_copy, proxy - nm->nsh_proxy_sessions);
    }
    else {
        if (!entry) return -2;  // TODO API_ERROR_NO_SUCH_ENTRY;
...
int nsh_add_del_entry(nsh_add_del_entry_args_t *a, u32 *entry_indexp)
{
    ...
    if (a->is_add) {
        /* adding an entry, must not already exist */
        if (entry_id) return -1;  // TODO VNET_API_ERROR_INVALID_VALUE;
...
#undef _
        ...
        hash_set_mem(nm->nsh_entry_by_key, key_copy, nsh_entry - nm->nsh_entries);
        entry_index = nsh_entry - nm->nsh_entries;
    }
    else {
        if (!entry_id) return -2;  // TODO API_ERROR_NO_SUCH_ENTRY;
    ...
    if (entry_indexp) *entry_indexp = entry_index;
...
static void vl_api_nsh_add_del_entry_t_handler(vl_api_nsh_add_del_entry_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_NSH_ADD_DEL_ENTRY_REPLY, ({ rmp->entry_index = htonl(entry_index); }));
}
