...
u8 *format_nsh_map(u8 *s, va_list *args)
{
    ...
    s = format(s, "nsh entry nsp: %d nsi: %d ", (map->nsp_nsi >> NSH_NSP_SHIFT) & NSH_NSP_MASK, map->nsp_nsi & NSH_NSI_MASK);
    s = format(s, "maps to nsp: %d nsi: %d ", (map->mapped_nsp_nsi >> NSH_NSP_SHIFT) & NSH_NSP_MASK, map->mapped_nsp_nsi & NSH_NSI_MASK);
...
static clib_error_t *nsh_add_del_map_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "del"))
            ...
            mapped_nsi_set = 1;
        else if (unformat(line_input, "nsh_action %U", unformat_nsh_action, &nsh_action))
            nsh_action_set = 1;
    ...
    if (nsp_set == 0 || nsi_set == 0) return clib_error_return(0, "nsp nsi pair required. Key: for NSH entry");

    if (mapped_nsp_set == 0 || mapped_nsi_set == 0) return clib_error_return(0, "mapped-nsp mapped-nsi pair required. Key: for NSH entry");

    if (nsh_action_set == 0) return clib_error_return(0, "nsh_action required: swap|push|pop.");

    if (next_node == ~0)
        return clib_error_return(0,
                                 "must specific action: [encap-gre-intf <nn> | encap-vxlan-gpe-intf <nn> | encap-lisp-gpe-intf <nn> | encap-none "
                                 "<tx_sw_if_index> <rx_sw_if_index>]");
    ...
    if ((a->map.next_node == NSH_NODE_NEXT_ENCAP_VXLAN4) | (a->map.next_node == NSH_NODE_NEXT_ENCAP_VXLAN6)) {
        ...
        switch (rv) {
            ...
            default:
                return clib_error_return(0, "nsh_add_del_proxy_session() returned %d", rv);
        }
    }
...
static clib_error_t *show_nsh_map_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (pool_elts(nm->nsh_mappings) == 0) vlib_cli_output(vm, "No nsh maps configured.");
...
static clib_error_t *nsh_add_del_entry_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (nsp_set == 0) return clib_error_return(0, "nsp not specified");

    if (nsi_set == 0) return clib_error_return(0, "nsi not specified");

    if (md_type == 1 && has_ioam_trace_option == 1) return clib_error_return(0, "Invalid MD Type");
    ...
    if (md_type == 1) {
        ...
        if (has_ioam_trace_option) {
            ...
            if (nsh_option == NULL) return clib_error_return(0, "iOAM Trace not registered");

            if (nm->add_options[nsh_option->option_id] != NULL) {
                if (0 != nm->add_options[nsh_option->option_id]((u8 *) current, &option_size)) {
...
u8 *format_nsh_header(u8 *s, va_list *args)
{
    ...
    limit0                      = (nsh_md2_data_t *) ((u8 *) nsh_md2 + ((nsh_base->length & NSH_LEN_MASK) * 4 - sizeof(nsh_base_header_t)));

    s = format(s, "nsh ver %d ", (nsh_base->ver_o_c >> 6));
    if (nsh_base->ver_o_c & NSH_O_BIT) s = format(s, "O-set ");

    if (nsh_base->ver_o_c & NSH_C_BIT) s = format(s, "C-set ");

    s = format(s, "ttl %d ", (nsh_base->ver_o_c & NSH_TTL_H4_MASK) << 2 | (nsh_base->length & NSH_TTL_L2_MASK) >> 6);
        ...
        while (opt0 < limit0) {
            ...
            if (nsh_option != NULL) {
                if (nm->trace[nsh_option->option_id] != NULL) {
                    ...
                    s = format(s, "\n    untraced option %d length %d", opt0->type, opt0->length);
                }
            }
            else {
                s = format(s, "\n    unrecognized option %d length %d", opt0->type, opt0->length);
            }
...
static clib_error_t *show_nsh_entry_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (pool_elts(nm->nsh_entries) == 0) vlib_cli_output(vm, "No nsh entries configured.");
