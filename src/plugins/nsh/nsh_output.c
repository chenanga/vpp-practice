...
static u8 *format_nsh_output_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "\n%U%U", format_white_space, indent, format_ip_adjacency_packet_data, t->adj_index, t->packet_data, sizeof(t->packet_data));
    return s;
}

static inline uword nsh_output_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, int is_midchain)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (adj_index0 == adj_index1) {
                vlib_increment_combined_counter(&adjacency_counters, thread_index, adj_index0, 2, pkt_len0 + rw_len0 + pkt_len1 + rw_len1);
            }
            else {
                vlib_increment_combined_counter(&adjacency_counters, thread_index, adj_index0, 1, pkt_len0 + rw_len0);
                vlib_increment_combined_counter(&adjacency_counters, thread_index, adj_index1, 1, pkt_len1 + rw_len1);
            }
            ...
            if (PREDICT_TRUE(pkt_len0 <= adj0[0].rewrite_header.max_l3_packet_bytes)) {
                ...
                if (PREDICT_FALSE(adj0[0].rewrite_header.flags & VNET_REWRITE_HAS_FEATURES))
                    vnet_feature_arc_start(nm->output_feature_arc_index, adj0[0].rewrite_header.sw_if_index, &next0, p0);
            }
            else {
            ...
            if (PREDICT_TRUE(pkt_len1 <= adj1[0].rewrite_header.max_l3_packet_bytes)) {
                ...
                if (PREDICT_FALSE(adj1[0].rewrite_header.flags & VNET_REWRITE_HAS_FEATURES))
                    vnet_feature_arc_start(nm->output_feature_arc_index, adj1[0].rewrite_header.sw_if_index, &next1, p1);
            }
            else {
            ...
            if (is_midchain) {
                adj0->sub_type.midchain.fixup_func(vm, adj0, p0, adj0->sub_type.midchain.fixup_data);
                adj1->sub_type.midchain.fixup_func(vm, adj1, p1, adj1->sub_type.midchain.fixup_data);
            }
            ...
            if (PREDICT_FALSE(p0->flags & VLIB_BUFFER_IS_TRACED)) {
                nsh_output_trace_t *tr = vlib_add_trace(vm, node, p0, sizeof(*tr));
                tr->adj_index          = vnet_buffer(p0)->ip.adj_index[VLIB_TX];
                tr->flow_hash          = vnet_buffer(p0)->ip.flow_hash;
            }
            if (PREDICT_FALSE(p1->flags & VLIB_BUFFER_IS_TRACED)) {
                nsh_output_trace_t *tr = vlib_add_trace(vm, node, p1, sizeof(*tr));
...
VLIB_NODE_FN(nsh_eth_output_node)
...
VLIB_NODE_FN(nsh_midchain_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
 ...
 * exception case.
 */
VLIB_NODE_FN(nsh_adj_incomplete_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(p0->flags & VLIB_BUFFER_IS_TRACED)) {
                nsh_adj_incomplete_trace_t *tr = vlib_add_trace(vm, node, p0, sizeof(*tr));
                tr->next                       = next0;
            }
