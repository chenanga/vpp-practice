...
static uword lldp_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    /* with ethernet input */
    ethernet_register_input_type(vm, ETHERNET_TYPE_802_1_LLDP /* LLDP */, lldp_input_node.index);
    ...
    while (1) {
        ...
        while (1) {
            lldp_intf_t *n = pool_elt_at_index(lm->intfs, lm->intfs_timeouts[lm->intfs_timeouts_idx]);
            if (n->last_sent < 0.01 || now > n->last_sent + lm->msg_tx_interval) {
#if LLDP_DEBUG
                clib_warning("send packet to lldp %p, if idx %d", n, n->hw_if_index);
#endif
...
#if LLDP_DEBUG
    clib_warning("DEBUG: unschedule interface %p, if idx %d", n, n->hw_if_index);
#endif
    const int idx = n - lm->intfs;
    u32       v;
