...
static void
lldp_rpc_update_peer(u32 hw_if_index, const u8 *chid, u8 chid_len, u8 chid_subtype, const u8 *portid, u8 portid_len, u8 portid_subtype, u16 ttl)
{
...
#define TLV_VIOLATES_PKT_BOUNDARY(pkt, tlv) \
    (((((u8 *) tlv) + sizeof(lldp_tlv_t)) > ((u8 *) pkt + vec_len(pkt))) || ((((u8 *) tlv) + lldp_tlv_get_length(tlv)) > ((u8 *) pkt + vec_len(pkt))))

    /* first tlv is always chassis id, followed by port id and ttl tlvs */
    if (TLV_VIOLATES_PKT_BOUNDARY(pkt, tlv) || LLDP_TLV_NAME(chassis_id) != lldp_tlv_get_code(tlv)) {
        return LLDP_ERROR_BAD_TLV;
    }

    u16 l = lldp_tlv_get_length(tlv);
    if (l < STRUCT_SIZE_OF(lldp_chassis_id_tlv_t, subtype) + LLDP_MIN_CHASS_ID_LEN
        || l > STRUCT_SIZE_OF(lldp_chassis_id_tlv_t, subtype) + LLDP_MAX_CHASS_ID_LEN) {
        return LLDP_ERROR_BAD_TLV;
    }
    ...
    if (TLV_VIOLATES_PKT_BOUNDARY(pkt, tlv) || LLDP_TLV_NAME(port_id) != lldp_tlv_get_code(tlv)) {
        return LLDP_ERROR_BAD_TLV;
    }
    l = lldp_tlv_get_length(tlv);
    if (l < STRUCT_SIZE_OF(lldp_port_id_tlv_t, subtype) + LLDP_MIN_PORT_ID_LEN
        || l > STRUCT_SIZE_OF(lldp_chassis_id_tlv_t, subtype) + LLDP_MAX_PORT_ID_LEN) {
        return LLDP_ERROR_BAD_TLV;
    }
    ...
    if (TLV_VIOLATES_PKT_BOUNDARY(pkt, tlv) || LLDP_TLV_NAME(ttl) != lldp_tlv_get_code(tlv)) {
        return LLDP_ERROR_BAD_TLV;
    }
    l = lldp_tlv_get_length(tlv);
    ...
    tlv     = (lldp_tlv_t *) ((u8 *) tlv + STRUCT_SIZE_OF(lldp_tlv_t, head) + l);
    while (!TLV_VIOLATES_PKT_BOUNDARY(pkt, tlv) && LLDP_TLV_NAME(pdu_end) != lldp_tlv_get_code(tlv)) {
...
#undef F
        ...
        tlv = (lldp_tlv_t *) ((u8 *) tlv + STRUCT_SIZE_OF(lldp_tlv_t, head) + lldp_tlv_get_length(tlv));
    }
    /* last tlv is pdu_end */
    if (TLV_VIOLATES_PKT_BOUNDARY(pkt, tlv) || LLDP_TLV_NAME(pdu_end) != lldp_tlv_get_code(tlv) || 0 != lldp_tlv_get_length(tlv)) {
        return LLDP_ERROR_BAD_TLV;
    }
    lldp_rpc_update_peer(hw_if_index, chid, chid_len, chid_subtype, portid, portid_len, portid_subtype, ttl);
...
lldp_error_t lldp_input(vlib_main_t *vm, vlib_buffer_t *b0, u32 bi0)
{
    ...
    /* find our interface */
    vnet_sw_interface_t *sw_interface = vnet_get_sw_interface(lm->vnet_main, vnet_buffer(b0)->sw_if_index[VLIB_RX]);
...
static clib_error_t *lldp_init(vlib_main_t *vm)
{
    ...
    if ((error = vlib_call_init_function(vm, lldp_template_init))) return error;
