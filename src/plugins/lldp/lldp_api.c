...
static void vl_api_lldp_config_t_handler(vl_api_lldp_config_t *mp)
{
    ...
    if (lldp_cfg_set(&sys_name, ntohl(mp->tx_hold), ntohl(mp->tx_interval)) != lldp_ok) {
        vec_free(sys_name);
...
static void vl_api_sw_interface_set_lldp_t_handler(vl_api_sw_interface_set_lldp_t *mp)
{
    ...
    if (lldp_cfg_intf_set(ntohl(mp->sw_if_index), (u8 **) &port_desc, &mgmt_ip4, &mgmt_ip6, &mgmt_oid, mp->enable) != lldp_ok) {
        vec_free(port_desc);
        vec_free(mgmt_ip4);
        vec_free(mgmt_ip6);
        vec_free(mgmt_oid);
...
static void send_lldp(u32 index, vl_api_registration_t *rp, u32 context)
{
    ...
    REPLY_MACRO_DETAILS4_END(VL_API_LLDP_DETAILS, rp, context, ({
                                 ...
                                 rmp->chassis_id_subtype = (vl_api_chassis_id_subtype_t) n->chassis_id_subtype;
...
static void vl_api_lldp_dump_t_handler(vl_api_lldp_dump_t *mp)
{
    int                       rv = 0;
    lldp_main_t              *lm = &lldp_main;
    vl_api_lldp_dump_reply_t *rmp;

    REPLY_AND_DETAILS_MACRO_END(VL_API_LLDP_DUMP_REPLY, lm->intfs, ({ send_lldp(cursor, rp, mp->context); }));
}
