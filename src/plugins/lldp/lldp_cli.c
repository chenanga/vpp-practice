...
lldp_cfg_err_t lldp_cfg_intf_set(u32 hw_if_index, u8 **port_desc, u8 **mgmt_ip4, u8 **mgmt_ip6, u8 **mgmt_oid, int enable)
{
    ...
    if (enable) {
        ...
        if (hi->caps & VNET_HW_IF_CAP_MAC_FILTER) {
            error = vnet_hw_interface_add_del_mac_address(lm->vnet_main, hw_if_index, lldp_mac_addr, 1 /* is_add */);
            if (error) {
        ...
        const vnet_sw_interface_t *sw = vnet_get_sw_interface(lm->vnet_main, hi->sw_if_index);
        ...
        if ((n) && (hi->caps & VNET_HW_IF_CAP_MAC_FILTER)) {
            error = vnet_hw_interface_add_del_mac_address(lm->vnet_main, hw_if_index, lldp_mac_addr, 0 /* is_add */);
            if (error) {
                clib_error_free(error);
            }
        }
...
static clib_error_t *lldp_intf_cmd(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        if (unformat(input, "%U", unformat_vnet_sw_interface, vnm, &sw_if_index))
            ...
            ;
        else if (unformat(input, "mgmt-ip4 %U", unformat_ip4_address, &ip4_addr)) {
            vec_validate(mgmt_ip4, sizeof(ip4_address_t) - 1);
            clib_memcpy(mgmt_ip4, &ip4_addr, sizeof(ip4_addr));
        }
        else if (unformat(input, "mgmt-ip6 %U", unformat_ip6_address, &ip6_addr)) {
    ...
    if (sw_if_index == (u32) ~0) return clib_error_return(0, "Interface name is invalid!");

    return lldp_cfg_err_to_clib_err(lldp_cfg_intf_set(sw_if_index, &port_desc, &mgmt_ip4, &mgmt_ip6, &mgmt_oid, enable));
}
...
lldp_cfg_err_t lldp_cfg_set(u8 **host, int hold_time, int tx_interval)
{
    ...
    if (tx_interval) {
        if (tx_interval < LLDP_MIN_TX_INTERVAL || tx_interval > LLDP_MAX_TX_INTERVAL) {
            return lldp_invalid_arg;
        }
    ...
    if (reschedule) {
        vlib_process_signal_event(lm->vlib_main, lm->lldp_process_node_index, LLDP_EVENT_RESCHEDULE, 0);
    }
...
static clib_error_t *lldp_cfg_cmd(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "system-name %s", &host)) {
            ...
            if (hold_time < LLDP_MIN_TX_HOLD || hold_time > LLDP_MAX_TX_HOLD) {
                ret = clib_error_return(0, "invalid tx-hold `%d' (out of range <%d,%d>)", hold_time, LLDP_MIN_TX_HOLD, LLDP_MAX_TX_HOLD);
                goto out;
            }
        }
            ...
            if (tx_interval < LLDP_MIN_TX_INTERVAL || tx_interval > LLDP_MAX_TX_INTERVAL) {
                ret =
                    clib_error_return(0, "invalid tx-interval `%d' (out of range <%d,%d>)", tx_interval, LLDP_MIN_TX_INTERVAL, LLDP_MAX_TX_INTERVAL);
                goto out;
            }
        }
        else {
    ...
    ret = lldp_cfg_err_to_clib_err(lldp_cfg_set(&host, hold_time, tx_interval));
out:
    vec_free(host);
    return ret;
}
...
u8 *format_lldp_port_id(u8 *s, va_list *va)
{
    ...
    switch (subtype) {
        ...
        case LLDP_PORT_ID_SUBTYPE_NAME(intf_name):
            if (detail) {
                s = format(s, "%U(%s)", format_ascii_bytes, id, len, lldp_port_id_subtype_str(subtype));
            }
            else {
        ...
        case LLDP_PORT_ID_SUBTYPE_NAME(mac_addr):
            if (ETHER_ADDR_LEN == len) {
                if (detail) {
                    s = format(s, "%U(%s)", format_mac_address, id, lldp_port_id_subtype_str(subtype));
                }
                else {
        ...
        default:
            if (detail) {
                s = format(s, "%U(%s)", format_hex_bytes, id, len, lldp_port_id_subtype_str(subtype));
            }
            else {
...
{
    const lldp_chassis_id_subtype_t subtype = va_arg(*va, lldp_chassis_id_subtype_t);
    const u8                       *id      = va_arg(*va, u8 *);
    const unsigned                  len     = va_arg(*va, unsigned);
    const int                       detail  = va_arg(*va, int);
    if (!id) {
    ...
    switch (subtype) {
        ...
        case LLDP_CHASS_ID_SUBTYPE_NAME(intf_name):
            if (detail) {
                s = format(s, "%U(%s)", format_ascii_bytes, id, len, lldp_chassis_id_subtype_str(subtype));
            }
            else {
                s = format(s, "%U", format_ascii_bytes, id, len);
            }
        ...
        case LLDP_CHASS_ID_SUBTYPE_NAME(mac_addr):
            if (ETHER_ADDR_LEN == len) {
                if (detail) {
                    s = format(s, "%U(%s)", format_mac_address, id, lldp_chassis_id_subtype_str(subtype));
                }
                else {
                    s = format(s, "%U", format_mac_address, id);
                }
        ...
        default:
            if (detail) {
                s = format(s, "%U(%s)", format_hex_bytes, id, len, lldp_chassis_id_subtype_str(subtype));
            }
            else {
                s = format(s, "%U", format_hex_bytes, id, len);
            }
    ...
    switch (lldp_tlv_get_code(tlv)) {
        ...
        case LLDP_TLV_NAME(port_id):
            s = format(s,
                       ...
                       l - STRUCT_SIZE_OF(lldp_port_id_tlv_t, subtype),
                       1);
            break;
...
static u8 *format_lldp_intfs_detail(u8 *s, vlib_main_t *vm, const lldp_main_t *lm)
{
    ...
    if (lm->sys_name) {
        s = format(s, "Configured system name: %U\n", format_ascii_bytes, lm->sys_name, vec_len(lm->sys_name));
    }
    ...
    pool_foreach (n, lm->intfs) {
        ...
        if (!(sw->flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP)) {
            ...
            s = format(s,
                       ...
                       format_lldp_chassis_id,
                       n->chassis_id_subtype,
                       n->chassis_id,
                       vec_len(n->chassis_id),
                       1,
                       format_lldp_port_id,
                       n->port_id_subtype,
                       n->port_id,
...
static u8 *format_lldp_intfs(u8 *s, va_list *va)
{
    ...
    pool_foreach (n, lm->intfs) {
        ...
        const vnet_sw_interface_t *sw = vnet_get_sw_interface(lm->vnet_main, hw->sw_if_index);
        /* Interface shutdown */
        if (!(sw->flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP)) continue;
...
u8 *lldp_input_format_trace(u8 *s, va_list *args)
{
    ...
    while (((cur + lldp_tlv_get_length((lldp_tlv_t *) cur)) < t->data + t->len)) {
        tlv = (lldp_tlv_t *) cur;
        if (cur == t->data) {
            s = format(s, "TLV #%d(%s): %U\n", lldp_tlv_get_code(tlv), lldp_tlv_code_str(lldp_tlv_get_code(tlv)), format_lldp_tlv, tlv);
        }
        else {
            s = format(s, "  TLV #%d(%s): %U\n", lldp_tlv_get_code(tlv), lldp_tlv_code_str(lldp_tlv_get_code(tlv)), format_lldp_tlv, tlv);
        }
