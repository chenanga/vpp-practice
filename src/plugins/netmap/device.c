...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left && cur_ring <= nif->last_tx_ring) {
        ...
        if (nm_tx_pending(ring)) {
            if (ioctl(nif->fd, NIOCTXSYNC, NULL) < 0) clib_unix_warning("NIOCTXSYNC");
        ...
        while (n_left && n_free_slots) {
            ...
            do {
                ...
                /* memcpy */
                clib_memcpy_fast((u8 *) NETMAP_BUF(ring, slot->buf_idx) + offset, vlib_buffer_get_current(b0), len);
                offset += len;
    ...
    if (n_left < frame->n_vectors) ioctl(nif->fd, NIOCTXSYNC, NULL);

    clib_spinlock_unlock_if_init(&nif->lockp);

    if (n_left)
        vlib_error_count(vm, node->node_index, (n_left == frame->n_vectors ? NETMAP_TX_ERROR_PENDING_MSGS : NETMAP_TX_ERROR_NO_FREE_SLOTS), n_left);
...
static void netmap_set_interface_next_node(vnet_main_t *vnm, u32 hw_if_index, u32 node_index)
{
    ...
    nif->per_interface_next_index = vlib_node_add_next(vlib_get_main(), netmap_input_node.index, node_index);
}
