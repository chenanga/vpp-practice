...
static u8 *format_netmap_input_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "netmap: hw_if_index %d next-index %d", t->hw_if_index, t->next_index);
    s = format(s, "\n%Uslot: flags 0x%x len %u buf_idx %u", format_white_space, indent + 2, t->slot.flags, t->slot.len, t->slot.buf_idx);
    return s;
}
...
always_inline uword netmap_device_input_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, netmap_if_t *nif)
{
    ...
    if (nif->per_interface_next_index != ~0) next_index = nif->per_interface_next_index;

    n_free_bufs = vec_len(nm->rx_buffers[thread_index]);
    if (PREDICT_FALSE(n_free_bufs < VLIB_FRAME_SIZE)) {
        vec_validate(nm->rx_buffers[thread_index], VLIB_FRAME_SIZE + n_free_bufs - 1);
        n_free_bufs += vlib_buffer_alloc(vm, &nm->rx_buffers[thread_index][n_free_bufs], VLIB_FRAME_SIZE);
    ...
    while (cur_ring <= nif->last_rx_ring && n_free_bufs) {
        ...
        if (r > n_free_bufs) r = n_free_bufs;
        ...
        while (r) {
            ...
            while (r && n_left_to_next) {
                ...
                CLIB_PREFETCH(NETMAP_BUF(ring, ring->slot[next_slot_index].buf_idx), CLIB_CACHE_LINE_BYTES, LOAD);

                while (data_len && n_free_bufs) {
                    vlib_buffer_t *b0;
                    /* grab free buffer */
                    u32 last_empty_buffer = vec_len(nm->rx_buffers[thread_index]) - 1;
                    prev_bi0              = bi0;
                    bi0                   = nm->rx_buffers[thread_index][last_empty_buffer];
                    b0                    = vlib_get_buffer(vm, bi0);
                    vec_set_len(nm->rx_buffers[thread_index], last_empty_buffer);
                    n_free_bufs--;

                    /* copy data */
                    u32 bytes_to_copy = data_len > n_buffer_bytes ? n_buffer_bytes : data_len;
                    b0->current_data  = 0;
                    clib_memcpy_fast(vlib_buffer_get_current(b0), (u8 *) NETMAP_BUF(ring, slot->buf_idx) + offset, bytes_to_copy);
    ...
    if (n_rx_packets) ioctl(nif->fd, NIOCRXSYNC, NULL);
...
VLIB_NODE_FN(netmap_input_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    for (i = 0; i < vec_len(nm->interfaces); i++) {
        ...
        if (nmi->is_admin_up && (i % nm->input_cpu_count) == (thread_index - nm->input_cpu_first_index))
