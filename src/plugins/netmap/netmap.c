...
int netmap_create_if(vlib_main_t *vm, u8 *if_name, u8 *hw_addr_set, u8 is_pipe, u8 is_master, u32 *sw_if_index)
{
    ...
    if (p) return VNET_API_ERROR_SUBIF_ALREADY_EXISTS;

    fd = open("/dev/netmap", O_RDWR);
    if (fd < 0) return VNET_API_ERROR_SUBIF_ALREADY_EXISTS;
    ...
    if (reg->region_size == 0) {
        reg->mem = mmap(NULL, req->nr_memsize, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    ...
    if (nr_reg == NR_REG_SW) { /* host stack */
        ...
        /* XXX check validity */
        nif->first_tx_ring = nif->last_tx_ring = nif->first_rx_ring = nif->last_rx_ring = nif->req->nr_ringid & NETMAP_RING_MASK;
    }
    else { /* pipes */
    ...
    if (tm->n_vlib_mains > 1) clib_spinlock_init(&nif->lockp);
    ...
    if (sw_if_index) *sw_if_index = nif->sw_if_index;
...
int netmap_delete_if(vlib_main_t *vm, u8 *host_if_name)
{
    ...
    if (tm->n_vlib_mains > 1 && pool_elts(nm->interfaces) == 0) netmap_worker_thread_disable();
...
static clib_error_t *netmap_init(vlib_main_t *vm)
{
    ...
    vec_validate_aligned(nm->rx_buffers, tm->n_vlib_mains - 1, CLIB_CACHE_LINE_BYTES);
