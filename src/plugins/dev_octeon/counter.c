...
vnet_dev_counter_t oct_port_counters[] = {
    ...
    VNET_DEV_CTR_VENDOR(OCT_PORT_CTR_RX_DROP_MCAST, RX, PACKETS, "drop multicast"),
    VNET_DEV_CTR_VENDOR(OCT_PORT_CTR_RX_DROP_BCAST, RX, PACKETS, "drop broadcast"),
    VNET_DEV_CTR_VENDOR(OCT_PORT_CTR_RX_DROP_L3_MCAST, RX, PACKETS, "drop L3 multicast"),
    VNET_DEV_CTR_VENDOR(OCT_PORT_CTR_RX_DROP_L3_BCAST, RX, PACKETS, "drop L3 broadcast"),
...
void oct_port_add_counters(vlib_main_t *vm, vnet_dev_port_t *port)
{
    vnet_dev_port_add_counters(vm, port, oct_port_counters, ARRAY_LEN(oct_port_counters));

    foreach_vnet_dev_port_rx_queue (rxq, port) {
        vnet_dev_rx_queue_add_counters(vm, rxq, oct_rxq_counters, ARRAY_LEN(oct_rxq_counters));
    }

    foreach_vnet_dev_port_tx_queue (txq, port) {
        vnet_dev_tx_queue_add_counters(vm, txq, oct_txq_counters, ARRAY_LEN(oct_txq_counters));
    }
}

vnet_dev_rv_t oct_port_get_stats(vlib_main_t *vm, vnet_dev_port_t *port)
{
    ...
    if ((rrv = roc_nix_stats_get(nix, &stats))) return oct_roc_err(dev, rrv, "roc_nix_stats_get() failed");
    ...
    foreach_vnet_dev_counter (c, port->counter_main) {
        switch (c->user_data) {
            ...
            case OCT_PORT_CTR_RX_PACKETS:
                vnet_dev_counter_value_update(vm, c, stats.rx_ucast + stats.rx_bcast + stats.rx_mcast);
                break;
            case OCT_PORT_CTR_TX_PACKETS:
                vnet_dev_counter_value_update(vm, c, stats.tx_ucast + stats.tx_bcast + stats.tx_mcast);
                break;
...
vnet_dev_rv_t oct_rxq_get_stats(vlib_main_t *vm, vnet_dev_port_t *port, vnet_dev_rx_queue_t *rxq)
{
    ...
    if ((rrv = roc_nix_stats_queue_get(nix, crq->rq.qid, 1, &qstats))) return oct_roc_err(dev, rrv, "roc_nix_stats_queue_get() failed");
...
vnet_dev_rv_t oct_txq_get_stats(vlib_main_t *vm, vnet_dev_port_t *port, vnet_dev_tx_queue_t *txq)
{
    ...
    if ((rrv = roc_nix_stats_queue_get(nix, ctq->sq.qid, 0, &qstats))) return oct_roc_err(dev, rrv, "roc_nix_stats_queue_get() failed");
...
void oct_port_clear_counters(vlib_main_t *vm, vnet_dev_port_t *port)
{
    vnet_dev_t     *dev = port->dev;
    oct_device_t   *cd  = vnet_dev_get_data(dev);
    struct roc_nix *nix = cd->nix;
    ...
    if ((rrv = roc_nix_stats_reset(nix))) oct_roc_err(dev, rrv, "roc_nix_stats_reset() failed");
}

void oct_rxq_clear_counters(vlib_main_t *vm, vnet_dev_rx_queue_t *rxq)
{
    oct_rxq_t      *crq = vnet_dev_get_rx_queue_data(rxq);
    ...
    if ((rrv = roc_nix_stats_queue_reset(nix, crq->rq.qid, 1)))
        oct_roc_err(dev, rrv, "roc_nix_stats_queue_reset() failed for rx queue %u", rxq->queue_id);
}

void oct_txq_clear_counters(vlib_main_t *vm, vnet_dev_tx_queue_t *txq)
{
    oct_txq_t      *ctq = vnet_dev_get_tx_queue_data(txq);
    ...
    if ((rrv = roc_nix_stats_queue_reset(nix, ctq->sq.qid, 0)))
        oct_roc_err(dev, rrv, "roc_nix_stats_queue_reset() failed for tx queue %u", txq->queue_id);
}
