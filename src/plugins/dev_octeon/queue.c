...
vnet_dev_rv_t oct_rx_queue_alloc(vlib_main_t *vm, vnet_dev_rx_queue_t *rxq)
{
...
void oct_rx_queue_free(vlib_main_t *vm, vnet_dev_rx_queue_t *rxq)
{
    vnet_dev_port_t *port = rxq->port;
    vnet_dev_t      *dev  = port->dev;
...
vnet_dev_rv_t oct_tx_queue_alloc(vlib_main_t *vm, vnet_dev_tx_queue_t *txq)
{
    ...
    clib_memset_u64(ctq->ba_buffer, OCT_BATCH_ALLOC_IOVA0_MASK, ROC_CN10K_NPA_BATCH_ALLOC_MAX_PTRS);

    return rv;
}

void oct_tx_queue_free(vlib_main_t *vm, vnet_dev_tx_queue_t *txq)
{
    oct_txq_t       *ctq  = vnet_dev_get_tx_queue_data(txq);
    vnet_dev_port_t *port = txq->port;
    vnet_dev_t      *dev  = port->dev;
...
vnet_dev_rv_t oct_rxq_init(vlib_main_t *vm, vnet_dev_rx_queue_t *rxq)
{
    oct_rxq_t          *crq = vnet_dev_get_rx_queue_data(rxq);
    vnet_dev_t         *dev = rxq->port->dev;
    oct_device_t       *cd  = vnet_dev_get_data(dev);
    vlib_buffer_pool_t *bp  = vlib_get_buffer_pool(vm, vnet_dev_get_rx_queue_buffer_pool_index(rxq));
    ...
    if ((rrv = roc_npa_pool_create(&crq->aura_handle, bp->alloc_size, rxq->size, &aura, &npapool, 0))) {
    ...
    if ((rrv = roc_nix_cq_init(nix, &crq->cq))) {
        oct_rxq_deinit(vm, rxq);
        return oct_roc_err(dev, rrv, "roc_nix_cq_init(qid = %u, nb_desc = %u) failed", crq->cq.nb_desc, crq->cq.nb_desc);
    }

    crq->cq_initialized = 1;
    log_debug(dev, "CQ %u initialised (qmask 0x%x wdata 0x%lx)", crq->cq.qid, crq->cq.qmask, crq->cq.wdata);
    ...
    if ((rrv = roc_nix_rq_init(nix, &crq->rq, 1 /* disable */))) {
        oct_rxq_deinit(vm, rxq);
        return oct_roc_err(dev, rrv, "roc_nix_rq_init(qid = %u) failed", crq->rq.qid);
    }

    crq->rq_initialized         = 1;
    crq->lmt_base_addr          = roc_idev_lmt_base_addr_get();
    crq->aura_batch_free_ioaddr = (roc_npa_aura_handle_to_base(crq->aura_handle) + NPA_LF_AURA_BATCH_FREE0) | (0x7 << 4);
...
void oct_rxq_deinit(vlib_main_t *vm, vnet_dev_rx_queue_t *rxq)
{
    oct_rxq_t  *crq = vnet_dev_get_rx_queue_data(rxq);
    vnet_dev_t *dev = rxq->port->dev;
    int         rrv;
    ...
    if (crq->rq_initialized) {
        ...
        if (rrv) oct_roc_err(dev, rrv, "roc_nix_rq_fini() failed");
        crq->rq_initialized = 0;
    }

    if (crq->cq_initialized) {
        rrv = roc_nix_cq_fini(&crq->cq);
        if (rrv) oct_roc_err(dev, rrv, "roc_nix_cq_fini() failed");
        crq->cq_initialized = 0;
    }

    if (crq->npa_pool_initialized) {
        rrv = roc_npa_pool_destroy(crq->aura_handle);
        if (rrv) oct_roc_err(dev, rrv, "roc_npa_pool_destroy() failed");
        crq->npa_pool_initialized = 0;
    }
}

vnet_dev_rv_t oct_txq_init(vlib_main_t *vm, vnet_dev_tx_queue_t *txq)
{
    oct_txq_t          *ctq     = vnet_dev_get_tx_queue_data(txq);
    ...
    if ((rrv = roc_npa_pool_create(&ctq->aura_handle,
                                   bp->alloc_size,
    ...
    if ((rrv = roc_nix_sq_init(nix, &ctq->sq))) {
        oct_txq_deinit(vm, txq);
        return oct_roc_err(dev, rrv, "roc_nix_sq_init(qid = %u, nb_desc = %u, max_sqe_sz = %u) failed", ctq->sq.nb_desc, ctq->sq.max_sqe_sz);
    }

    ctq->sq_initialized = 1;
    log_debug(dev, "SQ initialised, qid %u, nb_desc %u, max_sqe_sz %u", ctq->sq.qid, ctq->sq.nb_desc, ctq->sq.max_sqe_sz);

    ctq->hdr_off = vm->buffer_main->ext_hdr_size;

    if (ctq->sq.lmt_addr == 0) ctq->sq.lmt_addr = (void *) nix->lmt_base;
...
void oct_txq_deinit(vlib_main_t *vm, vnet_dev_tx_queue_t *txq)
{
    oct_txq_t  *ctq = vnet_dev_get_tx_queue_data(txq);
    vnet_dev_t *dev = txq->port->dev;
    int         rrv;
    ...
    if (ctq->sq_initialized) {
        ...
        if (rrv) oct_roc_err(dev, rrv, "roc_nix_sq_fini() failed");
...
u8 *format_oct_rxq_info(u8 *s, va_list *args)
{
...
u8 *format_oct_txq_info(u8 *s, va_list *args)
{
    vnet_dev_format_args_t *a      = va_arg(*args, vnet_dev_format_args_t *);
    ...
    if (a->debug) {
        s = format(s, "n_enq %u sq_nb_desc %u io_addr %p lmt_addr %p", ctq->n_enq, ctq->sq.nb_desc, ctq->io_addr, ctq->lmt_addr);
        s = format(s,
                   "\n%Uaura: id 0x%x count %u limit %u avail %u",
                   format_white_space,
                   indent,
