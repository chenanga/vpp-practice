...
static_always_inline vlib_buffer_t *oct_seg_to_bp(void *p)
{
    return (vlib_buffer_t *) p - 1;
}

static_always_inline void oct_rx_attach_tail(vlib_main_t *vm, oct_rx_node_ctx_t *ctx, vlib_buffer_t *h, oct_nix_rx_cqe_desc_t *d)
{
    ...
    if (d->sg1.subdc != NIX_SUBDC_SG) goto done;

    segs1 = d->sg1.segs;
    if (segs1 == 0) goto done;
...
static_always_inline u32 oct_rx_batch(vlib_main_t *vm, oct_rx_node_ctx_t *ctx, vnet_dev_rx_queue_t *rxq, u32 n)
{
    ...
    for (; n_left; d += 1, n_left -= 1, ctx->to_next += 1) {
        ...
        b[0]->flow_id                           = d[0].parse.w[3] >> 48;
        ctx->n_segs += 1;
        if (d[0].sg0.segs > 1) oct_rx_attach_tail(vm, ctx, b[0], d + 0);
...
static_always_inline u32 oct_rxq_refill(vlib_main_t *vm, vnet_dev_rx_queue_t *rxq, u16 n_refill)
{
    u32            n_alloc, n_free;
    ...
    const uint64_t addr = roc_npa_aura_handle_to_base(crq->aura_handle) + NPA_LF_AURA_OP_FREE0;

    if (n_refill < 256) return 0;

    n_alloc = vlib_buffer_alloc(vm, buffer_indices, n_refill);
    if (PREDICT_FALSE(n_alloc < n_refill)) goto alloc_fail;
...
#else
static_always_inline void oct_rxq_refill_batch(vlib_main_t                       *vm,
                                               u64                                lmt_id,
                                               u64                                addr,
...
static_always_inline u32 oct_rxq_refill(vlib_main_t *vm, vnet_dev_rx_queue_t *rxq, u16 n_refill)
{
    ...
    if (n_refill < bufs_per_line) return 0;
    ...
    while (n_lines >= batch_max_lines) {
        n_alloc = vlib_buffer_alloc_from_pool(vm, buffer_indices, batch_max_bufs, bpi);
        if (PREDICT_FALSE(n_alloc < batch_max_bufs)) goto alloc_fail;
        oct_rxq_refill_batch(vm, lmt_id, addr, lines, buffer_indices, w0, batch_max_lines);
        n_lines -= batch_max_lines;
        n_enq += batch_max_bufs;
    }

    if (n_lines == 0) return n_enq;
    ...
    alloc_fail:
        if (n_alloc >= bufs_per_line) {
            ...
            if (n_unalloc) vlib_buffer_unalloc_to_pool(vm, buffer_indices + batch_bufs, n_unalloc, bpi);
        }
        else {
            if (n_alloc) vlib_buffer_unalloc_to_pool(vm, buffer_indices, n_alloc, bpi);
            return n_enq;
        }
    }
...
static_always_inline void oct_rx_trace(vlib_main_t *vm, vlib_node_runtime_t *node, oct_rx_node_ctx_t *ctx, oct_nix_rx_cqe_desc_t *d, u32 n_desc)
{
    u32 i = 0;
    if (PREDICT_TRUE(ctx->trace_count == 0)) return;
...
oct_rx_node_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, vnet_dev_port_t *port, vnet_dev_rx_queue_t *rxq, int with_flows)
{
    ...
    if (status.cq_err || status.op_err) return 0;

    head   = status.head;
    n_desc = (status.tail - head) & cq_mask;

    if (n_desc == 0) goto refill;

    vlib_get_new_next_frame(vm, node, ctx->next_index, ctx->to_next, ctx->n_left_to_next);
    ...
    while (1) {
        ...
        if (ctx->n_left_to_next == 0) break;

        status.as_u64 = roc_atomic64_add_sync(crq->cq.wdata, crq->cq.status);
        if (status.cq_err || status.op_err) break;
    ...
    if (ctx->n_traced) vlib_set_trace_count(vm, node, ctx->trace_count - ctx->n_traced);
    ...
    if (PREDICT_TRUE(ctx->next_index == VNET_DEV_ETH_RX_PORT_NEXT_ETH_INPUT)) {
        ...
        if (p.f.errcode == 0 && p.f.errlev == 0) f->flags |= ETH_INPUT_FRAME_F_IP4_CKSUM_OK;
