...
static_always_inline u32 oct_batch_free(vlib_main_t *vm, oct_tx_ctx_t *ctx, vnet_dev_tx_queue_t *txq)
{
    ...
    if ((n = roc_npa_aura_op_available(ah)) >= 32) {
        ...
        vlib_get_buffer_indices_with_offset(vm, (void **) &buffers, bi, n_freed, off);
...
static_always_inline void oct_lmt_copy(void *lmt_addr, u64 io_addr, void *desc, u64 dwords)
{
    u64 lmt_status;
...
static_always_inline u32 oct_batch_free(vlib_main_t *vm, oct_tx_ctx_t *ctx, vnet_dev_tx_queue_t *txq)
{
    ...
    if (num_cl) {
        ...
        for (cl = ctq->ba_buffer + ctq->ba_first_cl; num_cl > 0; num_cl--, cl++) {
            ...
            if ((st.as_u64 = __atomic_load_n(cl->iova, __ATOMIC_RELAXED)) == OCT_BATCH_ALLOC_IOVA0_MASK + ALLOC_CCODE_INVAL) {
            cl_not_ready:
            ...
            if (st.status.count > 8 && __atomic_load_n(cl->iova + 8, __ATOMIC_RELAXED) == OCT_BATCH_ALLOC_IOVA0_MASK) goto cl_not_ready;
            ...
            if (PREDICT_TRUE(st.status.count == 16)) {
                ...
                vlib_get_buffer_indices_with_offset(vm, (void **) cl, bi, 16, off);
                bi += 16;
            }
            else {
                vlib_get_buffer_indices_with_offset(vm, (void **) cl, bi, st.status.count, off);
                bi += st.status.count;
            }
        }

        n_freed = bi - (u32 *) ctq->ba_buffer;
        if (n_freed > 0) vlib_buffer_free_no_next(vm, (u32 *) ctq->ba_buffer, n_freed);

        /* clear status bits in each cacheline */
        n = cl - ctq->ba_buffer;
        for (u32 i = 0; i < n; i++)
            ctq->ba_buffer[i].iova[0] = ctq->ba_buffer[i].iova[8] = OCT_BATCH_ALLOC_IOVA0_MASK;
    ...
    if ((n = roc_npa_aura_op_available(ah)) >= 32) {
        ...
        oct_npa_batch_alloc_compare_t cmp = {.compare_s = {.aura = roc_npa_aura_handle_to_aura(ah), .stype = ALLOC_STYPE_STF, .count = n}};

        addr = roc_npa_aura_handle_to_base(ah) + NPA_LF_AURA_BATCH_ALLOC;
        res  = roc_atomic64_casl(cmp.as_u64, (uint64_t) ctq->ba_buffer, (i64 *) addr);
...
oct_tx_enq1(vlib_main_t *vm, oct_tx_ctx_t *ctx, vlib_buffer_t *b, lmt_line_t *line, u32 flags, int simple, int trace, u32 *n, u8 *dpl)
{
    u8            n_dwords  = 2;
    u32           total_len = 0;
    oct_tx_desc_t d         = {
...
static_always_inline u32 oct_tx_enq16(vlib_main_t *vm, oct_tx_ctx_t *ctx, vnet_dev_tx_queue_t *txq, vlib_buffer_t **b, u32 n_pkts, int trace)
{
    ...
    u32         n_left, n = 0;
    const u32   not_simple_flags = VLIB_BUFFER_NEXT_PRESENT | VNET_BUFFER_F_OFFLOAD;
    ...
    if (PREDICT_FALSE(!n_lines)) return n_pkts;
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE)) {
        for (n_left = clib_min(n_pkts, txq->size - n_enq), n = 0; n_left >= 16; n_left -= 16, b += 16)
            n += oct_tx_enq16(vm, &ctx, txq, b, 16, /* trace */ 1);

        if (n_left) n += oct_tx_enq16(vm, &ctx, txq, b, n_left, /* trace */ 1);
    }
    else {
        for (n_left = clib_min(n_pkts, txq->size - n_enq), n = 0; n_left >= 16; n_left -= 16, b += 16)
            n += oct_tx_enq16(vm, &ctx, txq, b, 16, /* trace */ 0);

        if (n_left) n += oct_tx_enq16(vm, &ctx, txq, b, n_left, /* trace */ 0);
    }
    ...
    if (n < n_pkts) {
        ...
        vlib_error_count(vm, node->node_index, OCT_TX_NODE_CTR_NO_FREE_SLOTS, n_free);
        n_pkts -= n_free;
    }

    if (ctx.n_drop) vlib_error_count(vm, node->node_index, OCT_TX_NODE_CTR_CHAIN_TOO_LONG, ctx.n_drop);

    if (PREDICT_FALSE(ctx.n_exd_mtu)) vlib_error_count(vm, node->node_index, OCT_TX_NODE_CTR_MTU_EXCEEDED, ctx.n_exd_mtu);

    if (ctx.batch_alloc_not_ready) vlib_error_count(vm, node_index, OCT_TX_NODE_CTR_AURA_BATCH_ALLOC_NOT_READY, ctx.batch_alloc_not_ready);

    if (ctx.batch_alloc_issue_fail) vlib_error_count(vm, node_index, OCT_TX_NODE_CTR_AURA_BATCH_ALLOC_ISSUE_FAIL, ctx.batch_alloc_issue_fail);
