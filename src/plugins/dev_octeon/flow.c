...
#define FLOW_IS_IPV4_CLASS(f)                                                                                                     \
    ((f->type == VNET_FLOW_TYPE_IP4) || (f->type == VNET_FLOW_TYPE_IP4_N_TUPLE) || (f->type == VNET_FLOW_TYPE_IP4_N_TUPLE_TAGGED) \
     || (f->type == VNET_FLOW_TYPE_IP4_VXLAN) || (f->type == VNET_FLOW_TYPE_IP4_GTPC) || (f->type == VNET_FLOW_TYPE_IP4_GTPU)     \
...
#define FLOW_IS_IPV6_CLASS(f)                                                                                                     \
    ((f->type == VNET_FLOW_TYPE_IP6) || (f->type == VNET_FLOW_TYPE_IP6_N_TUPLE) || (f->type == VNET_FLOW_TYPE_IP6_N_TUPLE_TAGGED) \
...
#define FLOW_IS_L4_TYPE(f)                                                                                                                \
    ((f->type == VNET_FLOW_TYPE_IP4_N_TUPLE) || (f->type == VNET_FLOW_TYPE_IP6_N_TUPLE) || (f->type == VNET_FLOW_TYPE_IP4_N_TUPLE_TAGGED) \
     || (f->type == VNET_FLOW_TYPE_IP6_N_TUPLE_TAGGED))

#define FLOW_IS_L4_TUNNEL_TYPE(f)                                                                                           \
    ((f->type == VNET_FLOW_TYPE_IP4_VXLAN) || (f->type == VNET_FLOW_TYPE_IP6_VXLAN) || (f->type == VNET_FLOW_TYPE_IP4_GTPC) \
     || (f->type == VNET_FLOW_TYPE_IP4_GTPU))

#define FLOW_IS_GENERIC_TYPE(f) (f->type == VNET_FLOW_TYPE_GENERIC)

#define OCT_FLOW_UNSUPPORTED_ACTIONS(f) ((f->actions == VNET_FLOW_ACTION_BUFFER_ADVANCE) || (f->actions == VNET_FLOW_ACTION_REDIRECT_TO_NODE))
...
static void oct_flow_convert_rss_types(u64 *key, u64 rss_types)
{
#define _(a, b, c) \
    if (rss_types & (1UL << a)) *key |= b;
...
vnet_dev_rv_t oct_flow_validate_params(vlib_main_t *vm, vnet_dev_port_t *port, vnet_dev_port_cfg_type_t type, u32 flow_index, uword *priv_data)
{
    vnet_flow_t *flow = vnet_get_flow(flow_index);
    u32          last_queue;
    u32          qid;

    if (type == VNET_DEV_PORT_CFG_GET_RX_FLOW_COUNTER || type == VNET_DEV_PORT_CFG_RESET_RX_FLOW_COUNTER) {
    ...
    if (flow->actions & VNET_FLOW_ACTION_REDIRECT_TO_QUEUE) {
        ...
        if (qid > port->intf.num_rx_queues - 1 || qid < 0) {
            log_err(port->dev, "Given Q(%d) is invalid, supported range is %d-%d", qid, 0, port->intf.num_rx_queues - 1);
            return VNET_DEV_ERR_NOT_SUPPORTED;
        }
    }
    ...
    if (flow->actions & VNET_FLOW_ACTION_RSS) {
        ...
        if (last_queue > port->intf.num_rx_queues - 1) {
            log_err(port->dev,
                    ...
                    flow->queue_index,
                    flow->queue_index + flow->queue_num,
                    0,
...
static vnet_dev_rv_t oct_flow_rule_create(vnet_dev_port_t          *port,
                                          struct roc_npc_action    *actions,
                                          struct roc_npc_item_info *item_info,
                                          vnet_flow_t              *flow,
                                          uword                    *private_data)
{
    ...
    for (int i = 0; item_info[i].type != ROC_NPC_ITEM_TYPE_END; i++) {
        log_debug(port->dev,
                  "Flow[%d] Item[%d] type %d spec 0x%U mask 0x%U",
                  flow->index,
                  i,
                  item_info[i].type,
                  format_hex_bytes,
    ...
    if (rv) {
        log_err(port->dev, "roc_npc_flow_create failed with '%s' error", roc_error_msg_get(rv));
        return VNET_DEV_ERR_NOT_SUPPORTED;
    }
...
static int oct_parse_l2(oct_flow_parse_state *pst)
{
    struct roc_npc_flow_item_eth *eth_spec     = (struct roc_npc_flow_item_eth *) &pst->oct_drv.spec[pst->oct_drv.off];
    struct roc_npc_flow_item_eth *eth_mask     = (struct roc_npc_flow_item_eth *) &pst->oct_drv.mask[pst->oct_drv.off];
    ethernet_header_t            *eth_hdr_mask = (ethernet_header_t *) &pst->generic.mask[pst->generic.off];
    ethernet_header_t            *eth_hdr      = (ethernet_header_t *) &pst->generic.spec[pst->generic.off];
    u16                           tpid, etype;
    ...
    struct roc_npc_flow_item_vlan *vlan_spec = (struct roc_npc_flow_item_vlan *) &pst->oct_drv.spec[pst->oct_drv.off];
    struct roc_npc_flow_item_vlan *vlan_mask = (struct roc_npc_flow_item_vlan *) &pst->oct_drv.mask[pst->oct_drv.off];
    ethernet_vlan_header_t        *vlan_hdr, *vlan_hdr_mask;
    u8                             vlan_cnt = 0;

    while (tpid == ETHERNET_TYPE_DOT1AD || tpid == ETHERNET_TYPE_VLAN) {
        if (pst->generic.off >= pst->generic.len) break;

        vlan_hdr      = (ethernet_vlan_header_t *) &pst->generic.spec[pst->generic.off];
        vlan_hdr_mask = (ethernet_vlan_header_t *) &pst->generic.mask[pst->generic.off];
        tpid = etype = clib_net_to_host_u16(vlan_hdr->type);
        clib_memcpy(&vlan_spec[vlan_cnt], vlan_hdr, sizeof(ethernet_vlan_header_t));
        clib_memcpy(&vlan_mask[vlan_cnt], vlan_hdr_mask, sizeof(ethernet_vlan_header_t));
    ...
    /* Inner most vlan tag */
    if (vlan_cnt) vlan_spec[vlan_cnt - 1].has_more_vlan = 0;

    pst->nxt_proto = etype;
    return 0;
}

static int oct_parse_l3(oct_flow_parse_state *pst)
{
    if (pst->generic.off >= pst->generic.len || pst->nxt_proto == 0) return 0;
    ...
    if (pst->nxt_proto == ETHERNET_TYPE_MPLS) {
        ...
        ip4_header_t *ip4_spec      = (ip4_header_t *) &pst->generic.spec[pst->generic.off];
        ip4_header_t *ip4_mask      = (ip4_header_t *) &pst->generic.mask[pst->generic.off];
        ...
        struct roc_npc_flow_item_ipv6 *ip6_spec     = (struct roc_npc_flow_item_ipv6 *) &pst->oct_drv.spec[pst->oct_drv.off];
        struct roc_npc_flow_item_ipv6 *ip6_mask     = (struct roc_npc_flow_item_ipv6 *) &pst->oct_drv.mask[pst->oct_drv.off];
        ip6_header_t                  *ip6_hdr_mask = (ip6_header_t *) &pst->generic.mask[pst->generic.off];
        ip6_header_t                  *ip6_hdr      = (ip6_header_t *) &pst->generic.spec[pst->generic.off];
        ...
        while (nxt_hdr == IP_PROTOCOL_IP6_HOP_BY_HOP_OPTIONS || nxt_hdr == IP_PROTOCOL_IP6_DESTINATION_OPTIONS || nxt_hdr == IP_PROTOCOL_IPV6_ROUTE) {
            if (pst->generic.off >= pst->generic.len) return 0;

            ip6_ext_header_t *ip6_ext_spec = (ip6_ext_header_t *) &pst->generic.spec[pst->generic.off];
            ip6_ext_header_t *ip6_ext_mask = (ip6_ext_header_t *) &pst->generic.mask[pst->generic.off];
        ...
        if (nxt_hdr == IP_PROTOCOL_IPV6_FRAGMENTATION) {
            ip6_frag_hdr_t *ip6_ext_frag_spec = (ip6_frag_hdr_t *) &pst->generic.spec[pst->generic.off];
            ip6_frag_hdr_t *ip6_ext_frag_mask = (ip6_frag_hdr_t *) &pst->generic.mask[pst->generic.off];
...
static int oct_parse_l4(oct_flow_parse_state *pst)
{
    if (pst->generic.off >= pst->generic.len || pst->nxt_proto == 0) return 0;
...
static int oct_parse_tunnel(oct_flow_parse_state *pst)
{
    if (pst->generic.off >= pst->generic.len) return 0;
    ...
    if (pst->items[pst->layer - 1].type == ROC_NPC_ITEM_TYPE_GRE) {
        ...
        if (dport == GTPU_PORT) {
            gtpu_header_t *gtpu_spec    = (gtpu_header_t *) &pst->generic.spec[pst->generic.off];
            gtpu_header_t *gtpu_mask    = (gtpu_header_t *) &pst->generic.mask[pst->generic.off];
            ...
            vxlan_header_t *vxlan_spec  = (vxlan_header_t *) &pst->generic.spec[pst->generic.off];
            vxlan_header_t *vxlan_mask  = (vxlan_header_t *) &pst->generic.spec[pst->generic.off];
    ...
    /* No supported Tunnel detected. */
        ...
        log_err(pst->port->dev, "Partially parsed till offset %u, not able to parse further", pst->generic.off);
        return 0;
    }
parse_l2:
    if (oct_parse_l2(pst)) return -1;
parse_l3:
    if (oct_parse_l3(pst)) return -1;

    return oct_parse_l4(pst);
}

static vnet_dev_rv_t oct_flow_generic_pattern_parse(oct_flow_parse_state *pst)
{
    if (oct_parse_l2(pst)) goto err;

    if (oct_parse_l3(pst)) goto err;

    if (oct_parse_l4(pst)) goto err;

    if (oct_parse_tunnel(pst)) goto err;
...
static vnet_dev_rv_t oct_flow_add(vlib_main_t *vm, vnet_dev_port_t *port, vnet_flow_t *flow, uword *private_data)
{
    ...
    if (FLOW_IS_GENERIC_TYPE(flow)) {
        ...
        unformat_init_string(&input, (const char *) flow->generic.pattern.spec, strlen((const char *) flow->generic.pattern.spec));
        unformat_user(&input, unformat_hex_string, &flow_spec);
        unformat_free(&input);

        unformat_init_string(&input, (const char *) flow->generic.pattern.mask, strlen((const char *) flow->generic.pattern.mask));
        ...
        oct_flow_parse_state pst = {
            ...
            .generic   = {.spec = flow_spec, .mask = flow_mask, .len = vec_len(flow_spec)},
        };
    ...
    switch (proto) {
        ...
        default:
            log_err(port->dev, "Unsupported IP protocol '%U'", format_ip_protocol, proto);
            return VNET_DEV_ERR_NOT_SUPPORTED;
    }
...
parse_flow_actions:
    ...
    if (flow->actions & VNET_FLOW_ACTION_MARK) {
        if (flow->mark_flow_id == 0 || flow->mark_flow_id > (NPC_FLOW_FLAG_VAL - 2)) {
    ...
    if (queues) clib_mem_free(queues);
...
static vnet_dev_rv_t oct_flow_del(vlib_main_t *vm, vnet_dev_port_t *port, vnet_flow_t *flow, uword *private_data)
{
    oct_port_t          *oct_port = vnet_dev_get_port_data(port);
    struct roc_npc      *npc      = &oct_port->npc;
    struct roc_npc_flow *npc_flow;
    ...
    if (rv) {
        log_err(port->dev, "roc_npc_flow_destroy failed with '%s' error", roc_error_msg_get(rv));
        return VNET_DEV_ERR_NOT_SUPPORTED;
    }
...
vnet_dev_rv_t oct_flow_query(vlib_main_t *vm, vnet_dev_port_t *port, u32 flow_index, uword private_data, u64 *hits)
{
    oct_port_t          *oct_port = vnet_dev_get_port_data(port);
    struct roc_npc      *npc      = &oct_port->npc;
    struct roc_npc_flow *npc_flow;
    ...
    if (npc_flow->ctr_id == NPC_COUNTER_NONE) {
        log_err(port->dev, "Counters are not available for given flow id (%u)", flow_index);
        return VNET_DEV_ERR_NOT_SUPPORTED;
    }

    rv = roc_npc_mcam_read_counter(npc, npc_flow->ctr_id, hits);
    if (rv != 0) {
        log_err(port->dev, "Error reading flow counter for given flow id (%u)", flow_index);
...
vnet_dev_rv_t oct_flow_ops_fn(vlib_main_t *vm, vnet_dev_port_t *port, vnet_dev_port_cfg_type_t type, u32 flow_index, uword *priv_data)
{
    vnet_flow_t *flow = vnet_get_flow(flow_index);

    if (type == VNET_DEV_PORT_CFG_ADD_RX_FLOW) return oct_flow_add(vm, port, flow, priv_data);

    if (type == VNET_DEV_PORT_CFG_DEL_RX_FLOW) return oct_flow_del(vm, port, flow, priv_data);
