...
#define _(id, device_type, desc)                                                        \
    {                                                                                   \
        .device_id = (id), .type = OCT_DEVICE_TYPE_##device_type, .description = (desc) \
    }
...
static u8 *oct_probe(vlib_main_t *vm, vnet_dev_bus_index_t bus_index, void *dev_info)
{
...
vnet_dev_rv_t cnx_return_roc_err(vnet_dev_t *dev, int rrv, char *fmt, ...)
{
    va_list va;
    va_start(va, fmt);
...
static vnet_dev_rv_t oct_alloc(vlib_main_t *vm, vnet_dev_t *dev)
{
    oct_device_t *cd = vnet_dev_get_data(dev);
    cd->nix          = clib_mem_alloc_aligned(sizeof(struct roc_nix), CLIB_CACHE_LINE_BYTES);
    return VNET_DEV_OK;
}

static vnet_dev_rv_t oct_init_nix(vlib_main_t *vm, vnet_dev_t *dev)
{
    oct_device_t *cd = vnet_dev_get_data(dev);
    u8            mac_addr[6];
    int           rrv;
    ...
    *cd->nix = (struct roc_nix){
    ...
    if ((rrv = roc_nix_dev_init(cd->nix))) return cnx_return_roc_err(dev, rrv, "roc_nix_dev_init");

    if ((rrv = roc_nix_npc_mac_addr_get(cd->nix, mac_addr))) return cnx_return_roc_err(dev, rrv, "roc_nix_npc_mac_addr_get");
...
static vnet_dev_rv_t oct_init_cpt(vlib_main_t *vm, vnet_dev_t *dev)
{
    oct_device_t  *cd = vnet_dev_get_data(dev);
    int            rrv;
    struct roc_cpt cpt = {
    ...
    if ((rrv = roc_cpt_dev_init(&cpt))) return cnx_return_roc_err(dev, rrv, "roc_cpt_dev_init");
    return VNET_DEV_OK;
}

static vnet_dev_rv_t oct_init(vlib_main_t *vm, vnet_dev_t *dev)
{
    oct_device_t         *cd = vnet_dev_get_data(dev);
    ...
    if (pci_hdr.vendor_id != 0x177d) return VNET_DEV_ERR_UNSUPPORTED_DEVICE;

    FOREACH_ARRAY_ELT (dt, oct_dev_types) {
        if (dt->device_id == pci_hdr.device_id) cd->type = dt->type;
    }

    if (cd->type == OCT_DEVICE_TYPE_UNKNOWN) return rv;
    ...
    strncpy((char *) cd->plt_pci_dev.name, dev->device_id, sizeof(cd->plt_pci_dev.name) - 1);
...
static void oct_deinit(vlib_main_t *vm, vnet_dev_t *dev)
{
    oct_device_t *cd = vnet_dev_get_data(dev);

    if (cd->nix_initialized) roc_nix_dev_fini(cd->nix);
}

static void oct_free(vlib_main_t *vm, vnet_dev_t *dev)
{
    oct_device_t *cd = vnet_dev_get_data(dev);
...
static clib_error_t *oct_plugin_init(vlib_main_t *vm)
{
    int                         rv;
    extern oct_plt_init_param_t oct_plt_init_param;

    rv = oct_plt_init(&oct_plt_init_param);
    if (rv) return clib_error_return(0, "oct_plt_init failed");

    rv = roc_model_init(&oct_model);
    if (rv) return clib_error_return(0, "roc_model_init failed");
    return 0;
}
