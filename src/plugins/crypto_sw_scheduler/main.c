...
int crypto_sw_scheduler_set_worker_crypto(u32 worker_idx, u8 enabled)
{
    ...
    if (enabled || count > 1) {
        cm->per_thread_data[vlib_get_worker_thread_index(worker_idx)].self_crypto_enabled = enabled;
    }
...
static int crypto_sw_scheduler_frame_enqueue(vlib_main_t *vm, vnet_crypto_async_frame_t *frame, u8 is_enc)
{
    crypto_sw_scheduler_main_t            *cm  = &crypto_sw_scheduler_main;
    crypto_sw_scheduler_per_thread_data_t *ptd = vec_elt_at_index(cm->per_thread_data, vm->thread_index);
    crypto_sw_scheduler_queue_t           *current_queue =
        is_enc ? &ptd->queue[CRYPTO_SW_SCHED_QUEUE_TYPE_ENCRYPT] : &ptd->queue[CRYPTO_SW_SCHED_QUEUE_TYPE_DECRYPT];
...
static int crypto_sw_scheduler_frame_enqueue_decrypt(vlib_main_t *vm, vnet_crypto_async_frame_t *frame)
{
    return crypto_sw_scheduler_frame_enqueue(vm, frame, 0);
}
static int crypto_sw_scheduler_frame_enqueue_encrypt(vlib_main_t *vm, vnet_crypto_async_frame_t *frame)
{
                                                     ...
                                                     u32                                    len)
{
    ...
    if (len) {
        ...
        if (vlib_buffer_space_left_at_end(vm, b) >= len) ch->len += len;
    }
                                                           ...
                                                           u8                                     tag_len)
{
    ...
    if (fe->flags & VNET_CRYPTO_OP_FLAG_CHAINED_BUFFERS) {
        ...
        cryptodev_sw_scheduler_sgl(vm, ptd, b, op, fe->crypto_start_offset, fe->crypto_total_length);
    }
    else {
                                                                  ...
                                                                  u8                                     is_enc)
{
    ...
    if (fe->flags & VNET_CRYPTO_OP_FLAG_CHAINED_BUFFERS) {
        ...
        cryptodev_sw_scheduler_sgl(vm, ptd, b, crypto_op, fe->crypto_start_offset, fe->crypto_total_length);
        cryptodev_sw_scheduler_sgl(vm, ptd, b, integ_op, fe->integ_start_offset, fe->crypto_total_length + fe->integ_length_adj);
    }
    else {
...
static_always_inline void crypto_sw_scheduler_process_aead(vlib_main_t                           *vm,
                                                           crypto_sw_scheduler_per_thread_data_t *ptd,
                                                           vnet_crypto_async_frame_t             *f,
                                                           u32                                    aead_op,
                                                           u32                                    aad_len,
                                                           u32                                    digest_len)
{
    ...
    while (n_elts--) {
        if (n_elts > 1) clib_prefetch_load(fe + 1);

        crypto_sw_scheduler_convert_aead(vm, ptd, fe, fe - f->elts, bi[0], aead_op, aad_len, digest_len);
        bi++;
        fe++;
    }
...
static_always_inline void crypto_sw_scheduler_process_link(vlib_main_t                           *vm,
                                                           ...
                                                           u8                                     is_enc)
{
    ...
    while (n_elts--) {
        ...
        crypto_sw_scheduler_convert_link_crypto(vm, ptd, cm->keys + fe->key_index, fe, fe - f->elts, bi[0], crypto_op, auth_op, digest_len, is_enc);
        bi++;
        fe++;
    }
...
static_always_inline int convert_async_crypto_id(vnet_crypto_async_op_id_t async_op_id,
                                                 u32                      *crypto_op,
                                                 u32                      *auth_op_or_aad_len,
                                                 u16                      *digest_len,
                                                 u8                       *is_enc)
{
    switch (async_op_id) {
...
static_always_inline vnet_crypto_async_frame_t *crypto_sw_scheduler_dequeue(vlib_main_t *vm, u32 *nb_elts_processed, u32 *enqueue_thread_idx)
{
    crypto_sw_scheduler_main_t            *cm            = &crypto_sw_scheduler_main;
    crypto_sw_scheduler_per_thread_data_t *ptd           = cm->per_thread_data + vm->thread_index;
...
run_next_queues:
    ...
    if (ptd->self_crypto_enabled) {
        ...
        while (1) {
            ...
            if (i >= vec_len(cm->per_thread_data)) i = 0;
             ...
             * (queue size - 2).
             */
            if ((tail > head) && (head >= CRYPTO_SW_SCHEDULER_QUEUE_MASK)) goto skip_queue;
            ...
            for (j = tail; j != head; j++) {
                ...
                if (clib_atomic_bool_cmp_and_swap(&f->state, VNET_CRYPTO_FRAME_STATE_PENDING, VNET_CRYPTO_FRAME_STATE_WORK_IN_PROGRESS)) {
                    found = 1;
                    break;
                }
            }
    ...
    if (found) {
        ...
        ret = convert_async_crypto_id(f->op, &crypto_op, &auth_op_or_aad_len, &digest_len, &is_enc);

        if (ret == 1)
            crypto_sw_scheduler_process_aead(vm, ptd, f, crypto_op, auth_op_or_aad_len, digest_len);
        else if (ret == 0)
            crypto_sw_scheduler_process_link(vm, cm, ptd, f, crypto_op, auth_op_or_aad_len, digest_len, is_enc);
    ...
    if (current_queue->jobs[tail] && current_queue->jobs[tail]->state >= VNET_CRYPTO_FRAME_STATE_SUCCESS) {
        CLIB_MEMORY_STORE_BARRIER();
        current_queue->tail++;
...
static clib_error_t *sw_scheduler_show_workers(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (i = 1; i < vlib_thread_main.n_vlib_mains; i++) {
        vlib_cli_output(vm,
                        ...
                        cm->per_thread_data[i].self_crypto_enabled ? "on" : "off");
    }
...
clib_error_t              *crypto_sw_scheduler_init(vlib_main_t *vm)
{
    ...
    vec_validate_aligned(cm->per_thread_data, tm->n_vlib_mains - 1, CLIB_CACHE_LINE_BYTES);
    ...
    for (i = 0; i < tm->n_vlib_mains; i++) {
        ...
        vec_validate_aligned(ptd->queue[CRYPTO_SW_SCHED_QUEUE_TYPE_DECRYPT].jobs, CRYPTO_SW_SCHEDULER_QUEUE_SIZE - 1, CLIB_CACHE_LINE_BYTES);
        ...
        vec_validate_aligned(ptd->queue[CRYPTO_SW_SCHED_QUEUE_TYPE_ENCRYPT].jobs, CRYPTO_SW_SCHEDULER_QUEUE_SIZE - 1, CLIB_CACHE_LINE_BYTES);
    }

    cm->crypto_engine_index = vnet_crypto_register_engine(vm, "sw_scheduler", 100, "SW Scheduler Async Engine");

    vnet_crypto_register_key_handler(vm, cm->crypto_engine_index, crypto_sw_scheduler_key_handler);

    crypto_sw_scheduler_api_init(vm);

#define _(n, s, k, t, a)                                                             \
    vnet_crypto_register_enqueue_handler(vm,                                         \
