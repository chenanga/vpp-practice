...
typedef enum { SNORT_INPUT = 1, SNORT_OUTPUT = 2, SNORT_INOUT = 3 } snort_attach_dir_t;
...
clib_error_t *snort_interface_enable_disable(vlib_main_t *vm, char *instance_name, u32 sw_if_index, int is_enable, snort_attach_dir_t dir);
clib_error_t *snort_set_node_mode(vlib_main_t *vm, u32 mode);

always_inline void snort_freelist_init(u32 *fl)
{
