...
static u8 *format_snort_instance(u8 *s, va_list *args)
{
    ...
    s                   = format(s, "%s [idx:%d sz:%d fd:%d]", i->name, i->index, i->shm_size, i->shm_fd);
...
static clib_error_t *snort_create_instance_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    err = snort_instance_create(vm, (char *) name, min_log2(queue_size), drop_on_diconnect);
...
static clib_error_t *snort_attach_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    err = snort_interface_enable_disable(vm, (char *) name, sw_if_index, 1, dir);
...
static clib_error_t *snort_show_interfaces_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vec_foreach (index, sm->instance_by_sw_if_index) {
        if (index[0] != ~0) {
            ...
            vlib_cli_output(vm, "%U:\t%s", format_vnet_sw_if_index_name, vnm, index - sm->instance_by_sw_if_index, si->name);
        }
    }
    return 0;
}
...
{
    snort_main_t *sm   = &snort_main;
    char         *mode = sm->input_mode == VLIB_NODE_STATE_POLLING ? "polling" : "interrupt";
