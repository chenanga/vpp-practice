...
static void snort_client_disconnect(clib_file_t *uf)
{
    ...
    if (c->instance_index != ~0) {
        snort_per_thread_data_t *ptd = vec_elt_at_index(sm->per_thread_data, vm->thread_index);
        snort_instance_t        *si  = pool_elt_at_index(sm->instances, c->instance_index);
...
static snort_instance_t *snort_get_instance_by_name(char *name)
{
    snort_main_t *sm = &snort_main;
    uword        *p;
    if ((p = hash_get_mem(sm->instance_by_name, name)) == 0) return 0;
...
static clib_error_t *snort_conn_fd_write_ready(clib_file_t *uf)
{
    ...
    if (clib_fifo_elts(c->msg_queue) == 0) clib_file_set_data_available_to_write(&file_main, c->file_index, 0);

    return clib_socket_sendmsg(&c->socket, &e->msg, sizeof(*e), e->fds, e->n_fds);
}
...
clib_error_t *snort_instance_create(vlib_main_t *vm, char *name, u8 log2_queue_sz, u8 drop_on_disconnect)
{
    ...
    if (snort_get_instance_by_name(name)) return clib_error_return(0, "instance already exists");
    ...
    size = round_pow2((uword) tm->n_vlib_mains * qpair_mem_sz, clib_mem_get_page_size());
    fd   = clib_mem_vm_create_fd(CLIB_MEM_PAGE_SZ_DEFAULT, "snort instance %s", name);

    if (fd == -1) {
        err = clib_error_return(0, "memory fd failure: %U", format_clib_error, clib_mem_get_last_error());
        goto done;
    }
    ...
    log_debug("instnce '%s' createed with fd %d at %p, len %u", name, fd, base, size);

    vec_validate_aligned(sm->per_thread_data, tm->n_vlib_mains - 1, CLIB_CACHE_LINE_BYTES);
    vec_validate_aligned(si->qpairs, tm->n_vlib_mains - 1, CLIB_CACHE_LINE_BYTES);
    ...
    for (int i = 0; i < tm->n_vlib_mains; i++) {
        ...
        vec_validate_aligned(qp->buffer_indices, qsz - 1, CLIB_CACHE_LINE_BYTES);
        ...
        t.description         = format(0, "snort dequeue for instance '%s' qpair %u", si->name, i);
    ...
    for (i = 0; i < vlib_get_n_threads(); i++)
        vlib_node_set_state(vlib_get_main_by_index(i), snort_deq_node.index, sm->input_mode);

done:
    if (err) {
        if (base != CLIB_MEM_VM_MAP_FAILED) clib_mem_vm_unmap(base);
        if (fd != -1) close(fd);
    }
    return err;
}

clib_error_t *snort_interface_enable_disable(vlib_main_t *vm, char *instance_name, u32 sw_if_index, int is_enable, snort_attach_dir_t snort_dir)
{
    snort_main_t     *sm  = &snort_main;
    ...
    if (is_enable) {
        ...
        if (snort_dir & SNORT_INPUT) {
            fa_data = (u64) index;
            vnet_feature_enable_disable("ip4-unicast", "snort-enq", sw_if_index, 1, &fa_data, sizeof(fa_data));
        }
        if (snort_dir & SNORT_OUTPUT) {
            fa_data = (1LL << 32 | index);
            vnet_feature_enable_disable("ip4-output", "snort-enq", sw_if_index, 1, &fa_data, sizeof(fa_data));
        }
    }
    else {
        if (sw_if_index >= vec_len(sm->instance_by_sw_if_index) || sm->instance_by_sw_if_index[sw_if_index] == ~0) {
            err = clib_error_return(0,
        ...
        if (snort_dir & SNORT_INPUT) {
            fa_data = (u64) index;
            vnet_feature_enable_disable("ip4-unicast", "snort-enq", sw_if_index, 0, &fa_data, sizeof(fa_data));
        }
        if (snort_dir & SNORT_OUTPUT) {
            fa_data = (1LL << 32 | index);
            vnet_feature_enable_disable("ip4-output", "snort-enq", sw_if_index, 0, &fa_data, sizeof(fa_data));
        }
    }

done:
    if (err) log_err("%U", format_clib_error, err);
    return 0;
}
...
clib_error_t *snort_set_node_mode(vlib_main_t *vm, u32 mode)
{
    ...
    for (i = 0; i < vlib_get_n_threads(); i++)
        vlib_node_set_state(vlib_get_main_by_index(i), snort_deq_node.index, mode);
    return 0;
}

static void snort_set_default_socket(snort_main_t *sm, u8 *socket_name)
{
    if (sm->socket_name) return;

    if (!socket_name) socket_name = (u8 *) DAQ_VPP_DEFAULT_SOCKET_FILE;

    sm->socket_name = format(0, "%s/%s", vlib_unix_get_runtime_dir(), socket_name);
...
static clib_error_t *snort_init(vlib_main_t *vm)
{
    ...
    vec_foreach (bp, vm->buffer_main->buffer_pools) {
        vlib_physmem_map_t *pm = vlib_physmem_get_map(vm, bp->physmem_map_index);
        vec_add1(sm->buffer_pool_base_addrs, pm->base);
    }

    if (!sm->socket_name) snort_set_default_socket(sm, 0);
