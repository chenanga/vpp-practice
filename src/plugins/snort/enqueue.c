...
static_always_inline uword snort_enq_node_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int with_trace)
{
    ...
    while (n_left) {
        ...
        fa_data = *(u64 *) vnet_feature_next_with_data(&next_index, b[0], sizeof(u64));

        instance_index = (u32) (fa_data & 0xffffffff);
        l3_offset      = (fa_data >> 32) ? vnet_buffer(b[0])->ip.save_rewrite_length : 0;
        ...
        if (PREDICT_FALSE(si->client_index == ~0)) {
            ...
            if (with_trace && b[0]->flags & VLIB_BUFFER_IS_TRACED) n_trace++;

            /* fill descriptor */
            d->buffer_pool      = b[0]->buffer_pool_index;
            d->length           = b[0]->current_length;
            d->offset           = (u8 *) b[0]->data + b[0]->current_data + l3_offset - sm->buffer_pool_base_addrs[d->buffer_pool];
    ...
    if (n_processed) {
        vlib_node_increment_counter(vm, snort_enq_node.index, SNORT_ENQ_ERROR_NO_INSTANCE, n_processed);
        vlib_buffer_enqueue_to_next(vm, node, vlib_frame_vector_args(frame), nexts, n_processed);
    }
    ...
    vec_foreach (si, sm->instances) {
        ...
        if (freelist_len < n_pending) {
            n_enq = freelist_len;
            vlib_buffer_free(vm, qp->pending_buffers + n_enq, n_pending - n_enq);
            vlib_node_increment_counter(vm, snort_enq_node.index, SNORT_ENQ_ERROR_NO_ENQ_SLOTS, n_pending - n_enq);
        }
        else
            n_enq = n_pending;
        ...
        for (u32 i = 0; i < n_enq; i++) {
            ...
            clib_memcpy_fast(qp->descriptors + desc_index, qp->pending_descs + i, sizeof(daq_vpp_desc_t));
            ...
            if (with_trace && n_trace) {
                ...
                if (tb->flags & VLIB_BUFFER_IS_TRACED) {
                    snort_enq_trace_t *t = vlib_add_trace(vm, node, tb, sizeof(*t));
                    ...
                    clib_memcpy_fast(&t->desc, qp->pending_descs + i, sizeof(daq_vpp_desc_t));
                }
            }
            head = head + 1;
        }
        ...
        if (sm->input_mode == VLIB_NODE_STATE_INTERRUPT) {
            if (write(qp->enq_fd, &ctr, sizeof(ctr)) < 0) vlib_node_increment_counter(vm, snort_enq_node.index, SNORT_ENQ_ERROR_SOCKET_ERROR, 1);
        }
    }
