...
static_always_inline uword snort_deq_instance(vlib_main_t *vm, u32 instance_index, snort_qpair_t *qp, u32 *buffer_indices, u16 *nexts, u32 max_recv)
{
    ...
    while (n_left) {
        ...
        if ((desc_index = qp->deq_ring[next & mask]) & ~mask) {
            vlib_node_increment_counter(vm, snort_deq_node.index, SNORT_DEQ_ERROR_BAD_DESC_INDEX, 1);
            goto next;
        }
        ...
        if ((bi = qp->buffer_indices[desc_index]) == ~0) {
            vlib_node_increment_counter(vm, snort_deq_node.index, SNORT_DEQ_ERROR_BAD_DESC, 1);
            goto next;
        }
                                                            ...
                                                            u8             drop_on_disconnect)
{
    ...
    n_processed = snort_process_all_buffer_indices(qp, buffer_indices, nexts, max_recv, drop_on_disconnect);
...
static u32 snort_deq_node_interrupt(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while ((inst = clib_interrupt_get_next_and_clear(ptd->interrupts, inst)) != -1) {
        ...
        if (!ready)
            n = snort_deq_instance_all_interrupt(vm, inst, qp, bi, nexts, n_left, si->drop_on_disconnect);
        else
        ...
        if (n_left == 0) goto enq;
    }

    if (n_left == VLIB_FRAME_SIZE) return 0;
...
static_always_inline uword snort_deq_instance_poll(vlib_main_t *vm, snort_qpair_t *qp, u32 *buffer_indices, u16 *nexts, u32 max_recv)
{
    ...
    if (n_left > max_recv) n_left = max_recv;
...
snort_deq_instance_all_poll(vlib_main_t *vm, snort_qpair_t *qp, u32 *buffer_indices, u16 *nexts, u32 max_recv, u8 drop_on_disconnect)
{
    u32 n_processed = snort_process_all_buffer_indices(qp, buffer_indices, nexts, max_recv, drop_on_disconnect);
    if (n_processed < max_recv) {
        *qp->enq_head = *qp->deq_head = qp->next_desc = 0;
        snort_freelist_init(qp->freelist);
        __atomic_store_n(&qp->ready, 1, __ATOMIC_RELEASE);
...
static u32 snort_deq_node_polling(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    vec_foreach (si, sm->instances) {
        ...
        u32 ready = __atomic_load_n(&qp->ready, __ATOMIC_ACQUIRE);
        if (!ready)
            n = snort_deq_instance_all_poll(vm, qp, bi, nexts, n_left, si->drop_on_disconnect);
        else
...
{
    snort_main_t *sm = &snort_main;
    if (sm->input_mode == VLIB_NODE_STATE_POLLING) return snort_deq_node_polling(vm, node, frame);
