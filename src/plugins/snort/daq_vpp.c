...
{
    int free = 0;
    while (!__atomic_compare_exchange_n(&p->lock, &free, 1, 0, __ATOMIC_ACQUIRE, __ATOMIC_RELAXED)) {
...
{
    if (base_api->api_version != DAQ_BASE_API_VERSION || base_api->api_size != sizeof(DAQ_BaseAPI_t)) return DAQ_ERROR;
...
{
    const int       ctl_sz = CMSG_SPACE(sizeof(int) * n_fds) + CMSG_SPACE(sizeof(struct ucred));
    char            ctl[ctl_sz];
    struct msghdr   mh = {};
    struct iovec    iov[1];
    struct cmsghdr *cmsg;
    ...
    int rv;
    if ((rv = recvmsg(fd, &mh, 0)) != sizeof(daq_vpp_msg_t)) return DAQ_ERROR_NODEV;
...
static void vpp_daq_destroy(void *handle)
{
    ...
    if (vc->shm_base != MAP_FAILED) munmap(vc->shm_base, vc->shm_size);

    if (vc->shm_fd != -1) close(vc->shm_fd);

    if (vc->bpools) {
        for (int i = 0; i < vc->num_bpools; i++) {
            VPPBufferPool *bp = vc->bpools + i;
            if (bp->fd != -1) close(bp->fd);
            if (bp->base && bp->base != MAP_FAILED) munmap(bp->base, bp->size);
        }
        free(vc->bpools);
    }
    ...
    if (vc->qpairs) {
        for (int i = 0; i < vc->num_qpairs; i++) {
            ...
            if (qp->enq_fd != -1) close(qp->enq_fd);
            if (qp->deq_fd != -1) close(qp->deq_fd);
            if (qp->desc_data) free(qp->desc_data);
        }
        free(vc->qpairs);
    }
    ...
    close(vc->sock_fd);
    if (vc->epoll_fd != -1) close(vc->epoll_fd);
    free(vc);
}
...
static int vpp_daq_instantiate(const DAQ_ModuleConfig_h modcfg, DAQ_ModuleInstance_h modinst, void **ctxt_ptr)
{
    ...
    if (!vc) ERR(DAQ_ERROR_NOMEM, "%s: Couldn't allocate memory for the new VPP context!", __func__);
    ...
    if (!vc->socket_name) /* try to use default socket path */
    ...
    if (connect(fd, (struct sockaddr *) &sun, sizeof(struct sockaddr_un)) != 0)
        ERR(DAQ_ERROR_NODEV, "%s: Couldn't connect to socket! '%s'", __func__, vc->socket_name);

    /* craft and send connect message */
    msg.type = DAQ_VPP_MSG_TYPE_HELLO;
    snprintf((char *) &msg.hello.inst_name, DAQ_VPP_INST_NAME_LEN - 1, "%s", input);

    if (send(fd, &msg, sizeof(msg), 0) != sizeof(msg)) ERR(DAQ_ERROR_NODEV, "%s: Couldn't send connect message!", __func__);

    /* receive config message */
    rval = vpp_daq_recvmsg(fd, &msg, 1, &shm_fd);

    if (rval != DAQ_SUCCESS || msg.type != DAQ_VPP_MSG_TYPE_CONFIG || shm_fd == -1)
    ...
    if (vc->bpools == 0 || vc->qpairs == 0) ERR(DAQ_ERROR_NOMEM, "%s: Couldn't allocate memory for the new VPP context!", __func__);

    for (i = 0; i < vc->num_bpools; i++)
        vc->bpools[i].fd = -1;

    base = mmap(0, vc->shm_size, PROT_READ | PROT_WRITE, MAP_SHARED, vc->shm_fd, 0);

    if (base == MAP_FAILED) ERR(DAQ_ERROR_NOMEM, "%s: Couldn't map shared memory for the new VPP context!", __func__);
    ...
    for (int i = 0; i < vc->num_bpools; i++) {
        ...
        if (rval != DAQ_SUCCESS || msg.type != DAQ_VPP_MSG_TYPE_BPOOL || bp->fd == -1)
            ERR(DAQ_ERROR_NODEV,
        ...
        if (bp->base == MAP_FAILED) ERR(DAQ_ERROR_NOMEM, "%s: Couldn't map shared memory for the new VPP context!", __func__);
        if (vc->debug) printf("  Buffer pool %u size: %u\n", i, bp->size);
    }

    if ((vc->epoll_fd = epoll_create(1)) == -1) ERR(DAQ_ERROR_NODEV, "%s: Couldn't create epoll fd for the new VPP context!", __func__);
    ...
    for (int i = 0; i < vc->num_qpairs; i++) {
        ...
        if (rval != DAQ_SUCCESS || msg.type != DAQ_VPP_MSG_TYPE_QPAIR || fds[0] == -1 || fds[1] == -1)
            ERR(DAQ_ERROR_NODEV,
        ...
        if (!qp->desc_data) ERR(DAQ_ERROR_NOMEM, "%s: Couldn't allocate memory for the new VPP context!", __func__);
...
static inline uint32_t vpp_daq_msg_receive_one(VPP_Context_t *vc, VPPQueuePair *qp, const DAQ_Msg_t *msgs[], unsigned max_recv)
{
    uint32_t n_recv, n_left;
    uint32_t head, next, mask = qp->queue_size - 1;

    if (max_recv == 0) return 0;
...
static unsigned vpp_daq_msg_receive(void *handle, const unsigned max_recv, const DAQ_Msg_t *msgs[], DAQ_RecvStatus *rstat)
{
    ...
    while (n_qpairs_left) {
        ...
        /* next */
        vc->next_qpair++;
        if (vc->next_qpair == vc->num_qpairs) vc->next_qpair = 0;
        n_qpairs_left--;
    }
...
static int vpp_daq_msg_finalize(void *handle, const DAQ_Msg_t *msg, DAQ_Verdict verdict)
{
    ...
    if (vc->input_mode == DAQ_VPP_INPUT_MODE_INTERRUPT) {
        ...
        if (rv != sizeof(counter_increment)) retv = DAQ_ERROR;
    }
