...
static void intel_dsa_channel_lock(intel_dsa_channel_t *ch)
{
    u8 expected = 0;
    if (ch->n_threads < 2) return;

    /* channel is used by multiple threads so we need to lock it */
    while (!__atomic_compare_exchange_n(&ch->lock,
                                        &expected,
                                        /* desired */ 1,
                                        /* weak */ 0,
                                        __ATOMIC_ACQUIRE,
...
static void intel_dsa_channel_unlock(intel_dsa_channel_t *ch)
{
    if (ch->n_threads < 2) return;
...
static vlib_dma_batch_t *intel_dsa_batch_new(vlib_main_t *vm, struct vlib_dma_config_data *cd)
{
    intel_dsa_main_t   *idm = &intel_dsa_main;
    intel_dsa_config_t *idc;
    intel_dsa_batch_t  *b;

    idc = vec_elt_at_index(idm->dsa_config_heap, cd->private_data + vm->thread_index);
    ...
    if (vec_len(idc->freelist) > 0)
        ...
        *b               = idc->batch_template;
        ...
        if (b->ch->block_on_fault) def_flags |= INTEL_DSA_FLAG_BLOCK_ON_FAULT;
...
static_always_inline void __movdir64b(volatile void *dst, const void *src)
{
    asm volatile(".byte 0x66, 0x0f, 0x38, 0xf8, 0x02" : : "a"(dst), "d"(src) : "memory");
}
#endif

static_always_inline void intel_dsa_batch_fallback(vlib_main_t *vm, intel_dsa_batch_t *b, intel_dsa_channel_t *ch)
{
...
int intel_dsa_batch_submit(vlib_main_t *vm, struct vlib_dma_batch *vb)
{
    intel_dsa_main_t    *idm = &intel_dsa_main;
    ...
    if (ch->n_enq >= ch->size) {
        ...
        if (n_pendings) lb = idm->dsa_threads[vm->thread_index].pending_batches[n_pendings - 1];
    ...
    if (PREDICT_FALSE(vb->n_enq == 1)) {
        ...
        desc->op_flags |= INTEL_DSA_FLAG_COMPLETION_ADDR_VALID | INTEL_DSA_FLAG_REQUEST_COMPLETION;
#if defined(__x86_64__) || defined(i386)
        ...
        batch_desc->op_flags  = (INTEL_DSA_OP_BATCH << INTEL_DSA_OP_SHIFT) | INTEL_DSA_FLAG_COMPLETION_ADDR_VALID | INTEL_DSA_FLAG_REQUEST_COMPLETION;
...
static int intel_dsa_check_channel(intel_dsa_channel_t *ch, vlib_dma_config_data_t *cd)
{
    if (!ch) {
...
static_always_inline void intel_dsa_alloc_dma_batch(vlib_main_t *vm, intel_dsa_config_t *idc)
{
    intel_dsa_batch_t *b;
    b = vlib_physmem_alloc(vm, idc->alloc_size);
    /* if no free space in physmem, force quit */
...
static int intel_dsa_config_add_fn(vlib_main_t *vm, vlib_dma_config_data_t *cd)
{
    intel_dsa_main_t   *idm = &intel_dsa_main;
    intel_dsa_config_t *idc;
    ...
    index = heap_alloc_aligned(idm->dsa_config_heap, n_threads, CLIB_CACHE_LINE_BYTES, idm->dsa_config_heap_handle_by_config_index[cd->config_index]);
    ...
    for (u32 thread = 0; thread < n_threads; thread++) {
        ...
        idc->alloc_size    = sizeof(intel_dsa_batch_t) + sizeof(intel_dsa_desc_t) * (idc->max_transfers + 1);
        /* fill batch template */
        idb     = &idc->batch_template;
        idb->ch = idm->dsa_threads[thread].ch;
        if (intel_dsa_check_channel(idb->ch, cd)) return 0;

        dsa_log_debug("config %d in thread %d using channel %u/%u", cd->config_index, thread, idb->ch->did, idb->ch->qid);
...
static void intel_dsa_config_del_fn(vlib_main_t *vm, vlib_dma_config_data_t *cd)
{
    intel_dsa_main_t   *idm = &intel_dsa_main;
    intel_dsa_thread_t *t   = vec_elt_at_index(idm->dsa_threads, vm->thread_index);
    u32                 n_pending, n_threads, config_heap_index, n = 0;
    n_threads = vlib_get_n_threads();

    if (!t->pending_batches) goto free_heap;
    ...
    for (u32 i = 0; i < n_pending; i++) {
        ...
        if (b->config_index == cd->config_index) {
            vec_add1(idm->dsa_config_heap[b->config_heap_index].freelist, b);
            if (b->status == INTEL_DSA_STATUS_SUCCESS || b->status == INTEL_DSA_STATUS_BUSY) b->ch->n_enq--;
        }
        else
...
free_heap:
    ...
    heap_dealloc(idm->dsa_config_heap, idm->dsa_config_heap_handle_by_config_index[cd->config_index]);

    dsa_log_debug("config %u removed", cd->private_data);
}

static uword intel_dsa_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    intel_dsa_main_t   *idm       = &intel_dsa_main;
    intel_dsa_thread_t *t         = vec_elt_at_index(idm->dsa_threads, vm->thread_index);
    u32                 n_pending = 0, n = 0;
    ...
    for (u32 i = 0; i < n_pending; i++) {
        ...
        status = b->status;
        if ((status == INTEL_DSA_STATUS_SUCCESS || status == INTEL_DSA_STATUS_CPU_SUCCESS) && !glitch) {
            /* callback */
            if (b->batch.callback_fn) b->batch.callback_fn(vm, &b->batch);

            /* restore last descriptor fields */
            if (b->batch.n_enq == 1) {
                b->descs[0].completion = 0;
                b->descs[0].op_flags   = (INTEL_DSA_OP_MEMMOVE << INTEL_DSA_OP_SHIFT) | INTEL_DSA_FLAG_CACHE_CONTROL;
                if (b->ch->block_on_fault) b->descs[0].op_flags |= INTEL_DSA_FLAG_BLOCK_ON_FAULT;
            }
            /* add to freelist */
            vec_add1(idm->dsa_config_heap[b->config_heap_index].freelist, b);
...
u8 *format_dsa_info(u8 *s, va_list *args)
{
    intel_dsa_main_t    *idm = &intel_dsa_main;
    ...
    s  = format(s,
               "thread %d dma %u/%u request %-16lld hw %-16lld cpu %-16lld",
               vm->thread_index,
               ch->did,
               ch->qid,
               ch->submitted,
