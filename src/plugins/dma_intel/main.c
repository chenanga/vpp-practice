...
void intel_dsa_assign_channels(vlib_main_t *vm)
{
    ...
    if (vec_len(chv) >= vlib_get_n_threads())
        ...
        n_threads = vlib_get_n_threads() % vec_len(chv) ? vlib_get_n_threads() / vec_len(chv) + 1 : vlib_get_n_threads() / vec_len(chv);
    ...
    for (int i = 0; i < vlib_get_n_threads(); i++) {
        ...
        dsa_log_debug("Assigning channel %u/%u to thread %u (numa %u)", ch->did, ch->qid, i, tvm->numa_node);
    }
...
static clib_error_t *intel_dsa_map_region(intel_dsa_channel_t *ch)
{
    ...
    if (fd < 0) return clib_error_return(0, "failed to open dsa device %s", path);

    ch->portal = clib_mem_vm_map_shared(0, size, fd, offset, "%s", (char *) path);
...
static clib_error_t *intel_dsa_get_info(intel_dsa_channel_t *ch, clib_error_t **error)
{
    clib_error_t *err;
    u8           *tmpstr;
...
clib_error_t *intel_dsa_add_channel(vlib_main_t *vm, intel_dsa_channel_t *ch)
{
    intel_dsa_main_t *dm  = &intel_dsa_main;
    clib_error_t     *err = 0;

    if (intel_dsa_map_region(ch)) return clib_error_return(0, "dsa open device failed");

    if (intel_dsa_get_info(ch, &err)) return clib_error_return(err, "dsa info not scanned");
...
static clib_error_t *dsa_config(vlib_main_t *vm, unformat_input_t *input)
{
    clib_error_t        *error = 0;
    intel_dsa_channel_t *ch;
    u32                  did, qid;

    if (intel_dsa_main.lock == 0) clib_spinlock_init(&(intel_dsa_main.lock));

    if ((error = vlib_dma_register_backend(vm, &intel_dsa_backend))) goto done;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "dev wq%d.%d", &did, &qid)) {
            ...
            ch->did = did;
            ch->qid = qid;
            if (intel_dsa_add_channel(vm, ch)) clib_mem_free(ch);
        }
...
clib_error_t *intel_dsa_num_workers_change(vlib_main_t *vm)
{
