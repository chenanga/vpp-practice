...
static u8 *format_af_packet_input_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "af_packet: hw_if_index %d rx-queue %u next-index %d", t->hw_if_index, t->queue_id, t->next_index);
...
always_inline void buffer_add_to_chain(vlib_buffer_t *b, vlib_buffer_t *first_b, vlib_buffer_t *prev_b, u32 bi)
{
    /* update first buffer */
...
static_always_inline void fill_gso_offload(vlib_buffer_t *b, u32 gso_size, u8 l4_hdr_sz)
{
...
static_always_inline void fill_cksum_offload(vlib_buffer_t *b, u8 *l4_hdr_sz, u8 is_ip)
{
    ...
    if (oflags) vnet_buffer_offload_flags_set(b, oflags);
}

always_inline uword af_packet_v3_device_input_fn(vlib_main_t         *vm,
                                                 ...
                                                 u8                   is_cksum_gso_enabled)
{
    ...
    if ((((block_desc_t *) (block_start = rx_queue->rx_ring[block]))->hdr.bh1.block_status & TP_STATUS_USER) != 0) {
        u32 n_required = 0;
        ...
        if (PREDICT_FALSE(rx_queue->is_rx_pending)) {
            ...
            if (TP_STATUS_BLK_TMO & bd->hdr.bh1.block_status) timedout_blk++;
        }
        ...
        if (PREDICT_FALSE(n_free_bufs < n_required)) {
            vec_validate(apm->rx_buffers[thread_index], n_required + n_free_bufs - 1);
            n_free_bufs += vlib_buffer_alloc(vm, &apm->rx_buffers[thread_index][n_free_bufs], n_required);
        ...
        while (num_pkts && (n_free_bufs >= min_bufs)) {
            ...
            while (num_pkts && n_left_to_next && (n_free_bufs >= min_bufs)) {
                ...
                if (num_pkts > 1) CLIB_PREFETCH(block_start + rx_frame_offset + tph->tp_next_offset, 2 * CLIB_CACHE_LINE_BYTES, LOAD);
                ...
                if (is_cksum_gso_enabled) vnet_hdr = (vnet_virtio_net_hdr_t *) ((u8 *) tph + tph->tp_mac - sizeof(vnet_virtio_net_hdr_t));

                // save current state and return
                if (PREDICT_FALSE(((data_len / n_buffer_bytes) + 1) > vec_len(apm->rx_buffers[thread_index]))) {
                ...
                while (data_len) {
                    ...
                    if (PREDICT_FALSE(tph->tp_status & TP_STATUS_VLAN_VALID)) {
                        if (PREDICT_TRUE(offset == 0)) {
                            clib_memcpy_fast(vlib_buffer_get_current(b0), (u8 *) tph + tph->tp_mac, sizeof(ethernet_header_t));
                            ethernet_header_t      *eth  = vlib_buffer_get_current(b0);
                            ethernet_vlan_header_t *vlan = (ethernet_vlan_header_t *) (eth + 1);
                            vlan->priority_cfi_and_id    = clib_host_to_net_u16(tph->hv1.tp_vlan_tci);
                            vlan->type                   = eth->type;
                    ...
                    clib_memcpy_fast(((u8 *) vlib_buffer_get_current(b0)) + bytes_copied + vlan_len,
                    ...
                    if (offset == 0) {
                        ...
                        if (is_cksum_gso_enabled) {
                            if (vnet_hdr->flags & VIRTIO_NET_HDR_F_NEEDS_CSUM) fill_cksum_offload(first_b0, &l4_hdr_sz, is_ip);
                            if (vnet_hdr->gso_type & (VIRTIO_NET_HDR_GSO_TCPV4 | VIRTIO_NET_HDR_GSO_TCPV6))
                                fill_gso_offload(first_b0, vnet_hdr->gso_size, l4_hdr_sz);
                        }
                    }
                    else
                ...
                if (PREDICT_FALSE(tph->tp_len != tph->tp_snaplen)) {
                    ...
                    if (PREDICT_FALSE(apif->mode == AF_PACKET_IF_MODE_IP)) {
                        ...
                        if (PREDICT_FALSE(apif->per_interface_next_index != ~0)) next0 = apif->per_interface_next_index;
                    }
                    else {
                        next0 = VNET_DEVICE_INPUT_NEXT_ETHERNET_INPUT;
                        if (PREDICT_FALSE(apif->per_interface_next_index != ~0)) next0 = apif->per_interface_next_index;
                        /* redirect if feature path enabled */
                        vnet_feature_start_device_input(sw_if_index, &next0, first_b0);
                    }
                }
                ...
                if (PREDICT_FALSE(n_trace > 0
                    ...
                    if (is_cksum_gso_enabled)
                        clib_memcpy_fast(&tr->vnet_hdr, vnet_hdr, sizeof(vnet_virtio_net_hdr_t));
                    else
                        clib_memset_u8(&tr->vnet_hdr, 0, sizeof(vnet_virtio_net_hdr_t));
                }

                /* enque and take next packet */
                vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, first_bi0, next0);
    ...
    vlib_error_count(vm, node->node_index, AF_PACKET_INPUT_ERROR_TOTAL_RECV_BLK, total);
    vlib_error_count(vm, node->node_index, AF_PACKET_INPUT_ERROR_TIMEDOUT_BLK, timedout_blk);

    vlib_increment_combined_counter(vnet_get_main()->interface_main.combined_sw_if_counters + VNET_INTERFACE_COUNTER_RX,
...
always_inline uword af_packet_v2_device_input_fn(vlib_main_t         *vm,
                                                 vlib_node_runtime_t *node,
                                                 vlib_frame_t        *frame,
                                                 af_packet_if_t      *apif,
                                                 u16                  queue_id,
                                                 u8                   is_cksum_gso_enabled)
{
    ...
    if (PREDICT_FALSE(n_free_bufs < VLIB_FRAME_SIZE)) {
        ...
        n_free_bufs += vlib_buffer_alloc(vm, &apm->rx_buffers[thread_index][n_free_bufs], VLIB_FRAME_SIZE);
        vec_set_len(apm->rx_buffers[thread_index], n_free_bufs);
    }
...
always_inline uword af_packet_device_input_fn(vlib_main_t         *vm,
                                              vlib_node_runtime_t *node,
                                              vlib_frame_t        *frame,
                                              af_packet_if_t      *apif,
                                              u16                  queue_id,
...

{
    if (apif->version == TPACKET_V3)
        return af_packet_v3_device_input_fn(vm, node, frame, apif, queue_id, is_cksum_gso_enabled);
    else
        return af_packet_v2_device_input_fn(vm, node, frame, apif, queue_id, is_cksum_gso_enabled);
}
    ...
    for (int i = 0; i < vec_len(pv); i++) {
        ...
        if (apif->is_admin_up) {
            if (apif->is_cksum_gso_enabled)
                n_rx_packets += af_packet_device_input_fn(vm, node, frame, apif, pv[i].queue_id, 1);
            else
                n_rx_packets += af_packet_device_input_fn(vm, node, frame, apif, pv[i].queue_id, 0);
        }
    }
    return n_rx_packets;
}
