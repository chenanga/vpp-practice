...
static u8 *format_af_packet_device(u8 *s, va_list *args)
{
    ...
    s = format(s, "Linux PACKET socket interface %s", (apif->version == TPACKET_V2) ? "v2" : "v3");
    s = format(s, "\n%UFEATURES:", format_white_space, indent);
    if (apif->is_qdisc_bypass_enabled) s = format(s, "\n%Uqdisc-bpass-enabled", format_white_space, indent + 2);
    if (apif->is_cksum_gso_enabled) s = format(s, "\n%Ucksum-gso-enabled", format_white_space, indent + 2);
    if (apif->is_fanout_enabled) s = format(s, "\n%Ufanout-enabled", format_white_space, indent + 2);
    ...
    vec_foreach (rx_queue, apif->rx_queues) {
        ...
        s = format(s, "\n%URX Queue %u:", format_white_space, indent, rx_queue->queue_id);
        s = format(s,
    ...
    vec_foreach (tx_queue, apif->tx_queues) {
        ...
        s = format(s, "\n%UTX Queue %u:", format_white_space, indent, tx_queue->queue_id);
        s = format(s,
                   "\n%Ublock size:%d nr:%d  frame size:%d nr:%d",
                   format_white_space,
                   indent + 2,
        ...
        if (apif->version & TPACKET_V3) do {
                tph3     = (tpacket3_hdr_t *) (tx_block_start + tx_frame * tx_frame_sz);
            ...
            do {
                tph2     = (tpacket2_hdr_t *) (tx_block_start + tx_frame * tx_frame_sz);
                tx_frame = (tx_frame + 1) % tx_frame_nr;
                if (tph2->tp_status == 0)
                    n_avail++;
...
static u8 *format_af_packet_tx_trace(u8 *s, va_list *va)
{
    ...
    s = format(s, "af_packet: hw_if_index %u tx-queue %u", t->hw_if_index, t->queue_id);
    ...
    s = format(s,
               ...
               indent + 4,
               t->vnet_hdr.gso_size,
               t->vnet_hdr.csum_start,
               t->vnet_hdr.csum_offset);
    ...
    s = format(s,
               ...
               &t->buffer);
    s = format(s, "\n%U%U", format_white_space, indent + 2, format_ethernet_header_with_length, t->buffer.pre_data, sizeof(t->buffer.pre_data));
    return s;
}

static void af_packet_tx_trace(vlib_main_t           *vm,
...
static_always_inline void fill_gso_offload(vlib_buffer_t *b0, vnet_virtio_net_hdr_t *vnet_hdr)
{
    ...
    if (b0->flags & VNET_BUFFER_F_IS_IP4) {
        ...
        tcp_header_t *tcp = (tcp_header_t *) (b0->data + vnet_buffer(b0)->l4_hdr_offset);
...
static_always_inline void fill_cksum_offload(vlib_buffer_t *b0, vnet_virtio_net_hdr_t *vnet_hdr)
{
    vnet_buffer_oflags_t oflags        = vnet_buffer(b0)->oflags;
    i16                  l4_hdr_offset = vnet_buffer(b0)->l4_hdr_offset - b0->current_data;
    if (b0->flags & VNET_BUFFER_F_IS_IP4) {
        ...
        if (oflags & VNET_BUFFER_OFFLOAD_F_TCP_CKSUM) {
            ...
            udp_header_t *udp     = (udp_header_t *) (b0->data + vnet_buffer(b0)->l4_hdr_offset);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    if (tf->shared_queue) clib_spinlock_lock(&tx_queue->lockp);
    ...
    if (is_v2) {
        ...
        while (n_left) {
            ...
            if (PREDICT_FALSE(tph2->tp_status & (TP_STATUS_SEND_REQUEST | TP_STATUS_SENDING))) {
                frame_not_ready++;
                goto nextv2;
            }
            ...
            if (PREDICT_TRUE(is_cksum_gso_enabled)) {
                vnet_hdr = (vnet_virtio_net_hdr_t *) ((u8 *) tph2 + tpacket_align);
            ...
            clib_memcpy_fast((u8 *) tph2 + tpacket_align + offset, vlib_buffer_get_current(b0), len);
            offset += len;
            ...
            if (PREDICT_FALSE(b0_first->flags & VLIB_BUFFER_IS_TRACED)) {
                if (PREDICT_TRUE(is_cksum_gso_enabled))
                    af_packet_tx_trace(vm, node, b0_first, bi_first, tph2, vnet_hdr, apif->hw_if_index, queue_id, 1);
                else {
                    vnet_virtio_net_hdr_t vnet_hdr2 = {};
                    af_packet_tx_trace(vm, node, b0_first, bi_first, tph2, &vnet_hdr2, apif->hw_if_index, queue_id, 1);
                }
            }
        ...
        nextv2:
            ...
            if (PREDICT_FALSE(frame_not_ready + n_sent == frame_num)) break;
        }
    }
    else {
        ...
        while (n_left) {
            ...
            if (PREDICT_FALSE(tph3->tp_status & (TP_STATUS_SEND_REQUEST | TP_STATUS_SENDING))) {
                frame_not_ready++;
                goto nextv3;
            }
            ...
            if (PREDICT_TRUE(is_cksum_gso_enabled)) {
                vnet_hdr = (vnet_virtio_net_hdr_t *) ((u8 *) tph3 + tpacket_align);
            ...
            if (PREDICT_FALSE(b0_first->flags & VLIB_BUFFER_IS_TRACED)) {
                if (PREDICT_TRUE(is_cksum_gso_enabled))
                    af_packet_tx_trace(vm, node, b0_first, bi_first, tph3, vnet_hdr, apif->hw_if_index, queue_id, 0);
                else {
                    vnet_virtio_net_hdr_t vnet_hdr2 = {};
                    af_packet_tx_trace(vm, node, b0_first, bi_first, tph3, &vnet_hdr2, apif->hw_if_index, queue_id, 0);
                }
            }
            tx_frame = (tx_frame + 1) % frame_num;
    ...
    if (PREDICT_TRUE(n_sent || tx_queue->is_tx_pending)) {
        ...
        if (PREDICT_FALSE(sendto(tx_queue->fd, NULL, 0, MSG_DONTWAIT, NULL, 0) == -1)) {
    ...
    if (tf->shared_queue) clib_spinlock_unlock(&tx_queue->lockp);

    if (PREDICT_FALSE(frame_not_ready)) vlib_error_count(vm, node->node_index, AF_PACKET_TX_ERROR_FRAME_NOT_READY, frame_not_ready);

    if (PREDICT_FALSE(frame_not_ready + n_sent == frame_num)) vlib_error_count(vm, node->node_index, AF_PACKET_TX_ERROR_TXRING_OVERRUN, n_left);
...
static void af_packet_set_interface_next_node(vnet_main_t *vnm, u32 hw_if_index, u32 node_index)
{
    ...
    apif->per_interface_next_index = vlib_node_add_next(vlib_get_main(), af_packet_input_node.index, node_index);
}
...
static clib_error_t *af_packet_interface_admin_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    ...
    if (apif->host_if_index < 0) return 0; /* no error */
...
static clib_error_t *af_packet_set_mac_address_function(struct vnet_hw_interface_t *hi, const u8 *old_address, const u8 *address)
{
    ...
    if (apif->mode == AF_PACKET_IF_MODE_IP) {
        vlib_log_warn(apm->log_class, "af_packet_%s interface is in IP mode", apif->host_if_name);
        return clib_error_return(0, " MAC update failed, interface is in IP mode");
    }

    fd = socket(AF_UNIX, SOCK_DGRAM, 0);
    if (0 > fd) {
        vlib_log_warn(apm->log_class, "af_packet_%s could not open socket", apif->host_if_name);
        return 0;
    }

    /* if interface is a bridge ignore */
    if (apif->host_if_index < 0) goto error; /* no error */

    /* use host_if_index in case host name has changed */
    ifr.ifr_ifindex = apif->host_if_index;
    if ((rv = ioctl(fd, SIOCGIFNAME, &ifr)) < 0) {
        vlib_log_warn(apm->log_class, "af_packet_%s ioctl could not retrieve eth name, error: %d", apif->host_if_name, rv);
        goto error;
    }
    ...
    if ((rv = ioctl(fd, SIOCSIFHWADDR, &ifr)) < 0) {
        vlib_log_warn(apm->log_class, "af_packet_%s ioctl could not set mac, error: %d", apif->host_if_name, rv);
        goto error;
    }
...
static clib_error_t *af_packet_interface_rx_mode_change(vnet_main_t *vnm, u32 hw_if_index, u32 qid, vnet_hw_if_rx_mode mode)
{
    af_packet_main_t    *apm = &af_packet_main;
    vnet_hw_interface_t *hw  = vnet_get_hw_interface(vnm, hw_if_index);
    af_packet_if_t      *apif;
    ...
    if (mode == VNET_HW_IF_RX_MODE_ADAPTIVE) {
        vlib_log_err(apm->log_class, "af_packet_%s adaptive mode is not supported", apif->host_if_name);
        return clib_error_return(0, "af_packet_%s adaptive mode is not supported", apif->host_if_name);
    }
