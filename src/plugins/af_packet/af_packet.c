...
#define AF_PACKET_OFFLOAD_FLAG_MASK                                                                                           \
    (AF_PACKET_OFFLOAD_FLAG_RXCKSUM | AF_PACKET_OFFLOAD_FLAG_TXCKSUM | AF_PACKET_OFFLOAD_FLAG_SG | AF_PACKET_OFFLOAD_FLAG_TSO \
     | AF_PACKET_OFFLOAD_FLAG_UFO | AF_PACKET_OFFLOAD_FLAG_GSO | AF_PACKET_OFFLOAD_FLAG_GRO)
...
static u32 af_packet_get_if_capabilities(u8 *host_if_name)
{
    struct ifreq         ifr;
    ...
    clib_memcpy(ifr.ifr_name, host_if_name, strlen((const char *) host_if_name));
    ifr.ifr_data = (void *) &e;

    e.cmd = ETHTOOL_GRXCSUM;
    AF_PACKET_IOCTL(ctl_fd, SIOCETHTOOL, &ifr);
    if (e.data) oflags |= AF_PACKET_OFFLOAD_FLAG_RXCKSUM;

    e.cmd = ETHTOOL_GTXCSUM;
    AF_PACKET_IOCTL(ctl_fd, SIOCETHTOOL, &ifr);
    if (e.data) oflags |= AF_PACKET_OFFLOAD_FLAG_TXCKSUM;

    e.cmd = ETHTOOL_GTSO;
    AF_PACKET_IOCTL(ctl_fd, SIOCETHTOOL, &ifr);
    if (e.data) oflags |= AF_PACKET_OFFLOAD_FLAG_TSO;

    e.cmd = ETHTOOL_GGSO;
    AF_PACKET_IOCTL(ctl_fd, SIOCETHTOOL, &ifr);
    if (e.data) oflags |= AF_PACKET_OFFLOAD_FLAG_GSO;

    e.cmd = ETHTOOL_GGRO;
    AF_PACKET_IOCTL(ctl_fd, SIOCETHTOOL, &ifr);
    if (e.data) oflags |= AF_PACKET_OFFLOAD_FLAG_GRO;

done:
    if (ctl_fd != -1) close(ctl_fd);

    return oflags;
}

static clib_error_t *af_packet_eth_set_max_frame_size(vnet_main_t *vnm, vnet_hw_interface_t *hi, u32 frame_size)
{
    clib_error_t     *error, *rv;
    af_packet_main_t *apm  = &af_packet_main;
    af_packet_if_t   *apif = pool_elt_at_index(apm->interfaces, hi->dev_instance);

    error = vnet_netlink_set_link_mtu(apif->host_if_index, frame_size + hi->frame_overhead);

    if (error) {
        vlib_log_err(apm->log_class, "netlink failed to change MTU: %U", format_clib_error, error);
        rv = vnet_error(VNET_ERR_SYSCALL_ERROR_1, "netlink error: %U", format_clib_error, error);
...
static int af_packet_read_mtu(af_packet_if_t *apif)
{
    ...
    if (error) {
        vlib_log_err(apm->log_class, "netlink failed to get MTU: %U", format_clib_error, error);
        clib_error_free(error);
...
static clib_error_t *af_packet_fd_error(clib_file_t *uf)
{
    af_packet_main_t *apm = &af_packet_main;
    clib_error_t     *err = 0;
    u64               u64;
    ...
    if (ret < 0) {
        ...
        vlib_log_notice(apm->log_class, "fd %u %U", uf->file_descriptor, format_clib_error, err);
        clib_error_free(err);
    }
...
static void af_packet_set_rx_queues(vlib_main_t *vm, af_packet_if_t *apif)
{
    vnet_main_t       *vnm = vnet_get_main();
    af_packet_queue_t *rx_queue;

    vnet_hw_if_set_input_node(vnm, apif->hw_if_index, af_packet_input_node.index);

    vec_foreach (rx_queue, apif->rx_queues) {
        rx_queue->queue_index = vnet_hw_if_register_rx_queue(vnm, apif->hw_if_index, rx_queue->queue_id, VNET_HW_IF_RXQ_THREAD_ANY);

        {
            ...
            template.description      = format(0, "%U queue %u", format_af_packet_device_name, apif->dev_instance, rx_queue->queue_id);
            rx_queue->clib_file_index = clib_file_add(&file_main, &template);
        }
        vnet_hw_if_set_rx_queue_file_index(vnm, rx_queue->queue_index, rx_queue->clib_file_index);
        vnet_hw_if_set_rx_queue_mode(vnm, rx_queue->queue_index, VNET_HW_IF_RX_MODE_INTERRUPT);
        rx_queue->mode = VNET_HW_IF_RX_MODE_INTERRUPT;
    }
    vnet_hw_if_update_runtime_data(vnm, apif->hw_if_index);
}

static void af_packet_set_tx_queues(vlib_main_t *vm, af_packet_if_t *apif)
{
    vnet_main_t       *vnm = vnet_get_main();
    af_packet_main_t  *apm = &af_packet_main;
    ...
    vec_foreach (tx_queue, apif->tx_queues) {
        tx_queue->queue_index = vnet_hw_if_register_tx_queue(vnm, apif->hw_if_index, tx_queue->queue_id);
    }

    if (apif->num_txqs == 0) {
        vlib_log_err(apm->log_class, "Interface %U has 0 txq", format_vnet_hw_if_index_name, vnm, apif->hw_if_index);
        return;
    }
...
static int create_packet_sock(int                   host_if_index,
                              ...
                              int                   ver)
{
    ...
    if ((*fd = socket(AF_PACKET, SOCK_RAW, htons(ETH_P_ALL))) < 0) {
        vlib_log_err(apm->log_class, "Failed to create AF_PACKET socket: %s (errno %d)", strerror(errno), errno);
    ...
    if (bind(*fd, (struct sockaddr *) &sll, sizeof(sll)) < 0) {
        vlib_log_err(apm->log_class, "Failed to bind rx packet socket: %s (errno %d)", strerror(errno), errno);
        ret = VNET_API_ERROR_SYSCALL_ERROR_1;
        goto error;
    }

    if (setsockopt(*fd, SOL_PACKET, PACKET_VERSION, &ver, sizeof(ver)) < 0) {
        vlib_log_err(apm->log_class, "Failed to set rx packet interface version: %s (errno %d)", strerror(errno), errno);
        ret = VNET_API_ERROR_SYSCALL_ERROR_1;
        goto error;
    }
    ...
    if (setsockopt(*fd, SOL_PACKET, PACKET_LOSS, &opt, sizeof(opt)) < 0) {
        vlib_log_err(apm->log_class, "Failed to set packet tx ring error handling option: %s (errno %d)", strerror(errno), errno);
        ret = VNET_API_ERROR_SYSCALL_ERROR_1;
        goto error;
    }
    ...
    if (*flags & AF_PACKET_IF_FLAGS_QDISC_BYPASS) /* Introduced with Linux 3.14 so the ifdef should eventually be removed  */
    ...
    if (rx_req) {
        if (*flags & AF_PACKET_IF_FLAGS_FANOUT) {
            ...
            if (setsockopt(*fd, SOL_PACKET, PACKET_FANOUT, &fanout, sizeof(fanout)) < 0) {
                ...
                *flags &= ~AF_PACKET_IF_FLAGS_FANOUT;
                vlib_log_err(apm->log_class, "Failed to set fanout options: %s (errno %d)", strerror(errno), errno);
                ret = VNET_API_ERROR_SYSCALL_ERROR_1;
                goto error;
            }
        }
        ...
        if (setsockopt(*fd, SOL_PACKET, PACKET_RX_RING, rx_req, req_sz) < 0) {
            vlib_log_err(apm->log_class, "Failed to set packet rx ring options: %s (errno %d)", strerror(errno), errno);
            ret = VNET_API_ERROR_SYSCALL_ERROR_1;
            goto error;
        }
    }
    ...
    if (tx_req) {
        ...
        if (setsockopt(*fd, SOL_PACKET, PACKET_TX_RING, tx_req, req_sz) < 0) {
            vlib_log_err(apm->log_class, "Failed to set packet tx ring options: %s (errno %d)", strerror(errno), errno);
            ret = VNET_API_ERROR_SYSCALL_ERROR_1;
            goto error;
        }
    }
    ...
    if (ring->ring_start_addr == MAP_FAILED) {
        vlib_log_err(apm->log_class, "mmap failure: %s (errno %d)", strerror(errno), errno);
        ret = VNET_API_ERROR_SYSCALL_ERROR_1;
        goto error;
    }
...
int af_packet_queue_init(vlib_main_t               *vm,
                         af_packet_if_t            *apif,
                         ...
                         u8                         queue_id)
{
    ...
    if (rx_queue) {
        rx_frames_per_block = arg->rx_frames_per_block ? arg->rx_frames_per_block
                                                       : ((apif->version == TPACKET_V3) ? AF_PACKET_DEFAULT_RX_FRAMES_PER_BLOCK
                                                                                        : AF_PACKET_DEFAULT_RX_FRAMES_PER_BLOCK_V2);

        rx_frame_size = arg->rx_frame_size ? arg->rx_frame_size
                                           : ((apif->version == TPACKET_V3) ? AF_PACKET_DEFAULT_RX_FRAME_SIZE : AF_PACKET_DEFAULT_RX_FRAME_SIZE_V2);
        ...
        rx_queue->rx_req->req.tp_block_nr   = (apif->version == TPACKET_V3) ? AF_PACKET_RX_BLOCK_NR : AF_PACKET_RX_BLOCK_NR_V2;
        rx_queue->rx_req->req.tp_frame_nr   = rx_queue->rx_req->req.tp_block_nr * rx_frames_per_block;
    ...
    if (tx_queue) {
        tx_frames_per_block = arg->tx_frames_per_block ? arg->tx_frames_per_block : AF_PACKET_DEFAULT_TX_FRAMES_PER_BLOCK;
        tx_frame_size       = arg->tx_frame_size ? arg->tx_frame_size : AF_PACKET_DEFAULT_TX_FRAME_SIZE;

        vec_validate(tx_queue->tx_req, 0);
        tx_queue->tx_req->req.tp_block_size = tx_frame_size * tx_frames_per_block;
    ...
    if (rx_queue || tx_queue) {
        ret = create_packet_sock(apif->host_if_index, rx_req, tx_req, &fd, &ring, apif->dev_instance, &arg->flags, apif->version);
    ...
    if (rx_queue) {
        ...
        vec_foreach_index (i, rx_queue->rx_ring) {
            rx_queue->rx_ring[i] = ring_addr + i * rx_queue->rx_req->req.tp_block_size;
        }

        rx_queue->next_rx_block = 0;
        rx_queue->queue_id      = queue_id;
        rx_queue->is_rx_pending = 0;
        ring_addr               = ring_addr + rx_queue->rx_req->req.tp_block_size * rx_queue->rx_req->req.tp_block_nr;
    }
    ...
    if (tx_queue) {
        ...
        vec_foreach_index (i, tx_queue->tx_ring) {
            tx_queue->tx_ring[i] = ring_addr + i * tx_queue->tx_req->req.tp_block_size;
        }
...
error:
    ...
    if (rx_queue) vec_free(rx_queue->rx_req);
    if (tx_queue) vec_free(tx_queue->tx_req);
    return ret;
}

int af_packet_device_init(vlib_main_t *vm, af_packet_if_t *apif, af_packet_create_if_arg_t *args)
{
    af_packet_main_t  *apm      = &af_packet_main;
    ...
    if (args->num_rxqs > 1) args->flags |= AF_PACKET_IF_FLAGS_FANOUT;
...
int af_packet_create_if(af_packet_create_if_arg_t *arg)
{
    ...
    if ((fd2 = socket(AF_UNIX, SOCK_DGRAM, 0)) < 0) {
        vlib_log_debug(apm->log_class, "Failed to create AF_UNIX socket: %s (errno %d)", strerror(errno), errno);
        ret = VNET_API_ERROR_SYSCALL_ERROR_1;
        goto error;
    }

    clib_memcpy(ifr.ifr_name, (const char *) arg->host_if_name, vec_len(arg->host_if_name));
    if (ioctl(fd2, SIOCGIFINDEX, &ifr) < 0) {
        vlib_log_debug(apm->log_class, "Failed to retrieve the interface (%s) index: %s (errno %d)", arg->host_if_name, strerror(errno), errno);
    ...
    if (ioctl(fd2, SIOCGIFFLAGS, &ifr) < 0) {
        vlib_log_debug(apm->log_class, "Failed to get the active flag: %s (errno %d)", strerror(errno), errno);
        ret = VNET_API_ERROR_SYSCALL_ERROR_1;
        goto error;
    }
    ...
    if (!(ifr.ifr_flags & IFF_UP)) {
        ...
        if (ioctl(fd2, SIOCSIFFLAGS, &ifr) < 0) {
            vlib_log_debug(apm->log_class, "Failed to set the active flag: %s (errno %d)", strerror(errno), errno);
            ret = VNET_API_ERROR_SYSCALL_ERROR_1;
            goto error;
        }
    }
    ...
    if (apif->mode != AF_PACKET_IF_MODE_IP) {
        ...
        apif->hw_if_index = vnet_register_interface(vnm,
                                                    af_packet_device_class.index,
                                                    apif->dev_instance,
                                                    af_packet_ip_device_hw_interface_class.index,
                                                    apif->dev_instance);
    }
    ...
    if (arg->flags & AF_PACKET_IF_FLAGS_FANOUT) apif->is_fanout_enabled = 1;

    apif->is_qdisc_bypass_enabled = (arg->flags & AF_PACKET_IF_FLAGS_QDISC_BYPASS);
    ...
    if (arg->flags & AF_PACKET_IF_FLAGS_CKSUM_GSO) {
        if (apif->host_interface_oflags & AF_PACKET_OFFLOAD_FLAG_TXCKSUM) {
            ...
            caps |= VNET_HW_IF_CAP_TX_IP4_CKSUM | VNET_HW_IF_CAP_TX_TCP_CKSUM | VNET_HW_IF_CAP_TX_UDP_CKSUM;
        }

        if (apif->host_interface_oflags & AF_PACKET_OFFLOAD_FLAG_GSO) {
            apif->is_cksum_gso_enabled = 1;
            caps |= VNET_HW_IF_CAP_TCP_GSO | VNET_HW_IF_CAP_TX_IP4_CKSUM | VNET_HW_IF_CAP_TX_TCP_CKSUM | VNET_HW_IF_CAP_TX_UDP_CKSUM;
        }
    }
    ...
    mhash_set_mem(&apm->if_index_by_host_if_name, host_if_name_dup, &if_index, 0);
    arg->sw_if_index = apif->sw_if_index;
...
error:
    ...
    if (apif) {
        vec_foreach_index (i, apif->fds)
            if (apif->fds[i] != -1) close(apif->fds[i]);
        vec_free(apif->fds);
...
static int af_packet_rx_queue_free(af_packet_if_t *apif, af_packet_queue_t *rx_queue)
{
...
static int af_packet_tx_queue_free(af_packet_if_t *apif, af_packet_queue_t *tx_queue)
{
    tx_queue->fd = -1;
...
static int af_packet_ring_free(af_packet_if_t *apif, af_packet_ring_t *ring)
{
    af_packet_main_t *apm = &af_packet_main;
...
int af_packet_delete_if(u8 *host_if_name)
{
    ...
    if (p == NULL) {
        vlib_log_warn(apm->log_class, "Host interface %s does not exist", host_if_name);
        return VNET_API_ERROR_SYSCALL_ERROR_1;
    }
...
int af_packet_enable_disable_qdisc_bypass(u32 sw_if_index, u8 enable_disable)
{
    ...
    if (hw->dev_class_index != af_packet_device_class.index) return VNET_API_ERROR_INVALID_INTERFACE;
    ...
    vec_foreach_index (i, apif->fds) {
        if (enable_disable) {
             ...
             * removed  */
            if (setsockopt(apif->fds[i], SOL_PACKET, PACKET_QDISC_BYPASS, &opt, sizeof(opt)) < 0) {
...
int af_packet_dump_ifs(af_packet_if_detail_t **out_af_packet_ifs)
{
    ...
    pool_foreach (apif, apm->interfaces) {
        ...
        if (apif->host_if_name) {
            clib_memcpy(af_packet_if->host_if_name,
                        apif->host_if_name,
                        MIN(ARRAY_LEN(af_packet_if->host_if_name) - 1, strlen((const char *) apif->host_if_name)));
        }
    }
...
static clib_error_t *af_packet_init(vlib_main_t *vm)
{
    ...
    vec_validate_aligned(apm->rx_buffers, tm->n_vlib_mains - 1, CLIB_CACHE_LINE_BYTES);
