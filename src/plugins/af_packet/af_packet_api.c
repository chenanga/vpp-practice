...
static void vl_api_af_packet_create_t_handler(vl_api_af_packet_create_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_AF_PACKET_CREATE_REPLY, ({ rmp->sw_if_index = clib_host_to_net_u32(arg->sw_if_index); }));
}

static void vl_api_af_packet_create_v2_t_handler(vl_api_af_packet_create_v2_t *mp)
{
    af_packet_create_if_arg_t           _arg, *arg = &_arg;
    ...
    if (mp->num_rx_queues > 1) arg->num_rxqs = clib_net_to_host_u16(mp->num_rx_queues);

    rv = af_packet_create_if(arg);

    vec_free(arg->host_if_name);
    REPLY_MACRO2(VL_API_AF_PACKET_CREATE_V2_REPLY, ({ rmp->sw_if_index = clib_host_to_net_u32(arg->sw_if_index); }));
}

static void vl_api_af_packet_create_v3_t_handler(vl_api_af_packet_create_v3_t *mp)
{
    af_packet_create_if_arg_t           _arg, *arg = &_arg;
    ...
    STATIC_ASSERT(((int) AF_PACKET_API_FLAG_QDISC_BYPASS == (int) AF_PACKET_IF_FLAGS_QDISC_BYPASS), "af-packet qdisc-bypass api flag mismatch");
    STATIC_ASSERT(((int) AF_PACKET_API_FLAG_CKSUM_GSO == (int) AF_PACKET_IF_FLAGS_CKSUM_GSO), "af-packet checksum/gso offload api flag mismatch");

    STATIC_ASSERT(((int) AF_PACKET_API_FLAG_VERSION_2 == (int) AF_PACKET_IF_FLAGS_VERSION_2), "af-packet version 2 api flag mismatch");

    // Default flags
    arg->flags = clib_net_to_host_u32(mp->flags);

    if (clib_net_to_host_u16(mp->num_rx_queues) > 1) arg->num_rxqs = clib_net_to_host_u16(mp->num_rx_queues);

    if (clib_net_to_host_u16(mp->num_tx_queues) > 1) arg->num_txqs = clib_net_to_host_u16(mp->num_tx_queues);
...
error:
    vec_free(arg->host_if_name);
    REPLY_MACRO2(VL_API_AF_PACKET_CREATE_V3_REPLY, ({ rmp->sw_if_index = clib_host_to_net_u32(arg->sw_if_index); }));
}
...
static void af_packet_send_details(vpe_api_main_t *am, vl_api_registration_t *reg, af_packet_if_detail_t *af_packet_if, u32 context)
{
    ...
    clib_memcpy(mp->host_if_name,
                af_packet_if->host_if_name,
