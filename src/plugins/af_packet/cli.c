...
static clib_error_t *af_packet_create_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "name %s", &arg->host_if_name))
            ...
            ;
        else if (unformat(line_input, "rx-per-block %u", &arg->rx_frames_per_block))
            ;
        else if (unformat(line_input, "tx-per-block %u", &arg->tx_frames_per_block))
            ;
    ...
    vlib_cli_output(vm, "%U\n", format_vnet_sw_if_index_name, vnet_get_main(), arg->sw_if_index);
...
VLIB_CLI_COMMAND(af_packet_create_command, static) = {
    ...
    .short_help =
        "create host-interface [v2] name <ifname> [num-rx-queues <n>] "
...
static clib_error_t *af_packet_set_l4_cksum_offload_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (af_packet_set_l4_cksum_offload(sw_if_index, set) < 0) error = clib_error_return(0, "not an af_packet interface");
...
static clib_error_t *af_packet_enable_disable_qdisc_bypass_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    u8               enable_disable = 0;
    clib_error_t    *error          = NULL;
    ...
    if (af_packet_enable_disable_qdisc_bypass(sw_if_index, enable_disable) < 0) error = clib_error_return(0, "not an af_packet interface");
...
VLIB_CLI_COMMAND(af_packet_enable_disable_qdisc_bypass_command, static) = {
    ...
    .short_help = "set host-interface qdisc-bypass <host-if-name> <enable|disable>",
