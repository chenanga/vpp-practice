...
static u8 *format_arping_trace(u8 *s, va_list *args)
{
    ...
    s = format(s,
               "sw-if-index: %u, opcode: %U, from %U (%U)",
               t->sw_if_index,
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 2 && n_left_to_next >= 2) {
            ...
            if (PREDICT_TRUE(arp0->opcode == clib_host_to_net_u16(ETHERNET_ARP_OPCODE_reply))) {
                aif0 = am->interfaces[sw_if_index0];
                if (PREDICT_TRUE(aif0->address.ip.ip4.as_u32 == arp0->ip4_over_ethernet[0].ip4.as_u32)) {
                    aif0->recv.from4.ip4.as_u32 = arp0->ip4_over_ethernet[0].ip4.as_u32;
                    clib_memcpy_fast(&aif0->recv.from4.mac, &arp0->ip4_over_ethernet[0].mac, 6);
                    aif0->reply_count++;
                }
            }
            if (PREDICT_TRUE(arp1->opcode == clib_host_to_net_u16(ETHERNET_ARP_OPCODE_reply))) {
                aif1 = am->interfaces[sw_if_index1];
                if (PREDICT_TRUE(aif1->address.ip.ip4.as_u32 == arp1->ip4_over_ethernet[0].ip4.as_u32)) {
                    aif1->recv.from4.ip4.as_u32 = arp1->ip4_over_ethernet[0].ip4.as_u32;
                    clib_memcpy_fast(&aif1->recv.from4.mac, &arp1->ip4_over_ethernet[0].mac, 6);
                    aif1->reply_count++;
                }
            }
            ...
            if (PREDICT_FALSE((b0->flags & VLIB_BUFFER_IS_TRACED))) {
                ...
                clib_memcpy_fast(&t->reply.mac, &arp0->ip4_over_ethernet[0].mac, 6);
            }
            if (PREDICT_FALSE((b1->flags & VLIB_BUFFER_IS_TRACED))) {
                ...
                clib_memcpy_fast(&t->reply.mac, &arp1->ip4_over_ethernet[0].mac, 6);
            }
...
VLIB_REGISTER_NODE(arping_input_node) = {
    ...
    .format_trace  = format_arping_trace,
    .type          = VLIB_NODE_TYPE_INTERNAL,
    .n_errors      = ARPING_N_ERROR,
    .error_strings = arping_error_strings,
    .n_next_nodes  = ARPING_N_NEXT,
    .next_nodes =
        {
            [ARPING_NEXT_DROP] = "error-drop",
            [ARPING_NEXT_IO]   = "interface-output",
        },
};
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 2 && n_left_to_next >= 2) {
            ...
            u32                                                            sw_if_index0, sw_if_index1;
            arping_intf_t                                                 *aif0, *aif1;
            icmp6_neighbor_solicitation_or_advertisement_header_t         *sol_adv0, *sol_adv1;
            icmp6_neighbor_discovery_ethernet_link_layer_address_option_t *lladdr0, *lladdr1;
            ...
            lladdr0  = (icmp6_neighbor_discovery_ethernet_link_layer_address_option_t *) (sol_adv0 + 1);

            if (PREDICT_TRUE(sol_adv0->icmp.type == ICMP6_neighbor_advertisement)) {
                aif0 = am->interfaces[sw_if_index0];
                if (PREDICT_TRUE(clib_memcmp(&aif0->address.ip.ip6, &sol_adv0->target_address, sizeof(aif0->address.ip.ip6)) == 0)) {
                    clib_memcpy_fast(&aif0->recv.from6.ip6, &sol_adv0->target_address, sizeof(aif0->recv.from6.ip6));
                    clib_memcpy_fast(&aif0->recv.from6.mac, lladdr0->ethernet_address, 6);
                    aif0->reply_count++;
                }
            }

            sol_adv1 = ip6_next_header(ip61);
            lladdr1  = (icmp6_neighbor_discovery_ethernet_link_layer_address_option_t *) (sol_adv1 + 1);

            if (PREDICT_TRUE(sol_adv1->icmp.type == ICMP6_neighbor_advertisement)) {
                aif1 = am->interfaces[sw_if_index1];
                if (PREDICT_TRUE(clib_memcmp(&aif1->address.ip.ip6, &sol_adv1->target_address, sizeof(aif1->address.ip.ip6)) == 0)) {
                    clib_memcpy_fast(&aif1->recv.from6.ip6, &sol_adv1->target_address, sizeof(aif1->recv.from6.ip6));
                    clib_memcpy_fast(&aif1->recv.from6.mac, lladdr1->ethernet_address, 6);
                    aif1->reply_count++;
                }
            }
            ...
            if (PREDICT_FALSE((b0->flags & VLIB_BUFFER_IS_TRACED))) {
                ...
                clib_memcpy_fast(&t->reply.ip6, &sol_adv0->target_address, sizeof(t->reply.ip6));
            ...
            if (PREDICT_FALSE((b1->flags & VLIB_BUFFER_IS_TRACED))) {
                ...
                clib_memcpy_fast(&t->reply.ip6, &sol_adv1->target_address, sizeof(t->reply.ip6));
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            icmp6_neighbor_discovery_ethernet_link_layer_address_option_t *lladdr0;
...
VLIB_REGISTER_NODE(arping6_input_node) = {
    .name          = "arping6-input",
    .vector_size   = sizeof(u32),
    .format_trace  = format_arping6_trace,
    .type          = VLIB_NODE_TYPE_INTERNAL,
    .n_errors      = ARPING_N_ERROR,
    .error_strings = arping_error_strings,
    .n_next_nodes  = ARPING_N_NEXT,
    .next_nodes =
        {
            [ARPING6_NEXT_DROP] = "error-drop",
            [ARPING6_NEXT_IO]   = "interface-output",
        },
};
...
static clib_error_t *arping_neighbor_advertisement(vlib_main_t *vm, arping_args_t *args)
{
    ...
    while (args->repeat > 0) {
        ...
        if (args->address.version == AF_IP4) {
            if (args->silence == 0) vlib_cli_output(vm, "Sending %u GARP to %U", send_count, format_ip4_address, &args->address.ip.ip4);
            ip4_neighbor_advertise(vm, vnm, args->sw_if_index, vlib_get_thread_index(), &args->address.ip.ip4);
        }
        else {
            if (args->silence == 0)
                vlib_cli_output(vm, "Sending %u Neighbor Advertisement to %U", send_count, format_ip6_address, &args->address.ip.ip6);
            ip6_neighbor_advertise(vm, vnm, args->sw_if_index, vlib_get_thread_index(), &args->address.ip.ip6);
        }
        args->repeat--;
        if ((args->interval > 0.0) && (args->repeat > 0)) vlib_process_suspend(vm, args->interval);
    }
...
static clib_error_t *arping_neighbor_probe_dst(vlib_main_t *vm, arping_args_t *args)
{
    ...
    if (args->address.version == AF_IP4)
        arping_vnet_feature_enable_disable(vm, "arp", "arping-input", args->sw_if_index, 1, 0, 0);
    else
        arping_vnet_feature_enable_disable(vm, "ip6-local", "arping6-input", args->sw_if_index, 1, 0, 0);

    while (args->repeat > 0) {
        send_count++;
        if (args->address.version == AF_IP4) {
            if (args->silence == 0) vlib_cli_output(vm, "Sending %u ARP Request to %U", send_count, format_ip4_address, &args->address.ip.ip4);
            ip4_neighbor_probe_dst(args->sw_if_index, vlib_get_thread_index(), &args->address.ip.ip4);
        }
        else {
            if (args->silence == 0)
                vlib_cli_output(vm, "Sending %u Neighbor Solicitation  to %U", send_count, format_ip6_address, &args->address.ip.ip6);
            ip6_neighbor_probe_dst(args->sw_if_index, vlib_get_thread_index(), &args->address.ip.ip6);
        }
        args->repeat--;
        if ((args->interval > 0.0) && (args->repeat > 0)) vlib_process_suspend(vm, args->interval);
    }
    ...
    if (args->address.version == AF_IP4) {
        clib_memcpy(&args->recv.from4, &aif.recv.from4, sizeof(args->recv.from4));
        arping_vnet_feature_enable_disable(vm, "arp", "arping-input", args->sw_if_index, 0, 0, 0);
    }
    else {
        clib_memcpy(&args->recv.from6, &aif.recv.from6, sizeof(args->recv.from6));
        arping_vnet_feature_enable_disable(vm, "ip6-local", "arping6-input", args->sw_if_index, 0, 0, 0);
    }
...
static clib_error_t *arping_ip_address(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (unformat(input, "gratuitous")) args.is_garp = 1;
    ...
    if (!unformat_user(input, unformat_vnet_sw_interface, vnm, &args.sw_if_index)) {
    ...
    if (args.reply_count) {
        if (args.address.version == AF_IP4)
            vlib_cli_output(vm,
                            "Received %u ARP Replies from %U (%U)",
                            args.reply_count,
                            format_mac_address,
                            &args.recv.from4.mac,
                            format_ip4_address,
