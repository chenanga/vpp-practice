...
/* arping request API */
static int api_arping(vat_main_t *vam)
{
    vl_api_arping_t       *mp;
    ...
    if (unformat(input, "%U", unformat_ip4_address, &args.address.ip.ip4))
        ...
        errmsg(
            "expecting IP4/IP6 address `%U'. Usage: arping [gratuitous] "
    ...
    while (!unformat_eof(input, NULL)) {
        if (unformat(input, "interval")) {
            if (!unformat(input, "%f", &interval)) {
                errmsg("expecting interval (floating point number) got `%U'", format_unformat_error, input);
                return -99;
            }
            ...
            if (!unformat(input, "%u", &args.repeat)) {
                errmsg("expecting repeat count but got `%U'", format_unformat_error, input);
                return -99;
            }
        }
        else {
    ...
    if (!atm->ping_id) atm->ping_id = vl_msg_api_get_msg_index((u8 *) (VL_API_CONTROL_PING_CRC));
...
static void vl_api_arping_reply_t_handler(vl_api_arping_reply_t *mp)
{
...
static int api_arping_acd(vat_main_t *vam)
{
    // NOT YET IMPLEMENTED
    return -99;
}

static void vl_api_arping_acd_reply_t_handler(vl_api_arping_reply_t *mp)
{
    // NOT YET IMPLEMENTED
}
