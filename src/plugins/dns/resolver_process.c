...
static void resolve_event(vlib_main_t *vm, dns_main_t *dm, f64 now, u8 *reply)
{
    ...
    if (pool_is_free_index(dm->entries, pool_index)) {
        vec_free(reply);
        if (0) clib_warning("pool index %d is free", pool_index);
        vlib_node_increment_counter(vm, dns46_reply_node.index, DNS46_REPLY_ERROR_NO_ELT, 1);
        dns_cache_unlock(dm);
        return;
    }
    ...
    if (rv < 0) {
        ...
        if (ep->server_af /* ip6 */) {
            if (0) clib_warning("Server %U failed to resolve '%s'", format_ip6_address, dm->ip6_name_servers + ep->server_rotor, ep->name);
            ...
            ep->server_rotor++;
            ep->server_fails++;
            if (ep->server_rotor >= vec_len(dm->ip6_name_servers)) ep->server_rotor = 0;
            if (0) clib_warning("Try server %U", format_ip6_address, dm->ip6_name_servers + ep->server_rotor);
            vnet_dns_send_dns6_request(vm, dm, ep, dm->ip6_name_servers + ep->server_rotor);
        }
        else {
            if (0) clib_warning("Server %U failed to resolve '%s'", format_ip4_address, dm->ip4_name_servers + ep->server_rotor, ep->name);
            ...
            ep->server_rotor++;
            ep->server_fails++;
            if (ep->server_rotor >= vec_len(dm->ip4_name_servers)) ep->server_rotor = 0;
            if (0) clib_warning("Try server %U", format_ip4_address, dm->ip4_name_servers + ep->server_rotor);
...
reply:
    ...
    if (0) clib_warning("resolving '%s', was %s valid", ep->name, (ep->flags & DNS_CACHE_ENTRY_FLAG_VALID) ? "already" : "not");
    /*
    ...
    if (vec_len(ep->dns_response)) ep->flags |= DNS_CACHE_ENTRY_FLAG_VALID;
    ...
    for (i = 0; i < vec_len(ep->pending_requests); i++) {
        ...
        switch (pr->request_type) {
            case DNS_API_PENDING_NAME_TO_IP: {
                ...
                rmp->_vl_msg_id = clib_host_to_net_u16(VL_API_DNS_RESOLVE_NAME_REPLY + dm->msg_id_base);
                rmp->context    = pr->client_context;
                min_ttl         = ~0;
                rv              = vnet_dns_response_to_reply(ep->dns_response, rmp, &min_ttl);
                if (min_ttl != ~0) ep->expiration_time = now + min_ttl;
            ...
            case DNS_API_PENDING_IP_TO_NAME: {
                ...
                rmp             = vl_msg_api_alloc(sizeof(*rmp));
                rmp->_vl_msg_id = clib_host_to_net_u16(VL_API_DNS_RESOLVE_IP_REPLY + dm->msg_id_base);
                rmp->context    = pr->client_context;
                min_ttl         = ~0;
    ...
    if (remove_count == 0) {
        u32 error_code = entry_was_valid ? DNS46_REPLY_ERROR_MULTIPLE_REPLY : DNS46_REPLY_ERROR_NO_UNRESOLVED_ENTRY;
    ...
    switch (rcode) {
        ...
        case DNS_RCODE_REFUSED:
            if (ep->server_af == 0)
                clib_warning("name server %U can't resolve '%s'", format_ip4_address, dm->ip4_name_servers + ep->server_rotor, ep->name);
            else
                clib_warning("name server %U can't resolve '%s'", format_ip6_address, dm->ip6_name_servers + ep->server_rotor, ep->name);
            /* FALLTHROUGH */
...
static uword dns_resolver_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    while (1) {
        ...
        if (vec_len(dm->unresolved_entries) == 0) timeout = 1000.0;
    }
    return 0; /* or not */
}
...
void vnet_dns_create_resolver_process(vlib_main_t *vm, dns_main_t *dm)
{
    ...
    if (dm->resolver_process_node_index > 0) return;

    /* No, create it now and make a note of the node index */
    dm->resolver_process_node_index = vlib_process_create(vm, "dns-resolver-process", dns_resolver_process, 16 /* log2_n_stack_bytes */);
}
