...
static void vl_api_dns_resolve_name_reply_t_handler(vl_api_dns_resolve_name_reply_t *mp)
{
    vat_main_t *vam    = dns_test_main.vat_main;
    i32         retval = (i32) clib_net_to_host_u32(mp->retval);
    if (retval == 0) {
        if (mp->ip4_set) clib_warning("resolved: %U", format_ip4_address, mp->ip4_address);
        if (mp->ip6_set) clib_warning("resolved: %U", format_ip6_address, mp->ip6_address);
    }
    if (vam->async_mode)
...
static void vl_api_dns_resolve_ip_reply_t_handler(vl_api_dns_resolve_ip_reply_t *mp)
{
    vat_main_t *vam    = dns_test_main.vat_main;
    i32         retval = (i32) clib_net_to_host_u32(mp->retval);
    if (retval == 0) clib_warning("resolved: %s", mp->name);
    if (vam->async_mode)
        vam->async_errors += (retval < 0);
    else {
        vam->retval       = retval;
...
static int api_dns_enable_disable(vat_main_t *vam)
{
...
static int api_dns_resolve_name(vat_main_t *vam)
{
...
static int api_dns_resolve_ip(vat_main_t *vam)
{
    unformat_input_t        *line_input = vam->input;
...
static int api_dns_name_server_add_del(vat_main_t *vam)
{
    unformat_input_t                 *i = vam->input;
