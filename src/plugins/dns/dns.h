...
static inline void dns_cache_lock(dns_main_t *dm, int tag)
{
    if (dm->cache_lock) {
        ASSERT(tag);
...
static inline void dns_cache_unlock(dns_main_t *dm)
{
    if (dm->cache_lock) {
...
extern int dns_resolve_name(u8 *name, dns_cache_entry_t **ep, dns_pending_request_t *t0, dns_resolve_name_t *rn);
