...
static u8 *format_dns46_request_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "DNS46_REPLY: pool index %d, disposition  %d", t->pool_index, t->disposition);
    return s;
}
	  ...
	  vlib_validate_buffer_enqueue_x2 (vm, node, next_index,
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_TRUE(vec_len(name0) > 5)) {
                ...
                if (!memcmp(aptr0, ".arpa", 5)) t0->request_type = DNS_PEER_PENDING_IP_TO_NAME;
            }
            ...
            t0->name         = name0;
            if (is_ip6)
                clib_memcpy_fast(t0->dst_address, ip60->src_address.as_u8, sizeof(ip6_address_t));
            else
                clib_memcpy_fast(t0->dst_address, ip40->src_address.as_u8, sizeof(ip4_address_t));
        ...
        done0:
            ...
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                dns46_request_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
