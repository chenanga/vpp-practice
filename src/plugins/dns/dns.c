 ...
 * its sanity back
 */
static_always_inline void dns_terminate_c_string(u8 **v)
{
    vec_add1(*v, 0);
    vec_dec_len(*v, 1);
...
static int dns_cache_clear(dns_main_t *dm)
{
    ...
    if (dm->is_enabled == 0) return VNET_API_ERROR_NAME_RESOLUTION_NOT_ENABLED;
...
static int dns_enable_disable(vlib_main_t *vm, dns_main_t *dm, int is_enable)
{
    ...
    if (is_enable) {
        if (vec_len(dm->ip4_name_servers) == 0 && (vec_len(dm->ip6_name_servers) == 0)) return VNET_API_ERROR_NO_NAME_SERVERS;

        if (dm->udp_ports_registered == 0) {
            udp_register_dst_port(vm, UDP_DST_PORT_dns_reply, dns46_reply_node.index, 1 /* is_ip4 */);

            udp_register_dst_port(vm, UDP_DST_PORT_dns_reply6, dns46_reply_node.index, 0 /* is_ip4 */);

            udp_register_dst_port(vm, UDP_DST_PORT_dns, dns4_request_node.index, 1 /* is_ip4 */);

            udp_register_dst_port(vm, UDP_DST_PORT_dns6, dns6_request_node.index, 0 /* is_ip4 */);

            dm->udp_ports_registered = 1;
        }

        if (dm->cache_entry_by_name == 0) {
            if (n_vlib_mains > 1) clib_spinlock_init(&dm->cache_lock);
...
static int dns6_name_server_add_del(dns_main_t *dm, u8 *server_address_as_u8, int is_add)
{
    ...
    if (is_add) {
        ...
        for (i = 0; i < vec_len(dm->ip6_name_servers); i++) {
            if (!memcmp(dm->ip6_name_servers + i, server_address_as_u8, sizeof(ip6_address_t))) return 0;
        }
        ...
        for (i = 0; i < vec_len(dm->ip6_name_servers); i++) {
            if (!memcmp(dm->ip6_name_servers + i, server_address_as_u8, sizeof(ip6_address_t))) {
...
static int dns4_name_server_add_del(dns_main_t *dm, u8 *server_address_as_u8, int is_add)
{
    ...
    if (is_add) {
        ...
        for (i = 0; i < vec_len(dm->ip4_name_servers); i++) {
            if (!memcmp(dm->ip4_name_servers + i, server_address_as_u8, sizeof(ip4_address_t))) return 0;
        }
        ...
        for (i = 0; i < vec_len(dm->ip4_name_servers); i++) {
            if (!memcmp(dm->ip4_name_servers + i, server_address_as_u8, sizeof(ip4_address_t))) {
...
void vnet_dns_send_dns4_request(vlib_main_t *vm, dns_main_t *dm, dns_cache_entry_t *ep, ip4_address_t *server)
{
    ...
    if (!ip4_sas(0 /* default VRF for now */, ~0, server, &src_address)) return;

    /* Go get a buffer */
    if (vlib_buffer_alloc(vm, &bi, 1) != 1) return;

    b                                          = vlib_get_buffer(vm, bi);
    b->current_length                          = sizeof(ip4_header_t) + sizeof(udp_header_t) + vec_len(ep->dns_request);
    ...
    udp->length   = clib_host_to_net_u16(sizeof(udp_header_t) + vec_len(ep->dns_request));
    udp->checksum = 0;
...
void vnet_dns_send_dns6_request(vlib_main_t *vm, dns_main_t *dm, dns_cache_entry_t *ep, ip6_address_t *server)
{
    ...
    if (!ip6_sas(0 /* default VRF for now */, ~0, server, &src_address)) return;

    /* Go get a buffer */
    if (vlib_buffer_alloc(vm, &bi, 1) != 1) return;

    b                                          = vlib_get_buffer(vm, bi);
    b->current_length                          = sizeof(ip6_header_t) + sizeof(udp_header_t) + vec_len(ep->dns_request);
    b->total_length_not_including_first_buffer = 0;
    b->flags                                   = VLIB_BUFFER_TOTAL_LENGTH_VALID | VNET_BUFFER_F_LOCALLY_ORIGINATED;
    ...
    ip->payload_length = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, b) - sizeof(ip6_header_t));
    ip->hop_limit      = 255;
    ip->protocol       = IP_PROTOCOL_UDP;
...
u8 *name_to_labels(u8 *name)
{
    ...
    while (i < vec_len(rv)) {
        if (rv[i] == '.') {
            ...
            if ((i - last_label_index) > 63) clib_warning("stupid name, label length %d", i - last_label_index);
            last_label_index = i;
            rv[i]            = 0;
        }
        i++;
...
u8 *vnet_dns_labels_to_name(u8 *label, u8 *full_text, u8 **parse_from_here)
{
    ...
    *parse_from_here = 0;
    ...
    while (len) {
        ...
        len = *label++;
        if (len) vec_add1(reply, '.');
    }
    if (*parse_from_here == 0) *parse_from_here = label;
    return reply;
}
...
void vnet_send_dns_request(vlib_main_t *vm, dns_main_t *dm, dns_cache_entry_t *ep)
{
    ...
    if (ep->flags & DNS_CACHE_ENTRY_FLAG_VALID || ep->server_fails > 1) return;
    ...
    if (ep->retry_count++ < DNS_RETRIES_PER_SERVER) {
        if (ep->server_af == 1 /* ip6 */) {
            if (vec_len(dm->ip6_name_servers)) {
                vnet_dns_send_dns6_request(vm, dm, ep, dm->ip6_name_servers + ep->server_rotor);
                goto out;
            }
            else
                ep->server_af = 0;
        ...
        if (vec_len(dm->ip4_name_servers)) {
            vnet_dns_send_dns4_request(vm, dm, ep, dm->ip4_name_servers + ep->server_rotor);
            goto out;
        }
    }
...
out:
    ...
    vlib_process_signal_event_mt(vm, dm->resolver_process_node_index, DNS_RESOLVER_EVENT_PENDING, 0);
}
...
int vnet_dns_delete_entry_by_index_nolock(dns_main_t *dm, u32 index)
{
    ...
    if (pool_is_free_index(dm->entries, index)) return VNET_API_ERROR_NO_SUCH_ENTRY;
    ...
    if (!(ep->flags & DNS_CACHE_ENTRY_FLAG_VALID)) {
        ...
        clib_warning("pool elt %d supposedly pending, but not found...", index);
    }
...
static int delete_random_entry(dns_main_t *dm)
{
    ...
    if (dm->is_enabled == 0) return VNET_API_ERROR_NAME_RESOLUTION_NOT_ENABLED;
         ...
         */
#ifdef __COVERITY__
    if (pool_elts(dm->entries) == 0) return VNET_API_ERROR_UNSPECIFIED;
#endif
    ...
    for (i = 0; i < limit; i++) {
        ...
        if (!pool_is_free_index(dm->entries, victim_index)) {
            ...
            if ((ep->flags & DNS_CACHE_ENTRY_FLAG_VALID) && ((ep->flags & DNS_CACHE_ENTRY_FLAG_STATIC) == 0)) {
...
int vnet_dns_resolve_name(vlib_main_t *vm, dns_main_t *dm, u8 *name, dns_pending_request_t *t, dns_cache_entry_t **retp)
{
    ...
    *retp = 0;
    ...
    if (name[0] == 0) return VNET_API_ERROR_INVALID_VALUE;
...
search_again:
    ...
    if (p) {
        ...
        if (ep->flags & DNS_CACHE_ENTRY_FLAG_VALID) {
            ...
            if (((ep->flags & DNS_CACHE_ENTRY_FLAG_STATIC) == 0) && (now > ep->expiration_time)) {
                int  i;
                ...
                for (i = 0; i < vec_len(indices_to_delete); i++) {
                    ...
                    if (0) {
                        ep = pool_elt_at_index(dm->entries, indices_to_delete[i]);
                        clib_warning("Re-resolve %s", ep->name);
                    }

                    vnet_dns_delete_entry_by_index_nolock(dm, indices_to_delete[i]);
                }
...
re_resolve:
    ...
    if (t->request_type == DNS_API_PENDING_NAME_TO_IP || t->request_type == DNS_API_PENDING_IP_TO_NAME) {
...
found_last_request:
    ...
    /* Save the response */
    if (ep->dns_response) vec_free(ep->dns_response);
    ...
    hash_set_mem(dm->cache_entry_by_name, next_ep->name, next_ep - dm->entries);
...
int vnet_dns_response_to_reply(u8 *response, dns_resolve_name_t *rn, u32 *min_ttlp)
{
    ...
    /* No answers? Loser... */
    if (clib_net_to_host_u16(h->anscount) < 1) return VNET_API_ERROR_NAME_SERVER_NO_ADDRESSES;
    ...
    for (i = 0; i < limit; i++) {
        ...
        switch (clib_net_to_host_u16(rr->type)) {
            case DNS_TYPE_A:
                ...
                addr_set += 1;
                if (min_ttlp && *min_ttlp > ttl) *min_ttlp = ttl;
                break;
            case DNS_TYPE_AAAA:
    ...
    if (addr_set == 0) return VNET_API_ERROR_NAME_SERVER_NO_ADDRESSES;
    return 0;
}
...
int vnet_dns_response_to_name(u8 *response, vl_api_dns_resolve_ip_reply_t *rmp, u32 *min_ttlp)
{
    ...
    if (name_set == 0) return VNET_API_ERROR_NAME_SERVER_NO_SUCH_NAME;
    return 0;
}
...
uword unformat_dns_reply(unformat_input_t *input, va_list *args)
{
    ...
    if (unformat(input, "%v", &name)) name_set = 1;

    if (unformat(input, "%U", unformat_ip4_address, &a4)) {
        a4_set = 1;
        if (unformat(input, "%U", unformat_ip6_address, &a6)) a6_set = 1;
    }

    if (unformat(input, "%U", unformat_ip6_address, &a6)) {
        a6_set = 1;
        if (unformat(input, "%U", unformat_ip4_address, &a6)) a4_set = 1;
    }
...
u8 *format_dns_reply_data(u8 *s, va_list *args)
{
    ...
    switch (rrtype_host_byte_order) {
        case DNS_TYPE_A:
            if (verbose > 1) {
                s = format(s, "A: ttl %d %U\n", clib_net_to_host_u32(rr->ttl), format_ip4_address, rr->rdata);
            }
            else {
                if (*print_ip4) s = format(s, "%U [%u] ", format_ip4_address, rr->rdata, clib_net_to_host_u32(rr->ttl));
                if (verbose == 0) *print_ip4 = 0;
            }
            pos += sizeof(*rr) + 4;
            break;
        ...
        case DNS_TYPE_AAAA:
            if (verbose > 1) {
                s = format(s, "AAAA: ttl %d %U\n", clib_net_to_host_u32(rr->ttl), format_ip6_address, rr->rdata);
            }
            else {
                if (*print_ip6) s = format(s, "%U [%u] ", format_ip6_address, rr->rdata, clib_net_to_host_u32(rr->ttl));
                if (verbose == 0) *print_ip6 = 0;
            }
            pos += sizeof(*rr) + 16;
            break;
        ...
        case DNS_TYPE_NAMESERVER:
            if (verbose > 1) {
                ...
                while (len) {
                    ...
                    if (pos2[0] == 0xC0) pos2 = reply + pos2[1];
        ...
        case DNS_TYPE_MAIL_EXCHANGE:
            if (verbose > 1) {
                ...
                s = format(s, "Mail Exchange: Preference %d ", (u32) clib_net_to_host_u16(*tp));
        ...
        default:
            if (verbose > 1)
                s = format(s, "type %d: len %d\n", (int) clib_net_to_host_u16(rr->type), sizeof(*rr) + clib_net_to_host_u16(rr->rdlength));
            pos += sizeof(*rr) + clib_net_to_host_u16(rr->rdlength);
            break;
    }
...
u8 *format_dns_reply(u8 *s, va_list *args)
{
    ...
    if (h->qdcount) {
        if (verbose > 1) s = format(s, "  Queries:\n");
        for (i = 0; i < clib_net_to_host_u16(h->qdcount); i++) {
    ...
    if (h->anscount) {
        if (verbose > 1) s = format(s, "  Replies:\n");

        for (i = 0; i < clib_net_to_host_u16(h->anscount); i++) {
            /* curpos is a value-result parm */
            s = format(s, "%U", format_dns_reply_data, reply_as_u8, &curpos, verbose, &print_ip4, &print_ip6);
        }
    }
    return s;
}
        ...
        if (verbose == 3) {
            ...
            if (ep->flags & DNS_CACHE_ENTRY_FLAG_VALID) {
                ...
                if (verbose < 2 && ep->flags & DNS_CACHE_ENTRY_FLAG_CNAME)
                    ...
                    s = format(s, "%s%s -> %U", ss, ep->name, format_dns_reply, ep->dns_response, verbose);
                if (!(ep->flags & DNS_CACHE_ENTRY_FLAG_STATIC)) {
                ...
                ASSERT(ep->dns_request);
                s = format(s, "[P] %U", format_dns_reply, ep->dns_request, verbose);
            }
            vec_add1(s, '\n');
        }
        return s;
        ...
        pool_foreach (ep, dm->entries) {
            if (ep->flags & DNS_CACHE_ENTRY_FLAG_VALID) {
                ...
                if (!(ep->flags & DNS_CACHE_ENTRY_FLAG_STATIC)) {
                    ...
                    if (verbose > 2) s = format(s, "    %d client notifications pending\n", vec_len(ep->pending_requests));
                }
            }
            else {
                ASSERT(ep->dns_request);
...
static clib_error_t *show_dns_servers_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if ((vec_len(dm->ip4_name_servers) + vec_len(dm->ip6_name_servers)) == 0) return clib_error_return(0, "No name servers configured...");

    if (vec_len(dm->ip4_name_servers)) {
        vlib_cli_output(vm, "ip4 name servers:");
        for (i = 0; i < vec_len(dm->ip4_name_servers); i++)
            vlib_cli_output(vm, "%U", format_ip4_address, dm->ip4_name_servers + i);
    }
    if (vec_len(dm->ip6_name_servers)) {
        vlib_cli_output(vm, "ip6 name servers:");
        for (i = 0; i < vec_len(dm->ip6_name_servers); i++)
            vlib_cli_output(vm, "%U", format_ip6_address, dm->ip4_name_servers + i);
    }
    return 0;
}
...
static clib_error_t *dns_cache_add_del_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (unformat(input, "del")) is_add = 0;
    if (unformat(input, "clear")) is_clear = 1;

    if (is_add == -1 && is_clear == -1) return clib_error_return(0, "add / del / clear required...");
    ...
    if (is_add == 0) {
        if (unformat(input, "%v", &name)) {
            ...
            switch (rv) {
                ...
                default:
                    error = clib_error_return(0, "dns_delete_by_name returned %d", rv);
                    vec_free(name);
                    return error;
            }
        }
    ...
    if (unformat(input, "%U", unformat_dns_reply, &dns_reply_data, &name)) {
        ...
        switch (rv) {
            ...
            default:
                return clib_error_return(0, "dns_add_static_entry returned %d", rv);
        }
    }
...
/* www.weatherlink.com */
static u8 dns_reply_data_initializer[] = {
    0x00, 0x00, 0x81, 0x80, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x03, 0x77, 0x77, 0x77, 0x0b, 0x77, 0x65, 0x61,
    0x74, 0x68, 0x65, 0x72, 0x6c, 0x69, 0x6e, 0x6b, 0x03, 0x63, 0x6f, 0x6d, 0x00, 0x00, 0xff, 0x00, 0x01, 0xc0, 0x0c, 0x00,
    0x05, 0x00, 0x01, 0x00, 0x00, 0x0c, 0x9e, 0x00, 0x1f, 0x0e, 0x64, 0x33, 0x6b, 0x72, 0x30, 0x67, 0x75, 0x62, 0x61, 0x31,
    0x64, 0x76, 0x77, 0x66, 0x0a, 0x63, 0x6c, 0x6f, 0x75, 0x64, 0x66, 0x72, 0x6f, 0x6e, 0x74, 0x03, 0x6e, 0x65, 0x74, 0x00,
};
...
static clib_error_t *test_dns_fmt_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    memcpy(dns_reply_data, dns_reply_data_initializer, ARRAY_LEN(dns_reply_data_initializer));
...
static clib_error_t *test_dns_unfmt_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (reply_set == 0) return clib_error_return(0, "dns data not set...");
...
void vnet_send_dns4_reply(vlib_main_t *vm, dns_main_t *dm, dns_pending_request_t *pr, dns_cache_entry_t *ep, vlib_buffer_t *b0)
{
    ...
    if (b0->flags & VLIB_BUFFER_NEXT_PRESENT) vlib_buffer_free_one(vm, b0->next_buffer);
     ...
     */
    b0->flags |= (VNET_BUFFER_F_LOCALLY_ORIGINATED | VLIB_BUFFER_TOTAL_LENGTH_VALID);
    vnet_buffer(b0)->sw_if_index[VLIB_RX] = 0; /* "local0" */
    vnet_buffer(b0)->sw_if_index[VLIB_TX] = 0; /* default VRF for now */

    if (!ip4_sas(0 /* default VRF for now */, ~0, (const ip4_address_t *) &pr->dst_address, &src_address)) return;
    ...
    if (is_fail) tmp |= DNS_RCODE_NAME_ERROR;
