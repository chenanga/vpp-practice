...
static void vl_api_idpf_create_t_handler(vl_api_idpf_create_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_IDPF_CREATE_REPLY, ({ rmp->sw_if_index = ntohl(args.sw_if_index); }));
}

static void vl_api_idpf_delete_t_handler(vl_api_idpf_delete_t *mp)
{
    vlib_main_t                *vm  = vlib_get_main();
...
static clib_error_t *idpf_plugin_api_hookup(vlib_main_t *vm)
{
