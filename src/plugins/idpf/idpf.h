...
#define IDPF_CTLQ_DESC_UNUSED(R) (u16)((((R)->next_to_clean > (R)->next_to_use) ? 0 : (R)->ring_size) + (R)->next_to_clean - (R)->next_to_use - 1)

#define IDPF_GET_PTYPE_SIZE(p) (sizeof(virtchnl2_ptype_t) + (((p)->proto_id_count ? ((p)->proto_id_count - 1) : 0) * sizeof((p)->proto_id[0])))

/* log configuration */
extern vlib_log_class_registration_t idpf_log;
extern vlib_log_class_registration_t idpf_stats_log;

#define idpf_log_err(dev, f, ...) vlib_log(VLIB_LOG_LEVEL_ERR, idpf_log.class, "%U: " f, format_vlib_pci_addr, &dev->pci_addr, ##__VA_ARGS__)

#define idpf_log_warn(dev, f, ...) vlib_log(VLIB_LOG_LEVEL_WARNING, idpf_log.class, "%U: " f, format_vlib_pci_addr, &dev->pci_addr, ##__VA_ARGS__)

#define idpf_log_debug(dev, f, ...) vlib_log(VLIB_LOG_LEVEL_DEBUG, idpf_log.class, "%U: " f, format_vlib_pci_addr, &dev->pci_addr, ##__VA_ARGS__)

#define idpf_stats_log_debug(dev, f, ...) \
    vlib_log(VLIB_LOG_LEVEL_DEBUG, idpf_stats_log.class, "%U: " f, format_vlib_pci_addr, &dev->pci_addr, ##__VA_ARGS__)
...
#define LIST_FOR_EACH_ENTRY_SAFE(pos, temp, head, entry_type, list) LIST_FOREACH(pos, head, list)
#endif

#ifndef LIST_FOR_EACH_ENTRY
#define LIST_FOR_EACH_ENTRY(pos, head, entry_type, list) LIST_FOREACH(pos, head, list)
#endif
...
static inline void clear_cmd(idpf_device_t *id)
{
...
static_always_inline idpf_device_t *idpf_get_device(u32 dev_instance)
{
    return pool_elt_at_index(idpf_main.devices, dev_instance)[0];
}

static inline void idpf_reg_write(idpf_device_t *id, u32 addr, u32 val)
{
    *(volatile u32 *) ((u8 *) id->bar0 + addr) = val;
}

static inline u32 idpf_reg_read(idpf_device_t *id, u32 addr)
{
    u32 val = *(volatile u32 *) (id->bar0 + addr);
    return val;
}

static inline void idpf_reg_flush(idpf_device_t *id)
{
...
int   idpf_ctlq_add(vlib_main_t *vm, idpf_device_t *id, idpf_ctlq_create_info_t *qinfo, struct idpf_ctlq_info **cq);
