 ...
 */
static int idpf_ctlq_alloc_desc_ring(vlib_main_t *vm, idpf_device_t *id, struct idpf_ctlq_info *cq)
{
    size_t size = cq->ring_size * sizeof(idpf_ctlq_desc_t);

    /* Fixme: alloc dma va */
    cq->desc_ring.va = idpf_alloc_dma_mem(vm, id, &cq->desc_ring, size);
    if (!cq->desc_ring.va) return IDPF_ERR_NO_MEMORY;
 ...
 */
static int idpf_ctlq_alloc_bufs(vlib_main_t *vm, idpf_device_t *id, struct idpf_ctlq_info *cq)
{
    int i = 0;
    u16 len;

    /* Do not allocate DMA buffers for transmit queues */
    if (cq->cq_type == IDPF_CTLQ_TYPE_MAILBOX_TX) return IDPF_SUCCESS;
    ...
    if (!cq->bi.rx_buff) return IDPF_ERR_NO_MEMORY;
    ...
    for (i = 0; i < cq->ring_size - 1; i++) {
        ...
        cq->bi.rx_buff[i] = (idpf_dma_mem_t *) clib_mem_alloc(num * sizeof(idpf_dma_mem_t));
        if (!cq->bi.rx_buff[i]) goto unwind_alloc_cq_bufs;
 ...
 * and de-allocated
 */
static void idpf_ctlq_free_desc_ring(idpf_device_t *id, struct idpf_ctlq_info *cq)
{
 ...
 */
static void idpf_ctlq_free_bufs(idpf_device_t *id, struct idpf_ctlq_info *cq)
{
    void *bi;
 ...
 */
int idpf_ctlq_alloc_ring_res(vlib_main_t *vm, idpf_device_t *id, struct idpf_ctlq_info *cq)
{
    int ret_code;

    /* verify input for valid configuration */
    if (!cq->ring_size || !cq->buf_size) return IDPF_ERR_CFG;
    ...
    if (ret_code) goto idpf_init_cq_free_ring;
 ...
 */
static void idpf_ctlq_setup_regs(struct idpf_ctlq_info *cq, idpf_ctlq_create_info_t *q_create_info)
{
 ...
 */
static void idpf_ctlq_init_regs(vlib_main_t *vm, idpf_device_t *id, struct idpf_ctlq_info *cq, bool is_rxq)
{
    /* Update tail to post pre-allocated buffers for rx queues */
    if (is_rxq) idpf_reg_write(id, cq->reg.tail, (u32) (cq->ring_size - 1));
 ...
 */
static void idpf_ctlq_shutdown(idpf_device_t *id, struct idpf_ctlq_info *cq)
{
    clib_spinlock_init(&cq->cq_lock);

    if (!cq->ring_size) goto shutdown_sq_out;
 ...
 */
int idpf_ctlq_add(vlib_main_t *vm, idpf_device_t *id, idpf_ctlq_create_info_t *qinfo, struct idpf_ctlq_info **cq_out)
{
    bool is_rxq = false;
    int  status = IDPF_SUCCESS;

    if (!qinfo->len || !qinfo->buf_size || qinfo->len > IDPF_CTLQ_MAX_RING_SIZE || qinfo->buf_size > IDPF_CTLQ_MAX_BUF_LEN) return IDPF_ERR_CFG;

    /* Fixme: memory allocation */
    *cq_out = vlib_physmem_alloc_aligned_on_numa(vm, sizeof(struct idpf_ctlq_info), CLIB_CACHE_LINE_BYTES, id->numa_node);
    if (!(*cq_out)) return IDPF_ERR_NO_MEMORY;
    ...
    if (is_rxq) {
        ...
        (*cq_out)->bi.tx_msg = (idpf_ctlq_msg_t **) clib_mem_alloc(qinfo->len * sizeof(idpf_ctlq_msg_t *));
        if (!(*cq_out)->bi.tx_msg) {
            status = IDPF_ERR_NO_MEMORY;
 ...
 * APIs.
 */
int idpf_ctlq_init(vlib_main_t *vm, idpf_device_t *id, u8 num_q, idpf_ctlq_create_info_t *q_info)
{
    ...
    for (i = 0; i < num_q; i++) {
        ...
        if (ret_code) goto init_destroy_qs;
    }

    return ret_code;

init_destroy_qs:
    LIST_FOR_EACH_ENTRY_SAFE(cq, NULL, &id->cq_list_head, struct idpf_ctlq_info, cq_list)
    {
 ...
 */
int idpf_ctlq_send(idpf_device_t *id, struct idpf_ctlq_info *cq, u16 num_q_msg, idpf_ctlq_msg_t q_msg[])
{
    ...
    if (!cq || !cq->ring_size) return -ENOBUFS;
    ...
    for (i = 0; i < num_q_msg; i++) {
        ...
        desc->flags = (msg->host_id & IDPF_HOST_ID_MASK) << IDPF_CTLQ_FLAG_HOST_ID_S;
        if (msg->data_len) {
            ...
            clib_memcpy(&desc->params, msg->ctx.indirect.context, IDPF_INDIRECT_CTX_SIZE);
        }
        else {
        ...
        (cq->next_to_use)++;
        if (cq->next_to_use == cq->ring_size) cq->next_to_use = 0;
    }
 ...
 */
int idpf_ctlq_clean_sq(struct idpf_ctlq_info *cq, u16 *clean_count, idpf_ctlq_msg_t *msg_status[])
{
    idpf_ctlq_desc_t *desc;
    ...
    if (!cq || !cq->ring_size) return IDPF_ERR_CTLQ_EMPTY;

    if (*clean_count == 0) return IDPF_SUCCESS;
    if (*clean_count > cq->ring_size) return IDPF_ERR_PARAM;
    ...
    for (i = 0; i < num_to_clean; i++) {
        ...
        if (!(desc->flags & IDPF_CTLQ_FLAG_DD)) break;
 ...
 */
int idpf_ctlq_post_rx_buffs(idpf_device_t *id, struct idpf_ctlq_info *cq, u16 *buff_count, idpf_dma_mem_t **buffs)
{
    idpf_ctlq_desc_t *desc;
    ...
    if (*buff_count > cq->ring_size) return IDPF_ERR_PARAM;

    if (*buff_count > 0) buffs_avail = true;

    clib_spinlock_lock(&cq->cq_lock);

    if (tbp >= cq->ring_size) tbp = 0;

    if (tbp == cq->next_to_clean) /* Nothing to do */
        goto post_buffs_out;
    ...
    while (ntp != cq->next_to_clean) {
        ...
        if (cq->bi.rx_buff[ntp]) goto fill_desc;
        if (!buffs_avail) {
            ...
            if (tbp == cq->next_to_clean) goto post_buffs_out;
        }
        else {
            ...
            if (i >= *buff_count) buffs_avail = false;
        }
    ...
    fill_desc:
        ...
        desc->params.indirect.addr_high = IDPF_HI_DWORD(cq->bi.rx_buff[ntp]->pa);
        desc->params.indirect.addr_low  = IDPF_LO_DWORD(cq->bi.rx_buff[ntp]->pa);

        ntp++;
        if (ntp == cq->ring_size) ntp = 0;
    }
 ...
 * to free buffers
 */
int idpf_ctlq_recv(struct idpf_ctlq_info *cq, u16 *num_q_msg, idpf_ctlq_msg_t *q_msg)
{
    ...
    for (i = 0; i < num_to_clean; i++) {
        ...
        if (!(flags & IDPF_CTLQ_FLAG_DD)) break;

        ret_val = desc->ret_val;

        q_msg[i].vmvf_type = (flags & (IDPF_CTLQ_FLAG_FTYPE_VM | IDPF_CTLQ_FLAG_FTYPE_PF)) >> IDPF_CTLQ_FLAG_FTYPE_S;

        if (flags & IDPF_CTLQ_FLAG_ERR) ret_code = IDPF_ERR_CTLQ_ERROR;
        ...
        if (desc->datalen) {
            clib_memcpy_fast(q_msg[i].ctx.indirect.context, &desc->params.indirect, IDPF_INDIRECT_CTX_SIZE);
            ...
            clib_memcpy_fast(q_msg[i].ctx.direct, desc->params.raw, IDPF_DIRECT_CTX_SIZE);
        }
    ...
    *num_q_msg = i;
    if (*num_q_msg == 0) ret_code = -ENOMSG;
