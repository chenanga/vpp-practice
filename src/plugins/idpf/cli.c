...
static clib_error_t *idpf_create_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
VLIB_CLI_COMMAND(idpf_create_command, static) = {
    ...
    .short_help =
        "create interface idpf <pci-address> "
        "[vport <size>] [rx-single <size>] [tx-single <size>]",
    .function = idpf_create_command_fn,
};

static clib_error_t *idpf_delete_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t     _line_input, *line_input = &_line_input;
    u32                  sw_if_index = ~0;
    ...
    if (hw == NULL || idpf_device_class.index != hw->dev_class_index) return clib_error_return(0, "not an IDPF interface");

    vlib_process_signal_event(vm, idpf_process_node.index, IDPF_PROCESS_EVENT_DELETE_IF, hw->dev_instance);
...
clib_error_t *idpf_cli_init(vlib_main_t *vm)
{
    return 0;
}
