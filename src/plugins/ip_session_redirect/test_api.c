...
static int api_ip_session_redirect_add_parse(vat_main_t        *vam,
                                             u32               *table_index,
                                             u32               *opaque_index,
                                             dpo_proto_t       *proto,
                                             int               *is_punt,
    ...
    while (unformat_check_input(vam->input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(vam->input, "punt"))
            *is_punt = 1;
            ...
            ;
        else if (unformat(vam->input, "match %U", unformat_classify_match, cm, match, *table_index))
            ;
        else if (unformat(vam->input, "via %U", unformat_fib_route_path, &path, proto))
            vec_add1(*paths, path);
        else {
...
static int api_ip_session_redirect_add(vat_main_t *vam)
{
    ...
    ret = api_ip_session_redirect_add_parse(vam, &table_index, &opaque_index, &proto, &is_punt, &match, &paths);
    if (ret) goto err;
...
static int api_ip_session_redirect_add_v2(vat_main_t *vam)
{
...
static int api_ip_session_redirect_del(vat_main_t *vam)
{
    vnet_classify_main_t             *cm = &vnet_classify_main;
