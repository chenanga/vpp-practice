...
static int ip_session_redirect_stack(ip_session_redirect_t *ipr)
{
    dpo_id_t dpo = DPO_INVALID;

    fib_path_list_contribute_forwarding(ipr->pl,
                                        ipr->payload_type,
                                        fib_path_list_is_popular(ipr->pl) ? FIB_PATH_LIST_FWD_FLAG_NONE : FIB_PATH_LIST_FWD_FLAG_COLLAPSE,
                                        &dpo);
    dpo_stack_from_node(ipr->parent_node_index, &ipr->dpo, &dpo);
    dpo_reset(&dpo);

    /* update session with new next_index */
    return vnet_classify_add_del_session(&vnet_classify_main,
                                         ipr->table_index,
                                         ipr->match_and_table_index,
                                         ipr->dpo.dpoi_next_node /* hit_next_index */,
                                         ipr->opaque_index,
                                         0 /* advance */,
                                         CLASSIFY_ACTION_SET_METADATA,
                                         ipr->dpo.dpoi_index /* metadata */,
                                         1 /* is_add */);
}

static ip_session_redirect_t *ip_session_redirect_find(ip_session_redirect_main_t *im, u32 table_index, const u8 *match)
{
    ...
    uword *p = hash_get_mem(im->session_by_match_and_table_index, match_and_table_index);
    vec_free(match_and_table_index);
    if (!p) return 0;
    return pool_elt_at_index(im->pool, p[0]);
}

__clib_export int ip_session_redirect_add(vlib_main_t            *vm,
                                          u32                     table_index,
                                          u32                     opaque_index,
                                          dpo_proto_t             proto,
                                          int                     is_punt,
                                          ...
                                          const fib_route_path_t *rpaths)
{
    ...
    if (ipr) {
        ...
        hash_set_mem(im->session_by_match_and_table_index, ipr->match_and_table_index, ipr - im->pool);
    }

    ipr->payload_type      = payload_type;
    ipr->pl                = fib_path_list_create(FIB_PATH_LIST_FLAG_SHARED | FIB_PATH_LIST_FLAG_NO_URPF, rpaths);
    ipr->sibling           = fib_path_list_child_add(ipr->pl, im->fib_node_type, ipr - im->pool);
...
__clib_export int ip_session_redirect_del(vlib_main_t *vm, u32 table_index, const u8 *match)
{
    ip_session_redirect_main_t *im = &ip_session_redirect_main;
    ...
    ipr = ip_session_redirect_find(im, table_index, match);
    if (!ipr) return VNET_API_ERROR_NO_SUCH_ENTRY;
    ...
    hash_unset_mem(im->session_by_match_and_table_index, ipr->match_and_table_index);
...
static int ip_session_redirect_show_yield(vlib_main_t *vm, f64 *start)
{
...
static u8 *format_ip_session_redirect(u8 *s, va_list *args)
{
    const ip_session_redirect_main_t *im   = &ip_session_redirect_main;
    const ip_session_redirect_t      *ipr  = va_arg(*args, const ip_session_redirect_t *);
    index_t                           ipri = ipr - im->pool;
    const char                       *type = ipr->is_punt ? "[punt]" : "[acl]";
    const char                       *ip   = ipr->is_ip6 ? "[ip6]" : "[ip4]";
    s                                      = format(s,
               "[%u] %s %s table %d key %U opaque_index 0x%x\n",
               ipri,
               type,
               ip,
               ipr->table_index,
               format_hex_bytes,
               ipr->match_and_table_index,
               vec_len(ipr->match_and_table_index) - 4,
               ipr->opaque_index);
    s                                      = format(s, " via:\n");
...
static clib_error_t *ip_session_redirect_show_cmd(vlib_main_t *vm, unformat_input_t *main_input, vlib_cli_command_t *cmd)
{
    ip_session_redirect_main_t *im = &ip_session_redirect_main;
    ...
    if (unformat_is_eof(main_input))
        unformat_init(line_input, 0, 0); /* support straight "sh ip session redirect" */
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "all"))
            ...
            ;
        else if (unformat(line_input, "match %U", unformat_classify_match, cm, &match, table_index))
            ;
    ...
    if (match) {
        ...
        pool_foreach (ipr, iprs) {
            ...
            if ((~0 == table_index || ipr->table_index == table_index) && (-1 == is_punt || ipr->is_punt == is_punt)
...
VLIB_CLI_COMMAND(ip_session_redirect_show_command, static) = {
    ...
    .short_help =
        "show ip session redirect [all|[table <table-index>] "
        "[punt|acl] [ip4|ip6] [match]]",
};

static clib_error_t *ip_session_redirect_cmd(vlib_main_t *vm, unformat_input_t *main_input, vlib_cli_command_t *cmd)
{
    unformat_input_t      _line_input, *line_input = &_line_input;
    vnet_classify_main_t *cm           = &vnet_classify_main;
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "del"))
            ...
            ;
        else if (unformat(line_input, "via %U", unformat_fib_route_path, &rpath, &proto))
    ...
    if (is_add) {
        ...
        rv = ip_session_redirect_add(vm, table_index, opaque_index, proto, is_punt, match, rpaths);
    }
    else {
        rv = ip_session_redirect_del(vm, table_index, match);
    }

    if (rv) error = clib_error_create("failed with error %d", rv);
...
VLIB_CLI_COMMAND(ip_session_redirect_command, static) = {.path     = "ip session redirect",
                                                         .function = ip_session_redirect_cmd,
                                                         .short_help =
                                                             "ip session redirect [add] [punt] table <index> match <match> "
                                                             "via <path> | del table <index> match <match>"};

static fib_node_t *ip_session_redirect_get_node(fib_node_index_t index)
{
    ip_session_redirect_main_t *im  = &ip_session_redirect_main;
...
static ip_session_redirect_t *ip_session_redirect_get_from_node(fib_node_t *node)
{
    return (ip_session_redirect_t *) (((char *) node) - STRUCT_OFFSET_OF(ip_session_redirect_t, node));
}

static void ip_session_redirect_last_lock_gone(fib_node_t *node)
{
...
static fib_node_back_walk_rc_t ip_session_redirect_back_walk_notify(fib_node_t *node, fib_node_back_walk_ctx_t *ctx)
{
    int                    rv;
    ip_session_redirect_t *ipr = ip_session_redirect_get_from_node(node);
    rv                         = ip_session_redirect_stack(ipr);
    ASSERT(0 == rv);
    if (rv) clib_warning("ip_session_redirect_stack() error %d", rv);
...
static clib_error_t *ip_session_redirect_init(vlib_main_t *vm)
{
    ip_session_redirect_main_t *im       = &ip_session_redirect_main;
    im->session_by_match_and_table_index = hash_create_vec(0, sizeof(u8), sizeof(u32));
    im->fib_node_type                    = fib_node_register_new_type("ip-session-redirect", &ip_session_redirect_vft);
    return 0;
}
