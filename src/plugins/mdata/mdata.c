...
static void mdata_trace_callback(vlib_node_runtime_perf_callback_data_t *data, vlib_node_runtime_perf_callback_args_t *args)
{
    ...
    if (PREDICT_FALSE(args->call_type == VLIB_NODE_RUNTIME_PERF_RESET)) return;
    ...
    if (args->call_type == VLIB_NODE_RUNTIME_PERF_AFTER) goto after_pass;
...
after_pass:
    ...
    while (n_left_from > 0) {
        ...
        for (i = 0; i < ARRAY_LEN(before->mdata); i++) {
            ...
            if (before->mdata[i] != after[i]) modifies->mdata[i] = 0xff;
        }
...
int mdata_enable_disable(mdata_main_t *mmp, int enable_disable)
{
    ...
    if (mmp->modify_lock == 0 && thread_main->n_vlib_mains > 1) clib_spinlock_init(&mmp->modify_lock);
    ...
    for (i = 0; i < vlib_get_n_threads(); i++) {
        ...
        clib_callback_data_enable_disable(&ovm->vlib_node_runtime_perf_callbacks, mdata_trace_callback, enable_disable);
    }
...
static clib_error_t *mdata_enable_disable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "disable") || unformat(input, "off")) enable_disable = 0;
...
VLIB_PLUGIN_REGISTER() = {.version = VPP_BUILD_VER, .description = "Buffer metadata change tracker."};
...
static u8 *format_buffer_metadata_changes(u8 *s, va_list *args)
{
    ...
    for (i = 0; i < vec_len(mm->modifies); i++) {
        ...
        for (j = 0; j < ARRAY_LEN(modifies->mdata); j++) {
            if (modifies->mdata[j]) goto found;
        }
...
#undef _
        ...
        if (printed == 0) s = format(s, "no vlib_buffer_t metadata changes");
...
#undef _
        ...
        if (printed == 0) s = format(s, "no changes");
...
static clib_error_t *show_metadata_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vlib_cli_output(vm, "%U", format_buffer_metadata_changes, &mdata_main, verbose);
    return 0;
}
