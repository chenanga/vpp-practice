...
static inline ipfix_field_specifier_t *flowprobe_template_ip4_fields(ipfix_field_specifier_t *f)
{
#define flowprobe_template_ip4_field_count() 4
    /* sourceIpv4Address, TLV type 8, u32 */
    f->e_id_length = ipfix_e_id_length(0 /* enterprise */, sourceIPv4Address, 4);
    f++;
    /* destinationIPv4Address, TLV type 12, u32 */
    f->e_id_length = ipfix_e_id_length(0 /* enterprise */, destinationIPv4Address, 4);
    f++;
    /* protocolIdentifier, TLV type 4, u8 */
    f->e_id_length = ipfix_e_id_length(0 /* enterprise */, protocolIdentifier, 1);
    f++;
...
static inline ipfix_field_specifier_t *flowprobe_template_ip6_fields(ipfix_field_specifier_t *f)
{
#define flowprobe_template_ip6_field_count() 4
    /* sourceIpv6Address, TLV type 27, 16 octets */
    f->e_id_length = ipfix_e_id_length(0 /* enterprise */, sourceIPv6Address, 16);
    f++;
    /* destinationIPv6Address, TLV type 28, 16 octets */
    f->e_id_length = ipfix_e_id_length(0 /* enterprise */, destinationIPv6Address, 16);
    f++;
    /* protocolIdentifier, TLV type 4, u8 */
    f->e_id_length = ipfix_e_id_length(0 /* enterprise */, protocolIdentifier, 1);
    f++;
...
static inline ipfix_field_specifier_t *flowprobe_template_l2_fields(ipfix_field_specifier_t *f)
{
#define flowprobe_template_l2_field_count() 3
    ...
    f->e_id_length = ipfix_e_id_length(0 /* enterprise */, destinationMacAddress, 6);
    f++;
...
static inline ipfix_field_specifier_t *flowprobe_template_common_fields(ipfix_field_specifier_t *f)
{
#define flowprobe_template_common_field_count() 6
    ...
    f->e_id_length = ipfix_e_id_length(0 /* enterprise */, flowStartNanoseconds, 8);
    f++;

    /* flowEndNanoseconds, TLV type 157, u64 */
    f->e_id_length = ipfix_e_id_length(0 /* enterprise */, flowEndNanoseconds, 8);
    f++;

    return f;
}

static inline ipfix_field_specifier_t *flowprobe_template_l4_fields(ipfix_field_specifier_t *f)
{
#define flowprobe_template_l4_field_count() 3
    /* sourceTransportPort, TLV type 7, u16 */
    f->e_id_length = ipfix_e_id_length(0 /* enterprise */, sourceTransportPort, 2);
    f++;
    /* destinationTransportPort, TLV type 11, u16 */
    f->e_id_length = ipfix_e_id_length(0 /* enterprise */, destinationTransportPort, 2);
    f++;
 ...
 */
static inline u8 *flowprobe_template_rewrite_inline(ipfix_exporter_t *exp, flow_report_t *fr, u16 collector_port, flowprobe_variant_t which)
{
    ip4_header_t                *ip;
    udp_header_t                *udp;
    ...
    if (flags & FLOW_RECORD_L3) {
        ...
        if (which == FLOW_VARIANT_L2_IP4) flags |= FLOW_RECORD_L2_IP4;
        if (which == FLOW_VARIANT_L2_IP6) flags |= FLOW_RECORD_L2_IP6;
    }
    ...
    if (flags & FLOW_RECORD_L2) field_count += flowprobe_template_l2_field_count();
    if (collect_ip4) field_count += flowprobe_template_ip4_field_count();
    if (collect_ip6) field_count += flowprobe_template_ip6_field_count();
    if (collect_l4) field_count += flowprobe_template_l4_field_count();

    /* allocate rewrite space */
    vec_validate_aligned(rewrite, sizeof(ip4_ipfix_template_packet_t) + field_count * sizeof(ipfix_field_specifier_t) - 1, CLIB_CACHE_LINE_BYTES);
    ...
    if (flags & FLOW_RECORD_L2) f = flowprobe_template_l2_fields(f);
    if (collect_ip4) f = flowprobe_template_ip4_fields(f);
    if (collect_ip6) f = flowprobe_template_ip6_fields(f);
    if (collect_l4) f = flowprobe_template_l4_fields(f);
...
static u8 *flowprobe_template_rewrite_ip6(ipfix_exporter_t       *exp,
                                          flow_report_t          *fr,
                                          u16                     collector_port,
                                          ...
                                          u32                    *stream_index)
{
    return flowprobe_template_rewrite_inline(exp, fr, collector_port, FLOW_VARIANT_IP6);
}

static u8 *flowprobe_template_rewrite_ip4(ipfix_exporter_t       *exp,
                                          flow_report_t          *fr,
                                          u16                     collector_port,
                                          ipfix_report_element_t *elts,
                                          u32                     n_elts,
                                          u32                    *stream_index)
{
    return flowprobe_template_rewrite_inline(exp, fr, collector_port, FLOW_VARIANT_IP4);
}

static u8 *flowprobe_template_rewrite_l2(ipfix_exporter_t       *exp,
                                         flow_report_t          *fr,
                                         u16                     collector_port,
                                         ipfix_report_element_t *elts,
                                         u32                     n_elts,
                                         u32                    *stream_index)
{
    return flowprobe_template_rewrite_inline(exp, fr, collector_port, FLOW_VARIANT_L2);
}

static u8 *flowprobe_template_rewrite_l2_ip4(ipfix_exporter_t       *exp,
                                             flow_report_t          *fr,
                                             u16                     collector_port,
                                             ipfix_report_element_t *elts,
                                             u32                     n_elts,
...
static u8 *flowprobe_template_rewrite_l2_ip6(ipfix_exporter_t       *exp,
                                             flow_report_t          *fr,
                                             u16                     collector_port,
                                             ipfix_report_element_t *elts,
                                             u32                     n_elts,
                                             u32                    *stream_index)
{
    return flowprobe_template_rewrite_inline(exp, fr, collector_port, FLOW_VARIANT_L2_IP6);
}
...
static clib_error_t *flowprobe_create_state_tables(u32 active_timer)
{
    ...
    if (active_timer) {
        ...
        for (i = 0; i < num_threads; i++) {
            ...
            fm->timers_per_worker[i] = clib_mem_alloc(sizeof(TWT(tw_timer_wheel)));
            tw_timer_wheel_init_2t_1w_2048sl(fm->timers_per_worker[i], flowprobe_expired_timer_callback, 1.0, 1024);
        }
        fm->disabled = true;
    }
    else {
...
static clib_error_t *flowprobe_clear_state_if_index(u32 sw_if_index)
{
    flowprobe_main_t *fm    = &flowprobe_main;
    clib_error_t     *error = 0;
    u32               worker_i;
    ...
    if (fm->active_timer > 0) {
        vec_foreach_index (worker_i, fm->pool_per_worker) {
            pool_foreach_index (entry_i, fm->pool_per_worker[worker_i]) {
                flowprobe_entry_t *e = pool_elt_at_index(fm->pool_per_worker[worker_i], entry_i);
                if (e->key.rx_sw_if_index == sw_if_index || e->key.tx_sw_if_index == sw_if_index) {
                    e->packetcount    = 0;
                    e->octetcount     = 0;
                    e->prot.tcp.flags = 0;
                    if (fm->passive_timer > 0) {
                        tw_timer_stop_2t_1w_2048sl(fm->timers_per_worker[worker_i], e->passive_timer_handle);
                    }
...
static int flowprobe_interface_add_del_feature(flowprobe_main_t *fm, u32 sw_if_index, u8 which, u8 direction, int is_add)
{
    vlib_main_t       *vm          = vlib_get_main();
    int                rv          = 0;
    u16                template_id = 0;
    ...
    if (is_add && fm->template_per_flow[which] > 1) template_id = fm->template_reports[flags];

    if ((is_add && fm->template_per_flow[which] == 1) || (!is_add && fm->template_per_flow[which] == 0)) {
        if (which == FLOW_VARIANT_L2) {
            if (!is_add) {
                flowprobe_flush_callback_l2();
            }
            if (fm->record & FLOW_RECORD_L2) {
                rv                          = flowprobe_template_add_del(1,
                                                UDP_DST_PORT_ipfix,
                                                flags,
            ...
            if (fm->record & FLOW_RECORD_L3 || fm->record & FLOW_RECORD_L4) {
                rv                                               = flowprobe_template_add_del(1,
                                                ...
                                                is_add,
                                                &template_id);
                fm->template_reports[flags | FLOW_RECORD_L2_IP4] = (is_add) ? template_id : 0;
                rv                                               = flowprobe_template_add_del(1,
                                                UDP_DST_PORT_ipfix,
                                                flags,
                                                flowprobe_data_callback_l2,
                                                ...
                                                is_add,
                                                &template_id);
                fm->template_reports[flags | FLOW_RECORD_L2_IP6] = (is_add) ? template_id : 0;
    ...
    if (direction == FLOW_DIRECTION_RX || direction == FLOW_DIRECTION_BOTH) {
        if (which == FLOW_VARIANT_IP4) {
            vnet_feature_enable_disable("ip4-unicast", "flowprobe-input-ip4", sw_if_index, is_add, 0, 0);
            vnet_feature_enable_disable("ip4-multicast", "flowprobe-input-ip4", sw_if_index, is_add, 0, 0);
        }
        else if (which == FLOW_VARIANT_IP6) {
            vnet_feature_enable_disable("ip6-unicast", "flowprobe-input-ip6", sw_if_index, is_add, 0, 0);
            vnet_feature_enable_disable("ip6-multicast", "flowprobe-input-ip6", sw_if_index, is_add, 0, 0);
        }
        else if (which == FLOW_VARIANT_L2)
            vnet_feature_enable_disable("device-input", "flowprobe-input-l2", sw_if_index, is_add, 0, 0);
    }

    if (direction == FLOW_DIRECTION_TX || direction == FLOW_DIRECTION_BOTH) {
        if (which == FLOW_VARIANT_IP4)
            vnet_feature_enable_disable("ip4-output", "flowprobe-output-ip4", sw_if_index, is_add, 0, 0);
        else if (which == FLOW_VARIANT_IP6)
            vnet_feature_enable_disable("ip6-output", "flowprobe-output-ip6", sw_if_index, is_add, 0, 0);
        else if (which == FLOW_VARIANT_L2)
            vnet_feature_enable_disable("interface-output", "flowprobe-output-l2", sw_if_index, is_add, 0, 0);
    }

    /* Stateful flow collection */
    if (is_add && !fm->initialized) {
        flowprobe_create_state_tables(fm->active_timer);
        if (fm->active_timer) vlib_process_signal_event(vm, flowprobe_timer_node.index, 1, 0);
    }
...
void vl_api_flowprobe_tx_interface_add_del_t_handler(vl_api_flowprobe_tx_interface_add_del_t *mp)
{
    ...
    rv = flowprobe_interface_add_del_feature(fm, sw_if_index, mp->which, FLOW_DIRECTION_TX, mp->is_add);
...
void vl_api_flowprobe_interface_add_del_t_handler(vl_api_flowprobe_interface_add_del_t *mp)
{
    flowprobe_main_t                           *fm = &flowprobe_main;
    ...
    rv = flowprobe_interface_add_del_feature(fm, sw_if_index, which, direction, is_add);
...
static void send_flowprobe_interface_details(u32 sw_if_index, u8 which, u8 direction, vl_api_registration_t *reg, u32 context)
{
    flowprobe_main_t                     *fm  = &flowprobe_main;
    ...
    rmp->_vl_msg_id = ntohs(VL_API_FLOWPROBE_INTERFACE_DETAILS + REPLY_MSG_ID_BASE);
    rmp->context    = context;
...
static void vl_api_flowprobe_interface_dump_t_handler(vl_api_flowprobe_interface_dump_t *mp)
{
    flowprobe_main_t      *fm = &flowprobe_main;
    ...
    if (sw_if_index == ~0) {
        ...
        vec_foreach (which, fm->flow_per_interface) {
            if (*which == (u8) ~0) continue;

            sw_if_index = which - fm->flow_per_interface;
            send_flowprobe_interface_details(sw_if_index, *which, fm->direction_per_interface[sw_if_index], reg, mp->context);
        }
    }
    else if (vec_len(fm->flow_per_interface) > sw_if_index && fm->flow_per_interface[sw_if_index] != (u8) ~0) {
        send_flowprobe_interface_details(sw_if_index,
                                         fm->flow_per_interface[sw_if_index],
...
static int flowprobe_params(flowprobe_main_t *fm, u8 record_l2, u8 record_l3, u8 record_l4, u32 active_timer, u32 passive_timer)
{
    ...
    if (vec_neg_search(fm->flow_per_interface, (u8) ~0) != ~0) return VNET_API_ERROR_UNSUPPORTED;

    if (record_l2) flags |= FLOW_RECORD_L2;
    if (record_l3) flags |= FLOW_RECORD_L3;
    if (record_l4) flags |= FLOW_RECORD_L4;
     ...
     */
    fm->active_timer  = (active_timer == (u32) ~0 ? FLOWPROBE_TIMER_ACTIVE : active_timer);
    fm->passive_timer = (passive_timer == (u32) ~0 ? FLOWPROBE_TIMER_PASSIVE : passive_timer);
...
void vl_api_flowprobe_set_params_t_handler(vl_api_flowprobe_set_params_t *mp)
{
    flowprobe_main_t                    *fm = &flowprobe_main;
    ...
    rv = flowprobe_params(fm, record_l2, record_l3, record_l4, active_timer, passive_timer);
    if (rv == VNET_API_ERROR_UNSUPPORTED) clib_warning("Cannot change params when feature is enabled on some interfaces");

out:
    REPLY_MACRO(VL_API_FLOWPROBE_SET_PARAMS_REPLY);
}

void vl_api_flowprobe_get_params_t_handler(vl_api_flowprobe_get_params_t *mp)
{
    flowprobe_main_t                    *fm = &flowprobe_main;
    ...
    if (fm->record & FLOW_RECORD_L2) record_flags |= FLOWPROBE_RECORD_FLAG_L2;
    if (fm->record & FLOW_RECORD_L3) record_flags |= FLOWPROBE_RECORD_FLAG_L3;
    if (fm->record & FLOW_RECORD_L4) record_flags |= FLOWPROBE_RECORD_FLAG_L4;
...
u8 *format_flowprobe_direction(u8 *s, va_list *args)
{
...
u8 *format_flowprobe_entry(u8 *s, va_list *args)
{
    ...
    s = format(s, " %U %U", format_ethernet_address, &e->key.src_mac, format_ethernet_address, &e->key.dst_mac);
    s = format(s, " %U -> %U", format_ip46_address, &e->key.src_address, IP46_TYPE_ANY, format_ip46_address, &e->key.dst_address, IP46_TYPE_ANY);
    s = format(s, " %d", e->key.protocol);
    s = format(s, " %d %d\n", clib_net_to_host_u16(e->key.src_port), clib_net_to_host_u16(e->key.dst_port));
...
u8 *format_flowprobe_params(u8 *s, va_list *args)
{
    ...
    if (flags & FLOW_RECORD_L2) s = format(s, " l2");
    if (flags & FLOW_RECORD_L3) s = format(s, " l3");
    if (flags & FLOW_RECORD_L4) s = format(s, " l4");

    if (active_timer != (u32) ~0) s = format(s, " active: %d", active_timer);

    if (passive_timer != (u32) ~0) s = format(s, " passive: %d", passive_timer);
...
static clib_error_t *flowprobe_show_table_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cm)
{
    ...
    for (i = 0; i < vec_len(fm->pool_per_worker); i++) {
        pool_foreach (e, fm->pool_per_worker[i]) {
            vlib_cli_output(vm, "%U", format_flowprobe_entry, e);
        }
    }
    return 0;
}
...
static clib_error_t *flowprobe_show_stats_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cm)
{
    ...
    for (i = 0; i < vec_len(fm->pool_per_worker); i++)
        vlib_cli_output(vm, "Pool utilisation thread %d is %d%%\n", i, (100 * pool_elts(fm->pool_per_worker[i])) / (0x1 << FLOWPROBE_LOG2_HASHSIZE));
    return 0;
}

static clib_error_t *flowprobe_interface_add_del_feature_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    flowprobe_main_t     *fm          = &flowprobe_main;
    u32                   sw_if_index = ~0;
    int                   is_add      = 1;
    ...
    if (fm->record == 0) return clib_error_return(0, "Please specify flowprobe params record first...");

    if (sw_if_index == ~0) return clib_error_return(0, "Please specify an interface...");

    rv = validate_feature_on_interface(fm, sw_if_index, which);
    if (rv == 1) {
        if (is_add) return clib_error_return(0, "Datapath is already enabled for given interface...");
    }
    else if (rv == 0)
        return clib_error_return(0, "Interface has enable different datapath ...");
    else if (rv == -1) {
        if (!is_add) {
    ...
    switch (rv) {
        ...
        default:
            return clib_error_return(0, "flowprobe_enable_disable returned %d", rv);
    }
    return 0;
}
...
static clib_error_t *flowprobe_params_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (passive_timer > 0 && active_timer > passive_timer) return clib_error_return(0, "Passive timer has to be greater than active one...");

    if (flowprobe_params(fm, record_l2, record_l3, record_l4, active_timer, passive_timer))
        return clib_error_return(0, "Couldn't change flowperpacket params when feature is enabled on some interface ...");
    return 0;
}
...
static clib_error_t *flowprobe_show_params_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vlib_cli_output(vm, "%U", format_flowprobe_params, flags, active_timer, passive_timer);
    return 0;
}
...
VLIB_CLI_COMMAND(flowprobe_enable_disable_command, static) = {
    ...
    .short_help =
        "flowprobe feature add-del <interface-name> [(l2|ip4|ip6)] "
        "[(rx|tx|both)] [disable]",
...
VLIB_CLI_COMMAND(flowprobe_params_command, static) = {
    ...
    .short_help =
        "flowprobe params record [l2] [l3] [l4] [active <timer>] "
        "[passive <timer>]",
...
VLIB_CLI_COMMAND(flowprobe_show_feature_command, static) = {
    ...
    .short_help = "show flowprobe feature",
...
VLIB_CLI_COMMAND(flowprobe_show_params_command, static) = {
    ...
    .short_help = "show flowprobe params",
...
static uword timer_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    if (event_type != 1) clib_warning("bogus kickoff event received, %d", event_type);
    ...
    if (vlib_get_n_threads() == 0)
        ...
        for (i = 0; i < vlib_get_n_threads(); i++) {
            ...
            if (worker_vm) vec_add1(worker_vms, worker_vm);
        }
    }
    ...
    while (1) {
        ...
        for (i = 0; i < vec_len(worker_vms); i++) {
            ...
            if (worker_vm) {
                vlib_node_set_interrupt_pending(worker_vm, flowprobe_walker_node.index);
                sleep_duration = (fm->expired_passive_per_worker[i] > 0) ? 1e-4 : 0.1;
            }
        }
...
static clib_error_t *flowprobe_init(vlib_main_t *vm)
{
    ...
    for (i = 0; i < FLOW_N_VARIANTS; i++) {
        ...
        vec_validate(fm->context[i].next_record_offset_per_worker, num_threads - 1);
    }
