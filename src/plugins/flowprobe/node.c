...
static u8 *format_flowprobe_trace(u8 *s, va_list *args)
{
    ...
    s = format(s,
               ...
               flowprobe_variant_strings[t->which],
               t->rx_sw_if_index,
               t->tx_sw_if_index,
               t->timestamp,
               t->buffer_size);
    ...
    if (t->which == FLOW_VARIANT_L2)
        s = format(s, "\n%U -> %U", format_white_space, indent, format_ethernet_address, &t->src_mac, format_ethernet_address, &t->dst_mac);

    if (t->protocol > 0
        && (t->which == FLOW_VARIANT_L2_IP4 || t->which == FLOW_VARIANT_IP4 || t->which == FLOW_VARIANT_L2_IP6 || t->which == FLOW_VARIANT_IP6))
        s = format(s,
                   "\n%U%U: %U -> %U",
                   format_white_space,
                   indent,
...
#define FLOWPROBE_NEXT_NODES                                                              \
    {                                                                                     \
        [FLOWPROBE_NEXT_DROP] = "error-drop", [FLOWPROBE_NEXT_IP4_LOOKUP] = "ip4-lookup", \
    }

static inline flowprobe_variant_t flowprobe_get_variant(flowprobe_variant_t which, flowprobe_record_t flags, u16 ethertype)
{
    if (which == FLOW_VARIANT_L2 && (flags & FLOW_RECORD_L3 || flags & FLOW_RECORD_L4))
        return ethertype == ETHERNET_TYPE_IP6 ? FLOW_VARIANT_L2_IP6 : ethertype == ETHERNET_TYPE_IP4 ? FLOW_VARIANT_L2_IP4 : FLOW_VARIANT_L2;
    return which;
}
...
static inline u32 flowprobe_common_add(vlib_buffer_t *to_b, flowprobe_entry_t *e, u16 offset)
{
    u16 start = offset;
...
static inline u32 flowprobe_l2_add(vlib_buffer_t *to_b, flowprobe_entry_t *e, u16 offset)
{
    u16 start = offset;
...
static inline u32 flowprobe_l3_ip6_add(vlib_buffer_t *to_b, flowprobe_entry_t *e, u16 offset)
{
    u16 start = offset;

    /* ip6 src address */
    clib_memcpy_fast(to_b->data + offset, &e->key.src_address, sizeof(ip6_address_t));
    offset += sizeof(ip6_address_t);

    /* ip6 dst address */
    clib_memcpy_fast(to_b->data + offset, &e->key.dst_address, sizeof(ip6_address_t));
    offset += sizeof(ip6_address_t);
...
static inline u32 flowprobe_l3_ip4_add(vlib_buffer_t *to_b, flowprobe_entry_t *e, u16 offset)
{
    u16 start = offset;

    /* ip4 src address */
    clib_memcpy_fast(to_b->data + offset, &e->key.src_address.ip4, sizeof(ip4_address_t));
    offset += sizeof(ip4_address_t);

    /* ip4 dst address */
    clib_memcpy_fast(to_b->data + offset, &e->key.dst_address.ip4, sizeof(ip4_address_t));
    offset += sizeof(ip4_address_t);
...
static inline u32 flowprobe_l4_add(vlib_buffer_t *to_b, flowprobe_entry_t *e, u16 offset)
{
    u16 start = offset;
...
static inline u32 flowprobe_hash(flowprobe_key_t *k)
{
    ...
    *poolindex = fm->hash_per_worker[my_cpu_number][h];
    if (*poolindex != ~0) {
        ...
        if (e) {
            ...
            if (memcmp(k, &e->key, sizeof(flowprobe_key_t))) *collision = true;
            return e;
        }
    }
...
flowprobe_entry_t *flowprobe_create(u32 my_cpu_number, flowprobe_key_t *k, u32 *poolindex)
{
    ...
    *poolindex                            = e - fm->pool_per_worker[my_cpu_number];
    ...
    if (fm->passive_timer > 0) {
        e->passive_timer_handle = tw_timer_start_2t_1w_2048sl(fm->timers_per_worker[my_cpu_number], *poolindex, 0, fm->passive_timer);
    }
    return e;
}

static inline void add_to_flow_record_state(vlib_main_t          *vm,
                                            ...
                                            flowprobe_trace_t    *t)
{
    ...
    ASSERT(b);
    ethernet_header_t *eth           = (direction == FLOW_DIRECTION_TX) ? vlib_buffer_get_current(b) : ethernet_buffer_get_header(b);
    ...
    if (fm->active_timer > 0) {
        ...
        if (collision) {
            ...
            if (e->packetcount) flowprobe_export_entry(vm, e);
            e->key        = k;
            e->flow_start = timestamp;
            vlib_node_increment_counter(vm, node->node_index, FLOWPROBE_ERROR_COLLISION, 1);
        }
    ...
    if (e) {
        ...
        if (fm->active_timer == 0 || (now > e->last_exported + fm->active_timer)) flowprobe_export_entry(vm, e);
    }
}

static u16 flowprobe_get_headersize(void)
{
    return sizeof(ip4_header_t) + sizeof(udp_header_t) + sizeof(ipfix_message_header_t) + sizeof(ipfix_set_header_t);
}
...
static void flowprobe_export_send(vlib_main_t *vm, vlib_buffer_t *b0, flowprobe_variant_t which)
{
    ...
    /* Nothing to send */
    if (fm->context[which].next_record_offset_per_worker[my_cpu_number] <= flowprobe_get_headersize()) return;
    ...
    h->export_time = (u32) (((f64) frm->unix_time_0) + (vlib_time_now(vm) - frm->vlib_time_0));
    ...
    s->set_id_length  = ipfix_set_id_length(fm->template_reports[flags], b0->current_length - (sizeof(*ip) + sizeof(*udp) + sizeof(*h)));
    h->version_length = version_length(b0->current_length - (sizeof(*ip) + sizeof(*udp)));
    ...
    vlib_node_increment_counter(vm, flowprobe_output_l2_node.index, FLOWPROBE_ERROR_EXPORTED_PACKETS, 1);

    fm->context[which].frames_per_worker[my_cpu_number]             = 0;
    fm->context[which].buffers_per_worker[my_cpu_number]            = 0;
    fm->context[which].next_record_offset_per_worker[my_cpu_number] = flowprobe_get_headersize();
}
...
static vlib_buffer_t *flowprobe_get_buffer(vlib_main_t *vm, flowprobe_variant_t which)
{
    ...
    if (PREDICT_FALSE(b0 == 0)) {
        if (vlib_buffer_alloc(vm, &bi0, 1) != 1) {
            vlib_node_increment_counter(vm, flowprobe_output_l2_node.index, FLOWPROBE_ERROR_BUFFER, 1);
            return 0;
        }

        /* Initialize the buffer */
        b0 = fm->context[which].buffers_per_worker[my_cpu_number] = vlib_get_buffer(vm, bi0);
        ...
        fm->context[which].next_record_offset_per_worker[my_cpu_number] = b0->current_length;
    }
...
static void flowprobe_export_entry(vlib_main_t *vm, flowprobe_entry_t *e)
{
    ...
    if (offset < flowprobe_get_headersize()) offset = flowprobe_get_headersize();
    ...
    if (flags & FLOW_RECORD_L2) offset += flowprobe_l2_add(b0, e, offset);
    if (collect_ip6) offset += flowprobe_l3_ip6_add(b0, e, offset);
    if (collect_ip4) offset += flowprobe_l3_ip4_add(b0, e, offset);
    if (collect_l4) offset += flowprobe_l4_add(b0, e, offset);
    ...
    if (offset + fm->template_size[flags] > exp->path_mtu) flowprobe_export_send(vm, b0, which);
}

uword flowprobe_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, flowprobe_variant_t which, flowprobe_direction_t direction)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (PREDICT_TRUE((b0->flags & VNET_BUFFER_F_FLOW_REPORT) == 0))
                add_to_flow_record_state(vm,
                                         node,
                                         fm,
                                         b0,
                                         timestamp,
                                         len0,
                                         flowprobe_get_variant(which, fm->context[which].flags, ethertype0),
                                         direction,
                                         0);
            ...
            if (PREDICT_TRUE((b1->flags & VNET_BUFFER_F_FLOW_REPORT) == 0))
                add_to_flow_record_state(vm,
                                         ...
                                         timestamp,
                                         len1,
                                         flowprobe_get_variant(which, fm->context[which].flags, ethertype1),
                                         direction,
                                         0);
...
static uword flowprobe_input_ip4_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return flowprobe_node_fn(vm, node, frame, FLOW_VARIANT_IP4, FLOW_DIRECTION_RX);
}

static uword flowprobe_input_ip6_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return flowprobe_node_fn(vm, node, frame, FLOW_VARIANT_IP6, FLOW_DIRECTION_RX);
}

static uword flowprobe_input_l2_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return flowprobe_node_fn(vm, node, frame, FLOW_VARIANT_L2, FLOW_DIRECTION_RX);
}

static uword flowprobe_output_ip4_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return flowprobe_node_fn(vm, node, frame, FLOW_VARIANT_IP4, FLOW_DIRECTION_TX);
}

static uword flowprobe_output_ip6_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return flowprobe_node_fn(vm, node, frame, FLOW_VARIANT_IP6, FLOW_DIRECTION_TX);
}

static uword flowprobe_output_l2_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return flowprobe_node_fn(vm, node, frame, FLOW_VARIANT_L2, FLOW_DIRECTION_TX);
}

static inline void flush_record(flowprobe_variant_t which)
{
    vlib_main_t   *vm = vlib_get_main();
    vlib_buffer_t *b  = flowprobe_get_buffer(vm, which);
    if (b) flowprobe_export_send(vm, b, which);
}
...
void flowprobe_flush_callback_ip4(void)
{
    ...
    for (i = 1; i < vlib_get_n_threads(); i++) {
        ...
        if (worker_vm) vlib_node_set_interrupt_pending(worker_vm, flowprobe_flush_ip4_node.index);
    }
...
void flowprobe_flush_callback_ip6(void)
{
    ...
    for (i = 1; i < vlib_get_n_threads(); i++) {
        worker_vm = vlib_get_main_by_index(i);
        if (worker_vm) vlib_node_set_interrupt_pending(worker_vm, flowprobe_flush_ip6_node.index);
    }
...
void flowprobe_flush_callback_l2(void)
{
    ...
    for (i = 1; i < vlib_get_n_threads(); i++) {
        worker_vm = vlib_get_main_by_index(i);
        if (worker_vm) vlib_node_set_interrupt_pending(worker_vm, flowprobe_flush_l2_node.index);
    }
...
static uword flowprobe_walker_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
     ...
     */
    if (ip_address_is_zero(&exp->ipfix_collector) || ip_address_is_zero(&exp->src_address)) {
        fm->disabled = true;
        return 0;
    }
    fm->disabled = false;
    ...
    vec_foreach (i, fm->expired_passive_per_worker[cpu_index]) {
        u32 exported = 0;
        f64 now      = vlib_time_now(vm);
        if (now > start_time + 100e-6 || exported > FLOW_MAXIMUM_EXPORT_ENTRIES - 1) break;
    ...
    if (count) vec_delete(fm->expired_passive_per_worker[cpu_index], count, 0);
...
static uword flowprobe_flush_ip4(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    flush_record(FLOW_VARIANT_IP4);
...
static uword flowprobe_flush_ip6(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    flush_record(FLOW_VARIANT_IP6);
...
static uword flowprobe_flush_l2(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    flush_record(FLOW_VARIANT_L2);
    flush_record(FLOW_VARIANT_L2_IP4);
    flush_record(FLOW_VARIANT_L2_IP6);
