...
static int api_flowprobe_tx_interface_add_del(vat_main_t *vam)
{
...
static int api_flowprobe_interface_add_del(vat_main_t *vam)
{
    unformat_input_t                     *i              = vam->input;
    int                                   enable_disable = 1;
...
static int api_flowprobe_interface_dump(vat_main_t *vam)
{
    unformat_input_t                  *i = vam->input;
...
static void vl_api_flowprobe_interface_details_t_handler(vl_api_flowprobe_interface_details_t *mp)
{
    ...
    which = mp->which;
    if (which > ARRAY_LEN(variants) - 2) which = ARRAY_LEN(variants) - 1;

    direction = mp->direction;
    if (direction > ARRAY_LEN(directions) - 2) direction = ARRAY_LEN(directions) - 1;

    out = format(0, "sw_if_index: %u, variant: %s, direction: %s\n%c", sw_if_index, variants[which], directions[direction], 0);

    fformat(vam->ofp, (char *) out);
    vec_free(out);
}

static int api_flowprobe_params(vat_main_t *vam)
{
    unformat_input_t          *i             = vam->input;
    u32                        active_timer  = ~0;
    u32                        passive_timer = ~0;
...
static int api_flowprobe_set_params(vat_main_t *vam)
{
    unformat_input_t              *i = vam->input;
...
static int api_flowprobe_get_params(vat_main_t *vam)
{
...
static void vl_api_flowprobe_get_params_reply_t_handler(vl_api_flowprobe_get_params_reply_t *mp)
{
    vat_main_t *vam = flowprobe_test_main.vat_main;
    u8         *out = 0;

    out = format(0, "active: %u, passive: %u, record:", ntohl(mp->active_timer), ntohl(mp->passive_timer));

    if (mp->record_flags & FLOWPROBE_RECORD_FLAG_L2) out = format(out, " l2");
    if (mp->record_flags & FLOWPROBE_RECORD_FLAG_L3) out = format(out, " l3");
    if (mp->record_flags & FLOWPROBE_RECORD_FLAG_L4) out = format(out, " l4");
