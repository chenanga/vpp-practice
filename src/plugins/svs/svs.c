...
int svs_table_delete(fib_protocol_t fproto, u32 table_id)
{
    ...
    vec_foreach_index (ii, svs_itf_db[fproto]) {
        if (svs_itf_db[fproto][ii] == fib_index) return VNET_API_ERROR_INSTANCE_IN_USE;
    }

    if (~0 == fib_index) return VNET_API_ERROR_NO_SUCH_FIB;
...
static int svs_route_add_i(u32 fib_index, const fib_prefix_t *pfx, u32 src_fib_index)
{
    ...
    fib_table_entry_special_dpo_add(fib_index, pfx, svs_fib_src, FIB_ENTRY_FLAG_EXCLUSIVE, &dpo);
...
int svs_route_add(u32 table_id, const fib_prefix_t *pfx, u32 source_table_id)
{
    ...
    if (~0 == src_fib_index) return (VNET_API_ERROR_NO_SUCH_FIB);
...
int svs_enable(fib_protocol_t fproto, u32 table_id, u32 sw_if_index)
{
     ...
     */
    svs_route_add_i(fib_index, &pfx, fib_table_get_index_for_sw_if_index(fproto, sw_if_index));
    ...
    vnet_feature_enable_disable((FIB_PROTOCOL_IP4 == fproto ? "ip4-unicast" : "ip6-unicast"),
                                (FIB_PROTOCOL_IP4 == fproto ? "svs-ip4" : "svs-ip6"),
                                sw_if_index,
                                1,
                                NULL,
                                0);
...
static void svs_table_bind(fib_protocol_t fproto, u32 sw_if_index, u32 itf_fib_index)
{
    ...
    if (sw_if_index >= vec_len(svs_itf_db[FIB_PROTOCOL_IP6])) return;
...
int svs_disable(fib_protocol_t fproto, u32 table_id, u32 sw_if_index)
{
    ...
    if (sw_if_index >= vec_len(svs_itf_db[fproto])) return VNET_API_ERROR_INVALID_SW_IF_INDEX;
...
void svs_walk(svs_walk_fn_t fn, void *ctx)
{
    ...
    FOR_EACH_FIB_IP_PROTOCOL(fproto)
    {
        vec_foreach_index (ii, svs_itf_db[fproto]) {
            ...
            if (~0 != fib_index) {
                if (WALK_CONTINUE != fn(fproto, fib_table_get_table_id(fib_index, fproto), ii, ctx)) return;
            }
        }
    }
}
...
always_inline uword svs_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, fib_protocol_t fproto)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (FIB_PROTOCOL_IP4 == fproto) {
                ...
                lbi0 = ip4_fib_forwarding_lookup(svs_itf_db[fproto][sw_if_index0], &ip0->src_address);
            }
            else {
                ip6_header_t *ip0;

                ip0  = vlib_buffer_get_current(b0);
                lbi0 = ip6_fib_table_fwding_lookup(svs_itf_db[fproto][sw_if_index0], &ip0->src_address);
            }
...
static clib_error_t *svs_table_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (~0 == table_id) return clib_error_return(0, "table-id must be specified");
...
static clib_error_t *svs_enable_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (~0 == sw_if_index) return clib_error_return(0, "interface must be specified");
    if (~0 == table_id) return clib_error_return(0, "table-id must be specified");
...
static clib_error_t *svs_route_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "add"))
            ...
            ;
        else if (unformat(input, "%U/%d", unformat_ip4_address, &pfx.fp_addr.ip4, &pfx.fp_len)) {
            pfx.fp_proto = FIB_PROTOCOL_IP4;
        }
        else if (unformat(input, "%U/%d", unformat_ip6_address, &pfx.fp_addr.ip6, &pfx.fp_len)) {
    ...
    if (~0 == table_id) return clib_error_return(0, "table-id must be specified");
    if (~0 == src_table_id) return clib_error_return(0, "src-table-id must be specified");
    ...
    if (rv != 0) return clib_error_return(0, "failed, rv=%d:%U", (int) rv, format_vnet_api_errno, rv);
    ...
    FOR_EACH_FIB_IP_PROTOCOL(fproto)
    {
        ...
        vec_foreach_index (ii, svs_itf_db[fproto]) {
            if (~0 != svs_itf_db[fproto][ii])
                vlib_cli_output(vm, "  %U -> %d", format_vnet_sw_if_index_name, vnet_get_main(), ii, svs_itf_db[fproto][ii]);
        }
    }
    return (NULL);
}
...
static clib_error_t *svs_init(vlib_main_t *vm)
{
    ...
    svs_fib_src = fib_source_allocate("svs", FIB_SOURCE_PRIORITY_LOW, FIB_SOURCE_BH_SIMPLE);
