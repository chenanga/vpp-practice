...
static char *get_pvti_trace_type_name(u8 ptype)
{
...
static u8 *format_pvti_input_trace(u8 *s, va_list *args)
{
    int i;
    ...
    s       = format(s,
               ...
               t->sw_if_index,
               t->next_index,
               get_pvti_trace_type_name(t->trace_type),
               t->trace_type,
               t->chunk_count);
    s       = format(s, "  src %U sport %d dport %d\n", format_ip_address, &t->remote_ip, t->remote_port, t->local_port);
