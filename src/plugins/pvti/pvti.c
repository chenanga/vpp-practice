...
u8 *format_pvti_tx_peer_ptr(u8 *s, va_list *args)
{
    ...
    s = format(s,
               ...
               peer->remote_port,
               peer->underlay_mtu,
               peer->underlay_fib_index,
               peer->pvti_if_index,
               peer->bo0_max_current_length,
               peer->current_tx_seq,
               peer->chunk_count,
...
u8 *format_pvti_rx_peer_ptr(u8 *s, va_list *args)
{
    pvti_rx_peer_t *peer = va_arg(*args, pvti_rx_peer_t *);

    s = format(s,
               "[%p]%s local:%U:%d remote:%U:%d pvti_idx:%d",
               peer,
               peer->deleted ? " DELETED" : "",
               format_ip46_address,
               &peer->local_ip,
...
void vnet_int_pvti_bypass_mode(u32 sw_if_index, u8 is_ip6, u8 is_enable)
{
    pvti_main_t *pvm = &pvti_main;

    if (pool_is_free_index(pvm->vnet_main->interface_main.sw_interfaces, sw_if_index)) return;
    ...
    if (is_ip6) {
        if (clib_bitmap_get(pvm->bm_ip6_bypass_enabled_by_sw_if, sw_if_index) != is_enable) {
            vnet_feature_enable_disable("ip6-unicast", "ip6-pvti-bypass", sw_if_index, is_enable, 0, 0);
            pvm->bm_ip6_bypass_enabled_by_sw_if = clib_bitmap_set(pvm->bm_ip6_bypass_enabled_by_sw_if, sw_if_index, is_enable);
        }
    }
    else {
        if (clib_bitmap_get(pvm->bm_ip4_bypass_enabled_by_sw_if, sw_if_index) != is_enable) {
            vnet_feature_enable_disable("ip4-unicast", "ip4-pvti-bypass", sw_if_index, is_enable, 0, 0);
            pvm->bm_ip4_bypass_enabled_by_sw_if = clib_bitmap_set(pvm->bm_ip4_bypass_enabled_by_sw_if, sw_if_index, is_enable);
        }
    }
}

static clib_error_t *set_ip_pvti_bypass(u32 is_ip6, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *set_ip4_pvti_bypass(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *set_ip6_pvti_bypass(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *pvti_interface_create_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    clib_error_t    *error = 0;
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "peer %U %d %d", unformat_ip_address, &peer_ip, &peer_port, &local_port)) {
            peer_set = 1;
        }
        else if (unformat(line_input, "underlay-mtu %d", &underlay_mtu)) {
            // MTU set
        }
        else if (unformat(line_input, "local-ip %U", unformat_ip_address, &local_ip)) {
            // local IP set
        }
            ...
            if (~0 == fib_index) {
                error = clib_error_return(0, "Nonexistent table id %d", underlay_table_id);
                goto done;
            }
    ...
    rv = pvti_if_create(&local_ip, local_port, &peer_ip, peer_port, peer_address_method, underlay_mtu, underlay_fib_index, &sw_if_index);
...
static clib_error_t *pvti_interface_delete_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    // pvti_main_t * pmp = &pvti_main;
    u32  sw_if_index  = ~0;
    int  rv           = 0;
    ...
    if (!if_index_set) return clib_error_return(0, "Please specify a sw_if_index...");
...
static clib_error_t *pvti_show_interface_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    pvti_if_t *pvti_if;
...
static clib_error_t *pvti_show_tx_peers_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    pvti_per_thread_data_t *ptd;
    int                     is_ip6;
    for (is_ip6 = 0; is_ip6 <= 1; is_ip6++) {
        vec_foreach (ptd, pvti_main.per_thread_data[is_ip6]) {
            vlib_cli_output(vm, "thread %d (%s)", ptd - pvti_main.per_thread_data[is_ip6], is_ip6 ? "IPv6" : "IPv4");
            pvti_tx_peer_t *peer;
...
static clib_error_t *pvti_show_rx_peers_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    pvti_per_thread_data_t *ptd;
    int                     is_ip6;
    for (is_ip6 = 0; is_ip6 <= 1; is_ip6++) {
...
static clib_error_t *pvti_early_config(vlib_main_t *vm, unformat_input_t *input)
{
    u8 *runs_before = 0;
    int rbi         = 0;
...
static clib_error_t *pvti_init(vlib_main_t *vm)
{
    pvti_main_t  *pmp   = &pvti_main;
    clib_error_t *error = 0;
