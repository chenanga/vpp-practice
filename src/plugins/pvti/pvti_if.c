...
static u8 *format_pvti_if_name(u8 *s, va_list *args)
{
...
u8 *format_pvti_if(u8 *s, va_list *args)
{
    ...
    s = format(s,
               "[%d] %U local:%U:%d remote:%U:%d underlay_mtu:%d underlay_fib_idx:%d",
               pvtii,
               ...
               IP46_TYPE_ANY,
               pvti_if->remote_port,
               pvti_if->underlay_mtu,
               pvti_if->underlay_fib_index);
...
{
    if (vec_len(pvti_main.if_index_by_sw_if_index) <= sw_if_index) return INDEX_INVALID;
    u32 ti = pvti_main.if_index_by_sw_if_index[sw_if_index];
    if (ti == ~0) return INDEX_INVALID;

    return (ti);
}

index_t pvti_if_find_by_remote_ip4_and_port(ip4_address_t *remote_ip4, u16 remote_port)
{
    pvti_if_t *ifc;
    pool_foreach (ifc, pvti_main.if_pool) {
        if ((ifc->remote_port == remote_port) && (ifc->remote_ip.version == AF_IP4)
            && ((ifc->remote_ip.ip.ip4.as_u32 == remote_ip4->as_u32) || ifc->peer_address_from_payload)) {
...
index_t pvti_if_find_by_remote_ip6_and_port(ip6_address_t *remote_ip6, u16 remote_port)
{
    pvti_if_t *ifc;
    pool_foreach (ifc, pvti_main.if_pool) {
        if ((ifc->remote_port == remote_port) && (ifc->remote_ip.version == AF_IP6)
            && ((0 == memcmp(&ifc->remote_ip.ip.ip6, remote_ip6, sizeof(*remote_ip6))) || ifc->peer_address_from_payload)) {
            return (ifc - pvti_main.if_pool);
        }
    }
    return INDEX_INVALID;
...
index_t pvti_if_find_by_remote_ip_and_port(ip_address_t *remote_ip, u16 remote_port)
{
    pvti_if_t *ifc;
    pool_foreach (ifc, pvti_main.if_pool) {
        if ((ifc->remote_port == remote_port) && (ifc->peer_address_from_payload || (0 == ip_address_cmp(remote_ip, &ifc->remote_ip)))) {
            return (ifc - pvti_main.if_pool);
        }
    }
    return INDEX_INVALID;
...
static void pvti_add_tidx_by_port(index_t t_index, u16 port)
{
...
static void pvti_del_tidx_by_port(index_t t_index, u16 port)
{
    pvti_main_t *pvm = &pvti_main;
    index_t     *ii;
    ...
    vec_foreach (ii, pvm->if_indices_by_port[port]) {
        if (*ii == t_index) {
            vec_del1(pvm->if_indices_by_port[port], pvm->if_indices_by_port[port] - ii);
            break;
        }
    }
}

static u32 pvti_get_tunnel_count_by_port(u16 port)
{
    pvti_main_t *pvm = &pvti_main;
    if (!pvm->if_indices_by_port) {
        return 0;
...
static clib_error_t *pvti_if_admin_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    // vnet_hw_interface_t *hi;
    u32 hw_flags;

    // hi = vnet_get_hw_interface (vnm, hw_if_index);
    hw_flags = (flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP ? VNET_HW_INTERFACE_FLAG_LINK_UP : 0);
    vnet_hw_interface_set_flags(vnm, hw_if_index, hw_flags);

    return (NULL);
}

void pvti_if_update_adj(vnet_main_t *vnm, u32 sw_if_index, adj_index_t ai)
{
...
int pvti_if_create(ip_address_t              *local_ip,
                   u16                        local_port,
                   ...
                   u32                       *sw_if_indexp)
{
    ...
    *sw_if_indexp = (u32) ~0;
    ...
    hw_if_index = vnet_register_interface(vnm, pvti_if_device_class.index, t_idx, pvti_hw_interface_class.index, t_idx);
    ...
    vec_validate_init_empty(pvm->if_index_by_sw_if_index, hi->sw_if_index, INDEX_INVALID);
    ...
    /*
      ...
      */
    vnet_set_interface_l3_output_node(vnm->vlib_main, hi->sw_if_index, (u8 *) "pvti4-output");
    ...
    if (1 == pvti_get_tunnel_count_by_port(local_port)) {
        ...
        udp_register_dst_port(vlib_get_main(), local_port, pvti4_input_node.index, UDP_IP4);
        udp_register_dst_port(vlib_get_main(), local_port, pvti6_input_node.index, UDP_IP6);
    }
    else {
        clib_warning("Not registering the port");
    }

    vnet_hw_interface_set_flags(vnm, pvti_if->hw_if_index, VNET_HW_INTERFACE_FLAG_LINK_UP);
...
void pvti_if_walk(pvti_if_walk_cb_t fn, void *data)
{
    ...
    pool_foreach_index (pvtii, pvti_main.if_pool) {
        if (WALK_STOP == fn(pvtii, data)) break;
    }
}
...
int pvti_if_delete(u32 sw_if_index)
{
    ...
    if (hw == 0 || hw->dev_class_index != pvti_if_device_class.index) return VNET_API_ERROR_INVALID_VALUE;
