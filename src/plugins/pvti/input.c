...
always_inline void pvti_enqueue_rx_bi_to_next_and_trace(vlib_main_t *vm, vlib_node_runtime_t *node, pvti_per_thread_data_t *ptd, u32 bi0, u16 next0)
{
...
always_inline pvti_rx_peer_t *pvti_try_find_or_create_rx_peer(pvti_per_thread_data_t *ptd, vlib_buffer_t *b0, bool is_ip6)
{
    pvti_rx_peer_t *peer;
    ...
    u16          remote_port;
    if (is_ip6) {
        pvti_ip6_encap_header_t *h0 = ((pvti_ip6_encap_header_t *) vlib_buffer_get_current(b0)) - 1;
        ip_address_set(&remote_ip, &h0->ip6.src_address, AF_IP6);
        remote_port = clib_net_to_host_u16(h0->udp.src_port);
    }
    else {
        pvti_ip4_encap_header_t *h0 = ((pvti_ip4_encap_header_t *) vlib_buffer_get_current(b0)) - 1;
    ...
    index_t pvti_if_index0 = pvti_if_find_by_remote_ip_and_port(&remote_ip, remote_port);
...
always_inline u16 pvti_input_node_common(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, bool is_ip6)
{
    ...
    while (n_left_from > 0) {
        ...
        pvti_ip4_encap_header_t *h0            = ((pvti_ip4_encap_header_t *) vlib_buffer_get_current(b0)) - 1;
        pvti_rx_peer_t          *pvti_rx_peer0 = pvti_try_find_or_create_rx_peer(ptd, b0, is_ip6);
        if (!pvti_rx_peer0) {
        ...
        u16 pvti_packet_header_sz0         = pvti0->pad_bytes + offsetof(pvti_packet_header_t, pad);
        ...
        pvti_per_rx_stream_data_t *rx_stream0 = &pvti_rx_peer0->rx_streams[stream_index];
        ...
        if (new_seq0 == rx_stream0->last_rx_seq + 1) {
            ...
            while ((b0->current_length > 0) && true_chunk_count < pvti0->reass_chunk_count) {
                ...
                const u16            chunk_payload_length = clib_net_to_host_u16(pvc0->total_chunk_length) - sizeof(*pvc0);
                vlib_buffer_advance(b0, sizeof(*pvc0));

                if (rx_stream0->rx_bi0 == INDEX_INVALID) {
                    clib_warning("RX internal error: not-first chunk but no wip block");
                }
                else {
                    vlib_buffer_t *rb0            = vlib_get_buffer(vm, rx_stream0->rx_bi0);
                    u16            allowed_length = PVTI_RX_MAX_LENGTH - rb0->current_length;
                    if (allowed_length > chunk_payload_length) {
                        // simple case - there is space in the buffer to fit
                        // the whole chunk
                        void *tail = vlib_buffer_put_uninit(rb0, chunk_payload_length);
                        clib_memcpy(tail, vlib_buffer_get_current(b0), chunk_payload_length);
                    }
                    else {
                        ...
                        void *tail = vlib_buffer_put_uninit(rb0, allowed_length);
                        clib_memcpy(tail, vlib_buffer_get_current(b0), allowed_length);
                        u16 remaining_payload_length = chunk_payload_length - allowed_length;
                        ...
                        if (INDEX_INVALID == nrbi0) {
                            ...
                            void *tail         = vlib_buffer_put_uninit(nrb0, remaining_payload_length);
                            clib_memcpy(tail, vlib_buffer_get_current(b0) + allowed_length, remaining_payload_length);
                        }
                    }
                    pvti_rx_peer0->rx_streams[stream_index].rx_received_inner_length += chunk_payload_length;
                    if (pvti_rx_peer0->rx_streams[stream_index].rx_received_inner_length
                        == pvti_rx_peer0->rx_streams[stream_index].rx_expected_inner_length) {
                        next0 = rx_stream0->rx_next0;
                        pvti_enqueue_rx_bi_to_next_and_trace(vm, node, ptd, rx_stream0->rx_bi0_first, next0);
                        pkts_decapsulated += 1;

                        // clean out the current reassemly state
                        rx_stream0->rx_bi0                                               = INDEX_INVALID;
                        rx_stream0->rx_bi0_first                                         = INDEX_INVALID;
                        pvti_rx_peer0->rx_streams[stream_index].rx_received_inner_length = 0;
                        pvti_rx_peer0->rx_streams[stream_index].rx_expected_inner_length = 0;
        ...
        while ((b0->current_length > 0) && true_chunk_count < max_chunk_count) {
            ...
            chunks[true_chunk_count]  = pvc0;
            true_chunk_count += 1;
            u16 total_chunk_length = clib_net_to_host_u16(pvc0->total_chunk_length);
            if (b0->current_length < total_chunk_length) {
                clib_warning("RX ERR: length 0x%x too big for a chunk", true_chunk_count);
                break;
            }
            u8 *pkt = (u8 *) (pvc0 + 1);
            u16 inner_length;
            ...
            switch (*pkt & 0xf0) {
                ...
                case 0x60:
                    ...
                    inner_length = clib_net_to_host_u16(*((u16 *) (pkt + 4))) + sizeof(ip6_header_t);
                    break;
                default:
            ...
            if (inner_length + sizeof(pvti_chunk_header_t) > total_chunk_length) {
                ...
                rx_stream0->rx_received_inner_length = total_chunk_length - sizeof(pvti_chunk_header_t);
                rx_stream0->last_rx_seq              = new_seq0;
    ...
    vlib_buffer_enqueue_to_next_vec(vm, node, &ptd->pending_rx_buffers, &ptd->pending_rx_nexts, vec_len(ptd->pending_rx_nexts));
    vec_reset_length(ptd->pending_rx_buffers);
    vec_reset_length(ptd->pending_rx_nexts);

    vlib_node_increment_counter(vm, node->node_index, PVTI_INPUT_ERROR_PROCESSED, pkts_processed);
    vlib_node_increment_counter(vm, node->node_index, PVTI_INPUT_ERROR_DECAPSULATED, pkts_decapsulated);
    vlib_node_increment_counter(vm, node->node_index, PVTI_INPUT_ERROR_NO_BUFFERS, decap_failed_no_buffers);
