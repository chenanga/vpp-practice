...
static_always_inline u32 ip6_vtcfl(u8 stream_index)
{
    u32 vtcfl = 0x6 << 28;
    vtcfl |= stream_index;
...
always_inline vlib_buffer_t *pvti_alloc_new_tx_buffer(vlib_main_t *vm)
{
...
always_inline bool pvti_find_or_try_create_tx_peer(vlib_main_t            *vm,
                                                   ...
                                                   u32                    *out_index)
{
    ...
    pool_foreach (peer, ptd->tx_peers) {
        if (peer->remote_port == remote_port && 0 == ip_address_cmp(remote_ip, &peer->remote_ip)) {
            if (peer->deleted) {
    ...
    u16 pvti_encap_overhead = (dst_ver == AF_IP6) ? sizeof(pvti_ip6_encap_header_t) : sizeof(pvti_ip4_encap_header_t);

    u16 pvti_packet_overhead = pvti_encap_overhead + sizeof(pvti_packet_header_t) + PVTI_ALIGN_BYTES;
...
always_inline bool pvti_try_get_tx_peer_index(vlib_main_t            *vm,
                                              pvti_per_thread_data_t *ptd,
                                              pvti_if_t              *pvti_if0,
                                              vlib_buffer_t          *b0,
                                              bool                    is_ip6,
                                              u32                    *out_index)
{
    if (pvti_if0->peer_address_from_payload) {
        ...
        return pvti_find_or_try_create_tx_peer(vm, ptd, pvti_if0, &remote_ip, pvti_if0->remote_port, out_index);
    }
    else {
        return pvti_find_or_try_create_tx_peer(vm, ptd, pvti_if0, &pvti_if0->remote_ip, pvti_if0->remote_port, out_index);
    }
    /* not reached */
}

always_inline void pvti_finalize_chunk(pvti_tx_peer_t *tx_peer, pvti_chunk_header_t *chunk_header, u8 *tail, bool is_reassembly_chunk)
{
    clib_memset(chunk_header, 0xab, sizeof(pvti_chunk_header_t));
    chunk_header->total_chunk_length = clib_host_to_net_u16(tail - (u8 *) chunk_header);
    tx_peer->chunk_count++;
...
always_inline pvti_output_next_t encap_pvti_buffer_ip46(vlib_main_t *vm, vlib_node_runtime_t *node, pvti_tx_peer_t *tx_peer, int is_ip6)
{
    ...
    vlib_buffer_advance(b0, -(sizeof(pvti_packet_header_t) + PVTI_ALIGN_BYTES));
    ...
    if (is_ip6_encap) {
        ...
        ve->udp.length   = clib_host_to_net_u16(b0->current_length - offsetof(pvti_ip6_encap_header_t, udp));
        ve->udp.checksum = 0;

        ve->ip6.ip_version_traffic_class_and_flow_label = ip6_vtcfl(stream_index);
        ...
        ve->udp.src_port = clib_host_to_net_u16(tx_peer->local_port);
        ve->udp.dst_port = clib_host_to_net_u16(tx_peer->remote_port);
        ve->udp.length   = clib_host_to_net_u16(b0->current_length - offsetof(pvti_ip4_encap_header_t, udp));
        ve->udp.checksum = 0;

        ve->ip4.ip_version_and_header_length = 0x45;
        ve->ip4.tos                          = 0;
        ve->ip4.length                       = clib_host_to_net_u16(b0->current_length);
        ve->ip4.fragment_id                  = clib_host_to_net_u16(tx_peer->current_tx_seq & 0xffff);
    ...
    return is_ip6_encap ? PVTI_OUTPUT_NEXT_IP6_LOOKUP : PVTI_OUTPUT_NEXT_IP4_LOOKUP;
}

always_inline void pvti_enqueue_tx_and_trace(vlib_main_t            *vm,
                                             vlib_node_runtime_t    *node,
                                             pvti_per_thread_data_t *ptd,
                                             vlib_buffer_t          *b0,
                                             u16                     next0,
                                             u8                      stream_index,
                                             pvti_tx_peer_t         *tx_peer)
{
    if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && tx_peer->is_bo0_traced)) {
        if (PREDICT_TRUE(vlib_trace_buffer(vm, node, next0, b0, /* follow_chain */ 0))) {
...
always_inline void pvti_enqueue_tx_drop_and_trace(vlib_main_t            *vm,
                                                  vlib_node_runtime_t    *node,
                                                  pvti_per_thread_data_t *ptd,
                                                  vlib_buffer_t          *b0,
                                                  u8                      stream_index)
...
always_inline bool pvti_flush_peer_and_recharge(vlib_main_t            *vm,
                                                vlib_node_runtime_t    *node,
                                                pvti_per_thread_data_t *ptd,
                                                u32                     tx_peer_index,
                                                u8                      stream_index,
                                                const bool              is_ip6)
{
    ...
    pvti_enqueue_tx_and_trace(vm, node, ptd, tx_peer->bo0, next0, stream_index, tx_peer);
...
always_inline u16 pvti_output_node_common(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, const bool is_ip6)
{
    ...
    pvti_per_thread_data_t *ptd          = vec_elt_at_index(pvm->per_thread_data[is_ip6], thread_index);
    ...
    while (1 && n_left_from > 0) {
        ...
        ib++;
        u32  bi0          = vlib_get_buffer_index(vm, b0);
        bool is_b0_traced = is_node_traced && ((b0->flags & VLIB_BUFFER_IS_TRACED) ? 1 : 0);
        pkts_processed += 1;
        ...
        u32        tx_peer_index;
        if (!pvti_try_get_tx_peer_index(vm, ptd, pvti_if0, b0, is_ip6, &tx_peer_index)) {
        ...
        if (tx_peer->bo0_max_current_length >= tx_peer->bo0->current_length + total_chunk_len) {
            /* Happy case, we can fit the entire new chunk */
            pvti_chunk_header_t *chunk_header = vlib_buffer_put_uninit(tx_peer->bo0, sizeof(pvti_chunk_header_t));
            u8                  *tail         = vlib_buffer_put_uninit(tx_peer->bo0, b0_len);
            vlib_buffer_t       *b0_curr;
            b0_curr = b0;
            while (b0_len > 0) {
                clib_memcpy(tail, vlib_buffer_get_current(b0_curr), b0_curr->current_length);
                tail += b0_curr->current_length;
                b0_len -= b0_curr->current_length;
                ASSERT((b0_len == 0) || (b0_curr->flags & VLIB_BUFFER_NEXT_PRESENT));
...
#define PVTI_TINY_PACKET_SZ 20
            int threshold_len = sizeof(pvti_chunk_header_t) + PVTI_TINY_PACKET_SZ;

            /* Can we fit anything meaningful into bo0 ? if not - flush */
            if (tx_peer->bo0_max_current_length <= tx_peer->bo0->current_length + threshold_len) {
                if (!pvti_flush_peer_and_recharge(vm, node, ptd, tx_peer_index, stream_index, is_ip6)) {
            ...
            while (b0_len > 0) {
                ASSERT(tx_peer->bo0_max_current_length > tx_peer->bo0->current_length);
                int copy_len =
                    clib_min(b0_curr->current_length - curr_b0_start_offset, tx_peer->bo0_max_current_length - tx_peer->bo0->current_length);
                tail = vlib_buffer_put_uninit(tx_peer->bo0, copy_len);
                clib_memcpy(tail, (u8 *) vlib_buffer_get_current(b0_curr) + curr_b0_start_offset, copy_len);
                tail += copy_len;
                b0_len -= copy_len;
                // Advance the start offset or reset it if we copied the entire
                // block
                curr_b0_start_offset = curr_b0_start_offset + copy_len == b0_curr->current_length ? 0 : curr_b0_start_offset + copy_len;
                ASSERT((b0_len == 0) || (curr_b0_start_offset > 0) || (b0_curr->flags & VLIB_BUFFER_NEXT_PRESENT));
                if (curr_b0_start_offset > 0) {
                    pvti_finalize_chunk(tx_peer, chunk_header, tail, is_reassembly);
                    tx_peer->is_bo0_traced |= is_b0_traced;
                    if (!pvti_flush_peer_and_recharge(vm, node, ptd, tx_peer_index, stream_index, is_ip6)) {
                    ...
                    is_reassembly = true;
                    chunk_header  = vlib_buffer_put_uninit(tx_peer->bo0, sizeof(pvti_chunk_header_t));
                }
                else {
                    if ((b0_curr->flags & VLIB_BUFFER_NEXT_PRESENT)) {
                        b0_curr = vlib_get_buffer(vm, b0_curr->next_buffer);
    ...
    for (i = 0; i < vec_len(ptd->tx_peers); i++) {
        if (ptd->tx_peers[i].chunk_count) {
            pvti_flush_peer_and_recharge(vm, node, ptd, i, stream_index, is_ip6);
            pkts_encapsulated += 1;
        }
    }

    vlib_buffer_enqueue_to_next_vec(vm, node, &ptd->pending_tx_buffers, &ptd->pending_tx_nexts, vec_len(ptd->pending_tx_nexts));
    vec_reset_length(ptd->pending_tx_buffers);
    vec_reset_length(ptd->pending_tx_nexts);

    vlib_node_increment_counter(vm, node->node_index, PVTI_OUTPUT_ERROR_ENCAPSULATED, pkts_encapsulated);
    vlib_node_increment_counter(vm, node->node_index, PVTI_OUTPUT_ERROR_PROCESSED, pkts_processed);
    vlib_node_increment_counter(vm, node->node_index, PVTI_OUTPUT_ERROR_CHOPPED, pkts_chopped);
    vlib_node_increment_counter(vm, node->node_index, PVTI_OUTPUT_ERROR_OVERFLOW, pkts_overflow);
    vlib_node_increment_counter(vm, node->node_index, PVTI_OUTPUT_ERROR_OVERFLOW_CANTFIT, pkts_overflow_cantfit);
