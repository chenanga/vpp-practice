...
always_inline u16 pvti_bypass_node_common(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, bool is_ip6)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (is_ip6) {
                pvti_index0 = pvti_if_find_by_remote_ip6_and_port(&ip60->src_address, clib_net_to_host_u16(udp0->src_port));
            }
            else {
                pvti_index0 = pvti_if_find_by_remote_ip4_and_port(&ip40->src_address, clib_net_to_host_u16(udp0->src_port));
            }
            if (pvti_index0 == INDEX_INVALID) goto exit;
        ...
        exit:
            ...
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                pvti_bypass_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
    ...
    vlib_node_increment_counter(vm, node->node_index, PVTI_BYPASS_ERROR_PROCESSED, pkts_processed);
