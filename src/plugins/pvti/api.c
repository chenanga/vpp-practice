...
static walk_rc_t pvti_if_send_details(index_t pvtii, void *data)
{
    ...
    rmp->_vl_msg_id = htons(VL_API_PVTI_INTERFACE_DETAILS + pvti_main.msg_id_base);
...
static void vl_api_pvti_interface_dump_t_handler(vl_api_pvti_interface_dump_t *mp)
{
    vl_api_registration_t *reg;
    ...
    if (sw_if_index == ~0)
        ...
        if (pvtii != INDEX_INVALID) pvti_if_send_details(pvtii, &ctx);
    }
}

static void vl_api_pvti_interface_create_t_handler(vl_api_pvti_interface_create_t *mp)
{
    ...
    u32                        underlay_fib_index = clib_host_to_net_u32(mp->interface.underlay_fib_index);
    pvti_peer_address_method_t peer_address_method =
        mp->interface.peer_address_from_payload ? PVTI_PEER_ADDRESS_FROM_PAYLOAD : PVTI_PEER_ADDRESS_FIXED;

    if (underlay_mtu == 0) {
        underlay_mtu = 1500;
    }

    rv = pvti_if_create(&local_ip, lport, &remote_ip, rport, peer_address_method, underlay_mtu, underlay_fib_index, &sw_if_index);

    REPLY_MACRO2(VL_API_PVTI_INTERFACE_CREATE_REPLY, { rmp->sw_if_index = htonl(sw_if_index); });
}

static void vl_api_pvti_interface_delete_t_handler(vl_api_pvti_interface_delete_t *mp)
{
