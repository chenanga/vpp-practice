...
static clib_error_t *test_ipsec_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    u64 seq_num;
    u32 sa_id;
    ...
    if (~0 != sa_id) {
        ...
        return clib_error_return(0, "unknown SA `%U'", format_unformat_error, input);
    }

    return (NULL);
}

static clib_error_t *test_ipsec_spd_outbound_perf_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    clib_error_t      *err        = 0;
    ...
    ipsec_key_t        ck         = {0};
    u8                 key_data[] = {31, 32, 33, 34, 35, 36, 37, 38, 39, 30, 31, 32, 33, 34, 35, 36};
...
#define BURST_MAX_SIZE 256
    ...
    /* creating a new SA */
    rv = ipsec_sa_add_and_lock(sa_id,
                               spi,
                               proto,
                               crypto_alg,
                               &ck,
    ...
    for (i = 0; i < flows; i++) {
        ...
        ip4_start                         = 16777216;
        p_vec[i].laddr.start.ip4.data_u32 = clib_host_to_net_u32(ip4_start + i * 32);
        p_vec[i].laddr.stop.ip4.data_u32  = clib_host_to_net_u32(ip4_start + i * 32);
        p_vec[i].raddr.start.ip4.data_u32 = clib_host_to_net_u32(ip4_start + i * 32);
        p_vec[i].raddr.stop.ip4.data_u32  = clib_host_to_net_u32(ip4_start + i * 32);
    }
    ...
    for (i = 0; i < n_lookup; i++) {
        ...
        if (flow_cache_enabled) {
            p0 = ipsec4_out_spd_find_flow_cache_entry(im,
                                                      0,
                                                      clib_net_to_host_u32(ip4_start + ((flows - 1) - rand_val[j]) * 32),
                                                      clib_net_to_host_u32(ip4_start + ((flows - 1) - rand_val[j]) * 32),
                                                      clib_net_to_host_u16(1),
                                                      clib_net_to_host_u16(1));
            if (p0) count_cached++;
        }
        if (p0 == NULL) {
            if (burst_enabled) {
                ...
                ipsec4_spd_5tuple_t ip4_5tuple = {.ip4_addr = {(ip4_address_t) src_addr, (ip4_address_t) dst_addr},
                                                  .port     = {1, 1},
                                                  .proto    = IP_PROTOCOL_UDP};

                if (k == burst_size) {
                    k = 0;
                    clib_memset(policies, 0, burst_size * sizeof(ipsec_policy_t *));
                    burst_counter += ipsec_output_policy_match_n(spd0, ip4_5tuples, policies, burst_size, flow_cache_enabled);
                    for (m = 0; m < burst_size; m++) {
                        ASSERT(policies[m] != 0);
                    }
                }

                clib_memcpy(ip4_5tuples + k, &ip4_5tuple, sizeof(ipsec4_spd_5tuple_t));
                k++;
            }
            else {
                p0 = ipsec_output_policy_match(spd0,
                                               IP_PROTOCOL_UDP,
        ...
        j++;
        if (!burst_enabled) ASSERT(p0 != 0);
    }

    if (burst_enabled && k > 0) {
        clib_memset(policies, 0, k * sizeof(ipsec_policy_t *));
        burst_counter += ipsec_output_policy_match_n(spd0, ip4_5tuples, policies, k, flow_cache_enabled);
        for (m = 0; m < k; m++) {
            ASSERT(policies[m] != 0);
        }
    }
    ...
    vlib_cli_output(vm, "Time to add %u flows: \t\t%12.10f s", flows, (t_add / vm->clib_time.clocks_per_second));
    vlib_cli_output(vm, "Average time to add 1 flow: \t\t%12.10f s", ((t_add / flows) / vm->clib_time.clocks_per_second));
    vlib_cli_output(vm, "Time to lookup %u flows: \t\t%12.10f s", flows, (t_look / vm->clib_time.clocks_per_second));
    vlib_cli_output(vm, "Average time to lookup 1 flow: \t\t%12.10f s", ((t_look / n_lookup) / vm->clib_time.clocks_per_second));

    vlib_cli_output(vm, " ");

    vlib_cli_output(vm, "Cycle CPU to add %u flows: \t\t%32lu cycles", flows, t_add);
    vlib_cli_output(vm, "Average cycle CPU to add 1 flow: \t%32lu cycles", t_add / flows);
    vlib_cli_output(vm, "Cycle CPU to lookup %u flows: \t%32lu cycles", flows, t_look);
    vlib_cli_output(vm, "Average cycle CPU to lookup 1 flow: \t%32lu cycles", t_look / n_lookup);

    if (count_slow_path || count_cached)
        vlib_cli_output(vm,
                        "flow cache hit rate: \t\t%12.10f\n cached: \t%d\n slow_path: \t%d",
                        ((float) count_cached) / ((float) count_cached + count_slow_path),
                        count_cached,
                        count_slow_path);

    if (burst_enabled) vlib_cli_output(vm, "Total number of packets matched in bursts: \t\t%d\n", burst_counter);
    ...
    vlib_cli_output(vm, "Time for test: \t%12.10f s", ((t1 - t0) / vm->clib_time.clocks_per_second));
