...
static void placeholder_session_reset_callback(session_t *s)
{
    clib_warning("called...");
}

static int placeholder_session_connected_callback(u32 app_index, u32 api_context, session_t *s, session_error_t err)
{
    clib_warning("called...");
    return 0;
}

static int placeholder_add_segment_callback(u32 client_index, u64 segment_handle)
{
    clib_warning("called...");
    return 0;
}

static int placeholder_del_segment_callback(u32 client_index, u64 segment_handle)
{
    clib_warning("called...");
    return 0;
}

static void placeholder_session_disconnect_callback(session_t *s)
{
    clib_warning("called...");
}

static int placeholder_session_accept_callback(session_t *s)
{
    clib_warning("called...");
    return 0;
}

static int placeholder_server_rx_callback(session_t *s)
{
    clib_warning("called...");
    return -1;
}
...
static int segment_manager_test_pressure_1(vlib_main_t *vm, unformat_input_t *input)
{
    int                rv;
    ...
    sm = segment_manager_get(SEGMENT_MANAGER_GET_INDEX_FROM_HANDLE(attach_args.segment_handle));
    ...
    SEG_MGR_TEST((rv == MEMORY_PRESSURE_NO_PRESSURE), "fifo_segment_get_mem_status %s", states_str[rv]);

    /* allocate a fifo : 128KB x2 */
    rv = segment_manager_alloc_session_fifos(sm, vlib_get_thread_index(), &rx_fifo, &tx_fifo);
    ...
    /* 10 chunks : 61% */
    rv = fifo_segment_get_mem_status(fs);
    SEG_MGR_TEST((rv == MEMORY_PRESSURE_LOW_PRESSURE), "fifo_segment_get_mem_status %s", states_str[rv]);
    ...
    /* 14 chunks : 85% */
    rv = fifo_segment_get_mem_status(fs);
    SEG_MGR_TEST((rv == MEMORY_PRESSURE_HIGH_PRESSURE), "fifo_segment_get_mem_status %s", states_str[rv]);
...
static int segment_manager_test_pressure_2(vlib_main_t *vm, unformat_input_t *input)
{
    int                rv, i;
    segment_manager_t *sm;
    fifo_segment_t    *fs0, *fs;
...
static int segment_manager_test_fifo_balanced_alloc(vlib_main_t *vm, unformat_input_t *input)
{
    int                rv, i, fs_index;
    segment_manager_t *sm;
    fifo_segment_t    *fs[4];
    ...
    /* allocate fifos : 4KB x2 */
    rv = segment_manager_alloc_session_fifos(sm, vlib_get_thread_index(), &rx_fifo[0], &tx_fifo[0]);
    SEG_MGR_TEST((rv == 0), "segment_manager_alloc_session_fifos %d", rv);
    SEG_MGR_TEST((rx_fifo[0]->segment_index == 0), "segment_index %d", rx_fifo[0]->segment_index);
    SEG_MGR_TEST((tx_fifo[0]->segment_index == 0), "segment_index %d", tx_fifo[0]->segment_index);
     ...
     */
    rv = segment_manager_alloc_session_fifos(sm, vlib_get_thread_index(), &rx_fifo[1], &tx_fifo[1]);
    SEG_MGR_TEST((rv == 0), "segment_manager_alloc_session_fifos %d", rv);
    SEG_MGR_TEST((rx_fifo[1]->segment_index == 1), "segment_index %d", rx_fifo[1]->segment_index);
    SEG_MGR_TEST((tx_fifo[1]->segment_index == 1), "segment_index %d", tx_fifo[1]->segment_index);

    /* allocate fifos : 4KB x2
     * expected to be allocated on the newer segment.
     */
    rv = segment_manager_alloc_session_fifos(sm, vlib_get_thread_index(), &rx_fifo[2], &tx_fifo[2]);
    SEG_MGR_TEST((rv == 0), "segment_manager_alloc_session_fifos %d", rv);
    SEG_MGR_TEST((rx_fifo[2]->segment_index == 1), "segment_index %d", rx_fifo[2]->segment_index);
    SEG_MGR_TEST((tx_fifo[2]->segment_index == 1), "segment_index %d", tx_fifo[2]->segment_index);
     ...
     */
    rv = segment_manager_alloc_session_fifos(sm, vlib_get_thread_index(), &rx_fifo[3], &tx_fifo[3]);
    SEG_MGR_TEST((rv == 0), "segment_manager_alloc_session_fifos %d", rv);
    SEG_MGR_TEST((rx_fifo[3]->segment_index == 0), "segment_index %d", rx_fifo[3]->segment_index);
    SEG_MGR_TEST((tx_fifo[3]->segment_index == 0), "segment_index %d", tx_fifo[3]->segment_index);
...
__clib_unused static int segment_manager_test_fifo_ops(vlib_main_t *vm, unformat_input_t *input)
{
    int                rv, i;
    segment_manager_t *sm;
    fifo_segment_t    *fs;
    ...
    SEG_MGR_TEST((rx_fifo->shr->size == most_grown), "rx_fifo->size %u", rx_fifo->shr->size);
...
static int segment_manager_test_prealloc_hdrs(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    segment_manager_parse_segment_handle(attach_args.segment_handle, &sm_index, &fs_index);
    sm = segment_manager_get(sm_index);

    SEG_MGR_TEST((sm != 0), "seg manager is valid", sm);

    fs = segment_manager_get_segment(sm, fs_index);
    SEG_MGR_TEST(fifo_segment_num_free_fifos(fs) == prealloc_hdrs, "prealloc should be %u", prealloc_hdrs);
...
static clib_error_t *segment_manager_test(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
    int                           res  = 0;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "pressure_levels_1"))
            ...
            if ((res = segment_manager_test_pressure_1(vm, input))) goto done;
            if ((res = segment_manager_test_pressure_2(vm, input))) goto done;
            if ((res = segment_manager_test_fifo_balanced_alloc(vm, input))) goto done;
            if ((res = segment_manager_test_prealloc_hdrs(vm, input))) goto done;
        }
        else
            break;
    }
    ...
    if (res) return clib_error_return(0, "Segment manager unit test failed.");
    return 0;
}
...
VLIB_CLI_COMMAND(tcp_test_command, static) = {
    ...
    .short_help =
        "test segment manager [pressure_levels_1]"
