...
static clib_error_t *test_hash_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    baseaddr = mmap(NULL, 8192, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0 /* offset */);
...
static void *leak_memory_fn(void *args)
{
    u8 *p = 0;
    vec_validate(p, 100);
    p = 0;
...
static clib_error_t *test_mem_leak_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
