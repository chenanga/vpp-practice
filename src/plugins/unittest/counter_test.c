 ...
 */
static uint64_t get_stats_epoch()
{
 ...
 * is expanded.
 */
static clib_error_t *test_simple_counter_expand(vlib_main_t *vm)
{
    ...
    for (i = 0; i < EXPAND_TEST_ROUNDS; i++) {
        ...
        for (index = 1; index <= mem_size - 1; index++) {
            ...
            if (new_epoch != epoch) return clib_error_return(0, "Stats segment epoch should not increase");
        }
        ...
        new_epoch = get_stats_epoch();
        if (new_epoch == epoch) return clib_error_return(0, "Stats segment epoch should have increased");
        epoch = new_epoch;
    }
 ...
 * is expanded.
 */
static clib_error_t *test_combined_counter_expand(vlib_main_t *vm)
{
...
static clib_error_t *test_simple_counter(vlib_main_t *vm, int test_case)
{
    clib_error_t *error;
...
static clib_error_t *test_combined_counter(vlib_main_t *vm, int test_case)
{
    clib_error_t *error;
...
static clib_error_t *test_counter_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    clib_error_t *error;
    int           counter_type = -1;
    int           test_case    = -1;
    ...
    if (test_case == -1) return clib_error_return(0, "no such test");
...
static clib_error_t *test_counter_init(vlib_main_t *vm)
{
    return (0);
}
