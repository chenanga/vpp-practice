...
// ipv4
u8 gso_ipv4_tcp_data[64] = {0x02, 0xfe, 0x39, 0xe5, 0x09, 0x8f, 0x02, 0xfe, 0x2d, 0x18, 0x63, 0x18, 0x08, 0x00, 0x45, 0x00,
...
// ipv6
u8 gso_ipv6_tcp_data[] = {0x02, 0xfe, 0x39, 0xe5, 0x09, 0x8f, 0x02, 0xfe, 0x2d, 0x18, 0x63, 0x18, 0x08, 0x00, 0x60, 0x0d, 0xf4,
                          0x97, 0x00, 0x40, 0x06, 0x40, 0xfd, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                          0x00, 0x00, 0x10, 0x00, 0xfd, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                          0x00, 0x10, 0x01, 0xd8, 0xde, 0x14, 0x51, 0x34, 0x93, 0xa8, 0x1b, 0x7b, 0xef, 0x2e, 0x7e, 0x80, 0x10,
                          0x00, 0xe5, 0xc7, 0x03, 0x00, 0x00, 0x01, 0x01, 0x08, 0x0a, 0xce, 0xaa, 0x00, 0x2f, 0xf2, 0xc3};
 ...
 */
static void set_hdr_offsets(vlib_buffer_t *b0, u8 is_l2)
{
...
static u32
fill_buffers(vlib_main_t *vm, u32 *buffer_indices, gso_test_data_t *gso_test_data, u32 n_buffers, u32 buffer_size, u32 packet_size, u32 gso_size)
{
    u32 i;
    ...
    for (i = 0; i < n_buffers; i++) {
        ...
        u32            len            = 0;
        u32            remaining_data = (packet_size > buffer_size) ? (packet_size - buffer_size) : 0;
        ...
        if (remaining_data) {
            vlib_buffer_t *pb = b;
            u32            n_alloc, n_bufs = ((remaining_data + buffer_size - 1) / buffer_size);
            u32           *buffers = 0;
            u32            fill_data_size;
            u32            k = 0;
            ...
            if (n_alloc < n_bufs) {
                ...
                vlib_cli_output(vm, "vlib buffer alloc failed at %u requested %u actual %u", i, n_bufs, n_alloc);
                return i;
            }
...
static_always_inline u32 gso_segment_buffer_test(vlib_main_t *vm, u32 bi, vnet_interface_per_thread_data_t *ptd, u8 is_l2)
{
...
static clib_error_t *test_gso_perf(vlib_main_t *vm, gso_test_main_t *gtm)
{
    clib_error_t                     *err         = 0;
    ...
    if (gtm->buffer_size > buffer_size) return clib_error_return(0, "buffer size must be <= %u", buffer_size);

    if (gtm->packet_size > packet_size) return clib_error_return(0, "gso packet size must be <= %u", packet_size);

    if ((gtm->gso_size > MAX_GSO_SEGMENT_SIZE) || (gtm->gso_size < MIN_GSO_SEGMENT_SIZE))
        return clib_error_return(0, "gso segment size must be in between %u >= and <= %u", MIN_GSO_SEGMENT_SIZE, MAX_GSO_SEGMENT_SIZE);
    ...
    while (gso_test_data) {
        ...
        n_filled = fill_buffers(vm, buffer_indices, gso_test_data, n_buffers, buffer_size, packet_size, gso_size);
        ...
        for (i = 0; i < 10; i++) {
            ...
            vlib_cli_output(vm, "%-2u: %.03f ticks/packet, %.02f Kpps, %.02f Gbps\n", i + 1, tpp1, Kpps1, Gbps1);
        }
...
static clib_error_t *test_gso_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    gso_test_main_t *gtm = &gso_test_main;
    clib_error_t    *err = 0;
...
VLIB_CLI_COMMAND(test_gso_command, static) = {
    .path = "test gso",
    .short_help =
        "test gso [buffers <n>] [buffer-size <size>] [packet-size "
        "<size>] [gso-size <size>] [rounds <n>] "
        "[warmup-rounds <n>]",
    .function = test_gso_command_fn,
};

static clib_error_t *gso_test_init(vlib_main_t *vm)
{
    return (0);
}
