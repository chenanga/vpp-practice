...
#define BIER_TEST_I(_cond, _comment, _args...)                                                     \
    ({                                                                                             \
        ...
        if (!(_evald)) {                                                                           \
            ...
            if (bier_test_do_debug) fformat(stderr, "PASS:%d: " _comment "\n", __LINE__, ##_args); \
        }                                                                                          \
        res;                                                                                       \
    })
...
static int bier_test_mk_intf(u32 ninterfaces)
{
...
static int bier_test_validate_entry(index_t bei, int n_buckets, ...)
{
    ...
    res = BIER_TEST_I((DPO_LOAD_BALANCE == dpo.dpoi_type), "Entry links to %U", format_dpo_type, dpo.dpoi_type);
...
static int bier_test_mpls_spf(void)
{
...
#define N_BIER_ECMP_TABLES 16
    ...
    for (ii = 0; ii < N_BIER_ECMP_TABLES; ii++) {
        ...
        l_o_bt[ii].bier.table = bier_table_ecmp_create_and_lock(&bt_ecmp_0_0_0_256);
    };
    ...
    mpls_fib_index = fib_table_find(FIB_PROTOCOL_MPLS, MPLS_FIB_DEFAULT_TABLE_ID);
    ...
    BIER_TEST(!fib_test_validate_entry(lfei,
                                       ...
                                       &l_o_bt[14],
                                       &l_o_bt[15]),
              "1600/1 LB stacks on BIER table %d",
              bti);
    ...
    BIER_TEST(!fib_test_validate_entry(lfei,
                                       ...
                                       &l_o_bt[14],
                                       &l_o_bt[15]),
              "1601/1 LB stacks on BIER table %d",
              bti);
    ...
    fib_entry_contribute_forwarding(fei, FIB_FORW_CHAIN_TYPE_MPLS_NON_EOS, &neos_dpo_1_1_1_1);

    bfmi_1_1_1_1 = bier_fmask_db_find(bti, &path_1_1_1_1);
    bfm_1_1_1_1  = bier_fmask_get(bfmi_1_1_1_1);

    BIER_TEST(!dpo_cmp(drop_dpo_get(DPO_PROTO_MPLS), &bfm_1_1_1_1->bfm_dpo), "Fmask via 1.1.1.1 stacks on MPLS drop");
    ...
    BIER_TEST(!dpo_cmp(&dpo_bei, drop_dpo_get(DPO_PROTO_BIER)), "BP:1 stacks on bier drop");
    ...
    fib_entry_contribute_forwarding(fei, FIB_FORW_CHAIN_TYPE_MPLS_NON_EOS, &neos_dpo_1_1_1_1);
    BIER_TEST(!fib_test_validate_lb(&neos_dpo_1_1_1_1, 1, &bucket_neos_99_via_10_10_10_1), "1.1.1.1/32 n-eos LB 1 buckets via: 99 + 10.10.10.1");
    BIER_TEST(!dpo_cmp(&neos_dpo_1_1_1_1, &bfm_1_1_1_1->bfm_dpo), "Fmask via 1.1.1.1 stacks on updated non-eos of 1.1.1.1/32");
    bier_entry_contribute_forwarding(bei_1, &dpo_bei);
    BIER_TEST((dpo_bei.dpoi_index == bfmi_1_1_1_1), "BP:1  stacks on fmask 1.1.1.1");
    ...
    ai_mpls_10_10_10_2 = adj_nbr_add_or_lock(FIB_PROTOCOL_IP4, VNET_LINK_MPLS, &nh_10_10_10_2, tm->hw[0]->sw_if_index);
    ...
    fib_entry_contribute_forwarding(fei, FIB_FORW_CHAIN_TYPE_MPLS_NON_EOS, &neos_dpo_1_1_1_1);
    BIER_TEST(!fib_test_validate_lb(&neos_dpo_1_1_1_1, 2, &bucket_neos_99_via_10_10_10_1, &bucket_neos_100_via_10_10_10_2),
    ...
    BIER_TEST((dpo_bei.dpoi_index == bfmi_1_1_1_1), "BP:2  stacks on fmask 1.1.1.1");
    ...
    fib_route_path_t *paths_1_1_1_2 = NULL, *input_paths_1_1_1_2,
                     path_1_1_1_2   = {
                           .frp_addr        = nh_1_1_1_2,
                           .frp_sw_if_index = ~0,
                     };
    ...
    fib_entry_contribute_forwarding(fei, FIB_FORW_CHAIN_TYPE_MPLS_NON_EOS, &neos_dpo_1_1_1_2);

    bfmi_1_1_1_2 = bier_fmask_db_find(bti, &path_1_1_1_2);
    bfm_1_1_1_2  = bier_fmask_get(bfmi_1_1_1_2);

    BIER_TEST(!dpo_cmp(&neos_dpo_1_1_1_2, &bfm_1_1_1_2->bfm_dpo), "Fmask via 1.1.1.2 stacks on non-eos of 1.1.1.2/32");
    ...
    BIER_TEST((dpo_bei.dpoi_index == bfmi_1_1_1_2), "BP:2 stacks on fmask 1.1.1.2");
    ...
    BIER_TEST(!bier_test_validate_entry(bei_3, 2, &dpo_o_bfm_1_1_1_1, &dpo_o_bfm_1_1_1_2), "BP:3 stacks on fmask 1.1.1.2 & 1.1.1.1");

    /*
     * test that the ECMP choices for BP:3 have been spread over the
     * ECMP tables
     */
    BIER_TEST((bier_table_fwd_lookup(bier_table_get(l_o_bt[0].bier.table), 3) == bfmi_1_1_1_1), "fwd lookup for BP:3 ECMP:0 is 1.1.1.1");
    BIER_TEST((bier_table_fwd_lookup(bier_table_get(l_o_bt[1].bier.table), 3) == bfmi_1_1_1_2), "fwd lookup for BP:3 ECMP:1 is 1.1.1.2");
    ...
    bier_entry_contribute_forwarding(bei_3, &dpo_bei);
    BIER_TEST((dpo_bei.dpoi_index == bfmi_1_1_1_1), "BP:3 stacks on fmask 1.1.1.1");

    BIER_TEST((bier_table_fwd_lookup(bier_table_get(l_o_bt[0].bier.table), 3) == bfmi_1_1_1_1), "fwd lookup for BP:3 ECMP:0 is 1.1.1.1");
    BIER_TEST((bier_table_fwd_lookup(bier_table_get(l_o_bt[1].bier.table), 3) == bfmi_1_1_1_1), "fwd lookup for BP:3 ECMP:1 is 1.1.1.1");
    ...
    BIER_TEST(lb_count + 1 == pool_elts(load_balance_pool), "Load-balance resources freed ");
    BIER_TEST((0 == adj_nbr_db_size()), "ADJ DB size is %d", adj_nbr_db_size());
...
static int bier_test_mpls_imp(void)
{
    ...
    mfib_table_entry_path_update(0,  // default table
                                 ...
                                 &path_via_bier_imp_1);
    mfib_table_entry_delete(0, &pfx_1_1_1_1_c_239_1_1_1, MFIB_SOURCE_API);

    bier_imp_unlock(bii);
    bier_table_unlock(&bt_0_0_0_256);

    BIER_TEST(0 == pool_elts(bier_imp_pool), "BIER imposition resources freed ");
    BIER_TEST(0 == pool_elts(bier_table_pool), "BIER table resources freed ");

    return (0);
}

static int bier_test_mpls_disp(void)
{
    /*
     * Add the BIER Main table
     */
    ...
    BIER_TEST(!dpo_cmp(&dpo_disp_tbl_1, &bfm_0_0_0_0->bfm_dpo), "Fmask via 0.0.0.0 stacks on BIER disp table 1");
    ...
    bier_disp_table_entry_path_add(bier_disp_tbl_id, src, BIER_HDR_PROTO_IPV4, rpaths);
    ...
    BIER_TEST((bdti1 == lkd->lkd_fib_index), "disp is deag in %d %U", lkd->lkd_fib_index, format_dpo_id, dpo_disp_entry_v4, 0);
    BIER_TEST((LOOKUP_INPUT_DST_ADDR == lkd->lkd_input), "disp is destination deag in %d %U", lkd->lkd_input, format_dpo_id, dpo_disp_entry_v4, 0);
    BIER_TEST((LOOKUP_MULTICAST == lkd->lkd_cast), "disp is multicast deag in %d %U", lkd->lkd_input, format_dpo_id, dpo_disp_entry_v4, 0);
    ...
    bier_disp_table_entry_path_remove(bier_disp_tbl_id, src, BIER_HDR_PROTO_IPV4, rpaths);
    ...
    BIER_TEST(0 == pool_elts(bier_fmask_pool), "BIER fmask resources freed ");
    BIER_TEST(0 == pool_elts(bier_table_pool), "BIER table resources freed ");
    BIER_TEST(0 == pool_elts(bier_disp_table_pool), "BIER Disposition table resources freed ");
    BIER_TEST(0 == pool_elts(bier_disp_entry_pool), "BIER Disposition entry resources freed ");

    vec_free(paths_via_disp);
    return (0);
}

static clib_error_t *bier_test(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
    int res = 0;
...
clib_error_t *bier_test_init(vlib_main_t *vm)
{
    return 0;
}
