...
static clib_error_t *policer_test(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
    int              policer_index, i;
    ...
    for (i = 0; i < num_pkts; i++) {
        ...
        vlib_increment_combined_counter(&policer_counters[result], 0, policer_index, 1, PKT_LEN);
    }
...
clib_error_t *policer_test_init(vlib_main_t *vm)
{
    return 0;
}
