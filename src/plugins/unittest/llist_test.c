...
#define list_elt_is_sane(pl, name, E, P, N) (E->name.next == (N) -pl && E->name.prev == (P) -pl && P->name.next == (E) -pl && N->name.prev == (E) -pl)
...
#define list_test_is_sane(pl, name, h)                                                                                             \
    do {                                                                                                                           \
        ...
        clib_llist_foreach(pl, name, h, e, ({                                                                                      \
                               rv = list_elt_is_sane((pl), name, (e), clib_llist_prev(pl, name, e), clib_llist_next(pl, name, e)); \
                               if (!rv)                                                                                            \
                                   LLIST_TEST(0,                                                                                   \
                                              "invalid elt %u prev %u/%u next %u/%u",                                              \
                                              e - pl,                                                                              \
                                              e->name.prev,                                                                        \
                                              clib_llist_prev(pl, name, e) - pl,                                                   \
                                              e->name.next,                                                                        \
                                              clib_llist_next(pl, name, e) - pl);                                                  \
                           }));                                                                                                    \
    } while (0)

static int llist_test_basic(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    LLIST_TEST(he == clib_llist_next(pelts, ll_test, he), "should be the same");
    LLIST_TEST(he == clib_llist_prev(pelts, ll_test, he), "should be the same");
    ...
    clib_llist_foreach(pelts, ll_test, he, e, ({
                           if (i != e->data) LLIST_TEST(0, "incorrect element i = %u data = %u", i, e->data);
                           i--;
                       }));
    ...
    he = clib_llist_elt(pelts, head);
    list_test_is_sane(pelts, ll_test, he);
    LLIST_TEST(clib_llist_is_empty(pelts, ll_test, he), "list should be empty");
    ...
    LLIST_TEST(!clib_llist_is_empty(pelts, ll_test2, he2), "list should not be empty");
    ...
    while (e != he2) {
        ...
        if (e->data != i) LLIST_TEST(0, "incorrect element i = %u data = %u", i, e->data);
    ...
    e = clib_llist_prev(pelts, ll_test, he);
    LLIST_TEST(e->data == 300, "data for last spliced should be 300 is %u", e->data);
    for (i = 0; i < 10; i++) {
        if (e->data != 300 + i) LLIST_TEST(0, "incorrect element i = %u data = %u", i, e->data);
        e = clib_llist_prev(pelts, ll_test, e);
    }

    LLIST_TEST(e->data == old_tail, "data should be old tail %u is %u", old_tail, e->data);
...
static clib_error_t *llist_test(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
    int res = 0;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "basic")) {
            ...
            if ((res = llist_test_basic(vm, input))) goto done;
        }
        else
            break;
    }
    ...
    if (res) return clib_error_return(0, "llist unit test failed");
    return 0;
}
