...
static int
build_chain(vlib_main_t *vm, const chained_buffer_template_t *tmpl, u32 n, clib_random_buffer_t *randbuf, u8 **rand, vlib_buffer_t **b_, u32 *bi_)
{
    vlib_buffer_t *bufs[2 * VLIB_BUFFER_LINEARIZE_MAX], **b    = bufs;
    u32            bis[2 * VLIB_BUFFER_LINEARIZE_MAX + 1], *bi = bis;
    u32            n_alloc;

    if (rand) vec_reset_length(*rand);
    ...
    while (n > 0) {
        ...
        if (rand) {
            ...
            if (len) {
                ...
                const void *src = vec_elt_at_index(*rand, vec_len(*rand) - len);
...
static int check_chain(vlib_main_t *vm, vlib_buffer_t *b, const u8 *rand)
{
    int len_chain = vlib_buffer_length_in_chain(vm, b);
    int len;

    /* check for data corruption */
    if (clib_memcmp(vlib_buffer_get_current(b), vec_elt_at_index(rand, 0), b->current_length)) return 0;
    len = b->current_length;
    while (b->flags & VLIB_BUFFER_NEXT_PRESENT) {
        b = vlib_get_buffer(vm, b->next_buffer);
        if (clib_memcmp(vlib_buffer_get_current(b), vec_elt_at_index(rand, len), b->current_length)) return 0;
        len += b->current_length;
    }

    /* check for data truncation */
    if (len != vec_len(rand)) return 0;

    /* check total length update is correct */
    if (len != len_chain) return 0;

    return 1;
}

static int test_chain(vlib_main_t                     *vm,
                      ...
                      u8                             **rand)
{
    ...
    if (!build_chain(vm, tmpl, n, randbuf, rand, &b, bi)) goto err0;

    if (clone_off) {
        if (2 != vlib_buffer_clone(vm, bi[0], bi, 2, clone_off)) goto err1;
        b = vlib_get_buffer(vm, bi[0]);
    }

    if (!(ret = vlib_buffer_chain_linearize(vm, b))) goto err2;
...
err2:
    if (clone_off) vlib_buffer_free_one(vm, bi[1]);
err1:
...
static int linearize_test(vlib_main_t *vm)
{
    ...
    TEST(2 == test_chain(vm, tmpl, 2, 0, &randbuf, &rand), "linearize chain with negative current data");
    ...
    TEST(1 == test_chain(vm, tmpl, 2, 0, &randbuf, &rand), "linearize chain with empty next");
    ...
    TEST(1 == test_chain(vm, tmpl, 4, 0, &randbuf, &rand), "linearize chain into a single buffer");
    ...
    TEST(2 == test_chain(vm, tmpl, 4, data_size - 1, &randbuf, &rand), "linearize cloned chain");
    ...
    for (i = 0; i < 100; i++) {
        ...
        TEST(test_chain(vm, tmpl, n, r[0] > 250 ? r[0] % 128 : 0, &randbuf, &rand), "linearize random chain %d", i);
    }
...
static clib_error_t *test_linearize_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *test_linearize_speed_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    /* typical 9000-bytes TCP jumbo frames */
    const chained_buffer_template_t tmpl[5] = {{14, 2034, 1}, {0, 2048, 1}, {0, 2048, 1}, {0, 2048, 1}, {0, 808, 1}};
    int                             i, j;
    ...
    for (i = 0; i < 10; i++) {
        ...
        for (j = 0; j < 100000; j++) {
            ...
            if (!build_chain(vm, tmpl, 5, 0, 0, &b, &bi)) return clib_error_create("build_chain() failed");
