...
static_always_inline void compute_ip_phc(void *p)
{
    if ((((u8 *) p)[0] & 0xf0) == 0x40)
...
void compute_ip_phc_func(void **p, u32 n_packets)
{
...
// ipv4
u8 phc_ipv4_tcp_data[50] = {0x45, 0x00, 0x05, 0xdc, 0xdb, 0x42, 0x40, 0x00, 0x40, 0x06, 0xc4, 0x85, 0xc0, 0xa8, 0x0a, 0x02, 0xc0,
...
u8 phc_ipv6_udp_data[65] = {
    0x60, 0x0d, 0xf4, 0x97, 0x00, 0x40, 0x3a, 0x40, 0xfd, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x10, 0x00, 0xfd, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x01, 0x80, 0x00, 0x10, 0x84,
    0xb1, 0x25, 0x00, 0x01, 0x22, 0x57, 0xf0, 0x60, 0x00, 0x00, 0x00, 0x00, 0xcb, 0x4a, 0x07, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10,
};
...
static void fill_buffers(vlib_main_t *vm, u32 *buffer_indices, u8 *data, u32 data_size, u32 n_buffers, u32 buffer_size)
{
    int i, j;
...
static clib_error_t *test_phc_perf(vlib_main_t *vm, phc_test_main_t *ptm)
{
    clib_error_t    *err         = 0;
    ...
    if (ptm->buffer_size > buffer_size) return clib_error_return(0, "buffer size must be <= %u", buffer_size);
    ...
    while (phc_test_data) {
        fill_buffers(vm, buffer_indices, phc_test_data->data, phc_test_data->data_size, n_buffers, buffer_size);
...
static clib_error_t *test_phc_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    phc_test_main_t *ptm = &phc_test_main;
    clib_error_t    *err = 0;
...
VLIB_CLI_COMMAND(test_phc_command, static) = {
    .path = "test phc",
    .short_help =
        "test phc [buffers <n>] [buffer-size <size>] [rounds <n>] "
        "[warmup-rounds <n>]",
    .function = test_phc_command_fn,
};

static clib_error_t *phc_test_init(vlib_main_t *vm)
{
    return (0);
}
