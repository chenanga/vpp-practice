...
static clib_error_t *check_bitmap(const char *test_name, const uword *bm, u32 expected_len, ...)
{
...
static clib_error_t *check_bitmap_will_expand(const char *test_name, uword **bm, uword index, bool expected_will_expand)
{
...
static clib_error_t *test_bitmap_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    clib_error_t *error = 0;
    uword        *bm    = 0;
    uword        *bm2   = 0;
     ...
     */
    bm    = clib_bitmap_set_multiple(bm, 2, pow2_mask(BITS(uword) - 3), BITS(uword));
    error = check_bitmap("clib_bitmap_set_multiple 3", bm, 2, pow2_mask(BITS(uword) - 3) << 2, 0);
    if (error != 0) goto done;
    ...
    error = check_bitmap("clib_bitmap_xori 2", bm, 2, pow2_mask(BITS(uword) - 3) << 2, 1ULL << (BITS(uword) - 1));
    if (error != 0) goto done;
    ...
    error = check_bitmap("clib_bitmap_andi", bm, 2, 4, 1ULL << (BITS(uword) - 1));
    if (error != 0) goto done;
    ...
    bm2   = clib_bitmap_set_multiple(bm2, BITS(uword) + 2, ~0ULL, BITS(uword) - 3);
    ...
    error = check_bitmap("clib_bitmap_set_region 1", bm, 4, ~0ULL << 2, ~0ULL, ~0ULL, ~0ULL);
    if (error != 0) goto done;
    ...
    error = check_bitmap("clib_bitmap_set_region 2", bm, 4, ~0ULL << 2, ~0ULL, ~0ULL, pow2_mask(BITS(uword) - 5));
    if (error != 0) goto done;

    error = check_bitmap_will_expand("clib_bitmap_will_expand 1", &bm, 0, 0);
    if (error != 0) goto done;

    error = check_bitmap_will_expand("clib_bitmap_will_expand 2", &bm, vec_max_len(bm) * BITS(uword) - 1, 0);
    if (error != 0) goto done;

    error = check_bitmap_will_expand("clib_bitmap_will_expand 3", &bm, vec_max_len(bm) * BITS(uword), 1);
    if (error != 0) goto done;
