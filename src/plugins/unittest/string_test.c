...
static int test_memset_s(vlib_main_t *vm, unformat_input_t *input)
{
    u8      dst[64];
    int     i;
    errno_t err;
...
static int test_clib_memset(vlib_main_t *vm, unformat_input_t *input)
{
    u8      dst[64];
    int     i;
    errno_t err;
...
static int test_memcpy_s(vlib_main_t *vm, unformat_input_t *input)
{
    char    src[64], dst[64];
    int     i;
    errno_t err;
...
static int test_clib_memcpy(vlib_main_t *vm, unformat_input_t *input)
{
    char    src[64], dst[64];
    int     i;
    errno_t err;
...
static int test_memcmp_s(vlib_main_t *vm, unformat_input_t *input)
{
    char    src[64], dst[64];
    errno_t err;
    int     diff = 0;
...
static int test_clib_memcmp(vlib_main_t *vm, unformat_input_t *input)
{
    char    src[64], dst[64];
    errno_t err;
    char   *s;
    ...
    /* s1 > s2, > 0 is expected in diff */
    if (!(clib_memcmp(dst, src, ARRAY_LEN(src)) > 0)) return -1;
    /* verify it against memcmp */
    if (!(memcmp(dst, src, ARRAY_LEN(src)) > 0)) return -1;

    /* s1 < s2, < 0 is expected in diff */
    if (!(clib_memcmp(src, dst, ARRAY_LEN(dst)) < 0)) return -1;
    /* verify it against memcmp */
    if (!(memcmp(src, dst, ARRAY_LEN(dst)) < 0)) return -1;
    ...
    /* Try negative tests */
    s = 0;
    if (clib_memcmp(s, s, 0) != 0) return -1;
    /* verify it against memcmp */
    if (memcmp(s, s, 0) != 0) return -1;

    /* OK, seems to work */
    return 0;
}

static int test_strcmp_s(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    /* s1 == s2, 0 is expected */
    err = strcmp_s(s1, s1len, "Simplicity is the ultimate sophistication", &indicator);
    if (err != EOK) return -1;
...
static int test_clib_strcmp(vlib_main_t *vm, unformat_input_t *input)
{
    char  s1[] = "Simplicity is the ultimate sophistication";
    int   indicator;
    char *s;
...
static int test_strncmp_s(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    /* s1 == s2, 0 is expected */
    err = strncmp_s(s1, s1len, "Every moment is a fresh beginning", s1len, &indicator);
    if (err != EOK) return -1;
    if (indicator != 0) return -1;

    /* s1 > s2, 0 is expected since comparison is no more than n character */
    err = strncmp_s(s1, s1len, "Every moment is a fresh begin", sizeof("Every moment is a fresh begin") - 1, &indicator);
    if (err != EOK) return -1;
    if (indicator != 0) return -1;

    /* s1 < s2, < 0 is expected */
    err = strncmp_s(s1, s1len, "Every moment is fresh beginning", sizeof("Every moment is fresh beginning") - 1, &indicator);
    if (err != EOK) return -1;
    if (!(indicator < 0)) return -1;

    /* s1 > s2, > 0 is expected */
    err = strncmp_s("Every moment is fresh beginning. ", sizeof("Every moment is fresh beginning. ") - 1, s1, s1len, &indicator);
    if (err != EOK) return -1;
    if (!(indicator > 0)) return -1;
    ...
    /* n > s1max */
    err = strncmp_s(s1, s1len, "Every moment is a fresh beginning", s1len + 1, &indicator);
    if (err != EINVAL) return -1;

    /* unterminated s1 */
    s1[s1len] = 0x1;
    err       = strncmp_s(s1, s1len, "Every moment is a fresh beginning", sizeof("Every moment is a fresh beginning") - 1, &indicator);
    if (err != EINVAL) return -1;
...
static int test_clib_strncmp(vlib_main_t *vm, unformat_input_t *input)
{
    char  s1[]  = "Every moment is a fresh beginning";
    uword s1len = sizeof(s1) - 1;  // excluding null
    ...
    if (v_indicator != 0) return -1;

    /* s1 > s2, 0 is expected since comparison is no more than n character */
    indicator = clib_strncmp(s1, "Every moment is a fresh begin", sizeof("Every moment is a fresh begin") - 1);
    if (indicator != 0) return -1;
    /* verify it against strncmp */
    v_indicator = strncmp(s1, "Every moment is a fresh begin", sizeof("Every moment is a fresh begin") - 1);
    if (v_indicator != 0) return -1;

    /* s1 < s2, < 0 is expected */
    indicator = clib_strncmp(s1, "Every moment is fresh beginning", sizeof("Every moment is fresh beginning") - 1);
    if (!(indicator < 0)) return -1;
    /* verify it against strncmp */
    v_indicator = strncmp(s1, "Every moment is fresh beginning", sizeof("Every moment is fresh beginning") - 1);
    if (!(v_indicator < 0)) return -1;
    if (v_indicator != indicator) return -1;
    ...
    if (indicator != EOK) return -1;

    /* n > s1 len */
    indicator = clib_strncmp(s1, "Every moment is a fresh beginning", s1len + 1);
    if (indicator != 0) return -1;
    /* verify it against strncmp */
    ...
    indicator = clib_strncmp(s1, "Every moment is a fresh beginning", sizeof("every moment is a fresh beginning") - 1);
    if (indicator != 0) return -1;
    /* verify it against strncmp */
    v_indicator = strncmp(s1, "Every moment is a fresh beginning", sizeof("Every moment is a fresh beginning") - 1);
    if (v_indicator != 0) return -1;
...
static int test_strcpy_s(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    /* This better not fail but check anyhow */
    if (strcmp_s(dst, clib_strnlen(dst, sizeof(dst)), src, &indicator) != EOK) return -1;
    if (indicator != 0) return -1;
...
static int test_strncpy_s(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    err = strncpy_s(dst, s1size, "The price of greatness is responsibility.", 10);
    if (err != EOK) return -1;
    if (strcmp_s(dst, clib_strnlen(dst, sizeof(dst)), "The price ", &indicator) != EOK) return -1;
    if (indicator != 0) return -1;
    ...
    /* truncation, n >= dmax */
    err = strncpy_s(dst, clib_strnlen(src, sizeof(src)), src, clib_strnlen(src, sizeof(src)));
    if (err != EOVERFLOW) return -1;

    /* Check dst content */
    if (dst[strlen(dst)] != '\0') return -1;
    if (strncmp_s(dst, clib_strnlen(dst, sizeof(dst)), src, clib_strnlen(dst, sizeof(dst)), &indicator) != EOK) return -1;
    if (indicator != 0) return -1;
    ...
    if (strcmp_s(dst, clib_strnlen(dst, sizeof(dst)), old_dst, &indicator) != EOK) return -1;
    if (indicator != 0) return -1;
...
static int test_clib_strncpy(vlib_main_t *vm, unformat_input_t *input)
{
    char    src[] = "Those who dare to fail miserably can achieve greatly.";
    char    dst[100], old_dst[100];
    int     indicator;
...
static int test_strcat_s(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    if (err != EOK) return -1;
    if (strcmp_s(dst, s1size - 1, "Tough time never last but tough people do", &indicator) != EOK) return -1;
    if (indicator != 0) return -1;
    ...
    if (err != EOK) return -1;
    /* verify dst is untouched */
    if (strcmp_s(dst, s1size - 1, old_dst, &indicator) != EOK) return -1;
    if (indicator != 0) return -1;
...
static int test_strncat_s(vlib_main_t *vm, unformat_input_t *input)
{
    char    src[100], dst[100], old_dst[100];
    size_t  s1size = sizeof(dst);  // including null
    errno_t err;
    ...
    if (err != EOK) return -1;
    if (strcmp_s(dst, s1size - 1, "Two things are infinite: the universe and human stupidity; ", &indicator) != EOK) return -1;
    if (indicator != 0) return -1;

    /* truncation, n >= dmax - strnlen_s (dst, dmax) */
    err = strncat_s(dst, clib_strnlen(dst, sizeof(dst)) + clib_strnlen(s3, sizeof(s3)), s3, clib_strnlen(s3, sizeof(s3)));
    if (err != EOVERFLOW) return -1;
    /*
    ...
    if (err != EOK) return -1;
    if (strcmp_s(dst, s1size - 1, "Two things are infinite: the universe ", &indicator) != EOK) return -1;
    if (indicator != 0) return -1;
    ...
    err = strncat_s(dst, clib_strnlen(dst, sizeof(dst)), s2, clib_strnlen(s2, sizeof(s2)));
    if (err != EINVAL) return -1;
...
static int test_strtok_s(vlib_main_t *vm, unformat_input_t *input)
{
    int   indicator;
    char *tok, *ptr;
    char  str2[20];
    ...
    if ((tok1 == 0) || strcmp_s(tok1, strlen(tok1), "brevity", &indicator) != EOK) return -1;
    if (indicator != 0) return -1;
    if ((tok2 == 0) || strcmp_s(tok2, strlen(tok2), "is", &indicator) != EOK) return -1;
    if (indicator != 0) return -1;
    if ((tok3 == 0) || strcmp_s(tok3, strlen(tok3), "the", &indicator) != EOK) return -1;
    if (indicator != 0) return -1;
    if ((tok4 == 0) || strcmp_s(tok4, strlen(tok4), "soul", &indicator) != EOK) return -1;
    if (indicator != 0) return -1;
    if ((tok5 == 0) || strcmp_s(tok5, strlen(tok5), "of", &indicator) != EOK) return -1;
    if (indicator != 0) return -1;
    if ((tok6 == 0) || strcmp_s(tok6, strlen(tok6), "wit", &indicator) != EOK) return -1;
    if (indicator != 0) return -1;
    if (tok7 != 0) return -1;
    ...
    p2str = 0;
    tok1  = strtok_s(str1, &len, ",", &p2str);
    if ((tok1 == 0) || strcmp_s(tok1, strlen(tok1), str1, &indicator) != EOK) return -1;
    if (indicator != 0) return -1;
...
static int test_clib_strtok(vlib_main_t *vm, unformat_input_t *input)
{
    int   indicator;
...
static int test_strnlen_s(vlib_main_t *vm, unformat_input_t *input)
{
...
static int test_clib_strnlen(vlib_main_t *vm, unformat_input_t *input)
{
    const char s1[] = "Truth is incontrovertible";
    size_t     len;
...
static int test_strstr_s(vlib_main_t *vm, unformat_input_t *input)
{
    errno_t err;
    char   *sub = 0;
    char    s1[64];
    ...
    ;
    if (err != ESRCH) return -1;

    /* substring present */
    err = strstr_s(s1, s1len, "failure", sizeof("failure"), &sub);
    if (err != EOK) return -1;

    if ((sub == 0) || strcmp_s(sub, strlen(sub), "failure is not fatal.", &indicator) != EOK) return -1;
    if (indicator != 0) return -1;
...
static uword unformat_string_test(unformat_input_t *input, va_list *args)
{
    u8 *r = va_arg(*args, u8 *);
...
static clib_error_t *string_test_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
...
#undef _
    ...
    if (specific_test == ~0) {
        for (specific_test = STRING_TEST_FIRST; specific_test <= STRING_TEST_LAST; specific_test++) {
            ok = (string_func[specific_test]).test(vm, input);
            res += ok;
            if (ok != 0) vlib_cli_output(vm, "test_%s failed", string_table[specific_test]);
        }
    }
    else
