 ...
 */
static vapi_ctx_t connect_to_vpp(void)
{
    vapi_ctx_t ctx;
    ...
    if (vapi_connect_from_vpp(ctx, "apifromplugin", 64, 32, VAPI_MODE_BLOCKING, true) != VAPI_OK) {
...
{
    if (rv != VAPI_OK) clib_warning("Return value: %d", rv);
    fformat(stdout,
...
static void *api_show_version_blocking_fn(void *args)
{
    vapi_ctx_t ctx;

    if ((ctx = connect_to_vpp()) == 0) return clib_error_return(0, "API connection failed");

    int                    called;
    vapi_msg_show_version *sv      = vapi_alloc_show_version(ctx);
    vapi_error_e           vapi_rv = vapi_show_version(ctx, sv, show_version_cb, &called);
    if (vapi_rv != VAPI_OK) clib_warning("call failed");
...
static clib_error_t *test_api_test_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
