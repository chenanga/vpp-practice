...
static int rbtree_test_basic(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    RBTREE_TEST(aux->key == n_keys / 2 + 1, "successor should be %u is %u", n_keys / 2 + 1, aux->key);

    aux = rb_tree_predecessor(rt, n);
    RBTREE_TEST(aux->key == n_keys / 2 - 1, "predecessor should be %u is %u", n_keys / 2 - 1, aux->key);
    ...
    RBTREE_TEST(rb_node_is_tnil(rt, n), "search result for %u should be tnil", search_key);
    ...
    aux = rb_tree_successor(rt, n);
    RBTREE_TEST(aux->key == search_key + 2, "successor should be %u is %u", search_key + 2, aux->key);

    aux = rb_tree_predecessor(rt, n);
    RBTREE_TEST(aux->key == search_key - 2, "predecessor should be %u is %u", search_key - 2, aux->key);
    ...
    RBTREE_TEST(rb_tree_n_nodes(rt) == n_keys + 1, "number nodes %u is %u", n_keys + 1, rb_tree_n_nodes(rt));
    ...
    aux = rb_tree_successor(rt, n);
    RBTREE_TEST(aux->key == search_key + 1, "successor should be %u is %u", search_key + 1, aux->key);

    aux = rb_tree_predecessor(rt, n);
    RBTREE_TEST(aux->key == search_key - 1, "predecessor should be %u is %u", search_key - 1, aux->key);
    ...
    for (i = 0; i < n_keys; i++) {
        rb_tree_del(rt, i);
        if (rt->nodes[RBTREE_TNIL_INDEX].color != RBTREE_BLACK) RBTREE_TEST(0, "tnil should be black");
    }

    RBTREE_TEST(rb_tree_n_nodes(rt) == 1, "number nodes %u is %u", 1, rb_tree_n_nodes(rt));
    ...
    RBTREE_TEST(rb_tree_n_nodes(rt) == 0, "number nodes %u is %u", 0, rb_tree_n_nodes(rt));

    return 0;
}

static clib_error_t *rbtree_test(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
    int res = 0;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "basic")) {
            ...
            if ((res = rbtree_test_basic(vm, input))) goto done;
        }
        else
            break;
    }
