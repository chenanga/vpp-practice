...
static void fuzz_hook(u16 id, void *the_msg)
{
    /*
...
static void default_fuzz_config(void)
{
    fuzz_first     = vl_msg_api_get_msg_index((u8 *) "memclnt_keepalive_reply_e8d4e804");
    fuzz_cli_first = vl_msg_api_get_msg_index((u8 *) "cli_23bfbfff");
    fuzz_cli_last  = vl_msg_api_get_msg_index((u8 *) "cli_inband_reply_05879051");
}

static clib_error_t *test_api_fuzz_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    u32 tmp;
    ...
    if (fuzz_first == 0xFFFF) {
        ...
        vlib_cli_output(vm, "Manual setting required, use 'show api message table'");
    }
    ...
    vlib_cli_output(vm, "Fuzzing enabled: first %d, skip cli range %d - %d", (u32) fuzz_first, (u32) fuzz_cli_first, (u32) fuzz_cli_last);
...
static clib_error_t *api_fuzz_config(vlib_main_t *vm, unformat_input_t *input)
{
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "off") || unformat(input, "disable") || unformat(input, "no"))
            ; /* ok, no action */
        else if (unformat(input, "on") || unformat(input, "enable") || unformat(input, "yes"))
...
static clib_error_t *api_fuzz_api_init(vlib_main_t *vm)
{
    /* Are we supposed to fuzz API messages? */
    if (main_loop_enter_enable_api_fuzz == 0) return 0;

    default_fuzz_config();

    if (fuzz_first == 0xFFFF) {
        return clib_error_return(0, "Couldn't find 'memclnt_keepalive_reply' ID");
    }
    /* Turn on fuzzing */
    vl_mem_api_fuzz_hook = fuzz_hook;
    return 0;
