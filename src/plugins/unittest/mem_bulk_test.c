...
static int mem_bulk_test_basic(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    for (i = 0; i < n_iter; i++)
        if (elts[i]->data != i) MB_TEST(0, "data corrupted");
...
static clib_error_t *mem_bulk_test(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
    int res = 0;
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "basic")) {
            ...
            if ((res = mem_bulk_test_basic(vm, input))) goto done;
        }
        else
            break;
    }
