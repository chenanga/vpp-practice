...
#undef _
    for (i = 0; i < vec_len(sp->splits); i++) {
        if (sp->splits[i] > 0 || verbose) s = format(s, "    splits[%d]: %lld\n", 1 << i, sp->splits[i]);
    }
    return s;
}

static void inc_stats_callback(BVT(clib_bihash) * h, int stat_id, u64 count)
{
    uword          *statp = h->inc_stats_context;
    bihash_stats_t *for_splits;

    if (PREDICT_TRUE(stat_id * sizeof(u64) < STRUCT_OFFSET_OF(bihash_stats_t, splits))) {
        statp[stat_id] += count;
        return;
    }
...
static clib_error_t *test_bihash_vec64(bihash_test_main_t *tm)
{
    ...
    for (j = 0; j < vec_len(cum_times); j++)
        fformat(stdout, "Cum time for %d: %.4f (us)\n", (j + 1) * 1000, cum_times[j] * 1e6);

    return 0;
}

void *test_bihash_thread_fn(void *arg)
{
    BVT(clib_bihash) * h;
    BVT(clib_bihash_kv) kv;
    ...
    for (i = 0; i < tm->ncycles; i++) {
        for (j = 0; j < tm->nitems; j++) {
            ...
            (void) __atomic_add_fetch(&tm->sequence_number, 1, __ATOMIC_ACQUIRE);
            BV(clib_bihash_add_del)(h, &kv, 1 /* is_add */);
        }
        for (j = 0; j < tm->nitems; j++) {
            kv.key   = ((u64) my_thread_index << 32) | (u64) j;
...
static clib_error_t *test_bihash_threads(bihash_test_main_t *tm)
{
    int       i;
    pthread_t handle;
    BVT(clib_bihash) * h;
    ...
    for (i = 0; i < tm->nthreads; i++) {
        rv = pthread_create(&handle, NULL, test_bihash_thread_fn, (void *) (uword) i);
        if (rv)
    ...
    clib_warning("%lld items in %.2f seconds, %.2f items/sec",
                 (u64) tm->nthreads * (u64) tm->nitems,
                 delta,
                 delta > 0.0 ? ((f64) ((u64) tm->nthreads * (u64) tm->nitems)) / delta : 0.0);
 ...
 */
static int count_items(BVT(clib_bihash_kv) * kvp, void *notused)
{
    _clib_error(CLIB_ERROR_ABORT, 0, 0, "bihash test FAILED, items left in table!");
    return (BIHASH_WALK_CONTINUE);
}

static clib_error_t *test_bihash(bihash_test_main_t *tm)
{
    int    i, j;
    uword *p;
    ...
    for (acycle = 0; acycle < tm->ncycles; acycle++) {
        ...
        if ((acycle % tm->report_every_n) == 0) fformat(stdout, "Add items...\n");
        ...
        for (j = 0; j < tm->search_iter; j++) {
            for (i = 0; i < tm->nitems; i++) {
                ...
                if (BV(clib_bihash_search)(h, &kv, &kv) < 0) {
                    if (BV(clib_bihash_search)(h, &kv, &kv) < 0) {
                        return clib_error_return(0, "[%d] search for key %lld failed unexpectedly\n", i, tm->keys[i]);
                    }
                }
                if (kv.value != (u64) (i + 1))
                    return clib_error_return(0, "[%d] search for key %lld returned %lld, not %lld\n", i, tm->keys, kv.value, (u64) (i + 1));
            }
        }
        ...
        if ((acycle % tm->report_every_n) == 0) {
            ...
            if (delta > 0) fformat(stdout, "%.f searches per second\n", ((f64) total_searches) / delta);
        ...
        for (j = 0; j < tm->search_iter; j++) {
            for (i = 0; i < tm->nitems; i++) {
                ...
                if (p == 0 || p[0] != (uword) (i + 1)) return clib_error_return(0, "ugh, couldn't find %lld\n", tm->keys[i]);
            }
        }
        ...
        for (i = 0; i < tm->nitems; i++) {
            ...
            if (rv < 0) return clib_error_return(0, "delete key %lld not ok but should be", tm->keys[i]);
            ...
            if (tm->careful_delete_tests) {
                for (j = 0; j < tm->nitems; j++) {
                    ...
                    if (j <= i && rv >= 0) {
                        return clib_error_return(0, "i %d j %d search ok but should not be, value %lld", i, j, kv.value);
                    }
                    if (j > i && rv < 0) {
                        return clib_error_return(0, "i %d j %d search not ok but should be", i, j);
                    }
                }
            }
        }
        ...
        if (acycle != (tm->ncycles - 1)) tm->key_hash = hash_create(tm->nitems, sizeof(uword));
    }
...
static clib_error_t *test_bihash_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    bihash_test_main_t *tm = &bihash_test_main;
    clib_error_t       *error;
    int                 which = 0;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "seed %u", &tm->seed))
            ...
            ;
        else if (unformat(input, "memory-size %U", unformat_memory_size, &tm->hash_memory_size))
            ;
...
static clib_error_t *bihash_test_init(vlib_main_t *vm)
{
    bihash_test_main_t *tm = &bihash_test_main;
