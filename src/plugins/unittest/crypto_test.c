...
static int sort_registrations(void *a0, void *a1)
{
...
static void print_results(vlib_main_t                          *vm,
                          unittest_crypto_test_registration_t **rv,
                          vnet_crypto_op_t                     *ops,
                          vnet_crypto_op_chunk_t               *chunks,
                          u32                                   n_ops,
                          crypto_test_main_t                   *tm)
{
    int                                  i;
    ...
    vec_foreach (op, ops) {
        ...
        if (op->status != VNET_CRYPTO_OP_STATUS_COMPLETED)
            err = format(err, "%sengine error: %U", vec_len(err) ? ", " : "", format_vnet_crypto_op_status, op->status);
            ...
            if (exp_ct_chunks) {
                ...
                for (i = 0; i < op->n_chunks; i++) {
                    if (memcmp(chp->dst, exp_ct_chunks[i].data, chp->len))
                        err = format(err, "%sciphertext mismatch [chunk %d]", vec_len(err) ? ", " : "", i);
                    chp += 1;
                }
            }
            ...
            if (exp_pt_chunks) {
                ...
                for (i = 0; i < op->n_chunks; i++) {
                    if (memcmp(chp->dst, exp_pt_chunks[i].data, chp->len))
                        err = format(err, "%splaintext mismatch [chunk %d]", vec_len(err) ? ", " : "", i);
                    chp += 1;
                }
            }
        }
        else {
            if (exp_ct && memcmp(op->dst, exp_ct->data, exp_ct->length) != 0) err = format(err, "%sciphertext mismatch", vec_len(err) ? ", " : "");

            if (exp_pt && memcmp(op->dst, exp_pt->data, exp_pt->length) != 0) err = format(err, "%splaintext mismatch", vec_len(err) ? ", " : "");
        }

        if (exp_tag && memcmp(op->tag, exp_tag->data, exp_tag->length) != 0) err = format(err, "%stag mismatch", vec_len(err) ? ", " : "");

        if (exp_digest && memcmp(op->digest, exp_digest->data, exp_digest->length) != 0)
            err = format(err, "%sdigest mismatch", vec_len(err) ? ", " : "");

        vec_reset_length(s);
        s = format(s, "%s (%U)", r->name, format_vnet_crypto_op, op->op, r->is_chained);

        if (vec_len(err)) fail = 1;

        vlib_cli_output(vm, "%-65v%s%v", s, vec_len(err) ? "FAIL: " : "OK", err);
        if (tm->verbose) {
            if (tm->verbose == 2) fail = 1;
...
static void validate_data(u8 **data, u32 len)
{
    u32 i, diff, old_len;
    if (vec_len(data[0]) >= len) return;
...
static void generate_digest(vlib_main_t *vm, unittest_crypto_test_registration_t *r, vnet_crypto_op_id_t id)
{
    ...
    op->key_index  = vnet_crypto_key_add(vm, r->alg, cm->inc_data, r->key.length);

    /* at this point openssl is set for each algo */
    vnet_crypto_process_ops(vm, op, 1);
}

static int restore_engines(u32 *engs)
{
...
static int save_current_engines(u32 *engs)
{
    vnet_crypto_main_t   *cm = &crypto_main;
    uword                *p;
    u32                   i;
...
static clib_error_t *test_crypto_incremental(vlib_main_t                          *vm,
                                             crypto_test_main_t                   *tm,
                                             unittest_crypto_test_registration_t **rv,
                                             u32                                   n_ops,
        ...
        for (t = 0; t < VNET_CRYPTO_OP_N_TYPES; t++) {
            ...
            switch (t) {
                case VNET_CRYPTO_OP_TYPE_ENCRYPT:
                    vnet_crypto_op_init(op, id);
                    op->iv        = tm->inc_data;
                    op->key_index = vnet_crypto_key_add(vm, r->alg, tm->inc_data, r->key.length);
...
static clib_error_t *test_crypto_static(vlib_main_t                          *vm,
                                        crypto_test_main_t                   *tm,
                                        unittest_crypto_test_registration_t **rv,
                                        u32                                   n_ops,
                                        u32                                   n_chained_ops,
                                        u32                                   computed_data_total_len)
{
    ...
    vec_foreach_index (i, rv) {
        ...
        for (t = 0; t < VNET_CRYPTO_OP_N_TYPES; t++) {
            ...
            switch (t) {
                ...
                case VNET_CRYPTO_OP_TYPE_DECRYPT:
                    op->iv        = r->iv.data;
                    op->key_index = vnet_crypto_key_add(vm, r->alg, r->key.data, r->key.length);
                    vec_add1(key_indices, op->key_index);
                    ...
                    if (r->is_chained) {
                        ...
                        while (pt->data) {
                            ch.src = t == VNET_CRYPTO_OP_TYPE_ENCRYPT ? pt->data : ct->data;
                            ch.len = pt->length;
                        ...
                        op->src = t == VNET_CRYPTO_OP_TYPE_ENCRYPT ? r->plaintext.data : r->ciphertext.data;
    ...
    vnet_crypto_process_chained_ops(vm, chained_ops, chunks, vec_len(chained_ops));
...
static u32 test_crypto_get_key_sz(vnet_crypto_alg_t alg)
{
    switch (alg) {
#define _(n, s, l)            \
...
#undef _
            case VNET_CRYPTO_ALG_HMAC_MD5 : case VNET_CRYPTO_ALG_HMAC_SHA1 : return 20;
...
static clib_error_t *test_crypto(vlib_main_t *vm, crypto_test_main_t *tm)
{
    clib_error_t                         *err          = 0;
    vnet_crypto_main_t                   *cm           = &crypto_main;
    ...
    u32                                   saved_engs[VNET_CRYPTO_N_OP_IDS] = {
                                          ~0,
    };
    ...
    if (rc) return clib_error_return(0, "failed to set default crypto engine!");
            ...
            switch (i) {
                ...
                case VNET_CRYPTO_OP_TYPE_AEAD_DECRYPT:
                    if (r->is_chained) {
                        ...
                        j  = 0;
                        while (ct->data) {
                            if (j > CRYPTO_TEST_MAX_OP_CHUNKS) return clib_error_return(0, "test case '%s' exceeds extra data!", r->name);
                            computed_data_total_len += ct->length;
                            ct++;
                            j++;
                        }
    ...
    err = test_crypto_static(vm, tm, static_tests, n_ops_static, n_chained_ops, computed_data_total_len);
    if (err) goto done;
    ...
    while (r) {
        if (r->plaintext_incremental) vec_free(r->digest.data);
        r = r->next;
    }
...
static clib_error_t *test_crypto_perf(vlib_main_t *vm, crypto_test_main_t *tm)
{
    vnet_crypto_main_t     *cm  = &crypto_main;
    clib_error_t           *err = 0;
    ...
    if (tm->buffer_size > buffer_size) return clib_error_return(0, "buffer size must be <= %u", buffer_size);
    ...
    if (buffer_size > vlib_buffer_get_default_data_size(vm)) return clib_error_return(0, "buffer size too big");
    ...
    if (n_alloc != n_buffers) {
        if (n_alloc) vlib_buffer_free(vm, buffer_indices, n_alloc);
    ...
    vlib_cli_output(vm,
                    ...
                    rounds,
                    warmup_rounds);
    vlib_cli_output(vm, "   cpu-freq %.2f GHz", (f64) vm->clib_time.clocks_per_second * 1e-9);
    ...
    key_index = vnet_crypto_key_add(vm, tm->alg, key, test_crypto_get_key_sz(tm->alg));
    ...
    for (i = 0; i < n_buffers; i++) {
        ...
        switch (ot) {
            ...
            case VNET_CRYPTO_OP_TYPE_AEAD_ENCRYPT:
            case VNET_CRYPTO_OP_TYPE_AEAD_DECRYPT:
                vnet_crypto_op_init(op1, ad->op_by_type[VNET_CRYPTO_OP_TYPE_AEAD_ENCRYPT]);
                vnet_crypto_op_init(op2, ad->op_by_type[VNET_CRYPTO_OP_TYPE_AEAD_DECRYPT]);
                op1->src = op2->src = op1->dst = op2->dst = b->data;
                op1->key_index = op2->key_index = key_index;
    ...
    for (i = 0; i < 5; i++) {
        for (j = 0; j < warmup_rounds; j++) {
            ...
            if (ot != VNET_CRYPTO_OP_TYPE_HMAC) vnet_crypto_process_ops(vm, ops2, n_buffers);
        }
    ...
    if (key_index != ~0) vnet_crypto_key_del(vm, key_index);
...
static clib_error_t *test_crypto_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
static clib_error_t *crypto_test_init(vlib_main_t *vm)
{
    return (0);
}
