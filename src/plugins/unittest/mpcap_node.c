...
always_inline uword mpcap_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ip4, int is_trace)
{
    ...
    while (n_left_from >= 4) {
        ...
        mpcap_add_buffer(mm, vm, now, from[i++], vlib_buffer_length_in_chain(vm, b[0]));

        mpcap_add_buffer(mm, vm, now, from[i++], vlib_buffer_length_in_chain(vm, b[1]));

        mpcap_add_buffer(mm, vm, now, from[i++], vlib_buffer_length_in_chain(vm, b[2]));

        mpcap_add_buffer(mm, vm, now, from[i++], vlib_buffer_length_in_chain(vm, b[3]));
