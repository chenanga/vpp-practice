...
int pattern_cmp(const void *arg1, const void *arg2)
{
...
static u8 fifo_validate_pattern(vlib_main_t *vm, test_pattern_t *pattern, u32 pattern_length)
{
    ...
    for (i = 0; i < pattern_length - 1; i++) {
        if (tp->offset + tp->len != (tp + 1)->offset) {
            vlib_cli_output(vm, "[%d] missing {%d, %d}", i, (tp->offset + tp->len), (tp + 1)->offset - (tp->offset + tp->len));
            return 0;
        }
        tp++;
    }
...
static test_pattern_t *fifo_get_validate_pattern(vlib_main_t *vm, test_pattern_t *test_data, u32 test_data_len)
{
    test_pattern_t *validate_pattern = 0;

    /* Validate, and try segments in order... */
    vec_validate(validate_pattern, test_data_len - 1);
    memcpy(validate_pattern, test_data, test_data_len * sizeof(test_pattern_t));
    qsort((u8 *) validate_pattern, test_data_len, sizeof(test_pattern_t), pattern_cmp);

    if (fifo_validate_pattern(vm, validate_pattern, test_data_len) == 0) return 0;

    return validate_pattern;
}

static fifo_segment_t *fifo_segment_prepare(fifo_segment_main_t *sm, char *seg_name, u32 seg_size)
{
    ...
    if (fifo_segment_create(sm, a)) return 0;

    return fifo_segment_get_segment(sm, a->new_segment_indices[0]);
}

static void ft_fifo_segment_free(fifo_segment_main_t *sm, fifo_segment_t *fs)
{
    fifo_segment_delete(sm, fs);
}

static svm_fifo_t *fifo_segment_alloc_fifo(fifo_segment_t *fs, u32 data_bytes, fifo_segment_ftype_t ftype)
{
    return fifo_segment_alloc_fifo_w_slice(fs, 0, data_bytes, ftype);
}

static svm_fifo_t *fifo_prepare(fifo_segment_t *fs, u32 fifo_size)
{
    svm_fifo_t       *f;
    svm_fifo_chunk_t *c;
...
static void ft_fifo_free(fifo_segment_t *fs, svm_fifo_t *f)
{
    fifo_segment_free_fifo(fs, f);
}

static int compare_data(u8 *data1, u8 *data2, u32 start, u32 len, u32 *index)
{
    int i;
...
int sfifo_test_fifo1(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (verbose) vlib_cli_output(vm, "add [%d] [%d, %d]", 2 * i + 1, offset, offset + sizeof(u32));
        if (rv) {
    ...
    if (verbose) vlib_cli_output(vm, "fifo after odd segs: %U", format_svm_fifo, f, 1);

    SFIFO_TEST((f->shr->tail == 8), "fifo tail %u", f->shr->tail);
    SFIFO_TEST((svm_fifo_n_ooo_segments(f) == 2), "number of ooo segments %u", svm_fifo_n_ooo_segments(f));
    ...
    if (verbose) vlib_cli_output(vm, "fifo after overlap seg: %U", format_svm_fifo, f, 1);
    ...
    for (i = 3; i > 1; i--) {
        ...
        rv     = svm_fifo_enqueue_with_offset(f, offset, sizeof(u32), data);
        if (verbose) vlib_cli_output(vm, "add [%d] [%d, %d]", 2 * i, offset, offset + sizeof(u32));
        if (rv) {
            clib_warning("enqueue returned %d", rv);
            goto err;
        }
    ...
    if (verbose) vlib_cli_output(vm, "fifo before missing link: %U", format_svm_fifo, f, 1);
    SFIFO_TEST((svm_fifo_n_ooo_segments(f) == 1), "number of ooo segments %u", svm_fifo_n_ooo_segments(f));
    ooo_seg = svm_fifo_first_ooo_segment(f);
    SFIFO_TEST((ooo_seg->start == 12), "first ooo seg position %u", ooo_seg->start);
    SFIFO_TEST((ooo_seg->length == 16), "first ooo seg length %u", ooo_seg->length);

    /*
     * Enqueue the missing u32
     */
    rv = svm_fifo_enqueue(f, sizeof(u32), (u8 *) (test_data + 2));
    if (verbose) vlib_cli_output(vm, "fifo after missing link: %U", format_svm_fifo, f, 1);
    SFIFO_TEST((rv == 20), "bytes to be enqueued %u", rv);
    SFIFO_TEST((svm_fifo_n_ooo_segments(f) == 0), "number of ooo segments %u", svm_fifo_n_ooo_segments(f));
    ...
    for (i = 0; i < 7; i++) {
        ...
        if (data_word != test_data[i]) {
            clib_warning("recovered [%d] %d not %d", i, data_word, test_data[i]);
            goto err;
        }
    }
    ...
    if (compare_data(data_buf, data, 0, 4, &j)) SFIFO_TEST(0, "[%d] peeked %u expected %u", j, data_buf[j], data[j]);
    svm_fifo_peek(f, 8, 21, data_buf);
    if (compare_data(data_buf, data, 0, 21, &j)) SFIFO_TEST(0, "[%d] peeked %u expected %u", j, data_buf[j], data[j]);
    ...
    if (verbose) vlib_cli_output(vm, "fifo after enqueue: %U", format_svm_fifo, f, 1);

    rv = svm_fifo_enqueue(f, 29, data);
    if (verbose) vlib_cli_output(vm, "fifo after enqueueing 29: %U", format_svm_fifo, f, 1);
...
static int sfifo_test_fifo2(vlib_main_t *vm)
{
    fifo_segment_main_t _fsm = {0}, *fsm = &_fsm;
    ...
    for (i = 0; i < test_data_len; i++) {
        tp     = vp + i;
        data64 = tp->offset;
        svm_fifo_enqueue_with_offset(f, tp->offset - f->shr->tail, tp->len, (u8 *) &data64);
    }

    /* Expected result: one big fat chunk at offset 4 */
    SFIFO_TEST((svm_fifo_n_ooo_segments(f) == 1), "number of ooo segments %u", svm_fifo_n_ooo_segments(f));
    ooo_seg = svm_fifo_first_ooo_segment(f);
    SFIFO_TEST((ooo_seg->start == 4), "first ooo seg position %u", ooo_seg->start);
    SFIFO_TEST((ooo_seg->length == 2996), "first ooo seg length %u", ooo_seg->length);
    ...
    for (i = 0; i < test_data_len; i++) {
        tp     = &test_data[i];
        data64 = tp->offset;
        rv     = svm_fifo_enqueue_with_offset(f, tp->offset - f->shr->tail, tp->len, (u8 *) &data64);
        if (rv) {
            clib_warning("enqueue returned %d", rv);
        }
    }
...
static int sfifo_test_fifo3(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "fifo-size %d", &fifo_size))
            ...
            clib_error_t *e = clib_error_return(0, "unknown input `%U'", format_unformat_error, input);
            clib_error_report(e);
            return -1;
        }
    }
    ...
    if (total_size > fifo_size) {
        clib_warning("total_size %d greater than fifo size %d", total_size, fifo_size);
        return -1;
    }
    ...
    while (length_so_far < total_size) {
        ...
        if (overlap && (len_this_chunk == seg_size)) do {
                offset_increment = len_this_chunk % (1 + (random_u32(&seed) % len_this_chunk));
     ...
     */
    if (overlap == 0) fifo_validate_pattern(vm, generate, vec_len(generate));

    if (verbose) {
        vlib_cli_output(vm, "raw data pattern:");
        for (i = 0; i < vec_len(generate); i++) {
            vlib_cli_output(vm, "[%d] offset %u len %u", i, generate[i].offset, generate[i].len);
        }
    }
    ...
    for (i = !randomize; i < vec_len(generate); i++) {
        tp = generate + i;
        svm_fifo_enqueue_with_offset(f, fifo_initial_offset + tp->offset - f->shr->tail, tp->len, (u8 *) data_pattern + tp->offset);
    }

    /* Add the first segment in order for non random data */
    if (!randomize) svm_fifo_enqueue(f, generate[0].len, (u8 *) data_pattern);

    /*
     * Expected result: one big fat chunk at offset 1 if randomize == 1
     */

    if (verbose) vlib_cli_output(vm, "fifo before missing link: %U", format_svm_fifo, f, 1 /* verbose */);
    ...
    if (randomize) {
        u32 bytes_to_enq = 1;
        if (in_seq_all) bytes_to_enq = total_size;
        rv = svm_fifo_enqueue(f, bytes_to_enq, data_pattern + 0);

        if (verbose) vlib_cli_output(vm, "in-order enqueue returned %d", rv);

        SFIFO_TEST((rv == total_size), "enqueued %u expected %u", rv, total_size);
    }

    SFIFO_TEST((svm_fifo_has_ooo_data(f) == 0), "number of ooo segments %u", svm_fifo_n_ooo_segments(f));
    ...
    if (compare_data(data_buf, data_pattern, 0, vec_len(data_pattern), &j)) {
        SFIFO_TEST(0, "[%d] peeked %u expected %u", j, data_buf[j], data_pattern[j]);
    }
    ...
    if (drop) {
        ...
        if (compare_data(data_buf, data_pattern, 0, vec_len(data_pattern), &j)) {
            SFIFO_TEST(0, "[%d] dequeued %u expected %u", j, data_buf[j], data_pattern[j]);
        }
    }

    SFIFO_TEST((svm_fifo_max_dequeue(f) == 0), "fifo has %d bytes", svm_fifo_max_dequeue(f));
...
static int sfifo_test_fifo4(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    for (i = test_n_bytes - 1; i > 0; i--) {
        rv = svm_fifo_enqueue_with_offset(f, fifo_initial_offset + i - f->shr->tail, sizeof(u8), &test_data[i]);
        if (verbose) vlib_cli_output(vm, "add [%d] [%d, %d]", i, i, i + sizeof(u8));
        if (rv) SFIFO_TEST(0, "enqueue returned %d", rv);
    }
    ...
    if (rv) vlib_cli_output(vm, "[%d] dequeued %u expected %u", j, data_buf[j], test_data[j]);
...
static u32 fifo_pos(svm_fifo_t *f, u32 pos)
{
    return pos;
}

/* Avoids exposing svm_fifo.c internal function */
static ooo_segment_t *ooo_seg_next(svm_fifo_t *f, ooo_segment_t *s)
{
    if (pool_is_free_index(f->ooo_segments, s->next)) return 0;
    return pool_elt_at_index(f->ooo_segments, s->next);
}

static int sfifo_test_fifo5(vlib_main_t *vm, unformat_input_t *input)
{
    fifo_segment_main_t _fsm = {0}, *fsm = &_fsm;
    ...
    if (verbose) vlib_cli_output(vm, "fifo after [100, 200] and [300, 400] : %U", format_svm_fifo, f, 2 /* verbose */);
    ...
    if (verbose) vlib_cli_output(vm, "fifo after [225, 275] : %U", format_svm_fifo, f, 2 /* verbose */);
    SFIFO_TEST((svm_fifo_n_ooo_segments(f) == 3), "number of ooo segments %u", svm_fifo_n_ooo_segments(f));
    ooo_seg = svm_fifo_first_ooo_segment(f);
    SFIFO_TEST((ooo_seg->start == fifo_pos(f, 100 + offset)), "first seg start %u expected %u", ooo_seg->start, fifo_pos(f, 100 + offset));
    SFIFO_TEST((ooo_seg->length == 100), "first seg length %u expected %u", ooo_seg->length, 100);
    ooo_seg = ooo_seg_next(f, ooo_seg);
    SFIFO_TEST((ooo_seg->start == fifo_pos(f, 225 + offset)), "second seg start %u expected %u", ooo_seg->start, fifo_pos(f, 225 + offset));
    SFIFO_TEST((ooo_seg->length == 50), "second seg length %u expected %u", ooo_seg->length, 50);
    ooo_seg = ooo_seg_next(f, ooo_seg);
    SFIFO_TEST((ooo_seg->start == fifo_pos(f, 300 + offset)), "third seg start %u expected %u", ooo_seg->start, fifo_pos(f, 300 + offset));
    SFIFO_TEST((ooo_seg->length == 100), "third seg length %u expected %u", ooo_seg->length, 100);
    SFIFO_TEST((f->ooos_newest == 2), "newest %u", f->ooos_newest);
    /*
     * Add [190, 310]
     */
    rv = svm_fifo_enqueue_with_offset(f, 190, 120, &test_data[190]);
    if (verbose) vlib_cli_output(vm, "fifo after [190, 310] : %U", format_svm_fifo, f, 1 /* verbose */);
    SFIFO_TEST((svm_fifo_n_ooo_segments(f) == 1), "number of ooo segments %u", svm_fifo_n_ooo_segments(f));
    ooo_seg = svm_fifo_first_ooo_segment(f);
    SFIFO_TEST((ooo_seg->start == fifo_pos(f, offset + 100)), "first seg start %u expected %u", ooo_seg->start, fifo_pos(f, offset + 100));
    SFIFO_TEST((ooo_seg->length == 300), "first seg length %u expected %u", ooo_seg->length, 300);
    ...
    if (verbose) vlib_cli_output(vm, "fifo after [0 150] : %U", format_svm_fifo, f, 2 /* verbose */);
    ...
    if (compare_data(data_buf, test_data, 0, 400, &j)) {
        SFIFO_TEST(0, "[%d] peeked %u expected %u", j, data_buf[j], test_data[j]);
    }
    ...
    SFIFO_TEST((svm_fifo_n_ooo_segments(f) == 1), "number of ooo segments %u", svm_fifo_n_ooo_segments(f));
    ooo_seg = svm_fifo_first_ooo_segment(f);
    SFIFO_TEST((ooo_seg->start == 50), "first seg start %u expected %u", ooo_seg->start, 50);
    SFIFO_TEST((ooo_seg->length == 200), "first seg length %u expected %u", ooo_seg->length, 200);
 ...
 */
static int sfifo_test_fifo6(vlib_main_t *vm, unformat_input_t *input)
{
    fifo_segment_main_t _fsm = {0}, *fsm = &_fsm;
    ...
    SFIFO_TEST((svm_fifo_n_ooo_segments(f) == 0), "number of ooo segments %u", svm_fifo_n_ooo_segments(f));
    SFIFO_TEST(f->ooos_list_head == OOO_SEGMENT_INVALID_INDEX, "there should be no ooo seg");

    svm_fifo_peek(f, 5, 10, &data_buf[5]);
    if (compare_data(data_buf, test_data, 5, 10, (u32 *) &j)) SFIFO_TEST(0, "[%d] dequeued %u expected %u", j, data_buf[j], test_data[j]);

    svm_fifo_dequeue(f, 20, data_buf);
    if (compare_data(data_buf, test_data, 0, 20, (u32 *) &j)) SFIFO_TEST(0, "[%d] dequeued %u expected %u", j, data_buf[j], test_data[j]);
 ...
 */
static int sfifo_test_fifo7(vlib_main_t *vm, unformat_input_t *input)
{
    fifo_segment_main_t _fsm = {0}, *fsm = &_fsm;
    ...
    for (i = 0; i < n_iterations; i++) {
        for (j = n_test_bytes - 1; j > 0; j -= 2) {
            ...
            if (rv != (n_test_bytes - j) / 2 + 1) SFIFO_TEST(0, "number of ooo segments expected %u is %u", (n_test_bytes - j) / 2 + 1, rv);
        }

        svm_fifo_enqueue_with_offset(f, 1, n_test_bytes - 1, &test_data[1]);
        rv = svm_fifo_n_ooo_segments(f);
        if (rv != 1) SFIFO_TEST(0, "number of ooo segments %u", rv);

        svm_fifo_enqueue(f, 1, test_data);
        rv = svm_fifo_n_ooo_segments(f);
        if (rv != 0) SFIFO_TEST(0, "number of ooo segments %u", rv);

        svm_fifo_dequeue(f, n_test_bytes, data_buf);
        if (compare_data(data_buf, test_data, 0, n_test_bytes, (u32 *) &j))
            SFIFO_TEST(0, "[%d] dequeued %u expected %u", j, data_buf[j], test_data[j]);
        svm_fifo_init_pointers(f, (~0 - i) % f->shr->size, (~0 - i) % f->shr->size);
    }
 ...
 */
static int sfifo_test_fifo_large(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    for (i = 0; i < n_iterations; i++) {
        ...
        if (compare_data(data_buf, test_data, 0, n_bytes_per_iter, (u32 *) &j))
            SFIFO_TEST(0, "[%d][%d] dequeued %u expected %u", i, j, data_buf[j], test_data[j]);
    }
...
static void validate_test_and_buf_vecs(u8 **test_data, u8 **data_buf, u32 len)
{
    int i, cur_len;
...
static int enqueue_ooo(svm_fifo_t *f, u8 *test_data, u32 len, u32 iterations)
{
    ...
    for (i = iterations; i > 0; i--) {
        ...
        if (!enq_now) continue;
        rv = svm_fifo_enqueue_with_offset(f, offset, enq_now, test_data + offset);
        if (rv) return rv;
    }
...
static int enqueue_ooo_packets(svm_fifo_t *f, u32 len, u32 enq_chunk, u8 *test_data)
{
    u32 offset, enq_now;
    int i, rv;
...
static int enqueue_packets_inc(svm_fifo_t *f, u32 len, u32 enq_chunk, u8 *test_data)
{
    u32 enq_now, offset;
    int i, rv;
...
static int dequeue_ooo(svm_fifo_t *f, u8 *data_buf, u32 len, u32 iterations)
{
    ...
    for (i = iterations; i >= 0; i--) {
        ...
        if (deq_now == 0) continue;
        rv = svm_fifo_peek(f, offset, deq_now, data_buf + offset);
        if (rv != deq_now) return rv;
    }
    return 0;
}

static int dequeue_ooo_inc(svm_fifo_t *f, u8 *data_buf, u32 len, u32 iterations)
{
    u32 offset, ooo_chunk, deq_now;
    int i, rv;
...
static int dequeue_packets(svm_fifo_t *f, u32 len, u32 deq_chunk, u8 *data_buf)
{
    u32 offset, deq_now;
    int i, rv;
...
static int sfifo_test_fifo_grow(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    SFIFO_TEST(svm_fifo_size(f) == fifo_size, "size expected %u is %u", fifo_size, svm_fifo_size(f));
    SFIFO_TEST(svm_fifo_max_dequeue(f) == fifo_size, "max deq should be %u", fifo_size);
    ...
    rv = compare_data(data_buf, test_data, 0, vec_len(test_data), (u32 *) &i);

    if (rv) vlib_cli_output(vm, "[%d] dequeued %u expected %u", i, data_buf[i], test_data[i]);
    SFIFO_TEST((rv == 0), "dequeued compared to original returned %d", rv);
    ...
    c = f_end_cptr(f);
    SFIFO_TEST(c->start_byte == last_start_byte, "end chunk should start at %u", last_start_byte);
    ...
    for (i = 10; i >= 0; i--) {
        ...
        rv      = svm_fifo_peek(f, offset, deq_now, data_buf + offset);
        if (rv != deq_now) SFIFO_TEST(0, "failed to peek");
    }
    ...
    rv = svm_fifo_n_chunks(f);
    SFIFO_TEST(rv == (fifo_size / 4096) + 1, "should have %u chunks has %u", (fifo_size / 4096) + 1, rv);
...
static int sfifo_test_fifo_shrink(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    rv = svm_fifo_n_chunks(f);
    SFIFO_TEST(rv == (fifo_size / 4096), "should have %u chunks has %u", (fifo_size / 4096), rv);
     ...
     */
    rv = dequeue_ooo(f, data_buf + deq_chunk, fifo_size - deq_chunk, fifo_size - deq_chunk);
    SFIFO_TEST(!rv, "ooo deq should work %d", rv);
    ...
    for (i = 0; i < 1000; i++) {
        rv = svm_fifo_enqueue(f, enq_chunk, test_data);
        if (rv != enq_chunk) SFIFO_TEST(0, "enq fail");
        rv = svm_fifo_dequeue(f, deq_chunk, data_buf);
        if (rv != deq_chunk) SFIFO_TEST(0, "deq fail");
    }
...
static int sfifo_test_fifo_indirect(vlib_main_t *vm, unformat_input_t *input)
{
    int __clib_unused   verbose = 0, fifo_size = 4096, deq_chunk;
    fifo_segment_main_t _fsm = {0}, *fsm = &_fsm;
    u8                 *test_data = 0, *data_buf = 0;
    ...
    SFIFO_TEST(f_cptr(f, c->next)->length == 4 << 20, "new chunk should be 4MB");
...
static int sfifo_test_fifo_replay(vlib_main_t *vm, unformat_input_t *input)
{
    svm_fifo_t f;
    int        verbose = 0;
    u8         no_read = 0, *str = 0;
...
static int sfifo_test_fifo_make_rcv_wnd_zero(vlib_main_t *vm, unformat_input_t *input)
{
    int __clib_unused   verbose = 0, fifo_size = 4096, deq_chunk;
    fifo_segment_main_t _fsm = {0}, *fsm = &_fsm;
    u8                 *test_data = 0, *data_buf = 0;
...
static int sfifo_test_fifo_segment_hello_world(int verbose)
{
    fifo_segment_create_args_t _a, *a = &_a;
    ...
    SFIFO_TEST(!memcmp(retrieved_data, test_data, vec_len(test_data)), "data should be identical");
...
static int sfifo_test_fifo_segment_fifo_grow(int verbose)
{
    ...
    SFIFO_TEST(n_chunks == n_batch - 1,
               "free 2^10B chunks "
               "should be %u is %u",
               n_batch - 1,
               n_chunks);
    rv = fifo_segment_fl_chunk_bytes(fs);
    SFIFO_TEST(rv == (n_batch - 1) * fifo_size,
               "free chunk bytes %u "
               "expected %u",
               rv,
               (n_batch - 1) * fifo_size);
    ...
    n_chunks = fifo_segment_num_free_chunks(fs, fifo_size);
    SFIFO_TEST(n_chunks == n_batch - 2,
               "free 2^10B chunks "
               "should be %u is %u",
               n_batch - 2,
               n_chunks);
    rv = fifo_segment_fl_chunk_bytes(fs);
    SFIFO_TEST(rv == (n_batch - 2) * fifo_size,
               "free chunk bytes %u "
               "expected %u",
               rv,
               (n_batch - 2) * fifo_size);
    ...
    SFIFO_TEST(rv > 16 * fifo_size, "free bytes %u more than %u", rv, 16 * fifo_size);
    ...
    SFIFO_TEST(svm_fifo_size(f) == 18 * fifo_size,
               "fifo size should be %u "
               "is %u",
               18 * fifo_size,
               svm_fifo_size(f));
    ...
    rv = fifo_segment_fl_chunk_bytes(fs);
    SFIFO_TEST(rv == (16 + n_batch) * fifo_size,
               "free chunk bytes expected %u"
               " is %u",
               (16 + n_batch) * fifo_size,
               rv);
    n_chunks = fifo_segment_num_free_chunks(fs, fifo_size);
    SFIFO_TEST(n_chunks == n_batch,
               "free 2^10B chunks "
               "should be %u is %u",
               n_batch,
               n_chunks);
    n_chunks = fifo_segment_num_free_chunks(fs, 16 * fifo_size);
    SFIFO_TEST(n_chunks == 1, "free 2^14B chunks should be %u is %u", 1, n_chunks);
    n_chunks = fifo_segment_num_free_chunks(fs, ~0);
    SFIFO_TEST(n_chunks == 1 + n_batch, "free chunks should be %u is %u", 1 + n_batch, n_chunks);
    ...
    n_chunks = fifo_segment_num_free_chunks(fs, fifo_size);
    SFIFO_TEST(n_chunks == n_batch - 2, "free 2^10B chunks should be %u is %u", n_batch - 2, n_chunks);
    ...
    n_chunks = fifo_segment_num_free_chunks(fs, 16 * fifo_size);
    SFIFO_TEST(n_chunks == 0, "free 2^14B chunks should be %u is %u", 0, n_chunks);
    n_chunks = fifo_segment_num_free_chunks(fs, ~0);
    SFIFO_TEST(n_chunks == n_batch - 2, "free chunks should be %u is %u", n_batch - 2, n_chunks);
    ...
    SFIFO_TEST(n_free_chunk_bytes - 16 * fifo_size == rv,
               "free chunk bytes "
               "expected %u is %u",
               n_free_chunk_bytes - 16 * fifo_size,
               rv);
    SFIFO_TEST(svm_fifo_is_sane(f), "fifo should be sane");
    ...
    SFIFO_TEST(n_free_chunk_bytes == rv, "free chunk bytes expected %u is %u", n_free_chunk_bytes, rv);
    ...
    rv = fifo_segment_free_bytes(fs);
    SFIFO_TEST(rv < 32 * fifo_size, "free bytes %u less than %u", rv, 32 * fifo_size);
    ...
    SFIFO_TEST(n_free_chunk_bytes - 17 * fifo_size == rv,
               "free chunk bytes "
               "expected %u is %u",
               n_free_chunk_bytes - 17 * fifo_size,
               rv);
     ...
     */
    tf = fifo_segment_alloc_fifo(fs, n_free_chunk_bytes / 2, FIFO_SEGMENT_RX_FIFO);
    SFIFO_TEST(tf != 0, "allocation should work");
    SFIFO_TEST(svm_fifo_is_sane(tf), "fifo should be sane");

    f = fifo_segment_alloc_fifo(fs, n_free_chunk_bytes / 2, FIFO_SEGMENT_RX_FIFO);
    ...
    f = fifo_segment_alloc_fifo(fs, n_free_chunk_bytes + fifo_size, FIFO_SEGMENT_RX_FIFO);
    SFIFO_TEST(f == 0, "allocation should fail");

    /*
     * Allocate fifo and try to grow beyond available space
     */
    f = fifo_segment_alloc_fifo(fs, fifo_segment_free_bytes(fs), FIFO_SEGMENT_RX_FIFO);
    ...
    SFIFO_TEST(rv != new_size, "grow should fail size %u wrote %d", new_size, rv);
...
static int sfifo_test_fifo_segment_slave(int verbose)
{
    fifo_segment_create_args_t _a, *a = &_a;
    fifo_segment_main_t       *sm = &segment_main;
    u8                        *test_data, *retrieved_data = 0;
...
static int sfifo_test_fifo_segment_master_slave(int verbose)
{
    fifo_segment_create_args_t _a, *a = &_a;
    fifo_segment_main_t       *sm = &segment_main;
    fifo_segment_t            *sp;
    ...
    pid = fork();
    if (pid < 0) SFIFO_TEST(0, "fork failed");

    if (!pid) sfifo_test_fifo_segment_slave(verbose);
...
static int sfifo_test_fifo_segment_mempig(int verbose)
{
    fifo_segment_create_args_t _a, *a = &_a;
    fifo_segment_main_t       *sm = &segment_main;
    fifo_segment_t            *sp;
    ...
    SFIFO_TEST(vec_len(flist), "second try created %d fifos", vec_len(flist));
    for (i = 0; i < vec_len(flist); i++) {
        f = flist[i];
        fifo_segment_free_fifo(sp, f);
    }
...
static int approx_leq(uword a, uword b, u32 margin)
{
    if (a - margin <= b && b <= a) return 1;
    return 0;
}

static int sfifo_test_fifo_segment_prealloc(int verbose)
{
    ...
    SFIFO_TEST(free_space - 4096 <= 256 << 10, "free space expected %u is %u", 256 << 10, free_space);
    ...
    SFIFO_TEST(approx_leq(free_space, rv, 16), "free space expected %u is %u", free_space, rv);
    free_space = rv;
    rv         = fifo_segment_fl_chunk_bytes(fs);
    SFIFO_TEST(rv == 4096 * 50, "chunk free space expected %u is %u", 4096 * 50, rv);
    ...
    /* Memory alloc alignment accounts for the difference */
    SFIFO_TEST(approx_leq(free_space, rv, 128), "free space expected %u is %u", free_space, rv);
    free_space = rv;

    rv = fifo_segment_free_bytes(fs);
    SFIFO_TEST(clib_abs(rv - (int) free_space) < 512, "free space expected %u is %u", free_space, rv);
    ...
    rv = fifo_segment_num_free_chunks(fs, 4096);
    SFIFO_TEST(rv == max_pairs * 2, "prealloc chunks expected %u is %u", max_pairs * 2, rv);

    rv = fifo_segment_free_bytes(fs);
    SFIFO_TEST(rv < 2 * pair_mem, "free bytes %u less than %u", rv, 2 * pair_mem);
    ...
    rv = fifo_segment_num_free_chunks(fs, 4096);
    SFIFO_TEST(rv == max_pairs * 2 + alloc,
               "prealloc chunks expected %u "
               "is %u",
               max_pairs * 2 + alloc,
               rv);

    rv = fifo_segment_free_bytes(fs);
    SFIFO_TEST(rv < pair_mem, "free bytes expected less than %u is %u", pair_mem, rv);
...
static int sfifo_test_fifo_segment(vlib_main_t *vm, unformat_input_t *input)
{
    int rv, verbose = 0;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "verbose"))
            ...
            if ((rv = sfifo_test_fifo_segment_master_slave(verbose))) return -1;
        }
        else if (unformat(input, "basic")) {
            if ((rv = sfifo_test_fifo_segment_hello_world(verbose))) return -1;
        }
        else if (unformat(input, "mempig")) {
            if ((rv = sfifo_test_fifo_segment_mempig(verbose))) return -1;
        }
        else if (unformat(input, "grow fifo")) {
            if ((rv = sfifo_test_fifo_segment_fifo_grow(verbose))) return -1;
        }
        else if (unformat(input, "prealloc")) {
            if ((rv = sfifo_test_fifo_segment_prealloc(verbose))) return -1;
        }
...
static clib_error_t *svm_fifo_test(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
    int   res = 0;
    char *str;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "fifo1"))
            ...
            if ((res = sfifo_test_fifo1(vm, input))) goto done;

            if ((res = sfifo_test_fifo2(vm))) goto done;
            ...
            if ((res = sfifo_test_fifo3(vm, input))) goto done;
            unformat_free(input);
            ...
            if ((res = sfifo_test_fifo4(vm, input))) goto done;

            if ((res = sfifo_test_fifo5(vm, input))) goto done;

            if ((res = sfifo_test_fifo6(vm, input))) goto done;

            if ((res = sfifo_test_fifo7(vm, input))) goto done;

            if ((res = sfifo_test_fifo_grow(vm, input))) goto done;

            if ((res = sfifo_test_fifo_shrink(vm, input))) goto done;

            if ((res = sfifo_test_fifo_indirect(vm, input))) goto done;

            if ((res = sfifo_test_fifo_make_rcv_wnd_zero(vm, input))) goto done;

            str = "all";
            unformat_init_cstring(input, str);
            if ((res = sfifo_test_fifo_segment(vm, input))) goto done;
        }
        else {
            vlib_cli_output(vm, "unknown input `%U'", format_unformat_error, input);
            res = -1;
            goto done;
        }
    }
    ...
    if (res) return clib_error_return(0, "svm fifo unit test failed");
    return 0;
}
