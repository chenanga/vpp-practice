...
static int test_time_range_main(unformat_input_t *input)
{
    ...
    /* Init time base */
    clib_timebase_init(tb, -5 /* EST */, CLIB_TIMEBASE_DAYLIGHT_USA, &vm->clib_time);
    ...
    fformat(stdout, "%U should not be in DST, and it %s\n", format_clib_timebase_time, then, (timezone_offset != 0.0) ? "is" : "is not");
    ...
    if (unformat(input2, "%U", unformat_clib_timebase_range_vector, &rp)) {
        vec_foreach (this_rp, rp) {
            ...
            fformat(stdout, "range: %U - %U\n", format_clib_timebase_time, start_time, format_clib_timebase_time, end_time);
        }
        vec_free(rp);
    }
    else {
...
static clib_error_t *test_time_range_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    int rv;

    rv = test_time_range_main(input);

    if (rv) return clib_error_return(0, "test time range FAILED, error %d", rv);
