...
void placeholder_session_reset_callback(session_t *s)
{
    clib_warning("called...");
}
...
int          placeholder_session_connected_callback(u32 app_index, u32 api_context, session_t *s, session_error_t err)
{
    if (s) {
...
int placeholder_add_segment_callback(u32 client_index, u64 segment_handle)
{
    placeholder_segment_count = 1;
    return 0;
}

int placeholder_del_segment_callback(u32 client_index, u64 segment_handle)
{
    placeholder_segment_count = 0;
    return 0;
}

void placeholder_session_disconnect_callback(session_t *s)
{
    clib_warning("called...");
}
...
int placeholder_session_accept_callback(session_t *s)
{
...
int placeholder_server_rx_callback(session_t *s)
{
    clib_warning("called...");
    return -1;
}
...
static int session_create_lookpback(u32 table_id, u32 *sw_if_index, ip4_address_t *intf_addr)
{
    u8 intf_mac[6];
...
static void session_delete_loopback(u32 sw_if_index)
{
    /* fails spectacularly  */
...
static int session_test_basic(vlib_main_t *vm, unformat_input_t *input)
{
...
static void session_add_del_route_via_lookup_in_table(u32 in_table_id, u32 via_table_id, ip4_address_t *ip, u8 mask, u8 is_add)
{
...
static int session_test_endpoint_cfg(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    session_add_del_route_via_lookup_in_table(0, 1, &intf_addr[1], 32, 1 /* is_add */);
    session_add_del_route_via_lookup_in_table(1, 0, &intf_addr[0], 32, 1 /* is_add */);
    ...
    SESSION_TEST((memcmp(&tc->lcl_ip, &client_sep.peer.ip, sizeof(tc->lcl_ip)) == 0), "ips should be equal");
    ...
    session_add_del_route_via_lookup_in_table(0, 1, &intf_addr[1], 32, 0 /* is_add */);
...
static int session_test_namespace(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    SESSION_TEST((app_ns->ns_secret == placeholder_secret), "secret should be %d", placeholder_secret);
    SESSION_TEST((app_ns->sw_if_index == APP_NAMESPACE_INVALID_INDEX), "sw_if_index should be invalid");
    ...
    SESSION_TEST((s->app_wrk_index == server_wrk_index),
                 "app_index should be"
                 " that of the server");
    server_local_st_index = application_local_session_table(server);
    SESSION_TEST((server_local_st_index == APP_INVALID_INDEX), "server shouldn't have access to local table");
    ...
    server           = application_get(server_index);
    server_wrk_index = application_get_default_worker(server)->wrk_index;
    SESSION_TEST((server->ns_index == app_namespace_index(app_ns)), "server should be in the right ns");
    ...
    SESSION_TEST((error == SESSION_E_INVALID_RMT_IP), "error code should be invalid value (zero ip)");
    SESSION_TEST((placeholder_segment_count == 0), "shouldn't have received request to map new segment");
    ...
    error = vnet_connect(&connect_args);
    SESSION_TEST((error != 0), "client connect should return error code");
    SESSION_TEST((error == SESSION_E_NOINTF), "error code should be connect (nothing in local scope)");
    detach_args.app_index = client_index;
    vnet_application_detach(&detach_args);
    ...
    server_index = attach_args.app_index;
    server       = application_get(server_index);
    SESSION_TEST((server->ns_index == app_namespace_index(app_ns)), "app should be in the right ns");
...
static void session_test_disable_rt_backend_engine(vlib_main_t *vm)
{
    session_enable_disable_args_t args = {.is_en = 0, .rt_engine_type = RT_BACKEND_ENGINE_DISABLE};
    vnet_session_enable_disable(vm, &args);
}

static void session_test_enable_rule_table_engine(vlib_main_t *vm)
{
    session_enable_disable_args_t args = {.is_en = 1, .rt_engine_type = RT_BACKEND_ENGINE_RULE_TABLE};
    vnet_session_enable_disable(vm, &args);
}

static void session_test_enable_sdl_engine(vlib_main_t *vm)
{
    session_enable_disable_args_t args = {.is_en = 1, .rt_engine_type = RT_BACKEND_ENGINE_SDL};
    vnet_session_enable_disable(vm, &args);
}

static int session_test_rule_table(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    res = session_rules_table_lookup4(st->srtg_handle, TRANSPORT_PROTO_TCP, &lcl_ip, &rmt_ip, lcl_port, rmt_port);
    SESSION_TEST((res == 1),
    ...
    res             = session_rules_table_lookup4(st->srtg_handle, TRANSPORT_PROTO_TCP, &lcl_lkup, &rmt_lkup, lcl_port, rmt_port);
    SESSION_TEST((res == 1),
                 "Lookup 1.2.2.4 1234 5.6.7.9 4321, action "
                 "should be 1: %d",
                 res);

    res = session_rules_table_lookup4(st->srtg_handle, TRANSPORT_PROTO_TCP, &lcl_ip3, &rmt_ip3, lcl_port, rmt_port);
    SESSION_TEST((res == 6),
    ...
    SESSION_TEST((res == 7),
                 ...
                 " be 7 (lpm dst): %d",
                 res);
    res = session_rules_table_lookup4(st->srtg_handle, TRANSPORT_PROTO_TCP, &lcl_ip, &rmt_ip, lcl_port + 1, rmt_port);
    SESSION_TEST((res == 7),
    ...
    if (verbose) session_rules_table_cli_dump(vm, st->srtg_handle, TRANSPORT_PROTO_TCP, FIB_PROTOCOL_IP4);
    ...
    SESSION_TEST((res == 9),
                 ...
                 "be 9: %d",
                 res);
    res = session_rules_table_lookup4(st->srtg_handle, TRANSPORT_PROTO_TCP, &lcl_ip, &rmt_ip, lcl_port + 1, rmt_port + 1);
    SESSION_TEST((res == 8),
...
static int session_test_rules(vlib_main_t *vm, unformat_input_t *input)
{
    session_endpoint_t      server_sep = SESSION_ENDPOINT_NULL;
    ...
    bind_args.app_index = server_index;
    error               = vnet_listen(&bind_args);
    SESSION_TEST((error == 0), "server bound to %U/%d", format_ip46_address, &server_sep.ip, 1, server_sep.port);
    ...
    tc = session_lookup_connection_wt4(0, &lcl_pref.fp_addr.ip4, &rmt_pref.fp_addr.ip4, lcl_port, rmt_port, TRANSPORT_PROTO_TCP, 0, &is_filtered);
    ...
    SESSION_TEST((error == 0), "Add 1.2.3.4/16 1234 5.6.7.8/16 4321 action %d", args.table_args.action_index);

    tc = session_lookup_connection4(0, &lcl_pref.fp_addr.ip4, &rmt_pref.fp_addr.ip4, lcl_port, rmt_port, TRANSPORT_PROTO_TCP);
    SESSION_TEST((tc->c_index == listener->connection_index), "optimized lookup should return the listener");
    tc = session_lookup_connection_wt4(0, &lcl_pref.fp_addr.ip4, &rmt_pref.fp_addr.ip4, lcl_port, rmt_port, TRANSPORT_PROTO_TCP, 0, &is_filtered);
    SESSION_TEST((tc->c_index == listener->connection_index), "lookup should return the listener");
    s = session_lookup_safe4(0, &lcl_pref.fp_addr.ip4, &rmt_pref.fp_addr.ip4, lcl_port, rmt_port, TRANSPORT_PROTO_TCP);
    SESSION_TEST((s->connection_index == listener->connection_index), "safe lookup should return the listener");
    ...
    SESSION_TEST((handle != server_index),
                 "local session endpoint lookup "
                 "should not work (global scope)");

    tc = session_lookup_connection_wt4(0, &lcl_pref.fp_addr.ip4, &rmt_pref.fp_addr.ip4, lcl_port + 1, rmt_port, TRANSPORT_PROTO_TCP, 0, &is_filtered);
    ...
    error                    = vnet_session_rule_add_del(&args);
    SESSION_TEST((error == 0), "Add 1.2.3.4/16 * 5.6.7.8/16 4321 action %d", args.table_args.action_index);
    tc = session_lookup_connection_wt4(0, &lcl_pref.fp_addr.ip4, &rmt_pref.fp_addr.ip4, lcl_port + 1, rmt_port, TRANSPORT_PROTO_TCP, 0, &is_filtered);
    SESSION_TEST((tc->c_index == listener->connection_index), "optimized lookup for lcl port + 1 should work");
    handle = session_lookup_local_endpoint(local_ns_index, &sep);
    SESSION_TEST((handle == server_index),
                 "local session endpoint lookup "
    ...
    error                           = vnet_session_rule_add_del(&args);
    SESSION_TEST((error == 0), "Add 1.2.3.4/30 1234 5.6.7.8/30 4321 action %d", args.table_args.action_index);
    ...
    tc = session_lookup_connection_wt4(0, &lcl_pref.fp_addr.ip4, &rmt_pref.fp_addr.ip4, lcl_port, rmt_port, TRANSPORT_PROTO_TCP, 0, &is_filtered);
    SESSION_TEST((tc == 0),
                 "lookup for 1.2.3.4/32 1234 5.6.7.8/16 4321 "
                 "should fail (deny rule)");
    SESSION_TEST((is_filtered == SESSION_LOOKUP_RESULT_FILTERED), "lookup should be filtered (deny)");
    ...
    tc = session_lookup_connection_wt4(0, &lcl_pref.fp_addr.ip4, &rmt_pref.fp_addr.ip4, lcl_port + 1, rmt_port, TRANSPORT_PROTO_TCP, 0, &is_filtered);
    SESSION_TEST((tc->c_index == listener->connection_index), "lookup 1.2.3.4/32 123*5* 5.6.7.8/16 4321 should work");
    ...
    handle = session_lookup_local_endpoint(local_ns_index, &sep);
    SESSION_TEST((handle == SESSION_DROP_HANDLE), "local session endpoint lookup should return deny");
    ...
    handle = session_lookup_local_endpoint(local_ns_index, &sep);
    SESSION_TEST((handle == SESSION_INVALID_HANDLE), "local session endpoint lookup should return invalid");
    ...
    tc = session_lookup_connection_wt4(0, &lcl_pref.fp_addr.ip4, &rmt_pref.fp_addr.ip4, lcl_port, rmt_port, TRANSPORT_PROTO_TCP, 0, &is_filtered);
    SESSION_TEST((tc == 0),
                 "lookup 1.2.3.4/32 1234 5.6.7.8/16 4321 should "
                 "not work (del + deny)");
    ...
    error                           = vnet_session_rule_add_del(&args);
    SESSION_TEST((error == 0),
                 "Add 1.2.3.4/16 1234 5.6.7.8/16 4321 deny "
                 "tag test_rule");
    if (verbose) {
        session_lookup_dump_rules_table(0, FIB_PROTOCOL_IP4, TRANSPORT_PROTO_TCP);
        session_lookup_dump_local_rules_table(local_ns_index, FIB_PROTOCOL_IP4, TRANSPORT_PROTO_TCP);
    ...
    tc = session_lookup_connection_wt4(0, &lcl_pref.fp_addr.ip4, &rmt_pref.fp_addr.ip4, lcl_port, rmt_port, TRANSPORT_PROTO_TCP, 0, &is_filtered);
    SESSION_TEST((tc->c_index == listener->connection_index), "lookup 1.2.3.4/32 1234 5.6.7.8/16 4321 should work");
    ...
    tc = session_lookup_connection_wt4(0, &lcl_pref.fp_addr.ip4, &rmt_pref.fp_addr.ip4, lcl_port, rmt_port, TRANSPORT_PROTO_TCP, 0, &is_filtered);
    SESSION_TEST((tc == 0),
                 "lookup 1.2.3.4/32 1234 5.6.7.8/32 4321 should not"
                 " work (del)");
    ...
    error                           = vnet_session_rule_add_del(&args);
    SESSION_TEST((error == 0), "Add 1.2.3.4/32 1234 5.6.7.8/32 4321 action %d", args.table_args.action_index);
    /*
    ...
    vnet_app_namespace_add_del_args_t ns_args = {.ns_id = ns_id, .secret = 0, .sw_if_index = APP_NAMESPACE_INVALID_INDEX, .is_add = 1};
    error                                     = vnet_app_namespace_add_del(&ns_args);
    SESSION_TEST((error == 0), "app ns insertion should succeed: %d", error);
    app_ns = app_namespace_get_from_id(ns_id);
...
static int session_test_proxy(vlib_main_t *vm, unformat_input_t *input)
{
    u64                     options[APP_OPTIONS_N_OPTIONS];
    ...
    tc = session_lookup_connection_wt4(0, &lcl_ip, &rmt_ip, lcl_port, rmt_port, TRANSPORT_PROTO_TCP, 0, &is_filtered);
    SESSION_TEST((tc != 0),
                 "lookup 1.2.3.4 1234 5.6.7.8 4321 should be "
                 "successful");
    s  = listen_session_get(tc->s_index);
    al = app_listener_get(s->al_index);
    SESSION_TEST((al->app_index == server_index),
                 "lookup should return"
                 " the server");

    tc = session_lookup_connection_wt4(0, &rmt_ip, &rmt_ip, lcl_port, rmt_port, TRANSPORT_PROTO_TCP, 0, &is_filtered);
    SESSION_TEST((tc == 0),
                 "lookup 5.6.7.8 1234 5.6.7.8 4321 should"
                 " not work");

    app_index = session_lookup_local_endpoint(app_ns->local_table_index, &sep);
    SESSION_TEST((app_index == server_index),
                 "local session endpoint lookup"
                 " should work");
    ...
    app_index = session_lookup_local_endpoint(app_ns->local_table_index, &sep);
    SESSION_TEST((app_index == SESSION_RULES_TABLE_INVALID_INDEX), "local session endpoint lookup should not work after detach");
    if (verbose) unformat_free(&tmp_input);
    vec_free(attach_args.name);
    session_delete_loopback(sw_if_index);
    return 0;
}

static inline void wait_for_event(svm_msg_q_t *mq, int fd, int epfd, u8 use_eventfd)
{
    if (!use_eventfd) {
        ...
        while (1) {
            ...
            if (!svm_msg_q_is_empty(mq)) break;
        }
    }
}
 ...
 * async rx
 */
static int test_mq_try_lock_and_alloc_msg(svm_msg_q_t *mq, session_mq_rings_e ring, svm_msg_q_msg_t *msg)
{
    int rv, n_try = 0;
 ...
 */
static inline int test_app_send_io_evt_rx(app_worker_t *app_wrk, session_t *s)
{
    ...
    if (app_worker_application_is_builtin(app_wrk)) return app_worker_rx_notify(app_wrk, s);
...
static int session_test_mq_speed(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    pid = fork();
    if (pid < 0) SESSION_TEST(0, "fork failed");
...
static int session_test_mq_basic(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    *(u32 *) svm_msg_q_msg_data(mq, &msg2) = 123;
    ...
    for (i = 0; i < 12; i++) {
        if (svm_msg_q_sub(mq, &msg[i], SVM_Q_NOWAIT, 0)) SESSION_TEST(0, "dequeue2");
        if (i < 8) {
            if (msg[i].ring_index != 0 || msg[i].elt_index != (i + 1) % 8) SESSION_TEST(0, "dequeue2 result2");
        }
        else {
            if (msg[i].ring_index != 1 || msg[i].elt_index != (i - 8) + 1) SESSION_TEST(0, "dequeue2 result3");
        }
        if (*(u32 *) svm_msg_q_msg_data(mq, &msg[i]) != i) SESSION_TEST(0, "dequeue2 wrong data");
...
static f32 session_get_memory_usage(void)
{
...
static int session_test_enable_disable(vlib_main_t *vm, unformat_input_t *input)
{
    u32   iteration = 100, i;
    uword was_enabled;
    ...
    SESSION_TEST((was_using == now_using), "was using %.2fM, now using %.2fM", was_using, now_using);

    return 0;
}

static int session_test_sdl(vlib_main_t *vm, unformat_input_t *input)
{
    session_table_t *st       = session_table_alloc();
    ...
    error = session_rules_table_add_del(st->srtg_handle, TRANSPORT_PROTO_TCP, &args);
    SESSION_TEST((error == 0), "Add %s/%d action %d", ip_str_1200, args.lcl.fp_len, action_index - 1);
    ...
    args.action_index = action_index++;
    error             = session_rules_table_add_del(st->srtg_handle, TRANSPORT_PROTO_TCP, &args);
    SESSION_TEST((error == 0), "Add %s/%d action %d", ip_str_1230, args.lcl.fp_len, action_index - 1);
    ...
    inet_pton(AF_INET, ip_str_1200, &args.lcl.fp_addr.ip4.as_u32);
    error = session_rules_table_add_del(st->srtg_handle, TRANSPORT_PROTO_TCP, &args);
    SESSION_TEST((error == SESSION_E_IPINUSE), "Add %s/%d action %d", ip_str_1200, args.lcl.fp_len, error);
    /*
    ...
    error                       = session_rules_table_add_del(st->srtg_handle, TRANSPORT_PROTO_TCP, &args);
    SESSION_TEST((error == SESSION_E_IPINUSE), "Add 0.0.0.0/%d action %d", args.lcl.fp_len, error);

    /* delete 0.0.0.0 should be rejected */
    args.is_add = 0;
    error       = session_rules_table_add_del(st->srtg_handle, TRANSPORT_PROTO_TCP, &args);
    SESSION_TEST((error == SESSION_E_NOROUTE), "Del 0.0.0.0/%d action %d", args.lcl.fp_len, error);
    if (verbose) session_rules_table_cli_dump(vm, st->srtg_handle, TRANSPORT_PROTO_TCP, FIB_PROTOCOL_IP4);
    ...
    args.is_add     = 0;
    error           = session_rules_table_add_del(st->srtg_handle, TRANSPORT_PROTO_TCP, &args);
    SESSION_TEST((error == 0), "Del %s/%d should 0: %d", ip_str_1200, args.lcl.fp_len, error);

    inet_pton(AF_INET, ip_str_1230, &args.lcl.fp_addr.ip4.as_u32);
    args.lcl.fp_len = 24;
    error           = session_rules_table_add_del(st->srtg_handle, TRANSPORT_PROTO_TCP, &args);
    SESSION_TEST((error == 0), "Del %s/%d, should be 0: %d", ip_str_1230, args.lcl.fp_len, error);
    if (verbose) session_rules_table_cli_dump(vm, st->srtg_handle, TRANSPORT_PROTO_TCP, FIB_PROTOCOL_IP4);
    ...
    args.is_add       = 1;
    error             = session_rules_table_add_del(st->srtg_handle, TRANSPORT_PROTO_TCP, &args);
    SESSION_TEST((error == 0), "Add %s/%d action %d", ip6_str, args.lcl.fp_len, action_index - 1);
    if (verbose) session_rules_table_cli_dump(vm, st->srtg_handle, TRANSPORT_PROTO_TCP, FIB_PROTOCOL_IP6);

    /* Lookup 2001:0db8:85a3:0000:0000:8a2e:0371:1 */
    res = session_rules_table_lookup6(st->srtg_handle, TRANSPORT_PROTO_TCP, &args.lcl.fp_addr.ip6, &rmt_ip6, lcl_port, rmt_port);
    SESSION_TEST((res == action_index - 1),
                 "Lookup %s action should "
                 "be 3: %d",
                 ip6_str,
    ...
    res = session_rules_table_lookup6(st->srtg_handle, TRANSPORT_PROTO_TCP, &lcl_lkup, &rmt_ip6, lcl_port, rmt_port);
    SESSION_TEST((res == SESSION_TABLE_INVALID_INDEX),
                 "Lookup %s action should "
                 "be -1: %d",
                 ip6_str2,
    ...
    args.lcl.fp_len = 124;
    error           = session_rules_table_add_del(st->srtg_handle, TRANSPORT_PROTO_TCP, &args);
    SESSION_TEST((error == 0), "del %s/%d, should be 0: %d", ip6_str, args.lcl.fp_len, error);
    if (verbose) session_rules_table_cli_dump(vm, st->srtg_handle, TRANSPORT_PROTO_TCP, FIB_PROTOCOL_IP6);
...
static clib_error_t *session_test(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd_arg)
{
    int res = 0;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "basic"))
            ...
            if ((res = session_test_basic(vm, input))) goto done;
            if ((res = session_test_namespace(vm, input))) goto done;
            if ((res = session_test_rule_table(vm, input))) goto done;
            if ((res = session_test_rules(vm, input))) goto done;
            if ((res = session_test_proxy(vm, input))) goto done;
            if ((res = session_test_endpoint_cfg(vm, input))) goto done;
            if ((res = session_test_mq_speed(vm, input))) goto done;
            if ((res = session_test_mq_basic(vm, input))) goto done;
            if ((res = session_test_sdl(vm, input))) goto done;
            if ((res = session_test_enable_disable(vm, input))) goto done;
        }
        else
            break;
    }
    ...
    if (res) return clib_error_return(0, "Session unit test failed");
    return 0;
}
