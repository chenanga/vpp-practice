...
u8 *format_virtchnl_op_name(u8 *s, va_list *args)
{
...
#undef _
    ...
    if (op >= ARRAY_LEN(op_names) || op_names[op] == 0) return format(s, "UNKNOWN(%u)", op);

    return format(s, "%s", op_names[op]);
}

u8 *format_virtchnl_status(u8 *s, va_list *args)
{
...
static u8 *format_virtchnl_vlan_support_caps(u8 *s, va_list *args)
{
...
static u8 *format_virtchnl_op_req(u8 *s, va_list *args)
{
    virtchnl_op_t op     = va_arg(*args, virtchnl_op_t);
    void         *p      = va_arg(*args, void *);
    u32           indent = format_get_indent(s);

    if (p == 0) return format(s, "no data");
    ...
    switch (op) {
        ...
        case VIRTCHNL_OP_GET_STATS: {
            ...
            s                          = format(s, "vsi %u rx 0x%x tx 0x%x", r->vsi_id, r->rx_queues, r->tx_queues);
        } break;
        ...
        case VIRTCHNL_OP_DEL_ETH_ADDR: {
            ...
            s                             = format(s, "vsi %u num_elements %u elts: ", r->vsi_id, r->num_elements);
            for (int i = 0; i < r->num_elements; i++)
                s = format(s,
                           ...
                           r->list[i].addr,
                           r->list[i].primary ? " primary" : "",
                           r->list[i].extra ? " extra" : "");
        } break;
        case VIRTCHNL_OP_CONFIG_PROMISCUOUS_MODE: {
            virtchnl_promisc_info_t *r = p;
            s                          = format(s,
                       "promisc_info: vsi %u flags 0x%x (unicast %s multicast %s)",
                       r->vsi_id,
                       r->flags,
...
static u8 *format_virtchnl_op_resp(u8 *s, va_list *args)
{
    virtchnl_op_t op     = va_arg(*args, virtchnl_op_t);
    void         *p      = va_arg(*args, void *);
    u32           indent = format_get_indent(s);
    ...
    switch (op) {
        ...
        case VIRTCHNL_OP_GET_VF_RESOURCES: {
            ...
            s                         = format(s,
                       ...
                       r->rss_key_size,
                       r->rss_lut_size);
            s = format(s, "\n%Uvf_cap_flags 0x%x (%U)", format_white_space, indent + 2, r->vf_cap_flags, format_iavf_vf_cap_flags, r->vf_cap_flags);
            for (int i = 0; i < r->num_vsis; i++)
                s = format(s,
                           ...
                           i,
                           r->vsi_res[i].vsi_id,
                           r->vsi_res[i].num_queue_pairs,
                           r->vsi_res[i].vsi_type,
        ...
        case VIRTCHNL_OP_GET_OFFLOAD_VLAN_V2_CAPS: {
            ...
            s = format(s, "filtering: ethertype_init 0x%x max_filters %u", r->filtering.ethertype_init, r->filtering.max_filters);
            s = format(s,
                       "\n%U outer [%U] inner [%U]",
                       format_white_space,
                       indent,
...
vnet_dev_rv_t iavf_virtchnl_req(vlib_main_t *vm, vnet_dev_t *dev, iavf_virtchnl_req_t *r)
{
    ...
    log_debug(dev, "%U req:\n  %U", format_virtchnl_op_name, r->op, format_virtchnl_op_req, r->op, r->req);
    ...
    if (r->no_reply) return VNET_DEV_OK;
...
retry:
    ...
    if (d->v_retval) {
        log_err(dev, "error [v_opcode = %u, v_retval %d]", d->v_opcode, d->v_retval);
        rv = VNET_DEV_ERR_BUG;
        goto done;
    }

    if (r->resp_sz && d->flags.buf) clib_memcpy_fast(r->resp, b, r->resp_sz);

done:
    iavf_aq_arq_next_rel(vm, dev);
    if (rv == VNET_DEV_OK) log_debug(dev, "%U resp:\n  %U", format_virtchnl_op_name, r->op, format_virtchnl_op_resp, r->op, r->resp);
    return rv;
}
