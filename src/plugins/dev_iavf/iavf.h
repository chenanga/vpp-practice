...
vnet_dev_rv_t iavf_port_cfg_change(vlib_main_t *, vnet_dev_port_t *, vnet_dev_port_cfg_change_req_t *);
vnet_dev_rv_t iavf_port_cfg_change_validate(vlib_main_t *, vnet_dev_port_t *, vnet_dev_port_cfg_change_req_t *);
...
static inline u32 iavf_get_u32(void *start, int offset)
{
    return *(u32 *) (((u8 *) start) + offset);
}

static inline void iavf_reg_write(iavf_device_t *ad, u32 addr, u32 val)
{
    __atomic_store_n((u32 *) ((u8 *) ad->bar0 + addr), val, __ATOMIC_RELEASE);
}

static inline u32 iavf_reg_read(iavf_device_t *ad, u32 addr)
{
    return __atomic_load_n((u32 *) (ad->bar0 + addr), __ATOMIC_RELAXED);
    ;
}

static inline void iavf_reg_flush(iavf_device_t *ad)
{
...
#define log_debug(dev, f, ...) \
    vlib_log(VLIB_LOG_LEVEL_DEBUG, iavf_log.class, "%U" f, format_vnet_dev_log, (dev), clib_string_skip_prefix(__func__, "iavf_"), ##__VA_ARGS__)
#define log_info(dev, f, ...)   vlib_log(VLIB_LOG_LEVEL_INFO, iavf_log.class, "%U: " f, format_vnet_dev_addr, (dev), ##__VA_ARGS__)
#define log_notice(dev, f, ...) vlib_log(VLIB_LOG_LEVEL_NOTICE, iavf_log.class, "%U: " f, format_vnet_dev_addr, (dev), ##__VA_ARGS__)
#define log_warn(dev, f, ...)   vlib_log(VLIB_LOG_LEVEL_WARNING, iavf_log.class, "%U: " f, format_vnet_dev_addr, (dev), ##__VA_ARGS__)
#define log_err(dev, f, ...)    vlib_log(VLIB_LOG_LEVEL_ERR, iavf_log.class, "%U: " f, format_vnet_dev_addr, (dev), ##__VA_ARGS__)
