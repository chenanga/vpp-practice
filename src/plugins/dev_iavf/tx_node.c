...
static_always_inline u8 iavf_tx_desc_get_dtyp(iavf_tx_desc_t *d)
{
...
static_always_inline u64 iavf_tx_prepare_cksum(vlib_buffer_t *b, u8 is_tso)
{
    u64 flags = 0;
    if (!is_tso && !(b->flags & VNET_BUFFER_F_OFFLOAD)) return 0;
    ...
    flags |= IAVF_TXD_OFFSET_MACLEN(l2_len) | IAVF_TXD_OFFSET_IPLEN(l3_len) | IAVF_TXD_OFFSET_L4LEN(l4_len);
    flags |= is_ip4 ? IAVF_TXD_CMD_IIPT_IPV4 : IAVF_TXD_CMD_IIPT_IPV6;
    flags |= is_tcp ? IAVF_TXD_CMD_L4T_TCP : IAVF_TXD_CMD_L4T_UDP;

    if (is_ip4) ip4->checksum = 0;
    ...
    if (is_ip4) {
        ...
        psh.l4len               = is_tso ? 0 : clib_host_to_net_u16(clib_net_to_host_u16(ip4->length) - (l4_hdr_offset - l3_hdr_offset));
...
static_always_inline u32 iavf_tx_fill_ctx_desc(vlib_main_t *vm, vnet_dev_tx_queue_t *txq, iavf_tx_desc_t *d, vlib_buffer_t *b)
{
...
next:
    ...
    u16 l234hdr_sz = vnet_buffer(b)->l4_hdr_offset - b->current_data + vnet_buffer2(b)->gso_l4_hdr_sz;
    u16 tlen       = vlib_buffer_length_in_chain(vm, b) - l234hdr_sz;
    d[0].qword[0]  = 0;
    d[0].qword[1]  = IAVF_TXD_DTYP_CTX | IAVF_TXD_CTX_CMD_TSO | IAVF_TXD_CTX_SEG_MSS(vnet_buffer2(b)->gso_size) | IAVF_TXD_CTX_SEG_TLEN(tlen);
    return bi[0];
}

static_always_inline void iavf_tx_copy_desc(iavf_tx_desc_t *d, iavf_tx_desc_t *s, u32 n_descs)
{
...
static_always_inline void iavf_tx_fill_data_desc(vlib_main_t *vm, iavf_tx_desc_t *d, vlib_buffer_t *b, u64 cmd, int use_va_dma)
{
    if (use_va_dma)
...
iavf_tx_prepare(vlib_main_t *vm, vlib_node_runtime_t *node, vnet_dev_tx_queue_t *txq, u32 *buffers, u32 n_packets, u16 *n_enq_descs, int use_va_dma)
{
    iavf_txq_t *atq     = vnet_dev_get_tx_queue_data(txq);
    ...
    while (n_packets_left && n_desc_left) {
        ...
        *(u64x8 *) &b = base_ptr + u64x8_from_u32x8(buf_indices << CLIB_LOG2_CACHE_LINE_BYTES);
...
#if defined CLIB_HAVE_VEC512
        flags_mask_vec512 = u64x8_splat(VLIB_BUFFER_NEXT_PRESENT | VNET_BUFFER_F_OFFLOAD | VNET_BUFFER_F_GSO);
        if (PREDICT_FALSE(!u64x8_is_all_zero(or_flags_vec512 & flags_mask_vec512)))
#else
        if (PREDICT_FALSE(or_flags & (VLIB_BUFFER_NEXT_PRESENT | VNET_BUFFER_F_OFFLOAD | VNET_BUFFER_F_GSO)))
#endif
            goto one_by_one;
    ...
    one_by_one:
        ...
        if (PREDICT_TRUE((flags & (VLIB_BUFFER_NEXT_PRESENT | VNET_BUFFER_F_GSO)) == 0)) {
            u64 cmd = cmd_eop;

            if (PREDICT_FALSE(flags & VNET_BUFFER_F_OFFLOAD)) cmd |= iavf_tx_prepare_cksum(b[0], 0 /* is_tso */);
            ...
            if (flags & VNET_BUFFER_F_GSO) {
                ...
                vlib_error_count(vm, node->node_index, IAVF_TX_NODE_CTR_SEG_SZ_EXCEEDED, 1);
                n_packets_left -= 1;
                buffers += 1;
                continue;
            }

            if (PREDICT_FALSE(n_desc_left < n_desc_needed)) break;
...
retry:
    ...
    if (atq->n_enqueued) {
        ...
        while (1) {
            ...
            if (iavf_tx_desc_get_dtyp(atq->descs + slot[0]) != 0x0F) break;
        ...
        if (complete_slot >= 0) {
            ...
            vlib_buffer_free_from_ring_no_next(vm, atq->buffer_indices, first, txq->size, n_free);
        }
    }
    ...
    if (n_desc) {
        if (PREDICT_TRUE(next + n_desc <= txq->size)) {
            ...
            vlib_buffer_copy_indices(atq->buffer_indices + next, atq->tmp_bufs, n_desc);
        }
        else {
            /* wrap */
            u32 n_not_wrap = txq->size - next;
            iavf_tx_copy_desc(atq->descs + next, atq->tmp_descs, n_not_wrap);
            iavf_tx_copy_desc(atq->descs, atq->tmp_descs + n_not_wrap, n_desc - n_not_wrap);
            vlib_buffer_copy_indices(atq->buffer_indices + next, atq->tmp_bufs, n_not_wrap);
            vlib_buffer_copy_indices(atq->buffer_indices, atq->tmp_bufs + n_not_wrap, n_desc - n_not_wrap);
        }
    ...
    if (n_left) {
        ...
        if (n_retry--) goto retry;

        vlib_buffer_free(vm, buffers, n_left);
        vlib_error_count(vm, node->node_index, IAVF_TX_NODE_CTR_NO_FREE_SLOTS, n_left);
    }
