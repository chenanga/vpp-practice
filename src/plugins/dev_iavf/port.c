...
vnet_dev_rv_t iavf_port_vlan_strip_disable(vlib_main_t *vm, vnet_dev_port_t *port)
{
    ...
    if (ap->vf_cap_flags & VIRTCHNL_VF_OFFLOAD_VLAN_V2) {
        if ((rv = iavf_vc_op_get_offload_vlan_v2_caps(vm, dev, &vc))) return rv;
        ...
        if ((rv = iavf_vc_op_disable_vlan_stripping_v2(vm, dev, &vs))) return rv;
    }

    if (ap->vf_cap_flags & VIRTCHNL_VF_OFFLOAD_VLAN) return iavf_vc_op_disable_vlan_stripping(vm, dev);

    return rv;
}

vnet_dev_rv_t iavf_port_init_rss(vlib_main_t *vm, vnet_dev_port_t *port)
{
    vnet_dev_t         *dev    = port->dev;
    iavf_port_t        *ap     = vnet_dev_get_port_data(port);
    ...
    if (!port->attr.caps.rss) return VNET_DEV_OK;
...
vnet_dev_rv_t iavf_port_update_rss_lut(vlib_main_t *vm, vnet_dev_port_t *port)
{
    vnet_dev_t         *dev      = port->dev;
    iavf_port_t        *ap       = vnet_dev_get_port_data(port);
    ...
    *lut = (virtchnl_rss_lut_t){
    ...
    foreach_vnet_dev_port_rx_queue (q, port)
        if (q->enabled) enabled_rxq_bmp |= 1ULL << q->queue_id;

    /* config RSS LUT */
    for (u32 i = 0, j; i < lut->lut_entries;)
        foreach_set_bit_index (j, enabled_rxq_bmp) {
            lut->lut[i++] = j;
            if (i >= lut->lut_entries) break;
        }

    return iavf_vc_op_config_rss_lut(vm, dev, lut);
}

vnet_dev_rv_t iavf_port_init_vsi_queues(vlib_main_t *vm, vnet_dev_port_t *port)
{
    vnet_dev_t                       *dev = port->dev;
    iavf_port_t                      *ap  = vnet_dev_get_port_data(port);
...
vnet_dev_rv_t iavf_port_rx_irq_config(vlib_main_t *vm, vnet_dev_port_t *port, int enable)
{
    vnet_dev_t              *dev          = port->dev;
    ...
    *im = (virtchnl_irq_map_info_t){
    ...
    if (port->attr.caps.interrupt_mode) {
        ...
        if (enable)
            foreach_vnet_dev_port_rx_queue (rxq, port)
                if (rxq->enabled) {
                    ...
                    n_intr_mode_queues_per_vector[i] += u64_is_bit_set(ap->intr_mode_per_rxq_bitmap, rxq->queue_id);
                }
    }
    else {
        ...
        if (enable)
            foreach_vnet_dev_port_rx_queue (rxq, port)
                if (rxq->enabled) im->vecmap[0].rxq_map |= 1 << rxq->queue_id;
    }

    if ((rv = iavf_vc_op_config_irq_map(vm, dev, im))) return rv;
    ...
    for (int i = 0; i < n_rx_vectors; i++) {
        ...
        else if (ap->vf_cap_flags & VIRTCHNL_VF_OFFLOAD_WB_ON_ITR && n_intr_mode_queues_per_vector[i] == 0)
...
static void avf_msix_n_handler(vlib_main_t *vm, vnet_dev_t *dev, u16 line)
{
    iavf_device_t   *ad   = vnet_dev_get_data(dev);
    ...
    vlib_node_set_interrupt_pending(vlib_get_main_by_index(line), port->intf.rx_node_index);
}

vnet_dev_rv_t iavf_port_init(vlib_main_t *vm, vnet_dev_port_t *port)
{
    vnet_dev_t   *dev = port->dev;
    iavf_port_t  *ap  = vnet_dev_get_port_data(port);
    vnet_dev_rv_t rv;
    ...
    foreach_vnet_dev_port_rx_queue (q, port)
        if (q->interrupt_mode) u64_bit_set(&ap->intr_mode_per_rxq_bitmap, q->queue_id, 1);
    ...
    if ((rv = iavf_port_init_rss(vm, port))) return rv;
    ...
    if (port->dev->poll_stats) iavf_port_add_counters(vm, port);

    return VNET_DEV_OK;
}

static vnet_dev_rv_t iavf_enable_disable_queues(vlib_main_t *vm, vnet_dev_port_t *port, int enable)
{
    iavf_port_t *ap = vnet_dev_get_port_data(port);
    ...
    foreach_vnet_dev_port_rx_queue (q, port)
        if ((enable && q->enabled) || (!enable && q->started)) qs.rx_queues |= 1ULL << q->queue_id;

    foreach_vnet_dev_port_tx_queue (q, port)
        if ((enable && q->enabled) || (!enable && q->started)) qs.tx_queues |= 1ULL << q->queue_id;

    return enable ? iavf_vc_op_enable_queues(vm, port->dev, &qs) : iavf_vc_op_disable_queues(vm, port->dev, &qs);
}

vnet_dev_rv_t iavf_port_start(vlib_main_t *vm, vnet_dev_port_t *port)
{
    vnet_dev_rv_t rv;
    ...
    foreach_vnet_dev_port_rx_queue (q, port)
        if (q->enabled)
            if ((rv = iavf_rx_queue_start(vm, q))) goto done;

    foreach_vnet_dev_port_tx_queue (q, port)
        if ((rv = iavf_tx_queue_start(vm, q))) goto done;

    if ((rv = iavf_port_update_rss_lut(vm, port))) goto done;

    /* configure qpairs */
    if ((rv = iavf_port_init_vsi_queues(vm, port))) goto done;

    if ((rv = iavf_port_rx_irq_config(vm, port, /* enable */ 1))) goto done;

    if ((rv = iavf_enable_disable_queues(vm, port, 1))) goto done;

    if (port->dev->poll_stats) vnet_dev_poll_port_add(vm, port, 1, iavf_port_poll_stats);
...
void iavf_port_stop(vlib_main_t *vm, vnet_dev_port_t *port)
{
    log_debug(port->dev, "port %u", port->port_id);
    ...
    if (port->dev->poll_stats) vnet_dev_poll_port_remove(vm, port, iavf_port_poll_stats);
...
vnet_dev_rv_t iavf_port_cfg_change_validate(vlib_main_t *vm, vnet_dev_port_t *port, vnet_dev_port_cfg_change_req_t *req)
{
...
static vnet_dev_rv_t iavf_port_add_del_eth_addr(vlib_main_t *vm, vnet_dev_port_t *port, vnet_dev_hw_addr_t *addr, int is_add, int is_primary)
{
    iavf_port_t                *ap = vnet_dev_get_port_data(port);
    ...
    *al = (virtchnl_ether_addr_list_t){
    ...
    return is_add ? iavf_vc_op_add_eth_addr(vm, port->dev, al) : iavf_vc_op_del_eth_addr(vm, port->dev, al);
}

static vnet_dev_rv_t iavf_port_cfg_rxq_int_mode_change(vlib_main_t *vm, vnet_dev_port_t *port, u16 qid, u8 state, u8 all)
{
    vnet_dev_rv_t rv  = VNET_DEV_OK;
    iavf_port_t  *ap  = vnet_dev_get_port_data(port);
    vnet_dev_t   *dev = port->dev;
...
vnet_dev_rv_t iavf_port_cfg_change(vlib_main_t *vm, vnet_dev_port_t *port, vnet_dev_port_cfg_change_req_t *req)
{
    vnet_dev_t   *dev = port->dev;
    iavf_port_t  *ap  = vnet_dev_get_port_data(port);
    vnet_dev_rv_t rv  = VNET_DEV_OK;
    ...
    switch (req->type) {
        ...
        case VNET_DEV_PORT_CFG_CHANGE_PRIMARY_HW_ADDR:
            rv = iavf_port_add_del_eth_addr(vm,
                                            port,
