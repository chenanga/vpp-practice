...
vnet_dev_counter_t iavf_port_counters[] = {
    ...
    VNET_DEV_CTR_VENDOR(IIAVF_PORT_CTR_RX_UNKNOWN_PROTOCOL, RX, PACKETS, "unknown protocol"),
...
void iavf_port_add_counters(vlib_main_t *vm, vnet_dev_port_t *port)
{
    vnet_dev_port_add_counters(vm, port, iavf_port_counters, ARRAY_LEN(iavf_port_counters));
}

void iavf_port_poll_stats(vlib_main_t *vm, vnet_dev_port_t *port)
{
    vnet_dev_rv_t           rv;
    ...
    foreach_vnet_dev_counter (c, port->counter_main) {
        switch (c->user_data) {
            ...
            case IIAVF_PORT_CTR_RX_PACKETS:
                vnet_dev_counter_value_update(vm, c, stats.rx_unicast + stats.rx_broadcast + stats.rx_multicast);
                break;
            case IIAVF_PORT_CTR_TX_PACKETS:
                vnet_dev_counter_value_update(vm, c, stats.tx_unicast + stats.tx_broadcast + stats.tx_multicast);
                break;
