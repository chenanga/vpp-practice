...
static u8 *iavf_probe(vlib_main_t *vm, vnet_dev_bus_index_t bus_index, void *dev_info)
{
    vnet_dev_bus_pci_device_info_t *di = dev_info;

    if (di->vendor_id != 0x8086) return 0;

    FOREACH_ARRAY_ELT (dt, iavf_dev_types) {
        if (dt->device_id == di->device_id) return format(0, "%s", dt->desc);
    }

    return 0;
}

static vnet_dev_rv_t iavf_reset(vlib_main_t *vm, vnet_dev_t *dev)
{
    ...
    do {
        if (n_tries-- == 0) return VNET_DEV_ERR_TIMEOUT;
...
static vnet_dev_rv_t iavf_alloc(vlib_main_t *vm, vnet_dev_t *dev)
{
    log_debug(dev, "alloc");
    return iavf_aq_alloc(vm, dev);
}

static vnet_dev_rv_t iavf_init(vlib_main_t *vm, vnet_dev_t *dev)
{
    iavf_device_t          *ad = vnet_dev_get_data(dev);
    ...
    if ((rv = vnet_dev_pci_map_region(vm, dev, 0, &ad->bar0))) return rv;

    if ((rv = vnet_dev_pci_bus_master_enable(vm, dev))) return rv;

    if ((rv = iavf_reset(vm, dev))) return rv;

    if ((rv = iavf_vc_op_version(vm, dev, &driver_virtchnl_version, &ver))) return rv;

    if (ver.major != driver_virtchnl_version.major || ver.minor != driver_virtchnl_version.minor) return VNET_DEV_ERR_UNSUPPORTED_DEVICE_VER;

    if ((rv = iavf_vc_op_get_vf_resources(vm, dev, &driver_cap_flags, &res))) return rv;

    if (res.num_vsis != 1 || res.vsi_res[0].vsi_type != VIRTCHNL_VSI_SRIOV) return VNET_DEV_ERR_UNSUPPORTED_DEVICE;

    if (res.max_mtu == 0) {
        log_warn(dev,
                 "PF driver is reporting invalid value of 0 for max_mtu, "
                 "consider upgrade");
    ...
    if (res.vf_cap_flags & VIRTCHNL_VF_OFFLOAD_RSS_PF) {
        if (res.rss_key_size < IAVF_MAX_RSS_KEY_SIZE) {
            ...
            if (res.rss_lut_size > IAVF_MAX_RSS_LUT_SIZE) log_notice(dev, "device supports bigger RSS LUT than driver");
        }
    }
...
static void iavf_deinit(vlib_main_t *vm, vnet_dev_t *dev)
{
...
static void iavf_free(vlib_main_t *vm, vnet_dev_t *dev)
{
    log_debug(dev, "free");
    iavf_aq_free(vm, dev);
}
