...
static_always_inline int iavf_rxd_is_not_eop(iavf_rx_desc_t *d)
{
    return (d->qw1.as_u64 & mask_eop.as_u64) == 0;
}

static_always_inline int iavf_rxd_is_not_dd(iavf_rx_desc_t *d)
{
    return (d->qw1.as_u64 & mask_dd.as_u64) == 0;
}

static_always_inline void iavf_rx_desc_write(iavf_rx_desc_t *d, u64 addr)
{
...
static_always_inline void iavf_rxq_refill(vlib_main_t *vm, vlib_node_runtime_t *node, vnet_dev_rx_queue_t *rxq, int use_va_dma)
{
    ...
    if (PREDICT_TRUE(n_refill <= IAVF_RX_REFILL_TRESHOLD)) return;

    slot = (arq->next - n_refill - 1) & mask;

    n_refill &= ~7; /* round to 8 */
    n_alloc = vlib_buffer_alloc_to_ring_from_pool(vm, arq->buffer_indices, slot, size, n_refill, vnet_dev_get_rx_queue_buffer_pool_index(rxq));

    if (PREDICT_FALSE(n_alloc != n_refill)) {
        vlib_error_count(vm, node->node_index, IAVF_RX_NODE_CTR_BUFFER_ALLOC, 1);
        if (n_alloc) vlib_buffer_free_from_ring(vm, arq->buffer_indices, slot, size, n_alloc);
        return;
    }
    ...
    while (n_alloc >= 8) {
        ...
        if (use_va_dma) {
            vlib_get_buffers_with_offset(vm, arq->buffer_indices + slot, p, 8, sizeof(vlib_buffer_t));
...
static_always_inline uword iavf_rx_attach_tail(vlib_main_t *vm, vlib_buffer_template_t *bt, vlib_buffer_t *b, u64 qw1, iavf_rx_tail_t *t)
{
    vlib_buffer_t *hb     = b;
    u32            tlnifb = 0, i = 0;

    if (qw1 & mask_eop.as_u64) return 0;
...
static_always_inline void iavf_process_flow_offload(vnet_dev_port_t *port, iavf_rt_data_t *rtd, uword n_rx_packets)
{
    uword                    n;
    ...
    for (n = 0; n < n_rx_packets; n++) {
        if ((rtd->qw1s[n] & mask_flm.as_u64) == 0) continue;

        fle = *pool_elt_at_index(ap->flow_lookup_entries, rtd->flow_ids[n]);

        if (fle.next_index != (u16) ~0) rtd->next[n] = fle.next_index;

        if (fle.flow_id != ~0) rtd->bufs[n]->flow_id = fle.flow_id;

        if (fle.buffer_advance != ~0) vlib_buffer_advance(rtd->bufs[n], fle.buffer_advance);
    }
}

static_always_inline uword iavf_process_rx_burst(vlib_main_t            *vm,
                                                 ...
                                                 int                     maybe_multiseg)
{
    ...
    while (n_left >= 4) {
        ...
        n_rx_bytes += b[0]->current_length = ((iavf_rx_desc_qw1_t) qw1[0]).length;
        n_rx_bytes += b[1]->current_length = ((iavf_rx_desc_qw1_t) qw1[1]).length;
        n_rx_bytes += b[2]->current_length = ((iavf_rx_desc_qw1_t) qw1[2]).length;
        n_rx_bytes += b[3]->current_length = ((iavf_rx_desc_qw1_t) qw1[3]).length;
    ...
    while (n_left) {
        ...
        if (maybe_multiseg) n_rx_bytes += iavf_rx_attach_tail(vm, bt, b[0], qw1[0], tail + 0);
...
static_always_inline uword iavf_device_input_inline(vlib_main_t         *vm,
                                                    vlib_node_runtime_t *node,
                                                    vlib_frame_t        *frame,
                                                    ...
                                                    int                  with_flows)
{
    ...
    d = descs + next;
    if ((d->qword[1] & mask_dd.as_u64) == 0) goto done;
    ...
    while (n_rx_packets < IAVF_RX_VECTOR_SZ) {
...
#ifdef CLIB_HAVE_VEC256
        if (n_rx_packets >= IAVF_RX_VECTOR_SZ - 4 || next >= size - 4) goto one_by_one;

        q1x4 = u64x4_gather((void *) &d[0].qword[1], (void *) &d[1].qword[1], (void *) &d[2].qword[1], (void *) &d[3].qword[1]);

        /* not all packets are ready or at least one of them is chained */
        if (!u64x4_is_equal(q1x4 & dd_eop_mask4, dd_eop_mask4)) goto one_by_one;
        ...
        q1x4_lo = u32x4_gather((void *) &d[0].qword[1], (void *) &d[1].qword[1], (void *) &d[2].qword[1], (void *) &d[3].qword[1]);

        /* not all packets are ready or at least one of them is chained */
        if (!u32x4_is_equal(q1x4_lo & dd_eop_mask4, dd_eop_mask4)) goto one_by_one;

        q1x4_hi = u32x4_gather((void *) &d[0].qword[1] + 4, (void *) &d[1].qword[1] + 4, (void *) &d[2].qword[1] + 4, (void *) &d[3].qword[1] + 4);
        ...
        if (with_flows) {
            fdidx4 = u32x4_gather((void *) &d[0].fdid_flex_hi, (void *) &d[1].fdid_flex_hi, (void *) &d[2].fdid_flex_hi, (void *) &d[3].fdid_flex_hi);
    ...
    one_by_one:
        ...
        if (PREDICT_FALSE(iavf_rxd_is_not_eop(d))) {
            ...
            do {
                ...
                if (iavf_rxd_is_not_dd(td)) goto no_more_desc;
...
no_more_desc:
    ...
    if (PREDICT_FALSE((n_trace = vlib_get_trace_count(vm, node)))) {
        ...
        while (n_trace && n_left) {
            ...
            if (PREDICT_FALSE(single_next == 0)) next_index = next_indices[0];
            ...
            if (PREDICT_TRUE(vlib_trace_buffer(vm,
                ...
                tr->flow_id         = (tr->qw1s[0] & mask_flm.as_u64) ? rtd->flow_ids[i] : 0;
    ...
    if (PREDICT_FALSE(with_flows && (or_qw1 & mask_flm.as_u64))) {
        ...
        if (PREDICT_TRUE(next_index == VNET_DEV_ETH_RX_PORT_NEXT_ETH_INPUT)) {
            ...
            if ((or_qw1 & mask_ipe.as_u64) == 0) f->flags |= ETH_INPUT_FRAME_F_IP4_CKSUM_OK;
