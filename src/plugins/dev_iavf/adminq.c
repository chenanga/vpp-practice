...
static inline void iavf_irq_0_disable(iavf_device_t *ad)
{
...
static inline void iavf_irq_0_enable(iavf_device_t *ad)
{
...
static_always_inline int iavf_aq_desc_is_done(iavf_aq_desc_t *d)
{
...
static u8 *format_iavf_aq_desc_flags(u8 *s, va_list *args)
{
...
static u8 *format_iavf_aq_desc_retval(u8 *s, va_list *args)
{
...
#undef _
    ...
    if (rv >= ARRAY_LEN(retvals) || retvals[rv] == 0) return format(s, "UNKNOWN(%d)", rv);

    return format(s, "%s", retvals[rv]);
}

static u8 *format_iavf_aq_desc(u8 *s, va_list *args)
{
    iavf_aq_desc_t *d      = va_arg(*args, iavf_aq_desc_t *);
    u32             indent = format_get_indent(s);

    s = format(s,
               "opcode 0x%04x datalen %u retval %U (%u) flags %U",
               d->opcode,
               d->datalen,
               format_iavf_aq_desc_retval,
               d->retval,
               d->retval,
               format_iavf_aq_desc_flags,
               d->flags);
    ...
    if (d->opcode == IIAVF_AQ_DESC_OP_SEND_TO_PF || d->opcode == IIAVF_AQ_DESC_OP_MESSAGE_FROM_PF) {
        s = format(s,
                   ...
                   format_white_space,
                   indent,
                   format_virtchnl_op_name,
                   d->v_opcode,
                   d->v_opcode,
                   format_virtchnl_status,
                   d->v_retval,
                   d->v_retval,
                   (uword) d->param2 << 32 | d->param3);
    }
    else {
        s = format(s,
...
vnet_dev_rv_t iavf_aq_alloc(vlib_main_t *vm, vnet_dev_t *dev)
{
    iavf_device_t *ad = vnet_dev_get_data(dev);
    return vnet_dev_dma_mem_alloc(vm, dev, sizeof(iavf_adminq_dma_mem_t), 0, (void **) &ad->aq_mem);
}

void iavf_aq_free(vlib_main_t *vm, vnet_dev_t *dev)
{
    iavf_device_t *ad = vnet_dev_get_data(dev);
    vnet_dev_dma_mem_free(vm, dev, ad->aq_mem);
}

static void iavf_aq_arq_slot_init(vlib_main_t *vm, vnet_dev_t *dev, u16 slot)
{
    iavf_device_t *ad     = vnet_dev_get_data(dev);
...
static void iavf_aq_poll(vlib_main_t *vm, vnet_dev_t *dev)
{
    iavf_device_t  *ad = vnet_dev_get_data(dev);
    iavf_aq_desc_t *d;
    u8             *b;
...
#undef _
        ...
        vec_foreach (e, ad->events) {
            log_debug(dev, "event %s (%u) sev %d", virtchnl_event_names[e->event], e->event, e->severity);
            ...
            if (e->event == VIRTCHNL_EVENT_LINK_CHANGE) {
                ...
                if (port) {
                    ...
                    if (ap->vf_cap_flags & VIRTCHNL_VF_CAP_ADV_LINK_SPEED) {
                        ...
                        const u32 speed_table[8] = {100, 1000, 10000, 40000, 20000, 25000, 2500, 5000};
                    ...
                    if (changes.change.any) vnet_dev_port_state_change(vm, port, changes);
                }
            }
        }
...
static void iavf_adminq_msix_handler(vlib_main_t *vm, vnet_dev_t *dev, u16 line)
{
    iavf_device_t *ad = vnet_dev_get_data(dev);
    iavf_reg_write(ad, IAVF_VFINT_DYN_CTL0, dyn_ctl0_enable.as_u32);
...
static void iavf_adminq_intx_handler(vlib_main_t *vm, vnet_dev_t *dev)
{
    iavf_adminq_msix_handler(vm, dev, 0);
}

void iavf_aq_init(vlib_main_t *vm, vnet_dev_t *dev)
{
    iavf_device_t *ad = vnet_dev_get_data(dev);
    uword          pa;
    u32            len;
...
void iavf_aq_poll_on(vlib_main_t *vm, vnet_dev_t *dev)
{
    iavf_device_t *ad = vnet_dev_get_data(dev);
...
void iavf_aq_poll_off(vlib_main_t *vm, vnet_dev_t *dev)
{
    iavf_device_t *ad = vnet_dev_get_data(dev);
...
vnet_dev_rv_t iavf_aq_atq_enq(vlib_main_t *vm, vnet_dev_t *dev, iavf_aq_desc_t *desc, const u8 *data, u16 len, f64 timeout)
{
    iavf_device_t  *ad  = vnet_dev_get_data(dev);
    ...
    *d = *desc;
    ...
    if (timeout > 0) {
        ...
        while (1) {
            ...
            if (flags.dd && flags.cmp) return VNET_DEV_OK;
...
void iavf_aq_deinit(vlib_main_t *vm, vnet_dev_t *dev)
{
    iavf_device_t *ad = vnet_dev_get_data(dev);
    if (ad->adminq_active) {
        iavf_aq_desc_t d = {
...
int iavf_aq_arq_next_acq(vlib_main_t *vm, vnet_dev_t *dev, iavf_aq_desc_t **dp, u8 **bp, f64 timeout)
{
    iavf_device_t  *ad = vnet_dev_get_data(dev);
    ...
    if (timeout) {
        ...
        while (!iavf_aq_desc_is_done(d)) {
            if (vlib_time_now(vm) - t0 > timeout) return 0;
...
void iavf_aq_arq_next_rel(vlib_main_t *vm, vnet_dev_t *dev)
{
    iavf_device_t *ad = vnet_dev_get_data(dev);
