...
#define foreach_ipsecmb_gcm_cipher_op                          \
  ...
  _(AES_256_GCM, 256)
/* clang-format on */
static_always_inline vnet_crypto_op_status_t ipsecmb_status_job(IMB_STATUS status)
{
    switch (status) {
...
always_inline void ipsecmb_retire_hmac_job(IMB_JOB *job, u32 *n_fail, u32 digest_size)
{
...
ipsecmb_ops_hmac_inline(vlib_main_t *vm, vnet_crypto_op_t *ops[], u32 n_ops, u32 block_size, u32 hash_size, u32 digest_size, IMB_HASH_ALG alg)
{
    ipsecmb_main_t            *imbm = &ipsecmb_main;
    ipsecmb_per_thread_data_t *ptd  = vec_elt_at_index(imbm->per_thread_data, vm->thread_index);
    IMB_JOB                   *job;
    u32                        i, n_fail = 0, ops_index = 0;
    u8                         scratch[n_ops][digest_size];
    const u32                  burst_sz = (n_ops > IMB_MAX_BURST_SIZE) ? IMB_MAX_BURST_SIZE : n_ops;
...
#else
static_always_inline u32
ipsecmb_ops_hmac_inline(vlib_main_t *vm, vnet_crypto_op_t *ops[], u32 n_ops, u32 block_size, u32 hash_size, u32 digest_size, JOB_HASH_ALG alg)
{
    ipsecmb_main_t            *imbm = &ipsecmb_main;
    ipsecmb_per_thread_data_t *ptd  = vec_elt_at_index(imbm->per_thread_data, vm->thread_index);
    IMB_JOB                   *job;
    ...
    for (i = 0; i < n_ops; i++) {
        ...
        if (job) ipsecmb_retire_hmac_job(job, &n_fail, digest_size);
    }
...
always_inline void ipsecmb_retire_cipher_job(IMB_JOB *job, u32 *n_fail)
{
    vnet_crypto_op_t *op = job->user_data;
                                                       ...
                                                       IMB_CIPHER_MODE      cipher_mode)
{
    ...
    while (n_ops) {
        ...
        IMB_SUBMIT_CIPHER_BURST_NOCHECK(ptd->mgr, ptd->burst_jobs, n, cipher_mode, direction, key_len / 8);
        for (i = 0; i < n; i++) {
            job = &ptd->burst_jobs[i];
                                                       ...
                                                       JOB_CIPHER_MODE      cipher_mode)
{
    ...
    for (i = 0; i < n_ops; i++) {
        ...
        job->chain_order      = (direction == IMB_DIR_ENCRYPT ? IMB_ORDER_CIPHER_HASH : IMB_ORDER_HASH_CIPHER);
        ...
        if (job) ipsecmb_retire_cipher_job(job, &n_fail);
    }
...
always_inline void ipsecmb_retire_aead_job(IMB_JOB *job, u32 *n_fail)
{
    vnet_crypto_op_t *op  = job->user_data;
    u32               len = op->tag_len;
...
static_always_inline u32 ipsecmb_ops_chacha_poly(vlib_main_t *vm, vnet_crypto_op_t *ops[], u32 n_ops, IMB_CIPHER_DIRECTION dir)
{
    ...
    for (i = 0; i < n_ops; i++) {
        ...
        if (job) ipsecmb_retire_aead_job(job, &n_fail);
...
static_always_inline u32
ipsecmb_ops_chacha_poly_chained(vlib_main_t *vm, vnet_crypto_op_t *ops[], vnet_crypto_op_chunk_t *chunks, u32 n_ops, IMB_CIPHER_DIRECTION dir)
{
    ipsecmb_main_t            *imbm = &ipsecmb_main;
    ipsecmb_per_thread_data_t *ptd  = vec_elt_at_index(imbm->per_thread_data, vm->thread_index);
    IMB_MGR                   *m    = ptd->mgr;
    ...
    if (dir == IMB_DIR_ENCRYPT) {
        for (i = 0; i < n_ops; i++) {
            ...
            IMB_CHACHA20_POLY1305_INIT(m, key, &ctx, op->iv, op->aad, op->aad_len);

            chp = chunks + op->chunk_index;
            for (j = 0; j < op->n_chunks; j++) {
                IMB_CHACHA20_POLY1305_ENC_UPDATE(m, key, &ctx, chp->dst, chp->src, chp->len);
                chp += 1;
            }
    ...
    else /* dir == IMB_DIR_DECRYPT */
    {
        for (i = 0; i < n_ops; i++) {
            ...
            chp = chunks + op->chunk_index;
            for (j = 0; j < op->n_chunks; j++) {
                IMB_CHACHA20_POLY1305_DEC_UPDATE(m, key, &ctx, chp->dst, chp->src, chp->len);
                chp += 1;
            }
...
{
    return ipsecmb_ops_chacha_poly_chained(vm, ops, chunks, n_ops, IMB_DIR_ENCRYPT);
}
...
{
    return ipsecmb_ops_chacha_poly_chained(vm, ops, chunks, n_ops, IMB_DIR_DECRYPT);
}
#endif
...
static void crypto_ipsecmb_key_handler(vlib_main_t *vm, vnet_crypto_key_op_t kop, vnet_crypto_key_index_t idx)
{
    ...
    if (kop == VNET_CRYPTO_KEY_OP_DEL) {
        if (idx >= vec_len(imbm->key_data)) return;

        if (imbm->key_data[idx] == 0) return;
    ...
    kd = imbm->key_data[idx] = clib_mem_alloc_aligned(ad->data_size, CLIB_CACHE_LINE_BYTES);

    /* AES CBC key expansion */
    if (ad->keyexp) {
        ad->keyexp(key->data, ((ipsecmb_aes_key_data_t *) kd)->enc_key_exp, ((ipsecmb_aes_key_data_t *) kd)->dec_key_exp);
        return;
    }
...
static clib_error_t *crypto_ipsecmb_init(vlib_main_t *vm)
{
    ...
    if (!clib_cpu_supports_aes()) return 0;

    /*
     * A priority that is better than OpenSSL but worse than VPP natvie
     */
    name = format(0, "Intel(R) Multi-Buffer Crypto for IPsec Library %s%c", IMB_VERSION_STR, 0);
    eidx = vnet_crypto_register_engine(vm, "ipsecmb", 80, (char *) name);

    vec_validate_aligned(imbm->per_thread_data, tm->n_vlib_mains - 1, CLIB_CACHE_LINE_BYTES);
    ...
    vec_foreach (ptd, imbm->per_thread_data) {
        ...
        if (ptd == imbm->per_thread_data) m = ptd->mgr;
    }

#define _(a, b, c, d, e, f)                                                                      \
    vnet_crypto_register_ops_handler(vm, eidx, VNET_CRYPTO_OP_##a##_HMAC, ipsecmb_ops_hmac_##a); \
...
#undef _
#define _(a, b, c)                                                                                    \
    vnet_crypto_register_ops_handler(vm, eidx, VNET_CRYPTO_OP_##a##_ENC, ipsecmb_ops_cipher_enc_##a); \
    vnet_crypto_register_ops_handler(vm, eidx, VNET_CRYPTO_OP_##a##_DEC, ipsecmb_ops_cipher_dec_##a); \
...
#undef _
#define _(a, b)                                                                                                             \
    vnet_crypto_register_ops_handler(vm, eidx, VNET_CRYPTO_OP_##a##_ENC, ipsecmb_ops_gcm_cipher_enc_##a);                   \
    vnet_crypto_register_ops_handler(vm, eidx, VNET_CRYPTO_OP_##a##_DEC, ipsecmb_ops_gcm_cipher_dec_##a);                   \
    vnet_crypto_register_chained_ops_handler(vm, eidx, VNET_CRYPTO_OP_##a##_ENC, ipsecmb_ops_gcm_cipher_enc_##a##_chained); \
    vnet_crypto_register_chained_ops_handler(vm, eidx, VNET_CRYPTO_OP_##a##_DEC, ipsecmb_ops_gcm_cipher_dec_##a##_chained); \
    ad              = imbm->alg_data + VNET_CRYPTO_ALG_##a;                                                                 \
...
#ifdef HAVE_IPSECMB_CHACHA_POLY
    vnet_crypto_register_ops_handler(vm, eidx, VNET_CRYPTO_OP_CHACHA20_POLY1305_ENC, ipsecmb_ops_chacha_poly_enc);
    vnet_crypto_register_ops_handler(vm, eidx, VNET_CRYPTO_OP_CHACHA20_POLY1305_DEC, ipsecmb_ops_chacha_poly_dec);
    vnet_crypto_register_chained_ops_handler(vm, eidx, VNET_CRYPTO_OP_CHACHA20_POLY1305_ENC, ipsec_mb_ops_chacha_poly_enc_chained);
    vnet_crypto_register_chained_ops_handler(vm, eidx, VNET_CRYPTO_OP_CHACHA20_POLY1305_DEC, ipsec_mb_ops_chacha_poly_dec_chained);
