...
static uint32_t         noise_remote_handshake_index_get(vlib_main_t *vm, noise_remote_t *);
static void             noise_remote_handshake_index_drop(vlib_main_t *vm, noise_remote_t *);
...
void noise_remote_init(vlib_main_t *vm, noise_remote_t *r, uint32_t peer_pool_idx, const uint8_t public[NOISE_PUBLIC_KEY_LEN], u32 noise_local_idx)
{
...
void noise_remote_precompute(vlib_main_t *vm, noise_remote_t *r)
{
    noise_local_t *l = noise_local_get(r->r_local_idx);

    if (!curve25519_gen_shared(r->r_ss, l->l_private, r->r_public)) clib_memset(r->r_ss, 0, NOISE_PUBLIC_KEY_LEN);
                             ...
                             uint8_t         ets[NOISE_TIMESTAMP_LEN + NOISE_AUTHTAG_LEN])
{
    ...
    key_idx = vnet_crypto_key_add(vm, VNET_CRYPTO_ALG_CHACHA20_POLY1305, _key, NOISE_SYMMETRIC_KEY_LEN);
    ...
    if (!curve25519_gen_public(ue, hs->hs_e)) goto error;
    noise_msg_ephemeral(hs->hs_ck, hs->hs_hash, ue);

    /* es */
    if (!noise_mix_dh(hs->hs_ck, key, hs->hs_e, r->r_public)) goto error;
    vnet_crypto_key_update(vm, key_idx);

    /* s */
    noise_msg_encrypt(vm, es, l->l_public, NOISE_PUBLIC_KEY_LEN, key_idx, hs->hs_hash);

    /* ss */
    if (!noise_mix_ss(hs->hs_ck, key, r->r_ss)) goto error;
    vnet_crypto_key_update(vm, key_idx);
                              ...
                              uint8_t          ets[NOISE_TIMESTAMP_LEN + NOISE_AUTHTAG_LEN])
{
    ...
    /* es */
    if (!noise_mix_dh(hs.hs_ck, key, l->l_private, ue)) goto error;
    vnet_crypto_key_update(vm, key_idx);

    /* s */

    if (!noise_msg_decrypt(vm, r_public, es, NOISE_PUBLIC_KEY_LEN + NOISE_AUTHTAG_LEN, key_idx, hs.hs_hash)) goto error;

    /* Lookup the remote we received from */
    if ((r = l->l_upcall.u_remote_get(r_public)) == NULL) goto error;
    ...
    /* {t} */
    if (!noise_msg_decrypt(vm, timestamp, ets, NOISE_TIMESTAMP_LEN + NOISE_AUTHTAG_LEN, key_idx, hs.hs_hash)) goto error;
    ;
                           ...
                           uint8_t         en[0 + NOISE_AUTHTAG_LEN])
{
    ...
    if (hs->hs_state != CONSUMED_INITIATION) goto error;

    /* e */
    curve25519_gen_secret(e);
    if (!curve25519_gen_public(ue, e)) goto error;
    noise_msg_ephemeral(hs->hs_ck, hs->hs_hash, ue);

    /* ee */
    if (!noise_mix_dh(hs->hs_ck, NULL, e, hs->hs_e)) goto error;

    /* se */
    if (!noise_mix_dh(hs->hs_ck, NULL, e, r->r_public)) goto error;
                            ...
                            uint8_t         en[0 + NOISE_AUTHTAG_LEN])
{
    ...
    if (hs.hs_state != CREATED_INITIATION || hs.hs_local_index != r_idx) goto error;

    /* e */
    noise_msg_ephemeral(hs.hs_ck, hs.hs_hash, ue);

    /* ee */
    if (!noise_mix_dh(hs.hs_ck, NULL, hs.hs_e, ue)) goto error;

    /* se */
    if (!noise_mix_dh(hs.hs_ck, NULL, l->l_private, ue)) goto error;
    ...
    if (!noise_msg_decrypt(vm, NULL, en, 0 + NOISE_AUTHTAG_LEN, key_idx, hs.hs_hash)) goto error;

    hs.hs_remote_index = s_idx;

    if (r->r_handshake.hs_state == hs.hs_state && r->r_handshake.hs_local_index == hs.hs_local_index) {
        r->r_handshake          = hs;
...
bool noise_remote_begin_session(vlib_main_t *vm, noise_remote_t *r)
{
    ...
    if (hs->hs_state == CONSUMED_RESPONSE) {
        ...
        noise_kdf(key_send, key_recv, NULL, NULL, NOISE_SYMMETRIC_KEY_LEN, NOISE_SYMMETRIC_KEY_LEN, 0, 0, hs->hs_ck);
    }
    else if (hs->hs_state == CREATED_RESPONSE) {
        kp.kp_is_initiator = 0;
        noise_kdf(key_recv, key_send, NULL, NULL, NOISE_SYMMETRIC_KEY_LEN, NOISE_SYMMETRIC_KEY_LEN, 0, 0, hs->hs_ck);
    }
    else {
        return false;
    }
    ...
    kp.kp_valid        = 1;
    kp.kp_send_index   = vnet_crypto_key_add(vm, VNET_CRYPTO_ALG_CHACHA20_POLY1305, key_send, NOISE_SYMMETRIC_KEY_LEN);
    kp.kp_recv_index   = vnet_crypto_key_add(vm, VNET_CRYPTO_ALG_CHACHA20_POLY1305, key_recv, NOISE_SYMMETRIC_KEY_LEN);
...
{
    clib_rwlock_writer_lock(&r->r_keypair_lock);
    if (r->r_next != NULL) r->r_next->kp_valid = 0;
    if (r->r_current != NULL) r->r_current->kp_valid = 0;
    clib_rwlock_writer_unlock(&r->r_keypair_lock);
}
...
bool noise_remote_ready(noise_remote_t *r)
{
    ...
    if ((kp = r->r_current) == NULL || !kp->kp_valid || wg_birthdate_has_expired(kp->kp_birthdate, REJECT_AFTER_TIME)
        || kp->kp_ctr.c_recv >= REJECT_AFTER_MESSAGES || kp->kp_ctr.c_send >= REJECT_AFTER_MESSAGES)
        ret = false;
    else
        ret = true;
...
noise_remote_encrypt(vlib_main_t *vm, noise_remote_t *r, uint32_t *r_idx, uint64_t *nonce, uint8_t *src, size_t srclen, uint8_t *dst)
{
    noise_keypair_t       *kp;
    enum noise_state_crypt ret = SC_FAILED;

    if ((kp = r->r_current) == NULL) goto error;
     ...
     */
    if (!kp->kp_valid || wg_birthdate_has_expired(kp->kp_birthdate, REJECT_AFTER_TIME) || kp->kp_ctr.c_recv >= REJECT_AFTER_MESSAGES
    ...
    *r_idx = kp->kp_remote_index;
    ...
    wg_chacha20poly1305_calc(vm, src, srclen, dst, NULL, 0, *nonce, VNET_CRYPTO_OP_CHACHA20_POLY1305_ENC, kp->kp_send_index);
    ...
    if ((kp->kp_valid && *nonce >= REKEY_AFTER_MESSAGES) || (kp->kp_is_initiator && wg_birthdate_has_expired(kp->kp_birthdate, REKEY_AFTER_TIME)))
        goto error;
...
static uint32_t noise_remote_handshake_index_get(vlib_main_t *vm, noise_remote_t *r)
{
...
static void noise_remote_handshake_index_drop(vlib_main_t *vm, noise_remote_t *r)
{
    noise_handshake_t   *hs    = &r->r_handshake;
    noise_local_t       *local = noise_local_get(r->r_local_idx);
    struct noise_upcall *u     = &local->l_upcall;
    if (hs->hs_state != HS_ZEROED) u->u_index_drop(vm, hs->hs_local_index);
}
                      ...
                      const uint8_t  ck[NOISE_HASH_LEN])
{
    ...
    if (b == NULL || b_len == 0) goto out;

    /* Expand second key: key = sec, data = "a" || 0x2 */
    out[BLAKE2S_HASH_SIZE] = 2;
    HMAC(EVP_blake2s256(), sec, BLAKE2S_HASH_SIZE, out, BLAKE2S_HASH_SIZE + 1, out, &l);
    ASSERT(l == BLAKE2S_HASH_SIZE);
    clib_memcpy(b, out, b_len);

    if (c == NULL || c_len == 0) goto out;
                         ...
                         const uint8_t public[NOISE_PUBLIC_KEY_LEN])
{
    ...
    if (!curve25519_gen_shared(dh, private, public)) return false;
    noise_kdf(ck, key, NULL, dh, NOISE_HASH_LEN, NOISE_SYMMETRIC_KEY_LEN, 0, NOISE_PUBLIC_KEY_LEN, ck);
...
static bool noise_mix_ss(uint8_t ck[NOISE_HASH_LEN], uint8_t key[NOISE_SYMMETRIC_KEY_LEN], const uint8_t ss[NOISE_PUBLIC_KEY_LEN])
{
    ...
    if (clib_memcmp(ss, null_point, NOISE_PUBLIC_KEY_LEN) == 0) return false;
    noise_kdf(ck, key, NULL, ss, NOISE_HASH_LEN, NOISE_SYMMETRIC_KEY_LEN, 0, NOISE_PUBLIC_KEY_LEN, ck);
    return true;
}

static void noise_mix_hash(uint8_t hash[NOISE_HASH_LEN], const uint8_t *src, size_t src_len)
{
                          ...
                          const uint8_t psk[NOISE_SYMMETRIC_KEY_LEN])
{
    ...
    noise_kdf(ck, tmp, key, psk, NOISE_HASH_LEN, NOISE_HASH_LEN, NOISE_SYMMETRIC_KEY_LEN, NOISE_SYMMETRIC_KEY_LEN, ck);
    noise_mix_hash(hash, tmp, NOISE_HASH_LEN);
    wg_secure_zero_memory(tmp, NOISE_HASH_LEN);
}

static void noise_param_init(uint8_t ck[NOISE_HASH_LEN], uint8_t hash[NOISE_HASH_LEN], const uint8_t s[NOISE_PUBLIC_KEY_LEN])
{
    blake2s_state_t blake;

    blake2s(ck, NOISE_HASH_LEN, (uint8_t *) NOISE_HANDSHAKE_NAME, strlen(NOISE_HANDSHAKE_NAME), NULL, 0);

    blake2s_init(&blake, NOISE_HASH_LEN);
    blake2s_update(&blake, ck, NOISE_HASH_LEN);
    blake2s_update(&blake, (uint8_t *) NOISE_IDENTIFIER_NAME, strlen(NOISE_IDENTIFIER_NAME));
    blake2s_final(&blake, hash, NOISE_HASH_LEN);

    noise_mix_hash(hash, s, NOISE_PUBLIC_KEY_LEN);
}

static void noise_msg_encrypt(vlib_main_t *vm, uint8_t *dst, uint8_t *src, size_t src_len, uint32_t key_idx, uint8_t hash[NOISE_HASH_LEN])
{
    /* Nonce always zero for Noise_IK */
    wg_chacha20poly1305_calc(vm, src, src_len, dst, hash, NOISE_HASH_LEN, 0, VNET_CRYPTO_OP_CHACHA20_POLY1305_ENC, key_idx);
    noise_mix_hash(hash, dst, src_len + NOISE_AUTHTAG_LEN);
}

static bool noise_msg_decrypt(vlib_main_t *vm, uint8_t *dst, uint8_t *src, size_t src_len, uint32_t key_idx, uint8_t hash[NOISE_HASH_LEN])
{
    /* Nonce always zero for Noise_IK */
    if (!wg_chacha20poly1305_calc(vm, src, src_len, dst, hash, NOISE_HASH_LEN, 0, VNET_CRYPTO_OP_CHACHA20_POLY1305_DEC, key_idx)) return false;
    noise_mix_hash(hash, src, src_len);
    return true;
}

static void noise_msg_ephemeral(uint8_t ck[NOISE_HASH_LEN], uint8_t hash[NOISE_HASH_LEN], const uint8_t src[NOISE_PUBLIC_KEY_LEN])
{
    noise_mix_hash(hash, src, NOISE_PUBLIC_KEY_LEN);
    noise_kdf(ck, NULL, NULL, src, NOISE_HASH_LEN, 0, 0, NOISE_PUBLIC_KEY_LEN, ck);
}
