...
static clib_error_t *wg_if_create_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (unformat_user(input, unformat_line_input, line_input)) {
        while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
            if (unformat(line_input, "instance %d", &instance))
                ...
                generate_key = 1;
            else if (unformat(line_input, "src %U", unformat_ip_address, &src_ip))
                ;
            else {
    ...
    if (generate_key) curve25519_gen_secret(private_key);

    rv = wg_if_create(instance, private_key, port, &src_ip, &sw_if_index);

    if (rv) return clib_error_return(0, "wireguard interface create failed");
...
static clib_error_t *wg_if_delete_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (~0 != sw_if_index) {
        ...
        if (rv) return clib_error_return(0, "wireguard interface delete failed");
    }
    else
...
static clib_error_t *wg_peer_add_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "public-key %s", &public_key_64)) {
            ...
            ;
        else if (unformat(line_input, "persistent-keepalive %d", &persistent_keepalive))
            ;
        else if (unformat(line_input, "allowed-ip %U", unformat_ip_prefix, &pfx)) {
            ip_prefix_to_fib_prefix(&pfx, &allowed_ip);
            vec_add1(allowed_ips, allowed_ip);
        }
        else if (unformat(line_input, "%U", unformat_vnet_sw_interface, vnm, &tun_sw_if_index))
            ;
        else {
    ...
    rv = wg_peer_add(tun_sw_if_index, public_key, table_id, &ip_addr_46(&ip), allowed_ips, portDst, persistent_keepalive, &peer_index);
...
static clib_error_t *wg_set_async_mode_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    int              async_enable = 0;
...
static clib_error_t *wg_show_mode_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    vlib_cli_output(vm, "Wireguard mode");

#define _(v, f, s) vlib_cli_output(vm, "\t%s: %s", s, (wg_op_mode_is_set_##f() ? "enabled" : "disabled"));
