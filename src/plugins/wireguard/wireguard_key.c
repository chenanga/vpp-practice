...
bool curve25519_gen_shared(u8 shared_key[CURVE25519_KEY_SIZE], const u8 secret_key[CURVE25519_KEY_SIZE], const u8 basepoint[CURVE25519_KEY_SIZE])
{
    ...
    EVP_PKEY *pkey    = EVP_PKEY_new_raw_private_key(EVP_PKEY_X25519, NULL, secret_key, CURVE25519_KEY_SIZE);
    ...
    peerkey = EVP_PKEY_new_raw_public_key(EVP_PKEY_X25519, NULL, basepoint, CURVE25519_KEY_SIZE);
...
{
    if (!EVP_EncodeBlock(out, src, src_len)) return false;
    return true;
}

bool key_from_base64(const u8 *src, size_t src_len, u8 *out)
{
    if (EVP_DecodeBlock(out, src, src_len - 1) <= 0) return false;
    return true;
}
