...
bool wg_chacha20poly1305_calc(vlib_main_t            *vm,
                              u8                     *src,
                              u32                     src_len,
                              u8                     *dst,
                              u8                     *aad,
...
void wg_xchacha20poly1305_encrypt(vlib_main_t *vm,
                                  u8          *src,
                                  u32          src_len,
                                  u8          *dst,
                                  u8          *aad,
                                  ...
                                  u8           key[CHACHA20POLY1305_KEY_SIZE])
{
    ...
    key_idx = vnet_crypto_key_add(vm, VNET_CRYPTO_ALG_CHACHA20_POLY1305, (uint8_t *) derived_key, CHACHA20POLY1305_KEY_SIZE);

    wg_chacha20poly1305_calc(vm, src, src_len, dst, aad, aad_len, h_nonce, VNET_CRYPTO_OP_CHACHA20_POLY1305_ENC, key_idx);

    vnet_crypto_key_del(vm, key_idx);
    wg_secure_zero_memory(derived_key, CHACHA20POLY1305_KEY_SIZE);
}

bool wg_xchacha20poly1305_decrypt(vlib_main_t *vm,
                                  u8          *src,
                                  u32          src_len,
                                  u8          *dst,
                                  u8          *aad,
                                  ...
                                  u8           key[CHACHA20POLY1305_KEY_SIZE])
{
    ...
    ret = wg_chacha20poly1305_calc(vm, src, src_len, dst, aad, aad_len, h_nonce, VNET_CRYPTO_OP_CHACHA20_POLY1305_DEC, key_idx);
