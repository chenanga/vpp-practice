...
void wg_timers_any_authenticated_packet_received_opt(wg_peer_t *peer, f64 time);
...
static inline bool wg_birthdate_has_expired(f64 birthday_seconds, f64 expiration_seconds)
{
    if (birthday_seconds == 0.0) return true;
    f64 now_seconds = vlib_time_now(vlib_get_main());
    return (birthday_seconds + expiration_seconds) < now_seconds;
}

static_always_inline bool wg_birthdate_has_expired_opt(f64 birthday_seconds, f64 expiration_seconds, f64 time)
{
