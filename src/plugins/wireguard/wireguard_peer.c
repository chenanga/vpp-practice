...
static void wg_peer_clear(vlib_main_t *vm, wg_peer_t *peer)
{
    ...
    vec_foreach (adj_index, peer->adj_indices) {
        ...
        if (adj_is_valid(*adj_index)) adj_midchain_delegate_unstack(*adj_index);
    }
...
static void wg_peer_adj_stack(wg_peer_t *peer, adj_index_t ai)
{
    ip_adjacency_t *adj;
    u32             sw_if_index;
    wg_if_t        *wgi;
    ...
    if (!vnet_sw_interface_is_admin_up(vnet_get_main(), wgi->sw_if_index) || !wg_peer_can_send(peer)) {
...
static void wg_peer_adj_reset_stacking(adj_index_t ai)
{
    adj_midchain_delegate_remove(ai);
}

static void wg_peer_66_fixup(vlib_main_t *vm, const ip_adjacency_t *adj, vlib_buffer_t *b, const void *data)
{
    u8            iph_offset = 0;
    ...
    ip6_out->ip_version_traffic_class_and_flow_label = ip6_in->ip_version_traffic_class_and_flow_label;
}

static void wg_peer_46_fixup(vlib_main_t *vm, const ip_adjacency_t *adj, vlib_buffer_t *b, const void *data)
{
    u8            iph_offset = 0;
    ip6_header_t *ip6_out;
    ip4_header_t *ip4_in;
    ...
    ip6_out->ip_version_traffic_class_and_flow_label = clib_host_to_net_u32(vtcfl);
}

static adj_midchain_fixup_t wg_peer_get_fixup(wg_peer_t *peer, vnet_link_t lt)
{
    if (!ip46_address_is_ip4(&peer->dst.addr)) {
        if (lt == VNET_LINK_IP4) return (wg_peer_46_fixup);
        if (lt == VNET_LINK_IP6) return (wg_peer_66_fixup);
    }
    return (NULL);
}

static void wg_peer_disable(vlib_main_t *vm, wg_peer_t *peer)
{
...
static void wg_peer_enable(vlib_main_t *vm, wg_peer_t *peer)
{
    index_t  peeri = peer - wg_peer_pool;
    wg_if_t *wg_if;
...
walk_rc_t wg_peer_if_adj_change(index_t peeri, void *data)
{
    ...
    vec_foreach (allowed_ip, peer->allowed_ips) {
        if (fib_prefix_is_cover_addr_46(allowed_ip, &adj->sub_type.nbr.next_hop)) {
            ...
            adj_nbr_midchain_update_rewrite(*adj_index, fixup, NULL, ADJ_FLAG_MIDCHAIN_IP_STACK, vec_dup(peer->rewrite));
...
{
    return wg_peer_if_adj_change((*(index_t *) (data)), &ai) == WALK_CONTINUE ? ADJ_WALK_RC_CONTINUE : ADJ_WALK_RC_STOP;
}
                        ...
                        u32                   wg_sw_if_index)
{
    ...
    if (NULL == wgi) return (VNET_API_ERROR_INVALID_INTERFACE);

    ip_address_to_46(&wgi->src_ip, &peer->src.addr);
    peer->src.port = wgi->port;

    u8 is_ip4     = ip46_address_is_ip4(&peer->dst.addr);
    peer->rewrite = wg_build_rewrite(&peer->src.addr, peer->src.port, &peer->dst.addr, peer->dst.port, is_ip4);
...
void wg_peer_update_endpoint(index_t peeri, const ip46_address_t *addr, u16 port)
{
    wg_peer_t *peer = wg_peer_get(peeri);

    if (ip46_address_is_equal(&peer->dst.addr, addr) && peer->dst.port == port) return;
    ...
    vec_foreach (adj_index, peer->adj_indices) {
        if (adj_is_valid(*adj_index)) {
            adj_midchain_fixup_t fixup = wg_peer_get_fixup(peer, adj_get_link_type(*adj_index));
            adj_nbr_midchain_update_rewrite(*adj_index, fixup, NULL, ADJ_FLAG_MIDCHAIN_IP_STACK, vec_dup(peer->rewrite));
...
static void wg_peer_update_endpoint_thread_fn(wg_peer_upd_ep_args_t *args)
{
    wg_peer_update_endpoint(args->peeri, &args->addr, args->port);
}

void wg_peer_update_endpoint_from_mt(index_t peeri, const ip46_address_t *addr, u16 port)
{
    ...
    vlib_rpc_call_main_thread(wg_peer_update_endpoint_thread_fn, (u8 *) &args, sizeof(args));
}
                ...
                u32                  *peer_index)
{
    ...
    if (tun_sw_if_index == ~0) return (VNET_API_ERROR_INVALID_SW_IF_INDEX);

    wg_if = wg_if_get(wg_if_find_by_sw_if_index(tun_sw_if_index));
    if (!wg_if) return (VNET_API_ERROR_INVALID_SW_IF_INDEX);
    ...
    if (pool_elts(wg_peer_pool) > MAX_PEERS) return (VNET_API_ERROR_LIMIT_EXCEEDED);

    pool_get_zero(wg_peer_pool, peer);

    wg_peer_init(vm, peer);

    rv = wg_peer_fill(vm, peer, table_id, endpoint, (u16) port, persistent_keepalive, allowed_ips, tun_sw_if_index);
    ...
    noise_remote_init(vm, &peer->remote, peer - wg_peer_pool, public_key, wg_if->local_idx);
...
int wg_peer_remove(index_t peeri)
{
    ...
    if (pool_is_free_index(wg_peer_pool, peeri)) return VNET_API_ERROR_NO_SUCH_ENTRY;
...
index_t wg_peer_walk(wg_peer_walk_cb_t fn, void *data)
{
    ...
    pool_foreach_index (peeri, wg_peer_pool) {
        if (WALK_STOP == fn(peeri, data)) return peeri;
    }
...
static u8 *format_wg_peer_endpoint(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U:%d", format_ip46_address, &ep->addr, IP46_TYPE_ANY, ep->port);
