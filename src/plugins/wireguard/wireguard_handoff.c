...
static_always_inline uword wg_handoff(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u32 fq_index, wg_handoff_mode_t mode)
{
    ...
    while (n_left_from > 0) {
        ...
        if (PREDICT_FALSE(mode == WG_HANDOFF_HANDSHAKE)) {
            ...
            u32            *entry = wg_index_table_lookup(&wmp->index_table, data->receiver_index);
            peeri                 = *entry;
    ...
    if (n_enq < frame->n_vectors) vlib_node_increment_counter(vm, node->node_index, WG_HANDOFF_ERROR_CONGESTION_DROP, frame->n_vectors - n_enq);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    return wg_handoff(vm, node, from_frame, wmp->in4_fq_index, WG_HANDOFF_HANDSHAKE);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    return wg_handoff(vm, node, from_frame, wmp->in6_fq_index, WG_HANDOFF_HANDSHAKE);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    return wg_handoff(vm, node, from_frame, wmp->in4_fq_index, WG_HANDOFF_INP_DATA);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    return wg_handoff(vm, node, from_frame, wmp->in6_fq_index, WG_HANDOFF_INP_DATA);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    return wg_handoff(vm, node, from_frame, wmp->out4_fq_index, WG_HANDOFF_OUT_TUN);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    return wg_handoff(vm, node, from_frame, wmp->out6_fq_index, WG_HANDOFF_OUT_TUN);
}
