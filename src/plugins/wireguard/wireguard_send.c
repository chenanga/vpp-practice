...
{
    vlib_frame_t *f                 = 0;
    u32           lookup_node_index = is_ip4 ? ip4_lookup_node.index : ip6_lookup_node.index;
...
static void wg_buffer_prepend_rewrite(vlib_main_t *vm, vlib_buffer_t *b0, const u8 *rewrite, u8 is_ip4)
{
    if (is_ip4) {
        ...
        hdr4->udp.length = clib_host_to_net_u16(b0->current_length - sizeof(ip4_header_t));
        ip4_header_set_len_w_chksum(&hdr4->ip4, clib_host_to_net_u16(b0->current_length));
    }
    else {
        ...
        hdr6->ip6.payload_length = hdr6->udp.length = clib_host_to_net_u16(b0->current_length - sizeof(ip6_header_t));
...
static bool wg_create_buffer(vlib_main_t *vm, const u8 *rewrite, const u8 *packet, u32 packet_len, u32 *bi, u8 is_ip4)
{
    u32            n_buf0 = 0;
    vlib_buffer_t *b0;

    n_buf0 = vlib_buffer_alloc(vm, bi, 1);
    if (!n_buf0) return false;
...
u8 *wg_build_rewrite(ip46_address_t *src_addr, u16 src_port, ip46_address_t *dst_addr, u16 dst_port, u8 is_ip4)
{
    if (ip46_address_is_zero(dst_addr) || 0 == dst_port) return NULL;
...
bool wg_send_handshake(vlib_main_t *vm, wg_peer_t *peer, bool is_retry)
{
    ...
    if (!wg_peer_can_send(peer)) return false;

    message_handshake_initiation_t packet;

    if (!is_retry) peer->timer_handshake_attempts = 0;

    if (!wg_birthdate_has_expired(peer->last_sent_handshake, REKEY_TIMEOUT) || wg_peer_is_dead(peer)) return true;
    ...
    u32 bi0    = 0;
    if (!wg_create_buffer(vm, peer->rewrite, (u8 *) &packet, sizeof(packet), &bi0, is_ip4)) return false;
...
void wg_send_handshake_from_mt(u32 peer_idx, bool is_retry)
{
    ...
    bool handshake = __atomic_load_n(&peer->handshake_is_sent, __ATOMIC_ACQUIRE);

    if (handshake == false) {
        handshake = true;
        __atomic_store_n(&peer->handshake_is_sent, handshake, __ATOMIC_RELEASE);
        vl_api_rpc_call_main_thread(wg_send_handshake_thread_fn, (u8 *) &a, sizeof(a));
    }
}
...
bool wg_send_keepalive(vlib_main_t *vm, wg_peer_t *peer)
{
    ...
    message_data_t        *packet         = (message_data_t *) wg_main.per_thread_data[vm->thread_index].data;
    u32                    bi0            = 0;
    bool                   ret            = true;
    ...
    state = noise_remote_encrypt(vm, &peer->remote, &packet->receiver_index, &packet->counter, NULL, 0, packet->encrypted_data);
    ...
    if (!wg_create_buffer(vm, peer->rewrite, (u8 *) packet, size_of_packet, &bi0, is_ip4)) {
        ret = false;
        goto out;
    }
...
bool wg_send_handshake_cookie(vlib_main_t      *vm,
                              u32               sender_index,
                              cookie_checker_t *cookie_checker,
                              message_macs_t   *macs,
                              ip46_address_t   *wg_if_addr,
                              u16               wg_if_port,
                              ...
                              u16               remote_port)
{
    ...
    cookie_checker_create_payload(vm, cookie_checker, macs, packet.nonce, packet.encrypted_cookie, remote_addr, remote_port);

    u32  bi0    = 0;
    u8   is_ip4 = ip46_address_is_ip4(remote_addr);
    bool ret;
    rewrite = wg_build_rewrite(wg_if_addr, wg_if_port, remote_addr, remote_port, is_ip4);

    ret = wg_create_buffer(vm, rewrite, (u8 *) &packet, sizeof(packet), &bi0, is_ip4);
    vec_free(rewrite);
    if (!ret) return false;
