...
u8 *format_ip4_udp_header(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U:$U", format_ip4_header, &hdr4->ip4, format_udp_header, &hdr4->udp);
    return (s);
}

u8 *format_ip6_udp_header(u8 *s, va_list *args)
{
    ip6_udp_header_t *hdr6 = va_arg(*args, ip6_udp_header_t *);

    s = format(s, "%U:$U", format_ip6_header, &hdr6->ip6, format_udp_header, &hdr6->udp);
    return (s);
}
...
static u8 *format_wg_output_tun_trace(u8 *s, va_list *args)
{
    ...
    s = t->is_ip4 ? format(s, "%U", format_ip4_udp_header, t->header) : format(s, "%U", format_ip6_udp_header, t->header);
    return s;
}

/* post node - packet trace format function */
static u8 *format_wg_output_tun_post_trace(u8 *s, va_list *args)
{
    CLIB_UNUSED(vlib_main_t * vm)   = va_arg(*args, vlib_main_t *);
    CLIB_UNUSED(vlib_node_t * node) = va_arg(*args, vlib_node_t *);
...
wg_output_chain_crypto(vlib_main_t *vm, wg_per_thread_data_t *ptd, vlib_buffer_t *b, vlib_buffer_t *lb, u8 *start, u32 start_len, u16 *n_ch)
{
...
static_always_inline void wg_prepare_sync_enc_op(vlib_main_t            *vm,
...
static_always_inline void wg_output_process_chained_ops(vlib_main_t            *vm,
...
static_always_inline void wg_output_process_ops(vlib_main_t         *vm,
                                                vlib_node_runtime_t *node,
                                                vnet_crypto_op_t    *ops,
                                                vlib_buffer_t       *b[],
                                                u16                 *nexts,
...
static_always_inline void wg_output_tun_add_to_frame(vlib_main_t               *vm,
...
static_always_inline enum noise_state_crypt wg_output_tun_process(vlib_main_t          *vm,
                                                                  wg_per_thread_data_t *ptd,
                                                                  vlib_buffer_t        *b,
                                                                  vlib_buffer_t        *lb,
                                                                  vnet_crypto_op_t    **crypto_ops,
                                                                  ...
                                                                  f64                   time)
{
     ...
     */
    if (!kp->kp_valid || wg_birthdate_has_expired_opt(kp->kp_birthdate, REJECT_AFTER_TIME, time) || kp->kp_ctr.c_recv >= REJECT_AFTER_MESSAGES
    ...
    *r_idx = kp->kp_remote_index;
    ...
    wg_prepare_sync_enc_op(vm, ptd, b, lb, crypto_ops, src, srclen, dst, NULL, 0, *nonce, kp->kp_send_index, bi, iv);
    ...
    if ((kp->kp_valid && *nonce >= REKEY_AFTER_MESSAGES)
        || (kp->kp_is_initiator && wg_birthdate_has_expired_opt(kp->kp_birthdate, REKEY_AFTER_TIME, time)))
        goto error;
...
static_always_inline enum noise_state_crypt wg_add_to_async_frame(vlib_main_t                *vm,
                                                                  wg_per_thread_data_t       *ptd,
                                                                  ...
                                                                  f64                         time)
{
    ...
    *r_idx = kp->kp_remote_index;
    ...
    if (NULL == *async_frame || vnet_crypto_async_frame_is_full(*async_frame)) {
        *async_frame = vnet_crypto_async_get_frame(vm, VNET_CRYPTO_OP_CHACHA20_POLY1305_TAG16_AAD0_ENC);
        if (PREDICT_FALSE(NULL == *async_frame)) goto error;
        /* Save the frame to the list we'll submit at the end */
        vec_add1(ptd->async_frames, *async_frame);
    }

    if (b != lb) flag |= VNET_CRYPTO_OP_FLAG_CHAINED_BUFFERS;

    tag = vlib_buffer_get_tail(lb) - NOISE_AUTHTAG_LEN;

    /* this always succeeds because we know the frame is not full */
    wg_output_tun_add_to_frame(vm, *async_frame, kp->kp_send_index, payload_len, payload - b->data, bi, async_next, iv, tag, flag);
...
static_always_inline void wg_calc_checksum(vlib_main_t *vm, vlib_buffer_t *b)
{
    int bogus      = 0;
    ...
    if (ip_ver_out == 6) {
        ip6_header_t *ip6 = (ip6_header_t *) ((u8 *) vlib_buffer_get_current(b));
...
always_inline uword wg_output_tun_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_ip4, u16 async_next_node)
{
    ...
    while (n_left_from > 0) {
        ...
        if (PREDICT_FALSE(~0 == peer->output_thread_index)) {
             ...
             * for this thread.
             */
            clib_atomic_cmp_and_swap(&peer->output_thread_index, ~0, wg_peer_assign_thread(thread_index));
        }
         ...
         * trigger this)
         */
        ASSERT((signed) b[0]->current_data >= (signed) -VLIB_BUFFER_PRE_DATA_SIZE);
        b_space_left_at_beginning = b[0]->current_data + VLIB_BUFFER_PRE_DATA_SIZE;
        if (PREDICT_FALSE(b_space_left_at_beginning < sizeof(ethernet_header_t))) {
            u32 size_diff = sizeof(ethernet_header_t) - b_space_left_at_beginning;

            /* Can only move buffer when it's single and has enough free space*/
            if (lb == b[0] && vlib_buffer_space_left_at_end(vm, b[0]) >= size_diff) {
                vlib_buffer_move(vm, b[0], b[0]->current_data + (signed) size_diff);
            }
            else {
                b[0]->error = node->errors[WG_OUTPUT_ERROR_NO_BUFFERS];
                goto out;
         ...
         * write auth tag */
        if (PREDICT_FALSE(vlib_buffer_space_left_at_end(vm, lb) < NOISE_AUTHTAG_LEN)) {
            u32 tmp_bi = 0;
        ...
        plain_data_len_total        = vlib_buffer_length_in_chain(vm, b[0]) - iph_offset;
        ...
        if (is_async) {
            state = wg_add_to_async_frame(vm,
                                          ptd,
                                          &async_frame,
                                          b[0],
                                          lb,
                                          ...
                                          async_next_node,
                                          &peer->remote,
                                          &message_data_wg->receiver_index,
                                          &message_data_wg->counter,
                                          iv_data,
                                          time);
        }
        else {
            state = wg_output_tun_process(vm,
                                          ptd,
                                          b[0],
                                          lb,
                                          crypto_ops,
        ...
        if (is_ip4_out) {
            ...
            hdr4_out->udp.length     = clib_host_to_net_u16(encrypted_packet_len + sizeof(udp_header_t));
            ip4_header_set_len_w_chksum(&hdr4_out->ip4, clib_host_to_net_u16(encrypted_packet_len + sizeof(ip4_udp_header_t)));
        }
        else {
            hdr6_out->wg.header.type     = MESSAGE_DATA;
            hdr6_out->ip6.payload_length = hdr6_out->udp.length = clib_host_to_net_u16(encrypted_packet_len + sizeof(udp_header_t));
        }

    out:
        if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b[0]->flags & VLIB_BUFFER_IS_TRACED))) {
            wg_output_tun_trace_t *t = vlib_add_trace(vm, node, b[0], sizeof(*t));
    ...
    if (n_sync) {
        ...
        wg_output_process_ops(vm, node, ptd->crypto_ops, sync_bufs, nexts, drop_next);
        wg_output_process_chained_ops(vm, node, ptd->chained_crypto_ops, sync_bufs, nexts, ptd->chunks, drop_next);
...
always_inline uword wg_output_tun_post(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    vlib_buffer_t *bufs[VLIB_FRAME_SIZE], **b    = bufs;
    u16            nexts[VLIB_FRAME_SIZE], *next = nexts;
    u32           *from   = vlib_frame_vector_args(frame);
    ...
    while (n_left > 8) {
        ...
        if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE)) {
            if (b[0]->flags & VLIB_BUFFER_IS_TRACED) {
                wg_output_tun_post_trace_t *tr = vlib_add_trace(vm, node, b[0], sizeof(*tr));
                peeri                          = wg_peer_get_by_adj_index(vnet_buffer(b[0])->ip.adj_index[VLIB_TX]);
                tr->peer                       = peeri;
                tr->next_index                 = next[0];
            }
            if (b[1]->flags & VLIB_BUFFER_IS_TRACED) {
                wg_output_tun_post_trace_t *tr = vlib_add_trace(vm, node, b[1], sizeof(*tr));
                peeri                          = wg_peer_get_by_adj_index(vnet_buffer(b[1])->ip.adj_index[VLIB_TX]);
                tr->next_index                 = next[1];
            }
            if (b[2]->flags & VLIB_BUFFER_IS_TRACED) {
                wg_output_tun_post_trace_t *tr = vlib_add_trace(vm, node, b[2], sizeof(*tr));
                peeri                          = wg_peer_get_by_adj_index(vnet_buffer(b[2])->ip.adj_index[VLIB_TX]);
                tr->next_index                 = next[2];
            }
            if (b[3]->flags & VLIB_BUFFER_IS_TRACED) {
                wg_output_tun_post_trace_t *tr = vlib_add_trace(vm, node, b[3], sizeof(*tr));
                peeri                          = wg_peer_get_by_adj_index(vnet_buffer(b[3])->ip.adj_index[VLIB_TX]);
...
{
    return wg_output_tun_inline(vm, node, frame, /* is_ip4 */ 1, wg_encrypt_async_next.wg4_post_next);
}

VLIB_NODE_FN(wg6_output_tun_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return wg_output_tun_inline(vm, node, frame, /* is_ip4 */ 0, wg_encrypt_async_next.wg6_post_next);
}
