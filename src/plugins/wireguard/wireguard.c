...
static void wireguard_register_post_node(vlib_main_t *vm)

{
    ...
    eit->wg4_post_next = vnet_crypto_register_post_node(vm, "wg4-output-tun-post-node");
    eit->wg6_post_next = vnet_crypto_register_post_node(vm, "wg6-output-tun-post-node");

    dit->wg4_post_next = vnet_crypto_register_post_node(vm, "wg4-input-post-node");
    dit->wg6_post_next = vnet_crypto_register_post_node(vm, "wg6-input-post-node");
}
...
static clib_error_t *wg_init(vlib_main_t *vm)
{
    ...
    wmp->out4_fq_index = vlib_frame_queue_main_init(wg4_output_tun_node.index, 0);
    wmp->out6_fq_index = vlib_frame_queue_main_init(wg6_output_tun_node.index, 0);

    vlib_thread_main_t *tm = vlib_get_thread_main();

    vec_validate_aligned(wmp->per_thread_data, tm->n_vlib_mains, CLIB_CACHE_LINE_BYTES);
