...
static u8 *format_wg_input_post_trace(u8 *s, va_list *args)
{
    CLIB_UNUSED(vlib_main_t * vm)   = va_arg(*args, vlib_main_t *);
    CLIB_UNUSED(vlib_node_t * node) = va_arg(*args, vlib_node_t *);
...
static u8 is_ip4_header(u8 *data)
{
    return (data[0] >> 4) == 0x4;
}

static wg_input_error_t wg_handshake_process(vlib_main_t *vm, wg_main_t *wmp, vlib_buffer_t *b, u32 node_idx, u8 is_ip4)
{
    ...
    if (is_ip4) {
        ip4_header_t *iph4 = current_b_data - sizeof(udp_header_t) - sizeof(ip4_header_t);
        ip46_address_set_ip4(&src_ip, &iph4->src_address);
    }
    else {
        ip6_header_t *iph6 = current_b_data - sizeof(udp_header_t) - sizeof(ip6_header_t);
    ...
    if (PREDICT_FALSE(header->type == MESSAGE_HANDSHAKE_COOKIE)) {
        ...
        u32                        *entry  = wg_index_table_lookup(&wmp->index_table, packet->receiver_index);
        if (entry)
            peer = wg_peer_get(*entry);
        else
            return WG_INPUT_ERROR_PEER;

        if (!cookie_maker_consume_payload(vm, &peer->cookie_maker, packet->nonce, packet->encrypted_cookie)) return WG_INPUT_ERROR_COOKIE_DECRYPTION;

        return WG_INPUT_ERROR_NONE;
    }

    u32 len = (header->type == MESSAGE_HANDSHAKE_INITIATION ? sizeof(message_handshake_initiation_t) : sizeof(message_handshake_response_t));

    message_macs_t *macs = (message_macs_t *) ((u8 *) current_b_data + len - sizeof(*macs));

    index_t *ii;
    wg_ifs = wg_if_indexes_get_by_port(udp_dst_port);
    if (NULL == wg_ifs) return WG_INPUT_ERROR_INTERFACE;

    vec_foreach (ii, wg_ifs) {
        wg_if = wg_if_get(*ii);
        if (NULL == wg_if) continue;

        under_load = wg_if_is_under_load(vm, wg_if);
        mac_state  = cookie_checker_validate_macs(vm, &wg_if->cookie_checker, macs, current_b_data, len, under_load, &src_ip, udp_src_port);
    ...
    if (NULL == wg_if) return WG_INPUT_ERROR_HANDSHAKE_MAC;
    ...
    switch (header->type) {
        case MESSAGE_HANDSHAKE_INITIATION: {
            ...
            if (packet_needs_cookie) {
                if (!wg_send_handshake_cookie(vm,
                                              message->sender_index,
                                              &wg_if->cookie_checker,
                                              macs,
            ...
            noise_remote_t *rp;
            if (noise_consume_initiation(vm,
                                         noise_local_get(wg_if->local_idx),
                                         &rp,
                                         message->sender_index,
            ...
            if (PREDICT_FALSE(!wg_send_handshake_response(vm, peer))) {
                vlib_node_increment_counter(vm, node_idx, WG_INPUT_ERROR_HANDSHAKE_SEND, 1);
            }
            break;
        }
        case MESSAGE_HANDSHAKE_RESPONSE: {
            ...
            u32    *entry = wg_index_table_lookup(&wmp->index_table, resp->receiver_index);

            if (PREDICT_TRUE(entry != NULL)) {
                peeri = *entry;
                peer  = wg_peer_get(peeri);
                if (wg_peer_is_dead(peer)) return WG_INPUT_ERROR_PEER;
            }
            else
                return WG_INPUT_ERROR_PEER;

            if (!noise_consume_response(vm,
                                        &peer->remote,
                                        resp->sender_index,
                                        resp->receiver_index,
            ...
            if (noise_remote_begin_session(vm, &peer->remote)) {
                ...
                if (PREDICT_FALSE(!wg_send_keepalive(vm, peer))) {
                    vlib_node_increment_counter(vm, node_idx, WG_INPUT_ERROR_KEEPALIVE_SEND, 1);
                }
                else {
...
static_always_inline int wg_input_post_process(vlib_main_t    *vm,
                                               vlib_buffer_t  *b,
                                               u16            *next,
                                               wg_peer_t      *peer,
                                               message_data_t *data,
                                               bool           *is_keepalive)
{
    ...
    if ((kp = wg_get_active_keypair(&peer->remote, data->receiver_index)) == NULL) return -1;
    ...
    if (is_ip4_inner) {
        ip46_address_set_ip4(&src_ip, &((ip4_header_t *) vlib_buffer_get_current(b))->src_address);
    }
    else {
        ip46_address_set_ip6(&src_ip, &((ip6_header_t *) vlib_buffer_get_current(b))->src_address);
    }
    ...
    if (allowed) {
        ...
        next[0]                              = is_ip4_inner ? WG_INPUT_NEXT_IP4_INPUT : WG_INPUT_NEXT_IP6_INPUT;
    }

    return 0;
}

static_always_inline void wg_input_process_ops(vlib_main_t         *vm,
...
static_always_inline void wg_input_process_chained_ops(vlib_main_t            *vm,
                                                       vlib_node_runtime_t    *node,
                                                       vnet_crypto_op_t       *ops,
                                                       vlib_buffer_t          *b[],
                                                       u16                    *nexts,
...
wg_input_chain_crypto(vlib_main_t *vm, wg_per_thread_data_t *ptd, vlib_buffer_t *b, vlib_buffer_t *lb, u8 *start, u32 start_len, u16 *n_ch)
{
...
always_inline void wg_prepare_sync_dec_op(vlib_main_t            *vm,
                                          ...
                                          u8                     *iv)
{
    ...
    if (b != lb) {
        ...
        wg_input_chain_crypto(vm, ptd, b, lb, src, src_len + NOISE_AUTHTAG_LEN, &op->n_chunks);
    }
    else {
        op->src = !src ? src_ : src;
        op->len = src_len;
...
static_always_inline void wg_input_add_to_frame(vlib_main_t               *vm,
...
static_always_inline enum noise_state_crypt wg_input_process(vlib_main_t                *vm,
                                                             wg_per_thread_data_t       *ptd,
                                                             vnet_crypto_op_t          **crypto_ops,
                                                             ...
                                                             u16                         async_next_node)
{
    ...
    if (wg_birthdate_has_expired_opt(kp->kp_birthdate, REJECT_AFTER_TIME, time) || kp->kp_ctr.c_recv >= REJECT_AFTER_MESSAGES) goto error;
    ...
    if (is_async) {
        ...
        if (b != lb) flags |= VNET_CRYPTO_OP_FLAG_CHAINED_BUFFERS;

        if (NULL == *async_frame || vnet_crypto_async_frame_is_full(*async_frame)) {
            *async_frame = vnet_crypto_async_get_frame(vm, VNET_CRYPTO_OP_CHACHA20_POLY1305_TAG16_AAD0_DEC);
            if (PREDICT_FALSE(NULL == *async_frame)) goto error;
            /* Save the frame to the list we'll submit at the end */
            vec_add1(ptd->async_frames, *async_frame);
        }

        wg_input_add_to_frame(vm, *async_frame, kp->kp_recv_index, srclen_total, src - b->data, buf_idx, async_next_node, iv, tag, flags);
    }
    else {
        wg_prepare_sync_dec_op(vm, ptd, b, lb, crypto_ops, src, srclen, dst, NULL, 0, kp->kp_recv_index, from_idx, iv);
    }
    ...
    kp  = r->r_current;
    if (kp != NULL && kp->kp_valid && kp->kp_is_initiator && wg_birthdate_has_expired_opt(kp->kp_birthdate, REKEY_AFTER_TIME_RECV, time)) goto error;
...
static_always_inline void wg_find_outer_addr_port(vlib_buffer_t *b, ip46_address_t *addr, u16 *port, u8 is_ip4)
{
    if (is_ip4) {
        ip4_udp_header_t *ip4_udp_hdr = vlib_buffer_get_current(b) - sizeof(ip4_udp_header_t);
        ip46_address_set_ip4(addr, &ip4_udp_hdr->ip4.src_address);
        *port = clib_net_to_host_u16(ip4_udp_hdr->udp.src_port);
    }
    else {
        ip6_udp_header_t *ip6_udp_hdr = vlib_buffer_get_current(b) - sizeof(ip6_udp_header_t);
        ip46_address_set_ip6(addr, &ip6_udp_hdr->ip6.src_address);
        *port = clib_net_to_host_u16(ip6_udp_hdr->udp.src_port);
    }
}

always_inline uword wg_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_ip4, u16 async_next_node)
{
    vnet_main_t           *vnm         = vnet_get_main();
    vnet_interface_main_t *im          = &vnm->interface_main;
    wg_main_t             *wmp         = &wg_main;
    wg_per_thread_data_t  *ptd         = vec_elt_at_index(wmp->per_thread_data, vm->thread_index);
    ...
    while (n_left_from > 0) {
        ...
        if (PREDICT_TRUE(header_type == MESSAGE_DATA)) {
            ...
            u32             n_bufs;
            peer_idx = wg_index_table_lookup(&wmp->index_table, data->receiver_index);
            ...
            if (PREDICT_FALSE(~0 == peer->input_thread_index)) {
                 ...
                 * for this thread.
                 */
                clib_atomic_cmp_and_swap(&peer->input_thread_index, ~0, wg_peer_assign_thread(thread_index));
            }
            ...
            if (n_bufs > 1) {
                ...
                if (PREDICT_FALSE(lb->current_length < NOISE_AUTHTAG_LEN)) {
                    ...
                    if (before_last == b[0]) before_last->flags &= ~VLIB_BUFFER_TOTAL_LENGTH_VALID;

                    vlib_buffer_advance(lb, (signed) -len_diff);

                    clib_memcpy_fast(vlib_buffer_get_current(lb), vlib_buffer_get_tail(before_last), len_diff);
                }
            }

            u16 encr_len       = b[0]->current_length - sizeof(message_data_t);
            u16 decr_len       = encr_len - NOISE_AUTHTAG_LEN;
            u16 encr_len_total = vlib_buffer_length_in_chain(vm, b[0]) - sizeof(message_data_t);
            ...
            enum noise_state_crypt state_cr = wg_input_process(vm,
                                                               ptd,
                                                               crypto_ops,
                                                               &async_frame,
                                                               b[0],
            ...
            wg_input_error_t ret = wg_handshake_process(vm, wmp, b[0], node->node_index, is_ip4);
    ...
    /* decrypt packets */
    wg_input_process_ops(vm, node, ptd->crypto_ops, data_bufs, data_nexts, drop_next);
    wg_input_process_chained_ops(vm, node, ptd->chained_crypto_ops, data_bufs, data_nexts, ptd->chunks, drop_next);
    ...
    while (n_left_from > 0) {
        ...
        if (PREDICT_TRUE(peer != NULL)) {
            if (PREDICT_FALSE(wg_input_post_process(vm, b[0], data_next, peer, data, &is_keepalive) < 0)) goto trace;
        }
        else {
        ...
        if (PREDICT_FALSE(peer_idx && (last_peer_time_idx != peer_idx))) {
            if (PREDICT_FALSE(!ip46_address_is_equal(&peer->dst.addr, &out_src_ip) || peer->dst.port != out_udp_src_port))
                wg_peer_update_endpoint_from_mt(peeri, &out_src_ip, out_udp_src_port);
    ...
    if (n_async) {
        ...
        vec_foreach (async_frame, ptd->async_frames) {
            if (PREDICT_FALSE(vnet_crypto_async_submit_open_frame(vm, *async_frame) < 0)) {
    ...
    /* enqueue other bufs */
    if (n_other) vlib_buffer_enqueue_to_next(vm, node, other_bi, other_next, n_other);

    /* enqueue data bufs */
    if (n_data) vlib_buffer_enqueue_to_next(vm, node, data_bi, data_nexts, n_data);

    return frame->n_vectors;
}

always_inline uword wg_input_post(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_ip4)
{
    vnet_main_t           *vnm = vnet_get_main();
    vnet_interface_main_t *im  = &vnm->interface_main;
    wg_main_t             *wmp = &wg_main;
    ...
    while (n_left > 0) {
        ...
        if (PREDICT_TRUE(peer != NULL)) {
            if (PREDICT_FALSE(wg_input_post_process(vm, b[0], next, peer, data, &is_keepalive) < 0)) goto trace;
        }
        else {
            next[0] = WG_INPUT_NEXT_PUNT;
            goto trace;
    ...
    trace:
        if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b[0]->flags & VLIB_BUFFER_IS_TRACED))) {
            wg_input_post_trace_t *t = vlib_add_trace(vm, node, b[0], sizeof(*t));
            t->next                  = next[0];
            t->peer                  = peer_idx ? peeri : INDEX_INVALID;
        }
...
{
    return wg_input_inline(vm, node, frame, /* is_ip4 */ 1, wg_decrypt_async_next.wg4_post_next);
}

VLIB_NODE_FN(wg6_input_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return wg_input_inline(vm, node, frame, /* is_ip4 */ 0, wg_decrypt_async_next.wg6_post_next);
}
