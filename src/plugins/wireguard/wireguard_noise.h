...
static_always_inline uint64_t noise_counter_send(noise_counter_t *ctr)
{
    uint64_t ret;
    ret = ctr->c_send++;
    return ret;
...
void noise_remote_init(vlib_main_t *, noise_remote_t *, uint32_t, const uint8_t[NOISE_PUBLIC_KEY_LEN], uint32_t);
...
enum noise_state_crypt
noise_remote_encrypt(vlib_main_t *vm, noise_remote_t *, uint32_t *r_idx, uint64_t *nonce, uint8_t *src, size_t srclen, uint8_t *dst);

static_always_inline noise_keypair_t *wg_get_active_keypair(noise_remote_t *r, uint32_t r_idx)
{
...
inline bool noise_counter_recv(noise_counter_t *ctr, uint64_t recv)
{
    ...
    if (ctr->c_recv >= REJECT_AFTER_MESSAGES || recv >= REJECT_AFTER_MESSAGES) goto error;

    /* If the packet is out of the window, invalid */
    if (recv + COUNTER_WINDOW_SIZE < ctr->c_recv) goto error;
    ...
    if (ctr->c_backtrack[index_recv] & bit) goto error;
...
static_always_inline void noise_remote_keypair_free(vlib_main_t *vm, noise_remote_t *r, noise_keypair_t **kp)
{
