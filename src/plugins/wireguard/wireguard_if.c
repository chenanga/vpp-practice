...
u8 *format_wg_if(u8 *s, va_list *args)
{
    ...
    s = format(s, " %U", format_hex_bytes, local->l_private, NOISE_PUBLIC_KEY_LEN);

    key_to_base64(local->l_public, NOISE_PUBLIC_KEY_LEN, key);

    s = format(s, " public-key:%s", key);

    s = format(s, " %U", format_hex_bytes, local->l_public, NOISE_PUBLIC_KEY_LEN);

    s = format(s, " mac-key: %U", format_hex_bytes, &wgi->cookie_checker.cc_mac1_key, NOISE_PUBLIC_KEY_LEN);
...
{
    if (vec_len(wg_if_index_by_sw_if_index) <= sw_if_index) return INDEX_INVALID;
...
static walk_rc_t wg_if_find_peer_by_public_key(index_t peeri, void *data)
{
    ...
    if (!memcmp(peer->remote.r_public, public, NOISE_PUBLIC_KEY_LEN)) return (WALK_STOP);
...
static noise_remote_t *wg_remote_get(const uint8_t public[NOISE_PUBLIC_KEY_LEN])
{
    ...
    if (INDEX_INVALID != peeri) return &wg_peer_get(peeri)->remote;

    return NULL;
}

static uint32_t wg_index_set(vlib_main_t *vm, noise_remote_t *remote)
{
    wg_main_t *wmp      = &wg_main;
    u32        rnd_seed = (u32) (vlib_time_now(wmp->vlib_main) * 1e6);
    u32        ret      = wg_index_table_add(vm, &wmp->index_table, remote->r_peer_idx, rnd_seed);
    return ret;
}

static void wg_index_drop(vlib_main_t *vm, uint32_t key)
{
    wg_main_t *wmp = &wg_main;
...
int wg_if_create(u32 user_instance, const u8 private_key[NOISE_PUBLIC_KEY_LEN], u16 port, const ip_address_t *src_ip, u32 *sw_if_indexp)
{
    ...
    *sw_if_indexp = (u32) ~0;
    ...
    if (instance == ~0) return VNET_API_ERROR_INVALID_REGISTRATION;
    ...
    if (~0 == wg_if->user_instance) wg_if->user_instance = t_idx;

    vec_validate_init_empty(wg_if_indexes_by_port, port, NULL);
    if (vec_len(wg_if_indexes_by_port[port]) == 0) {
        udp_register_dst_port(vlib_get_main(), port, wg4_input_node.index, UDP_IP4);
        udp_register_dst_port(vlib_get_main(), port, wg6_input_node.index, UDP_IP6);
    }
    ...
    hw_if_index = vnet_register_interface(vnm, wg_if_device_class.index, t_idx, wg_hw_interface_class.index, t_idx);

    hi = vnet_get_hw_interface(vnm, hw_if_index);

    vec_validate_init_empty(wg_if_index_by_sw_if_index, hi->sw_if_index, INDEX_INVALID);
...
int wg_if_delete(u32 sw_if_index)
{
    ...
    if (hw == 0 || hw->dev_class_index != wg_if_device_class.index) return VNET_API_ERROR_INVALID_VALUE;

    wg_if_t *wg_if;
    index_t  wgii = wg_if_find_by_sw_if_index(sw_if_index);
    wg_if         = wg_if_get(wgii);
    if (NULL == wg_if) return VNET_API_ERROR_INVALID_SW_IF_INDEX_2;

    if (wg_if_instance_free(wg_if->user_instance) < 0) return VNET_API_ERROR_INVALID_VALUE_2;
...
void wg_if_peer_add(wg_if_t *wgi, index_t peeri)
{
    ...
    if (1 == hash_elts(wgi->peers)) {
        vnet_feature_enable_disable("ip4-output", "wg4-output-tun", wgi->sw_if_index, 1, 0, 0);
        vnet_feature_enable_disable("ip6-output", "wg6-output-tun", wgi->sw_if_index, 1, 0, 0);
    }
}
...
void wg_if_peer_remove(wg_if_t *wgi, index_t peeri)
{
    ...
    if (0 == hash_elts(wgi->peers)) {
        vnet_feature_enable_disable("ip4-output", "wg4-output-tun", wgi->sw_if_index, 0, 0, 0);
        vnet_feature_enable_disable("ip6-output", "wg6-output-tun", wgi->sw_if_index, 0, 0, 0);
    }
}
...
void wg_if_walk(wg_if_walk_cb_t fn, void *data)
{
    ...
    pool_foreach_index (wgii, wg_if_pool) {
        if (WALK_STOP == fn(wgii, data)) break;
    }
}
