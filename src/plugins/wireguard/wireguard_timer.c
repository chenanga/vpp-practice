...
static u32 get_random_u32_max_opt(u32 max, f64 time)
{
...
static void start_timer(wg_peer_t *peer, u32 timer_id, u32 interval_ticks)
{
    ...
    if (peer->timers[timer_id] == ~0) {
        peer->timers[timer_id] = tw_timer_start_16t_2w_512sl(peer->timer_wheel, peer - wg_peer_pool, timer_id, interval_ticks);
    }
}
...
static_always_inline void start_timer_from_mt(u32 peer_idx, u32 timer_id, u32 interval_ticks)
{
    wg_timers_args a = {
        .peer_idx       = peer_idx,
        .timer_id       = timer_id,
...
static inline u32 timer_ticks_left(vlib_main_t *vm, f64 init_time_sec, u32 interval_ticks)
{
...
{
    if (peer->rehandshake_started == ~0) return;

    u32 ticks = timer_ticks_left(vm, peer->rehandshake_started, peer->rehandshake_interval_tick);
    if (ticks) {
...
static void wg_expired_send_keepalive(vlib_main_t *vm, wg_peer_t *peer)
{
    if (peer->last_sent_packet < peer->last_received_packet) {
        u32 ticks = timer_ticks_left(vm, peer->last_received_packet, KEEPALIVE_TIMEOUT * WHZ);
        if (ticks) {
...
static void wg_expired_send_persistent_keepalive(vlib_main_t *vm, wg_peer_t *peer)
{
    if (peer->persistent_keepalive_interval) {
        f64 latest_time = peer->last_sent_packet > peer->last_received_packet ? peer->last_sent_packet : peer->last_received_packet;

        u32 ticks = timer_ticks_left(vm, latest_time, peer->persistent_keepalive_interval * WHZ);
        if (ticks) {
...
{
    u32 ticks = timer_ticks_left(vm, peer->last_sent_packet, peer->new_handshake_interval_tick);
    if (ticks) {
...
{
    u32 ticks = timer_ticks_left(vm, peer->session_derived, REJECT_AFTER_TIME * 3 * WHZ);
    if (ticks) {
...
inline void wg_timers_any_authenticated_packet_traversal(wg_peer_t *peer)
{
    if (peer->persistent_keepalive_interval) {
        start_timer_from_mt(peer - wg_peer_pool, WG_TIMER_PERSISTENT_KEEPALIVE, peer->persistent_keepalive_interval * WHZ);
    }
}
...
inline void wg_timers_any_authenticated_packet_sent_opt(wg_peer_t *peer, f64 time)
{
...
void wg_timers_handshake_initiated(wg_peer_t *peer)
{
    ...
    peer->rehandshake_interval_tick = REKEY_TIMEOUT * WHZ + get_random_u32_max(REKEY_TIMEOUT_JITTER);

    start_timer_from_mt(peer - wg_peer_pool, WG_TIMER_RETRANSMIT_HANDSHAKE, peer->rehandshake_interval_tick);
}

void wg_timers_send_first_handshake(wg_peer_t *peer)
{
    // zero value is not allowed
    peer->new_handshake_interval_tick = get_random_u32_max(REKEY_TIMEOUT_JITTER) + 1;
    start_timer_from_mt(peer - wg_peer_pool, WG_TIMER_NEW_HANDSHAKE, peer->new_handshake_interval_tick);
}
...
void wg_timers_session_derived(wg_peer_t *peer)
{
    ...
    start_timer_from_mt(peer - wg_peer_pool, WG_TIMER_KEY_ZEROING, REJECT_AFTER_TIME * 3 * WHZ);
}

/* Should be called after an authenticated data packet is sent. */
void wg_timers_data_sent(wg_peer_t *peer)
{
    peer->new_handshake_interval_tick = (KEEPALIVE_TIMEOUT + REKEY_TIMEOUT) * WHZ + get_random_u32_max(REKEY_TIMEOUT_JITTER);

    start_timer_from_mt(peer - wg_peer_pool, WG_TIMER_NEW_HANDSHAKE, peer->new_handshake_interval_tick);
}

inline void wg_timers_data_sent_opt(wg_peer_t *peer, f64 time)
{
    peer->new_handshake_interval_tick = (KEEPALIVE_TIMEOUT + REKEY_TIMEOUT) * WHZ + get_random_u32_max_opt(REKEY_TIMEOUT_JITTER, time);
...
void wg_timers_data_received(wg_peer_t *peer)
{
    if (peer->timers[WG_TIMER_SEND_KEEPALIVE] == ~0) {
        start_timer_from_mt(peer - wg_peer_pool, WG_TIMER_SEND_KEEPALIVE, KEEPALIVE_TIMEOUT * WHZ);
    }
    else
...
inline void wg_timers_any_authenticated_packet_received_opt(wg_peer_t *peer, f64 time)
{
...
void wg_timer_wheel_init()
{
    ...
    tw_timer_wheel_init_16t_2w_512sl(tw, expired_timer_callback, WG_TICK /* timer period in s */, ~0);
}
...
static uword wg_timer_mngr_fn(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    while (1) {
        ...
        tw_timer_expire_timers_16t_2w_512sl(&wmp->timer_wheel, vlib_time_now(vm));
    }
...
{
    if (wmp->feature_init) return;
    vlib_process_signal_event(wmp->vlib_main, wg_timer_mngr_node.index, WG_START_EVENT, 0);
    wmp->feature_init = 1;
}
