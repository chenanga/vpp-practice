...
static void vl_api_wireguard_interface_create_t_handler(vl_api_wireguard_interface_create_t *mp)
{
    ...
    rv = wg_if_create(ntohl(mp->interface.user_instance), private_key, ntohs(mp->interface.port), &src, &sw_if_index);

    REPLY_MACRO2(VL_API_WIREGUARD_INTERFACE_CREATE_REPLY, { rmp->sw_if_index = htonl(sw_if_index); });
}
...
static walk_rc_t wireguard_if_send_details(index_t wgii, void *data)
{
    ...
    rmp->_vl_msg_id = htons(VL_API_WIREGUARD_INTERFACE_DETAILS + wg_main.msg_id_base);

    if (ctx->show_private_key) clib_memcpy(rmp->interface.private_key, local->l_private, NOISE_PUBLIC_KEY_LEN);
    clib_memcpy(rmp->interface.public_key, local->l_public, NOISE_PUBLIC_KEY_LEN);
...
static void vl_api_wireguard_interface_dump_t_handler(vl_api_wireguard_interface_dump_t *mp)
{
    ...
    if (sw_if_index == ~0)
        ...
        if (wgii != INDEX_INVALID) wireguard_if_send_details(wgii, &ctx);
    }
}
...
static void vl_api_wireguard_peer_add_t_handler(vl_api_wireguard_peer_add_t *mp)
{
    ...
    rv = wg_peer_add(ntohl(mp->peer.sw_if_index),
                     ...
                     allowed_ips,
                     ntohs(mp->peer.port),
                     ntohs(mp->peer.persistent_keepalive),
                     &peeri);
    ...
    REPLY_MACRO2(VL_API_WIREGUARD_PEER_ADD_REPLY, { rmp->peer_index = ntohl(peeri); });
}
...
static walk_rc_t wg_api_send_peers_details(index_t peeri, void *data)
{
    ...
    if (pool_is_free_index(wg_peer_pool, peeri)) return (WALK_CONTINUE);
    ...
    rmp->_vl_msg_id = htons(VL_API_WIREGUARD_PEERS_DETAILS + wg_main.msg_id_base);

    rmp->peer.peer_index = htonl(peeri);
    rmp->peer.flags      = peer->flags;
    clib_memcpy(rmp->peer.public_key, peer->remote.r_public, NOISE_PUBLIC_KEY_LEN);
...
static vpe_client_registration_t *wg_api_client_lookup(wg_peer_t *peer, u32 client_index)
{
    uword                     *p;
    vpe_client_registration_t *api_client = NULL;

    p = hash_get(peer->api_client_by_client_index, client_index);
    if (p) api_client = vec_elt_at_index(peer->api_clients, p[0]);

    return api_client;
}

static walk_rc_t wg_api_update_peer_api_client(index_t peeri, void *data)
{
    if (pool_is_free_index(wg_peer_pool, peeri)) return (WALK_CONTINUE);
    ...
    if (ntohl(mp->sw_if_index) != ~0 && ntohl(mp->sw_if_index) != peer->wg_sw_if_index) {
        return (WALK_CONTINUE);
    }
    ...
    if (mp->enable_disable) {
        ...
        hash_set(peer->api_client_by_client_index, mp->client_index, api_client - peer->api_clients);
    }

    return (WALK_CONTINUE);
}

static void vl_api_want_wireguard_peer_events_t_handler(vl_api_want_wireguard_peer_events_t *mp)
{
    wg_main_t                                 *wmp = &wg_main;
...
static void wg_api_send_peer_event(vl_api_registration_t *rp, index_t peer_index, wg_peer_flags flags)
{
...
static void wg_api_peer_event_cb(wg_api_peer_event_args_t *args)
{
...
void wg_api_peer_event(index_t peeri, wg_peer_flags flags)
{
    ...
    vl_api_rpc_call_main_thread(wg_api_peer_event_cb, (u8 *) &args, sizeof(args));
}

static void vl_api_wg_set_async_mode_t_handler(vl_api_wg_set_async_mode_t *mp)
{
    wg_main_t                        *wmp = &wg_main;
