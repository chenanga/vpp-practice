...
static void cookie_checker_make_cookie(vlib_main_t *vm, cookie_checker_t *, uint8_t[COOKIE_COOKIE_SIZE], ip46_address_t *ip, u16 udp_port);
...
void cookie_checker_init(cookie_checker_t *cc, ratelimit_entry_t *pool)
{
...
void cookie_checker_deinit(cookie_checker_t *cc)
{
    ratelimit_deinit(&cc->cc_ratelimit_v4);
    ratelimit_deinit(&cc->cc_ratelimit_v6);
}

void cookie_checker_create_payload(vlib_main_t      *vm,
                                   cookie_checker_t *cc,
                                   message_macs_t   *cm,
                                   ...
                                   u16               udp_port)
{
    ...
    wg_xchacha20poly1305_encrypt(vm, cookie, COOKIE_COOKIE_SIZE, ecookie, cm->mac1, COOKIE_MAC_SIZE, nonce, cc->cc_cookie_key);

    wg_secure_zero_memory(cookie, sizeof(cookie));
}

bool cookie_maker_consume_payload(vlib_main_t *vm, cookie_maker_t *cp, uint8_t nonce[COOKIE_NONCE_SIZE], uint8_t ecookie[COOKIE_ENCRYPTED_SIZE])
{
    uint8_t cookie[COOKIE_COOKIE_SIZE];
    ...
    if (!wg_xchacha20poly1305_decrypt(vm, ecookie, COOKIE_ENCRYPTED_SIZE, cookie, cp->cp_mac1_last, COOKIE_MAC_SIZE, nonce, cp->cp_cookie_key)) {
        return false;
    }
...
void cookie_maker_mac(cookie_maker_t *cp, message_macs_t *cm, void *buf, size_t len)
{
    ...
    if (!wg_birthdate_has_expired(cp->cp_birthdate, COOKIE_SECRET_MAX_AGE - COOKIE_SECRET_LATENCY))
                                                   ...
                                                   u16               udp_port)
{
    ...
    if (clib_memcmp(our_cm.mac1, cm->mac1, COOKIE_MAC_SIZE) != 0) return INVALID_MAC;

    if (!busy) return VALID_MAC_BUT_NO_COOKIE;

    cookie_checker_make_cookie(vm, cc, cookie, ip, udp_port);
    cookie_macs_mac2(&our_cm, buf, len, cookie);

    /* If the mac2 is invalid, we want to send a cookie response */
    if (clib_memcmp(our_cm.mac2, cm->mac2, COOKIE_MAC_SIZE) != 0) return VALID_MAC_BUT_NO_COOKIE;
...
/* Private functions */
static void cookie_precompute_key(uint8_t *key, const uint8_t input[COOKIE_INPUT_SIZE], const char *label)
{
...
static void cookie_macs_mac1(message_macs_t *cm, const void *buf, size_t len, const uint8_t key[COOKIE_KEY_SIZE])
{
...
static void cookie_macs_mac2(message_macs_t *cm, const void *buf, size_t len, const uint8_t key[COOKIE_COOKIE_SIZE])
{
    blake2s_state_t state;
...
static void cookie_checker_make_cookie(vlib_main_t *vm, cookie_checker_t *cc, uint8_t cookie[COOKIE_COOKIE_SIZE], ip46_address_t *ip, u16 udp_port)
{
    blake2s_state_t state;

    if (wg_birthdate_has_expired(cc->cc_secret_birthdate, COOKIE_SECRET_MAX_AGE)) {
...
static void ratelimit_init(ratelimit_t *rl, ratelimit_entry_t *pool)
{
    rl->rl_pool = pool;
}

static void ratelimit_deinit(ratelimit_t *rl)
{
    ratelimit_gc(rl, /* force */ true);
    hash_free(rl->rl_table);
}

static void ratelimit_gc(ratelimit_t *rl, bool force)
{
    u32                r_key;
    u32                r_idx;
    ratelimit_entry_t *r;
...
static bool ratelimit_allow(ratelimit_t *rl, ip46_address_t *ip)
{
    u32                r_key;
    uword             *p;
    u32                r_idx;
    ...
    if (ip46_address_is_ip4(ip)) /* Use all 4 bytes of IPv4 address */
        r_key = ip->ip4.as_u32;
    else
    ...
    if (p) {
        ...
        if (tokens > TOKEN_MAX) tokens = TOKEN_MAX;
    ...
    if (hash_elts(rl->rl_table) >= RATELIMIT_SIZE_MAX) return false;
