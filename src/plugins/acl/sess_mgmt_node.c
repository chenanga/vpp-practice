...
static void acl_fa_verify_init_sessions(acl_main_t *am)
{
    if (!am->fa_sessions_hash_is_initialized) {
        ...
        clib_bihash_set_kvp_format_fn_40_8(&am->fa_ip6_sessions_hash, format_ip6_session_bihash_kv);

        clib_bihash_init_16_8(&am->fa_ip4_sessions_hash,
                              "ACL plugin FA IPv4 session bihash",
                              am->fa_conn_table_hash_num_buckets,
                              am->fa_conn_table_hash_memory_size);
        clib_bihash_set_kvp_format_fn_16_8(&am->fa_ip4_sessions_hash, format_ip4_session_bihash_kv);
...
{
    if (session_index == FA_SESSION_BOGUS_INDEX) return 0;
    fa_session_t *sess         = get_session_ptr(am, thread_index, session_index);
    u64           timeout_time = sess->link_enqueue_time + fa_session_get_list_timeout(am, sess);
...
static int acl_fa_check_idle_sessions(acl_main_t *am, u16 thread_index, u64 now)
{
    ...
    if (pw->wip_session_change_requests) vec_set_len(pw->wip_session_change_requests, 0);

    {
        ...
        for (tt = 0; tt < ACL_N_TIMEOUTS; tt++) {
            ...
            while (n_expired < am->fa_max_deleted_sessions_per_interval) {
                ...
                if (!acl_fa_conn_time_to_check(am, pw, now, thread_index, pw->fa_conn_list_head[tt])) {
                    break;
                }
        ...
        for (tt = 0; tt < ACL_N_TIMEOUTS; tt++) {
            ...
            if (session_index == FA_SESSION_BOGUS_INDEX) break;
            fa_session_t *sess = get_session_ptr(am, thread_index, session_index);
    ...
    vec_foreach (psid, pw->expired) {
        ...
        if (!pool_is_free_index(pw->fa_sessions_pool, fsid.session_index)) {
            ...
            u64           sess_timeout_time  = sess->last_active_time + fa_session_get_timeout(am, sess);
            int           timeout_passed     = (now >= sess_timeout_time);
            int           clearing_interface = clib_bitmap_get(pw->pending_clear_sw_if_index_bitmap, sw_if_index);
            if (am->trace_sessions > 3) {
                elog_acl_maybe_trace_X2(am, "acl_fa_check_idle_sessions: now %lu sess_timeout_time %lu", "i8i8", now, sess_timeout_time);
    ...
    if (pw->expired) vec_set_len(pw->expired, 0);
    /* if we were advancing and reached the end
     * (no more sessions to recycle), reset the fast-forward timestamp */

    if (pw->swipe_end_time && 0 == total_expired) pw->swipe_end_time = 0;

    elog_acl_maybe_trace_X1(am, "acl_fa_check_idle_sessions: done, total sessions expired: %d", "i4", (u32) total_expired);
...
static void send_one_worker_interrupt(vlib_main_t *vm, acl_main_t *am, int thread_index)
{
    ...
    if (!pw->interrupt_is_pending) {
        ...
        vlib_node_set_interrupt_pending(vlib_get_main_by_index(thread_index), acl_fa_worker_session_cleaner_process_node.index);
        elog_acl_maybe_trace_X1(am, "send_one_worker_interrupt: send interrupt to worker %u", "i4", ((u32) thread_index));
...
{
    acl_fa_per_worker_data_t *pw_me = &am->per_worker_data[os_get_thread_index()];
    ...
    vec_add1(pw->pending_session_change_requests, (((u64) request_type) << 32) | target_session);
...
static int purgatory_has_connections(vlib_main_t *vm, acl_main_t *am, int thread_index)
{
    ...
    return (FA_SESSION_BOGUS_INDEX != pw->fa_conn_list_head[ACL_TIMEOUT_PURGATORY]);
}
...
static uword acl_fa_worker_conn_cleaner_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    if (pw->clear_in_process) {
        if (0 == pw->swipe_end_time) {
             ...
             */
            if ((pw->pending_clear_sw_if_index_bitmap == 0) || (pw->serviced_sw_if_index_bitmap == 0)) {
...
#endif
                pw->pending_clear_sw_if_index_bitmap = clib_bitmap_and(pw->pending_clear_sw_if_index_bitmap, pw->serviced_sw_if_index_bitmap);
            }
...
static uword acl_fa_session_cleaner_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    while (1) {
        ...
        for (ti = 0; ti < vlib_get_n_threads(); ti++) {
            ...
            for (tt = 0; tt < vec_len(pw->fa_conn_list_head); tt++) {
                ...
                if ((head_expiry < next_expire) && !pw->interrupt_is_pending) {
                    elog_acl_maybe_trace_X3(am, "acl_fa_session_cleaner_process: now %lu, worker: %u tt: %u", "i8i2i2", now, ti, tt);
                    elog_acl_maybe_trace_X2(am,
        ...
        switch (event_type) {
            ...
            case ACL_FA_CLEANER_DELETE_BY_SW_IF_INDEX: {
                ...
                vec_foreach (sw_if_index0, event_data) {
                    ...
                    elog_acl_maybe_trace_X1(am, "acl_fa_session_cleaner_process: ACL_FA_CLEANER_DELETE_BY_SW_IF_INDEX %u", "i4", *sw_if_index0);
                    if (*sw_if_index0 == ~0) {
                        clear_all = 1;
                    }
                    else {
                ...
                acl_log_info("ACL_FA_CLEANER_DELETE_BY_SW_IF_INDEX bitmap: %U, clear_all: %u",
                             format_bitmap_hex,
                ...
                vec_foreach (pw0, am->per_worker_data) {
                    ...
                    if (pw0->clear_in_process) {
                        acl_log_err("ERROR-BUG! Could not initiate cleaning on worker because another cleanup in progress");
                    }
                    else {
                        ...
                        acl_log_info("ACL_FA_CLEANER: thread %u, pending clear bitmap: %U",
                                     (am->per_worker_data - pw0),
                                     format_bitmap_hex,
                ...
                acl_log_info("CLEANER mains len: %u per-worker len: %d", vlib_get_n_threads(), vec_len(am->per_worker_data));
                vec_foreach (pw0, am->per_worker_data) {
                    CLIB_MEMORY_BARRIER();
                    while (pw0->clear_in_process) {
                        CLIB_MEMORY_BARRIER();
            ...
            default:
#ifdef FA_NODE_VERBOSE_DEBUG
                clib_warning("ACL plugin connection cleaner: unknown event %u", event_type);
#endif
                vlib_node_increment_counter(vm, acl_fa_session_cleaner_process_node.index, ACL_FA_CLEANER_ERROR_UNKNOWN_EVENT, 1);
        ...
        if (event_data) vec_set_len(event_data, 0);
...
void acl_fa_enable_disable(u32 sw_if_index, int is_input, int enable_disable)
{
    ...
    if (enable_disable) {
        ...
        vlib_process_signal_event(am->vlib_main, am->fa_cleaner_node_index, ACL_FA_CLEANER_RESCHEDULE, 0);
    }
    else {
        am->fa_total_enabled_count--;
    }
    ...
    if (is_input) {
        ...
        vnet_feature_enable_disable("ip4-unicast", "acl-plugin-in-ip4-fa", sw_if_index, enable_disable, 0, 0);
        vnet_feature_enable_disable("ip6-unicast", "acl-plugin-in-ip6-fa", sw_if_index, enable_disable, 0, 0);
        am->fa_in_acl_on_sw_if_index = clib_bitmap_set(am->fa_in_acl_on_sw_if_index, sw_if_index, enable_disable);
    }
    else {
        ASSERT(clib_bitmap_get(am->fa_out_acl_on_sw_if_index, sw_if_index) != enable_disable);
        vnet_feature_enable_disable("ip4-output", "acl-plugin-out-ip4-fa", sw_if_index, enable_disable, 0, 0);
        vnet_feature_enable_disable("ip6-output", "acl-plugin-out-ip6-fa", sw_if_index, enable_disable, 0, 0);
        am->fa_out_acl_on_sw_if_index = clib_bitmap_set(am->fa_out_acl_on_sw_if_index, sw_if_index, enable_disable);
    }
    if ((!enable_disable) && (!acl_fa_ifc_has_in_acl(am, sw_if_index)) && (!acl_fa_ifc_has_out_acl(am, sw_if_index))) {
#ifdef FA_NODE_VERBOSE_DEBUG
...
void show_fa_sessions_hash(vlib_main_t *vm, u32 verbose)
{
    acl_main_t *am = &acl_main;
    if (am->fa_sessions_hash_is_initialized) {
        vlib_cli_output(vm, "\nIPv6 Session lookup hash table:\n%U\n\n", format_bihash_40_8, &am->fa_ip6_sessions_hash, verbose);

        vlib_cli_output(vm, "\nIPv4 Session lookup hash table:\n%U\n\n", format_bihash_16_8, &am->fa_ip4_sessions_hash, verbose);
    }
    else {
