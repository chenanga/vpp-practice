...
static void vl_api_acl_plugin_control_ping_t_handler(vl_api_acl_plugin_control_ping_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_ACL_PLUGIN_CONTROL_PING_REPLY, ({ rmp->vpe_pid = ntohl(getpid()); }));
}
...
static inline u8 *format_acl_action(u8 *s, u8 action)
{
    switch (action) {
        case 0:
            s = format(s, "deny");
...
static void acl_print_acl_x(acl_vector_print_func_t vpr, vlib_main_t *vm, acl_main_t *am, int acl_index)
{
    ...
    u8         *out0      = format(0, "acl-index %u count %u tag {%s}\n", acl_index, vec_len(acl_rules), am->acls[acl_index].tag);
    int         j;
    vpr(vm, out0);
    for (j = 0; j < vec_len(acl_rules); j++) {
        ...
        out0 = format(out0, " src %U/%d", format_ip46_address, &r->src, r->is_ipv6 ? IP46_TYPE_IP6 : IP46_TYPE_IP4, r->src_prefixlen);
        out0 = format(out0, " dst %U/%d", format_ip46_address, &r->dst, r->is_ipv6 ? IP46_TYPE_IP6 : IP46_TYPE_IP4, r->dst_prefixlen);
...
static void vl_api_acl_plugin_get_conn_table_max_entries_t_handler(vl_api_acl_plugin_get_conn_table_max_entries_t *mp)
{
    ...
    rmp->_vl_msg_id             = ntohs(VL_API_ACL_PLUGIN_GET_CONN_TABLE_MAX_ENTRIES_REPLY + am->msg_id_base);
    rmp->context                = mp->context;
    rmp->conn_table_max_entries = clib_net_to_host_u64(am->fa_conn_table_max_entries);
...
{
    u32 **ppolicy_epoch_by_swi = is_input ? &am->input_policy_epoch_by_sw_if_index : &am->output_policy_epoch_by_sw_if_index;
    vec_validate(*ppolicy_epoch_by_swi, sw_if_index);

    u32 *p_epoch = vec_elt_at_index((*ppolicy_epoch_by_swi), sw_if_index);
    *p_epoch     = ((1 + *p_epoch) & FA_POLICY_EPOCH_MASK) + (is_input * FA_POLICY_EPOCH_IS_INPUT);
}

static void try_increment_acl_policy_epoch(acl_main_t *am, u32 acl_num, int is_input)
{
    u32 ***p_swi_vec_by_acl = is_input ? &am->input_sw_if_index_vec_by_acl : &am->output_sw_if_index_vec_by_acl;
...
static void validate_and_reset_acl_counters(acl_main_t *am, u32 acl_index)
{
    ...
    for (i = old_len; i < vec_len(am->combined_acl_counters); i++) {
        ...
        /* filled in once only */
        am->combined_acl_counters[i].stat_segment_name = (void *) format(0, "/acl/%d/matches%c", i, 0);
        i32 rule_count                                 = vec_len(am->acls[i].rules);
        /* Validate one extra so we always have at least one counter for an ACL */
        vlib_validate_combined_counter(&am->combined_acl_counters[i], rule_count);
    ...
    /* Validate one extra so we always have at least one counter for an ACL */
    vlib_validate_combined_counter(&am->combined_acl_counters[acl_index], rule_count);
...
static int acl_api_invalid_prefix(const vl_api_prefix_t *prefix)
{
    ...
    int         valid_af = prefix->address.af == ADDRESS_IP4 || prefix->address.af == ADDRESS_IP6;
...
static int acl_add_list(u32 count, vl_api_acl_rule_t rules[], u32 *acl_list_index, u8 *tag)
{
    ...
    if (tag_len == sizeof(a->tag)) return VNET_API_ERROR_INVALID_VALUE;

    if (am->trace_acl > 255) clib_warning("API dbg: acl_add_list index %d tag %s", *acl_list_index, tag);

    /* check if what they request is consistent */
    for (i = 0; i < count; i++) {
        if (acl_api_invalid_prefix(&rules[i].src_prefix)) return VNET_API_ERROR_INVALID_SRC_ADDRESS;
        if (acl_api_invalid_prefix(&rules[i].dst_prefix)) return VNET_API_ERROR_INVALID_DST_ADDRESS;
        if (rules[i].src_prefix.address.af != rules[i].dst_prefix.address.af) return VNET_API_ERROR_INVALID_SRC_ADDRESS;
        if (ntohs(rules[i].srcport_or_icmptype_first) > ntohs(rules[i].srcport_or_icmptype_last)) return VNET_API_ERROR_INVALID_VALUE_2;
        if (ntohs(rules[i].dstport_or_icmpcode_first) > ntohs(rules[i].dstport_or_icmpcode_last)) return VNET_API_ERROR_INVALID_VALUE_2;
    }
    ...
    if (count > 0) vec_validate(acl_new_rules, count - 1);
    ...
    if (~0 == *acl_list_index) {
        ...
        *acl_list_index = a - am->acls;
        ...
        if (a->rules) vec_free(a->rules);
    }
    a->rules = acl_new_rules;
    memcpy(a->tag, tag, tag_len + 1);
    if (am->trace_acl > 255) warning_acl_print_acl(am->vlib_main, am, *acl_list_index);
...
static int acl_del_list(u32 acl_list_index)
{
    ...
    if (acl_is_used_by(acl_list_index, am->input_sw_if_index_vec_by_acl)) return VNET_API_ERROR_ACL_IN_USE_INBOUND;
    if (acl_is_used_by(acl_list_index, am->output_sw_if_index_vec_by_acl)) return VNET_API_ERROR_ACL_IN_USE_OUTBOUND;
    /* lookup contexts cover other cases, not just inbound/outbound, so check that */
    if (acl_is_used_by(acl_list_index, am->lc_index_vec_by_acl)) return VNET_API_ERROR_ACL_IN_USE_BY_LOOKUP_CONTEXT;
                                            ...
                                            int                   is_add)
{
    ...
    int ret = vnet_classify_add_del_table(cm,
                                          skip_mask_ptr,
                                          nbuckets,
                                          memory_size,
                                          skip,
...
{
    u16 **v         = is_input ? am->input_etype_whitelist_by_sw_if_index : am->output_etype_whitelist_by_sw_if_index;
...
{
    vlib_process_signal_event(am->vlib_main, am->fa_cleaner_node_index, ACL_FA_CLEANER_DELETE_BY_SW_IF_INDEX, sw_if_index);
}
...
static int acl_interface_in_enable_disable(acl_main_t *am, u32 sw_if_index, int enable_disable)
{
    ...
    /* Utterly wrong? */
    if (pool_is_free_index(am->vnet_main->interface_main.sw_interfaces, sw_if_index)) return VNET_API_ERROR_INVALID_SW_IF_INDEX;

    if (clib_bitmap_get(am->in_acl_on_sw_if_index, sw_if_index) == enable_disable) return 0;

    acl_fa_enable_disable(sw_if_index, 1, enable_disable);

    rv = vnet_l2_feature_enable_disable("l2-input-ip4", "acl-plugin-in-ip4-l2", sw_if_index, enable_disable, 0, 0);
    if (rv) clib_error("Could not enable on input");
    rv = vnet_l2_feature_enable_disable("l2-input-ip6", "acl-plugin-in-ip6-l2", sw_if_index, enable_disable, 0, 0);
    if (rv) clib_error("Could not enable on input");

    if (intf_has_etype_whitelist(am, sw_if_index, 1))
        vnet_l2_feature_enable_disable("l2-input-nonip", "acl-plugin-in-nonip-l2", sw_if_index, enable_disable, 0, 0);
    am->in_acl_on_sw_if_index = clib_bitmap_set(am->in_acl_on_sw_if_index, sw_if_index, enable_disable);
...
static int acl_interface_out_enable_disable(acl_main_t *am, u32 sw_if_index, int enable_disable)
{
    ...
    if (clib_bitmap_get(am->out_acl_on_sw_if_index, sw_if_index) == enable_disable) return 0;

    acl_fa_enable_disable(sw_if_index, 0, enable_disable);

    rv = vnet_l2_feature_enable_disable("l2-output-ip4", "acl-plugin-out-ip4-l2", sw_if_index, enable_disable, 0, 0);
    if (rv) clib_error("Could not enable on output");
    rv = vnet_l2_feature_enable_disable("l2-output-ip6", "acl-plugin-out-ip6-l2", sw_if_index, enable_disable, 0, 0);
    if (rv) clib_error("Could not enable on output");
    if (intf_has_etype_whitelist(am, sw_if_index, 0))
        vnet_l2_feature_enable_disable("l2-output-nonip", "acl-plugin-out-nonip-l2", sw_if_index, enable_disable, 0, 0);
    am->out_acl_on_sw_if_index = clib_bitmap_set(am->out_acl_on_sw_if_index, sw_if_index, enable_disable);
...
static int acl_interface_set_inout_acl_list(acl_main_t *am, u32 sw_if_index, u8 is_input, u32 *vec_acl_list_index, int *may_clear_sessions)
{
    ...
    if (am->trace_acl > 255)
        clib_warning("API dbg: acl_interface_set_inout_acl_list: sw_if_index %d is_input %d acl_vec: [%U]",
                     sw_if_index,
                     is_input,
                     format_vec32,
                     vec_acl_list_index,
    ...
    u32 **pinout_lc_index_by_sw_if_index = is_input ? &am->input_lc_index_by_sw_if_index : &am->output_lc_index_by_sw_if_index;

    u32 ***pinout_acl_vec_by_sw_if_index = is_input ? &am->input_acl_vec_by_sw_if_index : &am->output_acl_vec_by_sw_if_index;

    u32 ***pinout_sw_if_index_vec_by_acl = is_input ? &am->input_sw_if_index_vec_by_acl : &am->output_sw_if_index_vec_by_acl;
    ...
    (*pinout_acl_vec_by_sw_if_index)[sw_if_index] = vec_dup(vec_acl_list_index);
    ...
    if (am->reclassify_sessions) {
        ...
        if (may_clear_sessions && *may_clear_sessions && !clib_bitmap_is_zero(change_acl_bitmap)) {
    ...
    acl_interface_inout_enable_disable(am, sw_if_index, is_input, vec_len(vec_acl_list_index) > 0);
...
{
    acl_main_t *am = &acl_main;
    acl_interface_set_inout_acl_list(am, sw_if_index, is_input, 0, may_clear_sessions);
}
...
static int acl_interface_add_del_inout_acl(u32 sw_if_index, u8 is_add, u8 is_input, u32 acl_list_index)
{
    ...
    int error_already_applied = is_input ? VNET_API_ERROR_ACL_IN_USE_INBOUND : VNET_API_ERROR_ACL_IN_USE_OUTBOUND;

    u32 ***pinout_acl_vec_by_sw_if_index = is_input ? &am->input_acl_vec_by_sw_if_index : &am->output_acl_vec_by_sw_if_index;
    int    rv                            = 0;
    if (is_add) {
        vec_validate((*pinout_acl_vec_by_sw_if_index), sw_if_index);
        u32 index = vec_search((*pinout_acl_vec_by_sw_if_index)[sw_if_index], acl_list_index);
    ...
    rv = acl_interface_set_inout_acl_list(am, sw_if_index, is_input, acl_vec, &may_clear_sessions);
done:
    vec_free(acl_vec);
    return rv;
}
...
{
    if (is6)
        return (sizeof(ethernet_header_t) + offsetof(ip6_header_t, dst_address));
    else
        return (sizeof(ethernet_header_t) + offsetof(ip4_header_t, dst_address));
}
...
static int macip_create_classify_tables(acl_main_t *am, u32 macip_acl_index)
{
    ...
    for (i = 0; i < a->count; i++) {
        if (~0 == (match_type_index = macip_find_match_type(mvec, a->rules[i].src_mac_mask, a->rules[i].src_prefixlen, a->rules[i].is_ipv6))) {
            ...
            memcpy(mvec[match_type_index].mac_mask, a->rules[i].src_mac_mask, 6);
        ...
        mvec[match_type_index].has_egress |= macip_permit_also_egress(a->rules[i].is_permit);
    }
    ...
    vec_foreach (mt, mvec) {
        ...
        if (!is6) {
            ...
            for (tags = 2; tags >= 0; tags--) {
                ...
                if (mt->prefix_len % 8) mask[l3_offset + 14 + (mt->prefix_len / 8)] = 0xff - ((1 << (8 - mt->prefix_len % 8)) - 1);

                mask_len = ((l3_offset + 14 + ((mt->prefix_len + 7) / 8) + (sizeof(u32x4) - 1)) / sizeof(u32x4)) * sizeof(u32x4);
                acl_classify_add_del_table_small(cm, mask, mask_len, last_table, (~0 == last_table) ? 0 : ~0, last_tag_table, 1);
                ...
                if (mt->has_egress) {
                    ...
                    mask_len = ((l3_offset + (sizeof(u32x4) - 1)) / sizeof(u32x4)) * sizeof(u32x4);
                    acl_classify_add_del_table_small(cm, mask, mask_len, out_last_table, (~0 == out_last_table) ? 0 : ~0, out_last_tag_table, 1);
    ...
    vec_foreach (mt, mvec) {
        ...
        for (tags = 2; tags >= 0; tags--) {
             ...
             */
            mask_len = ((l3_src_offs + ((mt->prefix_len + 7) / 8) + (sizeof(u32x4) - 1)) / sizeof(u32x4)) * sizeof(u32x4);
            acl_classify_add_del_table_small(cm, mask, mask_len, last_table, (~0 == last_table) ? 0 : ~0, last_tag_table, 1);
            last_table = *last_tag_table;
        }
        if (mt->has_egress) {
            for (tags = 2; tags >= 0; tags--) {
                ...
                if (mt->prefix_len % 8) {
                    mask[l3_dst_offs + (mt->prefix_len / 8)] = 0xff - ((1 << (8 - mt->prefix_len % 8)) - 1);
                }
                /*
                 * Round-up the number of bytes needed to store the prefix,
                 * and round up the number of vectors too
                 */
                mask_len = ((l3_dst_offs + ((mt->prefix_len + 7) / 8) + (sizeof(u32x4) - 1)) / sizeof(u32x4)) * sizeof(u32x4);
                acl_classify_add_del_table_small(cm, mask, mask_len, out_last_table, (~0 == out_last_table) ? 0 : ~0, out_last_tag_table, 1);
                out_last_table = *out_last_tag_table;
            }
        }
    ...
    for (i = 0; i < a->count; i++) {
        ...
        for (tags = 2; tags >= 0; tags--) {
            ...
            vnet_classify_add_del_session(cm, tag_table, mask, a->rules[i].is_permit ? ~0 : 0, i, 0, action, metadata, 1);
        ...
        if (!is6 && (mvec[match_type_index].arp_table_index != ~0)) {
            ...
            for (tags = 2; tags >= 0; tags--) {
                ...
                memcpy(&mask[l3_src_offs + 14], &a->rules[i].src_ip_addr.ip4, 4);
                vnet_classify_add_del_session(cm, tag_table, mask, a->rules[i].is_permit ? ~0 : 0, i, 0, action, metadata, 1);
            }
        }
        if (macip_permit_also_egress(a->rules[i].is_permit)) {
            ...
            for (tags = 2; tags >= 0; tags--) {
                ...
                if (is6) {
                    memcpy(&mask[l3_dst_offs], &a->rules[i].src_ip_addr.ip6, 16);
                    mask[eth]     = 0x86;
                    mask[eth + 1] = 0xdd;
                }
                else {
                    memcpy(&mask[l3_dst_offs], &a->rules[i].src_ip_addr.ip4, 4);
                    mask[eth]     = 0x08;
                    mask[eth + 1] = 0x00;
                }
                ...
                for (tags = 2; tags >= 0; tags--) {
                    ...
                    switch (tags) {
                        ...
                        default:
                            ...
                            break;
                        case 1:
                            tag_table = mvec[match_type_index].out_arp_dot1q_table_index;
                            mask[12]  = 0x81;
                            mask[13]  = 0x00;
                            mask[16]  = 0x08;
                            mask[17]  = 0x06;
                            break;
                        case 2:
                            tag_table = mvec[match_type_index].out_arp_dot1ad_table_index;
                            mask[12]  = 0x88;
                            mask[13]  = 0xa8;
                            mask[16]  = 0x81;
                            mask[17]  = 0x00;
...
static void macip_destroy_classify_tables(acl_main_t *am, u32 macip_acl_index)
{
    ...
    if (a->out_ip4_table_index != ~0) {
        acl_classify_add_del_table_small(cm, 0, ~0, ~0, ~0, &a->out_ip4_table_index, 0);
        a->out_ip4_table_index = ~0;
    }
    if (a->out_ip6_table_index != ~0) {
        acl_classify_add_del_table_small(cm, 0, ~0, ~0, ~0, &a->out_ip6_table_index, 0);
        a->out_ip6_table_index = ~0;
    }
    if (a->out_l2_table_index != ~0) {
        acl_classify_add_del_table_small(cm, 0, ~0, ~0, ~0, &a->out_l2_table_index, 0);
...
static int macip_maybe_apply_unapply_classifier_tables(acl_main_t *am, u32 acl_index, int is_apply)
{
    ...
    for (i = 0; i < vec_len(am->macip_acl_by_sw_if_index); i++)
        if (vec_elt(am->macip_acl_by_sw_if_index, i) == acl_index) {
            rv0 = vnet_set_input_acl_intfc(am->vlib_main, i, a->ip4_table_index, a->ip6_table_index, a->l2_table_index, is_apply);
            /* return the first unhappy outcome but make try to plough through. */
            rv  = rv || rv0;
            rv0 = vnet_set_output_acl_intfc(am->vlib_main, i, a->out_ip4_table_index, a->out_ip6_table_index, a->out_l2_table_index, is_apply);
            /* return the first unhappy outcome but make try to plough through. */
            rv = rv || rv0;
        }
    return rv;
...
static int macip_acl_add_list(u32 count, vl_api_macip_acl_rule_t rules[], u32 *acl_list_index, u8 *tag)
{
    ...
    if (~0 != *acl_list_index) rv = macip_maybe_apply_unapply_classifier_tables(am, *acl_list_index, 0);
    /* Create and populate the rules */
    if (count > 0) vec_validate(acl_new_rules, count - 1);
    ...
    for (i = 0; i < count; i++) {
        ...
        mac_address_decode(rules[i].src_mac_mask, (mac_address_t *) &r->src_mac_mask);
...
static int macip_acl_interface_del_acl(acl_main_t *am, u32 sw_if_index)
{
    ...
    if (sw_if_index >= vec_len(am->macip_acl_by_sw_if_index)) return VNET_API_ERROR_NO_SUCH_ENTRY;

    macip_acl_index = am->macip_acl_by_sw_if_index[sw_if_index];
    /* No point in deleting MACIP ACL which is not applied */
    if (~0 == macip_acl_index) return VNET_API_ERROR_NO_SUCH_ENTRY;

    a = pool_elt_at_index(am->macip_acls, macip_acl_index);
    /* remove the classifier tables off the interface L2 ACL */
    rv = vnet_set_input_acl_intfc(am->vlib_main, sw_if_index, a->ip4_table_index, a->ip6_table_index, a->l2_table_index, 0);
    rv |= vnet_set_output_acl_intfc(am->vlib_main, sw_if_index, a->out_ip4_table_index, a->out_ip6_table_index, a->out_l2_table_index, 0);
    ...
    u32 index = vec_search(am->sw_if_index_vec_by_macip_acl[macip_acl_index], sw_if_index);
    if (index != ~0) vec_del1(am->sw_if_index_vec_by_macip_acl[macip_acl_index], index);
    return rv;
}
...
static int macip_acl_interface_add_acl(acl_main_t *am, u32 sw_if_index, u32 macip_acl_index)
{
    ...
    if (~0 != am->macip_acl_by_sw_if_index[sw_if_index]) macip_acl_interface_del_acl(am, sw_if_index);
    ...
    rv |= vnet_set_output_acl_intfc(am->vlib_main, sw_if_index, a->out_ip4_table_index, a->out_ip6_table_index, a->out_l2_table_index, 1);
    return rv;
}
...
static int verify_message_len(void *mp, u64 expected_len, char *where)
{
    ...
    if (supplied_len < expected_len) {
        clib_warning("%s: Supplied message length %d is less than expected %d", where, supplied_len, expected_len);
        return 0;
    }
    else {
        return 1;
...
static void vl_api_acl_add_replace_t_handler(vl_api_acl_add_replace_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_ACL_ADD_REPLACE_REPLY, ({ rmp->acl_index = htonl(acl_list_index); }));
}
...
static void vl_api_acl_interface_add_del_t_handler(vl_api_acl_interface_add_del_t *mp)
{
    ...
    if (pool_is_free_index(im->sw_interfaces, sw_if_index))
        ...
        rv = acl_interface_add_del_inout_acl(sw_if_index, mp->is_add, mp->is_input, ntohl(mp->acl_index));
...
static void copy_acl_rule_to_api_rule(vl_api_acl_rule_t *api_rule, acl_rule_t *r)
{
    ...
    ip_address_encode(&r->src, r->is_ipv6 ? IP46_TYPE_IP6 : IP46_TYPE_IP4, &api_rule->src_prefix.address);
    ip_address_encode(&r->dst, r->is_ipv6 ? IP46_TYPE_IP6 : IP46_TYPE_IP4, &api_rule->dst_prefix.address);
...
static void vl_api_acl_interface_list_dump_t_handler(vl_api_acl_interface_list_dump_t *mp)
{
    ...
    if (mp->sw_if_index == ~0) {
        ...
        if (!pool_is_free_index(im->sw_interfaces, sw_if_index)) send_acl_interface_list_details(am, reg, sw_if_index, mp->context);
    }
}
...
static void vl_api_macip_acl_add_t_handler(vl_api_macip_acl_add_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_MACIP_ACL_ADD_REPLY, ({ rmp->acl_index = htonl(acl_list_index); }));
}
...
static void vl_api_macip_acl_add_replace_t_handler(vl_api_macip_acl_add_replace_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_MACIP_ACL_ADD_REPLACE_REPLY, ({ rmp->acl_index = htonl(acl_list_index); }));
}
...
static void send_macip_acl_details(acl_main_t *am, vl_api_registration_t *reg, macip_acl_list_t *acl, u32 context)
{
    ...
    if (acl) {
        ...
        for (i = 0; i < acl->count; i++) {
            ...
            mac_address_encode((mac_address_t *) &r->src_mac, rules[i].src_mac);
            mac_address_encode((mac_address_t *) &r->src_mac_mask, rules[i].src_mac_mask);
            ip_address_encode(&r->src_ip_addr, r->is_ipv6 ? IP46_TYPE_IP6 : IP46_TYPE_IP4, &rules[i].src_prefix.address);
    ...
    if (sw_if_index == ~0) {
        vec_foreach_index (sw_if_index, am->macip_acl_by_sw_if_index) {
            if (~0 != am->macip_acl_by_sw_if_index[sw_if_index]) {
                send_macip_acl_interface_list_details(am, reg, sw_if_index, am->macip_acl_by_sw_if_index[sw_if_index], mp->context);
            }
        }
    }
    else {
...
static void send_acl_interface_etype_whitelist_details(acl_main_t *am, vl_api_registration_t *reg, u32 sw_if_index, u32 context)
{
    ...
    if (intf_has_etype_whitelist(am, sw_if_index, 0)) whitelist_out = vec_elt(am->output_etype_whitelist_by_sw_if_index, sw_if_index);

    if (intf_has_etype_whitelist(am, sw_if_index, 1)) whitelist_in = vec_elt(am->input_etype_whitelist_by_sw_if_index, sw_if_index);

    if ((0 == whitelist_in) && (0 == whitelist_out)) return; /* nothing to do */
    ...
    mp = vl_msg_api_alloc(msg_size);
    clib_memset(mp, 0, msg_size);
    mp->_vl_msg_id = ntohs(VL_API_ACL_INTERFACE_ETYPE_WHITELIST_DETAILS + am->msg_id_base);
...
static void vl_api_acl_interface_etype_whitelist_dump_t_handler(vl_api_acl_interface_list_dump_t *mp)
{
    ...
    if (mp->sw_if_index == ~0) {
        ...
        sw_if_index = ntohl(mp->sw_if_index);
        if (!pool_is_free_index(im->sw_interfaces, sw_if_index)) send_acl_interface_etype_whitelist_details(am, reg, sw_if_index, mp->context);
    }
}
...
static void vl_api_acl_plugin_use_hash_lookup_get_t_handler(vl_api_acl_plugin_use_hash_lookup_get_t *mp)
{
    ...
    rmp = vl_msg_api_alloc(msg_size);
    clib_memset(rmp, 0, msg_size);
    rmp->_vl_msg_id = ntohs(VL_API_ACL_PLUGIN_USE_HASH_LOOKUP_GET_REPLY + am->msg_id_base);
    rmp->context    = mp->context;
...
static clib_error_t *acl_set_aclplugin_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (unformat(input, "event-trace")) {
        if (!unformat(input, "%u", &val)) {
            error = clib_error_return(0, "expecting trace level, got `%U`", format_unformat_error, input);
            goto done;
        }
        else {
            am->trace_acl = val;
    ...
    if (unformat(input, "session")) {
        if (unformat(input, "table")) {
            ...
            if (unformat(input, "max-entries")) {
                if (!unformat(input, "%u", &val)) {
                    error = clib_error_return(0, "expecting maximum number of entries, got `%U`", format_unformat_error, input);
                    goto done;
                }
                else {
            ...
            if (unformat(input, "hash-table-buckets")) {
                if (!unformat(input, "%u", &val)) {
                    error = clib_error_return(0, "expecting maximum number of hash table buckets, got `%U`", format_unformat_error, input);
                    goto done;
                }
                else {
            ...
            if (unformat(input, "hash-table-memory")) {
                if (!unformat(input, "%U", unformat_memory_size, &memory_size)) {
                    error = clib_error_return(0, "expecting maximum amount of hash table memory, got `%U`", format_unformat_error, input);
                    goto done;
                }
                else {
            ...
            if (unformat(input, "udp")) {
                if (unformat(input, "idle")) {
                    if (!unformat(input, "%u", &timeout)) {
                        error = clib_error_return(0, "expecting timeout value in seconds, got `%U`", format_unformat_error, input);
                        goto done;
                    }
                    else {
...
static inline u8 *my_macip_acl_rule_t_pretty_format(u8 *out, va_list *args)
{
...
static clib_error_t *acl_set_aclplugin_interface_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (~0 == sw_if_index) return (clib_error_return(0, "invalid interface"));
    if (~0 == acl_index) return (clib_error_return(0, "invalid acl"));
...
static clib_error_t *acl_set_aclplugin_acl_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(line_input, "tcpflags %d mask %d", &tcpflags, &tcpmask)) {
            vec_validate_acl_rules(rules, rule_idx);
            rules[rule_idx].tcp_flags_value = tcpflags;
            rules[rule_idx].tcp_flags_mask  = tcpmask;
        }
    ...
    unformat_free(line_input);
    if (rv) return (clib_error_return(0, "failed"));
...
static clib_error_t *acl_delete_aclplugin_acl_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    int              rv;
    u32              acl_index = ~0;
...
static clib_error_t *acl_show_aclplugin_macip_acl_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (i = 0; i < vec_len(am->macip_acls); i++) {
        ...
        if (pool_is_free_index(am->macip_acls, i)) continue;
...
static clib_error_t *acl_set_aclplugin_macip_acl_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        ...
        if (unformat(line_input, "permit")) {
            ...
            memcpy(rules[rule_idx].src_mac, &src_mac, sizeof(rules[rule_idx].src_mac));
            memcpy(rules[rule_idx].src_mac_mask, &mac_mask_all_1, sizeof(rules[rule_idx].src_mac_mask));
        }
        else if (unformat(line_input, "mask %U", unformat_mac_address, &src_mac)) {
            memcpy(rules[rule_idx].src_mac_mask, &src_mac, sizeof(rules[rule_idx].src_mac_mask));
        }
        else if (unformat(line_input, "tag %s", &tag))
            ;
        else if (unformat(line_input, ",")) {
    ...
    unformat_free(line_input);
    if (rv) return clib_error_return(0, "Failed to set MACIP ACL rule");

    vlib_cli_output(vm, "ACL index:%u", acl_index);
    return 0;
}

static clib_error_t *acl_macip_delete_aclplugin_acl_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    int              rv;
    ...
    if (macip_acl_index == ~0) return (clib_error_return(0, "invalid acl index"));

    rv = macip_acl_del_list(macip_acl_index);

    unformat_free(line_input);
    if (rv) return (clib_error_return(0, "Failed to delete ACL index"));

    vlib_cli_output(vm, "Deleted ACL index:%u", macip_acl_index);
    return 0;
}

static clib_error_t *acl_set_aclplugin_macip_interface_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    int              rv          = 0;
    u32              sw_if_index = ~0;
    u32              acl_index   = ~0;
    ...
    if (sw_if_index == ~0) return (clib_error_return(0, "invalid interface"));

    if (acl_index == ~0) return (clib_error_return(0, "invalid acl index"));

    rv = macip_acl_interface_add_del_acl(sw_if_index, is_add, acl_index);

    if (rv) return (clib_error_return(0, "Failed to add acl rule to interface"));
...
static void acl_plugin_show_interface(acl_main_t *am, u32 sw_if_index, int show_acl, int detail)
{
    ...
    u32          swi;
    u32         *pj;
    for (swi = 0; (swi < vec_len(am->input_acl_vec_by_sw_if_index)) || (swi < vec_len(am->output_acl_vec_by_sw_if_index)); swi++) {
        /* if we need a particular interface, skip all the others */
        if ((sw_if_index != ~0) && (sw_if_index != swi)) continue;

        vlib_cli_output(vm, "sw_if_index %d:\n", swi);
        if (swi < vec_len(am->input_policy_epoch_by_sw_if_index))
            vlib_cli_output(vm, "   input policy epoch: %x\n", vec_elt(am->input_policy_epoch_by_sw_if_index, swi));
        if (swi < vec_len(am->output_policy_epoch_by_sw_if_index))
            vlib_cli_output(vm, "   output policy epoch: %x\n", vec_elt(am->output_policy_epoch_by_sw_if_index, swi));

        if (intf_has_etype_whitelist(am, swi, 1)) {
            vlib_cli_output(vm, "  input etype whitelist: %U", format_vec16, am->input_etype_whitelist_by_sw_if_index[swi], "%04x");
        }
        if (intf_has_etype_whitelist(am, swi, 0)) {
            vlib_cli_output(vm, " output etype whitelist: %U", format_vec16, am->output_etype_whitelist_by_sw_if_index[swi], "%04x");
        }

        if ((swi < vec_len(am->input_acl_vec_by_sw_if_index)) && (vec_len(am->input_acl_vec_by_sw_if_index[swi]) > 0)) {
        ...
        if ((swi < vec_len(am->output_acl_vec_by_sw_if_index)) && (vec_len(am->output_acl_vec_by_sw_if_index[swi]) > 0)) {
        ...
        if (detail && (swi < vec_len(am->input_lc_index_by_sw_if_index))) {
            vlib_cli_output(vm, "   input lookup context index: %d", am->input_lc_index_by_sw_if_index[swi]);
        }
        if (detail && (swi < vec_len(am->output_lc_index_by_sw_if_index))) {
            vlib_cli_output(vm, "  output lookup context index: %d", am->output_lc_index_by_sw_if_index[swi]);
        }
    }
}
...
static void acl_plugin_show_sessions(acl_main_t *am, u32 show_session_thread_id, u32 show_session_session_index)
{
...

    {
        ...
        vlib_cli_output(vm, "Sessions active: add %lu - deact %lu = %lu", n_adds, n_deact, n_adds - n_deact);
        vlib_cli_output(vm, "Sessions being purged: deact %lu - del %lu = %lu", n_deact, n_dels, n_deact - n_dels);
    }
    vlib_cli_output(vm, "now: %lu clocks per second: %lu", now, clocks_per_second);
    vlib_cli_output(vm, "\n\nPer-thread data:");
    for (wk = 0; wk < vec_len(am->per_worker_data); wk++) {
        ...
        pool_foreach (swif, im->sw_interfaces) {
            ...
            u64 n_epoch_changes =
                (sw_if_index < vec_len(pw->fa_session_epoch_change_by_sw_if_index) ? pw->fa_session_epoch_change_by_sw_if_index[sw_if_index] : 0);
            vlib_cli_output(vm,
        ...
        vlib_cli_output(vm, "  received session change requests: %d", pw->rcvd_session_change_requests);
        vlib_cli_output(vm, "  sent session change requests: %d", pw->sent_session_change_requests);
    }
...
#undef _
    ...
    vlib_cli_output(vm,
                    ...
                    ((f64) am->fa_current_cleaner_timer_wait_interval) * 1000.0 / (f64) vm->clib_time.clocks_per_second);
...
static clib_error_t *acl_show_aclplugin_sessions_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    (void) unformat(input, "thread %u index %u", &show_session_thread_id, &show_session_session_index);
    (void) unformat(input, "verbose %u", &show_bihash_verbose);

    acl_plugin_show_sessions(am, show_session_thread_id, show_session_session_index);
...
static clib_error_t *acl_show_aclplugin_tables_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vlib_cli_output(vm, "Stats counters enabled for interface ACLs: %d", acl_main.interface_acl_counters_enabled);
    vlib_cli_output(vm, "Use hash-based lookup for ACLs: %d", acl_main.use_hash_acl_matching);
    if (show_mask_type) acl_plugin_show_tables_mask_type();
    if (show_acl_hash_info) acl_plugin_show_tables_acl_hash_info(acl_index);
    if (show_applied_info) acl_plugin_show_tables_applied_info(lc_index);
    if (show_bihash) acl_plugin_show_tables_bihash(show_bihash_verbose);
...
static clib_error_t *acl_clear_aclplugin_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    clib_error_t *error = 0;
    acl_main_t   *am    = &acl_main;
    vlib_process_signal_event(am->vlib_main, am->fa_cleaner_node_index, ACL_FA_CLEANER_DELETE_BY_SW_IF_INDEX, ~0);
    return error;
}
...
VLIB_CLI_COMMAND(aclplugin_macip_set_acl_command, static) = {
    ...
    .short_help =
        "set acl-plugin macip acl <permit|deny|action N> "
        "ip <PREFIX> mac <MAC> mask <int> [tag FOO] {use comma "
        "separated list for multiple rules}",
...
static clib_error_t *acl_plugin_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(input, "connection hash memory %U", unformat_memory_size, &conn_table_hash_memory_size))
            am->fa_conn_table_hash_memory_size = conn_table_hash_memory_size;
        else if (unformat(input, "connection count max %d", &conn_table_max_entries))
            am->fa_conn_table_max_entries = conn_table_max_entries;
        else if (unformat(input, "main heap size %U", unformat_memory_size, &main_heap_size))
            clib_warning("WARNING: ACL heap is now part of the main heap. 'main heap size' is ineffective.");
        else if (unformat(input, "hash lookup heap size %U", unformat_memory_size, &hash_heap_size))
            clib_warning("WARNING: ACL heap is now part of the main heap. 'hash lookup heap size' is ineffective.");
        ...
        else if (unformat(input, "hash lookup hash memory %U", unformat_memory_size, &hash_lookup_hash_memory))
        ...
        else if (unformat(input, "tuple merge split threshold %d", &tuple_merge_split_threshold))
            am->tuple_merge_split_threshold = tuple_merge_split_threshold;

        else if (unformat(input, "reclassify sessions %d", &reclassify_sessions))
...
static clib_error_t *acl_init(vlib_main_t *vm)
{
    ...
    vec_validate(am->per_worker_data, tm->n_vlib_mains - 1);
    {
        ...
        for (wk = 0; wk < vec_len(am->per_worker_data); wk++) {
            ...
            vec_validate(pw->expired, ACL_N_TIMEOUTS * am->fa_max_deleted_sessions_per_interval);
            vec_set_len(pw->expired, 0);
            vec_validate_init_empty(pw->fa_conn_list_head, ACL_N_TIMEOUTS - 1, FA_SESSION_BOGUS_INDEX);
            vec_validate_init_empty(pw->fa_conn_list_tail, ACL_N_TIMEOUTS - 1, FA_SESSION_BOGUS_INDEX);
            vec_validate_init_empty(pw->fa_conn_list_head_expiry_time, ACL_N_TIMEOUTS - 1, ~0ULL);
        }
    }
...
#undef _
    ...
    am->interface_acl_user_id = acl_plugin.register_user_module("interface ACL", "sw_if_index", "is_input");

    am->acl_counter_lock    = clib_mem_alloc_aligned(CLIB_CACHE_LINE_BYTES, CLIB_CACHE_LINE_BYTES);
