...
static uword my_unformat_mac_address(unformat_input_t *input, va_list *args)
{
    u8 *a = va_arg(*args, u8 *);
    return unformat(input, "%x:%x:%x:%x:%x:%x", &a[0], &a[1], &a[2], &a[3], &a[4], &a[5]);
}
...
static void vl_api_acl_plugin_get_version_reply_t_handler(vl_api_acl_plugin_get_version_reply_t *mp)
{
    vat_main_t *vam = acl_test_main.vat_main;
...
static void vl_api_acl_plugin_use_hash_lookup_get_reply_t_handler(vl_api_acl_plugin_use_hash_lookup_get_reply_t *mp)
{
    vat_main_t *vam = acl_test_main.vat_main;
...
static void vl_api_acl_interface_list_details_t_handler(vl_api_acl_interface_list_details_t *mp)
{
    int         i;
    vat_main_t *vam = acl_test_main.vat_main;
    u8         *out = 0;
    ...
    for (i = 0; i < mp->count; i++) {
        if (i == mp->n_input) out = format(out, "\n  output ");
...
static void vl_api_macip_acl_interface_list_details_t_handler(vl_api_macip_acl_interface_list_details_t *mp)
{
    // NOT YET IMPLEMENTED
}

static void vl_api_acl_interface_etype_whitelist_details_t_handler(vl_api_acl_interface_etype_whitelist_details_t *mp)
{
    int         i;
    vat_main_t *vam = acl_test_main.vat_main;
    u8         *out = 0;
    vl_api_acl_interface_etype_whitelist_details_t_endian(mp, 0 /* from network */);
    out = format(out, "sw_if_index: %d, count: %d, n_input: %d\n", mp->sw_if_index, mp->count, mp->n_input);
    out = format(out, "   input ");
    for (i = 0; i < mp->count; i++) {
        if (i == mp->n_input) out = format(out, "\n  output ");
...
static void vl_api_acl_plugin_get_conn_table_max_entries_reply_t_handler(vl_api_acl_plugin_get_conn_table_max_entries_reply_t *mp)
{
    vat_main_t *vam = acl_test_main.vat_main;
    clib_warning("\nConn table max entries: %d", clib_net_to_host_u64(mp->conn_table_max_entries));
    vam->result_ready = 1;
}

static inline u8 *vl_api_acl_rule_t_pretty_format(u8 *out, vl_api_acl_rule_t *a)
{
...
static void vl_api_acl_details_t_handler(vl_api_acl_details_t *mp)
{
    int         i;
    vat_main_t *vam = acl_test_main.vat_main;
...
static inline u8 *vl_api_macip_acl_rule_t_pretty_format(u8 *out, vl_api_macip_acl_rule_t *a)
{
    int af = a->src_prefix.address.af ? AF_INET6 : AF_INET;
    u8  src[INET6_ADDRSTRLEN];
    inet_ntop(af, &a->src_prefix.address.un, (void *) src, sizeof(src));
    ...
    out = format(out,
                 "%s action %d ip %s/%d mac %U mask %U",
                 a->src_prefix.address.af ? "ipv6" : "ipv4",
                 a->is_permit,
                 src,
                 a->src_prefix.len,
...
static void vl_api_macip_acl_details_t_handler(vl_api_macip_acl_details_t *mp)
{
    int         i;
    vat_main_t *vam = acl_test_main.vat_main;
...
static void vl_api_macip_acl_interface_get_reply_t_handler(vl_api_macip_acl_interface_get_reply_t *mp)
{
    int         i;
    vat_main_t *vam = acl_test_main.vat_main;
...
static void vl_api_acl_plugin_control_ping_reply_t_handler(vl_api_acl_plugin_control_ping_reply_t *mp)
{
...
static int api_acl_add_replace(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(i, "src %U/%d", unformat_ip4_address, &src_v4address, &src_prefix_length)) {
            vec_validate_acl_rules(rules, rule_idx);
        ...
        else if (unformat(i, "src %U/%d", unformat_ip6_address, &src_v6address, &src_prefix_length)) {
            vec_validate_acl_rules(rules, rule_idx);
            memcpy(rules[rule_idx].src_prefix.address.un.ip6, &src_v6address, 16);
            rules[rule_idx].src_prefix.address.af = ADDRESS_IP6;
            rules[rule_idx].src_prefix.len        = src_prefix_length;
        }
        else if (unformat(i, "dst %U/%d", unformat_ip4_address, &dst_v4address, &dst_prefix_length)) {
            vec_validate_acl_rules(rules, rule_idx);
        ...
        else if (unformat(i, "dst %U/%d", unformat_ip6_address, &dst_v6address, &dst_prefix_length)) {
            vec_validate_acl_rules(rules, rule_idx);
    ...
    if (n_rules_override >= 0) n_rules = n_rules_override;
    ...
    mp->client_index = vam->my_client_index;
    if ((n_rules > 0) && rules) clib_memcpy(mp->r, rules, n_rules * sizeof(vl_api_acl_rule_t));
    if (tag) {
...
static int api_acl_plugin_use_hash_lookup_set(vat_main_t *vam)
{
    acl_test_main_t                         *sm = &acl_test_main;
    unformat_input_t                        *i  = vam->input;
    ...
    mp                = vl_msg_api_alloc_as_if_client(msg_size);
    memset(mp, 0, msg_size);
    mp->_vl_msg_id   = ntohs(VL_API_ACL_PLUGIN_USE_HASH_LOOKUP_SET + sm->msg_id_base);
    mp->client_index = vam->my_client_index;
    mp->enable       = 1;
...
static int api_acl_plugin_use_hash_lookup_get(vat_main_t *vam)
{
    acl_test_main_t                         *sm = &acl_test_main;
    vl_api_acl_plugin_use_hash_lookup_set_t *mp;
    u32                                      msg_size = sizeof(*mp);
    ...
    mp                = vl_msg_api_alloc_as_if_client(msg_size);
    memset(mp, 0, msg_size);
    mp->_vl_msg_id   = ntohs(VL_API_ACL_PLUGIN_USE_HASH_LOOKUP_GET + sm->msg_id_base);
    mp->client_index = vam->my_client_index;
...
static int api_acl_add_replace_from_file(vat_main_t *vam)
{
    int                       ret   = -1;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (!unformat(input,
                      "@%U/%d\t%U/%d\t%d : %d\t%d : %d\t0x%x/0x%x\t0x%x/0x%x",
    ...
    mp->_vl_msg_id   = ntohs(VL_API_ACL_ADD_REPLACE + sm->msg_id_base);
    mp->client_index = vam->my_client_index;
    if (n_rules > 0) clib_memcpy(mp->r, rules, n_rules * sizeof(vl_api_acl_rule_t));
    mp->acl_index = ntohl(acl_index);
    mp->count     = htonl(n_rules);
...
static int api_acl_interface_set_acl_list(vat_main_t *vam)
{
    ...
    if (vec_len(inacls) > 0) clib_memcpy(mp->acls, inacls, vec_len(inacls) * sizeof(u32));
...
static int api_acl_interface_set_etype_whitelist(vat_main_t *vam)
{
    ...
    if (vec_len(etypes_in) > 0) clib_memcpy(mp->whitelist, etypes_in, vec_len(etypes_in) * sizeof(etypes_in[0]));
...
static void api_acl_send_control_ping(vat_main_t *vam)
{
...
static int api_macip_acl_add(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(i, "ip %U/%d", unformat_ip4_address, &src_v4address, &src_prefix_length)
                 || unformat(i, "ip %U", unformat_ip4_address, &src_v4address)) {
            if (src_prefix_length == 0) src_prefix_length = 32;
            vec_validate_macip_acl_rules(rules, rule_idx);
            memcpy(rules[rule_idx].src_prefix.address.un.ip4, &src_v4address, 4);
            rules[rule_idx].src_prefix.address.af = ADDRESS_IP4;
            rules[rule_idx].src_prefix.len        = src_prefix_length;
        ...
        else if (unformat(i, "ip %U/%d", unformat_ip6_address, &src_v6address, &src_prefix_length)
                 || unformat(i, "ip %U", unformat_ip6_address, &src_v6address)) {
            if (src_prefix_length == 0) src_prefix_length = 128;
            vec_validate_macip_acl_rules(rules, rule_idx);
            memcpy(rules[rule_idx].src_prefix.address.un.ip4, &src_v6address, 4);
            rules[rule_idx].src_prefix.address.af = ADDRESS_IP6;
            rules[rule_idx].src_prefix.len        = src_prefix_length;
        }
        else if (unformat(i, "mac %U", my_unformat_mac_address, &src_mac)) {
            vec_validate_macip_acl_rules(rules, rule_idx);
            memcpy(rules[rule_idx].src_mac, &src_mac, 6);
            memcpy(rules[rule_idx].src_mac_mask, &mac_mask_all_1, 6);
        }
        else if (unformat(i, "mask %U", my_unformat_mac_address, &src_mac)) {
            vec_validate_macip_acl_rules(rules, rule_idx);
    ...
    if (rules) n_rules = vec_len(rules);
    ...
    mp->client_index = vam->my_client_index;
    if ((n_rules > 0) && rules) clib_memcpy(mp->r, rules, n_rules * sizeof(mp->r[0]));
    if (tag) {
        if (vec_len(tag) >= sizeof(mp->tag)) {
            tag[sizeof(mp->tag) - 1] = 0;
            vec_set_len(tag, sizeof(mp->tag));
