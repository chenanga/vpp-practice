...
always_inline u16 get_current_policy_epoch(acl_main_t *am, int is_input, u32 sw_if_index0)
{
    u32 **p_epoch_vec          = is_input ? &am->input_policy_epoch_by_sw_if_index : &am->output_policy_epoch_by_sw_if_index;
...
always_inline void maybe_trace_buffer(vlib_main_t         *vm,
...
always_inline int stale_session_deleted(acl_main_t               *am,
                                        int                       is_input,
...
{
    u16 current_policy_epoch = get_current_policy_epoch(am, is_input, sw_if_index0);
...
always_inline void get_sw_if_index_xN(int vector_sz, int is_input, vlib_buffer_t **b, u32 *out_sw_if_index)
{
    int ii;
...
always_inline void fill_5tuple_xN(int             vector_sz,
                                  acl_main_t     *am,
                                  int             is_ip6,
                                  int             is_input,
                                  int             is_l2_path,
                                  ...
                                  fa_5tuple_t    *out_fa_5tuple)
{
    int ii;
    for (ii = 0; ii < vector_sz; ii++)
        acl_fill_5tuple(am, sw_if_index[ii], b[ii], is_ip6, is_input, is_l2_path, &out_fa_5tuple[ii]);
}

always_inline void make_session_hash_xN(int vector_sz, acl_main_t *am, int is_ip6, u32 *sw_if_index, fa_5tuple_t *fa_5tuple, u64 *out_hash)
{
    int ii;
    for (ii = 0; ii < vector_sz; ii++)
        out_hash[ii] = acl_fa_make_session_hash(am, is_ip6, sw_if_index[ii], &fa_5tuple[ii]);
}

always_inline void prefetch_session_entry(acl_main_t *am, fa_full_session_id_t f_sess_id)
{
    fa_session_t *sess = get_session_ptr_no_check(am, f_sess_id.thread_index, f_sess_id.session_index);
    CLIB_PREFETCH(sess, sizeof(*sess), STORE);
}

always_inline u8 process_established_session(vlib_main_t         *vm,
                                             acl_main_t          *am,
                                             ...
                                             u32                 *trace_bitmap)
{
    ...
    action               = acl_fa_track_session(am, is_input, sw_if_index[0], now, sess, &fa_5tuple[0], pkt_len);
    ...
    if (PREDICT_FALSE(old_timeout_type != new_timeout_type)) {
        ...
        vlib_node_increment_counter(vm, counter_node_index, ACL_FA_ERROR_ACL_RESTART_SESSION_TIMER, 1);
        if (node_trace_on) *trace_bitmap |= 0x00010000 + ((0xff & old_timeout_type) << 8) + (0xff & new_timeout_type);
    }
    /*
    ...
    if (PREDICT_FALSE(sess->sw_if_index != sw_if_index[0])) {
        clib_warning("BUG: session LSB16(sw_if_index)=%d and 5-tuple=%d collision!", sess->sw_if_index, sw_if_index[0]);
        action = 0;
    }
    return action;
}
...
always_inline void acl_fa_node_common_prepare_fn(vlib_main_t         *vm,
                                                 vlib_node_runtime_t *node,
                                                 vlib_frame_t        *frame,
                                                 int                  is_ip6,
                                                 int                  is_input,
   ...
   int reclassify_sessions) */
{
    ...
    while (n_left >= (ACL_PLUGIN_PREFETCH_GAP + 1) * ACL_PLUGIN_VECTOR_SIZE) {
        ...
        {
            int ii;
            for (ii = ACL_PLUGIN_PREFETCH_GAP * vec_sz; ii < (ACL_PLUGIN_PREFETCH_GAP + 1) * vec_sz; ii++) {
        ...
        fill_5tuple_xN(vec_sz, am, is_ip6, is_input, is_l2_path, &b[0], &sw_if_index[0], &fa_5tuple[0]);
        if (with_stateful_datapath) make_session_hash_xN(vec_sz, am, is_ip6, &sw_if_index[0], &fa_5tuple[0], &hash[0]);
...
always_inline uword acl_fa_inner_node_fn(vlib_main_t         *vm,
                                         vlib_node_runtime_t *node,
                                         vlib_frame_t        *frame,
                                         int                  is_ip6,
                                         int                  is_input,
                                         ...
                                         int                  reclassify_sessions)
{
    ...
    no_error_existing_session = error_node->errors[ACL_FA_ERROR_ACL_EXIST_SESSION];
    ...
    if (with_stateful_datapath) acl_fa_find_session_with_hash(am, is_ip6, sw_if_index[0], hash[0], &fa_5tuple[0], &f_sess_id_next.as_u64);
    ...
    while (n_left > 0) {
        ...
        if (with_stateful_datapath) {
            ...
            switch (n_left) {
                ...
                case 3:
                case 2:
                    acl_fa_find_session_with_hash(am, is_ip6, sw_if_index[1], hash[1], &fa_5tuple[1], &f_sess_id_next.as_u64);
                ...
                case 1:
                    if (f_sess_id.as_u64 != ~0ULL) {
                        ...
                        action = process_established_session(vm,
                                                             am,
                                                             node->node_index,
                                                             is_input,
                                                             now,
                        ...
                        if (reclassify_sessions) {
                            if (PREDICT_FALSE(stale_session_deleted(am, is_input, pw, now, sw_if_index[0], f_sess_id))) {
                                acl_check_needed = 1;
                                if (node_trace_on) {
                                 ...
                                 * is wrong, correct it.
                                 */
                                if ((f_sess_id_next.as_u64 != ~0ULL) && 0 == memcmp(&fa_5tuple[1], &fa_5tuple[0], sizeof(fa_5tuple[1])))
            ...
            if (acl_check_needed) {
                ...
                int is_match = acl_plugin_match_5tuple_inline(am,
                                                              lc_index0,
                ...
                if (PREDICT_FALSE(is_match && am->interface_acl_counters_enabled)) {
                    ...
                    vlib_increment_combined_counter(am->combined_acl_counters + saved_matched_acl_index,
                                                    thread_index,
                    ...
                    vlib_prefetch_combined_counter(am->combined_acl_counters + saved_matched_acl_index, thread_index, saved_matched_ace_index);
                }

                b[0]->error = error_node->errors[action];

                if (1 == action) pkts_acl_permit++;

                if (2 == action) {
                    if (!acl_fa_can_add_session(am, is_input, sw_if_index[0]))
                        acl_fa_try_recycle_session(am, is_input, thread_index, sw_if_index[0], now);

                    if (acl_fa_can_add_session(am, is_input, sw_if_index[0])) {
                        u16                  current_policy_epoch = get_current_policy_epoch(am, is_input, sw_if_index[0]);
                        fa_full_session_id_t f_sess_id =
                            acl_fa_add_session(am, is_input, is_ip6, sw_if_index[0], now, &fa_5tuple[0], current_policy_epoch);

                        /* perform the accounting for the newly added session */
                        process_established_session(vm,
                                                    am,
                                                    node->node_index,
                                                    is_input,
                                                    now,
    ...
    vlib_node_increment_counter(vm, node->node_index, ACL_FA_ERROR_ACL_CHECK, frame->n_vectors);
    vlib_node_increment_counter(vm, node->node_index, ACL_FA_ERROR_ACL_EXIST_SESSION, pkts_exist_session);
    vlib_node_increment_counter(vm, node->node_index, ACL_FA_ERROR_ACL_NEW_SESSION, pkts_new_session);
    vlib_node_increment_counter(vm, node->node_index, ACL_FA_ERROR_ACL_PERMIT, pkts_acl_permit);
    return frame->n_vectors;
}

always_inline uword acl_fa_outer_node_fn(vlib_main_t         *vm,
                                         vlib_node_runtime_t *node,
                                         vlib_frame_t        *frame,
                                         int                  is_ip6,
                                         int                  is_input,
                                         ...
                                         int                  do_stateful_datapath)
{
    ...
    acl_fa_node_common_prepare_fn(vm, node, frame, is_ip6, is_input, is_l2_path, do_stateful_datapath);

    if (am->reclassify_sessions) {
        if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE))
            return acl_fa_inner_node_fn(vm, node, frame, is_ip6, is_input, is_l2_path, do_stateful_datapath, 1 /* trace */, 1 /* reclassify */);
        else
            return acl_fa_inner_node_fn(vm, node, frame, is_ip6, is_input, is_l2_path, do_stateful_datapath, 0, 1 /* reclassify */);
    }
    else {
        if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE))
            return acl_fa_inner_node_fn(vm, node, frame, is_ip6, is_input, is_l2_path, do_stateful_datapath, 1 /* trace */, 0);
        else
            return acl_fa_inner_node_fn(vm, node, frame, is_ip6, is_input, is_l2_path, do_stateful_datapath, 0, 0);
    }
}

always_inline uword acl_fa_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ip6, int is_input, int is_l2_path)
{
    ...
    if (am->fa_sessions_hash_is_initialized)
        rv = acl_fa_outer_node_fn(vm, node, frame, is_ip6, is_input, is_l2_path, 1);
    else
        rv = acl_fa_outer_node_fn(vm, node, frame, is_ip6, is_input, is_l2_path, 0);

    vlib_buffer_enqueue_to_next(vm, node, vlib_frame_vector_args(frame), pw->nexts, frame->n_vectors);
    return rv;
}
...
static u8 *format_fa_5tuple(u8 *s, va_list *args)
{
    ...
    void        *ip_af;
    void        *ip_frag_txt = p5t->pkt.is_nonfirst_fragment ? " non-initial fragment" : "";
    ...
    s = format(s, "lc_index %d l3 %s%s ", p5t->pkt.lc_index, ip_af, ip_frag_txt);
    s = format(s, "%U -> %U ", format_address_func, paddr0, format_address_func, paddr1);
    s = format(s, "%U ", format_fa_session_l4_key, &p5t->l4);
    s = format(s, "tcp flags (%s) %02x rsvd %x", p5t->pkt.tcp_flags_valid ? "valid" : "invalid", p5t->pkt.tcp_flags, p5t->pkt.flags_reserved);
    return s;
}
...
VLIB_REGISTER_NODE(acl_in_l2_ip6_node) = {.name          = "acl-plugin-in-ip6-l2",
...
VLIB_REGISTER_NODE(acl_in_l2_ip4_node) = {.name          = "acl-plugin-in-ip4-l2",
                                          .vector_size   = sizeof(u32),
                                          .format_trace  = format_acl_plugin_trace,
                                          .type          = VLIB_NODE_TYPE_INTERNAL,
                                          .n_errors      = ARRAY_LEN(acl_fa_error_strings),
...
VLIB_REGISTER_NODE(acl_out_l2_ip6_node) = {.name          = "acl-plugin-out-ip6-l2",
                                           .vector_size   = sizeof(u32),
                                           .format_trace  = format_acl_plugin_trace,
                                           .type          = VLIB_NODE_TYPE_INTERNAL,
                                           .n_errors      = ARRAY_LEN(acl_fa_error_strings),
...
VLIB_REGISTER_NODE(acl_out_l2_ip4_node) = {.name          = "acl-plugin-out-ip4-l2",
                                           .vector_size   = sizeof(u32),
                                           .format_trace  = format_acl_plugin_trace,
                                           .type          = VLIB_NODE_TYPE_INTERNAL,
                                           .n_errors      = ARRAY_LEN(acl_fa_error_strings),
...
VLIB_REGISTER_NODE(acl_in_fa_ip6_node) = {.name          = "acl-plugin-in-ip6-fa",
                                          .vector_size   = sizeof(u32),
                                          .format_trace  = format_acl_plugin_trace,
                                          .type          = VLIB_NODE_TYPE_INTERNAL,
                                          .n_errors      = ARRAY_LEN(acl_fa_error_strings),
...
VLIB_REGISTER_NODE(acl_in_fa_ip4_node) = {.name          = "acl-plugin-in-ip4-fa",
                                          .vector_size   = sizeof(u32),
                                          .format_trace  = format_acl_plugin_trace,
                                          .type          = VLIB_NODE_TYPE_INTERNAL,
                                          .n_errors      = ARRAY_LEN(acl_fa_error_strings),
...
VLIB_REGISTER_NODE(acl_out_fa_ip6_node) = {.name          = "acl-plugin-out-ip6-fa",
                                           .vector_size   = sizeof(u32),
                                           .format_trace  = format_acl_plugin_trace,
                                           .type          = VLIB_NODE_TYPE_INTERNAL,
                                           .n_errors      = ARRAY_LEN(acl_fa_error_strings),
...
VLIB_REGISTER_NODE(acl_out_fa_ip4_node) = {.name          = "acl-plugin-out-ip4-fa",
                                           .vector_size   = sizeof(u32),
                                           .format_trace  = format_acl_plugin_trace,
                                           .type          = VLIB_NODE_TYPE_INTERNAL,
                                           .n_errors      = ARRAY_LEN(acl_fa_error_strings),
