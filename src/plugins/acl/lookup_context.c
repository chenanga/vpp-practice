...
{
    if (pool_is_free_index(am->acl_users, acl_user_id)) return 0;
...
{
    if (pool_is_free_index(am->acl_lookup_contexts, lc_index)) return 0;
...
static int acl_plugin_get_lookup_context_index(u32 acl_user_id, u32 val1, u32 val2)
{
    ...
    if (!acl_user_id_valid(am, acl_user_id)) return VNET_API_ERROR_INVALID_REGISTRATION;
...
static int acl_plugin_match_5tuple(u32                 lc_index,
                                   ...
                                   u32                *trace_bitmap)
{
    return acl_plugin_match_5tuple_inline(&acl_main,
                                          lc_index,
                                          pkt_5tuple,
                                          is_ip6,
                                          r_action,
        ...
        if ((lc_index == ~0) || (curr_lc_index == lc_index)) {
            if (acl_user_id_valid(am, acontext->context_user_id)) {
                ...
                vlib_cli_output(vm,
                                ...
                                curr_lc_index,
                                auser->user_module_name,
                                auser->val1_label,
                                acontext->user_val1,
                                auser->val2_label,
                                acontext->user_val2,
                                format_vec32,
                                acontext->acl_indices,
                                "%d");
            }
            else {
                vlib_cli_output(vm,
