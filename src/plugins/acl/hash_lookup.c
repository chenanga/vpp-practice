...
static u8 first_mask_contains_second_mask(int is_ip6, fa_5tuple_t *mask1, fa_5tuple_t *mask2)
{
    ...
    if (is_ip6) {
        for (i = 0; i < 2; i++) {
            if ((mask1->ip6_addr[0].as_u64[i] & mask2->ip6_addr[0].as_u64[i]) != mask1->ip6_addr[0].as_u64[i]) return 0;
    ...
    if ((mask1->l4.as_u64 & mask2->l4.as_u64) != mask1->l4.as_u64) return 0;
...
static void relax_ip4_addr(ip4_address_t *ip4_mask, int relax2)
{
    ...
    if (ip4_mask->as_u32 == 0xffffffff)
        ...
        ip4_mask->as_u32 = shift_ip4_if(
            ip4_mask->as_u32,
            0xffffff00,
            shifts[1],
            shift_ip4_if(ip4_mask->as_u32, 0xffff0000, shifts[2], shift_ip4_if(ip4_mask->as_u32, 0xff000000, shifts[3], ip4_mask->as_u32)));
}
...
static void remake_hash_applied_mask_info_vec(acl_main_t *am, applied_hash_ace_entry_t **applied_hash_aces, u32 lc_index)
{
    ...
    hash_applied_mask_info_t *new_hash_applied_mask_info_vec = vec_new(hash_applied_mask_info_t, 0);
    ...
    for (i = 0; i < vec_len((*applied_hash_aces)); i++) {
        ...
        int search;
        for (search = 0; search < vec_len(new_hash_applied_mask_info_vec); search++) {
            minfo = vec_elt_at_index(new_hash_applied_mask_info_vec, search);
            if (minfo->mask_type_index == pae->mask_type_index) break;
        }
        ...
        if (vec_len(pae->colliding_rules) > minfo->max_collisions) minfo->max_collisions = vec_len(pae->colliding_rules);

        if (minfo->first_rule_index > i) minfo->first_rule_index = i;
    }
...
static void del_colliding_rule(applied_hash_ace_entry_t **applied_hash_aces, u32 head_index, u32 applied_entry_index)
{
    ...
    if (ACL_HASH_LOOKUP_DEBUG > 0) acl_plugin_print_pae(acl_main.vlib_main, head_index, head_pae);
...
{
    applied_hash_ace_entry_t *head_pae = vec_elt_at_index((*applied_hash_aces), head_index);
    applied_hash_ace_entry_t *pae      = vec_elt_at_index((*applied_hash_aces), applied_entry_index);
...
static u32 activate_applied_ace_hash_entry(acl_main_t *am, u32 lc_index, applied_hash_ace_entry_t **applied_hash_aces, u32 new_index)
{
    ...
    DBG("APPLY ADD KY: %016llx %016llx %016llx %016llx %016llx %016llx", kv.key[0], kv.key[1], kv.key[2], kv.key[3], kv.key[4], kv.key[5]);
...
void hash_acl_apply(acl_main_t *am, u32 lc_index, int acl_index, u32 acl_position)
{
    ...
    if (!am->acl_lookup_hash_initialized) {
        BV(clib_bihash_init)(&am->acl_lookup_hash, "ACL plugin rule lookup bihash", am->hash_lookup_hash_buckets, am->hash_lookup_hash_memory);
    ...
    if (index != ~0) {
        clib_warning("BUG: trying to apply twice acl_index %d on lc_index %d, according to lc", acl_index, lc_index);
        ASSERT(0);
        return;
    }
    ...
    if (index2 != ~0) {
        clib_warning("BUG: trying to apply twice acl_index %d on lc_index %d, according to hash h-acl info", acl_index, lc_index);
        ASSERT(0);
        return;
    }
    ...
    for (i = 0; i < vec_len(ha->rules); i++) {
        ...
        if (am->use_tuple_merge) check_collision_count_and_maybe_split(am, lc_index, is_ip6, first_index);
    }
...
static void move_applied_ace_hash_entry(acl_main_t *am, u32 lc_index, applied_hash_ace_entry_t **applied_hash_aces, u32 old_index, u32 new_index)
{
    ...
    *vec_elt_at_index((*applied_hash_aces), new_index) = *vec_elt_at_index((*applied_hash_aces), old_index);
    ...
    if (pae->collision_head_ae_index == old_index) {
        ...
        add_del_hashtable_entry(am, lc_index, applied_hash_aces, new_index, 1);
    }
...
static void deactivate_applied_ace_hash_entry(acl_main_t *am, u32 lc_index, applied_hash_ace_entry_t **applied_hash_aces, u32 old_index)
{
    ...
    if (pae->collision_head_ae_index != old_index) {
        ...
        if (vec_len(pae->colliding_rules) > 0) {
            ...
            add_del_hashtable_entry(am, lc_index, applied_hash_aces, next_pae_index, 1);
        }
        else {
            /* no next entry, so just delete the entry in the hash table */
            add_del_hashtable_entry(am, lc_index, applied_hash_aces, old_index, 0);
        }
    }
...
void hash_acl_unapply(acl_main_t *am, u32 lc_index, int acl_index)
{
    ...
    u32 index = vec_search(pal->applied_acls, acl_index);
    if (index == ~0) {
        clib_warning("BUG: trying to unapply unapplied acl_index %d on lc_index %d, according to lc", acl_index, lc_index);
        return;
    }
    ...
    u32 index2 = vec_search((*hash_acl_applied_lc_index), lc_index);
    if (index2 == ~0) {
        clib_warning("BUG: trying to unapply twice acl_index %d on lc_index %d, according to h-acl info", acl_index, lc_index);
        return;
    }
    ...
    for (i = 0; i < vec_len(ha->rules); i++) {
        deactivate_applied_ace_hash_entry(am, lc_index, applied_hash_aces, base_offset + i);
    }
...
always_inline void ip4_address_mask_from_width(ip4_address_t *a, u32 width)
{
...
{
    if (acl_index >= vec_len(am->hash_acl_infos)) return 0;
...
void acl_plugin_show_tables_mask_type(void)
{
    ...
    pool_foreach (mte, am->ace_mask_type_pool) {
        vlib_cli_output(vm,
                        ...
                        mte->mask.kv_40_8.key[0],
                        mte->mask.kv_40_8.key[1],
                        mte->mask.kv_40_8.key[2],
                        mte->mask.kv_40_8.key[3],
                        mte->mask.kv_40_8.key[4],
                        mte->mask.kv_40_8.value,
                        mte->refcount);
    }
}
...
static void acl_plugin_print_colliding_rule(vlib_main_t *vm, int j, collision_match_rule_t *cr)
{
    vlib_cli_output(vm,
                    ...
                    cr->ace_index,
                    cr->acl_position,
                    cr->applied_entry_index);
}

static void acl_plugin_print_pae(vlib_main_t *vm, int j, applied_hash_ace_entry_t *pae)
{
    vlib_cli_output(vm,
                    "    %4d: acl %d rule %d action %d bitmask-ready rule %d mask type index: %d colliding_rules: %d collision_head_ae_idx %d "
                    "hitcount %lld acl_pos: %d",
                    j,
                    pae->acl_index,
                    pae->ace_index,
                    pae->action,
                    pae->hash_ace_info_index,
                    pae->mask_type_index,
                    vec_len(pae->colliding_rules),
                    pae->collision_head_ae_index,
                    pae->hitcount,
                    pae->acl_position);
    int jj;
...
void acl_plugin_show_tables_applied_info(u32 lc_index)
{
    ...
    for (lci = 0; (lci < vec_len(am->applied_hash_acl_info_by_lc_index)); lci++) {
        if ((lc_index != ~0) && (lc_index != lci)) {
            continue;
        }
        vlib_cli_output(vm, "lc_index %d:", lci);
        if (lci < vec_len(am->applied_hash_acl_info_by_lc_index)) {
            applied_hash_acl_info_t *pal = &am->applied_hash_acl_info_by_lc_index[lci];
            vlib_cli_output(vm, "  applied acls: %U", format_vec32, pal->applied_acls, "%d");
        }
        if (lci < vec_len(am->hash_applied_mask_info_vec_by_lc_index)) {
            vlib_cli_output(vm, "  applied mask info entries:");
            for (j = 0; j < vec_len(am->hash_applied_mask_info_vec_by_lc_index[lci]); j++) {
                acl_plugin_print_applied_mask_info(vm, j, &am->hash_applied_mask_info_vec_by_lc_index[lci][j]);
            }
        }
        if (lci < vec_len(am->hash_entry_vec_by_lc_index)) {
            vlib_cli_output(vm, "  lookup applied entries:");
            for (j = 0; j < vec_len(am->hash_entry_vec_by_lc_index[lci]); j++) {
                acl_plugin_print_pae(vm, j, &am->hash_entry_vec_by_lc_index[lci][j]);
            }
        }
    }
}
...
{
    int update = (clib_net_to_host_u64(mask_addr->as_u64[0]) < clib_net_to_host_u64(min_addr->as_u64[0]))
                 || ((clib_net_to_host_u64(mask_addr->as_u64[0]) == clib_net_to_host_u64(min_addr->as_u64[0]))
                     && (clib_net_to_host_u64(mask_addr->as_u64[1]) < clib_net_to_host_u64(min_addr->as_u64[1])));
    if (update) {
...
{
    int update = (clib_net_to_host_u64(mask_addr->as_u64[0]) > clib_net_to_host_u64(max_addr->as_u64[0]))
                 || ((clib_net_to_host_u64(mask_addr->as_u64[0]) == clib_net_to_host_u64(max_addr->as_u64[0]))
                     && (clib_net_to_host_u64(mask_addr->as_u64[1]) > clib_net_to_host_u64(max_addr->as_u64[1])));
    if (update) {
...
{
    int update = (clib_net_to_host_u32(mask_addr->as_u32) < clib_net_to_host_u32(min_addr->as_u32));
    if (update) min_addr->as_u32 = mask_addr->as_u32;
}

static void ensure_ip4_max_addr(ip4_address_t *max_addr, ip4_address_t *mask_addr)
{
    int update = (clib_net_to_host_u32(mask_addr->as_u32) > clib_net_to_host_u32(max_addr->as_u32));
    if (update) max_addr->as_u32 = mask_addr->as_u32;
}
...
static void split_partition(acl_main_t *am, u32 first_index, u32 lc_index, int is_ip6)
{
    ...
    for (i = 0; i < collisions; i++) {
        ...
        DBG("TM-collision: base_ace:%d (ace_mask:%d, first_collision_mask:%d)", pae->ace_index, pae->mask_type_index, coll_mask_type_index);
        ...
        if (i == 0) {
            ...
            for (j = 0; j < 2; j++) {
                ...
                if ((mask->l4.port[j] < min_tuple->l4.port[j])) min_tuple->l4.port[j] = mask->l4.port[j];
            }

            if ((mask->l4.proto < min_tuple->l4.proto)) min_tuple->l4.proto = mask->l4.proto;

            if (mask->pkt.as_u64 < min_tuple->pkt.as_u64) min_tuple->pkt.as_u64 = mask->pkt.as_u64;
            ...
            for (j = 0; j < 2; j++) {
                ...
                if ((mask->l4.port[j] > max_tuple->l4.port[j])) max_tuple->l4.port[j] = mask->l4.port[j];
            }

            if ((mask->l4.proto < max_tuple->l4.proto)) max_tuple->l4.proto = mask->l4.proto;

            if (mask->pkt.as_u64 > max_tuple->pkt.as_u64) max_tuple->pkt.as_u64 = mask->pkt.as_u64;
        }
    }
    ...
    switch (best_dim) {
        case DIM_SRC_ADDR:
            ...
            min_tuple->ip4_addr[0].as_u32 = clib_host_to_net_u32((clib_net_to_host_u32(max_tuple->ip4_addr[0].as_u32) << (shifting)) & 0xFFFFFFFF);
            ...
            /*
                                    ...
                                    if(ipv4_block > shifting)
                                            min_tuple->addr[1].as_u64[1] =
                                                    clib_host_to_net_u64((clib_net_to_host_u64(max_tuple->addr[1].as_u64[1]) <<
               (shifting))&0xFFFFFFFF); else{ shifting = shifting - ipv4_block; min_tuple->addr[1].as_u64[1] = 0; min_tuple->addr[1].as_u64[0] =
                                                    clib_host_to_net_u64((clib_net_to_host_u64(max_tuple->addr[1].as_u64[0]) <<
               (shifting))&0xFFFFFFFF);
                                    }
            */
            min_tuple->ip4_addr[1].as_u32 = clib_host_to_net_u32((clib_net_to_host_u32(max_tuple->ip4_addr[1].as_u32) << (shifting)) & 0xFFFFFFFF);

            break;
        case DIM_SRC_PORT:
            min_tuple->l4.port[0] = max_tuple->l4.port[0] << (best_delta) / 2;
            break;
        case DIM_DST_PORT:
            min_tuple->l4.port[1] = max_tuple->l4.port[1] << (best_delta) / 2;
            break;
        case DIM_PROTO:
            min_tuple->l4.proto = max_tuple->l4.proto << (best_delta) / 2;
            break;
        default:
            relax_tuple(min_tuple, is_ip6, 1);
            break;
    }
    ...
    for (search = 0; search < vec_len((*hash_applied_mask_info_vec)); search++) {
        ...
        if (minfo->mask_type_index == new_mask_type_index) break;
    }
    ...
    for (i = 0; i < collisions; i++) {
        ...
        DBG("TM-Population-collision: base_ace:%d (ace_mask:%d, first_collision_mask:%d)",
            pop_pae->ace_index,
            pop_pae->mask_type_index,
            coll_mask_type_index);
        ...
        if (++repopulate_count > 1) lock_mask_type_index(am, new_mask_type_index);
