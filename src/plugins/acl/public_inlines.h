...
#define LOAD_SYMBOL_FROM_PLUGIN_TO(p, s, st)                                                  \
    ({                                                                                        \
        ...
        if (!st) return clib_error_return(0, "Plugin %s and/or symbol %s not found.", p, #s); \
    })
...
always_inline void *get_ptr_to_offset(vlib_buffer_t *b0, int offset)
{
    u8 *p = vlib_buffer_get_current(b0) + offset;
    return p;
}

always_inline int offset_within_packet(vlib_buffer_t *b0, int offset)
{
    /* For the purposes of this code, "within" means we have at least 8 bytes after it */
    return (offset <= (b0->current_length - 8));
}

always_inline int offset_beyond_packet(vlib_buffer_t *b0, int offset)
{
    /* For the purposes of this code, "within" means we have at least 8 bytes after it */
    return (offset > (b0->current_length - 8));
}

always_inline void acl_fill_5tuple_l3_data(acl_main_t *am, vlib_buffer_t *b0, int is_ip6, int l3_offset, fa_5tuple_t *p5tuple_pkt)
{
    if (is_ip6) {
...
always_inline void acl_fill_5tuple_l4_and_pkt_data(acl_main_t          *am,
                                                   u32                  sw_if_index0,
                                                   vlib_buffer_t       *b0,
                                                   int                  is_ip6,
                                                   int                  is_input,
    ...
    if (is_ip6) {
        ...
        if (PREDICT_FALSE(need_skip_eh)) {
            while (need_skip_eh && offset_within_packet(b0, l4_offset)) {
                ...
                need_skip_eh = clib_bitmap_get(am->fa_ipv6_known_eh_bitmap, proto);
            }
        }
    }
    else {
...
always_inline void
acl_fill_5tuple(acl_main_t *am, u32 sw_if_index0, vlib_buffer_t *b0, int is_ip6, int is_input, int is_l2_path, fa_5tuple_t *p5tuple_pkt)
{
    int l3_offset;
...
always_inline void acl_plugin_fill_5tuple_inline(void               *p_acl_main,
                                                 u32                 lc_index,
                                                 vlib_buffer_t      *b0,
                                                 int                 is_ip6,
                                                 int                 is_input,
...
always_inline int fa_acl_match_ip4_addr(ip4_address_t *addr1, ip4_address_t *addr2, int prefixlen)
{
    if (prefixlen == 0) {
...
always_inline int fa_acl_match_ip6_addr(ip6_address_t *addr1, ip6_address_t *addr2, int prefixlen)
{
    if (prefixlen == 0) {
        /* match any always succeeds */
        return 1;
...
always_inline int fa_acl_match_port(u16 port, u16 port_first, u16 port_last, int is_ip6)
{
    return ((port >= port_first) && (port <= port_last));
}

always_inline int single_acl_match_5tuple(acl_main_t  *am,
                                          u32          acl_index,
                                          ...
                                          u32         *trace_bitmap)
{
    ...
    for (i = 0; i < vec_len(acl_rules); i++) {
        ...
        if (is_ip6) {
            if (!fa_acl_match_ip6_addr(&pkt_5tuple->ip6_addr[1], &r->dst.ip6, r->dst_prefixlen)) continue;
            if (!fa_acl_match_ip6_addr(&pkt_5tuple->ip6_addr[0], &r->src.ip6, r->src_prefixlen)) continue;
        }
        else {
            if (!fa_acl_match_ip4_addr(&pkt_5tuple->ip4_addr[1], &r->dst.ip4, r->dst_prefixlen)) continue;
            if (!fa_acl_match_ip4_addr(&pkt_5tuple->ip4_addr[0], &r->src.ip4, r->src_prefixlen)) continue;
        }

        if (r->proto) {
            if (pkt_5tuple->l4.proto != r->proto) continue;

            if (PREDICT_FALSE(pkt_5tuple->pkt.is_nonfirst_fragment && am->l4_match_nonfirst_fragment)) {
            ...
            if (PREDICT_FALSE(!pkt_5tuple->pkt.l4_valid)) continue;

#ifdef FA_NODE_VERBOSE_DEBUG
            clib_warning("ACL_FA_NODE_DBG acl %d rule %d pkt proto %d match rule %d", acl_index, i, pkt_5tuple->l4.proto, r->proto);
#endif

            if (!fa_acl_match_port(pkt_5tuple->l4.port[0], r->src_port_or_type_first, r->src_port_or_type_last, is_ip6)) continue;

#ifdef FA_NODE_VERBOSE_DEBUG
            clib_warning("ACL_FA_NODE_DBG acl %d rule %d pkt sport %d match rule [%d..%d]",
                         acl_index,
                         i,
            ...
            if (!fa_acl_match_port(pkt_5tuple->l4.port[1], r->dst_port_or_code_first, r->dst_port_or_code_last, is_ip6)) continue;

#ifdef FA_NODE_VERBOSE_DEBUG
            clib_warning("ACL_FA_NODE_DBG acl %d rule %d pkt dport %d match rule [%d..%d]",
                         acl_index,
                         i,
...
#endif
            if (pkt_5tuple->pkt.tcp_flags_valid && ((pkt_5tuple->pkt.tcp_flags & r->tcp_flags_mask) != r->tcp_flags_value)) continue;
        }
        /* everything matches! */
#ifdef FA_NODE_VERBOSE_DEBUG
        clib_warning("ACL_FA_NODE_DBG acl %d rule %d FULL-MATCH, action %d", acl_index, i, r->is_permit);
#endif
        *r_action = r->is_permit;
        if (r_acl_match_p) *r_acl_match_p = acl_index;
        if (r_rule_match_p) *r_rule_match_p = i;
...
always_inline int acl_plugin_single_acl_match_5tuple(void        *p_acl_main,
                                                     u32          acl_index,
                                                     fa_5tuple_t *pkt_5tuple,
                                                     int          is_ip6,
                                                     u8          *r_action,
...
always_inline int linear_multi_acl_match_5tuple(void        *p_acl_main,
                                                u32          lc_index,
                                                fa_5tuple_t *pkt_5tuple,
                                                int          is_ip6,
                                                u8          *r_action,
...
#ifdef FA_NODE_VERBOSE_DEBUG
        clib_warning("ACL_FA_NODE_DBG: Trying to match ACL: %d", acl_vector[i]);
#endif
        if (single_acl_match_5tuple(am, acl_vector[i], pkt_5tuple, is_ip6, &action, acl_match_p, rule_match_p, trace_bitmap)) {
            *r_action  = action;
            *acl_pos_p = i;
            return 1;
        }
 ...
 */
always_inline int match_portranges(acl_main_t *am, fa_5tuple_t *match, u32 index)
{
...
always_inline int single_rule_match_5tuple(acl_rule_t *r, int is_ip6, fa_5tuple_t *pkt_5tuple)
{
    if (is_ip6 != r->is_ipv6) {
        return 0;
    }
    ...
    if (is_ip6) {
        if (!fa_acl_match_ip6_addr(&pkt_5tuple->ip6_addr[1], &r->dst.ip6, r->dst_prefixlen)) return 0;
        if (!fa_acl_match_ip6_addr(&pkt_5tuple->ip6_addr[0], &r->src.ip6, r->src_prefixlen)) return 0;
    }
    else {
        if (!fa_acl_match_ip4_addr(&pkt_5tuple->ip4_addr[1], &r->dst.ip4, r->dst_prefixlen)) return 0;
        if (!fa_acl_match_ip4_addr(&pkt_5tuple->ip4_addr[0], &r->src.ip4, r->src_prefixlen)) return 0;
    }

    if (r->proto) {
        if (pkt_5tuple->l4.proto != r->proto) return 0;

        /* A sanity check just to ensure we are about to match the ports extracted from the packet */
        if (PREDICT_FALSE(!pkt_5tuple->pkt.l4_valid)) return 0;
...
always_inline u32 multi_acl_match_get_applied_ace_index(acl_main_t *am, int is_ip6, fa_5tuple_t *match)
{
    ...
    DBG("TRYING TO MATCH: %016llx %016llx %016llx %016llx %016llx %016llx", pmatch[0], pmatch[1], pmatch[2], pmatch[3], pmatch[4], pmatch[5]);

    for (order_index = 0; order_index < vec_len((*hash_applied_mask_info_vec)); order_index++) {
...
always_inline int hash_multi_acl_match_5tuple(void        *p_acl_main,
                                              u32          lc_index,
                                              fa_5tuple_t *pkt_5tuple,
                                              int          is_ip6,
                                              u8          *action,
...
always_inline int acl_plugin_match_5tuple_inline(void               *p_acl_main,
                                                 u32                 lc_index,
                                                 ...
                                                 u32                *trace_bitmap)
{
    ...
    if (PREDICT_TRUE(am->use_hash_acl_matching)) {
        if (PREDICT_FALSE(pkt_5tuple_internal->pkt.is_nonfirst_fragment)) {
             ...
             */
            return linear_multi_acl_match_5tuple(p_acl_main,
                                                 lc_index,
                                                 pkt_5tuple_internal,
                                                 is_ip6,
                                                 r_action,
            ...
            return hash_multi_acl_match_5tuple(p_acl_main,
                                               lc_index,
                                               pkt_5tuple_internal,
                                               is_ip6,
                                               r_action,
...
always_inline int acl_plugin_match_5tuple_inline_and_count(void               *p_acl_main,
                                                           u32                 lc_index,
                                                           fa_5tuple_opaque_t *pkt_5tuple,
                                                           int                 is_ip6,
                                                           u8                 *r_action,
                                                           ...
                                                           u32                 packet_size)
{
    ...
    if (PREDICT_TRUE(am->use_hash_acl_matching)) {
        if (PREDICT_FALSE(pkt_5tuple_internal->pkt.is_nonfirst_fragment)) {
             ...
             * matching in that case.
             */
            ret = linear_multi_acl_match_5tuple(p_acl_main,
                                                lc_index,
                                                pkt_5tuple_internal,
                                                is_ip6,
                                                r_action,
            ...
            ret = hash_multi_acl_match_5tuple(p_acl_main,
                                              lc_index,
                                              pkt_5tuple_internal,
                                              is_ip6,
                                              r_action,
