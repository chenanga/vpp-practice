...
static inline void avf_flow_convert_rss_types(u64 type, u64 *avf_rss_type)
{
...
#undef _
#define _(n, f, s) \
    if (n != -1 && BIT_IS_SET(type, n)) *avf_rss_type |= f;
...
int avf_flow_vc_op_callback(void *vc_hdl, enum virthnl_adv_ops vc_op, void *in, u32 in_len, void *out, u32 out_len)
{
    ...
    err = avf_program_flow(dev_instance, is_add, vc_op, in, in_len, out, out_len);
    if (err != 0) {
...
static inline enum avf_eth_hash_function avf_flow_convert_rss_func(vnet_rss_function_t func)
{
...
static inline void avf_flow_convert_rss_queues(u32 queue_index, u32 queue_num, struct avf_flow_action_rss *act_rss)
{
...
void avf_parse_generic_pattern(struct avf_flow_item *item, u8 *pkt_buf, u8 *msk_buf, u16 spec_len)
{
    ...
    for (i = 0, j = 0; i < spec_len; i += 2, j++) {
        tmp_c = raw_spec[i];
        if (tmp_c >= 'a' && tmp_c <= 'f') tmp_val = tmp_c - 'a' + 10;
        if (tmp_c >= 'A' && tmp_c <= 'F') tmp_val = tmp_c - 'A' + 10;
        if (tmp_c >= '0' && tmp_c <= '9') tmp_val = tmp_c - '0';

        tmp_c = raw_spec[i + 1];
        if (tmp_c >= 'a' && tmp_c <= 'f') pkt_buf[j] = tmp_val * 16 + tmp_c - 'a' + 10;
        if (tmp_c >= 'A' && tmp_c <= 'F') pkt_buf[j] = tmp_val * 16 + tmp_c - 'A' + 10;
        if (tmp_c >= '0' && tmp_c <= '9') pkt_buf[j] = tmp_val * 16 + tmp_c - '0';

        tmp_c = raw_mask[i];
        if (tmp_c >= 'a' && tmp_c <= 'f') tmp_val = tmp_c - 0x57;
        if (tmp_c >= 'A' && tmp_c <= 'F') tmp_val = tmp_c - 0x37;
        if (tmp_c >= '0' && tmp_c <= '9') tmp_val = tmp_c - '0';

        tmp_c = raw_mask[i + 1];
        if (tmp_c >= 'a' && tmp_c <= 'f') msk_buf[j] = tmp_val * 16 + tmp_c - 'a' + 10;
        if (tmp_c >= 'A' && tmp_c <= 'F') msk_buf[j] = tmp_val * 16 + tmp_c - 'A' + 10;
        if (tmp_c >= '0' && tmp_c <= '9') msk_buf[j] = tmp_val * 16 + tmp_c - '0';
    }
}
...
static int avf_flow_add(u32 dev_instance, vnet_flow_t *f, avf_flow_entry_t *fe)
{
    ...
    if (flow_class == FLOW_IPV4_CLASS) {
        ...
        if ((ip6_address_is_zero(&ip6_ptr->src_addr.mask)) && (ip6_address_is_zero(&ip6_ptr->dst_addr.mask)) && (!ip6_ptr->protocol.mask)) {
            ;
        }
        else {
            clib_memcpy(ip6_spec.src_addr, &ip6_ptr->src_addr.addr, ARRAY_LEN(ip6_ptr->src_addr.addr.as_u8));
            clib_memcpy(ip6_mask.src_addr, &ip6_ptr->src_addr.mask, ARRAY_LEN(ip6_ptr->src_addr.mask.as_u8));
            clib_memcpy(ip6_spec.dst_addr, &ip6_ptr->dst_addr.addr, ARRAY_LEN(ip6_ptr->dst_addr.addr.as_u8));
            clib_memcpy(ip6_mask.dst_addr, &ip6_ptr->dst_addr.mask, ARRAY_LEN(ip6_ptr->dst_addr.mask.as_u8));
...
pattern_end:
    ...
    if (f->actions & VNET_FLOW_ACTION_RSS) {
        ...
        if ((act_rss.func = avf_flow_convert_rss_func(f->rss_fun)) == AVF_ETH_HASH_FUNCTION_MAX) {
            rv = VNET_FLOW_ERROR_NOT_SUPPORTED;
            goto done;
        }
    ...
    if (is_fdir) {
        if (flow_class == FLOW_GENERIC_CLASS) {
            ...
            if (ret) {
                avf_log_err(ad, "avf fdir parse generic pattern failed: %s", error.message);
                rv = VNET_FLOW_ERROR_NOT_SUPPORTED;
                goto done;
            }
        }
        ...
        if (ret) {
            avf_log_err(ad, "avf fdir rule create failed: %s", avf_fdir_prgm_error_decode(ret));
            rv = VNET_FLOW_ERROR_INTERNAL;
            goto done;
        }
        else {
        ...
        ret = avf_rss_parse_pattern_action(avf_items, avf_actions, rss_cfg, &error);
        if (ret) {
            avf_log_err(ad, "avf rss parse pattern action failed: %s", error.message);
            rv = VNET_FLOW_ERROR_NOT_SUPPORTED;
            goto done;
        }
        /* create flow rule, save rule */
...
int avf_flow_ops_fn(vnet_main_t *vm, vnet_flow_dev_op_t op, u32 dev_instance, u32 flow_index, uword *private_data)
{
    ...
    if (op == VNET_FLOW_DEV_OP_ADD_FLOW) {
        ...
        if (flow->actions & (VNET_FLOW_ACTION_MARK | VNET_FLOW_ACTION_REDIRECT_TO_NODE | VNET_FLOW_ACTION_BUFFER_ADVANCE)) {
            /* reserve slot 0 */
            if (ad->flow_lookup_entries == 0) pool_get_aligned(ad->flow_lookup_entries, fle, CLIB_CACHE_LINE_BYTES);
            pool_get_aligned(ad->flow_lookup_entries, fle, CLIB_CACHE_LINE_BYTES);
        ...
        switch (flow->type) {
            ...
            case VNET_FLOW_TYPE_GENERIC:
                if ((rv = avf_flow_add(dev_instance, flow, fe))) goto done;
                break;
            default:
                rv = VNET_FLOW_ERROR_NOT_SUPPORTED;
                goto done;
        ...
        *private_data = fe - ad->flow_entries;
        ...
        if (fe->flow_type_flag) {
            ...
            if (rv) return VNET_FLOW_ERROR_INTERNAL;
        }
        else {
...
disable_rx_offload:
    if ((ad->flags & AVF_DEVICE_F_RX_FLOW_OFFLOAD) != 0 && pool_elts(ad->flow_entries) == 0) {
        ad->flags &= ~AVF_DEVICE_F_RX_FLOW_OFFLOAD;
    }
