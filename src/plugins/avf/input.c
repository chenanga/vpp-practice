...
static_always_inline void avf_rxq_refill(vlib_main_t *vm, vlib_node_runtime_t *node, avf_rxq_t *rxq, int use_va_dma)
{
    ...
    if (PREDICT_TRUE(n_refill <= AVF_INPUT_REFILL_TRESHOLD)) return;

    slot = (rxq->next - n_refill - 1) & mask;

    n_refill &= ~7; /* round to 8 */
    n_alloc = vlib_buffer_alloc_to_ring_from_pool(vm, rxq->bufs, slot, size, n_refill, rxq->buffer_pool_index);

    if (PREDICT_FALSE(n_alloc != n_refill)) {
        vlib_error_count(vm, node->node_index, AVF_INPUT_ERROR_BUFFER_ALLOC, 1);
        if (n_alloc) vlib_buffer_free_from_ring(vm, rxq->bufs, slot, size, n_alloc);
        return;
    }
    ...
    while (n_alloc >= 8) {
        ...
        if (use_va_dma) {
            vlib_get_buffers_with_offset(vm, rxq->bufs + slot, p, 8, sizeof(vlib_buffer_t));
...
static_always_inline uword avf_rx_attach_tail(vlib_main_t *vm, vlib_buffer_t *bt, vlib_buffer_t *b, u64 qw1, avf_rx_tail_t *t)
{
    ...
    if (qw1 & AVF_RXD_STATUS_EOP) return 0;
...
static_always_inline void avf_process_flow_offload(avf_device_t *ad, avf_per_thread_data_t *ptd, uword n_rx_packets)
{
    ...
    for (n = 0; n < n_rx_packets; n++) {
        if ((ptd->qw1s[n] & AVF_RXD_STATUS_FLM) == 0) continue;
...
avf_process_rx_burst(vlib_main_t *vm, vlib_node_runtime_t *node, avf_per_thread_data_t *ptd, u32 n_left, int maybe_multiseg)
{
    vlib_buffer_t   bt;
    ...
    while (n_left) {
        ...
        if (maybe_multiseg) n_rx_bytes += avf_rx_attach_tail(vm, &bt, b[0], qw1[0], tail + 0);
...
static_always_inline uword
avf_device_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, avf_device_t *ad, u16 qid, int with_flows)
{
    ...
    d = fd + next;
    if ((d->qword[1] & AVF_RXD_STATUS_DD) == 0) goto done;

    if (PREDICT_FALSE(ad->per_interface_next_index != ~0)) next_index = ad->per_interface_next_index;

    if (PREDICT_FALSE(vnet_device_input_have_features(ad->sw_if_index))) vnet_feature_start_device_input(ad->sw_if_index, &next_index, bt);
...
#ifdef CLIB_HAVE_VEC256
        if (n_rx_packets >= AVF_RX_VECTOR_SZ - 4 || next >= size - 4) goto one_by_one;
    ...
    one_by_one:
        ...
        if (PREDICT_FALSE(avf_rxd_is_not_eop(d))) {
            ...
            do {
                ...
                if (avf_rxd_is_not_dd(td)) goto no_more_desc;
...
no_more_desc:
    ...
    if (PREDICT_FALSE((n_trace = vlib_get_trace_count(vm, node)))) {
        ...
        while (n_trace && n_left) {
            ...
            if (PREDICT_TRUE(vlib_trace_buffer(vm, node, next_index, b, /* follow_chain */ 0))) {
                avf_input_trace_t *tr = vlib_add_trace(vm, node, b, sizeof(*tr));
                tr->next_index        = next_index;
                tr->qid               = qid;
                tr->hw_if_index       = ad->hw_if_index;
                tr->qw1s[0]           = ptd->qw1s[i];
                tr->flow_id           = (tr->qw1s[0] & AVF_RXD_STATUS_FLM) ? ptd->flow_ids[i] : 0;
    ...
    if (PREDICT_FALSE(with_flows && (or_qw1 & AVF_RXD_STATUS_FLM))) {
        ...
        if (PREDICT_TRUE(next_index == VNET_DEVICE_INPUT_NEXT_ETHERNET_INPUT)) {
            ...
            if ((or_qw1 & AVF_RXD_ERROR_IPE) == 0) f->flags |= ETH_INPUT_FRAME_F_IP4_CKSUM_OK;
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    for (int i = 0; i < vec_len(pv); i++) {
        ...
        if ((ad->flags & AVF_DEVICE_F_ADMIN_UP) == 0) continue;
        if (PREDICT_FALSE(ad->flags & AVF_DEVICE_F_RX_FLOW_OFFLOAD))
            n_rx += avf_device_input_inline(vm, node, frame, ad, pv[i].queue_id, 1);
        else
            n_rx += avf_device_input_inline(vm, node, frame, ad, pv[i].queue_id, 0);
    }
