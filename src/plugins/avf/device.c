...
static inline void avf_irq_0_set_state(avf_device_t *ad, avf_irq_state_t state)
{
    ...
    if (state == AVF_IRQ_STATE_DISABLED) return;
...
static inline void avf_irq_n_set_state(avf_device_t *ad, u8 line, avf_irq_state_t state)
{
    u32 dyn_ctln = 0;
...
clib_error_t *avf_aq_desc_enq(vlib_main_t *vm, avf_device_t *ad, avf_aq_desc_t *dt, void *data, int len)
{
    ...
    if (len) d->datalen = len;
    if (len) {
        u64 pa;
        pa         = ad->atq_bufs_pa + ad->atq_next_slot * AVF_MBOX_BUF_SZ;
        d->addr_hi = (u32) (pa >> 32);
        d->addr_lo = (u32) pa;
        clib_memcpy_fast(ad->atq_bufs + ad->atq_next_slot * AVF_MBOX_BUF_SZ, data, len);
        d->flags |= AVF_AQ_F_BUF;
    }

    if (ad->flags & AVF_DEVICE_F_ELOG) clib_memcpy_fast(&dc, d, sizeof(avf_aq_desc_t));
...
retry:
    ...
    if (((d->flags & AVF_AQ_F_DD) == 0) || ((d->flags & AVF_AQ_F_CMP) == 0)) {
        ...
        if (t > AVF_AQ_ENQ_MAX_WAIT_TIME) {
            ...
            err = clib_error_return(0, "adminq enqueue timeout [opcode 0x%x]", d->opcode);
            goto done;
        }
        suspend_time *= 2;
        goto retry;
    ...
    if (d->flags & AVF_AQ_F_ERR)
        return clib_error_return(0,
                                 "adminq enqueue error [opcode 0x%x, retval "
                                 "%d]",
                                 d->opcode,
                                 d->retval);
...
clib_error_t *avf_rxq_init(vlib_main_t *vm, avf_device_t *ad, u16 qid, u16 rxq_size)
{
    ...
    rxq->size  = rxq_size;
    rxq->next  = 0;
    rxq->descs = vlib_physmem_alloc_aligned_on_numa(vm, rxq->size * sizeof(avf_rx_desc_t), 2 * CLIB_CACHE_LINE_BYTES, ad->numa_node);

    rxq->buffer_pool_index = vlib_buffer_pool_get_default_for_numa(vm, ad->numa_node);

    if (rxq->descs == 0) return vlib_physmem_last_error(vm);

    if ((err = vlib_pci_map_dma(vm, ad->pci_dev_handle, (void *) rxq->descs))) return err;
...
clib_error_t *avf_txq_init(vlib_main_t *vm, avf_device_t *ad, u16 qid, u16 txq_size)
{
    ...
    avf_txq_t    *txq;
    u16           n;
    u8            bpi = vlib_buffer_pool_get_default_for_numa(vm, ad->numa_node);
    ...
    txq->descs = vlib_physmem_alloc_aligned_on_numa(vm, txq->size * sizeof(avf_tx_desc_t), 2 * CLIB_CACHE_LINE_BYTES, ad->numa_node);
    if (txq->descs == 0) return vlib_physmem_last_error(vm);

    if ((err = vlib_pci_map_dma(vm, ad->pci_dev_handle, (void *) txq->descs))) return err;
...
static inline uword avf_dma_addr(vlib_main_t *vm, avf_device_t *ad, void *p)
{
    return (ad->flags & AVF_DEVICE_F_VA_DMA) ? pointer_to_uword(p) : vlib_physmem_get_pa(vm, p);
}
...
clib_error_t *avf_send_to_pf(vlib_main_t *vm, avf_device_t *ad, virtchnl_ops_t op, void *in, int in_len, void *out, int out_len)
{
     ...
     * is initialized */
    ASSERT((ad->flags & AVF_DEVICE_F_INITIALIZED) == 0 || vlib_get_current_process_node_index(vm) == avf_process_node.index);
    ...
    if ((err = avf_aq_desc_enq(vm, ad, &dt, in, in_len))) return err;
...
retry:
    ...
    if (d->v_opcode == VIRTCHNL_OP_EVENT) {
        ...
        if ((d->datalen != sizeof(virtchnl_pf_event_t)) || ((d->flags & AVF_AQ_F_BUF) == 0)) return clib_error_return(0, "event message error");
    ...
    if (d->v_retval) {
        err = clib_error_return(0, "error [v_opcode = %u, v_retval %d]", d->v_opcode, d->v_retval);
        goto done;
    }
...
clib_error_t *avf_op_version(vlib_main_t *vm, avf_device_t *ad, virtchnl_version_info_t *ver)
{
    ...
    err = avf_send_to_pf(vm, ad, VIRTCHNL_OP_VERSION, &myver, sizeof(virtchnl_version_info_t), ver, sizeof(virtchnl_version_info_t));
...
{
    clib_error_t *err    = 0;
    u32           bitmap = (VIRTCHNL_VF_OFFLOAD_L2 | VIRTCHNL_VF_OFFLOAD_RSS_PF | VIRTCHNL_VF_OFFLOAD_WB_ON_ITR | VIRTCHNL_VF_OFFLOAD_VLAN
                  | VIRTCHNL_VF_OFFLOAD_RX_POLLING | VIRTCHNL_VF_CAP_ADV_LINK_SPEED | VIRTCHNL_VF_OFFLOAD_FDIR_PF | VIRTCHNL_VF_OFFLOAD_ADV_RSS_PF
                  | VIRTCHNL_VF_OFFLOAD_VLAN_V2);

    avf_log_debug(ad, "get_vf_resources: bitmap 0x%x (%U)", bitmap, format_avf_vf_cap_flags, bitmap);
    err = avf_send_to_pf(vm, ad, VIRTCHNL_OP_GET_VF_RESOURCES, &bitmap, sizeof(u32), res, sizeof(virtchnl_vf_resource_t));
...
clib_error_t *avf_op_config_rss_lut(vlib_main_t *vm, avf_device_t *ad)
{
    ...
    return avf_send_to_pf(vm, ad, VIRTCHNL_OP_CONFIG_RSS_LUT, msg, msg_len, 0, 0);
}
...
clib_error_t *avf_op_config_rss_key(vlib_main_t *vm, avf_device_t *ad)
{
    ...
    if (sizeof(rss_key_default) != ad->rss_key_size)
        return clib_error_create("unsupported RSS key size (expected %d, got %d)", sizeof(rss_key_default), ad->rss_key_size);
    ...
    avf_log_debug(ad, "config_rss_key: vsi_id %u rss_key_size %u key 0x%U", rk->vsi_id, rk->key_len, format_hex_bytes_no_wrap, rk->key, rk->key_len);

    return avf_send_to_pf(vm, ad, VIRTCHNL_OP_CONFIG_RSS_KEY, msg, msg_len, 0, 0);
}
...
clib_error_t *avf_op_disable_vlan_stripping(vlib_main_t *vm, avf_device_t *ad)
{
    ...
    return avf_send_to_pf(vm, ad, VIRTCHNL_OP_DISABLE_VLAN_STRIPPING, 0, 0, 0, 0);
}
...
clib_error_t *avf_op_config_promisc_mode(vlib_main_t *vm, avf_device_t *ad, int is_enable)
{
    ...
    if (is_enable) pi.flags = FLAG_VF_UNICAST_PROMISC | FLAG_VF_MULTICAST_PROMISC;
    ...
    return avf_send_to_pf(vm, ad, VIRTCHNL_OP_CONFIG_PROMISCUOUS_MODE, &pi, sizeof(virtchnl_promisc_info_t), 0, 0);
}
...
clib_error_t *avf_op_config_vsi_queues(vlib_main_t *vm, avf_device_t *ad)
{
    ...
    int                               msg_len = sizeof(virtchnl_vsi_queue_config_info_t) + n_qp * sizeof(virtchnl_queue_pair_info_t);
    u8                                msg[msg_len];
    ...
    avf_log_debug(ad, "config_vsi_queues: vsi_id %u num_queue_pairs %u", ad->vsi_id, ci->num_queue_pairs);
    ...
    for (i = 0; i < n_qp; i++) {
        ...
        avf_log_debug(ad,
                      "config_vsi_queues_tx[%u]: ring_len %u "
                      "dma_ring_addr 0x%llx",
                      i,
                      txq->ring_len,
                      txq->dma_ring_addr);
    }

    return avf_send_to_pf(vm, ad, VIRTCHNL_OP_CONFIG_VSI_QUEUES, msg, msg_len, 0, 0);
}

clib_error_t *avf_op_config_irq_map(vlib_main_t *vm, avf_device_t *ad)
{
    int                      msg_len = sizeof(virtchnl_irq_map_info_t) + (ad->n_rx_irqs) * sizeof(virtchnl_vector_map_t);
    u8                       msg[msg_len];
    ...
    for (int i = 0; i < ad->n_rx_irqs; i++) {
        ...
        avf_log_debug(ad,
                      ...
                      ad->n_rx_irqs - 1,
                      ad->vsi_id,
                      imi->vecmap[i].vector_id,
                      imi->vecmap[i].rxq_map);
    }

    return avf_send_to_pf(vm, ad, VIRTCHNL_OP_CONFIG_IRQ_MAP, msg, msg_len, 0, 0);
}

clib_error_t *avf_op_add_del_eth_addr(vlib_main_t *vm, avf_device_t *ad, u8 count, u8 *macs, int is_add)
{
    int                         msg_len = sizeof(virtchnl_ether_addr_list_t) + count * sizeof(virtchnl_ether_addr_t);
    u8                          msg[msg_len];
    ...
    avf_log_debug(ad, "add_del_eth_addr: vsi_id %u num_elements %u is_add %u", ad->vsi_id, al->num_elements, is_add);

    for (i = 0; i < count; i++) {
        clib_memcpy_fast(&al->list[i].addr, macs + i * 6, 6);
        avf_log_debug(ad, "add_del_eth_addr[%u]: %U", i, format_ethernet_address, &al->list[i].addr);
    }
    return avf_send_to_pf(vm, ad, is_add ? VIRTCHNL_OP_ADD_ETH_ADDR : VIRTCHNL_OP_DEL_ETH_ADDR, msg, msg_len, 0, 0);
}
...
clib_error_t *avf_op_enable_queues(vlib_main_t *vm, avf_device_t *ad, u32 rx, u32 tx)
{
    ...
    avf_log_debug(ad, "enable_queues: vsi_id %u rx_queues %u tx_queues %u", ad->vsi_id, qs.rx_queues, qs.tx_queues);
    ...
    return avf_send_to_pf(vm, ad, VIRTCHNL_OP_ENABLE_QUEUES, &qs, sizeof(virtchnl_queue_select_t), 0, 0);
}
...
clib_error_t *avf_op_get_stats(vlib_main_t *vm, avf_device_t *ad, virtchnl_eth_stats_t *es)
{
    ...
    err = avf_send_to_pf(vm, ad, VIRTCHNL_OP_GET_STATS, &qs, sizeof(virtchnl_queue_select_t), es, sizeof(virtchnl_eth_stats_t));

    avf_stats_log_debug(ad, "get_stats: vsi_id %u\n  %U", ad->vsi_id, format_avf_eth_stats, es);
...
clib_error_t *avf_op_get_offload_vlan_v2_caps(vlib_main_t *vm, avf_device_t *ad, virtchnl_vlan_caps_t *vc)
{
    ...
    err = avf_send_to_pf(vm, ad, VIRTCHNL_OP_GET_OFFLOAD_VLAN_V2_CAPS, 0, 0, vc, sizeof(virtchnl_vlan_caps_t));

    avf_log_debug(ad, "get_offload_vlan_v2_caps:\n%U%U", format_white_space, 16, format_avf_vlan_caps, vc);
...
clib_error_t *avf_op_disable_vlan_stripping_v2(vlib_main_t *vm, avf_device_t *ad, u32 outer, u32 inner)
{
    ...
    avf_log_debug(ad, "disable_vlan_stripping_v2: outer: %U, inner %U", format_avf_vlan_support, outer, format_avf_vlan_support, inner);

    return avf_send_to_pf(vm, ad, VIRTCHNL_OP_DISABLE_VLAN_STRIPPING_V2, &vs, sizeof(virtchnl_vlan_setting_t), 0, 0);
}
...
clib_error_t *avf_device_reset(vlib_main_t *vm, avf_device_t *ad)
{
    ...
    if ((error = avf_aq_desc_enq(vm, ad, &d, 0, 0))) return error;
...
clib_error_t *avf_request_queues(vlib_main_t *vm, avf_device_t *ad, u16 num_queue_pairs)
{
    ...
    error =
        avf_send_to_pf(vm, ad, VIRTCHNL_OP_REQUEST_QUEUES, &res_req, sizeof(virtchnl_vf_res_request_t), &res_req, sizeof(virtchnl_vf_res_request_t));
     ...
     */
    if (!error) {
        return clib_error_return(0, "requested more than %u queue pairs", res_req.num_queue_pairs);
    }
...
retry:
    ...
    if ((rstat == VIRTCHNL_VFR_COMPLETED) || (rstat == VIRTCHNL_VFR_VFACTIVE)) goto done;
...
clib_error_t *avf_device_init(vlib_main_t *vm, avf_main_t *am, avf_device_t *ad, avf_create_if_args_t *args)
{
    ...
    if ((error = avf_request_queues(vm, ad, clib_max(txq_num, rxq_num)))) {
        ...
        if ((error = avf_device_reset(vm, ad))) return error;
    }
     ...
     * OP_VERSION
     */
    if ((error = avf_op_version(vm, ad, &ver))) return error;

    if (ver.major != VIRTCHNL_VERSION_MAJOR || ver.minor != VIRTCHNL_VERSION_MINOR)
        return clib_error_return(0,
     ...
     * OP_GET_VF_RESOURCES
     */
    if ((error = avf_op_get_vf_resources(vm, ad, &res))) return error;

    if (res.num_vsis != 1 || res.vsi_res[0].vsi_type != VIRTCHNL_VSI_SRIOV) return clib_error_return(0, "unexpected GET_VF_RESOURCE reply received");
    ...
    if (ad->max_vectors > ad->n_rx_queues) ad->flags |= AVF_DEVICE_F_RX_INT;
    ...
    if (ad->cap_flags & VIRTCHNL_VF_OFFLOAD_VLAN_V2) {
        ...
        if ((error = avf_op_get_offload_vlan_v2_caps(vm, ad, &vc))) return error;
        ...
        outer = vc.offloads.stripping_support.outer & VIRTCHNL_VLAN_TOGGLE ? outer : 0;
        inner = vc.offloads.stripping_support.inner & VIRTCHNL_VLAN_TOGGLE ? inner : 0;

        if ((outer || inner) && (error = avf_op_disable_vlan_stripping_v2(vm, ad, outer, inner))) return error;
    }
    ...
    for (i = 0; i < ad->n_rx_queues; i++)
        if ((error = avf_rxq_init(vm, ad, i, args->rxq_size))) return error;

    for (i = 0; i < ad->n_tx_queues; i++)
        if ((error = avf_txq_init(vm, ad, i, args->txq_size))) return error;

    if ((ad->cap_flags & VIRTCHNL_VF_OFFLOAD_RSS_PF) && (error = avf_op_config_rss_lut(vm, ad))) return error;

    if ((ad->cap_flags & VIRTCHNL_VF_OFFLOAD_RSS_PF) && (error = avf_op_config_rss_key(vm, ad))) return error;

    if ((error = avf_op_config_vsi_queues(vm, ad))) return error;

    if ((error = avf_op_config_irq_map(vm, ad))) return error;

    avf_irq_0_set_state(ad, AVF_IRQ_STATE_ENABLED);

    for (i = 0; i < ad->n_rx_irqs; i++)
        avf_irq_n_set_state(ad, i, wb_on_itr ? AVF_IRQ_STATE_WB_ON_ITR : AVF_IRQ_STATE_ENABLED);

    if ((error = avf_op_add_del_eth_addr(vm, ad, 1, ad->hwaddr, 1 /* add */))) return error;

    if ((error = avf_op_enable_queues(vm, ad, pow2_mask(ad->n_rx_queues), pow2_mask(ad->n_tx_queues)))) return error;
...
void avf_process_one_device(vlib_main_t *vm, avf_device_t *ad, int is_irq)
{
    ...
    if (ad->flags & AVF_DEVICE_F_ERROR) return;

    if ((ad->flags & AVF_DEVICE_F_INITIALIZED) == 0) return;

    ASSERT(ad->error == 0);

    /* do not process device in reset state */
    r = avf_get_u32(ad->bar0, AVFGEN_RSTAT);
    if (r != VIRTCHNL_VFR_VFACTIVE) return;
    ...
    if (is_irq == 0) avf_op_get_stats(vm, ad, &ad->eth_stats);

    vec_foreach (e, ad->events) {
        avf_log_debug(ad, "event: %s (%u) sev %d", virtchnl_event_names[e->event], e->event, e->severity);
        if (e->event == VIRTCHNL_EVENT_LINK_CHANGE) {
            ...
            if (ad->cap_flags & VIRTCHNL_VF_CAP_ADV_LINK_SPEED) mbps = e->event_data.link_event_adv.link_speed;
            ...
            avf_log_debug(ad, "event_link_change: status %d speed %u mbps", link_up, mbps);

            if (link_up && (ad->flags & AVF_DEVICE_F_LINK_UP) == 0) {
                ad->flags |= AVF_DEVICE_F_LINK_UP;
                flags |= (VNET_HW_INTERFACE_FLAG_FULL_DUPLEX | VNET_HW_INTERFACE_FLAG_LINK_UP);
                vnet_hw_interface_set_flags(vnm, ad->hw_if_index, flags);
                vnet_hw_interface_set_link_speed(vnm, ad->hw_if_index, (mbps == UINT32_MAX) ? UINT32_MAX : mbps * 1000);
                ad->link_speed = mbps;
            }
                                  ...
                                  u32                  status_len)
{
    ...
    avf_log_debug(ad, "avf_op_program_flow: vsi_id %u is_create %u", ad->vsi_id, is_create);

    switch (vc_op) {
        case VIRTCHNL_ADV_OP_ADD_FDIR_FILTER:
        case VIRTCHNL_ADV_OP_DEL_FDIR_FILTER:
            op = is_create ? VIRTCHNL_OP_ADD_FDIR_FILTER : VIRTCHNL_OP_DEL_FDIR_FILTER;
            break;
    ...
    return avf_send_to_pf(vm, ad, op, rule, rule_len, program_status, status_len);
}
...
static void avf_process_handle_request(vlib_main_t *vm, avf_process_req_t *req)
{
    ...
    if (req->type == AVF_PROCESS_REQ_ADD_DEL_ETH_ADDR)
        req->error = avf_op_add_del_eth_addr(vm, ad, 1, req->eth_addr, req->is_add);
        ...
        req->error = avf_op_program_flow(vm, ad, req->is_add, req->vc_op, req->rule, req->rule_len, req->program_status, req->status_len);
    else
        clib_panic("BUG: unknown avf proceess request type");

    if (req->calling_process_index != avf_process_node.index) vlib_process_signal_event(vm, req->calling_process_index, 0, 0);
}
...
static clib_error_t *avf_process_request(vlib_main_t *vm, avf_process_req_t *req)
{
    ...
    if (req->calling_process_index != avf_process_node.index) {
        vlib_process_signal_event_pointer(vm, avf_process_node.index, AVF_PROCESS_EVENT_REQ, req);

        vlib_process_wait_for_event_or_clock(vm, 5.0);

        if (vlib_process_get_events(vm, &event_data) != 0) clib_panic("avf process node failed to reply in 5 seconds");
        vec_free(event_data);
    }
    else
    ...
    while (1) {
        ...
        switch (event_type) {
            ...
            case AVF_PROCESS_EVENT_DELETE_IF:
                ...
                if (pool_elts(am->devices) < 1) enabled = 0;
                break;
...
static void avf_irq_0_handler(vlib_main_t *vm, vlib_pci_dev_handle_t h, u16 line)
{
    ...
    if (icr0 & (1 << 30)) vlib_process_signal_event(vm, avf_process_node.index, AVF_PROCESS_EVENT_AQ_INT, 0);
}
...
static void avf_irq_n_handler(vlib_main_t *vm, vlib_pci_dev_handle_t h, u16 line)
{
    ...
    if (ad->flags & AVF_DEVICE_F_RX_INT && rxq->int_mode) vnet_hw_if_rx_queue_set_int_pending(vnm, rxq->queue_index);
...
static u8 avf_validate_queue_size(avf_create_if_args_t *args)
{
    ...
    if ((args->rxq_size > AVF_QUEUE_SZ_MAX) || (args->txq_size > AVF_QUEUE_SZ_MAX)) {
        args->rv    = VNET_API_ERROR_INVALID_VALUE;
        args->error = clib_error_return(error, "queue size must not be greater than %u", AVF_QUEUE_SZ_MAX);
        return 1;
    }
    if ((args->rxq_size < AVF_QUEUE_SZ_MIN) || (args->txq_size < AVF_QUEUE_SZ_MIN)) {
        args->rv    = VNET_API_ERROR_INVALID_VALUE;
        args->error = clib_error_return(error, "queue size must not be smaller than %u", AVF_QUEUE_SZ_MIN);
        return 1;
    }
...
void avf_create_if(vlib_main_t *vm, avf_create_if_args_t *args)
{
    ...
    /* check input args */
    if (avf_validate_queue_size(args) != 0) return;
    ...
    adp[0] = ad = clib_mem_alloc_aligned(sizeof(avf_device_t), CLIB_CACHE_LINE_BYTES);
    ...
    if ((error = vlib_pci_device_open(vm, &args->addr, avf_pci_device_ids, &h))) {
    ...
    if ((error = vlib_pci_map_region(vm, h, 0, &ad->bar0))) goto error;

    ad->atq = vlib_physmem_alloc_aligned_on_numa(vm, sizeof(avf_aq_desc_t) * AVF_MBOX_LEN, CLIB_CACHE_LINE_BYTES, ad->numa_node);
    if (ad->atq == 0) {
        error = vlib_physmem_last_error(vm);
        goto error;
    }

    if ((error = vlib_pci_map_dma(vm, h, ad->atq))) goto error;

    ad->arq = vlib_physmem_alloc_aligned_on_numa(vm, sizeof(avf_aq_desc_t) * AVF_MBOX_LEN, CLIB_CACHE_LINE_BYTES, ad->numa_node);
    if (ad->arq == 0) {
        error = vlib_physmem_last_error(vm);
        goto error;
    }

    if ((error = vlib_pci_map_dma(vm, h, ad->arq))) goto error;

    ad->atq_bufs = vlib_physmem_alloc_aligned_on_numa(vm, AVF_MBOX_BUF_SZ * AVF_MBOX_LEN, CLIB_CACHE_LINE_BYTES, ad->numa_node);
    if (ad->atq_bufs == 0) {
        error = vlib_physmem_last_error(vm);
        goto error;
    }

    if ((error = vlib_pci_map_dma(vm, h, ad->atq_bufs))) goto error;

    ad->arq_bufs = vlib_physmem_alloc_aligned_on_numa(vm, AVF_MBOX_BUF_SZ * AVF_MBOX_LEN, CLIB_CACHE_LINE_BYTES, ad->numa_node);
    if (ad->arq_bufs == 0) {
        error = vlib_physmem_last_error(vm);
        goto error;
    }

    if ((error = vlib_pci_map_dma(vm, h, ad->arq_bufs))) goto error;

    if (vlib_pci_supports_virtual_addr_dma(vm, h)) ad->flags |= AVF_DEVICE_F_VA_DMA;

    if ((error = avf_device_init(vm, am, ad, args))) goto error;

    if ((error = vlib_pci_register_msix_handler(vm, h, 0, 1, &avf_irq_0_handler))) goto error;

    if ((error = vlib_pci_register_msix_handler(vm, h, 1, ad->n_rx_irqs, &avf_irq_n_handler))) goto error;

    if ((error = vlib_pci_enable_msix_irq(vm, h, 0, ad->n_rx_irqs + 1))) goto error;

    if ((error = vlib_pci_intr_enable(vm, h))) goto error;
    ...
    ethernet_set_flags(vnm, ad->hw_if_index, ETHERNET_INTERFACE_FLAG_DEFAULT_L3);
    ...
    vnet_hw_if_set_caps(vnm, ad->hw_if_index, VNET_HW_IF_CAP_INT_MODE | VNET_HW_IF_CAP_MAC_FILTER | VNET_HW_IF_CAP_TX_CKSUM | VNET_HW_IF_CAP_TCP_GSO);

    for (i = 0; i < ad->n_rx_queues; i++) {
        u32 qi, fi;
        qi = vnet_hw_if_register_rx_queue(vnm, ad->hw_if_index, i, VNET_HW_IF_RXQ_THREAD_ANY);
    ...
    if (pool_elts(am->devices) == 1) vlib_process_signal_event(vm, avf_process_node.index, AVF_PROCESS_EVENT_START, 0);
...
static clib_error_t *avf_interface_admin_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    ...
    if (ad->flags & AVF_DEVICE_F_ERROR) return clib_error_return(0, "device is in error state");
...
static clib_error_t *avf_interface_rx_mode_change(vnet_main_t *vnm, u32 hw_if_index, u32 qid, vnet_hw_if_rx_mode mode)
{
    ...
    if (mode == VNET_HW_IF_RX_MODE_POLLING) {
        ...
        if (rxq->int_mode == 1) return 0;
        if (ad->n_rx_irqs != ad->n_rx_queues) return clib_error_return(0, "not enough interrupt lines");
        rxq->int_mode = 1;
        avf_irq_n_set_state(ad, qid, AVF_IRQ_STATE_ENABLED);
    }
...
static void avf_set_interface_next_node(vnet_main_t *vnm, u32 hw_if_index, u32 node_index)
{
    ...
    ad->per_interface_next_index = vlib_node_add_next(vlib_get_main(), avf_input_node.index, node_index);
}
...
static void avf_clear_hw_interface_counters(u32 instance)
{
    ...
    clib_memcpy_fast(&ad->last_cleared_eth_stats, &ad->eth_stats, sizeof(ad->eth_stats));
}

clib_error_t *avf_program_flow(u32 dev_instance, int is_add, enum virthnl_adv_ops vc_op, u8 *rule, u32 rule_len, u8 *program_status, u32 status_len)
{
    vlib_main_t      *vm = vlib_get_main();
    avf_process_req_t req;
...
clib_error_t *avf_init(vlib_main_t *vm)
{
    ...
    vec_validate_aligned(am->per_thread_data, tm->n_vlib_mains - 1, CLIB_CACHE_LINE_BYTES);
