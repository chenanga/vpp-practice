...
void avf_elog_aq_enq_req(avf_device_t *ad, avf_aq_desc_t *d)
{
    if (d->opcode == 0x801) /* send_to_pf */
    {
        ELOG_TYPE_DECLARE(el) = {
            .format =
                "avf[%d] aq_enq_req: send_to_pf flags 0x%x datalen %d "
                "v_opcode %s (%d)",
...
#undef _
        ...
        ELOG_TYPE_DECLARE(el) = {.format = "avf[%d] aq_enq_req: opcode 0x%x flags 0x%x datalen %d", .format_args = "i4i2i2i2"};
...
{
    ELOG_TYPE_DECLARE(el) = {.format = "avf[%d] aq_enq_resp: flags 0x%x retval %d", .format_args = "i4i2i2"};
...
void avf_elog_arq_desc(avf_device_t *ad, avf_aq_desc_t *d)
{
    if (d->opcode == 0x802) {
        ELOG_TYPE_DECLARE(el) = {
            .format =
                "avf[%d] arq_desc: msg_from_pf flags 0x%x retval %d "
...
#undef _
        ...
        ELOG_TYPE_DECLARE(el) = {.format = "avf[%d] arq_desc: flags 0x%x retval %d opcode 0x%x", .format_args = "i4i2i2i2"};
...
{
    if (register_name_by_addr) return;
    ...
    for (int i = 0; i < 16; i++) {
        hash_set(register_name_by_addr, AVFINT_DYN_CTLN(i), format(0, "INT_DYN_CTLN(%u)%c", i, 0));
        hash_set(register_name_by_addr, AVF_QTX_TAIL(i), format(0, "QTX_TAIL(%u)%c", i, 0));
        hash_set(register_name_by_addr, AVF_QRX_TAIL(i), format(0, "QRX_TAIL(%u)%c", i, 0));
    }
}
