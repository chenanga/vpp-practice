...
u8 *format_avf_device_name(u8 *s, va_list *args)
{
    ...
    if (ad->name) return format(s, "%s", ad->name);

    s = format(s, "avf-%x/%x/%x/%x", addr->domain, addr->bus, addr->slot, addr->function);
    return s;
}
...
#define _(a, b, c) \
    if (ad->flags & (1 << a)) t = format(t, "%s%s", t ? " " : "", c);
...
static u8 *format_virtchnl_link_speed(u8 *s, va_list *args)
{
    ...
    if (speed == 0) return format(s, "unknown");
#define _(a, b, c) else if (speed == VIRTCHNL_LINK_SPEED_##b) return format(s, c);
...
u8 *format_avf_device(u8 *s, va_list *args)
{
    ...
    s = format(s, "rx: queues %u, desc %u (min %u max %u)", ad->n_rx_queues, rxq->size, AVF_QUEUE_SZ_MIN, AVF_QUEUE_SZ_MAX);
    s = format(s,
               ...
               AVF_QUEUE_SZ_MIN,
               AVF_QUEUE_SZ_MAX);
    s = format(s, "\n%Uflags: %U", format_white_space, indent, format_avf_device_flags, ad);
    s = format(s, "\n%Ucapability flags: %U", format_white_space, indent, format_avf_vf_cap_flags, ad->cap_flags);
    s = format(s, "\n%U Rx Queue: Total Packets", format_white_space, indent + 4);
    for (idx = 0; idx < ad->n_rx_queues; idx++) {
        rxq = vec_elt_at_index(ad->rxqs, idx);
        s   = format(s, "\n%U %8u : %llu", format_white_space, indent + 4, idx, rxq->total_packets);
    }
    s = format(s, "\n%U Tx Queue: Total Packets\t Total Drops", format_white_space, indent + 4);
    for (idx = 0; idx < ad->n_tx_queues; idx++) {
        txq = vec_elt_at_index(ad->txqs, idx);
        s   = format(s, "\n%U %8u : %llu\t %llu", format_white_space, indent + 4, idx, txq->total_packets, txq->no_free_tx_count);
    }
    ...
    s = format(s,
               ...
               ad->rss_key_size,
               ad->rss_lut_size);
    s = format(s, "\n%Uspeed %U", format_white_space, indent, format_virtchnl_link_speed, ad->link_speed);
    if (ad->error) s = format(s, "\n%Uerror %U", format_white_space, indent, format_clib_error, ad->error);

#define _(c)                                            \
    if (ad->eth_stats.c - ad->last_cleared_eth_stats.c) \
        a = format(a, "\n%U%-20U %u", format_white_space, indent + 2, format_c_identifier, #c, ad->eth_stats.c - ad->last_cleared_eth_stats.c);
...
u8 *format_avf_input_trace(u8 *s, va_list *args)
{
    ...
    } while ((t->qw1s[i++] & AVF_RXD_STATUS_EOP) == 0 && i < AVF_RX_MAX_DESC_IN_CHAIN);
...
#undef _
    ...
    if (v == VIRTCHNL_VLAN_UNSUPPORTED) return format(s, "unsupported");
...
{
    virtchnl_vlan_supported_caps_t *sc     = va_arg(*args, virtchnl_vlan_supported_caps_t *);
    u32                             indent = format_get_indent(s);

    s = format(s, "outer: %U", format_avf_vlan_support, sc->outer);
    s = format(s, "\n%Uinner: %U", format_white_space, indent, format_avf_vlan_support, sc->inner);
    return s;
}
...
u8 *format_avf_vlan_caps(u8 *s, va_list *args)
{
    ...
    s = format(s, "\n%U%U", format_white_space, indent + 4, format_avf_vlan_supported_caps, &vc->filtering.filtering_support);
    s = format(s, "\n%Uethertype-init: 0x%x", format_white_space, indent + 4, vc->filtering.ethertype_init);
    s = format(s, "\n%Umax-filters: %u", format_white_space, indent + 4, vc->filtering.max_filters);
    s = format(s, "\n%Uoffloads:", format_white_space, indent);
    s = format(s, "\n%Ustripping support:", format_white_space, indent + 2);
    s = format(s, "\n%U%U", format_white_space, indent + 4, format_avf_vlan_supported_caps, &vc->offloads.stripping_support);
    s = format(s, "\n%Uinserion support:", format_white_space, indent + 2);
    s = format(s, "\n%U%U", format_white_space, indent + 4, format_avf_vlan_supported_caps, &vc->offloads.insertion_support);
    s = format(s, "\n%Uethertype-init: 0x%x", format_white_space, indent + 4, vc->offloads.ethertype_init);
    s = format(s, "\n%Uethertype-match: 0x%x", format_white_space, indent + 4, vc->offloads.ethertype_match);
    return s;
}
