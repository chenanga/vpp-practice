...
#define avf_log_err(dev, f, ...) vlib_log(VLIB_LOG_LEVEL_ERR, avf_log.class, "%U: " f, format_vlib_pci_addr, &dev->pci_addr, ##__VA_ARGS__)

#define avf_log_warn(dev, f, ...) vlib_log(VLIB_LOG_LEVEL_WARNING, avf_log.class, "%U: " f, format_vlib_pci_addr, &dev->pci_addr, ##__VA_ARGS__)

#define avf_log_debug(dev, f, ...) vlib_log(VLIB_LOG_LEVEL_DEBUG, avf_log.class, "%U: " f, format_vlib_pci_addr, &dev->pci_addr, ##__VA_ARGS__)

#define avf_stats_log_debug(dev, f, ...) \
    vlib_log(VLIB_LOG_LEVEL_DEBUG, avf_stats_log.class, "%U: " f, format_vlib_pci_addr, &dev->pci_addr, ##__VA_ARGS__)
...
static inline u32 avf_get_u32(void *start, int offset)
{
    return *(u32 *) (((u8 *) start) + offset);
}

static inline u64 avf_get_u64(void *start, int offset)
{
    return *(u64 *) (((u8 *) start) + offset);
}

static inline u32 avf_get_u32_bits(void *start, int offset, int first, int last)
{
    u32 value = avf_get_u32(start, offset);
    if ((last == 0) && (first == 31)) return value;
    value >>= last;
    value &= (1 << (first - last + 1)) - 1;
    return value;
}

static inline u64 avf_get_u64_bits(void *start, int offset, int first, int last)
{
    u64 value = avf_get_u64(start, offset);
    if ((last == 0) && (first == 63)) return value;
    value >>= last;
    value &= (1 << (first - last + 1)) - 1;
    return value;
}

static inline void avf_set_u32(void *start, int offset, u32 value)
{
    (*(u32 *) (((u8 *) start) + offset)) = value;
}

static inline void avf_reg_write(avf_device_t *ad, u32 addr, u32 val)
{
    if (ad->flags & AVF_DEVICE_F_ELOG) avf_elog_reg(ad, addr, val, 0);
    __atomic_store_n((u32 *) ((u8 *) ad->bar0 + addr), val, __ATOMIC_RELEASE);
}

static inline u32 avf_reg_read(avf_device_t *ad, u32 addr)
{
    u32 val = *(volatile u32 *) (ad->bar0 + addr);

    if (ad->flags & AVF_DEVICE_F_ELOG) avf_elog_reg(ad, addr, val, 1);

    return val;
}

static inline void avf_reg_flush(avf_device_t *ad)
{
    avf_reg_read(ad, AVFGEN_RSTAT);
    asm volatile("" ::: "memory");
}

static inline void avf_tail_write(volatile u32 *addr, u32 val)
{
#ifdef __MOVDIRI__
    _mm_sfence();
