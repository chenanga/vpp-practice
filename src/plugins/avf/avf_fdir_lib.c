 ...
 */
static inline int fls_u32(u32 x)
{
    return (x == 0) ? 0 : 64 - count_leading_zeros(x);
}

static inline int ether_addr_is_zero(const struct avf_ether_addr *ea)
{
...
int avf_fdir_rcfg_set_hdr(struct avf_fdir_conf *rcfg, int layer, enum virtchnl_proto_hdr_type hdr)
{
    ...
    if (layer >= VIRTCHNL_MAX_NUM_PROTO_HDRS) return -1;
...
int avf_fdir_rcfg_set_field(struct avf_fdir_conf *rcfg, int layer, struct avf_flow_item *item, struct avf_flow_error *error)
{
    ...
    u8 ipv6_addr_mask[16] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
    ...
    switch (type) {
        case AVF_FLOW_ITEM_TYPE_ETH:
            ...
            if (eth_spec && eth_mask) {
                if (!ether_addr_is_zero(&eth_mask->src) || !ether_addr_is_zero(&eth_mask->dst)) {
                    ret = avf_flow_error_set(error, AVF_FAILURE, AVF_FLOW_ERROR_TYPE_ITEM, item, "Invalid MAC_addr mask.");
                    return ret;
                }

                if (eth_mask->type) {
                    if (eth_mask->type != 0xffff) {
                        ret = avf_flow_error_set(error, AVF_FAILURE, AVF_FLOW_ERROR_TYPE_ITEM, item, "Invalid type mask.");
                        return ret;
                    }
                }
            }
            ...
            if (eth_spec && eth_mask && eth_mask->type) {
                ...
                if (ether_type == AVF_ETHER_TYPE_IPV4 || ether_type == AVF_ETHER_TYPE_IPV6) {
                    ret = avf_flow_error_set(error, AVF_FAILURE, AVF_FLOW_ERROR_TYPE_ITEM, item, "Unsupported ether_type.");
                    return ret;
                }
        ...
        case AVF_FLOW_ITEM_TYPE_IPV4:
            ...
            if (ipv4_spec && ipv4_mask) {
                if (ipv4_mask->version_ihl || ipv4_mask->total_length || ipv4_mask->packet_id || ipv4_mask->fragment_offset
                    || ipv4_mask->hdr_checksum) {
                    ret = avf_flow_error_set(error, AVF_FAILURE, AVF_FLOW_ERROR_TYPE_ITEM, item, "Invalid IPv4 mask.");
                    return ret;
                }
        ...
        case AVF_FLOW_ITEM_TYPE_IPV6:
            ...
            if (ipv6_spec && ipv6_mask) {
                if (ipv6_mask->payload_len) {
                    ret = avf_flow_error_set(error, AVF_FAILURE, AVF_FLOW_ERROR_TYPE_ITEM, item, "Invalid IPv6 mask");
                    return ret;
                }

                if ((ipv6_mask->vtc_flow & clib_host_to_net_u32(AVF_IPV6_TC_MASK)) == (clib_host_to_net_u32(AVF_IPV6_TC_MASK))) {
                ...
                if (!clib_memcmp(ipv6_mask->src_addr, ipv6_addr_mask, sizeof(ipv6_mask->src_addr))) {
                    rcfg->input_set |= AVF_INSET_IPV6_SRC;
                    VIRTCHNL_ADD_PROTO_HDR_FIELD_BIT(hdr, IPV6, SRC);
                }
                if (!clib_memcmp(ipv6_mask->dst_addr, ipv6_addr_mask, sizeof(ipv6_mask->dst_addr))) {
        ...
        case AVF_FLOW_ITEM_TYPE_UDP:
            ...
            if (udp_spec && udp_mask) {
                if (udp_mask->dgram_len || udp_mask->dgram_cksum) {
                    ret = avf_flow_error_set(error, AVF_FAILURE, AVF_FLOW_ERROR_TYPE_ITEM, item, "Invalid UDP mask");
                    return ret;
                };
        ...
        case AVF_FLOW_ITEM_TYPE_TCP:
            ...
            if (tcp_spec && tcp_mask) {
                if (tcp_mask->sent_seq || tcp_mask->recv_ack || tcp_mask->data_off || tcp_mask->tcp_flags || tcp_mask->rx_win || tcp_mask->cksum
                    || tcp_mask->tcp_urp) {
                    ret = avf_flow_error_set(error, AVF_FAILURE, AVF_FLOW_ERROR_TYPE_ITEM, item, "Invalid TCP mask");
                    return ret;
                }
        ...
        case AVF_FLOW_ITEM_TYPE_GTPU:
            ...
            if (gtp_spec && gtp_mask) {
                if (gtp_mask->v_pt_rsv_flags || gtp_mask->msg_type || gtp_mask->msg_len) {
                    ret = avf_flow_error_set(error, AVF_FAILURE, AVF_FLOW_ERROR_TYPE_ITEM, item, "Invalid GTP mask");
                    return ret;
                }
        ...
        default:
            ret = avf_flow_error_set(error, AVF_FAILURE, AVF_FLOW_ERROR_TYPE_ITEM, item, "Invalid pattern item.");
            return ret;
    }
...
{
    if (act_idx >= VIRTCHNL_MAX_NUM_ACTIONS) return -AVF_FAILURE;
...
int avf_fdir_parse_action_qregion(struct avf_fdir_conf *rcfg, const struct avf_flow_action *act, int act_idx, struct avf_flow_error *error)
{
    ...
    if (rss->queue_num <= 1) {
        ret = avf_flow_error_set(error, AVF_FAILURE, AVF_FLOW_ERROR_TYPE_ACTION, act, "Queue region size can't be 0 or 1.");
        return ret;
    }
    ...
    for (i = 0; i < rss->queue_num - 1; i++) {
        if (rss->queue[i + 1] != rss->queue[i] + 1) {
            ret = avf_flow_error_set(error, AVF_FAILURE, AVF_FLOW_ERROR_TYPE_ACTION, act, "Discontinuous queue region");
            return ret;
        }
    }

    if (rss->queue[rss->queue_num - 1] >= rcfg->nb_rx_queues) {
        ret = avf_flow_error_set(error, AVF_FAILURE, AVF_FLOW_ERROR_TYPE_ACTION, act, "Invalid queue region indexes.");
        return ret;
    }

    if (!(is_pow2(rss->queue_num) && rss->queue_num <= AVF_FDIR_MAX_QREGION_SIZE)) {
        ret = avf_flow_error_set(error,
                                 AVF_FAILURE,
...
int avf_fdir_rcfg_validate(struct avf_flow_vc_ctx *ctx, struct avf_fdir_conf *rcfg)
{
    int ret;
    ...
    ret = ctx->vc_op(ctx->vc_hdl, VIRTCHNL_ADV_OP_ADD_FDIR_FILTER, &rcfg->add_fltr, sizeof(rcfg->add_fltr), &fdir_ret, sizeof(fdir_ret));
...
int avf_fdir_rule_create(struct avf_flow_vc_ctx *ctx, struct avf_fdir_conf *rcfg)
{
    int ret;
    rcfg->add_fltr.vsi_id        = rcfg->vsi;
...
int avf_fdir_rule_destroy(struct avf_flow_vc_ctx *ctx, struct avf_fdir_conf *rcfg)
{
    int                      ret;
    ...
    ret = ctx->vc_op(ctx->vc_hdl, VIRTCHNL_ADV_OP_DEL_FDIR_FILTER, &rcfg->del_fltr, sizeof(rcfg->del_fltr), &fdir_ret, sizeof(fdir_ret));
        ...
        switch (actions->type) {
            ...
            case AVF_FLOW_ACTION_TYPE_QUEUE:
                ...
                if (act_q->index >= rcfg->nb_rx_queues) {
                    ret = avf_flow_error_set(error, AVF_FAILURE, AVF_FLOW_ERROR_TYPE_ACTION, actions, "Invalid queue for FDIR.");
                    return -AVF_FAILURE;
                }
            ...
            default:
                ret = avf_flow_error_set(error, AVF_FAILURE, AVF_FLOW_ERROR_TYPE_ACTION, actions, "Invalid action.");
                return ret;
        }
    }

    if (dest_num >= 2) {
        ret = avf_flow_error_set(error, AVF_FAILURE, AVF_FLOW_ERROR_TYPE_ACTION, actions, "Unsupported action combination");
        return ret;
    }

    if (mark_num >= 2) {
        ret = avf_flow_error_set(error, AVF_FAILURE, AVF_FLOW_ERROR_TYPE_ACTION, actions, "Too many mark actions");
        return ret;
    }

    if (dest_num + mark_num == 0) {
        ret = avf_flow_error_set(error, AVF_FAILURE, AVF_FLOW_ERROR_TYPE_ACTION, actions, "Empty action");
        return ret;
    }
...
int avf_fdir_parse_generic_pattern(struct avf_fdir_conf *rcfg, struct avf_flow_item avf_items[], struct avf_flow_error *error)
{
...
char *avf_fdir_prgm_error_decode(int err_no)
{
    ...
    if (err_no >= VIRTCHNL_FDIR_FAILURE_MAX) return "Failed to program the rule due to other reasons";
