...
static_always_inline u64 avf_tx_prepare_cksum(vlib_buffer_t *b, u8 is_tso)
{
    ...
    flags |= AVF_TXD_OFFSET_MACLEN(l2_len) | AVF_TXD_OFFSET_IPLEN(l3_len) | AVF_TXD_OFFSET_L4LEN(l4_len);
...
next:
    ...
    d[0].qword[0]  = 0;
    d[0].qword[1]  = AVF_TXD_DTYP_CTX | AVF_TXD_CTX_CMD_TSO | AVF_TXD_CTX_SEG_MSS(vnet_buffer2(b)->gso_size) | AVF_TXD_CTX_SEG_TLEN(tlen);
    return bi[0];
}
...
avf_tx_prepare(vlib_main_t *vm, vlib_node_runtime_t *node, avf_txq_t *txq, u32 *buffers, u32 n_packets, u16 *n_enq_descs, int use_va_dma)
{
    ...
    one_by_one:
        ...
        if (PREDICT_TRUE((flags & (VLIB_BUFFER_NEXT_PRESENT | VNET_BUFFER_F_GSO)) == 0)) {
            ...
            if (PREDICT_FALSE(flags & VNET_BUFFER_F_OFFLOAD)) cmd |= avf_tx_prepare_cksum(b[0], 0 /* is_tso */);
            ...
            if (flags & VNET_BUFFER_F_GSO) {
                ...
                vlib_error_count(vm, node->node_index, AVF_TX_ERROR_SEGMENT_SIZE_EXCEEDED, 1);
                n_packets_left -= 1;
                buffers += 1;
                continue;
            }
...
retry:
    ...
    if (txq->n_enqueued) {
        ...
        while (1) {
            ...
            if (avf_tx_desc_get_dtyp(txq->descs + slot[0]) != 0x0F) break;
        ...
        if (complete_slot >= 0) {
            ...
            vlib_buffer_free_from_ring_no_next(vm, txq->bufs, first, txq->size, n_free);
        }
    }
    ...
    if (n_desc) {
        if (PREDICT_TRUE(next + n_desc <= txq->size)) {
            ...
            avf_tx_copy_desc(txq->descs, txq->tmp_descs + n_not_wrap, n_desc - n_not_wrap);
            vlib_buffer_copy_indices(txq->bufs + next, txq->tmp_bufs, n_not_wrap);
            vlib_buffer_copy_indices(txq->bufs, txq->tmp_bufs + n_not_wrap, n_desc - n_not_wrap);
        }
    ...
    if (n_left) {
        ...
        vlib_error_count(vm, node->node_index, AVF_TX_ERROR_NO_FREE_SLOTS, n_left);
