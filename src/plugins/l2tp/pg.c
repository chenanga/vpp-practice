...
static inline void pg_l2tp_header_init(pg_l2tp_header_t *e)
{
...
uword unformat_pg_l2tp_header(unformat_input_t *input, va_list *args)
{
    ...
    h = pg_create_edit_group(s, sizeof(h[0]), sizeof(l2tpv3_header_t) - sizeof(u32), &group_index);
    ...
    if (unformat(input, "l2_sublayer")) {
        ...
        if (!unformat_user(input, unformat_pg_edit, unformat_pg_number, &h2->l2_sublayer)) {
            goto done;
        }
    }
