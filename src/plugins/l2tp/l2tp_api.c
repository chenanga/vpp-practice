...
static void send_sw_if_l2tpv3_tunnel_details(vpe_api_main_t *am, vl_api_registration_t *reg, l2t_session_t *s, l2t_main_t *lm, u32 context)
{
    ...
    if_name = format(if_name, "%U", format_vnet_sw_interface_name, lm->vnet_main, si);

    mp = vl_msg_api_alloc(sizeof(*mp));
    clib_memset(mp, 0, sizeof(*mp));
    mp->_vl_msg_id = ntohs(VL_API_SW_IF_L2TPV3_TUNNEL_DETAILS + REPLY_MSG_ID_BASE);
    strncpy((char *) mp->interface_name, (char *) if_name, ARRAY_LEN(mp->interface_name) - 1);
    ...
    ip_address_encode((ip46_address_t *) &s->client_address, IP46_TYPE_IP6, &mp->client_address);
    ip_address_encode((ip46_address_t *) &s->our_address, IP46_TYPE_IP6, &mp->our_address);
...
out:
    REPLY_MACRO2(VL_API_L2TPV3_CREATE_TUNNEL_REPLY, ({ rmp->sw_if_index = ntohl(sw_if_index); }));
}
...
static void vl_api_l2tpv3_set_tunnel_cookies_t_handler(vl_api_l2tpv3_set_tunnel_cookies_t *mp)
{
    ...
    rv = l2tpv3_set_tunnel_cookies(lm,
...
static void vl_api_l2tpv3_interface_enable_disable_t_handler(vl_api_l2tpv3_interface_enable_disable_t *mp)
{
    ...
    rv = l2tpv3_interface_enable_disable(vnm, ntohl(mp->sw_if_index), mp->enable_disable);
