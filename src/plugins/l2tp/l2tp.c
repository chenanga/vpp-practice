...
u8 *format_l2t_session(u8 *s, va_list *args)
{
    ...
    counter_index = session_index_to_counter_index(session - lm->sessions, SESSION_COUNTER_USER_TO_NETWORK);

    vlib_get_combined_counter(&lm->counter_main, counter_index, &v);
    if (v.packets != 0) s = format(s, "   user-to-net: %llu pkts %llu bytes\n", v.packets, v.bytes);

    vlib_get_combined_counter(&lm->counter_main, counter_index + 1, &v);

    if (v.packets != 0) s = format(s, "   net-to-user: %llu pkts %llu bytes\n", v.packets, v.bytes);
    return s;
}
...
static clib_error_t *show_l2tp_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (unformat(input, "verbose") || unformat(input, "v")) verbose = 1;
...
static clib_error_t *test_counters_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    pool_foreach (session, lm->sessions) {
        ...
        counter_index = session_index_to_counter_index(session_index, SESSION_COUNTER_USER_TO_NETWORK);
        vlib_increment_combined_counter(&lm->counter_main, thread_index, counter_index, 1 /*pkt*/, 1111 /*bytes*/);
        vlib_increment_combined_counter(&lm->counter_main, thread_index, counter_index + 1, 1 /*pkt*/, 2222 /*bytes*/);
        nincr++;
    }
...
static u8 *format_l2tpv3_name(u8 *s, va_list *args)
{
    ...
    if (i < vec_len(lm->dev_inst_by_real)) show_dev_instance = lm->dev_inst_by_real[i];
                              ...
                              u32           *sw_if_index)
{
    ...
    s->l2tp_hdr_size   = l2_sublayer_present ? sizeof(l2tpv3_header_t) : sizeof(l2tpv3_header_t) - sizeof(l2tp_hdr.l2_specific_sublayer);
    s->admin_up        = 0;
    ...
    switch (lm->lookup_type) {
        case L2T_LOOKUP_SRC_ADDRESS:
            ...
            hash_set_mem(lm->session_by_src_address, src_address_copy, s - lm->sessions);
            break;
        case L2T_LOOKUP_DST_ADDRESS:
            dst_address_copy = clib_mem_alloc(sizeof(*dst_address_copy));
            clib_memcpy(dst_address_copy, our_address, sizeof(*dst_address_copy));
            hash_set_mem(lm->session_by_dst_address, dst_address_copy, s - lm->sessions);
            break;
        case L2T_LOOKUP_SESSION_ID:
    ...
    /* validate counters */
    counter_index = session_index_to_counter_index(s - lm->sessions, SESSION_COUNTER_USER_TO_NETWORK);
    ...
    if (vec_len(lm->free_l2tpv3_tunnel_hw_if_indices) > 0) {
        hw_if_index = lm->free_l2tpv3_tunnel_hw_if_indices[vec_len(lm->free_l2tpv3_tunnel_hw_if_indices) - 1];
        ...
        hw_if_index           = vnet_register_interface(vnm, l2tpv3_device_class.index, s - lm->sessions, l2tpv3_hw_class.index, s - lm->sessions);
        hi                    = vnet_get_hw_interface(vnm, hw_if_index);
    ...
    if (sw_if_index) *sw_if_index = hi->sw_if_index;
...
static clib_error_t *create_l2tpv3_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "client %U", unformat_ip6_address, &client_address))
...
VLIB_CLI_COMMAND(create_l2tpv3_tunnel_command, static) = {
    ...
    .short_help = "create l2tpv3 tunnel client <ip6> our <ip6> local-cookie <hex> remote-cookie <hex> local-session <dec> remote-session <dec>",
...
int l2tpv3_set_tunnel_cookies(l2t_main_t *lm, u32 sw_if_index, u64 new_local_cookie, u64 new_remote_cookie)
{
    ...
    if (pool_is_free_index(lm->sessions, hi->dev_instance)) return VNET_API_ERROR_INVALID_VALUE;
...
static clib_error_t *set_l2tp_tunnel_cookie_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (sw_if_index == ~0) return clib_error_return(0, "unknown interface");
    if (local_cookie == ~0) return clib_error_return(0, "local cookie required");
    if (remote_cookie == ~0) return clib_error_return(0, "remote cookie required");

    rv = l2tpv3_set_tunnel_cookies(lm, sw_if_index, local_cookie, remote_cookie);
...
int l2tpv3_interface_enable_disable(vnet_main_t *vnm, u32 sw_if_index, int enable_disable)
{
    ...
    vnet_feature_enable_disable("ip6-unicast", "l2tp-decap", sw_if_index, enable_disable, 0, 0);
    return 0;
}
...
static clib_error_t *set_ip6_l2tpv3(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (sw_if_index == ~0) return clib_error_return(0, "interface required");
    ...
    switch (rv) {
        ...
        default:
            return clib_error_return(0, "l2tp_interface_enable_disable returned %d", rv);
    }
    return 0;
}
...
clib_error_t *l2tp_sw_interface_up_down(vnet_main_t *vnm, u32 sw_if_index, u32 flags)
{
    ...
    if (hi->hw_class_index != l2tpv3_hw_class.index) return 0;
...
clib_error_t *l2tp_init(vlib_main_t *vm)
{
    ...
    lm->session_by_src_address = hash_create_mem(0, sizeof(ip6_address_t) /* key bytes */, sizeof(u32) /* value bytes */);
    lm->session_by_dst_address = hash_create_mem(0, sizeof(ip6_address_t) /* key bytes */, sizeof(u32) /* value bytes */);
