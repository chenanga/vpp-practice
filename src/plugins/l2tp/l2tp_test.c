...
static void vl_api_l2tpv3_create_tunnel_reply_t_handler(vl_api_l2tpv3_create_tunnel_reply_t *mp)
{
...
static int api_l2tpv3_create_tunnel(vat_main_t *vam)
{
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "client_address %U", unformat_ip6_address, &client_address))
            client_address_set = 1;
        else if (unformat(i, "our_address %U", unformat_ip6_address, &our_address))
            our_address_set = 1;
    ...
    clib_memcpy(mp->client_address.un.ip6, client_address.as_u8, sizeof(ip6_address_t));

    clib_memcpy(mp->our_address.un.ip6, our_address.as_u8, sizeof(ip6_address_t));
...
static void vl_api_sw_if_l2tpv3_tunnel_details_t_handler(vl_api_sw_if_l2tpv3_tunnel_details_t *mp)
{
    vat_main_t *vam = &vat_main;

    print(vam->ofp,
          "* %U (our) %U (client) (sw_if_index %d)",
          format_ip6_address,
          mp->our_address,
          format_ip6_address,
          mp->client_address,
...
static int api_sw_if_l2tpv3_tunnel_dump(vat_main_t *vam)
{
    ...
    if (!l2tp_test_main.ping_id) l2tp_test_main.ping_id = vl_msg_api_get_msg_index((u8 *) (VL_API_CONTROL_PING_CRC));
