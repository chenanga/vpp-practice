...
static inline void stage0(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t *b)
{
...
static inline u32 last_stage(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t *b)
{
    ...
    vlib_increment_combined_counter(&lm->counter_main,
                                    ...
                                    counter_index,
                                    1 /* packet_increment */,
                                    vlib_buffer_length_in_chain(vm, b) + sizeof(ethernet_header_t));
    ...
    if (next_index == L2T_DECAP_NEXT_NO_INTERCEPT) {
        ...
        if (l2tp_decap_local) {
            ...
            if (PREDICT_TRUE(session != 0)) vnet_feature_next(&next_index, b);
        }
    }
