...
__clib_export u8 *format_urpf_mode(u8 *s, va_list *a)
{
...
__clib_export uword unformat_urpf_mode(unformat_input_t *input, va_list *args)
{
...
__clib_export int urpf_feature_enable_disable(ip_address_family_t af, vlib_dir_t dir, urpf_mode_t mode, u32 sw_if_index, int enable)
{
    return vnet_feature_enable_disable(urpf_feat_arcs[af][dir], urpf_feats[af][dir][mode], sw_if_index, enable, 0, 0);
}

__clib_export int urpf_update(urpf_mode_t mode, u32 sw_if_index, ip_address_family_t af, vlib_dir_t dir, u32 table_id)
{
    ...
    if (table_id != ~0) {
        ...
        if (fib_index == (~0)) return VNET_API_ERROR_INVALID_VALUE;
    }
    else {
        bool is_ip4                   = (AF_IP4 == af);
        u32 *fib_index_by_sw_if_index = is_ip4 ? ip4_main.fib_index_by_sw_if_index : ip6_main.fib_index_by_sw_if_index;
    ...
    urpf_data_t data                = {.fib_index = fib_index, .mode = mode, .fib_index_is_custom = (table_id != ~0)};
    urpf_cfgs[af][dir][sw_if_index] = data;
    if (data.mode != old.mode || data.fib_index != old.fib_index) {
        if (URPF_MODE_OFF != old.mode) /* disable what we have */
            vnet_feature_enable_disable(urpf_feat_arcs[af][dir], urpf_feats[af][dir][old.mode], sw_if_index, 0, 0, 0);

        if (URPF_MODE_OFF != data.mode) /* enable what's new */
            vnet_feature_enable_disable(urpf_feat_arcs[af][dir], urpf_feats[af][dir][data.mode], sw_if_index, 1, 0, 0);
    }
    /* else - no change to existing config */
    return 0;
}

static void urpf_table_bind_v4(ip4_main_t *im, uword opaque, u32 sw_if_index, u32 new_fib_index, u32 old_fib_index)
{
    vlib_dir_t  dir;
...
static void urpf_table_bind_v6(ip6_main_t *im, uword opaque, u32 sw_if_index, u32 new_fib_index, u32 old_fib_index)
{
    vlib_dir_t  dir;
    urpf_data_t empty = {.fib_index = 0, .mode = URPF_MODE_OFF};
    FOREACH_VLIB_DIR(dir)
...
static clib_error_t *urpf_init(vlib_main_t *vm)
{
...
VLIB_CLI_COMMAND(set_interface_ip_source_check_command, static) = {
    ...
    .short_help =
        "set urpf [ip4|ip6] [rx|tx] [off|strict|loose] "
...
static clib_error_t *urpf_cli_accept(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (is_add)
        fib_table_entry_special_add(fib_index, &fpfx, FIB_SOURCE_URPF_EXEMPT, FIB_ENTRY_FLAG_DROP);
    else
