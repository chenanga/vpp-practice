...
static void vl_api_urpf_update_t_handler(vl_api_urpf_update_t *mp)
{
    ...
    rv = urpf_update(mode, htonl(mp->sw_if_index), af, (mp->is_input ? VLIB_RX : VLIB_TX), 0);
    if (rv) goto done;
...
static void vl_api_urpf_update_v2_t_handler(vl_api_urpf_update_v2_t *mp)
{
    vl_api_urpf_update_reply_t *rmp;
    ip_address_family_t         af;
    urpf_mode_t                 mode;
    ...
    rv = urpf_update(mode, htonl(mp->sw_if_index), af, (mp->is_input ? VLIB_RX : VLIB_TX), ntohl(mp->table_id));
...
static void send_urpf_interface_details(vpe_api_main_t           *am,
                                        ...
                                        const vlib_dir_t          dir)
{
    ...
    mp->table_id    = htonl(fib_table_get_table_id(ud->fib_index, (af == AF_IP4 ? FIB_PROTOCOL_IP4 : FIB_PROTOCOL_IP6)));
...
static void send_urpf_interface(vpe_api_main_t *am, vl_api_registration_t *reg, u32 context, const u32 sw_if_index)
{
    urpf_data_t        *ud;
    vlib_dir_t          dir;
    ip_address_family_t af;
    ...
    if (sw_if_index < vec_len(urpf_cfgs[af][dir])) {
        ...
        if (ud->mode || ud->fib_index_is_custom) send_urpf_interface_details(am, reg, context, sw_if_index, ud, af, dir);
    }
}

static void vl_api_urpf_interface_dump_t_handler(vl_api_urpf_interface_dump_t *mp)
{
