...
static u8 *format_tracenode_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "Packet traced from interface %U added", format_vnet_sw_if_index_name, vnm, t->sw_if_index);
    return s;
}

static_always_inline u32 tracenode_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_pcap)
{
    vnet_main_t   *vnm = vnet_get_main();
    ...
    while (n_left > 0) {
        ...
        if (PREDICT_FALSE(b[0]->flags & VLIB_BUFFER_IS_TRACED)) goto skip;

        if (is_pcap && vnet_is_packet_pcaped(pp, b[0], ~0)) {
            pcap_add_buffer(&pp->pcap_main, vm, from0[0], pp->max_bytes_per_pkt);
        }
        else if (!is_pcap && vlib_trace_buffer(vm, node, next[0], b[0], 1 /* follow_chain */)) {
...
VNET_FEATURE_INIT(trace_filtering4, static) = {
    ...
    .runs_after = VNET_FEATURES("ip4-full-reassembly-feature", "ip4-sv-reassembly-feature"),
};

VNET_FEATURE_INIT(trace_filtering6, static) = {
    .arc_name   = "ip6-unicast",
    .node_name  = "trace-filtering",
    .runs_after = VNET_FEATURES("ip6-full-reassembly-feature", "ip6-sv-reassembly-feature"),
};
