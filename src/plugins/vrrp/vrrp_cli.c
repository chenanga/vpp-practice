...
static clib_error_t *vrrp_vr_add_del_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd, u8 is_add)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        if (unformat(input, "%U", unformat_vnet_sw_interface, vmp->vnet_main, &sw_if_index))
            ;
    ...
    switch (rv) {
        ...
        case VNET_API_ERROR_ADDRESS_NOT_FOUND_FOR_INTERFACE:
            ret = clib_error_return(0,
                                    "Failed to add VR with priority 255 - "
...
VLIB_CLI_COMMAND(vrrp_proto_start_stop_command, static) = {
    .path       = "vrrp proto",
    .short_help = "vrrp proto (start|stop) (<intf_name>|sw_if_index <n>) vr_id <n> [ipv6]",
    .function   = vrrp_proto_start_stop_command_fn,
};

VLIB_CLI_COMMAND(vrrp_peers_command, static) = {
    .path       = "vrrp peers",
    .short_help = "vrrp peers (<intf_name>|sw_if_index <n>) vr_id <n> [ipv6] <peer1_addr> [<peer2_addr> ...]",
...
static clib_error_t *vrrp_vr_track_if_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "%U", unformat_vnet_sw_interface, vmp->vnet_main, &sw_if_index))
            ...
            is_ipv6 = 1;
        else if (unformat(input, "track-index %u priority %u", &track_if_index, &priority)) {
    ...
    vec_foreach (track_intf, track_intfs) {
        if (!vnet_sw_interface_is_valid(vnm, track_intf->sw_if_index)) {
            ret = clib_error_return(0, "tracked intf sw_if_index %u invalid", track_intf->sw_if_index);
            goto done;
        }
    ...
    if (rv) ret = clib_error_return(0, "vrrp_vr_tracking_ifs_add_del returned %d", rv);
