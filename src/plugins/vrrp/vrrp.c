...
static const mac_address_t ipv4_vmac = {.bytes = {0x00, 0x00, 0x5e, 0x00, 0x01, 0x00}};

static const mac_address_t ipv6_vmac = {.bytes = {0x00, 0x00, 0x5e, 0x00, 0x02, 0x00}};
...
void vrrp_incr_err_counter(vrrp_err_counter_t err_type)
{
    if (err_type >= VRRP_ERR_COUNTER_MAX) {
        clib_warning("Attempt to increse error counter of unknown type %u", err_type);
        return;
    }
    vlib_increment_simple_counter(&vrrp_errs[err_type], vlib_get_main()->thread_index, 0, 1);
}
...
void vrrp_incr_stat_counter(vrrp_stat_counter_t stat_type, u32 stat_index)
{
    if (stat_type >= VRRP_STAT_COUNTER_MAX) {
        clib_warning("Attempt to increse stat counter of unknown type %u", stat_type);
        return;
    }
    vlib_increment_simple_counter(&vrrp_stats[stat_type], vlib_get_main()->thread_index, stat_index, 1);
}
...
static walk_rc_t vrrp_hwif_master_count_walk(vnet_main_t *vnm, u32 sw_if_index, void *arg)
{
    ...
    vr = vrrp_vr_lookup(sw_if_index, vr_count->key.vr_id, vr_count->key.is_ipv6);

    if (vr && (vr->runtime.state == VRRP_VR_STATE_MASTER)) vr_count->count++;
...
static u32 vrrp_vr_hwif_master_vrs_by_vrid(u32 hw_if_index, u8 vr_id, u8 is_ipv6)
{
    ...
    vnet_hw_interface_walk_sw(vnm, hw_if_index, vrrp_hwif_master_count_walk, &vr_count);
...
void vrrp_vr_transition_vmac(vrrp_vr_t *vr, vrrp_vr_state_t new_state)
{
    ...
    n_master_vrs = vrrp_vr_hwif_master_vrs_by_vrid(hw->hw_if_index, vr->config.vr_id, vrrp_vr_is_ipv6(vr));
    ...
    if ((enable && !n_master_vrs) || (!enable && (n_master_vrs < 2))) {
        clib_warning("%s virtual MAC address %U on hardware interface %u",
                     ...
                     vr->runtime.mac.bytes,
                     hw->hw_if_index);

        error = vnet_hw_interface_add_del_mac_address(vnm, hw->hw_if_index, vr->runtime.mac.bytes, enable);
    }
...
static void vrrp_vr_transition_intf(vrrp_vr_t *vr, vrrp_vr_state_t new_state)
{
    ...
    vec_foreach (vr_index, intf->vr_indices[is_ipv6]) {
        ...
        if (intf_vr == vr) continue;

        if (intf_vr->runtime.state == VRRP_VR_STATE_INIT) continue;

        n_started++;

        if ((intf_vr->runtime.state == VRRP_VR_STATE_MASTER) && vrrp_vr_accept_mode_enabled(intf_vr)) n_master_accept++;
    }

    /* If entering/leaving init state, start/stop ARP or ND feature if no other
     * VRs are active on the interface.
     */
    if (((vr->runtime.state == VRRP_VR_STATE_INIT) || (new_state == VRRP_VR_STATE_INIT)) && (n_started == 0))
        vnet_feature_enable_disable(arc_name, node_name, vr->config.sw_if_index, (new_state != VRRP_VR_STATE_INIT), NULL, 0);

    /* Special housekeeping when entering/leaving master mode */
    if ((vr->runtime.state == VRRP_VR_STATE_MASTER) || (new_state == VRRP_VR_STATE_MASTER)) {
        ...
        if (vrrp_vr_accept_mode_enabled(vr) && !n_master_accept)
            vnet_feature_enable_disable(mc_arc_name, mc_node_name, vr->config.sw_if_index, (new_state == VRRP_VR_STATE_MASTER), NULL, 0);
    }
}
...
static void vrrp_vr_transition_addrs(vrrp_vr_t *vr, vrrp_vr_state_t new_state)
{
    ...
    if (!vrrp_vr_accept_mode_enabled(vr)) return;

    /* owner always has VR addresses configured, should never remove them */
    if (vrrp_vr_is_owner(vr)) return;

    /* only need to do something if entering or leaving master state */
    if ((vr->runtime.state != VRRP_VR_STATE_MASTER) && (new_state != VRRP_VR_STATE_MASTER)) return;

    is_del = (new_state != VRRP_VR_STATE_MASTER);

    clib_warning("%s VR addresses on sw_if_index %u", (is_del) ? "Deleting" : "Adding", vr->config.sw_if_index);
    ...
    vec_foreach (vr_addr, vr->config.vr_addrs) {
        ...
        if (!vrrp_vr_is_ipv6(vr)) {
            ...
            intf4 = ip4_interface_address_matching_destination(im, &vr_addr->ip4, vr->config.sw_if_index, &ia);

            ip4_add_del_interface_address(vm, vr->config.sw_if_index, &vr_addr->ip4, (intf4 ? ia->address_length : 24), is_del);
        }
        else {
            ip6_main_t    *im = &ip6_main;
            ip6_address_t *intf6;

            intf6 = ip6_interface_address_matching_destination(im, &vr_addr->ip6, vr->config.sw_if_index, &ia);

            ip6_add_del_interface_address(vm, vr->config.sw_if_index, &vr_addr->ip6, (intf6 ? ia->address_length : 64), is_del);
        }
    }
}
...
{
    clib_warning("VR %U transitioning to %U", format_vrrp_vr_key, vr, format_vrrp_vr_state, new_state);

    /* Don't do anything if transitioning to the state VR is already in.
     * This should never happen, just covering our bases.
     */
    if (new_state == vr->runtime.state) return;
    ...
    if (new_state == VRRP_VR_STATE_MASTER) {
        ...
        if (vr->runtime.state == VRRP_VR_STATE_MASTER) {
            ...
            if (args) vr->runtime.master_adv_int = args->max_adv_int;
        }
        else /* INIT, INTF_DOWN */
        ...
        if (vr->runtime.state == VRRP_VR_STATE_MASTER) vrrp_adv_send(vr, 1);
    }
...
static int vrrp_intf_enable_disable_mcast(u8 enable, u32 sw_if_index, u8 is_ipv6)
{
    ...
    vec_foreach_index (i, vrm->vrrp_intfs) {
        if (mfib_table_get_index_for_sw_if_index(proto, i) != fib_index) continue;
    ...
    if (enable) {
        ...
        mfib_table_entry_path_update(fib_index, vrrp_prefix, MFIB_SOURCE_API, MFIB_ENTRY_FLAG_NONE, &for_us);

        mfib_table_entry_path_update(fib_index, vrrp_prefix, MFIB_SOURCE_API, MFIB_ENTRY_FLAG_NONE, &via_itf);
        intf->mcast_adj_index[!!is_ipv6] = adj_mcast_add_or_lock(proto, link_type, sw_if_index);
    }
    else {
        /* Remove mcast local routes if this is the last VR being deleted */
        if (n_vrs_in_fib == 0) mfib_table_entry_path_remove(fib_index, vrrp_prefix, MFIB_SOURCE_API, &for_us);

        mfib_table_entry_path_remove(fib_index, vrrp_prefix, MFIB_SOURCE_API, &via_itf);
    }
...
static int vrrp_intf_vr_add_del(u8 is_add, u32 sw_if_index, u32 vr_index, u8 is_ipv6)
{
    ...
    if (is_add) {
        if (!vec_len(vr_intf->vr_indices[is_ipv6])) vrrp_intf_enable_disable_mcast(1, sw_if_index, is_ipv6);
        ...
        if (per_intf_index != ~0) vec_del1(vr_intf->vr_indices[is_ipv6], per_intf_index);

        /* no more VRs on this interface, disable multicast */
        if (!vec_len(vr_intf->vr_indices[is_ipv6])) vrrp_intf_enable_disable_mcast(0, sw_if_index, is_ipv6);
    }
...
static int vrrp_vr_valid_addrs_owner(vrrp_vr_config_t *vr_conf)
{
    ...
    vec_foreach (addr, vr_conf->vr_addrs) {
        if (!ip_interface_has_address(vr_conf->sw_if_index, addr, !is_ipv6)) return VNET_API_ERROR_ADDRESS_NOT_FOUND_FOR_INTERFACE;
    }

    return 0;
}

static int vrrp_vr_valid_addrs_unused(vrrp_vr_config_t *vr_conf, index_t vrrp_index)
{
    ip46_address_t *vr_addr;
    u8              is_ipv6 = (vr_conf->flags & VRRP_VR_IPV6) != 0;
    ...
    vec_foreach (vr_addr, vr_conf->vr_addrs) {
        ...
        if (vr_index != ~0 && vrrp_index != vr_index) return VNET_API_ERROR_ADDRESS_IN_USE;
    }

    return 0;
}

static int vrrp_vr_valid_addrs(vrrp_vr_config_t *vr_conf, index_t vrrp_index)
{
    int ret = 0;

    /* If the VR owns the addresses, make sure they are configured */
    if (vr_conf->priority == 255 && (ret = vrrp_vr_valid_addrs_owner(vr_conf)) < 0) return ret;
...
int vrrp_vr_addr_add_del(vrrp_vr_t *vr, u8 is_add, ip46_address_t *vr_addr)
{
    ...
    if (!vr || !vr_addr) return VNET_API_ERROR_INVALID_ARGUMENT;
...
int vrrp_vr_update(index_t *vrrp_index, vrrp_vr_config_t *vr_conf)
{
    ...
    if (pool_is_free_index(vrm->vrs, index)) return (VNET_API_ERROR_NO_SUCH_ENTRY);
    ...
    if (vr_conf->vr_id != key.vr_id || vr_conf->sw_if_index != key.sw_if_index || !!(vr_conf->flags & VRRP_VR_IPV6) != key.is_ipv6) {
        clib_warning(
            "Attempt to change VR ID, IP version or interface index "
    ...
    if (!vec_len(vr_conf->vr_addrs)) {
        clib_warning(
            "Conf of VR %u for IPv%d on sw_if_index %u "
    ...
    if ((ret = vrrp_vr_valid_addrs(vr_conf, index)) < 0) return ret;

    /* stop it if needed */
    must_restart = (vr->runtime.state != VRRP_VR_STATE_INIT);
    if (must_restart) vrrp_vr_start_stop(0, &key);
    ...
    if (must_restart) vrrp_vr_start_stop(1, &key);

    return 0;
}

static void vrrp_vr_del_common(vrrp_vr_t *vr, vrrp_vr_key_t *key)
{
    vrrp_main_t    *vrm = &vrrp_main;
...
int vrrp_vr_add_del(u8 is_add, vrrp_vr_config_t *vr_conf, index_t *ret_index)
{
    vrrp_main_t  *vrm = &vrrp_main;
    vnet_main_t  *vnm = vnet_get_main();
    vrrp_vr_key_t key;
    ...
    if (vr_conf->sw_if_index == ~0 || !vnet_sw_interface_is_valid(vnm, vr_conf->sw_if_index)) return VNET_API_ERROR_INVALID_SW_IF_INDEX;
    ...
    if (is_add) {
        ...
        if (p) {
            clib_warning("VR %u for IPv%d already exists on sw_if_index %u", key.vr_id, (key.is_ipv6) ? 6 : 4, key.sw_if_index);
        ...
        /* Make sure the addresses are ok to use */
        if ((ret = vrrp_vr_valid_addrs(vr_conf, INDEX_INVALID)) < 0) return ret;
        ...
        if (!p) {
            clib_warning("No VR %u for IPv%d exists on sw_if_index %u", key.vr_id, (key.is_ipv6) ? 6 : 4, key.sw_if_index);
            return VNET_API_ERROR_NO_SUCH_ENTRY;
        }
...
int vrrp_vr_start_stop(u8 is_start, vrrp_vr_key_t *vr_key)
{
    ...
    switch (vr->runtime.state) {
        case VRRP_VR_STATE_INIT:
            if (!is_start) {
                clib_warning("Attempting to stop already stopped VR (%U)", format_vrrp_vr_key, vr);
                return 0;
            }
            break;
        default:
            if (is_start) {
                clib_warning("Attempting to start already started VR (%U)", format_vrrp_vr_key, vr);
                return 0;
            }
            break;
    }
    ...
    if (is_start) {
        ...
        if (!vrrp_intf_is_up(vr->config.sw_if_index, vrrp_vr_is_ipv6(vr), NULL)) {
            clib_warning("VRRP VR started on down interface (%U)", format_vrrp_vr_key, vr);
    ...
    clib_warning("%d VRs configured, %d VRs running", pool_elts(vmp->vrs), vmp->n_vrs_started);
...
int vrrp_vr_set_peers(vrrp_vr_key_t *vr_key, ip46_address_t *peers)
{
    ...
    if (vr->config.peer_addrs) vec_free(vr->config.peer_addrs);
...
static void vrrp_intf_tracking_vr_add_del(u32 sw_if_index, vrrp_vr_t *vr, u8 is_add)
{
    ...
    vec_foreach_index (i, intf->tracking_vrs[is_ipv6]) {
        if (vec_elt(intf->tracking_vrs[is_ipv6], i) != vr_index) continue;

        /* Current index matches VR index */
        if (!is_add) vec_delete(intf->tracking_vrs[is_ipv6], 1, i);
    ...
    if (is_add) vec_add1(intf->tracking_vrs[is_ipv6], vr_index);
}
...
static int vrrp_intf_hw_link_up(u32 sw_if_index, vrrp_intf_update_t *pending)
{
    ...
    if (pending && (pending->type == VRRP_IF_UPDATE_HW_LINK) && (pending->hw_if_index == sup_sw->hw_if_index))
...
static int vrrp_intf_ip_up(u32 sw_if_index, u8 is_ipv6, vrrp_intf_update_t *pending)
{
    ...
    if (pending && pending->type == VRRP_IF_UPDATE_IP)
         ...
         */
        ip_up = ((ip_interface_get_first_ip(sw_if_index, !is_ipv6) != 0) || (is_ipv6 && ip6_link_is_enabled(sw_if_index)));
...
void vrrp_vr_tracking_ifs_compute(vrrp_vr_t *vr, vrrp_intf_update_t *pending)
{
    ...
    vec_foreach (intf, vr->tracking.interfaces) {
        if (vrrp_intf_is_up(intf->sw_if_index, vrrp_vr_is_ipv6(vr), pending)) continue;

        total_priority += intf->priority;
    }

    if (total_priority != vr->tracking.interfaces_dec) {
        clib_warning("VR %U interface track adjustment change from %u to %u", format_vrrp_vr_key, vr, vr->tracking.interfaces_dec, total_priority);
...
int vrrp_vr_tracking_if_add_del(vrrp_vr_t *vr, u32 sw_if_index, u8 prio, u8 is_add)
{
    ...
    if (!vnet_sw_interface_is_valid(vnm, sw_if_index)) return VNET_API_ERROR_INVALID_SW_IF_INDEX;

    /* VR can't track it's own interface */
    if (sw_if_index == vr->config.sw_if_index) return VNET_API_ERROR_INVALID_SW_IF_INDEX_2;
    ...
    vec_foreach (track_intf, vr->tracking.interfaces) {
        if (track_intf->sw_if_index != sw_if_index) continue;

        /* found it */
        if (!is_add) vec_delete(vr->tracking.interfaces, 1, track_intf - vr->tracking.interfaces);
...
int vrrp_vr_tracking_ifs_add_del(vrrp_vr_t *vr, vrrp_vr_tracking_if_t *track_ifs, u8 is_add)
{
    ...
    vec_foreach (track_if, ifs_copy) {
        rv = vrrp_vr_tracking_if_add_del(vr, track_if->sw_if_index, track_if->priority, (is_add != 0));
        ...
        if (rv) {
            ...
            for (rb_if = track_if - 1; rb_if >= track_ifs; rb_if -= 1)
                vrrp_vr_tracking_if_add_del(vr, rb_if->sw_if_index, rb_if->priority, !(is_add != 0));
            break;
        }
    }
...
static void vrrp_intf_tracking_vrs_compute(u32 sw_if_index, vrrp_intf_update_t *pending, u8 is_ipv6)
{
    ...
    vec_foreach (vr_index, intf->tracking_vrs[is_ipv6]) {
        ...
        if (vr) vrrp_vr_tracking_ifs_compute(vr, pending);
    }
}
...
clib_error_t *vrrp_sw_interface_up_down(vrrp_intf_update_t *pending)
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        vec_foreach (vr_index, intf->vr_indices[i]) {
            ...
            if (vr->runtime.state == VRRP_VR_STATE_INIT) continue; /* VR not started yet, no transition */

            if (!is_up)
                vr_state = VRRP_VR_STATE_INTF_DOWN;
            else {
                if (vr->runtime.state != VRRP_VR_STATE_INTF_DOWN) continue; /* shouldn't happen */

                vr_state = (vrrp_vr_is_owner(vr)) ? VRRP_VR_STATE_MASTER : VRRP_VR_STATE_BACKUP;
            }
    ...
    vrrp_intf_tracking_vrs_compute(pending->sw_if_index, pending, 0 /* is_ipv6 */);
    vrrp_intf_tracking_vrs_compute(pending->sw_if_index, pending, 1 /* is_ipv6 */);
...
static clib_error_t *vrrp_hw_interface_link_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    ...
    vnet_hw_interface_walk_sw(vnm, hw_if_index, vrrp_hw_interface_link_up_down_walk, &pending);
...
static u8 *format_vrrp_ip6_link(u8 *s, va_list *args)
{
    ...
    vec_foreach (vr_index, intf->tracking_vrs[1]) {
        ...
        s = format(s, "%U%U\n", format_white_space, indent + 2, format_vrrp_vr_key, vr);
    }
...
static clib_error_t *vrrp_init(vlib_main_t *vm)
{
    ...
    if ((error = vlib_call_init_function(vm, ip4_lookup_init)) || (error = vlib_call_init_function(vm, ip6_lookup_init))) return error;
    ...
    vmp->vrrp6_nd_lookup  = hash_create_mem(0, sizeof(vrrp6_nd_key_t), sizeof(uword));
