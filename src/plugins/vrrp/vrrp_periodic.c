...
void vrrp_vr_timer_cancel(vrrp_vr_t *vr)
{
    ...
    if (vr->runtime.timer_index == ~0) return;
    ...
    if (!pool_is_free_index(vmp->vr_timers, vr->runtime.timer_index)) pool_put_index(vmp->vr_timers, vr->runtime.timer_index);

    vr->runtime.timer_index = ~0;

    vlib_process_signal_event(vmp->vlib_main, vrrp_periodic_node.index, VRRP_EVENT_VR_TIMER_UPDATE, 0);
}
...
void vrrp_vr_timer_set(vrrp_vr_t *vr, vrrp_vr_timer_type_t type)
{
     ...
     */
    if (vr->runtime.timer_index != ~0) vrrp_vr_timer_cancel(vr);
