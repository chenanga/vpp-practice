...
static void vl_api_vrrp_vr_update_t_handler(vl_api_vrrp_vr_update_t *mp)
{
    ...
    if (api_flags & VRRP_API_VR_PREEMPT) vr_conf.flags |= VRRP_VR_PREEMPT;

    if (api_flags & VRRP_API_VR_ACCEPT) vr_conf.flags |= VRRP_VR_ACCEPT;

    if (api_flags & VRRP_API_VR_UNICAST) vr_conf.flags |= VRRP_VR_UNICAST;

    if (api_flags & VRRP_API_VR_IPV6) vr_conf.flags |= VRRP_VR_IPV6;
...
static void vl_api_vrrp_vr_del_t_handler(vl_api_vrrp_vr_del_t *mp)
{
...
{
    if (vr_state == VRRP_VR_STATE_BACKUP) return VRRP_API_VR_STATE_BACKUP;
    if (vr_state == VRRP_VR_STATE_MASTER) return VRRP_API_VR_STATE_MASTER;
    if (vr_state == VRRP_VR_STATE_INTF_DOWN) return VRRP_API_VR_STATE_INTF_DOWN;
...
static void send_vrrp_vr_details(vrrp_vr_t *vr, vl_api_registration_t *reg, u32 context)
{
    ...
    if (vr->config.flags & VRRP_VR_PREEMPT) api_flags |= VRRP_API_VR_PREEMPT;
    if (vr->config.flags & VRRP_VR_ACCEPT) api_flags |= VRRP_API_VR_ACCEPT;
    if (vrrp_vr_is_unicast(vr)) api_flags |= VRRP_API_VR_UNICAST;
    if (vrrp_vr_is_ipv6(vr)) api_flags |= VRRP_API_VR_IPV6;
...
static void vl_api_vrrp_vr_dump_t_handler(vl_api_vrrp_vr_dump_t *mp)
{
    ...
    pool_foreach (vr, vmp->vrs) {
        if (sw_if_index && (sw_if_index != ~0) && (sw_if_index != vr->config.sw_if_index)) continue;
...
static void vl_api_vrrp_vr_peer_dump_t_handler(vl_api_vrrp_vr_peer_dump_t *mp)
{
    ...
    pool_foreach (vr, vmp->vrs) {
        if (!vec_len(vr->config.peer_addrs)) continue;
...
static void vl_api_vrrp_vr_track_if_dump_t_handler(vl_api_vrrp_vr_track_if_dump_t *mp)
{
    ...
    pool_foreach (vr, vmp->vrs) {
        if (!vec_len(vr->tracking.interfaces)) continue;
...
void vrrp_vr_event(vrrp_vr_t *vr, vrrp_vr_state_t new_state)
{
    ...
    pool_foreach (reg, vam->vrrp_vr_events_registrations) {
        ...
        if (vl_reg) send_vrrp_vr_event(reg, vl_reg, vr, new_state);
    }
}
