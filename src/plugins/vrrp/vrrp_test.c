...
static int api_vrrp_vr_update(vat_main_t *vam)
{
    ...
    if (no_preempt) mp->flags &= ~VRRP_API_VR_PREEMPT;

    if (accept_mode) mp->flags |= VRRP_API_VR_ACCEPT;

    if (vr_unicast) mp->flags |= VRRP_API_VR_UNICAST;

    if (is_ipv6) mp->flags |= VRRP_API_VR_IPV6;
...
static void vl_api_vrrp_vr_update_reply_t_handler(vl_api_vrrp_vr_update_reply_t *mp)
{
}

static int api_vrrp_vr_del(vat_main_t *vam)
{
    unformat_input_t     *i = vam->input;
...
static int api_vrrp_vr_add_del(vat_main_t *vam)
{
    unformat_input_t         *i           = vam->input;
    u32                       sw_if_index = ~0;
...
static int api_vrrp_vr_dump(vat_main_t *vam)
{
...
static void vl_api_vrrp_vr_details_t_handler(vl_api_vrrp_vr_details_t *mp)
{
    ...
    if (state > ARRAY_LEN(states) - 2) state = ARRAY_LEN(states) - 1;
    ...
    fformat(vam->ofp,
            ...
            states[state],
            ntohs(mp->runtime.master_adv_int),
            ntohs(mp->runtime.skew),
    ...
    for (i = 0; i < mp->n_addrs; i++) {
        ...
        fformat(vam->ofp, "%U ", (addr->af) ? format_ip6_address : format_ip4_address, (u8 *) &addr->un);
    }

    fformat(vam->ofp, "\n");
}

static int api_vrrp_vr_start_stop(vat_main_t *vam)
{
    unformat_input_t            *i = vam->input;
...
static int api_vrrp_vr_track_if_add_del(vat_main_t *vam)
{
    unformat_input_t                  *i = vam->input;
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "%U", unformat_sw_if_index, vam, &sw_if_index))
            ...
            is_ipv6 = 1;
        else if (unformat(i, "track-index %u priority %u", &track_sw_if_index, &priority)) {
...
static int api_vrrp_vr_track_if_dump(vat_main_t *vam)
{
    vrrp_test_main_t               *vtm = &vrrp_test_main;
    unformat_input_t               *i   = vam->input;
    ...
    if ((sw_if_index == ~0) && !vr_id && !is_ipv6) dump_all = 1;
...
static void vl_api_vrrp_vr_track_if_details_t_handler(vl_api_vrrp_vr_track_if_details_t *mp)
{
    vat_main_t *vam = vrrp_test_main.vat_main;
    int         i;
...
static int api_vrrp_vr_set_peers(vat_main_t *vam)
{
    unformat_input_t           *i           = vam->input;
    u32                         sw_if_index = ~0;
    u32                         vr_id;
...
static int api_vrrp_vr_peer_dump(vat_main_t *vam)
{
    vrrp_test_main_t           *vtm = &vrrp_test_main;
    unformat_input_t           *i   = vam->input;
...
static void vl_api_vrrp_vr_peer_details_t_handler(vl_api_vrrp_vr_peer_details_t *mp)
{
    vat_main_t *vam = vrrp_test_main.vat_main;
    int         i;

    fformat(vam->ofp, "sw_if_index %u vr_id %u IPv%d ", ntohl(mp->sw_if_index), mp->vr_id, (mp->is_ipv6) ? 6 : 4);
...
static int api_want_vrrp_vr_events(vat_main_t *vam)
{
    unformat_input_t             *i = vam->input;
