...
static int vrrp_adv_l2_build_multicast(vrrp_vr_t *vr, vlib_buffer_t *b)
{
    ...
    rewrite = ethernet_build_rewrite(vnm, vr->config.sw_if_index, link_type, dst_mac);
    clib_memcpy(eth, rewrite, vec_len(rewrite));

    /* change the source mac from the HW addr to the VRRP virtual MAC */
    clib_memcpy(eth->src_address, vrrp_src_mac_prefix, sizeof(vrrp_src_mac_prefix));
...
always_inline u16 vrrp_adv_payload_len(vrrp_vr_t *vr)
{
...
static int vrrp_adv_l3_build(vrrp_vr_t *vr, vlib_buffer_t *b, const ip46_address_t *dst)
{
    if (!vrrp_vr_is_ipv6(vr)) /* IPv4 */
    {
        ...
        ip4->length             = clib_host_to_net_u16(sizeof(*ip4) + vrrp_adv_payload_len(vr));
        ...
        ip6_address_copy(&ip6->src_address, ip6_get_link_local_address(vr->config.sw_if_index));
...
u16 vrrp_adv_csum(void *l3_hdr, void *payload, u8 is_ipv6, u16 len)
{
    ...
    for (i = 0; i < (2 * addr_len); i += word_size) {
        if (word_size == sizeof(u64))
            csum = ip_csum_with_carry(csum, clib_mem_unaligned(src_addr + i, u64));
        else
            csum = ip_csum_with_carry(csum, clib_mem_unaligned(src_addr + i, u32));
    }
...
static int vrrp_adv_payload_build(vrrp_vr_t *vr, vlib_buffer_t *b, int shutdown)
{
    ...
    vrrp->checksum = vrrp_adv_csum(l3_hdr, vrrp, is_ipv6, vrrp_adv_payload_len(vr));
...
{
    if (vrrp_vr_is_ipv6(vr)) return &vrrp6_mcast_addr;
...
int vrrp_adv_send(vrrp_vr_t *vr, int shutdown)
{
    ...
    if (is_unicast) n_buffers = vec_len(vr->config.peer_addrs);

    if (n_buffers < 1) {
        /* A unicast VR will not start without peers added so this should
         * not happen. Just avoiding a crash if it happened somehow.
         */
        clib_warning("Unicast VR configuration corrupted for %U", format_vrrp_vr_key, vr);
        return -1;
    }
...
static void vrrp6_na_pkt_build(vrrp_vr_t *vr, vlib_buffer_t *b, ip6_address_t *addr6)
{
    ...
    rewrite = ethernet_build_rewrite(vnm, vr->config.sw_if_index, VNET_LINK_IP6, dst_mac);
    ...
    ll_opt = (icmp6_neighbor_discovery_ethernet_link_layer_address_option_t *) (na + 1);
    ...
    na->advertisement_flags = clib_host_to_net_u32(ICMP6_NEIGHBOR_ADVERTISEMENT_FLAG_OVERRIDE | ICMP6_NEIGHBOR_ADVERTISEMENT_FLAG_ROUTER);

    ll_opt->header.type        = ICMP6_NEIGHBOR_DISCOVERY_OPTION_target_link_layer_address;
    ll_opt->header.n_data_u64s = 1;
    clib_memcpy(ll_opt->ethernet_address, vr->runtime.mac.bytes, sizeof(vr->runtime.mac));

    ip6->payload_length = clib_host_to_net_u16(payload_length);
    na->icmp.checksum   = ip6_tcp_udp_icmp_compute_checksum(vm, b, ip6, &bogus_length);
}
...
static void vrrp4_garp_pkt_build(vrrp_vr_t *vr, vlib_buffer_t *b, ip4_address_t *ip4)
{
    ...
    rewrite       = ethernet_build_rewrite(vnm, vr->config.sw_if_index, VNET_LINK_ARP, broadcast_mac.bytes);
    rewrite_bytes = vec_len(rewrite);
...
int vrrp_garp_or_na_send(vrrp_vr_t *vr)
{
    ...
    if (vec_len(vr->config.peer_addrs)) return 0; /* unicast is used in routed environments - don't garp */

    n_buffers = vec_len(vr->config.vr_addrs);
    if (!n_buffers) {
        clib_warning("Unable to send gratuitous ARP for VR %U - no addresses", format_vrrp_vr_key, vr);
        return -1;
    }
...
static const ip4_header_t igmp_ip4_mcast = {
    ...
    .dst_address =
        {
            .as_u8 = IGMP4_MCAST_ADDR_AS_U8,
        },
};

static int vrrp_igmp_pkt_build(vrrp_vr_t *vr, vlib_buffer_t *b)
{
    ip4_header_t                *ip4;
    u8                          *ip4_options;
    ...
    report->header.checksum = ~ip_csum_fold(ip_incremental_checksum(0, report, payload_len));
...
static void vrrp_icmp6_mlr_pkt_build(vrrp_vr_t *vr, vlib_buffer_t *b)
{
    vlib_main_t                              *vm = vlib_get_main();
    ip6_header_t                             *ip6;
    ...
    ip6_set_reserved_multicast_address(&ip6->dst_address, IP6_MULTICAST_SCOPE_link_local, IP6_MULTICAST_GROUP_ID_mldv2_routers);
    ip6_address_copy(&ip6->src_address, ip6_get_link_local_address(vr->config.sw_if_index));
    ...
    ip6->payload_length = clib_host_to_net_u16(payload_length);
    rh->icmp.checksum   = ip6_tcp_udp_icmp_compute_checksum(vm, b, ip6, &bogus_length);
}
...
int vrrp_vr_multicast_group_join(vrrp_vr_t *vr)
{
    ...
    if (is_ipv6 && ip6_link_is_enabled(vr->config.sw_if_index) == 0) return 0;
    ...
    if (is_ipv6) {
        ...
        if (-1 == vrrp_igmp_pkt_build(vr, b)) {
            clib_warning("IGMP packet build failed for %U", format_vrrp_vr_key, vr);
