...
static u8 *format_vrrp_trace(u8 *s, va_list *args)
{
    ...
    for (i = 0; i < t->vrrp.n_addrs; i++) {
        if (t->is_ipv6)
            s = format(s, "%U ", format_ip6_address, (ip6_address_t *) (t->addrs + i * 16));
        else
            s = format(s, "%U ", format_ip4_address, (ip4_address_t *) (t->addrs + i * 4));
    }
 ...
 */
static int vrrp_vr_addr_cmp(vrrp_vr_t *vr, ip46_address_t *peer_addr)
{
    ...
    if (vrrp_vr_is_ipv6(vr)) {
        ...
        local_addr      = &addr.ip6;
        addr_size       = 16;
        ip6_address_copy(local_addr, ip6_get_link_local_address(vrc->sw_if_index));
    }
    else {
...
static void vrrp_input_process_master(vrrp_vr_t *vr, vrrp_input_process_args_t *args)
{
    /* received priority 0, another VR is shutting down. send an adv and
     * remain in the master state
     */
    if (args->priority == 0) {
        clib_warning("Received shutdown message from a peer on VR %U", format_vrrp_vr_key, vr);
     ...
     */
    if ((args->priority > vrrp_vr_priority(vr)) || ((args->priority == vrrp_vr_priority(vr)) && (vrrp_vr_addr_cmp(vr, &args->src_addr) < 0))) {
...
static void vrrp_input_process_backup(vrrp_vr_t *vr, vrrp_input_process_args_t *args)
{
    vrrp_vr_config_t  *vrc = &vr->config;
    ...
    if (args->priority == 0) {
        clib_warning("Master for VR %U is shutting down", format_vrrp_vr_key, vr);
        vrrp_incr_stat_counter(VRRP_STAT_COUNTER_PRIO0_RCVD, vr->stat_index);
    ...
    if (!(vrc->flags & VRRP_VR_PREEMPT) || (args->priority >= vrrp_vr_priority(vr))) {
...
always_inline void vrrp_input_process(vrrp_input_process_args_t *args)
{
    vrrp_vr_t *vr;
    ...
    switch (vr->runtime.state) {
        ...
        case VRRP_VR_STATE_MASTER:
            ...
            clib_warning("Received advertisement for master VR %U", format_vrrp_vr_key, vr);
            vrrp_input_process_master(vr, args);
            break;
        default:
            clib_warning("Received advertisement for VR %U in unknown state %d", format_vrrp_vr_key, vr, vr->runtime.state);
            break;
    }
...
static u8 *format_vrrp_arp_nd_input_trace(u8 *s, va_list *va)
{
    ...
    s = format(s, "address %U", (t->is_ipv6) ? format_ip6_address : format_ip4_address, (t->is_ipv6) ? (void *) &t->ip.ip6 : (void *) &t->ip.ip4);

    if (t->vr_index != ~0) s = format(s, ": vr_index %u vr_id %u", t->vr_index, t->vr_id);
...
static_always_inline void vrrp_arp_nd_next(vlib_buffer_t *b, u32 *next_index, u32 *vr_index, u8 is_ipv6)
{
    ...
    if (is_ipv6) {
        ...
        if (ip6->protocol != IP_PROTOCOL_ICMP6) return;

        sol_adv = ip6_next_header(ip6);
        lladdr  = (void *) (sol_adv + 1);

        /* skip anything other than neighbor solicitations */
        if (sol_adv->icmp.type != ICMP6_neighbor_solicitation) return;
        ...
        if (arp->opcode != clib_host_to_net_u16(ETHERNET_ARP_OPCODE_request)) return;
    ...
    *vr_index = vrrp_vr_lookup_address(sw_if_index, is_ipv6, lookup_addr);
    ...
    rewrite     = ethernet_build_rewrite(vnm, sw_if_index, link_type, eth->src_address);
    rewrite_len = vec_len(rewrite);
    if (rewrite_len == 0) return;
    ...
    *next_index                          = VRRP_ARP_INPUT_NEXT_REPLY_TX;
    ...
    if (is_ipv6) {
        if (ip6_address_is_unspecified(&ip6->src_address))
            ip6_set_reserved_multicast_address(&ip6->dst_address, IP6_MULTICAST_SCOPE_link_local, IP6_MULTICAST_GROUP_ID_all_hosts);
        else
        ...
        sol_adv->advertisement_flags = clib_host_to_net_u32(ICMP6_NEIGHBOR_ADVERTISEMENT_FLAG_ROUTER | ICMP6_NEIGHBOR_ADVERTISEMENT_FLAG_SOLICITED
                                                            | ICMP6_NEIGHBOR_ADVERTISEMENT_FLAG_OVERRIDE);

        clib_memcpy(lladdr->ethernet_address, vr->runtime.mac.bytes, sizeof(mac_address_t));
        lladdr->header.type = ICMP6_NEIGHBOR_DISCOVERY_OPTION_target_link_layer_address;

        sol_adv->icmp.checksum = ip6_tcp_udp_icmp_compute_checksum(vm, b, ip6, &bogus_length);
    }
    else {
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (b0->flags & VLIB_BUFFER_IS_TRACED) {
                vrrp_arp_nd_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                vrrp_vr_t           *vr;
...
static_always_inline uword vrrp_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_ipv6)
{
    ...
    while (n_left_from > 0) {
        ...
        if (b0->current_length < sizeof(*vrrp0) + ((u32) vrrp0->n_addrs) * addr_len) {
        ...
        if (!(vr0 = vrrp_vr_lookup(vnet_buffer(b0)->sw_if_index[VLIB_RX], vrrp0->vr_id, is_ipv6))) {
        ...
        vl_api_rpc_call_main_thread(vrrp_input_process, (u8 *) &args0, sizeof(args0));
    ...
    trace:
        ...
        if (b0->flags & VLIB_BUFFER_IS_TRACED) {
            ...
            clib_memcpy_fast(t->addrs, (void *) (vrrp0 + 1), (size_t) vrrp0->n_addrs * addr_len);
        }
...
static_always_inline void vrrp_accept_owner_next_node(u32 sw_if_index, u8 vr_id, u8 is_ipv6, u32 *next_index, u32 *error)
{
    ...
    if (vr && (vr->runtime.state == VRRP_VR_STATE_MASTER) && (vr->config.flags & VRRP_VR_ACCEPT)) {
        ...
        while (n_left_from >= 2 && n_left_to_next >= 2) {
            ...
            if (is_ipv6) {
                ...
                if (PREDICT_FALSE(ip60->protocol == IP_PROTOCOL_VRRP)) {
                    ...
                    vrrp_accept_owner_next_node(sw_if_index0, vrrp0->vr_id, is_ipv6, &next0, &error0);
                }
                if (PREDICT_FALSE(ip61->protocol == IP_PROTOCOL_VRRP)) {
                    vrrp1 = (vrrp_header_t *) (ip61 + 1);
                    vrrp_accept_owner_next_node(sw_if_index1, vrrp1->vr_id, is_ipv6, &next1, &error1);
                }
            }
            else {
            ...
            if (b0->flags & VLIB_BUFFER_IS_TRACED) {
                vrrp_accept_owner_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
            ...
            if (b1->flags & VLIB_BUFFER_IS_TRACED) {
                vrrp_accept_owner_trace_t *t = vlib_add_trace(vm, node, b1, sizeof(*t));
...
static clib_error_t *vrrp_input_init(vlib_main_t *vm)
{
    ...
    if ((error = vlib_call_init_function(vm, vrrp_init))) return error;
