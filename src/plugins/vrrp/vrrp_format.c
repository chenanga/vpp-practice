...
u8 *format_vrrp_vr_flags(u8 *s, va_list *args)
{
...
u8 *format_vrrp_vr_addrs(u8 *s, va_list *args)
{
    ...
    vec_foreach (addr, addrs) {
        s = format(s, "%U ", (is_ipv6) ? format_ip6_address : format_ip4_address, (is_ipv6) ? (u8 *) &addr->ip6 : (u8 *) &addr->ip4);
    }
...
u8 *format_vrrp_vr_key(u8 *s, va_list *args)
{
    ...
    s = format(s, "[%d] sw_if_index %u VR ID %u IPv%d", vr - vmp->vrs, vrc->sw_if_index, vrc->vr_id, (vrc->flags & VRRP_VR_IPV6) ? 6 : 4);

    return s;
}

u8 *format_vrrp_vr_track_ifs(u8 *s, va_list *args)
{
    ...
    vec_foreach (track_if, track_ifs)
        s = format(s, "sw_if_index %u priority %u ", track_if->sw_if_index, track_if->priority);
...
u8 *format_vrrp_vr(u8 *s, va_list *args)
{
    ...
    s = format(s, "   state %U flags: %U\n", format_vrrp_vr_state, vr->runtime.state, format_vrrp_vr_flags, vr->config.flags);
    s = format(s, "   priority: configured %u adjusted %u\n", vr->config.priority, vrrp_vr_priority(vr));
    s = format(s,
               "   timers: adv interval %u "
               "master adv %u skew %u master down %u\n",
               vr->config.adv_interval,
               vr->runtime.master_adv_int,
               vr->runtime.skew,
               vr->runtime.master_down_int);

    s = format(s, "   virtual MAC %U\n", format_ethernet_address, &vr->runtime.mac);

    s = format(s, "   addresses %U\n", format_vrrp_vr_addrs, (vr->config.flags & VRRP_VR_IPV6) != 0, vr->config.vr_addrs);

    s = format(s, "   peer addresses %U\n", format_vrrp_vr_addrs, (vr->config.flags & VRRP_VR_IPV6) != 0, vr->config.peer_addrs);

    s = format(s, "   tracked interfaces %U\n", format_vrrp_vr_track_ifs, vr->tracking.interfaces);
...
u8 *format_vrrp_packet_hdr(u8 *s, va_list *args)
{
    ...
    s = format(s,
               "ver %u, type %u, VRID %u, prio %u, "
               "n_addrs %u, interval %u%ss, csum 0x%x",
               version,
               pkt->vrrp_version_and_type & 0xf,
               pkt->vr_id,
               pkt->priority,
               pkt->n_addrs,
