...
#if OPENSSL_VERSION_NUMBER >= 0x10100000L
#define foreach_openssl_evp_op foreach_openssl_aes_evp_op foreach_openssl_chacha20_evp_op
#else
...
openssl_ops_enc_cbc(vlib_main_t *vm, vnet_crypto_op_t *ops[], vnet_crypto_op_chunk_t *chunks, u32 n_ops, const EVP_CIPHER *cipher, const int iv_len)
{
    ...
    for (i = 0; i < n_ops; i++) {
        ...
        if (op->flags & VNET_CRYPTO_OP_FLAG_CHAINED_BUFFERS) {
            ...
            for (j = 0; j < op->n_chunks; j++) {
                EVP_EncryptUpdate(ctx, out_buf + offset, &out_len, chp->src, chp->len);
                curr_len = chp->len;
                offset += out_len;
                chp += 1;
            }
            if (out_len < curr_len) EVP_EncryptFinal_ex(ctx, out_buf + offset, &out_len);
            ...
            if (out_len < op->len) EVP_EncryptFinal_ex(ctx, op->dst + out_len, &out_len);
        }
...
static_always_inline u32
openssl_ops_dec_cbc(vlib_main_t *vm, vnet_crypto_op_t *ops[], vnet_crypto_op_chunk_t *chunks, u32 n_ops, const EVP_CIPHER *cipher, const int iv_len)
{
    openssl_per_thread_data_t *ptd = vec_elt_at_index(per_thread_data, vm->thread_index);
    EVP_CIPHER_CTX            *ctx;
    vnet_crypto_op_chunk_t    *chp;
    ...
    for (i = 0; i < n_ops; i++) {
        ...
        if (op->flags & VNET_CRYPTO_OP_FLAG_CHAINED_BUFFERS) {
            ...
            u32 offset = 0;
            for (j = 0; j < op->n_chunks; j++) {
                EVP_DecryptUpdate(ctx, out_buf + offset, &out_len, chp->src, chp->len);
                curr_len = chp->len;
                offset += out_len;
                chp += 1;
            }
            if (out_len < curr_len) EVP_DecryptFinal_ex(ctx, out_buf + offset, &out_len);
            ...
            if (out_len < op->len) EVP_DecryptFinal_ex(ctx, op->dst + out_len, &out_len);
        }
        op->status = VNET_CRYPTO_OP_STATUS_COMPLETED;
    }
    return n_ops;
                                              ...
                                              const int               iv_len)
{
    ...
    for (i = 0; i < n_ops; i++) {
        ...
        if (i + 2 < n_ops) {
            ...
            CLIB_PREFETCH(ops[i + 1]->dst, 4 * CLIB_CACHE_PREFETCH_BYTES, STORE);

            CLIB_PREFETCH(ops[i + 2]->src, 4 * CLIB_CACHE_PREFETCH_BYTES, LOAD);
            CLIB_PREFETCH(ops[i + 2]->dst, 4 * CLIB_CACHE_PREFETCH_BYTES, STORE);
        }

        ctx = ptd->evp_cipher_enc_ctx[op->key_index];
        EVP_EncryptInit_ex(ctx, 0, 0, NULL, op->iv);
        if (op->aad_len) EVP_EncryptUpdate(ctx, NULL, &len, op->aad, op->aad_len);
        if (op->flags & VNET_CRYPTO_OP_FLAG_CHAINED_BUFFERS) {
            chp = chunks + op->chunk_index;
            for (j = 0; j < op->n_chunks; j++) {
                EVP_EncryptUpdate(ctx, is_gmac ? 0 : chp->dst, &len, chp->src, chp->len);
                chp += 1;
            }
        }
        else
...
static_always_inline u32 openssl_ops_enc_null_gmac(vlib_main_t            *vm,
                                                   vnet_crypto_op_t       *ops[],
                                                   vnet_crypto_op_chunk_t *chunks,
                                                   u32                     n_ops,
                                                   const EVP_CIPHER       *cipher,
                                                   const int               iv_len)
{
    return openssl_ops_enc_aead(vm,
                                ops,
                                chunks,
                                n_ops,
                                cipher,
...
static_always_inline u32
openssl_ops_enc_gcm(vlib_main_t *vm, vnet_crypto_op_t *ops[], vnet_crypto_op_chunk_t *chunks, u32 n_ops, const EVP_CIPHER *cipher, const int iv_len)
{
    return openssl_ops_enc_aead(vm,
                                ops,
                                chunks,
                                              ...
                                              const int               iv_len)
{
    ...
    for (i = 0; i < n_ops; i++) {
        ...
        if (op->aad_len) EVP_DecryptUpdate(ctx, 0, &len, op->aad, op->aad_len);
        if (op->flags & VNET_CRYPTO_OP_FLAG_CHAINED_BUFFERS) {
            chp = chunks + op->chunk_index;
            for (j = 0; j < op->n_chunks; j++) {
                EVP_DecryptUpdate(ctx, is_gmac ? 0 : chp->dst, &len, chp->src, chp->len);
                chp += 1;
            }
        }
        else {
            EVP_DecryptUpdate(ctx, is_gmac ? 0 : op->dst, &len, op->src, op->len);
        }
...
static_always_inline u32 openssl_ops_dec_null_gmac(vlib_main_t            *vm,
                                                   vnet_crypto_op_t       *ops[],
                                                   vnet_crypto_op_chunk_t *chunks,
                                                   u32                     n_ops,
                                                   const EVP_CIPHER       *cipher,
                                                   const int               iv_len)
{
    return openssl_ops_dec_aead(vm,
                                ops,
                                chunks,
                                n_ops,
                                cipher,
...
static_always_inline u32
openssl_ops_dec_gcm(vlib_main_t *vm, vnet_crypto_op_t *ops[], vnet_crypto_op_chunk_t *chunks, u32 n_ops, const EVP_CIPHER *cipher, const int iv_len)
{
    return openssl_ops_dec_aead(vm,
                                ops,
                                chunks,
...
static_always_inline void *openssl_ctx_cipher(vnet_crypto_key_t      *key,
                                              ...
                                              int                     is_gcm)
{
    ...
    if (VNET_CRYPTO_KEY_OP_ADD == kop) {
        vec_foreach (ptd, per_thread_data) {
            vec_validate_aligned(ptd->evp_cipher_enc_ctx, idx, CLIB_CACHE_LINE_BYTES);
            vec_validate_aligned(ptd->evp_cipher_dec_ctx, idx, CLIB_CACHE_LINE_BYTES);

            ctx = EVP_CIPHER_CTX_new();
            EVP_CIPHER_CTX_set_padding(ctx, 0);
            EVP_EncryptInit_ex(ctx, cipher, NULL, NULL, NULL);
            if (is_gcm) EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_IVLEN, 12, NULL);
            ...
            if (is_gcm) EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_IVLEN, 12, 0);
...
static_always_inline void *openssl_ctx_hmac(vnet_crypto_key_t *key, vnet_crypto_key_op_t kop, vnet_crypto_key_index_t idx, const EVP_MD *md)
{
    HMAC_CTX                  *ctx;
    openssl_per_thread_data_t *ptd;
    if (VNET_CRYPTO_KEY_OP_ADD == kop) {
...
static void crypto_openssl_key_handler(vlib_main_t *vm, vnet_crypto_key_op_t kop, vnet_crypto_key_index_t idx)
{
    vnet_crypto_key_t     *key = vnet_crypto_get_key(idx);
    crypto_openssl_main_t *cm  = &crypto_openssl_main;

    /** TODO: add linked alg support **/
    if (key->type == VNET_CRYPTO_KEY_TYPE_LINK) return;

    if (cm->ctx_fn[key->alg] == 0) return;

    cm->ctx_fn[key->alg](key, kop, idx);
}

#define _(m, a, b, iv)                                                                                                          \
    static u32 openssl_ops_enc_##a(vlib_main_t *vm, vnet_crypto_op_t *ops[], u32 n_ops)                                         \
    {                                                                                                                           \
        return openssl_ops_enc_##m(vm, ops, 0, n_ops, b(), iv);                                                                 \
    }                                                                                                                           \
                                                                                                                                \
    u32 openssl_ops_dec_##a(vlib_main_t *vm, vnet_crypto_op_t *ops[], u32 n_ops)                                                \
    {                                                                                                                           \
        return openssl_ops_dec_##m(vm, ops, 0, n_ops, b(), iv);                                                                 \
    }                                                                                                                           \
                                                                                                                                \
    static u32 openssl_ops_enc_chained_##a(vlib_main_t *vm, vnet_crypto_op_t *ops[], vnet_crypto_op_chunk_t *chunks, u32 n_ops) \
    {                                                                                                                           \
        return openssl_ops_enc_##m(vm, ops, chunks, n_ops, b(), iv);                                                            \
    }                                                                                                                           \
                                                                                                                                \
    static u32 openssl_ops_dec_chained_##a(vlib_main_t *vm, vnet_crypto_op_t *ops[], vnet_crypto_op_chunk_t *chunks, u32 n_ops) \
    {                                                                                                                           \
        return openssl_ops_dec_##m(vm, ops, chunks, n_ops, b(), iv);                                                            \
    }                                                                                                                           \
    static void *openssl_ctx_##a(vnet_crypto_key_t *key, vnet_crypto_key_op_t kop, vnet_crypto_key_index_t idx)                 \
    {                                                                                                                           \
        int is_gcm = ((VNET_CRYPTO_ALG_AES_128_GCM <= key->alg) && (VNET_CRYPTO_ALG_AES_256_NULL_GMAC >= key->alg)) ? 1 : 0;    \
...
#define _(a, b)                                                                                                                  \
    static u32 openssl_ops_hash_##a(vlib_main_t *vm, vnet_crypto_op_t *ops[], u32 n_ops)                                         \
    {                                                                                                                            \
        return openssl_ops_hash(vm, ops, 0, n_ops, b());                                                                         \
    }                                                                                                                            \
    static u32 openssl_ops_hash_chained_##a(vlib_main_t *vm, vnet_crypto_op_t *ops[], vnet_crypto_op_chunk_t *chunks, u32 n_ops) \
    {                                                                                                                            \
...
#define _(a, b)                                                                                                                  \
    static u32 openssl_ops_hmac_##a(vlib_main_t *vm, vnet_crypto_op_t *ops[], u32 n_ops)                                         \
    {                                                                                                                            \
        return openssl_ops_hmac(vm, ops, 0, n_ops, b());                                                                         \
    }                                                                                                                            \
    static u32 openssl_ops_hmac_chained_##a(vlib_main_t *vm, vnet_crypto_op_t *ops[], vnet_crypto_op_chunk_t *chunks, u32 n_ops) \
    {                                                                                                                            \
        return openssl_ops_hmac(vm, ops, chunks, n_ops, b());                                                                    \
    }                                                                                                                            \
    static void *openssl_ctx_hmac_##a(vnet_crypto_key_t *key, vnet_crypto_key_op_t kop, vnet_crypto_key_index_t idx)             \
    {                                                                                                                            \
...
#define _(m, a, b, iv)                                                                                                       \
    vnet_crypto_register_ops_handlers(vm, eidx, VNET_CRYPTO_OP_##a##_ENC, openssl_ops_enc_##a, openssl_ops_enc_chained_##a); \
    vnet_crypto_register_ops_handlers(vm, eidx, VNET_CRYPTO_OP_##a##_DEC, openssl_ops_dec_##a, openssl_ops_dec_chained_##a); \
...
#define _(a, b)                                                                                                                 \
    vnet_crypto_register_ops_handlers(vm, eidx, VNET_CRYPTO_OP_##a##_HMAC, openssl_ops_hmac_##a, openssl_ops_hmac_chained_##a); \
    cm->ctx_fn[VNET_CRYPTO_ALG_HMAC_##a] = openssl_ctx_hmac_##a;

    foreach_openssl_hmac_op;
#undef _

#define _(a, b) vnet_crypto_register_ops_handlers(vm, eidx, VNET_CRYPTO_OP_##a##_HASH, openssl_ops_hash_##a, openssl_ops_hash_chained_##a);

    foreach_openssl_hash_op;
#undef _

    vec_validate_aligned(per_thread_data, tm->n_vlib_mains - 1, CLIB_CACHE_LINE_BYTES);
    ...
    vnet_crypto_register_key_handler(vm, cm->crypto_engine_index, crypto_openssl_key_handler);
    return 0;
}
