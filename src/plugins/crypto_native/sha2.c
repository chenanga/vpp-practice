...
static_always_inline u32 crypto_native_ops_hash_sha2(vlib_main_t            *vm,
...
crypto_native_ops_hmac_sha2(vlib_main_t *vm, vnet_crypto_op_t *ops[], u32 n_ops, vnet_crypto_op_chunk_t *chunks, clib_sha2_type_t type)
{
    ...
    for (; n_left; n_left--, op++) {
        clib_sha2_hmac_init(&ctx, type, (clib_sha2_hmac_key_data_t *) cm->key_data[op->key_index]);
        if (op->flags & VNET_CRYPTO_OP_FLAG_CHAINED_BUFFERS) {
            vnet_crypto_op_chunk_t *chp = chunks + op->chunk_index;
            for (int j = 0; j < op->n_chunks; j++, chp++)
...
static void *sha2_key_add(vnet_crypto_key_t *key, clib_sha2_type_t type)
{
...
static int probe()
{
    ...
    if (clib_cpu_supports_sha() && clib_cpu_supports_avx512f()) return 30;
#elif defined(__SHA__) && defined(__AVX2__)
    if (clib_cpu_supports_sha() && clib_cpu_supports_avx2()) return 20;
#elif defined(__SHA__)
    if (clib_cpu_supports_sha()) return 10;
#endif

#elif defined(__aarch64__)
#if defined(__ARM_FEATURE_SHA2)
    if (clib_cpu_supports_sha2()) return 10;
#endif
#endif
    return -1;
}
...
#define _(b)                                                                                                                              \
    ...
    {                                                                                                                                     \
        return crypto_native_ops_hash_sha2(vm, ops, n_ops, 0, CLIB_SHA2_##b, 0);                                                          \
                                                                                                                                          ...
                                                                                                                                          \
    static u32 crypto_native_ops_chained_hash_sha##b(vlib_main_t *vm, vnet_crypto_op_t *ops[], vnet_crypto_op_chunk_t *chunks, u32 n_ops) \
    {                                                                                                                                     \
    ...
    {                                                                                                                                     \
        return crypto_native_ops_hmac_sha2(vm, ops, n_ops, 0, CLIB_SHA2_##b);                                                             \
                                                                                                                                          ...
                                                                                                                                          \
    static u32 crypto_native_ops_chained_hmac_sha##b(vlib_main_t *vm, vnet_crypto_op_t *ops[], vnet_crypto_op_chunk_t *chunks, u32 n_ops) \
    {                                                                                                                                     \
