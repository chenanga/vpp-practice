...
static void crypto_native_key_handler(vlib_main_t *vm, vnet_crypto_key_op_t kop, vnet_crypto_key_index_t idx)
{
    ...
    if (cm->key_fn[key->alg] == 0) return;

    if (kop == VNET_CRYPTO_KEY_OP_DEL) {
        if (idx >= vec_len(cm->key_data)) return;

        if (cm->key_data[idx] == 0) return;
...
clib_error_t *crypto_native_init(vlib_main_t *vm)
{
    ...
    while (oh) {
        ...
        if (best_by_op_id[oh->op_id] == 0 || best_by_op_id[oh->op_id]->priority < oh->priority) best_by_op_id[oh->op_id] = oh;
    ...
    while (kh) {
        ...
        if (best_by_alg_id[kh->alg_id] == 0 || best_by_alg_id[kh->alg_id]->priority < kh->priority) best_by_alg_id[kh->alg_id] = kh;

        kh = kh->next;
    }

    vec_foreach_pointer (oh, best_by_op_id)
        if (oh) vnet_crypto_register_ops_handlers(vm, cm->crypto_engine_index, oh->op_id, oh->fn, oh->cfn);

    vec_foreach_pointer (kh, best_by_alg_id)
        if (kh) cm->key_fn[kh->alg_id] = kh->key_fn;

    vec_free(best_by_op_id);
    vec_free(best_by_alg_id);

    vnet_crypto_register_key_handler(vm, cm->crypto_engine_index, crypto_native_key_handler);
    return 0;
}
