...
static void ct6_feature_init(ct6_main_t *cmp)
{
    ...
    if (cmp->feature_initialized) return;

    clib_bihash_init_48_8(&cmp->session_hash, "ct6 session table", cmp->session_hash_buckets, cmp->session_hash_memory);
...
int ct6_in2out_enable_disable(ct6_main_t *cmp, u32 sw_if_index, int enable_disable)
{
    ...
    /* Utterly wrong? */
    if (pool_is_free_index(cmp->vnet_main->interface_main.sw_interfaces, sw_if_index)) return VNET_API_ERROR_INVALID_SW_IF_INDEX;
    ...
    vnet_feature_enable_disable("interface-output", "ct6-in2out", sw_if_index, enable_disable, 0, 0);
...
int ct6_out2in_enable_disable(ct6_main_t *cmp, u32 sw_if_index, int enable_disable)
{
    ...
    vnet_feature_enable_disable("ip6-unicast", "ct6-out2in", sw_if_index, enable_disable, 0, 0);
...
static clib_error_t *set_ct6_enable_disable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "disable"))
            enable_disable = 0;
        else if (unformat(input, "%U", unformat_vnet_sw_interface, cmp->vnet_main, &sw_if_index))
            ;
    ...
    if (inside == ~0) return clib_error_return(0, "Please specify inside or outside");
...
static void vl_api_ct6_enable_disable_t_handler(vl_api_ct6_enable_disable_t *mp)
{
    ...
    if (mp->is_inside)
        rv = ct6_in2out_enable_disable(cmp, ntohl(mp->sw_if_index), (int) (mp->enable_disable));
    else
        rv = ct6_out2in_enable_disable(cmp, ntohl(mp->sw_if_index), (int) (mp->enable_disable));
...
u8 *format_ct6_session(u8 *s, va_list *args)
{
    ...
    if (s0 == 0) {
        s = format(s, "\n%6s%6s%40s%6s%40s%6s", "Sess", "Prot", "Src", "Sport", "Dst", "Dport");
        return s;
    }
    ...
    if (clib_bihash_search_48_8(&cmp->session_hash, &kvp0, &kvp0) < 0) {
        ...
        if (kvp0.value == s0 - cmp->sessions[s0->thread_index]) {
            s = format(s, " OK");
            if (verbose > 1) {
                s = format(s, " next %d prev %d", s0->next_index, s0->prev_index);
...
static clib_error_t *show_ct6_command_fn_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!cmp->feature_initialized) return clib_error_return(0, "ip6 connection tracking not enabled...");
    ...
    for (i = 0; i < vec_len(cmp->sessions); i++) {
        s = format(s, "Thread %d: %d sessions\n", i, pool_elts(cmp->sessions[i]));

        if (verbose == 0) continue;

        s = format(s, "%U", format_ct6_session, cmp, 0 /* pool */, 0 /* header */, verbose);
...
static clib_error_t *test_ct6_command_fn_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "num-sessions %d", &num_sessions))
            ;
        else if (unformat(input, "max-sessions %d", &cmp->max_sessions_per_worker))
            ;
        else
            break;
    }
    ...
    for (i = 0; i < num_sessions; i++) {
        ...
        s0 = ct6_create_or_recycle_session(cmp, &kvp0, 3.0 /* now */, 0 /* thread index */, &recycled, &created);

        s = format(s, "%U (%d, %d)", format_ct6_session, cmp, 0 /* thread index */, s0, 1 /* verbose */, recycled, created);
    ...
    vlib_cli_output(vm, "\nEnd state: first index %d last index %d\n%v", cmp->first_index[0], cmp->last_index[0], s);
    ...
    vlib_cli_output(vm, "\nSimulate LRU hit on session %d", s0 - cmp->sessions[0]);
...
static clib_error_t *ct6_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "session-hash-buckets %u", &cmp->session_hash_buckets))
            ;
        else if (unformat(input, "session-hash-memory %U", unformat_memory_size, &cmp->session_hash_memory))
            ;
        else if (unformat(input, "session-timeout %f", &cmp->session_timeout_interval))
            ;
        else {
