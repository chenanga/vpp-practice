...
static inline void ct6_lru_remove(ct6_main_t *cmp, ct6_session_t *s0)
{
    ...
    if (s0_index == cmp->first_index[thread_index]) cmp->first_index[thread_index] = s0->next_index;
    if (s0_index == cmp->last_index[thread_index]) cmp->last_index[thread_index] = s0->prev_index;

    /* Fix next->prev */
    if (s0->next_index != ~0) {
        next_sess             = pool_elt_at_index(cmp->sessions[thread_index], s0->next_index);
        next_sess->prev_index = s0->prev_index;
    }
    /* Fix prev->next */
    if (s0->prev_index != ~0) {
        prev_sess             = pool_elt_at_index(cmp->sessions[thread_index], s0->prev_index);
        prev_sess->next_index = s0->next_index;
    }
}

static inline void ct6_lru_add(ct6_main_t *cmp, ct6_session_t *s0, f64 now)
{
    ...
    if (cmp->first_index[thread_index] != ~0) {
        next_sess             = pool_elt_at_index(cmp->sessions[thread_index], cmp->first_index[thread_index]);
    ...
    if (cmp->last_index[thread_index] == ~0) cmp->last_index[thread_index] = s0_index;
}

static inline void ct6_update_session_hit(ct6_main_t *cmp, ct6_session_t *s0, f64 now)
{
