...
always_inline uword ct6_out2in_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_trace)
{
  ...
  while (n_left_from >= 4)
    ...
    while (n_left_from > 0) {
         ...
         */
        if (PREDICT_FALSE(!ip6_address_is_global_unicast(&ip0->src_address) || !ip6_address_is_global_unicast(&ip0->src_address))) goto trace0;
        ...
        clib_memcpy_fast(&key0->src, &ip0->src_address, sizeof(ip6_address_t));
        clib_memcpy_fast(&key0->dst, &ip0->dst_address, sizeof(ip6_address_t));
        key0->as_u64[4] = 0;
        key0->as_u64[5] = 0;
    ...
    trace0:
        if (is_trace) {
            if (b[0]->flags & VLIB_BUFFER_IS_TRACED) {
                ct6_out2in_trace_t *t = vlib_add_trace(vm, node, b[0], sizeof(*t));
                t->next_index         = next[0];
                t->sw_if_index        = vnet_buffer(b[0])->sw_if_index[VLIB_RX];
    ...
    vlib_node_increment_counter(vm, node->node_index, CT6_OUT2IN_ERROR_NO_SESSION, dropped);

    return frame->n_vectors;
}

VLIB_NODE_FN(ct6_out2in_node)
