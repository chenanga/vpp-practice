...
static inline void quic_crypto_context_make_key_from_ctx(clib_bihash_kv_24_8_t *kv, quic_ctx_t *ctx)
{
...
static inline void quic_crypto_context_make_key_from_crctx(clib_bihash_kv_24_8_t *kv, crypto_context_t *crctx)
{
    quic_crypto_context_data_t *data = (quic_crypto_context_data_t *) crctx->data;
...
static void quic_crypto_context_free_if_needed(crypto_context_t *crctx, u8 thread_index)
{
    ...
    if (crctx->n_subscribers) return;
    quic_crypto_context_make_key_from_crctx(&kv, crctx);
    clib_bihash_add_del_24_8(&qm->wrk_ctx[thread_index].crypto_context_hash, &kv, 0 /* is_add */);
...
static crypto_context_t *quic_crypto_context_get(u32 cr_index, u32 thread_index)
{
    ...
    return pool_elt_at_index(qm->wrk_ctx[thread_index].crypto_ctx_pool, cr_index & 0x00ffffff);
}
...
static int quic_init_crypto_context(crypto_context_t *crctx, quic_ctx_t *ctx)
{
    ...
    QUIC_DBG(2, "Init quic crctx %d thread %d", crctx->ctx_index, ctx->c_thread_index);
    ...
    if (qm->vnet_crypto_enabled && qm->default_crypto_engine == CRYPTO_ENGINE_VPP)
    ...
    quicly_ctx->transport_params.max_stream_data.bidi_local  = app->sm_properties.rx_fifo_size - 1;
    quicly_ctx->transport_params.max_stream_data.bidi_remote = app->sm_properties.tx_fifo_size - 1;
    ...
    quicly_ctx->cid_encryptor = quicly_new_default_cid_encryptor(&ptls_openssl_bfecb, &ptls_openssl_aes128ecb, &ptls_openssl_sha256, key_vec);
...
static int quic_acquire_crypto_context(quic_ctx_t *ctx)
{
    ...
    if (!clib_bitmap_get(qm->available_crypto_engines, ctx->crypto_engine)) {
        QUIC_DBG(1, "Quic does not support crypto engine %d", ctx->crypto_engine);
        return SESSION_E_NOCRYPTOENG;
    }

    /* Check for exisiting crypto ctx */
    quic_crypto_context_make_key_from_ctx(&kv, ctx);
    if (clib_bihash_search_24_8(&qm->wrk_ctx[ctx->c_thread_index].crypto_context_hash, &kv, &kv) == 0) {
    ...
    if (quic_init_crypto_context(crctx, ctx)) goto error;
    if (vnet_app_add_cert_key_interest(ctx->ckpair_index, qm->app_index)) goto error;
    crctx->n_subscribers++;
    clib_bihash_add_del_24_8(&qm->wrk_ctx[ctx->c_thread_index].crypto_context_hash, &kv, 1 /* is_add */);
    return 0;
...
static u32 quic_ctx_alloc(u32 thread_index)
{
    ...
    pool_get_aligned_safe(qm->ctx_pool[thread_index], ctx, CLIB_CACHE_LINE_BYTES);

    clib_memset(ctx, 0, sizeof(quic_ctx_t));
    ctx->c_thread_index = thread_index;
    ctx->timer_handle   = QUIC_TIMER_HANDLE_INVALID;
    QUIC_DBG(3, "Allocated quic_ctx %u on thread %u", ctx - qm->ctx_pool[thread_index], thread_index);
...
{
    if (pool_is_free_index(quic_main.ctx_pool[thread_index], ctx_index)) return 0;
    return pool_elt_at_index(quic_main.ctx_pool[thread_index], ctx_index);
}
...
{
    *quicly_get_data(conn) = (void *) (((u64) ctx->c_thread_index) << 32 | (u64) ctx->c_c_index);
}

static inline int quic_ctx_is_stream(quic_ctx_t *ctx)
{
    return (ctx->flags & QUIC_F_IS_STREAM);
}

static inline int quic_ctx_is_listener(quic_ctx_t *ctx)
{
    return (ctx->flags & QUIC_F_IS_LISTENER);
}

static inline int quic_ctx_is_conn(quic_ctx_t *ctx)
{
    return !(quic_ctx_is_listener(ctx) || quic_ctx_is_stream(ctx));
}

static inline session_t *get_stream_session_and_ctx_from_stream(quicly_stream_t *stream, quic_ctx_t **ctx)
{
...
static inline void quic_make_connection_key(clib_bihash_kv_16_8_t *kv, const quicly_cid_plaintext_t *id)
{
...
{
    crypto_context_t           *crctx = quic_crypto_context_get(ctx->crypto_context_index, ctx->c_thread_index);
    quic_crypto_context_data_t *data  = (quic_crypto_context_data_t *) crctx->data;
...
static quicly_context_t *quic_get_quicly_ctx_from_udp(u64 udp_session_handle)
{
    ...
    quic_ctx_t *ctx         = quic_ctx_get(udp_session->opaque, udp_session->thread_index);
    return quic_get_quicly_ctx_from_ctx(ctx);
}

static inline void quic_set_udp_tx_evt(session_t *udp_session)
{
    int rv = 0;
    if (svm_fifo_set_event(udp_session->tx_fifo)) rv = session_send_io_evt_to_thread(udp_session->tx_fifo, SESSION_IO_EVT_TX);
    if (PREDICT_FALSE(rv)) clib_warning("Event enqueue errored %d", rv);
}

static inline void quic_stop_ctx_timer(quic_ctx_t *ctx)
{
    tw_timer_wheel_1t_3w_1024sl_ov_t *tw;
    if (ctx->timer_handle == QUIC_TIMER_HANDLE_INVALID) return;
...
static void quic_ack_rx_data(session_t *stream_session)
{
    ...
    sctx = quic_ctx_get(stream_session->connection_index, stream_session->thread_index);
...
static void quic_disconnect_transport(quic_ctx_t *ctx)
{
    ...
    if (vnet_disconnect_session(&a)) clib_warning("UDP session 0x%lx disconnect errored", ctx->udp_session_handle);
}
...
static void quic_connection_delete(quic_ctx_t *ctx)
{
    ...
    if (ctx->conn == NULL) {
        QUIC_DBG(2, "Skipping redundant delete of connection %u", ctx->c_c_index);
        return;
    }
...
{
    QUIC_DBG(2, "QUIC connection %u/%u closed", ctx->c_thread_index, ctx->c_c_index);
...
static int quic_send_datagram(session_t *udp_session, struct iovec *packet, quicly_address_t *dest, quicly_address_t *src)
{
    ...
    if (max_enqueue < SESSION_CONN_HDR_LEN + len) {
        QUIC_ERR("Too much data to send, max_enqueue %u, len %u", max_enqueue, len + SESSION_CONN_HDR_LEN);
    ...
    svm_fifo_seg_t segs[2] = {{(u8 *) &hdr, sizeof(hdr)}, {packet->iov_base, len}};
...
static int quic_send_packets(quic_ctx_t *ctx)
{
    ...
    uint8_t          buf[QUIC_SEND_PACKET_VEC_SIZE * quic_get_quicly_ctx_from_ctx(ctx)->transport_params.max_udp_payload_size];
    ...
    if (quic_ctx_is_stream(ctx)) ctx = quic_ctx_get(ctx->quic_connection_ctx_id, ctx->c_thread_index);

    QUIC_ASSERT(!quic_ctx_is_stream(ctx));

    udp_session = session_get_from_handle_if_valid(ctx->udp_session_handle);
    if (!udp_session) goto quicly_error;
    ...
    do {
        ...
        if (max_packets < 2) break;

        num_packets = max_packets;
        if ((err = quicly_send(conn, &dest, &src, packets, &num_packets, buf, sizeof(buf)))) goto quicly_error;

        for (i = 0; i != num_packets; ++i) {
            if ((err = quic_send_datagram(udp_session, &packets[i], &dest, &src))) goto quicly_error;
        }
        n_sent += num_packets;
    ...
    QUIC_DBG(3, "%u[TX] %u[RX]", svm_fifo_max_dequeue(udp_session->tx_fifo), svm_fifo_max_dequeue(udp_session->rx_fifo));
    quic_update_timer(ctx);
    return n_sent;

quicly_error:
    if (err && err != QUICLY_ERROR_PACKET_IGNORED && err != QUICLY_ERROR_FREE_CONNECTION) clib_warning("Quic error '%U'.", quic_format_err, err);
...
static void quic_on_stream_destroy(quicly_stream_t *stream, int err)
{
    ...
    quic_ctx_t         *sctx        = quic_ctx_get(stream_data->ctx_id, stream_data->thread_index);
    QUIC_DBG(2, "DESTROYED_STREAM: session 0x%lx (%U)", session_handle(stream_session), quic_format_err, err);
...
static void quic_on_stop_sending(quicly_stream_t *stream, int err)
{
    ...
    quic_stream_data_t *stream_data    = (quic_stream_data_t *) stream->data;
    quic_ctx_t         *sctx           = quic_ctx_get(stream_data->ctx_id, stream_data->thread_index);
    session_t          *stream_session = session_get(sctx->c_s_index, sctx->c_thread_index);
    clib_warning("(NOT IMPLEMENTD) STOP_SENDING: session 0x%lx (%U)", session_handle(stream_session), quic_format_err, err);
#endif
...
#if QUIC_DEBUG >= 2
    session_t *stream_session = session_get(sctx->c_s_index, sctx->c_thread_index);
    clib_warning("RESET_STREAM: session 0x%lx (%U)", session_handle(stream_session), quic_format_err, err);
#endif
    session_transport_closing_notify(&sctx->connection);
}
...
void quic_fifo_egress_emit(quicly_stream_t *stream, size_t off, void *dst, size_t *len, int *wrote_all)
{
    ...
    if (off + *len > stream_data->app_tx_data_len) stream_data->app_tx_data_len = off + *len;

    svm_fifo_peek(f, off, *len, dst);
}

static const quicly_stream_callbacks_t quic_stream_callbacks = {.on_destroy       = quic_on_stream_destroy,
...
static int quic_on_stream_open(quicly_stream_open_t *self, quicly_stream_t *stream)
{
     ...
     * initiated stream */
    if (quicly_stream_is_self_initiated(stream)) return 0;
    ...
    QUIC_DBG(2, "ACCEPTED stream_session 0x%lx ctx %u", session_handle(stream_session), sctx_id);
    ...
    if (quicly_stream_is_unidirectional(stream->stream_id)) stream_session->flags |= SESSION_F_UNIDIRECTIONAL;
    ...
    stream_session->session_type     = session_type_from_proto_and_ip(TRANSPORT_PROTO_QUIC, qctx->udp_is_ip4);
    ...
    svm_fifo_add_want_deq_ntf(stream_session->rx_fifo, SVM_FIFO_WANT_DEQ_NOTIF_IF_FULL | SVM_FIFO_WANT_DEQ_NOTIF_IF_EMPTY);
                                     ...
                                     size_t                     reason_len)
{
    ...
    clib_warning("Session 0x%lx closed by peer (%U) %.*s ", session_handle(quic_session), quic_format_err, code, reason_len, reason);
#endif
...
{
    quic_ctx_t *ctx;
    QUIC_DBG(4, "Timer expired for conn %u at %ld", conn_index, quic_get_time(NULL));
...
static void quic_update_timer(quic_ctx_t *ctx)
{
    ...
    if (next_timeout == 0 || next_interval <= 0) {
        if (ctx->c_s_index == QUIC_SESSION_INVALID) {
            ...
            if (svm_fifo_set_event(quic_session->tx_fifo)) {
                rv = session_send_io_evt_to_thread_custom(quic_session, quic_session->thread_index, SESSION_IO_EVT_TX);
                if (PREDICT_FALSE(rv)) QUIC_ERR("Failed to enqueue builtin_tx %d", rv);
            }
            return;
        }
    }
    ...
    QUIC_DBG(4, "Timer set to %ld (int %ld) for ctx %u", next_timeout, next_interval, ctx->c_c_index);
    ...
    if (ctx->timer_handle == QUIC_TIMER_HANDLE_INVALID) {
        ...
        ctx->timer_handle = tw_timer_start_1t_3w_1024sl_ov(tw, ctx->c_c_index, 0, next_interval);
    }
    else {
        if (next_timeout == INT64_MAX) {
            quic_stop_ctx_timer(ctx);
...
static int quic_connect_stream(session_t *quic_session, session_endpoint_cfg_t *sep)
{
    ...
    if (session_type_transport_proto(quic_session->session_type) != TRANSPORT_PROTO_QUIC) {
    ...
    qctx       = quic_ctx_get(quic_session->connection_index, quic_session->thread_index);
    ...
    if (!conn || !quicly_connection_is_ready(conn)) return -1;
    ...
    stream_session = session_alloc(qctx->c_thread_index);
    QUIC_DBG(2, "Allocated stream_session 0x%lx ctx %u", session_handle(stream_session), sctx_index);
    ...
    stream_session->session_type     = session_type_from_proto_and_ip(TRANSPORT_PROTO_QUIC, qctx->udp_is_ip4);
    if (is_unidir) stream_session->flags |= SESSION_F_UNIDIRECTIONAL;
    ...
    if (app_worker_connect_notify(app_wrk, stream_session, SESSION_E_NONE, sep->opaque)) {
...
static int quic_connect_connection(session_endpoint_cfg_t *sep)
{
    ...
    if (!sep->ext_cfg) return SESSION_E_NOEXTCFG;
    ...
    if ((error = quic_acquire_crypto_context(ctx))) return error;

    if ((error = vnet_connect(cargs))) return error;
...
static void quic_proto_on_close(u32 ctx_index, u32 thread_index)
{
    ...
    if (!ctx) return;
    session_t *stream_session = session_get(ctx->c_s_index, ctx->c_thread_index);
#if QUIC_DEBUG >= 2
    clib_warning("Closing session 0x%lx", session_handle(stream_session));
#endif
    if (quic_ctx_is_stream(ctx)) {
        quicly_stream_t *stream = ctx->stream;
        if (!quicly_stream_has_send_side(quicly_is_client(stream->conn), stream->stream_id)) return;
        quicly_sendstate_shutdown(&stream->sendstate, ctx->bytes_written + svm_fifo_max_dequeue(stream_session->tx_fifo));
        err = quicly_stream_sync_sendbuf(stream, 1);
        if (err) {
            QUIC_DBG(1, "sendstate_shutdown failed for stream session %lu", session_handle(stream_session));
...
static u32 quic_start_listen(u32 quic_listen_session_index, transport_endpoint_cfg_t *tep)
{
    ...
    if ((rv = vnet_listen(args))) return rv;
    ...
    if ((rv = quic_acquire_crypto_context(lctx))) return rv;

    QUIC_DBG(2, "Listening UDP session 0x%lx", session_handle(udp_listen_session));
...
static u32 quic_stop_listen(u32 lctx_index)
{
    ...
    if (vnet_unlisten(&a)) clib_warning("unlisten errored");

    quic_release_crypto_context(lctx->crypto_context_index, 0 /* thread_index */);
    quic_ctx_free(lctx);
    return 0;
}
...
static u8 *format_quic_ctx(u8 *s, va_list *args)
{
    ...
    if (quic_ctx_is_listener(ctx))
        ...
        str = format(str, "Stream %ld conn %d", ctx->stream->stream_id, ctx->quic_connection_ctx_id);
    else /* connection */
        str = format(str, "Conn %d UDP %d", ctx->c_c_index, ctx->udp_session_handle);

    str = format(str, " app %d wrk %d", ctx->parent_app_id, ctx->parent_app_wrk_id);
...
static u8 *format_quic_half_open(u8 *s, va_list *args)
{
    ...
    u32         thread_index = va_arg(*args, u32);
    quic_ctx_t *ctx          = quic_ctx_get(qc_index, thread_index);
    s                        = format(s, "[#%d][Q] half-open app %u", thread_index, ctx->parent_app_id);
    return s;
}
...
static inline void quic_build_sockaddr(struct sockaddr *sa, socklen_t *salen, ip46_address_t *addr, u16 port, u8 is_ip4)
{
    if (is_ip4) {
...
static void quic_on_quic_session_connected(quic_ctx_t *ctx)
{
    ...
    quic_session->session_type     = session_type_from_proto_and_ip(TRANSPORT_PROTO_QUIC, ctx->udp_is_ip4);
    ...
    if ((rv = app_worker_connect_notify(app_wrk, quic_session, SESSION_E_NONE, ctx->client_opaque))) {
...
static void quic_check_quic_session_connected(quic_ctx_t *ctx)
{
    ...
    if (!ctx->conn || ctx->conn_state != QUIC_CONN_STATE_HANDSHAKE) return;
    if (!quicly_connection_is_ready(ctx->conn)) return;
    ctx->conn_state = QUIC_CONN_STATE_READY;
    if (!quicly_is_client(ctx->conn)) return;
    quic_on_quic_session_connected(ctx);
}

static inline void quic_update_conn_ctx(quicly_conn_t *conn, quicly_context_t *quicly_context)
{
...
static void quic_receive_connection(void *arg)
{
    ...
    QUIC_DBG(2, "Received conn %u (now %u)", temp_ctx->c_thread_index, new_ctx_id);
    ...
    if (svm_fifo_max_dequeue(udp_session->tx_fifo)) quic_set_udp_tx_evt(udp_session);
}
...
static void quic_transfer_connection(u32 ctx_index, u32 dest_thread)
{
    ...
    session_send_rpc_evt_to_thread(dest_thread, quic_receive_connection, (void *) temp_ctx);
}
...
{
    QUIC_DBG(2, "UDP Session is now connected (id %u)", udp_session->session_index);
    ...
    ret        = quicly_connect(&ctx->conn,
                         quicly_ctx,
                         (char *) ctx->srv_hostname,
                         sa,
                         NULL,
                         &quic_main.wrk_ctx[thread_index].next_cid,
                         ptls_iovec_init(NULL, 0),
                         &quic_main.hs_properties,
                         NULL);
...
static void quic_udp_session_cleanup_callback(session_t *udp_session, session_cleanup_ntf_t ntf)
{
    ...
    if (ntf != SESSION_CLEANUP_SESSION) return;
...
int quic_udp_session_accepted_callback(session_t *udp_session)
{
    ...
    udp_listen_session = listen_session_get_from_handle(udp_session->listener_handle);
    ...
    lctx                   = quic_ctx_get(udp_listen_session->opaque, udp_listen_session->thread_index);
...
static int quic_custom_app_rx_callback(transport_connection_t *tc)
{
    ...
    ctx = quic_ctx_get(stream_session->connection_index, stream_session->thread_index);
    quic_send_packets(ctx);
    return 0;
}
...
static int quic_custom_tx_callback(void *s, transport_send_params_t *sp)
{
    ...
    if (PREDICT_FALSE(stream_session->session_state >= SESSION_STATE_TRANSPORT_CLOSING)) return 0;
    ctx = quic_ctx_get(stream_session->connection_index, stream_session->thread_index);
 ...
 * will be set.
 */
static inline int quic_find_packet_ctx(quic_rx_packet_ctx_t *pctx, u32 caller_thread_index)
{
    clib_bihash_kv_16_8_t kv;
    ...
    if (!quicly_is_destination(ctx->conn, NULL, &pctx->sa, &pctx->packet)) return QUIC_PACKET_TYPE_NONE;
...
static void quic_accept_connection(quic_rx_packet_ctx_t *pctx)
{
    ...
    quicly_ctx = quic_get_quicly_ctx_from_ctx(ctx);
    if ((rv = quicly_accept(&conn, quicly_ctx, NULL, &pctx->sa, &pctx->packet, NULL, &quic_main.wrk_ctx[pctx->thread_index].next_cid, NULL))) {
        /* Invalid packet, pass */
        assert(conn == NULL);
    ...
    QUIC_DBG(2, "Allocated quic_session, 0x%lx ctx %u", session_handle(quic_session), ctx->c_c_index);
    ctx->c_s_index = quic_session->session_index;
...
static int quic_reset_connection(u64 udp_session_handle, quic_rx_packet_ctx_t *pctx)
{
    ...
    session_t        *udp_session;
    quicly_context_t *quicly_ctx;
    if (pctx->packet.cid.dest.plaintext.node_id != 0 || pctx->packet.cid.dest.plaintext.thread_id != 0) return 0;
    ...
    size_t           payload_len = quicly_send_stateless_reset(quicly_ctx, &src.sa, payload);
    ...
    struct _st_quicly_conn_public_t *conn = (struct _st_quicly_conn_public_t *) qctx->conn;

    udp_session = session_get_from_handle(udp_session_handle);
    rv          = quic_send_datagram(udp_session, &packet, &conn->remote.address, &conn->local.address);
    quic_set_udp_tx_evt(udp_session);
    return rv;
}
...
static int quic_process_one_rx_packet(u64 udp_session_handle, svm_fifo_t *f, u32 fifo_offset, quic_rx_packet_ctx_t *pctx)
{
    ...
    ret = svm_fifo_peek(f, SESSION_CONN_HDR_LEN + fifo_offset, pctx->ph.data_length, pctx->data);
    ...
    quic_build_sockaddr(&pctx->sa, &pctx->salen, &pctx->ph.rmt_ip, pctx->ph.rmt_port, pctx->ph.is_ip4);
    quicly_ctx = quic_get_quicly_ctx_from_udp(udp_session_handle);

    size_t off = 0;
    plen       = quicly_decode_packet(quicly_ctx, &pctx->packet, pctx->data, pctx->ph.data_length, &off);
    ...
    if (rv == QUIC_PACKET_TYPE_RECEIVE) {
        ...
        if (quic_main.vnet_crypto_enabled && quic_main.default_crypto_engine == CRYPTO_ENGINE_VPP) {
...
rx_start:
    max_deq = svm_fifo_max_dequeue(f);
    if (max_deq == 0) return 0;

    fifo_offset = 0;
    max_packets = QUIC_RCV_MAX_PACKETS;

#if CLIB_DEBUG > 0
    clib_memset(packets_ctx, 0xfa, QUIC_RCV_MAX_PACKETS * sizeof(quic_rx_packet_ctx_t));
#endif
    for (i = 0; i < max_packets; i++) {
        ...
        rv = quic_process_one_rx_packet(udp_session_handle, f, fifo_offset, &packets_ctx[i]);
        if (packets_ctx[i].ptype != QUIC_PACKET_TYPE_MIGRATE) fifo_offset += SESSION_CONN_HDR_LEN + packets_ctx[i].ph.data_length;
        if (rv) {
            max_packets = i + 1;
            break;
        }
    ...
    for (i = 0; i < max_packets; i++) {
        switch (packets_ctx[i].ptype) {
            case QUIC_PACKET_TYPE_RECEIVE:
                ...
                rv  = quicly_receive(ctx->conn, NULL, &packets_ctx[i].sa, &packets_ctx[i].packet);
    ...
    for (i = 0; i < max_packets; i++) {
        ...
        switch (packets_ctx[i].ptype) {
            case QUIC_PACKET_TYPE_RECEIVE:
                ctx = quic_ctx_get(packets_ctx[i].ctx_index, packets_ctx[i].thread_index);
        ...
        if (ctx != prev_ctx) quic_send_packets(ctx);
    }
    ...
    if (svm_fifo_max_dequeue(f)) goto rx_start;

    return 0;
}

always_inline void quic_common_get_transport_endpoint(quic_ctx_t *ctx, transport_endpoint_t *tep, u8 is_lcl)
{
    session_t *udp_session;
    if (!quic_ctx_is_stream(ctx)) {
...
static quicly_closed_by_remote_t on_closed_by_remote = {quic_on_closed_by_remote};
...
{
    quic_main_t             *qm            = &quic_main;
    segment_manager_props_t *seg_mgr_props = application_get_segment_manager_properties(qm->app_index);

    if (!seg_mgr_props) {
        clib_warning("error while getting segment_manager_props_t, can't update fifo-size");
        return;
    }
...
static clib_error_t *quic_init(vlib_main_t *vm)
{
    ...
    if (syscall(SYS_getrandom, &seed, sizeof(seed), 0) != sizeof(seed)) return clib_error_return_unix(0, "getrandom() failed");
    ...
    for (i = 0; i < num_threads; i++) {
        ...
        tw_timer_wheel_init_1t_3w_1024sl_ov(tw, quic_expired_timers_dispatch, 1e-3 /* timer period 1ms */, ~0);
        tw->last_run_time = vlib_time_now(vlib_get_main());
        clib_bihash_init_24_8(&qm->wrk_ctx[i].crypto_context_hash, "quic crypto contexts", 64, 128 << 10);
    }

    clib_bihash_init_16_8(&qm->connection_hash, "quic connections", 1024, 4 << 20);

    qm->app_index              = a->app_index;
    qm->tstamp_ticks_per_clock = vm->clib_time.seconds_per_clock / QUIC_TSTAMP_RESOLUTION;
    qm->session_cache.super.cb = quic_encrypt_ticket_cb;

    transport_register_protocol(TRANSPORT_PROTO_QUIC, &quic_proto, FIB_PROTOCOL_IP4, ~0);
    transport_register_protocol(TRANSPORT_PROTO_QUIC, &quic_proto, FIB_PROTOCOL_IP6, ~0);

    quic_load_openssl3_legacy_provider();
    clib_bitmap_alloc(qm->available_crypto_engines, app_crypto_engine_n_types());
    ...
    if (qm->vnet_crypto_enabled == 1) {
        ...
        for (i = 0; i < num_threads; i++) {
            qm->per_thread_crypto_key_indices[i] = vnet_crypto_key_add(vm, VNET_CRYPTO_ALG_AES_256_CTR, empty_key, 32);
        }
    }
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%U", unformat_memory_size, &tmp)) {
            if (tmp >= 0x100000000ULL) {
                return clib_error_return(0, "fifo-size %llu (0x%llx) too large", tmp, tmp);
            }
...
static inline u64 quic_get_counter_value(u32 event_code)
{
    vlib_node_t       *n;
    vlib_main_t       *vm;
    ...
    foreach_vlib_main () {
        ...
        if (i < vec_len(em->counters_last_clear)) c -= em->counters_last_clear[i];
        sum += c;
    }
    return sum;
}
...
static void quic_show_aggregated_stats(vlib_main_t *vm)
{
    ...
    vlib_cli_output(vm, "Opened:          %d", quic_get_counter_value(QUIC_ERROR_OPENED_CONNECTION));
    vlib_cli_output(vm, "Closed:          %d", quic_get_counter_value(QUIC_ERROR_CLOSED_CONNECTION));
    vlib_cli_output(vm, "---------- Streams ----------");
    vlib_cli_output(vm, "Current:         %u", nstream);
    vlib_cli_output(vm, "Opened:          %d", quic_get_counter_value(QUIC_ERROR_OPENED_STREAM));
    vlib_cli_output(vm, "Closed:          %d", quic_get_counter_value(QUIC_ERROR_CLOSED_STREAM));
    vlib_cli_output(vm, "---------- Packets ----------");
    vlib_cli_output(vm, "RX Total:        %d", quic_get_counter_value(QUIC_ERROR_RX_PACKETS));
    vlib_cli_output(vm, "RX 0RTT:         %d", quic_get_counter_value(QUIC_ERROR_ZERO_RTT_RX_PACKETS));
    vlib_cli_output(vm, "RX 1RTT:         %d", quic_get_counter_value(QUIC_ERROR_ONE_RTT_RX_PACKETS));
    vlib_cli_output(vm, "TX Total:        %d", quic_get_counter_value(QUIC_ERROR_TX_PACKETS));
    vlib_cli_output(vm, "----------- Stats -----------");
    vlib_cli_output(vm, "Min      RTT     %f", nconn > 0 ? agg_stats.rtt.minimum / nconn : 0);
    vlib_cli_output(vm, "Smoothed RTT     %f", nconn > 0 ? agg_stats.rtt.smoothed / nconn : 0);
    vlib_cli_output(vm, "Variance on RTT  %f", nconn > 0 ? agg_stats.rtt.variance / nconn : 0);
    vlib_cli_output(vm, "Packets Received %lu", agg_stats.num_packets.received);
    ...
    vlib_cli_output(vm, "Packets Acks     %lu", agg_stats.num_packets.ack_received);
...
static u8 *quic_format_quicly_stream_id(u8 *s, va_list *args)
{
    ...
    s                       = format(s, "%U S%lx", quic_format_quicly_conn_id, quicly_get_master_id(stream->conn), stream->stream_id);
    return s;
}
...
static u8 *quic_format_connection_ctx(u8 *s, va_list *args)
{
    ...
    s = format(s, "[%U]", quic_format_quicly_conn_id, quicly_get_master_id(ctx->conn));
    quicly_get_stats(ctx->conn, &quicly_stats);

    s = format(s,
               "[RTT >%3d, ~%3d, V%3d, last %3d]",
               quicly_stats.rtt.minimum,
               quicly_stats.rtt.smoothed,
               quicly_stats.rtt.variance,
    ...
    s = format(s, "\ncwnd:%u ssthresh:%u recovery_end:%lu", quicly_stats.cc.cwnd, quicly_stats.cc.ssthresh, quicly_stats.cc.recovery_end);
...
static clib_error_t *quic_config_fn(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "fifo-size %U", unformat_memory_size, &tmp)) {
            if (tmp >= 0x100000000ULL) {
                error = clib_error_return(0, "fifo-size %llu (0x%llx) too large", tmp, tmp);
                goto done;
            }
            qm->udp_fifo_size = tmp;
        }
