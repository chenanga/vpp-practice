...
int nsim_cross_connect_enable_disable(nsim_main_t *nsm, u32 sw_if_index0, u32 sw_if_index1, int enable_disable)
{
    ...
    if (nsm->is_configured == 0) return VNET_API_ERROR_CANNOT_ENABLE_DISABLE_FEATURE;

    /* Utterly wrong? */
    if (pool_is_free_index(nsm->vnet_main->interface_main.sw_interfaces, sw_if_index0)) return VNET_API_ERROR_INVALID_SW_IF_INDEX;

    if (pool_is_free_index(nsm->vnet_main->interface_main.sw_interfaces, sw_if_index1)) return VNET_API_ERROR_INVALID_SW_IF_INDEX;
    ...
    nsm->output_next_index0 = vlib_node_add_next(nsm->vlib_main, nsim_input_node.index, hw->output_node_index);

    hw                      = vnet_get_hw_interface(nsm->vnet_main, sw_if_index1);
    nsm->output_next_index1 = vlib_node_add_next(nsm->vlib_main, nsim_input_node.index, hw->output_node_index);
    ...
    vnet_feature_enable_disable("device-input", "nsim", sw_if_index1, enable_disable, 0, 0);
...
int nsim_output_feature_enable_disable(nsim_main_t *nsm, u32 sw_if_index, int enable_disable)
{
    ...
    /* Utterly wrong? */
    if (pool_is_free_index(nsm->vnet_main->interface_main.sw_interfaces, sw_if_index)) return VNET_API_ERROR_INVALID_SW_IF_INDEX;
    ...
    nsm->output_next_index_by_sw_if_index[sw_if_index] = vlib_node_add_next(nsm->vlib_main, nsim_input_node.index, hw->tx_node_index);

    vnet_feature_enable_disable("interface-output", "nsim-output-feature", sw_if_index, enable_disable, 0, 0);
    return rv;
}

static nsim_wheel_t *nsim_wheel_alloc(nsim_main_t *nsm)
{
    u32           pagesize = getpagesize();
    nsim_wheel_t *wp;

    nsm->mmap_size = sizeof(nsim_wheel_t) + nsm->wheel_slots_per_wrk * sizeof(nsim_wheel_entry_t);
...
static int nsim_configure(nsim_main_t *nsm, f64 bandwidth, f64 delay, u32 packet_size, f64 drop_fraction, f64 reorder_fraction)
{
    ...
    if (bandwidth == 0.0) return VNET_API_ERROR_INVALID_VALUE;

    if (delay == 0.0) return VNET_API_ERROR_INVALID_VALUE_2;

    if (packet_size < 64 || packet_size > 9000) return VNET_API_ERROR_INVALID_VALUE_3;

    if (reorder_fraction > 0.0 && delay == 0.0) return VNET_API_ERROR_INVALID_VALUE_4;
    ...
    for (; i < num_workers + 1; i++) {
        ...
        vlib_node_set_state(this_vm, nsim_input_node.index, VLIB_NODE_STATE_POLLING);
    }
...
static clib_error_t *nsim_cross_connect_enable_disable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "disable"))
            enable_disable = 0;
        else if (unformat(line_input, "%U", unformat_vnet_sw_interface, nsm->vnet_main, &tmp)) {
            if (sw_if_index0 == ~0)
                sw_if_index0 = tmp;
            else
                sw_if_index1 = tmp;
    ...
    rv = nsim_cross_connect_enable_disable(nsm, sw_if_index0, sw_if_index1, enable_disable);
...
static void vl_api_nsim_cross_connect_enable_disable_t_handler(vl_api_nsim_cross_connect_enable_disable_t *mp)
{
    ...
    rv = nsim_cross_connect_enable_disable(nsm, sw_if_index0, sw_if_index1, (int) (mp->enable_disable));
...
static void vl_api_nsim_output_feature_enable_disable_t_handler(vl_api_nsim_output_feature_enable_disable_t *mp)
{
    ...
    rv = nsim_output_feature_enable_disable(nsm, ntohl(mp->sw_if_index), (int) (mp->enable_disable));
...
static void vl_api_nsim_configure2_t_handler(vl_api_nsim_configure2_t *mp)
{
    ...
    if (packets_per_reorder > 0) reorder_rate = 1.0 / (f64) packets_per_reorder;
...
static clib_error_t *nsim_output_feature_enable_disable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "disable"))
            enable_disable = 0;
        else if (unformat(line_input, "%U", unformat_vnet_sw_interface, nsm->vnet_main, &sw_if_index))
            ;
        else {
    ...
    switch (rv) {
        ...
        default:
            return clib_error_return(0, "nsim_output_feature_enable_disable returned %d", rv);
    }
    return 0;
}
...
VLIB_CLI_COMMAND(nsim_output_feature_enable_disable_command, static) = {
    ...
    .short_help = "nsim output-feature enable-disable <interface-name> [disable]",
...
static u8 *format_delay(u8 *s, va_list *args)
{
...
static u8 *format_bandwidth(u8 *s, va_list *args)
{
...
static u8 *format_nsim_config(u8 *s, va_list *args)
{
    ...
    if (nsm->sw_if_index0 != 0) {
        ...
        for (i = 0; i < vec_len(nsm->output_next_index_by_sw_if_index); i++) {
            if (nsm->output_next_index_by_sw_if_index[i] != ~0) s = format(s, " %U %u\n", format_vnet_sw_if_index_name, nsm->vnet_main, i, i);
        }
    }
    else {
...
static clib_error_t *set_nsim_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "delay %U", unformat_delay, &delay))
            ;
        else if (unformat(input, "bandwidth %U", unformat_bandwidth, &bandwidth))
            ;
            ...
            if (drop_fraction < 0.0 || drop_fraction > 1.0) return clib_error_return(0, "drop fraction must be between zero and 1");
        }
        else if (unformat(input, "reorder-fraction %f", &reorder_fraction)) {
            if (reorder_fraction < 0.0 || reorder_fraction > 1.0) return clib_error_return(0, "reorder fraction must be between zero and 1");
        }
...
static clib_error_t *show_nsim_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (nsm->is_configured == 0) return clib_error_return(0, "Network simulator not configured");
