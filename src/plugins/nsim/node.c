...
static u8 *format_nsim_trace(u8 *s, va_list *args)
{
    ...
    if (t->is_drop)
        s = format(s, "NSIM: dropped, %s", t->is_lost ? "simulated network loss" : "no space in ring");
    else
        s = format(s, "NSIM: tx time %.6f sw_if_index %d", t->expires, t->tx_sw_if_index);
...
static void nsim_set_actions(nsim_main_t *nsm, vlib_buffer_t **b, nsim_node_ctx_t *ctx, u32 n_actions)
{
    ...
    if (PREDICT_FALSE(nsm->drop_fraction != 0.0)) {
        for (i = 0; i < n_actions; i++)
            if (random_f64(&nsm->seed) <= nsm->drop_fraction) ctx->action[i] |= NSIM_ACTION_DROP;
    }

    if (PREDICT_FALSE(nsm->reorder_fraction != 0.0)) {
        for (i = 0; i < n_actions; i++)
            if (random_f64(&nsm->seed) <= nsm->reorder_fraction) ctx->action[i] |= NSIM_ACTION_REORDER;
    }
}
...
always_inline void nsim_buffer_fwd_lookup(nsim_main_t *nsm, vlib_buffer_t *b, u32 *next, u8 is_cross_connect)
{
    if (is_cross_connect) {
        vnet_buffer(b)->sw_if_index[VLIB_TX] = (vnet_buffer(b)->sw_if_index[VLIB_RX] == nsm->sw_if_index0) ? nsm->sw_if_index1 : nsm->sw_if_index0;
        *next = (vnet_buffer(b)->sw_if_index[VLIB_TX] == nsm->sw_if_index0) ? nsm->output_next_index0 : nsm->output_next_index1;
    }
...
always_inline void nsim_dispatch_buffer(vlib_main_t         *vm,
                                        ...
                                        u8                   is_trace)
{
    if (PREDICT_TRUE(!(ctx->action[0] & NSIM_ACTION_DROP))) {
        ...
        wp->tail++;
        if (wp->tail == wp->wheel_size) wp->tail = 0;
        wp->cursize++;
...
trace:
    ...
    if (PREDICT_FALSE(is_trace)) nsim_trace_buffer(vm, node, b, ctx, 0);

    ctx->action += 1;
}

always_inline uword nsim_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_trace, int is_cross_connect)
{
    ...
    while (n_left_from >= 8) {
        ...
        if (PREDICT_FALSE(wp->cursize + 4 >= wp->wheel_size)) goto slow_path;

        nsim_dispatch_buffer(vm, node, nsm, wp, b[0], from[0], &ctx, is_cross_connect, is_trace);
        nsim_dispatch_buffer(vm, node, nsm, wp, b[1], from[1], &ctx, is_cross_connect, is_trace);
        nsim_dispatch_buffer(vm, node, nsm, wp, b[2], from[2], &ctx, is_cross_connect, is_trace);
        nsim_dispatch_buffer(vm, node, nsm, wp, b[3], from[3], &ctx, is_cross_connect, is_trace);
...
slow_path:
    ...
    while (n_left_from > 0) {
        ...
        if (PREDICT_TRUE(wp->cursize < wp->wheel_size || (ctx.action[0] & NSIM_ACTION_DROP) || (ctx.action[0] & NSIM_ACTION_REORDER))) {
            nsim_dispatch_buffer(vm, node, nsm, wp, b[0], from[0], &ctx, is_cross_connect, is_trace);
        }
        else {
            ctx.drop[0] = from[0];
            ctx.drop += 1;
            if (PREDICT_FALSE(is_trace)) nsim_trace_buffer(vm, node, b[0], &ctx, 1);
            ctx.action += 1;
        }
    ...
    if (PREDICT_FALSE(ctx.drop > drops)) {
        ...
        vlib_node_increment_counter(vm, node->node_index, NSIM_ERROR_DROPPED, n_left_to_drop - ctx.n_loss);
    }
    if (PREDICT_FALSE(ctx.reord > reorders)) {
        u32 n_reordered = ctx.reord - reorders;
        vlib_buffer_enqueue_to_next(vm, node, reorders, reorders_nexts, n_reordered);
        vlib_node_increment_counter(vm, node->node_index, NSIM_ERROR_REORDERED, n_reordered);
    }
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE))
        return nsim_inline(vm, node, frame, 1 /* is_trace */, 1 /* is_cross_connect */);
    else
        return nsim_inline(vm, node, frame, 0 /* is_trace */, 1 /* is_cross_connect */);
}
...
{
    if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE))
        return nsim_inline(vm, node, frame, 1 /* is_trace */, 0 /* is_cross_connect */);
    else
        return nsim_inline(vm, node, frame, 0 /* is_trace */, 0 /* is_cross_connect */);
}
