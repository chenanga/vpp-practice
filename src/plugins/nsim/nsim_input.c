...
always_inline uword nsim_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *f, int is_trace)
{
    ...
    while (n_tx_packets < n_burst && ep->tx_time <= now) {
        ...
        if ((((uword) ep) & (CLIB_CACHE_LINE_BYTES - 1)) == 0) clib_prefetch_load((ep + 2));
        ...
        wp->head++;
        if (wp->head == wp->wheel_size) wp->head = 0;
    ...
    vlib_node_increment_counter(vm, node->node_index, NSIM_TX_ERROR_TRANSMITTED, n_tx_packets);
    return n_tx_packets;
}
