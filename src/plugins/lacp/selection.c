...
static int lacp_set_port_selected(vlib_main_t *vm, bond_if_t *bif, member_if_t *mif)
{
    ...
    if (!memcmp(mif->partner.system, mif->actor.system, 6) && (mif->partner.key == mif->actor.key)) {
        mif->loopback_port = 1;
    ...
    if (vec_len(bif->active_members)) {
        ...
        if (mif2 && ((mif2->partner.key != mif->partner.key) || memcmp(mif2->partner.system, mif->partner.system, 6))) {
            mif->selected = LACP_PORT_UNSELECTED;
            lacp_machine_dispatch(&lacp_mux_machine, vm, mif, LACP_MUX_EVENT_UNSELECTED, &mif->mux_state);
    ...
    switch (mif->mux_state) {
        ...
        case LACP_MUX_STATE_WAITING:
            if (!mif->ready) return LACP_ERROR_NONE;
            break;
        case LACP_MUX_STATE_ATTACHED:
            if (!(mif->partner.state & LACP_STATE_SYNCHRONIZATION)) return LACP_ERROR_NONE;
            break;
    ...
    return lacp_machine_dispatch(&lacp_mux_machine, vm, mif, LACP_MUX_EVENT_SELECTED, &mif->mux_state);
}
...
int lacp_selection_logic(vlib_main_t *vm, member_if_t *mif)
{
    ...
    vec_foreach (sw_if_index, bif->members) {
        ...
        if (mif2 && (mif2->actor.state & LACP_STATE_SYNCHRONIZATION) && (mif2->ready_n == 0)) goto out;
    }
    ...
    vec_foreach (sw_if_index, bif->members) {
        ...
        if (mif2) {
            mif2->ready = 1;
            if (mif2->selected == LACP_PORT_SELECTED) lacp_machine_dispatch(&lacp_mux_machine, vm, mif2, LACP_MUX_EVENT_READY, &mif2->mux_state);
        }
    }
out:
