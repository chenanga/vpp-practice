...
static int lacp_packet_scan(vlib_main_t *vm, member_if_t *mif)
{
     ...
     */
    if ((lacpdu->actor.tlv_length != sizeof(lacp_actor_partner_t)) || (lacpdu->partner.tlv_length != sizeof(lacp_actor_partner_t))
        || (lacpdu->collector.tlv_length != sizeof(lacp_collector_t)) || (lacpdu->terminator.tlv_length != 0))
        return (LACP_ERROR_BAD_TLV);

    return lacp_machine_dispatch(&lacp_rx_machine, vm, mif, LACP_RX_EVENT_PDU_RECEIVED, &mif->rx_state);
}
...
static void send_ethernet_marker_response_pdu(vlib_main_t *vm, member_if_t *mif)
{
     ...
     */
    h0 = vlib_packet_template_get_packet(vm, &lm->marker_packet_templates[mif->packet_template_index], &bi0);
...
static int handle_marker_protocol(vlib_main_t *vm, member_if_t *mif)
{
     ...
     * But we may check the tlv lengths.
     */
    if ((marker->marker_info.tlv_length != sizeof(marker_information_t)) || (marker->terminator.tlv_length != 0)) return (LACP_ERROR_BAD_TLV);
...
lacp_error_t lacp_input(vlib_main_t *vm, vlib_buffer_t *b0, u32 bi0)
{
    ...
    mif = bond_get_member_by_sw_if_index(vnet_buffer(b0)->sw_if_index[VLIB_RX]);
    ...
    if (marker->subtype == MARKER_SUBTYPE) {
        ...
        if (mif->last_marker_pkt) vec_set_len(mif->last_marker_pkt, 0);
        vec_validate(mif->last_marker_pkt, vlib_buffer_length_in_chain(vm, b0) - 1);
     ...
     * and reuse it.
     */
    if (mif->last_rx_pkt) vec_set_len(mif->last_rx_pkt, 0);
    ...
    last_packet_signature = hash_memory(mif->last_rx_pkt, vec_len(mif->last_rx_pkt), 0xd00b);

    if (mif->last_packet_signature_valid && (mif->last_packet_signature == last_packet_signature)
        ...
        switch (lacpdu->subtype) {
            case MARKER_SUBTYPE:
                if (marker->version_number == MARKER_PROTOCOL_VERSION)
                    ...
                    s = format(s, "  Subtype %u, Version %u\n", marker->subtype, marker->version_number);
                ...
                s = format(s, "  Requester system: %U\n", format_ethernet_address, marker->marker_info.requester_system);
            ...
            case LACP_SUBTYPE:
                if (lacpdu->version_number == LACP_ACTOR_LACP_VERSION)
                    ...
                    s = format(s, "  Subtype %u, Version %u\n", lacpdu->subtype, lacpdu->version_number);
                s           = format(s, "  Actor Information TLV: length %u\n", lacpdu->actor.tlv_length);
                s           = format(s, "    System %U\n", format_ethernet_address, lacpdu->actor.port_info.system);
                ...
                while (state_entry->str) {
                    if (lacpdu->actor.port_info.state & (1 << state_entry->bit)) s = format(s, "      %s (%d)\n", state_entry->str, state_entry->bit);
                    state_entry++;
                }

                s           = format(s, "  Partner Information TLV: length %u\n", lacpdu->partner.tlv_length);
                s           = format(s, "    System %U\n", format_ethernet_address, lacpdu->partner.port_info.system);
