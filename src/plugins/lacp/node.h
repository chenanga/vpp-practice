...
#define LACP_DBG(n, args...)                            \
    {                                                   \
        ...
        if (_lm->debug || n->debug) clib_warning(args); \
    }

#define LACP_DBG2(n, e, s, m, t)                                               \
    {                                                                          \
        lacp_main_t *_lm = &lacp_main;                                         \
        if ((m)->debug && (_lm->debug || (n)->debug)) (*m->debug)(n, e, s, t); \
    }
...
static inline void lacp_stop_timer(f64 *timer)
{
    *timer = 0.0;
}

static inline u8 lacp_timer_is_running(f64 timer)
{
    return (timer != 0.0);
}

static inline u8 lacp_timer_is_expired(vlib_main_t *vm, f64 timer)
{
...
static inline u8 *format_rx_sm_state(u8 *s, va_list *args)
{
...
static inline u8 *format_tx_sm_state(u8 *s, va_list *args)
{
...
static inline u8 *format_mux_sm_state(u8 *s, va_list *args)
{
...
static inline u8 *format_ptx_sm_state(u8 *s, va_list *args)
{
...
static inline int lacp_bit_test(u8 val, u8 bit)
{
    if (val & (1 << bit))
        return 1;
    else
