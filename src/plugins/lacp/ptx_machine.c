...
int lacp_ptx_action_slow_periodic(void *p1, void *p2)
{
    ...
    if (!(mif->partner.state & LACP_STATE_LACP_ACTIVITY) && !(mif->actor.state & LACP_STATE_LACP_ACTIVITY))
        lacp_machine_dispatch(&lacp_ptx_machine, vm, mif, LACP_PTX_EVENT_NO_PERIODIC, &mif->ptx_state);
    else {
...
int lacp_ptx_action_fast_periodic(void *p1, void *p2)
{
    ...
    if (!(mif->partner.state & LACP_STATE_LACP_ACTIVITY) && !(mif->actor.state & LACP_STATE_LACP_ACTIVITY))
        ...
        if (timer_expired) lacp_machine_dispatch(&lacp_ptx_machine, vm, mif, LACP_PTX_EVENT_TIMER_EXPIRED, &mif->ptx_state);

        if (!(mif->partner.state & LACP_STATE_LACP_TIMEOUT))
            lacp_machine_dispatch(&lacp_ptx_machine, vm, mif, LACP_PTX_EVENT_LONG_TIMEOUT, &mif->ptx_state);
    }
...
int lacp_ptx_action_timer_expired(void *p1, void *p2)
{
    ...
    if (!(mif->partner.state & LACP_STATE_LACP_ACTIVITY) && !(mif->actor.state & LACP_STATE_LACP_ACTIVITY))
        ...
        mif->ntt = 1;
        lacp_machine_dispatch(&lacp_tx_machine, vm, mif, LACP_TX_EVENT_NTT, &mif->tx_state);
        if (mif->partner.state & LACP_STATE_LACP_TIMEOUT)
            lacp_machine_dispatch(&lacp_ptx_machine, vm, mif, LACP_PTX_EVENT_SHORT_TIMEOUT, &mif->ptx_state);
        else
            lacp_machine_dispatch(&lacp_ptx_machine, vm, mif, LACP_PTX_EVENT_LONG_TIMEOUT, &mif->ptx_state);
    }
