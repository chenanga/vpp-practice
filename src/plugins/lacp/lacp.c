...
static void lacp_send_ethernet_lacp_pdu(vlib_main_t *vm, member_if_t *mif)
{
     ...
     */
    h0 = vlib_packet_template_get_packet(vm, &lm->packet_templates[mif->packet_template_index], &bi0);
    ...
    clib_memcpy(h0->ethernet.src_address, hw->hw_address, vec_len(hw->hw_address));
...
void lacp_periodic(vlib_main_t *vm)
{
    ...
    pool_foreach (mif, bm->neighbors) {
        if (mif->port_enabled == 0) continue;
        ...
        if (lacp_timer_is_running(mif->current_while_timer) && lacp_timer_is_expired(vm, mif->current_while_timer)) {
            lacp_machine_dispatch(&lacp_rx_machine, vm, mif, LACP_RX_EVENT_TIMER_EXPIRED, &mif->rx_state);
        }

        if (lacp_timer_is_running(mif->periodic_timer) && lacp_timer_is_expired(vm, mif->periodic_timer)) {
            lacp_machine_dispatch(&lacp_ptx_machine, vm, mif, LACP_PTX_EVENT_TIMER_EXPIRED, &mif->ptx_state);
        }
        if (lacp_timer_is_running(mif->wait_while_timer) && lacp_timer_is_expired(vm, mif->wait_while_timer)) {
            mif->ready_n = 1;
        ...
        if (actor_state != mif->actor.state) {
            ...
            vlib_stats_set_gauge(bm->stats[bif->sw_if_index][mif->sw_if_index].actor_state, mif->actor.state);
        }
        if (partner_state != mif->partner.state) {
            bif = bond_get_bond_if_by_dev_instance(mif->bif_dev_instance);
            vlib_stats_set_gauge(bm->stats[bif->sw_if_index][mif->sw_if_index].partner_state, mif->partner.state);
        }
    }
}
...
static void lacp_interface_enable_disable(vlib_main_t *vm, bond_if_t *bif, member_if_t *mif, u8 enable)
{
    ...
    if (enable) {
        ...
        bif->port_number_bitmap = clib_bitmap_set(bif->port_number_bitmap, port_number, 1);
        ...
        lm->lacp_int++;
        if (lm->lacp_int == 1) {
            vlib_process_signal_event(vm, lm->lacp_process_node_index, LACP_PROCESS_EVENT_START, 0);
        }
    }
    else {
        ...
        if (lm->lacp_int == 0) {
            ...
            lm->lacp_int--;
            if (lm->lacp_int == 0) vlib_process_signal_event(vm, lm->lacp_process_node_index, LACP_PROCESS_EVENT_STOP, 0);
        }
    }
}
...
static clib_error_t *lacp_periodic_init(vlib_main_t *vm)
{
    ...
    vlib_packet_template_init(vm,
                              &lm->packet_templates[LACP_PACKET_TEMPLATE_ETHERNET],
                              /* data */ &h,
                              sizeof(h),
                              /* alloc chunk size */ 8,
                              "lacp-ethernet");
    ...
    vlib_packet_template_init(vm,
                              &lm->marker_packet_templates[MARKER_PACKET_TEMPLATE_ETHERNET],
                              /* data */ &m,
                              sizeof(m),
                              /* alloc chunk size */ 8,
                              "marker-ethernet");
...
int lacp_machine_dispatch(lacp_machine_t *machine, vlib_main_t *vm, member_if_t *mif, int event, int *state)
{
    ...
    *state = transition->next_state;
    if (transition->action) rc = (*transition->action)((void *) vm, (void *) mif);
...
void lacp_init_neighbor(member_if_t *mif, u8 *hw_address, u16 port_number, u32 group)
{
    ...
    if (mif->ttl_in_seconds == LACP_SHORT_TIMOUT_TIME) mif->actor.state |= LACP_STATE_LACP_TIMEOUT;
...
static clib_error_t *lacp_sw_interface_up_down(vnet_main_t *vnm, u32 sw_if_index, u32 flags)
{
    ...
    if (mif) {
        if (mif->lacp_enabled == 0) return 0;
        ...
        if (mif->port_enabled == 0) {
            lacp_init_neighbor(mif, mif->actor_admin.system, ntohs(mif->actor_admin.port_number), ntohs(mif->actor_admin.key));
            lacp_init_state_machines(vm, mif);
        }
    }
