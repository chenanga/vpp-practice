...
static void lacp_set_port_unselected(vlib_main_t *vm, member_if_t *mif)
{
    ...
    switch (mif->mux_state) {
        ...
        case LACP_MUX_STATE_COLLECTING_DISTRIBUTING:
            if (mif->partner.state & LACP_STATE_SYNCHRONIZATION) return;
            break;
        default:
            break;
    }
    lacp_machine_dispatch(&lacp_mux_machine, vm, mif, LACP_MUX_EVENT_UNSELECTED, &mif->mux_state);
}

static void lacp_update_default_selected(vlib_main_t *vm, member_if_t *mif)
{
    if ((mif->partner_admin.state & LACP_STATE_AGGREGATION) != (mif->partner.state & LACP_STATE_AGGREGATION)
        || memcmp(&mif->partner, &mif->partner_admin, sizeof(mif->partner) - sizeof(mif->partner.state))) {
...
static void lacp_update_selected(vlib_main_t *vm, member_if_t *mif)
{
    ...
    if ((lacpdu->actor.port_info.state & LACP_STATE_AGGREGATION) != (mif->partner.state & LACP_STATE_AGGREGATION)
        || memcmp(&mif->partner, &lacpdu->actor.port_info, sizeof(mif->partner) - sizeof(mif->partner.state))) {
        lacp_set_port_unselected(vm, mif);
    }
}
...
{
    lacp_pdu_t *lacpdu = (lacp_pdu_t *) mif->last_rx_pkt;
    u8          states = LACP_STATE_LACP_ACTIVITY | LACP_STATE_LACP_TIMEOUT | LACP_STATE_SYNCHRONIZATION | LACP_STATE_AGGREGATION;

    if ((states & lacpdu->partner.port_info.state) != (states & mif->actor.state)
        || memcmp(&mif->actor, &lacpdu->partner.port_info, sizeof(mif->actor) - sizeof(mif->actor.state))) {
        mif->ntt = 1;
...
static u8 lacp_compare_partner(member_if_t *mif)
{
    ...
    if ((!memcmp(&mif->partner, &lacpdu->actor.port_info, sizeof(mif->partner) - sizeof(mif->partner.state))
         && ((mif->actor.state & LACP_STATE_AGGREGATION) == (lacpdu->partner.port_info.state & LACP_STATE_AGGREGATION)))
...
static void lacp_record_pdu(vlib_main_t *vm, member_if_t *mif)
{
    ...
    if (!(mif->partner.state & LACP_STATE_LACP_ACTIVITY) && (lacpdu->actor.port_info.state & LACP_STATE_LACP_ACTIVITY))
...
static void lacp_set_port_moved(vlib_main_t *vm, member_if_t *mif, u8 val)
{
    ...
    if (mif->port_moved)
        lacp_machine_dispatch(&lacp_rx_machine, vm, mif, LACP_RX_EVENT_PORT_MOVED, &mif->rx_state);
    else if (!mif->port_enabled)
        lacp_machine_dispatch(&lacp_rx_machine, vm, mif, LACP_RX_EVENT_PORT_DISABLED, &mif->rx_state);
}
...
int lacp_rx_action_initialize(void *p1, void *p2)
{
    ...
    /* UCT */
    lacp_machine_dispatch(&lacp_rx_machine, vm, mif, LACP_RX_EVENT_BEGIN, &mif->rx_state);
...
int lacp_rx_action_port_disabled(void *p1, void *p2)
{
    ...
    if (mif->port_enabled) {
        if (mif->lacp_enabled)
            lacp_machine_dispatch(&lacp_rx_machine, vm, mif, LACP_RX_EVENT_LACP_ENABLED, &mif->rx_state);
        else
            lacp_machine_dispatch(&lacp_rx_machine, vm, mif, LACP_RX_EVENT_LACP_DISABLED, &mif->rx_state);
    }
...
int lacp_rx_action_expired(void *p1, void *p2)
{
    ...
    lacp_ptx_post_short_timeout_event(vm, mif);
    if (lacp_timer_is_running(mif->current_while_timer) && lacp_timer_is_expired(vm, mif->current_while_timer))
        timer_expired = 1;
    else
        timer_expired = 0;
    lacp_start_current_while_timer(vm, mif, mif->ttl_in_seconds);
    mif->actor.state |= LACP_STATE_EXPIRED;
    if (timer_expired) lacp_machine_dispatch(&lacp_rx_machine, vm, mif, LACP_RX_EVENT_TIMER_EXPIRED, &mif->rx_state);
    if (mif->last_rx_pkt && vec_len(mif->last_rx_pkt)) lacp_machine_dispatch(&lacp_rx_machine, vm, mif, LACP_RX_EVENT_PDU_RECEIVED, &mif->rx_state);
...
static int lacp_port_is_moved(vlib_main_t *vm, member_if_t *mif)
{
    ...
    pool_foreach (mif2, bm->neighbors) {
        {
            if ((mif != mif2) && (mif2->rx_state == LACP_RX_STATE_PORT_DISABLED) && !memcmp(mif2->partner.system, lacpdu->partner.port_info.system, 6)
...
int lacp_rx_action_current(void *p1, void *p2)
{
    ...
    lacp_start_current_while_timer(vm, mif, mif->ttl_in_seconds);
    mif->actor.state &= ~LACP_STATE_EXPIRED;
    if (lacp_port_is_moved(vm, mif)) lacp_set_port_moved(vm, mif, 1);
...
void lacp_rx_debug_func(member_if_t *mif, int event, int state, lacp_fsm_state_t *transition)
{
    ...
    ed->event = elog_string(&vlib_global_main.elog_main,
                            "%U-RX: %U, %U->%U%c",
