...
int lacp_mux_action_detached(void *p1, void *p2)
{
    ...
    if (mif->selected == LACP_PORT_SELECTED) lacp_machine_dispatch(&lacp_mux_machine, vm, mif, LACP_MUX_EVENT_SELECTED, &mif->mux_state);

    if (mif->selected == LACP_PORT_STANDBY) lacp_machine_dispatch(&lacp_mux_machine, vm, mif, LACP_MUX_EVENT_STANDBY, &mif->mux_state);
...
int lacp_mux_action_attached(void *p1, void *p2)
{
    ...
    if ((mif->selected == LACP_PORT_UNSELECTED) || (mif->selected == LACP_PORT_STANDBY))
        lacp_machine_dispatch(&lacp_mux_machine, vm, mif, LACP_MUX_EVENT_UNSELECTED, &mif->mux_state);

    if ((mif->selected == LACP_PORT_SELECTED) && (mif->partner.state & LACP_STATE_SYNCHRONIZATION))
        lacp_machine_dispatch(&lacp_mux_machine, vm, mif, LACP_MUX_EVENT_SYNC, &mif->mux_state);
    return 0;
}
...
int lacp_mux_action_waiting(void *p1, void *p2)
{
    ...
    if (!lacp_timer_is_running(mif->wait_while_timer)) lacp_start_wait_while_timer(vm, mif, LACP_AGGREGATE_WAIT_TIME);

    if ((mif->selected == LACP_PORT_SELECTED) && mif->ready) lacp_machine_dispatch(&lacp_mux_machine, vm, mif, LACP_MUX_EVENT_READY, &mif->mux_state);

    if (mif->selected == LACP_PORT_UNSELECTED) lacp_machine_dispatch(&lacp_mux_machine, vm, mif, LACP_MUX_EVENT_UNSELECTED, &mif->mux_state);
...
int lacp_mux_action_collecting_distributing(void *p1, void *p2)
{
    ...
    mif->actor.state |= LACP_STATE_SYNCHRONIZATION | LACP_STATE_COLLECTING | LACP_STATE_DISTRIBUTING;
    bond_enable_collecting_distributing(vm, mif);
    mif->ntt = 1;
    lacp_start_periodic_timer(vm, mif, 0);
    if ((mif->selected == LACP_PORT_UNSELECTED) || (mif->selected == LACP_PORT_STANDBY) || !(mif->partner.state & LACP_STATE_SYNCHRONIZATION))
        lacp_machine_dispatch(&lacp_mux_machine, vm, mif, LACP_MUX_EVENT_UNSELECTED, &mif->mux_state);
...
{
    lacp_machine_dispatch(&lacp_mux_machine, vm, mif, LACP_MUX_EVENT_BEGIN, &mif->mux_state);
}
