...
/* lacp-dump API */
static void vl_api_sw_interface_lacp_details_t_handler(vl_api_sw_interface_lacp_details_t *mp)
{
...
static int api_sw_interface_lacp_dump(vat_main_t *vam)
{
    ...
    if (!lm->ping_id) lm->ping_id = vl_msg_api_get_msg_index((u8 *) (VL_API_CONTROL_PING_CRC));
