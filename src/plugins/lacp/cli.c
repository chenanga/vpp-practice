...
int lacp_dump_ifs(lacp_interface_details_t **out_lacpifs)
{
    ...
    pool_foreach (mif, bm->neighbors) {
        if (mif->lacp_enabled == 0) continue;
        ...
        clib_memcpy(lacpif->interface_name, hi->name, MIN(ARRAY_LEN(lacpif->interface_name) - 1, vec_len(hi->name)));
        bif = bond_get_bond_if_by_dev_instance(mif->bif_dev_instance);
        hi  = vnet_get_hw_interface(vnm, bif->hw_if_index);
        clib_memcpy(lacpif->bond_interface_name, hi->name, MIN(ARRAY_LEN(lacpif->bond_interface_name) - 1, vec_len(hi->name)));
...
static void show_lacp(vlib_main_t *vm, u32 *sw_if_indices)
{
    ...
    if (!sw_if_indices) return;

    vlib_cli_output(vm, "%-55s %-32s %-32s", " ", "actor state", "partner state");
    vlib_cli_output(vm,
                    "%-25s %-12s %-16s %-31s  %-31s",
                    "interface name",
                    "sw_if_index",
    ...
    for (i = 0; i < vec_len(sw_if_indices); i++) {
        ...
        if (!mif || (mif->lacp_enabled == 0)) continue;
        bif = bond_get_bond_if_by_dev_instance(mif->bif_dev_instance);
        vlib_cli_output(vm,
                        ...
                        vnet_get_main(),
                        bif->sw_if_index,
                        lacp_bit_test(mif->actor.state, 7),
        ...
        vlib_cli_output(vm,
                        ...
                        mif->partner.system[0],
                        mif->partner.system[1],
                        mif->partner.system[2],
                        mif->partner.system[3],
                        mif->partner.system[4],
                        mif->partner.system[5],
...
static void show_lacp_details(vlib_main_t *vm, u32 *sw_if_indices)
{
    ...
    for (i = 0; i < vec_len(sw_if_indices); i++) {
        mif = bond_get_member_by_sw_if_index(sw_if_indices[i]);
        if (!mif || (mif->lacp_enabled == 0)) continue;
        vlib_cli_output(vm, "  %U", format_vnet_sw_if_index_name, vnet_get_main(), mif->sw_if_index);
        vlib_cli_output(vm, "    Good LACP PDUs received: %llu", mif->pdu_received);
        vlib_cli_output(vm, "    Bad LACP PDUs received: %llu", mif->bad_pdu_received);
        vlib_cli_output(vm, "    LACP PDUs sent: %llu", mif->pdu_sent);
        if (lacp_timer_is_running(mif->last_lacpdu_recd_time))
            vlib_cli_output(vm, "    last LACP PDU received: %10.2f seconds ago", now - mif->last_lacpdu_recd_time);
        if (lacp_timer_is_running(mif->last_lacpdu_sent_time))
            vlib_cli_output(vm, "    last LACP PDU sent: %10.2f seconds ago", now - mif->last_lacpdu_sent_time);
        vlib_cli_output(vm, "    Good Marker PDUs received: %llu", mif->marker_pdu_received);
        vlib_cli_output(vm, "    Bad Marker PDUs received: %llu", mif->marker_bad_pdu_received);
        if (lacp_timer_is_running(mif->last_marker_pdu_recd_time))
            vlib_cli_output(vm, "    last Marker PDU received: %10.2f seconds ago", now - mif->last_marker_pdu_recd_time);
        if (lacp_timer_is_running(mif->last_marker_pdu_sent_time))
            vlib_cli_output(vm, "    last Marker PDU sent: %10.2f seconds ago", now - mif->last_marker_pdu_sent_time);
        ...
        vlib_cli_output(vm, "      system: %U", format_ethernet_address, mif->actor.system);
        vlib_cli_output(vm, "      system priority: %u", ntohs(mif->actor.system_priority));
        vlib_cli_output(vm, "      key: %u", ntohs(mif->actor.key));
        vlib_cli_output(vm, "      port priority: %u", ntohs(mif->actor.port_priority));
        vlib_cli_output(vm, "      port number: %u", ntohs(mif->actor.port_number));
        vlib_cli_output(vm, "      state: 0x%x", mif->actor.state);

        state_entry = (lacp_state_struct *) &lacp_state_array;
        while (state_entry->str) {
            if (mif->actor.state & (1 << state_entry->bit)) vlib_cli_output(vm, "        %s (%d)", state_entry->str, state_entry->bit);
            state_entry++;
        }

        vlib_cli_output(vm, "    Partner");
        vlib_cli_output(vm, "      system: %U", format_ethernet_address, mif->partner.system);
        vlib_cli_output(vm, "      system priority: %u", ntohs(mif->partner.system_priority));
        vlib_cli_output(vm, "      key: %u", ntohs(mif->partner.key));
        vlib_cli_output(vm, "      port priority: %u", ntohs(mif->partner.port_priority));
        vlib_cli_output(vm, "      port number: %u", ntohs(mif->partner.port_number));
        vlib_cli_output(vm, "      state: 0x%x", mif->partner.state);

        state_entry = (lacp_state_struct *) &lacp_state_array;
        while (state_entry->str) {
            if (mif->partner.state & (1 << state_entry->bit)) vlib_cli_output(vm, "        %s (%d)", state_entry->str, state_entry->bit);
            state_entry++;
        }
        ...
        if (!lacp_timer_is_running(mif->wait_while_timer))
            ...
            vlib_cli_output(vm, "      wait while timer: %10.2f seconds", mif->wait_while_timer - now);
        if (!lacp_timer_is_running(mif->current_while_timer))
            vlib_cli_output(vm, "      current while timer: not running");
        else
            vlib_cli_output(vm, "      current while timer: %10.2f seconds", mif->current_while_timer - now);
        if (!lacp_timer_is_running(mif->periodic_timer))
            vlib_cli_output(vm, "      periodic timer: not running");
        else
            vlib_cli_output(vm, "      periodic timer: %10.2f seconds", mif->periodic_timer - now);
        vlib_cli_output(vm, "    RX-state: %U", format_rx_sm_state, mif->rx_state);
        vlib_cli_output(vm, "    TX-state: %U", format_tx_sm_state, mif->tx_state);
        vlib_cli_output(vm, "    MUX-state: %U", format_mux_sm_state, mif->mux_state);
        vlib_cli_output(vm, "    PTX-state: %U", format_ptx_sm_state, mif->ptx_state);
        vlib_cli_output(vm, "\n");
    }
}
...
static clib_error_t *debug_lacp_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    /* Get a line of input. */
    if (!unformat_user(input, unformat_line_input, line_input)) return clib_error_return(0, "missing argument");
    ...
    if (!input_found) return clib_error_return(0, "must specify on or off");

    if (sw_if_index != ~0) {
        mif = bond_get_member_by_sw_if_index(sw_if_index);
        if (!mif) return (clib_error_return(0, "Please add the member interface first"));
        mif->debug = onoff;
    }
    else
        lm->debug = onoff;
