...
static void lacp_send_sw_interface_details(vl_api_registration_t *reg, lacp_interface_details_t *lacp_if, u32 context)
{
    ...
    clib_memcpy(mp->interface_name, lacp_if->interface_name, MIN(ARRAY_LEN(mp->interface_name) - 1, strlen((const char *) lacp_if->interface_name)));
    clib_memcpy(mp->bond_interface_name,
                lacp_if->bond_interface_name,
                MIN(ARRAY_LEN(mp->bond_interface_name) - 1, strlen((const char *) lacp_if->bond_interface_name)));
...
clib_error_t *lacp_plugin_api_hookup(vlib_main_t *vm)
{
    ...
    vl_api_set_msg_thread_safe(am, lm->msg_id_base + VL_API_SW_INTERFACE_LACP_DUMP, 1);
