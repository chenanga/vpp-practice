...
static uword lacp_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE)) {
            ...
            len             = (b0->current_length < sizeof(t0->pkt)) ? b0->current_length : sizeof(t0->pkt);
            t0->len         = len;
...
static uword lacp_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    ethernet_register_input_type(vm, ETHERNET_TYPE_SLOW_PROTOCOLS /* LACP */, lacp_input_node.index);
...
void lacp_create_periodic_process(void)
{
    ...
    if (lm->lacp_process_node_index > 0) return;

    /* No, create it now and make a note of the node index */
    lm->lacp_process_node_index = vlib_process_create(lm->vlib_main, "lacp-process", lacp_process, 16 /* log2_n_stack_bytes */);
}
