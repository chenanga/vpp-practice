...
int lacp_tx_action_transmit(void *p1, void *p2)
{
    ...
    if (!lacp_timer_is_running(mif->periodic_timer)) return 0;

    // No more than 3 LACPDUs per fast interval
    if (now <= (mif->last_lacpdu_sent_time + 0.333)) return 0;
...
{
    lacp_machine_dispatch(&lacp_tx_machine, vm, mif, LACP_TX_EVENT_BEGIN, &mif->tx_state);
}
