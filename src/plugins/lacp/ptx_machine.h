...
static inline void lacp_start_periodic_timer(vlib_main_t *vm, member_if_t *mif, u8 expiration)
{
    mif->periodic_timer = vlib_time_now(vm) + expiration;
}

static inline void lacp_schedule_periodic_timer(vlib_main_t *vm, member_if_t *mif)
{
    ...
    if ((mif->partner.state & LACP_STATE_LACP_TIMEOUT)
        || (((mif->actor.state & (LACP_STATE_SYNCHRONIZATION | LACP_STATE_COLLECTING | LACP_STATE_DISTRIBUTING))
             != (LACP_STATE_SYNCHRONIZATION | LACP_STATE_COLLECTING | LACP_STATE_DISTRIBUTING))
...
static inline void lacp_ptx_post_short_timeout_event(vlib_main_t *vm, member_if_t *mif)
{
    if (mif->lacp_enabled && mif->port_enabled && ((mif->partner.state & LACP_STATE_LACP_ACTIVITY) || (mif->actor.state & LACP_STATE_LACP_ACTIVITY)))
