...
static u32 picotls_ctx_alloc(void)
{
    ...
    if (!(*ctx)) *ctx = clib_mem_alloc(sizeof(picotls_ctx_t));
...
static void picotls_ctx_free(tls_ctx_t *ctx)
{
    ...
    pool_put_index(picotls_main.ctx_pool[ctx->c_thread_index], ptls_ctx->ptls_ctx_idx);
}
...
{
    picotls_ctx_t **ctx;
    ctx = pool_elt_at_index(picotls_main.ctx_pool[vlib_get_thread_index()], ctx_index);
    return &(*ctx)->ctx;
}
...
static int picotls_transport_reset(tls_ctx_t *ctx)
{
    if (!picotls_handshake_is_over(ctx)) {
        picotls_handle_handshake_failure(ctx);
        return 0;
...
static inline int picotls_do_handshake(picotls_ctx_t *ptls_ctx, session_t *tcp_session)
{
    ...
    if (read) svm_fifo_dequeue_drop(tcp_rx_fifo, read);
...
static inline int ptls_copy_buf_to_fs(ptls_buffer_t *buf, u32 to_copy, svm_fifo_seg_t *fs, u32 *fs_idx, u32 max_fs)
{
    u32 idx = *fs_idx;

    while (to_copy) {
        if (fs[idx].len <= to_copy) {
            clib_memcpy_fast(fs[idx].data, buf->base + (buf->off - to_copy), fs[idx].len);
            to_copy -= fs[idx].len;
            idx += 1;
            ...
            clib_memcpy_fast(fs[idx].data, buf->base + (buf->off - to_copy), to_copy);
            fs[idx].len -= to_copy;
            fs[idx].data += to_copy;
            to_copy = 0;
        }
...
do_checks:
    ...
    if (wrote) svm_fifo_enqueue_nocopy(app_rx_fifo, wrote);

    return wrote;
}

static inline int picotls_ctx_read(tls_ctx_t *ctx, session_t *tcp_session)
{
    picotls_ctx_t *ptls_ctx = (picotls_ctx_t *) ctx;
    ...
    if (PREDICT_FALSE(!ptls_handshake_is_complete(ptls_ctx->tls))) {
        ...
        if (!svm_fifo_max_dequeue(tcp_session->rx_fifo)) return 0;
    }
    ...
    if (wrote) tls_notify_app_enqueue(ctx, app_session);

    if (ptls_ctx->read_buffer_offset || svm_fifo_max_dequeue(tcp_rx_fifo)) tls_add_vpp_q_builtin_rx_evt(tcp_session);

    return wrote;
}

static inline u32 ptls_compute_deq_len(picotls_ctx_t *ptls_ctx, u32 dst_chunk, u32 src_chunk, u32 dst_space, u8 *is_nocopy)
{
    ...
    if (dst_chunk >= clib_min(8192, src_chunk + record_overhead)) {
        *is_nocopy     = 1;
        ...
        if (deq_len + total_overhead > dst_chunk) deq_len = dst_chunk - total_overhead;
    }
    else {
        deq_len        = clib_min(src_chunk, dst_space);
        num_records    = ceil((f64) deq_len / PTLS_MAX_PLAINTEXT_RECORD_SIZE);
        total_overhead = num_records * record_overhead;
        if (deq_len + total_overhead > dst_space) deq_len = dst_space - total_overhead;
    }
...
static u32 ptls_app_to_tcp_write(picotls_ctx_t *ptls_ctx, session_t *app_session, svm_fifo_t *tcp_tx_fifo, u32 max_len)
{
    ...
    n_tcp_segs = svm_fifo_provision_chunks(tcp_tx_fifo, tcp_fs, n_tcp_segs, 1000 + max_len);
    if (n_tcp_segs <= 0) return 0;
    ...
    while ((left = len - read) && ti < n_tcp_segs) {
        ...
        if (app_fs[i].len < min_chunk && min_chunk < left) {
            ...
            clib_memcpy_fast(pm->rx_bufs[thread_index], app_fs[i].data, app_fs[i].len);
            clib_memcpy_fast(pm->rx_bufs[thread_index] + app_fs[i].len, app_fs[i + 1].data, app_buf_len - app_fs[i].len);
        ...
        deq_len = ptls_compute_deq_len(ptls_ctx, tcp_fs[ti].len, app_buf_len, max_enq, &is_nocopy);
        if (is_nocopy) {
    ...
    if (read) {
        ...
        if (svm_fifo_needs_deq_ntf(app_tx_fifo, read)) session_dequeue_notify(app_session);
    }

    if (wrote) {
        svm_fifo_enqueue_nocopy(tcp_tx_fifo, wrote);
        if (svm_fifo_set_event(tcp_tx_fifo)) session_send_io_evt_to_thread(tcp_tx_fifo, SESSION_IO_EVT_TX);
    }

    return wrote;
}

static inline int picotls_ctx_write(tls_ctx_t *ctx, session_t *app_session, transport_send_params_t *sp)
{
    picotls_ctx_t *ptls_ctx = (picotls_ctx_t *) ctx;
    ...
    if (enq_max < 2048) goto check_tls_fifo;
...
check_tls_fifo:
    ...
    if (ctx->flags & TLS_CONN_F_APP_CLOSED) picotls_app_close(ctx);
...
static int picotls_ctx_init_client(tls_ctx_t *ctx)
{
    ...
    if (ptls_handshake(ptls_ctx->tls, &hs_buf, NULL, NULL, &hsprop) != PTLS_ERROR_IN_PROGRESS) {
...
int picotls_reinit_ca_chain(void)
{
    /* Not supported yet */
    return 0;
}
