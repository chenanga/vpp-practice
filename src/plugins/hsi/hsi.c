...
static u8 *format_hsi_trace(u8 *s, va_list *args)
{
    ...
    s  = format(s, "session %sfound, next node: %v", t->next_node < HSI_INPUT_N_NEXT ? "" : "not ", nn->name);
    return s;
}

always_inline u8 hsi_udp_lookup(vlib_buffer_t *b, void *ip_hdr, u8 is_ip4)
{
    udp_header_t *hdr;
    session_t    *s;
...
always_inline transport_connection_t *hsi_tcp_lookup(vlib_buffer_t *b, void *ip_hdr, tcp_header_t **rhdr, u8 is_ip4)
{
...
always_inline void hsi_lookup_and_update(vlib_buffer_t *b, u32 *next, u8 is_ip4, u8 is_input)
{
...
static void hsi_input_trace_frame(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t **bufs, u16 *nexts, u32 n_bufs, u8 is_ip4)
{
    vlib_buffer_t *b;
    hsi_trace_t   *t;
    int            i;
...
always_inline uword hsi46_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_ip4, u8 is_input)
{
    ...
    if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE)) hsi_input_trace_frame(vm, node, bufs, nexts, frame->n_vectors, is_ip4);
