...
static int srv6_ad_localsid_creation_fn(ip6_sr_localsid_t *localsid)
{
    ...
    if (ls_mem->inner_type == AD_TYPE_L2) {
        ...
        int ret = vnet_feature_enable_disable("device-input", "srv6-ad2-rewrite", ls_mem->sw_if_index_in, 1, 0, 0);
        if (ret != 0) {
            clib_mem_free(ls_mem);
        ...
        int ret = vnet_feature_enable_disable("ip4-unicast", "srv6-ad4-rewrite", ls_mem->sw_if_index_in, 1, 0, 0);
        if (ret != 0) {
            adj_unlock(ls_mem->nh_adj);
            clib_mem_free(ls_mem);
            return SID_CREATE_IFACE_FEATURE_ERROR;
        ...
        int ret = vnet_feature_enable_disable("ip6-unicast", "srv6-ad6-rewrite", ls_mem->sw_if_index_in, 1, 0, 0);
        if (ret != 0) {
            adj_unlock(ls_mem->nh_adj);
            clib_mem_free(ls_mem);
            return SID_CREATE_IFACE_FEATURE_ERROR;
...
static int srv6_ad_localsid_removal_fn(ip6_sr_localsid_t *localsid)
{
    ...
    if (ls_mem->inner_type == AD_TYPE_L2) {
        ...
        int ret = vnet_feature_enable_disable("device-input", "srv6-ad2-rewrite", ls_mem->sw_if_index_in, 0, 0, 0);
        if (ret != 0) return -1;
        ...
        int ret = vnet_feature_enable_disable("ip4-unicast", "srv6-ad4-rewrite", ls_mem->sw_if_index_in, 0, 0, 0);
        if (ret != 0) return -1;
        ...
        int ret = vnet_feature_enable_disable("ip6-unicast", "srv6-ad6-rewrite", ls_mem->sw_if_index_in, 0, 0, 0);
        if (ret != 0) return -1;
...
#define PARAM_AD_IIF (1 << 2)
    ...
    if (!unformat(input, "end.ad")) return 0;
