...
static u8 *format_srv6_ad_rewrite_trace(u8 *s, va_list *args)
{
    ...
    return format(s, "SRv6-AD-rewrite: src %U dst %U", format_ip6_address, &t->src, format_ip6_address, &t->dst);
}
...
static_always_inline void end_ad_processing(vlib_buffer_t *b0, ip6_header_t *ip0, ip6_sr_header_t *sr0, ip6_sr_localsid_t *ls0, u32 *next0)
{
    ...
    if (PREDICT_FALSE(ip0->protocol != IP_PROTOCOL_IPV6_ROUTE || sr0->type != ROUTING_HEADER_TYPE_SR)) {
        return;
    }
    ...
    if (PREDICT_FALSE(next_hdr != IP_PROTOCOL_IPV6 && next_hdr != IP_PROTOCOL_IP_IN_IP && next_hdr != IP_PROTOCOL_IP6_ETHERNET)) {
        return;
    }
    ...
    if (next_hdr == IP_PROTOCOL_IP6_ETHERNET) {
        ...
        *next0 = (next_hdr == IP_PROTOCOL_IPV6) ? SRV6_AD_LOCALSID_NEXT_REWRITE6 : SRV6_AD_LOCALSID_NEXT_REWRITE4;
    }
}
...
static uword srv6_ad_localsid_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            vlib_increment_combined_counter(((next0 == SRV6_AD_LOCALSID_NEXT_ERROR) ? &(sm->sr_ls_invalid_counters) : &(sm->sr_ls_valid_counters)),
                                            vm->thread_index,
                                            ls0 - sm->localsids,
                                            1,
...
static uword srv6_ad2_rewrite_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            ls0     = pool_elt_at_index(srm->localsids, sm->sw_iface_localsid2[vnet_buffer(b0)->sw_if_index[VLIB_RX]]);
            ls0_mem = ls0->plugin_mem;
            ...
            if (PREDICT_FALSE(ls0_mem == NULL || ls0_mem->rewrite == NULL)) {
                ...
                ASSERT(VLIB_BUFFER_PRE_DATA_SIZE >= (ls0_mem->rw_len + b0->current_data));

                clib_memcpy_fast(((u8 *) en0) - ls0_mem->rw_len, ls0_mem->rewrite, ls0_mem->rw_len);
            ...
            vlib_increment_combined_counter(((next0 == SRV6_AD_LOCALSID_NEXT_ERROR) ? &(sm->invalid_counters) : &(sm->valid_counters)),
                                            vm->thread_index,
                                            ls0_mem->index,
                                            1,
                                            vlib_buffer_length_in_chain(vm, b0));
    ...
    /* Update counters */
    vlib_node_increment_counter(vm, srv6_ad4_rewrite_node.index, SRV6_AD_REWRITE_COUNTER_PROCESSED, cnt_packets);
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(ls0_mem == NULL || ls0_mem->rewrite == NULL)) {
                ...
                clib_memcpy_fast(((u8 *) ip0_encap) - ls0_mem->rw_len, ls0_mem->rewrite, ls0_mem->rw_len);
                vlib_buffer_advance(b0, -(word) ls0_mem->rw_len);
                ...
                new_l0              = ls0_mem->rw_len - sizeof(ip6_header_t) + clib_net_to_host_u16(ip0_encap->length);
...
static uword srv6_ad6_rewrite_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(ls0_mem == NULL || ls0_mem->rewrite == NULL)) {
                ...
                /* Update outer IPv6 length (in case it has changed) */
                new_l0              = ls0_mem->rw_len + clib_net_to_host_u16(ip0_encap->payload_length);
                ip0->payload_length = clib_host_to_net_u16(new_l0);
            }
    ...
    /* Update counters */
    vlib_node_increment_counter(vm, srv6_ad6_rewrite_node.index, SRV6_AD_REWRITE_COUNTER_PROCESSED, cnt_packets);
