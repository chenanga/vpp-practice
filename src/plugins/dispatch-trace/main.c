...
static u8 *format_buffer_metadata(u8 *s, va_list *args)
{
    ...
    s = format(s, "current_config_index/punt_reason: %d, flow_id: %x, next_buffer: %x\n", b->current_config_index, b->flow_id, b->next_buffer);
    s = format(s, "error: %d, ref_count: %d, buffer_pool_index: %d\n", (u32) (b->error), (u32) (b->ref_count), (u32) (b->buffer_pool_index));
...
uword dispatch_pcap_trace(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    dispatch_trace_thread_t *dtt = vec_elt_at_index(dtm->threads, vm->thread_index);
    ...
    if (frame == 0 || frame->n_vectors == 0) goto done;
    ...
    for (i = 0; i < frame->n_vectors; i++) {
        if (PREDICT_TRUE(pm->n_packets_captured < pm->n_packets_to_capture)) {
            ...
            dtt->pcap_buffer = format(dtt->pcap_buffer, "%U%c", format_buffer_metadata, b, 0);
            string_count++;
            dtt->pcap_buffer = format(dtt->pcap_buffer, "%U%c", format_vnet_buffer_opaque, b, 0);
            string_count++;
            dtt->pcap_buffer = format(dtt->pcap_buffer, "%U%c", format_vnet_buffer_opaque2, b, 0);
            string_count++;

            /* Is this packet traced? */
            if (PREDICT_FALSE(b->flags & VLIB_BUFFER_IS_TRACED)) {
                vlib_trace_header_t **h = pool_elt_at_index(tm->trace_buffer_pool, vlib_buffer_get_trace_index(b));

                dtt->pcap_buffer = format(dtt->pcap_buffer, "%U%c", format_vlib_trace, vm, h[0], 0);
                string_count++;
            }
            ...
            capture_size = vec_len(dtt->pcap_buffer) + +vlib_buffer_length_in_chain(vm, b);
            ...
            n_left = clib_min(vlib_buffer_length_in_chain(vm, b), (16384 - vec_len(dtt->pcap_buffer)));
...
static void pcap_postmortem_reset(vlib_main_t *vm)
{
    ...
    if (vec_len(vlib_worker_threads) == 1 && dtm->epoll_input_node_index) {
        vlib_node_runtime_t *epoll_input_rt       = vlib_node_get_runtime(vm, dtm->epoll_input_node_index);
...
static void pcap_postmortem_dump(void)
{
    ...
    pm->file_name            = (char *) format(0, "/tmp/dispatch_post_mortem.%d%c", getpid(), 0);
    error                    = pcap_write(pm);
    pcap_close(pm);
...
static int vlib_pcap_dispatch_trace_configure(vlib_pcap_dispatch_trace_args_t *a)
{
    ...
    if (a->status) {
        if (dtm->enable) {
            int i;
            vlib_cli_output(vm, "pcap dispatch capture enabled: %d of %d pkts...", pm->n_packets_captured, pm->n_packets_to_capture);
    ...
    if (dtm->enable && a->enable) return VNET_API_ERROR_INVALID_VALUE;

    /* Disable capture with capture already disabled, not interesting */
    if (dtm->enable == 0 && a->enable == 0) return VNET_API_ERROR_VALUE_EXIST;

    /* Change number of packets to capture while capturing */
    if (dtm->enable && a->enable && (pm->n_packets_to_capture != a->packets_to_capture)) return VNET_API_ERROR_INVALID_VALUE_2;
    ...
    if (a->buffer_trace_node_index != ~0) {
        foreach_vlib_main () {
            ...
            tm->trace_enable = 1;
            if (vlib_node_set_dispatch_wrapper(this_vlib_main, dispatch_pcap_trace))
                clib_warning(0, "Dispatch wrapper already in use on thread %u", this_vlib_main->thread_index);
        }
    ...
    if (a->enable) {
        ...
        if (a->filename == 0) a->filename = format(0, "/tmp/dispatch.pcap%c", 0);
    ...
    vlib_node_t *epoll_input_node = vlib_get_node_by_name(vm, (u8 *) "unix-epoll-input");

    /* Save the input node index, see the post-mortem callback */
    if (epoll_input_node) dtm->epoll_input_node_index = epoll_input_node->index;
...
static clib_error_t *dispatch_trace_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "on %=", &enable, 1))
            ...
            ;
        else if (unformat(line_input, "buffer-trace %U %d", unformat_vlib_node, vm, &node_index, &buffer_traces_to_capture))
            ;
