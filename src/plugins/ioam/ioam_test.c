...
clib_error_t *vat_plugin_register(vat_main_t *vam)
{
    ...
    if ((err = pot_vat_plugin_register(vam))) return err;

    if ((err = vxlan_gpe_ioam_export_vat_plugin_register(vam))) return err;

    if ((err = trace_vat_plugin_register(vam))) return err;

    if ((err = vxlan_gpe_vat_plugin_register(vam))) return err;

    if ((err = udp_ping_vat_plugin_register(vam))) return err;

    if ((err = ioam_export_vat_plugin_register(vam))) return err;
