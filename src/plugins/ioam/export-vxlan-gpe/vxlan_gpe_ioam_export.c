...
int vxlan_gpe_ioam_export_enable_disable(ioam_export_main_t *em, u8 is_disable, ip4_address_t *collector_address, ip4_address_t *src_address)
{
    ...
    if (is_disable == 0) {
        if (em->my_hbh_slot == ~0) {
            ...
            vxlan_gpe_decap_ioam_node = vlib_get_node_by_name(vm, (u8 *) "vxlan-gpe-decap-ioam-v4");
            if (!vxlan_gpe_decap_ioam_node) {
                /* node does not exist give up */
                return (-1);
            }
            em->my_hbh_slot = vlib_node_add_next(vm, vxlan_gpe_decap_ioam_node->index, node_index);
        }
...
static void vl_api_vxlan_gpe_ioam_export_enable_disable_t_handler(vl_api_vxlan_gpe_ioam_export_enable_disable_t *mp)
{
    ...
    rv = vxlan_gpe_ioam_export_enable_disable(sm, (int) (mp->is_disable), (ip4_address_t *) mp->collector_address, (ip4_address_t *) mp->src_address);
...
static clib_error_t *set_vxlan_gpe_ioam_export_ipfix_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (collector.as_u32 == 0) return clib_error_return(0, "collector address required");

    if (src.as_u32 == 0) return clib_error_return(0, "src address required");

    em->ipfix_collector.as_u32 = collector.as_u32;
    em->src_address.as_u32     = src.as_u32;

    vlib_cli_output(vm, "Collector %U, src address %U", format_ip4_address, &em->ipfix_collector, format_ip4_address, &em->src_address);

    /* Turn on the export timer process */
    // vlib_process_signal_event (vm, flow_report_process_node.index,
    // 1, 0);
    if (0 != vxlan_gpe_ioam_export_enable_disable(em, is_disable, &collector, &src)) {
