...
inline static ioam_analyser_data_t *ioam_analyse_get_data_from_flow_id(u32 flow_id)
{
    if (flow_id >= vec_len(ioam_analyser_main.aggregated_data)) return NULL;

    if (ioam_analyser_main.aggregated_data[flow_id].is_free) ioam_analyser_main.aggregated_data[flow_id].is_free = 0;

    return (ioam_analyser_main.aggregated_data + flow_id);
}

always_inline void *ip6_ioam_find_hbh_option(ip6_hop_by_hop_header_t *hbh0, u8 option)
{
    ...
    while (opt0 < limit0) {
        type0 = opt0->type;
        if (type0 == option) return ((void *) opt0);
...
always_inline int ip6_ioam_analyse_compare_path_delay(ip6_hop_by_hop_header_t *hbh0, ip6_hop_by_hop_header_t *hbh1, bool oneway)
{
    ...
    if (PREDICT_FALSE((trace0 == NULL) && (trace1 == NULL))) return 0;

    if (PREDICT_FALSE(trace1 == NULL)) return 1;

    if (PREDICT_FALSE(trace0 == NULL)) return -1;

    delay0 = ip6_ioam_analyse_calc_delay(&trace0->trace_hdr, trace0->hdr.length - 2, oneway);
    delay1 = ip6_ioam_analyse_calc_delay(&trace1->trace_hdr, trace1->hdr.length - 2, oneway);
