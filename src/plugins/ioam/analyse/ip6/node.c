...
static u8 *format_analyse_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "IP6-ioam-analyse: flow_id %d, next index %d", t->flow_id, t->next_index);
    return s;
}

always_inline u8 ioam_analyse_hbh(u32 flow_id, ip6_hop_by_hop_header_t *hbh0, ip6_hop_by_hop_option_t *opt0, ip6_hop_by_hop_option_t *limit0, u16 len)
{
        ...
        switch (type0) {
            ...
            default:
                if (am->analyse_hbh_handler[type0]) {
                    if (PREDICT_TRUE((*am->analyse_hbh_handler[type0])(flow_id, opt0, len) < 0)) {
...
static uword ip6_ioam_analyse_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(remote)) {
                vlib_buffer_advance(
                    p0,
                    -(word) (sizeof(udp_header_t) + sizeof(ip4_header_t) + sizeof(ipfix_message_header_t) + sizeof(ipfix_set_header_t)));
            }
            ...
            data += sizeof(ip4_header_t) + sizeof(udp_header_t) + sizeof(ipfix_message_header_t) + sizeof(ipfix_set_header_t);
            ...
            while (num_ioam_records >= 4) {
                ...
                {
                    CLIB_PREFETCH(data + (2 * DEFAULT_EXPORT_SIZE), (DEFAULT_EXPORT_SIZE), LOAD);
                    CLIB_PREFETCH(data + (3 * DEFAULT_EXPORT_SIZE), (DEFAULT_EXPORT_SIZE), LOAD);
                }
                ...
                flow_id0 = clib_net_to_host_u32(ip60->ip_version_traffic_class_and_flow_label) & 0xFFFFF;
                flow_id1 = clib_net_to_host_u32(ip61->ip_version_traffic_class_and_flow_label) & 0xFFFFF;
    ...
    vlib_node_increment_counter(vm, node->node_index, ANALYSE_ERROR_ANALYSED, pkts_analysed);

    if (PREDICT_FALSE(pkts_failed)) vlib_node_increment_counter(vm, node->node_index, ANALYSE_ERROR_FAILED, pkts_failed);
...
int ip6_ioam_analyse_hbh_pot(u32 flow_id, ip6_hop_by_hop_option_t *opt0, u16 len)
{
    ...
    (0 == ret) ? (data->pot_data.sfc_validated_count++) : (data->pot_data.sfc_invalidated_count++);
...
int ip6_ioam_analyse_register_hbh_handler(u8 option, int options(u32 flow_id, ip6_hop_by_hop_option_t *opt, u16 len))
{
    ...
    /* Already registered */
    if (am->analyse_hbh_handler[option]) return (-1);
...
{
    ip6_ioam_analyse_register_hbh_handler(HBH_OPTION_TYPE_IOAM_TRACE_DATA_LIST, ip6_ioam_analyse_hbh_trace_internal);
    ip6_ioam_analyse_register_hbh_handler(HBH_OPTION_TYPE_IOAM_PROOF_OF_TRANSIT, ip6_ioam_analyse_hbh_pot);
    ip6_ioam_analyse_register_hbh_handler(HBH_OPTION_TYPE_IOAM_EDGE_TO_EDGE, ip6_ioam_analyse_hbh_e2e_internal);
}

void ip6_ioam_analyse_unregister_handlers()
{
    ip6_ioam_analyse_unregister_hbh_handler(HBH_OPTION_TYPE_IOAM_TRACE_DATA_LIST);
    ip6_ioam_analyse_unregister_hbh_handler(HBH_OPTION_TYPE_IOAM_PROOF_OF_TRANSIT);
