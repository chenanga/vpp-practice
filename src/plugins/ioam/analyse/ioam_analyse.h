...
always_inline f64 ip6_ioam_analyse_calc_delay(ioam_trace_hdr_t *trace, u16 trace_len, u8 oneway)
{
    ...
    // Unknown trace type
    if (size_of_traceopt_per_node == 0) return 0;
    ...
    if ((num_nodes == 0) || (num_nodes <= trace->data_list_elts_left)) return 0;

    num_nodes -= trace->data_list_elts_left;

    start_elt = trace->elts;
    end_elt   = trace->elts + (u32) ((size_of_traceopt_per_node / sizeof(u32)) * (num_nodes - 1));
    ...
    if (oneway && (trace->ioam_trace_type & BIT_TTL_NODEID)) {
        ...
        do {
            ...
            if ((clib_net_to_host_u32(*start_elt) >> 24) <= (clib_net_to_host_u32(*uturn_elt) >> 24)) done = 1;
...
always_inline void ip6_ioam_analyse_set_paths_down(ioam_analyser_data_t *data)
{
    ioam_analyse_trace_data   *trace_data;
    ...
    for (i = 0; i < IOAM_MAX_PATHS_PER_FLOW; i++) {
        ...
        if (trace_record->is_free) continue;
...
always_inline void ip6_ioam_analyse_hbh_trace_loopback(ioam_analyser_data_t *data, ioam_trace_hdr_t *trace, u16 trace_len)
{
    ioam_analyse_trace_data   *trace_data;
    ioam_analyse_trace_record *trace_record;
    ioam_path_map_t           *path;
    ...
    size_of_traceopt_per_node = fetch_trace_data_size(trace->ioam_trace_type);
    if (0 == size_of_traceopt_per_node) goto end;
    ...
    for (i = 0; i < IOAM_MAX_PATHS_PER_FLOW; i++) {
        ...
        for (j = max_nodes, k = 0; k < num_nodes; j--, k++) {
            ...
            if (nodeid != path[k].node_id) goto end;

            if ((trace->ioam_trace_type == TRACE_TYPE_IF_TS_APP) || (trace->ioam_trace_type == TRACE_TYPE_IF)) {
...
always_inline int ip6_ioam_analyse_hbh_trace(ioam_analyser_data_t *data, ioam_trace_hdr_t *trace, u16 pak_len, u16 trace_len)
{
    ioam_analyse_trace_data   *trace_data;
    u16                        size_of_traceopt_per_node;
    u16                        size_of_all_traceopts;
    ...
    size_of_traceopt_per_node = fetch_trace_data_size(trace->ioam_trace_type);
    // Unknown trace type
    if (size_of_traceopt_per_node == 0) goto DONE;
    size_of_all_traceopts = trace_len;
    ...
    for (i = 0; i < IOAM_MAX_PATHS_PER_FLOW; i++) {
        ...
        if (trace_record->is_free || (num_nodes != trace_record->num_nodes) || (trace->ioam_trace_type != trace_record->trace_type)) continue;
        ...
        for (j = max_nodes, k = 0; k < num_nodes; j--, k++) {
            ...
            if (nodeid != path[k].node_id) break;
...
always_inline int ip6_ioam_analyse_hbh_e2e(ioam_analyser_data_t *data, ioam_e2e_packet_t *e2e, u16 len)
{
    clib_spinlock_lock(&data->writer_lock);

    ioam_analyze_seqno(&data->seqno_data, (u64) clib_net_to_host_u32(e2e->e2e_data));
...
always_inline u8 *format_path_map(u8 *s, va_list *args)
{
...
always_inline u8 *print_analyse_flow(u8 *s, ioam_analyser_data_t *record)
{
    int                        j;
    ioam_analyse_trace_record *trace_record;
    ...
    for (j = 0; j < IOAM_MAX_PATHS_PER_FLOW; j++) {
        ...
        s = format(s, "path_map:\n%U", format_path_map, trace_record->path, trace_record->num_nodes);
        s = format(s, "pkt_counter: %u\n", trace_record->pkt_counter);
