...
u8 *ioam_template_rewrite(ipfix_exporter_t *exp, flow_report_t *fr, u16 collector_port, ipfix_report_element_t *elts, u32 n_elts, u32 *stream_index)
{
    ip4_header_t                *ip;
    udp_header_t                *udp;
...
#define _(field, mask, item, length)                                              \
    {                                                                             \
        field_count++;                                                            \
        fr->fields_to_send = clib_bitmap_set(fr->fields_to_send, field_index, 1); \
    }                                                                             \
    field_index++;
...
#undef _
    ...
    /* Add Src address, dest address, src port, dest port
     * path map,  number of paths manually */
    f->e_id_length = ipfix_e_id_length(0 /* enterprise */, sourceIPv6Address, sizeof(ip6_address_t));
    f++;

    f->e_id_length = ipfix_e_id_length(0 /* enterprise */, destinationIPv6Address, sizeof(ip6_address_t));
    f++;
...
#define _(field, mask, item, length)                                          \
    {                                                                         \
        f->e_id_length = ipfix_e_id_length(0 /* enterprise */, item, length); \
        f++;                                                                  \
    }
    foreach_ioam_ipfix_field;
#undef _

    f->e_id_length = ipfix_e_id_length(0 /* enterprise */, ioamNumberOfPaths, 2);
    f++;

    /* Add ioamPathMap manually */
    f->e_id_length = ipfix_e_id_length(0 /* enterprise */, ioamPathMap, (sizeof(ioam_path) + (sizeof(ioam_path_map_t) * IOAM_TRACE_MAX_NODES)));
    f++;
...
#undef _
    ...
    for (i = 0; i < IOAM_MAX_PATHS_PER_FLOW; i++) {
        ...
        ioam_analyse_trace_record *trace_cached = record->chached_data_list->trace_data.path_data + i;
        ...
        if (!trace->is_free) {
            ...
            if (0 < (trace->pkt_counter - trace_cached->pkt_counter)) {
                ...
                path->mean_delay = (u32) ((new_sum - old_sum) / (trace->pkt_counter - trace_cached->pkt_counter));
                path->mean_delay = clib_host_to_net_u32(path->mean_delay);
            }
            else
                path->mean_delay = 0;

            path->bytes_counter = trace->bytes_counter - trace_cached->bytes_counter;
...
vlib_frame_t *ioam_send_flows(flow_report_main_t *frm, ipfix_exporter_t *exp, flow_report_t *fr, vlib_frame_t *f, u32 *to_next, u32 node_index)
{
    ...
    vec_foreach_index (i, aggregated_data) {
        ...
        /* Flush if last entry */
        if (i == (data_len - 1)) flush = 1;

        if (!record->is_free) {
            if (PREDICT_FALSE(b0 == NULL)) {
                if (vlib_buffer_alloc(vm, &bi0, 1) != 1) break;
            ...
            next_offset = ioam_analyse_add_ipfix_record(fr, record, b0, next_offset, &temp, &temp, 0, 0);

            /* Flush data if packet len is about to reach path mtu */
            if (next_offset > (exp->path_mtu - 250)) flush = 1;
        }

        if (PREDICT_FALSE(flush && b0)) {
            s->set_id_length   = ipfix_set_id_length(IOAM_FLOW_TEMPLATE_ID, next_offset - (sizeof(*ip) + sizeof(*udp) + sizeof(*h)));
