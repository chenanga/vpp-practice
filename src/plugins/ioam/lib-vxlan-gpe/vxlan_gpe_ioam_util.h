...
static u8 *format_vxlan_gpe_ioam_v4_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "VXLAN-GPE-IOAM: next_index %d len %d traced %d", t->next_index, fmt_trace0->length, t->trace_len);
...
always_inline void vxlan_gpe_encap_decap_ioam_v4_one_inline(vlib_main_t         *vm,
                                                            ...
                                                            u8                   use_adj)
{
    ...
    while (opt0 < limit0) {
        ...
        switch (type0) {
            ...
            default:
                if (hm->options[type0]) {
                    if ((*hm->options[type0])(b0, opt0, 1 /* is_ipv4 */, use_adj) < 0) {
                        *next0 = drop_node_val;
                        return;
                    }
                }
    ...
    if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
        vxlan_gpe_ioam_v4_trace_t *t         = vlib_add_trace(vm, node, b0, sizeof(*t));
