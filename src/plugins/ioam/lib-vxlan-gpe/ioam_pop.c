...
u8 *format_vxlan_gpe_pop_ioam_v4_trace(u8 *s, va_list *args)
{
    ...
    vxlan_gpe_pop_ioam_v4_trace_t *t1 = va_arg(*args, vxlan_gpe_pop_ioam_v4_trace_t *);
    ...
    s = format(s, "VXLAN_GPE_IOAM_POP: next_index %d len %d traced %d", t->next_index, fmt_trace0->length, t->trace_len);
    ...
    while (opt0 < limit0) {
        ...
        switch (type0) {
            ...
            default:
                ...
                opt0 = (vxlan_gpe_ioam_option_t *) (((u8 *) opt0) + opt0->length + sizeof(vxlan_gpe_ioam_option_t));
                break;
        }
    }
...
always_inline void vxlan_gpe_ioam_pop_v4(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t *b0)
{
    ip4_header_t         *ip0;
    ...
    /* Pop the iOAM data */
    vlib_buffer_advance(b0, (word) (sizeof(udp_header_t) + sizeof(ip4_header_t) + sizeof(vxlan_gpe_header_t) + gpe_ioam0->length));

    return;
}

always_inline void vxlan_gpe_pop_ioam_v4_one_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vxlan_gpe_main_t *ngm, vlib_buffer_t *b0, u32 *next0)
{
    ...
    while (opt0 < limit0) {
        ...
        opt0 = (vxlan_gpe_ioam_option_t *) (((u8 *) opt0) + opt0->length + sizeof(vxlan_gpe_ioam_hdr_t));
    }

    *next0 = (gpe_ioam0->protocol < VXLAN_GPE_PROTOCOL_MAX) ? ngm->decap_next_node_list[gpe_ioam0->protocol] : VXLAN_GPE_INPUT_NEXT_DROP;

trace00:
    if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
        vxlan_gpe_pop_ioam_v4_trace_t *t         = vlib_add_trace(vm, node, b0, sizeof(*t));
        u32                            trace_len = gpe_ioam0->length;
        t->fmt_trace.next_index                  = *next0;
        /* Capture the h-b-h option verbatim */
        trace_len              = trace_len < ARRAY_LEN(t->fmt_trace.option_data) ? trace_len : ARRAY_LEN(t->fmt_trace.option_data);
...
always_inline void vxlan_gpe_pop_ioam_v4_two_inline(vlib_main_t         *vm,
...
static uword vxlan_gpe_pop_ioam(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, u8 is_ipv6)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            vxlan_gpe_pop_ioam_v4_two_inline(vm, node, ngm, b0, b1, &next0, &next1);
