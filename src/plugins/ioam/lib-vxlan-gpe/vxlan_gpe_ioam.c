...
int vxlan_gpe_ioam_set_rewrite(vxlan_gpe_tunnel_t *t, int has_trace_option, int has_pot_option, int has_ppc_option, u8 ipv6_set)
{
    ...
    if (has_trace_option && hm->add_options[VXLAN_GPE_OPTION_TYPE_IOAM_TRACE] != 0) {
        size += sizeof(vxlan_gpe_ioam_option_t);
        size += hm->options_size[VXLAN_GPE_OPTION_TYPE_IOAM_TRACE];
    }
    if (has_pot_option && hm->add_options[VXLAN_GPE_OPTION_TYPE_IOAM_PROOF_OF_TRANSIT] != 0) {
        size += sizeof(vxlan_gpe_ioam_option_t);
    ...
    if (!ipv6_set) {
        vxlan4_gpe_rewrite(t, size, VXLAN_GPE_PROTOCOL_IOAM, hm->encap_v4_next_node);
        vxlan_gpe_ioam_hdr = (vxlan_gpe_ioam_hdr_t *) (t->rewrite + sizeof(ip4_vxlan_gpe_header_t));
    }
    else {
        vxlan6_gpe_rewrite(t, size, VXLAN_GPE_PROTOCOL_IOAM, VXLAN_GPE_ENCAP_NEXT_IP6_LOOKUP);
        vxlan_gpe_ioam_hdr = (vxlan_gpe_ioam_hdr_t *) (t->rewrite + sizeof(ip6_vxlan_gpe_header_t));
    }
    ...
    if (has_trace_option && hm->add_options[VXLAN_GPE_OPTION_TYPE_IOAM_TRACE] != 0) {
        if (0 != hm->add_options[VXLAN_GPE_OPTION_TYPE_IOAM_TRACE](current, &trace_data_size)) return -1;
        current += trace_data_size;
    }
    if (has_pot_option && hm->add_options[VXLAN_GPE_OPTION_TYPE_IOAM_PROOF_OF_TRANSIT] != 0) {
        pot_data_size = hm->options_size[VXLAN_GPE_OPTION_TYPE_IOAM_PROOF_OF_TRANSIT];
        if (0 == hm->add_options[VXLAN_GPE_OPTION_TYPE_IOAM_PROOF_OF_TRANSIT](current, &pot_data_size)) current += pot_data_size;
    }
...
clib_error_t *vxlan_gpe_ioam_clear(vxlan_gpe_tunnel_t *t, int has_trace_option, int has_pot_option, int has_ppc_option, u8 ipv6_set)
{
    ...
    if (rv == 0) {
        ...
        return clib_error_return_code(0, rv, 0, "vxlan_gpe_ioam_clear_rewrite returned %d", rv);
    }
}
...
{
    int rv;
    rv = vxlan_gpe_ioam_set_rewrite(t, has_trace_option, has_pot_option, has_ppc_option, ipv6_set);

    if (rv == 0) {
        return (0);
    }
    else {
        return clib_error_return_code(0, rv, 0, "vxlan_gpe_ioam_set_rewrite returned %d", rv);
    }
}
...
int vxlan_gpe_enable_disable_ioam_for_dest(vlib_main_t *vm, ip46_address_t dst_addr, u32 outer_fib_index, u8 is_ipv4, u8 is_add)
{
            ...
            for (j = 0; j < lb_b->lb_n_buckets; j++) {
                ...
                if (dpo1->dpoi_type == DPO_ADJACENCY) {
                    ...
                    if (is_add) {
                        ...
                        vec_validate_init_empty(hm->bool_ref_by_sw_if_index, sw_if_index0, ~0);
    ...
    if (is_ipv4) {
        ...
        if (is_add) {
            ...
            t1->fib_entry_index = fib_table_entry_special_add(outer_fib_index, &tun_dst_pfx, FIB_SOURCE_RR, FIB_ENTRY_FLAG_NONE);
            t1->sibling_index   = fib_entry_child_add(t1->fib_entry_index, hm->fib_entry_type, t1 - hm->dst_tunnels);
...
void vxlan_gpe_refresh_output_feature_on_all_dest(void)
{
    ...
    u32                            i;
    if (pool_elts(hm->dst_tunnels) == 0) return;
    ...
    pool_foreach (t, hm->dst_tunnels) {
        vxlan_gpe_enable_disable_ioam_for_dest(hm->vlib_main, t->dst_addr, t->outer_fib_index, (t->fp_proto == FIB_PROTOCOL_IP4), 1 /* is_add */);
    }
    return;
}
...
void vxlan_gpe_clear_output_feature_on_select_intfs(void)
{
    vxlan_gpe_ioam_main_t *hm           = &vxlan_gpe_ioam_main;
    u32                    sw_if_index0 = 0;
    for (sw_if_index0 = 0; sw_if_index0 < vec_len(hm->bool_ref_by_sw_if_index); sw_if_index0++) {
        if (hm->bool_ref_by_sw_if_index[sw_if_index0] == 0xFF) {
            vxlan_gpe_set_clear_output_feature_on_intf(hm->vlib_main, sw_if_index0, 0);
        }
    }
...
static clib_error_t *vxlan_gpe_set_ioam_rewrite_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (local_set == 0) return clib_error_return(0, "tunnel local address not specified");
    if (remote_set == 0) return clib_error_return(0, "tunnel remote address not specified");
    ...
    if (vni_set == 0) return clib_error_return(0, "vni not specified");
    if (!ipv6_set) {
    ...
    if (!p) return clib_error_return(0, "VxLAN Tunnel not found");
    t = pool_elt_at_index(gm->tunnels, p[0]);
    if (!disable) {
        rv = vxlan_gpe_ioam_set(t, hm->has_trace_option, hm->has_pot_option, hm->has_ppc_option, ipv6_set);
    }
    else {
...
static clib_error_t *vxlan_gpe_set_ioam_flags_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    rv = vxlan_gpe_ioam_enable(has_trace_option, has_pot_option, has_ppc_option);
    return rv;
}
...
int vxlan_gpe_ioam_disable_for_dest(vlib_main_t *vm, ip46_address_t dst_addr, u32 outer_fib_index, u8 ipv4_set)
{
    ...
    vxlan_gpe_enable_disable_ioam_for_dest(hm->vlib_main, dst_addr, outer_fib_index, ipv4_set, 0);
...
static clib_error_t *vxlan_gpe_set_ioam_transit_rewrite_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(input, "dst-ip %U", unformat_ip6_address, &dst_addr.ip6)) {
            dst_addr_set = 1;
            ipv6_set     = 1;
        }
    ...
    if (dst_addr_set == 0) return clib_error_return(0, "tunnel destination address not specified");
    if (ipv4_set && ipv6_set) return clib_error_return(0, "both IPv4 and IPv6 addresses specified");
    if (!disable) {
        vxlan_gpe_enable_disable_ioam_for_dest(hm->vlib_main, dst_addr, outer_fib_index, ipv4_set, 1);
    }
    else {
        vxlan_gpe_ioam_disable_for_dest(vm, dst_addr, outer_fib_index, ipv4_set);
    }
    return rv;
}
...
{
    vxlan_gpe_ioam_main_t *hm = &vxlan_gpe_ioam_main;
    hm->fib_entry_type        = fib_node_register_new_type("vxlan-gpe", &vxlan_gpe_ioam_vft);
    return;
}
