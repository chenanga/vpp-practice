...
always_inline void vxlan_gpe_decap_ioam_v4_two_inline(vlib_main_t         *vm,
                                                      ...
                                                      u32                 *next1)
{
    ...
    *next0 = *next1 = hm->decap_v4_next_override;
    vxlan_gpe_encap_decap_ioam_v4_one_inline(vm, node, b0, next0, VXLAN_GPE_DECAP_IOAM_V4_NEXT_DROP, 0 /* use_adj */);
    vxlan_gpe_encap_decap_ioam_v4_one_inline(vm, node, b1, next1, VXLAN_GPE_DECAP_IOAM_V4_NEXT_DROP, 0 /* use_adj */);
}
...
static uword vxlan_gpe_decap_ioam(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, u8 is_ipv6)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            vlib_buffer_advance(b0, -(word) (sizeof(udp_header_t) + sizeof(ip4_header_t) + sizeof(vxlan_gpe_header_t)));
            vlib_buffer_advance(b1, -(word) (sizeof(udp_header_t) + sizeof(ip4_header_t) + sizeof(vxlan_gpe_header_t)));

            vxlan_gpe_decap_ioam_v4_two_inline(vm, node, ngm, b0, b1, &next0, &next1);

            vlib_validate_buffer_enqueue_x2(vm, node, next_index, to_next, n_left_to_next, bi0, bi1, next0, next1);

            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                vxlan_gpe_ioam_v4_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
            }
        }
