...
static int api_vxlan_gpe_ioam_enable(vat_main_t *vam)
{
...
static int api_vxlan_gpe_ioam_disable(vat_main_t *vam)
{
...
static int api_vxlan_gpe_ioam_vni_enable(vat_main_t *vam)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(line_input, "remote %U", unformat_ip46_address, &remote)) {
            remote_set = 1;
        }
    ...
    ip_address_encode(&local, ip46_address_is_ip4(&local) ? IP46_TYPE_IP4 : IP46_TYPE_IP6, &mp->local);
    ip_address_encode(&local, ip46_address_is_ip4(&remote) ? IP46_TYPE_IP4 : IP46_TYPE_IP6, &mp->remote);
...
static int api_vxlan_gpe_ioam_vni_disable(vat_main_t *vam)
{
    unformat_input_t                    *line_input = vam->input;
...
static int api_vxlan_gpe_ioam_transit_enable(vat_main_t *vam)
{
    unformat_input_t                       *line_input = vam->input;
...
static int api_vxlan_gpe_ioam_transit_disable(vat_main_t *vam)
{
    unformat_input_t                        *line_input = vam->input;
