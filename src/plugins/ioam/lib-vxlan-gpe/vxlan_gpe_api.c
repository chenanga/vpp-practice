...
static void vl_api_vxlan_gpe_ioam_enable_t_handler(vl_api_vxlan_gpe_ioam_enable_t *mp)
{
     ...
     * is supported */
    error = vxlan_gpe_ioam_enable(mp->trace_enable, mp->pow_enable, mp->trace_ppc);
    if (error) {
...
static void vl_api_vxlan_gpe_ioam_vni_enable_t_handler(vl_api_vxlan_gpe_ioam_vni_enable_t *mp)
{
    ...
    if (clib_net_to_host_u32(mp->local.af) == ADDRESS_IP4 && clib_net_to_host_u32(mp->remote.af) == ADDRESS_IP4) {
    ...
    error = vxlan_gpe_ioam_set(t, hm->has_trace_option, hm->has_pot_option, hm->has_ppc_option, 0 /* is_ipv6 */);
...
static void vl_api_vxlan_gpe_ioam_transit_enable_t_handler(vl_api_vxlan_gpe_ioam_transit_enable_t *mp)
{
    ...
    rv          = vxlan_gpe_enable_disable_ioam_for_dest(sm->vlib_main, dst_addr, ntohl(mp->outer_fib_index), is_ip6, 1 /* is_add */);
...
static void vl_api_vxlan_gpe_ioam_transit_disable_t_handler(vl_api_vxlan_gpe_ioam_transit_disable_t *mp)
{
    ...
    ip_address_decode(&mp->dst_addr, &dst_addr);
    bool is_ip6 = clib_net_to_host_u32(mp->dst_addr.af) == ADDRESS_IP6;
    rv          = vxlan_gpe_ioam_disable_for_dest(sm->vlib_main, dst_addr, ntohl(mp->outer_fib_index), is_ip6);
...
static clib_error_t *ioam_vxlan_gpe_init(vlib_main_t *vm)
{
    ...
    sm->encap_v4_next_node = vlib_node_add_next(vm, vxlan_gpe_encap_node->index, encap_node_index);

    vxlan_gpe_decap_node = vlib_get_node_by_name(vm, (u8 *) "vxlan4-gpe-input");
    next_node            = vlib_node_add_next(vm, vxlan_gpe_decap_node->index, decap_node_index);
