...
int vxlan_gpe_ioam_register_option(u8  option,
                                   int options(vlib_buffer_t *b, vxlan_gpe_ioam_option_t *opt, u8 is_ipv4, u8 use_adj),
                                   u8 *trace(u8 *s, vxlan_gpe_ioam_option_t *opt))
{
...
always_inline void vxlan_gpe_ioam_trace_stats_increment_counter(u32 counter_index, u64 increment)
{
...
static u8 *format_ioam_data_list_element(u8 *s, va_list *args)
{
    ...
    if (trace_type & BIT_TTL_NODEID) {
        ...
        s = format(s, "ttl 0x%x node id 0x%x ", ttl_node_id_host_byte_order >> 24, ttl_node_id_host_byte_order & 0x00FFFFFF);
    ...
    if (trace_type & BIT_ING_INTERFACE && trace_type & BIT_ING_INTERFACE) {
        ...
        s                           = format(s, "ingress 0x%x egress 0x%x ", ingress_host_byte_order >> 16, ingress_host_byte_order & 0xFFFF);
        elt++;
    }
...
int vxlan_gpe_ioam_trace_rewrite_handler(u8 *rewrite_string, u8 *rewrite_size)
{
    ...
    if (PREDICT_FALSE(!rewrite_string)) return -1;
    ...
    trace_option->hdr.length          = 2 /*ioam_trace_type,data_list_elts_left */ + trace_option_elts * trace_data_size;
    trace_option->ioam_trace_type     = profile->trace_type & TRACE_TYPE_MASK;
    trace_option->data_list_elts_left = trace_option_elts;
    *rewrite_size                     = sizeof(vxlan_gpe_ioam_trace_option_t) + (trace_option_elts * trace_data_size);
...
{
    u8                             elt_index = 0;
    vxlan_gpe_ioam_trace_option_t *trace     = (vxlan_gpe_ioam_trace_option_t *) opt;
    time_u64_t                     time_u64;
    u32                           *elt;
    int                            rv      = 0;
    trace_profile                 *profile = NULL;
    ...
    if (PREDICT_TRUE(trace->data_list_elts_left)) {
        ...
        if (is_ipv4) {
            if (trace->ioam_trace_type & BIT_TTL_NODEID) {
                 ...
                 */
                *elt = clib_host_to_net_u32(((ip0->ttl - 1 + use_adj) << 24) | profile->node_id);
                elt++;
            }
            ...
            if (trace->ioam_trace_type & BIT_ING_INTERFACE) {
                ...
                *elt = (vnet_buffer(b)->sw_if_index[VLIB_RX] & 0xFFFF) << 16 | tx_if;
            ...
            if (trace->ioam_trace_type & BIT_TTL_NODEID) {
                ...
                *elt              = clib_host_to_net_u32((ip0->hop_limit << 24) | profile->node_id);
                elt++;
            }
            if (trace->ioam_trace_type & BIT_ING_INTERFACE) {
                u16 tx_if     = 0;
        ...
        vxlan_gpe_ioam_trace_stats_increment_counter(VXLAN_GPE_IOAM_TRACE_SUCCESS, 1);
    }
    else {
        vxlan_gpe_ioam_trace_stats_increment_counter(VXLAN_GPE_IOAM_TRACE_FAILED, 1);
    }
    return (rv);
}
...
u8 *vxlan_gpe_ioam_trace_data_list_trace_handler(u8 *s, vxlan_gpe_ioam_option_t *opt)
{
    ...
    while ((u8 *) elt < ((u8 *) (&trace->elts[0]) + trace->hdr.length - 2
        ...
        s = format(s, "    [%d] %U\n", elt_index, format_ioam_data_list_element, elt, &trace->ioam_trace_type);
        elt_index++;
...
static clib_error_t *vxlan_gpe_show_ioam_trace_cmd_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (i = 0; i < VXLAN_GPE_IOAM_TRACE_N_STATS; i++) {
        s = format(s, " %s - %lu\n", vxlan_gpe_ioam_trace_stats_strings[i], hm->counters[i]);
    }
...
static clib_error_t *vxlan_gpe_ioam_trace_init(vlib_main_t *vm)
{
    ...
    if (vxlan_gpe_ioam_register_option(VXLAN_GPE_OPTION_TYPE_IOAM_TRACE,
                                       vxlan_gpe_ioam_trace_data_list_handler,
                                       vxlan_gpe_ioam_trace_data_list_trace_handler)
        < 0)
        return (clib_error_create("registration of VXLAN_GPE_OPTION_TYPE_IOAM_TRACE failed"));

    if (vxlan_gpe_ioam_add_register_option(VXLAN_GPE_OPTION_TYPE_IOAM_TRACE,
                                           sizeof(vxlan_gpe_ioam_trace_option_t),
                                           vxlan_gpe_ioam_trace_rewrite_handler)
        < 0)
        return (clib_error_create("registration of VXLAN_GPE_OPTION_TYPE_IOAM_TRACE for rewrite failed"));

    return (0);
}

VLIB_INIT_FUNCTION(vxlan_gpe_ioam_trace_init) = {
    .runs_after = VLIB_INITS("ip_main_init", "ip6_lookup_init", "vxlan_gpe_init"),
};
...
static int vxlan_gpe_ioam_trace_get_sizeof_handler(u32 *result)
{
    ...
    *result = 0;
    ...
    trace_data_size = fetch_trace_data_size(profile->trace_type);
    if (PREDICT_FALSE(trace_data_size == 0)) return VNET_API_ERROR_INVALID_VALUE;

    if (PREDICT_FALSE(profile->num_elts * trace_data_size > 254)) return VNET_API_ERROR_INVALID_VALUE;

    size += sizeof(vxlan_gpe_ioam_trace_option_t) + profile->num_elts * trace_data_size;
    *result = size;
...
int vxlan_gpe_trace_profile_setup(void)
{
    ...
    if (vxlan_gpe_ioam_trace_get_sizeof_handler(&trace_size) < 0) return (-1);
