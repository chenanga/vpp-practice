...
static void vl_api_pot_profile_add_t_handler(vl_api_pot_profile_add_t *mp)
{
    ...
    if (profile) {
        ...
        if (1 == mp->validator) (void) pot_set_validator(profile, clib_net_to_host_u64(mp->secret_key));
...
static void send_pot_profile_details(vl_api_pot_profile_show_config_dump_t *mp, u8 id)
{
    ...
    int                                       rv      = 0;
    if (profile) {
        REPLY_MACRO2(VL_API_POT_PROFILE_SHOW_CONFIG_DETAILS, rmp->id = id; rmp->validator = profile->validator;
        ...
        REPLY_MACRO2(VL_API_POT_PROFILE_SHOW_CONFIG_DETAILS, rmp->id = id; rmp->validator = 0; rmp->secret_key = 0; rmp->secret_share = 0;
                     rmp->prime             = 0;
                     rmp->bit_mask          = 0;
                     rmp->lpc               = 0;
