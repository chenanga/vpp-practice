...
static void pot_main_profiles_reset(void)
{
    ...
    if (sm->profile_list_name) vec_free(sm->profile_list_name);
...
always_inline u64 pot_update_cumulative_inline(u64 cumulative, u64 random, u64 secret_share, u64 prime, u64 lpc, u64 pre_split, double prime_inv)
{
    u64 share_random   = 0;
...
u64 pot_update_cumulative(pot_profile *profile, u64 cumulative, u64 random)
{
    if (profile && profile->valid != 0) {
        return (pot_update_cumulative_inline(cumulative,
                                             random,
                                             profile->secret_share,
                                             profile->prime,
                                             profile->lpc,
...
always_inline u8 pot_validate_inline(u64 secret, u64 prime, double prime_inv, u64 cumulative, u64 random)
{
...
u8 pot_validate(pot_profile *profile, u64 cumulative, u64 random)
{
    if (profile && profile->validator) {
        return (pot_validate_inline(profile->secret_key, profile->prime, profile->primeinv, cumulative, random));
    }
    return (0);
}
...
u64 pot_generate_random(pot_profile *profile)
{
    ...
    if (PREDICT_FALSE(!seed)) seed = random_default_seed();
...
int pot_profile_set_bit_mask(pot_profile *profile, u16 bits)
{
    ...
    if (profile && !profile->in_use) {
        ...
        profile->bit_mask = (bits >= sizeInBits ? (u64) -1 : (u64) ((u64) 1 << (u64) bits) - 1);
        return (0);
    }
    return (-1);
}
...
static clib_error_t *set_pot_profile_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "name %s", &profile_list_name))
            ;
    ...
    if (profile) {
        ...
        if (validator) pot_set_validator(profile, secret_key);
...
static clib_error_t *set_pot_profile_activate_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!pot_profile_list_is_enabled(profile_list_name)) {
        result = clib_error_return(0, "%s list is not enabled, profile in use %s", profile_list_name, sm->profile_list_name);
    }
    else if (0 != pot_profile_set_active((u8) id)) {
        result = clib_error_return(0, "Profile %d not defined in %s", id, sm->profile_list_name);
    }
    vec_free(profile_list_name);
    return result;
}
...
static clib_error_t *show_pot_profile_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (i = 0; i < MAX_POT_PROFILES; i++) {
        ...
        if (p->validator == 1) s = format(s, "         Secret key : 0x%Lx (%Ld)\n", p->secret_key, p->secret_key);
    ...
    if (p && p->valid && p->in_use) {
        ...
        s = format(s, "Pkts passed : 0x%Lx (%Ld)\n", p->total_pkts_using_this_profile, p->total_pkts_using_this_profile);
        if (pot_is_decap(p)) s = format(s, "  This is Decap node.  \n");
    }
    else {
