...
static u8 *format_ipfix_collector_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "IPFIX_COLLECTOR: set_id %u, next_node %u", t->set_id, t->next_node);
    return s;
}
...
uword ipfix_collector_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            /* Prefetch next iteration. */
            {
                ...
                CLIB_PREFETCH(p2->data, (sizeof(ipfix_message_header_t) + sizeof(ipfix_set_header_t)), LOAD);
                CLIB_PREFETCH(p3->data, (sizeof(ipfix_message_header_t) + sizeof(ipfix_set_header_t)), LOAD);
            }
            ...
            vlib_buffer_advance(b0, (sizeof(ipfix_message_header_t) + sizeof(ipfix_set_header_t)));
            vlib_buffer_advance(b1, (sizeof(ipfix_message_header_t) + sizeof(ipfix_set_header_t)));

            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                ipfix_collector_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
                tr->next_node               = (client0 ? client0->client_node : 0xFFFFFFFF);
                tr->set_id                  = set_id0;
            }
            if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
                ipfix_collector_trace_t *tr = vlib_add_trace(vm, node, b1, sizeof(*tr));
    ...
    vlib_error_count(vm, node->node_index, IPFIX_COLLECTOR_ERROR_NO_LISTENER, n_no_listener);
    vlib_error_count(vm, node->node_index, IPFIX_COLLECTOR_ERROR_PROCESSED, n_listener);
