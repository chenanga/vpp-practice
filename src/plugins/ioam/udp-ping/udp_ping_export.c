...
static u8 *udp_ping_template_rewrite(ipfix_exporter_t       *exp,
                                     flow_report_t          *fr,
                                     u16                     collector_port,
                                     ...
                                     u32                    *stream_index)
{
    return ioam_template_rewrite(exp, fr, collector_port, elts, n_elts, stream_index);
}

static vlib_frame_t *udp_ping_send_flows(flow_report_main_t *frm,
                                         ...
                                         u32                 node_index)
{
    ...
    for (i = 0; i < data_len; i++) {
        ...
        for (src_port = ip46_flow->udp_data.start_src_port; src_port <= ip46_flow->udp_data.end_src_port; src_port++) {
            for (dst_port = ip46_flow->udp_data.start_dst_port; dst_port <= ip46_flow->udp_data.end_dst_port; dst_port++, j++) {
