...
extern int ip6_hbh_ioam_trace_data_list_handler(vlib_buffer_t *b, ip6_header_t *ip, ip6_hop_by_hop_option_t *opt);
...
void udp_ping_calculate_timer_interval(void)
{
    ...
    for (i = 0; i < vec_len(udp_ping_main.ip46_flow); i++) {
        if (pool_is_free_index(udp_ping_main.ip46_flow, i)) continue;

        flow = pool_elt_at_index(udp_ping_main.ip46_flow, i);

        if (min_interval > flow->udp_data.interval) min_interval = flow->udp_data.interval;
    }

    if (udp_ping_main.timer_interval != min_interval) {
        udp_ping_main.timer_interval = min_interval;
        vlib_process_signal_event(udp_ping_main.vlib_main, udp_ping_node.index, EVENT_SIG_RECHECK, 0);
    }
}
                            ...
                            u8             is_disable)
{
    ...
    for (i = 0; i < vec_len(udp_ping_main.ip46_flow); i++) {
        ...
        flow = pool_elt_at_index(udp_ping_main.ip46_flow, i);
        if ((0 == udp_ping_compare_flow(src, dst, start_src_port, end_src_port, start_dst_port, end_dst_port, flow))) {
            found = 1;
            break;
        }
    }
    ...
    if (found) {
        ...
        if (is_disable) {
            ...
            if (udp_ping_main.timer_interval == interval) udp_ping_calculate_timer_interval();
            return;
        }
    ...
    udp_ping_populate_flow(src, dst, start_src_port, end_src_port, start_dst_port, end_dst_port, interval, fault_det, flow);
...
uword unformat_port_range(unformat_input_t *input, va_list *args)
{
    ...
    *start_port = *end_port = 0;
    ...
    while ((c = unformat_get_input(input)) != UNFORMAT_END_OF_INPUT) {
        ...
        if (colon_present) break;
    }

    if (!colon_present) return 0;
    ...
    if (end_port < start_port) return 0;
...
static clib_error_t *set_udp_ping_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "src %U", unformat_ip46_address, &src, IP46_TYPE_ANY))
            ;
        else if (unformat(input, "src-port-range %U", unformat_port_range, &start_src_port, &end_src_port))
            ;
        else if (unformat(input, "dst %U", unformat_ip46_address, &dst, IP46_TYPE_ANY))
            ;
        else if (unformat(input, "dst-port-range %U", unformat_port_range, &start_dst_port, &end_dst_port))
            ;
    ...
    ip46_udp_ping_set_flow(src, dst, start_src_port, end_src_port, start_dst_port, end_dst_port, (u16) interval, fault_det, is_disable);
...
static clib_error_t *show_udp_ping_summary_cmd_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (i = 0; i < vec_len(udp_ping_main.ip46_flow); i++) {
        ...
        s = format(s, "Src: %U, Dst: %U\n", format_ip46_address, &ip46_flow->src, IP46_TYPE_ANY, format_ip46_address, &ip46_flow->dst, IP46_TYPE_ANY);

        s = format(s, "Start src port: %u, End src port: %u\n", ip46_flow->udp_data.start_src_port, ip46_flow->udp_data.end_src_port);
        s = format(s, "Start dst port: %u, End dst port: %u\n", ip46_flow->udp_data.start_dst_port, ip46_flow->udp_data.end_dst_port);
        s = format(s, "Interval: %u\n", ip46_flow->udp_data.interval);

        j = 0;
        for (src_port = ip46_flow->udp_data.start_src_port; src_port <= ip46_flow->udp_data.end_src_port; src_port++) {
            for (dst_port = ip46_flow->udp_data.start_dst_port; dst_port <= ip46_flow->udp_data.end_dst_port; dst_port++) {
                ...
                s     = print_analyse_flow(s, &ip46_flow->udp_data.stats[j].analyse_data);
                j++;
            }
        }
        s = format(s, "\n\n");
...
uword udp_ping_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    while (1) {
        ...
        for (i = 0; i < vec_len(udp_ping_main.ip46_flow); i++) {
            ...
            ip46_flow = pool_elt_at_index(udp_ping_main.ip46_flow, i);
            if (ip46_flow->udp_data.next_send_time < now) udp_ping_send_ip6_pak(udp_ping_main.vlib_main, ip46_flow);
        }
    }
    return 0;
}
                          ...
                          u16                      len)
{
    ...
    if (PREDICT_FALSE(pool_is_free_index(udp_ping_main.ip46_flow, flow_id))) return;

    ip46_flow = udp_ping_main.ip46_flow + flow_id;
    /* Check port is within range */
    if (PREDICT_FALSE((src_port < ip46_flow->udp_data.start_src_port) || (src_port > ip46_flow->udp_data.end_src_port)
                      || (dst_port < ip46_flow->udp_data.start_dst_port) || (dst_port > ip46_flow->udp_data.end_dst_port)))
        return;

    flow_index = (src_port - ip46_flow->udp_data.start_src_port) * (ip46_flow->udp_data.end_dst_port - ip46_flow->udp_data.start_dst_port + 1);
    ...
    vnet_buffer(b0)->l2_classify.opaque_index = ip46_flow->udp_data.stats[flow_index].flow_ctx;
    ...
    while (opt0 < limit0) {
        ...
        switch (type0) {
            case HBH_OPTION_TYPE_IOAM_TRACE_DATA_LIST:
                ...
                if (PREDICT_FALSE(trace->trace_hdr.ioam_trace_type & BIT_LOOPBACK_REPLY)) {
                    ip6_ioam_analyse_hbh_trace_loopback(data, &trace->trace_hdr, (trace->hdr.length - 2));
                    return;
                }
                ip6_hbh_ioam_trace_data_list_handler(b0, vlib_buffer_get_current(b0), opt0);
                (void) ip6_ioam_analyse_hbh_trace(data, &trace->trace_hdr, len, (trace->hdr.length - 2));
                break;
...
void udp_ping_local_analyse(vlib_node_runtime_t *node, vlib_buffer_t *b0, ip6_header_t *ip0, ip6_hop_by_hop_header_t *hbh0, u16 *next0)
{
    ...
    *next0 = UDP_PING_NEXT_IP6_DROP;
    ...
    if (PREDICT_TRUE(hbh0->protocol == IP_PROTOCOL_UDP)) {
        ...
        if ((udp0->ping_data.probe_marker1 == clib_host_to_net_u32(UDP_PING_PROBE_MARKER1))
            && (udp0->ping_data.probe_marker2 == clib_host_to_net_u32(UDP_PING_PROBE_MARKER2))) {
     ...
     * afterall
     */
    if (PREDICT_FALSE(hbh0->protocol == IPPROTO_IPV6_ROUTE)) goto end;
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE))) {
                if (p0->flags & VLIB_BUFFER_IS_TRACED) {
                    udp_ping_trace_t *t0 = vlib_add_trace(vm, node, p0, sizeof(*t0));
                    udp_ping_t       *udp0;
                    ...
                    t0->handle     = clib_net_to_host_u16(udp0->ping_data.sender_handle);
                    t0->msg_type   = udp0->ping_data.msg_type;
                    t0->next_index = next0;
                }
                if (p1->flags & VLIB_BUFFER_IS_TRACED) {
                    udp_ping_trace_t *t1 = vlib_add_trace(vm, node, p1, sizeof(*t1));
                    udp_ping_t       *udp1;

                    /* Check for udp ping packet */
                    udp1           = (udp_ping_t *) ((u8 *) hbh1 + ((hbh1->length + 1) << 3));
                    ...
                    t1->handle     = clib_net_to_host_u16(udp1->ping_data.sender_handle);
...
static clib_error_t *udp_ping_init(vlib_main_t *vm)
{
    ...
    ip6_local_hop_by_hop_register_protocol(IP_PROTOCOL_UDP, udp_ping_local.index);
    return 0;
}
