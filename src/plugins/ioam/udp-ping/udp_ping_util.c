...
u16 udp_ping_fill_udp_data(udp_ping_t *udp_ping, u16 src_port, u16 dst_port, u8 msg_type, u16 ctx)
{
    ...
    udp_ping->ping_data.probe_marker1 = clib_host_to_net_u32(UDP_PING_PROBE_MARKER1);
    udp_ping->ping_data.probe_marker2 = clib_host_to_net_u32(UDP_PING_PROBE_MARKER2);
    ...
    udp_ping->ping_data.max_len = udp_ping->ping_data.cur_len = clib_host_to_net_u16(0);
                            ...
                            u16           ctx)
{
    ...
    trace_len = sizeof(ioam_trace_option_t) + (5 * trace_data_size * 2) - sizeof(ip6_hop_by_hop_option_t);
    ...
    trace_option->trace_hdr.ioam_trace_type = TRACE_TYPE_IF_TS_APP & TRACE_TYPE_MASK;
    ...
    /* Populate udp ping header */
    udp_len = udp_ping_fill_udp_data((udp_ping_t *) current, src_port, dst_port, msg_type, ctx);
...
{
    if ((0 == ip46_address_cmp(&flow->src, &src)) && (0 == ip46_address_cmp(&flow->dst, &dst)) && (flow->udp_data.start_src_port == start_src_port)
        && (flow->udp_data.end_src_port == end_src_port) && (flow->udp_data.start_dst_port == start_dst_port)
...
void udp_ping_create_rewrite(ip46_udp_ping_flow *flow, u16 ctx)
{
    ...
    no_flows *= ((flow->udp_data.end_src_port - flow->udp_data.start_src_port) + 1);

    vec_validate_aligned(flow->udp_data.stats, no_flows - 1, CLIB_CACHE_LINE_BYTES);

    i = 0;
    for (src_port = flow->udp_data.start_src_port; src_port <= flow->udp_data.end_src_port; src_port++) {
        for (dst_port = flow->udp_data.start_dst_port; dst_port <= flow->udp_data.end_dst_port; dst_port++) {
            u8 *rewrite = NULL;
            ...
            stats->rewrite_len  = udp_ping_create_ip6_pak(rewrite, flow->src.ip6, flow->dst.ip6, src_port, dst_port, UDP_PING_PROBE, ctx);
...
void udp_ping_send_ip6_pak(vlib_main_t *vm, ip46_udp_ping_flow *flow)
{
    ...
    for (i = 0; i < no_pak; i++) {
        ...
        if ((stats->retry != 0) && ((stats->retry % MAX_PING_RETRIES) == 0)) ip6_ioam_analyse_set_paths_down(&stats->analyse_data);
        ...
        udp_header_t            *udp = (udp_header_t *) ((u8 *) hbh + ((hbh->length + 1) << 3));
        ...
        if (flow->fault_det && (stats->retry > MAX_PING_RETRIES)) {
            ioam_trace_option_t *opt = (ioam_trace_option_t *) ip6_hbh_get_option(hbh, HBH_OPTION_TYPE_IOAM_TRACE_DATA_LIST);
