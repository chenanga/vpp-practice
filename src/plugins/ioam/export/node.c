...
static u8 *format_export_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "EXPORT: flow_label %d, next index %d", t->flow_label, t->next_index);
    return s;
}
...
always_inline void copy3cachelines(void *dst, const void *src, size_t n)
{
#if 0
...
{
    ip6_header_t *ip6_temp        = (ip6_header_t *) (export_buf->data + export_buf->current_length);
    u32           flow_label_temp = clib_net_to_host_u32(ip6_temp->ip_version_traffic_class_and_flow_label) & 0xFFF00000;
    flow_label_temp |= IOAM_MASK_DECAP_BIT((vnet_buffer(pak_buf)->l2_classify.opaque_index));
