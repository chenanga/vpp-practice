...
static u8 *format_cache_ts_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "CACHE: flow_label %d, next index %d", t->flow_label, t->next_index);
    return s;
}
...
static uword ip6_ioam_cache_ts_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (IP_PROTOCOL_TCP == ip6_locate_header(p0, ip0, IP_PROTOCOL_TCP, &tcp_offset0)) {
                tcp0 = (tcp_header_t *) ((u8 *) ip0 + tcp_offset0);
                if ((tcp0->flags & TCP_FLAG_SYN) == TCP_FLAG_SYN && (tcp0->flags & TCP_FLAG_ACK) == TCP_FLAG_ACK) {
                    /* Look up and compare */
                    hbh0 = (ip6_hop_by_hop_header_t *) (ip0 + 1);

                    if (0
                        == ioam_cache_ts_lookup(ip0,
                                                hbh0->protocol,
                        ...
                        /* response seen */
                        result = -1;
                        if (hbh_cmp) result = ip6_ioam_analyse_compare_path_delay(hbh0, hbh_cmp, cm->criteria_oneway);
                        if (result >= 0) {
                            ...
                            ioam_cache_ts_check_and_send(cache_thread_id, cache_ts_index);
                        }
                        else {
                            /* Update cache with this buffer */
                            /* If successfully updated then skip sending it */
                            if (0 == (result = ioam_cache_ts_update(cache_thread_id, cache_ts_index, bi0, hbh0))) {
                                skip = 1;
                            }
                            else
                                next0 = IOAM_CACHE_TS_ERROR_NEXT_DROP;
                    ...
                    {
                        next0 = IOAM_CACHE_TS_ERROR_NEXT_DROP;
                        if (hbh_cmp) ioam_cache_ts_check_and_send(cache_thread_id, cache_ts_index);
                    }
                }
            }
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE))) {
                if (p0->flags & VLIB_BUFFER_IS_TRACED) {
                    cache_ts_trace_t *t = vlib_add_trace(vm, node, p0, sizeof(*t));
                    t->flow_label       = clib_net_to_host_u32(ip0->ip_version_traffic_class_and_flow_label);
                    t->next_index       = next0;
                }
            }
    ...
    vlib_node_increment_counter(vm, ioam_cache_ts_node.index, CACHE_TS_ERROR_RECORDED, recorded);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (IP_PROTOCOL_TCP != ip6_locate_header(b0, ip0, IP_PROTOCOL_TCP, &tcp_offset0)) {
                goto NEXT00;
            }
            tcp0 = (tcp_header_t *) ((u8 *) ip0 + tcp_offset0);
            if ((tcp0->flags & TCP_FLAG_SYN) == TCP_FLAG_SYN && (tcp0->flags & TCP_FLAG_ACK) == 0) {
                if (no_of_responses > 0) {
                    /* Create TS select entry */
                    if (0
                        == ioam_cache_ts_add(ip0,
                                             clib_net_to_host_u16(tcp0->src_port),
                                             clib_net_to_host_u16(tcp0->dst_port),
                ...
                ioam_e2e_id_rewrite_handler((ioam_e2e_id_option_t *) ((u8 *) e2e + sizeof(ioam_e2e_cache_option_t)), &cm->sr_localsid_ts);
        ...
        NEXT00:
            if (IP_PROTOCOL_TCP != ip6_locate_header(b1, ip1, IP_PROTOCOL_TCP, &tcp_offset1)) {
                goto TRACE00;
            }
            tcp1 = (tcp_header_t *) ((u8 *) ip1 + tcp_offset1);
            if ((tcp1->flags & TCP_FLAG_SYN) == TCP_FLAG_SYN && (tcp1->flags & TCP_FLAG_ACK) == 0) {
                if (no_of_responses > 0) {
                    /* Create TS select entry */
                    if (0
                        == ioam_cache_ts_add(ip1,
                                             clib_net_to_host_u16(tcp1->src_port),
                ...
                e2e             = (ioam_e2e_cache_option_t *) ((u8 *) hbh1 + cm->rewrite_pool_index_offset);
                e2e->pool_id    = (u8) vm->thread_index;
        ...
        TRACE00:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE))) {
                if (b0->flags & VLIB_BUFFER_IS_TRACED) {
                    ip6_reset_ts_hbh_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                    t->next_index               = next0;
                }
                if (b1->flags & VLIB_BUFFER_IS_TRACED) {
                    ip6_reset_ts_hbh_trace_t *t = vlib_add_trace(vm, node, b1, sizeof(*t));
                    t->next_index               = next1;
                }
            }
    ...
    vlib_node_increment_counter(vm, cm->ip6_reset_ts_hbh_node_index, IP6_RESET_TS_HBH_ERROR_PROCESSED, processed);
    vlib_node_increment_counter(vm, cm->ip6_reset_ts_hbh_node_index, IP6_RESET_TS_HBH_ERROR_SAVED, cache_ts_added);
...
static u8 *format_ioam_cache_ts_timer_tick_trace(u8 *s, va_list *args)
{
    CLIB_UNUSED(vlib_main_t * vm)       = va_arg(*args, vlib_main_t *);
    CLIB_UNUSED(vlib_node_t * node)     = va_arg(*args, vlib_node_t *);
    ioam_cache_ts_timer_tick_trace_t *t = va_arg(*args, ioam_cache_ts_timer_tick_trace_t *);
...
{
    vlib_node_set_state(vm, ioam_cache_ts_timer_tick_node.index, enable == 0 ? VLIB_NODE_STATE_DISABLED : VLIB_NODE_STATE_POLLING);
}
...
void expired_cache_ts_timer_callback(u32 *expired_timers)
{
    ...
    vlib_node_increment_counter(cm->vlib_main, ioam_cache_ts_timer_tick_node.index, IOAM_CACHE_TS_TIMER_TICK_ERROR_TIMER, count);
}
#endif /* CLIB_MARCH_VARIANT */
...
static uword ioam_cache_ts_timer_tick_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *f)
{
    ...
    tw_timer_expire_timers_16t_2w_512sl(&cm->timer_wheels[my_thread_index], vlib_time_now(vm));
    ts.tv_sec  = 0;
