...
int ioam_cache_ip6_enable_disable(ioam_cache_main_t *em, ip6_address_t *sr_localsid, u8 is_disable)
{
    ...
    if (is_disable == 0) {
        ...
        ip6_hbh_register_option(HBH_OPTION_TYPE_IOAM_EDGE_TO_EDGE_ID, 0, ioam_e2e_id_trace_handler);
        ip6_hbh_register_option(HBH_OPTION_TYPE_IOAM_E2E_CACHE_ID, 0, ioam_e2e_cache_trace_handler);
    }
    else {
...
static void vl_api_ioam_cache_ip6_enable_disable_t_handler(vl_api_ioam_cache_ip6_enable_disable_t *mp)
{
    ...
    rv                    = ioam_cache_ip6_enable_disable(cm, &sr_localsid, (int) (mp->is_disable));
...
static clib_error_t *set_ioam_cache_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "disable"))
            is_disable = 1;
        else if (!address_set && unformat(input, "sr_localsid %U", unformat_ip6_address, &sr_localsid))
            address_set = 1;
        else
            break;
    }

    if (is_disable == 0 && !address_set) return clib_error_return(0, "Error: SRv6 LocalSID address is mandatory");
...
/* *INDENT_OFF* */
VLIB_CLI_COMMAND(set_ioam_cache_command, static) = {.path       = "set ioam ip6 cache",
...
static clib_error_t *set_ioam_tunnel_select_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (is_disable == 0 && !address_set) return clib_error_return(0, "Error: SRv6 LocalSID address is mandatory to receive response.");

    ioam_tunnel_select_ip6_enable_disable(em, one_way, no_of_responses, &sr_localsid, is_disable);

    return 0;
}

/* *INDENT_OFF* */
VLIB_CLI_COMMAND(set_ioam_cache_ts_command, static) = {.path = "set ioam ip6 sr-tunnel-select",
                                                       .short_help =
        ...
        for (i = 0; i < no_of_threads; i++) {
            vlib_cli_output(vm,
                          ...
                          (pool found to be full: %lu times)",
                            i,
                            cm->ts_stats[i].inuse,
                            cm->ts_stats[i].add_failed);

            if (verbose == 1) vlib_worker_thread_barrier_sync(vm);
            pool_foreach (ts_entry, cm->ioam_ts_pool[i]) {
                vlib_cli_output(vm, "%U", format_ioam_cache_ts_entry, ts_entry, (u32) i);
            }
...
VLIB_CLI_COMMAND(show_ioam_cache_command,
                 static) = {.path = "show ioam ip6 cache", .short_help = "show ioam ip6 cache [verbose]", .function = show_ioam_cache_command_fn};
/* *INDENT_ON* */
