...
static uword ip6_ioam_cache_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE))) {
                if (p0->flags & VLIB_BUFFER_IS_TRACED) {
                    cache_trace_t *t = vlib_add_trace(vm, node, p0, sizeof(*t));
    ...
    vlib_node_increment_counter(vm, ioam_cache_node.index, CACHE_ERROR_RECORDED, recorded);
...
static u8 *format_ip6_add_from_cache_hbh_trace(u8 *s, va_list *args)
{
    CLIB_UNUSED(vlib_main_t * vm)     = va_arg(*args, vlib_main_t *);
    CLIB_UNUSED(vlib_node_t * node)   = va_arg(*args, vlib_node_t *);
    ip6_add_from_cache_hbh_trace_t *t = va_arg(*args, ip6_add_from_cache_hbh_trace_t *);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            clib_memcpy_fast(srh0->segments, ip0->dst_address.as_u64, sizeof(ip6_address_t));
            /* Rewrite the ip6 dst address with the first hop */
            clib_memcpy_fast(ip0->dst_address.as_u64, entry->next_hop.as_u64, sizeof(ip6_address_t));
            clib_memcpy_fast(&srh0->segments[1], (u8 *) hbh0 + entry->my_address_offset, sizeof(ip6_address_t));
            ...
            ip0->protocol       = 0;
            new_l0              = clib_net_to_host_u16(ip0->payload_length) + rewrite_len + sr_rewrite_len;
            ip0->payload_length = clib_host_to_net_u16(new_l0);
            processed++;
        TRACE0:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                ip6_add_from_cache_hbh_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                t->next_index                     = next0;
            }
    ...
    vlib_node_increment_counter(vm, cm->ip6_add_from_cache_hbh_node_index, IP6_ADD_FROM_CACHE_HBH_ERROR_PROCESSED, processed);
    return frame->n_vectors;
}
