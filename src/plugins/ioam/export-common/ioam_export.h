...
inline static void ioam_export_set_next_node(ioam_export_main_t *em, u8 *next_node_name)
{
...
inline static void ioam_export_reset_next_node(ioam_export_main_t *em)
{
    vlib_node_t *next_node;
...
{
    if (vec_len(em->buffer_per_thread) > thread_id) return (pool_elt_at_index(em->buffer_pool, em->buffer_per_thread[thread_id]));
    return (0);
}
...
inline static int ioam_export_init_buffer(ioam_export_main_t *em, vlib_main_t *vm, ioam_export_buffer_t *eb)
{
    vlib_buffer_t *b = 0;

    if (!eb) return (-1);
    /* TODO: Perhaps buffer init from template here */
    if (vlib_buffer_alloc(vm, &(eb->buffer_index), 1) != 1) return (-2);
...
inline static void ioam_export_thread_buffer_free(ioam_export_main_t *em)
{
    ...
    for (i = 0; i < vec_len(em->buffer_per_thread); i++) {
        ...
        if (eb) vlib_buffer_free(vm, &(eb->buffer_index), 1);
    }
...
inline static int ioam_export_thread_buffer_init(ioam_export_main_t *em, vlib_main_t *vm)
{
    ...
    pool_alloc_aligned(em->buffer_pool, no_of_threads - 1, CLIB_CACHE_LINE_BYTES);
    vec_validate_aligned(em->buffer_per_thread, no_of_threads - 1, CLIB_CACHE_LINE_BYTES);
...
inline static void ioam_export_header_cleanup(ioam_export_main_t *em, ip4_address_t *collector_address, ip4_address_t *src_address)
{
    vec_free(em->record_header);
    em->record_header = 0;
}

inline static int ioam_export_header_create(ioam_export_main_t *em, ip4_address_t *collector_address, ip4_address_t *src_address)
{
    ip4_header_t            *ip;
    udp_header_t            *udp;
    ipfix_message_header_t  *h;
    ...
    vec_validate_aligned(rewrite, sizeof(ip4_ipfix_data_packet_t) - 1, CLIB_CACHE_LINE_BYTES);
    ...
    /* FIXUP: UDP length */
    udp->length = clib_host_to_net_u16(vec_len(rewrite) + (DEFAULT_EXPORT_RECORDS * DEFAULT_EXPORT_SIZE) - sizeof(*ip));
    ...
    s->set_id_length = ipfix_set_id_length(em->set_id, (sizeof(*s) + (DEFAULT_EXPORT_RECORDS * DEFAULT_EXPORT_SIZE)));

    /* FIXUP: h version and length length in octets if records exported are not default */
    h->version_length = version_length(sizeof(*h) + (sizeof(*s) + (DEFAULT_EXPORT_RECORDS * DEFAULT_EXPORT_SIZE)));

    /* FIXUP: ip length if records exported are not default */
    /* FIXUP: ip checksum if records exported are not default */
    ip->length   = clib_host_to_net_u16(vec_len(rewrite) + (DEFAULT_EXPORT_RECORDS * DEFAULT_EXPORT_SIZE));
...
inline static int ioam_export_send_buffer(ioam_export_main_t *em, vlib_main_t *vm, ioam_export_buffer_t *eb)
{
    ip4_header_t            *ip;
    udp_header_t            *udp;
    ipfix_message_header_t  *h;
    ...
    /* FIXUP: message header export_time */
    h->export_time = clib_host_to_net_u32((u32) (((f64) em->unix_time_0) + (vlib_time_now(em->vlib_main) - em->vlib_time_0)));
    ...
    if (PREDICT_FALSE(eb->records_in_this_buffer != DEFAULT_EXPORT_RECORDS)) {
        s->set_id_length  = ipfix_set_id_length(em->set_id /* set_id */, b0->current_length - (sizeof(*ip) + sizeof(*udp) + sizeof(*h)));
...
inline static uword ioam_export_process_common(ioam_export_main_t *em, vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f, u32 index)
{
    f64                   now;
    f64                   timeout = 30.0;
    uword                 event_type;
    ...
    while (1) {
        ...
        for (i = 0; i < vec_len(em->buffer_per_thread); i++) {
            ...
            if (CLIB_SPINLOCK_IS_LOCKED(&em->lockp[i])) continue;
            eb = pool_elt_at_index(em->buffer_pool, em->buffer_per_thread[i]);
            if (eb->records_in_this_buffer > 0 && now > (eb->touched_at + EXPORT_TIMEOUT)) {
                pool_get_aligned(em->buffer_pool, new_eb, CLIB_CACHE_LINE_BYTES);
                ...
                if (ioam_export_init_buffer(em, vm, new_eb) == 1) {
                    ...
                    vec_add(vec_buffer_to_be_sent, &em->buffer_per_thread[i], 1);
        ...
        if (vec_len(thread_index) != 0) {
            ...
            for (i = 0; i < vec_len(thread_index); i++) {
                ...
                em->buffer_per_thread[thread_index[i]] = vec_pop(vec_buffer_indices);
                clib_spinlock_unlock(&em->lockp[thread_index[i]]);
            }

            /* Send the buffers */
            for (i = 0; i < vec_len(vec_buffer_to_be_sent); i++) {
                eb = pool_elt_at_index(em->buffer_pool, vec_buffer_to_be_sent[i]);
    ...
    do {                                                                                                             \
        ...
        while (n_left_from > 0) {                                                                                    \
            ...
            while (n_left_from >= 4 && n_left_to_next >= 2) {                                                        \
                ...
                if (PREDICT_FALSE(eb0 == 0)) goto NO_BUFFER1;                                                        \
                ip_len0 = ip_len0 > DEFAULT_EXPORT_SIZE ? DEFAULT_EXPORT_SIZE : ip_len0;                             \
                ip_len1 = ip_len1 > DEFAULT_EXPORT_SIZE ? DEFAULT_EXPORT_SIZE : ip_len1;                             \
                ...
                if (PREDICT_FALSE(((node)->flags & VLIB_NODE_FLAG_TRACE))) {                                         \
                    ...
                    if (p1->flags & VLIB_BUFFER_IS_TRACED) {                                                         \
                        export_trace_t *t = vlib_add_trace(VM, N, p1, sizeof(*t));                                   \
                        t->flow_label     = clib_net_to_host_u32(ip1->V);                                            \
                        t->next_index     = next1;                                                                   \
                    }                                                                                                \
                }                                                                                                    \
            NO_BUFFER1:                                                                                              \
                vlib_validate_buffer_enqueue_x2(VM, N, next_index, to_next, n_left_to_next, bi0, bi1, next0, next1); \
            }                                                                                                        \
            while (n_left_from > 0 && n_left_to_next > 0) {                                                          \
                ...
                ebi0    = my_buf->buffer_index;                                                                      \
                eb0     = vlib_get_buffer(VM, ebi0);                                                                 \
                if (PREDICT_FALSE(eb0 == 0)) goto NO_BUFFER;                                                         \
                ip_len0 = ip_len0 > DEFAULT_EXPORT_SIZE ? DEFAULT_EXPORT_SIZE : ip_len0;                             \
                copy3cachelines(eb0->data + eb0->current_length, ip0, ip_len0);                                      \
                FIXUP_FUNC(eb0, p0);                                                                                 \
                eb0->current_length += DEFAULT_EXPORT_SIZE;                                                          \
                ...
                if (PREDICT_FALSE(((N)->flags & VLIB_NODE_FLAG_TRACE) && (p0->flags & VLIB_BUFFER_IS_TRACED))) {     \
                ...
                pkts_recorded += 1;                                                                                  \
            NO_BUFFER:                                                                                               \
                vlib_validate_buffer_enqueue_x1(VM, N, next_index, to_next, n_left_to_next, bi0, next0);             \
            }                                                                                                        \
            vlib_put_next_frame(VM, N, next_index, n_left_to_next);                                                  \
        }                                                                                                            \
        vlib_node_increment_counter(VM, export_node.index, EXPORT_ERROR_RECORDED, pkts_recorded);                    \
