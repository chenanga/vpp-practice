...
static inline ioam_seqno_data *ioam_e2ec_get_seqno_data_from_flow_ctx(u32 flow_ctx)
{
    ioam_e2e_data_t *data = NULL;
    u32              index;

    index = get_flow_data_from_flow_ctx(flow_ctx, HBH_OPTION_TYPE_IOAM_EDGE_TO_EDGE);
    data  = &ioam_e2e_main.e2e_data[index];
    return &(data->seqno_data);
}

static inline u32 ioam_e2e_get_cur_seqno_from_flow_ctx(u32 flow_ctx)
{
