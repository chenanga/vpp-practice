...
always_inline void ip6_ioam_stats_increment_counter(u32 counter_index, u64 increment)
{
...
static u8 *format_ioam_pot(u8 *s, va_list *args)
{
    ...
    s = format(s, "random = 0x%Lx, Cumulative = 0x%Lx, Index = 0x%x", random, cumulative, pot0 ? pot0->reserved_profile_id : ~0);
    return s;
}
...
int ip6_hbh_ioam_proof_of_transit_handler(vlib_buffer_t *b, ip6_header_t *ip, ip6_hop_by_hop_option_t *opt0)
{
    ...
    if (pot_encap) {
        pot0->reserved_profile_id = pot_profile_index & PROFILE_ID_MASK;
        pot_profile_incr_usage_stats(pot_profile);
    }
    else { /* Non encap node */
        if (PREDICT_FALSE(pot0->reserved_profile_id != pot_profile_index || pot_profile == 0)) {
            /* New profile announced by encap node. */
            new_profile = pot_profile_find(pot0->reserved_profile_id);
            if (PREDICT_FALSE(new_profile == 0 || new_profile->valid == 0)) {
                ip6_ioam_stats_increment_counter(IP6_IOAM_POT_PROFILE_MISS, 1);
                return (-1);
            }
            else {
    ...
    random           = clib_net_to_host_u64(pot0->random);
    cumulative       = clib_net_to_host_u64(pot0->cumulative);
    pot0->cumulative = clib_host_to_net_u64(pot_update_cumulative(pot_profile, cumulative, random));
...
int ip6_hbh_ioam_proof_of_transit_pop_handler(vlib_buffer_t *b, ip6_header_t *ip, ip6_hop_by_hop_option_t *opt0)
{
    ...
    cumulative  = clib_net_to_host_u64(pot0->cumulative);
    pot_profile = pot_profile_get_active();
    result      = pot_validate(pot_profile, cumulative, random);
...
int ip6_hop_by_hop_ioam_pot_rewrite_handler(u8 *rewrite_string, u8 *rewrite_size)
{
    ...
    if (rewrite_string && *rewrite_size == sizeof(ioam_pot_option_t)) {
        ...
        pot_option->hdr.type   = HBH_OPTION_TYPE_IOAM_PROOF_OF_TRANSIT | HBH_OPTION_TYPE_DATA_CHANGE_ENROUTE;
...
static clib_error_t *ip6_show_ioam_pot_cmd_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (i = 0; i < IP6_IOAM_POT_N_STATS; i++) {
        s = format(s, " %s - %lu\n", ip6_hop_by_hop_ioam_pot_stats_strings[i], hm->counters[i]);
    }
...
static clib_error_t *ip6_hop_by_hop_ioam_pot_init(vlib_main_t *vm)
{
    ...
    if (ip6_hbh_register_option(HBH_OPTION_TYPE_IOAM_PROOF_OF_TRANSIT,
    ...
    if (ip6_hbh_add_register_option(HBH_OPTION_TYPE_IOAM_PROOF_OF_TRANSIT, sizeof(ioam_pot_option_t), ip6_hop_by_hop_ioam_pot_rewrite_handler) < 0)
        return (clib_error_create("registration of HBH_OPTION_TYPE_IOAM_PROOF_OF_TRANSIT for rewrite failed"));

    if (ip6_hbh_pop_register_option(HBH_OPTION_TYPE_IOAM_PROOF_OF_TRANSIT, ip6_hbh_ioam_proof_of_transit_pop_handler) < 0)
