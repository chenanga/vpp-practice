...
int ioam_e2e_config_handler(void *data, u8 disable)
{
    ...
    if (!disable) {
        ...
        if (0 == *analyse) {
            if (ip6_hbh_register_option(HBH_OPTION_TYPE_IOAM_EDGE_TO_EDGE, ioam_seqno_encap_handler, ioam_e2e_trace_handler) < 0) {
                return (-1);
            }
        }
        /* If analyze node then register for decap handler */
        else {
            if (ip6_hbh_pop_register_option(HBH_OPTION_TYPE_IOAM_EDGE_TO_EDGE, ioam_seqno_decap_handler) < 0) {
                return (-1);
            }
        }
        return 0;
...
int ioam_e2e_rewrite_handler(u8 *rewrite_string, u8 *rewrite_size)
{
    ...
    if (rewrite_string && *rewrite_size == sizeof(ioam_e2e_option_t)) {
        ...
        e2e_option->hdr.type   = HBH_OPTION_TYPE_IOAM_EDGE_TO_EDGE | HBH_OPTION_TYPE_SKIP_UNKNOWN;
        e2e_option->hdr.length = sizeof(ioam_e2e_option_t) - sizeof(ip6_hop_by_hop_option_t);
        return (0);
    }
    return (-1);
}
...
u32 ioam_e2e_flow_handler(u32 ctx, u8 add)
{
    ...
    for (i = 0; i < vec_len(ioam_e2e_main.e2e_data); i++) {
        if (pool_is_free_index(ioam_e2e_main.e2e_data, i)) continue;
...
static clib_error_t *ioam_show_e2e_cmd_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (i = 0; i < vec_len(ioam_e2e_main.e2e_data); i++) {
        ...
        s = show_ioam_seqno_cmd_fn(s, &e2e_data->seqno_data, !IOAM_DEAP_ENABLED(e2e_data->flow_ctx));
    }
...
static clib_error_t *ioam_e2e_init(vlib_main_t *vm)
{
     ...
     */
    if (ip6_hbh_config_handler_register(HBH_OPTION_TYPE_IOAM_EDGE_TO_EDGE, ioam_e2e_config_handler) < 0) {
        return (
            clib_error_create("Registration of "
                              "HBH_OPTION_TYPE_IOAM_EDGE_TO_EDGE for rewrite failed"));
    }

    if (ip6_hbh_add_register_option(HBH_OPTION_TYPE_IOAM_EDGE_TO_EDGE, sizeof(ioam_e2e_option_t), ioam_e2e_rewrite_handler) < 0) {
        return (
            clib_error_create("Registration of "
                              "HBH_OPTION_TYPE_IOAM_EDGE_TO_EDGE for rewrite failed"));
    }

    if (ip6_hbh_flow_handler_register(HBH_OPTION_TYPE_IOAM_EDGE_TO_EDGE, ioam_e2e_flow_handler) < 0) {
        return (
            clib_error_create("Registration of "
