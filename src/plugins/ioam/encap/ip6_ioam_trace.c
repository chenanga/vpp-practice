...
always_inline void ip6_ioam_trace_stats_increment_counter(u32 counter_index, u64 increment)
{
...
int ip6_hop_by_hop_ioam_trace_rewrite_handler(u8 *rewrite_string, u8 *rewrite_size)
{
    ...
    trace_option->trace_hdr.ioam_trace_type     = profile->trace_type & TRACE_TYPE_MASK;
...
always_inline void ip6_hbh_ioam_loopback_handler(vlib_buffer_t *b, ip6_header_t *ip, ioam_trace_option_t *trace)
{
    u32                         buf_index;
...
int ip6_hbh_ioam_trace_data_list_handler(vlib_buffer_t *b, ip6_header_t *ip, ip6_hop_by_hop_option_t *opt)
{
    ...
    if (PREDICT_TRUE(trace->trace_hdr.data_list_elts_left)) {
         ...
         */
        elt_index = trace->trace_hdr.data_list_elts_left * fetch_trace_data_size(trace->trace_hdr.ioam_trace_type) / 4;
        elt       = &trace->trace_hdr.elts[elt_index];
        if (trace->trace_hdr.ioam_trace_type & BIT_TTL_NODEID) {
            *elt = clib_host_to_net_u32((ip->hop_limit << 24) | profile->node_id);
            elt++;
        }
...
u8 *ip6_hbh_ioam_trace_data_list_trace_handler(u8 *s, ip6_hop_by_hop_option_t *opt)
{
    ...
    s     = format(s, "  Trace Type 0x%x , %d elts left\n", trace->trace_hdr.ioam_trace_type, trace->trace_hdr.data_list_elts_left);
    trace_data_size_in_words = fetch_trace_data_size(trace->trace_hdr.ioam_trace_type) / 4;
    elt                      = &trace->trace_hdr.elts[0];
    while ((u8 *) elt < ((u8 *) (&trace->trace_hdr.elts[0]) + trace->hdr.length - 2
                         /* -2 accounts for ioam_trace_type,elts_left */)) {
        s = format(s, "    [%d] %U\n", elt_index, format_ioam_data_list_element, elt, &trace->trace_hdr.ioam_trace_type);
        elt_index++;
...
static clib_error_t *ip6_hop_by_hop_ioam_trace_init(vlib_main_t *vm)
{
    ...
    if (ip6_hbh_register_option(HBH_OPTION_TYPE_IOAM_TRACE_DATA_LIST,
                                ip6_hbh_ioam_trace_data_list_handler,
                                ip6_hbh_ioam_trace_data_list_trace_handler)
        < 0)
        return (clib_error_create("registration of HBH_OPTION_TYPE_IOAM_TRACE_DATA_LIST failed"));

    if (ip6_hbh_add_register_option(HBH_OPTION_TYPE_IOAM_TRACE_DATA_LIST, sizeof(ioam_trace_option_t), ip6_hop_by_hop_ioam_trace_rewrite_handler) < 0)
        return (clib_error_create("registration of HBH_OPTION_TYPE_IOAM_TRACE_DATA_LIST for rewrite failed"));

    return (0);
}

VLIB_INIT_FUNCTION(ip6_hop_by_hop_ioam_trace_init) = {
    .runs_after = VLIB_INITS("ip_main_init", "ip6_lookup_init", "ip6_hop_by_hop_ioam_init"),
};
...
int ip6_trace_profile_setup(void)
{
    ...
    if (ip6_ioam_trace_get_sizeof_handler(&trace_size) < 0) return (-1);
