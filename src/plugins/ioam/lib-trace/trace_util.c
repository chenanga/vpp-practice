...
static clib_error_t *set_trace_profile_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (profile) {
        trace_profile_create(profile, trace_type, num_elts, trace_tsp, node_id, app_data);
    }
    return 0;
}
...
static clib_error_t *show_trace_profile_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    s = format(s,
               ...
               p->trace_tsp,
               (p->trace_tsp == TSP_SECONDS)
                   ? "Seconds"
                   : ((p->trace_tsp == TSP_MILLISECONDS) ? "Milliseconds" : (((p->trace_tsp == TSP_MICROSECONDS) ? "Microseconds" : "Nanoseconds"))));
