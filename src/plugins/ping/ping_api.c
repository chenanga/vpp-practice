...
static void ping_api_send_ping_event(vl_api_want_ping_finished_events_t *mp, u32 request_count, u32 reply_count)
{
...
void vl_api_want_ping_finished_events_t_handler(vl_api_want_ping_finished_events_t *mp)
{
    ...
    rmp->_vl_msg_id = htons((VL_API_WANT_PING_FINISHED_EVENTS_REPLY) + (REPLY_MSG_ID_BASE));
    ...
    for (i = 1; i <= ping_repeat; i++) {
        ...
        while ((sleep_interval = time_ping_sent + ping_interval - vlib_time_now(vm)) > 0.0) {
            ...
            if (event_type == PING_RESPONSE_IP4 || event_type == PING_RESPONSE_IP6) reply_count += 1;
        }
    }
