...
u8 *format_icmp_echo_trace(u8 *s, va_list *va)
{
    ...
    s = format(s, "ICMP%s echo id %d seq %d", t->is_ip6 ? "6" : "4", t->id, t->seq);
...
static_always_inline void ip46_post_icmp_reply_event(vlib_main_t *vm, uword cli_process_id, u32 bi0, int is_ip6)
{
    ...
    STATIC_ASSERT(ARRAY_LEN(vnet_buffer(b0)->unused) * sizeof(vnet_buffer(b0)->unused[0]) > sizeof(nowts),
...
ip46_echo_reply_maybe_trace_buffer(vlib_main_t *vm, vlib_node_runtime_t *node, uword cli_process_id, u16 id, u16 seq, vlib_buffer_t *b0, int is_ip6)
{
...
ip46_icmp_echo_reply_inner_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int do_trace, int is_ip6)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (do_trace) ip46_echo_reply_maybe_trace_buffer(vm, node, cli_process_id, icmp_id, icmp_seq, b0, is_ip6);
...
static_always_inline uword ip46_icmp_echo_reply_outer_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ip6)
{
    if (node->flags & VLIB_NODE_FLAG_TRACE)
        return ip46_icmp_echo_reply_inner_node_fn(vm, node, frame, 1 /* do_trace */, is_ip6);
    else
        return ip46_icmp_echo_reply_inner_node_fn(vm, node, frame, 0 /* do_trace */, is_ip6);
}
...
static uword ip4_icmp_echo_request(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    fid = fragment_ids = clib_random_buffer_get_data(&vm->random_buffer, n_packets * sizeof(fragment_ids[0]));
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 2 && n_left_to_next > 2) {
            ...
            sum0        = ip_csum_update(sum0, ICMP4_echo_request, ICMP4_echo_reply, icmp46_header_t, type);
            sum1        = ip_csum_update(sum1, ICMP4_echo_request, ICMP4_echo_reply, icmp46_header_t, type);
            ...
            sum0     = ip_csum_update(sum0, ip0->ttl, host_config_ttl, ip4_header_t, ttl);
            sum1     = ip_csum_update(sum1, ip1->ttl, host_config_ttl, ip4_header_t, ttl);
            ip0->ttl = host_config_ttl;
            ip1->ttl = host_config_ttl;

            /* New fragment id. */
            sum0             = ip_csum_update(sum0, ip0->fragment_id, fid[0], ip4_header_t, fragment_id);
            sum1             = ip_csum_update(sum1, ip1->fragment_id, fid[1], ip4_header_t, fragment_id);
    ...
    vlib_error_count(vm, ip4_icmp_input_node.index, ICMP4_ERROR_ECHO_REPLIES_SENT, frame->n_vectors);
...
static u8 *format_icmp_input_trace(u8 *s, va_list *va)
{
    ...
    s = format(s, "%U", format_ip4_header, t->packet_data, sizeof(t->packet_data));
...
static uword ip6_icmp_echo_request(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 2 && n_left_to_next > 2) {
            ...
            sum0 = ip_csum_update(sum0, ICMP6_echo_request, ICMP6_echo_reply, icmp46_header_t, type);
            sum1 = ip_csum_update(sum1, ICMP6_echo_request, ICMP6_echo_reply, icmp46_header_t, type);
            ...
            if (ip6_address_is_link_local_unicast(&ip0->src_address) && !ip6_address_is_link_local_unicast(&ip0->dst_address)) {
            ...
            if (ip6_address_is_link_local_unicast(&ip1->src_address) && !ip6_address_is_link_local_unicast(&ip1->dst_address)) {
    ...
    vlib_error_count(vm, ip6_icmp_input_node.index, ICMP6_ERROR_ECHO_REPLIES_SENT, frame->n_vectors);
                                    ...
                                    u16                    data_len)
{
    ...
    int l34_len      = l4_header_offset + sizeof(icmp46_header_t) + offsetof(icmp46_echo_request_t, data);
    ...
    while (remaining_data_len) {
        int this_buf_data_len =
            remaining_data_len < vlib_buffer_get_default_data_size(vm) ? remaining_data_len : vlib_buffer_get_default_data_size(vm);
...
{
    u32 fib_index = is_ip6 ? ip6_fib_index_from_table_id(table_id) : ip4_fib_index_from_table_id(table_id);
    return fib_index;
}
...
static fib_node_index_t ip46_fib_table_lookup_host(u32 fib_index, ip46_address_t *pa46, int is_ip6)
{
    fib_node_index_t fib_entry_index =
        is_ip6 ? ip6_fib_table_lookup(fib_index, &pa46->ip6, 128) : ip4_fib_table_lookup(ip4_fib_get(fib_index), &pa46->ip4, 32);
...
{
    if (is_ip6) {
        if (ip6_address_is_link_local_unicast(&pa46->ip6)) return ip6_ll_fib_get(sw_if_index);
...
static u16 ip46_fill_icmp_request_at(vlib_main_t *vm, int l4_offset, u16 seq_host, u16 id_host, u16 data_len, vlib_buffer_t *b0, int is_ip6)
{
    ...
    data_len = init_icmp46_echo_request(vm, b0, l4_offset, icmp46_echo, seq_host, id_host, clib_cpu_time_now(), data_len);
    return data_len;
}
...
u16 ip4_icmp_compute_checksum(vlib_main_t *vm, vlib_buffer_t *p0, ip4_header_t *ip0)
{
    ...
    if (n_this_buffer + ip_header_length > n_ip_bytes_this_buffer) {
        n_this_buffer = n_ip_bytes_this_buffer > ip_header_length ? n_ip_bytes_this_buffer - ip_header_length : 0;
    }
    while (1) {
...
{
    u16              payload_length = data_len + sizeof(icmp46_header_t) + offsetof(icmp46_echo_request_t, data);
    ...
    if (is_ip6) {
        ...
        int bogus_length = 0;
        icmp46->checksum = ip6_tcp_udp_icmp_compute_checksum(vm, b0, ip6, &bogus_length);
    }
    else {
        ip4_header_t *ip4 = vlib_buffer_get_current(b0);
...
static int ip46_enqueue_packet(vlib_main_t *vm, vlib_buffer_t *b0, u32 burst, u32 lookup_node_index)
{
     ...
     */
    for (n_to_send = at_most_a_frame(burst), burst -= n_to_send; n_to_send > 0; n_to_send = at_most_a_frame(burst), burst -= n_to_send) {
        ...
        while (n_to_send > 1) {
            ...
            if (PREDICT_FALSE(b0copy == NULL)) goto ship_and_ret;
                                              ...
                                              int             is_ip6)
{
    ...
    if (n_buf0 < 1) ERROR_OUT(SEND_PING_ALLOC_FAIL);
    ...
    if (~0 == sw_if_index) {
        ...
        fib_index = ip46_fib_table_get_index_for_sw_if_index(sw_if_index, is_ip6, pa46);

    if (~0 == fib_index) ERROR_OUT(SEND_PING_NO_TABLE);
    if (~0 == sw_if_index) ERROR_OUT(SEND_PING_NO_INTERFACE);
    ...
    if (!ip46_set_src_address(sw_if_index, b0, is_ip6)) ERROR_OUT(SEND_PING_NO_SRC_ADDRESS);
    if (verbose) ip46_print_buffer_src_address(vm, b0, is_ip6);

    data_len = ip46_fill_icmp_request_at(vm, l4_header_offset, seq_host, id_host, data_len, b0, is_ip6);
    ...
    if (is_ip6) {
        if (pa46->ip6.as_u32[0] == clib_host_to_net_u32(0xff020000)) {
            ...
            vnet_buffer(b0)->ip.adj_index[VLIB_TX] = ip6_link_get_mcast_adj(sw_if_index);
        }
    }
    else {
    ...
    if (n_sent < burst) err = SEND_PING_NO_BUFFERS;

done:
    if (err != SEND_PING_OK) {
        if (n_buf0 > 0) vlib_buffer_free(vm, &bi0, 1);
    }
    return err;
}

send_ip46_ping_result_t
send_ip6_ping(vlib_main_t *vm, u32 table_id, ip6_address_t *pa6, u32 sw_if_index, u16 seq_host, u16 id_host, u16 data_len, u32 burst, u8 verbose)
{
    ip46_address_t target;
    target.ip6 = *pa6;
    return send_ip46_ping(vm, table_id, &target, sw_if_index, seq_host, id_host, data_len, burst, verbose, 1 /* is_ip6 */);
}

send_ip46_ping_result_t
send_ip4_ping(vlib_main_t *vm, u32 table_id, ip4_address_t *pa4, u32 sw_if_index, u16 seq_host, u16 id_host, u16 data_len, u32 burst, u8 verbose)
{
    ip46_address_t target;
    ip46_address_set_ip4(&target, pa4);
    return send_ip46_ping(vm, table_id, &target, sw_if_index, seq_host, id_host, data_len, burst, verbose, 0 /* is_ip6 */);
}
...
static void print_ip46_icmp_reply(vlib_main_t *vm, u32 bi0, int is_ip6)
{
    ...
    if (is_ip6) {
        ...
        ttl               = ip4->ttl;
        l4_offset         = ip4_header_bytes(ip4);
        payload_length    = clib_net_to_host_u16(ip4->length) + ip4_header_bytes(ip4);
    }
    ...
    vlib_cli_output(vm,
                    ...
                    format_addr_func,
                    paddr,
                    clib_host_to_net_u16(icmp_echo->seq),
                    ttl,
                    rtt * 1000.0);
}
                                  ...
                                  u32            verbose)
{
    ...
    if (PREDICT_FALSE(!rand_seed)) rand_seed = random_default_seed();
    ...
    for (i = 1; i <= ping_repeat; i++) {
        ...
        if (pa6) {
            res = send_ip6_ping(vm, table_id, pa6, sw_if_index, i, icmp_id, data_len, ping_burst, verbose);
            if (SEND_PING_OK == res)
        ...
        if (pa4) {
            res = send_ip4_ping(vm, table_id, pa4, sw_if_index, i, icmp_id, data_len, ping_burst, verbose);
            if (SEND_PING_OK == res)
                n_requests += ping_burst;
            else
                vlib_cli_output(vm, "Failed: %U", format_ip46_ping_result, res);
        ...
        while ((i <= ping_repeat) && ((sleep_interval = time_ping_sent + ping_interval - vlib_time_now(vm)) > 0.0)) {
            ...
            switch (event_type) {
                ...
                case PING_RESPONSE_IP4: {
                    ...
                    for (ii = 0; ii < vec_len(event_data); ii++) {
                        ...
                        n_replies++;
                        if (0 != bi0) vlib_buffer_free(vm, &bi0, 1);
                    }
                } break;
...
double_break:
    vlib_cli_output(vm, "\n");
    {
        ...
        vlib_cli_output(vm, "Statistics: %u sent, %u received, %f%% packet loss\n", n_requests, n_replies, loss);
...
static clib_error_t *ping_ip_address(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (unformat(input, "%U", unformat_ip4_address, &a4)) {
        ...
        if (unformat(input, "%U", unformat_ip4_address, &a4)) {
            ...
            error = clib_error_return(0, "expecting IPv4 address but got `%U'", format_unformat_error, input);
        }
    }
        ...
        if (unformat(input, "%U", unformat_ip6_address, &a6)) {
            ...
            error = clib_error_return(0, "expecting IPv6 address but got `%U'", format_unformat_error, input);
        }
    }
    else {
    ...
    while (!unformat_eof(input, NULL)) {
        if (unformat(input, "source")) {
            ...
            if (!unformat(input, "%u", &data_len)) {
                error = clib_error_return(0, "expecting size but got `%U'", format_unformat_error, input);
                goto done;
            }
            if (data_len > PING_MAXIMUM_DATA_SIZE) {
                error = clib_error_return(0, "%d is bigger than maximum allowed payload size %d", data_len, PING_MAXIMUM_DATA_SIZE);
                goto done;
            }
        }
        else if (unformat(input, "table-id")) {
            if (!unformat(input, "%u", &table_id)) {
                error = clib_error_return(0, "expecting table-id but got `%U'", format_unformat_error, input);
                goto done;
            }
        }
        else if (unformat(input, "interval")) {
            if (!unformat(input, "%f", &ping_interval)) {
                error = clib_error_return(0, "expecting interval (floating point number) got `%U'", format_unformat_error, input);
                goto done;
            }
        }
        else if (unformat(input, "repeat")) {
            if (!unformat(input, "%u", &ping_repeat)) {
                error = clib_error_return(0, "expecting repeat count but got `%U'", format_unformat_error, input);
                goto done;
            }
        }
        else if (unformat(input, "burst")) {
            if (!unformat(input, "%u", &ping_burst)) {
                error = clib_error_return(0, "expecting burst count but got `%U'", format_unformat_error, input);
                goto done;
            }
        }
...
#define MAX_PING_BURST (VLIB_FRAME_SIZE)
    ...
    if (ping_burst < 1 || ping_burst > MAX_PING_BURST) return clib_error_return(0, "burst size must be between 1 and %u", MAX_PING_BURST);

    run_ping_ip46_address(vm,
                          table_id,
                          ping_ip4 ? &a4 : NULL,
                          ping_ip6 ? &a6 : NULL,
                          sw_if_index,
...
static clib_error_t *ping_cli_init(vlib_main_t *vm)
{
    ...
    ip4_icmp_register_type(vm, ICMP4_echo_reply, ip4_icmp_echo_reply_node.index);
    if (tm->n_vlib_mains > 1) clib_spinlock_init(&pm->ping_run_check_lock);
