...
static void child_handler(int sig)
{
    ...
    while ((pid = waitpid(-1, &status, WNOHANG)) > 0) {
        if (pid == kmp->monitor_pid) {
            clib_warning("Monitor child %d exited with status %d!", pid, status);
...
clib_error_t *launch_monitor(fateshare_main_t *kmp)
{
    ...
    int logfd = open((char *) kmp->monitor_logfile, O_APPEND | O_RDWR | O_CREAT, 0777);
    if (logfd < 0) {
...
static clib_error_t *fateshare_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    *new_command = 0;
    ...
    if (fmp->monitor_cmd == 0) {
        ...
        if (path == 0) return clib_error_return(0, "could not get exec path - set monitor manually");
        ...
        if ((p = strrchr((char *) path, '/')) == 0) {
            vec_free(path);
            return clib_error_return(0, "could not determine vpp directory - set monitor manually");
        }
        *p = 0;
    ...
    if (fmp->monitor_logfile == 0) {
        fmp->monitor_logfile = format(0, "/tmp/vpp-fateshare-monitor-log.txt\0");
    }
...
clib_error_t *fateshare_init(vlib_main_t *vm)
{
    fateshare_main_t *kmp   = &fateshare_main;
    clib_error_t     *error = 0;
...
static clib_error_t *fateshare_send_hup_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    clib_error_t     *error = 0;
    fateshare_main_t *kmp   = &fateshare_main;
    ...
    if (kmp->monitor_pid) {
        ...
        if (rc) {
            error = clib_error_return(0, "can not send signal to monitor process: %s", strerror(errno));
        }
    }
    else {
...
VLIB_PLUGIN_REGISTER() = {
    ...
    .description =
        "Run child processes which will share fate with VPP, restart "
        "them if they quit",
    .default_disabled = 1,
};
