...
static int api_mactime_enable_disable(vat_main_t *vam)
{
...
static int api_mactime_dump(vat_main_t *vam)
{
    ...
    if (PREDICT_FALSE((now - tm->sunday_midnight) > 86400.0 * 7.0)) tm->sunday_midnight = clib_timebase_find_sunday_midnight(now);
...
static int api_mactime_add_del_range(vat_main_t *vam)
{
    unformat_input_t               *i = vam->input;
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(i, "allow-range %U", unformat_clib_timebase_range_vector, &rp))
            allow = 1;
        else if (unformat(i, "allow-quota-range %U", unformat_clib_timebase_range_vector, &rp))
            allow_quota = 1;
        else if (unformat(i, "drop-range %U", unformat_clib_timebase_range_vector, &rp))
            drop = 1;
    ...
    if (name_set == 0) {
        device_name = format(0, "mac %U%c", my_format_mac_address, mac_address, 0);
    }
