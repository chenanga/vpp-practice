...
static uword mactime_node_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_tx)
{
    ...
    if (is_tx) fcm = vnet_feature_get_config_main(arc);
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (clib_bihash_search_8_8(lut, &kv, &kv) < 0) {
                ...
                mactime_send_create_entry_message(is_tx ? en0->dst_address : en0->src_address);
            ...
            if ((dp->flags & MACTIME_DEVICE_FLAG_DYNAMIC_ALLOW) && PREDICT_FALSE(dp->data_quota)) {
                vlib_counter_t device_current_count;
                vlib_get_combined_counter(&mm->allow_counters, dp - mm->devices, &device_current_count);
                if (device_current_count.bytes >= dp->data_quota) {
                    next0     = MACTIME_NEXT_DROP;
                    b0->error = node->errors[MACTIME_ERROR_QUOTA_DROP];
                    vlib_increment_combined_counter(&mm->drop_counters, thread_index, dp - mm->devices, 1, len0);
                    goto trace0;
                }
            }

            /* Static drop / allow? */
            if (PREDICT_FALSE(dp->flags & (MACTIME_DEVICE_FLAG_STATIC_DROP | MACTIME_DEVICE_FLAG_STATIC_ALLOW))) {
                ...
                else /* note next0 set to allow */
                {
                     ...
                     */
                    if (PREDICT_FALSE(dp->flags & MACTIME_DEVICE_FLAG_DROP_UDP_10001)) {
                        ip4_header_t *ip  = (void *) (((u8 *) en0) + 14);
                        udp_header_t *udp = (udp_header_t *) (ip + 1);
                        if (ip->protocol != IP_PROTOCOL_UDP) goto pass;
                        if (clib_net_to_host_u16(udp->dst_port) == 10001 || clib_net_to_host_u16(udp->dst_port) == 9603) {
                            next0     = MACTIME_NEXT_DROP;
                    ...
                    pass:
                        vlib_increment_combined_counter(&mm->allow_counters, thread_index, dp - mm->devices, 1, len0);
                        packets_ok++;
                    }
                }
                goto trace0;
            ...
            for (i = 0; i < vec_len(dp->ranges); i++) {
                ...
                if (dp->flags & MACTIME_DEVICE_FLAG_DYNAMIC_ALLOW_QUOTA) has_dynamic_range_allow = 1;
        ...
        trace0:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                ...
                clib_memcpy_fast(t->src_mac, en0->src_address, sizeof(t->src_mac));

                t->next_index   = next0;
                t->device_index = device_index0;

                if (dp) {
                    clib_memcpy_fast(t->device_name, dp->device_name, ARRAY_LEN(t->device_name));
    ...
    vlib_node_increment_counter(vm, node->node_index, MACTIME_ERROR_OK, packets_ok);
