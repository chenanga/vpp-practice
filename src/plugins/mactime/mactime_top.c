...
static void vl_api_mactime_details_t_handler(vl_api_mactime_details_t *mp)
{
    ...
    if (PREDICT_FALSE(mm->device_by_device_name == 0)) mm->device_by_device_name = hash_create_string(0, sizeof(uword));
    ...
    if (p)
        ...
        hash_set_mem(mm->device_by_device_name, hash_name_copy, dev - mm->devices);
    }

    clib_memcpy_fast(dev->mac_address, mp->mac_address, sizeof(dev->mac_address));
...
static int connect_to_vpp(char *name)
{
    ...
    mm->msg_id_base = vl_client_get_first_plugin_msg_id((char *) msg_base_lookup_name);

    vec_free(msg_base_lookup_name);

    if (mm->msg_id_base == (u16) ~0) return -1;
...
static void dump_mactime_table(mt_main_t *mm)
{
    ...
    mp->_vl_msg_id     = clib_host_to_net_u16(VL_API_MACTIME_DUMP + mm->msg_id_base);
    ...
    if (mm->retval && (mm->retval != VNET_API_ERROR_NO_CHANGE)) clib_warning("dump reply %d", mm->retval);
}
...
static void scrape_stats_segment(mt_main_t *mm)
{
    ...
    /* Nothing to do... */
    if (vec_len(pool_indices) == 0) return;
...
again1:
    ...
    if (stat_segment_access_end(&sa, sm)) goto again1;
...
again2:
    ...
    /* Ugh, segment changed during access. Try again */
    if (stat_segment_access_end(&sa, sm)) goto again2;
}
...
static u8 *format_device(u8 *s, va_list *args)
{
    ...
    if (dp == 0) {
        s = format(s, "%-15s %5s %18s %14s %10s %11s %13s", "Device Name", "Index", "Addresses", "Status", "AllowPkt", "AllowByte", "DropPkt");
        vec_add1(s, '\n');
        return s;
    }
    ...
    for (j = 0; j < vec_len(dp->ranges); j++) {
        ...
        if (verbose) s = format(s, "  Range %d: %U - %U\n", j, format_clib_timebase_time, start0, format_clib_timebase_time, end0);
        ...
        if (now >= start0 && now <= end0) {
            ...
            if (verbose) {
                ...
                s = format(s, "     %U - %U\n", format_clib_timebase_time, start0, format_clib_timebase_time, end0);
            }
            goto print;
        }
    }
    if (verbose && j) s = format(s, "  No range match.\n");
...
print:
    ...
    s = format(s,
               ...
               mm->allow_counters[dp->pool_index].bytes,
               10,
...
int main(int argc, char **argv)
{
    ...
    clib_timebase_init(&mm->timebase, mm->timezone_offset, CLIB_TIMEBASE_DAYLIGHT_USA, 0 /* allocate a clib_time_t */);
