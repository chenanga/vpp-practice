...
static void feature_init(mactime_main_t *mm)
{
    if (mm->feature_initialized == 0) {
        ...
        clib_bihash_init_8_8(&mm->lookup_table, "mactime lookup table", mm->lookup_table_num_buckets, mm->lookup_table_memory_size);
        clib_timebase_init(&mm->timebase, mm->timezone_offset, CLIB_TIMEBASE_DAYLIGHT_USA, &(mm->vlib_main->clib_time));
...
int mactime_enable_disable(mactime_main_t *mm, u32 sw_if_index, int enable_disable)
{
    ...
    /* Utterly wrong? */
    if (pool_is_free_index(mm->vnet_main->interface_main.sw_interfaces, sw_if_index)) return VNET_API_ERROR_INVALID_SW_IF_INDEX;

    /* Not a physical port? */
    sw = vnet_get_sw_interface(mm->vnet_main, sw_if_index);
    if (sw->type != VNET_SW_INTERFACE_TYPE_HARDWARE) return VNET_API_ERROR_INVALID_SW_IF_INDEX;

    vnet_feature_enable_disable("device-input", "mactime", sw_if_index, enable_disable, 0, 0);
    vnet_feature_enable_disable("interface-output", "mactime-tx", sw_if_index, enable_disable, 0, 0);
    if (url_init_done == 0) {
...
static void vl_api_mactime_enable_disable_t_handler(vl_api_mactime_enable_disable_t *mp)
{
    ...
    rv = mactime_enable_disable(mm, ntohl(mp->sw_if_index), (int) (mp->enable_disable));
...
static void vl_api_mactime_dump_t_handler(vl_api_mactime_dump_t *mp)
{
    ...
    pool_foreach (dev, mm->devices) {
        message_size = sizeof(*ep) + vec_len(dev->device_name) + vec_len(dev->ranges) * sizeof(ep->ranges[0]);

        ep = vl_msg_api_alloc(message_size);
        memset(ep, 0, message_size);
        ep->_vl_msg_id = clib_host_to_net_u16(VL_API_MACTIME_DETAILS + mm->msg_id_base);
        ...
        clib_memcpy_fast(ep->mac_address, dev->mac_address, sizeof(ep->mac_address));
        ...
        name_len = (name_len < ARRAY_LEN(ep->device_name)) ? name_len : ARRAY_LEN(ep->device_name) - 1;
...
send_reply:
    REPLY_MACRO2(VL_API_MACTIME_DUMP_REPLY, ({ rmp->table_epoch = clib_host_to_net_u32(mm->device_table_epoch); }));
}
...
static void vl_api_mactime_add_del_range_t_handler(vl_api_mactime_add_del_range_t *mp)
{
    ...
    if (PREDICT_FALSE(mm->device_table_epoch == 0)) mm->device_table_epoch++;
    ...
    if (clib_bihash_search_8_8(lut, &kv, &kv) < 0) found = 0;
    ...
    if (mp->is_add) {
        ...
        if (found == 0) {
            ...
            vlib_validate_combined_counter(&mm->allow_counters, dp - mm->devices);
            vlib_zero_combined_counter(&mm->allow_counters, dp - mm->devices);
            vlib_validate_combined_counter(&mm->drop_counters, dp - mm->devices);
            ...
            if (i) {
                ...
                if (mp->drop) dp->flags = MACTIME_DEVICE_FLAG_DYNAMIC_DROP;
                if (mp->allow) dp->flags = MACTIME_DEVICE_FLAG_DYNAMIC_ALLOW;
                if (mp->allow_quota) dp->flags = MACTIME_DEVICE_FLAG_DYNAMIC_ALLOW_QUOTA;
            }
            else {
                /* no ranges, it's a static allow/drop */
                if (mp->drop) dp->flags = MACTIME_DEVICE_FLAG_STATIC_DROP;
                if (mp->allow) dp->flags = MACTIME_DEVICE_FLAG_STATIC_ALLOW;
            }
            if (mp->no_udp_10001) dp->flags |= MACTIME_DEVICE_FLAG_DROP_UDP_10001;
...
static clib_error_t *mactime_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "lookup-table-buckets %u", &mm->lookup_table_num_buckets))
            ;
        else if (unformat(input, "lookup-table-memory %U", unformat_memory_size, &mm->lookup_table_memory_size))
            ;
...
static clib_error_t *show_mactime_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (mm->feature_initialized == 0) return clib_error_return(0, "Feature not initialized, suggest 'help mactime enable-disable'...");
    ...
    if (PREDICT_FALSE((now - mm->sunday_midnight) > 86400.0 * 7.0)) mm->sunday_midnight = clib_timebase_find_sunday_midnight(now);
    ...
    if (verbose) vlib_cli_output(vm, "Time now: %U", format_clib_timebase_time, now);

    pool_foreach (dp, mm->devices) {
        vec_add1(pool_indices, dp - mm->devices);
    }

    vlib_cli_output(vm, "%-15s %18s %14s %10s %11s %13s", "Device Name", "Addresses", "Status", "AllowPkt", "AllowByte", "DropPkt");
    ...
    for (i = 0; i < vec_len(pool_indices); i++) {
        ...
        for (j = 0; j < vec_len(dp->ranges); j++) {
            ...
            if (verbose > 1) vlib_cli_output(vm, "  Range %d: %U - %U", j, format_clib_timebase_time, start0, format_clib_timebase_time, end0);
            ...
            if (now >= start0 && now <= end0) {
                ...
                if (verbose) {
                    ...
                    vlib_cli_output(vm, "     %U - %U", format_clib_timebase_time, start0, format_clib_timebase_time, end0);
                }
                goto print;
            }
        }
        if (verbose && j) vlib_cli_output(vm, "  No range match.");
        if (dp->flags & MACTIME_DEVICE_FLAG_STATIC_DROP) current_status = 0;
        if (dp->flags & MACTIME_DEVICE_FLAG_STATIC_ALLOW) current_status = 1;
        if (dp->flags & MACTIME_DEVICE_FLAG_DYNAMIC_ALLOW) current_status = 2;
        if (dp->flags & MACTIME_DEVICE_FLAG_DYNAMIC_DROP) current_status = 3;
        if (dp->flags & MACTIME_DEVICE_FLAG_DYNAMIC_ALLOW_QUOTA) current_status = 4;

    print:
        vec_reset_length(macstring);
        macstring = format(macstring, "%U", format_mac_address, dp->mac_address);
        ...
        vlib_get_combined_counter(&mm->allow_counters, dp - mm->devices, &allow);
        vlib_get_combined_counter(&mm->drop_counters, dp - mm->devices, &drop);
        vlib_cli_output(vm,
                        "%-15s %18s %14s %10lld %U %13lld",
                        dp->device_name,
                        macstring,
                        status_string,
                        allow.packets,
        ...
        if (dp->data_quota > 0)
            vlib_cli_output(vm,
                            ...
                            "Use ",
                            format_bytes_with_width,
                            dp->data_used_in_range,
                            8);
        /* This is really only good for small N... */
        for (j = 0; j < vec_len(mm->arp_cache_copy); j++) {
            ipn = ip_neighbor_get(mm->arp_cache_copy[j]);
            if (!memcmp(dp->mac_address, ipn->ipn_mac.bytes, sizeof(ipn->ipn_mac))) {
                vlib_cli_output(vm, "%17s%U", " ", format_ip46_address, ip_neighbor_get_ip(ipn), IP46_TYPE_IP4);
            }
        }
    }
    vec_free(macstring);
