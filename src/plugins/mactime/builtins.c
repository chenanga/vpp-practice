...
static hss_url_handler_rc_t handle_get_mactime(hss_url_handler_args_t *args)
{
    ...
    print:
        ...
        s = format(s, "%sname%s: %s%s%s, %sstatus%s: %s%s%s,", q, q, q, dp->device_name, q, q, q, q, status_string, q);
        ...
        for (j = 0; j < vec_len(mm->arp_cache_copy); j++) {
            ...
            if (!memcmp(dp->mac_address, ip_neighbor_get_mac(n), sizeof(mac_address_t))) {
                s = format(s, ", %sip4_address%s: %s%U%s", q, q, q, format_ip46_address, ip_neighbor_get_ip(n), IP46_TYPE_IP4, q);
                break;
            }
        }
    }
