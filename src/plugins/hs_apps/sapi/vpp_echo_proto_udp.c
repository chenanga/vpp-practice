...
static void udp_echo_cleanup_cb(echo_session_t *s, u8 parent_died)
{
    ...
    if (!em->n_clients_connected) em->state = STATE_DATA_DONE;
}
...
static void udp_echo_connected_cb(session_connected_bundled_msg_t *mp, u32 session_index, u8 is_failed)
{
    ...
    if (is_failed) {
        ECHO_FAIL(ECHO_FAIL_UDP_BAPI_CONNECT, "Bapi connect errored on session %u", session_index);
...
static void udp_echo_bound_uri_cb(session_bound_msg_t *mp, echo_session_t *session)
{
    ...
    if (!em->i_am_master || em->uri_elts.transport_proto != TRANSPORT_PROTO_UDP) return;

    if (echo_attach_session(mp->segment_handle, mp->rx_fifo, mp->tx_fifo, mp->vpp_evt_q, session)) {
