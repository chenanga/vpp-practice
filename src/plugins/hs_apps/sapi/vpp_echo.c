...
static void echo_assert_test_suceeded(echo_main_t *em)
{
    if (em->rx_results_diff)
        CHECK_DIFF(ECHO_FAIL_TEST_ASSERT_RX_TOTAL, em->stats.rx_expected, em->stats.rx_total, "Invalid amount of data received");
    else
        CHECK_SAME(ECHO_FAIL_TEST_ASSERT_RX_TOTAL, em->stats.rx_expected, em->stats.rx_total, "Invalid amount of data received");

    if (em->tx_results_diff)
        CHECK_DIFF(ECHO_FAIL_TEST_ASSERT_TX_TOTAL, em->stats.tx_expected, em->stats.tx_total, "Invalid amount of data sent");
    else
        CHECK_SAME(ECHO_FAIL_TEST_ASSERT_TX_TOTAL, em->stats.tx_expected, em->stats.tx_total, "Invalid amount of data sent");

    clib_spinlock_lock(&em->sid_vpp_handles_lock);
    CHECK_SAME(ECHO_FAIL_TEST_ASSERT_ALL_SESSIONS_CLOSED, 0, hash_elts(em->session_index_by_vpp_handles), "Some sessions are still open");
    clib_spinlock_unlock(&em->sid_vpp_handles_lock);
}

always_inline void echo_session_dequeue_notify(echo_session_t *s)
{
    int rv;
    if (!svm_fifo_set_event(s->rx_fifo)) return;
    if ((rv = app_send_io_evt_to_vpp(s->vpp_evt_q, s->rx_fifo->shr->master_session_index, SESSION_IO_EVT_RX, SVM_Q_WAIT)))
...
static int connect_to_vpp(echo_main_t *em)
{
    api_main_t *am = vlibapi_get_main();

    if (em->use_app_socket_api) return echo_api_connect_app_socket(em);

    if (em->use_sock_api) {
        if (vl_socket_client_connect((char *) em->socket_name, (char *) em->app_name, 0 /* default rx, tx buffer */)) {
        ...
        if (vl_client_connect_to_vlib("/vpe-api", (char *) em->app_name, 32) < 0) {
...
{
    u8 *start_evt         = format(0, "%U", echo_format_timing_event, em->timing.start_event);
    u8 *end_evt           = format(0, "%U", echo_format_timing_event, em->timing.end_event);
    u8  start_evt_missing = !(em->timing.events_sent & em->timing.start_event);
    u8  end_evt_missing   = (em->rx_results_diff || em->tx_results_diff) ? 0 : !(em->timing.events_sent & em->timing.end_event);
    f64 deltat            = start_evt_missing || end_evt_missing ? 0 : em->timing.end_time - em->timing.start_time;

    if (start_evt_missing) ECHO_FAIL(ECHO_FAIL_MISSING_START_EVENT, "Expected event %v to happen, but it did not!", start_evt);

    if (end_evt_missing) ECHO_FAIL(ECHO_FAIL_MISSING_END_EVENT, "Expected event %v to happen, but it did not!", end_evt);
...
static void print_global_stats(echo_main_t *em)
{
    ...
    if (start_evt_missing) fformat(stdout, "Missing Start Timing Event (%v)!\n", start_evt);
    if (end_evt_missing) fformat(stdout, "Missing End Timing Event (%v)!\n", end_evt);
    fformat(stdout, "-------- TX --------\n");
    fformat(stdout,
            "%lld bytes (%lld mbytes, %lld gbytes) in %.6f seconds\n",
            em->stats.tx_total,
            em->stats.tx_total / (1ULL << 20),
            em->stats.tx_total / (1ULL << 30),
            deltat);
    if (deltat) fformat(stdout, "%.4f Gbit/second\n", (em->stats.tx_total * 8.0) / deltat / 1e9);
    fformat(stdout, "-------- RX --------\n");
    fformat(stdout,
            "%lld bytes (%lld mbytes, %lld gbytes) in %.6f seconds\n",
            em->stats.rx_total,
            em->stats.rx_total / (1ULL << 20),
            em->stats.rx_total / (1ULL << 30),
            deltat);
    ...
    if (em->has_failed) fformat(stdout, "\nFailure Return Status: %d\n%v", em->has_failed, em->fail_descr);
    vec_free(start_evt);
    vec_free(end_evt);
}
...
void echo_update_count_on_session_close(echo_main_t *em, echo_session_t *s)
{
    ECHO_LOG(2,
             "[%lu/%lu] -> %U -> [%lu/%lu]",
             s->bytes_received,
             s->bytes_received + s->bytes_to_receive,
             echo_format_session,
             s,
             s->bytes_sent,
             s->bytes_sent + s->bytes_to_send);

    if (PREDICT_FALSE((em->stats.rx_total == em->stats.rx_expected) && (em->stats.tx_total == em->stats.tx_expected)))
...
static void echo_session_detach_fifos(echo_session_t *s)
{
    ...
    fs = fifo_segment_get_segment_if_valid(&em->segment_main, s->rx_fifo->segment_index);
...
static void echo_free_sessions(echo_main_t *em)
{
    ...
    pool_foreach (s, em->sessions) {
        if (s->session_state == ECHO_SESSION_STATE_CLOSED) vec_add1(session_indexes, s->session_index);
    }
    vec_foreach (session_index, session_indexes) {
        /* Free session */
        s = pool_elt_at_index(em->sessions, *session_index);
        echo_session_detach_fifos(s);
        echo_session_handle_add_del(em, s->vpp_session_handle, SESSION_INVALID_INDEX);
...
static void test_recv_bytes(echo_main_t *em, echo_session_t *s, u8 *rx_buf, u32 n_read)
{
    ...
    for (i = 0; i < n_read; i++) {
        ...
        if (rx_buf[i] == expected || em->max_test_msg > 0) continue;
        ECHO_LOG(1, "Session 0x%lx byte %lld was 0x%x expected 0x%x", s->vpp_session_handle, s->bytes_received + i, rx_buf[i], expected);
        em->max_test_msg--;
        if (em->max_test_msg == 0) ECHO_LOG(1, "Too many errors, hiding next ones");
        if (em->test_return_packets == RETURN_PACKETS_ASSERT) ECHO_FAIL(ECHO_FAIL_TEST_BYTES_ERR, "test-bytes errored");
    }
}
...
static int recv_data_chunk(echo_main_t *em, echo_session_t *s, u8 *rx_buf)
{
    ...
    if (svm_fifo_needs_deq_ntf(s->rx_fifo, n_read)) echo_session_dequeue_notify(s);

    if (em->test_return_packets) test_recv_bytes(em, s, rx_buf, n_read);
...
static int send_data_chunk(echo_session_t *s, u8 *tx_buf, int offset, int len)
{
    ...
    if (!bytes_this_chunk) return 0;
    n_sent = app_send((app_session_t *) s, tx_buf + offset, bytes_this_chunk, SVM_Q_WAIT);
...
static inline void echo_check_closed_listener(echo_main_t *em, echo_session_t *s)
{
    echo_session_t *ls;
    /* if parent has died, terminate gracefully */
    if (s->listener_index == SESSION_INVALID_INDEX) {
        ECHO_LOG(3, "%U: listener_index == SESSION_INVALID_INDEX", echo_format_session, s);
        return;
    }
    ...
    ECHO_LOG(3, "%U died, close child %U", echo_format_session, ls, echo_format_session, s);
...
static void echo_handle_data(echo_main_t *em, echo_session_t *s, u8 *rx_buf)
{
    ...
    if ((em->data_source == ECHO_TEST_DATA_SOURCE) && s->bytes_to_send)
        n_sent = send_data_chunk(s, em->connect_test_data, s->bytes_sent % em->tx_buf_size, em->tx_buf_size);
    ...
    if (!s->bytes_to_send && !s->bytes_to_receive) {
        ...
        if (s->session_state == ECHO_SESSION_STATE_AWAIT_DATA)
            ...
            if (em->send_stream_disconnects == ECHO_CLOSE_F_ACTIVE) {
                echo_send_rpc(em, echo_send_disconnect_session, (echo_rpc_args_t *) &s->vpp_session_handle);
        ...
        ECHO_LOG(3, "%U: %U", echo_format_session, s, echo_format_session_state, s->session_state);
        return;
    }
    ...
    if (em->log_lvl > 1) {
        if (n_sent || n_read)
            ...
            s->idle_cycles = 0;
            ECHO_LOG(2, "Idle client TX:%dB RX:%dB", s->bytes_to_send, s->bytes_to_receive);
            ECHO_LOG(2, "Idle FIFOs TX:%dB RX:%dB", svm_fifo_max_dequeue(s->tx_fifo), svm_fifo_max_dequeue(s->rx_fifo));
            ECHO_LOG(2, "Session 0x%lx state %U", s->vpp_session_handle, echo_format_session_state, s->session_state);
        }
    }
}
...
static void *echo_data_thread_fn(void *arg)
{
    ...
    for (i = 0; !em->time_to_stop; i = (i + 1) % thread_n_sessions) {
        ...
        if (session_index == SESSION_INVALID_INDEX) continue;
        s = pool_elt_at_index(em->sessions, session_index);
        ...
        if (n_closed_sessions == thread_n_sessions) break;
    }
...
static void session_unlisten_handler(session_unlisten_reply_msg_t *mp)
{
    ...
    if (--em->listen_session_cnt == 0) em->state = STATE_DISCONNECTED;
}
...
static void session_bound_handler(session_bound_msg_t *mp)
{
    ...
    if (mp->retval) {
        ECHO_FAIL(ECHO_FAIL_BIND, "bind failed: %U", format_api_error, clib_net_to_host_u32(mp->retval));
        return;
    }
    ECHO_LOG(1,
             "listening on %U:%u",
    ...
    if (++em->listen_session_cnt == em->n_uris) em->state = STATE_LISTEN;
    if (em->proto_cb_vft->bound_uri_cb) em->proto_cb_vft->bound_uri_cb(mp, listen_session);
}
...
static void session_accepted_handler(session_accepted_msg_t *mp)
{
    ...
    if (!(ls = echo_get_session_from_handle(em, mp->listener_handle))) {
        ECHO_FAIL(ECHO_FAIL_SESSION_ACCEPTED_BAD_LISTENER, "Unknown listener handle 0x%lx", mp->listener_handle);
        return;
    }
    ...
    if (echo_attach_session(mp->segment_handle, mp->server_rx_fifo, mp->server_tx_fifo, mp->vpp_event_queue_address, session)) {
        ECHO_FAIL(ECHO_FAIL_ACCEPTED_WAIT_FOR_SEG_ALLOC, "accepted wait_for_segment_allocation errored");
        return;
    }
    ...
    clib_memcpy_fast(&session->transport.lcl_ip, &em->uri_elts.ip, sizeof(ip46_address_t));
    ...
    ECHO_LOG(2, "Accepted session 0x%lx S[%u] -> 0x%lx S[%u]", mp->handle, session->session_index, mp->listener_handle, session->listener_index);
    echo_session_handle_add_del(em, mp->handle, session->session_index);

    app_alloc_ctrl_evt_to_vpp(session->vpp_evt_q, app_evt, SESSION_CTRL_EVT_ACCEPTED_REPLY);
...
static void session_connected_handler(session_connected_msg_t *mp)
{
    ...
    if (mp->retval) {
        if (em->proto_cb_vft->connected_cb) em->proto_cb_vft->connected_cb((session_connected_bundled_msg_t *) mp, listener_index, 1 /* is_failed */);
        return;
    }
    ...
    if (echo_attach_session(mp->segment_handle, mp->server_rx_fifo, mp->server_tx_fifo, mp->vpp_event_queue_address, session)) {
        ECHO_FAIL(ECHO_FAIL_CONNECTED_WAIT_FOR_SEG_ALLOC, "connected wait_for_segment_allocation errored");
        return;
    }
    ...
    /* session->transport needed by app_send_dgram */
    clib_memcpy_fast(&session->transport.lcl_ip, &mp->lcl.ip, sizeof(ip46_address_t));
    session->transport.is_ip4   = mp->lcl.is_ip4;
    session->transport.lcl_port = mp->lcl.port;
    clib_memcpy_fast(&session->transport.rmt_ip, &em->uri_elts.ip, sizeof(ip46_address_t));
    session->transport.rmt_port = em->uri_elts.port;

    echo_session_handle_add_del(em, mp->handle, session->session_index);
    em->proto_cb_vft->connected_cb((session_connected_bundled_msg_t *) mp, session->session_index, 0 /* is_failed */);
}
...
static void session_disconnected_handler(session_disconnected_msg_t *mp)
{
    ...
    if (s->session_state == ECHO_SESSION_STATE_CLOSED) {
        ECHO_LOG(2, "%U: already in ECHO_SESSION_STATE_CLOSED", echo_format_session, s);
    }
    else {
...
static int echo_recv_fd(echo_main_t *em, int *fds, int n_fds)
{
    if (em->use_app_socket_api) return echo_sapi_recv_fd(em, fds, n_fds);
...
static void add_segment_handler(session_app_add_segment_msg_t *mp)
{
    ...
    if (mp->fd_flags & SESSION_FD_F_MEMFD_SEGMENT) {
        ...
        if (echo_segment_attach(segment_handle, seg_name, SSVM_SEGMENT_MEMFD, fds[0])) {
        ...
        if (fifo_segment_attach(sm, a)) {
            ECHO_FAIL(ECHO_FAIL_VL_API_FIFO_SEG_ATTACH, "fifo_segment_attach ('%s') failed", seg_name);
            goto failed;
        }
    }
...
static void handle_mq_event(session_event_t *e)
{
    switch (e->event_type) {
        ...
        case SESSION_CTRL_EVT_DISCONNECTED:
            return session_disconnected_handler((session_disconnected_msg_t *) e->data);
        case SESSION_CTRL_EVT_RESET:
            return session_reset_handler((session_reset_msg_t *) e->data);
        case SESSION_CTRL_EVT_UNLISTEN_REPLY:
            return session_unlisten_handler((session_unlisten_reply_msg_t *) e->data);
...
static inline void echo_print_periodic_stats(echo_main_t *em)
{
    ...
    if (delta < em->periodic_stats_delta) return;

    clib_memcpy_fast(st, &em->stats, sizeof(*st));
    if (st->rx_total - lst->rx_total) clib_warning("RX: %U", echo_format_bytes_per_sec, (st->rx_total - lst->rx_total) / delta);
    if (st->tx_total - lst->tx_total) clib_warning("TX: %U", echo_format_bytes_per_sec, (st->tx_total - lst->tx_total) / delta);
    if (st->connected_count.q - lst->connected_count.q) clib_warning("conn: %d/s", st->connected_count.q - lst->connected_count.q);
    if (st->accepted_count.q - lst->accepted_count.q) clib_warning("accept: %d/s", st->accepted_count.q - lst->accepted_count.q);
...
static void *echo_mq_thread_fn(void *arg)
{
    ...
    while (em->state < STATE_DETACHED && !em->time_to_stop) {
        if (em->periodic_stats_delta) echo_print_periodic_stats(em);
...
static inline void echo_cycle_ip(echo_main_t *em, ip46_address_t *ip, ip46_address_t *src_ip, u32 i)
{
    u8 *ipu8;
    u8  l;
    if (i % em->n_uris == 0) {
...
static void clients_run(echo_main_t *em)
{
    ...
    for (i = 0; i < em->n_connects; i++) {
        ...
        if (em->lcl_ip_set) echo_cycle_ip(em, &a->lcl_ip, &em->lcl_ip, i);
...
static void server_run(echo_main_t *em)
{
    ...
    if (wait_for_state_change(em, STATE_DISCONNECTED, TIMEOUT)) {
        ECHO_FAIL(ECHO_FAIL_SERVER_DISCONNECT_TIMEOUT, "Timeout waiting for state disconnected");
        return;
    }
}
...
static void print_usage_and_exit(void)
{
    ...
    for (i = 0; i < vec_len(em->available_proto_cb_vft); i++) {
        ...
        if (vft && vft->print_usage_cb) vft->print_usage_cb();
    }
    fprintf(stderr,
...
static int echo_process_each_proto_opts(unformat_input_t *a)
{
    ...
    for (i = 0; i < vec_len(em->available_proto_cb_vft); i++) {
        ...
        if (vft && vft->process_opts_cb)
            if ((rv = vft->process_opts_cb(a))) return rv;
    }
    return 0;
}
...
static void echo_set_each_proto_defaults_before_opts(echo_main_t *em)
{
    ...
    for (i = 0; i < vec_len(em->available_proto_cb_vft); i++) {
        echo_proto_cb_vft_t *vft = em->available_proto_cb_vft[i];
        if (vft && vft->set_defaults_before_opts_cb) vft->set_defaults_before_opts_cb();
    }
}
...
void echo_process_opts(int argc, char **argv)
{
    ...
    while (unformat_check_input(a) != UNFORMAT_END_OF_INPUT) {
        if (echo_process_each_proto_opts(a))
            ...
            em->appns_flags |= (APP_OPTIONS_FLAGS_USE_GLOBAL_SCOPE | APP_OPTIONS_FLAGS_USE_LOCAL_SCOPE);
            ...
            ;
        else if (unformat(a, "RX=%U", unformat_data_size, &em->bytes_to_receive))
            ;
            ...
            ;
        else if (unformat(a, "sclose=%U", echo_unformat_close, &em->send_stream_disconnects))
            ;
        else if (unformat(a, "time %U:%U", echo_unformat_timing_event, &em->timing.start_event, echo_unformat_timing_event, &em->timing.end_event))
            ;
    ...
    if (em->data_source == ECHO_RX_DATA_SOURCE) em->bytes_to_send = em->bytes_to_receive;

    /* disconnect flags  */
    if (em->i_am_master)
        default_f_active = em->bytes_to_send == 0 ? ECHO_CLOSE_F_ACTIVE : ECHO_CLOSE_F_PASSIVE;
    else
        default_f_active = em->bytes_to_receive == 0 ? ECHO_CLOSE_F_PASSIVE : ECHO_CLOSE_F_ACTIVE;
    if (em->send_stream_disconnects == ECHO_CLOSE_F_INVALID) em->send_stream_disconnects = default_f_active;

    if (em->max_sim_connects == 0) em->max_sim_connects = em->evt_q_size >> 1;
...
static int echo_needs_crypto(echo_main_t *em)
{
    u8 tr = em->uri_elts.transport_proto;
    if (tr == TRANSPORT_PROTO_QUIC || tr == TRANSPORT_PROTO_TLS) return 1;
    return 0;
}
...
void echo_process_uri(echo_main_t *em)
{
    ...
    if (unformat(input, "%U://%U/%d", unformat_transport_proto, &em->uri_elts.transport_proto, unformat_ip4_address, &em->uri_elts.ip.ip4, &port))
...
static int echo_detach(echo_main_t *em)
{
    if (em->use_app_socket_api) return echo_sapi_detach(em);
...
static void echo_add_cert_key(echo_main_t *em)
{
    if (em->use_app_socket_api)
        ...
        if (wait_for_state_change(em, STATE_ATTACHED, TIMEOUT)) {
            ECHO_FAIL(ECHO_FAIL_APP_ATTACH, "Couldn't add crypto context to vpp\n");
            exit(1);
        }
    }
}

static int echo_del_cert_key(echo_main_t *em)
{
    if (em->use_app_socket_api) return echo_sapi_del_cert_key(em);
...
static void echo_disconnect(echo_main_t *em)
{
    if (em->use_app_socket_api) return;
...
static int echo_attach(echo_main_t *em)
{
    if (em->use_app_socket_api)
        ...
        if (wait_for_state_change(em, STATE_ATTACHED_NO_CERT, TIMEOUT)) {
            ECHO_FAIL(ECHO_FAIL_ATTACH_TO_VPP, "Couldn't attach to vpp, did you run <session enable> ?");
            return -1;
        }
    }
    return 0;
...
int main(int argc, char **argv)
{
    ...
    if (!em->proto_cb_vft) {
        ECHO_FAIL(ECHO_FAIL_PROTOCOL_NOT_SUPPORTED, "Protocol %U is not supported", format_transport_proto, em->uri_elts.transport_proto);
        goto exit_on_error;
    }
    if (em->proto_cb_vft->set_defaults_after_opts_cb) em->proto_cb_vft->set_defaults_after_opts_cb();
    ...
    em->app_name = format(0, "%s%c", em->i_am_master ? "echo_server" : "echo_client", 0);
    ...
    if (echo_attach(em)) goto exit_on_error;

    if (echo_needs_crypto(em)) {
        ECHO_LOG(2, "Adding crypto context %U", echo_format_crypto_engine, em->crypto_engine);
    ...
    if (pthread_create(&em->mq_thread_handle, NULL /*attr */, echo_mq_thread_fn, 0)) {
        ECHO_FAIL(ECHO_FAIL_PTHREAD_CREATE, "pthread create errored");
        goto exit_on_error;
    }

    for (i = 0; i < em->n_rx_threads; i++)
        if (pthread_create(&em->data_thread_handles[i], NULL /*attr */, echo_data_thread_fn, (void *) i)) {
            ECHO_FAIL(ECHO_FAIL_PTHREAD_CREATE, "pthread create errored (index %d)", i);
            goto exit_on_error;
        }
    if (em->i_am_master)
        server_run(em);
    ...
    if (echo_needs_crypto(em)) {
        if (echo_del_cert_key(em)) goto exit_on_error;
    }

    if (echo_detach(em)) goto exit_on_error;
