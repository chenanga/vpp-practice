...
void echo_send_listen(echo_main_t *em, ip46_address_t *ip)
{
    ...
    if (echo_transport_needs_crypto(mp->proto)) echo_msg_add_crypto_ext_config(em, &mp->ext_config);
...
void echo_send_disconnect_session(echo_main_t *em, void *args)
{
    ...
    if (!(s = echo_get_session_from_handle(em, mp->handle))) return;
...
int echo_segment_attach(u64 segment_handle, char *name, ssvm_segment_type_t type, int fd)
{
    ...
    if ((rv = fifo_segment_attach(sm, a))) return rv;

    clib_spinlock_lock(&em->segment_handles_lock);
    hash_set(em->shared_segment_handles, segment_handle, a->new_segment_indices[0]);
...
u32 echo_segment_lookup(u64 segment_handle)
{
    ...
    clib_spinlock_unlock(&em->segment_handles_lock);
    if (segment_idxp) return ((u32) *segment_idxp);
...
int echo_attach_session(uword segment_handle, uword rxf_offset, uword txf_offset, uword mq_offset, echo_session_t *s)
{
    ...
    if (fs_index == (u32) ~0) {
        ECHO_LOG(0, "ERROR: segment for session %u is not mounted!", s->session_index);
        return -1;
    }
    ...
    if (mq_offset != (uword) ~0) {
        ...
        s->vpp_evt_q = fifo_segment_msg_q_attach(fs, mq_offset, s->rx_fifo->shr->slice_index);
    }
...
int echo_segment_attach_mq(uword segment_handle, uword mq_offset, u32 mq_index, svm_msg_q_t **mq)
{
    ...
    fs_index = echo_segment_lookup(segment_handle);
    if (fs_index == (u32) ~0) {
        ECHO_LOG(0, "ERROR: mq segment %lx for is not attached!", segment_handle);
        return -1;
    }
...
static void vl_api_app_add_cert_key_pair_reply_t_handler(vl_api_app_add_cert_key_pair_reply_t *mp)
{
    echo_main_t *em = &echo_main;
    if (mp->retval) {
        ECHO_FAIL(ECHO_FAIL_VL_API_CERT_KEY_ADD_REPLY, "Adding cert and key returned %d", clib_net_to_host_u32(mp->retval));
        return;
    }
...
static void vl_api_app_del_cert_key_pair_reply_t_handler(vl_api_app_del_cert_key_pair_reply_t *mp)
{
    echo_main_t *em = &echo_main;
    if (mp->retval) {
        ECHO_FAIL(ECHO_FAIL_VL_API_CERT_KEY_DEL_REPLY, "Delete cert and key returned %d", clib_net_to_host_u32(mp->retval));
        return;
    }
...
static void vl_api_app_attach_reply_t_handler(vl_api_app_attach_reply_t *mp)
{
    ...
    if (mp->retval) {
        ECHO_FAIL(ECHO_FAIL_VL_API_APP_ATTACH, "attach failed: %U", format_api_error, clib_net_to_host_u32(mp->retval));
        return;
    }
    ...
    if (mp->n_fds) {
        ...
        if (vl_socket_client_recv_fd_msg(fds, mp->n_fds, 5)) {
            ECHO_FAIL(ECHO_FAIL_VL_API_RECV_FD_MSG, "vl_socket_client_recv_fd_msg failed");
            goto failed;
        }

        if (mp->fd_flags & SESSION_FD_F_VPP_MQ_SEGMENT)
            if (echo_segment_attach(ECHO_MQ_SEG_HANDLE, 0, SSVM_SEGMENT_MEMFD, fds[n_fds++])) {
                ECHO_FAIL(ECHO_FAIL_VL_API_SVM_FIFO_SEG_ATTACH, "svm_fifo_segment_attach failed on SSVM_SEGMENT_MEMFD");
                goto failed;
            }
        echo_segment_attach_mq(ECHO_MQ_SEG_HANDLE, mp->vpp_ctrl_mq, mp->vpp_ctrl_mq_thread, &em->ctrl_mq);

        if (mp->fd_flags & SESSION_FD_F_MEMFD_SEGMENT) {
            segment_name = vl_api_from_api_to_new_c_string(&mp->segment_name);
            rv           = echo_segment_attach(segment_handle, segment_name, SSVM_SEGMENT_MEMFD, fds[n_fds++]);
            if (rv != 0) {
        ...
        if (mp->fd_flags & SESSION_FD_F_MQ_EVENTFD) svm_msg_q_set_eventfd(em->app_mq, fds[n_fds++]);

        vec_free(fds);
    }
    else {
        segment_name = vl_api_from_api_to_new_c_string(&mp->segment_name);
        rv           = echo_segment_attach(segment_handle, segment_name, SSVM_SEGMENT_SHM, -1);
        if (rv != 0) {
            ECHO_FAIL(ECHO_FAIL_VL_API_SVM_FIFO_SEG_ATTACH,
                      "svm_fifo_segment_attach ('%s') "
...
{
    if (mp->retval) {
        ECHO_FAIL(ECHO_FAIL_VL_API_DETACH_REPLY, "app detach returned with err: %d", mp->retval);
        return;
    }
