...
{
    ECHO_FAIL(ECHO_FAIL_QUIC_WRONG_CONNECT, "Got a wrong connected on session %u [%lx]", session_index, mp->handle);
}
...
{
    ECHO_FAIL(ECHO_FAIL_QUIC_WRONG_ACCEPT, "Got a wrong accept on session 0x%lx S[%u]", mp->handle, session_index);
}
...
static void quic_echo_on_accept_log_ip(session_accepted_msg_t *mp, u32 session_index)
{
    ...
    ECHO_LOG(1, "Accepted session from: %s:%d", ip_str, clib_net_to_host_u16(mp->rmt.port));
}
...
static inline void quic_echo_cleanup_listener(u32 listener_index, echo_main_t *em, quic_echo_proto_main_t *eqm)
{
    echo_session_t *ls;
    ls = pool_elt_at_index(em->sessions, listener_index);
    if (ls->session_type != ECHO_SESSION_TYPE_QUIC) {
        ECHO_LOG(2, "%U: Invalid listener session type", echo_format_session, ls);
        return;
    }
    if (!clib_atomic_sub_fetch(&ls->accepted_session_count, 1)) {
        if (eqm->send_quic_disconnects == ECHO_CLOSE_F_ACTIVE) {
            echo_send_rpc(em, echo_send_disconnect_session, (echo_rpc_args_t *) &ls->vpp_session_handle);
...
static void quic_echo_cleanup_cb(echo_session_t *s, u8 parent_died)
{
    echo_main_t            *em  = &echo_main;
    quic_echo_proto_main_t *eqm = &quic_echo_proto_main;
    if ((em->state == STATE_DATA_DONE) || !(s->session_state < ECHO_SESSION_STATE_CLOSED)) return;
    ECHO_LOG(3, "%U cleanup (parent_died %d)", echo_format_session, s, parent_died);
    s->session_state = ECHO_SESSION_STATE_CLOSED;
    if (s->session_type == ECHO_SESSION_TYPE_QUIC) {
        if (parent_died) clib_atomic_fetch_add(&em->stats.clean_count.q, 1);
    ...
    if (!em->n_clients_connected && !eqm->n_quic_clients_connected) em->state = STATE_DATA_DONE;
    ECHO_LOG(2, "Cleanup sessions (still %uQ %uS): app %U", eqm->n_quic_clients_connected, em->n_clients_connected, echo_format_app_state, em->state);
}
...
static void quic_echo_on_connected(session_connected_msg_t *mp, u32 session_index)
{
    ...
    if (session->listener_index == SESSION_INVALID_INDEX) {
        ...
        if (eqm->cb_vft.quic_connected_cb) eqm->cb_vft.quic_connected_cb(mp, session->session_index);
        clib_atomic_fetch_add(&eqm->n_quic_clients_connected, 1);

        if (em->stats.connected_count.q % LOGGING_BATCH == 0) ECHO_LOG(0, "Connected Q %d / %d", em->stats.connected_count.q, em->n_connects);
    }
    else {
        clib_atomic_fetch_add(&em->stats.connected_count.s, 1);
        listen_session        = pool_elt_at_index(em->sessions, session->listener_index);
        session->session_type = ECHO_SESSION_TYPE_STREAM;
        clib_atomic_fetch_add(&listen_session->accepted_session_count, 1);
        ECHO_LOG(2, "Connected %U -> %U", echo_format_session, session, echo_format_session, listen_session);
        if (em->i_am_master && eqm->cb_vft.server_stream_connected_cb) eqm->cb_vft.server_stream_connected_cb(mp, session->session_index);
        if (!em->i_am_master && eqm->cb_vft.client_stream_connected_cb) eqm->cb_vft.client_stream_connected_cb(mp, session->session_index);
        clib_atomic_fetch_add(&em->n_clients_connected, 1);

        if (em->stats.connected_count.s % LOGGING_BATCH == 0) ECHO_LOG(0, "Connected S %d / %d", em->stats.connected_count.s, em->n_clients);
    }

    if (em->n_clients_connected == em->n_clients && em->n_clients_connected != 0) echo_notify_event(em, ECHO_EVT_LAST_SCONNECTED);

    if (eqm->n_quic_clients_connected == em->n_connects && em->state < STATE_READY) {
        echo_notify_event(em, ECHO_EVT_LAST_QCONNECTED);
        em->state = STATE_READY;
        if (eqm->n_stream_clients == 0) quic_echo_initiate_qsession_close_no_stream(em);
    }
}
...
static void quic_echo_connected_cb(session_connected_bundled_msg_t *mp, u32 session_index, u8 is_failed)
{
    ...
    return quic_echo_on_connected((session_connected_msg_t *) mp, session_index);
}
...
static void quic_echo_accepted_cb(session_accepted_msg_t *mp, echo_session_t *session)
{
    ...
    if (ls->session_type == ECHO_SESSION_TYPE_LISTEN) {
        ...
        session->session_type           = ECHO_SESSION_TYPE_QUIC;
        session->accepted_session_count = 0;
        if (eqm->cb_vft.quic_accepted_cb) eqm->cb_vft.quic_accepted_cb(mp, session->session_index);
        clib_atomic_fetch_add(&eqm->n_quic_clients_connected, 1);

        if (em->stats.accepted_count.q % LOGGING_BATCH == 0) ECHO_LOG(0, "Accepted Q %d / %d", em->stats.accepted_count.q, em->n_connects);
    }
    else {
        ...
        if (em->i_am_master && eqm->cb_vft.server_stream_accepted_cb) eqm->cb_vft.server_stream_accepted_cb(mp, session->session_index);
        if (!em->i_am_master && eqm->cb_vft.client_stream_accepted_cb) eqm->cb_vft.client_stream_accepted_cb(mp, session->session_index);
        clib_atomic_fetch_add(&em->n_clients_connected, 1);

        if (em->stats.accepted_count.s % LOGGING_BATCH == 0) ECHO_LOG(0, "Accepted S %d / %d", em->stats.accepted_count.s, em->n_clients);

        if (em->connect_flag && !(mp->flags & em->connect_flag)) {
            ECHO_FAIL(ECHO_FAIL_UNIDIRECTIONAL, "expected unidirectional streams");
        }
    }
...
static int quic_echo_process_opts_cb(unformat_input_t *a)
{
    ...
    else if (unformat(a, "qclose=%U", echo_unformat_close, &eqm->send_quic_disconnects))
        ;
    else
        return 0;
    return 1;
...
static void quic_echo_set_defaults_after_opts_cb()
{
    ...
    em->n_sessions = clib_max(1, eqm->n_stream_clients) * em->n_clients + em->n_clients + em->n_uris;
    ...
    if (eqm->send_quic_disconnects == ECHO_CLOSE_F_INVALID) eqm->send_quic_disconnects = default_f_active;
}
