...
#define foreach_echo_fail_code                                                                \
    ...
    _(ECHO_FAIL_SESSION_ACCEPTED_BAD_LISTENER, "ECHO_FAIL_SESSION_ACCEPTED_BAD_LISTENER")     \
    ...
    _(ECHO_FAIL_CONNECTED_WAIT_FOR_SEG_ALLOC, "ECHO_FAIL_CONNECTED_WAIT_FOR_SEG_ALLOC")       \
    _(ECHO_FAIL_APP_ATTACH, "ECHO_FAIL_APP_ATTACH")                                           \
    _(ECHO_FAIL_SERVER_DISCONNECT_TIMEOUT, "ECHO_FAIL_SERVER_DISCONNECT_TIMEOUT")             \
    _(ECHO_FAIL_INVALID_URI, "ECHO_FAIL_INVALID_URI")                                         \
    _(ECHO_FAIL_PROTOCOL_NOT_SUPPORTED, "ECHO_FAIL_PROTOCOL_NOT_SUPPORTED")                   \
    ...
    _(ECHO_FAIL_VL_API_MISSING_SEGMENT_NAME, "ECHO_FAIL_VL_API_MISSING_SEGMENT_NAME")         \
    _(ECHO_FAIL_VL_API_NULL_APP_MQ, "ECHO_FAIL_VL_API_NULL_APP_MQ")                           \
    _(ECHO_FAIL_VL_API_RECV_FD_MSG, "ECHO_FAIL_VL_API_RECV_FD_MSG")                           \
    _(ECHO_FAIL_VL_API_SVM_FIFO_SEG_ATTACH, "ECHO_FAIL_VL_API_SVM_FIFO_SEG_ATTACH")           \
    _(ECHO_FAIL_VL_API_FIFO_SEG_ATTACH, "ECHO_FAIL_VL_API_FIFO_SEG_ATTACH")                   \
    ...
    _(ECHO_FAIL_VL_API_CERT_KEY_ADD_REPLY, "ECHO_FAIL_VL_API_CERT_KEY_ADD_REPLY")             \
    _(ECHO_FAIL_VL_API_CERT_KEY_DEL_REPLY, "ECHO_FAIL_VL_API_CERT_KEY_DEL_REPLY")             \
    _(ECHO_FAIL_GET_SESSION_FROM_HANDLE, "ECHO_FAIL_GET_SESSION_FROM_HANDLE")                 \
    ...
    _(ECHO_FAIL_TEST_ASSERT_ALL_SESSIONS_CLOSED, "ECHO_FAIL_TEST_ASSERT_ALL_SESSIONS_CLOSED") \
...
#define CHECK_SAME(fail, expected, result, _fmt, _args...)                                                                                 \
    do {                                                                                                                                   \
        if ((expected) != (result)) ECHO_FAIL((fail), "expected same (%lld, got %lld) : "_fmt, (u64) (expected), (u64) (result), ##_args); \
    } while (0)

#define CHECK_DIFF(fail, expected, result, _fmt, _args...)                                                                 \
    do {                                                                                                                   \
        if ((expected) == (result)) ECHO_FAIL((fail), "expected different (both %lld) : "_fmt, (u64) (expected), ##_args); \
    } while (0)
...
#define ECHO_FAIL(fail, _fmt, _args...)                                                                           \
    do {                                                                                                          \
        ...
        if (vec_len(em->fail_descr))                                                                              \
            em->fail_descr = format(em->fail_descr, " | %s (%u): "_fmt, echo_fail_code_str[fail], fail, ##_args); \
        else                                                                                                      \
            em->fail_descr = format(0, "%s (%u): "_fmt, echo_fail_code_str[fail], fail, ##_args);                 \
        em->time_to_stop = 1;                                                                                     \
        if (em->log_lvl > 0) clib_warning("%v", em->fail_descr);                                                  \
    } while (0)

#define ECHO_LOG(lvl, _fmt, _args...)                       \
    {                                                       \
        echo_main_t *em = &echo_main;                       \
        if (em->log_lvl > lvl) clib_warning(_fmt, ##_args); \
    }

#define ECHO_REGISTER_PROTO(proto, vft)                    \
    static void __clib_constructor vpp_echo_init_##proto() \
    {                                                      \
        echo_main_t *em = &echo_main;                      \
...
typedef enum { ECHO_NO_DATA_SOURCE, ECHO_TEST_DATA_SOURCE, ECHO_RX_DATA_SOURCE, ECHO_INVALID_DATA_SOURCE } data_source_t;
