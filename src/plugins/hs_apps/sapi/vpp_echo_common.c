...
u8 *echo_format_session(u8 *s, va_list *args)
{
    ...
    return format(s, "%U 0x%lx S[%u]", echo_format_session_type, session->session_type, session->vpp_session_handle, session->session_index);
}
...
u8 *echo_format_session_state(u8 *s, va_list *args)
{
    ...
    switch (session_state) {
        ...
        case ECHO_SESSION_STATE_AWAIT_CLOSING:
            return format(s, "ECHO_SESSION_STATE_AWAIT_CLOSING (%u)", session_state);
...
u8 *echo_format_app_state(u8 *s, va_list *args)
{
    ...
    if (state == STATE_START) return format(s, "STATE_START (%u)", state);
    if (state == STATE_ATTACHED) return format(s, "STATE_ATTACHED (%u)", state);
    if (state == STATE_ATTACHED_NO_CERT) return format(s, "STATE_ATTACHED_NO_CERT (%u)", state);
    if (state == STATE_LISTEN) return format(s, "STATE_LISTEN (%u)", state);
    if (state == STATE_READY) return format(s, "STATE_READY (%u)", state);
    if (state == STATE_DATA_DONE) return format(s, "STATE_DATA_DONE (%u)", state);
    if (state == STATE_DISCONNECTED) return format(s, "STATE_DISCONNECTED (%u)", state);
...
u8 *echo_format_timing_event(u8 *s, va_list *args)
{
    ...
    if (timing_event == ECHO_EVT_START) return format(s, "start");
    if (timing_event == ECHO_EVT_FIRST_QCONNECT) return format(s, "qconnect");
    if (timing_event == ECHO_EVT_LAST_QCONNECTED) return format(s, "qconnected");
    if (timing_event == ECHO_EVT_FIRST_SCONNECT) return format(s, "sconnect");
    if (timing_event == ECHO_EVT_LAST_SCONNECTED) return format(s, "sconnected");
    if (timing_event == ECHO_EVT_LAST_BYTE) return format(s, "lastbyte");
...
{
    u32 state = va_arg(*args, u32);
    if (state == CRYPTO_ENGINE_MBEDTLS) return format(s, "mbedtls");
    if (state == CRYPTO_ENGINE_OPENSSL) return format(s, "openssl");
    if (state == CRYPTO_ENGINE_PICOTLS) return format(s, "picotls");
...
echo_session_t *echo_session_new(echo_main_t *em)
{
    ...
    return pool_elt_at_index(em->sessions, clib_atomic_fetch_add(&em->nxt_available_sidx, 1));
}
...
int wait_for_state_change(echo_main_t *em, connection_state_t state, f64 timeout)
{
    ...
    while (!timeout || clib_time_now(&em->clib_time) < end_time) {
        if (em->state == state) return 0;
        if (em->time_to_stop) return 1;
    }
...
{
    if (em->timing.events_sent & e) return;
