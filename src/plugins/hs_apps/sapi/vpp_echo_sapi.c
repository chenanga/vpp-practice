...
static inline u64 echo_vpp_worker_segment_handle(u32 wrk_index)
{
    return (ECHO_INVALID_SEGMENT_HANDLE - wrk_index - 1);
}

static int echo_segment_discover_mqs(uword segment_handle, int *fds, u32 n_fds)
{
...
static int echo_api_attach_reply_handler(app_sapi_attach_reply_msg_t *mp, int *fds)
{
    echo_main_t *em = &echo_main;
    ...
    if (mp->fd_flags & SESSION_FD_F_VPP_MQ_SEGMENT)
        if (echo_segment_attach(echo_vpp_worker_segment_handle(0), "vpp-mq-seg", SSVM_SEGMENT_MEMFD, fds[n_fds_used++])) goto failed;

    if (mp->fd_flags & SESSION_FD_F_MEMFD_SEGMENT) {
        segment_name = format(0, "memfd-%ld%c", segment_handle, 0);
        rv           = echo_segment_attach(segment_handle, (char *) segment_name, SSVM_SEGMENT_MEMFD, fds[n_fds_used++]);
    ...
    echo_segment_discover_mqs(echo_vpp_worker_segment_handle(0), fds + n_fds_used, mp->n_fds - n_fds_used);
    echo_segment_attach_mq(echo_vpp_worker_segment_handle(0), mp->vpp_ctrl_mq, mp->vpp_ctrl_mq_thread, &em->ctrl_mq);
...
static int echo_api_send_attach(clib_socket_t *cs)
{
    echo_main_t           *em  = &echo_main;
    ...
    mp->options[APP_OPTIONS_FLAGS]               = APP_OPTIONS_FLAGS_ACCEPT_REDIRECT | APP_OPTIONS_FLAGS_ADD_SEGMENT;
...
int echo_sapi_attach(echo_main_t *em)
{
    ...
    cs = &em->app_api_sock;
    if (echo_api_send_attach(cs)) return -1;
...
int echo_sapi_recv_fd(echo_main_t *em, int *fds, int n_fds)
{
    ...
    clib_error_t  *err = clib_socket_recvmsg(&em->app_api_sock, msg, sizeof(*msg), fds, n_fds);
    if (err) {
        clib_error_report(err);
        return -1;
    }
