...
static void tcp_echo_connected_cb(session_connected_bundled_msg_t *mp, u32 session_index, u8 is_failed)
{
    ...
    if (is_failed) {
        ECHO_FAIL(ECHO_FAIL_TCP_BAPI_CONNECT, "Bapi connect errored on session %u", session_index);
        return; /* Dont handle bapi connect errors for now */
    }

    ECHO_LOG(2, "Connected session 0x%lx -> URI", ((session_connected_msg_t *) mp)->handle);
