...
static hts_session_t *hts_session_alloc(u32 thread_index)
{
...
static hts_session_t *hts_session_get(u32 thread_index, u32 hts_index)
{
    hts_main_t *htm = &hts_main;
...
static void hts_session_free(hts_session_t *hs)
{
    hts_main_t *htm    = &hts_main;
    u32         thread = hs->thread_index;

    if (htm->debug_level > 0) clib_warning("Freeing session %u", hs->session_index);

    vec_free(hs->rx_buf);

    if (CLIB_DEBUG) clib_memset(hs, 0xfa, sizeof(*hs));

    pool_put(htm->sessions[thread], hs);
}

static void hts_disconnect_transport(hts_session_t *hs)
{
    vnet_disconnect_args_t _a = {0}, *a = &_a;
    hts_main_t            *htm = &hts_main;
    session_t             *ts;

    if (htm->debug_level > 0) clib_warning("Actively closing session %u", hs->session_index);
...
static void hts_session_tx_zc(hts_session_t *hs, session_t *ts)
{
    u32 to_send, space;
    u64 max_send;
    int rv;
    ...
    if (to_send < max_send) svm_fifo_add_want_deq_ntf(ts->tx_fifo, SVM_FIFO_WANT_DEQ_NOTIF);

    if (svm_fifo_set_event(ts->tx_fifo)) session_program_tx_io_evt(ts->handle, SESSION_IO_EVT_TX);
}

static void hts_session_tx_no_zc(hts_session_t *hs, session_t *ts)
{
    ...
    sent = svm_fifo_enqueue_segments(ts->tx_fifo, seg, n_segs, 1 /* allow partial */);
    ...
    if (sent < left) svm_fifo_add_want_deq_ntf(ts->tx_fifo, SVM_FIFO_WANT_DEQ_NOTIF);

    if (svm_fifo_set_event(ts->tx_fifo)) session_program_tx_io_evt(ts->handle, SESSION_IO_EVT_TX);
}

static inline void hts_session_tx(hts_session_t *hs, session_t *ts)
{
    hts_main_t *htm = &hts_main;
    ...
    if (hs->close_threshold > 0) {
        if ((f64) hs->data_offset / hs->data_len > hs->close_threshold) hts_disconnect_transport(hs);
    }
}

static void hts_start_send_data(hts_session_t *hs, http_status_code_t status)
{
    http_msg_t     msg;
    session_t     *ts;
    u8            *headers_buf = 0;
    ...
    ts = session_get(hs->vpp_session_index, hs->thread_index);
    rv = svm_fifo_enqueue_segments(ts->tx_fifo, seg, n_segs, 0 /* allow partial */);
...
static int try_test_file(hts_session_t *hs, u8 *target)
{
    ...
    if (memcmp(target, test_str, clib_strnlen(test_str, 9))) return -1;
    ...
    if (htm->debug_level) clib_warning("Requested file size %U", format_memory_size, file_size);
    ...
    http_add_header(&hs->resp_headers, http_header_name_token(HTTP_HEADER_CONTENT_TYPE), http_content_type_token(HTTP_CONTENT_APP_OCTET_STREAM));
...
static inline void hts_session_rx_body(hts_session_t *hs, session_t *ts)
{
    hts_main_t *htm = &hts_main;
    u32         n_deq;
    int         rv;
    ...
    if (hs->close_threshold > 0) {
        if ((f64) (hs->total_recv - hs->left_recv) / hs->total_recv > hs->close_threshold) hts_disconnect_transport(hs);
    }
...
static int hts_ts_rx_callback(session_t *ts)
{
    hts_main_t    *htm = &hts_main;
    hts_session_t *hs;
    u8            *target = 0;
    ...
    if (hs->left_recv == 0) {
        ...
        if (msg.data.target_path_len == 0 || msg.data.target_form != HTTP_TARGET_ORIGIN_FORM) {
        ...
        if (htm->debug_level) clib_warning("%s request target: %v", msg.method_type == HTTP_REQ_GET ? "GET" : "POST", target);

        if (msg.method_type == HTTP_REQ_GET) {
            if (try_test_file(hs, target)) hts_start_send_data(hs, HTTP_STATUS_NOT_FOUND);
            vec_free(target);
        }
        else {
            vec_free(target);
            ...
            if (htm->no_zc) vec_validate(hs->rx_buf, HTS_RX_BUF_SIZE - 1);
...
static int hs_ts_tx_callback(session_t *ts)
{
    hts_session_t *hs;
...
static int hts_ts_accept_callback(session_t *ts)
{
    hts_main_t    *htm = &hts_main;
    ...
    if (lhs->close_rate) {
        ...
        if ((cnt % lhs->close_rate) == 0) hs->close_threshold = random_f64(&htm->seed);
    }

    if (htm->debug_level > 0) clib_warning("Accepted session %u close threshold %.2f", ts->opaque, hs->close_threshold);

    return 0;
}

static int hts_ts_connected_callback(u32 app_index, u32 api_context, session_t *s, session_error_t err)
{
    clib_warning("called...");
    return -1;
}

static void hts_ts_disconnect_callback(session_t *ts)
{
    hts_main_t            *htm = &hts_main;
    vnet_disconnect_args_t _a = {0}, *a = &_a;

    if (htm->debug_level > 0) clib_warning("Transport closing session %u", ts->opaque);
...
static void hts_ts_reset_callback(session_t *ts)
{
    hts_main_t            *htm = &hts_main;
    vnet_disconnect_args_t _a = {0}, *a = &_a;

    if (htm->debug_level > 0) clib_warning("Transport reset session %u", ts->opaque);
...
static void hts_ts_cleanup_callback(session_t *s, session_cleanup_ntf_t ntf)
{
    hts_session_t *hs;
...
static int hts_add_segment_callback(u32 client_index, u64 segment_handle)
{
    return 0;
}

static int hts_del_segment_callback(u32 client_index, u64 segment_handle)
{
    return 0;
}
...
static int hts_attach(hts_main_t *hm)
{
...
static int hts_transport_needs_crypto(transport_proto_t proto)
{
    return proto == TRANSPORT_PROTO_TLS || proto == TRANSPORT_PROTO_DTLS || proto == TRANSPORT_PROTO_QUIC;
}

static int hts_start_listen(hts_main_t *htm, session_endpoint_cfg_t *sep, u8 *uri, f64 rnd_close)
{
...
static int hts_stop_listen(hts_main_t *htm, u32 hls_index)
{
    hts_session_t *hls;
    session_t     *ls;
...
static clib_error_t *hts_listen(hts_main_t *htm, hts_listen_cfg_t *lcfg)
{
    ...
    if ((rv = hts_start_listen(htm, &sep, uri_key, lcfg->rnd_close))) {
        error = clib_error_return(0, "failed to listen on %v: %U", uri, format_session_error, rv);
    }
...
static int hts_create(vlib_main_t *vm)
{
    ...
    if (htm->no_zc) vec_validate(htm->test_data, (64 << 10) - 1);
...
static clib_error_t *hts_create_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    /* Get a line of input. */
    if (!unformat_user(input, unformat_line_input, line_input)) goto start_server;

    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "private-segment-size %U", unformat_memory_size, &mem_size))
            htm->segment_size = mem_size;
        else if (unformat(line_input, "fifo-size %U", unformat_memory_size, &mem_size))
            ...
            if (lcfg.rnd_close > 1.0) {
                error = clib_error_return(0, "invalid rnd close value %f", lcfg.rnd_close);
                break;
            }
        }
...
VLIB_CLI_COMMAND(http_tps_command, static) = {
    .path = "http tps",
    .short_help =
        "http tps [uri <uri>] [fifo-size <nbytes>] "
        "[segment-size <nMG>] [prealloc-fifos <n>] [debug] [no-zc] "
        "[del]",
    .function = hts_create_command_fn,
};

static clib_error_t *hts_show_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    hts_main_t      *htm          = &hts_main;
    clib_error_t    *error        = 0;
...
no_input:
    ...
    vlib_cli_output(vm, " app index: %u\n listeners: %u\n sesions: %u", htm->app_index, n_listeners, n_sessions - n_listeners);
...
static clib_error_t *hs_main_init(vlib_main_t *vm)
{
    hts_main_t *htm = &hts_main;
