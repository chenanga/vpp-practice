...
static inline char *hs_test_type_str(hs_test_t t)
{
    switch (t) {
...
static inline int hs_test_cfg_verify(hs_test_cfg_t *cfg, hs_test_cfg_t *valid_cfg)
{
    /* Note: txbuf & rxbuf on server are the same buffer,
     *       so txbuf_size is not included in this check.
     */
    return ((cfg->magic == valid_cfg->magic) && (cfg->test == valid_cfg->test) && (cfg->verbose == valid_cfg->verbose)
            && (cfg->rxbuf_size == valid_cfg->rxbuf_size) && (cfg->num_writes == valid_cfg->num_writes)
            && (cfg->total_bytes == valid_cfg->total_bytes));
}

static inline void hs_test_cfg_init(hs_test_cfg_t *cfg)
{
...
static inline char *hs_test_cmd_to_str(int cmd)
{
    switch (cmd) {
...
static inline void hs_test_cfg_dump(hs_test_cfg_t *cfg, uint8_t is_client)
{
    char *spc = "     ";
    ...
    printf("  test config (%p):\n" HS_TEST_SEPARATOR_STRING
           ...
           : is_client && (cfg->test == HS_TEST_TYPE_BI) ? "'" HS_TEST_TOKEN_RUN_BI "'"
                                                         : spc,
           hs_test_type_str(cfg->test),
           cfg->test,
           cfg->ctrl_handle,
           cfg->ctrl_handle,
           is_client ? "'" VCL_TEST_TOKEN_NUM_TEST_SESS "'" : spc,
           cfg->num_test_sessions,
           cfg->num_test_sessions,
...
static inline u16 hs_make_data_port(u16 p)
{
    p = clib_net_to_host_u16(p);
    return clib_host_to_net_u16(p + 1);
}

static inline void hs_test_app_session_init_(app_session_t *as, session_t *s)
{
...
#define hs_test_app_session_init(_as, _s) hs_test_app_session_init_((app_session_t *) (_as), (_s))
