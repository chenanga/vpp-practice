...
static void proxy_cb_fn(void *data, u32 data_len)
{
    ...
    vnet_connect(&a);
    if (a.sep_ext.ext_cfg) clib_mem_free(a.sep_ext.ext_cfg);
}
...
static proxy_session_t *proxy_session_alloc(void)
{
...
static inline proxy_session_t *proxy_session_get(u32 ps_index)
{
    proxy_main_t *pm = &proxy_main;
...
static inline proxy_session_t *proxy_session_get_if_valid(u32 ps_index)
{
    proxy_main_t *pm = &proxy_main;

    if (pool_is_free_index(pm->sessions, ps_index)) return 0;
    return pool_elt_at_index(pm->sessions, ps_index);
}
...
static void proxy_session_free(proxy_session_t *ps)
{
    ...
    if (CLIB_DEBUG > 0) clib_memset(ps, 0xFE, sizeof(*ps));
    pool_put(pm->sessions, ps);
}

static int proxy_session_postponed_free_rpc(void *arg)
{
...
static void proxy_session_postponed_free(proxy_session_t *ps)
{
    session_send_rpc_evt_to_thread(ps->po_thread_index, proxy_session_postponed_free_rpc, uword_to_pointer(ps->ps_index, void *));
}
...
static void proxy_try_delete_session(session_t *s, u8 is_active_open)
{
    ...
    if (is_active_open) {
        ...
        if (ps->vpp_active_open_handle == SESSION_INVALID_HANDLE) {
            if (!ps->active_open_establishing) proxy_session_free(ps);
        }
    }
    clib_spinlock_unlock_if_init(&pm->sessions_lock);
}
...
static int common_fifo_tuning_callback(session_t *s, svm_fifo_t *f, session_ft_action_t act, u32 bytes)
{
    ...
    if (act == SESSION_FT_ACTION_ENQUEUED) {
        ...
        if (update_size) svm_fifo_set_size(f, fifo_size + update_size);
    }
    else /* dequeued */
    {
...
static int proxy_rx_callback(session_t *s)
{
    ...
    if (PREDICT_TRUE(ps->vpp_active_open_handle != SESSION_INVALID_HANDLE)) {
        ...
        if (svm_fifo_set_event(ao_tx_fifo)) {
            ...
            if (session_send_io_evt_to_thread_custom(&ao_session_index, ao_thread_index, SESSION_IO_EVT_TX)) clib_warning("failed to enqueue tx evt");
        }

        if (svm_fifo_max_enqueue(ao_tx_fifo) <= TCP_MSS) svm_fifo_add_want_deq_ntf(ao_tx_fifo, SVM_FIFO_WANT_DEQ_NOTIF);
    }
    else {
        ...
        actual_transfer = svm_fifo_peek(rx_fifo, 0 /* relative_offset */, max_dequeue, pm->rx_buf[thread_index]);
...
static void proxy_force_ack(void *handlep)
{
    ...
    if (session_get_transport_proto(s) != TRANSPORT_PROTO_TCP) return;
...
static int proxy_tx_callback(session_t *proxy_s)
{
    ...
    if (ps->vpp_active_open_handle == SESSION_INVALID_HANDLE) goto unlock;
    ...
    session_send_rpc_evt_to_thread(ps->server_rx_fifo->master_thread_index, proxy_force_ack, arg);
...
static int active_open_alloc_session_fifos(session_t *s)
{
    proxy_main_t    *pm = &proxy_main;
    svm_fifo_t      *rxf, *txf;
    proxy_session_t *ps;
...
static int active_open_rx_callback(session_t *s)
{
    ...
    if (svm_fifo_set_event(proxy_tx_fifo)) {
        ...
        return session_send_io_evt_to_thread_custom(&session_index, thread_index, SESSION_IO_EVT_TX);
    }

    if (svm_fifo_max_enqueue(proxy_tx_fifo) <= TCP_MSS) svm_fifo_add_want_deq_ntf(proxy_tx_fifo, SVM_FIFO_WANT_DEQ_NOTIF);
...
static int active_open_tx_callback(session_t *ao_s)
{
    ...
    if (ps->vpp_server_handle == SESSION_INVALID_HANDLE) goto unlock;

    /* Force ack on proxy side to update rcv wnd */
    void *arg = uword_to_pointer(ps->vpp_server_handle, void *);
    session_send_rpc_evt_to_thread(session_thread_from_handle(ps->vpp_server_handle), proxy_force_ack, arg);
...
static int proxy_server_attach()
{
    ...
    a->options[APP_OPTIONS_PREALLOC_FIFO_PAIRS]   = pm->prealloc_fifos ? pm->prealloc_fifos : 0;
...
static int active_open_attach(void)
{
    ...
    options[APP_OPTIONS_PREALLOC_FIFO_PAIRS]   = pm->prealloc_fifos ? pm->prealloc_fifos : 0;

    options[APP_OPTIONS_FLAGS] = APP_OPTIONS_FLAGS_IS_BUILTIN | APP_OPTIONS_FLAGS_IS_PROXY;

    a->options = options;

    if (vnet_application_attach(a)) return -1;
...
static clib_error_t *proxy_server_create_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (vlib_num_workers()) clib_spinlock_init(&pm->sessions_lock);

    if (!unformat_user(input, unformat_line_input, line_input)) return 0;

    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "fifo-size %U", unformat_memory_size, &pm->fifo_size))
            ;
        else if (unformat(line_input, "max-fifo-size %U", unformat_memory_size, &pm->max_fifo_size))
            ;
            ...
            ;
        else if (unformat(line_input, "private-segment-count %d", &pm->private_segment_count))
            ;
        else if (unformat(line_input, "private-segment-size %U", unformat_memory_size, &tmp64)) {
    ...
    if (!server_uri) {
        clib_warning("No server-uri provided, Using default: %s", default_server_uri);
        server_uri = format(0, "%s%c", default_server_uri, 0);
    }
    if (!client_uri) {
        clib_warning("No client-uri provided, Using default: %s", default_client_uri);
