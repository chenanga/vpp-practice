...
static uword test_builtins_timer_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
...
static void send_data_to_hss(hss_session_handle_t sh, u8 *data)
{
    tb_main_t             *tbm  = &tb_main;
...
static hss_url_handler_rc_t handle_get_test1(hss_url_handler_args_t *args)
{
    u8 *data;
...
static hss_url_handler_rc_t handle_get_test2(hss_url_handler_args_t *args)
{
    u8 *data;
...
static void delayed_resp_cb(u32 *expired_timers)
{
    tb_main_t      *tbm = &tb_main;
    int             i;
    u32             pool_index;
...
static hss_url_handler_rc_t handle_get_test_delayed(hss_url_handler_args_t *args)
{
    tb_main_t      *tbm = &tb_main;
    tw_timer_elt_t *e;
    ...
    e->sh                = args->sh;
    e->stop_timer_handle = tw_timer_start_2t_1w_2048sl(&tbm->tw, e - tbm->delayed_resps, 0, 5);

    return HSS_URL_HANDLER_ASYNC;
}

static hss_url_handler_rc_t handle_post_test3(hss_url_handler_args_t *args)
{
    send_data_to_hss(args->sh, 0);
    return HSS_URL_HANDLER_ASYNC;
}

static void test_builtins_init(vlib_main_t *vm)
{
    tb_main_t          *tbm = &tb_main;
    hss_register_url_fn fp;
    vlib_node_t        *n;

    fp = vlib_get_plugin_symbol("http_static_plugin.so", "hss_register_url_handler");
    ...
    tbm->send_data = vlib_get_plugin_symbol("http_static_plugin.so", "hss_session_send_data");

    tw_timer_wheel_init_2t_1w_2048sl(&tbm->tw, delayed_resp_cb, 1.0, ~0);

    vlib_node_set_state(vm, test_builtins_timer_process_node.index, VLIB_NODE_STATE_POLLING);
    n = vlib_get_node(vm, test_builtins_timer_process_node.index);
    vlib_start_process(vm, n->runtime_index);
}

static clib_error_t *test_builtins_enable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
