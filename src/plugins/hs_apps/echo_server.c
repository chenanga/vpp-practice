...
#define es_dbg(_fmt, _args...)                                                  \
    do {                                                                        \
        if (PREDICT_FALSE(echo_server_main.cfg.verbose)) es_err(_fmt, ##_args); \
    } while (0)

#define es_cli(_fmt, _args...) vlib_cli_output(vm, _fmt, ##_args)

static inline es_worker_t *es_worker_get(u32 thread_index)
{
    return vec_elt_at_index(echo_server_main.wrk, thread_index);
}

static inline es_session_t *es_session_alloc(es_worker_t *wrk)
{
    es_session_t *es;
...
static inline es_session_t *es_session_get(es_worker_t *wrk, u32 es_index)
{
...
static int echo_server_ctrl_session_accept_callback(session_t *s)
{
    s->session_state = SESSION_STATE_READY;
    return 0;
}

static void es_session_alloc_and_init(session_t *s)
{
    es_session_t *es;
...
int quic_echo_server_session_accept_callback(session_t *s)
{
    ...
    if (PREDICT_FALSE(esm->ctrl_listener_handle == s->listener_handle)) return echo_server_ctrl_session_accept_callback(s);

    if (s->listener_handle == esm->listener_handle) return quic_echo_server_qsession_accept_callback(s);
...
static void es_foreach_thread(void *fp)
{
    echo_server_main_t *esm = &echo_server_main;
    uword               thread_index;
...
static int es_wrk_prealloc_sessions(void *args)
{
    echo_server_main_t *esm = &echo_server_main;
...
static int echo_server_setup_test(hs_test_cfg_t *c)
{
...
static void echo_server_ctrl_reply(session_t *s)
{
    echo_server_main_t *esm = &echo_server_main;
    int                 rv;
    ...
    session_send_io_evt_to_thread_custom(&s->session_index, s->thread_index, SESSION_IO_EVT_TX);
}

static int es_test_cmd_sync(echo_server_main_t *esm, session_t *s)
{
    int rv;

    rv = echo_server_setup_test(&esm->cfg);
    if (rv) es_err("setup test error!");

    echo_server_ctrl_reply(s);
    return 0;
}

static int es_wrk_cleanup_sessions(void *args)
{
    echo_server_main_t    *esm = &echo_server_main;
...
static int echo_server_rx_ctrl_callback(session_t *s)
{
    echo_server_main_t *esm = &echo_server_main;
    int                 rv;
    ...
    if (esm->cfg.verbose) hs_test_cfg_dump(&esm->cfg, 0);
...
{
    echo_server_main_t *esm = &echo_server_main;
    if (PREDICT_FALSE(esm->ctrl_listener_handle == s->listener_handle)) return echo_server_rx_ctrl_callback(s);
...
static void es_test_bytes(es_worker_t *wrk, es_session_t *es, int actual_transfer)
{
    int i;
    for (i = 0; i < actual_transfer; i++) {
        if (wrk->rx_buf[i] != ((es->byte_index + i) & 0xff)) {
            es_err("at %lld expected %d got %d", es->byte_index + i, (es->byte_index + i) & 0xff, wrk->rx_buf[i]);
        }
    }
...
int echo_server_rx_callback(session_t *s)
{
    ...
    if (PREDICT_FALSE(max_dequeue == 0)) return 0;
    ...
    rx_event:
        ...
        if (svm_fifo_set_event(rx_fifo)) {
            ...
            if (session_send_io_evt_to_thread(rx_fifo, SESSION_IO_EVT_BUILTIN_RX)) es_err("failed to enqueue self-tap");

            if (es->rx_retries == 500000) {
                es_err("session stuck: %U", format_session, s, 2);
            }
            if (es->rx_retries < 500001) es->rx_retries++;
        }
    ...
    if (n_written != max_transfer) es_err("short trout! written %u read %u", n_written, max_transfer);

    if (PREDICT_FALSE(svm_fifo_max_dequeue_cons(rx_fifo))) goto rx_event;

    return 0;
}

int echo_server_rx_callback_common(session_t *s)
{
    echo_server_main_t *esm = &echo_server_main;
    return esm->rx_callback(s);
}

static session_cb_vft_t echo_server_session_cb_vft = {.session_accept_callback     = echo_server_session_accept_callback,
...
static int echo_server_attach(u8 *appns_id, u64 appns_flags, u64 appns_secret)
{
    ...
    if (esm->transport_proto == TRANSPORT_PROTO_QUIC) echo_server_session_cb_vft.session_accept_callback = quic_echo_server_session_accept_callback;
    ...
    a->options[APP_OPTIONS_PREALLOC_FIFO_PAIRS]   = esm->prealloc_fifos ? esm->prealloc_fifos : 1;
...
static int echo_server_listen_ctrl()
{
    echo_server_main_t *esm   = &echo_server_main;
    vnet_listen_args_t  _args = {}, *args = &_args;
    session_error_t     rv;

    if ((rv = parse_uri(esm->server_uri, &args->sep_ext))) return -1;
...
static int echo_server_listen()
{
    ...
    if (echo_client_transport_needs_crypto(args->sep_ext.transport_proto)) {
        session_endpoint_alloc_ext_cfg(&args->sep_ext, TRANSPORT_ENDPT_EXT_CFG_CRYPTO);
    ...
    if (args->sep_ext.ext_cfg) clib_mem_free(args->sep_ext.ext_cfg);
    return rv;
}
...
static int echo_server_create(vlib_main_t *vm, u8 *appns_id, u64 appns_flags, u64 appns_secret)
{
    ...
    if (echo_server_listen_ctrl()) {
        ...
        if (echo_server_detach()) es_err("failed to detach");
        return -1;
    }
...
static clib_error_t *echo_server_create_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "uri %s", &esm->server_uri))
            server_uri_set = 1;
        else if (unformat(input, "fifo-size %U", unformat_memory_size, &esm->fifo_size))
            ;
        else if (unformat(input, "rcv-buf-size %d", &esm->rcv_buffer_size))
            ;
        else if (unformat(input, "prealloc-fifos %d", &esm->prealloc_fifos))
            ;
        else if (unformat(input, "private-segment-count %d", &esm->private_segment_count))
            ;
        else if (unformat(input, "private-segment-size %U", unformat_memory_size, &esm->private_segment_size))
            ;
