...
#define ec_dbg(_fmt, _args...)                          \
    do {                                                \
        if (ec_main.cfg.verbose) ec_err(_fmt, ##_args); \
    } while (0)

#define ec_cli(_fmt, _args...) vlib_cli_output(vm, _fmt, ##_args)

static void signal_evt_to_cli_i(void *codep)
{
...
static void signal_evt_to_cli(int code)
{
    if (vlib_get_thread_index() != 0)
        session_send_rpc_evt_to_thread_force(0, signal_evt_to_cli_i, uword_to_pointer((uword) code, void *));
    else
        signal_evt_to_cli_i(uword_to_pointer((uword) code, void *));
}

static inline ec_worker_t *ec_worker_get(u32 thread_index)
{
    return vec_elt_at_index(ec_main.wrk, thread_index);
}

static inline ec_session_t *ec_session_alloc(ec_worker_t *wrk)
{
    ec_session_t *ecs;
...
static inline ec_session_t *ec_session_get(ec_worker_t *wrk, u32 ec_index)
{
    return pool_elt_at_index(wrk->sessions, ec_index);
}

static void send_data_chunk(ec_main_t *ecm, ec_session_t *es)
{
    ...
    bytes_this_chunk = clib_min(test_buf_len - test_buf_offset, es->bytes_to_send);
    ...
    if (!es->is_dgram) {
        if (ecm->no_copy) {
            ...
            session_send_io_evt_to_thread_custom(&es->vpp_session_index, es->thread_index, SESSION_IO_EVT_TX);
        }
        else
            rv = app_send_stream((app_session_t *) es, test_data + test_buf_offset, bytes_this_chunk, 0);
    }
    else {
        svm_fifo_t *f           = es->tx_fifo;
        u32         max_enqueue = svm_fifo_max_enqueue_prod(f);

        if (max_enqueue < sizeof(session_dgram_hdr_t)) return;
        ...
        if (ecm->no_copy) {
            ...
            rv               = app_send_dgram((app_session_t *) es, test_data + test_buf_offset, bytes_this_chunk, 0);
        }
    }
...
static void receive_data_chunk(ec_worker_t *wrk, ec_session_t *es)
{
    ec_main_t  *ecm     = &ec_main;
    ...
    if (ecm->cfg.test_bytes) {
        n_read = app_recv((app_session_t *) es, wrk->rx_buf, vec_len(wrk->rx_buf));
    }
    else {
        ...
        if (ecm->cfg.test_bytes) {
            for (i = 0; i < n_read; i++) {
                if (wrk->rx_buf[i] != ((es->bytes_received + i) & 0xff)) {
                    ec_err("read %d error at byte %lld, 0x%x not 0x%x",
                           n_read,
                           es->bytes_received + i,
                           wrk->rx_buf[i],
...
static uword ec_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    if (ecm->run_test != EC_RUNNING) return 0;

    wrk              = ec_worker_get(thread_index);
    conn_indices     = wrk->conn_indices;
    conns_this_batch = wrk->conns_this_batch;

    if (((vec_len(conn_indices) == 0) && vec_len(conns_this_batch) == 0)) return 0;

    /* Grab another pile of connections */
    if (PREDICT_FALSE(vec_len(conns_this_batch) == 0)) {
        nconns_this_batch = clib_min(ecm->connections_per_batch, vec_len(conn_indices));

        ASSERT(nconns_this_batch > 0);
        vec_validate(conns_this_batch, nconns_this_batch - 1);
        clib_memcpy_fast(conns_this_batch, conn_indices + vec_len(conn_indices) - nconns_this_batch, nconns_this_batch * sizeof(u32));
     ...
     * Track progress
     */
    if (PREDICT_FALSE(ecm->prev_conns != ecm->connections_per_batch && ecm->prev_conns == vec_len(conns_this_batch))) {
        ecm->repeats++;
...
static void ec_reset_runtime_config(ec_main_t *ecm)
{
...
static int ec_init(vlib_main_t *vm)
{
    ec_main_t   *ecm = &ec_main;
    ec_worker_t *wrk;
    u32          num_threads;
    ...
    vec_foreach (wrk, ecm->wrk) {
        ...
        wrk->vpp_event_queue = session_main_get_vpp_event_queue(wrk->thread_index);
    }
    ...
    foreach_vlib_main ()
        vlib_node_set_state(this_vlib_main, echo_clients_node.index, VLIB_NODE_STATE_POLLING);
...
static void ec_prealloc_sessions(ec_main_t *ecm)
{
...
static void ec_worker_cleanup(ec_worker_t *wrk)
{
...
static void ec_cleanup(ec_main_t *ecm)
{
    ec_worker_t *wrk;
...
static int quic_ec_qsession_connected_callback(u32 app_index, u32 api_context, session_t *s, session_error_t err)
{
    ...
    if (parse_uri(a->uri, &sep)) return -1;
...
static int ec_ctrl_send(hs_test_cmd_t cmd)
{
    ec_main_t *ecm = &ec_main;
    session_t *s;
    int        rv;
    ...
    if (ecm->cfg.verbose) hs_test_cfg_dump(&ecm->cfg, 1);
...
static int ec_ctrl_session_connected_callback(session_t *s)
{
    ec_main_t *ecm = &ec_main;
...
static int quic_ec_session_connected_callback(u32 app_index, u32 api_context, session_t *s, session_error_t err)
{
    ec_main_t    *ecm = &ec_main;
    ec_session_t *es;
    ec_worker_t  *wrk;
    ...
    if (PREDICT_FALSE(api_context == HS_CTRL_HANDLE)) return ec_ctrl_session_connected_callback(s);

    if (PREDICT_FALSE(ecm->run_test != EC_STARTING)) return -1;
    ...
    if (s->listener_handle == SESSION_INVALID_HANDLE) return quic_ec_qsession_connected_callback(app_index, api_context, s, err);
    ec_dbg("STREAM Connection callback %d", api_context);

    thread_index = s->thread_index;
    ASSERT(thread_index == vlib_get_thread_index() || session_transport_service_type(s) == TRANSPORT_SERVICE_CL);
...
static int ec_session_connected_callback(u32 app_index, u32 api_context, session_t *s, session_error_t err)
{
    ec_main_t    *ecm = &ec_main;
    ec_session_t *es;
    u32           thread_index;
    ...
    if (err) {
        ec_err("connection %d failed! %U", api_context, format_session_error, err);
        ecm->run_test = EC_EXITING;
        signal_evt_to_cli(EC_CLI_CONNECTS_FAILED);
        return 0;
    }
...
static void ec_session_reset_callback(session_t *s)
{
    ec_main_t             *ecm = &ec_main;
    vnet_disconnect_args_t _a = {0}, *a = &_a;

    if (s->session_state == SESSION_STATE_READY) ec_err("Reset active connection %U", format_session, s, 2);
...
static int ec_session_accept_callback(session_t *s)
{
    return 0;
}

static void ec_session_disconnect_callback(session_t *s)
{
    ec_main_t             *ecm = &ec_main;
    vnet_disconnect_args_t _a = {0}, *a = &_a;
...
void ec_session_disconnect(session_t *s)
{
    ec_main_t             *ecm = &ec_main;
    vnet_disconnect_args_t _a = {0}, *a = &_a;
    a->handle    = session_handle(s);
...
static int ec_ctrl_session_rx_callback(session_t *s)
{
    ec_main_t    *ecm = &ec_main;
    int           rx_bytes;
    hs_test_cfg_t cfg = {0};
    ...
    if (ecm->cfg.verbose) hs_test_cfg_dump(&cfg, 1);
...
static int ec_session_rx_callback(session_t *s)
{
    ec_main_t    *ecm = &ec_main;
    ec_worker_t  *wrk;
    ec_session_t *es;

    if (PREDICT_FALSE(s->opaque == HS_CTRL_HANDLE)) return ec_ctrl_session_rx_callback(s);
    ...
    if (svm_fifo_max_dequeue_cons(s->rx_fifo)) session_enqueue_notify(s);

    return 0;
}

static int ec_add_segment_callback(u32 app_index, u64 segment_handle)
{
    /* New segments may be added */
    return 0;
}

static int ec_del_segment_callback(u32 app_index, u64 segment_handle)
{
    return 0;
}
...
static clib_error_t *ec_attach()
{
    ...
    if (ecm->transport_proto == TRANSPORT_PROTO_QUIC) ec_cb_vft.session_connected_callback = quic_ec_session_connected_callback;
    ...
    if ((rv = vnet_application_attach(a))) return clib_error_return(0, "attach returned %d", rv);
...
static int ec_detach()
{
    ec_main_t             *ecm = &ec_main;
    vnet_app_detach_args_t _da, *da = &_da;
    int                    rv;

    if (!ecm->test_client_attached) return 0;
...
static int ec_transport_needs_crypto(transport_proto_t proto)
{
    return proto == TRANSPORT_PROTO_TLS || proto == TRANSPORT_PROTO_DTLS || proto == TRANSPORT_PROTO_QUIC;
}

static int ec_connect_rpc(void *args)
{
    ec_main_t          *ecm = &ec_main;
    vnet_connect_args_t _a = {}, *a = &_a;
    int                 rv, needs_crypto;
    ...
    while (ci < n_clients) {
        ...
        a->api_context = ci;
        if (needs_crypto) {
            session_endpoint_alloc_ext_cfg(&a->sep_ext, TRANSPORT_ENDPT_EXT_CFG_CRYPTO);
            a->sep_ext.ext_cfg->crypto.ckpair_index = ecm->ckpair_index;
        }

        rv = vnet_connect(a);

        if (needs_crypto) clib_mem_free(a->sep_ext.ext_cfg);
    ...
    if (ci < ecm->expected_connections && ecm->run_test != EC_EXITING) ec_program_connects();
...
{
    session_send_rpc_evt_to_thread_force(transport_cl_thread(), ec_connect_rpc, 0);
}

static clib_error_t *ec_ctrl_connect_rpc()
{
    session_error_t     rv;
    ec_main_t          *ecm = &ec_main;
    vnet_connect_args_t _a = {}, *a = &_a;
...
static void ec_ctrl_connect(void)
{
    session_send_rpc_evt_to_thread_force(transport_cl_thread(), ec_ctrl_connect_rpc, 0);
}

static void ec_ctrl_session_disconnect()
{
    ec_main_t             *ecm = &ec_main;
    vnet_disconnect_args_t _a, *a = &_a;
    session_error_t        err;
    ...
    if (err) ec_err("vnet_disconnect_session: %U", format_session_error, err);
}

static int ec_ctrl_test_sync()
{
    ec_main_t *ecm = &ec_main;
...
static int ec_ctrl_test_start()
{
    return ec_ctrl_send(HS_TEST_CMD_START);
}

static int ec_ctrl_test_stop()
{
...
#define ec_wait_for_signal(_sig)                                                      \
    ...
    switch (event_type) {                                                             \
        case ~0:                                                                      \
            ...
            error = clib_error_return(0, "failed: timeout while waiting for " #_sig); \
            goto cleanup;                                                             \
        case _sig:                                                                    \
            break;                                                                    \
        default:                                                                      \
            ec_cli("unexpected event while waiting for " #_sig ": %d", event_type);   \
            error = clib_error_return(0, "failed: unexpected event: %d", event_type); \
            goto cleanup;                                                             \
    }

static clib_error_t *ec_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (ecm->test_client_attached) return clib_error_return(0, "failed: already running!");
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "uri %s", &ecm->connect_uri))
        ...
        else if (unformat(line_input, "bytes %U", unformat_memory_size, &ecm->bytes_to_send))
            ;
            ...
            ecm->echo_bytes = 1;
        else if (unformat(line_input, "fifo-size %U", unformat_memory_size, &ecm->fifo_size))
            ;
        else if (unformat(line_input, "private-segment-count %d", &ecm->private_segment_count))
            ;
        else if (unformat(line_input, "private-segment-size %U", unformat_memory_size, &ecm->private_segment_size))
            ;
            ...
            ecm->attach_flags |= (APP_OPTIONS_FLAGS_USE_GLOBAL_SCOPE | APP_OPTIONS_FLAGS_USE_LOCAL_SCOPE);
            ...
            error = clib_error_return(0, "failed: unknown input `%U'", format_unformat_error, line_input);
            goto cleanup;
        }
    }
...
parse_config:
    ...
    ecm->cfg.num_test_sessions = ecm->expected_connections = ecm->n_clients * ecm->quic_streams;
    ...
    if (ecm->prealloc_sessions) ec_prealloc_sessions(ecm);
    ...
    switch (event_type) {
        case ~0:
            ec_cli("Timeout with only %d sessions active...", ecm->ready_connections);
            error = clib_error_return(0, "failed: syn timeout with %d sessions", ecm->ready_connections);
            goto stop_test;

        case EC_CLI_CONNECTS_DONE:
            delta = vlib_time_now(vm) - ecm->syn_start_time;
            if (delta != 0.0) ec_cli("%d three-way handshakes in %.2f seconds %.2f/s", ecm->n_clients, delta, ((f64) ecm->n_clients) / delta);
            break;
        ...
        default:
            ...
            error = clib_error_return(0, "failed: unexpected event(2): %d", event_type);
            goto stop_test;
    }
    ...
    switch (event_type) {
        case ~0:
            ec_cli("Timeout at %.6f with %d sessions still active...", vlib_time_now(ecm->vlib_main), ecm->ready_connections);
            error = clib_error_return(0, "failed: timeout with %d sessions", ecm->ready_connections);
            goto stop_test;
        ...
        default:
            ...
            error = clib_error_return(0, "failed: unexpected event(3): %d", event_type);
            goto stop_test;
    }
    ...
    ec_cli("%lld bytes (%lld mbytes, %lld gbytes) in %.2f seconds", total_bytes, total_bytes / (1ULL << 20), total_bytes / (1ULL << 30), delta);
    ec_cli("%.2f bytes/second %s", ((f64) total_bytes) / (delta), transfer_type);
    ec_cli("%.4f gbit/second %s", (((f64) total_bytes * 8.0) / delta / 1e9), transfer_type);

    if (ecm->cfg.test_bytes && ecm->test_failed) error = clib_error_return(0, "failed: test bytes");
...
cleanup:
    ...
    ec_cleanup(ecm);
    if (had_config) unformat_free(line_input);

    if (error) ec_cli("test failed");
...
clib_error_t *ec_main_init(vlib_main_t *vm)
{
    ec_main_t *ecm   = &ec_main;
    ecm->app_is_init = 0;
    return 0;
