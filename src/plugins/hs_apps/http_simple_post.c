...
static inline hsp_worker_t *hsp_worker_get(u32 thread_index)
{
    return &hsp_main.wrk[thread_index];
}

static inline hsp_session_t *hsp_session_get(u32 session_index, u32 thread_index)
{
    hsp_worker_t *wrk = hsp_worker_get(thread_index);
    return pool_elt_at_index(wrk->sessions, session_index);
}

static hsp_session_t *hsp_session_alloc(hsp_worker_t *wrk)
{
    hsp_session_t *s;
...
static int hsp_session_connected_callback(u32 app_index, u32 hsp_session_index, session_t *s, session_error_t err)
{
    ...
    if (err) {
        clib_warning("hsp_session_index[%d] connected error: %U", hsp_session_index, format_session_error, err);
        vlib_process_signal_event_mt(hspm->vlib_main, hspm->cli_node_index, HSP_CONNECT_FAILED, 0);
        return -1;
    }
    ...
    if (hspm->is_file) {
        http_add_header(&headers, http_header_name_token(HTTP_HEADER_CONTENT_TYPE), http_content_type_token(HTTP_CONTENT_APP_OCTET_STREAM));
    }
    else {
        http_add_header(&headers, http_header_name_token(HTTP_HEADER_CONTENT_TYPE), http_content_type_token(HTTP_CONTENT_APP_X_WWW_FORM_URLENCODED));
    }
    ...
    rv = svm_fifo_enqueue(s->tx_fifo, vec_len(hspm->headers_buf), hspm->headers_buf);
    ...
    if (svm_fifo_set_event(s->tx_fifo)) session_program_tx_io_evt(s->handle, SESSION_IO_EVT_TX);

    return 0;
}

static void hsp_session_disconnect_callback(session_t *s)
{
    hsp_main_t            *hspm = &hsp_main;
    ...
    if ((rv = vnet_disconnect_session(a))) clib_warning("warning: disconnect returned: %U", format_session_error, rv);
}

static void hsp_session_transport_closed_callback(session_t *s)
{
    hsp_main_t *hspm = &hsp_main;

    vlib_process_signal_event_mt(hspm->vlib_main, hspm->cli_node_index, HSP_TRANSPORT_CLOSED, 0);
}

static void hsp_session_reset_callback(session_t *s)
{
    hsp_main_t            *hspm = &hsp_main;
...
static int hsp_rx_callback(session_t *s)
{
    hsp_main_t    *hspm = &hsp_main;
    hsp_session_t *hsp_session;
    http_msg_t     msg;
    ...
    vlib_process_signal_event_mt(hspm->vlib_main, hspm->cli_node_index, HSP_REPLY_RECEIVED, 0);
    return 0;
}

static int hsp_tx_callback(session_t *s)
{
    hsp_main_t *hspm = &hsp_main;
    u64         to_send;
    u32         n_enq;
...
static clib_error_t *hsp_attach()
{
    hsp_main_t            *hspm = &hsp_main;
    ...
    if ((rv = vnet_application_attach(a))) return clib_error_return(0, "attach returned: %U", format_session_error, rv);
...
static int hsp_connect_rpc(void *rpc_args)
{
...
static void hsp_connect()
{
    ...
    session_send_rpc_evt_to_thread_force(transport_cl_thread(), hsp_connect_rpc, a);
}

static clib_error_t *hsp_run(vlib_main_t *vm)
{
    hsp_main_t         *hspm = &hsp_main;
    ...
    if ((err = hsp_attach())) return clib_error_return(0, "http simple post attach: %U", format_clib_error, err);
...
static int hsp_detach()
{
    hsp_main_t            *hspm = &hsp_main;
    vnet_app_detach_args_t _da, *da = &_da;
    int                    rv;

    if (!hspm->attached) return 0;
...
static void hcc_worker_cleanup(hsp_worker_t *wrk)
{
    pool_free(wrk->sessions);
}

static void hsp_cleanup()
{
    hsp_main_t   *hspm = &hsp_main;
    hsp_worker_t *wrk;
...
static clib_error_t *hsp_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    hsp_main_t      *hspm = &hsp_main;
    clib_error_t    *err  = 0;
    ...
    if (hspm->attached) return clib_error_return(0, "failed: already running!");

    hspm->use_ptr = 0;

    /* Get a line of input. */
    if (!unformat_user(input, unformat_line_input, line_input)) return clib_error_return(0, "expected required arguments");
    ...
    if ((rv = parse_uri((char *) hspm->uri, &hspm->connect_sep))) {
        err = clib_error_return(0, "URI parse error: %U", format_session_error, rv);
        goto done;
    }
    ...
    hspm->cli_node_index = vlib_get_current_process(vm)->node_runtime.node_index;
    ...
    if ((rv = hsp_detach())) {
        ...
        if (!err)
            err = clib_error_return(0, "detach returned: %U", format_session_error, rv);
        else
            clib_warning("warning: detach returned: %U", format_session_error, rv);
    }
...
VLIB_CLI_COMMAND(hsp_command, static) = {
    .path = "http post",
    .short_help =
        "uri http://<ip-addr> target <origin-form> "
        "[data <form-urlencoded> | file <file-path>] [use-ptr]",
    .function   = hsp_command_fn,
    .is_mp_safe = 1,
};

static clib_error_t *hsp_main_init(vlib_main_t *vm)
{
    hsp_main_t *hspm = &hsp_main;
