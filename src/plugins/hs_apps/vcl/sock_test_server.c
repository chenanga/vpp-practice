...
static inline void conn_pool_expand(size_t expand_size)
{
...
static inline sock_server_conn_t *conn_pool_alloc(void)
{
    sock_server_main_t *ssm = &sock_server_main;
    int                 i;
...
static inline void conn_pool_free(sock_server_conn_t *conn)
{
    conn->fd       = 0;
    conn->is_alloc = 0;
}

static inline void sync_config_and_reply(sock_server_conn_t *conn, hs_test_cfg_t *rx_cfg)
{
    conn->cfg = *rx_cfg;
    vcl_test_buf_alloc(&conn->cfg, 1 /* is_rxbuf */, &conn->buf, &conn->buf_size);
    conn->cfg.txbuf_size = conn->cfg.rxbuf_size;
    ...
    (void) sock_test_write(conn->fd, (uint8_t *) &conn->cfg, sizeof(conn->cfg), NULL, conn->cfg.verbose);
}

static void stream_test_server_start_stop(sock_server_conn_t *conn, hs_test_cfg_t *rx_cfg)
{
    sock_server_main_t *ssm       = &sock_server_main;
    ...
    if (rx_cfg->ctrl_handle == conn->fd) {
        ...
        vcl_test_stats_dump("SERVER RESULTS", &conn->stats, 1 /* show_rx */, (test == HS_TEST_TYPE_BI) /* show_tx */, conn->cfg.verbose);
        hs_test_cfg_dump(&conn->cfg, 0 /* is_client */);
        if (conn->cfg.verbose) {
        ...
        rx_cfg->ctrl_handle = (rx_cfg->ctrl_handle == ~0) ? conn->fd : rx_cfg->ctrl_handle;
...
static inline void stream_test_server(sock_server_conn_t *conn, int rx_bytes)
{
    int       client_fd = conn->fd;
    hs_test_t test      = conn->cfg.test;

    if (test == HS_TEST_TYPE_BI) (void) sock_test_write(client_fd, conn->buf, rx_bytes, &conn->stats, conn->cfg.verbose);
...
static inline void af_unix_echo(void)
{
    sock_server_main_t *ssm = &sock_server_main;
    ...
    af_unix_client_fd = accept(ssm->af_unix_listen_fd, (struct sockaddr *) NULL, NULL);
    if (af_unix_client_fd < 0) stfail("af_unix_echo accept()");

    stinf("Got an AF_UNIX connection -- fd = %d (0x%08x)!", af_unix_client_fd, af_unix_client_fd);

    memset(buffer, 0, sizeof(buffer));

    rv = read(af_unix_client_fd, buffer, nbytes);
    if (rv < 0) stfail("af_unix_echo read()");
    ...
    if (!strncmp(SOCK_TEST_MIXED_EPOLL_DATA, (const char *) buffer, nbytes)) {
        ...
        if (rv < 0) stfail("af_unix_echo write()");
...
static inline void new_client(void)
{
    sock_server_main_t *ssm = &sock_server_main;
    int                 client_fd;
    sock_server_conn_t *conn;

    if (ssm->conn_pool_size < (ssm->num_conn + SOCK_SERVER_MAX_TEST_CONN + 1)) conn_pool_expand(SOCK_SERVER_MAX_TEST_CONN + 1);

    conn = conn_pool_alloc();
    if (!conn) stfail("No free connections!");

    client_fd = accept(ssm->listen_fd, (struct sockaddr *) NULL, NULL);
    if (client_fd < 0) stfail("new_client accept()");

    stinf("Got a connection -- fd = %d (0x%08x)!\n", client_fd, client_fd);
    if (fcntl(client_fd, F_SETFL, O_NONBLOCK) < 0) stfail("fcntl()");
    ...
    if (rv < 0) stfail("new_client epoll_ctl()");
...
static int socket_server_echo_af_unix_init(sock_server_main_t *ssm)
{
    ...
    if (ssm->af_unix_listen_fd > 0) return 0;

    unlink((const char *) SOCK_TEST_AF_UNIX_FILENAME);
    ssm->af_unix_listen_fd = socket(AF_UNIX, SOCK_STREAM, 0);
    if (ssm->af_unix_listen_fd < 0) stfail("echo_af_unix_init socket()");

    memset(&ssm->serveraddr, 0, sizeof(ssm->serveraddr));
    ssm->serveraddr.sun_family = AF_UNIX;
    strncpy(ssm->serveraddr.sun_path, SOCK_TEST_AF_UNIX_FILENAME, sizeof(ssm->serveraddr.sun_path));

    rv = bind(ssm->af_unix_listen_fd, (struct sockaddr *) &ssm->serveraddr, SUN_LEN(&ssm->serveraddr));
    if (rv < 0) stfail("echo_af_unix_init bind()");

    rv = listen(ssm->af_unix_listen_fd, 10);
    if (rv < 0) stfail("echo_af_unix_init listen()");

    ssm->af_unix_listen_ev.events   = EPOLLET | EPOLLIN;
    ssm->af_unix_listen_ev.data.u32 = SOCK_TEST_AF_UNIX_ACCEPT_DATA;
    rv                              = epoll_ctl(ssm->epfd, EPOLL_CTL_ADD, ssm->af_unix_listen_fd, &ssm->af_unix_listen_ev);
    if (rv < 0) stfail("echo_af_unix_init epoll_ctl()");
...
static void sts_server_echo(sock_server_conn_t *conn, int rx_bytes)
{
    ...
    if (conn->cfg.verbose) stinf("(fd %d): Echoing back\n", conn->fd);

    nbytes = strlen((const char *) conn->buf) + 1;

    tx_bytes = sock_test_write(conn->fd, conn->buf, nbytes, &conn->stats, conn->cfg.verbose);
    if (tx_bytes >= 0) stinf("(fd %d): TX (%d bytes) - '%s'\n", conn->fd, tx_bytes, conn->buf);
}

static int sts_handle_cfg(hs_test_cfg_t *rx_cfg, sock_server_conn_t *conn, int rx_bytes)
{
    sock_server_main_t *ssm = &sock_server_main;
    ...
    if (rx_bytes != sizeof(*rx_cfg)) {
        stinf("(fd %d): Invalid cfg message size (%d) expected %lu!", conn->fd, rx_bytes, sizeof(*rx_cfg));
        ...
        sock_test_write(conn->fd, (uint8_t *) &conn->cfg, sizeof(conn->cfg), NULL, conn->cfg.verbose);
        return -1;
    }
    ...
    switch (rx_cfg->test) {
        ...
        case HS_TEST_TYPE_ECHO:
            if (socket_server_echo_af_unix_init(ssm)) goto done;
...
{
    if (conn->cfg.test == HS_TEST_TYPE_ECHO) return 1;

    return (conn->stats.rx_bytes < 128 || conn->stats.rx_bytes > conn->cfg.total_bytes);
}
...
int main(int argc, char **argv)
{
    ...
    ssm->listen_fd = socket(ssm->cfg.address_ip6 ? AF_INET6 : AF_INET, ssm->cfg.transport_udp ? SOCK_DGRAM : SOCK_STREAM, 0);

    if (ssm->listen_fd < 0) stfail("main listen()");
    ...
    if (rv < 0) stfail("main bind()");

    rv = fcntl(ssm->listen_fd, F_SETFL, O_NONBLOCK);
    if (rv < 0) stfail("main fcntl()");

    rv = listen(ssm->listen_fd, 10);
    if (rv < 0) stfail("main listen()");

    ssm->epfd = epoll_create(1);
    if (ssm->epfd < 0) stfail("main epoll_create()");
    ...
    if (rv < 0) stfail("main epoll_ctl()");

    stinf("Waiting for a client to connect on port %d...\n", port);

    while (1) {
        int num_ev;
        num_ev = epoll_wait(ssm->epfd, ssm->wait_events, SOCK_SERVER_MAX_EPOLL_EVENTS, 60000);
        if (num_ev < 0) stfail("main epoll_wait()");
        ...
        for (i = 0; i < num_ev; i++) {
            ...
            else if (ssm->wait_events[i].data.u32 == SOCK_TEST_AF_UNIX_ACCEPT_DATA) {
                af_unix_echo();
                continue;
            }
            client_fd = conn->fd;
            ...
            read_again:
                rx_bytes = sock_test_read(client_fd, conn->buf, conn->buf_size, &conn->stats);
                ...
                if ((conn->cfg.test == HS_TEST_TYPE_UNI) || (conn->cfg.test == HS_TEST_TYPE_BI)) {
                    stream_test_server(conn, rx_bytes);
                    if (ioctl(conn->fd, FIONREAD)) goto read_again;
                    continue;
                }
    ...
    if (ssm->conn_pool) free(ssm->conn_pool);
