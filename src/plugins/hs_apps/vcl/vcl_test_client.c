...
static int vtc_cfg_sync(vcl_test_session_t *ts)
{
    ...
    if ((rx_bytes != sizeof(hs_test_cfg_t)) || !hs_test_cfg_verify(rx_cfg, &ts->cfg)) {
...
static int vtc_worker_alloc_sessions(vcl_test_client_worker_t *wrk)
{
    ...
    if (wrk->n_sessions >= n_test_sessions) goto done;

    if (wrk->n_sessions)
        wrk->sessions = realloc(wrk->sessions, n_test_sessions * sizeof(vcl_test_session_t));
    else
...
static void vtc_accumulate_stats(vcl_test_client_worker_t *wrk, vcl_test_session_t *ctrl)
{
    ...
    if (ctrl->cfg.test == HS_TEST_TYPE_BI || ctrl->cfg.test == HS_TEST_TYPE_ECHO) show_rx = 1;
    ...
    for (i = 0; i < wrk->cfg.num_test_sessions; i++) {
        ...
        if (ctrl->cfg.verbose > 1) {
            ...
            vcl_test_stats_dump(buf, &ts->stats, show_rx, 1 /* show tx */, ctrl->cfg.verbose);
        }

        vcl_test_stats_accumulate(&ctrl->stats, &ts->stats);
        if (vcl_comp_tspec(&ctrl->stats.stop, &ts->stats.stop) < 0) ctrl->stats.stop = ts->stats.stop;
    }
...
static void vtc_inc_stats_check(vcl_test_session_t *ts)
{
    ...
    if (ts->stats.tx_bytes - ts->old_stats.tx_bytes < 1 << 20) return;
...
static void vtc_worker_start_transfer(vcl_test_client_worker_t *wrk)
{
...
static int vtc_session_check_is_done(vcl_test_session_t *ts, uint8_t check_rx)
{
    if ((!check_rx && ts->stats.tx_bytes >= ts->cfg.total_bytes) || (check_rx && ts->stats.rx_bytes >= ts->cfg.total_bytes)) {
        clock_gettime(CLOCK_REALTIME, &ts->stats.stop);
        ts->is_done = 1;
        return 1;
    }
...
static int vtc_worker_connect_sessions_select(vcl_test_client_worker_t *wrk)
{
    vcl_test_client_main_t     *vcm = &vcl_client_main;
...
static int vtc_worker_run_select(vcl_test_client_worker_t *wrk)
{
    vcl_test_client_main_t *vcm = &vcl_client_main;
    ...
    while (n_active_sessions && vcm->test_running) {
        ...
        rv = vppcom_select(wrk->max_fd_index, (unsigned long *) rfdset, (unsigned long *) wfdset, NULL, 0);
        if (rv < 0) {
        ...
        for (i = 0; i < wrk->cfg.num_test_sessions; i++) {
            ts = &wrk->sessions[i];
            if (ts->is_done) continue;

            if (FD_ISSET(vppcom_session_index(ts->fd), rfdset) && ts->stats.rx_bytes < ts->cfg.total_bytes) {
                rv = ts->read(ts, ts->rxbuf, ts->rxbuf_size);
                if (rv < 0) break;
            }

            if (FD_ISSET(vppcom_session_index(ts->fd), wfdset) && ts->stats.tx_bytes < ts->cfg.total_bytes) {
                rv = ts->write(ts, ts->txbuf, ts->cfg.txbuf_size);
                if (rv < 0) {
                    vtwrn("vppcom_test_write (%d) failed -- aborting test", ts->fd);
                    break;
                }
                if (vcm->incremental_stats) vtc_inc_stats_check(ts);
            }
            if (vtc_session_check_is_done(ts, check_rx)) n_active_sessions -= 1;
        }
    }
...
static void vtc_worker_epoll_send_add(vcl_test_client_worker_t *wrk, vcl_test_session_t *ts)
{
    if (!wrk->next_to_send) {
        wrk->next_to_send = ts;
    }
...
static void vtc_worker_epoll_send_del(vcl_test_client_worker_t *wrk, vcl_test_session_t *ts, vcl_test_session_t *prev)
{
    if (!prev) {
        wrk->next_to_send = ts->next;
    }
...
static int vtc_worker_connect_sessions_epoll(vcl_test_client_worker_t *wrk)
{
    vcl_test_client_main_t     *vcm = &vcl_client_main;
    vcl_test_main_t            *vt  = &vcl_test_main;
    const vcl_test_proto_vft_t *tp;
    ...
    while (n_connected < wrk->cfg.num_test_sessions) {
         ...
         */
        n_ev = vppcom_epoll_wait(wrk->epoll_sh, wrk->ep_evts, VCL_TEST_CFG_MAX_EPOLL_EVENTS, 0 /* timeout */);
        if (n_ev < 0) {
...
static int vtc_worker_run_epoll(vcl_test_client_worker_t *wrk)
{
    vcl_test_client_main_t *vcm = &vcl_client_main;
    ...
    while (n_active_sessions && vcm->test_running) {
        ...
        if (rv > 0 && n_writes < max_writes) continue;
...
static inline int vtc_worker_run(vcl_test_client_worker_t *wrk)
{
    int rv;
...
static void *vtc_worker_loop(void *arg)
{
    ...
    if (vtc_worker_init(wrk)) goto done;

    if (vtc_worker_run(wrk)) goto done;
    ...
    if (wrk->wrk_index) vt_atomic_add(&vcm->active_workers, -1);
...
static void vtc_print_stats(vcl_test_session_t *ctrl)
{
    ...
    vcl_test_stats_dump("CLIENT RESULTS", &ctrl->stats, show_rx, 1 /* show tx */, ctrl->cfg.verbose);
    ...
    if (is_echo)
        ...
        snprintf(buf, sizeof(buf), "%s-directional Stream", ctrl->cfg.test == HS_TEST_TYPE_BI ? "Bi" : "Uni");
}
...
static void vtc_stream_client(vcl_test_client_main_t *vcm)
{
    ...
    vtinf("%s-directional Stream Test Starting!", ctrl->cfg.test == HS_TEST_TYPE_BI ? "Bi" : "Uni");
    ...
    for (i = 1; i < vcm->n_workers; i++) {
        wrk = &vcm->workers[i];
        if (pthread_create(&wrk->thread_handle, NULL, vtc_worker_loop, (void *) wrk)) {
    ...
    if (vtc_cfg_sync(ctrl)) vtwrn("post-test cfg sync failed!");
}
...
static void cfg_txbuf_size_set(void)
{
    ...
    if (txbuf_size >= VCL_TEST_CFG_BUF_SIZE_MIN) {
        ...
        vtwrn("Invalid txbuf size (%lu) < minimum buf size (%u)!", txbuf_size, VCL_TEST_CFG_BUF_SIZE_MIN);
}
...
static void cfg_num_test_sessions_set(void)
{
    ...
    if ((num_test_sessions > 0) && (num_test_sessions <= VCL_TEST_CFG_MAX_TEST_SESS)) {
        ctrl->cfg.num_test_sessions = num_test_sessions;
        hs_test_cfg_dump(&ctrl->cfg, 1 /* is_client */);
    }
    else {
        vtwrn("invalid num test sessions: %u, (%d max)", num_test_sessions, VCL_TEST_CFG_MAX_TEST_SESS);
    }
}
...
static void cfg_rxbuf_size_set(void)
{
    ...
    if (rxbuf_size >= VCL_TEST_CFG_BUF_SIZE_MIN) {
        ...
        vtwrn("Invalid rxbuf size (%lu) < minimum buf size (%u)!", rxbuf_size, VCL_TEST_CFG_BUF_SIZE_MIN);
}
...
static void vtc_process_opts(vcl_test_client_main_t *vcm, int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "chnp:w:xXE:I:N:R:T:b:UBV6DLs:q:S")) != -1)
        switch (c) {
            ...
            case 's':
                ...
                if (!ctrl->cfg.num_test_sessions || (ctrl->cfg.num_test_sessions > VCL_TEST_CFG_MAX_TEST_SESS)) {
                    vtwrn(
            ...
            case 'E':
                if (strlen(optarg) > ctrl->txbuf_size) {
                    vtwrn("Option -%c value larger than txbuf size (%d)!", optopt, ctrl->txbuf_size);
                    print_usage_and_exit();
                }
            ...
            case 'R':
                ...
                if (ctrl->cfg.rxbuf_size >= VCL_TEST_CFG_BUF_SIZE_MIN) {
                    ...
                    vtwrn("rxbuf size (%lu) less than minumum (%u)", ctrl->cfg.rxbuf_size, VCL_TEST_CFG_BUF_SIZE_MIN);
                    print_usage_and_exit();
                }
            ...
            case 'T':
                ...
                if (ctrl->cfg.txbuf_size >= VCL_TEST_CFG_BUF_SIZE_MIN) {
                    ...
                    vtwrn("txbuf size (%lu) less than minumum (%u)!", ctrl->cfg.txbuf_size, VCL_TEST_CFG_BUF_SIZE_MIN);
                    print_usage_and_exit();
                }
                break;
            case 'b':
                ...
                if (ctrl->cfg.total_bytes % ctrl->cfg.txbuf_size) {
                    vtwrn("total bytes must be mutliple of txbuf size(0x%lu)!", ctrl->cfg.txbuf_size);
                    print_usage_and_exit();
                }
            ...
            case 'p':
                if (vppcom_unformat_proto(&vcm->proto, optarg)) vtwrn("Invalid vppcom protocol %s, defaulting to TCP", optarg);
                break;
    ...
    ctrl->cfg.num_test_qsessions =
        vcm->proto != VPPCOM_PROTO_QUIC ? 0 : (ctrl->cfg.num_test_sessions + ctrl->cfg.num_test_sessions_perq - 1) / ctrl->cfg.num_test_sessions_perq;
...
static void vtc_ctrl_session_exit(void)
{
    ...
    if (vcm->post_test == HS_TEST_TYPE_EXIT_CLIENT) return;

    ctrl->cfg.test = HS_TEST_TYPE_EXIT;
    vtinf("(fd %d): Sending exit cfg to server...", ctrl->fd);
    if (verbose) hs_test_cfg_dump(&ctrl->cfg, 1 /* is_client */);
...
static void vtc_alloc_workers(vcl_test_client_main_t *vcm)
{
    vcl_test_main_t   *vt = &vcl_test_main;
...
int main(int argc, char **argv)
{
    ...
    if (rv < 0) vtfail("vppcom_app_create()", rv);

    /* Protos like tls/dtls/quic need init */
    if (vt->protos[vcm->proto]->init) {
        rv = vt->protos[vcm->proto]->init(&ctrl->cfg);
        if (rv) vtfail("client init failed", rv);
    }

    if ((rv = vtc_ctrl_session_init(vcm, ctrl))) vtfail("vppcom_session_create() ctrl session", rv);
