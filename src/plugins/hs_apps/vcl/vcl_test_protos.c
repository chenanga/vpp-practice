...
static int vt_tls_init(hs_test_cfg_t *cfg)
{
...
static int vt_tls_connect(vcl_test_session_t *ts, vppcom_endpt_t *endpt)
{
    ...
    vppcom_session_attr(ts->fd, VPPCOM_ATTR_SET_CKPAIR, &vt->ckpair_index, &ckp_len);
...
static int vt_dtls_init(hs_test_cfg_t *cfg)
{
    return vt_add_cert_key_pair();
}
...
static int vt_quic_init(hs_test_cfg_t *cfg)
{
    vcl_test_main_t *vt = &vcl_test_main;
...
static int vt_quic_maybe_init_wrk(vcl_test_main_t *vt, vcl_test_wrk_t *wrk, vppcom_endpt_t *endpt)
{
    ...
    if (wrk->n_qsessions == vt->cfg.num_test_qsessions) return 0;

    /* Make sure pool is large enough */
    if (!wrk->qsessions) {
        wrk->qsessions = calloc(vt->cfg.num_test_qsessions, sizeof(vcl_test_session_t));
    }
    else {
    ...
    for (i = 0; i < vt->cfg.num_test_qsessions; i++) {
        ...
        ckp_len = sizeof(vt->ckpair_index);
        vppcom_session_attr(tq->fd, VPPCOM_ATTR_SET_CKPAIR, &vt->ckpair_index, &ckp_len);
...
static int vt_quic_connect(vcl_test_session_t *ts, vppcom_endpt_t *endpt)
{
    ...
    if (!ts->noblk_connect) {
        ...
        flen  = sizeof(flags);
        vppcom_session_attr(ts->fd, VPPCOM_ATTR_SET_FLAGS, &flags, &flen);
        vtinf("Test (quic stream) session %d (fd %d) connected.", ts->session_index, ts->fd);
    }
...
static int vt_quic_close(vcl_test_session_t *ts)
{
    ...
    if ((vppcom_session_n_accepted(listener_fd) == 0) & vppcom_session_is_connectable_listener(listener_fd)) {
...
static unsigned char test_key[46] = {0xe1, 0xf9, 0x7a, 0x0d, 0x3e, 0x01, 0x8b, 0xe0, 0xd6, 0x4f, 0xa3, 0x2c, 0x06, 0xde, 0x41, 0x39,
                                     0x0e, 0xc6, 0x75, 0xad, 0x49, 0x8a, 0xfe, 0xeb, 0xb6, 0x96, 0x0b, 0x3a, 0xab, 0xe6, 0xc1, 0x73,
                                     0xc3, 0x17, 0xf2, 0xda, 0xbe, 0x35, 0x77, 0x93, 0xb6, 0x96, 0x0b, 0x3a, 0xab, 0xe6};
...
static void vt_session_add_srtp_policy(vcl_test_session_t *ts, int is_connect)
{
    ...
    vppcom_session_attr(ts->fd, VPPCOM_ATTR_SET_ENDPT_EXT_CFG, srtp_cfg, &cfg_size);
    free(srtp_cfg);
}
...
static int vt_srtp_write(vcl_test_session_t *ts, void *buf, uint32_t nbytes)
{
    ...
    do {
        ...
        if (rv < 0) {
            if ((rv == VPPCOM_EAGAIN || rv == VPPCOM_EWOULDBLOCK)) stats->tx_eagain++;
            break;
        }
        tx_bytes += rv;
...
static inline int vt_srtp_read(vcl_test_session_t *ts, void *buf, uint32_t nbytes)
{
    vcl_test_stats_t *stats = &ts->stats;
    rtp_hdr_t        *hdr;
    int               rx_bytes;
    ...
    if (rx_bytes < nbytes) stats->rx_incomp++;

    stats->rx_bytes += rx_bytes;

    hdr = &((rtp_headers_t *) ts->opaque)->rx_hdr;
    if (((rtp_hdr_t *) buf)->ssrc != hdr->ssrc) hdr->ssrc = ((rtp_hdr_t *) buf)->ssrc;
    return (rx_bytes);
}
...
static void vt_http_session_init(vcl_test_session_t *ts, u8 is_server)
{
...
static inline void vt_http_send_reply_msg(vcl_test_session_t *ts, http_status_code_t status)
{
    http_msg_t msg;
    int        rv = 0;
...
static inline int vt_process_http_server_read_msg(vcl_test_session_t *ts, void *buf, uint32_t nbytes)
{
    http_msg_t           msg;
    u8                  *target_path       = 0;
    ...
    do {
        ...
        if (PREDICT_TRUE(vcl_test_http_ctx->test_state == VCL_TEST_HTTP_IN_PROGRESS)) {
            ...
            if (msg.data.target_path_len) {
                ...
                memcpy(target_path, buf + sizeof(msg) + msg.data.target_path_offset - 1, msg.data.target_path_len + 1);
            ...
            if (msg.data.body_len) {
                ...
                vcl_test_http_ctx->rem_data -= (rv - sizeof(msg) - msg.data.body_offset);
                vcl_test_http_ctx->test_state = VCL_TEST_HTTP_IN_PROGRESS;
            }
        }

        if (rv < nbytes) stats->rx_incomp++;
    } while (rv <= 0);
...
static inline int vt_process_http_client_read_msg(vcl_test_session_t *ts, void *buf, uint32_t nbytes)
{
    http_msg_t msg;
    int        rv = 0;
...
static inline int vt_process_http_client_write_msg(vcl_test_session_t *ts, void *buf, uint32_t nbytes)
{
    http_msg_t           msg;
    ...
    if (PREDICT_TRUE(vcl_test_http_ctx->test_state == VCL_TEST_HTTP_IN_PROGRESS)) {
        do {
            rv = vppcom_session_write(ts->fd, buf, clib_min(nbytes, vcl_test_http_ctx->rem_data));
            ...
            if (vcl_test_http_ctx->rem_data == 0) {
                vcl_test_http_ctx->test_state = VCL_TEST_HTTP_COMPLETED;
                vtinf("client finished sending %ld bytes of data", ts->cfg.total_bytes);
            }

            if (rv < nbytes) stats->tx_incomp++;
        } while (rv <= 0);
    }

    else if (PREDICT_FALSE(vcl_test_http_ctx->test_state == VCL_TEST_HTTP_IDLE)) {
        http_add_header(&req_headers, http_header_name_token(HTTP_HEADER_CONTENT_TYPE), http_content_type_token(HTTP_CONTENT_APP_OCTET_STREAM));
        ...
        vppcom_data_segment_t segs[3] = {{(u8 *) &msg, sizeof(msg)}, {target, strlen((char *) target)}, {headers_buf, vec_len(headers_buf)}};
...
static inline int vt_process_http_server_write_msg(vcl_test_session_t *ts, void *buf, uint32_t nbytes)
{
    return 0;
}

static inline int vt_http_read(vcl_test_session_t *ts, void *buf, uint32_t nbytes)
{
    vcl_test_http_ctx_t *vcl_test_http_ctx = (vcl_test_http_ctx_t *) ts->opaque;
...
static inline int vt_http_write(vcl_test_session_t *ts, void *buf, uint32_t nbytes)
{
    vcl_test_http_ctx_t *vcl_test_http_ctx = (vcl_test_http_ctx_t *) ts->opaque;
...
static int vt_http_connect(vcl_test_session_t *ts, vppcom_endpt_t *endpt)
{
    uint32_t flags, flen;
    int      rv;
...
static int vt_http_listen(vcl_test_session_t *ts, vppcom_endpt_t *endpt)
{
    int rv;
...
static int vt_http_accept(int listen_fd, vcl_test_session_t *ts)
{
    int client_fd;
...
static int vt_http_close(vcl_test_session_t *ts)
{
    free(ts->opaque);
    return 0;
}
