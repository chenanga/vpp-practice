...
static int sock_test_cfg_sync(vcl_test_session_t *socket)
{
    ...
    if (socket->cfg.verbose) hs_test_cfg_dump(&socket->cfg, 1 /* is_client */);
    ...
    tx_bytes = sock_test_write(socket->fd, (uint8_t *) &ctrl->cfg, sizeof(ctrl->cfg), NULL, ctrl->cfg.verbose);
    if (tx_bytes < 0) stabrt("(fd %d): write test cfg failed (%d)!", socket->fd, tx_bytes);

    rx_bytes = sock_test_read(socket->fd, (uint8_t *) socket->rxbuf, sizeof(hs_test_cfg_t), NULL);
    if (rx_bytes < 0) return rx_bytes;

    if (rl_cfg->magic != HS_TEST_CFG_CTRL_MAGIC) stabrt("(fd %d): Bad server reply cfg -- aborting!\n", socket->fd);

    if ((rx_bytes != sizeof(hs_test_cfg_t)) || !hs_test_cfg_verify(rl_cfg, &ctrl->cfg))
    ...
    ctrl->cfg.ctrl_handle = ((ctrl->cfg.ctrl_handle == ~0) ? rl_cfg->ctrl_handle : ctrl->cfg.ctrl_handle);
...
static void sock_client_echo_af_unix(sock_client_main_t *scm)
{
    ...
    if (fd < 0) stfail("socket()");

    memset(&serveraddr, 0, sizeof(serveraddr));
    serveraddr.sun_family = AF_UNIX;
    strncpy(serveraddr.sun_path, SOCK_TEST_AF_UNIX_FILENAME, sizeof(serveraddr.sun_path));
    rv = connect(fd, (struct sockaddr *) &serveraddr, SUN_LEN(&serveraddr));
    if (rv < 0) stfail("connect()");
    ...
    if (rv < 0) stfail("write()");

    if (rv < nbytes) stabrt("write(fd %d, \"%s\", %lu) returned %d!", fd, buffer, nbytes, rv);
    ...
    if (rv < nbytes) stabrt("read(fd %d, %p, %lu) returned %d!\n", fd, buffer, nbytes, rv);
...
static void echo_test_client(void)
{
    ...
    for (n = 0; n != ctrl->cfg.num_test_sessions; n++) {
        ...
        if (sock_test_cfg_sync(tsock)) return;
    ...
    while (n) {
        ...
        if (rv < 0) stfail("select()");

        if (rv == 0) continue;

        for (i = 0; i < ctrl->cfg.num_test_sessions; i++) {
            tsock = &scm->test_socket[i];
            if (!((tsock->stats.stop.tv_sec == 0) && (tsock->stats.stop.tv_nsec == 0))) continue;

            if (FD_ISSET(tsock->fd, wfdset) && (tsock->stats.tx_bytes < ctrl->cfg.total_bytes)) {
                tx_bytes = sock_test_write(tsock->fd, (uint8_t *) tsock->txbuf, nbytes, &tsock->stats, ctrl->cfg.verbose);
                if (tx_bytes < 0) stabrt("sock_test_write(%d) failed -- aborting test!", tsock->fd);

                stinf("(fd %d): TX (%d bytes) - '%s'", tsock->fd, tx_bytes, tsock->txbuf);
            }

            if ((FD_ISSET(tsock->fd, rfdset)) && (tsock->stats.rx_bytes < ctrl->cfg.total_bytes)) {
                rx_bytes = sock_test_read(tsock->fd, (uint8_t *) tsock->rxbuf, nbytes, &tsock->stats);
                if (rx_bytes > 0) {
    ...
    for (i = 0; i < ctrl->cfg.num_test_sessions; i++) {
        ...
        if (ctrl->cfg.verbose) {
            ...
            vcl_test_stats_dump(buf, &tsock->stats, 1 /* show_rx */, 1 /* show tx */, ctrl->cfg.verbose);
        }
    ...
    if (ctrl->cfg.verbose) {
        vcl_test_stats_dump("CLIENT RESULTS", &ctrl->stats, 1 /* show_rx */, 1 /* show tx */, ctrl->cfg.verbose);
        hs_test_cfg_dump(&ctrl->cfg, 1 /* is_client */);
...
static void stream_test_client(hs_test_t test)
{
    sock_client_main_t *scm  = &sock_client_main;
    vcl_test_session_t *ctrl = &scm->ctrl_socket;
    vcl_test_session_t *tsock;
    ...
    if (sock_test_cfg_sync(ctrl)) stabrt("test cfg sync failed -- aborting!");
    ...
    while (n) {
        ...
        for (i = 0; i < ctrl->cfg.num_test_sessions; i++) {
            ...
            if ((test == HS_TEST_TYPE_BI) && FD_ISSET(tsock->fd, rfdset) && (tsock->stats.rx_bytes < ctrl->cfg.total_bytes)) {
                (void) sock_test_read(tsock->fd, (uint8_t *) tsock->rxbuf, tsock->rxbuf_size, &tsock->stats);
            }

            if (FD_ISSET(tsock->fd, wfdset) && (tsock->stats.tx_bytes < ctrl->cfg.total_bytes)) {
                tx_bytes = sock_test_write(tsock->fd, (uint8_t *) tsock->txbuf, ctrl->cfg.txbuf_size, &tsock->stats, ctrl->cfg.verbose);
                if (tx_bytes < 0) stabrt("sock_test_write(%d) failed -- aborting test!", tsock->fd);
            }

            if (((test == HS_TEST_TYPE_UNI) && (tsock->stats.tx_bytes >= ctrl->cfg.total_bytes))
                || ((test == HS_TEST_TYPE_BI) && (tsock->stats.rx_bytes >= ctrl->cfg.total_bytes))) {
                clock_gettime(CLOCK_REALTIME, &tsock->stats.stop);
                n--;
            }
        }
    ...
    for (i = 0; i < ctrl->cfg.num_test_sessions; i++) {
        ...
        if (ctrl->cfg.verbose) {
            ...
            snprintf(buf, sizeof(buf), "CLIENT (fd %d) RESULTS", tsock->fd);
            vcl_test_stats_dump(buf, &tsock->stats, test == HS_TEST_TYPE_BI /* show_rx */, 1 /* show tx */, ctrl->cfg.verbose);
        }

        vcl_test_stats_accumulate(&ctrl->stats, &tsock->stats);
    }

    vcl_test_stats_dump("CLIENT RESULTS", &ctrl->stats, test == HS_TEST_TYPE_BI /* show_rx */, 1 /* show tx */, ctrl->cfg.verbose);
    hs_test_cfg_dump(&ctrl->cfg, 1 /* is_client */);
    ...
    if (sock_test_cfg_sync(ctrl)) stabrt("post-test cfg sync failed!");
...
static void exit_client(void)
{
    ...
    for (i = 0; i < ctrl->cfg.num_test_sessions; i++) {
        ...
        (void) sock_test_write(tsock->fd, (uint8_t *) &tsock->cfg, sizeof(tsock->cfg), &tsock->stats, ctrl->cfg.verbose);
    }
    ...
    (void) sock_test_write(ctrl->fd, (uint8_t *) &ctrl->cfg, sizeof(ctrl->cfg), &ctrl->stats, ctrl->cfg.verbose);
...
static int sock_test_connect_test_sockets(uint32_t num_test_sockets)
{
    ...
    if (num_test_sockets < scm->num_test_sockets) {
        ...
        tsock = realloc(scm->test_socket, sizeof(vcl_test_session_t) * num_test_sockets);
        if (!tsock) stfail("realloc()");

        memset(&tsock[scm->num_test_sockets], 0, sizeof(vcl_test_session_t) * (num_test_sockets - scm->num_test_sockets));

        scm->test_socket = tsock;
        for (i = scm->num_test_sockets; i < num_test_sockets; i++) {
            tsock     = &scm->test_socket[i];
            tsock->fd = socket(ctrl->cfg.address_ip6 ? AF_INET6 : AF_INET, ctrl->cfg.transport_udp ? SOCK_DGRAM : SOCK_STREAM, 0);

            if (tsock->fd < 0) stfail("socket()");

            rv = connect(tsock->fd, (struct sockaddr *) &scm->server_addr, scm->server_addr_size);

            if (rv < 0) stfail("connect()");

            if (fcntl(tsock->fd, F_SETFL, O_NONBLOCK) < 0) stfail("fcntl");
...
static void cfg_txbuf_size_set(void)
{
    ...
    if (txbuf_size >= VCL_TEST_CFG_BUF_SIZE_MIN) {
        ...
        ctrl->cfg.total_bytes = ctrl->cfg.num_writes * ctrl->cfg.txbuf_size;
        vcl_test_buf_alloc(&ctrl->cfg, 0 /* is_rxbuf */, (uint8_t **) &ctrl->txbuf, &ctrl->txbuf_size);
        hs_test_cfg_dump(&ctrl->cfg, 1 /* is_client */);
    }
    else
        stabrt("Invalid txbuf size (%lu) < minimum buf size (%u)!", txbuf_size, VCL_TEST_CFG_BUF_SIZE_MIN);
}
...
static void cfg_num_test_sockets_set(void)
{
    ...
    if ((num_test_sockets > 0) && (num_test_sockets <= VCL_TEST_CFG_MAX_TEST_SESS)) {
...
static void cfg_rxbuf_size_set(void)
{
    ...
    if (rxbuf_size >= VCL_TEST_CFG_BUF_SIZE_MIN) {
        ...
        vcl_test_buf_alloc(&ctrl->cfg, 1 /* is_rxbuf */, (uint8_t **) &ctrl->rxbuf, &ctrl->rxbuf_size);
        hs_test_cfg_dump(&ctrl->cfg, 1 /* is_client */);
    }
    else
        stabrt("Invalid rxbuf size (%lu) < minimum buf size (%u)!", rxbuf_size, VCL_TEST_CFG_BUF_SIZE_MIN);
}
...
static hs_test_t parse_input()
{
    sock_client_main_t *scm  = &sock_client_main;
    vcl_test_session_t *ctrl = &scm->ctrl_socket;
    hs_test_t           rv   = HS_TEST_TYPE_NONE;

    if (!strncmp(VCL_TEST_TOKEN_EXIT, ctrl->txbuf, strlen(VCL_TEST_TOKEN_EXIT)))
        rv = HS_TEST_TYPE_EXIT;

    else if (!strncmp(VCL_TEST_TOKEN_HELP, ctrl->txbuf, strlen(VCL_TEST_TOKEN_HELP)))
        dump_help();

    else if (!strncmp(VCL_TEST_TOKEN_SHOW_CFG, ctrl->txbuf, strlen(VCL_TEST_TOKEN_SHOW_CFG)))
        scm->dump_cfg = 1;

    else if (!strncmp(VCL_TEST_TOKEN_VERBOSE, ctrl->txbuf, strlen(VCL_TEST_TOKEN_VERBOSE)))
        cfg_verbose_toggle();

    else if (!strncmp(VCL_TEST_TOKEN_TXBUF_SIZE, ctrl->txbuf, strlen(VCL_TEST_TOKEN_TXBUF_SIZE)))
        cfg_txbuf_size_set();

    else if (!strncmp(VCL_TEST_TOKEN_NUM_TEST_SESS, ctrl->txbuf, strlen(VCL_TEST_TOKEN_NUM_TEST_SESS)))
        cfg_num_test_sockets_set();

    else if (!strncmp(VCL_TEST_TOKEN_NUM_WRITES, ctrl->txbuf, strlen(VCL_TEST_TOKEN_NUM_WRITES)))
        cfg_num_writes_set();

    else if (!strncmp(VCL_TEST_TOKEN_RXBUF_SIZE, ctrl->txbuf, strlen(VCL_TEST_TOKEN_RXBUF_SIZE)))
        cfg_rxbuf_size_set();

    else if (!strncmp(HS_TEST_TOKEN_RUN_UNI, ctrl->txbuf, strlen(HS_TEST_TOKEN_RUN_UNI)))
        rv = ctrl->cfg.test = HS_TEST_TYPE_UNI;

    else if (!strncmp(HS_TEST_TOKEN_RUN_BI, ctrl->txbuf, strlen(HS_TEST_TOKEN_RUN_BI)))
...
int main(int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "chn:w:XE:I:N:R:T:UBV6D")) != -1)
        switch (c) {
            ...
            case 's':
                ...
                if (!ctrl->cfg.num_test_sessions || (ctrl->cfg.num_test_sessions > FD_SETSIZE)) {
                    stinf(
                        "ERROR: Invalid number of "
                        "sockets (%d) specified for option -%c!\n"
                        "       Valid range is 1 - %d\n",
                        ctrl->cfg.num_test_sessions,
                        c,
                        FD_SETSIZE);
                    print_usage_and_exit();
                }
            ...
            case 'E':
                if (strlen(optarg) > ctrl->txbuf_size) {
                    stinf("ERROR: Option -%c value larger than txbuf size (%d)!", optopt, ctrl->txbuf_size);
                    print_usage_and_exit();
                }
            ...
            case 'I':
                ...
                if (ctrl->cfg.num_test_sessions > VCL_TEST_CFG_MAX_TEST_SESS) {
                    stinf("ERROR: value greater than max number test sockets (%d)!", VCL_TEST_CFG_MAX_TEST_SESS);
                    print_usage_and_exit();
                }
                break;
            ...
            case 'T':
                ...
                if (ctrl->cfg.txbuf_size >= VCL_TEST_CFG_BUF_SIZE_MIN) {
                    ...
                    stinf("ERROR: txbuf size (%lu) less than minumum (%u)!", ctrl->cfg.txbuf_size, VCL_TEST_CFG_BUF_SIZE_MIN);
                    print_usage_and_exit();
                }
                break;
    ...
    ctrl->fd = socket(ctrl->cfg.address_ip6 ? AF_INET6 : AF_INET, ctrl->cfg.transport_udp ? SOCK_DGRAM : SOCK_STREAM, 0);

    if (ctrl->fd < 0) stfail("socket()");

    memset(&scm->server_addr, 0, sizeof(scm->server_addr));
    if (ctrl->cfg.address_ip6) {
        struct sockaddr_in6 *server_addr = (struct sockaddr_in6 *) &scm->server_addr;
        ...
        struct sockaddr_in *server_addr = (struct sockaddr_in *) &scm->server_addr;
        scm->server_addr_size           = sizeof(*server_addr);
    ...
    do {
        ...
        rv = connect(ctrl->fd, (struct sockaddr *) &scm->server_addr, scm->server_addr_size);
