...
static inline void conn_pool_expand(vcl_test_server_worker_t *wrk, size_t expand_size)
{
    vcl_test_session_t *conn_pool;
...
static inline vcl_test_session_t *conn_pool_alloc(vcl_test_server_worker_t *wrk)
{
...
static inline void conn_pool_free(vcl_test_session_t *ts)
{
    ts->fd       = 0;
    ts->is_alloc = 0;
...
static inline void sync_config_and_reply(vcl_test_session_t *conn, hs_test_cfg_t *rx_cfg)
{
    conn->cfg = *rx_cfg;
    vcl_test_buf_alloc(&conn->cfg, 1 /* is_rxbuf */, (uint8_t **) &conn->rxbuf, &conn->rxbuf_size);
...
static void vts_session_close(vcl_test_session_t *conn)
{
    ...
    if (vt->protos[vsm->server_cfg.proto]->close) vt->protos[vsm->server_cfg.proto]->close(conn);
...
static void vts_test_cmd(vcl_test_server_worker_t *wrk, vcl_test_session_t *conn, hs_test_cfg_t *rx_cfg)
{
    ...
    if (rx_cfg->cmd == HS_TEST_CMD_STOP) {
        ...
        for (i = 0; i < wrk->conn_pool_size; i++) {
            ...
            if (vcl_comp_tspec(&conn->stats.stop, &tc->stats.stop) < 0) conn->stats.stop = tc->stats.stop;
        }

        if (conn->cfg.verbose) {
            snprintf(buf, sizeof(buf), "SERVER (fd %d) RESULTS", conn->fd);
            vcl_test_stats_dump(buf, &conn->stats, 1 /* show_rx */, is_bi /* show tx */, conn->cfg.verbose);
        }

        vcl_test_stats_dump("SERVER RESULTS", &conn->stats, 1 /* show_rx */, is_bi /* show_tx */, conn->cfg.verbose);
        hs_test_cfg_dump(&conn->cfg, 0 /* is_client */);
        if (conn->cfg.verbose) {
        ...
        vtinf("Starting %s-directional Stream Test (fd %d)!", is_bi ? "Bi" : "Uni", conn->fd);
        rx_cfg->ctrl_handle = conn->fd;
        sync_config_and_reply(conn, rx_cfg);
...
static inline void vts_server_process_rx(vcl_test_session_t *conn, int rx_bytes)
{
    vcl_test_server_main_t *vsm = &vcl_server_main;
    ...
    if (vsm->use_ds) vppcom_session_free_segments(conn->fd, rx_bytes);

    if (conn->stats.rx_bytes >= conn->cfg.total_bytes) clock_gettime(CLOCK_REALTIME, &conn->stats.stop);
}
...
static void vts_server_echo(vcl_test_session_t *conn, int rx_bytes)
{
    ...
    if (conn->cfg.verbose) vtinf("(fd %d): Echoing back", conn->fd);

    nbytes   = strlen((const char *) conn->rxbuf) + 1;
    tx_bytes = conn->write(conn, conn->rxbuf, nbytes);
    if (tx_bytes >= 0) vtinf("(fd %d): TX (%d bytes) - '%s'", conn->fd, tx_bytes, conn->rxbuf);
}
...
static vcl_test_session_t *vts_accept_ctrl(vcl_test_server_worker_t *wrk, int listen_fd)
{
    ...
    if (vsm->ctrl) conn->cfg = vsm->ctrl->cfg;
    ...
    vtinf("CTRL accepted fd = %d (0x%08x) on listener fd = %d (0x%08x)", conn->fd, conn->fd, listen_fd, listen_fd);
...
static vcl_test_session_t *vts_accept_client(vcl_test_server_worker_t *wrk, int listen_fd)
{
    ...
    if (conn->cfg.num_test_sessions < VCL_TEST_CFG_MAX_SELECT_SESS)
        vtinf("Got a connection -- fd = %d (0x%08x) on listener fd = %d (0x%08x)", conn->fd, conn->fd, listen_fd, listen_fd);
...
static void vcl_test_server_process_opts(vcl_test_server_main_t *vsm, int argc, char **argv)
{
    ...
    while ((c = getopt(argc, argv, "6DLsw:hp:S")) != -1)
        switch (c) {
            ...
            case 'p':
                if (vppcom_unformat_proto(&vsm->server_cfg.proto, optarg)) vtwrn("Invalid vppcom protocol %s, defaulting to TCP", optarg);
                break;
...
int vts_handle_ctrl_cfg(vcl_test_server_worker_t *wrk, hs_test_cfg_t *rx_cfg, vcl_test_session_t *conn, int rx_bytes)
{
    if (rx_cfg->verbose) {
    ...
    if (rx_bytes != sizeof(*rx_cfg)) {
        vtinf("(fd %d): Invalid cfg msg size %d expected %lu!", conn->fd, rx_bytes, sizeof(*rx_cfg));
    ...
    switch (rx_cfg->test) {
        ...
        case HS_TEST_TYPE_EXIT:
            ...
            vts_session_cleanup(conn);
            wrk->nfds--;
            if (wrk->nfds) vts_wrk_cleanup_all(wrk);
...
static void vts_worker_init(vcl_test_server_worker_t *wrk)
{
    ...
    if (wrk->wrk_index)
        if (vppcom_worker_register()) vtfail("vppcom_worker_register()", 1);

    tp = vt->protos[vsm->server_cfg.proto];
    if ((rv = tp->listen(&wrk->listener, &vsm->server_cfg.endpt))) vtfail("proto listen", rv);

    /* First worker already has epoll fd */
    if (wrk->wrk_index) {
        wrk->epfd = vppcom_epoll_create();
        if (wrk->epfd < 0) vtfail("vppcom_epoll_create()", wrk->epfd);
    }

    listen_ev.events   = EPOLLET | EPOLLIN;
    listen_ev.data.u32 = VCL_TEST_DATA_LISTENER;
    rv                 = vppcom_epoll_ctl(wrk->epfd, EPOLL_CTL_ADD, wrk->listener.fd, &listen_ev);
    if (rv < 0) vtfail("vppcom_epoll_ctl", rv);

    vsm->active_workers += 1;
    vtinf("Waiting for client data connections on port %d ...", ntohs(vsm->server_cfg.endpt.port));
}

static inline int vts_conn_read(vcl_test_session_t *conn)
{
    vcl_test_server_main_t *vsm = &vcl_server_main;
    if (vsm->use_ds)
...
static void vts_inc_stats_check(vcl_test_session_t *ts)
{
    ...
    if (ts->stats.rx_bytes - ts->old_stats.rx_bytes < 1 << 20) return;
...
static void *vts_worker_loop(void *arg)
{
    ...
    if (wrk->wrk_index) vts_worker_init(wrk);

    while (1) {
        num_ev = vppcom_epoll_wait(wrk->epfd, ep_evts, VCL_TEST_CFG_MAX_EPOLL_EVENTS, 0 /* poll session events */);
        if (num_ev < 0) {
        ...
        for (i = 0; i < num_ev; i++) {
            ...
            read_again:
                ...
                if (vppcom_session_attr(conn->fd, VPPCOM_ATTR_GET_NREAD, 0, 0) > 0) goto read_again;
                if (vsm->incremental_stats) vts_inc_stats_check(conn);
                continue;
            }
            else {
...
fail:
    ...
    if (wrk->conn_pool) free(wrk->conn_pool);
    vsm->active_workers -= 1;
    return 0;
}
...
static void vts_ctrl_session_init(vcl_test_server_worker_t *wrk)
{
    ...
    vsm->ctrl_listen_fd = vppcom_session_create(VPPCOM_PROTO_TCP, 0 /* is_nonblocking */);
    if (vsm->ctrl_listen_fd < 0) vtfail("vppcom_session_create()", vsm->ctrl_listen_fd);

    rv = vppcom_session_bind(vsm->ctrl_listen_fd, &vsm->server_cfg.endpt);
    if (rv < 0) vtfail("vppcom_session_bind()", rv);

    rv = vppcom_session_listen(vsm->ctrl_listen_fd, 10);
    if (rv < 0) vtfail("vppcom_session_listen()", rv);
    ...
    rv                 = vppcom_epoll_ctl(wrk->epfd, EPOLL_CTL_ADD, vsm->ctrl_listen_fd, &listen_ev);
    if (rv < 0) vtfail("vppcom_epoll_ctl", rv);

    vtinf("Waiting for client ctrl connection on port %d ...", vsm->server_cfg.port);
}
...
int main(int argc, char **argv)
{
    ...
    if (rv) vtfail("vppcom_app_create()", rv);

    /* Protos like tls/dtls/quic need init */
    if (vt->protos[vsm->server_cfg.proto]->init) vt->protos[vsm->server_cfg.proto]->init(0);
    ...
    for (i = 1; i < vsm->server_cfg.workers; i++) {
        ...
        rv                        = pthread_create(&vsm->workers[i].thread_handle, NULL, vts_worker_loop, (void *) &vsm->workers[i]);
        if (rv) vtfail("pthread_create()", rv);
    }
