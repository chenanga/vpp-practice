...
static inline int vcl_test_worker_index(void)
{
    return __wrk_index;
}
...
static inline void vcl_test_stats_accumulate(vcl_test_stats_t *accum, vcl_test_stats_t *incr)
{
...
static inline void vcl_test_buf_alloc(hs_test_cfg_t *cfg, uint8_t is_rxbuf, uint8_t **buf, uint32_t *bufsize)
{
    ...
    if (lb) {
        ...
        *buf = lb;
        ...
        vtwrn("realloc failed. using buffer size %d instead of %u", *bufsize, alloc_size);
    }
}

static inline void vcl_test_session_buf_alloc(vcl_test_session_t *ts)
{
    ts->rxbuf_size = ts->cfg.rxbuf_size;
    ts->txbuf_size = ts->cfg.txbuf_size;
    vcl_test_buf_alloc(&ts->cfg, 0 /* is_rxbuf */, (uint8_t **) &ts->txbuf, &ts->txbuf_size);
    vcl_test_buf_alloc(&ts->cfg, 1 /* is_rxbuf */, (uint8_t **) &ts->rxbuf, &ts->rxbuf_size);
}

static inline void vcl_test_session_buf_free(vcl_test_session_t *ts)
{
    free(ts->rxbuf);
    free(ts->txbuf);
    ts->rxbuf = 0;
...
static inline void vcl_test_stats_dump(char *header, vcl_test_stats_t *stats, uint8_t show_rx, uint8_t show_tx, uint8_t verbose)
{
    ...
    if (show_tx) {
        printf(HS_TEST_SEPARATOR_STRING "  tx stats (0x%p):\n" HS_TEST_SEPARATOR_STRING
                                        "         writes:  %lu (0x%08lx)\n"
                                        "       tx bytes:  %lu (0x%08lx)\n"
                                        "      tx eagain:  %u (0x%08x)\n"
    ...
    if (show_rx) {
        printf(HS_TEST_SEPARATOR_STRING "  rx stats (0x%p):\n" HS_TEST_SEPARATOR_STRING
                                        "          reads:  %lu (0x%08lx)\n"
                                        "       rx bytes:  %lu (0x%08lx)\n"
                                        "      rx eagain:  %u (0x%08x)\n"
...
static inline double vcl_test_time_diff(struct timespec *old, struct timespec *new)
{
    uint64_t sec, nsec;
...
static inline void vcl_test_stats_dump_inc(vcl_test_session_t *ts, int is_rx)
{
    ...
    rate = (double) total_bytes * 8 / duration / 1e9;
    printf("%d: %s %lu Mbytes in %.2lf seconds %.2lf Gbps\n", ts->fd, dir_str, (uint64_t) (total_bytes / 1e6), duration, rate);
}

static inline int vcl_comp_tspec(struct timespec *a, struct timespec *b)
{
...
static inline int vcl_test_read(vcl_test_session_t *ts, void *buf, uint32_t nbytes)
{
...
static inline int vcl_test_read_ds(vcl_test_session_t *ts)
{
    vcl_test_stats_t *stats = &ts->stats;
    int               rx_bytes;
    ...
    do {
        ...
        if ((rx_bytes == 0) || ((rx_bytes < 0) && ((errno == EAGAIN) || (errno == EWOULDBLOCK)))) stats->rx_eagain++;
    } while ((rx_bytes == 0) || ((rx_bytes < 0) && ((errno == EAGAIN) || (errno == EWOULDBLOCK))));
...
static inline int vcl_test_write(vcl_test_session_t *ts, void *buf, uint32_t nbytes)
{
    ...
    do {
        ...
        if (rv < 0) {
            errno = -rv;
            if ((errno == EAGAIN || errno == EWOULDBLOCK)) stats->tx_eagain++;
            break;
        }
        tx_bytes += rv;
        ...
        buf += rv;
        if (rv < nbytes_left) stats->tx_incomp++;
...
static inline int vcl_test_write_ds(vcl_test_session_t *ts)
{
    vcl_test_stats_t *stats = &ts->stats;
    int               tx_bytes;
    ...
    do {
        ...
        if (tx_bytes < 0) errno = -tx_bytes;
        if ((tx_bytes == 0) || ((tx_bytes < 0) && ((errno == EAGAIN) || (errno == EWOULDBLOCK)))) stats->rx_eagain++;
    } while ((tx_bytes == 0) || ((tx_bytes < 0) && ((errno == EAGAIN) || (errno == EWOULDBLOCK))));
...
static inline void dump_help(void)
{
#define INDENT "\n  "

    printf("CLIENT: Test configuration commands:" INDENT VCL_TEST_TOKEN_HELP "\t\t\tDisplay help." INDENT VCL_TEST_TOKEN_EXIT
           "\t\t\tExit test client & server." INDENT VCL_TEST_TOKEN_SHOW_CFG "\t\t\tShow the current test cfg." INDENT HS_TEST_TOKEN_RUN_UNI
           "\t\t\tRun the Uni-directional test." INDENT HS_TEST_TOKEN_RUN_BI "\t\t\tRun the Bi-directional test." INDENT VCL_TEST_TOKEN_VERBOSE
           "\t\t\tToggle verbose setting." INDENT VCL_TEST_TOKEN_RXBUF_SIZE "<rxbuf size>\tRx buffer size (bytes)." INDENT VCL_TEST_TOKEN_TXBUF_SIZE
