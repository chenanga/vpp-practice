...
static inline int sock_test_read(int fd, uint8_t* buf, uint32_t nbytes, vcl_test_stats_t* stats)
{
    int rx_bytes;

    do {
        if (stats) stats->rx_xacts++;
        rx_bytes = read(fd, buf, nbytes);
        if (stats) {
            if ((rx_bytes == 0) || ((rx_bytes < 0) && ((errno == EAGAIN) || (errno == EWOULDBLOCK))))
                stats->rx_eagain++;
    ...
    if (rx_bytes < 0) stfail("sock_test_read()");

    if (stats) stats->rx_bytes += rx_bytes;

    return (rx_bytes);
}

static inline int sock_test_write(int fd, uint8_t* buf, uint32_t nbytes, vcl_test_stats_t* stats, uint32_t verbose)
{
    int tx_bytes = 0, nbytes_left = nbytes, rv;

    do {
        if (stats) stats->tx_xacts++;
        rv = write(fd, buf, nbytes_left);
        if (rv < 0) {
            if ((errno == EAGAIN) || (errno == EWOULDBLOCK)) {
                if (stats) stats->tx_eagain++;
                continue;
            }
            else
                break;
    ...
    if (tx_bytes < 0) stfail("sock_test_write()");

    if (stats) stats->tx_bytes += tx_bytes;
