...
static hcs_session_t *hcs_session_alloc(u32 thread_index)
{
...
static hcs_session_t *hcs_session_get(u32 thread_index, u32 hs_index)
{
    hcs_main_t *hcm = &hcs_main;
    if (pool_is_free_index(hcm->sessions[thread_index], hs_index)) return 0;
    return pool_elt_at_index(hcm->sessions[thread_index], hs_index);
}

static void hcs_session_free(hcs_session_t *hs)
{
    hcs_main_t *hcm    = &hcs_main;
    u32         thread = hs->thread_index;
    if (CLIB_DEBUG) memset(hs, 0xfa, sizeof(*hs));
    pool_put(hcm->sessions[thread], hs);
}

static void hcs_cli_process_free(hcs_cli_args_t *args)
{
...
static void hcs_cli_output(uword arg, u8 *buffer, uword buffer_bytes)
{
...
static void start_send_data(hcs_session_t *hs, http_status_code_t status)
{
    http_msg_t msg;
    session_t *ts;
    u8        *headers_buf = 0;
...
static void send_data_to_http(void *rpc_args)
{
    ...
    hs->tx_buf = args->buf;
    if (args->plain_text) type = HTTP_CONTENT_TEXT_PLAIN;
...
static uword hcs_cli_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    session_send_rpc_evt_to_thread_force(args->thread_index, send_data_to_http, rpc_args);
...
static void alloc_cli_process(hcs_cli_args_t *args)
{
    hcs_main_t      *hcm = &hcs_main;
...
static void alloc_cli_process_callback(void *cb_args)
{
    alloc_cli_process((hcs_cli_args_t *) cb_args);
}

static int hcs_ts_rx_callback(session_t *ts)
{
    hcs_cli_args_t args = {};
    hcs_session_t *hs;
    http_msg_t     msg;
    ...
    if (msg.type != HTTP_MSG_REQUEST || msg.method_type != HTTP_REQ_GET) {
        http_add_header(&hs->resp_headers, http_header_name_token(HTTP_HEADER_ALLOW), http_token_lit("GET"));
    ...
    rv = svm_fifo_peek(ts->rx_fifo, msg.data.target_path_offset, msg.data.target_path_len, args.buf);
    ...
    if (msg.data.headers_len) {
        ...
        rv = svm_fifo_peek(ts->rx_fifo, msg.data.headers_offset, msg.data.headers_len, headers);
        ASSERT(rv == msg.data.headers_len);
        ...
        if (accept_value) {
            ...
            if (strstr(accept_value, "text/plain")) args.plain_text = 1;
        }
    ...
    if (vlib_get_thread_index() != 0)
        vlib_rpc_call_main_thread(alloc_cli_process_callback, (u8 *) &args, sizeof(args));
    else
...
static int hcs_ts_tx_callback(session_t *ts)
{
    hcs_session_t *hs;
    u32            to_send;
    int            rv;
...
static int hcs_ts_accept_callback(session_t *ts)
{
    hcs_session_t *hs;
...
static int hcs_ts_connected_callback(u32 app_index, u32 api_context, session_t *s, session_error_t err)
{
    clib_warning("called...");
    return -1;
}

static void hcs_ts_disconnect_callback(session_t *s)
{
    hcs_main_t            *hcm = &hcs_main;
...
static void hcs_ts_reset_callback(session_t *s)
{
    hcs_main_t            *hcm = &hcs_main;
    vnet_disconnect_args_t _a = {0}, *a = &_a;
...
static void hcs_ts_cleanup_callback(session_t *s, session_cleanup_ntf_t ntf)
{
    hcs_session_t *hs;
...
static int hcs_add_segment_callback(u32 client_index, u64 segment_handle)
{
    return 0;
}

static int hcs_del_segment_callback(u32 client_index, u64 segment_handle)
{
    return 0;
}
...
static int hcs_attach()
{
    ...
    if (hcm->private_segment_size) segment_size = hcm->private_segment_size;
    ...
    a->options[APP_OPTIONS_RX_FIFO_SIZE]        = hcm->fifo_size ? hcm->fifo_size : 8 << 10;
    a->options[APP_OPTIONS_TX_FIFO_SIZE]        = hcm->fifo_size ? hcm->fifo_size : 32 << 10;
...
static int hcs_create(vlib_main_t *vm)
{
...
static clib_error_t *hcs_create_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
...
start_server:
    ...
    if (hcm->app_index != (u32) ~0) return clib_error_return(0, "test http server is already running");
...
VLIB_CLI_COMMAND(hcs_create_command, static) = {
    ...
    .short_help =
        "http cli server [uri <uri>] [fifo-size <nbytes>] "
        "[private-segment-size <nMG>] [prealloc-fifos <n>]",
    .function = hcs_create_command_fn,
};

static clib_error_t *hcs_main_init(vlib_main_t *vm)
{
