...
static hcc_worker_t *hcc_worker_get(u32 thread_index)
{
    return vec_elt_at_index(hcc_main.wrk, thread_index);
}

static hcc_session_t *hcc_session_alloc(hcc_worker_t *wrk)
{
    hcc_session_t *hs;
...
static hcc_session_t *hcc_session_get(u32 hs_index, u32 thread_index)
{
    hcc_worker_t *wrk = hcc_worker_get(thread_index);
    return pool_elt_at_index(wrk->sessions, hs_index);
}

static int hcc_ts_accept_callback(session_t *ts)
{
    clib_warning("bug");
    return -1;
}

static void hcc_ts_disconnect_callback(session_t *s)
{
...
static int hcc_ts_connected_callback(u32 app_index, u32 hc_index, session_t *as, session_error_t err)
{
    hcc_main_t    *hcm = &hcc_main;
    ...
    if (err) {
        clib_warning("connected error: hc_index(%d): %U", hc_index, format_session_error, err);
    ...
    http_add_header(&hs->req_headers, http_header_name_token(HTTP_HEADER_ACCEPT), http_content_type_token(HTTP_CONTENT_TEXT_HTML));
    ...
    svm_fifo_seg_t segs[3] = {{(u8 *) &msg, sizeof(msg)}, {hcm->http_query, vec_len(hcm->http_query)}, {headers_buf, vec_len(headers_buf)}};
    ...
    if (svm_fifo_set_event(as->tx_fifo)) session_program_tx_io_evt(as->handle, SESSION_IO_EVT_TX);

    return 0;
}

static void hcc_ts_reset_callback(session_t *s)
{
    hcc_main_t            *hcm = &hcc_main;
    hcc_session_t         *hs;
    vnet_disconnect_args_t _a = {0}, *a = &_a;
...
static int hcc_ts_tx_callback(session_t *ts)
{
    clib_warning("bug");
    return -1;
}

static void hcc_session_disconnect(session_t *s)
{
    hcc_main_t            *hcm = &hcc_main;
    vnet_disconnect_args_t _a = {0}, *a = &_a;
    a->handle    = session_handle(s);
...
static int hcc_ts_rx_callback(session_t *ts)
{
    hcc_main_t    *hcm = &hcc_main;
    hcc_session_t *hs;
    http_msg_t     msg;
    ...
    if (hs->to_recv == 0) {
        ...
        if (msg.code != HTTP_STATUS_OK && hs->to_recv == 0) {
            hcm->http_response = format(0, "request failed, response code: %U", format_http_status_code, msg.code);
            goto done;
        }
    ...
    if (hs->to_recv == 0) {
        ...
        vlib_process_signal_event_mt(hcm->vlib_main, hcm->cli_node_index, HCC_REPLY_RECEIVED, 0);
    }

    return 0;
}

static void hcc_ts_transport_closed(session_t *s)
{
    hcc_main_t *hcm = &hcc_main;

    HCC_DBG("transport closed");

    vlib_process_signal_event_mt(hcm->vlib_main, hcm->cli_node_index, HCC_TRANSPORT_CLOSED, 0);
}
...
static clib_error_t *hcc_attach()
{
    hcc_main_t            *hcm = &hcc_main;
...
static int hcc_connect_rpc(void *rpc_args)
{
    vnet_connect_args_t *a = rpc_args;
    int                  rv;

    rv = vnet_connect(a);
    if (rv) clib_warning(0, "connect returned: %U", format_session_error, rv);

    vec_free(a);
    return rv;
}

static void hcc_program_connect(vnet_connect_args_t *a)
{
    session_send_rpc_evt_to_thread_force(transport_cl_thread(), hcc_connect_rpc, a);
}

static clib_error_t *hcc_connect()
{
...
static clib_error_t *hcc_run(vlib_main_t *vm, int print_output)
{
    ...
    if ((err = hcc_attach())) {
        return clib_error_return(0, "http client attach: %U", format_clib_error, err);
    }

    if ((err = hcc_connect())) {
        return clib_error_return(0, "http client connect: %U", format_clib_error, err);
    }
...
static int hcc_detach()
{
    hcc_main_t            *hcm = &hcc_main;
    vnet_app_detach_args_t _da, *da = &_da;
    int                    rv;

    if (!hcm->test_client_attached) return 0;
...
static void hcc_worker_cleanup(hcc_worker_t *wrk)
{
    pool_free(wrk->sessions);
}

static void hcc_cleanup()
{
    hcc_main_t   *hcm = &hcc_main;
    hcc_worker_t *wrk;
...
static clib_error_t *hcc_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (hcm->test_client_attached) return clib_error_return(0, "failed: already running!");

    /* Get a line of input. */
    if (!unformat_user(input, unformat_line_input, line_input)) return clib_error_return(0, "expected URI");
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "prealloc-fifos %d", &hcm->prealloc_fifos))
            ...
            err = clib_error_return(0, "unknown input `%U'", format_unformat_error, line_input);
            goto done;
        }
    }
    ...
    if (hcc_detach()) {
        ...
        if (!err) err = clib_error_return(0, "failed: app detach");
...
VLIB_CLI_COMMAND(hcc_command, static) = {
    ...
    .short_help =
        "[appns <app-ns> secret <appns-secret>] uri http://<ip-addr> "
        "query <query-string> [no-output]",
    .function   = hcc_command_fn,
    .is_mp_safe = 1,
};

static clib_error_t *hcc_main_init(vlib_main_t *vm)
{
    hcc_main_t *hcm = &hcc_main;
