...
static inline u16 hash_uword_to_u16(uword *key)
{
    u16 *val;
    val = (u16 *) key;
#if uword_bits == 64
...
static inline u8 gtpu_type_get(u16 tag)
{
    u16 val;
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            ls0 = pool_elt_at_index(sm2->localsids, vnet_buffer(b0)->ip.adj_index[VLIB_TX]);
            ...
            if ((ip6srv0->ip.protocol == IPPROTO_IPV6_ROUTE && len0 < sizeof(ip6srv_combo_header_t) + ip6srv0->sr.length * 8)
                ...
                if (PREDICT_TRUE(shift == 0)) {
                    ...
                    if (gtpu_type == GTPU_TYPE_ECHO_REQUEST || gtpu_type == GTPU_TYPE_ECHO_REPLY || gtpu_type == GTPU_TYPE_ERROR_INDICATION) {
                ...
                else if (gtpu_type == GTPU_TYPE_ECHO_REQUEST || gtpu_type == GTPU_TYPE_ECHO_REPLY || gtpu_type == GTPU_TYPE_ERROR_INDICATION) {
                ...
                if (PREDICT_FALSE(gtpu_type == GTPU_TYPE_ERROR_INDICATION)) {
                    ...
                    if (ext_len > sizeof(ip6_address_t) * (ip6srv0->sr.last_entry + 1)) {
                        tlv = (ip6_sr_tlv_t *) ((u8 *) &ip6srv0->sr + sizeof(ip6_sr_header_t) + sizeof(ip6_address_t) * (ip6srv0->sr.last_entry + 1));
                ...
                if (ip6srv0->ip.protocol == IPPROTO_IPV6_ROUTE) {
                    vlib_buffer_advance(b0, (word) sizeof(ip6srv_combo_header_t) + ip6srv0->sr.length * 8);
                }
                else {
                ...
                if (gtpu_type == GTPU_TYPE_ECHO_REPLY || gtpu_type == GTPU_TYPE_ECHO_REQUEST || gtpu_type == GTPU_TYPE_ERROR_INDICATION) {
                ...
                hdr0->udp.length = clib_host_to_net_u16(len0 + sizeof(udp_header_t) + sizeof(gtpu_header_t));

                hdr0->ip4.length = clib_host_to_net_u16(len0 + sizeof(ip4_gtpu_header_t));
                ...
                if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE) && PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                    srv6_end_rewrite_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
                    clib_memcpy(tr->src.as_u8, hdr0->ip4.src_address.as_u8, sizeof(hdr0->ip4.src_address.as_u8));
                    clib_memcpy(tr->dst.as_u8, hdr0->ip4.dst_address.as_u8, sizeof(hdr0->ip4.dst_address.as_u8));
                    tr->teid = hdr0->gtpu.teid;
                }
            }

            vlib_increment_combined_counter(((next0 == SRV6_END_M_GTP4_E_NEXT_DROP) ? &(sm2->sr_ls_invalid_counters) : &(sm2->sr_ls_valid_counters)),
                                            thread_index,
                                            ls0 - sm2->localsids,
                                            1,
    ...
    vlib_node_increment_counter(vm, sm->end_m_gtp4_e_node_index, SRV6_END_ERROR_M_GTP4_E_BAD_PACKETS, bad_n);

    vlib_node_increment_counter(vm, sm->end_m_gtp4_e_node_index, SRV6_END_ERROR_M_GTP4_E_PACKETS, good_n);

    return frame->n_vectors;
}

// Function for SRv6 GTP4.D function.
static inline u32 srv6_gtp4_decap_processing(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t *b0)
{
    ...
    sl0 = pool_elt_at_index(sm2->sid_lists, vnet_buffer(b0)->ip.adj_index[VLIB_TX]);
    ...
    if (ip4->protocol != IP_PROTOCOL_UDP || len0 < sizeof(ip4_gtpu_header_t)) {
        ...
        if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE) && PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
            srv6_end_rewrite_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
            clib_memcpy(tr->src.as_u8, ip6srv->ip.src_address.as_u8, sizeof(tr->src.as_u8));
            clib_memcpy(tr->dst.as_u8, ip6srv->ip.dst_address.as_u8, sizeof(tr->dst.as_u8));
        }
    }
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(gtpu_type != GTPU_TYPE_GTPU || ls_param->drop_in)) {
                ...
                if ((clib_net_to_host_u32(ip6->ip_version_traffic_class_and_flow_label) >> 28) == 6) {
                    ipv4 = false;
                    if (((ip6->dst_address.as_u8[0] == 0xff) && (ip6->dst_address.as_u8[1] == 0x02))
                        || ((ip6->dst_address.as_u8[0] == 0xfe) && ((ip6->dst_address.as_u8[1] & 0xc0) == 0x80))) {
    ...
    vlib_node_increment_counter(vm, sm->t_m_gtp4_d_node_index, SRV6_T_ERROR_M_GTP4_D_BAD_PACKETS, bad_n);

    vlib_node_increment_counter(vm, sm->t_m_gtp4_d_node_index, SRV6_T_ERROR_M_GTP4_D_PACKETS, good_n);
...
VLIB_REGISTER_NODE(srv6_t_m_gtp4_d) = {
    .name          = "srv6-t-m-gtp4-d",
    .vector_size   = sizeof(u32),
    .format_trace  = format_srv6_end_rewrite_trace,
    .type          = VLIB_NODE_TYPE_INTERNAL,
    .n_errors      = ARRAY_LEN(srv6_t_error_v4_d_strings),
    .error_strings = srv6_t_error_v4_d_strings,
    .n_next_nodes  = SRV6_T_M_GTP4_D_N_NEXT,
    .next_nodes =
        {
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if ((ip6srv0->ip.protocol != IPPROTO_IPV6_ROUTE) || (len0 < sizeof(ip6srv_combo_header_t) + 8 * ip6srv0->sr.length)) {
                ...
                if (PREDICT_TRUE(shift == 0)) {
                    ...
                    if (gtpu_type == GTPU_TYPE_ECHO_REQUEST || gtpu_type == GTPU_TYPE_ECHO_REPLY || gtpu_type == GTPU_TYPE_ERROR_INDICATION) {
                        ...
                        for (index = 0; index < 2; index++) {
                            ...
                            sp[index] |= dst0.as_u8[offset + index + 2] >> (8 - shift);
                        }
                    }
                    else {
                        for (index = 0; index < 4; index++) {
                ...
                clib_memcpy_fast(hdr0->ip6.src_address.as_u8, src0.as_u8, sizeof(ip6_address_t));
                clib_memcpy_fast(hdr0->ip6.dst_address.as_u8, &seg0.as_u8, sizeof(ip6_address_t));

                hdr0->ip6.payload_length = clib_host_to_net_u16(len0 + sizeof(udp_header_t) + sizeof(gtpu_header_t));
                ...
                if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE) && PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                    srv6_end_rewrite_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
                    clib_memcpy(tr->src.as_u8, hdr0->ip6.src_address.as_u8, sizeof(ip6_address_t));
                    clib_memcpy(tr->dst.as_u8, hdr0->ip6.dst_address.as_u8, sizeof(ip6_address_t));
                    tr->teid = hdr0->gtpu.teid;
                }
            }

            vlib_increment_combined_counter(((next0 == SRV6_END_M_GTP6_E_NEXT_DROP) ? &(sm2->sr_ls_invalid_counters) : &(sm2->sr_ls_valid_counters)),
                                            thread_index,
                                            ls0 - sm2->localsids,
                                            1,
                                            vlib_buffer_length_in_chain(vm, b0));
    ...
    vlib_node_increment_counter(vm, sm->end_m_gtp6_e_node_index, SRV6_END_ERROR_M_GTP6_E_BAD_PACKETS, bad_n);

    vlib_node_increment_counter(vm, sm->end_m_gtp6_e_node_index, SRV6_END_ERROR_M_GTP6_E_PACKETS, good_n);

    return frame->n_vectors;
}

// Function for SRv6 GTP6.D function
static inline u32 srv6_gtp6_decap_processing(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t *b0)
{
    ...
    if ((hdr0->ip6.protocol != IP_PROTOCOL_UDP) || (hdr0->udp.dst_port != clib_host_to_net_u16(SRV6_GTP_UDP_DST_PORT))
        ...
        if (sl) {
            ...
            if (PREDICT_FALSE(gtpu_type) != GTPU_TYPE_GTPU) {
                ...
                if (ls_param->nhtype == SRV6_NHTYPE_NONE) {
                    if ((clib_net_to_host_u32(encap->ip_version_traffic_class_and_flow_label) >> 28) != 6) ip6srv->sr.protocol = IP_PROTOCOL_IP_IN_IP;
                }
                else if (ls_param->nhtype == SRV6_NHTYPE_IPV4) {
                    ip6srv->sr.protocol = IP_PROTOCOL_IP_IN_IP;
                    if ((clib_net_to_host_u32(encap->ip_version_traffic_class_and_flow_label) >> 28) != 4) {
        ...
        if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE) && PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
            srv6_end_rewrite_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
            clib_memcpy(tr->src.as_u8, ip6srv->ip.src_address.as_u8, sizeof(ip6_address_t));
            clib_memcpy(tr->dst.as_u8, ip6srv->ip.dst_address.as_u8, sizeof(ip6_address_t));
            tr->teid = teid;
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (gtp6) {
                ...
                vlib_increment_combined_counter(
                    ((next0 == SRV6_END_M_GTP6_D_NEXT_DROP) ? &(sm2->sr_ls_invalid_counters) : &(sm2->sr_ls_valid_counters)),
                    thread_index,
                    ls0 - sm2->localsids,
                    1,
                    vlib_buffer_length_in_chain(vm, b0));
    ...
    vlib_node_increment_counter(vm, sm->end_m_gtp6_d_node_index, SRV6_END_ERROR_M_GTP6_D_BAD_PACKETS, bad_n);

    vlib_node_increment_counter(vm, sm->end_m_gtp6_d_node_index, SRV6_END_ERROR_M_GTP6_D_PACKETS, good_n);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if ((hdr0->ip6.protocol != IP_PROTOCOL_UDP) || (hdr0->udp.dst_port != clib_host_to_net_u16(SRV6_GTP_UDP_DST_PORT))
                ...
                if (sl) hdr_len += vec_len(sl->segments) * sizeof(ip6_address_t);
        ...
        DONE:
            vlib_increment_combined_counter(
                ((next0 == SRV6_END_M_GTP6_D_DI_NEXT_DROP) ? &(sm2->sr_ls_invalid_counters) : &(sm2->sr_ls_valid_counters)),
                thread_index,
                ls0 - sm2->localsids,
                1,
                vlib_buffer_length_in_chain(vm, b0));
    ...
    vlib_node_increment_counter(vm, sm->end_m_gtp6_d_di_node_index, SRV6_END_ERROR_M_GTP6_D_DI_BAD_PACKETS, bad_n);

    vlib_node_increment_counter(vm, sm->end_m_gtp6_d_di_node_index, SRV6_END_ERROR_M_GTP6_D_DI_PACKETS, good_n);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if ((hdr0->ip6.protocol != IP_PROTOCOL_UDP) || (hdr0->udp.dst_port != clib_host_to_net_u16(SRV6_GTP_UDP_DST_PORT))
                ...
                clib_memcpy_fast(src.as_u8, hdr0->ip6.src_address.as_u8, sizeof(ip6_address_t));
                clib_memcpy_fast(dst.as_u8, hdr0->ip6.dst_address.as_u8, sizeof(ip6_address_t));
                ...
                if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE) && PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                    srv6_end_rewrite_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
                    clib_memcpy(tr->src.as_u8, src.as_u8, sizeof(ip6_address_t));
                    clib_memcpy(tr->dst.as_u8, dst.as_u8, sizeof(ip6_address_t));
                    tr->teid = teid;
                }
            }

        DONE:
            vlib_increment_combined_counter(((next0 == SRV6_END_M_GTP6_DT_NEXT_DROP) ? &(sm2->sr_ls_invalid_counters) : &(sm2->sr_ls_valid_counters)),
                                            thread_index,
                                            ls0 - sm2->localsids,
                                            1,
                                            vlib_buffer_length_in_chain(vm, b0));
    ...
    vlib_node_increment_counter(vm, sm->end_m_gtp6_dt_node_index, SRV6_END_ERROR_M_GTP6_DT_BAD_PACKETS, bad_n);

    vlib_node_increment_counter(vm, sm->end_m_gtp6_dt_node_index, SRV6_END_ERROR_M_GTP6_DT_PACKETS, good_n);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            b0  = vlib_get_buffer(vm, bi0);
            ls0 = pool_elt_at_index(sm2->sid_lists, vnet_buffer(b0)->ip.adj_index[VLIB_TX]);
            ...
            if ((hdr0->ip4.protocol != IP_PROTOCOL_UDP) || (hdr0->udp.dst_port != clib_host_to_net_u16(SRV6_GTP_UDP_DST_PORT))
                ...
                clib_memcpy_fast(src.as_u8, hdr0->ip4.src_address.as_u8, sizeof(ip4_address_t));
                clib_memcpy_fast(dst.as_u8, hdr0->ip4.dst_address.as_u8, sizeof(ip4_address_t));
    ...
    vlib_node_increment_counter(vm, sm->t_m_gtp4_dt_node_index, SRV6_T_ERROR_M_GTP4_DT_BAD_PACKETS, bad_n);

    vlib_node_increment_counter(vm, sm->t_m_gtp4_dt_node_index, SRV6_T_ERROR_M_GTP4_DT_PACKETS, good_n);
...
VLIB_REGISTER_NODE(srv6_end_m_gtp6_dt) = {
    .name          = "srv6-end-m-gtp6-dt",
    .vector_size   = sizeof(u32),
    .format_trace  = format_srv6_end_rewrite_trace6,
    .type          = VLIB_NODE_TYPE_INTERNAL,
    .n_errors      = ARRAY_LEN(srv6_end_error_v6_dt_strings),
    .error_strings = srv6_end_error_v6_dt_strings,
    .n_next_nodes  = SRV6_END_M_GTP6_DT_N_NEXT,
    .next_nodes =
        {
...
VLIB_REGISTER_NODE(srv6_t_m_gtp4_dt) = {
    .name          = "srv6-t-m-gtp4-dt",
    .vector_size   = sizeof(u32),
    .format_trace  = format_srv6_end_rewrite_trace6,
    .type          = VLIB_NODE_TYPE_INTERNAL,
    .n_errors      = ARRAY_LEN(srv6_t_error_v4_dt_strings),
    .error_strings = srv6_t_error_v4_dt_strings,
    .n_next_nodes  = SRV6_T_M_GTP4_DT_N_NEXT,
    .next_nodes =
        {
