...
static uword clb_unformat_srv6_end_m_gtp4_e(unformat_input_t *input, va_list *args)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(input, "end.m.gtp4.e v4src_addr %U fib-table %d", unformat_ip4_address, &v4src_addr, &fib_table)) {
            config = true;
        }
        else {
            return 0;
    ...
    alloc_param_srv6_end_m_gtp4_e(plugin_mem_p, &v4src_addr, v4src_position, fib_table);
...
static clib_error_t *srv6_end_m_gtp4_e_init(vlib_main_t *vm)
{
    ...
    if (rc < 0)
        clib_error_return(0,
                          "SRv6 Endpoint GTP4.E LocalSID function"
