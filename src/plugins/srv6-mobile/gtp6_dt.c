...
static uword clb_unformat_srv6_end_m_gtp6_dt(unformat_input_t *input, va_list *args)
{
    ...
    else if (unformat(input, "end.m.gtp6.dt6 fib-table %u local-fib-table %u", &fib_index, &local_fib_index)) {
        type = SRV6_GTP6_DT6;
    }
    else if (unformat(input, "end.m.gtp6.dt46 fib-table %u local-fib-table %u", &fib_index, &local_fib_index)) {
        type = SRV6_GTP6_DT46;
    }
    else {
        return 0;
    }
    alloc_param_srv6_end_m_gtp6_dt(plugin_mem_p, fib_index, local_fib_index, type);
    return 1;
}
...
static clib_error_t *srv6_end_m_gtp6_dt_init(vlib_main_t *vm)
{
    ...
    if (rc < 0)
        clib_error_return(0,
                          "SRv6 Endpoint GTP6.DT LocalSID function"
