...
static u8 param_str[]   = "<sr-prefix>/<sr-prefixlen> [nhtype <nhtype>] fib-table <id>";
...
void alloc_param_srv6_end_m_gtp6_d(void      **plugin_mem_p,
                                   const void *sr_prefix,
                                   const u32   sr_prefixlen,
                                   const u8    nhtype,
                                   const bool  drop_in,
                                   const u32   fib_table)
{
...
static uword clb_unformat_srv6_end_m_gtp6_d(unformat_input_t *input, va_list *args)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "end.m.gtp6.d %U/%d nh-type ipv4 fib-table %d", unformat_ip6_address, &sr_prefix, &sr_prefixlen, &fib_table)) {
            config = true;
            nhtype = SRV6_NHTYPE_IPV4;
        }
        else if (unformat(input, "end.m.gtp6.d %U/%d nh-type ipv6 fib-table %d", unformat_ip6_address, &sr_prefix, &sr_prefixlen, &fib_table)) {
            config = true;
            nhtype = SRV6_NHTYPE_IPV6;
        }
        else if (unformat(input, "end.m.gtp6.d %U/%d nh-type none", unformat_ip6_address, &sr_prefix, &sr_prefixlen)) {
            config = true;
            nhtype = SRV6_NHTYPE_NON_IP;
        }
        else if (unformat(input, "end.m.gtp6.d %U/%d fib-table %d", unformat_ip6_address, &sr_prefix, &sr_prefixlen, &fib_table)) {
            config = true;
    ...
    alloc_param_srv6_end_m_gtp6_d(plugin_mem_p, &sr_prefix, sr_prefixlen, nhtype, drop_in, fib_table);
...
static int clb_creation_srv6_end_m_gtp6_d_2(ip6_sr_policy_t *sr_policy)
{
    return 0;
}
...
static int clb_removal_srv6_end_m_gtp6_d_2(ip6_sr_policy_t *sr_policy)
{
    srv6_end_gtp6_d_param_t *ls_mem;
...
static clib_error_t *srv6_end_m_gtp6_d_init(vlib_main_t *vm)
{
    ...
    if (rc < 0)
        clib_error_return(0,
                          "SRv6 Endpoint GTP6.D LocalSID function"
                          "couldn't be registered");

    rc = sr_policy_register_function(vm,
                                     fn_name,
                                     keyword_str,
                                     def_str,
                                     param_str,
