...
static uword clb_unformat_srv6_t_m_gtp4_dt(unformat_input_t *input, va_list *args)
{
    ...
    else if (unformat(input, "t.m.gtp4.dt6 fib-table %u local-fib-table %u", &fib_index, &local_fib_index)) {
        type = SRV6_GTP4_DT6;
    }
    else if (unformat(input, "t.m.gtp4.dt46 fib-table %u local-fib-table %u", &fib_index, &local_fib_index)) {
        type = SRV6_GTP4_DT46;
    }
    else {
        return 0;
    ...
    alloc_param_srv6_t_m_gtp4_dt(plugin_mem_p, fib_index, local_fib_index, type);
...
static clib_error_t *srv6_t_m_gtp4_dt_init(vlib_main_t *vm)
{
    ...
    if (rc < 0)
        clib_error_return(0,
                          "SRv6 Transit GTP4.DT Policy function"
