...
void alloc_param_srv6_end_m_gtp6_di(void **plugin_mem_p, const void *sr_prefix, const u32 sr_prefixlen, const u8 nhtype)
{
...
static uword clb_unformat_srv6_end_m_gtp6_d_di(unformat_input_t *input, va_list *args)
{
    ...
    if (unformat(input, "end.m.gtp6.d.di %U/%d nhtype ipv4", unformat_ip6_address, &sr_prefix, &sr_prefixlen)) {
        nhtype = SRV6_NHTYPE_IPV4;
    }
    else if (unformat(input, "end.m.gtp6.d.di %U/%d nhtype ipv6", unformat_ip6_address, &sr_prefix, &sr_prefixlen)) {
        nhtype = SRV6_NHTYPE_IPV6;
    }
    else if (unformat(input, "end.m.gtp6.d.di %U/%d nhtype non-ip", unformat_ip6_address, &sr_prefix, &sr_prefixlen)) {
        nhtype = SRV6_NHTYPE_NON_IP;
    }
    else if (unformat(input, "end.m.gtp6.d.di %U/%d", unformat_ip6_address, &sr_prefix, &sr_prefixlen)) {
    ...
    alloc_param_srv6_end_m_gtp6_di(plugin_mem_p, &sr_prefix, sr_prefixlen, nhtype);
...
static clib_error_t *srv6_end_m_gtp6_d_di_init(vlib_main_t *vm)
{
    ...
    if (rc < 0)
        clib_error_return(0,
                          "SRv6 Endpoint GTP6.D.DI LocalSID function"
