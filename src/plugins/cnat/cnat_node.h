                                         ...
                                         u8                        flags)
{
    ...
    if (NULL != ct) flags |= CNAT_TRACE_TRANSLATION_FOUND;

    t                       = vlib_add_trace(vm, node, b, sizeof(*t));
    t->sw_if_index[VLIB_RX] = vnet_buffer(b)->sw_if_index[VLIB_RX];
    t->sw_if_index[VLIB_TX] = vnet_buffer(b)->sw_if_index[VLIB_TX];

    if (flags & (CNAT_TRACE_SESSION_FOUND | CNAT_TRACE_SESSION_CREATED)) clib_memcpy(&t->session, session, sizeof(t->session));
    if (flags & CNAT_TRACE_TRANSLATION_FOUND) clib_memcpy(&t->tr, ct, sizeof(cnat_translation_t));
    t->flags = flags;
}
...
static u8 *format_cnat_trace(u8 *s, va_list *args)
{
    ...
    if (t->flags & (CNAT_TRACE_NO_NAT)) s = format(s, " [policy:skip]");
    ...
    s = format(s,
               ...
               vnm,
               t->sw_if_index[VLIB_RX],
               format_vnet_sw_if_index_name,
               vnm,
               t->sw_if_index[VLIB_TX]);

    if (t->flags & (CNAT_TRACE_SESSION_CREATED | CNAT_TRACE_SESSION_FOUND))
        s = format(s, "\n%U%U", format_white_space, indent, format_cnat_session, &t->session, 1);

    if (t->flags & CNAT_TRACE_TRANSLATION_FOUND) s = format(s, "\n%Utranslation: %U", format_white_space, indent, format_cnat_translation, &t->tr, 0);
...
static_always_inline u16 ip4_pseudo_header_cksum2(ip4_header_t *ip4, ip4_address_t address[VLIB_N_DIR])
{
    ip4_psh_t psh = {0};
    psh.src       = address[VLIB_RX];
    psh.dst       = address[VLIB_TX];
    psh.proto     = ip4->protocol;
    psh.l4len     = clib_host_to_net_u16(clib_net_to_host_u16(ip4->length) - sizeof(ip4_header_t));
                                                ...
                                                u32           oflags)
{
    ...
    *sum = ip_csum_update(*sum, ip4->dst_address.as_u32, new_addr[VLIB_TX].as_u32, ip4_header_t, dst_address);
    *sum = ip_csum_update(*sum, ip4->src_address.as_u32, new_addr[VLIB_RX].as_u32, ip4_header_t, src_address);

    *sum = ip_csum_update(*sum, old_port[VLIB_TX], new_port[VLIB_TX], udp_header_t, dst_port);
    *sum = ip_csum_update(*sum, old_port[VLIB_RX], new_port[VLIB_RX], udp_header_t, src_port);
}

static_always_inline void cnat_ip4_translate_sctp(ip4_header_t *ip4, sctp_header_t *sctp, u16 new_port[VLIB_N_DIR])
{
    ...
    if (new_port[VLIB_TX]) sctp->dst_port = new_port[VLIB_TX];
    if (new_port[VLIB_RX]) sctp->src_port = new_port[VLIB_RX];

    sctp->checksum = 0;
    sctp->checksum = clib_host_to_little_u32(~clib_crc32c_with_init((u8 *) sctp, ntohs(ip4->length) - sizeof(ip4_header_t), ~0 /* init value */));
}

static_always_inline void cnat_ip4_translate_l3(ip4_header_t *ip4, ip4_address_t new_addr[VLIB_N_DIR], u32 oflags)
{
    ...
    // downstream.
    sum           = ip4->checksum;
    sum           = ip_csum_update(sum, old_addr[VLIB_TX].as_u32, new_addr[VLIB_TX].as_u32, ip4_header_t, dst_address);
    sum           = ip_csum_update(sum, old_addr[VLIB_RX].as_u32, new_addr[VLIB_RX].as_u32, ip4_header_t, src_address);
...
static_always_inline void cnat_tcp_update_session_lifetime(tcp_header_t *tcp, u32 index)
{
    ...
    if (PREDICT_FALSE(tcp_fin(tcp))) cnat_timestamp_set_lifetime(index, CNAT_DEFAULT_TCP_RST_TIMEOUT);

    if (PREDICT_FALSE(tcp_rst(tcp))) cnat_timestamp_set_lifetime(index, CNAT_DEFAULT_TCP_RST_TIMEOUT);

    if (PREDICT_FALSE(tcp_syn(tcp) && tcp_ack(tcp))) cnat_timestamp_set_lifetime(index, cm->tcp_max_age);
}
                                                      ...
                                                      u32              oflags)
{
    ...
    sum = icmp->checksum;
    sum = ip_csum_update(sum, old_port, new_port[VLIB_RX], udp_header_t, src_port);
                                                       ...
                                                       u32              oflags)
{
    ...
    /* translate outer ip. */
    if (!snat_outer_ip) outer_new_addr[VLIB_RX] = outer_ip4->src_address;
    cnat_ip4_translate_l3(outer_ip4, outer_new_addr, oflags);

    if (ip4->protocol == IP_PROTOCOL_TCP) {
        inner_l4_old_sum = inner_l4_sum = tcp->checksum;
        cnat_ip4_translate_l4(ip4, udp, &inner_l4_sum, new_addr, new_port, 0 /* flags */);
    ...
    sum = ip_csum_update(sum, inner_l4_old_sum, inner_l4_sum, ip4_header_t, checksum);

    /* UDP/TCP Ports changed */
    if (old_port[VLIB_TX] && new_port[VLIB_TX]) sum = ip_csum_update(sum, old_port[VLIB_TX], new_port[VLIB_TX], udp_header_t, dst_port);

    if (old_port[VLIB_RX] && new_port[VLIB_RX]) sum = ip_csum_update(sum, old_port[VLIB_RX], new_port[VLIB_RX], udp_header_t, src_port);
...
static_always_inline void cnat_translation_ip4(const cnat_session_t *session, ip4_header_t *ip4, udp_header_t *udp, u32 oflags)
{
    tcp_header_t *tcp = (tcp_header_t *) udp;
    ip4_address_t new_addr[VLIB_N_DIR];
    u16           new_port[VLIB_N_DIR];
    ...
    if (ip4->protocol == IP_PROTOCOL_TCP) {
        ...
        if (icmp_type_is_error_message(icmp->type)) {
            ...
            u8 snat_outer_ip = (ip4->src_address.as_u32 == session->key.cs_ip[VLIB_RX].ip4.as_u32);
            cnat_translation_icmp4_error(ip4, icmp, new_addr, new_port, snat_outer_ip, oflags);
        }
...
static_always_inline u16 ip6_pseudo_header_cksum2(ip6_header_t *ip6, ip6_address_t address[VLIB_N_DIR])
{
    ip6_psh_t psh = {0};
    psh.src       = address[VLIB_RX];
    psh.dst       = address[VLIB_TX];
                                                       ...
                                                       u8               snat_outer_ip)
{
    ...
    if (!icmp6_type_is_error_message(icmp->type)) return;
    ...
    if (!snat_outer_ip) ip6_address_copy(&outer_new_addr[VLIB_RX], &outer_ip6->src_address);
    ...
    if (ip6->protocol == IP_PROTOCOL_TCP) {
        inner_l4_old_sum = inner_l4_sum = tcp->checksum;
        cnat_ip6_translate_l4(ip6, udp, &inner_l4_sum, new_addr, new_port, 0 /* oflags */);
        tcp->checksum = ip_csum_fold(inner_l4_sum);
    }
...
static_always_inline void cnat_translation_ip6(const cnat_session_t *session, ip6_header_t *ip6, udp_header_t *udp, u32 oflags)
{
    tcp_header_t *tcp = (tcp_header_t *) udp;
    ip6_address_t new_addr[VLIB_N_DIR];
    u16           new_port[VLIB_N_DIR];
    ...
    if (ip6->protocol == IP_PROTOCOL_TCP) {
        ...
        if (icmp6_type_is_error_message(icmp->type)) {
            /* SNAT only if src_addr was translated */
            u8 snat_outer_ip = cmp_ip6_address(&ip6->src_address, &session->key.cs_ip[VLIB_RX].ip6);
            cnat_translation_icmp6_error(ip6, icmp, new_addr, new_port, snat_outer_ip);
        }
...
static_always_inline void cnat_session_make_key(vlib_buffer_t *b, ip_address_family_t af, cnat_session_location_t cs_loc, cnat_bihash_kv_t *bkey)
{
    ...
    if (cs_loc == CNAT_LOCATION_OUTPUT) /* rewind buffer */
    ...
    if (AF_IP4 == af) {
        ...
        if (PREDICT_FALSE(ip4->protocol == IP_PROTOCOL_ICMP)) {
            ...
            if (icmp_type_is_error_message(icmp->type)) {
                ...
                ip46_address_set_ip4(&session->key.cs_ip[VLIB_RX], &ip4->dst_address);
                ip46_address_set_ip4(&session->key.cs_ip[VLIB_TX], &ip4->src_address);
        ...
        if (PREDICT_FALSE(ip6->protocol == IP_PROTOCOL_ICMP6)) {
            ...
            if (icmp6_type_is_error_message(icmp->type)) {
                ...
                /* Swap dst & src for search as ICMP payload is reversed */
                ip46_address_set_ip6(&session->key.cs_ip[VLIB_RX], &ip6->dst_address);
                ip46_address_set_ip6(&session->key.cs_ip[VLIB_TX], &ip6->src_address);
                                                      ...
                                                      u32                      *dpoi_index)
{
    ...
    if (PREDICT_FALSE(!lb0->lb_n_buckets)) return (NULL);

    /* session table miss */
    hash_c0 = (AF_IP4 == af ? ip4_compute_flow_hash(ip4, lb0->lb_hash_config) : ip6_compute_flow_hash(ip6, lb0->lb_hash_config));
...
static_always_inline void cnat_session_create(cnat_session_t *session, cnat_node_ctx_t *ctx)
{
...
static_always_inline void cnat_rsession_create(cnat_session_t         *session,
                                               cnat_node_ctx_t        *ctx,
                                               ...
                                               cnat_session_flag_t     rsession_flags)
{
    ...
    ip46_address_copy(&rsession->key.cs_ip[VLIB_RX], &session->value.cs_ip[VLIB_TX]);
    ip46_address_copy(&rsession->key.cs_ip[VLIB_TX], &session->value.cs_ip[VLIB_RX]);
    ...
    ip46_address_copy(&rsession->value.cs_ip[VLIB_RX], &session->key.cs_ip[VLIB_TX]);
    ip46_address_copy(&rsession->value.cs_ip[VLIB_TX], &session->key.cs_ip[VLIB_RX]);
...
retry_add_ression:
    rv = cnat_bihash_add_del(&cnat_session_db, &rkey, 2 /* add but don't overwrite */);
    if (rv) {
        if (!(rsession_flags & CNAT_SESSION_RETRY_SNAT)) return;

        /* return session add failed pick an new random src port */
        rsession->value.cs_port[VLIB_TX] = session->key.cs_port[VLIB_RX] = random_u32(&sport_seed);
        if (n_retries++ < 100)
    ...
    if (!(rsession_flags & CNAT_SESSION_FLAG_NO_CLIENT)) {
        ...
        cc =
            (AF_IP4 == ctx->af ? cnat_client_ip4_find(&session->value.cs_ip[VLIB_RX].ip4) : cnat_client_ip6_find(&session->value.cs_ip[VLIB_RX].ip6));
        ...
        if (NULL == cc) {
            ...
            if (!p) vl_api_rpc_call_main_thread(cnat_client_learn, (u8 *) &addr, sizeof(addr));
        }
        else {
...
always_inline uword cnat_node_inline(vlib_main_t            *vm,
                                     vlib_node_runtime_t    *node,
                                     vlib_frame_t           *frame,
