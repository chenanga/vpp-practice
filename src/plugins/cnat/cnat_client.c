...
{
    cnat_client_t *cc;
    cc = (AF_IP4 == af ? cnat_client_ip4_find(&ip->ip4) : cnat_client_ip6_find(&ip->ip6));
    ASSERT(NULL != cc);

    if (0 == cnat_client_uncnt_session(cc) && 0 == cc->tr_refcnt) cnat_client_destroy(cc);
}
...
void cnat_client_throttle_pool_process()
{
    ...
    hash_foreach_mem(addr, refcnt, cnat_client_db.throttle_mem, {
        cc = (AF_IP4 == addr->version ? cnat_client_ip4_find(&ip_addr_v4(addr)) : cnat_client_ip6_find(&ip_addr_v6(addr)));
...
void cnat_client_translation_deleted(index_t cci)
{
    ...
    if (0 == cc->tr_refcnt && 0 == cc->session_refcnt) cnat_client_destroy(cc);
}
...
index_t cnat_client_add(const ip_address_t *ip, u8 flags)
{
    ...
    cc = (AF_IP4 == ip->version ? cnat_client_ip4_find(&ip->ip.ip4) : cnat_client_ip6_find(&ip->ip.ip6));

    if (NULL != cc) return (cc - cnat_client_pool);
    ...
    fei = fib_table_entry_special_dpo_add(CNAT_FIB_TABLE, &pfx, cnat_fib_source, fib_flags, &tmp);
...
static void cnat_client_dpo_interpose(const dpo_id_t *original, const dpo_id_t *parent, dpo_id_t *clone)
{
    ...
    dpo_stack(cnat_client_dpo, original->dpoi_proto, &cc_clone->cc_parent, parent);

    /* return the clone */
    dpo_set(clone, cnat_client_dpo, original->dpoi_proto, cc_clone - cnat_client_pool);
}

int cnat_client_purge(void)
{
    int rv = 0, rrv = 0;
    if ((rv = pool_elts(cnat_client_pool))) clib_warning("len(cnat_client_pool) isnt 0 but %d", rv);
    rrv |= rv;
    if ((rv = hash_elts(cnat_client_db.throttle_mem))) clib_warning("len(throttle_mem) isnt 0 but %d", rv);
    rrv |= rv;
    return (rrv);
}
...
u8 *format_cnat_client(u8 *s, va_list *args)
{
    ...
    s = format(s,
               "[%d] cnat-client:[%U] tr:%d sess:%d locks:%u",
               cci,
               format_ip_address,
               &cc->cc_ip,
               cc->tr_refcnt,
    ...
    if (cc->flags & CNAT_FLAG_EXCLUSIVE) s = format(s, " exclusive");
...
static clib_error_t *cnat_client_init(vlib_main_t *vm)
{
    ...
    cnat_client_dpo = dpo_register_new_type(&cnat_client_dpo_vft, cnat_client_dpo_nodes);

    clib_bihash_init_16_8(&cnat_client_db.cc_ip_id_hash, "CNat client DB", cm->client_hash_buckets, cm->client_hash_memory);

    cnat_fib_source = fib_source_allocate("cnat", CNAT_FIB_SOURCE_PRIORITY, FIB_SOURCE_BH_SIMPLE);

    clib_spinlock_init(&cnat_client_db.throttle_lock);
    cnat_client_db.throttle_mem = hash_create_mem(0, sizeof(ip_address_t), sizeof(uword));
