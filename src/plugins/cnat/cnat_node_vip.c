                              ...
                              cnat_session_t      *session)
{
    ...
    if (!session_not_found) {
        ...
        if (ip_address_is_zero(&trk0->ct_ep[VLIB_RX].ce_ip)) {
            ...
            ip46_address_copy(&session->value.cs_ip[VLIB_RX], &trk0->ct_ep[VLIB_RX].ce_ip.ip);
        }
        session->value.cs_port[VLIB_TX] = clib_host_to_net_u16(trk0->ct_ep[VLIB_TX].ce_port);
        session->value.cs_port[VLIB_RX] = clib_host_to_net_u16(trk0->ct_ep[VLIB_RX].ce_port);
        ...
        if (CNAT_SOURCE_ERROR_USE_DEFAULT == rv) rv = cspm->default_policy(vm, b, session, &rsession_flags, ct, ctx);
        if (rv) {
            if (CNAT_SOURCE_ERROR_EXHAUSTED_PORTS == rv) {
                vlib_node_registration_t *node = (AF_IP4 == ctx->af) ? &cnat_vip_ip4_node : &cnat_vip_ip6_node;
        ...
        if (!(ct->flags & CNAT_TR_FLAG_NO_RETURN_SESSION)) cnat_rsession_create(session, ctx, CNAT_LOCATION_FIB, rsession_flags);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE)))
        return cnat_node_inline(vm, node, frame, cnat_vip_node_fn, AF_IP4, CNAT_LOCATION_FIB, 1 /* do_trace */);
    return cnat_node_inline(vm, node, frame, cnat_vip_node_fn, AF_IP4, CNAT_LOCATION_FIB, 0 /* do_trace */);
}
...
{
    if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE)))
        return cnat_node_inline(vm, node, frame, cnat_vip_node_fn, AF_IP6, CNAT_LOCATION_FIB, 1 /* do_trace */);
    return cnat_node_inline(vm, node, frame, cnat_vip_node_fn, AF_IP6, CNAT_LOCATION_FIB, 0 /* do_trace */);
}
