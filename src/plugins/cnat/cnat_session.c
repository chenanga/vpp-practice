...
void cnat_session_walk(cnat_session_walk_cb_t cb, void *ctx)
{
    ...
    BV(clib_bihash_foreach_key_value_pair)(&cnat_session_db, cnat_session_walk_cb, &wctx);
}
...
u8 *format_cnat_session(u8 *s, va_list *args)
{
    ...
    if (!cnat_ts_is_free_index(sess->value.cs_ts_index)) ts = cnat_timestamp_exp(sess->value.cs_ts_index);

    s = format(s,
               "session:[%U;%d -> %U;%d, %U] => %U;%d -> %U;%d %U lb:%d age:%f",
               format_ip46_address,
               &sess->key.cs_ip[VLIB_RX],
               IP46_TYPE_ANY,
               clib_host_to_net_u16(sess->key.cs_port[VLIB_RX]),
               format_ip46_address,
               &sess->key.cs_ip[VLIB_TX],
               IP46_TYPE_ANY,
               clib_host_to_net_u16(sess->key.cs_port[VLIB_TX]),
               format_ip_protocol,
               sess->key.cs_proto,
               format_ip46_address,
               &sess->value.cs_ip[VLIB_RX],
               IP46_TYPE_ANY,
               clib_host_to_net_u16(sess->value.cs_port[VLIB_RX]),
               format_ip46_address,
               &sess->value.cs_ip[VLIB_TX],
               IP46_TYPE_ANY,
...
static clib_error_t *cnat_session_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vlib_cli_output(vm, "CNat Sessions: now:%f\n%U\n", vlib_time_now(vm), BV(format_bihash), &cnat_session_db, verbose);
...
void cnat_session_free(cnat_session_t *session)
{
    ...
    /* age it */
    if (session->value.flags & CNAT_SESSION_FLAG_ALLOC_PORT) cnat_free_port_cb(session->value.cs_port[VLIB_RX], session->key.cs_proto);
    if (!(session->value.flags & CNAT_SESSION_FLAG_NO_CLIENT)) cnat_client_free_by_ip(&session->key.cs_ip[VLIB_TX], session->key.cs_af);
...
int cnat_session_purge(void)
{
    ...
    BV(clib_bihash_foreach_key_value_pair)(&cnat_session_db, cnat_session_purge_walk, &ctx);
...
void cnat_reverse_session_free(cnat_session_t *session)
{
    ...
    rsession->key.cs_loc           = session->key.cs_loc == CNAT_LOCATION_OUTPUT ? CNAT_LOCATION_INPUT : CNAT_LOCATION_OUTPUT;
    ...
    for (/* caller saves starting point */; i < h->nbuckets; i++) {
        ...
        for (j = 0; j < (1 << b->log2_pages); j++) {
            for (k = 0; k < BIHASH_KVP_PER_PAGE; k++) {
                ...
                if (start_time > cnat_timestamp_exp(session->value.cs_ts_index)) {
                    /* age it */
...
static clib_error_t *cnat_session_init(vlib_main_t *vm)
{
    ...
    BV(clib_bihash_init)(&cnat_session_db, "CNat Session DB", cm->session_hash_buckets, cm->session_hash_memory);
    ...
    clib_bitmap_set_region(cnat_timestamps.ts_free, 0, 1, 1 << CNAT_TS_MPOOL_BITS);
...
static clib_error_t *cnat_timestamp_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (int i = 0; i < cnat_timestamps.next_empty_pool_idx; i++) {
        ...
        ts_cnt += cnt;
        vlib_cli_output(vm, "-- Pool %d [%d/%d]", i, cnt, pool_header(cnat_timestamps.ts_pools[i])->max_elts);
        if (!verbose) continue;
        pool_foreach (ts, cnat_timestamps.ts_pools[i])
            vlib_cli_output(vm, "[%d] last_seen:%f lifetime:%u ref:%u", ts - cnat_timestamps.ts_pools[i], ts->last_seen, ts->lifetime, ts->refcnt);
    }
