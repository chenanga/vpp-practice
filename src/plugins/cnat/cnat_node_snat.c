                               ...
                               cnat_session_t      *session)
{
    ...
    if (!session_not_found) {
        ...
        /* New flow, create the sessions if necessary. session will be a snat
           ...
           a VIP) */
        if (AF_IP4 == ctx->af) {
            if (!(cpm->snat_ip4.ce_flags & CNAT_EP_FLAG_RESOLVED)) goto trace;
            ...
            if (!(cpm->snat_ip6.ce_flags & CNAT_EP_FLAG_RESOLVED)) goto trace;
        ...
        cnat_rsession_create(session, ctx, CNAT_LOCATION_FIB, CNAT_SESSION_FLAG_HAS_SNAT);
    }
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE)))
        return cnat_node_inline(vm, node, frame, cnat_snat_node_fn, AF_IP4, CNAT_LOCATION_FIB, 1 /* do_trace */);
    return cnat_node_inline(vm, node, frame, cnat_snat_node_fn, AF_IP4, CNAT_LOCATION_FIB, 0 /* do_trace */);
}

VLIB_NODE_FN(cnat_snat_ip6_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE)))
        return cnat_node_inline(vm, node, frame, cnat_snat_node_fn, AF_IP6, CNAT_LOCATION_FIB, 1 /* do_trace */);
    return cnat_node_inline(vm, node, frame, cnat_snat_node_fn, AF_IP6, CNAT_LOCATION_FIB, 0 /* do_trace */);
}
