...
u8 cnat_resolve_addr(u32 sw_if_index, ip_address_family_t af, ip_address_t *addr)
{
    ...
    if (INDEX_INVALID == sw_if_index) return 0;
    if (af == AF_IP6) {
        ip6_address_t *ip6 = 0;
...
u8 cnat_resolve_ep(cnat_endpoint_t *ep)
{
    ...
    if (0 == rv) ep->ce_flags |= CNAT_EP_FLAG_RESOLVED;
    return rv;
}
...
uword unformat_cnat_ep(unformat_input_t *input, va_list *args)
{
    ...
    if (unformat(input, "%U %d", unformat_ip_address, &a->ce_ip, &port))
        ...
        ;
    else if (unformat(input, "%U v6 %d", unformat_vnet_sw_interface, vnm, &a->ce_sw_if_index, &port))
        a->ce_ip.version = AF_IP6;
    else if (unformat(input, "%U v6", unformat_vnet_sw_interface, vnm, &a->ce_sw_if_index))
        a->ce_ip.version = AF_IP6;
    else if (unformat(input, "%U %d", unformat_vnet_sw_interface, vnm, &a->ce_sw_if_index, &port))
        a->ce_ip.version = AF_IP4;
    else if (unformat_user(input, unformat_vnet_sw_interface, vnm, &a->ce_sw_if_index))
        a->ce_ip.version = AF_IP4;
...
uword unformat_cnat_ep_flags(unformat_input_t *input, va_list *args)
{
    ...
    if (unformat(input, ":nonat")) *a = CNAT_TRK_FLAG_NO_NAT;
    return 1;
}
...
uword unformat_cnat_ep_tuple(unformat_input_t *input, va_list *args)
{
    ...
    int                    flgs = 0;
    if (unformat(input, "%U->%U%U", unformat_cnat_ep, &a->src_ep, unformat_cnat_ep, &a->dst_ep, unformat_cnat_ep_flags, &flgs))
        a->ep_flags = flgs;
    else if (unformat(input, "->%U%U", unformat_cnat_ep, &a->dst_ep, unformat_cnat_ep_flags, &flgs))
        a->ep_flags = flgs;
    else if (unformat(input, "%U->%U", unformat_cnat_ep, &a->src_ep, unformat_cnat_ep_flags, &flgs))
        a->ep_flags = flgs;
    else
        return 0;
    return 1;
...
u8 *format_cnat_endpoint(u8 *s, va_list *args)
{
    ...
    if (INDEX_INVALID == cep->ce_sw_if_index)
        ...
        if (cep->ce_flags & CNAT_EP_FLAG_RESOLVED)
            s = format(s, "%U (%U);%d", format_vnet_sw_if_index_name, vnm, cep->ce_sw_if_index, format_ip_address, &cep->ce_ip, cep->ce_port);
        else
            s = format(s,
                       "%U (%U);%d",
...
static clib_error_t *cnat_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "session-db-buckets %u", &cm->session_hash_buckets))
            ;
        else if (unformat(input, "session-db-memory %U", unformat_memory_size, &cm->session_hash_memory))
            ;
        else if (unformat(input, "translation-db-buckets %u", &cm->translation_hash_buckets))
            ;
        else if (unformat(input, "translation-db-memory %U", unformat_memory_size, &cm->translation_hash_memory))
            ;
        else if (unformat(input, "client-db-buckets %u", &cm->client_hash_buckets))
            ;
        else if (unformat(input, "client-db-memory %U", unformat_memory_size, &cm->client_hash_memory))
            ;
            ...
            ;
        else if (unformat(input, "session-cleanup-timeout %f", &cm->scanner_timeout))
            ;
