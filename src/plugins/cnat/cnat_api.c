...
static void vl_api_cnat_translation_update_t_handler(vl_api_cnat_translation_update_t *mp)
{
    ...
    if (!mp->translation.is_real_ip) flags |= CNAT_FLAG_EXCLUSIVE;

    lb_type          = (cnat_lb_type_t) mp->translation.lb_type;
    flow_hash_config = (flow_hash_config_t) clib_net_to_host_u32(mp->translation.flow_hash_config);
    id               = cnat_translation_update(&vip, ip_proto, paths, flags, lb_type, flow_hash_config);

    vec_free(paths);

done:
    REPLY_MACRO2(VL_API_CNAT_TRANSLATION_UPDATE_REPLY, ({ rmp->id = htonl(id); }));
}
...
static walk_rc_t cnat_session_send_details(const cnat_session_t *session, void *args)
{
    ...
    ip_address2_from_46(&session->value.cs_ip[VLIB_TX], session->key.cs_af, &ep.ce_ip);
    ep.ce_port = clib_host_to_net_u16(session->value.cs_port[VLIB_TX]);
    cnat_endpoint_encode(&ep, &mp->session.new);

    ip_address2_from_46(&session->key.cs_ip[VLIB_RX], session->key.cs_af, &ep.ce_ip);
    ep.ce_port = clib_host_to_net_u16(session->key.cs_port[VLIB_RX]);
    cnat_endpoint_encode(&ep, &mp->session.src);

    ip_address2_from_46(&session->key.cs_ip[VLIB_TX], session->key.cs_af, &ep.ce_ip);
...
static void vl_api_cnat_snat_policy_add_del_if_t_handler(vl_api_cnat_snat_policy_add_del_if_t *mp)
{
    ...
    cnat_snat_interface_map_type_t table = (cnat_snat_interface_map_type_t) mp->table;
