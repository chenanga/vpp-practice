...
int cnat_snat_policy_add_del_if(u32 sw_if_index, u8 is_add, cnat_snat_interface_map_type_t table)
{
    ...
    if (table >= ARRAY_LEN(cpm->interface_maps)) return VNET_API_ERROR_INVALID_VALUE;
...
static clib_error_t *cnat_snat_policy_add_del_if_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "del"))
            is_add = 0;
        else if (unformat(input, "table %U", unformat_cnat_snat_interface_map_type, &table))
            ;
    ...
    if (rv) return clib_error_return(0, "Error %d", rv);
...
int cnat_snat_policy_add_pfx(ip_prefix_t *pfx)
{
    ...
    table->meta[af].non_empty_dst_address_length_bitmap = clib_bitmap_set(table->meta[af].non_empty_dst_address_length_bitmap, 128 - pfx->len, 1);
...
int cnat_snat_policy_del_pfx(ip_prefix_t *pfx)
{
    ...
    if (--table->meta[af].dst_address_length_refcounts[pfx->len] == 0) {
        table->meta[af].non_empty_dst_address_length_bitmap = clib_bitmap_set(table->meta[af].non_empty_dst_address_length_bitmap, 128 - pfx->len, 0);
        cnat_compute_prefix_lengths_in_search_order(table, af);
    }
    return 0;
}
...
int cnat_search_snat_prefix(ip46_address_t *addr, ip_address_family_t af)
{
    ...
    i = 0;
    for (; i < n_p; i++) {
        int            dst_address_length = table->meta[af].prefix_lengths_in_search_order[i];
...
int cnat_snat_policy_if_pfx(vlib_buffer_t *b, cnat_session_t *session)
{
    ...
    if (cnat_snat_policy_interface_enabled(in_if, af))
        if (cnat_search_snat_prefix(dst_addr, af)) /* Destination is not in the prefixes that don't require snat */
            return 1;
    return 0;
}
...
int cnat_snat_policy_k8s(vlib_buffer_t *b, cnat_session_t *session)
{
     ...
     * is for us */
    if (clib_bitmap_get(cpm->interface_maps[CNAT_SNAT_IF_MAP_INCLUDE_HOST], out_if)) {
        return 0;
    }
    ...
    if (!clib_bitmap_get(cpm->interface_maps[CNAT_SNAT_IF_MAP_INCLUDE_POD], in_if)
        && !clib_bitmap_get(cpm->interface_maps[CNAT_SNAT_IF_MAP_INCLUDE_POD], out_if)) {
        if (AF_IP6 == af && ip6_address_is_equal(&src_addr->ip6, &ip_addr_v6(&cpm->snat_ip6.ce_ip))) return 0;
    ...
    if (ip46_address_is_equal(src_addr, dst_addr)) return 1;
...
void cnat_set_snat(ip4_address_t *ip4, ip6_address_t *ip6, u32 sw_if_index)
{
    ...
    cnat_translation_watch_addr(INDEX_INVALID, 0, &cpm->snat_ip4, CNAT_RESOLV_ADDR_SNAT);
    cnat_translation_watch_addr(INDEX_INVALID, 0, &cpm->snat_ip6, CNAT_RESOLV_ADDR_SNAT);
}
...
static clib_error_t *cnat_set_snat_cli(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat_user(line_input, unformat_ip4_address, &ip4))
            ...
            ;
        else if (unformat_user(line_input, unformat_vnet_sw_interface, vnm, &sw_if_index))
            ;
        else {
...
VLIB_CLI_COMMAND(cnat_set_snat_command, static) = {
    ...
    .short_help = "set cnat snat-policy addr [<ip4-address>][<ip6-address>][sw_if_index]",
...
static clib_error_t *cnat_snat_policy_add_del_pfx_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (rv) return (clib_error_return(0, "error %d", rv, input));
...
{
    cnat_snat_exclude_pfx_table_t *excluded_pfx = &cnat_snat_policy_main.excluded_pfx;
    cnat_snat_policy_main_t       *cpm          = &cnat_snat_policy_main;
    vnet_main_t                   *vnm          = vnet_get_main();
    u32                            sw_if_index;

    vlib_cli_output(vm, "Source NAT\n  ip4: %U\n  ip6: %U\n\n", format_cnat_endpoint, &cpm->snat_ip4, format_cnat_endpoint, &cpm->snat_ip6);
    vlib_cli_output(vm, "Excluded prefixes:\n  %U\n", format_bihash_24_8, &excluded_pfx->ip_hash, 1);
...
static clib_error_t *cnat_snat_init(vlib_main_t *vm)
{
    ...
    for (i = 0; i < ARRAY_LEN(excluded_pfx->ip_masks); i++) {
        ...
        if (i1) excluded_pfx->ip_masks[i].as_u32[i0] = clib_host_to_net_u32(pow2_mask(i1) << (32 - i1));
    }
    clib_bihash_init_24_8(&excluded_pfx->ip_hash, "snat prefixes", cm->snat_hash_buckets, cm->snat_hash_memory);
