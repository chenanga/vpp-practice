...
void cnat_translation_watch_addr(index_t cti, u64 opaque, cnat_endpoint_t *ep, cnat_addr_resol_type_t type)
{
    ...
    if (INDEX_INVALID == ep->ce_sw_if_index) return;
...
void cnat_translation_unwatch_addr(u32 cti, cnat_addr_resol_type_t type)
{
    ...
    pool_foreach (ar, tr_resolutions) {
        if ((cti == INDEX_INVALID || ar->cti == cti) && (ar->type == type || CNAT_RESOLV_ADDR_ANY == type)) vec_add1(indexes, ar - tr_resolutions);
    }
...
static void cnat_tracker_release(cnat_ep_trk_t *trk)
{
    ...
    if (!(trk->ct_flags & CNAT_TRK_ACTIVE)) return;
...
static void cnat_tracker_track(index_t cti, cnat_ep_trk_t *trk)
{
    fib_prefix_t pfx;
    /* We only track fully resolved endpoints */
    if (trk->ct_ep[VLIB_TX].ce_flags & CNAT_EP_FLAG_RESOLVED && trk->ct_ep[VLIB_RX].ce_flags & CNAT_EP_FLAG_RESOLVED)
    ...
    trk->ct_fei = fib_entry_track(CNAT_FIB_TABLE, &pfx, cnat_translation_fib_node_type, cti, &trk->ct_sibling);

    fib_entry_contribute_forwarding(trk->ct_fei, fib_forw_chain_type_from_fib_proto(pfx.fp_proto), &trk->ct_dpo);
}
...
static void cnat_translation_stack(cnat_translation_t *ct)
{
    ...
    vec_foreach (trk, ct->ct_paths)
        if (trk->ct_flags & CNAT_TRK_ACTIVE) vec_add1(ct->ct_active_paths, *trk);

    flow_hash_config_t fhc = IP_FLOW_HASH_DEFAULT;
    if (ct->fhc != 0) fhc = ct->fhc;
    lbi = load_balance_create(vec_len(ct->ct_active_paths), fib_proto_to_dpo(fproto), fhc);

    ep_idx = 0;
    vec_foreach (trk, ct->ct_active_paths)
        load_balance_set_bucket(lbi, ep_idx++, &trk->ct_dpo);

    if (ep_idx > 0 && CNAT_LB_MAGLEV == ct->lb_type) cnat_translation_init_maglev(ct);
...
int cnat_translation_delete(u32 id)
{
    ...
    if (pool_is_free_index(cnat_translation_pool, id)) return (VNET_API_ERROR_NO_SUCH_ENTRY);
                            ...
                            flow_hash_config_t     fhc)
{
    ...
    cnat_translation_watch_addr(ct->index, 0, vip, CNAT_RESOLV_ADDR_TRANSLATION);
    ...
    vec_foreach (path, paths) {
        ...
        cnat_translation_watch_addr(ct->index, path_idx << 32 | VLIB_RX, &path->src_ep, CNAT_RESOLV_ADDR_BACKEND);
        cnat_translation_watch_addr(ct->index, path_idx << 32 | VLIB_TX, &path->dst_ep, CNAT_RESOLV_ADDR_BACKEND);
        path_idx++;
...
static u8 *format_cnat_ep_trk(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U->%U", format_cnat_endpoint, &ck->ct_ep[VLIB_RX], format_cnat_endpoint, &ck->ct_ep[VLIB_TX]);
    s = format(s, "\n%Ufib-entry:%d", format_white_space, indent, ck->ct_fei);
    s = format(s, "\n%U%U", format_white_space, indent, format_dpo_id, &ck->ct_dpo, 6);
...
u8 *format_cnat_translation(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U %U ", format_cnat_endpoint, &ct->ct_vip, format_ip_protocol, ct->ct_proto);
    ...
    if (!pool_is_free_index(load_balance_pool, ct->ct_lb.dpoi_index)) {
        s = format(s, "\n via:");
        s = format(s, "\n%U%U", format_white_space, 2, format_dpo_id, &ct->ct_lb, 2);
    }
    ...
    if (CNAT_LB_MAGLEV == ct->lb_type) {
        ...
        vec_foreach (ck, ct->ct_paths) {
            ...
            for (u32 i = 0; i < vec_len(ct->lb_maglev); i++)
                if (ct->lb_maglev[i] == bid) clib_bitmap_set(bitmap, i, 1);
...
{
    return ((cnat_translation_t *) (((char *) node) - STRUCT_OFFSET_OF(cnat_translation_t, ct_node)));
}
...
static void cnat_translation_last_lock_gone(fib_node_t *node)
{
 ...
 static fib_node_back_walk_rc_t cnat_translation_back_walk_notify(fib_node_t *node, fib_node_back_walk_ctx_t *ctx)
 {
      ...
      * (cnat_tracker_track) */
     if (!(ct->flags & CNAT_TR_FLAG_STACKED)) return (FIB_NODE_BACK_WALK_CONTINUE);
 ...
 VLIB_CLI_COMMAND(cnat_translation_cli_add_del_command, static) = {
     ...
     .short_help =
         "cnat translation [add|del] proto [TCP|UDP] [vip|real] [ip|sw_if_index [v6]] [port] [to [ip|sw_if_index [v6]] [port]->[ip|sw_if_index [v6]] "
         "[port]]",
 ...
 static void cnat_if_addr_add_del_translation_cb(addr_resolution_t *ar, ip_address_t *address, u8 is_del)
 {
     ...
     if (!is_del && ct->ct_vip.ce_flags & CNAT_EP_FLAG_RESOLVED) return;
     ...
     if (is_del) {
         ...
         if (0 == cnat_resolve_addr(ar->sw_if_index, ar->af, address)) is_del = 0;
     }
     ...
     cnat_add_translation_to_db(ct->ct_cci, &ct->ct_vip, ct->ct_proto, ct->index);
 }
 ...
 static void cnat_if_addr_add_del_backend_cb(addr_resolution_t *ar, ip_address_t *address, u8 is_del)
 {
     ...
     if (!is_del && ep->ce_flags & CNAT_EP_FLAG_RESOLVED) return;
 ...
 static void cnat_if_addr_add_del_callback(u32 sw_if_index, ip_address_t *address, u8 is_del)
 {
     addr_resolution_t *ar;
     pool_foreach (ar, tr_resolutions) {
         if (ar->sw_if_index != sw_if_index) continue;
         if (ar->af != ip_addr_version(address)) continue;
 ...
 static clib_error_t *cnat_translation_init(vlib_main_t *vm)
 {
     ...
     cnat_main_t *cm                = &cnat_main;
     cnat_translation_fib_node_type = fib_node_register_new_type("cnat-translation", &cnat_translation_vft);

     clib_bihash_init_8_8(&cnat_translation_db, "CNat translation DB", cm->translation_hash_buckets, cm->translation_hash_memory);
     ...
     cnat_translation_register_addr_add_cb(CNAT_RESOLV_ADDR_BACKEND, cnat_if_addr_add_del_backend_cb);
     cnat_translation_register_addr_add_cb(CNAT_RESOLV_ADDR_TRANSLATION, cnat_if_addr_add_del_translation_cb);
