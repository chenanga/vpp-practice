                                                           ...
                                                           cnat_node_ctx_t          *ctx)
{
    ...
    if (!session->value.cs_port[VLIB_RX]) {
        ...
        if ((ct->flags & CNAT_TR_FLAG_ALLOCATE_PORT) && (*rsession_flags & CNAT_SESSION_FLAG_HAS_SNAT)) {
            sport = 0; /* force allocation */
            session->value.flags |= CNAT_SESSION_FLAG_ALLOC_PORT;
            rv = cnat_allocate_port(&sport, iproto);
            if (rv) return CNAT_SOURCE_ERROR_EXHAUSTED_PORTS;
        }
...
always_inline cnat_src_port_allocator_t *cnat_get_src_port_allocator(ip_protocol_t iproto)
{
    ...
    *port = clib_net_to_host_u16(*port);
    ...
    if (clib_bitmap_get_no_check(ca->bmap, *port)) {
        *port = clib_bitmap_next_clear(ca->bmap, *port);
        if (PREDICT_FALSE(*port >= UINT16_MAX)) *port = clib_bitmap_next_clear(ca->bmap, MIN_SRC_PORT);
