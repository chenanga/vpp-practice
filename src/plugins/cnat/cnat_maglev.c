...
static int cnat_maglev_perm_compare(void *_a, void *_b)
{
 ...
 */
static void cnat_maglev_shuffle(cnat_maglev_perm_t *permutation, u32 *buckets)
{
    u32  N, M, i, done = 0;
    u32 *next = 0;
...
void cnat_translation_init_maglev(cnat_translation_t *ct)
{
    ...
    if (vec_len(ct->ct_active_paths) == 0) return;
    ...
    vec_foreach (trk, ct->ct_active_paths) {
        ...
        if (trk->ct_flags & CNAT_TRK_FLAG_TEST_DISABLED) continue;
...
static int cnat_u32_vec_contains(u32 *v, u32 e)
{
    int i;
...
static void cnat_maglev_print_changes(vlib_main_t *vm, u32 *changed_bk_indices, u32 *old_maglev_lb, u32 *new_maglev_lb)
{
    u32 good_flow_buckets = 0, reset_flow_buckets = 0, stable_to_reset = 0;
    u32 reset_to_stable = 0, switched_stable = 0;
    if (vec_len(new_maglev_lb) == 0) return;
    for (u32 i = 0; i < vec_len(new_maglev_lb); i++) {
        u8 is_new_changed = cnat_u32_vec_contains(changed_bk_indices, new_maglev_lb[i]);
        u8 is_old_changed = cnat_u32_vec_contains(changed_bk_indices, old_maglev_lb[i]);
    ...
    vlib_cli_output(vm,
                    ...
                    reset_flow_buckets,
                    reset_to_stable,
                    (f64) (reset_flow_buckets + reset_to_stable) / vec_len(new_maglev_lb) * 100.0,
                    stable_to_reset,
                    switched_stable,
                    (f64) (stable_to_reset + switched_stable) / vec_len(new_maglev_lb) * 100.0);
}

static u8 *format_cnat_maglev_buckets(u8 *s, va_list *args)
{
...
static clib_error_t *cnat_translation_test_init_maglev(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (num_backends == 0 || n_tests == 0) return (clib_error_return(0, "No backends / tests to run"));
    ;
    ...
    vec_foreach (ct, trs) {
        if (vec_len(ct->lb_maglev) != cm->maglev_len) vlib_cli_output(vm, "Unexpected bucket length %d", vec_len(ct->lb_maglev));

        vec_zero(backend_freqs);
        for (u32 i = 0; i < vec_len(ct->lb_maglev); i++) {
            if (ct->lb_maglev[i] >= num_backends) clib_warning("out of bound backend");
            backend_freqs[ct->lb_maglev[i]]++;
        }
        u32 fmin = ~0, fmax = 0;
        for (u32 i = 0; i < num_backends; i++) {
            if (backend_freqs[i] > fmax) fmax = backend_freqs[i];
            if (backend_freqs[i] < fmin) fmin = backend_freqs[i];
        }
        f64 fdiff = (fmax - fmin);
        if (fdiff / vec_len(ct->lb_maglev) - 1 > 0.02) vlib_cli_output(vm, "More than 2%% frequency diff (min %d max %d)", fmin, fmax);
    }
        ...
        vec_foreach (ct, trs) {
            ...
            for (u32 i = 0; i < verbose; i++) {
                ...
                vlib_cli_output(vm,
                                ...
                                j,
                                format_ip_address,
                                &trk->ct_ep[VLIB_TX].ce_ip,
                                trk->ct_ep[VLIB_TX].ce_port,
                                format_cnat_maglev_buckets,
                                ct->lb_maglev,
                                j,
                                verbose);
            }
    ...
    if (n_remove != 0) {
        vlib_cli_output(vm, "Removing %d entries (refered to as A), others (B,C) stay same", n_remove);
        vec_foreach (ct, trs) {
            u32 *old_maglev_lb      = 0;
            ...
            for (u32 i = 0; i < n_remove; i++) {
                ...
                changed_bk_indices[i] = random_u32(&rnd) % vec_len(ct->ct_active_paths);
            ...
            cnat_maglev_print_changes(vm, changed_bk_indices, old_maglev_lb, ct->lb_maglev);
    ...
    if (n_changes != 0) {
        vlib_cli_output(vm, "Changing %d entries (refered to as A->A'), others (B,C) stay same", n_changes);
        vec_foreach (ct, trs) {
            if (vec_len(ct->lb_maglev) != cm->maglev_len) vlib_cli_output(vm, "Unexpected bucket length %d", vec_len(ct->lb_maglev));
...
VLIB_CLI_COMMAND(cnat_translation_test_init_maglev_cmd, static) = {
    ...
    .short_help =
        "test cnat maglev tests [n_tests] backends [num_backends] len "
        "[maglev_len]",
