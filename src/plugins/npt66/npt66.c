...
static int npt66_feature_enable_disable(u32 sw_if_index, bool is_add)
{
    if (vnet_feature_enable_disable("ip6-unicast", "npt66-input", sw_if_index, is_add, 0, 0) != 0) return -1;
    if (vnet_feature_enable_disable("ip6-output", "npt66-output", sw_if_index, is_add, 0, 0) != 0) return -1;
    return 0;
}

static void ipv6_prefix_zero(ip6_address_t *address, int prefix_len)
{
...
int npt66_binding_add_del(u32 sw_if_index, ip6_address_t *internal, int internal_plen, ip6_address_t *external, int external_plen, bool is_add)
{
    ...
    if (is_add) {
        ...
        if (internal_plen > 64 || external_plen > 64) return VNET_API_ERROR_INVALID_VALUE;
        ...
        if (configure_feature) rv = npt66_feature_enable_disable(sw_if_index, is_add);
    }
    else {
        /* Delete a binding entry */
        npt66_binding_t *b = npt66_interface_by_sw_if_index(sw_if_index);
        if (!b) return VNET_API_ERROR_NO_SUCH_ENTRY;
...
npt66_binding_t *npt66_interface_by_sw_if_index(u32 sw_if_index)
{
    ...
    if (!nm->interface_by_sw_if_index || sw_if_index > (vec_len(nm->interface_by_sw_if_index) - 1)) return 0;
