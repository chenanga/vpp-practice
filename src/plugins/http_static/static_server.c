...
static hss_session_t *hss_session_alloc(u32 thread_index)
{
...
__clib_export hss_session_t *hss_session_get(u32 thread_index, u32 hs_index)
{
    hss_main_t *hsm = &hss_main;
    if (pool_is_free_index(hsm->sessions[thread_index], hs_index)) return 0;
    return pool_elt_at_index(hsm->sessions[thread_index], hs_index);
}

static void hss_session_free(hss_session_t *hs)
{
    hss_main_t *hsm = &hss_main;
 ...
 */
static void hss_session_disconnect_transport(hss_session_t *hs)
{
...
static void start_send_data(hss_session_t *hs, http_status_code_t status)
{
    http_msg_t msg;
    session_t *ts;
    u8        *headers_buf = 0;
    ...
    if (!msg.data.body_len) goto done;
...
__clib_export void hss_session_send_data(hss_url_handler_args_t *args)
{
    hss_session_t *hs;
    ...
    if (hs->data && hs->free_data) vec_free(hs->data);
    ...
    if (hs->data_len) {
        http_add_header(&hs->resp_headers, http_header_name_token(HTTP_HEADER_CONTENT_TYPE), http_content_type_token(args->ct));
    }
...
static int path_has_known_suffix(u8 *request)
{
    u8    *ext;
    uword *p;
...
static http_content_type_t content_type_from_request(u8 *request)
{
    u8    *ext;
    uword *p;
...
static int try_url_handler(hss_main_t *hsm, hss_session_t *hs, http_req_method_t rt, u8 *target_path, u8 *target_query, u8 *data)
{
    ...
    if (!hsm->enable_url_handlers || !target_path) return -1;
    ...
    url_table = (rt == HTTP_REQ_GET) ? hsm->get_url_handlers : hsm->post_url_handlers;
    ...
    if (hsm->debug_level > 0) clib_warning("%s '%s'", (rt == HTTP_REQ_GET) ? "GET" : "POST", target_path);
    ...
    if (rv == HSS_URL_HANDLER_ASYNC) return 0;

    if (rv == HSS_URL_HANDLER_ERROR) {
        clib_warning("builtin handler %llx hit on %s '%s' but failed!", p[0], (rt == HTTP_REQ_GET) ? "GET" : "POST", target_path);
    ...
    if (!hs->data_len) hss_session_disconnect_transport(hs);

    return 0;
}

static u8 file_path_is_valid(u8 *path)
{
...
static u32 try_index_file(hss_main_t *hsm, hss_session_t *hs, u8 *path)
{
    ...
    if (hsm->debug_level > 0) clib_warning("trying to find index: %s", path);

    if (!file_path_is_valid(path)) return HTTP_STATUS_NOT_FOUND;
    ...
    if ((proto == TRANSPORT_PROTO_TCP && local_port != 80) || (proto == TRANSPORT_PROTO_TLS && local_port != 443)) {
        print_port = 1;
    ...
    if (hsm->debug_level > 0) clib_warning("redirect: %s", redirect);

    vec_free(port_str);

    http_add_header(&hs->resp_headers, http_header_name_token(HTTP_HEADER_LOCATION), (const char *) redirect, vec_len(redirect));
...
static int try_file_handler(hss_main_t *hsm, hss_session_t *hs, http_req_method_t rt, u8 *target)
{
    http_status_code_t  sc = HTTP_STATUS_OK;
    ...
    if (hsm->debug_level > 0) clib_warning("%s '%s'", (rt == HTTP_REQ_GET) ? "GET" : "POST", path);

    if (hs->data && hs->free_data) vec_free(hs->data);

    hs->data_offset = 0;

    ce_index = hss_cache_lookup_and_attach(&hsm->cache, path, &hs->data, &hs->data_len, &last_modified);
    if (ce_index == ~0) {
        ...
        ce_index = hss_cache_add_and_attach(&hsm->cache, path, &hs->data, &hs->data_len, &last_modified);
        if (ce_index == ~0) {
    ...
    http_add_header(&hs->resp_headers, http_header_name_token(HTTP_HEADER_CONTENT_TYPE), http_content_type_token(type));
    ...
    http_add_header(&hs->resp_headers, http_header_name_token(HTTP_HEADER_LAST_MODIFIED), (const char *) last_modified, vec_len(last_modified));
...
static void handle_request(hss_session_t *hs, http_req_method_t rt, u8 *target_path, u8 *target_query, u8 *data)
{
    hss_main_t *hsm = &hss_main;

    if (!try_url_handler(hsm, hs, rt, target_path, target_query, data)) return;

    if (!try_file_handler(hsm, hs, rt, target_path)) return;
...
static int hss_ts_rx_callback(session_t *ts)
{
    hss_session_t *hs;
    ...
    if (hs->free_data) vec_free(hs->data);
    hs->data         = 0;
    hs->resp_headers = 0;
    ...
    if (msg.type != HTTP_MSG_REQUEST || (msg.method_type != HTTP_REQ_GET && msg.method_type != HTTP_REQ_POST)) {
        http_add_header(&hs->resp_headers, http_header_name_token(HTTP_HEADER_ALLOW), http_token_lit("GET, POST"));
    ...
    if (msg.data.target_query_len) {
        ...
        rv = svm_fifo_peek(ts->rx_fifo, msg.data.target_query_offset, msg.data.target_query_len, target_query);
    ...
    if (msg.data.body_len) {
        ...
        rv = svm_fifo_peek(ts->rx_fifo, msg.data.body_offset, msg.data.body_len, data);
...
static int hss_ts_tx_callback(session_t *ts)
{
    hss_session_t *hs;
    u32            n_enq;
    u64            to_send;
 ...
 */
static int hss_ts_accept_callback(session_t *ts)
{
    hss_session_t *hs;
    u32            thresh;
...
static void hss_ts_disconnect_callback(session_t *ts)
{
    hss_main_t            *hsm = &hss_main;
    vnet_disconnect_args_t _a = {0}, *a = &_a;
...
static void hss_ts_reset_callback(session_t *ts)
{
    hss_main_t            *hsm = &hss_main;
    vnet_disconnect_args_t _a = {0}, *a = &_a;
...
static int hss_ts_connected_callback(u32 app_index, u32 api_context, session_t *ts, session_error_t err)
{
    clib_warning("called...");
    return -1;
}

static int hss_add_segment_callback(u32 client_index, u64 segment_handle)
{
    return 0;
}

static void hss_ts_cleanup(session_t *s, session_cleanup_ntf_t ntf)
{
    hss_main_t    *hsm = &hss_main;
    hss_session_t *hs;
...
static int hss_attach()
{
    ...
    if (hsm->private_segment_size) segment_size = hsm->private_segment_size;
    ...
    a->options[APP_OPTIONS_RX_FIFO_SIZE]        = hsm->fifo_size ? hsm->fifo_size : 8 << 10;
    a->options[APP_OPTIONS_TX_FIFO_SIZE]        = hsm->fifo_size ? hsm->fifo_size : 32 << 10;
...
static int hss_transport_needs_crypto(transport_proto_t proto)
{
    return proto == TRANSPORT_PROTO_TLS || proto == TRANSPORT_PROTO_DTLS || proto == TRANSPORT_PROTO_QUIC;
}

static int hss_listen(void)
{
    hss_main_t            *hsm = &hss_main;
    ...
    if (hsm->uri) uri = (char *) hsm->uri;

    if (parse_uri(uri, &sep)) return -1;
    ...
    if (need_crypto) clib_mem_free(a->sep_ext.ext_cfg);

    return rv;
}

static void hss_url_handlers_init(hss_main_t *hsm)
{
...
int hss_create(vlib_main_t *vm)
{
    ...
    if (hsm->www_root) hss_cache_init(&hsm->cache, hsm->cache_size, hsm->debug_level);

    if (hsm->enable_url_handlers) hss_url_handlers_init(hsm);
...
static clib_error_t *hss_create_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (hsm->app_index != (u32) ~0) return clib_error_return(0, "http server already running...");
    ...
    /* Get a line of input. */
    if (!unformat_user(input, unformat_line_input, line_input)) goto no_input;

    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "www-root %s", &hsm->www_root))
            ;
        else if (unformat(line_input, "prealloc-fifos %d", &hsm->prealloc_fifos))
            ;
        else if (unformat(line_input, "private-segment-size %U", unformat_memory_size, &seg_size))
            hsm->private_segment_size = seg_size;
        else if (unformat(line_input, "fifo-size %d", &hsm->fifo_size))
            hsm->fifo_size <<= 10;
        else if (unformat(line_input, "cache-size %U", unformat_memory_size, &hsm->cache_size))
            ;
            ...
            hsm->debug_level = 1;
        else if (unformat(line_input, "ptr-thresh %U", unformat_memory_size, &hsm->use_ptr_thresh))
            ;
...
static u8 *format_hss_session(u8 *s, va_list *args)
{
    hss_session_t    *hs      = va_arg(*args, hss_session_t *);
    int __clib_unused verbose = va_arg(*args, int);

    s = format(s, "\n path %s, data length %llu, data_offset %llu", hs->path ? hs->path : (u8 *) "[none]", hs->data_len, hs->data_offset);
    return s;
}

static clib_error_t *hss_show_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    int         verbose = 0, show_cache = 0, show_sessions = 0;
    hss_main_t *hsm = &hss_main;

    if (hsm->www_root == 0) return clib_error_return(0, "Static server disabled");
    ...
    if ((show_cache + show_sessions) == 0) return clib_error_return(0, "specify one or more of cache, sessions");

    if (show_cache) vlib_cli_output(vm, "%U", format_hss_cache, &hsm->cache, verbose);
    ...
    if (show_sessions) {
        ...
        for (i = 0; i < vec_len(hsm->sessions); i++) {
            ...
            for (j = 0; j < vec_len(session_indices); j++) {
                vlib_cli_output(vm, "%U", format_hss_session, pool_elt_at_index(hsm->sessions[i], session_indices[j]), verbose);
            }
...
static clib_error_t *hss_clear_cache_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    hss_main_t *hsm        = &hss_main;
    u32         busy_items = 0;
...
static clib_error_t *hss_main_init(vlib_main_t *vm)
{
    hss_main_t *hsm = &hss_main;
    ...
    hsm->mime_type_indices_by_file_extensions = hash_create_string(0, sizeof(uword));

#define _(def, ext, str) hash_set_mem(hsm->mime_type_indices_by_file_extensions, ext, HTTP_CONTENT_##def);
