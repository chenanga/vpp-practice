...
hss_url_handler_rc_t handle_get_version(hss_url_handler_args_t *args)
{
    u8              *s = 0;
    ...
    if (args->query) {
        ...
        if (unformat(&input, "verbose=")) {
            if (unformat(&input, "true")) verbose = 1;
        }
    }
...
hss_url_handler_rc_t handle_get_interface_stats(hss_url_handler_args_t *args)
{
    u8                    *s = 0, *stats               = 0;
    ...
    for (i = 0; i < vec_len(sw_if_indices); i++) {
        ...
        s = format(s, "{%sname%s: %s%U%s, ", q, q, q, format_vnet_sw_if_index_name, vnm, sw_if_indices[i], q);

        stats = format_vnet_sw_interface_cntrs(stats, &vnm->interface_main, si, 1 /* want json */);
        if (vec_len(stats))
            s = format(s, "%v}", stats);
        else
...
hss_url_handler_rc_t handle_get_interface_list(hss_url_handler_args_t *args)
{
    u8                    *s = 0;
    int                    i;
    vnet_main_t           *vnm = vnet_get_main();
    ...
    pool_foreach (hi, im->hw_interfaces) {
        ...
        if (hi - im->hw_interfaces) vec_add1(hw_if_indices, hi - im->hw_interfaces);
    }
...
void hss_builtinurl_json_handlers_init(void)
{
    ...
    hss_register_url_handler(handle_get_interface_list, "interface_list.json", HTTP_REQ_GET);
    hss_register_url_handler(handle_get_interface_stats, "interface_stats.json", HTTP_REQ_GET);
    hss_register_url_handler(handle_get_interface_stats, "interface_stats.json", HTTP_REQ_POST);
}
