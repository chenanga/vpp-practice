...
static int api_http_static_enable(vat_main_t *vam)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "www-root %s", &www_root))
            ...
            ;
        else if (unformat(line_input, "private-segment-size %U", unformat_memory_size, &tmp)) {
        ...
        else if (unformat(line_input, "fifo-size %U", unformat_memory_size, &tmp)) {
            if (tmp >= 0x100000000ULL) {
        ...
        else if (unformat(line_input, "cache-size %U", unformat_memory_size, &tmp)) {
            if (tmp < (128ULL << 10)) {
    ...
    if (uri == 0) uri = format(0, "tcp://0.0.0.0/80%c", 0);
...
static int api_http_static_enable_v2(vat_main_t *vam)
{
    unformat_input_t               *line_input = vam->input;
