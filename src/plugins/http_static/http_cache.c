...
static void hss_cache_lock(hss_cache_t *hc)
{
    clib_spinlock_lock(&hc->cache_lock);
}

static void hss_cache_unlock(hss_cache_t *hc)
{
 ...
 */
static inline void lru_validate(hss_cache_t *hc)
{
#if CLIB_DEBUG > 0
    f64                last_timestamp;
    u32                index;
    ...
    for (i = 1, index = hc->first_index; index != ~0;) {
        ...
        if (ce->last_used > last_timestamp) {
            clib_warning("%d[%d]: last used %.6f, last_timestamp %.6f", index, i, ce->last_used, last_timestamp);
        }
        index          = ce->next_index;
 ...
 */
static inline void lru_remove(hss_cache_t *hc, hss_cache_entry_t *ce)
{
    ...
    if (ce_index == hc->first_index) hc->first_index = ce->next_index;
    if (ce_index == hc->last_index) hc->last_index = ce->prev_index;
 ...
 */
static inline void lru_add(hss_cache_t *hc, hss_cache_entry_t *ce, f64 now)
{
    ...
    if (hc->last_index == ~0) hc->last_index = ce_index;
    ce->last_used = now;
 ...
 */
static inline void lru_update(hss_cache_t *hc, hss_cache_entry_t *ep, f64 now)
{
    lru_remove(hc, ep);
    lru_add(hc, ep, now);
}

static void hss_cache_attach_entry(hss_cache_t *hc, u32 ce_index, u8 **data, u64 *data_len, u8 **last_modified)
{
    hss_cache_entry_t *ce;
    ...
    *last_modified = ce->last_modified;
    ...
    if (hc->debug_level > 1) clib_warning("index %d refcnt now %d", ce_index, ce->inuse);
}
...
static u32 hss_cache_lookup(hss_cache_t *hc, u8 *path)
{
    ...
    if (hc->debug_level > 1) clib_warning("lookup '%s' %s", kv.key, kv.value == ~0 ? "fail" : "found");

    return kv.value;
}

u32 hss_cache_lookup_and_attach(hss_cache_t *hc, u8 *path, u8 **data, u64 *data_len, u8 **last_modified)
{
    u32 ce_index;
    ...
    if (ce_index != ~0) hss_cache_attach_entry(hc, ce_index, data, data_len, last_modified);
...
static void hss_cache_do_evictions(hss_cache_t *hc)
{
    BVT(clib_bihash_kv) kv;
    hss_cache_entry_t *ce;
    u32                free_index;
    ...
    while (free_index != ~0) {
        ...
        if (ce->inuse) {
            if (hc->debug_level > 1) clib_warning("index %d in use refcnt %d", free_index, ce->inuse);
        }
        free_index = ce->prev_index;
        kv.key     = (u64) (ce->filename);
        kv.value   = ~0ULL;
        if (BV(clib_bihash_add_del)(&hc->name_to_data, &kv, 0 /* is_add */) < 0) {
        ...
        if (hc->debug_level > 1) clib_warning("pool put index %d", ce - hc->cache_pool);

        pool_put(hc->cache_pool, ce);
        if (hc->cache_size < hc->cache_limit) break;
    }
}

u32 hss_cache_add_and_attach(hss_cache_t *hc, u8 *path, u8 **data, u64 *data_len, u8 **last_modified)
{
    BVT(clib_bihash_kv) kv;
    hss_cache_entry_t *ce;
    clib_error_t      *error;
    ...
    if (hc->cache_size > hc->cache_limit) hss_cache_do_evictions(hc);
    ...
    if (stat((char *) path, &dm) == 0) {
        ce->last_modified = format(0, "%U GMT", format_clib_timebase_time, (f64) dm.st_mtime);
    }
    ...
    *last_modified = ce->last_modified;
    ...
    if (hc->debug_level > 1) clib_warning("add '%s' value %lld", kv.key, kv.value);
 ...
 */
static u8 *format_hss_cache_entry(u8 *s, va_list *args)
{
    ...
    s = format(s, "%40s%12lld%20.2f", ep->filename, vec_len(ep->data), now - ep->last_used);
    return s;
}
...
u8 *format_hss_cache(u8 *s, va_list *args)
{
    ...
    if (verbose == 0) {
        s = format(s, "cache size %lld bytes, limit %lld bytes, evictions %lld", hc->cache_size, hc->cache_limit, hc->cache_evictions);
        return s;
    }
