...
static_always_inline void rdma_device_output_free_mlx5(vlib_main_t *vm, const vlib_node_runtime_t *node, rdma_txq_t *txq)
{
    ...
    for (;;) {
        ...
        if (((idx >> log2_cq_sz) & MLX5_CQE_OWNER_MASK) != (op_own & MLX5_CQE_OWNER_MASK) || (op_own >> 4) == MLX5_CQE_INVALID) break;
        if (PREDICT_FALSE((op_own >> 4)) != MLX5_CQE_REQ) vlib_error_count(vm, node->node_index, RDMA_TX_ERROR_COMPLETION, 1);
        idx++;
        cur = cqes + (idx & cq_mask);
    }

    if (idx == txq->dv_cq_idx) return; /* nothing to do */
    ...
    if (PREDICT_FALSE(wqe->ctrl.imm == RDMA_TXQ_DV_INVALID_ID)) return; /* can happen if CQE reports error for an intermediate WQE */

    ASSERT(RDMA_TXQ_USED_SZ(txq->head, wqe->ctrl.imm) <= buf_sz && RDMA_TXQ_USED_SZ(wqe->ctrl.imm, txq->tail) < buf_sz);

    /* free sent buffers and update txq head */
    vlib_buffer_free_from_ring(vm, txq->bufs, txq->head & mask, buf_sz, RDMA_TXQ_USED_SZ(txq->head, wqe->ctrl.imm));
...
static_always_inline void rdma_device_output_tx_mlx5_doorbell(rdma_txq_t *txq, rdma_mlx5_wqe_t *last, const u16 tail, u32 sq_mask)
{
    ...
    ASSERT(tail != txq->tail && RDMA_TXQ_AVAIL_SZ(txq, txq->head, txq->tail) >= RDMA_TXQ_USED_SZ(txq->tail, tail));
...
static_always_inline void rdma_mlx5_wqe_init(rdma_mlx5_wqe_t *wqe, const void *tmpl, vlib_buffer_t *b, const u16 tail)
{
    ...
    STATIC_ASSERT(STRUCT_SIZE_OF(struct mlx5_wqe_eth_seg, inline_hdr_start) + STRUCT_SIZE_OF(struct mlx5_wqe_eth_seg, inline_hdr)
                      >= MLX5_ETH_L2_INLINE_HEADER_SIZE,
                  "wrong size");
    clib_memcpy_fast(wqe->eseg.inline_hdr_start, cur, MLX5_ETH_L2_INLINE_HEADER_SIZE);
...
#define RDMA_MLX5_WQE_DS_MAX (1 << 5)
            ...
            if (chained_b->flags & VLIB_BUFFER_NEXT_PRESENT) {
                 ...
                 */
                vlib_buffer_free_from_ring(vm, txq->bufs, tail & mask, RDMA_TXQ_BUF_SZ(txq), 1 + RDMA_TXQ_DV_DSEG2WQE(chained_n));
                vlib_error_count(vm, node->node_index, dseg_max == chained_n ? RDMA_TX_ERROR_SEGMENT_SIZE_EXCEEDED : RDMA_TX_ERROR_NO_FREE_SLOTS, 1);
    ...
    if (n != n_left_from) rdma_device_output_tx_mlx5_doorbell(txq, last, tail, sq_mask);
...
wrap_around:
    ...
    while (n >= 8) {
        ...
        if (PREDICT_FALSE(flags & VLIB_BUFFER_NEXT_PRESENT)) return rdma_device_output_tx_mlx5_chained(vm, node, rd, txq, n_left_from, bi, b, tail);
...
static_always_inline void rdma_device_output_free_ibverb(vlib_main_t *vm, const vlib_node_runtime_t *node, rdma_txq_t *txq)
{
    ...
    if (n <= 0) {
        if (PREDICT_FALSE(n < 0)) vlib_error_count(vm, node->node_index, RDMA_TX_ERROR_COMPLETION, 1);
        return;
    }
    ...
    tail = wc[n - 1].wr_id;
    vlib_buffer_free_from_ring(vm, txq->bufs, txq->head & mask, RDMA_TXQ_BUF_SZ(txq), RDMA_TXQ_USED_SZ(txq->head, tail));
    txq->head = tail;
}
                                                      ...
                                                      vlib_buffer_t            **b)
{
    ...
    if (PREDICT_FALSE(0 != ibv_post_send(txq->ibv_qp, w, &w))) {
        vlib_error_count(vm, node->node_index, RDMA_TX_ERROR_SUBMISSION, n_left_from - (w - wr));
        n_left_from = w - wr;
    }
...
static u32 rdma_device_output_tx_try(vlib_main_t               *vm,
                                     const vlib_node_runtime_t *node,
                                     const rdma_device_t       *rd,
                                     rdma_txq_t                *txq,
                                     u32                        n_left_from,
                                     u32                       *bi)
{
    ...
    if (PREDICT_FALSE(n_left_from == 0)) return 0;

    /* speculatively copy buffer indices */
    vlib_buffer_copy_indices_to_ring(txq->bufs, bi, txq->tail & mask, RDMA_TXQ_BUF_SZ(txq), n_left_from);
...
VNET_DEVICE_CLASS_TX_FN(rdma_device_class)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    rdma_txq_t                      *txq = vec_elt_at_index(rd->txqs, vm->thread_index % vec_len(rd->txqs));
    ...
    if (PREDICT_FALSE(n_left)) {
        ...
        vlib_error_count(vm, node->node_index, RDMA_TX_ERROR_NO_FREE_SLOTS, n_left);
    }
