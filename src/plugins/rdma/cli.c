...
static clib_error_t *rdma_create_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!unformat_user(input, unformat_rdma_create_if_args, &args)) return clib_error_return(0, "unknown input `%U'", format_unformat_error, input);
...
static clib_error_t *rdma_delete_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (hw == NULL || rdma_device_class.index != hw->dev_class_index) return clib_error_return(0, "not a RDMA interface");
...
static clib_error_t *test_rdma_dump_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if ((rd->flags & RDMA_DEVICE_F_MLX5DV) == 0) return clib_error_return(0, "not a mlx5 interface");

    vlib_cli_output(vm, "netdev %s pci-addr %U lkey 0x%x", rd->linux_ifname, format_vlib_pci_addr, &rd->pci->addr, &rd->lkey);
