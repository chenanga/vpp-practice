...
static u8 rdma_rss_hash_key[] = {
    0x2c, 0xc6, 0x81, 0xd1, 0x5b, 0xdb, 0xf4, 0xf7, 0xfc, 0xa2, 0x83, 0x19, 0xdb, 0x1a, 0x3e, 0x94, 0x6b, 0x9e, 0x38, 0xd9,
    0x2c, 0x9c, 0x03, 0xd1, 0xad, 0x99, 0x44, 0xa7, 0xd9, 0x56, 0x3d, 0x59, 0x06, 0x3c, 0x25, 0xf3, 0xfc, 0x1f, 0xdc, 0x2a,
};
...
#define rdma_log__(lvl, dev, f, ...)                                                \
    do {                                                                            \
        vlib_log((lvl), rdma_main.log_class, "%s: " f, (dev)->name, ##__VA_ARGS__); \
    } while (0)
                                           ...
                                           u32                  flags)
{
    ...
    if (!flow) rdma_log(VLIB_LOG_LEVEL_ERR, rd, "ibv_create_flow() failed");
    return flow;
}
...
static u32 rdma_dev_set_promisc(rdma_device_t *rd)
{
    ...
    rd->flow_ucast6 = rdma_rxq_init_flow(rd, rd->rx_qp6, &all, &all, ntohs(ETH_P_IPV6), 0);
    rd->flow_ucast4 = rdma_rxq_init_flow(rd, rd->rx_qp4, &all, &all, 0, 0);
    if (!rd->flow_ucast6 || !rd->flow_ucast4) return ~0;
...
static u32 rdma_dev_set_ucast(rdma_device_t *rd)
{
    ...
    rd->flow_ucast6 = rdma_rxq_init_flow(rd, rd->rx_qp6, &rd->hwaddr, &ucast, ntohs(ETH_P_IPV6), 0);
    rd->flow_mcast6 = rdma_rxq_init_flow(rd, rd->rx_qp6, &mcast, &mcast, ntohs(ETH_P_IPV6), IBV_FLOW_ATTR_FLAGS_DONT_TRAP
                                         /* let others receive mcast packet too (eg. Linux) */
    );
    rd->flow_ucast4 = rdma_rxq_init_flow(rd, rd->rx_qp4, &rd->hwaddr, &ucast, 0, 0);
    rd->flow_mcast4 = rdma_rxq_init_flow(rd, rd->rx_qp4, &mcast, &mcast, 0, IBV_FLOW_ATTR_FLAGS_DONT_TRAP
                                         /* let others receive mcast packet too (eg. Linux) */
    );
    if (!rd->flow_ucast6 || !rd->flow_mcast6 || !rd->flow_ucast4 || !rd->flow_mcast4) return ~0;
...
static clib_error_t *rdma_set_max_frame_size(vnet_main_t *vnm, vnet_hw_interface_t *hw, u32 frame_size)
{
...
static void rdma_update_state(vnet_main_t *vnm, rdma_device_t *rd, int port)
{
    ...
    switch (attr.state) {
        ...
        case IBV_PORT_ACTIVE_DEFER:
            ...
            vnet_hw_interface_set_flags(vnm, rd->hw_if_index, VNET_HW_INTERFACE_FLAG_LINK_UP);
            break;
        default:
            rd->flags &= ~RDMA_DEVICE_F_LINK_UP;
            vnet_hw_interface_set_flags(vnm, rd->hw_if_index, 0);
...
static clib_error_t *rdma_async_event_read_ready(clib_file_t *f)
{
    ...
    if (ret < 0) return clib_error_return_unix(0, "ibv_get_async_event() failed");
    ...
    switch (event.event_type) {
        ...
        case IBV_EVENT_DEVICE_FATAL:
            ...
            break;
        default:
            rdma_log__(VLIB_LOG_LEVEL_ERR, rd, "unhandeld RDMA async event %d", event.event_type);
            break;
    }
...
static clib_error_t *rdma_async_event_init(rdma_device_t *rd)
{
    ...
    if (ret < 0) return clib_error_return_unix(0, "fcntl(F_GETFL) failed");

    ret = fcntl(rd->ctx->async_fd, F_SETFL, ret | O_NONBLOCK);
    if (ret < 0) return clib_error_return_unix(0, "fcntl(F_SETFL, O_NONBLOCK) failed");
...
#define _(fn, arg)                                                                             \
    if (arg) {                                                                                 \
        int rv;                                                                                \
        if ((rv = fn(arg))) rdma_log(VLIB_LOG_LEVEL_DEBUG, rd, #fn "() failed (rv = %d)", rv); \
    }
...
static clib_error_t *rdma_rxq_init(vlib_main_t *vm, rdma_device_t *rd, u16 qid, u32 n_desc, u8 no_multi_seg, u16 max_pktlen)
{
    ...
    if (is_mlx5dv) {
        ...
        dvcq.comp_mask                  = MLX5DV_CQ_INIT_ATTR_MASK_COMPRESSED_CQE | MLX5DV_CQ_INIT_ATTR_MASK_CQE_SIZE;
        dvcq.cqe_comp_res_format        = MLX5DV_CQE_RES_FORMAT_HASH;
        dvcq.cqe_size                   = 64;
        if ((cqex = mlx5dv_create_cq(rd->ctx, &cqa, &dvcq)) == 0) return clib_error_return_unix(0, "Create mlx5dv rx CQ Failed");
    }
    else {
        if ((cqex = ibv_create_cq_ex(rd->ctx, &cqa)) == 0) return clib_error_return_unix(0, "Create CQ Failed");
    }
    ...
    if (is_mlx5dv) {
        if (is_striding) {
             ...
             */
            int max_chain_log_sz                        = max_pktlen ? max_log2((max_pktlen / (rxq->buf_sz)) + 1) : RDMA_RXQ_MAX_CHAIN_LOG_SZ;
            ...
            dv_wqia.striding_rq_attrs.single_wqe_log_num_of_strides  = max_chain_log_sz;
            dv_wqia.striding_rq_attrs.single_stride_log_num_of_bytes = data_seg_log2_sz;
            ...
            if (no_multi_seg) {
                ...
                int max_chain_sz     = max_pktlen ? (max_pktlen / (rxq->buf_sz)) + 1 : RDMA_RXQ_LEGACY_MODE_MAX_CHAIN_SZ;
    ...
    if (ibv_modify_wq(rxq->wq, &wqa) != 0) return clib_error_return_unix(0, "Modify WQ (RDY) Failed");
    ...
    if (is_mlx5dv) {
        ...
        if ((mlx5dv_init_obj(&obj, MLX5DV_OBJ_CQ | MLX5DV_OBJ_RWQ))) return clib_error_return_unix(0, "mlx5dv: failed to init rx obj");

        if (dv_cq.cqe_size != sizeof(mlx5dv_cqe_t)) return clib_error_return_unix(0, "mlx5dv: incompatible rx CQE size");
        ...
        for (int i = 0; i < rxq->wqe_cnt << rxq->log_wqe_sz; i++)
            if ((!is_striding && ((i & wqe_sz_mask) < rxq->n_ds_per_wqe))
                || (is_striding && ((i == 0) || !(((i - 1) >> rxq->log_stride_per_wqe) & 0x1))))
...
static uint64_t rdma_rss42ibv(const rdma_rss4_t rss4)
{
    switch (rss4) {
        ...
        case RDMA_RSS4_IP_UDP:
            return IBV_RX_HASH_SRC_IPV4 | IBV_RX_HASH_DST_IPV4 | IBV_RX_HASH_SRC_PORT_UDP | IBV_RX_HASH_DST_PORT_UDP;
        case RDMA_RSS4_AUTO: /* fallthrough */
        case RDMA_RSS4_IP_TCP:
            return IBV_RX_HASH_SRC_IPV4 | IBV_RX_HASH_DST_IPV4 | IBV_RX_HASH_SRC_PORT_TCP | IBV_RX_HASH_DST_PORT_TCP;
    }
    ASSERT(0);
    return 0;
}
...
static uint64_t rdma_rss62ibv(const rdma_rss6_t rss6)
{
    switch (rss6) {
        ...
        case RDMA_RSS6_IP_UDP:
            return IBV_RX_HASH_SRC_IPV6 | IBV_RX_HASH_DST_IPV6 | IBV_RX_HASH_SRC_PORT_UDP | IBV_RX_HASH_DST_PORT_UDP;
        case RDMA_RSS6_AUTO: /* fallthrough */
        case RDMA_RSS6_IP_TCP:
            return IBV_RX_HASH_SRC_IPV6 | IBV_RX_HASH_DST_IPV6 | IBV_RX_HASH_SRC_PORT_TCP | IBV_RX_HASH_DST_PORT_TCP;
    }
    ASSERT(0);
    return 0;
}
...
static clib_error_t *rdma_rxq_finalize(vlib_main_t *vm, rdma_device_t *rd)
{
    ...
    if (!is_pow2(ind_tbl_sz)) {
        ...
        if (ibv_query_device_ex(rd->ctx, 0, &attr)) return clib_error_return_unix(0, "device query failed");
        ind_tbl_sz = attr.rss_caps.max_rwq_indirection_table_size;
        if (ind_tbl_sz < rxq_sz)
            return clib_error_create(
                "too many rxqs requested (%d) compared to "
    ...
    qpia.comp_mask   = IBV_QP_INIT_ATTR_PD | IBV_QP_INIT_ATTR_IND_TABLE | IBV_QP_INIT_ATTR_RX_HASH;
    qpia.pd          = rd->pd;
    ...
    if ((rd->rx_qp4 = ibv_create_qp_ex(rd->ctx, &qpia)) == 0) return clib_error_return_unix(0, "IPv4 Queue Pair create failed");

    qpia.rx_hash_conf.rx_hash_fields_mask = rdma_rss62ibv(rd->rss6);
    if ((rd->rx_qp6 = ibv_create_qp_ex(rd->ctx, &qpia)) == 0) return clib_error_return_unix(0, "IPv6 Queue Pair create failed");

    if (rdma_dev_set_ucast(rd)) return clib_error_return_unix(0, "Set unicast mode failed");
...
static clib_error_t *rdma_txq_init(vlib_main_t *vm, rdma_device_t *rd, u16 qid, u32 n_desc)
{
    ...
    if (is_mlx5dv) {
        ...
        dvcq.cqe_size                   = 64;
        cqa.cqe                         = n_desc;
        if ((cqex = mlx5dv_create_cq(rd->ctx, &cqa, &dvcq)) == 0) return clib_error_return_unix(0, "Create mlx5dv tx CQ Failed");
        txq->cq = ibv_cq_ex_to_cq(cqex);
    }
    else {
        if ((txq->cq = ibv_create_cq(rd->ctx, n_desc, NULL, NULL, 0)) == 0) return clib_error_return_unix(0, "Create CQ Failed");
    }
    ...
    if ((txq->qp = ibv_create_qp(rd->pd, &qpia)) == 0) return clib_error_return_unix(0, "Queue Pair create failed");

    memset(&qpa, 0, sizeof(qpa));
    qp_flags     = IBV_QP_STATE | IBV_QP_PORT;
    qpa.qp_state = IBV_QPS_INIT;
    qpa.port_num = 1;
    if (ibv_modify_qp(txq->qp, &qpa, qp_flags) != 0) return clib_error_return_unix(0, "Modify QP (init) Failed");

    memset(&qpa, 0, sizeof(qpa));
    qp_flags     = IBV_QP_STATE;
    qpa.qp_state = IBV_QPS_RTR;
    if (ibv_modify_qp(txq->qp, &qpa, qp_flags) != 0) return clib_error_return_unix(0, "Modify QP (receive) Failed");

    memset(&qpa, 0, sizeof(qpa));
    qp_flags     = IBV_QP_STATE;
    qpa.qp_state = IBV_QPS_RTS;
    if (ibv_modify_qp(txq->qp, &qpa, qp_flags) != 0) return clib_error_return_unix(0, "Modify QP (send) Failed");
    ...
    if (rd->flags & RDMA_DEVICE_F_MLX5DV) {
        ...
        if (mlx5dv_init_obj(&obj, MLX5DV_OBJ_CQ | MLX5DV_OBJ_QP)) return clib_error_return_unix(0, "DV init obj failed");

        if (RDMA_TXQ_BUF_SZ(txq) > dv_qp.sq.wqe_cnt || !is_pow2(dv_qp.sq.wqe_cnt) || sizeof(rdma_mlx5_wqe_t) != dv_qp.sq.stride
            || (uword) dv_qp.sq.buf % sizeof(rdma_mlx5_wqe_t))
            return clib_error_return(0, "Unsupported DV SQ parameters");

        if (RDMA_TXQ_BUF_SZ(txq) > dv_cq.cqe_cnt || !is_pow2(dv_cq.cqe_cnt) || sizeof(struct mlx5_cqe64) != dv_cq.cqe_size
        ...
        mlx5dv_set_ctrl_seg(&tmpl->ctrl, 0, MLX5_OPCODE_SEND, 0, txq->qp->qp_num, 0, RDMA_MLX5_WQE_DS, 0, RDMA_TXQ_DV_INVALID_ID);
...
static clib_error_t *rdma_dev_init(vlib_main_t *vm, rdma_device_t *rd, rdma_create_if_args_t *args)
{
    ...
    if (rd->ctx == 0) return clib_error_return_unix(0, "Device Open Failed");

    if ((rd->pd = ibv_alloc_pd(rd->ctx)) == 0) return clib_error_return_unix(0, "PD Alloc Failed");

    if ((rd->mr = ibv_reg_mr(rd->pd, (void *) bm->buffer_mem_start, bm->buffer_mem_size, IBV_ACCESS_LOCAL_WRITE)) == 0)
    ...
    for (i = 0; i < tm->n_vlib_mains; i++)
        if ((err = rdma_txq_init(vm, rd, i, txq_size))) return err;

    for (i = 0; i < rxq_num; i++)
        if ((err = rdma_rxq_init(vm, rd, i, rxq_size, args->no_multi_seg, args->max_pktlen))) return err;
    if ((err = rdma_rxq_finalize(vm, rd))) return err;
...
void rdma_create_if(vlib_main_t *vm, rdma_create_if_args_t *args)
{
    ...
    if (args->rxq_size < VLIB_FRAME_SIZE || args->txq_size < VLIB_FRAME_SIZE || args->rxq_size > 65535 || args->txq_size > 65535
    ...
    if (n_devs == 0) {
        args->error = clib_error_return_unix(0, "no RDMA devices available. Is the ib_uverbs module loaded?");
        goto err0;
    }
    ...
    if (sysfs_path_to_pci_addr((char *) s, &pci_addr) == 0) {
        args->error = clib_error_return(0, "cannot find PCI address for device ");
        goto err1;
    }
    ...
    if (-1 == rd->pci->numa_node) rd->pci->numa_node = 0;
    ...
    for (i = 0; i < n_devs; i++) {
        ...
        if (sysfs_path_to_pci_addr((char *) s, &addr) == 0) continue;

        if (addr.as_u32 != rd->pci->addr.as_u32) continue;

        if ((rd->ctx = ibv_open_device(dev_list[i]))) break;
    }
    ...
    if (args->mode != RDMA_MODE_IBV) {
        ...
        if (mlx5dv_query_device(rd->ctx, &mlx5dv_attrs) == 0) {
            ...
            if ((mlx5dv_attrs.flags & MLX5DV_CONTEXT_FLAGS_CQE_V1)) rd->flags |= RDMA_DEVICE_F_MLX5DV;
            ...
            if (!args->no_multi_seg && !args->disable_striding_rq
                && data_seg_log2_sz <= mlx5dv_attrs.striding_rq_caps.max_single_stride_log_num_of_bytes
                && data_seg_log2_sz >= mlx5dv_attrs.striding_rq_caps.min_single_stride_log_num_of_bytes
                && RDMA_RXQ_MAX_CHAIN_LOG_SZ >= mlx5dv_attrs.striding_rq_caps.min_single_wqe_log_num_of_strides
                && RDMA_RXQ_MAX_CHAIN_LOG_SZ <= mlx5dv_attrs.striding_rq_caps.max_single_wqe_log_num_of_strides)
                rd->flags |= RDMA_DEVICE_F_STRIDING_RQ;
        }
        else {
            if (args->mode == RDMA_MODE_DV) {
                args->error = clib_error_return(0,
                                                "Direct Verbs mode not "
    ...
    if ((args->error = rdma_dev_init(vm, rd, args))) goto err2;

    if ((args->error = rdma_register_interface(vnm, rd))) goto err2;

    if ((args->error = rdma_async_event_init(rd))) goto err3;
    ...
    vec_foreach_index (qid, rd->rxqs) {
        u32 queue_index           = vnet_hw_if_register_rx_queue(vnm, rd->hw_if_index, qid, VNET_HW_IF_RXQ_THREAD_ANY);
...
static clib_error_t *rdma_interface_admin_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    ...
    if (rd->flags & RDMA_DEVICE_F_ERROR) return clib_error_return(0, "device is in error state");
...
static void rdma_set_interface_next_node(vnet_main_t *vnm, u32 hw_if_index, u32 node_index)
{
    ...
    rd->per_interface_next_index =
        ~0 == node_index ? VNET_DEVICE_INPUT_NEXT_ETHERNET_INPUT : vlib_node_add_next(vlib_get_main(), rdma_input_node.index, node_index);
}
...
clib_error_t *rdma_init(vlib_main_t *vm)
{
    ...
    vec_validate_aligned(rm->per_thread_data, tm->n_vlib_mains - 1, CLIB_CACHE_LINE_BYTES);
