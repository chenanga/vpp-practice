...
u8 *format_rdma_device_name(u8 *s, va_list *args)
{
    ...
    if (rd->name) return format(s, "%v", rd->name);
...
#define _(a, b, c) \
    if (rd->flags & (1 << a)) t = format(t, "%s%s", t ? " " : "", c);
...
u8 *format_rdma_device(u8 *s, va_list *args)
{
    ...
    if ((d = vlib_pci_get_device_info(vm, &rd->pci->addr, 0))) {
        s = format(s, "%Uproduct name: %v\n", format_white_space, indent, d->product_name);
        s = format(s, "%Upart number: %U\n", format_white_space, indent, format_vlib_pci_vpd, d->vpd_r, "PN");
        s = format(s, "%Urevision: %U\n", format_white_space, indent, format_vlib_pci_vpd, d->vpd_r, "EC");
        s = format(s, "%Userial number: %U\n", format_white_space, indent, format_vlib_pci_vpd, d->vpd_r, "SN");
        vlib_pci_free_device_info(d);
    }
    s = format(s, "%Uflags: %U\n", format_white_space, indent, format_rdma_device_flags, rd);
    s = format(s, "%Urss: %U %U", format_white_space, indent, format_rdma_rss4, &rd->rss4, format_rdma_rss6, &rd->rss6);
    if (rd->error) s = format(s, "\n%Uerror %U", format_white_space, indent, format_clib_error, rd->error);

    if (rd->flags & RDMA_DEVICE_F_MLX5DV) {
        struct mlx5dv_context c  = {};
        const char *str_flags[7] = {"cqe-v1", "obsolete", "mpw-allowed", "enhanced-mpw", "cqe-128b-comp", "cqe-128b-pad", "packet-based-credit-mode"};

        if (mlx5dv_query_device(rd->ctx, &c) != 0) return s;

        s = format(s, "\n%Umlx5: version %u", format_white_space, indent, c.version);
        s = format(s, "\n%Udevice flags: %U", format_white_space, indent + 2, format_rdma_bit_flag, c.flags, str_flags, ARRAY_LEN(str_flags));
    }
...
u8 *format_rdma_input_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "rdma: %v (%d) next-node %U", hi->name, t->hw_if_index, format_vlib_next_node_name, vm, node->index, t->next_index);

    if (t->cqe_flags & CQE_FLAG_L2_OK) s = format(s, " l2-ok");

    if (t->cqe_flags & CQE_FLAG_L3_OK) s = format(s, " l3-ok");

    if (t->cqe_flags & CQE_FLAG_L4_OK) s = format(s, " l4-ok");

    if (t->cqe_flags & CQE_FLAG_IP_FRAG) s = format(s, " ip-frag");

    if (l3_hdr_type) s = format(s, " %s", l3_hdr_types[l3_hdr_type]);

    if (l4_hdr_type) s = format(s, " %s", l4_hdr_types[l4_hdr_type]);

    if ((t->cqe_flags & CQE_FLAG_IP_EXT_OPTS)) {
        if (l3_hdr_type == CQE_FLAG_L3_HDR_TYPE_IP6) s = format(s, " ip4-ext-hdr");
        if (l3_hdr_type == CQE_FLAG_L3_HDR_TYPE_IP4) s = format(s, " ip4-opt");
    }
...
#define _(a, b, c, d) \
    if (mlx5_get_bits(cqe, a, b, c)) s = format(s, "%U%U\n", format_white_space, line++ ? indent : 0, format_mlx5_field, cqe, a, b, c, #d);
...
u8 *format_rdma_rxq(u8 *s, va_list *args)
{
    ...
    if (rd->flags & RDMA_DEVICE_F_MLX5DV) {
        ...
        s                  = format(s, "\n%Uwq: stride %u wqe-cnt %u", format_white_space, indent + 2, rxq->wq_stride, rxq->wqe_cnt);
        s = format(s, "\n%Ucq: cqn %u cqe-cnt %u ci %u", format_white_space, indent + 2, rxq->cqn, 1 << rxq->log2_cq_size, rxq->cq_ci);
        s = format(s, "\n%Unext-cqe(%u):", format_white_space, indent + 4, next_cqe_index);
        s = format(s, "\n%U%U", format_white_space, indent + 6, format_mlx5_cqe_rx, rxq->cqes + next_cqe_index);
        s = format(s, "\n%U%U", format_white_space, indent + 6, format_hexdump, rxq->cqes + next_cqe_index, (u32) sizeof(mlx5dv_cqe_t));
    }
