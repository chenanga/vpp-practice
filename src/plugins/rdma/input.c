                                                                    ...
                                                                    u32                     n_alloc)
{
    ...
    for (i = 0; i < n_alloc; i++) {
        ...
        u8               chain_sz_alloc;
        mlx5dv_wqe_ds_t *current_wqe = rxq->wqes + ((first_slot + i) << log_wqe_sz);
        if (chain_sz == 0) continue;
        if (PREDICT_FALSE((chain_sz_alloc = vlib_buffer_alloc_from_pool(vm, bi, chain_sz, rd->pool)) != chain_sz)) {
                                                   ...
                                                   const int      is_striding)
{
    ...
    if (PREDICT_FALSE(is_mlx5dv && !is_striding && (rxq->incomplete_tail != rxq->tail))) {
        int n_incomplete = rxq->incomplete_tail - rxq->tail;
        int n_completed  = rdma_device_legacy_input_refill_additional(vm, rd, rxq, ptd, bt, slot, n_incomplete);
        rxq->tail += n_completed;
        slot = rxq->tail & mask;
        /* Don't start recycling head buffers if there are incomplete chains */
        if (n_completed != n_incomplete) return;
    }
    ...
    if (n_alloc < 2 * alloc_multiple) return;
    ...
    n = vlib_buffer_alloc_to_ring_from_pool(vm, rxq->bufs, slot, rxq->size, n_alloc, rd->pool);

    if (PREDICT_FALSE(n != n_alloc)) {
        u32 n_free;
        if (n < alloc_multiple) {
            if (n) vlib_buffer_free_from_ring(vm, rxq->bufs, slot, rxq->size, n);
            return;
        }
        ...
        n -= n_free;
        if (n_free) vlib_buffer_free_from_ring(vm, rxq->bufs, (slot + n) & mask, rxq->size, n_free);
    }

    n_alloc = n;

    if (is_mlx5dv) {
        u64 __clib_aligned(32)
        va[8];
        ...
        while (n >= 1) {
            vlib_get_buffers_with_offset(vm, rxq->bufs + slot, (void **) va, 8, sizeof(vlib_buffer_t));
            ...
            *(u64x4 *) (va + 4) = u64x4_byte_swap(*(u64x4 *) (va + 4));
            ...
            if (is_striding && !(current_data_seg & (wqe_sz - 1)))
                *(mlx5dv_wqe_srq_next_t *) wqe =
                    (mlx5dv_wqe_srq_next_t){.rsvd0          = {0},
        ...
        if (!is_striding) {
            ...
            if (PREDICT_FALSE(rxq->n_total_additional_segs))
                n_alloc = rdma_device_legacy_input_refill_additional(vm, rd, rxq, ptd, bt, first_slot, n_alloc);
        }
        CLIB_MEMORY_STORE_BARRIER();
        rxq->tail += n_alloc;
        if (is_striding) {
            rxq->striding_wqe_tail += n_alloc >> log_stride_per_wqe;
            rxq->wq_db[MLX5_RCV_DBR] = clib_host_to_net_u32(rxq->striding_wqe_tail);
        }
        else
                                                  ...
                                                  int                  is_mlx5dv)
{
    ...
    if (PREDICT_TRUE(0 == n_trace)) return;
                                                     ...
                                                     int                  skip_ip4_cksum)
{
    ...
    if (PREDICT_FALSE(VNET_DEVICE_INPUT_NEXT_ETHERNET_INPUT != next_index)) return;

    nf       = vlib_node_runtime_get_next_frame(vm, node, VNET_DEVICE_INPUT_NEXT_ETHERNET_INPUT);
    f        = vlib_get_frame(vm, nf->frame);
    f->flags = ETH_INPUT_FRAME_F_SINGLE_SW_IF_IDX;
    if (skip_ip4_cksum) f->flags |= ETH_INPUT_FRAME_F_IP4_CKSUM_OK;
...
rdma_device_input_bufs(vlib_main_t *vm, const rdma_device_t *rd, vlib_buffer_t **b, struct ibv_wc *wc, u32 n_left_from, vlib_buffer_t *bt)
{
    u32 n_rx_bytes = 0;
...
static_always_inline uword rdma_device_poll_cq_mlx5dv(rdma_device_t *rd, rdma_rxq_t *rxq, u32 *byte_cnt, u16 *cqe_flags)
{
    ...
    if (rxq->n_mini_cqes_left) {
        ...
        process_mini_cqes(rxq, n_mini_cqes - n_mini_cqes_left, n_mini_cqes_left, cq_ci, mask, byte_cnt);
        compressed_cqe_reset_owner(rxq, n_mini_cqes, cq_ci, mask, log2_cq_size);
    ...
    while (n_rx_packets < VLIB_FRAME_SIZE) {
        ...
        if ((cqe_last_byte & 0x1) != owner) break;
    ...
    if (n_rx_packets) rxq->cq_db[0] = rxq->cq_ci = cq_ci;
    return n_rx_packets;
}
...
static_always_inline int rdma_device_mlx5dv_striding_rq_parse_bc(int n_rx_packets, int *n_rx_segs, u32 *bc)
{
    ...
    for (int i = 0; i < n_rx_packets; i++) {
        *n_rx_segs += (bc[i] & CQE_BC_CONSUMED_STRIDES_MASK) >> CQE_BC_CONSUMED_STRIDES_SHIFT;
...
static_always_inline int rdma_device_mlx5dv_legacy_rq_slow_path_needed(u32 buf_sz, int n_rx_packets, u32 *bc)
{
    ...
    for (int i = 0; i < n_rx_packets; i += 8)
        if (!u32x8_is_all_zero(*(u32x8 *) (bc + i) > thresh8)) return 1;
#elif defined CLIB_HAVE_VEC128
    u32x4 thresh4 = u32x4_splat(buf_sz);
    for (int i = 0; i < n_rx_packets; i += 4)
        if (!u32x4_is_all_zero(*(u32x4 *) (bc + i) > thresh4)) return 1;
#else
    while (n_rx_packets) {
        if (*bc > buf_sz) return 1;
        bc++;
        n_rx_packets--;
    }
#endif
...
static_always_inline int rdma_device_mlx5dv_l3_validate_and_swap_bc(rdma_per_thread_data_t *ptd, int n_rx_packets, u32 *bc)
{
    ...
    u16 match = CQE_FLAG_L3_HDR_TYPE_IP4 << CQE_FLAG_L3_HDR_TYPE_SHIFT | CQE_FLAG_L3_OK;
    ...
    if (n_left >= 16) {
        ...
        if (!u16x16_is_all_zero(r16)) skip_ip4_cksum = 0;
    }
#elif defined CLIB_HAVE_VEC128
    if (n_left >= 8) {
        ...
        if (!u16x8_is_all_zero(r8)) skip_ip4_cksum = 0;
    }
#endif

    while (n_left >= 1) {
        if ((cqe_flags[0] & mask) != match) skip_ip4_cksum = 0;
                                                       ...
                                                       u32             bc_mask)
{
    ...
    u32             n_rx_bytes = 0;
    vlib_buffer_copy_indices_from_ring(to_next, rxq->bufs, rxq->head & qs_mask, rxq->size, n_rx_segs);
    rxq->head += n_rx_segs;
                                                              ...
                                                              int                     slow_path_needed)
{
    ...
    if (PREDICT_TRUE(!slow_path_needed)) {
        ...
        n_rx_bytes += rdma_device_mlx5dv_fast_input(vm, rxq, bufs, mask, bt, to_next, n_rx_segs, bc, CQE_BC_BYTE_COUNT_MASK);
    }
    else /* Slow path with multiseg */
    {
        ...
        do {
            ...
            n_rx_segs -= n_left;
            vlib_buffer_copy_indices_from_ring(ptd->current_segs, rxq->bufs, rxq->head & mask, rxq->size, n_left);
            rxq->head += n_left;
            ...
            while (n_left > 0) {
                ...
                {
                    n_segs_remaining                                  = (bc[0] & CQE_BC_CONSUMED_STRIDES_MASK) >> CQE_BC_CONSUMED_STRIDES_SHIFT;
                    pkt_head                                          = pkt[0];
                    pkt_head_idx                                      = ptd->current_segs[pkt - bufs];
                    n_bytes_remaining                                 = bc[0] & CQE_BC_BYTE_COUNT_MASK;
                    pkt_head->total_length_not_including_first_buffer = n_segs_remaining > 1 ? n_bytes_remaining - pkt[0]->current_length : 0;
                }
                ...
                if (n_segs_remaining == 1) /* Last buffer of the chain */
                {
                    ...
                    if (bc[0] & CQE_BC_FILLER_MASK) {
                        ...
                        n_rx_bytes += pkt_head->current_length + pkt_head->total_length_not_including_first_buffer;
                    }
                    /*Go to next CQE */
                    bc++;
                }
        ...
        vlib_buffer_free(vm, ptd->to_free_buffers, next_to_free - ptd->to_free_buffers);
    }
    return n_rx_bytes;
}

static_always_inline uword
rdma_device_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, rdma_device_t *rd, u16 qid, const int use_mlx5dv)
{
    ...
    u32                     __clib_aligned(32)
    byte_cnts[VLIB_FRAME_SIZE];
    vlib_buffer_t bt;
    ...
    if (use_mlx5dv)
        n_rx_packets = rdma_device_poll_cq_mlx5dv(rd, rxq, byte_cnts, ptd->cqe_flags);
    else
    ...
    if (PREDICT_FALSE(n_rx_packets <= 0)) goto refill;

    /* update buffer template for input feature arcs if any */
    next_index = rd->per_interface_next_index;
    if (PREDICT_FALSE(vnet_device_input_have_features(rd->sw_if_index))) vnet_feature_start_device_input(rd->sw_if_index, &next_index, &bt);
    ...
    if (use_mlx5dv) {
        u32 *bc = byte_cnts;
        int  slow_path_needed;
        skip_ip4_cksum = rdma_device_mlx5dv_l3_validate_and_swap_bc(ptd, n_rx_packets, bc);
        if (is_striding) {
            int n_rx_segs    = 0;
            slow_path_needed = rdma_device_mlx5dv_striding_rq_parse_bc(n_rx_packets, &n_rx_segs, bc);
            n_rx_bytes       = rdma_device_mlx5dv_striding_rq_input(vm, ptd, rxq, &bt, to_next, n_rx_segs, &n_rx_packets, bc, slow_path_needed);
        }
        else {
            vlib_buffer_t *bufs[VLIB_FRAME_SIZE];
            slow_path_needed = rdma_device_mlx5dv_legacy_rq_slow_path_needed(rxq->buf_sz, n_rx_packets, bc);
            n_rx_bytes       = rdma_device_mlx5dv_fast_input(vm, rxq, bufs, mask, &bt, to_next, n_rx_packets, bc, ~0);

            /* If there are chained buffers, some of the head buffers have a current length
               higher than buf_sz: it needs to be fixed */
            if (PREDICT_FALSE(slow_path_needed)) rdma_device_mlx5dv_legacy_rq_fix_chains(vm, rxq, bufs, mask, n_rx_packets);
        }
    }
    else {
        vlib_buffer_t *bufs[VLIB_FRAME_SIZE];
        vlib_buffer_copy_indices_from_ring(to_next, rxq->bufs, rxq->head & mask, rxq->size, n_rx_packets);
    ...
    rdma_device_input_trace(vm, node, rd, n_rx_packets, to_next, next_index, ptd->cqe_flags, use_mlx5dv);
    /* reset flags to zero for the next run */
    if (use_mlx5dv) clib_memset_u16(ptd->cqe_flags, 0, VLIB_FRAME_SIZE);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    for (int i = 0; i < vec_len(pv); i++) {
        ...
        if (PREDICT_TRUE(rd->flags & RDMA_DEVICE_F_ADMIN_UP) == 0) continue;

        if (PREDICT_FALSE(rd->flags & RDMA_DEVICE_F_ERROR)) continue;

        if (PREDICT_TRUE(rd->flags & RDMA_DEVICE_F_MLX5DV))
            n_rx += rdma_device_input_inline(vm, node, frame, rd, pv[i].queue_id, 1);
        else
            n_rx += rdma_device_input_inline(vm, node, frame, rd, pv[i].queue_id, 0);
    }
    return n_rx;
}
