...
static vlib_buffer_t *igmp_pkt_get_buffer(igmp_pkt_build_t *bk)
{
    ...
    if (vlib_buffer_alloc(vm, &bi, 1) != 1) return (NULL);
    ...
    bk->n_avail = vnet_sw_interface_get_mtu(vnet_get_main(), bk->sw_if_index, VNET_MTU_IP4);
...
static vlib_buffer_t *igmp_pkt_build_ip_header(igmp_pkt_build_t *bk, igmp_msg_type_t msg_type, const igmp_group_t *group)
{
    ...
    switch (msg_type) {
        ...
        case IGMP_MSG_QUERY:
            if (group != NULL)
                clib_memcpy_fast(&ip4->dst_address, &group->key->ip4, sizeof(ip4_address_t));
            else
...
static void igmp_pkt_tx(igmp_pkt_build_t *bk)
{
    ...
    IGMP_DBG("  ..tx: %U", format_vnet_sw_if_index_name, vnet_get_main(), bk->sw_if_index);
...
{
    if (NULL == br->base.buffers) return (NULL);

    return (vlib_get_buffer(vlib_get_main(), br->base.buffers[vec_len(br->base.buffers) - 1]));
}
...
static void igmp_pkt_build_report_bake(igmp_pkt_build_report_t *br)
{
    ...
    igmp->header.checksum = ~ip_csum_fold(ip_incremental_checksum(0, igmp, br->base.n_bytes));
...
{
    if (NULL == br->base.buffers) return;
...
static u32 igmp_pkt_report_v3_get_size(const igmp_group_t *group)
{
    ...
    return ((hash_elts(group->igmp_src_by_key[IGMP_FILTER_MODE_INCLUDE]) * sizeof(ip4_address_t)) + sizeof(igmp_membership_group_v3_t));
}
                                   ...
                                   igmp_membership_group_v3_type_t type)
{
    ...
    if (NULL == b) {
        b = igmp_pkt_build_report_v3(br, NULL);
        if (NULL == b) /* failed to allocate buffer */
            return;
    }

    igmp_group = igmp_pkt_report_v3_append_group(br, grp, type);

    if (NULL == igmp_group) return;

    vec_foreach (s, srcs) {
        igmp_group = igmp_pkt_report_v3_append_src(br, igmp_group, grp, type, s);
        if (NULL == igmp_group) return;
    };
...
void igmp_pkt_report_v3_add_group(igmp_pkt_build_report_t *br, const igmp_group_t *group, igmp_membership_group_v3_type_t type)
{
     ...
     */
    if ((0 != br->n_groups) && (igmp_pkt_report_v3_get_size(group) > br->base.n_avail)) {
        igmp_pkt_build_report_bake(br);
        b = igmp_pkt_build_report_v3(br, NULL);
        if (NULL == b) /* failed to allocate buffer */
            return;
    ...
    FOR_EACH_SRC(src, group, IGMP_FILTER_MODE_INCLUDE, ({
                     igmp_group = igmp_pkt_report_v3_append_src(br, igmp_group, group->key, type, src->key);
                     if (NULL == igmp_group) return;
                 }));
    igmp_group->n_src_addresses = clib_host_to_net_u16(br->n_srcs);

    IGMP_DBG("  ..add-group: %U srcs:%d", format_igmp_key, group->key, hash_elts(group->igmp_src_by_key[IGMP_FILTER_MODE_INCLUDE]));
}
...
{
    if (NULL == bq->base.buffers) return (NULL);

    return (vlib_get_buffer(vlib_get_main(), bq->base.buffers[vec_len(bq->base.buffers) - 1]));
}
...
static void igmp_pkt_build_query_bake(igmp_pkt_build_query_t *bq)
{
    ...
    igmp->header.checksum = ~ip_csum_fold(ip_incremental_checksum(0, igmp, bq->base.n_bytes));
...
{
    if (NULL == bq->base.buffers) return;
