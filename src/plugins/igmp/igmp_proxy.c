...
void igmp_proxy_device_mfib_path_add_del(igmp_group_t *group, u8 add)
{
    ...
    mfib_index = mfib_table_get_index_for_sw_if_index(FIB_PROTOCOL_IP4, config->sw_if_index);
    ...
    if (add)
        mfib_table_entry_path_update(mfib_index, &mpfx_group_addr, MFIB_SOURCE_IGMP, MFIB_ENTRY_FLAG_NONE, &via_itf_path);
    else
        mfib_table_entry_path_remove(mfib_index, &mpfx_group_addr, MFIB_SOURCE_IGMP, &via_itf_path);
}
...
igmp_proxy_device_t *igmp_proxy_device_lookup(u32 vrf_id)
{
    ...
    if (vec_len(im->igmp_proxy_device_by_vrf_id) > vrf_id) {
        ...
        if (index != ~0) return (vec_elt_at_index(im->proxy_devices, index));
    }
    return NULL;
}
...
int igmp_proxy_device_add_del(u32 vrf_id, u32 sw_if_index, u8 add)
{
    ...
    if (mfib_index == ~0) return VNET_API_ERROR_INVALID_INTERFACE;
    if (vrf_id != mfib_table_get(mfib_index, FIB_PROTOCOL_IP4)->mft_table_id) return VNET_API_ERROR_INVALID_INTERFACE;
    ...
    if (!proxy_device && add) {
        ...
        im->igmp_proxy_device_by_vrf_id[vrf_id] = proxy_device - im->proxy_devices;
        ...
        while (vec_len(proxy_device->downstream_ifs) > 0) {
            igmp_proxy_device_add_del_interface(vrf_id, proxy_device->downstream_ifs[0], 0);
        }
...
int igmp_proxy_device_add_del_interface(u32 vrf_id, u32 sw_if_index, u8 add)
{
    ...
    config = igmp_config_lookup(sw_if_index);
    if (!config) return VNET_API_ERROR_INVALID_INTERFACE;
    if (config->mode != IGMP_MODE_ROUTER) return VNET_API_ERROR_INVALID_INTERFACE;
    ...
    if (add) {
        ...
        if (!proxy_device->downstream_ifs) return -2;
        index = vec_search(proxy_device->downstream_ifs, sw_if_index);
        if (index == ~0) return -3;
...
void igmp_proxy_device_block_src(igmp_config_t *config, igmp_group_t *group, igmp_src_t *src)
{
    ...
    igmp_pkt_report_v3_add_report(&br, proxy_group->key, srcaddrs, IGMP_MEMBERSHIP_GROUP_block_old_sources);
...
always_inline void igmp_proxy_device_merge_src(igmp_group_t **proxy_group, igmp_src_t *src, ip46_address_t **srcaddrs, u8 block)
{
    igmp_src_t *proxy_src;
    u32         d_config;
    ...
    if (proxy_src == NULL) {
        ...
        proxy_src = igmp_src_alloc(igmp_group_index(*proxy_group), src->key, IGMP_MODE_HOST);

        hash_set_mem((*proxy_group)->igmp_src_by_key[(*proxy_group)->router_filter_mode], proxy_src->key, igmp_src_index(proxy_src));

        vec_validate_init_empty(proxy_src->referance_by_config_index, d_config, 0);
...
always_inline igmp_group_t *igmp_proxy_device_merge_group(igmp_proxy_device_t *proxy_device, igmp_group_t *group, ip46_address_t **srcaddrs, u8 block)
{
    igmp_config_t *proxy_config;
    igmp_group_t  *proxy_group;
    igmp_src_t    *src;
    ...
    if (!proxy_group) {
        ...
        proxy_group = igmp_group_alloc(proxy_config, group->key, group->router_filter_mode);
    ...
    FOR_EACH_SRC(src, group, group->router_filter_mode, ({ igmp_proxy_device_merge_src(&proxy_group, src, srcaddrs, block); }));
    return proxy_group;
}
    ...
    FOR_EACH_GROUP(group, config, ({
                       ...
                       if ((vec_len(srcaddrs) > 0) && proxy_group) {
                           igmp_pkt_report_v3_add_report(&br,
                                                         proxy_group->key,
                                                         srcaddrs,
                                                         block ? IGMP_MEMBERSHIP_GROUP_block_old_sources : IGMP_MEMBERSHIP_GROUP_allow_new_sources);
                       }
                       vec_free(srcaddrs);
                   }));
