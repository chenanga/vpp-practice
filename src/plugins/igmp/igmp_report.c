...
static void igmp_handle_group_exclude(igmp_config_t *config, const igmp_membership_group_v3_t *igmp_group)
{
    ...
    if (0 == n) {
        ...
        IGMP_DBG(" ..group-update: %U (*, %U)", format_vnet_sw_if_index_name, vnet_get_main(), config->sw_if_index, format_igmp_key, &key);
...
static void igmp_handle_group_block(igmp_config_t *config, const igmp_membership_group_v3_t *igmp_group)
{
    ...
    if (group) {
        ...
        vec_foreach (s, srcs) {
            ...
            if (NULL != src) igmp_src_blocked(src);
        }
    }
    /*
...
{
    IGMP_DBG("rx-group-report: %U", format_vnet_sw_if_index_name, vnet_get_main(), config->sw_if_index);
...
void igmp_handle_report(const igmp_report_args_t *args)
{
    ...
    for (ii = 0; ii < n_groups; ii++) {
        ...
        igmp_group = group_cptr(igmp_group, igmp_membership_group_v3_length(igmp_group));
    }
