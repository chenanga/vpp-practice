...
static void igmp_src_exp(u32 obj, void *dat)
{
    ...
    if (IGMP_MODE_ROUTER == src->mode) {
        ...
        igmp_event(IGMP_FILTER_MODE_EXCLUDE, config->sw_if_index, src->key, group->key);
    ...
    if (0 == igmp_group_n_srcs(group, IGMP_FILTER_MODE_INCLUDE)) igmp_group_clear(&group);
}
...
igmp_src_t *igmp_src_alloc(u32 group_index, const igmp_key_t *skey, igmp_mode_t mode)
{
    ...
    if (IGMP_MODE_ROUTER == mode) {
         ...
         * active on the link
         */
        src->timers[IGMP_SRC_TIMER_EXP] = igmp_timer_schedule(igmp_timer_type_get(IGMP_TIMER_SRC), src - im->srcs, igmp_src_exp, NULL);
        ...
        igmp_event(IGMP_FILTER_MODE_INCLUDE, config->sw_if_index, src->key, group->key);
    }
    else {
...
void igmp_src_refresh(igmp_src_t *src)
{
    ...
    src->timers[IGMP_SRC_TIMER_EXP] = igmp_timer_schedule(igmp_timer_type_get(IGMP_TIMER_SRC), igmp_src_index(src), igmp_src_exp, NULL);
}
...
void igmp_src_blocked(igmp_src_t *src)
{
    ...
    src->timers[IGMP_SRC_TIMER_EXP] = igmp_timer_schedule(igmp_timer_type_get(IGMP_TIMER_LEAVE), igmp_src_index(src), igmp_src_exp, NULL);
}
...
u8 *format_igmp_src(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U%U %U", format_white_space, indent, format_igmp_key, src->key, format_igmp_timer_id, src->timers[IGMP_SRC_TIMER_EXP]);
