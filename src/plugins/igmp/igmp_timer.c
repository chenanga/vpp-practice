...
{
    if (0 == vec_len(pending_timers)) return (IGMP_TIMER_ID_INVALID);
...
static uword igmp_timer_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    while (1) {
        ...
        if (IGMP_TIMER_ID_INVALID != tid) {
            timer = pool_elt_at_index(timer_pool, tid);
            vlib_process_wait_for_event_or_clock(vm, timer->exp_time - vlib_time_now(vm));
        }
        else
        ...
        if (event_type == IGMP_PROCESS_EVENT_UPDATE_TIMER) goto next_timer;
...
igmp_timer_id_t igmp_timer_schedule(f64 when, u32 obj, igmp_timer_function_t fn, void *data)
{
    ...
    vlib_process_signal_event(vm, igmp_timer_process_node.index, IGMP_PROCESS_EVENT_UPDATE_TIMER, 0);
...
{
    if (IGMP_TIMER_ID_INVALID == *tid) return;
    vec_del1(pending_timers, vec_search(pending_timers, *tid));
    pool_put_index(timer_pool, *tid);
    *tid = IGMP_TIMER_ID_INVALID;

    vlib_process_signal_event(vlib_get_main(), igmp_timer_process_node.index, IGMP_PROCESS_EVENT_UPDATE_TIMER, 0);
}
...
u8 *format_igmp_timer_id(u8 *s, va_list *args)
{
    ...
    if (IGMP_TIMER_ID_INVALID == tid) {
        ...
        s = format(s, "[expires-in:%f]", timer->exp_time - vlib_time_now(vlib_get_main()));
    }
