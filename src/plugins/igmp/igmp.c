...
static void igmp_send_general_query(u32 obj, void *dat)
{
    ...
    IGMP_DBG("send-general-query: %U", format_vnet_sw_if_index_name, vnet_get_main(), config->sw_if_index);
    ...
    config->timers[IGMP_CONFIG_TIMER_GENERAL_QUERY] =
        igmp_timer_schedule(igmp_timer_type_get(IGMP_TIMER_QUERY), igmp_config_index(config), igmp_send_general_query, NULL);
}
...
static void igmp_send_state_change_group_report_v3(u32 sw_if_index, const igmp_group_t *group)
{
    ...
    igmp_pkt_report_v3_add_group(&br, group, IGMP_MEMBERSHIP_GROUP_allow_new_sources);
...
static void igmp_resend_state_change_group_report_v3(u32 gi, void *data)
{
    ...
    if (++group->n_reports_sent < config->robustness_var) {
        group->timers[IGMP_GROUP_TIMER_RESEND_REPORT] = igmp_timer_schedule(igmp_timer_type_get(IGMP_TIMER_REPORT_INTERVAL),
                                                                            ...
                                                                            igmp_resend_state_change_group_report_v3,
                                                                            NULL);
    }
}
...
int igmp_listen(vlib_main_t *vm, igmp_filter_mode_t mode, u32 sw_if_index, const ip46_address_t *saddrs, const ip46_address_t *gaddr)
{
    ...
    if (!group) {
        ...
        if (IGMP_FILTER_MODE_INCLUDE == mode) {
            ...
            if (!(vec_len(added) || vec_len(removed))) /* no change => done */
                goto error;

            igmp_pkt_build_report_init(&br, config->sw_if_index);

            if (vec_len(added)) {
                igmp_pkt_report_v3_add_report(&br, group->key, added, IGMP_MEMBERSHIP_GROUP_allow_new_sources);
            }

            if (vec_len(removed)) {
                igmp_pkt_report_v3_add_report(&br, group->key, removed, IGMP_MEMBERSHIP_GROUP_block_old_sources);
            }
            ...
            if (0 == igmp_group_n_srcs(group, mode)) igmp_group_clear(&group);
...
static walk_rc_t igmp_sw_if_down(vnet_main_t *vnm, u32 sw_if_index, void *ctx)
{
    igmp_config_t *config;
    config = igmp_config_lookup(sw_if_index);
    IGMP_DBG("down: %U", format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index);
    if (NULL != config) {
...
static clib_error_t *igmp_hw_interface_link_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    ...
    if (!(flags & VNET_HW_INTERFACE_FLAG_LINK_UP)) vnet_hw_interface_walk_sw(vnm, hw_if_index, igmp_sw_if_down, NULL);
    return error;
}
...
int igmp_enable_disable(u32 sw_if_index, u8 enable, igmp_mode_t mode)
{
    ...
    u32            mfib_index;
    IGMP_DBG("%s:  %U", (enable ? "Enabled" : "Disabled"), format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index);
    ...
    if (!config && enable) {
        ...
        vec_validate_init_empty(im->igmp_config_by_sw_if_index, sw_if_index, ~0);
        pool_get(im->configs, config);
        clib_memset(config, 0, sizeof(igmp_config_t));
        config->sw_if_index       = sw_if_index;
        config->igmp_group_by_key = hash_create_mem(0, sizeof(igmp_key_t), sizeof(uword));
        ...
        config->adj_index                                   = adj_mcast_add_or_lock(FIB_PROTOCOL_IP4, VNET_LINK_IP4, config->sw_if_index);
        im->igmp_config_by_sw_if_index[config->sw_if_index] = (config - im->configs);
        {
            ...
            if (1 == im->n_configs_per_mfib_index[mfib_index]) {
                ...
                mfib_table_entry_path_update(mfib_index, &mpfx_general_query, MFIB_SOURCE_IGMP, MFIB_ENTRY_FLAG_NONE, &for_us_path);
                mfib_table_entry_path_update(mfib_index, &mpfx_report, MFIB_SOURCE_IGMP, MFIB_ENTRY_FLAG_NONE, &for_us_path);
            }
            mfib_table_entry_path_update(mfib_index, &mpfx_general_query, MFIB_SOURCE_IGMP, MFIB_ENTRY_FLAG_NONE, &via_itf_path);
            mfib_table_entry_path_update(mfib_index, &mpfx_report, MFIB_SOURCE_IGMP, MFIB_ENTRY_FLAG_NONE, &via_itf_path);
        }
    }
        ...
        if (0 == im->n_configs_per_mfib_index[mfib_index]) {
            ...
            mfib_table_entry_path_remove(mfib_index, &mpfx_general_query, MFIB_SOURCE_IGMP, &for_us_path);
            mfib_table_entry_path_remove(mfib_index, &mpfx_report, MFIB_SOURCE_IGMP, &for_us_path);
            mfib_table_unlock(mfib_index, FIB_PROTOCOL_IP4, MFIB_SOURCE_IGMP);
        }

        mfib_table_entry_path_remove(mfib_index, &mpfx_general_query, MFIB_SOURCE_IGMP, &via_itf_path);
        mfib_table_entry_path_remove(mfib_index, &mpfx_report, MFIB_SOURCE_IGMP, &via_itf_path);
        ...
        if (config->mode == IGMP_MODE_ROUTER)
            igmp_proxy_device_add_del_interface(config->proxy_device_id, config->sw_if_index, 0);
        else if (config->mode == IGMP_MODE_HOST)
            igmp_proxy_device_add_del(config->proxy_device_id, config->sw_if_index, 0);
