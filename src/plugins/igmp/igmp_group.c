...
void igmp_group_free_all_srcs(igmp_group_t *group)
{
    ...
    FOR_EACH_SRC(src, group, IGMP_FILTER_MODE_INCLUDE, ({ igmp_src_free(src); }));
...
igmp_src_t *igmp_group_src_update(igmp_group_t *group, const igmp_key_t *skey, igmp_mode_t mode)
{
    ...
    if (NULL == src) {
        ...
        hash_set_mem(group->igmp_src_by_key[IGMP_FILTER_MODE_INCLUDE], src->key, igmp_src_index(src));
    }
    else {
...
void igmp_group_clear(igmp_group_t **group)
{
    ...
    IGMP_DBG("clear-group: %U %U", format_igmp_key, (*group)->key, format_vnet_sw_if_index_name, vnet_get_main(), config->sw_if_index);
...
igmp_group_t *igmp_group_alloc(igmp_config_t *config, const igmp_key_t *gkey, igmp_filter_mode_t mode)
{
    ...
    group->igmp_src_by_key[IGMP_FILTER_MODE_INCLUDE] = hash_create_mem(0, sizeof(igmp_key_t), sizeof(uword));
    group->igmp_src_by_key[IGMP_FILTER_MODE_EXCLUDE] = hash_create_mem(0, sizeof(igmp_key_t), sizeof(uword));
...
igmp_src_t *igmp_src_lookup(igmp_group_t *group, const igmp_key_t *key)
{
    ...
    if (p) src = vec_elt_at_index(igmp_main.srcs, p[0]);
...
u8 *format_igmp_group(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U%U", format_white_space, indent, format_igmp_key, group->key);

    for (ii = 0; ii < IGMP_GROUP_N_TIMERS; ii++)
        s = format(s, "\n%U  %U:%U", format_white_space, indent, format_igmp_group_timer_type, ii, format_igmp_timer_id, group->timers[ii]);

    FOR_EACH_SRC(src, group, IGMP_FILTER_MODE_INCLUDE, ({ s = format(s, "\n%U", format_igmp_src, src, indent + 4); }));
