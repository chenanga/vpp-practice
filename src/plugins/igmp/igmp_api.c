...
static void vl_api_igmp_listen_t_handler(vl_api_igmp_listen_t *mp)
{
    ...
    if ((vnet_sw_interface_get_flags(vnm, ntohl(mp->group.sw_if_index)) && VNET_SW_INTERFACE_FLAG_ADMIN_UP) == 0) {
    ...
    vec_foreach_index (ii, saddrs) {
        clib_memcpy(&saddrs[ii].ip4, &mp->group.saddrs[ii], sizeof(ip4_address_t));
    }

    rv = igmp_listen(vm, (mp->group.filter ? IGMP_FILTER_MODE_INCLUDE : IGMP_FILTER_MODE_EXCLUDE), ntohl(mp->group.sw_if_index), saddrs, &gaddr);
...
static void vl_api_igmp_enable_disable_t_handler(vl_api_igmp_enable_disable_t *mp)
{
    ...
    rv = igmp_enable_disable(ntohl(mp->sw_if_index), mp->enable, (mp->mode ? IGMP_MODE_HOST : IGMP_MODE_ROUTER));
...
static void vl_api_igmp_proxy_device_add_del_t_handler(vl_api_igmp_proxy_device_add_del_t *mp)
{
    ...
    rv = igmp_proxy_device_add_del(ntohl(mp->vrf_id), ntohl(mp->sw_if_index), mp->add);
...
static void vl_api_igmp_proxy_device_add_del_interface_t_handler(vl_api_igmp_proxy_device_add_del_interface_t *mp)
{
    ...
    rv = igmp_proxy_device_add_del_interface(ntohl(mp->vrf_id), ntohl(mp->sw_if_index), mp->add);
...
static void igmp_config_dump(igmp_main_t *im, vl_api_registration_t *rp, u32 context, igmp_config_t *config)
{
    ...
    FOR_EACH_GROUP(group, config, ({
                       FOR_EACH_SRC(src, group, IGMP_FILTER_MODE_INCLUDE, ({ send_igmp_details(rp, im, config, group, src, context); }));
                   }));
}
...
static vpe_client_registration_t *igmp_api_client_lookup(igmp_main_t *im, u32 client_index)
{
    ...
    if (p) api_client = vec_elt_at_index(im->api_clients, p[0]);
...
static void vl_api_want_igmp_events_t_handler(vl_api_want_igmp_events_t *mp)
{
    ...
    if (api_client) {
        ...
        hash_unset(im->igmp_api_client_by_client_index, api_client->client_index);
    ...
    if (mp->enable) {
        ...
        hash_set(im->igmp_api_client_by_client_index, mp->client_index, api_client - im->api_clients);
        goto done;
    }
    rv = VNET_API_ERROR_INVALID_REGISTRATION;
...
void igmp_event(igmp_filter_mode_t filter, u32 sw_if_index, const ip46_address_t *saddr, const ip46_address_t *gaddr)
{
    ...
    pool_foreach (api_client, im->api_clients) {
        ...
        if (rp) send_igmp_event(rp, filter, sw_if_index, saddr, gaddr);
    }
}
