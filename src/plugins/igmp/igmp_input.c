...
static u8 *format_igmp_input_trace(u8 *s, va_list *va)
{
    ...
    s = format(s, "sw_if_index %u next-index %u", t->sw_if_index, t->next_index);
    s = format(s, "\n%U", format_igmp_header, t->packet_data, sizeof(t->packet_data));
    return s;
}
...
static u8 *format_igmp_parse_report_trace(u8 *s, va_list *va)
{
    ...
    s = format(s, "sw_if_index %u next-index %u", t->sw_if_index, t->next_index);
    s = format(s, "\n%U", format_igmp_report_v3, t->packet_data, sizeof(t->packet_data));
    return s;
}
...
static u8 *format_igmp_parse_query_trace(u8 *s, va_list *va)
{
    ...
    s = format(s, "sw_if_index %u next-input %u len %u", t->sw_if_index, t->next_index, t->len);
    s = format(s, "\n%U", format_igmp_query_v3, t->packet_data, sizeof(t->packet_data));
...
static uword igmp_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            igmp->checksum = 0;
            sum            = ip_incremental_checksum(0, igmp, clib_net_to_host_u16(ip->length) - ip4_header_bytes(ip));
        ...
        next_buffer:
            ...
            if (node->flags & VLIB_NODE_FLAG_TRACE) {
                ...
                clib_memcpy_fast(tr->packet_data, vlib_buffer_get_current(b), sizeof(tr->packet_data));
            }
...
VLIB_REGISTER_NODE(igmp_input_node) = {.function    = igmp_input,
                                       .name        = "igmp-input",
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (vlib_buffer_length_in_chain(vm, b) >= len) {
                ...
                vl_api_rpc_call_main_thread(igmp_handle_query, (u8 *) args, sizeof(*args) + len);
            }
            else {
                /*
...
VLIB_REGISTER_NODE(igmp_parse_query_node) = {.function    = igmp_parse_query,
...
static uword igmp_parse_report(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    vlib_node_runtime_t *error_node = vlib_node_get_runtime(vm, igmp_input_node.index);
    u8                   error;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (vlib_buffer_length_in_chain(vm, b) >= len) {
                ...
                vl_api_rpc_call_main_thread(igmp_handle_report, (u8 *) args, sizeof(*args) + len);
            }
            else {
                /*
...
VLIB_REGISTER_NODE(igmp_parse_report_node) = {.function    = igmp_parse_report,
