...
u8 *format_igmp_group_prefix_type(u8 *s, va_list *args)
{
    ...
    switch (type) {
#define _(n, f)                      \
    case IGMP_GROUP_PREFIX_TYPE_##f: \
        return (format(s, "%s", #f));
...
void igmp_group_prefix_set(const fib_prefix_t *pfx, igmp_group_prefix_type_t type)
{
    ...
    pos = vec_search_with_function(igmp_group_prefixs, pfx, igmp_group_prefix_cmp);
...
igmp_group_prefix_type_t igmp_group_prefix_get_type(const ip46_address_t *gaddr)
{
    ...
    vec_foreach (igp, igmp_group_prefixs) {
        if (ip4_destination_matches_route(&ip4_main, &gaddr->ip4, &igp->igp_prefix.fp_addr.ip4, igp->igp_prefix.fp_len))
...
void igmp_ssm_range_walk(igmp_ssm_range_walk_t fn, void *ctx)
{
    ...
    vec_foreach (igp, igmp_group_prefixs) {
        if (WALK_STOP == fn(&igp->igp_prefix, igp->igp_type, ctx)) break;
    }
}
...
static clib_error_t *igmp_ssm_range_show(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vec_foreach (igp, igmp_group_prefixs) {
        vlib_cli_output(vm, "%U => %U", format_fib_prefix, &igp->igp_prefix, format_igmp_group_prefix_type, igp->igp_type);
    }
    return (NULL);
}
