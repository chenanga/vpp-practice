...
igmp_config_t *igmp_config_lookup(u32 sw_if_index)
{
    ...
    if (vec_len(im->igmp_config_by_sw_if_index) > sw_if_index) {
        ...
        if (~0 != index) return (vec_elt_at_index(im->configs, index));
    }
    return NULL;
}
...
igmp_group_t *igmp_group_lookup(igmp_config_t *config, const igmp_key_t *key)
{
    ...
    if (p) group = pool_elt_at_index(igmp_main.groups, p[0]);
...
u8 *format_igmp_config(u8 *s, va_list *args)
{
    ...
    s = format(s,
               ...
               config->proxy_device_id);

    for (ii = 0; ii < IGMP_CONFIG_N_TIMERS; ii++) {
        s = format(s, "\n  %U:%U", format_igmp_config_timer_type, ii, format_igmp_timer_id, config->timers[ii]);
    }

    FOR_EACH_GROUP(group, config, ({ s = format(s, "\n%U", format_igmp_group, group, 4); }));
