...
static_always_inline void ena_txq_adv_sq_tail(ena_tx_ctx_t *ctx, ena_txq_t *eq)
{
...
advance_sq_tail:
    ...
    for (; n >= 8; n -= 8, bi += 8)
        if (!u32x8_is_all_equal(*(u32x8u *) bi, VLIB_BUFFER_INVALID_INDEX)) break;
#elif defined(CLIB_HAVE_VEC128)
    for (; n >= 4; n -= 4, bi += 4)
        if (!u32x4_is_all_equal(*(u32x4u *) bi, VLIB_BUFFER_INVALID_INDEX)) break;
#endif

    for (; n > 0; n -= 1, bi += 1)
        if (bi[0] != VLIB_BUFFER_INVALID_INDEX) break;
...
static_always_inline void ena_txq_deq(vlib_main_t *vm, ena_tx_ctx_t *ctx, ena_txq_t *txq)
{
    ...
    const ena_tx_cdesc_t mask_reqid = {.req_id = reqid_sqe_idx_mask}, match_ph0_nic1 = {.req_id = reqid_nic1, .phase = 0},
                         match_ph1_nic1 = {.req_id = reqid_nic1, .phase = 1}, mask_ph_nic = {.req_id = ~reqid_sqe_idx_mask, .phase = 1};
    ...
    u32x8 match_ph_nic1_x8 = u32x8_splat(match_phase.phase ? match_ph1_nic1.as_u64 : match_ph0_nic1.as_u64);
...
more:
    while (n < n_to_check) {
        ...
        while (n + 7 < n_to_check) {
            ...
            r = u32x8_shuffle2(*(u32x8u *) cd, *(u32x8u *) (cd + 4), 0, 2, 4, 6, 8, 10, 12, 14);

            /* check if all 8 CQEs are completed and there is no chained bufs */
            if (u32x8_is_equal(r & mask_ph_nic_x8, match_ph_nic1_x8) == 0) goto one_by_one;

            r &= mask_reqid_x8;

            /* take consumed buffer indices from ring */
            v = u32x8_gather_u32(ctx->sq_buffer_indices, r, sizeof(ctx->sq_buffer_indices[0]));
            u32x8_scatter_u32(ctx->sq_buffer_indices, r, buf_inv_idx_x8, sizeof(ctx->sq_buffer_indices[0]));
    ...
    one_by_one:
        ...
        if ((cd->as_u64 & mask_phase.as_u64) != match_phase.as_u64) goto done;
...
static_always_inline u16 ena_txq_wr_sqe(vlib_main_t *vm, vlib_buffer_t *b, int use_iova, ena_tx_desc_t *dp, u32 n_in_chain, ena_tx_desc_t desc)
{
    uword dma_addr = use_iova ? vlib_buffer_get_current_va(b) : vlib_buffer_get_current_pa(vm, b);
...
static_always_inline void ena_txq_copy_sqes(ena_tx_ctx_t *ctx, u32 off, ena_tx_desc_t *s, u32 n_desc)
{
    ...
    if (ctx->llq) {
        ...
        for (; n < n_desc; n += 1, s += 1, off += 1) {
            ...
            vlib_buffer_t *b = vlib_get_buffer(vlib_get_main(), ctx->sq_buffer_indices[off]);
...
ena_txq_enq_one(vlib_main_t *vm, ena_tx_ctx_t *ctx, vlib_buffer_t *b0, ena_tx_desc_t *d, u16 n_free_desc, u32 *f, int use_iova)
{
    ...
    if (n > n_free_desc) return 0;

    /* first */
    f++[0] = ctx->from[0];
    ctx->from += 1;
    ctx->n_packets_left -= 1;
    ctx->n_bytes += ena_txq_wr_sqe(vm, b0, use_iova, d++, n, (ena_tx_desc_t){.first = 1});

    /* mid */
    for (i = 1, b = b0; i < n - 1; i++) {
        f++[0] = b->next_buffer;
        b      = vlib_get_buffer(vm, b->next_buffer);
        ctx->n_bytes += ena_txq_wr_sqe(vm, b, use_iova, d++, 0, (ena_tx_desc_t){});
    }

    /* last */
    f[0] = b->next_buffer;
    b    = vlib_get_buffer(vm, b->next_buffer);
    ctx->n_bytes += ena_txq_wr_sqe(vm, b, use_iova, d, 0, (ena_tx_desc_t){.last = 1});

    return n;
}

static_always_inline uword ena_txq_enq(vlib_main_t *vm, ena_tx_ctx_t *ctx, ena_txq_t *txq, int use_iova)
{
    ...
    if (ctx->n_packets_left == 0) return 0;
    ...
    while (n_desc_left >= 4 && ctx->n_packets_left >= 8) {
        ...
        if (PREDICT_FALSE(((b0->flags | b1->flags | b2->flags | b3->flags) & VLIB_BUFFER_NEXT_PRESENT) == 0)) {
            ctx->n_bytes += ena_txq_wr_sqe(vm, b0, use_iova, d++, 1, single);
    ...
    if (n) {
        ...
        if (n_before_wrap >= n) {
            n_copy = n;
            vlib_buffer_copy_indices(ctx->sq_buffer_indices + offset, f, n_copy);
            ...
            vlib_buffer_copy_indices(ctx->sq_buffer_indices, f + n_before_wrap, n_copy);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (eq->sq_head != eq->sq_tail) ena_txq_deq(vm, &ctx, eq);
        ...
        if (ctx.n_packets_left == 0) break;
    }

    vnet_dev_tx_queue_unlock_if_needed(txq);

    if (ctx.n_dropped_chain_too_long) vlib_error_count(vm, node->node_index, ENA_TX_NODE_CTR_CHAIN_TOO_LONG, ctx.n_dropped_chain_too_long);

    n_pkts = frame->n_vectors - ctx.n_packets_left;
    vlib_increment_combined_counter(vnet_get_main()->interface_main.combined_sw_if_counters + VNET_INTERFACE_COUNTER_TX,
                                    vm->thread_index,
                                    tnr->hw_if_index,
                                    n_pkts,
                                    ctx.n_bytes);
    ...
    if (ctx.n_packets_left) {
        ...
        vlib_error_count(vm, node->node_index, ENA_TX_NODE_CTR_NO_FREE_SLOTS, ctx.n_packets_left);
    }
