...
u8 *format_ena_dev_info(u8 *s, va_list *args)
{
    ...
    format(s, "\n%UDevice version is %u, implementation id is %u", format_white_space, indent, ed->dev_attr.device_version, ed->dev_attr.impl_id);
    format(s, "\n%Urx drops %lu, tx drops %lu", format_white_space, indent, ed->aenq.rx_drops, ed->aenq.tx_drops);
    format(s, "\n%ULast keepalive arrived ", format_white_space, indent);
    if (ed->aenq.last_keepalive != 0.0)
        format(s, "%.2f seconds ago", vlib_time_now(vm) - ed->aenq.last_keepalive);
    else
        format(s, "never");
    return s;
}

u8 *format_ena_mem_addr(u8 *s, va_list *args)
{
    ena_mem_addr_t *ema = va_arg(*args, ena_mem_addr_t *);
    return format(s, "0x%lx", (u64) ema->addr_hi << 32 | ema->addr_lo);
}

u8 *format_ena_tx_desc(u8 *s, va_list *args)
{
    ena_tx_desc_t *d = va_arg(*args, ena_tx_desc_t *);
    s                = format(s, "addr 0x%012lx", (u64) d->buff_addr_hi << 32 | d->buff_addr_lo);
    s                = format(s, " len %u", d->length);
    s                = format(s, " req_id 0x%x", d->req_id_lo | d->req_id_hi << 10);
    if (d->header_length) s = format(s, " hdr_len %u", d->header_length);
#define _(v, n) \
    if ((v) < 6 && #n[0] != '_' && d->n) s = format(s, " " #n " %u", d->n);
    foreach_ena_tx_desc
#undef _
        return s;
}

u8 *format_ena_rx_desc_status(u8 *s, va_list *args)
{
    ...
    if (st.as_u32 != 0) {
        ...
        s                  = format(s, " -> ");
#define _(b, n) \
    if (st.n) s = format(s, "%s%s %u", not_first_line++ ? ", " : "", #n, st.n);
...
u8 *format_ena_rx_trace(u8 *s, va_list *args)
{
    ...
    s = format(s,
               "ena: %v (%d) qid %u next-node %U length %u req-id 0x%x n-desc %u",
               hi->name,
               t->hw_if_index,
               t->qid,
               ...
               t->req_id,
               t->n_desc);
    s = format(s, "\n%Ustatus: %U", format_white_space, indent + 2, format_ena_rx_desc_status, t->status);
    return s;
}

u8 *format_ena_regs(u8 *s, va_list *args)
{
    vnet_dev_t *dev    = va_arg(*args, vnet_dev_t *);
