...
#define __name(s, n)                                                                               \
    {                                                                                              \
        s = format(s, "%s%U%-32s: ", line ? "\n" : "", format_white_space, line ? indent : 0, #n); \
        line++;                                                                                    \
    }
...
#define _format_ena_memory(s, c, n) _format_with_fn_and_ptr(s, c, n, format_ena_mem_addr)

u8 *format_ena_aq_opcode(u8 *s, va_list *args)
{
    u32 opcode = va_arg(*args, u32);

    if (opcode >= ARRAY_LEN(opcode_names) || opcode_names[opcode] == 0) return format(s, "UNKNOWN(%u)", opcode);
    return format(s, "%s", opcode_names[opcode]);
}

u8 *format_ena_aq_status(u8 *s, va_list *args)
{
    u32 status = va_arg(*args, u32);

    if (status >= ARRAY_LEN(status_names) || status_names[status] == 0) return format(s, "UNKNOWN(%u)", status);
    return format(s, "%s", status_names[status]);
}

u8 *format_ena_aq_aenq_groups(u8 *s, va_list *args)
{
...
#define _(x)                                                                              \
    if (g.x) {                                                                            \
        if (format_get_indent(s) > 80) s = format(s, "\n%U", format_white_space, indent); \
...
u8 *format_ena_aq_feat_id_bitmap(u8 *s, va_list *args)
{
...
u8 *format_ena_aq_feat_name(u8 *s, va_list *args)
{
...
#undef _
    ...
    if (feat_id >= ARRAY_LEN(feat_names) || feat_names[feat_id] == 0) return format(s, "UNKNOWN(%u)", feat_id);
    return format(s, "%s", feat_names[feat_id]);
}

u8 *format_ena_aq_feat_desc(u8 *s, va_list *args)
{
    ena_aq_feature_id_t feat_id = va_arg(*args, int);
    ...
    switch (feat_id) {
        case ENA_ADMIN_FEAT_ID_DEVICE_ATTRIBUTES: {
            ...
            _format_with_fn_and_val(s, d, supported_features, format_ena_aq_feat_id_bitmap);
        } break;

        case ENA_ADMIN_FEAT_ID_AENQ_CONFIG: {
            ena_aq_feat_aenq_config_t *d = data;
            _format_with_fn_and_val(s, d, supported_groups, format_ena_aq_aenq_groups);
            _format_with_fn_and_val(s, d, enabled_groups, format_ena_aq_aenq_groups);
        } break;
        ...
        default:
            if (info) s = format(s, "%U", format_hexdump, data, info->data_sz);
            break;
    }
...
u8 *format_ena_aq_create_sq_cmd(u8 *s, va_list *args)
{
...
u8 *format_ena_aq_create_cq_cmd(u8 *s, va_list *args)
{
...
u8 *format_ena_aq_create_sq_resp(u8 *s, va_list *args)
{
...
u8 *format_ena_aq_create_cq_resp(u8 *s, va_list *args)
{
...
u8 *format_ena_aq_destroy_sq_cmd(u8 *s, va_list *args)
{
...
u8 *format_ena_aq_destroy_cq_cmd(u8 *s, va_list *args)
{
...
u8 *format_ena_aq_basic_stats(u8 *s, va_list *args)
{
...
u8 *format_ena_aq_eni_stats(u8 *s, va_list *args)
{
