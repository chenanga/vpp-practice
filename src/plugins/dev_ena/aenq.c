...
void ena_aenq_free(vlib_main_t *vm, vnet_dev_t *dev)
{
...
vnet_dev_rv_t ena_aenq_olloc(vlib_main_t *vm, vnet_dev_t *dev, u16 depth)
{
    ena_device_t *ed       = vnet_dev_get_data(dev);
    ...
    if ((rv = vnet_dev_dma_mem_alloc(vm, dev, alloc_sz, 0, (void **) &ed->aenq.entries))) goto err;
...
static ena_aenq_entry_t *ena_get_next_aenq_entry(vnet_dev_t *dev)
{
    ena_device_t     *ed    = vnet_dev_get_data(dev);
...
static void ena_aenq_poll(vlib_main_t *vm, vnet_dev_t *dev)
{
    ena_aenq_entry_t *ae;
    ...
    while ((ae = ena_get_next_aenq_entry(dev))) {
        ...
        log_debug(dev, "aenq: group %u syndrome %u phase %u timestamp %lu", ae->group, ae->syndrome, ae->phase, ae->timestamp);

        switch (ae->group) {
            case ENA_AENQ_GROUP_LINK_CHANGE:
                log_debug(dev, "link_change: status %u", ae->link_change.link_status);
            ...
            case ENA_AENQ_GROUP_NOTIFICATION:
                log_warn(dev, "unhandled AENQ notification received [syndrome %u]", ae->syndrome);
                break;

            case ENA_AENQ_GROUP_KEEP_ALIVE:
                if (ae->keep_alive.rx_drops || ae->keep_alive.tx_drops)
                    log_debug(dev, "keep_alive: rx_drops %lu tx_drops %lu", ae->keep_alive.rx_drops, ae->keep_alive.tx_drops);
            ...
            default:
                log_debug(dev, "unknown aenq entry (group %u) %U", ae->group, format_hexdump, ae, sizeof(*ae));
        };
    }
}

vnet_dev_rv_t ena_aenq_start(vlib_main_t *vm, vnet_dev_t *dev)
{
    ena_device_t *ed       = vnet_dev_get_data(dev);
    ...
    if (ena_aq_feature_is_supported(dev, ENA_ADMIN_FEAT_ID_AENQ_CONFIG)) {
        ...
        if ((rv = ena_aq_get_feature(vm, dev, ENA_ADMIN_FEAT_ID_AENQ_CONFIG, &aenq))) {
        ...
        if ((rv = ena_aq_set_feature(vm, dev, ENA_ADMIN_FEAT_ID_AENQ_CONFIG, &aenq))) {
    ...
    ena_reg_set_dma_addr(vm, dev, ENA_REG_AENQ_BASE_LO, ENA_REG_AENQ_BASE_HI, ed->aenq.entries);
...
void ena_aenq_stop(vlib_main_t *vm, vnet_dev_t *dev)
{
    ena_device_t *ed = vnet_dev_get_data(dev);
