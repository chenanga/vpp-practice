...
static ena_aq_host_info_t host_info = {.os_type        = 3, /* DPDK */
...
static vlib_error_desc_t ena_rx_node_counters[] = {foreach_ena_rx_node_counter};
static vlib_error_desc_t ena_tx_node_counters[] = {foreach_ena_tx_node_counter};
#undef _
...
static void ena_deinit(vlib_main_t *vm, vnet_dev_t *dev)
{
    ena_aenq_stop(vm, dev);
    ena_aq_stop(vm, dev);
}

static vnet_dev_rv_t ena_alloc(vlib_main_t *vm, vnet_dev_t *dev)
{
    ena_device_t *ed = vnet_dev_get_data(dev);
    vnet_dev_rv_t rv;

    if ((rv = vnet_dev_dma_mem_alloc(vm, dev, 4096, 4096, (void **) &ed->host_info))) return rv;

    if ((rv = vnet_dev_dma_mem_alloc(vm, dev, sizeof(ena_mmio_resp_t), 0, (void **) &ed->mmio_resp))) return rv;

    if ((rv = ena_aq_olloc(vm, dev, ENA_ADMIN_QUEUE_DEPTH))) return rv;

    if ((rv = ena_aenq_olloc(vm, dev, ENA_ASYNC_QUEUE_DEPTH))) return rv;

    return VNET_DEV_OK;
}

static void ena_free(vlib_main_t *vm, vnet_dev_t *dev)
{
    ena_device_t *ed = vnet_dev_get_data(dev);
...
static vnet_dev_rv_t ena_init(vlib_main_t *vm, vnet_dev_t *dev)
{
    ena_device_t                  *ed        = vnet_dev_get_data(dev);
    ...
    if ((rv = vnet_dev_pci_read_config_header(vm, dev, &pci_cfg_hdr))) goto err;

    log_debug(dev, "revision_id 0x%x", pci_cfg_hdr.revision_id);

    ed->readless = (pci_cfg_hdr.revision_id & 1) == 0;

    if ((rv = vnet_dev_pci_map_region(vm, dev, 0, &ed->reg_bar))) goto err;

    if ((rv = ena_reg_reset(vm, dev, ENA_RESET_REASON_NORMAL))) goto err;

    if ((rv = ena_aq_start(vm, dev))) goto err;

    *ed->host_info          = host_info;
    ed->host_info->num_cpus = vlib_get_n_threads();
    ena_set_mem_addr(vm, dev, &host_attr.os_info_ba, ed->host_info);

    if ((rv = ena_aq_set_feature(vm, dev, ENA_ADMIN_FEAT_ID_HOST_ATTR_CONFIG, &host_attr))) return rv;

    if ((rv = ena_aq_get_feature(vm, dev, ENA_ADMIN_FEAT_ID_DEVICE_ATTRIBUTES, &ed->dev_attr))) return rv;

    if (ena_aq_feature_is_supported(dev, ENA_ADMIN_FEAT_ID_MAX_QUEUES_EXT)) {
        ena_aq_feat_max_queue_ext_t max_q_ext;
        if ((rv = ena_aq_get_feature(vm, dev, ENA_ADMIN_FEAT_ID_MAX_QUEUES_EXT, &max_q_ext))) goto err;
        port.port.attr.max_rx_queues  = clib_min(max_q_ext.max_rx_cq_num, max_q_ext.max_rx_sq_num);
        port.port.attr.max_tx_queues  = clib_min(max_q_ext.max_tx_cq_num, max_q_ext.max_tx_sq_num);
        port.rx_queue.config.max_size = clib_min(max_q_ext.max_rx_cq_depth, max_q_ext.max_rx_sq_depth);
        port.tx_queue.config.max_size = clib_min(max_q_ext.max_tx_cq_depth, max_q_ext.max_tx_sq_depth);
    }
    else {
    ...
    if ((rv = ena_aenq_start(vm, dev))) goto err;

    port.port.attr.max_supported_rx_frame_size = ed->dev_attr.max_mtu;

    if (ena_aq_feature_is_supported(dev, ENA_ADMIN_FEAT_ID_MTU)) port.port.attr.caps.change_max_rx_frame_size = 1;

    vnet_dev_set_hw_addr_eth_mac(&port.port.attr.hw_addr, ed->dev_attr.mac_addr);
...
static u8 *ena_probe(vlib_main_t *vm, vnet_dev_bus_index_t bus_index, void *dev_info)
{
    ...
    FOREACH_ARRAY_ELT (dt, ena_dev_types) {
        if (dt->device_id == di->device_id) return format(0, "%s", dt->description);
    }
