...
static_always_inline void ena_device_input_status_to_flags(ena_rx_cdesc_status_t *statuses,
                                                           u32                   *flags,
                                                           u32                    n_desc,
                                                           vlib_frame_bitmap_t    first_bmp,
                                                           int                    maybe_chained)
{
    const ena_rx_cdesc_status_t mask_first = {.first = 1}, match_first1 = {.first = 1};

    const ena_rx_cdesc_status_t mask_last = {.last = 1}, match_last0 = {.last = 0};

    const ena_rx_cdesc_status_t mask_l4_csum = {.ipv4_frag = 1, .l4_csum_checked = 1, .l4_csum_err = 1}, match_l4_csum_ok = {.l4_csum_checked = 1};
...
#define u32xN_splat u32x4_splat
    ...
    const u32xN f_l4_csum           = u32xN_splat(VNET_BUFFER_F_L4_CHECKSUM_CORRECT | VNET_BUFFER_F_L4_CHECKSUM_COMPUTED);
    ...
    while (n_left) {
        ...
        if ((st.as_u32 & mask_first.as_u32) == match_first1.as_u32) f |= VLIB_BUFFER_TOTAL_LENGTH_VALID;

        if ((st.as_u32 & mask_last.as_u32) == match_last0.as_u32) f |= VLIB_BUFFER_NEXT_PRESENT;

        if ((st.as_u32 & mask_l4_csum.as_u32) == match_l4_csum_ok.as_u32) f |= VNET_BUFFER_F_L4_CHECKSUM_COMPUTED | VNET_BUFFER_F_L4_CHECKSUM_CORRECT;
...
static_always_inline u16 ena_device_input_cq_dequeue_no_wrap(ena_rx_ctx_t *ctx, ena_rxq_t *q, ena_rx_cdesc_status_t *statuses, u16 *lengths, u16 *csi)
{
    u32                   next  = q->cq_next;
...
more:
    for (st = cd->status; i < n_to_check && st.phase != phase; i++, st = (++cd)->status) {
        ctx->st_or.as_u32 |= st.as_u32;
        ctx->st_and.as_u32 &= st.as_u32;
        statuses[i] = st;
        lengths[i]  = cd->length;
...
static_always_inline void ena_device_input_refill(vlib_main_t *vm, ena_rx_ctx_t *ctx, vnet_dev_rx_queue_t *rxq, int use_va)
{
    ...
    while (n_compl_sqes > 0) {
        n_alloc = vlib_buffer_alloc_from_pool(vm,
                                              buffer_indices,
                                              clib_min(ENA_RX_REFILL_BATCH, n_compl_sqes),
                                              vnet_dev_get_rx_queue_buffer_pool_index(rxq));

        if (PREDICT_FALSE(n_alloc == 0)) break;

        vlib_get_buffers_with_offset(vm, buffer_indices, (void **) dma_addr, ENA_RX_REFILL_BATCH, STRUCT_OFFSET_OF(vlib_buffer_t, data));
    ...
    if (n_compl_sqes == q->n_compl_sqes) return;

    q->sq_next += q->n_compl_sqes - n_compl_sqes;
    __atomic_store_n(q->sq_db, q->sq_next, __ATOMIC_RELEASE);

    if (PREDICT_FALSE(n_compl_sqes)) clib_memmove(ctx->comp_sqe_indices, csi, n_compl_sqes * sizeof(csi[0]));

    q->n_compl_sqes = n_compl_sqes;
}

static_always_inline uword ena_device_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vnet_dev_rx_queue_t *rxq)
{
    ena_rxq_t             *q    = vnet_dev_get_rx_queue_data(rxq);
    ...
    n_deq = ena_device_input_cq_dequeue_no_wrap(&ctx, q, statuses, lengths, csi);

    if (n_deq == 0) goto refill;

    q->n_compl_sqes += n_deq;

    maybe_chained = ctx.st_and.first && ctx.st_and.last ? 0 : 1;

    next_frame = vlib_get_next_frame_internal(vm, node, next_index, /* new frame */ 1);
    ...
    for (b = buffers, l = lengths, f = flags, n_left = n_deq; n_left >= 8; b += 4, f += 4, l += 4, n_left -= 4) {
    ...
    if (PREDICT_FALSE((n_trace = vlib_get_trace_count(vm, node)))) {
        u32 i;
        if (!maybe_chained) vlib_frame_bitmap_init(head_bmp, n_deq);
    ...
    if (PREDICT_FALSE(maybe_chained)) clib_compress_u32(bi, bi, head_bmp, n_deq);
    ...
    if (PREDICT_TRUE(next_index == VNET_DEVICE_INPUT_NEXT_ETHERNET_INPUT)) {
        ...
        if (ctx.st_or.l3_csum_err == 0) next_frame->flags |= ETH_INPUT_FRAME_F_IP4_CKSUM_OK;
