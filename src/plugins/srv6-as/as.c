...
unsigned char def_str[]       = "Endpoint with static proxy to SR-unaware appliance";
unsigned char params_str[]    = "nh <next-hop> oif <iface-out> iif <iface-in> src <src-addr> next <sid> [next <sid> ...]";

srv6_as_main_t srv6_as_main;

static inline u8 *prepare_rewrite(ip6_address_t src_addr, ip6_address_t *sid_list, u8 protocol)
{
    u8 *rewrite_str = NULL;
    ...
    if (num_sids > 1) {
        sr_hdr_len = sizeof(ip6_sr_header_t) + num_sids * sizeof(ip6_address_t);
...
static inline void free_ls_mem(srv6_as_localsid_t *ls_mem)
{
...
static int srv6_as_localsid_creation_fn(ip6_sr_localsid_t *localsid)
{
    ...
    if (ls_mem->inner_type != AS_TYPE_L2) {
        if (ls_mem->inner_type == AS_TYPE_IP4)
            nh_adj_index = adj_nbr_add_or_lock(FIB_PROTOCOL_IP4, VNET_LINK_IP4, &ls_mem->nh_addr, ls_mem->sw_if_index_out);
    ...
    if (pool_is_free_index(sm->vnet_main->interface_main.sw_interfaces, ls_mem->sw_if_index_in)) {
    ...
    vnet_sw_interface_t *sw = vnet_get_sw_interface(sm->vnet_main, ls_mem->sw_if_index_in);
    ...
    if (ls_mem->inner_type == AS_TYPE_L2) {
        ...
        int ret = vnet_feature_enable_disable("device-input", "srv6-as2-rewrite", ls_mem->sw_if_index_in, 1, 0, 0);
        if (ret != 0) {
            free_ls_mem(ls_mem);
        ...
        vnet_hw_interface_t *hi  = vnet_get_sup_hw_interface(vnm, ls_mem->sw_if_index_in);
        /* Make sure it is main interface */
        if (hi->sw_if_index == ls_mem->sw_if_index_in) ethernet_set_flags(vnm, hi->hw_if_index, ETHERNET_INTERFACE_FLAG_ACCEPT_ALL);

        /* Prepare rewrite string */
        ls_mem->rewrite = prepare_rewrite(ls_mem->src_addr, ls_mem->sid_list, IP_PROTOCOL_IP6_ETHERNET);
        ...
        if (ls_mem->sw_if_index_in >= vec_len(sm->sw_iface_localsid2)) {
            vec_resize(sm->sw_iface_localsid2, (pool_len(sm->vnet_main->interface_main.sw_interfaces) - vec_len(sm->sw_iface_localsid2)));
        }
        ...
        int ret = vnet_feature_enable_disable("ip4-unicast", "srv6-as4-rewrite", ls_mem->sw_if_index_in, 1, 0, 0);
        if (ret != 0) {
            adj_unlock(ls_mem->nh_adj);
            free_ls_mem(ls_mem);
            return SID_CREATE_IFACE_FEATURE_ERROR;
        ...
        /* Prepare rewrite string */
        ls_mem->rewrite = prepare_rewrite(ls_mem->src_addr, ls_mem->sid_list, IP_PROTOCOL_IP_IN_IP);

        /* Associate local SID index to this interface (resize vector if needed) */
        if (ls_mem->sw_if_index_in >= vec_len(sm->sw_iface_localsid4)) {
            vec_resize(sm->sw_iface_localsid4, (pool_len(sm->vnet_main->interface_main.sw_interfaces) - vec_len(sm->sw_iface_localsid4)));
        }
        sm->sw_iface_localsid4[ls_mem->sw_if_index_in] = localsid_index;
    }
    else if (ls_mem->inner_type == AS_TYPE_IP6) {
        /* Enable End.AS6 rewrite node for this interface */
        int ret = vnet_feature_enable_disable("ip6-unicast", "srv6-as6-rewrite", ls_mem->sw_if_index_in, 1, 0, 0);
        if (ret != 0) {
            adj_unlock(ls_mem->nh_adj);
            free_ls_mem(ls_mem);
            return SID_CREATE_IFACE_FEATURE_ERROR;
        ...
        /* Prepare rewrite string */
        ls_mem->rewrite = prepare_rewrite(ls_mem->src_addr, ls_mem->sid_list, IP_PROTOCOL_IPV6);

        /* Associate local SID index to this interface (resize vector if needed) */
        if (ls_mem->sw_if_index_in >= vec_len(sm->sw_iface_localsid6)) {
            vec_resize(sm->sw_iface_localsid6, (pool_len(sm->vnet_main->interface_main.sw_interfaces) - vec_len(sm->sw_iface_localsid6)));
        }
...
static int srv6_as_localsid_removal_fn(ip6_sr_localsid_t *localsid)
{
    ...
    if (ls_mem->inner_type == AS_TYPE_L2) {
        ...
        int ret;
        ret = vnet_feature_enable_disable("device-input", "srv6-as2-rewrite", ls_mem->sw_if_index_in, 0, 0, 0);
        if (ret != 0) return -1;
        ...
        vnet_hw_interface_t *hi  = vnet_get_sup_hw_interface(vnm, ls_mem->sw_if_index_in);
        /* Make sure it is main interface */
        if (hi->sw_if_index == ls_mem->sw_if_index_in) ethernet_set_flags(vnm, hi->hw_if_index, 0);
        ...
        int ret;
        ret = vnet_feature_enable_disable("ip4-unicast", "srv6-as4-rewrite", ls_mem->sw_if_index_in, 0, 0, 0);
        if (ret != 0) return -1;
        ...
        int ret;
        ret = vnet_feature_enable_disable("ip6-unicast", "srv6-as6-rewrite", ls_mem->sw_if_index_in, 0, 0, 0);
        if (ret != 0) return -1;
...
u8 *format_srv6_as_localsid(u8 *s, va_list *args)
{
    ...
    s = format(s, "\tSource address:\t%U\n", format_ip6_address, &ls_mem->src_addr);
    ...
    vlib_get_combined_counter(&(sm->invalid_counters), ls_mem->index, &invalid);
...
#define PARAM_AS_SRC (1 << 3)
    ...
    if (!unformat(input, "end.as")) return 0;

    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (!(params & PARAM_AS_NH) && unformat(input, "nh %U", unformat_ip4_address, &nh_addr.ip4)) {
            inner_type = AS_TYPE_IP4;
            params |= PARAM_AS_NH;
        }
        if (!(params & PARAM_AS_NH) && unformat(input, "nh %U", unformat_ip6_address, &nh_addr.ip6)) {
            inner_type = AS_TYPE_IP6;
            params |= PARAM_AS_NH;
        }
        else if (!(params & PARAM_AS_OIF) && unformat(input, "oif %U", unformat_vnet_sw_interface, vnm, &sw_if_index_out)) {
            params |= PARAM_AS_OIF;
        }
        else if (!(params & PARAM_AS_IIF) && unformat(input, "iif %U", unformat_vnet_sw_interface, vnm, &sw_if_index_in)) {
            params |= PARAM_AS_IIF;
        }
        else if (!(params & PARAM_AS_SRC) && unformat(input, "src %U", unformat_ip6_address, &src_addr)) {
            params |= PARAM_AS_SRC;
        }
