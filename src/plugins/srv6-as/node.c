...
static u8 *format_srv6_as_rewrite_trace(u8 *s, va_list *args)
{
    ...
    return format(s, "SRv6-AS-rewrite: src %U dst %U", format_ip6_address, &t->src, format_ip6_address, &t->dst);
}
...
static_always_inline void end_as_processing(vlib_buffer_t *b0, ip6_header_t *ip0, srv6_as_localsid_t *ls0_mem, u32 *next0)
{
    ...
    if (PREDICT_FALSE(hdr_type != IP_PROTOCOL_IPV6 && hdr_type != IP_PROTOCOL_IP_IN_IP && hdr_type != IP_PROTOCOL_IP6_ETHERNET)) {
        return;
    }
    ...
    if (hdr_type == IP_PROTOCOL_IP6_ETHERNET) {
        ...
        *next0 = (hdr_type == IP_PROTOCOL_IPV6) ? SRV6_AS_LOCALSID_NEXT_REWRITE6 : SRV6_AS_LOCALSID_NEXT_REWRITE4;
    }
}
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                srv6_as_localsid_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof *tr);
                tr->localsid_index           = ls0 - sm->localsids;
            }

            /* This increments the SRv6 per LocalSID counters. */
            vlib_increment_combined_counter(((next0 == SRV6_AS_LOCALSID_NEXT_ERROR) ? &(sm->sr_ls_invalid_counters) : &(sm->sr_ls_valid_counters)),
                                            vm->thread_index,
                                            ls0 - sm->localsids,
                                            1,
...
static uword srv6_as2_rewrite_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(ls0_mem == NULL || ls0_mem->rewrite == NULL)) {
                ...
                clib_memcpy_fast(((u8 *) en0) - vec_len(ls0_mem->rewrite), ls0_mem->rewrite, vec_len(ls0_mem->rewrite));
            ...
            if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE) && PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                srv6_as_rewrite_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof *tr);
                tr->error                   = 0;
            ...
            vlib_increment_combined_counter(((next0 == SRV6_AS_LOCALSID_NEXT_ERROR) ? &(sm->invalid_counters) : &(sm->valid_counters)),
                                            vm->thread_index,
                                            ls0_mem->index,
                                            1,
                                            vlib_buffer_length_in_chain(vm, b0));
    ...
    /* Update counters */
    vlib_node_increment_counter(vm, srv6_as4_rewrite_node.index, SRV6_AS_REWRITE_COUNTER_PROCESSED, cnt_packets);
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(ls0_mem == NULL || ls0_mem->rewrite == NULL)) {
                ...
                clib_memcpy_fast(((u8 *) ip0_encap) - vec_len(ls0_mem->rewrite), ls0_mem->rewrite, vec_len(ls0_mem->rewrite));
                vlib_buffer_advance(b0, -(word) vec_len(ls0_mem->rewrite));
...
static uword srv6_as6_rewrite_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(ls0_mem == NULL || ls0_mem->rewrite == NULL)) {
                ...
                /* Update outer IPv6 length (in case it has changed) */
                new_l0              = vec_len(ls0_mem->rewrite) + clib_net_to_host_u16(ip0_encap->payload_length);
                ip0->payload_length = clib_host_to_net_u16(new_l0);
            }
    ...
    /* Update counters */
    vlib_node_increment_counter(vm, srv6_as6_rewrite_node.index, SRV6_AS_REWRITE_COUNTER_PROCESSED, cnt_packets);
