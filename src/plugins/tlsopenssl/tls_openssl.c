...
static u32 openssl_ctx_alloc_w_thread(u32 thread_index)
{
    ...
    if (!(*ctx)) *ctx = clib_mem_alloc(sizeof(openssl_ctx_t));
...
static void openssl_ctx_free(tls_ctx_t *ctx)
{
    ...
    if (!(ctx->flags & TLS_CONN_F_MIGRATED)) {
        if (SSL_is_init_finished(oc->ssl) && !(ctx->flags & TLS_CONN_F_PASSIVE_CLOSE)) SSL_shutdown(oc->ssl);
    ...
    pool_put_index(openssl_main.ctx_pool[ctx->c_thread_index], oc->openssl_ctx_index);
}
...
{
    openssl_ctx_t **ctx;
    ctx = pool_elt_at_index(openssl_main.ctx_pool[vlib_get_thread_index()], ctx_index);
    return &(*ctx)->ctx;
}
...
#define ossl_check_err_is_fatal(_ssl, _rv) \
    if (PREDICT_FALSE(_rv < 0 && SSL_get_error(_ssl, _rv) == SSL_ERROR_SSL)) return -1;

static int openssl_read_from_ssl_into_fifo(svm_fifo_t *f, SSL *ssl, u32 max_len)
{
    int            read, rv, n_fs, i;
    const int      n_segs = 2;
...
static int openssl_write_from_fifo_into_ssl(svm_fifo_t *f, SSL *ssl, u32 max_len)
{
    ...
    if (wrote) svm_fifo_dequeue_drop(f, wrote);

    return wrote;
}

#ifdef HAVE_OPENSSL_ASYNC
static int openssl_check_async_status(tls_ctx_t *ctx, openssl_resume_handler *handler, session_t *session)
{
    openssl_ctx_t *oc = (openssl_ctx_t *) ctx;
    int            estatus;
...
int openssl_ctx_handshake_rx(tls_ctx_t *ctx, session_t *tls_session)
{
    ...
    while (SSL_in_init(oc->ssl)) {
        ...
        if (err == SSL_ERROR_WANT_ASYNC) {
            openssl_check_async_status(ctx, openssl_ctx_handshake_rx, tls_session);
        }
#endif
    ...
    TLS_DBG(2, "tls state for %u is %s", oc->openssl_ctx_index, SSL_state_string_long(oc->ssl));

    if (SSL_in_init(oc->ssl)) return -1;

    /* Renegotiated handshake, app must not be notified */
    if (PREDICT_FALSE(ctx->flags & TLS_CONN_F_HS_DONE)) return 0;
    ...
    TLS_DBG(1, "Handshake for %u complete. TLS cipher is %s", oc->openssl_ctx_index, SSL_get_cipher(oc->ssl));
    return rv;
}
...
static int openssl_ctx_write_tls(tls_ctx_t *ctx, session_t *app_session, transport_send_params_t *sp)
{
    ...
    if (!deq_max) goto check_tls_fifo;
    ...
    if (svm_fifo_provision_chunks(ts->tx_fifo, 0, 0, deq_max + TLSO_CTRL_BYTES)) goto check_tls_fifo;
    ...
    if (!wrote) goto check_tls_fifo;

    if (svm_fifo_needs_deq_ntf(f, wrote)) session_dequeue_notify(app_session);

check_tls_fifo:

    if (PREDICT_FALSE((ctx->flags & TLS_CONN_F_APP_CLOSED) && BIO_ctrl_pending(oc->rbio) <= 0)) openssl_confirm_app_close(ctx);
    ...
    if (space < wrote + enq_buf) {
        ...
        if (wrote) app_session->flags |= SESSION_F_CUSTOM_TX;
    }
...
static int openssl_ctx_write_dtls(tls_ctx_t *ctx, session_t *app_session, transport_send_params_t *sp)
{
    ...
    while (to_deq > 0) {
        ...
        if (svm_fifo_max_enqueue_prod(us->tx_fifo) < enq_max || svm_fifo_provision_chunks(us->tx_fifo, 0, 0, enq_max)) {
        ...
        rv = svm_fifo_peek(app_session->tx_fifo, SESSION_CONN_HDR_LEN, hdr.data_length, buf);
    ...
    if (svm_fifo_needs_deq_ntf(app_session->tx_fifo, read)) session_dequeue_notify(app_session);

    if (read) tls_add_vpp_q_tx_evt(us);

    if (PREDICT_FALSE((ctx->flags & TLS_CONN_F_APP_CLOSED) && !svm_fifo_max_enqueue_prod(us->rx_fifo))) openssl_confirm_app_close(ctx);

    return read;
}

static inline int openssl_ctx_write(tls_ctx_t *ctx, session_t *app_session, transport_send_params_t *sp)
{
...
static inline int openssl_ctx_read_tls(tls_ctx_t *ctx, session_t *tls_session)
{
    openssl_ctx_t *oc      = (openssl_ctx_t *) ctx;
    ...
    if (PREDICT_FALSE(SSL_in_init(oc->ssl))) {
        if (openssl_ctx_handshake_rx(ctx, tls_session) < 0) return 0;
...
static inline int openssl_ctx_read_dtls(tls_ctx_t *ctx, session_t *us)
{
    openssl_main_t     *om = &openssl_main;
    openssl_ctx_t      *oc = (openssl_ctx_t *) ctx;
    ...
    if (PREDICT_FALSE(SSL_in_init(oc->ssl))) {
        ...
        if (openssl_ctx_handshake_rx(ctx, us) < 0) return 0;
    ...
    while (svm_fifo_max_dequeue_cons(us->rx_fifo) > 0) {
        ...
        if (PREDICT_FALSE(read <= 0)) {
            if (read < 0) tls_add_vpp_q_builtin_rx_evt(us);
            goto done;
        }
        wrote += read;
        ...
        rv = svm_fifo_enqueue_segments(app_session->rx_fifo, segs, 2, 0 /* allow partial */);
        ASSERT(rv > 0);
    }
...
static inline int openssl_ctx_read(tls_ctx_t *ctx, session_t *ts)
{
    if (ctx->tls_type == TRANSPORT_PROTO_TLS)
...
static int openssl_set_ckpair(SSL *ssl, u32 ckpair_index)
{
...
static int openssl_client_init_verify(SSL *ssl, const char *srv_hostname, int set_hostname_verification, int set_hostname_strict_check)
{
    if (set_hostname_verification) {
        ...
        if (set_hostname_strict_check) X509_VERIFY_PARAM_set_hostflags(param, X509_CHECK_FLAG_NO_PARTIAL_WILDCARDS);
...
static int openssl_ctx_init_client(tls_ctx_t *ctx)
{
    ...
    method = ctx->tls_type == TRANSPORT_PROTO_TLS ? SSLv23_client_method() : DTLS_client_method();
    if (method == NULL) {
...
#ifdef HAVE_OPENSSL_ASYNC
    if (om->async) SSL_CTX_set_mode(oc->client_ssl_ctx, SSL_MODE_ASYNC);
#endif
    ...
    rv = openssl_client_init_verify(oc->ssl, (const char *) ctx->srv_hostname, 0, 0);
    if (rv) {
     ...
     */
    TLS_DBG(1, "Initiating handshake for [%u]%u", ctx->c_thread_index, oc->openssl_ctx_index);
    ...
    TLS_DBG(2, "tls state for [%u]%u is su", ctx->c_thread_index, oc->openssl_ctx_index, SSL_state_string_long(oc->ssl));
    return 0;
}
...
static int openssl_start_listen(tls_ctx_t *lctx)
{
    ...
    if (!ckpair->cert || !ckpair->key) {
        TLS_DBG(1, "tls cert and/or key not configured %d", lctx->parent_app_wrk_index);
        return -1;
    }

    method  = lctx->tls_type == TRANSPORT_PROTO_TLS ? SSLv23_server_method() : DTLS_server_method();
...
static int openssl_transport_reset(tls_ctx_t *ctx)
{
    if (!openssl_handshake_is_over(ctx)) {
        openssl_handle_handshake_failure(ctx);
        return 0;
...
static int openssl_app_close(tls_ctx_t *ctx)
{
    ...
    app_session = session_get_from_handle(ctx->app_session_handle);
    if (BIO_ctrl_pending(oc->rbio) <= 0 && !svm_fifo_max_dequeue_cons(app_session->tx_fifo)) openssl_confirm_app_close(ctx);
    return 0;
}
...
int openssl_reinit_ca_chain(void)
{
    openssl_main_t *om = &openssl_main;
...
static clib_error_t *tls_openssl_init(vlib_main_t *vm)
{
    ...
    /* default ciphers */
    tls_openssl_set_ciphers("ALL:!ADH:!LOW:!EXP:!MD5:!RC4-SHA:!DES-CBC3-SHA:@STRENGTH");
...
#ifdef HAVE_OPENSSL_ASYNC
static clib_error_t *tls_openssl_set_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    openssl_main_t *om              = &openssl_main;
    ...
    if (om->engine_init) {
        ...
        return clib_error_return(0, "engine has started, and no config is accepted");
    }
    ...
    if (!engine_name_set) {
        ...
        if (openssl_engine_register(engine_name, engine_alg, async) < 0) {
            return clib_error_return(0, "Failed to register %s polling", engine_name);
        }
        else {
