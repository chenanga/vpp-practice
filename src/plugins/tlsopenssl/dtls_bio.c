...
static inline session_t *bio_session(BIO *bio)
{
...
static int bio_dtls_free(BIO *bio)
{
    ...
    if (BIO_get_shutdown(bio)) {
        if (BIO_get_init(bio)) session_close(bio_session(bio));
        BIO_set_init(bio, 0);
        BIO_set_flags(bio, 0);
    }
    return 1;
...
static int bio_dtls_read(BIO *b, char *out, int outl)
{
    ...
    rv = app_recv_dgram_raw(s->rx_fifo, (u8 *) out, outl, &at, 0 /* clear evt */, 0 /* peek */);
    ...
    if (svm_fifo_is_empty_cons(s->rx_fifo)) svm_fifo_unset_event(s->rx_fifo);
...
static int bio_dtls_write(BIO *b, const char *in, int inl)
{
    ...
    rv = app_send_dgram_raw(s->tx_fifo, &at, mq, (u8 *) in, inl, SESSION_IO_EVT_TX, 1 /* do_evt */, 0 /* noblock */);
...
static int dtls_dgram_overhead(BIO *b)
{
    ...
    if (session_type_is_ip4(s->session_type)) /* 20B ip 8B udp */
        return 28;
    else
        /* 40B ip 8B udp */
        return 48;
