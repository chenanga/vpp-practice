...
struct engine_polling engine_list[] = {{"qat", qat_polling, qat_pre_init, qat_init_thread}, {"dasync", dasync_polling, NULL, NULL}};
...
int openssl_engine_register(char *engine_name, char *algorithm, int async)
{
    ...
    for (i = 0; i < vlib_num_workers(); i++) {
        if (engine_list[registered].thread_init)
            session_send_rpc_evt_to_thread(i + 1, engine_list[registered].thread_init, uword_to_pointer(i, void *));
    }
...
{
    openssl_evt_t **evt;
    evt = pool_elt_at_index(openssl_async_main.evt_pool[vlib_get_thread_index()], evt_index);
    return *evt;
}
...
static openssl_evt_t *openssl_evt_get_w_thread(int evt_index, u8 thread_index)
{
    ...
    evt = pool_elt_at_index(openssl_async_main.evt_pool[thread_index], evt_index);
    return *evt;
}
...
static u32 openssl_evt_alloc(void)
{
    ...
    if (!(*evt)) *evt = clib_mem_alloc(sizeof(openssl_evt_t));
...
int vpp_openssl_is_inflight(tls_ctx_t *ctx)
{
    ...
    if (event->status == SSL_ASYNC_INFLIGHT) return 1;
    return 0;
}
...
int vpp_tls_async_update_event(tls_ctx_t *ctx, int eagain)
{
    ...
    if (eagain) return tls_async_openssl_callback(0, &event->cb_args);
...
void qat_init_thread(void *arg)
{
    ...
    ENGINE_ctrl_cmd(om->engine, "SET_INSTANCE_FOR_THREAD", thread_index, NULL, NULL, 0);
...
void openssl_async_node_enable_disable(u8 is_en)
{
    ...
    foreach_vlib_main () {
        if (have_workers && this_vlib_main->thread_index) {
            vlib_node_set_state(this_vlib_main, tls_async_process_node.index, state);
        }
    }
}
