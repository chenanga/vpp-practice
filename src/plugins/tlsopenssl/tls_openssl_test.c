...
static int api_tls_openssl_set_engine(vat_main_t *vam)
{
    ...
    clib_memcpy_fast(mp->engine, engine_name, strnlen((const char *) engine_name, 63));

    clib_memcpy_fast(mp->algorithm, engine_alg, strnlen((const char *) engine_alg, 63));

    if (ciphers) clib_memcpy_fast(mp->ciphers, ciphers, strnlen((const char *) ciphers, 63));
