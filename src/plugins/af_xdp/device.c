...
static clib_error_t *af_xdp_mac_change(vnet_hw_interface_t *hw, const u8 *old, const u8 *new)
{
    ...
    if (err) return clib_error_return_code(0, -err, CLIB_ERROR_ERRNO_VALID, "mac change failed");
    return 0;
}

static clib_error_t *af_xdp_set_max_frame_size(vnet_main_t *vnm, vnet_hw_interface_t *hw, u32 frame_size)
{
    af_xdp_main_t   *am = &af_xdp_main;
    af_xdp_device_t *ad = vec_elt_at_index(am->devices, hw->dev_instance);
...
int af_xdp_enter_netns(char *netns, int *fds)
{
    *fds = *(fds + 1) = -1;
    if (netns != NULL) {
        *fds = clib_netns_open(NULL /* self */);
        if ((*(fds + 1) = clib_netns_open((u8 *) netns)) == -1) return VNET_API_ERROR_SYSCALL_ERROR_8;
        if (clib_setns(*(fds + 1)) == -1) return VNET_API_ERROR_SYSCALL_ERROR_9;
    }
    return 0;
}

void af_xdp_cleanup_netns(int *fds)
{
    if (*fds != -1) close(*fds);
...
int af_xdp_exit_netns(char *netns, int *fds)
{
    int ret = 0;
    if (netns != NULL) {
        if (*fds != -1) ret = clib_setns(*fds);
...
static int af_xdp_remove_program(af_xdp_device_t *ad)
{
    u32 curr_prog_id = 0;
    int ret;
    int ns_fds[2];
    ...
    ret = bpf_xdp_query_id(ad->linux_ifindex, XDP_FLAGS_UPDATE_IF_NOEXIST, &curr_prog_id);
    if (ret != 0) {
    ...
    if (ad->bpf_obj) bpf_object__close(ad->bpf_obj);
...
void af_xdp_delete_if(vlib_main_t *vm, af_xdp_device_t *ad)
{
    ...
    if (af_xdp_remove_program(ad) != 0) af_xdp_log(VLIB_LOG_LEVEL_ERR, ad, "Error while removing XDP program.\n");
...
static int af_xdp_load_program(af_xdp_create_if_args_t *args, af_xdp_device_t *ad)
{
    ...
    if (setrlimit(RLIMIT_MEMLOCK, &r))
        af_xdp_log(VLIB_LOG_LEVEL_WARNING, ad, "setrlimit(%s) failed: %s (errno %d)", ad->linux_ifname, strerror(errno), errno);

    ad->bpf_obj = bpf_object__open_file(args->prog, NULL);
    if (libbpf_get_error(ad->bpf_obj)) {
        args->rv    = VNET_API_ERROR_SYSCALL_ERROR_5;
        args->error = clib_error_return_unix(0, "bpf_object__open_file(%s) failed", args->prog);
        goto err0;
    }

    bpf_prog = bpf_object__next_program(ad->bpf_obj, NULL);
    if (!bpf_prog) goto err1;

    bpf_program__set_type(bpf_prog, BPF_PROG_TYPE_XDP);

    if (bpf_object__load(ad->bpf_obj)) goto err1;

    fd = bpf_program__fd(bpf_prog);

    if (bpf_xdp_attach(ad->linux_ifindex, fd, XDP_FLAGS_UPDATE_IF_NOEXIST, NULL)) {
        args->rv    = VNET_API_ERROR_SYSCALL_ERROR_6;
        args->error = clib_error_return_unix(0, "bpf_xdp_attach(%s) failed", ad->linux_ifname);
        goto err1;
    }
...
static int af_xdp_create_queue(vlib_main_t *vm, af_xdp_create_if_args_t *args, af_xdp_device_t *ad, int qid)
{
    ...
    if (xsk_umem__create(umem, uword_to_pointer(vm->buffer_main->buffer_mem_start, void *), vm->buffer_main->buffer_mem_size, fq, cq, &umem_config)) {
         ...
         * check */
        if (umem_config.frame_size < XDP_UMEM_MIN_CHUNK_SIZE || umem_config.frame_size > sys_page_size)
            args->error = clib_error_return(args->error,
    ...
    if (args->prog) sock_config.libbpf_flags = XSK_LIBBPF_FLAGS__INHIBIT_PROG_LOAD;
    if (xsk_socket__create(xsk, ad->linux_ifname, qid, *umem, rx, tx, &sock_config)) {
        args->rv    = VNET_API_ERROR_SYSCALL_ERROR_2;
        args->error = clib_error_return_unix(0, "xsk_socket__create() failed (is linux netdev %s up?)", ad->linux_ifname);
        goto err1;
    }

    fd = xsk_socket__fd(*xsk);
    if (args->prog) {
        struct bpf_map *map = bpf_object__find_map_by_name(ad->bpf_obj, "xsks_map");
        int             ret = xsk_socket__update_xskmap(*xsk, bpf_map__fd(map));
        if (ret) {
            args->rv    = VNET_API_ERROR_SYSCALL_ERROR_3;
            args->error = clib_error_return_unix(0, "xsk_socket__update_xskmap %s qid %d return %d", ad->linux_ifname, qid, ret);
            goto err2;
        }
    }
    optlen = sizeof(opt);
...
#define SOL_XDP 283
    ...
    if (opt.flags & XDP_OPTIONS_ZEROCOPY) ad->flags |= AF_XDP_DEVICE_F_ZEROCOPY;
...
static clib_error_t *af_xdp_device_set_rxq_mode(const af_xdp_device_t *ad, af_xdp_rxq_t *rxq, const af_xdp_rxq_mode_t mode)
{
    ...
    switch (mode) {
        ...
        case AF_XDP_RXQ_MODE_INTERRUPT:
            if (ad->flags & AF_XDP_DEVICE_F_SYSCALL_LOCK) return clib_error_create("kernel workaround incompatible with interrupt mode");
...
static u32 af_xdp_find_rxq_for_thread(vnet_main_t *vnm, const af_xdp_device_t *ad, const u32 thread)
{
    u32 i;
    for (i = 0; i < ad->rxq_num; i++) {
        const u32 qid = vec_elt(ad->rxqs, i).queue_index;
        const u32 tid = vnet_hw_if_get_rx_queue(vnm, qid)->thread_index;
        if (tid == thread) return i;
    }
    return ~0;
}

static clib_error_t *af_xdp_finalize_queues(vnet_main_t *vnm, af_xdp_device_t *ad, const int n_vlib_mains)
{
    clib_error_t *err = 0;
    int           i;
    ...
    for (i = 0; i < ad->rxq_num; i++) {
        ...
        rxq->queue_index  = vnet_hw_if_register_rx_queue(vnm, ad->hw_if_index, i, VNET_HW_IF_RXQ_THREAD_ANY);
...
void af_xdp_create_if(vlib_main_t *vm, af_xdp_create_if_args_t *args)
{
    ...
    if (args->rxq_size < VLIB_FRAME_SIZE || args->txq_size < VLIB_FRAME_SIZE || args->rxq_size > 65535 || args->txq_size > 65535
        ...
        args->rv    = VNET_API_ERROR_INVALID_VALUE;
        args->error = clib_error_return(0, "queue size must be a power of two between %i and 65535", VLIB_FRAME_SIZE);
        goto err0;
    }
    ...
    if (tm->n_vlib_mains > 1 && 0 == (args->flags & AF_XDP_CREATE_FLAGS_NO_SYSCALL_LOCK)) ad->flags |= AF_XDP_DEVICE_F_SYSCALL_LOCK;
    ...
    if (args->netns) ad->netns = (char *) format(0, "%s%c", args->netns, 0);
    ...
    if (args->prog && (af_xdp_remove_program(ad) || af_xdp_load_program(args, ad))) goto err2;
    ...
    for (i = 0; i < q_num; i++) {
        if (af_xdp_create_queue(vm, args, ad, i)) {
             ...
             */
            af_xdp_log(VLIB_LOG_LEVEL_DEBUG, ad, "create interface failed to create queue qid=%d", i);
...
static clib_error_t *af_xdp_interface_admin_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    ...
    if (ad->flags & AF_XDP_DEVICE_F_ERROR) return clib_error_return(0, "device is in error state");
...
static void af_xdp_set_interface_next_node(vnet_main_t *vnm, u32 hw_if_index, u32 node_index)
{
    ...
    ad->per_interface_next_index = vlib_node_add_next(vlib_get_main(), af_xdp_input_node.index, node_index);
}
