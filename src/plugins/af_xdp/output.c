...
static_always_inline void af_xdp_device_output_free(vlib_main_t *vm, const vlib_node_runtime_t *node, af_xdp_txq_t *txq)
{
    ...
    STATIC_ASSERT(BITS(bi[0]) + CLIB_LOG2_CACHE_LINE_BYTES <= XSK_UNALIGNED_BUF_OFFSET_SHIFT, "wrong size");
                                                     ...
                                                     const u32                  n_tx)
{
    ...
    if (!xsk_ring_prod__needs_wakeup(&txq->tx)) return;
...
af_xdp_device_output_tx_try(vlib_main_t *vm, const vlib_node_runtime_t *node, af_xdp_device_t *ad, af_xdp_txq_t *txq, u32 n_tx, u32 *bi)
{
    ...
    if (PREDICT_FALSE(0 == n_tx)) return 0;
...
wrap_around:
    ...
    while (n >= 8) {
        if (PREDICT_FALSE(b[0]->flags & VLIB_BUFFER_NEXT_PRESENT || b[1]->flags & VLIB_BUFFER_NEXT_PRESENT || b[2]->flags & VLIB_BUFFER_NEXT_PRESENT
                          || b[3]->flags & VLIB_BUFFER_NEXT_PRESENT)) {
            break;
        }

        vlib_prefetch_buffer_header(b[4], LOAD);
        offset       = (sizeof(vlib_buffer_t) + b[0]->current_data) << XSK_UNALIGNED_BUF_OFFSET_SHIFT;
        addr         = pointer_to_uword(b[0]) - start;
        desc[0].addr = offset | addr;
        desc[0].len  = b[0]->current_length;

        vlib_prefetch_buffer_header(b[5], LOAD);
        offset       = (sizeof(vlib_buffer_t) + b[1]->current_data) << XSK_UNALIGNED_BUF_OFFSET_SHIFT;
        addr         = pointer_to_uword(b[1]) - start;
        desc[1].addr = offset | addr;
        desc[1].len  = b[1]->current_length;

        vlib_prefetch_buffer_header(b[6], LOAD);
        offset       = (sizeof(vlib_buffer_t) + b[2]->current_data) << XSK_UNALIGNED_BUF_OFFSET_SHIFT;
        addr         = pointer_to_uword(b[2]) - start;
        desc[2].addr = offset | addr;
        desc[2].len  = b[2]->current_length;

        vlib_prefetch_buffer_header(b[7], LOAD);
        offset       = (sizeof(vlib_buffer_t) + b[3]->current_data) << XSK_UNALIGNED_BUF_OFFSET_SHIFT;
    ...
    while (n >= 1) {
        if (PREDICT_FALSE(b[0]->flags & VLIB_BUFFER_NEXT_PRESENT)) {
            if (vlib_buffer_chain_linearize(vm, b[0]) != 1) {
                af_xdp_log(VLIB_LOG_LEVEL_ERR, ad, "vlib_buffer_chain_linearize failed");
