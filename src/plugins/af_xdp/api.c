...
static af_xdp_create_flag_t af_xdp_api_flags(vl_api_af_xdp_flag_t flags)
{
    ...
    if (flags & AF_XDP_API_FLAGS_NO_SYSCALL_LOCK) cflags |= AF_XDP_CREATE_FLAGS_NO_SYSCALL_LOCK;
...
static void vl_api_af_xdp_create_t_handler(vl_api_af_xdp_create_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_AF_XDP_CREATE_REPLY, ({ rmp->sw_if_index = ntohl(args.sw_if_index); }));
}

static void vl_api_af_xdp_create_v2_t_handler(vl_api_af_xdp_create_v2_t *mp)
{
    vlib_main_t                     *vm = vlib_get_main();
    af_xdp_main_t                   *rm = &af_xdp_main;
...
static void vl_api_af_xdp_create_v3_t_handler(vl_api_af_xdp_create_v3_t *mp)
{
    vlib_main_t                     *vm = vlib_get_main();
    af_xdp_main_t                   *rm = &af_xdp_main;
