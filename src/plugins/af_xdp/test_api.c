...
static vl_api_af_xdp_mode_t api_af_xdp_mode(af_xdp_mode_t mode)
{
    switch (mode) {
...
/* af_xdp create API */
static int api_af_xdp_create(vat_main_t *vam)
{
    ...
    if (args.flags & AF_XDP_CREATE_FLAGS_NO_SYSCALL_LOCK) mp->flags |= AF_XDP_API_FLAGS_NO_SYSCALL_LOCK;
...
static int api_af_xdp_create_v2(vat_main_t *vam)
{
    ...
    snprintf((char *) mp->host_if, sizeof(mp->host_if), "%s", args.linux_ifname ?: "");
    snprintf((char *) mp->name, sizeof(mp->name), "%s", args.name ?: "");
    snprintf((char *) mp->namespace, sizeof(mp->namespace), "%s", args.netns ?: "");
    mp->rxq_num  = clib_host_to_net_u16(args.rxq_num);
    mp->rxq_size = clib_host_to_net_u16(args.rxq_size);
    mp->txq_size = clib_host_to_net_u16(args.txq_size);
    mp->mode     = api_af_xdp_mode(args.mode);
...
/* af_xdp create v2 API */
static int api_af_xdp_create_v3(vat_main_t *vam)
{
...
static void vl_api_af_xdp_create_reply_t_handler(vl_api_af_xdp_create_reply_t *mp)
{
...
static void vl_api_af_xdp_create_v2_reply_t_handler(vl_api_af_xdp_create_v2_reply_t *mp)
{
    vat_main_t *vam    = af_xdp_test_main.vat_main;
    i32         retval = ntohl(mp->retval);
...
static void vl_api_af_xdp_create_v3_reply_t_handler(vl_api_af_xdp_create_v2_reply_t *mp)
{
    vat_main_t *vam    = af_xdp_test_main.vat_main;
...
/* af_xdp delete API */
static int api_af_xdp_delete(vat_main_t *vam)
{
