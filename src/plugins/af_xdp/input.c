                                                        ...
                                                        const u32                  n_alloc)
{
    ...
    if (AF_XDP_RXQ_MODE_INTERRUPT == rxq->mode || !xsk_ring_prod__needs_wakeup(&rxq->fq)) return;

    if (node) vlib_error_count(vm, node->node_index, AF_XDP_INPUT_ERROR_SYSCALL_REQUIRED, 1);
    ...
    if (clib_spinlock_trylock_if_init(&rxq->syscall_lock)) {
        ...
        if (PREDICT_FALSE(ret < 0)) {
            ...
            if (node) vlib_error_count(vm, node->node_index, AF_XDP_INPUT_ERROR_SYSCALL_FAILURES, 1);
...
af_xdp_device_input_bufs(vlib_main_t *vm, const af_xdp_device_t *ad, af_xdp_rxq_t *rxq, u32 *bis, const u32 n_rx, vlib_buffer_t *bt, u32 idx)
{
...
static_always_inline uword af_xdp_device_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, af_xdp_device_t *ad, u16 qid)
{
    ...
    if (PREDICT_FALSE(0 == n_rx_packets)) goto refill;
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    vec_foreach (p, pv) {
        ...
        if ((ad->flags & AF_XDP_DEVICE_F_ADMIN_UP) == 0) continue;
