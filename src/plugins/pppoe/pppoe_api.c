...
out:
    REPLY_MACRO2(VL_API_PPPOE_ADD_DEL_SESSION_REPLY, ({ rmp->sw_if_index = ntohl(sw_if_index); }));
}
...
static void send_pppoe_session_details(pppoe_session_t *t, vl_api_registration_t *reg, u32 context)
{
    ...
    ip_address_encode(&t->client_ip, is_ipv6 ? IP46_TYPE_IP6 : IP46_TYPE_IP4, &rmp->client_ip);
...
static void vl_api_pppoe_session_dump_t_handler(vl_api_pppoe_session_dump_t *mp)
{
    ...
    if (~0 == sw_if_index) {
        ...
        if ((sw_if_index >= vec_len(pem->session_index_by_sw_if_index)) || (~0 == pem->session_index_by_sw_if_index[sw_if_index])) {
            return;
        }
