...
VLIB_NODE_FN(pppoe_cp_dispatch_node)
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (rx_sw_if_index0 == pem->cp_if_index) {
                pppoe_lookup_1(&pem->link_table, &cached_key, &cached_result, h0->dst_address, 0, &key0, &bucket0, &result0);
                ...
                pppoe_lookup_1(&pem->link_table, &cached_key, &cached_result, h0->src_address, 0, &key0, &bucket0, &result0);
                tx_sw_if_index0 = result0.fields.sw_if_index;

                /* learn client session */
                pppoe_learn_process(&pem->link_table, rx_sw_if_index0, &key0, &cached_key, &bucket0, &result0);
        ...
        trace00:
            ...
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                pppoe_cp_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
                tr->next_index       = next0;
                tr->error            = error0;
