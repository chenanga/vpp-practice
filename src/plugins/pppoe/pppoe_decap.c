...
static u8 *format_pppoe_rx_trace(u8 *s, va_list *args)
{
    ...
    if (t->session_index != ~0) {
        s = format(s, "PPPoE decap from pppoe_session%d session_id %d next %d error %d", t->session_index, t->session_id, t->next_index, t->error);
    }
    else {
        s = format(s, "PPPoE decap error - session for session_id %d does not exist", t->session_id);
    }
    return s;
}

VLIB_NODE_FN(pppoe_input_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if ((ppp_proto0 != PPP_PROTOCOL_ip4) && (ppp_proto0 != PPP_PROTOCOL_ip6)) {
                vlan0 == 0 ? vlib_buffer_advance(b0, sizeof(*h0)) : vlib_buffer_advance(b0, sizeof(*h0) + sizeof(*vlan0));
            ...
            pppoe_lookup_1(&pem->session_table, &cached_key, &cached_result, h0->src_address, pppoe0->session_id, &key0, &bucket0, &result0);
            ...
            vlan0 == 0 ? vlib_buffer_advance(b0, sizeof(*h0) + sizeof(*pppoe0))
                       : vlib_buffer_advance(b0, sizeof(*h0) + sizeof(*vlan0) + sizeof(*pppoe0));

            next0 = (ppp_proto0 == PPP_PROTOCOL_ip4) ? PPPOE_INPUT_NEXT_IP4_INPUT : PPPOE_INPUT_NEXT_IP6_INPUT;
        ...
        trace0:
            ...
            if (type1 == ETHERNET_TYPE_VLAN) {
                ...
                if (type1 != ETHERNET_TYPE_PPPOE_DISCOVERY && type1 != ETHERNET_TYPE_PPPOE_SESSION) {
                    ...
                    result1.fields.session_index = ~0;  // avoid tracing random data
            ...
            if ((ppp_proto1 != PPP_PROTOCOL_ip4) && (ppp_proto1 != PPP_PROTOCOL_ip6)) {
                vlan1 == 0 ? vlib_buffer_advance(b1, sizeof(*h1)) : vlib_buffer_advance(b1, sizeof(*h1) + sizeof(*vlan1));
            ...
            pppoe_lookup_1(&pem->session_table, &cached_key, &cached_result, h1->src_address, pppoe1->session_id, &key1, &bucket1, &result1);
            ...
            t1 = pool_elt_at_index(pem->sessions, result1.fields.session_index);

            /* Pop Eth and PPPoE header */
            vlan1 == 0 ? vlib_buffer_advance(b1, sizeof(*h1) + sizeof(*pppoe1))
                       : vlib_buffer_advance(b1, sizeof(*h1) + sizeof(*vlan1) + sizeof(*pppoe1));

            next1 = (ppp_proto1 == PPP_PROTOCOL_ip4) ? PPPOE_INPUT_NEXT_IP4_INPUT : PPPOE_INPUT_NEXT_IP6_INPUT;
        ...
        trace1:
            ...
            if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
                pppoe_rx_trace_t *tr = vlib_add_trace(vm, node, b1, sizeof(*tr));
                tr->next_index       = next1;
                tr->error            = error1;
    ...
    vlib_node_increment_counter(vm, pppoe_input_node.index, PPPOE_ERROR_DECAPSULATED, pkts_decapsulated);
