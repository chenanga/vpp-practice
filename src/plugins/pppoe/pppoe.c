...
u8 *format_pppoe_session(u8 *s, va_list *args)
{
    ...
    s = format(s, "    local-mac %U  client-mac %U", format_ethernet_address, t->local_mac, format_ethernet_address, t->client_mac);
...
static u8 *pppoe_build_rewrite(vnet_main_t *vnm, u32 sw_if_index, vnet_link_t link_type, const void *dst_address)
{
    ...
    if (si->type == VNET_SW_INTERFACE_TYPE_SUB) {
        if (si->sub.eth.flags.one_tag == 1) {
            ...
            ethernet_vlan_header_t *vlan = (ethernet_vlan_header_t *) (eth_hdr + 1);
            vlan->type                   = clib_host_to_net_u16(ETHERNET_TYPE_PPPOE_SESSION);
            vlan->priority_cfi_and_id    = clib_host_to_net_u16(si->sub.eth.outer_vlan_id);
...
static void pppoe_fixup(vlib_main_t *vm, const ip_adjacency_t *adj, vlib_buffer_t *b0, const void *data)
{
    ...
    pppoe0->length = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, b0) - sizeof(pppoe_header_t) + sizeof(pppoe0->ppp_proto) - len);
}
...
static void pppoe_update_adj(vnet_main_t *vnm, u32 sw_if_index, adj_index_t ai)
{
    ...
    switch (adj->lookup_next_index) {
        ...
        case IP_LOOKUP_NEXT_BCAST:
            adj_nbr_midchain_update_rewrite(ai, pppoe_fixup, (void *) len, ADJ_FLAG_NONE, pppoe_build_rewrite(vnm, sw_if_index, adj->ia_link, NULL));
            break;
        case IP_LOOKUP_NEXT_MCAST:
             ...
             */
            adj_mcast_midchain_update_rewrite(ai,
                                              pppoe_fixup,
                                              (void *) len,
                                              ADJ_FLAG_NONE,
                                              pppoe_build_rewrite(vnm, sw_if_index, adj->ia_link, NULL),
                                              0,
                                              0);
            break;
    ...
    interface_tx_dpo_add_or_lock(vnet_link_to_dpo_proto(adj->ia_link), t->encap_if_index, &dpo);
...
int vnet_pppoe_add_del_session(vnet_pppoe_add_del_session_args_t *a, u32 *sw_if_indexp)
{
    ...
    pppoe_lookup_1(&pem->link_table, &cached_key, &cached_result, a->client_mac, 0, &key, &bucket, &result);
    a->encap_if_index = result.fields.sw_if_index;

    if (a->encap_if_index == ~0) return VNET_API_ERROR_INVALID_SW_IF_INDEX;

    si = vnet_get_sw_interface(vnm, a->encap_if_index);
    hi = vnet_get_hw_interface(vnm, si->hw_if_index);

    /* lookup session_table */
    pppoe_lookup_1(&pem->session_table, &cached_key, &cached_result, a->client_mac, clib_host_to_net_u16(a->session_id), &key, &bucket, &result);

    /* learn client session */
    pppoe_learn_process(&pem->session_table, a->encap_if_index, &key, &cached_key, &bucket, &result);

    if (a->is_add) {
        /* adding a session: session must not already exist */
        if (result.fields.session_index != ~0) return VNET_API_ERROR_TUNNEL_EXIST;

        /*if not set explicitly, default to ip4 */
        if (!pppoe_decap_next_is_valid(pem, is_ip6, a->decap_fib_index)) return VNET_API_ERROR_INVALID_DECAP_NEXT;
...
#undef _
        ...
        pppoe_update_1(&pem->session_table, a->client_mac, clib_host_to_net_u16(a->session_id), &key, &bucket, &result);

        vnet_hw_interface_t *hi;
        if (vec_len(pem->free_pppoe_session_hw_if_indices) > 0) {
            vnet_interface_main_t *im = &vnm->interface_main;
            hw_if_index               = pem->free_pppoe_session_hw_if_indices[vec_len(pem->free_pppoe_session_hw_if_indices) - 1];
            ...
            hw_if_index = vnet_register_interface(vnm, pppoe_device_class.index, t - pem->sessions, pppoe_hw_class.index, t - pem->sessions);
            hi          = vnet_get_hw_interface(vnm, hw_if_index);
        }
        ...
        vnet_set_interface_l3_output_node(vnm->vlib_main, sw_if_index, (u8 *) "tunnel-output");

        /* add reverse route for client ip */
        fib_table_entry_path_add(a->decap_fib_index,
                                 &pfx,
                                 pppoe_fib_src,
                                 FIB_ENTRY_FLAG_NONE,
        ...
        if (result.fields.session_index == ~0) return VNET_API_ERROR_NO_SUCH_ENTRY;
        ...
        fib_table_entry_path_remove(a->decap_fib_index,
                                    &pfx,
                                    pppoe_fib_src,
                                    fib_proto_to_dpo(pfx.fp_proto),
                                    &pfx.fp_addr,
...
#undef _
    ...
    switch (rv) {
        ...
        default:
            error = clib_error_return(0, "vnet_pppoe_add_del_session returned %d", rv);
            goto done;
    }
...
static clib_error_t *show_pppoe_session_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (pool_elts(pem->sessions) == 0) vlib_cli_output(vm, "No pppoe sessions configured...");
...
static int pppoe_show_walk_cb(BVT(clib_bihash_kv) * kvp, void *arg)
{
    ...
    if (ctx->first_entry) {
        ctx->first_entry = 0;
        vlib_cli_output(ctx->vm, "%=19s%=12s%=13s%=14s", "Mac-Address", "session_id", "sw_if_index", "session_index");
    }
    ...
    vlib_cli_output(ctx->vm,
                    ...
                    result.fields.session_index == ~0 ? -1 : result.fields.session_index);
    ctx->total_entries++;
...
clib_error_t *pppoe_init(vlib_main_t *vm)
{
    ...
    pppoe_fib_src = fib_source_allocate("pppoe", FIB_SOURCE_PRIORITY_HI, FIB_SOURCE_BH_API);
