...
static void vl_api_vxlan_add_del_tunnel_t_handler(vl_api_vxlan_add_del_tunnel_t *mp)
{
    ...
    if (rv) goto out;
    a.dst_port = a.is_ip6 ? UDP_DST_PORT_vxlan6 : UDP_DST_PORT_vxlan, a.src_port = a.is_ip6 ? UDP_DST_PORT_vxlan6 : UDP_DST_PORT_vxlan,
    rv = vnet_vxlan_add_del_tunnel(&a, &sw_if_index);

out:
    REPLY_MACRO2(VL_API_VXLAN_ADD_DEL_TUNNEL_REPLY, ({ rmp->sw_if_index = ntohl(sw_if_index); }));
}
...
static void vl_api_vxlan_add_del_tunnel_v2_t_handler(vl_api_vxlan_add_del_tunnel_v2_t *mp)
{
    ...
    rv = vnet_vxlan_add_del_tunnel(&a, &sw_if_index);
out:
    REPLY_MACRO2(VL_API_VXLAN_ADD_DEL_TUNNEL_V2_REPLY, ({ rmp->sw_if_index = ntohl(sw_if_index); }));
}
...
static void vl_api_vxlan_add_del_tunnel_v3_t_handler(vl_api_vxlan_add_del_tunnel_v3_t *mp)
{
    ...
    rv = vnet_vxlan_add_del_tunnel(&a, &sw_if_index);
out:
    REPLY_MACRO2(VL_API_VXLAN_ADD_DEL_TUNNEL_V3_REPLY, ({ rmp->sw_if_index = ntohl(sw_if_index); }));
}
...
static clib_error_t *vxlan_api_hookup(vlib_main_t *vm)
{
    ...
    vl_api_increase_msg_trace_size(am, VL_API_VXLAN_ADD_DEL_TUNNEL, 16 * sizeof(u32));
