...
u8 *format_vxlan_encap_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "VXLAN encap to vxlan_tunnel%d vni %d", t->tunnel_index, t->vni);
    return s;
}
#endif

always_inline uword vxlan_encap_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, u8 is_ip4)
{
    ...
    u32 const outer_packet_csum_offload_flags = is_ip4 ? (VNET_BUFFER_OFFLOAD_F_OUTER_IP_CKSUM | VNET_BUFFER_OFFLOAD_F_TNL_VXLAN)
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (sw_if_index1 != vnet_buffer(b1)->sw_if_index[VLIB_TX]) {
                if (sw_if_index0 == vnet_buffer(b1)->sw_if_index[VLIB_TX]) {
                    ...
                    vnet_hw_interface_t *hi1 = vnet_get_sup_hw_interface(vnm, sw_if_index1);
            ...
            if (b0->flags & VNET_BUFFER_F_OFFLOAD) {
                ...
                vnet_buffer_offload_flags_set(b0, outer_packet_csum_offload_flags);
            }
            /* IPv4 checksum only */
            else if (is_ip4) {
                ip_csum_t sum0 = ip4_0->checksum;
                sum0           = ip_csum_update(sum0, 0, ip4_0->length, ip4_header_t, length /* changed member */);
                if (PREDICT_FALSE(ip4_0_tos)) {
                    sum0 = ip_csum_update(sum0, 0, ip4_0_tos, ip4_header_t, tos /* changed member */);
                }
            ...
            if (b1->flags & VNET_BUFFER_F_OFFLOAD) {
                ...
                vnet_buffer_offload_flags_set(b1, outer_packet_csum_offload_flags);
            }
            /* IPv4 checksum only */
            else if (is_ip4) {
                ip_csum_t sum1 = ip4_1->checksum;
                sum1           = ip_csum_update(sum1, 0, ip4_1->length, ip4_header_t, length /* changed member */);
                if (PREDICT_FALSE(ip4_1_tos)) {
                    sum1 = ip_csum_update(sum1, 0, ip4_1_tos, ip4_header_t, tos /* changed member */);
                }
            ...
            if (sw_if_index0 == sw_if_index1) {
                vlib_increment_combined_counter(tx_counter, thread_index, sw_if_index0, 2, len0 + len1);
            }
            else {
                vlib_increment_combined_counter(tx_counter, thread_index, sw_if_index0, 1, len0);
                vlib_increment_combined_counter(tx_counter, thread_index, sw_if_index1, 1, len1);
            }
            pkts_encapsulated += 2;

            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                vxlan_encap_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
                tr->tunnel_index        = t0 - vxm->tunnels;
                tr->vni                 = t0->vni;
            }

            if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
                vxlan_encap_trace_t *tr = vlib_add_trace(vm, node, b1, sizeof(*tr));
    ...
    vlib_node_increment_counter(vm, node->node_index, VXLAN_ENCAP_ERROR_ENCAPSULATED, pkts_encapsulated);
...
VLIB_NODE_FN(vxlan6_encap_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
