...
static u8 *format_vxlan_rx_trace(u8 *s, va_list *args)
{
    ...
    if (t->tunnel_index == ~0) return format(s, "VXLAN decap error - tunnel for vni %d does not exist", t->vni);
    return format(s, "VXLAN decap from vxlan_tunnel%d vni %d next %d error %d", t->tunnel_index, t->vni, t->next_index, t->error);
}

typedef vxlan4_tunnel_key_t last_tunnel_cache4;

static const vxlan_decap_info_t decap_not_found = {.sw_if_index = ~0, .next_index = VXLAN_INPUT_NEXT_DROP, .error = VXLAN_ERROR_NO_SUCH_TUNNEL};

static const vxlan_decap_info_t decap_bad_flags = {.sw_if_index = ~0, .next_index = VXLAN_INPUT_NEXT_DROP, .error = VXLAN_ERROR_BAD_FLAGS};
...
{
    if (PREDICT_FALSE(vxlan0->flags != VXLAN_FLAGS_I)) return decap_bad_flags;
    ...
    if (PREDICT_TRUE(key4.key[0] == cache->key[0] && key4.key[1] == cache->key[1])) {
        /* cache hit */
    ...
    /* try multicast */
    if (PREDICT_TRUE(!ip4_address_is_multicast(&ip4_0->dst_address))) return decap_not_found;

    /* search for mcast decap info by mcast address */
    key4.key[0] = dst;
    rv          = clib_bihash_search_inline_16_8(&vxm->vxlan4_tunnel_by_key, &key4);
    if (rv != 0) return decap_not_found;
    ...
    rv                     = clib_bihash_search_inline_16_8(&vxm->vxlan4_tunnel_by_key, &key4);
    if (PREDICT_FALSE(rv != 0)) return decap_not_found;
...
always_inline vxlan_decap_info_t
vxlan6_find_tunnel(vxlan_main_t *vxm, last_tunnel_cache6 *cache, u32 fib_index, ip6_header_t *ip6_0, vxlan_header_t *vxlan0, u32 *stats_sw_if_index)
{
    if (PREDICT_FALSE(vxlan0->flags != VXLAN_FLAGS_I)) return decap_bad_flags;
    ...
    if (PREDICT_TRUE(ip6_address_is_equal(&ip6_0->dst_address, &t0->src.ip6)))
        *stats_sw_if_index = t0->sw_if_index;
        ...
        /* try multicast */
        if (PREDICT_TRUE(!ip6_address_is_multicast(&ip6_0->dst_address))) return decap_not_found;
...
always_inline uword vxlan_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, u32 is_ip4)
{
    ...
    while (n_left_from >= 4) {
        ...
        vxlan_decap_info_t di0 = is_ip4 ? vxlan4_find_tunnel(vxm, &last4, fi0, ip4_0, vxlan0, &stats_if0)
                                        : vxlan6_find_tunnel(vxm, &last6, fi0, ip6_0, vxlan0, &stats_if0);
        vxlan_decap_info_t di1 = is_ip4 ? vxlan4_find_tunnel(vxm, &last4, fi1, ip4_1, vxlan1, &stats_if1)
        ...
        if (PREDICT_TRUE(any_error == 0)) {
            ...
            vlib_increment_combined_counter(rx_counter, thread_index, stats_if0, 1, len0);
            vlib_increment_combined_counter(rx_counter, thread_index, stats_if1, 1, len1);
        }
        else {
            if (di0.error == 0) {
                vnet_update_l2_len(b[0]);
        ...
        if (PREDICT_FALSE(b[0]->flags & VLIB_BUFFER_IS_TRACED)) {
            vxlan_rx_trace_t *tr = vlib_add_trace(vm, node, b[0], sizeof(*tr));
            tr->next_index       = next[0];
            tr->error            = di0.error;
            tr->tunnel_index     = di0.sw_if_index == ~0 ? ~0 : vxm->tunnel_index_by_sw_if_index[di0.sw_if_index];
            tr->vni              = vnet_get_vni(vxlan0);
        }
        if (PREDICT_FALSE(b[1]->flags & VLIB_BUFFER_IS_TRACED)) {
            vxlan_rx_trace_t *tr = vlib_add_trace(vm, node, b[1], sizeof(*tr));
            tr->next_index       = next[1];
            tr->error            = di1.error;
            tr->tunnel_index     = di1.sw_if_index == ~0 ? ~0 : vxm->tunnel_index_by_sw_if_index[di1.sw_if_index];
    ...
    vlib_node_increment_counter(vm, node_idx, VXLAN_ERROR_DECAPSULATED, from_frame->n_vectors - pkts_dropped);
...
VLIB_NODE_FN(vxlan6_input_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
...
always_inline uword ip_vxlan_bypass_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u32 is_ip4)
{
    vxlan_main_t        *vxm = &vxlan_main;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            /* Process packet 0 */
            if (proto0 != IP_PROTOCOL_UDP) goto exit0; /* not UDP packet */
            ...
            vxlan_decap_info_t di0 = is_ip4 ? vxlan4_find_tunnel(vxm, &last4, fi0, ip40, vxlan0, &stats_if0)
                                            : vxlan6_find_tunnel(vxm, &last6, fi0, ip60, vxlan0, &stats_if0);

            if (PREDICT_FALSE(di0.sw_if_index == ~0)) goto exit0; /* unknown interface */

            /* Validate DIP against VTEPs */
            if (is_ip4) {
#ifdef CLIB_HAVE_VEC512
                if (!vtep4_check_vector(&vxm->vtep_table, b0, ip40, &last_vtep4, &vxm->vtep4_u512))
#else
                ...
                if (!vtep6_check(&vxm->vtep_table, b0, ip60, &last_vtep6)) goto exit0; /* no local VTEP for VXLAN packet */
            }
            ...
            next0     = error0 ? IP_VXLAN_BYPASS_NEXT_DROP : IP_VXLAN_BYPASS_NEXT_VXLAN;
        ...
        exit0:
            /* Process packet 1 */
            if (proto1 != IP_PROTOCOL_UDP) goto exit1; /* not UDP packet */
            ...
            vxlan_decap_info_t di1 = is_ip4 ? vxlan4_find_tunnel(vxm, &last4, fi1, ip41, vxlan1, &stats_if1)
                                            : vxlan6_find_tunnel(vxm, &last6, fi1, ip61, vxlan1, &stats_if1);

            if (PREDICT_FALSE(di1.sw_if_index == ~0)) goto exit1; /* unknown interface */

            /* Validate DIP against VTEPs */
            if (is_ip4) {
#ifdef CLIB_HAVE_VEC512
                if (!vtep4_check_vector(&vxm->vtep_table, b1, ip41, &last_vtep4, &vxm->vtep4_u512))
#else
                ...
                if (!vtep6_check(&vxm->vtep_table, b1, ip61, &last_vtep6)) goto exit1; /* no local VTEP for VXLAN packet */
            }
            ...
            next1     = error1 ? IP_VXLAN_BYPASS_NEXT_DROP : IP_VXLAN_BYPASS_NEXT_VXLAN;
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (proto0 != IP_PROTOCOL_UDP) goto exit; /* not UDP packet */
            ...
            if (PREDICT_FALSE(di0.sw_if_index == ~0)) goto exit; /* unknown interface */
            ...
            if (is_ip4) {
                ...
                if (!vtep6_check(&vxm->vtep_table, b0, ip60, &last_vtep6)) goto exit; /* no local VTEP for VXLAN packet */
            }
...
VLIB_NODE_FN(ip4_vxlan_bypass_node)
...
VLIB_NODE_FN(ip6_vxlan_bypass_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static_always_inline u8 vxlan_validate_udp_csum(vlib_main_t *vm, vlib_buffer_t *b)
{
    u32 flags = b->flags;
    enum {
        offset = sizeof(ip4_header_t) + sizeof(udp_header_t) + sizeof(vxlan_header_t),
    };
...
static_always_inline u8 vxlan_check_udp_csum(vlib_main_t *vm, vlib_buffer_t *b)
{
    ...
    /* Don't verify UDP checksum for packets with explicit zero checksum. */
    u8 good_csum = (b->flags & VNET_BUFFER_F_L4_CHECKSUM_CORRECT) != 0 || udp->checksum == 0;
...
static_always_inline u8 vxlan_err_code(u8 ip_err0, u8 udp_err0, u8 csum_err0)
{
    ...
    if (ip_err0) error0 = VXLAN_FLOW_ERROR_IP_HEADER_ERROR;
    if (udp_err0) error0 = VXLAN_FLOW_ERROR_UDP_LENGTH_ERROR;
    if (csum_err0) error0 = VXLAN_FLOW_ERROR_UDP_CHECKSUM_ERROR;
    return error0;
}

VLIB_NODE_FN(vxlan4_flow_input_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *f)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 3 && n_left_to_next > 3) {
            ...
            u32 next0 = VXLAN_FLOW_NEXT_L2_INPUT, next1 = VXLAN_FLOW_NEXT_L2_INPUT, next2 = VXLAN_FLOW_NEXT_L2_INPUT,
                next3 = VXLAN_FLOW_NEXT_L2_INPUT;
            ...
            if (PREDICT_FALSE(csum_err)) {
                if (csum_err0) csum_err0 = !vxlan_validate_udp_csum(vm, b0);
                if (csum_err1) csum_err1 = !vxlan_validate_udp_csum(vm, b1);
                if (csum_err2) csum_err2 = !vxlan_validate_udp_csum(vm, b2);
                if (csum_err3) csum_err3 = !vxlan_validate_udp_csum(vm, b3);
