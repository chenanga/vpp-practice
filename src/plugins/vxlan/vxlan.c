...
static clib_error_t *vxlan_eth_set_max_frame_size(vnet_main_t *vnm, vnet_hw_interface_t *hw, u32 frame_size)
{
    /* nothing for now */
    return 0;
}
...
u8 *format_vxlan_tunnel(u8 *s, va_list *args)
{
    ...
    if (PREDICT_FALSE(t->decap_next_index != VXLAN_INPUT_NEXT_L2_INPUT)) s = format(s, "decap-next-%U ", format_decap_next, t->decap_next_index);

    if (PREDICT_FALSE(ip46_address_is_multicast(&t->dst))) s = format(s, "mcast-sw-if-idx %d ", t->mcast_sw_if_index);

    if (t->flow_index != ~0) s = format(s, "flow-index %d [%U]", t->flow_index, format_flow_enabled_hw, t->flow_index);
...
static u8 *format_vxlan_name(u8 *s, va_list *args)
{
    ...
    if (dev_instance == ~0) return format(s, "<cached-unused>");

    if (dev_instance >= vec_len(vxm->tunnels)) return format(s, "<improperly-referenced>");
...
static void vxlan_tunnel_restack_dpo(vxlan_tunnel_t *t)
{
    ...
    fib_forward_chain_type_t forw_type = is_ip4 ? FIB_FORW_CHAIN_TYPE_UNICAST_IP4 : FIB_FORW_CHAIN_TYPE_UNICAST_IP6;
    ...
    u32 encap_index = is_ip4 ? vxlan4_encap_node.index : vxlan6_encap_node.index;
...
static vxlan_tunnel_t *vxlan_tunnel_from_fib_node(fib_node_t *node)
{
    ...
    return ((vxlan_tunnel_t *) (((char *) node) - STRUCT_OFFSET_OF(vxlan_tunnel_t, node)));
}
...
static inline mcast_shared_t mcast_shared_get(ip46_address_t *ip)
{
...
static inline void mcast_shared_add(ip46_address_t *dst, fib_node_index_t mfei, adj_index_t ai)
{
...
static inline void mcast_shared_remove(ip46_address_t *dst)
{
...
int vnet_vxlan_add_del_tunnel(vnet_vxlan_add_del_tunnel_args_t *a, u32 *sw_if_indexp)
{
    ...
    /* Set udp-ports */
    if (a->src_port == 0) a->src_port = is_ip6 ? UDP_DST_PORT_vxlan6 : UDP_DST_PORT_vxlan;
    ...
    if (!is_ip6) {
        ...
        key4.key[0] = ip46_address_is_multicast(&a->dst) ? a->dst.ip4.as_u32 : a->dst.ip4.as_u32 | (((u64) a->src.ip4.as_u32) << 32);
        key4.key[1] = ((u64) clib_host_to_net_u16(a->src_port) << 48) | (((u64) a->encap_fib_index) << 32) | clib_host_to_net_u32(a->vni << 8);
        not_found   = clib_bihash_search_inline_16_8(&vxm->vxlan4_tunnel_by_key, &key4);
        p           = (void *) &key4.value;
    }
    else {
        key6.key[0] = a->dst.ip6.as_u64[0];
        key6.key[1] = a->dst.ip6.as_u64[1];
        key6.key[2] = (((u64) clib_host_to_net_u16(a->src_port) << 48) | ((u64) a->encap_fib_index) << 32) | clib_host_to_net_u32(a->vni << 8);
        not_found   = clib_bihash_search_inline_24_8(&vxm->vxlan6_tunnel_by_key, &key6);
        p           = (void *) &key6.value;
    }
    ...
    if (a->is_add) {
        ...
        if (p) return VNET_API_ERROR_TUNNEL_EXIST;

        /*if not set explicitly, default to l2 */
        if (a->decap_next_index == ~0) a->decap_next_index = VXLAN_INPUT_NEXT_L2_INPUT;
        if (!vxlan_decap_next_is_valid(vxm, is_ip6, a->decap_next_index)) return VNET_API_ERROR_INVALID_DECAP_NEXT;
...
#undef _
        ...
        if (user_instance == ~0) user_instance = dev_instance;
        ...
        if (a->is_l3)
            t->hw_if_index = vnet_register_interface(vnm, vxlan_device_class.index, dev_instance, vxlan_hw_class.index, dev_instance);
        else {
        ...
        u32 encap_index = !is_ip6 ? vxlan4_encap_node.index : vxlan6_encap_node.index;
        ...
        if (is_ip6) {
            ...
            add_failed = clib_bihash_add_del_24_8(&vxm->vxlan6_tunnel_by_key, &key6, 1 /*add */);
        }
        else {
            ...
            add_failed = clib_bihash_add_del_16_8(&vxm->vxlan4_tunnel_by_key, &key4, 1 /*add */);
        }
        ...
        if (!ip46_address_is_multicast(&t->dst)) {
            ...
            t->fib_entry_index = fib_entry_track(t->encap_fib_index, &tun_dst_pfx, FIB_NODE_TYPE_VXLAN_TUNNEL, dev_instance, &t->sibling_index);
             ...
             */
            if (vtep_addr_ref(&vxm->vtep_table, t->encap_fib_index, &t->dst) == 1) {
                 ...
                 */
                mfib_table_entry_path_update(t->encap_fib_index, &mpfx, MFIB_SOURCE_VXLAN, MFIB_ENTRY_FLAG_NONE, &path);

                path.frp_sw_if_index = a->mcast_sw_if_index;
                path.frp_flags       = FIB_ROUTE_PATH_FLAG_NONE;
                path.frp_mitf_flags  = MFIB_ITF_FLAG_ACCEPT;
                mfei                 = mfib_table_entry_path_update(t->encap_fib_index, &mpfx, MFIB_SOURCE_VXLAN, MFIB_ENTRY_FLAG_NONE, &path);
            ...
            dpo_set(&dpo, DPO_ADJACENCY_MCAST, fib_proto_to_dpo(fp), ep.mcast_adj_index);
        ...
        u32             instance = is_ip6 ? key6.value : vxm->tunnel_index_by_sw_if_index[p->sw_if_index];
        ...
        if (!ip46_address_is_multicast(&t->dst)) {
            if (t->flow_index != ~0) vnet_flow_del(vnm, t->flow_index);

            vtep_addr_unref(&vxm->vtep_table, t->encap_fib_index, &t->src);
            fib_entry_untrack(t->fib_entry_index, t->sibling_index);
        }
        else if (vtep_addr_unref(&vxm->vtep_table, t->encap_fib_index, &t->dst) == 0) {
    ...
    if (sw_if_indexp) *sw_if_indexp = sw_if_index;

    if (a->is_add) {
        /* register udp ports */
        if (!is_ip6 && !udp_is_valid_dst_port(a->src_port, 1)) udp_register_dst_port(vxm->vlib_main, a->src_port, vxlan4_input_node.index, 1);
        if (is_ip6 && !udp_is_valid_dst_port(a->src_port, 0)) udp_register_dst_port(vxm->vlib_main, a->src_port, vxlan6_input_node.index, 0);
    }
...
static uword get_decap_next_for_node(u32 node_index, u32 ipv4_set)
{
    vxlan_main_t *vxm        = &vxlan_main;
    vlib_main_t  *vm         = vxm->vlib_main;
    uword         input_node = (ipv4_set) ? vxlan4_input_node.index : vxlan6_input_node.index;
...
static clib_error_t *vxlan_add_del_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "del")) {
            ...
            ;
        else if (unformat(line_input, "src %U", unformat_ip46_address, &src, IP46_TYPE_ANY)) {
            src_set = 1;
            ...
            is_l3 = 1;
        else if (unformat(line_input, "decap-next %U", unformat_decap_next, &decap_next_index, ipv4_set))
            ;
            ...
            parse_error = clib_error_return(0, "parse error: '%U'", format_unformat_error, line_input);
            break;
        }
    }
    ...
    if (is_l3 && decap_next_index == VXLAN_INPUT_NEXT_L2_INPUT) {
        vlib_node_t *node = vlib_get_node_by_name(vm, (u8 *) (ipv4_set ? "ip4-input" : "ip6-input"));
        decap_next_index  = get_decap_next_for_node(node->index, ipv4_set);
    }

    if (encap_fib_index == ~0) return clib_error_return(0, "nonexistent encap-vrf-id %d", table_id);

    if (src_set == 0) return clib_error_return(0, "tunnel src address not specified");

    if (dst_set == 0) return clib_error_return(0, "tunnel dst address not specified");

    if (grp_set && !ip46_address_is_multicast(&dst)) return clib_error_return(0, "tunnel group address not multicast");

    if (grp_set == 0 && ip46_address_is_multicast(&dst)) return clib_error_return(0, "dst address must be unicast");

    if (grp_set && mcast_sw_if_index == ~0) return clib_error_return(0, "tunnel nonexistent multicast device");

    if (ipv4_set && ipv6_set) return clib_error_return(0, "both IPv4 and IPv6 addresses specified");

    if (ip46_address_cmp(&src, &dst) == 0) return clib_error_return(0, "src and dst addresses are identical");

    if (decap_next_index == ~0) return clib_error_return(0, "next node not found");

    if (vni == 0) return clib_error_return(0, "vni not specified");

    if (vni >> 24) return clib_error_return(0, "vni %d out of range", vni);
...
#undef _
    ...
    switch (rv) {
        case 0:
            if (is_add) vlib_cli_output(vm, "%U\n", format_vnet_sw_if_index_name, vnet_get_main(), tunnel_sw_if_index);
            break;
        ...
        default:
            return clib_error_return(0, "vnet_vxlan_add_del_tunnel returned %d", rv);
    }
...
static clib_error_t *show_vxlan_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (pool_elts(vxm->tunnels) == 0) vlib_cli_output(vm, "No vxlan tunnels configured...");
    ...
    if (raw) {
        vlib_cli_output(vm, "Raw IPv4 Hash Table:\n%U\n", format_bihash_16_8, &vxm->vxlan4_tunnel_by_key, 1 /* verbose */);
        vlib_cli_output(vm, "Raw IPv6 Hash Table:\n%U\n", format_bihash_24_8, &vxm->vxlan6_tunnel_by_key, 1 /* verbose */);
    }
...
void vnet_int_vxlan_bypass_mode(u32 sw_if_index, u8 is_ip6, u8 is_enable)
{
    ...
    if (pool_is_free_index(vxm->vnet_main->interface_main.sw_interfaces, sw_if_index)) return;

    is_enable = !!is_enable;

    if (is_ip6) {
        if (clib_bitmap_get(vxm->bm_ip6_bypass_enabled_by_sw_if, sw_if_index) != is_enable) {
            vnet_feature_enable_disable("ip6-unicast", "ip6-vxlan-bypass", sw_if_index, is_enable, 0, 0);
            vxm->bm_ip6_bypass_enabled_by_sw_if = clib_bitmap_set(vxm->bm_ip6_bypass_enabled_by_sw_if, sw_if_index, is_enable);
        }
    }
    else {
        if (clib_bitmap_get(vxm->bm_ip4_bypass_enabled_by_sw_if, sw_if_index) != is_enable) {
            vnet_feature_enable_disable("ip4-unicast", "ip4-vxlan-bypass", sw_if_index, is_enable, 0, 0);
            vxm->bm_ip4_bypass_enabled_by_sw_if = clib_bitmap_set(vxm->bm_ip4_bypass_enabled_by_sw_if, sw_if_index, is_enable);
        }
    }
}
...
int vnet_vxlan_add_del_rx_flow(u32 hw_if_index, u32 t_index, int is_add)
{
    ...
    if (is_add) {
        if (t->flow_index == ~0) {
            vxlan_main_t *vxm  = &vxlan_main;
            vnet_flow_t   flow = {
                  .actions             = VNET_FLOW_ACTION_REDIRECT_TO_NODE | VNET_FLOW_ACTION_MARK | VNET_FLOW_ACTION_BUFFER_ADVANCE,
...
u32 vnet_vxlan_get_tunnel_index(u32 sw_if_index)
{
    ...
    if (sw_if_index >= vec_len(vxm->tunnel_index_by_sw_if_index)) return ~0;
...
static clib_error_t *vxlan_offload_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "hw %U", unformat_vnet_hw_interface, vnm, &hw_if_index)) continue;
        if (unformat(line_input, "rx %U", unformat_vnet_sw_interface, vnm, &rx_sw_if_index)) continue;
        if (unformat(line_input, "del")) {
            is_add = 0;
            continue;
        }
    ...
    if (rx_sw_if_index == ~0) return clib_error_return(0, "missing rx interface");
    if (hw_if_index == ~0) return clib_error_return(0, "missing hw interface");

    u32 t_index = vnet_vxlan_get_tunnel_index(rx_sw_if_index);
    ;
    if (t_index == ~0) return clib_error_return(0, "%U is not a vxlan tunnel", format_vnet_sw_if_index_name, vnm, rx_sw_if_index);

    vxlan_main_t   *vxm = &vxlan_main;
    vxlan_tunnel_t *t   = pool_elt_at_index(vxm->tunnels, t_index);

    if (!ip46_address_is_ip4(&t->dst)) return clib_error_return(0, "currently only IPV4 tunnels are supported");

    vnet_hw_interface_t *hw_if        = vnet_get_hw_interface(vnm, hw_if_index);
    ip4_main_t          *im           = &ip4_main;
    u32                  rx_fib_index = vec_elt(im->fib_index_by_sw_if_index, hw_if->sw_if_index);

    if (t->encap_fib_index != rx_fib_index) return clib_error_return(0, "interface/tunnel fib mismatch");

    if (vnet_vxlan_add_del_rx_flow(hw_if_index, t_index, is_add)) return clib_error_return(0, "error %s flow", is_add ? "enabling" : "disabling");
...
VLIB_CLI_COMMAND(vxlan_offload_command, static) = {
    ...
    .short_help = "set flow-offload vxlan hw <interface-name> rx <tunnel-name> [del]",
...
clib_error_t *vxlan_init(vlib_main_t *vm)
{
    ...
    vnet_flow_get_range(vxm->vnet_main, "vxlan", 1024 * 1024, &vxm->flow_id_start);

    vxm->bm_ip4_bypass_enabled_by_sw_if = 0;
    vxm->bm_ip6_bypass_enabled_by_sw_if = 0;

    /* initialize the ip6 hash */
    clib_bihash_init_16_8(&vxm->vxlan4_tunnel_by_key, "vxlan4", VXLAN_HASH_NUM_BUCKETS, VXLAN_HASH_MEMORY_SIZE);
    ...
    vxm->mcast_shared = hash_create_mem(0, sizeof(ip46_address_t), sizeof(mcast_shared_t));
