...
{
    if (vec_len(l3xc_db[fproto]) <= sw_if_index) return ~0;
...
static void l3xc_stack(l3xc_t *l3xc)
{
    ...
    fib_path_list_contribute_forwarding(l3xc->l3xc_pl,
                                        (FIB_PROTOCOL_IP4 == l3xc->l3xc_proto ? FIB_FORW_CHAIN_TYPE_UNICAST_IP4 : FIB_FORW_CHAIN_TYPE_UNICAST_IP6),
                                        FIB_PATH_LIST_FWD_FLAG_COLLAPSE,
                                        &via_dpo);

    dpo_stack_from_node((FIB_PROTOCOL_IP4 == l3xc->l3xc_proto ? l3xc_ip4_node.index : l3xc_ip6_node.index), &l3xc->l3xc_dpo, &via_dpo);
    dpo_reset(&via_dpo);
}
...
int l3xc_update(u32 sw_if_index, u8 is_ip6, const fib_route_path_t *rpaths)
{
    ...
    if (INDEX_INVALID == l3xci) {
         ...
         */
        l3xc->l3xc_pl      = fib_path_list_create((FIB_PATH_LIST_FLAG_SHARED | FIB_PATH_LIST_FLAG_NO_URPF), rpaths);
        l3xc->l3xc_sibling = fib_path_list_child_add(l3xc->l3xc_pl, l3xc_fib_node_type, l3xci);
        l3xc_stack(l3xc);
...
static u8 *format_l3xc(u8 *s, va_list *args)
{
    ...
    s = format(s, "l3xc:[%d]: %U", l3xc - l3xc_pool, format_vnet_sw_if_index_name, vnm, l3xc->l3xc_sw_if_index);
    s = format(s, "\n");
    if (FIB_NODE_INDEX_INVALID == l3xc->l3xc_pl) {
