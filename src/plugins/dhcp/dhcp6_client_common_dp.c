...
u32 server_index_get_or_create(u8 *data, u16 len)
{
    ...
    for (i = 0; i < vec_len(ccm->server_ids); i++) {
        se = &ccm->server_ids[i];
        if (se->len == len && 0 == memcmp(se->data, data, len)) return i;
    }
...
static void generate_client_duid(void)
{
    ...
    if (eth_if)
        ...
        clib_warning(
            "Failed to find any Ethernet interface, "
...
static uword dhcpv6_client_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            options_length                = clib_net_to_host_u16(ip0->payload_length) - dhcpv6_ip6_payload_offset - sizeof(*dhcpv60);
                ...
                case DHCPV6_MSG_REPLY:
                    ...
                    while (options_length > 0) {
                        ...
                        if (oo == DHCPV6_OPTION_IA_NA || oo == DHCPV6_OPTION_IA_PD) {
                            ...
                            if (iaid != DHCPV6_CLIENT_IAID) discard_option = 1;
                            if (T1 != 0 && T2 != 0 && T1 > T2) discard_option = 1;
                            if (!discard_option) {
                                report.T1 = T1;
                                report.T2 = T2;
                            }
                            dhcpv6_option_t *inner_option = (void *) ia_header->data;
                            u16 inner_options_length      = clib_net_to_host_u16(option->length) - (sizeof(*ia_header) - sizeof(dhcpv6_option_t));
                            while (inner_options_length > 0) {
                                ...
                                if (discard_option)
                                    ...
                                    address_info->preferred_time       = clib_net_to_host_u32(iaaddr->preferred);
                                    ...
                                    prefix_info->preferred_time      = clib_net_to_host_u32(iaprefix->preferred);
                                ...
                                inner_options_length -= sizeof(*inner_option) + clib_net_to_host_u16(inner_option->length);
                                inner_option = (void *) ((u8 *) inner_option + sizeof(*inner_option) + clib_net_to_host_u16(inner_option->length));
                            }
                        }
                            ...
                            if (client_id_present) {
                                ...
                                client_id_present = 1;
                                if (len != CLIENT_DUID_LENGTH || 0 != memcmp(option->data, client_duid.bin_string, CLIENT_DUID_LENGTH)) {
                                    clib_warning("Unrecognized client DUID inside received DHVPv6 message");
                                    discard = 1;
                                }
                            }
                        }
                            ...
                            if (report.server_index != ~0) {
                                ...
                                if (ol - 2 /* 2 byte DUID type code */ > 128) {
                                    clib_warning("Server DUID (without type code) is longer than 128 octets");
                                    discard = 1;
                                }
                                else {
                        ...
                        option = (void *) ((u8 *) option + sizeof(*option) + clib_net_to_host_u16(option->length));
                    }
...
void dhcp6_clients_enable_disable(u8 enable)
{
    ...
    if (enable) {
        if (client_duid.duid_type == 0) generate_client_duid();
        udp_register_dst_port(vm, UDP_DST_PORT_dhcpv6_to_client, dhcpv6_client_node.index, 0 /* is_ip6 */);
    }
    else
