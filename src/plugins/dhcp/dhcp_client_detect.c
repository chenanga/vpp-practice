...
VLIB_NODE_FN(dhcp_client_detect_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    dhcp_client_port_network_order = clib_net_to_host_u16(UDP_DST_PORT_dhcp_to_client);
    next_index                     = 0;
    extractions                    = 0;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 8 && n_left_to_next >= 4) {
            ...
            /* Prefetch next iteration. */
            {
                ...
                CLIB_PREFETCH(p2->data, sizeof(ip0[0]) + sizeof(udp0[0]), STORE);
                CLIB_PREFETCH(p3->data, sizeof(ip0[0]) + sizeof(udp0[0]), STORE);
                CLIB_PREFETCH(p4->data, sizeof(ip0[0]) + sizeof(udp0[0]), STORE);
                CLIB_PREFETCH(p5->data, sizeof(ip0[0]) + sizeof(udp0[0]), STORE);
            }
    ...
    vlib_node_increment_counter(vm, node->node_index, DHCP_CLIENT_DETECT_ERROR_EXTRACT, extractions);
...
static u8 *format_dhcp_client_detect_trace(u8 *s, va_list *args)
{
    ...
    dhcp_client_detect_trace_t *t   = va_arg(*args, dhcp_client_detect_trace_t *);
