...
static int api_dhcp_proxy_config(vat_main_t *vam)
{
    ...
    if (!(v4_src_address_set && v4_address_set) && !(v6_src_address_set && v6_address_set)) {
    ...
    if (v6_address_set) {
        ...
        clib_memcpy(&mp->dhcp_src_address.un, &v6srcaddress, sizeof(v6address));
    }
    else {
        clib_memcpy(&mp->dhcp_server.un, &v4address, sizeof(v4address));
        clib_memcpy(&mp->dhcp_src_address.un, &v4srcaddress, sizeof(v4address));
    }
...
static void vl_api_dhcp_proxy_details_t_handler(vl_api_dhcp_proxy_details_t *mp)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if (mp->is_ipv6)
            print(vam->ofp, " Server Table-ID %d, Server Address %U", ntohl(s->server_vrf_id), format_ip6_address, s->dhcp_server);
        else
            print(vam->ofp, " Server Table-ID %d, Server Address %U", ntohl(s->server_vrf_id), format_ip4_address, s->dhcp_server);
    }
}

static int api_dhcp_proxy_dump(vat_main_t *vam)
{
    unformat_input_t                  *i = vam->input;
...
static int api_want_dhcp6_reply_events(vat_main_t *vam)
{
    return -1;
}

static int api_want_dhcp6_pd_reply_events(vat_main_t *vam)
{
    return -1;
}

static int api_dhcp6_send_client_message(vat_main_t *vam)
{
    return -1;
}

static int api_dhcp6_pd_send_client_message(vat_main_t *vam)
{
    return -1;
}

static void vl_api_dhcp_client_details_t_handler(vl_api_dhcp_client_details_t *mp)
{
    vat_main_t           *vam = &vat_main;
    ...
    print(vam->ofp, "leased address %U, router address %U", format_ip4_address, &lp->host_address.un, format_ip4_address, &lp->router_address.un);
}

static int api_dhcp_client_dump(vat_main_t *vam)
{
    vl_api_dhcp_plugin_control_ping_t *mp_ping;
...
static int api_dhcp6_duid_ll_set(vat_main_t *vam)
{
    return -1;
}

static int api_dhcp6_clients_enable_disable(vat_main_t *vam)
{
    return -1;
}

static int api_dhcp_plugin_control_ping(vat_main_t *vam)
{
    return -1;
}

static int api_dhcp_plugin_get_version(vat_main_t *vam)
{
    return -1;
}
static int api_dhcp_client_detect_enable_disable(vat_main_t *vam)
{
    return -1;
}

static void vl_api_dhcp_plugin_get_version_reply_t_handler(vl_api_dhcp_plugin_get_version_reply_t *mp)
{
    vat_main_t *vam = dhcp_test_main.vat_main;
    clib_warning("DHCP plugin version: %d.%d", ntohl(mp->major), ntohl(mp->minor));
    vam->result_ready = 1;
}

static void vl_api_dhcp_plugin_control_ping_reply_t_handler(vl_api_dhcp_plugin_control_ping_reply_t *mp)
{
    vat_main_t *vam    = dhcp_test_main.vat_main;
