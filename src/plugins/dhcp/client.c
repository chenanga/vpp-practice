...
static u8 *format_dhcp_client(u8 *s, va_list *va)
{
    ...
    if (0 != c->dscp) s = format(s, " dscp %d", c->dscp);
    ...
    if (verbose) {
        s = format(s,
                   ...
                   c->lease_expires,
                   vlib_time_now(dcm->vlib_main));
        s = format(s, "\n retry-count:%d, next-xmt:%.2f", c->retry_count, c->next_transmit);
...
static void dhcp_client_acquire_address(dhcp_client_main_t *dcm, dhcp_client_t *c)
{
    ...
    if (!c->addresses_installed) {
        ip4_add_del_interface_address(dcm->vlib_main,
                                      c->sw_if_index,
        ...
        if (c->learned.router_address.as_u32) {
            ...
            fib_table_entry_path_add(fib_table_get_index_for_sw_if_index(FIB_PROTOCOL_IP4, c->sw_if_index),
                                     &all_0s,
                                     FIB_SOURCE_DHCP,
                                     FIB_ENTRY_FLAG_NONE,
                                     DPO_PROTO_IP4,
...
static void dhcp_client_release_address(dhcp_client_main_t *dcm, dhcp_client_t *c)
{
    ...
    if (c->addresses_installed) {
        ...
        if (c->installed.router_address.as_u32) {
            ...
            fib_table_entry_path_remove(fib_table_get_index_for_sw_if_index(FIB_PROTOCOL_IP4, c->sw_if_index),
                                        &all_0s,
                                        FIB_SOURCE_DHCP,
                                        DPO_PROTO_IP4,
                                        &nh,
...
static void dhcp_client_proc_callback(uword *client_index)
{
    ...
    vlib_process_signal_event(vm, dhcp_client_process_node.index, EVENT_DHCP_CLIENT_WAKEUP, *client_index);
}
...
static void dhcp_client_addr_callback(u32 *cindex)
{
    ...
    /* disable the feature */
    vnet_feature_enable_disable("ip4-unicast", "ip4-dhcp-client-detect", c->sw_if_index, 0 /* disable */, 0, 0);
     ...
     */
    if (c->event_callback) c->event_callback(c->client_index, c);
...
static void dhcp_client_reset(dhcp_client_main_t *dcm, dhcp_client_t *c)
{
    ...
    if (c->client_detect_feature_enabled == 1) {
        vnet_feature_enable_disable("ip4-unicast", "ip4-dhcp-client-detect", c->sw_if_index, 0, 0, 0);
        c->client_detect_feature_enabled = 0;
    }
    dhcp_client_release_address(dcm, c);
...
int dhcp_client_for_us(u32 bi, vlib_buffer_t *b, ip4_header_t *ip, udp_header_t *udp, dhcp_header_t *dhcp)
{
     ...
     */
    p = hash_get(dcm->client_by_sw_if_index, vnet_buffer(b)->sw_if_index[VLIB_RX]);
    if (p == 0) return 0; /* no */

    c = pool_elt_at_index(dcm->clients, p[0]);

    /* Mixing dhcp relay and dhcp proxy? DGMS... */
    if (c->state == DHCP_BOUND && c->retry_count == 0) return 0;

    /* Packet not for us? Turf it... */
    if (memcmp(dhcp->client_hardware_address, c->client_hardware_address, sizeof(c->client_hardware_address))) {
        vlib_node_increment_counter(vm, dhcp_client_process_node.index, DHCP_STAT_NOT_FOR_US, 1);
        return 0;
    }

    /* parse through the packet, learn what we can */
    if (dhcp->your_ip_address.as_u32) c->learned.leased_address.as_u32 = dhcp->your_ip_address.as_u32;

    c->learned.dhcp_server.as_u32 = dhcp->server_ip_address.as_u32;

    o = (dhcp_option_t *) dhcp->options;

    while (o->option != 0xFF /* end of options */ && (u8 *) o < (b->data + b->current_data + b->current_length)) {
        switch (o->option) {
            ...
            case 51: /* lease time */
            {
                u32 lease_time_in_seconds = clib_host_to_net_u32(o->data_as_u32[0]);
            ...
            {
                u32 lease_renew_time_in_seconds = clib_host_to_net_u32(o->data_as_u32[0]);
            ...
            {
                vec_free(c->domain_server_address);
                vec_validate(c->domain_server_address, o->length / sizeof(ip4_address_t) - 1);
    ...
    switch (c->state) {
        case DHCP_DISCOVER:
            if (dhcp_message_type != DHCP_PACKET_OFFER) {
                vlib_node_increment_counter(vm, dhcp_client_process_node.index, DHCP_STAT_NON_OFFER_DISCOVER, 1);
            ...
            vl_api_rpc_call_main_thread(dhcp_client_proc_callback, (u8 *) &client_id, sizeof(uword));
            break;

        case DHCP_BOUND:
        case DHCP_REQUEST:
            if (dhcp_message_type == DHCP_PACKET_NAK) {
                vlib_node_increment_counter(vm, dhcp_client_process_node.index, DHCP_STAT_NAK, 1);
            ...
            if (dhcp_message_type != DHCP_PACKET_ACK && dhcp_message_type != DHCP_PACKET_OFFER) {
                vlib_node_increment_counter(vm, dhcp_client_process_node.index, DHCP_STAT_NON_OFFER_DISCOVER, 1);
                clib_warning("sw_if_index %d state %U message type %d", c->sw_if_index, format_dhcp_client_state, c->state, dhcp_message_type);
                c->next_transmit = now + 5.0;
                break;
            }
            /* OK, we own the address (etc), add to the routing table(s) */
            {
                ...
                vl_api_force_rpc_call_main_thread(dhcp_client_addr_callback, (u8 *) &cindex, sizeof(u32));
            }
            ...
            vlib_node_increment_counter(vm, dhcp_client_process_node.index, DHCP_STAT_BOUND, 1);
            break;
...
static void send_dhcp_pkt(dhcp_client_main_t *dcm, dhcp_client_t *c, dhcp_packet_type_t type, int is_broadcast)
{
    ...
    DHCP_INFO("send: type:%U bcast:%d %U", format_dhcp_packet_type, type, is_broadcast, format_dhcp_client, dcm, c, 1 /* verbose */);

    /* Interface(s) down? */
    if ((hw->flags & VNET_HW_INTERFACE_FLAG_LINK_UP) == 0) return;
    if ((sup_sw->flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP) == 0) return;
    if ((sw->flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP) == 0) return;
    ...
    clib_memcpy(dhcp->client_hardware_address, vnet_sw_interface_get_hw_address(vnm, c->sw_if_index), 6);

    /* And remember it for rx-packet-for-us checking */
    clib_memcpy(c->client_hardware_address, dhcp->client_hardware_address, sizeof(c->client_hardware_address));

    /* Lease renewal, set up client_ip_address */
    if (is_broadcast == 0) dhcp->client_ip_address.as_u32 = c->learned.leased_address.as_u32;
    ...
    dhcp->flags                   = clib_host_to_net_u16(is_broadcast && c->set_broadcast_flag ? DHCP_FLAG_BROADCAST : 0);
    ...
    if (vec_len(c->client_identifier)) {
        ...
        o->data[0] = 0;
        clib_memcpy(o->data + 1, c->client_identifier, vec_len(c->client_identifier));
        o = (dhcp_option_t *) (((uword) o) + (o->length + 2));
    }
...
#define _(a, b)                        \
    case DHCP_PACKET_##a: {            \
        counter_index = DHCP_STAT_##a; \
        break;                         \
    }
        foreach_dhcp_sent_packet_stat
#undef _
            default : counter_index = DHCP_STAT_UNKNOWN;
        break;
    }

    vlib_node_increment_counter(vm, dhcp_client_process_node.index, counter_index, 1);
}
...
static int dhcp_discover_state(dhcp_client_main_t *dcm, dhcp_client_t *c, f64 now)
{
    ...
    if (c->client_detect_feature_enabled == 0) {
        vlib_worker_thread_barrier_sync(dcm->vlib_main);
        vnet_feature_enable_disable("ip4-unicast", "ip4-dhcp-client-detect", c->sw_if_index, 1 /* enable */, 0, 0);
        vlib_worker_thread_barrier_release(dcm->vlib_main);
...
static int dhcp_request_state(dhcp_client_main_t *dcm, dhcp_client_t *c, f64 now)
{
     ...
     */
    DHCP_INFO("enter request: %U", format_dhcp_client, dcm, c, 1 /*verbose */);
...
static int dhcp_bound_state(dhcp_client_main_t *dcm, dhcp_client_t *c, f64 now)
{
    ...
    if (now > c->lease_expires) {
        DHCP_INFO("lease expired: %U", format_dhcp_client, dcm, c, 1 /*verbose */);
...
static f64 dhcp_client_sm(f64 now, f64 timeout, uword pool_index)
{
    ...
    if (pool_is_free_index(dcm->clients, pool_index)) return timeout;

    c = pool_elt_at_index(dcm->clients, pool_index);

    /* Time for us to do something with this client? */
    if (now < c->next_transmit) return c->next_transmit;
...
again:
    switch (c->state) {
        case DHCP_DISCOVER: /* send a discover */
            if (dhcp_discover_state(dcm, c, now)) goto again;
            break;

        case DHCP_REQUEST: /* send a request */
            if (dhcp_request_state(dcm, c, now)) goto again;
            break;

        case DHCP_BOUND: /* bound, renew needed? */
            if (dhcp_bound_state(dcm, c, now)) goto again;
            break;

        default:
            clib_warning("dhcp client %d bogus state %d", c - dcm->clients, c->state);
            break;
    }
            ...
            case ~0:
                if (pool_elts(dcm->clients)) {
                    ...
                    pool_foreach (c, dcm->clients) {
                        this_next_expire_time = dhcp_client_sm(now, timeout, (uword) (c - dcm->clients));
                        next_expire_time      = this_next_expire_time < next_expire_time ? this_next_expire_time : next_expire_time;
                    }
...
static clib_error_t *show_dhcp_client_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "intfc %U", unformat_vnet_sw_interface, dcm->vnet_main, &sw_if_index))
            ;
    ...
    if (sw_if_index != ~0) {
        ...
        if (p == 0) return clib_error_return(0, "dhcp client not configured");
        c = pool_elt_at_index(dcm->clients, p[0]);
...
int dhcp_client_add_del(dhcp_client_add_del_args_t *a)
{
    ...
    if ((p && a->is_add) || (!p && a->is_add == 0)) return VNET_API_ERROR_INVALID_VALUE;
    ...
    if (a->is_add) {
        ...
        c->dscp               = a->dscp;
        c->ai_bcast           = adj_nbr_add_or_lock(FIB_PROTOCOL_IP4, VNET_LINK_IP4, &ADJ_BCAST_ADDR, c->sw_if_index);
        ...
        vlib_process_signal_event(vm, dhcp_client_process_node.index, EVENT_DHCP_CLIENT_WAKEUP, c - dcm->clients);
...
static clib_error_t *dhcp_client_set_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (sw_if_index_set == 0) return clib_error_return(0, "interface not specified");
    ...
    switch (rv) {
        ...
        case VNET_API_ERROR_INVALID_VALUE:
            ...
            vec_free(a->option_55_data);
            if (is_add)
                return clib_error_return(0, "dhcp client already enabled on %U", format_vnet_sw_if_index_name, dcm->vnet_main, sw_if_index);
            else
                return clib_error_return(0, "dhcp client not enabled on %U", format_vnet_sw_if_index_name, dcm->vnet_main, sw_if_index);
            break;
...
static clib_error_t *dhcp_client_init(vlib_main_t *vm)
{
    ...
    /* Should never happen... */
    if (ip4_lookup_node == 0) return clib_error_return(0, "ip4-lookup node not found");
