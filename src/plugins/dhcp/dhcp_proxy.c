...
void dhcp_vss_walk(fib_protocol_t proto, dhcp_vss_walk_fn_t fn, void *ctx)
{
    ...
    vec_foreach_index (i, dpm->vss_index_by_rx_fib_index[proto]) {
        ...
        if (FIB_PROTOCOL_IP4 == proto) {
            ...
            if (!fn(vss, fib->ft_table_id, ctx)) break;
        }
        else {
            mfib = mfib_table_get(i, proto);

            if (!fn(vss, mfib->mft_table_id, ctx)) break;
        }
    }
}
...
static u32 dhcp_proxy_server_find(dhcp_proxy_t *proxy, fib_protocol_t proto, ip46_address_t *addr, u32 server_table_id)
{
    ...
    vec_foreach_index (ii, proxy->dhcp_servers) {
        ...
        if (ip46_address_is_equal(&server->dhcp_server, addr) && (server->server_fib_index == fib_index)) {
            return (ii);
        }
    }
    return (~0);
...
int dhcp_proxy_server_add(fib_protocol_t proto, ip46_address_t *addr, ip46_address_t *src_address, u32 rx_fib_index, u32 server_table_id)
{
    ...
    if (NULL == proxy) {
        vec_validate_init_empty(dpm->dhcp_server_index_by_rx_fib_index[proto], rx_fib_index, ~0);

        pool_get(dpm->dhcp_servers[proto], proxy);
        clib_memset(proxy, 0, sizeof(*proxy));
        new = 1;

        dpm->dhcp_server_index_by_rx_fib_index[proto][rx_fib_index] = proxy - dpm->dhcp_servers[proto];
    ...
    dhcp_server_t server = {
        .dhcp_server      = *addr,
        .server_fib_index = fib_table_find_or_create_and_lock(proto, server_table_id, FIB_SOURCE_DHCP),
    };
...
int dhcp_vss_show_walk(dhcp_vss_t *vss, u32 rx_table_id, void *ctx)
{
    ...
    if (vss->vss_type == VSS_TYPE_VPN_ID) {
        ...
        vlib_cli_output(vm, " fib_table: %d  oui: %d vpn_index: %d", rx_table_id, oui, fib_id);
    }
    else if (vss->vss_type == VSS_TYPE_ASCII)
        vlib_cli_output(vm, " fib_table: %d  vpn_id: %s", rx_table_id, vss->vpn_ascii_id);
    else
...
int dhcp_proxy_set_vss(fib_protocol_t proto, u32 tbl_id, u8 vss_type, u8 *vpn_ascii_id, u32 oui, u32 vpn_index, u8 is_del)
{
    ...
    if (proto == FIB_PROTOCOL_IP4)
        rx_fib_index = fib_table_find_or_create_and_lock(proto, tbl_id, FIB_SOURCE_DHCP);
    else
        rx_fib_index = mfib_table_find_or_create_and_lock(proto, tbl_id, MFIB_SOURCE_DHCP);
    ...
    if (NULL != v) {
        ...
        if (is_del)
            ...
            /* create a new entry */
            vec_validate_init_empty(dm->vss_index_by_rx_fib_index[proto], rx_fib_index, ~0);

            /* hold a lock on the table whilst the VSS info exist */
            pool_get(dm->vss[proto], v);
            update_vss(v, vss_type, vpn_ascii_id, oui, vpn_index);
            dm->vss_index_by_rx_fib_index[proto][rx_fib_index] = v - dm->vss[proto];
