...
static void send_client_message_start_stop(u32 sw_if_index, u32 server_index, u8 msg_type, address_info_t *address_list, u8 start)
{
    ...
    params.T1 = 0;
    params.T2 = 0;
    if (vec_len(address_list) != 0) vec_validate(addresses, vec_len(address_list) - 1);
...
{
    if (address1->as_u64[0] != address2->as_u64[0]) return 0;
...
static clib_error_t *dhcp6_reply_event_handler(vl_api_dhcp6_reply_event_t *mp)
{
    ...
    if (mp->msg_type == DHCPV6_MSG_API_ADVERTISE && client_state->server_index == ~0) {
        ...
        if (inner_status_code == DHCPV6_STATUS_NOADDRS_AVAIL) {
            clib_warning("Advertise message arrived with NoAddrsAvail status code");
            return 0;
        }

        if (n_addresses > 0) vec_validate(address_list, n_addresses - 1);
        ...
        send_client_message_start_stop(sw_if_index, server_index, DHCPV6_MSG_REQUEST, address_list, 1);
    ...
    for (i = 0; i < n_addresses; i++) {
        ...
        if (ip6_address_is_link_local_unicast(address)) continue;
    ...
    address_pool_foreach_out:
        ...
        if (address_already_present) {
            ...
            address_info->due_time += (preferred_time > 0) ? preferred_time : valid_time;

            if (address_info->due_time > rm->max_valid_due_time) rm->max_valid_due_time = address_info->due_time;
            continue;
        }
        ...
        error = ip6_add_del_interface_address(vm, sw_if_index, &address_info->address, 64, 0);
        if (error) clib_warning("Failed to add interface address");
    }
        ...
        do {
            ...
            pool_foreach (address_info, rm->address_pool) {
                if (address_info->due_time > current_time) {
                    if (address_info->due_time < due_time) due_time = address_info->due_time;
                }
                else {
                    u32 sw_if_index = address_info->sw_if_index;
                    error           = ip6_add_del_interface_address(vm, sw_if_index, &address_info->address, 64, 1);
                    if (error) clib_warning("Failed to delete interface address");
                ...
                if (cs->enabled && cs->server_index != ~0) {
                    if (cs->T2_due_time > current_time) {
                        ...
                        if (cs->T1_due_time > current_time) {
                            ...
                            cs->rebinding = 1;
                            send_client_message_start_stop(i, cs->server_index, DHCPV6_MSG_RENEW, address_list, 1);
                            vec_free(address_list);
                        }
                    }
                    else {
                        ...
                        address_list  = create_address_list(i);
                        cs->rebinding = 1;
                        send_client_message_start_stop(i, ~0, DHCPV6_MSG_REBIND, address_list, 1);
                        vec_free(address_list);
                    }
                }
            }
...
static void interrupt_process(void)
{
    ...
    vlib_process_signal_event(vm, dhcp6_client_cp_process_node.index, RD_CP_EVENT_INTERRUPT, 0);
}
...
static void disable_process(void)
{
    ...
    vlib_process_signal_event(vm, dhcp6_client_cp_process_node.index, RD_CP_EVENT_DISABLE, 0);
}
...
static clib_error_t *dhcp6_addresses_show_command_function(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    pool_foreach (address_info, dm->address_pool) {
        vlib_cli_output(vm,
                        ...
                        address_info->preferred_lt,
                        address_info->valid_lt,
                        address_info->due_time - current_time);
    }
...
int dhcp6_client_enable_disable(u32 sw_if_index, u8 enable)
{
    ...
    if (enable) client_state->enabled = 1;
    client_state->server_index = ~0;
    ...
    if (!old_enabled && enable) {
        ...
        pool_foreach (address_info, rm->address_pool) {
            if (address_info->sw_if_index == sw_if_index) {
                ASSERT(sw_if_index < vec_len(rm->client_state_by_sw_if_index) && rm->client_state_by_sw_if_index[sw_if_index].enabled);
                client_state_t *client_state = &rm->client_state_by_sw_if_index[sw_if_index];
                send_client_message_start_stop(sw_if_index, client_state->server_index, DHCPV6_MSG_RELEASE, address_info, 1);
                error = ip6_add_del_interface_address(vm, sw_if_index, &address_info->address, 64, 1);
                if (error) clib_warning("Failed to delete interface address");
                pool_put(rm->address_pool, address_info);
            }
    ...
    if (!enable) client_state->enabled = 0;
...
static clib_error_t *dhcp6_client_enable_disable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (sw_if_index != ~0) {
        if (dhcp6_client_enable_disable(sw_if_index, enable) != 0) error = clib_error_return(0, "Invalid sw_if_index");
    }
    else
