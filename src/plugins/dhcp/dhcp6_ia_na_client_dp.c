...
static vlib_buffer_t *create_buffer_for_client_message(vlib_main_t *vm, u32 sw_if_index, dhcp6_ia_na_client_state_t *client_state, u32 type)
{
    ...
    if (type == DHCPV6_MSG_SOLICIT || type == DHCPV6_MSG_REQUEST || type == DHCPV6_MSG_RENEW || type == DHCPV6_MSG_REBIND
        ...
        ia_hdr->opt.length = clib_host_to_net_u16(sizeof(*ia_hdr) + n_addresses * sizeof(*opt_addr) - sizeof(ia_hdr->opt));
...
static inline u8 check_send_client_message(vlib_main_t *vm, dhcp6_ia_na_client_state_t *client_state, f64 current_time, f64 *due_time)
{
    vlib_buffer_t *p0;
    vlib_frame_t  *f;
    u32           *to_next;
    ...
    *elapsed_field     = clib_host_to_net_u16((u16) ((now - client_state->transaction_start) * 100));
    ...
    if (params->mrc != 0 && --client_state->n_left == 0)
        ...
        client_state->sleep_interval = (2 + random_f64_from_to(-0.1, 0.1)) * client_state->sleep_interval;
        if (client_state->sleep_interval > params->mrt) client_state->sleep_interval = (1 + random_f64_from_to(-0.1, 0.1)) * params->mrt;
    ...
    while (true) {
        ...
        do {
            ...
            for (i = 0; i < vec_len(cm->client_state_by_sw_if_index); i++) {
                ...
                if (check_send_client_message(vm, client_state, current_time, &dt) && (dt < due_time)) due_time = dt;
            }
            current_time = vlib_time_now(vm);
