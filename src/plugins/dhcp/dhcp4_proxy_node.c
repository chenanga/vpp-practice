...
#define VPP_DHCP_OPTION82_SIZE      (VPP_DHCP_OPTION82_SUB1_SIZE + VPP_DHCP_OPTION82_SUB5_SIZE + VPP_DHCP_OPTION82_VSS_SIZE + 3)
...
static u8 *format_dhcp_proxy_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "  %U", format_dhcp_header, t->packet_data, sizeof(t->packet_data));
...
static u8 *format_dhcp_proxy_header_with_length(u8 *s, va_list *args)
{
    ...
    if (max_header_bytes != 0 && header_bytes > max_header_bytes) return format(s, "dhcp header truncated");
...
static uword dhcp_proxy_to_server_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if ((b0->flags & VLIB_BUFFER_NEXT_PRESENT) != 0 || space_left < VPP_DHCP_OPTION82_SIZE) {
                error0 = DHCP_PROXY_ERROR_PKT_TOO_BIG;
                next0  = DHCP_PROXY_TO_SERVER_INPUT_NEXT_DROP;
                pkts_too_big++;
                goto do_trace;
            ...
            sum0                    = ip_csum_update(sum0, old0, new0, ip4_header_t /* structure */, src_address /* changed member */);
            ip0->checksum           = ip_csum_fold(sum0);
            ...
            if (o->option == DHCP_PACKET_OPTION_END && o <= end) {
                ...
                original_sw_if_index = sw_if_index = vnet_buffer(b0)->sw_if_index[VLIB_RX];
                ...
                *o_addr     = ia0->as_u32;
                ...
                vlib_node_increment_counter(vm, dhcp_proxy_to_server_node.index, DHCP_PROXY_ERROR_OPTION_82_ERROR, 1);
            }
            ...
            if (is_discover && vec_len(proxy->dhcp_servers) > 1) {
                ...
                for (ii = 1; ii < vec_len(proxy->dhcp_servers); ii++) {
                    ...
                    if (c0 == NULL) {
                        vlib_node_increment_counter(vm, dhcp_proxy_to_server_node.index, DHCP_PROXY_ERROR_ALLOC_FAIL, 1);
                        continue;
                    }
                    ...
                    if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                        ...
                        if (next0 == DHCP_PROXY_TO_SERVER_INPUT_NEXT_LOOKUP) tr->trace_ip4_address.as_u32 = server->dhcp_server.ip4.as_u32;

                        clib_memcpy_fast(tr->packet_data, h0, sizeof(tr->packet_data));
                    }
        ...
        do_trace:
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                dhcp_proxy_trace_t *tr   = vlib_add_trace(vm, node, b0, sizeof(*tr));
                tr->which                = 0; /* to server */
                tr->error                = error0;
                tr->original_sw_if_index = original_sw_if_index;
                tr->sw_if_index          = sw_if_index;
                if (next0 == DHCP_PROXY_TO_SERVER_INPUT_NEXT_LOOKUP) tr->trace_ip4_address.as_u32 = proxy->dhcp_servers[0].dhcp_server.ip4.as_u32;
                clib_memcpy_fast(tr->packet_data, h0, sizeof(tr->packet_data));
            }
    ...
    vlib_node_increment_counter(vm, dhcp_proxy_to_server_node.index, DHCP_PROXY_ERROR_RELAY_TO_CLIENT, pkts_to_client);
    vlib_node_increment_counter(vm, dhcp_proxy_to_server_node.index, DHCP_PROXY_ERROR_RELAY_TO_SERVER, pkts_to_server);
    vlib_node_increment_counter(vm, dhcp_proxy_to_server_node.index, DHCP_PROXY_ERROR_NO_SERVER, pkts_no_server);
    vlib_node_increment_counter(vm, dhcp_proxy_to_server_node.index, DHCP_PROXY_ERROR_NO_INTERFACE_ADDRESS, pkts_no_interface_address);
    vlib_node_increment_counter(vm, dhcp_proxy_to_server_node.index, DHCP_PROXY_ERROR_PKT_TOO_BIG, pkts_too_big);
...
static uword dhcp_proxy_to_client_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            dhcp_option_t *o = h0->options, *end = (void *) vlib_buffer_get_tail(b0);
            ...
            while (o->option != DHCP_PACKET_OPTION_END && o < end) {
                if (o->option == 82) {
                    ...
                    dhcp_option_t *subend    = (dhcp_option_t *) (o->data + o->length);
                    while (sub->option != DHCP_PACKET_OPTION_END && sub < subend) {
                        /* If this is one of ours, it will have
                           total length 12, circuit-id suboption type,
                           and the sw_if_index */
                        if (sub->option == 1 && sub->length == 4) {
                            sw_if_index = ((sub->data[0] << 24) | (sub->data[1] << 16) | (sub->data[2] << 8) | (sub->data[3]));
                        }
                        ...
                        else if (sub->option == 151 && sub->length == 7 && sub->data[0] == 1)
                            vss_exist = 1;
                    ...
                    if (vss_ctrl && vss_exist)
                        vlib_node_increment_counter(vm, dhcp_proxy_to_client_node.index, DHCP_PROXY_ERROR_OPTION_82_VSS_NOT_PROCESSED, 1);
                }
                o = (dhcp_option_t *) (o->data + o->length);
            }
            ...
            drop_packet:
                vlib_node_increment_counter(vm, dhcp_proxy_to_client_node.index, error0, 1);
        ...
        server_found:
            ...
            sum0                    = ip_csum_update(sum0, old0, new0, ip4_header_t /* structure */, dst_address /* offset of changed member */);
            ip0->checksum           = ip_csum_fold(sum0);
            ...
            new0                    = ia0->as_u32;
            ip0->src_address.as_u32 = new0;
            sum0                    = ip_csum_update(sum0, old0, new0, ip4_header_t /* structure */, src_address /* offset of changed member */);
            ip0->checksum           = ip_csum_fold(sum0);
...
void dhcp_maybe_register_udp_ports(dhcp_port_reg_flags_t ports)
{
    ...
    if (!port_regs_diff) return;

    if ((port_regs_diff & DHCP_PORT_REG_CLIENT) & ports)
        udp_register_dst_port(vm, UDP_DST_PORT_dhcp_to_client, dhcp_proxy_to_client_node.index, 1 /* is_ip4 */);

    if ((port_regs_diff & DHCP_PORT_REG_SERVER) & ports)
        udp_register_dst_port(vm, UDP_DST_PORT_dhcp_to_server, dhcp_proxy_to_server_node.index, 1 /* is_ip4 */);
...
int dhcp4_proxy_set_server(ip46_address_t *addr, ip46_address_t *src_addr, u32 rx_table_id, u32 server_table_id, int is_del)
{
    ...
    if (is_del) {
        if (dhcp_proxy_server_del(FIB_PROTOCOL_IP4, rx_fib_index, addr, server_table_id)) {
            fib_table_entry_special_remove(rx_fib_index, &all_1s, FIB_SOURCE_DHCP);
            fib_table_unlock(rx_fib_index, FIB_PROTOCOL_IP4, FIB_SOURCE_DHCP);
        }
    }
    else {
        if (dhcp_proxy_server_add(FIB_PROTOCOL_IP4, addr, src_addr, rx_fib_index, server_table_id)) {
            fib_table_entry_special_add(rx_fib_index, &all_1s, FIB_SOURCE_DHCP, FIB_ENTRY_FLAG_LOCAL);
...
static clib_error_t *dhcp4_proxy_set_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "server %U", unformat_ip4_address, &server_addr.ip4))
            set_server = 1;
        else if (unformat(input, "server-fib-id %d", &server_table_id))
            ;
        else if (unformat(input, "rx-fib-id %d", &rx_table_id))
            ;
        else if (unformat(input, "src-address %U", unformat_ip4_address, &src_addr.ip4))
            set_src = 1;
    ...
    if (is_del || (set_server && set_src)) {
        ...
        rv = dhcp4_proxy_set_server(&server_addr, &src_addr, rx_table_id, server_table_id, is_del);
        switch (rv) {
            case 0:
                return 0;
...
static u8 *format_dhcp4_proxy_server(u8 *s, va_list *args)
{
    ...
    s = format(s, "%=14u%=16U", rx_fib->hash.table_id, format_ip46_address, &proxy->dhcp_src_address, IP46_TYPE_ANY);

    vec_foreach (server, proxy->dhcp_servers) {
        server_fib = ip4_fib_get(server->server_fib_index);
        s          = format(s, "%u,%U  ", server_fib->hash.table_id, format_ip46_address, &server->dhcp_server, IP46_TYPE_ANY);
    }
    return s;
}
...
static clib_error_t *dhcp_option_82_vss_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    int rv = dhcp_proxy_set_vss(FIB_PROTOCOL_IP4, tbl_id, vss_type, vpn_ascii_id, oui, fib_id, is_del);
    switch (rv) {
        case 0:
            return 0;
        case VNET_API_ERROR_NO_SUCH_ENTRY:
            return clib_error_return(0, "option 82 vss for table %d not found in in pool.", tbl_id);
        default:
            return clib_error_return(0, "BUG: rv %d", rv);
    }
}
...
static clib_error_t *dhcp_option_82_address_show_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "%U", unformat_vnet_sw_interface, vnm, &sw_if_index0)) {
            ...
            ia0         = ip4_interface_first_address(&ip4_main, sw_if_index, 0);
            if (ia0) {
                vlib_cli_output(vm, "%=20s%=20s", "interface", "source IP address");

                vlib_cli_output(vm, "%=20U%=20U", format_vnet_sw_if_index_name, vnm, sw_if_index0, format_ip4_address, ia0);
            }
            else
                vlib_cli_output(vm, "%=34s %=20U", "No IPv4 address configured on", format_vnet_sw_if_index_name, vnm, sw_if_index);
        }
        else
            break;
    }
