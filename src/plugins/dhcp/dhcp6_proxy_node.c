...
static u8 *format_dhcpv6_proxy_trace(u8 *s, va_list *args)
{
    ...
    if (t->which == 0)
        s = format(s, "DHCPV6 proxy: sent to server %U", format_ip6_address, &t->packet_data, sizeof(ip6_address_t));
    else
        s = format(s, "DHCPV6 proxy: sent to client from %U", format_ip6_address, &t->packet_data, sizeof(ip6_address_t));
    if (t->error != (u32) ~0) s = format(s, " error: %s\n", dhcpv6_proxy_error_strings[t->error]);
...
static u8 *format_dhcpv6_proxy_header_with_length(u8 *s, va_list *args)
{
    ...
    if (max_header_bytes != 0 && header_bytes > max_header_bytes) return format(s, "dhcpv6 header truncated");
...
static uword dhcpv6_proxy_to_server_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (h0->msg_type == DHCPV6_MSG_RELAY_FORW && ((ip0->src_address.as_u8[0] & 0xe0) == 0x20 || (ip0->src_address.as_u8[0] & 0xfe) == 0xfc)) {
            ...
            if (swif->flags & VNET_SW_INTERFACE_FLAG_UNNUMBERED) sw_if_index = swif->unnumbered_sw_if_index;
        ...
        link_address_set:
            ...
            if ((b0->current_data + b0->current_length + sizeof(*id1) + sizeof(*vss1) + sizeof(*cmac)) > vlib_buffer_get_default_data_size(vm)) {
            ...
            if (h0->msg_type != DHCPV6_MSG_RELAY_FORW) {
                ...
                cmac->opt.length = clib_host_to_net_u16(sizeof(*cmac) - sizeof(cmac->opt));
            ...
            u1->length = clib_host_to_net_u16(clib_net_to_host_u16(fwd_opt->length) + sizeof(*r1) + sizeof(*fwd_opt) + sizeof(*u1) + sizeof(*id1)
                                              + u1->length);
            ...
            src = ((server->dhcp_server.ip6.as_u64[0] || server->dhcp_server.ip6.as_u64[1]) ? &server->dhcp_server.ip6 : &all_dhcpv6_server_address);
            copy_ip6_address(&ip1->dst_address, src);

            ia0 = ip6_interface_first_global_or_site_address(&ip6_main, vnet_buffer(b0)->sw_if_index[VLIB_RX]);

            src = (proxy->dhcp_src_address.ip6.as_u64[0] || proxy->dhcp_src_address.ip6.as_u64[1]) ? &proxy->dhcp_src_address.ip6 : ia0;
            if (ia0 == 0) {
            ...
            u1->checksum = ip6_tcp_udp_icmp_compute_checksum(vm, b0, ip1, &bogus_length);
            ...
            if (is_solicit && vec_len(proxy->dhcp_servers) > 1) {
                ...
                for (ii = 1; ii < vec_len(proxy->dhcp_servers); ii++) {
                    ...
                    if (c0 == NULL) {
                        vlib_node_increment_counter(vm, dhcpv6_proxy_to_server_node.index, DHCPV6_PROXY_ERROR_ALLOC_FAIL, 1);
                        continue;
                    }
                    ...
                    src = ((server->dhcp_server.ip6.as_u64[0] || server->dhcp_server.ip6.as_u64[1]) ? &server->dhcp_server.ip6
                    ...
                    vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, ci0, next0);
        ...
        do_trace:
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                dhcpv6_proxy_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof(*tr));
                tr->which                = 0; /* to server */
                tr->error                = error0;
                tr->original_sw_if_index = rx_sw_if_index;
                tr->sw_if_index          = sw_if_index;
    ...
    vlib_node_increment_counter(vm, dhcpv6_proxy_to_server_node.index, DHCPV6_PROXY_ERROR_RELAY_TO_CLIENT, pkts_to_client);
    vlib_node_increment_counter(vm, dhcpv6_proxy_to_server_node.index, DHCPV6_PROXY_ERROR_RELAY_TO_SERVER, pkts_to_server);
    vlib_node_increment_counter(vm, dhcpv6_proxy_to_server_node.index, DHCPV6_PROXY_ERROR_NO_INTERFACE_ADDRESS, pkts_no_interface_address);
    vlib_node_increment_counter(vm, dhcpv6_proxy_to_server_node.index, DHCPV6_PROXY_ERROR_WRONG_MESSAGE_TYPE, pkts_wrong_msg_type);
    vlib_node_increment_counter(vm, dhcpv6_proxy_to_server_node.index, DHCPV6_PROXY_ERROR_NO_SRC_ADDRESS, pkts_no_src_address);
    vlib_node_increment_counter(vm, dhcpv6_proxy_to_server_node.index, DHCPV6_PROXY_ERROR_PKT_TOO_BIG, pkts_too_big);
...
static uword dhcpv6_proxy_to_client_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    while (n_left_from > 0) {
        ...
        drop_packet:
            vlib_node_increment_counter(vm, dhcpv6_proxy_to_client_node.index, error0, 1);
        ...
        while ((u8 *) o < (b0->data + b0->current_data + b0->current_length)) {
            if (DHCPV6_OPTION_INTERFACE_ID == clib_net_to_host_u16(o->option)) {
                ...
                if (clib_net_to_host_u16(o->length) == sizeof(sw_if_index)) sw_if_index = clib_net_to_host_u32(((dhcpv6_int_id_t *) o)->int_idx);
            ...
            if ((relay_msg_opt_flag == 1) && (interface_opt_flag == 1)) break;
        ...
        server_fib_idx = im->fib_index_by_sw_if_index[vnet_buffer(b0)->sw_if_index[VLIB_RX]];

        vec_foreach (server, proxy->dhcp_servers) {
            if (server_fib_idx == server->server_fib_index && ip0->src_address.as_u64[0] == server->dhcp_server.ip6.as_u64[0]
    ...
    server_found:
        vnet_buffer(b0)->sw_if_index[VLIB_TX] = original_sw_if_index = sw_if_index;
        ...
        ip1->ip_version_traffic_class_and_flow_label = ip0->ip_version_traffic_class_and_flow_label & 0x00000fff;
        ip1->payload_length                          = u1->length;
        ip1->protocol                                = PROTO_UDP;
        ip1->hop_limit                               = HOP_COUNT_LIMIT;
        ...
        clib_memcpy(mac0->src_address, &ei0->address, sizeof(mac0->src_address));
        clib_memset(&mac0->dst_address, 0xff, sizeof(mac0->dst_address));

        if (si0->type == VNET_SW_INTERFACE_TYPE_SUB && outer_vlan != (u32) ~0) {
            mac0->type    = (si0->sub.eth.flags.dot1ad == 1) ? clib_net_to_host_u16(0x88a8) : clib_net_to_host_u16(0x8100);
    ...
    do_trace:
        if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
            ...
            if (ia0) copy_ip6_address((ip6_address_t *) tr->packet_data, ia0);
            tr->error                = error0;
...
int dhcp6_proxy_set_server(ip46_address_t *addr, ip46_address_t *src_addr, u32 rx_table_id, u32 server_table_id, int is_del)
{
    ...
    const mfib_prefix_t all_dhcp_servers = {.fp_len      = 128,
                                            .fp_proto    = FIB_PROTOCOL_IP6,
                                            .fp_grp_addr = {
                                                .ip6 = all_dhcpv6_server_relay_agent_address,
                                            }};

    if (ip46_address_is_zero(addr)) return VNET_API_ERROR_INVALID_DST_ADDRESS;

    if (ip46_address_is_zero(src_addr)) return VNET_API_ERROR_INVALID_SRC_ADDRESS;

    rx_fib_index = mfib_table_find_or_create_and_lock(FIB_PROTOCOL_IP6, rx_table_id, MFIB_SOURCE_DHCP);

    if (is_del) {
        if (dhcp_proxy_server_del(FIB_PROTOCOL_IP6, rx_fib_index, addr, server_table_id)) {
            mfib_table_entry_delete(rx_fib_index, &all_dhcp_servers, MFIB_SOURCE_DHCP);
            ...
            udp_unregister_dst_port(vm, UDP_DST_PORT_dhcpv6_to_server, 0 /* is_ip6 */);
        }
    }
    else {
        ...
        if (dhcp_proxy_server_add(FIB_PROTOCOL_IP6, addr, src_addr, rx_fib_index, server_table_id)) {
            mfib_table_entry_path_update(rx_fib_index, &all_dhcp_servers, MFIB_SOURCE_DHCP, MFIB_ENTRY_FLAG_NONE, &path_for_us);
            /*
             ...
             */
            mfib_table_entry_update(rx_fib_index, &all_dhcp_servers, MFIB_SOURCE_DHCP, MFIB_RPF_ID_NONE, MFIB_ENTRY_FLAG_ACCEPT_ALL_ITF);
            mfib_table_lock(rx_fib_index, FIB_PROTOCOL_IP6, MFIB_SOURCE_DHCP);

            udp_register_dst_port(vm, UDP_DST_PORT_dhcpv6_to_client, dhcpv6_proxy_to_client_node.index, 0 /* is_ip6 */);
            udp_register_dst_port(vm, UDP_DST_PORT_dhcpv6_to_server, dhcpv6_proxy_to_server_node.index, 0 /* is_ip6 */);
        }
    }
...
static clib_error_t *dhcpv6_proxy_set_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "server %U", unformat_ip6_address, &addr.ip6))
            set_server = 1;
        else if (unformat(input, "src-address %U", unformat_ip6_address, &src_addr.ip6))
            set_src_address = 1;
    ...
    if (is_del || (set_server && set_src_address)) {
        ...
        rv = dhcp6_proxy_set_server(&addr, &src_addr, rx_table_id, server_table_id, is_del);
        ...
        switch (rv) {
            ...
            case VNET_API_ERROR_NO_SUCH_ENTRY:
                return clib_error_return(0, "Fib id %d: no per-fib DHCP server configured", rx_table_id);
...
static u8 *format_dhcp6_proxy_server(u8 *s, va_list *args)
{
    ...
    if (proxy == 0) {
        s = format(s, "%=14s%=16s%s", "RX FIB", "Src Address", "Servers FIB,Address");
        return s;
    }

    rx_fib = ip6_mfib_get(proxy->rx_fib_index);

    s = format(s, "%=14u%=16U", rx_fib->table_id, format_ip46_address, &proxy->dhcp_src_address, IP46_TYPE_ANY);

    vec_foreach (server, proxy->dhcp_servers) {
        server_fib = fib_table_get(server->server_fib_index, FIB_PROTOCOL_IP6);
        s          = format(s, "%u,%U  ", server_fib->ft_table_id, format_ip46_address, &server->dhcp_server, IP46_TYPE_ANY);
    }
...
static clib_error_t *dhcpv6_vss_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (tbl_id == ~0) return clib_error_return(0, "no table ID specified.");

    int rv = dhcp_proxy_set_vss(FIB_PROTOCOL_IP6, tbl_id, vss_type, vpn_ascii_id, oui, fib_id, is_del);
    switch (rv) {
        case 0:
            return 0;
        case VNET_API_ERROR_NO_SUCH_ENTRY:
            return clib_error_return(0, "vss for table %d not found in pool.", tbl_id);
        default:
            return clib_error_return(0, "BUG: rv %d", rv);
    }
}
...
static clib_error_t *dhcpv6_link_address_show_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "%U", unformat_vnet_sw_interface, vnm, &sw_if_index0)) {
            swif        = vnet_get_sw_interface(vnm, sw_if_index0);
            sw_if_index = (swif->flags & VNET_SW_INTERFACE_FLAG_UNNUMBERED) ? swif->unnumbered_sw_if_index : sw_if_index0;
            ia0         = ip6_interface_first_address(&ip6_main, sw_if_index);
            if (ia0) {
                vlib_cli_output(vm, "%=20s%=48s", "interface", "link-address");

                vlib_cli_output(vm, "%=20U%=48U", format_vnet_sw_if_index_name, vnm, sw_if_index0, format_ip6_address, ia0);
            }
            else
                vlib_cli_output(vm, "%=34s%=20U", "No IPv6 address configured on", format_vnet_sw_if_index_name, vnm, sw_if_index);
        }
        else
            break;
    }
