...
static void vl_api_dhcp_plugin_get_version_t_handler(vl_api_dhcp_plugin_get_version_t *mp)
{
    ...
    rmp->_vl_msg_id = ntohs(VL_API_DHCP_PLUGIN_GET_VERSION_REPLY + REPLY_MSG_ID_BASE);
...
static void vl_api_dhcp_plugin_control_ping_t_handler(vl_api_dhcp_plugin_control_ping_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_DHCP_PLUGIN_CONTROL_PING_REPLY, ({ rmp->vpe_pid = ntohl(getpid()); }));
}
...
static void vl_api_dhcp_proxy_set_vss_t_handler(vl_api_dhcp_proxy_set_vss_t *mp)
{
    ...
    rv                                             = dhcp_proxy_set_vss((mp->is_ipv6 ? FIB_PROTOCOL_IP6 : FIB_PROTOCOL_IP4),
                            ntohl(mp->tbl_id),
                            ntohl(mp->vss_type),
                            vpn_ascii_id,
                            ntohl(mp->oui),
...
static void vl_api_dhcp_proxy_config_t_handler(vl_api_dhcp_proxy_config_t *mp)
{
    ...
    if (mp->dhcp_src_address.af == ADDRESS_IP4) {
        rv = dhcp4_proxy_set_server(&server, &src, (u32) ntohl(mp->rx_vrf_id), (u32) ntohl(mp->server_vrf_id), (int) (mp->is_add == 0));
    }
    else {
        rv = dhcp6_proxy_set_server(&server, &src, (u32) ntohl(mp->rx_vrf_id), (u32) ntohl(mp->server_vrf_id), (int) (mp->is_add == 0));
    }
...
static void vl_api_dhcp_proxy_dump_t_handler(vl_api_dhcp_proxy_dump_t *mp)
{
    ...
    dhcp_proxy_dump((mp->is_ip6 == 1 ? FIB_PROTOCOL_IP6 : FIB_PROTOCOL_IP4), reg, mp->context);
}
...
void dhcp_send_details(fib_protocol_t proto, void *opaque, u32 context, dhcp_proxy_t *proxy)
{
    ...
    mp->rx_vrf_id = htonl(dhcp_proxy_rx_table_get_table_id(proto, proxy->rx_fib_index));
    ...
    if (vss) {
        ...
        if (vss->vss_type == VSS_TYPE_ASCII) {
            ...
            u32 oui        = ((u32) vss->vpn_id[0] << 16) + ((u32) vss->vpn_id[1] << 8) + ((u32) vss->vpn_id[2]);
            u32 fib_id     = ((u32) vss->vpn_id[3] << 24) + ((u32) vss->vpn_id[4] << 16) + ((u32) vss->vpn_id[5] << 8) + ((u32) vss->vpn_id[6]);
...
static void dhcp_client_lease_encode(vl_api_dhcp_lease_t *lease, const dhcp_client_t *client)
{
    ...
    clib_memcpy(&lease->host_address.un, (u8 *) &client->installed.leased_address, sizeof(ip4_address_t));
    clib_memcpy(&lease->router_address.un, (u8 *) &client->installed.router_address, sizeof(ip4_address_t));

    lease->count = vec_len(client->domain_server_address);
    for (i = 0; i < lease->count; i++)
        clib_memcpy(&lease->domain_server[i].address.un.ip4, (u8 *) &client->domain_server_address[i], sizeof(ip4_address_t));
...
static void dhcp_client_data_encode(vl_api_dhcp_client_t *vclient, const dhcp_client_t *client)
{
    ...
    len = clib_min(sizeof(vclient->id) - 1, vec_len(client->client_identifier));
...
static void dhcp_compl_event_callback(u32 client_index, const dhcp_client_t *client)
{
    ...
    mp               = vl_msg_api_alloc(sizeof(*mp) + sizeof(vl_api_domain_server_t) * vec_len(client->domain_server_address));
...
static void vl_api_dhcp_client_config_t_handler(vl_api_dhcp_client_config_t *mp)
{
    ...
    rv = dhcp_client_config(mp->is_add,
                            ...
                            mp->client.hostname,
                            mp->client.id,
                            (mp->client.want_dhcp_event ? dhcp_compl_event_callback : NULL),
...
void vl_api_want_dhcp6_reply_events_t_handler(vl_api_want_dhcp6_reply_events_t *mp)
{
    ...
    uword                     *p = hash_get(am->dhcp6_reply_events_registration_hash, mp->client_index);
    ...
    if (p) {
        if (mp->enable_disable) {
            ...
            hash_unset(am->dhcp6_reply_events_registration_hash, mp->client_index);
            if (pool_elts(am->dhcp6_reply_events_registrations) == 0) dhcp6_set_publisher_node(~0, DHCP6_DP_REPORT_MAX);
            goto reply;
        }
    }
    ...
    hash_set(am->dhcp6_reply_events_registration_hash, rp->client_index, rp - am->dhcp6_reply_events_registrations);
    dhcp6_set_publisher_node(dhcp6_reply_process_node.index, DHCP6_DP_REPLY_REPORT);
...
void vl_api_want_dhcp6_pd_reply_events_t_handler(vl_api_want_dhcp6_pd_reply_events_t *mp)
{
    ...
    uword                     *p = hash_get(am->dhcp6_pd_reply_events_registration_hash, mp->client_index);
    vpe_client_registration_t *rp;
    if (p) {
        if (mp->enable_disable) {
            clib_warning("pid %d: already enabled...", ntohl(mp->pid));
            ...
            rp = pool_elt_at_index(am->dhcp6_pd_reply_events_registrations, p[0]);
            pool_put(am->dhcp6_pd_reply_events_registrations, rp);
            hash_unset(am->dhcp6_pd_reply_events_registration_hash, mp->client_index);
            if (pool_elts(am->dhcp6_pd_reply_events_registrations) == 0) dhcp6_pd_set_publisher_node(~0, DHCP6_PD_DP_REPORT_MAX);
            goto reply;
        }
    }
    if (mp->enable_disable == 0) {
    ...
    rp->client_index = mp->client_index;
    rp->client_pid   = ntohl(mp->pid);
    hash_set(am->dhcp6_pd_reply_events_registration_hash, rp->client_index, rp - am->dhcp6_pd_reply_events_registrations);
    dhcp6_pd_set_publisher_node(dhcp6_pd_reply_process_node.index, DHCP6_PD_DP_REPLY_REPORT);
...
void vl_api_dhcp6_send_client_message_t_handler(vl_api_dhcp6_send_client_message_t *mp)
{
    ...
    params.addresses = 0;
    if (n_addresses > 0) vec_validate(params.addresses, n_addresses - 1);
...
void vl_api_dhcp6_pd_send_client_message_t_handler(vl_api_dhcp6_pd_send_client_message_t *mp)
{
    ...
    params.prefixes = 0;
    if (n_prefixes > 0) vec_validate(params.prefixes, n_prefixes - 1);
    for (i = 0; i < n_prefixes; i++) {
        vl_api_dhcp6_pd_prefix_info_t                *pi   = &mp->prefixes[i];
        dhcp6_pd_send_client_message_params_prefix_t *pref = &params.prefixes[i];
    ...
    dhcp6_pd_send_client_message(vm, ntohl(mp->sw_if_index), mp->stop, &params);
}
...
static void vl_api_dhcp_client_detect_enable_disable_t_handler(vl_api_dhcp_client_detect_enable_disable_t *mp)
{
    ...
    if (mp->enable) {
        vnet_feature_enable_disable("ip4-unicast", "ip4-dhcp-client-detect", clib_net_to_host_u32(mp->sw_if_index), 1 /* enable */, 0, 0);
    }
    else {
        vnet_feature_enable_disable("ip4-unicast", "ip4-dhcp-client-detect", clib_net_to_host_u32(mp->sw_if_index), 0 /* disable */, 0, 0);
    }
    ...
    while (1) {
        ...
        if (event_type == DHCP6_DP_REPLY_REPORT) {
            ...
            for (i = 0; i < vec_len(events); i++) {
                u32 event_size = sizeof(vl_api_dhcp6_reply_event_t) + vec_len(events[i].addresses) * sizeof(vl_api_dhcp6_address_info_t);
                ...
                event->inner_status_code = htons(events[i].body.inner_status_code);
                ...
                vl_api_dhcp6_address_info_t *address = (typeof(address)) event->addresses;
                u32                          j;
                ...
                dhcp6_ia_na_client_public_main_t *dcpm = &dhcp6_ia_na_client_public_main;
                ...
                pool_foreach (reg, vpe_api_main.dhcp6_reply_events_registrations) {
                    ...
                    if (vl_reg && vl_api_can_send_msg(vl_reg)) {
                        vl_api_dhcp6_reply_event_t *msg = vl_msg_api_alloc(event_size);
...
static uword dhcp6_pd_reply_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    while (1) {
        ...
        if (event_type == DHCP6_PD_DP_REPLY_REPORT) {
            ...
            for (i = 0; i < vec_len(events); i++) {
                u32 event_size = sizeof(vl_api_dhcp6_pd_reply_event_t) + vec_len(events[i].prefixes) * sizeof(vl_api_dhcp6_pd_prefix_info_t);
                vl_api_dhcp6_pd_reply_event_t *event = clib_mem_alloc(event_size);
                clib_memset(event, 0, event_size);
                ...
                vl_api_dhcp6_pd_prefix_info_t *prefix = (typeof(prefix)) event->prefixes;
                u32                            j;
                ...
                dhcp6_pd_client_public_main_t *dpcpm = &dhcp6_pd_client_public_main;
                ...
                pool_foreach (reg, vpe_api_main.dhcp6_pd_reply_events_registrations) {
                    ...
                    vl_reg = vl_api_client_index_to_registration(reg->client_index);
                    if (vl_reg && vl_api_can_send_msg(vl_reg)) {
                        vl_api_dhcp6_pd_reply_event_t *msg = vl_msg_api_alloc(event_size);
                        clib_memcpy(msg, event, event_size);
