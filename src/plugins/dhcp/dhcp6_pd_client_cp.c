...
static_always_inline void active_prefix_index_by_prefix_group_index_set(u32 prefix_group_index, u32 prefix_index)
{
    ...
    if (prefix_index == ~0 && prefix_group_index >= vec_len(apm->active_prefix_index_by_prefix_group_index)) return;

    vec_validate_init_empty(apm->active_prefix_index_by_prefix_group_index, prefix_group_index, empty);
    apm->active_prefix_index_by_prefix_group_index[prefix_group_index] = prefix_index;
}
...
static_always_inline void set_is_dhcpv6_pd_prefix(prefix_info_t *prefix_info, u8 value)
{
    ...
    prefix_index                = prefix_info - pm->prefix_pool;
    rm->prefix_ownership_bitmap = clib_bitmap_set(rm->prefix_ownership_bitmap, prefix_index, value);
}
...
static void send_client_message_start_stop(u32 sw_if_index, u32 server_index, u8 msg_type, prefix_info_t *prefix_list, u8 start)
{
    ...
    ASSERT(sw_if_index < vec_len(rm->client_state_by_sw_if_index) && rm->client_state_by_sw_if_index[sw_if_index].enabled);
    ...
    params.T1 = 0;
    params.T2 = 0;
    if (vec_len(prefix_list) != 0) vec_validate(prefixes, vec_len(prefix_list) - 1);
...
{
    if (len >= 64) {
        if (prefix1->as_u64[0] != prefix2->as_u64[0]) return 0;
        if (len == 64) return 1;
        return clib_net_to_host_u64(prefix1->as_u64[1]) >> (128 - len) == clib_net_to_host_u64(prefix2->as_u64[1]) >> (128 - len);
    }
    return clib_net_to_host_u64(prefix1->as_u64[0]) >> (64 - len) == clib_net_to_host_u64(prefix2->as_u64[0]) >> (64 - len);
}
...
static clib_error_t *dhcp6_pd_reply_event_handler(vl_api_dhcp6_pd_reply_event_t *mp)
{
    ...
    if (sw_if_index >= vec_len(rm->client_state_by_sw_if_index)) return 0;

    client_state = &rm->client_state_by_sw_if_index[sw_if_index];

    if (!client_state->enabled) return 0;
    ...
    if (mp->msg_type == DHCPV6_MSG_API_ADVERTISE && client_state->server_index == ~0) {
        prefix_info_t *prefix_list = 0, *prefix_info;
        u8             prefix_length;

        if (inner_status_code == DHCPV6_STATUS_NOPREFIX_AVAIL) {
            clib_warning("Advertise message arrived with NoPrefixAvail status code");
            return 0;
        }

        if (n_prefixes > 0) vec_validate(prefix_list, n_prefixes - 1);
        ...
        send_client_message_start_stop(sw_if_index, server_index, DHCPV6_MSG_REQUEST, prefix_list, 1);
        vec_free(prefix_list);
    }

    if (mp->msg_type != DHCPV6_MSG_API_REPLY) return 0;
    ...
    send_client_message_start_stop(sw_if_index, server_index, mp->msg_type, 0, 0);
    ...
    for (i = 0; i < n_prefixes; i++) {
        ...
        if (ip6_address_is_link_local_unicast(prefix)) continue;

        valid_time     = ntohl(api_prefix->valid_time);
        preferred_time = ntohl(api_prefix->preferred_time);
        prefix_length  = api_prefix->prefix.len;

        if (preferred_time > valid_time) continue;
        ...
        for (j = 0; j < vec_len(pm->indices); j++) {
            ...
            if (is_dhcpv6_pd_prefix(prefix_info) && prefix_info->opaque_data == sw_if_index && prefix_info->prefix_length == prefix_length
        ...
        if (address_prefix_present) {
            ...
            if (prefix_info->due_time > rm->max_valid_due_time) rm->max_valid_due_time = prefix_info->due_time;

            /*
             * Tell the RA code to update any secondary per-interface
             * timers that it might be hoarding.
             */
            ip6_ra_update_secondary_radv_info(prefix, prefix_length, prefix_info->opaque_data /* sw_if_index */, valid_time, preferred_time);
            continue;
        }

        if (valid_time == 0) continue;
    ...
    if (client_state->T1 != 0) client_state->T1_due_time = current_time + client_state->T1;
    if (client_state->T2 != 0) client_state->T2_due_time = current_time + client_state->T2;
...
static prefix_info_t *create_prefix_list(u32 sw_if_index)
{
    ...
    pool_foreach (prefix_info, pm->prefix_pool) {
        if (is_dhcpv6_pd_prefix(prefix_info) && prefix_info->opaque_data == sw_if_index) {
            ...
            pool_foreach (prefix_info, pm->prefix_pool) {
                if (is_dhcpv6_pd_prefix(prefix_info)) {
                    if (prefix_info->due_time > current_time) {
                        if (prefix_info->due_time < due_time) due_time = prefix_info->due_time;
                    }
                    else {
                        u32 prefix_index = prefix_info - pm->prefix_pool;
            ...
            for (i = 0; i < vec_len(rm->client_state_by_sw_if_index); i++) {
                ...
                if (cs->enabled && cs->server_index != ~0) {
                    if (cs->T2_due_time > current_time) {
                        if (cs->T2_due_time < due_time) due_time = cs->T2_due_time;
                        if (cs->T1_due_time > current_time) {
                            if (cs->T1_due_time < due_time) due_time = cs->T1_due_time;
                        }
                        else {
                            cs->T1_due_time = DBL_MAX;
                            prefix_info_t *prefix_list;
                            prefix_list = create_prefix_list(i);
                            send_client_message_start_stop(i, cs->server_index, DHCPV6_MSG_RENEW, prefix_list, 1);
                            vec_free(prefix_list);
                        }
                    }
                    else {
                        ...
                        prefix_list   = create_prefix_list(i);
                        cs->rebinding = 1;
                        send_client_message_start_stop(i, ~0, DHCPV6_MSG_REBIND, prefix_list, 1);
                        vec_free(prefix_list);
                    }
                }
            }
...
static void interrupt_process(void)
{
    ...
    vlib_process_signal_event(vm, dhcp6_pd_client_cp_process_node.index, DHCPV6_PD_EVENT_INTERRUPT, 0);
}
...
static void disable_process(void)
{
    ...
    vlib_process_signal_event(vm, dhcp6_pd_client_cp_process_node.index, DHCPV6_PD_EVENT_DISABLE, 0);
}
...
static void cp_ip6_address_add_del_now(ip6_address_info_t *address_info, u8 is_add)
{
    ...
    if (address_info->prefix_group_index != ~0)
        prefix_index = active_prefix_index_by_prefix_group_index_get(address_info->prefix_group_index);
    else
        prefix_index = ~0;

    if (is_add && !address_info->configured_in_data_plane) {
        if (prefix_index != ~0) {
            if (cp_ip6_construct_address(address_info, prefix_index, &addr) != 0) return;
            error = ip6_add_del_interface_address(vm, address_info->sw_if_index, &addr, address_info->prefix_length, 0 /* add */);
            if (error)
                clib_warning("Failed adding IPv6 address: %U", format_clib_error, error);
            else {
                if (CLIB_DEBUG > 0)
            ...
            if (address_info->prefix_group_index == ~0) {
                error =
                    ip6_add_del_interface_address(vm, address_info->sw_if_index, &address_info->address, address_info->prefix_length, 0 /* add */);
                if (error)
                    clib_warning("Failed adding IPv6 address: %U", format_clib_error, error);
                else {
                    if (CLIB_DEBUG > 0)
        ...
        if (prefix_index == ~0) {
            if (address_info->prefix_group_index == ~0) {
                error =
                    ip6_add_del_interface_address(vm, address_info->sw_if_index, &address_info->address, address_info->prefix_length, 1 /* del */);
                if (error) clib_warning("Failed deleting IPv6 address: %U", format_clib_error, error);
                address_info->configured_in_data_plane = 0;
            }
            else
                clib_warning(
                    "Deleting address with prefix "
                    "but active prefix index is not set");
        }
        else {
            if (cp_ip6_construct_address(address_info, prefix_index, &addr) != 0) return;
            error = ip6_add_del_interface_address(vm, address_info->sw_if_index, &addr, address_info->prefix_length, 1 /* del */);
            if (error) clib_warning("Failed deleting IPv6 address: %U", format_clib_error, error);
            address_info->configured_in_data_plane = 0;
        }
    }
...
static u32 cp_ip6_address_find_new_active_prefix(u32 prefix_group_index, u32 ignore_prefix_index)
{
    ...
    pool_foreach (prefix_info, pm->prefix_pool) {
        if (prefix_info->prefix_group_index == prefix_group_index && prefix_info - pm->prefix_pool != ignore_prefix_index)
...
static void cp_ip6_address_prefix_add_del_handler(u32 prefix_index, u8 is_add)
{
    ...
    if (is_add) {
        if (active_prefix_index_by_prefix_group_index_get(prefix_group_index) == ~0) {
            active_prefix_index_by_prefix_group_index_set(prefix_group_index, prefix_index);
            for (i = 0; i < vec_len(apm->addresses); i++) {
                ...
                if (address_info->prefix_group_index == prefix_group_index) {
                    ...
                    cp_ip6_advertise_prefix(prefix, address_info, 1 /* enable */);
                }
            }
        }
    }
    else {
        if (active_prefix_index_by_prefix_group_index_get(prefix_group_index) == prefix_index) {
            for (i = 0; i < vec_len(apm->addresses); i++) {
                address_info = &apm->addresses[i];
                if (address_info->prefix_group_index == prefix_group_index) {
                    cp_ip6_advertise_prefix(prefix, address_info, 0 /* enable */);
                    cp_ip6_address_add_del_now(address_info, 0 /* del */);
                }
            }
            active_prefix_index_by_prefix_group_index_set(prefix_group_index, ~0);
            new_prefix_index = cp_ip6_address_find_new_active_prefix(prefix_group_index, prefix_index);
            if (new_prefix_index != ~0) {
                active_prefix_index_by_prefix_group_index_set(prefix_group_index, new_prefix_index);
                for (i = 0; i < vec_len(apm->addresses); i++) {
                    address_info = &apm->addresses[i];
                    if (address_info->prefix_group_index == prefix_group_index) {
                        cp_ip6_address_add_del_now(address_info, 1 /* add */);
...
static u32 prefix_group_find_or_create(const u8 *name, u8 create)
{
    ...
    for (i = 0; i < vec_len(pm->prefix_group_name_by_index); i++) {
        if (pm->prefix_group_name_by_index[i] == 0)
            free_index = i;
        else if (0 == strcmp((const char *) pm->prefix_group_name_by_index[i], (const char *) name))
            return i;
    }
    if (!create) return ~0;
...
int dhcp6_cp_ip6_address_add_del(u32 sw_if_index, const u8 *prefix_group, ip6_address_t address, u8 prefix_length, u8 is_add)
{
    ...
    if (prefix_group != 0 && prefix_group[0] != '\0') {
        if (strnlen((const char *) prefix_group, 64) == 64) return VNET_API_ERROR_INVALID_VALUE;
    ...
    vec_foreach (address_info, apm->addresses) {
        if (address_info->sw_if_index == sw_if_index && address_info->prefix_group_index == prefix_group_index
            && address_info->prefix_length == prefix_length && 0 == memcmp(&address_info->address, &address, 16)) {
            if (is_add) return VNET_API_ERROR_DUPLICATE_IF_ADDRESS;
            cp_ip6_address_add_del_now(address_info, 0 /* del */);
    ...
    if (!is_add) return VNET_API_ERROR_ADDRESS_NOT_FOUND_FOR_INTERFACE;
...
static clib_error_t *cp_ip6_address_add_del_command_function(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%U", unformat_vnet_sw_interface, vnm, &sw_if_index))
            ...
            ;
        else if (unformat(line_input, "%U/%d", unformat_ip6_address, &address, &prefix_length))
            address_set = 1;
        else if (unformat(line_input, "del"))
            add = 0;
        else {
            error = clib_error_return(0, "unexpected input `%U'", format_unformat_error, line_input);
    ...
    if (sw_if_index == ~0)
        ...
        if (dhcp6_cp_ip6_address_add_del(sw_if_index, prefix_group, address, prefix_length, add) != 0)
...
static clib_error_t *cp_ip6_addresses_show_command_function(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (i = 0; i < vec_len(apm->addresses); i++) {
        ...
        if (address_info->prefix_group_index == ~0)
            ...
            prefix_group = pm->prefix_group_name_by_index[address_info->prefix_group_index];
        vlib_cli_output(vm,
...
static clib_error_t *cp_ip6_prefixes_show_command_function(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    pool_foreach (prefix_info, pm->prefix_pool) {
        prefix_group = pm->prefix_group_name_by_index[prefix_info->prefix_group_index];
        vlib_cli_output(vm,
                        "opaque_data: %lu, prefix: %U/%d, prefix group: %s, "
                        "preferred lifetime: %u, valid lifetime: %u "
                        "(%f remaining)",
                        prefix_info->opaque_data,
                        format_ip6_address,
                        &prefix_info->prefix,
    ...
    for (i = 0; i < vec_len(rm->client_state_by_sw_if_index); i++) {
        ...
        if (cs->enabled) {
            ...
            if (cs->T1_due_time != DBL_MAX && cs->T1_due_time > current_time) {
                buf1 = format(buf1, "%u remaining", (u32) round(cs->T1_due_time - current_time));
            }
            else
                buf1 = format(buf1, "timeout");
            if (cs->T2_due_time != DBL_MAX && cs->T2_due_time > current_time)
                buf2 = format(buf2, "%u remaining", (u32) round(cs->T2_due_time - current_time));
            else
            ...
            if (cs->T1)
                ...
                vlib_cli_output(vm, "sw_if_index: %u, prefix group: %s%s", i, prefix_group, rebinding);
        }
    }
...
int dhcp6_pd_client_enable_disable(u32 sw_if_index, const u8 *prefix_group, u8 enable)
{
    ...
    vec_validate_init_empty(rm->client_state_by_sw_if_index, sw_if_index, empty_config);
    client_state = &rm->client_state_by_sw_if_index[sw_if_index];

    u8 old_enabled = client_state->enabled;

    if (enable) {
        if (strnlen((const char *) prefix_group, 64) == 64 || prefix_group[0] == '\0') return VNET_API_ERROR_INVALID_VALUE;
        prefix_group_index = prefix_group_find_or_create(prefix_group, !old_enabled);
        if (old_enabled && prefix_group_index != client_state->prefix_group_index) return VNET_API_ERROR_INVALID_VALUE;
    }
    ...
    if (!old_enabled && enable) {
        ...
        pool_foreach (prefix_info, pm->prefix_pool) {
            if (is_dhcpv6_pd_prefix(prefix_info) && prefix_info->opaque_data == sw_if_index) {
                ...
                send_client_message_start_stop(sw_if_index, client_state->server_index, DHCPV6_MSG_RELEASE, prefix_list, 1);
                u32 prefix_index = prefix_info - pm->prefix_pool;
                notify_prefix_add_del(prefix_index, 0);
                set_is_dhcpv6_pd_prefix(prefix_info, 0);
                pool_put(pm->prefix_pool, prefix_info);
...
static clib_error_t *dhcp6_pd_client_enable_disable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (prefix_group == 0 && enable)
        ...
        if (dhcp6_pd_client_enable_disable(sw_if_index, prefix_group, enable) != 0)
