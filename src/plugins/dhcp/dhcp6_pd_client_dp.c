...
static void signal_report(prefix_report_t *r)
{
    ...
    if (ni == (uword) ~0) return;
    prefix_report_t *q = vlib_process_signal_event_data(vm, ni, et, 1, sizeof *q);
...
static vlib_buffer_t *create_buffer_for_client_message(vlib_main_t *vm, u32 sw_if_index, dhcp6_pd_client_state_t *client_state, u32 type)
{
    ...
    if (type == DHCPV6_MSG_SOLICIT || type == DHCPV6_MSG_REQUEST || type == DHCPV6_MSG_RENEW || type == DHCPV6_MSG_REBIND
        ...
        client_state->elapsed_pos = (char *) &elapsed->elapsed_10ms - (char *) vlib_buffer_get_current(b);
        d += sizeof(*elapsed);
        ...
        ia_hdr->opt.length = clib_host_to_net_u16(sizeof(*ia_hdr) + n_prefixes * sizeof(*opt_pd) - sizeof(ia_hdr->opt));

        for (i = 0; i < n_prefixes; i++) {
            dhcp6_pd_send_client_message_params_prefix_t *pref = &client_state->params.prefixes[i];
            opt_pd                                             = (dhcpv6_ia_opt_pd_t *) d;
            opt_pd->opt.option                                 = clib_host_to_net_u16(DHCPV6_OPTION_IAPREFIX);
            opt_pd->opt.length                                 = clib_host_to_net_u16(sizeof(*opt_pd) - sizeof(opt_pd->opt));
...
static inline u8 check_pd_send_client_message(vlib_main_t *vm, dhcp6_pd_client_state_t *client_state, f64 current_time, f64 *due_time)
{
    vlib_buffer_t *p0;
    vlib_frame_t  *f;
    u32           *to_next;
    ...
    if (!client_state->keep_sending_client_message) return false;
    ...
    if (c0 == NULL) return client_state->keep_sending_client_message;
    ...
    *elapsed_field     = clib_host_to_net_u16((u16) ((now - client_state->transaction_start) * 100));
    ...
    if (params->mrc != 0 && --client_state->n_left == 0)
        ...
        client_state->sleep_interval = (2.0 + random_f64_from_to(-0.1, 0.1)) * client_state->sleep_interval;
        if (client_state->sleep_interval > params->mrt) client_state->sleep_interval = (1.0 + random_f64_from_to(-0.1, 0.1)) * params->mrt;
    ...
    while (true) {
        ...
        do {
            ...
            for (i = 0; i < vec_len(cm->client_state_by_sw_if_index); i++) {
                ...
                if (!client_state->entry_valid) continue;
                if (check_pd_send_client_message(vm, client_state, current_time, &dt) && (dt < due_time)) due_time = dt;
            }
            current_time = vlib_time_now(vm);
...
void dhcp6_pd_send_client_message(vlib_main_t *vm, u32 sw_if_index, u8 stop, dhcp6_pd_send_client_message_params_t *params)
{
    ...
    if (!stop) {
        ...
        client_state->buffer          = create_buffer_for_client_message(vm, sw_if_index, client_state, params->msg_type);
        if (client_state->buffer) vlib_process_signal_event(vm, send_dhcp6_pd_client_message_process_node.index, 1, 0);
    }
}
