...
static __clib_unused u8 *format_vhost_user_interface_name(u8 *s, va_list *args)
{
    ...
    if (i < vec_len(vum->show_dev_instance_by_real_dev_instance)) show_dev_instance = vum->show_dev_instance_by_real_dev_instance[i];

    if (show_dev_instance != ~0) i = show_dev_instance;
...
static __clib_unused int vhost_user_name_renumber(vnet_hw_interface_t *hi, u32 new_dev_instance)
{
    ...
    vhost_user_main_t *vum = &vhost_user_main;
    vhost_user_intf_t *vui = pool_elt_at_index(vum->vhost_user_interfaces, hi->dev_instance);

    vec_validate_init_empty(vum->show_dev_instance_by_real_dev_instance, hi->dev_instance, ~0);

    vum->show_dev_instance_by_real_dev_instance[hi->dev_instance] = new_dev_instance;

    vu_log_debug(vui, "renumbered vhost-user interface dev_instance %d to %d", hi->dev_instance, new_dev_instance);
...
static_always_inline void vhost_user_tx_trace(vhost_trace_t *t, vhost_user_intf_t *vui, u16 qid, vlib_buffer_t *b, vhost_user_vring_t *rxvq)
{
    ...
    if (!(rxvq->desc[desc_current].flags & VRING_DESC_F_NEXT) && !(rxvq->desc[desc_current].flags & VRING_DESC_F_INDIRECT)) {
...
static_always_inline u32 vhost_user_tx_copy(vhost_user_intf_t *vui, vhost_copy_t *cpy, u16 copy_len, u32 *map_hint)
{
    ...
    if (PREDICT_TRUE(copy_len >= 4)) {
        if (PREDICT_FALSE(!(dst2 = map_guest_mem(vui, cpy[0].dst, map_hint)))) return 1;
        if (PREDICT_FALSE(!(dst3 = map_guest_mem(vui, cpy[1].dst, map_hint)))) return 1;
        while (PREDICT_TRUE(copy_len >= 4)) {
            dst0 = dst2;
            dst1 = dst3;

            if (PREDICT_FALSE(!(dst2 = map_guest_mem(vui, cpy[2].dst, map_hint)))) return 1;
            if (PREDICT_FALSE(!(dst3 = map_guest_mem(vui, cpy[3].dst, map_hint)))) return 1;
    ...
    while (copy_len) {
        if (PREDICT_FALSE(!(dst0 = map_guest_mem(vui, cpy->dst, map_hint)))) return 1;
...
static_always_inline void vhost_user_handle_tx_offload(vhost_user_intf_t *vui, vlib_buffer_t *b, vnet_virtio_net_hdr_t *hdr)
{
    ...
    if (oflags & VNET_BUFFER_OFFLOAD_F_UDP_CKSUM) {
        udp_header_t *udp = (udp_header_t *) (vlib_buffer_get_current(b) + gho.l4_hdr_offset);
        ...
        tcp_header_t *tcp = (tcp_header_t *) (vlib_buffer_get_current(b) + gho.l4_hdr_offset);
    ...
    if (b->flags & VNET_BUFFER_F_GSO) {
        if (oflags & VNET_BUFFER_OFFLOAD_F_TCP_CKSUM) {
            if (is_ip4 && (vui->features & VIRTIO_FEATURE(VIRTIO_NET_F_GUEST_TSO4))) {
                hdr->gso_size = vnet_buffer2(b)->gso_size;
                hdr->gso_type = VIRTIO_NET_HDR_GSO_TCPV4;
            }
            else if (is_ip6 && (vui->features & VIRTIO_FEATURE(VIRTIO_NET_F_GUEST_TSO6))) {
                hdr->gso_size = vnet_buffer2(b)->gso_size;
                hdr->gso_type = VIRTIO_NET_HDR_GSO_TCPV6;
            }
        }
        else if ((vui->features & VIRTIO_FEATURE(VIRTIO_NET_F_GUEST_UFO)) && (oflags & VNET_BUFFER_OFFLOAD_F_UDP_CKSUM)) {
            hdr->gso_size = vnet_buffer2(b)->gso_size;
                                                         ...
                                                         u32                 n_left)
{
    ...
    if (PREDICT_FALSE(*n_descs_processed == 0)) return;

    if (rxvq->used_wrap_counter)
        flags = desc_table[last_used_idx & rxvq->qsz_mask].flags | (VRING_DESC_F_AVAIL | VRING_DESC_F_USED);
    else
        flags = desc_table[last_used_idx & rxvq->qsz_mask].flags & ~(VRING_DESC_F_AVAIL | VRING_DESC_F_USED);
    ...
    for (desc_idx = 1; desc_idx < *n_descs_processed; desc_idx++) {
        if (rxvq->used_wrap_counter)
            desc_table[rxvq->last_used_idx & rxvq->qsz_mask].flags |= (VRING_DESC_F_AVAIL | VRING_DESC_F_USED);
        else
            desc_table[rxvq->last_used_idx & rxvq->qsz_mask].flags &= ~(VRING_DESC_F_AVAIL | VRING_DESC_F_USED);
        vhost_user_advance_last_used_idx(rxvq);
    }
    ...
    *n_descs_processed = 0;
    ...
    if (chained) {
        ...
        while (desc_table[rxvq->last_used_idx & rxvq->qsz_mask].flags & VRING_DESC_F_NEXT)
            vhost_user_advance_last_used_idx(rxvq);
    ...
    if ((rxvq->callfd_idx != ~0) && (rxvq->avail_event->flags != VRING_EVENT_F_DISABLE)) {
        vhost_user_main_t *vum = &vhost_user_main;

        rxvq->n_since_last_int += frame->n_vectors - n_left;
        if (rxvq->n_since_last_int > vum->coalesce_frames) vhost_user_send_call(vm, vui, rxvq);
    }
}
...
static_always_inline void vhost_user_tx_trace_packed(vhost_trace_t *t, vhost_user_intf_t *vui, u16 qid, vlib_buffer_t *b, vhost_user_vring_t *rxvq)
{
    ...
    if (rxvq->packed_desc[desc_current].flags & VRING_DESC_F_INDIRECT) {
        t->virtio_ring_flags |= 1 << VIRTIO_TRACE_F_INDIRECT;
        /* Header is the first here */
        hdr_desc = map_guest_mem(vui, rxvq->packed_desc[desc_current].addr, &hint);
    }
    if (rxvq->packed_desc[desc_current].flags & VRING_DESC_F_NEXT) {
        t->virtio_ring_flags |= 1 << VIRTIO_TRACE_F_SIMPLE_CHAINED;
    }
    if (!(rxvq->packed_desc[desc_current].flags & VRING_DESC_F_NEXT) && !(rxvq->packed_desc[desc_current].flags & VRING_DESC_F_INDIRECT)) {
        t->virtio_ring_flags |= 1 << VIRTIO_TRACE_F_SINGLE_DESC;
    }
...
vhost_user_device_class_packed(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, vhost_user_intf_t *vui, vhost_user_vring_t *rxvq)
{
...
retry:
    ...
    while (n_left > 0) {
        ...
        indirect = 0;
        chained  = 0;
        if (PREDICT_TRUE(n_left > 1)) vlib_prefetch_buffer_with_index(vm, buffers[1], LOAD);

        b0 = vlib_get_buffer(vm, buffers[0]);
        if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
            cpu->current_trace = vlib_add_trace(vm, node, b0, sizeof(*cpu->current_trace));
            vhost_user_tx_trace_packed(cpu->current_trace, vui, qid / 2, b0, rxvq);
        }
        ...
        if (desc_table[desc_head].flags & VRING_DESC_F_INDIRECT) {
            indirect = 1;
            if (PREDICT_FALSE(desc_table[desc_head].len < sizeof(vnet_virtio_vring_packed_desc_t))) {
                error = VHOST_USER_TX_FUNC_ERROR_INDIRECT_OVERFLOW;
                goto done;
            }
            n_entries  = desc_table[desc_head].len >> 4;
            desc_table = map_guest_mem(vui, desc_table[desc_index].addr, &map_hint);
        ...
        if (or_flags && (vui->features & VIRTIO_FEATURE(VIRTIO_NET_F_GUEST_CSUM))) vhost_user_handle_tx_offload(vui, b0, &hdr->hdr);
        ...
        while (1) {
            if (buffer_len == 0) {
                ...
                if (chained) {
                     ...
                     */
                    if (PREDICT_FALSE(!(desc_table[desc_index].flags & VRING_DESC_F_NEXT))) {
                        /*
                         * Last descriptor in chain.
                         * Dequeue queued descriptors for this packet
                         */
                        vhost_user_dequeue_chained_descs(rxvq, &n_descs_processed);
                        error = VHOST_USER_TX_FUNC_ERROR_PKT_DROP_NOBUF;
                        goto done;
                    }
                     ...
                     */
                    vnet_virtio_net_hdr_mrg_rxbuf_t *hdr = &cpu->tx_headers[tx_headers_len - 1];
                    ...
                    if (PREDICT_FALSE(!vhost_user_packed_desc_available(rxvq, desc_index))) {
                        /* Dequeue queued descriptors for this packet */
            ...
            cpy->dst = buffer_map_addr;
            cpy->src = (uword) vlib_buffer_get_current(current_b0) + current_b0->current_length - bytes_left;
        ...
        if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) cpu->current_trace->hdr = cpu->tx_headers[tx_headers_len - 1];
        ...
        if (PREDICT_FALSE(copy_len >= VHOST_USER_TX_COPY_THRESHOLD) || chained) {
            ...
            vhost_user_mark_desc_available(vm, vui, rxvq, &n_descs_processed, chained, frame, n_left);
        }
    ...
    if (PREDICT_FALSE(n_left && error != VHOST_USER_TX_FUNC_ERROR_NONE)) {
        ...
        vlib_increment_simple_counter(vnet_main.interface_main.sw_if_counters + VNET_INTERFACE_COUNTER_DROP, thread_index, vui->sw_if_index, n_left);
    }
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    vhost_user_intf_t               *vui     = pool_elt_at_index(vum->vhost_user_interfaces, rd->dev_instance);
    u32                              qid;
    vhost_user_vring_t              *rxvq;
    u8                               error;
    u32                              thread_index = vm->thread_index;
    ...
    if (tf->shared_queue) clib_spinlock_lock(&rxvq->vring_lock);

    if (vhost_user_is_packed_ring_supported(vui)) return (vhost_user_device_class_packed(vm, node, frame, vui, rxvq));
...
retry:
    ...
    while (n_left > 0) {
        ...
        desc_head = desc_index = rxvq->avail->ring[rxvq->last_avail_idx & rxvq->qsz_mask];

        /* Go deeper in case of indirect descriptor
         * I don't know of any driver providing indirect for RX. */
        if (PREDICT_FALSE(rxvq->desc[desc_head].flags & VRING_DESC_F_INDIRECT)) {
            if (PREDICT_FALSE(rxvq->desc[desc_head].len < sizeof(vnet_virtio_vring_desc_t))) {
                error = VHOST_USER_TX_FUNC_ERROR_INDIRECT_OVERFLOW;
                goto done;
            }
            if (PREDICT_FALSE(!(desc_table = map_guest_mem(vui, rxvq->desc[desc_index].addr, &map_hint)))) {
                error = VHOST_USER_TX_FUNC_ERROR_MMAP_FAIL;
                goto done;
            }
            desc_index = 0;
        ...
        current_b0 = b0;
        while (1) {
            if (buffer_len == 0) {  // Get new output
                ...
                else if (vui->virtio_net_hdr_sz == 12)  // MRG is available
                {
                    ...
                    vhost_user_log_dirty_ring(vui, rxvq, ring[rxvq->last_used_idx & rxvq->qsz_mask]);
    ...
    /* interrupt (call) handling */
    if ((rxvq->callfd_idx != ~0) && !(rxvq->avail->flags & VRING_AVAIL_F_NO_INTERRUPT)) {
        rxvq->n_since_last_int += frame->n_vectors - n_left;
...
static __clib_unused clib_error_t *vhost_user_interface_rx_mode_change(vnet_main_t *vnm, u32 hw_if_index, u32 qid, vnet_hw_if_rx_mode mode)
{
    ...
    vhost_user_main_t   *vum  = &vhost_user_main;
    vhost_user_intf_t   *vui  = pool_elt_at_index(vum->vhost_user_interfaces, hif->dev_instance);
    vhost_user_vring_t  *txvq = &vui->vrings[VHOST_VRING_IDX_TX(qid)];
    vhost_cpu_t         *cpu;

    if (mode == txvq->mode) return 0;

    if ((mode != VNET_HW_IF_RX_MODE_POLLING) && (mode != VNET_HW_IF_RX_MODE_ADAPTIVE) && (mode != VNET_HW_IF_RX_MODE_INTERRUPT)) {
        vu_log_err(vui, "unhandled mode %d changed for if %d queue %d", mode, hw_if_index, qid);
        return clib_error_return(0, "unsupported");
    }

    if (txvq->thread_index == ~0) return clib_error_return(0, "Queue initialization is not finished yet");

    cpu = vec_elt_at_index(vum->cpus, txvq->thread_index);
    if ((mode == VNET_HW_IF_RX_MODE_INTERRUPT) || (mode == VNET_HW_IF_RX_MODE_ADAPTIVE)) {
        ...
        if (txvq->mode == VNET_HW_IF_RX_MODE_POLLING) {
            ...
            if (cpu->polling_q_count) cpu->polling_q_count--;
            vum->ifq_count++;
            // Start the timer if this is the first encounter on interrupt
            // interface/queue
            if ((vum->ifq_count == 1) && ((vum->coalesce_time > 0.0) || (vum->coalesce_frames > 0)))
                vlib_process_signal_event(vm, vhost_user_send_interrupt_node.index, VHOST_USER_EVENT_START_TIMER, 0);
        }
    }
    else if (mode == VNET_HW_IF_RX_MODE_POLLING) {
        if (((txvq->mode == VNET_HW_IF_RX_MODE_INTERRUPT) || (txvq->mode == VNET_HW_IF_RX_MODE_ADAPTIVE)) && vum->ifq_count) {
            cpu->polling_q_count++;
            vum->ifq_count--;
            // Stop the timer if there is no more interrupt interface/queue
            if (vum->ifq_count == 0) vlib_process_signal_event(vm, vhost_user_send_interrupt_node.index, VHOST_USER_EVENT_STOP_TIMER, 0);
        }
    }
...
static __clib_unused clib_error_t *vhost_user_interface_admin_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    ...
    if (link_old != link_new) vnet_hw_interface_set_flags(vnm, vui->hw_if_index, link_new ? VNET_HW_INTERFACE_FLAG_LINK_UP : 0);
