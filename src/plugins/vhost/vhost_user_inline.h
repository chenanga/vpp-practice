...
{
    int i = *hint;
    if (PREDICT_TRUE((vui->regions[i].guest_phys_addr <= addr) && ((vui->regions[i].guest_phys_addr + vui->regions[i].memory_size) > addr))) {
...
vhost_map_guest_mem_done:
    ...
    for (i = 0; i < vui->nregions; i++) {
        if ((vui->regions[i].guest_phys_addr <= addr) && ((vui->regions[i].guest_phys_addr + vui->regions[i].memory_size) > addr)) {
            *hint = i;
            return (void *) (vui->region_mmap_addr[i] + addr - vui->regions[i].guest_phys_addr);
        }
    }
...
static_always_inline void *map_user_mem(vhost_user_intf_t *vui, uword addr)
{
    int i;
    for (i = 0; i < vui->nregions; i++) {
        if ((vui->regions[i].userspace_addr <= addr) && ((vui->regions[i].userspace_addr + vui->regions[i].memory_size) > addr)) {
...
{
    if (PREDICT_TRUE(vui->log_base_addr == 0 || !(vui->features & VIRTIO_FEATURE(VHOST_F_LOG_ALL)))) {
        return;
    }
    if (is_host_address) {
...
#define vhost_user_log_dirty_ring(vui, vq, member)                                                                                               \
    if (PREDICT_FALSE(vq->log_used)) {                                                                                                           \
        vhost_user_log_dirty_pages_2(vui, vq->log_guest_addr + STRUCT_OFFSET_OF(vnet_virtio_vring_used_t, member), sizeof(vq->used->member), 0); \
    }
...
static_always_inline u8 *format_vhost_trace(u8 *s, va_list *va)
{
    ...
    s      = format(s, "%U %U queue %d\n", format_white_space, indent, format_vnet_sw_interface_name, vnm, sw, t->qid);

    s = format(s, "%U virtio flags:\n", format_white_space, indent);
#define _(n, i, st) \
    if (t->virtio_ring_flags & (1 << VIRTIO_TRACE_F_##n)) s = format(s, "%U  %s %s\n", format_white_space, indent, #n, st);
...
#undef _
    ...
    if (vui->virtio_net_hdr_sz == 12) s = format(s, "%U   num_buff %u", format_white_space, indent, t->hdr.num_buffers);
...
static_always_inline void vhost_user_kick(vlib_main_t *vm, vhost_user_vring_t *vq)
{
    ...
    if (PREDICT_FALSE(rv <= 0)) {
        clib_unix_warning("Error: Could not write to unix socket for callfd %d", fd);
        return;
    }
...
static_always_inline void vhost_user_send_call_event_idx(vlib_main_t *vm, vhost_user_vring_t *vq)
{
    ...
    vq->first_kick = 1;
    if (vhost_user_need_event(event_idx, vq->last_used_idx, vq->last_kick) || PREDICT_FALSE(!first_kick)) {
...
static_always_inline void vhost_user_send_call_event_idx_packed(vlib_main_t *vm, vhost_user_vring_t *vq)
{
    ...
    if (PREDICT_TRUE(vq->avail_event->flags == VRING_EVENT_F_DESC)) {
        ...
        if (vq->used_wrap_counter != (off_wrap >> 15)) event_idx -= (vq->qsz_mask + 1);

        if (new_idx <= old_idx) old_idx -= (vq->qsz_mask + 1);

        vq->first_kick = 1;
        vq->last_kick  = event_idx;
        if (vhost_user_need_event(event_idx, new_idx, old_idx) || PREDICT_FALSE(!first_kick))
            vhost_user_kick(vm, vq);
        else {
            vq->n_since_last_int = 0;
            vq->int_deadline     = vlib_time_now(vm) + vum->coalesce_time;
...
{
    return (((vring->packed_desc[idx].flags & VRING_DESC_F_AVAIL) == vring->avail_wrap_counter));
}
...
static_always_inline void vhost_user_advance_last_avail_table_idx(vhost_user_intf_t *vui, vhost_user_vring_t *vring, u8 chained)
{
    if (chained) {
        ...
        while (desc_table[vring->last_avail_idx & vring->qsz_mask].flags & VRING_DESC_F_NEXT)
...
{
    if (PREDICT_FALSE((vring->last_avail_idx & vring->qsz_mask) == 0)) vring->avail_wrap_counter ^= VRING_DESC_F_AVAIL;
...
static_always_inline void vhost_user_dequeue_descs(vhost_user_vring_t *rxvq, vnet_virtio_net_hdr_mrg_rxbuf_t *hdr, u16 *n_descs_processed)
{
    u16 i;
