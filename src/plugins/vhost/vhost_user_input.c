                                              ...
                                              u16                 last_avail_idx)
{
    ...
    if (!(txvq->desc[desc_current].flags & VRING_DESC_F_NEXT) && !(txvq->desc[desc_current].flags & VRING_DESC_F_INDIRECT)) {
...
static_always_inline u32 vhost_user_input_copy(vhost_user_intf_t *vui, vhost_copy_t *cpy, u16 copy_len, u32 *map_hint)
{
    ...
    if (PREDICT_TRUE(copy_len >= 4)) {
        if (PREDICT_FALSE(!(src2 = map_guest_mem(vui, cpy[0].src, map_hint)))) return 1;
        if (PREDICT_FALSE(!(src3 = map_guest_mem(vui, cpy[1].src, map_hint)))) return 1;

        while (PREDICT_TRUE(copy_len >= 4)) {
            src0 = src2;
            src1 = src3;

            if (PREDICT_FALSE(!(src2 = map_guest_mem(vui, cpy[2].src, map_hint)))) return 1;
            if (PREDICT_FALSE(!(src3 = map_guest_mem(vui, cpy[3].src, map_hint)))) return 1;
    ...
    while (copy_len) {
        if (PREDICT_FALSE(!(src0 = map_guest_mem(vui, cpy->src, map_hint)))) return 1;
...
static_always_inline u32 vhost_user_rx_discard_packet(vlib_main_t *vm, vhost_user_intf_t *vui, vhost_user_vring_t *txvq, u32 discard_max)
{
    ...
    while (discarded_packets != discard_max) {
        if (avail_idx == last_avail_idx) goto out;
...
static_always_inline void vhost_user_handle_rx_offload(vlib_buffer_t *b0, u8 *b0_data, vnet_virtio_net_hdr_t *hdr)
{
    u8                   l4_hdr_sz = 0;
    u8                   l4_proto  = 0;
    ...
    b0->flags |= (VNET_BUFFER_F_L2_HDR_OFFSET_VALID | VNET_BUFFER_F_L3_HDR_OFFSET_VALID | VNET_BUFFER_F_L4_HDR_OFFSET_VALID);
    ...
    if (l4_proto == IP_PROTOCOL_TCP) {
        tcp_header_t *tcp = (tcp_header_t *) (b0_data + vnet_buffer(b0)->l4_hdr_offset);
...
static_always_inline void vhost_user_input_do_interrupt(vlib_main_t *vm, vhost_user_intf_t *vui, vhost_user_vring_t *txvq, vhost_user_vring_t *rxvq)
{
    f64 now = vlib_time_now(vm);

    if ((txvq->n_since_last_int) && (txvq->int_deadline < now)) vhost_user_send_call(vm, vui, txvq);

    if ((rxvq->n_since_last_int) && (rxvq->int_deadline < now)) vhost_user_send_call(vm, vui, rxvq);
}
                                                       ...
                                                       u32                 *n_left_to_next)
{
    ...
    if (PREDICT_FALSE(vnet_have_features(feature_arc_idx, vui->sw_if_index))) {
        ...
        *current_config_index = vec_elt(cm->config_index_by_sw_if_index, vui->sw_if_index);
...
vhost_user_if_input(vlib_main_t *vm, vhost_user_main_t *vum, vhost_user_intf_t *vui, u16 qid, vlib_node_runtime_t *node, u8 enable_csum)
{
    ...
    if (PREDICT_FALSE(txvq->avail == 0)) goto done;
    ...
    if (PREDICT_FALSE(txvq->avail->flags & 0xFFFE)) goto done;

    n_left = (u16) (txvq->avail->idx - txvq->last_avail_idx);

    /* nothing to do */
    if (PREDICT_FALSE(n_left == 0)) goto done;
    ...
    if (PREDICT_FALSE(!vui->admin_up || !(txvq->enabled))) {
         ...
         */
        vhost_user_rx_discard_packet(vm, vui, txvq, VHOST_USER_DOWN_DISCARD_COUNT);
        goto done;
    }
    ...
    if (PREDICT_FALSE(n_left == (mask + 1))) {
         ...
         */
        vlib_error_count(vm, node->node_index, VHOST_USER_INPUT_FUNC_ERROR_FULL_RX_QUEUE, 1);
    }

    if (n_left > VLIB_FRAME_SIZE) n_left = VLIB_FRAME_SIZE;
     ...
     */
    if (PREDICT_FALSE(cpu->rx_buffers_len < n_left + 1 || cpu->rx_buffers_len < 40)) {
        u32 curr_len = cpu->rx_buffers_len;
        cpu->rx_buffers_len += vlib_buffer_alloc(vm, cpu->rx_buffers + curr_len, VHOST_USER_RX_BUFFERS_N - curr_len);

        if (PREDICT_FALSE(cpu->rx_buffers_len < VHOST_USER_RX_BUFFER_STARVATION)) {
            /* In case of buffer starvation, discard some packets from the queue
             * and log the event.
             * We keep doing best effort for the remaining packets. */
            u32 flush = (n_left + 1 > cpu->rx_buffers_len) ? n_left + 1 - cpu->rx_buffers_len : 1;
            flush     = vhost_user_rx_discard_packet(vm, vui, txvq, flush);

            n_left -= flush;
            vlib_increment_simple_counter(vnet_main.interface_main.sw_if_counters + VNET_INTERFACE_COUNTER_DROP,
                                          vm->thread_index,
                                          vui->sw_if_index,
                                          flush);

            vlib_error_count(vm, vhost_user_input_node.index, VHOST_USER_INPUT_FUNC_ERROR_NO_BUFFER, flush);
        }
    }

    vhost_user_input_setup_frame(vm, node, vui, &current_config_index, &next_index, &to_next, &n_left_to_next);
    ...
    while (n_left > 0) {
        ...
        vlib_prefetch_buffer_with_index(vm, cpu->rx_buffers[cpu->rx_buffers_len - 1], LOAD);
        ...
        if (PREDICT_FALSE(n_trace > 0
                          && vlib_trace_buffer(vm,
                                               ...
                                               b_head,
                                               /* follow_chain */ 0))) {
            vhost_trace_t *t0 = vlib_add_trace(vm, node, b_head, sizeof(t0[0]));
        ...
        if (txvq->desc[desc_current].flags & VRING_DESC_F_INDIRECT) {
            ...
            if (PREDICT_FALSE(desc_table == 0)) {
                vlib_error_count(vm, node->node_index, VHOST_USER_INPUT_FUNC_ERROR_MMAP_FAIL, 1);
                goto out;
            }
        }
        ...
        if (enable_csum) {
            ...
            if (hdr->hdr.flags & VIRTIO_NET_HDR_F_NEEDS_CSUM) {
                if ((desc_data_offset == desc_table[desc_current].len) && (desc_table[desc_current].flags & VRING_DESC_F_NEXT)) {
                    current = desc_table[desc_current].next;
                    b_data  = map_guest_mem(vui, desc_table[current].addr, &map_hint);
        ...
        while (1) {
            ...
            if (desc_data_offset == desc_table[desc_current].len) {
                if (PREDICT_FALSE(desc_table[desc_current].flags & VRING_DESC_F_NEXT)) {
            ...
            cpy->dst        = (uword) (vlib_buffer_get_current(b_current) + b_current->current_length);
    ...
    out:
        ...
        b_head->total_length_not_including_first_buffer -= b_head->current_length;
        ...
        if (PREDICT_FALSE(copy_len >= VHOST_USER_RX_COPY_THRESHOLD)) {
            if (PREDICT_FALSE(vhost_user_input_copy(vui, cpu->copy, copy_len, &map_hint))) {
                vlib_error_count(vm, node->node_index, VHOST_USER_INPUT_FUNC_ERROR_MMAP_FAIL, 1);
            }
            copy_len = 0;
...
stop:
    ...
    if ((txvq->callfd_idx != ~0) && !(txvq->avail->flags & VRING_AVAIL_F_NO_INTERRUPT)) {
    ...
    vlib_increment_combined_counter(vnet_main.interface_main.combined_sw_if_counters + VNET_INTERFACE_COUNTER_RX,
                                    vm->thread_index,
                                    vui->sw_if_index,
                                    n_rx_packets,
                                    n_rx_bytes);
...
static_always_inline void vhost_user_mark_desc_consumed(vhost_user_intf_t *vui, vhost_user_vring_t *txvq, u16 desc_head, u16 n_descs_processed)
{
    ...
    for (desc_idx = 0; desc_idx < n_descs_processed; desc_idx++) {
        if (txvq->used_wrap_counter)
            desc_table[(desc_head + desc_idx) & mask].flags |= (VRING_DESC_F_AVAIL | VRING_DESC_F_USED);
        else
            desc_table[(desc_head + desc_idx) & mask].flags &= ~(VRING_DESC_F_AVAIL | VRING_DESC_F_USED);
...
static_always_inline void vhost_user_rx_trace_packed(vhost_trace_t *t, vhost_user_intf_t *vui, u16 qid, vhost_user_vring_t *txvq, u16 desc_current)
{
    ...
    if (txvq->packed_desc[desc_current].flags & VRING_DESC_F_INDIRECT) {
        t->virtio_ring_flags |= 1 << VIRTIO_TRACE_F_INDIRECT;
        /* Header is the first here */
        hdr_desc = map_guest_mem(vui, txvq->packed_desc[desc_current].addr, &hint);
    }
    if (txvq->packed_desc[desc_current].flags & VRING_DESC_F_NEXT) t->virtio_ring_flags |= 1 << VIRTIO_TRACE_F_SIMPLE_CHAINED;

    if (!(txvq->packed_desc[desc_current].flags & VRING_DESC_F_NEXT) && !(txvq->packed_desc[desc_current].flags & VRING_DESC_F_INDIRECT))
        t->virtio_ring_flags |= 1 << VIRTIO_TRACE_F_SINGLE_DESC;
    ...
    if (!hdr_desc || !(hdr = map_guest_mem(vui, hdr_desc->addr, &hint)))
        ...
        u32 len = vui->virtio_net_hdr_sz;
        clib_memcpy_fast(&t->hdr, hdr, len > hdr_desc->len ? hdr_desc->len : len);
    }
}
...
static_always_inline u32 vhost_user_rx_discard_packet_packed(vlib_main_t *vm, vhost_user_intf_t *vui, vhost_user_vring_t *txvq, u32 discard_max)
{
     ...
     * Therefore, discarding a packet is like discarding a descriptor.
     */
    while ((discarded_packets != discard_max) && vhost_user_packed_desc_available(txvq, desc_current)) {
        vhost_user_advance_last_avail_idx(txvq);
        discarded_packets++;
        desc_current = (desc_current + 1) & mask;
    }

    if (PREDICT_TRUE(discarded_packets)) vhost_user_mark_desc_consumed(vui, txvq, desc_head, discarded_packets);
    return (discarded_packets);
}
...
static_always_inline u32 vhost_user_input_copy_packed(vhost_user_intf_t *vui, vhost_copy_t *cpy, u16 copy_len, u32 *map_hint)
{
    ...
    if (PREDICT_TRUE(copy_len >= 8)) {
        ...
        if (PREDICT_FALSE(bad)) goto one_by_one;
                                               ...
                                               u32                             *map_hint)
{
    ...
    if (PREDICT_FALSE(hdr == 0))
        ...
        if (desc_data_offset == desc_table[desc_current].len) {
            desc_current = (desc_current + 1) & mask;
            b_data       = map_guest_mem(vui, desc_table[desc_current].addr, map_hint);
            if (PREDICT_FALSE(b_data == 0))
                rc = VHOST_USER_INPUT_FUNC_ERROR_MMAP_FAIL;
            else
                vhost_user_handle_rx_offload(b_head, b_data, &hdr->hdr);
...
static_always_inline u32 vhost_user_compute_buffers_required(u32 desc_len, u32 buffer_data_size)
{
    ...
    if (PREDICT_TRUE(buffer_data_size == 2048)) {
        ...
        if ((desc_len & 2047) != 0) buffers_required++;
        return (buffers_required);
    }

    if (desc_len < buffer_data_size) return 1;
...
static_always_inline u32
vhost_user_compute_indirect_desc_len(vhost_user_intf_t *vui, vhost_user_vring_t *txvq, u32 buffer_data_size, u16 desc_current, u32 *map_hint)
{
    vnet_virtio_vring_packed_desc_t *desc_table       = txvq->packed_desc;
    u32                              desc_len         = 0;
    ...
    if (PREDICT_FALSE(desc_table == 0)) return 0;

    for (desc_idx = 0; desc_idx < n_descs; desc_idx++)
        desc_len += desc_table[desc_idx].len;

    if (PREDICT_TRUE(desc_len > desc_data_offset)) desc_len -= desc_data_offset;

    return vhost_user_compute_buffers_required(desc_len, buffer_data_size);
}

static_always_inline u32
vhost_user_compute_chained_desc_len(vhost_user_intf_t *vui, vhost_user_vring_t *txvq, u32 buffer_data_size, u16 *current, u16 *n_left)
{
    vnet_virtio_vring_packed_desc_t *desc_table = txvq->packed_desc;
    u32                              desc_len   = 0;
    u16                              mask       = txvq->qsz_mask;
    ...
    *current = (*current + 1) & mask;
    ...
    if (PREDICT_TRUE(desc_len > vui->virtio_net_hdr_sz)) desc_len -= vui->virtio_net_hdr_sz;

    return vhost_user_compute_buffers_required(desc_len, buffer_data_size);
}

static_always_inline void vhost_user_assemble_packet(vnet_virtio_vring_packed_desc_t *desc_table,
                                                     u16                             *desc_idx,
                                                     vlib_buffer_t                   *b_head,
...
static_always_inline u32
vhost_user_if_input_packed(vlib_main_t *vm, vhost_user_main_t *vum, vhost_user_intf_t *vui, u16 qid, vlib_node_runtime_t *node, u8 enable_csum)
{
    vhost_user_vring_t              *txvq             = &vui->vrings[VHOST_VRING_IDX_TX(qid)];
    vnet_feature_main_t             *fm               = &feature_main;
    u8                               feature_arc_idx  = fm->device_input_feature_arc_index;
    ...
    /* The descriptor table is not ready yet */
    if (PREDICT_FALSE(txvq->packed_desc == 0)) goto done;
    ...
    if (vhost_user_packed_desc_available(txvq, desc_current) == 0) goto done;
    ...
    if (PREDICT_FALSE(!vui->admin_up || !vui->is_ready || !(txvq->enabled))) {
         ...
         * and discard any TX packets."
         */
        rv = vhost_user_rx_discard_packet_packed(vm, vui, txvq, VHOST_USER_DOWN_DISCARD_COUNT);
        vlib_error_count(vm, vhost_user_input_node.index, VHOST_USER_INPUT_FUNC_ERROR_NOT_READY, rv);
        goto done;
    }
    ...
    while (vhost_user_packed_desc_available(txvq, current) && (n_left < VLIB_FRAME_SIZE)) {
        if (desc_table[current].flags & VRING_DESC_F_INDIRECT) {
            buffers_required += vhost_user_compute_indirect_desc_len(vui, txvq, buffer_data_size, current, &map_hint);
            n_left++;
            current = (current + 1) & mask;
            vhost_user_advance_last_avail_idx(txvq);
        }
        else {
            buffers_required += vhost_user_compute_chained_desc_len(vui, txvq, buffer_data_size, &current, &n_left);
        }
    }
    ...
    if (PREDICT_FALSE((buffers_required == 0) || (buffers_required > 10000))) {
        ...
        vlib_error_count(vm, vhost_user_input_node.index, VHOST_USER_INPUT_FUNC_ERROR_NO_BUFFER, rv);
        goto done;
    }
    ...
    while (n_left) {
        ...
        if (desc_table[desc_idx].flags & VRING_DESC_F_INDIRECT) {
            ...
            if (PREDICT_FALSE(desc_table == 0)
                || (enable_csum
                    && (PREDICT_FALSE(vhost_user_do_offload(vui, desc_table, desc_idx, mask, b_head, &map_hint)
                                      != VHOST_USER_INPUT_FUNC_ERROR_NO_ERROR)))) {
                vlib_error_count(vm, node->node_index, VHOST_USER_INPUT_FUNC_ERROR_MMAP_FAIL, 1);
                to_next--;
                next--;
                n_left_to_next++;
            ...
            while (n_descs) {
                vhost_user_assemble_packet(desc_table,
                                           &desc_idx,
                                           b_head,
                                           &b_current,
                                           &next,
            ...
            if (enable_csum) {
                rv = vhost_user_do_offload(vui, desc_table, desc_idx, mask, b_head, &map_hint);
    ...
    out:
        ...
        if (PREDICT_FALSE(copy_len >= VHOST_USER_RX_COPY_THRESHOLD)) {
            ...
            if (PREDICT_FALSE(rv != VHOST_USER_INPUT_FUNC_ERROR_NO_ERROR)) vlib_error_count(vm, node->node_index, rv, 1);
            copy_len = 0;
        }
    }
    vlib_put_next_frame(vm, node, next_index, n_left_to_next);
    ...
    if (PREDICT_FALSE(n_trace)) {
        ...
        b = cpu->rx_buffers_pdesc;
        while (n_trace && left) {
            if (PREDICT_TRUE(vlib_trace_buffer(vm, node, next_index, b[0], /* follow_chain */ 0))) {
                vhost_trace_t *t0;
    ...
    /* interrupt (call) handling */
    if ((txvq->callfd_idx != ~0) && (txvq->avail_event->flags != VRING_EVENT_F_DISABLE)) {
        txvq->n_since_last_int += n_rx_packets;
        if (txvq->n_since_last_int > vum->coalesce_frames) vhost_user_send_call(vm, vui, txvq);
    }
    ...
    if (PREDICT_FALSE(buffers_used < buffers_required)) vlib_buffer_free(vm, next, buffers_required - buffers_used);

done:
    return n_rx_packets;
}

VLIB_NODE_FN(vhost_user_input_node)
    ...
    vec_foreach (pve, pv) {
        ...
        if (vhost_user_is_packed_ring_supported(vui)) {
            if (vui->features & VIRTIO_FEATURE(VIRTIO_NET_F_CSUM))
                n_rx_packets += vhost_user_if_input_packed(vm, vum, vui, pve->queue_id, node, 1);
            else
                n_rx_packets += vhost_user_if_input_packed(vm, vum, vui, pve->queue_id, node, 0);
        }
        else {
            if (vui->features & VIRTIO_FEATURE(VIRTIO_NET_F_CSUM))
                n_rx_packets += vhost_user_if_input(vm, vum, vui, pve->queue_id, node, 1);
            else
                n_rx_packets += vhost_user_if_input(vm, vum, vui, pve->queue_id, node, 0);
        }
    }
