...
static void unmap_all_mem_regions(vhost_user_intf_t *vui)
{
    ...
    for (i = 0; i < vui->nregions; i++) {
        if (vui->region_mmap_addr[i] != MAP_FAILED) {
            ...
            ssize_t map_sz = (vui->regions[i].memory_size + vui->regions[i].mmap_offset + page_sz - 1) & ~(page_sz - 1);
            ...
            if (r == -1) {
                vu_log_err(vui, "failed to unmap memory region (errno %d)", errno);
            }
...
static_always_inline void vhost_user_tx_thread_placement(vhost_user_intf_t *vui, u32 qid)
{
    ...
    ASSERT((qid & 1) == 0);
    if (!rxvq->started || !rxvq->enabled) return;

    rxvq_count = (qid >> 1) + 1;
    if (rxvq->queue_index == ~0) {
        rxvq->queue_index = vnet_hw_if_register_tx_queue(vnm, vui->hw_if_index, q);
        rxvq->qid         = q;
    }
    ...
    FOR_ALL_VHOST_RXQ(q, vui)
    {
        ...
        if (rxvq->queue_index == ~0) break;
        for (u32 i = 0; i < vlib_get_n_threads(); i++)
            vnet_hw_if_tx_queue_unassign_thread(vnm, qi, i);
    }

    for (u32 i = 0; i < vlib_get_n_threads(); i++) {
        vhost_user_vring_t *rxvq = &vui->vrings[VHOST_VRING_IDX_RX(i % rxvq_count)];
        u32                 qi   = rxvq->queue_index;
...
static_always_inline void vhost_user_rx_thread_placement(vhost_user_intf_t *vui, u32 qid)
{
    ...
    if (txvq->queue_index != ~0) return;
    vnet_hw_if_set_input_node(vnm, vui->hw_if_index, vhost_user_input_node.index);
    txvq->queue_index  = vnet_hw_if_register_rx_queue(vnm, vui->hw_if_index, q, VNET_HW_IF_RXQ_THREAD_ANY);
    txvq->thread_index = vnet_hw_if_get_rx_queue_thread_index(vnm, txvq->queue_index);

    if (txvq->mode == VNET_HW_IF_RX_MODE_UNKNOWN) /* Set polling as the default */
...
static_always_inline int vhost_user_intf_ready(vhost_user_intf_t *vui)
{
    ...
    for (i = 0; i < vui->num_qid; i++)
        if (vui->vrings[i].started && vui->vrings[i].enabled) found[i & 1] = 1;
...
static_always_inline void vhost_user_update_iface_state(vhost_user_intf_t *vui)
{
    ...
    if (is_ready != vui->is_ready) {
        vu_log_debug(vui, "interface %d %s", vui->sw_if_index, is_ready ? "ready" : "down");
        if (vui->admin_up) vnet_hw_interface_set_flags(vnet_get_main(), vui->hw_if_index, is_ready ? VNET_HW_INTERFACE_FLAG_LINK_UP : 0);
...
static_always_inline void vhost_user_thread_placement(vhost_user_intf_t *vui, u32 qid)
{
    ...
    {
        if (vui->vrings[qid].queue_index == ~0) vhost_user_rx_thread_placement(vui, qid);
    }
    else
...
static clib_error_t *vhost_user_kickfd_read_ready(clib_file_t *uf)
{
    ...
    u8                              buff[8];
    vhost_user_main_t              *vum    = &vhost_user_main;
    vhost_user_intf_t              *vui    = pool_elt_at_index(vum->vhost_user_interfaces, uf->private_data >> 8);
    ...
    if (vq->started == 0) {
        ...
        if (is_txq) vnet_hw_if_set_rx_queue_file_index(vnm, vq->queue_index, vq->kickfd_idx);
    }

    if (is_txq && (vq->mode != VNET_HW_IF_RX_MODE_POLLING) && vhost_user_intf_ready(vui)) {
         ...
         */
        if (cpu->polling_q_count == 0) vnet_hw_if_rx_queue_set_int_pending(vnm, vq->queue_index);
    }
...
static_always_inline void vhost_user_vring_init(vhost_user_intf_t *vui, u32 qid)
{
     ...
     */
    if (qid == 0 || qid == 1) vring->enabled = 1;
}
...
static_always_inline void vhost_user_vring_close(vhost_user_intf_t *vui, u32 qid)
{
    ...
    if (vring->kickfd_idx != ~0) {
        clib_file_t *uf = pool_elt_at_index(file_main.file_pool, vring->kickfd_idx);
        clib_file_del(&file_main, uf);
        vring->kickfd_idx = ~0;
    }
    if (vring->callfd_idx != ~0) {
        clib_file_t *uf = pool_elt_at_index(file_main.file_pool, vring->callfd_idx);
        clib_file_del(&file_main, uf);
        vring->callfd_idx = ~0;
    }
    if (vring->errfd != -1) {
...
static clib_error_t *vhost_user_socket_read(clib_file_t *uf)
{
    ...
    if (n != VHOST_USER_MSG_HDR_SZ) {
        if (n == -1) {
            vu_log_debug(vui, "recvmsg returned error %d %s", errno, strerror(errno));
        }
        else {
    ...
    if (cmsg && (cmsg->cmsg_len > 0) && (cmsg->cmsg_level == SOL_SOCKET) && (cmsg->cmsg_type == SCM_RIGHTS)
    ...
    {
        int rv;
        rv = read(uf->file_descriptor, ((char *) &msg) + VHOST_USER_MSG_HDR_SZ, msg.size);
        if (rv < 0) {
            vu_log_debug(vui, "read failed %s", strerror(errno));
            goto close_socket;
        }
        else if (rv != msg.size) {
            vu_log_debug(vui, "message too short (read %dB should be %dB)", rv, msg.size);
            goto close_socket;
        }
    }
    ...
    switch (msg.request) {
        case VHOST_USER_GET_FEATURES:
            msg.flags |= 4;
            msg.u64 = VIRTIO_FEATURE(VIRTIO_NET_F_MRG_RXBUF) | VIRTIO_FEATURE(VIRTIO_NET_F_CTRL_VQ) | VIRTIO_FEATURE(VIRTIO_F_ANY_LAYOUT)
                      | VIRTIO_FEATURE(VIRTIO_RING_F_INDIRECT_DESC) | VIRTIO_FEATURE(VHOST_F_LOG_ALL) | VIRTIO_FEATURE(VIRTIO_NET_F_GUEST_ANNOUNCE)
                      | VIRTIO_FEATURE(VIRTIO_NET_F_MQ) | VIRTIO_FEATURE(VHOST_USER_F_PROTOCOL_FEATURES) | VIRTIO_FEATURE(VIRTIO_F_VERSION_1);
            msg.u64 &= vui->feature_mask;

            if (vui->enable_event_idx) msg.u64 |= VIRTIO_FEATURE(VIRTIO_RING_F_EVENT_IDX);
            if (vui->enable_gso) msg.u64 |= FEATURE_VIRTIO_NET_F_HOST_GUEST_TSO_FEATURE_BITS;
            if (vui->enable_packed) msg.u64 |= VIRTIO_FEATURE(VIRTIO_F_RING_PACKED);
        ...
        case VHOST_USER_SET_FEATURES:
            ...
            if (vui->features & (VIRTIO_FEATURE(VIRTIO_NET_F_MRG_RXBUF) | VIRTIO_FEATURE(VIRTIO_F_VERSION_1)))
                vui->virtio_net_hdr_sz = 12;
            else
                vui->virtio_net_hdr_sz = 10;

            vui->is_any_layout = (vui->features & VIRTIO_FEATURE(VIRTIO_F_ANY_LAYOUT)) ? 1 : 0;

            ASSERT(vui->virtio_net_hdr_sz < VLIB_BUFFER_PRE_DATA_SIZE);
            if (vui->enable_gso
                && ((vui->features & FEATURE_VIRTIO_NET_F_HOST_GUEST_TSO_FEATURE_BITS) == FEATURE_VIRTIO_NET_F_HOST_GUEST_TSO_FEATURE_BITS)) {
                vnet_hw_if_set_caps(vnm, vui->hw_if_index, VNET_HW_IF_CAP_TCP_GSO | VNET_HW_IF_CAP_TX_TCP_CKSUM | VNET_HW_IF_CAP_TX_UDP_CKSUM);
            }
            else {
                vnet_hw_if_unset_caps(vnm, vui->hw_if_index, VNET_HW_IF_CAP_TCP_GSO | VNET_HW_IF_CAP_L4_TX_CKSUM);
            }
            vnet_hw_interface_set_flags(vnm, vui->hw_if_index, 0);
            vui->is_ready = 0;
            vhost_user_update_iface_state(vui);
        ...
        case VHOST_USER_SET_MEM_TABLE:
            vu_log_debug(vui, "if %d msg VHOST_USER_SET_MEM_TABLE nregions %d", vui->hw_if_index, msg.memory.nregions);

            if ((msg.memory.nregions < 1) || (msg.memory.nregions > VHOST_MEMORY_MAX_NREGIONS)) {
                vu_log_debug(vui, "number of mem regions must be between 1 and %i", VHOST_MEMORY_MAX_NREGIONS);
                goto close_socket;
            }
            ...
            for (i = 0; i < msg.memory.nregions; i++) {
                ...
                /* align size to page */
                ssize_t map_sz = (msg.memory.regions[i].memory_size + msg.memory.regions[i].mmap_offset + page_sz - 1) & ~(page_sz - 1);

                region_mmap_addr[i] = mmap(0, map_sz, PROT_READ | PROT_WRITE, MAP_SHARED, fds[i], 0);
            ...
            unmap_all_mem_regions(vui);
            for (i = 0; i < msg.memory.nregions; i++) {
                clib_memcpy_fast(&(vui->regions[i]), &msg.memory.regions[i], sizeof(vhost_user_memory_region_t));

                vui->region_mmap_addr[i]     = region_mmap_addr[i];
                vui->region_guest_addr_lo[i] = vui->regions[i].guest_phys_addr;
                vui->region_guest_addr_hi[i] = vui->regions[i].guest_phys_addr + vui->regions[i].memory_size;
            ...
            FOR_ALL_VHOST_RX_TXQ(q, vui)
            {
                if (vui->vrings[q].desc_user_addr && vui->vrings[q].used_user_addr && vui->vrings[q].avail_user_addr) {
                    vui->vrings[q].desc  = map_user_mem(vui, vui->vrings[q].desc_user_addr);
                    vui->vrings[q].used  = map_user_mem(vui, vui->vrings[q].used_user_addr);
                    vui->vrings[q].avail = map_user_mem(vui, vui->vrings[q].avail_user_addr);
                }
            }
        ...
        case VHOST_USER_SET_VRING_NUM:
            vu_log_debug(vui, "if %d msg VHOST_USER_SET_VRING_NUM idx %d num %d", vui->hw_if_index, msg.state.index, msg.state.num);
        ...
        case VHOST_USER_SET_VRING_ADDR:
            vu_log_debug(vui, "if %d msg VHOST_USER_SET_VRING_ADDR idx %d", vui->hw_if_index, msg.state.index);
            ...
            if (msg.size < sizeof(msg.addr)) {
                vu_log_debug(vui, "vhost message is too short (%d < %d)", msg.size, sizeof(msg.addr));
                goto close_socket;
            }

            vnet_virtio_vring_desc_t  *desc  = map_user_mem(vui, msg.addr.desc_user_addr);
            vnet_virtio_vring_used_t  *used  = map_user_mem(vui, msg.addr.used_user_addr);
            vnet_virtio_vring_avail_t *avail = map_user_mem(vui, msg.addr.avail_user_addr);

            if ((desc == NULL) || (used == NULL) || (avail == NULL)) {
                vu_log_debug(vui, "failed to map user memory for hw_if_index %d", vui->hw_if_index);
                goto close_socket;
            }
            ...
            vui->vrings[msg.state.index].log_used       = (msg.addr.flags & (1 << VHOST_VRING_F_LOG)) ? 1 : 0;

            /* Spec says: If VHOST_USER_F_PROTOCOL_FEATURES has not been negotiated,
               the ring is initialized in an enabled state. */
            if (!(vui->features & VIRTIO_FEATURE(VHOST_USER_F_PROTOCOL_FEATURES))) vui->vrings[msg.state.index].enabled = 1;

            vui->vrings[msg.state.index].last_used_idx = vui->vrings[msg.state.index].last_avail_idx = vui->vrings[msg.state.index].used->idx;
            vui->vrings[msg.state.index].last_kick                                                   = vui->vrings[msg.state.index].last_used_idx;
        ...
        case VHOST_USER_RESET_OWNER:
            vu_log_debug(vui, "if %d msg VHOST_USER_RESET_OWNER", vui->hw_if_index);
            break;

        case VHOST_USER_SET_VRING_CALL:
            vu_log_debug(vui, "if %d msg VHOST_USER_SET_VRING_CALL %d", vui->hw_if_index, msg.u64);

            q = (u8) (msg.u64 & 0xFF);
            if (vui->num_qid > q) {
                /* if there is old fd, delete and close it */
                if (vui->vrings[q].callfd_idx != ~0) {
                    clib_file_t *uf = pool_elt_at_index(file_main.file_pool, vui->vrings[q].callfd_idx);
                    clib_file_del(&file_main, uf);
                ...
                if (vec_len(vui->vrings) < (VHOST_VRING_INIT_MQ_PAIR_SZ << 3))
                    ...
                    new_max_q = vec_len(vui->vrings) + (VHOST_VRING_INIT_MQ_PAIR_SZ << 2);
                if (new_max_q > (VHOST_VRING_MAX_MQ_PAIR_SZ << 1)) new_max_q = (VHOST_VRING_MAX_MQ_PAIR_SZ << 1);

                /* sync with the worker threads, vrings may move due to realloc */
                vlib_worker_thread_barrier_sync(vm);
                vec_validate_aligned(vui->vrings, new_max_q - 1, CLIB_CACHE_LINE_BYTES);
                vlib_worker_thread_barrier_release(vm);
            ...
            if (!(msg.u64 & VHOST_USER_VRING_NOFD_MASK)) {
                ...
                template.private_data     = ((vui - vhost_user_main.vhost_user_interfaces) << 8) + q;
        ...
        case VHOST_USER_SET_VRING_KICK:
            vu_log_debug(vui, "if %d msg VHOST_USER_SET_VRING_KICK %d", vui->hw_if_index, msg.u64);
            ...
            if (vui->vrings[q].kickfd_idx != ~0) {
                clib_file_t *uf = pool_elt_at_index(file_main.file_pool, vui->vrings[q].kickfd_idx);
                clib_file_del(&file_main, uf);
            ...
            if (!(msg.u64 & VHOST_USER_VRING_NOFD_MASK)) {
                ...
                template.file_descriptor  = fds[0];
                template.private_data     = (((uword) (vui - vhost_user_main.vhost_user_interfaces)) << 8) + q;
        ...
        case VHOST_USER_SET_VRING_ERR:
            vu_log_debug(vui, "if %d msg VHOST_USER_SET_VRING_ERR %d", vui->hw_if_index, msg.u64);
            ...
            if (vui->vrings[q].errfd != -1) close(vui->vrings[q].errfd);

            if (!(msg.u64 & VHOST_USER_VRING_NOFD_MASK)) {
                if (number_of_fds != 1) goto close_socket;
        ...
        case VHOST_USER_SET_VRING_BASE:
            vu_log_debug(vui, "if %d msg VHOST_USER_SET_VRING_BASE idx %d num 0x%x", vui->hw_if_index, msg.state.index, msg.state.num);
            if (msg.state.index >= vui->num_qid) {
                vu_log_debug(vui,
                             "invalid vring index VHOST_USER_SET_VRING_ADDR:"
                             " %u >= %u",
            ...
            if (vhost_user_is_packed_ring_supported(vui)) {
                ...
                vui->vrings[msg.state.index].last_avail_idx = msg.state.num & 0x7fff;
                /* avail wrap counter at bit 15 */
                vui->vrings[msg.state.index].avail_wrap_counter = !!(msg.state.num & (1 << 15));
                 ...
                 */
                vui->vrings[msg.state.index].last_used_idx     = vui->vrings[msg.state.index].last_avail_idx;
                vui->vrings[msg.state.index].last_kick         = vui->vrings[msg.state.index].last_used_idx;
                vui->vrings[msg.state.index].used_wrap_counter = vui->vrings[msg.state.index].avail_wrap_counter;

                if (vui->vrings[msg.state.index].avail_wrap_counter == 1) vui->vrings[msg.state.index].avail_wrap_counter = VRING_DESC_F_AVAIL;
            }
            vlib_worker_thread_barrier_release(vm);
            break;
        ...
        case VHOST_USER_GET_VRING_BASE:
            ...
            if (vhost_user_is_packed_ring_supported(vui)) {
                msg.state.num = (vui->vrings[msg.state.index].last_avail_idx & 0x7fff) | (!!vui->vrings[msg.state.index].avail_wrap_counter << 15);
                msg.state.num |= ((vui->vrings[msg.state.index].last_used_idx & 0x7fff) | (!!vui->vrings[msg.state.index].used_wrap_counter << 15))
                                 << 16;
            }
            msg.flags |= 4;
            ...
            vlib_worker_thread_barrier_release(vm);
            vu_log_debug(vui, "if %d msg VHOST_USER_GET_VRING_BASE idx %d num 0x%x", vui->hw_if_index, msg.state.index, msg.state.num);
            n = send(uf->file_descriptor, &msg, VHOST_USER_MSG_HDR_SZ + msg.size, 0);
            if (n != (msg.size + VHOST_USER_MSG_HDR_SZ)) {
                vu_log_debug(vui, "could not send message response");
                goto close_socket;
        ...
        case VHOST_USER_SET_LOG_BASE:
            vu_log_debug(vui, "if %d msg VHOST_USER_SET_LOG_BASE", vui->hw_if_index);
            ...
            ssize_t map_sz  = (msg.log.size + msg.log.offset + page_sz - 1) & ~(page_sz - 1);

            void *log_base_addr = mmap(0, map_sz, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
        ...
        case VHOST_USER_GET_PROTOCOL_FEATURES:
            msg.flags |= 4;
            msg.u64  = (1 << VHOST_USER_PROTOCOL_F_LOG_SHMFD) | (1 << VHOST_USER_PROTOCOL_F_MQ);
            msg.size = sizeof(msg.u64);
            vu_log_debug(vui,
        ...
        case VHOST_USER_GET_QUEUE_NUM:
            ...
            msg.size = sizeof(msg.u64);
            vu_log_debug(vui, "if %d msg VHOST_USER_GET_QUEUE_NUM - reply %d", vui->hw_if_index, msg.u64);
            n = send(uf->file_descriptor, &msg, VHOST_USER_MSG_HDR_SZ + msg.size, 0);
            if (n != (msg.size + VHOST_USER_MSG_HDR_SZ)) {
                vu_log_debug(vui, "could not send message response");
                goto close_socket;
...
static clib_error_t *vhost_user_socket_error(clib_file_t *uf)
{
    vlib_main_t       *vm  = vlib_get_main();
    vhost_user_main_t *vum = &vhost_user_main;
    vhost_user_intf_t *vui = pool_elt_at_index(vum->vhost_user_interfaces, uf->private_data);
...
static clib_error_t *vhost_user_socksvr_accept_ready(clib_file_t *uf)
{
    ...
    client_fd  = accept(uf->file_descriptor, (struct sockaddr *) &client, (socklen_t *) &client_len);

    if (client_fd < 0) return clib_error_return_unix(0, "accept");

    if (vui->clib_file_index != ~0) {
        vu_log_debug(vui, "Close client socket for vhost interface %d, fd %d", vui->sw_if_index, UNIX_GET_FD(vui->clib_file_index));
        clib_file_del(&file_main, file_main.file_pool + vui->clib_file_index);
    }

    vu_log_debug(vui, "New client socket for vhost interface %d, fd %d", vui->sw_if_index, client_fd);
...
static uword vhost_user_send_interrupt_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    while (1) {
        poll_time_remaining = vlib_process_wait_for_event_or_clock(vm, timeout);
        ...
        if (vlib_process_suspend_time_is_zero(timeout) || (timeout > vum->coalesce_time)) timeout = vum->coalesce_time;
        ...
        switch (event_type) {
            ...
            case VHOST_USER_EVENT_START_TIMER:
                stop_timer = 0;
                timeout    = 1e-3;
                if (!vlib_process_suspend_time_is_zero(poll_time_remaining)) break;
                /* fall through */
            ...
            case ~0:
                pool_foreach (vui, vum->vhost_user_interfaces) {
                    ...
                    FOR_ALL_VHOST_RX_TXQ(qid, vui)
                    {
                        ...
                        if ((next_timeout < timeout) && (next_timeout > 0.0)) timeout = next_timeout;
                    }
                }
                break;
        ...
        if (timeout < 1e-3) timeout = 1e-3;
        if (stop_timer) timeout = 3153600000.0;
    }
    return 0;
}
    ...
    while (1) {
        ...
        pool_foreach (vui, vum->vhost_user_interfaces) {
            if (vui->unix_server_index == ~0) {  // Nothing to do for server sockets
                if (vui->clib_file_index == ~0) {
                    if ((sockfd < 0) && ((sockfd = socket(AF_UNIX, SOCK_STREAM, 0)) < 0)) {
                        /*
                        ...
                        if (!vui->sock_errno || (vui->sock_errno != errno)) {
                            clib_unix_warning("Error: Could not open unix socket for %s", vui->sock_filename);
                    ...
                    if (fcntl(sockfd, F_SETFL, O_NONBLOCK) < 0) clib_unix_warning("fcntl");

                    if (connect(sockfd, (struct sockaddr *) &sun, sizeof(struct sockaddr_un)) == 0) {
                        /* Set the socket to blocking as it was before */
                        if (fcntl(sockfd, F_SETFL, 0) < 0) clib_unix_warning("fcntl2");
...
static void vhost_user_term_if(vhost_user_intf_t *vui)
{
    ...
    if (vui->unix_server_index != ~0) {
        // Close server socket
        clib_file_t *uf = pool_elt_at_index(file_main.file_pool, vui->unix_server_index);
        clib_file_del(&file_main, uf);
        vui->unix_server_index = ~0;
        unlink(vui->sock_filename);
    }

    mhash_unset(&vum->if_index_by_sock_name, vui->sock_filename, &vui->if_index);
}
...
int vhost_user_delete_if(vnet_main_t *vnm, vlib_main_t *vm, u32 sw_if_index)
{
    ...
    if (!(hwif = vnet_get_sup_hw_interface_api_visible_or_null(vnm, sw_if_index)) || hwif->dev_class_index != vhost_user_device_class.index)
        return VNET_API_ERROR_INVALID_SW_IF_INDEX;

    vui = pool_elt_at_index(vum->vhost_user_interfaces, hwif->dev_instance);

    vu_log_debug(vui, "Deleting vhost-user interface %s (instance %d)", hwif->name, hwif->dev_instance);

    FOR_ALL_VHOST_TXQ(qid, vui)
    {
        vhost_user_vring_t *txvq = &vui->vrings[qid];

        if ((txvq->mode == VNET_HW_IF_RX_MODE_POLLING) && (txvq->thread_index != ~0)) {
            vhost_cpu_t *cpu = vec_elt_at_index(vum->cpus, txvq->thread_index);
            ASSERT(cpu->polling_q_count != 0);
            cpu->polling_q_count--;
        }

        if ((vum->ifq_count > 0) && ((txvq->mode == VNET_HW_IF_RX_MODE_INTERRUPT) || (txvq->mode == VNET_HW_IF_RX_MODE_ADAPTIVE))) {
            vum->ifq_count--;
            // Stop the timer if there is no more interrupt interface/queue
            if (vum->ifq_count == 0) {
                vlib_process_signal_event(vm, vhost_user_send_interrupt_node.index, VHOST_USER_EVENT_STOP_TIMER, 0);
                break;
            }
        }
    }
    ...
    if (hwif->dev_instance < vec_len(vum->show_dev_instance_by_real_dev_instance))
...
static int vhost_user_init_server_sock(const char *sock_filename, int *sock_fd)
{
    ...
    if ((fd = socket(AF_UNIX, SOCK_STREAM, 0)) < 0) return VNET_API_ERROR_SYSCALL_ERROR_1;
...
static void vhost_user_create_ethernet(vnet_main_t *vnm, vlib_main_t *vm, vhost_user_intf_t *vui, vhost_user_create_if_args_t *args)
{
    ...
    eir.dev_instance = vui - vum->vhost_user_interfaces /* device instance */, eir.address = hwaddr;
...
static void vhost_user_vui_init(vnet_main_t *vnm, vhost_user_intf_t *vui, int server_sock_fd, vhost_user_create_if_args_t *args, u32 *sw_if_index)
{
    ...
    strncpy(vui->sock_filename, args->sock_filename, ARRAY_LEN(vui->sock_filename) - 1);
    vui->sock_errno       = 0;
    vui->is_ready         = 0;
     ...
     */
    if ((vui->enable_gso == 0)
        && ((args->feature_mask & FEATURE_VIRTIO_NET_F_HOST_GUEST_TSO_FEATURE_BITS) == (FEATURE_VIRTIO_NET_F_HOST_GUEST_TSO_FEATURE_BITS)))
        vui->enable_gso = 1;
    vhost_user_update_gso_interface_count(vui, 1 /* add */);
    mhash_set_mem(&vum->if_index_by_sock_name, vui->sock_filename, &vui->if_index, 0);

    vec_validate_aligned(vui->vrings, (VHOST_VRING_INIT_MQ_PAIR_SZ << 1) - 1, CLIB_CACHE_LINE_BYTES);
    vui->num_qid = 2;
    for (q = 0; q < vec_len(vui->vrings); q++)
    ...
    if (sw_if_index) *sw_if_index = vui->sw_if_index;
}
...
int vhost_user_create_if(vnet_main_t *vnm, vlib_main_t *vm, vhost_user_create_if_args_t *args)
{
    ...
    if_index = mhash_get(&vum->if_index_by_sock_name, (void *) args->sock_filename);
    if (if_index) {
    ...
    if (args->is_server) {
        if ((rv = vhost_user_init_server_sock(args->sock_filename, &server_sock_fd)) != 0) {
            return rv;
        }
    }
    ...
    if (args->renumber) vnet_interface_name_renumber(sw_if_idx, args->custom_dev_instance);
...
int vhost_user_modify_if(vnet_main_t *vnm, vlib_main_t *vm, vhost_user_create_if_args_t *args)
{
    ...
    if (!(hwif = vnet_get_sup_hw_interface_api_visible_or_null(vnm, args->sw_if_index)) || hwif->dev_class_index != vhost_user_device_class.index)
        return VNET_API_ERROR_INVALID_SW_IF_INDEX;

    if (args->sock_filename == NULL || !(strlen(args->sock_filename) > 0)) return VNET_API_ERROR_INVALID_ARGUMENT;
     ...
     */
    if_index = mhash_get(&vum->if_index_by_sock_name, (void *) args->sock_filename);
    if (if_index && (*if_index != vui->if_index)) return VNET_API_ERROR_IF_ALREADY_EXISTS;

    // First try to open server socket
    if (args->is_server)
        if ((rv = vhost_user_init_server_sock(args->sock_filename, &server_sock_fd)) != 0) return rv;
...
clib_error_t *vhost_user_connect_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(line_input, "feature-mask 0x%llx", &args.feature_mask))
            ;
        else if (unformat(line_input, "hwaddr %U", unformat_ethernet_address, args.hwaddr))
            args.use_custom_mac = 1;
        else if (unformat(line_input, "renumber %d", &args.custom_dev_instance))
            args.renumber = 1;
        else {
    ...
    vlib_cli_output(vm, "%U\n", format_vnet_sw_if_index_name, vnm, args.sw_if_index);
...
clib_error_t *vhost_user_delete_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "sw_if_index %d", &sw_if_index))
            ...
            vnet_hw_interface_t *hwif = vnet_get_sup_hw_interface_api_visible_or_null(vnm, sw_if_index);
            if (hwif == NULL || vhost_user_device_class.index != hwif->dev_class_index) {
...
int vhost_user_dump_ifs(vnet_main_t *vnm, vlib_main_t *vm, vhost_user_intf_details_t **out_vuids)
{
    ...
    for (i = 0; i < vec_len(hw_if_indices); i++) {
        ...
        snprintf((char *) vuid->sock_filename, sizeof(vuid->sock_filename), "%s", vui->sock_filename);
        memcpy_s(vuid->if_name, sizeof(vuid->if_name), hi->name, clib_min(vec_len(hi->name), sizeof(vuid->if_name) - 1));
...
static u8 *format_vhost_user_desc(u8 *s, va_list *args)
{
    ...
    vnet_virtio_vring_desc_t *desc_table = va_arg(*args, vnet_virtio_vring_desc_t *);
    ...
    s = format(s,
               ...
               desc_table[idx].next,
               pointer_to_uword(map_guest_mem(vui, desc_table[idx].addr, mem_hint)));
    return s;
}
...
static void vhost_user_show_desc(vlib_main_t *vm, vhost_user_intf_t *vui, int q, int show_descr, int show_verbose)
{
    ...
    if (vq->avail && vq->used)
        vlib_cli_output(vm,
                        ...
                        vq->avail->flags,
                        vhost_user_avail_event_idx(vq),
                        vq->avail->idx,
                        vq->used->flags,
                        vhost_user_used_event_idx(vq),
                        vq->used->idx);
    ...
    if (show_descr) {
        ...
        for (j = 0; j < vq->qsz_mask + 1; j++) {
            desc_table = vq->desc;
            vlib_cli_output(vm, "%U", format_vhost_user_desc, "  %-5d 0x%016lx %-5d 0x%04x %-5d 0x%016lx\n", vui, desc_table, j, &mem_hint);
            if (show_verbose && (desc_table[j].flags & VRING_DESC_F_INDIRECT)) {
                n_entries  = desc_table[j].len / sizeof(vnet_virtio_vring_desc_t);
                ...
                if (desc_table) {
                    ...
                    if (n_entries >= 20) vlib_cli_output(vm, "Skip displaying entries 20...%u\n", n_entries);
                }
            }
        }
    }
...
static void vhost_user_show_desc_packed(vlib_main_t *vm, vhost_user_intf_t *vui, int q, int show_descr, int show_verbose)
{
    ...
    vlib_cli_output(vm,
                    "  avail_event.flags %U avail_event.off_wrap %u "
                    "avail event idx %u\n",
                    format_vhost_user_event_idx_flags,
                    (u32) vq->avail_event->flags,
                    off_wrap,
                    event_idx);
    ...
    event_idx = off_wrap & 0x7fff;
    vlib_cli_output(vm,
                    "  used_event.flags %U used_event.off_wrap %u "
                    "used event idx %u\n",
                    format_vhost_user_event_idx_flags,
                    (u32) vq->used_event->flags,
                    off_wrap,
                    event_idx);
    ...
    if (show_descr) {
        ...
        for (j = 0; j < vq->qsz_mask + 1; j++) {
            ...
            vlib_cli_output(vm, "%U", format_vhost_user_packed_desc, "  %-5u 0x%016lx %-5u 0x%04x %-5u 0x%016lx\n", vui, desc_table, j, &mem_hint);
            if (show_verbose && (desc_table[j].flags & VRING_DESC_F_INDIRECT)) {
...
#undef _
    ...
    vlib_cli_output(vm, "Global:\n  coalesce frames %d time %e", vum->coalesce_frames, vum->coalesce_time);
    vlib_cli_output(vm, "  Number of rx virtqueues in interrupt mode: %d", vum->ifq_count);
    ...
    for (u32 tid = 0; tid <= vlib_num_workers(); tid++) {
        ...
        vlib_cli_output(vm, "  Thread %u: Polling queue count %u", tid, cpu->polling_q_count);
    }

    for (i = 0; i < vec_len(hw_if_indices); i++) {
        hi  = vnet_get_hw_interface(vnm, hw_if_indices[i]);
        vui = pool_elt_at_index(vum->vhost_user_interfaces, hi->dev_instance);
        vlib_cli_output(vm, "Interface: %U (ifindex %d)", format_vnet_hw_if_index_name, vnm, hw_if_indices[i], hw_if_indices[i]);
        vlib_cli_output(vm, "  Number of qids %u", vui->num_qid);
        if (vui->enable_gso) vlib_cli_output(vm, "  GSO enable");
        if (vui->enable_packed) vlib_cli_output(vm, "  Packed ring enable");
        if (vui->enable_event_idx) vlib_cli_output(vm, "  Event index enable");
        ...
        while (feat_entry->str) {
            if (vui->features & (1ULL << feat_entry->bit)) vlib_cli_output(vm, "   %s (%d)", feat_entry->str, feat_entry->bit);
            feat_entry++;
        }

        vlib_cli_output(vm, "  protocol features (0x%llx)", vui->protocol_features);
        feat_entry = (struct feat_struct *) &proto_feat_array;
        while (feat_entry->str) {
            if (vui->protocol_features & (1ULL << feat_entry->bit)) vlib_cli_output(vm, "   %s (%d)", feat_entry->str, feat_entry->bit);
            feat_entry++;
        }
        ...
        vlib_cli_output(vm,
                        " socket filename %s type %s errno \"%s\"\n\n",
                        vui->sock_filename,
        ...
        FOR_ALL_VHOST_TXQ(qid, vui)
        {
            ...
            if (txvq->qid == -1) continue;
            vlib_cli_output(vm, "   thread %d on vring %d, %U\n", txvq->thread_index, qid, format_vnet_hw_if_rx_mode, txvq->mode);
        }
        ...
        FOR_ALL_VHOST_RXQ(qid, vui)
        {
            ...
            if (rxvq->queue_index == ~0) continue;
            txq = vnet_hw_if_get_tx_queue(vnm, rxvq->queue_index);
            if (txq->threads)
                vlib_cli_output(vm,
                                "   threads %U on vring %u: %s\n",
                                format_bitmap_list,
                                txq->threads,
                                qid,
        ...
        FOR_ALL_VHOST_RX_TXQ(q, vui)
        {
            if (!vui->vrings[q].started) continue;

            vlib_cli_output(vm, "\n Virtqueue %d (%s%s)\n", q, (q & 1) ? "RX" : "TX", vui->vrings[q].enabled ? "" : " disabled");
