...
static void vl_api_create_vhost_user_if_t_handler(vl_api_create_vhost_user_if_t *mp)
{
    ...
    if (mp->disable_mrg_rxbuf) disabled_features = VIRTIO_FEATURE(VIRTIO_NET_F_MRG_RXBUF);

    if (mp->disable_indirect_desc) disabled_features |= VIRTIO_FEATURE(VIRTIO_RING_F_INDIRECT_DESC);
     ...
     */
    disabled_features |= FEATURE_VIRTIO_NET_F_HOST_GUEST_TSO_FEATURE_BITS | VIRTIO_FEATURE(VIRTIO_F_RING_PACKED);
    ...
    if (mp->use_custom_mac) mac_address_decode(mp->mac_address, (mac_address_t *) args.hwaddr);
    ...
    REPLY_MACRO2(VL_API_CREATE_VHOST_USER_IF_REPLY, ({ rmp->sw_if_index = ntohl(args.sw_if_index); }));
}
...
static void vl_api_create_vhost_user_if_v2_t_handler(vl_api_create_vhost_user_if_v2_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_CREATE_VHOST_USER_IF_V2_REPLY, ({ rmp->sw_if_index = ntohl(args.sw_if_index); }));
}
...
static void vhost_user_features_encode(u64 features, u32 *first, u32 *last)
{
...
static void send_sw_interface_vhost_user_details(vpe_api_main_t *am, vl_api_registration_t *reg, vhost_user_intf_details_t *vui, u32 context)
{
    ...
    mp->_vl_msg_id        = ntohs(REPLY_MSG_ID_BASE + VL_API_SW_INTERFACE_VHOST_USER_DETAILS);
    mp->sw_if_index       = ntohl(vui->sw_if_index);
    mp->virtio_net_hdr_sz = ntohl(vui->virtio_net_hdr_sz);
    vhost_user_features_encode(vui->features, (u32 *) &mp->features_first_32, (u32 *) &mp->features_last_32);
    mp->is_server   = vui->is_server;
    mp->num_regions = ntohl(vui->num_regions);
    mp->sock_errno  = ntohl(vui->sock_errno);
    mp->context     = context;

    strncpy((char *) mp->sock_filename, (char *) vui->sock_filename, ARRAY_LEN(mp->sock_filename) - 1);
    strncpy((char *) mp->interface_name, (char *) vui->if_name, ARRAY_LEN(mp->interface_name) - 1);
...
static void vl_api_sw_interface_vhost_user_dump_t_handler(vl_api_sw_interface_vhost_user_dump_t *mp)
{
    ...
    if (filter_sw_if_index != ~0) VALIDATE_SW_IF_INDEX(mp);

    rv = vhost_user_dump_ifs(vnm, vm, &ifaces);
    if (rv) return;

    vec_foreach (vuid, ifaces) {
        if ((filter_sw_if_index == ~0) || (vuid->sw_if_index == filter_sw_if_index)) send_sw_interface_vhost_user_details(am, reg, vuid, mp->context);
    }
...
static clib_error_t *vhost_user_api_hookup(vlib_main_t *vm)
{
    ...
    vl_api_set_msg_thread_safe(am, REPLY_MSG_ID_BASE + VL_API_CREATE_VHOST_USER_IF, 1);
    vl_api_set_msg_thread_safe(am, REPLY_MSG_ID_BASE + VL_API_CREATE_VHOST_USER_IF_V2, 1);
