...
u8 *format_gtpu_encap_trace(u8 *s, va_list *args)
{
    ...
    gtpu_encap_trace_t *t           = va_arg(*args, gtpu_encap_trace_t *);

    s = format(s, "GTPU encap to gtpu_tunnel%d tteid %u ", t->tunnel_index, t->tteid);
...
u8 *format_gtpu_tunnel(u8 *s, va_list *args)
{
    ...
    u32 encap_vrf_id = is_ipv6 ? im6->fibs[t->encap_fib_index].ft_table_id : im4->fibs[t->encap_fib_index].ft_table_id;
...
static void gtpu_tunnel_restack_dpo(gtpu_tunnel_t *t)
{
    ...
    u32                      encap_index = ip46_address_is_ip4(&t->dst) ? gtpu4_encap_node.index : gtpu6_encap_node.index;
    fib_forward_chain_type_t forw_type   = ip46_address_is_ip4(&t->dst) ? FIB_FORW_CHAIN_TYPE_UNICAST_IP4 : FIB_FORW_CHAIN_TYPE_UNICAST_IP6;
...
{
    return ((gtpu_tunnel_t *) (((char *) node) - STRUCT_OFFSET_OF(gtpu_tunnel_t, node)));
}
...
int vnet_gtpu_add_del_forwarding(vnet_gtpu_add_mod_del_tunnel_args_t *a, u32 *sw_if_indexp)
{
    gtpu_main_t  *gtm = &gtpu_main;
    bool          is_add;
    ...
    if (is_ip6) current_index_value = current_index_value_ipv6;
    ...
    if (!is_add) {
        if (current_index_value == ~0) return VNET_API_ERROR_NO_SUCH_ENTRY;
...
int vnet_gtpu_add_mod_del_tunnel(vnet_gtpu_add_mod_del_tunnel_args_t *a, u32 *sw_if_indexp)
{
    ...
    if (a->opn == GTPU_ADD_TUNNEL) {
        ...
        if (a->decap_next_index == ~0) a->decap_next_index = GTPU_INPUT_NEXT_L2_INPUT;
        if (!gtpu_decap_next_is_valid(gtm, is_ip6, a->decap_next_index)) return VNET_API_ERROR_INVALID_DECAP_NEXT;
...
#undef _
        ...
        /* copy the key */
        if (is_ip6)
            hash_set_mem_alloc(&gtm->gtpu6_tunnel_by_key, &key6, t - gtm->tunnels);
        else
        ...
        if (vec_len(gtm->free_gtpu_tunnel_hw_if_indices) > 0) {
            ...
            hw_if_index               = gtm->free_gtpu_tunnel_hw_if_indices[vec_len(gtm->free_gtpu_tunnel_hw_if_indices) - 1];
            ...
            vlib_zero_combined_counter(&im->combined_sw_if_counters[VNET_INTERFACE_COUNTER_TX], sw_if_index);
            vlib_zero_combined_counter(&im->combined_sw_if_counters[VNET_INTERFACE_COUNTER_RX], sw_if_index);
            vlib_zero_simple_counter(&im->sw_if_counters[VNET_INTERFACE_COUNTER_DROP], sw_if_index);
            vnet_interface_counter_unlock(im);
        }
        else {
            hw_if_index = vnet_register_interface(vnm, gtpu_device_class.index, t - gtm->tunnels, gtpu_hw_class.index, t - gtm->tunnels);
            hi          = vnet_get_hw_interface(vnm, hw_if_index);
        }

        /* Set gtpu tunnel output node */
        u32 encap_index = !is_ip6 ? gtpu4_encap_node.index : gtpu6_encap_node.index;
        vnet_set_interface_output_node(vnm, hw_if_index, encap_index);

        t->hw_if_index = hw_if_index;
        t->sw_if_index = sw_if_index = hi->sw_if_index;

        vec_validate_init_empty(gtm->tunnel_index_by_sw_if_index, sw_if_index, ~0);
        ...
        if (!ip46_address_is_multicast(&t->dst)) {
            ...
            t->fib_entry_index = fib_entry_track(t->encap_fib_index, &tun_dst_pfx, gtm->fib_node_type, t - gtm->tunnels, &t->sibling_index);
             ...
             */
            if (vtep_addr_ref(&gtm->vtep_table, t->encap_fib_index, &t->dst) == 1) {
                 ...
                 */
                mfib_table_entry_path_update(t->encap_fib_index, &mpfx, MFIB_SOURCE_GTPU, MFIB_ENTRY_FLAG_NONE, &path);

                path.frp_sw_if_index = a->mcast_sw_if_index;
                path.frp_flags       = FIB_ROUTE_PATH_FLAG_NONE;
                path.frp_mitf_flags  = MFIB_ITF_FLAG_ACCEPT;
                mfei                 = mfib_table_entry_path_update(t->encap_fib_index, &mpfx, MFIB_SOURCE_GTPU, MFIB_ENTRY_FLAG_NONE, &path);
        ...
        if (a->opn == GTPU_UPD_TTEID) {
            if (a->tteid == 0) return VNET_API_ERROR_INVALID_VALUE;
            t->tteid = a->tteid;
            vec_free(t->rewrite);
            ip_udp_gtpu_rewrite(t, is_ip6);
            return 0;
        ...
        set_int_l2_mode(gtm->vlib_main, vnm, MODE_L3, t->sw_if_index, 0, L2_BD_PORT_TYPE_NORMAL, 0, 0);
        ...
        else if (vtep_addr_unref(&gtm->vtep_table, t->encap_fib_index, &t->dst) == 0) {
    ...
    if (a->opn == GTPU_ADD_TUNNEL) {
        ...
        if (!is_ip6 && !udp_is_valid_dst_port(UDP_DST_PORT_GTPU, 1))
            udp_register_dst_port(gtm->vlib_main, UDP_DST_PORT_GTPU, gtpu4_input_node.index, /* is_ip4 */ 1);
        if (is_ip6 && !udp_is_valid_dst_port(UDP_DST_PORT_GTPU6, 0))
            udp_register_dst_port(gtm->vlib_main, UDP_DST_PORT_GTPU6, gtpu6_input_node.index, /* is_ip4 */ 0);
    }

    return 0;
}

int get_combined_counters(u32 sw_if_index, vlib_counter_t *result_rx, vlib_counter_t *result_tx)
{
    gtpu_main_t           *gtm = &gtpu_main;
    vnet_main_t           *vnm = gtm->vnet_main;
    vnet_interface_main_t *im  = &vnm->interface_main;
    vlib_get_combined_counter(im->combined_sw_if_counters + VNET_INTERFACE_COUNTER_RX, sw_if_index, result_rx);
    vlib_get_combined_counter(im->combined_sw_if_counters + VNET_INTERFACE_COUNTER_TX, sw_if_index, result_tx);
    return 0;
}
...
static uword get_decap_next_for_node(u32 node_index, u32 ipv4_set)
{
    gtpu_main_t *gtm        = &gtpu_main;
    vlib_main_t *vm         = gtm->vlib_main;
    uword        input_node = (ipv4_set) ? gtpu4_input_node.index : gtpu6_input_node.index;
...
#undef _
    ...
    switch (rv) {
        ...
        default:
            error = clib_error_return(0, "vnet_gtpu_add_del_tunnel returned %d", rv);
            goto done;
    }
...
static clib_error_t *show_gtpu_tunnel_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (pool_elts(gtm->tunnels) == 0) vlib_cli_output(vm, "No gtpu tunnels configured...");
...
{
    if (is_ip6)
        vnet_feature_enable_disable("ip6-unicast", "ip6-gtpu-bypass", sw_if_index, is_enable, 0, 0);
    else
        vnet_feature_enable_disable("ip4-unicast", "ip4-gtpu-bypass", sw_if_index, is_enable, 0, 0);
}
    ...
    if (is_add) {
        if (t->flow_index == ~0) {
            vnet_flow_t flow = {
                ...
                .buffer_advance      = sizeof(ethernet_header_t) + sizeof(ip4_header_t) + sizeof(udp_header_t),
...
u32 vnet_gtpu_get_tunnel_index(u32 sw_if_index)
{
    ...
    if (sw_if_index >= vec_len(gtm->tunnel_index_by_sw_if_index)) return ~0;
...
static clib_error_t *gtpu_offload_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    ;
    if (t_index == ~0) return clib_error_return(0, "%U is not a gtpu tunnel", format_vnet_sw_if_index_name, vnm, rx_sw_if_index);
    ...
    if ((t->decap_next_index != GTPU_INPUT_NEXT_IP4_INPUT) && (t->decap_next_index != GTPU_INPUT_NEXT_IP6_INPUT))
        return clib_error_return(0, "currently only inner IPv4/IPv6 protocol is supported");
    ...
    if (vnet_gtpu_add_del_rx_flow(hw_if_index, t_index, is_add)) return clib_error_return(0, "error %s flow", is_add ? "enabling" : "disabling");
...
VLIB_CLI_COMMAND(gtpu_offload_command, static) = {
    ...
    .short_help = "set flow-offload gtpu hw <inerface-name> rx <tunnel-name> [del]",
    .function   = gtpu_offload_command_fn,
};

static clib_error_t *gtpu_forward_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    ...
    switch (rv) {
        ...
        case VNET_API_ERROR_INVALID_ARGUMENT:
            error = clib_error_return(0,
                                      "one and only one of unknown-teid, unknown-type "
...
clib_error_t *gtpu_init(vlib_main_t *vm)
{
    ...
    vnet_flow_get_range(gtm->vnet_main, "gtpu", 1024 * 1024, &gtm->flow_id_start);

    /* initialize the ip6 hash */
    gtm->gtpu6_tunnel_by_key = hash_create_mem(0, sizeof(gtpu6_tunnel_key_t), sizeof(uword));
    gtm->vtep_table          = vtep_table_create();
    gtm->mcast_shared        = hash_create_mem(0, sizeof(ip46_address_t), sizeof(mcast_shared_t));
