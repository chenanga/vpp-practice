...
static u8 *format_gtpu_rx_trace(u8 *s, va_list *args)
{
    ...
    if (t->tunnel_index != ~0) {
        ...
        s = format(s, "GTPU decap error - tunnel for teid %u does not exist, ", t->teid);
    }
    s = format(s, "next %d error %d, ", t->next_index, t->error);
    s = format(s, "flags: 0x%x, type: %d, length: %d", t->header.ver_flags, t->header.type, t->header.length);
    return s;
}

always_inline u32 validate_gtpu_fib(vlib_buffer_t *b, gtpu_tunnel_t *t, u32 is_ip4)
{
    return t->encap_fib_index == vlib_buffer_get_ip_fib_index(b, is_ip4);
}

// Gets run with every input
always_inline uword gtpu_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame, u32 is_ip4)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (PREDICT_FALSE((!is_fast_track0) | (!has_space0))) {
                ...
                if (PREDICT_FALSE(!((ver0 & (GTPU_VER_MASK | GTPU_PT_BIT | GTPU_RES_BIT)) == (GTPU_V1_VER | GTPU_PT_BIT)) | (!has_space0))) {
                    ...
                    if (PREDICT_FALSE(tunnel_index0 != ~0)) goto forward0;
            ...
            if (is_ip4) {
                ...
                if (PREDICT_TRUE(ip4_0->dst_address.as_u32 == t0->src.ip4.as_u32)) goto next0; /* valid packet */
                ...
                if (PREDICT_TRUE(ip6_address_is_equal(&ip6_0->dst_address, &t0->src.ip6))) goto next0; /* valid packet */
        ...
        forward0:
            ...
            if (is_ip4) {
                ...
                if (udp0->checksum == 0) udp0->checksum = 0xffff;
            }
        ...
        next0:
            ...
            if (PREDICT_TRUE(next0 == GTPU_INPUT_NEXT_L2_INPUT)) vnet_update_l2_len(b0);
        ...
        trace0:
            ...
            if (PREDICT_FALSE((!is_fast_track1) | (!has_space1))) {
                ...
                if (PREDICT_FALSE(!((ver1 & (GTPU_VER_MASK | GTPU_PT_BIT | GTPU_RES_BIT)) == (GTPU_V1_VER | GTPU_PT_BIT)) | (!has_space1))) {
                    ...
                    if (PREDICT_FALSE(tunnel_index1 != ~0)) goto forward1;
            ...
            if (is_ip4) {
                ...
                /* Validate GTPU tunnel SIP against packet DIP */
                if (PREDICT_TRUE(ip4_1->dst_address.as_u32 == t1->src.ip4.as_u32)) goto next1; /* valid packet */
                ...
                /* Validate GTPU tunnel SIP against packet DIP */
                if (PREDICT_TRUE(ip6_address_is_equal(&ip6_1->dst_address, &t1->src.ip6))) goto next1; /* valid packet */
        ...
        forward1:
            ...
            if (is_ip4) {
                ...
                if (udp1->checksum == 0) udp1->checksum = 0xffff;
            }
        ...
        next1:
            ...
            /* Required to make the l2 tag push / pop code work on l2 subifs */
            if (PREDICT_TRUE(next1 == GTPU_INPUT_NEXT_L2_INPUT)) vnet_update_l2_len(b1);
        ...
        trace1:
            ...
            if (PREDICT_FALSE(b1->flags & VLIB_BUFFER_IS_TRACED)) {
                gtpu_rx_trace_t *tr = vlib_add_trace(vm, node, b1, sizeof(*tr));
                tr->next_index      = next1;
                tr->error           = error1;
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE((!is_fast_track0) | (!has_space0))) {
                ...
                if (PREDICT_FALSE(!((ver0 & (GTPU_VER_MASK | GTPU_PT_BIT | GTPU_RES_BIT)) == (GTPU_V1_VER | GTPU_PT_BIT)) | (!has_space0))) {
                    ...
                    if (PREDICT_FALSE(tunnel_index0 != ~0)) goto forward00;
            ...
            if (is_ip4) {
                ...
                /* Validate GTPU tunnel SIP against packet DIP */
                if (PREDICT_TRUE(ip4_0->dst_address.as_u32 == t0->src.ip4.as_u32)) goto next00; /* valid packet */
                if (PREDICT_FALSE(ip4_address_is_multicast(&ip4_0->dst_address))) {
                    key4_0.src  = ip4_0->dst_address.as_u32;
                    key4_0.teid = gtpu0->teid;
                    /* Make sure mcast GTPU tunnel exist by packet DIP and teid
            ...
            else /* !is_ip4 */
            {
                ...
                /* Validate GTPU tunnel SIP against packet DIP */
                if (PREDICT_TRUE(ip6_address_is_equal(&ip6_0->dst_address, &t0->src.ip6))) goto next00; /* valid packet */
                if (PREDICT_FALSE(ip6_address_is_multicast(&ip6_0->dst_address))) {
                    key6_0.src.as_u64[0] = ip6_0->dst_address.as_u64[0];
                    key6_0.src.as_u64[1] = ip6_0->dst_address.as_u64[1];
                    key6_0.teid          = gtpu0->teid;
    ...
    vlib_node_increment_counter(vm, is_ip4 ? gtpu4_input_node.index : gtpu6_input_node.index, GTPU_ERROR_DECAPSULATED, pkts_decapsulated);
...
always_inline uword ip_gtpu_bypass_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u32 is_ip4)
{
    gtpu_main_t         *gtm = &gtpu_main;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (udp0->dst_port != clib_host_to_net_u16(UDP_DST_PORT_GTPU)) goto exit0; /* not GTPU packet */

            /* Validate DIP against VTEPs*/
            if (is_ip4) {
#ifdef CLIB_HAVE_VEC512
                if (!vtep4_check_vector(&gtm->vtep_table, b0, ip40, &last_vtep4, &gtm->vtep4_u512))
#else
                ...
                if (!vtep6_check(&gtm->vtep_table, b0, ip60, &last_vtep6)) goto exit0; /* no local VTEP for GTPU packet */
            }
        ...
        exit0:
            ...
            if (udp1->dst_port != clib_host_to_net_u16(UDP_DST_PORT_GTPU)) goto exit1; /* not GTPU packet */

            /* Validate DIP against VTEPs*/
            if (is_ip4) {
#ifdef CLIB_HAVE_VEC512
                if (!vtep4_check_vector(&gtm->vtep_table, b1, ip41, &last_vtep4, &gtm->vtep4_u512))
#else
                ...
                if (!vtep6_check(&gtm->vtep_table, b1, ip61, &last_vtep6)) goto exit1; /* no local VTEP for GTPU packet */
            }
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (udp0->dst_port != clib_host_to_net_u16(UDP_DST_PORT_GTPU)) goto exit; /* not GTPU packet */
            ...
            if (is_ip4) {
                ...
                if (!vtep6_check(&gtm->vtep_table, b0, ip60, &last_vtep6)) goto exit; /* no local VTEP for GTPU packet */
            }
...
#define gtpu_local_need_csum_check(_b)                \
    (!(_b->flags & VNET_BUFFER_F_L4_CHECKSUM_COMPUTED \
       || (_b->flags & VNET_BUFFER_F_OFFLOAD && vnet_buffer(_b)->oflags & VNET_BUFFER_OFFLOAD_F_UDP_CKSUM)))
...
{
    ip4_header_t *ip4_hdr  = vlib_buffer_get_current(b) - sizeof(ip4_header_t) - sizeof(udp_header_t);
...
static_always_inline u8 gtpu_err_code(u8 ip_err0, u8 udp_err0, u8 csum_err0)
{
    ...
    if (ip_err0) error0 = GTPU_FLOW_ERROR_IP_HEADER_ERROR;
    if (udp_err0) error0 = GTPU_FLOW_ERROR_UDP_LENGTH_ERROR;
    if (csum_err0) error0 = GTPU_FLOW_ERROR_UDP_CHECKSUM_ERROR;
    return error0;
}

always_inline uword gtpu_flow_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    u32                    n_left_from, next_index, *from, *to_next;
    gtpu_main_t           *gtm               = &gtpu_main;
    vnet_main_t           *vnm               = gtm->vnet_main;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (PREDICT_FALSE(t0->decap_next_index != GTPU_INPUT_NEXT_IP4_INPUT) && (t0->decap_next_index != GTPU_INPUT_NEXT_IP6_INPUT)) {
        ...
        trace0:
            ...
            /* assign the next node */
            if (PREDICT_FALSE(t1->decap_next_index != GTPU_INPUT_NEXT_IP4_INPUT) && (t1->decap_next_index != GTPU_INPUT_NEXT_IP6_INPUT)) {
            ...
            if (PREDICT_FALSE(next1 == GTPU_INPUT_NEXT_L2_INPUT)) vnet_update_l2_len(b1);
    ...
    /* Do we still need this now that tunnel tx stats is kept? */
    vlib_node_increment_counter(vm, gtpu4_flow_input_node.index, GTPU_ERROR_DECAPSULATED, pkts_decapsulated);
