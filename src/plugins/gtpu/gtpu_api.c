...
static void vl_api_gtpu_add_del_tunnel_t_handler(vl_api_gtpu_add_del_tunnel_t *mp)
{
    ...
    a.encap_fib_index = fib_table_find(fib_ip_proto(is_ipv6), ntohl(mp->encap_vrf_id));
...
out:
    REPLY_MACRO2(VL_API_GTPU_ADD_DEL_TUNNEL_REPLY, ({ rmp->sw_if_index = ntohl(sw_if_index); }));
}

static void vl_api_gtpu_add_del_tunnel_v2_t_handler(vl_api_gtpu_add_del_tunnel_v2_t *mp)
{
...
static void send_gtpu_tunnel_details(gtpu_tunnel_t *t, vl_api_registration_t *reg, u32 context)
{
    ...
    ip_address_encode(&t->src, is_ipv6 ? IP46_TYPE_IP6 : IP46_TYPE_IP4, &rmp->src_address);
    ip_address_encode(&t->dst, is_ipv6 ? IP46_TYPE_IP6 : IP46_TYPE_IP4, &rmp->dst_address);

    rmp->encap_vrf_id      = is_ipv6 ? htonl(im6->fibs[t->encap_fib_index].ft_table_id) : htonl(im4->fibs[t->encap_fib_index].ft_table_id);
...
static void vl_api_gtpu_tunnel_dump_t_handler(vl_api_gtpu_tunnel_dump_t *mp)
{
    ...
    if (~0 == sw_if_index) {
        ...
        if ((sw_if_index >= vec_len(gtm->tunnel_index_by_sw_if_index)) || (~0 == gtm->tunnel_index_by_sw_if_index[sw_if_index])) {
            return;
        }
...
static void send_gtpu_tunnel_details_v2(gtpu_tunnel_t *t, vl_api_registration_t *reg, u32 context)
{
...
static void vl_api_gtpu_tunnel_v2_dump_t_handler(vl_api_gtpu_tunnel_v2_dump_t *mp)
{
    vl_api_registration_t *reg;
    gtpu_main_t           *gtm = &gtpu_main;
    gtpu_tunnel_t         *t;
...
static void vl_api_gtpu_add_del_forward_t_handler(vl_api_gtpu_add_del_forward_t *mp)
{
...
out:
    REPLY_MACRO2(VL_API_GTPU_ADD_DEL_FORWARD_REPLY, ({ rmp->sw_if_index = ntohl(sw_if_index); }));
}

static void vl_api_gtpu_get_transfer_counts_t_handler(vl_api_gtpu_get_transfer_counts_t *mp)
{
    ...
    REPLY_MACRO4(VL_API_GTPU_GET_TRANSFER_COUNTS_REPLY, extra_size, ({
                     for (; count < capacity; sw_if_index++) {
                         ...
                         get_combined_counters(sw_if_index, &result_rx, &result_tx);
                         rmp->tunnels[count].counters.packets_rx = clib_net_to_host_u64(result_rx.packets);
                         rmp->tunnels[count].counters.packets_tx = clib_net_to_host_u64(result_tx.packets);
                         rmp->tunnels[count].counters.bytes_rx   = clib_net_to_host_u64(result_rx.bytes);
                         rmp->tunnels[count].counters.bytes_tx   = clib_net_to_host_u64(result_tx.bytes);
                         count++;
                     }
