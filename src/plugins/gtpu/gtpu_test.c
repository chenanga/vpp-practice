...
static void vl_api_gtpu_add_del_tunnel_reply_t_handler(vl_api_gtpu_add_del_tunnel_reply_t *mp)
{
...
static uword api_unformat_hw_if_index(unformat_input_t *input, va_list *args)
{
    return 0;
}

static int api_sw_interface_set_gtpu_bypass(vat_main_t *vam)
{
...
static uword unformat_gtpu_decap_next(unformat_input_t *input, va_list *args)
{
...
static int api_gtpu_offload_rx(vat_main_t *vam)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "hw %U", api_unformat_hw_if_index, vam, &hw_if_index))
            ;
        else if (unformat(line_input, "rx %U", api_unformat_sw_if_index, vam, &rx_sw_if_index))
            ;
...
static int api_gtpu_add_del_tunnel(vat_main_t *vam)
{
    unformat_input_t             *line_input = vam->input;
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(line_input, "group %U %U", unformat_ip4_address, &dst.ip4, api_unformat_sw_if_index, vam, &mcast_sw_if_index)) {
            grp_set = dst_set = 1;
            ipv4_set          = 1;
        }
        else if (unformat(line_input, "group %U", unformat_ip4_address, &dst.ip4)) {
            grp_set = dst_set = 1;
            ipv4_set          = 1;
        }
        else if (unformat(line_input, "group %U %U", unformat_ip6_address, &dst.ip6, api_unformat_sw_if_index, vam, &mcast_sw_if_index)) {
            grp_set = dst_set = 1;
            ipv6_set          = 1;
        }
        else if (unformat(line_input, "group %U", unformat_ip6_address, &dst.ip6)) {
            grp_set = dst_set = 1;
            ipv6_set          = 1;
        }
        else if (unformat(line_input, "mcast_sw_if_index %u", &mcast_sw_if_index))
            ;
        else if (unformat(line_input, "encap-vrf-id %d", &encap_vrf_id))
            ;
        else if (unformat(line_input, "decap-next %U", unformat_gtpu_decap_next, &decap_next_index))
            ;
    ...
    ip_address_encode(&src, ipv6_set ? IP46_TYPE_IP6 : IP46_TYPE_IP4, &mp->src_address);
    ip_address_encode(&dst, ipv6_set ? IP46_TYPE_IP6 : IP46_TYPE_IP4, &mp->dst_address);
...
static void vl_api_gtpu_add_del_tunnel_v2_reply_t_handler(vl_api_gtpu_add_del_tunnel_v2_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    i32         retval = ntohl(mp->retval);
    if (vam->async_mode) {
...
static int api_gtpu_add_del_tunnel_v2(vat_main_t *vam)
{
    unformat_input_t                *line_input = vam->input;
...
static int api_gtpu_tunnel_update_tteid(vat_main_t *vam)
{
    unformat_input_t                  *line_input = vam->input;
...
static void vl_api_gtpu_tunnel_details_t_handler(vl_api_gtpu_tunnel_details_t *mp)
{
    vat_main_t    *vam = &vat_main;
    ip46_address_t src;
    ip46_address_t dst;
...
static void vl_api_gtpu_tunnel_v2_details_t_handler(vl_api_gtpu_tunnel_v2_details_t *mp)
{
    vat_main_t    *vam = &vat_main;
    ip46_address_t src;
    ip46_address_t dst;
    ...
    ip_address_decode(&mp->src_address, &src);
    print(vam->ofp,
          "%11d%24U%24U%14d%18d%13d%13d%19d%15d%5d%15d%17d",
          ntohl(mp->sw_if_index),
          format_ip46_address,
          &src,
          IP46_TYPE_ANY,
          ...
          mp->qfi,
          mp->is_forwarding,
          ntohl(mp->forwarding_type));
}

static void vl_api_gtpu_add_del_forward_reply_t_handler(vl_api_gtpu_add_del_forward_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    i32         retval = ntohl(mp->retval);
    if (vam->async_mode) {
...
static int api_gtpu_tunnel_dump(vat_main_t *vam)
{
    unformat_input_t          *i = vam->input;
    ...
    if (!vam->json_output) {
        print(vam->ofp,
              "%11s%24s%24s%14s%18s%13s%13s%19s",
              "sw_if_index",
              "src_address",
              "dst_address",
              "encap_vrf_id",
...
static int api_gtpu_tunnel_v2_dump(vat_main_t *vam)
{
    unformat_input_t          *i = vam->input;
    vl_api_gtpu_tunnel_dump_t *mp;
    u32                        sw_if_index;
    ...
    if (!vam->json_output) {
        print(vam->ofp,
              "%11s%24s%24s%14s%18s%13s%13s%19s%12s%5s%15s%17s",
              "sw_if_index",
              "src_address",
              "dst_address",
              "encap_vrf_id",
...
static int api_gtpu_add_del_forward(vat_main_t *vam)
{
    unformat_input_t              *line_input = vam->input;
...
static int api_gtpu_get_transfer_counts(vat_main_t *vam)
{
    unformat_input_t                  *line_input = vam->input;
...
static void vl_api_gtpu_get_transfer_counts_reply_t_handler(vl_api_gtpu_get_transfer_counts_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    i32         retval = ntohl(mp->retval);
    if (vam->async_mode) {
