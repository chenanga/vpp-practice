...
u8 *format_half_ip6_address(u8 *s, va_list *va)
{
    ...
    return format(s, "%04x:%04x:%04x:%04x", v >> 48, (v >> 32) & 0xffff, (v >> 16) & 0xffff, v & 0xffff);
}
...
#define _(i, n, st) \
    if (t == ILA_DIR_##i) return format(s, st);
...
#define _(i, n, st) \
    if (t == ILA_TYPE_##i) return format(s, st);
    ila_foreach_type
#undef _
...
static u8 *format_ila_entry(u8 *s, va_list *va)
{
    ...
    if (!e) {
        return format(s, "%-15s%=40s%=40s%+16s%+18s%+11s", "Type", "SIR Address", "ILA Address", "Checksum Mode", "Direction", "Next DPO");
    }
    else if (vnm) {
...
u8 *format_ila_ila2sir_trace(u8 *s, va_list *args)
{
    ...
    return format(s, "ILA -> SIR adj index: %d entry index: %d initial_dst: %U", t->adj_index, t->ila_index, format_ip6_address, &t->initial_dst);
}
...
static uword unformat_half_ip6_address(unformat_input_t *input, va_list *args)
{
    ...
    if (!unformat(input, "%x:%x:%x:%x", &a[0], &a[1], &a[2], &a[3])) return 0;

    if (a[0] > 0xFFFF || a[1] > 0xFFFF || a[2] > 0xFFFF || a[3] > 0xFFFF) return 0;

    *result = clib_host_to_net_u64((((u64) a[0]) << 48) | (((u64) a[1]) << 32) | (((u64) a[2]) << 16) | (((u64) a[3])));
...
static uword ila_ila2sir(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            ie0          = pool_elt_at_index(ilm->entries, vnet_buffer(p0)->ip.adj_index[VLIB_TX]);
            ie1          = pool_elt_at_index(ilm->entries, vnet_buffer(p1)->ip.adj_index[VLIB_TX]);
            ...
            vlib_validate_buffer_enqueue_x2(vm,
                                            node,
                                            next_index,
                                            to_next,
                                            n_left_to_next,
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, pi0, ie0->ila_dpo.dpoi_next_node);
        }
...
u8 *format_ila_sir2ila_trace(u8 *s, va_list *args)
{
    ...
    return format(s, "SIR -> ILA entry index: %d initial_dst: %U", t->ila_index, format_ip6_address, &t->initial_dst);
}
...
static uword ila_sir2ila(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 4 && n_left_to_next >= 2) {
            ...
            if (PREDICT_TRUE((BV(clib_bihash_search)(&ilm->id_to_entry_table, &kv0, &value0)) == 0)) {
                ie0          = &ilm->entries[value0.value];
                ila_address0 = (ie0->dir != ILA_DIR_ILA2SIR) ? &ie0->ila_address : ila_address0;
            }

            if ((BV(clib_bihash_search)(&ilm->id_to_entry_table, &kv1, &value1)) == 0) {
            ...
            if (PREDICT_FALSE(p0->flags & VLIB_BUFFER_IS_TRACED)) {
                ...
                tr->ila_index           = (ie0 != &ila_sir2ila_default_entry) ? (ie0 - ilm->entries) : ~0;
                tr->initial_dst         = ip60->dst_address;
            }

            if (PREDICT_FALSE(p1->flags & VLIB_BUFFER_IS_TRACED)) {
                ila_sir2ila_trace_t *tr = vlib_add_trace(vm, node, p1, sizeof(*tr));
                tr->ila_index           = (ie1 != &ila_sir2ila_default_entry) ? (ie1 - ilm->entries) : ~0;
                tr->initial_dst         = ip61->dst_address;
            }
...
static void ila_entry_stack(ila_entry_t *ie)
{
     ...
     */
    dpo_stack(ila_dpo_type, DPO_PROTO_IP6, &ie->ila_dpo, fib_entry_contribute_ip_forwarding(ie->next_hop_fib_entry_index));
}
...
int ila_add_del_entry(ila_add_del_entry_args_t *args)
{
    ...
    if (args->type == ILA_TYPE_IID || args->type == ILA_TYPE_LUID) {
        ...
        if (args->sir_address.as_u8[0] != 0xff || (args->sir_address.as_u8[1] & 0xf0) != 0xf0) {
            clib_warning("SIR multicast address must start with fff");
            return -1;
        }
        if (args->sir_address.as_u16[1] || args->sir_address.as_u16[2] || args->sir_address.as_u16[3] || args->sir_address.as_u16[4]
    ...
    if (!args->is_del) {
        ...
        if (!ip6_address_is_zero(&e->next_hop)) {
            ...
            e->next_hop_fib_entry_index = fib_table_entry_special_add(0, &next_hop, FIB_SOURCE_RR, FIB_ENTRY_FLAG_NONE);
            e->next_hop_child_index     = fib_entry_child_add(e->next_hop_fib_entry_index, ila_fib_node_type, e - ilm->entries);
            ...
            fib_table_entry_special_dpo_add(0, &pfx, ila_fib_src, FIB_ENTRY_FLAG_EXCLUSIVE, &dpo);
            dpo_reset(&dpo);
        ...
        if (!ip6_address_is_zero(&e->next_hop)) {
             ...
             */
            fib_entry_child_remove(e->next_hop_fib_entry_index, e->next_hop_child_index);
            fib_table_entry_delete_index(e->next_hop_fib_entry_index, FIB_SOURCE_RR);
...
{
    vnet_feature_enable_disable("ip4-unicast", "sir-to-ila", sw_if_index, !disable, 0, 0);
    return 0;
}
...
u8 *format_ila_dpo(u8 *s, va_list *va)
{
    ...
    return format(s, "ILA: idx:%d sir:%U", index, format_ip6_address, &ie->sir_address);
}
...
{
    return ((ila_entry_t *) (((char *) node) - STRUCT_OFFSET_OF(ila_entry_t, ila_fib_node)));
}
...
clib_error_t *ila_init(vlib_main_t *vm)
{
    ...
    BV(clib_bihash_init)(&ilm->id_to_entry_table, "ila id to entry index table", ilm->lookup_table_nbuckets, ilm->lookup_table_size);
...
static clib_error_t *ila_entry_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "type %U", unformat_ila_type, &args.type))
            ...
            ;
        else if (unformat(line_input, "next-hop %U", unformat_ip6_address, &args.next_hop_address))
            ;
        else if (unformat(line_input, "direction %U", unformat_ila_direction, &args.dir))
            next_hop_set = 1;
...
static clib_error_t *ila_interface_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    int ret;
    if ((ret = ila_interface(sw_if_index, disable))) return clib_error_return(0, "ila_interface returned error %d", ret);
