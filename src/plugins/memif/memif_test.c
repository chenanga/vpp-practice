...
static uword unformat_memif_queues(unformat_input_t *input, va_list *args)
{
...
static int api_memif_socket_filename_add_del(vat_main_t *vam)
{
...
/* memif_socket_filename_add_del API */
static int api_memif_socket_filename_add_del_v2(vat_main_t *vam)
{
    unformat_input_t                          *i = vam->input;
...
static void vl_api_memif_socket_filename_add_del_v2_reply_t_handler(vl_api_memif_socket_filename_add_del_v2_reply_t *mp)
{
...
static void vl_api_memif_socket_filename_add_del_reply_t_handler(vl_api_memif_socket_filename_add_del_reply_t *mp)
{
    vat_main_t *vam    = memif_test_main.vat_main;
    i32         retval = ntohl(mp->retval);
...
/* memif-create API */
static int api_memif_create(vat_main_t *vam)
{
    unformat_input_t      *i = vam->input;
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "id %u", &id))
            ...
            role = 0;
        else if (unformat(i, "slave %U", unformat_memif_queues, &rx_queues, &tx_queues))
            role = 1;
...
/* memif-create_v2 API */
static int api_memif_create_v2(vat_main_t *vam)
{
    unformat_input_t         *i = vam->input;
...
static void vl_api_memif_create_v2_reply_t_handler(vl_api_memif_create_reply_t *mp)
{
    vat_main_t *vam    = memif_test_main.vat_main;
    i32         retval = ntohl(mp->retval);
...
/* memif-delete API */
static int api_memif_delete(vat_main_t *vam)
{
    unformat_input_t      *i = vam->input;
...
/* memif-dump API */
static int api_memif_dump(vat_main_t *vam)
{
    ...
    if (!mm->ping_id) mm->ping_id = vl_msg_api_get_msg_index((u8 *) (VL_API_CONTROL_PING_CRC));
...
static int api_memif_socket_filename_dump(vat_main_t *vam)
{
    ...
    if (vam->json_output) {
        clib_warning("JSON output not supported for memif_socket_filename_dump");
        return -99;
    }
...
static void vl_api_memif_socket_filename_details_t_handler(vl_api_memif_socket_filename_details_t *mp)
{
    vat_main_t *vam = memif_test_main.vat_main;
