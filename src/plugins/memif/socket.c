...
static void memif_msg_enq_init(memif_if_t *mif)
{
    ...
    if (mif->secret) memif_msg_strlcpy(i->secret, sizeof(i->secret), mif->secret);
}
...
static void memif_msg_enq_connect(memif_if_t *mif)
{
    ...
    e->fd       = -1;
    memif_msg_snprintf(c->if_name, sizeof(c->if_name), "%U", format_memif_device_name, mif->dev_instance);
}
...
static clib_error_t *memif_msg_receive_hello(memif_if_t *mif, memif_msg_t *msg)
{
    ...
    mif->run.num_s2m_rings  = clib_min(h->max_s2m_ring + 1, mif->cfg.num_s2m_rings);
    mif->run.num_m2s_rings  = clib_min(h->max_m2s_ring + 1, mif->cfg.num_m2s_rings);
    mif->run.log2_ring_size = clib_min(h->max_log2_ring_size, mif->cfg.log2_ring_size);
...
static clib_error_t *memif_msg_receive_init(memif_if_t **mifp, memif_msg_t *msg, clib_socket_t *sock, uword socket_file_index)
{
    ...
    memif_socket_file_t *msf = vec_elt_at_index(mm->socket_files, socket_file_index);
    ...
    *mifp            = mif;
    ...
    if (mif->secret) {
        ...
        if (s == 0) return clib_error_return(0, "secret required");

        r = vec_cmp(s, mif->secret);
        vec_free(s);

        if (r) return clib_error_return(0, "incorrect secret");
    }
...
static clib_error_t *memif_msg_receive_add_region(memif_if_t *mif, memif_msg_t *msg, int fd)
{
    ...
    memif_region_t         *mr;
    if (fd < 0) return clib_error_return(0, "missing memory region fd");

    if (ar->index != vec_len(mif->regions)) return clib_error_return(0, "unexpected region index");

    if (ar->index > MEMIF_MAX_REGION) return clib_error_return(0, "too many regions");
...
static clib_error_t *memif_msg_receive_add_ring(memif_if_t *mif, memif_msg_t *msg, int fd)
{
    ...
    if (fd < 0) return clib_error_return(0, "missing ring interrupt fd");

    if (ar->private_hdr_size != 0) return clib_error_return(0, "private headers not supported");

    if (ar->flags & MEMIF_MSG_ADD_RING_FLAG_S2M) {
        if (ar->index != vec_len(mif->rx_queues)) return clib_error_return(0, "unexpected ring index");

        if (ar->index > MEMIF_MAX_S2M_RING) return clib_error_return(0, "too many rings");
        ...
        if (ar->index != vec_len(mif->tx_queues)) return clib_error_return(0, "unexpected ring index");

        if (ar->index > MEMIF_MAX_M2S_RING) return clib_error_return(0, "too many rings");
    ...
    mq->region              = ar->region;
    mq->offset              = ar->offset;
    mq->type                = (ar->flags & MEMIF_MSG_ADD_RING_FLAG_S2M) ? MEMIF_RING_S2M : MEMIF_RING_M2S;
...
static clib_error_t *memif_msg_receive_connect(memif_if_t *mif, memif_msg_t *msg)
{
    ...
    if ((err = memif_connect(mif))) return err;
...
static clib_error_t *memif_msg_receive(memif_if_t **mifp, clib_socket_t *sock, clib_file_t *uf)
{
    ...
    switch (msg.type) {
        ...
        case MEMIF_MSG_TYPE_HELLO:
            if ((err = memif_msg_receive_hello(mif, &msg))) goto error;
            if ((err = memif_init_regions_and_queues(mif))) goto error;
        ...
        case MEMIF_MSG_TYPE_INIT:
            if ((err = memif_msg_receive_init(mifp, &msg, sock, uf->private_data))) goto error;
            mif = *mifp;
            vec_reset_length(uf->description);
            uf->description = format(uf->description, "%U ctl", format_memif_device_name, mif->dev_instance);
            memif_msg_enq_ack(mif);
            break;

        case MEMIF_MSG_TYPE_ADD_REGION:
            if ((err = memif_msg_receive_add_region(mif, &msg, fd))) goto error;
            memif_msg_enq_ack(mif);
            break;

        case MEMIF_MSG_TYPE_ADD_RING:
            if ((err = memif_msg_receive_add_ring(mif, &msg, fd))) goto error;
            memif_msg_enq_ack(mif);
            break;

        case MEMIF_MSG_TYPE_CONNECT:
            if ((err = memif_msg_receive_connect(mif, &msg))) goto error;
            memif_msg_enq_connected(mif);
            break;

        case MEMIF_MSG_TYPE_CONNECTED:
            if ((err = memif_msg_receive_connected(mif, &msg))) goto error;
            break;

        case MEMIF_MSG_TYPE_DISCONNECT:
            if ((err = memif_msg_receive_disconnect(mif, &msg))) goto error;
            break;
    ...
    if (clib_fifo_elts(mif->msg_queue)) clib_file_set_data_available_to_write(&file_main, mif->sock->private_data, 1);
    return 0;
...
{
    memif_main_t        *mm  = &memif_main;
    memif_socket_file_t *msf = pool_elt_at_index(mm->socket_files, uf->private_data);
    uword               *p;
    memif_if_t          *mif  = 0;
...
static clib_error_t *memif_conn_fd_write_ready(clib_file_t *uf, memif_if_t *mif)
{
    ...
    clib_file_set_data_available_to_write(&file_main, mif->sock->private_data, 0);
    return clib_socket_sendmsg(mif->sock, &e->msg, sizeof(memif_msg_t), &e->fd, e->fd > -1 ? 1 : 0);
}
...
clib_error_t *memif_master_conn_fd_error(clib_file_t *uf)
{
    ...
    memif_log_warn(0, "Error on unknown file descriptor %d", uf->file_descriptor);
    if (uf->file_descriptor != ~0) memif_file_del(uf);
    return 0;
}
