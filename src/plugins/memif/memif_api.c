...
void vl_api_memif_socket_filename_add_del_t_handler(vl_api_memif_socket_filename_add_del_t *mp)
{
    ...
    rv = vnet_get_api_error_and_free(memif_socket_filename_add_del(is_add, socket_id, (char *) mp->socket_filename));
 ...
 * @param mp the vl_api_memif_socket_filename_add_del_t API message
 */
void vl_api_memif_socket_filename_add_del_v2_t_handler(vl_api_memif_socket_filename_add_del_v2_t *mp)
{
    ...
    if (mp->is_add && socket_id == (u32) ~0) socket_id = memif_get_unused_socket_id();

    rv = vnet_get_api_error_and_free(memif_socket_filename_add_del(mp->is_add, socket_id, socket_filename));

    vec_free(socket_filename);

reply:
    REPLY_MACRO2(VL_API_MEMIF_SOCKET_FILENAME_ADD_DEL_V2_REPLY, ({ rmp->socket_id = htonl(socket_id); }));
}
...
void vl_api_memif_create_t_handler(vl_api_memif_create_t *mp)
{
    ...
    if (strlen((char *) mp->secret) > 0) {
        ...
        strncpy((char *) args.secret, (char *) mp->secret, vec_len(args.secret));
    }
...
reply:
    REPLY_MACRO2(VL_API_MEMIF_CREATE_REPLY, ({ rmp->sw_if_index = htonl(args.sw_if_index); }));
}
 ...
 */
void vl_api_memif_create_v2_t_handler(vl_api_memif_create_v2_t *mp)
{
    memif_main_t                *mm = &memif_main;
    vlib_main_t                 *vm = vlib_get_main();
    vl_api_memif_create_reply_t *rmp;
...
reply:
    REPLY_MACRO2(VL_API_MEMIF_CREATE_V2_REPLY, ({ rmp->sw_if_index = htonl(args.sw_if_index); }));
}
...
void vl_api_memif_delete_t_handler(vl_api_memif_delete_t *mp)
{
    ...
    hi = vnet_get_sup_hw_interface_api_visible_or_null(vnm, ntohl(mp->sw_if_index));
...
static void send_memif_details(vl_api_registration_t *reg, memif_if_t *mif, vnet_sw_interface_t *swif, u8 *interface_name, u32 context)
{
    ...
    strncpy((char *) mp->if_name, (char *) interface_name, ARRAY_LEN(mp->if_name) - 1);
    ...
    mp->role        = (mif->flags & MEMIF_IF_FLAG_IS_SLAVE) ? MEMIF_ROLE_API_SLAVE : MEMIF_ROLE_API_MASTER;
    ...
    mp->flags |= (hwif->flags & VNET_HW_INTERFACE_FLAG_LINK_UP) ? IF_STATUS_API_FLAG_LINK_UP : 0;
...
void vl_api_memif_dump_t_handler(vl_api_memif_dump_t *mp)
{
    ...
    pool_foreach (mif, mm->interfaces) {
        ...
        if_name = format(if_name, "%U%c", format_vnet_sw_interface_name, vnm, swif, 0);
...
static void send_memif_socket_filename_details(vl_api_registration_t *reg, u32 socket_id, u8 *socket_filename, u32 context)
{
    ...
    mp->_vl_msg_id = htons(VL_API_MEMIF_SOCKET_FILENAME_DETAILS + mm->msg_id_base);
    mp->context    = context;

    mp->socket_id = clib_host_to_net_u32(socket_id);
    strncpy((char *) mp->socket_filename, (char *) socket_filename, ARRAY_LEN(mp->socket_filename) - 1);
