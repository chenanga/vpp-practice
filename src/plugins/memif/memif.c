...
static clib_error_t *memif_eth_set_max_frame_size(vnet_main_t *vnm, vnet_hw_interface_t *hi, u32 flags)
{
    /* nothing for now */
    return 0;
}
...
static void memif_disconnect_free_zc_queue_buffer(memif_queue_t *mq, u8 is_rx)
{
    ...
    if (is_rx)
        ...
        vlib_buffer_free_from_ring_no_next(vm, mq->buffers, start, ring_size, n_slots);
    vec_free(mq->buffers);
}
...
void memif_disconnect(memif_if_t *mif, clib_error_t *err)
{
    ...
    memif_log_debug(mif, "disconnect %u (%v)", mif->dev_instance, err ? err->what : 0);

    if (err) {
        clib_error_t *e        = 0;
        mif->local_disc_string = vec_dup(err->what);
        if (mif->sock && clib_socket_is_connected(mif->sock)) e = memif_msg_send_disconnect(mif, err);
        clib_error_free(e);
    }

    /* set interface down */
    mif->flags &= ~(MEMIF_IF_FLAG_CONNECTED | MEMIF_IF_FLAG_CONNECTING);
    if (mif->hw_if_index != ~0) vnet_hw_interface_set_flags(vnm, mif->hw_if_index, 0);
    ...
    vec_foreach (mr, mif->regions) {
        int rv;
        if (mr->is_external) continue;
        if ((rv = munmap(mr->shm, mr->region_size))) memif_log_err(mif, "munmap failed, rv = %d", rv);
        if (mr->fd > -1) close(mr->fd);
    }
...
clib_error_t *memif_connect(memif_if_t *mif)
{
    ...
    vec_foreach (mr, mif->regions) {
        ...
        if ((mr->shm = mmap(NULL, mr->region_size, PROT_READ | PROT_WRITE, MAP_SHARED, mr->fd, 0)) == MAP_FAILED) {
    ...
    with_barrier = 1;
    if (vlib_worker_thread_barrier_held()) with_barrier = 0;
    ...
    vec_foreach_index (i, mif->tx_queues) {
        ...
        mq->queue_index = vnet_hw_if_register_tx_queue(vnm, mif->hw_if_index, i);
        ...
        if (mif->flags & MEMIF_IF_FLAG_USE_DMA) {
            ...
            vec_validate_aligned(mq->dma_info, MEMIF_DMA_INFO_SIZE, CLIB_CACHE_LINE_BYTES);

            vec_foreach (dma_info, mq->dma_info) {
                vec_validate_aligned(dma_info->data.desc_data, pow2_mask(max_log2_ring_sz), CLIB_CACHE_LINE_BYTES);
                vec_validate_aligned(dma_info->data.desc_len, pow2_mask(max_log2_ring_sz), CLIB_CACHE_LINE_BYTES);
                vec_validate_aligned(dma_info->data.desc_status, pow2_mask(max_log2_ring_sz), CLIB_CACHE_LINE_BYTES);
                vec_validate_aligned(dma_info->data.copy_ops, 0, CLIB_CACHE_LINE_BYTES);
                vec_reset_length(dma_info->data.copy_ops);
                vec_validate_aligned(dma_info->data.buffers, 0, CLIB_CACHE_LINE_BYTES);
    ...
    vec_foreach_index (i, mif->rx_queues) {
        ...
        qi              = vnet_hw_if_register_rx_queue(vnm, mif->hw_if_index, i, VNET_HW_IF_RXQ_THREAD_ANY);
        mq->queue_index = qi;
        ...
        if (mq->int_fd > -1) {
            ...
            template.description     = format(0, "%U rx %u int", format_memif_device_name, mif->dev_instance, i);
            memif_file_add(&mq->int_clib_file_index, &template);
            vnet_hw_if_set_rx_queue_file_index(vnm, qi, mq->int_clib_file_index);
        }
        ti                    = vnet_hw_if_get_rx_queue_thread_index(vnm, qi);
        mq->buffer_pool_index = vlib_buffer_pool_get_default_for_numa(vm, vlib_get_main_by_index(ti)->numa_node);
    ...
    if (1 << max_log2_ring_sz > vec_len(mm->per_thread_data[0].desc_data)) {
        ...
        vec_foreach (ptd, mm->per_thread_data) {
            vec_validate_aligned(ptd->desc_data, pow2_mask(max_log2_ring_sz), CLIB_CACHE_LINE_BYTES);
            vec_validate_aligned(ptd->desc_len, pow2_mask(max_log2_ring_sz), CLIB_CACHE_LINE_BYTES);
            vec_validate_aligned(ptd->desc_status, pow2_mask(max_log2_ring_sz), CLIB_CACHE_LINE_BYTES);
        }
    }
    if (with_barrier) vlib_worker_thread_barrier_release(vm);
    ...
    vnet_hw_interface_set_flags(vnm, mif->hw_if_index, VNET_HW_INTERFACE_FLAG_LINK_UP);
    return 0;
...
{
    if (vec_len(mif->regions) == 0) return NULL;
    void *p         = mif->regions[0].shm;
    int   ring_size = sizeof(memif_ring_t) + sizeof(memif_desc_t) * (1 << mif->run.log2_ring_size);
...
clib_error_t *memif_init_regions_and_queues(memif_if_t *mif)
{
    ...
    buffer_offset =
        (mif->run.num_s2m_rings + mif->run.num_m2s_rings) * (sizeof(memif_ring_t) + sizeof(memif_desc_t) * (1 << mif->run.log2_ring_size));

    r->region_size = buffer_offset;

    if ((mif->flags & MEMIF_IF_FLAG_ZERO_COPY) == 0)
        r->region_size += mif->run.buffer_size * (1 << mif->run.log2_ring_size) * (mif->run.num_s2m_rings + mif->run.num_m2s_rings);

    if ((fd = clib_mem_vm_create_fd(CLIB_MEM_PAGE_SZ_DEFAULT, "%U region 0", format_memif_device_name, mif->dev_instance)) == -1) {
    ...
    r->shm = clib_mem_vm_map_shared(0, r->region_size, fd, 0, "memif%lu/%lu:0", msf->socket_id, mif->id);
    ...
    for (i = 0; i < mif->run.num_s2m_rings; i++) {
        ...
        if (mif->flags & MEMIF_IF_FLAG_ZERO_COPY) continue;

        for (j = 0; j < (1 << mif->run.log2_ring_size); j++) {
            u16 slot             = i * (1 << mif->run.log2_ring_size) + j;
            ring->desc[j].region = 0;
            ring->desc[j].offset = buffer_offset + (u32) (slot * mif->run.buffer_size);
    ...
    for (i = 0; i < mif->run.num_m2s_rings; i++) {
        ...
        for (j = 0; j < (1 << mif->run.log2_ring_size); j++) {
            u16 slot             = (i + mif->run.num_s2m_rings) * (1 << mif->run.log2_ring_size) + j;
            ring->desc[j].region = 0;
            ring->desc[j].offset = buffer_offset + (u32) (slot * mif->run.buffer_size);
            ring->desc[j].length = mif->run.buffer_size;
        }
    ...
    vec_validate_aligned(mif->tx_queues, mif->run.num_s2m_rings - 1, CLIB_CACHE_LINE_BYTES);
    ...
    vec_foreach_index (i, mif->tx_queues) {
        ...
        if (mif->flags & MEMIF_IF_FLAG_ZERO_COPY) vec_validate_aligned(mq->buffers, 1 << mq->log2_ring_size, CLIB_CACHE_LINE_BYTES);
    }

    ASSERT(mif->rx_queues == 0);
    vec_validate_aligned(mif->rx_queues, mif->run.num_m2s_rings - 1, CLIB_CACHE_LINE_BYTES);
...
static uword memif_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    while (1) {
        ...
        pool_foreach (mif, mm->interfaces) {
            ...
            if ((mif->flags & MEMIF_IF_FLAG_ADMIN_UP) == 0) continue;

            if (mif->flags & MEMIF_IF_FLAG_CONNECTING) continue;

            if (mif->flags & MEMIF_IF_FLAG_CONNECTED) continue;
            ...
            if (mif->flags & MEMIF_IF_FLAG_IS_SLAVE) {
                ...
                if ((err = clib_socket_init(sock))) {
                    ...
                    t.description = format(0, "%U ctl", format_memif_device_name, mif->dev_instance);
...
clib_error_t *memif_socket_filename_add_del(u8 is_add, u32 sock_id, char *sock_filename)
{
    memif_main_t        *mm = &memif_main;
    uword               *p;
    memif_socket_file_t *msf;
    ...
    if (sock_id == 0 && is_add == 0) return vnet_error(VNET_ERR_INVALID_ARGUMENT, "cannot delete socket id 0");

    if (sock_id == ~0) return vnet_error(VNET_ERR_INVALID_ARGUMENT, "socked id is not specified");

    if (is_add == 0) {
        p = hash_get(mm->socket_file_index_by_sock_id, sock_id);
        if (!p) /* Don't delete non-existent entries. */
            return vnet_error(VNET_ERR_INVALID_ARGUMENT, "socket file with id %u does not exist", sock_id);

        msf = pool_elt_at_index(mm->socket_files, *p);
        if (msf->ref_cnt > 0) return vnet_error(VNET_ERR_UNEXPECTED_INTF_STATE, "socket file '%s' is in use", msf->filename);
    ...
    if (sock_filename == 0 || sock_filename[0] == 0) return vnet_error(VNET_ERR_INVALID_ARGUMENT, "socket filename not specified");
    ...
    if (clib_socket_prefix_is_valid(sock_filename)) {
        ...
        vec_add(dir, vlib_unix_get_runtime_dir(), strlen(vlib_unix_get_runtime_dir()));
        vec_add1(dir, '/');
        ...
        if ((err = vlib_unix_recursive_mkdir(dir))) {
            clib_error_free(err);
            err = vnet_error(VNET_ERR_SYSCALL_ERROR_1, "unable to create socket dir");
            goto done;
        }

        name = format(0, "%s/%s%c", vlib_unix_get_runtime_dir(), sock_filename, 0);
    }
...
clib_error_t *memif_delete_if(vlib_main_t *vm, memif_if_t *mif)
{
    ...
    if (pool_elts(mm->interfaces) == 0) vlib_process_signal_event(vm, memif_process_node.index, MEMIF_PROCESS_EVENT_STOP, 0);
...
static void memif_prepare_dma_args(vlib_dma_config_t *args)
{
...
clib_error_t *memif_create_if(vlib_main_t *vm, memif_create_if_args_t *args)
{
    ...
    if (msf->ref_cnt > 0) {
        if ((!msf->is_listener != !args->is_master)) {
            err = vnet_error(VNET_ERR_SUBIF_ALREADY_EXISTS, "socket file cannot be used by both master and slave");
            goto done;
        }

        p = mhash_get(&msf->dev_instance_by_id, &args->id);
        if (p) {
            err = vnet_error(VNET_ERR_SUBIF_ALREADY_EXISTS, "interface already exists");
            goto done;
        }
    }
    ...
    if (msf->ref_cnt == 0) {
        mhash_init(&msf->dev_instance_by_id, sizeof(uword), sizeof(memif_interface_id_t));
    ...
    if (mm->per_thread_data == 0) {
        ...
        vec_validate_aligned(mm->per_thread_data, tm->n_vlib_mains - 1, CLIB_CACHE_LINE_BYTES);

        for (i = 0; i < tm->n_vlib_mains; i++) {
            memif_per_thread_data_t *ptd = vec_elt_at_index(mm->per_thread_data, i);
    ...
    mif->mode                                                           = args->mode;
    if (args->secret) mif->secret = vec_dup(args->secret);
    ...
    if (mif->mode == MEMIF_INTERFACE_MODE_ETHERNET) {
        ...
        mif->hw_if_index = vnet_register_interface(vnm, memif_device_class.index, mif->dev_instance, memif_ip_hw_if_class.index, mif->dev_instance);
    }
    else {
        err = vnet_error(VNET_ERR_SYSCALL_ERROR_2, "unsupported interface mode");
        goto error;
    }
    ...
    if (args->is_master == 0) {
        ...
        if (args->is_zero_copy) mif->flags |= MEMIF_IF_FLAG_ZERO_COPY;
    }

    if (args->use_dma) mif->flags |= MEMIF_IF_FLAG_USE_DMA;
    ...
    if (pool_elts(mm->interfaces) == 1) {
        vlib_process_signal_event(vm, memif_process_node.index, MEMIF_PROCESS_EVENT_START, 0);
    }
    goto done;

error:
    memif_delete_if(vm, mif);
    if (err) memif_log_err(mif, "%U", format_clib_error, err);
    return err;
...
clib_error_t *memif_interface_admin_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    ...
    vlib_process_signal_event(vnm->vlib_main, memif_process_node.index, MEMIF_PROCESS_EVENT_ADMIN_UP_DOWN, 0);
    return error;
}
