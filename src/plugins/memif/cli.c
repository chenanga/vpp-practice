...
static clib_error_t *memif_create_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!is_pow2(ring_size)) return clib_error_return(0, "ring size must be power of 2");

    if (ring_size > 32768) return clib_error_return(0, "maximum ring size is 32768");

    args.log2_ring_size = min_log2(ring_size);

    if (rx_queues > 255 || rx_queues < 1) return clib_error_return(0, "rx queue must be between 1 - 255");
    if (tx_queues > 255 || tx_queues < 1) return clib_error_return(0, "tx queue must be between 1 - 255");
...
static clib_error_t *memif_delete_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (hw == NULL || memif_device_class.index != hw->dev_class_index) return clib_error_return(0, "not a memif interface");
...
static u8 *format_memif_if_mode(u8 *s, va_list *args)
{
    ...
    if (mif->mode == MEMIF_INTERFACE_MODE_ETHERNET) return format(s, "ethernet");
    if (mif->mode == MEMIF_INTERFACE_MODE_IP) return format(s, "ip");
    if (mif->mode == MEMIF_INTERFACE_MODE_PUNT_INJECT) return format(s, "punt-inject");
...
static u8 *format_memif_queue(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U%s ring %u:\n", format_white_space, indent, (mq->type == MEMIF_RING_S2M) ? "slave-to-master" : "master-to-slave", i);
    s = format(s,
...
static u8 *format_memif_descriptor(u8 *s, va_list *args)
{
    ...
    if (ring) {
        ...
        for (slot = 0; slot < ring_size; slot++) {
            s = format(s,
                       "%U%-5d %-5d %-6d %-7d  0x%016lx %-10d 0x%016lx\n",
                       format_white_space,
                       indent,
                       slot,
                       ring->desc[slot].flags,
                       ring->desc[slot].region,
...
static clib_error_t *memif_show_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (hw_if_index = 0; hw_if_index < vec_len(hw_if_indices); hw_if_index++) {
        ...
        if (mif->remote_name) vlib_cli_output(vm, "  remote-name \"%s\"", mif->remote_name);
        if (mif->remote_if_name) vlib_cli_output(vm, "  remote-interface \"%s\"", mif->remote_if_name);
        vlib_cli_output(vm, "  socket-id %u id %u mode %U", msf->socket_id, mif->id, format_memif_if_mode, mif);
        vlib_cli_output(vm, "  flags%U", format_memif_if_flags, mif->flags);
        vlib_cli_output(vm, "  listener-fd %d conn-fd %d", msf->sock ? msf->sock->fd : 0, mif->sock ? mif->sock->fd : 0);
        vlib_cli_output(vm,
        ...
        if (mif->local_disc_string) vlib_cli_output(vm, "  local-disc-reason \"%s\"", mif->local_disc_string);
        if (mif->remote_disc_string) vlib_cli_output(vm, "  remote-disc-reason \"%s\"", mif->remote_disc_string);

        vec_foreach_index (i, mif->regions) {
            mr = vec_elt_at_index(mif->regions, i);
            vlib_cli_output(vm, "  region %u size %u fd %d", i, mr->region_size, mr->fd);
        }
        vec_foreach_index (i, mif->tx_queues) {
            mq = vec_elt_at_index(mif->tx_queues, i);
            vlib_cli_output(vm, "  %U", format_memif_queue, mq, i);
            if (show_descr) vlib_cli_output(vm, "  %U", format_memif_descriptor, mif, mq);
        }
