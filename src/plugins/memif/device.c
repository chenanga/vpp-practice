...
retry:
    ...
    while (n_left && free_slots) {
        ...
        if (PREDICT_TRUE(n_left >= 4)) vlib_prefetch_buffer_header(vlib_get_buffer(vm, buffers[3]), LOAD);
        bi0 = buffers[0];
        ...
        while (src_left) {
            if (PREDICT_FALSE(dst_left == 0)) {
                if (free_slots) {
                    ...
                    vlib_error_count(vm, node->node_index, MEMIF_TX_ERROR_ROLLBACK, 1);
                    slot = saved_slot;
                    goto no_free_slots;
                }
            }
            bytes_to_copy = clib_min(src_left, dst_left);
            memif_add_copy_op(ptd, mb0 + dst_off, bytes_to_copy, src_off, vec_len(ptd->buffers));
...
no_free_slots:
    ...
    if (n_left && n_retries--) goto retry;
...
retry:
    ...
    if (n_free >= 16) {
        vlib_buffer_free_from_ring_no_next(vm, mq->buffers, mq->last_tail & mask, ring_size, n_free);
        mq->last_tail += n_free;
    }
    ...
    next_in_chain:
        ...
        d0->offset = (void *) b0->data + b0->current_data - mif->regions[d0->region].shm;
...
CLIB_MARCH_FN(memif_tx_dma_completion_cb, void, vlib_main_t *vm, vlib_dma_batch_t *b)
{
    memif_main_t            *mm       = &memif_main;
...
static_always_inline uword memif_interface_tx_dma_inline(vlib_main_t         *vm,
                                                         vlib_node_runtime_t *node,
                                                         u32                 *buffers,
                                                         memif_if_t          *mif,
                                                         memif_ring_type_t    type,
                                                         ...
                                                         u32                  n_left)
{
    ...
    if ((((mq->dma_info_tail + 1) & dma_mask) == mq->dma_info_head) || ((mq->dma_info_head == dma_mask) && (mq->dma_info_tail == 0))) {
        if (!mq->dma_info_full)
            mq->dma_info_full = 1;
        else
            fallback = 1;
    ...
    if (PREDICT_TRUE(!fallback)) b = vlib_dma_batch_new(vm, mif->dma_tx_config);
    if (!b) return n_left;
...
no_free_slots:
    ...
    if (PREDICT_TRUE(!fallback)) {
        vlib_dma_batch_set_cookie(vm, b, ((u64) mif_id << 16) | (mq - mif->tx_queues));
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    memif_per_thread_data_t         *ptd = vec_elt_at_index(memif_main.per_thread_data, thread_index);
    uword                            n_left;

    ASSERT(vec_len(mif->tx_queues) > qid);
    mq = vec_elt_at_index(mif->tx_queues, qid);

    if (tf->shared_queue) clib_spinlock_lock(&mq->lockp);

    from   = vlib_frame_vector_args(frame);
    n_left = frame->n_vectors;
    if (mif->flags & MEMIF_IF_FLAG_ZERO_COPY)
        n_left = memif_interface_tx_zc_inline(vm, node, from, mif, mq, ptd, n_left);
    else if (mif->flags & MEMIF_IF_FLAG_IS_SLAVE)
        n_left = memif_interface_tx_inline(vm, node, from, mif, MEMIF_RING_S2M, mq, ptd, n_left);
    else {
        if ((mif->flags & MEMIF_IF_FLAG_USE_DMA) && (mif->dma_tx_config >= 0))
            n_left = memif_interface_tx_dma_inline(vm, node, from, mif, MEMIF_RING_M2S, mq, n_left);
        else
            n_left = memif_interface_tx_inline(vm, node, from, mif, MEMIF_RING_M2S, mq, ptd, n_left);
    }

    if (tf->shared_queue) clib_spinlock_unlock(&mq->lockp);

    if (n_left) vlib_error_count(vm, node->node_index, MEMIF_TX_ERROR_NO_FREE_SLOTS, n_left);
    ...
    if ((mif->flags & MEMIF_IF_FLAG_USE_DMA) && (mif->dma_tx_config >= 0)) {
        if (n_left) vlib_buffer_free(vm, from + frame->n_vectors - n_left, n_left);
    }
...
static void memif_set_interface_next_node(vnet_main_t *vnm, u32 hw_if_index, u32 node_index)
{
    ...
    mif->per_interface_next_index = vlib_node_add_next(vlib_get_main(), memif_input_node.index, node_index);
}
