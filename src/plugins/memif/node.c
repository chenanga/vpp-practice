...
static __clib_unused u8 *format_memif_input_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "memif: hw_if_index %d next-index %d", t->hw_if_index, t->next_index);
    s = format(s, "\n%Uslot: ring %u", format_white_space, indent + 2, t->ring);
    return s;
}
...
memif_trace_buffer(vlib_main_t *vm, vlib_node_runtime_t *node, memif_if_t *mif, vlib_buffer_t *b, u32 next, u16 qid, uword *n_tracep)
{
    if (PREDICT_TRUE(b != 0 && vlib_trace_buffer(vm, node, next, b, /* follow_chain */ 0))) {
...
static_always_inline void memif_add_to_chain(vlib_main_t *vm, vlib_buffer_t *b, u32 *buffers, u32 buffer_size)
{
    ...
    if (PREDICT_TRUE(bytes_left <= 0)) return;
...
static_always_inline u16 memif_parse_desc(memif_per_thread_data_t *ptd, memif_if_t *mif, memif_queue_t *mq, u16 next, u16 n_avail)
{
    ...
    while (i < n_avail) {
        ...
        if (PREDICT_FALSE((flags & MEMIF_DESC_FLAG_NEXT)) == 0) {
            n_desc = i;
            if (++n_pkts == MEMIF_RX_VECTOR_SZ) goto frame_full;
        }
    }
frame_full:
...
static_always_inline void memif_desc_status_set_err(memif_desc_status_t *p, memif_desc_status_err_code_t e)
{
    memif_desc_status_t s = {.err = 1, .err_code = e};
    p->as_u8 |= s.as_u8;
}

static_always_inline void memif_validate_desc_data(memif_per_thread_data_t *ptd, memif_if_t *mif, u16 n_desc, int is_ethernet)
{
    void               **desc_data   = ptd->desc_data;
    u16                 *desc_len    = ptd->desc_len;
    memif_desc_status_t *desc_status = ptd->desc_status;
    ...
    for (u32 i = 0; i < n_desc; i++) {
        ...
        if (region >= n_regions)
            memif_desc_status_set_err(desc_status + i, MEMIF_DESC_STATUS_ERR_BAD_REGION);
        else if (offset + len > r->region_size)
            memif_desc_status_set_err(desc_status + i, MEMIF_DESC_STATUS_ERR_REGION_OVERRUN);
        else if (is_ethernet && len > ETHERNET_MAX_PACKET_BYTES)
            memif_desc_status_set_err(desc_status + i, MEMIF_DESC_STATUS_ERR_DATA_TOO_BIG);
        else if (len == 0)
            memif_desc_status_set_err(desc_status + i, MEMIF_DESC_STATUS_ERR_ZERO_LENGTH);
        else {
...
static_always_inline u32 memif_process_desc(vlib_main_t *vm, vlib_node_runtime_t *node, memif_per_thread_data_t *ptd, memif_if_t *mif)
{
    ...
    next_slot:
        ...
        if (PREDICT_FALSE(desc_status[i].err)) {
            ...
            do {
                ...
                memif_add_copy_op(ptd, mb0 + src_off, bytes_to_copy, dst_off, n_buffers - 1);
...
static_always_inline void memif_fill_buffer_mdata_simple(vlib_node_runtime_t     *node,
                                                         ...
                                                         int                      is_ip)
{
    ...
    while (n_left) {
        ...
        vlib_buffer_copy_template(b[0], &bt);
        b[0]->current_length = dl[0];
        if (is_ip) next[0] = memif_next_from_ip_hdr(node, b[0]);
...
static_always_inline void
memif_fill_buffer_mdata(vlib_main_t *vm, vlib_node_runtime_t *node, memif_per_thread_data_t *ptd, memif_if_t *mif, u32 *bi, u16 *next, int is_ip)
{
    u16                buffer_size = vlib_buffer_get_default_data_size(vm);
    ...
    while (n_from) {
        ...
        if (is_ip) next[0] = memif_next_from_ip_hdr(node, b0);
...
static_always_inline void memif_advance_ring(memif_ring_type_t type, memif_queue_t *mq, memif_ring_t *ring, u16 cur_slot)
{
...
memif_device_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, memif_if_t *mif, memif_ring_type_t type, u16 qid, memif_interface_mode_t mode)
{
    ...
    memif_per_thread_data_t *ptd          = vec_elt_at_index(mm->per_thread_data, thread_index);
    ...
    if (n_desc != ptd->n_packets) is_simple = 0;
    ...
    if (ptd->max_desc_len > buffer_size - start_offset) is_simple = 0;

    if (ptd->xor_status != 0) is_simple = 0;
    ...
    n_alloc = vlib_buffer_alloc_from_pool(vm, ptd->buffers, n_buffers, mq->buffer_pool_index);
    if (PREDICT_FALSE(n_alloc != n_buffers)) {
        if (n_alloc) vlib_buffer_free(vm, ptd->buffers, n_alloc);
        vlib_error_count(vm, node->node_index, MEMIF_INPUT_ERROR_BUFFER_ALLOC_FAIL, 1);
        goto refill;
    }
    ...
    if (is_simple) {
        ...
        for (i = 0; i + 8 < n_pkts; i++) {
            ...
            clib_memcpy_fast(buffer_ptrs[i]->data + start_offset, desc_data[i], desc_len[i]);
        }
        for (; i < n_pkts; i++)
            clib_memcpy_fast(buffer_ptrs[i]->data + start_offset, desc_data[i], desc_len[i]);
    }
        ...
        for (i = 0; i + 8 < n_pkts; i++) {
            ...
            clib_memcpy_fast(b->data + co[i].buffer_offset, co[i].data, co[i].data_len);
        }
        for (; i < n_pkts; i++) {
            b = vlib_get_buffer(vm, ptd->buffers[co[i].buffer_vec_index]);
            clib_memcpy_fast(b->data + co[i].buffer_offset, co[i].data, co[i].data_len);
    ...
    if (PREDICT_FALSE((n_trace = vlib_get_trace_count(vm, node)))) {
        ...
        while (n_trace && n_left) {
            vlib_buffer_t       *b;
            memif_input_trace_t *tr;
            if (mode != MEMIF_INTERFACE_MODE_ETHERNET) ni = next[0];
            b = vlib_get_buffer(vm, bi[0]);
            if (PREDICT_TRUE(vlib_trace_buffer(vm, node, ni, b, /* follow_chain */ 0))) {
                tr              = vlib_add_trace(vm, node, b, sizeof(*tr));
                tr->next_index  = ni;
                tr->hw_if_index = mif->hw_if_index;
                tr->ring        = qid;
    ...
    if (mode == MEMIF_INTERFACE_MODE_ETHERNET) {
        ...
        vlib_buffer_enqueue_to_next(vm, node, to_next_bufs, nexts, ptd->n_packets);

    vlib_increment_combined_counter(vnm->interface_main.combined_sw_if_counters + VNET_INTERFACE_COUNTER_RX,
                                    thread_index,
                                    mif->sw_if_index,
                                    ptd->n_packets,
                                    ptd->n_rx_bytes);
...
static_always_inline uword
memif_device_input_zc_inline(vlib_main_t *vm, vlib_node_runtime_t *node, memif_if_t *mif, u16 qid, memif_interface_mode_t mode)
{
    vnet_main_t             *vnm = vnet_get_main();
    memif_main_t            *mm  = &memif_main;
    memif_ring_t            *ring;
    ...
    next_index = (mode == MEMIF_INTERFACE_MODE_IP) ? VNET_DEVICE_INPUT_NEXT_IP6_INPUT : VNET_DEVICE_INPUT_NEXT_ETHERNET_INPUT;
    ...
    if (cur_slot == last_slot) goto refill;
    n_slots = last_slot - cur_slot;

    /* process ring slots */
    vec_validate_aligned(ptd->buffers, MEMIF_RX_VECTOR_SZ, CLIB_CACHE_LINE_BYTES);
    while (n_slots && n_rx_packets < MEMIF_RX_VECTOR_SZ) {
        ...
        next_slot:
            ...
            cur_slot++;
            n_slots--;
            if ((d0->flags & MEMIF_DESC_FLAG_NEXT) && n_slots) goto next_slot;
        }
    }
    ...
    while (n_from) {
        ...
        while (n_from >= 8 && n_left_to_next >= 4) {
            ...
            if (mode == MEMIF_INTERFACE_MODE_IP) {
                ...
                if (PREDICT_FALSE(mif->per_interface_next_index != ~0)) {
                    ...
                    vnet_feature_start_device_input(mif->sw_if_index, &next0, b0);
                    vnet_feature_start_device_input(mif->sw_if_index, &next1, b1);
                    vnet_feature_start_device_input(mif->sw_if_index, &next2, b2);
                    vnet_feature_start_device_input(mif->sw_if_index, &next3, b3);
                }
            }
            ...
            if (PREDICT_FALSE(n_trace > 0)) {
                ...
                if (PREDICT_FALSE(n_trace > 0)) memif_trace_buffer(vm, node, mif, b1, next1, qid, &n_trace);
                if (PREDICT_FALSE(n_trace > 0)) memif_trace_buffer(vm, node, mif, b2, next2, qid, &n_trace);
                if (PREDICT_FALSE(n_trace > 0)) memif_trace_buffer(vm, node, mif, b3, next3, qid, &n_trace);
            }
        ...
        while (n_from && n_left_to_next) {
            ...
            /* trace */
            if (PREDICT_FALSE(n_trace > 0)) memif_trace_buffer(vm, node, mif, b0, next0, qid, &n_trace);
...
refill:
    ...
    if (n_slots < 32) goto done;

    memif_desc_t desc_template, *dt = &desc_template;
    clib_memset(dt, 0, sizeof(memif_desc_t));
    dt->length = buffer_length;

    n_alloc    = vlib_buffer_alloc_to_ring_from_pool(vm, mq->buffers, slot, ring_size, n_slots, mq->buffer_pool_index);
    dt->region = mq->buffer_pool_index + 1;
    offset     = (u64) mif->regions[dt->region].shm - start_offset;

    if (PREDICT_FALSE(n_alloc != n_slots)) vlib_error_count(vm, node->node_index, MEMIF_INPUT_ERROR_BUFFER_ALLOC_FAIL, 1);
    ...
    while (n_alloc) {
        ...
        if (PREDICT_FALSE(((slot + 7 > mask) || (n_alloc < 8)))) goto one_by_one;
...
CLIB_MARCH_FN(memif_dma_completion_cb, void, vlib_main_t *vm, vlib_dma_batch_t *b)
{
    ...
    /* prepare buffer template and next indices */
    i16 start_offset                                         = (dma_info->mode == MEMIF_INTERFACE_MODE_IP) ? MEMIF_IP_OFFSET : 0;
    vnet_buffer(&ptd->buffer_template)->sw_if_index[VLIB_RX] = mif->sw_if_index;
    vnet_buffer(&ptd->buffer_template)->feature_arc_index    = 0;
    ptd->buffer_template.current_data                        = start_offset;
    ptd->buffer_template.current_config_index                = 0;
    ...
    if (dma_info->mode == MEMIF_INTERFACE_MODE_ETHERNET) {
        ...
        vlib_get_new_next_frame(vm, dma_info->node, next_index, to_next_bufs, n_left_to_next);
        if (PREDICT_TRUE(next_index == VNET_DEVICE_INPUT_NEXT_ETHERNET_INPUT)) {
            vlib_next_frame_t      *nf;
            vlib_frame_t           *f;
            ethernet_input_frame_t *ef;
            nf       = vlib_node_runtime_get_next_frame(vm, dma_info->node, next_index);
            f        = vlib_get_frame(vm, nf->frame);
            f->flags = ETH_INPUT_FRAME_F_SINGLE_SW_IF_IDX;
    ...
    if (dma_info->mode == MEMIF_INTERFACE_MODE_IP)
        memif_fill_buffer_mdata(vm, dma_info->node, ptd, mif, to_next_bufs, nexts, 1);
    else
        memif_fill_buffer_mdata(vm, dma_info->node, ptd, mif, to_next_bufs, nexts, 0);
    ...
    if (PREDICT_FALSE((n_trace = vlib_get_trace_count(vm, dma_info->node)))) {
        ...
        while (n_trace && n_left) {
            vlib_buffer_t       *b;
            memif_input_trace_t *tr;
            if (dma_info->mode != MEMIF_INTERFACE_MODE_ETHERNET) ni = next[0];
            b = vlib_get_buffer(vm, bi[0]);
            if (PREDICT_TRUE(vlib_trace_buffer(vm,
                                               dma_info->node,
                                               ni,
                                               b,
                                               /* follow_chain */ 0))) {
    ...
    if (dma_info->mode == MEMIF_INTERFACE_MODE_ETHERNET) {
        ...
        vlib_buffer_enqueue_to_next(vm, dma_info->node, to_next_bufs, nexts, ptd->n_packets);
    ...
    mq->dma_info_head++;
    if (mq->dma_info_head == mq->dma_info_size) mq->dma_info_head = 0;
...
static_always_inline uword memif_device_input_inline_dma(vlib_main_t           *vm,
                                                         ...
                                                         memif_interface_mode_t mode)
{
    ...
    if ((mq->dma_info_tail + 1 == mq->dma_info_head) || ((mq->dma_info_head == mq->dma_info_size - 1) && (mq->dma_info_tail == 0))) return 0;
    ...
    dma_info->mode = mode;
    ptd            = &dma_info->data;
    vec_validate_aligned(dma_info->data.desc_len, pow2_mask(mq->log2_ring_size), CLIB_CACHE_LINE_BYTES);

    n_desc = memif_parse_desc(&dma_info->data, mif, mq, cur_slot, n_slots);
    cur_slot += n_desc;

    if (mif->mode == MEMIF_INTERFACE_MODE_ETHERNET)
        memif_validate_desc_data(&dma_info->data,
                                 mif,
                                 n_desc,
                                 /* is_ethernet */ 1);
    else
    ...
    /* allocate free buffers */
    vec_validate_aligned(dma_info->data.buffers, n_buffers - 1, CLIB_CACHE_LINE_BYTES);
    n_alloc = vlib_buffer_alloc_from_pool(vm, dma_info->data.buffers, n_buffers, mq->buffer_pool_index);
    if (PREDICT_FALSE(n_alloc != n_buffers)) {
        if (n_alloc) vlib_buffer_free(vm, dma_info->data.buffers, n_alloc);
        vlib_error_count(vm, node->node_index, MEMIF_INPUT_ERROR_BUFFER_ALLOC_FAIL, 1);
        goto done;
    }
    ...
    for (i = 0; i < n_pkts; i++) {
        b = vlib_get_buffer(vm, ptd->buffers[co[i].buffer_vec_index]);
        vlib_dma_batch_add(vm, db, b->data + co[i].buffer_offset, co[i].data, co[i].data_len);
    }
    ...
    mq->dma_info_tail++;
    if (mq->dma_info_tail == mq->dma_info_size) mq->dma_info_tail = 0;
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    for (int i = 0; i < vec_len(pv); i++) {
        ...
        qid = pv[i].queue_id;
        if ((mif->flags & MEMIF_IF_FLAG_ADMIN_UP) && (mif->flags & MEMIF_IF_FLAG_CONNECTED)) {
            if (mif->flags & MEMIF_IF_FLAG_ZERO_COPY) {
                if (mif->mode == MEMIF_INTERFACE_MODE_IP)
                    n_rx += memif_device_input_zc_inline(vm, node, mif, qid, mode_ip);
                else
                    n_rx += memif_device_input_zc_inline(vm, node, mif, qid, mode_eth);
            }
            else if (mif->flags & MEMIF_IF_FLAG_IS_SLAVE) {
                if (mif->mode == MEMIF_INTERFACE_MODE_IP)
                    n_rx += memif_device_input_inline(vm, node, mif, MEMIF_RING_M2S, qid, mode_ip);
                else
                    n_rx += memif_device_input_inline(vm, node, mif, MEMIF_RING_M2S, qid, mode_eth);
            }
            else {
                if ((mif->flags & MEMIF_IF_FLAG_USE_DMA) && (mif->dma_input_config >= 0)) {
                    if (mif->mode == MEMIF_INTERFACE_MODE_IP)
                        n_rx += memif_device_input_inline_dma(vm, node, mif, MEMIF_RING_S2M, qid, mode_ip);
                    else
                        n_rx += memif_device_input_inline_dma(vm, node, mif, MEMIF_RING_S2M, qid, mode_eth);
                }
                else {
                    if (mif->mode == MEMIF_INTERFACE_MODE_IP)
                        n_rx += memif_device_input_inline(vm, node, mif, MEMIF_RING_S2M, qid, mode_ip);
                    else
                        n_rx += memif_device_input_inline(vm, node, mif, MEMIF_RING_S2M, qid, mode_eth);
                }
            }
        }
    }
