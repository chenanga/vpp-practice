...
static void vl_api_lb_conf_t_handler(vl_api_lb_conf_t *mp)
{
    ...
    sticky_buckets_per_core = mp->sticky_buckets_per_core == ~0 ? lbm->per_cpu_sticky_buckets : ntohl(mp->sticky_buckets_per_core);
    flow_timeout            = mp->flow_timeout == ~0 ? lbm->flow_timeout : ntohl(mp->flow_timeout);

    rv = lb_conf((ip4_address_t *) &mp->ip4_src_address, (ip6_address_t *) &mp->ip6_src_address, sticky_buckets_per_core, flow_timeout);
...
static void vl_api_lb_add_del_vip_t_handler(vl_api_lb_add_del_vip_t *mp)
{
    ...
    if (mp->is_del) {
        u32 vip_index;
        if (!(rv = lb_vip_find_index(&(args.prefix), mp->pfx.len, mp->protocol, ntohs(mp->port), &vip_index))) rv = lb_vip_del(vip_index);
    }
    else {
        u32           vip_index;
...
static void vl_api_lb_add_del_vip_v2_t_handler(vl_api_lb_add_del_vip_v2_t *mp)
{
    lb_main_t              *lbm = &lb_main;
    vl_api_lb_conf_reply_t *rmp;
    int                     rv   = 0;
    ...
    if (mp->is_del) {
        ...
        if (mp->src_ip_sticky) args.src_ip_sticky = 1;
...
static void vl_api_lb_add_del_as_t_handler(vl_api_lb_add_del_as_t *mp)
{
    ...
    if ((rv = lb_vip_find_index(&vip_ip_prefix, mp->pfx.len, mp->protocol, ntohs(mp->port), &vip_index))) goto done;
...
static void send_lb_as_details(vl_api_registration_t *reg, u32 context, lb_vip_t *vip)
{
    ...
    pool_foreach (as_index, vip->as_indexes) {
        ...
        if (*as_index != 0) {
            ...
            rmp      = vl_msg_api_alloc(msg_size);
            memset(rmp, 0, msg_size);
            rmp->_vl_msg_id = htons(VL_API_LB_AS_DETAILS + lbm->msg_id_base);
            rmp->context    = context;
...
static void vl_api_lb_as_dump_t_handler(vl_api_lb_as_dump_t *mp)
{
    ...
    pool_foreach (vip, lbm->vips) {
        if (dump_all
            || ((prefix.as_u64[0] == vip->prefix.as_u64[0]) && (prefix.as_u64[1] == vip->prefix.as_u64[1]) && (mp->protocol == vip->protocol)
                && (mp->port == vip->port))) {
...
static void vl_api_lb_flush_vip_t_handler(vl_api_lb_flush_vip_t *mp)
{
    ...
    rv = lb_vip_find_index(&vip_prefix, vip_plen, mp->protocol, ntohs(mp->port), &vip_index);
