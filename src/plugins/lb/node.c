...
u8 *format_lb_nat_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "lb nat: rx_sw_if_index = %d, next_index = %d", t->rx_sw_if_index, t->next_index);
...
static_always_inline void lb_node_get_hash(lb_main_t *lbm, vlib_buffer_t *p, u8 is_input_v4, u32 *hash, u32 *vip_idx, u8 per_port_vip)
{
    ...
    *vip_idx = vnet_buffer(p)->ip.adj_index[VLIB_TX];
    ...
    if (is_input_v4) {
        ...
        if (PREDICT_TRUE(ip40->protocol == IP_PROTOCOL_TCP || ip40->protocol == IP_PROTOCOL_UDP))
            ports = ((u64) ((udp_header_t *) (ip40 + 1))->src_port << 16) | ((u64) ((udp_header_t *) (ip40 + 1))->dst_port);
        else
        ...
        if (PREDICT_TRUE(ip60->protocol == IP_PROTOCOL_TCP || ip60->protocol == IP_PROTOCOL_UDP))
            ports = ((u64) ((udp_header_t *) (ip60 + 1))->src_port << 16) | ((u64) ((udp_header_t *) (ip60 + 1))->dst_port);
        else
    ...
    if (is_input_v4) {
        ...
        if (lb_vip_is_src_ip_sticky(vip0)) {
            *hash =
                lb_hash_hash(ip60->src_address.as_u64[0], ip60->src_address.as_u64[1], ip60->dst_address.as_u64[0], ip60->dst_address.as_u64[1], 0);
        }
        else {
...
static uword lb_nodeport_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_input_v4)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (is_input_v4) {
                ip4_header_t *ip40;
                vlib_buffer_advance(p0, -(word) (sizeof(udp_header_t) + sizeof(ip4_header_t)));
                ip40  = vlib_buffer_get_current(p0);
                udp_0 = (udp_header_t *) (ip40 + 1);
            }
            else {
                ip6_header_t *ip60;
                vlib_buffer_advance(p0, -(word) (sizeof(udp_header_t) + sizeof(ip6_header_t)));
                ip60  = vlib_buffer_get_current(p0);
            ...
            // Enqueue to next
            vnet_buffer(p0)->ip.adj_index[VLIB_TX] = entry0 ? entry0[0] : ADJ_INDEX_INVALID;

            if (PREDICT_FALSE(p0->flags & VLIB_BUFFER_IS_TRACED)) {
                lb_nodeport_trace_t *tr = vlib_add_trace(vm, node, p0, sizeof(*tr));
                tr->vip_index           = entry0 ? entry0[0] : ADJ_INDEX_INVALID;
                tr->node_port           = (u32) clib_net_to_host_u16(udp_0->dst_port);
            }

            vlib_validate_buffer_enqueue_x1(vm,
                                            node,
                                            next_index,
                                            to_next,
                                            n_left_to_next,
                                            pi0,
                                            is_input_v4 ? LB4_NODEPORT_NEXT_IP4_NAT4 : LB6_NODEPORT_NEXT_IP6_NAT6);
        }
        vlib_put_next_frame(vm, node, next_index, n_left_to_next);
    }
...
static uword lb_nat_in2out_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u32 is_nat4)
{
    ...
    stats_node_index = is_nat4 ? lb_nat4_in2out_node.index : lb_nat6_in2out_node.index;
    ...
    vlib_node_increment_counter(vm, stats_node_index, LB_NAT_IN2OUT_ERROR_IN2OUT_PACKETS, pkts_processed);
    return frame->n_vectors;
}
