...
u8 *format_lb_vip_type(u8 *s, va_list *args)
{
    ...
    for (i = 0; i < LB_VIP_N_TYPES; i++)
        if (vipt == i) return format(s, lb_vip_type_strings[i]);
...
u8 *format_lb_vip(u8 *s, va_list *args)
{
    ...
    else if ((vip->type == LB_VIP_TYPE_IP4_NAT4) || (vip->type == LB_VIP_TYPE_IP6_NAT6)) {
        s = format(s,
                   " type:%s port:%u target_port:%u",
                   (vip->encap_args.srv_type == LB_SRV_TYPE_CLUSTERIP) ? "clusterip" : "nodeport",
                   ntohs(vip->port),
...
u8 *format_lb_as(u8 *s, va_list *args)
{
    ...
    return format(s, "%U %s", format_ip46_address, &as->address, IP46_TYPE_ANY, (as->flags & LB_AS_FLAGS_USED) ? "used" : "removed");
}
...
u8 *format_lb_vip_detailed(u8 *s, va_list *args)
{
  ...
  s = format(s, "%U %U [%lu] %U%s%s\n"
    ...
    if (vip->port != 0) {
        s = format(s, "%U  protocol:%u port:%u\n", format_white_space, indent, vip->protocol, vip->port);
    }

    if (vip->type == LB_VIP_TYPE_IP4_L3DSR) {
        s = format(s, "%U  dscp:%u\n", format_white_space, indent, vip->encap_args.dscp);
    }
    else if ((vip->type == LB_VIP_TYPE_IP4_NAT4) || (vip->type == LB_VIP_TYPE_IP6_NAT6)) {
        s = format(s,
    ...
    s = format(s, "%U  #as:%u\n", format_white_space, indent, pool_elts(vip->as_indexes));
...
static void lb_vip_garbage_collection(lb_vip_t *vip)
{
    ...
    if (!clib_u32_loop_gt(now, vip->last_garbage_collection + LB_GARBAGE_RUN)) return;
    ...
    pool_foreach (as_index, vip->as_indexes) {
        ...
        if (!(as->flags & LB_AS_FLAGS_USED) &&  // Not used
            ...
            if (lb_vip_is_nat4_port(vip)) {
                ...
                if (!clib_bihash_search_8_8(&lbm->mapping_by_as4, &kv4, &value4)) m = pool_elt_at_index(lbm->snat_mappings, value4.value);
                ASSERT(m);
                ...
                if (!clib_bihash_search_24_8(&lbm->mapping_by_as6, &kv6, &value6)) m = pool_elt_at_index(lbm->snat_mappings, value6.value);
                ASSERT(m);
...
void lb_garbage_collection()
{
    ...
    pool_foreach (vip, lbm->vips) {
        ...
        if (!(vip->flags & LB_VIP_FLAGS_USED) && (pool_elts(vip->as_indexes) == 0)) {
...
out:
    ...
    vec_foreach (pr, sort_arr) {
        ...
        u64 seed = clib_xxhash(as->address.as_u64[0] ^ as->address.as_u64[1]);
        /* We have 2^n buckets.
    ...
    while (1) {
        vec_foreach (pr, sort_arr) {
            ...
            if (done == vec_len(new_flow_table)) goto finished;
        }
    }
...
int lb_conf(ip4_address_t *ip4_address, ip6_address_t *ip6_address, u32 per_cpu_sticky_buckets, u32 flow_timeout)
{
    ...
    if (!is_pow2(per_cpu_sticky_buckets)) return VNET_API_ERROR_INVALID_MEMORY_SIZE;
...
static int lb_vip_port_find_index(ip46_address_t *prefix, u8 plen, u8 protocol, u16 port, lb_lkp_type_t lkp_type, u32 *vip_index)
{
    ...
    pool_foreach (vip, lbm->vips) {
        if ((vip->flags & LB_AS_FLAGS_USED) && vip->plen == plen && vip->prefix.as_u64[0] == prefix->as_u64[0]
            && vip->prefix.as_u64[1] == prefix->as_u64[1]) {
            if ((lkp_type == LB_LKP_SAME_IP_PORT && vip->protocol == protocol && vip->port == port)
                || (lkp_type == LB_LKP_ALL_PORT_IP && vip->port == 0)
                || (lkp_type == LB_LKP_DIFF_IP_PORT && (vip->protocol != protocol || vip->port != port))) {
...
{
    return lb_vip_port_find_index(prefix, plen, protocol, port, LB_LKP_SAME_IP_PORT, vip_index);
}

static int lb_vip_port_find_all_port_vip(ip46_address_t *prefix, u8 plen, u32 *vip_index)
{
    return lb_vip_port_find_index(prefix, plen, ~0, 0, LB_LKP_ALL_PORT_IP, vip_index);
}
...
{
    return lb_vip_port_find_index(prefix, plen, protocol, port, LB_LKP_DIFF_IP_PORT, vip_index);
}
...
int lb_vip_find_index(ip46_address_t *prefix, u8 plen, u8 protocol, u16 port, u32 *vip_index)
{
    int ret;
    lb_get_writer_lock();
    ret = lb_vip_port_find_index_with_lock(prefix, plen, protocol, port, vip_index);
    lb_put_writer_lock();
    return ret;
}
...
static int lb_as_find_index_vip(lb_vip_t *vip, ip46_address_t *address, u32 *as_index)
{
    ...
    pool_foreach (asi, vip->as_indexes) {
        as = &lbm->ass[*asi];
        if (as->vip_index == (vip - lbm->vips) && as->address.as_u64[0] == address->as_u64[0] && as->address.as_u64[1] == address->as_u64[1]) {
            *as_index = as - lbm->ass;
            return 0;
        }
    }
...
int lb_vip_add_ass(u32 vip_index, ip46_address_t *addresses, u32 n)
{
    ...
    while (n--) {
        ...
        if (n) {
            ...
            while (n2--)  // Check for duplicates
                if (addresses[n2].as_u64[0] == addresses[n].as_u64[0] && addresses[n2].as_u64[1] == addresses[n].as_u64[1]) goto next;
        }
    ...
    vec_foreach (ip, to_be_added) {
        ...
        *as_index = as - lbm->ass;
        ...
        as->next_hop_fib_entry_index = fib_table_entry_special_add(0, &nh, FIB_SOURCE_RR, FIB_ENTRY_FLAG_NONE);
        as->next_hop_child_index     = fib_entry_child_add(as->next_hop_fib_entry_index, lbm->fib_node_type, as - lbm->ass);
...
int lb_flush_vip_as(u32 vip_index, u32 as_index)
{
    ...
    for (thread_index = 0; thread_index < tm->n_vlib_mains; thread_index++) {
        ...
        if (h != NULL) {
            ...
            {
                if ((vip_index == ~0) || ((b->vip[i] == vip_index) && (as_index == ~0)) || ((b->vip[i] == vip_index) && (b->value[i] == as_index))) {
...
static int lb_vip_prefix_index_free(lb_main_t *lbm, u32 instance)
{
    ...
    lbm->vip_prefix_indexes = clib_bitmap_set(lbm->vip_prefix_indexes, instance, 0);
...
static void lb_vip_add_adjacency(lb_main_t *lbm, lb_vip_t *vip, u32 *vip_prefix_index)
{
    ...
    if (vip->port != 0) {
        ...
        if (!lb_vip_port_find_diff_port(&(vip->prefix), vip->plen, vip->protocol, vip->port, &vip_idx)) {
    ...
    fib_table_entry_special_dpo_add(0, &pfx, lb_fib_src, FIB_ENTRY_FLAG_EXCLUSIVE, &dpo);
    dpo_reset(&dpo);
}
...
int lb_vip_add(lb_vip_add_args_t args, u32 *vip_index)
{
    ...
    if (!lb_vip_port_find_index_with_lock(&(args.prefix), args.plen, args.protocol, args.port, vip_index)) {
        lb_put_writer_lock();
        return VNET_API_ERROR_VALUE_EXIST;
    }
    ...
    if ((args.port != 0) && !lb_vip_port_find_all_port_vip(&(args.prefix), args.plen, vip_index)) {
        lb_put_writer_lock();
        return VNET_API_ERROR_VALUE_EXIST;
    }
    ...
    if ((args.port == 0) && !lb_vip_port_find_diff_port(&(args.prefix), args.plen, args.protocol, args.port, vip_index)) {
        lb_put_writer_lock();
        return VNET_API_ERROR_VALUE_EXIST;
    }
    ...
    if (ip46_prefix_is_ip4(&(args.prefix), args.plen) && !lb_vip_is_ip4(type)) {
        lb_put_writer_lock();
        return VNET_API_ERROR_INVALID_ADDRESS_FAMILY;
    }

    if ((!ip46_prefix_is_ip4(&(args.prefix), args.plen)) && !lb_vip_is_ip6(type)) {
        lb_put_writer_lock();
        return VNET_API_ERROR_INVALID_ADDRESS_FAMILY;
    }
    ...
    else if ((args.type == LB_VIP_TYPE_IP4_NAT4) || (args.type == LB_VIP_TYPE_IP6_NAT6)) {
        vip->encap_args.srv_type    = args.encap_args.srv_type;
        vip->encap_args.target_port = clib_host_to_net_u16(args.encap_args.target_port);
    }
    ...
    if ((lb_vip_is_nat4_port(vip) || lb_vip_is_nat6_port(vip)) && (args.encap_args.srv_type == LB_SRV_TYPE_NODEPORT)) {
        u32    key;
        uword *entry;
...
int lb_vip_del(u32 vip_index)
{
    ...
    if (vip_index == 0) return VNET_API_ERROR_INVALID_VALUE;
...

    {
        ...
        if (vec_len(ass)) lb_vip_del_ass_withlock(vip_index, ass, vec_len(ass), 0);
        vec_free(ass);
    }
...
{
    return ((lb_as_t *) (((char *) node) - STRUCT_OFFSET_OF(lb_as_t, fib_node)));
}
...
{
    if (is_del) {
        vnet_feature_enable_disable("ip4-unicast", "lb-nat4-in2out", sw_if_index, 0, 0, 0);
    }
    else {
        vnet_feature_enable_disable("ip4-unicast", "lb-nat4-in2out", sw_if_index, 1, 0, 0);
    }
...
{
    if (is_del) {
        vnet_feature_enable_disable("ip6-unicast", "lb-nat6-in2out", sw_if_index, 0, 0, 0);
    }
    else {
        vnet_feature_enable_disable("ip6-unicast", "lb-nat6-in2out", sw_if_index, 1, 0, 0);
    }
...
clib_error_t *lb_init(vlib_main_t *vm)
{
    ...
    lbm->dpo_gre4_port_type        = dpo_register_new_type(&lb_vft, lb_dpo_gre4_port_nodes);
    lbm->dpo_gre6_port_type        = dpo_register_new_type(&lb_vft, lb_dpo_gre6_port_nodes);
    lbm->dpo_l3dsr_type            = dpo_register_new_type(&lb_vft, lb_dpo_l3dsr_nodes);
    lbm->dpo_l3dsr_port_type       = dpo_register_new_type(&lb_vft, lb_dpo_l3dsr_port_nodes);
    lbm->dpo_nat4_port_type        = dpo_register_new_type(&lb_vft, lb_dpo_nat4_port_nodes);
    lbm->dpo_nat6_port_type        = dpo_register_new_type(&lb_vft, lb_dpo_nat6_port_nodes);
    ...
    lbm->vip_index_by_nodeport = hash_create_mem(0, sizeof(u16), sizeof(uword));

    clib_bihash_init_8_8(&lbm->vip_index_per_port, "vip_index_per_port", LB_VIP_PER_PORT_BUCKETS, LB_VIP_PER_PORT_MEMORY_SIZE);

    clib_bihash_init_8_8(&lbm->mapping_by_as4, "mapping_by_as4", LB_MAPPING_BUCKETS, LB_MAPPING_MEMORY_SIZE);

    clib_bihash_init_24_8(&lbm->mapping_by_as6, "mapping_by_as6", LB_MAPPING_BUCKETS, LB_MAPPING_MEMORY_SIZE);

#define _(a, b, c) lbm->vip_counters[c].name = b;
    lb_foreach_vip_counter
#undef _

        lb_fib_src = fib_source_allocate("lb", FIB_SOURCE_PRIORITY_HI, FIB_SOURCE_BH_SIMPLE);
