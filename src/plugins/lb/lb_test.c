...
static int api_lb_add_del_vip(vat_main_t *vam)
{
    ...
    if (!unformat(line_input, "%U", unformat_ip46_prefix, &ip_prefix, &prefix_length, IP46_TYPE_ANY, &prefix_length)) {
...
static int api_lb_add_del_vip_v2(vat_main_t *vam)
{
    unformat_input_t           *line_input = vam->input;
...
static void vl_api_lb_vip_details_t_handler(vl_api_lb_vip_details_t *mp)
{
...
static void vl_api_lb_as_details_t_handler(vl_api_lb_as_details_t *mp)
{
    vat_main_t *vam = &vat_main;
