...
u8 *format_ikev2_id_type_and_data(u8 *s, va_list *args)
{
    ...
    if (id->type == 0 || vec_len(id->data) == 0) return format(s, "none");
    ...
    switch (id->type) {
        ...
        case IKEV2_ID_TYPE_ID_RFC822_ADDR:
            ...
            break;
        default:
            s = format(s, "0x%U", format_hex_bytes, &id->data, (uword) (vec_len(id->data)));
            break;
    }
...
static u8 *format_ikev2_sa(u8 *s, va_list *va)
{
    ...
    s = format(s, "iip %U ispi %lx rip %U rspi %lx", format_ip_address, &sa->iaddr, sa->ispi, format_ip_address, &sa->raddr, sa->rspi);
    if (!details) return s;
    ...
    s = format(s, "%USK_d    %U\n", format_white_space, indent, format_hex_bytes, sa->sk_d, vec_len(sa->sk_d));
    if (sa->sk_ai) {
        s = format(s,
                   "%USK_a  i:%U\n%Ur:%U\n",
                   format_white_space,
    ...
    s = format(s, "%Uidentifier (i) %U\n", format_white_space, indent, format_ikev2_id_type_and_data, &sa->i_id);
    s = format(s, "%Uidentifier (r) %U\n", format_white_space, indent, format_ikev2_id_type_and_data, &sa->r_id);

    vec_foreach (child, sa->childs) {
        s = format(s, "%U%U", format_white_space, indent + 2, format_ikev2_child_sa, child, child - sa->childs);
    }

    s = format(s, "Stats:\n");
    s = format(s, " keepalives :%u\n", sa->stats.n_keepalives);
    s = format(s, " rekey :%u\n", sa->stats.n_rekey_req);
    s = format(s, " SA init :%u (retransmit: %u)\n", sa->stats.n_sa_init_req, sa->stats.n_init_retransmit);
...
static clib_error_t *ikev2_profile_add_del_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "add %U", unformat_ikev2_token, &name)) {
        ...
        else if (unformat(line_input, "set %U auth shared-key-mic string %v", unformat_ikev2_token, &name, &data)) {
            r = ikev2_set_profile_auth(vm, name, IKEV2_AUTH_METHOD_SHARED_KEY_MIC, data, 0);
            goto done;
        }
        else if (unformat(line_input, "set %U auth shared-key-mic hex %U", unformat_ikev2_token, &name, unformat_hex_string, &data)) {
            r = ikev2_set_profile_auth(vm, name, IKEV2_AUTH_METHOD_SHARED_KEY_MIC, data, 1);
            goto done;
        }
        else if (unformat(line_input, "set %U auth rsa-sig cert-file %v", unformat_ikev2_token, &name, &data)) {
        ...
        else if (unformat(line_input, "set %U id local %U %v", unformat_ikev2_token, &name, unformat_ikev2_id_type, &id_type, &data)) {
            r = ikev2_set_profile_id(vm, name, (u8) id_type, data, /*local */ 1);
            goto done;
        }
        else if (unformat(line_input,
        ...
        else if (unformat(line_input, "set %U id remote %U %v", unformat_ikev2_token, &name, unformat_ikev2_id_type, &id_type, &data)) {
            r = ikev2_set_profile_id(vm,
                                     name,
                                     (u8) id_type,
                                     data, /*remote */
            ...
            goto done;
                          ...
                          &tmp2,
                          &tmp3)) {
            r = ikev2_set_profile_ts(vm, name, (u8) tmp3, (u16) tmp1, (u16) tmp2, ip, end_addr, /*local */ 1);
            goto done;
        }
        else if (unformat(line_input,
                          ...
                          &tmp2,
                          &tmp3)) {
            r = ikev2_set_profile_ts(vm, name, (u8) tmp3, (u16) tmp1, (u16) tmp2, ip, end_addr, /*remote */ 0);
            goto done;
        }
        else if (unformat(line_input,
                          "set %U responder %U %U",
                          ...
                          unformat_ip_address,
                          &ip)) {
            r = ikev2_set_profile_responder(vm, name, responder_sw_if_index, ip);
            goto done;
        }
        else if (unformat(line_input,
                          "set %U responder %U %v",
                          ...
                          &responder_sw_if_index,
                          &data)) {
            r = ikev2_set_profile_responder_hostname(vm, name, data, responder_sw_if_index);
            goto done;
        }
        else if (unformat(line_input, "set %U tunnel %U", unformat_ikev2_token, &name, unformat_vnet_sw_interface, vnm, &tun_sw_if_index)) {
            ...
            goto done;
                          ...
                          &dh_type)) {
            r = ikev2_set_profile_ike_transforms(vm, name, crypto_alg, integ_alg, dh_type, tmp1);
            goto done;
        }
        else if (unformat(line_input,
                          ...
                          unformat_ikev2_transform_dh_type,
                          &dh_type)) {
            r = ikev2_set_profile_ike_transforms(vm, name, crypto_alg, IKEV2_TRANSFORM_INTEG_TYPE_NONE, dh_type, tmp1);
            goto done;
        }
        else if (unformat(line_input,
                          ...
                          unformat_ikev2_transform_integ_type,
                          &integ_alg)) {
            r = ikev2_set_profile_esp_transforms(vm, name, crypto_alg, integ_alg, tmp1);
            goto done;
        }
        else if (unformat(line_input,
        ...
        else if (unformat(line_input, "set %U sa-lifetime %lu %u %u %lu", unformat_ikev2_token, &name, &tmp4, &tmp1, &tmp2, &tmp5)) {
            r = ikev2_set_profile_sa_lifetime(vm, name, tmp4, tmp1, tmp2, tmp5);
            goto done;
        }
        else if (unformat(line_input, "set %U udp-encap", unformat_ikev2_token, &name)) {
            r = ikev2_set_profile_udp_encap(vm, name);
            goto done;
        }
        else if (unformat(line_input, "set %U ipsec-over-udp port %u", unformat_ikev2_token, &name, &tmp1)) {
            int rv = ikev2_set_profile_ipsec_udp_port(vm, name, tmp1, 1);
            if (rv) r = clib_error_return(0, "Error: %U", format_vnet_api_errno, rv);
            goto done;
        }
        else if (unformat(line_input, "set %U disable natt", unformat_ikev2_token, &name)) {
    ...
    r = clib_error_return(0, "parse error: '%U'", format_unformat_error, line_input);
    ...
    pool_foreach (p, km->profiles) {
        ...
        if (p->auth.data) {
            if (p->auth.hex)
                ...
                vlib_cli_output(vm, "  auth-method %U auth data %v", format_ikev2_auth_method, p->auth.method, p->auth.data);
        }

        if (p->loc_id.data) vlib_cli_output(vm, "  local %U", format_ikev2_id_type_and_data, &p->loc_id);

        if (p->rem_id.data) vlib_cli_output(vm, "  remote %U", format_ikev2_id_type_and_data, &p->rem_id);
        ...
        if (~0 != p->tun_itf) vlib_cli_output(vm, "  protected tunnel %U", format_vnet_sw_if_index_name, vnet_get_main(), p->tun_itf);
        if (~0 != p->responder.sw_if_index)
            vlib_cli_output(vm,
                            ...
                            format_ip_address,
                            &p->responder.addr,
                            p->responder.hostname);
        if (p->udp_encap) vlib_cli_output(vm, "  udp-encap");

        if (p->natt_disabled) vlib_cli_output(vm, "  NAT-T disabled");

        if (p->ipsec_over_udp_port != IPSEC_UDP_PORT_NONE) vlib_cli_output(vm, "  ipsec-over-udp port %d", p->ipsec_over_udp_port);
        ...
        if (p->esp_ts.crypto_alg || p->esp_ts.integ_alg || p->esp_ts.dh_type)
            vlib_cli_output(vm,
                            ...
                            format_ikev2_transform_encr_type,
                            p->esp_ts.crypto_alg,
                            p->esp_ts.crypto_key_size,
                            format_ikev2_transform_integ_type,
                            p->esp_ts.integ_alg);

        vlib_cli_output(vm, "  lifetime %d jitter %d handover %d maxdata %d", p->lifetime, p->lifetime_jitter, p->handover, p->lifetime_maxdata);
    }
...
static clib_error_t *ikev2_set_log_level_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (rc < 0) error = clib_error_return(0, "setting log level failed!");
