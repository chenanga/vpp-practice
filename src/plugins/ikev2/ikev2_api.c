...
static void cp_id(vl_api_ikev2_id_t *vl_api_id, ikev2_id_t *id)
{
    ...
    if (vec_len(id->data) < size_data) size_data = vec_len(id->data);
...
static void send_profile(ikev2_profile_t *profile, vl_api_registration_t *reg, u32 context)
{
    ...
    if (vec_len(profile->name) < size_data) size_data = vec_len(profile->name);
...
static vl_api_ikev2_state_t ikev2_state_encode(ikev2_state_t state)
{
    switch (state) {
...
static void vl_api_ikev2_sa_dump_t_handler(vl_api_ikev2_sa_dump_t *mp)
{
    ...
    vec_foreach (tkm, km->per_thread_data) {
        pool_foreach (sa, tkm->sas) {
            u32 api_sa_index = ikev2_encode_sa_index(sa - tkm->sas, tkm - km->per_thread_data);
            send_sa(sa, mp, api_sa_index);
        }
    }
}

static void send_sa_v2(ikev2_sa_t *sa, vl_api_ikev2_sa_v2_dump_t *mp, u32 api_sa_index)
{
    ikev2_main_t                 *km  = &ikev2_main;
    ...
    REPLY_MACRO2_ZERO(VL_API_IKEV2_SA_V2_DETAILS, {
        ...
        if (vec_len(p->name) < size_data) size_data = vec_len(p->name);
...
static void vl_api_ikev2_sa_v2_dump_t_handler(vl_api_ikev2_sa_v2_dump_t *mp)
{
    ikev2_main_t                 *km = &ikev2_main;
    ikev2_main_per_thread_data_t *tkm;
    ikev2_sa_t                   *sa;
...
static void send_sa_v3(ikev2_sa_t *sa, vl_api_ikev2_sa_v3_dump_t *mp, u32 api_sa_index)
{
    ikev2_main_t                 *km  = &ikev2_main;
...
static void vl_api_ikev2_sa_v3_dump_t_handler(vl_api_ikev2_sa_v3_dump_t *mp)
{
    ikev2_main_t                 *km = &ikev2_main;
    ikev2_main_per_thread_data_t *tkm;
    ikev2_sa_t                   *sa;
...
static void vl_api_ikev2_child_sa_dump_t_handler(vl_api_ikev2_child_sa_dump_t *mp)
{
    ...
    if (vec_len(im->per_thread_data) <= ti) return;

    tkm = vec_elt_at_index(im->per_thread_data, ti);

    if (pool_len(tkm->sas) <= sai || pool_is_free_index(tkm->sas, sai)) return;
...
static void send_child_sa_v2(ikev2_child_sa_t *child, vl_api_ikev2_child_sa_v2_dump_t *mp, u32 child_sa_index, u32 sa_index)
{
...
static void vl_api_ikev2_child_sa_v2_dump_t_handler(vl_api_ikev2_child_sa_v2_dump_t *mp)
{
    ikev2_main_t                 *im = &ikev2_main;
    ikev2_main_per_thread_data_t *tkm;
    ikev2_sa_t                   *sa;
...
static void vl_api_ikev2_traffic_selector_dump_t_handler(vl_api_ikev2_traffic_selector_dump_t *mp)
{
    ...
    if (vec_len(sa->childs) <= child_sa_index) return;
...
static void vl_api_ikev2_plugin_get_version_t_handler(vl_api_ikev2_plugin_get_version_t *mp)
{
    ...
    rmp->_vl_msg_id = ntohs(VL_API_IKEV2_PLUGIN_GET_VERSION_REPLY + im->msg_id_base);
...
static void vl_api_ikev2_profile_set_liveness_t_handler(vl_api_ikev2_profile_set_liveness_t *mp)
{
    ...
    int                                        rv = 0;
    clib_error_t                              *error;
    error = ikev2_set_liveness_params(clib_net_to_host_u32(mp->period), clib_net_to_host_u32(mp->max_retries));
    if (error) {
...
static void vl_api_ikev2_set_ike_transforms_t_handler(vl_api_ikev2_set_ike_transforms_t *mp)
{
    ...
    error = ikev2_set_profile_ike_transforms(vm, tmp, mp->tr.crypto_alg, mp->tr.integ_alg, mp->tr.dh_group, ntohl(mp->tr.crypto_key_size));
    vec_free(tmp);
    if (error) {
        ikev2_log_error("%U", format_clib_error, error);
        clib_error_free(error);
...
static void vl_api_ikev2_set_esp_transforms_t_handler(vl_api_ikev2_set_esp_transforms_t *mp)
{
    ...
    error = ikev2_set_profile_esp_transforms(vm, tmp, mp->tr.crypto_alg, mp->tr.integ_alg, ntohl(mp->tr.crypto_key_size));
    vec_free(tmp);
    if (error) {
        ikev2_log_error("%U", format_clib_error, error);
        clib_error_free(error);
...
static void vl_api_ikev2_set_sa_lifetime_t_handler(vl_api_ikev2_set_sa_lifetime_t *mp)
{
    ...
    error = ikev2_set_profile_sa_lifetime(vm,
                                          tmp,
                                          clib_net_to_host_u64(mp->lifetime),
                                          ntohl(mp->lifetime_jitter),
                                          ntohl(mp->handover),
                                          clib_net_to_host_u64(mp->lifetime_maxdata));
    vec_free(tmp);
    if (error) {
        ikev2_log_error("%U", format_clib_error, error);
        clib_error_free(error);
...
static void vl_api_ikev2_profile_set_ipsec_udp_port_t_handler(vl_api_ikev2_profile_set_ipsec_udp_port_t *mp)
{
    ...
    rv = ikev2_set_profile_ipsec_udp_port(vm, tmp, clib_net_to_host_u16(mp->port), mp->is_set);
    vec_free(tmp);
...
static void vl_api_ikev2_set_tunnel_interface_t_handler(vl_api_ikev2_set_tunnel_interface_t *mp)
{
    ...
    error = ikev2_set_profile_tunnel_interface(vlib_get_main(), tmp, ntohl(mp->sw_if_index));
