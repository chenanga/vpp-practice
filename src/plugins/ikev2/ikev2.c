...
static u8 *format_ikev2_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "ikev2: sw_if_index %d, next index %d", t->sw_if_index, t->next_index);
    return s;
}
...
static ikev2_sa_transform_t *ikev2_find_transform_data(ikev2_sa_transform_t *t)
{
    ...
    vec_foreach (td, km->supported_transforms) {
        if (td->type != t->type) continue;

        if (td->transform_id != t->transform_id) continue;

        if (td->type == IKEV2_TRANSFORM_TYPE_ENCR) {
            if (vec_len(t->attrs) != 4 || t->attrs[0] != 0x80 || t->attrs[1] != 14) continue;

            if (((t->attrs[2] << 8 | t->attrs[3]) / 8) != td->key_len) continue;
        }
        return td;
    }
    return 0;
...
static ikev2_sa_proposal_t *ikev2_select_proposal(ikev2_sa_proposal_t *proposals, ikev2_protocol_id_t prot_id)
{
    ...
    if (prot_id == IKEV2_PROTOCOL_IKE) {
        mandatory_bitmap = (1 << IKEV2_TRANSFORM_TYPE_ENCR) | (1 << IKEV2_TRANSFORM_TYPE_PRF) | (1 << IKEV2_TRANSFORM_TYPE_DH);
        optional_bitmap  = mandatory_bitmap | (1 << IKEV2_TRANSFORM_TYPE_INTEG);
    }
    else if (prot_id == IKEV2_PROTOCOL_ESP) {
        mandatory_bitmap = (1 << IKEV2_TRANSFORM_TYPE_ENCR) | (1 << IKEV2_TRANSFORM_TYPE_ESN);
        optional_bitmap  = mandatory_bitmap | (1 << IKEV2_TRANSFORM_TYPE_INTEG) | (1 << IKEV2_TRANSFORM_TYPE_DH);
    }
    else if (prot_id == IKEV2_PROTOCOL_AH) {
        mandatory_bitmap = (1 << IKEV2_TRANSFORM_TYPE_INTEG) | (1 << IKEV2_TRANSFORM_TYPE_ESN);
    ...
    vec_foreach (proposal, proposals) {
        u8 bitmap = 0;
        if (proposal->protocol_id != prot_id) continue;

        vec_foreach (transform, proposal->transforms) {
            if ((1 << transform->type) & bitmap) continue;
        ...
        if ((bitmap & mandatory_bitmap) == mandatory_bitmap && (bitmap & ~optional_bitmap) == 0) {
            rv->proposal_num = proposal->proposal_num;
            rv->protocol_id  = proposal->protocol_id;
            RAND_bytes((u8 *) &rv->spi, sizeof(rv->spi));
            if (rv->protocol_id != IKEV2_PROTOCOL_IKE) rv->spi &= 0xffffffff;
            goto done;
        }
        else {
...
ikev2_sa_transform_t *ikev2_sa_get_td_for_type(ikev2_sa_proposal_t *p, ikev2_transform_type_t type)
{
    ...
    vec_foreach (t, p->transforms) {
        if (t->type == type) return ikev2_find_transform_data(t);
    }
    return 0;
}
...
ikev2_child_sa_t *ikev2_sa_get_child(ikev2_sa_t *sa, u32 spi, ikev2_protocol_id_t prot_id, int by_initiator)
{
    ...
    vec_foreach (c, sa->childs) {
        ikev2_sa_proposal_t *proposal = by_initiator ? &c->i_proposals[0] : &c->r_proposals[0];
        if (proposal && proposal->spi == spi && proposal->protocol_id == prot_id) return c;
    }
...
static void ikev2_sa_free_all_vec(ikev2_sa_t *sa)
{
    ...
    if (sa->i_auth.key) EVP_PKEY_free(sa->i_auth.key);
    vec_free(sa->r_auth.data);
    if (sa->r_auth.key) EVP_PKEY_free(sa->r_auth.key);
...
static ikev2_generate_sa_error_t ikev2_generate_sa_init_data(ikev2_sa_t *sa)
{
    ...
    if (sa->dh_group == IKEV2_TRANSFORM_DH_TYPE_NONE) return IKEV2_GENERATE_SA_INIT_ERR_NO_DH;
...
#define _(A)                 \
    ...
    if (sai->natt_state == IKEV2_NATT_DISABLED) sa->natt_state = IKEV2_NATT_DISABLED;
...
static void ikev2_calc_keys_internal(ikev2_sa_t *sa, u8 *skeyseed)
{
    u8 *tmp;
...
static void ikev2_calc_keys_rekey(ikev2_sa_t *sa_new, ikev2_sa_t *sa_old)
{
    u8                   *s = 0, *skeyseed = 0;
    ikev2_sa_transform_t *tr_prf = ikev2_sa_get_td_for_type(sa_old->r_proposals, IKEV2_TRANSFORM_TYPE_PRF);
...
{
    u8                   *s = 0, *skeyseed = 0;
    ikev2_sa_transform_t *tr_prf = ikev2_sa_get_td_for_type(sa->r_proposals, IKEV2_TRANSFORM_TYPE_PRF);
...
static void ikev2_calc_child_keys(ikev2_sa_t *sa, ikev2_child_sa_t *child, u8 kex)
{
    u8 *s             = 0;
    u16 integ_key_len = 0;
    u8  salt_len      = 0;
    ...
    if (kex) vec_append(s, sa->dh_shared_key);
    vec_append(s, sa->i_nonce);
    vec_append(s, sa->r_nonce);
    /* calculate PRFplus */
    u8 *keymat;
...
{
    const u32 max_buf_size = sizeof(ispi) + sizeof(rspi) + sizeof(ip6_address_t) + sizeof(u16);
    u8        buf[max_buf_size];
...
static int ikev2_parse_nonce_payload(const void *p, u32 rlen, const u8 **nonce)
{
...
static int ikev2_process_sa_init_req(vlib_main_t *vm, ikev2_sa_t *sa, ike_header_t *ike, udp_header_t *udp, u32 len, u32 sw_if_index)
{
    ...
    ikev2_elog_exchange(
        "ispi %lx rspi %lx IKE_INIT request received "
        "from ",
    ...
    while (p < len && payload != IKEV2_PAYLOAD_NONE) {
        ...
        if (!ikev2_check_payload_length(ikep, current_length, &plen)) return 0;
        ...
        if (payload == IKEV2_PAYLOAD_SA) {
            ...
            if (!ikev2_parse_ke_payload(ikep, current_length, sa, &sa->i_dh_data)) return 0;
        }
        else if (payload == IKEV2_PAYLOAD_NONCE) {
            const u8 *nonce;
            int       nonce_len;
            vec_reset_length(sa->i_nonce);
            if ((nonce_len = ikev2_parse_nonce_payload(ikep, current_length, &nonce)) <= 0) return 0;
            vec_add(sa->i_nonce, nonce, nonce_len);
        }
        else if (payload == IKEV2_PAYLOAD_NOTIFY) {
            ikev2_notify_t *n = ikev2_parse_notify_payload(ikep, current_length);
            if (n->msg_type == IKEV2_NOTIFY_MSG_NAT_DETECTION_SOURCE_IP) {
                u8 *src_sha = ikev2_compute_nat_sha1(ike->ispi, 0, &sa->iaddr, udp->src_port);
                if (clib_memcmp(src_sha, n->data, vec_len(src_sha))) {
                    if (sa->natt_state == IKEV2_NATT_ENABLED) sa->natt_state = IKEV2_NATT_ACTIVE;
                    ikev2_elog_uint(IKEV2_LOG_DEBUG,
                                    "ispi %lx initiator"
                                    " behind NAT",
                                    sa->ispi);
                }
            ...
            else if (n->msg_type == IKEV2_NOTIFY_MSG_NAT_DETECTION_DESTINATION_IP) {
                u8 *dst_sha = ikev2_compute_nat_sha1(ike->ispi, 0, &sa->raddr, udp->dst_port);
            ...
            ikev2_elog_uint(IKEV2_LOG_ERROR, "Unknown payload! type=%d", payload);
...
static void ikev2_process_sa_init_resp(vlib_main_t *vm, ikev2_sa_t *sa, ike_header_t *ike, udp_header_t *udp, u32 len)
{
    ...
    ikev2_elog_exchange(
        "ispi %lx rspi %lx IKE_INIT response received "
        "from ",
        sa->ispi,
        sa->rspi,
    ...
    while (p < len && payload != IKEV2_PAYLOAD_NONE) {
        int current_length = len - p;
        ikep               = (ike_payload_header_t *) &ike->payload[p];
        if (!ikev2_check_payload_length(ikep, current_length, &plen)) return;
        ...
        if (payload == IKEV2_PAYLOAD_SA) {
            ...
            if (sa->r_proposals) {
                ikev2_set_state(sa, IKEV2_STATE_SA_INIT);
                ike->msgid = clib_host_to_net_u32(clib_net_to_host_u32(ike->msgid) + 1);
            }
        }
        else if (payload == IKEV2_PAYLOAD_KE) {
            if (!ikev2_parse_ke_payload(ikep, current_length, sa, &sa->r_dh_data)) return;
        }
        else if (payload == IKEV2_PAYLOAD_NONCE) {
            const u8 *nonce;
            int       nonce_len;
            vec_reset_length(sa->r_nonce);
            if ((nonce_len = ikev2_parse_nonce_payload(ikep, current_length, &nonce)) <= 0) return;
            vec_add(sa->r_nonce, nonce, nonce_len);
        }
        else if (payload == IKEV2_PAYLOAD_NOTIFY) {
            ikev2_notify_t *n = ikev2_parse_notify_payload(ikep, current_length);
            if (n->msg_type == IKEV2_NOTIFY_MSG_NAT_DETECTION_SOURCE_IP) {
                u8 *src_sha = ikev2_compute_nat_sha1(ike->ispi, ike->rspi, &sa->raddr, udp->src_port);
                if (clib_memcmp(src_sha, n->data, vec_len(src_sha))) {
                    ikev2_elog_uint(IKEV2_LOG_DEBUG,
                                    "ispi %lx responder"
                                    " behind NAT, unsupported",
                                    sa->ispi);
                }
                vec_free(src_sha);
            }
            else if (n->msg_type == IKEV2_NOTIFY_MSG_NAT_DETECTION_DESTINATION_IP) {
                u8 *dst_sha = ikev2_compute_nat_sha1(ike->ispi, ike->rspi, &sa->iaddr, udp->dst_port);
                if (clib_memcmp(dst_sha, n->data, vec_len(dst_sha))) {
                    if (sa->natt_state == IKEV2_NATT_ENABLED) sa->natt_state = IKEV2_NATT_ACTIVE;
                    ikev2_elog_uint(IKEV2_LOG_DEBUG,
                                    "ispi %lx initiator"
...
static u8 *ikev2_decrypt_sk_payload(ikev2_sa_t *sa, ike_header_t *ike, u8 *payload, u32 rlen, u32 *out_len)
{
    ...
    if (((!sa->sk_ar || !sa->sk_ai) && !is_aead) || (!sa->sk_ei || !sa->sk_er)) return 0;

    if (rlen <= sizeof(*ike)) return 0;

    int len = rlen - sizeof(*ike);
    while (p < len && *payload != IKEV2_PAYLOAD_NONE && last_payload != IKEV2_PAYLOAD_SK) {
        ikep               = (ike_payload_header_t *) &ike->payload[p];
        int current_length = len - p;
        if (!ikev2_check_payload_length(ikep, current_length, &plen)) return 0;
    ...
    if (is_aead) {
        if (plen < sizeof(*ikep) + IKEV2_GCM_ICV_SIZE) return 0;
        ...
        int rc = ikev2_decrypt_aead_data(ptd, sa, tr_encr, ikep->payload, plen, aad, aad_len, tag, &dlen);
        if (rc) {
            *out_len  = dlen;
        ...
        if (rlen < tr_integ->key_trunc) return 0;

        hmac = ikev2_calc_integr(tr_integ, sa->is_initiator ? sa->sk_ar : sa->sk_ai, (u8 *) ike, rlen - tr_integ->key_trunc);

        if (plen < sizeof(*ikep) + tr_integ->key_trunc) return 0;
        ...
        int rc = ikev2_decrypt_data(ptd, sa, tr_encr, ikep->payload, plen, &dlen);
        if (rc) {
            *out_len  = dlen;
...
static int ikev2_is_id_equal(const ikev2_id_t *i1, const ikev2_id_t *i2)
{
    if (i1->type != i2->type) return 0;

    if (vec_len(i1->data) != vec_len(i2->data)) return 0;

    if (clib_memcmp(i1->data, i2->data, vec_len(i1->data))) return 0;
...
static void ikev2_initial_contact_cleanup_internal(ikev2_main_per_thread_data_t *ptd, ikev2_sa_t *sa)
{
    ...
    pool_foreach (tmp, ptd->sas) {
        if (!ikev2_is_id_equal(&tmp->i_id, &sa->i_id) || !ikev2_is_id_equal(&tmp->r_id, &sa->r_id)) continue;

        if (sa->rspi != tmp->rspi) vec_add1(delete, tmp - ptd->sas);
    }
...
static int ikev2_parse_id_payload(const void *p, u16 rlen, ikev2_id_t *sa_id)
{
    ...
    if (plen < sizeof(*id) || plen > rlen) return 0;
...
static int ikev2_process_auth_req(vlib_main_t *vm, ikev2_sa_t *sa, ike_header_t *ike, u32 len)
{
    ...
    ikev2_elog_exchange(
        "ispi %lx rspi %lx EXCHANGE_IKE_AUTH received "
        "from ",
        clib_host_to_net_u64(ike->ispi),
        clib_host_to_net_u64(ike->rspi),
        sa->is_initiator ? ip_addr_v4(&sa->raddr).as_u32 : ip_addr_v4(&sa->iaddr).as_u32,
        ip_addr_version(&sa->raddr) == AF_IP4);
    ...
    while (p < dlen && payload != IKEV2_PAYLOAD_NONE) {
        ...
        if (!ikev2_check_payload_length(ikep, current_length, &plen)) goto malformed;

        if (payload == IKEV2_PAYLOAD_SA) /* 33 */
        {
            if (sa->is_initiator) {
                ikev2_sa_free_proposal_vector(&first_child_sa->r_proposals);
                first_child_sa->r_proposals = ikev2_parse_sa_payload(ikep, current_length);
            }
            else {
                ikev2_sa_free_proposal_vector(&first_child_sa->i_proposals);
                first_child_sa->i_proposals = ikev2_parse_sa_payload(ikep, current_length);
            }
        }
        else if (payload == IKEV2_PAYLOAD_IDI) /* 35 */
        {
            if (!ikev2_parse_id_payload(ikep, current_length, &sa->i_id)) goto malformed;
        }
        else if (payload == IKEV2_PAYLOAD_IDR) /* 36 */
        {
            if (!ikev2_parse_id_payload(ikep, current_length, &sa->r_id)) goto malformed;
        }
        else if (payload == IKEV2_PAYLOAD_AUTH) /* 39 */
        {
            if (sa->is_initiator) {
                if (!ikev2_parse_auth_payload(ikep, current_length, &sa->r_auth)) goto malformed;
            }
            else {
                if (!ikev2_parse_auth_payload(ikep, current_length, &sa->i_auth)) goto malformed;
            }
        }
...
static int ikev2_process_informational_req(vlib_main_t *vm, ikev2_sa_t *sa, ike_header_t *ike, u32 len)
{
    ...
    ikev2_elog_exchange(
        "ispi %lx rspi %lx INFORMATIONAL received "
        "from ",
        clib_host_to_net_u64(ike->ispi),
        clib_host_to_net_u64(ike->rspi),
        ip_addr_v4(&sa->iaddr).as_u32,
    ...
    while (p < dlen && payload != IKEV2_PAYLOAD_NONE) {
        ...
        if (p + sizeof(*ikep) > dlen) return 0;

        ikep     = (ike_payload_header_t *) &plaintext[p];
        u16 plen = clib_net_to_host_u16(ikep->length);

        if (plen < sizeof(*ikep) || plen > current_length) return 0;
...
static int ikev2_process_create_child_sa_rekey(ikev2_sa_t          *sa,
                                               ikev2_sa_t          *sar,
                                               ikev2_rekey_t       *rekey,
                                               ikev2_sa_proposal_t *proposal,
                                               ikev2_ts_t          *tsi,
                                               ...
                                               int                  nonce_len)
{
    ...
    if (sar->dh_group) {
        tr = ikev2_sa_get_td_for_type(rekey->r_proposal, IKEV2_TRANSFORM_TYPE_DH);
...
static void ikev2_complete_sa_rekey(ikev2_sa_t *sa_new, ikev2_sa_t *sa_old, ikev2_sa_rekey_t *sa_rekey)
{
    sa_new->del                          = 0;
    sa_new->rekey                        = 0;
    sa_new->new_child                    = 0;
    ...
    if (sa_new->i_auth.key) EVP_PKEY_up_ref(sa_new->i_auth.key);

    sa_new->r_auth.data = vec_dup(sa_old->r_auth.data);
    sa_new->r_auth.key  = sa_old->r_auth.key;
    if (sa_new->r_auth.key) EVP_PKEY_up_ref(sa_new->r_auth.key);
...
static void ikev2_process_sa_rekey(ikev2_sa_t *sa_new, ikev2_sa_t *sa_old, ikev2_sa_rekey_t *sa_rekey)
{
    ikev2_sa_transform_t *tr;
    ...
    sa_new->r_proposals = ikev2_select_proposal(sa_new->i_proposals, IKEV2_PROTOCOL_IKE);
...
static int ikev2_process_create_child_sa_req(vlib_main_t *vm, ikev2_sa_t *sa, ike_header_t *ike, u32 len)
{
    ...
    if (!plaintext) goto cleanup_and_exit;
    ...
    while (payload != IKEV2_PAYLOAD_NONE) {
        ikep               = (ike_payload_header_t *) &plaintext[p];
        int current_length = dlen - p;
        if (!ikev2_check_payload_length(ikep, current_length, &plen)) goto cleanup_and_exit;

        if (payload == IKEV2_PAYLOAD_SA) {
            proposal = ikev2_parse_sa_payload(ikep, current_length);
        }
        else if (payload == IKEV2_PAYLOAD_KE) {
            if (!ikev2_parse_ke_payload(ikep, current_length, &sar, &sar.i_dh_data)) goto cleanup_and_exit;
        }
        else if (payload == IKEV2_PAYLOAD_NOTIFY) {
            ikev2_notify_t *n0;
            ...
            if (nonce_len <= 0) goto cleanup_and_exit;
        }
    ...
    if (!proposal || !nonce || (proposal->protocol_id != IKEV2_PROTOCOL_ESP && proposal->protocol_id != IKEV2_PROTOCOL_IKE)) goto cleanup_and_exit;

    if (sa->is_initiator) {
        rekey = sa->rekey;
        if (vec_len(rekey) == 0) goto cleanup_and_exit;
        rekey->notify_type     = 0;
        ...
        if (n) {
            ...
            if (!child_sa) {
                ikev2_elog_uint(IKEV2_LOG_ERROR, "child SA spi %lx not found", n->spi);
                goto cleanup_and_exit;
            }
            ...
            else if (!ikev2_process_create_child_sa_rekey(sa, &sar, rekey, proposal, tsi, tsr, nonce, nonce_len)) {
                vec_free(proposal);
                vec_free(tsr);
                vec_free(tsi);
            }
            ...
            if (tsi || tsr) goto cleanup_and_exit;
            ...
            rekey->notify_type = 0;
            rekey->kex         = 0;
            if (!ikev2_process_create_child_sa_rekey(sa, &sar, rekey, proposal, tsi, tsr, nonce, nonce_len)) {
                vec_free(proposal);
                vec_free(tsr);
                vec_free(tsi);
            }
...
static int ikev2_match_profile(const ikev2_profile_t *p, const ikev2_id_t *id_loc, const ikev2_id_t *id_rem, int is_initiator)
{
    /* on the initiator, IDi is always present and must match
     * however on the responder, IDr (which is our local id) is optional */
    if ((is_initiator || id_loc->type != 0) && !ikev2_is_id_equal(&p->loc_id, id_loc)) return 0;
    ...
    if ((!is_initiator || p->rem_id.type != 0) && !ikev2_is_id_equal(&p->rem_id, id_rem)) return 0;
...
{
    if (ts1->ts_type == ts2->ts_type && ts1->protocol_id == ts2->protocol_id && ts1->start_port == ts2->start_port && ts1->end_port == ts2->end_port
        && !ip_address_cmp(&ts1->start_addr, &ts2->start_addr) && !ip_address_cmp(&ts1->end_addr, &ts2->end_addr))
        return 1;
...
static void ikev2_sa_match_ts(ikev2_sa_t *sa)
{
    ...
    pool_foreach (p, km->profiles) {
        ...
        if (!ikev2_match_profile(p, id_loc, id_rem, sa->is_initiator)) continue;
...
static ikev2_profile_t *ikev2_select_profile(vlib_main_t *vm, ikev2_main_t *km, ikev2_sa_t *sa, ikev2_sa_transform_t *tr_prf, u8 *key_pad)
{
    ...
    pool_foreach (p, km->profiles) {
        ...
        if (sa_auth->method == IKEV2_AUTH_METHOD_SHARED_KEY_MIC) {
            if (!p->auth.data || p->auth.method != IKEV2_AUTH_METHOD_SHARED_KEY_MIC) continue;
            ...
            if (!clib_memcmp(auth, sa_auth->data, vec_len(sa_auth->data))) {
                ...
                ikev2_elog_uint(IKEV2_LOG_ERROR, "shared key mismatch! ispi %lx", sa->ispi);
            }
        }
        else if (sa_auth->method == IKEV2_AUTH_METHOD_RSA_SIG) {
            if (p->auth.method != IKEV2_AUTH_METHOD_RSA_SIG) continue;
            ...
            if (ikev2_verify_sign(p->auth.key, sa_auth->data, authmsg) == 1) {
                ...
                ikev2_elog_uint(IKEV2_LOG_ERROR, "cert verification failed! ispi %lx", sa->ispi);
            }
        }
    }
    vec_free(authmsg);
...
static void ikev2_sa_auth(ikev2_sa_t *sa, vlib_main_t *vm)
{
    ikev2_main_t         *km    = &ikev2_main;
    ...
    if (!(sa->i_auth.method == IKEV2_AUTH_METHOD_SHARED_KEY_MIC || sa->i_auth.method == IKEV2_AUTH_METHOD_RSA_SIG)) {
        ikev2_elog_uint(IKEV2_LOG_ERROR, "unsupported authentication method %u", sa->i_auth.method);
    ...
    if (sel_p) {
        ...
        if (!sa->is_initiator) {
            ...
            sa->childs[0].r_proposals = ikev2_select_proposal(sa->childs[0].i_proposals, IKEV2_PROTOCOL_ESP);
...
static void ikev2_add_tunnel_from_main(ikev2_add_ipsec_tunnel_args_t *a)
{
    ...
    if (~0 == a->sw_if_index) {
        ...
        rv = ipip_add_tunnel(IPIP_TRANSPORT_IP4,
                             ~0,
                             &ip_addr_46(&a->local_ip),
                             &ip_addr_46(&a->remote_ip),
                             0,
        ...
        if (rv == VNET_API_ERROR_IF_ALREADY_EXISTS) {
            if (hash_get(km->sw_if_indices, sw_if_index)) /* interface is managed by IKE; proceed with updating SAs */
                rv = 0;
        }
    ...
    if (rv) {
        ikev2_elog_uint(IKEV2_LOG_ERROR, "installing ipip tunnel failed! local spi: %x", a->local_spi);
        return;
    }
    ...
    rv = ipsec_sa_add_and_lock(a->remote_sa_id,
                               a->remote_spi,
                               IPSEC_PROTOCOL_ESP,
                               a->encr_type,
                               &a->rem_ckey,
...
static int ikev2_create_tunnel_interface(vlib_main_t *vm, ikev2_sa_t *sa, ikev2_child_sa_t *child, u32 sa_index, u32 child_index, u8 is_rekey, u8 kex)
{
    ...
    if (ikev2_natt_active(sa)) a.flags |= IPSEC_SA_FLAG_UDP_ENCAP;
    a.is_rekey = is_rekey;

    tr = ikev2_sa_get_td_for_type(proposals, IKEV2_TRANSFORM_TYPE_ESN);
    if (tr && tr->esn_type) a.flags |= IPSEC_SA_FLAG_USE_ESN;
    ...
    if (tr) {
        ...
        else if (tr->encr_type == IKEV2_TRANSFORM_ENCR_TYPE_AES_GCM_16 && tr->key_len) {
            switch (tr->key_len) {
                case 16:
    ...
    if (sa->is_initiator) {
        ...
        a.dst_port = ikev2_natt_active(sa) ? sa->dst_port : sa->ipsec_over_udp_port;
        a.src_port = sa->ipsec_over_udp_port;
    }

    if (sa->is_initiator && sa->profile_index != ~0) p = pool_elt_at_index(km->profiles, sa->profile_index);
    ...
    if (thread_index & 0xffffffc0) ikev2_elog_error("error: thread index exceeds max range 0x3f!");

    if (child_index & 0xfffff000 || sa_index & 0xfffff000) ikev2_elog_error("error: sa/child index exceeds max range 0xfff!");

    child->local_sa_id = a.local_sa_id = ikev2_mk_local_sa_id(sa_index, child_index, thread_index);

    u32 remote_sa_id = ikev2_mk_remote_sa_id(sa_index, child_index, thread_index);
    ...
    vl_api_rpc_call_main_thread(ikev2_add_tunnel_from_main, (u8 *) &a, sizeof(a));
    return 0;
}
...
static void ikev2_del_tunnel_from_main(ikev2_del_ipsec_tunnel_args_t *a)
{
    ...
    if (~0 != sw_if_index) ipsec_tun_protect_del(sw_if_index, NULL);

    ipsec_sa_unlock_id(a->remote_sa_id);
    ipsec_sa_unlock_id(a->local_sa_id);
    ipsec_sa_unlock_id(ikev2_flip_alternate_sa_bit(a->remote_sa_id));

    if (ipip) ipip_del_tunnel(ipip->sw_if_index);
}
...
static int ikev2_delete_tunnel_interface(vnet_main_t *vnm, ikev2_sa_t *sa, ikev2_child_sa_t *child)
{
    ...
    vl_api_rpc_call_main_thread(ikev2_del_tunnel_from_main, (u8 *) &a, sizeof(a));
    return 0;
}

static void ikev2_add_invalid_ke_payload(ikev2_sa_t *sa, ikev2_payload_chain_t *chain)
{
    u8                   *data  = vec_new(u8, 2);
    ikev2_sa_transform_t *tr_dh = ikev2_sa_get_td_for_type(sa->r_proposals, IKEV2_TRANSFORM_TYPE_DH);
...
static void ikev2_add_notify_payload(ikev2_sa_t *sa, ikev2_payload_chain_t *chain, u16 notify_type)
{
...
static void ikev2_add_create_child_resp(ikev2_sa_t *sa, ikev2_rekey_t *rekey, ikev2_payload_chain_t *chain)
{
    ...
    if (rekey->kex) ikev2_payload_add_ke(chain, sa->dh_group, sa->r_dh_data);
...
static u32 ikev2_generate_message(vlib_buffer_t *b, ikev2_sa_t *sa, ike_header_t *ike, void *user, udp_header_t *udp, ikev2_stats_t *stats)
{
    ...
    if (ike->exchange == IKEV2_EXCHANGE_SA_INIT) {
        if (sa->r_proposals == 0) {
            ...
            ikev2_payload_add_notify(chain, IKEV2_NOTIFY_MSG_UNSUPPORTED_CRITICAL_PAYLOAD, data);
            vec_free(data);
        }
        else {
            ASSERT(udp);
            ...
            u8 *nat_detection_sha1 =
                ikev2_compute_nat_sha1(clib_host_to_net_u64(sa->ispi), clib_host_to_net_u64(sa->rspi), &sa->raddr, udp->dst_port);
            ikev2_payload_add_notify(chain, IKEV2_NOTIFY_MSG_NAT_DETECTION_SOURCE_IP, nat_detection_sha1);
            vec_free(nat_detection_sha1);
            nat_detection_sha1 = ikev2_compute_nat_sha1(clib_host_to_net_u64(sa->ispi), clib_host_to_net_u64(sa->rspi), &sa->iaddr, udp->src_port);
            ikev2_payload_add_notify(chain, IKEV2_NOTIFY_MSG_NAT_DETECTION_DESTINATION_IP, nat_detection_sha1);
            vec_free(nat_detection_sha1);
        }
    }
        ...
        if (sa->state == IKEV2_STATE_AUTHENTICATED) {
            ...
            if (sa->r_id.type != 0) ikev2_payload_add_id(chain, &sa->r_id, IKEV2_PAYLOAD_IDR);
            ikev2_payload_add_auth(chain, &sa->i_auth);
            ikev2_payload_add_sa(chain, sa->childs[0].i_proposals, 0);
            ikev2_payload_add_ts(chain, sa->childs[0].tsi, IKEV2_PAYLOAD_TSI);
            ikev2_payload_add_ts(chain, sa->childs[0].tsr, IKEV2_PAYLOAD_TSR);
            ikev2_payload_add_notify(chain, IKEV2_NOTIFY_MSG_INITIAL_CONTACT, 0);
        }
        else {
        ...
        if (sa->del) {
            if (sa->del[0].protocol_id == IKEV2_PROTOCOL_IKE) {
                if (ike_hdr_is_request(ike)) ikev2_payload_add_delete(chain, sa->del);
        ...
        if (sa->is_initiator) {
            ...
            *(u32 *) data      = clib_host_to_net_u32(notify.spi);
            ...
            ikev2_payload_add_ts(chain, sa->childs[0].tsi, IKEV2_PAYLOAD_TSI);
            ikev2_payload_add_ts(chain, sa->childs[0].tsr, IKEV2_PAYLOAD_TSR);
            ikev2_payload_add_notify_2(chain, IKEV2_NOTIFY_MSG_REKEY_SA, data, &notify);
            ...
            if (sa->sa_rekey[0].notify_type)
                ...
                ikev2_payload_add_ke(chain, sa->sa_rekey[0].dh_group, sa->sa_rekey[0].r_dh_data);
            }
    ...
    if (sa->is_initiator) ike->flags |= IKEV2_HDR_FLAG_INITIATOR;
    ...
    if (ike->exchange == IKEV2_EXCHANGE_SA_INIT) {
        ...
        if (is_aead) {
            if (!ikev2_encrypt_aead_data(ptd,
                                         sa,
                                         tr_encr,
                                         chain->data,
                                         ph->payload,
                                         ...
                                         ph->payload + plen - sizeof(*ph) - IKEV2_GCM_ICV_SIZE)) {
                tlen = ~0;
                goto done;
            }
        }
        else {
            if (!ikev2_encrypt_data(ptd, sa, tr_encr, chain->data, ph->payload)) {
                tlen = ~0;
                goto done;
            }
            integ = ikev2_calc_integr(tr_integ, sa->is_initiator ? sa->sk_ai : sa->sk_ar, (u8 *) ike, tlen - tr_integ->key_trunc);
            clib_memcpy_fast(ike->payload + tlen - tr_integ->key_trunc - sizeof(*ike), integ, tr_integ->key_trunc);
        }
...
static u32 ikev2_retransmit_sa_init_one(ikev2_sa_t *sa, ike_header_t *ike, ip_address_t iaddr, ip_address_t raddr, u32 rlen)
{
    ...
    if (sa->ispi != clib_net_to_host_u64(ike->ispi) || ip_address_cmp(&sa->iaddr, &iaddr) || ip_address_cmp(&sa->raddr, &raddr)) {
        return 0;
    }
    ...
    while (p < rlen && payload != IKEV2_PAYLOAD_NONE) {
        ...
        if (payload == IKEV2_PAYLOAD_NONCE && !clib_memcmp(sa->i_nonce, ikep->payload, plen - sizeof(*ikep))) {
            /* req is retransmit */
...
{
    if (ike_hdr_is_response(ike)) return 0;
    ...
    if (msg_id == sa->last_msg_id) {
        ...
        ike->length        = tmp->length;
        clib_memcpy_fast(ike->payload, tmp->payload, slen - sizeof(*ike));
        ikev2_elog_uint_peers(IKEV2_LOG_DEBUG, "IKE retransmit msgid %d", msg_id, ip_addr_v4(&sa->raddr).as_u32, ip_addr_v4(&sa->iaddr).as_u32);
        return slen;
    }

    /* old req ignore */
    ikev2_elog_uint_peers(IKEV2_LOG_DEBUG, "IKE req ignore msgid %d", msg_id, ip_addr_v4(&sa->raddr).as_u32, ip_addr_v4(&sa->iaddr).as_u32);
    return ~0;
}
...
{
    vl_api_rpc_call_main_thread(ikev2_del_sa_init_from_main, (u8 *) &ispi, sizeof(ispi));
}
...
static void ikev2_elog_uint_peers_addr(u32 exchange, ip4_header_t *ip4, ip6_header_t *ip6, u8 is_ip4)
{
    ...
    ikev2_elog_uint_peers(IKEV2_LOG_WARNING,
                          "IKEv2 exchange %d "
...
static void ikev2_generate_sa_init_data_and_log(ikev2_sa_t *sa)
{
    ...
    if (PREDICT_TRUE(rc == IKEV2_GENERATE_SA_INIT_OK)) return;
...
{
    vlib_node_increment_counter(vm, node_index, IKEV2_ERROR_KEEPALIVE, s->n_keepalives);
    vlib_node_increment_counter(vm, node_index, IKEV2_ERROR_REKEY_REQ, s->n_rekey_req);
    vlib_node_increment_counter(vm, node_index, IKEV2_ERROR_INIT_SA_REQ, s->n_sa_init_req);
    vlib_node_increment_counter(vm, node_index, IKEV2_ERROR_IKE_AUTH_REQ, s->n_sa_auth_req);
}
...
static uword ikev2_node_internal(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_ip4, u8 natt)
{
    ...
    while (n_left > 0) {
        ...
        if (PREDICT_TRUE(thread_index != km->handoff_thread)) {
            vlib_node_increment_counter(vm, node->node_index, IKEV2_ERROR_HANDOFF, 1);
        ...
        if (natt) {
            ASSERT(*((u32 *) ike0) == 0);
            ike0 = (ike_header_t *) ((u8 *) ike0 + sizeof(ikev2_non_esp_marker));
            rlen -= sizeof(ikev2_non_esp_marker);
        }

        if (clib_net_to_host_u32(ike0->length) != rlen) {
            vlib_node_increment_counter(vm, node->node_index, IKEV2_ERROR_BAD_LENGTH, 1);
            goto dispatch0;
        }

        if (ike0->version != IKE_VERSION_2) {
            vlib_node_increment_counter(vm, node->node_index, IKEV2_ERROR_NOT_IKEV2, 1);
            goto dispatch0;
        }
        ...
        if (ike0->exchange == IKEV2_EXCHANGE_SA_INIT) {
            ...
            if (ike_hdr_is_initiator(ike0)) {
                ...
                if (ike0->rspi == 0) {
                    if (is_ip4)
                        ikev2_set_ip_address(sa0, &ip40->src_address, &ip40->dst_address, AF_IP4);
                    else
                        ikev2_set_ip_address(sa0, &ip60->src_address, &ip60->dst_address, AF_IP6);

                    sa0->dst_port = clib_net_to_host_u16(udp0->src_port);

                    slen = ikev2_retransmit_sa_init(ike0, sa0->iaddr, sa0->raddr, rlen);
                    if (slen) {
                        vlib_node_increment_counter(vm,
                                                    node->node_index,
                                                    ~0 == slen ? IKEV2_ERROR_IKE_SA_INIT_IGNORE : IKEV2_ERROR_IKE_SA_INIT_RETRANSMIT,
                                                    1);
                        goto dispatch0;
                    }

                    res = ikev2_process_sa_init_req(vm, sa0, ike0, udp0, rlen, vnet_buffer(b0)->sw_if_index[VLIB_RX]);
                    if (!res) vlib_node_increment_counter(vm, node->node_index, IKEV2_ERROR_MALFORMED_PACKET, 1);

                    if (sa0->state == IKEV2_STATE_SA_INIT) {
                        ikev2_sa_free_proposal_vector(&sa0->r_proposals);
                        sa0->r_proposals = ikev2_select_proposal(sa0->i_proposals, IKEV2_PROTOCOL_IKE);
                        ikev2_generate_sa_init_data_and_log(sa0);
                    }

                    if (sa0->state == IKEV2_STATE_SA_INIT || sa0->state == IKEV2_STATE_NOTIFY_AND_DELETE) {
                        ike0->flags = IKEV2_HDR_FLAG_RESPONSE;
                        slen        = ikev2_generate_message(b0, sa0, ike0, 0, udp0, stats);
                        if (~0 == slen) vlib_node_increment_counter(vm, node->node_index, IKEV2_ERROR_NO_BUFF_SPACE, 1);
                    }
            ...
            {
                if (is_ip4)
                    ikev2_set_ip_address(sa0, &ip40->dst_address, &ip40->src_address, AF_IP4);
                else
                    ikev2_set_ip_address(sa0, &ip60->dst_address, &ip60->src_address, AF_IP6);
                ...
                if (sa0->state == IKEV2_STATE_SA_INIT) {
                    ...
                    if (p) {
                        ...
                        if (clib_atomic_bool_cmp_and_swap(&sai->init_response_received, 0, 1)) {
                            ikev2_complete_sa_data(sa0, sai);
                            ikev2_calc_keys(sa0);
                            ikev2_sa_auth_init(sa0);
                            ike0->flags           = IKEV2_HDR_FLAG_INITIATOR;
                            ike0->msgid           = clib_net_to_host_u32(sai->last_init_msg_id);
            ...
            if (p) {
                ...
                if (slen) {
                    vlib_node_increment_counter(vm, node->node_index, ~0 == slen ? IKEV2_ERROR_IKE_REQ_IGNORE : IKEV2_ERROR_IKE_REQ_RETRANSMIT, 1);
                    goto dispatch0;
                }
                ...
                if (res)
                    ...
                    vlib_node_increment_counter(vm, node->node_index, IKEV2_ERROR_MALFORMED_PACKET, 1);
                if (sa0->state == IKEV2_STATE_AUTHENTICATED) {
                    ikev2_initial_contact_cleanup(ptd, sa0);
                    p = hash_get(ptd->sa_by_rspi, clib_net_to_host_u64(ike0->rspi));
                    ikev2_sa_match_ts(sa0);
                    if (sa0->state != IKEV2_STATE_TS_UNACCEPTABLE) ikev2_create_tunnel_interface(vm, sa0, &sa0->childs[0], p[0], 0, 0, 0);
                }
                ...
                if (sa0->del) {
                    if (sa0->del[0].protocol_id != IKEV2_PROTOCOL_IKE) {
                        ...
                        vec_foreach (d, sa0->del) {
                            ...
                            ch_sa = ikev2_sa_get_child(sa0, d->spi, d->protocol_id, !sa0->is_initiator);
                            if (ch_sa) {
                                ikev2_delete_tunnel_interface(km->vnet_main, sa0, ch_sa);
                                if (!sa0->is_initiator) {
                                    vec_add2(resp, tmp, 1);
            ...
            if (p) {
                ...
                if (vec_len(sa0->rekey) > 0) {
                    if (!sa0->rekey[0].notify_type && sa0->rekey[0].protocol_id != IKEV2_PROTOCOL_IKE) {
                        if (vec_len(sa0->childs) > 0) ikev2_sa_free_all_child_sa(&sa0->childs);
                        ...
                        ikev2_create_tunnel_interface(vm, sa0, child, p[0], child - sa0->childs, 1, sa0->rekey[0].kex);
                    }
                    ...
                    ikev2_create_tunnel_interface(vm, sa0, c, p[0], c - sa0->childs, 0, sa0->new_child[0].kex);
                    if (ike_hdr_is_request(ike0)) {
                        ike0->flags = IKEV2_HDR_FLAG_RESPONSE;
                        slen        = ikev2_generate_message(b0, sa0, ike0, 0, udp0, 0);
                        if (~0 == slen) vlib_node_increment_counter(vm, node->node_index, IKEV2_ERROR_NO_BUFF_SPACE, 1);
                    }
                }
    ...
    dispatch0:
        ...
        if (slen && ~0 != slen) {
            ...
            if (is_req) {
                udp0->dst_port = udp0->src_port = clib_net_to_host_u16(ikev2_get_port(sa0));

                if (udp0->dst_port == clib_net_to_host_u16(IKEV2_PORT_NATT) && ikev2_natt_active(sa0)) {
                    if (!natt) slen = ikev2_insert_non_esp_marker(ike0, slen);
                }
            }
            else {
                if (natt) slen += sizeof(ikev2_non_esp_marker);
            ...
            if (is_ip4) {
                ...
                ip60->payload_length = clib_host_to_net_u16(b0->current_length - sizeof(*ip60));
            }
        }
        /* delete sa */
        if (sa0 && (sa0->state == IKEV2_STATE_DELETED || sa0->state == IKEV2_STATE_NOTIFY_AND_DELETE)) {
            ikev2_child_sa_t *c;
    ...
    vlib_node_increment_counter(vm, node->node_index, IKEV2_ERROR_PROCESSED, frame->n_vectors);
                                                   ...
                                                   int                   is_ike)
{
    ...
    error = 1;
    vec_foreach (td, km->supported_transforms) {
        if (td->type == IKEV2_TRANSFORM_TYPE_ENCR && td->encr_type == ts->crypto_alg && td->key_len == ts->crypto_key_size / 8) {
            u16 attr[2];
    ...
    if (IKEV2_TRANSFORM_INTEG_TYPE_NONE != ts->integ_alg) {
        ...
        error = 1;
        vec_foreach (td, km->supported_transforms) {
            if (td->type == IKEV2_TRANSFORM_TYPE_INTEG && td->integ_type == ts->integ_alg) {
                vec_add1(proposal->transforms, *td);
                error = 0;
                break;
            }
        ...
        if (error) {
            ikev2_elog_error("Didn't find any supported algorithm for IKEV2_TRANSFORM_TYPE_INTEG");
            r = clib_error_return(0, "Unsupported algorithm");
            return r;
        }
    }
    ...
    if (is_ike) {
        error = 1;
        vec_foreach (td, km->supported_transforms) {
            if (td->type == IKEV2_TRANSFORM_TYPE_PRF && td->prf_type == IKEV2_TRANSFORM_PRF_TYPE_PRF_HMAC_SHA2_256) {
                vec_add1(proposal->transforms, *td);
                error = 0;
                break;
            }
...
static void ikev2_send_ike(vlib_main_t *vm, ip_address_t *src, ip_address_t *dst, u32 bi0, u32 len, u16 src_port, u16 dst_port, u32 sw_if_index)
{
    ...
    if (ip_addr_version(dst) == AF_IP4) {
        ...
        ip60->hop_limit                               = 0xff;
        clib_memcpy_fast(ip60->src_address.as_u8, ip_addr_v6(src).as_u8, sizeof(ip60->src_address));
        clib_memcpy_fast(ip60->dst_address.as_u8, ip_addr_v6(dst).as_u8, sizeof(ip60->src_address));
    }
    ...
    u32 next_index = (ip_addr_version(dst) == AF_IP4) ? ip4_lookup_node.index : ip6_lookup_node.index;
...
clib_error_t *ikev2_set_local_key(vlib_main_t *vm, u8 *file)
{
    ...
    if (km->pkey) EVP_PKEY_free(km->pkey);
    km->pkey = ikev2_load_key_file(file);
    if (km->pkey == NULL) return clib_error_return(0, "load key '%s' failed", file);
...
{
    if (p->ipsec_over_udp_port == IPSEC_UDP_PORT_NONE) return;
...
static void ikev2_initiate_delete_ike_sa_internal(vlib_main_t *vm, ikev2_main_per_thread_data_t *tkm, ikev2_sa_t *sa, u8 send_notification)
{
    ...
    if (send_notification) {
        ...
        if (ikev2_natt_active(sa)) len = ikev2_insert_non_esp_marker(ike0, len);
        ...
        ikev2_send_ike(vm, src, dst, bi0, len, ikev2_get_port(sa), sa->dst_port, 0);
    }
...
static void ikev2_cleanup_profile_sessions(ikev2_main_t *km, ikev2_profile_t *p)
{
    ...
    pool_foreach (sa, km->sais) {
        if (pi == sa->profile_index) vec_add1(del_sai, sa - km->sais);
    }
    ...
    vec_foreach (tkm, km->per_thread_data) {
        pool_foreach (sa, tkm->sas) {
            if (sa->profile_index != ~0 && pi == sa->profile_index) vec_add1(del_sai, sa - tkm->sas);
        }
...
static void ikev2_profile_free(ikev2_profile_t *p)
{
    ...
    if (p->auth.key) EVP_PKEY_free(p->auth.key);
...
static void ikev2_bind(vlib_main_t *vm, ikev2_main_t *km)
{
    if (0 == km->bind_refcount) {
        ...
        vlib_punt_register(km->punt_hdl, ipsec_punt_reason[IPSEC_PUNT_IP4_SPI_UDP_0], "ikev2-ip4-natt");
    }

    km->bind_refcount++;
}

static void ikev2_unbind(vlib_main_t *vm, ikev2_main_t *km)
{
    km->bind_refcount--;
    if (0 == km->bind_refcount) {
        vlib_punt_unregister(km->punt_hdl, ipsec_punt_reason[IPSEC_PUNT_IP4_SPI_UDP_0], "ikev2-ip4-natt");
...
clib_error_t *ikev2_add_del_profile(vlib_main_t *vm, u8 *name, int is_add)
{
    ...
    if (is_add) {
        if (ikev2_profile_index_by_name(name)) return clib_error_return(0, "policy %v already exists", name);
        ...
        if (!p) return clib_error_return(0, "policy %v does not exists", name);
...
clib_error_t *ikev2_set_profile_auth(vlib_main_t *vm, u8 *name, u8 auth_method, u8 *auth_data, u8 data_hex_format)
{
    ...
    if (auth_method == IKEV2_AUTH_METHOD_RSA_SIG) {
        ...
        if (p->auth.key == NULL) return clib_error_return(0, "load cert '%s' failed", p->auth.data);
    }
...
{
    return (id_type == IKEV2_ID_TYPE_ID_IPV4_ADDR || id_type == IKEV2_ID_TYPE_ID_IPV6_ADDR || id_type == IKEV2_ID_TYPE_ID_RFC822_ADDR
...
clib_error_t *ikev2_set_profile_id(vlib_main_t *vm, u8 *name, u8 id_type, u8 *data, int is_local)
{
    ...
    if (!ikev2_is_id_supported(id_type)) {
        r = clib_error_return(0, "unsupported identity type %U", format_ikev2_id_type, id_type);
        return r;
    }
                                   ...
                                   int          is_local)
{
    ...
    if (ip_addr_version(&start_addr) != ip_addr_version(&end_addr)) return clib_error_return(0, "IP address version mismatch!");
                                               ...
                                               u32                          crypto_key_size)
{
    ...
    p = ikev2_profile_index_by_name(name);
    if (!p) return clib_error_return(0, "unknown profile %v", name);

    if ((IKEV2_TRANSFORM_INTEG_TYPE_NONE != integ_alg) + (IKEV2_TRANSFORM_ENCR_TYPE_AES_GCM_16 == crypto_alg) != 1)
...
vnet_api_error_t ikev2_set_profile_ipsec_udp_port(vlib_main_t *vm, u8 *name, u16 port, u8 is_set)
{
    ...
    if (!p) return VNET_API_ERROR_INVALID_VALUE;

    if (is_set) {
        if (p->ipsec_over_udp_port != IPSEC_UDP_PORT_NONE) return VNET_API_ERROR_VALUE_EXIST;

        rv = ikev2_register_udp_port(p, port);
    }
    else {
        if (p->ipsec_over_udp_port == IPSEC_UDP_PORT_NONE) return VNET_API_ERROR_INVALID_VALUE;
...
static clib_error_t *ikev2_resolve_responder_hostname(vlib_main_t *vm, ikev2_responder_t *r)
{
    ...
    if (!km->dns_resolve_name_ptr) return clib_error_return(0, "cannot load symbols from dns plugin");
    ...
    rv = ((__typeof__(dns_resolve_name) *) km->dns_resolve_name_ptr)(name, &ep, t0, rn);
    vec_free(name);
    if (rv < 0) return clib_error_return(0, "dns lookup failure");
...
clib_error_t *ikev2_initiate_sa_init(vlib_main_t *vm, u8 *name)
{
    ...
    if (p->responder.sw_if_index == ~0 || (ip_address_is_zero(&p->responder.addr) && vec_len(p->responder.hostname) == 0)) {
    ...
    if (p->responder.is_resolved && ikev2_get_if_address(p->responder.sw_if_index, ip_addr_version(&p->responder.addr), &src_if_ip)) {
        valid_ip = 1;
    }
    ...
    if (p->natt_disabled) sa.natt_state = IKEV2_NATT_DISABLED;
    ...
    ikev2_set_initiator_proposals(vm, &sa, &p->esp_ts, &sa.childs[0].i_proposals, 0);
    sa.childs[0].i_proposals[0].proposal_num = 1;
    sa.childs[0].i_proposals[0].protocol_id  = IKEV2_PROTOCOL_ESP;
    RAND_bytes((u8 *) &sa.childs[0].i_proposals[0].spi, sizeof(sa.childs[0].i_proposals[0].spi));
    sa.childs[0].i_proposals[0].spi &= 0xffffffff;

    /* Add NAT detection notification messages (mandatory) */
    u8 *nat_detection_sha1 =
        ikev2_compute_nat_sha1(clib_host_to_net_u64(sa.ispi), clib_host_to_net_u64(sa.rspi), &src_if_ip, clib_host_to_net_u16(IKEV2_PORT));

    ikev2_payload_add_notify(chain, IKEV2_NOTIFY_MSG_NAT_DETECTION_SOURCE_IP, nat_detection_sha1);
    vec_free(nat_detection_sha1);
    nat_detection_sha1 =
        ikev2_compute_nat_sha1(clib_host_to_net_u64(sa.ispi), clib_host_to_net_u64(sa.rspi), &p->responder.addr, clib_host_to_net_u16(sa.dst_port));
    ikev2_payload_add_notify(chain, IKEV2_NOTIFY_MSG_NAT_DETECTION_DESTINATION_IP, nat_detection_sha1);
    vec_free(nat_detection_sha1);
    ...
    ikev2_payload_add_notify(chain, IKEV2_NOTIFY_MSG_SIGNATURE_HASH_ALGORITHMS, sig_hash_algo);
    ...
    if (valid_ip) {
        ikev2_send_ike(vm, &src_if_ip, &p->responder.addr, bi0, len, IKEV2_PORT, sa.dst_port, sa.sw_if_index);

        ikev2_elog_exchange("ispi %lx rspi %lx IKEV2_EXCHANGE_SA_INIT sent to ",
                            clib_host_to_net_u64(sa0->ispi),
                            0,
...
static void ikev2_delete_child_sa_internal(vlib_main_t *vm, ikev2_sa_t *sa, ikev2_child_sa_t *csa)
{
    ...
    if (ikev2_natt_active(sa)) len = ikev2_insert_non_esp_marker(ike0, len);
    ikev2_send_ike(vm, &sa->iaddr, &sa->raddr, bi0, len, ikev2_get_port(sa), sa->dst_port, sa->sw_if_index);
...
static void ikev2_rekey_child_sa_internal(vlib_main_t *vm, ikev2_sa_t *sa, ikev2_child_sa_t *csa)
{
    ...
    if (ikev2_natt_active(sa)) len = ikev2_insert_non_esp_marker(ike0, len);
    ikev2_send_ike(vm, &sa->iaddr, &sa->raddr, bi0, len, ikev2_get_port(sa), ikev2_get_port(sa), sa->sw_if_index);
    vec_free(proposals);
}
...
static void ikev2_sa_del(ikev2_profile_t *p, u32 sw_if_index)
{
    ...
    vec_foreach (tkm, km->per_thread_data) {
        pool_foreach (sa, tkm->sas) {
            if (ikev2_sa_sw_if_match(sa, sw_if_index)) vec_add1(sa_vec, sa);
        }
    ...
    pool_foreach (sa, km->sais) {
        if (ikev2_sa_sw_if_match(sa, sw_if_index)) vec_add1(ispi_vec, sa->ispi);
    }
...
static clib_error_t *ikev2_sw_interface_add_del(vnet_main_t *vnm, u32 sw_if_index, u32 is_add)
{
    ...
    pool_foreach (p, km->profiles) {
        if (p->responder.sw_if_index == sw_if_index) ikev2_sa_del(p, sw_if_index);
    }
...
static clib_error_t *ikev2_config(vlib_main_t *vm, unformat_input_t *input)
{
    ikev2_main_t *km = &ikev2_main;
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "handoff-thread %d", &km->handoff_thread)) {
            if (km->handoff_thread > vlib_num_workers()) {
                return clib_error_return(0, "wrong handoff-thread %d", km->handoff_thread);
            }
        }
        else
...
static u8 ikev2_mngr_process_child_sa(ikev2_sa_t *sa, ikev2_child_sa_t *csa, u8 del_old_ids)
{
    ...
    if (sa->profile_index != ~0) p = pool_elt_at_index(km->profiles, sa->profile_index);

    if (sa->is_initiator && p && csa->time_to_expiration && now > csa->time_to_expiration) {
        if (!csa->is_expired || csa->rekey_retries > 0) {
            ...
            if (csa->rekey_retries == 0) {
                ...
                csa->rekey_retries--;
                ikev2_log_debug("Rekeying Child SA 0x%x, retries left %d", csa->i_proposals->spi, csa->rekey_retries);
                if (csa->rekey_retries == 0) {
                    csa->rekey_retries = -1;
                }
            }
    ...
    if (del_old_ids) {
        ...
        if (~0 == sw_if_index) {
            ...
            ip46_address_t remote_ip;
            if (sa->is_initiator) {
                local_ip  = to_ip46(ip_addr_version(&sa->iaddr), ip_addr_bytes(&sa->iaddr));
                remote_ip = to_ip46(ip_addr_version(&sa->raddr), ip_addr_bytes(&sa->raddr));
            }
            else {
                local_ip  = to_ip46(ip_addr_version(&sa->raddr), ip_addr_bytes(&sa->raddr));
                remote_ip = to_ip46(ip_addr_version(&sa->iaddr), ip_addr_bytes(&sa->iaddr));
            }
        ...
        int rv = ipsec_tun_protect_update(sw_if_index, NULL, csa->local_sa_id, sas_in);
        if (rv) vec_free(sas_in);
...
clib_error_t *ikev2_set_liveness_params(u32 period, u32 max_retries)
{
    ...
    if (period == 0 || max_retries == 0) return clib_error_return(0, "invalid args");
...
static void ikev2_mngr_process_ipsec_sa(ipsec_sa_t *ipsec_sa)
{
    ...
    vlib_get_combined_counter(&ipsec_sa_counters, ipsec_sa->stat_index, &counts);

    if (fsa && fsa->profile_index != ~0 && fsa->is_initiator) p = pool_elt_at_index(km->profiles, fsa->profile_index);
...
static void ikev2_process_pending_sa_init_one(vlib_main_t *vm, ikev2_main_t *km, ikev2_sa_t *sa)
{
    ...
    if (!p->responder.is_resolved) {
        ...
        if (!p->responder.is_resolved) return;

        ip_address_copy(&sa->raddr, &p->responder.addr);

        /* update nat detection destination hash */
        np = ikev2_find_ike_notify_payload((ike_header_t *) sa->last_sa_init_req_packet_data, IKEV2_NOTIFY_MSG_NAT_DETECTION_DESTINATION_IP);
        if (np) {
            nat_sha = ikev2_compute_nat_sha1(clib_host_to_net_u64(sa->ispi),
    ...
    if (ip_address_is_zero(&sa->iaddr)) {
        if (!ikev2_get_if_address(p->responder.sw_if_index, ip_addr_version(&p->responder.addr), &sa->iaddr)) return;

        /* update NAT detection payload */
        np = ikev2_find_ike_notify_payload((ike_header_t *) sa->last_sa_init_req_packet_data, IKEV2_NOTIFY_MSG_NAT_DETECTION_SOURCE_IP);
        if (np) {
            nat_sha =
                ikev2_compute_nat_sha1(clib_host_to_net_u64(sa->ispi), clib_host_to_net_u64(sa->rspi), &sa->iaddr, clib_host_to_net_u16(IKEV2_PORT));
            clib_memcpy_fast(np, nat_sha, vec_len(nat_sha));
            vec_free(nat_sha);
        }
    }

    if (vlib_buffer_alloc(km->vlib_main, &bi0, 1) != 1) return;

    vlib_buffer_t *b = vlib_get_buffer(km->vlib_main, bi0);
    clib_memcpy_fast(vlib_buffer_get_current(b), sa->last_sa_init_req_packet_data, vec_len(sa->last_sa_init_req_packet_data));

    ikev2_send_ike(km->vlib_main,
                   &sa->iaddr,
                   &sa->raddr,
                   bi0,
...
static void ikev2_process_pending_sa_init(vlib_main_t *vm, ikev2_main_t *km)
{
    ...
    hash_foreach(ispi, sai, km->sa_by_ispi, ({
                     ...
                     if (sa->init_response_received) continue;
...
static void ikev2_send_informational_request(ikev2_sa_t *sa)
{
    ...
    ikev2_send_ike(km->vlib_main, src, dst, bi0, len, ikev2_get_port(sa), dp, sa->sw_if_index);
}
...
static int ikev2_mngr_process_responder_sas(ikev2_sa_t *sa)
{
    ...
    if (!sa->keys_generated) return 0;

    if (sa->liveness_retries >= km->liveness_max_retries) return 1;
    ...
    while (1) {
        ...
        vec_foreach (tkm, km->per_thread_data) {
            ...
            pool_foreach (sa, tkm->sas) {
                ...
                if (sa->state == IKEV2_STATE_SA_INIT) {
                    if (vec_len(sa->childs) > 0) vec_add1(to_be_deleted, sa - tkm->sas);
                }
                ...
                if (!km->dpd_disabled && ikev2_mngr_process_responder_sas(sa)) vec_add1(to_be_deleted, sa - tkm->sas);
            }
...
static void ikev2_lazy_init(ikev2_main_t *km)
{
    ...
    if (km->lazy_init_done) return;

    ikev2_crypto_init(km);

    mhash_init_vec_string(&km->profile_index_by_name, sizeof(uword));

    vec_validate_aligned(km->per_thread_data, tm->n_vlib_mains - 1, CLIB_CACHE_LINE_BYTES);
    for (thread_id = 0; thread_id < tm->n_vlib_mains; thread_id++) {
        ikev2_main_per_thread_data_t *ptd = vec_elt_at_index(km->per_thread_data, thread_id);
    ...
    km->dns_resolve_name_ptr = vlib_get_plugin_symbol("dns_plugin.so", "dns_resolve_name");
    if (!km->dns_resolve_name_ptr) ikev2_log_error("cannot load symbols from dns plugin");

    km->handoff_ip4_fq_index = vlib_frame_queue_main_init(ikev2_node_ip4.index, 0);

    km->handoff_ip4_natt_fq_index = vlib_frame_queue_main_init(ikev2_node_ip4_natt.index, 0);

    km->handoff_ip6_fq_index = vlib_frame_queue_main_init(ikev2_node_ip6.index, 0);

    /* wake up ikev2 process */
    vlib_process_signal_event(vlib_get_first_main(), ikev2_mngr_process_node.index, 0, 0);
