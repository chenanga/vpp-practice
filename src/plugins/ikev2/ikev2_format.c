...
u8 *format_ikev2_sa_transform(u8 *s, va_list *args)
{
    ...
    if (tr->type >= IKEV2_TRANSFORM_NUM_TYPES) return s;
    ...
    if (tr->type == IKEV2_TRANSFORM_TYPE_ENCR && tr->encr_type == IKEV2_TRANSFORM_ENCR_TYPE_AES_CBC && tr->key_len)
        ...
        s = format(s, "(unknown attr %U)", format_hex_bytes, tr->attrs, vec_len(tr->attrs));
