...
void ikev2_payload_add_sa(ikev2_payload_chain_t *c, ikev2_sa_proposal_t *proposals, u8 force_spi)
{
    ...
    vec_foreach (p, proposals) {
        ...
        vec_foreach (t, p->transforms) {
            ...
            if (vec_len(t->attrs) > 0) clib_memcpy_fast(tr->attributes, t->attrs, vec_len(t->attrs));
        }
...
void ikev2_payload_add_ke(ikev2_payload_chain_t *c, u16 dh_group, u8 *dh_data)
{
    ...
    ke = (ike_ke_payload_header_t *) ikev2_payload_add_hdr(c, IKEV2_PAYLOAD_KE, sizeof(*ke));
...
void ikev2_payload_add_delete(ikev2_payload_chain_t *c, ikev2_delete_t *d)
{
    ...
    ikev2_delete_t              *d2;
    dp = (ike_delete_payload_header_t *) ikev2_payload_add_hdr(c, IKEV2_PAYLOAD_DELETE, sizeof(*dp));
...
void ikev2_payload_add_auth(ikev2_payload_chain_t *c, ikev2_auth_t *auth)
{
    ...
    ap = (ike_auth_payload_header_t *) ikev2_payload_add_hdr(c, IKEV2_PAYLOAD_AUTH, sizeof(*ap));
...
void ikev2_payload_add_ts(ikev2_payload_chain_t *c, ikev2_ts_t *ts, u8 type)
{
    ...
    vec_foreach (ts2, ts) {
        ASSERT(ts2->ts_type == TS_IPV4_ADDR_RANGE || ts2->ts_type == TS_IPV6_ADDR_RANGE);
...
ikev2_sa_proposal_t *ikev2_parse_sa_payload(ike_payload_header_t *ikep, u32 rlen)
{
    ...
    if (sizeof(*ikep) > rlen) return 0;

    rlen -= sizeof(*ikep);
    do {
        if (proposal_ptr + sizeof(*sap) > rlen) goto data_corrupted;

        sap = (ike_sa_proposal_data_t *) &ikep->payload[proposal_ptr];
        int i, transform_ptr;

        /* IKE proposal should have 8 bytes or no SPI */
        if (sap->protocol_id == IKEV2_PROTOCOL_IKE && sap->spi_size != 0 && sap->spi_size != 8) goto data_corrupted;

        /* IKE proposal should not have SPI */
        if (sap->protocol_id == IKEV2_PROTOCOL_ESP && sap->spi_size != 4) goto data_corrupted;

        transform_ptr = proposal_ptr + sizeof(*sap) + sap->spi_size;
        if (transform_ptr > rlen) goto data_corrupted;
        ...
        for (i = 0; i < sap->num_transforms; i++) {
            ...
            if (transform_ptr + sizeof(*tr) > rlen) goto data_corrupted;
            u16 tlen = clib_net_to_host_u16(tr->transform_len);

            if (tlen < sizeof(*tr)) goto data_corrupted;
            ...
            if (transform_ptr + tlen > rlen) goto data_corrupted;
            if (tlen > sizeof(*tr)) vec_add(transform->attrs, tr->attributes, tlen - sizeof(*tr));
            transform_ptr += tlen;
        }
    ...
    /* data validation */
    if (proposal_ptr != (plen - sizeof(*ikep)) || sap->last_or_more) goto data_corrupted;
...
ikev2_ts_t *ikev2_parse_ts_payload(ike_payload_header_t *ikep, u32 rlen)
{
    ...
    while (n_left && p + sizeof(*pe) < rlen) {
        ...
        if (pe->ts_type != TS_IPV4_ADDR_RANGE && pe->ts_type != TS_IPV6_ADDR_RANGE) {
            ikev2_elog_uint(IKEV2_LOG_ERROR, "unsupported TS type received (%u)", pe->ts_type);
            return 0;
        }
...
ikev2_notify_t *ikev2_parse_notify_payload(ike_payload_header_t *ikep, u32 rlen)
{
    ...
    if (n->spi_size == 4) {
        if (sizeof(spi) + sizeof(*n) > rlen) goto cleanup;
    ...
    if (plen > (sizeof(*n) + n->spi_size)) {
        if (plen <= sizeof(*n) + n->spi_size) goto cleanup;
...
ikev2_delete_t *ikev2_parse_delete_payload(ike_payload_header_t *ikep, u32 rlen)
{
    ...
    if (d->protocol_id == IKEV2_PROTOCOL_IKE) {
        ...
        if (sizeof(*d) + num_of_spi * sizeof(u32) > rlen) return 0;
...
u8 *ikev2_find_ike_notify_payload(ike_header_t *ike, u32 msg_type)
{
    ...
    while (payload != IKEV2_PAYLOAD_NONE) {
        ...
        if (payload == IKEV2_PAYLOAD_NOTIFY) {
            ...
            if (n->msg_type == clib_net_to_host_u16(msg_type)) return n->payload;
        }
