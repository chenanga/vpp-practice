...
u8 *format_ikev2_handoff_trace(u8 *s, va_list *args)
{
    CLIB_UNUSED(vlib_main_t * vm)   = va_arg(*args, vlib_main_t *);
    CLIB_UNUSED(vlib_node_t * node) = va_arg(*args, vlib_node_t *);

    ikev2_handoff_trace_t *t = va_arg(*args, ikev2_handoff_trace_t *);
    s                        = format(s, "ikev2 handoff  %d to %d", t->current_worker_index, t->next_worker_index);
    return s;
}

static_always_inline uword ikev2_handoff_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u32 fq_index)
{
    ...
    while (n_left_from > 0) {
        ...
        if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && b[0]->flags & VLIB_BUFFER_IS_TRACED)) {
            ikev2_handoff_trace_t *t = vlib_add_trace(vm, node, b[0], sizeof(*t));
    ...
    if (n_enq < frame->n_vectors) vlib_node_increment_counter(vm, node->node_index, IKEV2_HANDOFF_ERROR_CONGESTION_DROP, frame->n_vectors - n_enq);
    return n_enq;
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *from_frame)
{
    ...
    return ikev2_handoff_inline(vm, node, from_frame, km->handoff_ip4_natt_fq_index);
}
