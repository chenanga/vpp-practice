                            ...
                            u32                          *out_len)
{
    ...
    if (data_len <= IKEV2_GCM_IV_SIZE) /* runt data */
        return 0;
...
void ikev2_complete_dh(ikev2_sa_t *sa, ikev2_sa_transform_t *t)
{
    ...
    if (t->dh_group == IKEV2_DH_GROUP_MODP) {
        ...
        priv_key = BN_bin2bn(sa->dh_private_key, vec_len(sa->dh_private_key), NULL);
        DH_set0_key(dh, NULL, priv_key);
#else
        BN_hex2bn(&dh->p, t->dh_p);
        BN_hex2bn(&dh->g, t->dh_g);

        dh->priv_key = BN_bin2bn(sa->dh_private_key, vec_len(sa->dh_private_key), NULL);
#endif
        BIGNUM *ex;
        sa->dh_shared_key = vec_new(u8, t->key_len);
        ...
        EC_POINT_set_affine_coordinates_GFp(group, i_point, x, y, bn_ctx);
#endif
        EC_POINT_mul(group, shared_point, NULL, r_point, EC_KEY_get0_private_key(ec), NULL);
#if OPENSSL_VERSION_NUMBER >= 0x30000000L
        EC_POINT_get_affine_coordinates(group, shared_point, x, y, bn_ctx);
#else
        EC_POINT_get_affine_coordinates_GFp(group, shared_point, x, y, bn_ctx);
...
EVP_PKEY *ikev2_load_cert_file(u8 *file)
{
    ...
    X509_free(x509);
    if (pkey == NULL) ikev2_log_error("get pubkey %s failed", file);
...
EVP_PKEY *ikev2_load_key_file(u8 *file)
{
    ...
    fclose(fp);
    if (pkey == NULL) ikev2_log_error("read %s failed", file);
