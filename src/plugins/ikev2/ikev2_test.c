...
u8 *format_ikev2_id_type_and_data(u8 *s, va_list *args)
{
    ...
    if (id->type == 0) return format(s, "none");
...
{
    vl_api_ikev2_sa_transform_t *tr = va_arg(*args, vl_api_ikev2_sa_transform_t *);

    if (!tr) return s;

    if (tr->transform_type >= IKEV2_TRANSFORM_NUM_TYPES) return s;
    ...
    switch (tr->transform_type) {
        ...
        case IKEV2_TRANSFORM_TYPE_INTEG:
            s = format(s, "%U", format_ikev2_transform_integ_type, tr->transform_id);
            break;
    ...
    if (tr->transform_type == IKEV2_TRANSFORM_TYPE_ENCR && tr->transform_id == IKEV2_TRANSFORM_ENCR_TYPE_AES_CBC && tr->key_len)
        s = format(s, "-%u", tr->key_len * 8);

    return s;
}

static int api_ikev2_profile_disable_natt(vat_main_t *vam)
{
...
static int api_ikev2_profile_dump(vat_main_t *vam)
{
    ...
    if (!ik->ping_id) ik->ping_id = vl_msg_api_get_msg_index((u8 *) (VL_API_CONTROL_PING_CRC));
...
static void vl_api_ikev2_profile_details_t_handler(vl_api_ikev2_profile_details_t *mp)
{
    ...
    if (p->auth.method) {
        if (p->auth.hex)
            ...
            fformat(vam->ofp, "  auth-method %U auth data %v\n", format_ikev2_auth_method, p->auth.method, format(0, "%s", p->auth.data));
    }
    ...
    if (~0 != tun_itf) fformat(vam->ofp, "  protected tunnel idx %d\n", tun_itf);

    u32 sw_if_index = clib_net_to_host_u32(p->responder.sw_if_index);
    if (~0 != sw_if_index) fformat(vam->ofp, "  responder idx %d %U\n", sw_if_index, format_ip_address, &p->responder.addr);

    if (p->udp_encap) fformat(vam->ofp, "  udp-encap\n");

    if (p->natt_disabled) fformat(vam->ofp, "  NAT-T disabled\n");

    u32 ipsec_over_udp_port = clib_net_to_host_u16(p->ipsec_over_udp_port);
    if (ipsec_over_udp_port != IPSEC_UDP_PORT_NONE) fformat(vam->ofp, "  ipsec-over-udp port %d\n", ipsec_over_udp_port);

    u32 crypto_key_size = clib_net_to_host_u32(p->ike_ts.crypto_key_size);
    if (p->ike_ts.crypto_alg || p->ike_ts.integ_alg || p->ike_ts.dh_group || crypto_key_size)
        fformat(vam->ofp,
    ...
    fformat(vam->ofp,
            "  lifetime %d jitter %d handover %d maxdata %d\n",
...
static int api_ikev2_sa_dump(vat_main_t *vam)
{
    ...
    /* Use a control ping for synchronization */
    if (!im->ping_id) im->ping_id = vl_msg_api_get_msg_index((u8 *) (VL_API_CONTROL_PING_CRC));
    mp_ping               = vl_msg_api_alloc_as_if_client(sizeof(*mp_ping));
...
static void vl_api_ikev2_sa_details_t_handler(vl_api_ikev2_sa_details_t *mp)
{
    vat_main_t          *vam = ikev2_test_main.vat_main;
    ...
    fformat(vam->ofp, " iip %U ispi %lx rip %U rspi %lx\n", format_ip_address, &iaddr, sa->ispi, format_ip_address, &raddr, sa->rspi);
    ...
    fformat(vam->ofp, "  SK_a  i:%U\n        r:%U\n", format_hex_bytes, k->sk_ai, k->sk_ai_len, format_hex_bytes, k->sk_ar, k->sk_ar_len);

    fformat(vam->ofp, "  SK_e  i:%U\n        r:%U\n", format_hex_bytes, k->sk_ei, k->sk_ei_len, format_hex_bytes, k->sk_er, k->sk_er_len);

    fformat(vam->ofp, "  SK_p  i:%U\n        r:%U\n", format_hex_bytes, k->sk_pi, k->sk_pi_len, format_hex_bytes, k->sk_pr, k->sk_pr_len);
...
static int api_ikev2_sa_v2_dump(vat_main_t *vam)
{
    ikev2_test_main_t         *im = &ikev2_test_main;
...
static void vl_api_ikev2_sa_v2_details_t_handler(vl_api_ikev2_sa_v2_details_t *mp)
{
    vat_main_t           *vam = ikev2_test_main.vat_main;
...
static int api_ikev2_sa_v3_dump(vat_main_t *vam)
{
    ikev2_test_main_t         *im = &ikev2_test_main;
...
static void vl_api_ikev2_sa_v3_details_t_handler(vl_api_ikev2_sa_v3_details_t *mp)
{
    vat_main_t           *vam = ikev2_test_main.vat_main;
...
static int api_ikev2_child_sa_dump(vat_main_t *vam)
{
    unformat_input_t             *i  = vam->input;
    ikev2_test_main_t            *im = &ikev2_test_main;
...
static void vl_api_ikev2_child_sa_details_t_handler(vl_api_ikev2_child_sa_details_t *mp)
{
    vat_main_t              *vam      = ikev2_test_main.vat_main;
    ...
    fformat(vam->ofp, "    %U ", format_ikev2_sa_transform, &child_sa->encryption);
...
static int api_ikev2_child_sa_v2_dump(vat_main_t *vam)
{
    unformat_input_t             *i  = vam->input;
    ikev2_test_main_t            *im = &ikev2_test_main;
    vl_api_ikev2_child_sa_dump_t *mp;
...
static void vl_api_ikev2_child_sa_v2_details_t_handler(vl_api_ikev2_child_sa_details_t *mp)
{
    vat_main_t              *vam      = ikev2_test_main.vat_main;
    vl_api_ikev2_child_sa_t *child_sa = &mp->child_sa;
    vl_api_ikev2_keys_t     *k        = &child_sa->keys;
...
static int api_ikev2_traffic_selector_dump(vat_main_t *vam)
{
    unformat_input_t                     *i  = vam->input;
    ikev2_test_main_t                    *im = &ikev2_test_main;
    ...
    if (child_sa_index == ~0 || sa_index == ~0 || is_initiator == ~0) return -99;
...
static void vl_api_ikev2_traffic_selector_details_t_handler(vl_api_ikev2_traffic_selector_details_t *mp)
{
    vat_main_t        *vam = ikev2_test_main.vat_main;
...
static int api_ikev2_nonce_get(vat_main_t *vam)
{
    unformat_input_t         *i  = vam->input;
    ikev2_test_main_t        *im = &ikev2_test_main;
    ...
    if (sa_index == ~0 || is_initiator == ~0) return -99;
...
static void vl_api_ikev2_nonce_get_reply_t_handler(vl_api_ikev2_nonce_get_reply_t *mp)
{
    vat_main_t *vam = ikev2_test_main.vat_main;
...
static int api_ikev2_plugin_get_version(vat_main_t *vam)
{
...
static void vl_api_ikev2_plugin_get_version_reply_t_handler(vl_api_ikev2_plugin_get_version_reply_t *mp)
{
    vat_main_t *vam = ikev2_test_main.vat_main;
    clib_warning("IKEv2 plugin version: %d.%d", ntohl(mp->major), ntohl(mp->minor));
    vam->result_ready = 1;
}

static int api_ikev2_profile_set_ipsec_udp_port(vat_main_t *vam)
{
    return 0;
}

static int api_ikev2_profile_set_liveness(vat_main_t *vam)
{
    unformat_input_t                    *i = vam->input;
...
static int api_ikev2_profile_add_del(vat_main_t *vam)
{
    unformat_input_t               *i = vam->input;
...
static int api_ikev2_profile_set_auth(vat_main_t *vam)
{
    unformat_input_t                *i = vam->input;
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "name %U", unformat_token, valid_chars, &name))
            vec_add1(name, 0);
        else if (unformat(i, "auth_method %U", unformat_ikev2_auth_method, &auth_method))
            ;
...
static int api_ikev2_profile_set_id(vat_main_t *vam)
{
    unformat_input_t              *i = vam->input;
...
static int api_ikev2_profile_set_ts(vat_main_t *vam)
{
    unformat_input_t              *i = vam->input;
...
static int api_ikev2_set_local_key(vat_main_t *vam)
{
    unformat_input_t             *i = vam->input;
...
static int api_ikev2_profile_set_udp_encap(vat_main_t *vam)
{
    unformat_input_t                     *i = vam->input;
...
static int api_ikev2_set_tunnel_interface(vat_main_t *vam)
{
    return (0);
}

static int api_ikev2_set_responder_hostname(vat_main_t *vam)
{
    unformat_input_t                      *i = vam->input;
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "%U hostname %v", unformat_token, valid_chars, &name, &hn)) {
            vec_add1(name, 0);
            vec_add1(hn, 0);
        }
        else {
...
static int api_ikev2_set_responder(vat_main_t *vam)
{
    unformat_input_t             *i = vam->input;
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "%U interface %d address %U", unformat_token, valid_chars, &name, &sw_if_index, unformat_ip_address, &address))
            vec_add1(name, 0);
        else {
            errmsg("parse error '%U'", format_unformat_error, i);
            return -99;
...
static int api_ikev2_set_ike_transforms(vat_main_t *vam)
{
    unformat_input_t                  *i = vam->input;
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "%U %d %d %d %d", unformat_token, valid_chars, &name, &crypto_alg, &crypto_key_size, &integ_alg, &dh_group))
            vec_add1(name, 0);
        else {
            errmsg("parse error '%U'", format_unformat_error, i);
            return -99;
...
static int api_ikev2_set_esp_transforms(vat_main_t *vam)
{
    unformat_input_t                  *i = vam->input;
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "%U %d %d %d", unformat_token, valid_chars, &name, &crypto_alg, &crypto_key_size, &integ_alg))
            vec_add1(name, 0);
        else {
            errmsg("parse error '%U'", format_unformat_error, i);
            return -99;
...
static int api_ikev2_set_sa_lifetime(vat_main_t *vam)
{
    unformat_input_t               *i = vam->input;
    ...
    while (unformat_check_input(i) != UNFORMAT_END_OF_INPUT) {
        if (unformat(i, "%U %lu %u %u %lu", unformat_token, valid_chars, &name, &lifetime, &lifetime_jitter, &handover, &lifetime_maxdata))
            vec_add1(name, 0);
        else {
            errmsg("parse error '%U'", format_unformat_error, i);
            return -99;
...
static int api_ikev2_initiate_sa_init(vat_main_t *vam)
{
    unformat_input_t                *i = vam->input;
...
static int api_ikev2_initiate_del_ike_sa(vat_main_t *vam)
{
    unformat_input_t                   *i = vam->input;
...
static int api_ikev2_initiate_del_child_sa(vat_main_t *vam)
{
    unformat_input_t                     *i = vam->input;
...
static int api_ikev2_initiate_rekey_child_sa(vat_main_t *vam)
{
    unformat_input_t                       *i = vam->input;
