...
void lcp_itf_pair_sync_state(lcp_itf_pair_t *lip)
{
    ...
    if (!lcp_sync() || lcp_get_netlink_processing_active()) return;

    sw = vnet_get_sw_interface_or_null(vnet_get_main(), lip->lip_phy_sw_if_index);
    if (!sw) return;
    sup_sw = vnet_get_sw_interface_or_null(vnet_get_main(), sw->sup_sw_if_index);
    if (!sup_sw) return;
    ...
    if (mtu == 0) mtu = sup_sw->mtu[VNET_MTU_L3];

    if (sup_sw->mtu[VNET_MTU_L3] < sw->mtu[VNET_MTU_L3]) {
        LCP_ITF_PAIR_WARN(
            "sync_state: %U flags %u mtu %u sup-mtu %u: "
    ...
    if (NULL == vnet_netlink_get_link_mtu(lip->lip_vif_index, &netlink_mtu)) {
        if (netlink_mtu != mtu) vnet_netlink_set_link_mtu(lip->lip_vif_index, mtu);
    }
...
static walk_rc_t lcp_itf_pair_walk_sync_state_all_cb(index_t lipi, void *ctx)
{
    lcp_itf_pair_t *lip;
...
static walk_rc_t lcp_itf_pair_walk_sync_state_hw_cb(vnet_main_t *vnm, u32 sw_if_index, void *arg)
{
    lcp_itf_pair_t *lip;
...
{
    if (!hi) return;
    LCP_ITF_PAIR_DBG("sync_state_hw: hi %U", format_vnet_sw_if_index_name, vnet_get_main(), hi->hw_if_index);

    vnet_hw_interface_walk_sw(vnet_get_main(), hi->hw_if_index, lcp_itf_pair_walk_sync_state_hw_cb, NULL);
}

static clib_error_t *lcp_itf_admin_state_change(vnet_main_t *vnm, u32 sw_if_index, u32 flags)
{
    lcp_itf_pair_t      *lip;
    vnet_hw_interface_t *hi;
    vnet_sw_interface_t *si;

    if (!lcp_sync() || lcp_get_netlink_processing_active()) return 0;

    LCP_ITF_PAIR_DBG("admin_state_change: sw %U %u", format_vnet_sw_if_index_name, vnm, sw_if_index, flags);
...
static clib_error_t *lcp_itf_mtu_change(vnet_main_t *vnm, u32 sw_if_index, u32 flags)
{
    vnet_sw_interface_t *si;
    vnet_hw_interface_t *hi;

    if (!lcp_sync() || lcp_get_netlink_processing_active()) return NULL;

    LCP_ITF_PAIR_DBG("mtu_change: sw %U %u", format_vnet_sw_if_index_name, vnm, sw_if_index, flags);

    if (vnet_sw_interface_is_sub(vnm, sw_if_index)) {
        lcp_itf_pair_t *lip;
        lip = lcp_itf_pair_get(lcp_itf_pair_find_by_phy(sw_if_index));
        if (lip) lcp_itf_pair_sync_state(lip);
        return NULL;
    }
...
static void lcp_itf_ip4_add_del_interface_addr(ip4_main_t    *im,
                                               uword          opaque,
                                               u32            sw_if_index,
                                               ...
                                               u32            is_del)
{
    ...
    LCP_ITF_PAIR_DBG("ip4_addr_%s: si:%U %U/%u",
                     is_del ? "del" : "add",
                     format_vnet_sw_if_index_name,
                     vnet_get_main(),
                     sw_if_index,
    ...
    LCP_ITF_PAIR_DBG("ip4_addr_%s: %U ip4 %U/%u", is_del ? "del" : "add", format_lcp_itf_pair, lip, format_ip4_address, address, address_length);
...
static void lcp_itf_ip6_add_del_interface_addr(ip6_main_t    *im,
                                               uword          opaque,
                                               u32            sw_if_index,
                                               ...
                                               u32            is_del)
{
    ...
    LCP_ITF_PAIR_DBG("ip6_addr_%s: si:%U %U/%u",
                     is_del ? "del" : "add",
                     format_vnet_sw_if_index_name,
                     vnet_get_main(),
                     sw_if_index,
    ...
    LCP_ITF_PAIR_DBG("ip6_addr_%s: %U ip4 %U/%u", is_del ? "del" : "add", format_lcp_itf_pair, lip, format_ip6_address, address, address_length);
    if (is_del)
...
static clib_error_t *lcp_itf_interface_add_del(vnet_main_t *vnm, u32 sw_if_index, u32 is_create)
{
    const vnet_sw_interface_t *sw;
    uword                      is_sub;

    if (!lcp_auto_subint()) return NULL;
    ...
    LCP_ITF_PAIR_DBG("interface_%s: sw %U parent %U",
                     is_create ? "add" : "del",
                     format_vnet_sw_if_index_name,
                     vnet_get_main(),
                     sw->sw_if_index,
    ...
    if (is_create) {
        ...
        if (!sup_lip) return NULL;

        name = format(name, "%s.%d%c", sup_lip->lip_host_name, sw->sub.id, 0);

        LCP_ITF_PAIR_INFO("interface_%s: %U has parent %U, auto-creating LCP with host-if %s",
                          is_create ? "add" : "del",
                          format_vnet_sw_if_index_name,
                          vnet_get_main(),
                          sw->sw_if_index,
        ...
        lcp_itf_pair_create(sw->sw_if_index, name, LCP_ITF_HOST_TAP, sup_lip->lip_namespace, NULL);
...
static clib_error_t *lcp_itf_sync_init(vlib_main_t *vm)
{
