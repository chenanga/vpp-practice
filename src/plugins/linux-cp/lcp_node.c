...
static u8 *format_lip_punt_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "lip-punt: %u -> %u", t->phy_sw_if_index, t->host_sw_if_index);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(lipi0 == INDEX_INVALID)) goto trace0;
        ...
        trace0:
            if (PREDICT_FALSE((b0->flags & VLIB_BUFFER_IS_TRACED))) {
                ...
                t->host_sw_if_index = (lipi0 == INDEX_INVALID) ? ~0 : lip0->lip_host_sw_if_index;
            }
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (lipi0 != INDEX_INVALID) {
                 ...
                 */
                lip0 = lcp_itf_pair_get(lipi0);
                if (lip0->lip_host_type == LCP_ITF_HOST_TUN) b0->flags |= VNET_BUFFER_F_LOCALLY_ORIGINATED;
            }

            if (PREDICT_FALSE((b0->flags & VLIB_BUFFER_IS_TRACED))) {
                lcp_punt_l3_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
...
static_always_inline u32 lcp_xc_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, ip_address_family_t af)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (!ethernet_address_cast(eth->dst_address)) ai = lcp_adj_lkup((u8 *) eth, lip->lip_rewrite_len, vnet_buffer(b0)->sw_if_index[VLIB_TX]);
            if (ai == ADJ_INDEX_INVALID) ai = lip->lip_phy_adjs.adj_index[af];
...
static_always_inline uword lcp_xc_mpls_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32           n_left_from, *from, *to_next, n_left_to_next;
    lcp_xc_next_t next_index;
...
VLIB_REGISTER_NODE(lcp_xc_mpls) = {.name         = "linux-cp-xc-mpls",
                                   .vector_size  = sizeof(u32),
                                   .format_trace = format_lcp_xc_trace,
                                   .type         = VLIB_NODE_TYPE_INTERNAL,
...
static_always_inline u32 lcp_xc_l3_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, ip_address_family_t af)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_TRUE(vnet_sw_interface_is_p2p(vnm, lip->lip_phy_sw_if_index))) {
                vnet_buffer(b0)->sw_if_index[VLIB_TX]  = lip->lip_phy_sw_if_index;
                vnet_buffer(b0)->ip.adj_index[VLIB_TX] = lip->lip_phy_adjs.adj_index[af];
            ...
            /* P2MP tunnels require a fib lookup to find the right adjacency */
                ...
                vnet_buffer(b0)->sw_if_index[VLIB_RX] = lip->lip_phy_sw_if_index;
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from >= 2 && n_left_to_next >= 2) {
            ...
            if (arp1->opcode == clib_host_to_net_u16(ETHERNET_ARP_OPCODE_reply)) {
                ...
                lipi1 = lcp_itf_pair_find_by_phy(vnet_buffer(b1)->sw_if_index[VLIB_RX]);
                ...
                if (lip1) {
                    ...
                    if (c1) {
                        /* Send to the host */
                        vnet_buffer(c1)->sw_if_index[VLIB_TX] = lip1->lip_host_sw_if_index;
    ...
    if (n_copies) vlib_buffer_enqueue_to_single_next(vm, node, reply_copies, LCP_ARP_NEXT_IO, n_copies);
