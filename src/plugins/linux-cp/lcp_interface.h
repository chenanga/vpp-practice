...
#define LCP_ITF_PAIR_DBG(...) vlib_log_debug(lcp_itf_pair_logger, __VA_ARGS__);

#define LCP_ITF_PAIR_INFO(...) vlib_log_info(lcp_itf_pair_logger, __VA_ARGS__);

#define LCP_ITF_PAIR_NOTICE(...) vlib_log_notice(lcp_itf_pair_logger, __VA_ARGS__);

#define LCP_ITF_PAIR_WARN(...) vlib_log_warn(lcp_itf_pair_logger, __VA_ARGS__);
 ...
 * @return error code
 */
extern int lcp_itf_pair_add(u32 host_sw_if_index, u32 phy_sw_if_index, u8 *host_name, u32 host_index, lip_host_type_t host_type, u8 *ns);
 ...
 * @return error code
 */
extern int lcp_itf_pair_create(u32 phy_sw_if_index, u8 *host_if_name, lip_host_type_t host_if_type, u8 *ns, u32 *host_sw_if_indexp);
...
always_inline index_t lcp_itf_pair_find_by_phy(u32 phy_sw_if_index)
{
    if (phy_sw_if_index >= vec_len(lip_db_by_phy)) return INDEX_INVALID;
    return (lip_db_by_phy[phy_sw_if_index]);
}

always_inline index_t lcp_itf_pair_find_by_host(u32 host_sw_if_index)
{
    if (host_sw_if_index >= vec_len(lip_db_by_host)) return INDEX_INVALID;
