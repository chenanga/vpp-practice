...
u8 *lcp_get_default_ns(void)
{
    ...
    if (!lcpm->default_namespace || lcpm->default_namespace[0] == 0) return NULL;
...
int lcp_set_default_ns(u8 *ns)
{
    ...
    if (len >= LCP_NS_LEN) return -1;

    if (!p || *p == 0) {
        lcpm->default_namespace = NULL;
        if (lcpm->default_ns_fd > 0) close(lcpm->default_ns_fd);
        lcpm->default_ns_fd = 0;
        return 0;
    }

    vec_validate_init_c_string(lcpm->default_namespace, p, clib_strnlen(p, LCP_NS_LEN));
...
u16 lcp_get_default_num_queues(u8 is_tx)
{
    ...
    if (is_tx) return lcpm->num_tx_queues;
