...
{
    if (type == LCP_API_ITF_HOST_TUN) return LCP_ITF_HOST_TUN;
...
{
    if (type == LCP_ITF_HOST_TUN) return LCP_API_ITF_HOST_TUN;
                                   ...
                                   u32            *vif_index_p)
{
    ...
    rv = lcp_itf_pair_create(phy_sw_if_index, host_if_name, lip_host_type, netns, host_sw_if_index_p);

    if (!rv && (vif_index_p != NULL)) {
        lcp_itf_pair_t *pair = lcp_itf_pair_get(lcp_itf_pair_find_by_phy(phy_sw_if_index));
...
static void vl_api_lcp_itf_pair_add_del_t_handler(vl_api_lcp_itf_pair_add_del_t *mp)
{
    ...
    if (mp->is_add) {
        rv = vl_api_lcp_itf_pair_add(phy_sw_if_index,
                                     lip_host_type,
                                     mp->host_if_name,
                                     sizeof(mp->host_if_name),
                                     mp->netns,
                                     sizeof(mp->netns),
                                     NULL,
                                     NULL);
...
static void vl_api_lcp_itf_pair_add_del_v2_t_handler(vl_api_lcp_itf_pair_add_del_v2_t *mp)
{
    ...
    BAD_SW_IF_INDEX_LABEL;
    REPLY_MACRO2_END(VL_API_LCP_ITF_PAIR_ADD_DEL_V2_REPLY, { rmp->host_sw_if_index = host_sw_if_index; });
}

static void vl_api_lcp_itf_pair_add_del_v3_t_handler(vl_api_lcp_itf_pair_add_del_v3_t *mp)
{
...
static void send_lcp_itf_pair_details(index_t lipi, vl_api_registration_t *rp, u32 context)
{
    ...
    REPLY_MACRO_DETAILS4_END(VL_API_LCP_ITF_PAIR_DETAILS, rp, context, ({
                                 ...
                                 memcpy_s(rmp->host_if_name, sizeof(rmp->host_if_name), lcp_pair->lip_host_name, vec_len(lcp_pair->lip_host_name));
                                 rmp->host_if_name[vec_len(lcp_pair->lip_host_name)] = 0;

                                 memcpy_s(rmp->netns, sizeof(rmp->netns), lcp_pair->lip_namespace, vec_len(lcp_pair->lip_namespace));
...
static void vl_api_lcp_itf_pair_get_t_handler(vl_api_lcp_itf_pair_get_t *mp)
{
    ...
    REPLY_AND_DETAILS_MACRO_END(VL_API_LCP_ITF_PAIR_GET_REPLY, lcp_itf_pair_pool, ({ send_lcp_itf_pair_details(cursor, rp, mp->context); }));
}

static void vl_api_lcp_itf_pair_get_v2_t_handler(vl_api_lcp_itf_pair_get_v2_t *mp)
{
    ...
    if (mp->sw_if_index == ~0) {
        ...
        send_lcp_itf_pair_details(pair_index, vl_api_client_index_to_registration(mp->client_index), mp->context);

        BAD_SW_IF_INDEX_LABEL;
        REPLY_MACRO2_END(VL_API_LCP_ITF_PAIR_GET_V2_REPLY, ({ rmp->cursor = ~0; }));
    }
}
...
static void vl_api_lcp_default_ns_get_t_handler(vl_api_lcp_default_ns_get_t *mp)
{
    ...
    REPLY_MACRO_DETAILS2(VL_API_LCP_DEFAULT_NS_GET_REPLY, ({
                             ...
                             if (ns) clib_strncpy((char *) rmp->netns, ns, LCP_NS_LEN - 1);
                         }));
}
...
static clib_error_t *lcp_api_init(vlib_main_t *vm)
{
