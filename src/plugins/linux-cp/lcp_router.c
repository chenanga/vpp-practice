...
static u32 lcp_router_intf_h2p(u32 host)
{
    lcp_itf_pair_t *lip;
    index_t         lipi;
    uword          *p;
    ...
    if (INDEX_INVALID == lipi) return (~0);
 ...
 * Else, return -1.
 */
static int lcp_router_lip_ts_check(nl_msg_info_t *msg_info, lcp_itf_pair_t *lip)
{
    if (!msg_info) return 0;

    if (msg_info->ts > lip->lip_create_ts) return 0;

    LCP_ROUTER_INFO("Early message received for %U", format_vnet_sw_if_index_name, vnet_get_main(), lip->lip_phy_sw_if_index);
    return -1;
}

static void lcp_router_link_del(struct rtnl_link *rl, void *ctx)
{
    index_t lipi;

    if (!lcp_auto_subint()) return;
    ...
    if (INDEX_INVALID != lipi) {
        ...
        if (lcp_router_lip_ts_check((nl_msg_info_t *) ctx, lip)) return;

        LCP_ROUTER_INFO("delete link: %s - %U", rtnl_link_get_type(rl), format_vnet_sw_if_index_name, vnet_get_main(), lip->lip_phy_sw_if_index);
        lcp_itf_pair_delete(lip->lip_phy_sw_if_index);

        if (rtnl_link_is_vlan(rl)) {
            LCP_ROUTER_INFO("delete vlan: %s -> %U", rtnl_link_get_name(rl), format_vnet_sw_if_index_name, vnet_get_main(), lip->lip_phy_sw_if_index);
            vnet_delete_sub_interface(lip->lip_phy_sw_if_index);
            vnet_delete_sub_interface(lip->lip_host_sw_if_index);
        }
    }
    else
        LCP_ROUTER_INFO("ignore link del: %s - %s", rtnl_link_get_type(rl), rtnl_link_get_name(rl));
}

static void lcp_router_ip4_mroutes_add_del(u32 sw_if_index, u8 is_add)
{
    ...
    mfib_index = mfib_table_get_index_for_sw_if_index(FIB_PROTOCOL_IP4, sw_if_index);

    for (ii = 0; ii < ARRAY_LEN(ip4_specials); ii++) {
        if (is_add) {
            mfib_table_entry_path_update(mfib_index, &ip4_specials[ii], MFIB_SOURCE_PLUGIN_LOW, MFIB_ENTRY_FLAG_NONE, &path);
        }
        else {
            mfib_table_entry_path_remove(mfib_index, &ip4_specials[ii], MFIB_SOURCE_PLUGIN_LOW, &path);
        }
    }
}

static void lcp_router_ip6_mroutes_add_del(u32 sw_if_index, u8 is_add)
{
    const fib_route_path_t path = {
    ...
    mfib_index = mfib_table_get_index_for_sw_if_index(FIB_PROTOCOL_IP6, sw_if_index);

    for (ii = 0; ii < ARRAY_LEN(ip6_specials); ii++) {
        if (is_add) {
            mfib_table_entry_path_update(mfib_index, &ip6_specials[ii], MFIB_SOURCE_PLUGIN_LOW, MFIB_ENTRY_FLAG_NONE, &path);
        }
        else {
            mfib_table_entry_path_remove(mfib_index, &ip6_specials[ii], MFIB_SOURCE_PLUGIN_LOW, &path);
        }
    }
}

static void lcp_router_link_mtu(struct rtnl_link *rl, u32 sw_if_index)
{
    ...
    if ((sw->sw_if_index == sw->sup_sw_if_index) && (hw->hw_class_index == ethernet_hw_interface_class.index))
        vnet_hw_interface_set_mtu(vnm, hw->hw_if_index, mtu);
    else
        vnet_sw_interface_set_mtu(vnm, sw->sw_if_index, mtu);
}

static walk_rc_t lcp_router_link_addr_adj_upd_cb(vnet_main_t *vnm, u32 sw_if_index, void *arg)
{
    lcp_itf_pair_t *lip;
    ...
    vnet_update_adjacency_for_sw_interface(vnm, lip->lip_phy_sw_if_index, lip->lip_phy_adjs.adj_index[AF_IP4]);
    vnet_update_adjacency_for_sw_interface(vnm, lip->lip_phy_sw_if_index, lip->lip_phy_adjs.adj_index[AF_IP6]);

    return WALK_CONTINUE;
}

static void lcp_router_link_addr(struct rtnl_link *rl, lcp_itf_pair_t *lip)
{
    vnet_main_t         *vnm = vnet_get_main();
    ...
    vnet_hw_interface_walk_sw(vnm, hw->hw_if_index, lcp_router_link_addr_adj_upd_cb, NULL);
}

static void lcp_router_table_flush(lcp_router_table_t *nlt, u32 *sw_if_index_to_bool, fib_source_t source);

static void lcp_router_link_add(struct rtnl_link *rl, void *ctx)
{
    index_t      lipi;
    int          up;
    vnet_main_t *vnm = vnet_get_main();
    ...
    if (INDEX_INVALID != lipi) {
        ...
        lip = lcp_itf_pair_get(lipi);
        if (!vnet_get_sw_interface(vnm, lip->lip_phy_sw_if_index)) return;

        if (lcp_router_lip_ts_check((nl_msg_info_t *) ctx, lip)) return;

        sw_if_flags = vnet_sw_interface_get_flags(vnm, lip->lip_phy_sw_if_index);
        sw_if_up    = (sw_if_flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP);

        if (!sw_if_up && up) {
            vnet_sw_interface_admin_up(vnet_get_main(), lip->lip_phy_sw_if_index);
        }
        else if (sw_if_up && !up) {
            vnet_sw_interface_admin_down(vnet_get_main(), lip->lip_phy_sw_if_index);
            ...
            fib_index = fib_table_get_index_for_sw_if_index(FIB_PROTOCOL_IP4, lip->lip_phy_sw_if_index);

            pool_foreach (nlt, lcp_router_table_pool) {
                if (fib_index == nlt->nlt_fib_index && FIB_PROTOCOL_IP4 == nlt->nlt_proto) {
                    u32 *sw_if_index_to_bool = NULL;

                    vec_validate_init_empty(sw_if_index_to_bool, lip->lip_phy_sw_if_index, false);
                    sw_if_index_to_bool[lip->lip_phy_sw_if_index] = true;

                    lcp_router_table_flush(nlt, sw_if_index_to_bool, lcp_rt_fib_src);
                    lcp_router_table_flush(nlt, sw_if_index_to_bool, lcp_rt_fib_src_dynamic);
        ...
        if (INDEX_INVALID != lipi) {
            ...
            if (vnet_create_sub_interface(lip->lip_phy_sw_if_index, vlan, 18, 0, vlan, &sub_phy_sw_if_index)) {
            ...
            if (vnet_create_sub_interface(lip->lip_host_sw_if_index, vlan, 18, 0, vlan, &sub_host_sw_if_index)) {
            ...
            LCP_ROUTER_INFO("create vlan: %s -> (%U, %U) : (%U, %U)",
                            rtnl_link_get_name(rl),
                            format_vnet_sw_if_index_name,
                            vnet_get_main(),
                            lip->lip_phy_sw_if_index,
            ...
            if ((if_name = rtnl_link_get_name(rl)) != NULL) vec_validate_init_c_string(if_namev, if_name, strnlen(if_name, IFNAMSIZ));
            lcp_itf_pair_add(sub_host_sw_if_index, sub_phy_sw_if_index, if_namev, rtnl_link_get_ifindex(rl), lip->lip_host_type, ns);
            if (up) vnet_sw_interface_admin_up(vnet_get_main(), sub_phy_sw_if_index);
            vnet_sw_interface_admin_up(vnet_get_main(), sub_host_sw_if_index);

            vec_free(if_namev);
        }
        else {
            LCP_ROUTER_INFO("ignore parent-link add: %s - %s", rtnl_link_get_type(rl), rtnl_link_get_name(rl));
        }
    }
    else
        LCP_ROUTER_INFO("ignore link add: %s - %s", rtnl_link_get_type(rl), rtnl_link_get_name(rl));
}

static void lcp_router_link_sync_begin(void)
{
    LCP_ROUTER_INFO("Begin synchronization of interface configurations");
}

static void lcp_router_link_sync_end(void)
{
    LCP_ROUTER_INFO("End synchronization of interface configurations");
}

static clib_error_t *lcp_router_link_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    ...
    if (lipi == INDEX_INVALID) return 0;
     ...
     * removed.
     */
    if (!(flags & VNET_HW_INTERFACE_FLAG_LINK_UP) && (lcp_get_del_static_on_link_down() || lcp_get_del_dynamic_on_link_down())) {
        u32                 fib_index;
        u32               **fib_index_to_sw_if_index_to_bool = NULL;
        u32                 id, sw_if_index;
        lcp_router_table_t *nlt;

        fib_index = fib_table_get_index_for_sw_if_index(FIB_PROTOCOL_IP4, hi->sw_if_index);

        vec_validate_init_empty(fib_index_to_sw_if_index_to_bool, fib_index, NULL);
        vec_validate_init_empty(fib_index_to_sw_if_index_to_bool[fib_index], hi->sw_if_index, false);
	...
	fib_index_to_sw_if_index_to_bool[fib_index][sw_if_index] = true;
        ...
        vec_foreach_index (fib_index, fib_index_to_sw_if_index_to_bool) {
            ...
            if (NULL == sw_if_index_to_bool) continue;

            pool_foreach (nlt, lcp_router_table_pool) {
                if (fib_index == nlt->nlt_fib_index && FIB_PROTOCOL_IP4 == nlt->nlt_proto) {
                    if (lcp_get_del_static_on_link_down()) lcp_router_table_flush(nlt, sw_if_index_to_bool, lcp_rt_fib_src);
                    if (lcp_get_del_dynamic_on_link_down()) lcp_router_table_flush(nlt, sw_if_index_to_bool, lcp_rt_fib_src_dynamic);
                    break;
                }
            }
...
static fib_protocol_t lcp_router_proto_k2f(uint32_t k)
{
    switch (k) {
        case AF_INET6:
...
static void lcp_router_mk_addr(const struct nl_addr *rna, ip_address_t *ia)
{
    ...
    ip_address_set(ia, nl_addr_get_binary_addr(rna), FIB_PROTOCOL_IP4 == fproto ? AF_IP4 : AF_IP6);
}

static fib_protocol_t lcp_router_mk_addr46(const struct nl_addr *rna, ip46_address_t *ia)
{
    fib_protocol_t fproto;
...
static void lcp_router_link_addr_add_del(struct rtnl_addr *rla, int is_del)
{
    u32 sw_if_index;
    ...
    if (~0 != sw_if_index) {
        ...
        if (AF_IP4 == ip_addr_version(&nh)) {
            ip4_add_del_interface_address(vlib_get_main(), sw_if_index, &ip_addr_v4(&nh), rtnl_addr_get_prefixlen(rla), is_del);
            ...
            if (ip6_address_is_link_local_unicast(&ip_addr_v6(&nh)))
                ...
                ip6_add_del_interface_address(vlib_get_main(), sw_if_index, &ip_addr_v6(&nh), rtnl_addr_get_prefixlen(rla), is_del);
            lcp_router_ip6_mroutes_add_del(sw_if_index, !is_del);
        }

        LCP_ROUTER_DBG("link-addr: %U %U/%d",
                       format_vnet_sw_if_index_name,
                       vnet_get_main(),
                       sw_if_index,
                       format_ip_address,
...
static void lcp_router_link_addr_del(struct rtnl_addr *la)
{
    lcp_router_link_addr_add_del(la, 1);
}

static void lcp_router_link_addr_add(struct rtnl_addr *la)
{
    lcp_router_link_addr_add_del(la, 0);
}

static walk_rc_t lcp_router_address_mark(index_t index, void *ctx)
{
    vnet_main_t *vnm = vnet_get_main();

    lcp_itf_pair_t *lip = lcp_itf_pair_get(index);
    if (!lip) return WALK_CONTINUE;

    ip_interface_address_mark_one_interface(vnm, vnet_get_sw_interface(vnm, lip->lip_phy_sw_if_index), 0);

    return WALK_CONTINUE;
}

static void lcp_router_link_addr_sync_begin(void)
{
...
static void lcp_router_link_addr_sync_end(void)
{
...
static void lcp_router_mk_mac_addr(const struct nl_addr *rna, mac_address_t *mac)
{
    mac_address_from_bytes(mac, nl_addr_get_binary_addr(rna));
}

static void lcp_router_neigh_del(struct rtnl_neigh *rn)
{
    u32 sw_if_index;
    ...
    if (~0 != sw_if_index) {
        ...
        if ((rna = rtnl_neigh_get_dst(rn)) == NULL) return;
        lcp_router_mk_addr(rna, &nh);

        if (ip46_address_is_multicast(&ip_addr_46(&nh))) {
            LCP_ROUTER_DBG("ignore neighbor del: %U %U", format_ip_address, &nh, format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index);
            return;
        }
        ...
        if (rv) {
            LCP_ROUTER_ERROR("Failed to delete neighbor: %U %U", format_ip_address, &nh, format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index);
        }
        else {
            LCP_ROUTER_DBG("neighbor del: %U %U", format_ip_address, &nh, format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index);
        }
    }
    else
        LCP_ROUTER_INFO("ignore neighbour del on: %d", rtnl_neigh_get_ifindex(rn));
}
...
static void lcp_router_neigh_add(struct rtnl_neigh *rn)
{
    u32 sw_if_index;
    ...
    if (~0 != sw_if_index) {
        ...
        if (ip46_address_is_multicast(&ip_addr_46(&nh))) {
            LCP_ROUTER_DBG("ignore neighbor add: %U %U", format_ip_address, &nh, format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index);
            return;
        }
        ...
        if (ll && (state & NUD_VALID)) {
            ...
            if (rv) {
                ...
                LCP_ROUTER_DBG("neighbor add: %U %U", format_ip_address, &nh, format_vnet_sw_if_index_name, vnet_get_main(), sw_if_index);
            }
        }
        else
            /* It's a delete */
        ...
        LCP_ROUTER_INFO("ignore neighbour add on: %d", rtnl_neigh_get_ifindex(rn));
}

static walk_rc_t lcp_router_neighbor_mark(index_t index, void *ctx)
{
    lcp_itf_pair_t *lip = lcp_itf_pair_get(index);
    if (!lip) return WALK_CONTINUE;
...
static void lcp_router_neigh_sync_begin(void)
{
...
static void lcp_router_neigh_sync_end(void)
{
...
static lcp_router_table_t *lcp_router_table_find(uint32_t id, fib_protocol_t fproto)
{
    uword *p;

    p = hash_get(lcp_router_table_db[fproto], id);

    if (p) return pool_elt_at_index(lcp_router_table_pool, p[0]);

    return (NULL);
}

static uint32_t lcp_router_table_k2f(uint32_t k)
{
    // the kernel's table ID 255 is the default table
    if (k == 255 || k == 254) return 0;
    return k;
}

static lcp_router_table_t *lcp_router_table_add_or_lock(uint32_t id, fib_protocol_t fproto)
{
    lcp_router_table_t *nlt;
    ...
    if (NULL == nlt) {
        ...
        nlt->nlt_fib_index  = fib_table_find_or_create_and_lock(nlt->nlt_proto, nlt->nlt_id, lcp_rt_fib_src);
        nlt->nlt_mfib_index = mfib_table_find_or_create_and_lock(nlt->nlt_proto, nlt->nlt_id, MFIB_SOURCE_PLUGIN_LOW);

        hash_set(lcp_router_table_db[fproto], nlt->nlt_id, nlt - lcp_router_table_pool);

        if (FIB_PROTOCOL_IP4 == fproto) {
            /* Set the all 1s address in this table to punt */
            fib_table_entry_special_add(nlt->nlt_fib_index, &pfx_all1s, lcp_rt_fib_src, FIB_ENTRY_FLAG_LOCAL);
            ...
            for (ii = 0; ii < ARRAY_LEN(ip4_specials); ii++) {
                mfib_table_entry_path_update(nlt->nlt_mfib_index, &ip4_specials[ii], MFIB_SOURCE_PLUGIN_LOW, MFIB_ENTRY_FLAG_NONE, &path);
            }
        }
            ...
            for (ii = 0; ii < ARRAY_LEN(ip6_specials); ii++) {
                mfib_table_entry_path_update(nlt->nlt_mfib_index, &ip6_specials[ii], MFIB_SOURCE_PLUGIN_LOW, MFIB_ENTRY_FLAG_NONE, &path);
            }
        }
    }
...
static void lcp_router_table_unlock(lcp_router_table_t *nlt)
{
    nlt->nlt_refs--;
    ...
    if (0 == nlt->nlt_refs) {
        if (FIB_PROTOCOL_IP4 == nlt->nlt_proto) {
            /* Set the all 1s address in this table to punt */
            fib_table_entry_special_remove(nlt->nlt_fib_index, &pfx_all1s, lcp_rt_fib_src);
        }
...
static void lcp_router_route_mk_prefix(struct rtnl_route *r, fib_prefix_t *p)
{
...
static void lcp_router_route_mk_mprefix(struct rtnl_route *r, mfib_prefix_t *p)
{
    ...
    if (addr) p->fp_proto = lcp_router_mk_addr46(addr, &p->fp_src_addr);
}

static int lcp_router_mpls_nladdr_to_path(fib_route_path_t *path, struct nl_addr *addr)
{
    if (!addr) return 0;
...
static void lcp_router_route_path_parse(struct rtnl_nexthop *rnh, void *arg)
{
    ...
    if (~0 != sw_if_index) {
        ...
        if (!addr) addr = rtnl_route_nh_get_via(rnh);
        ...
        if (ctx->route_proto == FIB_PROTOCOL_MPLS) {
            ...
            if (label_count) {
                LCP_ROUTER_DBG(" is label swap to %u", path->frp_label_stack[0].fml_value);
            }
            else {
                fib_mpls_label_t fml = {
        ...
        label_count = lcp_router_mpls_nladdr_to_path(path, addr);
        if (label_count) LCP_ROUTER_DBG(" has encap mpls, %d labels", label_count);
#endif

        if (ctx->is_mcast) path->frp_mitf_flags = MFIB_ITF_FLAG_FORWARD;
 ...
 */
static void lcp_router_route_path_add_special(struct rtnl_route *rr, lcp_router_route_path_parse_t *ctx)
{
    fib_route_path_t *path;

    if (rtnl_route_get_type(rr) < RTN_BLACKHOLE) return;

    /* if it already has a path, it does not need us to add one */
    if (vec_len(ctx->paths) > 0) return;
...
static const fib_route_path_flags_t lcp_router_route_type_frpflags[__RTN_MAX] = {
    [RTN_UNREACHABLE] = FIB_ROUTE_PATH_ICMP_UNREACH,
    [RTN_PROHIBIT]    = FIB_ROUTE_PATH_ICMP_PROHIBIT,
    [RTN_BLACKHOLE]   = FIB_ROUTE_PATH_DROP,
};

static inline fib_source_t lcp_router_proto_fib_source(u8 rt_proto)
{
    return (rt_proto <= RTPROT_STATIC) ? lcp_rt_fib_src : lcp_rt_fib_src_dynamic;
}

static fib_entry_flag_t lcp_router_route_mk_entry_flags(uint8_t rtype, int table_id, uint8_t rproto)
{
    fib_entry_flag_t fef = FIB_ENTRY_FLAG_NONE;

    fef |= lcp_router_route_type_feflags[rtype];
    if ((rproto == RTPROT_KERNEL) || PREDICT_FALSE(255 == table_id)) /* kernel proto is interface prefixes, 255 is linux's 'local' table */
        fef |= FIB_ENTRY_FLAG_ATTACHED | FIB_ENTRY_FLAG_CONNECTED;

    return (fef);
}

static void lcp_router_route_del(struct rtnl_route *rr)
{
    ...
    if (!lcp_router_route_type_valid[rtype] || (table_id == 255)) return;
    ...
    LCP_ROUTER_DBG("route del: %d:%U %U", rtnl_route_get_table(rr), format_fib_prefix, &pfx, format_fib_entry_flags, entry_flags);
    ...
    if (0 != vec_len(np.paths)) {
        ...
        switch (pfx.fp_proto) {
            case FIB_PROTOCOL_IP6:
                ...
                break;
            case FIB_PROTOCOL_MPLS:
                fib_table_entry_path_remove2(nlt->nlt_fib_index, &pfx, fib_src, np.paths);
...
static fib_route_path_t *lcp_router_fib_route_path_dup(fib_route_path_t *old)
{
    int               idx;
    fib_route_path_t *p;
    ...
    for (idx = 0; idx < vec_len(new); idx++) {
        p = &new[idx];
        if (p->frp_label_stack) p->frp_label_stack = vec_dup(p->frp_label_stack);
    }

    return new;
}

static void lcp_router_route_add(struct rtnl_route *rr, int is_replace)
{
    fib_entry_flag_t    entry_flags;
    uint32_t            table_id;
    fib_prefix_t        pfx;
    ...
    if (rproto == RTPROT_KERNEL
        || (FIB_PROTOCOL_IP6 == pfx.fp_proto
            && (ip6_address_is_multicast(&pfx.fp_addr.ip6) || ip6_address_is_link_local_unicast(&pfx.fp_addr.ip6)))) {
        LCP_ROUTER_DBG("route skip: %d:%U %U", rtnl_route_get_table(rr), format_fib_prefix, &pfx, format_fib_entry_flags, entry_flags);
        return;
    }
    LCP_ROUTER_DBG("route %s: %d:%U %U",
                   is_replace ? "replace" : "add",
                   rtnl_route_get_table(rr),
                   format_fib_prefix,
                   &pfx,
    ...
    if (0 != vec_len(np.paths)) {
        if (rtype == RTN_MULTICAST) {
            ...
            mfib_table_entry_update(nlt->nlt_mfib_index, &mpfx, MFIB_SOURCE_PLUGIN_LOW, MFIB_RPF_ID_NONE, MFIB_ENTRY_FLAG_ACCEPT_ALL_ITF);

            mfib_table_entry_paths_update(nlt->nlt_mfib_index, &mpfx, MFIB_SOURCE_PLUGIN_LOW, MFIB_ENTRY_FLAG_NONE, np.paths);
        }
        else {
            fib_source_t            fib_src;
            ...
            if (pfx.fp_proto == FIB_PROTOCOL_MPLS) {
                ...
                fib_route_path_t *pathdup = lcp_router_fib_route_path_dup(np.paths);
                if (is_replace)
                    fib_table_entry_update(nlt->nlt_fib_index, &pfx, fib_src, entry_flags, pathdup);
                else
                    fib_table_entry_path_add2(nlt->nlt_fib_index, &pfx, fib_src, entry_flags, pathdup);
                vec_free(pathdup);
            ...
            if (is_replace)
                fib_table_entry_update(nlt->nlt_fib_index, &pfx, fib_src, entry_flags, np.paths);
            else
                fib_table_entry_path_add2(nlt->nlt_fib_index, &pfx, fib_src, entry_flags, np.paths);
        }
    }
    else {
        LCP_ROUTER_DBG("no paths for route: %d:%U %U", rtnl_route_get_table(rr), format_fib_prefix, &pfx, format_fib_entry_flags, entry_flags);
    }
    vec_free(np.paths);
}

static void lcp_router_route_sync_begin(void)
{
    lcp_router_table_t *nlt;

    pool_foreach (nlt, lcp_router_table_pool) {
        fib_table_mark(nlt->nlt_fib_index, nlt->nlt_proto, lcp_rt_fib_src);
        fib_table_mark(nlt->nlt_fib_index, nlt->nlt_proto, lcp_rt_fib_src_dynamic);

        LCP_ROUTER_INFO("Begin synchronization of %U routes in table %u", format_fib_protocol, nlt->nlt_proto, nlt->nlt_fib_index);
    }
}

static void lcp_router_route_sync_end(void)
{
    lcp_router_table_t *nlt;

    pool_foreach (nlt, lcp_router_table_pool) {
        fib_table_sweep(nlt->nlt_fib_index, nlt->nlt_proto, lcp_rt_fib_src);
        fib_table_sweep(nlt->nlt_fib_index, nlt->nlt_proto, lcp_rt_fib_src_dynamic);

        LCP_ROUTER_INFO("End synchronization of %U routes in table %u", format_fib_protocol, nlt->nlt_proto, nlt->nlt_fib_index);
    }
}
...
static fib_table_walk_rc_t lcp_router_table_flush_cb(fib_node_index_t fib_entry_index, void *arg)
{
    lcp_router_table_flush_ctx_t *ctx = arg;
    u32                           sw_if_index;

    sw_if_index = fib_entry_get_resolving_interface_for_source(fib_entry_index, ctx->lrtf_source);

    if (sw_if_index < vec_len(ctx->lrtf_sw_if_index_to_bool) && ctx->lrtf_sw_if_index_to_bool[sw_if_index]) {
...
static void lcp_router_table_flush(lcp_router_table_t *nlt, u32 *sw_if_index_to_bool, fib_source_t source)
{
    ...
    LCP_ROUTER_DBG("Flush table: proto %U, fib-index %u, max sw_if_index %u, source %U",
                   format_fib_protocol,
                   nlt->nlt_proto,
                   nlt->nlt_fib_index,
                   vec_len(sw_if_index_to_bool) - 1,
                   format_fib_source,
                   source);

    fib_table_walk(nlt->nlt_fib_index, nlt->nlt_proto, lcp_router_table_flush_cb, &ctx);

    LCP_ROUTER_DBG("Flush table: entries number to delete %u", vec_len(ctx.lrtf_entries));
...
const nl_vft_t lcp_router_vft = {
    ...
    .nvl_rt_link_sync_begin  = {.is_mp_safe = 0, .cb = lcp_router_link_sync_begin},
    ...
    .nvl_rt_addr_sync_begin  = {.is_mp_safe = 0, .cb = lcp_router_link_addr_sync_begin},
    .nvl_rt_addr_sync_end    = {.is_mp_safe = 0, .cb = lcp_router_link_addr_sync_end},
    .nvl_rt_neigh_add        = {.is_mp_safe = 0, .cb = lcp_router_neigh_add},
    .nvl_rt_neigh_del        = {.is_mp_safe = 0, .cb = lcp_router_neigh_del},
    .nvl_rt_neigh_sync_begin = {.is_mp_safe = 0, .cb = lcp_router_neigh_sync_begin},
    .nvl_rt_neigh_sync_end   = {.is_mp_safe = 0, .cb = lcp_router_neigh_sync_end},
    .nvl_rt_route_add        = {.is_mp_safe = 1, .cb = lcp_router_route_add},
    .nvl_rt_route_del        = {.is_mp_safe = 1, .cb = lcp_router_route_del},
    .nvl_rt_route_sync_begin = {.is_mp_safe = 0, .cb = lcp_router_route_sync_begin},
    .nvl_rt_route_sync_end   = {.is_mp_safe = 0, .cb = lcp_router_route_sync_end},
};

static clib_error_t *lcp_router_init(vlib_main_t *vm)
{
     ...
     */
    lcp_rt_fib_src = fib_source_allocate("lcp-rt", FIB_SOURCE_PRIORITY_HI, FIB_SOURCE_BH_API);

    lcp_rt_fib_src_dynamic = fib_source_allocate("lcp-rt-dynamic", FIB_SOURCE_PRIORITY_HI + 1, FIB_SOURCE_BH_API);
