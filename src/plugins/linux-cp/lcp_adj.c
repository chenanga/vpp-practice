...
static_always_inline void lcp_adj_mk_key_adj(const ip_adjacency_t *adj, lcp_adj_key_t *key)
{
    lcp_adj_mk_key(adj->rewrite_header.data, adj->rewrite_header.data_bytes, adj->rewrite_header.sw_if_index, key);
}
...
static void lcp_adj_delegate_adj_deleted(adj_delegate_t *aed)
{
    ...
    if (aed->ad_index != INDEX_INVALID) pool_put_index(adj_keys, aed->ad_index);
}
...
static void lcp_adj_delegate_adj_modified(adj_delegate_t *aed)
{
    ...
    if (aed->ad_index != INDEX_INVALID) adj_key = pool_elt_at_index(adj_keys, aed->ad_index);

    /* return if there was no stored adj and we will not add one */
    if (!adj_key && !save_adj) return;

    /* build kv if a new entry should be stored */
    if (save_adj) {
        lcp_adj_mk_key_adj(adj, &kv.k);
        kv.v = aed->ad_adj_index;
        if (adj_key) key_changed = (clib_memcmp(adj_key, &kv.k, sizeof(*adj_key)) != 0);
    }
...
static void lcp_adj_delegate_adj_created(adj_index_t ai)
{
    ...
    if (lipi == INDEX_INVALID) return;

    lip = lcp_itf_pair_get(lipi);
    if (lip->lip_host_type == LCP_ITF_HOST_TUN) return;
