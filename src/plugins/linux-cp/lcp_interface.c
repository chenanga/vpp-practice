...
u8 *format_lcp_itf_pair(u8 *s, va_list *args)
{
    ...
    s = format(s, " %v %d type %s", lip->lip_host_name, lip->lip_vif_index, (lip->lip_host_type == LCP_ITF_HOST_TAP) ? "tap" : "tun");

    if (lip->lip_namespace) s = format(s, " netns %s", lip->lip_namespace);
...
void lcp_itf_pair_show(u32 phy_sw_if_index)
{
    ...
    if (phy_sw_if_index == ~0) {
        ...
        if (api != INDEX_INVALID) lcp_itf_pair_walk_show_cb(api, 0);
    }
}
...
static void lcp_itf_set_adjs(lcp_itf_pair_t *lip)
{
    if (lip->lip_host_type == LCP_ITF_HOST_TUN) {
        lip->lip_phy_adjs.adj_index[AF_IP4] = adj_nbr_add_or_lock(FIB_PROTOCOL_IP4, VNET_LINK_IP4, &zero_addr, lip->lip_phy_sw_if_index);
        lip->lip_phy_adjs.adj_index[AF_IP6] = adj_nbr_add_or_lock(FIB_PROTOCOL_IP6, VNET_LINK_IP6, &zero_addr, lip->lip_phy_sw_if_index);
    }
    else {
        lip->lip_phy_adjs.adj_index[AF_IP4] = adj_mcast_add_or_lock(FIB_PROTOCOL_IP4, VNET_LINK_IP4, lip->lip_phy_sw_if_index);
        lip->lip_phy_adjs.adj_index[AF_IP6] = adj_mcast_add_or_lock(FIB_PROTOCOL_IP6, VNET_LINK_IP6, lip->lip_phy_sw_if_index);
    }
...
int lcp_itf_pair_add(u32 host_sw_if_index, u32 phy_sw_if_index, u8 *host_name, u32 host_index, lip_host_type_t host_type, u8 *ns)
{
    ...
    if (lipi != INDEX_INVALID) return VNET_API_ERROR_VALUE_EXIST;
    ...
    ip_feature_enable_disable(af, N_SAFI, IP_FEATURE_INPUT, lcp_itf_l3_feat_names[lip->lip_host_type][af], lip->lip_host_sw_if_index, 1, NULL, 0);
    ...
    if (lip->lip_host_type != LCP_ITF_HOST_TUN) {
        vnet_feature_enable_disable("arp", "linux-cp-arp-phy", lip->lip_phy_sw_if_index, 1, NULL, 0);
        vnet_feature_enable_disable("arp", "linux-cp-arp-host", lip->lip_host_sw_if_index, 1, NULL, 0);
    }
    else {
        if (hash_elts(lip_db_by_vif) == 1) {
            vnet_feature_enable_disable("ip4-punt", "linux-cp-punt-l3", 0, 1, NULL, 0);
            vnet_feature_enable_disable("ip6-punt", "linux-cp-punt-l3", 0, 1, NULL, 0);
        }
    }
    ...
    vec_foreach (vft, lcp_itf_vfts) {
        if (vft->pair_add_fn) vft->pair_add_fn(lip);
    }
...
static clib_error_t *lcp_netlink_del_link(const char *name)
{
    ...
    sk = nl_socket_alloc();
    if ((err = nl_connect(sk, NETLINK_ROUTE)) < 0) return clib_error_return(NULL, "Unable to connect socket: %d", err);

    link = rtnl_link_alloc();
    rtnl_link_set_name(link, name);

    if ((err = rtnl_link_delete(sk, link)) < 0) return clib_error_return(NULL, "Unable to del link %s: %d", name, err);
...
int lcp_itf_pair_del(u32 phy_sw_if_index)
{
    ...
    if (lipi == INDEX_INVALID) return VNET_API_ERROR_INVALID_SW_IF_INDEX;

    lip = lcp_itf_pair_get(lipi);

    LCP_ITF_PAIR_NOTICE("pair_del: host:%U phy:%U host_if:%v vif:%d ns:%v",
                        format_vnet_sw_if_index_name,
                        vnet_get_main(),
    ...
    vec_foreach (vft, lcp_itf_vfts) {
        if (vft->pair_del_fn) vft->pair_del_fn(lip);
    }

    FOR_EACH_IP_ADDRESS_FAMILY(af)
    ip_feature_enable_disable(af, N_SAFI, IP_FEATURE_INPUT, lcp_itf_l3_feat_names[lip->lip_host_type][af], lip->lip_host_sw_if_index, 0, NULL, 0);
    ...
    if (lip->lip_host_type != LCP_ITF_HOST_TUN) {
        vnet_feature_enable_disable("arp", "linux-cp-arp-phy", lip->lip_phy_sw_if_index, 0, NULL, 0);
        vnet_feature_enable_disable("arp", "linux-cp-arp-host", lip->lip_host_sw_if_index, 0, NULL, 0);
    }
    else {
        if (hash_elts(lip_db_by_vif) == 1) {
            vnet_feature_enable_disable("ip4-punt", "linux-cp-punt-l3", 0, 0, NULL, 0);
            vnet_feature_enable_disable("ip6-punt", "linux-cp-punt-l3", 0, 0, NULL, 0);
        }
    }
...
static void lcp_itf_pair_delete_by_index(index_t lipi)
{
    ...
    if (vnet_sw_interface_is_sub(vnet_get_main(), host_sw_if_index)) {
        ...
        if (ns) {
            ...
            if (vif_ns_fd != -1) clib_setns(vif_ns_fd);
        }

        lcp_netlink_del_link((const char *) host_name);
        if (vif_ns_fd != -1) close(vif_ns_fd);
 ...
 */
static clib_error_t *lcp_itf_interface_add_del(vnet_main_t *vnm, u32 sw_if_index, u32 is_add)
{
    if (!is_add) /* remove any interface pair we have for this interface */
...
void lcp_itf_set_interface_addr(const lcp_itf_pair_t *lip)
{
    ...
    foreach_ip_interface_address(lm4, ia, lip->lip_phy_sw_if_index, 1 /* honor unnumbered */, ({
    ...
    foreach_ip_interface_address(lm6, ia, lip->lip_phy_sw_if_index, 1 /* honor unnumbered */, ({
...
static walk_rc_t lcp_itf_pair_find_walk(vnet_main_t *vnm, u32 sw_if_index, void *arg)
{
    ...
    if (sw && (sw->sub.eth.inner_vlan_id == 0) && (sw->sub.eth.outer_vlan_id == match->vlan) && (sw->sub.eth.flags.dot1ad == match->dot1ad)) {
        LCP_ITF_PAIR_DBG(
            "find_walk: found match outer %d dot1ad %d "
...
static index_t lcp_itf_pair_find_by_outer_vlan(u32 sup_if_index, u16 vlan, bool dot1ad)
{
    ...
    vnet_hw_interface_walk_sw(vnet_get_main(), hw->hw_if_index, lcp_itf_pair_find_walk, &match);

    if (match.matched_sw_if_index >= vec_len(lip_db_by_phy)) return INDEX_INVALID;
...
int lcp_itf_pair_create(u32 phy_sw_if_index, u8 *host_if_name, lip_host_type_t host_if_type, u8 *ns, u32 *host_sw_if_indexp)
{
    ...
    if (hw->hw_class_index != ethernet_hw_interface_class.index && host_if_type == LCP_ITF_HOST_TAP) {
        LCP_ITF_PAIR_ERR("pair_create: don't create TAP for non-eth interface; use tun");
        return VNET_API_ERROR_INVALID_ARGUMENT;
    }
     ...
     */
    if (ns == 0 || ns[0] == 0) ns = lcp_get_default_ns();
    ...
    if (vnet_sw_interface_is_sub(vnm, phy_sw_if_index)) {
        ...
        if (INDEX_INVALID == parent_if_index) {
            LCP_ITF_PAIR_ERR("pair_create: can't find LCP for %U", format_vnet_sw_if_index_name, vnet_get_main(), sw->sup_sw_if_index);
            return VNET_API_ERROR_INVALID_SW_IF_INDEX;
        }
        ...
        if (ns && ns[0] != 0) {
            ...
            if (orig_ns_fd == -1 || ns_fd == -1) goto socket_close;
        ...
        if (!vif_index) {
            ...
            if (inner_vlan) {
                ...
                if (INDEX_INVALID == linux_parent_if_index || !(llip = lcp_itf_pair_get(linux_parent_if_index))) {
                    LCP_ITF_PAIR_ERR(
            ...
            err = lcp_netlink_add_link_vlan(parent_vif_index, vlan, proto, (const char *) host_if_name);
            if (err != 0) {
                LCP_ITF_PAIR_ERR(
                    "pair_create: cannot create link "
            ...
            if (!err) vif_index = if_nametoindex((char *) host_if_name);
        }
         ...
         */
        if (!err
            && vnet_create_sub_interface(lip->lip_host_sw_if_index, sw->sub.id, sw->sub.eth.raw_flags, inner_vlan, outer_vlan, &host_sw_if_index)) {
            LCP_ITF_PAIR_ERR("pair_create: failed to create tap subint: %d.%d on %U",
                             outer_vlan,
                             inner_vlan,
                             format_vnet_sw_if_index_name,
                             vnm,
            ...
            err = clib_error_return(0,
                                    "failed to create tap subint: %d.%d. on %U",
                                    outer_vlan,
                                    inner_vlan,
                                    format_vnet_sw_if_index_name,
                                    vnm,
    ...
    socket_close:
        ...
        if (err) return VNET_API_ERROR_INVALID_ARGUMENT;
    }
    else {
        tap_create_if_args_t args = {
            .num_rx_queues  = clib_max(1, lcp_get_default_num_queues(0 /* is_tx */)),
            .num_tx_queues  = clib_max(1, lcp_get_default_num_queues(1 /* is_tx */)),
            .id             = ~0,
            .auto_id_offset = 4096,
            .sw_if_index    = ~0,
            .rx_ring_sz     = 256,
        ...
        if (ns && ns[0] != 0) args.host_namespace = ns;
    ...
    LCP_ITF_PAIR_INFO("pair create: {%U, %U, %s}",
                      ...
                      host_sw_if_index,
                      host_if_name);
    lcp_itf_pair_add(host_sw_if_index, phy_sw_if_index, host_if_name, vif_index, host_if_type, ns);
    ...
    LCP_ITF_PAIR_INFO("pair create: %U sw-flags %u hw-flags %u", format_lcp_itf_pair, lip, sw->flags, hw->flags);
     ...
     * TAP interface.
     */
    if (host_if_type == LCP_ITF_HOST_TAP && !vnet_sw_interface_is_sub(vnm, phy_sw_if_index)) {
        hw = vnet_get_sup_hw_interface(vnm, phy_sw_if_index);
        lcp_itf_pair_link_up_down(vnm, hw->hw_if_index, hw->flags);
    }

    if (host_sw_if_indexp) *host_sw_if_indexp = host_sw_if_index;
...
static walk_rc_t lcp_itf_pair_walk_sweep(index_t lipi, void *arg)
{
    ...
    if (lip->lip_flags & LIP_FLAG_STALE) vec_add1(ctx->indicies, lipi);
...
static clib_error_t *lcp_itf_pair_link_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    ...
    if (!lcp_main.test_mode) {
        ...
        if (flags & VNET_HW_INTERFACE_FLAG_LINK_UP && hi->link_speed != UINT32_MAX) {
...
static clib_error_t *lcp_interface_init(vlib_main_t *vm)
{
    vlib_punt_hdl_t punt_hdl = vlib_punt_client_register("linux-cp");

    /* punt IKE */
    vlib_punt_register(punt_hdl, ipsec_punt_reason[IPSEC_PUNT_IP4_SPI_UDP_0], "linux-cp-punt");
    vlib_punt_register(punt_hdl, ipsec_punt_reason[IPSEC_PUNT_IP6_SPI_UDP_0], "linux-cp-punt");
