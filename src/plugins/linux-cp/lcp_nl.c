...
#define FOREACH_VFT(__func, __arg)                                                             \
    {                                                                                          \
        ...
        vec_foreach (__nv, nm->nl_vfts) {                                                      \
            if (!__nv->__func.cb) continue;                                                    \
                                                                                               \
            if (!__nv->__func.is_mp_safe) vlib_worker_thread_barrier_sync(vlib_get_main());    \
                                                                                               \
            __nv->__func.cb(__arg);                                                            \
                                                                                               \
            if (!__nv->__func.is_mp_safe) vlib_worker_thread_barrier_release(vlib_get_main()); \
        }                                                                                      \
    }
...
void nl_register_vft(const nl_vft_t *nv)
{
    nl_main_t *nm = &nl_main;
...
static void nl_route_del(struct rtnl_route *rr, void *arg)
{
    FOREACH_VFT(nvl_rt_route_del, rr);
}

static void nl_route_add(struct rtnl_route *rr, void *arg)
{
    int is_replace = 0;
...
static void nl_route_sync_begin(void)
{
    FOREACH_VFT_NO_ARG(nvl_rt_route_sync_begin);
}

static void nl_route_sync_end(void)
{
    FOREACH_VFT_NO_ARG(nvl_rt_route_sync_end);
}

static void nl_neigh_del(struct rtnl_neigh *rn, void *arg)
{
    FOREACH_VFT(nvl_rt_neigh_del, rn);
}

static void nl_neigh_add(struct rtnl_neigh *rn, void *arg)
{
    FOREACH_VFT(nvl_rt_neigh_add, rn);
}

static void nl_neigh_sync_begin(void)
{
    FOREACH_VFT_NO_ARG(nvl_rt_neigh_sync_begin);
}

static void nl_neigh_sync_end(void)
{
    FOREACH_VFT_NO_ARG(nvl_rt_neigh_sync_end);
}

static void nl_link_addr_del(struct rtnl_addr *rla, void *arg)
{
    FOREACH_VFT(nvl_rt_addr_del, rla);
}

static void nl_link_addr_add(struct rtnl_addr *rla, void *arg)
{
    FOREACH_VFT(nvl_rt_addr_add, rla);
}

static void nl_link_addr_sync_begin(void)
{
    FOREACH_VFT_NO_ARG(nvl_rt_addr_sync_begin);
}

static void nl_link_addr_sync_end(void)
{
    FOREACH_VFT_NO_ARG(nvl_rt_addr_sync_end);
}

static void nl_link_del(struct rtnl_link *rl, void *arg)
{
    FOREACH_VFT_CTX(nvl_rt_link_del, rl, arg);
}

static void nl_link_add(struct rtnl_link *rl, void *arg)
{
    FOREACH_VFT_CTX(nvl_rt_link_add, rl, arg);
}

static void nl_link_sync_begin(void)
{
    FOREACH_VFT_NO_ARG(nvl_rt_link_sync_begin);
}

static void nl_link_sync_end(void)
{
...
static int nl_route_process_msgs(void)
{
    nl_main_t     *nm = &nl_main;
    ...
    vec_foreach (msg_info, nm->nl_msg_queue) {
        if ((err = nl_msg_parse(msg_info->msg, nl_route_dispatch, msg_info)) < 0) NL_ERROR("Unable to parse object: %s", nl_geterror(err));
        nlmsg_free(msg_info->msg);
        if (++n_msgs >= nm->batch_size) break;
    }

    /* remove the messages we processed from the head of the queue */
    if (n_msgs) vec_delete(nm->nl_msg_queue, n_msgs, 0);
...
static int lcp_nl_route_discard_msgs(void)
{
    nl_main_t     *nm = &nl_main;
    nl_msg_info_t *msg_info;
    int            n_msgs;
...
static int lcp_nl_route_send_dump_req(nl_sock_type_t sock_type, int msg_type)
{
    nl_main_t      *nm       = &nl_main;
    ...
    err = nl_send_simple(sk_route, msg_type, NLM_F_DUMP, &rt_hdr, sizeof(rt_hdr));

    if (err < 0) {
        NL_ERROR("Unable to send a dump request: %s", nl_geterror(err));
    }
    else
        NL_INFO("Dump request sent via socket %d of type %d", nl_socket_get_fd(sk_route), sock_type);

    return err;
}

static int lcp_nl_route_dump_cb(struct nl_msg *msg, void *arg)
{
    int err;

    if ((err = nl_msg_parse(msg, nl_route_dispatch, NULL)) < 0) NL_ERROR("Unable to parse object: %s", nl_geterror(err));

    return NL_OK;
}

static int lcp_nl_recv_dump_replies(nl_sock_type_t sock_type, int msg_limit, int *is_done_rcvd)
{
    nl_main_t         *nm       = &nl_main;
    struct nl_sock    *sk_route = nm->sk_route_sync[sock_type];
...
continue_reading:
    ...
    if (!done && n_msgs < msg_limit) goto continue_reading;
...
static uword nl_route_process(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    nl_main_t *nm = &nl_main;
    uword      event_type;
    uword     *event_data = 0;
        ...
        if (nm->nl_status == NL_STATUS_NOTIF_PROC) {
            ...
            switch (event_type) {
                ...
                case NL_EVENT_READ:
                    ...
                    wait_time = (vec_len(nm->nl_msg_queue) != 0) ? nm->batch_delay_ms * 1e-3 : DAY_F64;
                    break;
             ...
             */
            NL_INFO("Wait before next synchronization attempt for %ums", nm->sync_attempt_delay_ms);
...
#define _(stype, mtype, tname, fn)                                                                            \
    ...
    is_done = 0;                                                                                              \
    do {                                                                                                      \
        n_msgs = lcp_nl_recv_dump_replies(stype, nm->sync_batch_limit, &is_done);                             \
        if (n_msgs < 0) {                                                                                     \
            NL_ERROR("Error receiving dump replies of type " tname ": %s (%d)", nl_geterror(n_msgs), n_msgs); \
            break;                                                                                            \
        }                                                                                                     \
        else if (n_msgs == 0) {                                                                               \
         ...
         * incomplete                                                                                         \
         */                                                                                                   \
        vlib_process_wait_for_event_or_clock(vm, nm->sync_batch_delay_ms * 1e-3);                             \
        event_type = vlib_process_get_events(vm, &event_data);                                                \
        vec_reset_length(event_data);                                                                         \
                                                                                                              \
         ...
         * attempt later                                                                                      \
         */                                                                                                   \
        if (event_type == NL_EVENT_ERR) goto sync_later;                                                      \
    } while (!is_done);                                                                                       \
    nl_##fn##_sync_end();
...
int lcp_nl_drain_messages(void)
{
    int        err;
    nl_main_t *nm = &nl_main;
    ...
    if (err != -NLE_AGAIN)
        vlib_process_signal_event(vlib_get_main(), nl_route_process_node.index, NL_EVENT_ERR, 0);
    else {
        /* If netlink notification processing is active, signal process node
         * there were notifications read
         */
        if (nm->nl_status == NL_STATUS_NOTIF_PROC) vlib_process_signal_event(vlib_get_main(), nl_route_process_node.index, NL_EVENT_READ, 0);
    }

    return err;
}

void lcp_nl_pair_add_cb(lcp_itf_pair_t *pair)
{
    lcp_nl_drain_messages();
}

static clib_error_t *nl_route_read_cb(clib_file_t *f)
{
    int err;
...
static clib_error_t *nl_route_error_cb(clib_file_t *f)
{
    ...
    return clib_error_return(0, "Error polling netlink socket %d", f->file_descriptor);
}

struct nl_cache *lcp_nl_get_cache(lcp_nl_obj_t t)
{
    nl_main_t *nm = &nl_main;
...
void lcp_nl_set_buffer_size(u32 buf_size)
{
    nl_main_t *nm = &nl_main;

    nm->rx_buf_size = buf_size;

    if (nm->sk_route) nl_socket_set_buffer_size(nm->sk_route, nm->rx_buf_size, nm->tx_buf_size);
}

/* Set the batch size - maximum netlink messages to process at one time */
void lcp_nl_set_batch_size(u32 batch_size)
{
    nl_main_t *nm = &nl_main;
...
void lcp_nl_set_batch_delay(u32 batch_delay_ms)
{
    nl_main_t *nm = &nl_main;
...
static clib_error_t *lcp_itf_pair_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "nl-rx-buffer-size %u", &buf_size))
            ...
            return clib_error_return(0, "invalid netlink option: %U", format_unformat_error, input);
    }
...
static void lcp_nl_open_socket(void)
{
    ...
    nl_socket_modify_cb(nm->sk_route, NL_CB_VALID, NL_CB_CUSTOM, nl_route_cb, NULL);
...
static void lcp_nl_open_sync_socket(nl_sock_type_t sock_type)
{
    ...
    if (dest_ns_fd > 0) {
        ...
        if (clib_setns(dest_ns_fd) == -1) NL_ERROR("Cannot set destination ns");
    }
    ...
    if (dest_ns_fd > 0) {
        if (curr_ns_fd == -1) {
            ...
            if (clib_setns(curr_ns_fd) == -1) NL_ERROR("Cannot set previous ns");
            close(curr_ns_fd);
        }
    }

    NL_INFO("Opened netlink synchronization socket %d of type %d", nl_socket_get_fd(sk_route), sock_type);
}
...
static void lcp_nl_close_sync_socket(nl_sock_type_t sock_type)
{
    ...
    if (sk_route) {
        NL_INFO("Closing netlink synchronization socket %d of type %d", nl_socket_get_fd(sk_route), sock_type);
...
clib_error_t *lcp_nl_init(vlib_main_t *vm)
{
    nl_main_t         *nm              = &nl_main;
...
VLIB_INIT_FUNCTION(lcp_nl_init) = {
    .runs_after = VLIB_INITS("lcp_interface_init", "tuntap_init", "ip_neighbor_init"),
};
