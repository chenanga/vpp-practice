 ...
 */
always_inline u32 mssc_mss_fixup(vlib_buffer_t *b0, tcp_header_t *tcp0, u16 max_mss0)
{
    ip_csum_t sum0;
    ...
    if (PREDICT_FALSE(tcp_syn(tcp0))) {
        ...
        for (; opts_len > 0; opts_len -= opt_len, data += opt_len) {
            ...
            if (kind == TCP_OPTION_EOL)
                ...
                /* broken options */
                if (opts_len < 2) return 0;
                opt_len = data[1];

                /* weird option length */
                if (opt_len < 2 || opt_len > opts_len) return 0;
            }
            ...
            if (kind == TCP_OPTION_MSS) {
                ...
                if (clib_net_to_host_u16(mss0) > max_mss0) {
                    ...
                    *((u16 *) (data + 2)) = new_mss0;
                    sum0                  = tcp0->checksum;
                    sum0                  = ip_csum_update(sum0, mss0, new_mss0, tcp_header_t, checksum);
...
always_inline uword mssc_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, vlib_dir_t dir, fib_protocol_t fproto)
{
    ...
    while (n_left >= 4) {
        ...
        if (FIB_PROTOCOL_IP4 == fproto) {
            ...
            if (IP_PROTOCOL_TCP == ip0->protocol) {
                clamped0 = mssc_mss_fixup(b[0], ip4_next_header(ip0), cm->max_mss4[sw_if_index0]);
            }
            if (IP_PROTOCOL_TCP == ip1->protocol) {
                clamped1 = mssc_mss_fixup(b[1], ip4_next_header(ip1), cm->max_mss4[sw_if_index1]);
            }
        }
        else if (FIB_PROTOCOL_IP6 == fproto) {
            ip6_header_t *ip0  = (ip6_header_t *) h0;
            ip6_header_t *ip1  = (ip6_header_t *) h1;
            tcp_header_t *tcp0 = ip6_ext_header_find(vm, b[0], ip0, IP_PROTOCOL_TCP, NULL);
            tcp_header_t *tcp1 = ip6_ext_header_find(vm, b[1], ip1, IP_PROTOCOL_TCP, NULL);

            if (tcp0) clamped0 = mssc_mss_fixup(b[0], tcp0, cm->max_mss6[sw_if_index0]);
            if (tcp1) clamped1 = mssc_mss_fixup(b[1], tcp1, cm->max_mss6[sw_if_index1]);
        }
        ...
        if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE)) {
            if (b[0]->flags & VLIB_BUFFER_IS_TRACED) {
                ...
                t->max_mss = (FIB_PROTOCOL_IP4 == fproto) ? cm->max_mss4[sw_if_index0] : cm->max_mss6[sw_if_index0];
                t->clamped = clamped0;
            }
            if (b[1]->flags & VLIB_BUFFER_IS_TRACED) {
                ...
                t->max_mss = (FIB_PROTOCOL_IP4 == fproto) ? cm->max_mss4[sw_if_index1] : cm->max_mss6[sw_if_index1];
                t->clamped = clamped1;
            }
        }
