...
static void vl_api_mss_clamp_enable_disable_t_handler(vl_api_mss_clamp_enable_disable_t *mp)
{
    ...
    rv = mssc_enable_disable(sw_if_index, mp->ipv4_direction, mp->ipv6_direction, ntohs(mp->ipv4_mss), ntohs(mp->ipv6_mss));
...
static void send_mss_clamp_details(u32 sw_if_index, vl_api_registration_t *rp, u32 context)
{
    ...
    if (rv == VNET_API_ERROR_FEATURE_DISABLED) return;
...
static void vl_api_mss_clamp_get_t_handler(vl_api_mss_clamp_get_t *mp)
{
    ...
    if (sw_if_index == ~0) {
        ...
        REPLY_AND_DETAILS_VEC_MACRO(VL_API_MSS_CLAMP_GET_REPLY, cm->dir_enabled4, mp, rmp, rv, ({
