...
int cdp_enable_disable(cdp_main_t *cm, int enable_disable)
{
    ...
    if (enable_disable) {
        ...
        vlib_process_signal_event(cm->vlib_main, cm->cdp_process_node_index, CDP_EVENT_ENABLE, 0);
    }
    else {
        vnet_cdp_create_periodic_process(cm);
        vlib_process_signal_event(cm->vlib_main, cm->cdp_process_node_index, CDP_EVENT_DISABLE, 0);
    }
