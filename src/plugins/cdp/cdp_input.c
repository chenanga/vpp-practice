...
static u8 *format_generic_tlv(u8 *s, va_list *va)
{
    ...
    s = format(s, "%s(%d): %U\n", h->name, t->t, format_hex_bytes, t->v, t->l - sizeof(*t));
    return s;
}
...
#define _(z, dbg)                                                                         \
    static cdp_error_t process_##z##_tlv(cdp_main_t *cm, cdp_neighbor_t *n, cdp_tlv_t *t) \
    {                                                                                     \
        int i;                                                                            \
        if (dbg) fformat(stdout, "%U\n", format_text_tlv, t);                             \
                                                                                          \
        if (n->z) vec_set_len(n->z, 0);                                                   \
                                                                                          \
        for (i = 0; i < (t->l - sizeof(*t)); i++)                                         \
            vec_add1(n->z, t->v[i]);                                                      \
                                                                                          \
...
static cdp_error_t process_cdp_hdr(cdp_main_t *cm, cdp_neighbor_t *n, cdp_hdr_t *h)
{
    ...
    if (h->version != 1 && h->version != 2) return CDP_ERROR_PROTOCOL_VERSION;
...
static int cdp_packet_scan(cdp_main_t *cm, cdp_neighbor_t *n)
{
    ...
    if (computed_checksum) return CDP_ERROR_CHECKSUM;
    ...
    // there are no tlvs
    if (vec_len(n->last_rx_pkt) <= 0) return CDP_ERROR_BAD_TLV;
    ...
    while (cur + 3 <= end) {
        ...
        if (tlv->l < 4) return CDP_ERROR_BAD_TLV;

        cur += tlv->l;
        if ((cur - 1) > end) return CDP_ERROR_BAD_TLV;
...
cdp_error_t cdp_input(vlib_main_t *vm, vlib_buffer_t *b0, u32 bi0)
{
    ...
    p = hash_get(cm->neighbor_by_sw_if_index, vnet_buffer(b0)->sw_if_index[VLIB_RX]);
    ...
    if (p == 0) {
        ...
        hash_set(cm->neighbor_by_sw_if_index, n->sw_if_index, n - cm->neighbors);
    }
    else {
    ...
    if (n->last_rx_pkt) vec_set_len(n->last_rx_pkt, 0);

    /* cdp disabled on this interface, we're done */
    if (n->disabled) return CDP_ERROR_DISABLED;
     ...
     */
    last_packet_signature = hash_memory(n->last_rx_pkt, vec_len(n->last_rx_pkt), 0xd00b);

    if (n->last_packet_signature_valid && n->last_packet_signature == last_packet_signature) {
...
static u8 *format_cdp_neighbors(u8 *s, va_list *va)
{
    ...
    pool_foreach (n, cm->neighbors) {
        ...
        if (n->disabled == 0) s = format(s, "%=25v %=25s %=25s %=10.1f\n", hw->name, n->device_name, n->port_id, n->last_heard);
    }
    return s;
}
