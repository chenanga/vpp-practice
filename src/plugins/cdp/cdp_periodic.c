...
static void send_ethernet_hello(cdp_main_t *cm, cdp_neighbor_t *n, int count)
{
    ...
    for (i = 0; i < count; i++) {
         ...
         */
        h0 = vlib_packet_template_get_packet(vm, &cm->packet_templates[n->packet_template_index], &bi0);
        ...
        b0->current_length = nbytes_to_checksum + sizeof(*h0) - sizeof(cdp_hdr_t);

        /* And the outbound interface */
        vnet_buffer(b0)->sw_if_index[VLIB_TX] = hw->sw_if_index;

        /* Set the 802.3 ethernet length */
        h0->ethernet.len = htons(b0->current_length - sizeof(ethernet_802_3_header_t));
...
void cdp_periodic(vlib_main_t *vm)
{
    ...
    for (i = 0; i < vec_len(n_list); i++) {
        ...
        if (n->disabled == 1) continue;

        sw = vnet_get_sw_interface(cm->vnet_main, n->sw_if_index);

        /* Interface shutdown or rx timeout? */
        if (!(sw->flags & VNET_SW_INTERFACE_FLAG_ADMIN_UP) || (now > (n->last_heard + (f64) n->ttl_in_seconds)))
    ...
    if (delete_list) vec_set_len(delete_list, 0);
    if (n_list) vec_set_len(n_list, 0);
}
