        ...
        switch (event_type) {
            ...
            case CDP_EVENT_ENABLE:
                if (!cm->cdp_protocol_registered) {
                     ...
                     */
                    snap_register_input_protocol(vm, "cdp-input", 0xC /* ieee_oui, Cisco */, 0x2000 /* protocol CDP */, cdp_input_node.index);

                    snap_register_input_protocol(vm, "cdp-input", 0xC /* ieee_oui, Cisco */, 0x2004 /* protocol CDP */, cdp_input_node.index);
#if 0
	      /*
	      ...
	      hdlc_register_input_protocol (vm, HDLC_PROTOCOL_cdp,
                    ...
                    ethernet_register_input_type(vm, ETHERNET_TYPE_CDP /* CDP */, cdp_input_node.index);
...
void vnet_cdp_create_periodic_process(cdp_main_t *cmp)
{
    ...
    if (cmp->cdp_process_node_index > 0) return;

    /* No, create it now and make a note of the node index */
    cmp->cdp_process_node_index = vlib_process_create(cmp->vlib_main, "cdp-process", cdp_process, 16 /* log2_n_stack_bytes */);
}
