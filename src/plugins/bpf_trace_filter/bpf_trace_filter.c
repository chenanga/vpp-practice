...
clib_error_t *bpf_trace_filter_init(vlib_main_t *vm)
{
...
u8 *format_bpf_trace_filter(u8 *s, va_list *a)
{
    ...
    if (!btm->prog_set) return format(s, "bpf trace filter is not set");
...
clib_error_t *bpf_trace_filter_set_unset(const char *bpf_expr, u8 is_del, u8 optimize)
{
    bpf_trace_filter_main_t *btm = &bpf_trace_filter_main;
    if (is_del) {
        if (btm->prog_set) {
        ...
        if (btm->prog_set) pcap_freecode(&btm->prog);
        btm->prog_set = 0;
...
int bpf_is_packet_traced(vlib_buffer_t *b, u32 classify_table_index, int func)
{
    ...
    if (classify_table_index != ~0 && (res1 = vnet_is_packet_traced(b, classify_table_index, 0)) != 1) return res1;

    if (!bfm->prog_set) return 1;
...
VLIB_REGISTER_TRACE_FILTER_FUNCTION(bpf_trace_filter_fn, static) = {.name        = "bpf_trace_filter",
