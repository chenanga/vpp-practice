...
static void send_stn_rules_details(stn_rule_t *r, vl_api_registration_t *reg, u32 context)
{
    ...
    rmp->_vl_msg_id = clib_host_to_net_u16(VL_API_STN_RULES_DETAILS + stn_main.msg_id_base);

    ip_address_encode(&r->address, ip46_address_is_ip4(&r->address) ? IP46_TYPE_IP4 : IP46_TYPE_IP6, &rmp->ip_address);
    rmp->context     = context;
