...
unsigned char def_str[]       = "Endpoint with flow-based dynamic proxy to SR-unaware appliance";
...
static int srv6_ad_flow_localsid_creation_fn(ip6_sr_localsid_t *localsid)
{
    ...
    if (ls_mem->inner_type == AD_TYPE_IP4) {
        ...
        int ret = vnet_feature_enable_disable("ip4-unicast", "srv6-ad4-flow-rewrite", ls_mem->sw_if_index_in, 1, 0, 0);
        if (ret != 0) {
            adj_unlock(ls_mem->nh_adj);
            clib_mem_free(ls_mem);
        ...
        int ret = vnet_feature_enable_disable("ip6-unicast", "srv6-ad6-flow-rewrite", ls_mem->sw_if_index_in, 1, 0, 0);
        if (ret != 0) {
            adj_unlock(ls_mem->nh_adj);
            clib_mem_free(ls_mem);
            return SID_CREATE_IFACE_FEATURE_ERROR;
    ...
    ls_mem->cache_memory_size = ad_flow_calc_bihash_memory(ls_mem->cache_buckets, sizeof(clib_bihash_40_8_t));
    ...
    clib_bihash_init_40_8(&ls_mem->ftable, "ad-flow", ls_mem->cache_buckets, ls_mem->cache_memory_size);
    ...
    *ls_p         = ls_mem;
    ...
    vlib_validate_combined_counter(&(sm->sid_cache_full_counters), ls_mem->index);
...
static int srv6_ad_flow_localsid_removal_fn(ip6_sr_localsid_t *localsid)
{
    ...
    if (ls_mem->inner_type == AD_TYPE_IP4) {
        ...
        int ret = vnet_feature_enable_disable("ip4-unicast", "srv6-ad4-flow-rewrite", ls_mem->sw_if_index_in, 0, 0, 0);
        if (ret != 0) return -1;
        ...
        int ret = vnet_feature_enable_disable("ip6-unicast", "srv6-ad6-flow-rewrite", ls_mem->sw_if_index_in, 0, 0, 0);
        if (ret != 0) return -1;
...
u8 *format_srv6_ad_flow_localsid(u8 *s, va_list *args)
{
    ...
    vlib_get_combined_counter(&(sm->sid_bypass_counters), ls_mem->index, &sid_bypass);
    vlib_get_combined_counter(&(sm->sid_punt_counters), ls_mem->index, &sid_punt);
    vlib_get_combined_counter(&(sm->sid_cache_full_counters), ls_mem->index, &sid_full);
    vlib_get_combined_counter(&(sm->rw_valid_counters), ls_mem->index, &rw_valid);
    vlib_get_combined_counter(&(sm->rw_invalid_counters), ls_mem->index, &rw_invalid);

    s = format(s, "\tTraffic that bypassed the NF: \t[%Ld packets : %Ld bytes]\n", sid_bypass.packets, sid_bypass.bytes);
    s = format(s, "\tPunted traffic: \t[%Ld packets : %Ld bytes]\n", sid_punt.packets, sid_punt.bytes);
    s = format(s, "\tDropped traffic (cache full): \t[%Ld packets : %Ld bytes]\n", sid_full.packets, sid_full.bytes);
...
#define PARAM_AD_IIF (1 << 2)
    ...
    if (!unformat(input, "end.ad.flow")) return 0;

    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (!(params & PARAM_AD_NH) && unformat(input, "nh %U", unformat_ip4_address, &nh_addr.ip4)) {
            inner_type = AD_TYPE_IP4;
            params |= PARAM_AD_NH;
        }
        if (!(params & PARAM_AD_NH) && unformat(input, "nh %U", unformat_ip6_address, &nh_addr.ip6)) {
            inner_type = AD_TYPE_IP6;
            params |= PARAM_AD_NH;
        }
        else if (!(params & PARAM_AD_OIF) && unformat(input, "oif %U", unformat_vnet_sw_interface, vnm, &sw_if_index_out)) {
            params |= PARAM_AD_OIF;
        }
        else if (!(params & PARAM_AD_IIF) && unformat(input, "iif %U", unformat_vnet_sw_interface, vnm, &sw_if_index_in)) {
            params |= PARAM_AD_IIF;
        }
        else {
            break;
...
static clib_error_t *srv6_ad_flow_init(vlib_main_t *vm)
{
    ...
    /* Create DPO */
    sm->srv6_ad_flow_dpo_type = dpo_register_new_type(&srv6_ad_flow_vft, srv6_ad_flow_nodes);
