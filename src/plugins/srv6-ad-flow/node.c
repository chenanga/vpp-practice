...
static u8 *format_srv6_ad_flow_localsid_trace(u8 *s, va_list *args)
{
    ...
    srv6_ad_flow_localsid_trace_t *t = va_arg(*args, srv6_ad_flow_localsid_trace_t *);

    return format(s, "SRv6-AD-Flow-localsid: localsid_index %d", t->localsid_index);
}
...
static u8 *format_srv6_ad_flow_rewrite_trace(u8 *s, va_list *args)
{
    CLIB_UNUSED(vlib_main_t * vm)   = va_arg(*args, vlib_main_t *);
    CLIB_UNUSED(vlib_node_t * node) = va_arg(*args, vlib_node_t *);
    srv6_ad_flow_rewrite_trace_t *t = va_arg(*args, srv6_ad_flow_rewrite_trace_t *);

    if (PREDICT_FALSE(t->error != 0)) {
        return format(s, "SRv6-AD-Flow-rewrite: cache is empty");
    }

    return format(s, "SRv6-AD-Flow-rewrite: src %U dst %U", format_ip6_address, &t->src, format_ip6_address, &t->dst);
}
...
always_inline void ad_flow_entry_update_lru(srv6_ad_flow_localsid_t *ls, srv6_ad_flow_entry_t *e)
{
...
always_inline void ad_flow_entry_delete(srv6_ad_flow_localsid_t *ls, srv6_ad_flow_entry_t *e, int lru_delete)
{
    clib_bihash_kv_40_8_t kv;

    if (ls->inner_type == AD_TYPE_IP4) {
        kv.key[0] = ((u64) e->key.s_addr.ip4.as_u32 << 32) | (u64) e->key.d_addr.ip4.as_u32;
...
always_inline u32 ad_flow_value_get_session_index(clib_bihash_kv_40_8_t *value)
{
                                                    ...
                                                    f64                            now)
{
    ...
    end_ad_flow_walk_expect_first_hdr(vm, b, (void *) (ip + 1), ip->protocol, IP_PROTOCOL_IPV6_ROUTE, &encap_length, (u8 **) &srh);

    /* Punt the packet if no SRH or SRH with SL = 0 */
    if (PREDICT_FALSE(srh == NULL || srh->type != ROUTING_HEADER_TYPE_SR || srh->segments_left == 0)) {
    ...
    int ret = end_ad_flow_walk_expect_first_hdr(vm,
                                                b,
                                                ip6_ext_next_header((ip6_ext_header_t *) srh),
                                                srh->protocol,
                                                IP_PROTOCOL_IPV6,
                                                &encap_length,
                                                (u8 **) &ulh);
    ...
    if (PREDICT_TRUE(ulh->protocol == IP_PROTOCOL_UDP || ulh->protocol == IP_PROTOCOL_TCP)) {
    ...
    if (!clib_bihash_search_40_8(h, &kv, &value)) {
        e = pool_elt_at_index(ls_mem->cache, ad_flow_value_get_session_index(&value));
    }
    ...
    if (!e) {
        ...
        ctx.now = now;
        ctx.ls  = ls_mem;
        clib_bihash_add_or_overwrite_stale_40_8(h, &kv, ad_flow_is_idle_entry_cb, &ctx);
    }
    e->last_heard = now;
...
static uword srv6_ad_flow_localsid_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            ls0 = pool_elt_at_index(srm->localsids, vnet_buffer(b0)->ip.adj_index[VLIB_TX]);
            ...
            if (ls_mem0->inner_type == AD_TYPE_IP6)
                end_ad_flow_processing_v6(vm, b0, ip0, ls_mem0, &next0, &cnt0, &cnt_idx0, now);
            else
                end_ad_flow_processing_v4(vm, b0, ip0, ls_mem0, &next0, &cnt0, &cnt_idx0, now);

            /* Trace packet (if enabled) */
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                srv6_ad_flow_localsid_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof *tr);
                tr->localsid_index                = ls_mem0->index;
            }

            /* Increment the appropriate per-SID counter */
            vlib_increment_combined_counter(cnt0, thread_index, cnt_idx0, 1, vlib_buffer_length_in_chain(vm, b0));
...
static uword srv6_ad4_flow_rewrite_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            ls0       = pool_elt_at_index(srm->localsids, sm->sw_iface_localsid4[vnet_buffer(b0)->sw_if_index[VLIB_RX]]);
            ...
            if (PREDICT_FALSE(ls0_mem == NULL)) {
                ...
                /* Compute flow hash */
                u64 ports = 0;
                if (PREDICT_TRUE(ip0_encap->protocol == IP_PROTOCOL_UDP || ip0_encap->protocol == IP_PROTOCOL_TCP)) {
                    udp_header_t *udp0 = (udp_header_t *) (ip0_encap + 1);
                    ports              = ((u64) udp0->src_port << 16) | ((u64) udp0->dst_port);
                }
                ...
                /* Lookup flow in hashtable */
                if (clib_bihash_search_40_8(&ls0_mem->ftable, &kv0, &value0) < 0) {
                    /* not found */
                    next0     = SRV6_AD_FLOW_REWRITE_NEXT_ERROR;
                    b0->error = node->errors[SRV6_AD_FLOW_REWRITE_COUNTER_NO_RW];
                }
                    ...
                    /* found */
                    s0 = pool_elt_at_index(ls0_mem->cache, ad_flow_value_get_session_index(&value0));
                    ASSERT(s0);
                    ASSERT(VLIB_BUFFER_PRE_DATA_SIZE >= (s0->rw_len + b0->current_data));

                    clib_memcpy_fast(((u8 *) ip0_encap) - s0->rw_len, s0->rw_data, s0->rw_len);
                    ...
                    new_l0              = s0->rw_len - sizeof(ip6_header_t) + clib_net_to_host_u16(ip0_encap->length);
                    ip0->payload_length = clib_host_to_net_u16(new_l0);
                }
            }

            if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE) && PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                srv6_ad_flow_rewrite_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof *tr);
                tr->error                        = 0;
            ...
            vlib_increment_combined_counter(((next0 == SRV6_AD_FLOW_REWRITE_NEXT_ERROR) ? &(sm->rw_invalid_counters) : &(sm->rw_valid_counters)),
                                            vm->thread_index,
                                            ls0_mem->index,
                                            1,
    ...
    /* Update counters */
    vlib_node_increment_counter(vm, srv6_ad4_flow_rewrite_node.index, SRV6_AD_FLOW_REWRITE_COUNTER_PROCESSED, cnt_packets);
...
static uword srv6_ad6_flow_rewrite_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            b0        = vlib_get_buffer(vm, bi0);
            ip0_encap = vlib_buffer_get_current(b0);
            ls0       = pool_elt_at_index(srm->localsids, sm->sw_iface_localsid6[vnet_buffer(b0)->sw_if_index[VLIB_RX]]);
            ls0_mem   = ls0->plugin_mem;
            ...
            if (PREDICT_FALSE(ls0_mem == NULL)) {
                ...
                if (clib_bihash_search_40_8(&ls0_mem->ftable, &kv0, &value0)) {
                    ...
                    /* Update outer IPv6 length (in case it has changed) */
                    new_l0              = s0->rw_len + clib_net_to_host_u16(ip0_encap->payload_length);
                    ip0->payload_length = clib_host_to_net_u16(new_l0);
                }
            }
    ...
    /* Update counters */
    vlib_node_increment_counter(vm, srv6_ad6_flow_rewrite_node.index, SRV6_AD_FLOW_REWRITE_COUNTER_PROCESSED, cnt_packets);
