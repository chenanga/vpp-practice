...
static clib_error_t *dslite_add_del_pool_addr_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    /* Get a line of input. */
    if (!unformat_user(input, unformat_line_input, line_input)) return clib_error_return(0, DSLITE_EXPECTED_ARGUMENT);
    ...
    switch (rv) {
        case VNET_API_ERROR_NO_SUCH_ENTRY:
            error = clib_error_return(0, "DS-Lite pool address %U not exist.", format_ip4_address, &this_addr);
            break;
        case VNET_API_ERROR_VALUE_EXIST:
            error = clib_error_return(0, "DS-Lite pool address %U exist.", format_ip4_address, &this_addr);
            break;
    }
...
static clib_error_t *dslite_set_aftr_tunnel_addr_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (rv) error = clib_error_return(0, "Set DS-Lite AFTR tunnel endpoint address failed.");
...
static clib_error_t *dslite_set_b4_tunnel_addr_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (rv) error = clib_error_return(0, "Set DS-Lite B4 tunnel endpoint address failed.");
...
static u8 *format_dslite_b4(u8 *s, va_list *args)
{
    ...
    s = format(s, "B4 %U %d sessions\n", format_ip6_address, &b4->addr, b4->nsessions);

    if (b4->nsessions == 0) return s;
