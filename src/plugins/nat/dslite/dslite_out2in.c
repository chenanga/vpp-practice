...
static inline u32 dslite_icmp_out2in(dslite_main_t *dm, ip4_header_t *ip4, dslite_session_t **sp, u32 next, u8 *error, u32 thread_index)
{
    ...
    if (icmp_type_is_error_message(icmp->type) || (icmp->type != ICMP4_echo_reply)) {
    ...
    if (clib_bihash_search_8_8(&dm->per_thread_data[thread_index].out2in, &kv, &value)) {
...
VLIB_NODE_FN(dslite_out2in_node)
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(proto0 == NAT_PROTOCOL_ICMP)) {
                next0 = dslite_icmp_out2in(dm, ip40, &s0, next0, &error0, thread_index);
                if (PREDICT_FALSE(next0 == DSLITE_OUT2IN_NEXT_DROP)) goto trace0;
            ...
            if (clib_bihash_search_8_8(&dm->per_thread_data[thread_index].out2in, &kv0, &value0)) {
                next0  = DSLITE_OUT2IN_NEXT_DROP;
