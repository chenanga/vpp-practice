...
static clib_error_t *dslite_init(vlib_main_t *vm)
{
    ...
    nat_fib_src_hi = fib_source_allocate("dslite-hi", FIB_SOURCE_PRIORITY_HI, FIB_SOURCE_BH_SIMPLE);
...
static void dslite_init_datastructures(void)
{
    ...
    vec_foreach (td, dm->per_thread_data) {
        clib_bihash_init_24_8(&td->in2out, "dslite in2out", translation_buckets, translation_memory_size);

        clib_bihash_init_8_8(&td->out2in, "dslite out2in", translation_buckets, translation_memory_size);
...
static clib_error_t *dslite_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "ce")) dslite_set_ce(dm, 1);
    }
    return 0;
}
...
int dslite_set_aftr_ip6_addr(dslite_main_t *dm, ip6_address_t *addr)
{
    ...
    if (!dm->is_enabled) dslite_init_datastructures();
    ...
    if (dm->is_ce) {
        ...
        fib_table_entry_special_dpo_add(0, &pfx, nat_fib_src_hi, FIB_ENTRY_FLAG_EXCLUSIVE, &dpo);
    }
    else {
...
void add_del_dslite_pool_addr_cb(ip4_address_t addr, u8 is_add, void *opaque)
{
    ...
    if (is_add) {
        ...
        fib_table_entry_special_dpo_add(0, &pfx, nat_fib_src_hi, FIB_ENTRY_FLAG_EXCLUSIVE, &dpo_v4);
        dpo_reset(&dpo_v4);
    }
    else {
