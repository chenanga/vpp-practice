...
int nat_add_del_ip4_pool_addr(nat_ip4_pool_t *pool, ip4_address_t addr, u8 is_add)
{
    ...
    if (is_add) {
        if (a) return NAT_ERROR_VALUE_EXIST;
...
#undef _
        ...
        if (!a) return NAT_ERROR_NO_SUCH_ENTRY;
#define _(N, id, n, s)                           \
...
int nat_add_del_ip4_pool_addrs(nat_ip4_pool_t *pool, ip4_address_t addr, u32 count, u8 is_add, void *opaque)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if ((rv = nat_add_del_ip4_pool_addr(pool, addr, is_add)) != 0) return rv;
...
{
    return min + random_u32(random_seed) / (random_u32_max() / (max - min + 1) + 1);
}
    ...
    case NAT_PROTOCOL_##N:                                                                                                           \
        if (a->busy_##n##_ports_per_thread[thread_index] < port_per_thread) {                                                        \
            if (a->fib_index == fib_index) {                                                                                         \
                while (1) {                                                                                                          \
                    portnum = (port_per_thread * nat_thread_index) + nat_random_port(&pool->random_seed, 1, port_per_thread) + 1024; \
                    if (clib_bitmap_get_no_check(a->busy_##n##_port_bitmap, portnum)) continue;                                      \
...
int nat_alloc_ip4_addr_and_port(nat_ip4_pool_t      *pool,
                                ...
                                nat_ip4_addr_port_t *out)
{
    return pool->alloc_addr_and_port_cb(pool, fib_index, thread_index, nat_thread_index, port_per_thread, protocol, out);
}
