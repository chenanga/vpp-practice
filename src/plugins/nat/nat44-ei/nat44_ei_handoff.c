...
static u8 *format_nat44_ei_handoff_trace(u8 *s, va_list *args)
{
    ...
    s      = format(s, "NAT44_EI_%s_WORKER_HANDOFF %s: next-worker %d trace index %d", tag, output, t->next_worker_index, t->trace_index);

    return s;
}

static inline uword nat44_ei_worker_handoff_fn_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_output, u8 is_in2out)
{
    ...
    while (n_left_from >= 4) {
        ...
        ip1 = (ip4_header_t *) ((u8 *) vlib_buffer_get_current(b[1]) + iph_offset1);
        ip2 = (ip4_header_t *) ((u8 *) vlib_buffer_get_current(b[2]) + iph_offset2);
        ip3 = (ip4_header_t *) ((u8 *) vlib_buffer_get_current(b[3]) + iph_offset3);
        ...
        if (is_in2out) {
            ...
            ti[1] = nat44_ei_get_in2out_worker_index(ip1, rx_fib_index1, is_output);
            ti[2] = nat44_ei_get_in2out_worker_index(ip2, rx_fib_index2, is_output);
            ti[3] = nat44_ei_get_in2out_worker_index(ip3, rx_fib_index3, is_output);
        }
        else {
            ti[0] = nat44_ei_get_out2in_worker_index(b[0], ip0, rx_fib_index0, is_output);
            ti[1] = nat44_ei_get_out2in_worker_index(b[1], ip1, rx_fib_index1, is_output);
            ti[2] = nat44_ei_get_out2in_worker_index(b[2], ip2, rx_fib_index2, is_output);
            ti[3] = nat44_ei_get_out2in_worker_index(b[3], ip3, rx_fib_index3, is_output);
        }
    ...
    while (n_left_from > 0) {
        ...
        if (is_output) iph_offset0 = vnet_buffer(b[0])->ip.save_rewrite_length;
    ...
    if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE))) {
        ...
        for (i = 0; i < frame->n_vectors; i++) {
            if (b[0]->flags & VLIB_BUFFER_IS_TRACED) {
                nat44_ei_handoff_trace_t *t = vlib_add_trace(vm, node, b[0], sizeof(*t));
    ...
    if (n_enq < frame->n_vectors) {
        vlib_node_increment_counter(vm, node->node_index, NAT44_EI_HANDOFF_ERROR_CONGESTION_DROP, frame->n_vectors - n_enq);
    }

    vlib_node_increment_counter(vm, node->node_index, NAT44_EI_HANDOFF_ERROR_SAME_WORKER, same_worker);
    vlib_node_increment_counter(vm, node->node_index, NAT44_EI_HANDOFF_ERROR_DO_HANDOFF, do_handoff);
