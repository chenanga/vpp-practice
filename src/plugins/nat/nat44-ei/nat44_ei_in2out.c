...
#define foreach_nat44_ei_hairpinning_handoff_error _(CONGESTION_DROP, "congestion drop")
...
static u8 *format_nat44_ei_in2out_trace(u8 *s, va_list *args)
{
    ...
    s   = format(s, "%s: sw_if_index %d, next index %d, session %d", tag, t->sw_if_index, t->next_index, t->session_index);
    if (t->is_hairpinning) s = format(s, ", with-hairpinning");
    return s;
}
...
static u8 *format_nat44_ei_in2out_fast_trace(u8 *s, va_list *args)
{
    ...
    CLIB_UNUSED(vlib_node_t * node) = va_arg(*args, vlib_node_t *);
    nat44_ei_in2out_trace_t *t      = va_arg(*args, nat44_ei_in2out_trace_t *);
    s                               = format(s, "NAT44_IN2OUT_FAST: sw_if_index %d, next index %d", t->sw_if_index, t->next_index);
    return s;
}
...
static u8 *format_nat44_ei_hairpin_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "new dst addr %U port %u fib-index %u", format_ip4_address, &t->addr, clib_net_to_host_u16(t->port), t->fib_index);
...
static u8 *format_nat44_ei_hairpinning_handoff_trace(u8 *s, va_list *args)
{
    CLIB_UNUSED(vlib_main_t * vm)           = va_arg(*args, vlib_main_t *);
    CLIB_UNUSED(vlib_node_t * node)         = va_arg(*args, vlib_node_t *);
    nat44_ei_hairpinning_handoff_trace_t *t = va_arg(*args, nat44_ei_hairpinning_handoff_trace_t *);

    s = format(s, "nat44-ei-hairpinning-handoff: next-worker %d", t->next_worker_index);

    return s;
}

static_always_inline int nat44_ei_not_translate_fast(vlib_node_runtime_t *node, u32 sw_if_index0, ip4_header_t *ip0, u32 proto0, u32 rx_fib_index0)
{
    nat44_ei_main_t *nm = &nat44_ei_main;

    if (nm->out2in_dpo) return 0;
    ...
    if (PREDICT_FALSE(nat44_ei_is_interface_addr(nm->ip4_main, node, sw_if_index0, ip0->dst_address.as_u32))) return 1;
        ...
        if (sw_if_index == ~0) {
            vec_foreach (outside_fib, nm->outside_fibs) {
                ...
                if (FIB_NODE_INDEX_INVALID != fei) {
                    ...
                    if (sw_if_index != ~0) break;
                }
            }
        }
        if (sw_if_index == ~0) return 1;
...
static_always_inline int nat44_ei_not_translate(nat44_ei_main_t     *nm,
                                                ...
                                                u32                  thread_index)
{
    ...
    init_nat_k(&kv0, ip0->dst_address, udp0->dst_port, nm->outside_fib_index, proto0);
    ...
    if (clib_bihash_search_8_8(&nm->out2in, &kv0, &value0)) {
        ...
        if (!nat44_ei_static_mapping_match(ip0->dst_address,
                                           udp0->dst_port,
                                           nm->outside_fib_index,
                                           proto0,
                                           &placeholder_addr,
                                           &placeholder_port,
                                           &placeholder_fib_index,
                                           1,
                                           0,
                                           0))
            return 0;
    ...
    if (nm->forwarding_enabled) return 1;

    return nat44_ei_not_translate_fast(node, sw_if_index0, ip0, proto0, rx_fib_index0);
}

static_always_inline int nat44_ei_not_translate_output_feature(nat44_ei_main_t *nm,
                                                               ip4_header_t    *ip0,
                                                               u32              proto0,
                                                               u16              src_port,
                                                               u16              dst_port,
                                                               ...
                                                               u32              sw_if_index)
{
    ...
    /* src NAT check */
    init_nat_k(&kv0, ip0->src_address, src_port, ip4_fib_table_get_index_for_sw_if_index(sw_if_index), proto0);

    if (!clib_bihash_search_8_8(&nm->out2in, &kv0, &value0)) return 1;

    /* dst NAT check */
    init_nat_k(&kv0, ip0->dst_address, dst_port, ip4_fib_table_get_index_for_sw_if_index(sw_if_index), proto0);
    if (!clib_bihash_search_8_8(&nm->in2out, &kv0, &value0)) {
        /* hairpinning */
        pool_foreach (i, nm->output_feature_interfaces) {
            if ((nat44_ei_interface_is_inside(i)) && (sw_if_index == i->sw_if_index)) return 0;
        }
        return 1;
    }
...
int nat44_i2o_is_idle_session_cb(clib_bihash_kv_8_8_t *kv, void *arg)
{
    ...
    nat44_ei_main_per_thread_data_t *tnm = vec_elt_at_index(nm->per_thread_data, ctx->thread_index);
    ...
    sess_timeout_time = s->last_heard + (f64) nat_session_get_timeout(&nm->timeouts, s->nat_proto, s->state);
    if (ctx->now >= sess_timeout_time) {
        init_nat_o2i_k(&s_kv, s);
        if (clib_bihash_add_del_8_8(&nm->out2in, &s_kv, 0)) nat_elog_warn(nm, "out2in key del failed");
        ...
        nat_ha_sdel(&s->out2in.addr, s->out2in.port, &s->ext_host_addr, s->ext_host_port, s->nat_proto, s->out2in.fib_index, ctx->thread_index);

        if (!nat44_ei_is_session_static(s))
            nat44_ei_free_outside_address_and_port(nm->addresses, ctx->thread_index, &s->out2in.addr, s->out2in.port, s->nat_proto);
                     ...
                     f64                  now)
{
    ...
    if (PREDICT_FALSE(nat44_ei_maximum_sessions_exceeded(nm, thread_index))) {
        ...
        nat_ipfix_logging_max_sessions(thread_index, nm->max_translations_per_thread);
        nat_elog_notice(nm, "maximum sessions exceeded");
        return NAT44_EI_IN2OUT_NEXT_DROP;
    }

    /* First try to match static mapping by local address and port */
    if (nat44_ei_static_mapping_match(i2o_addr, i2o_port, rx_fib_index0, nat_proto, &sm_addr, &sm_port, &sm_fib_index, 0, 0, &identity_nat)) {
        /* Try to create dynamic translation */
        if (nm->alloc_addr_and_port(nm->addresses,
                                    rx_fib_index0,
                                    thread_index,
                                    nat_proto,
                                    ip0->src_address,
    ...
    u = nat44_ei_user_get_or_create(nm, &ip0->src_address, rx_fib_index0, thread_index);
    if (!u) {
    ...
    if (is_sm) s->flags |= NAT44_EI_SESSION_FLAG_STATIC_MAPPING;
    ...
    *sessionp               = s;
    ...
    init_nat_i2o_kv(&kv0, s, thread_index, s - nm->per_thread_data[thread_index].sessions);
    if (clib_bihash_add_or_overwrite_stale_8_8(&nm->in2out, &kv0, nat44_i2o_is_idle_session_cb, &ctx0)) nat_elog_notice(nm, "in2out key add failed");

    init_nat_o2i_kv(&kv0, s, thread_index, s - nm->per_thread_data[thread_index].sessions);
    if (clib_bihash_add_or_overwrite_stale_8_8(&nm->out2in, &kv0, nat44_o2i_is_idle_session_cb, &ctx0)) nat_elog_notice(nm, "out2in key add failed");
    ...
    nat_syslog_nat44_apmadd(s->user_index, s->in2out.fib_index, &s->in2out.addr, s->in2out.port, &s->out2in.addr, s->out2in.port, s->nat_proto);

    nat_ha_sadd(&s->in2out.addr,
                s->in2out.port,
                &s->out2in.addr,
                s->out2in.port,
                &s->ext_host_addr,
...
static_always_inline u32 nat44_ei_icmp_match_in2out_slow(vlib_node_runtime_t *node,
                                                         u32                  thread_index,
                                                         vlib_buffer_t       *b0,
                                                         ip4_header_t        *ip0,
                                                         ip4_address_t       *addr,
    ...
    *fib_index   = ip4_fib_table_get_index_for_sw_if_index(sw_if_index0);
    ...
    if (clib_bihash_search_8_8(&nm->in2out, &kv0, &value0)) {
        if (vnet_buffer(b0)->sw_if_index[VLIB_TX] != ~0) {
            if (PREDICT_FALSE(nat44_ei_not_translate_output_feature(nm, ip0, *proto, *port, *port, thread_index, sw_if_index0))) {
                *dont_translate = 1;
                goto out;
            }
        }
        else {
            if (PREDICT_FALSE(nat44_ei_not_translate(nm, node, sw_if_index0, ip0, NAT_PROTOCOL_ICMP, *fib_index, thread_index))) {
                *dont_translate = 1;
                goto out;
            }
        }

        if (PREDICT_FALSE(icmp_type_is_error_message(vnet_buffer(b0)->ip.reass.icmp_type_or_tcp_flags))) {
            b0->error = node->errors[NAT44_EI_IN2OUT_ERROR_BAD_ICMP_TYPE];
            next0     = NAT44_EI_IN2OUT_NEXT_DROP;
            goto out;
        }

        next0 = slow_path(nm, b0, ip0, *addr, *port, *fib_index, *proto, &s0, node, next0, thread_index, vlib_time_now(vm));

        if (PREDICT_FALSE(next0 == NAT44_EI_IN2OUT_NEXT_DROP)) goto out;
        ...
        if (PREDICT_FALSE(vnet_buffer(b0)->ip.reass.icmp_type_or_tcp_flags != ICMP4_echo_request
                          && vnet_buffer(b0)->ip.reass.icmp_type_or_tcp_flags != ICMP4_echo_reply
                          && !icmp_type_is_error_message(vnet_buffer(b0)->ip.reass.icmp_type_or_tcp_flags))) {
            b0->error = node->errors[NAT44_EI_IN2OUT_ERROR_BAD_ICMP_TYPE];
            next0     = NAT44_EI_IN2OUT_NEXT_DROP;
            goto out;
        }

        s0 = pool_elt_at_index(tnm->sessions, nat_value_get_session_index(&value0));
    }
...
static_always_inline u32 nat44_ei_icmp_match_in2out_fast(vlib_node_runtime_t *node,
                                                         u32                  thread_index,
                                                         vlib_buffer_t       *b0,
                                                         ip4_header_t        *ip0,
                                                         ip4_address_t       *addr,
                                                         ...
                                                         u8                  *dont_translate)
{
    ...
    *fib_index   = ip4_fib_table_get_index_for_sw_if_index(sw_if_index0);
    ...
    if (nat44_ei_static_mapping_match(*addr, *port, *fib_index, *proto, &sm_addr, &sm_port, &sm_fib_index, 0, &is_addr_only, 0)) {
        if (PREDICT_FALSE(nat44_ei_not_translate_fast(node, sw_if_index0, ip0, IP_PROTOCOL_ICMP, *fib_index))) {
            *dont_translate = 1;
            goto out;
        }

        if (icmp_type_is_error_message(vnet_buffer(b0)->ip.reass.icmp_type_or_tcp_flags)) {
            next0 = NAT44_EI_IN2OUT_NEXT_DROP;
            goto out;
        }
    ...
    if (PREDICT_FALSE(vnet_buffer(b0)->ip.reass.icmp_type_or_tcp_flags != ICMP4_echo_request
                      && (vnet_buffer(b0)->ip.reass.icmp_type_or_tcp_flags != ICMP4_echo_reply || !is_addr_only)
                      && !icmp_type_is_error_message(vnet_buffer(b0)->ip.reass.icmp_type_or_tcp_flags))) {
        b0->error = node->errors[NAT44_EI_IN2OUT_ERROR_BAD_ICMP_TYPE];
        next0     = NAT44_EI_IN2OUT_NEXT_DROP;
        goto out;
...
static_always_inline u32 nat44_ei_icmp_hairpinning(nat44_ei_main_t *nm,
                                                   vlib_buffer_t   *b0,
                                                   u32              thread_index,
                                                   ip4_header_t    *ip0,
                                                   icmp46_header_t *icmp0,
                                                   u32             *required_thread_index)
{
    ...
    if (icmp_type_is_error_message(vnet_buffer(b0)->ip.reass.icmp_type_or_tcp_flags)) {
        ...
        if (protocol != NAT_PROTOCOL_TCP && protocol != NAT_PROTOCOL_UDP) return 1;

        init_nat_k(&kv0, ip0->dst_address, l4_header->src_port, nm->outside_fib_index, protocol);
        if (clib_bihash_search_8_8(&nm->out2in, &kv0, &value0)) return 1;
        ...
        sum0                          = icmp0->checksum;
        sum0                          = ip_csum_update(sum0, old_addr0, new_addr0, ip4_header_t, src_address);
        ...
        sum0                = icmp0->checksum;
        sum0                = ip_csum_update(sum0, old_checksum0, new_checksum0, ip4_header_t, checksum);
        icmp0->checksum     = ip_csum_fold(sum0);
        ...
        sum0                = icmp0->checksum;
        sum0                = ip_csum_update(sum0, old_port0, new_port0, tcp_udp_header_t, src_port);
        icmp0->checksum     = ip_csum_fold(sum0);
    }
    else {
        ...
        if (clib_bihash_search_8_8(&nm->static_mapping_by_external, &kv0, &value0)) {
            ...
            init_nat_k(&kv0, ip0->dst_address, icmp_id0, nm->outside_fib_index, NAT_PROTOCOL_ICMP);
            ...
            if (!rv) {
                ...
                sum0                                  = icmp0->checksum;
                sum0                                  = ip_csum_update(sum0, icmp_id0, s0->in2out.port, icmp_echo_header_t, identifier);
                icmp0->checksum                       = ip_csum_fold(sum0);
                goto change_addr;
            }
        ...
        if (vnet_buffer(b0)->sw_if_index[VLIB_TX] == ~0) vnet_buffer(b0)->sw_if_index[VLIB_TX] = m0->fib_index;
    }
change_addr:
    ...
    if (new_dst_addr0) {
        ...
        sum0                    = ip0->checksum;
        sum0                    = ip_csum_update(sum0, old_dst_addr0, new_dst_addr0, ip4_header_t, dst_address);
        ip0->checksum           = ip_csum_fold(sum0);
    }
    return 0;
}

static_always_inline u32 nat44_ei_icmp_in2out(vlib_buffer_t       *b0,
                                              ip4_header_t        *ip0,
                                              icmp46_header_t     *icmp0,
                                              u32                  sw_if_index0,
                                              u32                  rx_fib_index0,
                                              ...
                                              nat44_ei_session_t **p_s0)
{
    ...
    if (PREDICT_TRUE(nm->pat)) {
        next0_tmp = nat44_ei_icmp_match_in2out_slow(node, thread_index, b0, ip0, &addr, &port, &fib_index, &proto, p_s0, &dont_translate);
    }
    else {
        next0_tmp = nat44_ei_icmp_match_in2out_fast(node, thread_index, b0, ip0, &addr, &port, &fib_index, &proto, p_s0, &dont_translate);
    }

    if (next0_tmp != ~0) next0 = next0_tmp;
    if (next0 == NAT44_EI_IN2OUT_NEXT_DROP || dont_translate) goto out;

    if (PREDICT_TRUE(!ip4_is_fragment(ip0))) {
        sum0 =
            ip_incremental_checksum_buffer(vm, b0, (u8 *) icmp0 - (u8 *) vlib_buffer_get_current(b0), ntohs(ip0->length) - ip4_header_bytes(ip0), 0);
    ...
    sum0          = ip0->checksum;
    sum0          = ip_csum_update(sum0, old_addr0, new_addr0, ip4_header_t, src_address /* changed member */);
    ip0->checksum = ip_csum_fold(sum0);

    if (!vnet_buffer(b0)->ip.reass.is_non_first_fragment) {
        if (icmp0->checksum == 0) icmp0->checksum = 0xffff;
        ...
        if (!icmp_type_is_error_message(icmp0->type)) {
            ...
            if (PREDICT_FALSE(new_id0 != echo0->identifier)) {
                ...
                sum0            = icmp0->checksum;
                sum0            = ip_csum_update(sum0, old_id0, new_id0, icmp_echo_header_t, identifier);
                icmp0->checksum = ip_csum_fold(sum0);
            }
        }
        else {
            ...
            switch (proto) {
                ...
                case NAT_PROTOCOL_TCP:
                    ...
                    sum0            = icmp0->checksum;
                    sum0            = ip_csum_update(sum0, old_id0, new_id0, tcp_udp_header_t, dst_port);
                    icmp0->checksum = ip_csum_fold(sum0);
                    break;
                default:
                    ASSERT(0);
    ...
    if (vnet_buffer(b0)->sw_if_index[VLIB_TX] == ~0) {
        if (0 != nat44_ei_icmp_hairpinning(nm, b0, thread_index, ip0, icmp0, &required_thread_index))
                                                        ...
                                                        nat44_ei_session_t **p_s0)
{
    ...
    next0                  = nat44_ei_icmp_in2out(b0, ip0, icmp0, sw_if_index0, rx_fib_index0, node, next0, thread_index, p_s0);
    nat44_ei_session_t *s0 = *p_s0;
    if (PREDICT_TRUE(next0 != NAT44_EI_IN2OUT_NEXT_DROP && s0)) {
        /* Accounting */
        nat44_ei_session_update_counters(s0, now, vlib_buffer_length_in_chain(vm, b0), thread_index);
...
static_always_inline void nat44_ei_hairpinning_sm_unknown_proto(nat44_ei_main_t *nm, vlib_buffer_t *b, ip4_header_t *ip)
{
    ...
    if (clib_bihash_search_8_8(&nm->static_mapping_by_external, &kv, &value)) return;
    ...
    if (vnet_buffer(b)->sw_if_index[VLIB_TX] == ~0) vnet_buffer(b)->sw_if_index[VLIB_TX] = m->fib_index;
}
...
static_always_inline int nat44_ei_hairpinning(vlib_main_t         *vm,
                                              vlib_node_runtime_t *node,
                                              nat44_ei_main_t     *nm,
                                              u32                  thread_index,
                                              vlib_buffer_t       *b0,
                                              ...
                                              u32                 *required_thread_index)
{
    ...
    if (new_dst_addr0 == old_dst_addr0 && new_dst_port0 == old_dst_port0 && vnet_buffer(b0)->sw_if_index[VLIB_TX] == old_sw_if_index) return 0;
    ...
    if (new_dst_addr0) {
        ...
        if (PREDICT_TRUE(new_dst_port0 != old_dst_port0)) {
            if (PREDICT_TRUE(proto0 == NAT_PROTOCOL_TCP)) {
                ...
                sum0           = tcp0->checksum;
                sum0           = ip_csum_update(sum0, old_dst_addr0, new_dst_addr0, ip4_header_t, dst_address);
                sum0           = ip_csum_update(sum0, old_dst_port0, new_dst_port0, ip4_header_t /* cheat */, length);
    ...
    rv = 0;
trace:
    if (do_trace && PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
...
static_always_inline uword nat44_ei_hairpinning_handoff_fn_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u32 fq_index)
{
    ...
    while (n_left_from > 0) {
        ...
        if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b[0]->flags & VLIB_BUFFER_IS_TRACED))) {
            nat44_ei_hairpinning_handoff_trace_t *t = vlib_add_trace(vm, node, b[0], sizeof(*t));
    ...
    n_enq = vlib_buffer_enqueue_to_thread(vm, node, fq_index, from, thread_indices, frame->n_vectors, 1);

    if (n_enq < frame->n_vectors)
        vlib_node_increment_counter(vm, node->node_index, NAT44_EI_HAIRPINNING_HANDOFF_ERROR_CONGESTION_DROP, frame->n_vectors - n_enq);
    return frame->n_vectors;
}

static_always_inline uword
nat44_ei_in2out_node_fn_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_slow_path, int is_output_feature)
{
    ...
    while (n_left_from >= 2) {
        ...
        rx_fib_index0     = vec_elt(nm->ip4_main->fib_index_by_sw_if_index, rx_sw_if_index0);

        next0 = next1 = NAT44_EI_IN2OUT_NEXT_LOOKUP;

        if (PREDICT_FALSE(!is_output_feature && ip0->ttl == 1)) {
            vnet_buffer(b0)->sw_if_index[VLIB_TX] = (u32) ~0;
            icmp4_error_set_vnet_buffer(b0, ICMP4_time_exceeded, ICMP4_time_exceeded_ttl_exceeded_in_transit, 0);
        ...
        if (is_slow_path) {
            if (PREDICT_FALSE(proto0 == NAT_PROTOCOL_OTHER)) {
                if (nat_in2out_sm_unknown_proto(nm, b0, ip0, rx_fib_index0)) {
                    next0     = NAT44_EI_IN2OUT_NEXT_DROP;
                    b0->error = node->errors[NAT44_EI_IN2OUT_ERROR_UNSUPPORTED_PROTOCOL];
                }
                vlib_increment_simple_counter(is_slow_path ? &nm->counters.slowpath.in2out.other : &nm->counters.fastpath.in2out.other,
                                              thread_index,
                                              cntr_sw_if_index0,
                                              1);
                goto trace00;
            ...
            if (PREDICT_FALSE(proto0 == NAT_PROTOCOL_ICMP)) {
                next0 = nat44_ei_icmp_in2out_slow_path(nm, b0, ip0, icmp0, rx_sw_if_index0, rx_fib_index0, node, next0, now, thread_index, &s0);
                vlib_increment_simple_counter(is_slow_path ? &nm->counters.slowpath.in2out.icmp : &nm->counters.fastpath.in2out.icmp,
                                              thread_index,
                                              cntr_sw_if_index0,
                                              1);
                goto trace00;
        ...
        init_nat_k(&kv0, ip0->src_address, vnet_buffer(b0)->ip.reass.l4_src_port, rx_fib_index0, proto0);
        if (PREDICT_FALSE(clib_bihash_search_8_8(&nm->in2out, &kv0, &value0) != 0)) {
            if (is_slow_path) {
                if (is_output_feature) {
                    if (PREDICT_FALSE(nat44_ei_not_translate_output_feature(nm,
                                                                            ip0,
                                                                            proto0,
                    ...
                    if (PREDICT_FALSE(proto0 == NAT_PROTOCOL_UDP
                                      && (vnet_buffer(b0)->ip.reass.l4_dst_port == clib_host_to_net_u16(UDP_DST_PORT_dhcp_to_server))
                                      && ip0->dst_address.as_u32 == 0xffffffff))
                        goto trace00;
                }
                else {
                    if (PREDICT_FALSE(nat44_ei_not_translate(nm, node, rx_sw_if_index0, ip0, proto0, rx_fib_index0, thread_index))) goto trace00;
                }
                ...
                next0 = slow_path(nm,
                                  ...
                                  thread_index,
                                  now);
                if (PREDICT_FALSE(next0 == NAT44_EI_IN2OUT_NEXT_DROP)) goto trace00;
            ...
            s0 = pool_elt_at_index(nm->per_thread_data[thread_index].sessions, nat_value_get_session_index(&value0));
        ...
        if (!is_output_feature) vnet_buffer(b0)->sw_if_index[VLIB_TX] = s0->out2in.fib_index;
        ...
        if (PREDICT_TRUE(proto0 == NAT_PROTOCOL_TCP)) {
            if (!vnet_buffer(b0)->ip.reass.is_non_first_fragment) {
                ...
                sum0                       = tcp0->checksum;
                sum0                       = ip_csum_update(sum0, old_addr0, new_addr0, ip4_header_t, dst_address /* changed member */);
                sum0                       = ip_csum_update(sum0, old_port0, new_port0, ip4_header_t /* cheat */, length /* changed member */);
                mss_clamping(nm->mss_clamping, tcp0, &sum0);
                tcp0->checksum = ip_csum_fold(sum0);
            }
            vlib_increment_simple_counter(is_slow_path ? &nm->counters.slowpath.in2out.tcp : &nm->counters.fastpath.in2out.tcp,
                                          thread_index,
                                          cntr_sw_if_index0,
                                          1);
        }
            ...
            vlib_increment_simple_counter(is_slow_path ? &nm->counters.slowpath.in2out.udp : &nm->counters.fastpath.in2out.udp,
                                          thread_index,
                                          cntr_sw_if_index0,
                                          1);
        }
    ...
    trace00:
        ...
        if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
            nat44_ei_in2out_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
            t->is_slow_path            = is_slow_path;
            t->sw_if_index             = rx_sw_if_index0;
            t->next_index              = next0;
            t->session_index           = ~0;
            if (s0) t->session_index = s0 - nm->per_thread_data[thread_index].sessions;
        }

        if (next0 == NAT44_EI_IN2OUT_NEXT_DROP) {
            vlib_increment_simple_counter(is_slow_path ? &nm->counters.slowpath.in2out.drops : &nm->counters.fastpath.in2out.drops,
                                          thread_index,
                                          cntr_sw_if_index0,
                                          1);
        }
        ...
        ip1 = (ip4_header_t *) ((u8 *) vlib_buffer_get_current(b1) + iph_offset1);
        ...
        cntr_sw_if_index1 = is_output_feature ? tx_sw_if_index1 : rx_sw_if_index1;
        rx_fib_index1     = vec_elt(nm->ip4_main->fib_index_by_sw_if_index, rx_sw_if_index1);

        if (PREDICT_FALSE(!is_output_feature && ip1->ttl == 1)) {
            vnet_buffer(b1)->sw_if_index[VLIB_TX] = (u32) ~0;
            icmp4_error_set_vnet_buffer(b1, ICMP4_time_exceeded, ICMP4_time_exceeded_ttl_exceeded_in_transit, 0);
        ...
        if (is_slow_path) {
            if (PREDICT_FALSE(proto1 == NAT_PROTOCOL_OTHER)) {
                if (nat_in2out_sm_unknown_proto(nm, b1, ip1, rx_fib_index1)) {
                    ...
                    b1->error = node->errors[NAT44_EI_IN2OUT_ERROR_UNSUPPORTED_PROTOCOL];
                }
                vlib_increment_simple_counter(is_slow_path ? &nm->counters.slowpath.in2out.other : &nm->counters.fastpath.in2out.other,
                                              thread_index,
                                              cntr_sw_if_index1,
            ...
            if (PREDICT_FALSE(proto1 == NAT_PROTOCOL_ICMP)) {
                next1 = nat44_ei_icmp_in2out_slow_path(nm, b1, ip1, icmp1, rx_sw_if_index1, rx_fib_index1, node, next1, now, thread_index, &s1);
                vlib_increment_simple_counter(is_slow_path ? &nm->counters.slowpath.in2out.icmp : &nm->counters.fastpath.in2out.icmp,
                                              thread_index,
                                              cntr_sw_if_index1,
                                              1);
        ...
        init_nat_k(&kv1, ip1->src_address, vnet_buffer(b1)->ip.reass.l4_src_port, rx_fib_index1, proto1);
        if (PREDICT_FALSE(clib_bihash_search_8_8(&nm->in2out, &kv1, &value1) != 0)) {
            if (is_slow_path) {
                if (is_output_feature) {
                    if (PREDICT_FALSE(nat44_ei_not_translate_output_feature(nm,
                                                                            ip1,
                     ...
                     * be able to use dhcp client on the outside interface
                     */
                    if (PREDICT_FALSE(proto1 == NAT_PROTOCOL_UDP
                                      && (vnet_buffer(b1)->ip.reass.l4_dst_port == clib_host_to_net_u16(UDP_DST_PORT_dhcp_to_server))
                                      && ip1->dst_address.as_u32 == 0xffffffff))
                        goto trace01;
                }
                else {
                    if (PREDICT_FALSE(nat44_ei_not_translate(nm, node, rx_sw_if_index1, ip1, proto1, rx_fib_index1, thread_index))) goto trace01;
                }
                ...
                next1 = slow_path(nm,
                                  ...
                                  thread_index,
                                  now);
                if (PREDICT_FALSE(next1 == NAT44_EI_IN2OUT_NEXT_DROP)) goto trace01;

                if (PREDICT_FALSE(!s1)) goto trace01;
            }
            else {
                next1 = NAT44_EI_IN2OUT_NEXT_SLOW_PATH;
                goto trace01;
            ...
            s1 = pool_elt_at_index(nm->per_thread_data[thread_index].sessions, nat_value_get_session_index(&value1));
        ...
        if (!is_output_feature) vnet_buffer(b1)->sw_if_index[VLIB_TX] = s1->out2in.fib_index;

        sum1          = ip1->checksum;
        sum1          = ip_csum_update(sum1, old_addr1, new_addr1, ip4_header_t, src_address /* changed member */);
        ...
        if (PREDICT_TRUE(proto1 == NAT_PROTOCOL_TCP)) {
            if (!vnet_buffer(b1)->ip.reass.is_non_first_fragment) {
                ...
                sum1                       = ip_csum_update(sum1, old_port1, new_port1, ip4_header_t /* cheat */, length /* changed member */);
        ...
        /* Accounting */
        nat44_ei_session_update_counters(s1, now, vlib_buffer_length_in_chain(vm, b1), thread_index);
        /* Per-user LRU list maintenance */
        nat44_ei_session_update_lru(nm, s1, thread_index);
    trace01:

        if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b1->flags & VLIB_BUFFER_IS_TRACED))) {
            nat44_ei_in2out_trace_t *t = vlib_add_trace(vm, node, b1, sizeof(*t));
            t->sw_if_index             = rx_sw_if_index1;
            t->next_index              = next1;
            t->session_index           = ~0;
            if (s1) t->session_index = s1 - nm->per_thread_data[thread_index].sessions;
        }
        ...
        if (clib_bihash_search_8_8(&nm->in2out, &kv0, &value0)) {
            if (is_slow_path) {
                if (is_output_feature) {
                    ...
                    if (PREDICT_FALSE(nat44_ei_not_translate(nm, node, rx_sw_if_index0, ip0, proto0, rx_fib_index0, thread_index))) goto trace0;
                }
                ...
                if (PREDICT_FALSE(next0 == NAT44_EI_IN2OUT_NEXT_DROP)) goto trace0;

                if (PREDICT_FALSE(!s0)) goto trace0;
            }
            else {
                next0 = NAT44_EI_IN2OUT_NEXT_SLOW_PATH;
                goto trace0;
...
static_always_inline uword nat44_ei_in2out_hairpinning_finish_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            switch (proto0) {
                ...
                case NAT_PROTOCOL_UDP:
                    is_hairpinning =
                        nat44_ei_hairpinning(vm, node, nm, thread_index, b0, ip0, udp0, tcp0, proto0, 0 /* do_trace */, &required_thread_index);
                    break;
                case NAT_PROTOCOL_ICMP:
                    is_hairpinning = (0 == nat44_ei_icmp_hairpinning(nm, b0, thread_index, ip0, icmp0, &required_thread_index));
                    break;
            ...
            if (next0 != NAT44_EI_IN2OUT_HAIRPINNING_FINISH_NEXT_DROP) {
                vlib_increment_simple_counter(&nm->counters.fastpath.in2out.other, sw_if_index0, vm->thread_index, 1);
            }
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (nat44_ei_hairpinning(vm, node, nm, thread_index, b0, ip0, udp0, tcp0, proto0, 1, &required_thread_index)) {
            ...
            if (!next0_resolved) vnet_get_config_data(&cm->config_main, &b0->current_config_index, &next0, 0);

            if (next0 != NAT44_EI_HAIRPIN_NEXT_DROP) {
                vlib_increment_simple_counter(&nm->counters.hairpinning, vm->thread_index, sw_if_index0, 1);
            }
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return nat44_ei_hairpinning_handoff_fn_inline(vm, node, frame, nat44_ei_main.in2out_hairpinning_finish_ip4_lookup_node_fq_index);
}

VLIB_NODE_FN(nat44_ei_in2out_hairpinning_handoff_interface_output_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return nat44_ei_hairpinning_handoff_fn_inline(vm, node, frame, nat44_ei_main.in2out_hairpinning_finish_interface_output_node_fq_index);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return nat44_ei_hairpinning_handoff_fn_inline(vm, node, frame, nat44_ei_main.hairpinning_fq_index);
}
