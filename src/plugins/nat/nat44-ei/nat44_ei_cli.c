...
{
    nat44_ei_main_per_thread_data_t *tnm  = va_arg(*args, nat44_ei_main_per_thread_data_t *);
    nat44_ei_session_t              *sess = va_arg(*args, nat44_ei_session_t *);

    if (nat44_ei_is_unk_proto_session(sess)) {
        s = format(s, "  i2o %U proto %u fib %u\n", format_ip4_address, &sess->in2out.addr, sess->in2out.port, sess->in2out.fib_index);
        s = format(s, "  o2i %U proto %u fib %u\n", format_ip4_address, &sess->out2in.addr, sess->out2in.port, sess->out2in.fib_index);
    }
    else {
        s = format(s,
...
u8 *format_nat44_ei_static_mapping(u8 *s, va_list *args)
{
    ...
    if (is_sm_identity_nat(m->flags)) {
        if (is_sm_addr_only(m->flags))
            s = format(s, "identity mapping %U", format_ip4_address, &m->local_addr);
        else
            s = format(s,
    ...
    if (is_sm_addr_only(m->flags)) {
        s = format(s, "local %U external %U vrf %d", format_ip4_address, &m->local_addr, format_ip4_address, &m->external_addr, m->vrf_id);
    }
    else {
        s = format(s,
...
{
    nat44_ei_static_map_resolve_t *m   = va_arg(*args, nat44_ei_static_map_resolve_t *);
    vnet_main_t                   *vnm = vnet_get_main();

    if (is_sm_addr_only(m->flags))
        s = format(s, "local %U external %U vrf %d", format_ip4_address, &m->l_addr, format_vnet_sw_if_index_name, vnm, m->sw_if_index, m->vrf_id);
    else
        s = format(s,
                   "%U local %U:%d external %U:%d vrf %d",
                   format_nat_protocol,
...
static clib_error_t *nat44_ei_enable_disable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!unformat_user(input, unformat_line_input, line_input)) return clib_error_return(0, NAT44_EI_EXPECTED_ARGUMENT);
    ...
    if (enable) {
        ...
        if (nat44_ei_plugin_enable(c) != 0) error = clib_error_return(0, "enable failed");
    }
    else {
        if (!nm->enabled) {
        ...
        if (nat44_ei_plugin_disable() != 0) error = clib_error_return(0, "disable failed");
    }
...
static clib_error_t *nat_show_workers_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (nm->num_workers > 1) {
        ...
        vec_foreach (worker, nm->workers) {
            vlib_worker_thread_t *w = vlib_worker_threads + *worker + nm->first_worker_index;
...
static clib_error_t *nat44_ei_show_hash_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vlib_cli_output(vm, "%U", format_bihash_8_8, &nm->static_mapping_by_local, verbose);
    vlib_cli_output(vm, "%U", format_bihash_8_8, &nm->static_mapping_by_external, verbose);
...
static clib_error_t *nat44_ei_set_alloc_addr_and_port_alg_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(line_input, "map-e psid %d psid-offset %d psid-len %d", &psid, &psid_offset, &psid_length))
            nat44_ei_set_alloc_mape((u16) psid, (u16) psid_offset, (u16) psid_length);
        else if (unformat(line_input, "port-range %d - %d", &port_start, &port_end)) {
            if (port_end <= port_start) {
                error = clib_error_return(0, "The end-port must be greater than start-port");
                goto done;
            }
...
static clib_error_t *nat44_ei_show_alloc_addr_and_port_alg_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vlib_cli_output(vm, "NAT address and port: %U", format_nat44_ei_addr_and_port_alloc_alg, nm->addr_and_port_alloc_alg);
    switch (nm->addr_and_port_alloc_alg) {
        case NAT44_EI_ADDR_AND_PORT_ALLOC_ALG_MAPE:
            vlib_cli_output(vm, "  psid %d psid-offset %d psid-len %d", nm->psid, nm->psid_offset, nm->psid_length);
            break;
        case NAT44_EI_ADDR_AND_PORT_ALLOC_ALG_RANGE:
            vlib_cli_output(vm, "  start-port %d end-port %d", nm->start_port, nm->end_port);
            break;
        default:
            break;
    }
...
static clib_error_t *nat_ha_failover_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%U:%u", unformat_ip4_address, &addr, &port))
            ;
        else if (unformat(line_input, "refresh-interval %u", &session_refresh_interval))
            ;
        else {
            error = clib_error_return(0, "unknown input '%U'", format_unformat_error, line_input);
            goto done;
    ...
    if (rv) error = clib_error_return(0, "set HA failover failed");
...
static clib_error_t *nat_ha_listener_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (rv) error = clib_error_return(0, "set HA listener failed");
...
static clib_error_t *nat_show_ha_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (port)
        vlib_cli_output(vm, "  %U:%u refresh-interval %usec\n", format_ip4_address, &addr, port, session_refresh_interval);
    else
...
static clib_error_t *nat_ha_resync_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (nat_ha_resync(0, 0, 0)) error = clib_error_return(0, "NAT HA resync already running");
...
static clib_error_t *add_address_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (count > 1024) nat44_ei_log_info("%U - %U, %d addresses...", format_ip4_address, &start_addr, format_ip4_address, &end_addr, count);
    ...
    for (i = 0; i < count; i++) {
        ...
        if (nm->out2in_dpo) nat44_ei_add_del_address_dpo(this_addr, is_add);
...
#define _(N, i, n, s) vlib_cli_output(vm, "  %d busy %s ports", ap->busy_ports[i], s);
...
static clib_error_t *nat44_ei_show_interfaces_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    pool_foreach (i, nm->interfaces) {
        vlib_cli_output(
            ...
            vnm,
            i->sw_if_index,
            (nat44_ei_interface_is_inside(i) && nat44_ei_interface_is_outside(i)) ? "in out" : (nat44_ei_interface_is_inside(i) ? "in" : "out"));
    }
...
static clib_error_t *add_static_mapping_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (is_add) {
        rv = nat44_ei_add_static_mapping(l_addr, e_addr, l_port, e_port, proto, vrf_id, sw_if_index, flags, pool_addr, 0);
    }
    else {
        rv = nat44_ei_del_static_mapping(l_addr, e_addr, l_port, e_port, proto, vrf_id, sw_if_index, flags);
    }
...
static clib_error_t *add_identity_mapping_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%U", unformat_ip4_address, &addr))
            ...
            ;
        else if (unformat(line_input, "%U %u", unformat_nat_protocol, &proto, &port)) {
            port_set = 1;
        }
        else if (unformat(line_input, "del")) {
            is_add = 0;
    ...
    if (is_add) {
        rv = nat44_ei_add_static_mapping(addr, addr, port, port, proto, vrf_id, sw_if_index, flags, addr, 0);
    }
    else {
        rv = nat44_ei_del_static_mapping(addr, addr, port, port, proto, vrf_id, sw_if_index, flags);
    }
...
static clib_error_t *nat44_ei_add_interface_address_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%U", unformat_vnet_sw_interface, nm->vnet_main, &sw_if_index))
            ;
        else if (unformat(line_input, "del")) {
            is_del = 1;
        }
...
static clib_error_t *nat44_ei_show_sessions_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!unformat_user(input, unformat_line_input, line_input)) goto print;

    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "detail"))
            detail = 1;
        else if (unformat(line_input, "filter saddr %U", unformat_ip4_address, &saddr))
            filter_saddr = 1;
        else {
            error = clib_error_return(0, "unknown input '%U'", format_unformat_error, line_input);
            break;
...
print:
    ...
    vec_foreach_index (i, nm->per_thread_data) {
        ...
        vlib_cli_output(vm, "-------- thread %d %s: %d sessions --------\n", i, vlib_worker_threads[i].name, pool_elts(tnm->sessions));

        nat44_ei_user_t *u;
        pool_foreach (u, tnm->users) {
            if (filter_saddr && saddr.as_u32 != u->addr.as_u32) continue;
...
static clib_error_t *nat44_ei_del_session_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%U:%u %U", unformat_ip4_address, &addr, &port, unformat_nat_protocol, &proto))
            ;
    ...
    rv = nat44_ei_del_session(nm, &addr, clib_host_to_net_u16(port), proto, vrf_id, is_in);
...
static clib_error_t *set_timeout_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "udp %u", &nm->timeouts.udp))
            ;
        else if (unformat(line_input, "tcp-established %u", &nm->timeouts.tcp.established))
            ;
        else if (unformat(line_input, "tcp-transitory %u", &nm->timeouts.tcp.transitory))
            ;
...
static clib_error_t *nat_show_timeouts_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vlib_cli_output(vm, "tcp-established timeout: %dsec", nm->timeouts.tcp.established);
    vlib_cli_output(vm, "tcp-transitory timeout: %dsec", nm->timeouts.tcp.transitory);
...
VLIB_CLI_COMMAND(nat44_ei_ipfix_logging_enable_disable_command, static) = {
    ...
    .short_help =
        "nat44 ei ipfix logging <enable [domain <domain-id>] "
...
VLIB_CLI_COMMAND(nat_ha_failover_command, static) = {
    ...
    .short_help = "nat44 ei ha failover <ip4-address>:<port> [refresh-interval <sec>]",
...
VLIB_CLI_COMMAND(nat_ha_listener_command, static) = {
    ...
    .short_help = "nat44 ei ha listener <ip4-address>:<port> [path-mtu <path-mtu>]",
...
VLIB_CLI_COMMAND(add_static_mapping_command, static) = {
    ...
    .short_help =
        "nat44 ei add static mapping tcp|udp|icmp local <addr> "
        "[<port|icmp-echo-id>] "
...
VLIB_CLI_COMMAND(nat44_ei_del_session_command, static) = {
    ...
    .short_help =
        "nat44 ei del session in|out <addr>:<port> tcp|udp|icmp [vrf "
