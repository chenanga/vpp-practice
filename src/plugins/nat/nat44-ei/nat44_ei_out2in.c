...
static u8 *format_nat44_ei_out2in_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "NAT44_OUT2IN: sw_if_index %d, next index %d, session index %d", t->sw_if_index, t->next_index, t->session_index);
    return s;
}
...
int nat44_o2i_is_idle_session_cb(clib_bihash_kv_8_8_t *kv, void *arg)
{
    ...
    if (ctx->now >= sess_timeout_time) {
        ...
        if (clib_bihash_add_del_8_8(&nm->in2out, &s_kv, 0)) nat_elog_warn(nm, "out2in key del failed");
 ...
 */
static inline nat44_ei_session_t *create_session_for_static_mapping(nat44_ei_main_t     *nm,
                                                                    vlib_buffer_t       *b0,
                                    ...
                                    u8                  *dont_translate)
{
    ...
    *proto = 0;
    ...
    if (clib_bihash_search_8_8(&nm->out2in, &kv0, &value0)) {
        ...
        if (nat44_ei_static_mapping_match(*addr,
                                          *port,
                                          *fib_index,
                                          *proto,
                                          &mapping_addr,
            ...
            if (!nm->forwarding_enabled) {
                ...
                if (PREDICT_FALSE(nat44_ei_is_interface_addr(nm->ip4_main, node, sw_if_index0, ip0->dst_address.as_u32))) {
                    *dont_translate = 1;
                    goto out;
                }
        ...
        if (PREDICT_FALSE(vnet_buffer(b0)->ip.reass.icmp_type_or_tcp_flags != ICMP4_echo_reply
                          && (vnet_buffer(b0)->ip.reass.icmp_type_or_tcp_flags != ICMP4_echo_request || !is_addr_only))) {
        ...
        s0 = create_session_for_static_mapping(nm,
                                               b0,
                                               mapping_addr,
                                               mapping_port,
                                               mapping_fib_index,
                                    ...
                                    u8                  *dont_translate)
{
    ...
    *dont_translate = 0;
    ...
    if (nat44_ei_static_mapping_match(addr, port, rx_fib_index0, *proto, mapping_addr, mapping_port, mapping_fib_index, 1, &is_addr_only, 0)) {
        /* Don't NAT packet aimed at the intfc address */
        if (nat44_ei_is_interface_addr(nm->ip4_main, node, sw_if_index0, ip0->dst_address.as_u32)) {
            *dont_translate = 1;
            goto out;
        }
        b0->error = node->errors[NAT44_EI_OUT2IN_ERROR_NO_TRANSLATION];
                         ...
                         nat44_ei_session_t **p_s0)
{
    ...
    if (PREDICT_TRUE(nm->pat)) {
        next0_tmp = nat44_ei_icmp_match_out2in_slow(node, thread_index, b0, ip0, &addr, &port, &fib_index, &proto, p_s0, &dont_translate);
    }
    else {
        next0_tmp = nat44_ei_icmp_match_out2in_fast(node, thread_index, b0, ip0, &addr, &port, &fib_index, &proto, p_s0, &dont_translate);
    }

    if (next0_tmp != ~0) next0 = next0_tmp;
    if (next0 == NAT44_EI_OUT2IN_NEXT_DROP || dont_translate) goto out;
    ...
    if (!vnet_buffer(b0)->ip.reass.is_non_first_fragment) {
        ...
        if (!icmp_type_is_error_message(icmp0->type)) {
            ...
            if (PREDICT_FALSE(new_id0 != echo0->identifier)) {
                ...
                sum0            = icmp0->checksum;
                sum0            = ip_csum_update(sum0, old_id0, new_id0, icmp_echo_header_t, identifier /* changed member */);
            ...
            switch (proto) {
                ...
                case NAT_PROTOCOL_TCP:
                    ...
                    sum0            = icmp0->checksum;
                    sum0            = ip_csum_update(sum0, old_id0, new_id0, tcp_udp_header_t, src_port);
                    icmp0->checksum = ip_csum_fold(sum0);
                    break;
                default:
                    ASSERT(0);
...
static inline u32 nat44_ei_icmp_out2in_slow_path(nat44_ei_main_t     *nm,
                                                 vlib_buffer_t       *b0,
                                                 ip4_header_t        *ip0,
                                                 icmp46_header_t     *icmp0,
                                                 u32                  sw_if_index0,
                                                 ...
                                                 nat44_ei_session_t **p_s0)
{
    ...
    next0                  = nat44_ei_icmp_out2in(b0, ip0, icmp0, sw_if_index0, rx_fib_index0, node, next0, thread_index, p_s0);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from >= 2) {
        ...
        sw_if_index0  = vnet_buffer(b0)->sw_if_index[VLIB_RX];
        rx_fib_index0 = vec_elt(nm->ip4_main->fib_index_by_sw_if_index, sw_if_index0);
        ...
        if (PREDICT_FALSE(proto0 == NAT_PROTOCOL_OTHER)) {
            ...
            vlib_increment_simple_counter(&nm->counters.slowpath.out2in.other, thread_index, sw_if_index0, 1);

            goto trace0;
        }

        if (PREDICT_FALSE(proto0 == NAT_PROTOCOL_ICMP)) {
            next0 = nat44_ei_icmp_out2in_slow_path(nm, b0, ip0, icmp0, sw_if_index0, rx_fib_index0, node, next0, now, thread_index, &s0);
            vlib_increment_simple_counter(&nm->counters.slowpath.out2in.icmp, thread_index, sw_if_index0, 1);
            goto trace0;
        }

        init_nat_k(&kv0, ip0->dst_address, vnet_buffer(b0)->ip.reass.l4_dst_port, rx_fib_index0, proto0);
        if (clib_bihash_search_8_8(&nm->out2in, &kv0, &value0)) {
            /* Try to match static mapping by external address and port,
               destination address and port in packet */
        ...
        if (PREDICT_TRUE(proto0 == NAT_PROTOCOL_TCP)) {
            ...
            vlib_increment_simple_counter(&nm->counters.slowpath.out2in.tcp, thread_index, sw_if_index0, 1);
        }
        else {
            if (!vnet_buffer(b0)->ip.reass.is_non_first_fragment) {
                old_port0 = vnet_buffer(b0)->ip.reass.l4_dst_port;
            ...
            vlib_increment_simple_counter(&nm->counters.slowpath.out2in.udp, thread_index, sw_if_index0, 1);
        }
    ...
    trace0:
        ...
        if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
            nat44_ei_out2in_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
        ...
        if (next0 == NAT44_EI_OUT2IN_NEXT_DROP) {
            vlib_increment_simple_counter(&nm->counters.slowpath.out2in.drops, thread_index, sw_if_index0, 1);
        }
        ...
        rx_fib_index1 = vec_elt(nm->ip4_main->fib_index_by_sw_if_index, sw_if_index1);
        ...
        if (PREDICT_FALSE(proto1 == NAT_PROTOCOL_OTHER)) {
            if (nat_out2in_sm_unknown_proto(nm, b1, ip1, rx_fib_index1)) {
                if (!nm->forwarding_enabled) {
                    b1->error = node->errors[NAT44_EI_OUT2IN_ERROR_UNSUPPORTED_PROTOCOL];
                    next1     = NAT44_EI_OUT2IN_NEXT_DROP;
                }
            }
            vlib_increment_simple_counter(&nm->counters.slowpath.out2in.other, thread_index, sw_if_index1, 1);
            goto trace1;
        }

        if (PREDICT_FALSE(proto1 == NAT_PROTOCOL_ICMP)) {
            next1 = nat44_ei_icmp_out2in_slow_path(nm, b1, ip1, icmp1, sw_if_index1, rx_fib_index1, node, next1, now, thread_index, &s1);
            vlib_increment_simple_counter(&nm->counters.slowpath.out2in.icmp, thread_index, sw_if_index1, 1);
            goto trace1;
        }

        init_nat_k(&kv1, ip1->dst_address, vnet_buffer(b1)->ip.reass.l4_dst_port, rx_fib_index1, proto1);
        if (clib_bihash_search_8_8(&nm->out2in, &kv1, &value1)) {
            /* Try to match static mapping by external address and port,
               destination address and port in packet */
            if (nat44_ei_static_mapping_match(ip1->dst_address,
                ...
                if (PREDICT_FALSE(proto1 == NAT_PROTOCOL_UDP
                                  && (vnet_buffer(b1)->ip.reass.l4_dst_port == clib_host_to_net_u16(UDP_DST_PORT_dhcp_to_client)))) {
                    vnet_feature_next(&next1, b1);
                    goto trace1;
                }

                if (!nm->forwarding_enabled) {
                    b1->error = node->errors[NAT44_EI_OUT2IN_ERROR_NO_TRANSLATION];
                    next1     = NAT44_EI_OUT2IN_NEXT_DROP;
                }
                goto trace1;
            }

            if (PREDICT_FALSE(identity_nat1)) goto trace1;

            /* Create session initiated by host from external network */
            s1 = create_session_for_static_mapping(nm,
                                                   b1,
                                                   sm_addr1,
                                                   sm_port1,
                                                   sm_fib_index1,
        ...
        if (PREDICT_TRUE(proto1 == NAT_PROTOCOL_TCP)) {
            ...
            vlib_increment_simple_counter(&nm->counters.slowpath.out2in.tcp, thread_index, sw_if_index1, 1);
        }
        else {
            if (!vnet_buffer(b1)->ip.reass.is_non_first_fragment) {
                old_port1 = vnet_buffer(b1)->ip.reass.l4_dst_port;
            ...
            vlib_increment_simple_counter(&nm->counters.slowpath.out2in.udp, thread_index, sw_if_index1, 1);
        }
    ...
    trace1:
        ...
        if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b1->flags & VLIB_BUFFER_IS_TRACED))) {
            nat44_ei_out2in_trace_t *t = vlib_add_trace(vm, node, b1, sizeof(*t));
        ...
        if (next1 == NAT44_EI_OUT2IN_NEXT_DROP) {
            vlib_increment_simple_counter(&nm->counters.slowpath.out2in.drops, thread_index, sw_if_index1, 1);
        }
    ...
    while (n_left_from > 0) {
        ...
        if (clib_bihash_search_8_8(&nm->out2in, &kv0, &value0)) {
            ...
            if (PREDICT_FALSE(identity_nat0)) goto trace00;
