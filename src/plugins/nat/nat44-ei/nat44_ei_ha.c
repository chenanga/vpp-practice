                                           ...
                                           u32            thread_index)
{
    ...
    if (!(flags & NAT44_EI_SESSION_FLAG_STATIC_MAPPING)) {
        if (nat44_ei_set_outside_address_and_port(nm->addresses, thread_index, *out_addr, out_port, proto)) return;
    }
    ...
    if (clib_bihash_add_del_8_8(&nm->out2in, &kv, 1)) nat_elog_warn(nm, "out2in key add failed");
    ...
    if (clib_bihash_add_del_8_8(&nm->in2out, &kv, 1)) nat_elog_warn(nm, "in2out key add failed");
}

static_always_inline void
nat44_ei_ha_sdel(ip4_address_t *out_addr, u16 out_port, ip4_address_t *eh_addr, u16 eh_port, u8 proto, u32 fib_index, u32 thread_index)
{
    nat44_ei_main_t                 *nm = &nat44_ei_main;
    ...
    if (clib_bihash_search_8_8(&nm->out2in, &kv, &value)) return;
...
static_always_inline void nat44_ei_ha_sref(ip4_address_t *out_addr,
                                           u16            out_port,
                                           ip4_address_t *eh_addr,
                                           u16            eh_port,
                                           u8             proto,
...
static void nat_ha_resync_fin(void)
{
    ...
    if (ha->event_callback) ha->event_callback(ha->client_index, ha->pid, ha->resync_ack_missed);
}
...
static_always_inline void nat_ha_ack_recv(u32 seq, u32 thread_index)
{
    ...
    vec_foreach_index (i, td->resend_queue) {
        if (td->resend_queue[i].seq != seq) continue;

        vlib_increment_simple_counter(&ha->counters[NAT_HA_COUNTER_RECV_ACK], thread_index, 0, 1);
        ...
        nat_elog_debug_X1(nm, "ACK for seq %d received", "i4", clib_net_to_host_u32(seq));
...
static void nat_ha_resend_scan(vlib_main_t *vm, u32 thread_index)
{
    ...
    vec_foreach_index (i, td->resend_queue) {
        if (td->resend_queue[i].retry_timer > now) continue;

        /* maximum retry reached delete cached data */
        if (td->resend_queue[i].retry_count >= NAT_HA_RETRIES) {
            nat_elog_notice_X1(nm, "seq %d missed", "i4", clib_net_to_host_u32(td->resend_queue[i].seq));
            if (td->resend_queue[i].is_resync) {
            ...
            vlib_increment_simple_counter(&ha->counters[NAT_HA_COUNTER_MISSED_COUNT], thread_index, 0, 1);
            continue;
        }

        /* retry to send non-ACKed data */
        nat_elog_debug_X1(nm, "state sync seq %d resend", "i4", clib_net_to_host_u32(td->resend_queue[i].seq));
        td->resend_queue[i].retry_count++;
        vlib_increment_simple_counter(&ha->counters[NAT_HA_COUNTER_RETRY_COUNT], thread_index, 0, 1);
        ...
        clib_memcpy(ip, td->resend_queue[i].data, vec_len(td->resend_queue[i].data));
...
int nat_ha_set_listener(vlib_main_t *vm, ip4_address_t *addr, u16 port, u32 path_mtu)
{
    ...
    if (ha->src_port) udp_unregister_dst_port(vm, ha->src_port, 1);
    ...
    if (port) {
        ...
        if (ha->num_workers > 1) {
            if (ha->fq_index == ~0) ha->fq_index = vlib_frame_queue_main_init(ha->ha_node_index, 0);
            udp_register_dst_port(vm, port, ha->ha_handoff_node_index, 1);
        }
        else {
            udp_register_dst_port(vm, port, ha->ha_node_index, 1);
        }
        nat_elog_info_X1(nm, "HA listening on port %d for state sync", "i4", port);
    }
...
static_always_inline void nat_ha_recv_add(nat_ha_event_t *event, f64 now, u32 thread_index)
{
    ...
    vlib_increment_simple_counter(&ha->counters[NAT_HA_COUNTER_RECV_ADD], thread_index, 0, 1);
...
static_always_inline void nat_ha_recv_del(nat_ha_event_t *event, u32 thread_index)
{
    ...
    vlib_increment_simple_counter(&ha->counters[NAT_HA_COUNTER_RECV_DEL], thread_index, 0, 1);

    out_addr.as_u32 = event->out_addr;
    eh_addr.as_u32  = event->eh_addr;
    fib_index       = clib_net_to_host_u32(event->fib_index);

    nat44_ei_ha_sdel(&out_addr, event->out_port, &eh_addr, event->eh_port, event->protocol, fib_index, thread_index);
}
...
static_always_inline void nat_ha_recv_refresh(nat_ha_event_t *event, f64 now, u32 thread_index)
{
    ...
    vlib_increment_simple_counter(&ha->counters[NAT_HA_COUNTER_RECV_REFRESH], thread_index, 0, 1);
    ...
    nat44_ei_ha_sref(&out_addr, event->out_port, &eh_addr, event->eh_port, event->protocol, fib_index, total_pkts, total_bytes, thread_index);
}
...
static_always_inline void nat_ha_event_process(nat_ha_event_t *event, f64 now, u32 thread_index)
{
    ...
    switch (event->event_type) {
        ...
        case NAT_HA_REFRESH:
            ...
            break;
        default:
            nat_elog_notice_X1(nm, "Unsupported HA event type %d", "i4", event->event_type);
            break;
    }
}

static inline void nat_ha_header_create(vlib_buffer_t *b, u32 *offset, u32 thread_index)
{
    nat_ha_main_t           *ha = &nat_ha_main;
    ...
    *offset = sizeof(ip4_header_t) + sizeof(udp_header_t) + sizeof(nat_ha_message_header_t);
}

static inline void nat_ha_send(vlib_frame_t *f, vlib_buffer_t *b, u8 is_resync, u32 vlib_thread_index)
{
    nat_ha_main_t            *ha = &nat_ha_main;
    nat_ha_per_thread_data_t *td = &ha->per_thread_data[vlib_thread_index];
    nat_ha_message_header_t  *h;
    ...
    nat_ha_resend_queue_add(vm, h->sequence_number, (u8 *) ip, b->current_length, is_resync, vlib_thread_index);
...
static_always_inline void nat_ha_event_add(nat_ha_event_t *event, u8 do_flush, u32 session_thread_index, u8 is_resync)
{
    ...
    if (PREDICT_FALSE(td->state_sync_count == 0)) nat_ha_header_create(b, &offset, session_thread_index);
    ...
    if (PREDICT_TRUE(do_flush == 0)) {
        ...
        switch (event->event_type) {
            case NAT_HA_ADD:
                vlib_increment_simple_counter(&ha->counters[NAT_HA_COUNTER_SEND_ADD], vlib_thread_index, 0, 1);
                break;
            case NAT_HA_DEL:
                vlib_increment_simple_counter(&ha->counters[NAT_HA_COUNTER_SEND_DEL], vlib_thread_index, 0, 1);
                break;
            case NAT_HA_REFRESH:
                vlib_increment_simple_counter(&ha->counters[NAT_HA_COUNTER_SEND_REFRESH], vlib_thread_index, 0, 1);
                break;
            default:
                break;
        }
    ...
    if (PREDICT_FALSE(do_flush || offset + (sizeof(*event)) > ha->state_sync_path_mtu)) {
...
#define skip_if_disabled()                       \
    do {                                         \
        nat_ha_main_t *ha = &nat_ha_main;        \
        if (PREDICT_TRUE(!ha->dst_port)) return; \
    } while (0)
                 ...
                 f64            now)
{
    ...
    if ((*last_refreshed + ha->session_refresh_interval) > now) return;
...
static uword nat_ha_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    if (event_type) nat_elog_info(nm, "nat44-ei-ha-process: bogus kickoff event received");
    ...
    while (1) {
        ...
        for (ti = 0; ti < vlib_get_n_threads(); ti++) {
            if (ti >= vec_len(ha->per_thread_data)) continue;

            vlib_node_set_interrupt_pending(vlib_get_main_by_index(ti), nat_ha_worker_node.index);
        }
    }
...
static u8 *format_nat_ha_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "nat44-ei-ha: %u events from %U", t->event_count, format_ip4_address, &t->addr);
...
static uword nat_ha_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            sum0          = ip_csum_update(sum0, old_len0, ip0->length, ip4_header_t, length);
            ...
            h0->count = 0;
            vlib_increment_simple_counter(&ha->counters[NAT_HA_COUNTER_SEND_ACK], thread_index, 0, 1);
        ...
        done0:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                ...
                ip4_header_t   *ip = (void *) (b0->data + vnet_buffer(b0)->l3_hdr_offset);
    ...
    vlib_node_increment_counter(vm, ha->ha_node_index, NAT_HA_ERROR_PROCESSED, pkts_processed);
...
static u8 *format_nat_ha_handoff_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "NAT_HA_WORKER_HANDOFF: next-worker %d", t->next_worker_index);
...
static uword nat_ha_handoff_node_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b[0]->flags & VLIB_BUFFER_IS_TRACED))) {
            nat_ha_handoff_trace_t *t = vlib_add_trace(vm, node, b[0], sizeof(*t));
    ...
    n_enq = vlib_buffer_enqueue_to_thread(vm, node, ha->fq_index, from, thread_indices, frame->n_vectors, 1);

    if (n_enq < frame->n_vectors) vlib_node_increment_counter(vm, node->node_index, NAT_HA_HANDOFF_ERROR_CONGESTION_DROP, frame->n_vectors - n_enq);
    vlib_node_increment_counter(vm, node->node_index, NAT_HA_HANDOFF_ERROR_SAME_WORKER, same_worker);
    vlib_node_increment_counter(vm, node->node_index, NAT_HA_HANDOFF_ERROR_DO_HANDOFF, do_handoff);
    return frame->n_vectors;
}
