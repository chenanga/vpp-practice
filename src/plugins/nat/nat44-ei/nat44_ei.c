...
extern vlib_node_registration_t nat44_ei_in2out_hairpinning_finish_ip4_lookup_node;
extern vlib_node_registration_t nat44_ei_in2out_hairpinning_finish_interface_output_node;

#define skip_if_disabled()                       \
    do {                                         \
        nat44_ei_main_t *nm = &nat44_ei_main;    \
        if (PREDICT_FALSE(!nm->enabled)) return; \
    } while (0)
...
static int nat44_ei_add_static_mapping_internal(ip4_address_t  l_addr,
...
static int nat44_ei_del_static_mapping_internal(ip4_address_t  l_addr,
                                                ip4_address_t  e_addr,
                                                u16            l_port,
                                                u16            e_port,
                                                nat_protocol_t proto,
...
always_inline bool nat44_ei_port_is_used(nat44_ei_address_t *a, u8 proto, u16 port)
{
    return clib_bitmap_get(a->busy_port_bitmap[proto], port);
}

always_inline void nat44_ei_port_get(nat44_ei_address_t *a, u8 proto, u16 port)
{
    ASSERT(!nat44_ei_port_is_used(a, proto, port));
    a->busy_port_bitmap[proto] = clib_bitmap_set(a->busy_port_bitmap[proto], port, 1);
}

always_inline void nat44_ei_port_put(nat44_ei_address_t *a, u8 proto, u16 port)
{
    ASSERT(nat44_ei_port_is_used(a, proto, port));
...
int nat44_ei_set_workers(uword *bitmap)
{
    ...
    if (nm->num_workers < 2) return VNET_API_ERROR_FEATURE_DISABLED;

    if (clib_bitmap_last_set(bitmap) >= nm->num_workers) return VNET_API_ERROR_INVALID_WORKER;
...
static void nat44_ei_add_del_addr_to_fib_foreach_out_if(ip4_address_t *addr, u8 is_add)
{
    nat44_ei_main_t      *nm = &nat44_ei_main;
...
static_always_inline void nat44_ei_add_del_addr_to_fib_foreach_addr(u32 sw_if_index, u8 is_add)
{
    nat44_ei_main_t    *nm = &nat44_ei_main;
...
static_always_inline void nat44_ei_add_del_addr_to_fib_foreach_addr_only_sm(u32 sw_if_index, u8 is_add)
{
    nat44_ei_main_t           *nm = &nat44_ei_main;
    nat44_ei_static_mapping_t *m;

    pool_foreach (m, nm->static_mappings) {
        if (is_sm_addr_only(m->flags) && !(m->local_addr.as_u32 == m->external_addr.as_u32)) {
            nat44_ei_add_del_addr_to_fib(&m->external_addr, 32, sw_if_index, is_add);
        }
    }
}
...
#define _(x)                                                                                      \
    nat_init_simple_counter(nm->counters.fastpath.in2out.x, #x, "/nat44-ei/in2out/fastpath/" #x); \
    nat_init_simple_counter(nm->counters.fastpath.out2in.x, #x, "/nat44-ei/out2in/fastpath/" #x); \
    nat_init_simple_counter(nm->counters.slowpath.in2out.x, #x, "/nat44-ei/in2out/slowpath/" #x); \
    nat_init_simple_counter(nm->counters.slowpath.out2in.x, #x, "/nat44-ei/out2in/slowpath/" #x);
    foreach_nat_counter;
#undef _
    nat_init_simple_counter(nm->counters.hairpinning, "hairpinning", "/nat44-ei/hairpinning");
    ...
    nm->fib_src_low = fib_source_allocate("nat44-ei-low", FIB_SOURCE_PRIORITY_LOW, FIB_SOURCE_BH_SIMPLE);
    nm->fib_src_hi  = fib_source_allocate("nat44-ei-hi", FIB_SOURCE_PRIORITY_HI, FIB_SOURCE_BH_SIMPLE);

    // used only by out2in-dpo feature
    nat_dpo_module_init();
    nat_ha_init(vm, nm->num_workers, num_threads);

    nm->hairpinning_fq_index                               = vlib_frame_queue_main_init(nat44_ei_hairpinning_node.index, 0);
    nm->in2out_hairpinning_finish_ip4_lookup_node_fq_index = vlib_frame_queue_main_init(nat44_ei_in2out_hairpinning_finish_ip4_lookup_node.index, 0);
    nm->in2out_hairpinning_finish_interface_output_node_fq_index =
        vlib_frame_queue_main_init(nat44_ei_in2out_hairpinning_finish_interface_output_node.index, 0);
...
int nat44_ei_plugin_enable(nat44_ei_config_t c)
{
    ...
    if (!c.users) c.users = 1024;

    if (!c.sessions) c.sessions = 10 * 1024;

    if (!c.user_sessions) c.user_sessions = c.sessions;

    nm->rconfig = c;

    if (!nm->frame_queue_nelts) nm->frame_queue_nelts = NAT_FQ_NELTS_DEFAULT;
    ...
    nm->pat = (!c.static_mapping_only || (c.static_mapping_only && c.connection_tracking));
    ...
    nm->inside_fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, c.inside_vrf, nm->fib_src_hi);

    nm->outside_vrf_id    = c.outside_vrf;
    nm->outside_fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, c.outside_vrf, nm->fib_src_hi);

    nat_reset_timeouts(&nm->timeouts);
    nat44_ei_db_init(nm->translations, nm->translation_buckets, nm->user_buckets);
    ...
    if (nm->num_workers > 1) {
        if (nm->fq_in2out_index == ~0) {
            nm->fq_in2out_index = vlib_frame_queue_main_init(nm->in2out_node_index, nm->frame_queue_nelts);
        }
        if (nm->fq_out2in_index == ~0) {
            nm->fq_out2in_index = vlib_frame_queue_main_init(nm->out2in_node_index, nm->frame_queue_nelts);
        }
        if (nm->fq_in2out_output_index == ~0) {
            nm->fq_in2out_output_index = vlib_frame_queue_main_init(nm->in2out_output_node_index, nm->frame_queue_nelts);
        }
    }
...
static_always_inline nat44_ei_outside_fib_t *nat44_ei_get_outside_fib(nat44_ei_outside_fib_t *outside_fibs, u32 fib_index)
{
...
static_always_inline nat44_ei_interface_t *nat44_ei_get_interface(nat44_ei_interface_t *interfaces, u32 sw_if_index)
{
    nat44_ei_interface_t *i;
...
static_always_inline int nat44_ei_hairpinning_enable(u8 is_enable)
{
    nat44_ei_main_t *nm          = &nat44_ei_main;
    ...
    if (is_enable) {
        nm->hairpin_reg += 1;
        if (1 == nm->hairpin_reg) {
            return vnet_feature_enable_disable("ip4-local", "nat44-ei-hairpinning", sw_if_index, is_enable, 0, 0);
        }
    }
    else {
        if (0 == nm->hairpin_reg) return 1;
...
int nat44_ei_add_interface(u32 sw_if_index, u8 is_inside)
{
    ...
    if (i) {
        if ((nat44_ei_interface_is_inside(i) && is_inside) || (nat44_ei_interface_is_outside(i) && !is_inside)) {
            return 0;
        }
        if (nm->num_workers > 1) {
            del_feature_name = !is_inside ? "nat44-ei-in2out-worker-handoff" : "nat44-ei-out2in-worker-handoff";
        ...
        rv = vnet_feature_enable_disable("ip4-unicast", del_feature_name, sw_if_index, 0, 0, 0);
        if (rv) {
            return rv;
        }
        rv = vnet_feature_enable_disable("ip4-unicast", feature_name, sw_if_index, 1, 0, 0);
        if (rv) {
            return rv;
        }
        if (!is_inside) {
        ...
        if (nm->num_workers > 1) {
            feature_name = is_inside ? "nat44-ei-in2out-worker-handoff" : "nat44-ei-out2in-worker-handoff";
        }
        else {
...
int nat44_ei_del_interface(u32 sw_if_index, u8 is_inside)
{
    ...
    if (nat44_ei_interface_is_inside(i) && nat44_ei_interface_is_outside(i)) {
        ...
        rv = vnet_feature_enable_disable("ip4-unicast", feature_name, sw_if_index, 0, 0, 0);
        if (rv) {
            return rv;
        }
        if (is_inside) {
...
int nat44_ei_add_output_interface(u32 sw_if_index)
{
    ...
    if (nm->num_workers > 1) {
        ...
        rv = vnet_feature_enable_disable("ip4-unicast", "nat44-ei-out2in-worker-handoff", sw_if_index, 1, 0, 0);
        if (rv) {
            return rv;
        }
        rv = vnet_feature_enable_disable("ip4-output", "nat44-ei-in2out-output-worker-handoff", sw_if_index, 1, 0, 0);
        if (rv) {
            return rv;
        }
    }
        ...
        rv = vnet_feature_enable_disable("ip4-unicast", "nat44-ei-out2in", sw_if_index, 1, 0, 0);
        if (rv) {
            return rv;
        }
        rv = vnet_feature_enable_disable("ip4-output", "nat44-ei-in2out-output", sw_if_index, 1, 0, 0);
        if (rv) {
            return rv;
        }
    }
...
int nat44_ei_del_output_interface(u32 sw_if_index)
{
    ...
    if (nm->num_workers > 1) {
        ...
        rv = vnet_feature_enable_disable("ip4-unicast", "nat44-ei-out2in-worker-handoff", sw_if_index, 0, 0, 0);
        if (rv) {
            return rv;
        }
        rv = vnet_feature_enable_disable("ip4-output", "nat44-ei-in2out-output-worker-handoff", sw_if_index, 0, 0, 0);
        if (rv) {
            return rv;
        }
    }
        ...
        rv = vnet_feature_enable_disable("ip4-unicast", "nat44-ei-out2in", sw_if_index, 0, 0, 0);
        if (rv) {
            return rv;
        }
        rv = vnet_feature_enable_disable("ip4-output", "nat44-ei-in2out-output", sw_if_index, 0, 0, 0);
        if (rv) {
            return rv;
        }
    }
...
int nat44_ei_del_output_interfaces()
{
    nat44_ei_main_t      *nm = &nat44_ei_main;
    nat44_ei_interface_t *i, *pool;
    int                   error = 0;
...
static clib_error_t *nat44_ei_sw_interface_add_del(vnet_main_t *vnm, u32 sw_if_index, u32 is_add)
{
    nat44_ei_main_t      *nm = &nat44_ei_main;
    nat44_ei_interface_t *i;
    int                   error = 0;
...
int nat44_ei_del_static_mappings()
{
    nat44_ei_main_t           *nm = &nat44_ei_main;
    ...
    pool_foreach (m, pool) {
        error =
            nat44_ei_del_static_mapping_internal(m->local_addr, m->external_addr, m->local_port, m->external_port, m->proto, m->vrf_id, ~0, m->flags);
        if (error) {
...
int nat44_ei_set_outside_address_and_port(nat44_ei_address_t *addresses, u32 thread_index, ip4_address_t addr, u16 port, nat_protocol_t protocol)
{
    ...
    for (address_index = 0; address_index < vec_len(addresses); address_index++) {
        if (addresses[address_index].addr.as_u32 != addr.as_u32) continue;

        a = addresses + address_index;
        if (nat44_ei_port_is_used(a, protocol, port_host_byte_order)) return VNET_API_ERROR_INSTANCE_IN_USE;
...
void nat44_ei_add_del_address_dpo(ip4_address_t addr, u8 is_add)
{
    ...
    if (is_add) {
        ...
        fib_table_entry_special_dpo_add(0, &pfx, nm->fib_src_hi, FIB_ENTRY_FLAG_EXCLUSIVE, &dpo_v4);
        dpo_reset(&dpo_v4);
    }
    else {
...
void nat44_ei_free_outside_address_and_port(nat44_ei_address_t *addresses, u32 thread_index, ip4_address_t *addr, u16 port, nat_protocol_t protocol)
{
    ...
    for (address_index = 0; address_index < vec_len(addresses); address_index++) {
        if (addresses[address_index].addr.as_u32 == addr->as_u32) break;
    }
...
void nat44_ei_free_session_data_v2(nat44_ei_main_t *nm, nat44_ei_session_t *s, u32 thread_index, u8 is_ha)
{
    ...
    init_nat_i2o_k(&kv, s);
    if (clib_bihash_add_del_8_8(&nm->in2out, &kv, 0)) nat_elog_warn(nm, "in2out key del failed");
    init_nat_o2i_k(&kv, s);
    if (clib_bihash_add_del_8_8(&nm->out2in, &kv, 0)) nat_elog_warn(nm, "out2in key del failed");

    if (!is_ha)
        nat_syslog_nat44_apmdel(s->user_index, s->in2out.fib_index, &s->in2out.addr, s->in2out.port, &s->out2in.addr, s->out2in.port, s->nat_proto);

    if (nat44_ei_is_unk_proto_session(s)) return;
    ...
    if (!is_ha) {
        ...
        nat_ha_sdel(&s->out2in.addr, s->out2in.port, &s->ext_host_addr, s->ext_host_port, s->nat_proto, s->out2in.fib_index, thread_index);
    }

    if (nat44_ei_is_session_static(s)) return;

    nat44_ei_free_outside_address_and_port(nm->addresses, thread_index, &s->out2in.addr, s->out2in.port, s->nat_proto);
}
...
nat44_ei_user_t *nat44_ei_user_get_or_create(nat44_ei_main_t *nm, ip4_address_t *addr, u32 fib_index, u32 thread_index)
{
    ...
    if (clib_bihash_search_8_8(&tnm->user_hash, &kv, &value)) {
        if (pool_elts(tnm->users) >= nm->max_users_per_thread) {
            vlib_increment_simple_counter(&nm->user_limit_reached, thread_index, 0, 1);
        ...
        u->sessions_per_user_list_head_index = per_user_list_head_elt - tnm->list_pool;
        ...
        vlib_set_simple_counter(&nm->total_users, thread_index, 0, pool_elts(tnm->users));
    }
    else {
...
nat44_ei_session_t *nat44_ei_session_alloc_or_recycle(nat44_ei_main_t *nm, nat44_ei_user_t *u, u32 thread_index, f64 now)
{
    ...
    if ((u->nsessions + u->nstaticsessions) >= nm->max_translations_per_user) {
        oldest_per_user_translation_list_index = clib_dlist_remove_head(tnm->list_pool, u->sessions_per_user_list_head_index);

        ASSERT(oldest_per_user_translation_list_index != ~0);

        /* Add it back to the end of the LRU list */
        clib_dlist_addtail(tnm->list_pool, u->sessions_per_user_list_head_index, oldest_per_user_translation_list_index);
        /* Get the list element */
        oldest_per_user_translation_list_elt = pool_elt_at_index(tnm->list_pool, oldest_per_user_translation_list_index);
        ...
        clib_dlist_init(tnm->list_pool, per_user_translation_list_elt - tnm->list_pool);
        ...
        clib_dlist_addtail(tnm->list_pool, s->per_user_list_head_index, per_user_translation_list_elt - tnm->list_pool);

        s->user_index = u - tnm->users;
        vlib_set_simple_counter(&nm->total_sessions, thread_index, 0, pool_elts(tnm->sessions));
    }
...
static_always_inline void nat44_ei_user_del_sessions(nat44_ei_user_t *u, u32 thread_index)
{
    ...
    // get head
    elt = pool_elt_at_index(tnm->list_pool, u->sessions_per_user_list_head_index);
    // get first element
...
int nat44_ei_user_del(ip4_address_t *addr, u32 fib_index)
{
    ...
    if (nm->num_workers > 1) {
        vec_foreach (tnm, nm->per_thread_data) {
            if (!clib_bihash_search_8_8(&tnm->user_hash, &kv, &value)) {
                nat44_ei_user_del_sessions(pool_elt_at_index(tnm->users, value.value), tnm->thread_index);
                rv = 0;
                break;
            }
        }
    ...
    if (!clib_bihash_search_8_8(&tnm->user_hash, &kv, &value)) {
        ...
        if (u->nstaticsessions) {
            ...
            while (ses_index != ~0) {
                ...
                if (!addr_only) {
                    if ((s->out2in.addr.as_u32 != e_addr.as_u32) || (s->out2in.port != e_port)) continue;
                }

                if (!nat44_ei_is_session_static(s)) continue;
...
u32 nat44_ei_get_in2out_worker_index(ip4_header_t *ip0, u32 rx_fib_index0, u8 is_output)
{
    ...
    hash              = ip0->src_address.as_u32 + (ip0->src_address.as_u32 >> 8) + (ip0->src_address.as_u32 >> 16) + (ip0->src_address.as_u32 >> 24);
...
u32 nat44_ei_get_thread_idx_by_port(u16 e_port)
{
    ...
    if (nm->num_workers > 1) {
        thread_idx = nm->first_worker_index + nm->workers[(e_port - 1024) / nm->port_per_thread % _vec_len(nm->workers)];
    }
    return thread_idx;
}
...
u32 nat44_ei_get_out2in_worker_index(vlib_buffer_t *b, ip4_header_t *ip0, u32 rx_fib_index0, u8 is_output)
{
    ...
    next_worker_index = nat44_ei_get_thread_idx_by_port(clib_net_to_host_u16(port));
    return next_worker_index;
}
        ...
        for (i = s_addr_offset; i < vec_len(addresses); ++i) {
            ...
            if (a->busy_ports_per_thread[proto][thread_index] < port_per_thread) {
                if (a->fib_index == fib_index) {
                    while (1) {
                        portnum = (port_per_thread * snat_thread_index) + nat_random_port(&nm->random_seed, 0, port_per_thread - 1) + 1024;
                        if (nat44_ei_port_is_used(a, proto, portnum)) continue;
                                   ...
                                   u32                 snat_thread_index)
{
    ...
    if (!vec_len(addresses)) goto exhausted;

    if (a->busy_ports[proto] < ports) {
        while (1) {
            portnum = nat_random_port(&nm->random_seed, nm->start_port, nm->end_port);
            if (nat44_ei_port_is_used(a, proto, portnum)) continue;
            nat44_ei_port_get(a, proto, portnum);
            a->busy_ports[proto]++;
            *addr = a->addr;
                                  ...
                                  u32                 snat_thread_index)
{
    ...
    if (a->busy_ports[proto] < ports) {
        while (1) {
            A       = nat_random_port(&nm->random_seed, 1, pow2_mask(nm->psid_offset));
...
{
    nat44_ei_main_per_thread_data_t *tnm = vec_elt_at_index(nm->per_thread_data, thread_index);
    clib_bihash_kv_8_8_t             kv, value;
    nat44_ei_user_t                 *u;
    const nat44_ei_user_key_t        u_key    = {.addr = ses->in2out.addr, .fib_index = ses->in2out.fib_index};
...
int nat44_ei_del_session(nat44_ei_main_t *nm, ip4_address_t *addr, u16 port, nat_protocol_t proto, u32 vrf_id, int is_in)
{
    ...
    if (!clib_bihash_search_8_8(t, &kv, &value)) {
        ...
        if (pool_is_free_index(tnm->sessions, session_index)) return VNET_API_ERROR_UNSPECIFIED;
...
int nat44_ei_reserve_port(ip4_address_t addr, u16 port, nat_protocol_t proto)
{
    ...
    for (i = 0; i < vec_len(nm->addresses); i++) {
        ...
        if (a->addr.as_u32 != addr.as_u32) continue;

        if (nat44_ei_port_is_used(a, proto, port)) continue;
...
int nat44_ei_free_port(ip4_address_t addr, u16 port, nat_protocol_t proto)
{
    u32                 ti = nat44_ei_get_thread_idx_by_port(port);
    nat44_ei_main_t    *nm = &nat44_ei_main;
    nat44_ei_address_t *a  = 0;
...
void nat44_ei_add_resolve_record(ip4_address_t  l_addr,
                                 u16            l_port,
                                 u16            e_port,
                                 nat_protocol_t proto,
                                 u32            vrf_id,
...
int nat44_ei_get_resolve_record(ip4_address_t l_addr, u16 l_port, u16 e_port, nat_protocol_t proto, u32 vrf_id, u32 sw_if_index, u32 flags, int *out)
{
    nat44_ei_static_map_resolve_t *rp;
    nat44_ei_main_t               *nm = &nat44_ei_main;
    int                            i;
...
int nat44_ei_del_resolve_record(ip4_address_t l_addr, u16 l_port, u16 e_port, nat_protocol_t proto, u32 vrf_id, u32 sw_if_index, u32 flags)
{
    nat44_ei_main_t *nm = &nat44_ei_main;
    int              i;
    if (!nat44_ei_get_resolve_record(l_addr, l_port, e_port, proto, vrf_id, sw_if_index, flags, &i)) {
...
void delete_matching_dynamic_sessions(const nat44_ei_static_mapping_t *m, u32 worker_index)
{
    nat44_ei_main_t                 *nm = &nat44_ei_main;
    clib_bihash_kv_8_8_t             kv, value;
    nat44_ei_session_t              *s;
    ...
    if (nm->static_mapping_only) return;
    ...
    if (!clib_bihash_search_8_8(&tnm->user_hash, &kv, &value)) {
        ...
        if (u->nsessions) {
            ...
            while (ses_index != ~0) {
                ...
                if (!is_sm_addr_only(m->flags) && s->in2out.port != m->local_port) continue;

                nat44_ei_free_session_data_v2(nm, s, tnm - nm->per_thread_data, 0);
                nat44_ei_delete_session(nm, s, tnm - nm->per_thread_data);

                if (!is_sm_addr_only(m->flags)) break;
            }
        }
    }
}

int nat44_ei_add_static_mapping(ip4_address_t  l_addr,
                                ip4_address_t  e_addr,
                                u16            l_port,
                                u16            e_port,
                                nat_protocol_t proto,
...

{
    ...
    if (is_sm_switch_address(flags)) {
        if (!nat44_ei_get_resolve_record(l_addr, l_port, e_port, proto, vrf_id, sw_if_index, flags, 0)) {
            return VNET_API_ERROR_VALUE_EXIST;
        }

        nat44_ei_add_resolve_record(l_addr, l_port, e_port, proto, vrf_id, sw_if_index, flags, pool_addr, tag);

        ip4_address_t *first_int_addr = ip4_interface_first_address(nm->ip4_main, sw_if_index, 0);
        if (!first_int_addr) {
    ...
    return nat44_ei_add_static_mapping_internal(l_addr, e_addr, l_port, e_port, proto, vrf_id, sw_if_index, flags, pool_addr, tag);
}

int nat44_ei_del_static_mapping(ip4_address_t  l_addr,
                                ip4_address_t  e_addr,
                                u16            l_port,
                                u16            e_port,
                                nat_protocol_t proto,
                                ...
                                u32            flags)
{
    ...
    if (is_sm_switch_address(flags)) {
        if (nat44_ei_del_resolve_record(l_addr, l_port, e_port, proto, vrf_id, sw_if_index, flags)) {
            return VNET_API_ERROR_NO_SUCH_ENTRY;
        }
    ...
    return nat44_ei_del_static_mapping_internal(l_addr, e_addr, l_port, e_port, proto, vrf_id, sw_if_index, flags);
}
                                                ...
                                                u8            *tag)
{
    ...
    if (!clib_bihash_search_8_8(&nm->static_mapping_by_external, &kv, &value)) {
        ...
        local->vrf_id    = vrf_id;
        local->fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, vrf_id, nm->fib_src_low);

        init_nat_kv(&kv, m->local_addr, m->local_port, local->fib_index, m->proto, 0, m - nm->static_mappings);
    ...
    if (vrf_id != ~0) {
        fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, vrf_id, nm->fib_src_low);
    }
    else {
    ...
    if (!(is_sm_addr_only(flags) || nm->static_mapping_only)) {
        if (nat44_ei_reserve_port(e_addr, e_port, proto)) {
            ...
            if ((is_sm_switch_address(flags)) && !is_sm_identity_nat(flags)) {
                nat44_ei_del_resolve_record(l_addr, l_port, e_port, proto, vrf_id, sw_if_index, flags);
            }
            return VNET_API_ERROR_NO_SUCH_ENTRY;
        }
    }
    ...
    init_nat_kv(&kv, m->local_addr, m->local_port, fib_index, m->proto, 0, m - nm->static_mappings);
    clib_bihash_add_del_8_8(&nm->static_mapping_by_local, &kv, 1);

    init_nat_kv(&kv, m->external_addr, m->external_port, 0, m->proto, 0, m - nm->static_mappings);
                                                ...
                                                u32            flags)
{
    ...
    if (!nm->static_mapping_only || nm->static_mapping_connection_tracking) {
        ...
        kv.key          = u_key.as_u64;
        nat44_ei_static_mapping_del_sessions(nm, tnm, u_key, is_sm_addr_only(flags), e_addr, e_port);
    }
                                  ...
                                  u8            *is_identity_nat)
{
    ...
    if (by_external) {
        ...
        if (clib_bihash_search_8_8(&nm->static_mapping_by_external, &kv, &value)) {
            ...
            if (clib_bihash_search_8_8(&nm->static_mapping_by_external, &kv, &value)) return 1;
        }
        m = pool_elt_at_index(nm->static_mappings, value.value);
        ...
        *mapping_addr      = m->local_addr;
        ...
        if (clib_bihash_search_8_8(&nm->static_mapping_by_local, &kv, &value)) {
            ...
            if (clib_bihash_search_8_8(&nm->static_mapping_by_local, &kv, &value)) return 1;
        }
        m = pool_elt_at_index(nm->static_mappings, value.value);
...
u8 *format_nat44_ei_key(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U proto %U port %d fib %d", format_ip4_address, &addr, format_nat_protocol, protocol, clib_net_to_host_u16(port), fib_index);
    return s;
}
...
u8 *format_nat44_ei_user_kvp(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U fib %d user-index %llu", format_ip4_address, &k.addr, k.fib_index, v->value);
...
u8 *format_nat44_ei_static_mapping_kvp(u8 *s, va_list *args)
{
    ...
    s = format(s, "%U static-mapping-index %llu", format_nat44_ei_key, v->key, v->value);
...
static void nat44_ei_worker_db_init(nat44_ei_main_per_thread_data_t *tnm, u32 translations, u32 translation_buckets, u32 user_buckets)
{
    ...
    clib_bihash_set_kvp_format_fn_8_8(&tnm->user_hash, format_nat44_ei_user_kvp);
...
static void nat44_ei_db_init(u32 translations, u32 translation_buckets, u32 user_buckets)
{
    ...
    clib_bihash_init_8_8(&nm->static_mapping_by_local, "static_mapping_by_local", static_mapping_buckets, static_mapping_memory_size);
    clib_bihash_init_8_8(&nm->static_mapping_by_external, "static_mapping_by_external", static_mapping_buckets, static_mapping_memory_size);
    clib_bihash_set_kvp_format_fn_8_8(&nm->static_mapping_by_local, format_nat44_ei_static_mapping_kvp);
    clib_bihash_set_kvp_format_fn_8_8(&nm->static_mapping_by_external, format_nat44_ei_static_mapping_kvp);

    if (nm->pat) {
        clib_bihash_init_8_8(&nm->in2out, "in2out", translation_buckets, 0);
        clib_bihash_init_8_8(&nm->out2in, "out2in", translation_buckets, 0);
        clib_bihash_set_kvp_format_fn_8_8(&nm->in2out, format_nat44_ei_session_kvp);
        clib_bihash_set_kvp_format_fn_8_8(&nm->out2in, format_nat44_ei_session_kvp);
        vec_foreach (tnm, nm->per_thread_data) {
            nat44_ei_worker_db_init(tnm, translations, translation_buckets, user_buckets);
        }
    }
}
...
void nat44_ei_sessions_clear()
{
    ...
    if (nm->pat) {
        ...
        vec_foreach (tnm, nm->per_thread_data) {
            nat44_ei_worker_db_free(tnm);
            nat44_ei_worker_db_init(tnm, nm->translations, nm->translation_buckets, nm->user_buckets);
        }
    }
...
static void nat44_ei_update_outside_fib(ip4_main_t *im, uword opaque, u32 sw_if_index, u32 new_fib_index, u32 old_fib_index)
{
    ...
    if (!nm->enabled || (new_fib_index == old_fib_index) || (!vec_len(nm->outside_fibs))) {
        return;
    }

    pool_foreach (i, nm->interfaces) {
        if (i->sw_if_index == sw_if_index) {
            if (!(nat44_ei_interface_is_outside(i))) return;
            match = 1;
        }
    }
    ...
    vec_foreach (outside_fib, nm->outside_fibs) {
        if (outside_fib->fib_index == old_fib_index) {
            outside_fib->refcount--;
            if (!outside_fib->refcount) vec_del1(nm->outside_fibs, outside_fib - nm->outside_fibs);
            break;
        }
    }
...
int nat44_ei_add_address(ip4_address_t *addr, u32 vrf_id)
{
    ...
    if (vrf_id != ~0) {
        ap->fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, vrf_id, nm->fib_src_low);
    }
    ...
    for (proto = 0; proto < NAT_N_PROTOCOLS; ++proto) {
        ...
        vec_validate_init_empty(ap->busy_ports_per_thread[proto], tm->n_vlib_mains - 1, 0);
    }
...
int nat44_ei_del_address(ip4_address_t addr, u8 delete_sm)
{
    ...
    if (delete_sm) {
        pool_foreach (m, nm->static_mappings) {
            if (m->external_addr.as_u32 == addr.as_u32)
                nat44_ei_del_static_mapping_internal(m->local_addr,
                                                     m->external_addr,
                                                     m->local_port,
                                                     m->external_port,
                                                     m->proto,
    ...
    if (a->busy_ports[NAT_PROTOCOL_TCP] || a->busy_ports[NAT_PROTOCOL_UDP] || a->busy_ports[NAT_PROTOCOL_ICMP]) {
        vec_foreach (tnm, nm->per_thread_data) {
            pool_foreach (ses, tnm->sessions) {
                if (ses->out2in.addr.as_u32 == addr.as_u32) {
                    nat44_ei_free_session_data(nm, ses, tnm - nm->per_thread_data, 0);
                                                      ...
                                                      u32            is_delete)
{
    ...
    if (!is_sw_if_index_reg_for_auto_resolve(nm->auto_add_sw_if_indices, sw_if_index)) {
        return;
    }
    ...
    if (!is_delete) {
        ...
        for (i = 0; i < vec_len(nm->to_resolve); i++) {
            ...
            /* On this interface? */
            if (rp->sw_if_index == sw_if_index) {
                rv = nat44_ei_add_static_mapping_internal(rp->l_addr,
                                                          address[0],
                                                          rp->l_port,
                                                          rp->e_port,
                                                          rp->proto,
                                                          ...
                                                          rp->tag);
                if (rv) {
                    nat_elog_notice_X1(nm, "add_static_mapping_internal returned %d", "i4", rv);
                }
            }
        }
    }
                                                 ...
                                                 u32            is_delete)
{
    ...
    init_nat_k(&kv, *address, is_sm_addr_only(rp->flags) ? 0 : rp->e_port, nm->outside_fib_index, is_sm_addr_only(rp->flags) ? 0 : rp->proto);
    if (clib_bihash_search_8_8(&nm->static_mapping_by_external, &kv, &value))
        m = 0;
    else
        m = pool_elt_at_index(nm->static_mappings, value.value);
    ...
    if (is_delete) {
        if (!m) return;
        rv = nat44_ei_del_static_mapping_internal(rp->l_addr, address[0], rp->l_port, rp->e_port, rp->proto, rp->vrf_id, ~0, rp->flags);
        if (rv) {
            nat_elog_notice_X1(nm, "nat44_ei_del_static_mapping returned %d", "i4", rv);
        }
    }
    else {
        if (m) return;
        ...
        if (rv) {
            nat_elog_notice_X1(nm, "nat44_ei_add_static_mapping returned %d", "i4", rv);
        }
    }
}
...
static_always_inline uword nat44_ei_classify_inline_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(pool_elts(nm->static_mappings))) {
                ...
                if (!clib_bihash_search_8_8(&nm->static_mapping_by_external, &kv0, &value0)) {
                    ...
                    if (m->local_addr.as_u32 != m->external_addr.as_u32) next0 = NAT44_EI_CLASSIFY_NEXT_OUT2IN;
                    goto enqueue0;
                }
                init_nat_k(&kv0, ip0->dst_address, vnet_buffer(b0)->ip.reass.l4_dst_port, 0, ip_proto_to_nat_proto(ip0->protocol));
                if (!clib_bihash_search_8_8(&nm->static_mapping_by_external, &kv0, &value0)) {
                    m = pool_elt_at_index(nm->static_mappings, value0.value);
                    if (m->local_addr.as_u32 != m->external_addr.as_u32) next0 = NAT44_EI_CLASSIFY_NEXT_OUT2IN;
                }
        ...
        enqueue0:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                nat44_ei_classify_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                t->cached                    = 0;
    ...
    vlib_node_increment_counter(vm, node->node_index, NAT44_EI_CLASSIFY_ERROR_NEXT_IN2OUT, next_in2out);
    vlib_node_increment_counter(vm, node->node_index, NAT44_EI_CLASSIFY_ERROR_NEXT_OUT2IN, next_out2in);
