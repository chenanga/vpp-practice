...
static void vl_api_nat44_ei_show_running_config_t_handler(vl_api_nat44_ei_show_running_config_t *mp)
{
    ...
    REPLY_MACRO2_ZERO(VL_API_NAT44_EI_SHOW_RUNNING_CONFIG_REPLY, ({
                          ...
                          if (rc->static_mapping_only) rmp->flags |= NAT44_EI_STATIC_MAPPING_ONLY;
                          if (rc->connection_tracking) rmp->flags |= NAT44_EI_CONNECTION_TRACKING;
                          if (rc->out2in_dpo) rmp->flags |= NAT44_EI_OUT2IN_DPO;
                      }));
}
...
static void send_nat_worker_details(u32 worker_index, vl_api_registration_t *reg, u32 context)
{
    ...
    nat44_ei_main_t                  *nm = &nat44_ei_main;
    vlib_worker_thread_t             *w  = vlib_worker_threads + worker_index + nm->first_worker_index;
...
static void vl_api_nat44_ei_set_addr_and_port_alloc_alg_t_handler(vl_api_nat44_ei_set_addr_and_port_alloc_alg_t *mp)
{
    ...
    switch (mp->alg) {
        ...
        case NAT44_EI_ADDR_AND_PORT_ALLOC_ALG_MAPE:
            nat44_ei_set_alloc_mape(ntohs(mp->psid), mp->psid_offset, mp->psid_length);
            break;
...
static void vl_api_nat44_ei_ha_set_listener_t_handler(vl_api_nat44_ei_ha_set_listener_t *mp)
{
    ...
    rv = nat_ha_set_listener(vlib_get_main(), &addr, clib_net_to_host_u16(mp->port), clib_net_to_host_u32(mp->path_mtu));
...
static void vl_api_nat44_ei_ha_set_failover_t_handler(vl_api_nat44_ei_ha_set_failover_t *mp)
{
    ...
    memcpy(&addr, &mp->ip_address, sizeof(addr));
    rv = nat_ha_set_failover(vlib_get_main(), &addr, clib_net_to_host_u16(mp->port), clib_net_to_host_u32(mp->session_refresh_interval));
...
static void nat_ha_resync_completed_event_cb(u32 client_index, u32 pid, u32 missed_count)
{
    ...
    mp->_vl_msg_id   = ntohs(VL_API_NAT44_EI_HA_RESYNC_COMPLETED_EVENT + nm->msg_id_base);
...
static void vl_api_nat44_ei_ha_resync_t_handler(vl_api_nat44_ei_ha_resync_t *mp)
{
    ...
    rv = nat_ha_resync(mp->client_index, mp->pid, mp->want_resync_event ? nat_ha_resync_completed_event_cb : NULL);
...
static void vl_api_nat44_ei_add_del_address_range_t_handler(vl_api_nat44_ei_add_del_address_range_t *mp)
{
    ...
    if (count > 1024)
        nat44_ei_log_info("%U - %U, %d addresses...", format_ip4_address, mp->first_ip_address, format_ip4_address, mp->last_ip_address, count);
...
static void vl_api_nat44_ei_interface_add_del_feature_t_handler(vl_api_nat44_ei_interface_add_del_feature_t *mp)
{
    ...
    if (mp->is_add) {
        rv = nat44_ei_add_interface(sw_if_index, mp->flags & NAT44_EI_IF_INSIDE);
    }
    else {
        rv = nat44_ei_del_interface(sw_if_index, mp->flags & NAT44_EI_IF_INSIDE);
    }
...
static void send_nat44_ei_interface_details(nat44_ei_interface_t *i, vl_api_registration_t *reg, u32 context)
{
    ...
    rmp = vl_msg_api_alloc(sizeof(*rmp));
    clib_memset(rmp, 0, sizeof(*rmp));
    rmp->_vl_msg_id  = ntohs(VL_API_NAT44_EI_INTERFACE_DETAILS + nm->msg_id_base);
    rmp->sw_if_index = ntohl(i->sw_if_index);

    if (nat44_ei_interface_is_inside(i)) rmp->flags |= NAT44_EI_IF_INSIDE;
    if (nat44_ei_interface_is_outside(i)) rmp->flags |= NAT44_EI_IF_OUTSIDE;
...
static_always_inline int add_del_dummy_output_interface(u32 sw_if_index, u8 is_inside, u8 is_add)
{
    nat44_ei_main_t      *nm = &nat44_ei_main;
    nat44_ei_interface_t *i;
    int                   rv = 1;
...
static void vl_api_nat44_ei_interface_add_del_output_feature_t_handler(vl_api_nat44_ei_interface_add_del_output_feature_t *mp)
{
    ...
    rv = add_del_dummy_output_interface(sw_if_index, mp->flags & NAT44_EI_IF_INSIDE, mp->is_add);
...
static void send_nat44_ei_interface_output_feature_details(nat44_ei_interface_t *i, vl_api_registration_t *reg, u32 context)
{
    ...
    rmp = vl_msg_api_alloc(sizeof(*rmp));
    clib_memset(rmp, 0, sizeof(*rmp));
    rmp->_vl_msg_id  = ntohs(VL_API_NAT44_EI_INTERFACE_OUTPUT_FEATURE_DETAILS + nm->msg_id_base);
    rmp->sw_if_index = ntohl(i->sw_if_index);
    rmp->context     = context;
...
static void vl_api_nat44_ei_add_del_output_interface_t_handler(vl_api_nat44_ei_add_del_output_interface_t *mp)
{
...
#undef vl_endianfun
static void send_nat44_ei_output_interface_details(u32 index, vl_api_registration_t *rp, u32 context)
{
    nat44_ei_main_t                            *nm = &nat44_ei_main;
    vl_api_nat44_ei_output_interface_details_t *rmp;
    nat44_ei_interface_t                       *i = pool_elt_at_index(nm->output_feature_interfaces, index);

    /* Make sure every field is initiated (or don't skip the clib_memset()) */
    REPLY_MACRO_DETAILS4(VL_API_NAT44_EI_OUTPUT_INTERFACE_DETAILS, rp, context, ({
                             rmp->sw_if_index = i->sw_if_index;

                             /* Endian hack until apigen registers _details
                              * endian functions */
                             vl_api_nat44_ei_output_interface_details_t_endian(rmp, 1 /* to network */);
                             rmp->_vl_msg_id = htons(rmp->_vl_msg_id);
                             rmp->context    = htonl(rmp->context);
                         }));
}

static void vl_api_nat44_ei_output_interface_get_t_handler(vl_api_nat44_ei_output_interface_get_t *mp)
{
    ...
    REPLY_AND_DETAILS_MACRO(VL_API_NAT44_EI_OUTPUT_INTERFACE_GET_REPLY, nm->output_feature_interfaces, ({
...
static void vl_api_nat44_ei_add_del_static_mapping_t_handler(vl_api_nat44_ei_add_del_static_mapping_t *mp)
{
    ...
    if (mp->is_add) {
        ...
        rv = nat44_ei_add_static_mapping(l_addr, e_addr, l_port, e_port, proto, vrf_id, sw_if_index, flags, pool_addr, tag);
        vec_free(tag);
    }
    else {
...
static void send_nat44_ei_static_mapping_details(nat44_ei_static_mapping_t *m, vl_api_registration_t *reg, u32 context)
{
    ...
    rmp->_vl_msg_id = ntohs(VL_API_NAT44_EI_STATIC_MAPPING_DETAILS + nm->msg_id_base);
...
static void vl_api_nat44_ei_static_mapping_dump_t_handler(vl_api_nat44_ei_static_mapping_dump_t *mp)
{
    ...
    pool_foreach (m, nm->static_mappings) {
        if (!is_sm_identity_nat(m->flags)) send_nat44_ei_static_mapping_details(m, reg, mp->context);
    }

    for (j = 0; j < vec_len(nm->to_resolve); j++) {
        rp = nm->to_resolve + j;
        if (!is_sm_identity_nat(rp->flags)) send_nat44_ei_static_map_resolve_details(rp, reg, mp->context);
    }
}
...
static void vl_api_nat44_ei_add_del_identity_mapping_t_handler(vl_api_nat44_ei_add_del_identity_mapping_t *mp)
{
    ...
    if (mp->is_add) {
        ...
        rv = nat44_ei_add_static_mapping(addr, addr, port, port, proto, vrf_id, sw_if_index, flags, pool_addr, tag);
        vec_free(tag);
    }
    else {
...
static void send_nat44_ei_identity_mapping_details(nat44_ei_static_mapping_t *m, int index, vl_api_registration_t *reg, u32 context)
{
    ...
    rmp = vl_msg_api_alloc(sizeof(*rmp));
    clib_memset(rmp, 0, sizeof(*rmp));
    rmp->_vl_msg_id = ntohs(VL_API_NAT44_EI_IDENTITY_MAPPING_DETAILS + nm->msg_id_base);

    if (is_sm_addr_only(m->flags)) rmp->flags |= NAT44_EI_ADDR_ONLY_MAPPING;
...
static void send_nat44_ei_identity_map_resolve_details(nat44_ei_static_map_resolve_t *m, vl_api_registration_t *reg, u32 context)
{
    ...
    if (is_sm_addr_only(m->flags)) rmp->flags = (vl_api_nat44_ei_config_flags_t) NAT44_EI_ADDR_ONLY_MAPPING;
...
static void vl_api_nat44_ei_identity_mapping_dump_t_handler(vl_api_nat44_ei_identity_mapping_dump_t *mp)
{
    ...
    for (j = 0; j < vec_len(nm->to_resolve); j++) {
        rp = nm->to_resolve + j;
        if (is_sm_identity_nat(rp->flags)) send_nat44_ei_identity_map_resolve_details(rp, reg, mp->context);
    }
}
...
static void send_nat44_ei_interface_addr_details(u32 sw_if_index, vl_api_registration_t *reg, u32 context)
{
    ...
    rmp = vl_msg_api_alloc(sizeof(*rmp));
    clib_memset(rmp, 0, sizeof(*rmp));
    rmp->_vl_msg_id  = ntohs(VL_API_NAT44_EI_INTERFACE_ADDR_DETAILS + nm->msg_id_base);
...
static void send_nat44_ei_user_session_details(nat44_ei_session_t *s, vl_api_registration_t *reg, u32 context)
{
    ...
    rmp = vl_msg_api_alloc(sizeof(*rmp));
    clib_memset(rmp, 0, sizeof(*rmp));
    rmp->_vl_msg_id = ntohs(VL_API_NAT44_EI_USER_SESSION_DETAILS + nm->msg_id_base);
    clib_memcpy(rmp->outside_ip_address, (&s->out2in.addr), 4);
    clib_memcpy(rmp->inside_ip_address, (&s->in2out.addr), 4);

    if (nat44_ei_is_session_static(s)) rmp->flags |= NAT44_EI_STATIC_MAPPING;
...
static void send_nat44_ei_user_session_v2_details(nat44_ei_session_t *s, vl_api_registration_t *reg, u32 context)
{
    ...
    rmp = vl_msg_api_alloc(sizeof(*rmp));
    clib_memset(rmp, 0, sizeof(*rmp));
    rmp->_vl_msg_id = ntohs(VL_API_NAT44_EI_USER_SESSION_V2_DETAILS + nm->msg_id_base);
    clib_memcpy(rmp->outside_ip_address, (&s->out2in.addr), 4);
    clib_memcpy(rmp->inside_ip_address, (&s->in2out.addr), 4);
    ...
    rmp->last_heard            = clib_host_to_net_u64((u64) s->last_heard);
    rmp->time_since_last_heard = clib_host_to_net_u64((u64) (vlib_time_now(vlib_get_main()) - s->last_heard));
    rmp->total_bytes           = clib_host_to_net_u64(s->total_bytes);
    rmp->total_pkts            = ntohl(s->total_pkts);
    rmp->context               = context;
    if (nat44_ei_is_unk_proto_session(s)) {
...
static void vl_api_nat44_ei_user_session_dump_t_handler(vl_api_nat44_ei_user_session_dump_t *mp)
{
    ...
    key.key               = ukey.as_u64;
    if (nm->num_workers > 1)
        tnm = vec_elt_at_index(nm->per_thread_data, nat44_ei_get_in2out_worker_index(&ip, ukey.fib_index, 0));
    else
        tnm = vec_elt_at_index(nm->per_thread_data, nm->num_workers);

    if (clib_bihash_search_8_8(&tnm->user_hash, &key, &value)) return;
    u = pool_elt_at_index(tnm->users, value.value);
    if (!u->nsessions && !u->nstaticsessions) return;
...
static void vl_api_nat44_ei_user_session_v2_dump_t_handler(vl_api_nat44_ei_user_session_dump_t *mp)
{
    vl_api_registration_t           *reg;
    nat44_ei_main_t                 *nm = &nat44_ei_main;
    nat44_ei_main_per_thread_data_t *tnm;
