...
static inline u8 nat64_not_translate(u32 sw_if_index, ip6_address_t ip6_addr)
{
    ip6_address_t          *addr;
    ...
    foreach_ip_interface_address(lm6, ia, sw_if_index, 0, ({
                                     ...
                                     if (0 == ip6_address_compare(addr, &ip6_addr)) return 1;
                                 }));
...
static_always_inline int is_hairpinning(ip6_address_t *dst_addr)
{
    ...
    for (i = 0; i < vec_len(nm->addr_pool); i++) {
        if (nm->addr_pool[i].addr.as_u32 == dst_addr->as_u32[3]) return 1;
    }
...
static int nat64_in2out_tcp_udp(vlib_main_t *vm, vlib_buffer_t *p, u16 l4_offset, u16 frag_hdr_offset, nat64_in2out_set_ctx_t *ctx)
{
    ...
    u32 ip_version_traffic_class_and_flow_label = ip6->ip_version_traffic_class_and_flow_label;
    ...
    ip4->length                       = u16_net_add(payload_length, sizeof(*ip4) + sizeof(*ip6) - l4_offset);
    ...
    ste = nat64_db_st_entry_find(db, &old_saddr, &old_daddr, sport, dport, proto, fib_index, 1);
    ...
    if (ste) {
        ...
        bibe = nat64_db_bib_entry_find(db, &old_saddr, sport, proto, fib_index, 1);

        if (!bibe) {
            u16           out_port;
            ip4_address_t out_addr;
            if (nat64_alloc_out_addr_and_port(fib_index, ip_proto_to_nat_proto(proto), &out_addr, &out_port, ctx->thread_index)) return -1;

            bibe = nat64_db_bib_entry_create(ctx->thread_index, db, &old_saddr.ip6, &out_addr, sport, out_port, fib_index, proto, 0);
            if (!bibe) return -1;

            vlib_set_simple_counter(&nm->total_bibs, ctx->thread_index, 0, db->bib.bib_entries_num);
        }

        nat64_extract_ip4(&old_daddr.ip6, &new_daddr, fib_index);
        ste = nat64_db_st_entry_create(ctx->thread_index, db, bibe, &old_daddr.ip6, &new_daddr, dport);
        if (!ste) return -1;
...
static int nat64_in2out_icmp_set_cb(ip6_header_t *ip6, ip4_header_t *ip4, void *arg)
{
    ...
    if (icmp->type == ICMP4_echo_request || icmp->type == ICMP4_echo_reply) {
        ...
        ste       = nat64_db_st_entry_find(db, &saddr, &daddr, in_id, 0, IP_PROTOCOL_ICMP, fib_index, 1);

        if (ste) {
            bibe = nat64_db_bib_entry_by_index(db, IP_PROTOCOL_ICMP, ste->bibe_index);
            if (!bibe) return -1;
        }
        else {
            bibe = nat64_db_bib_entry_find(db, &saddr, in_id, IP_PROTOCOL_ICMP, fib_index, 1);

            if (!bibe) {
                u16           out_id;
                ip4_address_t out_addr;
                if (nat64_alloc_out_addr_and_port(fib_index, NAT_PROTOCOL_ICMP, &out_addr, &out_id, ctx->thread_index)) return -1;

                bibe = nat64_db_bib_entry_create(ctx->thread_index, db, &ip6->src_address, &out_addr, in_id, out_id, fib_index, IP_PROTOCOL_ICMP, 0);
                if (!bibe) return -1;
            ...
            ste = nat64_db_st_entry_create(ctx->thread_index, db, bibe, &ip6->dst_address, &daddr.ip4, 0);
            if (!ste) return -1;
        ...
        if (!vec_len(nm->addr_pool)) return -1;
...
static int nat64_in2out_inner_icmp_set_cb(ip6_header_t *ip6, ip4_header_t *ip4, void *arg)
{
    ...
    if (proto == IP_PROTOCOL_ICMP6) {
        ...
        if (!(icmp->type == ICMP4_echo_request || icmp->type == ICMP4_echo_reply)) return -1;

        ste = nat64_db_st_entry_find(db, &daddr, &saddr, in_id, 0, proto, fib_index, 1);
        if (!ste) return -1;
        ...
        ste = nat64_db_st_entry_find(db, &daddr, &saddr, dport, sport, proto, fib_index, 1);
        if (!ste) return -1;
...
static int unk_proto_st_walk(nat64_db_st_entry_t *ste, void *arg)
{
    ...
    if (ip6_address_is_equal(&ste->in_r_addr, &ctx->dst_addr)) {
        ...
        if (ip6_address_is_equal(&bibe->in_addr, &ctx->src_addr) && bibe->fib_index == ctx->fib_index) {
            ...
            if (nat64_db_st_entry_find(db, &daddr, &saddr, 0, 0, ctx->proto, ctx->fib_index, 0)) return -1;
...
static int nat64_in2out_unk_proto(vlib_main_t            *vm,
                                  vlib_buffer_t          *p,
                                  u8                      l4_protocol,
                                  u16                     l4_offset,
                                  u16                     frag_hdr_offset,
                                  nat64_in2out_set_ctx_t *s_ctx)
{
    ...
    ste = nat64_db_st_entry_find(db, &saddr, &daddr, 0, 0, l4_protocol, fib_index, 1);
    ...
    if (ste) {
        ...
        bibe = nat64_db_bib_entry_find(db, &saddr, 0, l4_protocol, fib_index, 1);
        ...
        if (!bibe) {
            ...
            if (!ctx.out_addr.as_u32) nat64_db_st_walk(db, IP_PROTOCOL_UDP, unk_proto_st_walk, &ctx);

            /* Verify if out address is not already in use for protocol */
            clib_memset(&addr, 0, sizeof(addr));
            addr.ip4.as_u32 = ctx.out_addr.as_u32;
            if (nat64_db_bib_entry_find(db, &addr, 0, l4_protocol, 0, 0)) ctx.out_addr.as_u32 = 0;

            if (!ctx.out_addr.as_u32) {
                for (i = 0; i < vec_len(nm->addr_pool); i++) {
                    addr.ip4.as_u32 = nm->addr_pool[i].addr.as_u32;
                    if (!nat64_db_bib_entry_find(db, &addr, 0, l4_protocol, 0, 0)) break;
                }
            }

            if (!ctx.out_addr.as_u32) return -1;

            bibe = nat64_db_bib_entry_create(s_ctx->thread_index, db, &ip6->src_address, &ctx.out_addr, 0, 0, fib_index, l4_protocol, 0);
            if (!bibe) return -1;

            vlib_set_simple_counter(&nm->total_bibs, s_ctx->thread_index, 0, db->bib.bib_entries_num);
        }

        nat64_extract_ip4(&ip6->dst_address, &daddr.ip4, fib_index);
        ste = nat64_db_st_entry_create(s_ctx->thread_index, db, bibe, &ip6->dst_address, &daddr.ip4, 0);
        if (!ste) return -1;

        vlib_set_simple_counter(&nm->total_sessions, s_ctx->thread_index, 0, db->st.st_entries_num);
    }
...
static int nat64_in2out_tcp_udp_hairpinning(vlib_main_t *vm, vlib_buffer_t *b, ip6_header_t *ip6, u32 l4_offset, u32 thread_index)
{
    ...
    ste = nat64_db_st_entry_find(db, &saddr, &daddr, sport, dport, proto, fib_index, 1);
    ...
    if (ste) {
        ...
        if (!bibe) {
            u16           out_port;
            ip4_address_t out_addr;
            if (nat64_alloc_out_addr_and_port(fib_index, ip_proto_to_nat_proto(proto), &out_addr, &out_port, thread_index)) return -1;

            bibe = nat64_db_bib_entry_create(thread_index, db, &ip6->src_address, &out_addr, sport, out_port, fib_index, proto, 0);
            if (!bibe) return -1;
        ...
        nat64_extract_ip4(&ip6->dst_address, &daddr.ip4, fib_index);
        ste = nat64_db_st_entry_create(thread_index, db, bibe, &ip6->dst_address, &daddr.ip4, dport);
        if (!ste) return -1;

        vlib_set_simple_counter(&nm->total_sessions, thread_index, 0, db->st.st_entries_num);
    }

    if (proto == IP_PROTOCOL_TCP) nat64_tcp_session_set_state(ste, tcp, 1);
...
static int nat64_in2out_icmp_hairpinning(vlib_main_t *vm, vlib_buffer_t *b, ip6_header_t *ip6, u32 thread_index)
{
    ...
    if (icmp->type == ICMP6_echo_request || icmp->type == ICMP6_echo_reply) return -1;

    inner_ip6 = (ip6_header_t *) u8_ptr_add(icmp, 8);

    proto = inner_ip6->protocol;

    if (proto == IP_PROTOCOL_ICMP6) return -1;
    ...
    ste = 0;
    vec_foreach (db, nm->db) {
        ste = nat64_db_st_entry_find(db, &saddr, &daddr, sport, dport, proto, 0, 0);
...
static int nat64_in2out_unk_proto_hairpinning(vlib_main_t *vm, vlib_buffer_t *b, ip6_header_t *ip6, u32 thread_index)
{
    ...
    ste = nat64_db_st_entry_find(db, &saddr, &daddr, 0, 0, proto, fib_index, 1);
    ...
    if (ste) {
        ...
        if (!bibe) {
            ...
            clib_memset(&addr, 0, sizeof(addr));
            addr.ip4.as_u32 = ctx.out_addr.as_u32;
            if (nat64_db_bib_entry_find(db, &addr, 0, proto, 0, 0)) ctx.out_addr.as_u32 = 0;

            if (!ctx.out_addr.as_u32) {
                for (i = 0; i < vec_len(nm->addr_pool); i++) {
                    addr.ip4.as_u32 = nm->addr_pool[i].addr.as_u32;
                    if (!nat64_db_bib_entry_find(db, &addr, 0, proto, 0, 0)) break;
                }
            }

            if (!ctx.out_addr.as_u32) return -1;

            bibe = nat64_db_bib_entry_create(thread_index, db, &ip6->src_address, &ctx.out_addr, 0, 0, fib_index, proto, 0);
            if (!bibe) return -1;
        ...
        nat64_extract_ip4(&ip6->dst_address, &daddr.ip4, fib_index);
        ste = nat64_db_st_entry_create(thread_index, db, bibe, &ip6->dst_address, &daddr.ip4, 0);
        if (!ste) return -1;
...
static inline uword nat64_in2out_node_fn_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_slow_path)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(ip6_parse(vm, b0, ip60, b0->current_length, &l4_protocol0, &l4_offset0, &frag_hdr_offset0))) {
            ...
            if (is_slow_path) {
                if (PREDICT_TRUE(proto0 == NAT_PROTOCOL_OTHER)) {
                    vlib_increment_simple_counter(&nm->counters.in2out.other, thread_index, sw_if_index0, 1);
                    if (is_hairpinning(&ip60->dst_address)) {
                        next0 = NAT64_IN2OUT_NEXT_IP6_LOOKUP;
                        if (nat64_in2out_unk_proto_hairpinning(vm, b0, ip60, thread_index)) {
                            next0     = NAT64_IN2OUT_NEXT_DROP;
                    ...
                    if (nat64_in2out_unk_proto(vm, b0, l4_protocol0, l4_offset0, frag_hdr_offset0, &ctx0)) {
                        next0     = NAT64_IN2OUT_NEXT_DROP;
                        b0->error = node->errors[NAT64_IN2OUT_ERROR_NO_TRANSLATION];
                        goto trace0;
                    }
            ...
            if (proto0 == NAT_PROTOCOL_ICMP) {
                vlib_increment_simple_counter(&nm->counters.in2out.icmp, thread_index, sw_if_index0, 1);
                if (is_hairpinning(&ip60->dst_address)) {
                    next0 = NAT64_IN2OUT_NEXT_IP6_LOOKUP;
                    if (nat64_in2out_icmp_hairpinning(vm, b0, ip60, thread_index)) {
                        next0     = NAT64_IN2OUT_NEXT_DROP;
                        b0->error = node->errors[NAT64_IN2OUT_ERROR_NO_TRANSLATION];
                    }
                    goto trace0;
                ...
                if (icmp6_to_icmp(vm, b0, nat64_in2out_icmp_set_cb, &ctx0, nat64_in2out_inner_icmp_set_cb, &ctx0)) {
                    next0     = NAT64_IN2OUT_NEXT_DROP;
                    b0->error = node->errors[NAT64_IN2OUT_ERROR_NO_TRANSLATION];
                    goto trace0;
                }
                ...
                if (proto0 == NAT_PROTOCOL_TCP)
                    vlib_increment_simple_counter(&nm->counters.in2out.tcp, thread_index, sw_if_index0, 1);
                else
                    vlib_increment_simple_counter(&nm->counters.in2out.udp, thread_index, sw_if_index0, 1);

                if (is_hairpinning(&ip60->dst_address)) {
                    next0 = NAT64_IN2OUT_NEXT_IP6_LOOKUP;
                    if (nat64_in2out_tcp_udp_hairpinning(vm, b0, ip60, l4_offset0, thread_index)) {
                        next0     = NAT64_IN2OUT_NEXT_DROP;
                        b0->error = node->errors[NAT64_IN2OUT_ERROR_NO_TRANSLATION];
                    }
                    goto trace0;
                ...
                if (nat64_in2out_tcp_udp(vm, b0, l4_offset0, frag_hdr_offset0, &ctx0)) {
                    next0     = NAT64_IN2OUT_NEXT_DROP;
                    b0->error = node->errors[NAT64_IN2OUT_ERROR_NO_TRANSLATION];
                    goto trace0;
                }
        ...
        trace0:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                nat64_in2out_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
            ...
            if (next0 == NAT64_IN2OUT_NEXT_DROP) {
                vlib_increment_simple_counter(&nm->counters.in2out.drops, thread_index, sw_if_index0, 1);
            }
...
VLIB_NODE_FN(nat64_in2out_node)
...
VLIB_NODE_FN(nat64_in2out_slowpath_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static u8 *format_nat64_in2out_handoff_trace(u8 *s, va_list *args)
{
    CLIB_UNUSED(vlib_main_t * vm)   = va_arg(*args, vlib_main_t *);
    CLIB_UNUSED(vlib_node_t * node) = va_arg(*args, vlib_node_t *);
    nat64_in2out_handoff_trace_t *t = va_arg(*args, nat64_in2out_handoff_trace_t *);

    s = format(s, "NAT64-IN2OUT-HANDOFF: next-worker %d", t->next_worker_index);

    return s;
}

VLIB_NODE_FN(nat64_in2out_handoff_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    nat64_main_t  *nm = &nat64_main;
    ...
    while (n_left_from > 0) {
        ...
        if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b[0]->flags & VLIB_BUFFER_IS_TRACED))) {
            nat64_in2out_handoff_trace_t *t = vlib_add_trace(vm, node, b[0], sizeof(*t));
    ...
    if (n_enq < frame->n_vectors)
        vlib_node_increment_counter(vm, node->node_index, NAT64_IN2OUT_HANDOFF_ERROR_CONGESTION_DROP, frame->n_vectors - n_enq);
    vlib_node_increment_counter(vm, node->node_index, NAT64_IN2OUT_HANDOFF_ERROR_SAME_WORKER, same_worker);
    vlib_node_increment_counter(vm, node->node_index, NAT64_IN2OUT_HANDOFF_ERROR_DO_HANDOFF, do_handoff);
