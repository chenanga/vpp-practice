...
static u8 well_known_prefix[] = {0x00, 0x64, 0xff, 0x9b, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
        ...
        if (sw_if_index == nm->auto_add_sw_if_indices[i]) {
            if (!is_delete) {
                ...
                for (j = 0; j < vec_len(nm->addr_pool); j++)
                    if (nm->addr_pool[j].addr.as_u32 == address->as_u32) return;

                (void) nat64_add_del_pool_addr(vlib_get_thread_index(), address, ~0, 1);
                return;
            }
            else {
                (void) nat64_add_del_pool_addr(vlib_get_thread_index(), address, ~0, 0);
                return;
            }
        }
    }
...
u32 nat64_get_worker_out2in(vlib_buffer_t *b, ip4_header_t *ip)
{
    ...
    if (port > 1024) return get_thread_idx_by_port(port);
...
clib_error_t *nat64_init(vlib_main_t *vm)
{
    ...
    nm->fib_src_hi  = fib_source_allocate("nat64-hi", FIB_SOURCE_PRIORITY_HI, FIB_SOURCE_BH_SIMPLE);
    nm->fib_src_low = fib_source_allocate("nat64-low", FIB_SOURCE_PRIORITY_LOW, FIB_SOURCE_BH_SIMPLE);
...
int nat64_add_del_pool_addr(u32 thread_index, ip4_address_t *addr, u32 vrf_id, u8 is_add)
{
    ...
    if (is_add) {
        if (a) return VNET_API_ERROR_VALUE_EXIST;

        vec_add2(nm->addr_pool, a, 1);
        a->addr      = *addr;
        a->fib_index = ~0;
        if (vrf_id != ~0) a->fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP6, vrf_id, nm->fib_src_hi);
#define _(N, id, n, s)                                                                  \
...
#undef _
        ...
        if (!a) return VNET_API_ERROR_NO_SUCH_ENTRY;

        if (a->fib_index != ~0) fib_table_unlock(a->fib_index, FIB_PROTOCOL_IP6, nm->fib_src_hi);
    ...
    pool_foreach (interface, nm->interfaces) {
        if (nat64_interface_is_inside(interface)) continue;
...
int nat64_add_interface_address(u32 sw_if_index, int is_add)
{
    ...
    for (i = 0; i < vec_len(nm->auto_add_sw_if_indices); i++) {
        if (nm->auto_add_sw_if_indices[i] == sw_if_index) {
            if (is_add)
                ...
                if (first_int_addr) (void) nat64_add_del_pool_addr(vlib_get_thread_index(), first_int_addr, ~0, 0);
    ...
    if (!is_add) return VNET_API_ERROR_NO_SUCH_ENTRY;

    /* add to the auto-address list */
    vec_add1(nm->auto_add_sw_if_indices, sw_if_index);

    /* If the address is already bound - or static - add it now */
    if (first_int_addr) (void) nat64_add_del_pool_addr(vlib_get_thread_index(), first_int_addr, ~0, 1);
...
int nat64_interface_add_del(u32 sw_if_index, u8 is_inside, u8 is_add)
{
    ...
    set_flags:
        ...
        vlib_process_signal_event(vm, nm->expire_walk_node_index, NAT64_CLEANER_RESCHEDULE, 0);
    }
    else {
        if (!interface) return VNET_API_ERROR_NO_SUCH_ENTRY;

        if ((nat64_interface_is_inside(interface) && nat64_interface_is_outside(interface)))
            interface->flags &= is_inside ? ~NAT64_INTERFACE_FLAG_IS_INSIDE : ~NAT64_INTERFACE_FLAG_IS_OUTSIDE;
        else
    ...
    if (nm->num_workers > 1) {
        feature_name = is_inside ? "nat64-in2out-handoff" : "nat64-out2in-handoff";
        if (nm->fq_in2out_index == ~0) nm->fq_in2out_index = vlib_frame_queue_main_init(nat64_in2out_node.index, 0);
        if (nm->fq_out2in_index == ~0) nm->fq_out2in_index = vlib_frame_queue_main_init(nat64_out2in_node.index, 0);
    }
    else
    ...
    case NAT_PROTOCOL_##N:                                                                                             \
        if (a->busy_##n##_ports_per_thread[thread_index] < port_per_thread) {                                          \
            if (a->fib_index == fib_index) {                                                                           \
                while (1) {                                                                                            \
                    portnum = (port_per_thread * nat_thread_index) + nat64_random_port(0, port_per_thread - 1) + 1024; \
                    if (a->busy_##n##_port_refcounts[portnum]) continue;                                               \
...
int nat64_alloc_out_addr_and_port(u32 fib_index, nat_protocol_t proto, ip4_address_t *addr, u16 *port, u32 thread_index)
{
    ...
    if (nm->num_workers > 1) worker_index = thread_index - nm->first_worker_index;

    rv = nat64_alloc_addr_and_port_default(nm->addr_pool, fib_index, thread_index, proto, addr, port, nm->port_per_thread, worker_index);
...
static void nat64_free_out_addr_and_port(struct nat64_db_s *db, ip4_address_t *addr, u16 port, u8 protocol)
{
    ...
    for (i = 0; i < vec_len(nm->addr_pool); i++) {
        a = nm->addr_pool + i;
        if (addr->as_u32 != a->addr.as_u32) continue;
        switch (proto) {
#define _(N, j, n, s)                                                    \
    case NAT_PROTOCOL_##N:                                               \
        ...
        break;
            foreach_nat_protocol
#undef _
                default : nat_elog_str("unknown protocol");
            return;
        }
        break;
    }
...
static uword nat64_static_bib_worker_fn(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    pool_foreach (static_bib, nm->static_bibs) {
        if ((static_bib->thread_index != thread_index) || (static_bib->done)) continue;

        if (static_bib->is_add) {
            (void) nat64_db_bib_entry_create(thread_index,
                                             db,
                                             &static_bib->in_addr,
                                             &static_bib->out_addr,
                                             static_bib->in_port,
            ...
            bibe           = nat64_db_bib_entry_find(db, &addr, static_bib->in_port, static_bib->proto, static_bib->fib_index, 1);
            if (bibe) {
...
int nat64_add_del_static_bib_entry(ip6_address_t *in_addr, ip4_address_t *out_addr, u16 in_port, u16 out_port, u8 proto, u32 vrf_id, u8 is_add)
{
    nat64_main_t                 *nm = &nat64_main;
    nat64_db_bib_entry_t         *bibe;
    u32                           fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP6, vrf_id, nm->fib_src_hi);
    ...
    bibe           = nat64_db_bib_entry_find(db, &addr, clib_host_to_net_u16(in_port), proto, fib_index, 1);

    if (is_add) {
        if (bibe) return VNET_API_ERROR_VALUE_EXIST;

        /* outside port must be assigned to same thread as internall address */
        if ((out_port > 1024) && (nm->num_workers > 1)) {
            if (thread_index != get_thread_idx_by_port(out_port)) return VNET_API_ERROR_INVALID_VALUE_2;
        }

        for (i = 0; i < vec_len(nm->addr_pool); i++) {
            a = nm->addr_pool + i;
            if (out_addr->as_u32 != a->addr.as_u32) continue;
            switch (p) {
#define _(N, j, n, s)                                                                    \
    case NAT_PROTOCOL_##N:                                                               \
        if (a->busy_##n##_port_refcounts[out_port]) return VNET_API_ERROR_INVALID_VALUE; \
        ...
        break;
                foreach_nat_protocol
#undef _
                    default : clib_memset(&addr, 0, sizeof(addr));
                addr.ip4.as_u32 = out_addr->as_u32;
                if (nat64_db_bib_entry_find(db, &addr, 0, proto, fib_index, 0)) return VNET_API_ERROR_INVALID_VALUE;
            }
            break;
        }
        if (!nm->num_workers) {
            bibe = nat64_db_bib_entry_create(thread_index,
                                             db,
                                             in_addr,
                                             out_addr,
                                             ...
                                             proto,
                                             1);
            if (!bibe) return VNET_API_ERROR_UNSPECIFIED;

            vlib_set_simple_counter(&nm->total_bibs, thread_index, 0, db->bib.bib_entries_num);
        }
    }
    else {
        if (!bibe) return VNET_API_ERROR_NO_SUCH_ENTRY;
    ...
    if (nm->num_workers) {
        pool_foreach (static_bib, nm->static_bibs) {
            if (static_bib->done) vec_add1(to_be_free, static_bib - nm->static_bibs);
        }
        ...
        if (worker_vm)
            vlib_node_set_interrupt_pending(worker_vm, nat64_static_bib_worker_node.index);
        else
...
void nat64_tcp_session_set_state(nat64_db_st_entry_t *ste, tcp_header_t *tcp, u8 is_ip6)
{
    switch (ste->tcp_state) {
        ...
        case NAT64_TCP_STATE_V4_INIT: {
            if (is_ip6 && (tcp->flags & TCP_FLAG_SYN)) ste->tcp_state = NAT64_TCP_STATE_ESTABLISHED;
            return;
        }
        case NAT64_TCP_STATE_V6_INIT: {
            if (!is_ip6 && (tcp->flags & TCP_FLAG_SYN)) ste->tcp_state = NAT64_TCP_STATE_ESTABLISHED;
        ...
        case NAT64_TCP_STATE_V4_FIN_RCV: {
            if (is_ip6 && (tcp->flags & TCP_FLAG_FIN)) ste->tcp_state = NAT64_TCP_STATE_V6_FIN_V4_FIN_RCV;
            return;
        }
        case NAT64_TCP_STATE_V6_FIN_RCV: {
            if (!is_ip6 && (tcp->flags & TCP_FLAG_FIN)) ste->tcp_state = NAT64_TCP_STATE_V6_FIN_V4_FIN_RCV;
        ...
        case NAT64_TCP_STATE_TRANS: {
            if (!(tcp->flags & TCP_FLAG_RST)) ste->tcp_state = NAT64_TCP_STATE_ESTABLISHED;
            return;
        }
        default:
            return;
...
int nat64_add_del_prefix(ip6_address_t *prefix, u8 plen, u32 vrf_id, u8 is_add)
{
    ...
    if (plen != 32 && plen != 40 && plen != 48 && plen != 56 && plen != 64 && plen != 96) return VNET_API_ERROR_INVALID_VALUE;
    ...
    if (is_add) {
        if (!p) {
            ...
            p->fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP6, vrf_id, nm->fib_src_hi);
            p->vrf_id    = vrf_id;
        }
...
static uword nat64_expire_walk_fn(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    while (1) {
        ...
        for (i = 0; i < vec_len(worker_vms); i++) {
            ...
            vlib_node_set_interrupt_pending(worker_vm, nm->expire_worker_walk_node_index);
        }
    }
...
void nat64_create_expire_walk_process()
{
    ...
    if (nm->expire_walk_node_index) return;
    nm->expire_walk_node_index = vlib_process_create(vlib_get_main(), "nat64-expire-walk", nat64_expire_walk_fn, 16 /* stack_bytes */);
}
...
int nat64_plugin_enable(nat64_config_t c)
{
    ...
    if (!c.bib_buckets) c.bib_buckets = 1024;

    if (!c.bib_memory_size) c.bib_memory_size = 128 << 20;

    if (!c.st_buckets) c.st_buckets = 2048;

    if (!c.st_memory_size) c.st_memory_size = 256 << 20;
