...
static clib_error_t *nat64_plugin_enable_disable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!unformat_user(input, unformat_line_input, line_input)) return clib_error_return(0, NAT64_EXPECTED_ARGUMENT);
    ...
    if (enable) {
        if (nat64_plugin_enable(c)) error = clib_error_return(0, "plugin enable failed");
    }
    else {
        if (nat64_plugin_disable()) error = clib_error_return(0, "plugin disable failed");
    }
done:
    ...
    for (i = 0; i < count; i++) {
        ...
        switch (rv) {
            case VNET_API_ERROR_NO_SUCH_ENTRY:
                error = clib_error_return(0, "NAT64 pool address %U not exist.", format_ip4_address, &this_addr);
                goto done;
            case VNET_API_ERROR_VALUE_EXIST:
                error = clib_error_return(0, "NAT64 pool address %U exist.", format_ip4_address, &this_addr);
                goto done;
            default:
                break;
        }
...
static int nat64_cli_pool_walk(nat64_address_t *ap, void *ctx)
{
    ...
    if (ap->fib_index != ~0) {
        ...
        if (!fib) return -1;
        vlib_cli_output(vm, " %U tenant VRF: %u", format_ip4_address, &ap->addr, fib->ft_table_id);
    }
    else
        vlib_cli_output(vm, " %U", format_ip4_address, &ap->addr);

#define _(N, i, n, s) vlib_cli_output(vm, "  %d busy %s ports", ap->busy_##n##_ports, s);
...
static clib_error_t *nat64_interface_feature_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (vec_len(inside_sw_if_indices)) {
        for (i = 0; i < vec_len(inside_sw_if_indices); i++) {
            ...
            switch (rv) {
                case VNET_API_ERROR_NO_SUCH_ENTRY:
                    error = clib_error_return(0, "%U NAT64 feature not enabled.", format_vnet_sw_if_index_name, vnm, sw_if_index);
                    goto done;
                case VNET_API_ERROR_VALUE_EXIST:
                    error = clib_error_return(0, "%U NAT64 feature already enabled.", format_vnet_sw_if_index_name, vnm, vnm, sw_if_index);
                    goto done;
                case VNET_API_ERROR_INVALID_VALUE:
                case VNET_API_ERROR_INVALID_VALUE_2:
                    error = clib_error_return(0, "%U NAT64 feature enable/disable failed.", format_vnet_sw_if_index_name, vnm, sw_if_index);
                    goto done;
                default:
                    break;
            }
        ...
        for (i = 0; i < vec_len(outside_sw_if_indices); i++) {
            ...
            switch (rv) {
                case VNET_API_ERROR_NO_SUCH_ENTRY:
                    ...
                    goto done;
                case VNET_API_ERROR_VALUE_EXIST:
                    error = clib_error_return(0, "%U NAT64 feature already enabled.", format_vnet_sw_if_index_name, vnm, sw_if_index);
                    goto done;
                case VNET_API_ERROR_INVALID_VALUE:
                case VNET_API_ERROR_INVALID_VALUE_2:
                    error = clib_error_return(0, "%U NAT64 feature enable/disable failed.", format_vnet_sw_if_index_name, vnm, sw_if_index);
...
static int nat64_cli_interface_walk(nat64_interface_t *i, void *ctx)
{
    ...
    vlib_cli_output(vm,
                    ...
                    vnm,
                    i->sw_if_index,
                    (nat64_interface_is_inside(i) && nat64_interface_is_outside(i)) ? "in out"
                    : nat64_interface_is_inside(i)                                  ? "in"
                                                                                    : "out");
    return 0;
}
...
static clib_error_t *nat64_add_del_static_bib_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%U %u", unformat_ip6_address, &in_addr, &in_port))
            ;
        else if (unformat(line_input, "%U %u", unformat_ip4_address, &out_addr, &out_port))
            ;
        else if (unformat(line_input, "vrf %u", &vrf_id))
            ;
        else if (unformat(line_input, "%U", unformat_nat_protocol, &proto))
            ;
        else if (unformat(line_input, "%U %U %u", unformat_ip6_address, &in_addr, unformat_ip4_address, &out_addr, &protocol))
            p = (u8) protocol;
        else if (unformat(line_input, "del"))
            is_add = 0;
        else {
    ...
    rv = nat64_add_del_static_bib_entry(&in_addr, &out_addr, (u16) in_port, (u16) out_port, p, vrf_id, is_add);
    ...
    switch (rv) {
        ...
        case VNET_API_ERROR_UNSPECIFIED:
            ...
            goto done;
        case VNET_API_ERROR_INVALID_VALUE:
            error = clib_error_return(0, "Outside address %U and port %u already in use.", format_ip4_address, &out_addr, out_port);
            goto done;
        case VNET_API_ERROR_INVALID_VALUE_2:
...
static int nat64_cli_bib_walk(nat64_db_bib_entry_t *bibe, void *ctx)
{
    ...
    switch (bibe->proto) {
        ...
        case IP_PROTOCOL_UDP:
            vlib_cli_output(vm,
                            " %U %u %U %u protocol %U vrf %u %s %u sessions",
                            format_ip6_address,
                            &bibe->in_addr,
                            ...
                            format_nat_protocol,
                            ip_proto_to_nat_proto(bibe->proto),
                            fib->ft_table_id,
                            bibe->is_static ? "static" : "dynamic",
                            bibe->ses_num);
            break;
        default:
            vlib_cli_output(vm,
                            " %U %U protocol %u vrf %u %s %u sessions",
                            format_ip6_address,
                            &bibe->in_addr,
                            format_ip4_address,
                            &bibe->out_addr,
...
static int nat64_cli_st_walk(nat64_db_st_entry_t *ste, void *arg)
{
    ...
    if (ste->proto == IP_PROTOCOL_ICMP)
        ...
        vlib_cli_output(vm,
                        " %U %u %U %u %U %u %U %u protcol %U vrf %u",
                        format_ip6_address,
                        &bibe->in_addr,
                        clib_net_to_host_u16(bibe->in_port),
                        format_ip6_address,
...
static clib_error_t *nat64_add_del_prefix_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (sw_if_index != ~0) {
        u32          fib_index;
        fib_prefix_t fibpfx = {.fp_len = plen, .fp_proto = FIB_PROTOCOL_IP6, .fp_addr = {.ip6 = prefix}};

        if (is_add) {
            fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP6, vrf_id, nm->fib_src_hi);
            ...
            fib_table_entry_path_remove(fib_index, &fibpfx, nm->fib_src_hi, DPO_PROTO_IP6, NULL, sw_if_index, ~0, 1, FIB_ROUTE_PATH_INTF_RX);
...
static int nat64_cli_prefix_walk(nat64_prefix_t *p, void *ctx)
{
    ...
    vlib_cli_output(vm, " %U/%u tenant-vrf %u", format_ip6_address, &p->prefix, p->plen, p->vrf_id);
