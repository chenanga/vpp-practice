...
{
    clib_bihash_init_24_8(&db->bib.in2out, "bib-in2out", c.bib_buckets, c.bib_memory_size);

    clib_bihash_init_24_8(&db->bib.out2in, "bib-out2in", c.bib_buckets, c.bib_memory_size);

    clib_bihash_init_48_8(&db->st.in2out, "st-in2out", c.st_buckets, c.st_memory_size);

    clib_bihash_init_48_8(&db->st.out2in, "st-out2in", c.st_buckets, c.st_memory_size);
...
#define _(N, i, n, s)                         \
    case NAT_PROTOCOL_##N:                    \
        ...
        foreach_nat_protocol
#undef _
            default : pool_get(db->bib._unk_proto_bib, bibe);
    ...
    nat_ipfix_logging_nat64_bib(thread_index, in_addr, out_addr, proto, in_port, out_port, fib->ft_table_id, 1);
    return bibe;
}
...
#define _(N, i, n, s)             \
    case NAT_PROTOCOL_##N:        \
        ...
        foreach_nat_protocol
#undef _
            default : bib = db->bib._unk_proto_bib;
    ...
    if (bibe->is_static) {
        pool_foreach (ste, st) {
            if (ste->bibe_index == bibe_index) vec_add1(ste_to_be_free, ste - st);
        }
        vec_foreach (ste_index, ste_to_be_free)
            nat64_db_st_entry_free(thread_index, db, pool_elt_at_index(st, ste_index[0]));
    ...
    if (!db->addr_free) db->free_addr_port_cb(db, &bibe->out_addr, bibe->out_port, bibe->proto);

    fib_table_t *fib = fib_table_get(bibe->fib_index, FIB_PROTOCOL_IP6);
    nat_ipfix_logging_nat64_bib(thread_index, &bibe->in_addr, &bibe->out_addr, bibe->proto, bibe->in_port, bibe->out_port, fib->ft_table_id, 0);
...
#undef _
    ...
    if (!clib_bihash_search_24_8(is_ip6 ? &db->bib.in2out : &db->bib.out2in, &kv, &value)) bibe = pool_elt_at_index(bib, value.value);
...
#define _(N, i, n, s)          \
    case NAT_PROTOCOL_##N:     \
        ...
        break;
            foreach_nat_protocol
#undef _
                default : st = db->st._unk_proto_st;
            break;
        }
...
#define _(N, i, n, s)                      \
    case NAT_PROTOCOL_##N:                 \
        ...
        foreach_nat_protocol
#undef _
            default : pool_get(db->st._unk_proto_st, ste);
    ...
    fib_table_t *fib = fib_table_get(bibe->fib_index, FIB_PROTOCOL_IP6);
    nat_ipfix_logging_nat64_session(thread_index,
                                    &bibe->in_addr,
                                    &bibe->out_addr,
                                    bibe->proto,
                                    bibe->in_port,
                                    ...
                                    fib->ft_table_id,
                                    1);
    nat_syslog_nat64_sadd(bibe->fib_index,
                          &bibe->in_addr,
                          bibe->in_port,
                          &bibe->out_addr,
                          bibe->out_port,
...
#undef _
    ...
    nat_ipfix_logging_nat64_session(thread_index,
                                    ...
                                    fib->ft_table_id,
                                    0);
    nat_syslog_nat64_sdel(bibe->fib_index,
                          &bibe->in_addr,
                          bibe->in_port,
                          &bibe->out_addr,
                          bibe->out_port,
    ...
    if (!bibe->is_static && !bibe->ses_num) nat64_db_bib_entry_free(thread_index, db, bibe);
}

nat64_db_st_entry_t *
nat64_db_st_entry_find(nat64_db_t *db, ip46_address_t *l_addr, ip46_address_t *r_addr, u16 l_port, u16 r_port, u8 proto, u32 fib_index, u8 is_ip6)
{
...
#undef _
    ...
    if (!clib_bihash_search_48_8(is_ip6 ? &db->st.in2out : &db->st.out2in, &kv, &value)) ste = pool_elt_at_index(st, value.value);
...
#define _(N, i, n, s)                                                                  \
    st = db->st._##n##_st;                                                             \
    pool_foreach (ste, st) {                                                           \
        if (i == NAT_PROTOCOL_TCP && !ste->tcp_state) continue;                        \
        if (ste->expire < now) vec_add1(ste_to_be_free, ste - st);                     \
    }                                                                                  \
    vec_foreach (ste_index, ste_to_be_free)                                            \
        nat64_db_st_entry_free(thread_index, db, pool_elt_at_index(st, ste_index[0])); \
    vec_free(ste_to_be_free);                                                          \
...
#define _(N, i, n, s)                                                                      \
    ...
    pool_foreach (ste, st) {                                                               \
        ...
        if (bibe->out_addr.as_u32 == out_addr->as_u32) vec_add1(ste_to_be_free, ste - st); \
    }                                                                                      \
    vec_foreach (ste_index, ste_to_be_free)                                                \
        nat64_db_st_entry_free(thread_index, db, pool_elt_at_index(st, ste_index[0]));     \
    vec_free(ste_to_be_free);                                                              \
