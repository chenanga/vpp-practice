...
static u8 *format_nat64_out2in_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "NAT64-out2in: sw_if_index %d, next index %d", t->sw_if_index, t->next_index);
...
static int nat64_out2in_tcp_udp(vlib_main_t *vm, vlib_buffer_t *b, nat64_out2in_set_ctx_t *ctx)
{
    ...
    ste = nat64_db_st_entry_find(db, &daddr, &saddr, dport, sport, proto, fib_index, 0);
    if (ste) {
        ...
        ste = nat64_db_st_entry_create(ctx->thread_index, db, bibe, &ip6_saddr, &saddr.ip4, sport);
    ...
    if (PREDICT_FALSE(frag != NULL)) {
        ...
        frag->rsv                      = 0;
        frag->fragment_offset_and_more = ip6_frag_hdr_offset_and_more(frag_offset, 1);
...
static int nat64_out2in_icmp_set_cb(vlib_buffer_t *b, ip4_header_t *ip4, ip6_header_t *ip6, void *arg)
{
    ...
    if (icmp->type == ICMP6_echo_request || icmp->type == ICMP6_echo_reply) {
        ...
        ste        = nat64_db_st_entry_find(db, &daddr, &saddr, out_id, 0, IP_PROTOCOL_ICMP, fib_index, 0);
        ...
        if (ste) {
            ...
            bibe = nat64_db_bib_entry_find(db, &daddr, out_id, IP_PROTOCOL_ICMP, fib_index, 0);
            if (!bibe) return -1;

            nat64_compose_ip6(&ip6_saddr, &ip4->src_address, bibe->fib_index);
            ste = nat64_db_st_entry_create(ctx->thread_index, db, bibe, &ip6_saddr, &saddr.ip4, 0);
        ...
        nat64_compose_ip6(&ip6->src_address, &ip4->src_address, vnet_buffer(ctx->b)->sw_if_index[VLIB_TX]);
...
static int nat64_out2in_inner_icmp_set_cb(vlib_buffer_t *b, ip4_header_t *ip4, ip6_header_t *ip6, void *arg)
{
    ...
    if (proto == IP_PROTOCOL_ICMP6) {
        ...
        ste = nat64_db_st_entry_find(db, &saddr, &daddr, out_id, 0, proto, fib_index, 0);
        if (!ste) return -1;
        ...
        ste = nat64_db_st_entry_find(db, &saddr, &daddr, sport, dport, proto, fib_index, 0);
        if (!ste) return -1;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(proto0 == NAT_PROTOCOL_OTHER)) {
                ...
                vlib_increment_simple_counter(&nm->counters.out2in.other, thread_index, sw_if_index0, 1);
                goto trace0;
            }

            if (proto0 == NAT_PROTOCOL_ICMP) {
                vlib_increment_simple_counter(&nm->counters.out2in.icmp, thread_index, sw_if_index0, 1);
                if (icmp_to_icmp6(b0, nat64_out2in_icmp_set_cb, &ctx0, nat64_out2in_inner_icmp_set_cb, &ctx0)) {
                    next0     = NAT64_OUT2IN_NEXT_DROP;
                    b0->error = node->errors[NAT64_OUT2IN_ERROR_NO_TRANSLATION];
                    goto trace0;
                }
                ...
                if (proto0 == NAT_PROTOCOL_TCP)
                    vlib_increment_simple_counter(&nm->counters.out2in.tcp, thread_index, sw_if_index0, 1);
                else
                    vlib_increment_simple_counter(&nm->counters.out2in.udp, thread_index, sw_if_index0, 1);
                ...
                if (nat64_out2in_tcp_udp(vm, b0, &ctx0)) {
                     ...
                     */
                    if ((proto0 == NAT_PROTOCOL_UDP) && (udp0->dst_port == clib_host_to_net_u16(UDP_DST_PORT_dhcp_to_client))) {
        ...
        trace0:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                nat64_out2in_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                t->sw_if_index          = vnet_buffer(b0)->sw_if_index[VLIB_RX];
                t->next_index           = next0;
            }

            if (next0 == NAT64_OUT2IN_NEXT_DROP) {
                vlib_increment_simple_counter(&nm->counters.out2in.drops, thread_index, sw_if_index0, 1);
            }
...
static u8 *format_nat64_out2in_handoff_trace(u8 *s, va_list *args)
{
    CLIB_UNUSED(vlib_main_t * vm)   = va_arg(*args, vlib_main_t *);
    CLIB_UNUSED(vlib_node_t * node) = va_arg(*args, vlib_node_t *);
    nat64_out2in_handoff_trace_t *t = va_arg(*args, nat64_out2in_handoff_trace_t *);
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b[0]->flags & VLIB_BUFFER_IS_TRACED))) {
            nat64_out2in_handoff_trace_t *t = vlib_add_trace(vm, node, b[0], sizeof(*t));
    ...
    if (n_enq < frame->n_vectors)
        vlib_node_increment_counter(vm, node->node_index, NAT64_OUT2IN_HANDOFF_ERROR_CONGESTION_DROP, frame->n_vectors - n_enq);
    vlib_node_increment_counter(vm, node->node_index, NAT64_OUT2IN_HANDOFF_ERROR_SAME_WORKER, same_worker);
    vlib_node_increment_counter(vm, node->node_index, NAT64_OUT2IN_HANDOFF_ERROR_DO_HANDOFF, do_handoff);
