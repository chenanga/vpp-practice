...
static u8 *format_nat44_ed_out2in_trace(u8 *s, va_list *args)
{
    ...
    tag = t->is_slow_path ? "NAT44_OUT2IN_ED_SLOW_PATH" : "NAT44_OUT2IN_ED_FAST_PATH";
...
static int next_src_nat(snat_main_t *sm, ip4_header_t *ip, u16 src_port, u16 dst_port, u32 rx_fib_index)
{
    ...
    init_ed_k(&kv, ip->src_address.as_u32, src_port, ip->dst_address.as_u32, dst_port, rx_fib_index, ip->protocol);
    if (!clib_bihash_search_16_8(&sm->flow_hash, &kv, &value)) return 1;
...
static inline u32 icmp_out2in_ed_slow_path(snat_main_t         *sm,
                                           vlib_buffer_t       *b,
                                           ip4_header_t        *ip,
                                           ...
                                           snat_session_t     **s_p)
{
    ...
    if (snat_static_mapping_match(vm,
        ...
        if (!sm->forwarding_enabled) {
            ...
            if (!is_interface_addr(sm, node, sw_if_index, ip->dst_address.as_u32)) {
    ...
    if (PREDICT_FALSE(vnet_buffer(b)->ip.reass.icmp_type_or_tcp_flags != ICMP4_echo_reply
                      && (vnet_buffer(b)->ip.reass.icmp_type_or_tcp_flags != ICMP4_echo_request || !is_addr_only))) {
    ...
    s = create_session_for_static_mapping_ed(sm,
                                             b,
                                             sm_addr,
                                             sm_port,
                                             sm_fib_index,
                                             ...
                                             vlib_time_now(vm),
                                             m);
    if (!s) next = NAT_NEXT_DROP;

    if (PREDICT_TRUE(!ip4_is_fragment(ip))) {
        sum = ip_incremental_checksum_buffer(vm, b, (u8 *) icmp - (u8 *) vlib_buffer_get_current(b), ntohs(ip->length) - ip4_header_bytes(ip), 0);
...
static_always_inline int nat44_ed_alloc_i2o_port(snat_main_t    *sm,
                                                 snat_address_t *a,
                                                 snat_session_t *s,
                                                 ip4_address_t   i2o_addr,
                                                 u16             i2o_port,
                                                 ...
                                                 u16            *outside_port)
{
    ...
    for (int i = 0; i < ED_PORT_ALLOC_ATTEMPTS; ++i) {
        portnum = (sm->port_per_thread * snat_thread_index) + snat_random_port(0, sm->port_per_thread - 1) + ED_USER_PORT_OFFSET;
        portnum = clib_host_to_net_u16(portnum);
        nat_6t_i2o_flow_init(sm, thread_index, s, i2o_addr, i2o_port, a->addr, portnum, i2o_fib_index, proto);
        if (!nat_ed_ses_i2o_flow_hash_add_del(sm, thread_index, s, 1 /* is_add */)) {
            *outside_addr = a->addr;
...
static_always_inline int nat44_ed_alloc_i2o_addr_and_port(snat_main_t    *sm,
    ...
    if (vec_len(addresses) > 0) {
        ...
        for (i = s_addr_offset; i < vec_len(addresses); ++i) {
            ...
            if (a->fib_index == i2o_fib_index) {
                return nat44_ed_alloc_i2o_port(sm,
                                               a,
                                               s,
                                               i2o_addr,
                                               i2o_port,
                                                            ...
                                                            snat_static_mapping_t *mapping)
{
    ...
    if (PREDICT_FALSE(nat44_ed_maximum_sessions_exceeded(sm, o2i_fib_index, thread_index))) {
    ...
    s->ext_host_port        = proto == IP_PROTOCOL_ICMP ? 0 : vnet_buffer(b)->ip.reass.l4_src_port;
    s->flags |= SNAT_SESSION_FLAG_STATIC_MAPPING;
    if (lb_nat) s->flags |= SNAT_SESSION_FLAG_LOAD_BALANCING;
    if (lb_nat == AFFINITY_LB_NAT) s->flags |= SNAT_SESSION_FLAG_AFFINITY;
    ...
    if (IP_PROTOCOL_ICMP == proto) {
        nat_6t_o2i_flow_init(sm, thread_index, s, s->ext_host_addr, o2i_port, o2i_addr, o2i_port, o2i_fib_index, ip->protocol);
        nat_6t_flow_icmp_id_rewrite_set(&s->o2i, i2o_port);
    }
    else {
        nat_6t_o2i_flow_init(sm, thread_index, s, s->ext_host_addr, s->ext_host_port, o2i_addr, o2i_port, o2i_fib_index, ip->protocol);
    ...
    if (twice_nat == TWICE_NAT || (twice_nat == TWICE_NAT_SELF && ip->src_address.as_u32 == i2o_addr.as_u32)) {
        ...
        if (filter) {
            ...
            rc = nat44_ed_alloc_i2o_addr_and_port(sm,
                                                  sm->twice_nat_addresses,
                                                  s,
                                                  i2o_addr,
                                                  i2o_port,
                                                  i2o_fib_index,
        ...
        if (IP_PROTOCOL_ICMP == proto) {
            nat_6t_i2o_flow_init(sm, thread_index, s, i2o_addr, i2o_port, s->ext_host_addr, i2o_port, i2o_fib_index, ip->protocol);
        }
        else {
            nat_6t_i2o_flow_init(sm, thread_index, s, i2o_addr, i2o_port, s->ext_host_addr, s->ext_host_port, i2o_fib_index, ip->protocol);
        }
...
static void create_bypass_for_fwd(snat_main_t *sm, vlib_buffer_t *b, snat_session_t *s, ip4_header_t *ip, u32 rx_fib_index, u32 thread_index)
{
    ...
    if (ip->protocol == IP_PROTOCOL_ICMP) {
        if (nat_get_icmp_session_lookup_values(b, ip, &lookup_daddr, &lookup_sport, &lookup_saddr, &lookup_dport, &lookup_protocol)) return;
    }
    else {
    ...
    init_ed_k(&kv, lookup_saddr.as_u32, lookup_sport, lookup_daddr.as_u32, lookup_dport, rx_fib_index, lookup_protocol);
    ...
    if (!clib_bihash_search_16_8(&sm->flow_hash, &kv, &value)) {
    ...
    else if (ip->protocol == IP_PROTOCOL_ICMP && icmp_type_is_error_message(vnet_buffer(b)->ip.reass.icmp_type_or_tcp_flags)) {
        return;
    }
    else {
        if (PREDICT_FALSE(nat44_ed_maximum_sessions_exceeded(sm, rx_fib_index, thread_index))) return;
        ...
        nat_6t_i2o_flow_init(sm, thread_index, s, ip->dst_address, lookup_sport, ip->src_address, lookup_dport, rx_fib_index, ip->protocol);
    ...
    if (ip->protocol == IP_PROTOCOL_TCP) {
        nat44_set_tcp_session_state_o2i(sm, now, s, vnet_buffer(b)->ip.reass.icmp_type_or_tcp_flags, thread_index);
    }
                                                              ...
                                                              vlib_node_runtime_t *node)
{
    ...
    nat_6t_o2i_flow_init(sm, thread_index, s, ip->dst_address, 0, ip->src_address, 0, m->fib_index, ip->protocol);
    ...
    nat_6t_o2i_flow_init(sm, thread_index, s, ip->src_address, 0, ip->dst_address, 0, rx_fib_index, ip->protocol);
...
static inline uword nat44_ed_out2in_fast_path_node_fn_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_multi_worker)
{
    ...
    while (n_left_from > 0) {
        ...
        if (is_multi_worker && !pool_is_free_index(tsm->sessions, vnet_buffer2(b0)->nat.cached_session_index)) {
            ...
            if (PREDICT_TRUE(nat_6t_t_eq(&s0->o2i.match, &lookup))
                || (s0->flags & SNAT_SESSION_FLAG_TWICE_NAT && nat_6t_t_eq(&s0->i2o.match, &lookup))) {
    ...
    skip_lookup:
        ...
        if (nat_6t_t_eq(&s0->o2i.match, &lookup)) {
        ...
        else if (s0->flags & SNAT_SESSION_FLAG_TWICE_NAT && nat_6t_t_eq(&s0->i2o.match, &lookup)) {
            f = &s0->i2o;
        }
        else {
            /*
            ...
            if (PREDICT_FALSE(proto0 == IP_PROTOCOL_UDP
                              && (vnet_buffer(b0)->ip.reass.l4_dst_port == clib_host_to_net_u16(UDP_DST_PORT_dhcp_to_client)))) {
                goto trace0;
            }
        ...
        if (NAT_ED_TRNSL_ERR_SUCCESS != (translation_error = nat_6t_flow_buf_translate_o2i(vm, sm, b0, ip0, f, proto0, 0 /* is_output_feature */))) {
            next[0]   = NAT_NEXT_DROP;
            b0->error = node->errors[NAT_OUT2IN_ED_ERROR_TRNSL_FAILED];
            goto trace0;
        }
        ...
        switch (proto0) {
            case IP_PROTOCOL_TCP:
                vlib_increment_simple_counter(&sm->counters.fastpath.out2in.tcp, thread_index, sw_if_index0, 1);
                nat44_set_tcp_session_state_o2i(sm, now, s0, vnet_buffer(b0)->ip.reass.icmp_type_or_tcp_flags, thread_index);
                break;
            case IP_PROTOCOL_UDP:
                vlib_increment_simple_counter(&sm->counters.fastpath.out2in.udp, thread_index, sw_if_index0, 1);
                break;
            case IP_PROTOCOL_ICMP:
                vlib_increment_simple_counter(&sm->counters.fastpath.out2in.icmp, thread_index, sw_if_index0, 1);
                break;
            default:
                vlib_increment_simple_counter(&sm->counters.fastpath.out2in.other, thread_index, sw_if_index0, 1);
                break;
        }
    ...
    trace0:
        if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
            nat44_ed_out2in_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
        ...
        if (next[0] == NAT_NEXT_DROP) {
            vlib_increment_simple_counter(&sm->counters.fastpath.out2in.drops, thread_index, sw_if_index0, 1);
        }
...
static inline uword nat44_ed_out2in_slow_path_node_fn_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32                          n_left_from, *from;
    snat_main_t                 *sm           = &snat_main;
    f64                          now          = vlib_time_now(vm);
    ...
    while (n_left_from > 0) {
        ...
        if (PREDICT_FALSE(nat44_ed_is_unk_proto(proto0))) {
            s0 = nat44_ed_out2in_slowpath_unknown_proto(sm, b0, ip0, rx_fib_index0, thread_index, now, vm, node);
            if (!sm->forwarding_enabled) {
            ...
            vlib_increment_simple_counter(&sm->counters.slowpath.out2in.other, thread_index, sw_if_index0, 1);
            goto trace0;
        }

        if (PREDICT_FALSE(proto0 == IP_PROTOCOL_ICMP)) {
            next[0] = icmp_out2in_ed_slow_path(sm, b0, ip0, icmp0, sw_if_index0, rx_fib_index0, node, next[0], now, thread_index, &s0);
            ...
            if (NAT_NEXT_DROP != next[0]) {
                vlib_increment_simple_counter(&sm->counters.slowpath.out2in.icmp, thread_index, sw_if_index0, 1);
            }
            goto trace0;
        }
        ...
        if (!s0) {
            ...
            if (snat_static_mapping_match(vm,
                ...
                if (!sm->forwarding_enabled) {
                    ...
                    if (next_src_nat(sm, ip0, vnet_buffer(b0)->ip.reass.l4_src_port, vnet_buffer(b0)->ip.reass.l4_dst_port, rx_fib_index0)) {
                        ...
                        create_bypass_for_fwd(sm, b0, s0, ip0, rx_fib_index0, thread_index);
                    }
                }
                goto trace0;
            }

            if (PREDICT_FALSE(identity_nat0)) goto trace0;

            if ((proto0 == IP_PROTOCOL_TCP) && !tcp_flags_is_init(vnet_buffer(b0)->ip.reass.icmp_type_or_tcp_flags)) {
            ...
            /* Create session initiated by host from external network */
            s0 = create_session_for_static_mapping_ed(sm,
                                                      b0,
                                                      sm_addr,
                                                      sm_port,
                                                      sm_fib_index,
        ...
        if (PREDICT_TRUE(proto0 == IP_PROTOCOL_TCP)) {
            vlib_increment_simple_counter(&sm->counters.slowpath.out2in.tcp, thread_index, sw_if_index0, 1);
            nat44_set_tcp_session_state_o2i(sm, now, s0, vnet_buffer(b0)->ip.reass.icmp_type_or_tcp_flags, thread_index);
        }
        else {
            vlib_increment_simple_counter(&sm->counters.slowpath.out2in.udp, thread_index, sw_if_index0, 1);
        }
    ...
    trace0:
        ...
        if (next[0] == NAT_NEXT_DROP) {
            vlib_increment_simple_counter(&sm->counters.slowpath.out2in.drops, thread_index, sw_if_index0, 1);
        }
...
VLIB_NODE_FN(nat44_ed_out2in_node)
...
VLIB_NODE_FN(nat44_ed_out2in_slowpath_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
...
static u8 *format_nat_pre_trace(u8 *s, va_list *args)
{
    ...
    return format(s, "out2in next_index %d arc_next_index %d", t->next_index, t->arc_next_index);
}

VLIB_NODE_FN(nat_pre_out2in_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return nat_pre_node_fn_inline(vm, node, frame, NAT_NEXT_OUT2IN_ED_FAST_PATH);
}
