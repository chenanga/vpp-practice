...
static void vl_api_nat44_ed_plugin_enable_disable_t_handler(vl_api_nat44_ed_plugin_enable_disable_t *mp)
{
    ...
    if (mp->enable) {
        if ((mp->flags & NAT44_API_IS_STATIC_MAPPING_ONLY) || (mp->flags & NAT44_API_IS_CONNECTION_TRACKING)) {
...
static void send_nat_worker_details(u32 worker_index, vl_api_registration_t *reg, u32 context)
{
    ...
    snat_main_t                 *sm = &snat_main;
    vlib_worker_thread_t        *w  = vlib_worker_threads + worker_index + sm->first_worker_index;
...
static void vl_api_nat_ipfix_enable_disable_t_handler(vl_api_nat_ipfix_enable_disable_t *mp)
{
    ...
    rv = nat_ipfix_logging_enable_disable(mp->enable, clib_host_to_net_u32(mp->domain_id), clib_host_to_net_u16(mp->src_port));
...
static void vl_api_nat44_add_del_address_range_t_handler(vl_api_nat44_add_del_address_range_t *mp)
{
    ...
    if (count > 1024)
        nat_log_info("%U - %U, %d addresses...", format_ip4_address, mp->first_ip_address, format_ip4_address, mp->last_ip_address, count);
...
static void send_nat44_address_details(snat_address_t *a, vl_api_registration_t *reg, u32 context, u8 twice_nat)
{
    ...
    if (a->fib_index != ~0) {
        ...
        rmp->vrf_id = ~0;
    if (twice_nat) rmp->flags |= NAT_API_IS_TWICE_NAT;
    rmp->context = context;
...
static void send_nat44_interface_details(snat_interface_t *i, vl_api_registration_t *reg, u32 context)
{
    ...
    if (nat44_ed_is_interface_inside(i)) rmp->flags |= NAT_API_IS_INSIDE;
    if (nat44_ed_is_interface_outside(i)) rmp->flags |= NAT_API_IS_OUTSIDE;
...
static void vl_api_nat44_ed_add_del_output_interface_t_handler(vl_api_nat44_ed_add_del_output_interface_t *mp)
{
...
#undef vl_endianfun
static void send_nat44_ed_output_interface_details(u32 index, vl_api_registration_t *rp, u32 context)
{
    snat_main_t                                *sm = &snat_main;
    vl_api_nat44_ed_output_interface_details_t *rmp;
    snat_interface_t                           *i = pool_elt_at_index(sm->output_feature_interfaces, index);

    /* Make sure every field is initiated (or don't skip the clib_memset()) */
    REPLY_MACRO_DETAILS4(VL_API_NAT44_ED_OUTPUT_INTERFACE_DETAILS, rp, context, ({
                             rmp->sw_if_index = i->sw_if_index;

                             /* Endian hack until apigen registers _details
                              * endian functions */
                             vl_api_nat44_ed_output_interface_details_t_endian(rmp, 1 /* to network */);
                             rmp->_vl_msg_id = htons(rmp->_vl_msg_id);
                             rmp->context    = htonl(rmp->context);
                         }));
}

static void vl_api_nat44_ed_output_interface_get_t_handler(vl_api_nat44_ed_output_interface_get_t *mp)
{
    ...
    REPLY_AND_DETAILS_MACRO(VL_API_NAT44_ED_OUTPUT_INTERFACE_GET_REPLY, sm->output_feature_interfaces, ({
...
static void vl_api_nat44_add_del_static_mapping_t_handler(vl_api_nat44_add_del_static_mapping_t *mp)
{
    ...
    if (mp->is_add) {
        ...
        rv = nat44_ed_add_static_mapping(l_addr, e_addr, l_port, e_port, proto, vrf_id, sw_if_index, flags, pool_addr, tag);
        vec_free(tag);
    }
    else {
...
static void send_nat44_static_mapping_details(snat_static_mapping_t *m, vl_api_registration_t *reg, u32 context)
{
    ...
    rmp->_vl_msg_id = ntohs(VL_API_NAT44_STATIC_MAPPING_DETAILS + sm->msg_id_base);
    ...
    if (m->tag) strncpy((char *) rmp->tag, (char *) m->tag, vec_len(m->tag));

    vl_api_send_msg(reg, (u8 *) rmp);
}

static void send_nat44_static_map_resolve_details(snat_static_mapping_resolve_t *m, vl_api_registration_t *reg, u32 context)
{
    vl_api_nat44_static_mapping_details_t *rmp;
    snat_main_t                           *sm = &snat_main;
...
static void vl_api_nat44_static_mapping_dump_t_handler(vl_api_nat44_static_mapping_dump_t *mp)
{
    ...
    pool_foreach (m, sm->static_mappings) {
        if (!is_sm_identity_nat(m->flags) && !is_sm_lb(m->flags)) send_nat44_static_mapping_details(m, reg, mp->context);
    }

    for (j = 0; j < vec_len(sm->sm_to_resolve); j++) {
        rp = sm->sm_to_resolve + j;
        if (!is_sm_identity_nat(rp->flags)) send_nat44_static_map_resolve_details(rp, reg, mp->context);
    }
}
...
static void vl_api_nat44_add_del_identity_mapping_t_handler(vl_api_nat44_add_del_identity_mapping_t *mp)
{
    ...
    if (mp->is_add) {
        ...
        rv = nat44_ed_add_static_mapping(addr, addr, port, port, proto, vrf_id, sw_if_index, flags, pool_addr, tag);
        vec_free(tag);
    }
    else {
...
static void send_nat44_identity_mapping_details(snat_static_mapping_t *m, int index, vl_api_registration_t *reg, u32 context)
{
    ...
    rmp = vl_msg_api_alloc(sizeof(*rmp));
    clib_memset(rmp, 0, sizeof(*rmp));
    rmp->_vl_msg_id = ntohs(VL_API_NAT44_IDENTITY_MAPPING_DETAILS + sm->msg_id_base);

    if (is_sm_addr_only(m->flags)) rmp->flags |= NAT_API_IS_ADDR_ONLY;
...
static void send_nat44_identity_map_resolve_details(snat_static_mapping_resolve_t *m, vl_api_registration_t *reg, u32 context)
{
    vl_api_nat44_identity_mapping_details_t *rmp;
    snat_main_t                             *sm = &snat_main;
    ...
    if (is_sm_addr_only(m->flags)) rmp->flags = (vl_api_nat_config_flags_t) NAT_API_IS_ADDR_ONLY;
...
static void vl_api_nat44_identity_mapping_dump_t_handler(vl_api_nat44_identity_mapping_dump_t *mp)
{
    ...
    for (j = 0; j < vec_len(sm->sm_to_resolve); j++) {
        rp = sm->sm_to_resolve + j;
        if (is_sm_identity_nat(rp->flags)) send_nat44_identity_map_resolve_details(rp, reg, mp->context);
    }
}
...
static void send_nat44_interface_addr_details(u32 sw_if_index, vl_api_registration_t *reg, u32 context, u8 twice_nat)
{
    ...
    rmp = vl_msg_api_alloc(sizeof(*rmp));
    clib_memset(rmp, 0, sizeof(*rmp));
    rmp->_vl_msg_id  = ntohs(VL_API_NAT44_INTERFACE_ADDR_DETAILS + sm->msg_id_base);
    rmp->sw_if_index = ntohl(sw_if_index);

    if (twice_nat) rmp->flags = (vl_api_nat_config_flags_t) NAT_API_IS_TWICE_NAT;
    rmp->context = context;
...
static void vl_api_nat44_interface_addr_dump_t_handler(vl_api_nat44_interface_addr_dump_t *mp)
{
    ...
    vec_foreach (ap, sm->addr_to_resolve) {
        send_nat44_interface_addr_details(ap->sw_if_index, reg, mp->context, ap->is_twice_nat);
    }
}
...
static void vl_api_nat44_add_del_lb_static_mapping_t_handler(vl_api_nat44_add_del_lb_static_mapping_t *mp)
{
    ...
    locals = unformat_nat44_lb_addr_port(mp->locals, clib_net_to_host_u32(mp->local_num));
    ...
    if (mp->is_add) {
        ...
        rv = nat44_ed_del_lb_static_mapping(e_addr, mp->external_port, proto, flags);
    }
...
static void vl_api_nat44_lb_static_mapping_add_del_local_t_handler(vl_api_nat44_lb_static_mapping_add_del_local_t *mp)
{
    ...
    rv = nat44_ed_add_del_lb_static_mapping_local(e_addr,
                                                  mp->external_port,
                                                  l_addr,
                                                  mp->local.port,
                                                  proto,
                                                  clib_net_to_host_u32(mp->local.vrf_id),
                                                  mp->local.probability,
                                                  mp->is_add);
...
static void send_nat44_lb_static_mapping_details(snat_static_mapping_t *m, vl_api_registration_t *reg, u32 context)
{
    ...
    rmp = vl_msg_api_alloc(sizeof(*rmp) + (pool_elts(m->locals) * sizeof(nat44_lb_addr_port_t)));

    clib_memset(rmp, 0, sizeof(*rmp));
    rmp->_vl_msg_id = ntohs(VL_API_NAT44_LB_STATIC_MAPPING_DETAILS + sm->msg_id_base);
...
static void vl_api_nat44_lb_static_mapping_dump_t_handler(vl_api_nat44_lb_static_mapping_dump_t *mp)
{
    ...
    pool_foreach (m, sm->static_mappings) {
        if (is_sm_lb(m->flags)) send_nat44_lb_static_mapping_details(m, reg, mp->context);
    }
}
...
static void vl_api_nat44_del_session_t_handler(vl_api_nat44_del_session_t *mp)
{
    ...
    rv = nat44_ed_del_session(sm, &addr, port, &eh_addr, eh_port, mp->protocol, vrf_id, is_in);
...
static void vl_api_nat44_ed_add_del_vrf_table_t_handler(vl_api_nat44_ed_add_del_vrf_table_t *mp)
{
    snat_main_t                               *sm = &snat_main;
    vl_api_nat44_ed_add_del_vrf_table_reply_t *rmp;
    int                                        rv = nat44_ed_add_del_vrf_table(clib_net_to_host_u32(mp->table_vrf_id), mp->is_add);
    REPLY_MACRO(VL_API_NAT44_ED_ADD_DEL_VRF_TABLE_REPLY);
}

static void vl_api_nat44_ed_add_del_vrf_route_t_handler(vl_api_nat44_ed_add_del_vrf_route_t *mp)
{
    snat_main_t                               *sm = &snat_main;
    vl_api_nat44_ed_add_del_vrf_route_reply_t *rmp;
    int rv = nat44_ed_add_del_vrf_route(clib_net_to_host_u32(mp->table_vrf_id), clib_net_to_host_u32(mp->vrf_id), mp->is_add);
    REPLY_MACRO(VL_API_NAT44_ED_ADD_DEL_VRF_ROUTE_REPLY);
}

static void nat44_ed_vrf_tables_send_details(vl_api_registration_t *rp, u32 context, vrf_table_t *t)
{
    snat_main_t                          *sm = &snat_main;
    ...
    mp             = vl_msg_api_alloc_zero(sizeof(*mp) + sizeof(mp->vrf_ids[0]) * vec_len(t->routes));
    mp->_vl_msg_id = ntohs(VL_API_NAT44_ED_VRF_TABLES_DETAILS + sm->msg_id_base);
    mp->context    = context;
    ...
    // copy the records
    clib_memcpy(mp->vrf_ids, vrf_ids, sizeof(mp->vrf_ids[0]) * vec_len(t->routes));
...
static void nat44_ed_vrf_tables_send_details_v2(vl_api_registration_t *rp, u32 context, vrf_table_t *t)
{
    snat_main_t                             *sm = &snat_main;
    ...
    mp               = vl_msg_api_alloc_zero(sizeof(*mp) + sizeof(mp->vrf_ids[0]) * vec_len(t->routes));
    mp->_vl_msg_id   = clib_net_to_host_u16(VL_API_NAT44_ED_VRF_TABLES_DETAILS + sm->msg_id_base);
    mp->context      = context;
...
static void vl_api_nat44_ed_vrf_tables_dump_t_handler(vl_api_nat44_ed_vrf_tables_dump_t *mp)
{
    snat_main_t           *sm = &snat_main;
...
static void vl_api_nat44_ed_vrf_tables_v2_dump_t_handler(vl_api_nat44_ed_vrf_tables_v2_dump_t *mp)
{
    snat_main_t           *sm = &snat_main;
    vl_api_registration_t *rp;
    vrf_table_t           *t;
...
static void send_nat44_user_session_details(snat_session_t *s, vl_api_registration_t *reg, u32 context)
{
    ...
    rmp = vl_msg_api_alloc(sizeof(*rmp));
    clib_memset(rmp, 0, sizeof(*rmp));
    rmp->_vl_msg_id = ntohs(VL_API_NAT44_USER_SESSION_DETAILS + sm->msg_id_base);
    clib_memcpy(rmp->outside_ip_address, (&s->out2in.addr), 4);
    clib_memcpy(rmp->inside_ip_address, (&s->in2out.addr), 4);

    if (nat44_ed_is_session_static(s)) rmp->flags |= NAT_API_IS_STATIC;

    if (nat44_ed_is_twice_nat_session(s)) rmp->flags |= NAT_API_IS_TWICE_NAT;
...
static void vl_api_nat44_user_session_dump_t_handler(vl_api_nat44_user_session_dump_t *mp)
{
    ...
    if (sm->num_workers > 1)
        tsm = vec_elt_at_index(sm->per_thread_data, nat44_ed_get_in2out_worker_index(0, &ip, ukey.fib_index, 0));
    else
...
static void send_nat44_user_session_v2_details(snat_session_t *s, vl_api_registration_t *reg, u32 context)
{
    ...
    rmp = vl_msg_api_alloc(sizeof(*rmp));
    clib_memset(rmp, 0, sizeof(*rmp));
    rmp->_vl_msg_id = ntohs(VL_API_NAT44_USER_SESSION_V2_DETAILS + sm->msg_id_base);
    clib_memcpy(rmp->outside_ip_address, (&s->out2in.addr), 4);
    clib_memcpy(rmp->inside_ip_address, (&s->in2out.addr), 4);
...
static void send_nat44_user_session_v3_details(snat_session_t *s, vl_api_registration_t *reg, u32 context)
{
    ...
    rmp = vl_msg_api_alloc(sizeof(*rmp));
    clib_memset(rmp, 0, sizeof(*rmp));
    rmp->_vl_msg_id = ntohs(VL_API_NAT44_USER_SESSION_V3_DETAILS + sm->msg_id_base);
    clib_memcpy(rmp->outside_ip_address, (&s->out2in.addr), 4);
    clib_memcpy(rmp->inside_ip_address, (&s->in2out.addr), 4);
    ...
    rmp->last_heard            = clib_host_to_net_u64((u64) s->last_heard);
    rmp->time_since_last_heard = clib_host_to_net_u64((u64) (now - s->last_heard));
    rmp->total_bytes           = clib_host_to_net_u64(s->total_bytes);
    rmp->total_pkts            = ntohl(s->total_pkts);
    rmp->context               = context;
    rmp->outside_port          = s->out2in.port;
...
static void vl_api_nat44_user_session_v2_dump_t_handler(vl_api_nat44_user_session_v2_dump_t *mp)
{
    snat_main_per_thread_data_t *tsm;
    snat_main_t                 *sm = &snat_main;
    vl_api_registration_t       *reg;
...
static void vl_api_nat44_user_session_v3_dump_t_handler(vl_api_nat44_user_session_v3_dump_t *mp)
{
    snat_main_per_thread_data_t *tsm;
    snat_main_t                 *sm = &snat_main;
    vl_api_registration_t       *reg;
