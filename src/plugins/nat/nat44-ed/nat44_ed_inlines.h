...
always_inline void init_ed_k(clib_bihash_kv_16_8_t *kv, u32 l_addr, u16 l_port, u32 r_addr, u16 r_port, u32 fib_index, ip_protocol_t proto)
{
    kv->key[0] = (u64) r_addr << 32 | l_addr;
    kv->key[1] = (u64) r_port << 48 | (u64) l_port << 32 | fib_index << 8 | proto;
}

always_inline void
init_ed_kv(clib_bihash_kv_16_8_t *kv, u32 l_addr, u16 l_port, u32 r_addr, u16 r_port, u32 fib_index, u8 proto, u32 thread_index, u32 session_index)
{
    init_ed_k(kv, l_addr, l_port, r_addr, r_port, fib_index, proto);
    kv->value = (u64) thread_index << 32 | session_index;
}

always_inline void nat44_ed_sm_init_i2o_kv(clib_bihash_kv_16_8_t *kv, u32 addr, u16 port, u32 fib_index, u8 proto, u32 sm_index)
{
    return init_ed_kv(kv, addr, port, 0, 0, fib_index, proto, 0, sm_index);
}

always_inline void nat44_ed_sm_init_o2i_kv(clib_bihash_kv_16_8_t *kv, u32 e_addr, u16 e_port, u32 fib_index, u8 proto, u32 sm_index)
{
    return init_ed_kv(kv, 0, 0, e_addr, e_port, fib_index, proto, 0, sm_index);
}

always_inline void nat44_ed_sm_init_i2o_k(clib_bihash_kv_16_8_t *kv, u32 addr, u16 port, u32 fib_index, u8 proto)
{
    return nat44_ed_sm_init_i2o_kv(kv, addr, port, fib_index, proto, 0);
}

always_inline void nat44_ed_sm_init_o2i_k(clib_bihash_kv_16_8_t *kv, u32 e_addr, u16 e_port, u32 fib_index, u8 proto)
{
    return nat44_ed_sm_init_o2i_kv(kv, e_addr, e_port, fib_index, proto, 0);
}

always_inline u32 ed_value_get_thread_index(clib_bihash_kv_16_8_t *value)
{
    return value->value >> 32;
}

always_inline u32 ed_value_get_session_index(clib_bihash_kv_16_8_t *value)
{
    return value->value & ~(u32) 0;
}

always_inline void
split_ed_kv(clib_bihash_kv_16_8_t *kv, ip4_address_t *l_addr, ip4_address_t *r_addr, u8 *proto, u32 *fib_index, u16 *l_port, u16 *r_port)
{
    if (l_addr) {
...
always_inline int nat44_ed_tcp_is_established(nat44_ed_tcp_state_e state)
{
    return state == NAT44_ED_TCP_STATE_ESTABLISHED ? 1 : 0;
}

always_inline u32 nat44_session_get_timeout(snat_main_t *sm, snat_session_t *s)
{
    switch (s->proto) {
        case IP_PROTOCOL_ICMP:
            /* fallthrough */
...
static_always_inline u8 nat44_ed_maximum_sessions_exceeded(snat_main_t *sm, u32 fib_index, u32 thread_index)
{
    ...
    if (vec_len(sm->max_translations_per_fib) <= fib_index) fib_index = 0;
...
{
    init_ed_k(kv, f->match.saddr.as_u32, f->match.sport, f->match.daddr.as_u32, f->match.dport, f->match.fib_index, f->match.proto);
}
...
static_always_inline int nat_ed_ses_o2i_flow_hash_add_del(snat_main_t *sm, u32 thread_idx, snat_session_t *s, int is_add)
{
    ...
    if (0 == is_add) {
        ...
        if (!(s->flags & SNAT_SESSION_FLAG_STATIC_MAPPING)) {
            if (nat44_ed_sm_o2i_lookup(sm, s->o2i.match.daddr, s->o2i.match.dport, 0, s->o2i.match.proto)) {
                return -1;
            }
        }
...
always_inline void nat_ed_session_delete(snat_main_t *sm, snat_session_t *ses, u32 thread_index, int lru_delete
                                         /* delete from global LRU list */)
{
    snat_main_per_thread_data_t *tsm = vec_elt_at_index(sm->per_thread_data, thread_index);

    if (lru_delete) {
        clib_dlist_remove(tsm->lru_pool, ses->lru_index);
    }
    pool_put_index(tsm->lru_pool, ses->lru_index);
    if (nat_ed_ses_i2o_flow_hash_add_del(sm, thread_index, ses, 0)) nat_elog_warn(sm, "flow hash del failed");
    if (nat_ed_ses_o2i_flow_hash_add_del(sm, thread_index, ses, 0)) nat_elog_warn(sm, "flow hash del failed");
    pool_put(tsm->sessions, ses);
    vlib_set_simple_counter(&sm->total_sessions, thread_index, 0, pool_elts(tsm->sessions));
}
...
static_always_inline int nat_lru_free_one(snat_main_t *sm, int thread_index, f64 now)
{
    ...
    int                          rc  = 0;
#define _(p)                                                                                 \
    if ((rc = nat_lru_free_one_with_head(sm, thread_index, now, tsm->p##_lru_head_index))) { \
        return rc;                                                                           \
    }
    _(tcp_trans);
    _(udp);
...
static_always_inline void per_vrf_sessions_cleanup(u32 thread_index)
{
    ...
    pool_foreach (per_vrf_sessions, tsm->per_vrf_sessions_pool) {
        if (per_vrf_sessions->expired && per_vrf_sessions->ses_count == 0) vec_add1(to_free, per_vrf_sessions - tsm->per_vrf_sessions_pool);
    }
...
static_always_inline void per_vrf_sessions_register_session(snat_session_t *s, u32 thread_index)
{
    ...
    pool_foreach (per_vrf_sessions, tsm->per_vrf_sessions_pool) {
        ...
        if (per_vrf_sessions->expired) continue;

        if ((s->in2out.fib_index == per_vrf_sessions->rx_fib_index) && (s->out2in.fib_index == per_vrf_sessions->tx_fib_index)) {
            goto done;
        }
        if ((s->in2out.fib_index == per_vrf_sessions->tx_fib_index) && (s->out2in.fib_index == per_vrf_sessions->rx_fib_index)) {
            goto done;
        }
    }
...
static_always_inline void per_vrf_sessions_unregister_session(snat_session_t *s, u32 thread_index)
{
    ...
    per_vrf_sessions = pool_elt_at_index(tsm->per_vrf_sessions_pool, s->per_vrf_sessions_index);
...
{
    snat_main_per_thread_data_t *tsm = vec_elt_at_index(sm->per_thread_data, thread_idx);
    nat_6t_flow_init(&s->i2o, thread_idx, saddr, sport, daddr, dport, fib_index, proto, s - tsm->sessions);
}
...
{
    snat_main_per_thread_data_t *tsm = vec_elt_at_index(sm->per_thread_data, thread_idx);
    nat_6t_flow_init(&s->o2i, thread_idx, saddr, sport, daddr, dport, fib_index, proto, s - tsm->sessions);
}
...
static inline uword nat_pre_node_fn_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u32 def_next)
{
...
always_inline u8 is_interface_addr(snat_main_t *sm, vlib_node_runtime_t *node, u32 sw_if_index0, u32 ip4_addr)
{
    ...
    if (PREDICT_FALSE(rt->cached_sw_if_index != sw_if_index0)) {
        ...
        foreach_ip_interface_address(lm, ia, sw_if_index0, 1 /* honor unnumbered */, ({
                                         a = ip_interface_address_get_address(lm, ia);
                                         hash_set(rt->cached_presence_by_ip4_address, a->as_u32, 1);
                                         rt->cached_sw_if_index = sw_if_index0;
                                     }));

        if (rt->cached_sw_if_index == ~0) return 0;
    }
...
always_inline void nat44_ed_session_reopen(u32 thread_index, snat_session_t *s)
{
 ...
 */
always_inline void nat44_set_tcp_session_state(snat_main_t *sm, f64 now, snat_session_t *ses, u8 tcp_flags, u32 thread_index, nat44_ed_dir_e dir)
{
    snat_main_per_thread_data_t *tsm       = &sm->per_thread_data[thread_index];
    u8                           old_flags = ses->tcp_flags[dir];
    ses->tcp_flags[dir] |= tcp_flags & (TCP_FLAG_FIN | TCP_FLAG_SYN | TCP_FLAG_RST | TCP_FLAG_ACK);
    if (old_flags == ses->tcp_flags[dir]) return;
    ...
    switch (old_state) {
        case NAT44_ED_TCP_STATE_CLOSED:
            ...
            if ((ses->tcp_flags[NAT44_ED_DIR_I2O] & ses->tcp_flags[NAT44_ED_DIR_O2I]) == (TCP_FLAG_SYN | TCP_FLAG_ACK)) {
        ...
        case NAT44_ED_TCP_STATE_ESTABLISHED:
            ...
            if ((ses->tcp_flags[dir] & TCP_FLAG_FIN) || (ses->tcp_flags[dir] & TCP_FLAG_RST)) {
    ...
    if (old_state == ses->tcp_state) return;
    ses->last_lru_update = now;
    clib_dlist_remove(tsm->lru_pool, ses->lru_index);
    clib_dlist_addtail(tsm->lru_pool, ses->lru_head_index, ses->lru_index);
}

always_inline void nat44_set_tcp_session_state_i2o(snat_main_t *sm, f64 now, snat_session_t *ses, u8 tcp_flags, u32 thread_index)
{
    return nat44_set_tcp_session_state(sm, now, ses, tcp_flags, thread_index, NAT44_ED_DIR_I2O);
}

always_inline void nat44_set_tcp_session_state_o2i(snat_main_t *sm, f64 now, snat_session_t *ses, u8 tcp_flags, u32 thread_index)
{
    return nat44_set_tcp_session_state(sm, now, ses, tcp_flags, thread_index, NAT44_ED_DIR_O2I);
}

always_inline void nat44_session_update_counters(snat_session_t *s, f64 now, uword bytes, u32 thread_index)
{
...
always_inline void nat44_session_update_lru(snat_main_t *sm, snat_session_t *s, u32 thread_index)
{
    /* don't update too often - timeout is in magnitude of seconds anyway */
    if (s->last_heard > s->last_lru_update + 1) {
        clib_dlist_remove(sm->per_thread_data[thread_index].lru_pool, s->lru_index);
        clib_dlist_addtail(sm->per_thread_data[thread_index].lru_pool, s->lru_head_index, s->lru_index);
        s->last_lru_update = s->last_heard;
    }
}

static_always_inline int nat44_ed_is_unk_proto(u8 proto)
{
