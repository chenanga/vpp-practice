...
static u8 *format_nat_in2out_ed_trace(u8 *s, va_list *args)
{
    ...
    tag = t->is_slow_path ? "NAT44_IN2OUT_ED_SLOW_PATH" : "NAT44_IN2OUT_ED_FAST_PATH";

    s = format(s, "%s: sw_if_index %d, next index %d", tag, t->sw_if_index, t->next_index);
    ...
    if (!t->is_slow_path) {
        if (t->lookup_skipped) {
            ...
            s = format(s, "\n  search key %U", format_ed_session_kvp, &t->search_key);
        }
    }
    if (IP_PROTOCOL_TCP == t->i2of.match.proto) {
        s = format(s, "\n  TCP state: %U", format_nat44_ed_tcp_state, t->tcp_state);
    }
...
static int nat_ed_alloc_addr_and_port_with_snat_address(snat_main_t    *sm,
                                                        ...
                                                        u16            *outside_port)
{
    const u16 port_thread_offset = (port_per_thread * snat_thread_index) + ED_USER_PORT_OFFSET;
    ...
    if (port < port_thread_offset || port >= port_thread_offset + port_per_thread) {
            ...
            for (i = s_addr_offset; i < vec_len(sm->addresses); ++i) {
                ...
                if (a->fib_index == rx_fib_index) {
                    if (a->sw_if_index == tx_sw_if_index) {
                        if ((a->addr_len != ~0) && (a->net.as_u32 == (d_addr.as_u32 & ip4_main.fib_masks[a->addr_len])))

                        {
                            return nat_ed_alloc_addr_and_port_with_snat_address(sm,
                                                                                nat_proto,
                                                                                thread_index,
                                                                                a,
                                                                                sm->port_per_thread,
...
static_always_inline int nat44_ed_external_sm_lookup(snat_main_t   *sm,
                                                     ...
                                                     u16           *dport)
{
    snat_static_mapping_t *m = nat44_ed_sm_o2i_lookup(sm, match_addr, match_port, 0, match_protocol);
    if (!m) {
...
static_always_inline vrf_table_t *get_vrf_table_by_fib(u32 fib_index)
{
...
static_always_inline u32 get_tx_fib_index(u32 rx_fib_index, ip4_address_t addr)
{
    ...
    if (0 != t) {
        ...
        pool_foreach (r, t->routes) {
            ...
            if ((FIB_NODE_INDEX_INVALID != fei) && (~0 != fib_entry_get_resolving_interface(fei))) {
...
static_always_inline int is_destination_resolvable(u32 rx_fib_index, ip4_address_t addr)
{
    fib_node_index_t fei = FIB_NODE_INDEX_INVALID;
    fib_prefix_t     pfx = {
            .fp_proto = FIB_PROTOCOL_IP4,
    ...
    if (0 != t) {
        ...
        pool_foreach (r, t->routes) {
            fei = fib_table_lookup(r->fib_index, &pfx);
            if ((FIB_NODE_INDEX_INVALID != fei) && (~0 != (ii = fib_entry_get_resolving_interface(fei)))) {
                return 1;
            }
        }
    }
        ...
        vec_foreach (f, sm->outside_fibs) {
            ...
            if ((FIB_NODE_INDEX_INVALID != fei) && (~0 != (ii = fib_entry_get_resolving_interface(fei)))) {
                ...
                pool_foreach (i, sm->interfaces) {
                    if ((nat44_ed_is_interface_outside(i)) && (ii == i->sw_if_index)) {
                        return 1;
                    }
                }
            }
                        ...
                        f64                  now)
{
    ...
    if (PREDICT_FALSE(nat44_ed_maximum_sessions_exceeded(sm, rx_fib_index, thread_index))) {
        if (!nat_lru_free_one(sm, thread_index, now)) {
            b->error = node->errors[NAT_IN2OUT_ED_ERROR_MAX_SESSIONS_EXCEEDED];
            nat_ipfix_logging_max_sessions(thread_index, sm->max_translations_per_thread);
    ...
    if (!snat_static_mapping_match(vm,
                                   l_addr,
                                   l_port,
                                   rx_fib_index,
                                   proto,
    ...
    if (PREDICT_TRUE(proto == IP_PROTOCOL_TCP)) {
        if (PREDICT_FALSE(!tcp_flags_is_init(vnet_buffer(b)->ip.reass.icmp_type_or_tcp_flags))) {
    ...
    if (!is_sm) {
        ...
        if (PREDICT_FALSE(nat44_ed_external_sm_lookup(sm, r_addr, r_port, proto, &daddr, &dport))) {
            s->flags |= SNAT_SESSION_FLAG_HAIRPINNING;
        }

        // destination addr/port updated with real values in
        // nat_ed_alloc_addr_and_port
        nat_6t_o2i_flow_init(sm, thread_index, s, daddr, dport, daddr, 0, s->out2in.fib_index, proto);
        ...
        if (nat_ed_alloc_addr_and_port(sm,
                                       rx_fib_index,
                                       tx_sw_if_index,
                                       proto,
                                       thread_index,
        ...
        // hairpinning?
        int is_hairpinning = nat44_ed_external_sm_lookup(sm, r_addr, r_port, proto, &daddr, &dport);
        s->flags |= is_hairpinning * SNAT_SESSION_FLAG_HAIRPINNING;

        if (IP_PROTOCOL_ICMP == proto) {
            nat_6t_o2i_flow_init(sm, thread_index, s, daddr, sm_port, sm_addr, sm_port, s->out2in.fib_index, proto);
            nat_6t_flow_icmp_id_rewrite_set(&s->o2i, l_port);
        }
        else {
            nat_6t_o2i_flow_init(sm, thread_index, s, daddr, dport, sm_addr, sm_port, s->out2in.fib_index, proto);
            nat_6t_flow_dport_rewrite_set(&s->o2i, l_port);
        }
        nat_6t_flow_daddr_rewrite_set(&s->o2i, l_addr.as_u32);
        nat_6t_flow_txfib_rewrite_set(&s->o2i, rx_fib_index);
    ...
    if (lb) s->flags |= SNAT_SESSION_FLAG_LOAD_BALANCING;
    s->ext_host_addr = r_addr;
    s->ext_host_port = r_port;

    nat_6t_i2o_flow_init(sm, thread_index, s, l_addr, l_port, r_addr, r_port, rx_fib_index, proto);
...
nat44_ed_not_translate(vlib_main_t *vm, vlib_node_runtime_t *node, u32 sw_if_index, vlib_buffer_t *b, ip4_header_t *ip, u32 proto, u32 rx_fib_index)
{
    snat_main_t *sm = &snat_main;
    ...
    if (PREDICT_FALSE(is_interface_addr(sm, node, sw_if_index, ip->dst_address.as_u32))) {
        return 1;
    }
                                                              ...
                                                              vlib_buffer_t *b)
{
    ...
    if (!sm->forwarding_enabled) return 0;
    ...
    if (ip->protocol == IP_PROTOCOL_ICMP) {
        ...
        init_ed_k(&kv, lookup_saddr.as_u32, lookup_sport, lookup_daddr.as_u32, lookup_dport, 0, lookup_protocol);
    }
        ...
        init_ed_k(&kv, ip->src_address.as_u32, 0, ip->dst_address.as_u32, 0, 0, ip->protocol);
    }
    ...
    if (!clib_bihash_search_16_8(&sm->flow_hash, &kv, &value)) {
        ...
        if (na44_ed_is_fwd_bypass_session(s)) {
            if (ip->protocol == IP_PROTOCOL_TCP) {
                nat44_set_tcp_session_state_i2o(sm, now, s, vnet_buffer(b)->ip.reass.icmp_type_or_tcp_flags, thread_index);
            }
            /* Accounting */
            nat44_session_update_counters(s, now, vlib_buffer_length_in_chain(vm, b), thread_index);
                                                               ...
                                                               int            is_multi_worker)
{
    ...
    /* src NAT check */
    init_ed_k(&kv, ip->src_address.as_u32, src_port, ip->dst_address.as_u32, dst_port, tx_fib_index, ip->protocol);
    if (!clib_bihash_search_16_8(&sm->flow_hash, &kv, &value)) {
        ASSERT(thread_index == ed_value_get_thread_index(&value));
        s = pool_elt_at_index(tsm->sessions, ed_value_get_session_index(&value));
        return 1;
    ...
    /* dst NAT check */
    if (is_multi_worker && PREDICT_TRUE(!pool_is_free_index(tsm->sessions, vnet_buffer2(b)->nat.cached_dst_nat_session_index))) {
        nat_6t_t lookup;
        ...
        s                   = pool_elt_at_index(tsm->sessions, vnet_buffer2(b)->nat.cached_dst_nat_session_index);
    ...
    init_ed_k(&kv, ip->dst_address.as_u32, dst_port, ip->src_address.as_u32, src_port, rx_fib_index, ip->protocol);
    if (!clib_bihash_search_16_8(&sm->flow_hash, &kv, &value)) {
        ASSERT(thread_index == ed_value_get_thread_index(&value));
        s = pool_elt_at_index(tsm->sessions, ed_value_get_session_index(&value));

    skip_dst_nat_lookup:
        if (na44_ed_is_fwd_bypass_session(s)) return 0;
...
static inline u32 icmp_in2out_ed_slow_path(snat_main_t         *sm,
                                           vlib_buffer_t       *b,
                                           ip4_header_t        *ip,
                                           ...
                                           int                  is_multi_worker)
{
    ...
    err = nat_get_icmp_session_lookup_values(b, ip, &lookup_saddr, &lookup_sport, &lookup_daddr, &lookup_dport, &lookup_protocol);
    if (err != 0) {
    ...
    if (tx_sw_if_index != ~0) {
        if (PREDICT_FALSE(nat44_ed_not_translate_output_feature(sm,
                                                                b,
                                                                ip,
                                                                lookup_sport,
                                                                lookup_dport,
        ...
        if (PREDICT_FALSE(nat44_ed_not_translate(vm, node, sw_if_index, b, ip, IP_PROTOCOL_ICMP, rx_fib_index))) {
            return next;
        }
    }

    if (PREDICT_FALSE(icmp_type_is_error_message(vnet_buffer(b)->ip.reass.icmp_type_or_tcp_flags))) {
        b->error = node->errors[NAT_IN2OUT_ED_ERROR_BAD_ICMP_TYPE];
        return NAT_NEXT_DROP;
    }

    next = slow_path_ed(vm,
                        sm,
                        b,
                        ip->src_address,
                        ip->dst_address,
    ...
    if (NAT_NEXT_DROP == next) goto out;

    if (PREDICT_TRUE(!ip4_is_fragment(ip))) {
        ip_csum_t sum =
            ip_incremental_checksum_buffer(vm, b, (u8 *) icmp - (u8 *) vlib_buffer_get_current(b), ntohs(ip->length) - ip4_header_bytes(ip), 0);
                                                              ...
                                                              vlib_node_runtime_t *node)
{
    ...
    m = nat44_ed_sm_i2o_lookup(sm, ip->src_address, 0, rx_fib_index, ip->protocol);
    if (m) {
        ...
        pool_foreach (s, tsm->sessions) {
            if (s->ext_host_addr.as_u32 == ip->dst_address.as_u32) {
                init_ed_k(&s_kv, s->out2in.addr.as_u32, 0, ip->dst_address.as_u32, 0, tx_fib_index, ip->protocol);
        ...
        if (!new_src_addr.as_u32) {
            for (i = 0; i < vec_len(sm->addresses); i++) {
                init_ed_k(&s_kv, sm->addresses[i].addr.as_u32, 0, ip->dst_address.as_u32, 0, tx_fib_index, ip->protocol);
                if (clib_bihash_search_16_8(&sm->flow_hash, &s_kv, &s_value)) {
    ...
    nat_6t_i2o_flow_init(sm, thread_index, s, ip->src_address, 0, ip->dst_address, 0, rx_fib_index, ip->protocol);
    nat_6t_flow_saddr_rewrite_set(&s->i2o, new_src_addr.as_u32);
    nat_6t_flow_txfib_rewrite_set(&s->i2o, tx_fib_index);

    // hairpinning?
    int is_hairpinning = nat44_ed_external_sm_lookup(sm, ip->dst_address, 0, ip->protocol, &new_dst_addr, NULL);
    s->flags |= is_hairpinning * SNAT_SESSION_FLAG_HAIRPINNING;

    nat_6t_flow_daddr_rewrite_set(&s->i2o, new_dst_addr.as_u32);
    nat_6t_flow_txfib_rewrite_set(&s->i2o, tx_fib_index);

    nat_6t_o2i_flow_init(sm, thread_index, s, new_dst_addr, 0, new_src_addr, 0, tx_fib_index, ip->protocol);
    ...
    if (m) s->flags |= SNAT_SESSION_FLAG_STATIC_MAPPING;
...
static inline uword nat44_ed_in2out_fast_path_node_fn_inline(vlib_main_t         *vm,
                                                             vlib_node_runtime_t *node,
                                                             vlib_frame_t        *frame,
                                                             ...
                                                             int                  is_multi_worker)
{
    ...
    snat_main_per_thread_data_t *tsm          = &sm->per_thread_data[thread_index];
    u32                          def_slow     = is_output_feature ? NAT_NEXT_IN2OUT_ED_OUTPUT_SLOW_PATH : NAT_NEXT_IN2OUT_ED_SLOW_PATH;
    ...
    while (n_left_from > 0) {
        ...
        if (is_output_feature) {
            if (PREDICT_FALSE(nat_not_translate_output_feature_fwd(sm, ip0, thread_index, now, vm, b0))) goto trace0;
        }

        if (PREDICT_FALSE(proto0 == IP_PROTOCOL_ICMP)) {
            if (vnet_buffer(b0)->ip.reass.icmp_type_or_tcp_flags != ICMP4_echo_request
            ...
            int err = nat_get_icmp_session_lookup_values(b0, ip0, &lookup.saddr, &lookup.sport, &lookup.daddr, &lookup.dport, &lookup.proto);
            if (err != 0) {
        ...
        if (is_multi_worker && !pool_is_free_index(tsm->sessions, vnet_buffer2(b0)->nat.cached_session_index)) {
        ...
        init_ed_k(&kv0, lookup.saddr.as_u32, lookup.sport, lookup.daddr.as_u32, lookup.dport, lookup.fib_index, lookup.proto);
        ...
        s0 = pool_elt_at_index(tsm->sessions, ed_value_get_session_index(&value0));
    ...
    skip_lookup:
        ...
        sess_timeout_time = s0->last_heard + (f64) nat44_session_get_timeout(sm, s0);
        ...
        else if (s0->flags & SNAT_SESSION_FLAG_HAIRPINNING && nat_6t_t_eq(&s0->o2i.match, &lookup)) {
            f = &s0->o2i;
        }
        else {
        ...
        if (NAT_ED_TRNSL_ERR_SUCCESS != (translation_error = nat_6t_flow_buf_translate_i2o(vm, sm, b0, ip0, f, proto0, is_output_feature))) {
            nat44_ed_free_session_data(sm, s0, thread_index, 0);
            nat_ed_session_delete(sm, s0, thread_index, 1);
            s0        = 0;
            next[0]   = NAT_NEXT_DROP;
        ...
        switch (proto0) {
            case IP_PROTOCOL_TCP:
                vlib_increment_simple_counter(&sm->counters.fastpath.in2out.tcp, thread_index, cntr_sw_if_index0, 1);
                nat44_set_tcp_session_state_i2o(sm, now, s0, vnet_buffer(b0)->ip.reass.icmp_type_or_tcp_flags, thread_index);
                break;
            case IP_PROTOCOL_UDP:
                vlib_increment_simple_counter(&sm->counters.fastpath.in2out.udp, thread_index, cntr_sw_if_index0, 1);
                break;
            case IP_PROTOCOL_ICMP:
                vlib_increment_simple_counter(&sm->counters.fastpath.in2out.icmp, thread_index, cntr_sw_if_index0, 1);
                break;
            default:
                vlib_increment_simple_counter(&sm->counters.fastpath.in2out.other, thread_index, cntr_sw_if_index0, 1);
                break;
        }

        /* Accounting */
        nat44_session_update_counters(s0, now, vlib_buffer_length_in_chain(vm, b0), thread_index);
        /* Per-user LRU list maintenance */
        nat44_session_update_lru(sm, s0, thread_index);

    trace0:
        if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
            nat_in2out_ed_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
        ...
        if (next[0] == NAT_NEXT_DROP) {
            vlib_increment_simple_counter(&sm->counters.fastpath.in2out.drops, thread_index, cntr_sw_if_index0, 1);
        }
...
static inline uword nat44_ed_in2out_slow_path_node_fn_inline(vlib_main_t         *vm,
                                                             vlib_node_runtime_t *node,
                                                             vlib_frame_t        *frame,
                                                             int                  is_output_feature,
                                                             int                  is_multi_worker)
{
    ...
    while (n_left_from > 0) {
        ...
        if (PREDICT_FALSE(nat44_ed_is_unk_proto(proto0))) {
            s0 = nat44_ed_in2out_slowpath_unknown_proto(sm, b0, ip0, rx_fib_index0, thread_index, now, vm, node);
            if (!s0) next[0] = NAT_NEXT_DROP;

            if (NAT_NEXT_DROP != next[0] && s0
                && NAT_ED_TRNSL_ERR_SUCCESS
                       != (translation_error = nat_6t_flow_buf_translate_i2o(vm, sm, b0, ip0, &s0->i2o, proto0, is_output_feature))) {
                nat44_ed_free_session_data(sm, s0, thread_index, 0);
                nat_ed_session_delete(sm, s0, thread_index, 1);
                s0        = 0;
                next[0]   = NAT_NEXT_DROP;
            ...
            vlib_increment_simple_counter(&sm->counters.slowpath.in2out.other, thread_index, cntr_sw_if_index0, 1);
            goto trace0;
        }

        if (PREDICT_FALSE(proto0 == IP_PROTOCOL_ICMP)) {
            next[0] = icmp_in2out_ed_slow_path(sm,
                                               b0,
                                               ip0,
                                               icmp0,
                                               rx_sw_if_index0,
            ...
            if (NAT_NEXT_DROP != next[0]) {
                vlib_increment_simple_counter(&sm->counters.slowpath.in2out.icmp, thread_index, cntr_sw_if_index0, 1);
            }
            goto trace0;
        }
        ...
        if (!s0) {
            if (is_output_feature) {
                 ...
                 */
                if (PREDICT_FALSE(proto0 == IP_PROTOCOL_UDP
                                  && (vnet_buffer(b0)->ip.reass.l4_dst_port == clib_host_to_net_u16(UDP_DST_PORT_dhcp_to_server))
                                  && ip0->dst_address.as_u32 == 0xffffffff))
                    goto trace0;
            }
            else {
                if (PREDICT_FALSE(nat44_ed_not_translate(vm, node, rx_sw_if_index0, b0, ip0, proto0, rx_fib_index0))) goto trace0;
            }

            next[0] = slow_path_ed(vm,
                                   sm,
                                   b0,
                                   ip0->src_address,
                                   ip0->dst_address,
            ...
            if (PREDICT_FALSE(next[0] == NAT_NEXT_DROP)) goto trace0;
        ...
        if (NAT_ED_TRNSL_ERR_SUCCESS != (translation_error = nat_6t_flow_buf_translate_i2o(vm, sm, b0, ip0, &s0->i2o, proto0, is_output_feature))) {
            nat44_ed_free_session_data(sm, s0, thread_index, 0);
            nat_ed_session_delete(sm, s0, thread_index, 1);
            s0        = 0;
            next[0]   = NAT_NEXT_DROP;
        ...
        if (PREDICT_TRUE(proto0 == IP_PROTOCOL_TCP)) {
            vlib_increment_simple_counter(&sm->counters.slowpath.in2out.tcp, thread_index, cntr_sw_if_index0, 1);
            nat44_set_tcp_session_state_i2o(sm, now, s0, vnet_buffer(b0)->ip.reass.icmp_type_or_tcp_flags, thread_index);
        }
        else {
            vlib_increment_simple_counter(&sm->counters.slowpath.in2out.udp, thread_index, cntr_sw_if_index0, 1);
        }
    ...
    trace0:
        ...
        if (next[0] == NAT_NEXT_DROP) {
            vlib_increment_simple_counter(&sm->counters.slowpath.in2out.drops, thread_index, cntr_sw_if_index0, 1);
        }
...
VLIB_NODE_FN(nat44_ed_in2out_node)
...
VLIB_NODE_FN(nat44_ed_in2out_output_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    if (snat_main.num_workers > 1) {
...
VLIB_NODE_FN(nat44_ed_in2out_slowpath_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    if (snat_main.num_workers > 1) {
...
VLIB_NODE_FN(nat44_ed_in2out_output_slowpath_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    if (snat_main.num_workers > 1) {
...
static u8 *format_nat_pre_trace(u8 *s, va_list *args)
{
    ...
    return format(s, "in2out next_index %d arc_next_index %d", t->next_index, t->arc_next_index);
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    return nat_pre_node_fn_inline(vm, node, frame, NAT_NEXT_IN2OUT_ED_OUTPUT_FAST_PATH);
}
