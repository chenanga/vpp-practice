...
u8 *format_ed_session_kvp(u8 *s, va_list *args)
{
    ...
    s = format(s,
               ...
               format_ip4_address,
               &l_addr,
               clib_net_to_host_u16(l_port),
               format_ip4_address,
               &r_addr,
               clib_net_to_host_u16(r_port),
               format_ip_protocol,
               proto,
               fib_index,
               ed_value_get_thread_index(v),
...
u8 *format_snat_session(u8 *s, va_list *args)
{
    ...
    snat_main_per_thread_data_t *tsm  = va_arg(*args, snat_main_per_thread_data_t *);
    ...
    if (nat44_ed_is_twice_nat_session(sess)) {
        s = format(s,
                   ...
                   format_ip4_address,
                   &sess->ext_host_nat_addr,
                   clib_net_to_host_u16(sess->ext_host_nat_port));
    }
    else {
        if (sess->ext_host_addr.as_u32)
            s = format(s, "       external host %U:%u\n", format_ip4_address, &sess->ext_host_addr, clib_net_to_host_u16(sess->ext_host_port));
    }
    ...
    if (na44_ed_is_fwd_bypass_session(sess)) s = format(s, "       forwarding-bypass\n");
    if (nat44_ed_is_lb_session(sess)) s = format(s, "       load-balancing\n");
    if (nat44_ed_is_twice_nat_session(sess)) s = format(s, "       twice-nat\n");
    return s;
}
...
u8 *format_snat_static_mapping(u8 *s, va_list *args)
{
    ...
    if (is_sm_addr_only(m->flags))
        s = format(s,
                   ...
                   &m->external_addr,
                   m->vrf_id,
                   is_sm_twice_nat(m->flags)        ? "twice-nat"
...
{
    snat_static_mapping_resolve_t *m   = va_arg(*args, snat_static_mapping_resolve_t *);
...
u8 *format_nat_6t_flow(u8 *s, va_list *args)
{
    ...
    if (f->ops & NAT_FLOW_OP_SADDR_REWRITE) {
        s = format(s, "rewrite: saddr %U ", format_ip4_address, f->rewrite.saddr.as_u8);
        r = 1;
    }
...
u8 *format_nat_6t(u8 *s, va_list *args)
{
    ...
    s = format(s,
               ...
               format_ip4_address,
               t->saddr.as_u8,
               clib_net_to_host_u16(t->sport),
               format_ip4_address,
               t->daddr.as_u8,
               clib_net_to_host_u16(t->dport),
               format_ip_protocol,
               t->proto,
               t->fib_index);
    return s;
...
u8 *format_nat44_ed_tcp_state(u8 *s, va_list *args)
{
