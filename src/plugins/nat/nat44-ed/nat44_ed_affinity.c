...
void nat_affinity_enable()
{
    ...
    if (tm->n_vlib_mains > 1) clib_spinlock_init(&nam->affinity_lock);
    clib_bihash_init_16_8(&nam->affinity_hash, "nat-affinity", AFFINITY_HASH_BUCKETS, AFFINITY_HASH_MEMORY);
    clib_bihash_set_kvp_format_fn_16_8(&nam->affinity_hash, format_affinity_kvp);
}
...
void nat_affinity_disable()
{
    ...
    if (tm->n_vlib_mains > 1) clib_spinlock_free(&nam->affinity_lock);
...
void nat_affinity_flush_service(u32 affinity_per_service_list_head_index)
{
    ...
    while ((elt_index = clib_dlist_remove_head(nam->list_pool, affinity_per_service_list_head_index)) != ~0) {
        ...
        if (clib_bihash_add_del_16_8(&nam->affinity_hash, &kv, 0)) nat_elog_warn(sm, "affinity key del failed");
                                 ...
                                 u32           affinity_per_service_list_head_index)
{
    ...
    rv       = clib_bihash_add_or_overwrite_stale_16_8(&nam->affinity_hash, &kv, affinity_is_expired_cb, NULL);
    if (rv) {
    ...
    clib_dlist_addtail(nam->list_pool, affinity_per_service_list_head_index, a->per_service_index);
...
void nat_affinity_unlock(ip4_address_t client_addr, ip4_address_t service_addr, u8 proto, u16 service_port)
{
    ...
    make_affinity_kv(&kv, client_addr, service_addr, proto, service_port);
    clib_spinlock_lock_if_init(&nam->affinity_lock);
    if (clib_bihash_search_16_8(&nam->affinity_hash, &kv, &value)) goto unlock;

    a = pool_elt_at_index(nam->affinity_pool, value.value);
    a->ref_cnt--;
    if (a->ref_cnt == 0) a->expire = (u64) a->sticky_time + vlib_time_now(nam->vlib_main);
