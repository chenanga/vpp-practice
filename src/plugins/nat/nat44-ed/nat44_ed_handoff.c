...
static u8 *format_nat44_handoff_trace(u8 *s, va_list *args)
{
    ...
    s      = format(s, "NAT44_%s_WORKER_HANDOFF %s: next-worker %d trace index %d", tag, output, t->next_worker_index, t->trace_index);

    return s;
}

static inline uword nat44_worker_handoff_fn_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, u8 is_output, u8 is_in2out)
{
    ...
    while (n_left_from >= 4) {
        ...
        if (is_in2out) {
            ...
            ti[1] = nat44_ed_get_in2out_worker_index(b[1], ip1, rx_fib_index1, is_output);
            ti[2] = nat44_ed_get_in2out_worker_index(b[2], ip2, rx_fib_index2, is_output);
            ti[3] = nat44_ed_get_in2out_worker_index(b[3], ip3, rx_fib_index3, is_output);
        }
        else {
            ti[0] = nat44_ed_get_out2in_worker_index(b[0], ip0, rx_fib_index0, is_output);
            ti[1] = nat44_ed_get_out2in_worker_index(b[1], ip1, rx_fib_index1, is_output);
            ti[2] = nat44_ed_get_out2in_worker_index(b[2], ip2, rx_fib_index2, is_output);
            ti[3] = nat44_ed_get_out2in_worker_index(b[3], ip3, rx_fib_index3, is_output);
        }
    ...
    if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE))) {
        ...
        for (i = 0; i < frame->n_vectors; i++) {
            if (b[0]->flags & VLIB_BUFFER_IS_TRACED) {
                nat44_handoff_trace_t *t = vlib_add_trace(vm, node, b[0], sizeof(*t));
    ...
    if (n_enq < frame->n_vectors) {
        vlib_node_increment_counter(vm, node->node_index, NAT44_HANDOFF_ERROR_CONGESTION_DROP, frame->n_vectors - n_enq);
    }

    vlib_node_increment_counter(vm, node->node_index, NAT44_HANDOFF_ERROR_SAME_WORKER, same_worker);
    vlib_node_increment_counter(vm, node->node_index, NAT44_HANDOFF_ERROR_DO_HANDOFF, do_handoff);
