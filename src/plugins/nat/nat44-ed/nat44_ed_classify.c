...
static inline uword nat44_handoff_classify_node_fn_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(pool_elts(sm->static_mappings))) {
                ...
                if (m) {
                    if (m->local_addr.as_u32 != m->external_addr.as_u32) next0 = NAT_NEXT_OUT2IN_CLASSIFY;
                    goto enqueue0;
                }
                m = nat44_ed_sm_o2i_lookup(sm, ip0->dst_address, vnet_buffer(b0)->ip.reass.l4_dst_port, 0, ip0->protocol);
                if (m) {
                    if (m->local_addr.as_u32 != m->external_addr.as_u32) next0 = NAT_NEXT_OUT2IN_CLASSIFY;
                }
            }
        ...
        enqueue0:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                nat44_classify_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                t->cached                 = 0;
    ...
    vlib_node_increment_counter(vm, node->node_index, NAT44_CLASSIFY_ERROR_NEXT_IN2OUT, next_in2out);
    vlib_node_increment_counter(vm, node->node_index, NAT44_CLASSIFY_ERROR_NEXT_OUT2IN, next_out2in);
    return frame->n_vectors;
}

static inline uword nat44_ed_classify_node_fn_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    u32                    n_left_from, *from, *to_next;
    nat44_classify_next_t  next_index;
    snat_main_t           *sm = &snat_main;
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (ip0->protocol != IP_PROTOCOL_ICMP) {
                ...
                if (!clib_bihash_search_16_8(&sm->flow_hash, &ed_kv0, &ed_value0)) {
                    ASSERT(vm->thread_index == ed_value_get_thread_index(&ed_value0));
                    snat_main_per_thread_data_t *tsm = &sm->per_thread_data[vm->thread_index];
                    snat_session_t              *s   = pool_elt_at_index(tsm->sessions, ed_value_get_session_index(&ed_value0));
                    clib_bihash_kv_16_8_t        i2o_kv;
                    nat_6t_flow_to_ed_k(&i2o_kv, &s->i2o);
                    vnet_buffer2(b0)->nat.cached_session_index = ed_value_get_session_index(&ed_value0);
                    if (i2o_kv.key[0] == ed_kv0.key[0] && i2o_kv.key[1] == ed_kv0.key[1]) {
            ...
            if (PREDICT_FALSE(pool_elts(sm->static_mappings))) {
                ...
                m = nat44_ed_sm_o2i_lookup(sm, ip0->dst_address, 0, 0, 0);
                if (m) {
                    if (m->local_addr.as_u32 != m->external_addr.as_u32) next0 = NAT_NEXT_OUT2IN_ED_FAST_PATH;
                    goto enqueue0;
                }
                m = nat44_ed_sm_o2i_lookup(sm, ip0->dst_address, vnet_buffer(b0)->ip.reass.l4_dst_port, 0, ip0->protocol);
                if (m) {
