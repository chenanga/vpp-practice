...
#define skip_if_disabled()                       \
    do {                                         \
        ...
        if (PREDICT_FALSE(!sm->enabled)) return; \
    } while (0)
...
VNET_FEATURE_INIT(nat_pre_out2in, static) = {
    ...
    .runs_after = VNET_FEATURES("acl-plugin-in-ip4-fa", "ip4-dhcp-client-detect", "ip4-sv-reassembly-feature"),
};
...
VNET_FEATURE_INIT(snat_out2in_worker_handoff, static) = {
    ...
    .runs_after = VNET_FEATURES("acl-plugin-in-ip4-fa", "ip4-dhcp-client-detect"),
};
...
static int nat44_ed_add_static_mapping_internal(ip4_address_t l_addr,
                                                ...
                                                u8           *tag);
static int
nat44_ed_del_static_mapping_internal(ip4_address_t l_addr, ip4_address_t e_addr, u16 l_port, u16 e_port, ip_protocol_t proto, u32 vrf_id, u32 flags);

u32 nat_calc_bihash_buckets(u32 n_elts);

static_always_inline int nat44_ed_sm_i2o_add(snat_main_t *sm, snat_static_mapping_t *m, ip4_address_t addr, u16 port, u32 fib_index, u8 proto)
{
    ASSERT(!pool_is_free(sm->static_mappings, m));
    clib_bihash_kv_16_8_t kv;
    nat44_ed_sm_init_i2o_kv(&kv, addr.as_u32, port, fib_index, proto, m - sm->static_mappings);
    return clib_bihash_add_del_16_8(&sm->flow_hash, &kv, 1 /*is_add*/);
}

static_always_inline int nat44_ed_sm_i2o_del(snat_main_t *sm, ip4_address_t addr, u16 port, u32 fib_index, u8 proto)
{
    clib_bihash_kv_16_8_t kv;
...
static_always_inline int nat44_ed_sm_o2i_add(snat_main_t *sm, snat_static_mapping_t *m, ip4_address_t addr, u16 port, u32 fib_index, u8 proto)
{
    ASSERT(!pool_is_free(sm->static_mappings, m));
    clib_bihash_kv_16_8_t kv;
    nat44_ed_sm_init_o2i_kv(&kv, addr.as_u32, port, fib_index, proto, m - sm->static_mappings);
    return clib_bihash_add_del_16_8(&sm->flow_hash, &kv, 1 /*is_add*/);
}

static_always_inline int nat44_ed_sm_o2i_del(snat_main_t *sm, ip4_address_t addr, u16 port, u32 fib_index, u8 proto)
{
    clib_bihash_kv_16_8_t kv;
...
void nat44_ed_free_session_data(snat_main_t *sm, snat_session_t *s, u32 thread_index, u8 is_ha)
{
    per_vrf_sessions_unregister_session(s, thread_index);

    if (nat_ed_ses_i2o_flow_hash_add_del(sm, thread_index, s, 0)) nat_elog_warn(sm, "flow hash del failed");

    if (nat_ed_ses_o2i_flow_hash_add_del(sm, thread_index, s, 0)) nat_elog_warn(sm, "flow hash del failed");

    if (na44_ed_is_fwd_bypass_session(s)) {
        return;
    }

    if (nat44_ed_is_affinity_session(s)) nat_affinity_unlock(s->ext_host_addr, s->out2in.addr, s->proto, s->out2in.port);

    if (!is_ha)
        nat_syslog_nat44_sdel(0,
                              s->in2out.fib_index,
                              &s->in2out.addr,
                              s->in2out.port,
                              &s->ext_host_nat_addr,
...
static ip_interface_address_t *nat44_ed_get_ip_interface_address(u32 sw_if_index, ip4_address_t addr)
{
    snat_main_t *sm = &snat_main;
    ...
    foreach_ip_interface_address(lm, ia, sw_if_index, 1, ({
                                     ip4a = ip_interface_address_get_address(lm, ia);
                                     nat_log_debug("sw_if_idx: %u addr: %U ? %U", sw_if_index, format_ip4_address, ip4a, format_ip4_address, &addr);
...
static int nat44_ed_resolve_nat_addr_len(snat_address_t *ap, snat_interface_t *interfaces)
{
    ip_interface_address_t *ia;
    snat_interface_t       *i;
    u32                     fib_index;
...
static void nat44_ed_update_outside_if_addresses(snat_address_t *ap)
{
    snat_main_t *sm = &snat_main;
...
static void nat44_ed_bind_if_addr_to_nat_addr(u32 sw_if_index)
{
    snat_main_t            *sm = &snat_main;
    ip_interface_address_t *ia;
    snat_address_t         *ap;
...
static_always_inline snat_fib_entry_reg_t *nat44_ed_get_fib_entry_reg(ip4_address_t addr, u32 sw_if_index, int *out_idx)
{
    snat_main_t          *sm = &snat_main;
...
static void nat44_ed_add_fib_entry_reg(ip4_address_t addr, u32 sw_if_index)
{
    ...
    if (!(fe = nat44_ed_get_fib_entry_reg(addr, sw_if_index, 0))) {
        ...
        fib_table_entry_update_one_path(fib_index,
                                        &prefix,
                                        sm->fib_src_low,
                                        (FIB_ENTRY_FLAG_CONNECTED | FIB_ENTRY_FLAG_LOCAL | FIB_ENTRY_FLAG_EXCLUSIVE),
                                        DPO_PROTO_IP4,
                                        NULL,
                                        sw_if_index,
                                        ~0,
...
static void nat44_ed_del_fib_entry_reg(ip4_address_t addr, u32 sw_if_index)
{
    snat_main_t          *sm = &snat_main;
    snat_fib_entry_reg_t *fe;
    int                   i;
...
static void nat44_ed_add_del_interface_fib_reg_entries(ip4_address_t addr, u8 is_add)
{
    snat_main_t      *sm = &snat_main;
    snat_interface_t *i;
...
static_always_inline void nat44_ed_add_del_nat_addr_fib_reg_entries(u32 sw_if_index, u8 is_add)
{
    snat_main_t    *sm = &snat_main;
    snat_address_t *ap;
...
static_always_inline void nat44_ed_add_del_sm_fib_reg_entries(u32 sw_if_index, u8 is_add)
{
    snat_main_t           *sm = &snat_main;
...
int nat44_ed_add_address(ip4_address_t *addr, u32 vrf_id, u8 twice_nat)
{
    ...
    if (vrf_id != ~0) {
        ap->fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, vrf_id, sm->fib_src_low);
    }
...
vrf_table_t *nat44_ed_get_vrf_table(u32 table_vrf_id)
{
    snat_main_t *sm = &snat_main;
    vrf_table_t *t;
...
vrf_route_t *nat44_ed_get_vrf_route(vrf_table_t *t, u32 vrf_id)
{
    vrf_route_t *r;
...
int nat44_ed_add_del_vrf_table(u32 table_vrf_id, bool is_add)
{
    ...
    if (t) {
        ...
        t->table_fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, table_vrf_id, sm->fib_src_low);
    }
...
int nat44_ed_add_del_vrf_route(u32 table_vrf_id, u32 vrf_id, bool is_add)
{
    ...
    if (r) {
        ...
        r->vrf_id    = vrf_id;
        r->fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, vrf_id, sm->fib_src_low);
    }
...
u32 get_thread_idx_by_port(u16 e_port)
{
    ...
    if (sm->num_workers > 1) {
        thread_idx = sm->first_worker_index + sm->workers[(e_port - ED_USER_PORT_OFFSET) / sm->port_per_thread % _vec_len(sm->workers)];
    }
    return thread_idx;
}
                                        ...
                                        u16                          e_port)
{
    ...
    pool_foreach (s, tsm->sessions) {
        if (s->in2out.fib_index != fib_index || s->in2out.addr.as_u32 != l_addr.as_u32) {
            continue;
        }
        if (!addr_only) {
            if ((s->out2in.addr.as_u32 != e_addr.as_u32) || s->out2in.port != e_port || s->in2out.port != l_port || s->proto != protocol) continue;
        }

        if (nat44_ed_is_lb_session(s)) continue;
        if (!nat44_ed_is_session_static(s)) continue;
...
static_always_inline snat_static_mapping_t *nat44_ed_sm_lookup(snat_main_t *sm, clib_bihash_kv_16_8_t *kv)
{
    clib_bihash_kv_16_8_t v;
    int                   rc = clib_bihash_search_16_8(&sm->flow_hash, kv, &v);
    if (!rc) {
        ASSERT(0 == ed_value_get_thread_index(&v));
        return pool_elt_at_index(sm->static_mappings, ed_value_get_session_index(&v));
    }
    return NULL;
}

snat_static_mapping_t *nat44_ed_sm_o2i_lookup(snat_main_t *sm, ip4_address_t addr, u16 port, u32 fib_index, u8 proto)
{
    clib_bihash_kv_16_8_t kv;
    nat44_ed_sm_init_o2i_k(&kv, addr.as_u32, port, fib_index, proto);
...
snat_static_mapping_t *nat44_ed_sm_i2o_lookup(snat_main_t *sm, ip4_address_t addr, u16 port, u32 fib_index, u8 proto)
{
    clib_bihash_kv_16_8_t kv;
    nat44_ed_sm_init_i2o_k(&kv, addr.as_u32, port, fib_index, proto);
    return nat44_ed_sm_lookup(sm, &kv);
...
nat44_ed_get_resolve_record(ip4_address_t l_addr, u16 l_port, u16 e_port, ip_protocol_t proto, u32 vrf_id, u32 sw_if_index, u32 flags, int *out_idx)
{
...
static int nat44_ed_del_resolve_record(ip4_address_t l_addr, u16 l_port, u16 e_port, ip_protocol_t proto, u32 vrf_id, u32 sw_if_index, u32 flags)
{
    snat_main_t *sm = &snat_main;
    int          i;
    if (nat44_ed_get_resolve_record(l_addr, l_port, e_port, proto, vrf_id, sw_if_index, flags, &i)) {
                                ...
                                u8           *tag)
{
    ...
    if (is_sm_switch_address(flags)) {
        if (nat44_ed_get_resolve_record(l_addr, l_port, e_port, proto, vrf_id, sw_if_index, flags, 0)) {
            return VNET_API_ERROR_VALUE_EXIST;
        }
        ...
        ip4_address_t *first_int_addr = ip4_interface_first_address(sm->ip4_main, sw_if_index, 0);
        if (!first_int_addr) {
            return 0;
        }
    ...
    rv = nat44_ed_add_static_mapping_internal(l_addr, e_addr, l_port, e_port, proto, vrf_id, sw_if_index, flags, pool_addr, tag);
    if ((0 != rv) && is_sm_switch_address(flags)) {
        nat44_ed_del_resolve_record(l_addr, l_port, e_port, proto, vrf_id, sw_if_index, flags);
    }
                                ...
                                u32           flags)
{
    ...
    // interface bound mapping
    if (is_sm_switch_address(flags)) {
        if (nat44_ed_del_resolve_record(l_addr, l_port, e_port, proto, vrf_id, sw_if_index, flags)) {
            return VNET_API_ERROR_NO_SUCH_ENTRY;
        }
    ...
    return nat44_ed_del_static_mapping_internal(l_addr, e_addr, l_port, e_port, proto, vrf_id, flags);
}
                                                ...
                                                u8           *tag)
{
    ...
    if (m) {
        ...
        local->vrf_id    = vrf_id;
        local->fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, vrf_id, sm->fib_src_low);

        nat44_ed_sm_i2o_add(sm, m, m->local_addr, m->local_port, local->fib_index, m->proto);

        return 0;
    }

    if (vrf_id != ~0) {
        fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, vrf_id, sm->fib_src_low);
    }
    else {
    ...
    if (!is_sm_out2in_only(flags)) {
        nat44_ed_sm_i2o_add(sm, m, m->local_addr, m->local_port, fib_index, m->proto);
    }
    ...
    if (sm->num_workers > 1) {
        ...
        u32 worker_index;
        worker_index = nat44_ed_get_in2out_worker_index(0, &ip, m->fib_index, 0);
...
nat44_ed_del_static_mapping_internal(ip4_address_t l_addr, ip4_address_t e_addr, u16 l_port, u16 e_port, ip_protocol_t proto, u32 vrf_id, u32 flags)
{
    ...
    nat_ed_static_mapping_del_sessions(sm, tsm, m->local_addr, m->local_port, m->proto, fib_index, is_sm_addr_only(flags), e_addr, e_port);
                                   ...
                                   u32                   affinity)
{
    ...
    if (affinity)
        m->affinity_per_service_list_head_index = nat_affinity_get_per_service_list_head_index();
    else
        m->affinity_per_service_list_head_index = ~0;

    if (nat44_ed_sm_o2i_add(sm, m, m->external_addr, m->external_port, 0, m->proto)) {
        nat_log_err("sm o2i key add failed");
        return VNET_API_ERROR_UNSPECIFIED;
    }

    for (i = 0; i < vec_len(locals); i++) {
        locals[i].fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, locals[i].vrf_id, sm->fib_src_low);
        if (!is_sm_out2in_only(flags)) {
            if (nat44_ed_sm_i2o_add(sm, m, locals[i].addr, locals[i].port, 0, proto)) {
        ...
        locals[i].prefix = (i == 0) ? locals[i].probability : (locals[i - 1].prefix + locals[i].probability);
        pool_get(m->locals, local);
        *local = locals[i];
        if (sm->num_workers > 1) {
            ip4_header_t ip = {
            ...
            bitmap = clib_bitmap_set(bitmap, nat44_ed_get_in2out_worker_index(0, &ip, m->fib_index, 0), 1);
        }
    }
...
int nat44_ed_del_lb_static_mapping(ip4_address_t e_addr, u16 e_port, ip_protocol_t proto, u32 flags)
{
    snat_main_t           *sm = &snat_main;
    snat_static_mapping_t *m;
    ...
    m = nat44_ed_sm_o2i_lookup(sm, e_addr, e_port, 0, proto);
    if (!m) return VNET_API_ERROR_NO_SUCH_ENTRY;

    if (!is_sm_lb(m->flags)) return VNET_API_ERROR_INVALID_VALUE;

    if (nat44_ed_sm_o2i_del(sm, m->external_addr, m->external_port, 0, m->proto)) {
    ...
    pool_foreach (local, m->locals) {
        ...
        if (!is_sm_out2in_only(flags)) {
            if (nat44_ed_sm_i2o_del(sm, local->addr, local->port, local->fib_index, m->proto)) {
        ...
        if (sm->num_workers > 1) {
            ...
            tsm = vec_elt_at_index(sm->per_thread_data, nat44_ed_get_in2out_worker_index(0, &ip, m->fib_index, 0));
        }
        else
            tsm = vec_elt_at_index(sm->per_thread_data, sm->num_workers);
        ...
        pool_foreach (s, tsm->sessions) {
            ...
            if ((s->in2out.addr.as_u32 != local->addr.as_u32) || s->in2out.port != local->port) continue;
                                             ...
                                             u8            is_add)
{
    ...
    pool_foreach (local, m->locals) {
        if ((local->addr.as_u32 == l_addr.as_u32) && (local->port == l_port) && (local->vrf_id == vrf_id)) {
            match_local = local;
            break;
        }
    }
    ...
    if (is_add) {
        ...
        if (!is_sm_out2in_only(m->flags)) {
            if (nat44_ed_sm_i2o_add(sm, m, l_addr, l_port, local->fib_index, proto)) {
                nat_log_err("sm i2o key add failed");
                pool_put(m->locals, local);
                return VNET_API_ERROR_UNSPECIFIED;
            }
        ...
        if (!match_local) return VNET_API_ERROR_NO_SUCH_ENTRY;

        if (pool_elts(m->locals) < 3) return VNET_API_ERROR_UNSPECIFIED;

        fib_table_unlock(match_local->fib_index, FIB_PROTOCOL_IP4, sm->fib_src_low);

        if (!is_sm_out2in_only(m->flags)) {
            if (nat44_ed_sm_i2o_del(sm, l_addr, l_port, match_local->fib_index, proto)) nat_log_err("sm i2o key del failed");
        }
        ...
        pool_foreach (s, tsm->sessions) {
            ...
            if ((s->in2out.addr.as_u32 != match_local->addr.as_u32) || s->in2out.port != match_local->port) continue;
    ...
    pool_foreach (local, m->locals) {
        ...
        if (sm->num_workers > 1) {
            ...
            bitmap                = clib_bitmap_set(bitmap, nat44_ed_get_in2out_worker_index(0, &ip, local->fib_index, 0), 1);
        }
    }
...
void expire_per_vrf_sessions(u32 fib_index)
{
    ...
    vec_foreach (tsm, sm->per_thread_data) {
        pool_foreach (per_vrf_sessions, tsm->per_vrf_sessions_pool) {
            if ((per_vrf_sessions->rx_fib_index == fib_index) || (per_vrf_sessions->tx_fib_index == fib_index)) {
...
void update_per_vrf_sessions_pool(u32 fib_index, int is_del)
{
    snat_main_t *sm = &snat_main;
    nat_fib_t   *fib;
...
static_always_inline nat_fib_t *nat44_ed_get_outside_fib(nat_fib_t *outside_fibs, u32 fib_index)
{
    nat_fib_t *f;
...
int nat44_ed_add_interface(u32 sw_if_index, u8 is_inside)
{
    ...
    if (i) {
        if ((nat44_ed_is_interface_inside(i) && is_inside) || (nat44_ed_is_interface_outside(i) && !is_inside)) {
            return 0;
        }
        if (sm->num_workers > 1) {
            del_feature_name = !is_inside ? "nat44-in2out-worker-handoff" : "nat44-out2in-worker-handoff";
...
int nat44_ed_del_interface(u32 sw_if_index, u8 is_inside)
{
    ...
    if (nat44_ed_is_interface_inside(i) && nat44_ed_is_interface_outside(i)) {
        ...
        vnet_feature_enable_disable("ip4-unicast", feature_name, sw_if_index, 0, 0, 0);
...
int nat44_ed_add_output_interface(u32 sw_if_index)
{
    ...
    if (sm->num_workers > 1) {
        ...
        vnet_feature_enable_disable("ip4-unicast", "nat44-out2in-worker-handoff", sw_if_index, 1, 0, 0);
        vnet_feature_enable_disable("ip4-output", "nat44-in2out-output-worker-handoff", sw_if_index, 1, 0, 0);
    }
    else {
        rv = ip4_sv_reass_enable_disable_with_refcnt(sw_if_index, 1);
        if (rv) {
        ...
        vnet_feature_enable_disable("ip4-unicast", "nat-pre-out2in", sw_if_index, 1, 0, 0);
        vnet_feature_enable_disable("ip4-output", "nat-pre-in2out-output", sw_if_index, 1, 0, 0);
    }
...
int nat44_ed_del_output_interface(u32 sw_if_index)
{
    ...
    if (sm->num_workers > 1) {
        ...
        vnet_feature_enable_disable("ip4-unicast", "nat44-out2in-worker-handoff", sw_if_index, 0, 0, 0);
        vnet_feature_enable_disable("ip4-output", "nat44-in2out-output-worker-handoff", sw_if_index, 0, 0, 0);
    }
    else {
        rv = ip4_sv_reass_enable_disable_with_refcnt(sw_if_index, 0);
        if (rv) {
        ...
        vnet_feature_enable_disable("ip4-unicast", "nat-pre-out2in", sw_if_index, 0, 0, 0);
        vnet_feature_enable_disable("ip4-output", "nat-pre-in2out-output", sw_if_index, 0, 0, 0);
    }
...
int snat_set_workers(uword *bitmap)
{
    ...
    if (sm->num_workers < 2) return VNET_API_ERROR_FEATURE_DISABLED;

    if (clib_bitmap_last_set(bitmap) >= sm->num_workers) return VNET_API_ERROR_INVALID_WORKER;
...
int nat44_ed_set_frame_queue_nelts(u32 frame_queue_nelts)
{
    ...
    if ((sm->fq_in2out_index != ~0) || (sm->fq_out2in_index != ~0) || (sm->fq_in2out_output_index != ~0)) {
        // frame queu nelts can be set only before first
        // call to nat44_plugin_enable after that it
        // doesn't make sense
        nat_log_err(
            "Frame queue was already initialized. "
...
static void nat44_ed_update_outside_fib_cb(ip4_main_t *im, uword opaque, u32 sw_if_index, u32 new_fib_index, u32 old_fib_index)
{
    ...
    if (!sm->enabled || (new_fib_index == old_fib_index) || (!vec_len(sm->outside_fibs))) {
        return;
    }

    pool_foreach (i, sm->interfaces) {
        if (i->sw_if_index == sw_if_index) {
            if (!(nat44_ed_is_interface_outside(i))) return;
            match = 1;
        }
    }
    ...
    vec_foreach (outside_fib, sm->outside_fibs) {
        if (outside_fib->fib_index == old_fib_index) {
            outside_fib->ref_count--;
            if (!outside_fib->ref_count) vec_del1(sm->outside_fibs, outside_fib - sm->outside_fibs);
            break;
        }
    }
...
static void nat44_ed_add_del_static_mapping_cb(ip4_main_t    *im,
                                               uword          opaque,
                                               u32            sw_if_index,
                                               ip4_address_t *address,
                                               u32            address_length,
...
void test_key_calc_split()
{
    ...
    clib_bihash_kv_16_8_t kv;
    init_ed_kv(&kv, l_addr.as_u32, l_port, r_addr.as_u32, r_port, fib_index, proto, thread_index, session_index);
    ...
    u8  proto2     = 0;
    u32 fib_index2 = 0;
    split_ed_kv(&kv, &l_addr2, &r_addr2, &proto2, &fib_index2, &l_port2, &r_port2);
...
static clib_error_t *nat_init(vlib_main_t *vm)
{
    ...
    nat_init_simple_counter(sm->total_sessions, "total-sessions", "/nat44-ed/total-sessions");
    sm->max_cfg_sessions_gauge = vlib_stats_add_gauge("/nat44-ed/max-cfg-sessions");

#define _(x)                                                                                      \
    nat_init_simple_counter(sm->counters.fastpath.in2out.x, #x, "/nat44-ed/in2out/fastpath/" #x); \
    nat_init_simple_counter(sm->counters.fastpath.out2in.x, #x, "/nat44-ed/out2in/fastpath/" #x); \
    nat_init_simple_counter(sm->counters.slowpath.in2out.x, #x, "/nat44-ed/in2out/slowpath/" #x); \
    nat_init_simple_counter(sm->counters.slowpath.out2in.x, #x, "/nat44-ed/out2in/slowpath/" #x);
    foreach_nat_counter;
#undef _
    nat_init_simple_counter(sm->counters.hairpinning, "hairpinning", "/nat44-ed/hairpinning");
    ...
    sm->fib_src_low = fib_source_allocate("nat-low", FIB_SOURCE_PRIORITY_LOW, FIB_SOURCE_BH_SIMPLE);
    sm->fib_src_hi  = fib_source_allocate("nat-hi", FIB_SOURCE_PRIORITY_HI, FIB_SOURCE_BH_SIMPLE);
...
int nat44_plugin_enable(nat44_config_t c)
{
    ...
    if (!c.sessions) c.sessions = 63 * 1024;
    ...
    sm->inside_fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, c.inside_vrf, sm->fib_src_hi);

    sm->outside_vrf_id    = c.outside_vrf;
    sm->outside_fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, c.outside_vrf, sm->fib_src_hi);
    ...
    if (sm->num_workers > 1) {
        ...
        if (sm->fq_in2out_output_index == ~0) {
            ...
            sm->fq_in2out_output_index = vlib_frame_queue_main_init(node->index, sm->frame_queue_nelts);
        }
    }
...
int nat44_ed_del_output_interfaces()
{
    snat_main_t      *sm = &snat_main;
    snat_interface_t *i, *pool;
    int               error = 0;
...
static clib_error_t *nat44_ed_sw_interface_add_del(vnet_main_t *vnm, u32 sw_if_index, u32 is_add)
{
    snat_main_t      *sm = &snat_main;
    snat_interface_t *i;
    int               error = 0;
...
int nat44_ed_del_static_mappings()
{
    snat_main_t           *sm = &snat_main;
    ...
    pool_foreach (m, pool) {
        error = nat44_ed_del_static_mapping_internal(m->local_addr, m->external_addr, m->local_port, m->external_port, m->proto, m->vrf_id, m->flags);
        if (error) {
...
static_always_inline snat_static_mapping_t *nat44_ed_sm_match(snat_main_t  *sm,
                                                              ...
                                                              int           by_external)
{
    snat_static_mapping_t *m;
    if (!by_external) {
        m = nat44_ed_sm_i2o_lookup(sm, match_addr, match_port, match_fib_index, match_protocol);
        if (m) return m;
        ...
        if (sm->inside_fib_index != match_fib_index) {
            m = nat44_ed_sm_i2o_lookup(sm, match_addr, match_port, sm->inside_fib_index, match_protocol);
            if (m) return m;

            // try address only mapping
            m = nat44_ed_sm_i2o_lookup(sm, match_addr, 0, sm->inside_fib_index, 0);
            if (m) return m;
        }
        // TODO: this specific use case may be deprecated (needs testing)
        if (sm->outside_fib_index != match_fib_index) {
            m = nat44_ed_sm_i2o_lookup(sm, match_addr, match_port, sm->outside_fib_index, match_protocol);
            if (m) return m;

            // try address only mapping
            m = nat44_ed_sm_i2o_lookup(sm, match_addr, 0, sm->outside_fib_index, 0);
            if (m) return m;
        }
    }
    else {
        m = nat44_ed_sm_o2i_lookup(sm, match_addr, match_port, 0, match_protocol);
        if (m) return m;
                              ...
                              snat_static_mapping_t **out)
{
    ...
    m = nat44_ed_sm_match(sm, match_addr, match_port, match_fib_index, match_protocol, by_external);
    if (!m) {
        return 1;
    }
    ...
    if (by_external) {
        if (is_sm_lb(m->flags)) {
            if (PREDICT_FALSE(lb != 0)) *lb = m->affinity ? AFFINITY_LB_NAT : LB_NAT;
            if (m->affinity && !nat_affinity_find_and_lock(vm, ext_host_addr[0], match_addr, match_protocol, match_port, &backend_index)) {
            ...
            if (PREDICT_FALSE(sm->num_workers > 1)) {
                ...
                pool_foreach_index (i, m->locals) {
                    ...
                    if (nat44_ed_get_in2out_worker_index(0, &ip, m->fib_index, 0) == thread_index) {
                        vec_add1(tmp, i);
                    }
                }
            ...
            if (!(local->prefix >= rand)) return 1;
            *mapping_addr      = local->addr;
            *mapping_port      = local->port;
            *mapping_fib_index = local->fib_index;
            if (m->affinity) {
                if (nat_affinity_create_and_lock(ext_host_addr[0],
                                                 match_addr,
                                                 match_protocol,
                                                 match_port,
                                                 tmp[lo],
            ...
            if (PREDICT_FALSE(lb != 0)) *lb = NO_LB_NAT;
            *mapping_fib_index = m->fib_index;
            *mapping_addr      = m->local_addr;
            /* Address only mapping doesn't change port */
            *mapping_port = is_sm_addr_only(m->flags) ? match_port : m->local_port;
        }
    }
    else {
        *mapping_addr = m->external_addr;
        /* Address only mapping doesn't change port */
        *mapping_port      = is_sm_addr_only(m->flags) ? match_port : m->external_port;
        *mapping_fib_index = sm->outside_fib_index;
    }

end:
    if (PREDICT_FALSE(is_addr_only != 0)) *is_addr_only = is_sm_addr_only(m->flags);
    ...
    if (PREDICT_FALSE(is_identity_nat != 0)) *is_identity_nat = is_sm_identity_nat(m->flags);
...
u32 nat44_ed_get_in2out_worker_index(vlib_buffer_t *b, ip4_header_t *ip, u32 rx_fib_index, u8 is_output)
{
    ...
    if (b) {
        ...
        if (PREDICT_FALSE(ip->protocol == IP_PROTOCOL_ICMP)) {
            ...
            if (!nat_get_icmp_session_lookup_values(b, ip, &lookup_saddr, &lookup_sport, &lookup_daddr, &lookup_dport, &lookup_protocol)) {
                init_ed_k(&kv16, lookup_saddr.as_u32, lookup_sport, lookup_daddr.as_u32, lookup_dport, rx_fib_index, lookup_protocol);
    ...
    hash = ip->src_address.as_u32 + (ip->src_address.as_u32 >> 8) + (ip->src_address.as_u32 >> 16) + (ip->src_address.as_u32 >> 24) + rx_fib_index
           + (rx_fib_index >> 8) + (rx_fib_index >> 16) + (rx_fib_index >> 24);
...
u32 nat44_ed_get_out2in_worker_index(vlib_buffer_t *b, ip4_header_t *ip, u32 rx_fib_index, u8 is_output)
{
    ...
    if (PREDICT_FALSE(IP_PROTOCOL_ICMP == proto)) {
        ...
        if (!nat_get_icmp_session_lookup_values(b, ip, &lookup_saddr, &lookup_sport, &lookup_daddr, &lookup_dport, &lookup_protocol)) {
            init_ed_k(&kv16, lookup_saddr.as_u32, lookup_sport, lookup_daddr.as_u32, lookup_dport, rx_fib_index, lookup_protocol);
            if (PREDICT_TRUE(!clib_bihash_search_16_8(&sm->flow_hash, &kv16, &value16))) {
                next_worker_index = ed_value_get_thread_index(&value16);
                nat_elog_debug_handoff(sm,
                                       "HANDOFF OUT2IN (session)",
                                       next_worker_index,
                                       rx_fib_index,
                                       clib_net_to_host_u32(ip->src_address.as_u32),
                                       clib_net_to_host_u32(ip->dst_address.as_u32));
    ...
    if (PREDICT_FALSE(ip->protocol == IP_PROTOCOL_ICMP)) {
        ...
        if (!icmp_type_is_error_message(vnet_buffer(b)->ip.reass.icmp_type_or_tcp_flags))
    ...
    if (PREDICT_FALSE(pool_elts(sm->static_mappings))) {
        ...
        if (m) {
            ...
            hash = ip->src_address.as_u32 + (ip->src_address.as_u32 >> 8) + (ip->src_address.as_u32 >> 16) + (ip->src_address.as_u32 >> 24);

            if (PREDICT_TRUE(is_pow2(_vec_len(m->workers))))
                next_worker_index = m->workers[hash & (_vec_len(m->workers) - 1)];
            else
...
u32 nat44_get_max_session_limit()
{
    ...
    for (; len < vec_len(sm->max_translations_per_fib); len++) {
        if (max_limit < sm->max_translations_per_fib[len]) max_limit = sm->max_translations_per_fib[len];
    }
    return max_limit;
}
...
int nat44_set_session_limit(u32 session_limit, u32 vrf_id)
{
    ...
    if (~0 == fib_index) return -1;

    vec_validate_init_empty(sm->max_translations_per_fib, fib_index, sm->max_translations_per_thread);
...
int nat44_update_session_limit(u32 session_limit, u32 vrf_id)
{
    ...
    if (nat44_set_session_limit(session_limit, vrf_id)) return 1;
    sm->max_translations_per_thread = nat44_get_max_session_limit();

    vlib_stats_set_gauge(sm->max_cfg_sessions_gauge, sm->max_translations_per_thread);

    sm->translation_buckets = nat_calc_bihash_buckets(sm->max_translations_per_thread);

    nat44_ed_sessions_clear();
    return 0;
}

static void nat44_ed_worker_db_init(snat_main_per_thread_data_t *tsm, u32 translations)
{
    dlist_elt_t *head;
...
static void nat44_ed_flow_hash_init()
{
    snat_main_t *sm = &snat_main;
    // we expect 2 flows per session, so multiply translation_buckets by 2
    clib_bihash_init_16_8(&sm->flow_hash, "ed-flow-hash", clib_max(1, sm->num_workers) * 2 * sm->translation_buckets, 0);
...
static void nat44_ed_flow_hash_free()
{
    snat_main_t *sm = &snat_main;
                                               ...
                                               u32            is_delete)
{
    ...
    vec_foreach (rp, sm->sm_to_resolve) {
        if (sw_if_index == rp->sw_if_index) {
            if (is_delete) {
                if (rp->is_resolved) {
                    rv = nat44_ed_del_static_mapping_internal(rp->l_addr, address[0], rp->l_port, rp->e_port, rp->proto, rp->vrf_id, rp->flags);
                    if (rv) {
                ...
                if (!rp->is_resolved) {
                    rv = nat44_ed_add_static_mapping_internal(rp->l_addr,
                                                              address[0],
                                                              rp->l_port,
                                                              rp->e_port,
                                                              rp->proto,
...
static int nat44_ed_get_addr_resolve_record(u32 sw_if_index, u8 twice_nat, int *out)
{
    snat_main_t            *sm = &snat_main;
...
static int nat44_ed_del_addr_resolve_record(u32 sw_if_index, u8 twice_nat)
{
    snat_main_t *sm = &snat_main;
    int          i;
                                                  ...
                                                  u32            is_delete)
{
    ...
    if (nat44_ed_get_addr_resolve_record(sw_if_index, twice_nat, &i)) {
        ...
        if (nat44_ed_get_addr_resolve_record(sw_if_index, twice_nat, &i)) {
            u32 fib_index = ip4_fib_table_get_index_for_sw_if_index(sw_if_index);
            vec_foreach (ap, sm->addresses) {
                if ((fib_index == ap->fib_index) && (address->as_u32 == ap->addr.as_u32)) {
                    if (!is_delete) {
...
int nat44_ed_del_session(snat_main_t *sm, ip4_address_t *addr, u16 port, ip4_address_t *eh_addr, u16 eh_port, u8 proto, u32 vrf_id, int is_in)
{
    ...
    if (sm->num_workers > 1)
        tsm = vec_elt_at_index(sm->per_thread_data, nat44_ed_get_in2out_worker_index(0, &ip, fib_index, 0));
    else
        tsm = vec_elt_at_index(sm->per_thread_data, sm->num_workers);
    ...
    if (pool_is_free_index(tsm->sessions, ed_value_get_session_index(&value))) return VNET_API_ERROR_UNSPECIFIED;
...
void nat_6t_l3_l4_csum_calc(nat_6t_flow_t *f)
{
    f->l3_csum_delta = 0;
    f->l4_csum_delta = 0;
    if (f->ops & NAT_FLOW_OP_SADDR_REWRITE && f->rewrite.saddr.as_u32 != f->match.saddr.as_u32) {
        f->l3_csum_delta = ip_csum_add_even(f->l3_csum_delta, f->rewrite.saddr.as_u32);
        f->l3_csum_delta = ip_csum_sub_even(f->l3_csum_delta, f->match.saddr.as_u32);
    }
    else {
        f->rewrite.saddr.as_u32 = f->match.saddr.as_u32;
    }
    if (f->ops & NAT_FLOW_OP_DADDR_REWRITE && f->rewrite.daddr.as_u32 != f->match.daddr.as_u32) {
        f->l3_csum_delta = ip_csum_add_even(f->l3_csum_delta, f->rewrite.daddr.as_u32);
        f->l3_csum_delta = ip_csum_sub_even(f->l3_csum_delta, f->match.daddr.as_u32);
    }
    else {
    ...
    if (f->ops & NAT_FLOW_OP_ICMP_ID_REWRITE && f->rewrite.icmp_id != f->match.sport) {
        f->l4_csum_delta = ip_csum_add_even(f->l4_csum_delta, f->rewrite.icmp_id);
        f->l4_csum_delta = ip_csum_sub_even(f->l4_csum_delta, f->match.sport);
    }
    else {
                                                    ...
                                                    int            skip_saddr_rewrite)
{
    ...
    if ((IP_PROTOCOL_TCP == proto || IP_PROTOCOL_UDP == proto) && !vnet_buffer(b)->ip.reass.is_non_first_fragment) {
        if (!is_icmp_inner_ip4) {  // regular case
            ip->src_address = f->rewrite.saddr;
            ip->dst_address = f->rewrite.daddr;
            udp->src_port   = f->rewrite.sport;
            udp->dst_port   = f->rewrite.dport;
        }
        else {  // icmp inner ip4 - reversed saddr/daddr
    ...
    if (0xffff == ip->checksum) ip->checksum = 0;
...
{
    int result =
        ((u8 *) object + size <= (u8 *) vlib_buffer_get_current(b) + b->current_length) && vlib_object_within_buffer_data(vm, b, object, size);
    return result;
}

static_always_inline int nat_6t_flow_icmp_translate(vlib_main_t *vm, snat_main_t *sm, vlib_buffer_t *b, ip4_header_t *ip, nat_6t_flow_t *f)
{
    if (IP_PROTOCOL_ICMP != ip->protocol) return NAT_ED_TRNSL_ERR_TRANSLATION_FAILED;
    ...
    if ((!vnet_buffer(b)->ip.reass.is_non_first_fragment)) {
        ...
        if (!icmp_type_is_error_message(icmp->type)) {
            if ((f->ops & NAT_FLOW_OP_ICMP_ID_REWRITE) && (f->rewrite.icmp_id != echo->identifier)) {
                ip_csum_t sum    = icmp->checksum;
                sum              = ip_csum_update(sum, echo->identifier, f->rewrite.icmp_id, nat_icmp_echo_header_t, identifier /* changed member */);
                echo->identifier = f->rewrite.icmp_id;
                icmp->checksum   = ip_csum_fold(sum);
            }
        }
        else {
            ip_csum_t sum = ip_incremental_checksum(0, icmp, clib_net_to_host_u16(ip->length) - ip4_header_bytes(ip));
            ...
            switch (inner_proto) {
                case IP_PROTOCOL_UDP:
                    ...
                    nat_6t_flow_ip4_translate(sm, b, inner_ip, f, inner_proto, 1 /* is_icmp_inner_ip4 */, 0 /* skip_saddr_rewrite */);
                    new_icmp_sum   = ip_csum_sub_even(old_icmp_sum, f->l3_csum_delta);
                    new_icmp_sum   = ip_csum_sub_even(new_icmp_sum, f->l4_csum_delta);
                    new_icmp_sum   = ip_csum_update(new_icmp_sum, old_inner_ip_sum, inner_ip->checksum, ip4_header_t, checksum);
                    new_icmp_sum   = ip_csum_update(new_icmp_sum, old_udp_sum, udp->checksum, udp_header_t, checksum);
                ...
                case IP_PROTOCOL_TCP:
                    ...
                    new_icmp_sum   = ip_csum_sub_even(new_icmp_sum, f->l4_csum_delta);
                    new_icmp_sum   = ip_csum_update(new_icmp_sum, old_inner_ip_sum, inner_ip->checksum, ip4_header_t, checksum);
                    new_icmp_sum   = ip_csum_update(new_icmp_sum, old_tcp_sum, tcp->checksum, tcp_header_t, checksum);
                    new_icmp_sum   = ip_csum_fold(new_icmp_sum);
                    icmp->checksum = new_icmp_sum;
                    break;
                case IP_PROTOCOL_ICMP:
                    ...
                    if (f->ops & NAT_FLOW_OP_ICMP_ID_REWRITE) {
                        ...
                        nat_icmp_echo_header_t *inner_echo = (nat_icmp_echo_header_t *) (inner_icmp + 1);
                        if (f->rewrite.icmp_id != inner_echo->identifier) {
                            ip_csum_t sum          = icmp->checksum;
                            sum                    = ip_csum_update(sum,
                                                 inner_echo->identifier,
                                                 f->rewrite.icmp_id,
                                                 nat_icmp_echo_header_t,
                                                 identifier /* changed member */);
                            icmp->checksum         = ip_csum_fold(sum);
                            ip_csum_t inner_sum    = inner_icmp->checksum;
                            inner_sum              = ip_csum_update(sum,
                                                       inner_echo->identifier,
                                                       f->rewrite.icmp_id,
                                                       nat_icmp_echo_header_t,
                                                       identifier /* changed member */);
                                                                       ...
                                                                       int            is_i2o)
{
    ...
    if (IP_PROTOCOL_ICMP == proto) {
        if (ip->src_address.as_u32 != f->rewrite.saddr.as_u32) {
            ...
            nat_6t_flow_ip4_translate(sm, b, ip, f, proto, 0 /* is_icmp_inner_ip4 */, !is_i2o /* skip_saddr_rewrite */);
        }
        else {
            nat_6t_flow_ip4_translate(sm, b, ip, f, proto, 0 /* is_icmp_inner_ip4 */, 0 /* skip_saddr_rewrite */);
        }
...
nat_translation_error_e nat_6t_flow_buf_translate_i2o(vlib_main_t   *vm,
                                                      ...
                                                      int            is_output_feature)
{
    return nat_6t_flow_buf_translate(vm, sm, b, ip, f, proto, is_output_feature, 1 /* is_i2o */);
}
...
nat_translation_error_e nat_6t_flow_buf_translate_o2i(vlib_main_t   *vm,
                                                      ...
                                                      int            is_output_feature)
{
    return nat_6t_flow_buf_translate(vm, sm, b, ip, f, proto, is_output_feature, 0 /* is_i2o */);
}

static_always_inline void nat_syslog_nat44_sess(u32            ssubix,
                                                u32            sfibix,
                                                ...
                                                u8             is_twicenat)
{
    ...
    if (!syslog_is_enabled()) return;

    if (syslog_severity_filter_block(SADD_SDEL_SEVERITY)) return;

    fib = fib_table_get(sfibix, FIB_PROTOCOL_IP4);

    syslog_msg_init(&syslog_msg, NAT_FACILITY, SADD_SDEL_SEVERITY, NAT_APPNAME, is_add ? SADD_MSGID : SDEL_MSGID);

    syslog_msg_sd_init(&syslog_msg, NSESS_SDID);
    syslog_msg_add_sd_param(&syslog_msg, SSUBIX_SDPARAM_NAME, "%d", ssubix);
    syslog_msg_add_sd_param(&syslog_msg, SVLAN_SDPARAM_NAME, "%d", fib->ft_table_id);
    syslog_msg_add_sd_param(&syslog_msg, IATYP_SDPARAM_NAME, IATYP_IPV4);
    syslog_msg_add_sd_param(&syslog_msg, ISADDR_SDPARAM_NAME, "%U", format_ip4_address, isaddr);
    syslog_msg_add_sd_param(&syslog_msg, ISPORT_SDPARAM_NAME, "%d", clib_net_to_host_u16(isport));
    syslog_msg_add_sd_param(&syslog_msg, XATYP_SDPARAM_NAME, IATYP_IPV4);
    syslog_msg_add_sd_param(&syslog_msg, XSADDR_SDPARAM_NAME, "%U", format_ip4_address, xsaddr);
    syslog_msg_add_sd_param(&syslog_msg, XSPORT_SDPARAM_NAME, "%d", clib_net_to_host_u16(xsport));
    syslog_msg_add_sd_param(&syslog_msg, PROTO_SDPARAM_NAME, "%d", proto);
    syslog_msg_add_sd_param(&syslog_msg, XDADDR_SDPARAM_NAME, "%U", format_ip4_address, xdaddr);
    syslog_msg_add_sd_param(&syslog_msg, XDPORT_SDPARAM_NAME, "%d", clib_net_to_host_u16(xdport));
    if (is_twicenat) {
        syslog_msg_add_sd_param(&syslog_msg, IDADDR_SDPARAM_NAME, "%U", format_ip4_address, idaddr);
        syslog_msg_add_sd_param(&syslog_msg, IDPORT_SDPARAM_NAME, "%d", clib_net_to_host_u16(idport));
    }
...
void nat_syslog_nat44_sadd(u32            ssubix,
                           ...
                           u8             is_twicenat)
{
    nat_syslog_nat44_sess(ssubix, sfibix, isaddr, isport, xsaddr, xsport, idaddr, idport, xdaddr, xdport, proto, 1, is_twicenat);
}
...
void nat_syslog_nat44_sdel(u32            ssubix,
                           ...
                           u8             is_twicenat)
{
    nat_syslog_nat44_sess(ssubix, sfibix, isaddr, isport, xsaddr, xsport, idaddr, idport, xdaddr, xdport, proto, 0, is_twicenat);
}
__clib_export void nat44_original_dst_lookup(ip4_address_t *i2o_src,
                                             u16            i2o_src_port,
                                             ip4_address_t *i2o_dst,
                                             u16            i2o_dst_port,
                                             ip_protocol_t  proto,
                                             u32           *original_dst,
                                             u16           *original_dst_port)
{
    ...
    init_ed_k(&kv, i2o_src->as_u32, i2o_src_port, i2o_dst->as_u32, i2o_dst_port, fib_index, proto);
    if (tsm->sessions == NULL || clib_bihash_search_16_8(&sm->flow_hash, &kv, &value)) {
        return;
    }
    s = pool_elt_at_index(tsm->sessions, ed_value_get_session_index(&value));
    if (s) {
