...
static clib_error_t *nat44_ed_enable_disable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!unformat_user(input, unformat_line_input, line_input)) return clib_error_return(0, NAT44_ED_EXPECTED_ARGUMENT);
    ...
    if (enable) {
        ...
        if (nat44_plugin_enable(c) != 0) error = clib_error_return(0, "enable failed");
    }
    else {
        if (!sm->enabled) {
        ...
        if (nat44_plugin_disable() != 0) error = clib_error_return(0, "disable failed");
    }
...
static clib_error_t *set_workers_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    switch (rv) {
        ...
        case VNET_API_ERROR_FEATURE_DISABLED:
            error = clib_error_return(0, "Supported only if 2 or more workes available.");
            goto done;
        default:
            break;
    }
...
static clib_error_t *nat_show_workers_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    snat_main_t *sm = &snat_main;
    u32         *worker;
    ...
    if (sm->num_workers > 1) {
        ...
        vec_foreach (worker, sm->workers) {
            vlib_worker_thread_t *w = vlib_worker_threads + *worker + sm->first_worker_index;
...
static clib_error_t *add_address_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%U - %U", unformat_ip4_address, &start_addr, unformat_ip4_address, &end_addr))
            ;
    ...
    if (count > 1024) nat_log_info("%U - %U, %d addresses...", format_ip4_address, &start_addr, format_ip4_address, &end_addr, count);
...
static void nat44_show_lru_summary(vlib_main_t *vm, snat_main_per_thread_data_t *tsm, u64 now, u64 sess_timeout_time)
{
    ...
    if (tsm->lru_pool) {
#define _(n, d)                                                                                    \
    oldest_index = clib_dlist_remove_head(tsm->lru_pool, tsm->n##_lru_head_index);                 \
    if (~0 != oldest_index) {                                                                      \
        oldest_elt        = pool_elt_at_index(tsm->lru_pool, oldest_index);                        \
        s                 = pool_elt_at_index(tsm->sessions, oldest_elt->value);                   \
        sess_timeout_time = s->last_heard + (f64) nat44_session_get_timeout(sm, s);                \
        vlib_cli_output(vm, d " LRU min session timeout %llu (now %llu)", sess_timeout_time, now); \
        clib_dlist_addhead(tsm->lru_pool, tsm->n##_lru_head_index, oldest_index);                  \
    }
...
static clib_error_t *nat44_show_summary_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    struct {
        u32 total;
        u32 timed_out;
    } udp = {0}, tcp = {0}, tcp_established = {0}, tcp_transitory = {0}, icmp = {0}, other = {0};

    u32 fib;

    for (fib = 0; fib < vec_len(sm->max_translations_per_fib); fib++)
        vlib_cli_output(vm, "max translations per thread: %u fib %u", sm->max_translations_per_fib[fib], fib);
            ...
            pool_foreach (s, tsm->sessions) {
                ...
                switch (s->proto) {
                    case IP_PROTOCOL_ICMP:
                        ++icmp.total;
                        if (now >= sess_timeout_time) ++icmp.timed_out;
                        break;
                    case IP_PROTOCOL_TCP:
                        ++tcp.total;
                        if (now >= sess_timeout_time) ++tcp.timed_out;
                        if (nat44_ed_tcp_is_established(s->tcp_state)) {
                            ++tcp_established.total;
                            if (now >= sess_timeout_time) ++tcp_established.timed_out;
                        }
                        else {
                            ++tcp_transitory.total;
                            if (now >= sess_timeout_time) ++tcp_transitory.timed_out;
                        }
                        break;
                    case IP_PROTOCOL_UDP:
                        ++udp.total;
                        if (now >= sess_timeout_time) ++udp.timed_out;
                        break;
                    default:
                        ++other.total;
                        if (now >= sess_timeout_time) ++other.timed_out;
                        break;
                }
            }
    ...
    u32 timed_out = tcp.timed_out + icmp.timed_out + udp.timed_out + other.timed_out;
    vlib_cli_output(vm, "total sessions: %u (timed out: %u)", count, timed_out);
    vlib_cli_output(vm, "tcp sessions:");
    vlib_cli_output(vm, "    total: %u (timed out: %u)", tcp.total, tcp.timed_out);
    vlib_cli_output(vm, "        established: %u (timed out: %u)", tcp_established.total, tcp_established.timed_out);
    vlib_cli_output(vm, "        transitory: %u (timed out: %u)", tcp_transitory.total, tcp_transitory.timed_out);
    vlib_cli_output(vm, "udp sessions:");
    vlib_cli_output(vm, "    total: %u (timed out: %u)", udp.total, udp.timed_out);
    vlib_cli_output(vm, "icmp sessions:");
    vlib_cli_output(vm, "    total: %u (timed out: %u)", icmp.total, icmp.timed_out);
    vlib_cli_output(vm, "other sessions:");
    vlib_cli_output(vm, "    total: %u (timed out: %u)", other.total, other.timed_out);
    return 0;
}
...
static clib_error_t *nat44_show_addresses_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vec_foreach (ap, sm->addresses) {
        ...
        if (ap->fib_index != ~0)
            vlib_cli_output(vm, "  tenant VRF: %u", fib_table_get(ap->fib_index, FIB_PROTOCOL_IP4)->ft_table_id);
        else
            vlib_cli_output(vm, "  tenant VRF independent");

        if (ap->addr_len != ~0) vlib_cli_output(vm, "  synced with interface address");
    }
...
static clib_error_t *snat_feature_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "in %U", unformat_vnet_sw_interface, vnm, &sw_if_index))
            vec_add1(inside_sw_if_indices, sw_if_index);
        else if (unformat(line_input, "out %U", unformat_vnet_sw_interface, vnm, &sw_if_index))
    ...
    if (vec_len(inside_sw_if_indices)) {
        for (i = 0; i < vec_len(inside_sw_if_indices); i++) {
            ...
            if (is_output_feature) {
                ...
                if (rv) {
                    error = clib_error_return(0, "%s %U failed", is_del ? "del" : "add", format_vnet_sw_if_index_name, vnm, sw_if_index);
                    goto done;
                }
            }
            else {
...
static clib_error_t *nat44_show_interfaces_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    pool_foreach (i, sm->interfaces) {
        vlib_cli_output(
            ...
            vnm,
            i->sw_if_index,
            (nat44_ed_is_interface_inside(i) && nat44_ed_is_interface_outside(i)) ? "in out" : (nat44_ed_is_interface_inside(i) ? "in" : "out"));
    }
...
static clib_error_t *add_static_mapping_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "local %U %u", unformat_ip4_address, &l_addr, &l_port)) {
            l_port_set = 1;
        }
        else if (unformat(line_input, "local %U", unformat_ip4_address, &l_addr))
            ;
        else if (unformat(line_input, "external %U %u", unformat_ip4_address, &e_addr, &e_port)) {
            e_port_set = 1;
        }
        else if (unformat(line_input, "external %U", unformat_ip4_address, &e_addr))
            ;
        else if (unformat(line_input, "external %U %u", unformat_vnet_sw_interface, vnm, &sw_if_index, &e_port)) {
            flags |= NAT_SM_FLAG_SWITCH_ADDRESS;
            e_port_set = 1;
        }
        else if (unformat(line_input, "external %U", unformat_vnet_sw_interface, vnm, &sw_if_index)) {
            flags |= NAT_SM_FLAG_SWITCH_ADDRESS;
        }
...
static clib_error_t *add_identity_mapping_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%U", unformat_ip4_address, &addr))
            ...
            ;
        else if (unformat(line_input, "%U %u", unformat_ip_protocol, &proto, &port)) {
            port_set = 1;
        }
        else if (unformat(line_input, "del")) {
            is_add = 0;
    ...
    if (is_add) {
        rv = nat44_ed_add_static_mapping(addr, addr, port, port, proto, vrf_id, sw_if_index, flags, addr, 0);
    }
    else {
        rv = nat44_ed_del_static_mapping(addr, addr, port, port, proto, vrf_id, sw_if_index, flags);
    }
...
static clib_error_t *add_lb_static_mapping_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "local %U:%u probability %u", unformat_ip4_address, &l_addr, &l_port, &probability)) {
        ...
        else if (unformat(line_input, "local %U:%u vrf %u probability %u", unformat_ip4_address, &l_addr, &l_port, &vrf_id, &probability)) {
            clib_memset(&local, 0, sizeof(local));
            local.addr        = l_addr;
            l_port            = clib_host_to_net_u16(l_port);
            local.port        = (u16) l_port;
        ...
        else if (unformat(line_input, "protocol %U", unformat_ip_protocol, &proto)) {
            proto_set = 1;
        }
        else if (unformat(line_input, "twice-nat")) {
            flags |= NAT_SM_FLAG_TWICE_NAT;
    ...
    if (is_add) {
        rv = nat44_ed_add_lb_static_mapping(e_addr, (u16) e_port, proto, locals, flags, 0, affinity);
    }
    else {
...
static clib_error_t *add_lb_backend_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    rv = nat44_ed_add_del_lb_static_mapping_local(e_addr, (u16) e_port, l_addr, l_port, proto, vrf_id, probability, is_add);

    switch (rv) {
        case VNET_API_ERROR_INVALID_VALUE:
            error = clib_error_return(0,
                                      "External is not load-balancing static "
                                      "mapping.");
            goto done;
        case VNET_API_ERROR_NO_SUCH_ENTRY:
...
static clib_error_t *snat_add_interface_address_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (0 != rv) {
        error = clib_error_return(0, "%s %U address failed", is_del ? "del" : "add", format_vnet_sw_if_index_name, vnm, sw_if_index);
        goto done;
    }
...
static clib_error_t *nat44_ed_add_del_vrf_table_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    clib_error_t    *error  = 0;
    ...
    if (rv) {
        error = clib_error_return(0, "%s vrf table returned %d", is_add ? "add" : "del", rv);
    }
...
static clib_error_t *nat44_ed_add_del_vrf_route_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    unformat_input_t _line_input, *line_input = &_line_input;
    clib_error_t    *error  = 0;
    bool             is_add = true, not_set = true;
...
static clib_error_t *nat44_ed_show_vrf_tables_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    snat_main_t *sm = &snat_main;
    vrf_table_t *t;
    vrf_route_t *r;
...
static clib_error_t *nat44_show_interface_address_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vec_foreach (ap, sm->addr_to_resolve) {
        vlib_cli_output(vm, " %U%s", format_vnet_sw_if_index_name, vnm, ap->sw_if_index, ap->is_twice_nat ? " twice-nat" : "");
    }
    return 0;
}
...
static clib_error_t *nat44_show_sessions_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "filter i2o saddr %U", unformat_ip4_address, &i2o_sa))
            filter_i2o_sa = filtering = 1;
        else if (unformat(line_input, "filter i2o daddr %U", unformat_ip4_address, &i2o_da))
            filter_i2o_da = filtering = 1;
        else if (unformat(line_input, "filter o2i saddr %U", unformat_ip4_address, &o2i_sa))
            filter_o2i_sa = filtering = 1;
        else if (unformat(line_input, "filter o2i daddr %U", unformat_ip4_address, &o2i_da))
        ...
        else if (unformat(line_input, "filter i2o proto %U", unformat_ip_protocol, &proto))
            filter_proto = filtering = 1;
        else if (unformat(line_input, "filter o2i proto %U", unformat_ip_protocol, &proto))
            filter_proto = filtering = 1;
        else {
            error = clib_error_return(0, "unknown input '%U'", format_unformat_error, line_input);
            goto done;
...
print:
    ...
    vec_foreach_index (i, sm->per_thread_data) {
        ...
        vlib_cli_output(vm, "-------- thread %d %s: %d sessions --------\n", i, vlib_worker_threads[i].name, pool_elts(tsm->sessions));

        showed_sessions = 0;
        snat_session_t *s;
        pool_foreach (s, tsm->sessions) {
            if (filtering) {
                if (filter_i2o_sa && i2o_sa.as_u32 != s->i2o.match.saddr.as_u32) continue;
                if (filter_i2o_da && i2o_da.as_u32 != s->i2o.match.daddr.as_u32) continue;
                if (filter_o2i_sa && o2i_sa.as_u32 != s->o2i.match.saddr.as_u32) continue;
                if (filter_o2i_da && o2i_da.as_u32 != s->o2i.match.daddr.as_u32) continue;
                if (filter_i2o_sp && i2o_sp != clib_net_to_host_u16(s->i2o.match.sport)) continue;
                if (filter_i2o_dp && i2o_dp != clib_net_to_host_u16(s->i2o.match.dport)) continue;
                if (filter_o2i_sp && o2i_sp != clib_net_to_host_u16(s->o2i.match.sport)) continue;
                if (filter_o2i_dp && o2i_dp != clib_net_to_host_u16(s->o2i.match.dport)) continue;
                if (filter_proto && proto != s->proto) continue;
                showed_sessions++;
            }
            vlib_cli_output(vm, "  %U\n", format_snat_session, sm, tsm, s, vlib_time_now(vm));
        }
        if (filtering) {
            vlib_cli_output(vm,
    ...
    if (had_input) unformat_free(line_input);
    return error;
}
...
static clib_error_t *nat44_del_session_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%U:%u %U", unformat_ip4_address, &addr, &port, unformat_ip_protocol, &proto))
            ;
            ...
            ;
        else if (unformat(line_input, "external-host %U:%u", unformat_ip4_address, &eh_addr, &eh_port))
            ;
        else {
            error = clib_error_return(0, "unknown input '%U'", format_unformat_error, line_input);
            goto done;
    ...
    rv = nat44_ed_del_session(sm, &addr, clib_host_to_net_u16(port), &eh_addr, clib_host_to_net_u16(eh_port), proto, vrf_id, is_in);
...
static clib_error_t *set_timeout_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "udp %u", &sm->timeouts.udp))
            ;
        else if (unformat(line_input, "tcp-established %u", &sm->timeouts.tcp.established))
            ;
        else if (unformat(line_input, "tcp-transitory %u", &sm->timeouts.tcp.transitory))
            ;
...
static clib_error_t *nat_show_timeouts_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vlib_cli_output(vm, "tcp-established timeout: %dsec", sm->timeouts.tcp.established);
    vlib_cli_output(vm, "tcp-transitory timeout: %dsec", sm->timeouts.tcp.transitory);
 ...
 *  vpp# nat44 add load-balancing static mapping protocol tcp external 1.2.3.4:80 local 10.100.10.10:8080 probability 80 local 10.100.10.20:8080
probability 20
 * @cliexend
?*/
...
VLIB_CLI_COMMAND(nat44_show_sessions_command, static) = {
    ...
    .short_help =
        "show nat44 sessions [filter {i2o | o2i} {saddr <ip4-addr> "
