...
static clib_error_t *nat66_enable_disable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!unformat_user(input, unformat_line_input, line_input)) return clib_error_return(0, NAT66_EXPECTED_ARGUMENT);
    ...
    if (enable) {
        ...
        if (nat66_plugin_enable(outside_vrf) != 0) error = clib_error_return(0, "enable failed");
    }
    else {
        if (!nm->enabled) {
        ...
        if (nat66_plugin_disable() != 0) error = clib_error_return(0, "disable failed");
    }
        ...
        for (i = 0; i < vec_len(inside_sw_if_indices); i++) {
            ...
            switch (rv) {
                ...
                case VNET_API_ERROR_VALUE_EXIST:
                    error = clib_error_return(0,
                                              "%U NAT66 feature already enabled.",
                                              format_vnet_sw_interface_name,
                                              vnm,
                                              vnet_get_sw_interface(vnm, sw_if_index));
                    goto done;
...
static clib_error_t *nat66_add_del_static_mapping_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "local %U external %U", unformat_ip6_address, &l_addr, unformat_ip6_address, &e_addr))
            ;
...
static int nat66_cli_static_mapping_walk(nat66_static_mapping_t *sm, void *ctx)
{
    ...
    vlib_cli_output(vm, " local %U external %U vrf %d", format_ip6_address, &sm->l_addr, format_ip6_address, &sm->e_addr, fib->ft_table_id);
    vlib_cli_output(vm, "  total pkts %lld, total bytes %lld", vc.packets, vc.bytes);
