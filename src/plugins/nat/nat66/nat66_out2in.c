...
static u8 *format_nat66_out2in_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "NAT66-out2in: sw_if_index %d, next index %d", t->sw_if_index, t->next_index);
...
VLIB_NODE_FN(nat66_out2in_node)
    ...
    while (n_left_from > 0) {
        ...
        trace0:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                nat66_out2in_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                t->sw_if_index          = vnet_buffer(b0)->sw_if_index[VLIB_RX];
                t->next_index           = next0;
            }

            if (next0 != NAT66_OUT2IN_NEXT_DROP) {
                vlib_increment_simple_counter(&nm->out2in_packets, thread_index, sw_if_index0, 1);
            }
