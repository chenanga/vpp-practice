...
static clib_error_t *nat66_init(vlib_main_t *vm)
{
    ...
    nm->nat_fib_src_hi = fib_source_allocate("nat66-hi", FIB_SOURCE_PRIORITY_HI, FIB_SOURCE_BH_SIMPLE);
...
int nat66_plugin_enable(u32 outside_vrf)
{
    ...
    clib_bihash_init_24_8(&nm->sm_l, "nat66-static-map-by-local", static_mapping_buckets, static_mapping_memory_size);
    clib_bihash_init_24_8(&nm->sm_e, "nat66-static-map-by-external", static_mapping_buckets, static_mapping_memory_size);

    nm->outside_vrf_id    = outside_vrf;
    nm->outside_fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP6, outside_vrf, nm->nat_fib_src_hi);
    nm->enabled           = 1;
    return 0;
}
...
int nat66_plugin_disable()
{
    ...
    pool_foreach (i, temp) {
        if (nat66_interface_is_inside(i)) error = nat66_interface_add_del(i->sw_if_index, 1, 0);

        if (nat66_interface_is_outside(i)) error = nat66_interface_add_del(i->sw_if_index, 0, 0);
...
int nat66_interface_add_del(u32 sw_if_index, u8 is_inside, u8 is_add)
{
    ...
    if (is_add) {
        if (interface) return VNET_API_ERROR_VALUE_EXIST;

        pool_get(nm->interfaces, interface);
        interface->sw_if_index = sw_if_index;
        interface->flags       = is_inside ? NAT66_INTERFACE_FLAG_IS_INSIDE : NAT66_INTERFACE_FLAG_IS_OUTSIDE;
    ...
    if (rv) return rv;
    return vnet_feature_enable_disable("ip6-unicast", feature_name, sw_if_index, is_add, 0, 0);
}
...
nat66_static_mapping_t *nat66_static_mapping_get(ip6_address_t *addr, u32 fib_index, u8 is_local)
{
    ...
    if (!clib_bihash_search_24_8(is_local ? &nm->sm_l : &nm->sm_e, &kv, &value)) sm = pool_elt_at_index(nm->sm, value.value);
...
int nat66_static_mapping_add_del(ip6_address_t *l_addr, ip6_address_t *e_addr, u32 vrf_id, u8 is_add)
{
    ...
    if (!clib_bihash_search_24_8(&nm->sm_l, &kv, &value)) sm = pool_elt_at_index(nm->sm, value.value);

    if (is_add) {
        if (sm) return VNET_API_ERROR_VALUE_EXIST;

        fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP6, vrf_id, nm->nat_fib_src_hi);
        pool_get(nm->sm, sm);
        ...
        kv.key[2]        = sm_key.as_u64[2];
        kv.value         = sm - nm->sm;
        if (clib_bihash_add_del_24_8(&nm->sm_l, &kv, 1)) nat66_elog_warn("nat66-static-map-by-local add key failed");
        ...
        kv.key[1]             = sm_key.as_u64[1];
        kv.key[2]             = sm_key.as_u64[2];
        if (clib_bihash_add_del_24_8(&nm->sm_e, &kv, 1)) nat66_elog_warn("nat66-static-map-by-external add key failed");

        vlib_validate_combined_counter(&nm->session_counters, kv.value);
        vlib_zero_combined_counter(&nm->session_counters, kv.value);
    }
    else {
        if (!sm) return VNET_API_ERROR_NO_SUCH_ENTRY;

        kv.value = sm - nm->sm;
        if (clib_bihash_add_del_24_8(&nm->sm_l, &kv, 0)) nat66_elog_warn("nat66-static-map-by-local delete key failed");
        sm_key.addr.as_u64[0] = e_addr->as_u64[0];
        sm_key.addr.as_u64[1] = e_addr->as_u64[1];
        sm_key.fib_index      = 0;
        kv.key[0]             = sm_key.as_u64[0];
        kv.key[1]             = sm_key.as_u64[1];
        kv.key[2]             = sm_key.as_u64[2];
        if (clib_bihash_add_del_24_8(&nm->sm_e, &kv, 0)) nat66_elog_warn("nat66-static-map-by-external delete key failed");
