...
static u8 *format_nat66_in2out_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "NAT66-in2out: sw_if_index %d, next index %d", t->sw_if_index, t->next_index);
...
static inline u8 nat66_not_translate(u32 rx_fib_index, ip6_address_t ip6_addr)
{
    ...
    if (FIB_NODE_INDEX_INVALID == fei) return 1;
    ...
    pool_foreach (i, nm->interfaces) {
        ...
        if (nat66_interface_is_outside(i) && sw_if_index == i->sw_if_index) return 0;
    }

    return 1;
}

VLIB_NODE_FN(nat66_in2out_node)
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(ip6_parse(vm, b0, ip60, b0->current_length, &l4_protocol0, &l4_offset0, &frag_offset0))) {
            ...
            if (nat66_not_translate(fib_index0, ip60->dst_address)) goto trace0;
        ...
        skip_csum0:
            ...
            vlib_increment_combined_counter(&nm->session_counters, thread_index, sm0 - nm->sm, 1, vlib_buffer_length_in_chain(vm, b0));

        trace0:
            if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
                nat66_in2out_trace_t *t = vlib_add_trace(vm, node, b0, sizeof(*t));
                t->sw_if_index          = vnet_buffer(b0)->sw_if_index[VLIB_RX];
                t->next_index           = next0;
            }

            if (next0 != NAT66_IN2OUT_NEXT_DROP) {
                vlib_increment_simple_counter(&nm->in2out_packets, thread_index, sw_if_index0, 1);
            }
