...
static void vl_api_det44_add_del_map_t_handler(vl_api_det44_add_del_map_t *mp)
{
    ...
    rv = snat_det_add_map(&in_addr, mp->in_plen, &out_addr, mp->out_plen, mp->is_add);
...
send_reply:
    REPLY_MACRO2(VL_API_DET44_REVERSE_REPLY, ({ clib_memcpy(rmp->in_addr, &in_addr, 4); }))
}
...
static void vl_api_det44_session_dump_t_handler(vl_api_det44_session_dump_t *mp)
{
    ...
    for (i = 0; i < DET44_SES_PER_USER; i++) {
        if (s->out.as_u64) send_det44_session_details(s, reg, mp->context);
        s++;
    }
}
...
send_reply:
    REPLY_MACRO2(VL_API_NAT_DET_REVERSE_REPLY, ({ clib_memcpy(rmp->in_addr, &in_addr, 4); }))
}
...
static void vl_api_nat_det_session_dump_t_handler(vl_api_nat_det_session_dump_t *mp)
{
    ...
    s = m->sessions + snat_det_user_ses_offset(&user_addr, m->in_plen);
    for (i = 0; i < DET44_SES_PER_USER; i++) {
        if (s->out.as_u64) send_nat_det_session_details(s, reg, mp->context);
        s++;
    }
}
