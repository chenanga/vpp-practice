...
static u8 *format_det44_in2out_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "DET44_IN2OUT: sw_if_index %d, next index %d, session %d", t->sw_if_index, t->next_index, t->session_index);
                          ...
                          u8                  *dont_translate)
{
    ...
    *dont_translate = 0;
    ...
    if (PREDICT_FALSE(!mp0)) {
        if (PREDICT_FALSE(det44_translate(node, sw_if_index0, ip0, IP_PROTOCOL_ICMP, rx_fib_index0))) {
            *dont_translate = 1;
            goto out;
        }
        next0     = DET44_IN2OUT_NEXT_DROP;
    ...
    if (PREDICT_FALSE(!ses0)) {
        ...
        for (i0 = 0; i0 < mp0->ports_per_host; i0++) {
            key0.out_port = clib_host_to_net_u16(lo_port0 + ((i0 + clib_net_to_host_u16(echo0->identifier)) % mp0->ports_per_host));

            if (snat_det_get_ses_by_out(mp0, &in_addr, key0.as_u64)) continue;

            ses0 = snat_det_ses_create(thread_index, mp0, &in_addr, echo0->identifier, &key0);
            break;
        }
        if (PREDICT_FALSE(!ses0)) {
            next0     = DET44_IN2OUT_NEXT_DROP;
...
out:
    *proto = protocol;
    ...
    if (d) *(snat_det_session_t **) d = ses0;
    if (e) *(snat_det_map_t **) e = mp0;
    return next0;
}
#endif
                      ...
                      void                *e)
{
    ...
    echo0     = (icmp_echo_header_t *) (icmp0 + 1);
    next0_tmp = icmp_match_in2out_det(node, thread_index, b0, ip0, &addr, &port, &fib_index, &protocol, d, e, &dont_translate);
    if (next0_tmp != ~0) next0 = next0_tmp;
    if (next0 == DET44_IN2OUT_NEXT_DROP || dont_translate) goto out;
    ...
    if (vnet_buffer(b0)->sw_if_index[VLIB_TX] == ~0) vnet_buffer(b0)->sw_if_index[VLIB_TX] = fib_index;
out:
    return next0;
}
#endif
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from >= 2) {
        ...
        if (PREDICT_FALSE(proto0 == NAT_PROTOCOL_ICMP)) {
            ...
            // TODO:
            next0 = det44_icmp_in2out(b0, ip0, icmp0, sw_if_index0, rx_fib_index0, node, next0, thread_index, &ses0, &mp0);
            goto trace0;
        }

        mp0 = snat_det_map_by_user(&ip0->src_address);
        if (PREDICT_FALSE(!mp0)) {
            det44_log_info("no match for internal host %U", format_ip4_address, &ip0->src_address);
            next0     = DET44_IN2OUT_NEXT_DROP;
            b0->error = node->errors[DET44_IN2OUT_ERROR_NO_TRANSLATION];
            goto trace0;
        }
        ...
        if (PREDICT_FALSE(!ses0)) {
            for (i0 = 0; i0 < mp0->ports_per_host; i0++) {
                key0.out_port = clib_host_to_net_u16(lo_port0 + ((i0 + clib_net_to_host_u16(tcp0->src)) % mp0->ports_per_host));

                if (snat_det_get_ses_by_out(mp0, &ip0->src_address, key0.as_u64)) continue;

                ses0 = snat_det_ses_create(thread_index, mp0, &ip0->src_address, tcp0->src, &key0);
                break;
            }
            if (PREDICT_FALSE(!ses0)) {
                /* too many sessions for user, send ICMP error packet */
                vnet_buffer(b0)->sw_if_index[VLIB_TX] = (u32) ~0;
                icmp4_error_set_vnet_buffer(b0, ICMP4_destination_unreachable, ICMP4_destination_unreachable_destination_unreachable_host, 0);
                next0 = DET44_IN2OUT_NEXT_ICMP_ERROR;
                goto trace0;
            }
        }
        ...
        sum0          = ip0->checksum;
        sum0          = ip_csum_update(sum0, old_addr0.as_u32, new_addr0.as_u32, ip4_header_t, src_address /* changed member */);
        ip0->checksum = ip_csum_fold(sum0);

        if (PREDICT_TRUE(proto0 == NAT_PROTOCOL_TCP)) {
            if (tcp0->flags & TCP_FLAG_SYN)
                ses0->state = DET44_SESSION_TCP_SYN_SENT;
            else if (tcp0->flags & TCP_FLAG_ACK && ses0->state == DET44_SESSION_TCP_SYN_SENT)
                ses0->state = DET44_SESSION_TCP_ESTABLISHED;
            else if (tcp0->flags & TCP_FLAG_FIN && ses0->state == DET44_SESSION_TCP_ESTABLISHED)
                ses0->state = DET44_SESSION_TCP_FIN_WAIT;
            else if (tcp0->flags & TCP_FLAG_ACK && ses0->state == DET44_SESSION_TCP_FIN_WAIT)
                snat_det_ses_close(mp0, ses0);
            else if (tcp0->flags & TCP_FLAG_FIN && ses0->state == DET44_SESSION_TCP_CLOSE_WAIT)
                ses0->state = DET44_SESSION_TCP_LAST_ACK;
            else if (tcp0->flags == 0 && ses0->state == DET44_SESSION_UNKNOWN)
                ses0->state = DET44_SESSION_TCP_ESTABLISHED;

            sum0 = tcp0->checksum;
            sum0 = ip_csum_update(sum0, old_addr0.as_u32, new_addr0.as_u32, ip4_header_t, dst_address /* changed member */);
    ...
    trace0:
        if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b0->flags & VLIB_BUFFER_IS_TRACED))) {
            ...
            t->next_index           = next0;
            t->session_index        = ~0;
            if (ses0) t->session_index = ses0 - mp0->sessions;
        }
        ...
        if (PREDICT_FALSE(proto1 == NAT_PROTOCOL_ICMP)) {
            ...
            next1 = det44_icmp_in2out(b1, ip1, icmp1, sw_if_index1, rx_fib_index1, node, next1, thread_index, &ses1, &mp1);
            goto trace1;
        }
        ...
        if (PREDICT_FALSE(!ses1)) {
            for (i1 = 0; i1 < mp1->ports_per_host; i1++) {
                key1.out_port = clib_host_to_net_u16(lo_port1 + ((i1 + clib_net_to_host_u16(tcp1->src)) % mp1->ports_per_host));

                if (snat_det_get_ses_by_out(mp1, &ip1->src_address, key1.as_u64)) continue;

                ses1 = snat_det_ses_create(thread_index, mp1, &ip1->src_address, tcp1->src, &key1);
                break;
            }
            if (PREDICT_FALSE(!ses1)) {
                /* too many sessions for user, send ICMP error packet */
                vnet_buffer(b1)->sw_if_index[VLIB_TX] = (u32) ~0;
                icmp4_error_set_vnet_buffer(b1, ICMP4_destination_unreachable, ICMP4_destination_unreachable_destination_unreachable_host, 0);
                next1 = DET44_IN2OUT_NEXT_ICMP_ERROR;
                goto trace1;
            }
        }
        ...
        sum1          = ip1->checksum;
        sum1          = ip_csum_update(sum1, old_addr1.as_u32, new_addr1.as_u32, ip4_header_t, src_address /* changed member */);
        ip1->checksum = ip_csum_fold(sum1);

        if (PREDICT_TRUE(proto1 == NAT_PROTOCOL_TCP)) {
            if (tcp1->flags & TCP_FLAG_SYN)
                ses1->state = DET44_SESSION_TCP_SYN_SENT;
            else if (tcp1->flags & TCP_FLAG_ACK && ses1->state == DET44_SESSION_TCP_SYN_SENT)
                ses1->state = DET44_SESSION_TCP_ESTABLISHED;
            else if (tcp1->flags & TCP_FLAG_FIN && ses1->state == DET44_SESSION_TCP_ESTABLISHED)
                ses1->state = DET44_SESSION_TCP_FIN_WAIT;
            else if (tcp1->flags & TCP_FLAG_ACK && ses1->state == DET44_SESSION_TCP_FIN_WAIT)
                snat_det_ses_close(mp1, ses1);
            else if (tcp1->flags & TCP_FLAG_FIN && ses1->state == DET44_SESSION_TCP_CLOSE_WAIT)
                ses1->state = DET44_SESSION_TCP_LAST_ACK;
            else if (tcp1->flags == 0 && ses1->state == DET44_SESSION_UNKNOWN)
                ses1->state = DET44_SESSION_TCP_ESTABLISHED;

            sum1 = tcp1->checksum;
            sum1 = ip_csum_update(sum1, old_addr1.as_u32, new_addr1.as_u32, ip4_header_t, dst_address /* changed member */);
    ...
    trace1:
        if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b1->flags & VLIB_BUFFER_IS_TRACED))) {
            ...
            t->next_index           = next1;
            t->session_index        = ~0;
            if (ses1) t->session_index = ses1 - mp1->sessions;
        }
    ...
    vlib_node_increment_counter(vm, dm->in2out_node_index, DET44_IN2OUT_ERROR_IN2OUT_PACKETS, pkts_processed);
