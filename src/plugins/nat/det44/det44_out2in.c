...
static u8 *format_det44_out2in_trace(u8 *s, va_list *args)
{
    ...
    s = format(s, "DET44_OUT2IN: sw_if_index %d, next index %d, session index %d", t->sw_if_index, t->next_index, t->session_index);
    return s;
}
                          ...
                          u8                  *dont_translate)
{
    ...
    *dont_translate = 0;
    ...
    if (PREDICT_FALSE(!mp0)) {
        ...
        if (PREDICT_FALSE(!det44_is_interface_addr(node, sw_if_index0, ip0->dst_address.as_u32))) {
            *dont_translate = 1;
            goto out;
        }
        det44_log_info("unknown dst address:  %U", format_ip4_address, &ip0->dst_address);
    ...
    snat_det_reverse(mp0, &ip0->dst_address, clib_net_to_host_u16(key0.out_port), &new_addr0);
                      ...
                      void                *e)
{
    ...
    echo0     = (icmp_echo_header_t *) (icmp0 + 1);
    next0_tmp = icmp_match_out2in_det(node, thread_index, b0, ip0, &addr, &port, &fib_index, &proto, d, e, &dont_translate);
    if (next0_tmp != ~0) next0 = next0_tmp;
    if (next0 == DET44_OUT2IN_NEXT_DROP || dont_translate) goto out;
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from >= 2) {
        ...
        if (PREDICT_FALSE(proto0 == NAT_PROTOCOL_ICMP)) {
            ...
            next0 = det44_icmp_out2in(b0, ip0, icmp0, sw_if_index0, rx_fib_index0, node, next0, thread_index, &ses0, &mp0);
            goto trace0;
        }
        ...
        snat_det_reverse(mp0, &ip0->dst_address, clib_net_to_host_u16(tcp0->dst), &new_addr0);
        ...
        if (PREDICT_TRUE(proto0 == NAT_PROTOCOL_TCP)) {
            if (tcp0->flags & TCP_FLAG_FIN && ses0->state == DET44_SESSION_TCP_ESTABLISHED)
                ses0->state = DET44_SESSION_TCP_CLOSE_WAIT;
            else if (tcp0->flags & TCP_FLAG_ACK && ses0->state == DET44_SESSION_TCP_LAST_ACK)
    ...
    trace0:
        ...
        if (PREDICT_FALSE(proto1 == NAT_PROTOCOL_ICMP)) {
            ...
            next1 = det44_icmp_out2in(b1, ip1, icmp1, sw_if_index1, rx_fib_index1, node, next1, thread_index, &ses1, &mp1);
            goto trace1;
        }
        ...
        if (PREDICT_FALSE(!mp1)) {
            det44_log_info("unknown dst address:  %U", format_ip4_address, &ip1->dst_address);
            next1     = DET44_OUT2IN_NEXT_DROP;
            b1->error = node->errors[DET44_OUT2IN_ERROR_NO_TRANSLATION];
            goto trace1;
        }

        snat_det_reverse(mp1, &ip1->dst_address, clib_net_to_host_u16(tcp1->dst), &new_addr1);
        ...
        if (PREDICT_TRUE(proto1 == NAT_PROTOCOL_TCP)) {
            if (tcp1->flags & TCP_FLAG_FIN && ses1->state == DET44_SESSION_TCP_ESTABLISHED)
                ses1->state = DET44_SESSION_TCP_CLOSE_WAIT;
            else if (tcp1->flags & TCP_FLAG_ACK && ses1->state == DET44_SESSION_TCP_LAST_ACK)
    ...
    trace1:
        ...
        if (PREDICT_FALSE((node->flags & VLIB_NODE_FLAG_TRACE) && (b1->flags & VLIB_BUFFER_IS_TRACED))) {
            det44_out2in_trace_t *t = vlib_add_trace(vm, node, b1, sizeof(*t));
    ...
    vlib_node_increment_counter(vm, dm->out2in_node_index, DET44_OUT2IN_ERROR_OUT2IN_PACKETS, pkts_processed);
