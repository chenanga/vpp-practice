...
{
    if (net->as_u32 == (addr->as_u32 & ip4_main.fib_masks[plen])) return 1;
    return 0;
}
...
static_always_inline snat_det_map_t *snat_det_map_by_user(ip4_address_t *user_addr)
{
    ...
    pool_foreach (mp, dm->det_maps) {
        if (is_addr_in_net(user_addr, &mp->in_addr, mp->in_plen)) return mp;
    }
    return 0;
}
...
static_always_inline snat_det_map_t *snat_det_map_by_out(ip4_address_t *out_addr)
{
    ...
    snat_det_map_t *mp;
    pool_foreach (mp, dm->det_maps) {
        if (is_addr_in_net(out_addr, &mp->out_addr, mp->out_plen)) return mp;
    }
    return 0;
}
...
static_always_inline void snat_det_forward(snat_det_map_t *dm, ip4_address_t *in_addr, ip4_address_t *out_addr, u16 *lo_port)
{
    ...
    in_offset        = clib_net_to_host_u32(in_addr->as_u32) - clib_net_to_host_u32(dm->in_addr.as_u32);
    out_offset       = in_offset / dm->sharing_ratio;
    out_addr->as_u32 = clib_host_to_net_u32(clib_net_to_host_u32(dm->out_addr.as_u32) + out_offset);
...
static_always_inline void snat_det_reverse(snat_det_map_t *dm, ip4_address_t *out_addr, u16 out_port, ip4_address_t *in_addr)
{
    ...
    out_offset      = clib_net_to_host_u32(out_addr->as_u32) - clib_net_to_host_u32(dm->out_addr.as_u32);
    in_offset1      = out_offset * dm->sharing_ratio;
    in_offset2      = (out_port - 1024) / dm->ports_per_host;
    in_addr->as_u32 = clib_host_to_net_u32(clib_net_to_host_u32(dm->in_addr.as_u32) + in_offset1 + in_offset2);
}

static_always_inline u32 snat_det_user_ses_offset(ip4_address_t *addr, u8 plen)
{
    return (clib_net_to_host_u32(addr->as_u32) & pow2_mask(32 - plen)) * DET44_SES_PER_USER;
}
...
static_always_inline snat_det_session_t *snat_det_get_ses_by_out(snat_det_map_t *dm, ip4_address_t *in_addr, u64 out_key)
{
    ...
    for (i = 0; i < DET44_SES_PER_USER; i++) {
        if (dm->sessions[i + user_offset].out.as_u64 == out_key) return &dm->sessions[i + user_offset];
    }
...
static_always_inline snat_det_session_t *snat_det_find_ses_by_in(snat_det_map_t *dm, ip4_address_t *in_addr, u16 in_port, snat_det_out_key_t out_key)
{
    ...
    for (i = 0; i < DET44_SES_PER_USER; i++) {
        ...
        if (ses->in_port == in_port && ses->out.ext_host_addr.as_u32 == out_key.ext_host_addr.as_u32
                                                             ...
                                                             snat_det_out_key_t *out)
{
    ...
    for (i = 0; i < DET44_SES_PER_USER; i++) {
        if (!dm->sessions[i + user_offset].in_port) {
            if (clib_atomic_bool_cmp_and_swap(&dm->sessions[i + user_offset].in_port, 0, in_port)) {
    ...
    nat_ipfix_logging_max_entries_per_user(thread_index, DET44_SES_PER_USER, in_addr->as_u32);
    return 0;
}
