...
int snat_det_add_map(ip4_address_t *in_addr, u8 in_plen, ip4_address_t *out_addr, u8 out_plen, int is_add)
{
    ...
    vec_foreach (mp, dm->det_maps) {
        ...
        if (mp->in_addr.as_u32 == in_cmp.as_u32 && mp->in_plen == in_plen && mp->out_addr.as_u32 == out_cmp.as_u32 && mp->out_plen == out_plen) {
            found = 1;
            break;
        }
    }
    ...
    if (found && is_add) return VNET_API_ERROR_VALUE_EXIST;

    /* If not found, don't delete */
    if (!found && !is_add) return VNET_API_ERROR_NO_SUCH_ENTRY;
    ...
    if (is_add) {
        ...
        vec_validate_init_empty(mp->sessions, DET44_SES_PER_USER * (1 << (32 - in_plen)) - 1, empty_snat_det_session);
    }
    else {
    ...
    pool_foreach (i, dm->interfaces) {
        if (det44_interface_is_inside(i)) continue;
...
{
    det44_main_t *dm = &det44_main;
    if (timeouts->udp) dm->timeouts.udp = timeouts->udp;
    if (timeouts->tcp.established) dm->timeouts.tcp.established = timeouts->tcp.established;
    if (timeouts->tcp.transitory) dm->timeouts.tcp.transitory = timeouts->tcp.transitory;
    if (timeouts->icmp) dm->timeouts.icmp = timeouts->icmp;
    return 0;
}
...
out:
    ...
    if (!is_inside) {
        ...
        pool_foreach (mp, dm->det_maps) {
            det44_add_del_addr_to_fib(&mp->out_addr, mp->out_plen, sw_if_index, !is_del);
        }
    }
    return 0;
}
...
void det44_create_expire_walk_process()
{
    ...
    if (dm->expire_walk_node_index) return;

    dm->expire_walk_node_index = vlib_process_create(vlib_get_main(), "det44-expire-walk", det44_expire_walk_fn, 16 /* stack_bytes */);
}
...
int det44_plugin_enable(det44_config_t c)
{
    ...
    dm->outside_fib_index = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, c.outside_vrf_id, dm->fib_src_hi);
    dm->inside_fib_index  = fib_table_find_or_create_and_lock(FIB_PROTOCOL_IP4, c.inside_vrf_id, dm->fib_src_hi);
...
int det44_plugin_disable()
{
    ...
    vec_foreach (i, interfaces) {
        ...
        if (i->flags & DET44_INTERFACE_FLAG_IS_INSIDE) {
            ...
            if (rv) {
                det44_log_err("inside interface %U del failed", unformat_vnet_sw_interface, vnm, i->sw_if_index);
            }
        }
        ...
        if (i->flags & DET44_INTERFACE_FLAG_IS_OUTSIDE) {
            rv = det44_interface_add_del(i->sw_if_index, i->flags, 1);
            if (rv) {
                det44_log_err("outside interface %U del failed", unformat_vnet_sw_interface, vnm, i->sw_if_index);
            }
        }
    }
    vec_free(interfaces);
...
static void det44_update_outside_fib(ip4_main_t *im, uword opaque, u32 sw_if_index, u32 new_fib_index, u32 old_fib_index)
{
    ...
    if (new_fib_index == old_fib_index) return;

    if (!vec_len(dm->outside_fibs)) return;

    pool_foreach (i, dm->interfaces) {
        if (i->sw_if_index == sw_if_index) {
            if (!(det44_interface_is_outside(i))) return;
            match = 1;
        }
    }
    ...
    vec_foreach (outside_fib, dm->outside_fibs) {
        if (outside_fib->fib_index == old_fib_index) {
            outside_fib->refcount--;
            if (!outside_fib->refcount) vec_del1(dm->outside_fibs, outside_fib - dm->outside_fibs);
            break;
        }
    }
...
static clib_error_t *det44_init(vlib_main_t *vm)
{
    ...
    dm->fib_src_hi  = fib_source_allocate("det44-hi", FIB_SOURCE_PRIORITY_HI, FIB_SOURCE_BH_SIMPLE);
    dm->fib_src_low = fib_source_allocate("det44-low", FIB_SOURCE_PRIORITY_LOW, FIB_SOURCE_BH_SIMPLE);
...
u8 *format_det_map_ses(u8 *s, va_list *args)
{
    ...
    in_addr.as_u32  = clib_host_to_net_u32(clib_net_to_host_u32(det_map->in_addr.as_u32) + user_index);
    in_offset       = clib_net_to_host_u32(in_addr.as_u32) - clib_net_to_host_u32(det_map->in_addr.as_u32);
    out_offset      = in_offset / det_map->sharing_ratio;
    out_addr.as_u32 = clib_host_to_net_u32(clib_net_to_host_u32(det_map->out_addr.as_u32) + out_offset);
    s               = format(s,
