...
static void vl_api_abf_plugin_get_version_t_handler(vl_api_abf_plugin_get_version_t *mp)
{
    ...
    rmp->_vl_msg_id = ntohs(VL_API_ABF_PLUGIN_GET_VERSION_REPLY + abf_base_msg_id);
...
static void vl_api_abf_policy_add_del_t_handler(vl_api_abf_policy_add_del_t *mp)
{
    ...
    if (mp->is_add) {
        rv = abf_policy_update(ntohl(mp->policy.policy_id), ntohl(mp->policy.acl_index), paths);
    }
    else {
...
static void vl_api_abf_itf_attach_add_del_t_handler(vl_api_abf_itf_attach_add_del_t *mp)
{
    ...
    fib_protocol_t                         fproto = (mp->attach.is_ipv6 ? FIB_PROTOCOL_IP6 : FIB_PROTOCOL_IP4);
    int                                    rv     = 0;

    if (mp->is_add) {
        rv = abf_itf_attach(fproto, ntohl(mp->attach.policy_id), ntohl(mp->attach.priority), ntohl(mp->attach.sw_if_index));
    }
    else {
        rv = abf_itf_detach(fproto, ntohl(mp->attach.policy_id), ntohl(mp->attach.sw_if_index));
    }
