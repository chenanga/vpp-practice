...
static abf_itf_attach_t *abf_itf_attach_db_find(u32 abf_index, u32 sw_if_index)
{
    ...
    if (NULL != p) return (pool_elt_at_index(abf_itf_attach_pool, p[0]));
...
static void abf_itf_attach_stack(abf_itf_attach_t *aia)
{
    ...
    fib_path_list_contribute_forwarding(ap->ap_pl,
                                        (FIB_PROTOCOL_IP4 == aia->aia_proto ? FIB_FORW_CHAIN_TYPE_UNICAST_IP4 : FIB_FORW_CHAIN_TYPE_UNICAST_IP6),
                                        FIB_PATH_LIST_FWD_FLAG_COLLAPSE,
                                        &via_dpo);

    dpo_stack_from_node((FIB_PROTOCOL_IP4 == aia->aia_proto ? abf_ip4_node.index : abf_ip6_node.index), &aia->aia_dpo, &via_dpo);
    dpo_reset(&via_dpo);
}
...
void abf_setup_acl_lc(fib_protocol_t fproto, u32 sw_if_index)
{
    ...
    if (~0 == abf_alctx_per_itf[fproto][sw_if_index]) return;

    vec_foreach (aiai, abf_per_itf[fproto][sw_if_index]) {
        aia = abf_itf_attach_get(*aiai);
        vec_add1(acl_vec, aia->aia_acl);
    }
    acl_plugin.set_acl_vec_for_context(abf_alctx_per_itf[fproto][sw_if_index], acl_vec);
    vec_free(acl_vec);
}
...
int abf_itf_attach(fib_protocol_t fproto, u32 policy_id, u32 priority, u32 sw_if_index)
{
    ...
    if (NULL != aia) return (VNET_API_ERROR_ENTRY_ALREADY_EXISTS);
    ...
    if (1 == vec_len(abf_per_itf[fproto][sw_if_index])) {
         ...
         */
        vnet_feature_enable_disable((FIB_PROTOCOL_IP4 == fproto ? "ip4-unicast" : "ip6-unicast"),
                                    (FIB_PROTOCOL_IP4 == fproto ? "abf-input-ip4" : "abf-input-ip6"),
                                    sw_if_index,
                                    1,
                                    NULL,
                                    0);
        ...
        abf_alctx_per_itf[fproto][sw_if_index] = acl_plugin.get_lookup_context_index(abf_acl_user_id, sw_if_index, 0);
    }
    else {
        vec_sort_with_function(abf_per_itf[fproto][sw_if_index], abf_cmp_attach_for_sort);
    }
     ...
     */
    aia->aia_sibling = fib_node_child_add(abf_policy_fib_node_type, api, abf_itf_attach_fib_node_type, aiai);
...
int abf_itf_detach(fib_protocol_t fproto, u32 policy_id, u32 sw_if_index)
{
    ...
    if (NULL == aia) return (VNET_API_ERROR_NO_SUCH_ENTRY);
    ...
    index = vec_search(abf_per_itf[fproto][sw_if_index], aia - abf_itf_attach_pool);
    ...
    if (0 == vec_len(abf_per_itf[fproto][sw_if_index])) {
        ...
        acl_plugin.put_lookup_context_index(abf_alctx_per_itf[fproto][sw_if_index]);
     ...
     */
    fib_node_child_remove(abf_policy_fib_node_type, aia->aia_abf, aia->aia_sibling);
...
static u8 *format_abf_intf_attach(u8 *s, va_list *args)
{
    ...
    ap = abf_policy_get(aia->aia_abf);
    s  = format(s, "abf-interface-attach: policy:%d priority:%d", ap->ap_id, aia->aia_prio);
...
static clib_error_t *abf_itf_attach_cmd(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (~0 == abf_policy_find(policy_id)) return (clib_error_return(0, "invalid policy ID:%d", policy_id));
...
static clib_error_t *abf_show_attach_cmd(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    FOR_EACH_FIB_IP_PROTOCOL(fproto)
    {
        if (sw_if_index < vec_len(abf_per_itf[fproto])) {
            if (vec_len(abf_per_itf[fproto][sw_if_index])) vlib_cli_output(vm, "%U:", format_fib_protocol, fproto);
...
always_inline uword abf_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, fib_protocol_t fproto)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            /*
               ...
               . . .
             */
            acl_plugin_fill_5tuple_inline(acl_plugin.p_acl_main, lc_index, b0, (FIB_PROTOCOL_IP6 == fproto), 1, 0, &fa_5tuple0);

            if (acl_plugin_match_5tuple_inline(acl_plugin.p_acl_main,
                                               lc_index,
                                               &fa_5tuple0,
    ...
    vlib_node_increment_counter(vm, (fproto = FIB_PROTOCOL_IP6 ? abf_ip4_node.index : abf_ip6_node.index), ABF_ERROR_MATCHED, matches);
    vlib_node_increment_counter(vm, (fproto = FIB_PROTOCOL_IP6 ? abf_ip4_node.index : abf_ip6_node.index), ABF_ERROR_MISSED, misses);
...
VLIB_REGISTER_NODE(abf_ip4_node) = {.function      = abf_input_ip4,
                                    .name          = "abf-input-ip4",
...
VLIB_REGISTER_NODE(abf_ip6_node) = {.function     = abf_input_ip6,
                                    .name         = "abf-input-ip6",
                                    .vector_size  = sizeof(u32),
                                    .format_trace = format_abf_input_trace,
                                    .type         = VLIB_NODE_TYPE_INTERNAL,
...
{
    return ((abf_itf_attach_t *) (((char *) node) - STRUCT_OFFSET_OF(abf_itf_attach_t, aia_node)));
}
...
{
    abf_itf_attach_fib_node_type = fib_node_register_new_type("abf-attach", &abf_itf_attach_vft);
    clib_error_t *acl_init_res   = acl_plugin_exports_init(&acl_plugin);
    if (acl_init_res) return (acl_init_res);

    abf_acl_user_id = acl_plugin.register_user_module("ABF plugin", "sw_if_index", NULL);
