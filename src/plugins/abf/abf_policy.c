...
static abf_policy_t *abf_policy_find_i(u32 policy_id)
{
    ...
    if (INDEX_INVALID != api) return (abf_policy_get(api));
...
int abf_policy_update(u32 policy_id, u32 acl_index, const fib_route_path_t *rpaths)
{
    ...
    if (INDEX_INVALID == api) {
        ...
        ap->ap_acl = acl_index;
        ap->ap_id  = policy_id;
        ap->ap_pl  = fib_path_list_create((FIB_PATH_LIST_FLAG_SHARED | FIB_PATH_LIST_FLAG_NO_URPF), rpaths);

        /*
         * become a child of the path list so we get poked when
         * the forwarding changes.
         */
        ap->ap_sibling = fib_path_list_child_add(ap->ap_pl, abf_policy_fib_node_type, api);
        ...
        if (FIB_NODE_INDEX_INVALID != old_pl) {
            ap->ap_pl = fib_path_list_copy_and_path_add(old_pl, (FIB_PATH_LIST_FLAG_SHARED | FIB_PATH_LIST_FLAG_NO_URPF), rpaths);
...
int abf_policy_delete(u32 policy_id, const fib_route_path_t *rpaths)
{
    ...
    ap->ap_pl = fib_path_list_copy_and_path_remove(ap->ap_pl, (FIB_PATH_LIST_FLAG_SHARED | FIB_PATH_LIST_FLAG_NO_URPF), rpaths);
...
static clib_error_t *abf_policy_cmd(vlib_main_t *vm, unformat_input_t *main_input, vlib_cli_command_t *cmd)
{
    ...
    if (!is_del) {
        ...
        if (rv == VNET_API_ERROR_INVALID_VALUE) {
            vlib_cli_output(vm, "ACL index must match existing ACL index in policy");
            goto out;
        }
    }
    else {
...
static u8 *format_abf(u8 *s, va_list *args)
{
    ...
    s = format(s, "abf:[%d]: policy:%d acl:%d", ap - abf_policy_pool, ap->ap_id, ap->ap_acl);
    s = format(s, "\n ");
    if (FIB_NODE_INDEX_INVALID == ap->ap_pl) {
...
{
    return ((abf_policy_t *) (((char *) node) - STRUCT_OFFSET_OF(abf_policy_t, ap_node)));
}
...
{
    abf_policy_fib_node_type = fib_node_register_new_type("abf-policy", &abf_policy_vft);
