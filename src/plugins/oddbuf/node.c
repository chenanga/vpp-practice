...
always_inline uword oddbuf_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, int is_ip4, int is_trace)
{
    ...
    while (n_left_from > 0) {
        ...
        if (om->first_chunk_offset) {
            memmove(b0->data + b0->current_data + om->first_chunk_offset, b0->data + b0->current_data, b0->current_length);
        ...
        if (1) {
            ...
            src                    = b0->data + b0->current_data + b0->current_length - om->n_to_copy;
            ...
            if (vlib_buffer_length_in_chain(vm, b0) != save_current_length) clib_warning("OOPS, length incorrect after chunk split...");
        }
