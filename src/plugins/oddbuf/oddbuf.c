...
int oddbuf_enable_disable(oddbuf_main_t *omp, u32 sw_if_index, int enable_disable)
{
    ...
    /* Utterly wrong? */
    if (pool_is_free_index(omp->vnet_main->interface_main.sw_interfaces, sw_if_index)) return VNET_API_ERROR_INVALID_SW_IF_INDEX;
    ...
    vnet_feature_enable_disable("device-input", "oddbuf", sw_if_index, enable_disable, 0, 0);
...
static clib_error_t *oddbuf_enable_disable_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "disable"))
            enable_disable = 0;
        else if (unformat(input, "%U", unformat_vnet_sw_interface, omp->vnet_main, &sw_if_index))
            ;
        else
            break;
    }
...
static clib_error_t *oddbuf_config_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "n_to_copy %d", &omp->n_to_copy))
            ...
            ;
        else if (unformat(line_input, "first_offset %d", &omp->first_chunk_offset))
            ;
        else
            break;
    }
...
VLIB_CLI_COMMAND(oddbuf_config_command, static) = {
    ...
    .short_help = "oddbuf configure n_to_copy <nn> offset <nn> first_offset <nn>",
