...
static void vl_api_sr_mpls_policy_add_t_handler(vl_api_sr_mpls_policy_add_t *mp)
{
    ...
    int rv = 0;
    rv     = sr_mpls_policy_add(ntohl(mp->bsid), segments, mp->is_spray, ntohl(mp->weight));
    vec_free(segments);
...
static void vl_api_sr_mpls_policy_mod_t_handler(vl_api_sr_mpls_policy_mod_t *mp)
{
    ...
    int rv = 0;
    rv     = sr_mpls_policy_mod(ntohl(mp->bsid), ntohl(mp->operation), segments, ntohl(mp->sl_index), ntohl(mp->weight));
    vec_free(segments);
...
{
#define _(id, n, crc) vl_msg_api_add_msg_name_crc(am, #n "_" #crc, id + REPLY_MSG_ID_BASE);
...
static clib_error_t *sr_mpls_api_hookup(vlib_main_t *vm)
{
    ...
    REPLY_MSG_ID_BASE = vl_msg_api_get_msg_ids((char *) name, VL_MSG_SR_MPLS_LAST);
    vec_free(name);
