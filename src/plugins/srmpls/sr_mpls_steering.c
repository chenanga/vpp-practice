...
u32 find_or_create_internal_label(ip46_address_t endpoint, u32 color)
{
    ...
    if (!sm->sr_policies_c2e2eclabel_hash.hash) mhash_init(&sm->sr_policies_c2e2eclabel_hash, sizeof(mhash_t), sizeof(u32));
    ...
    if (!color_table) {
        ...
        mhash_set_mem(&sm->sr_policies_c2e2eclabel_hash, &color, (uword *) &color_t, NULL);
        color_table = mhash_get(&sm->sr_policies_c2e2eclabel_hash, &color);
    }

    result_label = mhash_get((mhash_t *) color_table, &endpoint);

    if (result_label) return (u32) *result_label;

    /* Create and set a new internal label */
    u32 *new_internal_label = 0;
    pool_get(sm->ec_labels, new_internal_label);
    *new_internal_label = 0;
    mhash_set((mhash_t *) color_table, &endpoint, (new_internal_label - sm->ec_labels) + SRMPLS_TE_OFFSET, NULL);

    return (new_internal_label - sm->ec_labels) + SRMPLS_TE_OFFSET;
}

always_inline void internal_label_lock_co(ip46_address_t endpoint, u32 color, char co_bits)
{
...
void internal_label_lock(ip46_address_t endpoint, u32 color)
{
    ...
    if (!sm->sr_policies_c2e2eclabel_hash.hash) return;

    color_table = mhash_get(&sm->sr_policies_c2e2eclabel_hash, &color);
    if (!color_table) return;
    ...
    /* Lock it */
    u32 *label_lock = pool_elt_at_index(sm->ec_labels, *result_label - SRMPLS_TE_OFFSET);
    (*label_lock)++;
}

always_inline void internal_label_unlock_co(ip46_address_t endpoint, u32 color, char co_bits)
{
    ip46_address_t zero, any;
    ip46_address_reset(&zero);
    any.as_u64[0] = any.as_u64[1] = (u64) ~0;
    ...
    if (*label_lock == 0) {
        ...
        if (mhash_elts((mhash_t *) color_table) == 0) {
            ...
            if (mhash_elts(&sm->sr_policies_c2e2eclabel_hash) == 0) {
                ...
                fib_table_unlock(sm->fib_table_EC, FIB_PROTOCOL_MPLS, FIB_SOURCE_SR);
...
void compute_sr_te_automated_steering_fib_entry(mpls_sr_steering_policy_t *steer_pl)
{
    ...
    vec_foreach (color_i, steer_pl->color) {
        switch (steer_pl->co_bits) {
            case SR_TE_CO_BITS_10:
                vec_add1(internal_labels, find_or_create_internal_label(steer_pl->next_hop, *color_i));
                vec_add1(internal_labels, find_or_create_internal_label(zero, *color_i));
                vec_add1(internal_labels, find_or_create_internal_label(any, *color_i));
                break;
            case SR_TE_CO_BITS_01:
                vec_add1(internal_labels, find_or_create_internal_label(steer_pl->next_hop, *color_i));
                vec_add1(internal_labels, find_or_create_internal_label(zero, *color_i));
    ...
    if (sm->fib_table_EC == (u32) ~0) {
        sm->fib_table_EC = fib_table_create_and_lock(FIB_PROTOCOL_MPLS, FIB_SOURCE_SR, "SR-MPLS Traffic Engineering (NextHop,Color)");

        fib_table_flush(sm->fib_table_EC, FIB_PROTOCOL_MPLS, FIB_SOURCE_SPECIAL);
    }
    ...
    clib_memcpy(&path.frp_addr, &steer_pl->next_hop, sizeof(steer_pl->next_hop));
    ...
    if (steer_pl->nh_type == SR_STEER_IPV6) {
        ...
        path.frp_fib_index = fib_table_find(FIB_PROTOCOL_IP6, (steer_pl->classify.fib_table != (u32) ~0 ? steer_pl->classify.fib_table : 0));
    }
    else if (steer_pl->nh_type == SR_STEER_IPV4) {
        path.frp_proto     = DPO_PROTO_IP4;
        path.frp_fib_index = fib_table_find(FIB_PROTOCOL_IP4, (steer_pl->classify.fib_table != (u32) ~0 ? steer_pl->classify.fib_table : 0));
    }

    vec_add1(paths, path);
    if (steer_pl->classify.traffic_type == SR_STEER_IPV6)
        fib_table_entry_update(fib_table_find(FIB_PROTOCOL_IP6, (steer_pl->classify.fib_table != (u32) ~0 ? steer_pl->classify.fib_table : 0)),
                               &pfx,
                               FIB_SOURCE_SR,
                               FIB_ENTRY_FLAG_LOOSE_URPF_EXEMPT,
                               paths);
    else if (steer_pl->classify.traffic_type == SR_STEER_IPV4)
        fib_table_entry_update(fib_table_find(FIB_PROTOCOL_IP4, (steer_pl->classify.fib_table != (u32) ~0 ? steer_pl->classify.fib_table : 0)),
                               &pfx,
                               FIB_SOURCE_SR,
                               FIB_ENTRY_FLAG_LOOSE_URPF_EXEMPT,
                               paths);
                                ...
                                mpls_label_t    vpn_label)
{
    ...
    if (traffic_type != SR_STEER_IPV4 && traffic_type != SR_STEER_IPV6) return -1;
     ...
     * color.
     */
    if (!sm->sr_steer_policies_hash.hash) mhash_init(&sm->sr_steer_policies_hash, sizeof(uword), sizeof(sr_mpls_steering_key_t));

    p = mhash_get(&sm->sr_steer_policies_hash, &key);
    if (p) {
        steer_pl = pool_elt_at_index(sm->steer_policies, p[0]);
        if (steer_pl->bsid != (u32) ~0) return -1;  // Means we are rewritting the steering. Not allowed.

        /* Means we are adding a color. Check that NH match. */
        if (ip46_address_cmp(&steer_pl->next_hop, next_hop)) return -2;
        if (vec_search(steer_pl->color, color) != ~0) return -3;
        if (steer_pl->co_bits != co_bits) return -4;     /* CO colors should be the same */
        if (steer_pl->vpn_label != vpn_label) return -5; /* VPN label should be the same */
    ...
    mhash_set(&sm->sr_steer_policies_hash, &key, steer_pl - sm->steer_policies, NULL);

    /* Local steering */
    if (bsid != (u32) ~0) {
        if (!sm->sr_policies_index_hash) sm->sr_policies_index_hash = hash_create(0, sizeof(mpls_label_t));
        steer_pl->bsid = bsid;
        ...
        if (traffic_type == SR_STEER_IPV6) {
            ...
            vec_add1(paths, path);
            fib_table_entry_path_add2(fib_table_find(FIB_PROTOCOL_IP6, (table_id != (u32) ~0 ? table_id : 0)),
                                      &pfx,
                                      FIB_SOURCE_SR,
                                      FIB_ENTRY_FLAG_LOOSE_URPF_EXEMPT,
                                      paths);
            ...
            path.frp_preference = 0;
            vec_add1(paths, path);
            fib_table_entry_path_add2(fib_table_find(FIB_PROTOCOL_IP4, (table_id != (u32) ~0 ? table_id : 0)),
                                      &pfx,
                                      FIB_SOURCE_SR,
                                      FIB_ENTRY_FLAG_LOOSE_URPF_EXEMPT,
                                      paths);
...
int sr_mpls_steering_policy_del(ip46_address_t *prefix, u32 mask_width, u8 traffic_type, u32 table_id, u32 color)
{
    ...
    if (steer_pl->bsid == (u32) ~0) {
        ...
        if (vec_len(steer_pl->color)) {
            ...
            internal_label_unlock_co(steer_pl->next_hop, color, steer_pl->co_bits);
            return 0;
        }
        else {
            ...
            if (steer_pl->classify.traffic_type == SR_STEER_IPV6) {
                ...
                pfx.fp_len      = steer_pl->classify.mask_width;
                pfx.fp_addr.ip6 = steer_pl->classify.prefix.ip6;
                fib_table_entry_delete(fib_table_find(FIB_PROTOCOL_IP6, steer_pl->classify.fib_table), &pfx, FIB_SOURCE_SR);
            }
            else if (steer_pl->classify.traffic_type == SR_STEER_IPV4) {
                pfx.fp_proto    = FIB_PROTOCOL_IP4;
                pfx.fp_len      = steer_pl->classify.mask_width;
                pfx.fp_addr.ip4 = steer_pl->classify.prefix.ip4;
                fib_table_entry_delete(fib_table_find(FIB_PROTOCOL_IP4, steer_pl->classify.fib_table), &pfx, FIB_SOURCE_SR);
            }
            /* Remove all the locks for this ones... */
            internal_label_unlock_co(steer_pl->next_hop, color, steer_pl->co_bits);
        }
...
static clib_error_t *sr_mpls_steer_policy_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "del"))
            is_del = 1;
        else if (!traffic_type && unformat(input, "l3 %U/%d", unformat_ip6_address, &prefix.ip6, &dst_mask_width))
            traffic_type = SR_STEER_IPV6;
        else if (!traffic_type && unformat(input, "l3 %U/%d", unformat_ip4_address, &prefix.ip4, &dst_mask_width))
            traffic_type = SR_STEER_IPV4;
        else if (!sr_policy_set && unformat(input, "via sr policy bsid %U", unformat_mpls_unicast_label, &bsid))
            sr_policy_set = 1;
        else if (!sr_policy_set && unformat(input, "via next-hop %U color %d co %d", unformat_ip4_address, &nh.ip4, &color, &co_bits)) {
            sr_policy_set = 1;
            nh_type       = SR_STEER_IPV4;
        }
        else if (!sr_policy_set && unformat(input, "via next-hop %U color %d co %d", unformat_ip6_address, &nh.ip6, &color, &co_bits)) {
            sr_policy_set = 1;
            nh_type       = SR_STEER_IPV6;
        }
        else if (fib_table == (u32) ~0 && unformat(input, "fib-table %d", &fib_table))
            ;
        else if (unformat(input, "vpn-label %U", unformat_mpls_unicast_label, &vpn_label))
            ;
        else
            break;
    }

    if (!traffic_type) return clib_error_return(0, "No L3 traffic specified");
    if (!sr_policy_set) return clib_error_return(0, "No SR policy specified");
    ...
    if (nh_type) bsid = (u32) ~0;

    if (is_del)
        rv = sr_mpls_steering_policy_del(&prefix, dst_mask_width, traffic_type, fib_table, color);

    else
        rv = sr_mpls_steering_policy_add(bsid, fib_table, &prefix, dst_mask_width, traffic_type, &nh, nh_type, color, co_bits, vpn_label);
...
static clib_error_t *show_sr_mpls_steering_policies_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    for (i = 0; i < vec_len(steer_policies); i++) {
        ...
        if (steer_pl->classify.traffic_type == SR_STEER_IPV4) {
            vlib_cli_output(vm, "Prefix: %U/%d via:", format_ip4_address, &steer_pl->classify.prefix.ip4, steer_pl->classify.mask_width);
        }
        else if (steer_pl->classify.traffic_type == SR_STEER_IPV6) {
            vlib_cli_output(vm, "Prefix: %U/%d via:", format_ip6_address, &steer_pl->classify.prefix.ip6, steer_pl->classify.mask_width);
        }

        if (steer_pl->bsid != (u32) ~0) {
            vlib_cli_output(vm, "· BSID %U", format_mpls_unicast_label, steer_pl->bsid);
        }
        else {
            if (steer_pl->nh_type == SR_STEER_IPV4) {
                vlib_cli_output(vm, "· Next-hop %U", format_ip4_address, &steer_pl->next_hop.ip4);
            }
            else if (steer_pl->nh_type == SR_STEER_IPV6) {
                vlib_cli_output(vm, "· Next-hop %U", format_ip6_address, &steer_pl->next_hop.ip6);
            }
