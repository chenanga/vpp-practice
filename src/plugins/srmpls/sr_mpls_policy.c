 ...
 */
static inline mpls_sr_sl_t *create_sl(mpls_sr_policy_t *sr_policy, mpls_label_t *sl, u32 weight)
{
    ...
    FOR_EACH_MPLS_EOS_BIT(eos)
    {
        ...
        fib_table_entry_path_add2(0,
                                  &pfx,
                                  FIB_SOURCE_SR,
                                  (sr_policy->type == SR_POLICY_TYPE_DEFAULT ? FIB_ENTRY_FLAG_NONE : FIB_ENTRY_FLAG_MULTICAST),
                                  paths);
        vec_free(paths);
    }
...
int sr_mpls_policy_add(mpls_label_t bsid, mpls_label_t *segments, u8 behavior, u32 weight)
{
    ...
    if (~0 == fib_table_find(FIB_PROTOCOL_MPLS, MPLS_FIB_DEFAULT_TABLE_ID)) return (VNET_API_ERROR_NO_SUCH_TABLE);
    ...
    if (1 == pool_elts(sm->sr_policies)) fib_table_find_or_create_and_lock(FIB_PROTOCOL_MPLS, MPLS_FIB_DEFAULT_TABLE_ID, FIB_SOURCE_SR);
    sr_policy->bsid          = bsid;
...
int sr_mpls_policy_del(mpls_label_t bsid)
{
    ...
    vec_foreach (sl_index, sr_policy->segments_lists) {
        ...
        vec_add(path.frp_label_stack, segment_list + 1, vec_len(segment_list) - 1);
    ...
    if (0 == pool_elts(sm->sr_policies)) fib_table_unlock(MPLS_FIB_DEFAULT_TABLE_ID, FIB_PROTOCOL_MPLS, FIB_SOURCE_SR);
...
int sr_mpls_policy_mod(mpls_label_t bsid, u8 operation, mpls_label_t *segments, u32 sl_index, u32 weight)
{
    ...
    else if (operation == 2) { /* Delete SR List from an existing SR
                                * policy */
    ...
    else if (operation == 3) { /* Modify the weight of an existing
                                * SR List */
...
static clib_error_t *sr_mpls_policy_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (is_add) {
        ...
        rv = sr_mpls_policy_add(bsid, segments, (is_spray ? SR_POLICY_TYPE_SPRAY : SR_POLICY_TYPE_DEFAULT), weight);
        vec_free(segments);
    }
    else if (is_del)
...
static clib_error_t *show_sr_mpls_policies_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vec_foreach_index (i, vec_policies) {
        ...
        vlib_cli_output(vm, "[%u].-\tBSID: %U", (u32) (sr_policy - sm->sr_policies), format_mpls_unicast_label, sr_policy->bsid);
        switch (sr_policy->endpoint_type) {
            case SR_STEER_IPV6:
                vlib_cli_output(vm, "\tEndpoint: %U", format_ip6_address, &sr_policy->endpoint.ip6);
                vlib_cli_output(vm, "\tColor: %u", sr_policy->color);
                break;
            case SR_STEER_IPV4:
                vlib_cli_output(vm, "\tEndpoint: %U", format_ip4_address, &sr_policy->endpoint.ip4);
                vlib_cli_output(vm, "\tColor: %u", sr_policy->color);
                break;
            default:
                vlib_cli_output(vm, "\tTE disabled");
        }
        vlib_cli_output(vm, "\tType: %s", (sr_policy->type == SR_POLICY_TYPE_DEFAULT ? "Default" : "Spray"));
...
int sr_mpls_policy_assign_endpoint_color(mpls_label_t bsid, ip46_address_t *endpoint, u8 endpoint_type, u32 color)
{
    ...
    if (sr_policy->endpoint_type) {
        endpoint_table = mhash_get(&sm->sr_policies_c2e2eclabel_hash, &sr_policy->color);
        if (!endpoint_table) return -2;
        old_value = mhash_get((mhash_t *) endpoint_table, &sr_policy->endpoint);
        ...
        {
            pfx.fp_eos = eos;
            fib_table_entry_path_remove(sm->fib_table_EC, &pfx, FIB_SOURCE_SR, DPO_PROTO_MPLS, NULL, ~0, 0, 1, FIB_ROUTE_PATH_FLAG_NONE);
        }
        ...
        sr_policy->endpoint_type      = 0;
        sr_policy->endpoint.as_u64[0] = sr_policy->endpoint.as_u64[1] = (u64) ~0;
        sr_policy->color                                              = (u32) ~0;
    }
    ...
    if (endpoint_type) {
        ...
        pfx.fp_label = label;
        fib_table_entry_update(sm->fib_table_EC, &pfx, FIB_SOURCE_SR, FIB_ENTRY_FLAG_LOOSE_URPF_EXEMPT, paths);
...
static clib_error_t *cli_sr_mpls_policy_ec_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    int rv;
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (!endpoint_type && unformat(input, "endpoint %U", unformat_ip6_address, &endpoint.ip6))
            endpoint_type = SR_STEER_IPV6;
        else if (!endpoint_type && unformat(input, "endpoint %U", unformat_ip4_address, &endpoint.ip4))
            endpoint_type = SR_STEER_IPV4;
        else if (!color_set && unformat(input, "color %u", &color))
            color_set = 1;
        else if (!bsid_set && unformat(input, "bsid %U", unformat_mpls_unicast_label, &bsid))
            bsid_set = 1;
    ...
    if (!bsid_set) return clib_error_return(0, "No BSID specified");
    if (!endpoint_type && !clear) return clib_error_return(0, "No Endpoint specified");
    if (!color_set && !clear) return clib_error_return(0, "No Color set");
    ...
    rv = sr_mpls_policy_assign_endpoint_color(bsid, &endpoint, endpoint_type, color);

    if (rv) clib_error_return(0, "Error on Endpoint,Color");
