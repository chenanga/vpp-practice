...
static clib_error_t *vmxnet3_delete_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (hw == NULL || vmxnet3_device_class.index != hw->dev_class_index) return clib_error_return(0, "not a vmxnet3 interface");
...
static clib_error_t *vmxnet3_test_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (enable_elog) vd->flags |= VMXNET3_DEVICE_F_ELOG;

    if (disable_elog) vd->flags &= ~VMXNET3_DEVICE_F_ELOG;
...
static void show_vmxnet3(vlib_main_t *vm, u32 *hw_if_indices, u8 show_descr, u8 show_one_table, u32 which, u8 show_one_slot, u32 slot)
{
    ...
    for (u32 tid = 0; tid <= vlib_num_workers(); tid++) {
        vmxnet3_per_thread_data_t *ptd = vec_elt_at_index(vmxm->per_thread_data, tid);
        vlib_cli_output(vm, "  Thread %u: polling queue count %u", tid, ptd->polling_q_count);
    }
    ...
    for (i = 0; i < vec_len(hw_if_indices); i++) {
        ...
        vlib_cli_output(vm, "  PCI Address: %U", format_vlib_pci_addr, &vd->pci_addr);
        vlib_cli_output(vm, "  Mac Address: %U", format_ethernet_address, vd->mac_addr);
        ...
        vec_foreach_index (qid, vd->rxqs) {
            ...
            vlib_cli_output(vm, "    RX completion next index %u", rxq->rx_comp_ring.next);
            vlib_cli_output(vm, "    RX completion generation flag 0x%x", rxq->rx_comp_ring.gen);
            ...
            for (rid = 0; rid < VMXNET3_RX_RING_SIZE; rid++) {
                ...
                if (show_descr) {
                    ...
                    vlib_cli_output(vm, "  %5s  %18s  %10s", "slot", "address", "flags");
                    for (desc_idx = 0; desc_idx < rxq->size; desc_idx++) {
                        rxd = &rxq->rx_desc[rid][desc_idx];
                        vlib_cli_output(vm, "  %5u  0x%016llx  0x%08x", desc_idx, rxd->address, rxd->flags);
                    }
                }
                else if (show_one_table) {
                    if (((which == VMXNET3_SHOW_RX_DESC0) && (rid == 0)) || ((which == VMXNET3_SHOW_RX_DESC1) && (rid == 1))) {
                        vlib_cli_output(vm, "RX descriptors table");
                        vlib_cli_output(vm, "  %5s  %18s  %10s", "slot", "address", "flags");
                        if (show_one_slot) {
                            rxd = &rxq->rx_desc[rid][slot];
                            vlib_cli_output(vm, "  %5u  0x%016llx  0x%08x", slot, rxd->address, rxd->flags);
                        }
                        else
                            for (desc_idx = 0; desc_idx < rxq->size; desc_idx++) {
                                rxd = &rxq->rx_desc[rid][desc_idx];
            ...
            if (show_descr) {
                ...
                vlib_cli_output(vm, "  %5s  %10s  %10s  %10s  %10s", "slot", "index", "rss", "len", "flags");
                for (desc_idx = 0; desc_idx < rxq->size; desc_idx++) {
                    rx_comp = &rxq->rx_comp[desc_idx];
                    vlib_cli_output(vm, "  %5u  0x%08x  %10u  %10u  0x%08x", desc_idx, rx_comp->index, rx_comp->rss, rx_comp->len, rx_comp->flags);
                }
            }
            else if (show_one_table) {
                if (which == VMXNET3_SHOW_RX_COMP) {
                    ...
                    if (show_one_slot) {
                        ...
                        vlib_cli_output(vm, "  %5u  0x%08x  %10u  %10u  0x%08x", slot, rx_comp->index, rx_comp->rss, rx_comp->len, rx_comp->flags);
                    }
                    else
                        for (desc_idx = 0; desc_idx < rxq->size; desc_idx++) {
                            rx_comp = &rxq->rx_comp[desc_idx];
        ...
        vec_foreach_index (qid, vd->txqs) {
            ...
            vlib_cli_output(vm, "    TX completion next index %u", txq->tx_comp_ring.next);
            vlib_cli_output(vm, "    TX completion generation flag 0x%x", txq->tx_comp_ring.gen);
            vlib_cli_output(vm, "    size %u consume %u produce %u", txq->size, txq->tx_ring.consume, txq->tx_ring.produce);
            if (show_descr) {
                vlib_cli_output(vm, "TX descriptors table");
                vlib_cli_output(vm, "  %5s  %18s  %10s  %10s", "slot", "address", "flags0", "flags1");
                for (desc_idx = 0; desc_idx < txq->size; desc_idx++) {
                    txd = &txq->tx_desc[desc_idx];
                    vlib_cli_output(vm, "  %5u  0x%016llx  0x%08x  0x%08x", desc_idx, txd->address, txd->flags[0], txd->flags[1]);
                }

                vlib_cli_output(vm, "TX completion descriptors table");
                vlib_cli_output(vm, "  %5s  %10s  %10s", "slot", "index", "flags");
                for (desc_idx = 0; desc_idx < txq->size; desc_idx++) {
                    tx_comp = &txq->tx_comp[desc_idx];
                    vlib_cli_output(vm, "  %5u  0x%08x  0x%08x", desc_idx, tx_comp->index, tx_comp->flags);
                }
            }
            else if (show_one_table) {
                if (which == VMXNET3_SHOW_TX_DESC) {
                    ...
                    if (show_one_slot) {
                        ...
                        vlib_cli_output(vm, "  %5u  0x%016llx  0x%08x  0x%08x", slot, txd->address, txd->flags[0], txd->flags[1]);
                    }
                    else
                        for (desc_idx = 0; desc_idx < txq->size; desc_idx++) {
                            txd = &txq->tx_desc[desc_idx];
                    ...
                    if (show_one_slot) {
                        ...
                        vlib_cli_output(vm, "  %5u  0x%08x  0x%08x", slot, tx_comp->index, tx_comp->flags);
                    }
                    else
                        for (desc_idx = 0; desc_idx < txq->size; desc_idx++) {
                            tx_comp = &txq->tx_comp[desc_idx];
...
static clib_error_t *show_vmxnet3_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    show_vmxnet3(vm, hw_if_indices, show_descr, show_one_table, which, show_one_slot, slot);
