...
static_always_inline void vmxnet3_handle_offload(vmxnet3_rx_comp *rx_comp, vlib_buffer_t *hb, u16 gso_size)
{
    ...
    if (rx_comp->flags & VMXNET3_RXCF_IP4) {
        ip4_header_t *ip4 = (ip4_header_t *) (hb->data + sizeof(ethernet_header_t));

        vnet_buffer(hb)->l2_hdr_offset = 0;
        vnet_buffer(hb)->l3_hdr_offset = sizeof(ethernet_header_t);
        vnet_buffer(hb)->l4_hdr_offset = sizeof(ethernet_header_t) + ip4_header_bytes(ip4);
        hb->flags |= VNET_BUFFER_F_L2_HDR_OFFSET_VALID | VNET_BUFFER_F_L3_HDR_OFFSET_VALID | VNET_BUFFER_F_L4_HDR_OFFSET_VALID | VNET_BUFFER_F_IS_IP4;
        ...
        if (gso_size) {
            if (rx_comp->flags & VMXNET3_RXCF_TCP) {
                tcp_header_t *tcp = (tcp_header_t *) (hb->data + vnet_buffer(hb)->l4_hdr_offset);
                l4_hdr_sz         = tcp_header_bytes(tcp);
            }
            else if (rx_comp->flags & VMXNET3_RXCF_UDP) {
                udp_header_t *udp = (udp_header_t *) (hb->data + vnet_buffer(hb)->l4_hdr_offset);
        ...
        vnet_buffer(hb)->l2_hdr_offset = 0;
        vnet_buffer(hb)->l3_hdr_offset = sizeof(ethernet_header_t);
        vnet_buffer(hb)->l4_hdr_offset = sizeof(ethernet_header_t) + sizeof(ip6_header_t);
        hb->flags |= VNET_BUFFER_F_L2_HDR_OFFSET_VALID | VNET_BUFFER_F_L3_HDR_OFFSET_VALID | VNET_BUFFER_F_L4_HDR_OFFSET_VALID | VNET_BUFFER_F_IS_IP6;
    ...
    if (oflags) vnet_buffer_offload_flags_set(hb, oflags);
}
...
static_always_inline uword vmxnet3_device_input_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, vmxnet3_device_t *vd, u16 qid)
{
    ...
    while (PREDICT_TRUE((n_rx_packets < VLIB_FRAME_SIZE) && (comp_ring->gen == (rx_comp->flags & VMXNET3_RXCF_GEN)))) {
        vlib_buffer_t *b0;
        u32            bi0;
        ...
        if (PREDICT_TRUE(ring->fill >= 1))
            ...
            vlib_error_count(vm, node->node_index, VMXNET3_INPUT_ERROR_NO_BUFFER, 1);
            if (hb) {
        ...
        if (PREDICT_FALSE((rx_comp->index & VMXNET3_RXCI_EOP) && (rx_comp->len & VMXNET3_RXCL_ERROR))) {
            vlib_buffer_free_one(vm, bi0);
            vlib_error_count(vm, node->node_index, VMXNET3_INPUT_ERROR_RX_PACKET_EOP, 1);
        ...
        if (rx_comp->index & VMXNET3_RXCI_SOP) {
            ...
            /* start segment */
            if (vd->gso_enable && (rx_comp->flags & VMXNET3_RXCF_CT) == VMXNET3_RXCOMP_TYPE_LRO) {
            ...
            if (PREDICT_TRUE(prev_b0 != 0)) {
                ...
                vlib_error_count(vm, node->node_index, VMXNET3_INPUT_ERROR_RX_PACKET_NO_SOP, 1);
                vlib_buffer_free_one(vm, bi0);
                if (hb && vlib_get_buffer_index(vm, hb) != bi0) {
                    vlib_buffer_free_one(vm, vlib_get_buffer_index(vm, hb));
                    hb = 0;
        ...
        else if (prev_b0)  // !sop && !eop
        {
            ...
            vlib_error_count(vm, node->node_index, VMXNET3_INPUT_ERROR_RX_PACKET, 1);
            vlib_buffer_free_one(vm, bi0);
            if (hb && vlib_get_buffer_index(vm, hb) != bi0) {
                vlib_buffer_free_one(vm, vlib_get_buffer_index(vm, hb));
                hb = 0;
        ...
        if (got_packet) {
            ...
            if (PREDICT_FALSE(vnet_device_input_have_features(vd->sw_if_index))) {
                vnet_feature_start_device_input(vd->sw_if_index, &next_index, hb);
                known_next = 1;
            }
            ...
            if (PREDICT_FALSE(known_next))
                ...
                if (!ethernet_frame_is_tagged(ntohs(e->type))) vmxnet3_handle_offload(rx_comp, hb, gso_size);
            }
    ...
    if (PREDICT_FALSE((n_trace = vlib_get_trace_count(vm, node)))) {
        ...
        while (n_trace && n_left) {
            ...
            if (PREDICT_TRUE(vlib_trace_buffer(vm, node, next[0], b, /* follow_chain */ 0))) {
                vmxnet3_input_trace_t *tr = vlib_add_trace(vm, node, b, sizeof(*tr));
    ...
    if (PREDICT_FALSE(error != 0)) {
        vlib_error_count(vm, node->node_index, VMXNET3_INPUT_ERROR_BUFFER_ALLOC, 1);
    }
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    vec_foreach (pve, pv) {
        ...
        if ((vd->flags & VMXNET3_DEVICE_F_ADMIN_UP) == 0) continue;
