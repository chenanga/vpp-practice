...
static pci_device_id_t vmxnet3_pci_device_ids[] = {
    {.vendor_id = PCI_VENDOR_ID_VMWARE, .device_id = PCI_DEVICE_ID_VMWARE_VMXNET3},
    {0},
};
...
static clib_error_t *vmxnet3_interface_admin_up_down(vnet_main_t *vnm, u32 hw_if_index, u32 flags)
{
    ...
    if (vd->flags & VMXNET3_DEVICE_F_ERROR) return clib_error_return(0, "device is in error state");

    if (is_up) {
        vnet_hw_interface_set_flags(vnm, vd->hw_if_index, VNET_HW_INTERFACE_FLAG_LINK_UP);
...
static clib_error_t *vmxnet3_interface_rx_mode_change(vnet_main_t *vnm, u32 hw_if_index, u32 qid, vnet_hw_if_rx_mode mode)
{
    ...
    if (mode == rxq->mode) return 0;
    if ((mode != VNET_HW_IF_RX_MODE_POLLING) && (mode != VNET_HW_IF_RX_MODE_INTERRUPT))
        return clib_error_return(0, "Rx mode %U not supported", format_vnet_hw_if_rx_mode, mode);
    rxq->mode = mode;
...
static void vmxnet3_set_interface_next_node(vnet_main_t *vnm, u32 hw_if_index, u32 node_index)
{
    ...
    vd->per_interface_next_index = vlib_node_add_next(vlib_get_main(), vmxnet3_input_node.index, node_index);
}
...
static clib_error_t *vmxnet3_provision_driver_shared(vlib_main_t *vm, vmxnet3_device_t *vd)
{
    ...
    vd->driver_shared = vlib_physmem_alloc_aligned_on_numa(vm, sizeof(*vd->driver_shared), 512, vd->numa_node);
    if (vd->driver_shared == 0) return vlib_physmem_last_error(vm);
    ...
    vec_foreach_index (qid, vd->rxqs) {
        ...
        for (rid = 0; rid < VMXNET3_RX_RING_SIZE; rid++) {
            rx->cfg.desc_address[rid] = vmxnet3_dma_addr(vm, vd, rxq->rx_desc[rid]);
    ...
    if (vd->gso_enable) shared->misc.upt_features |= VMXNET3_F_LRO;
    ...
    shared->misc.queue_desc_len        = sizeof(*tx) * vd->num_tx_queues + sizeof(*rx) * vd->num_rx_queues;
    ...
    shared->rx_filter.mode             = VMXNET3_RXMODE_UCAST | VMXNET3_RXMODE_BCAST | VMXNET3_RXMODE_ALL_MULTI | VMXNET3_RXMODE_PROMISC;
...
static inline void vmxnet3_enable_interrupt(vmxnet3_device_t *vd)
{
    int             i;
...
static inline void vmxnet3_disable_interrupt(vmxnet3_device_t *vd)
{
    int             i;
    vmxnet3_shared *shared = vd->driver_shared;
...
static clib_error_t *vmxnet3_rxq_init(vlib_main_t *vm, vmxnet3_device_t *vd, u16 qid, u16 qsz)
{
    ...
    for (rid = 0; rid < VMXNET3_RX_RING_SIZE; rid++) {
        rxq->rx_desc[rid] = vlib_physmem_alloc_aligned_on_numa(vm, qsz * sizeof(*rxq->rx_desc[rid]), 512, vd->numa_node);

        if (rxq->rx_desc[rid] == 0) return vlib_physmem_last_error(vm);

        clib_memset(rxq->rx_desc[rid], 0, qsz * sizeof(*rxq->rx_desc[rid]));
    }
    rxq->rx_comp = vlib_physmem_alloc_aligned_on_numa(vm, qsz * sizeof(*rxq->rx_comp), 512, vd->numa_node);
    if (rxq->rx_comp == 0) return vlib_physmem_last_error(vm);
...
static clib_error_t *vmxnet3_txq_init(vlib_main_t *vm, vmxnet3_device_t *vd, u16 qid, u16 qsz)
{
    ...
    txq->tx_desc = vlib_physmem_alloc_aligned_on_numa(vm, size, 512, vd->numa_node);
    if (txq->tx_desc == 0) return vlib_physmem_last_error(vm);

    clib_memset(txq->tx_desc, 0, size);

    size         = qsz * sizeof(*txq->tx_comp);
    txq->tx_comp = vlib_physmem_alloc_aligned_on_numa(vm, size, 512, vd->numa_node);
    if (txq->tx_comp == 0) return vlib_physmem_last_error(vm);
...
static const u8 vmxnet3_rss_key[VMXNET3_RSS_MAX_KEY_SZ] = {
    0x3b, 0x56, 0xd1, 0x56, 0x13, 0x4a, 0xe7, 0xac, 0xe8, 0x79, 0x09, 0x75, 0xe8, 0x65, 0x79, 0x28, 0x35, 0x12, 0xb9, 0x56,
    0x7c, 0x76, 0x4b, 0x70, 0xd8, 0x56, 0xa3, 0x18, 0x9b, 0x0a, 0xee, 0xf3, 0x96, 0xa6, 0x9f, 0x8f, 0x9e, 0x8c, 0x90, 0xc9,
};
...
static clib_error_t *vmxnet3_rss_init(vlib_main_t *vm, vmxnet3_device_t *vd)
{
    ...
    if (vd->rss == 0) return vlib_physmem_last_error(vm);

    clib_memset(vd->rss, 0, size);
    rss               = vd->rss;
    rss->hash_type    = VMXNET3_RSS_HASH_TYPE_IPV4 | VMXNET3_RSS_HASH_TYPE_TCP_IPV4 | VMXNET3_RSS_HASH_TYPE_IPV6 | VMXNET3_RSS_HASH_TYPE_TCP_IPV6;
...
static clib_error_t *vmxnet3_device_init(vlib_main_t *vm, vmxnet3_device_t *vd, vmxnet3_create_if_args_t *args)
{
    ...
    if (vd->version == 0) {
        error = clib_error_return(0, "unsupported hardware version %u", vd->version);
        return error;
    }

    /* cap support version to 3 */
    vmxnet3_reg_write(vd, 1, VMXNET3_REG_VRRS, 1 << (clib_min(3, vd->version) - 1));
    ...
    size = sizeof(vmxnet3_rx_queue) * vd->num_rx_queues + sizeof(vmxnet3_tx_queue) * vd->num_tx_queues;

    vd->queues = vlib_physmem_alloc_aligned_on_numa(vm, size, 512, vd->numa_node);
    if (vd->queues == 0) return vlib_physmem_last_error(vm);
...
static void vmxnet3_rxq_irq_handler(vlib_main_t *vm, vlib_pci_dev_handle_t h, u16 line)
{
    ...
    if (vec_len(vd->rxqs) > qid && (rxq->mode != VNET_HW_IF_RX_MODE_POLLING)) {
        vmxnet3_per_thread_data_t *ptd = vec_elt_at_index(vmxm->per_thread_data, rxq->thread_index);
        if (ptd->polling_q_count == 0) vnet_hw_if_rx_queue_set_int_pending(vnm, rxq->queue_index);
    }
}
...
static void vmxnet3_event_irq_handler(vlib_main_t *vm, vlib_pci_dev_handle_t h, u16 line)
{
    ...
    if (ret & 1) {
        vd->flags |= VMXNET3_DEVICE_F_LINK_UP;
        vd->link_speed = ret >> 16;
        vnet_hw_interface_set_link_speed(vnm, vd->hw_if_index, (vd->link_speed == UINT32_MAX) ? UINT32_MAX : vd->link_speed * 1000);
        vnet_hw_interface_set_flags(vnm, vd->hw_if_index, VNET_HW_INTERFACE_FLAG_LINK_UP);
    }
    else {
        vd->flags &= ~VMXNET3_DEVICE_F_LINK_UP;
...
static u8 vmxnet3_tx_queue_num_valid(u16 num)
{
    ...
    if ((num > VMXNET3_TXQ_MAX) || (num > tm->n_vlib_mains)) return 0;
    return 1;
}

static u8 vmxnet3_rx_queue_num_valid(u16 num)
{
    if (num > VMXNET3_RXQ_MAX) return 0;
    return 1;
}
...
void vmxnet3_create_if(vlib_main_t *vm, vmxnet3_create_if_args_t *args)
{
    ...
    if (args->txq_num == 0) args->txq_num = 1;
    if (args->rxq_num == 0) args->rxq_num = 1;
    if (!vmxnet3_rx_queue_num_valid(args->rxq_num)) {
        args->rv    = VNET_API_ERROR_INVALID_VALUE;
        args->error = clib_error_return(error, "number of rx queues must be <= %u", VMXNET3_RXQ_MAX);
        vlib_log(VLIB_LOG_LEVEL_ERR,
                 vmxm->log_default,
                 "%U: %s",
                 format_vlib_pci_addr,
                 &args->addr,
                 "number of rx queues must be <= %u",
                 VMXNET3_RXQ_MAX);
        return;
    }
    ...
    if (!vmxnet3_tx_queue_num_valid(args->txq_num)) {
        args->rv    = VNET_API_ERROR_INVALID_VALUE;
        args->error = clib_error_return(error,
    ...
    if (args->rxq_size == 0) args->rxq_size = VMXNET3_NUM_RX_DESC;
    if (args->txq_size == 0) args->txq_size = VMXNET3_NUM_TX_DESC;

    if (!vmxnet3_queue_size_valid(args->rxq_size) || !vmxnet3_queue_size_valid(args->txq_size)) {
        args->rv    = VNET_API_ERROR_INVALID_VALUE;
        args->error = clib_error_return(error,
    ...
    pool_foreach (vd, vmxm->devices) {
        if (vd->pci_addr.as_u32 == args->addr.as_u32) {
            ...
            vlib_log(VLIB_LOG_LEVEL_ERR, vmxm->log_default, "%U: %s", format_vlib_pci_addr, &args->addr, "pci address in use");
            return;
        }
    }

    if (args->bind) {
        error = vlib_pci_bind_to_uio(vm, &args->addr, (char *) "auto", VMXNET3_BIND_FORCE == args->bind);
        if (error) {
            args->rv    = VNET_API_ERROR_INVALID_INTERFACE;
            args->error = clib_error_return(error, "%U: %s", format_vlib_pci_addr, &args->addr, "error encountered on binding pci device");
            vlib_log(VLIB_LOG_LEVEL_ERR, vmxm->log_default, "%U: %s", format_vlib_pci_addr, &args->addr, "error encountered on binding pci devicee");
            return;
        }
    }

    if ((error = vlib_pci_device_open(vm, &args->addr, vmxnet3_pci_device_ids, &h))) {
        args->rv    = VNET_API_ERROR_INVALID_INTERFACE;
        args->error = clib_error_return(error, "%U: %s", format_vlib_pci_addr, &args->addr, "error encountered on pci device open");
        vlib_log(VLIB_LOG_LEVEL_ERR, vmxm->log_default, "%U: %s", format_vlib_pci_addr, &args->addr, "error encountered on pci device open");
        return;
    }
    ...
    if (args->enable_elog) vd->flags |= VMXNET3_DEVICE_F_ELOG;
    ...
    if (num_intr < vd->num_rx_queues + 1) {
        vmxnet3_log_error(vd, "No sufficient interrupt lines (%u) for rx queues", num_intr);
        error = clib_error_return(0, "No sufficient interrupt lines (%u) for rx queues", num_intr);
        goto error;
    }
    if ((error = vlib_pci_register_msix_handler(vm, h, 0, vd->num_rx_queues, &vmxnet3_rxq_irq_handler))) {
        vmxnet3_log_error(vd, "error encountered on pci register msix handler 0");
        goto error;
    }

    if ((error = vlib_pci_register_msix_handler(vm, h, vd->num_rx_queues, 1, &vmxnet3_event_irq_handler))) {
        vmxnet3_log_error(vd, "error encountered on pci register msix handler 1");
        goto error;
    }
    ...
    vec_foreach_index (qid, vd->rxqs) {
        ...
        qi = vnet_hw_if_register_rx_queue(vnm, vd->hw_if_index, qid, VNET_HW_IF_RXQ_THREAD_ANY);
        fi = vlib_pci_get_msix_file_index(vm, vd->pci_dev_handle, qid);
        vnet_hw_if_set_rx_queue_file_index(vnm, qi, fi);
        rxq->queue_index  = qi;
        rxq->thread_index = vnet_hw_if_get_rx_queue_thread_index(vnm, rxq->queue_index);
        if (rxq->mode == VNET_HW_IF_RX_MODE_POLLING) {
            ptd = vec_elt_at_index(vmxm->per_thread_data, rxq->thread_index);
            ptd->polling_q_count++;
        }
        rxq->buffer_pool_index = vnet_hw_if_get_rx_queue_numa_node(vnm, rxq->queue_index);
    ...
    vec_foreach_index (qid, vd->txqs) {
        vmxnet3_txq_t *txq = vec_elt_at_index(vd->txqs, qid);
        txq->queue_index   = vnet_hw_if_register_tx_queue(vnm, vd->hw_if_index, qid);
    }
...
void vmxnet3_delete_if(vlib_main_t *vm, vmxnet3_device_t *vd)
{
    ...
    vec_foreach_index (i, vd->rxqs) {
        ...
        for (rid = 0; rid < VMXNET3_RX_RING_SIZE; rid++) {
            ...
            vlib_buffer_free_from_ring(vm, ring->bufs, desc_idx, rxq->size, ring->fill);
            vec_free(ring->bufs);
