...
#define vmxnet3_log_debug(dev, f, ...) \
    vlib_log(VLIB_LOG_LEVEL_DEBUG, vmxnet3_main.log_default, "%U: " f, format_vlib_pci_addr, &dev->pci_addr, ##__VA_ARGS__)

#define vmxnet3_log_error(dev, f, ...) \
    vlib_log(VLIB_LOG_LEVEL_ERR, vmxnet3_main.log_default, "%U: " f, format_vlib_pci_addr, &dev->pci_addr, ##__VA_ARGS__)
...
{
    return (vd->flags & VMXNET3_DEVICE_F_IOVA) ? pointer_to_uword(p) : vlib_physmem_get_pa(vm, p);
}
...
static_always_inline clib_error_t *vmxnet3_rxq_refill_ring0(vlib_main_t *vm, vmxnet3_device_t *vd, vmxnet3_rxq_t *rxq)
{
    ...
    if (PREDICT_TRUE(n_refill <= VMXNET3_INPUT_REFILL_THRESHOLD)) return 0;

    n_alloc = vlib_buffer_alloc_to_ring_from_pool(vm, ring->bufs, ring->produce, rxq->size, n_refill, rxq->buffer_pool_index);
    if (PREDICT_FALSE(n_alloc != n_refill)) {
        if (n_alloc) vlib_buffer_free_from_ring(vm, ring->bufs, ring->produce, rxq->size, n_alloc);
    ...
    if (PREDICT_FALSE(rx->ctrl.update_prod)) vmxnet3_reg_write_inline(vd, 0, VMXNET3_REG_RXPROD, ring->produce);
...
static_always_inline clib_error_t *vmxnet3_rxq_refill_ring1(vlib_main_t *vm, vmxnet3_device_t *vd, vmxnet3_rxq_t *rxq)
{
    ...
    rx = VMXNET3_RX_START(vd);
    if (PREDICT_FALSE(rx->ctrl.update_prod)) vmxnet3_reg_write_inline(vd, 0, VMXNET3_REG_RXPROD2, ring->produce);
