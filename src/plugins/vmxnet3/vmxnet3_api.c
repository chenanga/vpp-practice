...
static void vl_api_vmxnet3_create_t_handler(vl_api_vmxnet3_create_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_VMXNET3_CREATE_REPLY, ({ rmp->sw_if_index = ntohl(args.sw_if_index); }));
}
...
static void send_vmxnet3_details(vl_api_registration_t *reg, vmxnet3_device_t *vd, vnet_sw_interface_t *swif, u8 *interface_name, u32 context)
{
    ...
    if (hwif->hw_address) memcpy(mp->hw_addr, hwif->hw_address, ARRAY_LEN(mp->hw_addr));
...
static void vl_api_sw_vmxnet3_interface_dump_t_handler(vl_api_sw_vmxnet3_interface_dump_t *mp)
{
    ...
    if ((filter_sw_if_index != ~0) && (vnet_sw_interface_is_api_valid(vnm, filter_sw_if_index) == 0)) goto bad_sw_if_index;

    pool_foreach (vd, vmxm->devices) {
        if ((filter_sw_if_index == ~0) || (vd->sw_if_index == filter_sw_if_index)) {
            swif    = vnet_get_sw_interface(vnm, vd->sw_if_index);
            if_name = format(if_name, "%U%c", format_vnet_sw_interface_name, vnm, swif, 0);
            send_vmxnet3_details(reg, vd, swif, if_name, mp->context);
            vec_set_len(if_name, 0);
