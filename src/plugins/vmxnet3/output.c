...
static_always_inline u16 vmxnet3_tx_ring_space_left(vmxnet3_txq_t *txq)
{
    ...
    /* Wrapped? */
    if (txq->tx_ring.produce >= txq->tx_ring.consume) count += txq->size;
    return count;
}
...
(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    if (PREDICT_FALSE(!(vd->flags & VMXNET3_DEVICE_F_LINK_UP))) {
        ...
        vlib_error_count(vm, node->node_index, VMXNET3_TX_ERROR_LINK_DOWN, n_left);
        return (0);
    }
    ...
    while (PREDICT_TRUE(n_left)) {
        ...
        if (PREDICT_FALSE(space_left < space_needed)) {
            ...
            if (PREDICT_FALSE(space_left < space_needed)) {
                vlib_buffer_free_one(vm, bi0);
                vlib_error_count(vm, node->node_index, VMXNET3_TX_ERROR_NO_FREE_SLOTS, 1);
                buffers++;
                n_left--;
                /*
        ...
        for (i = 0; i < space_needed; i++) {
            ...
            if (PREDICT_FALSE(b0->flags & VNET_BUFFER_F_GSO)) {
                ...
                if (b0->flags & VNET_BUFFER_F_IS_IP6)
                    hdr_len = sizeof(ethernet_header_t) + sizeof(ip6_header_t) + l4_hdr_sz;
                else
                    hdr_len = sizeof(ethernet_header_t) + sizeof(ip4_header_t) + l4_hdr_sz;
            }
    ...
    if (PREDICT_TRUE(produce != txq->tx_ring.produce)) vmxnet3_reg_write_inline(vd, 0, txq->reg_txprod, txq->tx_ring.produce);
