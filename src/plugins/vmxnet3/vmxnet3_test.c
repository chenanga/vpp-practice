...
/* vmxnet3 create API */
static int api_vmxnet3_create(vat_main_t *vam)
{
...
static void vl_api_vmxnet3_create_reply_t_handler(vl_api_vmxnet3_create_reply_t *mp)
{
...
/* vmxnet3 delete API */
static int api_vmxnet3_delete(vat_main_t *vam)
{
    unformat_input_t        *i = vam->input;
...
static int api_vmxnet3_dump(vat_main_t *vam)
{
    ...
    if (!vxm->ping_id) vxm->ping_id = vl_msg_api_get_msg_index((u8 *) (VL_API_CONTROL_PING_CRC));
...
static int api_sw_vmxnet3_interface_dump(vat_main_t *vam)
{
    vmxnet3_test_main_t                *vxm = &vmxnet3_test_main;
...
static void vl_api_vmxnet3_details_t_handler(vl_api_vmxnet3_details_t *mp)
{
    ...
    for (qid = 0; qid < mp->rx_count; qid++) {
        ...
        fformat(vam->ofp,
                ...
                ntohs(rx_list->rx_qsize),
                ntohs(rx_list->rx_fill[0]),
                ntohs(rx_list->rx_consume[0]),
                ntohs(rx_list->rx_produce[0]),
                ntohs(rx_list->rx_qsize),
                ntohs(rx_list->rx_fill[1]),
    ...
    for (qid = 0; qid < mp->tx_count; qid++) {
        ...
        fformat(vam->ofp,
                ...
                ntohs(tx_list->tx_qsize),
                ntohs(tx_list->tx_consume),
                ntohs(tx_list->tx_produce));
    }
}

static void vl_api_sw_vmxnet3_interface_details_t_handler(vl_api_sw_vmxnet3_interface_details_t *mp)
{
    vat_main_t *vam      = vmxnet3_test_main.vat_main;
    u32         pci_addr = ntohl(mp->pci_addr);
    u16         qid;
