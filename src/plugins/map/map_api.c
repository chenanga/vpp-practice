...
static void vl_api_map_add_domain_t_handler(vl_api_map_add_domain_t *mp)
{
    ...
    rv                              = map_create_domain((ip4_address_t *) &mp->ip4_prefix.address,
                           mp->ip4_prefix.len,
...
static void send_domain_details(u32 map_domain_index, vl_api_registration_t *rp, u32 context)
{
    ...
    map_domain_extra_t *de      = vec_elt_at_index(mm->domain_extras, map_domain_index);
...
static void vl_api_map_domain_dump_t_handler(vl_api_map_domain_dump_t *mp)
{
    ...
    if (pool_elts(mm->domains) == 0) return;
...
static void vl_api_map_domains_get_t_handler(vl_api_map_domains_get_t *mp)
{
    ...
    REPLY_AND_DETAILS_MACRO(VL_API_MAP_DOMAINS_GET_REPLY, mm->domains, ({ send_domain_details(cursor, rp, mp->context); }));
}
...
int map_param_set_icmp(ip4_address_t *icmp_src_address)
{
    ...
    if (icmp_src_address == 0) return -1;
...
static void vl_api_map_param_add_del_pre_resolve_t_handler(vl_api_map_param_add_del_pre_resolve_t *mp)
{
    ...
    map_pre_resolve((ip4_address_t *) &mp->ip4_nh_address, (ip6_address_t *) &mp->ip6_nh_address, !mp->is_add);
...
static void vl_api_map_param_get_t_handler(vl_api_map_param_get_t *mp)
{
    ...
    clib_memcpy(&rmp->icmp_ip4_err_relay_src, &mm->icmp4_src_address, sizeof(rmp->icmp_ip4_err_relay_src));
...
int map_if_enable_disable(bool is_enable, u32 sw_if_index, bool is_translation)
{
    ...
    if (is_translation) {
        if (clib_bitmap_get(mm->bm_trans_enabled_by_sw_if, sw_if_index) == is_enable) return 0;
    }
    else {
        if (clib_bitmap_get(mm->bm_encap_enabled_by_sw_if, sw_if_index) == is_enable) return 0;
    }
    ...
    if (is_translation == false) {
        ...
        vnet_feature_enable_disable("ip4-unicast", "ip4-map", sw_if_index, is_enable ? 1 : 0, 0, 0);
        vnet_feature_enable_disable("ip6-unicast", "ip6-map", sw_if_index, is_enable ? 1 : 0, 0, 0);
        mm->bm_encap_enabled_by_sw_if = clib_bitmap_set(mm->bm_encap_enabled_by_sw_if, sw_if_index, is_enable);
    }
    else {
        ip4_sv_reass_enable_disable_with_refcnt(sw_if_index, is_enable);
        ip6_sv_reass_enable_disable_with_refcnt(sw_if_index, is_enable);
        vnet_feature_enable_disable("ip4-unicast", "ip4-map-t", sw_if_index, is_enable ? 1 : 0, 0, 0);
        vnet_feature_enable_disable("ip6-unicast", "ip6-map-t", sw_if_index, is_enable ? 1 : 0, 0, 0);
        mm->bm_trans_enabled_by_sw_if = clib_bitmap_set(mm->bm_trans_enabled_by_sw_if, sw_if_index, is_enable);
    }
...
static void vl_api_map_if_enable_disable_t_handler(vl_api_map_if_enable_disable_t *mp)
{
    ...
    rv = map_if_enable_disable(mp->is_enable, htonl(mp->sw_if_index), mp->is_translation);
