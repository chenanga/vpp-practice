 ...
 * ip4_map_ttl
 */
static inline void ip4_map_decrement_ttl(ip4_header_t *ip, u8 *error)
{
    i32 ttl = ip->ttl;
...
static u32 ip4_map_fragment(vlib_main_t *vm, u32 bi, u16 mtu, bool df, u32 **buffers, u8 *error)
{
    ...
    if (mm->frag_inner) {
        ...
        vec_foreach (i, *buffers) {
            ...
            ip6->payload_length = clib_host_to_net_u16(p->current_length - sizeof(ip6_header_t));
        }
    }
    else {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_TRUE(error0 == MAP_ERROR_NONE)) {
                if (PREDICT_FALSE(d0->mtu && (clib_net_to_host_u16(ip6h0->payload_length) + sizeof(*ip6h0) > d0->mtu))) {
                    next0 = ip4_map_fragment(vm, pi0, d0->mtu, df0, &buffer0, &error0);
        ...
        exit:
            ...
            while (frag_left0 > 0) {
                while (frag_left0 > 0 && n_left_to_next > 0) {
                    ...
                    vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, i0, next0);
                }
