...
static void lpm_32_add(lpm_t *lpm, void *addr_v, u8 pfxlen, u32 value)
{
    ...
    if (result) /* Entry exists */
        clib_warning("%U/%d already exists in table for domain %d", format_ip4_address, addr, pfxlen, result[0]);
...
static void lpm_128_delete(lpm_t *lpm, void *addr_v, u8 pfxlen)
{
    ...
    if (--lpm->prefix_length_refcount[pfxlen] == 0) {
        lpm->prefix_lengths_bitmap = clib_bitmap_set(lpm->prefix_lengths_bitmap, 128 - pfxlen, 0);
    }
}
...
lpm_t *lpm_table_init(enum lpm_type_e lpm_type)
{
    ...
    switch (lpm_type) {
        ...
        case LPM_TYPE_KEY128:
            ...
            BV(clib_bihash_init)(&(lpm->bihash), "LPM 128", 64 * 1024, 32 << 20);
