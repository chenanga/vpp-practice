...
static void map_free_extras(u32 map_domain_index)
{
    ...
    if (map_domain_index >= vec_len(mm->domain_extras)) return;
                      ...
                      u8            *tag)
{
    ...
    if (ea_bits_len > 0 && ((ip6_prefix_len + ea_bits_len) > 64 || ip6_prefix_len + suffix_len + psid_length > 64)) {
        clib_warning(
    ...
    *map_domain_index = d - mm->domains;
    ...
    if (tag) map_save_extras(*map_domain_index, tag);

    /* MAP longest match lookup table (input feature / FIB) */
    mm->ip4_prefix_tbl->add(mm->ip4_prefix_tbl, &d->ip4_prefix, d->ip4_prefix_len, *map_domain_index);

    /* Really needed? Or always use FIB? */
    mm->ip6_src_prefix_tbl->add(mm->ip6_src_prefix_tbl, &d->ip6_src, d->ip6_src_len, *map_domain_index);

    /* Let's build a table with the MAP rule ip6 prefixes as well [dgeist] */
    mm->ip6_prefix_tbl->add(mm->ip6_prefix_tbl, &d->ip6_prefix, d->ip6_prefix_len, *map_domain_index);
...
int map_delete_domain(u32 map_domain_index)
{
    ...
    mm->ip4_prefix_tbl->delete (mm->ip4_prefix_tbl, &d->ip4_prefix, d->ip4_prefix_len);
    mm->ip6_src_prefix_tbl->delete (mm->ip6_src_prefix_tbl, &d->ip6_src, d->ip6_src_len);
    ...
    /* Deleting rules */
    if (d->rules) clib_mem_free(d->rules);
...
static u8 *format_map_pre_resolve(u8 *s, va_list *ap)
{
    ...
    if (FIB_NODE_INDEX_INVALID != pr->fei) {
        ...
        return (format(s, "%U (%u)", format_ip46_address, &pfx->fp_addr, IP46_TYPE_ANY, pr->dpo.dpoi_index));
    }
    else {
...
static map_main_pre_resolved_t *map_from_fib_node(fib_node_t *node)
{
    ...
    return ((map_main_pre_resolved_t *) (((char *) node) - STRUCT_OFFSET_OF(map_main_pre_resolved_t, node)));
}
...
void map_pre_resolve(ip4_address_t *ip4, ip6_address_t *ip6, bool is_del)
{
    if (ip6 && (ip6->as_u64[0] != 0 || ip6->as_u64[1] != 0)) {
        ...
        if (is_del)
            map_fib_unresolve(&pre_resolved[FIB_PROTOCOL_IP6], FIB_PROTOCOL_IP6, 128, &addr);
        else
            map_fib_resolve(&pre_resolved[FIB_PROTOCOL_IP6], FIB_PROTOCOL_IP6, 128, &addr);
    }
    if (ip4 && (ip4->as_u32 != 0)) {
        ip46_address_t addr = {
            .ip4 = *ip4,
        ...
        if (is_del)
            map_fib_unresolve(&pre_resolved[FIB_PROTOCOL_IP4], FIB_PROTOCOL_IP4, 32, &addr);
        else
            map_fib_resolve(&pre_resolved[FIB_PROTOCOL_IP4], FIB_PROTOCOL_IP4, 32, &addr);
    }
}
#endif
...
static clib_error_t *map_security_check_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!saw_enable) {
        error = clib_error_return(0, "Must specify enable 'enable' or 'disable'");
        goto done;
    }
...
static clib_error_t *map_icmp_unreachables_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (num_m_args != 1) error = clib_error_return(0, "mandatory argument(s) missing");
...
{
    if (flags & MAP_DOMAIN_PREFIX) return "prefix";
    return "";
}
...
static u8 *format_map_domain(u8 *s, va_list *args)
{
    ...
    if (map_domain_index < vec_len(mm->domain_extras)) de = vec_elt_at_index(mm->domain_extras, map_domain_index);

    s = format(s,
               "[%d] tag {%s} ip4-pfx %U/%d ip6-pfx %U/%d ip6-src %U/%d "
               "ea-bits-len %d psid-offset %d psid-len %d mtu %d %s",
               map_domain_index,
               (de && de->tag) ? de->tag : (u8 *) "[no-tag]",
               format_ip4_address,
               &d->ip4_prefix,
               d->ip4_prefix_len,
               format_ip6_address,
    ...
    if (d->rules) {
        ...
        for (i = 0; i < (0x1 << d->psid_length); i++) {
            dst = d->rules[i];
            if (dst.as_u64[0] == 0 && dst.as_u64[1] == 0) continue;
            s = format(s, " rule psid: %d ip6-dst %U\n", i, format_ip6_address, &dst);
        }
    }
    return s;
}
...
static clib_error_t *show_map_domain_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (map_domain_index == ~0) {
        ...
        if (pool_is_free_index(mm->domains, map_domain_index)) {
            error = clib_error_return(0, "MAP domain does not exists %d", map_domain_index);
            goto done;
        }
...
static clib_error_t *show_map_stats_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vlib_cli_output(vm,
                    ...
                    &pre_resolved[FIB_PROTOCOL_IP6],
                    format_map_pre_resolve,
                    &pre_resolved[FIB_PROTOCOL_IP4]);
    ...
    if (mm->tcp_mss) vlib_cli_output(vm, "MAP TCP MSS clamping: %u", mm->tcp_mss);
    ...
    vlib_cli_output(vm, "Encapsulated packets: %lld bytes: %lld\n", total_pkts[MAP_DOMAIN_COUNTER_TX], total_bytes[MAP_DOMAIN_COUNTER_TX]);
    vlib_cli_output(vm, "Decapsulated packets: %lld bytes: %lld\n", total_pkts[MAP_DOMAIN_COUNTER_RX], total_bytes[MAP_DOMAIN_COUNTER_RX]);
...
VLIB_CLI_COMMAND(map_pre_resolve_command, static) = {
    ...
    .short_help =
        " map params pre-resolve {ip4-nh <address>} "
        "| {ip6-nh <address>}",
...
VLIB_CLI_COMMAND(map_add_domain_command, static) = {
    .path = "map add domain",
    .short_help =
        "map add domain [tag <tag>] ip4-pfx <ip4-pfx> "
        "ip6-pfx <ip6-pfx> "
