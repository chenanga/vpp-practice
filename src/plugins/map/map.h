...
always_inline void map_add_trace(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_buffer_t *b, u32 map_domain_index, u16 port)
{
...
static_always_inline u64 map_get_pfx(map_domain_t *d, u32 addr, u16 port)
{
    ...
    if (d->ea_bits_len == 0 && d->rules) return clib_net_to_host_u64(d->rules[psid].as_u64[0]);
...
{
    return clib_host_to_net_u64(map_get_pfx(d, clib_net_to_host_u32(addr), clib_net_to_host_u16(port)));
}
...
static_always_inline u64 map_get_sfx(map_domain_t *d, u32 addr, u16 port)
{
    ...
    /* Shared 1:1 mode. */
    if (d->ea_bits_len == 0 && d->rules) return clib_net_to_host_u64(d->rules[psid].as_u64[1]);
    if (d->ip6_prefix_len == 128) return clib_net_to_host_u64(d->ip6_prefix.as_u64[1]);

    if (d->ip6_src_len == 96) return (clib_net_to_host_u64(d->ip6_prefix.as_u64[1]) | addr);

    /* IPv4 prefix */
    if (d->flags & MAP_DOMAIN_PREFIX) return (u64) (addr & (0xFFFFFFFF << d->suffix_shift)) << 16;
...
{
    return clib_host_to_net_u64(map_get_sfx(d, clib_net_to_host_u32(addr), clib_net_to_host_u16(port)));
}
...
static inline void map_domain_counter_lock(map_main_t *mm)
{
    if (mm->counter_lock)
...
static inline void map_domain_counter_unlock(map_main_t *mm)
{
    if (mm->counter_lock) clib_atomic_release(mm->counter_lock);
}
...
static_always_inline void map_mss_clamping(tcp_header_t *tcp, ip_csum_t *sum, u16 mss_clamping)
{
    ...
    for (; opts_len > 0; opts_len -= opt_len, data += opt_len) {
        ...
        if (kind == TCP_OPTION_EOL)
            ...
            if (opts_len < 2) return;
            opt_len = data[1];

            if (opt_len < 2 || opt_len > opts_len) return;
        }

        if (kind == TCP_OPTION_MSS) {
            mss = *(u16 *) (data + 2);
            if (clib_net_to_host_u16(mss) > mss_clamping) {
                *sum = ip_csum_update(*sum, mss, mss_value_net, ip4_header_t, length);
...
static_always_inline bool ip4_map_ip6_lookup_bypass(vlib_buffer_t *p0, ip4_header_t *ip)
{
    ...
    if (FIB_NODE_INDEX_INVALID != pre_resolved[FIB_PROTOCOL_IP6].fei) {
        vnet_buffer(p0)->ip.adj_index[VLIB_TX] = pre_resolved[FIB_PROTOCOL_IP6].dpo.dpoi_index;
        return (true);
    }
#endif
    return (false);
...
static_always_inline bool ip6_map_ip4_lookup_bypass(vlib_buffer_t *p0, ip4_header_t *ip)
{
    ...
    if (FIB_NODE_INDEX_INVALID != pre_resolved[FIB_PROTOCOL_IP4].fei) {
        vnet_buffer(p0)->ip.adj_index[VLIB_TX] = pre_resolved[FIB_PROTOCOL_IP4].dpo.dpoi_index;
        return (true);
    }
#endif
    return (false);
