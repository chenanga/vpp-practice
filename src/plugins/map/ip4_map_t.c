...
static int ip4_to_ip6_set_icmp_cb(vlib_buffer_t *b, ip4_header_t *ip4, ip6_header_t *ip6, void *arg)
{
    ...
    ip6->dst_address.as_u64[0] = map_get_pfx_net(ctx->d, ip4->dst_address.as_u32, ctx->recv_port);
    ip6->dst_address.as_u64[1] = map_get_sfx_net(ctx->d, ip4->dst_address.as_u32, ctx->recv_port);
...
static int ip4_to_ip6_set_inner_icmp_cb(vlib_buffer_t *b, ip4_header_t *ip4, ip6_header_t *ip6, void *arg)
{
    ...
    ip6->src_address.as_u64[0] = map_get_pfx_net(ctx->d, old_src.as_u32, ctx->recv_port);
    ip6->src_address.as_u64[1] = map_get_sfx_net(ctx->d, old_src.as_u32, ctx->recv_port);
...
static uword ip4_map_t_icmp(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (icmp_to_icmp6(p0, ip4_to_ip6_set_icmp_cb, &ctx0, ip4_to_ip6_set_inner_icmp_cb, &ctx0)) {
                error0 = MAP_ERROR_ICMP;
                goto err0;
            }
            ...
            if (vnet_buffer(p0)->map_t.mtu < p0->current_length) {
                ...
                next0 = ip4_map_ip6_lookup_bypass(p0, NULL) ? IP4_MAPT_ICMP_NEXT_IP6_REWRITE : next0;
            }
        err0:
            if (PREDICT_TRUE(error0 == MAP_ERROR_NONE)) {
                vlib_increment_combined_counter(cm + MAP_DOMAIN_COUNTER_TX, thread_index, vnet_buffer(p0)->map_t.map_domain_index, 1, len0);
            }
            else {
 ...
 */
always_inline int map_ip4_to_ip6_fragmented(vlib_buffer_t *p, ip4_mapt_pseudo_header_t *pheader)
{
    ip4_header_t   *ip4;
    ip6_header_t   *ip6;
    ip6_frag_hdr_t *frag;
...
static uword ip4_map_t_fragmented(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    n_left_from                     = frame->n_vectors;
    next_index                      = node->cached_next_index;
    vlib_node_runtime_t *error_node = vlib_node_get_runtime(vm, ip4_map_t_fragmented_node.index);
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (map_ip4_to_ip6_fragmented(p0, pheader0)) {
                ...
                if (vnet_buffer(p0)->map_t.mtu < p0->current_length) {
                    ...
                    next0 = ip4_map_ip6_lookup_bypass(p0, NULL) ? IP4_MAPT_FRAGMENTED_NEXT_IP6_REWRITE : next0;
                }
            }
 ...
 */
always_inline int map_ip4_to_ip6_tcp_udp(vlib_buffer_t *p, ip4_mapt_pseudo_header_t *pheader)
{
...
static uword ip4_map_t_tcp_udp(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    n_left_from                     = frame->n_vectors;
    next_index                      = node->cached_next_index;
    vlib_node_runtime_t *error_node = vlib_node_get_runtime(vm, ip4_map_t_tcp_udp_node.index);
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (map_ip4_to_ip6_tcp_udp(p0, pheader0)) {
                ...
                if (vnet_buffer(p0)->map_t.mtu < p0->current_length) {
                    ...
                    next0 = ip4_map_ip6_lookup_bypass(p0, NULL) ? IP4_MAPT_TCP_UDP_NEXT_IP6_REWRITE : next0;
                }
            }
            vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, pi0, next0);
        }
                                             ...
                                             u16              l4_dst_port)
{
    if (PREDICT_FALSE(ip4_get_fragment_offset(ip40))) {
        ...
        *next0 = IP4_MAPT_NEXT_MAPT_ICMP;
        if (d0->ea_bits_len == 0 && d0->rules)
            *dst_port0 = 0;
        else if (((icmp46_header_t *) u8_ptr_add(ip40, sizeof(*ip40)))->type == ICMP4_echo_reply
                 || ((icmp46_header_t *) u8_ptr_add(ip40, sizeof(*ip40)))->type == ICMP4_echo_request)
            *dst_port0 = l4_dst_port;
    }
    else {
...
static uword ip4_map_t(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            d0 = ip4_map_get_domain(&ip40->dst_address, &vnet_buffer(p0)->map_t.map_domain_index, &error0);
            ...
            if (PREDICT_FALSE(df0 && !map_main.frag_ignore_df
                              && ((ip4_len0 + (sizeof(ip6_header_t) - sizeof(ip4_header_t))) > vnet_buffer(p0)->map_t.mtu))) {
                icmp4_error_set_vnet_buffer(p0,
                                            ICMP4_destination_unreachable,
                                            ICMP4_destination_unreachable_fragmentation_needed_and_dont_fragment_set,
                                            vnet_buffer(p0)->map_t.mtu - (sizeof(ip6_header_t) - sizeof(ip4_header_t)));
                p0->error = error_node->errors[MAP_ERROR_DF_SET];
                next0     = IP4_MAPT_NEXT_ICMP_ERROR;
                goto trace;
            }

            ip4_map_t_classify(p0, d0, ip40, ip4_len0, &dst_port0, &error0, &next0, l4_dst_port);

            /* Verify that port is not among the well-known ports */
            if ((d0->psid_length > 0 && d0->psid_offset > 0) && (clib_net_to_host_u16(dst_port0) < (0x1 << (16 - d0->psid_offset)))) {
