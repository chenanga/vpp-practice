...
static int ip6_to_ip4_set_icmp_cb(ip6_header_t *ip6, ip4_header_t *ip4, void *arg)
{
    ...
    ip4->dst_address.as_u32 = ip6_map_t_embedded_address(ctx->d, &ip6->dst_address);
...
static int ip6_to_ip4_set_inner_icmp_cb(ip6_header_t *ip6, ip4_header_t *ip4, void *arg)
{
    ...
    if (ip6->dst_address.as_u64[0] != map_get_pfx_net(ctx->d, inner_ip4_dadr, ctx->sender_port)
        || ip6->dst_address.as_u64[1] != map_get_sfx_net(ctx->d, inner_ip4_dadr, ctx->sender_port))
        return -1;

    ip4->dst_address.as_u32 = inner_ip4_dadr;
    ip4->src_address.as_u32 = ip6_map_t_embedded_address(ctx->d, &ip6->src_address);
...
static uword ip6_map_t_icmp(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            ctx0.d           = d0;
            ctx0.sender_port = 0;
            if (!ip6_get_port(vm, p0, ip60, p0->current_length, NULL, &ctx0.sender_port, NULL, NULL, NULL, NULL)) {
            ...
            if (icmp6_to_icmp(vm, p0, ip6_to_ip4_set_icmp_cb, &ctx0, ip6_to_ip4_set_inner_icmp_cb, &ctx0)) {
                error0 = MAP_ERROR_ICMP;
                goto err0;
            }
            ...
            if (vnet_buffer(p0)->map_t.mtu < p0->current_length) {
                ...
                next0 = ip6_map_ip4_lookup_bypass(p0, NULL) ? IP6_MAPT_ICMP_NEXT_IP4_REWRITE : next0;
            }
        err0:
            if (PREDICT_TRUE(error0 == MAP_ERROR_NONE)) {
                vlib_increment_combined_counter(cm + MAP_DOMAIN_COUNTER_RX, thread_index, vnet_buffer(p0)->map_t.map_domain_index, 1, len0);
            }
            else {
 ...
 */
always_inline int map_ip6_to_ip4_fragmented(vlib_main_t *vm, vlib_buffer_t *p)
{
    ip6_header_t   *ip6;
    ip6_frag_hdr_t *frag;
    ip4_header_t   *ip4;
    ...
    if (ip6_parse(vm, p, ip6, p->current_length, &l4_protocol, &l4_offset, &frag_offset)) return -1;
...
static uword ip6_map_t_fragmented(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    n_left_from                     = frame->n_vectors;
    next_index                      = node->cached_next_index;
    vlib_node_runtime_t *error_node = vlib_node_get_runtime(vm, ip6_map_t_fragmented_node.index);
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (map_ip6_to_ip4_fragmented(vm, p0)) {
                ...
                if (vnet_buffer(p0)->map_t.mtu < p0->current_length) {
                    ...
                    next0 = ip6_map_ip4_lookup_bypass(p0, NULL) ? IP6_MAPT_FRAGMENTED_NEXT_IP4_REWRITE : next0;
                }
            }
 ...
 */
always_inline int map_ip6_to_ip4_tcp_udp(vlib_main_t *vm, vlib_buffer_t *p, bool udp_checksum)
{
...
{
    u32                  n_left_from, *from, next_index, *to_next, n_left_to_next;
    vlib_node_runtime_t *error_node = vlib_node_get_runtime(vm, ip6_map_t_tcp_udp_node.index);
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (map_ip6_to_ip4_tcp_udp(vm, p0, true)) {
                ...
                if (vnet_buffer(p0)->map_t.mtu < p0->current_length) {
                    ...
                    next0 = ip6_map_ip4_lookup_bypass(p0, NULL) ? IP6_MAPT_TCP_UDP_NEXT_IP4_REWRITE : next0;
                }
            }
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            d0 =
                 ...
                 */
                ip6_map_get_domain(&ip60->src_address, &vnet_buffer(p0)->map_t.map_domain_index, &error0);
            ...
            if (PREDICT_FALSE(ip6_parse(vm,
                ...
                error0 = error0 == MAP_ERROR_NONE ? MAP_ERROR_MALFORMED : error0;
            }

            l4_len0 = (u32) clib_net_to_host_u16(ip60->payload_length) + sizeof(*ip60) - vnet_buffer(p0)->map_t.v6.l4_offset;
            frag0   = (ip6_frag_hdr_t *) u8_ptr_add(ip60, vnet_buffer(p0)->map_t.v6.frag_offset);

            if (PREDICT_FALSE(vnet_buffer(p0)->map_t.v6.frag_offset && ip6_frag_hdr_offset(frag0))) {
                ...
                if (((icmp46_header_t *) u8_ptr_add(ip60, vnet_buffer(p0)->map_t.v6.l4_offset))->type == ICMP6_echo_reply
                    || ((icmp46_header_t *) u8_ptr_add(ip60, vnet_buffer(p0)->map_t.v6.l4_offset))->type == ICMP6_echo_request)
                    map_port0 = l4_src_port;
            }
            else {
            ...
            if (PREDICT_FALSE(map_port0 != -1)
                && (ip60->src_address.as_u64[0] != map_get_pfx_net(d0, vnet_buffer(p0)->map_t.v6.saddr, map_port0)
                    || ip60->src_address.as_u64[1] != map_get_sfx_net(d0, vnet_buffer(p0)->map_t.v6.saddr, map_port0))) {
                // Security check when map_port0 is not zero (non-first
                // fragment, UDP or TCP)
                error0 = error0 == MAP_ERROR_NONE ? MAP_ERROR_SEC_CHECK : error0;
            }
            ...
            if (PREDICT_FALSE(error0 == MAP_ERROR_SEC_CHECK && mm->icmp6_enabled)) {
                icmp6_error_set_vnet_buffer(p0, ICMP6_destination_unreachable, ICMP6_destination_unreachable_source_address_failed_policy, 0);
                next0 = IP6_MAPT_NEXT_ICMP;
            }
            else {
        ...
        trace:
            if (PREDICT_FALSE(p0->flags & VLIB_BUFFER_IS_TRACED)) {
                map_add_trace(vm, node, p0, vnet_buffer(p0)->map_t.map_domain_index, map_port0);
            }
        exit:
            vlib_validate_buffer_enqueue_x1(vm, node, next_index, to_next, n_left_to_next, pi0, next0);
        }
