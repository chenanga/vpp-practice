...
static void send_graph_node_details(vlib_node_main_t *nm, vl_api_registration_t *reg, u32 context, vlib_node_t *n, bool want_arcs)
{
    ...
    msg_size = sizeof(*mp);
    if (want_arcs) msg_size += vec_len(n->next_nodes) * sizeof(*n->next_nodes);
    ...
    clib_strncpy((char *) mp->name, (char *) n->name, MIN(sizeof(mp->name) - 1, vec_len(n->name)));
...
static void vl_api_graph_node_get_t_handler(vl_api_graph_node_get_t *mp)
{
    ...
    if (cursor == ~0 && node_index != ~0) {
        if (node_index < vec_len(nm->nodes)) n = vlib_get_node(vm, node_index);
        if (!n) {
            send_graph_node_reply(rp, mp->context, VNET_API_ERROR_NO_SUCH_ENTRY, ~0);
            return;
        }
