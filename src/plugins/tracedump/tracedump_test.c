...
int api_trace_set_filters(vat_main_t *vam)
{
...
int api_trace_capture_packets(vat_main_t *vam)
{
    unformat_input_t               *i = vam->input;
...
static void vl_api_trace_details_t_handler(vl_api_trace_details_t *dmp)
{
    u32 packet_number;
    ...
    fformat(stdout,
            ...
            thread_id,
            position,
            (u32) dmp->more_this_thread,
            (u32) dmp->more_threads,
            (u32) dmp->done);
    fformat(stdout, "Packet %d\n%U\n\n", packet_number, vl_api_format_string, (&dmp->trace_data));
}

static void vl_api_trace_v2_details_t_handler(vl_api_trace_v2_details_t *dmp)
{
    u32 thread_id, position;

    thread_id = clib_net_to_host_u32(dmp->thread_id);
    position  = clib_net_to_host_u32(dmp->position);
    fformat(stdout, "thread %d position %d more %d", thread_id, position, dmp->more);
    fformat(stdout, "Packet %d\n%U\n\n", position, vl_api_format_string, (&dmp->trace_data));
}

static void vl_api_trace_dump_reply_t_handler(vl_api_trace_dump_reply_t *rmp)
{
...
static int api_trace_dump(vat_main_t *vam)
{
    vl_api_trace_dump_t *mp;
    int                  ret;
...
static int api_trace_v2_dump(vat_main_t *vam)
{
...
int api_trace_clear_capture(vat_main_t *vam)
{
...
static int api_trace_clear_cache(vat_main_t *vam)
{
    vl_api_trace_clear_capture_t *mp;
    int                           ret;
...
static int api_trace_set_filter_function(vat_main_t *vam)
{
...
static int api_trace_filter_function_dump(vat_main_t *vam)
{
...
static void vl_api_trace_filter_function_details_t_handler(vl_api_trace_filter_function_details_t *dmp)
{
    fformat(stdout, "name: %U, selected: %u\n\n", vl_api_format_string, &dmp->name, dmp->selected);
}
...
void manual_setup_message_id_table(vat_main_t *vam)
{
