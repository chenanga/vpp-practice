...
uword api_unformat_node_index(unformat_input_t *input, va_list *args)
{
...
static void vl_api_graph_node_get_reply_t_handler(vl_api_graph_node_get_reply_t *mp)
{
...
int api_graph_node_get(vat_main_t *vam)
{
    ...
    if (node_name && node_name[0]) clib_strncpy((char *) mp->name, node_name, sizeof(mp->name) - 1);

    int ret = 0;
    S(mp);

    if (!gtm->ping_id) gtm->ping_id = vl_msg_api_get_msg_index((u8 *) (VL_API_CONTROL_PING_CRC));
...
void vl_api_graph_node_details_t_handler(vl_api_graph_node_details_t *mp)
{
    vat_main_t *vam = &vat_main;
    u32         n_arcs;
    int         i;
...
void vl_api_graph_node_details_t_handler_json(vl_api_graph_node_details_t *mp)
{
...
static clib_error_t *graph_api_hookup_shim(vlib_main_t *vm)
{
