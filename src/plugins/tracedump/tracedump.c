...
static void vl_api_trace_capture_packets_t_handler(vl_api_trace_capture_packets_t *mp)
{
    ...
    if (!vnet_trace_placeholder) vec_validate_aligned(vnet_trace_placeholder, 2048, CLIB_CACHE_LINE_BYTES);
    ...
    if (pre_clear) vlib_trace_stop_and_clear();
...
static void vl_api_trace_dump_t_handler(vl_api_trace_dump_t *mp)
{
    ...
    if (rp->vl_input_queue) {
        ...
        if (chunk < max_records) max_records = chunk;
    }

    /* Need a fresh cache for this client? */
    if (vec_len(client_trace_cache) == 0 && (iterator_thread_id != ~0 || iterator_position != ~0)) {
        ...
        foreach_vlib_main () {
            ...
            if (vec_len(client_trace_cache[i])) vec_sort_with_function(client_trace_cache[i], trace_cmp);
    ...
    for (i = iterator_thread_id; i < vec_len(client_trace_cache); i++) {
        for (j = iterator_position; j < vec_len(client_trace_cache[i]); j++) {
            ...
            s = format(s, "%U", format_vlib_trace, vlib_get_first_main(), th[0]);
            ...
            dmp->done = 0;
            if (i == (vec_len(client_trace_cache) - 1) && j == (vec_len(client_trace_cache[i]) - 1)) {
...
/* API message handler */
static void vl_api_trace_v2_dump_t_handler(vl_api_trace_v2_dump_t *mp)
{
    vl_api_registration_t     *rp;
    ...
    if (rp->vl_input_queue) {
        ...
        if (chunk < max * (last_thread_id - first_thread_id + 1)) max = chunk / (last_thread_id - first_thread_id + 1);
    }
    ...
    for (i = first_thread_id; i <= last_thread_id && i < vec_len(client_trace_cache); i++) {
        // dump a number of 'max' packets per thead
        for (j = first_position; j < vec_len(client_trace_cache[i]) && j < first_position + max; j++) {
            th = &client_trace_cache[i][j];
            ...
            dmp             = vl_msg_api_alloc(sizeof(*dmp) + vec_len(s));
            dmp->_vl_msg_id = htons(VL_API_TRACE_V2_DETAILS + (tdmp->msg_id_base));
            dmp->context    = mp->context;
...
static void vl_api_trace_clear_cache_t_handler(vl_api_trace_clear_cache_t *mp)
{
    vl_api_registration_t            *rp;
    tracedump_main_t                 *tdmp = &tracedump_main;
    vlib_trace_header_t            ***client_trace_cache;
...
static void vl_api_trace_set_filter_function_t_handler(vl_api_trace_set_filter_function_t *mp)
{
...
static void vl_api_trace_filter_function_dump_t_handler(vl_api_trace_filter_function_dump_t *mp)
{
    vl_api_registration_t                     *rp;
    ...
    vlib_main_t                               *vm      = vlib_get_main();
    vlib_is_packet_traced_fn_t                *current = vm->trace_main.current_trace_filter_function;
    rp                                                 = vl_api_client_index_to_registration(mp->client_index);
    ...
    while (reg) {
        ...
        dmp->_vl_msg_id = htons(VL_API_TRACE_FILTER_FUNCTION_DETAILS + (tdmp->msg_id_base));
        dmp->context    = mp->context;
