...
static u8 *format_srv6_am_rewrite_trace(u8 *s, va_list *args)
{
    ...
    return format(s, "SRv6-AM-rewrite: src %U dst %U", format_ip6_address, &t->src, format_ip6_address, &t->dst);
}
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                srv6_am_localsid_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof *tr);
                tr->localsid_index           = ls0 - sm->localsids;
            }

            /* This increments the SRv6 per LocalSID counters. */
            vlib_increment_combined_counter(((next0 == SRV6_AM_LOCALSID_NEXT_ERROR) ? &(sm->sr_ls_invalid_counters) : &(sm->sr_ls_valid_counters)),
                                            thread_index,
                                            ls0 - sm->localsids,
                                            1,
...
static uword srv6_am_rewrite_fn(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame)
{
    ...
    while (n_left_from > 0) {
        ...
        while (n_left_from > 0 && n_left_to_next > 0) {
            ...
            if (PREDICT_FALSE(node->flags & VLIB_NODE_FLAG_TRACE) && PREDICT_FALSE(b0->flags & VLIB_BUFFER_IS_TRACED)) {
                srv6_am_rewrite_trace_t *tr = vlib_add_trace(vm, node, b0, sizeof *tr);
    ...
    /* Update counters */
    vlib_node_increment_counter(vm, srv6_am_rewrite_node.index, SRV6_AM_REWRITE_COUNTER_PROCESSED, cnt_packets);
