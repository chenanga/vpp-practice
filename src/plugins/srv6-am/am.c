...
static int srv6_am_localsid_creation_fn(ip6_sr_localsid_t *localsid)
{
    ...
    if (nh_adj_index == ADJ_INDEX_INVALID) return -5;
    ...
    if (pool_is_free_index(sm->vnet_main->interface_main.sw_interfaces, ls_mem->sw_if_index_in)) return -3;
    ...
    int ret = vnet_feature_enable_disable("ip6-unicast", "srv6-am-rewrite", ls_mem->sw_if_index_in, 1, 0, 0);
    if (ret != 0) return -1;
...
static int srv6_am_localsid_removal_fn(ip6_sr_localsid_t *localsid)
{
    ...
    int ret = vnet_feature_enable_disable("ip6-unicast", "srv6-am-rewrite", ls_mem->sw_if_index_in, 0, 0, 0);
    if (ret != 0) return -1;
