...
u8 *format_gre_header_with_length(u8 *s, va_list *args)
{
    ...
    if (max_header_bytes != 0 && header_bytes > max_header_bytes) return format(s, "gre header truncated");
    ...
    if (max_header_bytes != 0 && header_bytes < max_header_bytes) {
        ...
        if (node->format_buffer)
            s = format(s, "\n%U%U", format_white_space, indent, node->format_buffer, (void *) (h + 1), max_header_bytes - header_bytes);
    }
...
uword unformat_gre_protocol_host_byte_order(unformat_input_t *input, va_list *args)
{
    ...
    /* Named type. */
    if (unformat_user(input, unformat_vlib_number_by_name, gm->protocol_info_by_name, &i)) {
...
{
    u16 *result = va_arg(*args, u16 *);
    if (!unformat_user(input, unformat_gre_protocol_host_byte_order, result)) return 0;
...
uword unformat_gre_header(unformat_input_t *input, va_list *args)
{
    ...
    if (!unformat(input, "%U", unformat_gre_protocol_host_byte_order, &p)) return 0;
...
static void gre44_fixup(vlib_main_t *vm, const ip_adjacency_t *adj, vlib_buffer_t *b0, const void *data)
{
    ...
    ip0->ip4.length = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, b0));
...
static void gre46_fixup(vlib_main_t *vm, const ip_adjacency_t *adj, vlib_buffer_t *b0, const void *data)
{
    ...
    ip0->ip6.payload_length = clib_host_to_net_u16(vlib_buffer_length_in_chain(vm, b0) - sizeof(ip0->ip6));
...
{
    if (fproto == FIB_PROTOCOL_IP6 && lt == VNET_LINK_IP6) return (gre66_fixup);
    if (fproto == FIB_PROTOCOL_IP6 && lt == VNET_LINK_IP4) return (gre46_fixup);
    if (fproto == FIB_PROTOCOL_IP4 && lt == VNET_LINK_IP6) return (gre64_fixup);
    if (fproto == FIB_PROTOCOL_IP4 && lt == VNET_LINK_IP4) return (gre44_fixup);
    if (fproto == FIB_PROTOCOL_IP6) return (grex6_fixup);
    if (fproto == FIB_PROTOCOL_IP4) return (grex4_fixup);
...
void gre_update_adj(vnet_main_t *vnm, u32 sw_if_index, adj_index_t ai)
{
    ...
    adj_nbr_midchain_update_rewrite(ai,
                                    gre_get_fixup(t->tunnel_dst.fp_proto, adj_get_link_type(ai)),
                                    uword_to_pointer(t->flags, void *),
                                    af,
                                    gre_build_rewrite(vnm, sw_if_index, adj_get_link_type(ai), &t->tunnel_dst.fp_addr));
...
adj_walk_rc_t mgre_mk_complete_walk(adj_index_t ai, void *data)
{
    ...
    adj_nbr_midchain_update_rewrite(
        ai,
        gre_get_fixup(ctx->t->tunnel_dst.fp_proto, adj_get_link_type(ai)),
        uword_to_pointer(ctx->t->flags, void *),
        af,
        gre_build_rewrite(vnet_get_main(), ctx->t->sw_if_index, adj_get_link_type(ai), &teib_entry_get_nh(ctx->ne)->fp_addr));
...
adj_walk_rc_t mgre_mk_incomplete_walk(adj_index_t ai, void *data)
{
    ...
    adj_nbr_midchain_update_rewrite(ai, gre_get_fixup(t->tunnel_dst.fp_proto, adj_get_link_type(ai)), NULL, ADJ_FLAG_NONE, NULL);
...
void mgre_update_adj(vnet_main_t *vnm, u32 sw_if_index, adj_index_t ai)
{
    ...
    adj_nbr_walk_nh(sw_if_index, adj->ia_nh_proto, &adj->sub_type.nbr.next_hop, mgre_mk_complete_walk, &ctx);
}
#endif /* CLIB_MARCH_VARIANT */
...
static_always_inline u32 gre_encap_inline(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, gre_tunnel_type_t type)
{
    ...
    while (n_left_from >= 2) {
        if (PREDICT_FALSE(sw_if_index[0] != vnet_buffer(b[0])->sw_if_index[VLIB_TX])) {
        ...
        if (PREDICT_FALSE(sw_if_index[1] != vnet_buffer(b[1])->sw_if_index[VLIB_TX])) {
            const vnet_hw_interface_t *hi;
    ...
    vlib_buffer_enqueue_to_single_next(vm, node, from, GRE_ENCAP_NEXT_L2_MIDCHAIN, frame->n_vectors);
...
VNET_DEVICE_CLASS(gre_device_class) = {.name                   = "GRE tunnel device",
...
static clib_error_t *gre_init(vlib_main_t *vm)
{
    ...
    gm->tunnel_by_key4            = hash_create_mem(0, sizeof(gre_tunnel_key4_t), sizeof(uword));
    gm->tunnel_by_key6            = hash_create_mem(0, sizeof(gre_tunnel_key6_t), sizeof(uword));
    gm->seq_num_by_key            = hash_create_mem(0, sizeof(gre_sn_key_t), sizeof(uword));
