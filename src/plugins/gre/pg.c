...
static inline void pg_gre_header_init(pg_gre_header_t *e)
{
...
uword unformat_pg_gre_header(unformat_input_t *input, va_list *args)
{
    ...
    h = pg_create_edit_group(s, sizeof(h[0]), sizeof(gre_header_t), &group_index);
    pg_gre_header_init(h);

    pg_edit_set_fixed(&h->flags_and_version, 0);

    error = 1;
    if (!unformat(input, "%U", unformat_pg_edit, unformat_gre_protocol_net_byte_order, &h->protocol)) goto done;
