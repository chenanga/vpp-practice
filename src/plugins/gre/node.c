...
always_inline void gre_trace(vlib_main_t         *vm,
...
always_inline void gre_tunnel_get(const gre_main_t       *gm,
                                  vlib_node_runtime_t    *node,
                                  vlib_buffer_t          *b,
                                  u16                    *next,
...
{
    const uword *p;
    p = is_ipv6 ? hash_get_mem(gm->tunnel_by_key6, &key->gtk_v6) : hash_get_mem(gm->tunnel_by_key4, &key->gtk_v4);
    if (PREDICT_FALSE(!p)) {
...
always_inline uword gre_input(vlib_main_t *vm, vlib_node_runtime_t *node, vlib_frame_t *frame, const int is_ipv6)
{
    ...
    while (n_left_from >= 2) {
        ...
        if (PREDICT_TRUE(cached_protocol == gre[0]->protocol)) {
            ...
            cached_next_index = nidx[0] = sparse_vec_index(gm->next_by_protocol, gre[0]->protocol);
        ...
        if (PREDICT_TRUE(cached_protocol == gre[1]->protocol)) {
            ...
            cached_next_index = nidx[1] = sparse_vec_index(gm->next_by_protocol, gre[1]->protocol);
        ...
        b[0]->error = nidx[0] == SPARSE_VEC_INVALID_INDEX ? node->errors[GRE_ERROR_UNKNOWN_PROTOCOL] : node->errors[GRE_ERROR_NONE];
        b[1]->error = nidx[1] == SPARSE_VEC_INVALID_INDEX ? node->errors[GRE_ERROR_UNKNOWN_PROTOCOL] : node->errors[GRE_ERROR_NONE];
        ...
        b[0]->error = version[0] ? node->errors[GRE_ERROR_UNSUPPORTED_VERSION] : b[0]->error;
        next[0]     = version[0] ? GRE_INPUT_NEXT_DROP : next[0];
        b[1]->error = version[1] ? node->errors[GRE_ERROR_UNSUPPORTED_VERSION] : b[1]->error;
        ...
        if (is_ipv6) {
            gre_mk_key6(&ip6[0]->dst_address, &ip6[0]->src_address, vnet_buffer(b[0])->ip.fib_index, type[0], TUNNEL_MODE_P2P, 0, &key[0].gtk_v6);
            gre_mk_key6(&ip6[1]->dst_address, &ip6[1]->src_address, vnet_buffer(b[1])->ip.fib_index, type[1], TUNNEL_MODE_P2P, 0, &key[1].gtk_v6);
            matched[0] = gre_match_key6(&cached_key.gtk_v6, &key[0].gtk_v6);
            matched[1] = gre_match_key6(&cached_key.gtk_v6, &key[1].gtk_v6);
        }
        else {
            gre_mk_key4(ip4[0]->dst_address, ip4[0]->src_address, vnet_buffer(b[0])->ip.fib_index, type[0], TUNNEL_MODE_P2P, 0, &key[0].gtk_v4);
            gre_mk_key4(ip4[1]->dst_address, ip4[1]->src_address, vnet_buffer(b[1])->ip.fib_index, type[1], TUNNEL_MODE_P2P, 0, &key[1].gtk_v4);
        ...
        if (PREDICT_FALSE(!matched[0]))
            gre_tunnel_get(gm, node, b[0], &next[0], &key[0], &cached_key, &tun_sw_if_index[0], &cached_tun_sw_if_index, is_ipv6);
        if (PREDICT_FALSE(!matched[1]))
            gre_tunnel_get(gm, node, b[1], &next[1], &key[1], &cached_key, &tun_sw_if_index[1], &cached_tun_sw_if_index, is_ipv6);

        if (PREDICT_TRUE(next[0] > GRE_INPUT_NEXT_DROP)) {
            vlib_increment_combined_counter(&gm->vnet_main->interface_main.combined_sw_if_counters[VNET_INTERFACE_COUNTER_RX],
                                            vm->thread_index,
                                            tun_sw_if_index[0],
                                            1 /* packets */,
                                            len[0] /* bytes */);
        ...
        if (PREDICT_FALSE(b[0]->flags & VLIB_BUFFER_IS_TRACED)) gre_trace(vm, node, b[0], tun_sw_if_index[0], ip6[0], ip4[0], is_ipv6);
        if (PREDICT_FALSE(b[1]->flags & VLIB_BUFFER_IS_TRACED)) gre_trace(vm, node, b[1], tun_sw_if_index[1], ip6[1], ip4[1], is_ipv6);
    ...
    vlib_node_increment_counter(vm, is_ipv6 ? gre6_input_node.index : gre4_input_node.index, GRE_ERROR_PKTS_DECAP, n_left_from);
...
void gre_register_input_protocol(vlib_main_t *vm, gre_protocol_t protocol, u32 node_index, gre_tunnel_type_t tunnel_type)
{
    ...
    n              = sparse_vec_validate(em->next_by_protocol, clib_host_to_net_u16(protocol));
...
static clib_error_t *gre_input_init(vlib_main_t *vm)
{
    ...
    gm->next_by_protocol = sparse_vec_new(/* elt bytes */ sizeof(gm->next_by_protocol[0]),
    ...
    gre_register_input_protocol(vm, GRE_PROTOCOL_teb, ethernet_input->index, GRE_TUNNEL_TYPE_TEB);

    gre_register_input_protocol(vm, GRE_PROTOCOL_ip4, ip4_input->index, GRE_TUNNEL_TYPE_L3);

    gre_register_input_protocol(vm, GRE_PROTOCOL_ip6, ip6_input->index, GRE_TUNNEL_TYPE_L3);

    gre_register_input_protocol(vm, GRE_PROTOCOL_mpls_unicast, mpls_unicast_input->index, GRE_TUNNEL_TYPE_L3);
