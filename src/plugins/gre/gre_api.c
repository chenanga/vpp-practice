...
static void send_gre_tunnel_details(gre_tunnel_t *t, vl_api_gre_tunnel_dump_t *mp)
{
    ...
    REPLY_MACRO_DETAILS2(VL_API_GRE_TUNNEL_DETAILS, ({
                             ...
                             rmp->tunnel.outer_table_id = htonl(fib_table_get_table_id(t->outer_fib_index, t->tunnel_dst.fp_proto));
...
static void vl_api_gre_tunnel_dump_t_handler(vl_api_gre_tunnel_dump_t *mp)
{
    ...
    if (~0 == sw_if_index) {
        ...
        if ((sw_if_index >= vec_len(gm->tunnel_index_by_sw_if_index)) || (~0 == gm->tunnel_index_by_sw_if_index[sw_if_index])) {
            return;
        }
