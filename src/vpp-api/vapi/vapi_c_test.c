...
START_TEST(test_invalid_values)
{
    ...
    rv = vapi_connect_ex(ctx, app_name, api_prefix, max_outstanding_requests, response_queue_size, VAPI_MODE_BLOCKING, true, use_uds);
    ck_assert_int_eq(VAPI_OK, rv);
...
START_TEST(test_hton_4)
{
    ...
    char                            x[sizeof(vapi_msg_bridge_domain_details) + vla_count * sizeof(vapi_type_bridge_domain_sw_if)];
...
vapi_error_e show_version_cb(vapi_ctx_t ctx, void *caller_ctx, vapi_error_e rv, bool is_last, vapi_payload_show_version_reply *p)
{
    ck_assert_int_eq(VAPI_OK, rv);
...
vapi_error_e loopback_create_cb(vapi_ctx_t ctx, void *caller_ctx, vapi_error_e rv, bool is_last, vapi_payload_create_loopback_reply *p)
{
...
vapi_error_e loopback_delete_cb(vapi_ctx_t ctx, void *caller_ctx, vapi_error_e rv, bool is_last, vapi_payload_delete_loopback_reply *p)
{
...
void setup_nonblocking(void)
{
    vapi_error_e rv = vapi_ctx_alloc(&ctx);
    ck_assert_int_eq(VAPI_OK, rv);
    rv = vapi_connect_ex(ctx, app_name, api_prefix, max_outstanding_requests, response_queue_size, VAPI_MODE_NONBLOCKING, true, use_uds);
    ck_assert_int_eq(VAPI_OK, rv);
}
...
vapi_error_e sw_interface_dump_cb(struct vapi_ctx_s *ctx, void *callback_ctx, vapi_error_e rv, bool is_last, vapi_payload_sw_interface_details *reply)
{
...
vapi_error_e vapi_mss_clamp_enable_disable_reply_cb(struct vapi_ctx_s                           *ctx,
                                                    void                                        *callback_ctx,
                                                    vapi_error_e                                 rv,
                                                    bool                                         is_last,
...
vapi_error_e vapi_mss_clamp_get_reply_cb(struct vapi_ctx_s                *ctx,
                                         void                             *callback_ctx,
                                         vapi_error_e                      rv,
                                         bool                              is_last,
...
vapi_error_e vapi_mss_clamp_get_details_cb(struct vapi_ctx_s              *ctx,
                                           void                           *callback_ctx,
                                           vapi_error_e                    rv,
                                           bool                            is_last,
...
START_TEST(test_loopbacks_1)
{
    ...
    for (i = 0; i < num_ifs; ++i) {
        ...
        vapi_error_e rv = vapi_create_loopback(ctx, cl, loopback_create_cb, &clcs[i]);
        ck_assert_int_eq(VAPI_OK, rv);
    }
    for (i = 0; i < num_ifs; ++i) {
        ck_assert_int_eq(1, clcs[i].called);
        printf(
            "Created loopback with MAC %02x:%02x:%02x:%02x:%02x:%02x --> "
            "sw_if_index %u\n",
            mac_addresses[i][0],
            mac_addresses[i][1],
            mac_addresses[i][2],
    ...
    {  // new context
        for (int i = 0; i < num_ifs; ++i) {
            vapi_msg_mss_clamp_enable_disable *mc = vapi_alloc_mss_clamp_enable_disable(ctx);
            ...
            vapi_error_e rv = vapi_mss_clamp_enable_disable(ctx, mc, vapi_mss_clamp_enable_disable_reply_cb, &reply_ctx);
            ck_assert_int_eq(VAPI_OK, rv);
    ...
    {  // new context
        ...
        vapi_error_e rv = vapi_mss_clamp_get(ctx, msg, vapi_mss_clamp_get_reply_cb, &counter, vapi_mss_clamp_get_details_cb, &counter);
        printf("counter is %d", counter);
        ck_assert_int_eq(VAPI_OK, rv);
    ...
    for (i = 0; i < attempts; ++i) {
        ...
        while (VAPI_EAGAIN == (rv = vapi_sw_interface_dump(ctx, dump, sw_interface_dump_cb, &dctx)))
            ;
    ...
    for (i = 0; i < num_ifs; ++i) {
        ...
        vapi_error_e rv              = vapi_delete_loopback(ctx, dl, loopback_delete_cb, &dlcs[i]);
        ck_assert_int_eq(VAPI_OK, rv);
    }
    for (i = 0; i < num_ifs; ++i) {
...
START_TEST(test_show_version_3)
{
    ...
    vapi_msg_show_version *sv = vapi_alloc_show_version(ctx);
    ck_assert_ptr_ne(NULL, sv);
    while (VAPI_EAGAIN == (rv = vapi_show_version(ctx, sv, show_version_cb, &called)))
        ;
    ck_assert_int_eq(VAPI_OK, rv);
...
START_TEST(test_show_version_4)
{
    ...
    for (i = 0; i < num_req; ++i) {
        vapi_msg_show_version *sv = vapi_alloc_show_version(ctx);
        ck_assert_ptr_ne(NULL, sv);
        while (VAPI_EAGAIN == (rv = vapi_show_version(ctx, sv, show_version_cb, &contexts[i])))
            ;
        ck_assert_int_eq(VAPI_OK, rv);
        int j;
        for (j = 0; j < num_req; ++j) {
...
START_TEST(test_loopbacks_2)
{
    ...
    for (i = 0; i < num_ifs; ++i) {
        ...
        while (VAPI_EAGAIN == (rv = vapi_create_loopback(ctx, cl, loopback_create_cb, &clcs[i])))
            ;
        ck_assert_int_eq(VAPI_OK, rv);
    }
    while (VAPI_EAGAIN == (rv = vapi_dispatch(ctx)))
    ...
    for (i = 0; i < num_ifs; ++i) {
        ck_assert_int_eq(1, clcs[i].called);
        printf(
            "Loopback with MAC %02x:%02x:%02x:%02x:%02x:%02x --> "
            "sw_if_index %u\n",
            mac_addresses[i][0],
            mac_addresses[i][1],
            mac_addresses[i][2],
    ...
    for (i = 0; i < num_ifs; ++i) {
        vapi_msg_delete_loopback *dl = vapi_alloc_delete_loopback(ctx);
        dl->payload.sw_if_index      = sw_if_indexes[i];
        while (VAPI_EAGAIN == (rv = vapi_delete_loopback(ctx, dl, loopback_delete_cb, &dlcs[i])))
            ;
        ck_assert_int_eq(VAPI_OK, rv);
    }
    while (VAPI_EAGAIN == (rv = vapi_dispatch(ctx)))
...
vapi_error_e generic_cb(vapi_ctx_t ctx, void *callback_ctx, vapi_msg_id_t id, void *msg)
{
...
{
    printf(
        "--- Receive show version using generic callback - nonblocking "
        "API ---\n");
    vapi_error_e           rv;
    vapi_msg_show_version *sv = vapi_alloc_show_version(ctx);
    ck_assert_ptr_ne(NULL, sv);
...
vapi_error_e show_version_no_cb(vapi_ctx_t ctx, void *caller_ctx, vapi_error_e rv, bool is_last, vapi_payload_show_version_reply *p)
{
...
START_TEST(test_no_response_1)
{
    ...
    int called            = 0;
    while (VAPI_EAGAIN == (rv = vapi_show_version(ctx, sv, show_version_no_cb, &called)))
        ;
    ck_assert_int_eq(VAPI_OK, rv);
    sv = vapi_alloc_show_version(ctx);
    ck_assert_ptr_ne(NULL, sv);
...
vapi_error_e no_msg_cb(struct vapi_ctx_s *ctx, void *callback_ctx, vapi_error_e rv, bool is_last, vapi_payload_sw_interface_details *reply)
{
    int *called = callback_ctx;
    ++*called;
    ck_assert_int_eq(VAPI_OK, rv);
...
START_TEST(test_no_response_2)
{
    ...
    int no_called                    = 0;
    while (VAPI_EAGAIN == (rv = vapi_sw_interface_dump(ctx, dump, no_msg_cb, &no_called)))
        ;
    ck_assert_int_eq(VAPI_OK, rv);
    while (VAPI_EAGAIN == (rv = vapi_dispatch(ctx)))
        ;
...
START_TEST(test_api_strings)
{
    ...
    vapi_msg_sw_interface_dump *dump = malloc(sizeof(vapi_msg_sw_interface_dump) + strlen(str));
    clib_mem_init(0, 1 << 20);

    vl_api_c_string_to_api_string(str, &dump->payload.name_filter);
    /* Assert nul terminator NOT present */
    ck_assert_int_eq(vl_api_string_len(&dump->payload.name_filter), strlen(str));
    ...
    vstr = vl_api_from_api_to_new_vec(0 /* not really an API message */, &dump->payload.name_filter);
    ...
    /* Assert nul terminator NOT present */
    ck_assert_int_eq(vl_api_string_len(&dump->payload.name_filter), vec_len(vec_str));
    ...
    vstr = vl_api_from_api_to_new_vec(0 /* not a real api msg */, &dump->payload.name_filter);
    ck_assert_ptr_ne(vstr, NULL);
    /* Assert nul terminator NOT present */
    ck_assert_int_eq(vec_len(vstr), strlen(str));
    vec_free(vstr);
...
int main(int argc, char *argv[])
{
    ...
    if (!strcmp(argv[3], "shm"))
        ...
        printf("Unrecognised required argument '%s', expected 'uds' or 'shm'.", argv[3]);
        return EXIT_FAILURE;
    }
