...
void vapi_store_request(vapi_ctx_t             ctx,
                        u32                    context,
                        vapi_msg_id_t          response_id,
                        enum vapi_request_type request_type,
                        vapi_cb_t              callback,
                        void                  *callback_ctx)
{
    ...
    VAPI_DBG("stored@%d: context:%x (start is @%d)", requests_end, context, ctx->requests_start);
    ++ctx->requests_count;
...
static void *vapi_shm_msg_alloc(vapi_ctx_t ctx, size_t size)
{
    if (!ctx->connected) {
        return NULL;
    }
...
static void *vapi_sock_msg_alloc(size_t size)
{
    u8 *rv = 0;
...
{
    if (ctx->use_uds) return vapi_sock_msg_alloc(size);
...
vapi_error_e vapi_ctx_alloc(vapi_ctx_t *result)
{
    ...
    ctx->vapi_msg_id_t_to_vl_msg_id = malloc(__vapi_metadata.count * sizeof(*ctx->vapi_msg_id_t_to_vl_msg_id));
    if (!ctx->vapi_msg_id_t_to_vl_msg_id) {
        goto fail;
    }
    clib_memset(ctx->vapi_msg_id_t_to_vl_msg_id, ~0, __vapi_metadata.count * sizeof(*ctx->vapi_msg_id_t_to_vl_msg_id));
...
__clib_nosanitize_addr static void VL_API_VEC_UNPOISON(const void *v)
{
    const vec_header_t *vh = &((vec_header_t *) v)[-1];
    clib_mem_unpoison(vh, sizeof(*vh) + vec_len(v));
}

static void vapi_api_name_and_crc_free(vapi_ctx_t ctx)
{
    int          i;
    u8         **keys = 0;
    hash_pair_t *hp;

    if (!ctx->msg_index_by_name_and_crc) return;
    hash_foreach_pair(hp, ctx->msg_index_by_name_and_crc, ({ vec_add1(keys, (u8 *) hp->key); }));
...
static vapi_error_e vapi_sock_get_errno(int err)
{
    switch (err) {
        case ENOTSOCK:
...
static vapi_error_e vapi_sock_send(vapi_ctx_t ctx, u8 *msg)
{
    size_t        n;
    struct msghdr hdr;
...
static vapi_error_e vapi_sock_send2(vapi_ctx_t ctx, u8 *msg1, u8 *msg2)
{
    size_t        n;
    struct msghdr hdr;
...
static vapi_error_e vapi_sock_recv_internal(vapi_ctx_t ctx, u8 **vec_msg, u32 timeout)
{
    ...
    if (ctx->client_socket.fd == 0) return VAPI_ENOTSOCK;
    ...
    while (1) {
        ...
        while (current_rx_index < sizeof(*mbp)) {
            ...
            n = recv(sock->fd, sock->rx_buffer + current_rx_index, sizeof(*mbp) - current_rx_index, MSG_DONTWAIT);
            if (n < 0) {
                if (errno == EAGAIN && clib_time_now(&ctx->time) >= deadline) return VAPI_EAGAIN;
        ...
        while (current_rx_index < msg_size) {
            n = recv(sock->fd, sock->rx_buffer + current_rx_index, msg_size - current_rx_index, MSG_DONTWAIT);
            if (n < 0) {
                if (errno == EAGAIN && clib_time_now(&ctx->time) >= deadline) return VAPI_EAGAIN;
        ...
        if (vec_len(sock->rx_buffer) >= data_len + sizeof(*mbp)) {
            ...
            if (vec_len(sock->rx_buffer) == 0) break;
        }
    }
    return rv;
}

static void vapi_memclnt_create_v2_reply_t_handler(vapi_ctx_t ctx, vl_api_memclnt_create_v2_reply_t *mp)
{
...
static void vapi_sockclnt_create_reply_t_handler(vapi_ctx_t ctx, vl_api_sockclnt_create_reply_t *mp)
{
    int i;
    u8 *name_and_crc;
    ...
    for (i = 0; i < be16toh(mp->count); i++) {
        ...
        hash_set_mem(ctx->msg_index_by_name_and_crc, name_and_crc, be16toh(mp->message_table[i].index));
    }
}

static void vapi_memclnt_delete_reply_t_handler(vapi_ctx_t ctx, vl_api_memclnt_delete_reply_t *mp)
{
    void *oldheap;
...
static void vapi_sockclnt_delete_reply_t_handler(vapi_ctx_t ctx, vl_api_sockclnt_delete_reply_t *mp)
{
    ctx->my_client_index = ~0;
    ctx->vl_input_queue  = 0;
}

static int vapi_shm_client_connect(vapi_ctx_t ctx, const char *name, int ctx_quota, int input_queue_size, bool keepalive)
{
    ...
    oldheap        = vl_msg_push_heap();
    vl_input_queue = svm_queue_alloc_and_init(input_queue_size, sizeof(uword), getpid());
    vl_msg_pop_heap(oldheap);
    ...
    while (1) {
        ...
        for (i = 0; i < 1000; i++) {
            qstatus = svm_queue_sub(vl_input_queue, (u8 *) &rp, SVM_Q_NOWAIT, 0);
            if (qstatus == 0) goto read_one_msg;
            ts.tv_sec  = 0;
...
static int vapi_sock_client_connect(vapi_ctx_t ctx, char *path, const char *name)
{
    clib_error_t                   *error;
    clib_socket_t                  *sock;
    ...
    if (ctx->client_socket.fd) return VAPI_EINVAL;

    if (name == 0) return VAPI_EINVAL;
...
static void vapi_shm_client_send_disconnect(vapi_ctx_t ctx, u8 do_cleanup)
{
...
static vapi_error_e vapi_sock_client_send_disconnect(vapi_ctx_t ctx)
{
...
static int vapi_shm_client_disconnect(vapi_ctx_t ctx)
{
    ...
    while (1) {
        ...
        if (svm_queue_sub(vl_input_queue, (u8 *) &rp, SVM_Q_NOWAIT, 0) < 0) continue;
...
static vapi_error_e vapi_sock_client_disconnect(vapi_ctx_t ctx)
{
    ...
    while (1) {
        ...
        if (vapi_sock_recv_internal(ctx, &msg, 0) != VAPI_OK) continue;
...
int vapi_client_disconnect(vapi_ctx_t ctx)
{
    if (ctx->use_uds) {
...
u32 vapi_api_get_msg_index(vapi_ctx_t ctx, u8 *name_and_crc)
{
    uword *p;
                             ...
                             bool        use_uds)
{
    ...
    if (use_uds) {
        ...
        if (vapi_shm_client_connect(ctx, (char *) name, 0, response_queue_size, true) < 0) {
            vl_client_api_unmap();
            return VAPI_ECON_FAIL;
        }
    ...
    for (i = 0; i < __vapi_metadata.count; ++i) {
        ...
        if (VAPI_INVALID_MSG_ID != id) {
            ...
            if (id > ctx->vl_msg_id_max) {
                vapi_msg_id_t *tmp = realloc(ctx->vl_msg_id_to_vapi_msg_t, sizeof(*ctx->vl_msg_id_to_vapi_msg_t) * (id + 1));
                if (!tmp) {
                    rv = VAPI_ENOMEM;
                    goto fail;
                }
...
#if VAPI_DEBUG_CONNECT
            VAPI_DBG("Message `%s' has vl_msg_id `%u'", m->name_with_crc, (unsigned) id);
#endif
        }
        else {
...
#endif
    if (!vapi_is_msg_available(ctx, vapi_msg_id_control_ping) || !vapi_is_msg_available(ctx, vapi_msg_id_control_ping_reply)) {
        VAPI_ERR("control ping or control ping reply not available, cannot connect");
...
vapi_error_e vapi_connect(vapi_ctx_t  ctx,
                          ...
                          bool        handle_keepalives)
{
    return vapi_connect_ex(ctx, name, chroot_prefix, max_outstanding_requests, response_queue_size, mode, handle_keepalives, false);
}
                                   ...
                                   bool        handle_keepalives)
{
    ...
    VAPI_DBG("connect client `%s'", name);
    if (vapi_shm_client_connect(ctx, (char *) name, 0, response_queue_size, handle_keepalives) < 0) {
        return VAPI_ECON_FAIL;
    }
...
vapi_error_e vapi_disconnect_from_vpp(vapi_ctx_t ctx)
{
    ...
    while (1) {
        ...
        vapi_memclnt_delete_reply_t_handler(ctx, (void *) rp /*, ntohl (msgbuf->data_len)*/);
        break;
    }
fail:
    vapi_api_name_and_crc_free(ctx);
...
static vapi_error_e vapi_shm_disconnect(vapi_ctx_t ctx)
{
    vl_api_memclnt_delete_reply_t *rp;
    svm_queue_t                   *vl_input_queue;
    time_t                         begin;
...
static vapi_error_e vapi_sock_disconnect(vapi_ctx_t ctx)
{
    vl_api_sockclnt_delete_reply_t *rp;
    time_t                          begin;
    u8                             *msg = 0;
    ...
    while (1) {
        ...
        if (vapi_sock_recv_internal(ctx, &msg, 0) < 0) continue;
        ...
        vapi_sockclnt_delete_reply_t_handler(ctx, (void *) rp /*, ntohl (msgbuf->data_len)*/);
        break;
    }
fail:
    clib_socket_close(&ctx->client_socket);
...
#if VAPI_DEBUG
static void vapi_debug_log(vapi_ctx_t ctx, void *msg, const char *fun)
{
    ...
    if (msgid <= ctx->vl_msg_id_max) {
        ...
        if (id < __vapi_metadata.count) {
            VAPI_DBG("%s msg@%p:%u[%s]", fun, msg, msgid, __vapi_metadata.msgs[id]->name);
        }
        else {
...
static vapi_error_e vapi_shm_send(vapi_ctx_t ctx, void *msg)
{
    int          rv = VAPI_OK;
    int          tmp;
...
#endif
    tmp = svm_queue_add(q, (u8 *) &msg, VAPI_MODE_BLOCKING == ctx->mode ? 0 : 1);
    if (tmp < 0) {
        rv = VAPI_EAGAIN;
    }
    else
...
static vapi_error_e vapi_shm_send2(vapi_ctx_t ctx, void *msg1, void *msg2)
{
    vapi_error_e rv = VAPI_OK;
    svm_queue_t *q  = vlibapi_get_main()->shmem_hdr->vl_input_queue;
#if VAPI_DEBUG
...
#endif
    int tmp = svm_queue_add2(q, (u8 *) &msg1, (u8 *) &msg2, VAPI_MODE_BLOCKING == ctx->mode ? 0 : 1);
    if (tmp < 0) {
        rv = VAPI_EAGAIN;
    }
    else
...
static vapi_error_e vapi_shm_recv(vapi_ctx_t ctx, void **msg, size_t *msg_size, svm_q_conditional_wait_t cond, u32 time)
{
    vapi_error_e rv = VAPI_OK;
    uword        data;
...
static vapi_error_e vapi_sock_recv(vapi_ctx_t ctx, void **msg, size_t *msg_size, u32 time)
{
    vapi_error_e rv   = VAPI_OK;
    u8          *data = 0;
    if (time == 0 && ctx->mode == VAPI_MODE_BLOCKING) time = 1;
...
again:
    ...
    if (ctx->handle_keepalives) {
        ...
        if (msgid == vapi_lookup_vl_msg_id(ctx, vapi_msg_id_memclnt_keepalive)) {
            ...
            reply->header._vl_msg_id = vapi_lookup_vl_msg_id(ctx, vapi_msg_id_memclnt_keepalive_reply);
...
{
    if (ctx->use_uds) return VAPI_ENOTSUP;
...
static vapi_error_e vapi_dispatch_response(vapi_ctx_t ctx, vapi_msg_id_t id, u32 context, void *msg)
{
    ...
    VAPI_DBG("dispatch, search from %d, %s at %d", ctx->requests_start, ctx->requests[tmp].context == context ? "matched" : "stopped", tmp);
    vapi_error_e rv = VAPI_OK;
    if (ctx->requests[tmp].context == context) {
        while (ctx->requests_start != tmp) {
            VAPI_ERR("No response to req with context=%u", (unsigned) ctx->requests[tmp].context);
            ctx->requests[ctx->requests_start].callback(ctx, ctx->requests[ctx->requests_start].callback_ctx, VAPI_ENORESP, true, NULL);
            clib_memset(&ctx->requests[ctx->requests_start], 0, sizeof(ctx->requests[ctx->requests_start]));
            ++ctx->requests_start;
            --ctx->requests_count;
        ...
        switch (ctx->requests[tmp].type) {
            case VAPI_REQUEST_STREAM:
                if (ctx->requests[tmp].response_id == id) {
                    ...
                    VAPI_DBG(
                        "Stream response ID doesn't match current ID, move to "
                        "next ID");
                    clib_memset(&ctx->requests[tmp], 0, sizeof(ctx->requests[tmp]));
                    ++ctx->requests_start;
                    --ctx->requests_count;
                    if (ctx->requests_start == ctx->requests_size) {
                        ctx->requests_start = 0;
                    ...
                    if (ctx->requests[tmp].context != context) {
                        VAPI_ERR("Unexpected context %u, expected context %u!", ctx->requests[tmp].context, context);
                    }
                }
                break;
        ...
        if (payload_offset != -1) {
            rv = ctx->requests[tmp].callback(ctx, ctx->requests[tmp].callback_ctx, VAPI_OK, is_last, payload);
        }
        else {
            /* this is a message without payload, so bend the callback a little
             */
            rv = ((vapi_error_e(*)(vapi_ctx_t, void *, vapi_error_e, bool)) ctx->requests[tmp].callback)(ctx,
        ...
        VAPI_DBG("after dispatch, req start = %d, end = %d, count = %d", ctx->requests_start, requests_end, ctx->requests_count);
    }
    if (0 != (mrv = pthread_mutex_unlock(&ctx->requests_mutex))) {
        VAPI_DBG("pthread_mutex_unlock() failed, rv=%d:%s", mrv, strerror(mrv));
...
static vapi_error_e vapi_dispatch_event(vapi_ctx_t ctx, vapi_msg_id_t id, void *msg)
{
    if (ctx->event_cbs[id].cb) {
        ...
        VAPI_DBG("No handler/generic handler for msg id %u[%s], message ignored", (unsigned) id, __vapi_metadata.msgs[id]->name);
    }
    return VAPI_OK;
}
...
static int vapi_verify_msg_size(vapi_msg_id_t id, void *buf, uword buf_size)
{
    assert(id < __vapi_metadata.count);
...
vapi_error_e vapi_dispatch_one_timedwait(vapi_ctx_t ctx, u32 wait_time)
{
    ...
    void                    *msg;
    uword                    size;
    svm_q_conditional_wait_t cond = vapi_is_nonblocking(ctx) ? (wait_time ? SVM_Q_TIMEDWAIT : SVM_Q_NOWAIT) : SVM_Q_WAIT;
    ...
    if (vpp_id > ctx->vl_msg_id_max) {
        VAPI_ERR("Unknown msg ID received, id `%u', out of range <0,%u>", (unsigned) vpp_id, (unsigned) ctx->vl_msg_id_max);
        vapi_msg_free(ctx, msg);
        return VAPI_EINVAL;
    }
    if (VAPI_INVALID_MSG_ID == (unsigned) ctx->vl_msg_id_to_vapi_msg_t[vpp_id]) {
        VAPI_ERR("Unknown msg ID received, id `%u' marked as not supported", (unsigned) vpp_id);
        vapi_msg_free(ctx, msg);
        return VAPI_EINVAL;
    }
