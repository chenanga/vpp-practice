...
static void vac_api_handler(void *msg)
{
    ...
    if (l == 0) clib_warning("Message ID %d has wrong length: %d\n", id, l);
...
static void *vac_timeout_thread_fn(void *arg)
{
    ...
    while (pm->timeout_loop) {
        ...
        if (!timeout_cancelled) {
            rv = pthread_cond_timedwait(&pm->timeout_cancel_cv, &pm->timeout_lock, &ts);
...
int vac_connect(char *name, char *chroot_prefix, vac_callback_t cb, int rx_qlen)
{
    ...
    init();
    if (chroot_prefix != NULL) vl_set_memory_root_path(chroot_prefix);
    ...
    rv                       = pthread_create(&pm->timeout_thread_handle, NULL, vac_timeout_thread_fn, 0);
    if (rv) {
        clib_warning("pthread_create returned %d", rv);
        vl_client_api_unmap();
        return (-1);
...
int vac_disconnect(void)
{
    ...
    if (pm->rx_thread_handle) {
        ...
        pthread_mutex_lock(&pm->queue_lock);
        if (rx_thread_done == false) rv = pthread_cond_timedwait(&pm->terminate_cv, &pm->queue_lock, &ts);
        pthread_mutex_unlock(&pm->queue_lock);
...
static u32 vac_client_index(void)
{
...
int vac_msg_table_max_index(void)
{
    ...
    hash_foreach_pair(hp, h, ({
                          if (hp->value[0] > max) max = hp->value[0];
                      }));
