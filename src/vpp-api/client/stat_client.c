...
static vlib_stats_entry_t *get_stat_vector_r(stat_client_main_t *sm)
{
...
int stat_segment_connect_r(const char *socket_name, stat_client_main_t *sm)
{
    ...
    sm->directory_vector = stat_segment_adjust(sm, (void *) sm->shared_header->directory_vector);
...
#define stat_vec_dup(S, V)                                                                                                                 \
    ({                                                                                                                                     \
        ...
        if (V && ((void *) V > (void *) S->shared_header) && (((void *) V + vec_bytes(V)) < ((void *) S->shared_header + S->memory_size))) \
            _v(v) = vec_dup(V);                                                                                                            \
        _v(v);                                                                                                                             \
    })
 ...
 */
static stat_segment_data_t copy_data(vlib_stats_entry_t *ep, u32 index2, char *name, stat_client_main_t *sm, bool via_symlink)
{
    ...
    switch (ep->type) {
        ...
        case STAT_DIR_TYPE_COUNTER_VECTOR_COMBINED:
            ...
            for (i = 0; i < vec_len(combined_c); i++) {
                ...
                if (index2 != ~0)
                    result.combined_counter_vec[i] = stat_vec_combined_init(cb[index2]);
                else
...
uint32_t *stat_segment_ls_r(uint8_t **patterns, stat_client_main_t *sm)
{
    ...
    for (j = 0; j < vec_len(counter_vec); j++) {
        ...
        if (vec_len(patterns) == 0) vec_add1(dir, j);
    }
...
stat_segment_data_t *stat_segment_dump_r(uint32_t *stats, stat_client_main_t *sm)
{
    ...
    if (res) stat_segment_data_free(res);
    return 0;
}
