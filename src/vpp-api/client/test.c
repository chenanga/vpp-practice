...
static void test_messages(void)
{
    ...
    printf("\nTook %.2f msec, %.0f msgs/msec \n", (timestamp_end - timestamp_start), no_msgs / (timestamp_end - timestamp_start));
    printf("Exiting...\n");
    vac_disconnect();
}
...
static void test_stats(void)
{
    ...
    for (i = 0; i < vec_len(res); i++) {
        switch (res[i].type) {
            case STAT_DIR_TYPE_NAME_VECTOR:
                if (res[i].name_vector == 0) continue;
                for (k = 0; k < vec_len(res[i].name_vector); k++)
                    if (res[i].name_vector[k]) fformat(stdout, "[%d]: %s %s\n", k, res[i].name_vector[k], res[i].name);
                break;
            default:
                assert(0);
        }
    ...
    u8 *leak_report = format(0, "%U", format_clib_mem_heap, 0, 1 /* verbose, i.e. print leaks */);
