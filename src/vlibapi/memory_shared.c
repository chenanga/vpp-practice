...
__clib_nosanitize_addr static inline void *vl_msg_api_alloc_internal(svm_region_t *vlib_rp, int nbytes, int pool, int may_return_null)
{
    int             i;
    msgbuf_t       *rv;
    ring_alloc_t   *ap;
    ...
    for (i = 0; i < vec_len(ap); i++) {
        ...
        if (rv->q) {
            ...
            if (PREDICT_TRUE(rv->gc_mark_timestamp == 0))
                ...
                if (now - rv->gc_mark_timestamp > 10) {
                    if (CLIB_DEBUG > 0) {
                        u16               *msg_idp, msg_id;
                        vl_api_msg_data_t *m;
                        clib_warning("garbage collect pool %d ring %d index %d", pool, i, q->head);
                        msg_idp = (u16 *) (rv->data);
                        msg_id  = clib_net_to_host_u16(*msg_idp);
                        m       = vl_api_get_msg_data(am, msg_id);
                        if (m) clib_warning("msg id %d name %s", (u32) msg_id, m->name);
                    }
            ...
            ap[i].misses++;
            if (pool == 0) pthread_mutex_unlock(&q->mutex);
            continue;
        }
    collected:
...
void *vl_msg_api_alloc(int nbytes)
{
    ...
    return vl_msg_api_alloc_internal(am->vlib_rp, nbytes, pool, 0 /* may_return_null */);
}
...
void *vl_msg_api_alloc_or_null(int nbytes)
{
    ...
    pool = (am->our_pid == shmem_hdr->vl_pid);
    return vl_msg_api_alloc_internal(am->vlib_rp, nbytes, pool, 1 /* may_return_null */);
}

void *vl_msg_api_alloc_as_if_client(int nbytes)
{
    api_main_t *am = vlibapi_get_main();
    return vl_msg_api_alloc_internal(am->vlib_rp, nbytes, 0, 0 /* may_return_null */);
}
...
{
    api_main_t *am = vlibapi_get_main();
    return vl_msg_api_alloc_internal(am->vlib_rp, nbytes, 0, 1 /* may_return_null */);
}

void *vl_mem_api_alloc_as_if_client_w_reg(vl_api_registration_t *reg, int nbytes)
{
    return vl_msg_api_alloc_internal(reg->vlib_rp, nbytes, 0, 0 /* may_return_null */);
}
...
static void vl_api_default_mem_config(vl_shmem_hdr_t *shmem_hdr)
{
    ...
    if (am->vlib_input_queue_length) vlib_input_queue_length = am->vlib_input_queue_length;

    shmem_hdr->vl_input_queue = svm_queue_alloc_and_init(vlib_input_queue_length, sizeof(uword), getpid());
...
void vl_init_shmem(svm_region_t *vlib_rp, vl_api_shm_elem_config_t *config, int is_vlib, int is_private_region)
{
    ...
    if (is_private_region == 0) {
        ...
        am->vlib_rp   = vlib_rp;
        am->our_pid   = getpid();
        if (is_vlib) am->shmem_hdr->vl_pid = am->our_pid;
    }
    else
...
int vl_map_shmem(const char *region_name, int is_vlib)
{
    ...
    if (strstr(region_name, vpe_api_region_suffix)) {
        ...
        vec_set_len(root_path, vec_len(root_path) - strlen(vpe_api_region_suffix));
    ...
    if (is_vlib == 0) {
         ...
         */
        if (am->root_path)
            api_name = format(0, "/dev/shm/%s-%s%c", am->root_path, region_name + 1, 0);
        else
    ...
    if (vlib_rp->user_ctx) {
        ...
        mutex_ok:
            am->vlib_rp = vlib_rp;
            while (svm_queue_sub(q, (u8 *) &old_msg, SVM_Q_NOWAIT, 0) != -2 /* queue underflow */) {
    ...
    if (!is_vlib) {
        ...
        for (i = 0; i < 10000; i++) {
            ...
            while (nanosleep(&ts, &tsrem) < 0)
                ts = tsrem;
            if (vlib_rp->user_ctx) goto ready;
        }
        /* Clean up and leave... */
    ...
    vl_init_shmem(vlib_rp, 0 /* default config */, 1 /* is vlib */, 0 /* is_private_region */);
...
void vl_msg_api_send_shmem(svm_queue_t *q, u8 *elem)
{
    ...
    if (am->tx_trace && am->tx_trace->enabled) vl_msg_api_trace(am, am->tx_trace, msg);
     ...
     */
    if (PREDICT_FALSE(am->vl_clients /* vpp side */ && (q->cursize == q->maxsize))) {
