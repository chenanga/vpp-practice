...
void vl_msg_api_trace(api_main_t *am, vl_api_trace_t *tp, void *msg)
{
    ...
    if (vec_len(tp->traces) < tp->nitems) {
        ...
        if (tp->curindex == tp->nitems) tp->curindex = 0;
...
static int vl_msg_api_trace_write_one(api_main_t *am, u8 *msg, FILE *fp)
{
    u8 *tmpmem = 0;
    int tlen, slen;
...
static int vl_msg_write_json_fn(u8 *msg, void *ctx)
{
    ...
    if (arg->i < arg->n_traces - 1) vl_msg_fwrite(",\n", fp);
    arg->i++;
    return 0;
}

static int vl_msg_api_trace_write_json(api_main_t *am, vl_api_trace_t *tp, FILE *fp)
{
...
int vl_msg_traverse_trace(vl_api_trace_t *tp, vl_msg_traverse_trace_fn fn, void *ctx)
{
    int i;
    u8 *msg;
    int rv = 0;
...
static int vl_api_msg_write_fn(u8 *msg, void *ctx)
{
    FILE *fp         = ctx;
...
int vl_msg_api_trace_save(api_main_t *am, vl_api_trace_which_t which, FILE *fp, u8 is_json)
{
    vl_api_trace_t            *tp;
    ...
    if (is_json) return vl_msg_api_trace_write_json(am, tp, fp);
...
always_inline void msg_handler_internal(api_main_t *am, void *the_msg, uword msg_len, int trace_it, int do_it, int free_it)
{
    ...
    if (m && m->handler) {
        if (trace_it) vl_msg_api_trace(am, am->rx_trace, the_msg);
        ...
        if (m->calc_size_func) {
            ...
            clib_warning(
                "Message '%s' (id %u) has NULL calc_size_func, cannot "
        ...
        if (do_it && calc_size <= msg_len) {
            ...
            if (m->is_autoendian) m->endian_handler(the_msg, 0);
    ...
    if (free_it) vl_msg_api_free(the_msg);
...
void vl_msg_api_handler(void *the_msg, uword msg_len)
{
    ...
    msg_handler_internal(am, the_msg, msg_len, (am->rx_trace && am->rx_trace->enabled) /* trace_it */, 1 /* do_it */, 1 /* free_it */);
}

void vl_msg_api_handler_no_free(void *the_msg, uword msg_len)
{
    api_main_t *am = vlibapi_get_main();
    msg_handler_internal(am, the_msg, msg_len, (am->rx_trace && am->rx_trace->enabled) /* trace_it */, 1 /* do_it */, 0 /* free_it */);
}

void vl_msg_api_handler_no_trace_no_free(void *the_msg, uword msg_len)
{
    api_main_t *am = vlibapi_get_main();
    msg_handler_internal(am, the_msg, msg_len, 0 /* trace_it */, 1 /* do_it */, 0 /* free_it */);
}
...
void vl_msg_api_trace_only(void *the_msg, uword msg_len)
{
    ...
    msg_handler_internal(am, the_msg, msg_len, (am->rx_trace && am->rx_trace->enabled) /* trace_it */, 0 /* do_it */, 0 /* free_it */);
}
...
void vl_msg_api_cleanup_handler(void *the_msg)
{
    ...
    if (m->cleanup_handler) m->cleanup_handler(the_msg);
...
void vl_msg_api_replay_handler(void *the_msg)
{
    ...
    if (m->handler) m->handler(the_msg);
...
void vl_msg_api_config(vl_msg_api_msg_config_t *c)
{
    ...
    if (!am->msg_id_by_name) am->msg_id_by_name = hash_create_string(0, sizeof(uword));
...
void vl_msg_api_post_mortem_dump(void)
{
    ...
    if (post_mortem_dump_enabled == 0) return;
...
u16 vl_msg_api_get_msg_ids(const char *name, int n)
{
    ...
    if (am->msg_range_by_name == 0) am->msg_range_by_name = hash_create_string(0, sizeof(uword));

    name_copy = format(0, "%s%c", name, 0);

    p = hash_get_mem(am->msg_range_by_name, name_copy);
    if (p) {
        clib_warning("WARNING: duplicate message range registration for '%s'", name_copy);
        vec_free(name_copy);
        return ((u16) ~0);
    }

    if (n < 0 || n > 1024) {
        clib_warning("WARNING: bad number of message-IDs (%d) requested by '%s'", n, name_copy);
        vec_free(name_copy);
        return ((u16) ~0);
    }
...
void vl_msg_api_add_msg_name_crc(api_main_t *am, const char *string, u32 id)
{
    ...
    if (am->msg_index_by_name_and_crc == 0) am->msg_index_by_name_and_crc = hash_create_string(0, sizeof(uword));
...
int vl_api_c_string_to_api_string(const char *buf, vl_api_string_t *str)
{
    ...
    u32 len = strlen(buf);
    if (len > 0) clib_memcpy_fast(str->buf, buf, len);
...
u8 *vl_api_from_api_to_new_vec(void *mp, vl_api_string_t *astr)
{
    ...
    if (vl_msg_api_max_length(mp) < clib_net_to_host_u32(astr->length))
        return format(0, "Invalid astr->length %u > max (%u)%c", clib_net_to_host_u32(astr->length), vl_msg_api_max_length(mp), 0);
