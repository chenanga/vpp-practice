        ...
        for (i = 0; i < vec_len(nodes); i++) {
            ...
            if (n->type == VLIB_NODE_TYPE_PROCESS) {
                ...
                switch (p->flags & (VLIB_PROCESS_IS_SUSPENDED_WAITING_FOR_CLOCK | VLIB_PROCESS_IS_SUSPENDED_WAITING_FOR_EVENT)) {
                    default:
                        if (!(p->flags & VLIB_PROCESS_IS_RUNNING)) state_code = STATE_DONE;
                        break;
            ...
            if (include_nexts) {
                serialize_likely_small_unsigned_integer(sm, vec_len(n->next_nodes));
...
vlib_node_t ***vlib_node_unserialize(u8 *vector)
{
    ...
    for (i = 0; i < nstat_vms; i++) {
        ...
        for (j = 0; j < nnodes; j++) {
            ...
            if (nnexts > 0) vec_validate(node->next_nodes, nnexts - 1);
            for (k = 0; k < nnexts; k++)
                node->next_nodes[k] = unserialize_likely_small_unsigned_integer(sm);
...
static clib_error_t *test_node_serialize_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    vector = vlib_node_serialize(nm, vector, max_threads, include_nexts, include_stats);
    ...
    for (i = 0; i < vec_len(nodes_by_thread); i++) {
        ...
        for (j = 0; j < vec_len(nodes); j++) {
            ...
            vlib_cli_output(vm, "[%d] %s state %s", j, node->name, node->state_string);
