...
#define REPLY_MACRO(msg)                                                           \
    do {                                                                           \
        STATIC_ASSERT(msg##_IS_CONSTANT_SIZE,                                      \
...
#define REPLY_MACRO2(t, body)                                                       \
    do {                                                                            \
        STATIC_ASSERT(t##_IS_CONSTANT_SIZE,                                         \
...
#define REPLY_AND_DETAILS_MACRO(t, p, body)                                 \
    do {                                                                    \
        ...
        if (pool_is_free_index(p, cursor)) {                                \
            ...
            if (cursor == ~0) rv = VNET_API_ERROR_INVALID_VALUE;            \
        }                                                                   \
        while (cursor != ~0) {                                              \
            do {                                                            \
                body;                                                       \
            ...
            if (vl_api_process_may_suspend(vm, rp, start)) {                \
                if (cursor != ~0) rv = VNET_API_ERROR_EAGAIN;               \
                break;                                                      \
            }                                                               \
        }                                                                   \
...
#define REPLY_AND_DETAILS_VEC_MACRO(t, v, mp, rmp, rv, body)                \
    do {                                                                    \
        ...
        while (cursor != ~0 && cursor < vec_len(v)) {                       \
            ...
            ++cursor;                                                       \
            if (vl_api_process_may_suspend(vm, rp, start)) {                \
                if (cursor < vec_len(v)) rv = VNET_API_ERROR_EAGAIN;        \
                break;                                                      \
            }                                                               \
        }                                                                   \
        REPLY_MACRO2(t, ({ rmp->cursor = clib_host_to_net_u32(cursor); })); \
...
/* "trust, but verify" */
#define vnet_sw_if_index_is_api_valid(sw_if_index) vnet_sw_interface_is_api_valid(vnet_get_main(), sw_if_index)
...
#define pub_sub_handler(lca, UCA)                                                                \
    ...
    {                                                                                            \
        ...
        hash_set(vam->lca##_registration_hash, rp->client_index, rp - vam->lca##_registrations); \
                                                                                                 \
    reply:                                                                                       \
