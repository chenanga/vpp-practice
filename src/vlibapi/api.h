...
#define _VL_MSG_API_FUNCTION_SYMBOL(x, type) _vl_msg_api_##type##_function_##x
...
#define VLIB_DECLARE_REAPER_FUNCTION(x, tag)                                                                  \
    ...
    static void                 __vl_msg_api_add_##tag##_function_##x(void) __attribute__((__constructor__)); \
                                                                                                              \
    static void __vl_msg_api_add_##tag##_function_##x(void)                                                   \
    {                                                                                                         \
        api_main_t                            *am = vlibapi_get_main();                                       \
        static _vl_msg_api_function_list_elt_t _vl_msg_api_function;                                          \
        _vl_msg_api_function.next_init_function = am->tag##_function_registrations;                           \
...
static inline u32 vl_msg_api_get_msg_length_inline(void *msg_arg)
{
    u8 *msg = (u8 *) msg_arg;
...
always_inline void vl_api_increase_msg_trace_size(api_main_t *am, u32 msg_id, u32 inc)
{
    am->msg_data[msg_id].trace_size += inc;
}

always_inline void vl_api_set_msg_thread_safe(api_main_t *am, u32 msg_id, int v)
{
    am->msg_data[msg_id].is_mp_safe = v != 0;
}

always_inline void vl_api_set_msg_autoendian(api_main_t *am, u32 msg_id, int v)
{
    am->msg_data[msg_id].is_autoendian = v != 0;
}

always_inline void vl_api_allow_msg_replay(api_main_t *am, u32 msg_id, int v)
{
