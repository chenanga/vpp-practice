...
#define VCFG_DBG(_lvl, _fmt, _args...)                              \
    {                                                               \
        if (vcm->debug > _lvl) fprintf(stderr, _fmt "\n", ##_args); \
    }
void vppcom_cfg_heapsize(char *conf_fname)
{
    ...
    if (fp == NULL) {
        VCFG_DBG(0, "VCL<%d>: using default heapsize %lu (0x%lx)", getpid(), (unsigned long) vcl_cfg->heapsize, (unsigned long) vcl_cfg->heapsize);
        goto defaulted;
    }
    ...
    while (1) {
        if (fgets(inbuf, 4096, fp) == 0) break;
        p = strtok(inbuf, " \t\n");
        while (p != NULL) {
            if (*p == '#') break;
            argc++;
...
defaulted:
    ...
    vcl_mem = mmap(0, vcl_cfg->heapsize, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    ...
    VCFG_DBG(0,
             "VCL<%d>: allocated VCL heap = %p, size %lu (0x%lx)",
             getpid(),
             heap,
             (unsigned long) vcl_cfg->heapsize,
             (unsigned long) vcl_cfg->heapsize);
...
void vppcom_cfg_read_file(char *conf_fname)
{
    vppcom_cfg_t    *vcl_cfg = &vcm->cfg;
    int              fd;
    ...
    if (fstat(fd, &s) < 0) {
        VCFG_DBG(0, "VCL<%d>: failed to stat `%s' using default configuration", getpid(), conf_fname);
        goto file_done;
    }
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        ...
        if (vc_cfg_input) {
            if (unformat(line_input, "heapsize %U", unformat_memory_size, &vcl_cfg->heapsize)) {
                VCFG_DBG(0, "VCL<%d>: configured heapsize %lu", getpid(), (unsigned long) vcl_cfg->heapsize);
            }
                ...
                VCFG_DBG(0, "VCL<%d>: configured api-socket-name (%s)", getpid(), vcl_cfg->vpp_bapi_socket_name);
            }
            else if (unformat(line_input, "app-socket-api %s", &vcl_cfg->vpp_app_socket_api)) {
                vec_terminate_c_string(vcl_cfg->vpp_app_socket_api);
                VCFG_DBG(0, "VCL<%d>: configured app-socket-api (%s)", getpid(), vcl_cfg->vpp_app_socket_api);
            }
                ...
                VCFG_DBG(0, "VCL<%d>: configured segment_size 0x%lx (%lu)", getpid(), vcl_cfg->segment_size, vcl_cfg->segment_size);
            }
            else if (unformat(line_input, "segment-size %lu", &vcl_cfg->segment_size)) {
                VCFG_DBG(0, "VCL<%d>: configured segment_size %lu (0x%lx)", getpid(), vcl_cfg->segment_size, vcl_cfg->segment_size);
            }
            else if (unformat(line_input, "add-segment-size 0x%lx", &vcl_cfg->add_segment_size)) {
                VCFG_DBG(0, "VCL<%d>: configured add_segment_size 0x%lx (%lu)", getpid(), vcl_cfg->add_segment_size, vcl_cfg->add_segment_size);
            }
            else if (unformat(line_input, "add-segment-size %lu", &vcl_cfg->add_segment_size)) {
                VCFG_DBG(0, "VCL<%d>: configured add_segment_size %lu (0x%lx)", getpid(), vcl_cfg->add_segment_size, vcl_cfg->add_segment_size);
            }
            else if (unformat(line_input, "preallocated-fifo-pairs %u", &vcl_cfg->preallocated_fifo_pairs)) {
                VCFG_DBG(0,
                ...
                VCFG_DBG(0, "VCL<%d>: configured rx_fifo_size 0x%x (%u)", getpid(), vcl_cfg->rx_fifo_size, vcl_cfg->rx_fifo_size);
            }
            else if (unformat(line_input, "rx-fifo-size %u", &vcl_cfg->rx_fifo_size)) {
                VCFG_DBG(0, "VCL<%d>: configured rx_fifo_size %u (0x%x)", getpid(), vcl_cfg->rx_fifo_size, vcl_cfg->rx_fifo_size);
            }
            else if (unformat(line_input, "tx-fifo-size 0x%x", &vcl_cfg->tx_fifo_size)) {
                VCFG_DBG(0, "VCL<%d>: configured tx_fifo_size 0x%x (%u)", getpid(), vcl_cfg->tx_fifo_size, vcl_cfg->tx_fifo_size);
            }
            else if (unformat(line_input, "tx-fifo-size %u", &vcl_cfg->tx_fifo_size)) {
                VCFG_DBG(0, "VCL<%d>: configured tx_fifo_size %u (0x%x)", getpid(), vcl_cfg->tx_fifo_size, vcl_cfg->tx_fifo_size);
            }
            else if (unformat(line_input, "event-queue-size 0x%x", &vcl_cfg->event_queue_size)) {
                VCFG_DBG(0, "VCL<%d>: configured event_queue_size 0x%x (%u)", getpid(), vcl_cfg->event_queue_size, vcl_cfg->event_queue_size);
            }
            else if (unformat(line_input, "event-queue-size %u", &vcl_cfg->event_queue_size)) {
                VCFG_DBG(0, "VCL<%d>: configured event_queue_size %u (0x%x)", getpid(), vcl_cfg->event_queue_size, vcl_cfg->event_queue_size);
            }
                ...
                VCFG_DBG(0, "VCL<%d>: configured session_timeout %f", getpid(), vcl_cfg->session_timeout);
            }
                ...
                VCFG_DBG(0, "VCL<%d>: configured app_proxy_transport_udp (%d)", getpid(), vcl_cfg->app_proxy_transport_udp);
            }
            else if (unformat(line_input, "app-scope-local")) {
                vcl_cfg->app_scope_local = 1;
                VCFG_DBG(0, "VCL<%d>: configured app_scope_local (%d)", getpid(), vcl_cfg->app_scope_local);
            }
            else if (unformat(line_input, "app-scope-global")) {
                vcl_cfg->app_scope_global = 1;
                VCFG_DBG(0, "VCL<%d>: configured app_scope_global (%d)", getpid(), vcl_cfg->app_scope_global);
            }
                ...
                VCFG_DBG(0, "VCL<%d>: configured namespace_id %s", getpid(), (char *) vcl_cfg->namespace_id);
            }
                ...
                VCFG_DBG(0, "VCL<%d>: configured tls-engine %u (0x%x)", getpid(), vcl_cfg->tls_engine, vcl_cfg->tls_engine);
            }
                ...
                if (line_input->buffer[line_input->index] != '#') {
                    clib_warning("VCL<%d>: Unknown vppcom config option: '%s'", getpid(), (char *) &line_input->buffer[line_input->index]);
                }
            }
            unformat_free(line_input);
        }
...
void vppcom_cfg(vppcom_cfg_t *vcl_cfg)
{
    ...
    if (env_var_str) {
        ...
        if (sscanf(env_var_str, "%u", &tmp) != 1) {
            ...
            vcm->debug = tmp;
            VCFG_DBG(0, "VCL<%d>: configured VCL debug level (%u) from " VPPCOM_ENV_DEBUG "!", getpid(), vcm->debug);
        }
    }
    conf_fname = getenv(VPPCOM_ENV_CONF);
    if (!conf_fname) conf_fname = VPPCOM_CONF_DEFAULT;
    ...
    if (env_var_str) {
        ...
        VCFG_DBG(0, "VCL<%d>: configured namespace_id (%s) from " VPPCOM_ENV_APP_NAMESPACE_ID "!", getpid(), env_var_str);
    }
    ...
    if (getenv(VPPCOM_ENV_APP_SCOPE_LOCAL)) {
        ...
        VCFG_DBG(0, "VCL<%d>: configured app_scope_local (%u) from " VPPCOM_ENV_APP_SCOPE_LOCAL "!", getpid(), vcm->cfg.app_scope_local);
    }
    if (getenv(VPPCOM_ENV_APP_SCOPE_GLOBAL)) {
        vcm->cfg.app_scope_global = 1;
        VCFG_DBG(0, "VCL<%d>: configured app_scope_global (%u) from " VPPCOM_ENV_APP_SCOPE_GLOBAL "!", getpid(), vcm->cfg.app_scope_global);
    }
