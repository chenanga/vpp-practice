...
STATIC_ASSERT(sizeof(clib_bitmap_t) == sizeof(fd_mask), "ldp bitmap size mismatch");
STATIC_ASSERT(sizeof(vcl_si_set) == sizeof(fd_mask), "ldp bitmap size mismatch");
...
static inline ldp_worker_ctx_t *ldp_worker_get_current(void)
{
 ...
 * */
static inline void ldp_set_app_name(char *app_name)
{
    snprintf(ldp->app_name, LDP_APP_NAME_MAX, "%s-ldp-%d", app_name, getpid());
}

static inline char *ldp_get_app_name()
{
    if (ldp->app_name[0] == '\0') ldp_set_app_name(program_invocation_short_name);

    return ldp->app_name;
}

static inline int ldp_vlsh_to_fd(vls_handle_t vlsh)
{
    return (vlsh + ldp->vlsh_bit_val);
}

static inline vls_handle_t ldp_fd_to_vlsh(int fd)
{
    if (fd < ldp->vlsh_bit_val) return VLS_INVALID_HANDLE;

    return (fd - ldp->vlsh_bit_val);
}

static void ldp_alloc_workers(void)
{
    if (ldp->workers) return;
    ldp->workers = vec_new(ldp_worker_ctx_t, LDP_MAX_NWORKERS);
}

static void ldp_init_cfg(void)
{
    ...
    if (env_var_str) {
        ...
        if (sscanf(env_var_str, "%u", &tmp) != 1)
            clib_warning(
                "LDP<%d>: WARNING: Invalid LDP debug level specified in"
                " the env var " LDP_ENV_DEBUG " (%s)!",
                getpid(),
                env_var_str);
        else {
            ldp->debug = tmp;
            LDBG(0, "configured LDP debug level (%u) from env var " LDP_ENV_DEBUG "!", ldp->debug);
        }
    }
    ...
    if (env_var_str) {
        ...
        if (sscanf(env_var_str, "%u", &sb) != 1) {
            LDBG(0,
                 "WARNING: Invalid LDP sid bit specified in the env var " LDP_ENV_SID_BIT " (%s)! sid bit value %d (0x%x)",
                 env_var_str,
                 ldp->vlsh_bit_val,
                 ldp->vlsh_bit_val);
        }
            ...
            LDBG(0,
                 "WARNING: LDP sid bit (%u) specified in the env var " LDP_ENV_SID_BIT
                 " (%s) is too small. Using LDP_SID_BIT_MIN"
                 " (%d)! sid bit value %d (0x%x)",
                 sb,
                 env_var_str,
                 LDP_SID_BIT_MIN,
                 ldp->vlsh_bit_val,
            ...
            LDBG(0,
                 "WARNING: LDP sid bit (%u) specified in the env var " LDP_ENV_SID_BIT
                 " (%s) is too big. Using LDP_SID_BIT_MAX"
                 " (%d)! sid bit value %d (0x%x)",
                 sb,
                 env_var_str,
                 LDP_SID_BIT_MAX,
            ...
            LDBG(0, "configured LDP sid bit (%u) from " LDP_ENV_SID_BIT "!  sid bit value %d (0x%x)", sb, ldp->vlsh_bit_val, ldp->vlsh_bit_val);
        }
...
static int fcntl_internal(int fd, int cmd, va_list ap)
{
    vls_handle_t vlsh;
    int          rv = 0;
...
always_inline void ldp_select_init_maps(fd_set *__restrict original,
                                        clib_bitmap_t **resultb,
                                        clib_bitmap_t **libcb,
                                        clib_bitmap_t **vclb,
                                        int             nfds,
                                        u32             minbits,
...
always_inline int ldp_select_vcl_map_to_libc(clib_bitmap_t *vclb, fd_set *__restrict libcb)
{
    vls_handle_t vlsh;
    uword        si;
    int          fd;
...
always_inline void ldp_select_libc_map_merge(clib_bitmap_t *result, fd_set *__restrict libcb)
{
    uword fd;
                ...
                const __sigset_t *__restrict sigmask)
{
    ...
    if (PREDICT_FALSE(ldpw->clib_time.init_cpu_time == 0)) clib_time_init(&ldpw->clib_time);

    if (timeout) {
        time_out = (timeout->tv_sec == 0 && timeout->tv_nsec == 0) ? (f64) 0 : (f64) timeout->tv_sec + (f64) timeout->tv_nsec / (f64) 1e9;
    ...
    if (nfds <= ldp->vlsh_bit_val) {
        rv = libc_pselect(nfds, readfds, writefds, exceptfds, timeout, sigmask);
        goto done;
    }
    ...
    if (readfds)
        ldp_select_init_maps(readfds, &ldpw->rd_bitmap, &ldpw->libc_rd_bitmap, &ldpw->si_rd_bitmap, nfds, minbits, n_bytes, &si_bits, &libc_bits);
    if (writefds)
        ldp_select_init_maps(writefds, &ldpw->wr_bitmap, &ldpw->libc_wr_bitmap, &ldpw->si_wr_bitmap, nfds, minbits, n_bytes, &si_bits, &libc_bits);
    if (exceptfds)
        ldp_select_init_maps(exceptfds, &ldpw->ex_bitmap, &ldpw->libc_ex_bitmap, &ldpw->si_ex_bitmap, nfds, minbits, n_bytes, &si_bits, &libc_bits);
    ...
    if (!si_bits) libc_tspec = timeout ? *timeout : libc_tspec;

    do {
        if (si_bits) {
            if (readfds) clib_memcpy_fast(ldpw->rd_bitmap, ldpw->si_rd_bitmap, vec_len(ldpw->si_rd_bitmap) * sizeof(clib_bitmap_t));
            if (writefds) clib_memcpy_fast(ldpw->wr_bitmap, ldpw->si_wr_bitmap, vec_len(ldpw->si_wr_bitmap) * sizeof(clib_bitmap_t));
            if (exceptfds) clib_memcpy_fast(ldpw->ex_bitmap, ldpw->si_ex_bitmap, vec_len(ldpw->si_ex_bitmap) * sizeof(clib_bitmap_t));

            rv = vls_select(si_bits,
        ...
        if (libc_bits) {
            if (readfds) clib_memcpy_fast(ldpw->rd_bitmap, ldpw->libc_rd_bitmap, vec_len(ldpw->libc_rd_bitmap) * sizeof(clib_bitmap_t));
            if (writefds) clib_memcpy_fast(ldpw->wr_bitmap, ldpw->libc_wr_bitmap, vec_len(ldpw->libc_wr_bitmap) * sizeof(clib_bitmap_t));
            if (exceptfds) clib_memcpy_fast(ldpw->ex_bitmap, ldpw->libc_ex_bitmap, vec_len(ldpw->libc_ex_bitmap) * sizeof(clib_bitmap_t));
...
int select(int nfds, fd_set *__restrict readfds, fd_set *__restrict writefds, fd_set *__restrict exceptfds, struct timeval *__restrict timeout)
{
    ...
    return ldp_pselect(nfds, readfds, writefds, exceptfds, timeout ? &tspec : NULL, NULL);
}
 ...
 */
static int load_cert_key_pair(void)
{
...
static int assign_cert_key_pair(vls_handle_t vlsh)
{
    uint32_t ckp_len;

    if (ldp->ckpair_index == ~0 && load_cert_key_pair() < 0) return -1;
...
int socket(int domain, int type, int protocol)
{
    ...
    if (((domain == AF_INET) || (domain == AF_INET6)) && ((sock_type == SOCK_STREAM) || (sock_type == SOCK_DGRAM))) {
        u8 proto;
        ...
        LDBG(0, "calling vls_create: proto %u (%s), is_nonblocking %u", proto, vppcom_proto_str(proto), is_nonblocking);
        ...
        if (vlsh < 0) {
            ...
            if (ldp->transparent_tls) {
                if (assign_cert_key_pair(vlsh) < 0) return -1;
            }
...
int bind(int fd, __CONST_SOCKADDR_ARG _addr, socklen_t len)
{
        ...
        switch (addr->sa_family) {
            case AF_INET:
                if (len != sizeof(struct sockaddr_in)) {
                    LDBG(0, "ERROR: fd %d: vlsh %u: Invalid AF_INET addr len %u!", fd, vlsh, len);
                    errno = EINVAL;
                    rv    = -1;
                    goto done;
                }
            ...
            case AF_INET6:
                if (len != sizeof(struct sockaddr_in6)) {
                    LDBG(0, "ERROR: fd %d: vlsh %u: Invalid AF_INET6 addr len %u!", fd, vlsh, len);
                    errno = EINVAL;
                    rv    = -1;
                    goto done;
                }
            ...
            default:
                LDBG(0, "ERROR: fd %d: vlsh %u: Unsupported address family %u!", fd, vlsh, addr->sa_family);
                errno = EAFNOSUPPORT;
                rv    = -1;
                goto done;
        }
        LDBG(0, "fd %d: calling vls_bind: vlsh %u, addr %p, len %u", fd, vlsh, addr, len);
...
static inline int ldp_copy_ep_to_sockaddr(struct sockaddr *addr, socklen_t *__restrict len, vppcom_endpt_t *ep)
{
    ...
    if (addr && len && ep) {
        ...
        switch (addr->sa_family) {
            case AF_INET:
                ...
                copy_len = *len - sa_len;
                if (copy_len > 0) memcpy(&((struct sockaddr_in *) addr)->sin_addr, ep->ip, copy_len);
                break;
            ...
            case AF_INET6:
                ...
                copy_len = *len - sa_len;
                if (copy_len > 0) memcpy(((struct sockaddr_in6 *) addr)->sin6_addr.__in6_u.__u6_addr8, ep->ip, copy_len);
                break;
...
int getsockname(int fd, __SOCKADDR_ARG _addr, socklen_t *__restrict len)
{
...
int connect(int fd, __CONST_SOCKADDR_ARG _addr, socklen_t len)
{
    const struct sockaddr *addr = SOCKADDR_GET_SA(_addr);
    vls_handle_t           vlsh;
    int                    rv;
    ...
    if (vlsh != VLS_INVALID_HANDLE) {
        ...
        switch (addr->sa_family) {
            case AF_INET:
                if (len != sizeof(struct sockaddr_in)) {
                    LDBG(0, "fd %d: ERROR vlsh %u: Invalid AF_INET addr len %u!", fd, vlsh, len);
                    errno = EINVAL;
                    rv    = -1;
                    goto done;
                }
            ...
            case AF_INET6:
                if (len != sizeof(struct sockaddr_in6)) {
                    LDBG(0, "fd %d: ERROR vlsh %u: Invalid AF_INET6 addr len %u!", fd, vlsh, len);
                    errno = EINVAL;
                    rv    = -1;
                    goto done;
                }
            ...
            default:
                LDBG(0, "fd %d: ERROR vlsh %u: Unsupported address family %u!", fd, vlsh, addr->sa_family);
                errno = EAFNOSUPPORT;
                rv    = -1;
                goto done;
        }
        LDBG(0, "fd %d: calling vls_connect(): vlsh %u addr %p len %u", fd, vlsh, addr, len);
        ...
        LDBG(0, "fd %d: calling libc_connect(): addr %p, len %u", fd, addr, len);
...
int getpeername(int fd, __SOCKADDR_ARG _addr, socklen_t *__restrict len)
{
    struct sockaddr *addr = SOCKADDR_GET_SA(_addr);
    vls_handle_t     vlsh;
    int              rv;
...
ssize_t sendfile(int out_fd, int in_fd, off_t *offset, size_t len)
{
    ...
    if (vlsh != VLS_INVALID_HANDLE) {
        ...
        if (PREDICT_FALSE(rv != VPPCOM_OK)) {
            LDBG(0, "ERROR: out fd %d: vls_attr: vlsh %u, returned %d (%s)!", out_fd, vlsh, rv, vppcom_retval_str(rv));
        ...
        do {
            ...
            if (size < 0) {
                LDBG(0, "ERROR: fd %d: vls_attr: vlsh %u returned %ld (%s)!", out_fd, vlsh, size, vppcom_retval_str(size));
                vec_reset_length(ldpw->io_buffer);
                errno = -size;
                size  = -1;
                goto done;
...
ssize_t sendfile64(int out_fd, int in_fd, off_t *offset, size_t len)
{
...
static inline int
ldp_vls_sendo(vls_handle_t vlsh, const void *buf, size_t n, vppcom_endpt_tlv_t *app_tlvs, int flags, __CONST_SOCKADDR_ARG _addr, socklen_t addr_len)
{
    const struct sockaddr *addr = SOCKADDR_GET_SA(_addr);
    vppcom_endpt_t        *ep   = 0;
    vppcom_endpt_t         _ep;
...
static int ldp_vls_recvfrom(vls_handle_t vlsh, void *__restrict buf, size_t n, int flags, __SOCKADDR_ARG _addr, socklen_t *__restrict addr_len)
{
...
ssize_t sendto(int fd, const void *buf, size_t n, int flags, __CONST_SOCKADDR_ARG _addr, socklen_t addr_len)
{
    const struct sockaddr *addr = SOCKADDR_GET_SA(_addr);
    vls_handle_t           vlsh;
    ssize_t                size;
...
static int ldp_parse_cmsg(vls_handle_t vlsh, const struct msghdr *msg, vppcom_endpt_tlv_t **app_tlvs)
{
            ...
            case SOL_IP:
                switch (cmsg->cmsg_type) {
                    case IP_PKTINFO:
                        ...
                        clib_memcpy_fast(ad, &pi->ipi_spec_dst, sizeof(struct in_addr));
                        break;
                    default:
...
static int ldp_make_cmsg(vls_handle_t vlsh, struct msghdr *msg)
{
    ...
    if (!vls_attr(vlsh, VPPCOM_ATTR_GET_IP_PKTINFO, (void *) &optval, &optlen)) return 0;
...
ssize_t sendmsg(int fd, const struct msghdr *msg, int flags)
{
    ...
    if (vlsh != VLS_INVALID_HANDLE) {
        ...
        for (i = 0; i < msg->msg_iovlen; ++i) {
            rv = ldp_vls_sendo(vlsh, iov[i].iov_base, iov[i].iov_len, app_tlvs, flags, msg->msg_name, msg->msg_namelen);
            if (rv < 0)
                break;
            else {
                total += rv;
...
int sendmmsg(int fd, struct mmsghdr *vmessages, unsigned int vlen, int flags)
{
    ...
    if (LDP_DEBUG > 2) {
        if (size < 0) {
            int errno_val = errno;
            clib_warning(
                "LDP<%d>: ERROR: fd %d (0x%x): %s() failed! "
                "rv %d, errno = %d",
                getpid(),
                fd,
                fd,
            ...
            clib_warning("LDP<%d>: fd %d (0x%x): returning %d (0x%x)", getpid(), fd, fd, size, size);
    }
    return size;
}
#endif
...
ssize_t recvmsg(int fd, struct msghdr *msg, int flags)
{
    ...
    if (vlsh != VLS_INVALID_HANDLE) {
        ...
        for (i = 0; i < msg->msg_iovlen; i++) {
            rv = ldp_vls_recvfrom(vlsh, iov[i].iov_base, iov[i].iov_len, flags, (i == 0 ? msg->msg_name : NULL), (i == 0 ? &msg->msg_namelen : NULL));
            if (rv <= 0)
                break;
            else {
                total += rv;
        ...
        if (rv < 0 && total == 0) {
            ...
            if (msg->msg_controllen) ldp_make_cmsg(vlsh, msg);
            size = total;
        }
    }
    else {
...
#ifdef _GNU_SOURCE
int recvmmsg(int fd, struct mmsghdr *vmessages, unsigned int vlen, int flags, struct timespec *tmo)
{
    ldp_worker_ctx_t *ldpw = ldp_worker_get_current();
    u32               sh;
    ...
    if (sh != VLS_INVALID_HANDLE) {
        ...
        while (nvecs < vlen) {
            ...
            if (!time_out || clib_time_now(&ldpw->clib_time) >= time_out) break;
        ...
        switch (level) {
            case SOL_TCP:
                switch (optname) {
                    ...
                    case TCP_MAXSEG:
                        ...
                        break;
                    case TCP_KEEPIDLE:
                        rv = vls_attr(vlsh, VPPCOM_ATTR_GET_TCP_KEEPIDLE, optval, optlen);
                        break;
                    case TCP_KEEPINTVL:
                    ...
                    default:
                        LDBG(0,
                             "ERROR: fd %d: getsockopt SOL_TCP: sid %u, "
            ...
            case SOL_IP:
                switch (optname) {
                    case SO_ORIGINAL_DST:
                        rv = vls_attr(vlsh, VPPCOM_ATTR_GET_ORIGINAL_DST, optval, optlen);
                        break;
                    default:
                        LDBG(0,
            ...
            case SOL_IPV6:
                switch (optname) {
                    ...
                    default:
                        LDBG(0,
                             "ERROR: fd %d: getsockopt SOL_IPV6: vlsh %u "
                             "optname %d unsupported!",
                             fd,
                             vlsh,
                             optname);
                ...
                switch (optname) {
                    ...
                    case SO_PROTOCOL:
                        ...
                        *(int *) optval = *(int *) optval ? SOCK_DGRAM : SOCK_STREAM;
                        break;
                    case SO_SNDBUF:
                        rv = vls_attr(vlsh, VPPCOM_ATTR_GET_TX_FIFO_LEN, optval, optlen);
                        break;
                    case SO_RCVBUF:
                    ...
                    default:
                        LDBG(0,
                             "ERROR: fd %d: getsockopt SOL_SOCKET: vlsh %u "
                             "optname %d unsupported!",
                             fd,
                             vlsh,
                             optname);
        ...
        switch (level) {
            case SOL_TCP:
                switch (optname) {
                    case TCP_NODELAY:
                        ...
                        break;
                    case TCP_MAXSEG:
                        rv = vls_attr(vlsh, VPPCOM_ATTR_SET_TCP_USER_MSS, (void *) optval, &optlen);
                        break;
                    case TCP_KEEPIDLE:
                    ...
                    default:
                        LDBG(0,
                             "ERROR: fd %d: setsockopt() SOL_TCP: vlsh %u"
                             "optname %d unsupported!",
                             fd,
                             vlsh,
                             optname);
            ...
            case SOL_IPV6:
                switch (optname) {
                    ...
                    default:
                        LDBG(0,
                             "ERROR: fd %d: setsockopt SOL_IPV6: vlsh %u"
                             "optname %d unsupported!",
                             fd,
                             vlsh,
                             optname);
            ...
            case SOL_SOCKET:
                switch (optname) {
                    ...
                    case SO_REUSEADDR:
                        ...
                        break;
                    case SO_REUSEPORT:
                        rv = vls_attr(vlsh, VPPCOM_ATTR_SET_REUSEPORT, (void *) optval, &optlen);
                        break;
                    case SO_BROADCAST:
                    ...
                    default:
                        LDBG(0,
                             "ERROR: fd %d: setsockopt SOL_SOCKET: vlsh %u "
                             "optname %d unsupported!",
                             fd,
                             vlsh,
                             optname);
            ...
            case SOL_IP:
                switch (optname) {
                    case IP_PKTINFO:
                        rv = vls_attr(vlsh, VPPCOM_ATTR_SET_IP_PKTINFO, (void *) optval, &optlen);
                        break;
                    default:
                        LDBG(0,
...
static inline int ldp_accept4(int listen_fd, __SOCKADDR_ARG _addr, socklen_t *__restrict addr_len, int flags)
{
    struct sockaddr *addr = SOCKADDR_GET_SA(_addr);
    ...
    if (listen_vlsh != VLS_INVALID_HANDLE) {
        ...
        LDBG(0,
             "listen fd %d: calling vppcom_session_accept: listen sid %u,"
             " ep %p, flags 0x%x",
             listen_fd,
             listen_vlsh,
             &ep,
        ...
        LDBG(0,
             "listen fd %d: calling libc_accept4(): addr %p, addr_len %p,"
             " flags 0x%x",
             listen_fd,
             addr,
             addr_len,
...
int epoll_ctl(int epfd, int op, int fd, struct epoll_event *event)
{
    ...
    LDBG(0, "epfd %d ep_vlsh %d, fd %u vlsh %d, op %u", epfd, vep_vlsh, fd, vlsh, op);
    ...
    if (vlsh != VLS_INVALID_HANDLE) {
        ...
        if (!libc_epfd) {
            LDBG(1,
                 "epfd %d, vep_vlsh %d calling libc_epoll_create1: "
                 "EPOLL_CLOEXEC",
                 epfd,
                 vep_vlsh);
            ...
            rv = vls_attr(vep_vlsh, VPPCOM_ATTR_SET_LIBC_EPFD, &libc_epfd, &size);
            if (rv < 0) {
                errno = -rv;
                rv    = -1;
                goto done;
        ...
        LDBG(1,
             "epfd %d: calling libc_epoll_ctl: libc_epfd %d, op %d, fd %d,"
             " event %p",
             epfd,
             libc_epfd,
             op,
...
static inline int ldp_epoll_pwait(int epfd, struct epoll_event *events, int maxevents, int timeout, const sigset_t *sigmask)
{
    ldp_worker_ctx_t *ldpw;
    ...
    if (PREDICT_FALSE(vppcom_worker_index() == ~0)) vls_register_vcl_worker();

    ldpw = ldp_worker_get_current();
    if (epfd == ldpw->vcl_mq_epfd) return libc_epoll_pwait(epfd, events, maxevents, timeout, sigmask);
    ...
    LDBG(2,
         "epfd %d: vep_idx %d, libc_epfd %d, events %p, maxevents %d, "
...
static inline int ldp_epoll_pwait_eventfd(int epfd, struct epoll_event *events, int maxevents, int timeout, const sigset_t *sigmask)
{
    ...
    if (PREDICT_FALSE(!ldpw->mq_epfd_added)) {
        ...
        if (libc_epoll_ctl(libc_epfd, EPOLL_CTL_ADD, ldpw->vcl_mq_epfd, &e) < 0) {
            LDBG(0, "epfd %d, add libc mq epoll fd %d to libc epoll fd %d", epfd, ldpw->vcl_mq_epfd, libc_epfd);
            rv = -1;
            goto done;
        }
...
epoll_again:
    ...
    libc_evts   = &events[rv];
    libc_num_ev = libc_epoll_pwait(libc_epfd, libc_evts, maxevents, timeout, sigmask);
    if (libc_num_ev <= 0) {
        rv = rv >= 0 ? rv : -1;
        goto done;
    }
    ...
    for (int i = 0; i < libc_num_ev; i++) {
        if (libc_evts[i].data.fd == ldpw->vcl_mq_epfd) {
            ...
            num_ev = vls_epoll_wait(ep_vlsh, &libc_evts[libc_num_ev], maxevents - libc_num_ev, 0);
            if (PREDICT_TRUE(num_ev > 0)) rv += num_ev;
            /* Woken up by vcl but no events generated. Accept it once */
            if (rv == 0 && libc_num_ev == 0 && timeout && vcl_wups++ < 1) goto epoll_again;
            break;
        }
    }
...
{
    if (vls_use_eventfd())
        return ldp_epoll_pwait_eventfd(epfd, events, maxevents, timeout, sigmask);
    else
...
#ifdef __USE_XOPEN2K
            if (fds[i].events & POLLRDNORM) vp->events |= POLLIN;
            if (fds[i].events & POLLWRNORM) vp->events |= POLLOUT;
#endif
...
#ifdef __USE_XOPEN2K
        if ((fds[vp->fds_ndx].revents & POLLIN) && (fds[vp->fds_ndx].events & POLLRDNORM)) fds[vp->fds_ndx].revents |= POLLRDNORM;
        if ((fds[vp->fds_ndx].revents & POLLOUT) && (fds[vp->fds_ndx].events & POLLWRNORM)) fds[vp->fds_ndx].revents |= POLLWRNORM;
#endif
    }
...
void ldp_destructor(void)
{
     ...
     */
    if (LDP_DEBUG > 0) fprintf(stderr, "%s:%d: LDP<%d>: LDP destructor: done!\n", __func__, __LINE__, getpid());
}
