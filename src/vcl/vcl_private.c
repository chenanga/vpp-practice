 ...
 */
static int vcl_mq_epoll_add_api_sock(vcl_worker_t *wrk)
{
...
int vcl_mq_epoll_add_evfd(vcl_worker_t *wrk, svm_msg_q_t *mq)
{
    ...
    if (wrk->mqs_epfd < 0 || mq_fd == -1) return -1;
...
int vcl_mq_epoll_del_evfd(vcl_worker_t *wrk, u32 mqc_index)
{
    ...
    if (wrk->mqs_epfd || mqc_index == ~0) return -1;
...
static vcl_worker_t *vcl_worker_alloc(void)
{
    vcl_worker_t *wrk;
...
static void vcl_worker_free(vcl_worker_t *wrk)
{
    pool_put(vcm->workers, wrk);
}

int vcl_api_app_worker_add(void)
{
    if (vcm->cfg.vpp_app_socket_api) return vcl_sapi_app_worker_add();

    return vcl_bapi_app_worker_add();
}

void vcl_api_app_worker_del(vcl_worker_t *wrk)
{
    if (wrk->api_client_handle == ~0) return;

    if (vcm->cfg.vpp_app_socket_api) return vcl_sapi_app_worker_del(wrk);
...
void vcl_worker_cleanup(vcl_worker_t *wrk, u8 notify_vpp)
{
    ...
    if (notify_vpp) vcl_api_app_worker_del(wrk);

    if (wrk->mqs_epfd > 0) close(wrk->mqs_epfd);
...
static void vcl_worker_cleanup_cb(void *arg)
{
    vcl_worker_t *wrk;
    u32           wrk_index;
...
void vcl_worker_detach_sessions(vcl_worker_t *wrk)
{
    ...
    pool_foreach (s, wrk->sessions) {
        ...
        if ((s->flags & VCL_SESSION_F_IS_VEP) || s->session_state == VCL_STATE_LISTEN_NO_MQ || s->session_state == VCL_STATE_CLOSED) continue;
    ...
    hash_foreach(seg_index, val, seg_indices_map, ({ vec_add1(seg_indices, seg_index); }));
...
int vcl_worker_register_with_vpp(void)
{
    ...
    if (pthread_key_create(&vcl_worker_stop_key, vcl_worker_cleanup_cb)) VDBG(0, "failed to add pthread cleanup function");
    if (pthread_setspecific(vcl_worker_stop_key, &wrk->thread_id)) VDBG(0, "failed to setup key value");
...
int vcl_session_read_ready(vcl_session_t *s)
{
    if (PREDICT_FALSE(s->flags & VCL_SESSION_F_IS_VEP)) {
        VDBG(0, "ERROR: session %u: cannot read from an epoll session!", s->session_index);
        return VPPCOM_EBADFD;
    }

    if (vcl_session_is_open(s)) {
        if (vcl_session_is_ct(s)) return svm_fifo_max_dequeue_cons(s->ct_rx_fifo);
        ...
        if (s->is_dgram) {
            ...
            if (max_deq <= SESSION_CONN_HDR_LEN) return 0;
...
int vcl_session_write_ready(vcl_session_t *s)
{
    if (PREDICT_FALSE(s->flags & VCL_SESSION_F_IS_VEP)) {
        VDBG(0, "session %u [0x%llx]: cannot write to an epoll session!", s->session_index, s->vpp_handle);
        return VPPCOM_EBADFD;
    }

    if (vcl_session_is_open(s)) {
        if (vcl_session_is_ct(s)) return svm_fifo_max_enqueue_prod(s->ct_tx_fifo);

        if (s->is_dgram) {
            u32 max_enq = svm_fifo_max_enqueue_prod(s->tx_fifo);

            if (max_enq <= sizeof(session_dgram_hdr_t)) return 0;
...
int vcl_segment_attach(u64 segment_handle, char *name, ssvm_segment_type_t type, int fd)
{
    ...
    if (type == SSVM_SEGMENT_MEMFD) a->memfd_fd = fd;
...
u32 vcl_segment_table_lookup(u64 segment_handle)
{
    ...
    if (!seg_indexp) return VCL_INVALID_SEGMENT_INDEX;
...
int vcl_segment_attach_session(uword segment_handle, uword rxf_offset, uword txf_offset, uword mq_offset, u32 mq_index, u8 is_ct, vcl_session_t *s)
{
    ...
    if (fs_index == VCL_INVALID_SEGMENT_INDEX) {
        VDBG(0, "ERROR: segment for session %u is not mounted!", s->session_index);
        return -1;
    }
...
void vcl_session_detach_fifos(vcl_session_t *s)
{
    ...
    fs = fifo_segment_get_segment_if_valid(&vcm->segment_main, s->rx_fifo->segment_index);
    if (!fs) goto done;

    fifo_segment_free_client_fifo(fs, s->rx_fifo);
    fifo_segment_free_client_fifo(fs, s->tx_fifo);
    if (s->ct_rx_fifo) {
        fs = fifo_segment_get_segment_if_valid(&vcm->segment_main, s->ct_rx_fifo->segment_index);
        if (!fs) goto done;
...
u8 *vcl_format_ip46_address(u8 *s, va_list *args)
{
    ...
    return is_ip4 ? format(s, "%U", vcl_format_ip4_address, &ip46->ip4) : format(s, "%U", vcl_format_ip6_address, &ip46->ip6);
}
