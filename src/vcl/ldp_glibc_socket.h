...
extern int __THROW socketpair(int __domain, int __type, int __protocol, int __fds[2]);

/* Give the socket FD the local address ADDR (which is LEN bytes long).  */
extern int __THROW bind(int __fd, __CONST_SOCKADDR_ARG __addr, socklen_t __len);

/* Put the local address of FD into *ADDR and its length in *LEN.  */
extern int __THROW getsockname(int __fd, __SOCKADDR_ARG __addr, socklen_t *__restrict __len);
...
extern int __THROW getpeername(int __fd, __SOCKADDR_ARG __addr, socklen_t *__restrict __len);
...
extern ssize_t sendfile(int __out_fd, int __in_fd, off_t *__offset, size_t __len);
...
/* Read N bytes into BUF from socket FD with buffer overflow checking.
   ...
   This function is a cancellation point and therefore not marked with
   __THROW.  */
extern ssize_t __recv_chk(int fd, void *buf, size_t n, size_t buflen, int flags);
...
extern int __THROW getsockopt(int __fd, int __level, int __optname, void *__restrict __optval, socklen_t *__restrict __optlen);

/* Set socket FD's option OPTNAME at protocol level LEVEL
   to *OPTVAL (which is OPTLEN bytes long).
   Returns 0 on success, -1 for errors.  */
extern int __THROW setsockopt(int __fd, int __level, int __optname, const void *__optval, socklen_t __optlen);
...
extern int __THROW epoll_ctl(int __epfd, int __op, int __fd, struct epoll_event *__event);
