...
void        vls_send_session_cleanup_rpc(vcl_worker_t *wrk, u32 dst_wrk_index, u32 dst_session_index);
void        vls_send_clone_and_share_rpc(vcl_worker_t *wrk,
                                         u32           origin_vls_index,
                                         u32           session_index,
                                         u32           vls_wrk_index,
                                         u32           dst_wrk_index,
                                         u32           dst_vls_index,
                                         u32           dst_session_index);
static void vls_cleanup_forked_child(vcl_worker_t *wrk, vcl_worker_t *child_wrk);
static void vls_handle_pending_wrk_cleanup(void);

static inline u32 vls_get_worker_index(void)
{
    return vlsl->vls_wrk_index;
}

static u32 vls_shared_data_alloc(void)
{
...
static u32 vls_shared_data_index(vls_shared_data_t *vls_shd)
{
    return vls_shd - vlsm->shared_data_pool;
}

vls_shared_data_t *vls_shared_data_get(u32 shd_index)
{
    if (pool_is_free_index(vlsm->shared_data_pool, shd_index)) return 0;
    return pool_elt_at_index(vlsm->shared_data_pool, shd_index);
}

static void vls_shared_data_free(u32 shd_index)
{
    vls_shared_data_t *vls_shd;
...
static inline void vls_shared_data_pool_rlock(void)
{
    clib_rwlock_reader_lock(&vlsm->shared_data_lock);
}

static inline void vls_shared_data_pool_runlock(void)
{
    clib_rwlock_reader_unlock(&vlsm->shared_data_lock);
}

static inline void vls_mt_pool_rlock(void)
{
    if (vlsl->vls_mt_n_threads > 1) clib_rwlock_reader_lock(&vlsl->vls_pool_lock);
}

static inline void vls_mt_pool_runlock(void)
{
    if (vlsl->vls_mt_n_threads > 1) clib_rwlock_reader_unlock(&vlsl->vls_pool_lock);
}

static inline void vls_mt_pool_wlock(void)
{
    if (vlsl->vls_mt_n_threads > 1) clib_rwlock_writer_lock(&vlsl->vls_pool_lock);
}

static inline void vls_mt_pool_wunlock(void)
{
    if (vlsl->vls_mt_n_threads > 1) clib_rwlock_writer_unlock(&vlsl->vls_pool_lock);
}
...
typedef enum { VLS_MT_LOCK_MQ = 1 << 0, VLS_MT_LOCK_SPOOL = 1 << 1 } vls_mt_lock_type_t;

static void vls_mt_add(void)
{
    ...
    if (vls_mt_wrk_supported()) {
        if (vppcom_worker_register() != VPPCOM_OK) VERR("failed to register worker");
    }
    else
        vcl_set_worker_index(vlsl->vls_wrk_index);
}

static inline void vls_mt_mq_lock(void)
{
    pthread_mutex_lock(&vlsl->vls_mt_mq_mlock);
}

static inline void vls_mt_mq_unlock(void)
{
    pthread_mutex_unlock(&vlsl->vls_mt_mq_mlock);
}

static inline void vls_mt_spool_lock(void)
{
    pthread_mutex_lock(&vlsl->vls_mt_spool_mlock);
}

static inline void vls_mt_create_unlock(void)
{
    pthread_mutex_unlock(&vlsl->vls_mt_spool_mlock);
}

static void vls_mt_locks_init(void)
{
    pthread_mutex_init(&vlsl->vls_mt_mq_mlock, NULL);
    pthread_mutex_init(&vlsl->vls_mt_spool_mlock, NULL);
}

u8 vls_is_shared(vcl_locked_session_t *vls)
{
...
{
    if ((vlsl->vls_mt_n_threads > 1) || vls_is_shared(vls)) clib_spinlock_lock(&vls->lock);
}

static inline void vls_unlock(vcl_locked_session_t *vls)
{
    if ((vlsl->vls_mt_n_threads > 1) || vls_is_shared(vls)) clib_spinlock_unlock(&vls->lock);
}

static inline vcl_session_handle_t vls_to_sh(vcl_locked_session_t *vls)
{
    return vcl_session_handle_from_index(vls->session_index);
}

static inline vcl_session_handle_t vls_to_sh_tu(vcl_locked_session_t *vls)
{
...
static vls_worker_t *vls_worker_get_current(void)
{
    return pool_elt_at_index(vlsm->workers, vls_get_worker_index());
}

static inline u8 vls_n_workers(void)
{
    return pool_elts(vlsm->workers);
}

static void vls_worker_alloc(void)
{
    vls_worker_t *wrk;

    pool_get_zero(vlsm->workers, wrk);
    if (vls_mt_wrk_supported()) clib_rwlock_init(&wrk->sh_to_vlsh_table_lock);
...
static void vls_worker_free(vls_worker_t *wrk)
{
    hash_free(wrk->sh_to_vlsh_table);
    if (vls_mt_wrk_supported()) clib_rwlock_free(&wrk->sh_to_vlsh_table_lock);
    pool_free(wrk->vls_pool);
    pool_put(vlsm->workers, wrk);
}

static vls_worker_t *vls_worker_get(u32 wrk_index)
{
    if (pool_is_free_index(vlsm->workers, wrk_index)) return 0;
    return pool_elt_at_index(vlsm->workers, wrk_index);
}

static void vls_sh_to_vlsh_table_add(vls_worker_t *wrk, vcl_session_handle_t sh, u32 vlsh)
{
    if (vls_mt_wrk_supported()) clib_rwlock_writer_lock(&wrk->sh_to_vlsh_table_lock);
    hash_set(wrk->sh_to_vlsh_table, sh, vlsh);
    if (vls_mt_wrk_supported()) clib_rwlock_writer_unlock(&wrk->sh_to_vlsh_table_lock);
}

static void vls_sh_to_vlsh_table_del(vls_worker_t *wrk, vcl_session_handle_t sh)
{
    if (vls_mt_wrk_supported()) clib_rwlock_writer_lock(&wrk->sh_to_vlsh_table_lock);
...
static uword *vls_sh_to_vlsh_table_get(vls_worker_t *wrk, vcl_session_handle_t sh)
{
    if (vls_mt_wrk_supported()) clib_rwlock_reader_lock(&wrk->sh_to_vlsh_table_lock);
    uword *vlshp = hash_get(wrk->sh_to_vlsh_table, sh);
    if (vls_mt_wrk_supported()) clib_rwlock_reader_unlock(&wrk->sh_to_vlsh_table_lock);
    return vlshp;
}

static vls_handle_t vls_alloc(vcl_session_handle_t sh)
{
    ...
    if (vls_mt_wrk_supported()) {
        hash_set(vls->vcl_wrk_index_to_session_index, vls->vcl_wrk_index, vls->session_index);
...
static vcl_locked_session_t *vls_get(vls_handle_t vlsh)
{
    vls_worker_t *wrk = vls_worker_get_current();
    if (pool_is_free_index(wrk->vls_pool, vlsh)) return 0;
    return pool_elt_at_index(wrk->vls_pool, vlsh);
}

static void vls_free(vcl_locked_session_t *vls)
{
    vls_worker_t *wrk = vls_worker_get_current();

    ASSERT(vls != 0);
    vls_sh_to_vlsh_table_del(wrk, vcl_session_handle_from_index(vls->session_index));
    clib_spinlock_free(&vls->lock);
    pool_put(wrk->vls_pool, vls);
}

static vcl_locked_session_t *vls_get_and_lock(vls_handle_t vlsh)
{
    vls_worker_t         *wrk = vls_worker_get_current();
    vcl_locked_session_t *vls;
    if (pool_is_free_index(wrk->vls_pool, vlsh)) return 0;
...
static vcl_locked_session_t *vls_get_w_dlock(vls_handle_t vlsh)
{
    vcl_locked_session_t *vls;
    vls_mt_pool_rlock();
    vls = vls_get_and_lock(vlsh);
    if (!vls) vls_mt_pool_runlock();
    return vls;
}

static inline void vls_get_and_unlock(vls_handle_t vlsh)
{
    vcl_locked_session_t *vls;
    vls_mt_pool_rlock();
    vls = vls_get(vlsh);
...
static inline void vls_dunlock(vcl_locked_session_t *vls)
{
    vls_unlock(vls);
    vls_mt_pool_runlock();
}

static vcl_locked_session_t *vls_session_get(vls_worker_t *wrk, u32 vls_index)
{
    if (pool_is_free_index(wrk->vls_pool, vls_index)) return 0;
...
vcl_session_handle_t vlsh_to_sh(vls_handle_t vlsh)
{
    ...
    if (!vls) return INVALID_SESSION_ID;
    rv = vls_to_sh(vls);
    vls_dunlock(vls);
    return rv;
}
...
vls_handle_t vls_si_wi_to_vlsh(u32 session_index, u32 vcl_wrk_index)
{
    vls_worker_t *wrk   = vls_worker_get_current();
    uword        *vlshp = vls_sh_to_vlsh_table_get(wrk, vcl_session_handle_from_wrk_session_index(session_index, vcl_wrk_index));
...
u8 vls_is_shared_by_wrk(vcl_locked_session_t *vls, u32 wrk_index)
{
    vls_shared_data_t *vls_shd;
    int                i;

    if (vls->shared_data_index == ~0) return 0;
...
static void vls_listener_wrk_set(vcl_locked_session_t *vls, u32 wrk_index, u8 is_active)
{
    vls_shared_data_t *vls_shd;
    ...
    clib_spinlock_lock(&vls_shd->lock);
    vls_shd->listeners = clib_bitmap_set(vls_shd->listeners, wrk_index, is_active);
    clib_spinlock_unlock(&vls_shd->lock);

    vls_shared_data_pool_runlock();
}

static u32 vls_shared_get_owner(vcl_locked_session_t *vls)
{
    vls_shared_data_t *vls_shd;
    u32                owner_wrk;
...
static u8 vls_listener_wrk_is_active(vcl_locked_session_t *vls, u32 wrk_index)
{
    vls_shared_data_t *vls_shd;
    u8                 is_set;
...
static void vls_listener_wrk_start_listen(vcl_locked_session_t *vls, u32 wrk_index)
{
    vcl_worker_t  *wrk;
    vcl_session_t *ls;
    ...
    if (ls->flags & VCL_SESSION_F_PENDING_LISTEN) return;
...
static void vls_listener_wrk_stop_listen(vcl_locked_session_t *vls, u32 wrk_index)
{
    vcl_worker_t  *wrk;
    vcl_session_t *s;
    ...
    if (s->session_state != VCL_STATE_LISTEN) return;
    vcl_send_session_unlisten(wrk, s);
    s->session_state = VCL_STATE_LISTEN_NO_MQ;
    vls_listener_wrk_set(vls, wrk_index, 0 /* is_active */);
}

static int vls_shared_data_subscriber_position(vls_shared_data_t *vls_shd, u32 wrk_index)
{
    int i;

    for (i = 0; i < vec_len(vls_shd->workers_subscribed); i++) {
        if (vls_shd->workers_subscribed[i] == wrk_index) return i;
    }
    return -1;
}

int vls_unshare_session(vcl_locked_session_t *vls, vcl_worker_t *wrk)
{
    vls_shared_data_t *vls_shd;
    ...
    s = vcl_session_get(wrk, vls->session_index);
    if (s->session_state == VCL_STATE_LISTEN) vls_listener_wrk_set(vls, wrk->wrk_index, 0 /* is_active */);
    ...
    if (pos < 0) {
        clib_warning("worker %u not subscribed for vls %u", wrk->wrk_index, vls->vcl_wrk_index);
        goto done;
    }
    ...
    if (vls_shd->owner_wrk_index != wrk->wrk_index) goto done;
    ...
    if (s->vpp_evt_q) vcl_send_session_worker_update(wrk, s, vls_shd->owner_wrk_index);

    /* XXX is this still needed? */
    if (vec_len(vls_shd->workers_subscribed) > 1) clib_warning("more workers need to be updated");
...
void vls_init_share_session(vls_worker_t *vls_wrk, vcl_locked_session_t *vls)
{
    vls_shared_data_t *vls_shd;
...
void vls_share_session(vls_worker_t *vls_wrk, vcl_locked_session_t *vls)
{
    ...
    if (!s) {
        clib_warning("wrk %u session %u vls %u NOT AVAILABLE", vcl_wrk->wrk_index, vls->session_index, vls->vls_index);
        return;
    }
...
static void vls_share_sessions(vls_worker_t *vls_parent_wrk, vls_worker_t *vls_wrk)
{
...
static void vls_validate_veps(vcl_worker_t *wrk)
{
    vcl_session_t *s;
    ...
    pool_foreach (s, wrk->sessions) {
        ...
        if (s->vep.next_sh != ~0) {
            vcl_session_handle_parse(s->vep.next_sh, &wrk_index, &session_index);
            s->vep.next_sh = vcl_session_handle_from_index(session_index);
        }
        if (s->vep.prev_sh != ~0) {
            vcl_session_handle_parse(s->vep.prev_sh, &wrk_index, &session_index);
            s->vep.prev_sh = vcl_session_handle_from_index(session_index);
        }
    }
}

void vls_worker_copy_on_fork(vcl_worker_t *parent_wrk)
{
    ...
    vcl_wrk->session_index_by_vpp_handles = hash_dup(parent_wrk->session_index_by_vpp_handles);
...
static void vls_mt_acq_locks(vcl_locked_session_t *vls, vls_mt_ops_t op, int *locks_acq)
{
    ...
    if (vls) {
        s = vcl_session_get(wrk, vls->session_index);
        if (PREDICT_FALSE(!s)) return;
        is_nonblk = vcl_session_has_attr(s, VCL_SESS_ATTR_NONBLOCK);
    }

    switch (op) {
        case VLS_MT_OP_READ:
            if (!is_nonblk) is_nonblk = vcl_session_read_ready(s) != 0;
            if (!is_nonblk) {
                vls_mt_mq_lock();
        ...
        case VLS_MT_OP_WRITE:
            ASSERT(s);
            if (!is_nonblk) is_nonblk = vcl_session_write_ready(s) != 0;
            if (!is_nonblk) {
                vls_mt_mq_lock();
                *locks_acq |= VLS_MT_LOCK_MQ;
            }
...
static void vls_mt_rel_locks(int locks_acq)
{
    if (locks_acq & VLS_MT_LOCK_MQ) vls_mt_mq_unlock();
    if (locks_acq & VLS_MT_LOCK_SPOOL) vls_mt_create_unlock();
}

static inline u8 vls_mt_session_should_migrate(vcl_locked_session_t *vls)
{
    return (vls_mt_wrk_supported() && vls->vcl_wrk_index != vcl_get_worker_index());
}

static vcl_locked_session_t *vls_mt_session_migrate(vcl_locked_session_t *vls)
{
    ...
    if (!(p = hash_get(vls->vcl_wrk_index_to_session_index, vls->owner_vcl_wrk_index))) {
    ...
    VDBG(1, "migrate session of worker (session): %u (%u) -> %u (%u)", vls->owner_vcl_wrk_index, src_sid, wrk_index, sid);
    ...
    vls_unlock(vls);
    vls_mt_pool_runlock();
    vls_send_clone_and_share_rpc(wrk, vls_index, sid, vls_get_worker_index(), own_vcl_wrk_index, vls_index, src_sid);
    ...
    else if (PREDICT_FALSE((session->flags & VCL_SESSION_F_IS_VEP) && session->vep.next_sh != ~0)) {
        VERR("can't migrate nonempty epoll session");
        ASSERT(0);
        goto err;
    }
    else if (PREDICT_FALSE(!(session->flags & VCL_SESSION_F_IS_VEP) && session->session_state != VCL_STATE_CLOSED)) {
        VERR("migrate NOT supported, session_status (%u)", session->session_state);
        ASSERT(0);
        goto err;
    }
    ...
    vls_sh_to_vlsh_table_add(vls_wrk, vcl_session_handle(session), vls->vls_index);
    return vls;
...
{
    if (PREDICT_FALSE(vcl_get_worker_index() == ~0)) vls_mt_add();
}

#define vls_mt_guard(_vls, _op)                                                                                \
    int _locks_acq = 0;                                                                                        \
    if (vls_mt_wrk_supported()) {                                                                              \
        if (PREDICT_FALSE(_vls && ((vcl_locked_session_t *) _vls)->vcl_wrk_index != vcl_get_worker_index())) { \
            _vls = vls_mt_session_migrate(_vls);                                                               \
            if (PREDICT_FALSE(!_vls)) return VPPCOM_EBADFD;                                                    \
        }                                                                                                      \
    }                                                                                                          \
    else {                                                                                                     \
        if (PREDICT_FALSE(vlsl->vls_mt_n_threads > 1)) vls_mt_acq_locks(_vls, _op, &_locks_acq);               \
    }

#define vls_mt_unguard() \
    if (PREDICT_FALSE(_locks_acq)) vls_mt_rel_locks(_locks_acq)
...
int vls_write(vls_handle_t vlsh, void *buf, size_t nbytes)
{
    ...
    vls_mt_detect();
    if (!(vls = vls_get_w_dlock(vlsh))) return VPPCOM_EBADFD;
...
int vls_sendto(vls_handle_t vlsh, void *buf, int buflen, int flags, vppcom_endpt_t *ep)
{
    vcl_locked_session_t *vls;
    int                   rv;
...
ssize_t vls_recvfrom(vls_handle_t vlsh, void *buffer, uint32_t buflen, int flags, vppcom_endpt_t *ep)
{
    vcl_locked_session_t *vls;
    int                   rv;
    ...
    if (!(vls = vls_get_w_dlock(vlsh))) return VPPCOM_EBADFD;
    vls_mt_guard(vls, VLS_MT_OP_READ);
    rv = vppcom_session_recvfrom(vls_to_sh_tu(vls), buffer, buflen, flags, ep);
    vls_mt_unguard();
    vls_get_and_unlock(vlsh);
    return rv;
}
...
int vls_attr(vls_handle_t vlsh, uint32_t op, void *buffer, uint32_t *buflen)
{
    ...
    if (vls_mt_session_should_migrate(vls)) {
        ...
        if (PREDICT_FALSE(!vls)) return VPPCOM_EBADFD;
    }
...
static inline void vls_mp_checks(vcl_locked_session_t *vls, int is_add)
{
    vcl_worker_t  *wrk = vcl_worker_get_current();
    vcl_session_t *s;
    u32            owner_wrk;

    if (vls_mt_wrk_supported() && vls_n_workers() <= 1) return;
    ...
    switch (s->session_state) {
        ...
        case VCL_STATE_LISTEN_NO_MQ:
            ...
            if (!vls_listener_wrk_is_active(vls, owner_wrk)) vls_listener_wrk_stop_listen(vls, owner_wrk);
            break;
        default:
            break;
    }
...
vls_handle_t vls_accept(vls_handle_t listener_vlsh, vppcom_endpt_t *ep, int flags)
{
    ...
    vls_mt_detect();
    if (!(vls = vls_get_w_dlock(listener_vlsh))) return VPPCOM_EBADFD;
    if (vcl_n_workers() > 1) vls_mp_checks(vls, 1 /* is_add */);
    ...
    if (PREDICT_FALSE(accepted_vlsh == VLS_INVALID_HANDLE)) vppcom_session_close(sh);
...
vls_handle_t vls_create(uint8_t proto, uint8_t is_nonblocking)
{
    ...
    vls_mt_unguard();
    if (sh == INVALID_SESSION_ID) return VLS_INVALID_HANDLE;

    vlsh = vls_alloc(sh);
    if (PREDICT_FALSE(vlsh == VLS_INVALID_HANDLE)) vppcom_session_close(sh);

    return vlsh;
}

static void vls_mt_session_cleanup(vcl_locked_session_t *vls)
{
    ...
    hash_foreach(wrk_index, session_index, vls->vcl_wrk_index_to_session_index, ({
                     if (current_vcl_wrk != wrk_index) vls_send_session_cleanup_rpc(wrk, wrk_index, session_index);
                 }));
...
int vls_close(vls_handle_t vlsh)
{
    ...
    if (vls_mt_wrk_supported()) vls_mt_session_cleanup(vls);
...
vls_handle_t vls_epoll_create(void)
{
    ...
    vlsh = vls_alloc(sh);
    if (vlsh == VLS_INVALID_HANDLE) vppcom_session_close(sh);

    return vlsh;
}

static void vls_epoll_ctl_mp_checks(vcl_locked_session_t *vls, int op)
{
    if (vcl_n_workers() <= 1 || op == EPOLL_CTL_MOD) return;

    vls_mp_checks(vls, op == EPOLL_CTL_ADD);
}

int vls_epoll_ctl(vls_handle_t ep_vlsh, int op, vls_handle_t vlsh, struct epoll_event *event)
{
...
int vls_epoll_wait(vls_handle_t ep_vlsh, struct epoll_event *events, int maxevents, double wait_for_time)
{
    vcl_locked_session_t *vls, *vls_tmp = NULL;
    int                   rv;

    vls_mt_detect();
    if (!(vls = vls_get_w_dlock(ep_vlsh))) return VPPCOM_EBADFD;
    vls_mt_guard(vls_tmp, VLS_MT_OP_XPOLL);
    rv = vppcom_epoll_wait(vls_to_sh_tu(vls), events, maxevents, wait_for_time);
    vls_mt_unguard();
...
static void vls_select_mp_checks(vcl_si_set *read_map)
{
    vcl_locked_session_t *vls;
    vcl_worker_t         *wrk;
    vcl_session_t        *s;
...
int vls_select(int n_bits, vcl_si_set *read_map, vcl_si_set *write_map, vcl_si_set *except_map, double wait_for_time)
{
    ...
    vls_mt_detect();
    vls_mt_guard(vls, VLS_MT_OP_XPOLL);
    if (PREDICT_FALSE(!vlsl->select_mp_check)) vls_select_mp_checks(read_map);
...
static void vls_unshare_vcl_worker_sessions(vcl_worker_t *wrk)
{
    u32                   current_wrk, is_current;
    vcl_locked_session_t *vls;
    vcl_session_t        *s;

    if (pool_elts(vcm->workers) <= 1) return;
    ...
    pool_foreach (s, wrk->sessions) {
        ...
        if (vls && (is_current || vls_is_shared_by_wrk(vls, current_wrk))) vls_unshare_session(vls, wrk);
    }
}

static void vls_cleanup_vcl_worker(vcl_worker_t *wrk)
{
    ...
    if (child_wrk->forked_child != ~0) {
        ...
        if (sub_child) {
            ...
            while (kill(sub_child->current_pid, 0) >= 0 && tries++ < 50)
                usleep(1e3);
            if (kill(sub_child->current_pid, 0) < 0) vls_cleanup_forked_child(child_wrk, sub_child);
        }
    }
...
static void vls_handle_pending_wrk_cleanup(void)
{
    ...
    if (PREDICT_TRUE(vec_len(vls_wrk->pending_vcl_wrk_cleanup) == 0)) return;

    wrk = vcl_worker_get_current();
    vec_foreach (wip, vls_wrk->pending_vcl_wrk_cleanup) {
        child_wrk = vcl_worker_get_if_valid(*wip);
        if (!child_wrk) continue;
...
static void vls_intercept_sigchld_handler(int signum, siginfo_t *si, void *uc)
{
    vcl_worker_t *wrk, *child_wrk;
    vls_worker_t *vls_wrk;

    if (vcl_get_worker_index() == ~0) return;
    ...
    wrk = vcl_worker_get_current();
    if (wrk->forked_child == ~0) return;

    child_wrk = vcl_worker_get_if_valid(wrk->forked_child);
    if (!child_wrk) goto done;
...
static void vls_incercept_sigchld()
{
    struct sigaction sa;
...
static void vls_app_pre_fork(void)
{
    vls_incercept_sigchld();
    vcl_flush_mq_events();
}

static void vls_app_fork_child_handler(void)
{
    vcl_worker_t *parent_wrk;
    int           parent_wrk_index;

    parent_wrk_index = vcl_get_worker_index();
    VDBG(0, "initializing forked child %u with parent wrk %u", getpid(), parent_wrk_index);
...
static void vls_app_fork_parent_handler(void)
{
    vcm->forking = 1;
    while (vcm->forking)
        ;
...
static void vls_clone_and_share_rpc_handler(void *args)
{
    ...
    VDBG(1,
         "process session clone of worker (session): %u (%u) -> %u (%u)",
         vcl_wrk->wrk_index,
         msg->session_index,
         msg->origin_vcl_wrk,
...
static void vls_session_cleanup_rpc_handler(void *args)
{
    ...
    VDBG(1, "process session cleanup of worker (session): %u (%u) from %u ()", wrk->wrk_index, msg->session_index, msg->origin_vcl_wrk);

    vppcom_session_close(sh);
    vls_sh_to_vlsh_table_del(vls_wrk, sh);
}

static void vls_rpc_handler(void *args)
{
                                  ...
                                  u32           dst_session_index)
{
    ...
    VDBG(1,
         "send session clone to wrk (session): %u (%u) -> %u (%u), ret=%d",
         dst_wrk_index,
         msg->session_index,
         msg->origin_vcl_wrk,
         msg->origin_session_index,
         ret);
    while (!ret && wrk->rpc_done == VLS_RPC_STATE_INIT && clib_time_now(&wrk->clib_time) < timeout)
        ;
...
void vls_send_session_cleanup_rpc(vcl_worker_t *wrk, u32 dst_wrk_index, u32 dst_session_index)
{
    ...
    VDBG(1, "send session cleanup to wrk (session): %u (%u) from %u, ret=%d", dst_wrk_index, msg->session_index, msg->origin_vcl_wrk, ret);
}
...
int vls_app_create(char *app_name)
{
    ...
    if ((rv = vppcom_app_create(app_name))) return rv;
    ...
    pthread_atfork(vls_app_pre_fork, vls_app_fork_parent_handler, vls_app_fork_child_handler);
    atexit(vls_app_exit);
    vls_worker_alloc();
    vlsl->vls_wrk_index = vcl_get_worker_index();
    clib_rwlock_init(&vlsl->vls_pool_lock);
