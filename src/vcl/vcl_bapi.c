...
static void vl_api_session_enable_disable_reply_t_handler(vl_api_session_enable_disable_reply_t *mp)
{
    ...
    if (mp->retval) {
        clib_warning("VCL<%d>: session_enable_disable failed: %U", getpid(), format_api_error, ntohl(mp->retval));
    }
    else
...
static void vl_api_app_attach_reply_t_handler(vl_api_app_attach_reply_t *mp)
{
    ...
    if (mp->n_fds) {
        ...
        if (vl_socket_client_recv_fd_msg2(&wrk->bapi_sock_ctx, fds, mp->n_fds, 5)) goto failed;

        if (mp->fd_flags & SESSION_FD_F_VPP_MQ_SEGMENT)
            if (vcl_segment_attach(vcl_vpp_worker_segment_handle(0), "vpp-mq-seg", SSVM_SEGMENT_MEMFD, fds[n_fds++])) goto failed;

        if (mp->fd_flags & SESSION_FD_F_MEMFD_SEGMENT) {
            segment_name = vl_api_from_api_to_new_c_string(&mp->segment_name);
            rv           = vcl_segment_attach(segment_handle, segment_name, SSVM_SEGMENT_MEMFD, fds[n_fds++]);
        ...
        vcl_segment_discover_mqs(vcl_vpp_worker_segment_handle(0), fds + n_fds, mp->n_fds - n_fds);
        vcl_segment_attach_mq(vcl_vpp_worker_segment_handle(0), mp->vpp_ctrl_mq, mp->vpp_ctrl_mq_thread, &wrk->ctrl_mq);
        ...
        segment_name = vl_api_from_api_to_new_c_string(&mp->segment_name);
        rv           = vcl_segment_attach(segment_handle, segment_name, SSVM_SEGMENT_SHM, -1);
        vec_free(segment_name);
        if (rv != 0) goto failed;
    }
...
static void vl_api_app_worker_add_del_reply_t_handler(vl_api_app_worker_add_del_reply_t *mp)
{
    ...
    if (mp->retval) {
        clib_warning("VCL<%d>: add/del worker failed: %U", getpid(), format_api_error, ntohl(mp->retval));
        goto failed;
    }
    ...
    if (mp->n_fds) {
        ...
        if (mp->fd_flags & SESSION_FD_F_VPP_MQ_SEGMENT)
            if (vcl_segment_attach(vcl_vpp_worker_segment_handle(wrk_index), "vpp-worker-seg", SSVM_SEGMENT_MEMFD, fds[n_fds++])) goto failed;
...
static void vl_api_app_add_cert_key_pair_reply_t_handler(vl_api_app_add_cert_key_pair_reply_t *mp)
{
    ...
    if (mp->retval) {
        VDBG(0, "Adding cert and key failed: %U", format_api_error, ntohl(mp->retval));
        return;
    }
...
{
    if (mp->retval) {
        VDBG(0, "Deleting cert and key failed: %U", format_api_error, ntohl(mp->retval));
        return;
    }
}
...
static void vcl_bapi_hookup(void)
{
    u8 *msg_base_lookup_name = format(0, "session_%08x%c", api_version, 0);

    REPLY_MSG_ID_BASE = vl_client_get_first_plugin_msg_id((char *) msg_base_lookup_name);

    vec_free(msg_base_lookup_name);

    if (REPLY_MSG_ID_BASE == (u16) ~0) return;
 ...
 */
static void vcl_bapi_send_session_enable_disable(u8 is_enable)
{
    vcl_worker_t                    *wrk = vcl_worker_get_current();
...
void vcl_bapi_send_attach(void)
{
    ...
    u8                   app_is_proxy = (vcm->cfg.app_proxy_transport_tcp || vcm->cfg.app_proxy_transport_udp);
    ...
    bmp->options[APP_OPTIONS_FLAGS] =
        APP_OPTIONS_FLAGS_ACCEPT_REDIRECT | APP_OPTIONS_FLAGS_ADD_SEGMENT | (vcm->cfg.app_scope_local ? APP_OPTIONS_FLAGS_USE_LOCAL_SCOPE : 0)
        | (vcm->cfg.app_scope_global ? APP_OPTIONS_FLAGS_USE_GLOBAL_SCOPE : 0) | (app_is_proxy ? APP_OPTIONS_FLAGS_IS_PROXY : 0)
        | (vcm->cfg.use_mq_eventfd ? APP_OPTIONS_FLAGS_EVT_MQ_USE_EVENTFD : 0) | (vcm->cfg.huge_page ? APP_OPTIONS_FLAGS_USE_HUGE_PAGE : 0)
    ...
    bmp->options[APP_OPTIONS_PREALLOC_FIFO_PAIRS] = vcm->cfg.preallocated_fifo_pairs;
...
static void vcl_bapi_send_app_worker_add_del(u8 is_add)
{
    vcl_worker_t                *wrk = vcl_worker_get_current();
    ...
    mp->is_add       = is_add;
    if (!is_add) mp->wrk_index = clib_host_to_net_u32(wrk->vpp_wrk_index);

    vl_msg_api_send_shmem(wrk->vl_input_queue, (u8 *) &mp);
}

static void vcl_bapi_send_child_worker_del(vcl_worker_t *child_wrk)
{
    vcl_worker_t                *wrk = vcl_worker_get_current();
    vl_api_app_worker_add_del_t *mp;
...
static void vcl_bapi_send_app_add_cert_key_pair(vppcom_cert_key_pair_t *ckpair)
{
    vcl_worker_t                   *wrk      = vcl_worker_get_current();
...
static void vcl_bapi_send_app_del_cert_key_pair(u32 ckpair_index)
{
    vcl_worker_t                   *wrk = vcl_worker_get_current();
...
static void vcl_bapi_init_error_string_table(void)
{
...
static void vcl_bapi_cleanup(void)
{
...
static int vcl_bapi_connect_to_vpp(void)
{
    vcl_worker_t *wrk     = vcl_worker_get_current();
    ...
    if (vl_socket_client_connect2(&wrk->bapi_sock_ctx, (char *) vcl_cfg->vpp_bapi_socket_name, (char *) wrk_name, 0 /* default rx/tx buffer */)) {
        VERR("app (%s) socket connect failed!", wrk_name);
        rv = VPPCOM_ECONNREFUSED;
        goto error;
    }

    if (vl_socket_client_init_shm2(&wrk->bapi_sock_ctx, 0, 1 /* want_pthread */)) {
...
static const char *vcl_bapi_app_state_str(vcl_bapi_app_state_t state)
{
    char *st;
...
static int vcl_bapi_wait_for_wrk_state_change(vcl_bapi_app_state_t app_state)
{
    vcl_worker_t *wrk     = vcl_worker_get_current();
    f64           timeout = clib_time_now(&wrk->clib_time) + vcm->cfg.app_timeout;

    while (clib_time_now(&wrk->clib_time) < timeout) {
        if (wrk->bapi_app_state == app_state) return VPPCOM_OK;
        if (wrk->bapi_app_state == STATE_APP_FAILED) return VPPCOM_ECONNABORTED;
    }
    VDBG(0, "timeout waiting for state %s, current state %d", vcl_bapi_app_state_str(app_state), wrk->bapi_app_state);
    vcl_evt(VCL_EVT_SESSION_TIMEOUT, vcm, bapi_app_state);

    return VPPCOM_ETIMEDOUT;
}

static int vcl_bapi_session_enable(void)
{
    vcl_worker_t *wrk = vcl_worker_get_current();
    int           rv;
    ...
    if (wrk->bapi_app_state != STATE_APP_ENABLED) {
        ...
        if (PREDICT_FALSE(rv)) {
            VDBG(0, "application session enable timed out! returning %d (%s)", rv, vppcom_retval_str(rv));
            return rv;
        }
    }
    return VPPCOM_OK;
...
int vcl_bapi_attach(void)
{
    ...
    if ((rv = vcl_bapi_init())) return rv;

    vcl_bapi_send_attach();
    rv = vcl_bapi_wait_for_wrk_state_change(STATE_APP_ATTACHED);
    if (PREDICT_FALSE(rv)) {
        VDBG(0, "application attach timed out! returning %d (%s)", rv, vppcom_retval_str(rv));
        return rv;
    }
...
int vcl_bapi_app_worker_add(void)
{
    ...
    if (vcl_bapi_connect_to_vpp()) return -1;

    wrk->bapi_app_state = STATE_APP_ADDING_WORKER;
    vcl_bapi_send_app_worker_add_del(1 /* is_add */);
    if (vcl_bapi_wait_for_wrk_state_change(STATE_APP_READY)) return -1;
    return 0;
}
...
int vcl_bapi_recv_fds(vcl_worker_t *wrk, int *fds, int n_fds)
{
    ...
    if ((err = vl_socket_client_recv_fd_msg2(&wrk->bapi_sock_ctx, fds, n_fds, 5))) {
...
int vcl_bapi_add_cert_key_pair(vppcom_cert_key_pair_t *ckpair)
{
    ...
    if (ckpair->key_len == 0 || ckpair->key_len == ~0) return VPPCOM_EINVAL;
    ...
    if (wrk->bapi_app_state == STATE_APP_READY) return wrk->bapi_return;
