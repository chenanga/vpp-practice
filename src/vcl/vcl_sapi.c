...
static int vcl_api_connect_app_socket(vcl_worker_t *wrk)
{
...
static int vcl_api_attach_reply_handler(app_sapi_attach_reply_msg_t *mp, int *fds)
{
    ...
    if (!mp->n_fds) goto failed;

    if (mp->fd_flags & SESSION_FD_F_VPP_MQ_SEGMENT)
        if (vcl_segment_attach(vcl_vpp_worker_segment_handle(0), "vpp-mq-seg", SSVM_SEGMENT_MEMFD, fds[n_fds_used++])) goto failed;

    if (mp->fd_flags & SESSION_FD_F_MEMFD_SEGMENT) {
        segment_name = format(0, "memfd-%ld%c", segment_handle, 0);
        rv           = vcl_segment_attach(segment_handle, (char *) segment_name, SSVM_SEGMENT_MEMFD, fds[n_fds_used++]);
    ...
    vcl_segment_discover_mqs(vcl_vpp_worker_segment_handle(0), fds + n_fds_used, mp->n_fds - n_fds_used);
...
static int vcl_api_send_attach(clib_socket_t *cs)
{
...
int vcl_sapi_attach(void)
{
     ...
     */
    if (vcl_api_connect_app_socket(wrk)) return -1;

    cs = &wrk->app_api_sock;
    if (vcl_api_send_attach(cs)) return -1;
    ...
    if (rmp->type != APP_SAPI_MSG_TYPE_ATTACH_REPLY) return -1;

    return vcl_api_attach_reply_handler(&rmp->attach_reply, fds);
}

static int vcl_api_add_del_worker_reply_handler(app_sapi_worker_add_del_reply_msg_t *mp, int *fds)
{
    int           n_fds = 0, i, rv;
    u64           segment_handle;
    vcl_worker_t *wrk;
    ...
    if (mp->fd_flags & SESSION_FD_F_VPP_MQ_SEGMENT)
        if (vcl_segment_attach(vcl_vpp_worker_segment_handle(wrk->wrk_index), "vpp-worker-seg", SSVM_SEGMENT_MEMFD, fds[n_fds++])) goto failed;

    if (mp->fd_flags & SESSION_FD_F_MEMFD_SEGMENT) {
        u8 *segment_name = format(0, "memfd-%ld%c", segment_handle, 0);
        rv               = vcl_segment_attach(segment_handle, (char *) segment_name, SSVM_SEGMENT_MEMFD, fds[n_fds++]);
        vec_free(segment_name);
        if (rv != 0) goto failed;
    }
...
int vcl_sapi_recv_fds(vcl_worker_t *wrk, int *fds, int n_fds)
{
    ...
    if (msg->type != APP_SAPI_MSG_TYPE_SEND_FDS) return -1;
...
int vcl_sapi_add_cert_key_pair(vppcom_cert_key_pair_t *ckpair)
{
    ...
    if (!rmp->cert_key_add_del_reply.retval) rv = rmp->cert_key_add_del_reply.index;
...
int vcl_sapi_del_cert_key_pair(u32 ckpair_index)
{
    ...
    if (rmp->cert_key_add_del_reply.retval) return -1;
