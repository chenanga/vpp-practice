...
#define VDBG(_lvl, _fmt, _args...) \
    if (VCL_DBG_ON && vcm->debug > _lvl) clib_warning("vcl<%d:%d>: " _fmt, vcm->workers[__vcl_worker_index].current_pid, __vcl_worker_index, ##_args)

#define VWRN(_fmt, _args...) clib_warning("vcl<%d:%d>: " _fmt, vcm->workers[__vcl_worker_index].current_pid, __vcl_worker_index, ##_args)

#define VERR(_fmt, _args...) clib_warning("vcl<%d:%d>: ERROR " _fmt, vcm->workers[__vcl_worker_index].current_pid, __vcl_worker_index, ##_args)
...
#define VCL_EVT_INIT_HANDLER(_vcm, ...)                                                                \
    {                                                                                                  \
        _vcm->elog_track.name = (char *) format(0, "P:%d:C:%d%c", getpid(), _vcm->my_client_index, 0); \
...
#define VCL_EVT_SESSION_INIT_HANDLER(_s, _s_index, ...)                                              \
    {                                                                                                \
        _s->elog_track.name = (char *) format(0, "CI:%d:S:%d%c", vcm->my_client_index, _s_index, 0); \
...
#define vcl_elog_init(_vcm)                                                                                     \
    {                                                                                                           \
        _vcm->elog_main.lock            = clib_mem_alloc_aligned(CLIB_CACHE_LINE_BYTES, CLIB_CACHE_LINE_BYTES); \
...
#define vcl_elog_stop(_vcm)                                                                                                                  \
    {                                                                                                                                        \
        ...
        char         *chroot_file = (char *) format(0, "%s/%d-%d-vcl-elog%c", _vcm->cfg.event_log_path, _vcm->my_client_index, getpid(), 0); \
        error                     = elog_write_file(&_vcm->elog_main, chroot_file, 1 /* flush ring */);                                      \
