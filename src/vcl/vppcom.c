...
static inline int vcl_mq_dequeue_batch(vcl_worker_t *wrk, svm_msg_q_t *mq, u32 n_max_msg)
{
    u32 n_msgs = 0, sz, len;
...
static void vcl_msg_add_ext_config(vcl_session_t *s, uword *offset)
{
    svm_fifo_chunk_t *c;

    c = vcl_segment_alloc_chunk(vcl_vpp_worker_segment_handle(0), 0 /* one slice only */, s->ext_config->len, offset);
    if (c) clib_memcpy_fast(c->data, s->ext_config, s->ext_config->len);
}
...
void vcl_send_session_listen(vcl_worker_t *wrk, vcl_session_t *s)
{
    ...
    mp->vrf   = s->vrf;
    if (s->flags & VCL_SESSION_F_CONNECTED) mp->flags = TRANSPORT_CFG_F_CONNECTED;
    if (s->ext_config) vcl_msg_add_ext_config(s, &mp->ext_config);
...
static void vcl_send_session_connect(vcl_worker_t *wrk, vcl_session_t *s)
{
    app_session_evt_t      _app_evt, *app_evt = &_app_evt;
...
static void vcl_send_session_shutdown(vcl_worker_t *wrk, vcl_session_t *s)
{
    app_session_evt_t       _app_evt, *app_evt = &_app_evt;
...
static void vcl_send_session_disconnect(vcl_worker_t *wrk, vcl_session_t *s)
{
    app_session_evt_t         _app_evt, *app_evt = &_app_evt;
...
static void vcl_send_app_detach(vcl_worker_t *wrk)
{
    app_session_evt_t         _app_evt, *app_evt = &_app_evt;
...
static void vcl_send_session_accepted_reply(svm_msg_q_t *mq, u32 context, session_handle_t handle, int retval)
{
    app_session_evt_t             _app_evt, *app_evt = &_app_evt;
...
static void vcl_send_session_disconnected_reply(vcl_worker_t *wrk, vcl_session_t *s, int retval)
{
    app_session_evt_t                 _app_evt, *app_evt = &_app_evt;
    session_disconnected_reply_msg_t *rmp;
    app_alloc_ctrl_evt_to_vpp(s->vpp_evt_q, app_evt, SESSION_CTRL_EVT_DISCONNECTED_REPLY);
...
static void vcl_send_session_reset_reply(vcl_worker_t *wrk, vcl_session_t *s, int retval)
{
    app_session_evt_t          _app_evt, *app_evt = &_app_evt;
    session_reset_reply_msg_t *rmp;
    app_alloc_ctrl_evt_to_vpp(s->vpp_evt_q, app_evt, SESSION_CTRL_EVT_RESET_REPLY);
...
void vcl_send_session_worker_update(vcl_worker_t *wrk, vcl_session_t *s, u32 wrk_index)
{
    app_session_evt_t            _app_evt, *app_evt = &_app_evt;
    session_worker_update_msg_t *mp;

    app_alloc_ctrl_evt_to_vpp(s->vpp_evt_q, app_evt, SESSION_CTRL_EVT_WORKER_UPDATE);
...
int vcl_send_worker_rpc(u32 dst_wrk_index, void *data, u32 data_len)
{
    ...
    if (data_len > sizeof(mp->data)) goto done;

    clib_spinlock_lock(&vcm->workers_lock);

    dst_wrk = vcl_worker_get_if_valid(dst_wrk_index);
    if (!dst_wrk) goto done;
...
int vcl_session_transport_attr(vcl_worker_t *wrk, vcl_session_t *s, u8 is_get, transport_endpt_attr_t *attr)
{
    app_session_evt_t             _app_evt, *app_evt = &_app_evt;
    ...
    if (!wrk->session_attr_op_rv && is_get) *attr = wrk->session_attr_rv;
...
static u32 vcl_session_accepted_handler(vcl_worker_t *wrk, session_accepted_msg_t *mp, u32 ls_index)
{
    ...
    if (listen_session->vpp_handle != mp->listener_handle) {
        VDBG(0, "ERROR: listener handle %lu does not match session %u", mp->listener_handle, ls_index);
        goto error;
    }

    if (vcl_segment_attach_session(mp->segment_handle,
                                   mp->server_rx_fifo,
                                   mp->server_tx_fifo,
                                   ...
                                   0,
                                   session)) {
        VDBG(0, "session %u [0x%llx]: failed to attach fifos", session->session_index, mp->handle);
        goto error;
    }
    ...
    clib_memcpy_fast(&session->transport.rmt_ip, &mp->rmt.ip, sizeof(ip46_address_t));
    ...
    if (session->is_dgram) session->flags |= (listen_session->flags & VCL_SESSION_F_CONNECTED);
    ...
    vcl_send_session_accepted_reply(session->vpp_evt_q, mp->context, session->vpp_handle, 0);

    return session->session_index;

error:
    vcl_segment_attach_mq(vcl_vpp_worker_segment_handle(0), mp->vpp_event_queue_address, mp->mq_index, &evt_q);
    vcl_send_session_accepted_reply(evt_q, mp->context, mp->handle, VNET_API_ERROR_INVALID_ARGUMENT);
    vcl_session_free(wrk, session);
    return VCL_INVALID_SESSION_INDEX;
}

static u32 vcl_session_connected_handler(vcl_worker_t *wrk, session_connected_msg_t *mp)
{
    ...
    if (PREDICT_FALSE(!session)) {
        VERR("vpp handle 0x%llx has no session index (%u)!", mp->handle, session_index);
        /* Should not happen but if it does, force vpp session cleanup */
        vcl_session_t tmp_session = {
            .vpp_handle = mp->handle,
            .vpp_evt_q  = 0,
        };
        vcl_segment_attach_session(mp->segment_handle, mp->server_rx_fifo, mp->server_tx_fifo, mp->vpp_event_queue_address, mp->mq_index, 0, session);
        if (tmp_session.vpp_evt_q) vcl_send_session_disconnect(wrk, &tmp_session);
        return VCL_INVALID_SESSION_INDEX;
    }

    if (mp->retval) {
        VDBG(0, "session %u: connect failed! %U", session_index, format_session_error, mp->retval);
    ...
    if (vcl_segment_attach_session(mp->segment_handle,
                                   ...
                                   0,
                                   session)) {
        VDBG(0, "session %u [0x%llx]: failed to attach fifos", session->session_index, session->vpp_handle);
    ...
    if (mp->ct_rx_fifo) {
        if (vcl_segment_attach_session(mp->ct_segment_handle, mp->ct_rx_fifo, mp->ct_tx_fifo, (uword) ~0, ~0, 1, session)) {
            VDBG(0, "session %u [0x%llx]: failed to attach ct fifos", session->session_index, session->vpp_handle);
            session->session_state = VCL_STATE_UPDATED;
            vcl_send_session_disconnect(wrk, session);
            return session_index;
        }
    ...
    clib_memcpy_fast(&session->transport.lcl_ip, &mp->lcl.ip, sizeof(session->transport.lcl_ip));
    session->transport.lcl_port = mp->lcl.port;

    /* Application closed session before connect reply */
    if (vcl_session_has_attr(session, VCL_SESS_ATTR_NONBLOCK) && session->session_state == VCL_STATE_CLOSED)
        vcl_send_session_disconnect(wrk, session);
    else
        session->session_state = VCL_STATE_READY;

    VDBG(0,
         "session %u [0x%llx] connected local: %U:%u remote %U:%u",
         session->session_index,
         session->vpp_handle,
         vcl_format_ip46_address,
...
static int vcl_flag_accepted_session(vcl_session_t *session, u64 handle, u32 flags)
{
...
static u32 vcl_session_reset_handler(vcl_worker_t *wrk, session_reset_msg_t *reset_msg)
{
    ...
    if (session->session_state == VCL_STATE_LISTEN || session->session_state == VCL_STATE_LISTEN_NO_MQ) {
        if (!vcl_flag_accepted_session(session, reset_msg->handle, VCL_ACCEPTED_F_RESET)) VDBG(0, "session was not accepted!");
        return VCL_INVALID_SESSION_INDEX;
    }

    if (session->session_state != VCL_STATE_CLOSED) session->session_state = VCL_STATE_DISCONNECT;
...
static u32 vcl_session_bound_handler(vcl_worker_t *wrk, session_bound_msg_t *mp)
{
    vcl_session_t *session;
    u32            sid = mp->context;
    ...
    session = vcl_session_get(wrk, sid);
    if (mp->retval) {
        VERR("session %u [0x%llx]: bind failed: %U", sid, mp->handle, format_session_error, mp->retval);
        if (session) {
            session->session_state = VCL_STATE_DETACHED;
            session->vpp_handle    = mp->handle;
            session->vpp_error     = mp->retval;
            ...
            VDBG(0, "session %u [0x%llx]: Invalid session index!", sid, mp->handle);
            return VCL_INVALID_SESSION_INDEX;
        }
    }
    ...
    clib_memcpy_fast(&session->transport.lcl_ip, mp->lcl_ip, sizeof(ip46_address_t));
    ...
    if (vcl_session_is_cl(session)) {
        if (vcl_segment_attach_session(mp->segment_handle, mp->rx_fifo, mp->tx_fifo, mp->vpp_evt_q, mp->mq_index, 0, session)) {
            VDBG(0, "session %u [0x%llx]: failed to attach fifos", session->session_index, session->vpp_handle);
            session->session_state = VCL_STATE_DETACHED;
            return VCL_INVALID_SESSION_INDEX;
        }
...
static void vcl_session_unlisten_reply_handler(vcl_worker_t *wrk, void *data)
{
    ...
    if (s->session_state != VCL_STATE_DISCONNECT) {
        ...
        if (s->session_type == VPPCOM_PROTO_UDP && s->session_state == VCL_STATE_CLOSED) return;

        VDBG(0, "Unlisten session in wrong state %llx", mp->handle);
        return;
    }

    if (mp->retval) VDBG(0, "ERROR: session %u [0xllx]: unlisten failed: %U", s->session_index, mp->handle, format_session_error, mp->retval);

    if (mp->context != wrk->wrk_index) VDBG(0, "wrong context");

    vcl_session_table_del_vpp_handle(wrk, mp->handle);
    vcl_session_free(wrk, s);
}

static void vcl_session_migrated_handler(vcl_worker_t *wrk, void *data)
{
    ...
    if (mp->segment_handle != SESSION_INVALID_HANDLE) {
        ...
        if (fs_index == VCL_INVALID_SEGMENT_INDEX) {
            VDBG(0, "segment %lx for session %u is not mounted!", mp->segment_handle, s->session_index);
    ...
    vcl_segment_attach_mq(vcl_vpp_worker_segment_handle(0), mp->vpp_evt_q, mp->vpp_thread_index, &s->vpp_evt_q);

    vcl_session_table_del_vpp_handle(wrk, mp->handle);
    vcl_session_table_add_vpp_handle(wrk, mp->new_handle, s->session_index);

    /* Generate new tx event if we have outstanding data */
    if (svm_fifo_has_event(s->tx_fifo)) app_send_io_evt_to_vpp(s->vpp_evt_q, s->tx_fifo->shr->master_session_index, SESSION_IO_EVT_TX, SVM_Q_WAIT);

    VDBG(0, "Migrated 0x%lx to thread %u 0x%lx", mp->handle, mp->vpp_thread_index, mp->new_handle);
}

static vcl_session_t *vcl_session_accepted(vcl_worker_t *wrk, session_accepted_msg_t *msg)
{
    vcl_session_msg_t *vcl_msg;
    vcl_session_t     *session;

    session = vcl_session_get_w_vpp_handle(wrk, msg->handle);
    if (PREDICT_FALSE(session != 0)) VWRN("session overlap handle %lu state %u!", msg->handle, session->session_state);

    session = vcl_session_table_lookup_listener(wrk, msg->listener_handle);
    if (!session) {
        VERR("couldn't find listen session: listener handle %llx", msg->listener_handle);
        return 0;
    }
...
static vcl_session_t *vcl_session_disconnected_handler(vcl_worker_t *wrk, session_disconnected_msg_t *msg)
{
    vcl_session_t *session;
    ...
    if (session->session_state == VCL_STATE_CLOSED) return 0;

    /* Caught a disconnect before actually accepting the session */
    if (session->session_state == VCL_STATE_LISTEN || session->session_state == VCL_STATE_LISTEN_NO_MQ) {
        if (!vcl_flag_accepted_session(session, msg->handle, VCL_ACCEPTED_F_CLOSED)) VDBG(0, "session was not accepted!");
        return 0;
    }

    /* If not already reset change state */
    if (session->session_state != VCL_STATE_DISCONNECT) session->session_state = VCL_STATE_VPP_CLOSING;

    return session;
}

int vppcom_session_shutdown(uint32_t session_handle, int how)
{
    ...
    if (PREDICT_FALSE(!session)) return VPPCOM_EBADFD;

    vpp_handle = session->vpp_handle;
    state      = session->session_state;

    VDBG(1, "session %u [0x%llx] state 0x%x (%s)", session->session_index, vpp_handle, state, vcl_session_state_str(state));
    ...
    if (how == SHUT_RD || how == SHUT_RDWR) {
        ...
        if (how == SHUT_RD) return VPPCOM_OK;
    }
    session->flags |= VCL_SESSION_F_WR_SHUTDOWN;

    if (PREDICT_TRUE(state == VCL_STATE_READY)) {
        VDBG(1, "session %u [0x%llx]: sending shutdown...", session->session_index, vpp_handle);
...
static int vppcom_session_disconnect(u32 session_handle)
{
    vcl_worker_t       *wrk = vcl_worker_get_current();
    vcl_session_t      *session, *listen_session;
    vcl_session_state_t state;
    ...
    session = vcl_session_get_w_handle(wrk, session_handle);
    if (!session) return VPPCOM_EBADFD;

    vpp_handle = session->vpp_handle;
    state      = session->session_state;

    VDBG(1, "session %u [0x%llx]: disconnecting state (%s)", session->session_index, vpp_handle, vcl_session_state_str(state));
    ...
    if (state == VCL_STATE_VPP_CLOSING) {
        ...
        VDBG(1, "session %u [0x%llx]: sending disconnect REPLY...", session->session_index, vpp_handle);
    }
    else {
        /* Session doesn't have an event queue yet. Probably a non-blocking
         * connect. Wait for the reply */
        if (PREDICT_FALSE(!session->vpp_evt_q)) return VPPCOM_OK;

        VDBG(1, "session %u [0x%llx]: sending disconnect", session->session_index, vpp_handle);
        vcl_send_session_disconnect(wrk, session);
    }

    if (session->listener_index != VCL_INVALID_SESSION_INDEX) {
        listen_session = vcl_session_get(wrk, session->listener_index);
        if (listen_session) listen_session->n_accepted_sessions--;
    }

    return VPPCOM_OK;
}

static void vcl_session_cleanup_handler(vcl_worker_t *wrk, void *data)
{
...
static void vcl_session_req_worker_update_handler(vcl_worker_t *wrk, void *data)
{
...
static void vcl_session_worker_update_reply_handler(vcl_worker_t *wrk, void *data)
{
    ...
    if (s->rx_fifo) {
        if (vcl_segment_attach_session(msg->segment_handle, msg->rx_fifo, msg->tx_fifo, (uword) ~0, ~0, 0, s)) {
    ...
    VDBG(0, "session %u[0x%llx] moved to worker %u", s->session_index, s->vpp_handle, wrk->wrk_index);
}

static int vcl_api_recv_fd(vcl_worker_t *wrk, int *fds, int n_fds)
{
    if (vcm->cfg.vpp_app_socket_api) return vcl_sapi_recv_fds(wrk, fds, n_fds);

    return vcl_bapi_recv_fds(wrk, fds, n_fds);
}

static void vcl_session_app_add_segment_handler(vcl_worker_t *wrk, void *data)
{
    ...
    if (vcl_segment_attach(segment_handle, (char *) msg->segment_name, seg_type, fd)) {
        VDBG(0, "vcl_segment_attach ('%s') failed", msg->segment_name);
        return;
    }

    VDBG(1, "mapped new segment '%s' size %d", msg->segment_name, msg->segment_size);
}

static void vcl_session_app_del_segment_handler(vcl_worker_t *wrk, void *data)
{
...
static void vcl_worker_rpc_handler(vcl_worker_t *wrk, void *data)
{
    if (!vcm->wrk_rpc_fn) return;

    (vcm->wrk_rpc_fn)(((session_app_wrk_rpc_msg_t *) data)->data);
}

static void vcl_session_transport_attr_reply_handler(vcl_worker_t *wrk, void *data)
{
    session_transport_attr_reply_msg_t *mp;

    if (!wrk->session_attr_op) return;
...
static int vcl_handle_mq_event(vcl_worker_t *wrk, session_event_t *e)
{
    ...
    switch (e->event_type) {
        ...
        case SESSION_IO_EVT_TX:
            ...
            if (!s || !vcl_session_is_open(s)) break;
        ...
        case SESSION_CTRL_EVT_CONNECTED:
            ...
            if (!(s = vcl_session_get(wrk, sid))) break;
            if (vcl_session_has_attr(s, VCL_SESS_ATTR_NONBLOCK)) {
                vec_add2(wrk->unhandled_evts_vector, ecpy, 1);
                *ecpy               = *e;
                ecpy->postponed     = 1;
        ...
        case SESSION_CTRL_EVT_DISCONNECTED:
            ...
            if (!(s = vcl_session_get_w_vpp_handle(wrk, disconnected_msg->handle))) break;
            if (s->session_state == VCL_STATE_CLOSED) break;
             ...
             */
            if (vcl_session_has_attr(s, VCL_SESS_ATTR_NONBLOCK)
                && !(s->session_state == VCL_STATE_LISTEN || s->session_state == VCL_STATE_LISTEN_NO_MQ)) {
            ...
            if (!(s = vcl_session_disconnected_handler(wrk, disconnected_msg))) break;
            VDBG(0, "disconnected session %u [0x%llx]", s->session_index, s->vpp_handle);
            break;
        case SESSION_CTRL_EVT_RESET:
            reset_msg = (session_reset_msg_t *) e->data;
            if (!(s = vcl_session_get_w_vpp_handle(wrk, reset_msg->handle))) break;
            if (s->session_state == VCL_STATE_CLOSED) break;
            /* We do not postpone for blocking sessions or listen sessions because:
             * 1. Blocking sessions are not part of epoll instead they're used in a
             *    synchronous manner, such as read/write and etc.
...
static int vppcom_wait_for_session_state_change(u32 session_index, vcl_session_state_t state, f64 wait_for_time)
{
    vcl_worker_t *wrk     = vcl_worker_get_current();
    ...
    VDBG(0, "timeout waiting for state 0x%x (%s)", state, vcl_session_state_str(state));
    vcl_evt(VCL_EVT_SESSION_TIMEOUT, session, session_state);

    return VPPCOM_ETIMEDOUT;
}

static void vcl_handle_pending_wrk_updates(vcl_worker_t *wrk)
{
    vcl_session_state_t state;
    vcl_session_t      *s;
    u32                *sip;

    if (PREDICT_TRUE(vec_len(wrk->pending_session_wrk_updates) == 0)) return;
    ...
    vec_foreach (sip, wrk->pending_session_wrk_updates) {
        ...
        vppcom_wait_for_session_state_change(s->session_index, VCL_STATE_UPDATED, 5);
        s->session_state = state;
    }
    vec_reset_length(wrk->pending_session_wrk_updates);
}

void vcl_worker_flush_mq_events(vcl_worker_t *wrk)
{
    svm_msg_q_msg_t *msg;
    session_event_t *e;
    svm_msg_q_t     *mq;
...
static int vppcom_session_unbind(u32 session_handle)
{
    vcl_worker_t           *wrk = vcl_worker_get_current();
    ...
    while (clib_fifo_elts(session->accept_evts_fifo)) {
        ...
        vcl_send_session_accepted_reply(session->vpp_evt_q, accepted_msg->context, accepted_msg->handle, -1);
    }
    clib_fifo_free(session->accept_evts_fifo);

    vcl_send_session_unlisten(wrk, session);

    VDBG(0, "session %u [0x%llx]: sending unbind!", session->session_index, session->vpp_handle);
 ...
 */
void vppcom_app_exit(void)
{
    if (!pool_elts(vcm->workers)) return;
    vcl_worker_cleanup(vcl_worker_get_current(), 1 /* notify vpp */);
    vcl_set_worker_index(~0);
    vcl_elog_stop(vcm);
}

static int vcl_api_attach(void)
{
    if (vcm->cfg.vpp_app_socket_api) return vcl_sapi_attach();

    return vcl_bapi_attach();
}

int vcl_is_first_reattach_to_execute()
{
    if (vcm->reattach_count == 0) return 1;

    return 0;
}

void vcl_set_reattach_counter()
{
    ++vcm->reattach_count;

    if (vcm->reattach_count == vec_len(vcm->workers)) vcm->reattach_count = 0;
}
...
static void vcl_api_retry_attach(vcl_worker_t *wrk)
{
    vcl_session_t *s;
    ...
    pool_foreach (s, wrk->sessions) {
        if (s->flags & VCL_SESSION_F_IS_VEP) continue;
...
static void vcl_api_handle_disconnect(vcl_worker_t *wrk)
{
    wrk->api_client_handle = ~0;
    vcl_worker_detach_sessions(wrk);
}

static void vcl_api_detach(vcl_worker_t *wrk)
{
    if (wrk->api_client_handle == ~0) return;

    vcl_send_app_detach(wrk);

    if (vcm->cfg.vpp_app_socket_api) return vcl_sapi_detach(wrk);
 ...
 */
int vppcom_app_create(const char *app_name)
{
    ...
    fifo_segment_main_init(&vcm->segment_main, (uword) ~0, 20 /* timeout in secs */);
    ...
    VDBG(0, "app_name '%s', my_client_index %d (0x%x)", app_name, vcm->workers[0].api_client_handle, vcm->workers[0].api_client_handle);
...
void vppcom_app_destroy(void)
{
    ...
    pool_foreach (wrk, vcm->workers) {
        if (current_wrk != wrk) vcl_worker_cleanup(wrk, 0 /* notify vpp */);
    }
...
int vppcom_session_create(u8 proto, u8 is_nonblocking)
{
    vcl_worker_t  *wrk = vcl_worker_get_current();
    vcl_session_t *session;
    ...
    if (is_nonblocking) vcl_session_set_attr(session, VCL_SESS_ATTR_NONBLOCK);

    vcl_evt(VCL_EVT_CREATE, session, session_type, session->session_state, is_nonblocking, session_index);
...
static void vcl_epoll_lt_add(vcl_worker_t *wrk, vcl_session_t *s)
{
...
static void vcl_epoll_lt_del(vcl_worker_t *wrk, vcl_session_t *s)
{
    ...
    if (s->session_index == wrk->ep_lt_current) wrk->ep_lt_current = s->vep.lt_next;

    s->vep.lt_next = VCL_INVALID_SESSION_INDEX;
    s->vep.lt_prev = VCL_INVALID_SESSION_INDEX;
}

int vcl_session_cleanup(vcl_worker_t *wrk, vcl_session_t *s, vcl_session_handle_t sh, u8 do_disconnect)
{
    int rv = VPPCOM_OK;
    ...
    if (s->flags & VCL_SESSION_F_IS_VEP) {
        ...
        while (next_sh != ~0) {
            ...
            if (PREDICT_FALSE(rv < 0))
                VDBG(0,
                     "vpp handle 0x%llx, sh %u: EPOLL_CTL_DEL vep_idx %u"
                     " failed! rv %d (%s)",
                     s->vpp_handle,
                     next_sh,
                     s->vep.vep_sh,
    ...
    if (s->flags & VCL_SESSION_F_IS_VEP_SESSION) {
        ...
        if (rv < 0)
            VDBG(0,
                 "session %u [0x%llx]: EPOLL_CTL_DEL vep_idx %u "
                 "failed! rv %d (%s)",
                 s->session_index,
                 s->vpp_handle,
                 s->vep.vep_sh,
    ...
    if (!do_disconnect) {
        VDBG(1, "session %u [0x%llx] disconnect skipped", s->session_index, s->vpp_handle);
        goto cleanup;
    }
    ...
    if (s->session_state == VCL_STATE_LISTEN) {
        ...
        if (PREDICT_FALSE(rv < 0))
            VDBG(0,
                 "session %u [0x%llx]: listener unbind failed! "
                 "rv %d (%s)",
                 s->session_index,
                 s->vpp_handle,
                 rv,
    ...
    else if (vcl_session_is_ready(s) || (vcl_session_is_connectable_listener(wrk, s))) {
        rv = vppcom_session_disconnect(sh);
        if (PREDICT_FALSE(rv < 0))
            VDBG(0,
                 "ERROR: session %u [0x%llx]: disconnect failed!"
                 " rv %d (%s)",
                 s->session_index,
                 s->vpp_handle,
                 rv,
        ...
        if (!(s->flags & VCL_SESSION_F_PENDING_DISCONNECT)) goto free_session;
...
int vppcom_session_close(uint32_t session_handle)
{
    vcl_worker_t  *wrk = vcl_worker_get_current();
    vcl_session_t *session;
    ...
    session = vcl_session_get_w_handle(wrk, session_handle);
    if (!session) return VPPCOM_EBADFD;
    return vcl_session_cleanup(wrk, session, session_handle, 1 /* do_disconnect */);
}

int vppcom_session_bind(uint32_t session_handle, vppcom_endpt_t *ep)
{
    vcl_worker_t  *wrk     = vcl_worker_get_current();
    vcl_session_t *session = 0;

    if (!ep || !ep->ip) return VPPCOM_EINVAL;

    session = vcl_session_get_w_handle(wrk, session_handle);
    if (!session) return VPPCOM_EBADFD;

    if (session->flags & VCL_SESSION_F_IS_VEP) {
        VDBG(0, "ERROR: cannot bind to epoll session %u!", session->session_index);
        return VPPCOM_EBADFD;
    }

    session->transport.is_ip4 = ep->is_ip4;
    if (ep->is_ip4)
        clib_memcpy_fast(&session->transport.lcl_ip.ip4, ep->ip, sizeof(ip4_address_t));
    else
        clib_memcpy_fast(&session->transport.lcl_ip.ip6, ep->ip, sizeof(ip6_address_t));
    ...
    if (session->session_type == VPPCOM_PROTO_UDP) return vppcom_session_listen(session_handle, 10);

    return VPPCOM_OK;
}

int vppcom_session_listen(uint32_t listen_sh, uint32_t q_len)
{
    vcl_worker_t  *wrk            = vcl_worker_get_current();
    ...
    if (!listen_session || (listen_session->flags & VCL_SESSION_F_IS_VEP)) return VPPCOM_EBADFD;

    listen_vpp_handle = listen_session->vpp_handle;
    if (listen_session->session_state == VCL_STATE_LISTEN) {
        VDBG(0, "session %u [0x%llx]: already in listen state!", listen_sh, listen_vpp_handle);
        return VPPCOM_OK;
    }
    ...
    if (PREDICT_FALSE(rv)) {
        listen_session = vcl_session_get_w_handle(wrk, listen_sh);
        VDBG(0, "session %u [0x%llx]: listen failed! returning %d (%s)", listen_sh, listen_session->vpp_handle, rv, vppcom_retval_str(rv));
        return rv;
    }
...
int vppcom_unformat_proto(uint8_t *proto, char *proto_str)
{
...
int vppcom_session_accept(uint32_t ls_handle, vppcom_endpt_t *ep, uint32_t flags)
{
...
again:
    ...
    if (!ls) return VPPCOM_EBADFD;

    if ((ls->session_state != VCL_STATE_LISTEN) && (ls->session_state != VCL_STATE_LISTEN_NO_MQ) && (!vcl_session_is_connectable_listener(wrk, ls))) {
        VDBG(0, "ERROR: session [0x%llx]: not in listen state! state (%s)", ls->vpp_handle, vcl_session_state_str(ls->session_state));
        return VPPCOM_EBADFD;
    }
    ...
    while (1) {
        if (svm_msg_q_is_empty(wrk->app_event_queue) && is_nonblocking) return VPPCOM_EAGAIN;
...
handle:
    ...
    client_session_index = vcl_session_accepted_handler(wrk, &accepted_msg, ls_index);
    if (client_session_index == VCL_INVALID_SESSION_INDEX) return VPPCOM_ECONNABORTED;

    ls             = vcl_session_get(wrk, ls_index);
    client_session = vcl_session_get(wrk, client_session_index);

    if (flags & O_NONBLOCK) vcl_session_set_attr(client_session, VCL_SESS_ATTR_NONBLOCK);
    ...
    if (ep) {
        ...
        if (client_session->transport.is_ip4)
            clib_memcpy_fast(ep->ip, &client_session->transport.rmt_ip.ip4, sizeof(ip4_address_t));
        else
            clib_memcpy_fast(ep->ip, &client_session->transport.rmt_ip.ip6, sizeof(ip6_address_t));
    }
    ...
    VDBG(0,
         ...
         vcl_format_ip46_address,
         &client_session->transport.lcl_ip,
         client_session->transport.is_ip4 ? IP46_TYPE_IP4 : IP46_TYPE_IP6,
...
int vppcom_session_connect(uint32_t session_handle, vppcom_endpt_t *server_ep)
{
    vcl_worker_t  *wrk     = vcl_worker_get_current();
    vcl_session_t *session = 0;
    u32            session_index;
    ...
    if (PREDICT_FALSE(session->session_state == VCL_STATE_LISTEN)) {
        if (session->session_type != VPPCOM_PROTO_UDP) return VPPCOM_EINVAL;
        vcl_send_session_unlisten(wrk, session);
        session->session_state = VCL_STATE_CLOSED;
    }
    ...
    VDBG(0,
         "session %u: connecting to peer %U:%d proto %s",
         session->session_index,
         vcl_format_ip46_address,
         &session->transport.rmt_ip,
         session->transport.is_ip4 ? IP46_TYPE_IP4 : IP46_TYPE_IP6,
     ...
     */
    rv = vppcom_wait_for_session_state_change(session_index, VCL_STATE_READY, vcm->cfg.session_timeout);

    session = vcl_session_get(wrk, session_index);
    VDBG(0, "session %u [0x%llx]: connect %s!", session->session_index, session->vpp_handle, rv ? "failed" : "succeeded");

    return rv;
}

int vppcom_session_stream_connect(uint32_t session_handle, uint32_t parent_session_handle)
{
    vcl_worker_t  *wrk = vcl_worker_get_current();
    ...
    if (!parent_session) return VPPCOM_EBADFD;

    session_index        = session->session_index;
    parent_session_index = parent_session->session_index;
    if (PREDICT_FALSE(session->flags & VCL_SESSION_F_IS_VEP)) {
        VDBG(0, "ERROR: cannot connect epoll session %u!", session->session_index);
        return VPPCOM_EBADFD;
    }
    ...
    VDBG(0, "session handle %u: connecting to session %u [0x%llx]", session_handle, parent_session_handle, parent_session->vpp_handle);
    ...
    parent_session          = vcl_session_get_w_handle(wrk, parent_session_handle);
    if (parent_session) parent_session->n_accepted_sessions++;
...
static inline int vppcom_session_read_internal(uint32_t session_handle, void *buf, int n, u8 peek)
{
    vcl_worker_t    *wrk = vcl_worker_get_current();
    ...
    if (PREDICT_FALSE(!buf)) return VPPCOM_EFAULT;

    s = vcl_session_get_w_handle(wrk, session_handle);
    if (PREDICT_FALSE(!s || (s->flags & VCL_SESSION_F_IS_VEP))) return VPPCOM_EBADFD;

    if (PREDICT_FALSE(!vcl_session_is_open(s))) {
        VDBG(0,
             "session %u[0x%llx] is not open! state 0x%x (%s)",
             s->session_index,
             s->vpp_handle,
             s->session_state,
    ...
    if (svm_fifo_is_empty_cons(rx_fifo)) {
        if (is_ct) svm_fifo_unset_event(s->rx_fifo);
        svm_fifo_unset_event(rx_fifo);
        if (is_nonblocking) {
            if (vcl_session_is_closing(s)) return vcl_session_closing_error(s);
...
read_again:
    ...
    if (svm_fifo_is_empty_cons(rx_fifo)) {
        if (is_ct) svm_fifo_unset_event(s->rx_fifo);
        svm_fifo_unset_event(rx_fifo);
        if (!svm_fifo_is_empty_cons(rx_fifo) && svm_fifo_set_event(rx_fifo) && is_nonblocking) {
    ...
    if (PREDICT_FALSE(svm_fifo_needs_deq_ntf(rx_fifo, n_read))) {
        ...
        app_send_io_evt_to_vpp(s->vpp_evt_q, s->rx_fifo->shr->master_session_index, SESSION_IO_EVT_RX, SVM_Q_WAIT);
    }

    VDBG(2, "session %u[0x%llx]: read %d bytes from (%p)", s->session_index, s->vpp_handle, n_read, rx_fifo);

    return n_read;
}

int vppcom_session_read(uint32_t session_handle, void *buf, size_t n)
{
    return (vppcom_session_read_internal(session_handle, buf, n, 0));
}

static int vppcom_session_peek(uint32_t session_handle, void *buf, int n)
{
    return (vppcom_session_read_internal(session_handle, buf, n, 1));
}

int vppcom_session_read_segments(uint32_t session_handle, vppcom_data_segment_t *ds, uint32_t n_segments, uint32_t max_bytes)
{
    vcl_worker_t  *wrk    = vcl_worker_get_current();
    ...
    if (PREDICT_FALSE(!vcl_session_is_open(s))) return vcl_session_closed_error(s);
    ...
    n_read = svm_fifo_segments(rx_fifo, s->rx_bytes_pending, (svm_fifo_seg_t *) ds, &n_segments, max_bytes);
    if (n_read < 0) return VPPCOM_EAGAIN;

    if (svm_fifo_max_dequeue_cons(rx_fifo) == n_read) {
        if (is_ct) svm_fifo_unset_event(s->rx_fifo);
        svm_fifo_unset_event(rx_fifo);
        if (svm_fifo_max_dequeue_cons(rx_fifo) != n_read && svm_fifo_set_event(rx_fifo) && vcl_session_has_attr(s, VCL_SESS_ATTR_NONBLOCK)) {
            session_event_t *e;
            vec_add2(wrk->unhandled_evts_vector, e, 1);
            e->event_type    = SESSION_IO_EVT_RX;
            e->session_index = s->session_index;
...
void vppcom_session_free_segments(uint32_t session_handle, uint32_t n_bytes)
{
    vcl_worker_t  *wrk = vcl_worker_get_current();
    vcl_session_t *s;
    u8             is_ct;
    ...
    s = vcl_session_get_w_handle(wrk, session_handle);
    if (PREDICT_FALSE(!s || (s->flags & VCL_SESSION_F_IS_VEP))) return;
...
always_inline u8 vcl_fifo_is_writeable(svm_fifo_t *f, u32 len, u8 is_dgram)
{
...
always_inline int vppcom_session_write_inline(vcl_worker_t *wrk, vcl_session_t *s, void *buf, size_t n, u8 is_flush, u8 is_dgram)
{
    ...
    if (PREDICT_FALSE(!n)) return VPPCOM_OK;

    if (PREDICT_FALSE(!buf)) return VPPCOM_EFAULT;

    if (PREDICT_FALSE(s->flags & VCL_SESSION_F_IS_VEP)) {
        VDBG(0,
             "ERROR: session %u [0x%llx]: cannot write to an epoll"
             " session!",
             s->session_index,
             s->vpp_handle);
        return VPPCOM_EBADFD;
    ...
    if (PREDICT_FALSE(s->flags & VCL_SESSION_F_WR_SHUTDOWN)) {
        VDBG(1,
             "session %u [0x%llx]: is shutdown! state 0x%x (%s)",
             s->session_index,
             s->vpp_handle,
             s->session_state,
             vcl_session_state_str(s->session_state));
    ...
    if (is_flush && !is_ct) et = SESSION_IO_EVT_TX_FLUSH;

    if (is_dgram) {
        et      = vcl_session_dgram_tx_evt(s, et);
        n_write = app_send_dgram_raw_gso(tx_fifo, &s->transport, s->vpp_evt_q, buf, n, s->gso_size, et, 0 /* do_evt */, SVM_Q_WAIT);
    }
    else {
        n_write = app_send_stream_raw(tx_fifo, s->vpp_evt_q, buf, n, et, 0 /* do_evt */, SVM_Q_WAIT);
    }

    if (svm_fifo_set_event(s->tx_fifo)) app_send_io_evt_to_vpp(s->vpp_evt_q, s->tx_fifo->shr->master_session_index, et, SVM_Q_WAIT);

    /* The underlying fifo segment can run out of memory */
    if (PREDICT_FALSE(n_write < 0)) return VPPCOM_EAGAIN;

    VDBG(2, "session %u [0x%llx]: wrote %d bytes", s->session_index, s->vpp_handle, n_write);

    return n_write;
}

int vppcom_session_write_segments(uint32_t session_handle, vppcom_data_segment_t *ds, uint32_t n_segments)
{
    vcl_worker_t  *wrk     = vcl_worker_get_current();
    ...
    if (PREDICT_FALSE(!ds)) return VPPCOM_EFAULT;

    /* Accept zero length writes but just return */
    if (PREDICT_FALSE(ds[0].len == 0)) return VPPCOM_OK;
    ...
    if (PREDICT_FALSE(s->flags & VCL_SESSION_F_WR_SHUTDOWN)) return VPPCOM_EPIPE;
    ...
    n_write = svm_fifo_enqueue_segments(tx_fifo, (svm_fifo_seg_t *) ds, n_segments, 0 /* allow_partial */);

    /* The underlying fifo segment can run out of memory */
    if (PREDICT_FALSE(n_write < 0)) return VPPCOM_EAGAIN;

    if (svm_fifo_set_event(s->tx_fifo)) app_send_io_evt_to_vpp(s->vpp_evt_q, s->tx_fifo->shr->master_session_index, SESSION_IO_EVT_TX, SVM_Q_WAIT);

    return n_write;
}

int vppcom_session_write(uint32_t session_handle, void *buf, size_t n)
{
    vcl_worker_t  *wrk = vcl_worker_get_current();
    vcl_session_t *s;

    s = vcl_session_get_w_handle(wrk, session_handle);
    if (PREDICT_FALSE(!s)) return VPPCOM_EBADFD;

    return vppcom_session_write_inline(wrk, s, buf, n, 0 /* is_flush */, s->is_dgram ? 1 : 0);
}

int vppcom_session_write_msg(uint32_t session_handle, void *buf, size_t n)
{
    vcl_worker_t  *wrk = vcl_worker_get_current();
    vcl_session_t *s;
    ...
    switch (e->event_type) {
        ...
        case SESSION_CTRL_EVT_CONNECTED:
            if (!e->postponed) {
                ...
                sid = e->session_index;
            if (sid == VCL_INVALID_SESSION_INDEX) break;
            if (!(sid < n_bits && write_map)) break;
            clib_bitmap_set_no_check((uword *) write_map, sid, 1);
            *bits_set += 1;
            s = vcl_session_get(wrk, sid);
        ...
        case SESSION_CTRL_EVT_DISCONNECTED:
            ...
            if (vcl_session_is_closed(s)) {
                if (s && (s->flags & VCL_SESSION_F_PENDING_FREE)) vcl_session_free(wrk, s);
                break;
            }
            sid = s->session_index;
...
static int vcl_select_handle_mq(vcl_worker_t  *wrk,
                                svm_msg_q_t   *mq,
                                unsigned long  n_bits,
                                unsigned long *read_map,
                                unsigned long *write_map,
                                ...
                                u32           *bits_set)
{
    ...
    if (svm_msg_q_is_empty(mq)) {
        ...
        if (!time_to_wait)
            ...
            if (svm_msg_q_timedwait(mq, time_to_wait)) return 0;
        }
    }
    vcl_mq_dequeue_batch(wrk, mq, ~0);
    ...
    for (i = 0; i < vec_len(wrk->mq_msg_vector); i++) {
        msg = vec_elt_at_index(wrk->mq_msg_vector, i);
        e   = svm_msg_q_msg_data(mq, msg);
        vcl_select_handle_mq_event(wrk, e, n_bits, read_map, write_map, except_map, bits_set);
        svm_msg_q_free_msg(mq, msg);
    }
    vec_reset_length(wrk->mq_msg_vector);
    vcl_handle_pending_wrk_updates(wrk);
...
static int vppcom_select_condvar(vcl_worker_t *wrk,
                                 int           n_bits,
                                 vcl_si_set   *read_map,
                                 vcl_si_set   *write_map,
                                 vcl_si_set   *except_map,
                                 double        time_to_wait,
                                 u32          *bits_set)
{
    ...
    do {
        vcl_select_handle_mq(wrk, wrk->app_event_queue, n_bits, read_map, write_map, except_map, wait, bits_set);
        if (*bits_set) return *bits_set;
        if (wait == -1) continue;
...
static int vppcom_select_eventfd(vcl_worker_t *wrk,
                                 int           n_bits,
                                 vcl_si_set   *read_map,
                                 vcl_si_set   *write_map,
                                 vcl_si_set   *except_map,
                                 ...
                                 u32          *bits_set)
{
    ...
    if (time_to_wait > 0) end = clib_time_now(&wrk->clib_time) + (time_to_wait / 1e3);

    do {
        n_mq_evts = epoll_wait(wrk->mqs_epfd, wrk->mq_events, vec_len(wrk->mq_events), time_to_wait);
        if (n_mq_evts < 0) {
        ...
        for (i = 0; i < n_mq_evts; i++) {
            ...
            vcl_select_handle_mq(wrk, mqc->mq, n_bits, read_map, write_map, except_map, 0, bits_set);
        }

        if (*bits_set || !time_to_wait) return (int) *bits_set;
    } while (end == -1 || clib_time_now(&wrk->clib_time) < end);

    return 0;
}

int vppcom_select(int n_bits, vcl_si_set *read_map, vcl_si_set *write_map, vcl_si_set *except_map, double time_to_wait)
{
...
check_mq:
    ...
    for (i = 0; i < vec_len(wrk->unhandled_evts_vector); i++) {
        vcl_select_handle_mq_event(wrk, &wrk->unhandled_evts_vector[i], n_bits, read_map, write_map, except_map, &bits_set);
    }
    vec_reset_length(wrk->unhandled_evts_vector);

    if (vcm->cfg.use_mq_eventfd)
        vppcom_select_eventfd(wrk, n_bits, read_map, write_map, except_map, time_to_wait, &bits_set);
    else
        vppcom_select_condvar(wrk, n_bits, read_map, write_map, except_map, time_to_wait, &bits_set);

    return (bits_set);
}

static inline void vep_verify_epoll_chain(vcl_worker_t *wrk, u32 vep_handle)
{
    vppcom_epoll_t *vep;
    u32             sh = vep_handle;
    vcl_session_t  *s;

    if (VPPCOM_DEBUG <= 3) return;
    ...
    for (sh = vep->next_sh; sh != ~0; sh = vep->next_sh) {
        ...
        vep = &s->vep;
        if (PREDICT_FALSE(vep->vep_sh != vep_handle)) VDBG(0, "ERROR: session (%u) vep_sh (%u) != vep_sh (%u)!", sh, s->vep.vep_sh, vep_handle);
        if (s->flags & VCL_SESSION_F_IS_VEP_SESSION) {
            VDBG(0,
                 "vep_sh[%u]: sh 0x%x (%u)\n"
                 "{\n"
...
static void vcl_epoll_ctl_add_unhandled_event(vcl_worker_t *wrk, vcl_session_t *s, u32 is_epollet, session_evt_type_t evt)
{
    if (!is_epollet) {
        if (s->vep.lt_next == VCL_INVALID_SESSION_INDEX) vcl_epoll_lt_add(wrk, s);
        return;
    }
    ...
    e.session_index   = s->session_index;
    e.event_type      = evt;
    if (evt == SESSION_IO_EVT_RX) s->flags &= ~VCL_SESSION_F_HAS_RX_EVT;
    vec_add1(wrk->unhandled_evts_vector, e);
}

int vppcom_epoll_ctl(uint32_t vep_handle, int op, uint32_t session_handle, struct epoll_event *event)
{
    vcl_worker_t  *wrk = vcl_worker_get_current();
    ...
    switch (op) {
        case EPOLL_CTL_ADD:
            ...
            if (vep_session->vep.next_sh != ~0) {
                ...
                next_session = vcl_session_get_w_handle(wrk, vep_session->vep.next_sh);
                if (PREDICT_FALSE(!next_session)) {
                    VDBG(0,
                         "EPOLL_CTL_ADD: Invalid vep.next_sh (%u) on "
                         "vep_idx (%u)!",
            ...
            if ((event->events & EPOLLOUT)) {
                ...
                if (write_ready > 0) {
                    ...
                    vcl_epoll_ctl_add_unhandled_event(wrk, s, event->events & EPOLLET, SESSION_IO_EVT_TX);
                    add_evt = 1;
                }
                else {
                    vcl_session_add_want_deq_ntf(s, SVM_FIFO_WANT_DEQ_NOTIF);
            ...
            if ((event->events & EPOLLIN) && (vcl_session_read_ready(s) > 0)) {
                vcl_epoll_ctl_add_unhandled_event(wrk, s, event->events & EPOLLET, SESSION_IO_EVT_RX);
                add_evt = 1;
            }
            ...
            VDBG(1, "EPOLL_CTL_ADD: vep_sh %u, sh %u, events 0x%x, data 0x%llx!", vep_handle, session_handle, event->events, event->data.u64);
        ...
        case EPOLL_CTL_MOD:
            if (PREDICT_FALSE(!event)) {
                ...
                VDBG(0, "EPOLL_CTL_MOD: sh %u vep_sh (%u) != vep_sh (%u)!", session_handle, s->vep.vep_sh, vep_handle);
                rv = VPPCOM_EINVAL;
                goto done;
            }
            ...
            if ((event->events & EPOLLIN) && !(s->vep.ev.events & EPOLLIN) && (vcl_session_read_ready(s) > 0)) {
                vcl_epoll_ctl_add_unhandled_event(wrk, s, event->events & EPOLLET, SESSION_IO_EVT_RX);
            }
            s->vep.et_mask = VEP_DEFAULT_ET_MASK;
            s->vep.ev      = *event;
            ...
            VDBG(1, "EPOLL_CTL_MOD: vep_sh %u, sh %u, events 0x%x, data 0x%llx!", vep_handle, session_handle, event->events, event->data.u64);
            break;
        ...
        case EPOLL_CTL_DEL:
            if (PREDICT_FALSE(!(s->flags & VCL_SESSION_F_IS_VEP_SESSION))) {
                ...
                VDBG(0, "EPOLL_CTL_DEL: sh %u vep_sh (%u) != vep_sh (%u)!", session_handle, s->vep.vep_sh, vep_handle);
                rv = VPPCOM_EINVAL;
                goto done;
            }
            ...
            if (s->vep.prev_sh == vep_handle)
                ...
                if (PREDICT_FALSE(!prev_session)) {
                    VDBG(0, "EPOLL_CTL_DEL: Invalid prev_sh (%u) on sh (%u)!", s->vep.prev_sh, session_handle);
                    return VPPCOM_EBADFD;
                }
            ...
            if (s->vep.next_sh != ~0) {
                ...
                if (PREDICT_FALSE(!next_session)) {
                    VDBG(0, "EPOLL_CTL_DEL: Invalid next_sh (%u) on sh (%u)!", s->vep.next_sh, session_handle);
                    return VPPCOM_EBADFD;
                }
            ...
            if (s->vep.lt_next != VCL_INVALID_SESSION_INDEX) vcl_epoll_lt_del(wrk, s);
            ...
            if (vcl_session_is_open(s)) vcl_session_del_want_deq_ntf(s, SVM_FIFO_WANT_DEQ_NOTIF_IF_FULL);

            VDBG(1, "EPOLL_CTL_DEL: vep_idx %u, sh %u!", vep_handle, session_handle);
...
always_inline u8 vcl_ep_session_needs_evt(vcl_session_t *s, u32 evt)
{
    /* No event if not epolled / events reset on hup or level-trigger on */
    return ((s->vep.ev.events & evt) && s->vep.lt_next == VCL_INVALID_SESSION_INDEX);
}

static inline void vcl_epoll_wait_handle_mq_event(vcl_worker_t *wrk, session_event_t *e, struct epoll_event *events, u32 *num_ev)
{
    session_disconnected_msg_t *disconnected_msg;
    session_connected_msg_t    *connected_msg;
    ...
    switch (e->event_type) {
        case SESSION_IO_EVT_RX:
            sid = e->session_index;
            s   = vcl_session_get(wrk, sid);
            if (vcl_session_is_closed(s)) break;
            vcl_fifo_rx_evt_valid_or_break(s);
            if (!vcl_ep_session_needs_evt(s, EPOLLIN) || (s->flags & VCL_SESSION_F_HAS_RX_EVT)) break;
        ...
        case SESSION_IO_EVT_TX:
            ...
            s   = vcl_session_get(wrk, sid);
            if (!s || !vcl_session_is_open(s)) break;
            svm_fifo_reset_has_deq_ntf(vcl_session_is_ct(s) ? s->ct_tx_fifo : s->tx_fifo);
            if (!vcl_ep_session_needs_evt(s, EPOLLOUT)) break;
            session_events         = s->vep.ev.events;
            add_event              = 1;
        ...
        case SESSION_CTRL_EVT_ACCEPTED:
            if (!e->postponed)
                ...
                s = vcl_session_get(wrk, e->session_index);
            if (!s || !vcl_ep_session_needs_evt(s, EPOLLIN)) break;
            sid                    = s->session_index;
            session_events         = s->vep.ev.events;
            add_event              = 1;
            events[*num_ev].events = EPOLLIN;
        ...
        case SESSION_CTRL_EVT_CONNECTED:
            if (!e->postponed) {
                ...
                sid = e->session_index;
            s = vcl_session_get(wrk, sid);
            if (vcl_session_is_closed(s) || !vcl_ep_session_needs_evt(s, EPOLLOUT)) break;
            /* We didn't have a fifo when the event was added */
            vcl_session_add_want_deq_ntf(s, SVM_FIFO_WANT_DEQ_NOTIF_IF_FULL);
            add_event      = 1;
            session_events = s->vep.ev.events;
        ...
        case SESSION_CTRL_EVT_RESET:
            ...
            if ((EPOLLRDHUP & session_events) && (s->flags & VCL_SESSION_F_RD_SHUTDOWN)) {
                events[*num_ev].events |= EPOLLRDHUP;
            }
    ...
    if (add_event) {
        ...
        if (EPOLLONESHOT & session_events) {
            s = vcl_session_get(wrk, sid);
            if (!(events[*num_ev].events & EPOLLHUP)) s->vep.ev.events = EPOLLHUP | EPOLLERR;
        }
...
static int vcl_epoll_wait_handle_mq(vcl_worker_t *wrk, svm_msg_q_t *mq, struct epoll_event *events, u32 maxevents, double wait_for_time, u32 *num_ev)
{
    svm_msg_q_msg_t *msg;
    session_event_t *e;
    int              i;

    if (vec_len(wrk->mq_msg_vector) && svm_msg_q_is_empty(mq)) goto handle_dequeued;
    ...
    if (svm_msg_q_is_empty(mq)) {
        if (!wait_for_time)
            ...
            if (svm_msg_q_timedwait(mq, wait_for_time / 1e3)) return 0;
        }
    }
...
static int vppcom_epoll_wait_condvar(vcl_worker_t *wrk, struct epoll_event *events, int maxevents, u32 n_evts, double timeout_ms)
{
    double end = -1;

    if (!n_evts) {
        if (timeout_ms > 0) end = clib_time_now(&wrk->clib_time) + (timeout_ms / 1e3);
    }

    do {
        vcl_epoll_wait_handle_mq(wrk, wrk->app_event_queue, events, maxevents, timeout_ms, &n_evts);
        if (n_evts || !timeout_ms) return n_evts;
    } while (end == -1 || clib_time_now(&wrk->clib_time) < end);

    return 0;
}

static int vppcom_epoll_wait_eventfd(vcl_worker_t *wrk, struct epoll_event *events, int maxevents, u32 n_evts, double timeout_ms)
{
    int __clib_unused  n_read;
    vcl_mq_evt_conn_t *mqc;
    int                n_mq_evts, i;
    ...
    do {
        n_mq_evts = epoll_wait(wrk->mqs_epfd, wrk->mq_events, vec_len(wrk->mq_events), timeout_ms);
        if (n_mq_evts < 0) {
            if (errno == EINTR) continue;
...
static void vcl_epoll_wait_handle_lt(vcl_worker_t *wrk, struct epoll_event *events, int maxevents, u32 *n_evts)
{
    ...
    do {
        ...
        if (add_event) {
            ...
            if (EPOLLONESHOT & s->vep.ev.events) s->vep.ev.events = EPOLLHUP | EPOLLERR;
            if (evt_flags & EPOLLHUP) s->vep.ev.events = 0;
            *n_evts += 1;
            add_event = 0;
            evt_flags = 0;
            if (*n_evts == maxevents) {
...
int vppcom_epoll_wait(uint32_t vep_handle, struct epoll_event *events, int maxevents, double wait_for_time)
{
    vcl_worker_t  *wrk = vcl_worker_get_current();
    vcl_session_t *vep_session;
    u32            n_evts = 0;
    ...
    vep_session = vcl_session_get_w_handle(wrk, vep_handle);
    if (!vep_session) return VPPCOM_EBADFD;
    ...
    if (vec_len(wrk->unhandled_evts_vector)) {
        for (i = 0; i < vec_len(wrk->unhandled_evts_vector); i++) {
            vcl_epoll_wait_handle_mq_event(wrk, &wrk->unhandled_evts_vector[i], events, &n_evts);
            if (n_evts == maxevents) {
    ...
    if (PREDICT_FALSE(wrk->ep_lt_current != VCL_INVALID_SESSION_INDEX)) vcl_epoll_wait_handle_lt(wrk, events, maxevents, &n_evts);

    /* Request to only drain unhandled */
    if ((int) wait_for_time == -2) return n_evts;

    if (vcm->cfg.use_mq_eventfd)
        n_evts = vppcom_epoll_wait_eventfd(wrk, events, maxevents, n_evts, wait_for_time);
    else
        n_evts = vppcom_epoll_wait_condvar(wrk, events, maxevents, n_evts, wait_for_time);

    return n_evts;
}

int vppcom_session_attr(uint32_t session_handle, uint32_t op, void *buffer, uint32_t *buflen)
{
    vcl_worker_t          *wrk   = vcl_worker_get_current();
    u32                   *flags = buffer;
    ...
    switch (op) {
        case VPPCOM_ATTR_GET_NREAD:
            ...
            VDBG(2, "VPPCOM_ATTR_GET_NREAD: sh %u, nread = %d", session_handle, rv);
            break;

        case VPPCOM_ATTR_GET_NWRITE:
            rv = vcl_session_write_ready(session);
            VDBG(2, "VPPCOM_ATTR_GET_NWRITE: sh %u, nwrite = %d", session_handle, rv);
            break;
        ...
        case VPPCOM_ATTR_GET_FLAGS:
            if (PREDICT_TRUE(buffer && buflen && (*buflen >= sizeof(*flags)))) {
                *flags  = O_RDWR | (vcl_session_has_attr(session, VCL_SESS_ATTR_NONBLOCK) ? O_NONBLOCK : 0);
                *buflen = sizeof(*flags);
                VDBG(2,
                     "VPPCOM_ATTR_GET_FLAGS: sh %u, flags = 0x%08x, "
                     "is_nonblocking = %u",
                     session_handle,
                     *flags,
        ...
        case VPPCOM_ATTR_SET_FLAGS:
            if (PREDICT_TRUE(buffer && buflen && (*buflen == sizeof(*flags)))) {
                ...
                VDBG(2,
                     "VPPCOM_ATTR_SET_FLAGS: sh %u, flags = 0x%08x,"
                     " is_nonblocking = %u",
                     session_handle,
                     *flags,
                     vcl_session_has_attr(session, VCL_SESS_ATTR_NONBLOCK));
        ...
        case VPPCOM_ATTR_GET_PEER_ADDR:
            if (PREDICT_TRUE(buffer && buflen && (*buflen >= sizeof(*ep)) && ep->ip)) {
                ep->is_ip4 = session->transport.is_ip4;
                ep->port   = session->transport.rmt_port;
                if (session->transport.is_ip4)
                    clib_memcpy_fast(ep->ip, &session->transport.rmt_ip.ip4, sizeof(ip4_address_t));
                else
                    clib_memcpy_fast(ep->ip, &session->transport.rmt_ip.ip6, sizeof(ip6_address_t));
                *buflen = sizeof(*ep);
                VDBG(1,
        ...
        case VPPCOM_ATTR_GET_LCL_ADDR:
            if (PREDICT_TRUE(buffer && buflen && (*buflen >= sizeof(*ep)) && ep->ip)) {
                ...
                if (session->transport.is_ip4)
                    clib_memcpy_fast(ep->ip, &session->transport.lcl_ip.ip4, sizeof(ip4_address_t));
                else
                    clib_memcpy_fast(ep->ip, &session->transport.lcl_ip.ip6, sizeof(ip6_address_t));
                *buflen = sizeof(*ep);
                VDBG(1,
        ...
        case VPPCOM_ATTR_GET_ORIGINAL_DST:
            ...
            if (PREDICT_TRUE(buffer && buflen && (*buflen >= sizeof(*ep)) && ep->ip)) {
                ...
                clib_memcpy_fast(ep->ip, &session->original_dst_ip4, sizeof(ip4_address_t));
                *buflen = sizeof(*ep);
                VDBG(1,
        ...
        case VPPCOM_ATTR_SET_LIBC_EPFD:
            if (PREDICT_TRUE(buffer && buflen && (*buflen == sizeof(session->libc_epfd)))) {
                session->libc_epfd = *(int *) buffer;
                *buflen            = sizeof(session->libc_epfd);

                VDBG(2, "VPPCOM_ATTR_SET_LIBC_EPFD: libc_epfd %d, buflen %d", session->libc_epfd, *buflen);
            }
            else
                rv = VPPCOM_EINVAL;
            break;
        ...
        case VPPCOM_ATTR_GET_PROTOCOL:
            if (buffer && buflen && (*buflen >= sizeof(int))) {
                ...
                *buflen         = sizeof(int);
                ...
                VDBG(2, "VPPCOM_ATTR_GET_PROTOCOL: %d (%s), buflen %d", *(int *) buffer, *(int *) buffer ? "UDP" : "TCP", *buflen);
            }
            else
                rv = VPPCOM_EINVAL;
            break;
        ...
        case VPPCOM_ATTR_GET_LISTEN:
            if (buffer && buflen && (*buflen >= sizeof(int))) {
                *(int *) buffer = vcl_session_has_attr(session, VCL_SESS_ATTR_LISTEN);
                *buflen         = sizeof(int);

                VDBG(2, "VPPCOM_ATTR_GET_LISTEN: %d, buflen %d", *(int *) buffer, *buflen);
            }
            else
                rv = VPPCOM_EINVAL;
            break;
        ...
        case VPPCOM_ATTR_GET_TX_FIFO_LEN:
            if (buffer && buflen && (*buflen >= sizeof(u32))) {
                ...
                *buflen            = sizeof(u32);
                ...
                VDBG(2,
                     "VPPCOM_ATTR_GET_TX_FIFO_LEN: %u (0x%x), buflen %d,"
                     " #VPP-TBD#",
                     *(size_t *) buffer,
                     *(size_t *) buffer,
                     *buflen);
        ...
        case VPPCOM_ATTR_SET_DSCP:
            if (buffer && buflen && (*buflen >= sizeof(u8))) {
                ...
                VDBG(2, "VPPCOM_ATTR_SET_DSCP: %u (0x%x), buflen %d,", *(u8 *) buffer, *(u8 *) buffer, *buflen);
            }
            else
                rv = VPPCOM_EINVAL;
            break;
        ...
        case VPPCOM_ATTR_SET_TX_FIFO_LEN:
            if (buffer && buflen && (*buflen == sizeof(u32))) {
                ...
                VDBG(2,
                     "VPPCOM_ATTR_SET_TX_FIFO_LEN: %u (0x%x), buflen %d,"
                     " #VPP-TBD#",
                     session->sndbuf_size,
                     session->sndbuf_size,
                     *buflen);
        ...
        case VPPCOM_ATTR_GET_RX_FIFO_LEN:
            if (buffer && buflen && (*buflen >= sizeof(u32))) {
                ...
                *buflen            = sizeof(u32);
                ...
                VDBG(2,
                     "VPPCOM_ATTR_GET_RX_FIFO_LEN: %u (0x%x), buflen %d, "
                     "#VPP-TBD#",
                     *(size_t *) buffer,
                     *(size_t *) buffer,
                     *buflen);
        ...
        case VPPCOM_ATTR_SET_RX_FIFO_LEN:
            if (buffer && buflen && (*buflen == sizeof(u32))) {
                ...
                VDBG(2,
                     "VPPCOM_ATTR_SET_RX_FIFO_LEN: %u (0x%x), buflen %d,"
                     " #VPP-TBD#",
                     session->sndbuf_size,
                     session->sndbuf_size,
                     *buflen);
        ...
        case VPPCOM_ATTR_GET_REUSEADDR:
            if (buffer && buflen && (*buflen >= sizeof(int))) {
                /* VPP-TBD */
                *(int *) buffer = vcl_session_has_attr(session, VCL_SESS_ATTR_REUSEADDR);
                *buflen         = sizeof(int);

                VDBG(2, "VPPCOM_ATTR_GET_REUSEADDR: %d, buflen %d, #VPP-TBD#", *(int *) buffer, *buflen);
            }
            else
                rv = VPPCOM_EINVAL;
            break;
        ...
        case VPPCOM_ATTR_SET_REUSEADDR:
            if (buffer && buflen && (*buflen == sizeof(int)) && !vcl_session_has_attr(session, VCL_SESS_ATTR_LISTEN)) {
                /* VPP-TBD */
                if (*(int *) buffer)
                ...
                VDBG(2, "VPPCOM_ATTR_SET_REUSEADDR: %d, buflen %d, #VPP-TBD#", vcl_session_has_attr(session, VCL_SESS_ATTR_REUSEADDR), *buflen);
            }
            else
                rv = VPPCOM_EINVAL;
            break;
        ...
        case VPPCOM_ATTR_GET_REUSEPORT:
            if (buffer && buflen && (*buflen >= sizeof(int))) {
                /* VPP-TBD */
                *(int *) buffer = vcl_session_has_attr(session, VCL_SESS_ATTR_REUSEPORT);
                *buflen         = sizeof(int);

                VDBG(2, "VPPCOM_ATTR_GET_REUSEPORT: %d, buflen %d, #VPP-TBD#", *(int *) buffer, *buflen);
            }
            else
                rv = VPPCOM_EINVAL;
            break;
        ...
        case VPPCOM_ATTR_SET_REUSEPORT:
            if (buffer && buflen && (*buflen == sizeof(int)) && !vcl_session_has_attr(session, VCL_SESS_ATTR_LISTEN)) {
                ...
                VDBG(2, "VPPCOM_ATTR_SET_REUSEPORT: %d, buflen %d, #VPP-TBD#", vcl_session_has_attr(session, VCL_SESS_ATTR_REUSEPORT), *buflen);
            }
            else
                rv = VPPCOM_EINVAL;
            break;
        ...
        case VPPCOM_ATTR_GET_BROADCAST:
            if (buffer && buflen && (*buflen >= sizeof(int))) {
                /* VPP-TBD */
                *(int *) buffer = vcl_session_has_attr(session, VCL_SESS_ATTR_BROADCAST);
                *buflen         = sizeof(int);

                VDBG(2, "VPPCOM_ATTR_GET_BROADCAST: %d, buflen %d, #VPP-TBD#", *(int *) buffer, *buflen);
            }
            else
                rv = VPPCOM_EINVAL;
            break;
        ...
        case VPPCOM_ATTR_SET_BROADCAST:
            if (buffer && buflen && (*buflen == sizeof(int))) {
                ...
                VDBG(2, "VPPCOM_ATTR_SET_BROADCAST: %d, buflen %d, #VPP-TBD#", vcl_session_has_attr(session, VCL_SESS_ATTR_BROADCAST), *buflen);
            }
            else
                rv = VPPCOM_EINVAL;
            break;
        ...
        case VPPCOM_ATTR_GET_V6ONLY:
            if (buffer && buflen && (*buflen >= sizeof(int))) {
                /* VPP-TBD */
                *(int *) buffer = vcl_session_has_attr(session, VCL_SESS_ATTR_V6ONLY);
                *buflen         = sizeof(int);

                VDBG(2, "VPPCOM_ATTR_GET_V6ONLY: %d, buflen %d, #VPP-TBD#", *(int *) buffer, *buflen);
            }
            else
                rv = VPPCOM_EINVAL;
            break;
        ...
        case VPPCOM_ATTR_SET_V6ONLY:
            if (buffer && buflen && (*buflen == sizeof(int))) {
                ...
                VDBG(2, "VPPCOM_ATTR_SET_V6ONLY: %d, buflen %d, #VPP-TBD#", vcl_session_has_attr(session, VCL_SESS_ATTR_V6ONLY), *buflen);
            }
            else
                rv = VPPCOM_EINVAL;
            break;
        ...
        case VPPCOM_ATTR_GET_KEEPALIVE:
            if (buffer && buflen && (*buflen >= sizeof(int))) {
                /* VPP-TBD */
                *(int *) buffer = vcl_session_has_attr(session, VCL_SESS_ATTR_KEEPALIVE);
                *buflen         = sizeof(int);

                VDBG(2, "VPPCOM_ATTR_GET_KEEPALIVE: %d, buflen %d, #VPP-TBD#", *(int *) buffer, *buflen);
            }
            else
                rv = VPPCOM_EINVAL;
            break;
        ...
        case VPPCOM_ATTR_SET_KEEPALIVE:
            if (buffer && buflen && (*buflen == sizeof(int))) {
                ...
                VDBG(2, "VPPCOM_ATTR_SET_KEEPALIVE: %d, buflen %d, #VPP-TBD#", vcl_session_has_attr(session, VCL_SESS_ATTR_KEEPALIVE), *buflen);
            }
            else
                rv = VPPCOM_EINVAL;
            break;
        ...
        case VPPCOM_ATTR_GET_TCP_NODELAY:
            if (buffer && buflen && (*buflen >= sizeof(int))) {
                /* VPP-TBD */
                *(int *) buffer = vcl_session_has_attr(session, VCL_SESS_ATTR_TCP_NODELAY);
                *buflen         = sizeof(int);

                VDBG(2, "VPPCOM_ATTR_GET_TCP_NODELAY: %d, buflen %d, #VPP-TBD#", *(int *) buffer, *buflen);
            }
            else
                rv = VPPCOM_EINVAL;
            break;
        ...
        case VPPCOM_ATTR_SET_TCP_NODELAY:
            if (buffer && buflen && (*buflen == sizeof(int))) {
                ...
                VDBG(2, "VPPCOM_ATTR_SET_TCP_NODELAY: %d, buflen %d, #VPP-TBD#", vcl_session_has_attr(session, VCL_SESS_ATTR_TCP_NODELAY), *buflen);
            }
            else
                rv = VPPCOM_EINVAL;
            break;
        ...
        case VPPCOM_ATTR_GET_TCP_KEEPIDLE:
            if (buffer && buflen && (*buflen >= sizeof(int))) {
                /* VPP-TBD */
                *(int *) buffer = vcl_session_has_attr(session, VCL_SESS_ATTR_TCP_KEEPIDLE);
                *buflen         = sizeof(int);

                VDBG(2, "VPPCOM_ATTR_GET_TCP_KEEPIDLE: %d, buflen %d, #VPP-TBD#", *(int *) buffer, *buflen);
            }
            else
                rv = VPPCOM_EINVAL;
            break;
        ...
        case VPPCOM_ATTR_SET_TCP_KEEPIDLE:
            if (buffer && buflen && (*buflen == sizeof(int))) {
                ...
                VDBG(2, "VPPCOM_ATTR_SET_TCP_KEEPIDLE: %d, buflen %d, #VPP-TBD#", vcl_session_has_attr(session, VCL_SESS_ATTR_TCP_KEEPIDLE), *buflen);
            }
            else
                rv = VPPCOM_EINVAL;
            break;
        ...
        case VPPCOM_ATTR_GET_TCP_KEEPINTVL:
            if (buffer && buflen && (*buflen >= sizeof(int))) {
                /* VPP-TBD */
                *(int *) buffer = vcl_session_has_attr(session, VCL_SESS_ATTR_TCP_KEEPINTVL);
                *buflen         = sizeof(int);

                VDBG(2, "VPPCOM_ATTR_GET_TCP_KEEPINTVL: %d, buflen %d, #VPP-TBD#", *(int *) buffer, *buflen);
            }
            else
                rv = VPPCOM_EINVAL;
            break;
        ...
        case VPPCOM_ATTR_SET_TCP_KEEPINTVL:
            if (buffer && buflen && (*buflen == sizeof(int))) {
                ...
                VDBG(2,
                     "VPPCOM_ATTR_SET_TCP_KEEPINTVL: %d, buflen %d, #VPP-TBD#",
                     vcl_session_has_attr(session, VCL_SESS_ATTR_TCP_KEEPINTVL),
                     *buflen);
            }
            else
                rv = VPPCOM_EINVAL;
        ...
        case VPPCOM_ATTR_GET_TCP_USER_MSS:
            ...
            tea.mss  = *(u32 *) buffer;
            if (vcl_session_transport_attr(wrk, session, 1 /* is_get */, &tea)) rv = VPPCOM_ENOPROTOOPT;
            ...
            VDBG(2, "VPPCOM_ATTR_GET_TCP_USER_MSS: %d, buflen %d", *(int *) buffer, *buflen);
            break;
        case VPPCOM_ATTR_SET_TCP_USER_MSS:
            ...
            tea.type = TRANSPORT_ENDPT_ATTR_MSS;
            tea.mss  = *(u32 *) buffer;
            if (vcl_session_transport_attr(wrk, session, 0 /* is_get */, &tea)) rv = VPPCOM_ENOPROTOOPT;

            VDBG(2, "VPPCOM_ATTR_SET_TCP_USER_MSS: %u, buflen %d", tea.mss, *buflen);
            break;
        ...
        case VPPCOM_ATTR_SET_CKPAIR:
            if (!(buffer && buflen && (*buflen == sizeof(int))) || !vcl_session_has_crypto(session)) {
                rv = VPPCOM_EINVAL;
                break;
            }
            if (!session->ext_config) {
                vcl_session_alloc_ext_cfg(session, TRANSPORT_ENDPT_EXT_CFG_CRYPTO, sizeof(transport_endpt_ext_cfg_t));
            }
        ...
        case VPPCOM_ATTR_GET_DOMAIN:
            ...
            *buflen = sizeof(int);
            ...
            VDBG(2, "VPPCOM_ATTR_GET_DOMAIN: %d, buflen %u", *(int *) buffer, *buflen);
            break;
        ...
        case VPPCOM_ATTR_SET_ENDPT_EXT_CFG:
            ...
            vcl_session_alloc_ext_cfg(session, TRANSPORT_ENDPT_EXT_CFG_NONE, *buflen + sizeof(u32));
            clib_memcpy(session->ext_config->data, buffer, *buflen);
            session->ext_config->len = *buflen;
            break;
        case VPPCOM_ATTR_SET_IP_PKTINFO:
            if (buffer && buflen && (*buflen == sizeof(int)) && !vcl_session_has_attr(session, VCL_SESS_ATTR_IP_PKTINFO)) {
                if (*(int *) buffer)
                    vcl_session_set_attr(session, VCL_SESS_ATTR_IP_PKTINFO);
                else
                    vcl_session_clear_attr(session, VCL_SESS_ATTR_IP_PKTINFO);

                VDBG(2, "VCL_SESS_ATTR_IP_PKTINFO: %d, buflen %d", vcl_session_has_attr(session, VCL_SESS_ATTR_IP_PKTINFO), *buflen);
            }
            else
                rv = VPPCOM_EINVAL;
            break;
        ...
        case VPPCOM_ATTR_GET_IP_PKTINFO:
            if (buffer && buflen && (*buflen >= sizeof(int))) {
                *(int *) buffer = vcl_session_has_attr(session, VCL_SESS_ATTR_IP_PKTINFO);
                *buflen         = sizeof(int);

                VDBG(2, "VCL_SESS_ATTR_IP_PKTINFO: %d, buflen %d", *(int *) buffer, *buflen);
            }
            else
                rv = VPPCOM_EINVAL;
            break;
...
int vppcom_session_recvfrom(uint32_t session_handle, void *buffer, uint32_t buflen, int flags, vppcom_endpt_t *ep)
{
    vcl_worker_t  *wrk = vcl_worker_get_current();
    vcl_session_t *session;
    int            rv = VPPCOM_OK;
...
static void vcl_handle_ep_app_tlvs(vcl_session_t *s, vppcom_endpt_t *ep)
{
    ...
    do {
        switch (tlv->data_type) {
            case VCL_UDP_SEGMENT:
                ...
                break;
            case VCL_IP_PKTINFO:
                clib_memcpy_fast(&s->transport.lcl_ip, (ip4_address_t *) tlv->data, sizeof(ip4_address_t));
                break;
            default:
...
int vppcom_session_sendto(uint32_t session_handle, void *buffer, uint32_t buflen, int flags, vppcom_endpt_t *ep)
{
    vcl_worker_t  *wrk = vcl_worker_get_current();
    vcl_session_t *s;
    ...
    if (ep) {
        ...
        if (PREDICT_FALSE(s->session_state == VCL_STATE_CLOSED)) {
            ...
            rv = vppcom_wait_for_session_state_change(session_index, VCL_STATE_READY, timeout);
            if (rv < 0) return rv;
...
int vppcom_poll(vcl_poll_t *vp, uint32_t n_sids, double wait_for_time)
{
    vcl_worker_t    *wrk     = vcl_worker_get_current();
    f64              timeout = clib_time_now(&wrk->clib_time) + wait_for_time;
    u32              i, keep_trying = 1;
    ...
    do {
        ...
        if (wait_for_time != -1) keep_trying = (clib_time_now(&wrk->clib_time) <= timeout) ? 1 : 0;
    } while ((num_ev == 0) && keep_trying);

    return num_ev;
}

int vppcom_mq_epoll_fd(void)
{
    vcl_worker_t *wrk = vcl_worker_get_current();
    return wrk->mqs_epfd;
}

int vppcom_session_index(vcl_session_handle_t session_handle)
{
    return session_handle & 0xFFFFFF;
}

int vppcom_session_worker(vcl_session_handle_t session_handle)
{
...
void vppcom_worker_unregister(void)
{
    vcl_worker_cleanup(vcl_worker_get_current(), 1 /* notify vpp */);
    vcl_set_worker_index(~0);
}

void vppcom_worker_index_set(int index)
{
    vcl_set_worker_index(index);
}

int vppcom_worker_index(void)
{
    return vcl_get_worker_index();
}

int vppcom_worker_mqs_epfd(void)
{
    vcl_worker_t *wrk = vcl_worker_get_current();
    if (!vcm->cfg.use_mq_eventfd) return -1;
    return wrk->mqs_epfd;
}

int vppcom_session_is_connectable_listener(uint32_t session_handle)
{
    vcl_session_t *session;
    vcl_worker_t  *wrk = vcl_worker_get_current();
    session            = vcl_session_get_w_handle(wrk, session_handle);
...
int vppcom_session_listener(uint32_t session_handle)
{
    vcl_worker_t  *wrk = vcl_worker_get_current();
    vcl_session_t *listen_session, *session;
    session = vcl_session_get_w_handle(wrk, session_handle);
    if (!session) return VPPCOM_EBADFD;
    if (session->listener_index == VCL_INVALID_SESSION_INDEX) return VPPCOM_EBADFD;
    listen_session = vcl_session_get_w_handle(wrk, session->listener_index);
    if (!listen_session) return VPPCOM_EBADFD;
    return vcl_session_handle(listen_session);
}

int vppcom_session_n_accepted(uint32_t session_handle)
{
    vcl_worker_t  *wrk     = vcl_worker_get_current();
...
const char *vppcom_proto_str(vppcom_proto_t proto)
{
...
const char *vppcom_retval_str(int retval)
{
    char const *st;
...
int vppcom_add_cert_key_pair(vppcom_cert_key_pair_t *ckpair)
{
...
int vppcom_del_cert_key_pair(uint32_t ckpair_index)
{
    if (vcm->cfg.vpp_app_socket_api)
...
int vppcom_session_get_error(uint32_t session_handle)
{
    vcl_worker_t  *wrk     = vcl_worker_get_current();
    vcl_session_t *session = 0;
...
int vppcom_worker_is_detached(void)
{
    vcl_worker_t *wrk = vcl_worker_get_current();

    if (!vcm->cfg.use_mq_eventfd) return VPPCOM_ENOTSUP;
