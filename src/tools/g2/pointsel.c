...
static void point_select_callback(GtkToggleButton *item, gpointer data)
{
    ...
    g_eventdefs[i].selected = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(s_event_buttons[i]));
...
static void up_button(void)
{
    ...
    if (s_min_shown_pointsel == 0) return;

    s_min_shown_pointsel -= increment;

    if (s_min_shown_pointsel < 0) s_min_shown_pointsel = 0;

    s_max_shown_pointsel = s_min_shown_pointsel + c_maxpointsel;

    for (i = 0; i < g_neventdefs; i++) {
        if (i >= s_min_shown_pointsel && i <= s_max_shown_pointsel)
...
static void down_button(void)
{
    ...
    if (s_max_shown_pointsel == g_neventdefs) return;

    s_max_shown_pointsel += increment;

    if (s_max_shown_pointsel >= g_neventdefs) s_max_shown_pointsel = (g_neventdefs - 1);
...
static void button_click_callback(GtkButton *item, gpointer data)
{
    ...
    switch (click) {
        case ALL_BUTTON:
            for (i = 0; i < g_neventdefs; i++) {
                gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(s_event_buttons[i]), TRUE);
        ...
        case NONE_BUTTON:
            for (i = 0; i < g_neventdefs; i++) {
                gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(s_event_buttons[i]), FALSE);
...
void point_selector_init(void)
{
    ...
    gtk_signal_connect(GTK_OBJECT(s_allbutton), "clicked", GTK_SIGNAL_FUNC(button_click_callback), (gpointer) ALL_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_nonebutton), "clicked", GTK_SIGNAL_FUNC(button_click_callback), (gpointer) NONE_BUTTON);

    gtk_box_pack_start(GTK_BOX(s_pointselbuttons), s_allbutton, FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(s_pointselbuttons), s_nonebutton, FALSE, FALSE, 0);

    gtk_widget_show(s_pointselbuttons);
    gtk_widget_ref(s_pointselbuttons);

    gtk_box_pack_start(GTK_BOX(s_pointselbox), s_pointselbuttons, FALSE, FALSE, 0);

    gtk_box_pack_end(GTK_BOX(g_mainhbox), s_pointselbox, FALSE, FALSE, 0);
    ...
    gtk_signal_connect(GTK_OBJECT(s_ps_vsadj), "value-changed", GTK_SIGNAL_FUNC(scroll_callback), (gpointer) s_ps_vscroll);
...
static void reset_point_selector(void)
{
    ...
    gtk_container_remove(GTK_CONTAINER(s_pointselbox), s_pointselbuttons);

    for (i = 0; i < g_neventdefs; i++) {
        if (s_event_buttons[i]) {
            gtk_container_remove(GTK_CONTAINER(s_pointselbox), s_event_buttons[i]);
            s_event_buttons[i] = 0;
        }
    }
}
...
static void create_point_selector(void)
{
    ...
    for (i = 0; i < g_neventdefs; i++) {
        ep = &g_eventdefs[i];
        snprintf(tmpbuf, sizeof(tmpbuf), "[%lu] %s", ep->event, ep->name ? ep->name : "(none)");
        ...
        gtk_signal_connect(GTK_OBJECT(wp), "toggled", GTK_SIGNAL_FUNC(point_select_callback), (gpointer) (unsigned long long) i);
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(wp), TRUE);
...
static void remove_all_events(void)
{
    ...
    for (i = 0; i < g_neventdefs; i++) {
        ep = &g_eventdefs[i];
        if (!ep->is_clib) {
            if (ep->name) g_free(ep->name);
            if (ep->format) g_free(ep->format);
        }
    }
    g_neventdefs = 0;
}
...
void add_event_from_cpel_file(ulong event, char *event_format, char *datum_format)
{
    event_def_t *ep;
...
static void read_header_file(void)
{
    ...
    while (fgets(tmpbuf, sizeof(tmpbuf), s_hfp)) {
        ...
        if (strncmp(cp, "#define", 7)) continue;
...
static boolean read_header_files(void)
{
    ...
    while (fgets(tmpbuf, sizeof(tmpbuf), s_elog_hfp)) {
        ...
        if (strncmp(cp, "pointdefs:", 10)) continue;
...
boolean read_event_definitions(char *filename)
{
    ...
    if (g_neventdefs == 0) {
        snprintf(tmpbuf, sizeof(tmpbuf), "No event definitions found in %s\n", filename);
...
event_def_t *find_event_definition(ulong code)
{
    ...
    if (g_neventdefs == 0) goto use_placeholder;
...
void pointsel_next_snapshot(void)
{
    ...
    for (i = 0; i < g_neventdefs; i++) {
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(s_event_buttons[i]), g_eventdefs[i].selected);
    }
}
