...
void read_events(char *filename)
{
    ...
    if (size != (g_nevents * sizeof(raw_event_t) + sizeof(g_nevents))) {
        snprintf(tmpbuf, sizeof(tmpbuf), "%s was damaged, or isn't an event log.\n", filename);
    ...
    s_pidhash = (pid_data_t **) g_malloc0(PIDHASH_NBUCKETS * sizeof(pid_data_t *));
...
{
    snprintf(tmpbuf + strlen(tmpbuf), 1024 - strlen(tmpbuf), "%d total events, %.3f ticks per us\n", (int) g_nevents, ticks_per_ns);
}
