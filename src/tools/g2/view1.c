...
#define zi_x_hot  22
#define zi_y_hot  14
static unsigned char zi_bits[] = {};

static unsigned char zi_bkgd[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x80,
                                  0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00,
                                  0x82, 0x00, 0x00, 0x00, 0x84, 0x00, 0x00, 0x00, 0x88, 0x00, 0x00, 0x00, 0x90, 0x00, 0x00, 0x00, 0xa0, 0x00, 0x00,
                                  0x00, 0xc0, 0x00, 0x00, 0xfc, 0xff, 0x00, 0x00, 0x00, 0xc0, 0x00, 0x00, 0x00, 0xa0, 0x00, 0x00, 0x00, 0x90, 0x00,
                                  0x00, 0x00, 0x88, 0x00, 0x00, 0x00, 0x84, 0x00, 0x00, 0x00, 0x82, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x80,
...
{
    return ((double) (vp->maxvistime - vp->minvistime)) / ((double) (vp->total_width - vp->pid_ax_width));
}
...
void format_popbox_string(char *tmpbuf, int len, event_t *ep, event_def_t *edp)
{
    ...
    while (fp && *fp) {
        if (*fp != ' ') {
            ...
            if (fp[1] == 's') {
                snprintf(tmpbuf + strlen(tmpbuf), len - strlen(tmpbuf), edp->format, strtab_ref(ep->datum));
            }
            else {
                snprintf(tmpbuf + strlen(tmpbuf), len - strlen(tmpbuf), edp->format, ep->datum);
            }
            return;
        }
        fp++;
...
static void next_snapshot(void)
{
    ...
    if (summary_mode != next->summary_mode) {
        view1_button_click_callback(NULL, (gpointer) (unsigned long long) (summary_mode ? NOSUMMARY_BUTTON : SUMMARY_BUTTON));
    }
...
static void del_snapshot(void)
{
    ...
    if (s_cursnap == NULL) s_cursnap = s_snapshots;
}
...
static void write_snapshot(void)
{
    ...
    for (snap = s_snapshots; !error && snap != NULL; snap = snap->next) {
        if (fwrite(&snap->geometry, sizeof(snap->geometry), 1, file) != 1 || fwrite(&snap->show_event, sizeof(snap->show_event), 1, file) != 1
            || fwrite(snap->pidvec, sizeof(pid_sort_t) * g_npids, 1, file) != 1
            || fwrite(&snap->vscroll_value, sizeof(snap->vscroll_value), 1, file) != 1
            || fwrite(&snap->summary_mode, sizeof(snap->summary_mode), 1, file) != 1
            || fwrite(&snap->color_mode, sizeof(snap->color_mode), 1, file) != 1) {
    ...
    if (error) {
        ...
        char buf[64];
        snprintf(buf, sizeof(buf), "Wrote %d snapshots to snapshots.g2", records);
        message_line(buf);
    }
}
...
static void read_snapshot(void)
{
    ...
    while (!error && !feof(file)) {
        ...
        if (fread(&snap->vscroll_value, sizeof(snap->vscroll_value), 1, file) != 1
            || fread(&snap->summary_mode, sizeof(snap->summary_mode), 1, file) != 1
            || fread(&snap->color_mode, sizeof(snap->color_mode), 1, file) != 1) {
    ...
    if (error) {
        ...
        char buf[64];
        snprintf(buf, sizeof(buf), "Read %d snapshots from snapshots.g2", records);
        message_line(buf);
    }
}
...
static void set_color(int pid_index)
{
    ...
    if (psp->selected)
        ...
        gdk_gc_set_foreground(da->style->black_gc, &s_color[g_pids[pid_index].color_index]);
    }
}
...
static int toggle_event_select(GdkEventButton *event, v1_geometry_t *vp)
{
    ...
    /* Too far right? */
    if (start_index >= g_nevents) return 0;
    ...
    while ((ep->time < vp->maxvistime) && (ep < (g_events + g_nevents))) {
        pid_index = ep->pid->pid_index;

        /* First filter: pid out of range */
        if ((pid_index < vp->first_pid_index) || (pid_index >= vp->first_pid_index + vp->npids)) {
            ep++;
            continue;
        }
        ...
        x = vp->pid_ax_width + (int) (((double) (ep->time - vp->minvistime)) / time_per_pixel);
...
static void toggle_track_select(GdkEventButton *event, v1_geometry_t *vp)
{
    int         i;
    int         pid_index;
    int         y, delta_y;
...
static void move_current_track(GdkEventButton *event, v1_geometry_t *vp, move_type type)
{
    int         i;
    int         pid_index;
    int         y, delta_y;
...
void zoom_event(GdkEventButton *e1, GdkEventButton *e2, v1_geometry_t *vp)
{
    ...
    if (e1->x < vp->pid_ax_width) e1->x = vp->pid_ax_width;

    if (e2->x < vp->pid_ax_width) e2->x = vp->pid_ax_width;

    if (e2->x == e1->x) goto loser_zoom_repaint;

    xrange = (double) (e2->x - e1->x);
    if (xrange < 0.00) xrange = -xrange;
    ...
    center_on_pixel = (double) ((e2->x + e1->x) / 2.00) - (double) vp->pid_ax_width;
...
static void scroll_y(int delta)
{
    ...
    if (new_index + s_v1->npids > g_npids) new_index = g_npids - s_v1->npids;
    ...
    switch (event->type) {
        ...
        case GDK_BUTTON_RELEASE:
            ...
            if (press1_valid) {
                ...
                if (xdelta < 10) {
                    ...
                    if ((press1_event.state & GDK_CONTROL_MASK) == GDK_CONTROL_MASK) {
                        move_current_track(event, s_v1, MOVE_BOTTOM);
                    }
                    else if ((press1_event.state & GDK_SHIFT_MASK) == GDK_SHIFT_MASK) {
                        move_current_track(event, s_v1, MOVE_TOP);
                    }
                    else {
                        /* No modifiers: toggle the event / select track */
                        if (toggle_event_select(event, s_v1)) toggle_track_select(event, s_v1);
                    }
        ...
        case GDK_MOTION_NOTIFY:
            ...
            if (press1_valid) {
                ...
                if (zoom_bar_up) return (TRUE);

                xdelta = (int) (press1_event.x - event->x);
                if (xdelta < 0) xdelta = -xdelta;

                /* Haven't moved enough to declare a zoom sequence yet */
                if (xdelta < 10) return (TRUE);

                /* Draw the zoom fence, use the key-down X coordinate */
                time_ax_y = s_v1->npids * s_v1->strip_height + s_v1->pid_ax_offset;

                line((int) (press1_event.x), s_v1->pop_offset, (int) (press1_event.x), time_ax_y, LINE_DRAW_BLACK);
                tbox("Zoom From Here...", (int) (press1_event.x), s_v1->pop_offset, TBOX_DRAW_BOXED);
            ...
            if (press3_valid) {
                 ...
                 */
                if (event->time - last_truler_time < 75) return (TRUE);

                last_truler_time = event->time;

                line((int) (press3_event.x), s_v1->pop_offset, (int) (press3_event.x), time_ax_y, LINE_DRAW_BLACK);

                xdelta = (int) (press3_event.x - event->x);
                if (xdelta < 0) xdelta = -xdelta;

                time_per_pixel = ((double) (s_v1->maxvistime - s_v1->minvistime)) / ((double) (s_v1->total_width - s_v1->pid_ax_width));
                ...
                tbox(tmpbuf, (int) (press3_event.x), s_v1->pop_offset, TBOX_DRAW_BOXED);
                return (TRUE);
            }
...
static gint configure_event(GtkWidget *widget, GdkEventConfigure *event)
{
    ...
    pm = gdk_pixmap_new(widget->window, widget->allocation.width, widget->allocation.height, -1);
    gdk_draw_rectangle(pm, widget->style->white_gc, TRUE, 0, 0, widget->allocation.width, widget->allocation.height);
    ...
    s_v1->npids        = (s_v1->total_height - s_v1->time_ax_height) / s_v1->strip_height;
...
boolean event_search_internal(void)
{
     ...
     */
#ifdef notdef
    if (ep->time < s_v1->minvistime) s_srchindex = find_event_index(s_v1->minvistime);
#endif

    for (i = 1; i <= g_nevents; i++) {
        index = (srch_chase_dir == SRCH_CHASE_BACKWARD) ? (s_srchindex - i) % g_nevents : (i + s_srchindex) % g_nevents;

        ep = (g_events + index);

        if (ep->code == s_srchcode) {
            if (s_srchfail_up) message_line("");
            s_srchindex = index;
            pid_index   = ep->pid->pid_index;

            /* Need a vertical scroll? */
            if ((pid_index < s_v1->first_pid_index) || (pid_index >= s_v1->first_pid_index + s_v1->npids)) {
                if (pid_index > (g_npids - s_v1->npids)) pid_index = (g_npids - s_v1->npids);
            ...
            if (!full_redisplay) {
                if (!s_result_up) {
                    ...
                    x = s_v1->pid_ax_width + (int) (((double) (ep->time - s_v1->minvistime)) / time_per_pixel);
            ...
            if (full_redisplay) view1_display_when_idle();
            return (TRUE);
        }
    }
    snprintf(tmpbuf, sizeof(tmpbuf), "Search for event %ld failed...\n", s_srchcode);
    message_line(tmpbuf);
    s_srchfail_up = TRUE;
    return (TRUE);
}
...
static int anomaly_statistics_init(void)
{
    ...
    for (i = 0; i < g_nevents; i++) {
        ...
        if (PREDICT_FALSE(s_v1->matches[pid->pid_index] == 1)) {
            ...
            s_v1->mins[pid->pid_index]  = (fdata < s_v1->mins[pid->pid_index]) ? fdata : s_v1->mins[pid->pid_index];
            s_v1->maxes[pid->pid_index] = (fdata > s_v1->maxes[pid->pid_index]) ? fdata : s_v1->maxes[pid->pid_index];
        }
        ep++;
    }
    if (vec_len(s_v1->matches) == 0) return -1;

    /* Compute s_v1->means */
    for (i = 0; i < vec_len(s_v1->means); i++)
        s_v1->means[i] = s_v1->matches[i] ? (s_v1->means[i] / (f64) s_v1->matches[i]) : 0.0;
    ...
    for (i = 0; i < g_nevents; i++) {
        ...
        data  = clib_mem_unaligned(eep->data, u32);
        fdata = data;
        s_v1->variances[pid->pid_index] += (fdata - s_v1->means[pid->pid_index]) * (fdata - s_v1->means[pid->pid_index]);
        ep++;
    }

    /* Normalize variances */
    for (i = 0; i < vec_len(s_v1->variances); i++)
        s_v1->variances[i] = s_v1->matches[i] ? (s_v1->variances[i] / (f64) s_v1->matches[i]) : 0.0;

    /* Compute the anomaly threshold, by default 2.5*stddev */
    for (i = 0; i < vec_len(s_v1->variances); i++)
        vec_add1(s_v1->two_stddevs, s_v1->anomaly_threshold_stddevs * sqrt(s_v1->variances[i]));
    return 0;
}
...
boolean anomaly_search_internal(void)
{
    ...
    if (vec_len(s_v1->matches) == 0) anomaly_statistics_init();
    ...
    for (i = 1; i <= g_nevents; i++) {
        ...
        ep = (g_events + index);
        if (ep->code != s_anomalycode) continue;
        pid = ep->pid;
         ...
         */
        if ((fdata - s_v1->means[pid->pid_index]) > s_v1->two_stddevs[pid->pid_index]) {
            u8 *s;

            s = format(0, "%.1f*stddev {min,max,mean,threshold}: ", s_v1->anomaly_threshold_stddevs);

            for (i = 0; i < vec_len(s_v1->means); i++) {
                if (s_v1->matches[i] > 0)
                    s = format(s, "{%.0f, %.0f, %.0f, %.0f} ", s_v1->mins[i], s_v1->maxes[i], s_v1->means[i], s_v1->means[i] + s_v1->two_stddevs[i]);
                else
                    s = format(s, "{no match} ");
            }
    ...
    snprintf(tmpbuf, sizeof(tmpbuf), "Search for an anomalous event %ld failed...\n", s_anomalycode);
    message_line(tmpbuf);
    s_srchfail_up = TRUE;
    return (TRUE);
}
...
boolean anomaly_search_callback(char *s)
{
    ...
    if (new_anomalycode == 0) return (FALSE);

    if (new_anomalycode != s_anomalycode || vec_len(s_v1->matches) == 0) {
...
boolean anomaly_threshold_callback(char *s)
{
    ...
    if (new_threshold == 0.0 || new_threshold > 10.0) return (FALSE);
...
{
    modal_dialog("Event Search: Please Enter Event Code", "Invalid: Please Reenter Event Code", NULL, event_search_callback);
}
...
{
    modal_dialog("Anomaly Search: Please Enter Event Code", "Invalid: Please Reenter Event Code", NULL, anomaly_search_callback);
}
...
static void init_track_colors(void)
{
     ...
     */
    if (s_color != NULL) {
        gdk_colormap_free_colors(gtk_widget_get_colormap(da), s_color, g_npids);
     ...
     * values.
     */
    gdk_colormap_alloc_colors(gtk_widget_get_colormap(da), s_color, g_npids + 1, FALSE, TRUE, dont_care);
}
...
static void chase_event_etc(enum chase_mode mode)
{
    ...
    while (1) {
        if (srch_chase_dir == SRCH_CHASE_FORWARD) {
            if (ep >= g_events + g_nevents) break;
        }
        else {
            if (ep < g_events) break;
        }
...
static boolean print_screen_callback(char *filename)
{
    ...
    if (s_printfp == NULL) return (FALSE);
...
static void view1_button_click_callback(GtkButton *item, gpointer data)
{
    ...
    if (event_incdec == 0LL) event_incdec = 1;
    ...
    switch (click) {
        ...
        case BOTTOM_BUTTON:
            ...
            if (s_v1->first_pid_index < 0) s_v1->first_pid_index = 0;
            GTK_ADJUSTMENT(s_view1_vsadj)->value = (gdouble) s_v1->first_pid_index;
            gtk_adjustment_value_changed(GTK_ADJUSTMENT(s_view1_vsadj));
            break;
        ...
        case ZOOMOUT_BUTTON:
            if (zoom_delta == 0LL) zoom_delta = 1;
            ...
            if ((s_v1->maxvistime - s_v1->minvistime) * 8 > g_events[g_nevents - 1].time * 9) {
                s_v1->minvistime = 0;
                s_v1->maxvistime = g_events[g_nevents - 1].time * 9 / 8;
                /* Single event? Make window 1s wide... */
                if (g_nevents == 1) s_v1->maxvistime = 1000000;
            }
            recompute_hscrollbar();
            break;
        ...
        case END_BUTTON:
            ...
            if (s_v1->minvistime > s_v1->maxvistime) goto start_button;
            recompute_hscrollbar();
            break;
...
{
    modal_dialog("Print Screen (PostScript format) to file:", "Invalid file: Print Screen to file:", "g2.ps", print_screen_callback);
}
...
void view1_init(void)
{
    ...
    s_v1->first_pid_index           = 0;
    s_v1->anomaly_threshold_stddevs = atof(getprop_default("anomaly_threshold_stddevs", "2.5"));
    s_v1->npids                     = (s_v1->total_height - s_v1->time_ax_height) / s_v1->strip_height;
    ...
    gtk_drawing_area_size(GTK_DRAWING_AREA(da), c_view1_draw_width, c_view1_draw_height);

#ifdef NOTDEF
    gtk_signal_connect(GTK_OBJECT(da), "motion_notify_event", (GtkSignalFunc) motion_notify_event, NULL);
#endif

    gtk_signal_connect(GTK_OBJECT(da), "expose_event", (GtkSignalFunc) expose_event, NULL);

    gtk_signal_connect(GTK_OBJECT(da), "configure_event", (GtkSignalFunc) configure_event, NULL);

    gtk_signal_connect(GTK_OBJECT(da), "button_press_event", (GtkSignalFunc) button_press_event, NULL);

    gtk_signal_connect(GTK_OBJECT(da), "button_release_event", (GtkSignalFunc) button_press_event, NULL);

    gtk_signal_connect(GTK_OBJECT(da), "motion_notify_event", (GtkSignalFunc) button_press_event, NULL);

    gtk_widget_set_events(da, GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK | GDK_EXPOSURE_MASK | GDK_BUTTON_MOTION_MASK);
    ...
    gtk_signal_connect(GTK_OBJECT(s_view1_vsadj), "value-changed", GTK_SIGNAL_FUNC(view1_vscroll), (gpointer) s_view1_vscroll);

    s_view1_topbutton    = gtk_button_new_with_label("Top");
    s_view1_bottombutton = gtk_button_new_with_label("Bottom");

    gtk_signal_connect(GTK_OBJECT(s_view1_topbutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback), (gpointer) TOP_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_bottombutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback), (gpointer) BOTTOM_BUTTON);
    ...
    gtk_signal_connect(GTK_OBJECT(s_view1_more_traces_button),
                       "clicked",
                       GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) MORE_TRACES_BUTTON);
    gtk_signal_connect(GTK_OBJECT(s_view1_less_traces_button),
                       "clicked",
                       GTK_SIGNAL_FUNC(view1_button_click_callback),
    ...
    gtk_box_pack_start(GTK_BOX(s_view1_vmenubox), s_view1_pad1, TRUE, FALSE, 0);

#endif

    gtk_box_pack_start(GTK_BOX(s_view1_vmenubox), s_view1_topbutton, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_vmenubox), s_view1_vscroll, TRUE, TRUE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_vmenubox), s_view1_bottombutton, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_vmenubox), s_view1_more_traces_button, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_vmenubox), s_view1_less_traces_button, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_hbox), s_view1_vmenubox, FALSE, FALSE, 0);
    ...
    s_view1_anomalythresholdbutton = gtk_button_new_with_label("Anomaly Threshold");

    s_view1_zoomoutbutton = gtk_button_new_with_label("ZoomOut");

    s_view1_endbutton = gtk_button_new_with_label("End");

    gtk_signal_connect(GTK_OBJECT(s_view1_startbutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback), (gpointer) START_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_zoominbutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback), (gpointer) ZOOMIN_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_searchbutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback), (gpointer) SEARCH_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_srchagainbutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback), (gpointer) SEARCH_AGAIN_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_anomalybutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback), (gpointer) ANOMALY_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_anomalynextbutton),
                       "clicked",
                       GTK_SIGNAL_FUNC(view1_button_click_callback),
    ...
    gtk_signal_connect(GTK_OBJECT(s_view1_zoomoutbutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback), (gpointer) ZOOMOUT_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_endbutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback), (gpointer) END_BUTTON);
    ...
    gtk_signal_connect(GTK_OBJECT(s_view1_hsadj), "value-changed", GTK_SIGNAL_FUNC(view1_hscroll), (gpointer) s_view1_hscroll);

    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox), s_view1_startbutton, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox), s_view1_hscroll, TRUE, TRUE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox), s_view1_endbutton, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox), s_view1_zoominbutton, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox), s_view1_anomalybutton, FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox), s_view1_anomalynextbutton, FALSE, FALSE, 0);
    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox), s_view1_searchbutton, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox), s_view1_srchagainbutton, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox), s_view1_zoomoutbutton, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_vbox), s_view1_hbox, TRUE, TRUE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_vbox), s_view1_hmenubox, FALSE, FALSE, 0);
    ...
    gtk_signal_connect(GTK_OBJECT(s_view1_snapbutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback), (gpointer) SNAP_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_nextbutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback), (gpointer) NEXT_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_delbutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback), (gpointer) DEL_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_chase_event_button),
                       "clicked",
                       GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) CHASE_EVENT_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_chase_datum_button),
                       "clicked",
                       GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) CHASE_DATUM_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_chase_track_button),
                       "clicked",
                       GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) CHASE_TRACK_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_unchasebutton), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback), (gpointer) UNCHASE_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_forward_button), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback), (gpointer) FORWARD_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_backward_button), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback), (gpointer) BACKWARD_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_summary_button), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback), (gpointer) SUMMARY_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_nosummary_button), "clicked", GTK_SIGNAL_FUNC(view1_button_click_callback), (gpointer) NOSUMMARY_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_time_slew_left_button),
                       "clicked",
                       GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) SLEW_LEFT_BUTTON);

    gtk_signal_connect(GTK_OBJECT(s_view1_time_slew_right_button),
                       "clicked",
                       GTK_SIGNAL_FUNC(view1_button_click_callback),
                       (gpointer) SLEW_RIGHT_BUTTON);

    gtk_box_pack_start(GTK_BOX(s_view1_vbox), s_view1_hmenubox2, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox2), s_view1_snapbutton, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox2), s_view1_nextbutton, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox2), s_view1_delbutton, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox2), s_view1_chase_event_button, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox2), s_view1_chase_datum_button, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox2), s_view1_chase_track_button, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox2), s_view1_unchasebutton, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox2), s_view1_forward_button, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox2), s_view1_backward_button, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox2), s_view1_summary_button, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox2), s_view1_nosummary_button, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox2), s_view1_time_slew_left_button, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox2), s_view1_time_slew_right_button, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(s_view1_hmenubox2), s_view1_anomalythresholdbutton, FALSE, FALSE, 0);

    s_view1_label = gtk_label_new(NULL);

    gtk_box_pack_start(GTK_BOX(s_view1_vbox), s_view1_label, FALSE, FALSE, 0);

    gtk_box_pack_start(GTK_BOX(g_mainhbox), s_view1_vbox, TRUE, TRUE, 0);
    ...
    gtk_widget_hide(s_view1_forward_button);
    gtk_widget_hide(summary_mode ? s_view1_summary_button : s_view1_nosummary_button);

    zi_source = gdk_bitmap_create_from_data(NULL, (char *) zi_bits, zi_width, zi_height);
    zi_mask   = gdk_bitmap_create_from_data(NULL, (char *) zi_bkgd, zi_width, zi_height);

    zi_cursor = (GdkCursor *) gdk_cursor_new_from_pixmap(zi_source, zi_mask, &fg_black, &bg_white, zi_x_hot, zi_y_hot);
...
void line_print(int x1, int y1, int x2, int y2)
{
    ...
    fprintf(s_printfp, "%d %d moveto\n", xrt(x1, s_v1->total_height - y1), yrt(x1, s_v1->total_height - y1));

    fprintf(s_printfp, "%d %d lineto\n", xrt(x2, s_v1->total_height - y2), yrt(x2, s_v1->total_height - y2));
 ...
 * tbox_print
 ****************************************************************************/
GdkRectangle *tbox_print(char *s, int x, int y, enum view1_tbox_fn function, GdkRectangle *rp)
{
    if (function == TBOX_PRINT_BOXED) {
        rp->width -= 4;
    }

    if ((function == TBOX_PRINT_BOXED) || (function == TBOX_PRINT_EVENT)) {
        fprintf(s_printfp, "newpath\n");
        fprintf(s_printfp, "0 setlinewidth\n");
        fprintf(s_printfp, "%d %d moveto\n", xrt(rp->x, s_v1->total_height - rp->y), yrt(rp->x, s_v1->total_height - rp->y));

        fprintf(s_printfp, "%d %d lineto\n", xrt(rp->x + rp->width, s_v1->total_height - rp->y), yrt(rp->x + rp->width, s_v1->total_height - rp->y));
    ...
    if ((function == TBOX_PRINT_BOXED) || (function == TBOX_PRINT_PLAIN)) {
        fprintf(s_printfp, "newpath\n");
        fprintf(s_printfp, "%d %d moveto\n", xrt(x, s_v1->total_height - (y - 2)), yrt(x, s_v1->total_height - (y - 2)));
...
GdkRectangle *tbox(char *s, int x, int y, enum view1_tbox_fn function)
{
    ...
    gdk_string_extents(g_font, s, &lbearing, &rbearing, &width, &ascent, &descent);
    ...
    switch (function) {
        case TBOX_DRAW_BOXED:
            gdk_draw_rectangle(pm, da->style->white_gc, TRUE, x, y - (ascent + descent + 3), width + 2, ascent + descent + 3);

            gdk_draw_rectangle(pm, da->style->black_gc, FALSE, x, y - (ascent + descent + 3), width + 2, ascent + descent + 3);

            gdk_draw_string(pm, g_font, da->style->black_gc, x + 1, y - 1, (const gchar *) s);
            /* NOTE FALLTHROUGH */
        case TBOX_GETRECT_BOXED:
            ...
            if (function == TBOX_DRAW_BOXED) gtk_widget_draw(da, &update_rect);
            break;

        case TBOX_DRAW_EVENT:
            /* We have a small event to draw...no text */
            gdk_draw_rectangle(pm, da->style->black_gc, FALSE, x, y - 1, 3, 3);
            /* NOTE FALLTHROUGH */
        case TBOX_GETRECT_EVENT:
            update_rect.x      = x;
            update_rect.y      = y - 1;
            ...
            if (function == TBOX_DRAW_EVENT) gtk_widget_draw(da, &update_rect);
            break;
        ...
        case TBOX_GETRECT_PLAIN:
            ...
            if (function == TBOX_DRAW_PLAIN) gtk_widget_draw(da, &update_rect);
            break;
...
void line(int x1, int y1, int x2, int y2, enum view1_line_fn function)
{
    ...
    switch (line_batch_mode) {
        ...
        case BATCH_EXISTING:
            if (line_minx > x1) line_minx = x1;
            if (line_miny > y1) line_miny = y1;
            if (line_maxx < x2) line_maxx = x2;
            if (line_maxy < y2) line_maxy = y2;
            line_batch_count++;
            break;
    }
}
...
static void display_pid_axis(v1_geometry_t *vp)
{
    ...
    if (g_pids == NULL) return;

    line_batch_start();

    for (i = 0; i < vp->npids; i++) {
        pid_index = vp->first_pid_index + i;
        if (pid_index >= g_npids) break;
        ...
        if (vp->strip_height > 4) {
            line(vp->pid_ax_width - label_tick, y + 4 * s_print_offset, vp->total_width, y + 4 * s_print_offset, LINE_DRAW_BLACK + s_print_offset);
        }
    }
...
void view1_read_events_callback(void)
{
    ...
    max_vis_index = 300;
    if (max_vis_index > g_nevents) max_vis_index = g_nevents - 1;
...
static void display_event_data(v1_geometry_t *vp)
{
    ...
    if (start_index >= g_nevents) return;
    ...
    while (ep < (g_events + g_nevents) && (ep->time < vp->maxvistime)) {
        pid_index = ep->pid->pid_index;
        set_color(pid_index);
        ...
        if (ep->flags & (EVENT_FLAG_SELECT | EVENT_FLAG_SEARCHRSLT)) {
            ...
            print_rect = tbox(tmpbuf, x, y - vp->pop_offset, TBOX_DRAW_BOXED + s_print_offset);
            line(x, y - vp->pop_offset, x, y, LINE_DRAW_BLACK + s_print_offset);
            if (last_x_used != NULL) last_x_used[pid_index] = x + print_rect->width;
        }
        if (summary_mode) {
    ...
    if (last_x_used) g_free(last_x_used);
    line_batch_end();
    set_color(COLOR_DEFAULT);
}
...
static void display_clear(void)
{
    ...
    gdk_draw_rectangle(pm, da->style->white_gc, TRUE, 0, 0, da->allocation.width, da->allocation.height);
