...
void print_state_statistics(int verbose, FILE *ofp)
{
    ...
    for (i = 0; i < vec_len(bound_tracks); i++) {
        ...
        if (!summary_stats_only) {
            ...
            fprintf(ofp, "%10.3f +- %10.3f", bp->mean_ticks_in_state / ticks_per_us, sqrt(bp->variance_ticks_in_state) / (f64) ticks_per_us);
    ...
    fprintf(ofp, "Total runtime: %10.3f (us), Total state switches: %.0f\n", total_time / ticks_per_us, total_switches);
...
int main(int argc, char **argv)
{
    ...
    usage:
        fprintf(stderr, "cpelstate --input-file <filename> [--output-file <filename>]\n");
