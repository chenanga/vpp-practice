...
void convert_clib_file(char *clib_file)
{
    ...
    for (i = 0; i < vec_len(em->events); i++) {
        ...
        for (j = 0; j < vec_len(brief_event_name); j++) {
            if (brief_event_name[j] == ' ' || brief_event_name[j] == '%' || brief_event_name[j] == '(') {
...
int main(int argc, char **argv)
{
    ...
    usage:
        fformat(stderr, "c2cpel [--input-file] <filename> --output-file <filename>\n");
        exit(1);
    }

    if (vec_len(inputfiles) == 0 || outputfile == 0) goto usage;

    if (vec_len(inputfiles) > 1) goto usage;
