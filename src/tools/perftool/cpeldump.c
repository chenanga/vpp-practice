...
int event_pass2(cpel_section_header_t *sh, int verbose, FILE *ofp)
{
    ...
    for (i = 0; i < nevents; i++) {
        ...
        if (starttime == 0xFFFFFFFFFFFFFFFFULL) starttime = now;
        ...
        if (hours) delta -= ((u64) hours * USEC_PER_HOUR);
        minutes = delta / USEC_PER_MINUTE;
        if (minutes) delta -= ((u64) minutes * USEC_PER_MINUTE);
        seconds = delta / USEC_PER_SECOND;
        if (seconds) delta -= ((u64) seconds * USEC_PER_SECOND);
        msec = delta / USEC_PER_MS;
        if (msec) delta -= ((u64) msec * USEC_PER_MS);
        ...
        vec_free(s);
        if (bp->is_strtab_ref) {
            fprintf(ofp, (char *) bp->datum_str, &this_strtab[ntohl(ep->event_datum)]);
        }
        else {
...
int cpel_dump(u8 *cpel, int verbose, FILE *ofp)
{
    ...
    for (i = 0; i < nsections; i++) {
        ...
        if (verbose) {
            fprintf(stderr, "Section type %d, size %d\n", ntohl(sh->section_type), section_size);
        }
...
int main(int argc, char **argv)
{
    ...
    usage:
        fprintf(stderr, "cpeldump --input-file <filename> [--output-file <filename>]\n");
        fprintf(stderr, "%s\n", version);
        exit(1);
    }
