...
int evtdef_pass1(cpel_section_header_t *sh, int verbose, FILE *ofp)
{
    ...
    for (i = 0; i < nevents; i++) {
        ...
        if (thislen > widest_name_format) widest_name_format = thislen;
...
int trackdef_pass1(cpel_section_header_t *sh, int verbose, FILE *ofp)
{
    ...
    for (i = 0; i < nevents; i++) {
        ...
        if (thislen > widest_track_format) widest_track_format = thislen;
...
int process_section(cpel_section_header_t *sh, int verbose, FILE *ofp, pass_t pass)
{
    u32 type;
...
int cpel_dump_file_header(cpel_file_header_t *fh, int verbose, FILE *ofp)
{
    ...
    if (verbose) {
        fprintf(ofp,
                "CPEL file: %s-endian, version %d\n",
                ((fh->endian_version & CPEL_FILE_LITTLE_ENDIAN) ? "little" : "big"),
...
int cpel_dump(u8 *cpel, int verbose, FILE *ofp)
{
    ...
    for (i = 0; i < nsections; i++) {
        ...
        if (verbose) {
            fprintf(ofp, "Section type %d, size %d\n", ntohl(sh->section_type), section_size);
        }

        if (process_section(sh, verbose, ofp, PASS1)) return (1);
    ...
    sh = (cpel_section_header_t *) (fh + 1);
    for (i = 0; i < nsections; i++) {
        if (process_section(sh, verbose, ofp, PASS2)) return (1);
        section_size = ntohl(sh->data_length);
        sh++;
        sh = (cpel_section_header_t *) (((u8 *) sh) + section_size);
    }
...
void compute_state_statistics(int verbose, FILE *ofp)
{
    ...
    for (i = 0; i < vec_len(bound_tracks); i++) {
        ...
        if (vec_len(bp->ticks_in_state)) {
            bp->mean_ticks_in_state = bp->total_ticks_in_state / ((f64) vec_len(bp->ticks_in_state));
        }
        /* Accumulate sum: (Xi-Xbar)**2 */
        for (j = 0; j < vec_len(bp->ticks_in_state); j++) {
            fticks = bp->ticks_in_state[j];
            bp->variance_ticks_in_state += (fticks - bp->mean_ticks_in_state) * (fticks - bp->mean_ticks_in_state);
        }
        /* Compute s**2, the unbiased estimator of sigma**2 */
        if (vec_len(bp->ticks_in_state) > 1) {
            bp->variance_ticks_in_state /= (f64) (vec_len(bp->ticks_in_state) - 1);
        }
    }
}
...
void sort_state_statistics(sort_t type, FILE *ofp)
{
    ...
    qsort(bound_tracks, vec_len(bound_tracks), sizeof(bound_track_t), compare);
}
...
void print_state_statistics(int verbose, FILE *ofp)
{
    ...
    for (i = 0; i < vec_len(bound_tracks); i++) {
        bp = &bound_tracks[i];
        if (bp->mean_ticks_in_state == 0.0) continue;

        if (name_filter && strncmp((char *) bp->track_str, (char *) name_filter, strlen((char *) name_filter))) continue;

        /*
         * Exclude kernel threads (e.g. idle thread) from
         * state statistics
         */
        if (exclude_kernel_from_summary_stats && !strncmp((char *) bp->track_str, "kernel ", 7)) continue;

        total_switches += (f64) vec_len(bp->ticks_in_state);

        if (!summary_stats_only) {
            fprintf(ofp, (char *) trackpad, bp->track_str);
            fprintf(ofp, "%10.3f +- %10.3f", bp->mean_ticks_in_state / ticks_per_us, sqrt(bp->variance_ticks_in_state) / ticks_per_us);
            fprintf(ofp, "%12.3f", bp->total_ticks_in_state / ticks_per_us);
    ...
    if (!summary_stats_only) fprintf(ofp, "\n");
    fprintf(ofp, "Note: the following statistics %s kernel-thread activity.\n", exclude_kernel_from_summary_stats ? "exclude" : "include");
    if (name_filter) fprintf(ofp, "Note: only pid/proc/threads matching '%s' are included.\n", name_filter);

    fprintf(ofp, "Total time in state: %10.3f (us), Total state occurrences: %.0f\n", total_time / ticks_per_us, total_switches);
...
int main(int argc, char **argv)
{
    ...
    usage:
        fprintf(stderr, "cpelatency --input-file <filename> [--output-file <filename>]\n");
    ...
    if (cpel_dump((u8 *) cpel, verbose, ofp)) {
        if (outputfile) unlink(outputfile);
    }
