...
void report_actors(void)
{
    ...
    double      e10k;
    extern void linreg(double *x, double *y, int nitems, double *a, double *b, double *minp, double *maxp, double *meanp, double *r);
    ...
    for (i = 0; i < NBUCKETS; i++) {
        ...
        while (ap) {
            ...
            if (ninstance > 1) {
                ...
                linreg(x, y, ninstance, &ap->a, &ap->b, &ap->min, &ap->max, &ap->mean, &ap->r);
            }
            else {
                ap->a = 0.00;
                ap->b = 0.00;
    ...
    for (i = 0; i < nactors; i++) {
        ...
        if (ap->ninst) {
            if (summary_stats) {
                ...
                printf("%6ld %11.2f %11.2f %11.2f 0x%08lx ", ap->ninst, ap->a, ap->b, e10k, ap->key);
...
int event_pass(cpel_section_header_t *sh, int verbose, FILE *ofp)
{
    ...
    for (i = 0; i < nevents; i++) {
        ...
        if (event_code != entry_event && event_code != exit_event) {
            ep++;
            continue;
        }
    ...
    again:
        switch (tp->state) {
            case 0: /* not in state */
                ...
                if (event_code == exit_event) {
                    ...
                    if (vec_len(tp->start_datum) > 0) {
                        ...
                        fprintf(stderr, "End event before start event, key 0x%x.", ntohl(ep->event_datum));
                ...
                printf("sp %lld key 0x%x start time %llu\n",
                       (long long) vec_len(tp->start_time) - 1,
                       event_datum,
                       (unsigned long long) tp->start_time[vec_len(tp->start_time) - 1]);
                 ...
                 * trash the duplicate.
                 */
                if (inline_mokus && vec_len(tp->start_datum) > 1
                    && tp->start_datum[vec_len(tp->start_datum) - 1] == tp->start_datum[vec_len(tp->start_datum) - 2]) {
            ...
            case 1: /* in state */
                ...
                if (!tp->dup_event[sp]) {
#ifdef HAVING_TROUBLE
                    printf("sp %d key 0x%x charged %llu\n", sp, tp->start_datum[sp], end_time - tp->start_time[sp]);
                    printf("  start %llu, end %llu\n", (unsigned long long) tp->start_time[sp], (unsigned long long) end_time);
#endif

                    record_instance(tp->start_datum[sp], (end_time - tp->start_time[sp]));
