            ...
            while (input[input_index]) {
                if (input[input_index] == fmt[0]) {
                    ...
                    if (fmt[0] == ' ') {
                        ...
                        while (input[input_index] == ' ' || input[input_index] == '\t' || input[input_index] == '\n')
                            input_index++;
                        input_index--;
                    }
                    goto found;
...
utest_t tests[] = {
    ...
    {"                       pid:1", " *"},
    {"t:0x53034cbb CPU:00 THREAD  :THCREATE      pid:1 tid:1", ": :  : pid: tid:*"},
    ...
    {"Feb 28 02:38:26.125 seqtrace 0/1/CPU0 t1  "
     ...
     "  ::.  //  :msg_receive_event::*"} {"t:0x645dd86d CPU:00 USREVENT:EVENT:100, d0:0x00000002 d1:0x00000000", ": : USREVENT:EVENT:, d0: *"} {
        "t:0x5303f950 CPU:00 COMM    :REC_PULSE     scoid:0x40000003 pid:364659",
        ": :  : *"},
    {"t:0x2ccf9f5a CPU:00 INT_ENTR:0x80000000 (-2147483648)       "
     "IP:0x002d8b18",
     ": : INT_ENTR:  IP:*"} {"t:0xd473951c CPU:00 KER_EXIT:SCHED_GET/88 ret_val:2 sched_priority:10", ": : KER_EXIT:SCHED_GET : sched_priority:*"} {
    ...
    {"t:0x5af8de95 CPU:00 INT_HANDLER_ENTR:0x0000004d (77)       PID:8200 "
     ...
     ": : INT_HANDLER_*"},
#endif
    {"t:0x6d1ff92f CPU:00 CONTROL: BUFFER sequence = 1053, num_events = 714", ": : CONTROL*"},
    {"t:0x6d1ff92f CPU:00 CONTROL :TIME msb:0x0000003c lsb(offset):0x6d1ff921", ": : CONTROL*"},
};
