...
int write_string_table(FILE *ofp)
{
    ...
    if (fwrite(&sh, sizeof(sh), 1, ofp) != 1) return (0);
...
int write_cpel_header(FILE *ofp, u32 nsections)
{
    ...
    if (fwrite(&h, sizeof(h), 1, ofp) != 1) return (0);
...
int write_event_defs(FILE *ofp)
{
    ...
    sh.data_length  = ntohl(vec_len(the_event_definitions) * sizeof(the_event_definitions[0]) + sizeof(event_definition_section_header_t));
    ...
    if (fwrite(&edsh, sizeof(edsh), 1, ofp) != 1) return (0);
    ...
    for (i = 0; i < vec_len(the_event_definitions); i++) {
        ...
        this_event_definition->event_format = ntohl(this_event_definition->event_format);
        this_event_definition->datum_format = ntohl(this_event_definition->datum_format);

        if (fwrite(this_event_definition, sizeof(the_event_definitions[0]), 1, ofp) != 1) return (0);
    }
    return (1);
}
...
u64 ntohll(u64 x)
{
    if (clib_arch_is_little_endian)
        x = ((((x >> 0) & 0xff) << 56) | (((x >> 8) & 0xff) << 48) | (((x >> 16) & 0xff) << 40) | (((x >> 24) & 0xff) << 32)
             | (((x >> 32) & 0xff) << 24) | (((x >> 40) & 0xff) << 16) | (((x >> 48) & 0xff) << 8) | (((x >> 56) & 0xff) << 0));
...
int write_events(FILE *ofp, u64 clock_ticks_per_second)
{
    ...
    sh.data_length  = ntohl(number_of_events * sizeof(e) + sizeof(event_section_header_t));
    ...
    if (fwrite(&eh, sizeof(eh), 1, ofp) != 1) return (0);
    ...
    for (i = 0; i < number_of_events; i++) {
        ...
        if (fwrite(&e, sizeof(e), 1, ofp) != 1) return (0);
    }
    return (1);
}
...
int write_track_defs(FILE *ofp)
{
    ...
    sh.data_length  = ntohl(vec_len(the_tracks) * sizeof(this_track_definition[0]) + sizeof(track_definition_section_header_t));
    ...
    if (fwrite(&tdsh, sizeof(edsh), 1, ofp) != 1) return (0);

    for (i = 0; i < vec_len(the_tracks); i++) {
        this_track_definition->track        = ntohl(i);
        this_track_definition->track_format = ntohl(the_tracks[i].strtab_offset);

        if (fwrite(this_track_definition, sizeof(this_track_definition[0]), 1, ofp) != 1) return (0);
    }
    return (1);
}
...
void alpha_sort_tracks(void)
{
    ...
    qsort(the_tracks, vec_len(the_tracks), sizeof(track_t), alpha_compare_tracks);
