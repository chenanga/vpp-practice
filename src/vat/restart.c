...
int restart_main_fn(unformat_input_t *i)
{
    ...
    if (geteuid()) clib_error("vpp_restart: must be root...");
    ...
    pidp = svmdb_local_get_variable_reference(svmdb_client, SVMDB_NAMESPACE_VEC, "vpp_pid");
    if (pidp == 0) {
...
found2:
     ...
     */
    if (verbose) fformat(stdout, "Sanity check current vpp pid %d\n", old_pid);
    ...
    if (verbose) fformat(stdout, "Sanity check vpp pid %d OK\n", old_pid);
    ...
    if (verbose) fformat(stdout, "Old inode %u\n", old_inode);
...
new_inode:
    if (verbose && sleeps > 0) fformat(stdout, "Inode sleeps %d\n", sleeps);

    if (verbose) fformat(stdout, "New inode %u\n", statb.st_ino);
    ...
    if (pidp == 0) {
        clib_error(
            "post_restart: 'vpp_pid' svm variable not found,"
            "vpp did not restart?");
        return 2;
    }
    ...
    for (wait = 0; wait < 150; wait++) {
        if (*pidp && (*pidp != old_pid)) goto restarted;
        req->tv_sec  = 0;
        req->tv_nsec = 100000 * 1000; /* 100 ms */
        while (nanosleep(req, rem) < 0)
            *req = *rem;
...
restarted:
    ...
    if (verbose && sleeps) fformat(stdout, "pid sleeps %d\n", sleeps);

    if (verbose) fformat(stdout, "New PID %d... Restarted...\n", *pidp);
