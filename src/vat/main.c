...
int connect_to_vpe(char *name)
{
    ...
    if (vl_client_connect_to_vlib("/vpe-api", name, 32) < 0) return -1;
...
void do_one_file(vat_main_t *vam)
{
    ...
    if (setjmp(vam->jump_buf) != 0) return;
    ...
    while (1) {
        ...
        if (vam->do_exit || fgets((char *) vam->inbuf, vec_len(vam->inbuf), vam->ifp) == 0) break;

        vam->input_line_number++;

        vec_free(this_cmd);

        this_cmd = (u8 *) clib_macro_eval(&vam->macro_main, (i8 *) vam->inbuf, 1 /* complain */, 0 /* level */, 8 /* max_level */);
        ...
        if (vam->exec_mode == 0) {
            ...
            *argsp++ = 0;
            ...
            unformat_init_string(vam->input, (char *) this_cmd, strlen((char *) this_cmd));
            cmdp = this_cmd;
            fp   = exec;
        }

        rv = (*fp)(vam);
        if (rv < 0) errmsg("%s error: %U\n", cmdp, format_api_error, vam, rv);
...
static i8 *eval_current_file(clib_macro_main_t *mm, i32 complain)
{
    vat_main_t *vam = &vat_main;
    return ((i8 *) format(0, "%s%c", vam->current_file, 0));
}

static i8 *eval_current_line(clib_macro_main_t *mm, i32 complain)
{
    vat_main_t *vam = &vat_main;
    return ((i8 *) format(0, "%d%c", vam->input_line_number, 0));
}

static void signal_handler(int signum, siginfo_t *si, ucontext_t *uc)
{
    vat_main_t *vam = &vat_main;
...
static void setup_signal_handlers(void)
{
    ...
    for (i = 1; i < 32; i++) {
        ...
        if (sigaction(i, &sa, 0) < 0) clib_unix_warning("sigaction %U", format_signal, i);
    }
}

static void vat_find_plugin_path()
{
    ...
    if ((rv = readlink("/proc/self/exe", path, PATH_MAX - 1)) == -1) return;

    /* readlink doesn't provide null termination */
    path[rv] = 0;

    /* strip filename */
    if ((p = strrchr(path, '/')) == 0) return;
    *p = 0;
  ...
  if (do_sort && (error = vlib_sort_init_exit_functions (headp)))
    ...
    while (i) {
        if (call_once && !hash_get(vgm->init_functions_called, i->f)) {
            if (call_once) hash_set1(vgm->init_functions_called, i->f);
            error = i->f(vm);
...
static void vat_register_interface_dump(vat_main_t *vam)
{
    void          *handle;
    plugin_info_t *pi;
    ...
    if (!vam->api_sw_interface_dump) {
        fformat(stderr, "sw_interface_dump not found in interface_test plugin!\n");
        exit(1);
    }
}
...
int main(int argc, char **argv)
{
    ...
    while (unformat_check_input(a) != UNFORMAT_END_OF_INPUT) {
        ...
        else if (unformat(a, "plugin_name_filter %s", (u8 *) &vat_plugin_name_filter))
    ...
    if (vam->socket_name && vat_socket_connect(vam)) fformat(stderr, "WARNING: socket connection failed");

    if ((!vam->socket_client_main || vam->socket_client_main->socket_fd == 0) && connect_to_vpe("vpp_api_test") < 0) {
        svm_region_exit();
    ...
    if (!json_output) vam->api_sw_interface_dump(vam);

    /* Set up the init function hash table */
    vgm->init_functions_called = hash_create(0, 0);

    /* Execute plugin init and api_init functions */
    error = vlib_call_init_exit_functions(vm, &vgm->init_function_registrations, 1 /* call once */, 1 /* is_global*/);

    if (error) clib_error_report(error);

    error = vlib_call_init_exit_functions(vm, &vgm->api_init_function_registrations, 1 /* call_once */, 1 /* is_global */);
    ...
    if (vat_time_now(vam) > timeout) clib_warning("BUG: message reply spin-wait timeout");
