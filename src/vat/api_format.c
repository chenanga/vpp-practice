...
int vat_socket_connect(vat_main_t *vam)
{
    ...
    if ((rv = vl_socket_client_connect((char *) vam->socket_name, "vpp_api_test", 0 /* default socket rx, tx buffer */))) return rv;
...
void errmsg(char *fmt, ...)
{
    ...
    {
        if (vam->ifp != stdin)
            fformat(vam->ofp, "%s(%d): \n", vam->current_file, vam->input_line_number);
        else
            fformat(vam->ofp, "%s\n", (char *) s);
        fflush(vam->ofp);
    }
...
uword unformat_ip4_address(unformat_input_t *input, va_list *args)
{
    ...
    if (!unformat(input, "%d.%d.%d.%d", &a[0], &a[1], &a[2], &a[3])) return 0;
...
uword unformat_ethernet_address(unformat_input_t *input, va_list *args)
{
    ...
    if (!unformat(input, "%_%x:%x:%x:%x:%x:%x%_", &a[0], &a[1], &a[2], &a[3], &a[4], &a[5])) return 0;
...
uword unformat_ip6_address(unformat_input_t *input, va_list *args)
{
    ...
    while ((c = unformat_get_input(input)) != UNFORMAT_END_OF_INPUT) {
        ...
        /* Too many hex quads. */
        if (n_hex_quads >= ARRAY_LEN(hex_quads)) return 0;

        if (hex_digit < 16) {
            hex_quad = (hex_quad << 4) | hex_digit;

            /* Hex quad must fit in 16 bits. */
            if (n_hex_digits >= 4) return 0;
        ...
        if (n_colon == 2) {
            /* More than one :: ? */
            if (double_colon_index < ARRAY_LEN(hex_quads)) return 0;
    ...
    if (n_hex_digits > 0) hex_quads[n_hex_quads++] = hex_quad;
...
static uword api_unformat_sw_if_index(unformat_input_t *input, va_list *args)
{
...
uword unformat_mfib_itf_flags(unformat_input_t *input, va_list *args)
{
    ...
    {
        if (unformat(input, mfib_itf_flag_long_names[attr])) *iflags |= (1 << attr);
    }
    FOR_EACH_MFIB_ITF_ATTRIBUTE(attr)
    {
        if (unformat(input, mfib_itf_flag_names[attr])) *iflags |= (1 << attr);
    }
...
uword unformat_mfib_entry_flags(unformat_input_t *input, va_list *args)
{
    ...
    {
        if (unformat(input, mfib_flag_long_names[attr])) *eflags |= (1 << attr);
    }
    FOR_EACH_MFIB_ATTRIBUTE(attr)
    {
        if (unformat(input, mfib_flag_names[attr])) *eflags |= (1 << attr);
    }
...
u8 *format_ip6_address(u8 *s, va_list *args)
{
    ...
    for (i = 0; i < ARRAY_LEN(a->as_u16); i++) {
        ...
        n_zeros += is_zero;
        if ((!is_zero && n_zeros > max_n_zeros) || (i + 1 >= ARRAY_LEN(a->as_u16) && n_zeros > max_n_zeros)) {
...
void ip_set(ip46_address_t *dst, void *src, u8 is_ip4)
{
    if (is_ip4)
        ...
        clib_memcpy_fast(&dst->ip6, (ip6_address_t *) src, sizeof(ip6_address_t));
}
...
static void vl_api_get_first_msg_id_reply_t_handler_json(vl_api_get_first_msg_id_reply_t *mp)
{
    vat_main_t     *vam = &vat_main;
    ...
    vat_json_object_add_uint(&node, "first_msg_id", (uint) ntohs(mp->first_msg_id));
...
u8 *format_hex_bytes(u8 *s, va_list *va)
{
    ...
    if (n_bytes == 0) return s;

    for (i = 0; i < n_bytes; i++) {
        if (!short_form && (i % 32) == 0) s = format(s, "%08x: ", i);
        s = format(s, "%02x", bytes[i]);
        if (!short_form && ((i + 1) % 32) == 0 && (i + 1) < n_bytes) s = format(s, "\n%U", format_white_space, indent);
    }
...
static void vl_api_control_ping_reply_t_handler(vl_api_control_ping_reply_t *mp)
{
    ...
    if (vam->socket_client_main) vam->socket_client_main->control_pings_outstanding--;
}

static void vl_api_control_ping_reply_t_handler_json(vl_api_control_ping_reply_t *mp)
{
    vat_main_t *vam    = &vat_main;
    i32         retval = ntohl(mp->retval);
...
#define _(n)                                                         \
    static void vl_api_##n##_t_handler_json(vl_api_##n##_t *mp)      \
    {                                                                \
        vat_main_t     *vam = &vat_main;                             \
        vat_json_node_t node;                                        \
        vat_json_init_object(&node);                                 \
...
uword unformat_vlib_pci_addr(unformat_input_t *input, va_list *args)
{
    ...
    if (!unformat(input, "%x:%x:%x.%x", &x[0], &x[1], &x[2], &x[3])) return 0;
...
uword unformat_fib_path(unformat_input_t *input, va_list *args)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "%U %U", unformat_vl_api_ip4_address, &path->nh.address.ip4, api_unformat_sw_if_index, vam, &path->sw_if_index)) {
            path->proto = FIB_API_PATH_NH_PROTO_IP4;
        }
        else if (unformat(input, "%U %U", unformat_vl_api_ip6_address, &path->nh.address.ip6, api_unformat_sw_if_index, vam, &path->sw_if_index)) {
        ...
        else if (unformat(input, "%U next-hop-table %d", unformat_vl_api_ip4_address, &path->nh.address.ip4, &path->table_id)) {
            path->proto = FIB_API_PATH_NH_PROTO_IP4;
        }
        else if (unformat(input, "%U next-hop-table %d", unformat_vl_api_ip6_address, &path->nh.address.ip6, &path->table_id)) {
            path->proto = FIB_API_PATH_NH_PROTO_IP6;
        }
        else if (unformat(input, "%U", unformat_vl_api_ip4_address, &path->nh.address.ip4)) {
            /*
        ...
        else if (unformat(input, "%U", unformat_vl_api_ip6_address, &path->nh.address.ip6)) {
            /*
             * the recursive next-hops are by default in the default table
             */
            path->table_id    = 0;
...
uword api_unformat_ip_next_index(unformat_input_t *input, va_list *args)
{
    u32 *miss_next_indexp = va_arg(*args, u32 *);
    u32  next_index       = 0;
    u32  tmp;
...
uword api_unformat_acl_next_index(unformat_input_t *input, va_list *args)
{
    u32 *miss_next_indexp = va_arg(*args, u32 *);
    u32  next_index       = 0;
    u32  tmp;
...
uword unformat_ip6_match(unformat_input_t *input, va_list *args)
{
    ...
    if (src) clib_memcpy(&ip->src_address, &src_val, sizeof(ip->src_address));

    if (dst) clib_memcpy(&ip->dst_address, &dst_val, sizeof(ip->dst_address));
...
uword unformat_l2_match(unformat_input_t *input, va_list *args)
{
    ...
    if (dst) clib_memcpy(match, dst_val, 6);

    if (src) clib_memcpy(match + 6, src_val, 6);
...
uword api_unformat_classify_match(unformat_input_t *input, va_list *args)
{
    u8 **matchp          = va_arg(*args, u8 **);
    ...
    if (match || l2 || l3 || l4) {
        ...
        vec_validate_aligned(match, ((match_n_vectors + skip_n_vectors) * sizeof(u32x4)) - 1, sizeof(u32x4));
...
static int api_get_first_msg_id(vat_main_t *vam)
{
...
static int api_sock_init_shm(vat_main_t *vam)
{
#if VPP_API_TEST_BUILTIN == 0
    unformat_input_t         *i      = vam->input;
    ...
    if (!rv) vam->client_index_invalid = 1;
    return rv;
#else
    return -99;
#endif
...
static int q(vat_main_t *vam)
{
    return q_or_quit(vam);
}

static int quit(vat_main_t *vam)
{
    return q_or_quit(vam);
}
...
static int elog_save(vat_main_t *vam)
{
#if VPP_API_TEST_BUILTIN == 0
    ...
    errmsg("Saving %wd of %wd events to %s", elog_n_events_in_buffer(em), elog_buffer_capacity(em), chroot_file);
...
static int elog_setup(vat_main_t *vam)
{
#if VPP_API_TEST_BUILTIN == 0
    elog_main_t      *em      = &vam->elog_main;
    unformat_input_t *i       = vam->input;
...
static int elog_enable(vat_main_t *vam)
{
#if VPP_API_TEST_BUILTIN == 0
    elog_main_t *em = &vam->elog_main;
...
static int elog_disable(vat_main_t *vam)
{
#if VPP_API_TEST_BUILTIN == 0
    elog_main_t *em = &vam->elog_main;
...
static int statseg(vat_main_t *vam)
{
    ...
    print(vam->ofp, "vector_rate %.2f input_rate %.2f", vector_rate, input_rate);
    print(vam->ofp, "thread 0 sw_if_index 1 rx pkts %lld, bytes %lld", thread0_index1_packets, thread0_index1_bytes);
...
static int set(vat_main_t *vam)
{
    ...
    if (unformat(i, "%s", &name)) {
        ...
        vec_add1(value, 0);
        (void) clib_macro_set_value(&vam->macro_main, (char *) name, (char *) value);
    }
    else
...
static int unset(vat_main_t *vam)
{
    ...
    if (unformat(vam->input, "%s", &name))
        if (clib_macro_unset(&vam->macro_main, (char *) name) == 1) errmsg("unset: %s wasn't set", name);
    vec_free(name);
    return 0;
}
...
static int value_sort_cmp(void *a1, void *a2)
{
    name_sort_t *n1 = a1;
    name_sort_t *n2 = a2;
...
static int dump_msg_api_table(vat_main_t *vam)
{
    api_main_t  *am   = vlibapi_get_main();
...
static int get_msg_id(vat_main_t *vam)
{
    u8 *name_and_crc;
    u32 message_index;
    ...
    if (unformat(vam->input, "%s", &name_and_crc)) {
        ...
        print(vam->ofp, " '%s' has message index %d", name_and_crc, message_index);
        return 0;
    }
...
static int search_node_table(vat_main_t *vam)
{
    ...
    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "%s", &node_to_find)) {
            ...
            clib_warning("parse error '%U'", format_unformat_error, line_input);
            return -99;
        }
...
static int dump_sub_interface_table(vat_main_t *vam)
{
    ...
    vec_foreach (sub, vam->sw_if_subif_table) {
        print(vam->ofp,
              ...
              sub->sw_if_index,
              sub->sub_id,
              sub->sub_dot1ad ? "dot1ad" : "dot1q",
              sub->sub_number_of_tags,
              sub->sub_outer_vlan_id,
              sub->sub_inner_vlan_id,
              sub->sub_exact_match,
              sub->sub_default,
              sub->sub_outer_vlan_id_any,
              sub->sub_inner_vlan_id_any);
        if (sub->vtr_op != L2_VTR_DISABLED) {
            print(vam->ofp,
                  "  vlan-tag-rewrite - op: %-14s [ dot1q: %d "
                  "tag1: %d tag2: %d ]",
                  str_vtr_op(sub->vtr_op),
                  sub->vtr_push_dot1q,
                  sub->vtr_tag1,
                  sub->vtr_tag2);
        }
...
#define _(N, n)                                                \
    static void vl_api_##n##_t_handler_uni(vl_api_##n##_t *mp) \
    {                                                          \
        vat_main_t *vam = &vat_main;                           \
        if (vam->json_output) {                                \
...
#if VPP_API_TEST_BUILTIN
static clib_error_t *vat_api_hookup_shim(vlib_main_t *vm)
{
