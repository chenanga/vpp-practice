...
static clib_error_t *virtual_ip_cmd_fn_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    if (!unformat(line_input, "%U %U", unformat_ip4_address, &prefix.fp_addr.ip4, unformat_vnet_sw_interface, vnm, &sw_if_index)) {
        error = clib_error_return(0, "unknown input `%U'", format_unformat_error, line_input);
        goto done;
    }

    while (unformat_check_input(line_input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(line_input, "mac %U", unformat_ethernet_address, &mac_addr)) {
            mac_addr_t *ma;
            vec_add2(mac_addrs, ma, 1);
            clib_memcpy(ma, mac_addr, sizeof(mac_addr));
        }
        else if (unformat(line_input, "next-hop %U", unformat_ip4_address, &next_hop.ip4)) {
    ...
    for (i = 0; i < vec_len(mac_addrs); i++) {
        ...
        adj_nbr_add_or_lock_w_rewrite(FIB_PROTOCOL_IP4, VNET_LINK_IP4, &next_hops[i], sw_if_index, mac_addrs[i].mac_addr);
