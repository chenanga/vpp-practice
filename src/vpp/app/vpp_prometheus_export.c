...
static void print_metric_v1(FILE *stream, stat_segment_data_t *res)
{
    int j, k;
    ...
    switch (res->type) {
        case STAT_DIR_TYPE_COUNTER_VECTOR_SIMPLE:
            ...
            for (k = 0; k < vec_len(res->simple_counter_vec); k++)
                for (j = 0; j < vec_len(res->simple_counter_vec[k]); j++)
                    fformat(stream, "%s{thread=\"%d\",interface=\"%d\"} %lld\n", prom_string(res->name), k, j, res->simple_counter_vec[k][j]);
            break;
        ...
        case STAT_DIR_TYPE_NAME_VECTOR:
            ...
            for (k = 0; k < vec_len(res->name_vector); k++)
                if (res->name_vector[k]) fformat(stream, "%s_info{index=\"%d\",name=\"%s\"} 1\n", prom_string(res->name), k, res->name_vector[k]);
            break;
...
static void sanitize(char *str, int len)
{
    for (int i = 0; i < len; i++) {
...
static int tokenize(const char *name, char **tokens, int *lengths, int max_tokens)
{
    char *p     = (char *) name;
    char *savep = p;
...
static void print_metric_v2(FILE *stream, stat_segment_data_t *res)
{
    int   num_tokens = 0;
    ...
    switch (res->type) {
        case STAT_DIR_TYPE_COUNTER_VECTOR_SIMPLE:
            if (res->simple_counter_vec == 0) return;
            for (k = 0; k < vec_len(res->simple_counter_vec); k++)
                for (j = 0; j < vec_len(res->simple_counter_vec[k]); j++) {
                    if ((num_tokens == 4) && (!strncmp(tokens[1], "nodes", lengths[1]) || !strncmp(tokens[1], "interfaces", lengths[1]))) {
        ...
        case STAT_DIR_TYPE_COUNTER_VECTOR_COMBINED:
            if (res->combined_counter_vec == 0) return;
            for (k = 0; k < vec_len(res->combined_counter_vec); k++)
                for (j = 0; j < vec_len(res->combined_counter_vec[k]); j++) {
                    if ((num_tokens == 4) && !strncmp(tokens[1], "interfaces", lengths[1])) {
                        sanitize(tokens[1], lengths[1]);
                        sanitize(tokens[3], lengths[3]);
                        fformat(stream,
        ...
        case STAT_DIR_TYPE_SCALAR_INDEX:
            if ((num_tokens == 4) && !strncmp(tokens[1], "buffer-pools", lengths[1])) {
                sanitize(tokens[1], lengths[1]);
                sanitize(tokens[3], lengths[3]);
                fformat(stream,
                        "%.*s_%.*s{pool=\"%.*s\"} %.2f\n",
                ...
                fformat(stream, "%.*s_%.*s %.2f\n", lengths[1], tokens[1], lengths[2], tokens[2], res->scalar_value);
...
static void dump_metrics(FILE *stream, u8 **patterns, u8 v2)
{
...
static void http_handler(FILE *stream, u8 **patterns, u8 v2)
{
    char status[80] = {0};
...
int main(int argc, char **argv)
{
    ...
    char            *usage = "%s: usage [socket-name <name>] [port <0 - 65535>] [v2] <patterns> ...\n";
    int              rv;
    u8               v2 = 0;
