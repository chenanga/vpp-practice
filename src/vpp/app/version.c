...
char *vpe_version_string = "vpp v" VPP_BUILD_VER " built by " VPP_BUILD_USER " on " VPP_BUILD_HOST " at " VPP_BUILD_DATE;
...
#undef _
    ...
    if (cmdline) {
        ...
        for (i = 0; argv[i]; i++) {
            if (strstr(argv[i], "{")) indent += 2;
            vlib_cli_output(vm, "%U%s", format_white_space, indent, argv[i]);
            if (strstr(argv[i], "}")) indent -= 2;
        }
    }
    if ((verbose + cmdline) == 0) vlib_cli_output(vm, "%s", vpe_version_string);
    return 0;
}
