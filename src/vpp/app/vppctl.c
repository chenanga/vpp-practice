...
static void send_ttype(int sock_fd, int is_interactive)
{
    char       *term;
...
static int process_input(int sock_fd, unsigned char *rx_buf, int rx_buf_len, int is_interactive, int *sent_ttype)
{
    int i = 0;
    int j = 0;
...
#if DEBUG
                fprintf(stderr, "IAC at %d, IAC %s %s", i, TELCMD(rx_buf[i + 1]), TELOPT(rx_buf[i + 2]));
#endif
                i += 3;
            }
        }
...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        if (event.data.fd == STDIN_FILENO && cmd == 0) {
            ...
            if (!sent_ttype) continue; /* not ready for this yet */

            n = read(STDIN_FILENO, c, sizeof(c));
            if (n > 0) {
                int n_written = write(sock_fd, c, n);
                if (n_written < n) error = errno;
                if (error) goto done;
            }
            else if (n < 0)
            ...
            if (nread < 0) error = errno;
            if (error) break;

            if (nread == 0) break;

            int len = process_input(sock_fd, rx_buf, nread, is_interactive, &sent_ttype);
    ...
    if (is_interactive) tcsetattr(STDIN_FILENO, TCSAFLUSH, &orig_tio);
