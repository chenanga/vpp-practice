...
static void setup_signal_handlers(void)
{
    ...
    for (i = 1; i < 32; i++) {
        ...
        if (sigaction(i, &sa, 0) < 0) return clib_unix_warning("sigaction %U", format_signal, i);
    }
}
...
int main(int argc, char **argv)
{
    ...
    while (unformat_check_input(&input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(&input, "chroot %s", &chroot_path_u8)) {
            ...
            fformat(stderr, "usage: vpp_get_metrics [chroot <path>] [interval <nn>]\n");
            exit(1);
        }
    }
    ...
    vpp_pidp        = svmdb_local_get_variable_reference(c, SVMDB_NAMESPACE_VEC, "vpp_pid");
    vector_ratep    = svmdb_local_get_variable_reference(c, SVMDB_NAMESPACE_VEC, "vpp_vector_rate");
    rx_ratep        = svmdb_local_get_variable_reference(c, SVMDB_NAMESPACE_VEC, "vpp_input_rate");
    sig_error_ratep = svmdb_local_get_variable_reference(c, SVMDB_NAMESPACE_VEC, "vpp_sig_error_rate");
    ...
    if (vpp_pidp == 0 || vector_ratep == 0 || rx_ratep == 0 || sig_error_ratep == 0) {
