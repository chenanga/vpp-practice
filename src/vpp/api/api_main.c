...
clib_error_t *vat_builtin_main_init(vlib_main_t *vm)
{
    ...
    if (rv) clib_warning("vat_plugin_init returned %d", rv);
...
static void maybe_register_api_client(vat_main_t *vam)
{
    ...
    if (vam->my_client_index != ~0) return;
    ...
    *regpp = clib_mem_alloc(sizeof(vl_api_registration_t));
    ...
    vam->my_client_index = vl_msg_api_handle_from_index_and_epoch(regp->vl_api_registration_pool_index, am->shmem_hdr->application_restarts);
