...
static u64 get_significant_errors(gmon_main_t *gm)
{
    ...
    clib_bitmap_foreach (code, gm->sig_error_bitmap) {
        for (vm_index = 0; vm_index < vec_len(gm->my_vlib_mains); vm_index++) {
            ...
            significant_errors += em->counters[code] - ((vec_len(em->counters_last_clear) > code) ? em->counters_last_clear[code] : 0);
        }
    }
...
static uword gmon_process(vlib_main_t *vm, vlib_node_runtime_t *rt, vlib_frame_t *f)
{
    ...
    while (1) {
        ...
        *gm->sig_error_rate_ptr = ((f64) (new_sig_errors - gm->last_sig_errors)) / dt;
...
static clib_error_t *gmon_init(vlib_main_t *vm)
{
    ...
    if ((error = vlib_call_init_function(vm, vpe_api_init))) return (error);

    if ((error = vlib_call_init_function(vm, vlibmemory_init))) return (error);
    ...
    vec_add1(v, 0.0);
    svmdb_local_set_vec_variable(gm->svmdb_client, "vpp_vector_rate", (char *) v, sizeof(*v));
    vec_free(v);
    vec_add1(v, 0.0);
    svmdb_local_set_vec_variable(gm->svmdb_client, "vpp_input_rate", (char *) v, sizeof(*v));
    vec_free(v);
    vec_add1(v, 0.0);
    svmdb_local_set_vec_variable(gm->svmdb_client, "vpp_sig_error_rate", (char *) v, sizeof(*v));
    vec_free(v);

    vec_add1(swp, 0.0);
    svmdb_local_set_vec_variable(gm->svmdb_client, "vpp_pid", (char *) swp, sizeof(*swp));
    vec_free(swp);

    /* the value cells will never move, so acquire references to them */
    gm->vector_rate_ptr    = svmdb_local_get_variable_reference(gm->svmdb_client, SVMDB_NAMESPACE_VEC, "vpp_vector_rate");
    gm->input_rate_ptr     = svmdb_local_get_variable_reference(gm->svmdb_client, SVMDB_NAMESPACE_VEC, "vpp_input_rate");
    gm->sig_error_rate_ptr = svmdb_local_get_variable_reference(gm->svmdb_client, SVMDB_NAMESPACE_VEC, "vpp_sig_error_rate");
    gm->vpef_pid_ptr       = svmdb_local_get_variable_reference(gm->svmdb_client, SVMDB_NAMESPACE_VEC, "vpp_pid");
    return 0;
}
...
static int significant_error_enable_disable(gmon_main_t *gm, u32 index, int enable)
{
    ...
    if (index >= vec_len(em->counters)) return VNET_API_ERROR_NO_SUCH_ENTRY;
...
static clib_error_t *set_significant_error_command_fn(vlib_main_t *vm, unformat_input_t *input, vlib_cli_command_t *cmd)
{
    ...
    switch (rv) {
        ...
        default:
            return clib_error_return(0, "significant_error_enable_disable returned %d", rv);
    }
