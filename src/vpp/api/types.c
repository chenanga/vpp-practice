...
u8 *format_vl_api_version(u8 *s, va_list *args)
{
    ...
    if (ver->pre_release[0] != 0) {
        ...
        if (ver->build_metadata[0] != 0) s = format(s, "+%v", ver->build_metadata);
    }
    return s;
}
...
uword unformat_vl_api_ip4_address(unformat_input_t *input, va_list *args)
{
    ...
    if (unformat(input, "%U", unformat_ip4_address, ip)) return (1);
    return (0);
}
...
uword unformat_vl_api_ip6_address(unformat_input_t *input, va_list *args)
{
    ...
    if (unformat(input, "%U", unformat_ip6_address, ip)) return (1);
    return (0);
}
...
uword unformat_vl_api_prefix(unformat_input_t *input, va_list *args)
{
    ...
    if (unformat(input, "%U/%U", unformat_vl_api_address, &pfx->address, unformat_u8, &pfx->len)) return (1);
...
uword unformat_vl_api_mprefix(unformat_input_t *input, va_list *args)
{
    ...
    if (unformat(input, "%U/%U", unformat_vl_api_ip4_address, &pfx->grp_address.ip4, unformat_u16, &pfx->grp_address_length))
        pfx->af = ADDRESS_IP4;
    else if (unformat(input, "%U/%U", unformat_vl_api_ip6_address, &pfx->grp_address.ip6, unformat_u16, &pfx->grp_address_length))
        pfx->af = ADDRESS_IP6;
    else if (unformat(input, "%U %U", unformat_vl_api_ip4_address, &pfx->src_address.ip4, unformat_vl_api_ip4_address, &pfx->grp_address.ip4)) {
        pfx->af                 = ADDRESS_IP4;
        pfx->grp_address_length = 64;
    }
    else if (unformat(input, "%U %U", unformat_vl_api_ip6_address, &pfx->src_address.ip6, unformat_vl_api_ip6_address, &pfx->grp_address.ip6)) {
        pfx->af                 = ADDRESS_IP6;
        pfx->grp_address_length = 256;
    }
    else if (unformat(input, "%U", unformat_vl_api_ip4_address, &pfx->grp_address.ip4)) {
        pfx->af                 = ADDRESS_IP4;
        pfx->grp_address_length = 32;
        clib_memset(&pfx->src_address, 0, sizeof(pfx->src_address));
    }
    else if (unformat(input, "%U", unformat_vl_api_ip6_address, &pfx->grp_address.ip6)) {
        pfx->af                 = ADDRESS_IP6;
...
uword unformat_vl_api_version(unformat_input_t *input, va_list *args)
{
    ...
    if (unformat(input, "%u.%u.%u-%s+%s", ver->major, ver->minor, ver->patch, ver->pre_release, ver->build_metadata)
        || unformat(input, "%u.%u.%u-%s", ver->major, ver->minor, ver->patch, ver->pre_release)
