...
int vat_plugin_init(vat_main_t *vam)
{
    ...
    vat_builtin_logger = vlib_log_register_class_rate_limit("vat-plug", "load", 0x7FFFFFFF /* aka no rate limit */);

    plugin_path        = vlib_get_vat_plugin_path();
    plugin_name_filter = vlib_get_vat_plugin_name_filter();

    if (plugin_path) vat_plugin_path = (char *) plugin_path;

    if (plugin_name_filter) vat_plugin_name_filter = (char *) plugin_name_filter;

    pm->plugin_path = format(0, "%s%c", vat_plugin_path, 0);

    if (vat_plugin_name_filter) pm->plugin_name_filter = format(0, "%s%c", vat_plugin_name_filter, 0);
