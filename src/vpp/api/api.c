...
static void vl_api_show_version_t_handler(vl_api_show_version_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_SHOW_VERSION_REPLY, ({
                     ...
                     strncpy((char *) rmp->build_directory, vpe_api_get_build_directory(), ARRAY_LEN(rmp->build_directory) - 1);
                     strncpy((char *) rmp->version, vpe_api_get_version(), ARRAY_LEN(rmp->version) - 1);
                     strncpy((char *) rmp->build_date, vpe_api_get_build_date(), ARRAY_LEN(rmp->build_date) - 1);
                 }));
}
...
static void vl_api_show_vpe_system_time_t_handler(vl_api_show_vpe_system_time_t *mp)
{
    ...
    REPLY_MACRO2(VL_API_SHOW_VPE_SYSTEM_TIME_REPLY, ({ rmp->vpe_system_time = clib_host_to_net_f64(unix_time_now()); }));
}
...
static void show_log_details(vl_api_registration_t *reg, u32 context, f64 timestamp, vl_api_log_level_t *level, u8 *msg_class, u8 *message)
{
    ...
    int                   class_len   = clib_min(vec_len(msg_class) + 1, ARRAY_LEN(rmp->msg_class));
...
static clib_error_t *vpe_api_hookup(vlib_main_t *vm)
{
    ...
    vl_api_set_msg_thread_safe(am, msg_id_base + VL_API_SHOW_VPE_SYSTEM_TIME, 1);
...
static clib_error_t *api_segment_config(vlib_main_t *vm, unformat_input_t *input)
{
    ...
    while (unformat_check_input(input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(input, "prefix %s", &chroot_path)) {
        ...
        else if (unformat(input, "global-pvt-heap-size size %lld", &pvt_heap_size))
            ...
            if (rv < 0) {
                e = clib_error_return_code(0, rv, CLIB_ERROR_ERRNO_VALID | CLIB_ERROR_FATAL, "cannot fetch username %s", s);
                vec_free(s);
                vec_free(buf);
                return e;
            }
            ...
            if (rv != 0) {
                e = clib_error_return_code(0, rv, CLIB_ERROR_ERRNO_VALID | CLIB_ERROR_FATAL, "cannot fetch group %s", s);
                vec_free(s);
                vec_free(buf);
                return e;
            }
