...
static void vpp_find_plugin_path()
{
    ...
    /* strip filename */
    if ((p = strrchr((char *) path, '/')) == 0) goto done;
    *p = 0;
...
static void vpe_main_init(vlib_main_t *vm)
{
    ...
    if (!vlib_plugin_path) vpp_find_plugin_path();
}
...
#undef _
    ...
    for (i = 1; i < (argc - 1); i++) {
        if (!strncmp(argv[i], "plugin_path", 11)) {
            if (i < (argc - 1)) vlib_plugin_path = argv[++i];
        }
        if (!strncmp(argv[i], "test_plugin_path", 16)) {
            if (i < (argc - 1)) vat_plugin_path = argv[++i];
        }
...
defaulted:
    ...
    while (unformat_check_input(&input) != UNFORMAT_END_OF_INPUT) {
        if (unformat(&input, "memory %v", &v)) {
            ...
            while (unformat_check_input(&sub_input) != UNFORMAT_END_OF_INPUT) {
                if (unformat(&sub_input, "main-heap-size %U", unformat_memory_size, &main_heap_size))
                    ;
                else if (unformat(&sub_input, "main-heap-page-size %U", unformat_log2_page_size, &main_heap_log2_page_sz))
                    ;
                else if (unformat(&sub_input, "default-hugepage-size %U", unformat_log2_page_size, &default_log2_hugepage_sz))
                    ;
                else {
    ...
    if (main_core == ~0) main_core = sched_getcpu();
    ...
    if (main_core != ~0) {
        ...
        if (pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), &cpuset)) {
            clib_unix_error("pthread_setaffinity_np() on cpu %d failed for main thread", main_core);
        }
    }
    ...
    if ((main_heap = clib_mem_init_with_page_size(main_heap_size, main_heap_log2_page_sz))) {
        /* Figure out which numa runs the main thread */
        __os_numa_index = clib_get_current_numa_node();

        if (default_log2_hugepage_sz != CLIB_MEM_PAGE_SZ_UNKNOWN) clib_mem_set_log2_default_hugepage_size(default_log2_hugepage_sz);
